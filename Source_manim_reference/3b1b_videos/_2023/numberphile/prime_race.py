...
class PrimeRace(InteractiveScene):
    ...
    def construct(self):
        ...
        labels = VGroup(Tex('p \\equiv 1 \\mod 4', t2c={'1': ONE_COLOR}), Tex('p \\equiv 3 \\mod 4', t2c={'3': THREE_COLOR}))
        ...
        self.play(LaggedStartMap(FadeIn, VGroup(*blocks[10:30]), lag_ratio=0.9), MoveToTarget(frame, rate_func=rush_into), run_time=12)
        curr = 30
        tups = [(200, 10, linear, 1.25), (len(blocks) - 100, 60, linear, 1.25), (len(blocks) - 1, 5, smooth, 0.8)]
        for index, rt, func, sf in tups:
            frame.target = frame.generate_target()
            frame.target.scale(sf)
            frame.target.set_x(blocks[index].get_right()[0] + 1)
            self.play(ShowIncreasingSubsets(VGroup(*blocks[curr:index])), MoveToTarget(frame), run_time=rt, rate_func=func)
            curr = index
...
class RaceGraph(InteractiveScene):
    ...
    def construct(self):
        ...
        x_unit = 10
        axes = Axes(x_range=(0, len(primes) + 10, x_unit), y_range=self.y_range, width=len(primes) / x_unit / 1.6, height=7, axis_config=dict(tick_size=0.05))
        axes.to_edge(LEFT, buff=0.8)
        y_label = TexText('\\#Team3 $-$ \\#Team1')
        ...
        graph = VMobject()
        graph.set_points_as_corners([axes.c2p(i, diff) for i, diff in enumerate(diffs)])
        ...
        for x in range(10, 30):
            ...
            self.wait(6 / 20)
        set_x_shift(200, anims=[Restore(frame)], run_time=10, rate_func=linear)
        ...
        def set_x_squish(x1, x2, **kwargs):
            ...
            self.add(group, rect)
            self.play(UpdateFromAlphaFunc(group, lambda m, a: m.stretch(full_width / get_norm(axes.c2p(interpolate(x1, x2, a), 0) - origin), 0, about_point=origin)), UpdateFromAlphaFunc(prime_label, lambda m, a: m.set_value(primes[min(int(interpolate(x1, x2, a)), len(primes) - 1)])), **kwargs)
        ...
        self.play(VGroup(axes, graph).animate.set_width(len(primes) / 10, about_point=axes.c2p(len(primes), 0), stretch=True), FadeOut(rect), run_time=6)
        self.wait()
