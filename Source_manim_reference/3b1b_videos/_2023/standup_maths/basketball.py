...
def load_data(year):
    frame = pd.read_csv(year_to_file_name(year), usecols=['LOC_X', 'LOC_Y', 'SHOT_MADE_FLAG'])
...
class ShotHistory(InteractiveScene, ThreeDScene):
    ...
    def construct(self):
        ...
        all_dots.restore()
        all_bars.restore()
        self.play(frame.animate.reorient(-100), ShowSubmobjectsOneByOne(all_dots, rate_func=linear, int_func=np.floor), ShowSubmobjectsOneByOne(all_bars, rate_func=linear, int_func=np.floor), UpdateFromAlphaFunc(year_mob, lambda m, a: m.set_value(integer_interpolate(year_range[0], year_range[1] - 1, a)[0]), rate_func=linear), run_time=20)
        ...
        self.play(UpdateFromFunc(curr_dots, update_dots), UpdateFromFunc(curr_bars, update_bars), ChangeDecimalToValue(year_mob, 2000), self.frame.animate.reorient(-117, 69, 0).move_to([-0.54, -2.5, 0.31]).set_height(5.5), run_time=3)
        self.wait(2)
        self.play(self.frame.animate.reorient(42, 98, 0).move_to([-0.54, -2.5, 0.31]).set_height(5.5), run_time=3)
        self.wait()
        self.play(self.frame.animate.reorient(-29, 95, 0).move_to([-0.57, -2.46, 0.52]).set_height(6.1), run_time=3)
        self.play(UpdateFromFunc(curr_dots, update_dots), UpdateFromFunc(curr_bars, update_bars), ChangeDecimalToValue(year_mob, 2010), self.frame.animate.reorient(-159, 84, 0).move_to([-0.41, -2.56, 0.76]).set_height(6.73), run_time=4)
        self.play(self.frame.animate.reorient(-232, 84, 0).move_to([-0.41, -2.56, 0.76]).set_height(6.73), run_time=15)
        self.frame.reorient(128)
        self.wait()
        self.play(UpdateFromFunc(curr_dots, update_dots), UpdateFromFunc(curr_bars, update_bars), ChangeDecimalToValue(year_mob, 2020), self.frame.animate.reorient(0, 31, 0).move_to([-0.37, -2.11, 0.71]).set_height(7.26), run_time=4)
        self.wait()
        self.play(self.frame.animate.reorient(-34, 80, 0).move_to([-0.37, -2.11, 0.71]).set_height(7.26), run_time=5)
        self.play(self.frame.animate.reorient(29, 70, 0).move_to([0.43, -1.87, 0.95]).set_height(7.49), run_time=25)
        self.wait()