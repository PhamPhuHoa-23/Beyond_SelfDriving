...
class ThumbnailProgression(InteractiveScene):
    ...
    def construct(self):
        ...
        for thumbnail in thumbnails:
            ...
            cover = Group(SurroundingRectangle(blurry_image, buff=0).set_stroke(WHITE, 3), blurry_image)
        ...
        self.play(LaggedStart(*(FadeIn(covers[indices.index(n)]) for n in range(len(indices))), run_time=4, lag_ratio=0.1))
        self.wait()
        ...
        self.play(LaggedStart(*(Restore(cover, path_arc=30 * DEGREES) for cover in covers[1:])), lag_ratio=0.01, run_time=2)
        for image, name, cover in list(zip(framed_thumbnails, names, covers))[1:]:
            self.reveal(image, name, cover)
        winners = ['The Mathematics of String Art', 'Minimal Surfaces & the Calculus of Variations', 'Rethinking the real line', 'Pixel Art Anti Aliasing', 'How Computers Use Numbers']
        winner_ftns = Group(*(framed_thumbnails[names.index(winner)] for winner in winners))
        to_fade = Group(*(ftn for ftn in framed_thumbnails if ftn not in winner_ftns))
...
class GenerallyGreat(InteractiveScene):
    ...
    def construct(self):
        audience_names = ['a child', 'a middle school student', 'a high school student', 'an undergraduate in math', 'an undergraduate in engineering', "a college student who doesn't like math", 'a math Ph.D. student', 'a professional in a technical field', "a person who hasn't thought about math for a decade"]
        audiences = VGroup(*(Text(f'for {name}') for name in audience_names))
        audiences.scale(0.9)
        ...
        qualities3 = VGroup(Text('Terrible').set_color(RED), Text('Bad').set_color(RED), Text('Okay').set_color(YELLOW), Text('Good').set_color(GREEN_YELLOW), Text('Great').set_color(GREEN), Text('Bad').set_color(RED), Text('Okay').set_color(YELLOW), Text('Good').set_color(GREEN_YELLOW), Text('Bad').set_color(RED))
        ...
        annotations = VGroup(Text('Not\nPossible', font_size=90, fill_color=RED, alignment='LEFT'), Text('Rare, but\nPossible', font_size=90, fill_color=YELLOW, alignment='LEFT'), Text('', font_size=90, fill_color=GREEN, alignment='LEFT'))
        for annotation in annotations:
            annotation.to_corner(UR)
        rect = SurroundingRectangle(VGroup(qualities3[4], audiences[4]))
        rect.set_stroke(GREEN, 2)
        self.play(LaggedStartMap(FadeIn, qualities1), FadeIn(annotations[0]))
        self.wait()
        self.play(LaggedStartMap(FadeOut, qualities1, shift=LEFT), LaggedStartMap(FadeIn, qualities2, shift=LEFT), FadeOut(annotations[0], LEFT), FadeIn(annotations[1], LEFT))
        self.wait()
        self.play(LaggedStartMap(FadeOut, qualities2, shift=LEFT), LaggedStartMap(FadeIn, qualities3, shift=LEFT), FadeIn(rect), FadeOut(annotations[1], LEFT), FadeIn(annotations[2], LEFT))
        self.wait()
...
class OtherGreatOnes(InteractiveScene):
    ...
    def construct(self):
        ...
        names = ['Signed Distance Functions', 'A Study In Greyscale', 'Conservative matrix fields', 'Are Elo Systems Overrated?', 'The essence of multivariable calculus', 'The Principle of Least Action']
        thumbnail_dir = '/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/SoME3/EntryThumbnails'
        pieces = Group(*(Group(Text(name), ImageMobject(os.path.join(thumbnail_dir, name), height=4)).arrange(UP) for name in names))
...
class Credits(InteractiveScene):
    ...
    def construct(self):
        credit_words = [('Community management and logistics', 'James Schloss'), ('Web development', 'Frédéric Crozatier'), ('Guest judging, advising, and funding', 'Jane Street')]
        credits = VGroup(*(VGroup(Text(role, font_size=36, fill_color=GREY_B), Text(name, font_size=48)).arrange(DOWN) for role, name in credit_words))
