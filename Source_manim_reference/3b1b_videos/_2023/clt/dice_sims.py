...
class DiceSimulation(InteractiveScene):
    ...
    n_samples = 3000
    distribution = [1 / 6] * 6
    die_distribution_config = dict(axes_config=dict(width=3.5, height=1.5), y_range=(0, 0.5, 0.25))
    brick_height = 0.2
    ...
    def add_die_distribution(self, animate=False):
        die_dist = get_die_distribution_chart(self.distribution, **self.die_distribution_config)
        die_dist.to_corner(UL)
        die_dist.shift(0.25 * DOWN)
        axes, bars, dice = die_dist
        if animate:
            self.play(Write(axes), LaggedStartMap(GrowFromEdge, bars, edge=DOWN), Write(dice))
        self.add(die_dist)
    ...
    def add_sum_axes(self):
        ...
        axes = Axes(x_range, (0, int(math.sqrt(self.n_samples))), width=FRAME_WIDTH - 1, height=4)
        x_axis = axes.x_axis
        x_axis.add_numbers(font_size=16)
        x_axis.numbers.remove(x_axis.numbers[-1])
        x_axis.numbers.shift(0.5 * x_axis.get_unit_size() * RIGHT + 0.1 * UP)
        x_axis.center()
        x_axis.to_edge(DOWN)
        self.add(x_axis)
        self.sum_axes = axes
    ...
    def add_sample_label(self):
        label = TexText('\\# Sums = 0', font_size=36)
    ...
    def run_one_sum(self, run_time=3.0, transition_time=1.0, still_frame=False):
        ...
        if s not in self.buckets:
            self.buckets[s] = VGroup(VectorizedPoint(self.sum_axis.n2p(s + 0.5)))
        ...
        if still_frame:
            ...
            self.play(ShowIncreasingSubsets(dice, int_func=np.ceil), ShowIncreasingSubsets(tips, int_func=np.ceil), ShowSubmobjectsOneByOne(bar_highlights, remover=True), UpdateFromFunc(sum_label, lambda m: m.count.set_value(get_sum())), self.all_bricks.animate.set_fill(color=self.full_dist_color), run_time=run_time)
            self.wait(transition_time)
            self.play(FadeInFromPoint(brick, count_copy.get_center()), MoveToTarget(count_copy), FadeOut(sum_label, lag_ratio=0.1), FadeOut(dice, lag_ratio=0.1), FadeOut(tips, lag_ratio=0.1), run_time=transition_time)
            self.sample_label.count.increment_value()
            self.wait(transition_time)
    ...
    def get_brick(self):
        return Rectangle(stroke_width=self.brick_height * 5, stroke_color=BLACK, fill_color=self.initial_brick_color, fill_opacity=1, height=self.brick_height, width=0.8 * self.sum_axis.get_unit_size())
