...
EXP_DISTRIBUTION = [0.41, 0.25, 0.15, 0.1, 0.06, 0.03]
...
class ChartBars(VGroup):
    ...
    def __init__(self, axes, values: Sequence[float | int], xs: Sequence[float] | None=None, width_ratio: float=1.0, offset: float=0.5, fill_color: ManimColor=BLUE, fill_opacity: float=0.5, stroke_color: ManimColor=WHITE, stroke_width: float=1.0):
        ...
        for x, y in zip(xs, values):
            rect = Rectangle(width=width, height=max(y * y_unit, epsilon), fill_color=fill_color, fill_opacity=fill_opacity, stroke_color=stroke_color, stroke_width=stroke_width)
    ...
    def set_values(self, values: Iterable[float | int]):
        ...
        for rect, value in zip(self, values):
            rect.set_height(y_unit * value, stretch=True, about_edge=DOWN)

def get_die_distribution_chart(dist: list[float], die_config=dict(), axes_config=dict(width=4, height=3), font_size=25, y_range=(0, 1, 0.2), max_value: int=6, bar_colors=(BLUE, TEAL)):
...
class RandomDieRolls(InteractiveScene):
    ...
    def construct(self):
        ...
        faces = VGroup(*(DieFace(n, fill_color=BLUE_E, dot_color=WHITE) for n in range(1, 7)))
        for x in range(n_runs):
            random_faces = VGroup(*(random.choice(faces).copy() for x in range(n_movements_per_run)))
...
class DiceSumDistributions(InteractiveScene):
    ...
    def construct(self):
        plots = VGroup(*(self.get_sum_distribution_plot(n, y_max=0.2 if n > 1 else 0.5) for n in range(1, self.n_examples + 1)))
        plots.scale(0.9)
        plots.arrange(DOWN, buff=1.0, aligned_edge=LEFT)
        plots.to_corner(UL)
        labels = VGroup(*(self.get_plot_label(plot, n, die_face=True) for plot, n in zip(plots, it.count(1))))
        plot1 = plots[0]
        plot1.save_state()
        plot1.set_height(4)
        plot1.center()
        ...
        axes, bars = plot1
        prob_labels = Tex('1 / 6').replicate(6)
        ...
        self.play(MoveToTarget(plot1), LaggedStart(*(TransformFromCopy(VGroup(dice[n]), die_groups[n]) for n in range(6)), lag_ratio=0.1), Write(grid), run_time=3)
        self.wait()
        for square in grid:
        ...
        for column in columns:
            ...
            self.play(Transform(column, ct, run_time=1, lag_ratio=0.001), Restore(axes))
        ...
        highlights = [VGroup(columns[n - 2], prob_labels[n - 2]).copy() for n in [6, 10]]
        last = VMobject()
        for highlight in highlights:
            self.play(FadeOut(last), FadeIn(highlight), labelled_bars.animate.set_opacity(0.075))
            last = highlight
            self.wait()
        self.play(FadeOut(last), Restore(labelled_bars))
        ...
        self.play(Restore(plot2), MoveToTarget(columns, remover=True), MoveToTarget(prob_labels), FadeIn(labels[:2], lag_ratio=0.2))
        ...
        for n in range(1, len(plots) - 1):
            ...
            frame.target.set_width(max(FRAME_WIDTH, min_width), about_edge=LEFT)
            self.play(LaggedStart(MoveToTarget(frame, run_time=2), FadeTransform(plots[n].copy().set_opacity(0), plots[n + 1], run_time=2), TransformMatchingShapes(labels[n].copy(), labels[n + 1]), lag_ratio=0.2))
            self.wait()
        ...
        plot_groups = VGroup(*(VGroup(plot, label) for plot, label in zip(plots, labels)))
        ...
        for n in range(6):
            ...
            self.play(LaggedStart(MoveToTarget(frame), TransformFromCopy(*mean_labels[n:n + 2]), lag_ratio=0.5))
            self.wait()
        ...
        for label in sd_labels:
            ...
            label.stretch(0, 0)
            label.set_opacity(0)
        self.play(LaggedStartMap(Restore, sd_labels), LaggedStart(*(FadeOut(label[1]) for label in mean_labels)), run_time=3)
        self.wait()
        self.play(*(FadeOut(label[2]) for label in sd_labels))
        self.wait()
        ...
        axes = Axes((-3, 3), (0, 1, 0.25), width=0.45 * frame.get_width(), height=0.18 * frame.get_height())
        ...
        graph_label = Tex('e^{-x^2}')
        graph_label.set_height(0.5 * graph.get_height())
        graph_label.move_to(graph, UL)
        self.play(labels.animate.shift(1.5 * RIGHT), all_bars.animate.shift(4.5 * RIGHT), FadeOut(mean_lines, 4.5 * RIGHT), FadeOut(sd_lines, 4.5 * RIGHT), FadeIn(axes), ShowCreation(graph, run_time=2), Write(graph_label))
        self.wait()
        alt_graph_label = Tex('{1 \\over \\sigma \\sqrt{2 \\pi}} e^{-x^2 / 2\\sigma^2}')
        alt_graph_label['{1 \\over \\sigma \\sqrt{2 \\pi}}'].scale(0.75, about_edge=RIGHT)
        ...
        alt_graph = axes.get_graph(lambda x: 1 / sigma / math.sqrt(2 * PI) * math.exp(-x ** 2 / 2 * sigma ** 2))
        alt_graph.match_style(graph)
        self.play(Transform(graph, alt_graph), TransformMatchingTex(graph_label, alt_graph_label), run_time=2)
        ...
        self.play(MoveToTarget(axes), graph.animate.stretch(sf, 1, about_edge=DOWN))
        self.wait()
        graph_copies = VGroup(*(graph.copy().replace(bars[n:], dim_to_match=1).set_stroke(width=1.5) for n, bars in zip(it.count(1), all_bars[1:])))
        self.play(LaggedStart(*(TransformFromCopy(graph, graph_copy) for graph_copy in graph_copies)), run_time=3)
        self.wait()
    ...
    def get_plot_label(self, plot, n, die_face=False, height=0.35):
        ...
        if n > 5:
            terms.replace_submobject(2, Tex('\\cdots'))
            if not die_face:
    ...
    def get_sum_distribution_plot(self, n, **kwargs):
        return self.get_distribution_plot(self.get_sum_distribution(n), **kwargs)

    def get_distribution_plot(self, distribution, x_unit=0.5, height=1.5, bar_colors=(BLUE, TEAL), y_max=None):
        n = len(distribution)
        ...
        axes = Axes((0, n), (0, y_max, y_step), width=n * x_unit, height=height)
        axes.set_stroke(width=1)
        axes.center()
        axes.y_axis.add_numbers(num_decimal_places=len(str(y_step)) - 2, font_size=16)
        axes.x_axis.add_numbers(range(1, n + 1), num_decimal_places=0, font_size=int(axes.x_axis.get_unit_size() * 40), buff=0.1)
    ...
    def get_dice_axis_labels(self, plot):
        ...
        die_faces = VGroup(*(DieFace(n + 1, fill_color=BLUE_E, dot_color=WHITE).replace(number, dim_to_match=1) for n, number in enumerate(numbers)))
    ...
    def get_mean_labels(self, plots, color=PINK, num_decimal_places=1):
        ...
        for n, plot in zip(it.count(1), plots):
            axes = plot[0]
            label = Tex(f'{n}\\mu = {np.round(n * mu, num_decimal_places)}')
            if n == 1:
                label[0].scale(0, about_edge=RIGHT)
            v_line = Line(*axes.y_axis.get_start_and_end())
            v_line.move_to(axes.c2p(n * mu - 0.5, 0), DOWN)
            label.shift(v_line.get_top() + MED_SMALL_BUFF * UP - label[:2].get_bottom())
    ...
    def get_sd_labels(self, plots, color=RED):
        ...
        for n, plot in zip(it.count(1), plots):
            ...
            arrows = VGroup(FillArrow(v_lines.get_center(), v_lines[0].get_center()), FillArrow(v_lines.get_center(), v_lines[1].get_center()))
            for arrow in arrows:
                arrow.scale(0.75)
            arrows.move_to(v_lines)
            if n == 1:
                sigma_label = Tex(f'\\sigma')
            else:
                sigma_label = Tex(f'\\sqrt{n} \\cdot \\sigma')
            sigma_label.scale(0.8)
            sigma_label.next_to(arrows[1], UP, SMALL_BUFF)
            sd_labels.add(VGroup(v_lines, arrows, sigma_label))
    ...
    def realign_distributions(self, all_bars, labels, mean_lines, sd_lines):
        ...
        for bars, mean_line, sd_line in zip(all_bars, mean_lines, sd_lines):
            ...
            bar_group.target.shift((frame.get_center()[0] - mean_line.get_x()) * RIGHT)
    ...
    def get_rescaling_arrows(self, bars, n):
        ...
        arrows = VGroup(*(FillArrow(ORIGIN, length * vect, **kw).move_to(center - 3.0 * sigma * bar_width * vect, -vect) for vect in [LEFT, RIGHT]))
        labels = Tex(f'\\sigma \\sqrt{{{n}}}').replicate(2)
        for arrow, label in zip(arrows, labels):
            label.next_to(arrow, UP, buff=0)
            label.set_max_width(arrow.get_width())
            label['\\sigma'].set_color(RED)
            arrow.add(label)
        return arrows
    ...
    def show_sample_sum(self, plot, distribution, n, low_plot=None):
        ...
        bar_highlights = VGroup(*(bars[sample - 1].copy() for sample in samples))
        ...
        self.play(ShowIncreasingSubsets(tips, **kw), ShowSubmobjectsOneByOne(bar_highlights, remover=True, **kw), ShowIncreasingSubsets(sum_expr[0:-1:2], **kw), ShowIncreasingSubsets(sum_expr[1::2], **kw), run_time=0.5)
        self.wait(0.1)
        self.add(sum_expr)
...
class TransitionToSkewDistribution(DiceSumDistributions):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, prob_labels, shift=UP), LaggedStartMap(Restore, plot1[1]))
        self.wait()
        self.play(ReplacementTransform(plot1, plot2, run_time=2), LaggedStart(*(FadeTransform(*labels) for labels in zip(prob_labels, exp_prob_labels)), lag_ratio=0.05, run_time=2))
        self.wait()
        self.play(VGroup(plot2, exp_prob_labels, dice).animate.set_height(2).to_corner(UL))
        self.wait()
        axes.add(dice, prob_labels)
...
class ExpDistSumDistributions(DiceSumDistributions):
    ...
    def construct(self):
        distributions = [[1 / 6] * 6, list(self.distribution)]
        plots_list = VGroup()
        for dist in distributions:
            self.distribution = dist
            plots = VGroup(*(self.get_sum_distribution_plot(n, y_max=0.2 if n > 1 else 0.4) for n in range(1, self.n_examples + 1)))
            plots.scale(0.9)
            plots.arrange(DOWN, buff=1.0, aligned_edge=LEFT)
            plots.to_corner(UL)
            plots_list.add(plots)
        plots = plots_list[0]
        labels, die_labels = [VGroup(*(self.get_plot_label(plot, n, die_face) for plot, n in zip(plots, it.count(1)))) for die_face in [False, True]]
        self.add(plots)
        self.add(die_labels)
        self.play(Transform(plots, plots_list[1], run_time=3, lag_ratio=0.001), die_labels[0].animate.shift(0.25 * RIGHT))
        self.wait()
        self.play(FadeOut(plots[2:]), FadeOut(die_labels[2:]))
        for _ in range(10):
        ...
        self.play(FadeIn(plots[2], DOWN), FadeIn(die_labels[2], DOWN), plots[1].animate.set_opacity(0.2), die_labels[1].animate.set_opacity(0.2))
        for _ in range(10):
        ...
        self.play(LaggedStartMap(FadeOut, die_labels, shift=UP, lag_ratio=0.25), LaggedStartMap(FadeIn, labels, shift=UP, lag_ratio=0.25))
        self.wait()
        frame = self.frame
        self.add(plots)
        self.play(frame.animate.set_height(plots.get_height() + 2).move_to(plots, LEFT).shift(LEFT), run_time=4)
        self.wait()
        mu, sigma = self.get_mean_and_standard_deviation(self.distribution)
        ...
        all_axes = VGroup(*(plot[0] for plot in plots))
        arrows = VGroup(*(FillArrow(axes.c2p(0, 0), axes.c2p(n * mu, 0)).move_to(axes.y_axis.pfp(0.75), LEFT).scale(0.7) for n, axes in zip(it.count(1), all_axes[1:])))
        arrows.set_color(PINK)
        self.play(LaggedStartMap(GrowArrow, arrows, lag_ratio=0.2), LaggedStartMap(FadeIn, mean_lines, shift=RIGHT, lag_ratio=0.2))
        self.wait()
        for line in sd_lines:
            line.save_state()
            line.stretch(0, 0)
            line.set_opacity(0)
        self.play(FadeOut(arrows), LaggedStartMap(Restore, sd_lines), LaggedStartMap(FadeIn, sd_arrows))
        self.wait()
        frame.target = frame.generate_target()
        ...
        self.play(MoveToTarget(frame), FadeOut(sd_lines, lag_ratio=0.1), FadeOut(sd_arrows, lag_ratio=0.1), run_time=5)
        ...
        for n in range(6):
            ...
            frame.target.set_height(plots[:n + 2].get_height() + 2)
            frame.target.move_to(plots[n + 1], DL).shift(0.75 * DL)
            self.play(LaggedStart(MoveToTarget(frame), TransformFromCopy(*mu_labels[n:n + 2]), lag_ratio=0.5))
            self.wait()
        ...
        self.play(LaggedStartMap(FadeIn, sd_lines, scale=2), LaggedStartMap(FadeIn, sd_arrows, scale=2), FadeOut(mu_labels, lag_ratio=0.01))
        self.play(frame.animate.become(frame_around_plot0).set_anim_args(run_time=5), Write(sigma_labels[0]))
        self.wait()
        for n in range(6):
            frame.target = frame.generate_target()
            frame.target.set_height(plots[:n + 2].get_height() + 2)
            frame.target.move_to(plots[n + 1], DL).shift(0.75 * DL)
            self.play(LaggedStart(MoveToTarget(frame), TransformFromCopy(*sigma_labels[n:n + 2]), lag_ratio=0.5))
            self.wait()
        self.play(FadeOut(sigma_labels, lag_ratio=0.1), FadeOut(sd_arrows, lag_ratio=0.1))
        self.add(mean_lines, sd_lines)
        all_axes = VGroup(*(plot[0] for plot in plots))
        all_bars = VGroup(*(plot[1] for plot in plots))
        ...
        self.play(*(VShowPassingFlash(line, run_time=2, time_width=2.0) for line in big_sd_lines))
        self.wait()
        self.play(FadeOut(mean_lines), FadeOut(sd_lines), lag_ratio=0.1)
        axes = Axes((-5, 5), (0, 1, 0.25), width=0.45 * frame.get_width(), height=0.5 * frame.get_height())
        axes.next_to(frame.get_left(), RIGHT, buff=1)
        ...
        graph.set_stroke(YELLOW, 3)
        graph_label = Tex('{1 \\over \\sqrt{2\\pi}} e^{-{1 \\over 2}x^2}')
        ...
        bar_shift = 5 * RIGHT
        self.play(LaggedStart(FadeIn(axes), all_bars.animate.shift(bar_shift), ShowCreation(graph), Write(graph_label), lag_ratio=0.25))
        self.wait()
        self.play(Write(words), ShowCreation(box))
        self.wait()
        ...
        self.play(LaggedStart(*(TransformFromCopy(graph, curve) for curve in target_curves)))
        self.wait()
...
class MeanAndStandardDeviation(InteractiveScene):
    ...
    def construct(self):
        ...
        dist /= dist.sum()
        original_dist = dist
        die_config = dict(fill_color=BLUE_E, dot_color=WHITE)
        chart = get_die_distribution_chart(dist, y_range=(0, 0.5, 0.1), die_config=die_config)
        chart.to_edge(LEFT)
        self.add(*chart)
        ...
        tex_kw = dict(t2c={'\\mu': PINK, '\\sigma': RED}, font_size=48)
        ...
        mean_arrow.add_updater(lambda m: m.put_start_and_end_on(mean_name.get_bottom(), mu_label.get_corner(UL)).scale(0.7))
        self.play(ShowCreation(mean_line), VFadeIn(mu_label))
        self.wait()
        self.play(ShowCreation(mean_arrow), FadeIn(mean_name, 0.5 * UL))
        dists = [EXP_DISTRIBUTION[::-1], EXP_DISTRIBUTION, U_SHAPED_DISTRIBUTION, original_dist]
        for dist in dists:
            set_dist(dist, run_time=2)
            self.wait()
        mean_eq = Tex('\\mu = E[X] = \\sum_{x} P(X = x) \\cdot x =', **tex_kw)
        mean_eq['\\mu'].scale(1.2, about_edge=RIGHT)
        rhs = VGroup()
        for n in range(1, 7):
            term = Tex(f'P(\\square) \\cdot {n} \\, + ', **tex_kw)
            die = DieFace(n, **die_config)
            die.replace(term['\\square'])
        ...
        for highlight, die, part in zip(highlights, chart.dice, rhs):
            self.play(ShowCreation(highlight), mean_eq[-1].animate.set_opacity(1))
            self.play(TransformMatchingShapes(die.copy(), part), FadeOut(highlight), run_time=1)
        for dist in dists:
            set_dist(dist, run_time=2)
            self.wait()
        ...
        self.play(MoveToTarget(mu_label), LaggedStart(FadeOut(mean_name), FadeOut(mean_arrow), FadeOut(rhs), FadeOut(mean_eq[-1])))
        mu_label.remove(*mu_label[1:])
        sd_lines = mean_line.copy().replicate(2)
        sd_lines.set_stroke(RED, 3)
        sd_lines.clear_updaters()
        sd_lines[0].add_updater(lambda m: m.move_to(axes.c2p(get_mu() - get_sigma() - 0.5, 0), DOWN))
        sd_lines[1].add_updater(lambda m: m.move_to(axes.c2p(get_mu() + get_sigma() - 0.5, 0), DOWN))
        ...
        self.play(Restore(sd_lines), ShowCreation(sd_arrows, lag_ratio=0))
        self.wait()
        var_dists = [np.array([1, 3, 12, 4, 2, 1], dtype='float'), np.array([10, 2, 1, 1, 2, 10], dtype='float')]
        for dist in var_dists:
            dist /= dist.sum()
            set_dist(dist, run_time=3)
            self.wait()
        var_eq = VGroup(Tex('\\text{Var}(X) &= E[(X - \\mu)^2]', **tex_kw), Tex('= \\sum_x P(X = x) \\cdot (x - \\mu)^2', **tex_kw))
        ...
        self.play(FadeIn(var_eq[0], DOWN), mean_eq.animate.set_opacity(0.5))
        self.play(Write(variance_name), ShowCreation(variance_arrow))
        self.wait()
        ...
        labels = VGroup(*(Tex(f'P({n}) \\cdot ({n} - \\mu)^2', **tex_kw) for n in range(1, 7)))
        labels.scale(0.5)
        ...
        part1 = label['P(1) \\cdot']
        part2 = label['(1 - \\mu)^2']
        part2.save_state()
        part2.match_x(squares[0])
        self.play(FadeOut(sd_group), FadeIn(square, lag_ratio=0.8), FadeIn(part2))
        self.wait()
        self.play(square[1].animate.set_fill(opacity=0.5), FadeIn(part1), Restore(part2))
        self.add(label)
        ...
        self.play(FadeOut(last_group), FadeIn(square), FadeIn(label))
        self.wait()
        ...
        self.wait()
        sd_equation = Tex('\\sigma = \\sqrt{\\text{Var}(X)}', **tex_kw)
        ...
        sd_arrow = Arrow(sd_name, sd_equation['\\sigma'], buff=0.1)
        sd_arrow.match_color(sd_name)
        sigma_labels = Tex('\\sigma', **tex_kw).replicate(2)
        for label, arrow in zip(sigma_labels, sd_arrows):
            label.arrow = arrow
            label.add_updater(lambda m: m.next_to(m.arrow, UP))
        self.play(FadeTransformPieces(variance_name, sd_name), ReplacementTransform(variance_arrow, sd_arrow), Write(sd_equation[:-len('Var(X)')]), TransformFromCopy(var_eq[0]['\\text{Var}(X)'], sd_equation['\\text{Var}(X)']))
        self.wait()
        self.play(*(TransformFromCopy(sd_equation['\\sigma'][0], label) for label in sigma_labels))
        self.wait()
...
class BuildUpGaussian(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(axes)
        self.add(bell_graph)
        kw = dict(tex_to_color_map={'\\mu': PINK, '\\sigma': RED}, font_size=72)
        formulas = VGroup(*(Tex(tex, **kw) for tex in ['e^x', 'e^{-x}', 'e^{-x^2}', 'e^{-{1 \\over 2} x^2}', '{1 \\over \\sqrt{2\\pi}} e^{-{1 \\over 2} x^2}', '{1 \\over \\sigma \\sqrt{2\\pi}} e^{-{1 \\over 2} \\left({x \\over \\sigma}\\right)^2}', '{1 \\over \\sigma \\sqrt{2\\pi}} e^{-{1 \\over 2} \\left({x - \\mu \\over \\sigma}\\right)^2}']))
        ...
        self.wait()
        self.play(FadeOut(axes, 2 * DOWN), FadeOut(bell_graph, DOWN), VFadeIn(morty), morty.change('raise_right_hand'), formulas[-1].animate.next_to(morty.get_corner(UL), UP))
        ...
        self.play(formulas[0].animate.center().to_edge(UP), FadeIn(axes, 3 * UP), FadeOut(morty, DOWN))
        self.wait()
        ...
        def decay_anim(graph):
            ...
            return AnimationGroup(MoveAlongPath(dot, sub_graph), UpdateFromAlphaFunc(VectorizedPoint(), lambda m, a: dot.set_radius(0.2 * clip(10 * there_and_back(a), 0, 1)), remover=True), run_time=1.5, remover=True)
        graph = get_graph(np.exp)
        self.play(ShowCreation(graph))
        self.wait()
        self.play(LaggedStart(TransformMatchingTex(formulas[0], formulas[1]), graph.animate.stretch(-1, 0).set_anim_args(run_time=2), lag_ratio=0.5))
        graph.become(get_graph(lambda x: np.exp(-x)))
        self.play(LaggedStart(*(decay_anim(graph) for x in range(8)), lag_ratio=0.15, run_time=4))
        abs_formula = Tex('e^{-|x|}', **kw)
        abs_formula.to_edge(UP)
        abs_graph = get_graph(lambda x: np.exp(-np.abs(x)))
        abs_graph.make_jagged()
        smooth_graph = get_graph(lambda x: np.exp(-x ** 2))
        self.play(TransformMatchingTex(formulas[1], abs_formula), Transform(graph, abs_graph))
        self.wait()
        self.play(Flash(axes.c2p(0, 1)))
        self.wait()
        self.play(TransformMatchingTex(abs_formula, formulas[2]), Transform(graph, smooth_graph))
        self.wait()
        ...
        axes.bind_graph_to_func(graph, lambda x: np.exp(-get_c() * np.log(get_base()) * x ** 2))
        self.play(TransformMatchingTex(formulas[2], form_with_const), FadeIn(c_display))
        self.wait()
        for value in [5.0, 0.2, 1.0]:
            self.play(c_tracker.animate.set_value(value), run_time=2)
            self.wait()
        rhs = Tex('= \\left(e^c\\right)^{-x^2}', **kw)
        rhs['c'].set_color(RED)
        rhs.next_to(form_with_const)
        self.play(Write(rhs.shift(2 * LEFT)), form_with_const.animate.shift(2 * LEFT))
        self.wait()
        alt_base_form = Tex('=(2.718)^{-x^2}', **kw)
        ...
        base.add_updater(lambda m: m.set_value(np.exp(get_c() * np.log(get_base()))))
        base.add_updater(lambda m: m.set_width(base_width))
        self.play(FadeIn(alt_base_form, RIGHT))
        self.wait()
        for value in [2.2, 0.1, 1.0]:
            self.play(c_tracker.animate.set_value(value), run_time=2)
            self.wait()
        es = VGroup(form_with_const[0], rhs[2])
        pis, twos, threes = [VGroup(*(Tex(tex, **kw).move_to(e, DR) for e in es)) for tex in ['\\pi', '2', '3']]
        ...
        self.play(FlashAround(es[0], run_time=2), FadeIn(note), GrowArrow(arrow))
        self.wait()
        last_mobs = es
        for mobs, value in [(pis, PI), (twos, 2), (threes, 3), (es, math.exp(1))]:
            base.suspend_updating()
            self.play(FadeOut(last_mobs, 0.25 * UP, lag_ratio=0.2), FadeIn(mobs, 0.25 * UP, lag_ratio=0.2), c_tracker.animate.set_value(1 / math.log(value)), base_tracker.animate.set_value(value))
            ...
            for c in [3, 0.5, 1]:
                ...
                self.wait()
            last_mobs = mobs
        self.play(form_with_const.animate.set_x(0), FadeOut(rhs, RIGHT), FadeOut(alt_base_form, RIGHT), FadeOut(note, RIGHT), FadeOut(arrow, RIGHT))
        self.play(FlashAround(form_with_const['c'], color=RED))
        form_with_sigma = Tex('e^{-{1 \\over 2} \\left({x / \\sigma}\\right)^2}', **kw)
        form_with_sigma.to_edge(UP)
        sigma_display, sigma_tracker = self.get_variable_display('\\sigma', RED, (0, 3))
        sigma_tracker.set_value(math.sqrt(0.5))
        get_sigma = sigma_tracker.get_value
        sigma_display.update()
        graph.clear_updaters()
        axes.bind_graph_to_func(graph, lambda x: np.exp(-0.5 * (x / get_sigma()) ** 2))
        graph.update()
        self.play(TransformMatchingTex(form_with_const, form_with_sigma), FadeOut(c_display, UP), FadeIn(sigma_display, UP), run_time=1)
        self.wait()
        ...
        arrows.add_updater(lambda m: m.set_width(max(0.9 * v_lines.get_width(), 1e-05)))
        arrows.add_updater(lambda m: m.move_to(axes.c2p(0, 0.575 * axes.y_axis.x_max)))
        sigma_labels = Tex('\\sigma').replicate(2)
        ...
        self.play(Restore(v_lines), ShowCreation(arrows, lag_ratio=0), FadeIn(sigma_labels))
        ...
        sd_group.save_state()
        self.play(FadeOut(sd_group), FadeOut(sigma_display), TransformMatchingTex(form_with_sigma.copy(), simple_form))
        ...
        def update_area(area):
            ...
            area.pointwise_become_partial(graph, (a - x_min) / (x_max - x_min), (b - x_min) / (x_max - x_min))
        ...
        prob_arrow.add_updater(lambda m: m.put_start_and_end_on(prob_label.get_bottom() + SMALL_BUFF * DOWN, area.get_center()))
        self.add(area, graph)
        self.play(FadeIn(area), Write(note, run_time=1), ShowCreation(note_arrow))
        self.wait()
        self.play(Write(prob_label), ShowCreation(prob_arrow), ab_tracker.animate.set_value([-1, 0]), FadeOut(note), FadeOut(note_arrow))
        self.wait()
        for value in [(-0.5, 0.5), (-1.5, -0.5), (0, 1), (-2, 1), (-4, 4)]:
            self.play(ab_tracker.animate.set_value(value), run_time=3)
            self.wait()
        self.play(FadeOut(prob_label), FadeOut(prob_arrow), FadeIn(note), FadeIn(note_arrow))
        self.wait()
        self.remove(area)
        graph.set_fill(area.get_fill_color(), area.get_fill_opacity())
        area_label = TexText('Area = $\\sqrt{\\pi}$', **kw)
        area_label.next_to(note_arrow.get_start(), UR, SMALL_BUFF)
        area_label.set_backstroke(width=5)
        graph.clear_updaters()
        normalized_form = Tex('{1 \\over \\sqrt{\\pi}} e^{-x^2}', **kw)
        normalized_form['{1 \\over \\sqrt{\\pi}}'].scale(0.8, about_edge=RIGHT)
        normalized_form.to_edge(UP, buff=MED_SMALL_BUFF)
        one = Tex('1', **kw)
        one.move_to(area_label['\\sqrt{\\pi}'], LEFT)
        one.align_to(area_label[0], DOWN)
        self.play(FadeOut(note, 0.5 * UP), FadeIn(area_label, 0.5 * UP))
        self.wait()
        self.play(TransformMatchingTex(simple_form, normalized_form, run_time=1), FadeTransform(area_label['\\sqrt{\\pi}'], normalized_form['\\sqrt{\\pi}']), graph.animate.stretch(1 / math.sqrt(PI), 1, about_edge=DOWN), note_arrow.animate.put_start_and_end_on(note_arrow.get_start(), note_arrow.get_end() + 0.5 * DOWN))
        self.play(Write(one))
        self.play(FlashAround(one))
        self.wait()
        sigma_forms = [Tex('{1 \\over \\sqrt{\\pi}} e^{-{1 \\over 2}(x / \\sigma)^2}', **kw), Tex('{1 \\over \\sigma \\sqrt{2}} {1 \\over \\sqrt{\\pi}} e^{-{1 \\over 2}(x / \\sigma)^2}', **kw), Tex('{1 \\over \\sigma \\sqrt{2\\pi}} e^{-{1 \\over 2}(x / \\sigma)^2}', **kw)]
        for form in sigma_forms:
            e = form['e^'][0][0]
            form[:form.submobjects.index(e)].scale(0.7, about_edge=RIGHT)
            form.to_edge(UP, buff=MED_SMALL_BUFF)
        sqrt2_sigma = Tex('\\sigma \\sqrt{2}', **kw)
        sqrt2_sigma.move_to(one, LEFT)
        sigma_display.update()
        sd_group.restore().update()
        self.play(TransformMatchingTex(normalized_form, sigma_forms[0]), FadeIn(sigma_display), FadeIn(sd_group))
        self.play(FadeOut(one, UP), FadeIn(sqrt2_sigma, UP), sigma_tracker.animate.set_value(1), graph.animate.stretch(math.sqrt(2), 0))
        self.wait()
        self.play(TransformMatchingTex(*sigma_forms[:2], run_time=1), FadeTransform(sqrt2_sigma, sigma_forms[1]['\\sigma \\sqrt{2}'][0]), graph.animate.stretch(1 / math.sqrt(2) / get_sigma(), 1, about_edge=DOWN), FadeIn(one))
        self.wait()
        self.play(TransformMatchingTex(*sigma_forms[1:3]))
        self.play(FlashAround(sigma_forms[2]['{1 \\over \\sigma \\sqrt{2\\pi}}'], run_time=3))
        self.wait()
        self.play(FlashAround(sigma_forms[2], run_time=3, time_width=1.0))
        axes.bind_graph_to_func(graph, lambda x: gauss_func(x, 0, get_sigma()))
        for value in [0.2, 0.5, math.sqrt(0.5)]:
            self.play(sigma_tracker.animate.set_value(value), run_time=3)
            self.wait()
        area_label['\\sqrt{\\pi}'].set_opacity(0)
        self.play(LaggedStartMap(FadeOut, VGroup(area_label, one, note_arrow)))
        self.wait()
        curr_form = sigma_forms[2].copy()
        self.remove(*sigma_forms)
        self.add(curr_form)
        standard_form = Tex('{1 \\over \\sqrt{2\\pi}} e^{-{1 \\over 2} x^2}', **kw)
        standard_form['{1 \\over \\sqrt{2\\pi}}'].scale(0.7, about_edge=RIGHT)
        ...
        self.play(FadeOut(sigma_labels, 0.1 * UP, lag_ratio=0.1), FadeIn(one_labels, 0.1 * UP, lag_ratio=0.1))
        self.wait()
        self.play(ShowCreation(rect), Write(std_words))
        self.wait()
        self.play(FadeOut(rect), FadeOut(std_words), FadeOut(one_labels, 0.2 * DOWN), FadeIn(sigma_labels, 0.2 * DOWN), TransformMatchingTex(standard_form, sigma_forms[2]))
        ...
        mu_display, mu_tracker = self.get_variable_display('\\mu', PINK, (-2, 2))
        ...
        self.add(mu_label)
        self.play(TransformMatchingTex(sigma_forms[2], final_form, matched_pairs=[(sigma_forms[2]['/ \\sigma'], final_form['\\over \\sigma'][1])], match_animation=FadeTransform, mismatch_animation=FadeTransform), ShowCreation(mu_line), FadeIn(mu_display))
        self.wait()
        for value in [2, -2, 1]:
        ...
        ms_mobs = VGroup(mu_display, sigma_display, mu_line, mu_label, sd_group)
        self.remove(ms_mobs)
        self.wait()
        self.play(LaggedStartMap(FadeIn, ms_mobs, lag_ratio=0.2))
        pairs = [(1, 1.5), (1, 1.0), (1.5, 1.0), (1, 0.7), (0, 1)]
        for mu, sigma in pairs:
            self.play(mu_tracker.animate.set_value(mu), sigma_tracker.animate.set_value(sigma), run_time=3)
            self.wait()
    ...
    def get_variable_display(self, name, color, value_range):
        ...
        number_line = NumberLine(value_range, width=1.25, tick_size=0.05)
    ...
    def get_axes(self, x_range=(-4, 4), y_range=(-1.0, 2.0, 1.0)):
        axes = NumberPlane(x_range, y_range, width=FRAME_WIDTH, height=6, background_line_style=dict(stroke_color=GREY_A, stroke_width=2, stroke_opacity=0.5))
...
class LimitingDistributions(InteractiveScene):
    ...
    def construct(self):
        ...
        top_label = TexText('Random variable \\\\ $X$', font_size=36)
        ...
        s_plot = self.get_sum_plot(dist, get_N(), top_plot)
        self.add(s_plot)
        bars = top_plot.bars
        mean, sd = get_mean_and_sd(get_dist(bars))
        top_mean_sd_labels = self.get_mean_sd_labels('\\mu', '\\sigma', mean, sd)
        sum_mean_sd_labels = self.get_mean_sd_labels('\\mu \\cdot N', '\\sigma \\cdot \\sqrt{N}', mean * 10, sd * math.sqrt(10))
        scaled_sum_mean_sd_labels = self.get_mean_sd_labels('', '', 0, 1)
        ...
        for n in range(3, 11):
            ...
            self.play(ReplacementTransform(s_plot.bars, new_s_plot.bars[:len(s_plot.bars)]), GrowFromEdge(new_s_plot.bars[len(s_plot.bars):], DOWN), ReplacementTransform(s_plot.axes, new_s_plot.axes), FadeTransform(sum_label, new_sum_label), run_time=0.5)
            s_plot = new_s_plot
        ...
        self.play(TransformMatchingTex(top_mean_sd_labels[0].copy(), sum_mean_sd_labels[0]), TransformMatchingTex(sum_label, new_sum_label), TransformFromCopy(mu_label[:1], low_mu_label[:1]), run_time=1.5)
        sum_label = new_sum_label
        self.wait()
        self.play(TransformMatchingTex(top_mean_sd_labels[1].copy(), sum_mean_sd_labels[1]), TransformFromCopy(sigma_label[:1], low_sigma_label[:1]), run_time=1.5)
        self.wait()
        formula = Tex('{1 \\over \\sigma \\sqrt{2\\pi N}} e^{-{1 \\over 2} \\left({x - \\mu N \\over \\sigma \\sqrt{N}} \\right)^2}', t2c={'\\mu': PINK, '\\sigma': RED, 'N': YELLOW}, font_size=36)
        N = get_N()
        ...
        self.play(ShowCreation(graph), FadeIn(formula, UP))
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(graph, *low_lines, formula), lag_ratio=0.5))
        ...
        self.play(LaggedStart(Transform(sum_label, scaled_sum_label[sum_label.get_string()][0].copy(), remover=True), Write(scaled_sum_label), s_plot.animate.shift(x_shift), lag_ratio=0.5))
        self.wait()
        sub_part = scaled_sum_label['- \\decimalmob \\cdot \\mu']
        rect = SurroundingRectangle(sum_mean_sd_labels[0], color=RED)
        target_x = ss_plot.axes.c2p(0, 0)[0]
        self.play(ShowCreation(rect))
        self.play(FadeTransform(sum_mean_sd_labels[0].copy(), sub_part), rect.animate.set_points(SurroundingRectangle(sub_part).get_points()), s_plot.bars.animate.shift((target_x - low_lines[0].get_x() - x_shift) * RIGHT), s_plot.axes.animate.shift((target_x - s_plot.axes.c2p(0, 0)[0]) * RIGHT))
        self.play(FadeOut(rect), FadeOut(sum_mean_sd_labels[0], RIGHT), FadeIn(scaled_sum_mean_sd_labels[0], RIGHT))
        self.wait()
        div_part = scaled_sum_label['\\sigma \\cdot \\sqrt{\\decimalmob}']
        rect = SurroundingRectangle(sum_mean_sd_labels[1], color=RED)
        self.play(ShowCreation(rect))
        self.play(FadeTransform(sum_mean_sd_labels[1].copy(), div_part), rect.animate.set_points(SurroundingRectangle(div_part).get_points()))
        ...
        self.play(FadeOut(s_plot.axes), FadeIn(ss_plot.axes), ReplacementTransform(s_plot.bars, ss_plot.bars), FadeOut(sum_label))
        ...
        self.play(Restore(sd_lines), FadeOut(sum_mean_sd_labels[1], RIGHT), FadeIn(scaled_sum_mean_sd_labels[1], RIGHT))
        self.wait()
        ...
        meaning = TexText('How many std devs away from the mean is $X_1 + \\cdots + X_{10}$', t2c={'std devs': RED, 'mean': PINK}, font_size=40)
        meaning.move_to(words, LEFT)
        self.add(full_screen_rect, scaled_sum_label)
        self.play(FadeIn(full_screen_rect))
        self.wait()
        self.play(FadeIn(top_rect), FadeIn(words))
        self.wait()
        self.play(words.animate.shift(UP), Write(meaning))
        self.wait()
        ...
        self.play(FadeOut(full_screen_rect), top_rect.animate.set_opacity(1), FadeIn(bar))
        self.wait()
        self.play(FadeIn(sum_tex, lag_ratio=0.2, run_time=2), GrowArrow(arrow))
        self.wait()
        self.play(arrow.animate.become(Vector(0.5 * UP).next_to(bar, DOWN, SMALL_BUFF)))
        self.wait()
        self.play(LaggedStart(FadeOut(top_rect), Restore(ss_plot.bars), FadeOut(bar), FadeOut(words), FadeOut(meaning), FadeOut(sum_tex), FadeOut(arrow)))
        ...
        self.play(ss_plot.bars.animate.set_opacity(0.2), FadeIn(bar))
        ...
        self.wait()
        self.play(FadeIn(bar_range, lag_ratio=0.2), FadeOut(bar))
        self.wait()
        self.play(Restore(ss_plot.bars), FadeOut(bar_range))
        self.wait()
        ...
        for n in range(9, 2, -1):
            ...
            self.play(FadeTransform(scaled_sum_label, new_scaled_sum_label), FadeTransform(ss_plot.bars, new_ss_plot.bars), run_time=0.1)
            self.wait(0.4)
            scaled_sum_label = new_scaled_sum_label
            ss_plot = new_ss_plot
        alt_dists = [np.array(U_SHAPED_DISTRIBUTION), np.random.random(6), np.random.random(6), np.array(STEEP_U_SHAPED_DISTRIBUTION), np.random.random(6)]
        for dist in alt_dists:
            dist /= dist.sum()
        words = Text('Change this\ndistribution')
        words.set_color(YELLOW)
        words.move_to(top_plot, UR)
        self.play(FadeOut(mu_label), FadeOut(sigma_label), Write(words))
        self.change_distribution(alt_dists, top_plot, ss_plot, top_mean_sd_labels, get_N())
        ...
        for n in range(4, self.max_n + 1):
            ...
            if n < 5:
                ...
                wt = 0.1
            if rt > 0:
                self.play(FadeOut(ss_plot.bars), FadeIn(new_ss_plot.bars), ReplacementTransform(ss_plot.axes, new_ss_plot.axes), FadeTransform(scaled_sum_label, new_scaled_sum_label), run_time=rt)
            else:
            ...
            if n in [10, self.max_n]:
                self.wait()
                self.change_distribution([EXP_DISTRIBUTION, *alt_dists], top_plot, ss_plot, top_mean_sd_labels, get_N())
        self.wait()
        graph = ss_plot.axes.get_graph(lambda x: gauss_func(x, 0, 1), x_range=(-8, 8))
        graph.set_stroke(YELLOW, 3)
        label = Tex('{1 \\over \\sqrt{2\\pi}} e^{-x^2 / 2}')
        label['{1 \\over \\sqrt{2\\pi}}'].scale(0.8, about_edge=RIGHT)
        label.next_to(graph.pfp(0.55), UR)
        self.play(ShowCreation(graph), Write(label), scaled_sum_mean_sd_labels.animate.to_edge(RIGHT))
        self.play(FlashAround(label, run_time=2))
        self.wait()
        random_dists = [normalize(np.random.random(6)) ** 2 for x in range(8)]
        self.change_distribution([*random_dists, alt_dists[-1]], top_plot, ss_plot, top_mean_sd_labels, get_N(), run_time=1.5)
        self.wait()
        ...
        rect.set_stroke(BLUE, 2)
        statement = Tex('\\lim_{N \\to \\infty} P(a < \\text{This value} < b) = \\int_a^b ' + label.get_string() + 'dx', font_size=36)
        ...
        self.wait()
        self.play(FadeIn(statement['P(a < \\text{This value} < b)'], 0.5 * DOWN))
        self.play(statement['This value'].animate.set_color(BLUE), ShowCreation(arrow))
        self.play(to_fade.animate.set_fill(opacity=0.1).set_stroke(width=0), graph.animate.set_stroke(width=1.5))
        self.wait()
        self.play(Write(statement['\\lim_{N \\to \\infty}']))
        self.wait()
        self.play(LaggedStart(shift_group.animate.shift(3 * RIGHT), FadeTransform(label, statement[label.get_string()]), Write(statement[' = \\int_a^b ']), Write(statement['dx']), lag_ratio=0.4, run_time=3))
        self.wait()
        self.play(LaggedStart(*(Indicate(bar, color=BLUE) for bar in bars[mid_i - 5:mid_i + 10])))
        self.wait()

    def get_top_distribution_plot(self, dist, width=5, height=2.5, y_range=(0, 0.5, 0.1)):
        x_min = self.x_min
        axes = Axes((x_min - 1, x_min + len(dist) - 1), y_range, width=width, height=height)
    ...
    def get_mu_sigma_annotations(self, plot, mu_tex='\\mu', sigma_tex='\\sigma', min_height=2.0, x_min=1):
        bars = plot.bars
        axes = plot.axes
        dist = get_dist(bars)
        ...
        return (VGroup(mean_line, mu_label), VGroup(sd_lines, sd_arrows, sigma_label))
    ...
    def get_sum_label(self, n: str | int, dots=True, scaled: bool=False):
        ...
        if n_str == '2':
            ...
            sum_tex = f'X_1 + \\cdots + X_{{{n_str}}}'
        else:
            sum_tex = ' + '.join((f'X_{{{k}}}' for k in range(1, int(n) + 1)))
        if scaled:
            sum_tex = '(' + sum_tex + ')' + f' - {n_str} \\cdot \\mu \\over \\sigma \\cdot \\sqrt{{{n_str}}}'
        sum_label = Tex(sum_tex, t2c={'\\mu': PINK, '\\sigma': RED, n_str: YELLOW}, font_size=40)
        if scaled:
            sum_label.scale(0.8)
    ...
    def get_normal_plot_axes(self):
        sum_axes = Axes((-6, 6), self.normal_y_range, width=12, height=self.normal_axes_height)
    ...
    def get_sum_plot(self, dist, n, top_plot, x_range=(0, 40), y_range=(0, 0.2, 0.1), x_num_range=(5, 40, 5), max_width=10):
        sum_dist = np.array(dist)
        for _ in range(n - 1):
            sum_dist = np.convolve(sum_dist, dist)
        x_min = n * self.x_min
        x_max = x_range[1]
        axes = Axes(x_range, y_range, width=top_plot.axes.x_axis.get_unit_size() * x_max, height=self.normal_axes_height, axis_config=dict(tick_size=0.05))
        ...
        axes.x_axis.numbers.shift(0.5 * axes.x_axis.get_unit_size() * LEFT)
        axes.y_axis.add_numbers(num_decimal_places=len(str(y_range[2])) - 2, font_size=24, excluding=[0])
    ...
    def get_mean_sd_labels(self, mu_tex, sigma_tex, mean, sd):
        label_kw = dict(t2c={'\\mu': PINK, '\\sigma': RED, 'N': YELLOW}, font_size=36)
        if len(mu_tex) > 0:
            mu_tex = '= ' + mu_tex
        if len(sigma_tex) > 0:
            sigma_tex = '= ' + sigma_tex
        labels = VGroup(Tex(f'\\text{{mean}} {mu_tex} = 0.00', **label_kw), Tex(f'\\text{{std dev}} {sigma_tex} = 0.00', **label_kw))
    ...
    def change_distribution(self, alt_dists, top_plot, ss_plot, top_mean_sd_labels, n, run_time=1):
        for dist in alt_dists:
            ...
            self.play(Transform(top_plot.bars, new_top_plot.bars), UpdateFromFunc(ss_plot.bars, lambda m: m.set_submobjects(self.get_scaled_sum_plot(get_dist(top_plot.bars), n).bars)), ChangeDecimalToValue(top_mean_sd_labels[0][-1], mean), ChangeDecimalToValue(top_mean_sd_labels[1][-1], sd))
            self.wait()
            self.distribution = dist
...
class HowVarianceAdds(LimitingDistributions):
    ...
    def construct(self):
        ...
        label_kw = dict(font_size=60, t2c={'X': BLUE, 'Y': GREEN})
        top_labels = VGroup(Tex('X', **label_kw), Tex('Y', **label_kw))
        ...
        sum_plot = self.get_top_distribution_plot(sum_dist, y_range=(0, 0.301, 0.1), width=10)
        ...
        var_form = Tex('\\text{Var}(X + Y) = \\text{Var}(X) + \\text{Var}(Y)', **label_kw)
        var_form.scale(0.75)
        ...
        self.add(h_line)
        self.play(LaggedStartMap(FadeIn, top_plots, lag_ratio=0.7, run_time=2), LaggedStartMap(FadeIn, top_labels, lag_ratio=0.7, run_time=2), LaggedStartMap(FadeIn, top_annotations, lag_ratio=0.7, run_time=2))
        self.play(TransformFromCopy(plot1, sum_plot), TransformFromCopy(plot2, sum_plot.copy().fade(1)), TransformFromCopy(top_labels[0], sum_label[:1]), TransformFromCopy(top_labels[1], sum_label[1:]), TransformFromCopy(top_annotations1, sum_annotations), TransformFromCopy(top_annotations2, sum_annotations))
        self.play(LaggedStart(TransformMatchingShapes(sum_label.copy(), var_form['\\text{Var}(X + Y) = ']), TransformMatchingShapes(top_labels[0].copy(), var_form['\\text{Var}(X)']), TransformMatchingShapes(top_labels[1].copy(), var_form['+ \\text{Var}(Y)']), FadeIn(note, 0.25 * DOWN), lag_ratio=0.25))
        self.wait()
        sigma_form = Tex('\\sigma_{X + Y}^2 = \\sigma_X^2 + \\sigma_Y^2', **label_kw)
        sigma_form.scale(0.75)
        sigma_form.move_to(var_form, LEFT)
        self.play(LaggedStart(FadeIn(sigma_form, 0.5 * DOWN), var_form.animate.shift(DOWN), note.animate.shift(DOWN).set_opacity(0.7), lag_ratio=0.2, run_time=2))
        self.wait()
        ...
        new_sum_plot = self.get_top_distribution_plot(sum_dist, y_range=(0, 0.2, 0.1), width=10)
        new_sum_plot.axes.x_axis.remove(new_sum_plot.axes.x_axis.numbers)
        new_sum_plot.to_corner(DL)
        new_sum_label = Tex('X_1 + \\cdots + X_n', t2c={'X_1': BLUE, 'X_n': BLUE})
        new_sum_label.next_to(new_sum_plot.axes.c2p(0, 0.2), UR)
        rules = VGroup(sigma_form, var_form, note)
        self.play(FadeOut(plot2, RIGHT), FadeOut(top_labels[1], RIGHT), FadeOut(top_annotations2, RIGHT))
        self.remove(plot1_group)
        self.play(FadeTransformPieces(plot1_group.replicate(4), new_top_groups), Write(dots), h_line.animate.shift(UP), FadeOut(sum_plot), FadeOut(sum_annotations), FadeOut(sum_label), rules.animate.scale(0.5).next_to(new_sum_plot.axes.c2p(0, 0), UP).to_edge(RIGHT), FadeIn(new_sum_plot), FadeIn(new_sum_label))
        self.wait()
        t2c = {'X_1': BLUE, 'X_n': BLUE}
        new_var_form = Tex('\\sigma_{X_1 + \\cdots + X_n}^2 = n \\cdot \\sigma_{X_1}^2', t2c=t2c)
        new_sigma_form = Tex('\\sigma_{X_1 + \\cdots + X_n} = \\sqrt{n} \\cdot \\sigma_{X_1}', t2c=t2c)
        ...
        rects = VGroup(SurroundingRectangle(new_var_form['\\sigma_{X_1 + \\cdots + X_n}^2']), SurroundingRectangle(new_var_form['\\sigma_{X_1}^2']))
        var_labels = VGroup(Tex('\\text{Var}(X_1 + \\cdots + X_n)', t2c=t2c), Tex('\\text{Var}(X_1)', t2c=t2c))
        ...
        self.play(FlashAround(new_sigma_form['\\sqrt{n}'], time_width=1.5, run_time=2))
        self.wait()
...
class InfiniteVariance(LimitingDistributions):
    ...
    def construct(self):
        ...
        old_plot = VGroup()
        var_label = Tex('\\text{Var}(X) = 0.00', font_size=60)
        ...
        for n in range(6, len(full_dist)):
            ...
            plot = self.get_top_distribution_plot(dist, width=min(n * (5 / 6), 12), height=4, y_range=(0, 0.301, 0.1))
            ...
            var = sigma ** 2
            if n < 12:
                self.play(FadeOut(old_plot), FadeIn(plot), ChangeDecimalToValue(var_label[-1], var), run_time=0.5)
                self.wait(0.5)
            else:
...
class RuleOfThumb(BuildUpGaussian):
    ...
    def construct(self):
        ...
        t2c = {'68': colors[0], '95': colors[1], '99.7': colors[2]}
        title = Text('The 68–95–99.7 rule', font_size=60, t2c=t2c)
        title.to_edge(UP, buff=MED_SMALL_BUFF)
        underline = Underline(title)
        underline.set_stroke(GREY, [0, 3, 3, 3, 0]).scale(1.2)
        self.add(title, underline)
        axes = NumberPlane((-4, 4), (0, 1.0, 0.1), width=0.5 * FRAME_WIDTH, height=5.5, background_line_style=dict(stroke_color=GREY_A, stroke_width=2, stroke_opacity=0.5))
        axes.to_edge(LEFT, buff=0)
        axes.to_edge(DOWN)
        axes.x_axis.add_numbers()
        axes.y_axis.add_numbers(np.arange(0.2, 1.2, 0.2), num_decimal_places=1, direction=DL, font_size=16)
        ...
        def update_area(area, dx=0.01):
            ...
            area.set_points_as_corners([*samples, axes.c2p(b, 0), axes.c2p(a, 0), samples[0]])
        area.add_updater(update_area)
        self.add(area)
        ...
        func_label = VGroup(Text('Standard normal\ndistribution', font_size=30), Tex('\\frac{1}{\\sqrt{2\\pi}} e^{-x^2 / 2}').set_color(YELLOW))
        func_label[1]['\\frac{1}{\\sqrt{2\\pi}}'].scale(0.75, about_edge=RIGHT)
        func_label.arrange(DOWN)
        func_label.next_to(graph, UP).shift(DOWN)
        func_label.to_edge(LEFT)
        func_label.set_backstroke(width=8)
        self.play(FadeIn(func_label[0]), Write(func_label[1]))
        self.wait()
        labels = VGroup()
        ...
        for n in range(3):
            anims = [FadeIn(labels[n], DOWN), ab_tracker.animate.set_value([-n - 1, n + 1])]
            if n == 0:
...
class AnalyzeHundreDiceQuestion(LimitingDistributions):
    ...
    def construct(self):
        ...
        sum_plot_config = dict(x_range=(0, 500, 10), y_range=(0, 0.05, 0.01), x_num_range=(0, 600, 100), max_width=14)
        ...
        dice = VGroup(*(DieFace(value, **dot_config) for value in range(1, 7)))
        ...
        for n in range(5, 101):
            ...
            self.wait(1 / 30)
        n = 100
        kw = dict(t2c={'100': YELLOW, '\\mu': PINK, '\\mu_s': PINK, '\\mu_a': PINK, '\\sigma': RED, '\\sigma_s': RED, '\\sigma_a': RED, '3.5': PINK, '1.71': RED}, font_size=36)
        top_mean = Tex('\\mu = \\frac{1}{6}\\big(1 + 2 + 3 + 4 + 5 + 6\\big) = 3.5', **kw)
        top_var = Tex('\\text{Var}(X) = \\frac{1}{6}\\Big((1 - 3.5)^2 + \\cdots + (6 - 3.5)^2 \\Big) = 2.92', **kw)
        top_var.scale(0.8)
        top_var['\\text{Var}(X) = '].scale(1 / 0.8, about_edge=RIGHT)
        top_var['= 2.92'].scale(1 / 0.8, about_edge=LEFT)
        top_var.refresh_bounding_box()
        top_sd = Tex('\\sigma = \\sqrt{\\text{Var}(X)} = 1.71', **kw)
        top_eqs = VGroup(top_mean, top_var, top_sd)
        top_eqs.arrange(DOWN, buff=0.5, aligned_edge=LEFT)
        top_eqs.to_corner(UR, buff=0.25)
        top_mean_simple = Tex('\\mu = 3.5', **kw).scale(1.25)
        top_sd_simple = Tex('\\sigma = 1.71', **kw).scale(1.25)
        ...
        self.wait()
        self.play(Write(top_sd), Transform(top_var['\\text{Var}(X)'].copy(), top_sd['\\text{Var}(X)'].copy(), remover=True))
        self.wait()
        self.play(TransformMatchingTex(top_mean.copy(), top_mean_simple), TransformMatchingTex(top_sd.copy(), top_sd_simple), top_eqs.animate.scale(0.75, about_edge=UR).set_opacity(0.5))
        self.wait()
        low_mean = Tex('\\mu_s = 100 \\cdot \\mu = 350', **kw)
        low_sd = Tex('\\sigma_s = \\sqrt{100} \\cdot \\sigma = 17.1', **kw)
        ...
        mean_label, sd_label = self.get_mu_sigma_annotations(sum_plot, '350', '17.1', min_height=1.0, x_min=n)
        ...
        bound_equations = VGroup(Tex('\\mu_s - 2 \\sigma_s = 350 - 2 \\cdot 17.1 \\approx 316', **kw), Tex('\\mu_s + 2 \\sigma_s = 350 + 2 \\cdot 17.1 \\approx 384', **kw))
        ...
        self.play(sd_lines.animate.stretch(2, 0), sd_arrows.animate.stretch(2, 0), TransformMatchingShapes(sd_num, new_sd_num, run_time=1))
        ...
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(*bound_equations, rhs_rect), shift=DOWN))
        sample_mean_expr = Tex('X_1 + \\cdots + X_{100} \\over 100', **kw)
        ...
        new_diagram_labels = VGroup(DecimalNumber(3.16), DecimalNumber(3.5), DecimalNumber(3.84), Tex('2 \\cdot 0.171'))
        ...
        self.play(FadeTransform(sum_label, sample_mean_expr[:length]), Write(sample_mean_expr[length:]))
        self.wait()
        self.play(LaggedStartMap(FlashAround, x_numbers, lag_ratio=0.1, time_width=1.5))
        self.play(LaggedStart(*number_swaps), low_eqs.animate.set_opacity(0.4))
        rect = SurroundingRectangle(sample_mean_expr)
        rect.set_stroke(BLUE, 2)
        avg_words = Text('Average of\n100 rolls')
        avg_words.next_to(rect, RIGHT)
        dice = VGroup(*(DieFace(random.randint(1, 6), **dot_config) for x in range(100)))
        dice.arrange_in_grid(10, 10)
        dice.set_height(2)
        dice.next_to(sum_plot.axes.c2p(0, 0), UR, SMALL_BUFF)
        dice.match_x(avg_words)
        self.play(ShowCreation(rect), Write(avg_words), FadeIn(dice, lag_ratio=0.1))
        self.wait()
        avg_mean = Tex('\\mu_a = 3.5', **kw)
        avg_sd = Tex('\\sigma_a = \\sigma / \\sqrt{100} = 0.171', **kw)
        ...
        self.play(FadeIn(avg_mean, RIGHT), low_mean.animate.to_edge(RIGHT))
        self.wait()
        self.play(FadeIn(avg_sd, RIGHT), low_sd.animate.to_edge(RIGHT))
        self.wait()
        self.play(FlashAround(low_sd), low_sd.animate.set_opacity(1))
        self.wait()
...
class Thumbnail(LimitingDistributions):
    ...
    def construct(self):
        ...
        ss_plot.scale(2)
        ss_plot.bars.set_submobject_colors_by_gradient(*3 * [BLUE], *3 * [YELLOW])
        ss_plot.shift(2 * DOWN - ss_plot.axes.c2p(0, 0))
        ss_plot.to_edge(DOWN)
        self.add(ss_plot)
        top_plots = VGroup()
        np.random.seed(3)
        dists = [EXP_DISTRIBUTION, normalize(np.random.random(6)) ** 2, normalize(np.random.random(6)) ** 2, U_SHAPED_DISTRIBUTION]
        for dist in dists:
        ...
        arrows = VGroup(*(Arrow(tp.get_bottom(), ss_plot.bars.get_top() + vect, buff=0.3, stroke_width=10, stroke_color=YELLOW) for tp, vect in zip(top_plots, np.linspace(2 * LEFT, 2 * RIGHT, len(top_plots)))))
        self.add(top_plots)
        self.add(arrows)
        ...
        form = Tex('{1 \\over \\sqrt{2\\pi}} e^{-x^2 / 2}', font_size=90)
