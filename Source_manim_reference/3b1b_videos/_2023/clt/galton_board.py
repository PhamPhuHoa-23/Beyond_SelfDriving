...
class GaltonBoard(InteractiveScene):
    ...
    def setup(self):
        super().setup()
        self.ball_template = Sphere(radius=self.ball_radius, color=YELLOW, resolution=(51, 26))
    ...
    def construct(self):
        ...
        sum_labels = VGroup(*(Integer(s, font_size=24, include_sign=True) for s in sums))
        ...
        for x in range(3):
            ...
            self.add_single_clink_sound()
            self.play(self.falling_anim(ball, pieces[-1]), corner_sum_anim)
            self.wait()
            self.play(FadeOut(all_arrows, lag_ratio=0.025), corner_sum_fade)
        ...
        def get_peg_label(n, k, split=False):
            ...
            if n == 0:
                ...
                label = VGroup(Tex(f'{choose(n - 1, k - 1)} \\over {2 ** n}', **kw), Tex(f' + {{{choose(n - 1, k)} \\over {2 ** n}}}', **kw))
        ...
        for n in range(1, self.n_rows + 1):
            ...
            anims = [TransformFromCopy(last_labels[0], split_labels[0]), TransformFromCopy(last_labels[-1], split_labels[-1])]
            for k in range(1, n):
            ...
            self.play(*anims)
            self.play(*(FadeTransformPieces(sl1, sl2) for sl1, sl2 in zip(split_labels, unsplit_labels)))
    ...
    def get_ball_arrows(self, ball, labels, sub_labels=[], colors=[RED, BLUE]):
        ...
        for vect, color, label in zip([LEFT, RIGHT], colors, labels):
            arrow = Vector(0.5 * self.spacing * vect, tip_width_ratio=3, stroke_color=color)
    ...
    def show_corner_sum(self, pm_arrows, bits, font_size=48):
        ...
        anim2 = LaggedStart(*(ReplacementTransform(part.copy().set_opacity(0), sum_term, path_arc=-30 * DEGREES) for part in parts.target))
    ...
    def random_trajectory(self, ball, pegs, buckets, bits=None):
        ...
        final_line = Line(bounces[-1].get_end(), bucket.bottom.get_center() + self.ball_radius * UP)
    ...
    def falling_anim(self, ball, trajectory):
        return MoveAlongPath(ball, trajectory, rate_func=linear, run_time=self.fall_factor * trajectory.get_arc_length())

    def add_single_clink_sound(self, time_offset=0, gain=-20):
        self.add_sound(sound_file=self.clink_sound.replace('click', 'click' + str(random.randint(1, 12))), time_offset=time_offset, gain=gain)
    ...
    def drop_n_balls(self, n, pegs, buckets, lr_factor=1, sound=False):
        ...
        if sound:
            ...
            for time, traj in zip(start_times, trajs):
                self.add_falling_clink_sounds(traj[1], time + 0.0 * random.random(), gain=-30)
        self.play(full_anim)
        return balls
...
class GaltonTrickle(GaltonBoard):
    ...
    def construct(self):
        ...
        trajs = [self.random_trajectory(ball, pegs, buckets, bits) for ball, bits in zip(balls, all_bits)]
        falling_anims = (self.falling_anim(ball, traj[0]) for ball, traj in zip(balls, trajs))
        arrow_copies = VGroup()
        ...
        self.add(arrows)
        self.play(LaggedStart(*falling_anims, lag_ratio=0.4, run_time=rt))
        self.wait()
...
class BiggerGaltonBoard(GaltonBoard):
    ...
    def construct(self):
        ...
        formula = Tex('{1 \\over \\sqrt{2\\pi}} e^{-x^2 / 2}', font_size=60)
...
class NotIdenticallyDistributed(GaltonBoard):
    ...
    def construct(self):
        ...
        def get_peg_arrow(peg, angle, length, color=RED_E):
            ...
            arrow = FillArrow(ORIGIN, length * vect, buff=0, fill_color=color, tip_width_ratio=3, thickness=0.025)
        ...
        def get_bounce_distribution(peg, sigma=30 * DEGREES):
            ...
            arrows = VGroup(*(get_peg_arrow(peg, angle, denom * gauss_func(angle, 0, sigma) * max_arrow_len) for angle in angles))
            return arrows
        all_dists = VGroup(*(get_bounce_distribution(peg) for row in pegs for peg in row))
        ...
        self.play(frame.animate.set_height(3, about_edge=UP), FadeOut(all_dists, lag_ratio=0.01), self.falling_anim(ball, pieces[0]), run_time=2)
