...
class NormalName(InteractiveScene):
    ...
    def construct(self):
        names = VGroup(Text('Normal distribution'), Text('Bell curve'), Text('Gaussian distribution'))
        point = 2 * LEFT + 1.5 * UP
        ...
        self.wait()
        self.play(names[0].animate.scale(0.5, about_edge=DOWN).shift(shift), FadeIn(names[1], shift=0.5 * shift))
        self.wait()
        ...
        words.shift(0.5 * UL)
        arrow = Arrow(words.get_bottom() + LEFT, words.get_bottom() + DOWN, stroke_width=3, stroke_color=GREY_A)
        self.play(LaggedStart(FadeOut(names[1:]), FadeIn(words, lag_ratio=0.1), ShowCreation(arrow), lag_ratio=0.25))
        self.wait()
...
class ErdosKac(InteractiveScene):
    ...
    def construct(self):
        ...
        titles = VGroup(Tex('N'), TexText('\\# of distinct prime factors of $N$'))
...
class VideoPlan(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(TransformMatchingStrings(big_name, name, run_time=1), full_rect.animate.set_opacity(1))
        ...
        self.play(FadeTransform(screen_rect.copy(), screen2), FadeTransform(name.copy(), name2))
        self.wait()
        group1.target = group1.generate_target()
        ...
        arrows = VGroup(Arrow(group1.target[2].get_right(), screen2.get_left() + UP), Arrow(conv_rect.get_right(), screen2.get_left() + DOWN))
        arrows.set_color(BLUE)
        self.play(LaggedStart(MoveToTarget(group1), FadeIn(conv_group, DOWN), name2.animate.next_to(screen2, UP), lag_ratio=0.25))
...
class NextVideoInlay(InteractiveScene):
    ...
    def construct(self):
        plane = NumberPlane((-5, 5), (-0.25, 1.0, 0.25), width=2.5, height=1.25, background_line_style=dict(stroke_color=GREY_B, stroke_width=1), faded_line_style=dict(stroke_color=GREY_B, stroke_width=1, stroke_opacity=0.25), faded_line_ratio=4)
        plane.set_height(FRAME_HEIGHT)
        graph = plane.get_graph(lambda x: gauss_func(x, 0, 1))
        graph.set_stroke(YELLOW, 3)
        self.add(plane)
        self.play(VShowPassingFlash(graph.copy().set_stroke(TEAL, 10), time_width=1.5), ShowCreation(graph), run_time=2)
        expr = Tex('{1 \\over \\sqrt{2\\pi}} e^{-x^2 / 2}')
        expr.set_height(3)
        ...
        self.wait()
        rects = VGroup(SurroundingRectangle(expr['e^{-x^2 / 2}']).set_stroke(TEAL, 5), SurroundingRectangle(expr['\\pi']).set_stroke(RED, 5))
        questions = [Text('Why this function?'), Text("Where's\nthe circle?", alignment='LEFT')]
        ...
        self.play(FadeIn(questions[0]), ShowCreation(rects[0]))
        self.wait()
        self.play(FadeIn(questions[1]), ShowCreation(rects[1]))
        self.wait()
...
class SumsOfSizeFive(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(morty.change('raise_right_hand', self.screen), self.change_students('pondering', 'happy', 'tease', look_at=self.screen))
        self.wait()
        ...
        def get_sum():
            ...
            result = VGroup(lhs, *values, Tex(' = '), rhs)
        ...
        for x in range(7):
            new_sum = get_sum()
            self.play(FadeOut(curr_sum[1:], lag_ratio=0.1, shift=0.2 * UP), FadeIn(new_sum[1:], lag_ratio=0.1, shift=0.2 * UP), *(pi.animate.look_at(new_sum) for pi in self.pi_creatures))
            self.wait()
            curr_sum = new_sum
        self.wait()
...
class GoalsThisLesson(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        morty = self.teacher
        stds = self.students
        self.play(morty.change('tease'), self.change_students('confused', 'thinking', 'hesitant', look_at=self.screen))
        self.wait(2)
        ...
        goals = BulletedList('Make this quantitative', 'Put formulas to it', 'Use it to predict', buff=0.35, font_size=48)
        goals.next_to(underline, DOWN, MED_LARGE_BUFF)
        goals.align_to(title, LEFT)
        self.play(FadeIn(title), ShowCreation(underline), morty.change('raise_right_hand', title), self.change_students('erm', 'thinking', 'tease', look_at=title))
        self.wait()
        for goal in goals:
            self.play(FadeIn(goal, UP))
            self.wait()
        new_title = VGroup(Text('3 assumptions underlie the'), Text('Central Limit Theorem'))
        ...
        self.play(FadeOut(title, 0.5 * DOWN), FadeIn(new_title, 0.5 * DOWN), underline.animate.next_to(new_title, DOWN, SMALL_BUFF).set_color(RED_E), LaggedStartMap(FadeOut, goals, shift=DOWN), morty.change('tease', new_title), self.change_students('hesitant', 'pondering', 'skeptical', new_title))
        self.play(FadeIn(numbers, lag_ratio=0.25), morty.change('raise_left_hand'))
        self.wait(2)
        self.play(LaggedStartMap(Restore, boxes), LaggedStart(morty.change('tease'), stds[0].change('angry'), stds[1].change('hesitant'), stds[2].change('erm'), lag_ratio=0.2))
        self.wait(4)
...
class CommentOnSpikeyBellCurve(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        morty = self.teacher
        stds = self.students
        self.play(morty.says('Kind of a\nbell curve', mode='shruggie'), self.change_students('erm', 'hesitant', 'hesitant', look_at=self.screen))
        self.wait(3)
        self.play(stds[1].says('Is it...supposed to\nlook like that?'), morty.debubble())
        self.look_at(self.screen)
        self.wait(2)
        self.play(stds[2].says("How many samples\nuntil we're sure?"), stds[1].debubble())
        self.wait(5)
...
class WhatElseDoYouNotice(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        self.remove(self.background)
        self.play(morty.says('What else do\nyou notice?'), self.change_students('pondering', 'erm', 'pondering', look_at=self.screen))
        self.wait(2)
...
class ReferenceMeanAndSD(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        stds = self.students
        self.remove(self.background)
        self.play(morty.says('We must put\nnumbers to those!', mode='surprised'), self.change_students('hesitant', 'concerned_musician', 'tease'))
        self.look_at(self.screen)
        self.wait(3)
        kw = dict(t2c={'\\mu': PINK, '\\sigma': RED}, font_size=42)
        terms = VGroup(Tex('\\text{Mean: } \\mu = E[X]', **kw), Tex('\\text{Std dev: } \\sigma = \\sqrt{E[(X - \\mu)^2]}', **kw))
        terms.arrange(DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
        terms.next_to(morty, UP, LARGE_BUFF)
        terms.to_edge(RIGHT)
        self.play(morty.debubble(mode='raise_right_hand', look_at=terms), self.change_students('pondering', 'hesitant', 'happy', look_at=terms), LaggedStartMap(FadeIn, terms, shift=UP, lag_ratio=0.5))
        self.wait(3)
        self.play(stds[2].says('Great!', mode='hooray'), morty.change('happy'))
        self.play(stds[0].says('Wait, can you\nremind me?', mode='guilty'), morty.change('tease'))
        self.wait(5)
...
class AskWhy(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(morty.change('tease'), self.change_students('pondering', 'raise_left_hand', 'confused', look_at=morty.get_top() + 3 * UP))
        self.wait(8)
...
class AskAboutPi(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        stds = self.students
        self.remove(self.background)
        self.play(LaggedStart(morty.change('happy'), stds[1].says('Wait, what?!', mode='surprised'), stds[0].change('confused', self.screen), stds[2].change('hesitant', self.screen)))
        self.wait()
        self.play(stds[2].says("Where's the\ncircle?", mode='raise_left_hand'))
        self.wait(3)
        self.play(stds[1].debubble(), stds[2].debubble(), morty.says(TexText("We'll cover \\\\ that later")))
        self.wait(2)
...
class WhyPiQuestion(InteractiveScene):
    ...
    def construct(self):
        ...
        area_label = Tex('\\text{Area} = \\sqrt{\\pi}')
...
class WeCanBeMoreElegant(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.says('We can be\nmore elegant'), self.change_students('pondering', 'confused', 'sassy', look_at=self.screen))
        self.wait()
...
class AskAboutFormalStatement(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        question = Text('But what is the\ncentral limit\ntheorem', t2s={'theorem': ITALIC}, t2c={'theorem': YELLOW})
        self.play(stds[2].says(question, mode='sassy', bubble_direction=LEFT))
        self.play(morty.change('guilty'))
        self.play(stds[0].change('pondering'), stds[1].change('erm'))
        self.wait(8)
...
class ExampleQuestion(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        part1 = re.findall('Consider .*\\n.* results.', question.get_string())[0]
        self.play(morty.change('raise_right_hand', question), self.change_students('pondering', 'pondering', 'tease', look_at=question), FadeIn(question[part1], lag_ratio=0.1, run_time=1.5), FadeOut(stds[2].bubble), FadeOut(stds[2].bubble.content))
        self.play(FlashUnder(question['fair die']), question['fair die'].animate.set_color(YELLOW), FadeIn(dice, lag_ratio=0.03, run_time=2))
        self.play(*(pi.animate.look_at(dice) for pi in [morty, *stds]))
        n_tracker.set_value(0)
        self.play(VFadeIn(sum_label), n_tracker.animate.set_value(100).set_anim_args(run_time=2), ShowIncreasingSubsets(die_highlights, run_time=2))
        self.play(FadeOut(die_highlights))
        self.wait()
        part2 = re.findall('Find .*\\n.*\\n.* range.', question.get_string())[0]
        self.play(morty.change('tease', question), self.change_students('erm', 'hesitant', 'pondering', look_at=question), FadeIn(question[part2], lag_ratio=0.1, run_time=1.5))
        self.wait()
        self.play(FlashUnder(question['95% sure'], color=TEAL), question['95% sure'].animate.set_color(TEAL))
        self.wait(2)
...
class ExampleQuestionNoPiCreatures(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(question[1], lag_ratio=0.1, run_time=1.5))
        self.wait()
        self.play(FlashUnder(question[1]['95% sure'], color=TEAL), question[1]['95% sure'].animate.set_color(TEAL))
        for x in range(5):
            old_dice = dice.copy()
...
class AverageDiceValues(InteractiveScene):
    ...
    def show_sample(self, avg_label, n_dice, added_anims=[]):
        dice = VGroup(*(DieFace(random.randint(1, 6), fill_color=BLUE_E, dot_color=WHITE, stroke_width=1) for x in range(n_dice)))
        dice.arrange_in_grid()
        dice.set_height(3.5)
        dice.next_to(avg_label, LEFT, buff=LARGE_BUFF)
        dice.to_edge(UP, buff=MED_SMALL_BUFF)
        self.play(LaggedStart(FadeOut(self.old_dice, run_time=0.5), ShowIncreasingSubsets(dice, run_time=1, int_func=np.ceil), UpdateFromFunc(avg_label[-1], lambda m: m.set_value(np.mean([die.value for die in dice]))), *added_anims))
...
class DoesThisMakeSense(InteractiveScene):
    ...
    def construct(self):
        rect = SurroundingRectangle(Tex('\\sigma_a = \\sigma / \\sqrt{100} = 0.171'))
...
class OneMoreSideNote(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(self.teacher.says('Will you tolerate\none more\nside note?', mode='speaking'))
...
class ThreeAssumptions(InteractiveScene):
    ...
    def construct(self):
        ...
        points = [TexText("1. All $X_i$'s are independent \\\\ from each other."), TexText('2. Each $X_i$ is drawn from \\\\ the same distribution.'), TexText('3. $0 < \\text{Var}(X_i) < \\infty$')]
        ...
        self.play(LaggedStart(*(Write(point[:2]) for point in points), lag_ratio=0.7), LaggedStartMap(FadeIn, rects, lag_ratio=0.7))
        ...
        self.play(FlashUnder(word, color=TEAL), word.animate.set_color(TEAL))
        self.wait()
        self.play(rects[1].animate.stretch(0, 0, about_edge=RIGHT))
        self.remove(rects[1])
        self.wait()
        word = points[1]['same distribution']
        self.play(FlashUnder(word, color=YELLOW), word.animate.set_color(YELLOW))
        self.wait()
        box = SurroundingRectangle(VGroup(*points[:2]))
        box.set_stroke(GREY_C, 2)
        iid_words = TexText('i.i.d. $\\rightarrow$ independent and identically distributed')
        iid_words.next_to(box, UP, MED_LARGE_BUFF, aligned_edge=LEFT)
        title.add(underline)
        self.play(FadeOut(title), ShowCreation(box))
        self.play(Write(iid_words))
        self.wait(2)
        self.play(FadeOut(box), FadeOut(iid_words), FadeIn(title))
        self.wait()
        gen_words = TexText('These can be relaxed (see Lindeberg CLT, $\\alpha$-mixing, etc.)')
        gen_words.scale(0.7)
        ...
        self.wait(2)
        self.play(FadeOut(box), FadeOut(gen_words), FadeIn(title))
        self.wait()
...
class AssumingNormality(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(randy.says(TexText("\n                    It's a $3\\sigma$ event, \\\\\n                    so $p < 0.003$\n                    ", t2c={'\\sigma': RED}), look_at=morty.eyes, mode='tease'), morty.change('hesitant', randy.eyes))
        self.play(Blink(morty))
        self.wait()
        self.play(Blink(randy))
        self.wait()
        self.play(morty.says('Is it though?', look_at=randy.eyes, mode='sassy'), randy.change('guilty', morty.eyes))
        self.wait()
        self.play(Blink(randy))
        self.wait()
...
class FiniteExpectations(InteractiveScene):
    ...
    def construct(self):
        tex = TexText('And finite $E[X]$ for that matter', t2c={'X': BLUE})
        self.add(tex)
