...
EQUATION_T2C = {'\\vec{\\textbf{x}}(t)': RED, '\\vec{\\textbf{F}}(t)': YELLOW, '\\vec{\\textbf{a}}(t)': YELLOW, '\\frac{d^2 \\vec{\\textbf{x}}}{dt^2}(t) ': YELLOW, '\\vec{\\textbf{E}}_0': BLUE, '\\omega_r': PINK, '\\omega_l': TEAL}

def get_bordered_thumbnail(path, height=3, stroke_width=3):
    image = ImageMobject(path)
    thumbnail = Group(SurroundingRectangle(image, buff=0).set_stroke(WHITE, stroke_width), image)
...
class HoldUpAlbumCover(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.change('raise_right_hand', album), self.change_students('hooray', 'coin_flip_2', 'tease', look_at=album))
        self.play(FadeIn(album))
        self.wait(2)
        ...
        titles = VGroup(Text('Genuine simulation'), Text('Pink Floyd'))
        for title, mob in zip(titles, [true_prism, album.target]):
            title.move_to(mob)
            title.to_edge(UP, buff=0.5)
        self.add(full_rect, album, true_prism)
        self.play(FadeIn(full_rect), MoveToTarget(album), FadeInFromPoint(true_prism, album.get_center()), run_time=2)
        ...
        self.play(FadeIn(why_white, lag_ratio=0.1), GrowArrow(red_arrow))
        self.wait()
        roygbiv = Text('ROYGBV')
        ...
        self.play(FadeIn(roygbiv, lag_ratio=0.5), ShowCreation(rainbow, lag_ratio=0.5), run_time=4)
        self.wait()
        self.play(LaggedStartMap(FadeOut, Group(why_white, red_arrow, roygbiv, rainbow, true_prism, *titles), shift=DOWN, scale=0.5, lag_ratio=0.1))
        self.add(self.pi_creatures, full_rect, album)
        self.play(Restore(album), FadeOut(full_rect))
        self.wait()
        ...
        self.play(FadeOut(album, UP), FadeIn(question, UP), self.teacher.change('confused', arrow.get_end()), self.change_students('erm', 'hesitant', 'sassy', look_at=arrow.get_end()))
        self.play(GrowArrow(arrow))
        self.wait(5)
        title = Text('Standard explanation:', font_size=60)
        title.to_edge(UP, buff=MED_SMALL_BUFF)
        self.play(FadeTransform(question, title), FadeOut(arrow, LEFT), self.teacher.change('raise_right_hand', look_at=3 * UP), self.change_students('pondering', 'pondering', 'pondering', look_at=3 * UP))
        self.wait(5)
...
class DefineIndexOfRefraction(InteractiveScene):
    ...
    def construct(self):
        ior = '1.52'
        speed1 = Tex('\\text{Speed} = c \\approx 3\\times 10^8 \\, {\\text{m} / \\text{s}}')
        speed2 = Tex('\\text{Speed} \\approx c / ' + ior)
        ...
        for speed, x in zip([speed1, speed2], [-1, 1]):
            ...
            self.wait()
        num_words = '\\text{Speed in a vacuum}'
        den_words = '\\text{Speed in a glass}'
        equation = Tex(f'{{{num_words} \\over {den_words}}} \\approx {ior}', font_size=36)
        ...
        self.play(TransformMatchingShapes(speed1['Speed'].copy(), equation[num_words]), TransformMatchingShapes(speed2['Speed'].copy(), equation[den_words]), Write(equation['\\over']), Write(equation['\\approx']), TransformFromCopy(speed2[ior], equation[ior]))
        self.wait()
...
class LightDoesntHaveTreads(TeacherStudentsScene):
    ...
    def construct(self):
        stds = self.students
        self.play(stds[0].change('confused', self.screen), stds[1].change('erm', self.screen), stds[2].says("But, light doesn't\nhave treads...", mode='sassy', bubble_direction=LEFT), self.teacher.change('guilty'))
        self.wait(4)
...
class WhySlowAtAll(TeacherStudentsScene):
    ...
    def construct(self):
        stds = self.students
        self.play(stds[0].change('pondering', self.screen), stds[1].says('Why would light\nslow down at all?', mode='confused', bubble_direction=LEFT, look_at=self.screen), stds[2].change('erm', self.screen), self.teacher.change('tease'))
        self.wait(2)
...
class ReactToStandardExplanation(InteractiveScene):
    ...
    def construct(self):
        phrases = VGroup(Text('Light slows down in glass\ndifferent amounts for different colors', alignment='LEFT'), TexText('Slowing down $\\Rightarrow$ bending'), TexText('Therefore colors refract at different angles'))
        ...
        morty.to_edge(RIGHT)
        self.play(ShowCreation(rect), Write(why), p2.animate.set_opacity(0.25), phrases[1:].animate.set_opacity(0.25), VFadeIn(morty), morty.change('maybe', rect))
        self.play(Blink(morty))
        self.wait()
        self.play(FadeTransform(why, how), rect.animate.surround(slows_down), morty.change('angry', rect))
        self.play(Blink(morty))
        self.wait()
        why.next_to(p2, DOWN)
        self.play(rect.animate.surround(p2), p2.animate.set_opacity(1), p1.animate.set_opacity(0.25), FadeTransform(how, why), morty.change('confused', rect))
        self.play(Blink(morty))
        self.wait()
        self.remove(why)
        self.play(rect.animate.surround(phrases[0], buff=0.25), phrases[0].animate.set_opacity(1), FadeTransformPieces(why.copy(), discovered, remover=True), morty.change('tease', rect))
        self.add(discovered)
        self.play(Blink(morty))
        self.wait()
        ...
        self.play(phrases[0].animate.set_opacity(0.25), phrases[1].animate.set_opacity(1), rect.animate.surround(phrases[1]), FadeTransform(discovered, new_why), morty.change('pondering', phrases[1]))
        self.play(Blink(morty))
        self.wait()
        why.next_to(p1, UP)
        self.play(rect.animate.surround(p1), p1.animate.set_opacity(1), phrases[1].animate.set_opacity(0.25), FadeTransform(new_why, why), morty.change('raise_right_hand'))
        self.play(Blink(morty))
        self.wait()
...
class KeyPoints(InteractiveScene):
    ...
    def construct(self):
        key_points = VGroup(Text('Key point #1: Phase kicks'), Text('Key point #2: Layer oscillations'), Text('Key point #3: Resonance'))
        ...
        for kp1, kp2 in zip(key_points, key_points[1:]):
            self.play(FadeOut(kp1, 0.5 * UP), FadeIn(kp2, 0.5 * UP))
            self.wait()
...
class AsideOnWaveTerminology(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        terms = VGroup(Text('Phase'), Text('Frequency'), Text('Wave number'), Text('Amplitude'))
        terms.arrange(DOWN, buff=0.5, aligned_edge=LEFT)
        terms.next_to(title, DOWN)
        terms.shift(0.5 * LEFT)
        self.add(title, terms)
        self.play(self.teacher.change('raise_right_hand', terms), self.change_students('pondering', 'tease', 'hesitant', look_at=terms), LaggedStartMap(FadeIn, terms, shift=0.25 * DOWN, lag_ratio=0.5))
        self.wait(3)
...
class NewQuestion(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(q1))
        self.wait()
        self.play(ShowCreation(arrow1), Restore(q1), TransformMatchingStrings(q1.copy(), q2, run_time=1))
        self.wait()
        self.play(FlashUnder(q2['layer of glass'], color=BLUE), q2['layer of glass'].animate.set_color(BLUE))
        self.wait()
        self.play(FlashUnder(q2['phase']), q2['phase'].animate.set_color(YELLOW))
        self.wait()
        self.play(FadeOut(q1, UP), FadeIn(q3, UP), FadeOut(arrow1), FadeOut(q2))
        self.wait()
        self.play(ShowCreation(arrow2), Restore(q3), TransformMatchingStrings(q3.copy(), q4, run_time=1))
        self.wait()
...
class HoldUpLastVideo(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        tn = Group(SurroundingRectangle(image, buff=0).set_stroke(WHITE, 3), image)
        tn.set_height(3)
        ...
        self.play(self.teacher.change('raise_right_hand', tn), FadeIn(tn, UP), self.change_students('pondering', 'erm', 'tease', look_at=tn))
...
class WhyDoesAddingCauseAShift(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(stds[2].says('But, why?', look_at=morty.eyes, mode='raise_left_hand', bubble_direction=LEFT), self.change_students('pondering', 'pondering', look_at=self.screen), morty.change('guilty', self.screen))
        self.wait()
        self.play(stds[2].change('confused', self.screen))
        self.wait(2)
        self.play(morty.change('tease', stds[2].eyes))
        self.wait()
        self.play(morty.change('raise_right_hand', 3 * UR), stds[2].debubble(look_at=3 * UR), self.change_students('erm', 'hesitant', look_at=3 * UR))
        self.wait(3)
...
class AnnoateWaveSumPhaseShift(InteractiveScene):
    ...
    def construct(self):
        ...
        label = TexText('$90^\\circ$ behind')
        label.next_to(arrow.get_center(), RIGHT)
        self.play(LaggedStartMap(ShowCreation, lines2, lag_ratio=0, run_time=0.5))
        self.wait()
        self.play(TransformFromCopy(lines2, lines1), GrowArrow(arrow), FadeIn(label, lag_ratio=0.1))
        self.wait()

class PreviewQuarterBehindReason(InteractiveScene):

    def construct(self):
        plane = ComplexPlane((-3, 3), (-3, 3), background_line_style=dict(stroke_width=2, stroke_color=BLUE), faded_line_style=dict(stroke_width=1, stroke_opacity=0.3, stroke_color=BLUE))
        plane.set_height(7)
        self.add(plane)
        z1 = 1 + complex(-0.004, -0.05)
        amp = 0.08
        ...
        big_vect.add_updater(lambda m: m.put_start_and_end_on(plane.n2p(0), vects[-1].get_end() if len(vects) > 0 else plane.n2p(amp)))
        self.add(big_vect)
...
class IsThatTrue(TeacherStudentsScene):
    ...
    def construct(self):
        stds = self.students
        morty = self.teacher
        law = Tex('\\vec{\\textbf{F}}(t) = -k \\vec{\\textbf{x}}(t)', t2c=EQUATION_T2C)
        law.move_to(self.hold_up_spot, DOWN)
        morty.change_mode('raise_right_hand')
        self.add(law)
        self.play(stds[0].change('pondering', self.screen), stds[1].says('Is that accurate?', mode='sassy', look_at=self.screen), stds[2].change('erm', self.screen))
        self.wait(2)
        self.play(morty.says(TexText('For small $\\vec{\\textbf{x}}$, \\\\ accurate enough', t2c={'$\\vec{\\textbf{x}}$': RED})), self.change_students('thinking', 'erm', 'happy', look_at=morty.eyes), law.animate.to_edge(UP))
        self.wait()
        label = TexText("``Linear restoring force''")
        label.next_to(law, DOWN)
        self.play(FadeInFromPoint(label, morty.get_corner(UL)), self.change_students(look_at=label), morty.debubble('raise_right_hand', look_at=label), stds[1].debubble('pondering', look_at=label))
        self.wait(2)
        group = VGroup(law, label)
        true_law = Tex('\n            \\text{True force: }\n            F = -k x + c_2 x^2 + c_3 x^3 + c_4 x^4 + \\cdots\n            ', t2c={'F': YELLOW, 'x': RED}, font_size=60)
        true_law.move_to(UP)
        rect = SurroundingRectangle(true_law['F = -k x'], buff=0.25)
        rect.set_stroke(BLUE, 1)
        self.play(group.animate.set_height(1).to_corner(UR), FadeIn(true_law, lag_ratio=0.1, run_time=2), morty.change('hesitant'), self.change_students('pondering', 'pondering', 'pondering', look_at=true_law))
        self.wait(2)
        self.play(ShowCreation(rect), true_law[' + c_2 x^2 + c_3 x^3 + c_4 x^4 + \\cdots'].animate.set_opacity(0.2), morty.change('tease'))
        self.wait(2)

class VelocityZero(InteractiveScene):

    def construct(self):
        label = Tex('(\\text{Velocity} = 0)')
...
class FrequencyVsAngularFrequency(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        omega = Tex('\\omega_r', font_size=72)
        ...
        self.wait()
        self.play(MoveToTarget(rf_words), TransformMatchingStrings(rf_words.copy(), raf_words, run_time=1), self.change_students('pondering', 'hesitant', 'hesitant'))
        plane = ComplexPlane((-1, 1), (-1, 1), background_line_style=dict(stroke_width=1, stroke_color=BLUE), faded_line_style=dict(stroke_width=0.5, stroke_opacity=0.5, stroke_color=BLUE))
        plane.set_height(4)
        plane.to_edge(DOWN)
        ...
        t2c = {'\\omega': PINK, 'f': TEAL}
        f_eq = Tex('f = {\\text{Cycles} \\over \\text{Seconds}}', t2c=t2c)
        f_eq.next_to(f_words, DOWN, MED_LARGE_BUFF)
        f_eq['f'].set_color(YELLOW)
        af_eq = Tex('\\omega = {\\text{Radians} \\over \\text{Seconds}}', t2c=t2c)
        af_eq.next_to(af_words, DOWN, MED_LARGE_BUFF)
        af_eq['\\omega'].set_color(PINK)
        ...
        vect.add_updater(lambda m: m.put_start_and_end_on(plane.n2p(0), plane.n2p(np.exp(complex(0, angle_tracker.get_value())))))
        self.add(angle_tracker)
        self.play(FadeOut(self.background), LaggedStartMap(FadeOut, self.pi_creatures, shift=DOWN), ShowCreation(plane, lag_ratio=0.1), FadeIn(circle), TransformMatchingStrings(rf_words, f_words), TransformMatchingStrings(raf_words, af_words), FadeTransform(omega, af_eq['\\omega']), FadeIn(af_eq[1:]), FadeIn(f_eq), VFadeIn(vect))
        self.wait(4)
        f_value_eq = Tex('f = 0.25', t2c=t2c)
        af_value_eq = Tex('\\omega = 2\\pi f = 1.57', t2c=t2c)
        ...
        arc = always_redraw(lambda: Arc(0, get_arc_angle(), arc_center=plane.n2p(0), radius=0.5))
        arc_angle_label = DecimalNumber(0, unit='\\text{ Radians}')
        ...
        self.add(arc)
        self.play(arc_angle_tracker.animate.set_value(TAU / 4), VFadeIn(arc_angle_label))
        arc.clear_updaters()
        self.wait(10)
        self.play(VFadeOut(arc), VFadeOut(arc_angle_label), FadeOut(values))
        plane.add(circle)
        ...
        cos_eq = Tex('\\cos(\\omega t) = \\cos(2\\pi f t)', t2c=t2c)
        cos_eq.next_to(graph, UP)
        omega_arrow = Vector(0.5 * DOWN).set_color(PINK)
        omega_arrow.next_to(cos_eq['\\omega'], UP, SMALL_BUFF)
        dot = GlowDot()
        ...
        self.add(dot, line)
        self.play(ShowCreation(graph, rate_func=linear), FadeIn(cos_eq, time_span=(0, 1)), ShowCreation(omega_arrow, time_span=(4, 5)), run_time=16)

class UnequalFrequencies(InteractiveScene):

    def construct(self):
        word_eq = TexText('Freq. of light $\\ne$ Resonant freq.')
        eq = Tex('\\omega_l \\ne \\omega_r')
        word_lhs = word_eq['Freq. of light']
        word_rhs = word_eq['Resonant freq.']
        word_lhs.set_color(TEAL)
        word_rhs.set_color(PINK)
        eq['\\omega_l'].set_color(TEAL)
        eq['\\omega_r'].set_color(PINK)
        ...
        self.play(FadeIn(word_eq), ShowCreation(arrow))
        self.wait()
...
class DisectDrivenEq(InteractiveScene):
    ...
    def construct(self):
        ...
        lhs = driven_eq['\\vec{\\textbf{F}}(t) = - k \\vec{\\textbf{x}}(t)']
        parts = VGroup(*(driven_eq[tex] for tex in ['\\vec{\\textbf{E}}_0 q \\cos(\\omega_l t)', '\\omega_l', '\\vec{\\textbf{E}}_0', 'q']))
        light_part, omega_l, E0, q = parts
        words = VGroup(Text('Force from a\nlight wave'), Text('Angular frequency\nof the light (color)'), Text('Strength of\nthe wave'), Text('Charge of the\nelectron'))
        words[0].scale(0.75)
        words[1:].scale(0.75)
        ...
        self.wait()
        self.play(Write(driven_eq['+']), FadeIn(light_part, lag_ratio=0.1))
        self.play(ShowCreation(rect), Write(words[0]))
        self.wait()
        self.play(rect.animate.surround(omega_l), FadeTransform(*words[0:2]))
        self.wait()
        self.play(rect.animate.surround(E0), FadeTransform(*words[1:3]))
        self.wait()
        self.play(rect.animate.surround(q), FadeTransform(*words[2:4]))
        self.wait()
        self.play(rect.animate.surround(light_part), FadeTransform(words[3], words[0]))
        self.wait()
...
class PiCreaturesReactingToEquation(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(morty.change('raise_left_hand', point), self.change_students('pondering', 'thinking', 'erm', look_at=point))
        self.wait(3)
        self.play(self.change_students('thinking', 'pondering', 'pondering', look_at=point))
        self.wait(3)
        self.play(morty.says('Oh hey, it depends\non the frequency!', mode='hooray'), self.change_students('happy', 'tease', 'thinking', look_at=point))
        self.wait(4)
...
class HighlightExpression(InteractiveScene):
    ...
    def construct(self):
        ...
        expr.to_edge(UP)
        rect1 = SurroundingRectangle(expr['\\omega_r^2-\\omega_l^2'])
        rect2 = SurroundingRectangle(expr['\\frac{q ||\\vec{\\textbf{E}}_0||}{m\\left(\\omega_r^2-\\omega_l^2\\right)}'])
        self.add(expr)
...
class Amplitude(InteractiveScene):
    ...
    def construct(self):
        ...
        equation.to_corner(UL)
        lf_rect = SurroundingRectangle(equation['\\omega_l'])
        rf_rect = SurroundingRectangle(equation['\\omega_r'])
        ...
        VGroup(lf_rect, lf_words).set_color(EQUATION_T2C['\\omega_l'])
        VGroup(rf_rect, rf_words).set_color(EQUATION_T2C['\\omega_r'])
        self.add(equation)
        self.wait()
        self.play(ShowCreation(lf_rect), FadeIn(lf_words, 0.5 * DOWN))
        self.wait()
        self.play(ReplacementTransform(lf_rect, rf_rect), FadeTransform(lf_words, rf_words))
        self.wait()
...
class GuessSteadyState(InteractiveScene):
    ...
    def construct(self):
        prompt = VGroup(Text('Guess that: '), Tex('\n                \\vec{\\textbf{x}}(t) =\n                A \\cos(\\omega_l t)\n            ', t2c=EQUATION_T2C))
        prompt.arrange(RIGHT, buff=0.5)
        prompt.to_edge(UP)
        A_part = prompt[1]['A']
        A_part.set_color(GREY_B)
        freq = prompt[1]['\\omega_l']
        ...
        self.add(rect)
        self.add(prompt)
        self.play(GrowArrow(A_arrow), FadeIn(A_word))
        self.play(GrowArrow(freq_arrow), FadeIn(freq_word))
        self.wait()
        amp_tex = '\\frac{q \\vec{\\textbf{E}}_0}{m\\left(\\omega_r^2-\\omega_l^2\\right)}'
        solution = Tex(f'\\vec{{\\textbf{{x}}}}(t) = {amp_tex} \\cos(\\omega_l t)', t2c=EQUATION_T2C)
        solution.move_to(np.array([3.85, 0.65, 0.0]))
        self.play(rect.animate.set_height(FRAME_HEIGHT, about_edge=UP, stretch=True).set_anim_args(run_time=2), FadeOut(VGroup(prompt[0], A_arrow, A_word, freq_arrow, freq_word)), TransformMatchingTex(prompt[1], solution, matched_pairs=[(prompt[1]['\\vec{\\textbf{x}}(t)'], solution['\\vec{\\textbf{x}}(t)']), (A_part, solution[amp_tex])]))
        self.wait()

class StrongerResonanceStrongerPhaseShift(InteractiveScene):

    def construct(self):
        words = TexText('Stronger resonance $\\rightarrow$ Bigger phase kick', font_size=60)
        words.to_edge(UP)
        self.add(words)
...
class WhyIsThisTrue(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(stds[2].says('Why is it\na phase shift?', look_at=self.screen), stds[1].change('confused', self.screen), stds[0].change('pondering', self.screen), morty.change('guilty', stds[0].eyes))
        self.wait(2)
        self.play(self.change_students('pondering', 'pleading', 'maybe', look_at=self.screen), morty.change('hesitant', self.screen))
        self.wait(3)
...
class QuestionsFromPatrons(InteractiveScene):
    ...
    def construct(self):
        ...
        questions = BulletedList('Why does slowing imply bending?', 'What causes birefringence?', "What's the end of the barber pole explanation?", 'How can the index of refraction be lower than 1?', buff=0.75)
        questions.next_to(title, DOWN, buff=1.0)
        self.play(FadeIn(title, lag_ratio=0.1), LaggedStartMap(FadeIn, questions, shift=0.5 * DOWN, lag_ratio=0.25))
        self.wait()
        ...
        for question in questions:
            ...
            self.play(FadeIn(background), MoveToTarget(question))
            self.wait()
            self.play(Restore(question), FadeOut(background))
...
class HoldUpMainVideo(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        morty = self.teacher
        self.play(morty.change('raise_left_hand'), FadeIn(thumbnail, UP), self.change_students('happy', 'guilty', 'erm', look_at=thumbnail))
        self.wait(2)
        key_points = VGroup(Text('Key point #1: Phase kicks'), Text('Key point #2: Layer oscillations'), Text('Key point #3: Resonance'))
        key_points.scale(1.25)
        key_points.set_backstroke(BLACK, 4)
        ...
        self.play(MoveToTarget(thumbnail), morty.change('raise_right_hand', key_points), self.change_students('pondering', 'hesitant', 'pondering', look_at=thumbnail.target), LaggedStartMap(FadeIn, key_points, shift=0.5 * LEFT, lag_ratio=0.5))
        self.wait(2)
        self.play(Restore(key_points[0]), LaggedStartMap(FadeOut, key_points[1:], shift=DOWN), LaggedStartMap(FadeOut, self.pi_creatures, shift=DOWN), FadeOut(thumbnail, scale=0.5, shift=DOWN))
        self.wait()
...
class HoldUpBarberPoleVideos(HoldUpMainVideo):
    ...
    def construct(self):
        ...
        thumbnails = Group(*(get_bordered_thumbnail(os.path.join(folder, name), stroke_width=2) for name in ['CleanThumbnail', 'Part2Thumbnail6']))
        thumbnails.arrange(RIGHT, buff=1.0)
        thumbnails.set_height(2.5)
        thumbnails.move_to(self.hold_up_spot, DR)
        morty = self.teacher
        self.play(morty.change('raise_right_hand', thumbnails), LaggedStartMap(FadeIn, thumbnails, shift=0.5 * UP, lag_ratio=0.5), self.change_students('maybe', 'erm', 'happy', look_at=thumbnails))
        self.wait()
        self.play(self.change_students('confused', 'maybe'), morty.animate.look_at(self.students))
        self.wait()
...
class MissingDetails(InteractiveScene):
    ...
    def construct(self):
        ...
        title.add(Underline(title))
        title.set_color(BLUE)
        details = BulletedList('There will be many resonant frequencies', 'Our force law should include a damping term', 'Why the wave produced by a plane of charges is a \\\\' + 'quarter-cycle out of phase with the incoming light', 'Everything related to the magnetic field', 'Whether accounting for quantum effects makes a difference')
...
class SteadyStateSolutionCircledAmplitude(InteractiveScene):
    ...
    def construct(self):
        ...
        rect = SurroundingRectangle(equation['\\frac{q ||\\vec{\\textbf{E}}_0||}{m\\left(\\omega_r^2-\\omega_l^2\\right)}'])
...
class LimitToContinuity(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(GrowArrow(arrow), FadeIn(words))
        self.wait()

class KickForwardOrBackCondition(InteractiveScene):

    def construct(self):
        words = VGroup(TexText('If this $< 0$'), TexText('Refractive Index $< 1$', font_size=48))
        rect = Rectangle(2, 1)
        ...
        self.wait()
        freq_ineq = Tex('\\text{If } \\omega_l > \\omega_r', t2c=EQUATION_T2C)
        freq_ineq.move_to(words[0])
        lf_rect = SurroundingRectangle(freq_ineq['\\omega_l'])
        rf_rect = SurroundingRectangle(freq_ineq['\\omega_r'])
        lf_words = Text('Light frequency', font_size=36)
        rf_words = Text('Resonant frequency', font_size=36)
        ...
        self.play(FadeOut(words[0], 0.5 * UP), FadeIn(freq_ineq, 0.5 * UP))
        self.wait()
        self.play(FadeIn(lf_rect), FadeIn(lf_words, shift=0.25 * UP))
        self.wait()
        self.play(ReplacementTransform(lf_rect, rf_rect), FadeTransform(lf_words, rf_words))
        self.wait()
...
class ThisReallyDoesHappen(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.says('This really\ndoes happen!', mode='surprised'))
...
class LookAround(InteractiveScene):
    ...
    def construct(self):
        ...
        arrows = VGroup(*(Arrow(v, 2 * v, buff=0, stroke_width=8, stroke_color=GREY_B) for alpha in np.arange(0, 1, 1 / 12) for v in [circle.pfp(alpha)]))
        words1 = Text('Look\naround', font_size=90)
...
class CausalInfluence(InteractiveScene):
    ...
    def construct(self):
        words = TexText('No causal influence can \\\\ propagate faster than $c$', font_size=72)
        c = words['$c$']
        c.set_color(YELLOW)
        ...
        self.add(words)
        self.wait()
        self.play(GrowArrow(arrow), ShowCreation(rect), FlashAround(c))
        self.wait()
...
class SameRotationRate(InteractiveScene):
    ...
    def construct(self):
        ...
        words.to_edge(RIGHT)
        arrows = VGroup(*(Arrow(words.get_corner(corner), y * UP + 1.25 * RIGHT, buff=0.2) for y, corner in zip([2.75, 0, -2.75], [UL, LEFT, DL])))
