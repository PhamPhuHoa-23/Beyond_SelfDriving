...
class SlicedWave(Group):
    ...
    def __init__(self, axes, layer_xs, phase_kick_back=0, layer_height=4.0, damping_per_layer=1.0, wave_config=dict(), vect_wave_style=dict(), layer_style=dict()):
...
class SpeedInMediumFastPart(InteractiveScene):
    ...
    def construct(self):
        ...
        wave = OscillatingWave(axes, z_amplitude=self.z_amplitude, y_amplitude=self.y_amplitude, color=self.color, wave_len=self.wave_len, speed=self.speed)
...
class PhaseKickBacks(SpeedInMediumFastPart):
    ...
    kick_back_value = 0
    axes_config = dict(x_range=(-8, 8), y_range=(-4, 4))
    line_style = dict()
    wave_config = dict()
    ...
    def get_sliced_wave(self):
        return SlicedWave(self.get_axes(), self.get_layer_xs(), wave_config=self.wave_config, vect_wave_style=self.vect_wave_style, layer_style=self.line_style, phase_kick_back=self.kick_back_value, damping_per_layer=self.damping_per_layer)
...
class RevertToOneLayerAtATime(PhaseKickBacks):
    ...
    axes_config = dict(x_range=(-12, 12), y_range=(-4, 4))
    ...
    def construct(self):
        ...
        self.play(layers.animate.arrange(RIGHT, buff=0.3).move_to(ORIGIN, LEFT).set_stroke(width=2, opacity=1), FadeIn(rect), *(pkt.animate.set_value(0) for pkt in pkts), run_time=2)
        self.wait(3)
        left_layers = VGroup()
        for layer in layers:
        ...
        self.play(LaggedStart(*(layer.animate().set_stroke(opacity=0).shift(DOWN) for layer in left_layers[:0:-1]), **kw), layers[0].animate.set_stroke(width=2, opacity=1).set_height(5).set_anim_args(time_span=(4, 5)), FadeTransform(block_label, layer_label, time_span=(4, 5)), FadeOut(rect, time_span=(3, 5)))
        self.wait(4)
        ...
        wave.stop_clock()
        self.wait()
        self.play(pkts[0].animate.set_value(phase_kick), FadeIn(kick_label, LEFT))
        self.wait()
        form1 = Tex('A\\sin(kx)')
        form2 = Tex('A\\sin(kx + 1.00)')
        ...
        for value in [-0.01, phase_kick]:
            self.play(pkts[0].animate.set_value(value), ChangeDecimalToValue(pk_decimal, -value), run_time=2)
            self.wait()
        ...
        self.add(form2)
        self.play(pk_decimal.animate.match_height(pk_label).next_to(pk_label, RIGHT, 0.2, DOWN), ReplacementTransform(kick_words['Kick'].copy(), pk_label['kick']), ReplacementTransform(kick_words['phase'].copy(), pk_label['Phase']), FadeIn(pk_label['=']), run_time=2)
        pk_label.add(pk_decimal)
        self.add(pk_label)
        self.play(FadeOut(form1), FadeOut(form2))
        ...
        for layer, pkt in zip(layers[nls::nls], pkts[nls::nls]):
            ...
            layer_arrows.target = VGroup(*(Arrow(layer_label.target.get_bottom(), sl.get_top(), buff=0.1, stroke_width=2, stroke_opacity=0.5) for sl in shown_layers))
            self.play(FadeOut(kick_label), GrowFromCenter(layer), MoveToTarget(layer_label), MoveToTarget(layer_arrows))
        ...
        for value in [-0.01, self.exagerated_phase_kick]:
            phase_kick = value
            self.play(ChangeDecimalToValue(pk_decimal, -phase_kick), *(pkt.animate.set_value(phase_kick) for pkt in pkts[::nls]), run_time=4)
            self.wait(2)
        self.wait(2)
        ...
        while nls > 1:
            ...
            self.play(LaggedStart(*(GrowFromCenter(layer) for layer in new_layers), **kw), old_layers.animate.set_stroke(width=stroke_width, opacity=opacity), LaggedStart(*(pkt.animate.set_value(phase_kick) for pkt in pkts[::nls]), **kw), LaggedStart(FadeOut(nl_decimal, 0.2 * UP), FadeIn(new_nl_decimal, 0.2 * UP), FadeOut(pk_decimal, 0.2 * UP), FadeIn(new_pk_decimal, 0.2 * UP), lag_ratio=0.1, run_time=1))
...
class SlowedAndAbsorbed(RevertToOneLayerAtATime):
    ...
    kick_back_value = -0.2
    damping_per_layer = 1 - 0.02
    wave_config = dict(color=YELLOW, sample_resolution=0.001)
    line_style = dict(stroke_color=BLUE_B, stroke_width=1.5, stroke_opacity=0.5)
    ...
    def construct(self):
        ...
        mu_label = Tex('\\mu').set_color(PINK)
        ...
        for at in ats:
            at.set_value(1)
        self.play(FadeIn(label, time_span=(0, 2)), FlashAround(abs_words, color=PINK, time_span=(2, 4)), abs_words.animate.set_color(PINK).set_anim_args(time_span=(2, 4)), FadeIn(mu_line), FadeIn(mu_indicator), FadeIn(mu_label), LaggedStart(*(pkt.animate.set_value(self.kick_back_value) for pkt in pkts)), LaggedStart(*(at.animate.set_value(self.damping_per_layer) for at in ats)), LaggedStart(*(FadeIn(layer, scale=0.8) for layer in layers)), run_time=5)
        for mu in [0.1, 1.0]:
            self.play(mu_tracker.animate.set_value(mu), *(at.animate.set_value(1 - mu * 0.02) for at in ats), run_time=3)
        self.wait(17)
...
class PlayWithIndex(RevertToOneLayerAtATime):
    ...
    def construct(self):
        ...
        self.add(equation)
        self.add(arrow)
        speed_label = Tex('\\text{Speed} = c / 1.00')
...
class RedLight(RevertToOneLayerAtATime):
    wave_config = dict(color=RED, stroke_width=6, sample_resolution=0.001, wave_len=4.0)
...
class XRay(RedLight):
    wave_config = dict(color=YELLOW, stroke_width=6, sample_resolution=0.001, wave_len=1.0)
...
class DissolveLayers(PhaseKickBacks):
    ...
    def construct(self):
        ...
        self.play(frame.animate.scale(0.2).move_to(layers).set_anim_args(run_time=4), FadeIn(fade_rect, time_span=(1, 3)), FadeIn(layers_copy, time_span=(1, 3)))
        self.wait()
        self.play(frame.animate.to_default_state().set_anim_args(run_time=2), FadeOut(fade_rect), FadeOut(layers_copy))
        self.wait(2)
        kw = dict(run_time=8, lag_ratio=0.1)
        self.play(LaggedStart(*(layer.animate().set_stroke(WHITE, 2, 0).set_height(5) for layer in layers[:0:-1]), **kw), LaggedStart(*(pkt.animate.set_value(0) for pkt in pkts[:0:-1]), **kw), layers[0].animate.set_stroke(WHITE, 2).set_height(5).set_anim_args(time_span=(7, 8)))
        self.wait(3)
...
class IntroducePhaseKickBack(PhaseKickBacks):
    ...
    def construct(self):
        ...
        self.add(wave)
        self.add(*pkts)
        self.play(GrowFromCenter(layers[0]))
        arrow = Vector(2 * LEFT, stroke_width=8)
        arrow.next_to(wave, UP, buff=0.75)
        arrow.set_x(0, LEFT).shift(0.5 * RIGHT)
        phase_kick = -0.5
        self.play(pkts[0].animate.set_value(phase_kick), FadeIn(arrow, LEFT))
        self.play(FadeOut(arrow))
        for layer, pkt in zip(layers[1:], pkts[1:]):
            arrow.align_to(layer, LEFT).shift(0.25 * RIGHT)
            self.play(GrowFromCenter(layer), FadeIn(arrow, LEFT), pkt.animate.set_value(phase_kick))
...
class PhaseKickBackAddInLayers(PhaseKickBacks):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(*(FadeIn(line, 0.5 * DOWN) for line in sliced_wave.layers), **lag_kw), LaggedStart(*(pkt.animate.set_value(self.target_kick_back) for pkt in sliced_wave.phase_kick_trackers), **lag_kw))
        self.wait(8)

class DensePhaseKickBacks25(PhaseKickBackAddInLayers):
    n_layers = 25
    target_kick_back = -0.4
    line_style = dict(stroke_width=1.0, stroke_color=BLUE_B)
...
class DensePhaseKickBacks100(PhaseKickBackAddInLayers):
    ...
    line_style = dict(stroke_width=1.5, stroke_color=BLUE_C, stroke_opacity=0.7)
...
class KickForward(PhaseKickBacks):
    ...
    kick_back_value = 0.25
    line_style = dict(stroke_color=BLUE, stroke_width=2, stroke_opacity=0.75)
    wave_config = dict(color=YELLOW, sample_resolution=0.001)
    time_per_layer = 0.25
    ...
    def construct(self):
        ...
        wave.stop_clock()
        self.remove(layers)
        self.play(ShowCreation(layers[0]), FadeIn(layer_label, lag_ratio=0.1))
        self.wait()
        kick_back = VGroup(Vector(LEFT, stroke_width=6), Text('Kick back\nthe phase', font_size=36))
        kick_back.set_color(RED)
        kick_forward = VGroup(Vector(RIGHT, stroke_width=6), Text('Kick forward\nthe phase', font_size=36, t2s={'forward': ITALIC}))
        ...
        self.play(pkts[0].animate.set_value(-0.75), FadeIn(kick_back, LEFT))
        self.wait()
        self.play(FadeOut(kick_back))
        self.play(pkts[0].animate.set_value(self.kick_back_value), FadeIn(kick_forward, RIGHT))
        self.wait()
        self.play(FadeOut(layer_label))
        wave.start_clock()
        remaining_layers = layers[1:]
        self.play(ShowIncreasingSubsets(remaining_layers, rate_func=linear), UpdateFromFunc(kick_forward, lambda m: m.align_to(remaining_layers.get_right(), LEFT).shift(MED_SMALL_BUFF * RIGHT)), LaggedStart(*(pkt.animate.set_value(self.kick_back_value) for pkt in pkts[1:]), lag_ratio=1), run_time=len(remaining_layers) * self.time_per_layer)
        self.wait(20)

class DenserKickForward(KickForward):
    layer_xs = np.arange(0, 8, 0.1)
    kick_back_value = 0.1
    line_style = dict(stroke_color=BLUE, stroke_width=1.5, stroke_opacity=0.75)
    time_per_layer = 0.1
...
class DensestKickForward(DenserKickForward):
    ...
    line_style = dict(stroke_color=BLUE, stroke_width=1.0, stroke_opacity=0.65)