...
class WaveIntoMedium(TimeVaryingVectorField):
    ...
    def __init__(self, interface_origin=ORIGIN, interface_normal=DR, prop_direction=RIGHT, index=1.5, c=2.0, frequency=0.25, amplitude=1.0, x_density=5.0, y_density=5.0, width=15.0, height=15.0, norm_to_opacity_func=lambda n: np.tanh(n), **kwargs):
        ...
        super().__init__(time_func, x_density=x_density, y_density=y_density, width=width, height=height, norm_to_opacity_func=norm_to_opacity_func, **kwargs)

class ScalarFieldByOpacity(DotCloud):

    def __init__(self, opacity_func, width=15, height=8, density=10, color=WHITE):
        step = 1.0 / density
        radius = step / 2.0
        points = np.array([[x, y, 0] for x in np.arange(-width / 2, width / 2 + step, step) for y in np.arange(-height / 2, height / 2 + step, step)])
...
class WavesByOpacity(ScalarFieldByOpacity):
    ...
    def __init__(self, wave: VectorField, vects_to_opacities=lambda v: np.tanh(v[:, 2]), **kwargs):
        super().__init__(opacity_func=lambda p: vects_to_opacities(wave.func(p)), **kwargs)
...
class SnellsLaw(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(beam, **anim_kw), UpdateFromFunc(glow, lambda m: m.move_to(beam.get_end())))
        self.wait()
        ...
        ineq = Tex('\\theta_1 > \\theta_2')
        ineq.move_to(FRAME_WIDTH * RIGHT / 4 + 3 * UP)
        self.play(LaggedStart(ShowCreation(normal_line), ShowCreation(arc1), ShowCreation(arc2), Write(theta1_label), Write(theta2_label), lag_ratio=0.1, run_time=1), FadeIn(ineq, UP))
        self.wait()
        ...
        self.play(tank.animate.move_to(beam.pfp(0.49)).set_anim_args(rate_func=linear, run_time=6), VFadeIn(tank), self.frame.animate.set_height(4).set_anim_args(time_span=(4, 6)))
        self.play(tank.animate.rotate(get_theta2() - get_theta1()).move_to(hit_point + 0.3 * UP + 0.1 * LEFT), rate_func=linear, run_time=1.5)
        self.play(tank.animate.move_to(beam.pfp(0.65)).set_anim_args(rate_func=linear, run_time=8), VFadeOut(tank, time_span=(7, 8)), self.frame.animate.set_height(8).set_anim_args(time_span=(6, 8)))
        self.wait()
        law = Tex('{\\sin(\\theta_1) \\over v_1} = {\\sin(\\theta_2) \\over v_2}', font_size=52)
        title = TexText("Snell's Law", font_size=60)
        group = VGroup(title, law)
        group.arrange(DOWN, buff=0.75)
        group.to_corner(UR)
        self.play(FadeIn(angle_labels))
        self.play(Write(title), FlashUnder(title, run_time=2), FadeOut(ineq['>']), Transform(ineq['\\theta_1'], law['\\theta_1']), Transform(ineq['\\theta_2'], law['\\theta_2']))
        self.play(FadeIn(law))
        self.wait()
        tl1_copy = theta1_label.copy()
        tl2_copy = theta2_label.copy()
        self.play(LaggedStart(Transform(tl1_copy, law['\\theta_1']), Transform(tl2_copy, law['\\theta_2']), lag_ratio=0.5))
        ...
        def get_dot_anim():
            dot = GlowDot()
            return Succession(MoveAlongPath(dot, in_beam, rate_func=linear, run_time=3), MoveAlongPath(dot, out_beam, rate_func=linear, run_time=3), remover=True)
        self.play(LaggedStart(*(get_dot_anim() for x in range(50)), lag_ratio=0.02, run_time=20))
    ...
    def get_beam(self, angle_tracker, hit_point, stroke_color=YELLOW, stroke_width=3):
        ...
        def update_beam(beam):
            ...
            beam.set_points_as_corners([hit_point + 0.6 * FRAME_HEIGHT * rotate_vector(UP, theta1) / math.cos(theta1), hit_point, hit_point + 0.6 * FRAME_HEIGHT * rotate_vector(DOWN, theta2) / math.cos(theta2)])
...
class WavesIntoAngledMedium(InteractiveScene):
    ...
    def get_medium(self, width=10, height=30, depth=5, color=BLUE, opacity=0.2):
        ...
        medium.rotate(angle_of_vector(self.interface_normal), about_point=self.interface_origin)
        return medium

    def get_wave(self, **kwargs):
        config = dict(interface_origin=self.interface_origin, interface_normal=self.interface_normal, prop_direction=self.prop_direction, frequency=self.frequency, c=self.c, index=self.index, max_vect_len=np.inf, amplitude=self.amplitude, norm_to_opacity_func=lambda n: sigmoid(n))
        config.update(kwargs)
        return WaveIntoMedium(**config)

    def get_wave_dots(self, wave, density=20, offset=0.2, color=WHITE, max_opacity=1.0, **kwargs):
        return WavesByOpacity(wave, density=density, vects_to_opacities=lambda v: max_opacity * np.tanh(v[:, 2] - offset * self.amplitude), color=color, **kwargs)
...
class TransitionToOverheadView(WavesIntoAngledMedium):
    ...
    def construct(self):
        ...
        plane = NumberPlane(background_line_style=dict(stroke_color=BLUE_D, stroke_width=1, stroke_opacity=1))
        plane.axes.set_stroke(width=1)
        plane.fade(0.5)
        self.add(plane, medium, wave_1d)
        frame.reorient(-37, 61, 0)
        self.play(frame.animate.reorient(0, 90).set_height(6), run_time=5)
        self.wait(10)
        ...
        brace1.add(brace1.get_tex(f'\\lambda = {wave_len}'))
        brace2 = Brace(Line(ORIGIN, 0.6 * mult * wave_len * RIGHT), UP)
        brace2.add(brace2.get_tex(f'\\lambda = {0.6 * wave_len}'))
        ...
        self.play(FadeOut(brace1, RIGHT), FadeOut(brace2, 0.6 * RIGHT))
        self.wait(3)
        wave_2d = self.get_wave(width=20.0, height=8.0, norm_to_opacity_func=lambda n: 0.5 * sigmoid(2 * n))
        wave_2d.set_stroke(YELLOW)
        invisible_wave_2d = self.get_wave(width=20.0, height=12.0, norm_to_opacity_func=None, stroke_opacity=0)
        wave_dots = self.get_wave_dots(invisible_wave_2d)
        self.remove(wave_1d)
        wave_2d.time = wave_1d.time
        self.add(wave_2d)
        self.play(frame.animate.reorient(-10, 45).set_height(8).set_anim_args(run_time=7, time_span=(1, 7)))
        self.wait(2)
        self.wait(8)
        self.remove(wave_2d)
        ...
        self.wait(2)
        self.play(frame.animate.reorient(0, 0), plane.animate.fade(0.5), medium.animate.set_opacity(0.2), run_time=4)
        self.wait(8)
        ...
        self.play(FadeIn(wave_1d), frame.animate.reorient(0, 70), braces.animate.rotate(90 * DEGREES, RIGHT, about_point=ORIGIN).shift(0.3 * OUT), run_time=5)
        self.wait()
...
class AngledMediumAnnotations(InteractiveScene):
    ...
    def construct(self):
        ...
        lane_points = [h_lines[0].get_left(), h_lines[0].get_right(), diag_lines[0].get_end(), diag_lines[2].get_end(), h_lines[2].get_right(), h_lines[2].get_left()]
        ...
        self.play(ShowCreation(beam, run_time=3), VShowPassingFlash(beam.copy().set_stroke(width=5), run_time=3))
        self.wait()
...
class LineGame(InteractiveScene):
    ...
    def construct(self):
        ...
        low_lines = always_redraw(lambda: self.get_line_group(interface, rotate_vector(dial.get_vector(), -PI / 2), small_spacing, line_color=GREEN, dot_color=YELLOW))
        top_spacing_label = self.get_spacing_label(top_lines[1], '\\lambda_1')
        low_spacing_label = self.get_spacing_label(low_lines[1], '\\lambda_2')
        ...
        self.play(top_lines.animate.fade(0.8), FadeIn(low_lines, lag_ratio=0.1))
        self.play(Write(low_spacing_label))
        self.wait()
        dial.set_stroke(opacity=0)
        self.play(Rotate(dial, -key_angle, run_time=2, remover=True), Rotate(low_spacing_label, -key_angle, run_time=2, about_point=ORIGIN))
        dial.set_stroke(opacity=1)
        self.wait()
        self.highlight_intersection_points(low_lines, YELLOW, RIGHT)
        self.play(Restore(top_lines))
        self.add(low_lines)
        self.play(FadeIn(circ), FadeIn(dial))
        low_lines.resume_updating()
        for angle in [-15, -15, 10.1]:
            self.play(Rotate(dial, angle * DEGREES), Rotate(low_spacing_label, angle * DEGREES, about_point=ORIGIN), run_time=3)
            self.wait()
        ...
        theta1_label = Tex('\\theta_1', font_size=36)
        theta2_label = Tex('\\theta_2', font_size=36)
        theta1_label.next_to(arc1.pfp(0.5), UP, SMALL_BUFF)
        theta2_label.next_to(arc2.pfp(0.75), DL, SMALL_BUFF)
        self.play(ShowCreation(arc1), Write(theta1_label), self.frame.animate.set_height(6.5).set_anim_args(run_time=2), FadeOut(circ), FadeOut(dial))
        self.wait()
        self.play(ShowCreation(arc2), Write(theta2_label))
        self.wait()

    def get_line_group(self, interface, line_direction=UP, spacing=1.0, n_lines=17, length=30, line_color=WHITE, line_stroke_width=2, dot_radius=0.05, dot_color=RED, dot_opacity=1):
        ...
        points = [interface_center + i * dot_spacing * interface_vect for i in range(-n_lines // 2, n_lines // 2 + 1)]
        dots = VGroup(*(Dot(point, radius=dot_radius).set_fill(dot_color, dot_opacity) for point in points))
        lines = VGroup(*(Line(point, point + length * line_vect, stroke_color=line_color, stroke_width=line_stroke_width) for point in points))
    ...
    def get_spacing_label(self, lines, label_tex):
        ...
        x2 = line2.get_x()
        dist = x2 - x1
        arrows = VGroup(Arrow(dist * LEFT / 4, dist * RIGHT / 2, buff=SMALL_BUFF), Arrow(dist * RIGHT / 4, dist * LEFT / 2, buff=SMALL_BUFF))
        label = Tex(label_tex)
    ...
    def highlight_intersection_points(self, line_group, color, arrow_direction=LEFT):
        ...
        arrows = VGroup(*(Vector(arrow_direction).next_to(point, -arrow_direction, SMALL_BUFF) for point in points))
        arrows.set_color(color)
        self.play(LaggedStartMap(GrowArrow, arrows, lag_ratio=0.25))
        self.play(LaggedStartMap(Indicate, line_group[0], scale_factor=2, color=color, lag_ratio=0.2), LaggedStartMap(FlashAround, line_group[0], buff=0.2, color=color, lag_ratio=0.2), run_time=4)
...
class Prism(InteractiveScene):
    ...
    def construct(self):
        ...
        def get_beams(light_in):
            return self.get_beams(min_index=1.3, max_index=1.45, n_beams=200, in_beam=light_in, in_edge=in_edge, out_edge=out_edge)
        beams = always_redraw(lambda: get_beams(in_beam))
        self.play(ShowCreation(beams, time_span=(0.5, 1.5), lag_ratio=0), ShowCreation(in_beam, time_span=(0, 1.0)), rate_func=linear)
        ...
        self.wait()
        self.remove(x_ray)
        self.play(FadeOut(flat_prism), FadeIn(prism), frame.animate.reorient(-90, 35, 90).set_height(15).move_to(RIGHT + 4 * DOWN), run_time=4)
        turn_animation_into_updater(ApplyMethod(frame.reorient, -90, 10, 90, run_time=15))
        for vect, alpha in zip([5 * DOWN, 3 * DOWN, 4 * DOWN], [0.3, 0.3, 0.5]):
            self.play(in_beam.animate.put_start_and_end_on(left_side + vect, in_edge.pfp(alpha)))
        frame.clear_updaters()
        self.play(frame.animate.reorient(0, 0, 0).set_height(8), in_beam.animate.put_start_and_end_on(left_side + 2 * DOWN, in_edge.get_center()), FadeOut(prism, time_span=(1, 2)), FadeIn(flat_prism, time_span=(1, 2)), run_time=2)
        self.wait()
    ...
    def get_beams(self, min_index, max_index, n_beams, in_beam, in_edge: Line, out_edge: Line):
        ...
        vect2s = np.array([rotate_vector(normal1, theta2) for theta2 in theta2s])
        out_points = np.array([find_intersection(in_point, vect2, out_edge.get_start(), out_edge.get_vector()) for vect2 in vect2s])
        theta3s = np.array([angle_between_vectors(normal2, vect2) for vect2 in vect2s])
        theta4s = np.arcsin(np.sin(theta3s) * indices)
        vect3s = np.array([rotate_vector(normal2, -theta4) for theta4 in theta4s])
        beams = VGroup(*(VMobject().set_points_as_corners([in_point - 1.0 * FRAME_WIDTH * vect1, in_point, out_point, out_point + 3.0 * FRAME_WIDTH * vect3]) for out_point, vect3 in zip(out_points, vect3s)))
