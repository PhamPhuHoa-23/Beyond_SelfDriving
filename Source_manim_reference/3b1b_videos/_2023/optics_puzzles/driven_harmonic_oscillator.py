...
class HarmonicOscillator(TrueDot):
    ...
    def __init__(self, center=np.zeros(3), initial_velocity=np.zeros(3), k=20.0, damping=0.1, mass=1.0, radius=0.5, color=BLUE, three_d=True, **kwargs):
        self.k = k
        self.mass = mass
...
class Spring(VMobject):
    ...
    def __init__(self, mobject, base_point, edge=ORIGIN, stroke_color=GREY, stroke_width=2, twist_rate=8.0, n_twists=8, radius=0.1, lead_length=0.25, **kwargs):
        super().__init__(**kwargs)
        helix = ParametricCurve(lambda t: [radius * math.cos(TAU * t), radius * math.sin(TAU * t), t / twist_rate], t_range=(0, n_twists, 0.01))
...
class DynamicPlot(VMobject):
    ...
    def __init__(self, axes, func, stroke_color=RED, stroke_width=3):
        self.axes = axes
        self.func = func
        self.time = 0
...
class DrivenHarmonicOscillator(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(-90, 0, 90).center().set_focal_distance(100).set_anim_args(time_span=(0, 4)), charges.animate.center().set_height(charges_target_height).set_radius(radius).shift(in_shift), run_time=6)
        frame.reorient(0, 0, 0)
        sho = HarmonicOscillator(center=charges.get_center() - in_shift, radius=radius, color=charges.get_color())
        ...
        springs = VGroup(*(Spring(sho, sho.get_center() + (spacing - small_radius) * vect) for vect in compass_directions(4)))
        sho.add_updater(lambda m: springs.update())
        self.add(cover, springs, sho)
        self.play(charges.animate.set_opacity(0.5).set_radius(small_radius), FadeIn(springs))
        self.wait()
        ...
        t2c = {'\\vec{\\textbf{x}}(t)': RED, '\\vec{\\textbf{v}}(t)': PINK, '\\vec{\\textbf{F}}(t)': YELLOW, '\\vec{\\textbf{a}}(t)': YELLOW, '\\frac{d^2 \\vec{\\textbf{x}}}{dt^2}(t) ': YELLOW, '\\omega_r': PINK, '\\omega_l': TEAL}
        x_vect = always_redraw(lambda: Arrow(axes.c2p(0, 0), sho.get_center(), stroke_width=5, stroke_color=RED, buff=0))
        equation = Tex('\\vec{\\textbf{F}}(t) = -k \\vec{\\textbf{x}}(t)', t2c=t2c)
        equation.move_to(axes.c2p(0.5, 0.5), LEFT)
        x_label = equation['\\vec{\\textbf{x}}(t)']
        ...
        self.wait()
        self.play(Write(equation['\\vec{\\textbf{F}}(t) = -k ']), Restore(x_label))
        self.add(equation)
        ...
        self.play(FlashAround(equation['\\vec{\\textbf{F}}']), ReplacementTransform(x_vect, F_vect, path_arc=PI))
        self.wait()
        ...
        self.wait(6)
        up_shift = 1.5 * UP
        plot_rect, plot_axes, plot = self.get_plot_group(lambda: np.sign(sho.get_center()[1]) * get_norm(sho.get_center()))
        ...
        self.play(frame.animate.shift(up_shift), equation.animate.to_corner(UL).match_y(plot_rect), FadeIn(plot_rect), FadeIn(plot_axes), FadeOut(charges), run_time=2)
        self.wait(15)
        ...
        equations.add(Tex('m \\vec{\\textbf{a}}(t) = -k \\vec{\\textbf{x}}(t)', **tex_kw), Tex('\\frac{d^2 \\vec{\\textbf{x}}}{dt^2}(t) = -{k \\over m} \\vec{\\textbf{x}}(t)', **tex_kw), Tex('\\vec{\\textbf{x}}(t) = \\vec{\\textbf{x}}_0 \\cos( \\sqrt{k \\over m} \\cdot t)', **tex_kw), Tex('\\vec{\\textbf{x}}(t) = \\vec{\\textbf{x}}_0 \\cos(\\omega_r t)', **tex_kw))
        ...
        implies = Tex('\\Downarrow').replicate(2)
        implies[0].move_to(VGroup(eq1, eq2))
        implies[1].move_to(VGroup(eq2, eq3))
        implies[1].match_x(implies[0])
        eq1_copy = eq1.copy()
        self.play(TransformMatchingTex(eq1_copy, eq2, matched_pairs=[(eq1_copy['\\vec{\\textbf{F}}(t)'], eq2['\\vec{\\textbf{a}}(t)']), (eq1_copy['= -k \\vec{\\textbf{x}}(t)'], eq2['= -k \\vec{\\textbf{x}}(t)'])], run_time=1), FadeIn(implies[0], 0.5 * DOWN))
        self.wait()
        eq2_copy = eq2.copy()
        self.play(TransformMatchingTex(eq2_copy, eq3, matched_pairs=[(eq2_copy['\\vec{\\textbf{a}}(t)'], eq3['\\frac{d^2 \\vec{\\textbf{x}}}{dt^2}(t)']), (eq2_copy['k'], eq3['k'])], path_arc=PI / 4), FadeIn(implies[1]))
        self.wait()
        self.play(eq3.animate.move_to(eq2).align_to(eq1, LEFT), FadeOut(eq2), FadeOut(implies[1]))
        initial_position = 0.5 * UR
        x0 = eq4['\\vec{\\textbf{x}}_0']
        cos_part = eq4['\\cos( \\sqrt{k \\over m} \\cdot t)']
        x0_copy = x0.copy()
        ...
        self.wait()
        self.play(TransformFromCopy(x0_copy, x0), Write(implies[1]), *(TransformFromCopy(eq3[tex], eq4[tex]) for tex in ['=', '\\vec{\\textbf{x}}(t)']))
        self.play(FadeIn(cos_part, lag_ratio=0.1))
        self.play(FlashAround(cos_part, color=RED, run_time=3, time_width=1.5))
        self.wait()
        self.play(FadeTransform(ic_label, x0_rect))
        self.play(plot.animate.stretch(0.5, 1), sho.animate.move_to(0.5 * initial_position), x0_copy.animate.next_to(0.25 * initial_position, UL, SMALL_BUFF), rate_func=there_and_back_with_pause, run_time=6)
        ...
        sho.move_to(initial_position)
        sho.reset_velocity()
        self.play(ReplacementTransform(k_rect, m_rect), FadeOut(k_rect.arrow), GrowArrow(m_rect.arrow))
        self.wait(8)
        sho.set_k(original_k)
        omega0_eq = Tex('\\omega_r = \\sqrt{k / m}')
        omega0_eq['\\omega_r'].set_color(PINK)
        ...
        plot.reset()
        self.play(FadeOut(m_rect), FadeOut(m_rect.arrow))
        self.play(TransformFromCopy(sqrt_km, omega0_eq['\\sqrt{k / m}']), Write(omega0_eq['\\omega_r = ']), TransformMatchingTex(eq4, eq5))
        self.wait(2)
        ...
        sho.suspend_updating()
        self.play(FadeIn(corner_box), MoveToTarget(free_solution), *map(FadeOut, [eq3, implies[1], omega0_name]), sho.animate.center())
        corner_box.push_self_into_submobjects()
        corner_box.add(free_solution)
        self.wait()
        self.play(FadeOut(plot_group1), frame.animate.shift(-up_shift), corner_box.animate.shift(-up_shift), run_time=2)
        corner_box.fix_in_frame()
        free_label = Text('No external\nforces', font_size=36)
        free_label.next_to(corner_box, DOWN)
        t2c['\\vec{\\textbf{E}}_0'] = BLUE_D
        driven_eq = Tex('\n                \\vec{\\textbf{F}}(t) =\n                - k \\vec{\\textbf{x}}(t)\n                + \\vec{\\textbf{E}}_0 q \\cos(\\omega_l t)\n            ', t2c=t2c)
        driven_eq.to_edge(UP)
        driven_eq.set_x(FRAME_WIDTH / 4)
        external_force = driven_eq['\\vec{\\textbf{E}}_0 q \\cos(\\omega_l t)']
        ...
        driven_eq_group = VGroup(BackgroundRectangle(driven_eq, buff=0.5).set_fill(BLACK, 0.9), BackgroundRectangle(external_force_label, buff=0.5).set_fill(BLACK, 0.9), driven_eq, external_force_rect, external_force_label)
        ...
        def time_func(points, time):
            ...
            return result
        field_config = dict(stroke_color=TEAL, stroke_width=3, stroke_opacity=0.5, max_vect_len=1.0, x_density=1.0, y_density=1.0)
        ...
        full_field = TimeVaryingVectorField(time_func, depth=16, z_density=5, height=5, width=5, norm_to_opacity_func=lambda n: n, **field_config)
        ...
        self.play(VFadeIn(planar_field), FadeOut(corner_box, time_span=(0, 1)), FadeOut(free_label, time_span=(0, 1)))
        self.wait(12)
        ...
        self.play(full_field_opacity_mult.animate.set_value(0), VFadeIn(z_axis_field), run_time=2)
        self.remove(full_field)
        self.play(frame.animate.reorient(-100, 80, 90), run_time=6)
        planar_field.set_stroke(opacity=0.5)
        self.play(frame.animate.reorient(-90, 0, 90).set_focal_distance(100).set_height(8), VFadeOut(z_axis_field), VFadeIn(planar_field), run_time=4)
        self.wait(4)
        full_field.time = planar_field.time
        frame.set_focal_distance(10)
        self.add(full_field)
        self.play(frame.animate.reorient(-100, 80, 90).set_height(10), full_field_opacity_mult.animate.set_value(1).set_anim_args(time_span=(2, 4)), VFadeOut(planar_field, time_span=(2, 4), remover=False), run_time=4)
        planar_field.set_stroke(opacity=0)
        self.remove(driven_eq_group)
        full_field_opacity_mult.set_value(0)
        frame.to_default_state().reorient(-90, 0, 90)
        self.play(full_field_opacity_mult.animate.set_value(1).set_anim_args(time_span=(1, 4)), frame.animate.reorient(-95, 60, 90).set_height(10), run_time=4)
        ...
        self.play(frame.animate.reorient(-90, 0, 90).set_height(8), full_field_opacity_mult.animate.set_value(0).set_anim_args(time_span=(8, 10)), VFadeIn(planar_field, time_span=(8, 10)), run_time=12)
        self.add(driven_eq_group)
        self.wait(5)
        up_shift = UP
        ...
        sho.suspend_updating()
        self.play(frame.animate.shift(up_shift), FadeIn(plot_rect), FadeIn(plot_axes), driven_eq_group.animate.shift(1.0 * DOWN), sho.animate.center(), VFadeOut(planar_field, time_span=(0, 1)), run_time=2)
        self.wait()
        planar_field.time = 0
        sho.resume_updating()
        plot.reset()
        ...
        plot.suspend_updating()
        self.wait(3)
        self.play(VFadeOut(axes), VFadeOut(planar_field), FadeOut(sho), VFadeOut(springs))
        down_shift = 2.5 * DOWN
        plot_group2.add(*plot_boxes)
        plot_group1.next_to(plot_group2, UP, aligned_edge=LEFT).shift(down_shift)
        top_axes = plot_group1[1]
        VGroup(top_axes.x_axis, plot_group1[2]).stretch(plot_axes.x_axis.get_unit_size() / top_axes.x_axis.get_unit_size(), 0, about_edge=LEFT)
        ...
        self.play(FadeIn(plot_group1), FadeIn(corner_box), plot_group2.animate.shift(down_shift), driven_eq.animate.shift(down_shift), FadeOut(driven_eq_group[-2:], down_shift), run_time=3)
        self.wait()
        omega0_eq_copy = omega0_eq.copy()
        omega_copy = driven_eq['\\omega_l'].copy()
        ...
        self.play(MoveToTarget(driven_eq), FadeIn(implies, LEFT), FadeIn(solution, RIGHT))
        self.wait()
        full_rect = SurroundingRectangle(solution)
        amp_rect = SurroundingRectangle(solution['\\frac{q \\vec{\\textbf{E}}_0}{m\\left(\\omega_r^2-\\omega_l^2\\right)}'])
        E_rect = SurroundingRectangle(solution['\\vec{\\textbf{E}}_0'])
        q_rect = SurroundingRectangle(solution[6])
        freq_diff_rect = SurroundingRectangle(solution['\\omega_r^2-\\omega_l^2'], buff=0.05)
        ...
        self.wait()
        self.play(ReplacementTransform(full_rect, amp_rect), FadeOut(steady_state_rect))
        self.wait()
        ...
        to_fade = VGroup(plot_group1, omega0_eq_copy, corner_box, driven_eq, implies)
        ...
        sho.center()
        sho.suspend_updating()
        self.play(frame.animate.shift(UP), solution.animate.move_to(top_rect), MaintainPositionRelativeTo(freq_diff_rect, solution), MoveToTarget(plot_group2), FadeOut(to_fade, UP), FadeIn(sho), VFadeIn(springs), VFadeIn(planar_field), run_time=2)
        close_freq_words = Tex('\\text{If } \\omega_l \\approx \\omega_r', t2c=t2c)
        close_freq_words.next_to(plot_rect, DOWN, aligned_edge=LEFT)
        self.add(*plot_group2)
        self.play(LaggedStart(FadeOut(plot_boxes), FadeOut(plot), FadeOut(omega_copy), FadeIn(close_freq_words), Transform(freq_diff_rect, SurroundingRectangle(close_freq_words).set_stroke(width=0), remover=True), *(TransformFromCopy(solution[tex][0], close_freq_words[tex][0]) for tex in ['\\omega_r', '\\omega_l'])))
        self.wait()
        self.play(plot_axes.y_axis.animate.stretch(0.75, 1), plot_axes[-1].animate.shift(0.1 * DOWN + 0.4 * LEFT))
        sho.set_k(16)
        sho.set_damping(0.25)
        sho.center()
        sho.reset_velocity()
        ...
        self.wait(30)
        half = Tex('0.5')
        omega_r = close_freq_words['\\omega_r'][0]
        half.move_to(omega_r, LEFT)
        half.align_to(omega_r[0], DOWN)
        sho.suspend_updating()
        plot.suspend_updating()
        self.play(sho.animate.center(), planar_field.animate.set_opacity(0), FadeOut(plot))
    ...
    def scrap(self):
        ...
        self.clear()
        self.add(driven_eq)
        self.play(ShowCreation(external_force_rect), FadeIn(external_force_label, lag_ratio=0.1))
        damp_term = Tex('- \\mu \\vec{\\textbf{v}}(t)', t2c=t2c)
        ...
        up_shift = 1.5 * UP
        plot_rect, plot_axes, plot = self.get_plot_group(lambda: np.sign(sho.get_center()[1]) * get_norm(sho.get_center()), width=14, max_t=20)
        plot_group1 = VGroup(plot_rect, plot_axes, plot)
        plot_group1.to_corner(UR, buff=0.1)
        plot_group1.shift(up_shift)
        sho.move_to(initial_position)
    ...
    def get_plot_group(self, func, width=10.0, height=2.0, max_t=12.0):
        ...
        y_axis_label = Tex('x(t)', font_size=20)
...
class JigglesInCalcite(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(1, 84, 0).move_to([-0.08, -0.16, -0.53]).set_height(9), run_time=3)
        self.wait()
        spring_length = 2.5
        springs = VGroup(*(Spring(calcium, calcium.get_center() + spring_length * (v_vect + 0.2 * h_vect), edge=v_vect) for v_vect in [UP, DOWN] for h_vect in [LEFT, ORIGIN, RIGHT]), *(Spring(calcium, calcium.get_center() + spring_length * h_vect, edge=h_vect) for h_vect in [LEFT, RIGHT]))
        springs.set_stroke(opacity=0.7)
        self.play(VFadeIn(springs), calcium.animate.shift(RIGHT), calcite.balls.animate.set_opacity(0.1), frame.animate.reorient(-2, 25, 0).move_to(calcium).set_height(6), run_time=2)
        ...
        def time_func(points, time):
            ...
            return result
        field_config = dict(stroke_color=TEAL, stroke_width=3, stroke_opacity=0.5, max_vect_len=1.0, x_density=1.0, y_density=1.0, center=calcium_center)
        z_axis_field = TimeVaryingVectorField(time_func, height=0, width=0, depth=16, z_density=5, **field_config)
        ...
        self.play(VFadeIn(z_axis_field), frame.animate.reorient(108, 46, -102).move_to(calcium).set_height(12), run_time=3)
        self.wait(25)
...
class SpiralPaths(InteractiveScene):
    ...
    def construct(self):
        ...
        helix = ParametricCurve(lambda t: [math.cos(self.sign * t), math.sin(self.sign * t), 0.25 * t], t_range=(-TAU, TAU, 0.01))
        ...
        self.add(spiral)
        charge = Group(GlowDot(color=self.color), TrueDot(color=self.color, radius=0.15))
        self.add(charge)
        for _ in range(5):
