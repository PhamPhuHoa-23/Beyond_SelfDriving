...
class SimpleLightBeam(InteractiveScene):
    ...
    speed = 1.0
    color = YELLOW
    oscillating_field_config = dict(stroke_opacity=0.5, stroke_width=2, tip_width_ratio=1)

    def construct(self):
        axes, plane = get_axes_and_plane(**self.axes_config)
        self.add(axes, plane)
        wave = OscillatingWave(axes, z_amplitude=self.z_amplitude, wave_len=self.wave_len, speed=self.speed, color=self.color)
        vect_wave = OscillatingFieldWave(axes, wave, **self.oscillating_field_config)

        def update_wave(wave):
            st = self.time * self.speed
        ...
        self.add(wave)
        self.play(self.frame.animate.reorient(-98, 77, 0).move_to([-0.87, 0.9, -0.43]), run_time=8)
        self.add(vect_wave, wave)
        self.play(VFadeIn(vect_wave), self.frame.animate.reorient(-10, 77, 0).move_to([-0.87, 0.9, -0.43]), run_time=4)
        self.wait(3)
        z_label = Tex('z')
        ...
        x_label = VGroup(TexText('$x$-direction'), Vector(RIGHT, stroke_color=WHITE))
        ...
        self.play(FadeIn(z_label, 0.5 * OUT), FadeIn(y_label, 0.5 * UP))
        self.wait(3)
        self.play(Write(x_label[0]), GrowArrow(x_label[1]))
        self.play(self.frame.animate.reorient(-41, 77, 0).move_to([-0.87, 0.9, -0.43]), run_time=12)
        self.wait(6)
...
class TwistingLightBeam(SimpleLightBeam):
    ...
    def construct(self):
        ...
        self.play(Restore(cylinder, time_span=(0, 3)), twist_rate_tracker.animate.set_value(self.twist_rate).set_anim_args(time_span=(0, 3)), self.frame.animate.reorient(-47, 80, 0).move_to([0.06, -0.05, 0.05]).set_height(8.84), run_time=6)
        self.wait(2)
        self.play(self.frame.animate.reorient(-130, 77, 0).move_to([0.35, -0.36, 0.05]), run_time=10)
        self.wait()
        self.play(self.frame.animate.reorient(-57, 77, 0).move_to([0.35, -0.36, 0.05]), run_time=10)
        ...
        self.play(frame_anim, FadeIn(rod), Restore(plane), FadeIn(x_label), UpdateFromAlphaFunc(wave, lambda m, a: m.set_stroke(width=interpolate(2, 1, a), opacity=interpolate(1, 0.5, a)), run_time=3, time_span=(0, 2)), UpdateFromAlphaFunc(ball, lambda m, a: m.set_opacity(a)))
        self.wait(9)
        ...
        self.add(x_tracker)
        self.wait(5)
        self.play(self.frame.animate.reorient(-87, 88, 0).move_to([0.63, 0.47, -0.25]).set_height(10.51), run_time=5)
        self.wait(3)
        self.play(self.frame.animate.reorient(-43, 78, 0).move_to([0.63, 0.47, -0.25]).set_height(10.51), run_time=5)
        self.play(self.frame.animate.reorient(-34, 80, 0).move_to([1.61, -0.05, 0.3]).set_height(10.3), run_time=15)
        self.wait(10)
    ...
    def get_polarization_rod(self, axes, wave, get_x, stroke_color=None, length_mult=2.0, stroke_width=3):
        rod = Line(IN, OUT)
        rod.set_stroke(color=stroke_color or wave.get_stroke_color(), width=stroke_width)
        ...
        def update_rod(rod):
            x = get_x()
            rod.put_start_and_end_on(axes.c2p(x, wave_y, wave_z - length_mult * wave.z_amplitude), axes.c2p(x, wave_y, wave_z + length_mult * wave.z_amplitude))
    ...
    def get_slice_plane(self, axes, get_x):
        ...
        circle = Circle(radius=axes.z_axis.get_unit_size() * self.z_amplitude, n_components=100)
...
class TwistingWithinCylinder(InteractiveScene):
    ...
    def construct(self):
        frame = self.frame
        axes, plane = get_axes_and_plane(x_range=(0, 8), y_range=(-2, 2), z_range=(-2, 2), y_unit=1, z_unit=1, origin_point=3 * LEFT)
        ...
        self.play(Write(polarizer_label), FadeIn(polarizer, IN), light.animate.shift(LEFT).set_anim_args(time_span=(1, 3)), self.frame.animate.reorient(-104, 77, 0).center().set_anim_args(run_time=3))
        waves = VGroup(*(OscillatingWave(axes, z_amplitude=0.3, wave_len=wave_len, color=line.get_color(), offset=LEFT + line.get_y() * UP) for line, wave_len in zip(lines, np.linspace(2.0, 0.5, len(lines)))))
        ...
        self.play(*(TransformFromCopy(superposition, wave, run_time=2) for wave in waves))
        self.add(waves)
        for wave1 in waves:
            anims = []
            for wave2 in waves:
                ...
                anims.append(UpdateFromAlphaFunc(wave2, lambda m, a: m.set_stroke(opacity=interpolate(m.current_opacity, m.target_opacity, a))))
            self.play(*anims, run_time=0.5)
            self.wait()
        for wave in waves:
            wave.current_opacity = wave.get_stroke_opacity()
            wave.target_opacity = 1
        self.play(*(UpdateFromAlphaFunc(wave, lambda m, a: m.set_stroke(opacity=interpolate(m.current_opacity, m.target_opacity, a))) for wave in waves), frame.animate.reorient(-55, 76, 0).move_to([-0.09, 0.13, -0.17]).set_height(7.5), run_time=3)
        ...
        plane.set_stroke(width=0)
        plane.add(Circle(radius=0.5 * cylinder.get_depth(), n_components=100).set_stroke(BLACK, 1))
        plane.rotate(PI / 2, UP)
        plane.move_to(axes.get_origin())
        plane.save_state()
        plane.stretch(0, 2, about_edge=UP)
        self.play(ReplacementTransform(waves, lines, lag_ratio=0.1, run_time=3), frame.animate.reorient(-61, 83, 0).move_to([0.03, -0.16, -0.28]).set_height(7).set_anim_args(run_time=2), Restore(plane), FadeIn(cylinder))
        self.add(axes, lines)
        self.wait()
        self.play(lines.animate.arrange(UP, buff=0).move_to(axes.get_origin()), FadeIn(white_lines), FadeOut(polarizer), FadeOut(polarizer_label), FadeOut(light))
        self.wait()
        line_start, line_end = white_lines[0].get_start_and_end()
        distance_tracker = ValueTracker(0)
        wave_lengths = np.linspace(700, 400, self.n_lines)
        ...
        continuous_trails = Group(*(self.get_continuous_wave_trail(axes, line) for line in lines))
        for trail in continuous_trails:
            x_unit = axes.x_axis.get_unit_size()
            x0 = axes.get_origin()[0]
            trail.add_updater(lambda t: t.set_clip_plane(LEFT, distance_tracker.get_value() + x0))
        ...
        if self.pause_down_the_tube:
            self.play(self.frame.animate.reorient(-42, 76, 0).move_to([0.03, -0.16, -0.28]).set_height(7.0), distance_tracker.animate.set_value(4), run_time=6, rate_func=linear)
            trails.suspend_updating()
            self.play(self.frame.animate.reorient(67, 77, 0).move_to([-0.31, 0.48, -0.33]).set_height(4.05), run_time=3)
            self.wait(2)
            trails.resume_updating()
            self.play(distance_tracker.animate.set_value(axes.x_axis.x_max), self.frame.animate.reorient(-36, 79, 0).move_to([-0.07, 0.06, 0.06]).set_height(7.42), run_time=6, rate_func=linear)
            trails.clear_updaters()
            self.play(self.frame.animate.reorient(-10, 77, 0).move_to([0.42, -0.16, -0.03]).set_height(5.2), trails.animate.set_stroke(width=3, opacity=0.25).set_anim_args(time_span=(0, 3)), run_time=10)
        else:
            self.play(self.frame.animate.reorient(-63, 84, 0).move_to([1.04, -1.86, 0.55]).set_height(1.39), distance_tracker.animate.set_value(axes.x_axis.x_max), run_time=15, rate_func=linear)
            trails.clear_updaters()
            lines.clear_updaters()
            self.play(self.frame.animate.reorient(64, 81, 0).move_to([3.15, 0.46, -0.03]).set_height(5), run_time=3)
            self.wait()
        end_polarizer = Polarizer(axes, radius=0.6)
        end_polarizer.next_to(lines, RIGHT, buff=0.5)
        self.play(FadeIn(end_polarizer, OUT), FadeOut(plane), self.frame.animate.reorient(54, 78, 0).move_to([3.15, 0.46, -0.03]).set_height(5.0).set_anim_args(run_time=4))
        ...
        for index in range(len(lines)):
            ...
            dashed_lines = VGroup(*(DashedLine(axes.c2p(x, y_min, u * z), axes.c2p(x, y_max, u * z), dash_length=0.02) for u in [1, -1]))
            dashed_lines.set_stroke(WHITE, 0.5)
            dashed_lines.set_flat_stroke(False)
            self.play(MoveToTarget(lines), MoveToTarget(trails), FadeIn(dashed_lines), FadeIn(vcomp), self.frame.animate.reorient(77, 87, 0).move_to([3.1, 0.4, 0]).set_height(5))
            self.play(FadeOut(dashed_lines))
            vertical_components.add(vcomp)
        self.play(lines.animate.set_opacity(1), trails.animate.set_opacity(0.05))
        ...
        self.play(Restore(end_polarizer), TransformFromCopy(vertical_components, new_lines), self.frame.animate.reorient(43, 73, 0).move_to([3.3, 0.66, -0.38]).set_height(5.68), run_time=4)
        self.play(self.frame.animate.reorient(45, 72, 0).move_to([3.17, 0.4, -0.56]), run_time=8)
        ...
        twist_arrows = VGroup(Arrow(p1, p2, path_arc=PI), Arrow(p2, p1, path_arc=PI))
        ...
        self.play(ShowCreation(twist_arrows, lag_ratio=0), Rotate(line_group, PI, axis=RIGHT, run_time=12, rate_func=linear))
    ...
    def get_continuous_wave_trail(self, axes, line, opacity=0.4):
        return TwistedRibbon(axes, amplitude=0.5 * line.get_length(), twist_rate=get_twist(line.wave_length, TAU), color=line.get_color(), opacity=opacity)

class InducedWiggleInCylinder(TwistingLightBeam):
    random_seed = 3
    cylinder_radius = 0.5
    wave_config = dict(z_amplitude=0.15, wave_len=0.5, color=get_spectral_color(0.1), speed=1.0, twist_rate=-1 / 24)
    ...
    def construct(self):
        ...
        self.play(self.frame.animate.reorient(-86, 70, 0).move_to([1.01, -2.98, -0.79]).set_height(11.33), FadeIn(randy, time_span=(0, 1)), run_time=2)
        ...
        dot.make_3d()
        for x in range(10):
            point = axes.c2p(random.uniform(axes.x_axis.x_min, axes.x_axis.x_max), random.uniform(-max_y, -max_y), random.uniform(-max_y, -max_y))
            ...
            if x == 0:
                ...
                self.play(line.animate.set_points_as_corners(line_points), dot.animate.move_to(point))
                self.wait()
        self.play(FadeOut(line), FadeOut(dot))
        x_tracker.set_value(0)
        self.add(wave, cylinder)
        self.play(self.frame.animate.reorient(-73, 78, 0).move_to([0.8, -2.22, -0.83]).set_height(10.64), light.animate.scale(0.5), polarizer.animate.fade(0.5), VFadeIn(wave))
        self.wait(4)
        self.add(wave, cylinder)
        self.play(FadeIn(plane), FadeIn(x_label), FadeIn(rod))
        self.play(x_tracker.animate.set_value(12), run_time=12, rate_func=linear)
        ...
        self.play(self.frame.animate.reorient(-60, 79, 0).move_to([0.73, -0.59, -0.39]).set_height(9.63), Write(line_of_sight, time_span=(3, 4), lag_ratio=0), run_time=5)
        self.wait(2)
        ...
        self.remove(ball)
        self.play(x_tracker.animate.set_value(6), FadeOut(line_of_sight, time_span=(0, 0.5)), run_time=4)
        self.add(ball, wave, cylinder, plane)
        self.play(ShowCreation(new_line_of_sight))
        self.wait(4)
        self.show_propagation(rod)
        ribbon = TwistedRibbon(axes, amplitude=wave.z_amplitude, twist_rate=wave.twist_rate, color=wave.get_color())
        self.add(ribbon, cylinder)
        self.play(ShowCreation(ribbon, run_time=5))
        self.wait()
        self.play(self.frame.animate.reorient(8, 77, 0).move_to([2.01, -0.91, -0.58]).set_height(5.55), FadeOut(randy), run_time=2)
        self.wait(4)
        self.play(self.frame.animate.reorient(-25, 76, 0).move_to([4.22, -1.19, -0.5]), x_tracker.animate.set_value(12), FadeOut(new_line_of_sight, time_span=(0, 0.5)), run_time=3)
        self.wait(4)
        self.play(self.frame.animate.reorient(-61, 78, 0).move_to([0.7, 0.05, -0.69]).set_height(9.68), FadeIn(randy), run_time=3)
        self.play(LaggedStartMap(FadeOut, Group(line_of_sight, plane, rod, ball, x_label)))
        n_waves = 11
        amp = 0.03
        zs = np.linspace(0.5 - amp, -0.5 + amp, n_waves)
        small_wave_config = dict(self.wave_config)
        small_wave_config['z_amplitude'] = amp
        waves = VGroup(*(OscillatingWave(axes, offset=axes.c2p(0, 0, z)[2] * OUT, **small_wave_config) for z in zs))
        self.remove(ribbon)
        self.play(FadeOut(wave), VFadeIn(waves))
        self.wait(4)
        x_tracker.set_value(0)
        rods = VGroup(*(self.get_polarization_rod(axes, lil_wave, x_tracker.get_value, length_mult=1, stroke_width=2) for lil_wave in waves))
        balls = Group(*(self.get_wave_ball(lil_wave, x_tracker.get_value, radius=0.025) for lil_wave in waves))
        ...
        self.play(self.frame.animate.reorient(-90, 83, 0).move_to([0.17, -0.37, -0.63]).set_height(7.35).set_anim_args(run_time=3), FadeOut(light), FadeOut(polarizer), FadeIn(plane), FadeIn(rods), FadeIn(x_label), waves.animate.set_stroke(width=0.5, opacity=0.5).set_anim_args(time_span=(1, 2), suspend_mobject_updating=False), cylinder.animate.set_opacity(0.05).set_anim_args(time_span=(1, 2)))
        self.wait(4)
        self.play(self.frame.animate.reorient(-91, 90, 0).move_to([-0.01, -1.39, 0.21]).set_height(3.7), x_tracker.animate.set_value(5).set_anim_args(rate_func=linear), run_time=12)
        self.wait(4)
        lines_of_sight = VGroup(*(self.get_line_of_sign(rod, randy, stroke_width=0.5) for rod in rods))
        ...
        self.play(randy.animate.shift(3.5 * UP + 0.5 * IN), run_time=2)
        self.wait(8)
        self.play(self.frame.animate.reorient(-91, 89, 0).move_to([-0.05, -3.75, 0.07]).set_height(8.92), randy.animate.shift(10 * DOWN), run_time=2)
        self.wait(8)
    ...
    def get_line_of_sign(self, rod, observer, stroke_color=WHITE, stroke_width=1):
        ...
        line.add_updater(lambda l: l.put_start_and_end_on(observer.eyes.get_top(), rod.get_center()))
        line.set_flat_stroke(False)
        return line
...
class VectorFieldWigglingNew(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(axes, plane)
        wave = OscillatingWave(axes, wave_len=3.0, speed=1.5, color=BLUE, z_amplitude=0.5)
        ...
        self.play(self.frame.animate.reorient(-33, 83, 0).move_to([-0.75, -1.84, 0.38]).set_height(8.0), run_time=10)
        self.play(self.frame.animate.reorient(-27, 80, 0).move_to([-0.09, -0.42, -0.1]).set_height(9.03), wave_opacity_tracker.animate.set_value(1).set_anim_args(time_span=(1, 2)), vector_opacity_tracker.animate.set_value(0.5).set_anim_args(time_span=(1, 2)), run_time=4)
        x_line = Line(*axes.x_axis.get_start_and_end())
        x_line.set_stroke(BLUE, 10)
        self.play(wave_opacity_tracker.animate.set_value(0.25), vector_opacity_tracker.animate.set_value(0.25), charge_opacity_tracker.animate.set_value(0.25))
        self.play(ShowCreation(x_line, run_time=2))
        self.wait(5)
        wave_3d = VGroup()
        origin = axes.get_origin()
        for y in np.linspace(-1, 1, 5):
            for z in np.linspace(-1, 1, 5):
                vects = OscillatingFieldWave(axes, wave, max_vect_len=0.5, norm_to_opacity_func=lambda n: 0.75 * np.arctan(n))
                vects.y = y
                vects.z = z
        ...
        self.add(wave_3d)
        self.wait(2)
        self.play(self.frame.animate.reorient(22, 69, 0).move_to([0.41, -0.67, -0.1]).set_height(10.31), run_time=8)
        self.play(self.frame.animate.reorient(-48, 68, 0).move_to([0.41, -0.67, -0.1]), run_time=10)
...
class ClockwiseCircularLight(InteractiveScene):
    ...
    def setup(self):
        ...
        self.add(axes, plane)
        wave = OscillatingWave(axes, wave_len=3, speed=0.5, z_amplitude=self.amplitude, y_amplitude=self.amplitude, y_phase=-PI / 2 if self.clockwise else PI / 2, color=self.color)
    ...
    def construct(self):
        ...
        for pair in [(100, 70), (59, 72), (110, 65), (60, 80)]:
            self.play(self.frame.animate.reorient(*pair), run_time=12)
...
class TransitionTo2D(InteractiveScene):
    default_frame_orientation = (-20, 70)
    wave_config = dict(color=BLUE, wave_len=3)
    ...
    def construct(self):
        ...
        self.wait(6)
        self.play(self.frame.animate.reorient(73, 84, 0), VFadeIn(single_vect, time_span=(2, 3)), wave_opacity_tracker.animate.set_value(0.35).set_anim_args(time_span=(2, 3)), run_time=5)
        self.wait()
        self.play(self.frame.animate.reorient(90, 90, 0), wave_opacity_tracker.animate.set_value(0), run_time=4)
        self.wait(4)

class TransitionTo2DRightHanded(TransitionTo2D):
    wave_config = dict(wave_len=3, y_amplitude=0.5, z_amplitude=0.5, y_phase=PI / 2, color=RED)

class TransitionTo2DLeftHanded(TransitionTo2D):
    wave_config = dict(wave_len=3, y_amplitude=0.5, z_amplitude=0.5, y_phase=-PI / 2, color=YELLOW)
...
class LinearAsASuperpositionOfCircular(InteractiveScene):
    ...
    def construct(self):
        plane_config = dict(background_line_style=dict(stroke_color=GREY, stroke_width=1), faded_line_style=dict(stroke_color=GREY, stroke_width=0.5, stroke_opacity=0.5))
        planes = VGroup(ComplexPlane((-1, 1), (-1, 1), **plane_config), ComplexPlane((-1, 1), (-1, 1), **plane_config), ComplexPlane((-2, 2), (-2, 2), **plane_config))
        ...
        vects = VGroup(self.get_vector(planes[0], get_z1, color=RED), self.get_vector(planes[1], get_z2, color=YELLOW), self.get_vector(planes[2], get_sum, color=BLUE), self.get_vector(planes[2], get_z1, color=RED), self.get_vector(planes[2], get_sum, get_base=get_z1, color=YELLOW))
        self.add(*vects)
        ...
        def update_pol_line(line):
            if abs(vects[2].get_length()) > 0.001:
        ...
        arcs = VGroup(Arrow(LEFT, RIGHT, path_arc=-PI, stroke_width=2), Arrow(RIGHT, LEFT, path_arc=-PI, stroke_width=2))
    ...
    def get_vector(self, plane, get_z, get_base=lambda: 0, color=BLUE):
        ...
        vect.add_updater(lambda m: m.put_start_and_end_on(plane.n2p(get_base()), plane.n2p(get_z())))
        return vect