...
def get_influence_ring(center_point, color=WHITE, speed=2.0, max_width=3.0, width_decay_exp=0.5):
    ...
    def update_ring(ring, dt):
        ...
        ring.set_width(max(2 * radius, 0.001))
...
class TestFields(InteractiveScene):
    ...
    def construct(self):
        ...
        particle = ChargedParticle(rotation=0, radius=0.1, track_position_history=True)
        particles = particle.get_grid(20, 1, buff=0.25)
        particles.add_updater(lambda m: m.move_to(pos_func(self.time)))
        field = LorentzField(*particles, radius_of_suppression=1.0, x_density=4, y_density=4, max_vect_len=1, height=10)
...
class IntroduceEField(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(arrow0)
        self.play(Write(question[0]), charges[0].animate.shift(UR).set_anim_args(rate_func=wiggle, time_span=(1, 3)))
        self.play(Write(question[1]), ShowCreation(arrow1))
        self.wait()

        def show_coulomb_force(arrow, charge1, charge2):
            root = charge2.get_center()
            vect = 4 * coulomb_force(charge2.get_center()[np.newaxis, :], charge1)[0]
        ...
        self.play(FadeOut(question, time_span=(0, 1)), FadeOut(arrows, time_span=(0, 1)), charges.animate.arrange(RIGHT, buff=1.25), run_time=2)
        ...
        self.wait()
        d_line = always_redraw(lambda: DashedLine(charges[0].get_right(), charges[1].get_left(), dash_length=0.025))
        ...
        axes.add(Text('Distance', font_size=36).next_to(axes.c2p(10, 0), UP), Text('Force', font_size=36).next_to(axes.c2p(0, 0.8), LEFT))
        ...
        d_label.update()
        self.play(frame.animate.move_to([3.5, 2.5, 0.0]), LaggedStart(FadeIn(axes), ShowCreation(graph), FadeIn(graph_dot), ShowCreation(d_line), FadeIn(d_label, 0.25 * UP)), run_time=2)
        self.wait()
        for buff in (0.4, 8, 1.25):
            self.play(charges[1].animate.next_to(charges[0], RIGHT, buff=buff), run_time=4)
            self.wait()
        ...
        self.play(FadeIn(coulombs_law_title), FadeIn(coulombs_law, UP))
        self.wait()
        self.add(rect, coulombs_law)
        self.play(FadeIn(rect))
        self.wait()
        self.play(rect.animate.surround(coulombs_law['4 \\pi \\epsilon_0']))
        self.wait()
        self.play(rect.animate.surround(coulombs_law['\\frac{1}{r^2}']))
        self.wait()
        self.play(charges[1].animate.next_to(charges[0], RIGHT, buff=3.0), run_time=3)
        self.play(FadeOut(rect))
        self.wait()
        d_line.clear_updaters()
        self.play(frame.animate.center(), VGroup(coulombs_law, coulombs_law_title).animate.to_corner(UL), LaggedStartMap(FadeOut, Group(axes, graph, graph_dot, d_line, d_label, force_words, coulomb_vects)), charges[0].animate.center(), FadeOut(charges[1]), run_time=2)
        self.wait()
        ...
        r_label = e_coulombs_law['\\vec{r}'][0].copy()
        r_label.next_to(r_vect.get_center(), UP, buff=0.1)
        r_label.set_backstroke(BLACK, 20)
        e_words = VGroup(Text('Electric Field:'), Text('\n                What force would be\n                applied to a unit charge\n                at a given point\n                ', t2s={'would': ITALIC}, t2c={'unit charge': RED}, alignment='LEFT', font_size=36))
        ...
        rect.surround(e_coulombs_law['\\vec{E}'])
        rect.scale(0.9, about_edge=DR)
        self.play(FadeOut(coulombs_law, UP), FadeIn(ebr, UP), FadeIn(e_coulombs_law, UP))
        self.wait()
        self.add(ebr, rect, e_coulombs_law)
        self.play(FadeIn(rect))
        self.play(Write(e_words, stroke_color=BLACK))
        self.wait()
        self.play(FadeOut(e_words), rect.animate.surround(e_coulombs_law['(\\vec{r})'][0], buff=0))
        self.add(r_vect, charges[0])
        self.play(field.animate.set_stroke(opacity=0.4), FadeTransform(e_coulombs_law['\\vec{r}'][0].copy(), r_label), ShowCreation(r_vect))
        self.wait()
        self.play(rect.animate.surround(e_coulombs_law['\\frac{\\vec{r}}{||\\vec{r}||}']))
        self.wait()
        e_vect = r_vect.copy()
        e_vect.scale(0.25)
        e_vect.set_stroke(BLUE)
        e_vect.shift(r_vect.get_end() - e_vect.get_start())
        e_vect_label = Tex('\\vec{E}', font_size=36)
        e_vect_label.set_backstroke(BLACK, 5)
        e_vect_label.next_to(e_vect.get_center(), UL, buff=0.1).shift(0.05 * UR)
        self.play(TransformFromCopy(r_vect, e_vect, path_arc=PI / 2), FadeTransform(e_coulombs_law[:2].copy(), e_vect_label), run_time=2)
        self.wait()
        ...
        self.add(field)
        self.play(FadeIn(words, lag_ratio=0.1), ShowCreation(arrow), FadeOut(rect), FadeOut(r_vect), FadeOut(r_label), FadeOut(e_vect), FadeOut(e_vect_label))
        self.wait()
        self.play(LaggedStartMap(FadeOut, Group(dots, coulombs_law_title, e_coulombs_law, words, arrow)), FadeOut(ebr), charges[0].animate.to_edge(LEFT, buff=1.0), charges[1].animate.to_edge(RIGHT, buff=1.0), run_time=3)
        ...
        lorentz_field = ColoumbPlusLorentzField(*tmp_charges, x_density=6.0, y_density=6.0, norm_to_opacity_func=lambda n: np.clip(0.5 * n, 0, 0.75))
        self.remove(field)
        ...
        wiggle_kwargs = dict(rate_func=lambda t: wiggle(t, 3), run_time=1.5, suspend_mobject_updating=False)
        ...
        self.wait(6)
        self.play(FadeOut(influence_ring0), FadeOut(influence_ring1), FadeOut(lorentz_field))
        ...
        a_label = Tex('\\vec{a}(t_0)', font_size=48)
        ...
        field = VectorField(field_func, height=0, x_density=4.0, max_vect_len=1.0)
        ...
        self.play(FadeIn(f_vect), FadeIn(f_label), FadeOut(field))
        lorentz_law = Tex('\n            \\vec{F}(t) = \n            {-q_1 q_2 \\over 4\\pi \\epsilon_0 c^2}\n            {1 \\over r}\n            \\vec{a}_\\perp(t - r / c)\n        ')
        lorentz_law.to_edge(UP)
        lorentz_law['\\vec{F}(t)'][0].match_style(f_label)
        a_hat_perp = lorentz_law['\\vec{a}_\\perp'][0]
        ...
        lorentz_law[')'][1].match_style(a_label)
        self.play(Transform(f_label.copy(), lorentz_law['\\vec{F}(t)'][0].copy(), remover=True, run_time=1.5), FadeIn(lorentz_law, time_span=(1, 2)))
        self.wait()
        ...
        self.play(FadeIn(rect))
        self.wait()
        self.play(rect.animate.surround(lorentz_law['4\\pi \\epsilon_0 c^2']))
        self.wait()
        self.play(rect.animate.surround(lorentz_law['{1 \\over r}']), ShowCreation(r_line))
        self.play(TransformFromCopy(lorentz_law['r'][1], r_label))
        self.wait()
        self.play(rect.animate.surround(lorentz_law['\\vec{a}_\\perp(t - r / c)']))
        self.wait()
        self.play(rect.animate.surround(lorentz_law['t - r / c'], buff=0.05))
        self.wait()
        new_a_label = Tex('\\vec{a}(t - r / c)')
        ...
        self.add(charges[0])
        self.play(ring.animate.scale(0.001), UpdateFromFunc(time_decimal, lambda m: m.set_value(ring.get_radius() / 2)), charges[0].animate.shift(2 * DOWN).set_anim_args(time_span=(1, 4), rate_func=lambda t: smooth(t) ** 0.5), run_time=4)
        time_decimal.set_value(0)
        self.play(TransformMatchingStrings(a_label, new_a_label), FadeOut(rect))
        self.remove(rect)
        self.remove(ring)
        ring = self.get_influence_ring(charges[0].get_center())
        time_decimal.add_updater(lambda m: m.set_value(ring.time))
        ...
        r_line.target.become(DashedLine(charges.target[0].get_center(), charges.target[1].get_center()))
        ...
        self.play(MoveToTarget(charges), MoveToTarget(r_line), MoveToTarget(f_vect), r_label.animate.next_to(r_line.target.get_center(), UL, SMALL_BUFF), f_label.animate.next_to(f_vect.target.get_center(), UR, buff=0), new_a_label.animate.next_to(charges.target[0], UL, buff=0), MaintainPositionRelativeTo(a_vect, charges[0]), run_time=2)
        self.wait()
        r_unit = normalize(charges[1].get_center() - charges[0].get_center())
        a_perp_vect = Vector(a_vect.get_vector() - np.dot(a_vect.get_vector(), r_unit) * r_unit)
        ...
        rings = VGroup()
        for x in range(2):
            wiggle_kwargs = dict(run_time=2, rate_func=lambda t: wiggle(t, 5))
            ring = self.get_influence_ring(charges[0].get_center())
            rings.add(ring)
        ...
        plane = NumberPlane(background_line_style=dict(stroke_color=GREY_D, stroke_opacity=0.75, stroke_width=1), axis_config=dict(stroke_opacity=0.25))
        new_lorentz = Tex('\n            \\vec{E}_{\\text{rad}}(\\vec{r}, t) = \n            {-q \\over 4\\pi \\epsilon_0 c^2}\n            {1 \\over ||\\vec{r}||}\n            \\vec{a}_\\perp(t - ||\\vec{r}|| / c)\n        ', font_size=36)
        new_lorentz.to_corner(UL)
        lhs = new_lorentz['\\vec{E}_{\\text{rad}}(\\vec{r}, t)']
        lhs.set_color(BLUE)
        new_lorentz['\\vec{a}_\\perp('].set_color(PINK)
        new_lorentz[')'][1].set_color(PINK)
        lhs_rect = SurroundingRectangle(lhs)
        arrow = Vector(UP).next_to(lhs_rect, DOWN)
        self.add(plane, lorentz_law, *charges)
        self.remove(rect)
        self.play(LaggedStartMap(FadeOut, Group(r_line, r_label, a_hat_perp2, a_perp_vect, a_vect, new_a_label, new_a_label, f_vect, f_label, charges[1])), FadeIn(plane, time_span=(1, 2)), charges[0].animate.center().set_anim_args(time_span=(1, 2)), FadeTransform(lorentz_law, new_lorentz))
        self.play(ShowCreation(lhs_rect), GrowArrow(arrow))
        self.wait()
        self.play(FadeOut(lhs_rect), FadeOut(arrow))
        charge = ChargedParticle(track_position_history=True)
        field = LorentzField(charge, stroke_width=3, x_density=4.0, y_density=4.0, max_vect_len=0.25, norm_to_opacity_func=lambda n: np.clip(1.5 * n, 0, 1))
        ...
        charge.ignore_last_motion()
        wiggle_kwargs = dict(rate_func=lambda t: wiggle(t, 3), run_time=3.0, suspend_mobject_updating=False)
        lemniscate = ParametricCurve(lambda t: np.sin(t) ** 2 * (np.cos(t) * RIGHT + np.sin(t) * UP), t_range=(0, TAU, TAU / 200))
        self.play(charge.animate.shift(0.4 * UP).set_anim_args(**wiggle_kwargs))
        self.wait(3)
        self.play(MoveAlongPath(charge, lemniscate, run_time=6))
        self.wait(3)
        ...
        charge.init_clock()
        charge.ignore_last_motion()
        charge.add_updater(lambda m: m.move_to(0.25 * np.sin(0.5 * TAU * m.get_internal_time()) * UP))
        self.wait(30)
...
class AltEFieldIntroduction(IntroduceEField):
    ...
    def construct(self):
        title1 = TexText('Light $\\rightarrow$ Wave in the Electromagnetic field')
        title2 = TexText('Electric field')
        title2.scale(1.5)
        VGroup(title1, title2).to_edge(UP)
        self.add(title1)
        self.wait()
        self.play(TransformMatchingStrings(title1, title2))
        title2.add_to_back(BackgroundRectangle(title2))
        density = 4.0
        charges = Group(ChargedParticle(ORIGIN, 1), ChargedParticle(3 * UL, -1, color=BLUE, sign='-'), ChargedParticle([3, -2, 0], 2), ChargedParticle([5, 1, 0], -2, color=BLUE, sign='-'))
        field_config = dict(x_density=density, y_density=density, width=2 * FRAME_WIDTH)
        ...
        self.play(ShowCreation(points), FadeIn(charges))
        self.add(points, c_field, charges, title2)
        self.play(charges[0].animate.shift(0.0001 * UP).set_anim_args(rate_func=there_and_back), c_field_opacity_tracker.animate.set_value(1))
        self.wait()
        for charge in charges:
            charge.save_state()
            charge.target = charge.generate_target()
            charge.target.fade(0.5)
            charge.target.scale(0.001)
        ...
        self.play(*map(MoveToTarget, charges), c_field_opacity_tracker.animate.set_value(0.5), VFadeIn(vect), FadeIn(hyp_charge), FadeIn(hyp_words))
        ...
        l_field = ColoumbPlusLorentzField(c1, c2, norm_to_opacity_func=lambda n: np.arctan(n), **field_config)
        self.play(FadeOut(points), ReplacementTransform(c_field, l_field), Restore(c1), FadeOut(hyp_charge), FadeOut(hyp_words), FadeOut(vect), *(c.animate.move_to(10 * c.get_center()) for c in charges[1:]))
        self.add(l_field, charges[0], title2)
        self.wait()
        hyp_charge.next_to(charges[0], RIGHT, buff=0.5)
        vect.field = l_field
        self.play(VFadeIn(vect), FadeIn(hyp_charge))
        self.play(hyp_charge.animate.next_to(charges[0], LEFT, buff=0.5).set_anim_args(path_arc=PI), run_time=8)
        self.play(FadeOut(hyp_charge), FadeOut(vect), FadeOut(title2))

        def wiggle_charge(charge, direction, run_time=2):
            return charge.animate.shift(direction).set_anim_args(rate_func=lambda t: wiggle(t, 3), run_time=run_time, suspend_mobject_updating=False)
        ...
        self.play(wiggle_charge(c1, 0.5 * UP), VFadeIn(speed_words))
        self.wait(5)
        self.add(c2)
        self.play(self.frame.animate.move_to(Group(c1, c2)), run_time=3)
        amp = 0.5
        for sign in [1, -1, 1, -1]:
...
class TestForMithuna(InteractiveScene):
    ...
    def construct(self):
        ...
        charges = Group(*(ChargedParticle(track_position_history=True, charge=10 / n_charges, radius=0.1, show_sign=False) for _ in range(n_charges)))
        charges.arrange_in_grid(n_rows, n_cols, buff=0.5)
        charges.center()
        self.add(charges)
        columns = Group(*(charges[i::n_cols] for i in range(n_cols)))
        field = LorentzField(*charges, stroke_width=3, x_density=4.0, y_density=4.0, radius_of_suppression=0.1, max_vect_len=None, norm_to_opacity_func=lambda n: np.clip(1.5 * n, 0, 0.7))
        self.add(field)
        c_dot = GlowDot().get_grid(1, 100, buff=0.5)
        c_dot.move_to(charges)
        c_dot.add_updater(lambda m, dt: m.shift(field.c * dt * RIGHT))
        self.wait(0.1)
        self.play(LaggedStart(*(col.animate.shift(0.5 * UP).set_anim_args(rate_func=lambda t: wiggle(t, 6), suspend_mobject_updating=False, run_time=8) for col in columns), lag_ratio=1 / n_cols))
        self.wait(5)
...
class ShowTheEffectsOfOscillatingCharge(InteractiveScene):
    ...
    origin = None
    axes_config = dict(axis_config=dict(stroke_opacity=0.7), x_range=(-10, 10), y_range=(-5, 5), z_range=(-3, 3))
    particle_config = dict(track_position_history=True, radius=0.15)
    acceleration_vector_config = dict()
    field_config = dict(max_vect_len=0.35, stroke_opacity=0.75, radius_of_suppression=1.0, height=10, x_density=4.0, y_density=4.0, c=2.0, norm_to_opacity_func=lambda n: np.clip(2 * n, 0, 0.8))
    ...
    def add_particles(self, axes):
        ...
        self.particles.add_updater(lambda m: m.move_to(axes.c2p(*self.oscillation_function(self.time))))
    ...
    def add_acceleration_vectors(self, particles):
        self.acceleration_vectors = VGroup(*(AccelerationVector(particle) for particle in particles))
...
class SendingLittlePulses(ShowTheEffectsOfOscillatingCharge):
    axes_config = dict(axis_config=dict(stroke_opacity=0.7), x_range=(0, 10), y_range=(-3, 3), z_range=(-1, 1))
    field_config = dict(max_vect_len=0.25, stroke_opacity=0.75, radius_of_suppression=1.0, height=10, x_density=4.0, y_density=4.0, c=2.0, norm_to_opacity_func=lambda n: np.clip(1.5 * n, 0, 0.8))
    ...
    def construct(self):
        ...
        for _ in range(10):
            ...
            self.play(particle.animate.shift(0.06 * shake_size * UP), rate_func=lambda t: wiggle(t, 2), run_time=shake_size)
            self.wait(random.choice([1, 2]))
        self.wait(4)

class OscillateOnYOneDField(ShowTheEffectsOfOscillatingCharge):
    origin = 5 * LEFT
    axes_config = dict(axis_config=dict(stroke_opacity=0.7), z_axis_config=dict(stroke_opacity=0), x_range=(-3, 12), y_range=(-3, 3))
    field_config = dict(max_vect_len=1, stroke_opacity=1.0, radius_of_suppression=0.25, height=0, x_density=4.0, c=2.0, norm_to_opacity_func=None)
    ...
    def construct(self):
        ...
        self.play(frame.animate.set_height(3, about_point=axes.get_origin()), MoveToTarget(particle), self.acceleration_vectors.animate.set_stroke(opacity=0.2), run_time=2)
        last_line = VMobject()
        last_ghost = Group()
        ...
        for x in np.arange(1, 9):
            ...
            line = Line(ghost.get_center(), axes.c2p(x * step, 0))
            ...
            self.play(ShowCreation(line), FadeOut(last_line), FadeOut(last_ghost, scale=0), GrowFromCenter(ghost), FadeIn(elbow, time_span=(0.5, 1)))
            self.wait(0.5)
            last_line = Group(line, elbow)
            last_ghost = ghost
        self.play(FadeOut(last_line))
        self.time = paused_time
        self.play(Restore(particle), frame.animate.to_default_state().set_anim_args(run_time=3))
...
class OscillateOnYTwoDField(ShowTheEffectsOfOscillatingCharge):
    particle_config = dict(track_position_history=True, radius=0.15)
    field_config = dict(max_vect_len=0.25, stroke_opacity=0.75, radius_of_suppression=0.25, height=10, x_density=4.0, y_density=4.0, c=2.0, norm_to_opacity_func=lambda n: np.clip(1.5 * n, 0, 1.0))
...
class DiscussDecay(OscillateOnYOneDField):
    ...
    def construct(self):
        ...
        words = TexText('Decays proportionally to $\\frac{1}{r}$')
        words['$\\frac{1}{r}$'].scale(1.5, about_edge=LEFT).set_color(TEAL)
        words.move_to(2 * UP)
        particles[0].ignore_last_motion()
        self.play(ShowCreation(graph), Write(words))
        self.wait(20)
...
class ChargeOnZAxis(ShowTheEffectsOfOscillatingCharge):
    ...
    direction = OUT
    origin = ORIGIN
    axes_config = dict(axis_config=dict(stroke_opacity=0.7), x_range=(-8, 8), y_range=(-6, 6), z_range=(-3, 3))
    particle_config = dict(show_sign=False, rotation=PI / 2, track_position_history=True, radius=0.2)
    field_config = dict(max_vect_len=0.5, stroke_opacity=0.7, radius_of_suppression=1.0, width=40, height=40, depth=0, x_density=4.0, y_density=4.0, z_density=1.0, c=2.0, norm_to_opacity_func=lambda n: np.clip(n, 0, 0.8))
...
class ThreeCharges(ChargeOnZAxis):
    ...
    def get_particles(self):
        return Group(*(ChargedParticle(**self.particle_config) for n in range(3))).arrange(UP, buff=2)

class Introduce3dMovements(ChargeOnZAxis):
    axes_config = dict(axis_config=dict(stroke_opacity=0.7), x_range=(-5, 5), y_range=(-5, 5), z_range=(-3, 3))
    ...
    def construct(self):
        ...
        self.play(Rotate(charge, TAU, axis=UP, about_point=RIGHT, **kw), self.frame.animate.reorient(-17, 71, 0).move_to([-0.12, 0.17, 0.27]).set_height(7.16), run_time=6)
        self.play(self.frame.animate.reorient(-20, 69, 0).set_height(8).center(), Rotate(charge, TAU, axis=OUT, about_point=RIGHT, **kw), run_time=6)
        self.play(charge.animate.shift(0.8 * (UP + OUT)).set_anim_args(rate_func=lambda t: wiggle(t, 6), **kw), self.frame.animate.reorient(20, 71, 0).move_to([0.31, 0.54, -0.3]).set_height(8.22), run_time=8)
        self.play(charge.animate.shift(4 * DOWN).set_anim_args(rate_func=there_and_back, **kw), self.frame.animate.reorient(-21, 64, 0).move_to([0.31, 0.54, -0.3]).set_height(8.22), run_time=9)
        self.wait(4)

class Introduce3dMovements3DVects(Introduce3dMovements):
    field_config = dict(max_vect_len=0.25, stroke_opacity=0.7, radius_of_suppression=1.0, width=20, height=20, depth=8, x_density=4.0, y_density=4.0, z_density=1.0, c=2.0, norm_to_opacity_func=lambda n: np.clip(n, 0, 0.6))

class CoulombLorentzExample(Introduce3dMovements):
    field_class = ColoumbPlusLorentzField
    field_config = dict(max_vect_len=0.3, stroke_opacity=0.7, radius_of_suppression=1.0, width=40, height=40, depth=0, x_density=4.0, y_density=4.0, z_density=1.0, c=2.0, norm_to_opacity_func=lambda n: np.clip(n, 0, 0.8))

class CoulombLorentzExample3D(Introduce3dMovements3DVects):
    field_class = ColoumbPlusLorentzField
    field_config = dict(max_vect_len=0.35, stroke_opacity=0.7, radius_of_suppression=1.0, width=10, height=10, depth=8, x_density=2.0, y_density=2.0, z_density=2.0, c=2.0, norm_to_opacity_func=lambda n: np.clip(n, 0, 0.75))

class RowOfCharges(ChargeOnZAxis):
    n_charges = 17
    particle_buff = 0.25
    particle_config = dict(rotation=PI / 2, track_position_history=True, radius=0.1, show_sign=False, charge=0.15)
    field_config = dict(max_vect_len=0.5, stroke_opacity=0.7, radius_of_suppression=1.0, width=30, height=30, depth=0, x_density=4.0, y_density=4.0, z_density=1.0, c=2.0, norm_to_opacity_func=lambda n: np.clip(1.5 * n, 0, 0.8))
    ...
    def get_particles(self):
        return Group(*(ChargedParticle(**self.particle_config) for n in range(self.n_charges))).arrange(UP, buff=self.particle_buff)
...
class PlaneOfCharges(RowOfCharges):
    ...
    particle_buff = 0.1
    grid_height = 6
    particle_config = dict(rotation=PI / 2, track_position_history=True, radius=0.05, show_sign=False, charge=2.0 / 400.0)

    def get_particles(self):
        result = Group(*(ChargedParticle(**self.particle_config) for _ in range(self.n_rows * self.n_cols)))
        result.arrange_in_grid(self.n_rows, self.n_cols, buff=self.particle_buff)
...
class RowOfChargesMoreCharges(RowOfCharges):
    n_charges = 100
    particle_buff = 0.01
    particle_config = dict(rotation=PI / 2, track_position_history=True, radius=0.05, show_sign=False, charge=0.0)
    field_config = dict(max_vect_len=0.5, stroke_opacity=0.7, radius_of_suppression=1.0, width=30, height=30, depth=0, x_density=4.0, y_density=4.0, z_density=1.0, c=2.0, norm_to_opacity_func=lambda n: np.clip(1.5 * n, 0, 0.8))
...
class AltRowOfCharges(RowOfCharges):
    ...
    def construct(self):
        ...
        self.play(self.frame.animate.reorient(-20, 69, 0).set_height(8.0), run_time=12)
...
class RowOfChargesXAxis(RowOfCharges):
    field_config = dict(max_vect_len=1.0, stroke_opacity=0.7, radius_of_suppression=0.25, width=40, height=0, depth=0, x_density=8.0, c=2.0, norm_to_opacity_func=lambda n: np.clip(1.5 * n, 0, 0.8))
    axes_config = dict(axis_config=dict(stroke_opacity=0.7), x_range=(-20, 20), y_range=(-6, 6), z_range=(-3, 3))
    ...
    def construct(self):
        self.wait(20)
        self.play(self.frame.animate.reorient(-15, 84, 0).move_to([4.36, -1.83, 0.37]).set_height(5.59), run_time=3)
        ...
        words = VGroup(TexText('Instead of decaying like $\\frac{1}{r}$'), TexText('It decays much more gently'))
        words.fix_in_frame()
        words.to_edge(UP, buff=1.5)
        self.play(ShowCreation(graph1, run_time=2), FadeIn(words[0], 0.5 * UP))
        self.wait()
        self.play(FadeOut(words[0], 0.5 * UP), FadeIn(words[1], 0.5 * UP), Transform(*graphs))
        self.wait(6)

class RowOfChargesXAxisMoreCharges(RowOfChargesXAxis):
    n_charges = 100
    particle_buff = 0.1
    particle_config = dict(rotation=PI / 2, track_position_history=True, radius=0.05, show_sign=False, charge=3.0 / 50)
...
class WavesIn3D(ChargeOnZAxis):
    field_config = dict(max_vect_len=0.5, stroke_opacity=0.25, radius_of_suppression=1.0, height=10, depth=10, x_density=4.0, y_density=4.0, z_density=1.0, c=2.0, norm_to_opacity_func=lambda n: np.clip(0.5 * n, 0, 0.8))

class WiggleHereWiggleThere(IntroduceEField):

    def construct(self):
        charges = Group(*(ChargedParticle(track_position_history=True) for _ in range(2)))
        charges[0].to_edge(LEFT, buff=2.0)
        charges[1].to_edge(RIGHT, buff=2.0)
        dist = get_norm(charges[0].get_center() - charges[1].get_center())
        for charge in charges:
            charge.ignore_last_motion()
        field = LorentzField(*charges, x_density=6.0, y_density=6.0, norm_to_opacity_func=lambda n: np.clip(0.5 * n, 0, 0.75))
        self.add(field)
        self.add(*charges)
        wiggle_kwargs = dict(rate_func=lambda t: wiggle(t, 3), run_time=1.5, suspend_mobject_updating=False)
...
class CircularPolarization1D(ShowTheEffectsOfOscillatingCharge):
    default_frame_orientation = (-20, 70)
    amplitude = 0.2
    field_config = dict(max_vect_len=1.0, stroke_opacity=0.85, radius_of_suppression=0.4, height=0, width=30, x_density=5.0, c=2.0, norm_to_opacity_func=lambda n: np.clip(2 * n, 0, 0.8))
    particle_config = dict(track_position_history=True, radius=0.15, show_sign=False)
    ...
    def construct(self):
        ...
        self.play(self.frame.animate.reorient(81, 86, 0), run_time=12)
        self.add(lone_vect)
        self.play(FadeIn(frame), field_opacity_tracker.animate.set_value(0.25), self.frame.animate.reorient(41, 76, 0), run_time=5)
        self.play(self.frame.animate.reorient(79, 70, 0), run_time=12)
        self.play(field_opacity_tracker.animate.set_value(0.15), self.frame.animate.reorient(97, 84, 0), run_time=12)
        self.play(field_opacity_tracker.animate.set_value(0.35), self.frame.animate.reorient(59, 73, 0), run_time=12)
...
class PIPHelper(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(frame)
        self.play(Rotating(vect, -15 * TAU, about_point=frame.get_center()), run_time=30, rate_func=linear)

class CircularPolarization2D(CircularPolarization1D):
    field_config = dict(max_vect_len=0.35, stroke_opacity=0.85, radius_of_suppression=0.5, height=30, width=30, x_density=4.0, y_density=4.0, c=2.0, norm_to_opacity_func=lambda n: np.clip(2 * n, 0, 0.8))

    def construct(self):
        self.play(self.frame.animate.reorient(79, 78, 0), run_time=12)
        self.play(self.frame.animate.reorient(-83, 80, 0), run_time=24)
        self.play(self.frame.animate.reorient(17, 62, 0), run_time=18)
...
class RandomRicochet(InteractiveScene):
    ...
    def construct(self):
        ...
        for _ in range(n_beams):
            ...
            beam = VMobject().set_points_as_corners([light.get_center(), ball.get_center(), point])
        ...
        for beam in beams:
            self.play(VShowPassingFlash(beam, time_width=0.3, run_time=1.5))

    def get_plane_and_axes(self):
        axes = ThreeDAxes(axis_config=dict(tick_size=0))
        axes.set_stroke(opacity=0.5)
        plane = NumberPlane(x_range=axes.x_range, y_range=axes.y_range, background_line_style=dict(stroke_color=GREY_B, stroke_width=1.0, stroke_opacity=0.5))
        plane.axes.set_opacity(0)
        return (plane, axes)

class PolarizedScattering(RandomRicochet):
    field_config = dict(width=20, height=20, x_density=5.0, y_density=5.0, stroke_color=BLUE, norm_to_opacity_func=lambda n: np.clip(n, 0, 0.75))

    def construct(self):
        frame = self.frame
        plane, axes = self.get_plane_and_axes()
        self.add(plane, axes)
        wave = OscillatingWave(axes, wave_len=2.0, color=YELLOW)
        ...
        charge.add_updater(lambda m: m.move_to(-0.5 * wave.xt_to_point(0, self.time)))
        a_vect = AccelerationVector(charge, norm_func=lambda n: 0.5 * np.tanh(n))
        ...
        self.play(self.frame.animate.reorient(15, 70, 0), run_time=12)
        rings = ProbagatingRings(axes.z_axis, n_rings=5, spacing=0.4)
        direction_vectors = VGroup(*(Arrow(v, 3 * v) for v in compass_directions(8)))
        self.add(rings)
        self.play(LaggedStartMap(GrowArrow, direction_vectors, lag_ratio=0.1), field_opacity_multiple.animate.set_value(0.5), vects_opacity_tracker.animate.set_value(0.25))
        self.play(self.frame.animate.reorient(-32, 73, 0), run_time=8)
        self.play(VFadeOut(rings))
        direction_vectors.generate_target()
        for dv in direction_vectors.target:
            dv.scale(0.5)
            dv.shift(-0.5 * dv.get_start())
            dv.rotate(70 * DEGREES, axis=cross(dv.get_vector(), OUT), about_point=ORIGIN)
...
class PolarizedScatteringYZ(PolarizedScattering):
    field_config = dict(width=0, height=20, depth=20, x_density=5.0, y_density=5.0, stroke_color=BLUE, norm_to_opacity_func=lambda n: np.clip(n, 0, 1.0))
...
class OneOfManyCharges(InteractiveScene):
    ...
    plane_wave_field_config = dict(x_density=4.0, y_density=0.5, z_density=1.0, width=28, height=0, depth=12, tip_width_ratio=3, stroke_color=TEAL, stroke_width=3, stroke_opacity=0.5, norm_to_opacity_func=lambda n: 2 * n, max_vect_len=1.0)
    plane_wave_amlpitude = 0.5
    charge_field_config = dict(x_density=4.0, y_density=4.0, z_density=4.0, width=28, height=0, depth=16, tip_width_ratio=3, stroke_color=BLUE, stroke_width=3, norm_to_opacity_func=lambda n: np.tanh(n))
    charge_index = 3155
    dots_dims = (11, 20, 20)
    dots_shape = (6, 4, 4)
    ...
    def setup(self):
        ...
        def plane_wave_func(points, t):
            return self.plane_wave_amlpitude * np.outer(np.cos(k * np.dot(points, RIGHT) - omega * t), OUT)
        plane_wave = TimeVaryingVectorField(plane_wave_func, **self.plane_wave_field_config)
    ...
    def construct(self):
        frame = self.frame
        self.play(frame.animate.reorient(25, 72, 0), run_time=4)
        if self.charge_index > 0:
            charge = ChargedParticle(charge=-5, color=BLUE, show_sign=False, radius=0.1)
            charge.add_updater(lambda m: m.move_to(self.dots.get_points()[self.charge_index]))
            charge.update()
            charge.ignore_last_motion()
            charge_field = LorentzField(charge, center=charge.get_y() * UP, radius_of_suppression=0.2, **self.charge_field_config)
            acc_vect = VectorizedPoint()
            self.add(charge_field, acc_vect, charge, self.dots)
            self.play(self.plane_wave.opacity_multiplier.animate.set_value(0), self.dots.opacity_multiplier.animate.set_value(0.1), FadeIn(charge, suspend_mobject_updating=False), VFadeIn(acc_vect), run_time=2)
        else:
            self.play(self.dots.opacity_multiplier.animate.set_value(0.2), run_time=2)
        self.play(frame.animate.reorient(25, 72, 0), run_time=8)
        self.play(frame.animate.reorient(-25, 72, 0), run_time=8)
...
class AlternateCompositeChargesInPlane(OneOfManyCharges):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(16, 77, 0), run_time=12)
        self.play(dots.animate.set_height(0, stretch=True), self.dot_center_refs.animate.set_height(0, stretch=True), frame.animate.reorient(0, 90).set_height(6), self.axes.y_axis.animate.set_stroke(opacity=0), plane_wave.opacity_multiplier.animate.set_value(0.5), run_time=2)
        self.wait(2)
        self.play(dots.opacity_multiplier.animate.set_value(0.25), plane_wave.opacity_multiplier.animate.set_value(0.0))
        self.wait()
        w, h, d = self.dots_dims
        indices = np.arange(0, w * h * d, w)
        charge = ChargedParticle(color=BLUE, radius=dots.get_radius(), show_sign=False)
        ...
        charge_field = LorentzField(charges[0], radius_of_suppression=0.2, **self.charge_field_config)
        self.add(charges[0])
...
class FullCompositeEffect(OneOfManyCharges):
    ...
    def construct(self):
        ...
        new_wave = TimeVaryingVectorField(new_pw_func, stroke_color=YELLOW, x_density=10, z_density=1, width=14, height=0, depth=0, max_vect_len=np.inf, norm_to_opacity_func=lambda n: 2 * n)
        ...
        self.add(new_wave)
        self.play(frame.animate.reorient(0, 90, 0).set_height(8).set_focal_distance(100), dots.opacity_multiplier.animate.set_value(0.75), plane_wave.opacity_multiplier.animate.set_value(0), new_wave.opacity_multiplier.animate.set_value(1), VFadeIn(new_wave), run_time=5)
        self.wait(15)
...
class ResponsiveCharge(InteractiveScene):
    ...
    def construct(self):
        ...
        field = field_type(charge1, charge2, x_density=4.0, y_density=4.0, norm_to_opacity_func=lambda n: np.clip(0.5 * n, 0, 1), c=1.0)
        self.add(field)
        axes = ThreeDAxes()
        ...
        plane.fade(0.5)
        self.add(plane)
        self.play(charge1.animate.shift(UP).set_anim_args(rate_func=wiggle, run_time=3))
        self.wait(4)