...
class AnnotateDemo(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(Write(sugar_label, lag_ratio=0.01, run_time=2), ShowCreation(sugar_arrow))
        self.wait()
        ...
        self.play(FadeTransform(sugar_label, light_label), ReplacementTransform(sugar_arrow, light_arrow))
        self.wait()
        ...
        self.play(FadeTransform(light_label, filter_label), ReplacementTransform(light_arrow, filter_arrow))
        self.wait()
        self.play(filter_label.animate.set_x(-2.2).to_edge(UP, buff=0.5), filter_arrow.animate.set_x(-3.3))
        self.wait()
...
class HoldUp(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.change('raise_right_hand', look_at=3 * UR), self.change_students('pondering', 'erm', 'sassy', look_at=self.screen))
        self.wait(3)
...
class DidntSteveCover(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(stds[2].says("Didn't Steve Mould\ncover this?", mode='raise_right_hand'), morty.change('tease'), self.change_students('pondering', 'pondering', look_at=thumbnail), FadeIn(thumbnail, UP))
        self.wait(4)
...
class FocusOnWall(InteractiveScene):
    ...
    def construct(self):
        ...
        words.to_edge(UP)
        arrow = Arrow(words['Notice what'].get_bottom(), 6.5 * LEFT + 0.5 * UP, buff=0.5, stroke_color=RED, stroke_width=14)
        self.play(FadeIn(words, 0.25 * UP), GrowArrow(arrow))
        self.wait()
...
class WhatWeNeedToUnderstand(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(self.change_students('pondering', 'maybe', 'confused', look_at=self.screen), morty.change('tease'))
        self.wait(3)
        self.play(self.change_students('erm', 'sassy', 'maybe', look_at=self.screen))
        self.wait(3)
        concepts = VGroup(Text('Polarization'), Text('Scattering'), Text('The index of refraction'))
        concepts.arrange(DOWN, aligned_edge=LEFT)
        concepts.next_to(self.hold_up_spot, UP)
        for i in range(3):
            concepts[i].align_to(concepts, DOWN)
            self.play(FadeIn(concepts[i], 0.5 * UP), concepts[:i].animate.shift(UP), morty.change('raise_right_hand', concepts), *(pi.animate.look_at(concepts) for pi in stds))
            self.wait()
        self.wait()
        self.play(morty.says('To begin:\nAn overview'), FadeOut(concepts, UP), self.change_students('tease', 'happy', 'well', look_at=morty.eyes))
...
class ThisIsStillWhiteLight(InteractiveScene):
    ...
    def construct(self):
        ...
        words.to_edge(UP)
        arrow = Arrow(words['This'].get_bottom(), LEFT + 0.5 * UP, stroke_color=WHITE, stroke_width=8)
        self.play(Write(words, run_time=1), FadeIn(arrow, RIGHT, run_time=4.0, rate_func=rush_into))
        self.wait()
...
class Questions(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        marks = VGroup(*(Checkmark(font_size=60).next_to(question[0], RIGHT).shift(0.15 * UP) for question in questions))
        for mark in marks:
        ...
        for question in questions:
            ...
            cross_lines.add(VGroup(*(Line(LEFT, RIGHT).set_stroke(YELLOW, 3).replace(full_question[phrase], dim_to_match=0) for phrase in phrases)))
        self.play(Write(marks[0], stroke_color=GREEN), ShowCreation(cross_lines[0]))
        self.wait()
        rect = SurroundingRectangle(questions[3])
        rect.set_stroke(YELLOW, 2)
        arrow = Vector(LEFT, stroke_color=YELLOW)
        arrow.next_to(rect, RIGHT)
        self.play(questions[:3].animate.set_opacity(0.5), ShowCreation(rect), FadeInFromPoint(arrow, ORIGIN))
        self.wait()
        self.play(FadeOut(rect), FadeOut(arrow), ShowCreation(cross_lines[3]), Write(marks[3], stroke_color=GREEN), questions[:3].animate.set_opacity(1))
        self.wait()
        rect.set_stroke(opacity=0)
        arrow.set_stroke(opacity=0)
        self.play(rect.animate.surround(questions[1]).set_stroke(opacity=1), arrow.animate.next_to(questions[1], RIGHT).set_stroke(opacity=1))
        self.wait()
        marks[1].set_fill(opacity=0)
        marks[1].set_stroke(GREEN, 2)
        self.play(FadeOut(rect), Write(marks[1]))
        self.wait()
    ...
    def get_questions(self):
        ...
        question_texts = ['What exactly is wiggling?', 'Why does sugar make light twist?', 'Why does the twisting rate\ndepend on frequency?', 'Why do we see colors\nin diagonal stripes?']
        questions = VGroup(*(VGroup(Text(f'Question #{n}:', **kw).set_fill(GREY_A), Text(text, **kw)).arrange(DOWN, buff=0.25, aligned_edge=LEFT) for n, text in enumerate(question_texts)))
...
class FromOnHigh(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Transform(top_arrows, low_arrows), FadeTransform(VGroup(top_words, cross), low_words))
        self.wait()
...
class UnitRVector(InteractiveScene):
    ...
    def construct(self):
        text = TexText('Unit vector in \\\\ the direction of $\\vec{r}$')
...
class RSquaredVsR(InteractiveScene):
    ...
    def construct(self):
        ...
        labels = VGroup(Tex('f(r) = \\frac{1}{r^2}').set_color(YELLOW), Tex('f(r) = \\frac{1}{r}').set_color(BLUE))
        labels.arrange(DOWN, buff=1.0, aligned_edge=LEFT)
        labels.move_to(axes, UR)
        for graph, label in zip(graphs, labels):
            self.play(ShowCreation(graph, run_time=3), FadeIn(label, 0.5 * UP))
        self.wait(3)
...
class MentionMaxwell(TeacherStudentsScene):
    ...
    def construct(self):
        stds = self.students
        morty = self.teacher
        kw = dict(tex_to_color_map={'\\mathbf{E}': BLUE, '\\mathbf{B}': YELLOW}, font_size=30)
        maxwells_equations = VGroup(Tex('\\nabla \\cdot \\mathbf{E}=\\frac{\\rho}{\\varepsilon_0}', **kw), Tex('\\nabla \\cdot \\mathbf{B}=0', **kw), Tex('\\nabla \\times \\mathbf{E}=-\\frac{\\partial \\mathbf{B}}{\\partial t}', **kw), Tex('\\nabla \\times \\mathbf{B}=\\mu_0\\left(\\mathbf{J}+\\varepsilon_0 \\frac{\\partial \\mathbf{E}}{\\partial t}\\right)', **kw))
        ...
        self.play(stds[2].says(TexText("What about \\\\ Maxwell's equations?")), morty.change('tease'), self.change_students('confused', 'pondering'))
        self.play(stds[2].change('raise_right_hand'), FadeIn(maxwells_equations, UP))
        self.play(FadeTransform(stds[2].bubble.content["Maxwell's equations"].copy(), equations_title), stds[2].debubble(), *(pi.animate.look_at(equations_title) for pi in [morty, *stds[:2]]))
        self.wait()
        ...
        lhs = equation['\\vec{E}_{\\text{rad}}(\\vec{r}, t)']
        lhs.set_color(BLUE)
        equation['\\vec{a}_\\perp('].set_color(PINK)
        equation[')'][1].set_color(PINK)
        ...
        self.play(morty.change('raise_right_hand'), FadeIn(equation, 0.5 * UP), MoveToTarget(maxwells_equations), GrowArrow(implies_arrow), FadeIn(implies_word, lag_ratio=0.1), self.change_students('well', 'happy'))
        self.wait()
        self.add(eq_rect, equation)
        self.play(FadeIn(eq_rect), ShowCreation(arrow), FadeIn(question, lag_ratio=0.1), morty.change('tease', look_at=question), self.change_students('tease', 'well', 'happy', look_at=equation))
        self.wait(4)
        self.remove(equation)
        everything = Group(*self.mobjects)
        self.play(LaggedStartMap(FadeOut, everything), equation.copy().animate.scale(36.0 / 42).to_corner(UR), run_time=1)
        self.wait()
...
class StrengthInDifferentDirectionsWithDecimal(InteractiveScene):
    ...
    def construct(self):
        ...
        def get_cos_tex():
            cos_tex = Tex('\\cos(10^\\circ) = 0.00', font_size=36)
        ...
        self.wait(2)
        self.play(self.frame.animate.set_height(12), Rotate(rot_group, 30 * DEGREES, about_point=ORIGIN), run_time=3)
        self.wait(2)
        self.play(self.frame.animate.set_height(15), Rotate(rot_group, 30 * DEGREES, about_point=ORIGIN), run_time=3)
        self.wait(2)
...
class TransitionBeforePolarization(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.change_students('pondering', 'thinking', 'happy', look_at=self.screen))
        self.wait(3)
        self.play(self.teacher.change('raise_left_hand', look_at=3 * UR), self.change_students('well', 'tease', 'happy', look_at=3 * UR))
        self.wait(4)
...
class AskAboutQuantum(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(stds[0].says('What about, like,\nphotons and quantum stuff?'), morty.change('well'), self.change_students(None, 'sassy', 'hesitant', look_at=stds[0].eyes))
        self.wait(3)
        titles = VGroup(Text('Classical'), Text('Quantum'))
        ...
        points = VGroup(Text('Waves radiate away from changing charges'), Text('These waves have polarization'), Text('Energy scales continuously'))
        points.scale(0.7)
        ...
        self.play(morty.change('raise_right_hand'), FadeTransform(stds[0].bubble.content['quantum'].copy(), titles[1]), FadeIn(titles[0], shift=0.25 * UP), ShowCreation(h_line), ShowCreation(v_lines), Write(soft_h_lines), stds[0].debubble(mode='pondering'), self.change_students(None, 'pondering', 'pondering', look_at=UP), run_time=2)
        self.wait()
        self.play(FadeIn(points[0], lag_ratio=0.1), Write(checks[:2], lag_ratio=0.7, stroke_color=GREEN), *(pi.animate.look_at(titles) for pi in self.pi_creatures))
        self.wait()
        self.play(FadeIn(points[1], lag_ratio=0.1), Write(checks[2:4], lag_ratio=0.7, stroke_color=GREEN))
        self.wait()
        self.play(FadeIn(points[2], lag_ratio=0.1), Write(checks[4], stroke_color=GREEN), FadeIn(cross, scale=0.5), morty.change('sassy', look_at=cross))
        self.wait()
        self.play(FadeOut(self.pi_creatures, DOWN), FadeOut(soft_h_lines[-1]), v_lines.animate.set_height(7, about_edge=UP, stretch=True))
        self.wait()
...
class ContinuousWave(InteractiveScene):
    ...
    def construct(self):
        ...
        wave.add_updater(lambda m: m.set_points(points).stretch(1 - math.sin(self.time), 1))
        self.add(wave)
        self.wait(20)
...
class DiscreteWave(ContinuousWave):
    ...
    def construct(self):
        ...
        for n, wave in zip(it.count(1), waves):
            ...
            labels.add(label)
            self.play(FadeIn(wave), FadeIn(label))
        dots = Tex('\\vdots', font_size=60)
...
class DiscreteGraph(InteractiveScene):
    ...
    def construct(self):
        axes = Axes((0, 5), (0, 5), width=4, height=4)
        graph = axes.get_graph(lambda x: np.floor(x) + 0.5, discontinuities=np.arange(0, 8), x_range=(0, 4.99))
...
class RedFilter(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(indicator)
        self.wait()
        self.play(indicator.animate.move_to(plane.c2p(1.5, -0.125), DOWN), run_time=6)
        self.wait()
        self.play(indicator.animate.move_to(plane.c2p(-3.5, 0), DOWN), run_time=6)
        self.wait()
...
class LengthsOnDifferentColors(InteractiveScene):
    ...
    def construct(self):
        ...
        lines = VGroup(Line(plane.c2p(3.2, 2.8), plane.c2p(-0.5, 2.8)), Line(plane.c2p(3.2, 0.9), plane.c2p(0.6, 0.9)), Line(plane.c2p(3.2, -1.0), plane.c2p(1.0, -1.0)), Line(plane.c2p(3.2, -3.0), plane.c2p(1.3, -3.0)))
        braces = VGroup(*(Brace(line, DOWN, buff=SMALL_BUFF) for line in lines))
        braces.set_backstroke(BLACK, 3)
        numbers = VGroup(*(DecimalNumber(line.get_length() / 7, font_size=36, unit=' \\text{m}') for line in lines))
        ...
        for brace, rect, number in zip(braces, rects, numbers):
            ...
            self.play(GrowFromPoint(brace, brace.get_right()), CountInFrom(number, 0), UpdateFromFunc(VGroup(), lambda m: number.next_to(brace, DOWN, SMALL_BUFF)), rect.animate.set_opacity(0), other_rects.animate.set_opacity(0.8))
            self.wait(2)
        ...
        for line, twist_rate in zip(lines, twist_rates):
            ribbon = TwistedRibbon(axes_3d, amplitude=0.25, twist_rate=twist_rate, color=rgb_to_color(image.point_to_rgb(line.get_start())))
...
class AskNoteVerticalVariation(RedFilter):
    ...
    def construct(self):
        ...
        self.add(indicator)
        self.play(FadeIn(lr_arrows, time_span=(0, 1)), indicator.animate.move_to(plane.c2p(3, -0.5), DOWN), run_time=3)
        self.play(indicator.animate.move_to(plane.c2p(1.5, -0.5), DOWN), run_time=3)
        self.wait()
        self.play(FadeIn(ud_arrows, time_span=(0, 1)), FadeOut(lr_arrows, time_span=(0, 1)), indicator.animate.move_to(plane.c2p(1.5, -0.2), DOWN), run_time=3)
...
class CombineColors(InteractiveScene):
    ...
    def construct(self):
        ...
        images = Group(*(ImageMobject(os.path.join(folder, ext + 'Still.jpg')) for ext in ['Red', 'Orange', 'Green', 'Blue', 'Rainbow']))
        colors = images[:4]
        rainbow = images[4]
...
class IndexOfRefraction(InteractiveScene):
    ...
    def construct(self):
        equation = Tex('{\\text{Speed in a vacuum} \\over \\text{Speed in water}} \\approx 1.33')
        equation.shift(UP)
        rhs = equation['\\approx 1.33']
        equation.scale(0.75)
...
class SugarIsChiral(InteractiveScene):
    ...
    title = 'Sucrose $\\text{C}_{12}\\text{H}_{22}\\text{O}_{11}$ '
    ...
    def construct(self):
        ...
        frame = self.frame
        title = VGroup(TexText(self.title), TexText(self.subtitle))
        ...
        self.add(title)
        self.play(FadeIn(sucrose, scale=5))
        ...
        self.play(frame.animate.move_to(0.75 * OUT), MoveToTarget(sucrose), FadeIn(mirror), title.animate.scale(0.75).match_x(sucrose.target).set_y(1.75), run_time=1.5)
        ...
        self.play(MoveToTarget(mirror_image), FadeIn(mirror_words))
        definition = TexText('Chiral $\\rightarrow$ Cannot be superimposed onto its mirror image')
        ...
        self.play(self.frame.animate.reorient(-8, 76, 0), run_time=15)
        self.wait(15)
...
class SimplerChiralShape(InteractiveScene):
    ...
    def construct(self):
        ...
        for name, mob in [(spiral_name, ribbon), (mirror_name, mirror_image)]:
            ...
            name.to_edge(UP)
            name.match_x(mob)
        self.play(FadeIn(spiral_name, 0.5 * UP), ShowCreation(ribbon, run_time=3))
        self.wait()
        self.play(ReplacementTransform(ribbon.copy().shift(0.1 * DOWN), mirror_image), FadeTransformPieces(spiral_name.copy(), mirror_name))
        self.wait()
        r_copy = ribbon.copy()
...
class ThatSeemsIrrelevant(TeacherStudentsScene):
    ...
    def construct(self):
        stds = self.students
        morty = self.teacher
        self.play(stds[0].says('That seems irrelevant', mode='sassy'), self.change_students(None, 'erm', 'hesitant', look_at=stds[0].eyes), morty.change('guilty'))
        self.wait(2)
        self.play(stds[0].debubble(mode='raise_left_hand', look_at=self.screen), self.change_students(None, 'pondering', 'pondering', look_at=self.screen))
        self.wait(5)
...
class CurvyCurvyArrow(InteractiveScene):
    ...
    def construct(self):
        ...
        arrows = VGroup(Arrow(DOWN, UP, **kw), Arrow(UP, DOWN, **kw))
...
class GlowDot(InteractiveScene):
    ...
    def construct(self):
        ...
        mid_point = 0.85 * LEFT
        mask = VMobject().set_points_as_corners([UR, mid_point, DR, DL, UL])
