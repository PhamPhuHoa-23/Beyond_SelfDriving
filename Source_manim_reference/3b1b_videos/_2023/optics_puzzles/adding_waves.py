...
class WhiteLightAsASum(InteractiveScene):
    ...
    def construct(self):
        ...
        axes_width = 9.0
        x_max = 8
        spectral_axes = VGroup(*(ThreeDAxes((0, x_max), (-1, 1), (-1, 1)) for _ in range(n_colors)))
        ...
        for alpha, s_axes in zip(alphas, spectral_axes):
            ...
            for axes, group in [(s_axes, spectral_waves), (white_axes, white_parts)]:
                wave = OscillatingWave(axes, y_amplitude=0, z_amplitude=1, wave_len=wave_len, color=get_spectral_color(1 - alpha))
                group.add(wave)
        ...
        self.play(frame.animate.reorient(50, 65, 0).move_to([0.6, 2.9, -0.16]).set_height(11).set_anim_args(run_time=3), LaggedStartMap(FadeIn, spectral_axes), LaggedStartMap(VFadeIn, spectral_waves), LaggedStartMap(VFadeIn, spectral_vect_waves), LaggedStartMap(FadeIn, symbols))
        self.play(self.frame.animate.reorient(19, 65, 0).move_to([0.6, 2.9, -0.16]).set_height(11.0), run_time=8)
        return
        arrows = VGroup(*(Arrow(sa.get_left(), interpolate(white_axes.get_corner(UR), white_axes.get_corner(DR), alpha), buff=0.3, stroke_color=wave.get_color()) for sa, wave, alpha in zip(spectral_axes, spectral_waves, np.linspace(0, 1, n_colors))))
        pass
...
class AddTwoSineWaves(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(axes1), ShowCreation(wave1[0]))
        self.play(LaggedStartMap(FadeIn, wave1[2]))
        self.change_all_parameters(wave1, 0.5, PI, PI / 4)
        self.wait()
        self.play(FadeIn(axes2), ShowCreation(wave2[0]))
        self.play(LaggedStartMap(FadeIn, wave2[2]))
        self.change_all_parameters(wave2, 0.75, 2 * PI, PI / 2)
        self.wait()
        self.add(axes2)
        self.add(wave2)
        wave3 = axes3.get_graph(lambda x: wave1.func(x) + wave2.func(x), color=GREEN, bind=True)
        wave3_label = Tex('f(t) + g(t)', font_size=36)
        wave3_label.move_to(axes3.c2p(0.5, 1.5), DL)
        self.play(LaggedStart(TransformFromCopy(axes1, axes3), TransformFromCopy(axes2, axes3), TransformFromCopy(wave1.labels[3]['f(t)'], wave3_label['f(t)']), TransformFromCopy(wave2.labels[3]['g(t)'], wave3_label['+ g(t)']), lag_ratio=0.05))
        self.play(TransformFromCopy(wave1[0], wave3), TransformFromCopy(wave2[0], wave3))
        self.wait()
        ...
        frame = self.frame
        t2c = {'\\omega': GOLD}
        top_words = TexText('If both have matching \\\\ frequencies, $\\omega$...', font_size=36, t2c=t2c)
        ...
        top_arrows = VGroup(*(Arrow(top_words.get_right(), lock.get_left()) for lock in omega_locks))
        self.play(frame.animate.set_x(-5), FadeIn(top_words, 0.5 * LEFT), LaggedStartMap(ShowCreation, top_arrows), run_time=2)
        self.play(LaggedStartMap(FadeIn, omega_locks), *self.get_wave_change_animations(wave1, 'omega', PI), *self.get_wave_change_animations(wave2, 'omega', PI), run_time=2)
        self.wait()
        low_words = TexText('Then this is also a sine \\\\ wave with frequency $\\omega$', font_size=36, t2c=t2c)
        low_words.align_to(top_words, LEFT)
        low_words.match_y(axes3).shift(UP)
        low_arrow = Arrow(low_words.get_right(), axes3.get_left())
        self.play(FadeTransform(top_words.copy(), low_words), FadeIn(low_arrow, 2 * DOWN))
        self.wait()
        sum_sine = Tex('= 1.00\\sin(\\omega t +2.00)', t2c={'\\omega': GOLD}, font_size=36)
        ...
        self.play(LaggedStart(FadeOut(top_words), FadeOut(top_arrows), FadeOut(low_words), FadeOut(low_arrow), FadeOut(param_arrows), FadeOut(param_qmarks), FadeIn(low_fade_rect)))
        ...
        for wave, phasor in [(wave1, phasor1), (wave2, phasor2)]:
            ...
            phasor.A_label.add_updater(lambda m: m.next_to(m.vector.pfp(0.5), normalize(rotate_vector(m.vector.get_vector(), (1 if m.vector.get_angle() > 0 else -1) * 90 * DEGREES)), buff=0.05))
            ...
            phasor.phi_label.add_updater(lambda m: m.move_to(m.plane.n2p(1.5 * m.plane.p2n(m.arc.pfp(0.5)))))
            self.play(FadeIn(phasor.plane, LEFT), FadeIn(phasor.rot_vect, LEFT))
            self.wait()
            wave.output_indicator = self.get_output_indicator(wave.axes, wave.wave, phasor.t_tracker)
            self.play(FadeIn(phasor.y_line), FadeIn(phasor.y_dot), FadeIn(wave.output_indicator))
            self.play(phasor.t_tracker.animate.set_value(4), run_time=8, rate_func=linear)
            self.wait()
            self.play(FadeOut(wave.output_indicator), FadeOut(phasor.y_line), FadeOut(phasor.y_dot))
            self.add(phasor.vector, phasor.rot_vect)
            self.play(TransformFromCopy(A_label, phasor.A_label))
            self.play(wave.trackers[0].animate.set_value(1.5 * wave.trackers[0].get_value()), run_time=2, rate_func=there_and_back)
            self.wait()
            self.play(TransformFromCopy(phi_label, phasor.phi_label), FadeIn(phasor.arc))
            self.play(wave.trackers[2].animate.set_value(wave.trackers[2].get_value() + PI / 2), run_time=4, rate_func=there_and_back)
            self.play(low_fade_rect.animate.set_height(3, about_edge=DOWN, stretch=True))
        sum_plane = ComplexPlane((-1, 1), (-1, 1), background_line_style=dict(stroke_color=GREY_B, stroke_width=1), faded_line_style=dict(stroke_color=GREY_B, stroke_width=0.5, stroke_opacity=0.25), faded_line_ratio=4)
        sum_plane.set_height(2.0)
        sum_plane.match_x(phasor1)
        sum_plane.match_y(axes3)
        sum_plane.to_edge(DOWN, buff=MED_SMALL_BUFF)
        low_vects = VGroup(*(Vector(stroke_width=3, stroke_color=wave.wave.get_color()) for wave in [wave1, wave2]))

        def update_low_vects(vects):
            z1 = phasor1.get_z()
            z2 = phasor2.get_z()
            vects[0].put_start_and_end_on(sum_plane.n2p(0), sum_plane.n2p(z1))
            vects[1].put_start_and_end_on(sum_plane.n2p(z1), sum_plane.n2p(z1 + z2))
        low_vects.add_updater(update_low_vects)
        self.play(FadeIn(sum_plane), FadeOut(low_fade_rect))
        self.wait()
        self.play(TransformFromCopy(phasor1.rot_vect, low_vects[0]), TransformFromCopy(phasor2.rot_vect, low_vects[1]))
        self.add(low_vects)
        self.wait()
        sum_vect = Vector(stroke_color=wave3.get_color(), stroke_width=3)
        sum_vect.add_updater(lambda m: m.put_start_and_end_on(sum_plane.n2p(0), sum_plane.n2p(phasor1.get_z() + phasor2.get_z())))
        ...
        def show_rotation(max_t=4):
            ...
            self.add(wave1.output_indicator, wave2.output_indicator, sum_output_indicator)
            self.play(phasor1.t_tracker.animate.set_value(max_t), phasor2.t_tracker.animate.set_value(max_t), run_time=4 * max_t, rate_func=linear)
            self.play(*map(FadeOut, (wave1.output_indicator, wave2.output_indicator, sum_output_indicator)))
        self.play(ShowCreation(sum_vect))
        show_rotation()
        self.wait()
        param_rects = VGroup(*(SurroundingRectangle(param).match_color(param) for param in sum_parameters))
        ...
        self.play(ShowCreation(param_arrows[0]), FadeIn(param_qmarks[0]), ShowCreation(param_rects[0]))
        self.wait()
        self.play(TransformFromCopy(sum_parameters[0], sum_A_copy))
        self.wait()
        self.play(ReplacementTransform(*param_arrows), ReplacementTransform(*param_qmarks), ReplacementTransform(*param_rects))
        self.wait()
        self.play(TransformFromCopy(sum_parameters[1], sum_phi_copy), ShowCreation(sum_arc))
        self.wait()
        self.play(*map(FadeOut, (sum_A_copy, sum_phi_copy, sum_arc, param_arrows[1], param_qmarks[1], param_rects[1])))
        self.wait()
        phi0 = PI / 6
        self.play(*self.get_wave_change_animations(wave1, 'phi', phi0), *self.get_wave_change_animations(wave2, 'phi', phi0), run_time=3)
        self.wait()
    ...
    def get_variable_wave(self, axes, color=YELLOW, func_name='f(t)', A=1.0, omega=TAU, phi=0.0, index='', label_font_size=36, parameter_font_size=24, A_color=RED, k_color=GOLD, phi_color=PINK, func_name_coords=(0.5, 1.25)):
        ...
        A_tex = f'A_{{{index}}}'
        omega_tex = f'\\omega_{{{index}}}'
        phi_tex = f'\\phi_{{{index}}}'
        t2c = {A_tex: A_color, omega_tex: k_color, phi_tex: phi_color}
        ...
        labels = VGroup(*(Tex(tex, font_size=parameter_font_size, t2c=t2c) for tex in [f'{A_tex} = 1.00', f'{omega_tex} = 1.00', f'{phi_tex} = 1.00']))
        ...
        name_label = Tex(f'{func_name} = {A_tex} \\sin({omega_tex} t + {phi_tex})', tex_to_color_map=t2c, font_size=label_font_size)
    ...
    def get_phasor(self, axes, wave_group, plane_height=2.0):
        plane = ComplexPlane((-1, 1), (-1, 1), background_line_style=dict(stroke_color=GREY_B, stroke_width=1), faded_line_style=dict(stroke_color=GREY_B, stroke_width=0.5, stroke_opacity=0.5), faded_line_ratio=4)
        ...
        vector = Arrow(plane.n2p(0), plane.n2p(1), buff=0, stroke_color=GREY_B, stroke_width=3, stroke_opacity=0.5)
        vector.add_updater(lambda m: m.put_start_and_end_on(plane.n2p(0), plane.n2p(get_z0())))
        arc = always_redraw(lambda: Arc(angle=get_phi(), radius=min(plane_height / 6, 0.5 * vector.get_length()), arc_center=plane.n2p(0)).set_stroke(WHITE, 1))
        ...
        rot_vect.add_updater(lambda m: m.put_start_and_end_on(plane.n2p(0), plane.n2p(get_z())))
        ...
        y_line.add_updater(lambda m: m.put_start_and_end_on(rot_vect.get_end(), y_dot.get_center()))
    ...
    def get_output_indicator(self, axes, graph, t_tracker):
        ...
        v_line.add_updater(lambda m: m.put_start_and_end_on(axes.x_axis.n2p(get_t()), dot.get_center()))
    ...
    def get_wave_change_animations(self, wave_group, parameter, value):
        ...
        return [VFadeInThenOut(rects, rate_func=lambda t: there_and_back(t) ** 0.5), trackers[index].animate.set_value(value)]

    def change_wave_parameter(self, wave_group, parameter, value, run_time=2):
        self.play(*self.get_wave_change_animations(wave_group, parameter, value), run_time=run_time)
...
class WavePlusLayerInfluence(InteractiveScene):
    ...
    def construct(self):
        ...
        y_range = (-1, 1)
        z_range = (-1, 1)
        top_axes, mid_axes, low_axes = (ThreeDAxes(x_range, y_range, z_range), Axes(x_range, y_range), Axes(x_range, y_range))
        ...
        labels = VGroup(Text('Incoming light', **text_kw), Text('Wave from layer oscillations', **text_kw), Text('Net effect', **text_kw))
        for label, box in zip(labels, boxes):
            label.next_to(box.get_corner(UL), DR, buff=MED_SMALL_BUFF)
        labels[2].shift(0.25 * UP)
        wave1 = OscillatingWave(top_axes, y_amplitude=0.75, z_amplitude=0.0, wave_len=4.0, speed=1.5)
        ...
        vect_wave2 = GraphAsVectorField(mid_axes, wave2_func, stroke_color=wave2.get_color(), **field_kw)
        vect_wave3 = GraphAsVectorField(low_axes, sum_func, stroke_color=wave3.get_color(), **field_kw)
        ...
        for x in layer_xs:
            ...
            charges.add_updater(lambda m: m.move_to(mid_axes.c2p(m.x, m.amplitude_tracker.get_value() * wave1.xt_to_yz(m.x, wave1.time)[0], 0)))
            layers.add(charges)
        glass = VCube()
        ...
        self.wait(2)
        self.play(Write(glass, time_span=(0, 2)), frame.animate.reorient(-110, 10, 110).set_anim_args(run_time=4))
        self.play(LaggedStart(*(layer.amplitude_tracker.animate.set_value(0.1) for layer in layers), lag_ratio=0, time_span=(0, 2)), LaggedStart(*(FadeIn(layer, suspend_mobject_updating=False) for layer in layers), lag_ratio=0, time_span=(0, 2)))
        self.play(self.frame.animate.reorient(-90, -40, 90), FadeOut(glass), run_time=9)
        self.wait(3)
        self.play(LaggedStart(*(FadeOut(charges, suspend_mobject_updating=False) for charges in layers[1:]), run_time=5, lag_ratio=0.25), layers[0].amplitude_tracker.animate.set_value(0.2), self.frame.animate.reorient(-90, -20, 90), run_time=5)
        self.wait(3)
        self.play(VFadeIn(wave2_group), FadeIn(mid_axes), run_time=1)
        self.wait(5)
        self.play(top_axes.animate.move_to(boxes[0], DOWN), frame.animate.reorient(-90, 0, 90).set_focal_distance(100), LaggedStartMap(FadeIn, labels[:2], shift=UP), run_time=2)
        self.wait(6)
        ...
        self.play(LaggedStartMap(FadeIn, plus_eq), ShowCreation(low_axes))
        self.play(VFadeIn(wave3_group), Write(labels[2]))
        self.wait(12)
        reflection_label = VGroup(Vector(2 * LEFT), Text('Reflected light', font_size=36))
        ...
        self.play(GrowArrow(reflection_label[0]), Write(reflection_label[1]))
        self.wait(8)
        cover = FullScreenFadeRectangle()
        cover.set_fill(BLACK, 0.9)
        cover.stretch(0.5, 0, about_edge=LEFT)
        self.add(cover, charges, labels)
        self.play(FadeOut(reflection_label), FadeIn(cover), plus_eq.animate.set_x(1.5), frame.animate.set_x(0.5 * FRAME_WIDTH - 2), *(label.animate.set_x(FRAME_WIDTH - 2.5, RIGHT) for label in labels), run_time=2)
        self.wait(9)
        wave1.stop_clock()
        ...
        def find_peak(wave, threshold=0.01):
        ...
        shift_arrow = Vector(line.get_length() * LEFT * 2, stroke_width=3, max_tip_length_to_length_ratio=10)
        shift_arrow.next_to(line, UP, buff=0.2)
        shift_label = Text('shift', font_size=24)
        always(shift_label.next_to, shift_arrow, UP)
        self.play(ShowCreation(shift_arrow), Write(shift_label))
        self.wait()
        net_stretch = 1.0
        for stretch_factor in [3.0, 0.5, 2.0, 0.5, 1.0]:
            ...
            self.play(Restore(scale_arrows), wave2_scale_tracker.animate.set_value(stretch * wave2_scale_tracker.get_value()), shift_arrow.animate.stretch(stretch, 0, about_edge=RIGHT))
            self.play(FadeOut(scale_arrows))
            self.wait()
        self.play(wave2_scale_tracker.animate.set_value(0.2), FadeOut(wave1_copy), FadeOut(shift_arrow), FadeOut(shift_label))
        wave1.start_clock()
        self.wait(8)
...
class Pulse(InteractiveScene):
    ...
    def construct(self):
        n_parts = 10
        x_max = 4
        left_axes_group = VGroup(*(Axes((0, x_max), (-1, 1), width=0.5 * FRAME_WIDTH - 2, height=1, num_sampled_graph_points_per_tick=20) for _ in range(n_parts)))
        ...
        sigma = Tex('\\Sigma', font_size=72).next_to(arrow, UP)
        ...
        left_graphs = VGroup(*(self.create_graph(axes, freq, speed, color, time_tracker) for axes, freq, speed, color in zip(left_axes_group, frequencies, speeds, colors)))

        def sum_func(x):
            time = time_tracker.get_value()
            return sum((weight * self.wave_func(x, freq, speed, time) for weight, freq, speed in zip(weights, frequencies, speeds)))
        ...
        self.play(time_tracker.animate.increment_value(final_time), rate_func=linear, run_time=final_time)
    ...
    def create_graph(self, axes, freq, speed, color, time_tracker):
        return axes.get_graph(lambda x: self.wave_func(x, freq, speed, time_tracker.get_value()), stroke_color=color, stroke_width=2, bind=True)