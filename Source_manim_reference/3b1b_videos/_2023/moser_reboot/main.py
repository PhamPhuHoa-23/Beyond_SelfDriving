...
class Introduction(InteractiveScene):
    ...
    def construct(self):
        ...
        paths = VGroup(*(VMobject().set_points(path) for path in tale[0].get_subpaths()))
        ...
        self.add(tale)
        self.play(rect.animate.next_to(tale, RIGHT, buff=LARGE_BUFF).set_anim_args(rate_func=linear), Write(paths, stroke_width=5), run_time=3)
        self.remove(rect, paths)
        self.wait()
        name = TexText("Moser's Circle Problem", font_size=72)
        name.to_edge(UP)
        self.play(FadeOut(tale), FadeIn(name, UP))
        self.wait(2)
...
class ShowPattern(InteractiveScene):
    ...
    def construct(self):
        ...
        expression = Tex(','.join(map(str, values)) + '\\dots')
        ...
        self.play(GrowFromCenter(brace1), FadeIn(question, 0.5 * DOWN))
        self.wait()
        self.play(ReplacementTransform(brace1, brace2), FadeTransform(question, coincidence))
        self.wait()
        self.play(ReplacementTransform(brace2, brace3), FadeTransform(coincidence, what))
        self.wait()
        self.play(FadeOut(brace3), FadeOut(what))
        fn = Tex('f(n) = \\, ???', font_size=60)
        ...
        for n, part in zip(it.count(1), parts):
            ...
            term = Tex(f'f({n})', font_size=48)
            term.set_color(YELLOW)
            term.next_to(rect, DOWN)
            self.play(FadeIn(term), FadeIn(rect), FadeOut(last_term), FadeOut(last_rect), run_time=0.5)
            self.wait(0.5)
            last_term = term
            last_rect = rect
...
class AskAboutPosition(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(stds[1].says("Doesn't it depend on\nwhere the points are?", mode='sassy'), stds[0].change('confused', self.screen), stds[2].change('pondering', self.screen), morty.change('guilty', stds[1].eyes)))
        self.wait(5)
...
class ExplainNChoose2(InteractiveScene):
    ...
    def construct(self):
        ...
        for i, j in it.combinations(indices, 2):
            ...
            last_label = label
        self.play(chords.animate.set_opacity(1), numbers.animate.set_opacity(1), dots.animate.set_opacity(1))
        self.wait()
        nc2 = Tex('{n \\choose 2}')
        ...
        self.play(FadeIn(nc2, DOWN), pair_labels.animate.set_height(4).to_edge(DOWN))
        ...
        pair_rects = VGroup(*(SurroundingRectangle(pair, buff=SMALL_BUFF).set_stroke(YELLOW, 1) for pair in pair_labels))
        rhs = Tex('= 0')
        rhs_num = rhs.make_number_changeable('0')
        rhs.next_to(nc2, RIGHT)
        nc2[1].set_opacity(0)
        self.play(Write(number_rects, lag_ratio=0.5), ChangeDecimalToValue(n_value, self.n))
        self.play(FadeOut(number_rects, lag_ratio=0.1))
        self.wait()
        self.play(VFadeIn(rhs), nc2_label.animate.next_to(nc2, DOWN), ChangeDecimalToValue(rhs_num, choose(self.n, 2), run_time=3), ShowIncreasingSubsets(pair_rects, run_time=3))
        self.wait()
        self.play(FadeOut(pair_rects, lag_ratio=0.04))
        new_rhs = Tex('= {7 \\cdot (7 - 1) \\over 2}')
        new_rhs.next_to(nc2, RIGHT)
        self.play(rhs.animate.next_to(new_rhs, RIGHT), Write(new_rhs[:2]), Write(new_rhs['\\over']))
        self.wait()
        self.play(Write(new_rhs['\\cdot (7 - 1)']))
        self.wait()
        self.play(Write(new_rhs['2']))
        self.wait()
    ...
    def get_circle_diagram(self):
        ...
        circle.set_height(6)
        circle.to_edge(LEFT)
        points = [circle.pfp(a + self.fudge_factor * np.random.uniform(-0.0, 0.5)) for a in np.arange(0, 1, 1 / self.n)]
        dots = VGroup(*(Dot(point, radius=0.04).set_fill(WHITE) for point in points))
        chords = VGroup(*(Line(p1, p2).set_stroke(BLUE_B, 1) for p1, p2 in it.combinations(points, 2)))
        numbers = VGroup()
...
class SimpleCircle(InteractiveScene):
    ...
    def construct(self):
        ...
        dots = VGroup(*(Dot(circle.pfp(radians[i] / TAU)) for i in [0, 2, 4, 1, 3, 5]))
        self.add(circle)
        for dot in dots:
            self.wait()
            self.add(dot)
...
class LeftDiagram(ExplainNChoose2):
    ...
    def construct(self):
        ...
        diagrams = VGroup()
        for n in range(2, 6):
            diagram = VGroup(circle.copy(), self.get_chords(dots[:n]), dots[:n].copy())
        ...
        numbers = VGroup(*(Integer(2 ** n, font_size=60).next_to(diagram, RIGHT) for n, diagram in zip(it.count(1), diagrams)))
        for diagram, number in zip(diagrams, numbers):
            for dot in diagram[2]:
                dot.scale(2)
            self.play(TransformFromCopy(diagram.preimage, diagram), FadeInFromPoint(number, circle.get_center()))
            self.wait()

    def get_chords(self, dots):
        return VGroup(*(Line(d1.get_center(), d2.get_center(), stroke_width=2, stroke_color=BLUE_B) for d1, d2 in it.combinations(dots, 2)))
...
class ProblemSolvingRule1(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        title = Text('Problem-solving rule #1', font_size=72)
        title.to_edge(UP)
        title.set_backstroke()
        underline = Underline(title, buff=-0.05)
        self.play(morty.change('raise_right_hand'), self.change_students('pondering', 'thinking', 'pondering', look_at=title), GrowFromPoint(title, morty.get_corner(UL)))
        self.add(title)
        ...
        self.play(FadeIn(words, lag_ratio=0.1, run_time=2), self.change_students('tease', 'thinking', 'well', look_at=words), morty.change('tease'))
        self.wait(6)
...
class PairOfChords(TeacherStudentsScene):
    ...
    def construct(self):
        self.remove(self.background)
        self.play(self.students[0].change('confused', self.screen), self.students[1].says('Pairs of chords?', 'raise_left_hand'), self.students[2].change('confused', self.screen), self.teacher.change('well'))
        self.wait(5)
...
class PauseAndPonder(TeacherStudentsScene):
    ...
    def construct(self):
        self.remove(self.background)
        self.play(LaggedStart(self.students[0].change('pondering', self.screen), self.students[1].change('thinking', self.screen), self.students[2].change('erm', self.screen), self.teacher.says('Pause and\nponder', mode='hooray')))
        self.wait(5)
...
class CountIntersections(ExplainNChoose2):
    ...
    def construct(self):
        ...
        self.add(quad_words)
        int_dots, quad_labels = self.show_quadruplets(diagram, quad_words)
        nc4 = Tex('n \\choose 4')
        ...
        self.play(FadeIn(nc4, 0.5 * DOWN), Write(nc4_label), quad_labels.animate.set_height(4).to_edge(DOWN))
        self.wait()
        rhs = Tex('= 0')
        rhs_num = rhs.make_number_changeable('0')
        rhs.next_to(nc4, RIGHT)
        quad_rects = VGroup(*(SurroundingRectangle(label, buff=SMALL_BUFF).set_stroke(YELLOW, 1) for label in quad_labels))
        self.play(VFadeIn(rhs, time_span=(0, 1)), ShowIncreasingSubsets(quad_rects), ChangeDecimalToValue(rhs_num, choose(self.n, 4)), run_time=3)
        self.wait()
        self.play(FadeOut(quad_rects, lag_ratio=0.02), rhs.animate.set_opacity(0))
        self.wait()
        full_rhs = Tex('= {n(n-1)(n-2)(n-3) \\over 1 \\cdot 2 \\cdot 3 \\cdot 4}')
        nc4.generate_target()
        nc4.target.shift(1.25 * LEFT)
        full_rhs.next_to(nc4.target, RIGHT, SMALL_BUFF)
        self.play(FadeOut(diagram), FadeOut(int_dots), Write(full_rhs), MoveToTarget(nc4), FadeOut(rhs, 2 * RIGHT), MaintainPositionRelativeTo(nc4_label, nc4))
        self.wait()
    ...
    def show_quadruplets(self, diagram, quad_words):
        ...
        indices = list(range(self.n))
        quad_labels = VGroup()
        quad_label_template = Tex('(0, 0, 0, 0)', font_size=self.tuple_font_size)
        last_label = VectorizedPoint()
        ...
        for sub_indices in it.combinations(indices, 4):
            ...
            numbers.set_opacity(0.25)
            i, j, k, l = sub_indices
            temp_lines = VGroup(Line(dots[i].get_center(), dots[k].get_center()), Line(dots[j].get_center(), dots[l].get_center()))
            int_dot = Dot(find_intersection(temp_lines[0].get_start(), temp_lines[0].get_vector(), temp_lines[1].get_start(), temp_lines[1].get_vector()), radius=0.04)
            ...
            self.add(temp_lines)
            self.play(LaggedStart(*(TransformFromCopy(dots[i], int_dot) for i in sub_indices)), lag_ratio=0.1)
            self.add(int_dot)
            self.wait(0.5)
            ...
            self.add(int_dots)
            last_label = label
        self.play(chords.animate.set_opacity(1), numbers.animate.set_opacity(1), dots.animate.set_opacity(1), int_dots.animate.set_opacity(1))
        self.wait()
...
class SimpleRect(InteractiveScene):
    ...
    def construct(self):
        self.play(FlashAround(Tex('1.2.3.4').scale(2), time_width=1.5, run_time=3, stroke_width=8))
...
class FillIn100PointDiagram(ExplainNChoose2):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(chords), run_time=9)
...
class PlanarNonPlanar(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(v_line)
        titles = VGroup(Text('Planar graph'), Text('Non-planar graph'))
...
class NonPlanarGraph(InteractiveScene):
    ...
    def construct(self):
        ...
        lines = VGroup(*(Line(d1.get_center(), d2.get_center()) for d1, d2 in it.product(dots[:3], dots[3:])))
...
class Polyhedra(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(cube, dodec)
        self.play(LaggedStart(Rotating(cube, axis=np.array([1, 1, 1]), angle=PI, rate_func=smooth, about_point=cube.get_center()), Rotating(dodec, axis=np.array([1, -1, 0]), angle=PI, rate_func=smooth, about_point=dodec.get_center()), run_time=16, lag_ratio=0.2))
...
class BalancedEquation(InteractiveScene):
    ...
    def construct(self):
        ...
        equation.to_edge(UP)
        variables = VGroup(*(equation[char][0] for char in 'VEF'))
        ...
        edges = [(0, 1), (0, 2), (0, 3), (1, 4), (2, 5), (2, 4), (5, 0), (3, 5), (1, 3)]
        faces = [(0, 1, 4, 2), (0, 2, 5), (0, 3, 5), (0, 1, 3)]
        dots = VGroup(*(Dot(point) for point in original_points))
        dots.space_out_submobjects(1.5)
        dots.move_to(DOWN)
        points = [dot.get_center() for dot in dots]
        edges = VGroup(*(Line(points[i], points[j]) for i, j in edges))
        edges.set_stroke(BLUE_B, 2)
        colors = [BLUE_E, BLUE_C, TEAL_C, GREY_BROWN]
        faces = VGroup(*(Polygon(*[points[i] for i in face]).set_fill(color, 1) for face, color in zip(faces, colors)))
        ...
        self.add(dots[0])
        self.play(Write(word, run_time=1), GrowArrow(arrow))
        self.wait()
        self.play(FlashAround(equation['V']), FlashAround(dots[0]), FadeIn(numbers[0], 0.5 * DOWN))
        self.wait(0.5)
        self.play(FlashAround(equation['F']), full_rect.animate.set_fill(BLUE_E, 0.5).set_anim_args(rate_func=there_and_back), FadeIn(numbers[2], 0.5 * DOWN))
        self.wait()
        self.play(FlashAround(equation['E']), FadeIn(numbers[1], 0.5 * DOWN))
        self.wait()
        self.play(FadeOut(word), FadeOut(arrow))
        number_y = numbers.get_y()
        increments = VGroup(Integer(1, include_sign=True), Integer(-1, include_sign=True), Integer(1, include_sign=True))
        for number, incr in zip(numbers, increments):
            incr.next_to(number, DOWN)
            incr.set_color(YELLOW)

        def incr_anim(*ns):
            return LaggedStart(*(AnimationGroup(UpdateFromAlphaFunc(increments[n], lambda m, a: m.set_y(number_y - 0.75 * a).set_opacity(there_and_back(a))), ChangeDecimalToValue(numbers[n], numbers[n].get_value() + 1, run_time=0.25)) for n in ns), lag_ratio=0.1)
        new_vert_word = TexText('New edge $\\rightarrow$ New vertex')
        new_vert_word['New vertex'].set_color(YELLOW)
        new_vert_word.next_to(dots[1], UP)
        new_vert_word.set_backstroke(width=2)
        self.play(ShowCreation(edges[0]), FadeIn(new_vert_word['New edge $\\rightarrow$'][0]))
        self.wait()
        self.play(FadeIn(dots[1], scale=0.5), FadeIn(new_vert_word['New vertex'][0], lag_ratio=0.1))
        self.play(incr_anim(0, 1))
        for i in [2, 3, 5, 4]:
            self.add(edges[i - 1], new_vert_word)
            self.play(ShowCreation(edges[i - 1]), new_vert_word.animate.next_to(dots[i], UP), FadeIn(dots[i]))
            self.play(incr_anim(0, 1))
        new_face_word = TexText('New edge $\\rightarrow$ New face')
        new_face_word['New face'].set_color(BLUE)
        new_face_word.next_to(faces[0], UP)
        self.play(FadeTransform(new_vert_word, new_face_word), ShowCreation(edges[5]))
        self.play(Write(faces[0]))
        self.play(incr_anim(1, 2))
        self.wait()
        for i, vect in zip([1, 2, 3], [UP, DOWN, DOWN]):
            self.add(edges[i + 5], dots)
            self.play(ShowCreation(edges[i + 5]), new_face_word.animate.next_to(faces[i], vect))
            self.add(faces[i], edges[:i + 5], dots, new_face_word)
            self.play(Write(faces[i]))
            self.play(incr_anim(1, 2))
        self.add(*faces, *edges, *dots)
        self.play(FadeOut(new_face_word), FadeOut(numbers))
        ...
        self.play(ShowCreation(rect), FlashAround(equation), Write(name, run_time=2))
        self.wait()
        ...
        self.remove(faces)
        self.play(ShowIncreasingSubsets(faces), ShowIncreasingSubsets(face_labels), run_time=2)
        self.add(edges, *dots)
        self.wait(0.2)
        ...
        self.wait()
        self.play(TransformMatchingTex(equation, new_equation, path_arc=90 * DEGREES))
        self.wait()
...
class OnDumbJoke(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        morty = self.teacher
        stds = self.students
        self.play(morty.change('raise_left_hand', 3 * UR), self.change_students('hesitant', 'happy', 'tease', look_at=3 * UR))
        self.wait(3)
        self.play(stds[0].says('Um, no, Euler was\na person...', mode='sassy'), stds[1].change('hesitant', stds[0].eyes), stds[2].change('jamming', stds[0].eyes), morty.change('well', stds[0].eyes))
        self.wait(4)
...
class SimpleFEq(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(F_eq)
        self.wait()
        self.play(FadeOut(F_eq['2'][0], 0.5 * UP), FadeIn(one, 0.5 * UP))
        self.wait()
...
class NonPlanarComplaint(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        morty = self.teacher
        self.remove(self.background)
        self.play(stds[2].says('But our graph\nis not planar!', mode='angry'), stds[1].change('sassy', 3 * UR), stds[0].change('hesitant', 3 * UR), morty.change('tease'))
        self.wait(3)
        self.play(morty.change('raise_left_hand'), self.change_students('angry', 'sassy', 'hesitant'))
        self.wait(4)

class VEquation(InteractiveScene):

    def construct(self):
        eq = Tex('V = n + {n \\choose 4}')
        eq.to_corner(UL)
        self.play(Write(eq))
        self.wait()
...
class EEquation(InteractiveScene):
    ...
    def construct(self):
        eq = Tex('E = {n \\choose 2} + 2 {n \\choose 4} + n', font_size=72)
        eq.to_corner(UR)
...
class FinalRearrangment(InteractiveScene):
    ...
    def construct(self):
        ...
        V_rhs = 'n + {n \\choose 4}'
        E_rhs = '{n \\choose 2} + 2{n \\choose 4} + n'
        V_eq = Tex(f'V = {V_rhs}')
        E_eq = Tex(f'E = {E_rhs}')
        ...
        t2c = {f'\\left({V_rhs}\\right)': TEAL, f'\\left({E_rhs}\\right)': BLUE_B}
        self.play(FadeIn(top_eq, UP))
        self.wait()
        top_eq2 = Tex(f'F = E - \\left({V_rhs}\\right) + 1', t2c=t2c)
        top_eq2.move_to(top_eq)
        self.play(top_eq['V'][0].animate.set_color(TEAL), FadeTransform(top_eq['V'][0].copy(), V_eq[0]), Write(V_eq[1:], run_time=1))
        self.wait()
        self.play(FadeTransform(V_eq[2:].copy(), top_eq2[f'\\left({V_rhs}\\right)']), FadeOut(top_eq['V'][0], UP), ReplacementTransform(top_eq['F = E - '], top_eq2['F = E - ']), ReplacementTransform(top_eq[-2:], top_eq2[-2:]))
        self.wait()
        E_eq.set_color(BLUE_B)
        top_eq3 = Tex(f'F = \\left({E_rhs}\\right) - \\left({V_rhs}\\right) + 1', t2c=t2c)
        top_eq3.move_to(top_eq)
        self.play(top_eq2[2].animate.set_color(BLUE_B), FadeTransform(top_eq2[2].copy(), E_eq[0]), Write(E_eq[1:], run_time=1), V_eq.animate.shift(2.0 * DOWN))
        self.wait()
        self.play(FadeTransform(E_eq[2:].copy(), top_eq3[f'\\left({E_rhs}\\right)']), FadeOut(top_eq2[2], UP), ReplacementTransform(top_eq2[:2], top_eq3[:2]), ReplacementTransform(top_eq2[-11:], top_eq3[-11:]))
        final_eq = Tex('F = 1 + {n \\choose 2} + {n \\choose 4}')
        final_eq.move_to(top_eq)
        self.play(LaggedStart(FadeOut(E_eq, DOWN), FadeOut(V_eq, DOWN)))
        self.play(FlashAround(top_eq3[14], color=RED), FlashAround(top_eq3[18], color=RED))
        self.play(FadeOut(top_eq3[13:15], DOWN), FadeOut(top_eq3[18:20], DOWN))
        self.play(FlashAround(top_eq3[9:13], color=RED), FlashAround(top_eq3[20:24], color=RED))
        self.play(FadeOut(top_eq3[8], DOWN), FadeOut(top_eq3[20:24], DOWN))
        kw = dict(path_arc=90 * DEGREES)
        self.play(LaggedStart(Transform(top_eq3[:2], final_eq[:2], **kw), Transform(top_eq3[26], final_eq[2], **kw), Transform(top_eq3[25], final_eq[3], **kw), Transform(top_eq3[3:7], final_eq[4:8], **kw), Transform(top_eq3[7], final_eq[8], **kw), Transform(top_eq3[9:13], final_eq[9:13], **kw), *(FadeOut(top_eq3[i], DOWN) for i in [2, 15, 16, 17, 24]), run_time=2))
        self.wait()

class ExamplesOfFormula(InteractiveScene):

    def construct(self):
        examples = VGroup(*(self.get_example(n) for n in range(1, 7)))
    ...
    def get_example(self, n):
        result = VGroup(Tex(f'1 + {{{n} \\choose 2 }} + {{{n} \\choose 4 }}'), Tex(f'= 1 + {choose(n, 2)} + {choose(n, 4)} = {1 + choose(n, 2) + choose(n, 4)}'))
...
class WhyThePowersOfTwo(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        morty = self.teacher
        self.remove(self.background)
        self.play(LaggedStart(stds[0].change('confused', self.screen), stds[1].says('Why the powers \n of 2?'), stds[2].change('pondering', self.screen), morty.change('tease')))
        self.wait(6)
...
class ChallengeProblem(InteractiveScene):
    ...
    def construct(self):
        numbers = VGroup(*(Integer(1 + choose(n, 2) + choose(n, 4)) for n in range(100)))
        ...
        question = VGroup(Text('Challenge question', font_size=72).set_fill(YELLOW), Text('Will there ever be another power of 2?'))
...
class AskAboutSome(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(morty.change('raise_left_hand', words), FadeIn(words, 0.25 * UP, lag_ratio=0.01))
        self.play(Blink(morty))
        self.wait(3)
        self.play(TransformMatchingStrings(words, small_words, path_arc=45 * DEGREES), morty.change('raise_right_hand'))
        self.play(Blink(morty))
        self.wait(2)
        self.play(VFadeIn(randy), randy.change('shruggie', three_some), morty.change('sassy', randy.eyes), TransformMatchingStrings(small_words, three_some, path_arc=45 * DEGREES))
        self.play(morty.change('angry', randy.eyes))
        self.play(Blink(morty))
        self.play(TransformMatchingStrings(three_some, small_words_copy, run_time=1), morty.change('raise_right_hand'), randy.change('sassy'))
        self.play(Blink(morty))
