...
class TransitionToContinuousProbability(InteractiveScene):
    ...
    def construct(self):
        ...
        all_rects = VGroup(*(axes.get_riemann_rectangles(graph, x_range=(0, min(6 + n, 12)), dx=1 / n, input_sample_type='right').set_stroke(WHITE, width=2.0 / n, opacity=2.0 / n, behind=False) for n in (*range(1, 10), *range(10, 20, 2), *range(20, 100, 5))))
        area = all_rects[-1]
        area.set_stroke(width=0)
        self.remove(bars)
        self.play(ShowSubmobjectsOneByOne(all_rects, rate_func=bezier([0, 0, 0, 0, 1, 1])), FadeOut(y_label), run_time=5)
        ...
        x_labels = VGroup(*(Text(text) for text in ['Temperature tomorrow at noon', 'Value of XYZ next year', 'Time before the next bus comes']))
        ...
        for xl1, xl2 in zip(x_labels, x_labels[1:]):
            self.wait()
            self.play(FadeOut(xl1, 0.5 * UP), FadeIn(xl2, 0.5 * UP))
        self.wait()
        self.play(FadeOut(x_labels[-1]), x_tracker.animate.set_value(0).set_anim_args(run_time=3))
        self.play(x_tracker.animate.set_value(12), run_time=5)
        ...
        self.wait()
        self.play(VGroup(y_label, cross).animate.shift(0.75 * UP), FadeIn(density), self.frame.animate.set_y(0))
        self.wait()
        ...
        prob_label = Tex('P(a < x < b) = \\text{This area}')
        prob_label.move_to(2 * UR)
        rhs = prob_label['\\text{This area}']
        prob_arrow = Arrow(LEFT, RIGHT)
        prob_arrow.add_updater(lambda m: m.put_start_and_end_on(rhs.get_bottom() + 0.1 * DOWN, sub_area.get_center()))
        self.add(area, sub_area, graph, bound_labels)
        self.play(area.animate.set_opacity(0.1), range_tracker.animate.set_value([3, 4.5]), sub_area_opacity_tracker.animate.set_value(1), VFadeIn(v_lines), FadeIn(prob_label), VFadeIn(prob_arrow), run_time=2)
        self.wait()
        ...
        self.wait()
        int_rhs = Tex('\\int_a^b p_X(x) \\, dx')
        int_rhs.move_to(rhs, LEFT)
        self.play(rhs.animate.set_opacity(0).shift(0.5 * DOWN + 1.0 * LEFT), FadeIn(int_rhs, DL))
        self.wait()
...
class CompareFormulas(InteractiveScene):
    ...
    def construct(self):
        ...
        disc_title, cont_title = titles = VGroup(Text('Discrete case', **kw), Text('Continuous case', **kw))
        ...
        all_axes = VGroup(Axes((-2, 2), **kw), Axes((-2, 2), **kw), Axes((-3, 3), **kw))
        ...
        graphs = VGroup(all_axes[0].get_graph(wedge_func, use_smoothing=False).set_stroke(BLUE), all_axes[1].get_graph(double_lump).set_stroke(RED), get_conv_graph(all_axes[2], wedge_func, double_lump).set_stroke(TEAL))
        graphs.set_stroke(width=2)
        tex_kw = dict()
        labels = VGroup(Tex('f(x)', font_size=24), Tex('g(y)', font_size=24), Tex('[f * g](s)', font_size=36))
        for label, axes in zip(labels, all_axes):
            label.move_to(midpoint(axes.get_corner(UR), axes.get_top()), UP)
        plots = VGroup(*(VGroup(*tup) for tup in zip(all_axes, graphs, labels)))
        self.add(plots)
        disc_formula = Tex('\\big[P_X * P_Y\\big](s) = \\sum_{x = 1}^N P_X(x) \\cdot P_Y(s - x)', font_size=36, t2c={'X': BLUE, 'Y': RED})
        disc_formula.next_to(disc_title, DOWN, MED_LARGE_BUFF)
        cont_formula = Tex('\\big[f * g \\big](s) = \\int_{-\\infty}^\\infty f(x) g(s - x) \\, dx', font_size=36)
        cont_formula.match_x(cont_title)
        cont_formula.match_y(disc_formula)
        self.play(FadeIn(disc_formula, run_time=2, lag_ratio=0.1), FlashAround(disc_formula, time_width=1.5, run_time=2))
        self.wait()
        rect = SurroundingRectangle(disc_formula)
        rect.set_stroke(YELLOW, 2, opacity=0)
        target_rects = VGroup(*(SurroundingRectangle(disc_formula[s][0]) for s in ['P_X', 'P_Y', 'P_X * P_Y', re.compile('\\\\sum.*')]))
        ...
        self.play(TransformMatchingTex(disc_formula.copy(), cont_formula, lag_ratio=-0.001, path_arc=-0.1 * PI))
        self.wait()
        fade_rect = FullScreenFadeRectangle()
        randy = Randolph()
        randy.next_to(cont_formula, DL, LARGE_BUFF)
        self.add(fade_rect, cont_formula)
        self.play(FadeIn(fade_rect), VFadeIn(randy), randy.change('horrified', cont_formula))
...
class RepeatedSamplesFromContinuousDistributions(InteractiveScene):
    ...
    def get_plots(self):
        ...
        pdfs = self.get_pdfs()
        graphs = VGroup(*(axes.get_graph(func).set_stroke(color) for axes, func, color in zip(all_axes, self.get_pdfs(), self.graph_colors)))
    ...
    def get_axes(self):
        return VGroup(*(Axes((-5, 5), (0, 0.5, 0.25), width=5.5, height=2) for x in range(3)))

    def get_axes_labels(self, all_axes):
        a1, a2, a3 = all_axes
        return VGroup(Tex('X').move_to(midpoint(a1.get_corner(UR), a1.get_top())), Tex('Y').move_to(midpoint(a2.get_corner(UR), a2.get_top())), Tex('X + Y').next_to(a3, UP, buff=1.0))

    def repeated_samples(self, plots, n_repetitions, **kwargs):
        for n in range(n_repetitions):
            self.animate_samples(plots, **kwargs)

    def animate_samples(self, plots, time_between_samples=0.25, time_before_fade=1.0, animate=True):
        ...
        for sample, plot in zip(samples, plots):
            ...
            lines.add(line)
        if len(plots) > 2:
            sum_label = VGroup(DecimalNumber(samples[0]), Tex('+') if samples[1] > 0 else Tex('-'), DecimalNumber(abs(samples[1])), Tex('='), DecimalNumber(samples[2]))
        ...
        if len(plots) > 2:
            self.play(LaggedStart(Transform(dots[0].copy(), dots[2].copy().set_opacity(0.5), remover=True), Transform(dots[1].copy(), dots[2].copy().set_opacity(0.5), remover=True), FadeTransform(sum_label[:3].copy(), sum_label[3:]), run_time=1.0 if animate else 0))
            self.add(sum_label)
            self.add(dots[2])
        self.wait(time_before_fade)
        kw = dict(run_time=0.25 if animate else 0)
        self.play(LaggedStart(*(dot.animate.set_radius(0.1).set_opacity(self.dot_fade_factor) for dot in dots), **kw), LaggedStartMap(FadeOut, labels, **kw), LaggedStartMap(FadeOut, lines[:2], **kw))
        self.all_dots.add(*dots)
        self.add(self.all_dots)

    def get_random_variables(self):
        return [scipy.stats.norm(0, self.sigma1), scipy.stats.norm(0, self.sigma2)]

    def get_samples(self):
        return [np.round(var.rvs(), 2) for var in self.random_variables]
...
class SampleTwoNormals(RepeatedSamplesFromContinuousDistributions):
    ...
    def construct(self):
        ...
        normal_parameters = VGroup(*(self.get_normal_parameter_labels(plot, 0, sigma) for plot, sigma in zip(plots, [self.sigma1, self.sigma2])))
        normal_words = VGroup(*(Text('Normal\ndistribution', font_size=30, alignment='LEFT').next_to(parameters, UP, MED_LARGE_BUFF, LEFT) for parameters in normal_parameters))
        if self.annotations:
            plots.set_opacity(0)
        frame = self.frame
        ...
        if self.annotations:
            ...
            self.play(FadeIn(rv_words, lag_ratio=0.1), ShowCreation(label_rect), GrowArrow(rv_arrow))
            self.wait()
            self.play(FadeTransform(rv_words.copy(), sample_words), TransformFromCopy(rv_arrow, sample_arrow), FadeOut(label_rect))
            self.wait()
            curve_copy = graph.copy()
            curve_copy.set_stroke(TEAL, 7, 1)
            self.play(LaggedStartMap(FadeOut, VGroup(sample_words, sample_arrow, rv_arrow, rv_words)), Write(normal_words[0], run_time=2), FadeIn(normal_parameters[0]), VShowPassingFlash(curve_copy, time_width=0.7, time_span=(0.5, 5)))
            self.wait()
            bound_tracker = ValueTracker([-1, -1])
            area = always_redraw(lambda: axes.get_area_under_graph(graph, bound_tracker.get_value()))
            self.add(area)
            self.play(bound_tracker.animate.set_value([-1, 2]), run_time=3)
            self.wait()
        ...
        self.play(MoveToTarget(frame), FadeIn(plots[1]), FadeOut(self.all_dots))
        self.all_dots.clear()
        if self.annotations:
            self.play(TransformFromCopy(*normal_words))
            self.play(LaggedStartMap(FadeIn, normal_parameters[1], lag_ratio=0.25), LaggedStartMap(FlashAround, normal_parameters[1], stroke_width=1, time_width=1.0, lag_ratio=0.25))
            self.wait()
        else:
            self.repeated_samples(plots[:2], 10, time_before_fade=0.5)
        self.play(frame.animate.to_default_state(), FadeIn(plots[2]), FadeOut(self.all_dots))
        self.all_dots.clear()
        if self.annotations:
            axes, graph, label = plots[2]
            graphs = VGroup(graph.copy(), axes.get_graph(lambda x: 0.3 * np.exp(-0.1 * x ** 4)), axes.get_graph(lambda x: 0.3 * (1 / (1 + x ** 2))))
            graphs.set_stroke(TEAL, 2, 1)
            kw = dict(font_size=24)
            words = [Text('Another\nnormal?', **kw), Text('Something\nnew?', **kw), Text('Maybe this?', **kw)]
            for word in words:
                word.move_to(axes)
                word.align_to(axes.x_axis.get_start(), LEFT)
            curr_graph = graphs[0].copy()
            self.play(ShowCreation(curr_graph), FadeIn(words[0], 0.5 * UP))
            self.wait()
            for i in range(2):
                self.play(FadeOut(words[i], 0.5 * UP), FadeIn(words[i + 1], 0.5 * UP), Transform(curr_graph, graphs[i + 1]))
                self.wait()
            self.wait()
            self.play(FadeOut(words[-1]), Transform(curr_graph, graphs[0]))
            self.wait()
            self.play(FadeOut(curr_graph, run_time=3))
        else:
            self.repeated_samples(plots, 10, time_between_samples=0.25, time_before_fade=1.0)
            self.repeated_samples(plots[:3], 100, time_between_samples=1 / 30, time_before_fade=0.2, animate=False)

    def get_normal_parameter_labels(self, plot, mean, sigma, font_size=18, color=GREY_A):
        kw = dict(font_size=font_size)
        labels = VGroup(Tex('\\text{Mean} = 0.0', **kw), Tex('\\text{Std. Dev.} = 0.0', **kw))
...
class AddTwoGammaDistributions(RepeatedSamplesFromContinuousDistributions):
    ...
    def construct(self):
        ...
        graph_labels = VGroup(Tex('e^{-x}', **kw), Tex('\\frac{1}{2} x^2 \\cdot e^{-x}', **kw), Tex('\\frac{1}{6} x^3 \\cdot e^{-x}', **kw))
        ...
        self.repeated_samples(plots, 40, animate=False, time_between_samples=0.1, time_before_fade=0.5)
        frame = self.frame
        ...
        graph_groups = VGroup(*(VGroup(plot[1], label).copy() for plot, label in zip(plots, graph_labels)))
        ...
        equation = VGroup(lp.copy(), graph_groups.target[0], rp.copy(), Tex('*', **kw), lp.copy(), graph_groups.target[1], rp.copy(), Tex('=', **kw), graph_groups.target[2])
        ...
        symbols = VGroup(*(mob for mob in equation if mob not in graph_groups.target))
        self.play(frame.animate.set_height(13, about_point=3 * DOWN), FadeIn(fuller_rect), FadeIn(fs_rect), MoveToTarget(graph_groups, run_time=2), Write(symbols, run_time=2))
        self.wait()
        ...
        self.repeated_samples(plots, 50, animate=False, time_between_samples=0.1, time_before_fade=0.5)

    def get_axes(self):
        kw = dict(width=5.5, height=2)
        return VGroup(Axes((0, 10), (0, 1.0, 0.25), **kw), Axes((0, 10), (0, 0.5, 0.25), **kw), Axes((0, 10), (0, 0.5, 0.25), **kw))

    def get_random_variables(self):
        return [scipy.stats.gamma(1), scipy.stats.gamma(3)]
...
class SampleWedgePlusDoubleLump(RepeatedSamplesFromContinuousDistributions):
    ...
    def get_axes(self):
        return VGroup(*(Axes((-2, 2), (0, 1, 0.25), width=5.5, height=2.5) for x in range(3)))
...
class ContinuousSampleAnnotations(SampleWedgePlusDoubleLump):
    ...
    def construct(self):
        plots = self.get_plots()
        plots[0][1].make_jagged()
        thick_graphs = VGroup(*(plot[1].copy().set_stroke(width=10) for plot in plots))
        ...
        for graph, label in zip(thick_graphs, func_labels):
            ...
            self.play(Write(label, time_span=(1, 2)), VShowPassingFlash(graph, time_width=1.5, run_time=3))
...
class UniformSamples(RepeatedSamplesFromContinuousDistributions):
    ...
    def construct(self):
        ...
        for plot, func in zip(plots, funcs):
            ...
            axes.y_axis.add_numbers(np.arange(0.5, 2.5, 0.5), font_size=12, buff=0.15, num_decimal_places=1)
            new_graph = axes.get_graph(func, x_range=(-2, 2, 0.01), use_smoothing=False)
    ...
    def get_axes(self):
        return VGroup(*(Axes((-2, 2), (0, 2, 0.5), width=5.5, height=2) for x in range(3)))
...
class WedgeAndExpSamples(SampleWedgePlusDoubleLump):
    ...
    def get_axes(self):
        return VGroup(Axes((-2, 2), (0, 1, 0.25), width=5.5, height=2), Axes((-2, 5), (0, 1.0, 0.25), width=5.5, height=2), Axes((-3, 6), (0, 1.0, 0.25), width=5.5, height=2))
...
class Convolutions(InteractiveScene):
    axes_config = dict(x_range=(-3, 3, 1), y_range=(-1, 1, 1.0), width=6, height=2)
    ...
    f_label_tex = 'f(x)'
    g_label_tex = 'g(s - x)'
    fg_label_tex = 'f(x) \\cdot g(s - x)'
    conv_label_tex = '[f * g](s) = \\int_{-\\infty}^\\infty f(x) \\cdot g(s - x) dx'
    ...
    def setup(self):
        ...
        if self.g_is_rect:
            ...
            g_graph.add_updater(lambda m: m.set_points_as_corners([g_axes.c2p(x, y) for s in [get_s()] for k1 in [k1_tracker.get_value()] for k2 in [k2_tracker.get_value()] for x, y in [(x_min, 0), (-0.5 / k1 + s, 0), (-0.5 / k1 + s, k2), (0.5 / k1 + s, k2), (0.5 / k1 + s, 0), (x_max, 0)]]))
        else:
        ...
        kw = dict(jagged=self.jagged_product, get_discontinuities=get_discontinuities)
        ...
        graph_dot.add_updater(lambda d: d.move_to(conv_graph.quick_point_from_proportion(inverse_interpolate(x_min, x_max, get_s()))))
        graph_line = self.graph_line = Line(stroke_color=WHITE, stroke_width=1)
        graph_line.add_updater(lambda l: l.put_start_and_end_on(graph_dot.get_center(), [graph_dot.get_x(), conv_axes.get_y(), 0]))
    ...
    def get_all_axes(self):
        ...
        all_axes[3].y_axis.stretch(self.conv_y_stretch_factor, 1)
    ...
    def get_conv_s_indicator(self):
        ...
        conv_s_indicator.add_updater(lambda m: m.shift(conv_axes.c2p(get_s(), 0) - g_axes.c2p(get_s(), 0)))
...
class ProbConvolutions(Convolutions):
    ...
    def construct(self):
        ...
        alt_fg_label = Tex('p_X(x) \\cdot g(-x)', **self.label_config)
        ...
        self.play(TransformFromCopy(f_term, f_label), FadeIn(f_axes))
        self.play(ShowCreation(f_graph), VShowPassingFlash(f_graph.copy().set_stroke(width=5)), run_time=2)
        self.wait()
        ...
        self.play(TransformFromCopy(g_term, gy_label), FadeIn(g_axes), FadeIn(y_label))
        self.play(ShowCreation(true_g_graph), VShowPassingFlash(true_g_graph.copy().set_stroke(width=5)), run_time=2)
        self.wait()
        int_rect = SurroundingRectangle(conv_label[re.compile('\\\\int.*')])
        x_rects = VGroup(*(SurroundingRectangle(x, buff=0.05) for x in conv_label['x']))
        ...
        x_brace, y_brace, s_brace = braces = VGroup(*(Brace(term, UP, SMALL_BUFF) for term in [x_term, y_term, s_term]))
        ...
        x_line = always_redraw(lambda: f_axes.get_v_line_to_graph(get_x(), f_graph, line_func=Line, color=WHITE))
        y_line = always_redraw(lambda: g_axes.get_v_line_to_graph(get_y(), true_g_graph, line_func=Line, color=WHITE))
        ...
        self.wait()
        self.play(LaggedStart(conv_cover.animate.set_opacity(1), FadeIn(equation), FadeIn(braces), VFadeIn(x_line), VFadeIn(y_line), FadeIn(x_dot), FadeIn(y_dot)))
        for x in [1.0, -1.0]:
            self.play(x_tracker.animate.set_value(x), run_time=8)
        self.wait()
        self.remove(int_rect)
        self.play(*(ReplacementTransform(int_rect.copy(), x_rect) for x_rect in x_rects))
        self.wait()
        self.play(FadeOut(x_rects, lag_ratio=0.5))
        self.play(FadeTransform(conv_label['s - x'].copy(), alt_y_label), y_brace[-1].animate.set_opacity(0))
        self.remove(alt_y_label)
        y_brace[-1].become(alt_y_label)
        for x in [1.0, -1.0]:
            self.play(x_tracker.animate.set_value(x), run_time=8)
        self.play(LaggedStart(*map(FadeOut, [x_line, x_dot, y_line, y_dot, *equation, *braces])), lag_ratio=0.2)
        ...
        self.wait()
        self.play(true_group.animate.to_edge(DOWN, buff=MED_SMALL_BUFF))
        self.add(*true_group)
        g_axes.generate_target()
        (g_axes.target.x_label.set_opacity(1),)
        self.play(TransformMatchingShapes(gy_label.copy(), gmx_label), true_g_graph.copy().animate.flip().move_to(g_graph).set_anim_args(remover=True), MoveToTarget(g_axes))
        self.add(g_graph)
        self.wait()
        self.play(FadeOut(true_group))
        self.play(s_indicator.animate.match_x(g_axes.c2p(2, 0)).set_anim_args(run_time=3), VFadeIn(s_indicator), VFadeIn(s_label), TransformMatchingTex(gmx_label, g_label, run_time=1))
        self.wait()
        self.play(s_indicator.animate.match_x(g_axes.c2p(0.3, 0)))
        fg_rect = SurroundingRectangle(conv_label['f(x) \\cdot g(s - x)'])
        fg_rect.match_style(g_rect)
        self.play(ReplacementTransform(gsmx_rect, fg_rect))
        self.play(LaggedStart(FadeTransform(f_axes.copy(), fg_axes), FadeTransform(g_axes.copy(), fg_axes), Transform(f_graph.copy(), prod_graphs[0].copy(), remover=True), Transform(g_graph.copy(), prod_graphs[0].copy(), remover=True), TransformFromCopy(VGroup(*f_label, *g_label), fg_label), FadeOut(fg_rect), run_time=2))
        self.add(*prod_graphs)
        self.play(FadeIn(prod_graphs[1]))
        self.add(prod_graphs)
        self.wait()
        self.play(s_indicator.animate.match_x(g_axes.c2p(-0.8, 0)))

        def get_s():
            return g_axes.x_axis.p2n(s_indicator.get_x())
        conv_s_indicator = self.get_conv_s_indicator()
        self.play(FadeOut(conv_cover))
        self.play(Transform(VGroup(indicator, s_label).copy().clear_updaters(), conv_s_indicator.copy().clear_updaters(), remover=True))
    ...
    def highlight_several_regions(self, highlighted_xs=None, s=0, reference=None):
        ...
        for x in highlighted_xs:
            ...
            label = Tex(f'p_X({x_tex}) \\cdot p_Y({diff_tex})', tex_to_color_map={diff_tex: YELLOW, x_tex: BLUE}, font_size=36)
            if reference:
...
class ConvolveTwoUniforms(Convolutions):
    jagged_product = True
    jagged_convolution = True
    axes_config = dict(x_range=(-2, 2, 0.5), y_range=(-1, 1, 1.0), width=6, height=2)
    f_graph_x_step = 0.025
    g_graph_x_step = 0.025
    conv_y_stretch_factor = 1.0

    def construct(self):
        self.all_axes[0].x_axis.add_numbers(font_size=16, num_decimal_places=1, excluding=[0], buff=0.1)
...
class MovingAverageAsConvolution(Convolutions):
    ...
    def construct(self):
        ...
        def change_ks(k1, k2, run_time=3):
            new_conv_graph = get_conv_graph(self.all_axes[3], self.f, lambda x: self.g(k1 * x) * k2)
            new_conv_graph.match_style(self.conv_graph)
            self.play(self.k1_tracker.animate.set_value(k1), self.k2_tracker.animate.set_value(k2), Transform(self.conv_graph, new_conv_graph), run_time=run_time)
        top_line.set_stroke(WHITE, 3)
        side_line.set_stroke(RED, 3)
        self.play(ShowCreation(side_line), VFadeIn(side_label))
        self.wait()
        self.play(ShowCreation(top_line), VFadeIn(top_label))
        self.wait()
        change_ks(0.5, 1)
        self.wait()
        change_ks(0.5, 0.5)
        ...
        avg_label = TexText('Average value of\\\\$f(x)$ in the window', font_size=24)
        ...
        self.play(Write(area_label, stroke_color=WHITE), ShowCreation(arrow), FadeIn(rect))
        self.wait()
        self.play(FadeIn(avg_label, lag_ratio=0.1), ShowCreation(arrow2))
        self.wait()
...
class RepeatedConvolution(MovingAverageAsConvolution):
    ...
    f_label_tex = 'f_1(x)'
    g_label_tex = 'f_1(s - x)'
    fg_label_tex = 'f_1(x) \\cdot f_1(s - x)'
    conv_label_tex = 'f_2(s) = [f_1 * f_1](s)'
    conv_y_stretch_factor = 1.0
    ...
    def construct(self):
        ...
        self.remove(self.f_graph, self.prod_graphs, self.conv_graph, self.conv_graph_dot, self.conv_graph_line)
        ...
        def update_prod_graph(prod_graph):
            s = self.get_s()
            prod_samples = np.array([f_sample * self.g(s - x) for f_sample, x in zip(f_samples, x_samples)])
            prod_graph.set_points_as_corners(axes3.c2p(x_samples, prod_samples))
        prod_graph = VMobject()
        ...
        for n in range(1, self.n_iterations + 1):
            conv_samples, conv_graph = self.create_convolution(x_samples, f_samples, g_samples, conv_axes)
    ...
    def create_convolution(self, x_samples, f_samples, g_samples, conv_axes):
        ...
        conv_samples, conv_graph = self.get_conv(x_samples, f_samples, g_samples, conv_axes)
        endpoint_dot = GlowDot(color=WHITE)
        endpoint_dot.add_updater(lambda m: m.move_to(conv_graph.get_points()[-1]))
        self.play(self.set_s(x_samples[-1]), ShowCreation(conv_graph), UpdateFromAlphaFunc(endpoint_dot, lambda m, a: m.move_to(conv_graph.get_end()).set_opacity(min(6 * a, 1))), run_time=self.convolution_creation_time, rate_func=bezier([0, 0, 1, 1]))
    ...
    def swap_graphs(self, f_graph, conv_graph, f_axes, conv_axes, n):
        ...
        anims = [Transform(conv_axes_copy, f_axes, remover=True), new_f_graph.animate.shift(shift_value).match_style(f_graph), FadeOut(f_group, shift_value), new_f_label.animate.replace(f_label, dim_to_match=1).set_opacity(1)]
    ...
    def swap_labels(self, n, conv_graph):
        ...
        to_remove = VGroup(self.conv_label[f'f_{{{n + 1}}}'], self.conv_label[f'f_{{{n}}}'], prod_rhs)
        to_add = VGroup(new_conv_label[f'f_{{{n + 2}}}'], new_conv_label[f'f_{{{n + 1}}}'], new_prod_rhs)
        anims = [LaggedStartMap(FadeOut, to_remove, shift=0.5 * UP), LaggedStartMap(FadeIn, to_add, shift=0.5 * UP), FadeOut(conv_graph)]
    ...
    def add_rescale_arrow(self):
        ...
        label = TexText('Rescale so that\\\\std. dev. = 1', font_size=30)
        label.next_to(arrow)
    ...
    def get_conv(self, x_samples, f_samples, g_samples, axes):
        conv_samples = self.resolution * scipy.signal.fftconvolve(f_samples, g_samples, mode='same')
        conv_graph = VMobject()
    ...
    def get_conv_label(self, n):
        lhs = f'f_{{{n}}}(s)'
        last = f'f_{{{n - 1}}}'
        result = Tex(lhs, '=', '\\big[', last, '*', 'f_1', '\\big]', '(s)')
...
class RepeatedConvolutionDoubleLump(RepeatedConvolution):
    ...
    g_is_rect = False
    axes_config = dict(x_range=(-5, 5, 1), y_range=(-1, 1, 1.0), width=6, height=2)
...
class RepeatedConvolutionExp(RepeatedConvolutionDoubleLump):
    ...
    axes_config = dict(x_range=(-10, 10, 1), y_range=(-1, 1, 1.0), width=6, height=2)
...
class RepeatedConvolutionGaussian(RepeatedConvolution):
    g_is_rect = False
    when_to_renormalize = 1
    axes_config = dict(x_range=(-7, 7, 1), y_range=(-0.5, 0.5, 0.5), width=6, height=2)
...
class RectConvolutionsNewNotation(MovingAverageOfRectFuncs):
    ...
    def construct(self):
        x_min, x_max = (-1.0, 1.0)
        all_axes = axes1, axes2, axes3 = VGroup(*(Axes((x_min, x_max, 0.5), (0, 5), width=3.75, height=4) for x in range(3)))
        ...
        rect_defs = VGroup(self.get_rect_func_def(), *(self.get_rect_k_def(k) for k in k_range))
        ...
        conv_labels = VGroup(Tex('\\big[\\text{rect} * \\text{rect}_3\\big](x)'), Tex('\\big[\\text{rect} * \\text{rect}_3 * \\text{rect}_5\\big](x)'), Tex('\\big[\\text{rect} * \\text{rect}_3 * \\text{rect}_5 * \\text{rect}_7 \\big](x)'))
        conv_labels.scale(0.75)
        conv_labels.match_x(axes3).match_y(rect_defs)
        rect_graphs = VGroup(*(self.get_rect_k_graph(axes2, k) for k in [1, *k_range]))
        ...
        self.wait()
        self.play(Transform(rect_defs[0][:4].copy(), conv_labels[0][0][1:5], remover=True, path_arc=-PI / 3), Transform(rect_defs[1][:5].copy(), conv_labels[0][0][6:11], remover=True, path_arc=-PI / 3), FadeIn(conv_labels[0][0], lag_ratio=0.1, time_span=(1.5, 2.5)), FadeOut(rect), run_time=2)
        self.wait()
        for n in range(2):
            ...
            self.play(FadeOut(left_graph, 1.5 * LEFT), FadeOut(lefs_label, 1.5 * LEFT), FadeOut(rect_defs[n + 1]), FadeOut(rect_graphs[n + 1]), conv_labels[n].animate.match_x(axes1), conv_graphs[n].animate.match_x(axes1))
            self.play(Write(rect_defs[n + 2], stroke_color=WHITE), ShowCreation(rect_graphs[n + 2]), FadeIn(new_rect), run_time=1)
            self.wait()
            left_conv = conv_labels[n][0][1:-4]
            r = len(left_conv) + 1
            self.play(Transform(left_conv.copy(), conv_labels[n + 1][0][1:r], remover=True, path_arc=-PI / 3), Transform(rect_defs[2][:5].copy(), conv_labels[n + 1][0][r + 1:r + 6], remover=True, path_arc=-PI / 3), FadeIn(conv_labels[n + 1][0], lag_ratio=0.1, time_span=(0.5, 1.5)), ShowCreation(conv_graphs[n + 1]))
    ...
    def get_rect_k_graph(self, axes, k):
        ...
        x_range[2] = 1 / k
        return axes.get_graph(lambda x: k * rect_func(k * x), discontinuities=(-1 / (2 * k), 1 / (2 * k)), stroke_color=YELLOW, stroke_width=3)
...
class RectConvolutionFacts(InteractiveScene):
    ...
    def construct(self):
        equations = VGroup(Tex('\\text{rect}', '(0)', '=', '1.0'), Tex('\\big[', '\\text{rect}', '*', '\\text{rect}_3', '\\big]', '(0)', '=', '1.0'), Tex('\\big[', '\\text{rect}', '*', '\\text{rect}_3', '*', '\\text{rect}_5', '\\big]', '(0)', '=', '1.0'), Tex('\\vdots'), Tex('\\big[', '\\text{rect}', '*', '\\text{rect}_3', '*', '\\cdots', '*', '\\text{rect}_{13}', '\\big]', '(0)', '=', '1.0'), Tex('\\big[', '\\text{rect}', '*', '\\text{rect}_3', '*', '\\cdots', '*', '\\text{rect}_{13}', '*', '\\text{rect}_{15}', '\\big]', '(0)', '=', SUB_ONE_FACTOR + '\\dots'))
        for eq in equations:
