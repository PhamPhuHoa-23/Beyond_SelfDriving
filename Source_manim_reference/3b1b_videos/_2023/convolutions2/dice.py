...
def get_bar_group(dist, bar_colors=(BLUE_D, TEAL_D), value_labels=None, width_ratio=0.7, height=2.0, number_config=dict(), label_buff=SMALL_BUFF):
    ...
    labeled_bars = VGroup(*(VGroup(bar, value_label, p_label) for bar, value_label, p_label in zip(bars, value_labels, p_labels)))
...
def die_sum_labels(color1=BLUE_E, color2=RED_E, height=1.0):
    ...
    result = VGroup()
    for n in range(2, 13):
        sum_sym = VGroup(die1.copy(), Tex('+', font_size=24), die2.copy(), Tex('=', font_size=24).rotate(90 * DEGREES), Tex(str(n), font_size=30))
...
class SumAlongDiagonal(InteractiveScene):
    ...
    def construct(self):
        ...
        bar_groups = VGroup(get_bar_group(dist1, self.dist1_colors, blue_dice), get_bar_group(dist2, self.dist2_colors, red_dice))
        bar_groups.arrange(DOWN, buff=LARGE_BUFF)
        bar_groups.to_edge(LEFT)
        self.add(bar_groups)
        conv_dist = np.convolve(dist1, dist2)
        sum_labels = die_sum_labels()
        sum_bar_group = get_bar_group(conv_dist, self.sum_colors, sum_labels, number_config=dict(num_decimal_places=3, font_size=30), label_buff=MED_SMALL_BUFF)
        ...
        self.add(sum_labels)
        self.add(buckets)
        self.show_repeated_samples(dist1, dist2, *bar_groups, buckets, sum_labels, n_animated_runs=1, n_total_runs=2)
        ...
        self.wait()
        self.play(TransformMatchingStrings(words1, words2, run_time=1), FadeOut(rects[0]), FadeIn(rects[1]))
        self.wait()
        for i in range(1, len(rects) - 1):
            self.play(FadeOut(rects[i]), FadeIn(rects[i + 1]), words2.animate.match_x(rects[i + 1]), run_time=0.5)
        self.wait()
        ...
        self.play(ShowCreation(grid, lag_ratio=0.5), TransformFromCopy(blue_dice, blue_row), TransformFromCopy(red_dice, red_col))
        dice_pairs = VGroup()
        anims = []
        for n, square in enumerate(grid):
            templates = VGroup(blue_row[n // 6], red_col[n % 6])
            pair = templates.copy()
            pair.arrange(RIGHT, buff=SMALL_BUFF)
            pair.set_width(square.get_width() * 0.7)
            pair.move_to(square)
            dice_pairs.add(pair)
            anims.extend([TransformFromCopy(templates, pair)])
        self.play(LaggedStart(*anims, lag_ratio=0.1))
        self.add(dice_pairs)
        self.wait()
        full_table = VGroup(blue_row, red_col, grid, dice_pairs)
        pairs = [(1, 1), (1, 2), (1, 3), (1, 4), (1, 5), (2, 5), (2, 4), (2, 3), (3, 3), (3, 2), (4, 2)]
        for pair in pairs:
        ...
        def get_p_label(dice):
            p_label = VGroup(p_mob(dice), Tex('='), p_mob(dice[0]), p_mob(dice[1]))
        ...
        movers = VGroup(dice_pairs[(i0 - 1) * 6 + j0 - 1], blue_row[i0 - 1], red_col[j0 - 1]).copy()
        self.play(bar_groups.animate.set_width(2.5, about_edge=DL), full_table.animate.set_width(5.5, about_edge=DR), LaggedStart(movers[0].animate.replace(p_label[0][2]), movers[1].animate.replace(p_label[2][2]), movers[2].animate.replace(p_label[3][2]), lag_ratio=0.25))
        ...
        self.play(LaggedStart(Transform(bar_groups[0][i0 - 1][2].copy(), value1.copy(), remover=True), Transform(bar_groups[1][j0 - 1][2].copy(), value2.copy(), remover=True), Write(prod_rhs, lag_ratio=0.1), lag_ratio=0.5))
        self.wait()
        ...
        self.play(morty.says('Assuming rolls\nare independent!', mode='surprised'), VFadeIn(morty))
        ...
        self.play(MoveToTarget(full_table), example.animate.scale(0.75, about_edge=UL).shift(0.5 * LEFT))
        ...
        self.play(FadeOut(example), bar_groups.animate.set_opacity(1))
        self.wait()
        ...
        for square, prob in zip(grid, grid_probs):
            ...
            prism.set_opacity(0)
            bars_3d.add(prism)
        self.play(LaggedStartMap(Restore, bars_3d), self.frame.animate.reorient(12, 65, 0).move_to([-0.48, 0.37, 0.77]).set_height(9.43), run_time=3)
        self.add(full_table, *bars_3d)
        self.play(self.frame.animate.reorient(43, 66, 0).move_to([-0.06, 0.21, -0.29]).set_height(10.59), run_time=7)
        self.wait()
        self.play(LaggedStartMap(FadeOut, bar_groups, shift=2 * LEFT), FadeIn(sum_bar_group), full_table.animate.to_edge(LEFT), self.frame.animate.reorient(4, 65, 0).move_to([0.64, 0.8, 0.69]).set_height(10.59), *(MaintainPositionRelativeTo(bar, full_table) for bar in bars_3d), run_time=2)
        self.wait()
        self.play(self.frame.animate.to_default_state(), FadeOut(bars_3d), run_time=2)
        self.wait()
        low_group = VGroup(blue_row, marginal_labels[0])
        left_group = VGroup(red_col, marginal_labels[1])
        diagonals = VGroup(*(VGroup(*(VGroup(dice_pairs[n], grid_probs[n]) for n in range(36) if n // 6 + n % 6 == s)) for s in range(11)))
        ...
        for n, rect in zip(it.count(), rects):
            ...
            self.play(FadeOut(last_rect), FadeIn(rect), MoveToTarget(diagonals), MoveToTarget(sum_bar_group))
            last_rect = rect
        self.wait()
        self.play(diagonals.animate.set_opacity(1), sum_bar_group.animate.set_opacity(1), FadeOut(last_rect), FadeIn(low_group), FadeIn(left_group))
        self.play(self.frame.animate.reorient(27, 66, 0).move_to([-0.16, 1.41, 0.77]).set_height(9.36), FadeIn(bars_3d), sum_bar_group.animate.to_edge(RIGHT), run_time=2)
        self.wait()
        ...
        for s in range(11):
            ...
            self.play(MoveToTarget(sum_bar_group), *map(MoveToTarget, bars_3d))
            self.wait()
        self.play(bars_3d.animate.set_opacity(0.8).set_stroke(width=0.5), sum_bar_group.animate.set_opacity(1))
        self.wait()
        bars_3d.save_state()
        ...
        self.play(self.frame.animate.reorient(36, 46, 0).move_to([-0.56, 0.55, 1.22]).set_height(7.71), MoveToTarget(bars_3d), sum_bar_group.animate.set_width(4.0).to_edge(RIGHT), run_time=2)
        self.wait()
        ...
        self.play(self.frame.animate.reorient(40, 61, 0).move_to([1.69, 0.33, -0.73]).set_height(12.96), ReplacementTransform(diagonal_bar_groups, new_diagonals, lag_ratio=0.001), run_time=5)
        self.add(full_table, new_diagonals)
        self.play(self.frame.animate.reorient(40, 85, 0).move_to([3.05, 1.93, 0.77]).set_height(14.93), sum_bar_group.animate.set_width(5.5, about_edge=RIGHT), FadeOut(full_table), run_time=3)
        self.wait()

    def show_repeated_samples(self, dist1, dist2, bar_group1, bar_group2, buckets, sum_labels, n_animated_runs=20, n_total_runs=150, marker_height=0.1, marker_color=YELLOW_D):
        marker_template = Rectangle(height=marker_height, width=buckets[0].get_width() * 0.8, fill_color=marker_color, fill_opacity=1, stroke_color=WHITE, stroke_width=1)
        ...
        for n in range(n_total_runs):
            ...
            self.add(markers)
            if animate:
                self.play(LaggedStart(FadeOut(dice[0]), bar_group1.animate.set_opacity(0.5), FadeOut(dice[1]), bar_group2.animate.set_opacity(0.5), sum_labels.animate.set_opacity(0.25), run_time=0.5))
            else:
                self.wait(0.1)
                self.remove(dice)
                ...
                sum_labels.set_opacity(0.25)
        self.wait()
        self.play(FadeOut(markers, lag_ratio=0.01), bar_group1.animate.set_opacity(1), bar_group2.animate.set_opacity(1), sum_labels.animate.set_opacity(1))
...
class ConvolveDiscreteDistributions(SumAlongDiagonal):
    ...
    def construct(self):
        ...
        blue_dice = get_die_faces(fill_color=BLUE_E, dot_color=WHITE)
        red_dice = get_die_faces(fill_color=RED_E, dot_color=WHITE)
        top_bars, low_bars = bar_groups = VGroup(get_bar_group(dist1, self.dist1_colors, blue_dice), get_bar_group(dist2, self.dist2_colors, red_dice))
        bar_groups.arrange(DOWN, buff=LARGE_BUFF)
        bar_groups.to_edge(LEFT)
        self.add(bar_groups)
        conv_dist = np.convolve(dist1, dist2)
        ...
        self.wait()
        self.play(FadeOut(label), FadeOut(low_arrow), ShowCreation(v_lines, lag_ratio=0.1))
        self.wait()
        ...
        self.play(MoveToTarget(rows, run_time=2), MoveToTarget(sum_bar_group))
        self.wait()
        last_rect = VMobject()
        ...
        for n in [7, 5, *range(2, 13)]:
            ...
            self.play(get_row_shift(top_bars, low_bars, n), MoveToTarget(sum_bar_group))
            ...
            pair_rects = VGroup(*(SurroundingRectangle(pair, buff=0.05).set_stroke(YELLOW, 2).round_corners() for pair in pairs))
            ...
            for die_pair in die_pairs:
                product = VGroup(p_mob(die_pair[0]), p_mob(die_pair[1]))
                product.arrange(RIGHT, buff=SMALL_BUFF)
                die_pair_targets.add(VGroup(product[0].arg, product[1].arg))
            ...
            self.play(ShowIncreasingSubsets(products), ShowIncreasingSubsets(plusses), ShowIncreasingSubsets(pair_rects), run_time=0.35 * len(products))
            self.wait(0.5)
            ...
            self.wait(0.5)
            self.play(FadeOut(prod_group), FadeOut(pair_rects))
        self.play(get_row_shift(top_bars, low_bars, 7), sum_bar_group.animate.set_opacity(1.0), run_time=0.5)
        ...
        x_args = VGroup(*(Tex(f'({x}) = {np.round(dist1[x - 1], 2)}').next_to(PX, RIGHT, SMALL_BUFF) for x in range(1, 7)))
        die_rects = VGroup()
        value_rects = VGroup()
        ...
        self.play(Write(PX), Write(func_label), ShowCreation(arrow))
        self.wait()
        ...
        self.play(TransformFromCopy(func_group, func_group_Y), Write(PY), FadeOut(die_rects[-1]), FadeOut(value_rects[-1]))
        self.play(TransformFromCopy(func_group, func_group_XY), Write(PXY))
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(func_group, func_group_Y, func_group_XY, x_args[-1])))
        sum_bar_group.generate_target()
        sum_bar_group.target.shift(DOWN)
        conv_def = Tex('\\big[P_X * P_Y\\big](s) = \\sum_{x = 1}^6 P_X(x) \\cdot P_Y(s - x)', font_size=36, isolate=['x = 1', '6'], **plabel_kw)
        ...
        self.play(LaggedStart(MoveToTarget(sum_bar_group), MoveToTarget(PXY), FadeIn(PXY_arg, UP), Write(eq), TransformFromCopy(PX, lhs[1:3]), TransformFromCopy(PY, lhs[4:6]), Write(VGroup(lhs[0], lhs[3], *lhs[6:]))))
        self.wait()
        self.play(Write(conv_word), ShowCreation(conv_rect))
        self.wait()
        ...
        self.wait()
        ex_rhs = Tex('(4) = P_{-}(1)P_{-}(3) + P_{-}(2)P_{-}(2) + P_{-}(3)P_{-}(1)')
        ex_rhs.scale(0.9)
        ...
        self.play(LaggedStart(MoveToTarget(PXY_copy), MoveToTarget(eq), MoveToTarget(PXY), Transform(PXY_arg, ex_rhs['(4)'], remover=True), conv_def.animate.next_to(eq.target, RIGHT), MaintainPositionRelativeTo(question_rhs, conv_def), FadeIn(ex_rhs, LEFT), PX.animate.next_to(bar_groups[0], LEFT), PY.animate.next_to(bar_groups[1], LEFT), self.frame.animate.set_height(9).move_to(0.5 * UP), sum_bar_group.animate.shift(0.5 * DOWN), ShowCreation(example_box), FadeIn(example_words, UP), run_time=2))
        self.wait()
        example = VGroup(PXY_copy, ex_rhs)
        self.add(example)
        example_box.save_state()
        for part in ex_rhs[re.compile('P[^+]*P[^+]*\\)')]:
            self.play(example_box.animate.replace(part, stretch=True).scale(1.1).set_stroke(width=2), run_time=0.5)
            self.wait()
        ...
        self.play(Write(conv_def[10:]), FadeOut(question_rhs, DOWN), FadeIn(general_words, DOWN), MoveToTarget(example_words), example.animate.scale(0.75).next_to(example_words.target, DOWN))
        self.wait()
        ...
        self.wait()
        self.play(LaggedStart(TransformFromCopy(s_arrow, x_arrow), TransformFromCopy(s_arrow, y_arrow)))
        self.wait()
        xeq1 = conv_def['x = 1']
        y_arrow.save_state()
        self.play(s_arrow.animate.scale(0.75).rotate(90 * DEGREES).next_to(xeq1, LEFT, SMALL_BUFF), y_arrow.animate.scale(0.75).rotate(-90 * DEGREES).next_to(xeq1, RIGHT, SMALL_BUFF))
        self.wait()
        self.play(s_arrow.animate.next_to(conv_def['6'], LEFT, SMALL_BUFF), y_arrow.animate.next_to(conv_def['6'], RIGHT, SMALL_BUFF))
        self.wait()
        self.play(Restore(y_arrow), FadeOut(s_arrow))
        self.wait()
        ...
        example = Tex('\n            [P_X * P_Y](4) =\n            &P_X(1) \\cdot P_Y(3) \\; + \\\\\n            &P_X(2) \\cdot P_Y(2) \\; + \\\\\n            &P_X(3) \\cdot P_Y(1) \\; + \\\\\n            &P_X(4) \\cdot P_Y(0) \\; + \\\\\n            &P_X(5) \\cdot P_Y(-1) \\; + \\\\\n            &P_X(6) \\cdot P_Y(-2)\n            ', t2c={'X': BLUE, 'Y': RED}, font_size=36)
        example.next_to(example_words, DOWN, buff=0.5)
        summands = VGroup(*(example[f'P_X({x}) \\cdot P_Y({4 - x})'] for x in range(1, 7)))
        plusses = example['+']
        plusses.shift(SMALL_BUFF * RIGHT)
        plusses.add(VectorizedPoint(summands[-1].get_center()))
        self.play(FadeIn(example['[P_X * P_Y](4) =']), FadeOut(v_lines), MoveToTarget(bar_groups), MaintainPositionRelativeTo(PX, bar_groups[0]), Write(example_words), PY.animate.next_to(bar_groups.target[1], RIGHT).match_x(PX), sum_bar_group.animate.scale(0.5).next_to(bar_groups.target, DOWN, buff=0.75, aligned_edge=LEFT))
        ...
        for x, summand, plus in zip(it.count(1), summands, plusses):
            ...
            rect.round_corners()
            rect.add(Tex(f'x = {x}').next_to(rect, DOWN, SMALL_BUFF))
            self.play(FadeTransform(conv_def['P_X(x) \\cdot P_Y(s - x)'].copy(), summand), FadeTransform(last_rect, rect), FadeIn(plus), run_time=0.5)
            self.wait()
            last_rect = rect
        self.play(FadeOut(last_rect))
        zeroed_terms = VGroup(*(example[f'P_Y({n})'] for n in range(0, -4, -1)))
        ...
        self.play(summands[3:].animate.set_opacity(0.35), plusses[3:].animate.set_opacity(0.35))
        self.wait()
    ...
    def show_bars_creation(self, bars, lag_ratio=0.05, run_time=3):
        ...
        for bar in bars:
            ...
            rect.set_opacity(0)
            anims.extend([FadeIn(face), rect.animate.restore(), CountInFrom(num, 0), UpdateFromAlphaFunc(num, lambda m, a: m.next_to(m.rect, UP, SMALL_BUFF).set_opacity(a))])
...
class ShowConvolutionOfLists(SumAlongDiagonal):
    ...
    def construct(self):
        ...
        conv_dist = np.convolve(dist1, dist2)
        kw = dict(height=1.5)
        blue_bars, red_bars, sum_bars = bar_groups = VGroup(get_bar_group(dist1, self.dist1_colors, **kw), get_bar_group(dist2, self.dist2_colors, **kw), get_bar_group(conv_dist, self.sum_colors, **kw))
        parens = Tex('()()')
        parens.stretch(2, 1)
        parens.match_height(bar_groups)
        asterisk = Tex('*', font_size=96)
        equation = VGroup(parens[0], blue_bars, parens[1], asterisk, parens[2], red_bars, parens[3], Tex('=', font_size=96), sum_bars)
        equation.arrange(RIGHT)
        equation.set_width(FRAME_WIDTH - 1)
        equation.to_edge(UP, buff=1.0)
        self.add(equation)
        self.remove(sum_bars)
        self.play(TransformFromCopy(blue_bars, sum_bars, lag_ratio=0.003), TransformFromCopy(red_bars, sum_bars, lag_ratio=0.003), run_time=1.5)
        self.wait()
        ...
        for list_word, bar_group in zip(list_words, bar_groups):
            self.play(FadeIn(list_word, DOWN), LaggedStart(*(FlashAround(bar[2], time_width=1.5, buff=0.05) for bar in bar_group), lag_ratio=0.03, run_time=2))
            self.wait()
        self.play(LaggedStartMap(ShowCreation, crosses, lag_ratio=0.1, run_time=1))
        self.play(LaggedStartMap(FadeIn, func_words, shift=0.5 * DOWN, scale=0.5, lag_ratio=0.5), list_words.animate.shift(0.5 * DOWN), crosses.animate.shift(0.5 * DOWN), n)
        self.wait()
...
class RepeatedDiscreteConvolutions(InteractiveScene):
    ...
    def construct(self):
        ...
        for n in range(5):
            ...
            self.wait()
            self.play(new_bars.animate.move_to(top_bars, DL).set_anim_args(path_arc=-120 * DEGREES), FadeOut(top_bars, UP), Restore(low_bars))
            top_bars = new_bars

    def get_bar_group(self, dist, colors=(BLUE, TEAL), y_unit=4, bar_width=0.35, num_decimal_places=2, min_value=1):
        bars = self.get_bars(dist, colors, y_unit, bar_width)
        result = VGroup(bars, self.get_bar_value_labels(bars, min_value), self.get_bar_prob_labels(bars, dist, num_decimal_places))
        result.dist = dist
        return result
    ...
    def get_dist_label(self, indices):
        ...
        if len(indices) > 3:
            index_strs = [index_strs[0], '\\cdots', index_strs[-1]]
    ...
    def flip_bar_group(self, bar_group):
        ...
        self.play(MoveToTarget(bars, lag_ratio=0.05, path_arc=0.5), *(MaintainPositionRelativeTo(VGroup(value, prob), bar) for bar, value, prob in zip(*bar_group)))
        self.add(bar_group)

    def show_convolution(self, top_bars, low_bars):
        new_dist = np.convolve(top_bars.dist, low_bars.dist)
        new_bars = self.get_bar_group(new_dist, y_unit=8, num_decimal_places=3, min_value=top_bars[1][0].get_value() + low_bars[1][0].get_value())
        ...
        for n in range(len(new_bars[0])):
            ...
            index_pairs = [(k, n - k) for k in range(n + 1) if 0 <= n - k < len(low_bars[0]) if 0 <= k < len(top_bars[0])]
            ...
            for partial_value in np.cumsum(products):
                rect = conv_rect.copy()
                rect.stretch(partial_value / new_bars.dist[n], dim=1, about_edge=DOWN)
            ...
            self.play(ShowSubmobjectsOneByOne(highlights, remover=True), ShowSubmobjectsOneByOne(partial_rects, remover=True), ShowSubmobjectsOneByOne(partial_labels, remover=True), run_time=0.15 * len(products))
