...
class IfYouReallyUnderstand(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        self.play(morty.says(TexText('If you \\emph{really} \\\\ understand'), run_time=1.5), self.change_students('pondering', 'confused', 'erm', look_at=self.screen))
        self.wait()
        old_bubble = morty.bubble
        old_words = morty.bubble.content
        new_bubble = morty.get_bubble(TexText("You'll understand \\\\ why normal distributions \\\\ are special"), bubble_type=SpeechBubble, width=5, height=4)
        new_bubble.content.scale(1.2)
        VGroup(new_bubble, new_bubble.content).shift(0.5 * RIGHT)
        self.play(Transform(old_bubble, new_bubble), Transform(old_words, new_bubble.content, run_time=1), morty.change('tease'))
        ...
        self.play(morty.debubble('raise_right_hand'), FadeIn(title, UP), self.change_students('pondering', 'pondering', 'hesitant', look_at=title))
...
class AlreadyCoveredConvolutions(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(stds[2].says("Wait, didn't you already\ncover convolutions?", mode='dance_1'), morty.change('guilty'))
        self.play(self.change_students('hesitant', 'awe', None, look_at=self.screen))
        self.wait(2)
        words = VGroup(Text('Only discrete examples', font_size=48), Text('\n                Probability\n                Moving averages\n                Image processing\n                Polynomial multiplication\n                Relationship with FFTs\n                ', alignment='LEFT', font_size=36))
        words.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        words[1].shift(0.25 * RIGHT)
        words.to_corner(UL)
        words.to_edge(UP)
        self.play(LaggedStart(morty.change('raise_right_hand', look_at=self.screen), stds[2].debubble(mode='sassy', look_at=self.screen), stds[1].change('pondering', look_at=self.screen)))
        self.wait(2)
        self.play(Write(words[0], run_time=1), self.change_students('pondering', 'tease', 'plain', look_at=words))
        self.wait(3)
        ...
        self.play(morty.change('guilty'), self.change_students('hesitant', 'pondering', 'hesitant', look_at=morty.eyes))
        self.wait()
        self.look_at(no_prereq, added_anims=[FadeOut(words, LEFT), FadeIn(no_prereq, LEFT)])
        self.play(GrowArrow(arrow), morty.change('raise_left_hand', self.screen))
        self.wait(2)
        self.play(morty.change('maybe'))
        self.look_at(no_prereq, added_anims=[FadeOut(no_prereq, UP), FadeIn(redundancy_words, UP), arrow.animate.stretch(0.8, 0, about_edge=RIGHT)])
        self.wait(3)
        self.play(morty.change('tease'), self.change_students('happy', 'thinking', 'happy', look_at=morty.eyes))
        self.wait(4)
...
class PauseAndPonder(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait(5)
        self.play(self.teacher.debubble(mode='raise_right_hand', look_at=3 * UP), self.change_students('tease', 'thinking', 'awe', look_at=3 * UP))
        self.wait(8)

class CountOutcomes(InteractiveScene):

    def construct(self):
        equation = Tex('6 \\times 6 = 36 \\text{ outcomes}')
...
class AssumingIndependence(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(morty.says('Assuming\nindependence!', mode='surprised', run_time=1), self.change_students('hesitant', 'guilty', 'plain'))
        self.wait(3)
        self.play(morty.debubble('tease'), stds[1].says(TexText("Isn't that a \\\\ little pedantic?"), mode='sassy'), stds[2].change('angry'))
        self.wait(3)
...
class IsntThatOvercomplicated(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        stds = self.students
        self.remove(self.background)
        self.play(stds[0].change('erm', self.screen), stds[2].change('confused', self.screen), stds[1].says("Isn't that...a little\nover-complicated?", mode='angry', bubble_config=dict(width=7, height=4)), morty.change('shruggie'))
        self.wait(2)
        self.play(morty.change('guilty'))
        self.wait()
        self.play(morty.says("It's fun!", mode='hooray', bubble_config=dict(width=2, height=1.5)), stds[1].debubble())
        self.play(self.change_students('hesitant', 'tease', 'pondering', look_at=self.screen))
        self.wait(3)
        self.play(morty.debubble(mode='raise_right_hand'), self.change_students('pondering', 'erm', 'pondering', look_at=self.screen))
        self.wait(5)
...
class QuestionTheFormula(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(stds[0].change('erm', 3 * UR), stds[1].says('Hang on...', mode='hesitant'), stds[2].change('confused', 3 * UR))
        self.wait(3)
...
class RuleOfThumb(InteractiveScene):
    ...
    def construct(self):
        ...
        title.to_edge(UP)
        sigma, arrow, integral = group = VGroup(Tex('\\sum \\dots').set_height(1.5), Vector(2 * RIGHT, stroke_width=10), Tex('\\int\\dots dx').set_height(2.5))
        ...
        self.wait()
        self.play(TransformFromCopy(sigma, integral), sigma.animate.restore(), GrowFromCenter(arrow))
        self.wait()
...
class CanWeSeeAnExample(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        stds = self.students
        self.remove(self.background)
        self.play(stds[1].says('Can we see\nan example?', mode='raise_left_hand'))
        self.play(stds[0].change('confused', self.screen), stds[2].change('maybe', self.screen), morty.change('happy'))
        self.wait(2)
        self.play(morty.change('raise_right_hand', 3 * UP), self.change_students('pondering', None, 'pondering', look_at=3 * UP), stds[1].debubble())
        self.look_at(3 * UP)
        self.wait(5)
...
class AskAboutAddingThreeUniforms(InteractiveScene):
    ...
    def construct(self):
        ...
        for brace, func, color in zip(all_braces, funcs, colors):
            ...
            axes = Axes(x_range, (0, 1.5, 0.5), width=2.0, height=1.0, axis_config=dict(tick_size=0.025))
        ...
        self.add(two_sum)
        self.play(LaggedStartMap(GrowFromCenter, term_braces[:2], lag_ratio=0), LaggedStartMap(FadeIn, term_plots[:2], shift=DOWN), FadeIn(symbols[:2], lag_ratio=0.2), run_time=1)
        self.play(TransformFromCopy(term_plots[0], expr_plots[0]), TransformFromCopy(term_plots[1], expr_plots[0]))
        self.wait()
        self.play(LaggedStart(ShowCreation(h_line), TransformFromCopy(two_sum, three_sum['X_1 + X_2'][0]), TransformFromCopy(term_braces[:2], term_braces[2:4]), TransformFromCopy(term_plots[:2], term_plots[2:4]), TransformFromCopy(two_sum['+ X_2'][0], three_sum['+ X_3'][0]), TransformFromCopy(term_braces[1], term_braces[4]), TransformFromCopy(term_plots[1], term_plots[4]), TransformFromCopy(symbols[0].replicate(2), symbols[2:4]), TransformFromCopy(symbols[1], symbols[4]), lag_ratio=0.02))
        self.wait()
        self.play(Write(q_marks))
        self.wait()
        self.play(TransformFromCopy(expr_plots[0], x12_plot), ReplacementTransform(term_braces[2], x12_brace), ReplacementTransform(term_braces[3], x12_brace), FadeOut(term_plots[2:4], DOWN), FadeOut(symbols[2], DOWN), symbols[3].animate.shift(0.5 * LEFT))
        self.wait()
        self.play(TransformFromCopy(x12_plot, expr_plots[1]), TransformFromCopy(term_plots[4], expr_plots[1]), FadeOut(q_marks, RIGHT))
        self.wait()
...
class ConfusedAtThree(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(morty.change('raise_right_hand'), self.change_students('maybe', 'confused', 'sassy', look_at=self.screen))
        self.wait(2)
...
class KeepGoing(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(stds[0].says('More\niterations!', mode='hooray', bubble_config=dict(width=4, height=2)), stds[1].change('tease', stds[0].eyes), stds[2].change('coin_flip_1', stds[0].eyes), morty.change('happy'), run_time=1)
        self.wait(5)
...
class WhyNormals(InteractiveScene):
    ...
    def construct(self):
        plane = NumberPlane((-4, 4), (0, 0.75, 0.125), width=7, height=5, axis_config=dict(stroke_width=1), background_line_style=dict(stroke_color=GREY_B, stroke_width=1, stroke_opacity=0.5))
        plane.to_edge(RIGHT)
        graph = plane.get_graph(lambda x: np.exp(-x ** 2 / 2) / math.sqrt(TAU))
        graph.set_stroke(BLUE, 3)
        formula = Tex('\\frac{1}{\\sqrt{2\\pi}} e^{-x^2 / 2}')
        ...
        self.add(randy)
        self.play(LaggedStart(randy.thinks(Text('Why this\nfunction?', font_size=32), mode='pondering', look_at=formula, run_time=1), FlashAround(formula, run_time=3, time_width=1.5)))
        self.play(Blink(randy))
        self.wait()
        self.play(randy.debubble(mode='tease', look_at=morty.eyes), VFadeIn(morty), morty.says(Text("There's a very\nfun answer", font_size=32), look_at=randy.eyes, run_time=1))
        self.wait(4)
...
class AskIfItsTheArea(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        stds = self.students
        self.remove(self.background)
        self.play(stds[0].says(TexText('Is $[f*g](s)$ the\narea of this slice?'), mode='raise_left_hand', look_at=self.screen), stds[1].change('pondering', self.screen), stds[2].change('pondering', self.screen), morty.change('tease', stds[0].eyes))
        self.wait(3)
...
class Sqrt2Correction(InteractiveScene):
    ...
    def construct(self):
        words = TexText('Technically, this shape is $\\sqrt{2}$ times wider than this shape')
        shape_words = words['this shape']
        words.to_edge(UP, buff=0.5)
        points = [[-3.75, 0.25, 0], [2.5, -2.5, 0]]
        colors = [YELLOW, TEAL]
        arrows = VGroup()
...
class SimpleQuestion(InteractiveScene):
    ...
    def construct(self):
        ...
        equations = VGroup(Tex('X + Y', font_size=60), Tex('\\int_{-\\infty}^\\infty f(x)g(s - x)dx'))
        for equation, rect in zip(equations, rects):
            equation.next_to(rect, UP)
        words = VGroup(Text('Such a \n simple question!'), Text('Such a seemingly \n simple question.', t2s={'seemingly': ITALIC}, t2c={'seemingly': YELLOW}))
        for word in words:
            word.match_x(rects[0])
            word.to_edge(UP, buff=MED_SMALL_BUFF)
        self.play(Write(words[0], run_time=1), FadeIn(equations[0]))
        self.wait()
        self.play(TransformMatchingStrings(words[0], words[1], key_map={'!': '.'}, run_time=1))
        self.wait()
...
class ComplainAboutCalculation(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        stds = self.students
        self.remove(self.background)
        self.play(stds[2].says('But do these help\nwith actual computations?', mode='sassy'), morty.change('guilty'))
        self.play(self.change_students('erm', 'frustrated', look_at=self.screen))
        self.wait(3)
        self.play(stds[1].says('Like, the opening quiz?', mode='raise_right_hand'), stds[2].debubble(mode='hesitant', look_at=self.screen), stds[0].change('pondering'))
        self.play(morty.change('well'))
        self.look_at(self.screen)
        self.wait(3)
        self.play(morty.change('raise_left_hand', 4 * UR), stds[0].animate.look_at(3 * UR), stds[1].debubble(mode='pondering', look_at=3 * UR), stds[2].change('pondering', 3 * UR), self.frame.animate.set_height(12, about_edge=DOWN))
        self.wait(4)
        self.play(morty.change('raise_right_hand', look_at=self.screen), self.change_students('hesitant', 'pondering', 'guilty', look_at=self.screen))
        self.wait(2)
        self.play(self.change_students('horrified', 'pleading', 'concentrating', look_at=2 * UR))
        self.play(morty.change('tease'))
        self.wait(4)
        self.play(morty.change('coin_flip_2'), self.change_students('guilty', 'guilty', 'sick', look_at=2 * UR))
        self.wait()
        self.play(morty.change('hooray'), self.change_students('well', 'tease', 'happy', look_at=morty.eyes))
        self.wait(4)
...
class OrdinaryApproach(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(underline), FadeIn(title, lag_ratio=0.1))
        self.wait()
        tex1 = 'f(x) = \\frac{1}{\\sigma_1 \\sqrt{2\\pi}} e^{-x^2 / 2\\sigma_1^2}'
        tex2 = 'g(y) = \\frac{1}{\\sigma_2 \\sqrt{2\\pi}} e^{-y^2 / 2\\sigma_2^2}'
        tex_kw = dict(font_size=30, t2c={'x': BLUE, 'y': YELLOW})
        equations = VGroup(Tex(tex1, **tex_kw), Tex(tex2, **tex_kw))
        equations.arrange(DOWN, buff=0.75, aligned_edge=LEFT)
        equations.next_to(title, DOWN, buff=0.75, aligned_edge=LEFT)
        brace = Brace(equations[0], RIGHT, tex_string='\\underbrace{\\qquad\\qquad\\qquad}')
        brace.stretch(1.5, 1)
        brace_text = brace.get_text('Formula\nfor a normal\ndistribution', font_size=30, alignment='LEFT', buff=MED_SMALL_BUFF)
        ...
        self.wait()
        conv_equation = Tex('\n                [f * g](s) = \\int_{-\\infty}^\\infty f(x)g(s - x) dx\n                = \\int_{-\\infty}^\\infty \\frac{1}{\\sigma_1 \\sigma_2 2 \\pi} e^{-x^2 / 2\\sigma_1^2} e^{-(s - x)^2 / 2\\sigma_2^2} \\; dx\n            ', t2c={'x': BLUE, 's': GREEN}, font_size=36)
        conv_equation.move_to(1.0 * DOWN)
        rhss = conv_equation[re.compile('=.*dx')]
        lhs = conv_equation['[f * g](s)']
        self.play(FadeIn(lhs), FadeIn(rhss[0]))
        self.wait()
        self.play(LaggedStart(FadeTransform(equations[0].copy(), rhss[1]), FadeTransform(equations[1].copy(), rhss[1]), FadeTransform(rhss[0].copy(), rhss[1]), lag_ratio=0.05), self.frame.animate.move_to(DOWN), run_time=2)
        self.wait()
        ...
        self.play(FadeOut(to_toss, 6 * RIGHT, path_arc=-30 * DEGREES, rate_func=running_start, run_time=1))
        self.wait()
...
class GaussianFunctionAnnotations(InteractiveScene):
    ...
    def construct(self):
        ...
        functions = VGroup(Tex('f(x) = \\frac{1}{\\sigma \\sqrt{2\\pi}} e^{-x^2 / 2\\sigma^2}', **tex_kw), Tex('g(y) = \\frac{1}{\\sigma \\sqrt{2\\pi}} e^{-y^2 / 2\\sigma^2}', **tex_kw))
...
class PiScene(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(morty.change('pondering', ORIGIN), randy.change('thinking', ORIGIN))
        self.wait()
