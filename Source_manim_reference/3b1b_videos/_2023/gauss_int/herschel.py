...
class TwoDGaussianAsADistribution(InteractiveScene):
    ...
    def construct(self):
        ...
        for sigma in [0.8, 1.0, 0.6]:
            ...
            for submob in mesh.family_members_with_points():
                submob.set_points([p + unit_size * func(*plane.p2c(p), sigma) * OUT for p in submob.get_points()])
                submob.set_stroke(WHITE, width=0.5 * submob.get_stroke_width(), opacity=0.5 * submob.get_stroke_opacity())
            mesh.shift(0.01 * OUT)
            graphs.append(Group(graph, mesh))
        graph1, graph2, graph3 = graphs
        self.add(graph1)
        graph1[0].set_opacity(0)
        self.play(frame.animate.reorient(20, 70), graph1[0].animate.set_opacity(0.5), TransformFromCopy(graph1[1].start, graph1[1]), run_time=3)
        self.play(frame.animate.reorient(-20), run_time=7)
        graph1.save_state()
        ...
        self.wait(2)
        self.play(FadeOut(graph1[0]), Restore(graph1[1]), frame.animate.reorient(0, 0), run_time=3)
        ...
        self.play(dartboard.animate.set_opacity(0.2), DrawBorderThenFill(blob), GrowArrow(arrow))
        self.wait()
        self.play(ShowCreation(radial_line))
        self.play(Rotate(blob, TAU, about_point=plane.get_origin(), run_time=6), Rotate(radial_line, TAU, about_point=plane.get_origin(), run_time=6), MaintainPositionRelativeTo(arrow, blob))
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(blob, arrow, radial_line)), dartboard.animate.set_opacity(0.75))
        self.play(self.frame.animate.set_gamma(-0.75 * PI), rate_func=there_and_back, run_time=6)
        self.wait()
        self.wait(0.1 * self.n_points)

    def get_plane(self):
        plane = NumberPlane((-2, 2), (-2, 2), background_line_style=dict(stroke_color=GREY, stroke_width=1, stroke_opacity=1))
        plane.set_height(7)
    ...
    def add_random_points_anim(self, plane):
        ...
        dots = Group(*(GlowDot(plane.c2p(x, y), glow_factor=4.0, radius=0.3) for x, y in coords))
        anim = LaggedStart(*(FadeIn(dot, rate_func=there_and_back) for dot in dots), lag_ratio=1 / self.n_dots_per_moment, run_time=self.n_points / self.n_dots_per_moment)
...
class ShowXYCoordinate(TwoDGaussianAsADistribution):
    ...
    def construct(self):
        ...
        self.play(FadeIn(dot), ShowCreation(x_line), FadeIn(x_label, 0.5 * LEFT))
        self.add(y_line, dot)
        self.play(ShowCreation(y_line), FadeIn(y_label, 0.5 * UP))
        self.wait()
        self.add(r_line, dot)
        self.play(ShowCreation(r_line), FadeIn(r_label, shift=0.5 * normalize(r_line.get_vector())))
        self.wait()
...
class IndependentCoordinates(TwoDGaussianAsADistribution):
    ...
    def construct(self):
        ...
        for n in range(self.n_iterations):
            ...
            lines = VGroup(DashedLine(plane.c2p(x, 0), plane.c2p(x, y), dash_length=0.05), DashedLine(plane.c2p(0, y), plane.c2p(x, y), dash_length=0.05))
            lines.set_stroke(WHITE, 2)
            dot = GlowDot().move_to(plane.c2p(x, y))
            self.play(LaggedStart(x_tip.animate.match_x(dot).set_opacity(1), y_tip.animate.match_y(dot).set_opacity(1), lag_ratio=0.5))
            self.play(*map(ShowCreation, lines), FadeIn(dot), run_time=0.5)
            self.wait(0.5)
            self.play(LaggedStartMap(FadeOut, Group(x_tip, y_tip, lines, dot)), run_time=1)
...
class ShowPointR0(TwoDGaussianAsADistribution):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Rotate(r_line, -angle, about_point=plane.get_origin()), Rotate(dot, -angle, about_point=plane.get_origin()), ReplacementTransform(coord_label, new_coord_label, path_arc=-angle, time_span=(1, 2)), run_time=3)
        self.wait()
...
class RescaleG(InteractiveScene):
    ...
    def construct(self):
        ...
        label1 = TexText('$g(0) \\ne 1$', font_size=36)
        label2 = TexText('$g(0) = 1$', font_size=36)
        label3 = TexText('Later we \\\\ re-scale anyway', font_size=36)
        labels = [label1, label2, label3]
        curves = [curve, curve.target, curve]
        for label, crv in zip(labels, curves):
            label.next_to(crv.get_top(), UR)
        self.play(ShowCreation(curve), FadeIn(label1, lag_ratio=0.1))
        self.wait()
        self.play(MoveToTarget(curve), FadeTransform(label1, label2))
        self.wait(2)
        self.play(Restore(curve), FadeTransform(label2, label3))
...
class ManyDifferentFs(InteractiveScene):
    ...
    def construct(self):
        ...
        axes.y_axis.add_numbers(num_decimal_places=2, excluding=[0], font_size=16)
        self.add(axes)
        curves = VGroup(axes.get_graph(lambda x: math.exp(-x ** 2)), axes.get_graph(lambda x: 1 / (1 + x ** 2)), axes.get_graph(lambda x: math.exp(-x)), axes.get_graph(lambda x: 0.5 * math.exp(-x ** 4 + x ** 2)), axes.get_graph(lambda x: math.cos(x) ** 2 / (x + 1)), axes.get_graph(lambda x: 1 / 2 * x ** 2 * np.exp(-x)))
        curves.set_stroke(RED, 3)
        func_names = VGroup(Tex('e^{-x^2}'), Tex('1 \\over (1 + x^2)'), Tex('e^{-x}'), Tex('\\frac{1}{2} e^{-x^4 + x^2}'), Tex('\\cos^2(x) \\over (x + 1)'), Tex('\\frac{1}{2} x^2 e^{-x}'))
        ...
        for new_curve, new_name in zip(curves[1:], func_names[1:]):
            self.play(Transform(curve, new_curve), TransformMatchingTex(name, new_name, run_time=1))
            name = new_name
            self.wait()
...
class VariableInputs(InteractiveScene):
    ...
    def construct(self):
        equation = Tex('f(\\sqrt{(1.00)^2 + (0.00)^2}) = f(1.00)f(0.00)')
...
class RationalNumbers(InteractiveScene):
    ...
    def construct(self):
        ...
        for i, j in [(0, 9), (9, 27), (27, len(labels))]:
            self.play(LaggedStartMap(FadeIn, lines[i:j], lag_ratio=0.75), LaggedStartMap(FadeIn, labels[i:j], lag_ratio=0.75), rate_func=linear, run_time=3)
        self.play(LaggedStartMap(FadeIn, lines[27:], lag_ratio=0.75), run_time=10, rate_func=rush_into)
        self.wait()
        ...
        self.play(LaggedStart(*(Rotate(line, -90 * DEGREES) for line in lines), lag_ratio=1 / len(lines), run_time=3), FadeOut(labels, lag_ratio=0.1, run_time=1), ShowCreation(real_line, time_span=(2, 3)))
        self.wait()

class TwoProperties(InteractiveScene):

    def construct(self):
        properties = VGroup(VGroup(Text('Property 1'), TexText('\n                    The probability (density) depends \\\\\n                    only on the distance from the origin\n                ', alignment='', font_size=36, color=GREY_A)), VGroup(Text('Property 2'), TexText('\n                    The $x$ and $y$ coordinates are \\\\\n                    independent from each other.\n                ', alignment='', font_size=36, color=GREY_A)))
        ...
        prop_boxes = VGroup(*(SurroundingRectangle(prop[1]).set_fill(GREY_E, 1).set_stroke(RED, 1, 0.5) for prop in properties))
        ...
        kw = dict(t2c={'x': BLUE, 'y': YELLOW, '\\sigma': RED, '{r}': RED})
        form1, form2, form3 = forms = VGroup(Tex('f_2(x, y) = e^{-(x^2 + y^2)}', **kw), Tex('f_2(x, y) = e^{-(x^2 + y^2) / 2 \\sigma^2}', **kw), Tex('f_2(x, y) = {1 \\over 4 \\sigma^2 \\pi} e^{-(x^2 + y^2) / 2 \\sigma^2}', **kw))
        ...
        self.play(prop_boxes[0].animate.stretch(0, 0, about_edge=RIGHT).set_opacity(0), FadeOut(implies), FadeOut(form1['= e^{-(x^2 + y^2)}']))
        self.remove(prop_boxes[0])
        self.add(lhs)
        self.wait()
        phrase = properties[0][1]['only on the distance']
        self.play(FlashUnder(phrase, color=TEAL, buff=0), phrase.animate.set_color(TEAL))
        self.wait(2)
        lhs.generate_target()
        lhs.target.to_edge(LEFT)
        radial_rhs = Tex('= f({r})', **kw)
        full_radial_rhs = Tex('= f(\\sqrt{x^2 + y^2})', **kw)
        ...
        self.wait()
        self.play(MoveToTarget(lhs), Write(radial_rhs))
        ...
        self.play(prop_boxes[1].animate.stretch(0, 0, about_edge=RIGHT).set_opacity(0))
        self.remove(prop_boxes[0])
        self.wait()
        ...
        factored_rhs = Tex('= g(x) h(y)', **kw)
        simpler_rhs = Tex('= g(x) g(y)', **kw)
        ...
        self.play(MoveToTarget(lhs), radial_rhss.animate.to_edge(LEFT).shift(0.5 * DOWN).set_opacity(0.35))
        self.play(TransformMatchingShapes(lhs.copy(), factored_rhs))
        self.wait()
        self.play(ShowCreation(g_box), FadeIn(g_words))
        self.wait()
        self.play(ReplacementTransform(g_box, h_box), ReplacementTransform(g_words, h_words))
        self.wait()
        self.play(FadeOut(h_box), FadeOut(h_words))
        self.wait()
        self.play(FadeOut(factored_rhs['h(y)'], 0.5 * UP), FadeIn(simpler_rhs['g(y)'], 0.5 * UP))
        self.wait()
        self.remove(factored_rhs)
        self.add(simpler_rhs)
        arrow = Arrow(simpler_rhs, radial_rhs)
        self.play(GrowArrow(arrow), radial_rhs.animate.set_opacity(1), FadeOut(full_radial_rhs))
        self.wait()
        radial_rhs.generate_target()
        (radial_rhs.target.next_to(lhs, RIGHT),)
        self.play(MoveToTarget(radial_rhs), simpler_rhs.animate.next_to(radial_rhs.target, RIGHT), Uncreate(arrow))
        self.wait()
        ...
        self.play(LaggedStartMap(FadeOut, VGroup(*xs, *ys), shift=0.5 * UP), LaggedStartMap(FadeIn, VGroup(*rs, *zeros), shift=0.5 * UP))
        self.wait()
        ...
        f_rhs = Tex('= f(x)f(y)', **kw)
        f_rhs.move_to(simpler_rhs, LEFT)
        gs = simpler_rhs['g']
        fs = f_rhs['f']
        self.play(FadeIn(assumption, 0.5 * DOWN), FadeOut(const_words, 0.5 * DOWN))
        self.wait()
        self.play(TransformFromCopy(VGroup(radial_rhs[1], *simpler_rhs[:2]), f_eq_g))
        self.wait()
        self.play(LaggedStartMap(FadeIn, VGroup(*xs, *ys), shift=0.5 * DOWN), LaggedStartMap(FadeOut, VGroup(*rs, *zeros), shift=0.5 * DOWN), FadeOut(const_rect), FadeOut(assumption))
        self.wait()
        self.play(TransformMatchingShapes(f_eq_g[0].copy(), fs), ReplacementTransform(simpler_rhs, f_rhs))
        self.wait()
        key_equation = VGroup(*radial_rhs[1:], *f_rhs)
        self.play(key_equation.animate.set_x(0.25 * FRAME_WIDTH).to_edge(UP), FadeOut(f_eq_g), FadeOut(lhs), FadeOut(radial_rhs[0]))
        ...
        self.play(GrowFromCenter(full_radial_rhs, lag_ratio=0.02), radial_rhs[1:].animate.next_to(full_radial_rhs, LEFT, aligned_edge=DOWN), key_equation[4:].animate.next_to(full_radial_rhs, RIGHT))
        self.wait()
        ...
        self.play(TransformFromCopy(key_equation[:4], example_f[:4]), GrowFromPoint(example_f[4:], key_equation.get_left()))
        self.wait()
        let = Text('Let')
        h_def = Tex('h({x}) = f(\\sqrt{{x}})', **kw)
        h_def.next_to(key_equation, DOWN, LARGE_BUFF)
        let.next_to(h_def, LEFT, MED_LARGE_BUFF)
        h_def2 = Tex('h({x}^2) = f({x})', **kw)
        h_def2.next_to(h_def['h'], DOWN, MED_LARGE_BUFF, aligned_edge=LEFT)
        h_eq = Tex('h(x^2 + y^2) = h(x^2) h(y^2)', **kw)
        h_eq.to_corner(UR)
        h_eq.to_edge(RIGHT, buff=1.25)
        example_h = Tex('h({r}) = e^{-{r}}', **kw)
        example_h.scale(0.75)
        example_h.next_to(example_f, DOWN, MED_LARGE_BUFF)
        self.play(FadeIn(h_def, DOWN), Write(let))
        self.wait()
        self.play(TransformMatchingTex(h_def.copy(), h_def2, key_map={'\\sqrt': '^2'}, run_time=1))
        self.wait()
        self.play(TransformFromCopy(h_def, example_h))
        self.wait(2)
        self.play(VGroup(key_equation, full_radial_rhs).animate.scale(0.8).to_edge(LEFT), VGroup(let, h_def, h_def2).animate.scale(0.8).to_edge(LEFT), FadeOut(func_eq_name, LEFT), Uncreate(arrow))
        self.play(TransformMatchingShapes(VGroup(*full_radial_rhs[1:], *f_rhs).copy(), h_eq))
        self.wait()
        ...
        self.play(ShowCreation(sum_box), FadeIn(sum_words, lag_ratio=0.1))
        self.wait()
        self.play(ReplacementTransform(sum_box, prod_box), FadeTransform(sum_words, prod_words))
        self.wait()
        self.play(FadeOut(prod_box), FadeOut(prod_words))
        self.wait()
        implies = Tex('\\Downarrow', font_size=72)
        implies.next_to(h_eq, DOWN)
        full_h_eq = Tex('h(x_1 + x_2 + \\cdots + x_n) = h(x_1)h(x_2) \\cdots h(x_n)')
        ...
        self.play(Write(implies), FadeIn(full_h_eq, DOWN))
        self.wait()
        implies2 = implies.copy()
        implies2.next_to(full_h_eq, DOWN, buff=MED_LARGE_BUFF)
        five_eq = Tex('h(5) &= h(1 + 1 + 1 + 1 + 1) \\\\ &= h(1)h(1)h(1)h(1)h(1) = h(1)^5')
        five_eq.next_to(implies2, DOWN)
        five_eq.to_edge(RIGHT)
        n_eq = Tex('h(n) = h(1 + \\cdots + 1) = h(1) \\cdots h(1) = h(1)^n')
        n_eq.scale(0.75)
        n_eq.next_to(implies2, DOWN, MED_LARGE_BUFF)
        sum_brace = Brace(n_eq['1 + \\cdots + 1'], UP, SMALL_BUFF)
        sum_tex = sum_brace.get_tex('n \\text{ times}', buff=SMALL_BUFF).scale(0.5, about_edge=DOWN)
        prod_brace = Brace(n_eq['h(1) \\cdots h(1)'], UP, SMALL_BUFF)
        prod_tex = prod_brace.get_tex('n \\text{ times}', buff=SMALL_BUFF).scale(0.5, about_edge=DOWN)
        for tex in (n_eq, sum_tex, prod_tex):
            tex['n'].set_color(BLUE)
        self.play(Write(five_eq['h(5)']))
        self.wait()
        self.play(TransformFromCopy(five_eq['h('][0], five_eq['h('][1]), TransformFromCopy(five_eq[')'][0], five_eq[')'][1]), Write(five_eq['='][0]))
        self.play(ShowIncreasingSubsets(five_eq['1 + 1 + 1 + 1 + 1'][0]))
        self.wait()
        self.play(FadeTransform(five_eq['= h(1 + 1 + 1 + 1 + 1)'].copy(), five_eq['= h(1)h(1)h(1)h(1)h(1)']))
        self.wait()
        self.play(Write(five_eq['= h(1)^5']))
        self.wait()
        self.play(FadeOut(five_eq), FadeIn(n_eq), FadeIn(implies2))
        self.play(LaggedStart(GrowFromCenter(sum_brace), FadeIn(sum_tex, 0.25 * DOWN), GrowFromCenter(prod_brace), FadeIn(prod_tex, 0.25 * DOWN)))
        self.wait()
        exp_eq1 = Tex('h(n) = h(1)^n')
        exp_eq2 = Tex('h(n) = b^n')
        ...
        self.play(TransformFromCopy(n_eq['h(n)'], exp_eq1['h(n)']), TransformFromCopy(n_eq['= h(1)^n'], exp_eq1['= h(1)^n']))
        self.wait()
        self.play(ShowCreation(h1_rect), FadeIn(h1_words))
        self.wait()
        self.play(TransformMatchingTex(exp_eq1, exp_eq2), FadeOut(h1_rect, scale=0.5), FadeOut(h1_words, scale=0.5))
        self.wait()
        self.play(exp_eq2.animate.next_to(implies2, DOWN), FadeOut(VGroup(n_eq, sum_brace, sum_tex, prod_brace, prod_tex), UP))
        rational_eq = Tex('h(p / q) = b^{\\,p / q}')
        rational_eq['p / q'].set_color(RED)
        rational_eq.move_to(exp_eq2)
        exercise = TexText('Exercise: Show this is also true for rational inputs, $p / q$')
        exercise['p / q'].set_color(RED)
        hint = TexText('Hint, think about $h\\left(\\frac{p}{q} + \\cdots + \\frac{p}{q} \\right)$')
        exercise.next_to(rational_eq, DOWN, LARGE_BUFF)
        exercise.to_edge(RIGHT)
        hint.scale(0.7)
        hint.set_fill(GREY_A)
        hint.next_to(exercise, DOWN)
        self.play(Write(exercise), FadeOut(VGroup(example_box, example_word, example_f, example_h), shift=DL))
        self.wait()
        pq_target = rational_eq['p / q'].copy()
        self.play(TransformMatchingTex(exp_eq2, rational_eq), TransformMatchingShapes(exercise['p / q'].copy(), pq_target))
        self.remove(pq_target)
        self.wait()
        self.play(FadeIn(hint, DOWN))
        self.wait(2)
        self.play(LaggedStart(FadeOut(exercise, 0.5 * DOWN), FadeOut(hint, 0.5 * DOWN), lag_ratio=0.25))
        assumption = TexText('Assuming $f$ (and hence also $h$) \\\\ is continuous...', font_size=36)
        ...
        range1 = TexText('For all $x \\in \\mathds{R}$', **kw)
        range2 = TexText('For all $x \\in \\mathds{R}^+$', **kw)
        ...
        self.play(FadeOut(assumption, LEFT), Uncreate(arrow), FadeOut(range2, LEFT), ShowCreation(b_rect), Write(b_words, run_time=1))
        self.wait()
        c = hx_eq2['{c}'][0][0]
        c_copy = c.copy()
        c.set_opacity(0)
        self.play(ReplacementTransform(b_rect, c_copy), TransformMatchingTex(hx_eq, hx_eq2, key_map={'b': 'e'}), FadeOut(b_words, 0.2 * DOWN))
        self.remove(c_copy)
        c.set_opacity(1)
        self.add(hx_eq2)
        self.wait()
        ...
        f_form = Tex('f(x) = e^{cx^2}', **kw)
        f_form['c'].set_color(RED)
        f_form.next_to(implies3, LEFT)
        f_form.align_to(hx_eq2, DOWN)
        self.play(Write(implies3), TransformMatchingTex(hx_eq2.copy(), f_form, run_time=1))
        self.wait()
        f_form.generate_target()
        f_form.target.scale(1.5, about_edge=RIGHT)
        rect = SurroundingRectangle(f_form.target)
        rect.set_stroke(YELLOW, 2)
        self.play(MoveToTarget(f_form), FlashAround(f_form.target, time_width=1, run_time=2, stroke_width=5), ShowCreation(rect, run_time=2))
        self.wait()
...
class VariableC(InteractiveScene):
    ...
    def get_c_group(self):
        ...
        get_c = c_tracker.get_value
        c_interval = NumberLine((-1, 1, 0.25), width=3, tick_size=0.05, big_tick_numbers=[-1, 0, 1])
    ...
    def get_axes(self):
        axes = Axes((-1, 5), (0, 4), width=6, height=4)
        return axes
...
class VariableCWithF(VariableC):
    ...
    def get_axes(self):
        axes = Axes((-4, 4), (0, 2), width=8, height=3)
...
class TalkAboutSignOfConstant3D(VariableCWithF):
    ...
    def construct(self):
        ...
        def get_graph(c):
            ...
            x_slice = ParametricCurve(lambda t: axes.c2p(t, 0, np.exp(c * t ** 2)), t_range=(-4, 4, 0.1))
        ...
        for value in [-0.5, -0.8, -0.3, -1.0, -0.3, 0.05, 0.1, -0.3, -1.0, -0.7, -1.0]:
            ...
            self.play(c_tracker.animate.set_value(value), Transform(graph, new_graph), run_time=5)
            self.wait()
...
class OldTalkAboutSignOfConstantScraps(InteractiveScene):
    ...
    def construct(self):
        ...
        def update_area(area):
            area.set_points_as_corners([plane.c2p(-4, 0), *graph.get_anchors(), plane.c2p(4, 0)])
...
class OldTwoKeyProperties(InteractiveScene):
    ...
    def construct(self):
        ...
        radial_exp = Tex('= e^{-{r}^2}', **kw)
        radial = Tex('= f_1({r})', **kw)
        radial_full = Tex('= f_1(\\sqrt{x^2 + y^2})', **kw)
        expressions = VGroup(one_var, two_var, factored_exp, factored, radial_exp, radial, radial_full)
        ...
        arrow = Arrow(one_var.get_right(), two_var.get_corner(UR) + 0.5 * LEFT, path_arc=-PI)
        words = Text('Two interpretations')
        words.next_to(arrow, RIGHT)
        self.play(FadeIn(rects[0]), FadeIn(rect_words[0]))
        self.wait()
        self.play(ReplacementTransform(*rects), FadeTransform(*rect_words))
        self.wait()
        two_var_copy = two_var.copy()
        self.play(FadeOut(rects[1]), FadeOut(rect_words[1]), ShowCreation(arrow), TransformMatchingTex(one_var.copy(), two_var_copy, run_time=1), Write(words, run_time=1))
        self.remove(two_var_copy)
        self.wait()
        self.play(LaggedStart(Write(factored_exp['=']), TransformFromCopy(one_var['e^{-x^2}'], factored_exp['e^{-x^2}']), TransformFromCopy(one_var['e^{-x^2}'], factored_exp['e^{-y^2}']), run_time=2, lag_ratio=0.4))
        self.wait()
        self.play(LaggedStart(Write(factored['=']), TransformFromCopy(one_var['f_1(x)'], factored['f_1(x)'], path_arc=PI / 4), TransformFromCopy(one_var['f_1(x)'], factored['f_1(y)'], path_arc=PI / 4), run_time=3, lag_ratio=0.5))
        self.wait()
        ...
        self.play(Write(radial_exp['=']), TransformFromCopy(one_var['e^{-x^2}'], radial_exp['e^{-{r}^2}']))
        self.wait()
        self.play(Write(radial['=']), TransformFromCopy(one_var['f_1(x)'], radial['f_1({r})']))
        self.wait()
        ...
        self.play(LaggedStart(FadeOut(arrow), FadeOut(words), MoveToTarget(prop), MoveToTarget(f2_eq), FadeOut(radial_full), lag_ratio=0.15, run_time=3))
        self.wait()
        self.play(FlashAround(one_var.target, time_width=1.0, run_time=2), MoveToTarget(one_var))
        self.wait()
        ...
        self.play(FadeIn(radial_full), radial.animate.next_to(radial_full, RIGHT), factored.animate.shift(left_shift))
        self.wait()
        ...
        self.play(one_var.animate.scale(1 / 1.5).to_corner(DL).set_opacity(0.5), FadeOut(f2_eq, UP), MoveToTarget(prop))
        self.play(FadeIn(question, lag_ratio=0.1), GrowArrow(arrow))
        self.wait()
        h_eq = Tex('h(x^2) h(y^2) = h(x^2 + y^2)', **kw)
        h_eq.next_to(prop, DOWN, buff=MED_LARGE_BUFF)
        h_eq.shift((prop[1]['='].get_x() - h_eq['='].get_x()) * RIGHT)
        h_def = Tex('h(x) = f_1(\\sqrt{x})', **kw)
        h_def.to_edge(LEFT).match_y(h_eq)
        h_def2 = Tex('h(x^2) = f_1(x)', **kw)
        ...
        self.play(FadeIn(example_box), FadeIn(example_word), MoveToTarget(one_var))
        self.wait()
        self.play(FadeIn(h_example, DOWN))
        self.wait()
        self.play(FlashAround(prop[:2], time_width=1, run_time=2))
        self.play(TransformFromCopy(factored, h_eq['h(x^2) h(y^2)'][0]), TransformFromCopy(radial_full, h_eq['= h(x^2 + y^2)'][0]))
        self.wait()
        ...
        self.play(ReplacementTransform(sum_box, prod_box), FadeTransform(sum_words, prod_words))
        self.wait()
        self.play(ReplacementTransform(prod_box, h_box), FadeTransform(prod_words, exp_words))
        self.wait()
        implies = Tex('\\Downarrow', font_size=72)
        implies.next_to(h_eq, DOWN)
        implies.rotate(PI)
        h_exp = Tex('h(x) = a \\cdot b^x', **kw)
        h_exp.next_to(implies, DOWN)
        h_exp2 = Tex('h(x) = a \\cdot e^{{c}x}', **kw)
        h_exp2.move_to(h_exp)
        h_exp0 = Tex('h(x) = b^x', **kw)
        ...
        b_words.next_to(b_rect, DOWN, SMALL_BUFF, LEFT)
        b_words.match_color(b_rect)
        self.play(FadeTransform(h_box, implies), FadeTransform(exp_words, h_exp0))
        self.wait()
        ...
        c_copy = c.copy()
        c.set_opacity(0)
        self.play(ReplacementTransform(b_rect, c_copy), TransformMatchingTex(h_exp, h_exp2), FadeOut(b_words, 0.2 * DOWN))
        self.remove(c_copy)
        c.set_opacity(1)
        self.add(h_exp2)
        self.wait()
        ...
        f_eq = Tex('f_1(x) = a \\cdot e^{{c}x^2}', **kw)
        f_eq.next_to(implies2, DOWN)
        rect = SurroundingRectangle(f_eq)
        rect.set_stroke(YELLOW, 1)
        self.play(TransformMatchingTex(h_exp2.copy(), f_eq), Write(implies2, run_time=1))
        self.wait()
        self.play(ShowCreation(rect, run_time=2), FlashAround(f_eq, time_width=1, run_time=2, stroke_width=5))
        self.wait()