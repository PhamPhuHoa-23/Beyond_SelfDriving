...
class BellCurveArea(InteractiveScene):
    ...
    def construct(self):
        axes = NumberPlane((-4, 4), (0, 1.5, 0.5), width=14, height=5, background_line_style=dict(stroke_color=GREY_C, stroke_width=2, stroke_opacity=0.5))
        ...
        self.wait()
        integral = Tex('\\int_{-\\infty}^\\infty e^{-x^2} dx', t2c=t2c)
        integral.to_edge(UP)
        self.play(graph.animate.set_fill(BLUE, 0.5))
        self.wait()
        self.play(Write(integral['\\int_{-\\infty}^\\infty']), FadeTransform(graph_label.copy(), integral['e^{-x^2}']))
        ...
        self.play(FadeIn(rects, lag_ratio=0.1, run_time=3), graph.animate.set_fill(opacity=0).set_anim_args(time_span=(1, 2)), graph_label.animate.shift(SMALL_BUFF * UR).set_anim_args(time_span=(1, 2)))
        self.wait()
        self.play(rects.animate.set_opacity(0.1), MoveToTarget(axes), FadeIn(rect))
        self.wait()
        ...
        self.wait()
        self.play(GrowFromCenter(brace), FadeIn(dx_label, 0.5 * UP))
        self.play(FlashAround(integral['dx'], time_width=1, run_time=1.5))
        self.wait()
        rects.set_fill(opacity=0.8)
        rects.set_stroke(WHITE, 1)
        self.play(graph_label.animate.set_height(0.7).next_to(graph.pfp(0.4), UL), rects.animate.set_opacity(0.75), FadeOut(rect))
        self.wait()
        self.play(LaggedStart(*(r.animate.shift(0.25 * UP).set_color(YELLOW).set_anim_args(rate_func=there_and_back) for r in rects), run_time=5, lag_ratio=0.1), LaggedStart(FlashAround(integral[2:4], time_width=1), FlashAround(integral[1], time_width=1), lag_ratio=0.25, run_time=5))
        self.wait()
        for dx in [0.1, 0.075, 0.05, 0.03, 0.02, 0.01, 0.005]:
            ...
            self.play(Transform(rects, new_rects), brace.animate.set_width(dx * axes.x_axis.get_unit_size(), about_edge=LEFT), MaintainPositionRelativeTo(dx_label, brace))
        self.add(graph)
        self.play(FadeOut(brace), FadeOut(dx_label), ShowCreation(graph))
        frame = self.frame
        equals = Tex('=')
        ...
        self.play(frame.animate.set_height(9, about_edge=DOWN), integral.animate.next_to(equals, LEFT), FadeIn(equals), Write(answer_box))
        integral.save_state()
        ...
        self.play(ShowCreation(low_arrow), FadeIn(low_words, 0.5 * LEFT), FadeTransform(answer_box.copy(), int_box, path_arc=arc))
        self.wait()
        self.play(ShowCreation(top_arrow), FadeIn(top_words, 0.5 * RIGHT))
        self.wait()
        impossible = Text('Impossible!', font_size=72, color=RED)
        impossible.next_to(answer_box, RIGHT)
        functions = VGroup(Tex('a_n x^n + \\cdots a_1 x + a_0', t2c=t2c), Tex('\\sin(x), \\cos(x), \\tan(x)', t2c=t2c), Tex('b^x', t2c=t2c), Tex('\\vdots'))
        ...
        self.wait()
        self.play(LaggedStartMap(FadeIn, functions, shift=DOWN, lag_ratio=0.5), frame.animate.shift(4 * RIGHT), run_time=3)
        self.wait()
...
class AntiDerivative(InteractiveScene):
    ...
    def construct(self):
        x_min, x_max = (-3, 3)
        planes = VGroup(*(NumberPlane((x_min, x_max), (0, 2), width=5.5, height=2.75, background_line_style=dict(stroke_color=GREY, stroke_width=1, stroke_opacity=1.0), faded_line_ratio=3) for x in range(2)))
        planes.arrange(DOWN, buff=LARGE_BUFF)
        planes.to_corner(UL)
        self.add(planes)
        titles = VGroup(Tex('f(x) = e^{-x^2}', font_size=66), Tex('F(x) = \\int_0^x e^{-t^2} dt'))
        ...
        low_line = always_redraw(lambda: DashedLine(planes[1].c2p(get_x(), 0), planes[1].i2gp(get_x(), low_graph)).set_stroke(WHITE, 2))
...
class UsualFunctionTypes(InteractiveScene):
    ...
    def construct(self):
        t2c = {'x': YELLOW}
        functions = VGroup(Tex('a_n x^n + \\cdots a_1 x + a_0', t2c=t2c), Tex('\\sin(x), \\cos(x), \\arctan(x)', t2c=t2c), Tex('b^x, \\log(x), \\cosh(x)', t2c=t2c), Tex('\\vdots'))
        functions.arrange(DOWN, MED_LARGE_BUFF, aligned_edge=LEFT)
        functions.set_height(2.5)
        ...
        words.set_fill(RED)
        self.play(FadeIn(words, lag_ratio=0.1), FadeIn(box), LaggedStartMap(FadeIn, functions, shift=DOWN, lag_ratio=0.5, run_time=3))
        self.wait()
...
class GaussianIntegral(ThreeDScene, InteractiveScene):
    ...
    def get_axes(self, x_range=(-3, 3), y_range=(-3, 3), z_range=(0, 1.5, 0.5), width=8, height=8, depth=3, center=0.5 * IN, include_plane=False):
        axes = ThreeDAxes(x_range, y_range, z_range, width=width, height=height, depth=depth)
        axes.set_stroke(GREY_C)
        if include_plane:
            plane = NumberPlane(x_range, y_range, width=width, height=height, background_line_style=dict(stroke_color=GREY_C, stroke_width=1))
    ...
    def get_gaussian_graph(self, axes, color=interpolate_color(BLUE_E, BLACK, 0.6), opacity=1.0, shading=(0.2, 0.2, 0.4)):
    ...
    def get_thick_cylinder(self, cylinder, delta_r):
        ...
        annulus = ParametricSurface(lambda u, v: (radius + u * delta_r) * np.array([math.cos(v), math.sin(v), 0]), u_range=(0, 1), v_range=(0, TAU))
    ...
    def get_dynamic_slice(self, axes, stroke_color=BLUE, stroke_width=2, fill_color=BLUE_E, fill_opacity=0.5):
...
class CylinderSlices(GaussianIntegral):
    ...
    def construct(self):
        ...
        self.add(axes)
        bell_halves = Group(*(axes.get_parametric_surface(lambda r, theta: np.array([r * np.cos(theta), r * np.sin(theta), np.exp(-r ** 2)]), u_range=(0, 3), v_range=v_range) for v_range in [(0, PI), (PI, TAU)]))
        ...
        label2d, label3d = func_labels = VGroup(Tex('f_1(x) = e^{-x^2}', **kw), Tex('f_2(x, y) = e^{-(x^2 + y^2)}', **kw))
        ...
        self.wait()
        self.play(ShowCreation(bell_halves[0]), ShowCreation(bell_halves[1]), Rotate(bell2d, PI, axis=OUT, about_point=axes.c2p(0, 0, 0)), frame.animate.move_to(ORIGIN).reorient(-20, 70), Restore(axes), TransformMatchingTex(label2d.copy(), label3d, time_span=(0, 2)), label2d.animate.next_to(label3d, UP, MED_LARGE_BUFF, LEFT), run_time=6)
        self.wait()
        self.play(FadeOut(bell_halves, 0.01 * IN), FadeOut(bell2d, 0.1 * IN), FadeIn(graph, 0.01 * IN))
        self.play(Write(graph_mesh, stroke_width=1, lag_ratio=0.01))
        self.wait()
        self.play(frame.animate.set_theta(20 * DEGREES), rate_func=there_and_back, run_time=30)
        frame.save_state()
        graph_mesh.save_state()
        func_labels.use_winding_fill(False)
        self.play(frame.animate.reorient(0, 0).set_height(10).move_to(1.5 * LEFT).set_field_of_view(1 * DEGREES), graph.animate.set_opacity(0.25), func_labels.animate.scale(0.75).to_corner(UL), graph_mesh.animate.set_stroke(width=1), run_time=3)
        x, y = (1.5, 0.75)
        ...
        pythag.fix_in_frame()
        self.play(FadeTransform(label2d['= e^{-x^2}'].copy(), r_func), FadeOut(arrow, scale=0.8, shift=DR + RIGHT), FadeOut(r_label_rect))
        self.wait()
        line_copies = lines.copy()
        self.add(*line_copies, set_depth_test=False)
        self.play(*(VShowPassingFlash(line.insert_n_curves(20).set_stroke(width=8), time_width=1.5) for line in line_copies), *map(ShowCreation, lines))
        self.play(FadeTransform(r_func['r^2'][0].copy(), pythag['r^2']), FadeTransform(label3d['x^2 + y^2'][0].copy(), pythag['x^2 + y^2']), Write(pythag['=']))
        self.wait()
        self.wait()
        ...
        self.play(Rotate(r_line, TAU, about_point=axes.get_origin()), ShowCreation(circle), frame.animate.reorient(30, 60).move_to(ORIGIN).set_height(8).set_field_of_view(45 * DEGREES), Restore(graph_mesh), run_time=7)
        self.wait()
        self.play(r_line.animate.scale(0.1, about_point=axes.get_origin()), UpdateFromFunc(circle, lambda c: c.replace(get_circle(r_line.get_start()))), rate_func=there_and_back, run_time=8)
        self.wait()
        ...
        self.play(graph.animate.set_opacity(0.1).set_anim_args(time_span=(0, 2)), FadeIn(cylinders, lag_ratio=0.9), r_tracker.animate.set_value(3).set_anim_args(rate_func=linear, time_span=(0.5, 10)), frame.animate.reorient(-15, 75).set_height(5.5), run_time=10)
        self.wait()
        self.play(r_tracker.animate.set_value(0.7), cylinders.animate.set_opacity(0.1), frame.animate.reorient(-27, 71), run_time=3)
        ...
        eq_kw = dict(font_size=35, t2c={'{r}': RED})
        area_eq1 = TexText('Area = (Circumference)(Height)', **eq_kw)
        area_eq2 = TexText('Area = $2 \\pi {r} \\cdot e^{-{r}^2}$', **eq_kw)
        ...
        self.play(FadeIn(area_eq1, DOWN), functions.animate.shift(1.5 * DOWN).scale(0.7, about_edge=DL).set_fill(opacity=0.75))
        self.wait()
        pre_rect = cylinder.copy()
        pre_rect.clear_updaters()
        self.add(pre_rect, graph)
        self.play(pre_rect.animate.scale(0.95).next_to(cylinder, OUT, buff=1.0), frame.animate.set_height(7).move_to([1.0, 0.15, 1.0]), run_time=2)
        ...
        circ_brace = Brace(area_eq2['2 \\pi {r}'], DOWN, SMALL_BUFF)
        height_brace = Brace(area_eq2['e^{-{r}^2}'], DOWN, SMALL_BUFF)
        ...
        self.play(ShowCreation(circle), ShowCreation(rect_top))
        self.play(FadeIn(circ_brace), circ_word.animate.scale(0.75).next_to(circ_brace, DOWN, SMALL_BUFF), Write(area_eq2['2 \\pi {r}']), height_word.animate.next_to(area_eq2['2 \\pi {r}'], RIGHT))
        self.wait()
        self.add(height_line, set_depth_test=False)
        self.play(FadeOut(circle), FadeOut(rect_top), ShowCreation(rect_side), ShowCreation(height_line))
        self.play(FadeIn(height_brace), height_word.animate.scale(0.75).next_to(height_brace, DOWN, SMALL_BUFF, aligned_edge=LEFT), circ_word.animate.align_to(circ_brace, RIGHT), FadeInFromPoint(area_eq2['\\cdot e^{-{r}^2}'], r_func[1:].get_center()))
        self.remove(area_eq1)
        self.add(area_eq2, circ_word, height_word)
        self.wait()
        self.play(frame.animate.center().reorient(-15, 66).set_height(4).set_anim_args(run_time=15), *map(FadeOut, [rect, rect_side, height_line]))
        volume_word = Text('Volume', **eq_kw)
        volume_word.fix_in_frame()
        volume_word.move_to(area_eq2, DL)
        area_part = area_eq2['= $2 \\pi {r} \\cdot e^{-{r}^2}$']
        ...
        self.wait()
        self.play(LaggedStartMap(FadeOut, annotations, shift=DOWN, run_time=1), FadeOut(area_eq2['Area'], DOWN), FadeIn(volume_word, DOWN), area_part.animate.next_to(volume_word, RIGHT, SMALL_BUFF, DOWN))
        ...
        integral = Tex('\\int_0^\\infty', **eq_kw)
        integral.fix_in_frame()
        integral.move_to(volume_word, LEFT)
        thick_cylinders = Group(*(self.get_thick_cylinder(cyl, delta_r * axes.x_axis.get_unit_size()) for cyl in cylinders))
        thick_cylinders.set_opacity(0.8)
        thick_cylinders.set_shading(0.25, 0.25, 0.25)
        small_dr = 0.02
        thin_cylinders = Group(*(self.get_thick_cylinder(self.get_cylinder(axes, r), small_dr) for r in np.arange(0, 5, small_dr)))
        thin_cylinders.set_opacity(0.5)
        self.play(FadeOut(volume_word, LEFT), FadeOut(area_part[0][0], LEFT), FadeIn(integral, LEFT), integrand.animate.next_to(integral, RIGHT, buff=0))
        ...
        self.play(Transform(thin_cylinder, thin_cylinders[int(np.round(r_tracker.get_value() / small_dr))]), dr_brace[:-1].animate.stretch(small_dr / delta_r, 0, about_edge=RIGHT), UpdateFromFunc(dr_brace[-1], lambda m: m.next_to(dr_brace[:-1], OUT, SMALL_BUFF)), run_time=3)
        self.add(thin_cylinders, cylinders, graph, graph_mesh)
        self.add(dr_brace)
        dr_brace.deactivate_depth_test()
        self.play(ShowIncreasingSubsets(thin_cylinders), frame.animate.reorient(20, 70).set_height(8).move_to(OUT), FadeOut(dr_brace, time_span=(0, 2)), FadeOut(thin_cylinder, 2 * IN, time_span=(0, 2)), FadeOut(functions, time_span=(0, 2)), FadeOut(integral, time_span=(0, 2)), FadeOut(integrand, time_span=(0, 2)), run_time=20)
        self.wait()
        t0 = self.time
...
class CylinderIntegral(InteractiveScene):
    ...
    def construct(self):
        kw = dict(font_size=48, t2c={'{r}': RED, '{0}': RED, '{\\infty}': RED})
        exprs = VGroup(Tex('\\int_0^\\infty 2\\pi {r} \\cdot e^{-{r}^2} \\, d{r}', **kw), Tex('\\pi \\int_0^\\infty 2 {r} \\cdot e^{-{r}^2}\\,  d{r}', **kw), Tex('= \\pi \\left[ -e^{-{\\infty}^2} - \\left(-e^{-{0}^2} \\right) \\right]', **kw), Tex('= \\pi', **kw))
        ...
        self.wait()
        integrand = exprs[1]['2 {r} \\cdot e^{-{r}^2}']
        integrand_rect = SurroundingRectangle(integrand, buff=SMALL_BUFF)
        integrand_rect.set_stroke(YELLOW, 2)
        anti_derivative = Tex('-e^{-{r}^2}', **kw)
        anti_derivative.next_to(integrand_rect, DOWN, buff=1.5)
        arrow = Arrow(anti_derivative, integrand_rect)
        arrow.set_color(YELLOW)
        arrow_label = Tex('d \\over d{r}', **kw)
        arrow_label.scale(0.75)
        arrow_label.next_to(arrow, RIGHT, MED_SMALL_BUFF)
        self.play(ShowCreation(integrand_rect), GrowArrow(arrow), FadeIn(arrow_label, UP))
        self.wait()
        self.play(TransformMatchingShapes(integrand.copy(), anti_derivative))
        self.wait()
        self.play(Write(exprs[2]['=']), TransformFromCopy(exprs[1]['\\pi'], exprs[2]['\\pi']), TransformFromCopy(exprs[1]['\\int'], exprs[2]['[']), TransformFromCopy(exprs[1]['\\int'], exprs[2][']']))
        self.wait()
        self.play(LaggedStart(FadeTransform(anti_derivative.copy(), exprs[2]['-e^{-{\\infty}^2}']), FadeIn(VGroup(*exprs[2]['- \\left('], exprs[2]['\\right)'])), FadeTransform(anti_derivative.copy(), exprs[2]['-e^{-{0}^2}']), lag_ratio=0.7))
        self.play(TransformMatchingShapes(VGroup(*exprs[1]['\\pi \\int_0^\\infty'], *anti_derivative).copy(), exprs[2]))
        self.wait()
        self.play(TransformMatchingTex(exprs[2].copy(), exprs[3]))
        self.wait()
        rects = VGroup(SurroundingRectangle(exprs[2]['-e^{-{\\infty}^2}']), SurroundingRectangle(exprs[2]['-e^{-{0}^2}']))
        ...
        zero, one = values
        self.play(TransformFromCopy(integrand_rect, rects[0]), integrand_rect.animate.set_stroke(YELLOW, 1, 0.5))
        ...
        self.wait()
        answer = exprs[3]['\\pi']
        self.play(LaggedStartMap(FadeOut, VGroup(integrand_rect, arrow, arrow_label, anti_derivative, *rects, *values)), answer.animate.scale(2, about_edge=LEFT))
...
class CartesianSlices(GaussianIntegral):
    ...
    def construct(self):
        ...
        self.add(x_slice)
        self.play(y_tracker.animate.set_value(-x_max), run_time=5, rate_func=linear)
        self.wait()
        ...
        self.play(FadeOut(graph, time_span=(0, 1)), FadeOut(x_slice, time_span=(0, 1)), FadeIn(x_slices, 0.1 * OUT, lag_ratio=0.1), axes.labels[2].animate.set_opacity(0), frame.animate.reorient(-80), run_time=4)
        self.play(frame.animate.reorient(-100), run_time=3)
        self.wait()
        y_tracker.set_value(-x_max)
        self.add(x_slice, x_slices, graph, graph_mesh)
        self.play(FadeOut(x_slices, 0.1 * IN, time_span=(0, 2.5)), FadeIn(graph, time_span=(0, 2.5)), VFadeIn(x_slice), frame.animate.reorient(-15).set_height(6), y_tracker.animate.set_value(0), run_time=5)
        ...
        swap.begin()
        swap.update(1)
        func_label = Tex('e^{-(x^2 + y^2)}', **tex_kw)
        func_label.rotate(90 * DEGREES, RIGHT)
        func_label.next_to(x_slice_label, OUT, MED_LARGE_BUFF)
        fx0 = Tex('e^{-(x^2 + 0^2)} = e^{-x^2}', **tex_kw)
        fx0.rotate(90 * DEGREES, RIGHT)
        fx0.next_to(func_label, IN, MED_LARGE_BUFF, aligned_edge=LEFT)
        fx0['0'].set_color(YELLOW)
        self.always_depth_test = False
        self.play(*(VShowPassingFlash(mob, time_width=1.5, run_time=3) for mob in [x_slice.copy().set_stroke(YELLOW, 8).set_fill(opacity=0).shift(0.02 * OUT), Line(*axes.x_axis.get_start_and_end()).set_stroke(YELLOW, 8).insert_n_curves(40)]), Write(y0_label))
        self.wait()
        ...
        self.play(Restore(x_slice_label), FadeOut(func_label, OUT))
        self.wait()

        def get_area_label():
            area_label = TexText('Area = $0.00 \\cdot C$', font_size=30)
        ...
        self.play(FadeIn(x_slices, 0.1 * OUT, lag_ratio=0.1, run_time=4), FadeOut(graph), FadeOut(x_slice, time_span=(3, 4)), FadeOut(x_slice_label, time_span=(3, 4)), frame.animate.reorient(-83, 72, 0).set_height(8).center().set_anim_args(run_time=5))
        t0 = self.time
...
class CartesianSliceOverlay(InteractiveScene):
    ...
    def construct(self):
        kw = dict(t2c={'{x}': BLUE, '{y}': YELLOW, 'C': RED}, font_size=48)
        integral1 = Tex('\\int_{-\\infty}^\\infty C \\cdot e^{-{y}^2} d{y}', **kw)
        integral2 = Tex('= C \\int_{-\\infty}^\\infty e^{-{y}^2}d{y}', **kw)
        rhs = Tex('= C^2', **kw)
        ...
        self.wait()
        area_part = integral1['C \\cdot e^{-{y}^2}']
        volume_part = integral1['C \\cdot e^{-{y}^2} d{y}']
        ...
        self.wait()
        self.play(Transform(*rects), Transform(*arrows), TransformMatchingStrings(area_word, volume_word, run_time=1))
        self.wait()
        self.play(*map(FadeOut, [area_rect, arrows[0], volume_word]))
        self.wait()
        sub_int = Tex('C = \\int_{-\\infty}^\\infty e^{-{x}^2} d{x}', **kw)
        ...
        for mob in (box, sub_int):
            ...
            mob.set_opacity(0)
        self.play(ShowCreation(arrow), Restore(box), Restore(sub_int))
        self.wait()
        ...
        self.play(FlashAround(C2, time_width=1.5, run_time=3), everything.animate.set_opacity(0.7))
        self.wait()