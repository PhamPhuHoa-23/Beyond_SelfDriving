...
class WignerZoom(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(im1)
        self.play(self.frame.animate.scale(0.4419, about_edge=UL), run_time=4)
        self.wait()
...
class WignerReverseZoom(InteractiveScene):
    ...
    def construct(self):
        ...
        words = TexText("Now you're pushing \\\\ the joke too far...", font_size=20)
        ...
        self.play(Restore(frame), words.animate.set_stroke(width=3), run_time=7)
        self.wait()
...
class PaperTitle(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(part.animate.set_color(BLUE), FlashUnder(part, time_width=1.5, run_time=2, color=BLUE))
        self.wait()
...
class StoryWords(InteractiveScene):
    ...
    def construct(self):
        quote = TexText(self.quote, font_size=75, alignment='')
        quote.to_corner(DL)
...
class OtherVideos(InteractiveScene):
    ...
    def construct(self):
        ...
        images = Group(*(ImageMobject(os.path.join(folder, name)) for name in ['VCubingxThumbnail', 'BrimathThumbnail', 'DrAlters', 'KeithConrad']))
        images.set_height(0.35 * FRAME_HEIGHT)
        images.arrange_in_grid(h_buff=2.0, v_buff=1.0)
        images.to_edge(DOWN)
        names = VGroup(Text('vcubingx'), Text('BriTheMathGuy'), Text('idan-alter.github.io'), Text('kconrad.math.uconn.edu'))
        rects = VGroup()
        groups = Group()
...
class LastVideoFrame(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(screen)
        titles = VGroup(Text('Central Limit Theorem'), Text('Normal Distribution'), Text('Normal Distribution (aka a Gaussian)'))
        for title in titles:
            title.scale(56 / 48)
        ...
        self.wait(2)
        self.play(FadeOut(titles[0], UP), FadeIn(titles[1], UP))
        self.wait()
        self.play(ReplacementTransform(titles[1], titles[2]['Normal Distribution'][0]), Write(titles[2]['(aka a Gaussian)']))
        self.wait(3)
...
class SatisfyThisDisbelief(InteractiveScene):
    ...
    def construct(self):
        ...
        pis = VGroup(Mortimer(), Randolph(color=BLUE_D).flip(), Randolph(color=BLUE_E).flip(), Randolph(color=BLUE_C).flip())
        pis.arrange_in_grid()
        pis.set_height(4)
        pis.to_corner(DR)
        self.add(pis)
        pis[0].change_mode('happy')
        self.play(LaggedStart(pis[0].change('raise_right_hand'), pis[1].change('pondering', look_at=arrow.get_end()), pis[2].change('pondering', look_at=arrow.get_end()), pis[3].change('pondering', look_at=arrow.get_end()), lag_ratio=0.5))
        for x in range(2):
            self.play(Blink(random.choice(pis)))
            self.wait()
        self.play(FadeIn(words, lag_ratio=0.1), GrowArrow(arrow), pis[0].change('raise_left_hand', look_at=words), *(pi.animate.look_at(words) for pi in pis[1:]))
        for x in range(3):
            self.play(Blink(random.choice(pis)))
            self.wait()
        self.wait()
...
class ThreeStepPlan(InteractiveScene):
    ...
    def construct(self):
        ...
        kw = dict(t2c={'e^{-{x}^2}': BLUE, '\\sqrt{\\pi}': TEAL}, color=GREY_A, font_size=36, alignment='')
        step_contents = VGroup(TexText('Explain why the area under \\\\$e^{-{x}^2}$ is $\\sqrt{\\pi}$', **kw), TexText('Explain where $e^{-{x}^2}$ comes from \\\\ in the first place', **kw), TexText('Connect it all to the \\\\ Central Limit Theorem', **kw))
        steps = VGroup()
        ...
        steps.to_corner(UL)
        step_rects = VGroup(*(SurroundingRectangle(content) for content in step_contents))
...
class ManyBulgingFunctions(InteractiveScene):
    ...
    def construct(self):
        ...
        functions = [lambda x: 1 / (1 + x ** 2), lambda x: 1 / (1 + x ** 4), lambda x: np.exp(-x ** 2), lambda x: np.exp(-x ** 4), lambda x: eem1 * (np.abs(x) + ie) ** (-np.abs(x) - ie), lambda x: np.sinc(x) ** 2]
        func_labels = VGroup(Tex('\\frac{1}{1 + x^2}'), Tex('\\frac{1}{1 + x^4}'), Tex('e^{-x^2}', font_size=60), Tex('e^{-x^4}', font_size=60), Tex('e^{-1/e} \\left(\\left|x\\right|+\\frac{1}{e}\\right)^{-\\left(\\left|x\\right|+\\frac{1}{e}\\right)}', font_size=36), Tex('\\left(\\frac{\\sin(\\pi x)}{\\pi x} \\right)^2'))
        ...
        self.play(FadeIn(key_plot[0]), ShowCreation(key_plot[1]), Write(key_plot[2]))
        self.wait()
...
class WarningCalculus(InteractiveScene):
    ...
    def construct(self):
        ...
        signs.arrange(RIGHT)
        warning = VGroup(Text('Warning', font_size=120), Text('Calculus ahead', font_size=90))
...
class IntegralTitleCard(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(Write(VGroup(parens, two, equals), run_time=1), TransformFromCopy(curve.copy().set_fill(opacity=0), circle), FadeIn(VGroup(radius, r_label)), FadeIn(circle.copy().set_fill(opacity=0.5)), lag_ratio=0.5, run_time=3))
        self.add(rhs)
        self.wait(2)
        return
        func_label = Tex('e^{-x^2}', font_size=60)
        func_label.next_to(curve.pfp(0.6), UR)
        area_label = Tex('\\sqrt{\\pi}', font_size=120)
...
class SorryWhat(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        morty = self.teacher
        self.remove(self.background)
        self.play(LaggedStart(stds[1].says('Sorry, what?!', mode='angry'), stds[0].change('sassy', look_at=morty), stds[2].change('hesitant', look_at=morty), morty.change('tease'), lag_ratio=0.2))
        self.wait(3)
...
class WhoOrderedMoreDimensions(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        morty = self.teacher
        self.remove(self.background)
        self.play(LaggedStart(stds[0].says('Why', mode='raise_left_hand'), stds[2].change('hesitant', look_at=morty), morty.change('tease'), stds[1].says('Who ordered \n another dimension?', mode='confused', bubble_direction=LEFT), lag_ratio=0.2))
        self.wait(3)
        self.play(LaggedStart(morty.says('Well, watch\nwhat happens', mode='shruggie'), stds[0].debubble(mode='hesitant'), stds[1].debubble(mode='maybe'), stds[2].change('sassy')))
        self.wait(3)
        self.play(FadeOut(VGroup(morty.bubble, morty.bubble.content)), morty.says('It never hurts to\ntry similar problems'), self.change_students('pondering', 'erm', 'pondering', look_at=self.screen))
        self.wait(3)
...
class VolumeEqualsPi(InteractiveScene):
    ...
    def construct(self):
        text = TexText('Volume = $\\pi$')
        text['\\pi'].scale(2, about_edge=LEFT)
        self.add(text)
...
class OurKind(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.says('Where there is\ncircular symmetry,\nour kind thrives', mode='hooray'))
...
class CompareThreeIntegrals(InteractiveScene):
    ...
    def construct(self):
        ...
        titles = VGroup(TexText('Area = $C$', **kw), TexText('Volume = $C^2$', **kw), TexText('Volume = $\\pi$', **kw))
        titles[2]['\\pi'].scale(1.5, about_edge=DL)
        integrals = VGroup(Tex('\\int_{-\\infty}^\\infty e^{-{x}^2}\\,d{x} = C', **kw), Tex('\\int_{-\\infty}^\\infty \\int_{-\\infty}^\\infty e^{-{y}^2} e^{-{x}^2}\\,d{x}\\,d{y} = C^2', **kw), Tex('\\int_0^\\infty 2\\pi r \\cdot e^{-{r}^2} \\,d{r} = \\pi', **kw))
        integrals.scale(0.5)
        ...
        self.wait(2)
        self.play(FadeIn(rects[2]), FadeIn(titles[2], 0.5 * UP), FadeIn(integrals[2], 0.5 * DOWN))
        self.wait()
        eq_pi = titles[2]['= $\\pi$'][0].copy()
        eq_pi.generate_target()
        eq_sqrt_pi = Tex('= \\sqrt{\\pi}', **kw)
        ...
        self.play(LaggedStart(MoveToTarget(eq_pi), MoveToTarget(titles[1]), lag_ratio=0.35))
        self.play(LaggedStart(TransformMatchingShapes(eq_pi.copy(), eq_sqrt_pi, run_time=1), MoveToTarget(titles[0]), lag_ratio=0.35))
        self.wait()
...
class FeelsLikeATrick(InteractiveScene):
    ...
    def construct(self):
        words = VGroup(Text('This feels \n like a trick'), Text('How could you have \n discovered this?'))
        words.to_corner(UL)
        screen = ScreenRectangle(height=5)
        screen.to_corner(DR)
        arrows = VGroup(*(Arrow(word.get_right(), screen.get_top(), path_arc=-0.3 * PI) for word in words))
        self.play(Write(words[0], run_time=1), ShowCreation(arrows[0]))
        self.wait(2)
        self.play(FadeOut(words[0], UP), FadeIn(words[1], UP), Transform(*arrows))
        self.wait(2)
...
class HerschelMaxwellTitle(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FadeOut(title, lag_ratio=0.1), ReplacementTransform(pre_name, name['Herschel']), FadeInFromPoint(name['John'], pre_name.get_left()))
...
class HerschelDescription(InteractiveScene):
    ...
    def construct(self):
        positions = VGroup(Text('Mathematician'), Text('Scientist'), Text('Inventor'))
        positions.scale(1.5)
        positions.to_edge(RIGHT, buff=1.5)
        last = VGroup()
        for position in positions:
            self.play(FadeIn(position, 0.5 * UP), FadeOut(last, 0.5 * UP))
            self.wait(0.5)
            last = position
        self.wait()
        self.play(FadeOut(positions[-1]))
        contributions = VGroup(Text('Chemistry'), Text('Astronomy'), Text('Photography'), Text('Botany'))
        ...
        self.wait()
        desc = TexText("He named many of \\\\ Saturn's moons")
        desc.move_to(3.5 * RIGHT).to_edge(UP)
        moon_names = VGroup(Text('Mimas'), Text('Enceladus'), Text('Tethys'), Text('Dione'), Text('Rhea'), Text('Titan'), Text('Iapetus'))
        ...
        self.play(FadeIn(desc), LaggedStartMap(FadeIn, moons, shift=0.5 * DOWN, lag_ratio=0.5, run_time=3))
        self.wait()
...
class GaussianQuestion(InteractiveScene):
    ...
    def construct(self):
        ...
        curve.set_stroke(BLUE, 3)
        func = Tex('{1 \\over \\sqrt{2 \\pi} } e^{-x^2 / 2}', font_size=40)
...
class WhatWouldBeNice(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.says('You know what\nwould be nice?'), self.change_students('pondering', 'happy', 'tease', look_at=self.screen))
        self.wait(4)
...
class HowDoYouApproachThat(TeacherStudentsScene):
    ...
    def construct(self):
        stds = self.students
        self.remove(self.background)
        self.play(stds[0].says('How do you \n approach that?', mode='raise_left_hand'), stds[1].change('maybe', look_at=3 * UP), stds[2].change('erm', look_at=3 * UP), self.teacher.change('tease'))
        self.wait()
        self.play(self.teacher.change('raise_right_hand'), self.change_students('confused', 'confused', 'pondering', look_at=3 * UP))
        self.wait(3)
...
class MaxwellsEquations(InteractiveScene):
    ...
    def construct(self):
        kw = dict(t2c={'\\mathbf{E}': RED, '\\mathbf{B}': TEAL})
        equations = VGroup(Tex('\\nabla \\cdot \\mathbf{E}=4 \\pi \\rho', **kw), Tex('\\nabla \\cdot \\mathbf{B}=0', **kw), Tex('\\nabla \\times \\mathbf{E}=-\\frac{1}{c} \\frac{\\partial \\mathbf{B}}{\\partial t}', **kw), Tex('\\nabla \\times \\mathbf{B}=\\frac{1}{c}\\left(4 \\pi \\mathbf{J}+\\frac{\\partial \\mathbf{E}}{\\partial t}\\right)', **kw))
...
class ThreeDExpression(InteractiveScene):
    ...
    def construct(self):
        expr = Tex('{1 \\over \\sigma^3 (2\\pi)^{3/2}} e^{-(x^2 + y^2 + z^2) / 2 \\sigma^2}', t2c={'\\sigma': RED}, font_size=48)
        expr['1 \\over \\sigma^3 (2\\pi)^{3/2}}'].scale(0.7, about_edge=RIGHT)
        expr.to_corner(UL)
        self.add(expr)
...
class DefiningProperty(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(title)
        self.wait()
        words = Text('Suppose we take this\nto define a Gaussian...', t2s={'define': ITALIC}, t2c={'define': TEAL}, alignment='LEFT')
        words.match_width(derivation_box)
        words.next_to(derivation_box, UP)
        self.play(LaggedStart(FadeIn(words, 0.5 * UP), FadeOut(title, UP), lag_ratio=0.2))
        self.wait()
...
class ReactToDefiningProperty(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        stds = self.students
        self.remove(self.background)
        self.play(morty.change('raise_right_hand'), self.change_students('sassy', 'hesitant', 'erm', look_at=self.screen))
        self.play(self.change_students('erm', 'sassy', 'hesitant', look_at=self.screen))
        self.wait(2)
        self.play(LaggedStart(stds[1].says("But that assumes \n we're in 2D", mode='angry'), stds[0].change('sassy'), stds[2].animate.look_at(morty), morty.change('guilty'), lag_ratio=0.2))
        self.wait(3)
        self.play(LaggedStart(stds[1].debubble(mode='sassy'), stds[2].says('What about\nthe Central Limit Theorem?'), stds[0].change('hesitant'), morty.change('hesitant')))
        self.wait(3)
        self.play(morty.change('tease', self.screen))
        self.wait()
        self.play(stds[0].change('pondering', self.screen), stds[1].change('pondering', self.screen))
        self.wait(3)
...
class NextVideo(InteractiveScene):
    ...
    def construct(self):
        ...
        images = Group(ImageMobject('/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2022/convolutions/discrete/images/EquationThumbnail.png'), ImageMobject('/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/clt/main/images/Thumbnail.png'), ImageMobject('/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/clt/integral/images/ProofSummary.jpg'))
        images.set_height(3)
        titles = VGroup(Text('Convolutions'), Text('Central Limit Theorem'), TexText('Why $\\sqrt{\\pi}$?'))
        thumbnails = Group()
        ...
        self.play(VFadeIn(morty), morty.change('speaking'), FadeIn(bubble), Write(bubble.content, run_time=1))
        ...
        self.play(MoveToTarget(morty), Transform(bubble, bubble2), Transform(words, words2), ghost1.animate.set_opacity(0.25))
        words.become(words2)
        ...
        to_fade.add(ghost2)
        self.play(MoveToTarget(morty), Transform(bubble, bubble3), Transform(words, words3), ghost2.animate.set_opacity(0.25))
        words.become(words3)
        ...
        arrows = VGroup(*(Arrow(thumbnail.get_right(), screen.get_left() + vect, buff=0.3) for thumbnail, vect in zip(thumbnails, [UP, ORIGIN, DOWN])))
        self.play(LaggedStartMap(GrowArrow, arrows), FadeIn(screen), LaggedStartMap(FadeOut, VGroup(*to_fade, morty, bubble, words)))
        self.wait()
...
class NDimensionalBallGrid(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(grid)
        kw = dict(t2c={'r': BLUE})
        numbers = VGroup(*(Integer(n) for n in range(1, 6)), Tex('\\cdots'), Text('N'))
        interiors = VGroup(Tex('2r', **kw), Tex('\\pi r^2', **kw), Tex('\\frac{4}{3} \\pi r^3', **kw), Tex('\\frac{1}{2} \\pi^2 r^4', **kw), Tex('\\frac{8}{15} \\pi^2 r^5', **kw), Tex('\\cdots'), Tex('\\frac{\\pi^{n / 2}}{(n / 2)!} r^n', **kw))
        exteriors = VGroup(Tex('2', **kw), Tex('2 \\pi r', **kw), Tex('4 \\pi r^2', **kw), Tex('2 \\pi^2 r^3', **kw), Tex('\\frac{8}{3} \\pi^2 r^4', **kw), Tex('\\cdots'), Tex('n \\frac{\\pi^{n / 2}}{(n / 2)!} r^{n - 1}', **kw))
        for group, row in zip([numbers, interiors, exteriors], grid):
            for elem, square in zip(group, row[1:]):
                ...
                elem.move_to(square)
        row_titles = VGroup(Text('Dimension'), TexText('Volume of \\\\ $n$-ball', font_size=30), TexText('Volume of \\\\ $n$-ball boundary', font_size=30))
        ...
        shapes = Group(one_ball, circle, ball, Randolph('shruggie'), Tex('\\dots'), *VectorizedPoint().replicate(2))
