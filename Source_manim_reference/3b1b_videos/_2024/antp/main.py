    ...
    def construct(self):
        ...
        for i in range(len(primes) - 1):
            if primes[i] + 2 == primes[i + 1]:
                ...
                arc = Line(prime_mobs[i].get_top(), prime_mobs[i + 1].get_top(), path_arc=-PI, stroke_color=twin_prime_color, stroke_width=2)
                arc.scale(0.9, about_edge=UP)
                plus_2 = Tex('+2', font_size=24)
                plus_2.next_to(arc, UP, SMALL_BUFF)
                plus_2.set_color(twin_prime_color)
                tp_groups.add(VGroup(*prime_mobs[i:i + 2].copy(), arc, plus_2))
        non_twin_primes = VGroup(*(pm for pm, prime in zip(prime_mobs, primes) if prime not in twin_primes))
        highlight_point = 5
        ...
        def update_tp_groups(tp_groups):
            for tp_group in tp_groups:
                ...
                if x_coord < -10:
                    ...
                    continue
                pre_alpha = inverse_interpolate(highlight_point + 0.25, highlight_point - 0.25, x_coord)
...
class Timeline(InteractiveScene):
    ...
    def construct(self):
        timeline = NumberLine((-500, 2050, 100), width=FRAME_WIDTH - 1)
        timeline.move_to(2.5 * DOWN)
        timeline.add_numbers(list(range(0, 2050, 500)), direction=DOWN, group_with_commas=False, font_size=36)
        label = Tex('\\sim 300 BC', font_size=36)
        v_line = Line(DOWN, UP)
        ...
        self.add(v_line)
        self.add(label)
        question = TexText("``Are there infinitely many twin primes?''\\\\ \\quad -Euclid", font_size=48, alignment='')
        question[-7:].shift(RIGHT + 0.1 * DOWN)
        question.next_to(v_line, UP)
        question.shift_onto_screen(buff=0.2)
        arrows = VGroup(Vector(3 * LEFT), Text('No one can\nanswer', font_size=60).set_color(YELLOW), Vector(3 * RIGHT))
...
class InfinitePrimes(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(prime_mobs)
        self.play(prime_mobs.animate.shift(70 * OUT), FadeIn(rect, time_span=(10, 12)), run_time=12, rate_func=rush_into)
    ...
    def old_attempt(self):
        ...
        for n, mob in enumerate(prime_mobs):
            ...
            mob.move_to(point + sum((shift * shift_scale_factor ** k for k in range(n))))
...
class EuclidProof(InteractiveScene):
    ...
    def construct(self):
        prime_sequence = Tex('2, 3, 5, \\dots , p_n', font_size=72)
        ...
        last_arrow = Arrow(finite_words['prime'].get_corner(DL), last, path_arc=-PI / 3, buff=0.1)
        VGroup(finite_words, sequence_rect).set_color(YELLOW)
        self.add(prime_sequence)
        self.add(finite_words)
        self.add(sequence_rect)
        product = Tex('N = 2 \\cdot 3 \\cdot 5 \\cdots p_n', font_size=72)
        ...
        self.play(TransformMatchingTex(psc, product, matched_pairs=[(psc[','], product['\\cdot']), (psc['\\dots'], product['\\cdots'])], run_time=1))
        self.play(Write(plus_one))
        self.wait()
        N_rect = SurroundingRectangle(VGroup(product[2:], plus_one))
        ...
        factor_eq = Tex('N = q_1 \\cdots q_k', font_size=72)
        factor_eq['q_1 \\cdots q_k'].set_color(RED)
        factor_eq.next_to(product, DOWN, buff=1.5, aligned_edge=LEFT)
        self.play(FadeTransformPieces(product.copy(), factor_eq), FadeIn(N_rect), GrowArrow(factor_arrow), FadeIn(factor_word, 0.5 * DOWN))
        self.wait(3)
        q_rect = SurroundingRectangle(factor_eq['q_1'], buff=0.1)
        q_rect.set_stroke(WHITE, 3)
        q_words = TexText('Cannot be in $\\{2, 3, 5, \\dots, p_n\\}$', font_size=60)
        q_words.next_to(q_rect, UP, aligned_edge=LEFT)
        q_words.match_color(factor_eq[2])
        rect = SurroundingRectangle(Group(*(mob for mob in self.mobjects if isinstance(mob, StringMobject))), buff=0.5)
        ...
        self.play(Transform(N_rect, q_rect), FadeTransformPieces(factor_word, q_words), FadeOut(factor_arrow))
        self.wait(4)
        self.play(FadeIn(rect), FadeIn(cont_word, 0.5 * UP))
        self.wait()

    def infinite(self):
        inf_sequence = Tex('2, 3, 5, 7, 11, \\dots', font_size=72)
...
class PrimeDensityHistogram(InteractiveScene):
    ...
    def construct(self):
        ...
        axes = Axes((0, max_x, step), (0, 0.5, 0.1), width=FRAME_WIDTH - 2, height=6, x_axis_config=dict(tick_size=0.03, big_tick_numbers=labeled_xs, longer_tick_multiple=3))
        axes.x_axis.add_numbers(labeled_xs, font_size=16, buff=0.25)
        axes.y_axis.add_numbers(np.arange(0, 0.6, 0.1), num_decimal_places=2)
...
class PrimesNearMillion(InteractiveScene):
    ...
    def construct(self):
        ...
        number_line = NumberLine((T - radius, T + radius), width=250, tick_size=0.075)
        ...
        for p1, p2 in zip(primes, primes[1:]):
            if p1 + 2 == p2:
                arc = Line(number_line.n2p(p1), number_line.n2p(p2), path_arc=-PI)
        ...
        line_group.shift(1.5 * DOWN + -number_line.n2p(1000000.0))
        ...
        arrow = Arrow(lhs['T'].get_bottom(), example.get_right(), stroke_width=8, stroke_color=BLUE, path_arc=-PI / 2, buff=0.2)
        self.add(words)
        self.wait(13)
        self.play(FadeTransform(words, lhs, run_time=1), ShowCreation(arrow), FadeIn(example, time_span=(1, 2)))
        self.wait(5)
        self.play(FadeOut(arrow), FadeOut(example), Write(approx), FadeIn(rhs[:-2], DOWN), FadeIn(rhs[-1], DOWN), TransformFromCopy(lhs['T'], rhs['T']))
        self.wait(45)

    def old_zooming(self):
        sf = 0.1
        self.play(number_line.animate.scale(sf, about_point=ORIGIN), dots.animate.scale(sf, about_point=ORIGIN).set_radius(0.1), rate_func=rush_from, run_time=17)
        self.wait()
        zoom_point = number_line.n2p(1000210)
        frame = self.frame
        self.play(frame.animate.move_to(zoom_point).set_height(0.6), dots.animate.set_radius(0.03), run_time=5)

class PrimePanning(InteractiveScene):

    def construct(self):
        N_max = 500
        number_line = NumberLine((0, N_max), unit_size=0.5, tick_size=0.075)
...
class SieveWithMod(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(grid)
        self.add(labels)
        non_primes = VGroup((label for label in labels if not sympy.isprime(label.get_value())))
        primes = VGroup((label for label in labels if sympy.isprime(label.get_value())))
        ...
        self.play(non_primes.animate.set_fill(opacity=0.25), lag_ratio=0.025, run_time=5)
        self.wait()
        self.play(TransformFromCopy(primes, prime_list))
        self.play(labels.animate.set_fill(opacity=1), FadeOut(prime_list))
        ...
        for _ in range(5):
            ...
            rows = VGroup((labeled_boxes[n:n + row_size] for n in range(0, len(grid), row_size)))
            ...
            self.play(ShowCreation(highlight), FadeOut(arrows), FadeOut(reductions))
            self.play(TransformFromCopy(prime_label, list_prime), FadeIn(comma, UP), TransformFromCopy(highlight, list_prime_highlight))
            self.play(FadeOut(list_prime_highlight), FadeOut(highlight), TransformFromCopy(list_prime, reduction_prime), reduction_label.animate.set_opacity(1), MoveToTarget(rows))
            ...
            for label, box in zip(labels, grid):
                ...
                arrow = Arrow(label, reduction, buff=0.15, max_tip_length_to_length_ratio=0.4, stroke_width=3, stroke_color=GREY_A)
                arrows.add(arrow)
            ...
            for n, arrow, reduction in zip(it.count(), arrows, reductions):
                ...
                if n < 36:
                    self.wait(0.2 * (1 + n % 2))
            killed_indices = [n for n, label in enumerate(labels) if label.get_value() % prime_value == 0]
            self.play(*(LaggedStartMap(FadeOut, VGroup((group[n] for n in killed_indices)), shift=0.1 * DOWN, lag_ratio=0.1) for group in [labeled_boxes, arrows, reductions]), FadeOut(reduction_prime))
...
class DensityFormula(InteractiveScene):
    ...
    def old_mess(self):
        kw = dict(t2c={'T': BLUE, '1{,}000{,}000': BLUE})
        lhs = TexText("What's the density of primes near $T$", **kw)
        rhs = Tex('\\approx \\frac{1}{\\ln(T)}', **kw)
        ...
        lhs_note = TexText('(Some big number, e.g. $T = 1{,}000{,}000$)', font_size=36, **kw)
...
class NewGapsInPrimes(InteractiveScene):
    ...
    def construct(self):
        x_min = 99980
        x_max = 100100
        line = NumberLine(x_range=(x_min, x_max), width=0.5 * (x_max - x_min))
        ...
        self.play(self.frame.animate.match_x(center_label).set_height(11), run_time=8)
        prime_label = TexText('Big prime, $p$', font_size=96)
        prime_label.set_color(YELLOW)
        prime_label.next_to(center_label.get_corner(DL), DOWN, 1.5, aligned_edge=RIGHT)
        arrow = Arrow(prime_label[-1].get_top(), center_label.get_bottom(), buff=0.25)
        arrow.set_stroke(YELLOW)
        self.play(FadeIn(prime_label), GrowArrow(arrow))
        self.wait()
        brace = Brace(Line(*dots.get_points()[-2:]), UP)
        gap_label = Text('gap', font_size=72)
        gap_label.next_to(brace, UP)
        self.play(GrowFromPoint(brace, brace.get_left()), FadeIn(gap_label, 2 * RIGHT))
        self.wait()
        eq = TexText('E[gap] = $\\ln(p)$', font_size=96)
        eq['p'][-1].set_color(YELLOW)
        eq.next_to(self.frame.get_top(), DOWN, buff=MED_LARGE_BUFF)
        self.play(LaggedStart(FadeIn(eq['E[']), FadeIn(eq['] = ']), FadeIn(eq['\\ln(']), FadeIn(eq[')']), FadeTransform(gap_label.copy(), eq['gap'][0]), FadeTransform(prime_label[-1].copy(), eq['p'][-1]), lag_ratio=0.1))
        self.wait()
...
class CrankEmail(InteractiveScene):
    ...
    def create_array(self, numbers, modulus, color=BLUE, height=0.85, spacing=10, buff=0.75, width=10):
        result = VGroup()
        ...
        while n < len(row1_content):
            result.add(self.create_table(row1_title, row2_title, row1_content[n:n + spacing], row2_content[n:n + spacing], color=color, height=height))
            n += spacing
            row1_title = ''
            row2_title = ''
    ...
    def create_table(self, row1_title, row2_title, row1_content, row2_content, x_spacing=0.6, font_size=36, color=BLUE, height=0.85):
        row1_mobs, row2_mobs = (VGroup(*(Integer(n, font_size=font_size) for n in content)) for content in [row1_content, row2_content])
        ...
        v_lines = VGroup(*(Line(grid.get_height() * UP, ORIGIN).set_x(0.5 * (row1_mobs[i].get_right()[0] + row1_mobs[i + 1].get_left()[0])).align_to(grid, UP) for i in range(len(row1_mobs) - 1)))
        ...
        for grid in array:
            for m1, m2 in zip(grid[0], grid[1]):
                if m2.get_value() == 0:
                    ...
                    rects.add(rect)
        self.play(ShowCreation(crosses, lag_ratio=0), FadeIn(rects, lag_ratio=0.1))

    def anticipate_frame_to_y(self, y, run_time=3):
        turn_animation_into_updater(ApplyMethod(self.frame.set_y, y, run_time=run_time))
...
class SieveOfEratosthenes(InteractiveScene):
    ...
    def construct(self):
        ...
        for n in range(self.n_iterations):
            ...
            to_remove = VGroup(*(mob for mob in number_mobs if mob.get_value() % modulus == 0))
            rects = VGroup(*(SurroundingRectangle(tr, buff=self.rect_buff) for tr in to_remove if tr.get_fill_opacity() > 0.5))
            rects.set_stroke(RED, 1)
            self.play(to_remove.animate.set_color(RED), Write(rects, stroke_color=RED, stroke_width=2), lag_ratio=0.1, run_time=2)
            self.wait()
            self.play(to_remove[0].animate.set_color(WHITE), to_remove[1:].animate.set_opacity(0), number_mobs[0].animate.set_opacity(0), FadeOut(rects))
            modulus = numbers[0]

    def get_number_mobs(self, grid):
        return VGroup(*(Integer(i).set_height(0.3 * box.get_height()).move_to(box) for i, box in zip(it.count(1), grid)))
...
class WannaProve(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(randy.says('Twin primes\nare infinite!', mode='hooray'), morty.change('concentrating'))
        self.wait()
        self.play(FadeOut(randy.bubble), randy.change('pondering', look_at=3 * UP), morty.change('raise_right_hand', look_at=3 * UP))