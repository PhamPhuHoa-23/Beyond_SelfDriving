...
class HoldUpThumbnail(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        morty = self.teacher
        stds = self.students
        self.play(FadeIn(im_group, UP), morty.change('raise_right_hand', look_at=im_group), self.change_students('tease', 'happy', 'tease', look_at=im_group))
        self.wait(4)
...
class IsThisUsefulToShare(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        self.play(morty.says('Do you find\nthis useful?'), self.change_students('pondering', 'hesitant', 'well', look_at=self.screen))
        self.wait(3)
...
class AskAboutAttention(TeacherStudentsScene):
    ...
    def construct(self):
        stds = self.students
        morty = self.teacher
        self.play(morty.change('tease'), stds[2].says('Can you explain what\nAttention does?', mode='raise_left_hand', bubble_direction=LEFT), stds[1].change('pondering', self.screen), stds[0].change('pondering', self.screen))
        self.wait(4)
...
class PredictTheNextWord(SimpleAutogregression):
    ...
    def construct(self):
        ...
        self.play(FadeIn(blocks, shift=0.0, lag_ratio=0.01), LaggedStartMap(VShowPassingFlash, block_outlines.family_members_with_points(), time_width=2.0, lag_ratio=0.01, remover=True), LaggedStartMap(VFadeInThenOut, flat_dials, lag_ratio=0.001, remover=True), Write(llm_text, time_span=(2, 4), stroke_color=WHITE), FadeIn(last_dials, time_span=(4, 5)), frame.animate.reorient(0, 0, 0, (-0.17, -0.12, 0.0), 4.5), run_time=6)
        blocks[-1].add(last_dials)
        self.play(frame.animate.to_default_state(), FadeIn(text_group, UP), GrowFromCenter(in_arrow), run_time=3)
        ...
        self.animate_text_input(text_mob, machine, position_text_over_machine=False)
        self.play(LaggedStart((TransformFromCopy(VectorizedPoint(machine.get_right()), letter) for letter in prediction), lag_ratio=0.05), GrowArrow(out_arrow))
        self.wait()
        ...
        self.wait()
        self.play(Restore(text_group), FadeOut(in_arrow))
        seed_label = Text('Seed text')
        seed_label.set_color(YELLOW)
        seed_label.next_to(text_mob, DOWN)
        self.play(FadeIn(seed_label, rate_func=there_and_back_with_pause), FlashAround(text_mob, time_width=2), frame.animate.reorient(0, 0, 0, (0.7, -0.01, 0.0), 8.52), run_time=2)
        self.animate_random_sample(bar_groups)
        new_text_mob = self.animate_word_addition(bar_groups, text_mob, next_word_line)
        for n in range(20):
            text_mob = self.new_selection_cycle(text_mob, next_word_line, machine, quick=True, skip_anims=n > 5)
            self.wait(0.25)
...
class LotsOfTextIntoTheMachine(PredictTheNextWord):
    ...
    def construct(self):
        ...
        self.play(LaggedStart((Succession(FadeIn(text_mob), text_mob.animate.set_opacity(0).move_to(machine.get_center())) for text_mob in text_mobs), lag_ratio=0.05, run_time=self.run_time))
...
class FirthQuote(InteractiveScene):
    ...
    def construct(self):
        quote = TexText("``You shall know a word\\\\by the company it keeps!''", font_size=60)
        image = ImageMobject('JohnRFirth')
        ...
        self.wait()
        phrases = VGroup(Text('Down by the river bank'), Text('Deposit a check at the bank'))
        ...
        self.play(FadeIn(bank, scale=2, lag_ratio=0.25), quote.animate.scale(0.7, about_edge=UP).set_opacity(0.75))
        self.wait()
        self.remove(bank)
        self.play(FadeIn(phrases[0][:len('downbytheriver')], lag_ratio=0.1), FadeIn(phrases[1][:len('depositacheckatthe')], lag_ratio=0.1), *(TransformFromCopy(bank, bank2) for bank2 in banks))
        self.wait()
        self.play(phrases[0].animate.set_opacity(0.5), phrases[1].animate.set_opacity(1))
        self.wait()
        self.play(LaggedStart(FadeOut(image, LEFT, scale=0.5), FadeOut(name, LEFT, scale=0.5), FadeOut(quote, LEFT, scale=0.5), phrases.animate.set_opacity(1).arrange(DOWN, buff=3.5, aligned_edge=LEFT).move_to(0.5 * UP)))
        self.wait()
        ...
        self.wait()
        self.remove(word)
        self.play(*(FadeIn(phrase[phrase.get_text().replace('bank', '')]) for phrase in phrases), *(TransformFromCopy(word, phrase['bank'][0]) for phrase in phrases))
        self.add(phrases)
        ...
        key_rects = VGroup(SurroundingRectangle(phrases[0]['river']), SurroundingRectangle(phrases[1]['Deposit']), SurroundingRectangle(phrases[1]['check']))
        key_rects.set_stroke(BLUE, 2)
        key_rects.set_fill(BLUE, 0.5)
        key_rects[2].match_height(key_rects[1], about_edge=UP, stretch=True)
        arrows = VGroup(Arrow(key_rects[0].get_top(), banks[0].get_top(), path_arc=-180 * DEGREES, buff=0.1), Arrow(key_rects[1].get_top(), banks[1].get_top(), path_arc=-90 * DEGREES), Arrow(key_rects[2].get_top(), banks[1].get_top(), path_arc=-90 * DEGREES))
        ...
        self.wait()
        images = Group(ImageMobject('RiverBank'), ImageMobject('FederalReserve'))
        for image, bank in zip(images, banks):
            image.set_height(2.0)
            image.next_to(bank, DOWN, MED_SMALL_BUFF, aligned_edge=LEFT)
        self.play(LaggedStart((FadeTransform(Group(word).copy(), image) for word, image in zip(banks, images)), lag_ratio=0.5, group_type=Group))
        self.wait(2)
...
class RiverBankProbParts(SimpleAutogregression):
    ...
    def construct(self):
        text_mob, next_word_line, machine = self.init_text_and_machine()
        machine.set_x(0)
        words = ['water', 'river', 'lake', 'grass', 'waves', 'shallows', 'pool', 'depths', 'foam', 'mist']
...
class FourStepsWithParameters(InteractiveScene):
    ...
    def construct(self):
        ...
        names = VGroup(*map(TexText, ['Text snippets\\\\$\\downarrow$\\\\Vectors', 'Attention', 'Feedforward', 'Final prediction']))
        ...
        self.wait()
        machines = VGroup((MachineWithDials(width=rect.get_width(), height=3.0, n_rows=9, n_cols=6) for rect in rects))
        for machine, rect in zip(machines, rects):
            ...
            for dial in machine.dials:
                dial.set_value(0)
        self.play(LaggedStart((LaggedStart((GrowFromPoint(dial, machine.get_top()) for dial in machine.dials), lag_ratio=0.025) for machine in machines), lag_ratio=0.25), LaggedStartMap(FadeOut, names))
        for _ in range(2):
            self.play(LaggedStart((machine.random_change_animation() for machine in machines), lag_ratio=0.2))
...
class ChatbotFeedback(InteractiveScene):
    ...
    def judge_answer(self):
        mark = random.choice([Checkmark().set_color(GREEN), Exmark().set_color(RED)])
        mark.scale(5)
...
class ContrastWithEarlierFrame(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(vline)
        titles = VGroup(VGroup(Text('Most earlier models')), VGroup(Text('Transformers')))
...
class SequentialProcessing(InteractiveScene):
    ...
    def construct(self):
        ...
        for n in range(len(blocks) - 1):
            ...
            self.play(LaggedStart((ContextAnimation(elem, blocks[n][1], lag_ratio=0.01) for elem in vect.get_entries()), lag_ratio=0.01), RandomizeMatrixEntries(vect), run_time=2)
...
class PartialScript(SimpleAutogregression):
    ...
    def construct(self):
        ...
        curled_script_templates = Group((ParametricSurface(lambda u, v: (*curve.pfp(v)[:2], u), **surface_kw) for curve in curves))
        ...
        self.play(TransformFromCopy(curled_script0, curled_script1), frame.animate.reorient(56, -17, 0, (-0.2, -1.52, -2.39), 20.05), run_time=3)
        self.play(frame.animate.reorient(-6, -11, 0, (1.06, -1.22, -2.65), 20.05), run_time=8)
        self.play(FadeOut(curled_script1, shift=0.01 * IN), FadeIn(curled_script1_torn, shift=0.01 * IN))
        self.play(ReplacementTransform(curled_script1_torn, flat_script), frame.animate.to_default_state(), run_time=2)
        self.wait()
        ...
        self.play(flat_script.animate.set_height(5).to_edge(LEFT), FadeIn(machine, lag_ratio=0.01))
        self.add(machine)
        self.wait()
        ...
        self.play(GrowArrow(out_arrow), FadeIn(prediction, DOWN))
        self.wait()
        ...
        self.play(FadeIn(script_text), FadeOut(flat_script), FadeOut(VGroup(in_text, in_arrow, prediction)))
        ...
        for n in range(6):
            ...
            self.play(mover.animate.set_height(1.8).next_to(machine, UP, SMALL_BUFF).set_anim_args(path_arc=-30 * DEGREES))
            self.animate_text_input(mover, machine, position_text_over_machine=False, lag_ratio=0.001)
            self.play(FadeIn(prediction, DOWN, rate_func=rush_from, run_time=0.5))
            if n > 2:
                self.skip_animations = False
                self.wait(0.5)
                self.skip_animations = True
            self.play(curr_answer.animate.set_color(WHITE), Transform(prediction, word_in_answer), FadeOut(mover))
...
class ShowMachineWithDials(PredictTheNextWord):
    ...
    def construct(self):
        ...
        self.clear()
        self.add(frame)
        frame.reorient(0, 0, 0, (-0.17, -0.12, 0.0), 4.5)
        ...
        rect_lines = VGroup(Line(small_rect.get_corner(UL), big_rect.get_corner(DL)), Line(small_rect.get_corner(UR), big_rect.get_corner(DR)))
        ...
        self.play(MoveToTarget(last_dials), FadeOut(llm_text), FadeIn(small_rect))
        ...
        self.play(LaggedStart(ShowCreation(rect_lines, lag_ratio=0), TransformFromCopy(small_rect, big_rect), TransformFromCopy(last_dials[0], big_dial), FadeIn(in_text), GrowArrow(in_arrow), FadeIn(bar_groups), GrowArrow(out_arrow)), frame.animate.reorient(0, 0, 0, (-0.43, 0.38, 0.0), 7.05), run_time=2)
        self.play(last_dials[0].animate_set_value(0.8), big_dial.animate_set_value(0.8), LaggedStart((dial.animate_set_value(dial.get_random_value()) for dial in last_dials[1:]), lag_ratio=1.0 / len(last_dials)), *(self.bar_group_change_animation(bg, value) for bg, value in zip(bar_groups[:-1], true_probs)), run_time=3)
        self.wait()
        self.play(LaggedStart((dial.animate_set_value(0) for dial in last_dials[:12]), lag_ratio=0.01), big_dial.animate_set_value(0), self.bar_group_change_animation(bar_groups[0], 50), self.bar_group_change_animation(bar_groups[1], 34), self.bar_group_change_animation(bar_groups[2], 5), run_time=4)
        self.play(LaggedStart((dial.animate_set_value(1) for dial in last_dials[:12]), lag_ratio=0.01), big_dial.animate_set_value(1), self.bar_group_change_animation(bar_groups[0], 80), self.bar_group_change_animation(bar_groups[1], 5), self.bar_group_change_animation(bar_groups[2], 15), run_time=4)
        self.wait()
        random_words = Text('Initially random')
        random_words.next_to(blocks, UP)
        random_words.set_color(RED)
        out_dots = Tex('...', font_size=120)
        out_dots.next_to(out_arrow, RIGHT)
        self.play(FadeOut(big_rect), Uncreate(rect_lines, lag_ratio=0), FadeOut(small_rect), Transform(big_dial, last_dials[0]))
        self.play(Write(random_words), LaggedStart((dial.animate_set_value(dial.get_random_value()) for dial in last_dials), lag_ratio=0.5 / len(last_dials), run_time=2), FadeOut(bar_groups))
        self.play(Write(out_dots))
        self.wait()
        self.play(FadeOut(dots), FadeOut(random_words), FadeIn(bar_groups))
        example.save_state()
        blocks.save_state()
        ...
        new_dials = VGroup((all_dials.target.copy().shift(3 * 2 * x * (flat_dials.get_center() - last_dials.get_center())) for x in range(1, 9)))
        self.play(FadeOut(example), FadeOut(blocks), FadeIn(flat_dials), FadeOut(bar_groups), FadeOut(out_arrow))
        self.play(FadeOut(highlighed_parameter_group), MoveToTarget(all_dials), LaggedStart((TransformFromCopy(all_dials.copy().set_opacity(0), nd) for nd in new_dials), lag_ratio=0.05), frame.animate.reorient(-9, 0, 0, (-0.71, -0.07, -0.06), 9.64), run_time=4)
        self.wait()
    ...
    def bar_group_change_animation(self, bar_group, new_value):
        ...
        return AnimationGroup(rect.animate.stretch(factor, 0, about_edge=LEFT), ChangeDecimalToValue(value_mob, new_value), UpdateFromFunc(text, lambda m: value_mob.move_to(rect.get_right() + buff, LEFT)))
...
class ShowSingleTrainingExample(ShowMachineWithDials):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(DrawBorderThenFill, VGroup(start_rect, end_rect)), FadeIn(arrow))
        self.remove(last_dials)
        self.play(LaggedStart(AnimationGroup(TransformFromCopy(start, in_text[:-1]), TransformFromCopy(end_rect, in_text[-1]), FadeIn(in_arrow)), LaggedStart((block.animate.set_color(block.get_color() if block is blocks[-1] else TEAL).set_anim_args(rate_func=there_and_back) for block in blocks), group=blocks, lag_ratio=0.1, run_time=1), Animation(last_dials), GrowArrow(out_arrow), LaggedStartMap(GrowFromPoint, bar_groups, point=out_arrow.get_start()), lag_ratio=0.3))
        self.wait()
        out_rects = VGroup((SurroundingRectangle(bg) for bg in bar_groups[:2]))
        out_rects.set_stroke(RED, 3)
        annotations = VGroup((Tex(tex, font_size=60).next_to(rect, LEFT, buff=SMALL_BUFF) for rect, tex in zip(out_rects, ['\\uparrow', '\\downarrow'])))
        annotations.set_color(RED)
        self.play(FadeTransform(end_rect.copy(), out_rects[0]), Write(annotations[0]))
        self.wait()
        self.play(FadeTransform(*out_rects), FadeTransform(*annotations))
        self.wait()
        self.play(FadeOut(out_rects[1]), FadeOut(annotations[1]))
        self.play(LaggedStart((dial.animate_set_value(dial.get_random_value()) for dial in last_dials), lag_ratio=1.0 / len(last_dials)), LaggedStart((FlashAround(dial, stroke_width=2, color=YELLOW, time_width=1, buff=0.025) for dial in last_dials), lag_ratio=1.0 / len(last_dials)), self.bar_group_change_animation(bar_groups[0], 70), self.bar_group_change_animation(bar_groups[1], 20), self.bar_group_change_animation(bar_groups[2], 8), run_time=6)
...
class ParameterWeight(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(LaggedStart(Restore(param), FadeIn(text['/ Weight'])))
        self.wait()
...
class LargeInLargeLanguageModel(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(Restore(large, path_arc=-30 * DEGREES), Write(text[len(large):], time_span=(0.5, 1.5)))
        self.wait()
...
class EnormousAmountOfTrainingText(PremiseOfMLWithText):
    ...
    def construct(self):
        ...
        for n in range(n_examples):
            self.play(LaggedStart(*(self.change_example_animation(screen, show_dial_change=True) for screen in inner_screens), lag_ratio=0.1, run_time=0.5))
    ...
    def change_example_animation(self, screen, show_dial_change=True):
        ...
        time_span = (0, 0.35)
        anims = [FadeOut(screen.training_example, time_span=time_span), FadeIn(new_example, time_span=time_span)]
        if show_dial_change:
...
class BadChatBot(InteractiveScene):
    ...
    def construct(self):
        ...
        self.blink(bot)
        self.wait()
        self.play(LaggedStart((Transform(line, self.get_scribble(line)) for line in lines), lag_ratio=0.1, run_time=2))
        for _ in range(2):
            self.blink(bot)
            self.wait(2)
    ...
    def get_scribble(self, line):
        ...
        graph = FunctionGraph(lambda x: 0.05 * sum((math.sin(freq * TAU * x) for freq in freqs)), x_range=(0, 5, 0.1))
...
class WriteRLHF(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(text)
        self.wait()
        self.play(TransformFromCopy(text['RLHF'][0], initials, lag_ratio=0.25), Write(full_text, time_span=(1.5, 3)), run_time=3)
        self.wait()
...
class RLHFWorkers(ShowMachineWithDials):
    ...
    def construct(self):
        ...
        self.add(machine)
        for _ in range(8):
            self.play(LaggedStart((dial.animate_set_value(dial.get_random_value()) for dial in last_dials), lag_ratio=0.5 / len(last_dials), run_time=2))
            self.wait()
...
class SerialProcessing(InteractiveScene):
    ...
    def construct(self):
        ...
        for word, vect in zip(words, vectors):
            self.play(FadeIn(vect, run_time=2), LaggedStart((ContextAnimation(square, VGroup(*word, *last_vect), direction=DOWN, lag_ratio=0.01, path_arc=30 * DEGREES) for square in vect), lag_ratio=0.05, run_time=2), last_vect.animate.set_opacity(0.2))
            last_vect = vect
...
class ParallelProcessing(SerialProcessing):
    ...
    def construct(self):
        ...
        lines = VGroup((Line(rect.get_bottom(), vect.get_top(), buff=0.05, stroke_color=WHITE, stroke_width=2 * random.random() ** 3) for rect in rects for vect in vectors))
        lines.shuffle()
        ...
        self.play(LaggedStartMap(ShowCreation, lines, lag_ratio=0.01), LaggedStartMap(Restore, vectors, lag_ratio=0))
...
class ManyComputationsPerUnitTimeV2(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait(3)
        width = FRAME_WIDTH - 1
        number_lines = VGroup((minute_line := NumberLine((0, 60, 1), width=width, big_tick_spacing=10)), (hour_line := NumberLine((0, 60, 1), width=width, big_tick_spacing=10)), (day_line := NumberLine((0, 24, 1), width=width, big_tick_spacing=6)), (month_line := NumberLine((0, 31, 1), width=width)), (year_line := NumberLine((0, 12, 1), width=width)), (y100_line := NumberLine((0, 100, 1), width=width)), (y10k_line := NumberLine((0, 100, 1), width=width)), (y1M_line := NumberLine((0, 100, 1), width=width)), (y100M_line := NumberLine((0, 100, 1), width=width)))
        number_lines.move_to(DOWN)
        first_ticks = minute_line.ticks[:2]
        sec_brace = Brace(first_ticks, DOWN, buff=0, tex_string='\\underbrace{\\qquad\\qquad}')
        sec_label = Text('Second', font_size=30).next_to(sec_brace, DOWN, SMALL_BUFF)
        self.play(ShowCreation(minute_line, lag_ratio=0.01), box.animate.match_width(first_ticks).move_to(first_ticks.get_center(), DOWN).set_stroke(width=1), TransformFromCopy(label['Second'][0], sec_label), GrowFromCenter(sec_brace), run_time=2)
        ...
        self.play(FadeIn(minute_label, DOWN), LaggedStartMap(Restore, new_boxes, lag_ratio=0.1), run_time=2)
        self.wait(2)
        ...
        for nl1, nl2 in zip(number_lines, number_lines[1:]):
            ...
            pair = VGroup(DashedLine(nl1.get_start(), mini_line.get_start()), DashedLine(nl1.get_end(), mini_line.get_end()))
    ...
    def fade_in_bigger_interval(self, new_interval, prev_interval, fader, scale_factor, added_anims=[]):
        ...
        self.play(Restore(new_interval), prev_interval.animate.scale(1.0 / scale_factor, about_point=pivot).set_fill(border_width=0), fader.animate.scale(1.0 / scale_factor, about_point=pivot).set_opacity(0), *added_anims, run_time=4, rate_func=rush_from)
        self.remove(fader)
        ...
        for iteration in range(n_iterations):
            ...
            for n in range(n_lines):
                ...
                if random.choice([True, False]):
                    ...
                    comb = x + y
                    sym = Tex('+')
                line = VGroup(DecimalNumber(x, num_decimal_places=3), sym, DecimalNumber(y, num_decimal_places=3), Tex('='), DecimalNumber(comb, num_decimal_places=3))
    ...
    def old(self):
        ...
        for new_int, prev_int, scale_factor in zip(number_lines[1:], number_lines[0:], scale_factors):
            self.fade_in_bigger_interval(new_int, prev_int, to_fade, scale_factor, added_anims=[label.animate.set_opacity(0)])
            self.wait(2)
            to_fade = prev_int
        self.fade_in_bigger_interval(y1M_line, millenium_line, year_line, 1000, added_anims=[self.frame.animate.reorient(0, 0, 0, (-3.51, -5.18, 0.0), 12.93)])
        ...
        self.play(LaggedStart((TransformFromCopy(lines[0].copy().set_opacity(0), line) for line in lines), lag_ratio=0.03, run_time=2), FadeIn(side_brace, scale=10, shift=2 * DOWN, time_span=(1, 2)), FadeIn(label100M, time_span=(1, 2)))
        self.wait()
...
class ParameterToVectorAnnotation(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(Write(dials, lag_ratio=0.01), LaggedStartMap(GrowArrow, arrows))
        self.play(LaggedStart((dial.animate_set_value(value) for dial, value in zip(dials, values)), lag_ratio=0.05))
        self.wait()
...
class ThreeWordsToOne(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        rects = VGroup((SurroundingRectangle(word).set_fill(color, 0.2).set_stroke(color, 2) for word, color in zip(words.saved_state, colors)))
        words.set_z_index(1)
        icons = VGroup(SVGMobject('GenericComputer.svg'), SVGMobject('History.svg'), SVGMobject('Museum.svg'))
        ...
        self.play(ReplacementTransform(VGroup(rect), rects), Restore(words), *(FadeTransform(chm_image.copy(), icon) for icon in icons))
        self.wait()
...
class TrainingDataCHM(InteractiveScene):
    ...
    def construct(self):
        passages = ['The Computer History Museum (CHM) is a museum ... located in Mountain View...', 'Computer History Museum ... 1401 N. Shoreline Blvd. Mountain View...', 'Things to do in Mountain View ... the Computer History Museum ...', 'While I was in Mountain View ... stopped by the Computer History Museum ...']
        ...
        self.play(FadeIn(title), ShowCreation(underline), LaggedStartMap(FadeIn, items, shift=DOWN, lag_ratio=0.15))
        self.wait()
        ...
        arrows = VGroup((Arrow(item.get_right(), interpolate(item.get_right(), ffn_point, 0.6), path_arc=arc * DEGREES) for item, arc in zip(items[:-1], range(-40, 40, 20))))
        arrows.set_fill(border_width=1)
        self.play(Write(arrows, lag_ratio=0.1), run_time=3)
        self.play(LaggedStart(*(FadeOutToPoint(letter.copy(), ffn_point) for letter in VGroup(chm_phrases, mv_phrases).family_members_with_points()), lag_ratio=0.01, run_time=3))
        self.wait()
...
class DivyUpParameters(ShowMachineWithDials):
    ...
    def construct(self):
        ...
        self.play(FadeIn(blocks, shift=0.0, lag_ratio=0.01), LaggedStartMap(VShowPassingFlash, block_outlines.family_members_with_points(), time_width=2.0, lag_ratio=0.01, remover=True), LaggedStartMap(VFadeInThenOut, flat_dials, lag_ratio=0.001, remover=True), FadeIn(last_dials, time_span=(2, 3)), self.frame.animate.reorient(10, -2, 0, (-0.25, -1.58, -0.02), 4.61), run_time=3)
        ...
        labels = VGroup(TexText('Word $\\to$ Vector'), Text('Attention'), Text('Feedforward'))
        for label, block in zip(labels, block_targets):
            label.next_to(block, DOWN)
        self.add(blocks[0], top_blocks[0], blocks[1], top_blocks[1], blocks[2], top_blocks[2], blocks[3:], last_dials)
        self.play(MoveToTarget(top_blocks[1], time_span=(0, 2)), MoveToTarget(top_blocks[2], time_span=(1, 3)), MoveToTarget(top_blocks[0], time_span=(2, 4)), Write(labels[1], time_span=(1.5, 2)), Write(labels[2], time_span=(2.5, 3)), Write(labels[0], time_span=(3.5, 4)), frame.animate.to_default_state(), run_time=4)
        self.wait()
        self.play(LaggedStart((dial.animate_set_value(dial.get_random_value()) for dial in all_dials), lag_ratio=1 / len(all_dials), run_time=6), LaggedStart((FlashAround(dial, buff=0, color=YELLOW) for dial in all_dials), lag_ratio=1 / len(all_dials), run_time=6))
        self.wait()
...
class ShowPreviousVideos(InteractiveScene):
    ...
    def construct(self):
        ...
        thumbnails = Group((Group(Rectangle(16, 9).set_height(1).set_stroke(WHITE, 2), ImageMobject(f'https://img.youtube.com/vi/{slug}/maxresdefault.jpg', height=1)) for slug in ['aircAruvnKk', 'IHZwWFHWa-w', 'Ilg3gGewQ5U', 'tIeHLnjs5U8', 'wjZofJX0v4M', 'eMlx5fFNoYc', '9-Jl0dxWQs8']))
        ...
        self.play(series_name.animate.set_x(left_x), thumbnails.animate.arrange_in_grid(n_cols=2, buff=0.25).set_height(6).set_x(left_x).to_edge(DOWN), ShowCreation(line, time_span=(1, 2)), run_time=2)
        self.wait()
