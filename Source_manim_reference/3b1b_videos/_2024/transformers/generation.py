...
def gpt3_predict_next_token(text, n_shown=10, random_seed=0):
    ...
    response = openai.Completion.create(engine='gpt-3.5-turbo-instruct', prompt=text, max_tokens=1, n=1, temperature=1.0, user=str(random_seed), logprobs=50)
...
def clean_text(text):
    return ' '.join(filter(lambda s: s.strip(), re.split('\\s', text)))

def next_token_bar_chart(words, probs, reference_point=ORIGIN, font_size=24, width_100p=1.0, prob_exp=0.75, bar_height=0.25, bar_space_factor=0.5, buff=1.2, show_ellipses=True, use_percent=True):
    ...
    if show_ellipses:
        ellipses = Tex('\\vdots', font_size=font_size)
...
class SimpleAutogregression(InteractiveScene):
    ...
    def construct(self):
        ...
        for n in range(self.n_predictions):
            text_mob = self.new_selection_cycle(text_mob, next_word_line, machine, quick=n > 10, skip_anims=self.skip_through)
    ...
    def get_transformer_drawing(self):
        ...
        out_arrow = Vector(0.5 * RIGHT, stroke_width=10, max_tip_length_to_length_ratio=0.5, max_width_to_length_ratio=12)
        out_arrow.next_to(blocks[-1], RIGHT, buff=SMALL_BUFF)
        out_arrow.set_opacity(0)
        result = VGroup(blocks, word, out_arrow)
        return result

    def get_distribution(self, words, probs, machine, font_size=24, width_100p=1.8, bar_height=0.25, show_ellipses=True):
        labels = VGroup((Text(word, font_size=font_size) for word in words))
    ...
    def animate_text_input(self, text_mob, machine, position_text_over_machine=True, added_anims=[], lag_ratio=0.02):
        ...
        self.play(LaggedStart(*added_anims, Transform(text_copy, VGroup(VectorizedPoint(machine.get_top())), lag_ratio=lag_ratio, run_time=1, path_arc=-45 * DEGREES, remover=True), LaggedStart((block.animate.set_color(block.get_color() if block is blocks[-1] else TEAL).set_anim_args(rate_func=there_and_back) for block in blocks), lag_ratio=0.1, run_time=1), Animation(machine[1:]), lag_ratio=0.5))
    ...
    def animate_prediction_ouptut(self, machine, cur_str):
        ...
        self.play(LaggedStart((FadeInFromPoint(bar_group, machine[0][-1].get_right()) for bar_group in bar_groups), lag_ratio=0.025, group=bar_groups, run_time=1))
        return bar_groups
    ...
    def animate_random_sample(self, bar_groups):
        ...
        def highlight_randomly(rect, dist, alpha):
            ...
            rect.stretch(1.1, 0)
        self.play(UpdateFromAlphaFunc(highlight_rect, lambda rect, a: highlight_randomly(rect, dist, a)), Animation(bar_groups))
    ...
    def animate_word_addition(self, bar_groups, text_mob, next_word_line, force_unskip=False):
        ...
        self.add(bar_groups)
        self.play(FadeTransform(bar_group[0].copy(), target), Transform(next_word_line, self.get_next_word_line(new_text_mob)))
        if force_unskip:
...
class AnnotateNextWord(SimpleAutogregression):
    ...
    def construct(self):
        ...
        highlight = Union(SurroundingRectangle(text_mob['Behold, a wild pi creature,']), SurroundingRectangle(text_mob['foraging in its native']))
        ...
        self.wait()
        self.play(arrow.animate.rotate(PI / 2).next_to(dist_rect, UP), ReplacementTransform(highlight, dist_rect))
        self.wait()
        self.play(FadeOut(dist_rect), FadeOut(arrow))
        self.remove(bar_groups)
        text_mob = self.new_selection_cycle(text_mob, next_word_line, machine)
...
class GPT3CleverestAutocomplete(QuickRegressionGPT3):
    ...
    def construct(self):
        text_mob, next_word_line, machine = self.init_text_and_machine()
        for n in range(self.n_predictions):
            text_mob = self.new_selection_cycle(text_mob, next_word_line, machine, skip_anims=n > 2)
...
class GPT3OnLearningSimpler(QuickRegressionGPT3):
    ...
    def construct(self):
        ...
        for n in range(self.n_predictions):
            ...
            self.add(text_mob)
            if self.show_dist:
                dist = self.get_distribution(words[:self.n_shown_predictions], probs[:self.n_shown_predictions], buff=0)
                dist.set_height(4)
                dist.to_edge(DOWN)
...
class ChatBotPrompt(SimpleAutogregression):
    ...
    def construct(self):
        ...
        system_prompt, user_prompt, ai_seed = all_text = VGroup((get_paragraph(s.split(' '), font_size=self.font_size, line_len=self.line_len) for s in all_strs))
        ...
        user_rect = Union(SurroundingRectangle(low_line), SurroundingRectangle(top_line))
        user_rect.set_stroke(BLUE, 2)
        sys_rect.insert_n_curves(100)
        self.play(ReplacementTransform(sys_rect, user_rect), top_line.animate.set_color(BLUE_B), low_line.animate.set_color(BLUE_B))
        self.wait()
        self.play(FadeOut(user_rect))
        text_mob = all_text
        self.add(all_text.copy())
        for n in range(self.n_predictions):
            text_mob = self.new_selection_cycle(text_mob, next_word_line, machine, skip_anims=n > 0)
...
class TextToImage(VoiceToTextExample):
    ...
    def generate_output(self):
        ...
        response = openai.Image.create(model='dall-e-3', prompt=prompt, size='1024x1024', quality='standard', n=1)
        image_url = response.data[0].url
        print(prompt)
        print(image_url)
        response = openai.Image.create_variation(image=open('/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/images/raster/PiCreatureDalle3_17.png', 'rb'), n=1, size='1024x1024')
...
class PredictionVsGeneration(SimpleAutogregression):
    ...
    def construct(self):
        ...
        self.add(prediction)
        self.play(morty.change('raise_right_hand', prediction), FadeIn(prediction[0], UP), GrowFromCenter(prediction[1]), LaggedStart((FadeInFromPoint(bar, prediction[1].get_center()) for bar in prediction[2]), lag_ratio=0.05))
        self.play(Blink(morty))
        self.play(morty.change('raise_left_hand', 3 * UR))
        self.wait()
        self.play(Blink(morty))
        self.wait()
...
class ManyParallelPredictions(SimpleAutogregression):
    ...
    def construct(self):
        ...
        for prediction, arrow, rect in zip(predictions, arrows, rects):
            ...
            arrow.become(Arrow(rect.get_top(), prediction[1].get_left()))
            arrow.set_stroke(YELLOW)
        last_group = VGroup(rects[0].copy().set_opacity(0), arrows[0].copy().set_opacity(0), predictions[0].copy().set_opacity(0))
        for rect, arrow, prediction in zip(rects, arrows, predictions):
            self.remove(last_group)
            self.play(TransformFromCopy(last_group[0], rect), TransformFromCopy(last_group[1], arrow), TransformMatchingStrings(last_group[2][0].copy(), prediction[0], run_time=1), FadeTransform(last_group[2][1].copy(), prediction[1]), FadeTransform(last_group[2][2].copy(), prediction[2]))
            self.wait()
...
class PeekUnderTheHood(SimpleAutogregression):
    ...
    def construct(self):
        ...
        self.play(self.frame.animate.reorient(-23, -12, 0, (1.79, -0.56, 1.27), 8.4).set_anim_args(run_time=3), MoveToTarget(blocks, run_time=3), FadeOut(arrow, RIGHT), FadeOut(label, 2 * OUT))
        self.wait()
