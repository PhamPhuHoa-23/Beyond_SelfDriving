...
class GPTInitials(InteractiveScene):
    ...
    def construct(self):
        ...
        def letter_anim(letters, point):
            ...
            return LaggedStart((Restore(letter) for letter in letters), lag_ratio=0.05, time_span=(0.25, 0.75))
        self.play(LaggedStartMap(FadeIn, initials, scale=2, lag_ratio=0.25, run_time=1), morty.change('raise_right_hand', initials))
        self.play(Blink(morty))
        self.play(ReplacementTransform(initials[0], words[0][0]), letter_anim(words[0][1:], initials[0].get_center()), morty.animate.look_at(words[0]), run_time=1)
        self.wait(0.5)
        self.play(words[0].animate.next_to(words[1], LEFT, aligned_edge=DOWN), Transform(initials[2], t_target), ReplacementTransform(initials[1], words[1][0]), letter_anim(words[1][1:], initials[1].get_center()), morty.change('well', words[1]), run_time=1)
        self.remove(initials)
        self.wait(0.5)
        self.play(Transform(name[:-len(words[2])], name.saved_state[:-len(words[2])]), ReplacementTransform(initials[2], words[2][0]), letter_anim(words[2][1:], initials[2].get_center()), morty.animate.look_at(words[2]))
        self.add(name)
        self.play(Blink(morty))
        self.wait()
        ...
        self.play(LaggedStart(MoveToTarget(words), ShowCreation(line), frame.animate.center(), morty.change('thinking', words.target[0]).set_opacity(0), Write(t_lines, stroke_width=2), FlashAround(words.target[0].copy())))
        self.remove(morty)
        self.wait()
        for i in [1, 2]:
            words.target = words.generate_target()
            ...
            self.play(line.animate.become(Underline(words[i])).set_stroke(YELLOW).set_anim_args(run_time=0.75), FlashAround(words[i]), MoveToTarget(words))
            self.wait()
        self.play(words[2].animate.set_x(0).set_color(WHITE).shift(0.25 * UP), line.animate.set_x(0).set_color(WHITE).set_width(6).shift(0.25 * UP), FadeOut(words[0], LEFT), FadeOut(words[1], 3 * LEFT), Uncreate(t_lines, lag_ratio=0))
        self.wait()

class DifferentUsesOfModel(InteractiveScene):

    def construct(self):
        sentences = VGroup(Text('A machine learning model ...'), Text('A fashion model ...'))
        images = Group(NeuralNetwork([8, 6, 6, 8]), ImageMobject('Zoolander'))
        ...
        self.wait()
        self.play(FadeTransform(word.copy(), sentences[0]['model']), LaggedStart(Write(sentences[0]), Write(images[0], lag_ratio=0.01, stroke_width=0.5), lag_ratio=0.5, run_time=2))
        self.wait()
        self.play(FadeTransform(word.copy(), sentences[1]['model']), LaggedStart(Write(sentences[1]), FadeIn(images[1], shift=0.5 * UP, scale=1.25), lag_ratio=0.2, run_time=2))
        self.wait()
        s0, s1 = sentences
        path_arc = -0.65 * PI
        left_arrows = VGroup((Arrow(s0[word].get_top(), s0['model'].get_top(), path_arc=path_arc) for word in ['machine', 'learning']))
        right_arrow = Arrow(s1['fashion'].get_top(), s1['model'].get_top(), path_arc=path_arc)
        ...
        self.play(LaggedStartMap(ShowCreation, left_arrows, lag_ratio=0.5), self.frame.animate.move_to(DOWN), images.animate.shift(UP))
        self.play(ShowCreation(right_arrow))
        self.wait()
        words = VGroup(*(s0[word] for word in s0.get_text().split(' ')[:-1]), *(s1[word] for word in s1.get_text().split(' ')[:-1]))
        vectors = VGroup((NumericEmbedding().set_height(2).next_to(word, DOWN, buff=0.2) for word in words))
        self.play(LaggedStartMap(FadeIn, vectors, shift=0.25 * DOWN, lag_ratio=0.25, run_time=3))
        self.play(LaggedStartMap(RandomizeMatrixEntries, vectors))
        self.wait()
...
class LongListOFQuestions(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(word_group)
        questions = VGroup(map(Text, ['Is it English?', 'Is it a noun?', 'Does it refer to a person?', 'Is it an amount?', 'Is A tone assertive', 'Is it a piece of a bigger word?', 'Is it part of a quote?', 'Is it part of a lie?']))
        questions.scale(0.75)
        n_questions = len(questions)
        neurons = VGroup((Circle(radius=0.2) for n in range(n_questions)))
        neurons.add(Tex('\\vdots', font_size=72))
        ...
        connections = VGroup((VGroup((Line(elem.get_right(), neuron.get_center(), buff=neuron.get_width() / 2).set_stroke(color=value_to_color(random.uniform(-10, 10)), width=2 * random.random()) for elem in vector.get_entries())) for neuron in neurons[:-1]))
        lag_ratio = 0.3
        self.play(LaggedStart((ShowCreation(line_group, lag_ratio=0) for line_group in connections), lag_ratio=lag_ratio), LaggedStartMap(FadeIn, neurons, lag_ratio=lag_ratio), LaggedStartMap(FadeIn, questions, lag_ratio=lag_ratio), run_time=4)
        self.wait()
...
class ChatBotIcon(InteractiveScene):
    ...
    def construct(self):
        ...
        bot.set_height(3)
        bot.to_edge(RIGHT)
        arrow = Vector(1.5 * RIGHT, max_tip_length_to_length_ratio=0.4, max_width_to_length_ratio=9.0)
        arrow.set_stroke(width=20)
        arrow.next_to(bot, LEFT).match_y(bot[0])
        self.play(ShowCreation(arrow), Write(bot))
        self.wait()
...
class GamePlan(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(highlight_video(tr_vids[0], tr_vids), MaintainPositionRelativeTo(here_group, tr_vids[0]), VFadeIn(here_group))
        self.wait()
        curly = self.get_curly_brace(tr_vids[0])
        topics = VGroup(Text('Beginning'), Text('Ending'), Text('Background material'), Text('Premise of deep learning'), Text('Word embeddings'), Text('Dot products'), Text('Softmax'))
        ...
        self.play(FadeOut(here_group), ShowCreation(curly), FadeIn(screen, RIGHT), FadeInFromPoint(topics[0], here_group.get_center()))
        self.wait()
        self.play(topics[0].animate.set_opacity(0.5), FadeIn(topics[1]), screen.animate.next_to(topics[1], DOWN, aligned_edge=LEFT))
        self.wait()
        self.play(screen.animate.scale(0.5, about_edge=DR).to_edge(RIGHT), topics[1].animate.set_opacity(0.5), LaggedStartMap(FadeIn, topics[2:], shift=0.1 * DOWN, lag_ratio=0.5))
        self.wait()
        ...
        self.play(highlight_video(tr_vids[1], tr_vids), curly.animate.become(new_curly), FadeOut(topics), MoveToTarget(screen), FadeInFromPoint(att_title, tr_vids[1].get_center()))
        self.wait()
        new_curly = self.get_curly_brace(tr_vids[2].copy().shift(0.5 * RIGHT))
        chapter3_topics = Text('MLPs, Training, Positional encodings, ...')
        chapter3_topics.next_to(screen, UP, aligned_edge=LEFT)
        self.play(highlight_video(tr_vids[2], tr_vids), curly.animate.become(new_curly), FadeOut(att_title), FadeIn(chapter3_topics, lag_ratio=0.1, time_span=(1, 3)))
        self.wait()
        prev_thumbnails = Group((ImageMobject(f'nn{k}_thumbnail.png') for k in range(1, 5)))
        ...
        new_thumbnails = Group((ImageMobject(os.path.join(tn_dir, f'Chapter{n}')) for n in range(5, 8)))
        for tn1, tn2 in zip(prev_thumbnails, new_thumbnails):
            tn2.replace(tn1, stretch=True)
            tn2.next_to(tn1, DOWN, buff=1.0)
        chapter_titles = VGroup((Text(f'Chapter {k}', font_size=30) for k in range(1, 8)))
        for title, rect in zip(chapter_titles, (*prev_thumbnails, *new_thumbnails)):
            title.next_to(rect, UP, buff=0.2, aligned_edge=LEFT)
        tr_rect = SurroundingRectangle(Group(new_thumbnails, chapter_titles[4:]), buff=0.25)
        tr_rect.set_stroke(BLUE, 2)
        tr_label = Text('Transformers')
        tr_label.next_to(tr_rect, DOWN)
        self.play(FadeOut(curly), FadeOut(screen), FadeOut(chapter3_topics), LaggedStartMap(FadeIn, chapter_titles), FadeIn(prev_thumbnails, shift=0.5 * UP, lag_ratio=0.25), *(FadeTransform(vid, tn) for vid, tn in zip(tr_vids, new_thumbnails)))
        self.play(ShowCreation(tr_rect), FadeIn(tr_label))
        self.wait()
    ...
    def get_curly_brace(self, video, width=2.0, height=6.5, buff=0.1):
        ...
        result = VGroup((CubicBezier(start, start + width * RIGHT, point + width * LEFT, point) for point in [top_point, low_point]))
...
class SkipAhead(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        morty = self.teacher
        self.play(morty.change('hesitant', self.students), self.change_students('confused', 'pondering', 'pondering', look_at=self.screen))
        self.wait(2)
...
class SeaOfNumbersUnderlay(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(morty.change('pleading'), self.change_students('surprised', 'horrified', 'droopy'))
        self.look_at(3 * LEFT + 2 * UP)
        self.look_at(3 * RIGHT + 2 * UP)
        self.look_at(3 * LEFT + 2 * UP)
        self.play(morty.change('raise_right_hand', self.screen), self.change_students('hesitant', 'pondering', 'maybe', look_at=self.screen))
        self.wait(2)
        ...
        self.wait(3)
        self.play(morty.change('well'), self.change_students('pondering', 'pondering', 'erm', look_at=self.screen))
        self.wait(3)
        self.play(morty.change('raise_left_hand', look_at=5 * RIGHT + 3 * UP), self.change_students('tease', 'thinking', 'pondering', look_at=5 * RIGHT + 3 * UP))
        self.wait(8)
...
class Outdated(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        morty = self.teacher
        morty.body.insert_n_curves(100)
        self.play(morty.change('raise_right_hand'), FadeIn(gpt3_label, UP))
        self.play(self.change_students('raise_left_hand', 'hesitant', 'sassy'))
        self.play(self.students[0].says(TexText("Isn't that outdated?")))
        self.wait(3)
...
class ConfusionAtScreen(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.change('well'), self.change_students('maybe', 'confused', 'concentrating', look_at=self.screen))
        self.wait(2)
        self.play(self.teacher.change('tease'), self.change_students('hesitant', 'plain', 'erm', look_at=self.teacher.eyes))
        self.wait(3)
...
class HoldUpExample(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.change('raise_right_hand'), self.change_students('happy', 'hooray', 'well', look_at=4 * UR))
        self.wait(5)
...
class ReactToWordVectors(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(pis)
        self.play(PiCreatureSays(morty, 'This is how search\nworks you know!', target_mode='hooray', content_introduction_class=FadeIn, content_introduction_kwargs=dict(lag_ratio=0.1)), randy.change('guilty'))
        self.play(Blink(randy))
        self.wait()
        dots = Text('.....', font_size=120)
        dots[:1].set_opacity(0)
        dots[-1:].set_opacity(0)
        self.play(morty.debubble(), PiCreatureBubbleIntroduction(randy, dots, target_mode='confused', bubble_type=ThoughtBubble), morty.change('tease', look_at=6 * LEFT))
        self.play(Blink(morty))
        self.wait()
...
class DimensionComparrison(InteractiveScene):
    ...
    def construct(self):
        titles = VGroup(Text('3d vectors'), Text('Word vectors'))
        titles.scale(1.5)
        ...
        self.play(ShowCreation(lines, lag_ratio=0.5), LaggedStartMap(Write, titles, lag_ratio=0.5))
        self.wait()
...
class AtLeastKindOf(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(morty.says('...kind of', mode='hesitant'), self.change_students('hesitant', 'sassy', 'erm', look_at=self.screen))
        self.wait(3)
        self.play(self.change_students('sassy', 'hesitant', 'hesitant', look_at=morty.eyes), morty.change('guilty'))
        self.wait(4)
...
class NetworkEndAnnotation(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(im)
        prof = Text('Professor').set_height(0.25).move_to(np.array([4.77, 3.36, 0.0]))
        hp = Text('Harry Potter').set_height(0.33).move_to(np.array([-5.58, 3.33, 0.0]))
...
class LowTempHighTempContrast(InteractiveScene):
    ...
    def construct(self):
        titles = VGroup(Text('Temp = 0', font_size=72).set_x(-FRAME_WIDTH / 4), Text('Temp = 5', font_size=72).set_x(FRAME_WIDTH / 4))
        ...
        lines = VGroup(h_line, v_line)
        lines.set_stroke(GREY_B, 2)
        self.play(LaggedStartMap(FadeIn, titles, shift=0.25 * UP, lag_ratio=0.25), LaggedStartMap(Write, lines, lag_ratio=0.5), run_time=1)
        self.wait()

class Intuitions(TeacherStudentsScene):

    def construct(self):
        words = VGroup(Text('Structure of Deep Learning'), Text('Word embeddings'), Text('Dot products'), Text('Softmax'))
        words.arrange(DOWN, buff=0.5, aligned_edge=LEFT)
        words.move_to(self.hold_up_spot, DOWN)
        checks = VGroup((Checkmark(font_size=72).next_to(word, LEFT) for word in words))
        checks.set_color(GREEN)
        morty = self.teacher
        self.play(LaggedStartMap(FadeIn, words, shift=UP, lag_ratio=0.1), morty.change('raise_right_hand'), self.change_students('thinking', 'pondering', 'well', look_at=words), run_time=1)
        self.play(LaggedStartMap(Write, checks, lag_ratio=0.25, stroke_color=GREEN))
        for pi in self.students:
            pi.body.insert_n_curves(100)
        self.play(self.change_students('tease', 'thinking', 'well'))
        self.wait(4)
...
class HighlightAttentionTitle(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for pi in self.pi_creatures:
            pi.body.insert_n_curves(100)
        self.play(im.animate.set_opacity(0.1), title.animate.set_fill(WHITE).scale(2).next_to(morty, UP, MED_LARGE_BUFF).to_edge(RIGHT), LaggedStartMap(VFadeIn, self.pi_creatures), morty.change('raise_right_hand'), self.change_students('pondering', 'well', 'thinking', look_at=self.hold_up_spot))
        self.wait()
        att = title['Attention'][0]
        rest = title['is All You Need'][0]
        self.play(FlashAround(att, run_time=2), att.animate.set_color(YELLOW))
        self.wait(2)
        self.play(att.animate.center().to_edge(UP), FadeOut(rest, DR), FadeOut(im, scale=1.5), self.background.animate.set_opacity(0.75), morty.change('tease', 3 * UP), self.change_students(None, None, 'pondering', look_at=3 * UP))
        self.look_at(3 * UL)
        self.wait()
        self.look_at(3 * UR)
        self.wait(2)
        ...
        self.play(att.animate.replace(sentence['Attention']), FadeIn(sentence, lag_ratio=0.1), morty.change('raise_right_hand', sentence), self.change_students('sassy', 'confused', 'pondering', look_at=sentence))
        self.play(ShowCreation(par_line), morty.animate.look_at(par_line), FadeIn(par_box), self.change_students('pondering', look_at=par_line))
        self.wait(5)
...
class ThinkOfMoreExamples(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for pi in self.pi_creatures:
            pi.body.insert_n_curves(100)
        self.play(morty.change('raise_right_hand'), self.change_students('confused', 'maybe', 'confused', look_at=3 * UP, run_time=2, lag_ratio=0.25))
        self.wait(2)
        self.play(morty.change('guilty'))
        self.play(self.change_students('confused', 'pleading', 'concentrating', look_at=3 * UP, run_time=2, lag_ratio=0.25))
        self.wait(3)
        self.play(self.change_students('maybe', 'confused', 'dejected', look_at=morty.eyes, lag_ratio=0), morty.change('well'))
        self.wait(2)
        self.wait()
        self.play(LaggedStart(self.students[2].says('What is attention\nsupposed to do?'), self.students[0].change('maybe'), self.students[1].change('pondering'), morty.change('tease'), lag_ratio=0.1))
        self.wait(5)
...
class SimplerExample(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.says('Take a simpler\nexample'), self.change_students('pondering', look_at=self.teacher.eyes))
...
class NotQuiteTrue(InteractiveScene):
    ...
    def construct(self):
        morty = Mortimer()
        morty.to_corner(DR)
        self.play(morty.says("Actually, that's not\nquite true!"), run_time=1)
        for x in range(2):
            self.play(Blink(morty))
            self.wait()
...
class ThisIsMadeUp(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.says('This is a made-up\nmotivating example'), self.change_students('pondering', look_at=self.teacher.eyes))
...
class AskAboutOtherEmbeddings(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.students[1].says(TexText('What does $W_Q$ do \\\\ to the non-nouns?'), mode='raise_left_hand'), self.teacher.change('guilty'))
        self.play(self.change_students('confused', None, 'pondering', look_at=self.screen))
        self.wait()
        self.play(self.teacher.change('shruggie'))
        self.play(self.change_students('sassy', 'maybe', 'sassy'))
        self.wait(3)

class ShoutSoftmax(TeacherStudentsScene):

    def construct(self):
        self.play(LaggedStart(self.students[0].change('happy'), self.students[1].change('hooray'), self.students[2].says('Softmax!', mode='surprised', bubble_config=dict(buff=0.5, direction=LEFT)), self.teacher.change('well')))
        self.wait(5)
...
class SetThemToZero(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.students[0].says('Set them to 0?', mode='maybe'), self.students[1].change('pondering', look_at=self.screen), self.students[1].change('pondering', look_at=self.screen))
        self.wait()
        self.play(self.teacher.says("Then they wouldn't\nbe normalized", mode='tease'))
        self.wait(3)

class CalledMasking(TeacherStudentsScene):

    def construct(self):
        self.play(self.teacher.says(TexText("This is called\\\\``masking''")), self.change_students('pondering', 'confused', 'erm', look_at=self.screen))
        self.wait(5)

class ReferenceLargerContextTechnologies(InteractiveScene):

    def construct(self):
        words = VGroup(Text('Sparse Attention Mechanisms'), Text('Blockwise Attention'), Text('Linformer'), Text('Reformer'), Text('Ring attention'), Text('Longformer'), Text('Adaptive Attention Span'), Tex('\\vdots'))
        words.arrange(DOWN, aligned_edge=LEFT, buff=MED_LARGE_BUFF)
        words[-1].shift(0.5 * RIGHT)
        self.play(LaggedStartMap(FadeIn, words, shift=0.5 * DOWN, lag_ratio=0.5, run_time=4))
        self.wait()
        self.play(LaggedStartMap(FadeOut, words, shift=RIGHT, lag_ratio=0.1))
        self.wait()
...
class AskAboutCrossAttention(TeacherStudentsScene):
    ...
    def construct(self):
        stds = self.students
        self.play(stds[0].change('hesitant', look_at=stds[1].eyes), stds[1].says('What about\ncross-attention?', bubble_config=dict(buff=0.5), mode='raise_left_hand'), stds[2].change('pondering', look_at=stds[1].eyes), self.teacher.change('well', look_at=stds[1].eyes))
        self.wait(5)
...
class SelfVsCrossFrames(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(screens)
        titles = VGroup(Text('Self-attention', font_size=60), Text('Cross-attention', font_size=60))
...
class ReferenceStraightforwardValueMatrix(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        morty.body.insert_n_curves(100)
        self.play(morty.change('raise_right_hand'), self.change_students('happy', 'well', 'tease', look_at=3 * UR))
        self.wait(3)
        self.play(morty.change('hesitant'), self.change_students('erm', 'hesitant', 'guilty', look_at=3 * UR))
        self.wait(5)
...
class SeekingMatchedParameters(TeacherStudentsScene):
    ...
    def construct(self):
        for pi in self.pi_creatures:
            pi.body.insert_n_curves(100)
        equation = VGroup(Text('# Value params').set_color(RED), Tex('=', font_size=72).rotate(PI / 2), Text('(# Query params) + (# Key params)'))
        ...
        self.play(self.teacher.change('raise_right_hand', equation), FadeIn(equation, UP), self.change_students('erm', 'confused', 'sassy', look_at=equation))
        self.wait(2)
        self.play(self.change_students('pondering', 'confused', 'hesitant', look_at=self.screen))
        self.wait(4)
        self.play(self.change_students('erm', 'confused', 'sassy', look_at=equation))
        self.wait(4)
...
class HeadName(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FlashAround(head, time_width=2, run_time=2), head.animate.set_color(YELLOW))
        self.wait()
...
class DInputAndOutputOfValue(InteractiveScene):
    ...
    def construct(self):
        d_embed = 12288
        in_label, out_label = [VGroup(Text(text), Integer(d_embed)) for text in ['d_input', 'd_output']]
...
class NowRepeatManyTimes(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(self.teacher.says('Now do that about\n10,000 times'), self.change_students('droopy', 'erm', 'well', look_at=self.teacher.eyes))
        self.wait(5)

class ALotToHoldInYouHead(TeacherStudentsScene):

    def construct(self):
        self.play(self.teacher.says("It's a lot to\nhold in your head!", mode='surprised'), self.change_students('confused', 'erm', 'dejected', look_at=self.screen))
        self.wait(5)

class ReactToMHSA(TeacherStudentsScene):

    def construct(self):
        self.play(self.teacher.change('hesitant'), self.change_students('sad', 'confused', 'dejected', look_at=self.screen))
        self.wait(3)
        self.play(self.change_students('guilty', 'maybe', 'erm'))
        self.wait(3)
...
class AskAboutOutput(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(stds[0].change('hesitant', look_at=stds[1].eyes), stds[1].says('What about the\nOutput matrix?', mode='raise_left_hand'), stds[2].change('hesitant', look_at=stds[1].eyes))
        self.play(morty.change('concentrating'))
        self.play(Blink(morty))
        self.wait(5)
...
class OneThirdOfWhatYouNeed(InteractiveScene):
    ...
    def construct(self):
        ...
        lines = VGroup((CubicBezier(all_word.get_corner(UP + v), all_word.get_corner(UP + v) + 0.5 * UP, correction.get_corner(DOWN + v) + 0.5 * DOWN, correction.get_corner(DOWN + v)) for v in [LEFT, RIGHT]))
        lines.set_stroke(RED, 2)
        self.add(title)
        self.wait()
        self.add(all_word, cross)
        self.play(ShowCreation(cross), all_word.animate.set_fill(opacity=0.5))
        self.play(FadeTransform(all_word.copy(), correction), ShowCreation(lines, lag_ratio=0))
        self.wait()
...
class MoreResourcesBelow(InteractiveScene):
    ...
    def construct(self):
        ...
        morty.to_corner(DR)
        self.add(words)
        self.play(LaggedStartMap(GrowArrow, arrows, lag_ratio=0.5), morty.change('thinking', look_at=4 * DOWN))
        self.play(Blink(morty))
        self.wait()
...
class HowAndWhere(TeacherStudentsScene):
    ...
    def construct(self):
        self.remove(self.background)
        self.play(self.students[1].says('How?', mode='raise_left_hand', look_at=self.screen), self.teacher.change('tease'), self.students[2].change('pondering', look_at=self.screen))
        self.play(self.students[0].says('Where?', mode='maybe', look_at=self.screen))
        self.wait(3)
...
class IntroducingMLPs(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        screen = self.screen
        self.play(morty.change('raise_right_hand', screen), self.change_students('pondering', 'confused', 'pondering', look_at=screen))
        self.wait(2)
        ...
        self.play(FadeIn(words[0], UP), self.change_students('tease', 'happy', 'thinking', look_at=words[0]), morty.change('raise_left_hand', words[0]))
        self.play(Write(check, stroke_color=GREEN))
        self.wait(3)
        self.play(FadeIn(words[1], UP), self.change_students('erm', 'confused', 'pondering', words[1]), morty.change('maybe', words[1]))
...
class ReferenceFactStorage(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(morty.change('raise_left_hand'), FadeIn(words, UP), self.change_students('erm', 'maybe', 'sassy', look_at=morty.eyes))
        self.look_at(self.screen)
        self.wait(3)
        self.play(morty.change('tease'))
        self.wait(2)
        self.play(FadeOut(words, DOWN), self.change_students('pondering', 'tease', 'happy', look_at=self.screen))
        self.wait(3)
...
class LookingAtPreview(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        bubble.to_edge(LEFT)
        self.play(self.teacher.change('raise_right_hand'), self.change_students('pondering', 'confused', 'pondering', look_at=bubble))
...
class RefreshersNeverHurt(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.change_students('confused', 'horrified', 'sad', look_at=self.screen))
        self.wait(2)
        self.play(self.teacher.says(TexText("Let's do a\\\\quick refresher"), mode='tease'), self.change_students('pondering', 'hesitant', 'erm', look_at=self.teacher.eyes))
        self.wait(3)
...
class EmbeddingLabel(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(GrowFromCenter(brace), Write(name))
        self.wait()
        self.play(FadeTransform(name, numbers_label), CountInFrom(length, 0, run_time=1.5))
        self.play(FadeIn(gpt3_label, lag_ratio=0.1), GrowFromCenter(arrow))
        self.wait(1.5)
...
class ContrastBetweenSimpleComputationDifficultInterpretation(InteractiveScene):
    ...
    def construct(self):
        ...
        morty.body.insert_n_curves(100)
        items = VGroup(VGroup(Text('Computation'), Checkmark().set_height(0.5).set_color(GREEN)), VGroup(Text('Interpretation'), SVGMobject('warning').set_color(RED).set_height(0.5)))
        for item, vect in zip(items, [LEFT, RIGHT]):
            item.scale(0.75)
            item.arrange(RIGHT)
            item.next_to(morty, UP + vect, buff=0.5)
            item.shift(-1.0 * vect * RIGHT)
        self.play(morty.change('raise_right_hand', items[0]), FadeIn(items[0], UP))
        self.play(Blink(morty))
        self.wait()
        self.play(morty.change('raise_left_hand', items[1]), FadeIn(items[1], UP), items[0].animate.fade(0.5))
        self.play(Blink(morty))
        self.wait()
...
class MakeSomeAssumptions(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.says('We need a\nfew assumptions'), self.change_students('pondering', 'sassy', 'tease', look_at=self.screen))
...
class MovingToSecondToken(InteractiveScene):
    ...
    def construct(self):
        ...
        embeddings = VGroup((NumericEmbedding().scale(0.5).next_to(rect, DOWN, LARGE_BUFF) for rect in rects))
        arrows = VGroup((Arrow(rect, emb, buff=0.1) for rect, emb in zip(rects, embeddings)))
        self.add(arrows)
        self.add(embeddings)
        self.play(Write(rects), LaggedStartMap(GrowArrow, arrows), LaggedStartMap(FadeIn, embeddings, shift=0.5 * DOWN))
        self.wait()
        highlight_rect = SurroundingRectangle(VGroup(rects[:2], embeddings[:2]))
        self.play(ShowCreation(highlight_rect), tokens[2:].animate.fade(0.5), rects[2:].animate.fade(0.5), arrows[2:].animate.fade(0.5), embeddings[2:].animate.fade(0.5))
        self.wait()
        self.play(LaggedStart((ContextAnimation(e2, embeddings[0].get_entries(), path_arc=90 * DEGREES, lag_ratio=0.1, min_stroke_width=2) for e2 in embeddings[1].get_entries()), lag_ratio=0.1), RandomizeMatrixEntries(embeddings[1]), run_time=4)
        self.play(highlight_rect.animate.surround(VGroup(rects[1], embeddings[1]), buff=0))
        self.wait()
...
class WhatAboutBiggerThanOne(TeacherStudentsScene):
    ...
    def construct(self):
        self.screen.set_x(0)
        self.play(self.students[0].change('pondering', self.screen), self.students[1].says("And if it's\nbigger than 1?", mode='sassy', bubble_direction=RIGHT), self.students[2].change('erm', self.screen), self.teacher.change('guilty'))
        self.wait(2)
        self.play(self.teacher.change('maybe'))
        self.play(self.change_students('hesitant', 'sassy', 'angry'))
        self.wait(3)
...
class AskWhy(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for pi in self.pi_creatures:
            pi.body.insert_n_curves(100)
        self.play(self.teacher.change('guilty'), self.students[2].says(Text('Um, Why?', font_size=72), mode='maybe', look_at=self.screen), self.students[0].change('confused', self.screen), self.students[1].change('erm', self.screen))
        self.wait()
        self.play(self.teacher.change('tease'))
        self.wait(2)
...
class GPT3Logo(InteractiveScene):
    ...
    def construct(self):
        ...
        title.arrange(RIGHT)
        title.to_edge(UP)
        self.play(FadeIn(gpt3_label, lag_ratio=0.1), Write(openai_logo, stroke_color=BLUE, stroke_width=0.5))
        self.wait()
...
class AndGate(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(Write(gate), FadeIn(name, lag_ratio=0.1, time_span=(0, 2)), run_time=3)
        self.wait()

class MJFactsAsVectorSum(InteractiveScene):

    def construct(self):
        facts = VGroup((Tex(f'\\overrightarrow{{\\text{{{fact}}}}}') for fact in ['Basketball', 'Chicago Bulls', 'Number 23', 'Born 1963']))
        facts.add(Tex('\\vdots'))
        facts.arrange(DOWN, buff=0.75)
        colors = ['#F88158', '#CE1141', YELLOW, GREY, WHITE]
        for fact, color in zip(facts, colors):
            fact.set_color(color)
        plusses = Tex('+').replicate(len(facts) - 1)
        ...
        for fact, plus in zip(facts[1:], plusses):
            self.play(FadeIn(fact, shift=0.5 * DOWN), Write(plus), run_time=1)
            self.wait()
...
class AskAboutBias(TeacherStudentsScene):
    ...
    def construct(self):
        for pi in self.pi_creatures:
            pi.body.insert_n_curves(100)
        self.play(self.students[0].change('erm', look_at=self.screen), self.students[1].change('confused', look_at=self.screen), self.students[2].says("What's that\nbias doing?", look_at=self.screen, bubble_direction=LEFT))
        self.play(self.teacher.change('maybe'))
        self.wait(4)
        self.play(self.change_students('sassy', 'maybe', 'pondering', look_at=self.screen))
        self.wait(4)
...
class ThatsIt(TeacherStudentsScene):
    ...
    def construct(self):
        for pi in self.pi_creatures:
            pi.body.insert_n_curves(100)
        self.play(self.teacher.says("That's it!", mode='hooray', look_at=self.students), self.change_students('happy', 'thinking', 'well', look_at=self.screen))
        self.wait()
        self.play(self.teacher.debubble(mode='raise_right_hand', look_at=self.screen))
        self.wait()
        self.play(self.change_students('thinking', 'tease', 'happy', look_at=self.screen))
        self.wait(3)
...
class AddTwoMatrixSizes(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(rect), Write(total))
        self.wait()
...
class ReflectOnTwoThings(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.change_students('pondering', 'thinking', 'happy', look_at=screen))
        self.wait(2)
        points = VGroup(Text('Two points of reflection'), Text('1.'), Text('2.'))
        ...
        dials[-2].become(Tex('\\dots').replace(dials[-2], dim_to_match=0))
        ...
        self.play(morty.change('tease', points[0]), self.change_students('erm', 'plain', 'hesitant', look_at=points[0]), Write(points[0], stroke_color=YELLOW_B))
        self.wait(2)
        self.play(Write(points[1][:2]), LaggedStartMap(FadeIn, dials, lag_ratio=0.25), self.change_students('tease', 'plain', 'erm', look_at=points[1]), morty.change('raise_right_hand', points[1]))
        self.wait(2)
        self.play(VFadeIn(points[2]), Rotate(vectors, PI, axis=UP, run_time=8), self.change_students('confused', 'hesitant', 'erm', look_at=points[2]), morty.change('surprised', points[2]))
        self.wait(3)
...
class RotatingVectors(ReflectOnTwoThings):
    ...
    def construct(self):
        ...
        self.play(Rotate(vectors, TAU, axis=UP, run_time=25, rate_func=linear))
...
class AskIfThisIsReal(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(stds[0].says('Is this how ChatGPT stores facts?'), morty.change('well'))
        self.look_at(self.screen)
        self.wait()
        self.play(self.change_students('speaking', 'pondering', 'skeptical', look_at=self.screen))
        self.wait(2)
        self.play(morty.says('Almost certainly\nnot quite...', mode='guilty', bubble_direction=RIGHT), stds[1].change('angry'), stds[2].change('erm'))
        self.wait(4)
...
class SingleNeuronVsMultiple(InteractiveScene):
    ...
    def construct(self):
        radius = 0.1
        layers = VGroup((Dot(radius=radius).get_grid(n, 1, buff=radius) for n in [8, 16, 8]))
        layers.arrange(RIGHT, buff=2.0)
        layers.set_stroke(WHITE, 1)
        for layer in layers:
            for dot in layer:
                dot.set_fill(opacity=random.random())
        connections = VGroup((get_network_connections(layers[i], layers[i + 1]) for i in (0, 1)))
        ...
        self.play(LaggedStartMap(VShowPassingFlash, thick_connections[0], lag_ratio=1 / len(thick_connections[0]), time_width=2.0), layers[1][0].animate.set_fill(opacity=1), layers[1][1:].animate.set_fill(opacity=0), Restore(name, lag_ratio=0.05), run_time=2)
        self.play(ShowCreation(rect))
        self.wait()
        ...
        self.play(network.animate.to_edge(LEFT), TransformFromCopy(network, network_copy), ShowCreation(v_line))
        self.play(LaggedStart(Write(ex, stroke_color=RED), Write(check, stroke_color=GREEN), lag_ratio=0.5))
        self.wait()
...
class WriteSuperposition(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(word, lag_ratio=0.1), LaggedStartMap(VShowPassingFlash, outline, time_width=2, run_time=5, lag_ratio=0.01))
        self.wait()

class JohnsonLindenstraussName(InteractiveScene):

    def construct(self):
        text = VGroup(Text('Johnson–Lindenstrauss\nLemma'), Tex('\\Rightarrow', font_size=120))
        text[0].set_color(RED_B)
        text.arrange(RIGHT, buff=MED_LARGE_BUFF)
        self.play(FadeIn(text[0], lag_ratio=0.1), Write(text[1], run_time=1))
...
class ContrastGPTDimensionSizes(InteractiveScene):
    ...
    def construct(self):
        ...
        model_names = VGroup(Text('GPT-2'), Text('GPT-3'), Text('GPT-4'))
        model_names.scale(1.25)
        model_names.arrange(RIGHT, buff=2.0)
        model_names.set_color(GREY_A)
        arrows = VGroup((Arrow(n1, n2, buff=0.25) for n1, n2 in zip(model_names, model_names[1:])))
        dim_counts = VGroup((Text(f'Model dim: {dim}', font_size=36) for dim in ['768', '12,288', '???']))
        ...
        for name, count, arrow in zip(model_names, dim_counts, arrows):
            self.play(FadeIn(name), FadeIn(count, 0.5 * DOWN), GrowArrow(arrow))
        self.wait()
...
class ReferenceSAP(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(FadeIn(bubble, lag_ratio=0.1), stds[0].change('raise_left_hand'), stds[1].change('confused'), stds[2].change('maybe'), morty.change('tease')))
        self.wait(2)
        self.play(morty.says(TexText("There's nice\\\\research using\\\\Sparse Autoencoders"), mode='hooray'))
        self.play(self.change_students(None, 'pondering', 'erm', look_at=morty.bubble))
        self.wait()
        self.look_at(self.screen)
        self.wait(5)
...
class DetailsNotDiscussed(InteractiveScene):
    ...
    def construct(self):
        ...
        details = VGroup(Text('Tokenization'), Text('Positional encoding'), Text('Layer normalization'), Text('Training'))
        ...
        self.add(title)
        self.play(LaggedStartMap(FadeIn, details, shift=0.25 * DOWN, lag_ratio=0.5), run_time=4)
        self.wait()
        self.play(details[-2][1:].animate.scale(2, about_edge=LEFT).set_color(WHITE), details[-1].animate.shift(0.1 * DOWN), details[:-2].animate.set_opacity(0.5).scale(0.9, about_edge=UL))
        self.wait()
...
class WriteRLHF(InteractiveScene):
    ...
    def construct(self):
        words = VGroup(Text('Reinforcement'), Text('Learning with'), Text('Human'), Text('Feedback'))
        words.scale(1.5)
        words.arrange(DOWN, aligned_edge=LEFT)
        words.set_color(GREY_A)
        self.play(LaggedStartMap(FadeIn, words, shift=0.25 * DOWN, lag_ratio=0.25))
        self.play(*(word[1:].animate.set_opacity(0.75) for word in words))
        self.wait()
...
class ListOfFacts(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(title)
        n_facts = 15
        facts = VGroup((Text(line) for line in Path(DATA_DIR, 'facts.txt').read_text().split('\n')[:n_facts]))
        ...
        vline.scale(0.7)
        lines = VGroup((Line(fact.get_right(), vline.pfp(a), path_arc=interpolate(-20, 20, a) * DEGREES, color=random_bright_color(hue_range=(0.1, 0.2))).insert_n_curves(20).set_stroke(width=(0, 5, 5, 0)) for fact, a in zip(facts, np.linspace(0, 1, len(facts)))))
        self.add(lines)