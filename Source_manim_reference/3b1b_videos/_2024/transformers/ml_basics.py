...
class MLWithinDeepL(InteractiveScene):
    ...
    def construct(self):
        ...
        model_boxes = VGroup(self.get_titled_box('Multilayer Perceptrons', BLUE_D, **kw), self.get_titled_box('Convolutional Neural Networks', BLUE_D, **kw), self.get_titled_box('Transformers', BLUE, **kw))
        for box in model_boxes:
            box.box.set_width(model_boxes.get_width(), stretch=True)
        dots = Tex('\\vdots', font_size=72)
        model_boxes.add(dots)
        model_boxes.arrange(DOWN, buff=0.1)
        dots.shift(0.2 * DOWN)
        transformer_box = model_boxes[2]
        dl_box = self.get_titled_box('Deep Learning', TEAL, font_size=60, y_space=model_boxes.get_height() + 1.0, x_space=2.75, opacity=0.05)
        ...
        self.play(LaggedStart(FadeIn(dl_box, scale=1.2), Restore(transformer_box), *(FadeIn(model_boxes[i]) for i in [0, 1, 3])), lag_ratio=0.75, run_time=2)
        self.wait()
        dl_box.add(model_boxes)
        self.add(dl_box)
        ml_box = self.get_titled_box('Machine Learning', GREEN, opacity=0.1, font_size=72, x_space=6.0, y_space=5.0)
        ...
        self.play(FadeIn(ml_box), MoveToTarget(dl_box), LaggedStartMap(FadeIn, blank_boxes, scale=2.0, lag_ratio=0.5))
        self.wait()
        ...
        self.play(MoveToTarget(ml_box), GrowFromCenter(arrow), TransformFromCopy(ml_box.title['Learn'][0], learn))
        self.play(Restore(learn), FadeIn(words['from data'][0], lag_ratio=0.1, shift=0.2 * RIGHT))
        self.wait()
        self.play(FadeOut(ml_box), FadeOut(arrow))
        self.wait()
        self.clear()
        ml_box.center()
        self.add(ml_box)
        ...
        lines = VGroup(*(Line(ml_box.target[-1].get_corner(RIGHT + v), dl_box.target.get_corner(LEFT + v)) for v in [UP, DOWN]))
        lines.set_stroke(TEAL, 2)
        self.play(MoveToTarget(ml_box), MoveToTarget(dl_box), GrowFromPoint(lines[0], dl_box.get_corner(UR)), GrowFromPoint(lines[1], dl_box.get_corner(DR)), run_time=1.5)
        self.wait()
        network = NeuralNetwork([5, 10, 5])
        network.next_to(dl_box, RIGHT, buff=1.0)
        self.play(FadeIn(network.layers[0]), ShowCreation(network.lines[0], lag_ratio=0.01), FadeIn(network.layers[1], lag_ratio=0.5), run_time=2)
        self.play(ShowCreation(network.lines[1], lag_ratio=0.01), FadeIn(network.layers[2], lag_ratio=0.5), run_time=2)
        for _ in range(6):
            self.play(network.animate.randomize_line_style().randomize_layer_values(), run_time=3, lag_ratio=0.0001)
        ...
        self.play(FadeOut(network, 2 * DOWN), ShowCreation(arrow), FadeInFromPoint(pile_words, dl_box.title.get_center(), path_arc=path_arc), FadeOut(network, DOWN))
        mat_shift = 0.5 * IN + 0.25 * DOWN
        self.play(LaggedStart(*(Succession(FadeIn(matrix, shift=mat_shift), MoveToTarget(matrix)) for matrix in matrices[:-1]), lag_ratio=0.25, run_time=5), Animation(Point()), FadeIn(matrices[-1], shift=mat_shift, time_span=(3.75, 4.75)))
        self.wait()
    ...
    def get_bayes_net_drawing(self):
        ...
        node.shift(2 * DOWN)
        nodes = VGroup((node.copy().shift(x * RIGHT + y * UP) for x, y in [(-1, 0), (1, 0), (-2, 2), (0, 2), (2, 2), (-2, 4), (0, 4)]))
        edge_index_pairs = [(2, 0), (3, 0), (3, 1), (4, 1), (5, 2), (6, 3)]
        edges = VGroup()
        for i1, i2 in edge_index_pairs:
            n1, n2 = (nodes[i1], nodes[i2])
            edge = Arrow(n1.get_center(), n2.get_center(), buff=radius, color=WHITE, stroke_width=3)
            edges.add(edge)
...
class TweakedMachine(InteractiveScene):
    ...
    def construct(self):
        machine = MachineWithDials(dial_config=dict(value_to_color_config=dict(low_negative_color=BLUE_E, high_negative_color=BLUE_B)))
...
class PremiseOfML(InteractiveScene):
    ...
    def construct(self):
        ...
        in_word, out_word = [Text(word).next_to(machine, UP).match_x(mob).shift_onto_screen() for word, mob in [('Input', in_data), ('Output', out_data)]]
        self.play(FadeIn(in_data, lag_ratio=0.001), FadeIn(in_word, 0.5 * UP))
        self.play(FadeOutToPoint(in_data.copy(), machine.get_left(), lag_ratio=0.005, path_arc=-60 * DEGREES))
        self.play(FadeInFromPoint(out_data, machine.get_right(), lag_ratio=0.1, path_arc=60 * DEGREES), FadeIn(out_word, 0.5 * UP))
        self.wait()
        ...
        self.play(MoveToTarget(model_label), ShowIncreasingSubsets(code, run_time=3))
        self.wait()
        param_label = Text('Tunable parameters')
        param_label.next_to(machine, UP)
        param_label.set_color(BLUE)
        self.play(FadeOut(code, 0.25 * DOWN, lag_ratio=0.01), Write(machine.dials, lag_ratio=0.001), FadeOut(model_label, 0.5 * UP), FadeIn(param_label, 0.5 * UP))
        ...
        for n in range(self.n_examples):
            ...
            time_span = (0, 0.35)
            self.play(machine.random_change_animation(run_time=0.5), FadeOut(in_data, time_span=time_span), FadeOut(out_data, time_span=time_span), FadeIn(new_in_data, time_span=time_span), FadeIn(new_out_data, time_span=time_span))
        ...
        self.play(in_data.animate.scale(0.75).shift(up_shift + 0.5 * UP), UpdateFromFunc(out_data, lambda m: m.match_y(in_data)), in_word.animate.shift(up_shift), down_group.animate.shift(down_shift))
        image = in_data
        ...
        self.play(TransformFromCopy(pixels, VGroup(*in_array.get_entries().family_members_with_points()), run_time=2, lag_ratio=0.001), FadeInFromPoint(in_array.get_brackets(), image.get_bottom()), Write(in_array.get_ellipses(), time_span=(1, 2)))
        ...
        self.play(TransformFromCopy(in_array.get_brackets(), vector.get_brackets()), TransformFromCopy(in_array.get_columns()[5], vector.get_columns()[0]), *map(FadeOut, in_array.get_columns()))
        self.wait()
        self.remove(vector)
        self.play(LaggedStart(TransformFromCopy(vector.get_brackets(), in_array.get_brackets()), TransformFromCopy(vector.get_columns()[0], in_array.get_columns()[5]), *(FadeIn(col, shift=col.get_center() - vector.get_center()) for col in in_array.get_columns())))
        self.wait()
        ...
        for layer in tensor:
            for dot in (*layer.get_entries(), *layer.get_ellipses()):
                dot.set_fill(interpolate_color(GREY_C, GREY_B, random.random()), opacity=0.5)
        ...
        self.play(FadeInFromPoint(out_array, machine.get_right(), lag_ratio=0.001), out_data.animate.scale(0.75).fade(0.5).rotate(-PI / 2).next_to(out_array, RIGHT, buff=0.25))
        self.wait()
        weights_label = Text('Weights')
        weights_label.next_to(machine, UP, buff=0.5)
        weights_label.match_color(param_label)
        equiv = Tex('\\Updownarrow')
        ...
        self.play(FadeIn(weights_label, scale=2), param_label.animate.next_to(equiv, UP), Write(equiv))
        self.play(LaggedStart(*(VFadeInThenOut(VGroup(arrow, rect)) for arrow, rect in zip(dial_arrows, dial_rects)), lag_ratio=0.25, run_time=3))
        self.wait()
        ...
        weighted_sum = Tex('w_1 x_1 + w_2 x_2 + w_3 x_3 + \\cdots + w_n x_n', font_size=42)
        weighted_sum.next_to(machine, UP, buff=1.0)
        weight_parts = weighted_sum[re.compile('w_\\d|w_n')]
        weight_parts.set_color(BLUE)
        data_parts = weighted_sum[re.compile('x_\\d|x_n')]
        ...
        self.play(FadeOut(VGroup(param_label, equiv), UP), MoveToTarget(weights_label), machine.dials[8:].animate.fade(0.75), LaggedStart(*(TransformFromCopy(column[n], data_parts[n]) for n in indices)), Group(in_data, in_word).animate.to_edge(LEFT, buff=0.25))
        self.play(Write(weighted_sum['+']), Write(weighted_sum['\\cdots']), LaggedStart(*(FadeTransform(top_dials[n].copy(), weight_parts[n]) for n in indices)), LaggedStartMap(ShowCreation, dial_lines), run_time=1)
        self.wait()
        for x in range(3):
        ...
        self.play(FadeIn(func_wrapper), FadeIn(nl_words, lag_ratio=0.1), ShowCreation(nl_arrow))
        self.wait()
        weights_label.target = weights_label.generate_target()
        weights_label.target.next_to(weighted_sum, UP, buff=1.0)
        dial_lines.target = VGroup(*(Line(weights_label.target, weight_parts[index].get_top(), buff=SMALL_BUFF) for index in indices))
        ...
        dots = Tex('\\dots').next_to(mid_arrow, RIGHT)
        ...
        self.play(LaggedStart(FadeOut(weights_label), FadeOut(dial_lines), FadeOut(nl_words), FadeOut(nl_arrow), FadeOut(col_rect), FadeOut(machine), FadeIn(expr_rect)))
        self.play(TransformFromCopy(in_array.get_brackets(), layer1.get_brackets()), TransformFromCopy(in_arrow, mid_arrow), out_arrow.animate.next_to(dots, RIGHT), Write(dots))
        self.play(TransformFromCopy(expr_rect, x01_rect), ShowCreation(rect_lines, lag_ratio=0), FadeInFromPoint(layer1.elements[0], expr_rect.get_center()))
        ...
        self.play(LaggedStartMap(ShowCreationThenFadeOut, in_subset_rects, lag_ratio=0.02), LaggedStartMap(ShowCreationThenFadeOut, data_part_rects, lag_ratio=0.04), run_time=3)
        self.wait()
        to_fade = VGroup(func_wrapper, expr_rect, rect_lines, x01_rect, weighted_sum)
        self.play(LaggedStartMap(FadeOut, to_fade, run_time=1), in_arrow.animate.scale(0.5, about_edge=LEFT), layer1.animate.rotate(70 * DEGREES, UP).next_to(in_arrow, RIGHT, buff=-0.25), mid_arrow.animate.scale(0.5).next_to(in_arrow, RIGHT, buff=0.75))
        ...
        self.play(TransformFromCopy(layer2_group, layer3_group), VGroup(dots, out_arrow).animate.next_to(layer3_group, RIGHT))
        self.play(LaggedStart(*(dot.animate.shift(0.1 * UP).set_anim_args(rate_func=there_and_back) for dot in dots), lag_ratio=0.25))
        self.wait()
        layers = VGroup(layer1_group, layer2_group, layer3_group, dots)
        self.play(FadeIn(machine, scale=0.8), FadeIn(weights_label, shift=DOWN), ShowCreation(dial_lines, lag_ratio=0.1), FadeIn(weighted_sum, shift=UP), FadeOut(layers, scale=0.8))
        self.wait()
        self.play(machine.random_change_animation())
        self.wait()
        frame = self.frame
        ...
        brace = VGroup(CubicBezier(p0, p0 + 2 * UP, p1 + 2 * DOWN, p1 + 0.1 * DOWN), CubicBezier(p2, p2 + 2 * UP, p3 + 2 * DOWN, p3 + 0.1 * DOWN))
        brace.set_stroke(WHITE, 5)
        self.play(LaggedStart(TransformFromCopy(data_parts, vector.get_columns()[0]), TransformFromCopy(weight_parts, matrix.get_rows()[0]), FadeTransform(weighted_sum, rhs.get_rows()[0]), frame.animate.set_height(10, about_edge=DOWN), FadeOut(in_data, DOWN), FadeOut(out_data, DOWN), in_word.animate.next_to(in_array, UP), FadeIn(matrix, lag_ratio=0.1), ShowCreation(brace, lag_ratio=0), weights_label.animate.set_height(0.5).next_to(matrix, UP, buff=MED_SMALL_BUFF), Uncreate(dial_lines, lag_ratio=0.1), FadeOut(col_rect), machine.dials.animate.restore(), FadeIn(vector.get_brackets()), FadeIn(rhs.get_brackets()), FadeIn(equals), run_time=3, lag_ratio=0.1))
        self.wait()
        ...
        for dial, w_elem in zip(moving_dials.target, w_elems):
            dial.move_to(w_elem)
            dial.scale(2)
        self.play(w_elems.animate.set_opacity(0.25), MoveToTarget(moving_dials, run_time=2))
        self.play(LaggedStart(*(dial.animate_set_value(dial.get_random_value()) for dial in moving_dials), lag_ratio=0.02, run_time=3))
        self.wait()
        self.play(FadeOut(moving_dials), w_elems.animate.set_opacity(1))
        v_rect = SurroundingRectangle(vector.get_entries())
        self.play(ShowCreation(v_rect), ShowCreation(col_rect))
        self.wait()
        self.play(FadeOut(v_rect), FadeOut(col_rect))
        self.wait()
        lhs = VGroup(matrix, vector)
        small_mat_product = Tex('W_{10} v_{11}')
        small_mat_product['W_{10}'].set_color(BLUE)
        ...
        self.play(FadeTransform(mat_prod_group, small_mat_products[0]), Uncreate(brace, lag_ratio=0), FadeOut(machine.dials, run_time=0.5), FadeTransform(weights_label, mv_label), GrowFromPoint(mv_arrows[0], weights_label.get_bottom()), frame.animate.set_height(FRAME_HEIGHT).move_to(DOWN).set_anim_args(time_span=(1, 2)), run_time=2)
        self.wait()
        self.remove(mv_arrows)
        self.play(FadeIn(mv_top_label, UP), mv_label[-1].animate.set_opacity(1), ShowIncreasingSubsets(small_mat_products, rate_func=linear, run_time=12, int_func=np.ceil), ShowSubmobjectsOneByOne(mv_arrows, rate_func=linear, run_time=12, int_func=np.ceil))
        self.remove(mv_arrows)
...
class LinearRegression(InteractiveScene):
    ...
    def construct(self):
        ...
        m = 0.75
        y0 = 1
        data = np.array([(x, y0 + m * x + 0.75 * np.random.normal(0, 1)) for x in np.random.uniform(2, x_max, n_data_points)])
        ...
        def update_line(line):
            ...
            line.put_start_and_end_on(axes.c2p(0, curr_y0), axes.c2p(x_max, curr_y0 + curr_m * x_max))
        line.add_updater(update_line)
        self.play(FadeIn(title, UP), ShowCreation(line))
        self.wait()
        ...
        self.play(LaggedStart(FadeIn(in_labels[0], lag_ratio=0.1), FadeIn(out_labels[0], lag_ratio=0.1), lag_ratio=0.5))
        self.wait()
        self.play(LaggedStart(FadeTransform(*in_labels), FadeTransform(*out_labels), lag_ratio=0.8))
        self.wait()
        self.play(VShowPassingFlash(line.copy().set_stroke(BLUE, 8).scale(1.1).insert_n_curves(100), time_width=1.5, run_time=2))
        self.wait()
        words = ['slope', 'y-intercept']
        value_ranges = [(0, 2, 0.2), (-2, 3, 0.5)]
        m_label, y0_label = labels = VGroup((VGroup(Dial(value_range=value_range), Text(f'{text} = '), DecimalNumber()) for text, value_range in zip(words, value_ranges)))
        ...
        labels.to_edge(UP)
        self.play(FadeOut(title, UP), FadeIn(m_label, UP))
        self.play(m_tracker.animate.set_value(1.5), run_time=2)
        ...
        for n in range(10):
            ...
            self.play(LaggedStart(m_tracker.animate.set_value(new_m), y0_tracker.animate.set_value(new_y0), run_time=1.5, lag_ratio=0.25))
            self.wait(0.5)
...
class ShowGPT3Numbers(InteractiveScene):
    ...
    def construct(self):
        ...
        dial_matrix = MobjectMatrix(pre_dials, *param_shape, ellipses_row=-2, ellipses_col=-2)
        ...
        self.play(FadeIn(weights_text[:-1], time_span=(0, 3)), CountInFrom(weights_count, 0), GrowArrow(weights_arrow, time_span=(0, 3)), LaggedStartMap(FadeIn, pre_dials, scale=3, lag_ratio=0.1), run_time=10)
        self.play(LaggedStart((dial.animate_set_value(dial.get_random_value()) for dial in dials), lag_ratio=1.0 / len(dials), run_time=5))
        self.wait()
        new_name = Text('Total weights: ')
        new_name.move_to(weights_text[0], RIGHT)
        self.play(Transform(weights_text[0]['Total'][0], new_name['Total'][0]), Transform(weights_text[0]['parameters:'][0], new_name['weights:'][0]))
        self.wait()
        ...
        matrices.arrange_in_grid(*mat_grid_shape, v_buff_ratio=0.3, h_buff_ratio=0.2)
        matrices.set_width(FRAME_WIDTH - 1)
        mat_dots = VGroup(*(Tex('\\dots').next_to(mat, RIGHT) for mat in matrices[m - 1::m]), *(Tex('\\vdots').next_to(mat, DOWN) for mat in matrices[-m:]))
        ...
        self.play(Write(mat_text), LaggedStartMap(MoveToTarget, pre_entries, lag_ratio=lag_ratio), TransformFromCopy(dots, mat_dots), *(FadeIn(mat.get_brackets()) for mat in matrices))
        self.play(FadeOut(pre_entries, lag_ratio=0.2 * lag_ratio), FadeIn(all_entries, lag_ratio=0.2 * lag_ratio), run_time=2)
        self.add(matrices)
        self.wait()
        ...
        category_names = VGroup(*map(TexText, ['Embedding', 'Key', 'Query', 'Value$_\\downarrow$', 'Value$_\\uparrow$', 'Up-projection', 'Down-projection', 'Unembedding']))
        ...
        self.play(LaggedStart(MoveToTarget(count_text), title.animate.scale(title_scale_factor).next_to(count_text.target, RIGHT, LARGE_BUFF), FadeOut(weights_arrow), GrowFromCenter(h_line), FadeIn(category_names), LaggedStart(map(MoveToTarget, mat_groups), lag_ratio=0.05), LaggedStart(map(MoveToTarget, mat_dots[:n_groups]), lag_ratio=0.05), LaggedStart(map(FadeOut, mat_dots[n_groups:]), lag_ratio=0.05), FadeOut(matrices[sum(counts):])))
        ...
        for name, line in zip(category_names, h_lines):
            ...
            name.line = line
        v_line = Line(mat_groups.get_corner(DL) + 0.5 * DOWN, mat_groups.get_corner(UL) + 0.25 * UP)
        v_line.shift(SMALL_BUFF * LEFT)
        v_line.match_style(h_lines)
        self.play(Write(h_lines), Write(v_line))
        self.wait()
        const_to_value = {'n_vocab': 50257, 'd_embed': 12288, 'd_query': 128, 'd_value': 128, 'n_heads': 96, 'n_layers': 96, 'n_neurons': 4 * 12288}
        const_lists = [['d_embed', 'n_vocab'], ['d_query', 'd_embed', 'n_heads', 'n_layers'], ['d_query', 'd_embed', 'n_heads', 'n_layers'], ['d_value', 'd_embed', 'n_heads', 'n_layers'], ['d_embed', 'd_value', 'n_heads', 'n_layers'], ['n_neurons', 'd_embed', 'n_layers'], ['d_embed', 'n_neurons', 'n_layers'], ['n_vocab', 'd_embed']]
        ...
        def get_product_expression(category, consts, font_size=30, suffix=None):
            ...
            expr = VGroup()
            expr = Text(' * '.join(consts) + ' = ' + result_str, font_size=font_size)
            ...
            counts = VGroup((Integer(const_to_value[const], font_size=0.8 * font_size) for const in consts))
            ...
            if suffix is not None:
                ...
                result.add(label)
            return result
        product_expressions = VGroup((get_product_expression(category, consts) for category, consts in zip(category_names, const_lists)))
        ...
        self.play(FadeIn(exprs[0]), FadeIn(counts[0], 0.25 * UP))
        self.wait()
        total = Integer(2 * 12288 * 50257)
        ...
        self.play(highlight_category(0, 7), TransformMatchingStrings(exprs[0].copy(), exprs[7]), TransformFromCopy(counts[0][0].copy(), counts[7][1]), TransformFromCopy(counts[0][1].copy(), counts[7][0]), run_time=2)
        self.wait()
        self.play(ShowCreation(lines, lag_ratio=0), FadeIn(total_box), FadeTransform(exprs[0][-11:].copy(), total), FadeTransform(exprs[7][-11:].copy(), total))
        self.wait()
        self.play(FlashAround(weights_count, time_width=1.5, run_time=2))
        self.wait()
        self.play(FadeOut(lines), FadeOut(total_box), FadeOut(total))
        self.wait()
        covered_categories = [0, 7]
        att_categories = [1, 2, 3, 4]
        per_head_factors = [['d_query', 'd_embed'], ['d_query', 'd_embed'], ['d_value', 'd_embed'], ['d_embed', 'd_value']]
        per_head_exprs = VGroup((get_product_expression(name, factors, suffix='per head') for name, factors in zip(category_names[1:5], per_head_factors)))
        per_layer_exprs = VGroup((get_product_expression(name, factors + ['n_heads'], suffix='per layer') for name, factors in zip(category_names[1:5], per_head_factors)))
        ...
        self.play(*(product_expressions[i].animate.set_fill(opacity=0.25, border_width=0) for i in covered_categories), highlight_category(att_categories[0]), FadeIn(per_head_exprs[0], shift=0.5 * RIGHT))
        self.wait()
        self.play(LaggedStartMap(FadeIn, per_head_exprs[1:], shift=0.5 * DOWN, lag_ratio=0.5), highlight_category(*att_categories))
        self.wait()
        self.play(FadeIn(per_head_exprs.sum_box, run_time=3, rate_func=there_and_back_with_pause))
        self.wait()
        self.play(FadeOut(per_head_exprs), FadeIn(per_layer_exprs))
        self.wait()
        self.play(FadeIn(per_layer_exprs.sum_box, run_time=3, rate_func=there_and_back_with_pause))
        self.wait()
        self.play(FadeOut(per_layer_exprs), FadeIn(full_att_exprs))
        self.wait()
        ...
        self.wait()
        self.play(FadeOut(box), FadeOut(full_att_exprs.sum_box))
        self.wait()
        mlp_categories = [5, 6]
        mlp_exprs = product_expressions[5:7]
        per_layer_exprs = VGroup((get_product_expression(category_names[i], const_lists[i][:2], suffix='per layer') for i in mlp_categories))
        self.play(full_att_exprs.animate.set_fill(opacity=0.25, border_width=0), highlight_category(*mlp_categories))
        self.wait()
        self.play(FadeIn(per_layer_exprs[0]))
        self.wait()
        self.play(TransformMatchingStrings(per_layer_exprs[0][0].copy(), per_layer_exprs[1][0]), TransformFromCopy(per_layer_exprs[0][1][0], per_layer_exprs[1][1][1]), TransformFromCopy(per_layer_exprs[0][1][1], per_layer_exprs[1][1][0]), TransformFromCopy(per_layer_exprs[0][2], per_layer_exprs[1][2]), run_time=1)
        self.wait()
        self.play(FadeOut(per_layer_exprs), FadeIn(mlp_exprs))
        self.wait()
        ...
        self.play(FadeIn(rhs_rect), FadeIn(mlp_total_rect), FadeTransform(mlp_exprs[0][0].rhs.copy(), mlp_total), FadeTransform(mlp_exprs[1][0].rhs.copy(), mlp_total))
        self.wait()
        self.play(category_names.animate.set_fill(opacity=1, border_width=0.5), product_expressions.animate.set_fill(opacity=1, border_width=0.5))
        ...
        self.play(FadeOut(mlp_total_rect, RIGHT), FadeOut(mlp_total, RIGHT), ReplacementTransform(rhs_rect, all_rhss_rect), MoveToTarget(all_rhss))
        self.wait()
        self.play(LaggedStart(h_line.animate.scale(0.5, about_edge=LEFT), weights_text.animate.arrange(DOWN).scale(1.5).next_to(all_rhss_rect, UP), FadeOut(mat_text, LEFT), title.animate.to_edge(LEFT, buff=2.5), lag_ratio=0.2, run_time=2))
        self.wait()
...
class DistinguishWeightsAndData(InteractiveScene):
    ...
    def construct(self):
        ...
        matrices = VGroup((WeightMatrix(shape=(6, 8), ellipses_row=None, ellipses_col=None) for n in range(4)))
        ...
        self.play(LaggedStart((Transform(num, num.states[2], path_arc=PI) for num in all_nums), lag_ratio=lag_ratio, run_time=3))
        self.wait()
        self.play(LaggedStart((LaggedStart((Transform(num, num.states[1]) for num in group), lag_ratio=lag_ratio, run_time=2) for group in [mat_nums, vec_nums]), lag_ratio=0.5), ShowCreation(v_line))
        self.play(Write(weights_title), LaggedStart((Transform(num, num.states[0]) for num in mat_nums), lag_ratio=lag_ratio, run_time=2), FadeIn(mat_braces, lag_ratio=0.1, time_span=(1, 2)))
        self.play(Write(data_title), LaggedStart((Transform(num, num.states[0]) for num in vec_nums), lag_ratio=lag_ratio, run_time=2), FadeIn(vec_braces, lag_ratio=0.1, time_span=(1, 2)))
        self.wait()
        subtitles = VGroup(Text('What defines the model', font_size=40), Text('What the model processes', font_size=40))
        for subtitle, title, group in zip(subtitles, titles, tensors):
            subtitle.next_to(title, DOWN)
            self.play(FadeIn(subtitle, lag_ratio=0.1), group.animate.next_to(subtitle, DOWN, buff=0.5))
            self.wait()
...
class SoftmaxBreakdown(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, probs, shift=0.25 * DOWN, lag_ratio=0.3), FadeIn(seq_title), run_time=1)
        self.wait()
        self.play(Restore(chart, lag_ratio=0.1), MoveToTarget(probs), FadeTransform(seq_title, prob_title))
        self.wait()
        self.play(LaggedStartMap(FadeIn, words))
        self.wait()
        index = 3
        ...
        self.wait()
        self.play(FadeIn(one_line, time_span=(0, 1)), bar.animate.set_height(max_height, about_edge=DOWN, stretch=True), run_time=2)
        self.play(bar.animate.set_height(0.0001, about_edge=DOWN, stretch=True), run_time=2)
        ...
        rhs = DecimalNumber(1.0)
        rhs.next_to(equals, RIGHT)
        self.play(TransformFromCopy(probs, prob_copies), Write(plusses), Write(equals), FadeOut(one_line))
        ...
        matrix = WeightMatrix(values=matrix_values, ellipses_row=None, ellipses_col=None, num_decimal_places=2)
        for mob in (matrix, vector):
            mob.set_height(4)
        vector.to_edge(UP).set_x(2.5)
        matrix.next_to(vector, LEFT)
        self.play(LaggedStart(chart_group.animate.scale(0.35).to_corner(DL), FadeOut(sum_group, UP), FadeIn(matrix, UP), FadeIn(vector, UP)))
        ...
        sum_sym = Tex('\\sum', font_size=36).next_to(sum_arrow, LEFT)
        sum_num = DecimalNumber(sum((e.get_value() for e in rhs.get_entries())))
        sum_num.next_to(sum_arrow, DOWN)
        self.play(ShowCreation(rhs_rect), FadeIn(rhs_words))
        self.wait()
        self.play(ReplacementTransform(VGroup(rhs_rect), neg_rects), LaggedStart(*(FadeIn(rect.word, 0.5 * RIGHT) for rect in neg_rects)))
        self.wait()
        self.play(ReplacementTransform(neg_rects, gt1_rects), FadeTransformPieces(neg_words, gt1_words))
        self.wait()
        self.play(LaggedStart(FadeOut(rhs_words), FadeOut(gt1_rects), FadeOut(gt1_words)), GrowArrow(sum_arrow), FadeIn(sum_num, DOWN), FadeIn(sum_sym))
        self.wait()
        ...
        for bar, entry in zip(bars, output.get_entries()):
            ...
            bar.next_to(output)
            bar.match_y(entry)
        self.play(LaggedStart(FadeOut(matrix, 2 * LEFT), FadeOut(vector, 3 * LEFT), FadeOut(eq, 3.5 * LEFT), FadeOut(chart_group, DL), GrowArrow(in_arrow), FadeIn(softmax_box, RIGHT), FadeIn(softmax_label, RIGHT), MoveToTarget(rhs), GrowArrow(out_arrow), FadeIn(output, RIGHT), TransformFromCopy(chart.bars, bars)), lag_ratio=0.2, run_time=2)
        self.wait()
        ...
        self.wait()
        variables = VGroup(*(Tex(f'x_{{{n}}}', font_size=48).move_to(elem) for n, elem in enumerate(rhs_entries, start=1)))
        self.remove(rhs_entries)
        self.play(LaggedStart(*(TransformFromCopy(entry, variable, path_arc=PI / 2) for entry, variable in zip(rhs_entries, variables)), lag_ratio=0.1, run_time=1.0))
        self.wait()
        ...
        gt0s = VGroup((Tex('> 0').next_to(exp_part, aligned_edge=DOWN) for exp_part in exp_parts))
        self.play(softmax_label.animate.next_to(softmax_box, UP, buff=0.15), LaggedStart(*(TransformMatchingStrings(var.copy(), exp_part) for var, exp_part in zip(variables, exp_parts)), run_time=1, lag_ratio=0.01))
        self.play(LaggedStartMap(FadeIn, gt0s, shift=0.5 * RIGHT, lag_ratio=0.25, run_time=1))
        self.wait()
        self.play(FadeOut(gt0s))
        exp_sum = Tex('\\sum_{n=0}^{N-1} e^{x_{n}}', font_size=42)
        exp_sum['e^{x_{n}}'].scale(1.5, about_edge=LEFT)
        ...
        lines.set_stroke(TEAL, 2)
        self.play(LaggedStart(*(FadeTransform(exp_part.copy(), exp_sum) for exp_part in exp_parts), lag_ratio=0.01), LaggedStartMap(ShowCreation, lines, lag_ratio=0.01), run_time=1)
        self.wait()
        ...
        self.remove(exp_sum)
        self.play(exp_parts.animate.next_to(lil_denoms, LEFT, buff=0), LaggedStart(*(FadeTransform(exp_sum.copy(), denom) for denom in lil_denoms), lag_ratio=0.01))
        self.wait()
        ...
        self.play(softmax_label.animate.next_to(softmax_box.target, UP), MoveToTarget(softmax_box), MoveToTarget(sm_terms), MoveToTarget(full_output), MoveToTarget(rhs), FadeTransform(out_arrow, equals), in_arrow.animate.become(Arrow(rhs.target, softmax_box.target).match_style(in_arrow)))
        self.wait()
        ...
        def update_bars(bars):
            for bar, entry in zip(bars, output_entries):
                width = max(bar_width_ratio * entry.get_value(), 0.001)
        ...
        for index, value in index_value_pairs:
            ...
            self.play(ChangeDecimalToValue(entry, value), FadeIn(rect, time_span=(0, 1)), run_time=4)
        ...
        t_line = NumberLine((0, 10, 0.2), tick_size=0.025, big_tick_spacing=1, longer_tick_multiple=2.0, width=4)
        ...
        self.play(frame.animate.move_to(0.75 * UP), TransformMatchingStrings(softmax_label, new_title), FadeIn(t_line), FadeIn(t_tri), FadeIn(t_label), run_time=1)
        template = Tex('e^{x_{0} / T} / \\sum_{n=0}^{N - 1} e^{x_n / T}')
        template['T'].set_color(temp_color)
        template['/'][1].scale(1.9, about_edge=LEFT)
        template['\\sum_{n=0}^{N - 1}'][0].scale(0.7, about_edge=RIGHT)
        ...
        self.play(LaggedStart(*(FadeTransform(old_term, new_term) for old_term, new_term in zip(sm_terms, new_sm_terms))), LaggedStart(*(TransformFromCopy(t_label[0], t_mob[0]) for t_mob in all_Ts)))
        self.wait()
        for value in [4, 10, 2]:
            self.play(temp_tracker.animate.set_value(value), run_time=8)
            self.wait()
        self.play(temp_tracker.animate.set_value(0), run_time=3)
        max_rects = VGroup(SurroundingRectangle(rhs.get_entries()[4]), SurroundingRectangle(VGroup(output.get_entries()[4], bars[4])))
        self.play(LaggedStartMap(ShowCreationThenFadeOut, max_rects))
        self.wait()
        for value in [5, 1, 7]:
            self.play(temp_tracker.animate.set_value(value), run_time=4)
            self.wait()
        prob_arrows, logit_arrows = (VGroup(*(Vector(-vect).next_to(entry, vect, buff=0.25) for entry in matrix.get_entries())) for matrix, vect in [(output, RIGHT), (rhs, LEFT)])
        ...
        for arrows, word, rects in [prob_group, logit_group]:
            self.play(t_line.animate.set_y(3.35), Write(word), Write(rects, stroke_width=5, stroke_color=rects[0].get_stroke_color(), lag_ratio=0.3, run_time=3))
            self.wait()
...
class CostFunction(InteractiveScene):
    ...
    def construct(self):
        ...
        graph.set_color(RED)
        expr = Tex('\\text{Cost} = -\\log(p)', font_size=60)
        ...
        bar_chart = next_token_bar_chart(tokens[:8], probs[:8], width_100p=7.0, bar_space_factor=1.0, use_percent=False)
        ...
        self.play(ShowCreation(graph, run_time=3), Write(expr, run_time=2))
        self.wait()
