...
class HighLevelNetworkFlow(InteractiveScene):
    ...
    possible_next_tokens = [('Mountain', 0.706), ('the', 0.128), ('San', 0.078), ('Seattle', 0.006), ('New', 0.003), ('Palo', 0.002), ('Google', 0.002), ('southern', 0.002)]
    ...
    def construct(self):
        ...
        self.progress_through_mlp_block(show_one_by_one=True, sideview_orientation=(-78, -10, 0))
    ...
    def get_embedding_array(self, shape=(9, 10), height=4, dots_index=-4, buff_ratio=0.4, bracket_color=GREY_B, backstroke_width=3, add_background_rectangle=False):
        return EmbeddingArray(shape=shape, height=height, dots_index=dots_index, buff_ratio=buff_ratio, bracket_color=bracket_color, backstroke_width=backstroke_width, add_background_rectangle=add_background_rectangle)
    ...
    def get_block(self, layer, depth=2.0, buff=1.0, size_buff=1.0, color=GREY_E, opacity=0.75, shading=(0.25, 0.1, 0.0), title='Attention', title_font_size=96, title_backstroke_width=3):
    ...
    def show_initial_text_embedding(self, word_scale_factor=0.6, bump_first=False):
        ...
        self.wait()
        self.play(FadeOut(big_rect), LaggedStart(*(DrawBorderThenFill(rect) for rect in rects[:-1]), lag_ratio=0.02), LaggedStart(*(token.animate.match_color(rect) for token, rect in zip(words, rects))), FadeOut(arrow))
        self.wait()
        ...
        self.play(self.frame.animate.move_to(1.0 * UP), MoveToTarget(blocks), q_group.animate.scale(word_scale_factor).next_to(blocks.target, RIGHT, aligned_edge=UP), LaggedStartMap(FadeIn, vectors, shift=0.5 * DOWN), LaggedStartMap(GrowFromCenter, arrows), Write(layer.dots))
        self.wait()
    ...
    def progress_through_attention_block(self, depth=2.0, target_orientation=(-40, -15, 0), target_frame_height=14, target_frame_x=-2, target_frame_y=0, attention_anim_run_time=5, label_text='Attention'):
        layer = self.layers[-1]
        ...
        self.play(MoveToTarget(self.frame), LaggedStart(layer.animate.set_opacity(0.25), FadeIn(block), TransformFromCopy(layer, new_layer), (self.blocks[-1].title if len(self.blocks) > 0 else VGroup()).animate.set_opacity(0.25), lag_ratio=0.3), self.token_arrows.animate.set_opacity(0.1), run_time=2)
        self.play_simple_attention_animation(new_layer, run_time=attention_anim_run_time, added_anims=[self.frame.animate.reorient(0, -15, 0, (-0.07, 0.45, 2.03), 9.25)])
        self.add(*block.body, block.title, new_layer)
        new_z = block.get_z() + z_diff
        self.play(block.body[0].animate.set_opacity(block_opacity), new_layer.animate.set_z(new_z), self.frame.animate.set_z(new_z), Restore(layer))
        ...
        if False:
            ...
            self.play(LaggedStart(self.frame.animate.reorient(0, -14, 0, (-0.18, 0.73, 3.26), 8.31), block.body.animate.set_color(BLACK).set_shading(0.1, 0.1, 0.5), block.title.animate.set_opacity(0.1), new_layer.animate.fade(0.75).set_stroke(width=0), self.token_blocks[index].animate.scale(2, about_edge=DOWN), self.token_blocks[:index].animate.shift(0.35 * LEFT), FadeIn(highlight), ShowCreation(rect), FadeIn(new_arrow, scale=4), run_time=3))
            self.wait()
        ...
        for _ in range(3):
            for n, e1 in enumerate(layer.embeddings):
                ...
                for e2 in layer.embeddings[n + 1:]:
                    ...
                    arc_group.add(Line(e1.get_top(), e2.get_top(), path_arc=sign * PI / 3, stroke_color=random_bright_color(hue_range=(0.1, 0.3)), stroke_width=5 * random.random() ** 5))
                arc_group.shuffle()
                if len(arc_group) > 0:
                    arc_groups.add(arc_group)
        self.play(LaggedStart(*(AnimationGroup(LaggedStartMap(VShowPassingFlash, arc_group.copy(), time_width=2, lag_ratio=0.15), LaggedStartMap(ShowCreationThenFadeOut, arc_group, lag_ratio=0.15)) for arc_group in arc_groups), lag_ratio=0.0), LaggedStartMap(RandomizeMatrixEntries, layer.embeddings, lag_ratio=0.0), *added_anims, run_time=run_time)
        self.add(layer)

    def progress_through_mlp_block(self, n_neurons=20, depth=4.0, buff=1.0, dot_buff_ratio=0.2, neuron_color=GREY_C, neuron_shading=(0.25, 0.75, 0.2), sideview_orientation=(-60, -5, 0), final_orientation=(-51, -18, 0), show_one_by_one=False, label_text='Feedforward', title_font_size=90):
        layer = self.layers[-1]
        block = self.get_block(layer, depth=depth, buff=buff, size_buff=1.0, title=label_text, title_font_size=title_font_size)
        ...
        for embedding in layer.embeddings:
            ...
            weights = VGroup(*(Line(p1, p2, buff=0.1, stroke_width=3, stroke_opacity=random.random(), stroke_color=value_to_color(random.uniform(-10, 10))) for points1, points2 in [[l1_points, l2_points], [l2_points, l3_points]] for p1, p2 in it.product(points1, points2) if random.random() < 0.2))
        ...
        if show_one_by_one:
            ...
            self.play(self.blocks[-1].title.animate.set_opacity(0.25), FadeIn(block.title, time_span=(0, 1)), self.frame.animate.reorient(*sideview_orientation), Write(networks[index][1]), FadeIn(networks[index][0]), FadeTransform(emb_pairs[index][0].copy(), emb_pairs[index][1]), run_time=3)
            lag_kw = dict(lag_ratio=0.5, run_time=9)
            self.remove(block.title)
            self.play(LaggedStart(*(FadeTransform(e1.copy(), e2) for e1, e2 in [*emb_pairs[:index], *emb_pairs[index + 1:]]), **lag_kw), LaggedStart(*(Write(network[1]) for network in [*networks[:index], *networks[index + 1:]]), **lag_kw), LaggedStart(*(FadeIn(network[0]) for network in [*networks[:index], *networks[index + 1:]]), **lag_kw), self.frame.animate.reorient(0, -37, 0, (-1.08, 2.29, 7.99), 12.27), block.title.animate.rotate(PI / 2, UP), run_time=15)
            self.remove(networks)
            self.add(neurons, connections, block.body, block.title, new_layer)
            self.play(FadeIn(block.body), FadeIn(new_layer), FadeOut(new_layer.embeddings.copy()), self.frame.animate.reorient(-48, -12, 0).match_z(new_layer), run_time=2)
        else:
            self.play(FadeIn(block.title, time_span=(0, 1)), self.blocks[-1].title.animate.set_opacity(0.25), self.frame.animate.reorient(*sideview_orientation), FadeIn(neurons, time_span=(0, 1)), Write(connections, stroke_width=3), TransformFromCopy(layer, new_layer), run_time=3)
            self.add(block.body, block.title, new_layer)
            self.play(self.frame.animate.reorient(*final_orientation).match_z(new_layer), FadeIn(block.body), run_time=2)
    ...
    def mention_repetitions(self, depth=8):
        ...
        self.play(frame.animate.reorient(-57, -8, 0, (-6.59, 1.2, 57.84), 30), FadeIn(thin_blocks, lag_ratio=0.1), GrowFromCenter(brace), Write(brace_text, time_span=(1, 2)), Write(dots), run_time=3)
    ...
    def focus_on_last_layer(self):
        ...
        self.play(FadeIn(rect), MoveToTarget(self.rep_label), *map(FadeOut, [block.title for block in self.blocks if hasattr(block, 'title')]), self.frame.animate.reorient(-3, -12, 0, target_frame_center, 12.0), run_time=3)
    ...
    def show_unembedding(self):
        ...
        for bar, word, value in zip(bars, words, dist):
            percentage = DecimalNumber(100 * value, num_decimal_places=2, unit='%', font_size=label_font_size)
        ...
        arrow.add_cubic_bezier_curve_to(rect.get_top() + UP, rect.get_top() + UR, rect.get_top() + 2 * RIGHT)
        arrow.add_line_to(brace.get_left() + 0.1 * LEFT)
        arrow.make_smooth(approx=False)
        arrow.add_tip()
        arrow.set_color(YELLOW)
        self.play(ShowCreation(rect))
        self.play(ShowCreation(arrow), GrowFromCenter(brace), LaggedStartMap(FadeIn, bars, shift=DOWN))
        self.wait()
        self.play(self.frame.animate.reorient(-5, -8, 0, (2.99, 2.67, 71.6), 13.6), run_time=8)
...
class SimplifiedFlow(HighLevelNetworkFlow):
    ...
    def show_simple_flow(self, x_range, orientation=None):
        ...
        for x in x_range:
            self.progress_through_attention_block(target_orientation=orientation, target_frame_x=self.target_frame_x, attention_anim_run_time=self.attention_anim_run_time)
            self.progress_through_mlp_block(sideview_orientation=orientation, final_orientation=orientation)
...
class MentionContextSizeAndUnembedding(SimplifiedFlow):
    ...
    def cycle_through_embeddings(self):
        ...
        rect_groups = VGroup(*(VGroup(*(BackgroundRectangle(mob, buff=0.2 if mob in arrows else 0.1) for mob in tup)) for tup in zip(blocks, arrows, embeddings)))
    ...
    def show_context_size(self):
        ...
        layer = self.layers[-1]
        self.play(frame.animate.reorient(-27, -18, 0, (-0.84, -0.36, 18.15), 9.0), FadeOut(block_titles, lag_ratio=0.01), run_time=2)
        font_size = 72
        ...
        self.play(GrowFromCenter(brace), FadeIn(label, 0.5 * DOWN))
        self.wait()
        self.play(MoveToTarget(label), Write(rhs))
        self.wait()
        self.play(FadeOut(layer.brackets), GrowFromCenter(dim_brace), FadeIn(dim_label, 0.5 * LEFT))
        self.wait(3)
        self.play(Restore(frame, run_time=3), FadeIn(block_titles, time_span=(2, 3), lag_ratio=0.01), FadeIn(layer.brackets), LaggedStartMap(FadeOut, VGroup(brace, label, rhs, dim_brace, dim_label), shift=DOWN))
    ...
    def show_desired_output(self):
        ...
        self.wait()
        layer = self.layers[-1]
        word_strs = ['...', 'Snake', 'Snape', 'Snare', '...', 'Treks', 'Trelawney', 'Trellis', '...', 'Quirky', 'Quirrell', 'Quirt', '...']
        words = VGroup()
        dots = VGroup()
        ...
        self.play(LaggedStartMap(FadeIn, words, shift=0.1 * DOWN), LaggedStartMap(FadeIn, bars), LaggedStartMap(FadeIn, probs, shift=0.2 * LEFT), GrowFromCenter(brace), GrowArrow(arrow))
        self.wait()
    ...
    def show_unembedding_matrix(self, vector_index=-1):
        ...
        self.play(FadeOut(self.phrase, UP), MoveToTarget(prob_group), frame.animate.reorient(0, 1, 0, (4.98, 3.34, 30.08), 12.0), run_time=2)
        layer = self.layers[-1]
        vector = layer.embeddings[vector_index].copy()
        matrix = WeightMatrix(shape=(15, vector.shape[0]), ellipses_row=8)
        matrix.set_height(6)
        ...
        last_vect_arrow = Arrow(rect.get_top(), vector.target.get_bottom())
        last_vect_arrow.set_color(YELLOW)
        self.play(FadeIn(matrix, scale=0.8, shift=DR), *map(FadeOut, [self.token_blocks, self.token_arrows, self.final_word_question]))
        self.play(ShowCreation(rect))
        ...
        brace_label = brace.get_tex('\\sim 50k \\text{ values}', font_size=60, buff=0.25)
        self.play(GrowFromCenter(brace), FadeIn(brace_label, lag_ratio=0.1))
        brace_group = VGroup(brace, brace_label)
        word_strs = ['aah', 'aardvark', 'aardwolf', 'aargh', 'ab', 'aback', 'abacterial', 'abacus', '...', 'zygote', 'zygotic', 'zyme', 'zymogen', 'zymosis', 'zzz']
        ...
        for word, entry in zip(words, rhs):
            ...
            word.match_y(entry)
        self.play(LaggedStartMap(FadeIn, words, shift=0.5 * RIGHT), brace_group.animate.next_to(words, RIGHT))
        self.wait()
        ...
        self.play(LaggedStart(FadeOut(brace_group), DrawBorderThenFill(big_rect), ShowCreation(softmax_arrow), FadeIn(softmax_label, lag_ratio=0.1)), MoveToTarget(prob_group, time_span=(1, 2)))
        ...
        self.play(frame.animate.reorient(-1, 0, 0, (1.69, 1.57, 29.79), 15.88), Write(question), run_time=2)
        last_rect = VGroup()
        for rect, arrow in zip(rects, question_arrows):
            self.play(FadeOut(last_rect), FadeIn(rect), FadeIn(arrow))
            last_rect = rect
        self.play(FadeOut(last_rect))
        self.wait()
        self.play(FadeOut(question_arrows, lag_ratio=0.1), FadeOut(question, lag_ratio=0.1), frame.animate.reorient(0, 1, 0, (4.27, 3.46, 29.83), 12.86), run_time=2)
        self.wait()
        ...
        self.play(Write(matrix_rect, stroke_width=5, stroke_color=BLUE), Write(name, run_time=2), frame.animate.reorient(0, 1, 0, (4.24, 3.15, 29.81), 13.23))
        self.wait()
        ...
        top_equation = VGroup(Text('Total parameters = '), Integer(vocab_count.get_value()), Tex('\\times'), Integer(dim_count.get_value()), Tex('='), Integer(vocab_count.get_value() * dim_count.get_value()).set_color(YELLOW))
        ...
        self.play(label.animate.move_to(matrix), FadeOut(name, lag_ratio=0.1), entries.animate.set_fill(opacity=0.5, border_width=0))
        self.add(row_rects, label)
        self.play(GrowFromCenter(left_brace, time_span=(0, 1)), CountInFrom(vocab_count), LaggedStartMap(VFadeInThenOut, row_rects, lag_ratio=0.2), run_time=2.5)
        self.add(col_rects, label)
        self.play(GrowFromCenter(top_brace, time_span=(0, 1)), CountInFrom(dim_count), LaggedStartMap(VFadeInThenOut, col_rects, lag_ratio=0.2), frame.animate.reorient(0, 0, 0, (5.66, 4.08, 29.89), 14.32), run_time=2.5)
        self.wait()
        self.play(LaggedStart(Write(top_equation[0:6:2]), TransformFromCopy(vocab_count, top_equation[1]), TransformFromCopy(dim_count, top_equation[3]), frame.animate.reorient(0, -1, 0, (4.15, 5.07, 29.75), 17.21), run_time=3))
...
class FlowForMLPIntroReview(SimplifiedFlow):
    ...
    possible_next_tokens = [('stronger', 0.906), ('stranger', 0.028), ('more', 0.006), ('weaker', 0.003), ('...', 0.003), ('Strong', 0.002), ('wish', 0.002), ('STR', 0.002)]
...
class FlowForCHM(SimplifiedFlow):
    ...
    def progress_through_mlp_block(self, *args, **kwargs):
        kwargs.update(label_text='Feed Forward', title_font_size=92)
...
class TextPassageIntro(InteractiveScene):
    ...
    def construct(self):
        ...
        passage = TexText(passage_str, alignment='', additional_preamble='\\tiny')
        ...
        self.wait()
        self.play(Restore(word, time_span=(0, 0.5), remover=True), ShowIncreasingSubsets(section), run_time=1)
        self.play(ContextAnimation(word, section))
        self.wait()
        self.play(Restore(section, remover=True), ShowIncreasingSubsets(VGroup(*passage[:lh], *passage[rh:])))
        self.add(passage)
        self.play(ContextAnimation(passage[lh:rh][word_lh:word_rh], VGroup(*passage[0:11], *passage[211:217]), lag_ratio=0.01))
        self.wait()
        ...
        dots = short_text['...'][0]
        lh, rh = (31, 2474)
        self.play(FadeTransformPieces(passage[:lh], short_text[start][0]), ReplacementTransform(passage[lh:rh], dots), FadeTransformPieces(passage[rh:], short_text[end][0]), run_time=3)
        self.add(short_text)
        self.wait()
...
class MoleExample1(HighLevelNetworkFlow):
    ...
    def construct(self):
        phrase_strs = ['American shrew mole', 'One mole of carbon dioxide', 'Take a biopsy of the mole']
        ...
        self.wait()
        for i in [1, 2]:
            self.play(Transform(phrases[i - 1]['mole'].copy(), phrases[i]['mole'].copy(), remover=True), FadeIn(phrases[i], lag_ratio=0.1))
            self.wait()
        images = Group(ImageMobject('ShrewMole').set_height(1), Tex('6.02 \\times 10^{23}').set_color(TEAL), ImageMobject('LipMole').set_height(1))
        braces = VGroup()
        mole_words = VGroup()
        ...
        self.play(LaggedStartMap(GrowFromCenter, braces, lag_ratio=0.5), LaggedStartMap(FadeIn, images, shift=UP, lag_ratio=0.5), mole_words.animate.set_color(YELLOW).set_anim_args(lag_ratio=0.1))
        self.wait()
        word_groups = VGroup()
        ...
        self.play(FadeIn(word_groups), LaggedStartMap(FadeOut, braces, shift=0.25 * DOWN, lag_ratio=0.25), LaggedStartMap(FadeOut, images, shift=0.25 * DOWN, lag_ratio=0.25), run_time=1)
        self.remove(phrases)
        self.wait()
        ...
        self.play(LaggedStartMap(MoveToTarget, word_groups), ShowCreation(v_lines, lag_ratio=0.5, time_span=(1, 2)))
        embs = VGroup()
        arrows = VGroup()
        ...
        self.play(LaggedStartMap(GrowArrow, arrows), LaggedStartMap(FadeIn, embs, shift=0.25 * DOWN))
        self.wait()
        self.play(LaggedStartMap(ShowCreation, mole_vect_rects), VGroup((arrows[j] for j in non_mole_indices)).animate.set_fill(opacity=0.5), VGroup((embs[j] for j in non_mole_indices)).animate.set_fill(opacity=0.5))
        self.wait()
        ...
        self.play(FadeOut(v_lines, time_span=(0, 1)), MoveToTarget(highlighted_group, lag_ratio=0.0005), *(FadeOut(fg, shift=fg.get_center() - highlighted_group.get_center() + 2 * DOWN, lag_ratio=0.001) for fg in fade_groups), run_time=2)
        self.wait()
