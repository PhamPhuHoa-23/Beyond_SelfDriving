...
class AnnotateNextWord(SimpleAutogregression):
    ...
    def construct(self):
        ...
        highlight = Union(SurroundingRectangle(text_mob['in its native']), SurroundingRectangle(text_mob['Behold, a wild pi creature, foraging']))
...
class AthleteCompletion(SimpleAutogregression):
    ...
    def construct(self):
        ...
        self.play(LaggedStart((TransformFromCopy(last_block_copy.copy().set_opacity(0), block) for block in machine), lag_ratio=0.05), Write(dials), Write(llm_title), frame.animate.reorient(0, 0, 0), run_time=3)
        ...
        self.play(LaggedStart((Succession(FadeIn(fact_mob), fact_mob.animate.set_opacity(0).move_to(machine.get_center())) for fact_mob in fact_mobs), lag_ratio=0.05, run_time=8))
        ...
        self.play(GrowArrow(in_arrow), Transform(full_input.copy(), full_input.copy().scale(0.5).set_opacity(0).move_to(machine.get_top())))
        self.play(frame.animate.reorient(-14, -2, 0, (1.83, 0.07, -0.38), 8.63), LaggedStart((block.animate.set_color(TEAL).set_anim_args(rate_func=there_and_back) for block in machine[:-1]), lag_ratio=0.1, run_time=1))
        ...
        for _ in range(5):
            self.play(LaggedStart((dial.animate_set_value(dial.get_random_value()) for dial in dials), lag_ratio=0.25 / len(dials), run_time=1))
...
class ThatWhichDoesNotKillMe(SimpleAutogregression):
    ...
    def construct(self):
        text_mob, next_word_line, machine = self.init_text_and_machine()
        machine.set_x(0)
        text_mob = self.new_selection_cycle(text_mob, next_word_line, machine, quick=False, skip_anims=False)