...
class LastTwoChapters(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, thumbnails, shift=UP, lag_ratio=0.5))
        self.wait()
        ...
        self.play(frame.animate.reorient(-32, 0, 0, (0.56, 2.48, 0.32), 12.75), thumbnails.animate.scale(0.5).arrange(RIGHT, buff=2.0).to_edge(UP, buff=0.25), LaggedStartMap(FadeIn, blocks, shift=0.25 * UP, scale=1.5, lag_ratio=0.1), FadeIn(trans_title, UP))
        self.wait()
        ...
        self.play(frame.animate.reorient(-3, -2, 0, (0.23, 2.57, 0.3), 12.75), trans_title.animate.shift(2 * UP), att_blocks.animate.shift(4 * LEFT), mlp_blocks.animate.shift(4 * RIGHT))
        ...
        self.play(FadeIn(att_icon, lag_ratio=0.001), FadeIn(att_title, UP), trans_title.animate.scale(0.75).set_opacity(0.5))
        self.wait()
        self.play(Write(mlp_icon), FadeIn(mlp_title_full, UP))
        self.wait()
        self.play(TransformMatchingStrings(mlp_title_full, mlp_title))
        self.wait()
        sport_facts = VGroup((Text(line) for line in Path(DATA_DIR, 'athlete_sports.txt').read_text().split('\n')))
        ...
        self.play(FadeOut(thumbnails), FadeOut(trans_title), LaggedStart((Succession(FadeIn(fact), fact.animate.scale(0.5).set_opacity(0).move_to(mlp_blocks)) for fact in sport_facts), lag_ratio=0.15))
        self.wait()
        ...
        self.play(Group(att_blocks, att_title).animate.fade(0.5), ShowCreation(rect), Write(question), GrowArrow(arrow))
        self.wait()

    def get_thumbnails(self):
        folder = '/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2024/transformers/Thumbnails'
        images = [ImageMobject(str(Path(folder, 'Chapter5_TN5'))), ImageMobject(str(Path(folder, 'Chapter6_TN4')))]
        thumbnails = Group((Group(SurroundingRectangle(image, buff=0).set_stroke(WHITE, 3), image) for n, image in zip([5, 6], images)))
    ...
    def get_mlp_icon(self, block, dot_buff=0.15, layer_buff=1.5, layer0_size=5):
        layers = VGroup(Dot().get_grid(layer0_size, 1, buff=dot_buff), Dot().get_grid(2 * layer0_size, 1, buff=dot_buff), Dot().get_grid(layer0_size, 1, buff=dot_buff))
        ...
        lines = VGroup((Line(dot1.get_center(), dot2.get_center(), buff=dot1.get_width() / 2) for l1, l2 in zip(layers, layers[1:]) for dot1 in l1 for dot2 in l2))
        for line in lines:
            line.set_stroke(color=value_to_color(random.uniform(-10, 10)), width=3 * random.random() ** 3)
...
class AltLastTwoChapters(LastTwoChapters):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(MoveToTarget, thumbnails))
        self.wait()
...
class MLPIcon(LastTwoChapters):
    ...
    def construct(self):
        ...
        self.play(Write(network, stroke_width=0.5, lag_ratio=0.01, run_time=5))
        self.wait()
        ...
        self.play(LaggedStartMap(VShowPassingFlash, thick_layers, time_width=1.5, lag_ratio=0.005, run_time=3))
        self.wait()
...
class MLPStepsPreview(InteractiveScene):
    ...
    def construct(self):
        ...
        titles = VGroup(VGroup(Text('Structure:'), Text('Easy')), VGroup(Text('Emergent behavior:'), Text('Exceedingly challenging')))
        ...
        self.play(LaggedStartMap(GrowArrow, arrows, lag_ratio=0.5), LaggedStartMap(FadeIn, labels, shift=0.5 * RIGHT, lag_ratio=0.5), Write(titles[0][1]))
        self.play(LaggedStart(FadeIn(up_proj, shift=0.5 * UP), FadeIn(down_proj, shift=0.5 * UP), lag_ratio=0.5))
        self.play(FadeIn(plot, lag_ratio=0.01))
        self.wait(3)
        self.play(Restore(top_frame), Restore(titles[0]), structure.animate.set_width(0.9 * top_frame.saved_state.get_width()).move_to(top_frame.saved_state), FadeIn(low_frame, DOWN), FadeIn(titles[1][0], DOWN))
        self.play(Write(titles[1][1], stroke_color=RED))
        ...
        self.play(ShowCreation(strike), FadeOut(titles[1][1]), titles[1][0].animate.set_opacity(0.5))
        self.add(top_matrices)
        self.play(MoveToTarget(low_matrices), FadeIn(toy_example_title, DOWN))
        self.wait()
        ...
        self.play(LaggedStart(FadeIn(row_cover), FadeIn(row_rect), GrowFromCenter(mj_brace), FadeIn(mj, 0.5 * UP)))
        self.play(FadeIn(col_cover), FadeIn(col_rect), GrowArrow(bb_arrow), GrowFromCenter(bb_brace), FadeIn(bb, 0.5 * UP))
        self.add(*low_matrices, row_cover, col_cover, row_rect, col_rect)
        self.play(RandomizeMatrixEntries(low_matrices[0]), RandomizeMatrixEntries(low_matrices[1]))
        self.wait()
...
class BasicMLPWalkThrough(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(MoveToTarget(embedding_array), ShowCreation(highlight_rect))
        self.wait()
        ...
        self.play(frame.animate.reorient(-3, -2, 0, (-0.0, -2.0, 0.01), 6.48), MoveToTarget(full_block), run_time=2)
        values = np.random.uniform(-10, 10, 9)
        values[0] = 1.0
        vects = VGroup(NumericEmbedding(values=values, dark_color=GREY_B), NumericEmbedding(values=np.clip(values, 0, np.inf), dark_color=GREY_B), NumericEmbedding(length=6))
        vects.set_width(emb.get_depth())
        vects.arrange(RIGHT, buff=2.0)
        vects.next_to(emb, RIGHT, buff=2.0)
        arrows = VGroup((Arrow(v1, v2) for v1, v2 in zip([emb, *vects[:-1]], vects)))
        ...
        self.play(LaggedStartMap(FadeIn, vects, shift=RIGHT, lag_ratio=0.8), LaggedStartMap(ShowCreation, arrows, lag_ratio=0.8), LaggedStartMap(FadeIn, arrow_labels, lag_ratio=0.8))
        self.wait()
        sum_circuit, output_emb = self.get_sum_circuit(emb, vects[-1])
        self.play(frame.animate.reorient(15, -4, 0, (0.82, -1.91, 0.04), 7.18), ShowCreation(sum_circuit, lag_ratio=0.1), run_time=2)
        self.play(TransformFromCopy(emb, output_emb, path_arc=-30 * DEGREES), TransformFromCopy(vects[2], output_emb, path_arc=-30 * DEGREES), run_time=2)
        self.wait()
        simple_phases.add_to_back(highlight_rect)
        simple_phases.add(VGroup(sum_circuit, output_emb))
        simple_phase_copies = VGroup((simple_phases.copy().match_z(emb) for emb in embedding_array.embeddings))
        for sp_copy in simple_phase_copies:
            for group in sp_copy[1:]:
                ...
                for entry in vect.get_entries():
                    ...
                    entry.become(dot)
                group.fade(0.5)
        self.play(frame.animate.reorient(0, -48, 0, (0.55, -2.21, 0.18), 7.05), LaggedStart((TransformFromCopy(simple_phases, sp_copy) for sp_copy in simple_phase_copies), lag_ratio=0.1), FadeOut(block_title, time_span=(0, 1)), run_time=3)
        ...
        block_title.next_to(block, UP)
        self.play(frame.animate.to_default_state(), LaggedStartMap(FadeOut, simple_phase_copies, lag_ratio=0.1), FadeIn(block_title), run_time=2)
        self.wait()
        ...
        mj.next_to(emb, UL)
        mj_lines = VGroup((Line(char.get_bottom(), emb.get_top(), buff=0.1, path_arc=10 * DEGREES) for char in mj))
        ...
        self.play(Write(mj_lines, stroke_width=2, stroke_color=YELLOW_B, lag_ratio=0.01))
        self.wait()
        mover = emb.copy()
        for vect in vects:
        ...
        up_emb[-2:].set_fill(YELLOW)
        self.play(phases[1:].animate.set_opacity(0.1), sum_circuit.animate.set_stroke(opacity=0.1), output_emb.animate.set_opacity(0.1), FadeOut(mj), FadeOut(mj_lines), FadeOut(basketball), FadeOut(example_fact))
        self.wait()
        ...
        self.wait()
        machine = MachineWithDials(width=up_proj.get_width() + SMALL_BUFF, height=up_proj.get_height() + SMALL_BUFF, n_rows=8, n_cols=9)
        ...
        n, m = up_proj.shape
        n_rows_shown = 5
        R_labels = VGroup((Tex('\\vec{\\textbf{R}}_' + f'{{{n}}}') for n in [*range(n_rows_shown - 1), 'n']))
        R_labels[-2].become(Tex('\\vdots').replace(R_labels[-2], dim_to_match=1))
        R_labels.arrange(DOWN, buff=0.5)
        R_labels.match_height(up_proj)
        R_labels.move_to(up_proj)
        h_lines = VGroup(Line(up_proj.get_brackets()[0], R_labels, buff=0.1), Line(R_labels, up_proj.get_brackets()[1], buff=0.1))
        h_lines.set_stroke(GREY_A, 2)
        row_labels = VGroup((VGroup(R_label, h_lines.copy().match_y(R_label)) for R_label in R_labels))
        row_matrix = VGroup(up_proj.get_brackets().copy(), row_labels)
        E_label = Tex('\\vec{\\textbf{E}}')
        E_label.match_height(R_labels[0])
        E_label.set_color(YELLOW)
        E_label.move_to(up_emb)
        E_col = VGroup(up_emb[-2:].copy(), Line(up_emb.get_top(), E_label, buff=0.1).set_stroke(GREY_A, 2), E_label, Line(E_label, up_emb.get_bottom(), buff=0.1).set_stroke(GREY_A, 2))
        dot_prods = VGroup()
        for n, R_label in enumerate(R_labels):
            if n == len(R_labels) - 2:
                dot_prod = R_label.copy()
            else:
                dot_prod = VGroup(R_label.copy(), Tex('\\cdot'), E_label.copy())
        ...
        dot_prod_rhs = VGroup(rhs.get_brackets().copy(), dot_prods)
        self.play(LaggedStart(FadeOut(up_proj, scale=1.1), FadeIn(row_matrix, scale=1.1), FadeOut(up_emb, scale=1.1), FadeIn(E_col, scale=1.1), FadeOut(rhs, scale=1.1), FadeIn(dot_prod_rhs[0], scale=1.1), lag_ratio=0.1))
        self.wait()
        for row_label, dot_prod in zip(row_labels, dot_prods):
            R_label = row_label[0]
            self.play(TransformFromCopy(R_label, dot_prod[0]), TransformFromCopy(R_label, dot_prod[1]), TransformFromCopy(E_label, dot_prod[2]), VShowPassingFlash(Line(row_label.get_left(), row_label.get_right()).set_stroke(YELLOW, 5).insert_n_curves(100), time_width=1.5), VShowPassingFlash(Line(E_col.get_top(), E_col.get_bottom()).set_stroke(YELLOW, 5).insert_n_curves(100), time_width=1.5), run_time=1)
        self.wait()
        ...
        first_name_label = Tex('\\overrightarrow{\\text{First Name Michael}}')
        ...
        mde_rhs = VGroup(Tex("\\approx 1 \\quad \\text{If } \\vec{\\textbf{E}} \\text{ encodes ``First Name Michael''}"), Tex('\\le 0 \\quad \\text{If not}'))
        mde_rhs[0]['\\vec{\\textbf{E}}'].set_color(YELLOW)
        mde_rhs.scale(0.75)
        ...
        self.play(FadeIn(row_rect, scale=2), FadeTransform(row_labels[0].copy(), first_name_label), GrowFromCenter(row_eq), frame.animate.reorient(0, 0, 0, (0.22, 0.54, 0.0), 9.27))
        self.wait()
        self.play(TransformFromCopy(row_rect.copy().set_fill(opacity=0), dp_rect))
        self.play(Write(dp_eq), GrowFromCenter(rhs_brace), FadeIn(mde_rhs))
        self.wait()
        fn_tex = '\\overrightarrow{\\text{F.N. Michael}}'
        ln_tex = '\\overrightarrow{\\text{L.N. Jordan}}'
        name_sum_label = Tex(f'{fn_tex} + {ln_tex}')
        name_sum_label.match_width(row_rect).scale(1.2)
        name_sum_label.next_to(row_eq, UP)
        self.play(FadeTransform(first_name_label, name_sum_label[:21]), FadeIn(name_sum_label[21:], shift=RIGHT, scale=2), FadeOut(mde_rhs), FadeOut(rhs_brace))
        self.wait()
        dist_rhs = VGroup(Tex('(\\vec{\\textbf{M}} + \\vec{\\textbf{J}}) \\cdot \\vec{\\textbf{E}}'), Tex('='), Tex('\\vec{\\textbf{M}} \\cdot \\vec{\\textbf{E}} + \\vec{\\textbf{J}} \\cdot \\vec{\\textbf{E}}'))
        dist_rhs.scale(0.75)
        dist_rhs.arrange(RIGHT, buff=0.2)
        dist_rhs.next_to(dp_eq, RIGHT)
        for part in dist_rhs:
            part['\\vec{\\textbf{M}}'].set_color(RED_B)
            part['\\vec{\\textbf{J}}'].set_color(RED)
            part['\\vec{\\textbf{E}}'].set_color(YELLOW)
        under_brace = Brace(dist_rhs[2])
        two_condition = TexText("$\\approx 2$ \\; if $\\vec{\\textbf{E}}$ encodes ``Michael Jordan''")
        two_condition['\\vec{\\textbf{E}}'].set_color(YELLOW)
        else_condition = TexText('$\\le 1$ \\; Otherwise')
        ...
        self.play(LaggedStart(FadeTransformPieces(name_sum_label[:21].copy(), dist_rhs[0][1:3]), FadeTransformPieces(name_sum_label[21].copy(), dist_rhs[0][3]), FadeTransformPieces(name_sum_label[22:].copy(), dist_rhs[0][4:6]), FadeTransformPieces(dot_prod[1:].copy(), dist_rhs[0][7:]), FadeIn(dist_rhs[0][0]), FadeIn(dist_rhs[0][6]), lag_ratio=0.2))
        self.wait()
        self.play(TransformMatchingStrings(dist_rhs[0].copy(), dist_rhs[2], lag_ratio=0.01, path_arc=-45 * DEGREES), Write(dist_rhs[1]))
        self.wait()
        self.play(frame.animate.set_y(0.5), GrowFromCenter(under_brace), FadeIn(two_condition, DOWN))
        self.wait()
        ...
        self.play(LaggedStart(*map(FadeOut, [name_sum_label, row_eq, row_rect, dp_rect, dp_eq, dist_rhs, under_brace, two_condition, else_condition]), lag_ratio=0.1, run_time=1), frame.animate.reorient(0, 0, 0, (-0.06, -0.06, 0.0), 8.27))
        self.play(FadeOut(row_matrix), FadeIn(up_proj), FadeOut(E_col), FadeIn(up_emb), FadeOut(dot_prod_rhs), FadeIn(rhs))
        questions = VGroup(*map(Text, ['Blah', 'Is it English?', 'Part of source code?', 'European country?', 'In quotation marks?', 'Something metallic?', 'A four-legged animal?']))
        ...
        for index in range(1, 7):
            ...
            self.play(MoveToTarget(rows), MoveToTarget(rhs_entries), FadeOut(last_question), FadeIn(question), FadeTransform(last_rect, row_rect, time_span=(0, 0.75)), run_time=1.0)
            self.wait(0.5)
            last_question = question
            last_rect = row_rect
        self.play(rows.animate.set_opacity(1), rhs.animate.set_opacity(1), FadeOut(last_question), FadeOut(last_rect))
        self.wait()
        plus = Tex('+')
        ...
        self.play(Write(plus), FadeIn(bias, lag_ratio=0.1), MoveToTarget(eq), MoveToTarget(rhs))
        self.wait()
        self.play(frame.animate.scale(1.1, about_edge=DOWN), Write(bias_name))
        self.wait()
        data_modifying_matrix(self, bias, word_shape=(5, 1), alpha_maxes=(0.4, 0.9), fix_in_frame=True)
        bias.get_entries()[0].set_value(-1).set_color(RED)
        fe_rect = SurroundingRectangle(rhs.get_entries()[0], buff=0.1)
        ...
        self.play(ShowCreation(fe_rect), FadeIn(fe_eq, RIGHT), Write(fe_expr))
        self.wait()
        ...
        self.wait()
        conditions = VGroup(TexText("$\\approx 1$ \\; if $\\vec{\\textbf{E}}$ encodes ``Michael Jordan''"), TexText('$\\le 0$ \\; Otherwise'))
        conditions[0]['\\vec{\\textbf{E}}'].set_color(YELLOW)
        ...
        self.play(FadeOut(bias_rect), GrowFromCenter(under_brace), FadeIn(conditions[0], DOWN))
        self.wait()
        self.play(FadeIn(conditions[1], 0.25 * DOWN))
        self.wait(2)
        self.play(frame.animate.reorient(0, 0, 0, (-2.5, 0.44, 0.0), 9.33), LaggedStart(*map(FadeOut, [fe_rect, fe_eq, fe_expr, under_brace, *conditions])))
        up_proj.refresh_bounding_box()
        row_rects = VGroup((SurroundingRectangle(row, buff=0.1) for row in up_proj.get_rows()))
        ...
        num_rows_expr = Tex('4 \\times 12{,}288')
        num_rows_expr.next_to(side_brace, LEFT)
        self.play(FadeIn(row_rects, lag_ratio=0.5), GrowFromCenter(side_brace), CountInFrom(num_rows))
        self.wait()
        self.play(FadeTransform(num_rows, num_rows_expr))
        self.wait()
        self.play(FadeTransform(num_rows_expr['12{,}288'].copy(), row_size), TransformFromCopy(side_brace, over_brace))
        self.wait()
        self.play(FadeOut(row_rects, lag_ratio=0.1))
        full_product = VGroup(num_rows_expr.copy(), Tex('\\times'), row_size.copy(), Tex(f'='), Integer(4 * d_model * d_model))
        full_product.scale(1.5)
        full_product.arrange(RIGHT, buff=MED_SMALL_BUFF)
        full_product.next_to(row_rects, UP, buff=2.5)
        self.play(LaggedStart(frame.animate.reorient(0, 0, 0, (-3.88, 1.51, 0.0), 11.35), TransformFromCopy(num_rows_expr, full_product[0]), FadeIn(full_product[1], UP), TransformFromCopy(row_size, full_product[2]), lag_ratio=0.25, run_time=2))
        self.play(TransformFromCopy(full_product[:3], full_product[3:]))
        self.wait()
        self.play(FlashAround(full_product[-1], run_time=2, time_width=1.5))
        bias_count = Tex('4 \\times 12{,}288')
        ...
        div_eq = Tex('{4 \\times 12{,}288 \\over 603{,}979{,}776} \\approx 0.00008 ')
        div_eq['{4 \\times 12{,}288'].match_color(bias_rect)
        div_eq.next_to(frame.get_corner(UR), DL, buff=MED_LARGE_BUFF)
        div_eq.shift(RIGHT)
        self.play(ShowCreation(bias_rect))
        self.play(GrowArrow(bias_arrow), FadeInFromPoint(bias_count, bias_arrow.get_start()), full_product.animate.scale(0.8).shift(3.5 * LEFT))
        self.wait()
        self.play(frame.animate.set_x(-3.0), FadeTransform(bias_count.copy(), div_eq['4 \\times 12{,}288']), Write(div_eq['\\over']), FadeTransform(full_product[-1].copy(), div_eq['603{,}979{,}776}']), Write(div_eq['\\approx 0.00008']))
        self.wait()
        self.play(frame.animate.reorient(0, 0, 0, (-2.5, 0.44, 0.0), 9.33), *map(FadeOut, [full_product, bias_rect, bias_arrow, bias_count, div_eq]))
        substrs = ['W_\\uparrow', '\\vec{\\textbf{E}}_i', '+', '\\vec{\\textbf{B}}_\\uparrow']
        ...
        self.play(frame.animate.reorient(0, 0, 0, (-0.03, 0.03, 0.0), 8.34), ReplacementTransform(up_proj, W_up, lag_ratio=0.001), FadeOut(side_brace, RIGHT, scale=0.5), FadeOut(num_rows_expr, RIGHT, scale=0.5), FadeOut(over_brace, DR, scale=0.5), FadeOut(row_size, DR, scale=0.5))
        self.wait()
        self.play(ReplacementTransform(up_emb, E_i, lag_ratio=0.01))
        self.play(TransformFromCopy(E_i, low_emb_label))
        self.wait()
        self.play(ReplacementTransform(plus, plus2), ReplacementTransform(bias, B_up, lag_ratio=0.01), FadeOut(bias_name, DL), VGroup(eq, rhs).animate.next_to(B_up, RIGHT).shift(0.1 * DOWN), run_time=2)
        self.wait()
        self.play(linear_expr.animate.scale(0.5).next_to(arrows[0], DOWN, buff=0.1), ReplacementTransform(rhs, vects[0]), FadeOut(eq, 4 * DOWN + LEFT), run_time=2)
        self.wait()
        self.play(phases[1].animate.set_opacity(1))
        phase1_copy = VGroup(vects[0], arrows[1], vects[1]).copy()
        phase1_copy.save_state()
        self.play(phase1_copy.animate.scale(2.0).next_to(full_block, UP, buff=0.5), frame.animate.reorient(0, 0, 0, (-0.26, 0.54, 0.0), 9.4))
        self.wait()
        ...
        self.wait()
        self.play(TransformFromCopy(neg_left_rects, zero_right_rects, lag_ratio=0.5), ShowCreation(neg_arrows, lag_ratio=0.5), FadeOut(relu_arrow))
        self.wait()
        self.play(FadeOut(neg_left_rects, lag_ratio=0.25), FadeOut(zero_right_rects, lag_ratio=0.25), FadeOut(neg_arrows, lag_ratio=0.25), ShowCreation(pos_left_rects))
        self.wait()
        self.play(ShowCreation(pos_arrows, lag_ratio=0.5), TransformFromCopy(pos_left_rects, pos_right_rects, lag_ratio=0.5))
        self.wait()
        ...
        self.play(frame.animate.set_x(2.7), FadeIn(relu_arrow), FadeIn(relu_title_full, 0.1 * UP, lag_ratio=0.1, run_time=2), FadeOut(pos_arrows, lag_ratio=0.25), FadeOut(pos_left_rects, lag_ratio=0.25), FadeOut(pos_right_rects, lag_ratio=0.25), FadeIn(plot, RIGHT))
        self.wait()
        self.play(*(TransformFromCopy(relu_title_full[substr], relu_graph_label[substr]) for substr in ['Re', 'L', 'U']))
        ...
        self.play(frame.animate.reorient(0, 0, 0, (2.61, 0.97, 0.0), 11.5), ShowCreation(fe_rect), GrowFromCenter(under_brace))
        self.play(TransformFromCopy(fe_rect, conditions_rect), FadeInFromPoint(conditions, fe_rect.get_center()))
        self.wait()
        self.play(condition_group.animate.match_x(phase1_copy[2]))
        equals = Tex('=')
        ineq = conditions[1][0]
        equals.replace(ineq, dim_to_match=0)
        self.play(FlashAround(equals, run_time=2, time_width=1.5), ineq.animate.become(equals))
        self.wait()
        self.play(frame.animate.reorient(0, 0, 0, (2.48, 0.33, 0.0), 9.17), FadeOut(condition_group, lag_ratio=0.01))
        ...
        self.play(FadeTransform(relu_title_full, gelu_title_full), relu_graph_label.animate.set_fill(opacity=0.25), relu_graph.animate.set_stroke(opacity=0.25), ShowCreation(gelu_graph), TransformFromCopy(relu_graph_label, gelu_graph_label))
        self.wait(2)
        self.play(gelu_graph.animate.set_stroke(opacity=0.25), gelu_graph_label.animate.set_fill(opacity=0.25), relu_graph.animate.set_stroke(opacity=1), relu_graph_label.animate.set_fill(opacity=1), FadeTransform(gelu_title_full, relu_title_full))
        self.wait()
        neuron_word = Text('Neurons', font_size=72)
        neuron_word.next_to(phase1_copy, RIGHT, buff=2.5)
        neuron_arrows = VGroup((Arrow(neuron_word.get_left(), entry.get_right(), buff=0.4, stroke_width=3) for entry in phase1_copy[2].get_entries()))
        self.play(plot.animate.set_width(2).next_to(relu_arrow, DOWN), FadeOut(VGroup(relu_graph_label, gelu_graph_label, gelu_graph)), Write(neuron_word), ShowCreation(neuron_arrows, lag_ratio=0.2, run_time=3), LaggedStartMap(FlashAround, phase1_copy[2].get_entries(), time_width=3.0, lag_ratio=0.05, time_span=(1, 4), run_time=4))
        self.wait()
        ...
        dots = VGroup((Dot(radius=0.15).move_to(entry).set_fill(WHITE, opacity=clip(entry.get_value(), 0, 1)) for entry in phase1_copy[2].get_entries()))
        ...
        lines = VGroup((Line(entry.get_right() + 0.05 * RIGHT, dot).set_stroke(color=value_to_color(random.uniform(-10, 10)), width=3 * random.random() ** 2) for entry in up_emb.get_entries() for dot in dots))
        self.play(FadeIn(blocking_rect), Write(dots))
        self.play(TransformFromCopy(emb, up_emb))
        self.play(ShowCreation(lines, lag_ratio=3 / len(lines)))
        self.wait()
        self.play(LaggedStart(*map(FadeOut, [up_emb, *lines, blocking_rect, *dots]), lag_ratio=0.01))
        entry = phase1_copy[2].get_entries()[0]
        entry_rect = SurroundingRectangle(entry)
        entry_rect.set_stroke(YELLOW, 2)
        active_words = TexText("``Michael Jordan'' neuron is \\emph{active}")
        ...
        inactive = TexText('\\emph{inactive}')
        inactive.set_color(RED)
        inactive.move_to(active, LEFT)
        self.play(frame.animate.reorient(0, 0, 0, (2.45, 0.58, 0.0), 9.65), ShowCreation(entry_rect), Write(active_words, run_time=1))
        self.wait()
        self.play(ChangeDecimalToValue(entry, 0), ReplacementTransform(active, inactive[2:]), GrowFromCenter(inactive[:2]))
        active_words.add(inactive)
        self.wait()
        self.play(Restore(phase1_copy), TransformMatchingStrings(relu_title_full, arrow_labels[1]), plot.animate.scale(0.5).next_to(arrows[1], DOWN, SMALL_BUFF), FadeOut(neuron_word, DOWN), FadeOut(neuron_arrows, DOWN, lag_ratio=0.1), FadeOut(entry_rect, DOWN), FadeOut(active_words, DOWN, lag_ratio=0.01), run_time=1.5)
        ...
        self.wait()
        self.play(FadeOut(temp_eq, DOWN), FadeOut(temp_rhs, DOWN), Write(plus), FadeIn(bias, RIGHT))
        self.wait()
        self.play(Write(equals), TransformFromCopy(vects[2], rhs))
        self.wait()
        ...
        self.play(CountInFrom(dim_count), GrowFromCenter(side_brace))
        self.wait()
        self.play(Write(name), GrowFromCenter(over_brace))
        self.wait()
        ...
        n_labels = VGroup((Tex(f'n_{{{m}}}') for m in [*range(6), 'm']))
        n_labels.arrange(DOWN, buff=0.5)
        n_labels.match_height(neurons.get_entries())
        n_labels.move_to(neurons.get_entries())
        n_labels.replace_submobject(-2, Tex('\\vdots').move_to(n_labels[-2]))
        n_labels.set_color(BLUE)
        n_vect = VGroup(neurons[-2:].copy(), n_labels)
        self.play(LaggedStart(*map(FadeOut, [over_brace, name, side_brace, dim_count])), LaggedStart(FadeOut(down_proj), FadeIn(col_matrix), FadeOut(neurons), FadeIn(n_vect), FadeOut(bias), FadeIn(bias_as_col)))
        self.wait()
        over_brace = Brace(VGroup(col_matrix, n_vect), UP)
        scaled_cols = VGroup((VGroup(n_label, col_label[0]).copy() for n_label, col_label in zip(n_labels, col_matrix[1])))
        ...
        scaled_cols.target[-2].become(Tex('\\dots'))
        ...
        plusses = VGroup((Tex('+').move_to(midpoint(m1.get_right(), m2.get_left())) for m1, m2 in zip(scaled_cols.target, scaled_cols.target[1:])))
        self.play(frame.animate.reorient(0, 0, 0, (-0.27, 1.04, 0.0), 11.06), GrowFromCenter(over_brace), LaggedStartMap(MoveToTarget, scaled_cols, lag_ratio=0.7, run_time=5), LaggedStartMap(FadeIn, plusses, lag_ratio=0.7, run_time=5))
        self.wait()
        last_rects = VGroup()
        ...
        self.play(ShowCreation(col_rect), col_matrix[1][1:].animate.set_opacity(0.5), n_labels[1:].animate.set_opacity(0.5), scaled_cols[1:].animate.set_opacity(0.5), plusses.animate.set_opacity(0.5))
        self.play(Restore(basketball, path_arc=PI / 2))
        self.wait()
        self.play(TransformFromCopy(col_rect, n_rect))
        self.wait()
        self.play(TransformFromCopy(col_rect, prod_rect), TransformFromCopy(n_rect, prod_rect))
        ...
        for index in range(1, len(all_rect_groups)):
            self.play(FadeOut(all_rect_groups[index - 1]), FadeIn(all_rect_groups[index]), col_matrix[1][index].animate.set_opacity(1), n_labels[index].animate.set_opacity(1), scaled_cols[index].animate.set_opacity(1), plusses[index - 1].animate.set_opacity(1))
            self.wait(0.5)
        ...
        self.play(ReplacementTransform(over_brace, bias_brace), FadeIn(bias_rect), FadeOut(plusses, lag_ratio=0.1), FadeOut(scaled_cols, lag_ratio=0.1))
        self.play(FadeIn(bias_word, 0.5 * UP))
        self.wait()
        self.play(LaggedStart(*map(FadeOut, [bias_word, bias_brace, bias_rect])))
        W_down = Tex('W_\\downarrow', font_size=60).set_color(BLUE)
        B_down = Tex('\\vec{\\textbf{B}}_\\downarrow', font_size=60).set_color(BLUE_B)
        ...
        self.play(ReplacementTransform(col_matrix, W_down, lag_ratio=0.005, run_time=2))
        self.play(ReplacementTransform(bias_as_col, B_down, lag_ratio=0.01))
        self.wait()
        self.play(LaggedStart(MoveToTarget(down_proj_expr), FadeOut(equals, 2 * DOWN + 0.5 * LEFT), ReplacementTransform(rhs, vects[2]), lag_ratio=0.25, time_span=(0, 1.5)), frame.animate.reorient(0, -14, 0, (-0.1, -2.03, 0.01), 6.31), run_time=2)
        self.wait()
        ...
        self.play(frame.animate.reorient(13, -8, 0, (0.15, -2.05, 0.0), 6.52), ShowCreation(sum_circuit, lag_ratio=0.5), low_emb_label.animate.shift(0.2 * LEFT).set_anim_args(time_span=(0, 1)), FadeOut(output_emb), run_time=2)
        self.remove(faded_sum_circuit)
        output_emb.set_fill(opacity=1)
        self.play(LaggedStart(TransformFromCopy(emb, output_emb, path_arc=-45 * DEGREES), TransformFromCopy(vects[2], output_emb, path_arc=-45 * DEGREES), run_time=2, lag_ratio=0.2))
        self.wait()
        m_color = interpolate_color_by_hsl(GREY_BROWN, WHITE, 0.5)
        j_color = RED_B
        b_color = basketball.get_color()
        m_tex = Tex('\\overrightarrow{\\text{F.N. Michael}}').set_color(m_color)
        j_tex = Tex('\\overrightarrow{\\text{L.N. Jordan}}').set_color(j_color)
        b_tex = Tex('\\overrightarrow{\\text{Basketball}}').set_color(b_color)
        ...
        self.play(frame.animate.reorient(4, -6, 0, (-0.29, -1.76, 0.02), 7.7), FadeIn(mj, lag_ratio=0.1), ShowCreation(mj_arrow))
        self.play(Transform(mj.copy(), emb.copy().set_opacity(0), lag_ratio=0.005, remover=True, run_time=2))
        mover = emb.copy()
        for vect in [*vects, output_emb]:
            self.play(Transform(mover, vect, rate_func=linear))
        self.remove(mover)
        self.play(frame.animate.reorient(-3, -5, 0, (1.09, -1.48, -0.03), 9.61), FadeTransform(mj.copy(), mjb[:3]), FadeTransformPieces(mj.copy()[-1:], mjb[3:]), ShowCreation(mjb_arrow), run_time=2)
        self.wait(2)
        self.play(frame.animate.reorient(21, -14, 0, (-0.13, -2.21, 0.11), 6.91).set_anim_args(run_time=5), LaggedStartMap(FadeOut, VGroup(mj, mj_arrow, mjb_arrow, mjb)))
        self.play(frame.animate.reorient(14, -12, 0, (0.55, -2.21, 0.18), 7.05), LaggedStart((TransformFromCopy(simple_phases, sp_copy) for sp_copy in simple_phase_copies), lag_ratio=0.1), FadeOut(block_title, time_span=(0, 1)), run_time=5)
        self.play(frame.animate.reorient(42, -23, 0, (0.55, -2.21, 0.18), 7.05), run_time=8)
        self.wait()
        sum_circuits = VGroup(sum_circuit, *(sp[0] for sp in simple_phase_copies), *(sp[-1] for sp in simple_phase_copies))
        n_vects = VGroup(vects[1], *(sp[2][1] for sp in simple_phase_copies))
        neuron_points = np.array([entry.get_center() for vect in n_vects[1:] for entry in vect.get_entries()])
        ...
        neuron_ellipses = VGroup((n_vect.get_ellipses() for n_vect in n_vects[1:]))
        self.play(frame.animate.reorient(11, -5, 0, (0.55, -2.21, 0.18), 7.05), sum_circuits.animate.set_stroke(width=1, opacity=0.2), FadeOut(block[4]), run_time=2)
        self.play(frame.animate.reorient(-11, -5, 0, (0.55, -2.21, 0.18), 7.05).set_anim_args(run_time=4), FadeOut(n_vects), ShowCreation(neurons, run_time=2), FadeIn(neuron_ellipses, time_span=(1, 2)))
        self.add(neuron_ellipses)
        self.play(frame.animate.reorient(13, -7, 0, (0.55, -2.21, 0.18), 7.05), run_time=4)
        self.wait()

    def get_sum_circuit(self, in_vect, diff_vect, v_buff=0.15, h_buff=0.5, y_diff=0.65, color=YELLOW):
    ...
    def get_col_matrix(self, matrix, n_cols_shown, dots_index=-2, sym='C', top_index='m-1', width_multiple=1.0):
        C_labels = VGroup((Tex(f'\\vec{{\\textbf{{{sym}}}}}_{{{n}}}') for n in [*range(n_cols_shown - 1), top_index]))
        ...
        v_lines = VGroup(Line(matrix.get_bottom(), C_labels.get_bottom() + SMALL_BUFF * DOWN), Line(C_labels.get_top() + SMALL_BUFF * UP, matrix.get_top()))
        v_lines.set_stroke(WHITE, 1)
        col_labels = VGroup((VGroup(C_label, v_lines.copy().match_x(C_label)) for C_label in C_labels))
...
class NonlinearityOfLanguage(InteractiveScene):
    ...
    def construct(self):
        unit_size = 2.5
        plane = NumberPlane(axis_config=dict(stroke_width=1), background_line_style=dict(stroke_color=BLUE_D, stroke_width=1, stroke_opacity=0.75), faded_line_ratio=1, unit_size=unit_size)
        ...
        self.add(m_label)
        self.add(j_label)
        expr = Tex('(\\vec{\\textbf{M}} + \\vec{\\textbf{J}}) \\cdot \\textbf{E}')
        ...
        shadow.add_updater(lambda m: m.put_start_and_end_on(ORIGIN, get_line_point()))
        dot = Dot()
        dot.set_fill(PINK, 1)
        dot.f_always.move_to(get_line_point)
        dashed_line = always_redraw(lambda: DashedLine(emb_point.get_center(), get_line_point()).set_stroke(PINK, 2))
        ...
        names = VGroup((Text(name, font_size=36) for name in ['Michael Jordan', 'Michael Phelps', 'Alexis Jordan']))
        name_points = [sum_point, m_vect.get_end(), j_vect.get_end()]
        ...
        self.play(FadeIn(name, 0.5 * UP), Rotate(emb_point, TAU, about_point=emb_point.get_center() + 0.15 * DL, run_time=4))
        self.wait()
        self.add(name_ghosts[0])
        self.play(Transform(name, names[1]), emb_point.animate.move_to(m_vect.get_end()), run_time=2)
        self.wait()
        self.add(name_ghosts[1])
        self.play(Transform(name, names[2]), emb_point.animate.move_to(j_vect.get_end()).set_anim_args(path_arc=30 * DEGREES), run_time=2)
        ...
        self.play(emb_point.animate.move_to(other_point), LaggedStart(FadeOut(name), FadeIn(other_word), lag_ratio=0.5), run_time=3)
        self.wait()
        ...
        yes_no_words = VGroup(Text('Yes', font_size=72).set_fill(GREEN).to_corner(UR), Text('No', font_size=72).set_fill(RED).to_edge(UP).shift(LEFT))
...
class Superposition(InteractiveScene):
    ...
    def construct(self):
        ...
        bubble_label = TexText('$N$-dimensional\\\\ Space')
        ...
        self.play(Restore(frame, run_time=7), LaggedStartMap(VFadeInThenOut, idea_vects, lag_ratio=0.5, run_time=5))
        conditions = ['$90^\\circ$ apart', 'between $89^\\circ$ and $91^\\circ$ apart']
        task1, task2 = tasks = VGroup((TexText(f'Choose multiple vectors,\\\\ each pair {phrase}', font_size=42, alignment='') for phrase in conditions))
        task1['90^\\circ'].set_color(RED)
        task2['$89^\\circ$ and $91^\\circ$'].set_color(BLUE)
        task1.center().to_edge(UP)
        task2.move_to(task1, UL)
        maximum1, maximum2 = maxima = VGroup((TexText(f'Maximum \\# of vectors: {answer}', font_size=42) for answer in ['$N$', '$\\approx \\exp(\\epsilon \\cdot N)$']))
        ...
        elbows = VGroup((Elbow(width=0.1).rotate(angle, axis, about_point=ORIGIN).set_stroke(WHITE, 2) for angle, axis in [(0, UP), (-PI / 2, UP), (PI / 2, RIGHT)]))
        ...
        self.play(FadeIn(task1), LaggedStartMap(GrowArrow, perp_vectors[:2], lag_ratio=0.5))
        self.play(ShowCreation(elbows[0]))
        self.play(GrowArrow(perp_vectors[2]), LaggedStartMap(ShowCreation, elbows[1:3], lag_ratio=0.5))
        self.play(Rotate(perp_group, -50 * DEGREES, axis=perp_vectors[1].get_vector(), run_time=15), Write(maximum1, time_span=(2, 4)))
        ...
        pairs.arrange(RIGHT)
        pairs.to_corner(UL)
        self.play(FadeOut(maximum1), ShowCreation(cross))
        self.play(crossed_part.animate.shift(0.5 * DOWN).set_fill(opacity=0.5), Write(new_cond), LaggedStartMap(FadeIn, pairs, lag_ratio=0.25))
        self.play(Rotate(perp_group, 50 * DEGREES, axis=perp_vectors[1].get_vector(), run_time=10))
        three_d_label = TexText('3-dimensional\\\\ Space')
        ...
        def shake(vect):
            self.play(vect.animate.rotate(5 * DEGREES, RIGHT, about_point=origin), rate_func=lambda t: wiggle(t, 9))
        self.play(FadeIn(three_d_label, DOWN), bubble_label.animate.to_edge(DOWN).set_opacity(0.5))
        self.play(GrowArrow(new_vect), Transform(perp_vectors, alt_vects))
        shake(new_vect)
        self.play(Restore(perp_vectors), Transform(new_vect, new_vects[1]))
        shake(new_vect)
        self.play(Transform(perp_vectors, alt_vects), Transform(new_vect, new_vects[2]))
        shake(new_vect)
        self.wait()
        self.play(new_vect.animate.scale(0, about_point=origin), ApplyMethod(perp_group.scale, 0, dict(about_point=origin), lag_ratio=0.25), Restore(bubble_label), FadeOut(three_d_label, UP), run_time=2)
        ...
        self.wait(6)
        self.play(FadeOut(crossed_part), Write(maximum2), Rotating(vectors, TAU, axis=UP, run_time=20), LaggedStartMap(VFadeIn, vectors, lag_ratio=0.5, run_time=8))
        self.wait()
...
class StackOfVectors(InteractiveScene):
    ...
    def construct(self):
        rows = VGroup((NumericEmbedding(shape=(1, 9), ellipses_col=-5, value_range=(-1, 1)) for n in range(20)))
        rows.arrange(DOWN)
        for row in rows:
        ...
        rows.set_height(6)
        rows.to_edge(DOWN)
        rows[-2].become(Tex('\\vdots').replace(rows[-2], dim_to_match=1))
        ...
        self.play(GrowFromCenter(top_brace), FadeIn(top_label, lag_ratio=0.1), LaggedStartMap(FadeIn, rows, shift=0.25 * DOWN, lag_ratio=0.1, run_time=3), *map(GrowFromCenter, brackets))
        self.play(LaggedStart((RandomizeMatrixEntries(row) for row in rows[:-2]), lag_ratio=0.05))
        self.wait()
        self.play(GrowFromCenter(side_brace), FadeIn(side_label, lag_ratio=0.1))
        self.wait(4)
...
class ShowAngleRange(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(vect_pair)
        self.play(angle_tracker.animate.set_value(180), run_time=8)
        self.wait()
        self.play(angle_tracker.animate.set_value(95), run_time=3)
        self.wait()
...
class MLPFeatures(InteractiveScene):
    ...
    def construct(self):
        radius = 0.15
        layer1, layer2 = layers = VGroup((Dot(radius=radius).get_grid(n, 1, buff=radius / 2) for n in [8, 16]))
        ...
        features = iter(['Table', 'Slang', 'AM Radio', 'Humble', 'Notebook', 'Transparent', 'Duration', 'Madonna', 'Mirror', 'Pole Vaulting', 'Albert Einstein', 'Authentic', 'Scientific', 'Passionate', 'Bell Laboratories', 'Uzbekistan', 'Umbrella', 'Immanuel Kant', 'Baroque Music', 'Intense', 'Clock', 'Water skiing', 'Ancient Egypt', 'Ambiguous', 'Volume', 'Alexander the Great', 'Innovative', 'Religious'])
        last_neuron = VGroup()
        ...
        for neuron in layer2[:15]:
            ...
            self.play(FadeOut(last_feature_label), FadeIn(feature_label), last_neuron.animate.set_fill(opacity=0), neuron.animate.set_fill(opacity=1))
            last_neuron = neuron
        ...
        self.play(feature_label.animate.scale(48 / 36).next_to(brace, RIGHT), GrowFromCenter(brace), to_random_state(layer2))
        self.wait()
        for n in range(12):
            feature_label = Text(next(features))
            feature_label.next_to(brace, RIGHT)
            self.play(FadeOut(last_feature_label), FadeIn(feature_label), to_random_state(layer2))
            self.wait(0.5)
            last_feature_label = feature_label
...
class BreakDownThreeSteps(BasicMLPWalkThrough):
    ...
    def construct(self):
        vectors = VGroup((NumericEmbedding(length=n) for n in [8, 16, 16, 8]))
        ...
        arrows = VGroup((Arrow(v1, v2) for v1, v2 in zip(vectors, vectors[1:])))
        arrows.shift(DOWN)
        E_sym = Tex('\\vec{\\textbf{E}}')
        E_sym.next_to(arrows[0], LEFT).shift(0.1 * UP)
        for vect in vectors:
            vect.scale(0.75)
            vect.shift(0.25 * UP)
        up_proj, down_proj = matrices = VGroup(WeightMatrix(shape=(12, 6)), WeightMatrix(shape=(6, 11)))
        matrices.scale(0.25)
        ...
        self.play(LaggedStartMap(GrowArrow, arrows, lag_ratio=0.5), FadeIn(up_proj.get_rows(), lag_ratio=0.1, time_span=(0.0, 1.5)), FadeIn(down_proj.get_rows(), lag_ratio=0.1, time_span=(1.5, 3.0)), Restore(up_proj.brackets, time_span=(0.0, 1.5)), Restore(down_proj.brackets, time_span=(1.5, 3.0)), Write(relu, time_span=(1, 2)), run_time=3)
        self.wait()
        n, m = up_proj.shape
        n_rows_shown = 8
        R_labels = VGroup((Tex('\\vec{\\textbf{R}}_{' + str(n) + '}') for n in [*range(n_rows_shown - 1), 'n-1']))
        R_labels[-2].become(Tex('\\vdots').replace(R_labels[-2], dim_to_match=1))
        R_labels.arrange(DOWN, buff=0.5)
        R_labels.match_height(up_proj)
        R_labels.move_to(up_proj)
        ...
        row_matrix = VGroup(up_proj.get_brackets().copy(), row_labels)
        self.play(FadeOut(up_proj.get_rows(), lag_ratio=0.1), FadeIn(row_labels, lag_ratio=0.1))
        self.wait()
        self.play(row_labels[0][0].copy().animate.scale(2).next_to(title, UL).shift(2 * LEFT).set_opacity(0))
        self.wait()
        dots = VGroup((Dot().set_fill(opacity=random.random()).move_to(entry) for entry in vectors[2].get_columns()[0]))
        for dot in dots:
            dot.match_x(dots[0])
        ...
        self.play(FadeOut(down_proj.get_columns(), lag_ratio=0.1), FadeIn(col_labels, lag_ratio=0.1))
        self.wait()
        self.play(col_labels[0][0].copy().animate.scale(2).next_to(title, UR).shift(2 * RIGHT).set_opacity(0))
        self.wait()
        return
        ...
        for i in (0, 1):
            self.play(FadeTransform(vectors[i].copy(), vectors[i + 1]), rate_func=linear)
...
class SuperpositionVectorBundle(InteractiveScene):
    ...
    def construct(self):
        ...
        axes.scale(0.5)
        vects = VGroup((self.get_new_vector(v) for v in np.identity(3)))
...
class ShowBiasBakedIntoWeightMatrix(LastTwoChapters):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(frame.animate.match_x(blocks), ReplacementTransform(lil_wrapper, big_wrapper), FadeIn(lnm_block, RIGHT))
        self.wait()
        self.play(FlashAround(lnm_block[2], run_time=3, time_width=2))
        self.wait()

    def get_layer_norm_icon(self):
        axes1, axes2 = all_axes = VGroup((Axes((-4, 4), (0, 1, 0.25)) for x in range(2)))
    ...
    def get_block(self, content, upper_label='', lower_label='', upper_font_size=42, lower_font_size=36, buff=0.25, height=2, color=BLUE, stroke_width=3, fill_opacity=0.2):
    ...
    def get_wrapped_arrow(self, big_block, buff=0.75, color=GREY_B, stroke_width=4):
        vertices = [big_block.get_corner(RIGHT), big_block.get_corner(RIGHT) + buff * RIGHT, big_block.get_corner(UR) + buff * UR, big_block.get_corner(UL) + buff * UL, big_block.get_corner(LEFT) + buff * LEFT, big_block.get_corner(LEFT)]
