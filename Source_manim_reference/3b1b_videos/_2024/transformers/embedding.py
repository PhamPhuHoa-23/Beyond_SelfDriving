...
def get_piece_rectangles(phrase_pieces, h_buff=0.05, v_buff=0.1, fill_opacity=0.15, fill_color=None, stroke_width=1, stroke_color=None, hue_range=(0.5, 0.6), leading_spaces=False):
    rects = VGroup()
    ...
    for piece in phrase_pieces:
        ...
        rect = Rectangle(width=right_x - left_x + 2 * h_buff, height=height, fill_color=fill, fill_opacity=fill_opacity, stroke_color=stroke, stroke_width=stroke_width)
        if leading_spaces:
...
def get_direction_lines(axes, direction, n_lines=500, color=YELLOW, line_length=1.0, stroke_width=3):
    ...
    for line in lines:
        line.move_to(axes.c2p(random.uniform(*axes.x_range[:2]), random.uniform(*axes.y_range[:2]), random.uniform(*axes.z_range[:2])))
    return lines
...
class LyingAboutTokens2(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(rects[-1])
        self.play(LaggedStart(FadeIn(big_rect), ShowCreation(arrow), Write(q_marks), lag_ratio=0.3))
        self.wait()
        self.play(FadeOut(big_rect), LaggedStart(*(DrawBorderThenFill(rect) for rect in rects[:-1]), lag_ratio=0.02), LaggedStart(*(word.animate.match_color(rect) for word, rect in zip(words, rects))), FadeOut(arrow))
        self.wait()
        ...
        self.play(MoveToTarget(blocks), q_group.animate.next_to(blocks.target, RIGHT, aligned_edge=UP), LaggedStartMap(FadeIn, vectors, shift=0.5 * DOWN), LaggedStartMap(GrowFromCenter, arrows))
        self.wait()
        h_line = Line(LEFT, RIGHT).set_width(FRAME_WIDTH)
        title1, title2 = titles = VGroup(Text('The Truth', font_size=72).to_edge(UP), Text('A Convenient Lie', font_size=72).next_to(h_line, DOWN))
        h_line.set_stroke(WHITE, 2)
        h_line.next_to(titles[1], UP)
        for title in titles:
            title.add(Underline(title))
        phrase1, phrase2 = phrases = VGroup(Text('This process (known fancifully as tokenization) frequently subdivides words'), Text("Let's pretend that tokens are always simply words"))
        ...
        self.play(FadeOut(blocks), FadeOut(q_group), FadeOut(arrows), FadeOut(vectors), ShowCreation(h_line), FadeIn(title1, lag_ratio=0.1), FadeIn(tokens))
        self.add(token_rects, tokens)
        self.play(LaggedStartMap(FadeIn, token_rects), LaggedStart(*(token.animate.set_color(rect.get_color()) for token, rect in zip(tokens, token_rects))))
        self.wait()
        self.play(FadeIn(title2, lag_ratio=0.1), FadeIn(words))
        self.add(word_rects, words)
        self.play(LaggedStartMap(FadeIn, word_rects), LaggedStart(*(token.animate.set_color(rect.get_color()) for token, rect in zip(words, word_rects))))
        self.wait()
...
class ImageTokens(InteractiveScene):
    ...
    def construct(self):
        image = ImageMobject('SmallFluffCreature')
        image.set_height(5)
        self.add(image)
        ...
        self.wait()
        self.play(LaggedStart((patch.animate.set_stroke(TEAL, 3).set_anim_args(rate_func=there_and_back) for patch in patches), lag_ratio=5.0 / len(patches)))
        self.wait()
...
class SoundTokens(InteractiveScene):
    ...
    def construct(self):
        ...
        def func(x):
            x *= 1.7
            return sum([math.sin(x), 0.5 * math.sin(2 * x), 0.3 * math.sin(3 * x), 0.2 * math.sin(4 * x), 0.1 * math.sin(5 * x), 0.15 * math.sin(6 * x)])
        ...
        self.play(LaggedStart((chunk.animate.set_stroke(TEAL, 3).scale(1.5).set_anim_args(rate_func=there_and_back) for chunk in chunks), lag_ratio=2.0 / len(chunks), run_time=2))
        self.wait()

class IntroduceEmbeddingMatrix(InteractiveScene):

    def construct(self):
        words = ['aah', 'aardvark', 'aardwolf', 'aargh', 'ab', 'aback', 'abacterial', 'abacus', 'abalone', 'abandon', 'zygoid', 'zygomatic', 'zygomorphic', 'zygosis', 'zygote', 'zygotic', 'zyme', 'zymogen', 'zymosis', 'zzz']
        dots = Tex('\\vdots')
        shown_words = VGroup(*map(Text, words[:10]), dots, *map(Text, words[-10:]))
        ...
        brace_text = brace.get_tex('\\text{All words, } \\sim 50\\text{k}')
        self.play(LaggedStartMap(FadeIn, shown_words, shift=0.5 * LEFT, lag_ratio=0.1, run_time=2), GrowFromCenter(brace, time_span=(0.5, 2.0)), FadeIn(brace_text, time_span=(0.5, 1.5)))
        self.wait()
        ...
        self.play(MoveToTarget(shown_words), brace.animate.become(new_brace), brace_text.animate.next_to(new_brace, UP, buff=0.1), LaggedStart(*(Write(column, lag_ratio=0.01, stroke_width=1) for column in columns), lag_ratio=0.2, run_time=2), LaggedStartMap(FadeIn, matrix.get_brackets(), scale=0.5, lag_ratio=0))
        ...
        for index in range(len(columns)):
            ...
            self.play(*map(MoveToTarget, [shown_words, columns]), FadeIn(rect), FadeOut(last_rect))
            last_rect = rect
            self.wait(0.5)
        self.play(FadeOut(last_rect), shown_words.animate.set_opacity(1), columns.animate.set_opacity(1))
        frame = self.frame
        ...
        self.wait()
        rects = VGroup(*(SurroundingRectangle(entry).insert_n_curves(20) for entry in matrix.get_entries() if entry not in matrix.ellipses))
        rects.set_stroke(WHITE, 1)
        for x in range(1):
            self.play(RandomizeMatrixEntries(matrix, lag_ratio=0.01), LaggedStartMap(VShowPassingFlash, rects, lag_ratio=0.01, time_width=1.5), run_time=2)
        data_modifying_matrix(self, matrix)
        self.wait()
        matrix_group = VGroup(lhs, matrix, shown_words, matrix_name)
        index = words.index('aardvark')
        vector = VGroup(matrix.get_brackets()[0], matrix.get_columns()[index], matrix.get_brackets()[1]).copy()
        ...
        self.play(LaggedStart(matrix_group.animate.scale(0.5).next_to(frame.get_top(), DOWN, 0.5), FadeOut(brace, UP), FadeOut(brace_text, 0.5 * UP), MoveToTarget(word), MoveToTarget(vector), GrowFromPoint(arrow, word.get_center())), lag_ratio=0.15)
        self.wait()
        ...
        shown_tokens = VGroup(*(Text(encoding.decode([i]), **kw) for i in range(10)), shown_words[dots_index].copy().rotate(-45 * DEGREES), *(Text(encoding.decode([i]), **kw) for i in range(n_vocab - 10, n_vocab)))
        ...
        self.play(GrowFromCenter(top_brace), CountInFrom(vocab_count, 0), FadeIn(vocab_label[1]))
        self.wait()
        self.play(FadeOut(vocab_label[1], 0.5 * UP), FadeIn(token_label, 0.5 * UP), LaggedStartMap(FadeOut, shown_words, shift=0.25 * UP, lag_ratio=0.1), LaggedStartMap(FadeIn, shown_tokens, shift=0.25 * UP, lag_ratio=0.1))
        self.wait()
        matrix_name.target = matrix_name.generate_target()
        matrix_name.target.shift(RIGHT)
        self.play(MoveToTarget(matrix_name), lhs.animate.next_to(matrix_name.target, LEFT), GrowFromCenter(left_brace), CountInFrom(dim_count, 0))
        self.play(FlashAround(dim_count))
        self.wait()
        matrix_group = VGroup(top_brace, vocab_count, left_brace, dim_count, matrix, shown_words, matrix_name, lhs)
        top_equation = VGroup(Text('Total parameters = '), dim_count.copy(), Tex('\\times'), vocab_count.copy(), Tex('='), Integer(vocab_count.get_value() * dim_count.get_value()).set_color(YELLOW))
        ...
        self.play(LaggedStartMap(FadeIn, top_equation[::2], shift=0.25 * UP, lag_ratio=0.5), TransformFromCopy(dim_count, top_equation[1]), TransformFromCopy(vocab_count, top_equation[3]), frame.animate.set_height(11).move_to(matrix_group, DOWN).shift(DOWN))
...
class Word2VecScene(InteractiveScene):
    ...
    axes_config = dict(x_range=(-5, 5, 1), y_range=(-5, 5, 1), z_range=(-4, 4, 1), width=8, height=8, depth=6.4)
    label_rotation = PI / 2
    ...
    def add_plane(self, color=GREY, stroke_width=1.0):
        axes = self.axes
        plane = NumberPlane(axes.x_range, axes.y_range, width=axes.get_width(), height=axes.get_height(), background_line_style=dict(stroke_color=color, stroke_width=stroke_width), faded_line_style=dict(stroke_opacity=0.25, stroke_width=0.5 * stroke_width), faded_line_ratio=1)
        self.plane = plane
        self.add(plane)
        return plane

    def get_labeled_vector(self, word, coords=None, thickness=5, color=YELLOW, func_name: str | None='E', buff=0.05, direction=None, label_config: dict=dict()):
        axes = self.axes
        if coords is None:
        ...
        arrow = LabeledArrow(axes.get_origin(), point, thickness=thickness, fill_color=color, label_text=word if func_name is None else f'{func_name}({word})', buff=0, direction=direction, **label_config)
...
class AmbientWordEmbedding(Word2VecScene):
    ...
    def construct(self):
        ...
        embeddings = VGroup(*(self.get_labeled_vector(word, coord, stroke_width=2, color=interpolate_color(BLUE_D, BLUE_A, random.random()), func_name=None, label_config=dict(font_size=24)) for word, coord in zip(words, coords)))
...
class ThreeDSpaceExample(InteractiveScene):
    ...
    def construct(self):
        ...
        lines = VGroup(Line(axes.c2p(0, 0, 0), axes.c2p(x, 0, 0)), Line(axes.c2p(x, 0, 0), axes.c2p(x, y, 0)), Line(axes.c2p(x, y, 0), axes.c2p(x, y, z)))
        ...
        for entry, line, label, value in zip(coords.get_entries(), lines, labels, coordinates):
            ...
            self.play(ShowCreation(line), FadeInFromPoint(label, line.get_start()), VFadeInThenOut(rect), ChangeDecimalToValue(entry, value), dot.animate.move_to(line.get_end()))
            self.wait(0.5)
        self.play(ShowCreation(vect))
        self.wait(15)
        points = GlowDots(np.random.uniform(-3, 3, size=(50, 3)), radius=0.1)
        frame.clear_updaters()
        self.play(FadeOut(coords), FadeOut(dot), FadeOut(plane), LaggedStartMap(FadeOut, VGroup(*lines, vect, *labels)), frame.animate.reorient(-81, 61, 0, (-0.82, 0.6, 0.36), 8.95), ShowCreation(points), run_time=2)
        ...
        grid = NumberPlane((-5, 5), (-5, 5), background_line_style=dict(stroke_color=GREY_B, stroke_width=1), faded_line_ratio=0)
        ...
        self.wait(2)
        self.play(axes.animate.set_stroke(opacity=0.25), points.animate.set_opacity(0.5), TransformFromCopy(points, projected_points), ShowCreation(projection_lines, lag_ratio=0.05), run_time=3)
...
class HighDimensionalSpaceCompanion(InteractiveScene):
    ...
    def construct(self):
        ...
        label = VGroup(Integer(10000), Text('coordinates'))
        ...
        self.play(GrowFromCenter(brace), CountInFrom(label[0], 0), FadeIn(label[1]))
        self.wait()
        self.play(TransformFromCopy(label[0], dimension_label[0]), FadeInFromPoint(dimension_label[1], label[0].get_center()))
        ...
        self.play(bubble_label.animate.scale(0.5).shift(1.2 * UP + 1.5 * LEFT), FadeOut(q_marks), Write(axes, lag_ratio=0.01), Write(axes_label))
        self.wait(2)
        vectors = VGroup(*(Arrow(axes.get_origin(), axes.c2p(*np.random.uniform(-3, 3, 3)), buff=0, stroke_color=random_bright_color(hue_range=(0.55, 0.65))) for x in range(5)))
...
class LearningEmbeddings(Word2VecScene):
    ...
    def construct(self):
        ...
        word_vects = VGroup(*(self.get_labeled_vector(word, coords=coords, color=color, buff=0.05, func_name=None, label_config=dict(font_size=36)) for word, color, coords in zip(words, colors, initial_coords)))
        labels = VGroup()
        for vect in word_vects:
            ...
            labels.add(label)
        self.play(LaggedStartMap(GrowArrow, word_vects, lag_ratio=0.2), LaggedStartMap(FadeIn, labels, lag_ratio=0.2), run_time=4)
        self.wait()
        turn_animation_into_updater(ApplyMethod(frame.reorient, 4, 72, 0, (-0.04, -0.18, -0.5), 8.0), run_time=8)
        self.progressive_nudges(word_vects, true_embeddings, 8)
        frame.clear_updaters()
        turn_animation_into_updater(ApplyMethod(frame.reorient, 38, 69, 0, (-0.32, 0.02, -0.54), 7.68), run_time=12)
    ...
    def progressive_nudges(self, word_vects, true_embeddings, n_nudges, step_size=0.2):
        for x in range(n_nudges):
            anims = [vect.animate.put_start_and_end_on(self.axes.get_origin(), interpolate(vect.get_end(), self.axes.c2p(*embedding), step_size)) for vect, embedding in zip(word_vects, true_embeddings)]
...
class KingQueenExample(Word2VecScene):
    ...
    def construct(self):
        ...
        label_config = dict(font_size=30, label_rotation=0)
        man, woman, king, queen = word_vects = [self.get_labeled_vector(word, coords=coords, color=color, buff=0.05, direction=direction, label_config=label_config) for word, color, direction, coords in zip(words, colors, directions, all_coords)]
        woman.label.shift(SMALL_BUFF * DOWN)
        fake_queen_coords = all_coords[2] - all_coords[0] + all_coords[1]
        fake_queen = self.get_labeled_vector('queen', fake_queen_coords, color=colors[3], label_config=label_config)
        ...
        self.play(LaggedStart(*map(Write, [ew, minus2, em])), GrowArrow(woman), FadeInFromPoint(woman.label, man.get_center()), GrowArrow(man), FadeInFromPoint(man.label, man.get_center()), frame.animate.reorient(0, 0, 0, (2.04, 2.06, 0.38), 4.76).set_anim_args(run_time=6))
        self.play(GrowArrow(diff, time_span=(1, 3)), frame.animate.reorient(-179, 19, 179, (2.49, 1.96, 0.4), 4.76), run_time=5)
        self.add(top_rect, *equation)
        new_diff = diff.copy()
        new_diff.shift(king.get_end() - man.get_end())
        self.play(FadeIn(top_rect), *map(Write, [eq, minus1, ek, approx]), LaggedStart(TransformFromCopy(man, king), TransformFromCopy(man.label, king.label), TransformFromCopy(woman, fake_queen), TransformFromCopy(woman.label, fake_queen.label)), TransformFromCopy(diff, new_diff, time_span=(2, 3)), frame.animate.reorient(0, 2, 0, (0.04, 1.96, -0.13), 5.51).set_anim_args(run_time=3))
        ...
        self.play(FadeOut(fake_queen), FadeOut(fake_queen.label), FadeOut(new_diff))
        ...
        n_circs = 5
        src_circles = Circle(radius=0.01).set_stroke(width=5, opacity=1).replicate(n_circs)
        ...
        self.play(LaggedStart(*(Transform(src, trg) for src, trg in zip(src_circles, trg_circles)), lag_ratio=0.15, run_time=3))
        self.play(FadeIn(fake_queen), FadeIn(fake_queen.label))
        self.wait()
        self.play(TransformFromCopy(fake_queen, queen), TransformFromCopy(fake_queen.label, queen.label), VGroup(fake_queen, fake_queen.label).animate.set_opacity(0.2))
        self.play(FadeOut(fake_queen), FadeOut(fake_queen.label), frame.animate.reorient(103, 9, -101, (0.01, 1.45, 0.07), 6.72), run_time=10)
        word_pairs = [('uncle', 'aunt'), ('brother', 'sister'), ('nephew', 'niece'), ('father', 'mother'), ('son', 'daughter')]
        ...
        for word1, word2 in word_pairs:
            ...
            adj_point = np.array([np.random.uniform(-5, 0), np.random.uniform(2, 4), np.random.uniform(-3, 3)])
            ...
            self.play(LaggedStart(FadeOut(last_group), GrowArrow(vect1), FadeIn(vect1.label), GrowArrow(vect2), FadeIn(vect2.label)), *(FadeTransform(sm1, sm2) for sm1, sm2 in zip(last_equation, new_equation)))
    ...
    def get_equation1(self, word1, word2, word3, word4, colors=None):
        equation = TexText(f'{{{word1}}} - {{{word2}}} $\\approx$ {{{word3}}} - {{{word4}}}', font_size=48)
        ...
        pieces = VGroup(equation[f'{{{word1}}}'][0], equation['-'][0], equation[f'{{{word2}}}'][0], equation['$\\approx$'][0], equation[f'{{{word3}}}'][0], equation['-'][1], equation[f'{{{word4}}}'][0])
        pieces.fix_in_frame(True)
        return pieces

    def get_equation2(self, word1, word2, word3, word4, colors=None):
        equation = TexText(f'{{{word1}}} + {{{word2}}} - {{{word3}}} $\\approx$ {{{word4}}}', font_size=48)
        equation.fix_in_frame(True)
        equation.to_corner(UR)
        if colors:
            words = [word1, word2, word3, word4]
            for word, color in zip(words, colors):
                equation[word].set_fill(color)
        pieces = VGroup(equation[f'{{{word1}}}'], equation['+'], equation[f'{{{word2}}}'], equation['-'], equation[f'{{{word3}}}'], equation['$\\approx$ '], equation[f'{{{word4}}}'])
        pieces.fix_in_frame(True)
        return pieces
...
class HitlerMussoliniExample(KingQueenExample):
    ...
    def construct(self):
        ...
        self.add(equation[:-1])
        v1, v2, v3, v4 = vects = [self.get_labeled_vector(word, color=color) for word, color in zip(self.words, self.colors)]
        fudged_v4 = self.get_labeled_vector(self.words[3], axes.p2c(interpolate(v1.get_end() + v2.get_end() - v3.get_end(), v4.get_end(), self.interpolation_factor)), color=self.colors[3])
        vects[3] = fudged_v4
        for vect in vects:
            vect.apply_depth_test()
        diff = Arrow(v3.get_end(), v2.get_end(), buff=0, stroke_color=self.diff_color, stroke_width=2, flat_stroke=False)
        diff.apply_depth_test()
        rect = SurroundingRectangle(equation[2:5])
        rect.set_stroke(diff.get_stroke_color(), 2)
        self.play(GrowArrow(v2), GrowArrow(v3), FadeIn(v2.label), FadeIn(v3.label))
        self.play(ShowCreation(rect), Transform(v3.copy(), v2, remover=True), ShowCreation(diff))
        self.wait(2)
        diff_copy = diff.copy()
        diff_copy.shift(v1.get_end() - diff.get_start())
        self.play(GrowArrow(v1), FadeIn(v1.label), frame.animate.reorient(*self.second_frame_orientation))
        self.play(TransformFromCopy(diff, diff_copy), rect.animate.surround(equation[:5]))
        self.wait(2)
        self.play(rect.animate.surround(equation[-1]), FadeIn(equation[-1]), GrowArrow(fudged_v4), FadeIn(fudged_v4.label))
...
class SizeDirection(Word2VecScene):
    ...
    def construct(self):
        ...
        word_lists = [['micrometer', 'millimeter', 'meter'], ['microgram', 'milligram', 'gram'], ['microliter', 'milliliter', 'liter']]
        vect_groups = VGroup((VGroup((self.get_labeled_vector(word, color=color, func_name=None) for word, color in zip(word_list, colors))) for word_list in word_lists))
        ...
        self.play(frame.animate.reorient(49, 87, 0), LaggedStartMap(FadeIn, vect_groups[0], lag_ratio=0.25), LaggedStartMap(FadeIn, vect_groups[0].labels, lag_ratio=0.25), FadeIn(annotation, lag_ratio=0.1, time_span=(2, 3)), run_time=3)
        self.wait()
        for i in [0, 1]:
            self.play(ReplacementTransform(vect_groups[i], vect_groups[i + 1]), ReplacementTransform(vect_groups[i].labels, vect_groups[i + 1].labels))
            self.wait()
...
class PluralityDirection(Word2VecScene):
    ...
    def construct(self):
        ...
        colors = [BLUE, RED]
        cat, cats = [self.get_labeled_vector(word, coords=coords, color=color, buff=0.05) for word, color, coords in zip(words, colors, all_coords)]
...
class ShowNearestNeighbors(Word2VecScene):
    ...
    def construct(self):
        ...
        neighbors = VGroup(*(self.get_labeled_vector(word, color=WHITE) for word in nearest_words))
        ...
        for item, neighbor in zip(items, neighbors):
            ...
            self.play(FadeIn(item), FadeIn(neighbor), FadeOut(last_neighbor), FadeIn(faded_last_neighbor))
    ...
    def animate_in_neighbors(self, neighbors):
        ...
        for neighbor in neighbors:
            ...
            self.play(FadeIn(neighbor), FadeIn(neighbor.label), to_fade.animate.set_opacity(0.25))
...
class DotProducts(InteractiveScene):
    ...
    def construct(self):
        plane = NumberPlane((-4, 4), (-4, 4), background_line_style=dict(stroke_width=2, stroke_opacity=0.5, stroke_color=BLUE), faded_line_ratio=1)
        plane.set_height(6)
        plane.to_edge(LEFT, buff=0)
        vects = VGroup(Vector(0.5 * RIGHT + 2 * UP).set_stroke(MAROON_B, 6), Vector(1.0 * RIGHT + 0.5 * UP).set_stroke(YELLOW, 6))
        ...
        self.add(plane)
        self.add(vects)
        vect_labels = VGroup(*(Tex(f'\\vec{{\\textbf{{ {char} }} }}') for char in 'vw'))
        ...
        vect_coords = VGroup(*(TexMatrix([[char + f'_{{{str(n)}}}'] for n in [1, 2, 3, 4, 'n']], bracket_h_buff=0.1, ellipses_row=-2) for char in 'vw'))
        ...
        def dual_rotate(angle1, angle2, run_time=2):
            self.play(Rotate(vects[0], angle1 * DEGREES, about_point=plane.get_origin()), Rotate(vects[1], angle2 * DEGREES, about_point=plane.get_origin()), run_time=run_time)
        dual_rotate(-20, 20)
        dual_rotate(50, -60)
        dual_rotate(0, 80)
        dual_rotate(20, -80)
        ...
        prod_terms[-2].space_out_submobjects(0.001)
        ...
        plusses = VGroup(*(Tex('+', font_size=48).move_to(midpoint(m1.get_bottom(), m2.get_top())) for m1, m2 in zip(prod_terms.target, prod_terms.target[1:])))
        rhs.target = rhs.generate_target()
        rhs.target[0].rotate(PI / 2)
        rhs.target.arrange(DOWN)
        rhs.target.next_to(prod_terms, DOWN)
        self.add(equals)
        self.play(LaggedStart(*(TransformFromCopy(m1, m2) for m1, m2 in zip(entry_pairs, prod_terms)), lag_ratio=0.1, run_time=2), MoveToTarget(rhs))
        self.wait()
        self.play(MoveToTarget(prod_terms), rhs.animate.next_to(prod_terms.target, DOWN), LaggedStartMap(Write, plusses))
        self.wait()
        dual_rotate(-65, 65)
        ...
        dual_rotate((vects[1].get_angle() + PI / 2 - vects[0].get_angle()) / DEGREES, 0)
        self.remove(result)
        self.add(zero)
...
class DotProductWithPluralDirection(InteractiveScene):
    vec_tex = '\\vec{\\text{plur}}'
    ref_words = ['cat', 'cats']
    word_groups = [['puppy', 'puppies'], ['octopus', 'octopi', 'octopuses', 'octopodes'], ['student', 'students'], ['one', 'two', 'three', 'four']]
    x_range = (-4, 4 + 0.0001, 0.25)
    colors = [BLUE, RED]
    threshold = -1.0
    ...
    def construct(self):
        ...
        rhs = Tex(f'E(\\text{{{rf2}}}) - E(\\text{{{rf1}}})', tex_to_color_map={f'\\text{{{ref_word}}}': color for ref_word, color in zip(self.ref_words, self.colors)})
        ...
        self.play(LaggedStart(FadeIn(equals, 0.5 * LEFT), FadeIn(gen_lhs, 1.0 * LEFT)))
        self.wait()
        x_range = self.x_range
        number_line = NumberLine(x_range, big_tick_numbers=list(np.arange(*x_range[:2])), tick_size=0.05, longer_tick_multiple=3.0, width=12)
        number_line.rotate(PI / 2)
        number_line.add_numbers(np.arange(*x_range[:2]), num_decimal_places=1, font_size=40, direction=LEFT)
        ...
        self.play(top_eq.animate.scale(0.75).to_corner(UR), TransformFromCopy(gen_lhs, eq_lhs[0]), FadeIn(eq_lhs[1:], shift=DOWN), FadeIn(brace, shift=DOWN), UpdateFromAlphaFunc(eq_rhs, lambda m, a: m.set_value(a * dp).next_to(eq_lhs[-1], RIGHT), run_time=1), Write(number_line, run_time=1))
        ...
        for word_group in self.word_groups:
            for new_word in word_group:
                ...
                if new_word == word_group[0]:
                    ...
                    to_fade.add(ghost)
                self.play(Transform(equation_group, new_equation_group), added_anim)
                self.add_dot(new_lhs[2][2:-1], new_dot)
                to_fade.add(new_dot)

    def add_dot(self, word, dot):
        self.play(FadeInFromPoint(dot, word.get_center()), LaggedStart((FadeTransform(char.copy(), dot.copy().set_opacity(0)) for char in word), lag_ratio=0.02, group_type=Group), run_time=1)

    def get_equation_lhs(self, word):
        tex_pieces = [self.vec_tex, '\\cdot', f'E(\\text{{{word}}})', '=']
    ...
    def get_dot_with_key_word(self, word):
        if word == 'octopodes':
            ...
            return 1.8
        rf1, rf2 = self.ref_words
        return np.dot((self.model[rf2] - self.model[rf1]).flatten(), self.model[word].flatten())

class DotProductWithGenderDirection(DotProductWithPluralDirection):
    vec_tex = '\\vec{\\text{gen}}'
    ref_words = ['man', 'woman']
    words = ['mother', 'father', 'aunt', 'uncle', 'sister', 'brother', 'mama', 'papa']
    x_range = (-5, 7 + 0.0001, 0.25)
    colors = [BLUE, RED]
    threshold = 1.0
...
class RicherEmbedding(InteractiveScene):
    ...
    def construct(self):
        ...
        axes.shift(DOWN)
        plane = NumberPlane((-5, 5), (-5, 5), background_line_style=dict(stroke_width=1, stroke_color=BLUE_E), faded_line_ratio=1)
        ...
        self.play(TransformFromCopy(words[king_index], label), GrowArrow(vector))
        self.wait(3)
        index_labels = VGroup(*(Integer(n + 1, font_size=36).next_to(rect, DOWN, buff=0.2) for n, rect in enumerate(rects)))
        index_labels.fix_in_frame()
        idx_vect, idx_label = self.get_added_vector(vector.get_end(), 0.5 * (RIGHT + OUT), 'Pos. 2', TEAL, next_to_direction=UP, font_size=16)
        idx_label.rotate(45 * DEGREES, DOWN)
        idx_label.set_backstroke(BLACK, 1)
        self.play(LaggedStartMap(FadeIn, index_labels, shift=0.5 * DOWN))
        self.play(TransformFromCopy(index_labels[king_index].set_backstroke(), idx_label), GrowArrow(idx_vect), frame.animate.reorient(-28, -22, 0).set_anim_args(run_time=3))
        self.play(frame.animate.reorient(-11, -4, 0), LaggedStartMap(FadeOut, index_labels, lag_ratio=0.05, shift=0.5 * DOWN, time_span=(6, 7)), run_time=7)
        self.play(LaggedStart(*(ContextAnimation(words[king_index], [*words[:king_index], *words[king_index + 1:]], direction=DOWN, fix_in_frame=True, time_width=3, min_stroke_width=3, lag_ratio=0.05, path_arc=PI / 3) for n in range(3)), lag_ratio=0.5), frame.animate.reorient(-5, -12, 0), run_time=5)
        new_labeled_vector_args = [([2, 1, 0], 'lived in Scotland', None, DR), ([0, -1, -1], 'murdered predecessor', None, RIGHT), ([-1.5, 1, -2], 'in Shakespearean language', None, RIGHT)]
        ...
        self.play(GrowArrow(vect1), FadeIn(label1, 0.5 * DOWN), frame.animate.reorient(2, -16, 0, (0.6, -0.04, 0.02), 6.01).set_anim_args(run_time=8))
        self.play(GrowArrow(vect2), FadeIn(label2, 0.5 * DOWN), frame.animate.reorient(35, -23, 0, (0.6, -0.04, 0.02), 6.01).set_anim_args(run_time=5))
        self.play(GrowArrow(vect3), FadeIn(label3, 0.5 * DOWN), frame.animate.reorient(20, -29, 0, (0.61, 0.01, 0.0), 6.1).set_anim_args(run_time=5))
        self.play(frame.animate.reorient(-19, -25, 0, (0.61, 0.01, 0.0), 6.1), run_time=5)
...
class MultipleMoleEmbeddings(Word2VecScene):
    ...
    def construct(self):
        ...
        phrases = VGroup(map(Text, ['American shrew mole', 'One mole of carbon dioxide', 'Take a biopsy of the mole']))
        ...
        self.wait()
        self.play(GrowArrow(gen_vector), TransformFromCopy(mover, gen_vector.label))
        self.wait(10)
        images = Group(ImageMobject('ShrewMole'), Tex('6.02 \\times 10^{23}', font_size=24).set_color(BLUE), ImageMobject('LipMole'))
        for image in images[::2]:
            image.set_height(0.5)
            image.set_opacity(0.75)
        colors = [GREY_BROWN, BLUE, ORANGE]
        ref_vects = VGroup((self.get_labeled_vector('', coords=coords) for coords in [(-1.0, -1.5, 1.5), (-4.0, 0.5, 1.0), (-0.5, 1.0, 2.5)]))
        ...
        self.play(frame.animate.reorient(-30, -5, 0, (-1.11, 1.35, -0.72), 5.27), LaggedStart((TransformFromCopy(gen_vector, ref_vect) for ref_vect in ref_vects), lag_ratio=0.25, run_time=2), LaggedStart((FadeInFromPoint(image, gen_vector.label.get_center()) for image in images), lag_ratio=0.25, run_time=2, group_type=Group), gen_vect_group.animate.set_opacity(0.25).set_anim_args(run_time=2), run_time=2)
        self.wait(3)
        ...
        for n, diff in enumerate(diff_vects):
            ...
            phrase = phrases[n]
            self.play(gen_vect_group.animate.set_opacity(1), MoveToTarget(ref_vect_groups), FadeOut(last_phrase, UP), FadeIn(phrase, UP), FadeOut(last_diff))
            self.play(ShowCreation(diff, time_span=(1, 2)), TransformFromCopy(gen_vector, ref_vects[n], time_span=(1, 2)), ContextAnimation(phrase['mole'][0], phrase, direction=DOWN, fix_in_frame=True))
            self.wait(3)
            last_phrase = phrase
            last_diff = diff
        self.wait(5)
...
class RefineTowerMeaning(MultipleMoleEmbeddings):
    ...
    def construct(self):
        ...
        vectors = VGroup((self.get_labeled_vector('', coords=coords) for coords in [(4, -1, 3.0), (5, -2, 1.5), (-3, -1, 2.5)]))
        ...
        self.add(tower)
        self.wait()
        self.play(GrowArrow(vectors[0]), ReplacementTransform(pre_tower_image, tower_images[0]), run_time=2)
        ...
        self.play(frame.animate.reorient(-4, -7, 0, (2.95, 1.82, 0.49), 6.59), eiffel_tower.animate.set_opacity(1).arrange(RIGHT, aligned_edge=DOWN).to_edge(UP))
        self.play(vectors[0].animate.set_opacity(0.25), tower_images[-1].animate.set_opacity(0.2), TransformFromCopy(vectors[0], vectors[1]), FadeTransform(tower_images[-1].copy(), eiffel_tower_images[0]), ContextAnimation(words[2], words[1], direction=DOWN, fix_in_frame=True), run_time=2)
        for ti1, ti2 in zip(eiffel_tower_images, eiffel_tower_images[1:]):
            self.play(FadeTransform(ti1, ti2), run_time=2)
        self.wait(6)
        self.play(frame.animate.reorient(-14, -2, 0, (-0.12, 2.21, 0.72), 7.05).set_anim_args(run_time=2), words.animate.set_opacity(1).arrange(RIGHT, aligned_edge=DOWN).to_edge(UP))
        self.play(vectors[1].animate.set_opacity(0.25), eiffel_tower_images[-1].animate.set_opacity(0.2), TransformFromCopy(vectors[1], vectors[2]), FadeTransform(eiffel_tower_images[-1].copy(), mini_eiffel_tower_images[0]), ContextAnimation(words[2], words[0], direction=DOWN, fix_in_frame=True), run_time=2)
        self.wait(10)
...
class UpdatingPoetryEmbedding(RicherEmbedding):
    ...
    def construct(self):
        ...
        self.play(TransformFromCopy(words[one_index], label), GrowArrow(vector))
        self.wait(3)
        new_labeled_vector_args = [([2, 1, 0], 'of two roads', None, UL), ([2, -1, -1], 'symbolizing choice', None, UR), ([0.5, 1, -3], 'contrasting the original\nwith the familiar', None, DR)]
        new_labeled_vects = VGroup()
        last_vect = vector
        for args in new_labeled_vector_args:
            new_labeled_vects.add(self.get_added_vector(last_vect.get_end(), *args))
            last_vect = new_labeled_vects[-1][0]
            last_vect.apply_depth_test()
        orientation_args = [(-4, -12, 0, (-0.89, 0.03, -0.41), 8.1), (3, -9, 0, (-0.34, 0.49, -0.63), 8.6), (34, -14, 0, (-0.59, 0.49, -0.62), 9.2), (20, -29, 0, (0.61, 0.01, 0.0), 6.1)]
        for (vect, label), orientation in zip(new_labeled_vects, orientation_args):
            self.play(GrowArrow(vect, time_span=(2, 3)), FadeIn(label, 0.5 * DOWN, time_span=(2, 3)), frame.animate.reorient(*orientation).set_anim_args(run_time=6), ContextAnimation(one_rect, phrase[:-16], run_time=4, fix_in_frame=True, path_arc=60 * DEGREES, lag_ratio=0.001, direction=UP))
...
class SimpleSpaceExample(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait(5)
        ideas = VGroup(Text('Part of a command'), Text('Affectionate'), Text('Sadness'))
        ...
        for idea in ideas:
            ...
            self.play(FadeOut(last_idea), ShowCreation(new_vect), FadeIn(idea, new_vect.get_vector()), LaggedStartMap(ShowCreationThenFadeOut, lines, lag_ratio=2 / len(lines), run_time=2))
            self.wait(1)
            last_idea = VGroup(new_vect, idea)
            last_direction = direction
        self.play(FadeOut(last_idea))
        self.wait(5)
        ideas = VGroup(Text('needs an adjective next'), Text('preceded by "that which does not kill"'), Text('related to growth and strength'))
        ideas.scale(0.4)
        ideas.rotate(PI / 2, RIGHT)
        directions = [(-0.25, -1, 0.75), (-0.5, -0.25, 0.5), (1.0, -0.5, 1.0)]
        orientations = [(11, 92, 0, (2.69, 0.55, 1.12), 6.25), (-8, 83, 0, (2.73, 0.56, 1.24), 6.8), (-14, 79, 0, (2.49, 0.61, 1.41), 7.64)]
        vects = VGroup(vect)
        ...
        for idea, direction, orientation in zip(ideas, directions, orientations):
            ...
            self.play(frame.animate.reorient(*orientation), GrowArrow(new_vect), FadeIn(idea, 0.5 * new_vect.get_vector()))
            self.wait(2)
            vects.add(new_vect)
        self.wait(15)

    def add_plane_and_axes(self, x_range=(-4, 4), y_range=(-4, 4), z_range=(-3, 3)):
        axes = ThreeDAxes(x_range, y_range, z_range)
        plane = NumberPlane(x_range, y_range, background_line_style=dict(stroke_color=GREY_D, stroke_width=1), faded_line_ratio=1)
...
class ManyIdeasManyDirections(SimpleSpaceExample):
    ...
    def construct(self):
        ...
        frame.add_ambient_rotation()
        ideas = VGroup((Text(word) for word in ['Typewriter', 'Paradigm', 'Whimsical', 'Gelatinous', 'Rainbow', 'Serendipitous', 'Algorithm', 'Nebulous', 'Spatula', 'Lethargic', 'Effervescent', 'Asteroid', 'Pungent', 'Daydream', 'Mercurial', 'Cactus', 'Diaphanous', 'Hiccup', 'Viscous', 'Thunderclap']))
        ideas.set_backstroke(BLACK, 3)
        ideas.scale(0.5)
        ideas.rotate(PI / 2, RIGHT)
        last_idea = VGroup()
        ...
        for idea in ideas:
            ...
            self.play(FadeOut(last_idea), GrowArrow(new_vect), FadeIn(idea, new_vect.get_vector()), LaggedStartMap(ShowCreationThenFadeOut, lines, lag_ratio=1 / len(lines), run_time=1.5))
            self.wait()
            last_idea = VGroup(new_vect, idea)
            last_direction = direction
        self.play(FadeOut(last_idea))
...
class MJSpace(SimpleSpaceExample):
    ...
    def construct(self):
        ...
        self.add(token_rects, tokens)
        self.play(LaggedStartMap(FadeIn, arrs, shift=DOWN, lag_ratio=0.1), LaggedStartMap(GrowArrow, arrows, lag_ratio=0.1))
        self.wait()
        self.play(frame.animate.reorient(11, 76, 0, ORIGIN, FRAME_HEIGHT), FadeOut(VGroup(token_rects, tokens, arrows), UP, time_span=(1, 2)), LaggedStart((Transform(arrow, vect) for arrow, vect in zip(arrs, vects)), lag_ratio=0.05), run_time=3)
        self.remove(arrs)
        self.add(vects)
        self.wait()
        self.play(LaggedStart((vect.animate.scale(0, about_point=vect.get_start()) for vect in vects), lag_ratio=0.05, remover=True))
        ...
        orientations = [(17, 76, 0), (17, 80, 0), (-16, 77, 0)]
        for vect, label, orientation in zip(vects, labels, orientations):
            ...
            self.play(GrowArrow(vect), FadeIn(label, vect.get_vector()), frame.animate.reorient(*orientation))
        ...
        emb_label = Tex('\\vec{\\textbf{E}}', font_size=30)
        emb_label.rotate(89 * DEGREES, RIGHT)
        emb_label.add_updater(lambda m: m.move_to(1.1 * emb.get_end()))
        emb_label.suspend_updating()
        self.play(frame.animate.reorient(7, 66, 0).set_anim_args(run_time=2), FadeIn(emb, shift=2 * (IN + LEFT)), FadeIn(emb_label, shift=2 * (IN + LEFT)))
        self.wait()
        ...
        def get_dot_product_lines(vect, proj_line_color=GREY_A):
            dashed_line = always_redraw(lambda: Line(emb.get_end(), get_proj_point(emb, vect)).set_stroke(WHITE, 2).set_anti_alias_width(10))
            proj_line = always_redraw(lambda: Line(ORIGIN, get_proj_point(emb, vect)).set_stroke(proj_line_color, width=4, opacity=0.75))
            return (dashed_line, proj_line)
        m_dashed_line, m_proj_line = get_dot_product_lines(vects[0])
        formula = Tex('\\vec{\\textbf{E}} \\cdot \\big(\\overrightarrow{\\text{First Name Michael}}\\big) = ', font_size=36)
        ...
        self.play(ShowCreation(m_dashed_line), TransformFromCopy(Line(ORIGIN, emb.get_end(), flat_stroke=False), m_proj_line), FadeIn(formula, UP), vect_groups[1:].animate.set_opacity(0.25))
        m_proj_line.resume_updating()
        self.play(TransformFromCopy(rhs.copy().unfix_from_frame().set_opacity(0).move_to(m_proj_line), rhs))
        emb_label.resume_updating()
        for _ in range(2):
            self.play(emb.animate.put_start_and_end_on(ORIGIN, [-2.5, -2.0, -0.5]), rate_func=wiggle, run_time=5)
        self.wait(2)
        ...
        self.wait(2)
        self.play(frame.animate.reorient(-4, 73, 0), emb.animate.put_start_and_end_on(ORIGIN, emb_coords), run_time=3)
        self.wait(5)
        j_dashed_line, j_proj_line = get_dot_product_lines(vects[1])
        j_paren = Tex('\\big(\\overrightarrow{\\text{Last Name Jordan}}\\big) = ', font_size=36)
        ...
        self.play(frame.animate.reorient(15, 97, 0), FadeOut(m_paren, UP, time_span=(1, 2)), FadeIn(j_paren, UP, time_span=(1, 2)), rhs.animate.next_to(j_paren, RIGHT, buff=0.15).set_anim_args(time_span=(1, 2)), LaggedStart(vect_groups[0].animate.set_opacity(0.25), vect_groups[1].animate.set_opacity(1), FadeOut(m_dashed_line), FadeOut(m_proj_line), lag_ratio=0.25, run_time=2))
        j_proj_line.suspend_updating()
        self.play(ShowCreation(j_dashed_line), TransformFromCopy(Line(ORIGIN, emb.get_end(), flat_stroke=False), j_proj_line))
        j_proj_line.resume_updating()
        self.play(emb.animate.put_start_and_end_on(ORIGIN, [-1.5, -1.5, 0]).set_anim_args(run_time=3, rate_func=there_and_back))
        self.wait()
        b_dashed_line, b_proj_line = get_dot_product_lines(vects[2])
        b_paren = Tex('\\big(\\overrightarrow{\\text{Basketball}}\\big) = ', font_size=36)
        b_paren[:-1].set_color(vects[2].get_color())
        b_paren.move_to(m_paren, LEFT)
        b_paren.fix_in_frame()
        rhs.suspend_updating()
        self.play(frame.animate.reorient(2, 65, 0), FadeOut(j_paren, UP), FadeIn(b_paren, UP), rhs.animate.next_to(b_paren[-1], RIGHT, buff=0.2).set_value(0), FadeOut(j_dashed_line), FadeOut(j_proj_line), vect_groups[1].animate.set_opacity(0.25), vect_groups[2].animate.set_opacity(1.0))
        self.wait()
        rhs.target_vect = vects[2]
        rhs.resume_updating()
        self.add(b_dashed_line, b_proj_line)
        self.play(emb.animate.put_start_and_end_on(ORIGIN, [0.6, -2.2, 0]), rate_func=there_and_back, run_time=6)
        self.wait(3)
        self.play(frame.animate.reorient(5, 85, 0).set_anim_args(run_time=2), FadeOut(formula[:3]), FadeOut(b_paren), FadeOut(rhs), FadeOut(b_dashed_line), FadeOut(b_proj_line), vect_groups[:2].animate.set_opacity(1), vect_groups[2].animate.set_opacity(0.25))
        self.wait()
        self.play(ShowCreation(m_dashed_line), ShowCreation(m_proj_line))
        self.wait()
        self.play(ShowCreation(j_dashed_line), ShowCreation(j_proj_line))
        self.wait(20)
        self.play(*map(FadeOut, [j_dashed_line, j_proj_line, m_dashed_line, m_proj_line, emb, emb_label]))
        ...
        self.play(LaggedStart(TransformFromCopy(vects[1], m_vect_copy), TransformFromCopy(vects[0], j_vect_copy), lag_ratio=0.5))
        self.play(GrowArrow(emb))
        self.wait(4)
        self.play(*map(FadeOut, [m_vect_copy, j_vect_copy, emb]))
        self.play(frame.animate.reorient(-19, 77, 0, (1.32, -0.22, -0.12), 3.75), vect_groups[:2].animate.set_opacity(0.25), vect_groups[2][0].animate.set_opacity(1.0), vect_groups[2][1].animate.set_opacity(1.0), run_time=2)
        self.wait(20)