...
class LoopScene(InteractiveScene):
    ...
    def get_dot_group(self, vect_tracker: ValueTracker, loop_func: Callable[[float], Vect3], colors=None, radius: float=0.05, glow_factor: float=1.5):
        n = len(vect_tracker.get_value())
        if colors is None:
            colors = [random_bright_color() for _ in range(n)]
        dots = Group((get_special_dot(color, radius=radius, glow_factor=glow_factor) for _, color in zip(range(n), it.cycle(colors))))
    ...
    def get_movable_pair(self, uv_tracker: ValueTracker, loop_func: Callable[[float], Vect3], colors=[YELLOW, PINK], radius: float=0.05, glow_factor: float=1.5):
        return self.get_dot_group(uv_tracker, loop_func, colors, radius, glow_factor)

    def get_movable_quad(self, abcd_tracker: ValueTracker, loop_func: Callable[[float], Vect3], colors=[YELLOW, MAROON_B, PINK, RED], radius: float=0.05, glow_factor: float=1.5):
    ...
    def get_dot_polygon(self, dots, stroke_color=BLUE, stroke_width=3):
        ...
        polygon.add_updater(lambda m: m.set_points_as_corners([*(d.get_center() for d in dots), dots[0].get_center()]))
        return polygon
...
class StateThePuzzle(LoopScene):
    ...
    def construct(self):
        ...
        inscribed_words = TexText("``Inscribed\\\\Square''", font_size=72)
        ...
        self.wait()
        new_square_params = [[0.519, 0.308, 0.277, 0.177], [0.444, 0.105, 0.877, 0.65], [0.037, 0.739, 0.468, 0.372]]
        ...
        def true_find_square(loop_func, trg_angle=90 * DEG, cost_tol=0.01, max_tries=8):
            ...
            return min_params
        new_loops = [get_example_loop(1), get_example_loop(2), Tex('\\pi').family_members_with_points()[0], Tex('\\epsilon').family_members_with_points()[0], get_example_loop(1)]
        og_loop = loop.copy()
        ...
        for new_loop in new_loops:
            ...
            self.play(Transform(loop, new_loop), run_time=1)
            self.add(dots, polygon)
            for _ in range(5):
        ...
        self.play(VGroup(square_word, red_line).animate.shift(0.75 * DOWN), Write(rect_word), q_mark.animate.next_to(rect_word, RIGHT, SMALL_BUFF, aligned_edge=UP))
        self.wait()
        ...
        for new_loop in [og_loop, *new_loops[1:3]]:
            self.play(Transform(loop, new_loop), UpdateFromFunc(quad_tracker, lambda m: m.set_value(true_find_square(loop_func, 60 * DEG))), run_time=5)
            self.wait()
...
class ReframeToPairsOfPoints(LoopScene):
    ...
    def construct(self):
        ...
        question1 = VGroup(Text('Find four points'), Tex('(A, B, C, D)'), Text('That form a rectangle'))
        question2 = VGroup(Text('Find two pairs of points', t2s={'pairs': ITALIC}), Tex('\\{\\{A, C\\}, \\{B, D\\}\\}', t2c={'A': YELLOW, 'B': RED, 'C': YELLOW, 'D': RED}), Text('With the same midpoint\nand distance apart'))
        ...
        polygon.update()
        self.play(ShowCreation(polygon, suspend_mobject_updating=True), loop.animate.set_stroke(width=2, opacity=0.5), run_time=2)
        self.wait()
        ...
        self.play(FadeOut(polygon), FadeOut(question1[2], DOWN), TransformMatchingStrings(question1[0], question2[0], key_map={'four': 'two pairs of'}, mismatch_animation=FadeTransformPieces), TransformMatchingTex(question1[1], question2[1]), dots[2].animate.set_color(YELLOW), dots[1].animate.set_color(RED), labels[2].animate.set_fill(YELLOW), labels[1].animate.set_fill(RED))
        self.wait()
        ...
        self.play(FadeIn(mid_dot1), GrowArrow(arrow), FadeIn(question2[2]['With the same midpoint'], lag_ratio=0.1))
        self.play(FlashAround(question2[2]['midpoint'], color=TEAL), question2[2]['midpoint'].animate.set_color(TEAL))
        self.wait()
        frame = self.frame
        ...
        self.play(TransformFromCopy(lines, new_lines), Write(question2[2]['and distance apart']), run_time=2)
        self.play(question2[2]['distance apart'].animate.set_color(YELLOW), FlashUnder(question2[2]['distance apart']), run_time=1)
        self.wait()
        dots.clear_updaters()
        ...
        self.play(LaggedStartMap(FadeOut, Group(mid_dot1, arrow, loop, new_lines)), question2[2].animate.set_opacity(0.25), line1.animate.scale(0.35).rotate(45 * DEG).shift(UP), line2.animate.scale(0.9).rotate(-30 * DEG).shift(DOWN), run_time=2)
        self.wait()
        for dot in (mid_dot1, mid_dot2):
            dot.set_color(WHITE)
            dot.scale(0.5)
        target_midpoint = midpoint(line1.get_center(), line2.get_center())
        self.play(*map(FadeIn, [mid_dot1, mid_dot2]), question2[2]['With the same midpoint'].animate.set_fill(opacity=1))
        self.play(line1.animate.move_to(target_midpoint), line2.animate.move_to(target_midpoint))
        self.wait()
        self.play(line1.animate.set_length(line2.get_length()), question2[2]['and distance apart'].animate.set_opacity(1), run_time=2)
        self.wait()
        polygon.update()
        self.play(ShowCreation(polygon, suspend_mobject_updating=True))
...
class ShowTheSurface(LoopScene):
    ...
    def construct(self):
        ...
        self.play(UpdateFromFunc(uv_tracker, lambda m: m.set_value(np.random.random(2))))
        ...
        self.play(Write(coords, suspend_mobject_updating=True), FadeIn(midpoint_dot, scale=0.5))
        self.wait()
        ...
        self.play(TransformFromCopy(coords[:4], coords_3d[:4]), TransformFromCopy(d_label, coords_3d[4:6]), TransformFromCopy(coords[4:], coords_3d[6:]), run_time=2)
        ...
        self.play(frame.animate.reorient(5, 79, 0, (0.4, 0.01, 1.41), 5.07), FadeIn(axes), ReplacementTransform(coords_3d, top_dot_coords), TransformFromCopy(midpoint_dot, top_dot, suspend_mobject_updating=True), run_time=3)
        self.play(frame.animate.reorient(-21, 84, 0, (0.4, 0.01, 1.41), 5.07), run_time=5)
        self.play(frame.animate.reorient(0, 18, 0, (0.41, -0.13, 1.34), 4.76), run_time=2)
        self.play(FlashAround(coords, run_time=2, time_width=1.5))
        self.play(TransformFromCopy(connecting_line, z_line, suspend_mobject_updating=True, time_span=(4, 6)), frame.animate.reorient(-38, 88, 0, (1.19, -0.11, 1.48), 3.94), run_time=6)
        ...
        self.play(frame.animate.reorient(12, 78, 0, (-0.48, 0.08, 1.15), 5.27), LaggedStartMap(FadeIn, dot_group2), LaggedStartMap(FadeOut, Group(d_label, brace, coords, top_dot_coords)), run_time=4)
        self.play(uv_tracker2.animate.set_value([0.1, 0.2]), run_time=8)
        self.wait()
        nudge = 0.01
        for _ in range(3):
            self.play(uv_tracker.animate.increment_value(np.random.uniform(-nudge, nudge, 2)), uv_tracker2.animate.increment_value(np.random.uniform(-nudge, nudge, 2)), run_time=2, rate_func=lambda t: wiggle(t, 7))
        self.wait()
        rect_params = find_rectangle(loop_func, initial_condition=[0.54, 0.59, 0.73, 0.31], n_refinements=5, target_angle=64 * DEG)
        self.play(frame.animate.reorient(-18, 67, 0, (0.11, 0.07, 0.75), 4.63), uv_tracker.animate.set_value(rect_params[0::2]), uv_tracker2.animate.set_value(rect_params[1::2]), run_time=12)
        self.wait()
        ...
        self.play(FadeOut(z_group), FadeOut(axes), frame.animate.reorient(0, 0, 0, ORIGIN, 4.75), run_time=4)
        self.play(ShowCreation(rect), loop.animate.set_stroke(width=2))
        self.wait(2)
        self.play(FadeOut(rect), FadeIn(z_group), FadeIn(axes), frame.animate.reorient(22, 85, 0, (-0.33, 0.45, 1.52), 6.78), run_time=3)
        ...
        self.play(FadeIn(mesh), FadeIn(surface), FadeOut(top_dot.paths), FadeOut(top_dot2.paths), frame.animate.reorient(-29, 81, 0, (0.14, 0.39, 2.1), 6.47).set_anim_args(run_time=8))
        self.wait(15)
        self.play(FadeOut(dot_group1), FadeOut(dot_group2))
        ...
        self.play(surface.animate.set_opacity(1), mesh.animate.set_stroke(width=0, opacity=0))
        self.add(top_mesh)
        self.play(FadeIn(cross_plane))
        self.play(z_tracker.animate.set_value(0.25), surface.animate.set_color(BLUE_E, 1), run_time=8)
        self.wait(3)
        ...
        self.play(FadeIn(dot_group1), FadeIn(dot_group2), surface.animate.set_opacity(0.75))
        frame.clear_updaters()
        self.play(uv_tracker.animate.set_value(rect_params[0::2]), uv_tracker2.animate.set_value(rect_params[1::2]), FadeOut(z_line2, time_span=(2.5, 3)), FadeOut(top_dot2, time_span=(2.5, 3)), frame.animate.reorient(-3, 49, 0, (0.76, 0.62, 0.49), 6.46), run_time=3)
        ...
        self.play(ShowCreation(rect, suspend_mobject_updating=True))
        self.wait()
        self.play(z_tracker.animate.set_value(target_z), frame.animate.reorient(-3, 47, 0, (0.27, 0.98, 0.85), 3.8), top_mesh.animate.set_stroke(opacity=0.01), run_time=7)
        self.wait()
        traced_path = TracingTail(top_dot, stroke_color=WHITE, time_traced=5)
        self.add(traced_path)
        self.play(frame.animate.reorient(-3, 48, 0, (0.15, 0.76, 0.6), 5.26), run_time=4)
        ...
        for _ in range(5):
            new_rect_params = find_rectangle(loop_func, initial_condition=np.random.random(4), target_angle=np.random.uniform(30 * DEG, 90 * DEG))
            new_z = get_dist(*map(loop_func, new_rect_params[::2]))
            self.play(abcd_tracker.animate.set_value(new_rect_params), z_tracker.animate.set_value(new_z))
            self.wait()
        self.play(frame.animate.reorient(-1, 48, 0, (0.29, 1.22, 1.26), 8.99), z_tracker.animate.set_value(surface.get_z(OUT)), FadeOut(rect), FadeOut(dot_group2), run_time=10)
        self.wait()
        self.remove(abcd_tracker)
        uv_tracker.clear_updaters()
        self.play(z_tracker.animate.set_value(0.25), top_mesh.animate.set_stroke(opacity=0.1), frame.animate.reorient(-7, 78, 0, (0.31, 1.16, 0.94), 7.21), run_time=6)
        ...
        self.play(uv_tracker.animate.set_value([v + 0.01, v]), glow_tracker.animate.set_value(0.25), radius_tracker.animate.set_value(0.025), frame.animate.reorient(-7, 78, 0, (0.1, 1.02, 0.18), 3.8), FadeOut(midpoint_dot), run_time=5)
        self.play(uv_tracker.animate.set_value([0.26, 0.25]), run_time=6)
        self.wait()
        self.play(uv_tracker.animate.set_value([0.25, 0.25]), frame.animate.reorient(-7, 78, 0, (-0.15, 1.0, -0.07), 2.8), z_tracker.animate.set_value(0.01), run_time=7)
        self.play(frame.animate.reorient(2, 70, 0, (0.05, 1.06, 0.05), 4.29), run_time=10)
        self.wait()

    def get_axes_and_plane(self, x_range=(-3, 3), y_range=(-3, 3), z_range=(0, 5), depth=4):
    ...
    def get_top_dot(self, z_line, color=BLUE, radius=0.05, glow_factor=1.0):
        top_dot = Group(TrueDot(radius=radius).make_3d(), GlowDot(radius=radius * 2, glow_factor=glow_factor))
    ...
    def animate_to_rectangle_with_angle(self, abcd_tracker, loop_func, target_angle, n_samples=5, run_time=5, param_range_per_step=0.1, n_samples_per_range=10, n_refinements=3):
        ...
        for angle in np.linspace(curr_angle, target_angle, n_samples + 1)[1:]:
            rectangle_range.append(find_rectangle(loop_func=loop_func, initial_condition=rectangle_range[-1], target_angle=angle, initial_param_range=param_range_per_step, n_samples_per_range=n_samples_per_range, n_refinements=n_refinements))
        rectangle_range = np.array(rectangle_range)
        self.play(UpdateFromAlphaFunc(abcd_tracker, lambda m, a: m.set_value(smooth_index(rectangle_range, a))), run_time=run_time)

    def get_surface_info(self, loop_func: Callable[[float], Vect3], surface_color=BLUE, surface_opacity=0.25, surface_resolution=(101, 101), mesh_color=WHITE, mesh_stroke_width=0.5, mesh_stroke_opacity=0.1, mesh_resolution=(101, 101)):
        surface_func = get_surface_func(loop_func)
        surface = ParametricSurface(get_half_parametric_func(surface_func), resolution=surface_resolution)
...
class ChangeTheSurface(ShowTheSurface):
    ...
    def construct(self):
        ...
        frame.add_ambient_rotation()
        self.add(axes, plane)
        example_loops = VGroup(get_example_loop(1), get_example_loop(2), SVGMobject('gingerbread_outline')[0])
        ...
        for next_loop in example_loops[1:]:
            self.play(Transform(loop, next_loop), UpdateFromFunc(surface, update_surface), run_time=8)
            self.wait(2)
        circle = Circle(radius=2)
        circle.set_stroke(WHITE, 4)
        self.play(Transform(loop, circle), UpdateFromFunc(surface, update_surface), run_time=5)
        loop.become(circle)
        surface.always_sort_to_camera(self.camera)
        ...
        rect.f_always.set_points_as_corners(lambda: list(map(loop_func, [get_x(), 0.5 - get_x(), get_x() + 0.5, 1.0 - get_x(), get_x()])))
        rect_group = Group(dots1, dots2, line1, line2, z_line, top_dot, rect)
        self.add(uv_tracker1, uv_tracker2)
        self.play(FadeIn(rect_group), frame.animate.reorient(-14, 31, 0, (-0.08, -0.56, 1.19), 8.04), run_time=3)
        for value in [0.24, 0.01, 0.125]:
            self.play(x_tracker.animate.set_value(value), run_time=6)
        self.wait()
        ellipse = circle.copy().stretch(0.5, 1)
        self.play(frame.animate.reorient(-11, 67, 0, (-0.08, -0.56, 1.19), 8.04), run_time=2)
        self.play(Transform(loop, ellipse), UpdateFromFunc(surface, update_surface), run_time=5)
...
class ParameterizeTheLoop(InteractiveScene):
    ...
    def construct(self):
        ...
        sl_points = np.array(loop.get_points())
        ...
        self.play(Transform(moving_loop, snipped_loop, time_span=(0.5, 1.5)), scissors.animate.shift(-2 * scissors_shift), run_time=2)
        ...
        self.wait()
        self.play(FadeIn(loop_x_group), FadeIn(int_x_group), loop.animate.set_stroke(opacity=1), FadeOut(moving_loop))
        ...
        circle_ticks = VGroup((Line(1.1 * point, 0.9 * point) for a in np.linspace(0, 1, 11) for point in [circular_interval.pfp(a)]))
        ...
        interval.save_state()
        self.play(FadeOut(int_x_group), Transform(interval, circular_interval, run_time=3))
        ...
        self.play(x_tracker.animate.set_value(0.15), y_tracker.animate.set_value(0.25), run_time=2)
        x_axis = interval
        y_axis = UnitInterval()
        ...
        self.play(FadeIn(y_axis), VFadeIn(int_x_group), x_axis.animate.shift(y_axis.n2p(0) - x_axis.n2p(0)), FadeTransformPieces(loop_y_group.copy(), int_y_group), run_time=2)
        ...
        v_line.add_updater(lambda m: m.put_start_and_end_on(axes.c2p(get_x(), 0), axes.c2p(get_x(), get_y())))
        h_line.add_updater(lambda m: m.put_start_and_end_on(axes.c2p(0, get_y()), axes.c2p(get_x(), get_y())))
        coord_lines = VGroup(v_line, h_line)
        coord_label = Tex('(0.00, 0.00)', font_size=24)
        ...
        self.play(FadeIn(square), ShowCreation(v_line), ShowCreation(h_line), y_tip.animate.flip(UP, about_edge=LEFT), x_tip.animate.flip(RIGHT, about_edge=DOWN), x_dec.animate.move_to(coord_label[1]), y_dec.animate.move_to(coord_label[3]), FadeIn(coord_label), FadeIn(square_point, scale=0.5))
        ...
        self.play(xy_tracker.animate.set_value([0.5, get_y()]), run_time=4)
        self.play(xy_tracker.animate.set_value([get_x(), 0.2]), run_time=4)
        ...
        self.play(frame.animate.set_height(9), ShowCreation(left_edge), xy_tracker.animate.set_value([0, 1]), run_time=12)
        xy_tracker.set_value([1, 0])
        self.play(ShowCreation(right_edge), xy_tracker.animate.set_value([1, 1]), run_time=8)
        self.wait()
        self.play(Write(left_tips), Write(right_tips))
        ...
        self.play(xy_tracker.animate.set_value([1, 0]), ShowCreation(bottom_edge), Write(bottom_tips, time_span=(2, 4)), run_time=4)
        xy_tracker.set_value([0, 1])
        self.play(xy_tracker.animate.set_value([1, 1]), ShowCreation(top_edge), Write(top_tips, time_span=(2, 4)), run_time=4)
        h_arrows = VGroup(VGroup(bottom_edge, bottom_tips), VGroup(top_edge, top_tips))

        def half_torus_func(u, v):
            return torus_func(u, 0.5 * v)
        surfaces = Group((TexturedSurface(ParametricSurface(func), 'TorusTexture') for func in [square_func, tube_func, half_torus_func, torus_func]))
        ...
        self.add(surface)
        self.play(FadeIn(surface, shift=target_z * OUT), FadeIn(cover_rect), frame.animate.reorient(-13, 61, 0, (1.52, 1.67, 1.97), 15.41), run_time=3)
        ...
        self.play(FadeOut(cover_rect), loop.animate.set_height(6).next_to(y_axis, LEFT, buff=1.5), frame.animate.reorient(0, 0, 0, (0.44, 1.84, 0.0), 13.21), torus.animate.set_height(7).rotate(50 * DEG, LEFT).move_to(6 * UP), torus.animate.set_height(7).rotate(50 * DEG, LEFT).move_to(6 * UP).match_x(square), v_arrows.animate.set_opacity(0.25), h_arrows.animate.set_opacity(0.25), coord_label.animate.scale(1.5), run_time=3)
        torus_mesh = SurfaceMesh(torus, resolution=(21, 21))
        torus_mesh.set_stroke(WHITE, 0.5, 0.5)
        self.add(torus_point, torus_mesh, torus)
        self.play(Write(torus_mesh, lag_ratio=0.01, stroke_width=0.5, run_time=1), FadeIn(torus_point))
        target_xys = [[0.13, 0.25], [0.13, 0.65], [0.13, 0.35], [0.97, 0.35], [0.1, 0.35]]
        for xy in target_xys:
            self.play(xy_tracker.animate.set_value(xy), run_time=4)
        for _ in range(3):
            self.play(xy_tracker.animate.increment_value(0.02 * np.random.uniform(-1, 1, 2)), run_time=2, rate_func=lambda t: wiggle(t, 7))
            self.wait()
        self.play(frame.animate.reorient(0, 0, 0, (-0.57, 0.46, 0.0), 10), FadeOut(torus, UP), FadeOut(torus_mesh, UP), FadeOut(torus_point, UP), run_time=2)
        self.wait()
        coord_ghosts = Group()
        ...
        def get_double_arrow():
            ...
            result = VGroup(Arrow(point1, point2).shift(0.1 * vect), Arrow(point2, point1).shift(-0.1 * vect))
        ...
        def show_reflection():
            ...
            self.play(GrowFromPoint(get_double_arrow(), square_point.get_center()), xy_tracker.animate.set_value([get_y(), get_x()]), Swap(x_dot, y_dot), run_time=1)
        ...
        self.play(ShowCreation(fold_line), *map(FadeOut, [v_line, h_line, coord_label, square_point, x_tip, y_tip]))
        self.wait()
        self.play(FadeOut(coord_ghosts), FadeOut(double_arrows), v_arrows.animate.set_opacity(1), h_arrows.animate.set_opacity(1))
        ...
        for triangle in [ul_triangle, dr_triangle]:
            ...
            self.add(triangle)
        self.remove(square)
        self.play(Rotate(ul_triangle, PI, about_point=square.get_center(), axis=UR), Rotate(v_arrows[0], PI, about_point=square.get_center(), axis=UR), Rotate(h_arrows[1], PI, about_point=square.get_center(), axis=UR), run_time=2)
        self.remove(v_arrows)
        self.play(h_arrows.animate.set_color(PURPLE))
        folded_square = Group(dr_triangle, h_arrows, fold_line).copy()
        self.play(FadeIn(square_point), FadeIn(coord_label))
        ...
        self.play(FlashAround(coord_label))
        self.play(LaggedStartMap(FadeOut, Group(loop_x_group[1], loop_y_group[1], coord_label, square_point, x_axis, y_axis)))
        self.wait()
        ...
        self.play(ShowCreation(cut_line), Write(cut_tips, time_span=(0.5, 1.5)), FadeIn(pieces), FadeOut(old_tris), run_time=1.5)
        ...
        self.wait()
        self.play(piece1.animate.shift(square.get_center() - piece1[0].get_right()), piece2.animate.shift(square.get_center() - piece2[0].get_left()))
        self.play(piece1[1].animate.set_opacity(0), piece2[1].animate.set_opacity(0))
        self.wait()
        ...
        def get_partial_strip(upper_theta=1.0):
            result = ParametricSurface(lambda u, v: mobius_strip_func(u, custom_squish(v) * upper_theta / TAU))
            result.scale(2, about_point=ORIGIN)
            result.shift((0, 4, 4))
            return result
        surfaces = Group((TexturedSurface(plain_surface, 'MobiusStripTexture') for plain_surface in [ParametricSurface(square_func), get_partial_strip(2.0)]))
        for surface in surfaces:
            surface.set_shading(0.25, 0.25, 0)
            surface.set_opacity(0.75)
        target_z = 4
        ...
        self.play(FadeIn(cover_rect), FadeIn(surface, shift=target_z * OUT), frame.animate.reorient(2, 51, 0, (-0.35, 3.04, 0.42), 15.36), run_time=3)
        self.play(Transform(surface, partial_strip, run_time=2))
        self.play(UpdateFromAlphaFunc(surface, lambda m, a: m.set_points(get_partial_strip(interpolate(2, TAU, smooth(a))).get_points())), run_time=5)
        self.play(frame.animate.reorient(0, 42, 0, (-0.11, 2.48, 0.87), 13.94), Rotate(surface, PI, axis=RIGHT), run_time=8)
        mobius_strip = surface
        self.play(frame.animate.reorient(0, 0, 0, (-1.02, 3.21, 0.0), 14.55), mobius_strip.animate.rotate(40 * DEG, LEFT).move_to(7.5 * UP), loop.animate.scale(1.25, about_edge=RIGHT), FadeOut(pieces), FadeIn(x_axis), FadeIn(y_axis), FadeIn(folded_square), FadeIn(square_point), FadeIn(coord_label), FadeIn(loop_x_group[1]), FadeIn(loop_y_group[1]))
        ...
        xy_values = [[0.5, 0.25], [0.9, 0.1], [0.8, 0.7], [0.53, 0.12], [0.0, 0.0]]
        for xy in xy_values:
        ...
        self.play(mobius_strip.animate.shift(7 * LEFT), GrowFromCenter(torus_group), FadeIn(torus_point))
        self.play(ShowCreation(torus_fold_line), xy_tracker.animate.set_value([0.99, 0.99]), run_time=5)
        self.wait()
        squished_torus = TexturedSurface(ParametricSurface(lambda u, v: mobius_strip_func(*torus_uv_to_mobius_uv(u, v))), 'TorusTexture')
        ...
        self.play(Transform(torus, squished_torus), Transform(torus_mesh, squished_torus_mesh), torus_fold_line.animate.set_points(new_fold_line.get_points()), run_time=5)
        self.wait()
...
class DiscussOrderOfPoints(LoopScene):
    ...
    def construct(self):
        ...
        self.add(labels)
        dots.clear_updaters()
        question = TexText('Is $(A, B)$ distinct from $(B, A)$?', font_size=60)
        question.to_edge(UP)
        ...
        self.play(GrowFromCenter(line, suspend_mobject_updating=True), FadeIn(mid_dot), FadeIn(midpoint_word, lag_ratio=0.1))
        self.play(Swap(*dots))
        self.play(TransformMatchingStrings(midpoint_word, dist_word, key_map={'midpoint': 'distance'}, run_time=1))
        self.play(Swap(*dots))
        ...
        rect.update()
        self.play(FadeOut(loop), FadeIn(pair_group2), VFadeIn(rect), frame.animate.move_to(mid_dot.get_center() + 0.5 * UP).set_height(6))
        self.play(Rotate(pair_group2, -angle, about_point=mid_dot.get_center()), run_time=5)
        self.wait()
        self.play(Rotate(pair_group2, -angle, about_point=mid_dot.get_center()), run_time=5)
...
class MapTheStripOntoTheSurface(ShowTheSurface):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(-38, 82, 0, (-0.02, 0.06, 1.73), 8.03), FadeIn(pair_group), FadeIn(surface), Write(mesh, stroke_width=1, lag_ratio=0.001, time_span=(2, 4)), run_time=4)
        pair_group.add(dots)
        self.add(pair_group)
        frame.add_ambient_rotation()
        self.wait(20)
        ...
        edge_parts = VGroup(ParametricCurve(lambda t: mobius_strip_func(0, t)), ParametricCurve(lambda t: mobius_strip_func(1, t)))
        ...
        self.play(frame.animate.reorient(0, 68, 0, (-3.7, -0.17, 1.95), 9.56), FadeIn(strip_group), FadeIn(strip_point), run_time=3)
        self.wait(20)
        uv_tracker.clear_updaters()
        pre_surface = ParametricSurface(get_half_parametric_func(uv_to_strip_point))
        pre_surface.match_style(strip)
        pre_mesh = SurfaceMesh(ParametricSurface(uv_to_strip_point), resolution=mesh.resolution, normal_nudge=0)
        ...
        self.play(Transform(moving_surface, pre_surface), Transform(moving_mesh, pre_mesh), frame.animate.reorient(-25, 71, 0, (-4.86, 0.35, 1.64), 9.84), run_time=10)
        self.wait()
        self.play(ReplacementTransform(moving_surface, surface), ReplacementTransform(moving_mesh, mesh), frame.animate.reorient(36, 68, 0, (-3.94, 0.76, 0.7), 10.14), run_time=10)
        self.play(frame.animate.reorient(0, 60, 0, (-3.7, 0.73, 0.64), 10.44), FadeIn(strip_point), FadeIn(pair_group), run_time=5)
        u, v = uv_tracker.get_value()
        self.play(uv_tracker.animate.set_value([int(2 * u), int(2 * v)]), loop.animate.set_stroke(opacity=0.2), run_time=2)
        uv_tracker.set_value([0, 0])
        edge.set_stroke(width=3)
        loop_copy = loop.copy().set_stroke(opacity=1)
        self.play(ShowCreation(edge), ShowCreation(loop_copy), uv_tracker.animate.set_value([0.999, 0.999]), run_time=5)
        ...
        self.play(FadeIn(moving_surface), FadeIn(moving_mesh), FadeOut(strip_point), FadeOut(pair_group))
        self.play(Transform(moving_group, post_group), frame.animate.reorient(18, 65, 0, (-3.85, 0.76, 0.85), 10.44), run_time=10)
        self.wait()
        rect_params = find_rectangle(loop_func)
        pre_collision_dots, post_collision_dots = coll_dot_groups = Group((get_special_dot(color, radius=0.075) for color in [YELLOW, GREEN_SCREEN])).replicate(2)
        ...
        self.play(Transform(moving_group, pre_group), frame.animate.reorient(-32, 70, 0, (-3.85, 0.76, 0.85), 10.44), run_time=5)
        self.play(FadeIn(pre_collision_dots))
        self.wait()
        self.play(Transform(moving_group, post_group), Transform(moving_dots, post_collision_dots), frame.animate.reorient(1, 59, 0, (-4.08, 0.88, 1.17), 9.71), run_time=5)
        ...
        self.play(FadeOut(moving_group), FadeIn(pair_group1), FadeIn(pair_group2), mesh.animate.set_stroke(opacity=0.05), frame.animate.reorient(0, 38, 0, (-2.92, 0.03, 1.12), 6.65), run_time=3)
        self.play(ShowCreation(rectangle, time_span=(0, 2)), frame.animate.reorient(36, 68, 0, (-2.89, -0.15, 0.74), 8.11), run_time=8)
...
class AmbientRectangleSearch(ShowTheSurface):
    ...
    def construct(self):
        ...
        self.add(dots, polygon)
        for _ in range(20):
            rect_params = find_rectangle(loop_func, initial_condition=np.random.random(4), target_angle=np.random.uniform(0, 90 * DEG))
...
class SudaneseBand(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(99, 102, 0), Transform(strip, sudanese_band, time_span=(0, 6)), run_time=10)
        self.wait(30)
    ...
    def get_full_surface(self, surface_func, resolution=(101, 101)):
        ...
        surface.set_shading(0.25, 0.25, 0)
        mesh = VGroup(*SurfaceMesh(surface, resolution=(21, 21), normal_nudge=0.001), *SurfaceMesh(surface, resolution=(21, 21), normal_nudge=-0.001))
        mesh.set_stroke(WHITE, 0.5, 0.2)
        edge = VGroup(ParametricCurve(lambda t: surface_func(0, t), (0, 1, 0.01)), ParametricCurve(lambda t: surface_func(1, t), (0, 1, 0.01)))
...
class ShowSurfaceReflection(ShowTheSurface):
    ...
    def construct(self):
        ...
        self.play(ReplacementTransform(ghost_surface, reflection), frame.animate.reorient(33, 81, 0, (-0.15, 0.61, 0.29), 14.6), run_time=5)
        self.add(reflection, surface_group)
        frame.add_ambient_rotation(2 * DEG)
        low_edge = loop.copy()
        low_edge.set_color(RED)
        shift_vect = 0.25 * OUT
        self.play(surface_group.animate.shift(shift_vect), loop.animate.set_stroke(Color('red')).shift(shift_vect), low_edge.animate.shift(-shift_vect), reflection.animate.shift(-shift_vect), FadeOut(axes), FadeOut(plane), run_time=2)
        self.wait(1)
        self.play(FadeOut(loop, shift=-shift_vect), FadeOut(low_edge, shift=shift_vect), surface_group.animate.shift(-shift_vect), reflection.animate.shift(shift_vect), run_time=2)
        self.wait(12)
...
class ConstructKleinBottle(InteractiveScene):
    ...
    def construct(self):
        ...
        mobius_diagram = VGroup(dr_tri, self.get_tri_arrow(square.get_corner(DL), square.get_corner(DR)), self.get_tri_arrow(square.get_corner(DR), square.get_corner(UR)), Line(square.get_corner(DL), square.get_corner(UR)).set_stroke(Color('red'), 3))
        mobius_label = Text('Möbius Strip', font_size=60)
        mobius_label.next_to(mobius_diagram, UR)
        mobius_label.shift(DOWN + 0.25 * RIGHT)
        mobius_arrow = Arrow(mobius_label.get_bottom(), mobius_diagram.get_center() + 0.5 * DR, path_arc=-90 * DEG, thickness=5)
        mobius_arrow.set_z_index(1)
        self.add(mobius_diagram)
        self.play(FadeIn(mobius_label), FadeIn(mobius_arrow))
        self.wait()
        ...
        reflection_arrow = Arrow(reflection_label.get_top(), reflection.get_center() + 0.5 * LEFT + 0.25 * UP, path_arc=-90 * DEG, thickness=5)
        self.play(LaggedStart(TransformMatchingStrings(mobius_label.copy(), reflection_label, run_time=1), TransformFromCopy(mobius_diagram, reflection), TransformFromCopy(mobius_arrow, reflection_arrow), lag_ratio=0.1), run_time=2)
        self.wait()
        ...
        self.play(LaggedStart(FadeOut(reflection_arrow), FadeOut(mobius_arrow), reflection.animate.shift(DR), reflection_label.animate.scale(0.75).next_to(square, LEFT, MED_SMALL_BUFF), mobius_label.animate.scale(0.75).next_to(square, RIGHT, MED_SMALL_BUFF)), FadeIn(glue_label, lag_ratio=0.1))
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(glue_label, reflection_label, mobius_label)), mobius_diagram[-1].animate.set_stroke(width=0), reflection[-1].animate.set_stroke(width=0))
        ...
        self.wait()
        frame = self.frame
        self.play(dl_group.animate.next_to(ORIGIN, UP, 0.5), ur_group.animate.flip(UR).next_to(ORIGIN, DOWN, 0.5), frame.animate.set_height(10), run_time=2)
        self.wait()
        self.play(ur_group.animate.shift(-ur_tri.get_top()), dl_group.animate.shift(-dl_tri.get_bottom()))
        ...
        trg_yellow_arrows = VGroup(self.get_tri_arrow(square.get_corner(DR), square.get_corner(DL), color=YELLOW).flip(RIGHT), self.get_tri_arrow(square.get_corner(UL), square.get_corner(UR), color=YELLOW))
        self.remove(ur_tri, dl_tri)
        self.add(pre_square, pink_arrows, yellow_arrows)
        self.play(Transform(pre_square, square), Transform(yellow_arrows, trg_yellow_arrows), pink_arrows[0].animate.move_to(square.get_left()), pink_arrows[1].animate.move_to(square.get_right()), frame.animate.set_height(8), run_time=2)
        self.wait()
        klein_func = self.get_kelin_bottle_func()
        near_smooth = bezier([0, 0.1, 0.9, 1])
        surfaces = Group((TexturedSurface(ParametricSurface(func), 'KleinBottleTexture') for func in [square_func, tube_func, lambda u, v: torus_func(u, 0.5 * v), lambda u, v: klein_func(u, 0.5 * near_smooth(v))]))
        for surface in surfaces:
        ...
        self.play(FadeIn(moving_surface, shift=square3d.get_z() * OUT), frame.animate.reorient(0, 56, 0, (0.07, 0.52, 2.39), 11.25), run_time=3)
        ...
        v_upper_bound = 0.85
        self.play(UpdateFromAlphaFunc(moving_surface, lambda m, a: m.match_points(ParametricSurface(lambda u, v: klein_func(u, interpolate(0.5, 1, a) * near_smooth(v)))).set_opacity(interpolate(0.9, 0.75, a))), klein_diagram.animate.set_x(-5), frame.animate.reorient(0, 46, 0, (-0.71, -0.11, 1.71), 10.87), run_time=8)
        self.wait()
        self.play(klein_diagram.animate.next_to(moving_surface, LEFT, buff=2), frame.animate.reorient(0, 0, 0, (-3.2, 0.03, 0.0), 12.58), run_time=4)
        self.wait()
...
class PuzzleOverMobiusDiagram(ConstructKleinBottle):
    ...
    def construct(self):
        ...
        self.play(triangle.animate.set_opacity(0.25), edge.animate.set_opacity(0.1), low_arrow.animate.rotate(90 * DEG).next_to(right_arrow, RIGHT))
        self.wait()
        self.play(low_arrow.animate.move_to(right_arrow))
        self.play(low_arrow.animate.next_to(right_arrow, RIGHT))
        self.play(Restore(low_arrow), triangle.animate.set_fill(opacity=1), edge.animate.set_stroke(opacity=1))
        ...
        for _ in range(3):
            ...
            self.wait(1 / 30)
            self.remove(top_dot)
            self.play(dots[0].animate.move_to(low_arrow.get_right()), dots[1].animate.move_to(right_arrow.get_top()), run_time=3, rate_func=linear)
...
class ShowAngleInformation(ShowTheSurface):
    ...
    def construct(self):
        ...
        self.play(Write(coords), FlashAround(corner_coords['x, y'], time_width=1.5), run_time=1.5)
        brace = Brace(Line(LEFT, RIGHT).set_width(connecting_line.get_length()), DOWN, buff=SMALL_BUFF)
        ...
        self.play(GrowFromCenter(brace), Write(d_label), FlashAround(corner_coords['d'], time_width=1.5, run_time=1.5))
        self.wait()
        cross_point = connecting_line.pfp(inverse_interpolate(connecting_line.get_y(DOWN), connecting_line.get_y(UP), -1))
        ...
        self.play(ShowCreation(h_line), ShowCreation(arc), Write(theta))
        new_corner_coords = Tex('(x, y, d, \\theta)')
        new_corner_coords.move_to(corner_coords, RIGHT)
        self.play(*(ReplacementTransform(corner_coords[substr], new_corner_coords[substr]) for substr in ['(x, y, d', ')']), TransformFromCopy(theta, new_corner_coords['\\theta']), Write(new_corner_coords[','][-1]))
        ...
        self.play(ReplacementTransform(connecting_line.copy().clear_updaters(), connecting_line2, path_arc=-90 * DEG, suspend_mobject_updating=True), Rotate(brace, -90 * DEG, about_point=midpoint), d_label.animate.rotate(-90 * DEGREES, about_point=midpoint).rotate(90 * DEG), coords.animate.next_to(midpoint_dot, DL, buff=-0.1), loop.animate.set_stroke(opacity=0.5))
        self.play(FadeIn(dots2))
        self.wait()
        cross_point2 = connecting_line2.pfp(inverse_interpolate(connecting_line2.get_start()[1], connecting_line2.get_end()[1], 0))
        ...
        theta_plus = Tex('\\theta + 90^\\circ', font_size=42)
        ...
        perp_label = Tex('90^\\circ', font_size=36)
        perp_label.next_to(elbow.pfp(0.5), UP, SMALL_BUFF)
        self.play(FadeOut(brace), FadeOut(d_label), ShowCreation(elbow), Write(perp_label))
        shift_vect = 0.5 * RIGHT
        low_corner_coords = Tex('(x, y, d, \\theta + 90^\\circ)')
        low_corner_coords.next_to(new_corner_coords, DOWN, aligned_edge=RIGHT)
        low_corner_coords.shift(shift_vect)
        self.play(TransformMatchingStrings(new_corner_coords.copy(), low_corner_coords), new_corner_coords.animate.shift(shift_vect), frame.animate.shift(shift_vect))
        self.wait()
...
class RectanglesOfAllAspectRatios(LoopScene):
    ...
    def construct(self):
        ...
        square_params = find_rectangle(loop_func, initial_condition=np.arange(0.5, 1, 0.1), target_angle=angle_tracker.get_value())
        ...
        def true_find_rect(loop_func, angle, max_tries=10, max_cost=0.01):
            for _ in range(max_tries):
                result, cost = find_rectangle(loop_func, initial_condition=np.random.random(4), target_angle=angle, n_refinements=4, return_cost=True)
                if cost < max_cost:
                    return result
            return result
    ...
    def get_aspect_ratio_pairs(self):
        return [('16 : 9', 16 / 9), ('4 : 3', 4 / 3), ('10 : 1', 10 / 1), ('\\sqrt{2} : 1', math.sqrt(2)), ('5 : 2', 5 / 2), ('\\pi : 1', math.pi), ('\\varphi : 1', (1 + math.sqrt(5)) / 2), ('e : 1', math.e), ('21 : 9', 21 / 9), ('\\sqrt{3} : 1', math.sqrt(3)), ('3 : 2', 3 / 2), ('1 : 1', 1 / 1), ('2\\pi : 1', 2 * math.pi), ('7 : 5', 7 / 5), ('\\ln(10) : 1', math.log(10)), ('8 : 5', 8 / 5), ('12 : 5', 12 / 5), ('\\sqrt{5} : 1', math.sqrt(5)), ('9 : 16', 9 / 16), ('2 : 3', 2 / 3), ('\\pi : 2', math.pi / 2), ('7 : 3', 7 / 3), ('2\\sqrt{2} : 1', 2 * math.sqrt(2)), ('15 : 4', 15 / 4), ('\\sqrt{7} : 1', math.sqrt(7)), ('11 : 4', 11 / 4), ('e : 2', math.e / 2), ('13 : 5', 13 / 5), ('8 : 3', 8 / 3), ('\\pi^2 : 9', math.pi ** 2 / 9)]
...
class AllAspectRatioXMaxTree(RectanglesOfAllAspectRatios):
    ...
    def get_aspect_ratio_pairs(self):
        return [('\\sqrt{10} : 1', math.sqrt(10)), ('\\pi : \\sqrt{2}', math.pi / math.sqrt(2)), ('17 : 10', 17 / 10), ('\\ln(2) : 1', math.log(2)), ('14 : 3', 14 / 3), ('3\\sqrt{3} : 2', 3 * math.sqrt(3) / 2), ('19 : 6', 19 / 6), ('e^2 : 10', math.e ** 2 / 10), ('25 : 16', 25 / 16), ('\\varphi^2 : 2', ((1 + math.sqrt(5)) / 2) ** 2 / 2), ('23 : 9', 23 / 9), ('\\sqrt{6} : 1', math.sqrt(6)), ('18 : 5', 18 / 5), ('2\\pi : 3', 2 * math.pi / 3), ('16 : 10', 16 / 10), ('\\sqrt{13} : 2', math.sqrt(13) / 2), ('22 : 7', 22 / 7), ('e : \\sqrt{5}', math.e / math.sqrt(5)), ('20 : 11', 20 / 11), ('\\ln(5) : 1', math.log(5)), ('27 : 16', 27 / 16), ('3\\varphi : 4', 3 * ((1 + math.sqrt(5)) / 2) / 4), ('24 : 11', 24 / 11), ('\\sqrt{15} : 2', math.sqrt(15) / 2), ('21 : 13', 21 / 13), ('5\\pi : 8', 5 * math.pi / 8), ('26 : 15', 26 / 15), ('\\sqrt{17} : 3', math.sqrt(17) / 3), ('29 : 12', 29 / 12), ('e\\pi : 10', math.e * math.pi / 10)]

class AllAspectRatioPi(RectanglesOfAllAspectRatios):

    def get_loop(self):
        return Tex('\\pi').family_members_with_points()[0]
...
class TrackTheAngle(ShowTheSurface):
    ...
    def construct(self):
        ...
        line.f_always.put_start_and_end_on(lambda: loop.quick_point_from_proportion(get_x()), lambda: loop.quick_point_from_proportion(get_x() + dx))
        ...
        self.play(VFadeIn(theta_label, suspend_mobject_updating=True), VFadeIn(angle_label, suspend_mobject_updating=True), FadeOut(line), FadeIn(pair_group), loop.animate.set_stroke(WHITE, 1, 0.5))
        self.play(uv_tracker.animate.set_value(self.second_uv), run_time=4)
        self.play(uv_tracker.animate.set_value(self.limiting_uv), run_time=15, rate_func=bezier([0, 0, 1, 1, 1, 1, 1, 1, 1]))
...
class TrackTheAngleForFractal(TrackTheAngle):
    ...
    def get_loop(self, n_iters=7):
        ...
        snowflake = VMobject().set_points_as_corners(np.vstack([self.get_koch_line_points(a, c, n_iters), self.get_koch_line_points(c, b, n_iters), self.get_koch_line_points(b, a, n_iters)]))
    ...
    def get_koch_line_points(self, start, end, n_iters=7):
        ...
        return np.vstack([self.get_koch_line_points(a, b, n_iters - 1), self.get_koch_line_points(b, tip, n_iters - 1), self.get_koch_line_points(tip, c, n_iters - 1), self.get_koch_line_points(c, d, n_iters - 1)])
...
class KochZoom(TrackTheAngleForFractal):
    ...
    def construct(self):
        ...
        self.add(snowflake)
        self.play(self.frame.animate.reorient(0, 0, 0, (-1.12, 1.0, 0.0), 0.23), rate_func=bezier([0, 0, 1, 1, 1, 1, 1, 1]), run_time=10)
...
class MobiusStripsAndKleinBottlesIn4D(ConstructKleinBottle):
    ...
    def construct(self):
        ...
        for surface in surfaces:
            ...
            self.play(FadeIn(surface), Write(mesh, stroke_width=0.5, run_time=2), GrowArrow(arrow))
...
class MusicalIntervalsAsPairs(InteractiveScene):
    ...
    def construct(self):
        ...
        keys = piano[15:27]
        key_labels = VGroup(map(Tex, ['C', 'C^{\\#}', 'D', 'D^{\\#}', 'E', 'F', 'F^{\\#}', 'G', 'G^{\\#}', 'A', 'A^{\\#}', 'B']))
        ...
        self.play(FadeOut(piano[:15]), keys.animate.set_opacity(0.5), FadeOut(piano[27:]), VFadeIn(key_labels))
        self.remove(key_labels)
        self.play(ShowCreation(circle), LaggedStart((FadeTransform(Group(key), dot) for key, dot in zip(keys, dots)), lag_ratio=0.1, group_type=Group), MoveToTarget(key_labels, lag_ratio=0.01), run_time=2)
        random.seed(0)
