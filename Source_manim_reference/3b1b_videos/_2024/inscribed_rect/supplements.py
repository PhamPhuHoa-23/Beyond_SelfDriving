...
class UnsolvedQuestion(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.says('Here is an\nunsolved problem', mode='tease'), self.change_students('pondering', 'sassy', 'pondering'))
...
class AskWhoCares(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(stds[0].change('guilty', stds[1].eyes), stds[1].says('Who cares?!', mode='angry', look_at=self.screen), stds[2].change('erm', stds[1].eyes), morty.change('guilty', stds[1].eyes))
        self.wait(3)
        self.play(FadeOut(stds[1].bubble), stds[0].change('hesitant'), stds[1].says('What practial use\ndoes this have?', 'sassy'))
        self.wait(4)
        self.play(stds[0].change('thinking', morty.eyes), stds[1].debubble(), stds[2].change('pondering', morty.eyes), morty.says("Here's why\nI love it"))
        self.wait()
...
class PurePuzzleToApplication(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(LaggedStart(TransformFromCopy(puzzle_piece, gear, run_time=2), GrowArrow(arrow), Group(lightbulb, glow).animate.match_x(gear).set_anim_args(run_time=2), lag_ratio=0.25), frame.animate.set_x(0), run_time=3)
        self.wait()
...
class WhatIsTopology(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(answer2, lag_ratio=0.1, run_time=2), answer1.animate.set_opacity(0.25).to_edge(DOWN))
        self.wait()
        ...
        self.play(Transform(morty, conf_morty), Write(bubble))
        self.play(Blink(morty))
        self.wait()
...
class ThreeShapes(InteractiveScene):
    ...
    def construct(self):
        ...
        titles = VGroup(Text('Möbius Strip'), Text('Torus'), Text('Klein Bottle'))
...
class LabelMapping(InteractiveScene):
    ...
    def construct(self):
        ...
        body = VGroup(Text('Pair of\nloop points'), Vector(RIGHT), Text('3D space'))
        body.arrange(RIGHT)
        ...
        self.play(Write(new_title['Continuous']), Transform(title, new_title['Mapping'][0]), underline.animate.set_width(1.2 * new_title.get_width()))
        self.wait()
...
class ComplainAboutQuestion(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(morty.change('guilty'), stds[1].says("That's just a pretentious\nway to ask the same thing", 'angry', look_at=self.screen), stds[0].change('hesitant', stds[1].eyes), stds[2].change('erm', stds[1].eyes))
        self.wait(5)
...
class RememberThat(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.change_students('pondering', 'pondering', 'thinking', look_at=self.screen))
        self.wait(3)
...
class AskAboutACircle(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(stds[2].says('What does it look\nlike for a circle?', 'maybe'), morty.change('tease', stds[2].eyes), stds[1].change('pondering', 4 * UR), stds[0].change('confused', 4 * UR))
        self.wait(4)
...
class GraphLabel(InteractiveScene):
    ...
    def construct(self):
        group = VGroup(Text('Graph of'), Tex('f(x, y) = e^{-x^2 - y^2}'))
        group.arrange(DOWN)
        group.to_edge(UP)
...
class XXLabel(InteractiveScene):
    ...
    def construct(self):
        tex = TexText("``Pairs'' of the form (X, X)", font_size=60)
...
class CommutativeDiagram(InteractiveScene):
    ...
    def construct(self):
        ...
        down_arrow = Arrow(objs[0].get_corner(DL) + UP, objs[1].get_left(), path_arc=PI, **arrow_kw)
        lr_arrow = VGroup(Arrow(objs[0], objs[2], **arrow_kw).shift(0.15 * UP), Arrow(objs[2], objs[0], **arrow_kw).shift(0.15 * DOWN))
...
class ContinuousAssociation(InteractiveScene):
    ...
    def construct(self):
        ...
        words.to_corner(UR)
        arrows = VGroup(Arrow(3 * LEFT, 3 * RIGHT, path_arc=PI / 2, thickness=6).shift(2.5 * DOWN), Arrow(3 * RIGHT, 3 * LEFT, path_arc=PI / 2, thickness=6).shift(2.5 * UP))
...
class ThinkLikeATopologist(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.says('Think like a\n topologist'), self.change_students('erm', 'tease', 'thinking', look_at=self.screen))
        self.wait(5)
...
class UnorderedPair(InteractiveScene):
    ...
    def construct(self):
        tex = TexText('Unordered pairs: $\\{A, B\\} = \\{B, A\\}$', font_size=60)
        tex.to_edge(UP)
        self.add(tex)
...
class XYtoYX(InteractiveScene):
    ...
    def construct(self):
        tex = Tex('(x, y) \\leftrightarrow (y, x)')
        tex.to_edge(UP)
        self.add(tex)
...
class XXOnEdge(InteractiveScene):
    ...
    def construct(self):
        tex = TexText("``Pairs'' of the form $(X, X)$\\\\live on the edge")
        tex.to_corner(UL)
        self.add(tex)
...
class StripClaim(InteractiveScene):
    ...
    def construct(self):
        ...
        morty = Mortimer()
        morty.to_corner(DR)
        self.play(VFadeIn(morty), morty.says('False!', mode='surprised'))
        ...
        self.play(TransformMatchingStrings(title, new_title, key_map={'Desired': 'Revised'}, run_time=1, lag_ratio=0.5), FadeOut(VGroup(morty, morty.bubble)), FadeOut(claim['without'], DOWN), FadeOut(claim['intersecting itself.'], DOWN), Write(dots))
        ...
        self.play(FlashUnder(part1, color=YELLOW, time_width=1.5), part1.animate.set_color(YELLOW))
        self.wait()
        self.play(FadeOut(dots), FadeIn(new_claim[', and with']), FadeIn(part2))
        self.play(FlashUnder(part2, time_width=1.5, color=BLUE, run_time=2), part2.animate.set_color(BLUE))
        self.wait()
...
class AskAbout4DEmbeddings(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(morty.change('tease', randy.eyes), self.change_students('thinking', 'erm', 'pondering', look_at=self.screen))
        self.wait(2)
        self.play(stds[0].change('pondering', strip), stds[1].change('pondering', strip), randy.change('raise_right_hand', strip), ShowCreation(strip))
        self.play(GrowArrow(arrow), FadeIn(cloud, DOWN))
        self.wait()
        self.play(stds[0].change('pondering', tup), stds[1].change('pondering', tup), randy.change('raise_left_hand', tup), FadeIn(tup, UP))
        self.wait(5)

class GreeneAndLobb(InteractiveScene):

    def construct(self):
        names = VGroup(Text('Joshua Greene'), Text('Andrew Lobb'))
        images = Group(ImageMobject('JoshuaGreene'), ImageMobject('AndrewLobb'))
        for image in images:
            image.set_height(5)
...
class SmoothImplication(InteractiveScene):
    ...
    def construct(self):
        ...
        implies = Tex('\\Downarrow', font_size=120)
        ...
        self.add(smooth)
        self.wait()
        self.play(Write(implies), FadeIn(tangent, 0.5 * DOWN))
        self.wait()

class ProblemSolvingToRecreation(InteractiveScene):

    def construct(self):
        words = VGroup(Text('Solving\nproblems'), Text('Helpful\nconstructs'), Text('Recreational\nmath'))
        words.arrange(RIGHT, buff=2.5)
        words.set_y(-1.5)
        mind_bending = Text('(Sometimes Mind-bending)', font_size=24)
        mind_bending.next_to(words[1], DOWN)
        arrows = VGroup((Arrow(w1, w2, thickness=4, buff=0.45) for w1, w2 in zip(words, words[1:])))
        arrows.space_out_submobjects(1.1)
        self.play(FadeIn(words[0], lag_ratio=0.1))
        self.wait()
        self.play(FadeIn(words[1], lag_ratio=0.1), GrowArrow(arrows[0]))
        self.wait()
        self.play(FadeIn(mind_bending, 0.25 * DOWN))
        self.wait()
        mb = mind_bending['Mind-bending']
        helpful = words[1]['Helpful']
        self.play(mb.animate.scale(2).move_to(helpful), FadeOut(helpful, 0.25 * UP), FadeOut(mind_bending['(Sometimes'][0]), FadeOut(mind_bending[')'][0]))
        self.play(FadeIn(words[2], lag_ratio=0.1), GrowArrow(arrows[1]))
        self.wait()
        self.play(words[0].animate.set_opacity(0.1), arrows[0].animate.set_opacity(0.1))
        self.wait()
...
class AskAboutTopology(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(stds[0].says('What is a\ntopological space?', 'raise_left_hand'), stds[1].change('pondering', stds[0].eyes), stds[2].change('pondering', stds[0].eyes), morty.change('tease'))
        self.wait(2)
        self.play(stds[1].animate.look_at(stds[2].eyes), stds[2].says('What is\ntopology?', 'maybe', look_at=morty.eyes, bubble_direction=LEFT))
        self.wait()
...
class AskWhat(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(randy.change('confused', UP), Write(bubble))
        self.play(Blink(randy))
