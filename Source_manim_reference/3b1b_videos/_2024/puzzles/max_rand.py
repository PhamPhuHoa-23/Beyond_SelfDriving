...
class MaxProcess(InteractiveScene):
    ...
    def construct(self):
        ...
        x1_group, x2_group, max_group = groups = Group((get_random_var_label_group(interval, '', color=color) for interval, color in zip(intervals, colors)))
        ...
        labels = VGroup((Tex(tex + '\\rightarrow 0.00', t2c=tex_to_color) for tex in ['x_1 = \\text{rand}()', 'x_2 = \\text{rand}()', '\\max(x_1, x_2)']))
        ...
        self.add(line)
        self.play(Randomize(x1_tracker, frequency=4, run_time=30), Randomize(x2_tracker, frequency=4, run_time=30), TrackingDots(x1_tip.get_top, color=BLUE), TrackingDots(x2_tip.get_top, color=YELLOW), TrackingDots(max_tip.get_top, color=GREEN))
...
class SqrtProcess(InteractiveScene):
    ...
    def construct(self):
        ...
        for interval in intervals:
            ...
            interval.numbers.set_opacity(0.75)
        colors = [BLUE, TEAL]
        x_group, sqrt_group = groups = Group((get_random_var_label_group(interval, '', color=color) for interval, color in zip(intervals, colors)))
        ...
        self.add(groups)
        tex_to_color = {'x': BLUE}
        labels = VGroup((Tex(tex + '\\rightarrow 0.00', t2c=tex_to_color) for tex in ['x = \\text{rand}()', '\\sqrt{x}']))
        for label, group, interval in zip(labels, groups, intervals):
            label.next_to(interval, RIGHT, buff=0.5)
            num = label.make_number_changeable('0.00')
            num.tracker = group[0]
        ...
        label = Text('sqrt', font_size=60)
        label.next_to(arrow, RIGHT)
        self.add(arrow, label)
        self.play(Randomize(x_tracker, frequency=4, run_time=30), TrackingDots(x_tip.get_top, color=colors[0]), TrackingDots(sqrt_tip.get_top, color=colors[1]))

class SquareAndSquareRoot(InteractiveScene):

    def construct(self):
        lines = VGroup(Tex('\\left(\\frac{1}{2}\\right)^2 = \\frac{1}{4}'), Tex('\\sqrt{\\frac{1}{4}} = \\frac{1}{2}'))
        lines.scale(1.5)
        self.play(FadeIn(lines[0]))
        self.wait()
        self.play(TransformMatchingStrings(*lines, matched_pairs=[(lines[0]['2'][1], lines[1]['\\sqrt'][0])], path_arc=PI / 2))

class GawkAtEquivalence(InteractiveScene):

    def construct(self):
        expr = VGroup(Text('max(rand(), rand())'), Tex('\\updownarrow', font_size=90), Text('sqrt(rand())'))
        expr.arrange(DOWN)
        expr.to_edge(UP)
        randy = Randolph()
        randy.to_edge(DOWN)
...
class VisualizeMaxOfPairCDF(InteractiveScene):
    ...
    def construct(self):
        ...
        axes.add_coordinate_labels((0, 0.5, 1.0), (0.5, 1.0), excluding=[], num_decimal_places=1, buff=0.15, font_size=16)
        ...
        self.play(FadeIn(x1_tip), FadeIn(x1_label))
        self.wait()
        self.play(Randomize(x1_tracker, run_time=8), TrackingDots(x1_tip.get_top, run_time=10, color=BLUE))
        self.wait()
        eq = Tex('P(a < x_1 < b) = b - a')
        ...
        self.wait(2)
        self.play(FadeOut(rect), FadeOut(eq))
        self.play(FadeIn(axes.y_axis), Write(x2_tip), Write(x2_label))
        self.wait()
        self.play(Randomize(x2_tracker, run_time=8, final_value=0.48), TrackingDots(x2_tip.get_right, run_time=10, color=YELLOW))
        ...
        coord_label.update()
        self.play(ShowCreation(v_line), ShowCreation(h_line), FadeIn(xy_dot), FadeIn(coord_label[0::3]), FadeTransform(x1_label.copy(), coord_label['x_1'], remover=True), FadeTransform(x2_label.copy(), coord_label['x_2'], remover=True))
        ...
        self.play(FadeIn(big_square, run_time=2), Randomize(x2_tracker, run_time=6, frequency=8, final_value=0.69), Randomize(x1_tracker, run_time=6, frequency=8, final_value=0.42), TrackingDots(get_xy_point, run_time=9, fade_factor=0.97, color=GREEN))
        max_expr = Tex('\\max(x_1, x_2) = 0.7', t2c=tex_to_color)
        max_expr.to_corner(UR)
        max_lines = VGroup(Line(axes.c2p(0.7, 0.7), axes.c2p(0.7, 0)), Line(axes.c2p(0.7, 0.7), axes.c2p(0, 0.7)))
        max_lines.set_stroke(GREEN, 3)
        self.play(FadeIn(max_expr, UP))
        self.play(x1_tracker.animate.set_value(0.7), x2_tracker.animate.set_value(0.7))
        self.wait()
        for tracker, line in zip([x2_tracker, x1_tracker], max_lines):
            ...
            self.wait()
        prob_eq = Tex('P(\\max(x_1, x_2) = 0.7)', t2c=tex_to_color)
        prob_ineq = Tex('P(\\max(x_1, x_2) \\le 0.7)', t2c=tex_to_color)
        gen_prob_ineq = Tex('P(\\max(x_1, x_2) \\le R)', t2c=tex_to_color)
        cdf_expr = Tex('P(\\max(x_1, x_2) \\le R) = R^2', t2c=tex_to_color)
        ...
        self.play(TransformMatchingStrings(max_expr, prob_eq), self.frame.animate.move_to(0.5 * UP), run_time=1)
        self.wait()
        self.play(Write(words))
        self.wait()
        self.play(max_lines.animate.set_stroke(width=0.1).set_anim_args(rate_func=there_and_back), run_time=4)
        eq_rect = SurroundingRectangle(prob_eq['='])
        eq_rect.set_color(RED)
        inner_lines = VGroup((max_lines.copy().scale(sf, about_point=axes.get_origin()).set_stroke(width=2, opacity=0.5) for sf in np.linspace(1, 0, 100)))
        inner_square = Square()
        ...
        self.wait()
        self.play(TransformMatchingStrings(prob_eq, prob_ineq, key_map={'=': '\\le'}), eq_rect.animate.surround(prob_ineq['\\le']), run_time=1)
        self.play(LaggedStart((ReplacementTransform(max_lines.copy().set_stroke(opacity=0), mlc) for mlc in inner_lines), lag_ratio=0.01, run_time=8), Animation(xy_dot), FadeOut(eq_rect))
        self.add(inner_lines, inner_square, h_line, v_line, xy_dot, coord_label)
        self.play(FadeIn(inner_square), FadeOut(inner_lines))
        self.wait()
        self.play(Randomize(x1_tracker, run_time=4, frequency=8, final_value=0.38), Randomize(x2_tracker, run_time=4, frequency=8, final_value=0.42), TrackingDots(get_xy_point, run_time=6, fade_factor=0.97, color=GREEN))
        ...
        self.play(VGroup(inner_square, max_lines).animate.scale(0.5, about_edge=DL).set_anim_args(rate_func=there_and_back), run_time=3)
        self.wait()
        shift_value = 3 * RIGHT
        self.play(cdf_expr.animate.shift(5 * RIGHT + UP), self.frame.animate.shift(4.5 * RIGHT).scale(1.2))
        sqrt_lines = VGroup(Tex('P(\\sqrt{x_1} \\le R)', t2c=tex_to_color), Tex('P(x_1 \\le R^2)', t2c=tex_to_color), Tex('= R^2', t2c=tex_to_color))
        ...
        self.wait()
        self.play(TransformMatchingStrings(sqrt_lines[0].copy(), sqrt_lines[1]), Write(mid_eq))
        self.wait()
        ...
        frame.center()
        self.play(Write(axes3d.z_axis), frame.animate.reorient(26, 74, 0, (-0.42, 0.83, 1.66), 10.11), run_time=3)
        tex_to_color['x_3'] = RED
        new_cdf_expr = Tex('P(\\max(x_1, x_2, x_3) \\le R) = R^3', t2c=tex_to_color)
        new_cdf_expr.fix_in_frame()
        new_cdf_expr.move_to(cdf_expr, RIGHT)
        width = inner_square.get_width()
        cube = VGroup(inner_square.copy().shift(width * OUT), inner_square.copy().rotate(PI / 2, LEFT, about_edge=UP), inner_square.copy().rotate(PI / 2, UP, about_edge=RIGHT), inner_square.copy().rotate(PI / 2, DOWN, about_edge=LEFT), inner_square.copy().rotate(PI / 2, RIGHT, about_edge=DOWN))
        ...
        def get_xzy_point():
            return axes3d.c2p(x1_tracker.get_value(), x2_tracker.get_value(), x3_tracker.get_value())
        xyz_dot = TrueDot().make_3d()
        xyz_dot.add_updater(lambda m: m.move_to(get_xzy_point()))
        self.remove(max_lines)
        self.remove(inner_square)
        self.play(FadeIn(x3_tip), FadeIn(x3_label), Restore(cube), FadeOut(h_line), FadeOut(v_line), FadeOut(coord_label), FadeTransform(Group(xy_dot), xyz_dot), run_time=3)
        self.play(Randomize(x1_tracker, run_time=16), Randomize(x2_tracker, run_time=16), Randomize(x3_tracker, run_time=16), TrackingDots(get_xzy_point, run_time=20, color=GREEN, radius=0.15))

class MaxOfThreeTex(InteractiveScene):

    def construct(self):
        expr = TexText('max(rand(), rand(), rand()) $\\leftrightarrow$ rand()$^{1 / 3}$')
