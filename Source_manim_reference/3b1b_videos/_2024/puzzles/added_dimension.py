...
class ShowLozenge(InteractiveScene):
    ...
    def construct(self):
        ...
        VGroup(lozenge, arc1, arc2).stretch(0.95, 0)
        arc1_label = Tex('70.5^\\circ')
        arc1_label.next_to(arc1, RIGHT, MED_SMALL_BUFF)
        arc2_label = Tex('109.5^\\circ')
        arc2_label.next_to(arc2, DOWN, MED_SMALL_BUFF)
        angle_labels = VGroup(arc1, arc1_label, arc2, arc2_label)
        angle_labels.set_z_index(1)
        self.play(ShowCreation(lozenge, time_span=(1, 2.5)), VShowPassingFlash(lozenge.copy().insert_n_curves(20).set_stroke(width=5), time_width=2), run_time=3)
        self.play(Write(arc1_label), ShowCreation(arc1))
        self.play(Write(arc2_label), ShowCreation(arc2))
        ...
        n_center_tiles = 501
        self.play(self.frame.animate.set_height(40), lozenge.animate.set_fill(GREY, 1), LaggedStart((TransformFromCopy(lozenge, tile, path_arc=30 * DEGREES) for tile in row), lag_ratio=1.0 / len(row), time_span=(1, 3)), run_time=4)
        self.play(LaggedStart((TransformFromCopy(row, row2, path_arc=30 * DEGREES) for row2 in rows), lag_ratio=1.0 / len(rows), run_time=3))
        self.clear()
...
class CubesAsHexagonTiling(InteractiveScene):
    ...
    def setup(self):
        ...
        self.base_cube = self.get_half_cube(side_length=n, shared_corner=[-1, -1, -1], grid=True)
...
class RotationMove(InteractiveScene):
    ...
    def construct(self):
        ...
        hexagon = VGroup(lozenge, lozenge.copy().rotate(TAU / 3, about_point=ORIGIN), lozenge.copy().rotate(2 * TAU / 3, about_point=ORIGIN))
        ...
        self.wait()
        self.play(LaggedStart(VShowPassingFlash(arrow1.copy().set_stroke(YELLOW, 3)), TransformFromCopy(hexagon, rot_hex, path_arc=60 * DEGREES), lag_ratio=0.1, run_time=2))
        self.wait()
...
class IntroduceHexagonFilling(InteractiveScene):
    ...
    def fill_with_current_tiles(self):
        ...
        self.play(LaggedStart((TransformFromCopy(starter, tile) for tile in random_order), lag_ratio=1.0 / len(self.tiling), run_time=5))
        self.add(self.tiling)
...
class DrawHexagon(IntroduceHexagonFilling):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(hexagon), VShowPassingFlash(hexagon.copy().set_stroke(width=5).insert_n_curves(20), time_width=2), run_time=2)
        self.wait()
        self.play(GrowFromCenter(brace), Write(brace_label), FadeIn(tiles))
        self.wait()
...
class ShowAsThreeD(CubesAsHexagonTiling):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Restore(self.base_cube), FadeOut(full_grey))
        self.pre_populate()
        self.pre_populate()
        self.play(self.frame.animate.reorient(118, 79, 0, (-1.01, 0.75, 1.54), 12.0).set_field_of_view(40 * DEGREES), run_time=3)
        self.play(self.frame.animate.reorient(163, 83, 0, (1.82, -0.33, 1.89), 12.0), run_time=4)
        self.play(self.frame.animate.reorient(135, 55, 0, ORIGIN, 8).set_field_of_view(1 * DEGREES), run_time=3)
        new_cube = self.random_new_cube()
        self.animate_in_with_rotation(new_cube)
        self.wait()
        self.play(new_cube.animate.shift(5 * RIGHT + 3 * OUT), run_time=6, rate_func=there_and_back_with_pause)
        self.wait()
        for x in range(2):
            new_cube = self.random_new_cube()
...
class Project3DCube(InteractiveScene):
    ...
    def construct(self):
        ...
        axes = ThreeDAxes((-3, 3), (-3, 3), (-3, 3), axis_config=dict(tick_size=0.05))
        ...
        cube_shell = VGroup((Line(vertices[i], vertices[j]) for i, p1 in enumerate(vertices) for j, p2 in enumerate(vertices[i + 1:], start=i + 1) if get_norm(p2 - p1) == 1))
        ...
        for vert in vertices:
            ...
            labels.add(label)
        self.play(LaggedStartMap(FadeIn, labels), FadeIn(vert_dots), frame.animate.reorient(10, 61, 0, (0.9, 0.51, 0.48), 2.44), run_time=3)
        self.wait(note='Talk through the coordinates')
        edges = VGroup(*cube_shell)
        edges.sort(lambda p: p[2])
        self.play(edges[4:].animate.set_stroke(width=0.5, opacity=0.25), labels[1::2].animate.set_opacity(0.1))
        self.wait()
        self.play(edges[8:].animate.set_stroke(width=2, opacity=1), labels[1::2].animate.set_opacity(1), edges[:4].animate.set_stroke(width=0.5, opacity=0.25), labels[0::2].animate.set_opacity(0.1))
        self.wait()
        self.play(edges.animate.set_stroke(width=1, opacity=1), labels.animate.set_opacity(1))
        ...
        self.play(GrowArrow(diag_vect), FadeIn(diag_label, shift=np.ones(3)), cube_shell.animate.set_stroke(opacity=0.25))
        self.wait()
        self.play(TransformFromCopy(cube_shell, proj_cube_shell), TransformFromCopy(vert_dots, proj_vert_dots))
        self.wait(10, note='Talk through projection')
        frame.save_state()
        self.play(frame.animate.reorient(134.75, 54.47, 0, (-0.46, 0.29, 0.23), 3.45).set_field_of_view(1 * DEGREES), run_time=4)
        self.wait()
        self.play(Restore(frame, run_time=3))
        self.wait()
        cube_grid = VGroup((cube_shell.copy().shift(vect) for vect in it.product(*3 * [[0, 1, 2]])))
        ...
        self.play(LaggedStart((TransformFromCopy(ghost_cube, new_cube) for new_cube in cube_grid), lag_ratio=0.05), frame.animate.reorient(40, 72, 0, (1.25, 1.69, 0.99), 5.1), run_time=5)
        self.wait()
        self.play(TransformFromCopy(cube_grid, proj_cube_grid), frame.animate.reorient(60, 68, 0, (0.81, 1.09, 0.94), 5.36), run_time=3)
        self.wait(note='Any commentary?')
        self.play(FadeOut(cube_grid), FadeOut(proj_cube_grid), FadeOut(diag_label), FadeOut(diag_vect), FadeOut(vert_dots), FadeOut(proj_vert_dots), frame.animate.reorient(42, 62, 0, (0.68, 0.48, 0.41), 2.34), run_time=2)
        cube = Cube()
        ...
        self.play(FadeIn(cube), proj_cube_shell.animate.set_stroke(width=1, opacity=0.2))
        self.wait(10, note='Note the outer faces')
        self.add(axes, inner_faces, cube_shell, plane, proj_cube_shell)
        self.play(FadeOut(outer_faces), inner_faces.animate.set_submobject_colors_by_gradient(RED, GREEN, BLUE))
        ...
        coord_labels = VGroup((Tex(str(tuple(basis)), font_size=16).next_to(basis, UR, buff=0.05).rotate(45 * DEGREES, RIGHT, about_point=basis) for basis in bases))
        self.play(axes.animate.set_stroke(width=0.5), plane.axes.animate.set_stroke(width=0.5), FadeOut(inner_faces), LaggedStartMap(GrowArrow, vects), run_time=2)
        self.wait()
        self.play(LaggedStartMap(FadeIn, coord_labels), frame.animate.reorient(9, 63, 0, (1.03, 0.61, 0.56), 2.72), run_time=2)
        self.wait()
        ...
        for i in range(3):
            ...
            self.play(Transform(vects, vects_target), Transform(coord_labels, labels_target), FadeIn(ordered_faces[i]), FadeOut(last_face))
            self.wait()
            last_face = ordered_faces[i]
        proj_vects = VGroup((Vector(np.dot(basis, proj_mat.T), thickness=3) for basis in np.identity(3)))
        proj_coords = VGroup((Tex(f'P{str(tuple(basis))}', font_size=16) for basis in np.identity(3, dtype=int)))
        ...
        self.play(Transform(vects, vects_state), Transform(coord_labels, labels_state), FadeIn(faces), frame.animate.reorient(44, 55, 0, (1.03, 0.61, 0.56), 2.72), run_time=2)
        self.play(Transform(vects, proj_vects), Transform(coord_labels, proj_coords), faces.animate.apply_matrix(proj_mat))
    ...
    def add_coordinate_labels(self, axes):
        ...
        axes.z_axis.add_numbers(**coordinate_config, excluding=[0], direction=LEFT)
...
class Project4DCube(Project3DCube):
    ...
    def construct(self):
        ...
        cube_shell = VGroup((Line(cube_verts[i], cube_verts[j]) for i, p1 in enumerate(cube_verts) for j, p2 in enumerate(cube_verts[i + 1:], start=i + 1) if get_norm(p2 - p1) == 1))
        ...
        self.play(Write(cube_shells[0]), TransformFromCopy(pre_low_labels, low_labels), frame.animate.reorient(11, 67, 0, (1.08, 0.47, 0.77), 3.22), FadeOut(label_group3d, 3 * LEFT), MoveToTarget(label_group4d), run_time=3)
        self.wait(6, note='Pan somewhat')
        self.play(MoveToTarget(low_labels), LaggedStart((FlashUnder(label[-3:], color=RED) for label in low_labels), lag_ratio=0.05))
        self.wait()
        self.play(ShowCreation(edge_connectors, lag_ratio=0), TransformFromCopy(*cube_shells), TransformFromCopy(low_labels, high_labels), label_group4d[1][1::2].animate.set_opacity(1), run_time=3)
        self.wait()
        self.play(MoveToTarget(high_labels), LaggedStart((FlashUnder(label[-3:], color=RED) for label in high_labels), lag_ratio=0.05))
        self.wait(20, note='Pan and gesture')
        axes = ThreeDAxes((-3, 3), (-3, 3), (-3, 3))
        axes.set_height(12)
        pre_proj_points = np.array([*hypercube_points[:8, 1:], *hypercube_points[:8, 1:] + w_shift])
        pre_proj_frame = VGroup((Line(pre_proj_points[i], pre_proj_points[j]) for i, j in edge_indices))
        ...
        self.play(FadeOut(label_group4d), FadeOut(VGroup(cube_shells, edge_connectors, coord_labels)), FadeIn(pre_proj_frame))
        (self.add(cloud, pre_proj_frame),)
        self.play(FadeIn(cloud, time_span=(2, 3)), Write(cloud_label, time_span=(2, 3)), MoveToTarget(pre_proj_frame), frame.animate.reorient(22, 76, 0, (-1.33, 0.51, 0.63), 7.64), run_time=3)
        self.wait()
        self.play(GrowArrow(arrow, path_arc=-30 * DEGREES), Write(arrow_label), Write(axes))
        self.wait()
        ...
        proj_frame = VGroup((Line(proj_points[i], proj_points[j]) for i, j in edge_indices))
        ...
        self.play(FadeOut(proj_frame), FadeIn(inner_cells))
        self.wait()
        space_factor = 1.5
        ...
        self.play(MoveToTarget(inner_cells), FadeOut(corner_group), run_time=2)
        self.wait()
        self.play(FadeOut(inner_cells), FadeIn(ghost_cells, scale=0.8))
        proj_bases = self.construct_proj_matrix().T
        proj_basis_vectors = VGroup((Vector(axes.c2p(*basis)) for basis in proj_bases))
        proj_basis_labels = VGroup((Tex(f'P{tuple(basis)}', font_size=24) for basis in np.identity(4).astype(int)))
        ...
        self.play(axes.animate.set_stroke(width=1), LaggedStartMap(GrowArrow, proj_basis_vectors, suspend_mobject_updating=True), FadeIn(proj_basis_labels))
        self.wait()
        self.play(ghost_cells.animate.space_out_submobjects(space_factor).set_opacity(0.5), run_time=2)
        ordered_cells = Group((ghost_cells[i] for i in [0, 1, 2, 3]))
        vect_groups = VGroup((VGroup(vect, label) for vect, label in zip(proj_basis_vectors, proj_basis_labels)))
        ...
        for i in range(4):
            ...
            self.play(MoveToTarget(vect_groups), MoveToTarget(ordered_cells))
            self.wait()
        self.play(FadeOut(vect_groups), FadeOut(ordered_cells), FadeIn(inner_cells))
        self.wait(5)
        self.play(inner_cells.animate.space_out_submobjects(1.0 / space_factor))
        self.wait(10)
        self.play(FadeOut(inner_cells[1:]))
        self.wait()
        self.play(inner_cells[0].animate.move_to(-inner_cells[0].get_center()), rate_func=there_and_back_with_pause, run_time=6)
        self.wait()
        self.play(FadeIn(inner_cells[1:]))
        self.wait()
        inner_cells.save_state()
        self.play(LaggedStart((cell.animate.move_to(-cell.get_center()) for cell in inner_cells), group=inner_cells, group_type=Group, run_time=3, lag_ratio=0.25))
        self.wait()
        ...
        self.play(FadeOut(inner_cells[:2]), FadeOut(inner_cells[3:]), axes.animate.set_stroke(width=0, opacity=0))
        self.wait(15)
        self.remove(inner_cells)
        self.play(LaggedStart((TransformFromCopy(inner_cells[2], cell) for cell in tiling), group_type=Group, lag_ratio=0.05), frame.animate.reorient(19, 65, 0, (1.39, 1.51, 0.57), 21.55), run_time=8)
        self.wait(20)
    ...
    def get_hypercube_data(self):
        ...
        edge_indices = [(i, j) for i, p1 in enumerate(points) for j, p2 in enumerate(points[i + 1:], start=i + 1) if get_norm(p2 - p1) == 1]
...
class ShowRhombicDodecTesselation(Project4DCube):
    ...
    def construct(self):
        ...
        N = 6
        coords = [coords for coords in it.product(*4 * [list(range(N))]) if sum(coords) == 8]
        pieces = Group((dodec.copy().shift(np.dot(coord, proj_bases)) for coord in coords))
        ...
        self.play(LaggedStart((Restore(piece) for piece in pieces), lag_ratio=0.5, group_type=Group), frame.animate.reorient(-93, 68, 0, (0.07, 0.22, 0.17), 15), run_time=12)
...
class AskStripQuestion(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(radial_line), FadeIn(radius_label, RIGHT))
        self.play(Rotate(radial_line, 2 * PI, about_point=circle.get_center()), ShowCreation(circle), run_time=2)
        self.wait()
        r0_tracker = ValueTracker(0.2)
        r1_tracker = ValueTracker(0.8)
        strip1 = always_redraw(lambda: self.get_strip(circle, r0_tracker.get_value(), r1_tracker.get_value(), theta=TAU / 3, color=TEAL, include_arrow=True, label=''))
        ...
        self.play(r0_tracker.animate.set_value(0.49), r1_tracker.animate.set_value(0.51), run_time=4, rate_func=there_and_back)
        strip1.clear_updaters()
        width_label.clear_updaters()
        self.wait()
        self.play(ReplacementTransform(width_label, d_label))
        strip1.add(d_label)
        new_strips = VGroup((self.get_strip(circle, r0, r1, angle, color=color, include_arrow=True, label=f'd_{n}') for n, r0, r1, angle, color in [(2, 0.5, 0.75, 2 * TAU / 3, GREEN), (3, 0.1, 0.3, 0.8 * TAU, BLUE_D), (4, 0.4, 0.7, 0.1 * TAU, BLUE_B)]))
        for strip in new_strips:
            self.animate_strip_in(strip)
        np.random.seed(0)
        strips = VGroup((self.get_strip(circle, *sorted(np.random.uniform(-1, 1, 2)), TAU * np.random.uniform(0, TAU), opacity=0.25).set_stroke(width=1) for n in range(10)))
        ...
        top_expr = Tex('d_1 + d_2 + d_3 + d_4 + \\cdots + d_n')
        top_expr.to_edge(UP, buff=0)
        d_labels.target = VGroup((top_expr[f'd_{n}'][0] for n in range(1, 5)))
        self.play(LaggedStart(MoveToTarget(d_labels, lag_ratio=0.01), Write(top_expr['+']), Write(top_expr['\\cdots']), Write(top_expr['d_n']), lag_ratio=0.5), FadeOut(arrows), frame.animate.move_to(UP).set_anim_args(run_time=2))
        self.remove(d_labels)
        self.add(top_expr)
        self.wait()
        short_expr = Tex('\\min\\left( \\sum_i d_i \\right)')
        short_expr.move_to(top_expr)
        self.play(LaggedStart(ReplacementTransform(top_expr[re.compile('d_.')], short_expr['d_i']), ReplacementTransform(top_expr['+'], short_expr['\\sum']), ReplacementTransform(top_expr['\\cdots'], short_expr['i'][1]), lag_ratio=0.25))
        self.wait()
        self.play(LaggedStart(Write(short_expr['\\min\\left(']), Write(short_expr['\\right)']), lag_ratio=0.5))
        self.wait()
        ...
        for _ in range(4):
            ...
            curr_strips = VGroup((self.get_strip(circle, *sorted(np.random.uniform(-1, 1, 2)), TAU * np.random.uniform(0, TAU), color=random_bright_color(hue_range=(base_hue, base_hue + 0.2)), opacity=0.25).set_stroke(width=1) for n in range(15)))
        ...
        self.play(FadeOut(og_strips), short_expr.animate.next_to(circle, RIGHT, buff=LARGE_BUFF), frame.animate.move_to(0.5 * UP))
        ...
        strips = VGroup((self.get_strip(circle, r0, r1, theta=0, color=random_bright_color(hue_range=(0.3, 0.5))) for r0, r1 in zip(subdivision, subdivision[1:])))
        self.play(FadeOut(fat_strip.pre_rect), FadeIn(strips, lag_ratio=0.5, run_time=2))
        self.wait()
        fan_covering = VGroup((self.get_strip(circle, -0.4, 0.4, theta=theta) for theta in np.arange(0, TAU, TAU / 3)))
        fan_covering.add(*(self.get_strip(circle, 0.6, 0.9, theta=theta) for theta in np.arange(TAU / 12, TAU, TAU / 3)))
    ...
    def get_strip(self, circle, r0, r1, theta, color=None, opacity=0.5, include_arrow=False, label='', rect_length=10.0):
        ...
        rect.move_to(interpolate(circle.get_center(), circle.get_right(), r0), LEFT)
    ...
    def animate_strip_in(self, strip):
        ...
        self.play(LaggedStart(FadeIn(strip), FadeOut(strip.pre_rect), lag_ratio=0.5, run_time=1))
...
class StruggleWithStrips(AskStripQuestion):
    ...
    def construct(self):
        ...
        widths = [*3 * [0.8], *3 * [0.25]]
        strips = VGroup((self.get_strip(circle, -0.4, 0.4, theta=theta, include_arrow=True) for theta in angles[:3]))
        strips.add(*(self.get_strip(circle, 0.7, 0.95, theta=theta, include_arrow=True) for theta in angles[3:]))
        arrows = VGroup()
        for strip in strips:
            arrow = strip[0]
            strip.remove(arrow)
            arrows.add(arrow)
        self.play(LaggedStart((TransformFromCopy(strip.pre_rect, strip.rect) for strip in strips), lag_ratio=0.1))
        rects = VGroup((strip.rect for strip in strips))
        self.play(LaggedStartMap(FadeOut, rects), LaggedStartMap(FadeIn, strips))
        ...
        for i in range(len(strips)):
            ...
            arrow = arrows[i]
            width_term.next_to(arrow.get_center(), rotate_vector(UP, angles[i]))
            ...
            self.play(MoveToTarget(strips), FadeIn(width_term), FadeIn(arrow), FadeOut(last_arrow))
            self.play(Restore(width_term), FadeIn(plusses[i]))
            last_arrow = arrow
        sum_term.set_value(sum((wt.get_value() for wt in width_terms)))
        self.play(FadeOut(last_arrow), strips.animate.set_fill(opacity=0.5).set_stroke(opacity=1), Write(equals), FadeIn(sum_term))
        self.wait()
        np.random.seed(3)
        ...
        new_strips = VGroup((self.get_strip(circle, r0, r1, theta=0) for r0, r1 in r_pairs))
        new_strips.match_style(strips)
        new_rects = VGroup((s.rect for s in new_strips))
        self.play(FadeOut(strips), FadeIn(rects))
        self.play(ReplacementTransform(rects, new_rects), *(ChangeDecimalToValue(width_term, new_width) for width_term, new_width in zip(width_terms, new_widths)), ChangeDecimalToValue(sum_term, 2.0), run_time=2)
        self.play(FadeOut(new_rects), FadeIn(new_strips))
        self.wait()
        width_sum = Tex('\\sum_{\\text{strip}} \\textbf{Width}(\\text{strip})')
        area_sum = Tex('\\sum_{\\text{strip}} \\textbf{Area}(\\text{strip})')
        area_sum_rhs = Tex('\\ge \\pi r^2 = \\pi')
        ...
        self.wait()
        strip = new_strips[1]
        area_label = TexText('Area = $0.00$')
        area_dec = area_label.make_number_changeable('0.00')
        area_dec.add_updater(lambda m: m.set_value(get_norm(strip.get_area_vector()) / radius ** 2))
        area_label.add_updater(lambda m: m.next_to(strip, LEFT))
        area_label.match_color(strip)
        arrow = Tex('\\leftrightarrow').stretch(2, 0)
        arrow.match_width(strip)
        arrow.always.move_to(strip)
        width_label = VGroup(Text('Width'), Tex('=').rotate(90 * DEGREES), DecimalNumber(1))
        ...
        self.play(FadeOut(new_strips[:1]), FadeOut(new_strips[2:]), FadeOut(radial_line), FadeOut(radius_label))
        self.play(Write(area_label))
        self.wait()
        self.play(GrowFromCenter(arrow), FadeIn(width_label))
        self.wait()
        r0 = subdivision[1]
        delta_r = subdivision[2] - subdivision[1]
        delta_r_tracker = ValueTracker(delta_r)
        r0_tracker = ValueTracker(r0)
        strip.add_updater(lambda m: m.match_points(self.get_strip(circle, r0_tracker.get_value(), r0_tracker.get_value() + delta_r_tracker.get_value(), theta=0)))
        for value in [-1, 0.6, r0]:
            self.play(r0_tracker.animate.set_value(value), run_time=4)
        self.play(delta_r_tracker.animate.set_value(0.3), arrow.animate.scale(0.3 / 0.44), run_time=3)
        strip.clear_updaters()
        self.play(FadeOut(area_label), FadeOut(width_label), FadeOut(arrow), FadeIn(new_strips[:1]), FadeIn(new_strips[2:]))
        width_label_group = VGroup(width_sum, width_brace, width_annotation)
        circle_group = VGroup(circle, new_strips)
        dream_sum = Tex('\\sum_{\\text{strip}} {k} \\cdot \\textbf{Width}(\\text{strip})')
        dream_sum['{k}'].set_color(YELLOW)
        ...
        self.play(circle_group.animate.shift(3 * RIGHT), width_label_group.animate.scale(0.5, about_edge=UR))
        self.wait()
        self.play(GrowArrow(down_arrow), FadeIn(arrow_words, lag_ratio=0.1))
...
class SphereStrips(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(circle)
        sphere = ParametricSurface(lambda u, v: [np.sin(u) * np.cos(v), np.sin(u) * np.sin(v), np.cos(u)], u_range=(0, PI), v_range=(0, 2 * PI))
        sphere.set_opacity(0.5)
        sphere.set_shading(0.5, 0.5, 0.5)
        sphere.always_sort_to_camera(self.camera)
        sphere.set_clip_plane(OUT, 0.001)
        delta_x = 0.25
        x0 = 0.5
        strip = self.get_strip(x0, x0 + delta_x, 0)
        pre_strip = strip.copy()
        pre_strip.stretch(0.001, 2)
        pre_strip.set_z_index(1)
        circle.set_clip_plane(UP, 10)
        plane.set_clip_plane(UP, 10)
        self.play(ShowCreation(pre_strip, run_time=2))
        self.wait()
        pre_sphere = sphere.copy()
        pre_sphere.stretch(0, 2)
        pre_sphere.shift(0.01 * IN)
        ...
        self.play(frame.animate.reorient(-34, 59, 0), run_time=2)
        self.wait()
        self.add(pre_sphere, pre_strip)
        self.play(Restore(strip), Restore(sphere), run_time=3)
        self.play(frame.animate.reorient(40, 59, 0), run_time=7)
        self.wait()
        brace = Brace(pre_strip, UP)
        brace.add(brace.get_tex('d', font_size=24, buff=0.05))
        brace.rotate(90 * DEGREES, RIGHT)
        brace.next_to(strip, OUT, buff=0)
        area_label = TexText('Area = $\\pi d$')
        area_label.to_corner(UR)
        area_label.fix_in_frame()
        self.play(GrowFromCenter(brace, time_span=(1, 2)), frame.animate.reorient(-2, 94, 0, (0.31, 0.11, 0.63), 2.35), run_time=3)
        self.wait()
        self.play(Write(area_label), Transform(brace[-1].copy(), brace[-1].copy().scale(0.5).shift(1.5 * RIGHT + 0.25 * OUT).set_opacity(0), remover=True))
        self.play(FlashAround(area_label, run_time=2))
        self.wait()
        x0_tracker = ValueTracker(x0)
        strip.add_updater(lambda m: m.become(self.get_strip(x0_tracker.get_value(), x0_tracker.get_value() + delta_x, theta=0)))
        brace.add_updater(lambda m: m.next_to(strip, OUT, buff=0))
        self.play(x0_tracker.animate.set_value(-0.99), frame.animate.reorient(-24, 82, 0, (0.4, 0.08, 0.63), 2.67), run_time=5)
        self.play(x0_tracker.animate.set_value(0.5), frame.animate.reorient(2, 76, 0, (0.4, 0.08, 0.63), 2.67), run_time=5)
        strip.clear_updaters()
        brace.clear_updaters()
        self.play(FadeOut(brace), FadeOut(area_label))
        self.play(frame.animate.reorient(29, 74, 0, ORIGIN, 3.0).set_anim_args(run_time=2), sphere.animate.set_clip_plane(OUT, 1), strip.animate.set_clip_plane(OUT, 1))
        self.play(frame.animate.reorient(7, 67, 0), Rotate(strip, PI / 2, DOWN, about_point=ORIGIN), run_time=2)
        self.wait()
        clyinder = ParametricSurface(lambda u, v: [np.cos(v), np.sin(v), u], u_range=[-1, 1], v_range=[0, TAU])
        ...
        self.play(self.frame.animate.reorient(26, 69, 0, (-0.0, -0.0, 0.0), 3.0), run_time=3)
        ...
        self.play(frame.animate.reorient(-28, 62, 0).set_anim_args(run_time=4), Restore(proj_strip, run_time=2))
        self.wait()
        strip.add_updater(lambda m: m.become(self.get_strip(x0_tracker.get_value(), x0_tracker.get_value() + delta_x, theta=0).rotate(PI / 2, DOWN, about_point=ORIGIN)))
        ...
        self.play(FadeOut(cylinder_mesh), FadeOut(proj_strip, 5 * OUT))
        strip.set_clip_plane(OUT, 0)
        sphere.set_clip_plane(OUT, 1)
        self.play(frame.animate.reorient(23, 68, 0), sphere.animate.set_clip_plane(OUT, 0), Rotate(strip, PI / 2, axis=UP, about_point=ORIGIN), run_time=3)
        self.wait()
        strips = Group((self.get_strip(*sorted(np.random.random(2)), theta=random.uniform(0, TAU), color=random_bright_color()).shift(x * 0.001 * OUT) for x in range(1, 20)))
        strips.set_opacity(0.5)
        self.play(ShowCreation(strips, lag_ratio=0.9), frame.animate.reorient(-17, 31, 0), run_time=10)
        self.play(frame.animate.reorient(-24, 64, 0), run_time=8)
        self.wait()

    def get_strip(self, x0, x1, theta, color=BLUE):
        strip = ParametricSurface(lambda u, v: [np.cos(u), np.sin(u) * np.cos(v), np.sin(u) * np.sin(v)], u_range=(math.acos(x1), math.acos(x0)), v_range=(0, TAU))
        ...
        strip.set_clip_plane(OUT, 0.001)
        return strip
...
class MongesTheorem(InteractiveScene):
    ...
    def construct(self):
        ...
        for tangents, dot, circle_pair in zip(tangent_pairs, intersection_dots, it.combinations(circles, 2)):
            ...
            self.play(LaggedStart(c1.animate.scale(0, about_point=dot.get_center()), c2.animate.scale(0, about_point=dot.get_center()), FadeIn(dot), lag_ratio=0.2))
            self.remove(c1, c2)
            self.wait()
        ...
        monge_line = Line()
        monge_line.f_always.put_start_and_end_on(intersection_dots[0].get_center, intersection_dots[2].get_center)
        ...
        frame = self.frame
        self.wait()
        self.play(frame.animate.reorient(-11, 69, 0), FadeIn(plane), FadeIn(spheres, lag_ratio=0.25), run_time=4)
        self.wait()
        self.play(self.frame.animate.reorient(-41, 72, 0), run_time=5)
        self.play(frame.animate.reorient(-67, 76, 0), LaggedStartMap(GrowFromCenter, tangent_groups[2], lag_ratio=0.1), spheres[0].animate.set_opacity(0.05), run_time=3)
        self.wait(10, note="Emphasize how it's formed")
        self.play(frame.animate.reorient(-105, 46, 0), LaggedStartMap(GrowFromCenter, tangent_groups[1], lag_ratio=0.1), spheres[0].animate.set_opacity(0.5), spheres[1].animate.set_opacity(0.05), run_time=3)
        self.wait()
        self.play(frame.animate.reorient(-175, 51, 0, (1.2, 0.92, -0.26)), LaggedStartMap(GrowFromCenter, tangent_groups[0], lag_ratio=0.1), spheres[1].animate.set_opacity(0.5), spheres[2].animate.set_opacity(0.05), run_time=3)
        self.wait()
        self.play(frame.animate.reorient(-70, 59, 0, (0.22, 0.32, -1.5), 9.17), spheres[2].animate.set_opacity(0.5), run_time=6)
        self.wait()
        ...
        plane_lines = VGroup(tangent_groups[2][19].copy(), tangent_groups[0][31].copy(), tangent_groups[1][25].copy())
        plane_lines.set_stroke(width=4, opacity=1)
        self.play(frame.animate.reorient(-50, 74, 0, (0.22, 0.32, -1.5), 9.17), ShowCreation(tangent_plane, time_span=(0, 2)), run_time=6)
        self.wait()
        self.play(frame.animate.reorient(-77, 63, 0, (0.22, 0.32, -1.5), 9.17), FadeOut(tangent_groups), run_time=4)
        ...
        self.wait()
        self.play(FadeOut(xy_plane), FadeOut(tangent_plane), FadeOut(plane_lines), self.frame.animate.to_default_state(), run_time=2)
        dependents.resume_updating()
        self.add(dependents)
        self.play(circles[1].animate.move_to(2 * LEFT), circles[0].animate.move_to(0.2 * UP), circles[2].animate.move_to(3 * RIGHT), run_time=3)
        dependents.suspend_updating()
        self.wait()
        ...
        self.play(Rotating(partial_tangent_plane, PI / 2, axis=RIGHT, about_point=pivot_point), run_time=8, rate_func=there_and_back)
        self.wait()
        self.play(FadeOut(partial_tangent_plane), self.frame.animate.to_default_state(), run_time=3)
        dependents.resume_updating()
        self.add(dependents)
        ...
        self.play(self.frame.animate.reorient(-74, 72, 0, (-1.2, 0.14, -0.2), 8.0), run_time=3)
        ...
        for x in range(2):
            self.play(cone_ghost.animate.scale(0.01, about_point=intersection_dots[2].get_center()), run_time=8, rate_func=there_and_back)
            self.wait()
            self.play(self.frame.animate.reorient(0, 7, 0, (-1.92, 0.22, 0.0)), run_time=3)
        self.play(FadeOut(cone_ghost), self.frame.animate.reorient(-129, 75, 0, (-1.92, 0.22, 0.0)), run_time=4)
        self.play(ShowCreation(tip_lines[1:], lag_ratio=0.5, run_time=2))
        self.wait()
        plane = always_redraw(lambda: self.get_plane_through_points([intersection_dots[2].get_center(), intersection_dots[0].get_center(), cones[2].get_zenith()]))
        plane.suspend_updating()
        self.play(ShowCreation(plane), self.frame.animate.reorient(-74, 66, 0, (-1.92, 0.22, 0.0)), run_time=4)
        ...
        dependents.resume_updating()
        self.play(circles[0].animate.move_to(2 * UP), self.frame.animate.reorient(-122, 54, 0, (-1.54, 0.75, 0.38), 8.65), run_time=4)
    ...
    def get_initial_circles(self):
        ...
        radii = [1, 2, 4]
        circles = VGroup((Circle(radius=radius).move_to(center).set_color(color) for radius, center, color in zip(radii, centers, colors)))
        circles.scale(0.5)
    ...
    def get_all_intersection_dots(self, line_pairs):
        return Group((GlowDot(self.get_intersection(*pair)) for pair in line_pairs))

    def get_all_external_tangents(self, circles, **kwargs):
        return VGroup((self.get_external_tangents(circ1, circ2) for circ1, circ2 in it.combinations(circles, 2)))
    ...
    def get_intersection(self, line1, line2):
        try:
            return line_intersection(line1.get_start_and_end(), line2.get_start_and_end())
        except Exception:
...
class GeneralCentersOfSimilarity(MongesTheorem):
    ...
    def construct(self):
        ...
        theorem_line.add_updater(lambda m: m.put_start_and_end_on(cos_dots[0].get_center(), cos_dots[2].get_center()).scale(100))
        labels = VGroup()
        self.add(circles)
        for lines, dot, pair in zip(similarity_lines, cos_dots, it.combinations(circles, 2)):
            ...
            self.play(ghost.animate.scale(0, about_point=dot.get_center()), ShowCreation(lines, lag_ratio=0), FadeIn(dot), FadeIn(label))
            self.remove(ghost)
            self.wait()
            labels.add(label)
        theorem_line.update()
        theorem_line.suspend_updating()
        self.play(FadeOut(labels), GrowFromCenter(theorem_line))
        ...
        pis = VGroup((Tex('\\pi')[0].match_style(circle).replace(circle) for circle in circles))
        ...
        self.play(LaggedStart((FadeIn(cone, lag_ratio=0.05) for cone in cones), lag_ratio=0.5), frame.animate.reorient(16, 70, 0), run_time=5)
        self.wait(5)

    def get_center_of_similarity_dots(self, shapes):
        return Group((GlowDot(self.get_center_of_similarity(*pair)) for pair in it.combinations(shapes, 2)))
    ...
    def get_all_similarity_lines(self, shapes, **kwargs):
        return VGroup((self.get_similarity_lines(*pair, **kwargs) for pair in it.combinations(shapes, 2)))
    ...
    def get_similarity_lines(self, shape1, shape2, n_lines=25):
        ...
        if big.get_num_points() == 0:
            return VGroup()
        result = VGroup((Line(big.pfp(alpha), point) for alpha in np.linspace(0, 1, n_lines)))
        result.set_stroke(color, 1, 0.5)
        return result

    def get_cone(self, shape):
        top_z = 0.5 * shape.get_width()
        return VGroup((shape.copy().scale(a).set_z(z).set_stroke(width=1) for a, z in zip(np.linspace(1, 0), np.linspace(0, top_z))))

class SimilarDiagrams(MongesTheorem):

    def construct(self):
        circle1, circle2 = circles = VGroup(Circle(radius=1).move_to(LEFT).set_color(GREEN), Circle(radius=2).move_to(4 * RIGHT).set_color(BLUE))
        ...
        radii = VGroup((Line(circle1.get_center(), t_point) for t_point in t_points))
        elbows = VGroup(Elbow(width=0.1).rotate(PI - angle, about_point=ORIGIN).shift(t_point1), Elbow(width=0.1).rotate(-1.5 * PI + angle, about_point=ORIGIN).shift(t_point2))
        ...
        self.play(TransformFromCopy(circle1, circle2), VGroup(tangents, radii, elbows).animate.scale(2, about_point=int_point), run_time=3)
        self.wait()
...
class AskAboutVolumeOfParallelpiped(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(plane, axes)
        verts = [(-2, 1, 1), (1, 0, 3), (3, 0, 0), (2, -2, 0)]
        tetrahedron = VGroup((Polygon(*subset) for subset in it.combinations(verts, 3)))
        ...
        self.add(dots)
        labels = VGroup((Tex(f'(x_{n}, y_{n}, z_{n})', font_size=36) for n in range(1, 5)))
...
class TriangleAreaFormula(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(triangle)
        self.add(dots)
        labels = VGroup((Tex(f'(x_{n}, y_{n})', font_size=36) for n in [1, 2, 3]))
        ...
        self.play(LaggedStartMap(FadeIn, labels, shift=0.25 * UP, lag_ratio=0.5, run_time=2), self.frame.animate.to_default_state().set_anim_args(run_time=6))
        self.wait()
        self.play(triangle.animate.set_fill(YELLOW, 0.5))
        self.wait()
        labels_3d = VGroup((Tex(f'(x_{n}, y_{n}, 1)', font_size=36) for n in [1, 2, 3]))
        ...
        self.play(frame.animate.reorient(-13, 75, 0, (-0.14, 0.7, 1.48), 9.34).set_anim_args(run_time=2), FadeIn(z_axis))
        self.play(frame.animate.reorient(15, 81, 0, (-0.56, 0.95, 1.43), 9.34).set_anim_args(run_time=8), TransformFromCopy(plane, ghost_plane), triangle.animate.shift(OUT), Transform(labels, labels_3d), *(dot.animate.shift(OUT).make_3d() for dot in dots))
        self.wait()
        ...
        cube.apply_matrix(np.transpose([dot.get_center() for dot in dots]))
        self.play(frame.animate.reorient(-5, 63, 0, (-0.04, 0.69, 0.39), 7.73).set_anim_args(run_time=5), Write(cube))
        self.wait()
        tetrahedron = VGroup(triangle.copy(), Polygon(ORIGIN, dots[0].get_center(), dots[1].get_center()), Polygon(ORIGIN, dots[0].get_center(), dots[2].get_center()), Polygon(ORIGIN, dots[1].get_center(), dots[2].get_center()))
        tetrahedron.set_stroke(width=0)
        tetrahedron.set_fill(YELLOW, 0.5)
        self.play(frame.animate.reorient(-4, 77, 0, (0.28, 0.78, 0.41), 7.73).set_anim_args(run_time=4), Write(tetrahedron))
        self.wait()
        self.play(frame.animate.reorient(-5, 64, 0, (0.22, 0.72, -0.04), 7.14), run_time=8)
        self.wait()
...
class LogicForArea(InteractiveScene):
    ...
    def construct(self):
        ...
        equations = VGroup(TexText('Volume(Tetra.) = $\\frac{1}{3}$ Area(Tri.) $\\times 1$'), TexText('Volume(Tetra.) = $\\frac{1}{6}$ Volume(Para.)'), Tex('\\Downarrow'), TexText('Area(Tri.) = $\\frac{1}{2}$ Volume(Para.)'))
...
class FourDDet(InteractiveScene):
    ...
    def construct(self):
        ...
        group = VGroup(Text('Volume', font_size=72), Tex('=', font_size=72).rotate(90 * DEGREES), det_tex)
...
class RandomVectorStatistics(InteractiveScene):
    ...
    def construct(self):
        ...
        for _ in range(0):
            vects = VGroup((Vector(1.5 * rotate_vector(RIGHT, random.uniform(0, TAU)), thickness=4).set_fill(random_bright_color(), border_width=1) for _ in range(2)))
            vects.shift(center)
        ...
        self.play(dim_tracker.animate.set_value(1000).set_anim_args(rate_func=rush_into), UpdateFromFunc(chart, lambda m: m.become(self.get_random_angle_data_histogram(get_dim(), n_vects=50000000 // get_dim(), step_size=1 if get_dim() < 100 else 0.5))), run_time=20)
    ...
    def get_histogram(self, data, min_val=0, max_val=180, step_size=5, bar_color=BLUE_D):
        ...
        axes = Axes(x_range=(min_val, max_val, 5), y_range=(0, y_max, 2), width=6, height=4)
        axes.x_axis.add_numbers(np.arange(min_val, max_val + 1, 45), font_size=24, unit_tex='^\\circ')
        x_unit = axes.x_axis.get_unit_size()
        y_unit = axes.y_axis.get_unit_size()
        bar_width = x_unit * step_size
        bars = VGroup((Rectangle(width=bar_width, height=density * y_unit).move_to(axes.c2p(x, 0), DL) for x, density in zip(bins, densities)))
...
class ProbabilityQuestion(InteractiveScene):
    ...
    def construct(self):
        ...
        lines = VGroup((self.get_uniform_random_indicator(tracker, n) for n, tracker in enumerate(trackers, start=1)))
        ...
        self.add(dist_label)
        lhs = Tex('P\\left(x_1^2 + x_2^2 + x_3^2 + x_4^2 + x_5^2 + x_6^2 \\le 1 \\right)')
        rhs = Tex('\\frac{\\pi^3}{6} \\cdot \\frac{1}{2^6}')
        ...
        brace = Brace(lhs['x_1^2 + x_2^2 + x_3^2 + x_4^2 + x_5^2 + x_6^2'], UP)
        ...
        rect = SurroundingRectangle(rhs['\\frac{\\pi^3}{6}'])
    ...
    def get_uniform_random_indicator(self, value_tracker, n):
        ...
        line.add_numbers(np.arange(-1, 1.5, 0.5), font_size=12, num_decimal_places=1, buff=0.15)
        ...
        x_label = Tex(f'x_{n}', font_size=30)
        x_label.always.next_to(tip, UP, buff=0.05)
        x_label.always.match_color(tip)
        return VGroup(line, tip, x_label)

    def value_to_color(self, value):
        return interpolate_color_by_hsl(GREY_B, BLUE if value > 0 else RED, abs(value))
...
class IntersectingCircles(InteractiveScene):
    ...
    def construct(self):
        ...
        circles.pair_intersections = np.zeros((3, 3))
        vectors = [RIGHT, UL, DL]
        vector_trackers = VGroup((VectorizedPoint(vect) for vect in vectors))

        def update_circles(circles):
            self.place_circles_by_vectors(circles, [vt.get_center() for vt in vector_trackers])
        ...
        self.add(circles)
        self.wait()
        self.play(vector_trackers[2].animate.move_to(LEFT + 0.5 * DOWN), run_time=2)
        self.wait()
        self.play(vector_trackers[0].animate.move_to(RIGHT + 0.5 * DOWN), run_time=2)
        self.wait()
        self.play(vector_trackers[0].animate.move_to(RIGHT), run_time=2)
        self.wait()
        ...
        self.play(LaggedStartMap(FadeOut, circles[:3].copy(), lag_ratio=0.5, scale=0), FadeIn(centers, lag_ratio=0.5))
        self.wait()
        ...
        for i in range(3, 8, 2):
            ...
            self.wait()
        self.wait()
        self.play(LaggedStart((FadeTransform(radial_lines[i1].copy(), radial_lines[i2]) for i1, i2 in [(5, 9), (8, 10), (4, 11)]), lag_ratio=0.75, run_time=3))
        self.wait()
        radial_lines.add_updater(lambda m: m.become(self.get_radial_lines(circles, [vt.get_center() for vt in vector_trackers])))
        self.add(circles, centers, radial_lines)
        self.play(vector_trackers[1].animate.move_to(UP), run_time=3)
        self.play(vector_trackers[1].animate.move_to(UL), run_time=3)
        circles[3].set_stroke(opacity=1)
        self.play(ShowCreation(circles[3]))
        self.wait()
    ...
    def place_circles_by_vectors(self, circles, vectors):
        ...
        circles.tri_intersection = ORIGIN
        circles.pair_intersections = np.array([sum(pair) for pair in it.combinations(list(radial_vectors[:3]), 2)])
        return circles
...
class TriPod(MongesTheorem):
    ...
    def construct(self):
        ...
        tri1, tri2 = tris = VGroup((Polygon(*(proj_point + f * v for f, v in zip(factors, vects))) for factors in [factors1, factors2]))
        ...
        for labels in tri_labels:
            ...
            self.wait()
        tripod_legs = VGroup((Line(vert, proj_point) for vert in tri2.get_vertices()))
        tripod_legs.set_stroke(WHITE, 1)
        for line in tripod_legs:
            line.scale(4)
        self.play(LaggedStartMap(GrowFromCenter, tripod_legs, lag_ratio=0.75, run_time=4), VGroup((g[2] for g in tri_labels)).animate.shift(0.1 * RIGHT).set_anim_args(time_span=(3, 4)))
        self.wait()
        v1 = tri1.get_vertices()
        v2 = tri2.get_vertices()
        side_pairs = VGroup((VGroup(Line(v1[i], v1[j]), Line(v2[i], v2[j])) for i, j in [(1, 0), (0, 2), (1, 2)]))
        ...
        for pair in side_pairs:
            ...
            dot = GlowDot(point)
            int_dots.add(dot)
            self.play(ShowCreation(pair, lag_ratio=0))
            self.wait()
        planes = Group()
        meshes = VGroup()
        for tri in tris:
            a, b, c = tri.get_vertices()
            mat = np.array([normalize(b - a), normalize(c - a), normalize(cross(b - a, c - a))]).T
        ...
        self.play(FadeIn(planes), FadeIn(meshes))
        frame.clear_updaters()
