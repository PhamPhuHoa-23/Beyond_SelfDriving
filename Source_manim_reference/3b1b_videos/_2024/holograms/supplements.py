...
class PhotographVsHologram(InteractiveScene):
    ...
    def construct(self):
        ...
        photo = VGroup(Text('Photograph'), Text('One point of view'))
        holo = VGroup(Text('Our Recording'), Text('Many points of view'))
        ...
        for part in [photo, holo]:
            self.play(GrowArrow(part[0]), FadeInFromPoint(part[1], part[0].get_start()))
            self.wait()
...
class GlintArrow(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(GrowArrow(arrow), VShowPassingFlash(circle, time_width=1.5, time_span=(0.5, 1.5)))
        self.wait()
...
class IndicationOnPhotograph(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(TransformFromCopy(dot1, dot2, path_arc=-30 * DEGREES), ShowCreation(arrow, run_time=2))
        self.play(dot1.animate.move_to(p1), dot2.animate.move_to(p3), arrow.animate.match_points(StrokeArrow(p1, p3, buff=0.1, path_arc=-30 * DEGREES)), rate_func=there_and_back, run_time=3)
        self.play(FadeOut(dot1), FadeOut(dot2), FadeOut(arrow))
...
class ContrastPhotographyAndHolography(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FlashAround(points[1], color=TEAL, time_width=1.5, run_time=2), points[1].animate.set_color(TEAL))
        self.wait()
...
class Outline(InteractiveScene):
    ...
    def construct(self):
        ...
        top_images = Group(ImageMobject('HologramProcess.jpg'), ImageMobject('SimplestHologram.jpg'), ImageMobject('HologramEquation.png'))
        top_rects = Group((Group(outline, image.replace(outline)) for outline, image in zip(top_outlines, top_images)))
        top_titles = VGroup(Text('The process'), Text('The simplest hologram'), Text('The general derivation'))
        ...
        for rect, title in zip(top_rects, top_titles):
            ...
            self.wait()
        self.play(FadeOut(top_rects[1:]), FadeOut(top_titles[1:]), frame.animate.set_height(4).move_to(top_rects[0]), run_time=2)
        self.wait()
        self.play(FadeIn(top_rects[1:]), FadeIn(top_titles[1:]), frame.animate.to_default_state().set_y(2), run_time=2)

        def isolate(rects, titles, index, faded_opacity=0.15):
            return AnimationGroup(*(title.animate.set_opacity(1 if i == index else faded_opacity) for i, title in enumerate(titles)), *(rect.animate.set_opacity(1 if i == index else faded_opacity) for i, rect in enumerate(rects)))
        ...
        outer_lines = VGroup(CubicBezier(top_rects[1].get_corner(DL), top_rects[1].get_corner(DL) + DOWN, mid_outlines[0].get_corner(UL) + 2 * UP, mid_outlines[0].get_corner(UL)), CubicBezier(top_rects[1].get_corner(DR), top_rects[1].get_corner(DR) + DOWN, mid_outlines[2].get_corner(UR) + 2 * UP, mid_outlines[2].get_corner(UR)))
        mid_images = Group(ImageMobject('ZonePlateExposure'), ImageMobject('DotReconstruction'), ImageMobject('MultipleDotHologram'))
        mid_rects = Group((Group(outline, image.replace(outline)) for outline, image in zip(mid_outlines, mid_images)))
        mid_titles = VGroup(Text('Exposure pattern'), Text('Reconstruction'), Text('Added complexity'))
        ...
        for rect, title in zip(mid_rects, mid_titles):
            ...
            rect.set_opacity(0)
        self.play(LaggedStartMap(Restore, mid_rects), ShowCreation(outer_lines, lag_ratio=0), frame.animate.set_y(0), FadeIn(mid_titles, time_span=(1.5, 2.0), lag_ratio=0.025), run_time=2)
        self.wait()
        ...
        self.play(frame.animate.set_y(-4), FadeIn(low_rect, DOWN), FadeIn(low_title, DOWN), GrowArrow(in_arrow), run_time=2)
        self.play(GrowArrow(up_arrow))
        self.wait()
        frame.save_state()
        self.play(frame.animate.set_height(4).move_to(Group(low_rect, low_title)), FadeOut(VGroup(in_arrow, up_arrow)), run_time=2)
        self.wait()
        self.play(Restore(frame), FadeIn(VGroup(in_arrow, up_arrow)), run_time=2)
        self.wait()
        self.play(LaggedStartMap(FadeOut, Group(low_title, low_rect, arrow), lag_ratio=0.1, run_time=1), frame.animate.set_y(0).set_anim_args(run_time=2))
        self.wait()
        self.play(isolate(mid_rects, mid_titles, 2))
        self.wait()
        self.play(LaggedStart((mid_rect.animate.replace(top_rects[1]).set_opacity(0) for mid_rect in mid_rects), lag_ratio=0.05, group_type=Group), FadeOut(mid_titles), Uncreate(outer_lines, lag_ratio=0), frame.animate.set_y(2), run_time=2)
        self.wait()
...
class GoalOfRediscovery(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        self.play(self.change_students('pondering', 'confused', 'maybe', look_at=self.screen), morty.change('guilty'))
        self.wait(2)
        self.play(morty.says('I want this to\nfeel rediscovered'), self.change_students('tease', 'erm', 'pondering', look_at=morty.eyes))
        self.wait(2)
...
class NameCraigAndSally(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(GrowArrow(craig_arrow), FadeIn(craig_name))
        self.play(GrowArrow(sally_arrow), FadeIn(sally_name))
...
class AskAboutRecordingPhase(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(morty.says(Text('\n                    How could you\n                    detect this phase\n                    difference?\n                '), look_at=randy.eyes), randy.change('pondering', morty.eyes))
        self.play(Blink(randy))
        self.play(randy.animate.look_at(3 * UL), morty.change('tease', 3 * UL))
...
class HowIsThisHelpful(TeacherStudentsScene):
    ...
    def construct(self):
        stds = self.students
        morty = self.teacher
        self.play(LaggedStart(stds[1].says('What does this have\nto do with recording 3d?', mode='maybe', look_at=self.screen), stds[0].change('confused', self.screen), stds[2].change('erm', self.screen), lag_ratio=0.25, run_time=2))
...
class NoWhiteLight(InteractiveScene):
    ...
    def construct(self):
        white_label = VGroup(Text('White light', font_size=60), Text('(many frequencies)').set_color(GREY_B))
        white_label.arrange(DOWN)
        white_label.to_edge(UP).set_x(-3)
        laser_label = VGroup(Text('Laser', font_size=60).set_color(GREEN_SCREEN), Text('(single frequency)').set_color(GREY_B))
        ...
        self.wait()
        self.play(FadeOut(VGroup(white_label, cross), DR), FadeIn(laser_label, DR))
        self.wait()
...
class AnnotateSetup(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeOut(pol_words), FadeOut(pol_arrow))
        obj_beam_points = [(-1.625, 2.167, 0), (-1.625, 1.264, 0), (0.375, 1.264, 0), (-0.625, -3.625, 0), (-0.111, -3.167, 0)]
        ...
        obj_beam_spread = VGroup((Line(obj_beam_points[-1], scene_circle.pfp(a) + np.random.uniform(-0.025, 0.025, 3)) for a in np.arange(0, 1, 0.001)))
        obj_beam_spread.set_stroke(GREEN_SCREEN, 1, 0.25)
        self.play(ShowCreation(obj_beam, rate_func=linear), FadeIn(obj_beam_name, lag_ratio=0.1))
        self.play(ShowCreation(obj_beam_spread, lag_ratio=0, rate_func=rush_from))
        self.wait()
        ref_beam_points = [obj_beam_points[0], (-1.61, -1.39, 0), (-1.056, -1.809, 0)]
        ...
        ref_beam_spread = VGroup((Line(ref_beam_points[-1], ref_circle.pfp(a) + np.random.uniform(-0.025, 0.025, 3)) for a in np.arange(0, 1, 0.001)))
        ref_beam_spread.match_style(obj_beam_spread)
        self.play(FadeTransform(obj_beam_name, ref_beam_name), ReplacementTransform(obj_beam, ref_beam), ReplacementTransform(obj_beam_spread, ref_beam_spread))
        self.wait()
...
class NameElectromagneticField(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(name2.animate.next_to(arrow, LEFT), GrowArrow(arrow), FadeIn(expl, RIGHT))
        self.wait()

class HoldUpDiffractionEquationInAnticipation(TeacherStudentsScene):

    def construct(self):
        equation = Tex('d \\cdot \\sin(\\theta) = n \\lambda', font_size=60)
        ...
        self.play(morty.change('raise_right_hand', equation), FadeIn(equation, UP), self.change_students('confused', 'confused', 'erm', equation))
        self.play(Write(name), GrowArrow(arrow))
        self.wait()
        self.play(self.change_students('maybe', 'confused', 'sassy', look_at=self.screen))
        self.wait(3)
        self.play(morty.says('You can discover\nthis yourself', mode='hooray'), LaggedStartMap(FadeOut, VGroup(equation, arrow, name), shift=UR, lag_ratio=0.2, run_time=2), self.change_students('pondering', 'pondering', 'thinking', look_at=morty.eyes))
        self.wait(3)

class HowLongIsThatSnippet(TeacherStudentsScene):

    def construct(self):
        self.play(self.teacher.says('How long is\nthat snippet?'), self.change_students('happy', 'tease', 'thinking', look_at=self.screen))
        self.wait(3)
...
class HoldUpDiffractionEquation(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        equation = Tex('d \\cdot \\sin(\\theta) = \\lambda', t2c={'\\lambda': TEAL}, font_size=60)
        equation.move_to(self.hold_up_spot, DOWN)
        words = Text('Remember this')
        words.next_to(equation, UP, 1.5)
        equation.to_edge(RIGHT)
        arrow = Arrow(words, equation)
        self.play(morty.change('raise_left_hand', look_at=equation), FadeIn(equation, UP), self.change_students('pondering', 'pondering', 'pondering', equation))
        self.play(FadeIn(words, lag_ratio=0.1), GrowArrow(arrow))
        self.play(morty.change('tease'))
        self.wait(3)
        morty.body.insert_n_curves(100)
        self.play(self.change_students('thinking', 'erm', 'pondering', look_at=self.screen), morty.change('raise_right_hand', look_at=self.screen))
        self.wait(4)
...
class DistanceApproximation(InteractiveScene):
    ...
    def construct(self):
        expr = Tex('\\sqrt{L^2 + x^2} \\approx L + \\frac{x^2}{2L}')
        expr.to_corner(UL)
        circle = SurroundingRectangle(expr['\\frac{x^2}{2L}'])
...
class CircleDiffractionEquation(InteractiveScene):
    ...
    def construct(self):
        ...
        equation = Tex('{d} \\cdot \\sin(\\theta) = n \\lambda', font_size=72, t2c={'{d}': BLUE, '\\theta': YELLOW, '\\lambda': TEAL})
        ...
        self.add(title)
        self.add(equation)
        self.play(FlashAround(equation, run_time=4, time_width=1.5, buff=MED_SMALL_BUFF))
        self.wait()

class DiffractionGratingGreenLaserExampleNumbers(InteractiveScene):

    def construct(self):
        lines = VGroup(VGroup(Text('Grating: 500 lines / mm'), Tex('\\Rightarrow d = 2 \\times 10^{-6} \\text{m}')).arrange(RIGHT), VGroup(Text('Green light wavelength: 500 nm '), Tex('\\Rightarrow \\lambda = 5 \\times 10^{-7} \\text{m}')).arrange(RIGHT), Tex('\\theta = \\sin^{-1}(\\lambda / d) = \\sin^{-1}\\left(5 \\times 10^{-7} / 2 \\times 10^{-6} \\right) \\approx 14.5^\\circ'))
...
class AnnotateZerothOrderBeam2(InteractiveScene):
    ...
    def construct(self):
        ...
        points = [(-0.67, 1.64, 0.0), (3.64, 1.9, 0.0), (-4.9, 1.85, 0.0), (-5.4, 1.01), (2.42, 0.028)]
        names = VGroup(Text('Zeroth-order beam'), Text('First-order beams'), Text('Second-order beams'))
        names.next_to(points[0], UP, buff=2.5)
        names.shift_onto_screen()
        arrow0 = Arrow(names[0], points[0])
        arrows1 = VGroup(Arrow(names[1].get_bottom(), points[1]), Arrow(names[1].get_bottom(), points[2]))
        arrows2 = VGroup(Arrow(names[2].get_bottom(), points[3]), Arrow(names[2].get_bottom(), points[4]))
        VGroup(names, arrow0, arrows1, arrows2).set_fill(GREY_E)
        self.play(Write(names[0]), GrowArrow(arrow0), run_time=1)
        self.wait()
        self.remove(arrow0)
        self.play(TransformMatchingStrings(names[0], names[1], key_map={'Zeroth': 'First', 'm': 'ms'}), TransformFromCopy(arrow0.replicate(2), arrows1), run_time=1)
        self.wait()
        self.play(TransformMatchingStrings(names[1], names[2], key_map={'First': 'Second'}), ReplacementTransform(arrows1, arrows2), run_time=1)
        self.wait()
...
class ExactSpacingQuestion(InteractiveScene):
    ...
    def construct(self):
        ...
        question.to_edge(UP)
        subq = TexText("As a function of $\\theta'$")
...
class DejaVu(TeacherStudentsScene):
    ...
    def construct(self):
        stds = self.students
        self.play(stds[0].change('pondering', self.screen), stds[1].change('pondering', self.screen), stds[2].says('That looks\nfamiliar', bubble_config=dict(direction=LEFT), look_at=self.screen), self.teacher.change('tease'))
        self.wait(2)
        diff_eq = Tex('d \\cdot \\sin(\\theta) = \\lambda')
        diff_eq.next_to(stds[0], UR)
        self.play(stds[1].change('raise_left_hand', look_at=diff_eq), FadeIn(diff_eq, UP))
...
class WhatAHologramReconstructs(InteractiveScene):
    ...
    def construct(self):
        ...
        title.to_edge(UP, buff=MED_SMALL_BUFF)
        underline = Underline(title, buff=-0.05)
        items = VGroup(Text('1) Copy of the reference wave'), Text('2) Copy of the object wave'), TexText('3) Reflection$^*$ of the object wave'))
        items.scale(0.8)
        items.arrange(DOWN, buff=1.75, aligned_edge=LEFT)
        items.next_to(underline, DOWN, LARGE_BUFF)
        items.to_edge(LEFT)
        self.play(FadeIn(title), ShowCreation(underline))
        self.play(LaggedStart((FadeIn(item[:2]) for item in items), lag_ratio=0.05))
        self.wait()
        for n, item in enumerate(items):
            self.play(items[:n].animate.set_opacity(0.25), FadeIn(item[2:], lag_ratio=0.1))
            self.wait()
...
class BinaryVsSinusoidalDiffraction(InteractiveScene):
    ...
    def construct(self):
        top_axes, low_axes = two_axes = VGroup((Axes((0, 4, 0.5), (0, 1, 0.5), width=6, height=1.5) for _ in range(2)))
        ...
        for graph, grating in [(top_graph, top_grating), (low_graph, low_grating)]:
            ...
            self.play(graph.animate.set_stroke(width=2), VShowPassingFlash(graph.copy().set_stroke(width=5), time_width=3, rate_func=linear), FadeIn(grating, lag_ratio=0.001), run_time=2)
            self.play(grating.animate.next_to(grating, RIGHT, MED_LARGE_BUFF), run_time=2)
            self.wait()
        high_beam_labels = VGroup((Text('Higher order beams', font_size=36).next_to(grating, UP) for grating in gratings))
        ...
        self.play(LaggedStart(FadeIn(high_beam_labels), Write(check), Write(ex), lag_ratio=0.5))
        self.wait()
...
class AskWhyForSinusoidalGratingFact(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for pi in self.pi_creatures:
            pi.body.insert_n_curves(100)
        self.play(LaggedStart(stds[1].says('Wait, why?', mode='confused', look_at=self.screen), stds[0].change('pondering', self.screen), stds[2].change('maybe', self.screen)))
        self.wait()
        implication = VGroup(Tex('\\Leftarrow').scale(2), Text('More formal\nholography explanation'))
        implication.arrange(RIGHT, buff=0.25)
        implication.move_to(2.5 * UP + 2.5 * RIGHT)
        self.play(morty.change('raise_right_hand'), GrowFromPoint(implication, morty.get_corner(UL)))
...
class NeedsMoreRigor(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        words.next_to(arrow)
        self.play(morty.change('guilty'), self.change_students('erm', 'hesitant', 'sassy', look_at=self.screen))
...
class CompareFilmTypes(InteractiveScene):
    ...
    def construct(self):
        categories = VGroup(Text('Polaroid instant film'), Text('Microfilm'), Text('What we used (Bayfol HX200)'))
        categories[2]['(Bayfol HX200)'].set_color(GREY_B)
        resolutions = VGroup(Text('10-20 lines per mm'), Text('100-200 lines per mm'), Text('> 5,000 lines per mm'))
        ...
        for text, res, arrow in zip(categories, resolutions, arrows):
            ...
            text.save_state()
            text.set_x(0)
            self.play(FadeOut(last_group, 0.5 * UP), FadeIn(text, 0.5 * UP))
            self.wait()
...
class TheresADeeperIssue(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.says("There's a\ndeeper issue"), self.change_students('erm', 'guilty', 'hesitant', look_at=self.teacher.eyes))
        self.wait(2)
...
class WhatThisExplanationLacks(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        points = BulletedList('How do errors in the various approximations accumulate?', "Why don't higher order beams exist?", 'What happens when you change the reference beam angle?', buff=0.5, font_size=48)
...
class SimplicityToGenerality(InteractiveScene):
    ...
    def construct(self):
        ...
        triangles = VGroup((Polygon(*tri) for tri in zip(verts[0::3], verts[1::3], verts[2::3])))
        ...
        self.add(tri1)
        self.play(LaggedStart(Restore(ng1, lag_ratio=0.3), Restore(ng2, lag_ratio=0.02), Restore(ng3, lag_ratio=0.0003), lag_ratio=0.4), frame.animate.reorient(29, 64, 0, (-0.15, 0.02, -0.43), 3.21), run_time=9)
        self.add(triangles)
        equation = Tex('\\left(x^2+y^2+z^2+R^2-r^2\\right)^2=4 R^2\\left(x^2+y^2\\right)')
        equation.fix_in_frame()
        equation.to_edge(UP, buff=0.35)
        new_surface = ParametricSurface(lambda u, v: surface.uv_func(v, u), u_range=(0, TAU), v_range=(0, TAU), resolution=(250, 250))
        new_surface.set_width(4)
        new_surface.set_opacity(0.5)
        new_surface.always_sort_to_camera(self.camera)
        triangles.shuffle()
        self.play(FadeOut(triangles, scale=0.8, lag_ratio=0.0003), frame.animate.reorient(77, 62, 0, (-0.27, -0.05, -0.23), 4.29), run_time=2)
        self.play(ShowCreation(new_surface, run_time=3), frame.animate.reorient(58, 66, 0, (-0.27, -0.05, -0.23), 4.29).set_anim_args(run_time=4))
...
class ThinkingAboutRediscovery(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(rect, randy)
        self.play(randy.change('pondering'), Write(bubble, stroke_color=WHITE, lag_ratio=0.2))
        self.play(Blink(randy))
        self.wait(2)
...
class IntroduceFormalSection(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(LaggedStart(stds[0].change('sassy', morty.eyes), stds[1].says('Okay but really, why\ndoes it work?', look_at=morty.eyes), stds[2].change('angry', look_at=morty.eyes), morty.change('guilty')))
        self.wait(2)
        self.play(LaggedStart(morty.change('tease'), stds[0].change('well'), stds[1].debubble(), stds[2].change('hesitant'), lag_ratio=0.5))
        self.wait(2)
        ...
        a_label = Tex('a', font_size=24)
        a_label.match_color(h_line)
        a_label.next_to(h_line, DOWN, buff=0.1)
        b_label = Tex('bi', font_size=24)
        b_label.match_color(v_line)
        b_label.next_to(v_line, RIGHT, buff=0.1)
        z_label = Tex('a + bi', font_size=24)
        z_label.next_to(v_line.get_end(), UR, SMALL_BUFF)
        VGroup(a_label, b_label, z_label).set_backstroke(BLACK, 3)
        self.play(morty.change('raise_right_hand', plane), self.change_students(*3 * ['pondering'], look_at=plane), FadeIn(plane, UP))
        self.play(ShowCreation(h_line), FadeIn(a_label, 0.5 * RIGHT), z_dot.animate.move_to(plane.n2p(a)))
        self.play(ShowCreation(v_line), FadeIn(b_label, 0.5 * UP), z_dot.animate.move_to(plane.n2p(z)))
...
class ComplexConjugateFact(InteractiveScene):
    ...
    def construct(self):
        z = complex(2, 1)
        plane = ComplexPlane((-6, 6), (-4, 4), faded_line_ratio=4, background_line_style=dict(stroke_color=BLUE_D, stroke_width=1), faded_line_style=dict(stroke_color=BLUE_E, stroke_width=1, stroke_opacity=0.5))
        plane.set_height(7)
        ...
        self.play(TransformFromCopy(z_arrow, z_conj_arrow, path_arc=-PI / 2), TransformMatchingStrings(z_label.copy(), z_conj_label, path_arc=-PI / 2), run_time=2)
        self.wait()
        product_arrow = Arrow(plane.get_origin(), plane.n2p(z * z_conj), buff=0)
        product_arrow.set_color(WHITE)
        product_label = Tex('z \\cdot z^* = |z|^2')
        product_label.next_to(product_arrow.get_end(), UP)
        product_label.shift(0.75 * RIGHT)
        self.play(TransformFromCopy(z_label[0], product_label['z \\cdot'][0]), TransformFromCopy(z_conj_label[:2], product_label['z^*'][0]), TransformFromCopy(z_arrow, product_arrow), TransformFromCopy(z_conj_arrow, product_arrow))
...
class PrepareComplexAlgebra(InteractiveScene):
    ...
    def construct(self):
        lines = VGroup(Tex('R \\cdot (1 - \\text{Opacity})'), Tex('R \\cdot (1 - c|R + O|^2)'), Tex('R - c R \\cdot |R + O|^2'))
        ...
        rect = SurroundingRectangle(lines[2]['R \\cdot |R + O|^2'], buff=0.05)
        rect.set_stroke(TEAL, 2)
        self.play(FadeIn(label, lag_ratio=0.1), GrowArrow(arrow), FadeIn(lines[0], LEFT))
        self.wait()
        ...
        self.wait()
        self.play(ShowCreation(rect), lines[:2].animate.set_opacity(0.5), lines[2]['R - c'].animate.set_opacity(0.5))
        self.wait()

class ComplexAlgebra(InteractiveScene):

    def construct(self):
        lines = VGroup(Tex('R \\cdot |R + O|^2'), Tex('R \\cdot (R + O)(R^* + O^*)'), Tex('R \\cdot R \\cdot R^* + R \\cdot R^* \\cdot O  + R \\cdot R \\cdot O^*+ R \\cdot O \\cdot O^*'), Tex('\\left(|R|^2 + |O|^2 \\right) \\cdot R + |R|^2 \\cdot O + R^2 \\cdot O^*'))
        lines[2].scale(0.75)
        ...
        self.wait()
        self.play(LaggedStart(TransformFromCopy(lines[0][:-1], lines[1]['R \\cdot (R + O)'][0]), TransformFromCopy(lines[0]['|R + O|'][0].copy(), lines[1]['(R^* + O^*)'][0]), lag_ratio=0.1, run_time=2))
        self.wait()
        R0 = lines[1]['R'][0]
        R = lines[1]['R'][1]
        O = lines[1]['O'][0]
        Rc = lines[1]['R^*'][0]
        Oc = lines[1]['O^*'][0]
        l1_groups = [VGroup(R0, R, Rc), VGroup(R0, O, Rc), VGroup(R0, R, Oc), VGroup(R0, O, Oc)]
        l2_groups = [lines[2][substr] for substr in ['R \\cdot R \\cdot R^*', 'R \\cdot R^* \\cdot O', 'R \\cdot R \\cdot O^*', 'R \\cdot O \\cdot O^*']]
        plusses = lines[2]['+']
        ...
        for l1_group, l2_group, plus in zip(l1_groups, l2_groups, plusses):
            self.play(Transform(pre_rects, VGroup(map(self.get_term_rect, l1_group))), Transform(post_rect, self.get_term_rect(l2_group)), FadeIn(l2_group), FadeIn(plus))
            self.wait(0.5)
        self.add(lines[2])
        self.play(FadeOut(pre_rects), FadeOut(post_rect))
        self.wait()
        pair_rects = VGroup(self.get_term_rect(lines[2]['R \\cdot R^*'][0]), self.get_term_rect(lines[2]['R \\cdot R^*'][1]), self.get_term_rect(lines[2]['O \\cdot O^*']))
        ...
        self.wait()
        self.play(FadeOut(real_label), FadeOut(arrows), FadeOut(pair_rects))
        self.wait()
        lines[3].next_to(lines[2], DOWN, buff=1.5)
        lines[3].set_opacity(1)
        l3_groups = [lines[3]['\\left(|R|^2 + |O|^2 \\right) \\cdot R'], lines[3]['|R|^2 \\cdot O'], lines[3]['R^2 \\cdot O^*']]
        l2_plusses = lines[2]['+']
        l3_plusses = lines[3]['+']
        ...
        box1_lines = VGroup(self.connecting_line(l2_rects[0], l3_rects[0]), self.connecting_line(l2_rects[3], l3_rects[0]))
        box2_line = self.connecting_line(l2_rects[1], l3_rects[1])
        box3_line = self.connecting_line(l2_rects[2], l3_rects[2])
        real_brace1 = Brace(lines[3]['\\left(|R|^2 + |O|^2 \\right)'], DOWN)
        real_brace2 = Brace(lines[3]['|R|^2'][1], DOWN)
        real_label = real_brace1.get_text('Some real number', font_size=36)
        fade_opacity = 0.5
        self.play(ShowCreation(box1_lines, lag_ratio=0.5), FadeIn(l2_rects[0]), FadeIn(l2_rects[3]), FadeTransform(l2_groups[0].copy(), l3_groups[0], time_span=(0.5, 2)), FadeTransform(l2_groups[3].copy(), l3_groups[0], time_span=(1, 2)), FadeIn(l3_rects[0], time_span=(1, 2)), l2_groups[1].animate.set_opacity(fade_opacity), l2_groups[2].animate.set_opacity(fade_opacity), l2_plusses.animate.set_opacity(fade_opacity), run_time=2)
        self.wait()
        self.play(GrowFromCenter(real_brace1), FadeIn(real_label, shift=0.25 * DOWN))
        self.wait()
        self.play(box1_lines.animate.set_stroke(WHITE, 1, 0.5), VGroup(l2_rects[0], l2_rects[3], l3_rects[0]).animate.set_stroke(GREY, 1, 0.5), l2_groups[1].animate.set_opacity(1), l2_groups[0].animate.set_opacity(fade_opacity), l2_groups[3].animate.set_opacity(fade_opacity), l3_groups[0].animate.set_opacity(fade_opacity), FadeOut(real_brace1), real_label.animate.set_opacity(fade_opacity))
        self.play(FadeIn(l2_rects[1]), ShowCreation(box2_line), FadeIn(l3_rects[1], time_span=(0.5, 1.5)), TransformMatchingShapes(l2_groups[1].copy(), l3_groups[1], run_time=1), FadeIn(l3_plusses[1]))
        self.wait()
        self.play(GrowFromCenter(real_brace2), real_label.animate.next_to(real_brace2, DOWN).set_opacity(1))
        self.wait()
        self.play(box2_line.animate.set_stroke(WHITE, 1, 0.5), VGroup(l2_rects[1], l3_rects[1]).animate.set_stroke(GREY, 1, 0.5), l2_groups[2].animate.set_opacity(1), l2_groups[1].animate.set_opacity(fade_opacity), l3_groups[1].animate.set_opacity(fade_opacity), l3_plusses[1].animate.set_opacity(fade_opacity), FadeOut(real_brace2), FadeOut(real_label))
        self.play(FadeTransform(l2_groups[2].copy(), l3_groups[2], run_time=1.5), ShowCreation(box3_line, run_time=1.5), FadeIn(l2_rects[1]), FadeIn(l3_rects[2], time_span=(0.5, 1.5)), FadeIn(l3_plusses[2]))
        self.wait()
        self.play(FadeOut(VGroup(box1_lines, box2_line, box3_line)), FadeOut(l2_rects), l3_rects.animate.set_stroke(TEAL, 1, 1), lines[2].animate.set_opacity(0.5), lines[3].animate.set_opacity(1))
        self.wait()
    ...
    def connecting_line(self, high_box, low_box):
        return CubicBezier(high_box.get_bottom(), high_box.get_bottom() + 1.0 * DOWN, low_box.get_top() + 1.0 * UP, low_box.get_top()).set_stroke(WHITE, 2)
...
class PlaneAfterFilm(InteractiveScene):
    ...
    def construct(self):
        ...
        image.set_height(FRAME_HEIGHT)
        image.fix_in_frame()
        rect = Polygon((2.56, -0.12, 0.0), (5.69, -2.51, 0.0), (6.25, 1.31, 0.0), (2.72, 2.62, 0.0))
        ...
        self.play(DrawBorderThenFill(rect), FadeIn(words))
        self.wait()

class LookingAtScreen(TeacherStudentsScene):

    def construct(self):
        self.play(self.change_students('confused', 'pondering', 'maybe', look_at=self.screen), self.teacher.change('well'))
        self.wait(5)
...
class DiffractionEquation(InteractiveScene):
    ...
    def construct(self):
        equation = Tex('{d} \\cdot \\sin(\\theta) = \\lambda', font_size=60)
        equation.set_backstroke(BLACK)
        arrow = Vector(DOWN, thickness=4)
        arrow.set_color(BLUE)
        d, theta, lam = syms = [equation[s][0] for s in ['{d}', '\\theta', '\\lambda']]
        colors = [BLUE, YELLOW, TEAL]
        arrow.next_to(d, UP, LARGE_BUFF)
        arrow.set_fill(opacity=0)
        self.add(equation)
        for sym, color in zip(syms, colors):
            self.play(FlashAround(sym, color=color, time_span=(0.25, 1.25)), sym.animate.set_fill(color), arrow.animate.next_to(sym, UP).set_fill(color, 1))
            self.wait()
