...
class LightWaveSlice(Mobject):
    ...
    data_dtype: Sequence[Tuple[str, type, Tuple[int]]] = [('point', np.float32, (3,))]
    render_primitive: int = moderngl.TRIANGLE_STRIP

    def __init__(self, point_sources: DotCloud, shape: tuple[float, float]=(8.0, 8.0), color: ManimColor=BLUE_D, opacity: float=1.0, frequency: float=1.0, wave_number: float=1.0, max_amp: Optional[float]=None, decay_factor: float=0.5, show_intensity: bool=False, **kwargs):
        self.shape = shape
        ...
        self.set_uniforms(dict(frequency=frequency, wave_number=wave_number, max_amp=max_amp, time=0, decay_factor=decay_factor, show_intensity=float(show_intensity), time_rate=1.0))
    ...
    def set_color(self, color: ManimColor | Iterable[ManimColor] | None, opacity: float | Iterable[float] | None=None, recurse=False) -> Self:
    ...
    def interpolate(self, wave1: LightWaveSlice, wave2: LightWaveSlice, alpha: float, path_func: Callable[[np.ndarray, np.ndarray, float], np.ndarray]=straight_path) -> Self:
...
class LightFieldAroundScene(InteractiveScene):
    ...
    def construct(self):
        ...
        scene, scene_top, lamp = group = Group(ImageMobject(os.path.join(folder, 'LightFieldScene')), ImageMobject(os.path.join(folder, 'TopHalfCutoff')), ImageMobject(os.path.join(folder, 'Lamp')))
        group.set_height(7)
        group.to_edge(RIGHT)
        ...
        n_dots = 50
        light = Group((GlowDot(line.pfp(a), color=WHITE, radius=interpolate(0.5, 4, a), opacity=3 / n_dots) for a in np.linspace(0, 1, n_dots)))
        self.play(FadeIn(lamp, time_span=(1, 3)), FadeIn(light, lag_ratio=0.2, time_span=(1, 3)), frame.animate.reorient(0, 0, 0, (3.64, 0.68, 0.0), 5.3), run_time=2)
        self.wait()
        sources = DotCloud([(2.42, -0.27, 0.0), (1.96, -0.14, 0.0), (2.11, 0.05, 0.0), (2.51, 0.16, 0.0), (2.75, 0.07, 0.0), (3.19, 0.41, 0.0), (3.48, 0.39, 0.0), (4.81, 0.28, 0.0), (5.1, 0.22, 0.0), (5.53, -0.04, 0.0), (5.8, -0.18, 0.0), (4.87, -0.58, 0.0), (4.71, -0.68, 0.0), (4.28, -0.61, 0.0), (3.92, -0.51, 0.0), (4.18, -0.27, 0.0), (3.8, 0.09, 0.0), (3.55, 0.23, 0.0), (2.43, 0.7, 0.0), (2.43, 0.7, 0.0), (3.71, 0.51, 0.0), (3.94, -0.49, 0.0), (1.9, -0.3, 0.0), (2.61, 0.06, 0.0), (2.92, 0.23, 0.0), (3.25, 0.38, 0.0)])
        ...
        self.play(FadeIn(wave), FadeIn(perp_waves), light.animate.set_opacity(2 / n_dots), scene.animate.set_opacity(0.5), run_time=2)
        ...
        self.wait(3)
        self.play(eye_dot.animate.move_to(1.5 * LEFT + 2 * DOWN).set_anim_args(path_arc=45 * DEGREES), run_time=12, rate_func=there_and_back)
        ...
        laser_light = VGroup((Line(laser_point, film_rect.pfp(a + random.random() / 250)) for a in np.linspace(0, 1, 250)))
        laser_light.set_stroke(GREEN_SCREEN, (0, 1), 0.2)
        laser_light.shuffle()
        self.play(FadeOut(lamp), FadeOut(light), FadeOut(scene), FadeOut(scene_top), FadeIn(right_rect), FadeIn(film_rect))
...
class LightExposingFilm(DiffractionGratingScene):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(-88, -4, 0, (3.56, -0.71, 4.59), 14.6), FadeIn(exposure, time_span=(0, 3)), run_time=7.5)
        ...
        amp_label = Tex('\\text{Amplitude}', font_size=72)
        ...
        self.play(FadeIn(fade_rect), GrowFromCenter(brace), FadeIn(amp_lines[0]), ReplacementTransform(amp_lines[0].copy().fade(1), amp_lines[1]))
        ...
        self.play(wave.animate.pause(), FadeIn(phase_label), FadeIn(phase_arrow), frame.animate.reorient(-99, -2, 0, (3.56, -0.71, 4.59), 14.6))
        self.play(Rotate(phase_vect, -TAU, axis=LEFT, about_point=phase_vect.get_start()), ShowCreation(phase_line), phase_arrow.animate.put_start_and_end_on(phase_arrow.get_start(), phase_line.get_end()), run_time=3)
        self.play(Rotate(phase_vect, TAU, axis=LEFT, about_point=phase_vect.get_start()), phase_arrow.animate.put_start_and_end_on(phase_arrow.get_start(), phase_line.get_start()), run_time=3)
        self.wait(2)
        ...
        phase_vect.add_updater(lambda m: m.put_start_and_end_on(phase_circle.get_center(), phase_circle.pfp(wave.uniforms['frequency'] * wave.uniforms['time'] % 1)))
        phase_vect.always.set_perpendicular_to_camera(self.frame)
        self.play(wave.animate.set_time_rate(0.5), frame.animate.reorient(-55, -13, 0, (2.59, -0.61, 2.18), 17.0), phase_arrow.animate.fade(0.8), phase_label.animate.fade(0.8), FadeOut(phase_line), run_time=3)
        self.play(wave_amp_tracker.animate.set_value(0.25), exposure.animate.set_opacity(0.25), run_time=3)
        self.wait(2)
        self.play(wave_amp_tracker.animate.set_value(1.0), exposure.animate.set_opacity(1.0), run_time=4)
        self.wait()
        exp_expr = Tex('\\text{Exposure} = c \\cdot |\\text{Amplitude}|^2', font_size=72)
        exp_expr.move_to(2 * UP)
        self.play(LaggedStart(Write(exp_expr['\\text{Exposure} = c \\cdot |'][0]), TransformFromCopy(amp_label.copy().clear_updaters(), exp_expr['\\text{Amplitude}'][0]), Write(exp_expr['|^2'][0]), lag_ratio=0.1), frame.animate.reorient(-34, -15, 0, (1.48, -0.23, 1.29), 16.08), run_time=3)
        self.wait(8)
        to_fade = VGroup(fade_rect, amp_group, amp_label)
        wave.pause()
        self.play(FadeOut(to_fade, lag_ratio=0.01, time_span=(0, 1.5)), phase_label.animate.match_style(og_phase_label), phase_arrow.animate.set_fill(opacity=1), frame.animate.reorient(-86, -4, 0, (2.43, -0.97, 1.94), 12.91), run_time=2)
        shift_label = TexText('Shift back phase $\\rightarrow$')
        shift_label.rotate(PI / 2, DOWN)
        shift_label.move_to(7 * OUT + 1.5 * DOWN)
        self.play(wave.animate.set_uniform(time_rate=-0.5).set_anim_args(rate_func=there_and_back), FadeIn(shift_label, shift=OUT), run_time=2.0)
        ...
        self.play(FadeOut(phase_label), FadeOut(phase_arrow), wave_amp_tracker.animate.set_value(0.75), exposure.animate.set_opacity(0.75), frame.animate.reorient(-32, -20, 0, (1.48, -0.73, 0.84), 14.72), run_time=2)
        self.wait_until(lambda: 8 / 30 < wave.uniforms['time'] % 1 < 9 / 30)
        self.add(wave2)
        ...
        self.play(wave.animate.pause(), wave2.animate.pause())
        self.play(wave.animate.set_uniform(time_rate=-0.5).set_anim_args(rate_func=there_and_back), exposure.animate.set_max_amp(0.2).set_opacity(0.15), FadeIn(shift_label, OUT), run_time=2.0)
        self.play(FadeOut(shift_label))
        self.play(wave.animate.set_time_rate(0.5), wave2.animate.set_time_rate(0.5))
        self.play(frame.animate.reorient(-54, -16, 0, (3.01, -0.09, 0.55), 14.47), run_time=10)
        wave.pause()
        wave2.pause()
        self.play(wave.animate.set_uniform(time_rate=-0.5).set_anim_args(rate_func=there_and_back), exposure.animate.set_max_amp(0.1).set_opacity(1), FadeIn(shift_label, OUT), run_time=2.0)
        self.play(FadeOut(shift_label))
        wave.set_time_rate(0.5)
        wave2.set_time_rate(0.5)
        self.play(frame.animate.reorient(15, -22, 0, (2.14, -0.48, -0.07), 15.51), run_time=12)
        wave.pause()
        wave2.pause()
        self.play(wave.animate.set_uniform(time_rate=-0.5).set_anim_args(rate_func=there_and_back), exposure.animate.set_max_amp(0.2).set_opacity(0.15), run_time=2.0)
        wave.set_time_rate(0.5)
        wave2.set_time_rate(0.5)
        self.play(frame.animate.reorient(15, -14, 0, (1.47, 0.09, -0.04), 13.26), run_time=12)
...
class TwoInterferingWaves(DiffractionGratingScene):
    ...
    def construct(self):
        ...
        rect = film_border
        obj_wave.set_points([2 * obj_vect, rect.get_corner(UL), rect.get_corner(DL), 2 * obj_vect, rect.get_corner(UL), rect.get_corner(UR), 2 * obj_vect, rect.get_corner(UR), rect.get_corner(DR), 2 * obj_vect, rect.get_corner(DR), rect.get_corner(DL)])
        ref_wave.set_points([2 * ref_vect, rect.get_corner(UL), rect.get_corner(DL), 2 * ref_vect, rect.get_corner(UL), rect.get_corner(UR), 2 * ref_vect, rect.get_corner(UR), rect.get_corner(DR), 2 * ref_vect, rect.get_corner(DR), rect.get_corner(DL)])
        obj_wave.deactivate_depth_test()
        ref_wave.deactivate_depth_test()
        waves = Group(ref_wave, obj_wave)
        waves.set_opacity(0.25)
        self.play(FadeIn(waves), frame.animate.reorient(0, -21, 0, (-0.09, -0.86, 0.04), 9.4), run_time=3)
        self.wait(3)
        if False:
            ...
            self.wait(4)
            self.play(FadeOut(ref_wave), FadeOut(film_border), FadeOut(film_image), FadeOut(obj_wave), FadeIn(new_obj_wave))
            self.play(new_obj_wave.animate.scale(10), frame.animate.reorient(-3, -5, 0, (-0.96, 0.03, -0.15), 5.32), run_time=5)
            self.wait(4)
            ...
            self.play(FadeIn(full_rect), ShowCreation(circle))
            self.wait(8)
        ...
        self.add(exposure, waves)
        self.play(FadeIn(exposure, run_time=5), FadeOut(waves, time_span=(3, 5)))
        self.play(frame.animate.reorient(0, 0, 0, (0.5, 0.07, 0.0), 0.15), film_image.animate.set_opacity(0.1).set_anim_args(time_span=(0, 5)), run_time=8)
...
class SinglePointOnFilm(DiffractionGratingScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(frame.animate.reorient(-14, -11, 0, (0.27, 0.14, 3.59), 6.17), Rotate(source_label, PI / 2, UP, about_point=source_point.get_center()), Rotate(wave_stack, PI / 2, UP, about_point=source_point.get_center()), run_time=10)
        self.play(FadeOut(wave_stack), FadeOut(source_label), FadeOut(plate), FadeOut(film_label), obj_wave.animate.set_decay_factor(0.5), frame.animate.reorient(0, -90, 0, source_point.get_center(), 4).set_anim_args(run_time=4))
        self.wait(2)
...
class ExplainWaveVisualization(DiffractionGratingScene):
    ...
    def construct(self):
        ...
        def field_func(points):
            ...
            return result
        linear_field = VectorField(field_func, sample_points=np.linspace(ORIGIN, 10 * UP, 100), max_vect_len=1.0)
        linear_field.always.update_vectors()
        linear_field.set_stroke(WHITE, width=1.5, opacity=0.75)
        full_field = VectorField(field_func, width=full_width, height=full_width, x_density=5, y_density=5, max_vect_len=0.5)
        full_field.sample_points
        full_field.set_stroke(WHITE, width=1.5, opacity=0.25)
        full_field.always.update_vectors()
        self.play(frame.animate.reorient(71, 77, 0, (-0.62, 0.7, 0.19), 3.11), wave.animate.set_uniform(time_rate=0.5).set_anim_args(suspend_mobject_updating=False), VFadeIn(full_field, time_span=(0, 2)), run_time=6)
        self.wait(2)
        self.play(VFadeIn(linear_field), VFadeOut(full_field))
        self.wait(5)
        self.wait_until(lambda: 15 / 30 < wave.uniforms['time'] % 1 < 16 / 30)
        self.play(wave.animate.pause().set_anim_args(suspend_mobject_updating=False))
        self.wait()
        ...
        self.play(linear_field.animate.set_stroke(opacity=0.2), GrowArrow(sample_vect, run_time=2), frame.animate.reorient(81, 84, 0, (-0.5, 0.69, 0.23), 1.65).set_anim_args(run_time=3))
        self.wait()
        ...
        self.wait()
        self.play(VFadeOut(sample_vect, time_span=(0, 2)), FadeOut(sample_vect.base_point, time_span=(0, 2)), linear_field.animate.set_stroke(opacity=0.75).set_anim_args(time_span=(0, 2)), wave.animate.set_uniform(time_rate=0.5).set_anim_args(suspend_mobject_updating=False), frame.animate.reorient(49, 79, 0, (-0.14, 1.34, 0.23), 2.78), run_time=5)
        self.wait(2)
        full_field = VectorField(lambda p: 0.5 * field_func(p), x_density=4, y_density=4, z_density=2, depth=4, max_vect_len=0.5)
        full_field.set_stroke(WHITE, width=1, opacity=0.25)
        full_field.always.update_vectors()
        added_waves = Group((wave.copy().rotate(PI / 2, UP).shift(z * RIGHT).set_opacity(0.1) for z in np.arange(-2, 2, 0.1)))
        self.play(VFadeOut(linear_field), FadeIn(added_waves, suspend_mobject_updating=False), frame.animate.reorient(48, 79, 0, (-0.42, 1.07, 0.35), 4.18).set_anim_args(run_time=8))
        wave.save_state()
        wave.scale(0)
        self.play(Restore(wave, suspend_mobject_updating=False), FadeOut(added_waves, time_span=(0, 1), suspend_mobject_updating=False), run_time=3)
        self.wait(2)
        ...
        linear_field = VectorField(field_func, sample_points=line.get_anchors()[::2], max_vect_len=1.0)
        linear_field.set_stroke(WHITE, 0.75)
        linear_field.always.update_vectors()
        self.add(line, graph)
        self.play(ShowCreation(line, time_span=(0, 3)), ShowCreation(graph, time_span=(0, 3)), VFadeIn(linear_field, time_span=(0, 3), suspend_mobject_updating=False), frame.animate.reorient(0, 73, 0, (-0.03, 1.7, 0.2), 4.65), run_time=10)
        self.wait(8)
...
class CreateZonePlate(DiffractionGratingScene):
    ...
    def construct(self):
        ...
        def get_all_sources():
            return np.vstack([obj_wave.point_sources.get_points(), ref_wave.point_sources.get_points()])
        ...
        plate_body.move_to(plate.get_center() + 0.01 * IN)
        ...
        plate_body.move_to(plate.get_center() + 0.01 * IN)
        self.play(FadeIn(source_point), FadeIn(source_label), GrowArrow(source_arrow), frame.animate.reorient(-4, -10, 0, (-0.74, 0.32, -0.49), 7.08).set_anim_args(run_time=5))
        self.play(TransformMatchingStrings(source_label, obj_wave_label), FadeOut(source_arrow), frame.animate.reorient(0, -47, 0, (-0.74, 0.32, -0.49), 7.08).set_anim_args(run_time=3))
        self.wait(3)
        ...
        self.play(FadeOut(obj_wave, 0.1 * DOWN), FadeOut(obj_wave_label), FadeOut(source_point), FadeIn(ref_wave, 0.1 * DOWN), FadeIn(ref_wave_label), frame.animate.reorient(0, -40, 0, (-0.16, 0.01, -0.24), 7.08).set_anim_args(run_time=4))
        self.play(FadeIn(wave_fronts, time_span=(0, 2), lag_ratio=0.05), frame.animate.reorient(0, -37, 0, (-0.16, 0.01, -0.24), 7.08), run_time=8)
        ...
        self.play(MoveToTarget(ref_wave, run_time=2), Rotate(ref_wave.point_sources, angle, axis=UP, about_point=ORIGIN), Rotate(ref_wave_label, angle, axis=UP, about_point=film.get_center()), run_time=10, rate_func=lambda t: there_and_back_with_pause(t, 0.5))
        self.wait(8)
        ...
        comb_wave.point_sources = DotCloud([*(source_point.get_center() for x in range(2)), *np.linspace(20 * OUT + 4 * LEFT, 20 * OUT + 4 * RIGHT, 25)])
        ...
        self.wait(8)
        self.play(FadeOut(comb_label), comb_wave.animate.set_opacity(0.1), FadeOut(film_label), FadeOut(exposure), frame.animate.reorient(80, -2, 0, (-0.64, 0.23, -0.93), 4.34), run_time=5)
        ...
        obj_line, ref_line = lines = VGroup((Line(source_point.get_center(), film.get_center()).set_stroke(color, 1, 0.5) for color in colors))
        ref_line.scale(2, about_point=ref_line.get_end())
        ref_line.shift(0.02 * RIGHT)
        obj_graph, ref_graph = graphs = VGroup((self.get_graph_over_wave(line, wave, scale_factor=sf, direction=UP, color=color) for line, color, wave, sf in zip(lines, colors, [obj_wave, ref_wave], [0.15, 0.1])))
        ...
        self.play(ShowCreation(obj_line), ShowCreation(obj_graph), FadeIn(obj_label, lag_ratio=0.1), run_time=2)
        self.wait(3)
        self.play(ShowCreation(ref_line), ShowCreation(ref_graph), FadeIn(ref_label, lag_ratio=0.1))
        self.wait(4)
        round_exposure = self.get_round_exposure(exposure, radius=0.25)
        self.play(GrowFromCenter(round_exposure), frame.animate.reorient(53, -15, 0, (-0.83, 0.12, -0.62), 5.25).set_anim_args(run_time=3))
        exposure.replace(plate, stretch=True)
        exposure.set_shape(0.5 * plate.get_width(), 0.25)
        exposure.move_to(plate.get_center(), LEFT).shift(0.01 * OUT)
        full_exposure = exposure.copy()
        full_exposure.replace(plate, stretch=True)
        full_exposure.shift(0.02 * OUT)
        exposure.save_state()
        exposure.stretch(0, 0, about_edge=LEFT)
        O_point = obj_label[0].get_center()
        obj_label.add_updater(lambda m: m.rotate(angle_of_vector((m[-1].get_center() - m[0].get_center())[0::2]) - angle_of_vector(obj_line.get_vector()[0::2]), axis=UP).shift(O_point - m[0].get_center()))
        ...
        self.play(obj_wave.animate.pause(), ref_wave.animate.pause(), comb_wave.animate.pause())
        self.play(trg_point.animate.move_to(film.get_right()), round_exposure.animate.shift(0.01 * OUT), MaintainPositionRelativeTo(ref_label, ref_line), Restore(exposure), frame.animate.reorient(35, -14, 0, (1.01, 0.23, -2.8), 8.4), run_time=12)
        self.wait(2)
        trg_x = 0.9
        ...
        d_line_label = Tex('D + \\frac{\\lambda}{2}', font_size=30).rotate(PI / 2, LEFT)
        VGroup(mid_line_label, d_line_label).set_fill(TEAL, 1)
        self.play(FadeOut(obj_label), FadeOut(ref_label), FadeOut(ref_line), FadeOut(ref_graph), frame.animate.reorient(0, -62, 0, (-0.27, -1.26, -1.17), 7.38), trg_point.animate.move_to(plate.get_center() + trg_x * RIGHT), run_time=3)
        d_line_label.next_to(obj_line.get_center(), RIGHT, buff=SMALL_BUFF)
        self.play(FadeIn(mid_line), FadeIn(mid_line_label), FadeIn(d_line_label))
        self.wait(2)
        self.play(frame.animate.reorient(84, -9, 0, (-0.26, -0.27, -1.86), 3.23), FadeOut(VGroup(mid_line, mid_line_label, d_line_label)), FadeIn(ref_line), FadeIn(ref_graph), obj_wave.animate.unpause(), ref_wave.animate.unpause(), comb_wave.animate.unpause(), run_time=5)
        self.wait(5)
        ...
        self.play(frame.animate.reorient(41, -15, 0, (-0.54, -0.17, -1.78), 4.65), ShowCreation(circle), UpdateFromFunc(trg_point, lambda m, c=circle: m.move_to(c.get_end())), round_exposure.animate.set_width(3), FadeOut(exposure, time_span=(0, 1)), run_time=4)
        self.play(FadeOut(circle, run_time=2))
        self.remove(tail)
        self.wait(2)
        exposure.replace(plate, stretch=True).shift(0.01 * OUT)
        self.add(exposure, round_exposure)
        self.play(FadeOut(round_exposure, run_time=2), FadeIn(exposure, run_time=2), comb_wave.animate.set_opacity(0.5).set_anim_args(time_span=(6, 8)), frame.animate.reorient(0, -23, 0, (-0.14, 0.01, -2.23), 6.52).set_anim_args(run_time=8))
        time0 = self.time
        ...
        self.wait(4)
        self.play(FadeOut(VGroup(ref_line, ref_graph, obj_line, obj_graph)), FadeOut(comb_wave))
        ...
        self.play(ShowCreation(mid_line, suspend_mobject_updating=True), VFadeIn(dist_label))
        for vect in [2 * IN, 4 * OUT, 2 * IN]:
            self.play(source_point.animate.shift(vect), exposure.point_sources.animate.shift(vect), run_time=3)
            self.wait()
        axes = ThreeDAxes()
        ...
        self.play(Write(axes, lag_ratio=0.01), frame.animate.reorient(-44, -21, 0, (0.09, -0.06, -1.4), 7.22), exposure.animate.set_opacity(0.5), FadeOut(dist_label), run_time=3)
        ...
        for point in points:
            ...
            self.wait()
        frame.clear_updaters()
        self.play(FadeOut(axes), FadeOut(mid_line), FadeOut(source_point))
        ref_wave.set_opacity(0.75)
        ref_wave.set_wave_number(4.0)
        ref_wave.set_frequency(1.0)
        ref_wave.unpause()
        self.add(exposure, ref_wave)
        self.play(FadeIn(ref_wave, time_span=(0, 2)), frame.animate.reorient(0, -24, 0, (0.08, -0.07, -1.39), 7.22), run_time=5)
        self.wait(4)
        frame.reorient(-171, -18, 0, (0.12, -0.21, -1.33), 9.31)
        self.remove(film)
        self.add(exposure, plate)
        self.play(frame.animate.reorient(0, -16, 0, (-3.06, -0.18, -3.19), 1.49), run_time=10)
        self.wait(4)
...
class ShowEffectOfChangedReferenceAngle(InteractiveScene):
    ...
    def construct(self):
        ...
        exposure.set_decay_factor(0)
        exposure.set_wave_number(wave_number)
        exposure.replace(plate_border, stretch=True).shift(0.01 * OUT)
        exposure.set_color(WHITE, 0.85)
        ...
        self.wait()
        self.play(Rotate(source_points, 60 * DEGREES, axis=UP, about_point=source_points.get_points()[0], run_time=5), frame.animate.reorient(-48, -46, 0, (1.17, -0.41, 1.55), 9.28), run_time=5)
        self.wait(3)
...
class DoubleSlit(DiffractionGratingScene):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(33, 65, 0, (1.24, 1.09, -0.39), 10.01), UpdateFromAlphaFunc(graph, lambda m, a: m.set_stroke(width=3 * clip(there_and_back_with_pause(2 * a, 0.7), 0, 1))), Restore(n_slit_wall, time_span=(9, 12)), run_time=15)
        ...
        self.add(sources)
        self.play(frame.animate.reorient(1, 49, 0, (-0.02, 3.62, 0.61), 11.96), FadeIn(out_wave, time_span=(0, 2), suspend_mobject_updating=False), run_time=10)
        self.wait(3)
        wall = n_slit_wall
        ...
        in_wave.move_to(ORIGIN, UP)
        start_arrows, end_arrows = [VGroup((Tex('\\leftrightarrow').set_width(0.7 * block.get_width(), stretch=True).next_to(block, OUT) for block in group[1:-1])).rotate(30 * DEGREES, RIGHT).set_backstroke(BLACK, 5) for group in [wall, wall.target]]
        self.play(FadeIn(start_arrows))
        self.play(Transform(start_arrows, end_arrows), MoveToTarget(wall), sources.animate.match_points(self.get_point_sources_from_wall(wall.target)), run_time=5)
        ...
        self.play(FadeOut(wall, scale=0.9), FadeOut(out_wave, suspend_mobject_updating=False), FadeIn(single_slit_wall))
        self.wait(3)
        self.play(frame.animate.reorient(0, 9, 0, (-0.04, 1.61, 0.15), 8.66), FadeIn(radial_wave, suspend_mobject_updating=False), run_time=3)
        self.wait(2)
        single_slit_wall.set_z_index(1)
        self.play(FadeIn(source), single_slit_wall.animate.set_opacity(0.1), in_wave.animate.set_opacity(0.1))
        self.wait(6)
        ...
        exposure.set_opacity(0.001)
        radial_wave.set_z_index(1)
        single_slit_wall.set_opacity(1)
        single_slit_wall.set_depth(1.5, about_edge=IN)
        self.play(FadeOut(source), FadeIn(film, shift=5 * IN), FadeIn(exposure, shift=5 * IN), FadeIn(single_slit_wall), in_wave.animate.set_opacity(0.85).set_time_rate(1.0).set_anim_args(suspend_mobject_updating=False), radial_wave.animate.set_time_rate(1.0).set_anim_args(suspend_mobject_updating=False), frame.animate.reorient(-19, 68, 0, (-2.38, 4.22, 0.53), 10.86), run_time=3)
        ...
        self.play(VFadeIn(graph), VFadeIn(line), FadeIn(exposure_glow), frame.animate.reorient(-46, 65, 0, (-1.12, 3.76, 0.49), 6.83), run_time=3)
        self.wait(3)
        self.play(exposure_glow.animate.shift(5 * RIGHT).set_opacity(0.5), run_time=3)
        self.play(radial_wave.animate.set_decay_factor(1.0).set_max_amp(0.75).set_anim_args(suspend_mobject_updating=False), run_time=2)
        self.play(frame.animate.reorient(9, 59, 0, (0.54, 4.02, 0.04), 8.32), run_time=12)
        self.wait(4)
        ...
        self.wait(4)
        self.play(frame.animate.reorient(-25, 48, 0, (0.23, 4.15, -0.03), 9.8), run_time=8)
        self.play(frame.animate.reorient(28, 50, 0, (0.23, 4.15, -0.03), 9.8), run_time=8)
        out_wave = radial_wave
        source_pair = source
        source1 = source.copy().set_points(source.get_points()[0:1])
        source2 = source.copy().set_points(source.get_points()[1:2])
        self.play(FadeOut(two_slit_wall, shift=2 * IN), FadeOut(in_wave, suspend_mobject_updating=False), FadeIn(source1), FadeIn(source2))
        self.play(frame.animate.reorient(0, 68, 0, (0.23, 4.15, -0.03), 9.8), run_time=4)
        wave1 = out_wave.copy().set_sources(source1).shift(0.001 * OUT)
        wave2 = out_wave.copy().set_sources(source2).shift(0.001 * IN)
        exp1 = exposure.copy().set_sources(source1).shift(0.001 * DOWN)
        exp2 = exposure.copy().set_sources(source2).shift(0.002 * DOWN)
        self.play(FadeOut(source2), FadeOut(out_wave, suspend_mobject_updating=False), FadeIn(wave1, suspend_mobject_updating=False), exposure.animate.set_opacity(0), FadeIn(exp1))
        self.wait(3)
        self.add(wave2, wave1)
        self.play(FadeOut(source1), FadeIn(source2), FadeOut(wave1, suspend_mobject_updating=False), FadeIn(wave2, suspend_mobject_updating=False), FadeOut(exp1), FadeIn(exp2))
        self.wait(3)
        self.play(FadeIn(source1), FadeOut(wave2, suspend_mobject_updating=False), FadeIn(out_wave, suspend_mobject_updating=False), FadeOut(exp2), exposure.animate.set_opacity(1))
        self.wait(3)
        ...
        graphs = VGroup(self.get_graph_over_wave(lines[0], wave1), self.get_graph_over_wave(lines[1], wave2))
        self.play(out_wave.animate.pause().set_opacity(0.5), exposure.animate.set_opacity(0), frame.animate.reorient(0, 69, 0, (-0.09, 4.1, -0.16), 7.52), FadeIn(exposure_point), run_time=3)
        ...
        self.play(*(wave.animate.unpause().set_anim_args(suspend_mobject_updating=False) for wave in [out_wave, wave1, wave2]))
        self.wait(4)
        self.play(frame.animate.reorient(0, 66, 0, (0.12, 4.03, -0.38), 7.52), *(wave.animate.pause().set_anim_args(suspend_mobject_updating=False) for wave in [out_wave, wave1, wave2]), run_time=3)
        self.wait()
        self.play(exposure_point.animate.shift(0.9 * RIGHT).set_opacity(0.25), run_time=2)
        self.wait()
        self.play(frame.animate.reorient(-98, 82, 0, (0.27, 4.19, -0.01), 2.93), *(wave.animate.unpause().set_anim_args(suspend_mobject_updating=False) for wave in [out_wave, wave1, wave2]), run_time=3)
        self.wait(8)
        self.play(frame.animate.reorient(-2, 69, 0, (-0.23, 2.91, 0.1), 6.38), exposure.animate.set_opacity(1), run_time=4)
        self.wait(3)
        self.play(exposure_point.animate.shift(0.9 * RIGHT).set_opacity(1), run_time=2)
        self.wait(2)
        self.play(frame.animate.reorient(81, 88, 0, (-0.23, 2.91, 0.1), 6.38), run_time=5)
        self.wait(2)
        self.play(frame.animate.reorient(0, 67, 0, (-0.07, 1.75, 0.9), 6.42), *(wave.animate.pause().set_anim_args(suspend_mobject_updating=False) for wave in [out_wave, wave1, wave2]), run_time=4)
        ...
        self.play(*(wave.animate.set_wave_number(2).set_anim_args(suspend_mobject_updating=False) for wave in [out_wave, wave1, wave2, exposure]), UpdateFromAlphaFunc(Point(), lambda m, a: exposure.set_color(interpolate_color_by_hsl(GREEN_SCREEN, trg_color, a)), remover=True), exposure_point.animate.set_color(trg_color), run_time=6)
        ...
        self.play(exposure_point.animate.shift(3 * LEFT), rate_func=there_and_back, run_time=16)
        self.play(FadeOut(lines), FadeOut(graphs))
        ...
        self.play(FadeIn(new_in_wave, time_span=(0, 2), suspend_mobject_updating=False), FadeIn(two_slit_wall, time_span=(0, 1)), FadeOut(source1, time_span=(0, 2)), FadeOut(source2, time_span=(0, 2)), frame.animate.reorient(-25, 62, 0, (-0.82, 2.58, 0.5), 9.13), run_time=8)
        self.play(frame.animate.reorient(30, 60, 0, (-0.48, 2.77, 0.52), 9.13), rate_func=there_and_back, run_time=24)

class FullDiffractionGrating(DiffractionGratingScene):

    def construct(self):
        full_width = 100
        wave_number = 2 + 0.1 * PI
        frequency = 1
        slit_dist = 1.0
        frame = self.frame
        ...
        self.play(GrowFromCenter(brace), FadeIn(dist_label, 0.25 * OUT), frame.animate.reorient(11, 72, 0, (-1.12, 5.34, -0.64), 11.13).set_anim_args(run_time=8))
        ...
        individual_sources = Group((sources.copy().set_points(sources.get_points()[i:i + 1]) for i in range(sources.get_num_points())))
        ...
        self.play(out_wave.animate.set_opacity(0.2).set_anim_args(suspend_mobject_updating=False), FadeOut(dist_label), FadeIn(point_tracker))
        self.wait()
        self.add(lines, graphs, out_wave)
        self.add(waves)
        self.play(ShowCreation(lines, lag_ratio=0.01, time_span=(0, 2)), ShowCreation(graphs, lag_ratio=0.01, suspend_mobject_updating=False, time_span=(0, 2)), frame.animate.reorient(1, 57, 0, (-0.17, 5.75, 0.32), 14.54), run_time=5)
        self.wait(3)
        self.play(FadeOut(lines), FadeOut(graphs), FadeOut(point_tracker), out_wave.animate.set_opacity(0.85).set_anim_args(suspend_mobject_updating=False), frame.animate.reorient(0, 42, 0, (0, 5.4, 0.45), 13.0), run_time=3)
        self.remove(waves)
        out_wave.set_width(500, about_edge=DOWN)
        self.play(frame.animate.reorient(0, 0, 0, (0, 95, 0), 200), out_wave.animate.set_max_amp(1).set_anim_args(suspend_mobject_updating=False), run_time=20)
        self.wait(4)
        in_wave.scale(0)
        out_wave.scale(0)
        ...
        theta = math.asin(1.0 / wave_number / slit_dist)
        ...
        self.play(TransformFromCopy(beam_outlines[0], d_line), TransformFromCopy(beam_outlines[1], d_line))
        ...
        hyp_label = Tex('\\sqrt{L^2 + x^2}', font_size=800)
        ...
        self.play(FadeIn(beam_outlines), FadeOut(d_line), FadeOut(v_line), FadeOut(arc), FadeOut(theta_sym))
        self.wait(2)
        self.play(FadeIn(point_tracker), out_wave.animate.set_opacity(0.5).set_anim_args(suspend_mobject_updating=False))
        self.play(ShowCreation(L_line), VFadeIn(L_label), FadeOut(beam_outlines))
        self.wait()
        self.play(TransformFromCopy(L_line, hyp), ShowCreation(x_line), TransformMatchingStrings(L_label.copy(), hyp_label), FadeIn(x_label, shift=3 * LEFT))
        self.wait(4)
        ...
        self.play(Rotate(point_tracker, -new_angle, about_point=ORIGIN), VFadeIn(question), run_time=3)
        self.play(ShowCreation(d_line))
        self.play(TransformFromCopy(d_line, v_line), ShowCreation(arc), Write(theta_sym))
        self.wait(2)
        self.play(ShowCreation(lines, lag_ratio=0.01, run_time=2, suspend_mobject_updating=True), FadeOut(VGroup(v_line, d_line, arc, theta_sym)))
        self.wait(4)
        self.play(point_tracker.animate.move_to(1.15 * point_tracker.get_center()), run_time=2)
        self.play(frame.animate.reorient(0, 0, 0, (0.0, 2.75, 0.0), 8), lines.animate.set_stroke(width=5), out_wave.animate.set_opacity(0.1).set_anim_args(suspend_mobject_updating=False), in_wave.animate.set_opacity(0.1).set_anim_args(suspend_mobject_updating=False), sources.animate.set_radius(0.35), run_time=6)
        self.wait(4)
        ...
        self.play(MoveToTarget(lines), ShowCreation(line1, time_span=(0.5, 1.5)), FadeIn(long_label, time_span=(0.5, 1.5)))
        self.wait()
        self.play(ShowCreation(line2), TransformMatchingStrings(long_label.copy(), short_label))
        self.wait()
        for line in [line1, line2]:
        ...
        point_label = TexText("Point we're\\\\analyzing")
        point_label.set_height(1.5 * question.get_height())
        point_label.move_to(question, UL)
        self.remove(question)
        self.add(point_label)
        self.play(frame.animate.reorient(0, 0, 0, (14.74, 92.42, 0.0), 209.77), line2.animate.set_stroke(width=50), run_time=3)
        self.add(tail)
        self.wait(2)
        self.play(Rotate(line2, -30 * DEGREES, about_point=point_tracker.get_center(), rate_func=lambda t: wiggle(t, 2), run_time=8))
        self.wait(2)
        self.play(frame.animate.reorient(0, 0, 0, (0, 1.5, 0.0), 6.0), line2.animate.set_stroke(width=5), VFadeOut(tail), FadeOut(point_label), FadeOut(in_wave, suspend_mobject_updating=False), FadeOut(out_wave, suspend_mobject_updating=False), run_time=3)
        tail.add_updater(lambda m: m.set_stroke(width=(0, 5)))
        self.add(tail)
        self.play(Rotate(line2, -1 * DEGREES, about_point=point_tracker.get_center(), rate_func=there_and_back, run_time=5))
        self.wait(3)
        self.remove(tail)
        p1 = line1.get_start()
        p2 = line2.get_start()
        ...
        diff_label_group = always_redraw(lambda: self.get_diff_label_group(p1=sources.get_points()[15], p2=sources.get_points()[16], theta=PI / 2 - line1.get_angle()))
        diff_label_group.suspend_updating()
        triangle, elbow, altitude, arc, small_theta_sym, diff_segment, brace, d_label = diff_label_group
        self.play(ShowCreation(altitude), FadeOut(VGroup(long_label, short_label)), frame.animate.reorient(0, 0, 0, (0.5, 0.88, 0.0), 3.8), sources.animate.set_radius(0.2))
        ...
        label1 = TexText('Length of shorter line $\\rightarrow$', font_size=24)
        ...
        self.play(Write(label1, stroke_width=1), ShowCreation(line2), run_time=1)
        self.wait()
        self.play(TransformFromCopy(line2, matched_segment), line1.animate.set_stroke(width=1))
        self.wait()
        self.play(LaggedStart(GrowFromCenter(brace), GrowFromCenter(diff_segment), Write(diff_label, stroke_width=1), lag_ratio=0.2))
        self.wait()
        d_sine_theta = Tex('d \\cdot \\sin(\\theta)', font_size=24)
        d_sine_theta.move_to(diff_label, RIGHT)
        self.add(triangle, elbow, altitude, diff_segment)
        self.play(wall.animate.set_height(0.01, stretch=True), FadeIn(triangle), FadeOut(label1))
        self.wait()
        self.play(FadeIn(d_label, 0.25 * DOWN))
        self.wait()
        self.play(TransformMatchingStrings(d_label.copy(), d_sine_theta, run_time=1), FadeOut(diff_label))
        self.wait()
        self.play(TransformFromCopy(d_sine_theta['\\theta'][0], small_theta_sym), ShowCreation(arc))
        self.wait()
        self.remove(in_wave, out_wave)
        self.checkpoint('d*sin(theta)')
        lambda_label = Tex('= \\lambda')
        ...
        sine.add_updater(lambda m: m.put_start_and_end_on(diff_segment.get_start(), diff_segment.get_end()).scale(n_cycles, about_point=diff_segment.get_start()))
        ...
        self.play(d_sine_theta.animate.scale(0.75).next_to(lambda_label, LEFT, buff=0.05).shift(0.025 * DOWN), FadeIn(lambda_label, 0.25 * RIGHT), FadeOut(line2), FadeOut(matched_segment), diff_segment.animate.set_stroke(width=2), frame.animate.reorient(0, 0, 0, (0.14, 0.48, 0.0), 3.17).set_anim_args(run_time=2))
        ...
        for scale_factor in scale_factors:
            ...
            self.play(UpdateFromFunc(point_tracker, lambda m: m.move_to(get_dist_point(1.0 / wave_number))), MaintainPositionRelativeTo(d_sine_theta, lambda_label), sources.animate.scale(scale_factor, about_point=wall_center), wall.animate.scale(scale_factor, about_point=wall_center), FadeIn(arrows, scale=scale_factor, suspend_mobject_updating=False, time_span=(0, 2)), run_time=5)
            self.play(FadeOut(arrows))
        if False:
            d_angle2, d_angle3 = [angle_of_vector(get_dist_point(n / wave_number)) - angle_of_vector(get_dist_point((n + 1) / wave_number)) for n in (1, 2)]
            sine.clear_updaters()
            new_rhs = Tex(f'= 1.00 \\lambda', t2c={'\\lambda': TEAL})
            ...
            self.play(d_sine_theta.animate.next_to(new_rhs, LEFT, buff=0.05).shift(0.02 * DOWN), lambda_label.animate.set_opacity(0), FadeIn(new_rhs))
            self.play(Rotate(point_tracker, -d_angle2, about_point=ORIGIN), Rotate(sine, -d_angle2, about_point=sine.get_start()), factor_tracker.animate.set_value(2.0), MaintainPositionRelativeTo(d_sine_theta, lambda_label), run_time=5)
            self.wait()
            self.play(Rotate(point_tracker, -d_angle2, about_point=ORIGIN), Rotate(sine, -d_angle2, about_point=sine.get_start()), factor_tracker.animate.set_value(3.0), MaintainPositionRelativeTo(d_sine_theta, lambda_label), run_time=5)
            self.wait()
        ...
        self.wait()
        conclusion = VGroup(Text('Difference in distance:', font_size=36), Tex('d \\cdot \\sin(\\theta)'))
        ...
        self.play(FadeIn(conclusion_box), FadeIn(conclusion[0]), TransformFromCopy(d_sine_theta, conclusion[1]))
        self.wait()
        ...
        self.play(ShowCreation(v_line), ShowCreation(d_line), ShowCreation(arc), Write(theta_sym, stroke_width=20), run_time=1)
        theta_sym.resume_updating()
        self.wait(4)
        for scale_factor in scale_factors:
            self.play(UpdateFromFunc(point_tracker, lambda m: m.move_to(get_dist_point(1.0 / wave_number))), sources.animate.scale(scale_factor, about_point=wall_center), wall.animate.scale(scale_factor, about_point=wall_center), run_time=5)
            self.wait()
        for n in [1, 2]:
            d_angle = angle_of_vector(get_dist_point(n / wave_number)) - angle_of_vector(get_dist_point((n + 1) / wave_number))
            self.play(Rotate(point_tracker, -d_angle, about_point=ORIGIN), frame.animate.set_height(450), run_time=5)
            self.wait()
...
class PlaneWaveThroughZonePlate(DiffractionGratingScene):
    ...
    def construct(self):
        ...
        frame.reorient(19, 77, 0, ORIGIN, 8.0)
        self.add(plate)
        self.add(obj_dot)
        ...
        self.play(frame.animate.reorient(58, 73, 0, ORIGIN, 8.0), Write(plane, stroke_width=3, lag_ratio=0.01, time_span=(2, 6)), run_time=6)
        film_point = 2 * UP
        line = Line(film_point, obj_dot.get_center())
        line.set_stroke(TEAL, 3)
        self.play(ShowCreation(line), frame.animate.reorient(0, 0, 0), FadeIn(plate_top), run_time=3)
        self.wait()
        ...
        self.play(FadeOut(obj_dot), Write(dash_circle, stroke_width=3, run_time=1), Write(had_been_words, run_time=1))
        self.wait()
        ...
        theta_prime_sym = Tex("\\theta'")
        theta_prime_sym.set_max_height(0.8 * arc.get_height())
        theta_prime_sym.next_to(arc.pfp(0.4), LEFT, SMALL_BUFF)
        self.play(TransformFromCopy(line, h_line), ShowCreation(arc), Write(theta_prime_sym))
        self.wait()
        self.remove(plate)
        ...
        equations_tex = ['\\lambda = \\sqrt{L^2 + (x + d)^2} - \\sqrt{L^2 + x^2}', '= \\sqrt{L^2 + x^2 + 2xd + d^2} - \\sqrt{L^2 + x^2}', '\\approx \\sqrt{L^2 + x^2 + 2xd} - \\sqrt{L^2 + x^2}', '\\approx \\frac{1}{2\\sqrt{L^2 + x^2}} 2xd', '= d \\cdot \\frac{x}{\\sqrt{L^2 + x^2}}', "= d \\cdot \\sin(\\theta')"]
        equations = VGroup((Tex(eq, t2c={'\\lambda': YELLOW, 'L': BLUE, 'x': RED}, font_size=36) for eq in equations_tex))
        equations.arrange(DOWN, buff=0.65, aligned_edge=LEFT)
        equations.move_to(9.5 * LEFT + 5.65 * UP, UL)
        equations.set_backstroke(BLACK, 10)
        annotations = VGroup(Text('The distances between adjacent fringes and\nthe object should differ by one wavelength'), TexText('$d^2$ is small compared to $xd$'), TexText('Linear approximation:\\\\ \\quad \\\\$\\sqrt{X + \\epsilon} \\approx \\sqrt{X} + \\frac{1}{2\\sqrt{X}} \\epsilon$'))
        ...
        braces = VGroup(Brace(equations[0]['\\sqrt{L^2 + (x + d)^2}'], UP, SMALL_BUFF), Brace(equations[0]['\\sqrt{L^2 + x^2}'], UP, SMALL_BUFF))
        brace_texts = VGroup(TexText('Dist. to fringe\\\\at height $(x + d)$', font_size=24).next_to(braces[0], UP, SMALL_BUFF), TexText('Dist. to fringe\\\\at height $x$', font_size=24).next_to(braces[1], UP, SMALL_BUFF))
        self.play(FadeIn(terms, lag_ratio=0.1, time_span=(0, 2)), Write(equations, time_span=(2, 5)), frame.animate.reorient(0, 0, 0, (-2, 2.5, 0.0), 9).set_anim_args(run_time=3))
        self.wait()
        self.play(LaggedStart(FadeIn(annotations[0]), FadeIn(braces), FadeIn(brace_texts)))
        self.wait()
        self.play(FadeIn(annotations[1]))
        self.play(FadeIn(annotations[2]))
        self.wait()
        key_equation = Tex("d \\cdot \\sin(\\theta') = \\lambda", **kw)
        ...
        self.play(ReplacementTransform(equations[-1][0], key_equation[9], time_span=(0, 2)), ReplacementTransform(equations[-1][1:], key_equation[:9], time_span=(0, 2)), ReplacementTransform(equations[0][0], key_equation[10], time_span=(0, 2)), FadeOut(equations[0][1:], time_span=(1.0, 1.5)), FadeOut(equations[1:-1], lag_ratio=0.01, time_span=(1.0, 2.5)), FadeOut(annotations, lag_ratio=0.01), FadeOut(terms, lag_ratio=0.1, time_span=(1.0, 3.0)), FadeOut(h_line), FadeOut(braces), FadeOut(brace_texts), frame.animate.reorient(0, 0, 0, (0, 1, 0), 6).set_anim_args(time_span=(1, 3.5)))
        self.add(box, key_equation)
        self.play(Write(box), FlashAround(key_equation, buff=MED_SMALL_BUFF, time_width=1.5, run_time=2))
        self.wait()
        ...
        self.play(d_label.animate.next_to(arrow.get_start(), UR, 0.5 * SMALL_BUFF), GrowArrow(arrow), MoveToTarget(lil_brace))
        self.play(d_lines.animate.stretch(0.25, 1, about_edge=DOWN), lil_brace.animate.scale(0.25, about_edge=DL).set_stroke(WHITE, 1), arrow.animate.put_start_and_end_on(arrow.get_start(), arrow.get_end() + 0.05 * LEFT + 0.05 * DOWN), plate_top.animate.stretch(0, 0), Restore(new_plate_top), run_time=2)
        self.wait()
        ...
        theta_sym = Tex('\\theta', font_size=42)
        theta_sym.next_to(upper_arc.pfp(0.4), LEFT, SMALL_BUFF)
        self.play(ShowCreation(upper_arc), Write(theta_sym))
        self.wait(4)
        ...
        diff_eq_label = VGroup(Text('Diffraction\nequation', font_size=36), Vector(RIGHT))
        ...
        self.play(frame.animate.reorient(0, 0, 0, (0.0, 2, 0.0), 8.0), MoveToTarget(box), MoveToTarget(key_equation), Transform(theta_sym_copy, diff_eq['\\theta'][0]), run_time=2)
        self.play(Write(diff_eq), FadeIn(diff_eq_label[0], lag_ratio=0.1), GrowArrow(diff_eq_label[1]))
        self.remove(theta_sym_copy)
        self.wait(2)
        implication = VGroup(Tex('\\Downarrow'), Tex("\\theta = \\theta'"))
        implication.arrange(DOWN)
        implication.next_to(diff_eq, DOWN)
        implication.set_backstroke(width=5)
        self.play(Write(implication))
        self.wait()
        self.play(Transform(theta_sym.copy(), theta_prime_sym, remover=True), Transform(upper_arc.copy(), arc, remover=True), run_time=2)
        self.wait(4)
        ...
        def update_out_beams(beams):
            beams.become(self.get_triple_beam(film_dot.get_center(), obj_dot.get_center()))
            for beam in beams:
        ...
        theta_prime_sym.add_updater(lambda m: m.set_height(min(0.8 * arc.get_height(), 0.35)).next_to(arc.pfp(0.6), LEFT, SMALL_BUFF))
        theta_sym.add_updater(lambda m: m.replace(theta_prime_sym[0]).shift(film_dot.get_center() - obj_dot.get_center()))
        ...
        self.add(plate, plane)
        self.play(FadeOut(equaiton_group, lag_ratio=0.1, time_span=(0, 2)), film_dot.animate.set_y(1.0), FadeOut(plate_top, time_span=(0, 1)), FadeIn(randy, time_span=(1, 3)), frame.animate.reorient(-33, 43, 0, (-2.43, 0.5, -0.12), 9.6), run_time=5)
        self.play(randy.change('pondering', obj_dot))
        self.play(Blink(randy))
        self.wait(3)
        big_ref_wave = self.get_3d_ref_wave(plate)
        self.add(big_ref_wave, plate)
        self.play(FadeIn(big_ref_wave), FadeOut(ref_wave), frame.animate.reorient(-40, 67, 0, (-2.43, 0.5, -0.12), 9.6).set_anim_args(run_time=3))
        self.wait(2)
        ...
        out_beams.clear_updaters()
        self.play(FadeOut(out_beams), FadeOut(VGroup(theta_sym, theta_prime_sym, upper_arc, arc, line)), ShowCreation(lines_out, lag_ratio=0.01, run_time=4), frame.animate.reorient(-93, 62, 0, (-2.43, 0.5, -0.12), 9.6).set_anim_args(run_time=5))
        self.play(FadeOut(lines_out, time_span=(1, 2)), FadeOut(big_ref_wave), FadeIn(mid_lines_out, time_span=(1, 2)), frame.animate.to_default_state(), run_time=3)
        ...
        self.play(FadeIn(glass, time_span=(0, 1)), FadeOut(mid_lines_out, time_span=(1, 2)), FadeOut(ghost_lines, time_span=(1, 2)), FadeIn(dense_lines_out, time_span=(1, 2)), FadeIn(dense_ghost_lines, time_span=(1, 2)), randy.animate.rotate(PI / 2, RIGHT).shift(0.2 * (IN + DOWN)), frame.animate.reorient(-48, 71, 0, (0.53, -0.56, 0.06), 10.86), run_time=3)
        ...
        dense_lines_out.clear_updaters()
        dense_ghost_lines.clear_updaters()
        self.play(FadeOut(dense_lines_out, time_span=(0, 1)), FadeOut(dense_ghost_lines, time_span=(0, 1)), FadeOut(screen_dot, time_span=(0, 1)), FadeOut(glass, time_span=(2, 3)), FadeIn(plate_top, time_span=(2.0, 3)), randy.animate.rotate(PI / 2, LEFT).move_to(4 * LEFT, DOWN), frame.animate.reorient(0, 0, 0, ORIGIN, FRAME_HEIGHT), run_time=3)
        ...
        self.add(out_beams)
        self.play(out_beams[0][1].animate.set_opacity(0.25), out_beams[1][1].animate.set_opacity(0.25))
        ...
        self.play(film_dot.animate.set_y(-4), FadeIn(conj_lines, lag_ratio=0.25, run_time=4), run_time=5)
        ...
        self.play(FadeOut(out_beams), GrowFromPoint(out_wave, film_dot.get_center()), frame.animate.reorient(0, 0, 0, (-0.1, 1.85, 0.0), 11.08).set_anim_args(run_time=4))
        self.wait(4)
    ...
    def get_ghost_lines(self, point_cloud, obj_dot, dash_length=0.15, stroke_color=WHITE, stroke_width=1):
        lines = VGroup((DashedLine(point, obj_dot.get_center(), dash_length=dash_length) for point in point_cloud.get_points()))
    ...
    def get_beam(self, height=0.1, width=15, n_sources=8, source_height=0.15, wave_number=20, frequency=2.3, color=BLUE_A, opacity=0.75, angle=0):
        ...
        wave = LightWaveSlice(mini_sources, wave_number=wave_number, frequency=frequency, color=color, opacity=opacity, decay_factor=0.25, max_amp=0.4 * n_sources)
...
class SuperpositionOfPoints(InteractiveScene):
    ...
    def construct(self):
        ...
        all_points = all_points[:int(0.8 * len(all_points))]
        ...
        dot_cloud.set_points(all_points[:100000])
        ...
        self.add(plate)
        self.add(dot_cloud)
        point_sets = [(2 * LEFT, RIGHT + OUT), (2 * LEFT + IN, RIGHT + OUT), (2 * LEFT + IN, 3 * RIGHT + 2 * IN), (LEFT + 2 * OUT, RIGHT + OUT)]
        for point_set in point_sets:
            self.play(dot_cloud.animate.set_points([*point_set, dist_point]), run_time=3)
            self.wait(2)
        frame.clear_updaters()
        self.play(frame.animate.reorient(-18, -11, 0, (-1.52, 1.18, -0.67), 0.92), run_time=6)
        self.wait()
        ...
        self.play(dot_cloud.animate.set_points([*all_points[:2], dist_point]).set_radius(0.2), run_time=8)
        frame.clear_updaters()
        frame.add_ambient_rotation(0.5 * DEGREES)
        self.play(UpdateFromAlphaFunc(dot_cloud, lambda m, a: m.set_points([*all_points[:int(2 + a * 29)], dist_point])), UpdateFromFunc(plate, lambda m: m.set_max_amp(2 * np.sqrt(dot_cloud.get_num_points()))), run_time=10)
        self.wait(2)
        zone_plates = Group()
        ...
        for n, zone_plate, point in zip(it.count(1), zone_plates, all_points[:30]):
            zone_plate.shift(0.01 * IN)
            zone_plate.save_state()
            zone_plate.scale(0).move_to(point).set_max_amp(2).set_opacity(1)
        self.play(UpdateFromAlphaFunc(plate, lambda m, a: m.set_opacity(1 - there_and_back_with_pause(a, 0.6))), LaggedStartMap(Restore, zone_plates, lag_ratio=0.05), frame.animate.reorient(66, -18, 0, (1.44, 0.59, -6.18), 16.05), run_time=5)
        ...
        beam.shuffle()
        self.play(ShowCreation(beam, lag_ratio=1 / len(beam)), FadeOut(dot_cloud), frame.animate.reorient(83, -27, 0, (-0.63, -0.01, -0.79), 14.36), run_time=2)
        frame.clear_updaters()
        ...
        self.play(plate.animate.set_opacity(0.2).set_anim_args(time_span=(1.6, 1.7)), frame.animate.reorient(162, -3, 0, (-0.89, 0.06, 0.03), 12.77), run_time=4)
        dot_cloud.set_points([all_points[0], dist_point])
        plate.set_max_amp(2 * np.sqrt(dot_cloud.get_num_points()))
        self.add(dot_cloud)
        self.wait()
        self.play(UpdateFromAlphaFunc(dot_cloud, lambda m, a: m.set_points([*all_points[:int(1 + a * 29)], dist_point])), UpdateFromFunc(plate, lambda m: m.set_max_amp(2 * np.sqrt(dot_cloud.get_num_points()))), frame.animate.reorient(207, -8, 0, (-0.89, 0.06, 0.03), 12.77), run_time=12)
        self.wait(2)
        self.play(FadeOut(beam), FadeOut(dot_cloud), frame.animate.reorient(185, -39, 0, (-0.89, 0.06, 0.03), 12.77).set_anim_args(run_time=3))
        dot_cloud.set_color(BLUE).set_glow_factor(1).set_radius(0.1)
        self.play(FadeOut(plate), FadeIn(dot_cloud), frame.animate.reorient(115, -16, 0, (0.33, 0.28, -0.52), 4.38), run_time=3)
        self.play(UpdateFromAlphaFunc(dot_cloud, lambda m, a: m.set_points([*all_points[:int(interpolate(31, 500, a))], dist_point]).set_glow_factor(interpolate(1, 0, a ** 0.25)).set_radius(interpolate(0.1, 0.02, a ** 0.25)).set_opacity(interpolate(1, 0.5, a ** 0.25))), run_time=4)
        self.play(UpdateFromAlphaFunc(dot_cloud, lambda m, a: m.set_points([*all_points[:int(interpolate(500, len(all_points), a ** 3))]]).set_radius(interpolate(0.02, 0.01, a)).set_opacity(interpolate(0.5, 0.2, a))), run_time=5)
        ...
        self.add(sheet_dots)
        self.play(frame.animate.reorient(-16, -45, 0, (-0.97, 1.52, -1.18), 8.67), run_time=2)
        frame.clear_updaters()
        frame.add_ambient_rotation(3 * DEGREES)
        self.play(dot_cloud.animate.shift(2 * IN), UpdateFromFunc(sheet_dots, lambda m: self.color_sheet_by_exposure(m, dot_cloud.get_points()[:1000], wave_number=32)), run_time=3)
        self.wait(3)
        self.play(Rotate(dot_cloud, 120 * DEGREES, axis=UP), UpdateFromFunc(sheet_dots, lambda m: self.color_sheet_by_exposure(m, dot_cloud.get_points()[:1000], wave_number=32)), run_time=3)
        self.wait(3)
        self.play(dot_cloud.animate.shift(3 * OUT), UpdateFromFunc(sheet_dots, lambda m: self.color_sheet_by_exposure(m, dot_cloud.get_points()[:1000], wave_number=32)), run_time=3)
        self.wait(3)
        frame.clear_updaters()
        ...
        self.play(TransformFromCopy(pre_dots, sheet_dots, time_span=(2, 8)), frame.animate.reorient(17, -19, 0, (0.82, 0.57, -3.07), 7.99), run_time=12)
        self.play(dot_cloud.animate.shift(2 * IN), UpdateFromFunc(sheet_dots, lambda m: self.color_sheet_by_exposure(m, dot_cloud.get_points()[:1000], wave_number=32)), run_time=3)
        self.wait()
...
class ComplexWaves(InteractiveScene):
    ...
    def construct(self):
        amp_expr = Tex('\\text{Amplitude}(R + O)^2', font_size=60)
        amp_expr.to_edge(UP)
        RO = amp_expr['R + O'][0]
        RO.save_state()
        RO.set_x(0)
        ...
        self.wait()
        self.play(Write(amp_expr['\\text{Amplitude}('][0]), Write(amp_expr[')'][0], time_span=(1.5, 2)), Restore(RO, time_span=(0.5, 1.5)), run_time=2)
        self.wait()
        self.play(FadeIn(amp_expr['^2'], 0.25 * UP, scale=0.8))
        self.wait()
        amp_expr.save_state()
        ...
        time_word = Text('Time', font_size=36)
        time_word.next_to(time_rect, UP)
        self.play(amp_expr.animate.scale(0.5).to_corner(UL).set_opacity(0.5), TransformFromCopy(amp_expr['O'][0], O_func['O'][0]))
        self.play(Write(O_func[1:], run_time=1, stroke_color=WHITE))
        O_func.set_backstroke(BLACK, 5)
        self.play(ShowCreation(xyz_rect), GrowArrow(xyz_arrow), FadeIn(space_words, lag_ratio=0.1))
        self.wait()
        self.play(FadeTransformPieces(space_words, time_word), ShowCreation(time_rect))
        self.wait()
        frequency = 0.25
        amplitude = 1.5
        ...
        plane = ComplexPlane((-2, 2), (-2, 2), width=4, background_line_style=dict(stroke_color=GREY_C, stroke_width=1), faded_line_style=dict(stroke_color=GREY_D, stroke_width=0.5, stroke_opacity=1))
        ...
        self.play(GrowArrow(out_arrow), FadeIn(real_line), xyz_rect.animate.set_stroke(width=1), time_rect.animate.set_stroke(width=1), FadeOut(time_word), FadeIn(real_indicator))
        self.wait(12)
        ...
        self.play(FadeIn(plane), FadeOut(real_line), FadeIn(complex_arrow), FadeIn(v_line))
        ...
        self.play(GrowFromCenter(brace), Write(amp_label))
        self.wait()
        self.play(ShowCreation(arc), TransformFromCopy(rot_arrow, complex_arrow, path_arc=angle), Write(phase_label))
        self.wait()
        self.play(FadeOut(VGroup(brace, amp_label, arc, phase_label)))
        time_tracker.resume_updating()
        plane.save_state()
        self.play(plane.animate.fade(0.75), FadeIn(real_line), complex_arrow.animate.set_fill(opacity=0.25))
        self.wait(8)
        self.play(Restore(plane), FadeOut(real_line), complex_arrow.animate.set_fill(opacity=1.0))
        self.wait(8)
        self.play(FadeOut(xyz_rect), FadeOut(time_rect), FadeOut(xyz_arrow), FadeOut(out_arrow), FadeOut(real_indicator), FadeOut(v_line))
        time_tracker.suspend_updating()
        self.remove(complex_label)
        plane.add(complex_label)
        self.add(plane, complex_arrow)
        self.play(Transform(O_func, amp_expr.saved_state[-3], remover=True), Restore(amp_expr), plane.animate.move_to(DOWN), run_time=2)
        ...
        self.play(GrowArrow(R_arrow), O_arrow.animate.shift(R_arrow.get_vector()), FadeIn(R_label), FadeIn(O_label))
        self.play(FadeIn(comb_arrow), FadeIn(comb_label))
        self.wait()
        R_arrow.add_updater(lambda m: m.put_start_and_end_on(plane.get_origin(), plane.n2p(get_R())))
        O_arrow.add_updater(lambda m: m.put_start_and_end_on(plane.n2p(get_R()), plane.n2p(get_R() + get_O())))
        comb_arrow.add_updater(lambda m: m.put_start_and_end_on(plane.get_origin(), plane.n2p(get_R() + get_O())))
        lhs = Text('Film opacity = ')
        lhs.move_to(amp_expr, LEFT)
        lhs.set_color(GREY_B)
        new_amp_expr = Tex('c|R + O|^2')
        new_amp_expr.next_to(lhs, RIGHT)
        self.play(ReplacementTransform(amp_expr['(R + O)^2'][0], new_amp_expr), FadeOut(amp_expr['Amplitude'][0]))
        self.play(FadeIn(lhs, lag_ratio=0.1))
        self.wait()
        self.play(R_phase_tracker.animate.set_value(-45 * DEGREES), O_phase_tracker.animate.set_value(-45 * DEGREES), run_time=2)
        self.wait()
        self.play(O_phase_tracker.animate.set_value(-125 * DEGREES), R_phase_tracker.animate.set_value(45 * DEGREES), run_time=3)
        self.wait()
...
class StateOnA2DScreen(InteractiveScene):
    ...
    def construct(self):
        ...
        for wave in wave3d:
            wave.set_opacity(1)
            wave.set_max_amp(10)
        self.play(frame.animate.reorient(101, -1, 0, (-0.53, 0.13, 7.82), 13.4), FadeIn(wave3d, time_span=(3, 8)), run_time=8)
        self.wait(4)
        self.play(LaggedStart(*(wave.animate.match_points(wave2d).set_opacity(1).shift(0.01 * IN) for wave in wave3d), lag_ratio=0), frame.animate.reorient(25, -6, 0, (0.18, 0.29, 0.15), 9.15).set_anim_args(time_span=(1, 4)), run_time=4)
        self.wait(8)

class PointSourceDiffractionPattern(InteractiveScene):
    include_axes = True
    axes_config = dict(x_range=(-6, 6), y_range=(-6, 6), z_range=(-6, 6))
    light_frequency = 2.0
    wave_length = 1.0
    use_hue = False
    max_mag = 3.0
    ...
    def create_dot_sheet(self, width=4, height=4, radius=0.05, z=0, make_3d=False):
        ...
        dots.suspend_updating = lambda: None
        dots.set_z(z)
        if make_3d:
            dots.make_3d()
        return dots