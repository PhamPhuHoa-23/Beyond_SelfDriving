...
POINTS = np.array([DOWN, 2 * UP, DOWN + RIGHT, 2 * DOWN, 2 * DOWN + RIGHT, DOWN + 3 * LEFT, 2 * UP + 3 * LEFT, 4 * RIGHT, 3 * UP + 3 * RIGHT])
...
class AddParallelLines(DrawAllThreeSquares):
    args_list = [(1, False), (2, False), (3, False), (3, True)]
...
class HighlightEmergentTriangles(AddParallelLines):
    ...
    def construct(self, *args):
        ...
        triplets = [[(0, 2), (0, -1), (1, -1)], [(1, -1), (4, -1), (4, 0)], [(4, 0), (4, 3), (3, 3)], [(3, 3), (0, 3), (0, 2)]]
...
class DrawAllThreeSquaresWithMoreTriangles(DrawAllThreeSquares):
    args_list = [(1, True), (2, True), (3, True), (4, True), (5, True), (6, True), (7, True), (8, True), (9, True), (10, True), (10, False)]
    ...
    def construct(self, num, fill):
        DrawAllThreeSquares.construct(self)
        pairs = [((0, 2, 0), (1, -1, 0)), ((-3, -1, 0), (0, -2, 0)), ((4, -1, 0), (1, -2, 0)), ((0, -2, 0), (-3, -1, 0)), ((1, -2, 0), (4, -1, 0)), ((1, -1, 0), (4, 0, 0)), ((4, 0, 0), (3, 3, 0)), ((3, 3, 0), (0, 2, 0)), ((-3, 3, 0), (0, 2, 0)), ((0, 2, 0), (-3, 3, 0))]
        to_flip = [1, 3, 8, 9]
        for n in range(num):
            triangle = Triangle()
            if n in to_flip:
...
class FillInAreaOfBigRectangle(DrawAllThreeSquaresWithMoreTriangles):
    ...
    def construct(self, *args):
        ...
        self.set_color_region(region_from_polygon_vertices((-3, 3), (-3, -2), (4, -2), (4, 3)), color=color)
...
class DrawCSquareWithAllTraingles(Scene):
    args_list = [(False, False, False, False), (False, True, False, True), (True, True, False, False), (False, True, True, False)]
...
class HighlightCSquareInBigSquare(DrawCSquareWithAllTraingles):
    ...
    def construct(self, *args):
        ...
        self.set_color_region(region_from_polygon_vertices(*c_square().center().get_vertices()), color=YELLOW)
...
class DrawTriangleWithAngles(Scene):
    ...
    def construct(self):
        ...
        kwargs = {'color': WHITE}
        angle1_arc = Circle(radius=0.4, **kwargs).filter_out(lambda x_y_z: not (x_y_z[0] > 0 and x_y_z[1] < 0 and (x_y_z[1] < -3 * x_y_z[0]))).shift(vertices[1])
        angle2_arc = Circle(radius=0.2, **kwargs).filter_out(lambda x_y_z1: not (x_y_z1[0] < 0 and x_y_z1[1] > 0 and (x_y_z1[1] < -3 * x_y_z1[0]))).shift(vertices[2])
        alpha = OldTex('\\alpha')
...
class FillComponentsOfLargeSquare(LabelLargeSquare):
    ...
    def construct(self):
        LabelLargeSquare.construct(self)
        points = np.array([2 * UP + 2 * LEFT, UP + 2 * LEFT, 2 * DOWN + 2 * LEFT, 2 * DOWN + LEFT, 2 * DOWN + 2 * RIGHT, DOWN + 2 * RIGHT, 2 * UP + 2 * RIGHT, RIGHT + 2 * UP])
        for triplet in [[0, 1, 7], [2, 3, 1], [4, 5, 3], [6, 7, 5]]:
            triplet.append(triplet[0])
            self.set_color_region(region_from_line_boundary(*[[points[i], points[j]] for i, j in zip(triplet, triplet[1:])]), color=BLUE_D)
        vertices = points[[1, 3, 5, 7, 1]]
        self.set_color_region(region_from_line_boundary(*[[p1, p2] for p1, p2 in zip(vertices, vertices[1:])]), color=YELLOW)
...
class ShowRearrangementInBigSquareWithRegions(ShowRearrangementInBigSquare):
    ...
    def construct(self):
        ...
        self.set_color_region(region_from_polygon_vertices(2 * (LEFT + UP), 2 * LEFT + DOWN, RIGHT + DOWN, RIGHT + 2 * UP), color=B_COLOR)
        self.set_color_region(region_from_polygon_vertices(RIGHT + DOWN, RIGHT + 2 * DOWN, 2 * RIGHT + 2 * DOWN, 2 * RIGHT + DOWN), color=A_COLOR)