...
class PreferOtherProofDialogue(Scene):
    ...
    def construct(self):
        ...
        self.wait(2)
        self.play(Transform(Dot(student_bubble.tip).set_color('black'), Mobject(student_bubble, student_bubble.text)))
        self.wait(2)
...
class IllustrateDuality(GraphScene):
    ...
    def construct(self):
        ...
        self.play(*[Transform(*edge_pair, **kwargs) for edge_pair in zip(self.edges, self.dual_edges)] + [Transform(Mobject(*[self.vertices[index] for index in cycle]), dv, **kwargs) for cycle, dv in zip(self.graph.region_cycles, self.dual_vertices)])
        self.wait()
...
class IntroduceGraph(GraphScene):
    ...
    def construct(self):
        ...
        edges_to_remove = [self.edges[self.graph.edges.index(pair)] for pair in [(4, 5), (0, 5), (1, 5), (7, 1), (8, 3)]]
        ...
        self.add(not_okay)
        self.play(*[Transform(*pair, **kwargs) for pair in zip(self.edges + self.vertices, tweaked_self.edges + tweaked_self.vertices)])
...
class OldIntroduceGraphs(GraphScene):
    ...
    def construct(self):
        ...
        self.play(*[CounterclockwiseTransform(vertex, Dot(point)) for vertex, point in zip(self.vertices, self.points)] + [Transform(ann, line) for ann, line in zip(self.edge_annotations, self.edges)])
        self.wait()

class PlanarGraphDefinition(Scene):

    def construct(self):
        Not, quote, planar, end_quote = OldTexText(['Not \\\\', '``', 'Planar', "''"]).split()
        ...
        self.wait()
        self.play(FadeOut(quote), FadeOut(end_quote), ApplyMethod(planar.shift, shift_val), FadeIn(graphs[0]), run_time=1.5)
        self.wait()
...
class ThreePiecesOfTerminology(GraphScene):
    ...
    def construct(self):
        GraphScene.construct(self)
        terms = cycles, spanning_trees, dual_graphs = [OldTexText(phrase).shift(y * UP).to_edge() for phrase, y in [('Cycles', 3), ('Spanning Trees', 1), ('Dual Graphs', -1)]]
...
class WalkingRandolph(GraphScene):
    ...
    def construct(self):
        ...
        for next, last in zip(point_path[1:], point_path):
            self.play(WalkPiCreature(randy, next), ShowCreation(Line(last, next).set_color('yellow')), run_time=2.0)
        self.randy = randy
...
class PathExamples(GraphScene):
    ...
    def construct(self):
        ...
        for path, non_path in zip(paths, non_paths):
            path_lines = Mobject(*[Line(self.get_points()[path[i]], self.get_points()[path[i + 1]]).set_color('yellow') for i in range(len(path) - 1)])
            non_path_lines = Mobject(*[Line(self.get_points()[pp[0]], self.get_points()[pp[1]]).set_color('yellow') for pp in non_path])
...
class IntroduceRandolph(GraphScene):
    ...
    def construct(self):
        ...
        self.play(Transform(randy, deepcopy(randy).scale(RANDOLPH_SCALE_FACTOR).move_to(self.get_points()[0])))
        self.wait()
        name.shift((0, 1, 0))
        self.add(name)
        self.wait()
...
class DefineSpanningTree(GraphScene):
    ...
    def construct(self):
        ...
        def green_dot_at_index(index):
            return Dot(self.get_points()[index], radius=2 * Dot.DEFAULT_RADIUS, color='lightgreen')
        ...
        for pair in self.spanning_tree_index_pairs:
            self.play(ShowCreation(Line(self.get_points()[pair[0]], self.get_points()[pair[1]]).set_color('yellow'), run_time=run_time_per_branch))
...
class NamingTree(GraphScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(*[FadeIn(mob) for mob in self.edges + self.vertices] + [Transform(b1, b2, run_time=2) for b1, b2 in zip(branches, branches_copy)])
...
class FacebookGraphAsAbstractSet(Scene):
    ...
    def construct(self):
        names = ['Louis', 'Randolph', 'Mortimer', 'Billy Ray', 'Penelope']
        friend_pairs = [(0, 1), (0, 2), (1, 2), (3, 0), (4, 0), (1, 3), (1, 2)]
        names_string = '\\\\'.join(names + ['$\\vdots$'])
        friends_string = '\\\\'.join(['\\text{%s}&\\leftrightarrow\\text{%s}' % (names[i], names[j]) for i, j in friend_pairs] + ['\\vdots'])
        ...
        lines = Mobject(Line(UP * FRAME_Y_RADIUS, DOWN * FRAME_Y_RADIUS), Line(LEFT * FRAME_X_RADIUS + 3 * UP, RIGHT * FRAME_X_RADIUS + 3 * UP)).set_color('white')
...
class ExamplesOfGraphs(GraphScene):
    ...
    def construct(self):
        ...
        GraphScene.construct(self)
        self.generate_regions()
        objects, notions = Mobject(*TexText(['Objects \\quad\\quad ', 'Thing that connects objects'])).to_corner().shift(0.5 * RIGHT).split()
        horizontal_line = Line((-FRAME_X_RADIUS, FRAME_Y_RADIUS - 1, 0), (max(notions.get_points()[:, 0]), FRAME_Y_RADIUS - 1, 0))
        ...
        objects_and_notions = [('Facebook accounts', 'Friendship'), ('English Words', 'Differ by One Letter'), ('Mathematicians', 'Coauthorship'), ('Neurons', 'Synapses'), ('Regions our graph \\\\ cuts the plane into', 'Shared edges')]
        self.clear()
    ...
    def handle_english_words(self, words1, words2):
        ...
        lines = [Line(*pair) for pair in [(words[0].get_center() + RIGHT * words[0].get_width() / 2, words[1].get_center() + LEFT * words[1].get_width() / 2), (words[1].get_center() + UP * words[1].get_height() / 2, words[2].get_center() + DOWN * words[2].get_height() / 2), (words[1].get_center() + DOWN * words[1].get_height() / 2, words[3].get_center() + UP * words[3].get_height() / 2)]]
...
class DrawDualGraph(GraphScene):
    ...
    def construct(self):
        ...
        internal_edges = [e for e in self.dual_edges if abs(e.start[0]) < FRAME_X_RADIUS and abs(e.end[0]) < FRAME_X_RADIUS and (abs(e.start[1]) < FRAME_Y_RADIUS) and (abs(e.end[1]) < FRAME_Y_RADIUS)]
        ...
        self.wait()
        self.reset_background()
        self.play(ApplyFunction(lambda p: (FRAME_X_RADIUS + FRAME_Y_RADIUS) * p / get_norm(p), outer_region_mob))
        self.wait()
...
class ListOfCorrespondances(Scene):
    ...
    def construct(self):
        buff = 0.5
        correspondances = [['Regions cut out by', 'Vertices of'], ['Edges of', 'Edges of'], ['Cycles of', 'Connected components of'], ['Connected components of', 'Cycles of'], ['Spanning tree in', 'Complement of spanning tree in'], ['', 'Dual of']]
...
class CyclesCorrespondWithConnectedComponents(GraphScene):
    ...
    def construct(self):
        ...
        for last, next in zip(cycle, cycle[1:]):
            ...
            self.play(ShowCreation(line), WalkPiCreature(randy, self.get_points()[next]), run_time=1.0)
        ...
        self.wait(2)
        self.reset_background()
        lines = Mobject(*[Line(self.dual_points[last], self.dual_points[next]) for last, next in zip(dual_cycle, dual_cycle[1:])]).set_color('red')
        self.play(ShowCreation(lines))
        self.play(*[Transform(v, Dot(v.get_center(), radius=3 * Dot.DEFAULT_RADIUS).set_color('green')) for v in np.array(self.vertices)[enclosed_vertices]] + [ApplyMethod(self.edges[0].set_color, 'green')])
        self.wait()
...
class IntroduceMortimer(GraphScene):
    ...
    def construct(self):
        ...
        randy_path = (0, 1, 3)
        morty_path = (-2, -3, -4)
        morty_crossed_lines = [Line(self.get_points()[i], self.get_points()[j]).set_color('red') for i, j in [(7, 1), (1, 5)]]
        kwargs = {'run_time': 1.0}
        self.clear()
        self.add(randy)
        self.wait()
        ...
        self.play(*[FadeIn(mob) for mob in self.vertices + self.edges] + [Transform(randy, small_randy), Transform(morty, small_morty)], **kwargs)
        self.wait()
        ...
        for last, next in zip(randy_path, randy_path[1:]):
            line = Line(self.get_points()[last], self.get_points()[next])
            line.set_color('yellow')
            self.play(WalkPiCreature(randy, self.get_points()[next]), ShowCreation(line), **kwargs)
        self.wait()
        self.play(*[ApplyMethod(line.rotate, np.pi / 10, rate_func=wiggle) for line in morty_crossed_lines], **kwargs)
        self.wait()
...
class RandolphMortimerSpanningTreeGame(GraphScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(WalkPiCreature(morty, self.dual_points[attempted_dual_point_index], rate_func=lambda t: 0.3 * there_and_back(t), run_time=2.0))
        self.wait()
        ...
        self.play(ShowCreation(Mobject(*[Line(self.get_points()[last], self.get_points()[next]).set_color('green') for last, next in zip(cycle, list(cycle)[1:] + [cycle[0]])])))
        self.wait()
...
class MortimerCannotTraverseCycle(GraphScene):
    ...
    def construct(self):
        ...
        for last, next in zip(dual_cycle, dual_cycle[1:]):
            ...
            line.set_color('red')
            self.play(WalkPiCreature(morty, self.dual_points[next], **kwargs), ShowCreation(line, **kwargs))
        ...
        self.play(*[Transform(v, Dot(v.get_center(), radius=3 * Dot.DEFAULT_RADIUS, color='green')) for v in np.array(self.vertices)[trapped_points]])
        self.add(text)
        self.play(*[Transform(line, deepcopy(edge).set_color(line.get_color())) for line, edge in zip(all_lines, matching_edges)])
        self.wait()
...
class TwoPropertiesOfSpanningTree(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        for word, explanation, vect in [(spanning, spanning_explanation, 0.5 * UP), (tree, tree_explanation, 0.5 * DOWN)]:
            self.add(explanation)
            self.add(Arrow(explanation.get_center() + vect, tail=word.get_center() - vect))
...
class TreeCountFormula(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeOut(text), FadeIn(Mobject(*gs.edges + gs.vertices)), *[Transform(*pair) for pair in zip(branches, gs.spanning_tree.split())])

class FinalSum(Scene):

    def construct(self):
        lines = OldTex(["(\\text{Number of Randolph's Edges}) + 1 &= V \\\\ \n", "(\\text{Number of Mortimer's Edges}) + 1 &= F \\\\ \n", ' \\Downarrow \\\\', 'E', '+', '2', '&=', 'V', '+', 'F'], size='\\large').split()
