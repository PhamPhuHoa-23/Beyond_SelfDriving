...
FINGER_WORDS = ['Thumb', 'Index Finger', 'Middle Finger', 'Ring Finger', 'Pinky']
COUNT_TO_FRAME_NUM = {0: 0, 1: 27, 2: 76, 3: 110, 4: 163, 5: 189, 6: 226, 7: 264, 8: 318, 9: 356, 10: 384, 11: 423, 12: 457, 13: 513, 14: 528, 15: 590, 16: 620, 17: 671, 18: 691, 19: 740, 20: 781, 21: 810, 22: 855, 23: 881, 24: 940, 25: 970, 26: 1014, 27: 1055, 28: 1092, 29: 1143, 30: 1184, 31: 1219}
COUNT_TO_TIP_POS = {0: [5.0, 0.5, 0.0], 1: [3.1, 2.5, 0.0], 2: [1.5, 2.3, 0.0], 3: [0.7, 2.0, 0.0], 4: [0.0, 1.0, 0.0]}

def finger_tip_power_of_2(finger_no):
    return OldTex(str(2 ** finger_no)).shift(COUNT_TO_TIP_POS[finger_no])

class Hand(ImageMobject):
    STARTING_BOTTOM_RIGHT = [4.61111111, -3.98888889, 9.8045469e-16]
...
class ShowCounting(OverHand):
    ...
    def construct(self):
        ...
        self.frames = [disp.paint_mobject(self.get_counting_mob(32 * count // num_frames), frame) for frame, count in zip(self.frames, it.count())]
...
class CountTo1023(Scene):
    ...
    def construct(self):
        ...
        def get_num(count):
            return Mobject(*[OldTex(char).shift(0.35 * x * RIGHT) for char, x in zip(str(count), it.count())]).center().to_edge(UP)
        self.frames = [disp.paint_mobject(Mobject(rh_map[count % 32], lh_map[count // 32], get_num(count))) for count in range(2 ** 10)]
...
class ShowReadingRule(Scene):
    ...
    def construct(self):
        ...
        answer = OldTexText(['Think of each finger as representing a power of 2, ', 'then add up the numbers represented by the standing fingers.'], size='\\Huge').scale(0.75).to_corner(UP + LEFT).split()
        self.add(question)
    ...
    def read_sample(self, num):
        ...
        if num in [6, 17]:
            hand.shift(0.8 * LEFT)
        sum_mobs = OldTex(' + '.join([str(2 ** c) for c in counts]).split(' ') + ['=%d' % num]).to_corner(UP + RIGHT).split()
        self.add(hand, *count_mobs)
        self.wait()
        self.play(*[Transform(count_mobs[n / 2], sum_mobs[n]) if n % 2 == 0 and n / 2 < len(counts) else FadeIn(sum_mobs[n]) for n in range(len(sum_mobs))])
        self.wait(2.0)
...
class ShowIncrementRule(Scene):
    ...
    def construct(self):
        ...
        for phrase, (start, end, pause) in zip(phrases, ranges):
            ...
            self.frames += [disp.paint_mobject(phrase, frame) for frame in oh.frames[COUNT_TO_FRAME_NUM[start]:COUNT_TO_FRAME_NUM[end]]]
            if pause:
    ...
    def get_arrow_set(self, num):
        ...
        if num == 12:
            ...
            tips = [(6, 1.5, 0), (3, 1.5, 0)]
        elif num == 27:
            tips = [(5.5, 1.5, 0), (2.75, 3.5, 0), (2, 1.0, 0)]
        elif num == 23:
            tips = [(6, 1, 0), (3.25, 3.5, 0), (2.25, 3.5, 0), (1.5, 0.75, 0)]
...
class MindFindsShortcuts(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Transform(compound, Arrow(hand, seven).set_color('yellow')), ShimmerIn(OldTexText('Directly recognize').shift(1.5 * DOWN + 2 * RIGHT)))
        self.wait()
        self.clear()
        ...
        self.wait()
        self.play(Transform(deepcopy(hands[16]).set_color('black').center().shift(hands[23].get_center()), hands[16]), Transform(deepcopy(hands[7]).set_color('black').center().shift(hands[23].get_center()), hands[7]), Animation(hands[23]), FadeIn(equals), FadeIn(plus))
        self.wait()
...
class WithToes(Scene):
    ...
    def construct(self):
        words = OldTexText(['If you were dexterous enough to use your toes as well,', 'you could count to 1,048,575']).split()
        self.add(words[0])
        self.wait()
