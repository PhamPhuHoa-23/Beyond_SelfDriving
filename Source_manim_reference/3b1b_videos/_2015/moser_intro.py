...
def logo_to_circle():
    ...
    sc = Scene()
    small_circle = Circle(density=CIRCLE_DENSITY, color='skyblue').scale(LOGO_RADIUS).set_color(DARK_BROWN, lambda x_y_z: x_y_z[0] < 0 and x_y_z[1] > 0)
...
def count_sections(*radians):
    ...
    sc.add(circle)
    points = [(RADIUS * np.cos(angle), RADIUS * np.sin(angle), 0) for angle in radians]
...
def summarize_pattern(*radians):
    ...
    for x in range(len(points)):
        ...
        sc.animate(Transform(last_num, num) if last_num else ShowCreation(num), FadeIn(new_lines), FadeIn(dots[x]), run_time=0.5)
        sc.remove(last_num)
        last_num = num
...
def interesting_problems():
    ...
    fermat_copies, face_copies, tb_copies, sb_copies = (Mobject(*[deepcopy(mob).scale(0.5).shift(locale) for locale in locales]) for mob in [fermat, face, tb, sb])
    sc.add(face, tb)
    sc.animate(ShowCreation(fermat, run_time=1))
    sc.add(fermat)
    sc.wait()
    sc.animate(Transform(deepcopy(fermat).repeat(len(locales)), fermat_copies), FadeIn(face_copies, run_time=1.0))
...
def response_invitation():
    ...
    mini_videos = Mobject(*[deepcopy(video_icon).scale(0.5).shift((3, y, 0)) for y in [-2, 0, 2]])
    comments = Mobject(*[Line((-1.2, y, 0), (1.2, y, 0), color='white') for y in [-1.5, -1.75, -2]])
    sc.add(video_icon)
    sc.wait()
...
def different_points(radians1, radians2):
    ...
    circle = Circle(density=CIRCLE_DENSITY).scale(RADIUS)
    sc.add(circle)
    points1, points2 = ([(RADIUS * np.cos(angle), RADIUS * np.sin(angle), 0) for angle in radians] for radians in (radians1, radians2))
    dots1, dots2 = (Mobject(*[Dot(point) for point in points]) for points in (points1, points2))
    lines1, lines2 = ([Line(point1, point2) for point1, point2 in it.combinations(points, 2)] for points in (points1, points2))
    sc.add(dots1, *lines1)
    sc.animate(Transform(dots1, dots2, run_time=3), *[Transform(line1, line2, run_time=3) for line1, line2 in zip(lines1, lines2)])
    sc.wait()
    return sc
...
def next_few_videos(*radians):
    ...
    lines = Mobject(*[Line(point1, point2) for point1, point2 in it.combinations(points, 2)])
    ...
    sc.add(thumbnail)
    sc.wait()
    sc.animate(Transform(big_frame, frame), Transform(thumbnail, deepcopy(thumbnail).scale(0.15).shift((-5, 0, 0))))
    sc.add(frame, thumbnail)
    sc.wait()
    last = frame
    for x in [-2, 1, 4]:
        vi = VideoIcon().shift((x, 0, 0))
        sc.animate(Transform(deepcopy(last), vi), Animation(thumbnail))
        sc.add(vi)
        last = vi
    return sc
