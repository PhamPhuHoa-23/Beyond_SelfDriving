...
INTERVAL_COLOR_PALETTE = ['yellow', 'green', 'skyblue', '#AD1457', '#6A1B9A', '#26C6DA', '#FF8F00']
...
class Vibrate(Animation):
    CONFIG = {'num_periods': 1, 'overtones': 4, 'amplitude': 0.5, 'radius': INTERVAL_RADIUS, 'center': ORIGIN, 'color': 'white', 'run_time': 3.0, 'rate_func': None}

    def __init__(self, **kwargs):
        digest_config(self, kwargs)

        def func(x, t):
            return sum([self.amplitude / (k + 1) ** 2.5 * np.sin(2 * mult * t) * np.sin(k * mult * x) for k in range(self.overtones) for mult in [(self.num_periods + k) * np.pi]])
        self.func = func
    ...
    def interpolate_mobject(self, alpha):
        ...
        self.mobject.add_points([[x * self.radius, self.func(x, alpha * self.run_time) + y, 0] for x in np.arange(-1, 1, epsilon / self.radius) for y in epsilon * np.arange(3)])
...
class IntervalScene(NumberLineScene):
    ...
    def cover_fractions(self, epsilon=0.3, num_fractions=10, run_time_per_interval=0.5):
        ...
        while True:
            ...
            if fraction < self.number_line.left_num or fraction > self.number_line.right_num:
                continue
            num_intervals += 1
            interval, line = self.add_open_interval(fraction, epsilon / min(2 ** count, 2 ** 30), run_time=run_time_per_interval)
    ...
    def add_open_interval(self, num, width, color=None, run_time=0):
        ...
        interval_line = Line(center_point + spatial_width * LEFT / 2, center_point + spatial_width * RIGHT / 2)
        ...
        if run_time > 0:
            ...
            self.play(Transform(squished_interval, open_interval), ShowCreation(interval_line), run_time=run_time)
...
class ChallengeOne(Scene):
    ...
    def construct(self):
        ...
        top_vibrations = [Vibrate(num_periods=freq, run_time=3.0, center=2 * UP, color=next(colors)) for freq in [1, 2, 5.0 / 3, 4.0 / 3, 2]]
    ...
    def continuously_vary_frequency(self, top_vib_1, top_vib_2):
        ...
        self.play(self.bottom_vibration, Transform(arrow1, arrow2, **kwargs), Transform(r1, r2, **kwargs), TransformAnimations(top_vib_1, top_vib_2, **kwargs))
        for vib in vibrations:
...
class JustByAnalyzingTheNumber(Scene):
    ...
    def construct(self):
        nums = [1.5, np.sqrt(2), 2 ** (5 / 12.0), 4 / 3.0, 1.2020569031595942]
        last = None
...
class QuestionAndAnswer(Scene):
    ...
    def construct(self):
        ...
        question = OldTexText('For what values of $r$ will the frequencies 220~Hz and             $r\\times$220~Hz sound nice together?').next_to(Q)
        answer = OldTexText(['When $r$ is', 'sufficiently close to', 'a rational number'], size='\\small').scale(1.5)
        answer.next_to(A)
        ...
        self.play(Transform(Point(answer[1].get_left()), answer[1]), Transform(temp_answer_end, answer[2]), string1, string2)
        self.play(string1, string2, run_time=3.0)

class PlaySimpleRatio(Scene):
    args_list = [(Fraction(3, 2), 'green'), (Fraction(4, 3), 'purple'), (Fraction(8, 5), 'skyblue'), (Fraction(211, 198), 'orange'), (Fraction(1093, 826), 'red'), ((np.exp(np.pi) - np.pi) / 15, '#7e008e')]
...
class PatternInFrequencies(Scene):
    args_list = [(3, 2, 'green'), (4, 3, 'purple'), (8, 5, 'skyblue'), (35, 43, 'red')]
    ...
    def construct(self, num1, num2, color):
        ...
        top_lines = Mobject(*[deepcopy(line_template).shift(k * (float(num1) / num2) * RIGHT) for k in range(num_top_lines)])
...
class PianoTuning(Scene):
    ...
    def show_interval(self, interval, half_steps):
        ...
        product = OldTex(['\\left(2^{\\left(\\frac{1}{12}\\right)}\\right)'] * half_steps, size='\\small').next_to(self.piano, UP, buff=1.0)
        ...
        self.play(ApplyMethod(high.set_color, colors[interval]), Transform(Point(u_brace.get_left()), u_brace))
        self.wait()
...
class PowersOfTwelfthRoot(Scene):
    ...
    def construct(self):
        ...
        num_terms = 11
        mob_list = []
        fraction_map = {1: Fraction(16, 15), 2: Fraction(9, 8), 3: Fraction(6, 5), 4: Fraction(5, 4), 5: Fraction(4, 3), 7: Fraction(3, 2), 8: Fraction(8, 5), 9: Fraction(5, 3), 10: Fraction(16, 9)}
...
class SupposeThereIsASavant(Scene):
    ...
    def construct(self):
        words = 'Suppose there is a musical savant ' + 'who finds pleasure in all pairs of ' + 'notes whose frequencies have a rational ratio'
        words = words.split(' ')
...
class CoveringSetsWithOpenIntervals(IntervalScene):
    ...
    def construct(self):
        IntervalScene.construct(self)
        dots = Mobject(*[Dot().shift(self.number_line.number_to_point(num) + UP) for num in set([0.2, 0.25, 0.45, 0.6, 0.65])])
        theorems = [OldTexText(words).shift(UP) for words in ['Heine-Borel Theorem', "Lebesgue's Number Lemma", 'Vitali Covering Lemma', 'Besicovitch Covering Theorem', '$\\vdots$']]
        self.add(dots)
...
class ShowAllFractions(IntervalScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(*[CounterclockwiseTransform(mob, Point(mob.get_center())) for mob in self.mobjects if isinstance(mob, ImageMobject)])
        self.wait()
...
class StepsToSolution(IntervalScene):
    ...
    def construct(self):
        IntervalScene.construct(self)
        self.spacing = 0.7
        steps = list(map(TexText, ['Enumerate all rationals in (0, 1)', 'Assign one interval to each rational', 'Choose sum of the form $\\mathlarger{\\sum}_{n=1}^\\infty a_n = 1$', 'Pick any $\\epsilon$ such that $0 < \\epsilon < 1$', 'Stretch the $n$th interval to have length $\\epsilon/2^n$']))
        for step in steps:
            step.shift(DOWN)
        for step in steps[2:]:
            step.shift(DOWN)
...
class OurSumCanBeArbitrarilySmall(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.clear()
        words = OldTexText(['Not only can the sum be $< 1$,\\\\', 'it can be \\emph{arbitrarily small} !']).split()
        self.add(words[0])
        self.wait()
...
class TroubleDrawingSmallInterval(IntervalScene):
    ...
    def construct(self):
        ...
        self.play(Transform(line, deepcopy(line).scale(10).shift(DOWN), run_time=2.0, rate_func=there_and_back))
        self.wait()
...
class ShiftSetupByOne(IntervalScene):
    ...
    def construct(self):
        IntervalScene.construct(self)
        new_interval = UnitInterval(number_at_center=1.5, leftmost_tick=1, big_tick_numbers=[1, 2])
        ...
        intervals, lines = self.cover_fractions(epsilon=0.01, num_fractions=150, run_time_per_interval=0)
        ...
        for interval in intervals[50:]:
            self.add(interval)
        self.wait()
        mobs_shifts = [(intervals, UP), ([self.number_line, new_interval], side_shift_val * LEFT), (intervals, DOWN)]
        ...
        words = OldTexText(['To a', 'savant,', 'harmonious numbers could be ', '\\emph{precisely}', 'those 1\\% covered by the intervals']).shift(2 * UP)
        words = words.split()
        words[1].set_color()
        words[3].set_color()
        self.add(*words)
...
class FinalEquivalence(IntervalScene):
    ...
    def construct(self):
        ...
        words.next_to(arrow)
        self.play(ShowCreation(ticks), Transform(deepcopy(intervals).stretch_to_fit_height(0), intervals))
