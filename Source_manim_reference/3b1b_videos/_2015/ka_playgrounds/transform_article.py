...
def half_plane():
    plane = NumberPlane(x_radius=FRAME_X_RADIUS / 2, x_unit_to_spatial_width=0.5, y_unit_to_spatial_height=0.5, x_faded_line_frequency=0, y_faded_line_frequency=0, density=4 * DEFAULT_POINT_DENSITY_1D)
    plane.add_coordinates(x_vals=list(range(-6, 7, 2)), y_vals=list(range(-6, 7, 2)))
    return plane

class SingleVariableFunction(Scene):
    args_list = [(lambda x: x ** 2 - 3, 'ShiftedSquare', True), (lambda x: x ** 2 - 3, 'ShiftedSquare', False)]
...
class LineToPlaneFunction(Scene):
    args_list = [(lambda x: (np.cos(x), 0.5 * x * np.sin(x)), 'Swirl', []), (lambda x: (np.cos(x), 0.5 * x * np.sin(x)), 'Swirl', [('0', '(1, 0)', 0), ('\\frac{\\pi}{2}', '(0, \\pi / 4)', np.pi / 2), ('\\pi', '(-1, 0)', np.pi)])]
    ...
    def construct(self, func, name, numbers_to_follow):
        line = NumberLine(unit_length_to_spatial_width=0.5, tick_frequency=1, number_at_center=6, numerical_radius=6, big_tick_numbers=[0, 12], density=3 * DEFAULT_POINT_DENSITY_1D)
        line.to_edge(LEFT)
        line_copy = line.copy()
...
class PlaneToPlaneFunctionSeparatePlanes(Scene):
    args_list = [(lambda x_y3: (x_y3[0] ** 2 + x_y3[1] ** 2, x_y3[0] ** 2 - x_y3[1] ** 2), 'Quadratic')]
...
class PlaneToPlaneFunction(Scene):
    args_list = [(lambda x_y4: (x_y4[0] ** 2 + x_y4[1] ** 2, x_y4[0] ** 2 - x_y4[1] ** 2), 'Quadratic')]
...
class PlaneToLineFunction(Scene):
    args_list = [(lambda x_y: x_y[0] ** 2 + x_y[1] ** 2, 'Bowl')]
    ...
    def construct(self, func, name):
        line = NumberLine(color=GREEN, unit_length_to_spatial_width=0.5, tick_frequency=1, number_at_center=6, numerical_radius=6, big_tick_numbers=[0, 12]).to_edge(RIGHT)
        line.add_numbers()
...
class PlaneToSpaceFunction(Scene):
    args_list = [(lambda x_y1: (x_y1[0] * x_y1[0], x_y1[0] * x_y1[1], x_y1[1] * x_y1[1]), 'Quadratic')]
    ...
    def construct(self, func, name):
        ...
        rot_kwargs = {'run_time': 3, 'radians': 0.3 * np.pi, 'axis': [0.1, 1, 0.1]}
        ...
        self.play(TransformAnimations(Animation(plane.copy()), Rotating(target, **rot_kwargs), rate_func=smooth), Rotating(axes, **rot_kwargs))
        axes.add(target)
        self.clear()
        self.add(plane, divider, axes)
        self.play(Rotating(axes, **rot_kwargs))
...
class SpaceToSpaceFunction(Scene):
    args_list = [(lambda x_y_z: (x_y_z[1] * x_y_z[2], x_y_z[0] * x_y_z[2], x_y_z[0] * x_y_z[1]), 'Quadratic')]
    ...
    def construct(self, func, name):
        space = SpaceGrid()
        rot_kwargs = {'run_time': 10, 'radians': 2 * np.pi / 5, 'axis': [0.1, 1, 0.1], 'in_place': False}
        axes = XYZAxes()
        target = space.copy().apply_function(func)
        self.play(TransformAnimations(Rotating(space, **rot_kwargs), Rotating(target, **rot_kwargs), rate_func=squish_rate_func(smooth, 0.3, 0.7)), Rotating(axes, **rot_kwargs))
        axes.add(space)
        self.play(Rotating(axes, **rot_kwargs))