...
class ExampleMultivariableFunctionWithZoom(ZoomedScene, ExampleMultivariableFunction):
    ...
    def construct(self):
        ...
        self.wait()
        self.apply_nonlinear_transformation(example_function, added_anims=[ApplyMethod(self.little_rectangle.move_to, example_function(point), run_time=3)])
        self.wait()

    def get_dense_lines(self, point):
        radius = 0.4 * self.little_rectangle.get_height()
        n_steps = 5
        vert_lines = VGroup(*[Line(DOWN, UP).scale(FRAME_Y_RADIUS).shift(x * RIGHT) for x in np.linspace(point[0] - radius, point[0] + radius, n_steps)])
        horiz_lines = VGroup(*[Line(LEFT, RIGHT).scale(FRAME_X_RADIUS).shift(y * UP) for y in np.linspace(point[1] - radius, point[1] + radius, n_steps)])
...
class JacobianDeterminantAnimation(ExampleMultivariableFunctionWithMuchZoom):
    ...
    def construct(self):
        ...
        dense_lines = self.get_dense_lines(point)
        self.add_unit_square()
        tiny_unit = get_norm(dense_lines[0][1].get_center() - dense_lines[0][0].get_center())
