...
def matrix_mobject(matrix):
    return OldTexText('\n        \\left(\n            \\begin{array}{%s}\n                %d & %d \\\\\n                %d & %d\n            \\end{array}\n        \\right)\n        ' % tuple(['c' * matrix.shape[1]] + list(matrix.flatten())), size='\\Huge')

class ShowMultiplication(NumberLineScene):
    args_list = [(2, True), (0.5, True), (-3, True), (-3, True), (2, True), (6, True)]
    ...
    def construct(self, num, show_original_line):
        config = {'density': max(abs(num), 1) * DEFAULT_POINT_DENSITY_1D, 'stroke_width': 2 * DEFAULT_STROKE_WIDTH}
        if abs(num) < 1:
    ...
    def copy_original_line(self):
        ...
        self.play(ApplyFunction(lambda m: m.shift(DOWN).set_color('lightgreen'), copied_line), *[ApplyMethod(mob.shift, DOWN) for mob in copied_num_mobs])
        self.wait()
...
class ExamplesOfNonlinearOneDimensionalTransforms(NumberLineScene):
    ...
    def construct(self):
        ...
        pairs = [(sinx_plux_x, "numbers don't remain evenly spaced"), (shift_zero, 'zero does not remain fixed')]
    ...
    def run_function(self, function, explanation):
        ...
        config = {'stroke_width': 2 * DEFAULT_STROKE_WIDTH, 'density': 5 * DEFAULT_POINT_DENSITY_1D}
        NumberLineScene.construct(self, **config)
        words = OldTexText(explanation).set_color(LIGHT_RED)
        words.next_to(self.nonlinear, DOWN, buff=0.5)
        self.add(words)
        self.play(ApplyPointwiseFunction(function, self.number_line), *[ApplyMethod(mob.shift, function(mob.get_center()) - mob.get_center()) for mob in self.number_mobs], run_time=2.0)
...
class ShowTwoThenThree(ShowMultiplication):
    ...
    def construct(self):
        config = {'stroke_width': 2 * DEFAULT_STROKE_WIDTH, 'density': 6 * DEFAULT_POINT_DENSITY_1D}
        NumberLineScene.construct(self, **config)
        self.copy_original_line()
...
class TransformScene2D(Scene):
    ...
    def add_number_plane(self, density_factor=1, use_faded_lines=True):
        config = {'x_radius': FRAME_WIDTH, 'y_radius': FRAME_WIDTH, 'density': DEFAULT_POINT_DENSITY_1D * density_factor, 'stroke_width': 2 * DEFAULT_STROKE_WIDTH}
    ...
    def add_x_y_arrows(self):
        self.x_arrow = Arrow(ORIGIN, self.number_plane.num_pair_to_point((1, 0)), color='lightgreen', **ARROW_CONFIG)
        self.y_arrow = Arrow(ORIGIN, self.number_plane.num_pair_to_point((0, 1)), color=LIGHT_RED, **ARROW_CONFIG)
        self.add(self.x_arrow, self.y_arrow)
        self.number_plane.filter_out(lambda x_y_z: 0 < x_y_z[0] and x_y_z[0] < 1 and (abs(x_y_z[1]) < 0.1))
        self.number_plane.filter_out(lambda x_y_z1: 0 < x_y_z1[1] and x_y_z1[1] < 1 and (abs(x_y_z1[0]) < 0.1))
        return self

class ShowMatrixTransform(TransformScene2D):
    args_list = [([[1, 3], [-2, 0]], False, False), ([[1, 3], [-2, 0]], True, False), ([[1, 0.5], [0.5, 1]], True, False), ([[2, 0], [0, 2]], True, False), ([[0.5, 0], [0, 0.5]], True, False), ([[-1, 0], [0, -1]], True, False), ([[0, 1], [1, 0]], True, False), ([[-2, 0], [-1, -1]], True, False), ([[0, -1], [1, 0]], True, False)]
    ...
    def construct(self, matrix, with_background, show_matrix):
        ...
        number_plane_config = {'density_factor': self.get_density_factor(matrix)}
        if with_background:
            self.add_background()
        ...
        if hasattr(self, 'x_arrow') and hasattr(self, 'y_arrow'):
            for arrow, index in ((self.x_arrow, 0), (self.y_arrow, 1)):
                new_arrow = Arrow(ORIGIN, self.number_plane.num_pair_to_point(matrix[:, index]), color=arrow.get_color(), **ARROW_CONFIG)
                arrow.remove_tip()
    ...
    def get_path_func(self, matrix):
        rotational_components = np.array([np.log(multiplier * complex(*matrix[:, i])).imag for i, multiplier in [(0, 1), (1, complex(0, -1))]])
...
class ExamplesOfTwoDimensionalLinearTransformations(ShowMatrixTransform):
    ...
    def construct(self):
        matrices = [[[1, 0.5], [0.5, 1]], [[0, -1], [2, 0]], [[1, 3], [-2, 0]]]
...
class ExamplesOfNonlinearTwoDimensionalTransformations(Scene):
    ...
    def construct(self):
        ...
        pairs = [(squiggle, 'lines do not remain straight'), (shift_zero, 'the origin does not remain fixed')]
        self.get_blackness()
    ...
    def apply_function(self, function, explanation):
        self.clear()
        config = {'x_radius': FRAME_WIDTH, 'y_radius': FRAME_WIDTH, 'density': 3 * DEFAULT_POINT_DENSITY_1D, 'stroke_width': 2 * DEFAULT_STROKE_WIDTH}
        ...
        self.wait()
        self.play(ApplyPointwiseFunction(function, number_plane), *[ApplyMethod(mob.shift, function(mob.get_center()) - mob.get_center()) for mob in numbers] + [Animation(self.blackness), Animation(words), Animation(self.nonlinear)], run_time=2.0)
        self.wait(3)

    def get_blackness(self):
        vertices = [3.5 * LEFT + 1.05 * UP, 3.5 * RIGHT + 1.05 * UP, 3.5 * RIGHT + 2.75 * UP, 3.5 * LEFT + 2.75 * UP]
...
class TrickyExamplesOfNonlinearTwoDimensionalTransformations(Scene):
    ...
    def construct(self):
        config = {'x_radius': 0.6 * FRAME_WIDTH, 'y_radius': 0.6 * FRAME_WIDTH, 'density': 10 * DEFAULT_POINT_DENSITY_1D, 'stroke_width': 2 * DEFAULT_STROKE_WIDTH}
        number_plane = NumberPlane(**config)
        phrase1, phrase2 = OldTexText(['These might look like they keep lines straight...', 'but diagonal lines get curved']).to_edge(UP, buff=1.5).split()
        phrase2.set_color(LIGHT_RED)
        diagonal = Line(DOWN * FRAME_Y_RADIUS + LEFT * FRAME_X_RADIUS, UP * FRAME_Y_RADIUS + RIGHT * FRAME_X_RADIUS, density=10 * DEFAULT_POINT_DENSITY_1D)
    ...
    def run_function(self, function, plane, phrase, remove_plane=True):
        ...
        self.wait()
        self.play(ApplyPointwiseFunction(function, number_plane, run_time=2.0), Animation(self.blackness), Animation(phrase))
        self.wait(3)
        if remove_plane:
            self.remove(number_plane)

    def get_blackness(self):
        vertices = [4.5 * LEFT + 1.25 * UP, 4.5 * RIGHT + 1.25 * UP, 4.5 * RIGHT + 2.75 * UP, 4.5 * LEFT + 2.75 * UP]
        region = region_from_polygon_vertices(*vertices)
        image = disp.paint_region(region, color=WHITE)
        self.blackness = OldTexText('')
        ImageMobject.generate_points_from_image_array(self.blackness, image)
...
class ShowMatrixTransformWithDot(TransformScene2D):
    ...
    def construct(self, matrix, with_background, show_matrix):
        ...
        kwargs = {'run_time': 2.0, 'path_func': self.get_path_func(matrix)}
        anims = [ApplyFunction(func, self.number_plane, **kwargs), ApplyMethod(dot.shift, func(deepcopy(dot)).get_center() - dot.get_center(), **kwargs)]
        if hasattr(self, 'x_arrow') and hasattr(self, 'y_arrow'):
            for arrow, index in ((self.x_arrow, 0), (self.y_arrow, 1)):
                new_arrow = Arrow(ORIGIN, self.number_plane.num_pair_to_point(matrix[:, index]), color=arrow.get_color(), **ARROW_CONFIG)
                arrow.remove_tip()
    ...
    def get_path_func(self, matrix):
        rotational_components = [sign * np.arccos(matrix[i, i] / get_norm(matrix[:, i])) for i in [0, 1] for sign in [(-1) ** i * np.sign(matrix[1 - i, i])]]
...
class Show90DegreeRotation(TransformScene2D):
    ...
    def construct(self):
        ...
        self.add_x_y_arrows()
        self.wait()
        self.play(*[RotationAsTransform(mob, run_time=2.0) for mob in (self.number_plane, self.x_arrow, self.y_arrow)])
        self.wait()