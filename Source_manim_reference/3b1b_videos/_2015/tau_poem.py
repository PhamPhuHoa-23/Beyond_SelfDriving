...
FORMULAE = ['e^{x + \\tau i} = e^{x}', '&\\Leftrightarrow', 'e^{x + 2\\pi i} = e^{x} \\\\', 'A = \\frac{1}{2} \\tau r^2', '&\\Leftrightarrow', 'A = \\pi r^2 \\\\', 'n! \\sim \\sqrt{\\tau n}\\left(\\frac{n}{e}\\right)^n', '&\\Leftrightarrow', 'n! \\sim \\sqrt{2\\pi n}\\left(\\frac{n}{e}\\right)^n \\\\']
...
class TauPoem(Scene):
    ...
    def __init__(self, line_num, *args, **kwargs):
        ...
        self.anim_kwargs = {'run_time': 4.0}
        self.line_num_to_method = {0: self.line0, 1: self.line1, 2: self.line2, 3: self.line3, 4: self.line4, 5: self.line5, 6: self.line6, 7: self.line7, 8: self.line8, 9: self.line9, 10: self.line10, 11: self.line11, 12: self.line12, 13: self.line13, 14: self.line14, 15: self.line15, 16: self.line16, 17: self.line17, 18: self.line18, 19: self.line19}
    ...
    def add_line_and_number(self):
        self.first_digits, new_digit, last_digits = OldTex([''.join(DIGITS[:self.line_num]), DIGITS[self.line_num], ''.join(DIGITS[self.line_num + 1:])]).to_edge(UP, buff=0.2).split()
        ...
        first_word, rest_of_line = OldTexText([line_str[:index], line_str[index:]]).to_edge(UP).shift(BUFF * DOWN).split()
        first_word.shift(0.15 * RIGHT)
        ...
        kwargs = {'rate_func': squish_rate_func(smooth)}
    ...
    def first_word_to_last_digit(self):
        if self.line_num == 19:
            ...
            self.wait(2)
        self.play_over_time_range(0, 2, Transform(deepcopy(self.first_word), self.number_word, rate_func=squish_rate_func(smooth)))
        self.play_over_time_range(2, 4, Transform(self.number_word, self.new_digit, rate_func=squish_rate_func(smooth)))

    def line0(self):
        two, pi = OldTex(['2', '\\pi']).scale(2).split()
        self.add(two, pi)
        two_copy = deepcopy(two).rotate(np.pi / 10).set_color('yellow')
        self.play(Transform(two, two_copy, rate_func=squish_rate_func(lambda t: wiggle(t), 0.4, 0.9), **self.anim_kwargs))
    ...
    def line3(self):
        ...
        self.add(*tau.parts)
        self.wait()
        self.play(Transform(tau.leg, pi.left_leg), ShowCreation(pi.right_leg), run_time=1.0)
    ...
    def line6(self):
        ...
        self.play(ApplyFunction(lambda p: 2 * p / get_norm(p), bubble, rate_func=wiggle, run_time=3.0))
    ...
    def line8(self):
        ...
        self.wait()
        self.play(Transform(pi.left_leg, tau.leg), Transform(pi.right_leg, Point(pi.right_leg.get_points()[0, :]).set_color('black')), Transform(pi.mouth, tau.mouth), CounterclockwiseTransform(two, Dot(two.get_center()).set_color('black')))
    ...
    def line12(self):
        ...
        tau_line = Line(*[np.pi * interval_size * vect for vect in (LEFT, RIGHT)], density=5 * DEFAULT_POINT_DENSITY_1D)
        tau_line.set_color('red')
        tau = OldTex('\\tau')
        tau.shift(tau_line.get_center() + 0.5 * UP)
        self.add(axes, grid)
        self.play(TransformAnimations(ShowCreation(sine), ShowCreation(deepcopy(sine).shift(2 * np.pi * interval_size * RIGHT)), run_time=2.0, rate_func=smooth), ShowCreation(circle))
        self.play(CounterclockwiseTransform(sine_period, tau_line), CounterclockwiseTransform(circle, deepcopy(tau_line)), FadeOut(axes), FadeOut(grid), FadeOut(sine), FadeIn(tau))
        self.wait()

    def line13(self):
        formula = form_start, two_pi, form_end = OldTex(['\\hat{f^{(n)}}(\\xi) = (', '2\\pi', 'i\\xi)^n \\hat{f}(\\xi)']).shift(DOWN).split()
    ...
    def line17(self):
        ...
        white_rgb = np.ones(3)
        circle.rgbas = np.array([alpha * blue_rgb + (1 - alpha) * white_rgb for alpha in np.arange(0, 1, 1.0 / len(circle.rgbas))])
    ...
    def line18(self):
        ...
        self.wait(0.8)
        self.play(*[Transform(*eyes, run_time=0.2, rate_func=rush_into) for eyes in [(tau.left_eye, blinked_tau.left_eye), (tau.right_eye, blinked_tau.right_eye)]])
        self.remove(tau.left_eye, tau.right_eye)
        self.play(*[Transform(*eyes, run_time=0.2, rate_func=rush_from) for eyes in [(blinked_tau.left_eye, mad_tau.left_eye), (blinked_tau.right_eye, mad_tau.right_eye)]])
        self.remove(blinked_tau.left_eye, blinked_tau.right_eye)
        self.add(mad_tau.left_eye, mad_tau.right_eye)
        self.play(Transform(tau.arm, mad_tau.arm), Transform(tau.mouth, mad_tau.mouth), run_time=0.5)
        self.remove(*tau.parts + blinked_tau.parts)
        self.add(*mad_tau.parts)
        self.play(*[Transform(*eyes, run_time=0.2, rate_func=rush_into) for eyes in [(pi.left_eye, blinked_pi.left_eye), (pi.right_eye, blinked_pi.right_eye)]])
        self.remove(pi.left_eye, pi.right_eye)
        self.play(*[Transform(*eyes, run_time=0.2, rate_func=rush_from) for eyes in [(blinked_pi.left_eye, sad_pi.left_eye), (blinked_pi.right_eye, sad_pi.right_eye)]] + [Transform(pi.mouth, sad_pi.mouth, run_time=0.2)])
