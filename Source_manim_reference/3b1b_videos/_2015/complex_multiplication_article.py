...
class SuccessiveComplexMultiplications(ComplexMultiplication):
    args_list = [(complex(1, 2), complex(1, -2)), (complex(-2, 1), complex(-2, -1))]
    ...
    def construct(self, *multipliers):
        ...
        plane_config = {'density': norm * DEFAULT_POINT_DENSITY_1D, 'unit_to_spatial_width': shrink_factor, 'x_radius': shrink_factor * FRAME_X_RADIUS, 'y_radius': shrink_factor * FRAME_Y_RADIUS}
...
class ShowComplexPower(SuccessiveComplexMultiplications):
    args_list = [(complex(0, 1), 1), (complex(0, 1), 2), (np.exp(complex(0, 2 * np.pi / 5)), 1), (np.exp(complex(0, 2 * np.pi / 5)), 5), (np.exp(complex(0, 4 * np.pi / 5)), 5), (np.exp(complex(0, -2 * np.pi / 5)), 5), (complex(1, np.sqrt(3)), 1), (complex(1, np.sqrt(3)), 3)]
...
class ComplexDivision(ComplexMultiplication):
    args_list = [complex(np.sqrt(3), 1), complex(1.0 / 3, -1.0 / 3), complex(1, 2)]
...
class DrawComplexAngleAndMagnitude(Scene):
    args_list = [(('1+i\\sqrt{3}', complex(1, np.sqrt(3))), ('\\frac{\\sqrt{3}}{2} - \\frac{1}{2}i', complex(np.sqrt(3) / 2, -1.0 / 2))), (('1+i', complex(1, 1)),)]
    ...
    def construct(self, *reps_and_nums):
        ...
        plane_config = {'color': 'grey', 'unit_to_spatial_width': FRAME_Y_RADIUS / radius}
    ...
    def add_lines(self, tex_representation, number):
        point = self.plane.number_to_point(number)
        x_line, y_line, num_line = [Line(start, end, color=color, stroke_width=self.plane.stroke_width) for start, end, color in zip([ORIGIN, point[0] * RIGHT, ORIGIN], [point[0] * RIGHT, point, point], [BLUE_D, GOLD_E, WHITE])]
        norm = get_norm(point)
