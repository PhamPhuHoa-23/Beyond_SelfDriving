...
class TelestrationContribution(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(randy), news.scale, 0.8, {'about_edge': UP}, news.shift, RIGHT)
        ...
        self.play(Blink(randy))
        self.wait()
        axes = Axes(x_min=0, x_max=15, y_min=0, y_max=10)
        axes.center()
        ...
        self.play(FadeOut(news), LaggedStartMap(FadeOutAndShift, VGroup(randy, bangs, arrow, muscle), lambda m: (m, DOWN)), Uncreate(lines), MoveToTarget(beer, run_time=2), ShowCreation(axes))
        lil_cat = get_cat()
        ...
        self.play(DrawBorderThenFill(article), ShowCreation(lines, run_time=2), ShowCreation(arrow), graph.set_height, 3, graph.next_to, arrow, LEFT)
        article.add(lines)
        self.wait()
        ...
        self.play(ShowCreation(new_arrow), TransformFromCopy(article, new_article, path_arc=30 * DEGREES))
        ...
        self.play(Restore(graph, run_time=2), MoveToTarget(new_article, run_time=2), FadeOut(arrow), FadeOut(new_arrow), FadeOut(article), FadeOut(likes))
        self.wait()
        ...
        self.play(FadeOut(lil_cat, LEFT), FadeIn(lil_person, RIGHT))
        self.play(FadeOut(fat_cat, LEFT), FadeIn(big_person, RIGHT))
        self.wait()
        ...
        self.play(FadeIn(muscle, RIGHT), FadeOut(big_person, LEFT), FadeOut(cross, LEFT))
        self.wait()
