...
def layer_mobject(mobject, nudge=1e-06):
...
class Chessboard(SGroup):
    CONFIG = {'shape': (8, 8), 'height': 7, 'depth': 0.25, 'colors': [GREY_B, GREY_E], 'gloss': 0.2, 'square_resolution': (3, 3), 'top_square_resolution': (5, 5)}
...
class Coin(Group):
    CONFIG = {'disk_resolution': (4, 51), 'height': 1, 'depth': 0.1, 'color': GOLD_D, 'tails_color': RED, 'include_labels': True, 'numeric_labels': False}
...
class CoinsOnBoard(Group):
    CONFIG = {'proportion_of_square_height': 0.7, 'coin_config': {}}
...
class Key(SVGMobject):
    CONFIG = {'file_name': 'key', 'fill_color': YELLOW_D, 'fill_opacity': 1, 'stroke_color': YELLOW_D, 'stroke_width': 0, 'gloss': 0.5, 'depth_test': True}
...
class FlipCoin(Animation):
    CONFIG = {'axis': RIGHT, 'run_time': 1, 'shift_dir': OUT}
...
class IntroducePuzzle(Scene):
    ...
    def construct(self):
        ...
        grid.next_to(chessboard, OUT, 1e-08)
        low_grid = grid.copy()
        low_grid.next_to(chessboard, IN, 1e-08)
        grid.add(low_grid)
        ...
        self.play(FadeIn(chessboard), ShowCreationThenDestruction(grid, lag_ratio=0.01), frame.set_theta, 0, frame.set_phi, 45 * DEGREES, run_time=3)
        self.wait()
        self.add(count_label)
        self.play(ShowIncreasingSubsets(coins), UpdateFromFunc(count_label[0], lambda m, c=coins: m.set_value(len(c))), rate_func=bezier([0, 0, 1, 1]), run_time=2)
        self.wait()
        ...
        k = boolian_linear_combo(head_bools) ^ 63
        ...
        self.wait()
        self.play(FadeOut(coins, lag_ratio=0.01), MoveToTarget(chessboard))
        ks_top = key_square.get_top()
        self.play(Rotate(key_square, PI / 2, axis=LEFT, about_point=ks_top), MoveToTarget(key), frame.set_phi, 60 * DEGREES, run_time=2)
        self.play(Rotate(key_square, PI / 2, axis=RIGHT, about_point=ks_top), run_time=2)
        chessboard.saved_state[k][0].match_color(key_square)
        self.play(chessboard.restore, FadeIn(coins), frame.set_phi, 0 * DEGREES, frame.move_to, 2 * LEFT, run_time=3)
        goal = OldTexText('Communicate where\\\\the key is', ' by turning\\\\over one coin.', alignment='')
...
class PrisonerPuzzleSetting(PiCreatureScene):
    ...
    def construct(self):
        ...
        names = VGroup(OldTexText('Prisoner 1\\\\(you)'), OldTexText('Prisoner 2'), OldTexText('Warden'))
        for name, pi in zip(names, pis):
            name.match_color(pi.body)
            name.next_to(pi, DOWN)
        question = OldTexText('Why do mathematicians\\\\always set their puzzles\\\\in prisons?', alignment='')
        question.to_corner(UR)
        self.remove(warden)
        warden.look_at(p2.eyes)
        self.play(LaggedStartMap(FadeIn, pis[:2], run_time=1.5, lag_ratio=0.3), LaggedStartMap(FadeIn, names[:2], run_time=1.5, lag_ratio=0.3))
        self.play(p1.change, 'sad', p2.change, 'pleading', warden.eyes)
        self.play(FadeIn(warden), FadeIn(names[2]))
...
class FromCoinToSquareMaps(ThreeDScene):
    CONFIG = {'messages': ['Please, ', 'go watch', 'Stand-up', 'Maths on', 'YouTube.'], 'flip_lag_ratio': 0.05}
        ...
        for message in messages[1:]:
            ...
            for coin, to_heads in zip(coins, new_bools):
                if coin.is_heads() ^ to_heads:
                    coins_to_flip.add(coin)
            coins_to_flip.shuffle()
            self.play(LaggedStartMap(FlipCoin, coins_to_flip, lag_ratio=self.flip_lag_ratio, run_time=1))
            new_square = get_special_square()
            self.play(square.set_color, square.original_color, new_square.set_color, MAROON_C, rect.move_to, new_square, OUT, rect.shift, 0.01 * OUT)
            square = new_square
            self.wait()

class FromCoinToSquareMapsSingleFlips(FromCoinToSquareMaps):
    CONFIG = {'messages': ['FlipBits', 'BlipBits', 'ClipBits', 'ChipBits', 'ChipBats', 'ChipRats']}
...
class DiagramOfProgression(ThreeDScene):
    ...
    def construct(self):
        ...
        coins = CoinsOnBoard(chessboard, coin_config={'disk_resolution': (2, 25)})
        ...
        for panel in panels[1:]:
            ...
            panel.coins = co
            panel.add(cb, co)
        kw = {'tex_to_color_map': {'Prisoner 1': P1_COLOR, 'Prisoner 2': P2_COLOR}}
        titles = VGroup(OldTexText('Prisoners conspire', **kw), OldTexText('Prisoner 1 sees key', **kw), OldTexText('Prisoner 1 flips coin', **kw), OldTexText('Prisoner 2 guesses key square', **kw))
        ...
        key_arrow = Arrow(key.get_right() + SMALL_BUFF * UP, key_square.get_corner(UL), path_arc=-45 * DEGREES, buff=SMALL_BUFF)
        ...
        self.play(ShowCreation(key_arrow), FadeInFromLarge(key_square))
        panels[1].add(key_arrow, key_square)
        self.wait()
        self.play(FadeIn(panels[2], UP))
        self.play(ShowCreation(flip_square), FlipCoin(flip_coin), p2_pi.look_at, flip_coin)
        self.wait()
        self.play(FadeIn(panels[3], LEFT))
        self.wait()
        self.play(FadeIn(panels[0], LEFT), panels[1].shift, p1_shift)
        self.wait()

class ImpossibleVariations(FromCoinToSquareMaps):
    CONFIG = {'messages': ['FlipBits', 'BlipBits', 'ClipBits', 'ChipBits', 'ChipBats', 'ChipRats', 'ChipVats', 'ChipFats', 'ChapFats']}
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeInFromLarge, group, lambda m: (m, 1.3), lag_ratio=0.01), GrowArrow(arrow))
        square = Square()
        ...
        self.play(title.shift, 2 * LEFT, FadeIn(cross, 2 * RIGHT), FadeIn(imp_words, LEFT))
        self.wait()
        ...
        self.play(FadeIn(remove_words, DOWN), FadeOut(to_remove, 3 * IN))
    ...
    def set_board_message(self, message, left_board, coins, get_special_square):
        ...
        self.play(new_colored_square.set_color, BLUE, self.colored_square.set_color, self.colored_square.original_color, self.moving_square.move_to, get_special_square(), OUT)
...
class ErrorCorrectionMention(Scene):
    ...
    def construct(self):
        ...
        message_arrow = Arrow(message_label.get_bottom(), meaning_label.get_top())
        message_arrow = Arrow(message_label.get_left(), meaning_label.get_top(), path_arc=70 * DEGREES)
        self.play(LaggedStartMap(MoveToTarget, coins), LaggedStartMap(FadeOut, board), Write(bits), run_time=3)
        self.play(words[1].set_x, 0, FadeOut(words[0], LEFT), FadeOut(words[2], LEFT))
        self.play(ShowCreation(bits_rect), FadeIn(data_label, DOWN))
        self.play(FadeOut(right_board), FadeIn(message_label, DOWN), ShowCreation(message_arrow), FadeIn(meaning_label))
        self.wait()
        error_index = 8 * 4 + 5
        error_bit = bits[error_index]
        error_bit_rect = SurroundingRectangle(error_bit)
        error_bit_rect.set_stroke(RED, 2)
        self.play(FadeInFromLarge(error_bit_rect), error_bit.set_value, 1 - error_bit.get_value(), error_bit.set_color, RED)
        ...
        frame = self.camera.frame
        self.play(MoveToTarget(ecc_name), ShowIncreasingSubsets(ecc), ShowCreation(ecc_rect), ApplyMethod(frame.move_to, DOWN, run_time=2))
        self.wait()
        lines = VGroup()
        for bit in bits:
        ...
        self.play(LaggedStartMap(ShowCreationThenFadeOut, lines, lag_ratio=0.02, run_time=3))
        ...
        self.play(FadeOut(lines), FadeOut(alert), FadeOut(error_bit_rect), error_bit.set_color, WHITE)
        self.wait()
...
class TitleCard(Scene):
    ...
    def construct(self):
        ...
        grid.next_to(board, OUT, 1e-08)
        low_grid = grid.copy()
        low_grid.next_to(board, IN, 1e-08)
        grid.add(low_grid)
        ...
        text.fix_in_frame()
        self.play(ApplyMethod(frame.set_phi, 0, run_time=5), ShowCreationThenDestruction(grid, lag_ratio=0.02, run_time=3), LaggedStartMap(FadeIn, board, run_time=3, lag_ratio=0), Write(text, lag_ratio=0.1, run_time=3, stroke_color=BLUE_A))
        self.wait(2)

class WhatAreWeDoingHere(TeacherStudentsScene):

    def construct(self):
        self.student_says('Wait, what are we\\\\doing here then?', target_mode='sassy', added_anims=[self.change_students('hesitant', 'angry', 'sassy')], run_time=2)
...
class HowCanWeVisualizeSolutions(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says('How can we\\\\visualize solutions', bubble_config={'height': 3, 'width': 4, 'fill_opacity': 0}, added_anims=[self.change_students('pondering', 'thinking', 'pondering')])
...
class TwoSquareCase(ThreeDScene):
    ...
    def construct(self):
        ...
        self.play(Write(two_square_words, run_time=1), LaggedStartMap(FadeOut, to_remove, lambda m: (m, DOWN), run_time=2, lag_ratio=0.01), small_group.center, small_group.set_height, 1.5, frame.set_phi, 10 * DEGREES, run_time=2)
        self.wait(3)
        coins = coin_pair
        key = Key()
        ...
        self.add(key)
        angle = 170 * DEGREES
        self.play(Rotate(s0_rot_group, angle, LEFT, about_point=s0_top), Rotate(s1_rot_group, angle, LEFT, about_point=s1_top), frame.set_phi, 45 * DEGREES)
        self.wait()
        self.play(key.match_x, small_board[1], path_arc=PI, path_arc_axis=UP)
        self.wait()
        self.play(Rotate(s0_rot_group, angle, RIGHT, about_point=s0_top), Rotate(s1_rot_group, angle, RIGHT, about_point=s1_top), frame.set_phi, 0, run_time=2)
        self.wait()
        states = VGroup(*[OldTexText(letters, tex_to_color_map={'H': GOLD, 'T': RED_D}) for letters in ['TT', 'HT', 'TH', 'HH']])
        ...
        self.play(FadeOut(coins), MoveToTarget(small_board), MaintainPositionRelativeTo(key, small_board), MoveToTarget(small_board_copy), MaintainPositionRelativeTo(key_copy, small_board_copy))
        ...
        for board in small_boards:
            ...
            board_rects.add(br)
        self.play(ApplyMethod(frame.shift, 4.5 * LEFT, run_time=1), FadeIn(coins), FadeIn(state_rect), FadeIn(board_rects[1]), arrows[n].set_color, YELLOW, arrows[n].set_opacity, 1)
        self.wait()
        self.play(FlipCoin(coins[1]), FadeOut(board_rects[1]), FadeIn(board_rects[0]), state_rect.move_to, states[1], arrows[3].match_style, arrows[0], arrows[1].match_style, arrows[3])
        self.wait()
        self.play(FlipCoin(coins[0]), state_rect.move_to, states[0], arrows[0].match_style, arrows[1], arrows[1].match_style, arrows[3])
        self.wait()
        self.play(FlipCoin(coins[0]), state_rect.move_to, states[1], arrows[0].match_style, arrows[1], arrows[1].match_style, arrows[0])
        self.wait()
        bin_states = VGroup(*[OldTexText(letters, tex_to_color_map={'1': GOLD, '0': RED_D}) for letters in ['00', '10', '01', '11']])
        ...
        self.play(FadeOut(coins, IN), FadeIn(bin_coins))
        self.play(LaggedStartMap(GrowFromCenter, Group(*bin_states.family_members_with_points())), LaggedStartMap(ApplyMethod, Group(*states.family_members_with_points()), lambda m: (m.scale, 0)))
        self.wait()
        c_labels = VGroup(*[OldTex(name) for name in self.coin_names])
        arrow_kw = {'tip_config': {'width': 0.2, 'length': 0.2}, 'buff': 0.1, 'color': GREY_B}
        ...
        self.wait()
        bit1_rect = SurroundingRectangle(VGroup(bin_states[0][1], bin_states[-1][1]), buff=SMALL_BUFF)
        coin1_rect = SurroundingRectangle(Group(c_labels[1], bin_coins[1]), buff=SMALL_BUFF)
        for rect in (bit1_rect, coin1_rect):
            ...
            rect.set_stroke(WHITE, 2)
        kw = {'stroke_width': 2, 'stroke_color': YELLOW, 'buff': 0.05}
        zero_rects, one_rects = [VGroup(SurroundingRectangle(bin_states[0][1], **kw), SurroundingRectangle(bin_states[1][1], **kw)), VGroup(SurroundingRectangle(bin_states[2][1], **kw), SurroundingRectangle(bin_states[3][1], **kw))]
        self.play(ShowCreation(bit1_rect), ShowCreation(coin1_rect), FadeOut(state_rect))
        self.wait()
        ...
        self.play(FlipCoin(bin_coins[1]), arrows[1].match_style, arrows[0], arrows[3].match_style, arrows[1])
        ...
        self.wait()
        for i in (0, 1, 0):
            self.play(FlipCoin(bin_coins[0]), arrows[3 - i].match_style, arrows[0], arrows[2 + i].match_style, arrows[3 - i], FadeOut(one_rects[1 - i]), FadeIn(one_rects[i]))
            self.wait()
        ...
        rule_equation_long = OldTex('K', '=', '0', '\\cdot', self.coin_names[0], '+', '1', '\\cdot', self.coin_names[1])
        ...
        s_labels = VGroup(OldTex('K', '= 0'), OldTex('K', '= 1'))
        ...
        self.wait()
        self.play(GrowArrow(rule_arrow), FadeIn(rule_equation, LEFT))
        self.wait()
        mid_equation = rule_equation_long[2:-1]
        mid_equation.save_state()
        mid_equation.scale(0, about_edge=LEFT)
        self.play(Transform(rule_equation[:2], rule_equation_long[:2]), Transform(rule_equation[2], rule_equation_long[-1]), Restore(mid_equation))
        self.wait()
...
class IGotThis(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('Pssh, I got this', target_mode='tease', look_at=self.screen, added_anims=[self.teacher.change, 'happy', self.screen], run_time=2)
...
class WalkingTheSquare(ThreeDScene):
    ...
    def construct(self):
        plane = NumberPlane(x_range=(-2, 2, 1), y_range=(-2, 2, 1), height=15, width=15, faded_line_ratio=3, axis_config={'include_tip': False})
        ...
        self.play(FadeIn(board), FadeIn(coins), FadeIn(active_dot), FadeIn(coord_labels[0]))
        edges = VGroup()
        for i, j, c in [(0, 1, 1), (1, 3, 0), (3, 2, 1), (2, 0, 0)]:
            edge = Line(plane.c2p(*coords[i]), plane.c2p(*coords[j]))
            edge.set_stroke(PINK, 3)
            edges.add(edge)
            anims = [FlipCoin(coins[c]), ShowCreation(edge), ApplyMethod(active_dot.move_to, dots[j])]
            if j != 0:
                anims += [FadeInFromPoint(coord_labels[j], coord_labels[i].get_center())]
            self.add(edge, dots[i], active_dot)
            self.play(*anims)
        self.add(edges, dots, active_dot)
        self.wait()
        self.play(FlipCoin(coins[0]), active_dot.move_to, dots[2])
        self.play(FlipCoin(coins[1]), active_dot.move_to, dots[3])
        self.play(FlipCoin(coins[1]), active_dot.move_to, dots[2])
        self.wait()
        low_rect = SurroundingRectangle(VGroup(edges[3], coord_labels[0], coord_labels[2], plane.x_axis.numbers[-1]), buff=0.25)
        ...
        s_labels = VGroup(OldTex('\\text{Key} = 0').next_to(low_rect, UP, SMALL_BUFF), OldTex('\\text{Key} = 1').next_to(high_rect, UP, SMALL_BUFF))
        self.play(ShowCreation(low_rect))
        self.play(FadeIn(key, DOWN), FadeIn(s_labels[0], DOWN))
        self.play(FlipCoin(coins[0]), active_dot.move_to, dots[0])
        self.wait(0.5)
        self.play(FlipCoin(coins[0]), active_dot.move_to, dots[2])
        self.wait()
        self.play(TransformFromCopy(low_rect, high_rect), FadeIn(s_labels[1], DOWN), low_rect.set_stroke, GREY, 1, FlipCoin(coins[1]), active_dot.move_to, dots[3], key.match_x, board[1])
        self.wait()
        self.play(FlipCoin(coins[0]), active_dot.move_to, dots[1])
        self.wait()
        self.play(FlipCoin(coins[1]), active_dot.move_to, dots[0], key.match_x, board[0], high_rect.match_style, low_rect, low_rect.match_style, high_rect)
        self.wait()
...
class ThreeSquareCase(ThreeDScene):
    ...
    def construct(self):
        boards = Group(Chessboard(shape=(1, 2), height=0.25 * 1), Chessboard(shape=(1, 3), height=0.25 * 1), Chessboard(shape=(2, 2), height=0.25 * 2), Chessboard(shape=(8, 8), height=0.25 * 8))
        dots = OldTex('\\dots')
        ...
        def get_board_transform(i, bgs=board_groups):
            return TransformFromCopy(bgs[i], bgs[i + 1], path_arc=PI / 2, run_time=2)
        frame = self.camera.frame
        frame.save_state()
        frame.scale(0.5)
        ...
        self.play(Write(dots), get_board_transform(2))
        self.wait()
        ...
        self.save_state()
        self.play(MoveToTarget(board_group, rate_func=squish_rate_func(smooth, 0.5, 1)), Write(title), LaggedStartMap(FadeOut, Group(board_groups[0], board_groups[2], dots, board_groups[3]), lambda m: (m, DOWN)), run_time=2)
        self.wait()
        s_sum = OldTex('0', '\\cdot', self.coin_names[0], '+', '1', '\\cdot', self.coin_names[1], '+', '2', '\\cdot', self.coin_names[2])
        ...
        self.play(FadeOut(title), FadeIn(s_sum[:3]), FadeIn(c_sum[:2]))
        self.play(TransformFromCopy(coins[0], coin_copies[0]))
        self.wait()
        self.play(FadeIn(s_sum[3:7]), FadeIn(c_sum[3:6]))
        self.play(TransformFromCopy(coins[1], coin_copies[1]))
        self.wait()
        self.play(FadeIn(s_sum[7:11]), FadeIn(c_sum[7:10]))
        ...
        braces = VGroup(Brace(c_sum[0:3], DOWN), Brace(c_sum[0:7], DOWN), Brace(c_sum[0:11], DOWN))
        ...
        self.play(TransformFromCopy(braces[2][-1], rhs[1], path_arc=-PI / 2), Write(rhs[0]))
        ...
        key.move_to(board[0])
        self.play(coins.next_to, board, DOWN, coins.match_z, coins, board.set_opacity, 0.75, FadeIn(key), FadeIn(s_labels[0]))
        self.wait(0.5)
        for i in (1, 2):
            self.play(ApplyMethod(key.move_to, board[i], path_arc=-45 * DEGREES), s_labels[i - 1].set_fill, GREY, 0.25, FadeIn(s_labels[i]))
            self.wait(0.5)
        ...
        self.wait()
        self.play(FlipCoin(coins[2]), FlipCoin(coin_copies[2]), rhs[1].set_value, 3)
        self.wait()
        ...
        for i in [2, 1, 0, 2, 1, 2, 0]:
            ...
            self.play(FlipCoin(coins[i]), FlipCoin(coin_copies[i]), rhs[1].set_value, new_sum)
            self.wait()
        general_sum = OldTex('\\sum ^{63}_{n=0}n\\cdot c_n')
        ...
        self.play(s_labels[2].set_fill, GREY, 0.25, s_labels[0].set_fill, YELLOW, 1, ApplyMethod(key.move_to, board[0], path_arc=30 * DEGREES))
        self.wait()
        self.play(FlipCoin(coins[1]), FlipCoin(coin_copies[1]), rhs[1].set_value, 0)
        self.wait()
        square = Square()
        ...
        self.play(FlipCoin(coins[1]), FlipCoin(coin_copies[1]), rhs[1].set_value, 1)
        morty = Mortimer(height=1.5, mode='hooray')
        morty.to_corner(DR)
        bubble = SpeechBubble(height=2, width=2)
        bubble.pin_to(morty)
        bubble.write("There's another\\\\way!")
        self.play(FadeIn(morty), ShowCreation(bubble), Write(bubble.content, run_time=1))
...
class FailedMod3Addition(Scene):
    ...
    def construct(self):
        ...
        csum = Group(OldTex('0 \\cdot'), coin.deepcopy().flip(), OldTex(' + 1 \\cdot'), coin.deepcopy().flip(), OldTex('+ 2 \\cdot'), coin.deepcopy(), OldTex('='), Integer(2, color=YELLOW))
...
class TreeOfThreeFlips(ThreeDScene):
    ...
    def construct(self):
        csum = Group(OldTex('0 \\cdot'), Coin(numeric_labels=True), OldTex('+\\,1 \\cdot'), Coin(numeric_labels=True), OldTex('+\\,2 \\cdot'), Coin(numeric_labels=True), OldTex('='), Integer(0))
        ...
        is_label = OldTexText('Initial state: 010', tex_to_color_map={'0': RED_D, '1': GOLD_D})
        ...
        for i, arrow, ncs in zip(it.count(), arrows, new_csums):
            anims += [GrowArrow(arrow), FadeIn(arrow.label, lag_ratio=0.2)]
        self.play(LaggedStart(*anims))
        for indices in [[0], [1, 2]]:
            self.play(*[TransformFromCopy(csum, new_csums[i], path_arc=30 * DEGREES, run_time=2) for i in indices])
            self.wait()
            for i in indices:
                ncs = new_csums[i]
                ncs.coins[i].flip()
        ...
        self.play(MoveToTarget(key, path_arc=30 * DEGREES), Rotate(opening_square, 90 * DEGREES, LEFT, about_point=opening_square_top))
        self.play(Rotate(opening_square, 90 * DEGREES, RIGHT, about_point=opening_square_top), key.next_to, board[1], RIGHT, buff=0.01)
        self.wait()
        self.remove(key)
    ...
    def construct(self):
        ...
        for n in it.chain(range(n_shown), range(64 - n_shown, 64)):
            ...
            if (random.random() < 0.5 or n == 2) and n != 62:
                coin.flip()
            num = Integer(n)
            row = Group(coin, OldTex('\\cdot'), num, OldTex('+'))
        ...
        self.wait()
        self.play(LaggedStartMap(FadeIn, rows, lag_ratio=0.1, run_time=3), Animation(nums.copy(), remover=True))
        self.wait()
        ...
        self.wait(2)
        self.play(LaggedStart(FadeIn(want_label, UP), FadeIn(need_label, UP), lag_ratio=0.3))
        self.wait()
        ...
        self.wait(2)
        self.play(s_rect.move_to, rows[-2], s_rect.stretch, 1.1, 0)
        self.wait()
        self.play(FlipCoin(rows[-2][0]))
        self.wait()
...
class ModNStrategy(ThreeDScene):
    ...
    def construct(self):
        ...
        for num, coin in zip(nums.target, coins.target):
            coin.set_height(0.7)
            num.set_height(0.5)
            summand = Group(coin, OldTex('\\cdot'), num, OldTex('+'))
        ...
        s_label = VGroup(OldTexText('Sum (mod 64) = '), Integer(53))
        ...
        self.play(FadeOut(board, IN), MoveToTarget(mid_coins, remover=True), MoveToTarget(mid_nums, remover=True), MoveToTarget(nums), MoveToTarget(coins), Write(dots), FadeIn(to_fade_in, lag_ratio=0.1), run_time=2)
        ...
        for x in range(10):
            ...
            self.play(ChangeDecimalToValue(s_label[1], s_label[1].get_value() + n, rate_func=squish_rate_func(smooth, 0.5, 1)), FlipCoin(coin), FadeIn(diff_label, 0.5 * DOWN))
...
class ShowCube(ThreeDScene):
    ...
    def construct(self):
        ...
        axes = ThreeDAxes(x_range=(-2, 2, 1), y_range=(-2, 2, 1), z_range=(-2, 2, 1), height=15, width=15, depth=15)
        ...
        edge_indices = [(0, 1), (0, 2), (1, 3), (2, 3), (0, 4), (1, 5), (2, 6), (3, 7), (4, 5), (4, 6), (5, 7), (6, 7)]
        spheres = SGroup()
        for vert in verts:
            sphere = Sphere(radius=0.1, resolution=(9, 9))
        ...
        for i, j in edge_indices:
            edge = Line3D(verts[i], verts[j], resolution=(5, 51), width=0.04, gloss=0.5)
        ...
        def highlight(n, spheres=spheres, coord_labels=coord_labels):
            ...
            for k, sphere, cl in zip(it.count(), spheres, coord_labels):
                if k == n:
                    ...
                    anims += [Restore(sphere), Restore(cl)]
            return AnimationGroup(*anims)
        frame.move_to(1.5 * UP)
        self.add(plane)
        self.play(LaggedStartMap(FadeIn, coord_labels_2d), LaggedStartMap(GrowFromCenter, spheres[:4]), LaggedStartMap(GrowFromCenter, edges[:4]))
        self.wait()
        frame.generate_target()
        ...
        self.play(MoveToTarget(frame), ShowCreation(axes.z_axis), Restore(to_grow, rate_func=rf), FadeOut(coord_labels_2d, rate_func=rf), *[FadeInFromPoint(cl, cl2.get_center(), rate_func=squish_rate_func(smooth, 0.5, 1)) for cl, cl2 in zip(coord_labels[:4], coord_labels_2d)], run_time=3)
        frame.start_time = self.time
        frame.scene = self
        frame.add_updater(lambda m: m.set_theta(-25 * DEGREES * math.cos((m.scene.time - m.start_time) * PI / 60)))
        self.add(axes.z_axis)
        self.add(edges)
        self.add(spheres)
        self.play(LaggedStart(*[Indicate(s, color=GREEN) for s in spheres], run_time=2, lag_ratio=0.1), LaggedStart(*[Indicate(c, color=GREEN) for c in coord_labels], run_time=2, lag_ratio=0.1))
        board = Chessboard(shape=(1, 3), height=1, square_resolution=(5, 5))
        ...
        for ci in [1, 2, 0, 1, 2, 1, 0, 1]:
            ...
            self.play(FlipCoin(coin), highlight(new_n), ShowCreationThenDestruction(line))
            self.wait()
        self.play(highlight(-1), edges.set_color, GREY, 0.5)
        ...
        title.shift(LEFT)
        title.fix_in_frame()
        color_label_templates = [OldTex(char, color=color).rotate(PI / 2, RIGHT).match_depth(coord_labels[0]) for char, color in zip('RGB', colors)]
        coord_labels.color_labels = VGroup(*[VMobject() for cl in coord_labels])

        def get_coloring_animation(ns, spheres=spheres, coord_labels=coord_labels, colors=colors, color_label_templates=color_label_templates):
            anims = []
            ...
            for n, sphere, coord_label, old_color_label in zip(ns, spheres, coord_labels, coord_labels.color_labels):
                ...
                anims += [MoveToTarget(sphere), MoveToTarget(coord_label), FadeIn(color_label, 0.25 * IN), FadeOut(old_color_label, 0.25 * OUT)]
                new_color_labels.add(color_label)
            coord_labels.color_labels = new_color_labels
            return LaggedStart(*anims, run_time=2)
        self.play(FadeIn(title, DOWN), get_coloring_animation(np.random.randint(0, 3, 8)))
        self.wait()
        for x in range(4):
        ...
        self.play(FadeIn(bit_sum, DOWN), FadeOut(S0, UP), get_coloring_animation([sum(coords[:2]) for coords in vert_coords]))
        self.wait(6)
        bit_sum_with_coefs = OldTex('\\text{Key} = \\,&(0\\cdot c_0 + 1\\cdot c_1 + 2\\cdot c_2) \\\\ &\\quad \\mod 3')
        bit_sum_with_coefs.scale(0.8)
        bit_sum_with_coefs.move_to(bit_sum, LEFT)
        bit_sum_with_coefs.fix_in_frame()
        self.play(FadeIn(bit_sum_with_coefs, DOWN), FadeOut(bit_sum, UP), get_coloring_animation([np.dot(coords, [0, 1, 2]) % 3 for coords in vert_coords]))
        self.wait(4)
        self.play(FlipCoin(coins), coord_labels[1:].set_opacity, 0.2, coord_labels.color_labels[1:].set_opacity, 0.2, spheres[1:].set_opacity, 0.2)
        self.wait(2)
        lines = VGroup()
        for n in [1, 2, 4]:
            ...
            self.play(ShowCreationThenDestruction(line), spheres[n].set_opacity, 1, coord_labels[n].set_opacity, 1, coord_labels.color_labels[n].set_opacity, 1, FlipCoin(coin))
            line.reverse_points()
            self.add(line, coord_labels)
            self.play(FlipCoin(coin), ShowCreation(line))
            lines.add(line)
        self.wait(10)
        self.play(FlipCoin(coins[1]), Uncreate(lines[1]), FadeOut(lines[::2]), Group(spheres[0], coord_labels[0], coord_labels.color_labels[0], spheres[1], coord_labels[1], coord_labels.color_labels[1], spheres[4], coord_labels[4], coord_labels.color_labels[4]).set_opacity, 0.2)
        self.wait(3)
        lines = VGroup()
        curr_n = 2
        for n in [1, 2, 4]:
            ...
            line.set_stroke(YELLOW, 3)
            coin = coins[int(np.log2(n))]
            self.play(ShowCreationThenDestruction(line), spheres[new_n].set_opacity, 1, coord_labels[new_n].set_opacity, 1, coord_labels.color_labels[new_n].set_opacity, 1, FlipCoin(coin))
            line.reverse_points()
            self.add(line, coord_labels)
            self.play(FlipCoin(coin), ShowCreation(line))
            lines.add(line)
        self.wait(10)
        self.play(LaggedStartMap(Uncreate, lines), spheres.set_opacity, 1, coord_labels.set_opacity, 1, coord_labels.color_labels.set_opacity, 1, FadeOut(bit_sum_with_coefs))
        self.wait()
        for x in range(8):
            self.play(get_coloring_animation(np.random.randint(0, 3, 8)))
            self.wait()
        ...
        self.play(count.shift, UP, count.set_opacity, 0.5, ShowIncreasingSubsets(full_board, run_time=4), ShowIncreasingSubsets(full_coins, run_time=4), FadeIn(count64, DOWN), MoveToTarget(frame, run_time=5))
        messages = ['Or, use ', 'Burnside', 'to count', 'modulo  ', 'symmetry']
        ...
        self.play(MoveToTarget(frame, run_time=3), LaggedStartMap(FadeOut, full_board), LaggedStartMap(FadeOut, full_coins), FadeOut(count), FadeOut(count64))
        frame.add_updater(lambda m, dt: m.increment_theta(0.01 * dt))
        self.wait(30)

class CubeSupplement(ThreeDScene):
    CONFIG = {'try_different_strategies': False}
    ...
    def construct(self):
        ...
        boards.set_height(7)
        boards.to_edge(LEFT)
        coin_sets = Group(*[CoinsOnBoard(board, coin_config={'numeric_labels': True}) for board in boards])
        ...
        self.play(LaggedStartMap(FadeIn, boards, lag_ratio=0.25), LaggedStartMap(FadeIn, coin_sets, lag_ratio=0.25), run_time=3)
        self.play(LaggedStartMap(GrowArrow, s_arrows, lag_ratio=0.25), LaggedStartMap(FadeIn, choice_boards, lambda m: (m, LEFT), lag_ratio=0.25))
        self.wait()
        if self.try_different_strategies:
            for x in range(5):
                ...
                self.play(LaggedStartMap(FadeOut, choice_boards, lambda m: (m, 0.25 * UP)), LaggedStartMap(FadeIn, new_cboards, lambda m: (m, 0.25 * DOWN)))
                choice_boards = new_cboards
                self.wait(2)
        else:
            self.play(LaggedStartMap(MoveToTarget, choice_boards), LaggedStartMap(FadeIn, c_labels))
            self.wait()

class TryDifferentCaseThreeStrategies(CubeSupplement):
    CONFIG = {'try_different_strategies': True}

class CubeEdgeDescription(Scene):
    CONFIG = {'camera_config': {'background_color': GREY_E}}

    def construct(self):
        bits = VGroup(*[VGroup(*[Integer(int(b)) for b in string_to_bools(char)]).arrange(RIGHT, buff=SMALL_BUFF) for char in 'hi'])
        bits.arrange(DOWN, buff=LARGE_BUFF)
        arrow = Arrow(bits[0][7].get_bottom(), bits[1][7].get_top(), buff=SMALL_BUFF, tip_config={'length': 0.15, 'width': 0.15})
...
class EdgeColoringExample(Scene):
    ...
    def construct(self):
        words = VGroup(OldTexText('Color edges\\\\red or blue', tex_to_color_map={'red': RED, 'blue': BLUE}), OldTexText('Prove there is a\\\\monochromatic triangle', alignment=''))
        ...
        def get_graph(words=words):
            ...
            edges = VGroup(*[Line(p1, p2, color=random.choice([RED, BLUE])) for p1, p2 in it.combinations(points, 2)])
        ...
        for x in range(2):
            new_graph = get_graph()
            self.play(ShowCreation(new_graph, lag_ratio=0.1, run_time=3), ApplyMethod(graph[1].set_stroke, None, 0, run_time=2))
            graph = new_graph
        self.wait(4)
...
class GrahamsConstantAlt(Scene):
    ...
    def construct(self):
        ...
        for ndots in [1, 3, 6, 7, 9, 12]:
            row = VGroup(*[OldTex('2'), OldTex('\\uparrow\\uparrow'), VGroup(*[OldTex('\\cdot') for x in range(ndots)]).arrange(RIGHT, buff=0.2), OldTex('\\uparrow\\uparrow'), OldTex('3')])
            row.arrange(RIGHT, buff=MED_SMALL_BUFF)
            if ndots == 1:
                rc = row.get_center()
        ...
        self.play(Write(rhs[3]), TransformFromCopy(rhs[2], rhs[4]))
        self.play(TransformFromCopy(rhs[4], rhs[5]), GrowFromCenter(rbrace), Write(rbrace_tex))
        self.wait()
...
class AttemptAColoring(ThreeDScene):
    ...
    def construct(self):
        ...
        cube.to_edge(RIGHT)
        cube.set_height(4)
        self.play(ShowCreation(cube.edges, lag_ratio=0.1), LaggedStartMap(FadeInFromLarge, cube.verts, lambda m: (m, 0.2)), run_time=2)
        ...
        for vert, label in zip(cube.verts, color_labels):
            vert.label = label
        self.play(Transform(cube.verts, new_verts), Write(color_labels), run_time=2)
        self.wait()
        ...
        def get_color_change_animations(values, verts=cube.verts, labels=color_labels, gcv=get_colored_vertices):
            ...
            return [Transform(verts, new_verts), LaggedStartMap(FadeOut, old_labels, lambda m: (m, 0.5 * UP), lag_ratio=0.03), LaggedStartMap(FadeIn, labels, lambda m: (m, 0.5 * DOWN), lag_ratio=0.03)]
        ...
        self.play(MoveToTarget(trees[0]), run_time=3)
        self.wait()
        self.play(LaggedStartMap(MoveToTarget, trees[1:], lag_ratio=0.3), run_time=6)
        self.add(trees)
        self.wait()
        ...
        for tree in trees:
            ...
            for color, letter, leaf, label in zip(colors, letters, tree.leafs, tree.labels):
                ...
                anims += [FadeIn(label, 0.1 * LEFT), FadeOut(old_label, 0.1 * RIGHT), ApplyMethod(leaf.set_color, color)]
        cube.verts.generate_target()
        cube.verts.save_state()
        cube.verts.target.set_color(GREY)
        for vert in cube.verts.target:
            vert.scale(0.75)
        self.play(ShowCreation(want_rect), Write(want_label), LaggedStart(*anims, lag_ratio=0.001, run_time=3), FadeOut(color_labels), MoveToTarget(cube.verts))
        self.add(trees)
        self.wait()
        ...
        self.wait()
        frame = self.camera.frame
        self.play(UpdateFromAlphaFunc(frame, lambda m, a: m.move_to(0.1 * wiggle(a, 6) * RIGHT)), FadeOut(tree_copies[0]), FadeOut(tree_copies[1]))

        def restore_vertex(n, verts=cube.verts, labels=color_labels):
            return AnimationGroup(Transform(verts[n], verts.saved_state[n]), FadeIn(labels[n], DOWN))
        for i in [0, 4, 2, 1]:
        ...
        self.play(Transform(cube_copy.verts, new_verts), FadeOut(q_marks), FadeOut(color_labels[:3]), FadeOut(color_labels[4:7]))
        self.add(cube_copy)
        ...
        self.play(LaggedStartMap(Uncreate, cube_copy.edges), LaggedStartMap(MoveToTarget, cube_copy.verts))
        rects = VGroup()
        for tree in trees:
            t_rect = SurroundingRectangle(Group(tree.leafs, tree.labels), buff=SMALL_BUFF)
        ...
        number_labels = VGroup(*[OldTexText('Number of ', f'{color} vertices') for color in ['red', 'green', 'blue']])
        ...
        for label in number_labels:
            ...
            rhss.add(rhs)
        self.play(Write(implies), FadeOut(nv_label))
        self.play(GrowFromCenter(vert_eqs), FadeIn(number_labels[0], DOWN), FadeIn(number_labels[1]), FadeIn(number_labels[2], UP))
        self.wait()
        self.play(Write(rhss))
        self.wait(2)
        self.play(LaggedStartMap(FadeOut, VGroup(*number_labels, *vert_eqs, *rhss, *implies)), ShowCreation(cube.edges, lag_ratio=0.1), LaggedStartMap(FadeInFromLarge, cube.verts, lambda m: (m, 0.2)))
        self.add(cube)
        new_verts, color_labels = get_colored_vertices(mod3_strategy)
        true_trees = trees.saved_state
        self.play(Transform(cube.verts, new_verts), FadeIn(color_labels), FadeOut(trees), FadeOut(want_label))
        ...
        for n in range(8):
            ...
            self.play(*map(ShowCreation, [*new_edges, *new_tree_edges]))
            for highlight, rect in zip(neighbor_highlights, label_rects):
                self.play(FadeInFromLarge(highlight, 1.2), FadeInFromLarge(rect, 1.2), run_time=0.25)
                red_total.increment_value()
                self.wait(0.25)
            self.play(FadeOut(neighbor_highlights), FadeOut(new_edges), FadeOut(new_tree_edges))
            all_label_rects.add(*label_rects)
        self.wait()
        new_verts = get_colored_vertices([-1] * 8)[0]
        self.play(FadeOut(true_trees), FadeOut(all_label_rects), FadeOut(red_total), FadeOut(color_labels), Transform(cube.verts, new_verts))
        ...
        for tree in trees:
            ...
            label_rects.add(rect)
        self.play(ShowIncreasingSubsets(label_rects, rate_func=linear), UpdateFromFunc(red_total, lambda m, lr=label_rects: m.set_value(len(lr))))
        self.wait()
        ...
        self.play(LaggedStartMap(FadeInFromLarge, r_verts), LaggedStartMap(FadeInFromLarge, r_labels), run_time=1)
        self.wait()
        ...
        self.play(MoveToTarget(three, path_arc=90 * DEGREES), red_total.next_to, over, LEFT, MED_SMALL_BUFF, FadeIn(over, UR), rhs[2].move_to, three, LEFT)
        self.wait()
...
class TryTheProofYourself(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says('Can you predict\\\\the proof?', target_mode='hooray', bubble_config={'height': 3, 'width': 3})
        self.teacher.bubble.set_fill(opacity=0)
        self.play_student_changes('pondering', 'thinking', 'confused', look_at=self.screen)
        self.wait(3)
...
class HighDimensionalCount(ThreeDScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreationThenFadeOut(grid, lag_ratio=0.1), FadeIn(board), FadeIn(coins, lag_ratio=0.1), run_time=2)
        ...
        for n, neighbor in enumerate(neighbors):
            edge = Line(vert.get_center(), neighbor.get_center(), buff=vert.get_height() / 2)
        ...
        vertex_group = Group(vert_board, vert_label, vert, edges, neighbors, neighbor_labels, neighbor_boards)
        ...
        for m1, m2 in zip(cl_mover.target, vert_label[1::2]):
            m1.replace(m2)
        self.play(MoveToTarget(cl_mover))
        self.play(FadeIn(vert_label), FadeOut(cl_mover))
        self.play(FadeIn(vert_board))
        self.wait()
        self.play(ShowIncreasingSubsets(neighbor_labels), ShowCreation(edges))
        self.wait()
        self.play(LaggedStartMap(TransformFromCopy, neighbor_boards, lambda m, b=vert_board: (b, m)))
        self.wait()
        self.play(FadeInFromLarge(vert))
        self.play(LaggedStartMap(TransformFromCopy, neighbors, lambda m, v=vert: (v, m)))
        self.wait()
        edges.apply_depth_test()
        ...
        for edge, nv in zip(tree.target[1], tree.target[2]):
            new_edge = Line(vert.get_center(), nv.get_center())
        ...
        self.play(MoveToTarget(tree), MoveToTarget(short_label), LaggedStartMap(FadeOut, Group(vert_label[0::2], vert_board, *neighbor_labels, *neighbor_boards, *board, *coins)), run_time=2)
        ...
        self.wait()
        self.play(GrowFromCenter(dots), GrowFromCenter(top_brace), LaggedStartMap(TransformFromCopy, new_trees, lambda m, t=tree: (t, m)), Write(total_label, run_time=1), run_time=2)
        self.wait()
        middle_tree = tree
        frame = self.camera.frame
        ...
        self.wait()
        self.play(ChangeDecimalToValue(count, 63, rate_func=rush_into), LaggedStartMap(FadeIn, n_arrows[2:], lag_ratio=0.5), run_time=3)
        self.remove(count)
        ...
        self.play(MoveToTarget(new_trees), top_brace.match_width, new_trees.target, {'about_edge': RIGHT}, total_label.shift, shift_vect * 0.5, n_arrows[:2].shift, shift_vect, FadeOut(middle_tree, RIGHT), FadeOut(n_arrows[2], RIGHT), FadeOut(dots[0], 2 * RIGHT), Write(pairs))
        self.play(LaggedStartMap(FadeIn, marks, lambda m: (m, 0.2 * LEFT), lag_ratio=0.4, run_time=5))
        self.wait()

class SimpleRect(Scene):

    def construct(self):
        rect = SurroundingRectangle(VGroup(Integer(4), Integer(16), Integer(0)).arrange(RIGHT, MED_LARGE_BUFF))
        self.play(ShowCreation(rect))
        self.wait(2)
...
class WhenIsItHopeless(Scene):
    ...
    def construct(self):
        boards = Group(Chessboard(shape=(1, 3)), Chessboard(shape=(2, 2)), Chessboard(shape=(2, 3)), Chessboard(shape=(2, 3)), Chessboard(shape=(2, 4)), Chessboard(shape=(2, 4)), Chessboard(shape=(3, 3)), Chessboard(shape=(3, 4)), Chessboard(shape=(3, 4)), Chessboard(shape=(3, 4)))
        last_board = None
        last_coins = None
        last_words = None
        for n, board in zip(it.count(3), boards):
            ...
            if n == 3:
                self.play(FadeIn(board), FadeIn(coins), FadeIn(words, DOWN))
            else:
                self.play(ReplacementTransform(last_board, board), ReplacementTransform(last_coins, coins), FadeOut(last_words), FadeIn(words, DOWN))
            self.wait()
            last_board = board
            last_coins = coins
            last_words = words
...
class FourDCubeColoringFromTrees(ThreeDScene):
    ...
    def construct(self):
        ...
        for vert in cube.verts:
            tree = Group(vert, vert.edges, vert.neighbors).copy()
            original = tree.copy()
        ...
        self.play(frame.set_euler_angles, 20 * DEGREES, 70 * DEGREES, frame.move_to, ORIGIN, LaggedStart(*anims, lag_ratio=0.2), run_time=8)
        self.remove(trees)
        self.add(cube)
    ...
    def get_hypercube(self, dim=4, width=4):
        ...
        for n in range(2 ** dim):
            for power in range(dim):
                k = n ^ 1 << power
                edge = Line3D(hc_points[n], hc_points[k], width=0.05, resolution=(31, 31))
                cube.edges.add(edge)
    ...
    def get_hypercube_points(self, dim=4, width=4):
        ...
        vertex_holder.set_points([sum([c * v for c, v in zip(reversed(coords), [RIGHT, UP, OUT])]) for coords in all_coords])
...
class IntroduceHypercube(FourDCubeColoringFromTrees):
    ...
    def construct(self):
        ...
        def reconnect_edges(cube):
            for vert in cube.verts:
                for edge, neighbor in zip(vert.edges, vert.neighbors):
                    edge.become(Line3D(vert.get_center(), neighbor.get_center(), resolution=edge.resolution))
        ...
        def get_cube_intro_anim(n, cubes=cubes, reconnect_edges=reconnect_edges, label=label):
            ...
            return AnimationGroup(Restore(cubes[n]), ChangeDecimalToValue(label[0], n))
        self.play(FadeIn(label, DOWN), get_cube_intro_anim(0))
        self.wait()
        for n in [1, 2]:
            self.play(get_cube_intro_anim(n))
            self.wait()
        self.play(get_cube_intro_anim(3), ApplyMethod(frame.set_euler_angles, -20 * DEGREES, 75 * DEGREES, run_time=3))
        ...
        self.play(FadeIn(plane, run_time=2), Transform(cubes[3], flat_cube, run_time=2))
        self.wait(7)
        ...
        for x in range(6):
            ...
            self.play(ShowCreation(new_edges, lag_ratio=0.2), MoveToTarget(neighbors))
            self.wait(1)
            self.play(FadeOut(new_edges), Restore(neighbors))
...
class WantAdditionToBeSubtraction(ThreeDScene):
    ...
    def construct(self):
        coins = CoinsOnBoard(Chessboard(shape=(1, 4)), coin_config={'numeric_labels': True})
        ...
        arrows = VGroup(*[Arrow(some_label.get_top(), coef.get_bottom()) for coef in coefs])
        for coin in coins:
            coin.save_state()
            coin.rotate(90 * DEGREES, UP)
            coin.set_opacity(0)
        self.play(LaggedStartMap(Restore, coins, lag_ratio=0.3), run_time=1)
        self.play(FadeIn(to_fade), LaggedStartMap(FadeInFromPoint, coefs, lambda m: (m, some_label.get_top())), LaggedStartMap(GrowArrow, arrows), Write(some_label, run_time=1))
        self.wait()
...
class BitVectorSum(ThreeDScene):
    ...
    def construct(self):
        ...
        bit_vectors = VGroup(*[IntegerMatrix(coords.reshape((2, 1))).set_height(1) for coords in all_coords])
        bit_vectors.arrange(RIGHT, buff=2)
        bit_vectors.to_edge(UP)
        bit_vectors.set_stroke(BLACK, 4, background=True)
        arrows = VGroup(Arrow(board[0].get_corner(UL), bit_vectors[0].get_corner(DR)), Arrow(board[1].get_corner(UP), bit_vectors[1].get_corner(DOWN)), Arrow(board[2].get_corner(UP), bit_vectors[2].get_corner(DOWN)), Arrow(board[3].get_corner(UR), bit_vectors[3].get_corner(DL)))
        self.add(board)
        self.add(coins)
        for arrow, vector in zip(arrows, bit_vectors):
            self.play(GrowArrow(arrow), FadeInFromPoint(vector, arrow.get_start()))
            self.wait()
        ...
        for coin, cc, dot, plus in zip(coins, coin_copies, cdots, plusses):
            self.play(TransformFromCopy(coin, cc), Write(dot))
        ...
        def get_rhs(coins=coins, bit_vectors=bit_vectors, all_coords=all_coords, eq=eq):
            bit_coords = sum([b * coords for coords, b in zip(all_coords, coins.get_bools())]) % 2
        ...
        def get_rhs_anim(rhs, bit_vectors=bit_vectors):
            ...
            return AnimationGroup(MoveToTarget(bv_copies, remover=True), ShowIncreasingSubsets(Group(rhs), int_func=np.floor))
        rhs = get_rhs()
        mod2_label = OldTexText('(Add mod 2)')
        mod2_label.next_to(rhs, DOWN, MED_LARGE_BUFF)
        mod2_label.to_edge(RIGHT)
        self.play(Write(eq), get_rhs_anim(rhs), FadeIn(mod2_label), FadeOut(board), FadeOut(coins), FadeOut(arrows))
        self.wait(2)
        for x in range(8):
            ...
            coins[i].flip()
            rhs = get_rhs()
            self.play(ShowCreation(rect), FlipCoin(coin_copies[i]), FadeOut(old_rhs, RIGHT), FadeIn(rhs, LEFT))
            self.play(FadeOut(rect))
            self.wait(2)
...
class ExampleSquareAsBinaryNumber(Scene):
    ...
    def construct(self):
        ...
        bin_nums = VGroup()
        for n, square in enumerate(board):
            bin_num = VGroup(*[Integer(int(b)) for b in int_to_bit_coords(n, min_dim=6)])
        ...
        self.play(LaggedStart(*[AnimationGroup(MoveToTarget(num), Restore(bin_num)) for num, bin_num in zip(nums, bin_nums)]), lag_ratio=0.1)
        self.remove(nums)
        nums = original_nums
        self.wait(2)
        self.play(bin_nums.set_stroke, None, 0, bin_nums.set_opacity, 0.1)
        self.wait()
        n = 43
        self.play(board[n].set_color, MAROON_E, Animation(bin_nums[n]))
        last = VMobject()
        shown_nums = VGroup()
        ...
        self.wait()
        self.play(FadeOut(last), bin_nums[n].set_opacity, 1, bin_nums[n].set_fill, YELLOW)
        self.wait()
...
class SkipSkipYesYes(Scene):
    ...
    def construct(self):
        ...
        words = VGroup(OldTexText('Skip'), OldTexText('Skip'), OldTexText('Yes'), OldTexText('Yes'))
...
class ShowCurrAndTarget(Scene):
    CONFIG = {'bit_strings': ['011010', '110001', '101011']}

    def construct(self):
        words = VGroup(OldTexText('Current: '), OldTexText('Need to\\\\change:'), OldTexText('Target: '))
        ...
        def get_bit_aligned_bit_string(bit_coords):
            ...
            result.set_stroke(BLACK, 4, background=True)
            return result
        bit_strings = VGroup(*[get_bit_aligned_bit_string(bs) for bs in self.bit_strings])
        ...
        for n in reversed(range(6)):
            rect = SurroundingRectangle(Group(bit_strings[0][n], bit_strings[2][n], buff=0.05))
            rect.stretch(0.9, 0)
...
class ShowCurrAndTargetAlt(ShowCurrAndTarget):
    CONFIG = {'bit_strings': ['110100', '010101', '100001']}
...
class EulerDiagram(Scene):
    ...
    def construct(self):
        ...
        circles = VGroup(*[Circle(radius=2, fill_color=color, stroke_color=color, fill_opacity=0.5, stroke_width=3).shift(1.2 * vect) for vect, color in zip(vects, colors)])
        ...
        for coords, bstring in zip(bit_coords[1:], bit_strings[1:]):
            ...
            self.play(FadeIn(bstring), *map(MoveToTarget, circles), run_time=0.25)
            self.wait(0.75)
        self.wait()
...
class ShowBoardRegions(ThreeDScene):
    ...
    def construct(self):
        ...
        bin_nums = VGroup()
        for n, square in enumerate(board):
            bin_num = VGroup(*[Integer(int(b), fill_color=GREY_A) for b in int_to_bit_coords(n, min_dim=6)])
            bin_num.arrange(RIGHT, buff=SMALL_BUFF)
            bin_num.set_width(square.get_width() * 0.8)
            num = Integer(n)
            num.set_height(square.get_height() * 0.4)
        ...
        self.add(board)
        self.play(ShowIncreasingSubsets(nums, run_time=4, rate_func=bezier([0, 0, 1, 1])))
        self.wait()
        self.play(LaggedStart(*[AnimationGroup(MoveToTarget(num), MoveToTarget(pbn), Restore(bin_num)) for num, pbn, bin_num in zip(nums, pre_bin_nums, bin_nums)], lag_ratio=1.5 / 64))
    ...
    def construct(self):
        ...
        for i in range(6):
            ...
            for n, coin in enumerate(coins):
                if bool(n & 1 << i):
                    line_group.add(Line(coin.get_center(), bit.get_center()))
        ...
        for lines, fades, movers, og, cm, bit in zip(line_groups, fade_groups, mover_groups, one_groups, count_mobs, bits):
            self.play(FadeIn(add_words), fades.set_opacity, 0.1)
            if self.show_with_lines:
                ...
                bit.set_opacity(0)
                self.play(LaggedStartMap(ShowCreation, lines, run_time=2), LaggedStartMap(MoveToTarget, movers, lag_ratio=0.01), Restore(bit))
                self.remove(movers)
                self.add(bit)
                self.play(FadeOut(lines))
            else:
                self.play(ShowIncreasingSubsets(og), UpdateFromFunc(cm, lambda m: m.set_value(len(og))))
                self.play(FadeInFromPoint(bit, cm.get_center()))
                self.play(FadeOut(og), FadeOut(cm))
            self.play(FadeOut(add_words), Restore(fades))
            self.remove(fades)
            self.add(coins)
        self.wait()

class ShowFinalStrategyWithFadeLines(ShowFinalStrategy):
    CONFIG = {'show_with_lines': True}

class Thumbnail(FourDCubeColoringFromTrees):

    def construct(self):
        board = Chessboard(shape=(8, 8), square_resolution=(5, 5), top_square_resolution=(7, 7))
        board.set_gloss(0.3)
        k = 16
        board[k].set_color(YELLOW)
        board.set_height(7)
        coins = CoinsOnBoard(board, coin_config={'disk_resolution': (2, 12)})
        ...
        self.add(titles)
        return
        message = OldTexText('Flip one coin\\\\to describe a\\\\', 'unique square', alignment='')
        message[1].set_color(YELLOW)
        message.scale(1.25)
        message.to_edge(LEFT)
        message.shift(1.25 * DOWN)
        message.fix_in_frame()
        arrow = Arrow(message.get_corner(UR), message.get_corner(UR) + 3 * RIGHT + UP, path_arc=-70 * DEGREES)
        arrow.fix_in_frame()
