...
class Diffusion1D(Scene):
    CONFIG = {'n_dots': 100, 'y_range': (0, 100, 10), 'x_range': (-20, 20), 'show_y_axis': False, 'dot_radius': 0.07, 'dot_opacity': 0.5, 'dither_dots': True, 'total_steps': 100, 'clip_at_bounds': True}

    def construct(self):
        y_range = self.y_range
        x_range = self.x_range
        axes = Axes(x_range=x_range, y_range=y_range, axis_config={'stroke_width': 2, 'include_tip': False}, width=FRAME_WIDTH, height=FRAME_HEIGHT - 1)
        axes.x_axis.add_numbers(range(*x_range, 5))
        if self.show_y_axis:
            axes.y_axis.add_numbers(np.arange(*y_range) + y_range[2], height=0.2)
        else:
        ...
        def update_bars(bars, dots=dots, y_unit=y_unit, epsilon=epsilon):
            for bar in bars:
                ...
                for dot in dots:
                    if dot.x == bar.x:
                        count += 1
                bar.set_height(count * y_unit + epsilon, about_edge=bar.get_bottom(), stretch=True)
    ...
    def get_time_label(self):
        time_label = VGroup(OldTexText('Time: '), Integer(0))
    ...
    def get_dots(self):
        ...
        for dot in dots:
            dot.x = 0
            if self.dither_dots:
                dot.shift(self.dot_radius * random.random() * RIGHT, self.dot_radius * random.random() * UP)
            dot.set_color(interpolate_color(BLUE_B, BLUE_D, random.random()))
        return dots
...
class Diffusion1DWith1Dot(Diffusion1D):
    CONFIG = {'n_dots': 1, 'dot_radius': 0.1, 'dot_opacity': 1, 'dither_dots': False}

class Diffusion1DStepFunction(Diffusion1D):
    CONFIG = {'n_dots': 15000, 'initial_range': (-20, 0), 'y_range': (0, 1000, 100), 'total_steps': 100}
...
class Diffusion1DStepFunctionGraphed(Diffusion1DStepFunction):
    CONFIG = {'show_y_axis': True, 'total_steps': 500}

class DiffusionDeltaGraphed(Diffusion1D):
    CONFIG = {'n_dots': 1000, 'show_y_axis': True, 'y_range': (0, 1000, 100), 'total_steps': 200}
...
class DiffusionDeltaGraphedShowingMean(DiffusionDeltaGraphed):
    CONFIG = {'n_dots': 10000, 'y_range': (0, 10000, 1000), 'clip_at_bounds': False, 'total_steps': 100}

    def adjust_initial_dot_positions(self, dots, x_unit):
        label = VGroup(OldTex('\\overline{x^2} = '), DecimalNumber(0))
        label.arrange(RIGHT)
        label.to_corner(UL)
        label.add_updater(lambda m: m[1].set_value(np.mean([dot.x ** 2 for dot in dots])))
        self.add(label)

class Diffusion2D(Diffusion1D):
    CONFIG = {'n_dots': 100, 'dot_opacity': 0.5, 'dither_dots': True, 'grid_dimensions': (19, 35)}

    def construct(self):
        grid_dimensions = self.grid_dimensions
        grid = VGroup(*[Square() for x in range(grid_dimensions[0]) for y in range(grid_dimensions[1])])
...
class Diffusion2D1Dot(Diffusion2D):
    CONFIG = {'n_dots': 1, 'dither_dots': False, 'dot_opacity': 1, 'total_steps': 50}

class Diffusion2D10KDots(Diffusion2D):
    CONFIG = {'n_dots': 10000, 'dot_opacity': 0.2, 'total_steps': 200}