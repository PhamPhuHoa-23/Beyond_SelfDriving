...
class ConvolutionIntroduction(InteractiveScene):
    ...
    def construct(self):
        ...
        pixel_array = VGroup(*[Square(fill_color=rgb_to_hex(arr[i, j] / 255), fill_opacity=1) for i in range(height) for j in range(width)])
        ...
        def step(pos=0):
            ...
            j = pos % width
            h, w = kernel.shape
            pixels = np.array([square.data['fill_rgba'][0] for square in pixel_array]).reshape((height, width, 4))
            rgba = sum([kernel[k, l] * pixels[i - k, j - l] for k in range(-(w // 2), w // 2 + 1) for l in range(-(h // 2), h // 2 + 1) if 0 <= i - k < pixels.shape[0] and 0 <= j - l < pixels.shape[1]])
        ...
        def zoom_to_kernel():
            self.play(frame.animate.set_height(1.5 * kernel_array.get_height()).move_to(kernel_array), run_time=2)

        def zoom_to_new_pixel():
            self.play(frame.animate.set_height(1.5 * kernel_array.get_height()).move_to(right_rect), run_time=2)
        ...
        self.wait()
        gauss_kernel = np.array([[0.00296902, 0.0133062, 0.0219382, 0.0133062, 0.00296902], [0.0133062, 0.0596343, 0.0983203, 0.0596343, 0.0133062], [0.0219382, 0.0983203, 0.162103, 0.0983203, 0.0219382], [0.0133062, 0.0596343, 0.0983203, 0.0596343, 0.0133062], [0.00296902, 0.0133062, 0.0219382, 0.0133062, 0.00296902]])
        ...
        self.wait()
        self.play(frame.animate.set_height(8).reorient(0, 60).set_x(-1), run_time=3)
