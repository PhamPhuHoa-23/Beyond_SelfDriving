...
def get_value_grid(n_rows=6, n_cols=6):
    ...
    for box in boxes:
        ...
        value.move_to(box)
        box.add(value)
    boxes.array[5, 1].value.set_value(0.4)
    return boxes
...
class GreedyAlgorithm(Scene):
    ...
    def construct(self):
        ...
        final_sum = VGroup(OldTex('='), DecimalNumber(sum([np.round(st[1].get_value(), 1) for st in sum_terms]), height=sum_terms.get_height(), num_decimal_places=1).set_color(YELLOW))
    ...
    def construct(self):
        ...
        for i in range(n_rows - 2, -1, -1):
            for j in range(n_cols):
                ...
                boxes_below = [new_boxes.array[i + 1, j] for j in range(j - 1, j + 2) if 0 <= j < n_cols]
                ...
                new_box.target.set_fill(TEAL, opacity=op_factor * new_box.value.get_value())
                new_box.target[1].set_fill(WHITE, 1)
                self.play(FadeIn(box_highlight), FadeIn(new_box_highlight), FadeIn(low_box_highlight), MoveToTarget(new_box))
                self.play(FadeOut(box_highlight), FadeOut(new_box_highlight), FadeOut(low_box_highlight))