...
class IntroduceDFT(Scene):
    ...
    def construct(self):
        ...
        axes = Axes((0, 8), (0, 2, 0.5), width=12, height=4)
        ...
        self.play(LaggedStartMap(FadeIn, s_labels), LaggedStart(*(Transform(s_title[1].copy(), label[1].copy(), remover=True) for label in s_labels)), lag_ratio=0.3, run_time=2)
        self.wait()
        ...
        self.play(FadeOut(axes), MoveToTarget(vectors))
        self.wait()
        ...
        self.play(Write(plane), MoveToTarget(vectors))
        lhs1 = OldTex('\\hat s', '[1]', '=')
        ...
        for n, s_term, plus in zip(it.count(), rhs[0::2], rhs[1::2]):
            ...
            zeta.replace(plus)
            zeta.set_opacity(0)
        self.play(TransformFromCopy(lhs, lhs1), *map(Restore, rhs1))
        self.wait()
        ...
        unit_circle = Circle(radius=get_norm(plane.n2p(1) - plane.n2p(0)))
        ...
        self.play(vectors.set_opacity, 0.1, FadeIn(unit_circle), FadeIn(zeta_power_vectors[1]), Write(zeta_label))
        self.wait()
        ...
        kw = {'lag_ratio': 0.5, 'run_time': 3}
        self.play(LaggedStartMap(MoveToTarget, zeta_powers, **kw), LaggedStartMap(FadeIn, zeta_power_vectors, **kw), Animation(zeta_power_vectors[1].copy(), remover=True), plane.coordinate_labels.set_opacity, 0)
        self.wait()
        self.play(FadeOut(zeta_powers), FadeOut(zeta_power_vectors), vectors.set_opacity, 1, plane.coordinate_labels.set_opacity, 1)
        self.wait()
        ...
        side_vectors = VGroup(*(Arrow(side_axes.c2p(x, 0), side_axes.c2p(x, cos_signal[x]), buff=0, thickness=0.05) for x in range(N)))
        side_vectors.match_style(vectors)
        side_vectors.set_stroke(background=True)
        vectors.generate_target()
        for x, vect in enumerate(vectors.target):
            vect.put_start_and_end_on(plane.n2p(0), plane.n2p(cos_signal[x] * zeta ** (-x)))
        self.play(MoveToTarget(vectors), FadeIn(side_axes), FadeIn(side_vectors))
        self.wait()
        ...
        def change_signal(new_signal, omega=1):
            ...
            for x, sv, v in zip(it.count(), side_vectors.target, vectors.target):
                sv.put_start_and_end_on(side_axes.c2p(x, 0), side_axes.c2p(x, new_signal[x]))
                v.put_start_and_end_on(plane.n2p(0), plane.n2p(new_signal[x] * zeta ** (-omega * x)))
            self.play(MoveToTarget(vectors), MoveToTarget(side_vectors))
        ...
        plane_group = VGroup(plane, vectors, unit_circle)
        self.add(plane_group)
        self.play(TransformFromCopy(lhs1, lhs2), TransformFromCopy(rhs1, rhs2), plane_group.scale, 0.8, {'about_edge': DOWN}, zeta_label.shift, DOWN, FadeOut(side_axes), FadeOut(side_vectors))
        self.add(vectors)
        anims = []
        for n, vect in enumerate(vectors):
            anims.append(Rotate(vect, -n * TAU / N, about_point=plane.n2p(0)))
        self.play(*anims)
        self.wait()
        for n in range(N):
            ...
            self.play(MoveToTarget(rhs2), MoveToTarget(vectors))
            self.wait()
        self.play(vectors.set_opacity, 1, rhs2.set_opacity, 1)
        self.wait()
        ...
        self.play(FadeIn(side_axes), FadeIn(side_vectors))
        self.wait()
        ...
        self.play(Write(dots), FadeIn(formula, DOWN))
        self.wait()
        omega_label = VGroup(OldTex('f = ', font_size=72), Integer(2))
