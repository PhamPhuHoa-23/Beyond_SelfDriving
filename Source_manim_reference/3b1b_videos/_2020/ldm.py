...
class StreamIntro(Scene):
    ...
    def construct(self):
        ...
        anims = []
        for spike in spikes:
            anims.append(Rotate(spike, spike.angle * 28 * 2, about_point=ORIGIN, rate_func=linear))
...
class OldStreamIntro(Scene):
    ...
    def construct(self):
        ...
        morty.set_height(2)
        morty.to_corner(DL)
        self.play(PiCreatureSays(morty, 'The lesson will\\\\begin soon.', bubble_config={'height': 2, 'width': 3}, target_mode='hooray'))
...
class QuadraticFormula(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.add(formula)
        self.play_student_changes('angry', 'tired', 'sad', look_at=formula)
        self.teacher_says("It doesn't have\\\\to be this way.", bubble_config={'width': 4, 'height': 3})
        self.wait(5)
...
class CosGraphs(Scene):
    ...
    def construct(self):
        axes = Axes(x_min=-0.75 * TAU, x_max=0.75 * TAU, y_min=-1.5, y_max=1.5, x_axis_config={'tick_frequency': PI / 4, 'include_tip': False}, y_axis_config={'tick_frequency': 0.5, 'include_tip': False, 'unit_size': 1.5})
...
class SineWave(Scene):
    ...
    def construct(self):
        ...
        def update_c_dot(dot, axes=c_axes, tracker=theta_tracker):
            ...
            dot.move_to(axes.c2p(np.cos(theta), np.sin(theta)))
        ...
        def update_c_line(line, axes=c_axes, tracker=theta_tracker):
            ...
            if y == 0:
                y = 1e-06
            line.put_start_and_end_on(axes.c2p(x, 0), axes.c2p(x, y))
        ...
        def get_partial_circle(circle=circle, tracker=theta_tracker):
            result = circle.copy()
            theta = tracker.get_value()
            result.pointwise_become_partial(circle, 0, clip(theta / TAU, 0, 1))
        ...
        self.add(sin_label)
        self.play(theta_tracker.set_value, 1.25 * TAU, run_time=15, rate_func=linear)

    def get_wave_axes(self):
        wave_axes = Axes(x_min=0, x_max=1.25 * TAU, y_min=-1.0, y_max=1.0, x_axis_config={'tick_frequency': TAU / 8, 'unit_size': 1.0}, y_axis_config={'tick_frequency': 0.5, 'include_tip': False, 'unit_size': 1.5})
        wave_axes.y_axis.add_numbers(-1, 1, num_decimal_places=1)
        wave_axes.to_edge(RIGHT, buff=MED_SMALL_BUFF)
        pairs = [(PI / 2, '\\frac{\\pi}{2}'), (PI, '\\pi'), (3 * PI / 2, '\\frac{3\\pi}{2}'), (2 * PI, '2\\pi')]
        syms = VGroup()
    ...
    def get_edge_group(self):
        axes_max = 1.25
        radius = 1.5
        axes = Axes(x_min=-axes_max, x_max=axes_max, y_min=-axes_max, y_max=axes_max, axis_config={'tick_frequency': 0.5, 'include_tip': False, 'big_tick_numbers': [-1, 1], 'tick_size': 0.05, 'unit_size': radius})
...
class CosWave(SineWave):
    ...
    def construct(self):
        ...
        def update_c_line(line, axes=c_axes, tracker=theta_tracker):
            ...
            x = np.cos(theta)
            y = np.sin(theta)
            line.set_points_as_corners([axes.c2p(0, y), axes.c2p(x, y)])
        ...
        def update_w_line(line, axes=w_axes, tracker=theta_tracker):
            ...
            if y == 0:
                y = 1e-06
            line.set_points_as_corners([axes.c2p(x, 0), axes.c2p(x, y)])
...
class ComplexMultiplication(Scene):
    ...
    def construct(self):
        ...
        self.play(MoveToTarget(one_vect), TransformFromCopy(one_label, z_rhs))
        self.wait()
        ...
        self.wait()
        self.play(MoveToTarget(i_vect), TransformFromCopy(i_label, i_vect_label), run_time=1)
        self.wait()
        self.play(TransformFromCopy(one_vect, i_vect.target, path_arc=-90 * DEGREES))
        self.wait()
        ...
        self.play(MoveToTarget(plane), MoveToTarget(new_plane), run_time=6, rate_func=there_and_back_with_pause)
        self.wait()
        w = complex(2, -1)
        ...
        w_group.target = VGroup(one_vect.copy().set_opacity(0.8), one_vect.copy().shift(plane.n2p(z)).set_opacity(0.8), i_vect.copy().rotate(PI, about_point=ORIGIN).shift(2 * plane.n2p(z)).set_opacity(0.8), Dot(plane.n2p(w * z), color=WHITE))
        self.play(FadeInFromLarge(w_dot))
        self.wait()
        self.play(ShowCreation(vects))
        self.wait()
        self.play(MoveToTarget(plane), MoveToTarget(new_plane), MoveToTarget(w_group), run_time=2, path_arc=np.log(z).imag)
        self.wait()
...
class RotatePiCreature(Scene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(plane), randy.set_opacity, 0.75)
        self.wait()
        dots = VGroup()
        ...
        self.wait()
        label = VGroup(OldTex('(x + iy)'), Vector(DOWN), OldTex('(\\cos(30^\\circ) + i\\sin(30^\\circ))', '(x + iy)'))
...
class ExpMeaning(Scene):
    ...
    def construct(self):
        ...
        if self.include_circle:
            ...
            plane.add(circle)
        equation = OldTex('\\text{exp}(i\\theta) = ', '1 + ', 'i\\theta + ', '{(i\\theta)^2 \\over 2} + ', '{(i\\theta)^3 \\over 6} + ', '{(i\\theta)^4 \\over 24} + ', '\\cdots', tex_to_color_map={'\\theta': YELLOW, 'i': GREEN})
        ...
        theta_label.add_to_back(BackgroundRectangle(theta_label, buff=MED_SMALL_BUFF, fill_opacity=1))
        ...
        self.play(theta_tracker.set_value, 1, run_time=3, rate_func=smooth)
        self.wait()
...
class EulersFormula(Scene):
    ...
    def construct(self):
        ...
        formula = OldTex('&e^{i\\theta} = \\\\ &\\cos\\left(\\theta\\right) + i\\cdot\\sin\\left(\\theta\\right)')[0]
...
class EtoILimit(Scene):
    ...
    def construct(self):
        tex = OldTex('\\lim_{n \\to \\infty} \\left(1 + \\frac{it}{n}\\right)^n')[0]
    ...
    def construct(self):
        ...
        def get_vectors(tracker=dt_tracker, plane=plane, T=8):
            ...
            for t in np.arange(0, T, dt):
                ...
                vects.add(Arrow(plane.n2p(last_z), plane.n2p(next_z), buff=0))
                last_z = next_z
            vects.set_submobject_colors_by_gradient(YELLOW, GREEN, BLUE)
            return vects
        vects = get_vectors()
        line = Line()
        line.add_updater(lambda m, v=vects: m.put_start_and_end_on(ORIGIN, v[-1].get_start() if len(v) > 0 else RIGHT))
        self.add(line)
        self.play(ShowIncreasingSubsets(vects, rate_func=linear, int_func=np.ceil), MoveToTarget(frame, rate_func=squish_rate_func(smooth, 0.5, 1)), run_time=8)
        self.wait()
        self.play(FadeOut(line))
        self.remove(vects)
        vects = always_redraw(get_vectors)
        self.add(vects)
        self.play(Restore(frame), dt_tracker.set_value, 0.2, run_time=5)
        self.wait()
...
class Logs(Scene):
    ...
    def construct(self):
        log = OldTex('&\\text{log}(ab) = \\\\ &\\text{log}(a) + \\text{log}(b)', tex_to_color_map={'a': BLUE, 'b': YELLOW}, alignment='')
        log.scale(1.5)
...
class HarmonicSum(Scene):
    ...
    def construct(self):
        axes = Axes(x_min=0, x_max=13, y_min=0, y_max=1.25, y_axis_config={'unit_size': 4, 'tick_frequency': 0.25})
...
class ComplexExponentialPlay(Scene):
    ...
    def construct(self):
        plane = ComplexPlane(x_min=-2 * FRAME_WIDTH, x_max=2 * FRAME_WIDTH, y_min=-2 * FRAME_HEIGHT, y_max=2 * FRAME_HEIGHT)
        plane.add_coordinates()
        self.add(plane)
        ...
        def update_sample(sample, f=func, plane=plane, scene=self):
            sample.restore()
            sample.apply_function_to_submobject_positions(lambda p: interpolate(p, plane.n2p(f(plane.p2n(p))), scene.transform_alpha))
            return sample
        ...
        def update_axis_liens(lines=axis_lines, f=func, plane=plane, scene=self):
            lines.restore()
            lines.apply_function(lambda p: interpolate(p, plane.n2p(f(plane.p2n(p))), scene.transform_alpha))
            lines.make_smooth()
        axis_lines.add_updater(update_axis_liens)
        labels = VGroup(OldTex('f(1)'), OldTex('f(i)'), OldTex('f(-1)'), OldTex('f(-i)'))
        ...
        self.play(UpdateFromAlphaFunc(VectorizedPoint(), update_transform_alpha))
...
class ProbDiagram(Scene):
    ...
    def construct(self):
        ...
        N = 1000
        for n in range(1, N):
            tri = Polygon(ORIGIN, RIGHT + UP / n, RIGHT + UP / (n + 1))
            tri.set_stroke(width=0)
