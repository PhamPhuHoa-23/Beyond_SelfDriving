...
def get_cube(color=BLUE_D, opacity=1, height=2, frame=None):
    ...
    for face, p_face in zip(cube, poor_cube):
        face.add(*[Line3D(p_face.get_points()[i], p_face.get_points()[j], width=0.02, color=GREY_B) for i, j in [(0, 1), (1, 3), (3, 2), (2, 0)]])
...
def get_glassy_cube(frame):
    ...
    corner_dots = Group(*[Sphere(resolution=(51, 26)).set_height(0.25).move_to(vert) for vert in verts])
...
def get_permutation_arrows(mobs, permutation, arc=PI / 2):
    ...
    for i in range(len(permutation)):
        ...
        arrow = Arrow(mobs[i].get_edge_center(u * UP), mobs[j].get_edge_center(u * UP), buff=SMALL_BUFF, path_arc=arc)
...
def permutation_animation(mobs, perm=None, arc=PI / 2, lag_factor=3, run_time=None):
    ...
    return LaggedStart(*[ApplyMethod(m1.move_to, m2, path_arc=arc) for m1, m2 in zip(mobs, targets)], **kw)
...
class Thumbnail(Scene):
    ...
    def construct(self):
        ...
        words = VGroup(OldTexText('The'), OldTexText('Monster'), OldTexText('Group'))
        words.scale(3.5)
...
class AskAboutFavoriteMegaNumber(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        title.to_edge(UP)
        images = Group(ImageMobject('standupmaths'), ImageMobject('singingbanana'), ImageMobject('Ben Sparks'), ImageMobject('Zoe Griffiths'), ImageMobject('tomrocksmaths'), ImageMobject('James Tanton'), ImageMobject('blackpenredpen'), ImageMobject('Eddie Woo'))
        images.arrange_in_grid(2, 4, buff=MED_LARGE_BUFF)
        images.set_width(FRAME_WIDTH - 2)
        images.next_to(title, DOWN, MED_LARGE_BUFF)
        self.play(FadeIn(title, DOWN), LaggedStartMap(FadeIn, images, lambda m: (m, -0.1 * m.get_center()), lag_ratio=0.3, run_time=5))
        self.wait()
        self.teacher_says('And we want\\\\you to join!', target_mode='surprised', bubble_config={'height': 3, 'width': 4}, added_anims=[VFadeIn(self.pi_creatures), images.scale, 0.2, images.space_out_submobjects, 10, images.set_opacity, 0])
        self.remove(images)
...
class IntroduceMonsterSize(Scene):
    ...
    def construct(self):
        ...
        self.play(UpdateFromAlphaFunc(size_label, lambda m, a: m.set_submobjects(partials[int(a * n_syms)].submobjects), run_time=6, rate_func=bezier([0, 0, 1, 1])))
        self.wait()
        factors = OldTex('= 2^{46} \\cdot 3^{20} \\cdot 5^{9} \\cdot 7^{6} \\cdot 11^{2} \\cdot 13^{3} \\cdot 17 \\cdot 19 \\cdot 23 \\cdot 29 \\cdot 31 \\cdot 41 \\cdot 47 \\cdot 59 \\cdot 71')
        ...
        self.play(size_label.next_to, factors, UP, MED_LARGE_BUFF, FadeIn(factors, 0.5 * DOWN), Write(approx))
        self.wait()
        ...
        self.play(UpdateFromAlphaFunc(jupiter, lambda m, a: m.set_opacity(a)), ApplyMethod(value_group.to_edge, UP, run_time=2))
        self.wait(4)
        ...
        self.play(FadeOut(jupiter, DOWN), DrawBorderThenFill(alien))
        ...
        question.scale(2)
        question.move_to(UP)
        self.play(LaggedStartMap(FadeOut, VGroup(factors, approx, alien_words, alien, server_words, server), lambda m: (m, DOWN)), ApplyMethod(size_label.move_to, 0.5 * DOWN, run_time=2), FadeIn(question, UP, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)))
        self.wait()
        monster = get_monster()
        ...
        self.play(question.shift, 2 * LEFT, DrawBorderThenFill(monster), run_time=2)
...
class IntroduceGroupTheory(Scene):
    ...
    def construct(self):
        ...
        sym_line.scale(1.05)
        sym_line.set_stroke(WHITE, 2)
        self.play(FadeIn(face, DOWN), FadeIn(face_citation), LaggedStartMap(FadeOut, VGroup(*arrows, title), lambda m: (m, UP)), sym_amb.replace, sym_word.saved_state, {'stretch': True}, sym_amb.set_opacity, 0, Restore(sym_word))
        self.remove(sym_amb)
        self.wait()
        ...
        self.play(ReplacementTransform(sym_word[0], sym_to_action[0]), Write(sym_to_action[1]), FadeIn(sym_to_action[2], LEFT))
        self.play(ApplyMethod(face.stretch, -1, 0, run_time=2))
        self.wait()
        snowflake = get_snowflake()
        snowflake.set_height(5)
        snowflake.next_to(sym_to_action, DOWN, MED_LARGE_BUFF)
        self.play(FadeOut(face), Uncreate(sym_line), ShowCreationThenFadeOut(snowflake.copy().set_stroke(WHITE, 2).set_fill(opacity=0)), FadeIn(snowflake, run_time=2))
        ...
        rot_icons = VGroup(get_flake_rot_icon(60), get_flake_rot_icon(120))
        flip_icons = VGroup(get_flake_flip_icon(60 * DEGREES), get_flake_flip_icon(30 * DEGREES))
        ...
        self.play(ShowCreation(sym_line), FadeIn(flip_icons[0]))
        self.play(Rotate(snowflake, PI, axis=rotate_vector(RIGHT, 60 * DEGREES)))
        self.play(sym_line.set_angle, 30 * DEGREES, sym_line.move_to, snowflake, FadeIn(flip_icons[1]))
        self.play(Rotate(snowflake, PI, axis=rotate_vector(RIGHT, 30 * DEGREES)))
        rot_icons.generate_target()
        flip_icons.generate_target()
        d6_group = VGroup(get_flake_rot_icon(0), rot_icons.target[0], rot_icons.target[1], get_flake_rot_icon(180), get_flake_rot_icon(-120), get_flake_rot_icon(-60), get_flake_flip_icon(0), flip_icons.target[1], flip_icons.target[0], get_flake_flip_icon(90 * DEGREES), get_flake_flip_icon(120 * DEGREES), get_flake_flip_icon(150 * DEGREES))
        ...
        self.play(MoveToTarget(rot_icons), MoveToTarget(flip_icons), ApplyMethod(snowflake.scale, 0, remover=True), ApplyMethod(sym_line.scale, 0, remover=True))
        ...
        self.play(FadeOut(sym_to_action, UP), FadeIn(group_name, DOWN), ShowCreation(d6_rect))
        ...
        self.play(FadeIn(id_words, lag_ratio=0.1), GrowArrow(id_arrow))
        self.wait()
        ...
        self.add(counter)
        self.play(FadeOut(id_words), FadeOut(id_arrow), ShowIncreasingSubsets(rects, int_func=np.ceil, run_time=2), UpdateFromAlphaFunc(counter, lambda m, a: m.set_opacity(a)))
        self.wait()
        d6_name = OldTex('D_6')
        d6_name.scale(2)
        d6_name.move_to(counter)
        d6_name.set_color(BLUE)
        self.play(FadeOut(rects, lag_ratio=0.1), FadeOut(counter, 0.2 * UP), FadeIn(d6_name, 0.2 * DOWN))
        self.wait()
        ...
        self.play(d6_rect.replace, face_group, {'stretch': True}, d6_rect.scale, 1.1, FadeOut(d6_group), FadeOut(d6_name, DOWN), *[FadeIn(f, -0.5 * f.get_center()) for f in face_group])
...
class ZooOfGroups(ThreeDScene):
    ...
    def construct(self):
        ...
        k4_axes = Group(Line3D(LEFT, RIGHT, color=RED), Line3D(DOWN, UP, color=GREEN), Line3D(IN, OUT, color=BLUE))
        ...
        qubit = OldTex('\\alpha|0\\rangle + \\beta|1\\rangle', tex_to_color_map={'\\alpha': BLUE, '\\beta': YELLOW})
        qubit.set_height(1)
        qubit.set_height(1)
        groups = Group(Group(OldTex('C_2'), dot_pair), Group(OldTex('D_6'), snowflake), Group(OldTex('K_4'), k4_axes), Group(OldTex('Q_8'), quat_group), Group(OldTex('S_4'), cube), Group(OldTex('SO(3)'), sphere), Group(OldTex('\\mathds{R}^+ / \\mathds{Z}'), circle), Group(OldTex('SU(2)'), qubit), Group(get_monster(), monster_object))
        for group in groups:
        ...
        self.play(LaggedStart(Rotate(dot_pair, PI), Blink(monster_object[-1]), Rotate(cube, PI / 2, axis=cube[0].get_center() - cube[-1].get_center()), Rotate(snowflake, 120 * DEGREES), Rotate(k4_axes, PI, axis=RIGHT), Rotate(sphere, 170 * DEGREES, axis=UP), run_time=3, lag_ratio=0.1))
        self.wait()
        self.play(groups[-1].scale, 3, groups[-1].center, groups[-1].space_out_submobjects, 1.5, *[FadeOut(mob, mob.get_center() - groups[-1].get_center()) for mob in groups[:-1]])
...
class SymmetriesOfACube(ThreeDScene):
    ...
    def construct(self):
        ...
        pairs = [(90, UP), (90, RIGHT), (90, OUT), (120, [1, 1, 1]), (120, [1, -1, 1]), (180, UP)]
        ...
        self.play(FadeIn(count_label, DOWN), FadeOut(question, UP))
        ...
        self.play(ShowCreation(cross24), ApplyMethod(cube.stretch, -1, 0))
        self.wait()
        self.play(Rotate(reflection_plane, PI / 2, axis=UP))
        self.play(ApplyMethod(cube.stretch, -1, 2))
        self.wait()
        self.play(Rotate(reflection_plane, PI / 4, UP))
        self.play(cube.stretch, -1, 2, cube.rotate, PI / 2, UP)
        self.wait()
        self.add(count_label[0], cross24)
        self.play(count_label[0].shift, 2 * LEFT, cross24.shift, 2 * LEFT, FadeIn(label48, UP))
        self.play(reflection_plane.rotate, PI / 4, UP, reflection_plane.rotate, PI / 2, OUT)
        self.play(cube.stretch, -1, 1)
        self.wait()
        ...
        def explostion_transform(self=self, cube=cube):
            cube_copy = cube.copy()
            self.play(cube.space_out_submobjects, 1.5, cube.shift, 0.5 * OUT)
            exploded_cube_copy = cube.copy()
            self.play(LaggedStart(*[Rotate(face, axis=face.get_center() - cube.get_center(), angle=random.choice([0, PI / 2, -PI / 2, PI])) for face in cube]))
            perm = list(range(6))
            random.shuffle(perm)
            globals()['perm'] = perm
            self.play(LaggedStart(*[Transform(face, cube[perm[i]]) for i, face in enumerate(cube)], lag_ratio=0.1))
        ...
        self.play(MoveToTarget(old_counts), FadeIn(count), count_label.next_to, count, RIGHT)
        for x in range(3):
            explostion_transform()
        self.wait(2)
...
class PermutationGroups(Scene):
    ...
    def construct(self):
        ...
        def get_permutation(self=self, dots=dots, arc=PI / 2):
            ...
            return Succession(UpdateFromAlphaFunc(arrows, lambda m, a: m.set_opacity(a)), LaggedStartMap(MoveToTarget, dots, path_arc=arc, lag_ratio=0.15, run_time=2), UpdateFromAlphaFunc(arrows, lambda m, a: m.set_opacity(1 - a)))
        ...
        dots = alt_dots
        self.add(alt_dots)
        self.play(FadeIn(full_count, LEFT), FadeOut(perm_words[0], RIGHT), perm_words[1].shift, 0.7 * RIGHT)
        all_perms = list(it.permutations(range(6)))
        arrows = VGroup()
        self.add(arrows)
        self.play(ChangeDecimalToValue(full_count, 720), UpdateFromAlphaFunc(arrows, lambda m, a, dots=dots, all_perms=all_perms: m.set_submobjects(get_permutation_arrows(dots, all_perms[int(np.round(719 * a))]))), run_time=20)
        self.play(FadeIn(perm_count, RIGHT), Write(equals))
        self.wait(2)
        ...
        for dot, point in zip(dots.target, compass_directions(6, UP)):
            dot.move_to(2 * point)
        self.play(FadeOut(arrows), FadeOut(perm_label, UP), FadeOut(question, 0.5 * UP), MoveToTarget(dots))
        lines = VGroup()
        for d1, d2 in it.combinations(dots, 2):
            lines.add(Line(d1.get_center(), d2.get_center(), buff=d1.get_width() / 4))
        ...
        self.play(MoveToTarget(dots), FadeIn(perm_label), LaggedStartMap(MoveToTarget, lines, run_time=1.5))
        new_dots = dots.copy()
        new_dots.shift(1.5 * DOWN)
        new_perm_label = VGroup(OldTex('12!'), OldTex('='), Integer(math.factorial(12)), OldTexText('permutations')[0])
        ...
        for x in range(5):
            ...
            self.wait()
        new_perm_label = VGroup(OldTex('101!'), OldTex('\\approx'), OldTex('9.43 \\times 10^{159}'), OldTexText('permutations')[0])
        new_perm_label.arrange(RIGHT)
        new_perm_label.match_height(perm_label)
        ...
        self.play(FadeOut(perm_label), FadeIn(new_perm_label), ReplacementTransform(dots, new_dots[-len(dots):]), ShowIncreasingSubsets(new_dots[:-len(dots)], run_time=2))
        self.add(new_dots)
        perm_label = new_perm_label
        dots = new_dots
        labels = VGroup()
        ...
        self.play(perm_label[0].replace, new_perm_label[0], {'stretch': True}, perm_label[0].set_opacity, 0, FadeOut(perm_label[1:3], RIGHT), MoveToTarget(perm_label[3]), Restore(new_perm_label[0]))
        ...
        self.play(MoveToTarget(new_dots), FadeOut(faders))
        dots = new_dots
        for x in range(2):
...
class IsItUseful(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('Is any of\\\\this useful?', index=2, target_mode='sassy', added_anims=[self.teacher.change, 'guilty'])
...
class SolutionsToPolynomials(Scene):
    ...
    def construct(self):
        ...
        quintic = OldTex('x^5 - x - 1', '=', '(x - r_0)', '(x - r_1)', '(x - r_2)', '(x - r_3)', '(x - r_4)', tex_to_color_map={f'r_{i}': colors[i] for i in range(5)})
        ...
        def get_root_dots(roots, plane=plane, colors=colors):
            return VGroup(*[Dot(plane.n2p(root), radius=0.1, color=color).set_stroke(BLACK, 2, background=True) for root, color in zip(roots, colors)])
        root_dots = get_root_dots([1.1673, 0.181232 + 1.08395j, 0.181232 - 1.08395j, -0.764884 + 0.352472j, -0.764884 - 0.352472j])
        self.add(quintic)
        self.add(plane)
        self.play(LaggedStart(*[ReplacementTransform(rs.copy(), rd) for rs, rd in zip(root_syms, root_dots)], run_time=3, lag_ratio=0.3))
        self.wait()
        ...
        for x in range(5):
            perm = list(range(5))
            random.shuffle(perm)
            self.play(*[permutation_animation(mob, perm, arc=30 * DEGREES, lag_factor=0.5) for mob in [root_syms, root_dots]])
            self.wait(0.5)
        self.play(Restore(root_syms, path_arc=60 * DEGREES), Restore(root_dots, path_arc=60 * DEGREES))
        quadratic_lhs = OldTex('x^2 - x - 1')
        quadratic_lhs.match_height(quintic[0])
        quadratic_lhs.move_to(quintic[0], RIGHT)
        self.play(FadeOut(quintic[0], UP), FadeIn(quadratic_lhs, DOWN), FadeOut(quintic[8:], UP), MaintainPositionRelativeTo(root_dots, plane), UpdateFromAlphaFunc(root_dots, lambda m, a: m.set_opacity(1 - a)), plane.to_edge, LEFT)
        self.remove(root_dots)
        root_dots.set_opacity(1)
        root_dots.save_state()
        quad_root_dots = get_root_dots([(1 + u * np.sqrt(5)) / 2 for u in [-1, 1]])
        self.play(LaggedStart(*[ReplacementTransform(root_sym.copy(), root_dot) for root_dot, root_sym in zip(quad_root_dots, root_syms)]))
        self.wait()
        quadratic_formula = OldTex('{-b \\pm \\sqrt{\\,b^2 - 4ac} \\over 2a}')
        ...
        cubic_root_dots = get_root_dots([1.3247, -0.66236 + 0.56228j, -0.66236 - 0.56228j])
        cubic_formula = OldTex('\\sqrt[3]{-\\frac{q}{2}+\\sqrt{\\frac{q^{2}}{4}+\\frac{p^{3}}{27}}}+\\sqrt[3]{-\\frac{q}{2}-\\sqrt{\\frac{q^{2}}{4}+\\frac{p^{3}}{27}}}')
        ...
        self.play(ReplacementTransform(quad_root_dots, cubic_root_dots), FadeIn(quintic[8:11], DOWN), FadeIn(cubic_lhs, DOWN), FadeOut(quadratic_lhs, UP))
        self.play(LaggedStart(FadeOut(quadratic_formula, 2 * RIGHT), FadeOut(quad_form_name, 2 * RIGHT)), LaggedStart(FadeIn(cubic_formula, 2 * LEFT), FadeIn(cubic_form_name, 2 * LEFT)))
        self.wait()
        quartic_lhs = OldTex('x^4 - x - 1')
        quartic_lhs.replace(quadratic_lhs)
        quartic_root_dots = get_root_dots([1.2207, -0.72449, -0.24813 + 1.034j, -0.24813 - 1.034j])
        quartic_formula = OldTex('\n            r_{i}&=-\\frac{b}{4 a}-S \\pm \\frac{1}{2} \\sqrt{-4 S^{2}-2 p \\pm \\frac{q}{S}}\\\\\\\\\n            &\\text{Where}\\\\\\\\\n            p&=\\frac{8 a c-3 b^{2}}{8 a^{2}} \\qquad \\qquad\n            q=\\frac{b^{3}-4 a b c+8 a^{2} d}{8 a^{3}}\\\\\\\\\n            S&=\\frac{1}{2} \\sqrt{-\\frac{2}{3} p+\\frac{1}{3 a}\\left(Q+\\frac{\\Delta_{0}}{Q}\\right)}\\\\\\\\\n            Q&=\\sqrt[3]{\\frac{\\Delta_{1}+\\sqrt{\\Delta_{1}^{2}-4 \\Delta_{0}^{3}}}{2}}\\\\\\\\\n            \\Delta_{0}&=c^{2}-3 b d+12 a e\\\\\\\\\n            \\Delta_{1}&=2 c^{3}-9 b c d+27 b^{2} e+27 a d^{2}-72 a c e\\\\\\\\\n        ')
        quartic_formula.set_height(6)
        quartic_formula.next_to(plane, RIGHT, LARGE_BUFF)
        self.play(FadeOut(cubic_formula, 2 * RIGHT), FadeOut(cubic_form_name, 2 * RIGHT), ReplacementTransform(cubic_root_dots, quartic_root_dots), FadeIn(quintic[11:14], DOWN), FadeIn(quartic_lhs, DOWN), FadeOut(cubic_lhs, UP))
        self.play(Write(quartic_formula, run_time=3))
        self.wait(2)
        self.play(ReplacementTransform(quartic_root_dots, root_dots), FadeIn(quintic[0], DOWN), FadeOut(quartic_lhs, UP), FadeIn(quintic[14:], DOWN), FadeOut(quartic_formula, 0.1 * DOWN, lag_ratio=0.01))
        mathy = PiCreature(color=GREY)
        mathy.flip()
        mathy.next_to(quintic, DOWN, buff=1.5)
        mathy.to_edge(RIGHT)
        self.play(VFadeIn(mathy), mathy.change, 'confused', root_syms)
        ...
        self.play(FadeIn(s5_name), ShowCreation(s5_lines, lag_ratio=0.5))
        ...
        for x in range(5):
            ...
            self.play(*[permutation_animation(mob, perm, arc=30 * DEGREES, lag_factor=0.5) for mob in [root_syms, root_dots]])
            self.wait(0.5)
        self.play(VFadeOut(s5_lines), Restore(root_syms), Restore(root_dots), FadeOut(mathy))
        r0_value = OldTex('r_0', '=', '1.1673\\dots')
        r0_value.set_color_by_tex('r_0', BLUE)
        r0_value.scale(1.5)
        r0_value.next_to(plane, RIGHT, MED_LARGE_BUFF)
        r0_value.shift(DOWN)
        self.play(LaggedStart(*[AnimationGroup(ShowCreationThenFadeAround(dot), ShowCreationThenFadeAround(sym)) for sym, dot in zip(root_syms, root_dots)], lag_ratio=0.3, run_time=3))
        ...
        cross = Cross(arrow)
        cross.stretch(0.5, 1)
        self.play(FadeIn(symbols, lag_ratio=0.2, run_time=1.5), ShowCreation(symbols_rect))
        self.wait()
...
class MentionGroupsInPhysics(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(nt_label, DOWN), FadeIn(noether, DOWN), FadeOut(VGroup(self.teacher.bubble, self.teacher.bubble.content)), self.teacher.change, 'raise_right_hand')
        ...
        rule = VGroup(OldTexText('Conservation law', color=BLUE), OldTex('\\Leftrightarrow'), OldTexText('Symmetry', color=YELLOW))
        rule.set_height(0.5)
        rule.arrange(RIGHT)
        rule.next_to(nt_label.target, DOWN, MED_LARGE_BUFF)
        self.look_at(nt_label.target, added_anims=[MoveToTarget(nt_label)])
        self.play(self.teacher.change, 'happy', rule, *[FadeIn(part, rule.get_center() - part.get_center()) for part in rule])
        self.wait(2)
        examples = VGroup(OldTexText('Momentum', ' $\\Leftrightarrow$ ', 'Translation in space'), OldTexText('Energy', ' $\\Leftrightarrow$ ', 'Translation in time'))
        ...
        self.play(self.teacher.change, 'raise_right_hand', FadeIn(examples[0], UP), self.change_students('confused', 'erm', 'pondering'))
        self.look_at(rule)
        self.wait()
...
class NotGroupsGroupAction(Scene):
    ...
    def construct(self):
        words = VGroup(OldTexText('Group'), OldTexText('Group', ' action'))
        words.scale(2)
        ...
        self.wait()
        self.play(ShowCreation(cross))
        self.play(TransformFromCopy(group[0], group_action[0]), Animation(cross.copy(), remover=True))
...
class ElementsAsAbstractions(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        s_copies.scale(0.3)
        bubble = self.students[0].get_bubble(s_copies, width=5, height=4)
        self.play(self.students[0].change, 'pondering', Write(bubble), FadeIn(bubble.content, lag_ratio=0.3))
        self.wait(2)
        numeral = Integer(3)
        ...
        self.play(Write(numeral), self.students[0].change, 'happy', numeral)
        self.look_at(numeral)
        self.wait(2)
        self.camera.light_source.set_x(0)
        snowflake = get_snowflake()
        rot_icon = get_rot_icon(60, snowflake)
        inclusion = VGroup(rot_icon, OldTex('\\in').scale(2), OldTex('D_6').scale(2))
        inclusion.arrange(RIGHT)
        inclusion.next_to(self.hold_up_spot, UL, MED_LARGE_BUFF)
        self.play(LaggedStart(FadeOut(self.teacher.bubble), FadeOut(self.teacher.bubble.content), FadeOut(bubble), FadeOut(numeral), FadeIn(inclusion, DOWN)), self.teacher.change, 'raise_right_hand')
        ...
        self.look_at(rot_icon.target, added_anims=[MoveToTarget(rot_icon)])
        self.look_at(r_sym, added_anims=[Write(r_sym)])
        self.play_all_student_changes('confused', look_at=r_sym)
        self.wait(2)
        inclusion.remove(rot_icon)
        inclusion.add(r_sym)
        numeral.move_to(self.hold_up_spot, DOWN)
        self.play(inclusion.to_edge, LEFT, inclusion.set_color, GREY_B, Write(numeral), self.change_students(*3 * ['pondering'], look_at=numeral), self.teacher.change, 'tease', numeral)
        self.wait(2)
        add = OldTex('3', '+', '5', '=', '8')
        ...
        self.remove(numeral)
        self.play(LaggedStart(*[TransformFromCopy(numeral[0], form[0]) for form in ops]), *[ApplyMethod(pi.look_at, ops) for pi in self.pi_creatures])
        ...
        self.play(FadeOut(ops[0][0::2], UP), FadeIn(sum_dots, DOWN))
        self.play(FadeOut(VGroup(*ops[1][:3], ops[1][4]), UP), FadeIn(mult_dots, DOWN), self.change_students(*3 * ['erm'], look_at=mult_dots))
        self.wait(4)
        self.play(LaggedStart(*[ApplyMethod(mob.scale, 0, remover=True) for mob in [*sum_dots, *mult_dots]]), LaggedStart(*[FadeIn(mob) for mob in [*ops[0][0::2], *ops[1][:3], ops[1][4]]]), self.change_students(*3 * ['happy']))
        self.wait(2)
        flip_icon = get_flip_icon(0, snowflake)
        rhs_icon = get_flip_icon(30 * DEGREES, snowflake)
        rot_icon.generate_target()
        group_prod = VGroup(rot_icon.target, OldTex('\\times').scale(2), flip_icon, OldTex('=').scale(2), rhs_icon)
        ...
        self.play(FadeOut(ops, RIGHT), FadeOut(inclusion, LEFT), MoveToTarget(rot_icon), LaggedStartMap(FadeIn, group_prod[1:], lag_ratio=0.5, run_time=2), self.change_students('sassy', 'erm', 'confused', look_at=group_prod), self.teacher.change, 'raise_right_hand')
        ...
        self.play(FadeIn(sn_nums, lag_ratio=0.1), self.change_students(*3 * ['pondering'], look_at=snowflake))
        self.add(*sn_nums)
        self.play(Rotate(snowflake, PI, RIGHT), ShowCreationThenFadeAround(flip_icon))
        self.wait()
        self.play(Rotate(snowflake, 60 * DEGREES), ShowCreationThenFadeAround(rot_icon))
        self.wait()
...
class MultiplicationTable(Scene):
    ...
    def construct(self):
        ...
        icons = VGroup(*[get_rot_icon(deg, square) for deg in [0, 90, 180, -90]] + [get_flip_icon(angle, square, opacity=1) for angle in np.arange(0, PI, PI / 4)])
        ...
        r2 = pmult(r, r)
        r3 = pmult(r2, r)
        perms = [list(range(4)), r, r2, r3, s, pmult(r, s), pmult(r2, s), pmult(r3, s)]
        ...
        for sorted_index, icon in enumerate(sorted_icons):
            ...
            i = n // 8
            j = n % 8
            rects = VGroup(SurroundingRectangle(left_icons[i]), SurroundingRectangle(top_icons[j]), grid[n].copy().set_fill(GREEN_E, 0.5))
        ...
        self.wait(2)
        symbols = VGroup(OldTex('1'), OldTex('r'), OldTex('r^2'), OldTex('r^3'), OldTex('s'), OldTex('rs'), OldTex('r^2 s'), OldTex('r^3 s'))
        ...
        self.play(LaggedStart(*[ApplyMethod(mob.scale, 0, remover=True) for mob in [*left_icons, *top_icons, *table_icons]]), LaggedStart(*[GrowFromCenter(mob) for mob in [*left_symbols, *top_symbols, *table_symbols]]))
        self.wait()
        last_rects = VGroup()
        for x in range(10):
            ...
            i = n // 8
            j = n % 8
            rects = VGroup(SurroundingRectangle(left_symbols[i]), SurroundingRectangle(top_symbols[j]), grid[n].copy().set_stroke(YELLOW, 4).set_fill(YELLOW, 0.5))
            self.add(rects, *self.mobjects)
            self.play(FadeOut(last_rects), FadeIn(rects))
            self.wait(2)
            last_rects = rects
...
class UsualMultiplicationTable(Scene):
    ...
    def construct(self):
        grid = VGroup(*[VGroup(*[Square() for x in range(4)]).arrange(RIGHT, buff=0) for y in range(4)]).arrange(DOWN, buff=0)
        grid.set_height(6)
        grid.to_edge(DOWN, buff=0.5)
        grid.set_fill(GREY_E, 1)
        dots = VGroup(*[Tex('\\dots').scale(2).next_to(row, RIGHT) for row in grid[:-1]], *[Tex('\\vdots').scale(2).next_to(square, DOWN) for square in grid[-1][:-1]], OldTex('\\ddots').scale(2).next_to(grid[-1][-1], DR))
        self.add(grid)
        table_dots = VGroup()
        ...
        self.play(LaggedStart(*[AnimationGroup(Transform(ls_copies[i].copy(), table_syms[4 * i + j].copy(), remover=True), Transform(ts_copies[j].copy(), table_syms[4 * i + j].copy(), remover=True)) for i, j in it.product(range(4), range(4))], lag_ratio=0.3))
        self.add(table_syms)
        self.wait()
        self.play(FadeOut(sym_groups), FadeIn(dot_groups))
        self.wait()
        last_rects = VGroup()
        ...
        for n in ns:
            i = n // 4
            j = n % 4
            rects = VGroup(SurroundingRectangle(left_dots[i]), SurroundingRectangle(top_dots[j]), grid[i][j].copy().set_fill(YELLOW, 0.5))
            rects.set_stroke(YELLOW, 4)
            self.play(FadeIn(rects), FadeOut(last_rects), run_time=0.5)
            self.wait()
            last_rects = rects
        self.play(FadeOut(last_rects))
        self.play(dot_groups.fade, 0.8, FadeIn(sym_groups))
        self.wait()
        frame = self.camera.frame
        frame.generate_target()
        frame.target.set_x(grid.get_right()[0])
        frame.target.scale(1.1)
        benefit = VGroup(OldTexText('Abstraction').scale(1.5), Vector(DOWN), OldTexText('Less cumbersome').scale(1.5))
...
class MentionTheMonster(Scene):
    ...
    def construct(self):
        ...
        self.play(ApplyMethod(monster.next_to, size_label, LEFT, LARGE_BUFF, run_time=2), ShowIncreasingSubsets(size_label, run_time=6))
        self.play(blink_monster(monster))
        self.wait()
...
class FrustratedAtGroups(TeacherStudentsScene):
    ...
    def construct(self):
        formula = OldTex('|G|=|Z(G)|+\\sum i\\left[G: C_{G}\\left(x_{i}\\right)\\right]')
        formula.move_to(self.hold_up_spot, DOWN)
        formula.shift(0.5 * UL)
        self.play(self.teacher.change, 'raise_right_hand', FadeIn(formula, DOWN))
...
class AnalogyWithCounts(Scene):
    ...
    def construct(self):
        ...
        line.scale(0.5)
        diagrams = VGroup(*[VGroup(mob1, line.copy(), mob2) for mob1, mob2 in [(OldTexText('Groups'), OldTexText('Symmetry actions')), (OldTex('D_6'), get_snowflake(height=1)), (OldTexText('Numbers'), OldTexText('Counts')), (OldTex('9').scale(1.5), VGroup(*[Dot() for x in range(9)]).arrange_in_grid(buff=SMALL_BUFF))]])
        ...
        self.play(Write(diagrams[0][1]), FadeIn(diagrams[0][2], 2 * UP))
        self.wait()
        self.play(*[AnimationGroup(ReplacementTransform(m2.copy().replace(m1, stretch=True).set_opacity(0), m2), Transform(m1.copy(), m1.copy().replace(m2, stretch=True).set_opacity(0), remover=True)) for m1, m2 in zip(diagrams[0], diagrams[2])])
        self.wait()
        self.play(FadeOut(diagrams[0]), FadeIn(diagrams[1]))
        flake = diagrams[1][2]
        self.add(flake)
        self.play(FadeOut(diagrams[2]), FadeIn(diagrams[3]), Rotate(flake, 60 * DEGREES))
        self.play(Rotate(flake, PI, UP))
        self.play(Rotate(flake, -120 * DEGREES))
        self.play(Rotate(flake, PI, RIGHT))
        self.play(Rotate(flake, 120 * DEGREES))
        self.play(Rotate(flake, PI, UP))
        self.play(VFadeOut(diagrams[1]), Rotate(flake, 180 * DEGREES), FadeIn(diagrams[0]), FadeOut(diagrams[3]), FadeIn(diagrams[2]))
        self.wait(2)

class ButWhy(TeacherStudentsScene):

    def construct(self):
        self.student_says('But, why?', target_mode='maybe', added_anims=[LaggedStart(ApplyMethod(self.teacher.change, 'guilty'), ApplyMethod(self.students[0].change, 'confused'), ApplyMethod(self.students[1].change, 'sassy'), lag_ratio=0.5)])
        self.wait(3)
...
class Isomorphism(Scene):
    ...
    def construct(self):
        ...
        def get_rot_icon(angle=0, axis=OUT, frame=frame):
            cube = get_glassy_cube(frame)
            cube.set_height(1)
            arc_arrows = VGroup(*[Arrow(u * RIGHT, u * rotate_vector(RIGHT, 160 * DEGREES), buff=0, path_arc=160 * DEGREES, width=0.05) for u in [1, -1]])
            ...
            return rot_icon
        rot_icon_equation = Group(get_rot_icon(90 * DEGREES, UP), OldTex('\\times').scale(2), get_rot_icon(90 * DEGREES, RIGHT), OldTex('=').scale(2), get_rot_icon(0, OUT))
        ...
        dots.set_color(GREY_B)
        dots.arrange(RIGHT)
        perms = [[1, 0, 3, 2], [3, 2, 1, 0], [2, 3, 0, 1]]
        perm_terms = VGroup()
        for perm in perms:
            perm_term = VGroup(dots.copy(), get_permutation_arrows(dots, perm))
            perm_term.perm = perm
            perm_terms.add(perm_term)
        perm_equation = VGroup(perm_terms[0], OldTex('\\times').scale(2), perm_terms[1], OldTex('=').scale(2), perm_terms[2])
        ...
        for rot_icon, line, perm_term in zip(rot_icons, bij_lines, perm_terms):
            self.play(GrowFromPoint(line, line.get_top()), FadeIn(perm_term, 2 * UP))
        self.wait()
        self.play(Write(VGroup(*rot_icon_equation[1::2], *perm_equation[1::2])))
        self.wait(2)
        rot_anims = [AnimationGroup(Rotate(rot_icon[2], PI, axis=rot_icon[1].get_vector()), ShowCreationThenFadeAround(rot_icon[-1])) for rot_icon in rot_icons]
...
class AskAboutCubeDiagonals(QuadrupletShufflings):
    ...
    def construct(self):
        ...
        diagonals = Group(*[Line3D(vect + OUT, -vect - OUT, color=color) for color, vect in zip(colors, [UL, UR, DR, DL])])
        ...
        for diag in diagonals:
            globals()['diag'] = diag
            Group(*[cds[np.argmin([get_norm(cd.get_center() - diag.get_points()[i]) for cd in cds])] for i in [0, -1]]).match_color(diag)
        ...
        for quat in self.get_quaternions():
            ...
            self.play(Rotate(cube, angle, axis=axis), permutation_animation(diag_markers, perm, lag_factor=0.1), run_time=2)
...
class S4WithMultipleChildren(Scene):
    ...
    def construct(self):
        ...
        for trip, rect in zip(triplets, s_rects):
            ...
            trip.move_to(rect)
        self.play(FadeOut(s4_label, UP), FadeIn(three, DOWN), LaggedStartMap(FadeIn, pis, lag_ratio=0.5, run_time=1))
        for trip in triplets[1:]:
            self.play(FadeIn(trip, lag_ratio=0.5))
        self.play(Blink(pis[0]))
        self.play(Blink(pis[2]))
        self.play(FadeOut(three, DOWN), FadeIn(s4_label, UP), FadeOut(triplets, lag_ratio=0.2, run_time=2))
        self.wait()
...
class GroupsBeyondActions(Scene):
    ...
    def construct(self):
        ...
        line.add_updater(lambda m: m.put_start_and_end_on(groups.get_bottom() + 0.3 * DOWN, sym_acts.get_top() + 0.3 * UP))
        ...
        self.wait()
        self.play(sym_acts.to_edge, LEFT, LARGE_BUFF, FadeIn(others, 2 * UL), ShowCreation(others_line), run_time=2)
        self.wait()
...
class AskAboutAllTheGroups(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.teacher_says('Now you can\\\\ask something\\\\more sophisticated.', target_mode='hooray', added_anims=[MoveToTarget(question, run_time=2), self.change_students('erm', 'pondering', 'pondering', look_at=question.target)])
        self.wait(2)
        self.play(RemovePiCreatureBubble(self.teacher, target_mode='tease', look_at=question.get_right()), question.set_x, 0)
        caveat = OldTexText('up to \\emph{isomorphism}')
        caveat.next_to(question, DOWN)
        caveat.set_color(YELLOW)
        self.play(Write(caveat, run_time=1), question[1].next_to, caveat[0][-1], RIGHT, SMALL_BUFF, DOWN, question[1].match_color, caveat, self.change_students(*3 * ['thinking'], look_at=question))
        question.add(caveat)
        self.wait(2)
        ...
        self.look_at(cross, added_anims=[ShowCreation(cross), self.change_students('erm', 'sassy', 'hesitant')])
        self.wait()
        ...
        self.play(*[ReplacementTransform(sym_question[i], abs_question[j], path_arc=10 * DEGREES) for i, j in [(0, 0), (1, 4), (2, 2), (3, 5)]], FadeOut(cross), self.change_students('happy', 'thinking', 'tease'))
        self.play(self.teacher.change, 'speaking', abs_question, Write(new_words))
        self.look_at(abs_question)
        self.wait(2)
        suggestions = VGroup(*[OldTexText('Some ', word, '?', tex_to_color_map={word: color}) for word, color in [('formula', RED), ('procedure', MAROON_B), ('algorithm', PINK)]])
        for words in suggestions:
            words.move_to(abs_question, UP)
            words.to_edge(LEFT, buff=LARGE_BUFF)
        self.play(FadeIn(suggestions[0], DOWN), self.change_students(*3 * ['pondering'], look_at=suggestions), self.teacher.change, 'happy')
        self.wait()
        for words1, words2 in zip(suggestions, suggestions[1:]):
            self.play(FadeOut(words1[1], UP), FadeIn(words2[1], DOWN), ReplacementTransform(words1[2], words2[2]))
            self.remove(words1)
            self.add(words2)
            self.wait()
        self.wait(3)
        self.play(FadeOut(VGroup(question, abs_question, suggestions[-1]), 0.5 * DOWN, lag_ratio=0.02, run_time=2))
...
class ThisQuestionIsHard(Scene):
    ...
    def construct(self):
        ...
        line.shift(UP)
        arrows = VGroup(Arrow(line.n2p(0.5), line.n2p(0), fill_color=GREEN), Arrow(line.n2p(0.5), line.n2p(1), fill_color=RED))
        ...
        self.add(arrows)
        self.add(words)
        problems = VGroup(OldTex('1 + 1'), VGroup(OldTex('\\frac{2^{289}+1}{2^{17}+1}=2^{a_{1}}+\\ldots+2^{a_{k}}'), OldTex('a_1 < \\ldots < a_k'), OldTex('a_1, \\dots, a_k \\in \\mathds{Z}^+')), VGroup(OldTex('{a \\over b + c} + {b \\over c + a} + {c \\over b + c} = 4'), OldTex('a, b, c \\in \\mathds{Z}^+')), VGroup(OldTex('x^n + y^n = z^n'), OldTex('x, y, z \\in \\mathds{Z}')))
        ...
        self.wait()
        self.play(ShowCreation(ext_line), question.move_to, line.n2p(1.3), DOWN, ApplyMethod(frame.scale, 1.3, {'about_edge': LEFT}, run_time=2))
        self.wait()
...
class IntroduceSimpleGroups(Scene):
    ...
    def construct(self):
        ...
        self.wait(2)
        self.play(ShowCreation(child_lines[1]), FadeIn(fin_groups, 2 * UL), FadeIn(s_rects[1]))
        self.wait()
        self.add(s_rects, fin_groups)
        self.play(Uncreate(child_lines), FadeOut(inf_groups, 3 * LEFT), FadeOut(s_rects[0], 3 * LEFT), FadeOut(groups, 2 * UP), fin_groups.move_to, groups, s_rects[1].replace, bg_rect, s_rects[1].set_stroke, {'width': 0}, run_time=2)
        self.remove(s_rects, bg_rect)
        bg_rects = VGroup(*[Rectangle(height=FRAME_HEIGHT, width=FRAME_WIDTH / 3, fill_color=color, fill_opacity=1, stroke_width=0) for color in [GREY_D, GREY_E, BLACK]])
        bg_rects.arrange(RIGHT, buff=0)
        bg_rects.center()
        fin_groups.generate_target()
        titles = VGroup(OldTexText('Integers').scale(1.5), OldTexText('Molecules').scale(1.5), fin_groups.target)
        ...
        H2O = Group(O_atom.copy(), H_atom.copy().move_to([-0.45, 0.35, 0]), H_atom.copy().move_to([0.45, 0.35, 0]))
        trees = Group(VGroup(Integer(60), VGroup(*map(Integer, [2, 2, 3, 5]))), Group(H2O, Group(H_atom.copy(), H_atom.copy(), O_atom.copy())), VGroup(OldTex('S_4'), VGroup(*[OldTex('C_' + str(n), fill_color=RED_B if n == 2 else BLUE) for n in [2, 2, 2, 3]])))
        ...
        for title, sub_title, tree in zip(titles, sub_titles, trees):
            ...
            self.play(FadeIn(title), FadeIn(root))
            globals()['root'] = root
            self.play(ShowCreation(lines), LaggedStart(*[GrowFromPoint(child, root) for child in children]), FadeIn(sub_title))
            self.wait()
...
class SymmetriesOfCirleAndLine(Scene):
    ...
    def construct(self):
        ...
        for n in range(10):
            ...
            self.play(line.shift, x * RIGHT, Rotate(circle, -x / PI), run_time=2)
            self.wait()
        self.embed()
...
class QuinticImpliesCyclicDecomposition(Scene):
    ...
    def construct(self):
        ...
        title.scale(1.5)
        title.to_edge(UP)
        details = OldTexText('Solve ', '$a_5 x^5 + a_4 x^4 + a_3 x^3 + a_2 x^2 + a_1 x + a_0$\\\\', ' using only ', '+, -, $\\times$, $/$, and $\\sqrt[n]{\\quad}$')
        ...
        s5 = OldTex('S_5')
        prime_children = VGroup(OldTex('C_{p_1}'), OldTex('C_{p_2}'), OldTex('\\vdots'), OldTex('C_{p_n}'))
        prime_children.set_color(RED_B)
        real_children = VGroup(OldTex('A_5'), OldTex('C_2'))
        ...
        VGroup(s5, prime_children.lines, prime_children, real_children.lines, real_children).next_to(implies, DOWN)
        implies.save_state()
        implies.stretch(0, 1, about_edge=UP)
        self.play(Restore(implies), GrowFromPoint(s5, implies.get_top()))
        self.play(LaggedStartMap(ShowCreation, prime_children.lines, lag_ratio=0.3), LaggedStartMap(FadeIn, prime_children, lambda m: (m, s5.get_right() - m.get_center()), lag_ratio=0.3))
        self.wait()
        self.play(FadeOut(prime_children, RIGHT), *[ApplyMethod(line.scale, 0, {'about_point': line.get_end()}, remover=True) for line in prime_children.lines], LaggedStartMap(ShowCreation, real_children.lines, lag_ratio=0.3), LaggedStartMap(FadeIn, real_children, lambda m: (m, s5.get_right() - m.get_center()), lag_ratio=0.3))
        ...
        self.play(Rotate(implies, PI), VFadeIn(title_rect), VFadeIn(not_exists), full_title.shift, 0.5 * RIGHT)

class CommentOnNontrivialFactFromGroupDecomposition(TeacherStudentsScene):

    def construct(self):
        self.student_says("I...don't\\\\get it.", target_mode='confused', index=2, look_at=self.screen, added_anims=[self.teacher.change, 'guilty'])
        ...
        self.teacher_holds_up(fp_words, added_anims=[RemovePiCreatureBubble(self.students[2])])
        ...
        self.play(FadeIn(as_words, RIGHT), Write(implies), self.change_students(*3 * ['pondering'], look_at=as_words))
        self.wait(5)
...
class TwoStepsToAllFiniteGroups(Scene):
    ...
    def construct(self):
        ...
        title.to_edge(UP)
        steps = VGroup(OldTexText('1. ', 'Find all the ', 'simple groups', '.'), OldTexText('2. ', 'Find all the ', 'ways to\\\\', 'combine ', 'simple groups', '.'))
        ...
        self.add(title)
        self.wait()
        self.play(LaggedStartMap(FadeIn, VGroup(steps[0][0], steps[1][0]), lambda m: (m, RIGHT), lag_ratio=0.4))
        self.wait()
        self.play(FadeIn(steps[0][1:], lag_ratio=0.1))
        self.wait()
        self.play(TransformFromCopy(steps[0][1], steps[1][1]), TransformFromCopy(steps[0][2], steps[1][4]), FadeIn(VGroup(*[steps[1][i] for i in (2, 3, 5)])))
        self.wait()
        table = VGroup(*[VGroup(*[Square() for x in range(n)]).arrange(DOWN, buff=0) for n in [7, 6, *[4] * 10, *[6] * 5, 7]])
        table.arrange(RIGHT, buff=0, aligned_edge=DOWN)
        table.set_width(4)
        table.to_edge(RIGHT)
        table.match_y(steps[0])
        table.set_stroke(GREY_A, 2)
        table_arrow = Arrow(steps[0].get_right(), table.get_left(), buff=0.5)
        self.play(GrowArrow(table_arrow), FadeIn(table, lag_ratio=0.1, run_time=3))
        self.wait()
        ...
        self.play(GrowArrow(chem_arrow), Write(chem_words))
        self.wait()
        ...
        check.shift(RIGHT)
        self.play(frame.scale, 1.1, bottom_group.shift, 0.5 * DOWN, bottom_group.set_opacity, 0.5, ShowCreation(top_rect), FadeOut(title, UP))
        ...
        self.wait()
        stats = VGroup(OldTexText('1955-2004'), OldTexText('$10{,}000+$ pages'), OldTexText("100's of mathematicians"), OldTexText('Plenty of computation'))
...
class ClassificationOfSimpleGroups(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(families_title), FadeIn(families_grid, lag_ratio=0.1, run_time=4))
        self.wait()
        ...
        faders = VGroup(*[column[:n] for column, n in zip(families_grid.target, [0, 1, *[3] * 10, *[1] * 5, 0])])
        ...
        self.play(FadeIn(pre_sporadics_title, DOWN), ShowIncreasingSubsets(sporadics, run_time=3))
        self.wait()
        self.play(ReplacementTransform(pre_sporadics_title[0], sporadics_title[0]), FadeOut(pre_sporadics_title[1], UP), FadeIn(sporadics_title[1], DOWN))
        self.wait()
        families_grid.save_state()
        ...
        self.play(MoveToTarget(column), FadeOut(families_grid[1:], lag_ratio=0.1), FadeOut(families_title), FadeOut(class_title, UP), FadeOut(sporadics, 2 * RIGHT), FadeOut(sporadics_title, 2 * RIGHT))
        ...
        self.play(MoveToTarget(c5), DrawBorderThenFill(pentagon, run_time=1))
        ...
        prepare_column(column)
        self.play(MoveToTarget(column), FadeOut(families_grid[0]), FadeOut(families_grid[2:], lag_ratio=0.1))
        ...
        self.play(FadeIn(dots, lag_ratio=0.1), MoveToTarget(a5))
        for x in range(5):
            perm = list(range(5))
            swaps = 1
            while swaps % 2 == 1:
                ...
                for i, j in it.combinations(perm, 2):
                    if j < i:
                        swaps += 1
            arrows = get_permutation_arrows(dots, perm)
            self.play(FadeIn(arrows), permutation_animation(dots, perm, lag_factor=0.1))
            self.play(FadeOut(arrows))
        self.wait()
        self.play(FadeOut(dots, lag_ratio=0.1), Restore(a5))
        self.wait()
        a_names.generate_target()
        a_names.target.replace(families_grid.saved_state[1])
        a_names.target.scale(0.9)
        a_names.target.fade(1)
        self.play(Restore(families_grid), MoveToTarget(a_names, remover=True), FadeIn(families_title), FadeIn(class_title))
        ...
        self.play(ShowIncreasingSubsets(sporadics), FadeIn(sporadics_title))
        self.wait()
        ...
        self.play(sporadics_title.scale, 1.25, sporadics_title.center, sporadics_title.to_edge, UP, MoveToTarget(sporadics), FadeOut(families_grid, LEFT), FadeOut(families_title, LEFT), FadeOut(class_title, UP))
        monster = get_monster()
        ...
        self.play(Write(monster_name), GrowArrow(monster_arrow))
        self.wait()
        self.play(monster_name.shift, 0.6 * UP, ShowIncreasingSubsets(size_label, run_time=2))
        self.wait()
        ...
        self.play(MoveToTarget(full_monster_label), TransformFromCopy(monster_arrow, baby_arrow), FadeIn(baby_name, DOWN), sporadics[1].set_fill, {'opacity': 0}, FadeIn(baby_monster), FadeOut(sporadics_title, UP))
        self.wait()
        self.play(baby_name.shift, 0.6 * UP, ShowIncreasingSubsets(baby_size_label, run_time=2))
        self.wait()
        top_20 = sporadics[:20]
        top_20.generate_target()
        top_20.target.shift(2 * UP)
        top_20.target[1:].set_fill(GREEN, 0.8)
        self.play(MoveToTarget(top_20), MaintainPositionRelativeTo(monster, sporadics[0]), MaintainPositionRelativeTo(baby_monster, sporadics[1]), UpdateFromAlphaFunc(baby_monster, lambda m, a: m.set_opacity(1 - a), remover=True), ApplyMethod(baby_arrow.scale, 0, {'about_point': baby_arrow.get_start()}, remover=True), FadeOut(baby_size_label, UP), FadeOut(baby_name, UP), full_monster_label.set_fill, WHITE, 1)
        ...
        self.play(MoveToTarget(monster_name), FadeOut(size_label, DOWN), FadeIn(happy_family_name, UP))
        self.wait()
        ...
        self.play(FadeIn(pariahs_name, UP), pariahs.set_fill, YELLOW, 0.7)
        self.wait()

class ImSorryWhat(TeacherStudentsScene):

    def construct(self):
        self.student_says("I'm sorry, what?!", target_mode='sassy', look_at=self.screen)
...
class TellMeTheresAChildrensBook(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says("Please tell me\\\\there's a children's\\\\book about this!", target_mode='surprised', added_anims=[self.teacher.change, 'tease'])
...
class AskWhatTheMonsterActsOn(Scene):
    ...
    def construct(self):
        question = OldTexText('The monster group describes the symmetries of ', '$\\underline{\\qquad\\qquad\\qquad}$')
        ...
        self.play(FadeIn(question, DOWN))
        self.wait()
        dim_words = VGroup(*[OldTexText('Something in\\\\', '{:,}'.format(n), ' dimensions', '?') for n in [2, 3, 4, 5, 196883]])
        ...
        self.play(Restore(final_words[0]), Restore(final_words[2]), FadeOut(penult[3]), ChangingDecimal(dim, lambda a: interpolate(5, 196883, a), run_time=8, rate_func=rush_from), UpdateFromAlphaFunc(Mobject(), lambda m, a, fw=final_words: fw.set_color(interpolate_color(WHITE, YELLOW, a)), remover=True))
        final_words.add(dim)
        ...
        self.play(MoveToTarget(monster), FadeOut(question, 3 * RIGHT), FadeOut(final_words, 2 * RIGHT), FadeIn(in_sym, 3 * LEFT))
        matrix = IntegerMatrix(np.random.randint(0, 2, size=(6, 6)), v_buff=0.8, h_buff=0.8)
        matrix.set_height(4)
        matrix.next_to(in_sym, LEFT, MED_LARGE_BUFF)
        mob_matrix = matrix.get_mob_matrix()
        groups_to_dots = [(mob_matrix[4, :4], OldTex('\\vdots')), (mob_matrix[:4, 4], OldTex('\\ldots')), (VGroup(mob_matrix[4, 4]), OldTex('\\ddots'))]
        for group, dots in groups_to_dots:
            for elem in group:
                dots_copy = dots.copy()
                dots_copy.move_to(elem)
                elem.set_submobjects(dots_copy)
        braces = VGroup(Brace(matrix.get_entries(), DOWN), Brace(matrix.get_entries(), LEFT))
        ...
        self.play(LaggedStartMap(GrowFromCenter, braces, lag_ratio=0.5, run_time=1), FadeIn(gigs_label, DOWN))
        self.play(blink_monster(monster))
        self.wait()
...
class MonsterQuotes(Scene):
    ...
    def construct(self):
        ...
        for image, quote, color in zip(images, quotes, [YELLOW, WHITE]):
            ...
            self.play(FadeIn(image, DOWN), FadeIn(quote, lag_ratio=0.1, run_time=3), *faders)
            self.wait(4)
...
class MonstrousMoonshine(Scene):
    ...
    def construct(self):
        ...
        timeline = NumberLine((decades[0], decades[-1], 1), big_tick_numbers=decades, tick_size=0.075, width=13)
        ...
        self.add(triangle)
        self.play(triangle.move_to, timeline.n2p(1978), DOWN, run_time=3)
        mckay = get_named_image('John McKay')
        mckay[0].flip()
        mckay.next_to(triangle, UP)
        self.play(FadeIn(mckay, DOWN))
        self.wait()
        theories = VGroup(OldTexText('Finite group theory'), OldTexText('Galois theory'))
        theories.arrange(RIGHT, buff=1.5)
        theories.next_to(mckay, RIGHT, LARGE_BUFF)
        theories_line, theories_arrow = [func(*[theory.get_top() for theory in theories], path_arc=-90 * DEGREES, buff=0.2).shift(0.25 * UP) for func in [Line, Arrow]]
        theories_line.set_stroke(BLUE_C, 6)
        theories_arrow.set_fill(BLUE_C)
        self.play(FadeIn(theories[0], LEFT))
        self.play(GrowFromPoint(theories[1], theories_line.get_start(), path_arc=-90 * DEGREES), ShowCreationThenFadeOut(theories_line, run_time=2), FadeIn(theories_arrow, rate_func=squish_rate_func(smooth, 0.3, 1), run_time=2))
        self.wait()
        theories_group = VGroup(theories, theories_arrow)
        j_func = OldTex('j(\\tau)  =q^{-1}+744+196{,}884 q+21{,}493{,}760 q^{2}+864{,}299{,}970 q^{3}+\\cdots\\\\', '\\big(q = e^{2\\pi i \\tau}\\big)', tex_to_color_map={'\\tau': TEAL, 'q': BLUE, '196{,}884': WHITE})
        j_func.scale(0.7)
        ...
        self.play(theories_group.scale, 0.5, {'about_edge': UL}, theories_group.to_edge, UP, Write(j_func))
        self.wait()
        self.play(ShowCreation(special_num_underline), special_num.set_color, YELLOW)
        self.wait()
        ...
        self.play(FadeOut(timeline, DOWN), FadeOut(triangle, DOWN), j_func.to_edge, DOWN, FadeOut(theories_group), FadeIn(conway, DOWN))
        self.play(Write(moonshine))
        self.wait()
        conjecture = OldTexText('Monstrous ', 'moonshine\\\\', ' conjecture')
        conjecture_icon = VGroup(get_monster().set_height(1), OldTex('\\leftrightarrow'), OldTex('j(\\tau)', tex_to_color_map={'\\tau': TEAL}))
        ...
        self.play(FadeIn(timeline, DOWN), FadeIn(triangle, DOWN), FadeOut(mckay, UP), Transform(moonshine, moonshine.copy().replace(conjecture[1], stretch=True).fade(1), remover=True), ReplacementTransform(conjecture[1].copy().replace(moonshine, stretch=True).fade(1), conjecture[1]), Write(conjecture[0::2]))
        self.play(FadeIn(conjecture_icon, lag_ratio=0.1), triangle.move_to, timeline.n2p(1979), DOWN)
        ...
        self.play(conjecture_group.scale, 0.5, {'about_edge': DOWN}, triangle.move_to, timeline.n2p(1992), DOWN, FadeOut(conway, RIGHT), FadeIn(borcherds, 2 * LEFT))
        self.wait(2)
        self.play(triangle.move_to, timeline.n2p(1998), DOWN, MaintainPositionRelativeTo(borcherds, triangle))
