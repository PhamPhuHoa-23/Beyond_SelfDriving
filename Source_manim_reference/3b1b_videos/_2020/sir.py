...
class Person(VGroup):
    ...
    def set_status(self, status, run_time=1):
        ...
        anims = [UpdateFromAlphaFunc(self.body, lambda m, a: m.set_color(interpolate_color(start_color, end_color, a)), run_time=run_time)]
        for anim in anims:
            self.push_anim(anim)
        self.status = status
    ...
    def add_infection_ring(self):
        self.infection_ring = Circle(radius=self.height / 2)
    ...
    def update_position(self, dt):
        ...
        if self.social_distance_factor > 0:
            ...
            self.body.set_stroke(self.social_distance_color, width=clip(sdct / min_dist - sdct, 0, self.max_social_distance_stroke_width), background=True)
    ...
    def update_infection_ring(self, dt):
        ...
        ring.set_height(interpolate(self.height, self.infection_radius, smooth(alpha)))
        ring.set_stroke(width=interpolate(0, 5, there_and_back(alpha)), opacity=min([min([ring_time, 1]), min([self.infection_end_time + 1 - self.time, 1])]))
        return self
...
class PiPerson(Person):
    CONFIG = {'mode_map': {'S': 'guilty', 'I': 'sick', 'R': 'tease'}}
    ...
    def set_status(self, status, run_time=1):
        ...
        anims = [UpdateFromAlphaFunc(self.body, update, run_time=run_time)]
        for anim in anims:
            self.push_anim(anim)
        return self

class SIRSimulation(VGroup):
    CONFIG = {'n_cities': 1, 'city_population': 100, 'box_size': 7, 'person_type': PiPerson, 'person_config': {'height': 0.2, 'infection_radius': 0.6, 'gravity_strength': 1, 'wander_step_size': 1}, 'p_infection_per_day': 0.2, 'infection_time': 5, 'travel_rate': 0, 'limit_social_distancing_to_infectious': False}
        ...
        if self.travel_rate > 0:
            ...
            for person in self.people:
                if random.random() < self.travel_rate * dt:
                    ...
                    anim = UpdateFromAlphaFunc(person, lambda m, a: m.move_to(path_func(m.old_center, m.new_center, a)), run_time=1)
...
class SIRGraph(VGroup):
    CONFIG = {'color_map': COLOR_MAP, 'height': 7, 'width': 5, 'update_frequency': 0.5, 'include_braces': False}
    ...
    def add_axes(self):
        axes = Axes(y_min=0, y_max=1, y_axis_config={'tick_frequency': 0.1}, x_min=0, x_max=1, axis_config={'include_tip': False})
        origin = axes.c2p(0, 0)
    ...
    def get_graph(self, data):
        ...
        r_points = [axes.c2p(0, 1), axes.c2p(1, 1), *s_points[::-1], axes.c2p(0, 1)]
        s_points.extend([*i_points[::-1], s_points[0]])
        i_points.extend([axes.c2p(1, 0), axes.c2p(0, 0), i_points[0]])
    ...
    def add_v_line(self, line_time=None, color=YELLOW, stroke_width=3):
        if line_time is None:
            line_time = self.time
        axes = self.axes
        v_line = Line(axes.c2p(1, 0), axes.c2p(1, 1), stroke_color=color, stroke_width=stroke_width)
        v_line.add_updater(lambda m: m.move_to(axes.c2p(line_time / max(self.time, 1e-06), 0), DOWN))
        self.add(v_line)

class GraphBraces(VGroup):
    CONFIG = {'update_frequency': 0.5}
    ...
    def __init__(self, graph, simulation, **kwargs):
        ...
        self.labels = VGroup(OldTexText('Susceptible', color=COLOR_MAP['S']), OldTexText('Infectious', color=COLOR_MAP['I']), OldTexText('Removed', color=COLOR_MAP['R']))
    ...
    def update_braces(self):
        ...
        for i, y1, y2 in zip([1, 0, 2], ys, ys[1:]):
            lines[i].set_points_as_corners([axes.c2p(1, y1), axes.c2p(1, y2)])
...
class ValueSlider(NumberLine):
    CONFIG = {'x_min': 0, 'x_max': 1, 'tick_frequency': 0.1, 'big_tick_numbers': [], 'numbers_to_show': np.linspace(0, 1, 6), 'decimal_number_config': {'num_decimal_places': 1}, 'stroke_width': 5, 'width': 8, 'marker_color': BLUE}
...
class Test(Scene):
    ...
    def construct(self):
        ...
        self.add(person)
        self.wait()
        self.play(UpdateFromAlphaFunc(person, lambda m, a: m.move_to(path_func(m.old_center, m.new_center, a)), run_time=3, rate_func=there_and_back))
        self.wait(3)
        self.wait(3)

class RunSimpleSimulation(Scene):
    CONFIG = {'simulation_config': {'person_type': PiPerson, 'n_cities': 1, 'city_population': 100, 'person_config': {'infection_radius': 0.75, 'social_distance_factor': 0, 'gravity_strength': 0.2, 'max_speed': 0.5}, 'travel_rate': 0, 'infection_time': 5}, 'graph_config': {'update_frequency': 1 / 15}, 'graph_height_to_frame_height': 0.5, 'graph_width_to_frame_height': 0.75, 'include_graph_braces': True}
    ...
    def add_R_label(self):
        label = VGroup(OldTex('R = '), DecimalNumber())
        label.arrange(RIGHT)
    ...
    def add_total_cases_label(self):
        label = VGroup(OldTexText('\\# Active cases = '), Integer(1))
        label.arrange(RIGHT)
    ...
    def add_graph(self):
        ...
        graph = SIRGraph(self.simulation, height=self.graph_height_to_frame_height * frame_height, width=self.graph_width_to_frame_height * frame_height, **self.graph_config)
...
class RunSimpleSimulationWithDots(RunSimpleSimulation):
    CONFIG = {'simulation_config': {'person_type': DotPerson}}

class LargerCity(RunSimpleSimulation):
    CONFIG = {'simulation_config': {'person_type': DotPerson, 'city_population': 1000, 'person_config': {'infection_radius': 0.25, 'social_distance_factor': 0, 'gravity_strength': 0.2, 'max_speed': 0.25, 'height': 0.2 / 3, 'wall_buffer': 1 / 3, 'social_distance_color_threshold': 2 / 3}}}

class LargerCity2(LargerCity):
    CONFIG = {'random_seed': 1}

class LargeCityHighInfectionRadius(LargerCity):
    CONFIG = {'simulation_config': {'person_config': {'infection_radius': 0.5}}, 'graph_config': {'update_frequency': 1 / 60}}

class LargeCityLowerInfectionRate(LargerCity):
    CONFIG = {'p_infection_per_day': 0.1}

class SimpleSocialDistancing(RunSimpleSimulation):
    CONFIG = {'simulation_config': {'person_type': PiPerson, 'n_cities': 1, 'city_population': 100, 'person_config': {'infection_radius': 0.75, 'social_distance_factor': 2, 'gravity_strength': 0.1}, 'travel_rate': 0, 'infection_time': 5}}

class DelayedSocialDistancing(RunSimpleSimulation):
    CONFIG = {'delay_time': 8, 'target_sd_factor': 2, 'sd_probability': 1, 'random_seed': 1}

    def construct(self):
        self.wait(self.delay_time)
        self.change_social_distance_factor(self.target_sd_factor, self.sd_probability)
    ...
    def add_sliders(self):
        slider = ValueSlider(self.get_sd_slider_name(), value=0, x_min=0, x_max=2, tick_frequency=0.5, big_tick_numbers=[], numbers_to_show=range(3), decimal_number_config={'num_decimal_places': 0})
...
class DelayedSocialDistancingLargeCity(DelayedSocialDistancing, LargerCity):
    CONFIG = {'trigger_infection_count': 50, 'simulation_config': {'city_population': 900}}
...
class ReduceInfectionDuration(LargerCity):
    ...
    def add_sliders(self):
        slider = ValueSlider('Infection duration', value=5, x_min=0, x_max=5, tick_frequency=1, big_tick_numbers=[], numbers_to_show=range(6), decimal_number_config={'num_decimal_places': 0}, marker_color=RED)
        slider.match_width(self.graph)
        slider.next_to(self.graph, DOWN, buff=0.2 * self.graph.get_height())
        self.add(slider)
        self.slider = slider
...
class SimpleTravel(RunSimpleSimulation):
    CONFIG = {'simulation_config': {'person_type': DotPerson, 'n_cities': 12, 'city_population': 100, 'person_config': {'infection_radius': 0.75, 'social_distance_factor': 0, 'gravity_strength': 0.5}, 'travel_rate': 0.02, 'infection_time': 5}}

    def add_sliders(self):
        slider = ValueSlider('Travel rate', self.simulation.travel_rate, x_min=0, x_max=0.02, tick_frequency=0.005, big_tick_numbers=[], numbers_to_show=np.arange(0, 0.03, 0.01), decimal_number_config={'num_decimal_places': 2})
        slider.match_width(self.graph)
        slider.next_to(self.graph, DOWN, buff=0.2 * self.graph.get_height())
        self.add(slider)
...
class SimpleTravelLongInfectionPeriod(SimpleTravel):
    CONFIG = {'simulation_config': {'infection_time': 10}}

class SimpleTravelDelayedSocialDistancing(DelayedSocialDistancing, SimpleTravel):
    CONFIG = {'target_sd_factor': 2, 'sd_probability': 0.7, 'delay_time': 15, 'trigger_infection_count': 50}
...
class SimpleTravelSocialDistancePlusZeroTravel(SimpleTravelDelayedSocialDistancing):
    ...
    def construct(self):
        ...
        self.graph.add_v_line()
        self.play(self.tr_slider.get_change_anim(self.simulation.travel_rate), self.sd_slider.get_change_anim(self.target_sd_factor))
        self.run_until_zero_infections()
...
class SecondWave(SimpleTravelSocialDistancePlusZeroTravel):
    ...
    def run_until_zero_infections(self):
        ...
        self.graph.add_v_line()
        self.play(self.tr_slider.get_change_anim(0.02), self.sd_slider.get_change_anim(0))
...
class SimpleTravelDelayedTravelReduction(SimpleTravelDelayedSocialDistancing):
    CONFIG = {'trigger_infection_count': 50, 'target_travel_rate': 0.002}
...
class SimpleTravelDelayedTravelReductionThreshold100(SimpleTravelDelayedTravelReduction):
    CONFIG = {'random_seed': 2, 'trigger_infection_count': 100}

class SimpleTravelDelayedTravelReductionThreshold100TargetHalfPercent(SimpleTravelDelayedTravelReduction):
    CONFIG = {'random_seed': 2, 'trigger_infection_count': 100, 'target_travel_rate': 0.005}
...
class SimpleTravelDelayedTravelReductionThreshold100TargetHalfPercent2(SimpleTravelDelayedTravelReductionThreshold100TargetHalfPercent):
    CONFIG = {'random_seed': 1, 'sd_probability': 0.5}
...
class SimpleTravelLargeCity(SimpleTravel, LargerCity):
    CONFIG = {'simulation_config': {'n_cities': 12, 'travel_rate': 0.02}}

class SimpleTravelLongerDelayedSocialDistancing(SimpleTravelDelayedSocialDistancing):
    CONFIG = {'trigger_infection_count': 100}
...
class SocialDistanceAfterFiveDays(DelayedSocialDistancing):
    CONFIG = {'sd_probability': 0.7, 'delay_time': 5, 'simulation_config': {'travel_rate': 0}}

class QuarantineInfectious(RunSimpleSimulation):
    CONFIG = {'trigger_infection_count': 10, 'target_sd_factor': 3, 'infection_time_before_quarantine': 1}
    ...
    def set_quarantine_updaters(self):
        ...
        def quarantine_if_ready(simulation):
            for person in simulation.people:
                send_to_q_box = all([not person.is_quarantined, person.symptomatic, person.time - person.infection_start_time > self.infection_time_before_quarantine])
                if send_to_q_box:
...
class QuarantineInfectious80p(QuarantineInfectious):
    CONFIG = {'simulation_config': {'person_config': {'p_symptomatic_on_infection': 0.8}}}
...
class QuarantineInfectious50p(QuarantineInfectious):
    CONFIG = {'simulation_config': {'person_config': {'p_symptomatic_on_infection': 0.5}}}
...
class CentralMarket(DelayedSocialDistancing):
    CONFIG = {'sd_probability': 0.7, 'delay_time': 5, 'simulation_config': {'person_type': DotPerson, 'travel_rate': 0}, 'shopping_frequency': 0.05, 'shopping_time': 1}
        ...
        for person in simulation.people:
            ...
            if time_since_trip > shopping_time:
                if random.random() < dt * self.shopping_frequency:
                    ...
                    point = VectorizedPoint(person.get_center())
                    anim1 = ApplyMethod(point.move_to, person.box.get_center(), path_arc=45 * DEGREES, run_time=shopping_time, rate_func=there_and_back_with_pause)
...
class CentralMarketLowerInfection(CentralMarketLargePopulation):
    CONFIG = {'simulation_config': {'p_infection_per_day': 0.1}}

class CentralMarketVeryFrequentLargePopulationDelayedSocialDistancing(CentralMarketLowerInfection):
    CONFIG = {'sd_probability': 0.7, 'trigger_infection_count': 25, 'simulation_config': {'person_type': DotPerson}, 'target_sd_factor': 2}
...
class CentralMarketLessFrequent(CentralMarketVeryFrequentLargePopulationDelayedSocialDistancing):
    CONFIG = {'target_shopping_frequency': 0.01, 'trigger_infection_count': 100, 'random_seed': 1, 'simulation_config': {'city_population': 900}}
    ...
    def add_sliders(self):
        slider = ValueSlider('Shopping frequency', value=self.shopping_frequency, x_min=0, x_max=0.05, tick_frequency=0.01, big_tick_numbers=[], numbers_to_show=np.arange(0, 0.06, 0.01), decimal_number_config={'num_decimal_places': 2})
        slider.match_width(self.graph)
        slider.next_to(self.graph, DOWN, buff=0.2 * self.graph.get_height())
        self.add(slider)
...
class CentralMarketDownToZeroFrequency(CentralMarketLessFrequent):
    CONFIG = {'target_shopping_frequency': 0}
...
class CentralMarketTransitionToLowerInfection(CentralMarketLessFrequent):
    CONFIG = {'target_p_infection_per_day': 0.05, 'trigger_infection_count': 100, 'random_seed': 1, 'simulation_config': {'city_population': 900}}
    ...
    def add_sliders(self):
        slider = ValueSlider('Infection rate', value=self.simulation.p_infection_per_day, x_min=0, x_max=0.2, tick_frequency=0.05, big_tick_numbers=[], numbers_to_show=np.arange(0, 0.25, 0.05), decimal_number_config={'num_decimal_places': 2}, marker_color=RED)
        slider.match_width(self.graph)
        slider.next_to(self.graph, DOWN, buff=0.2 * self.graph.get_height())
        self.add(slider)
...
class TableOfContents(Scene):
    ...
    def construct(self):
        chapters = VGroup(OldTexText('Basic setup'), OldTexText('Identify and isolate'), OldTexText('Social distancing'), OldTexText('Travel restrictions'), OldTexText('$R$'), OldTexText('Central hubs'))
...
class DescribeModel(Scene):
    ...
    def construct(self):
        words = OldTexText('Susceptible', 'Infectious', 'Recovered')
        words.scale(0.8 / words[0][0].get_height())
        colors = [COLOR_MAP['S'], COLOR_MAP['I'], interpolate_color(COLOR_MAP['R'], WHITE, 0.5)]
        initials = VGroup()
        ...
        self.wait()
        self.play(Restore(initials, path_arc=-90 * DEGREES), FadeOut(title[1]))
        self.wait()
        pi = PiPerson(height=3, max_speed=0, infection_radius=5)
        pi.color_map['R'] = words[2].get_color()
        pi.center()
        pi.body.change('pondering', initials[0])
        word_anims = []
        for word in words:
            word_anims.append(LaggedStartMap(FadeInFrom, word[1:], lambda m: (m, 0.2 * LEFT)))
        self.play(Succession(FadeInFromDown(pi), ApplyMethod(pi.body.change, 'guilty')), word_anims[0], run_time=2)
        words[0].pi = pi.copy()
        self.play(words[0].pi.set_height, 1, words[0].pi.next_to, words[0], RIGHT)
        self.play(Blink(pi.body))
        pi.set_status('I')
        point = VectorizedPoint(pi.get_center())
        self.play(point.shift, 3 * RIGHT, MaintainPositionRelativeTo(pi, point), word_anims[1], run_time=2)
        words[1].pi = pi.copy()
        self.play(words[1].pi.set_height, 1, words[1].pi.next_to, words[1], RIGHT)
        self.wait(3)
        pi.set_status('R')
        self.play(word_anims[2], Animation(pi, suspend_mobject_updating=False))
        words[2].pi = pi.copy()
        self.play(words[2].pi.set_height, 1, words[2].pi.next_to, words[2], RIGHT)
        self.wait()
        i_pi = PiPerson(height=1.5, max_speed=0, infection_radius=6, status='S')
        i_pi.set_status('I')
        s_pis = VGroup()
        for vect in [RIGHT, UP, LEFT, DOWN]:
            s_pi = PiPerson(height=1.5, max_speed=0, infection_radius=6, status='S')
        ...
        self.play(FadeOut(pi), FadeIn(s_pis), FadeIn(i_pi))
        anims = []
        for s_pi in s_pis:
        ...
        self.play(VFadeIn(clock), ClockPassesTime(clock, run_time=5, hours_passed=5))
        i_pi.set_status('R')
        self.wait(1)
        ...
        self.play(FadeOut(words[2], UP), FadeIn(removed, DOWN))
        self.play(i_pi.body.change, 'pleading', removed)
        self.play(Blink(i_pi.body))
        self.wait()
...
class DoubleRadiusInGroup(Scene):
    ...
    def construct(self):
        radius = 1
        pis = VGroup(*[PiPerson(height=0.5, max_speed=0, wander_step_size=0, infection_radius=4 * radius) for x in range(49)])
        pis.arrange_in_grid()
...
class CutPInfectionInHalf(Scene):
    ...
    def construct(self):
        sicky = PiPerson(height=1, infection_radius=4, max_speed=0, wander_step_size=0)
        ...
        self.wait()
        eq = VGroup(OldTex('P(\\text{Infection}) = '), DecimalNumber(0.2))
        ...
        def get_clock_run(clock):
            return ClockPassesTime(clock, hours_passed=1, run_time=1)
        self.play(VFadeIn(clock), get_clock_run(clock))
        choices = VGroup()
        for x in range(9):
...
class KeyTakeaways(Scene):
    ...
    def construct(self):
        ...
        for title, takeaway in zip(titles, takeaways):
            ...
            self.play(title.scale, 1.5, title.to_edge, UP, title.set_x, 0, title.set_color, YELLOW, FadeOut(other_titles), ShowCreation(temp_h_line))
            self.play(FadeIn(takeaway, lag_ratio=0.1, run_time=2, rate_func=linear))
            self.wait(2)
            temp_h_line.rotate(PI)
            self.play(Restore(title), FadeIn(other_titles), Uncreate(temp_h_line), FadeOut(takeaway, DOWN, lag_ratio=0.25 / len(takeaway.family_members_with_points())))
            self.wait()
        self.embed()
...
class AsymptomaticCases(Scene):
    ...
    def construct(self):
        pis = VGroup(*[PiPerson(height=1, infection_radius=2, wander_step_size=0, max_speed=0) for x in range(5)])
        ...
        arrow = Arrow(label.get_bottom(), sneaky.body.get_top(), buff=0.5, max_tip_length_to_length_ratio=0.5, stroke_width=6, max_stroke_width_to_length_ratio=10)
        self.play(FadeInFromDown(label), GrowArrow(arrow))
        self.wait(13)

class WeDontHaveThat(TeacherStudentsScene):

    def construct(self):
        self.student_says("But we don't\\\\have that!", target_mode='angry', added_anims=[self.teacher.change, 'guilty'])
...
class IntroduceSocialDistancing(Scene):
    ...
    def construct(self):
        pis = VGroup(*[PiPerson(height=2, wander_step_size=0, gravity_well=None, social_distance_color_threshold=5, max_social_distance_stroke_width=10, dl_bound=[-FRAME_WIDTH / 2 + 1, -2], ur_bound=[FRAME_WIDTH / 2 - 1, 2]) for x in range(3)])
        pis.arrange(RIGHT, buff=0.25)
        pis.move_to(DOWN)
        pi1, pi2, pi3 = pis
        slider = ValueSlider('Social distance factor', 0, x_min=0, x_max=5, tick_frequency=1, numbers_to_show=range(6), marker_color=YELLOW)
        slider.center()
        slider.to_edge(UP)
        self.add(slider)
        ...
        self.add(pis)
        self.play(FadeIn(slider), *[ApplyMethod(pi1.body.look_at, pi2.body.eyes) for pi1, pi2 in zip(pis, [*pis[1:], pis[0]])])
        self.add(*pis)
        self.wait()
        ...
        for i, vect in ((0, RIGHT), (2, LEFT)):
            ...
            self.play(MoveToTarget(pis[1]), path_arc=PI)
            pis.resume_updating()
            self.wait(5)
        self.wait(5)
        self.embed()
...
class FastForwardBy2(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeInFrom, triangles, lambda m: (m, 0.4 * LEFT)), FadeIn(label, 0.2 * LEFT), run_time=1)
        self.play(FadeOut(label), FadeOut(triangles))
...
class DontLetThisHappen(Scene):
    ...
    def construct(self):
        ...
        arrow = Arrow(text.get_top(), text.get_top() + 2 * UR + 0.5 * RIGHT, path_arc=-120 * DEGREES)
        self.add(text)
        self.play(Write(text, run_time=1), ShowCreation(arrow))
        self.wait()

class ThatsNotHowIBehave(TeacherStudentsScene):

    def construct(self):
        self.student_says("That's...not\\\\how I behave.", target_mode='sassy', look_at=self.screen)
...
class BetweenNothingAndQuarantineWrapper(Scene):
    ...
    def construct(self):
        ...
        titles = VGroup(OldTexText('Do nothing'), OldTexText('Quarantine\\\\', '80$\\%$ of cases'), OldTexText('Quarantine\\\\', 'all cases'))
        ...
        self.add(rects)
        self.play(LaggedStartMap(FadeInFromDown, VGroup(titles[0], titles[2], titles[1]), lag_ratio=0.3))
...
class SARS2002(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.change, 'raise_right_hand', FadeIn(image, DOWN, run_time=2), self.change_students('pondering', 'thinking', 'pondering', look_at=image))
        self.play(FadeIn(name, RIGHT))
        self.play(GrowArrow(arrow), UpdateFromAlphaFunc(n_cases, lambda m, a: m.set_opacity(a)), ChangeDecimalToValue(n_cases[0], 8098), self.change_students(look_at=n_cases))
        self.wait()
        self.play_all_student_changes('thinking', look_at=n_cases)
...
class QuarteringLines(Scene):
    ...
    def construct(self):
        lines = VGroup(Line(UP, DOWN), Line(LEFT, RIGHT))
...
class IndicationArrow(Scene):
    ...
    def construct(self):
        vect = Vector(0.5 * DR, max_tip_length_to_length_ratio=0.4, max_stroke_width_to_length_ratio=10, stroke_width=5)
...
class IntroduceR0(Scene):
    ...
    def construct(self):
        pis = VGroup(*[PiPerson(height=0.5, infection_radius=1.5, wander_step_size=0, max_speed=0) for x in range(5)])
        ...
        for pi in pis[:4]:
            ...
            self.play(point.move_to, pi.get_right() + 0.25 * RIGHT, MaintainPositionRelativeTo(sicky, point), run_time=0.5)
            if pi.status == 'S':
                count[1].increment_value()
                count[1].set_color(WHITE)
                pi.set_status('I')
            self.play(Flash(sicky.get_center(), color=RED, line_length=0.3, flash_radius=0.7), count[1].set_color, RED)
        self.wait()
        self.play(point.move_to, 2 * UP, MaintainPositionRelativeTo(sicky, point))
        count_copy = count[1].copy()
        self.play(count_copy.next_to, sicky.body, UR, count_copy.set_color, WHITE)
        self.wait()
        zeros = VGroup()
        for pi in pis[:4]:
            if pi.status == 'I':
        ...
        self.play(LaggedStartMap(FadeInFromDown, zeros), GrowArrow(arrow), FadeIn(R_label))
        brace = Brace(R_label[1], DOWN)
        name = OldTexText("``Effective reproductive number''")
        name.set_color(YELLOW)
        name.next_to(brace, DOWN)
        name.to_edge(LEFT)
        self.play(GrowFromCenter(brace), FadeIn(name, 0.5 * UP))
        self.wait(5)
        ...
        for pi in pis[:4]:
            pi.set_status('S')
        self.play(FadeOut(R_label[1], UP), FadeOut(name, UP), FadeIn(R0, DOWN), FadeIn(brr, DOWN), FadeOut(zeros), FadeOut(count_copy), brace.match_width, R0, {'stretch': True}, brace.match_x, R0)
        self.wait()
        count[1].set_value(0)
        for pi in pis[:4]:
            point = VectorizedPoint(sicky.get_center())
            self.play(point.move_to, pi.body.get_right() + 0.25 * RIGHT, MaintainPositionRelativeTo(sicky, point), run_time=0.5)
            count[1].increment_value()
            count[1].set_color(WHITE)
            pi.set_status('I')
            self.play(Flash(sicky.get_center(), color=RED, line_length=0.3, flash_radius=0.7), count[1].set_color, RED)
...
class HowR0IsCalculatedHere(Scene):
    ...
    def construct(self):
        words = VGroup(OldTexText('Count ', '\\# transfers'), OldTexText('for every infectious case'))
        words.arrange(DOWN)
        ...
        self.wait()
        self.play(FadeOut(words[0][0], UP), FadeIn(estimate, DOWN))
        self.wait()
        self.play(Write(parens), FadeIn(average, 0.5 * RIGHT), self.camera.frame.shift, LEFT)
        self.wait()
        self.embed()
...
class DoubleInfectionRadius(Scene):
    ...
    def construct(self):
        ...
        self.add(c1, title)
        self.wait()
        self.play(GrowArrow(arrow), TransformFromCopy(c1, c2))
        self.wait()
...
class PInfectionSlider(Scene):
    ...
    def construct(self):
        slider = ValueSlider('Probability of infection', 0.2, x_min=0, x_max=0.2, numbers_to_show=np.arange(0.05, 0.25, 0.05), decimal_number_config={'num_decimal_places': 2}, tick_frequency=0.05)
        self.add(slider)
        self.wait()
...
class R0Categories(Scene):
    ...
    def construct(self):
        titles = VGroup(OldTex('R > 1', color=RED), OldTex('R = 1', color=YELLOW), OldTex('R < 1', color=GREEN))
        titles.scale(1.25)
        ...
        self.add(titles)
        self.add(v_lines)
        names = VGroup(OldTexText('Epidemic', color=RED), OldTexText('Endemic', color=YELLOW), OldTexText('...Hypodemic?', color=GREEN))
        ...
        for x in range(4):
            ...
            for dot in rows[-1]:
                for child in dot.children:
                    new_lines.add(Line(dot.get_center(), child.get_center(), buff=0.2))
            rows.add(new_row)
            lines.add(new_lines)
        ...
        for d1, d2 in zip(mid_rows[:-1], mid_rows[1:]):
            d1.child = d2
            mid_lines.add(Line(d1.get_center(), d2.get_center(), buff=0.2))
...
class RealR0Estimates(Scene):
    ...
    def construct(self):
        labels = VGroup(OldTexText('COVID-19\\\\', '$R_0 \\approx 2$'), OldTexText('1918 Spanish flu\\\\', '$R_0 \\approx 2$'), OldTexText('Usual seasonal flu\\\\', '$R_0 \\approx 1.3$'))
        images = Group(ImageMobject('COVID-19_Map'), ImageMobject('spanish_flu'), ImageMobject('Influenza'))
        for image in images:
            image.set_height(3)
        ...
        for label, image in zip(labels, images):
            self.play(FadeIn(image, DOWN), FadeIn(label, UP))
            self.wait()
        self.embed()
...
class WhyChooseJustOne(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('Why choose\\\\just one?', target_mode='sassy', added_anims=[self.teacher.change, 'thinking'], run_time=1)
        self.play(self.change_students('confused', 'confused', 'sassy'))
        self.wait(2)
        self.teacher_says('For science!', target_mode='hooray', look_at=self.students[2].eyes)
...
class NickyCaseMention(Scene):
    ...
    def construct(self):
        ...
        words.scale(1)
        words.to_edge(LEFT)
        arrow = Arrow(words.get_top(), words.get_top() + 2 * UR + RIGHT, path_arc=-90 * DEGREES)
        self.play(Write(words[0]), ShowCreation(arrow), run_time=1)
        self.wait(2)
        self.play(Write(words[1]), run_time=1)
        self.wait()
...
class WideSpreadTesting(Scene):
    ...
    def construct(self):
        dots = VGroup(*[DotPerson(height=0.2, infection_radius=0.6, max_speed=0, wander_step_size=0, p_symptomatic_on_infection=0.8) for x in range(600)])
        ...
        self.play(LaggedStartMap(FadeIn, rings, lag_ratio=3 / len(rings), run_time=2.5, rate_func=there_and_back))
        box = Square()
        box.set_height(2)
        box.to_corner(DL)
        box.shift(LEFT)
        ...
        self.play(ShowCreation(box), LaggedStartMap(MoveToTarget, points, lag_ratio=0.05), self.camera.frame.shift, LEFT, run_time=3)
        self.wait(9)

    def get_rings(self, center, max_radius, delta_r):
        radii = np.arange(0, max_radius, delta_r)
        rings = VGroup(*[Circle(radius=r, stroke_opacity=0.75 * (1 - fdiv(r, max_radius)), stroke_color=TEAL, stroke_width=100 * delta_r) for r in radii])
        ...
        for x in range(8):
            ...
            for virus in viruses:
                ...
                for child in children:
                    ...
                    child.shift([(random.random() - 0.5) * 3, (random.random() - 0.5) * 3, 0])
...
WILL_BE_SCENES_IN_ORDER = [QuarantineInfectious, QuarantineInfectiousLarger, QuarantineInfectiousLarger80p, QuarantineInfectiousTravel, QuarantineInfectiousTravel80p, QuarantineInfectiousTravel50p, SimpleSocialDistancing, DelayedSocialDistancing, DelayedSocialDistancingLargeCity, SimpleTravelSocialDistancePlusZeroTravel, SimpleTravelDelayedSocialDistancing99p, SimpleTravelDelayedTravelReductionThreshold100TargetHalfPercent, SimpleTravelDelayedSocialDistancing50pThreshold100, SimpleTravelDelayedTravelReductionThreshold100, LargerCity2, LargeCityHighInfectionRadius, LargeCityLowerInfectionRate, SimpleTravelDelayedSocialDistancing99p, CentralMarketLargePopulation, CentralMarketLowerInfection, CentralMarketVeryFrequentLargePopulationDelayedSocialDistancing, CentralMarketLessFrequent, CentralMarketTransitionToLowerInfection, CentralMarketTransitionToLowerInfectionAndLowerFrequency, CentralMarketQuarantine, CentralMarketQuarantine80p]
to_run_later = [CentralMarketTransitionToLowerInfectionAndLowerFrequency, SimpleTravelDelayedTravelReduction, CentralMarketLargePopulation, CentralMarketLowerInfection, CentralMarketVeryFrequentLargePopulationDelayedSocialDistancing, CentralMarketLessFrequent, CentralMarketTransitionToLowerInfection, CentralMarketTransitionToLowerInfectionAndLowerFrequency, CentralMarketQuarantine, CentralMarketQuarantine80p, SecondWave]