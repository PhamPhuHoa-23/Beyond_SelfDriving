...
class UnemploymentTitle(Scene):
    ...
    def construct(self):
        ...
        words.to_edge(UP)
        arrow = Arrow(words.get_bottom(), words.get_bottom() + 3 * RIGHT + 3 * DOWN, stroke_width=10, tip_length=0.5)
        arrow.set_color(BLUE_E)
        words.set_color(BLACK)
        self.play(ShowIncreasingSubsets(words), ShowCreation(arrow))
        self.wait()

class ExplainTracing(Scene):

    def construct(self):
        words = VGroup(OldTexText('Testing, ', 'Testing, ', 'Testing!'), OldTexText('Contact Tracing'))
        ...
        self.play(sicky.change, 'sick', sicky.set_color, '#9BBD37', FadeIn(covid_words, RIGHT), GrowArrow(arrow))
        ...
        self.play(*anims)
        self.wait()
        self.play(VFadeIn(boxes[4:], run_time=2, rate_func=there_and_back_with_pause))
        self.wait()
        self.play(FadeOut(VGroup(covid_words, arrow, *boxes[:4], *pis), lag_ratio=0.1, run_time=3))
        ...
        self.wait()
        self.play(ShowCreation(cross), FadeOut(q_marks, RIGHT), FadeOut(lines))
        self.wait()
        ...
        self.play(FadeInFromDown(dp_3t), FadeOut(loc_tracking), FadeOut(implies), FadeOut(cross), ShowCreation(arrow))
...
class ContactTracingMisnomer(Scene):
    ...
    def construct(self):
        ...
        for vect, rect, expl, color in zip([UP, DOWN], reversed(rects), expls, colors):
            ...
            self.play(FadeIn(expl, -vect), GrowArrow(arrow), ShowCreation(rect))
            self.wait()
        self.play(Write(VGroup(*self.mobjects), rate_func=lambda t: smooth(1 - t), run_time=3))
...
class ContactTracingWords(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(words.set_height, 1, words.to_corner, UL)
        self.wait()

class WanderingDotsWithLines(Scene):

    def construct(self):
        sim = SIRSimulation(city_population=20, person_type=DotPerson, person_config={'color_map': {'S': GREY, 'I': GREY, 'R': GREY}, 'infection_ring_style': {'stroke_color': YELLOW}, 'max_speed': 0.5}, infection_time=100)
...
class WhatAboutPeopleWithoutPhones(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('What about people\\\\without phones?', target_mode='sassy', added_anims=[self.teacher.change, 'guilty'])
        ...
        words.scale(1.5)
        words.to_corner(UL)
        self.play(FadeInFromDown(words), RemovePiCreatureBubble(self.students[2]), *[ApplyMethod(pi.change, 'pondering', words) for pi in self.pi_creatures])
        self.wait(5)
...
class PiGesture1(Scene):
    ...
    def construct(self):
        ...
        bubble = randy.get_bubble(bubble_type=SpeechBubble, height=2, width=3)
...
class PiGesture2(Scene):
    ...
    def construct(self):
        ...
        bubble = randy.get_bubble(bubble_type=SpeechBubble, height=2, width=3, direction=LEFT)
...
class AppleGoogleCoop(Scene):
    ...
    def construct(self):
        logos = Group(self.get_apple_logo(), self.get_google_logo())
        for logo in logos:
            logo.set_height(2)
        apple, google = logos
        ...
        for vect, u in zip([UP, DOWN], [0, 1]):
            ...
            arrows.add(Arrow(m1.get_edge_center(vect), m2.get_edge_center(vect), path_arc=-90 * DEGREES, buff=MED_LARGE_BUFF, stroke_width=10))
        self.play(LaggedStart(Write(apple), FadeIn(google), lag_ratio=0.7))
        self.wait()
...
class LocationTracking(Scene):
    ...
    def construct(self):
        question = OldTexText('Would you like this company to track\\\\', 'and occasionally sell your location?')
        ...
        morty.to_edge(DOWN)
        bubble = morty.get_bubble(height=2, width=3, direction=LEFT)
        answer = OldTexText('Um...', 'no.')
        answer.set_height(0.4)
        answer.set_color(YELLOW)
        bubble.add_content(answer)
        self.add(morty)
        self.play(FadeInFromDown(question), Write(slider), FadeIn(dot))
        self.play(morty.change, 'confused', slider)
        self.play(Blink(morty))
        self.play(FadeIn(bubble), Write(answer[0]))
        self.wait()
        self.play(dot.move_to, slider, LEFT, slider.set_fill, {'opacity': 0}, FadeIn(answer[1]), morty.change, 'sassy')
        self.play(Blink(morty))
        self.wait(2)
        self.play(Blink(morty))
        self.wait(2)
