...
import scipy.stats
CASE_DATA = [9, 15, 30, 40, 56, 66, 84, 102, 131, 159, 173, 186, 190, 221, 248, 278, 330, 354, 382, 461, 481, 526, 587, 608, 697, 781, 896, 999, 1124, 1212, 1385, 1715, 2055, 2429, 2764, 3323, 4288, 5364, 6780, 8555, 10288, 12742, 14901, 17865, 21395]
SICKLY_GREEN = '#9BBD37'
...
class IntroducePlot(Scene):
    ...
    def construct(self):
        ...
        axes.y_axis.add(axes.h_lines, axes.small_h_lines, axes.tiny_h_lines, axes.tiny_ticks)
        ...
        v_line.add_updater(lambda m: m.put_start_and_end_on(axes.c2p(axes.x_axis.p2n(dot.get_center()), 0), dot.get_bottom()))
        self.add(dot)
        self.add(count)
        self.add(v_line)
        for new_day in range(day + 1, len(dots)):
            ...
            self.add(line, dot)
            self.play(dot.move_to, new_dot.get_center(), dot.set_color, RED, ChangeDecimalToValue(count, CASE_DATA[new_day]), ShowCreation(line))
            line.rotate(PI)
            self.play(dot.set_color, YELLOW, Uncreate(line), run_time=0.5)
            self.add(dots[new_day])
            day = new_day
            if day == 27:
                self.add(axes.y_axis, axes.tiny_y_labels, axes.tiny_h_lines, axes.tiny_ticks, title)
                self.play(axes.y_axis.stretch, 0.2, 1, {'about_point': origin}, VFadeOut(axes.tiny_y_labels), VFadeOut(axes.tiny_h_lines), VFadeOut(axes.tiny_ticks), MaintainPositionRelativeTo(dot, dots[new_day]), run_time=2)
                self.add(axes, title, *dots[:new_day])
            if day == 36:
                self.add(axes.y_axis, axes.small_y_labels, axes.small_h_lines, title)
                self.play(axes.y_axis.stretch, 0.2, 1, {'about_point': origin}, VFadeOut(axes.small_y_labels), VFadeOut(axes.small_h_lines), MaintainPositionRelativeTo(dot, dots[new_day]), run_time=2)
                self.add(axes, title, *dots[:new_day])
        ...
        for d1, d2 in zip(dots[25:], dots[26:]):
            line = Line(d1.get_top(), d2.get_corner(UL), path_arc=-90 * DEGREES)
            line.set_stroke(PINK, 2)
            label = VGroup(OldTex('\\times'), DecimalNumber(axes.y_axis.p2n(d2.get_center()) / axes.y_axis.p2n(d1.get_center())))
            ...
            label.set_opacity(0)
            self.play(ShowCreation(line), Restore(label), last_label.move_to, label.saved_state, VFadeOut(last_label), FadeOut(last_line))
            last_line = line
            last_label = label
        self.wait()
        self.play(FadeOut(last_label), FadeOut(last_line))

    def get_title(self, axes):
        title = OldTexText('Recorded COVID-19 cases\\\\outside mainland China', tex_to_color_map={'COVID-19': RED})
        title.next_to(axes.c2p(0, 1000.0), RIGHT, LARGE_BUFF)
        title.to_edge(UP)
    ...
    def get_axes(self, width=12, height=6):
        ...
        axes = Axes(x_range=(0, n_cases), y_range=(0, 25000, 1000), width=width, height=height)
        axes.center()
        axes.to_edge(DOWN, buff=LARGE_BUFF)
        text_pos_pairs = [('Mar 6', 0), ('Feb 23', -12), ('Feb 12', -23), ('Feb 1', -34), ('Jan 22', -44)]
        labels = VGroup()
        extra_ticks = VGroup()
        ...
        group_range_pairs = [(axes.h_lines, 5000.0 * np.arange(1, 6)), (axes.small_h_lines, 1000.0 * np.arange(1, 5)), (axes.tiny_h_lines, 200 * np.arange(1, 5))]
        for group, _range in group_range_pairs:
            for y in _range:
                group.add(Line(axes.c2p(0, y), axes.c2p(n_cases, y)))
...
class IntroQuestion(Scene):
    ...
    def construct(self):
        questions = VGroup(OldTexText('What is exponential growth?'), OldTexText('Where does it come from?'), OldTexText('What does it imply?'), OldTexText('When does it stop?'))
        questions.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        for question in questions:
            self.play(FadeIn(question, RIGHT))
            self.wait()
        self.play(LaggedStartMap(FadeOutAndShift, questions, lambda m: (m, DOWN)))

class ViralSpreadModel(Scene):
    CONFIG = {'num_neighbors': 5, 'infection_probability': 0.3, 'random_seed': 1}
    ...
    def show_patient0(self, randys):
        ...
        circle.scale(1.5)
        self.play(patient0.change, 'sick', patient0.set_color, SICKLY_GREEN, ShowCreationThenFadeOut(circle))
    ...
    def spread_infection(self, randys):
        ...
        for randy in randys:
            if randy.infected:
                cough_anims.append(Flash(randy, color=SICKLY_GREEN, num_lines=16, line_stroke_width=1, flash_radius=0.5, line_length=0.1))
        random.shuffle(cough_anims)
        self.play(LaggedStart(*cough_anims, run_time=1, lag_ratio=1 / len(cough_anims)))
        newly_infected = []
        for randy in randys:
            if randy.infected:
...
class GrowthEquation(Scene):
    ...
    def construct(self):
        ...
        self.play(GrowFromCenter(delta_N_brace), FadeIn(delta_N_text, 0.5 * UP))
        self.wait()
        self.play(LaggedStart(TransformFromCopy(N_label[0], nep[4]), TransformFromCopy(E_label[0], nep[0]), TransformFromCopy(p_label[0], nep[2]), FadeIn(nep[1]), FadeIn(nep[3]), lag_ratio=0.2, run_time=2))
        self.wait()
        self.play(ShowCreationThenFadeAround(nep[-1], surrounding_rectangle_config={'color': RED}))
        lhs = OldTex('N_{d + 1}', '=')
        lhs[0].set_color(YELLOW)
        lhs.move_to(eq, RIGHT)
        lhs.shift(DOWN)
        rhs = VGroup(nep[-1].copy(), OldTex('+'), nep.copy())
        rhs.arrange(RIGHT)
        rhs.next_to(lhs, RIGHT)
        self.play(FadeOut(delta_N_brace), FadeOut(delta_N_text), FadeIn(lhs, UP))
        self.play(FadeIn(rhs[:2]))
        self.play(TransformFromCopy(nep, rhs[2]))
        self.wait()
        alt_rhs = OldTex('(', '1', '+', 'E', '\\cdot', 'p', ')', 'N_d', tex_to_color_map={'E': BLUE, 'p': TEAL, 'N_d': YELLOW})
        new_lhs = lhs.copy()
        new_lhs.shift(DOWN)
        alt_rhs.next_to(new_lhs, RIGHT)
        self.play(TransformFromCopy(lhs, new_lhs))
        self.play(TransformFromCopy(rhs[0], alt_rhs[7].copy()), TransformFromCopy(rhs[2][4], alt_rhs[7]))
        self.play(TransformFromCopy(rhs[1][0], alt_rhs[2]), TransformFromCopy(rhs[2][0], alt_rhs[3]), TransformFromCopy(rhs[2][1], alt_rhs[4]), TransformFromCopy(rhs[2][2], alt_rhs[5]), TransformFromCopy(rhs[2][3], alt_rhs[6]), FadeIn(alt_rhs[0]), FadeIn(alt_rhs[1]))
        self.wait()
        ...
        self.wait()
        eq_group = VGroup(delta_N, eq, nep, lhs, rhs, new_lhs, alt_rhs, brace, text)
        self.clear()
        self.add(labels, eq_group)
        self.play(ShowCreationThenFadeAround(VGroup(delta_N, eq, nep), surrounding_rectangle_config={'color': RED}))
        self.play(ShowCreationThenFadeAround(VGroup(new_lhs, alt_rhs, brace, text), surrounding_rectangle_config={'color': RED}))
        self.wait()
        self.play(eq_group.to_edge, LEFT, LARGE_BUFF)
        exp_eq = OldTex('N_d = (1 + E \\cdot p)^{d} \\cdot N_0', tex_to_color_map={'N_d': YELLOW, 'E': BLUE, 'p': TEAL, '{d}': YELLOW, 'N_0': YELLOW})
        ...
        self.play(ShowCreation(ep_rect), FadeIn(ep_label, lag_ratio=0.1))
        self.wait()
        self.play(FadeOut(ep_rect), FadeOut(ep_label))
        p_factors = OldTex('\\left(1 - {N_d \\over \\text{pop. size}} \\right)', tex_to_color_map={'N_d': YELLOW})
        p_factors.next_to(nep, RIGHT, buff=3)
        p_factors_rect = SurroundingRectangle(p_factors)
        p_factors_rect.set_stroke(TEAL, 2)
        p_arrow = Arrow(p_factors_rect.get_corner(UL), nep[2].get_top(), path_arc=75 * DEGREES, color=TEAL)
        self.play(ShowCreation(p_factors_rect), ShowCreation(p_arrow))
        self.wait()
        self.play(Write(p_factors))
        self.wait()
        self.play(FadeOut(p_factors), FadeOut(p_arrow), FadeOut(p_factors_rect))
        ...
        self.play(FadeOut(ep_question), FadeIn(ep_value), FadeOut(text[0]), text[1].next_to, brace, DOWN)
        ...
        for tex in ['21{,}000', '61']:
            self.play(ShowCreationThenFadeOut(Underline(eq1.get_part_by_tex(tex), stroke_color=YELLOW, stroke_width=2, buff=SMALL_BUFF), run_time=2))
        value = eq1_rhs.get_value()
        eq1_rhs.set_value(0)
        self.play(ChangeDecimalToValue(eq1_rhs, value))
        self.wait()
        eq1.add(eq1_rhs)
        self.play(eq1.shift, DOWN, FadeIn(eq2))
        new_text = OldTexText('1.05')
        new_text.move_to(text[1])
        self.play(ChangeDecimalToValue(ep_value, 0.05), FadeOut(text[1]), FadeIn(new_text))
        self.wait()
        ...
        bubble = SpeechBubble(direction=RIGHT, height=2.5, width=5)
        bubble.next_to(morty, UL, buff=0)
        bubble.write('The only thing to fear\\\\is the lack of fear itself.')
        self.add(morty)
        self.add(bubble)
        self.add(bubble.content)
        self.play(labels.set_opacity, 0.5, VFadeIn(morty), morty.change, 'speaking', FadeIn(bubble), Write(bubble.content))
...
class RescaleToLogarithmic(IntroducePlot):
    ...
    def construct(self):
        ...
        log_y_axis = NumberLine(x_min=0, x_max=9)
        log_y_axis.rotate(90 * DEGREES)
        log_y_axis.move_to(axes.c2p(0, 0), DOWN)
        labels_text = ['10', '100', '1k', '10k', '100k', '1M', '10M', '100M', '1B']
        log_y_labels = VGroup()
        ...
        for exponent in range(0, 9):
            for mult in range(2, 12, 2):
                y = mult * 10 ** exponent
                line = Line(axes.c2p(0, y), axes.c2p(axes.x_max, y))
        ...
        self.play(MoveToTarget(dots), MoveToTarget(axes.y_axis.tick_marks), MoveToTarget(axes.h_lines), MoveToTarget(log_h_lines), VFadeOut(axes.y_labels), VFadeOut(axes.y_axis.tick_marks), VFadeOut(axes.h_lines), Restore(log_y_labels), FadeIn(logarithm_title), run_time=2)
        self.play(Restore(log_y_axis))
        self.wait()
        brace = Brace(log_y_axis.tick_marks[1:3], RIGHT, buff=SMALL_BUFF)
        ...
        for i, j in [(7, 27), (27, 40)]:
            ...
            self.wait()
            remove_anims += [ApplyMethod(rect.stretch, 0, 0, {'about_edge': RIGHT}, remover=True), FadeOut(label, RIGHT)]
        self.wait()
        ...
        reg_line = Line(c2p(0, 10 ** reg.intercept), c2p(x_max, 10 ** (reg.intercept + reg.slope * x_max)))
        ...
        for prop in [0.4, 0.6, 0.5]:
            reg_arrows.add(Arrow(reg_label.get_top(), reg_line.point_from_proportion(prop), buff=SMALL_BUFF))
        reg_arrow = reg_arrows[0].copy()
        self.play(Write(reg_label, run_time=1), Transform(reg_arrow, reg_arrows[1], run_time=2), VFadeIn(reg_arrow))
        ...
        self.play(ChangeDecimalToValue(R2_label[1], reg.rvalue ** 2, run_time=2), UpdateFromAlphaFunc(R2_label, lambda m, a: m.set_opacity(a)))
        self.wait()
        ...
        self.play(rect.set_stroke, WHITE, 2, rect.set_fill, GREY_E, 1)
        self.wait()
        self.play(FadeOut(rect), FadeOut(R2_label), FadeOut(reg_label), FadeOut(reg_arrow))
        extended_x_axis = NumberLine(x_min=axes.x_axis.x_max, x_max=axes.x_axis.x_max + 90, unit_size=get_norm(axes.x_axis.n2p(1) - axes.x_axis.n2p(0)), big_tick_numbers=[])
        extended_x_axis.move_to(axes.x_axis.get_right(), LEFT)
        self.play(self.camera.frame.scale, 2, {'about_edge': DL}, self.camera.frame.shift, 2.5 * DOWN + RIGHT, log_h_lines.stretch, 3, 0, {'about_edge': LEFT}, ShowCreation(extended_x_axis, rate_func=squish_rate_func(smooth, 0.5, 1)), run_time=3)
        self.play(reg_line.scale, 3, {'about_point': reg_line.get_start()})
        self.wait()
        target_ys = [1000000.0, 10000000.0, 100000000.0, 1000000000.0]
        last_point = dots[-1].get_center()
        last_label = None
        last_rect = None
        date_labels_text = ['Apr 5', 'Apr 22', 'May 9', 'May 26']
        for target_y, date_label_text in zip(target_ys, date_labels_text):
            ...
            v_line = DashedLine(axes.c2p(x, 0), c2p(x, target_y))
            v_line.set_stroke(WHITE, 2)
            if target_y is target_ys[-1]:
                self.play(self.camera.frame.scale, 1.1, {'about_edge': LEFT})
            if last_label:
                self.play(ReplacementTransform(last_label, label), ReplacementTransform(last_rect, rect))
            else:
                rect.save_state()
                rect.stretch(0, 0, about_edge=LEFT)
                self.play(Restore(rect), FadeIn(label, LEFT))
            self.wait()
            self.play(ShowCreation(v_line), FadeIn(date_label))
            last_label = label
            last_rect = rect
        self.wait()
        self.play(FadeOut(last_label, RIGHT), ApplyMethod(last_rect.stretch, 0, 0, {'about_edge': RIGHT}, remover=True))
        ...
        def get_dots_along_curve(curve):
            ...
            for x in range(x_min, x_max):
                prop = binary_search(lambda p: axes.x_axis.p2n(curve.point_from_proportion(p)), x, 0, 1)
                prop = prop or 0
        ...
        p100k = get_point_from_y(100000.0)
        p100M = get_point_from_y(100000000.0)
        curve1 = VMobject()
        curve1.append_points([dots[-1].get_center(), p100k, p100k + 5 * RIGHT])
        curve2 = VMobject()
        curve2.append_points([dots[-1].get_center(), p100M, p100M + 5 * RIGHT + 0.25 * UP])
...
class CompareCountries(Scene):
    ...
    def construct(self):
        ...
        self.play(ChangeDecimalToValue(labels[0][0], case_numbers[0]), ChangeDecimalToValue(labels[1][0], case_numbers[1]), UpdateFromAlphaFunc(labels, lambda m, a: m.set_opacity(a)))
        self.wait()
        arrow = Arrow(labels[1][0].get_bottom(), labels[0][0].get_bottom(), path_arc=-90 * DEGREES)
        ...
        self.wait(2)
        self.play(FadeIn(alt_arrow_label, 0.5 * UP), FadeOut(arrow_label, 0.5 * DOWN))
        self.wait(2)

class SARSvs1918(Scene):

    def construct(self):
        titles = VGroup(OldTexText('2002 SARS outbreak'), OldTexText('1918 Spanish flu'))
        images = Group(ImageMobject('sars_icon'), ImageMobject('spanish_flu'))
        ...
        n_cases_labels = VGroup(OldTexText('8,096 cases'), OldTexText('$\\sim$513{,}000{,}000 cases'))
        ...
        for image, title, label in zip(images, titles, n_cases_labels):
            self.play(FadeIn(image, DOWN), Write(title), run_time=1)
...
class ViralSpreadModelWithShuffling(ViralSpreadModel):
    ...
    def shuffle_randys(self, randys):
        ...
        for i, randy in zip(indices, randys):
            ...
            anims.append(MoveToTarget(randy, path_arc=30 * DEGREES))
...
class SneezingOnNeighbors(Scene):
    ...
    def construct(self):
        ...
        self.add(randys)
        self.play(randys[1].change, 'sick', randys[1].set_color, SICKLY_GREEN)
        self.play(Flash(randys[1], color=SICKLY_GREEN, flash_radius=0.8), randys[0].change, 'sassy', randys[1], randys[2].change, 'angry', randys[1])
        self.play(randys[0].change, 'sick', randys[0].set_color, SICKLY_GREEN, randys[2].change, 'sick', randys[2].set_color, SICKLY_GREEN)
        self.play(Flash(randys[1], color=SICKLY_GREEN, flash_radius=0.8))
        self.play(randys[0].change, 'sad', randys[1], randys[2].change, 'tired', randys[1])
        self.play(Flash(randys[1], color=SICKLY_GREEN, flash_radius=0.8))
        self.play(randys[0].change, 'angry', randys[1], randys[2].change, 'angry', randys[1])
        self.wait()
...
class ViralSpreadModelWithClustersAndTravel(ViralSpreadModelWithClusters):
    ...
    def travel_between_clusters(self):
        ...
        for rep, target in zip(reps, targets):
            ...
            anims.append(MoveToTarget(rep, path_arc=30 * DEGREES))
...
class ShowLogisticCurve(Scene):
    ...
    def construct(self):
        axes = self.get_axes()
        self.add(axes)
        ode = OldTex('{dN \\over dt} =', 'c', '\\left(1 - {N \\over \\text{pop.}}\\right)', 'N', tex_to_color_map={'N': YELLOW})
        ode.set_height(0.75)
        ode.center()
        ode.to_edge(RIGHT)
        ode.shift(1.5 * UP)
        self.add(ode)
        curve = axes.get_graph(lambda x: 8 * smooth(x / 10) + 0.2)
        ...
        self.wait()
        line = Line(curve.point_from_proportion(0), curve.point_from_proportion(0.25))
        rect = Rectangle()
        rect.set_stroke(width=0)
        rect.set_fill(TEAL, 0.5)
        rect.replace(line, stretch=True)
        ...
        line = DashedLine(axes.c2p(0, 8.2), axes.c2p(axes.x_max, 8.2))
        ...
        self.play(ShowCreation(tan_line), FadeInFromLarge(dot))
        self.play(x_tracker.set_value, 5, run_time=6)
        self.wait()
        self.play(x_tracker.set_value, 9.9, run_time=6)
        self.wait()
        gf_label = OldTex('\\text{Growth factor} =', '{\\Delta N_d \\over \\Delta N_{d - 1}}', tex_to_color_map={'\\Delta': WHITE, 'N_d': YELLOW, 'N_{d - 1}': BLUE})
        ...
        for label, tex, vect in ((numer_label, 'N_d', UL), (denom_label, 'N_{d - 1}', DL)):
            ...
            label.shift(2 * RIGHT)
            arrow = Arrow(label.get_corner(vect[1] * DOWN), part.get_corner(vect[1] * UP) + 0.25 * LEFT, buff=0.1)
        ...
        self.add(eq, gf)
        self.play(x_tracker.set_value, 5, run_time=6, rate_func=linear)
        self.wait()
        self.play(x_tracker.set_value, 9, run_time=6, rate_func=linear)

    def get_axes(self):
        axes = Axes(x_min=0, x_max=13, y_min=0, y_max=10, y_axis_config={'unit_size': 0.7, 'include_tip': False})
        axes.center()
        axes.to_edge(DOWN)
...
class SubtltyOfGrowthFactorShift(Scene):
    ...
    def construct(self):
        ...
        dates = VGroup(OldTexText('March 3, 2020'), OldTexText('March 4, 2020'), OldTexText('March 5, 2020'), OldTexText('March 6, 2020'))
        ...
        for t1, t2 in zip(totals, totals[1:]):
            arrow = Arrow(t1.get_right(), t2.get_right(), path_arc=-150 * DEGREES, buff=0.1, max_tip_length_to_length_ratio=0.15)
        ...
        for c1, c2 in zip(change_labels, change_labels[1:]):
            arrow = Arrow(c1.get_right(), c2.get_right(), path_arc=-150 * DEGREES, buff=0.1, max_tip_length_to_length_ratio=0.15)
        ...
        self.play(LaggedStartMap(FadeInFrom, totals[:-1], lambda m: (m, UP)))
        self.wait()
        self.play(ShowCreation(change_arrows[:-1]), LaggedStartMap(FadeInFrom, change_labels[:-1], lambda m: (m, LEFT)), FadeIn(change_title))
        self.wait()
        self.play(ShowCreation(gf_arrows[:-1]), LaggedStartMap(FadeIn, gf_labels[:-1]), FadeIn(gf_title))
        self.wait()
        self.play(LaggedStart(FadeIn(gf_labels[-1]), FadeIn(gf_arrows[-1]), FadeIn(change_labels[-1]), FadeIn(change_arrows[-1]), FadeIn(totals[-1])))
        self.wait()
        alt_change = data[-2] - data[-3]
        alt_total = data[-2] + alt_change
        alt_gf = 1
        self.play(ChangeDecimalToValue(gf_labels[-1][-1], alt_gf), ChangeDecimalToValue(gf_labels[-1][0][0], alt_change), ChangeDecimalToValue(change_labels[-1], alt_change), ChangeDecimalToValue(totals[-1], alt_total))
        self.wait()
...
class ContrastRandomShufflingWithClustersAndTravel(Scene):
    ...
    def construct(self):
        ...
        self.add(squares)
        titles = VGroup(OldTexText('Random shuffling'), OldTexText('Clusters with travel'))
        for title, square in zip(titles, squares):
            title.scale(1.4)
            title.next_to(square, UP)
        titles[1].align_to(titles[0], UP)
        self.play(LaggedStartMap(FadeInFrom, titles, lambda m: (m, 0.25 * DOWN)))
        self.wait()
...
class ShowVaryingExpFactor(Scene):
    ...
    def construct(self):
        ...
        arrow = Arrow(factor.get_right(), factor.get_right() + 4 * RIGHT + 0.5 * DOWN)
        self.add(rect, factor, arrow)
        for value in [0.05, 0.25, 0.15]:
            self.play(ChangeDecimalToValue(factor, value), run_time=3)
        self.wait()
...
class ShowVaryingExpCurve(ShowLogisticCurve):
    ...
    def construct(self):
        ...
        for value in [0.05, 0.25, 0.15]:
            ...
            self.play(Transform(curve, new_curve), run_time=3)
