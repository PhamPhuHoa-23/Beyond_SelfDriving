...
class Thumbnail1(Scene):
    ...
    def construct(self):
        ...
        n1 = '50'
        n2 = '200'
        t2c = {p1: BLUE, p2: YELLOW, n1: BLUE_C, n2: YELLOW}
        kw = {'tex_to_color_map': t2c}
        text = VGroup(OldTexText(f'{p1} with {n1} reviews', **kw), OldTexText('vs.', **kw), OldTexText(f'{p2} with {n2} reviews', **kw))
        text.scale(2)
...
class AltThumbnail1(Scene):
    ...
    def construct(self):
        ...
        hist = Histogram(counts, y_max=0.3, y_tick_freq=0.05, y_axis_numbers_to_show=[10, 20, 30], x_label_freq=10)
...
class Thumbnail2(Scene):
    ...
    def construct(self):
        ...
        self.add(graph)
        self.add(new_graph)
        arrow = Arrow(axes.input_to_graph_point(0.5, graph), axes.input_to_graph_point(0.8, new_graph), path_arc=-90 * DEGREES, buff=0.3)
        self.add(arrow)
        formula = OldTex('P(H|D) = {P(H)P(D|H) \\over P(D)}', tex_to_color_map={'H': YELLOW, 'D': GREEN})
...
class Thumbnail3(Thumbnail2):
    ...
    def construct(self):
        ...
        self.add(axes)
        self.add(graph)
        label = OldTex('\\text{Beta}(10, 3)', tex_to_color_map={'10': GREEN, '3': RED})
...
class HighlightReviewParts(Scene):
    ...
    def construct(self):
        ...
        for rect, rating, nr, color in zip(rects, ratings, n_reviews, colors):
            label = OldTex(f'{nr}', '\\text{ reviews }', f'{rating}', '\\%')
        ...
        for fsr, label in tuples:
            if curr_fsr is fsr:
                ...
                self.play(Transform(curr_fsr, fsr), MoveToTarget(curr_label))
        ...
        self.play(FadeOut(curr_fsr), FadeIn(br))
        self.wait()
...
class ShowThreeCases(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeInFrom, titles, lambda m: (m, DOWN), lag_ratio=0.2))
        self.play(LaggedStart(*[LaggedStartMap(FadeInFromLarge, review, lag_ratio=0.1) for review in reviews], lag_ratio=0.1))
        self.add(reviews)
        self.wait()
        ...
        randy.flip()
        randy.set_height(2)
        randy.next_to(reviews[0], RIGHT, LARGE_BUFF, aligned_edge=UP)
        ...
        for ar in alt_reviews:
            for m1, m2 in zip(ar, review):
                m1.replace(m2)
        alt_percents = VGroup(*[OldTex(str(10 * (10 - k)) + '\\%') for k in alt_negs])
        hundo = titles[0][0]
        ...
        for ar, ap in zip(alt_reviews, alt_percents):
            self.play(FadeIn(ar, 0.5 * DOWN, lag_ratio=0.2), FadeOut(last_review), FadeIn(ap, 0.5 * DOWN), FadeOut(last_percent, 0.5 * UP), run_time=1.5)
            last_review = ar
            last_percent = ap
        ...
        self.clear()
        self.play(MoveToTarget(lower_reviews), MoveToTarget(nums), FadeOut(titles[1][1:]), FadeOut(titles[2][1:]), FadeOut(titles[0]), FadeOut(reviews[0]), run_time=2)
        greater_than = OldTex('>')
        greater_than.scale(2)
        greater_than.move_to(midpoint(reviews[2].get_right(), reviews[1].get_left()))
        ...
        squares = VGroup(*[SurroundingRectangle(submob, buff=0.01, stroke_color=GREY_B, stroke_width=1) for submob in reviews[2]])
        squares.shuffle()
        self.play(LaggedStartMap(ShowCreationThenFadeOut, squares, lag_ratio=0.5 / len(squares), run_time=3), Write(greater_than))
        self.wait()
        self.play(MoveToTarget(nums), TransformFromCopy(greater_than, less_than))
        self.wait()

    def get_titles(self):
        titles = VGroup(OldTexText('$100\\%$ \\\\', '10 reviews'), OldTexText('$96\\%$ \\\\', '50 reviews'), OldTexText('$93\\%$ \\\\', '200 reviews'))
        colors = [PINK, BLUE, YELLOW]
    ...
    def get_reviews(self, titles):
        return VGroup(self.get_plusses_and_minuses(titles[0], 5, 2, 0), self.get_plusses_and_minuses(titles[1], 5, 10, 2), self.get_plusses_and_minuses(titles[2], 8, 25, 14))
    ...
    def get_plusses_and_minuses(self, title, n_rows, n_cols, n_minus):
        ...
        checks = VGroup(*[check.copy() for x in range(n_rows * n_cols)])
...
class PreviewThreeVideos(Scene):
    ...
    def construct(self):
        equations = VGroup(OldTex('{10', '\\over', '10}', '=', '100\\%'), OldTex('{48', '\\over', '50}', '=', '96\\%'), OldTex('{186', '\\over', '200}', '=', '93\\%'))
        ...
        topics = VGroup(OldTexText('Binomial distributions'), OldTexText('Bayesian updating'), OldTexText('Probability density functions'), OldTexText('Beta distribution'))
        ...
        for topic in topics:
            ...
            bullets.add(bullet)
        self.play(Write(title), Write(bullets), run_time=1)
        self.play(LaggedStart(*[FadeIn(topic, lag_ratio=0.1) for topic in topics], run_time=3, lag_ratio=0.3))
        self.wait()
        images = [ImageMobject(os.path.join(consts.VIDEO_DIR, OUTPUT_DIRECTORY, 'images', name)) for name in ['Thumbnail1', 'Thumbnail2', 'Thumbnail3']]
        thumbnails = Group()
        for image in images:
        ...
        topics[2].target.next_to(topics[1].target, DOWN, aligned_edge=LEFT)
        self.play(FadeOut(title, LEFT), FadeOut(bullets, LEFT), LaggedStartMap(MoveToTarget, topics), LaggedStartMap(FadeIn, thumbnails))
        self.wait()
        tn_groups = Group(Group(thumbnails[0], topics[0]), Group(thumbnails[1], topics[1], topics[2]), Group(thumbnails[2], topics[3]))
        ...
        self.play(FadeIn(setup_words, lag_ratio=0.1), topics[0].next_to, setup_words, DOWN, {'aligned_edge': LEFT})
        tn_groups[0].add(setup_words)
        self.wait(2)
        for i in (0, 1):
            self.play(ApplyFunction(lambda m: set_opacity(m, 0.2), tn_groups[i]), ApplyFunction(lambda m: set_opacity(m, 1), tn_groups[i + 1]))
            self.wait(2)
        self.play(FadeIn(analysis_words, 0.25 * UP))
        tn_groups[2].add(analysis_words)
        self.wait(2)
        self.play(FadeOut(setup_words), FadeOut(topics[0]), FadeOut(tn_groups[1]), FadeOut(tn_groups[2]), FadeOut(vs_labels, UP), FadeOut(equations, UP), ApplyFunction(lambda m: set_opacity(m, 1), thumbnails[0]))
...
class LetsLookAtOneAnswer(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.teacher_says('Let me show you\\\\one answer.', added_anims=[self.change_students('pondering', 'thinking', 'pondering')])
...
class LaplacesRuleOfSuccession(Scene):
    ...
    def construct(self):
        ...
        def get_review_label(num, denom):
            result = VGroup(Integer(num, color=GREEN), OldTexText('out of'), Integer(denom))
        ...
        self.play(ShowIncreasingSubsets(real_data, int_func=np.ceil), CountInFrom(review_label[0], 0), rate_func=lambda t: smooth(t, 3))
        self.wait()
        ...
        self.play(FadeIn(fd_label, UP), DrawBorderThenFill(fd_rect), ShowCreation(underlines[10:]))
        self.wait()
        for mark in data[10:]:
        ...
        self.play(MoveToTarget(old_review_label), FadeIn(arrow), eq.next_to, arrow, RIGHT, FadeOut(VGroup(fake_data, underlines, pretend_label, review_label, fd_rect, fd_label), DOWN, lag_ratio=0.01), real_data.match_width, old_review_label.target, real_data.next_to, old_review_label.target, DOWN)
        self.wait()
        data = get_checks_and_crosses(48 * [True] + 2 * [False] + [True, False], width=FRAME_WIDTH - 1)
        data.shift(DOWN)
        underlines = get_underlines(data)
        ...
        self.play(FadeIn(underlines[:-2]), ShowIncreasingSubsets(true_data, int_func=np.ceil), CountInFrom(review_label[0], 0), UpdateFromAlphaFunc(review_label, lambda m, a: m.set_opacity(a)))
        self.wait()
        self.play(FadeIn(fd_label), GrowArrow(fd_arrow), FadeIn(fd_rect), Write(fake_data), Write(underlines[-2:]))
        self.wait()
        old_review_label = VGroup(Integer(0), OldTexText('out of'), Integer(0))
        old_review_label.become(review_label)
        review_center = VectorizedPoint(review_label.get_center())
        self.play(review_center.set_x, data.get_center()[0] + 3, MaintainPositionRelativeTo(review_label, review_center), UpdateFromAlphaFunc(review_label[0], lambda m, a: m.set_value(int(interpolate(48, 49, a)))), UpdateFromAlphaFunc(review_label[2], lambda m, a: m.set_value(int(interpolate(50, 52, a)))), old_review_label.scale, 0.5, old_review_label.to_edge, LEFT)
        self.wait()
        arrow2 = Vector(0.5 * RIGHT)
        arrow2.next_to(old_review_label, RIGHT)
        eq2 = OldTex('{49', '\\over', '52}', '\\approx', '94.2\\%')
        fix_percent(eq2[-1][-1])
        ...
        self.play(Restore(eq2, run_time=1.5), FadeIn(arrow2))
        self.wait()
        faders = VGroup(fd_rect, fd_arrow, fd_label, fake_data, underlines, review_label)
        self.play(FadeOut(faders), true_data.match_width, old_review_label, true_data.next_to, old_review_label, DOWN)
        ...
        final_review_label.shift(arrow2.get_center() - arrow.get_center())
        ...
        true_data = data[:-2]
        fake_data = data[-2:]
        self.play(UpdateFromAlphaFunc(final_review_label, lambda m, a: m.set_opacity(a)), CountInFrom(final_review_label[0], 0), ShowIncreasingSubsets(true_data))
        self.wait()
        arrow3 = Vector(0.5 * RIGHT)
        arrow3.next_to(final_review_label, RIGHT)
        eq3 = OldTex('{187', '\\over', '202}', '\\approx', '92.6\\%')
        fix_percent(eq3[-1][-1])
        eq3[0].set_color(GREEN)
        eq3.next_to(arrow3, RIGHT)
        self.play(GrowArrow(arrow3), FadeIn(eq3), Write(fake_data))
        self.wait()
        self.play(true_data.match_width, final_review_label, true_data.next_to, final_review_label, DOWN, FadeOut(fake_data))
        self.wait()
        rect = SurroundingRectangle(VGroup(eq2, old_review_label))
        rect.set_stroke(YELLOW, 2)
        self.play(ShowCreation(rect), eq2[-1].set_color, YELLOW)
        self.wait()
        name = OldTexText("Laplace's rule of succession")
        name.match_height(title)
        name.move_to(title)
        name.set_color(TEAL)
        self.play(FadeInFromDown(name), FadeOut(title, UP), underline.match_width, name)
        self.wait()

class AskWhy(TeacherStudentsScene):

    def construct(self):
        self.student_says('Wait...why?', look_at=self.screen)
        self.play(self.students[0].change, 'confused', self.screen, self.students[1].change, 'confused', self.screen, self.teacher.change, 'tease', self.students[2].eyes)
        self.wait(3)
        self.student_says('Is that really\\\\the answer?', target_mode='raise_right_hand', added_anims=[self.teacher.change, 'thinking'])
        self.wait(2)
...
class WhatsTheModel(Scene):
    ...
    def add_questions(self):
        questions = VGroup(OldTexText("What's the model?"), OldTexText('What are you optimizing?'))
    ...
    def introduce_buyer_and_seller(self):
        if hasattr(self, 'questions'):
            ...
            added_anims = [questions[0].to_edge, UP, questions[1].set_opacity, 0.5, questions[1].scale, 0.25, questions[1].to_corner, UR]
        else:
            added_anims = []
        ...
        self.play(LaggedStartMap(FadeInFromDown, VGroup(seller, buyer, *labels), lag_ratio=0.2), *added_anims)
        self.wait()
        self.buyer = buyer
        self.seller = seller
    ...
    def add_probability_label(self):
        ...
        self.add(label)
        self.play(GrowFromCenter(brace), FadeIn(success_rate, 0.5 * DOWN))
        self.wait()
        self.play(TransformFromCopy(success_rate[0], s_sym), FadeOut(success_rate, 0.1 * RIGHT, lag_ratio=0.1))
        for x in range(2):
        ...
        self.play(FadeOut(rhs, 0.5 * DOWN), FadeIn(grey_box, 0.5 * UP), FadeIn(lil_q_marks, DOWN), buyer.change, 'confused', grey_box)
        rhs.set_opacity(0)
        for x in range(2):
            self.play(*self.experience_animations(seller, buyer, arc=30 * DEGREES))
        self.play(buyer.change, 'confused', lil_q_marks)
        self.play(Blink(buyer))
        self.prob_group = VGroup(label, grey_box, brace, s_sym)
    ...
    def bring_up_goal(self):
        ...
        self.play(FadeOut(self.buyer_q_marks), self.buyer.change, 'pondering', questions[0], self.seller.change, 'pondering', questions[0], MoveToTarget(prob_group), MoveToTarget(questions))
    ...
    def experience_animations(self, seller, buyer, arc=-30 * DEGREES, p=0.75):
        ...
        words = OldTexText('Positive\\\\experience' if positive else 'Negative\\\\experience')
        words.set_color(GREEN if positive else RED)
        if positive:
            new_mode = random.choice(['hooray', 'coin_flip_1'])
        else:
            new_mode = random.choice(['tired', 'angry', 'sad'])
        words.move_to(seller.get_corner(UR))
        result = [ApplyMethod(words.move_to, buyer.get_corner(UL), path_arc=arc, run_time=2), VFadeInThenOut(words, run_time=2), ApplyMethod(buyer.change, new_mode, seller.eyes, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)), ApplyMethod(seller.change, 'coin_flip_2', buyer.eyes, rate_func=there_and_back)]
        return result
...
class IsSellerOne100(Scene):
    ...
    def add_review(self):
        ...
        blanks = VGroup(*[Line(LEFT, RIGHT).match_width(rev).next_to(rev, DOWN, SMALL_BUFF) for rev in reviews])
        blanks.shift(0.25 * reviews[0].get_width() * LEFT)
        label = OldTexText(' out of ')
        ...
        self.play(ShowIncreasingSubsets(reviews, int_func=np.ceil), UpdateFromAlphaFunc(tens[0], lambda m, a: m.set_color(interpolate_color(RED, GREEN, a)).set_value(len(m.to_count))), run_time=2, rate_func=bezier([0, 0, 1, 1]))
        self.wait()
    ...
    def show_probability(self):
        ...
        arrow = Arrow(not_necessarily.get_top(), rhs_rect.get_corner(DL), buff=MED_SMALL_BUFF)
        arrow.set_color(RED)
        rhs.set_value(0)
        self.play(ChangeDecimalToValue(rhs, 1), UpdateFromAlphaFunc(prob_label, lambda m, a: m.set_opacity(a)), FadeIn(q_mark))
        self.wait()
        self.play(ShowCreation(rhs_rect), Write(not_necessarily), ShowCreation(arrow), review_group.to_edge, DOWN, run_time=1)
        self.wait()
        self.play(ChangeDecimalToValue(rhs, 0.95), FadeOut(rhs_rect), FadeOut(arrow), FadeOut(not_necessarily))
        self.wait()
        self.prob_label_group = VGroup(prob_label, rhs, q_mark)
    ...
    def show_simulated_reviews(self):
        ...
        self.play(review_group.scale, 0.25, review_group.to_corner, UR, Write(random_label), CountInFrom(decimals[0], 0))
        self.wait()
        self.play(FadeIn(lt_p95, LEFT))
        self.play(GrowArrow(arrows[0]), FadeIn(marks[0], DOWN))
        self.wait()
        self.play(FadeOut(lt_p95, 0.5 * RIGHT), FadeIn(gte_p95, 0.5 * LEFT))
        self.play(random_label.match_x, decimals[1], CountInFrom(decimals[1], 0), UpdateFromAlphaFunc(decimals[1], lambda m, a: m.set_opacity(a)))
        self.play(GrowArrow(arrows[1]), FadeIn(marks[1], DOWN))
        self.wait()
        self.play(LaggedStartMap(CountInFrom, decimals[2:]), UpdateFromAlphaFunc(decimals[2:], lambda m, a: m.set_opacity(a)), FadeOut(gte_p95), run_time=1)
        self.add(decimals)
        self.play(LaggedStartMap(GrowArrow, arrows[2:]), LaggedStartMap(FadeInFromDown, marks[2:]), run_time=1)
        ...
        rows = VGroup(marks)
        for x in range(3):
            self.play(arrows.shift, DOWN, UpdateFromFunc(decimals, randomize_decimals), *added_anims)
            added_anims = []
        ...
        self.play(MoveToTarget(rows), FadeOut(decimals), FadeOut(arrows))
        self.play(ShowIncreasingSubsets(added_rows), run_time=3)
        all_rows = VGroup(*rows, *added_rows)
        scores = VGroup()
        ten_rects = VGroup()
        for row in all_rows:
            score = Integer(sum([mark.get_color() == Color(GREEN) for mark in row]))
        ...
        self.add(rect)
        self.add(prob)
        self.play(LaggedStartMap(FadeOutAndShift, all_rows, lag_ratio=0.01), LaggedStartMap(FadeOutAndShift, scores, lag_ratio=0.01), Restore(review_group))
    ...
    def show_random_numbers(self):
        prob_label_group = self.prob_label_group
        random.seed(2)
        rows = VGroup(*[VGroup(*[Integer(random.randint(0, 99)).move_to(0.85 * x * RIGHT) for x in range(10)]) for y in range(10 * 2)])
        rows.arrange_in_grid(n_cols=2, buff=MED_LARGE_BUFF)
        rows[:10].shift(LEFT)
        rows.set_height(5.5)
        rows.center().to_edge(DOWN)
        lt_95 = VGroup(*[mob for row in rows for mob in row if mob.get_value() < 95])
        square = Square()
        square.set_stroke(width=0)
        square.set_fill(YELLOW, 0.5)
        square.set_width(1.5 * rows[0][0].get_height())
        row_rects = VGroup(*[SurroundingRectangle(row) for row in rows if all([m.get_value() < 95 for m in row])])
        row_rects.set_stroke(GREEN, 2)
        self.play(LaggedStartMap(ShowIncreasingSubsets, rows, run_time=3, lag_ratio=0.25), FadeOut(self.review_group, DOWN), prob_label_group.set_height, 0.75, prob_label_group.to_corner, UL)
        self.wait()
        self.play(lt_95.set_fill, BLUE, lt_95.set_stroke, BLUE, 2, {'background': True})
        self.wait()
...
class LookAtAllPossibleSuccessRates(Scene):
    ...
    def construct(self):
        ...
        sellers = VGroup(*[self.get_example_seller(label.get_value()) for label in x_labels])
        ...
        y_axis_label = OldTexText('A kind of probability\\\\', 'of probabilities')
        y_axis_label.scale(0.75)
        y_axis_label.next_to(axes.y_axis, RIGHT)
        y_axis_label.to_edge(UP)
        y_axis_label[1].set_color(YELLOW)
        graph_label = OldTexText('Some notion of likelihood\\\\', 'for each one')
        ...
        self.play(LaggedStartMap(FadeInFromPoint, shuffled_sellers[1:], lambda m: (m, sellers.get_center())))
        self.wait()
        self.play(MoveToTarget(sellers), FadeIn(axes), run_time=2)
        self.play(x_label.shift, 4 * RIGHT, UpdateFromAlphaFunc(x_label, lambda m, a: m.set_opacity(a), rate_func=there_and_back), ShowCreation(x_axis_line), run_time=3)
        self.play(FadeOut(x_axis_line))
        self.wait()
        self.play(FadeInFromDown(graph_label), ReplacementTransform(flat_graph, graph))
        self.wait()
        ...
        prob_label.decimal.add_updater(lambda m: m.set_value(m.tracker.get_value()))
        v_line = Line(DOWN, UP)
        v_line.set_stroke(YELLOW, 2)
        v_line.tracker = x_tracker
        v_line.graph = graph
        v_line.axes = axes
        v_line.add_updater(lambda m: m.put_start_and_end_on(m.axes.x_axis.n2p(m.tracker.get_value()), m.axes.input_to_graph_point(m.tracker.get_value(), m.graph)))
        self.add(v_line)
        for x in [0.95, 0.8, 0.9]:
            self.play(x_tracker.set_value, x, run_time=4)
        self.wait()
...
class AskAboutUnknownProbabilities(Scene):
    ...
    def construct(self):
        ...
        processes = VGroup(get_random_coin(shuffle_time=1), get_random_die(shuffle_time=1.5), get_random_card(shuffle_time=2))
        ...
        prob_labels = VGroup(OldTex('P(', '00', ')', '=', '1 / 2'), OldTex('P(', '00', ')', '=', '1 / 6}'), OldTex('P(', '00', ')', '=', '1 / 52}'))
        ...
        number_rects = VGroup(*[SurroundingRectangle(pl[-1]) for pl in prob_labels])
        ...
        for pl in prob_labels:
            ...
            pl.set_opacity(0)
        self.add(processes)
        self.play(LaggedStartMap(FadeInFromDown, titles), LaggedStart(ShowCreation(v_line), ShowCreation(h_line), lag_ratio=0.1), LaggedStartMap(Restore, prob_labels), run_time=1)
        self.wait(10)
        fade_rects = VGroup(*[VGroup(BackgroundRectangle(pl, buff=MED_SMALL_BUFF), BackgroundRectangle(pr, buff=MED_SMALL_BUFF)) for pl, pr in zip(prob_labels, processes)])
        ...
        self.play(FadeIn(fade_rects[1]), FadeIn(fade_rects[2]))
        self.wait(2)
        self.play(FadeIn(fade_rects[0]), FadeOut(fade_rects[1]))
        self.wait(3)
        self.play(FadeOut(fade_rects[0]), FadeOut(fade_rects[2]))
        self.wait(4)
        processes.suspend_updating()
        self.play(LaggedStart(FadeOut(unknown_title, UP), FadeOut(prob_title, UP), lag_ratio=0.2), FadeOut(h_line, UP, lag_ratio=0.1), FadeOut(processes, LEFT, lag_ratio=0.1), FadeOut(prob_labels[1]), FadeOut(prob_labels[2]), v_line.rotate, 90 * DEGREES, v_line.shift, 0.6 * FRAME_HEIGHT * UP, prob_half.center, prob_half.to_edge, UP, run_time=2)
        self.clear()
        self.add(prob_half)
        arrow = Vector(UP)
        ...
        self.play(GrowArrow(arrow), FadeIn(question, UP))
        self.wait(2)
        self.play(FadeOut(question, RIGHT), Rotate(arrow, 90 * DEGREES), VFadeOut(arrow))
        ...
        self.play(FadeOut(half, UP), FadeIn(q_marks, DOWN))
        self.play(FadeIn(randy))
        self.play(Blink(randy))
        self.wait()
    ...
    def show_many_coins(self, n_rows, n_cols):
        coin_choices = VGroup(get_coin('H'), get_coin('T'))
        coin_choices.set_stroke(width=0)
        coins = VGroup(*[random.choice(coin_choices).copy() for x in range(n_rows * n_cols)])
        ...
        self.play(LaggedStartMap(FadeInFrom, coins, lambda m: (m, 0.2 * DOWN), run_time=3, rate_func=linear))
        self.wait()
        self.play(LaggedStartMap(MoveToTarget, coins, path_arc=30 * DEGREES, run_time=2, lag_ratio=1 / len(coins)))
        self.wait()
...
class AskProbabilityOfCoins(Scene):
    ...
    def construct(self):
        condition = VGroup(OldTexText("If you've seen"), Integer(80, color=BLUE_C), get_coin('H').set_height(0.5), OldTexText('and'), Integer(20, color=RED_C), get_coin('T').set_height(0.5))
        condition.arrange(RIGHT)
        condition.to_edge(UP)
        self.add(condition)
        question = OldTex('\\text{What is }', 'P(', '00', ')', '?')
        coin = get_coin('H')
        coin.replace(question.get_part_by_tex('00'))
        question.replace_submobject(question.index_of_part_by_tex('00'), coin)
        question.next_to(condition, DOWN)
        self.add(question)
        values = ['H'] * 80 + ['T'] * 20
        random.shuffle(values)
        coins = VGroup(*[get_coin(symbol) for symbol in values])
        coins.arrange_in_grid(10, 10, buff=MED_SMALL_BUFF)
        coins.set_width(5)
        coins.next_to(question, DOWN, MED_LARGE_BUFF)
        self.play(ShowIncreasingSubsets(coins), run_time=8, rate_func=bezier([0, 0, 1, 1]))
        self.wait()
        self.embed()
...
class RunCarFactory(Scene):
    ...
    def construct(self):
        ...
        for car in (cars[4], cars[6]):
            ...
            car.add(scratch)
        self.add(cars, rect)
        self.play(LaggedStartMap(MoveCar, cars, lambda m: (m, m.get_corner(DR) + 10 * RIGHT), lag_ratio=0.3, rate_func=linear, run_time=1.5 * n_cars))
        self.remove(cars)

class CarFactoryNumbers(Scene):

    def construct(self):
        denom_words = OldTexText('in a test of 100 cars', tex_to_color_map={'100': BLUE})
        ...
        self.wait()
        self.play(denom_words.next_to, numer_words, DOWN, {'aligned_edge': LEFT}, FadeIn(numer_words))
        self.wait()
        question = VGroup(OldTexText('How do you plan'), OldTexText('for'), Integer(int(1000000.0), color=BLUE), OldTexText('cars?'))
        question[1:].arrange(RIGHT, aligned_edge=DOWN)
        question[2].shift((question[2][1].get_bottom()[1] - question[2][0].get_bottom()[1]) * UP)
        question[1:].next_to(question[0], DOWN, aligned_edge=LEFT)
        question.next_to(denom_words, DOWN, LARGE_BUFF, aligned_edge=LEFT)
        self.play(UpdateFromAlphaFunc(question, lambda m, a: m.set_opacity(a)), CountInFrom(question[2], 0, run_time=1.5))
        self.wait()
...
class ComplainAboutSimplisticModel(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.add(axes)
        self.play(self.teacher.change, 'raise_right_hand', axes, self.change_students('pondering', 'erm', 'sassy', look_at=axes), ShowCreation(axes.graph, run_time=3, rate_func=linear))
        self.wait(2)
        student = self.students[2]
        bubble = SpeechBubble(direction=LEFT, height=3, width=5)
        bubble.pin_to(student)
        bubble.write('What about something\\\\like this?')
        self.play(axes.next_to, student, UL, VFadeOut(axes.graph), FadeIn(bubble), Write(bubble.content, run_time=1), student.change, 'raise_left_hand', self.students[0].change, 'thinking', axes, self.students[1].change, 'thinking', axes, self.teacher.change, 'happy')
        new_graph = VMobject()
        new_graph.set_points_as_corners([axes.c2p(0, 0.75), axes.c2p(2, 0.9), axes.c2p(4, 0.5), axes.c2p(6, 0.75), axes.c2p(8, 0.55), axes.c2p(10, 0.95)])
        new_graph.make_smooth()
        new_graph.set_stroke([YELLOW, RED, GREEN], 2)
        self.play(ShowCreation(new_graph), *[ApplyMethod(pi.look_at, new_graph) for pi in self.pi_creatures])
        self.wait(3)

    def get_experience_graph(self):
        axes = Axes(x_min=-1, x_max=10, y_min=0, y_max=1.25, y_axis_config={'unit_size': 5, 'tick_frequency': 0.25, 'include_tip': False})
        ...
        lines = VGroup()
        for x in range(10):
            lines.add(Line(axes.c2p(x, 0), axes.c2p(x + 0.9, 0)))
...
class PreviewBeta(Scene):
    ...
    def construct(self):
        ...
        def update_graph(container):
            ...
            return container
        self.add(axes)
        self.play(ShowIncreasingSubsets(marks), UpdateFromFunc(beta_container, update_beta), UpdateFromFunc(graph_container, update_graph), run_time=15, rate_func=bezier([0, 0, 1, 1]))
        self.wait()
...
class AskInverseQuestion(WhatsTheModel):
    ...
    def construct(self):
        self.force_skipping()
        self.introduce_buyer_and_seller()
        self.bs_group = VGroup(self.buyer, self.seller, self.buyer.label, self.seller.label)
    ...
    def show_many_review_animations(self):
        for x in range(7):
            self.play(*self.experience_animations(self.seller, self.buyer, arc=30 * DEGREES, p=0.95))

    def ask_question(self):
        pis = [self.buyer, self.seller]
        labels = VGroup(self.get_prob_review_label(10, 0), self.get_prob_review_label(48, 2), self.get_prob_review_label(186, 14))
        labels.arrange(DOWN)
        labels.to_edge(UP)
        labels[0].save_state()
        ...
        self.wait()
        self.play(Restore(labels[0]), *[ApplyMethod(pi.change, 'pondering', labels) for pi in pis])
        self.play(Blink(pis[0]))
        self.play(Blink(pis[1]))
        self.play(LaggedStartMap(FadeInFromDown, labels[1:]))
        self.wait(2)
        short_label = OldTex('P(\\text{data} | s)', tex_to_color_map={'\\text{data}': GREY_B, 's': YELLOW})
        short_label.scale(2)
        ...
        self.play(FadeIn(short_label, UP), bs_group.scale, 0.5, {'about_edge': DOWN})
        self.play(ShowCreation(rect))
        self.wait()

    def get_prob_review_label(self, n_positive, n_negative):
        label = OldTex('P(', f'{n_positive}\\,{CMARK_TEX}', ',\\,', f'{n_negative}\\,{XMARK_TEX}', '\\,\\text{ Given that }', 's = 0.95', ')')
        label.set_color_by_tex_to_color_map({CMARK_TEX: GREEN, XMARK_TEX: RED, '0.95': YELLOW})
        return label

class SimulationsOf10Reviews(Scene):
    CONFIG = {'s': 0.95, 'histogram_height': 5, 'histogram_width': 10}
    ...
    def construct(self):
        ...
        count.add_updater(lambda m: m.set_value(sum([s.positive for s in row.syms])))
        ...
        self.play(UpdateFromFunc(row.nums, update_nums), run_time=2)
        row.nums.restore()
        self.wait()
        self.add(count)
        self.play(ShowIncreasingSubsets(row.syms), run_time=2, rate_func=linear)
        ...
        self.play(FadeIn(histogram), ApplyFunction(put_into_histogram, VGroup(row, count)))
        self.wait()
        for x in range(2):
            row = get_random_num_row(self.s)
            count = self.get_count(row)
            ...
            self.wait(0.5)
            self.play(ApplyFunction(put_into_histogram, VGroup(row, count)))
        for x in range(40):
            row = get_random_num_row(self.s)
            count = self.get_count(row)
        ...
        self.add(row, count)
        self.play(FadeOut(stacks), FadeOut(count), histogram.bars.become, histogram.get_bars(data), histogram.axes.y_labels.set_opacity, 1, histogram.axes.h_lines.set_opacity, 1, histogram.axes.y_axis.set_opacity, 1)
        self.remove(stacks)
        ...
        self.add(arrow)
        self.play(UpdateFromFunc(Group(row, arrow, histogram.bars), update), run_time=10)

    def get_histogram(self, data):
        histogram = Histogram(data, bar_colors=[RED, RED, BLUE, GREEN], height=self.histogram_height, width=self.histogram_width)
...
class SimulationsOf10ReviewsSquished(SimulationsOf10Reviews):
    CONFIG = {'histogram_height': 2, 'histogram_width': 11}
...
class SimulationsOf50Reviews(Scene):
    CONFIG = {'s': 0.95, 'histogram_config': {'x_label_freq': 5, 'y_axis_numbers_to_show': range(10, 70, 10), 'y_max': 0.6, 'y_tick_freq': 0.1, 'height': 5, 'bar_colors': [BLUE]}, 'random_seed': 1}
    ...
    def construct(self):
        ...
        count.set_value(0)
        self.add(histogram)
        self.play(ShowIncreasingSubsets(row), ChangeDecimalToValue(count, original_count))
        arrow = Vector(0.5 * DOWN)
        arrow.set_stroke(width=5)
        arrow.set_color(YELLOW)
        arrow.next_to(histogram.bars[10], UP, SMALL_BUFF)
        total_data_label = VGroup(OldTexText('Total samples: '), Integer(1))
        total_data_label.arrange(RIGHT)
        total_data_label.next_to(row, DOWN)
        total_data_label.add_updater(lambda m: m[1].set_value(data.sum()))
        ...
        self.play(UpdateFromFunc(group, update), run_time=4)
        self.play(UpdateFromFunc(group, lambda m: update(m, 1000)), run_time=4)
        random.seed(0)
        np.random.seed(0)
        update(group)
        self.wait()
        ...
        line = DashedLine(axes.c2p(0, y), axes.c2p(51, y))
        label = OldTex('{:.1f}\\%'.format(100 * y))
        fix_percent(label.family_members_with_points()[-1])
        label.next_to(line, RIGHT)
        self.play(ShowCreation(line), FadeInFromPoint(label, line.get_start()))
    ...
    def get_histogram(self, data):
        histogram = Histogram(data, **self.histogram_config)
        histogram.to_edge(DOWN)
        return histogram
...
class ShowBinomialFormula(SimulationsOf50Reviews):
    CONFIG = {'histogram_config': {'x_label_freq': 5, 'y_axis_numbers_to_show': range(10, 40, 10), 'y_max': 0.3, 'y_tick_freq': 0.1, 'height': 2.5, 'bar_colors': [BLUE]}, 'random_seed': 0}

    def construct(self):
        dist = scipy.stats.binom(50, self.s)
        data = np.array([dist.pmf(x) for x in range(0, 51)])
        histogram = self.get_histogram(data)
        histogram.bars.set_fill(GREY_C)
        histogram.bars[48].set_fill(GREEN)
        self.add(histogram)
        ...
        equation = VGroup(prob_label, eq, formula)
        ...
        for mob in prob_label[1::2]:
            ...
            self.wait(0.5)
        self.play(Restore(prob_label), FadeIn(equation[1:], lag_ratio=0.1))
        self.wait()
        ...
        self.play(FadeIn(value, LEFT), FadeIn(eq2), equation.next_to, eq2, LEFT)
        self.wait()
        n = 50
        ...
        self.play(long_equation.next_to, self.slots, DOWN, MED_LARGE_BUFF, long_equation.to_edge, RIGHT, FadeIn(bin_name, DOWN))
        self.wait()
        ...
        self.wait()
        likelihood_label = OldTex('P(', '\\text{data}', '\\,|\\,', '\\text{success rate}', ')')
        posterior_label = OldTex('P(', '\\text{success rate}', '\\,|\\,', '\\text{data}', ')')
        for label in (likelihood_label, posterior_label):
            label.set_color_by_tex_to_color_map({'data': GREEN, 'success': YELLOW})
        likelihood_label.next_to(prob_label, DOWN, LARGE_BUFF, aligned_edge=LEFT)
        ...
        self.play(FadeIn(likelihood_label, UP), bin_name.set_height, 0.4, bin_name.set_y, histogram.axes.c2p(0, 0.25)[1])
        self.wait()
        self.play(GrowArrow(right_arrow), FadeIn(ra_label, 0.5 * LEFT))
        anims = []
        for i, j in enumerate([0, 3, 2, 1, 4]):
            anims.append(TransformFromCopy(likelihood_label[i], posterior_label[j], path_arc=-45 * DEGREES, run_time=2))
        self.play(*anims)
        self.add(posterior_label)
        self.wait()
        histogram.add(bin_name)
        always(arrow.next_to, histogram.bars[48], UP, SMALL_BUFF)
        self.play(FadeOut(likelihood_label), FadeOut(posterior_label), FadeOut(right_arrow), FadeOut(ra_label), FadeOut(row, UP), FadeOut(self.slots, UP), histogram.scale, 0.7, histogram.to_edge, UP, arrow.scale, 0.5, arrow.set_stroke, None, 4, long_equation.center, run_time=1.5)
        self.add(arrow)
        low_axes = get_beta_dist_axes(y_max=0.3, y_unit=0.1, label_y=False)
        low_axes.y_axis.set_height(2, about_point=low_axes.c2p(0, 0), stretch=True)
        ...
        s_tip.st = s_tracker
        s_tip.add_updater(lambda m: m.next_to(m.axis.n2p(m.st.get_value()), UP, buff=0))
        ...
        self.add(pl_decimal)
        self.play(ShowCreation(sl_rect), Write(low_axes))
        self.play(s_label.next_to, s_tip, UP, 0.2, ORIGIN, s_label[1], ReplacementTransform(sl_rect, s_tip))
        ...
        def update_histogram(hist):
            ...
            new_data = np.array([new_dist.pmf(x) for x in range(0, 51)])
        ...
        bar_copy.add_updater(lambda m: m.set_height(max(histogram.bars[48].get_height() * 0.75, 1e-06), stretch=True, about_edge=DOWN))
        self.add(bar_copy)
        self.add(histogram)
        self.add(decimals)
        for s in [0.95, 0.5, 0.99, 0.9]:
            self.play(s_tracker.set_value, s, UpdateFromFunc(decimals, update_decimals), UpdateFromFunc(histogram, update_histogram), UpdateFromFunc(value, lambda m: m), UpdateFromFunc(s_label, lambda m: m.update), run_time=5)
            self.wait()

        def func(x):
            return scipy.stats.binom(50, x).pmf(48) + 1e-05
        ...
        v_line.st = s_tracker
        v_line.graph = graph
        v_line.add_updater(lambda m: m.put_start_and_end_on(m.axes.c2p(m.st.get_value(), 0), m.axes.input_to_graph_point(m.st.get_value(), m.graph)))
        v_line.set_stroke(GREEN, 2)
        dot = Dot()
        dot.line = v_line
        dot.set_height(0.05)
        dot.add_updater(lambda m: m.move_to(m.line.get_end()))
        self.play(ApplyMethod(histogram.bars[48].stretch, 2, 1, {'about_edge': DOWN}, rate_func=there_and_back, run_time=2))
        self.wait()
        self.play(low_axes.y_axis.set_opacity, 1)
        self.play(FadeIn(graph), FadeOut(s_label), FadeOut(s_tip))
        self.play(TransformFromCopy(histogram.bars[48], v_line), FadeIn(dot))
        self.add(histogram)
        decimals.remove(decimals[1])
        for s in [0.9, 0.96, 1, 0.8, 0.96]:
            self.play(s_tracker.set_value, s, UpdateFromFunc(decimals, update_decimals), UpdateFromFunc(histogram, update_histogram), UpdateFromFunc(value, lambda m: m), run_time=5)
            self.wait()
        clean_form = OldTex('P(', '\\text{data}', '\\,|\\,', '{s}', ')', '=', 'c', '\\cdot', '{s}', '^{\\#' + CMARK_TEX + '}', '(1 - ', '{s}', ')', '^{\\#' + XMARK_TEX + '}', tex_to_color_map={'{s}': YELLOW, '\\#' + CMARK_TEX: GREEN, '\\#' + XMARK_TEX: RED})
        ...
        fade_rects = VGroup(BackgroundRectangle(equation[:2]), BackgroundRectangle(formula), BackgroundRectangle(VGroup(eq2, bar_copy)))
        fade_rects.set_fill(BLACK, 0.8)
        fade_rects[1].set_fill(opacity=0)
        pre_c = formula[:4].copy()
        pre_s = formula[4:8].copy()
        pre_1ms = formula[8:].copy()
        self.play(FadeIn(fade_rects), FadeIn(clean_form[:6]))
        ...
        for cf, pre in ((clean_form[6], pre_c), (clean_form[8:11], pre_s), (clean_form[11:], pre_1ms)):
            self.play(GrowFromPoint(cf, pre.get_center()), pre.move_to, cf, pre.scale, 0)
            self.remove(pre)
            self.wait()
        self.wait()
        ...
        def func500(x):
            return scipy.stats.binom(500, x).pmf(480) + 1e-05
        graph500 = low_axes.get_graph(func500, step_size=0.05)
        graph500.set_stroke(TEAL, 3)
        self.play(top_fade_rect.set_opacity, 1, fade_rects.set_opacity, 1, FadeIn(new_formula))
        self.clear()
        ...
        self.play(Restore(y_axis, rate_func=reverse_smooth), graph.stretch, sf, 1, {'about_edge': DOWN}, graph500.stretch, sf, 1, {'about_edge': DOWN})
        ...
        for s in [0.94, 0.98, 0.96]:
            self.play(s_tracker.set_value, s, UpdateFromFunc(sub_decimals, update_decimals), run_time=5)
            self.wait()
    ...
    def explain_n_choose_k(self, row, formula):
        ...
        for sym in row:
            if sym.positive:
                checks.add(sym)
        self.play(GrowFromCenter(brace), FadeInFromDown(words))
        self.wait()
        self.play(FadeOut(words))
        formula.save_state()
        self.play(ShowIncreasingSubsets(slots), UpdateFromFunc(formula[1], lambda m: m.set_value(len(m.counted))), run_time=2)
        formula.restore()
        self.add(formula)
        self.wait()
        self.play(LaggedStartMap(ApplyMethod, checks, lambda m: (m.shift, 0.3 * DOWN), rate_func=there_and_back, lag_ratio=0.05), ShowCreationThenFadeOut(k_rect), run_time=2)
        self.remove(checks)
        self.add(row)
        self.wait()
        row_target = VGroup()
        ...
        row_target.arrange(RIGHT, buff=get_norm(row[0].get_right() - row[1].get_left()))
        row_target.move_to(row)
        self.play(LaggedStartMap(MoveToTarget, row, path_arc=30 * DEGREES, lag_ratio=0))
        self.wait()
        row.sort()
        ...
        bool_lists = [[i in comb for i in range(50)] for comb in combs]
        ...
        self.add(row)
        self.play(ChangeDecimalToValue(nck_count, choose(50, 48)), run_time=10)
        row.clear_updaters()
        self.wait()
        self.play(FadeOut(nck_count), FadeOut(brace))
...
class StateIndependence(Scene):
    ...
    def construct(self):
        ...
        for m1, m2 in zip(row, row[1:]):
            arrow = Arrow(m1.get_bottom() + 0.025 * DOWN, m2.get_bottom(), path_arc=145 * DEGREES, max_stroke_width_to_length_ratio=10, max_tip_length_to_length_ratio=0.5)
        ...
        self.play(ShowCreation(arrows[0]), FadeIn(words))
        for i in range(10):
            self.play(words.next_to, arrows[i + 1], DOWN, FadeOut(arrows[i]), ShowCreation(arrows[i + 1]))
            last_arrow = arrows[i + 1]
        self.play(FadeOut(words), FadeOut(last_arrow))

class IllustrateBinomialSetupWithCoins(Scene):

    def construct(self):
        coins = [get_coin('H'), get_coin('T')]
        coin_row = VGroup()
        for x in range(12):
        ...
        prob_label = OldTex('P(\\# 00 = k)', tex_to_color_map={'00': WHITE, 'k': GREEN})
        heads = get_coin('H')
        template = prob_label.get_part_by_tex('00')
        heads.replace(template)
        prob_label.replace_submobject(prob_label.index_of_part(template), heads)
        ...
        self.play(ShowIncreasingSubsets(coin_row, int_func=np.ceil), GrowFromPoint(brace, brace.get_left()), FadeIn(brace_label, 3 * LEFT))
        self.wait()
...
class WriteLikelihoodFunction(Scene):
    ...
    def construct(self):
        formula = OldTex('f({s}) = (\\text{const.})', '{s}^{\\#' + CMARK_TEX + '}', '(1 - {s})^{\\#' + XMARK_TEX, '}', tex_to_color_map={'{s}': YELLOW, '\\#' + CMARK_TEX: GREEN, '\\#' + XMARK_TEX: RED})
        formula.scale(2)
...
class LikelihoodGraphFor10of10(ShowBinomialFormula):
    CONFIG = {'histogram_config': {'x_label_freq': 2, 'y_axis_numbers_to_show': range(25, 125, 25), 'y_max': 1, 'y_tick_freq': 0.25, 'height': 2, 'bar_colors': [BLUE]}}
    ...
    def construct(self):
        ...
        rects = VGroup(BackgroundRectangle(formula[:4]), BackgroundRectangle(formula[8:]))
        rects.set_opacity(0.75)
        self.wait()
        self.play(FadeIn(equation))
        self.wait()
        self.play(FadeIn(rects), GrowFromCenter(brace), FadeIn(simpler_formula, UP))
        self.wait()
        pl_decimal = DecimalNumber(self.s)
        pl_decimal.set_color(YELLOW)
        pl_decimal.replace(prob_label[-2][2:])
        ...
        def update_histogram(hist):
            new_dist = scipy.stats.binom(hist.n, hist.s_tracker.get_value())
            new_data = np.array([new_dist.pmf(x) for x in range(0, 11)])
            new_bars = hist.get_bars(new_data)
            new_bars.match_style(hist.bars)
            hist.bars.become(new_bars)
        ...
        self.play(FadeIn(low_axes))
        self.play(ShowCreation(v_line), FadeIn(dot))
...
class StateNeedForBayesRule(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        axes.y_axis.set_height(2, about_point=axes.c2p(0, 0), stretch=True)
        axes.set_width(5)
        ...
        plot.shift(LEFT)
        v_lines = VGroup(DashedLine(axes.c2p(0.8, 0), axes.c2p(0.8, 1)), DashedLine(axes.c2p(1, 0), axes.c2p(1, 1)))
        v_lines.set_stroke(YELLOW, 2)
        self.play(LaggedStart(ApplyMethod(student0.change, 'pondering', plot), ApplyMethod(student1.change, 'pondering', plot), ApplyMethod(student2.change, 'raise_left_hand', plot)), FadeIn(plot, DOWN), run_time=1.5)
        self.play(*map(ShowCreation, v_lines))
        self.play(self.teacher.change, 'tease', *[ApplyMethod(v_line.move_to, axes.c2p(0.9, 0), DOWN) for v_line in v_lines])
        self.play_student_changes('thinking', 'thinking', 'pondering', look_at=v_lines)
        self.wait(2)
        self.teacher_says('But first...', added_anims=[FadeOut(plot, LEFT), FadeOut(v_lines, LEFT), self.change_students('erm', 'erm', 'erm', look_at=self.teacher.eyes)])
        self.wait(5)
