...
COIN_COLOR_MAP = {'H': BLUE_E, 'T': RED_E}

class Histogram(Group):
    CONFIG = {'height': 5, 'width': 10, 'y_max': 1, 'y_axis_numbers_to_show': range(20, 120, 20), 'y_axis_label_height': 0.25, 'y_tick_freq': 0.2, 'x_label_freq': 1, 'include_h_lines': True, 'h_line_style': {'stroke_width': 1, 'stroke_color': GREY_B}, 'bar_style': {'stroke_width': 1, 'stroke_color': WHITE, 'fill_opacity': 1}, 'bar_colors': [BLUE, GREEN]}
    ...
    def add_axes(self):
        ...
        axes_config = {'x_min': 0, 'x_max': n_bars, 'x_axis_config': {'unit_size': self.width / n_bars, 'include_tip': False}, 'y_min': 0, 'y_max': self.y_max, 'y_axis_config': {'unit_size': self.height / self.y_max, 'include_tip': False, 'tick_frequency': self.y_tick_freq}}
...
def get_random_process(choices, shuffle_time=2, total_time=3, change_rate=0.05, h_buff=0.1, v_buff=0.1):
    ...
    def update(container, dt):
        container.time += dt
        t = container.time
        change = all([t % total_time < shuffle_time, container.time - container.last_change_time > change_rate])
        if change:
...
def get_beta_dist_axes(y_max=20, y_unit=2, label_y=False, **kwargs):
    config = {'x_min': 0, 'x_max': 1, 'x_axis_config': {'unit_size': 0.1, 'tick_frequency': 0.1, 'include_tip': False}, 'y_min': 0, 'y_max': y_max, 'y_axis_config': {'unit_size': 1, 'tick_frequency': y_unit, 'include_tip': False}}
    result = Axes(**config)
    origin = result.c2p(0, 0)
    kw = {'about_point': origin, 'stretch': True}
...
def scaled_pdf_axes(scale_factor=3.5):
    axes = get_beta_dist_axes(label_y=True, y_unit=1)
...
def get_prob_review_label(n_positive, n_negative, s=0.95):
    label = OldTex('P(', f'{n_positive}\\,{CMARK_TEX}', ',\\,', f'{n_negative}\\,{XMARK_TEX}', '\\,|\\,', 's = {:.2f}'.format(s), ')')
...
def get_binomial_formula(n, k, p):
    ...
    formula = OldTex('\\left(', '{' + n_str, '\\over', k_str + '}', '\\right)', '(', p_str, ')', '^{' + k_str + '}', '(1 - ', p_str, ')', '^{' + nmk_str + '}')
    parens = VGroup(formula[0], formula[4])
    parens.space_out_submobjects(0.7)
    formula.remove(formula.get_part_by_tex('\\over'))
    pairs = ((n_mob, n_str), (k_mob, k_str), (nmk_mob, nmk_str), (p_mob, p_str))
    ...
    terms = VGroup(formula[:4], formula[4:7], formula[7], formula[8:11], formula[11])
...
def get_check_count_label(nc, nx, include_rect=True):
    result = VGroup(Integer(nc), OldTex(CMARK_TEX, color=GREEN), Integer(nx), OldTex(XMARK_TEX, color=RED))
...
def get_region_under_curve(axes, graph, min_x, max_x):
    props = [binary_search(function=lambda a: axes.x_axis.p2n(graph.pfp(a)), target=x, lower_bound=axes.x_min, upper_bound=axes.x_max) for x in [min_x, max_x]]
    region = graph.copy()
