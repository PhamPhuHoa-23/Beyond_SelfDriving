...
class WeightedCoin(Scene):
    ...
    def construct(self):
        ...
        rhs_arrow = Arrow(rhs_label.get_top(), rhs_box.get_right(), buff=SMALL_BUFF, path_arc=60 * DEGREES, color=rhs_box.get_color())
        self.play(FadeIn(grid, run_time=2, rate_func=linear, lag_ratio=3 / len(grid)))
        self.wait()
        ...
        self.play(FadeIn(rhs_label, lag_ratio=0.1), ShowCreation(rhs_arrow), ShowCreation(rhs_box), LaggedStartMap(MoveToTarget, grid, path_arc=30 * DEGREES, lag_ratio=0.01))
        ...
        for value in [0.2, 0.9, 0.0, 0.31]:
            ...
            if rhs_junk_on_screen:
                anims += [FadeOut(rhs_box), FadeOut(rhs_label), FadeOut(rhs_arrow)]
                rhs_junk_on_screen = False
            self.wait()
            self.play(FadeOut(old_grid, 0.1 * DOWN, lag_ratio=0.01, run_time=1.5), FadeIn(new_grid, lag_ratio=0.01, run_time=1.5), ChangeDecimalToValue(rhs, value), *anims)
            old_grid = new_grid
        long_rhs = DecimalNumber(0.31415926, num_decimal_places=8, show_ellipsis=True)
        ...
        box = get_q_box(rhs)
        self.remove(rhs)
        self.play(FadeOut(old_grid, lag_ratio=0.1), FadeOut(long_rhs, 0.1 * RIGHT, lag_ratio=0.1), Write(box))
        p_label.add(box)
        self.wait()
        ...
        heads_arrows = VGroup(*[Vector(0.5 * UP, max_stroke_width_to_length_ratio=15, max_tip_length_to_length_ratio=0.4).next_to(coin, DOWN) for coin in coins if coin.symbol == 'H'])
        ...
        self.play(ShowIncreasingSubsets(heads_arrows), ShowIncreasingSubsets(numbers), rate_func=linear)
        self.wait()
        ...
        v_line = Line(axes.c2p(0.7, 0), axes.input_to_graph_point(0.7, true_graph))
        ...
        eq_label = VGroup(p_label[:4].copy(), OldTex('= 0.7'))
        for mob in eq_label:
            mob.set_height(0.4)
        eq_label.arrange(RIGHT, buff=SMALL_BUFF)
        pp_label = VGroup(OldTex('P('), eq_label, OldTex(')'))
        for mob in pp_label[::2]:
            mob.set_height(0.7)
            mob.set_color(YELLOW)
        pp_label.arrange(RIGHT, buff=SMALL_BUFF)
        pp_label.move_to(axes.c2p(0.3, 3))
        self.play(FadeOut(heads_arrows), FadeOut(numbers), Write(axes), DrawBorderThenFill(graph))
        self.play(FadeIn(pp_label[::2]), ShowCreation(v_line))
        self.wait()
        ...
        v_line.add_updater(lambda m: m.put_start_and_end_on(m.axes.c2p(m.tracker.get_value(), 0), m.axes.input_to_graph_point(m.tracker.get_value(), m.graph)))
        for value in [0.4, 0.9, 0.7]:
            self.play(v_line.tracker.set_value, value, run_time=3)
        ...
        self.play(LaggedStartMap(FadeOutAndShift, coins, lambda m: (m, DOWN)), GrowFromCenter(brace), Write(h_label))
        self.wait()
        self.embed()
...
class ShowInfiniteContinuum(Scene):
    ...
    def construct(self):
        ...
        self.play(h_part.next_to, x_line.get_start(), UR, SMALL_BUFF, Write(eq), FadeInFromPoint(value, h_part.get_center()))
        h_part.tracked = x_line
        value.tracked = x_line
        value.x_axis = axes.x_axis
        self.play(ShowCreation(x_line), UpdateFromFunc(h_part, lambda m: m.next_to(m.tracked.get_end(), UR, SMALL_BUFF)), UpdateFromFunc(value, lambda m: m.set_value(m.x_axis.p2n(m.tracked.get_end()))), run_time=3)
        self.wait()
        self.play(FadeOut(eq), FadeOut(value))
        arrows = VGroup()
        ...
        kw = {'lag_ratio': 0.05, 'run_time': 5, 'rate_func': lambda t: t ** 5}
        arrows.save_state()
        for arrow in arrows:
            arrow.stretch(0, 0)
        ...
        self.play(LaggedStartMap(ApplyMethod, arrows, lambda m: (m.scale, 0, {'about_edge': DOWN}), lag_ratio=10 / len(arrows), rate_func=smooth, run_time=3))
        self.remove(arrows)
        self.wait()
...
class LabelH(Scene):
    ...
    def construct(self):
        ...
        self.play(GrowFromCenter(brace), FadeIn(h, UP))
        self.wait()
...
class TryAssigningProbabilitiesToSpecificValues(Scene):
    ...
    def construct(self):
        ...
        def get_p_label(value):
            result = OldTex('P(', '{h}', '=', value, ')')
            result.set_color_by_tex('{h}', YELLOW)
            return result
        labels = VGroup(get_p_label('0.70000000'), get_p_label('0.70000001'), get_p_label('0.70314159'), get_p_label('0.70271828'), get_p_label('0.70466920'), get_p_label('0.70161803'))
        ...
        self.wait()
        self.play(*[TransformFromCopy(m1, m2) for m1, m2 in [(q_marks[0], q_marks[1]), (labels[0][:3], labels[1][:3]), (labels[0][-1], labels[1][-1])]])
        self.play(ShowIncreasingSubsets(labels[1][3], run_time=3, int_func=np.ceil, rate_func=linear))
        self.add(labels[1])
        self.wait()
        self.play(LaggedStartMap(FadeInFrom, labels[2:], lambda m: (m, UP)), LaggedStartMap(FadeInFrom, q_marks[2:], lambda m: (m, UP)), Write(v_dots, rate_func=squish_rate_func(smooth, 0.5, 1)))
        self.add(labels, q_marks)
        self.wait()
        self.play(ReplacementTransform(q_marks, gt_zero, lag_ratio=0.05), run_time=2)
        self.wait()
        group = VGroup(labels, gt_zero, v_dots)
        sum_label = OldTex('\\sum_{0 \\le {h} \\le 1}', 'P(', '{h}', ')', '=', tex_to_color_map={'{h}': YELLOW})
        ...
        self.play(Write(sum_label), VFadeIn(morty), morty.change, 'confused', sum_label)
        infty = OldTex('\\infty')
        zero = OldTex('0')
        ...
        self.play(Write(infty), morty.change, 'horrified', infty)
        self.play(Blink(morty))
        self.wait()
        eq_zero.move_to(gt_zero)
        eq_zero.set_color(RED)
        self.play(ReplacementTransform(gt_zero, eq_zero, lag_ratio=0.05, run_time=2, path_arc=30 * DEGREES), morty.change, 'pondering', eq_zero)
        self.wait()
...
class ProbabilityToContinuousValuesSupplement(Scene):
    ...
    def construct(self):
        nl = UnitInterval()
        nl.set_width(10)
        nl.add_numbers(*np.arange(0, 1.1, 0.1), buff=0.3)
        nl.to_edge(LEFT)
        self.add(nl)
        ...
        self.play(MoveToTarget(lines_row, lag_ratio=0.1, rate_func=rush_into, run_time=4))
        self.wait()
        self.play(lines.set_height, 0.01, {'about_edge': DOWN, 'stretch': True}, ApplyMethod(lines_row.set_width, 0.01, {'about_edge': LEFT}, rate_func=rush_into), run_time=6)
        self.wait()
...
class CarFactoryNumbers(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        question = VGroup(OldTexText('What can you say'), OldTex('\\text{about } P(\\text{defect})?', tex_to_color_map={'\\text{defect}': RED}))
...
class ShowLimitToPdf(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(eq_label, 0.2 * DOWN), GrowArrow(arrows[0]))
        self.wait()
        vect = eq_label.get_center() - ineq_label.get_center()
        self.play(FadeOut(eq_label, -vect), FadeIn(ineq_label, vect), TransformFromCopy(*arrows), GrowFromPoint(brace, brace.get_left()))
        self.wait()
        arrow = arrows[0]
        ...
        kw = {'run_time': 2, 'rate_func': squish_rate_func(smooth, 0.3, 0.9)}
        self.play(MoveToTarget(arrow, **kw), ApplyMethod(ineq_label.next_to, arrows[0].target, UP, **kw), FadeOut(arrows[1]), FadeOut(brace), LaggedStartMap(Restore, bars, run_time=2, lag_ratio=0.025))
        self.wait()
        lines = VGroup()
        new_bars = VGroup()
        for bar in bars:
            line = Line(bar.get_corner(DL), bar.get_corner(DR))
        ...
        prob_label = OldTexText('Height', '$\\rightarrow$', 'Probability')
        ...
        self.play(LaggedStartMap(MoveToTarget, lines, lag_ratio=0.01), FadeInFromDown(prob_label))
        self.add(height_word)
        self.play(ShowCreation(height_cross), FadeOut(axis_prob_label, LEFT))
        self.wait()
        self.play(FadeOut(height_word, UP), FadeOut(height_cross, UP), FadeInFromDown(area_word))
        self.play(FadeOut(lines), LaggedStartMap(MoveToTarget, new_bars, lag_ratio=0.01))
        self.play(FadeOut(new_bars), area_word.set_color, BLUE)
        prob_label = VGroup(area_word, *prob_label[1:])
        self.add(prob_label)
        randy = Randolph(height=1)
        randy.next_to(prob_label, UP, aligned_edge=LEFT)
        bubble = SpeechBubble(height=2, width=4)
        ...
        self.play(Blink(randy))
        self.play(FadeOut(randy), FadeOut(bubble), FadeOut(bubble.content))
        ...
        for step_size in [0.025, 0.01, 0.005, 0.001]:
            ...
            new_ineq_label = OldTex('P(0.8 < h < {:.3})'.format(0.8 + step_size), tex_to_color_map={'h': YELLOW})
            if step_size <= 0.005:
            ...
            self.wait()
            self.play(ReplacementTransform(last_bars, new_bars, lag_ratio=step_size), MoveToTarget(arrow), FadeOut(last_ineq_label, vect), FadeIn(new_ineq_label, -vect), run_time=2)
        ...
        limit_words.next_to(axes.input_to_graph_point(0.75, graph_curve), UP, MED_LARGE_BUFF)
        self.play(FadeIn(graph), FadeOut(last_ineq_label), FadeOut(arrow), FadeOut(last_bars))
        ...
        for label, step in zip(all_ineq_labels, step_sizes):
            eq = OldTex('=')
            decimal = DecimalNumber(dist.cdf(0.8 + step) - dist.cdf(0.8), num_decimal_places=3)
        ...
        self.play(LaggedStartMap(FadeInFrom, all_ineq_labels, lambda m: (m, UP)), LaggedStartMap(FadeInFrom, rhss, lambda m: (m, UP)), MoveToTarget(prob_label))
        self.play(GrowArrow(arrow), FadeIn(to_zero_words))
        self.play(LaggedStartMap(Indicate, rhss, scale_factor=1.05))
        self.wait(2)
        bars.restore()
        ...
        for n1, n2 in zip(axes.y_axis.numbers, new_y_numbers):
            n2.match_height(n1)
            n2.add_background_rectangle(opacity=1, buff=SMALL_BUFF)
            n2.move_to(n1, RIGHT)
        self.play(FadeOut(limit_words), FadeOut(graph), FadeIn(bars), FadeOut(area_word, UP), FadeIn(height_word, DOWN), FadeIn(new_y_numbers, 0.5 * RIGHT))
        ...
        for step_size, rhs in zip(step_sizes[1:], rhss[1:]):
            ...
            bar = new_bars[int(0.8 * len(new_bars))]
            bar.set_color(highlighted_bar_color)
            new_bars.stretch(step_size / 0.05, 1, about_edge=DOWN)
            if step_size <= 0.05:
                new_bars.set_stroke(width=0)
            self.remove(last_bars)
            self.play(TransformFromCopy(last_bars, new_bars, lag_ratio=step_size), rect.move_to, rhs[1])
            last_bars = new_bars
        self.play(FadeOut(last_bars), FadeOutAndShiftDown(rect))
        self.wait()
        self.play(FadeIn(graph), FadeIn(area_word, 0.5 * DOWN), FadeOut(height_word, 0.5 * UP), FadeOut(new_y_numbers, lag_ratio=0.2))
        self.play(arrow.scale, 0, {'about_edge': DOWN}, FadeOut(to_zero_words, DOWN), LaggedStartMap(FadeOutAndShiftDown, all_ineq_labels), LaggedStartMap(FadeOutAndShiftDown, rhss))
        self.wait()
        arrow = Arrow(axes.y_axis.get_top() + 3 * RIGHT, axes.y_axis.get_top(), path_arc=90 * DEGREES)
        question = OldTexText('What are the\\\\units here?')
        question.next_to(arrow.get_start(), DOWN)
        self.play(FadeIn(question, lag_ratio=0.1), ShowCreation(arrow))
        self.wait()
        bars = self.get_bars(axes, dist, 0.05)
        self.play(FadeOut(graph), FadeIn(bars))
        bars.generate_target()
        bars.save_state()
        ...
        self.play(MoveToTarget(bars), Write(prob_word, run_time=1))
        self.wait()
        ...
        self.play(GrowFromCenter(top_brace), FadeIn(top_label))
        ...
        self.play(Uncreate(arrow), FadeOutAndShiftDown(question), Write(y_label))
        self.wait(2)
        self.play(Restore(bars), FadeOut(top_brace), FadeOut(side_brace), FadeOut(top_label), FadeOut(side_label), FadeOut(prob_word))
        ...
        bars.save_state()
        self.play(bars.arrange, RIGHT, {'aligned_edge': DOWN, 'buff': SMALL_BUFF}, bars.move_to, bars.get_bottom() + 0.5 * UP, DOWN)
        ...
        for step_size in step_sizes[1:]:
            ...
            if step_size <= 0.05:
                new_bars.set_stroke(width=0)
            self.play(ReplacementTransform(bars, new_bars, lag_ratio=step_size), run_time=3)
            self.wait()
            bars = new_bars
        ...
        self.play(ShowCreation(graph_curve), Write(func_name, run_time=1))
        self.wait()
        func_name_copy = func_name.copy()
        self.play(func_name.next_to, initials, UP, *[ReplacementTransform(np[0], ip[0]) for np, ip in zip(func_name_copy, initials)], *[FadeOut(np[1:]) for np in func_name_copy])
        self.add(initials)
        self.wait()
        self.play(FadeOut(func_name), FadeOut(total_label), FadeOut(graph_curve), initials.next_to, axes.input_to_graph_point(0.95, graph_curve), UR)
        min_x = 0.6
        max_x = 0.8
        region = get_region_under_curve(axes, graph_curve, min_x, max_x)
        area_label = DecimalNumber(dist.cdf(max_x) - dist.cdf(min_x), num_decimal_places=3)
        area_label.move_to(region)
        v_lines = VGroup()
        for x in [min_x, max_x]:
            v_lines.add(DashedLine(axes.c2p(x, 0), axes.c2p(x, 2.5)))
        v_lines.set_stroke(YELLOW, 2)
        p_label = VGroup(OldTex('P('), DecimalNumber(min_x), OldTex('\\le'), OldTex('h', color=YELLOW), OldTex('\\le'), DecimalNumber(max_x), OldTex(')'))
        ...
        self.play(FadeIn(p_label, 2 * DOWN), *map(ShowCreation, v_lines))
        self.wait()
        region.func = get_region_under_curve
        self.play(UpdateFromAlphaFunc(region, lambda m, a: m.become(m.func(m.axes, m.graph, m.min_x, interpolate(m.min_x, m.max_x, a)))), CountInFrom(area_label), UpdateFromAlphaFunc(area_label, lambda m, a: m.set_opacity(a)))
        self.wait()
        self.play(TransformFromCopy(area_label, rhs[1]), Write(rhs[0]))
        self.wait()
        new_x = np.mean([min_x, max_x])
        area_label.original_width = area_label.get_width()
        region.new_x = new_x
        self.play(ChangeDecimalToValue(p_label[1], new_x), ChangeDecimalToValue(p_label[5], new_x), ChangeDecimalToValue(area_label, 0), UpdateFromAlphaFunc(area_label, lambda m, a: m.set_width(interpolate(m.original_width, 1e-06, a))), ChangeDecimalToValue(rhs[1], 0), v_lines[0].move_to, axes.c2p(new_x, 0), DOWN, v_lines[1].move_to, axes.c2p(new_x, 0), DOWN, UpdateFromAlphaFunc(region, lambda m, a: m.become(m.func(m.axes, m.graph, interpolate(m.min_x, m.new_x, a), interpolate(m.max_x, m.new_x, a)))), run_time=2)
        self.wait()
        self.play(ChangeDecimalToValue(p_label[1], 0), ChangeDecimalToValue(p_label[5], 1), ChangeDecimalToValue(area_label, 1), UpdateFromAlphaFunc(area_label, lambda m, a: m.set_width(interpolate(1e-06, m.original_width, clip(5 * a, 0, 1)))), ChangeDecimalToValue(rhs[1], 1), v_lines[0].move_to, axes.c2p(0, 0), DOWN, v_lines[1].move_to, axes.c2p(1, 0), DOWN, UpdateFromAlphaFunc(region, lambda m, a: m.become(m.func(m.axes, m.graph, interpolate(m.new_x, 0, a), interpolate(m.new_x, 1, a)))), run_time=5)
        self.wait()

    def get_axes(self):
        axes = Axes(x_min=0, x_max=1, x_axis_config={'tick_frequency': 0.05, 'unit_size': 12, 'include_tip': False}, y_min=0, y_max=4, y_axis_config={'tick_frequency': 1, 'unit_size': 1.25, 'include_tip': False})
        axes.center()
        h_label = OldTex('h')
        h_label.set_color(YELLOW)
    ...
    def get_bars(self, axes, dist, step_size):
        ...
        for x in np.arange(0, 1, step_size):
            ...
            h_line = Line(axes.c2p(x, 0), axes.c2p(x + step_size, 0))
            v_line = Line(axes.c2p(0, 0), axes.c2p(0, dist.pdf(x)))
...
class FiniteVsContinuum(Scene):
    ...
    def construct(self):
        ...
        eqs = VGroup(self.get_union_equation(dice), self.get_union_equation(cards))
        for eq in eqs:
        ...
        for eq in eqs:
            ...
            anims.append(LaggedStartMap(MoveToTarget, movers, path_arc=30 * DEGREES, lag_ratio=0.1))
        self.wait()
        for anim in anims:
            self.play(anim)
        ...
        self.play(Write(c_title, run_time=1), ShowCreation(c_underline), eqs[0].shift, 0.5 * UP, eqs[1].shift, UP)
        c_eq = OldTex('P\\big(', 'x \\in [0.65, 0.75]', '\\big)', '=', '\\sum_{x \\in [0.65, 0.75]}', 'P(', 'x', ')')
    ...
    def get_union_equation(self, mobs):
        ...
        for mob in mobs:
            ...
            probs.add(prob)
        lhs = VGroup(OldTex('P\\big(', color=p_color), OldTex('x \\in'), mob_set, OldTex('\\big)', color=p_color))
...
class ComplainAboutRuleChange(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('Wait, the rules\\\\changed?', target_mode='sassy', added_anims=[self.teacher.change, 'tease'])
...
class HalfFiniteHalfContinuous(Scene):
    ...
    def construct(self):
        ...
        arrow_labels = VGroup()
        for vect in [UP, DOWN]:
            arrow = Arrow(box.get_corner(vect + RIGHT), box.get_corner(vect + RIGHT) + 3 * RIGHT + 1.5 * vect, buff=MED_SMALL_BUFF)
            label = OldTex('50\\%')
            fix_percent(label[0][-1])
            label.set_color(YELLOW)
            label.next_to(arrow.get_center(), vect + LEFT, buff=SMALL_BUFF)
            arrow_labels.add(label)
            arrows.add(arrow)
        zero = Integer(0)
        zero.set_height(0.5)
        zero.next_to(arrows[0].get_end(), RIGHT)
        axes = Axes(x_min=0, x_max=6.5, y_min=0, y_max=0.25, y_axis_config={'tick_frequency': 1 / 16, 'unit_size': 10, 'include_tip': False})
        ...
        half_gauss = Group(graph, graph_fill, axes)
        ...
        self.add(number)
        self.wait(4)
        self.play(GrowArrow(arrows[0]), FadeIn(arrow_labels[0]), GrowFromPoint(zero, box.get_corner(UR)))
        self.wait(2)
        self.play(GrowArrow(arrows[1]), FadeIn(arrow_labels[1]), FadeIn(half_gauss))
        self.add(v_line)
        self.wait(30)
...
class SumToIntegral(Scene):
    ...
    def construct(self):
        titles = VGroup(OldTexText('Discrete context'), OldTexText('Continuous context'))
        ...
        h_line.set_x(0)
        v_line.center()
        self.play(ShowCreation(VGroup(h_line, v_line)), LaggedStartMap(FadeInFrom, titles, lambda m: (m, -0.2 * m.get_center()[0] * RIGHT), run_time=1, lag_ratio=0.1))
        self.wait()
        ...
        for sym, title in zip(syms, titles):
            ...
            sym.match_x(title)
        arrow = Arrow(s_sym[0].get_corner(UP), i_sym[0].get_corner(UP), path_arc=-90 * DEGREES)
        arrow.set_color(YELLOW)
        ...
        for i, j in [(0, 0), (2, 1), (3, 2)]:
            ...
            anims += [Restore(target, path_arc=-60 * DEGREES), MoveToTarget(source, path_arc=-60 * DEGREES)]
        self.play(LaggedStart(*anims))
        self.play(FadeInFromDown(i_sym[3]))
        self.add(i_sym)
        self.wait()
        self.play(FadeOut(arrow, UP), syms.next_to, h_line, DOWN, {'buff': MED_LARGE_BUFF}, syms.match_x, syms)
        axes = Axes(x_min=0, x_max=10, y_min=0, y_max=7, y_axis_config={'unit_size': 0.75})
        ...
        for bar in bars:
            bar.save_state()
            bar.stretch(0, 1, about_edge=DOWN)
        self.play(Write(axes), LaggedStartMap(Restore, bars), LaggedStartMap(FadeInFromDown, addition_formula))
        self.wait()
        morty = Mortimer()
        morty.to_corner(DR)
        morty.look_at(i_sym)
        ...
        self.play(Blink(morty))
        self.wait()
        self.play(Uncreate(VGroup(v_line, h_line)), FadeOut(titles, UP), FadeOut(morty, RIGHT), FadeOut(s_sym, LEFT), i_sym.center, i_sym.to_edge, LEFT)
        arrows = VGroup()
        for vect in [UP, DOWN]:
            corner = i_sym[-1].get_corner(RIGHT + vect)
            arrows.add(Arrow(corner, corner + 2 * RIGHT + 2 * vect, path_arc=-np.sign(vect[1]) * 60 * DEGREES))
        ...
        self.play(LaggedStart(FadeIn(r_axes), FadeIn(r_graph), FadeIn(r_name), FadeIn(l_axes), FadeIn(l_graph), FadeIn(l_name), run_time=1))

        def get_riemann_rects(dx, axes=r_axes, func=dist.pdf):
            bars = VGroup()
            for x in np.arange(0, 1, dx):
                bar = Rectangle()
                line = Line(axes.c2p(x, 0), axes.c2p(x + dx, func(x)))
                bar.replace(line, stretch=True)
        ...
        self.play(FadeIn(r_bar_groups[0], lag_ratio=0.1), FadeIn(l_bar_groups[0], lag_ratio=0.1), FadeIn(footnote))
        self.wait()
        for rbg0, rbg1, lbg0, lbg1 in zip(r_bar_groups, r_bar_groups[1:], l_bar_groups, l_bar_groups[1:]):
            self.play(ReplacementTransform(rbg0, rbg1, lag_ratio=1 / len(rbg0), run_time=2), ReplacementTransform(lbg0, lbg1, lag_ratio=1 / len(lbg0), run_time=2))
            self.wait()
        self.play(FadeOut(r_bar_groups[-1]), FadeOut(l_bar_groups[-1]), r_graph.set_fill, BLUE_E, 1, l_graph.set_fill, RED_E, 1)
...
class MeasureTheoryLeadsTo(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(words, DOWN), GrowArrow(arrow), UpdateFromAlphaFunc(arrow, lambda m, a: m.set_opacity(a)))
        self.wait()
...
class WhenIWasFirstLearning(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.change_students('pondering', 'thinking', 'tease'), self.teacher.change, 'thinking')
        younger = BabyPiCreature(color=GREY_BROWN)
        younger.set_height(2)
        younger.move_to(self.students, DL)
        self.look_at(self.screen)
        self.wait()
        self.play(ReplacementTransform(self.teacher, younger), LaggedStartMap(FadeOutAndShift, self.students, lambda m: (m, DOWN)))
        bubble = ThoughtBubble()
        bubble[-1].set_fill(GREEN_SCREEN, 1)
        bubble.move_to(younger.get_corner(UR), DL)
        self.play(Write(bubble), younger.change, 'maybe', bubble.get_bubble_center())
...
class TiePossibleToDensity(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(VGroup(implies, cross, prob).shift, UP, FadeIn(implies2), FadeIn(dens))
        self.wait()
        self.embed()
...
class DrawBigRect(Scene):
    ...
    def construct(self):
        ...
        words.to_edge(UP)
        arrow = Arrow(words.get_bottom(), rect.get_top(), buff=0.25, color=RED)
...
class Thumbnail(Scene):
    ...
    def construct(self):
        dartboard = Dartboard()
        axes = NumberPlane(x_min=-1.25, x_max=1.25, y_min=-1.25, y_max=1.25, axis_config={'unit_size': 0.5 * dartboard.get_width(), 'tick_frequency': 0.25}, x_line_frequency=1.0, y_line_frequency=1.0)
        ...
        words = VGroup(OldTexText('Probability = 0'), OldTexText('$\\dots$but still possible'))
        for word in words:
            word.set_width(6)
