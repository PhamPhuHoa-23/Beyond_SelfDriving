...
class RemindOfWeightedCoin(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, grid, lag_ratio=2 / len(grid), run_time=3))
        self.wait()
        ...
        self.play(GrowFromCenter(brace), FadeIn(h_label, UP), grid.scale, 0.8, {'about_edge': DOWN})
        self.wait()
        ...
        def get_grid_swap_anims(g1, g2):
            return [LaggedStartMap(MoveToTarget, g1, lag_ratio=0.02, run_time=1.5, remover=True), LaggedStartMap(MoveToTarget, g2, lag_ratio=0.02, run_time=1.5, rate_func=reverse_smooth)]
        self.play(FadeIn(eq), UpdateFromAlphaFunc(h_decimal, lambda m, a: m.set_opacity(a)), ChangeDecimalToValue(h_decimal, 0, run_time=2), *get_grid_swap_anims(grid, tail_grid))
        self.wait()
        self.play(ChangeDecimalToValue(h_decimal, 1, run_time=1.5), *get_grid_swap_anims(tail_grid, head_grid))
        self.wait()
        self.play(ChangeDecimalToValue(h_decimal, 0.7, run_time=1.5), *get_grid_swap_anims(head_grid, grid70))
        self.wait()
        ...
        self.play(FadeOut(eq), FadeOut(h_decimal), LaggedStartMap(MoveToTarget, grid70, run_time=1, remover=True), FadeIn(axes))
        self.play(DrawBorderThenFill(graph), FadeIn(area_label, rate_func=squish_rate_func(smooth, 0.5, 1), run_time=2), Write(pdf_label, run_time=1))
        self.wait()
        ...
        range_label = VGroup(OldTex('0.6 \\le'), p_label[:4].copy(), OldTex('\\le 0.8'))
        for mob in range_label:
            mob.set_height(0.4)
        range_label.arrange(RIGHT, buff=SMALL_BUFF)
        pp_label = VGroup(OldTex('P('), range_label, OldTex(')'))
        for mob in pp_label[::2]:
            mob.set_height(0.7)
            mob.set_color(YELLOW)
        pp_label.arrange(RIGHT, buff=SMALL_BUFF)
        pp_label.move_to(axes.c2p(0.3, 3))
        self.play(FadeIn(pp_label[::2]), MoveToTarget(pdf_label), FadeOut(area_label))
        self.wait()
        ...
        self.wait()
        self.add(region)
        self.play(lh_tracker.set_value, 0.6, rh_tracker.set_value, 0.8, UpdateFromAlphaFunc(region_area_label, lambda m, a: m.set_opacity(a), rate_func=squish_rate_func(smooth, 0.25, 1)), run_time=3)
        self.wait()
        ...
        numbers = VGroup(*[Integer(i + 1).set_height(0.2).next_to(coin, DOWN, SMALL_BUFF) for i, coin in enumerate(heads)])
        for coin in coins:
            coin.save_state()
        ...
        self.play(LaggedStartMap(Restore, coins), MoveToTarget(pp_label), run_time=1)
        self.play(ShowIncreasingSubsets(numbers))
        self.wait()
        self.play(n_heads_tracker.set_value, 7, n_tails_tracker.set_value, 3, FadeOut(pdf_label, rate_func=squish_rate_func(smooth, 0, 0.5)), run_time=2)
        self.wait()
        ...
        self.play(FadeOut(numbers), ReplacementTransform(coins, grid[:10]))
        self.play(FadeIn(grid[10:], lag_ratio=0.1, rate_func=linear), pp_label.next_to, grid, DOWN)
        self.wait()
        self.add(graph, region, region_area_label, p_label, q_box, brace, h_label)
        self.play(n_heads_tracker.set_value, 70, n_tails_tracker.set_value, 30)
        self.wait()
        origin = axes.c2p(0, 0)
        self.play(axes.y_axis.stretch, 0.5, 1, {'about_point': origin}, h_lines.stretch, 0.5, 1, {'about_point': origin})
        self.wait()
        pairs = [(70 * 3, 30 * 3), (35, 15), (35 + 20, 15 + 20), (7, 3), (70, 30)]
        for nh, nt in pairs:
            self.play(n_heads_tracker.set_value, nh, n_tails_tracker.set_value, nt, run_time=2)
            self.wait()
        self.embed()
...
class LastTimeWrapper(Scene):
    ...
    def construct(self):
        ...
        self.play(DrawBorderThenFill(rect), FadeInFromDown(title))
        self.wait()
...
class BayesianFrequentistDivide(Scene):
    ...
    def construct(self):
        ...
        for label in labels:
            label.save_state()
            label.set_y(0)
            self.play(FadeIn(label, -normalize(label.get_center())))
        self.wait()
        self.play(ShowCreation(VGroup(v_line, h_line)), *map(Restore, labels))
        self.wait()
        kw = {'tex_to_color_map': {'$p$-value': YELLOW, '$H_0$': PINK, '$\\alpha$': BLUE}, 'alignment': ''}
        freq_list = VGroup(OldTexText('1. State a null hypothesis $H_0$', **kw), OldTexText('2. Choose a test statistic,\\\\', '$\\qquad$ compute its value', **kw), OldTexText('3. Calculate a $p$-value', **kw), OldTexText('4. Choose a significance value $\\alpha$', **kw), OldTexText('5. Reject $H_0$ if $p$-value\\\\', '$\\qquad$ is less than $\\alpha$', **kw))
        ...
        for H_word, graph, vect in zip(H_words, graphs[::2], [RIGHT, LEFT]):
            ...
            arrow = Arrow(H_word.get_bottom(), graph.get_center() + 0.75 * vect, buff=SMALL_BUFF)
            H_word.add(arrow)
...
class ShowBayesianUpdating(Scene):
    CONFIG = {'true_p': 0.72, 'random_seed': 4, 'initial_axis_scale_factor': 3.5}
    ...
    def construct(self):
        ...
        for x in range(n_previews):
            ...
            scaled_graph.apply_function(lambda p: axes.c2p(axes.x_axis.p2n(p), axes.y_axis.p2n(p) * likelihood(axes.x_axis.p2n(p))))
            ...
            if x < n_slow_previews:
                self.play(FadeInFromDown(coin), FadeIn(new_data_label), GrowArrow(arrow))
                self.play(FadeOut(new_data_label), FadeOut(arrow), ShowCreation(line), FadeIn(label))
                self.add(graph_copy, line, label)
                self.play(Transform(graph_copy, scaled_graph))
                self.play(FadeOut(line), FadeOut(label), FadeIn(renorm_label))
                self.play(Transform(graph_copy, renormalized_graph), FadeOut(graph))
        ...
        self.play(Restore(axes, rate_func=lambda t: smooth(1 - t)), graph.stretch, 1 / sf, 1, {'about_edge': DOWN}, run_time=2)
        ...
        for x in x_bounds:
            line = DashedLine(axes.c2p(x, 0), axes.c2p(x, 12))
        ...
        self.play(FadeIn(region), Write(label95))
        self.wait()
        for label in labels:
        ...
        self.wait()
        many_bools = np.hstack([bool_values, np.random.random(1000) < true_p])
        ...
        self.play(N_tracker.set_value, 1000, UpdateFromFunc(graph, update_graph), Animation(v_lines), Animation(labels), Animation(graph.region), Animation(graph.label95), run_time=5)
        self.wait()

    def get_coins(self, bool_values):
        coins = VGroup(*[get_coin('H' if heads else 'T') for heads in bool_values])
    ...
    def get_probability_label(self):
        head = get_coin('H')
        p_label = OldTex('P(00) = ', tex_to_color_map={'00': WHITE})
        template = p_label.get_part_by_tex('00')
        head.replace(template)
        p_label.replace_submobject(p_label.index_of_part(template), head)
...
class ShowBayesRule(Scene):
    ...
    def construct(self):
        hyp = '\\text{Hypothesis}'
        data = '\\text{Data}'
        bayes = OldTex(f'P({hyp} \\,|\\, {data})', '=', '{', f'P({data} \\,|\\, {hyp})', f'P({hyp})', '\\over', f'P({data})', tex_to_color_map={hyp: YELLOW, data: GREEN})
        ...
        self.add(*bayes[:5])
        self.wait()
        self.play(*[TransformFromCopy(bayes[i], bayes[j], path_arc=30 * DEGREES) for i, j in [(0, 7), (1, 10), (2, 9), (3, 8), (4, 11)]], FadeIn(bayes[5]), run_time=1.5)
        self.wait()
        self.play(*[TransformFromCopy(bayes[i], bayes[j], path_arc=30 * DEGREES) for i, j in [(0, 12), (1, 13), (4, 14), (0, 16), (3, 17), (4, 18)]], FadeIn(bayes[15]), run_time=1.5)
        self.add(bayes)
        self.wait()
        hyp_word = bayes.get_part_by_tex(hyp)
        example_hyp = OldTexText('For example,\\\\', '$0.9 < s < 0.99$')
        example_hyp[1].set_color(YELLOW)
        example_hyp.next_to(hyp_word, DOWN, buff=1.5)
        data_word = bayes.get_part_by_tex(data)
        example_data = OldTex('48\\,', CMARK_TEX, '\\,2\\,', XMARK_TEX)
        ...
        hyp_arrow = Arrow(hyp_word.get_bottom(), example_hyp.get_top())
        data_arrow = Arrow(data_word.get_bottom(), example_data.get_top())
        self.play(GrowArrow(hyp_arrow), FadeInFromPoint(example_hyp, hyp_word.get_center()))
        self.wait()
        self.play(GrowArrow(data_arrow), FadeInFromPoint(example_data, data_word.get_center()))
        self.wait()
...
class VisualizeBayesRule(Scene):
    ...
    def show_continuum(self):
        ...
        axes.y_axis.add_numbers(*np.arange(0.2, 1.2, 0.2), num_decimal_places=1)
        p_label = OldTex('P(s \\,|\\, \\text{data})', tex_to_color_map={'s': YELLOW, '\\text{data}': GREEN})
        p_label.scale(1.5)
        ...
        self.add(axes)
        self.add(p_label)
        self.play(s_part.next_to, x_line.get_start(), UR, SMALL_BUFF, GrowArrow(arrow), FadeInFromPoint(value, s_part.get_center()))
        s_part.tracked = x_line
        value.tracked = x_line
        value.x_axis = axes.x_axis
        self.play(ShowCreation(x_line), UpdateFromFunc(s_part, lambda m: m.next_to(m.tracked.get_end(), UR, SMALL_BUFF)), UpdateFromFunc(value, lambda m: m.set_value(m.x_axis.p2n(m.tracked.get_end()))), run_time=3)
        self.wait()
        self.play(FadeOut(arrow), FadeOut(value))
    ...
    def show_arrows(self):
        ...
        kw = {'lag_ratio': 0.5, 'run_time': 5, 'rate_func': lambda t: t ** 4}
        arrows.save_state()
        for arrow in arrows:
            arrow.stretch(0, 0)
        ...
        self.play(LaggedStartMap(ApplyMethod, arrows, lambda m: (m.scale, 0, {'about_edge': DOWN}), **kw))
        self.remove(arrows)
        self.wait()
    ...
    def show_discrete_probabilities(self):
        ...
        for x in np.arange(0, 1, dx):
            line = Line(axes.c2p(x, 0), axes.c2p(x + dx, 0))
            line.set_stroke(BLUE, 3)
            line.generate_target()
            line.target.rotate(90 * DEGREES, about_point=line.get_start())
            x_lines.add(line)
        self.add(x_lines)
        self.play(FadeOut(self.x_line), LaggedStartMap(MoveToTarget, x_lines))
        label = Integer(0)
        ...
        arrow.axes = axes
        arrow.label = label
        arrow.add_updater(lambda m: m.put_start_and_end_on(m.label.get_bottom() + MED_SMALL_BUFF * DOWN, m.axes.c2p(0.01 * m.label.get_value(), 0.03)))
        self.add(label, unit, arrow)
        self.play(ChangeDecimalToValue(label, 99), run_time=5)
        self.wait()
        ...
        p_label.save_state()
        self.play(given_data.scale, 0.5, given_data.set_opacity, 0.5, given_data.to_corner, UR, Transform(p_label[:2], prior_label[:2]), Transform(p_label[-1], prior_label[-1]))
        self.wait()
        new_ticks = VGroup()
        new_labels = VGroup()
        dy = 0.01
        ...
        origin = axes.c2p(0, 0)
        for x in np.arange(0, 1, dx):
            rect = Rectangle(width=get_norm(axes.c2p(dx, 0) - origin), height=get_norm(axes.c2p(0, dy) - origin))
            rect.x = x
            rect.set_stroke(BLUE, 1)
            rect.set_fill(BLUE, 0.5)
            rect.move_to(axes.c2p(x, 0), DL)
            bars.add(rect)
        stretch_group = VGroup(axes.y_axis, bars, new_ticks, x_lines)
        x_lines.set_height(bars.get_height(), about_edge=DOWN, stretch=True)
        self.play(stretch_group.stretch, 25, 1, {'about_point': axes.c2p(0, 0)}, VFadeIn(bars), VFadeIn(new_ticks), VFadeIn(new_labels), VFadeOut(x_lines), run_time=4)
        highlighted_bars = bars.copy()
        highlighted_bars.set_color(YELLOW)
        self.play(LaggedStartMap(FadeIn, highlighted_bars, lag_ratio=0.5, rate_func=there_and_back), ShowCreationThenFadeAround(new_labels[0]), run_time=3)
        ...
        for a, b in [(5, 2), (1, 6)]:
            ...
            self.play(LaggedStartMap(MoveToTarget, bars, lag_ratio=0.0))
            self.wait()
        ...
        self.play(prior_name[0].next_to, uniform_name, RIGHT, MED_SMALL_BUFF, DOWN, FadeOut(prior_name[1], RIGHT), FadeIn(uniform_name, LEFT))
        self.wait()
        self.bars = bars
    ...
    def show_bayes_formula(self):
        ...
        p_label = self.p_label
        bars = self.bars
        prior_label = VGroup(p_label[0].deepcopy(), p_label[1].deepcopy(), p_label[4].deepcopy())
        eq = OldTex('=')
        likelihood_label = OldTex('P(', '\\text{data}', '|', 's', ')')
        ...
        over.set_width(prior_label.get_width() + likelihood_label.get_width() + MED_SMALL_BUFF)
        ...
        self.play(p_label.restore, p_label.next_to, eq, LEFT, MED_SMALL_BUFF, prior_label.next_to, over, UP, MED_SMALL_BUFF, LEFT, FadeIn(eq), FadeIn(likelihood_label), FadeIn(over), FadeIn(p_data_label), FadeOut(uniform_label))
        post_bars = bars.copy()
        total_prob = 0
        ...
        post_bars.stretch(0.25, 1, about_edge=DOWN)
        ...
        self.play(ReplacementTransform(bars.copy().set_opacity(0), post_bars), GrowFromCenter(brace), FadeIn(post_word, 0.25 * UP))
        self.wait()
        self.play(eq.set_opacity, 1, likelihood_label.set_opacity, 1)
        self.wait()
        data = get_check_count_label(48, 2)
        data.scale(1.5)
        data.next_to(likelihood_label, DOWN, buff=2, aligned_edge=LEFT)
        data_arrow = Arrow(likelihood_label[1].get_bottom(), data.get_top())
        data_arrow.set_color(GREEN)
        self.play(GrowArrow(data_arrow), GrowFromPoint(data, data_arrow.get_start()))
        self.wait()
        self.play(FadeOut(data_arrow))
        self.play(over.set_opacity, 1, p_data_label.set_opacity, 1)
        self.wait()
        self.play(FadeOut(brace), FadeOut(post_word), FadeOut(post_bars), FadeOut(data), p_label.set_opacity, 0.1, eq.set_opacity, 0.1, likelihood_label.set_opacity, 0.1, over.set_opacity, 0.1, p_data_label.set_opacity, 0.1)
        self.bayes = VGroup(p_label, eq, prior_label, likelihood_label, over, p_data_label)
        self.data = data
    ...
    def parallel_universes(self):
        ...
        cols = VGroup()
        squares = VGroup()
        sample_colors = color_gradient([GREEN_C, GREEN_D, GREEN_E], 100)
        for bar in bars:
            n_rows = 12
            col = VGroup()
            for x in range(n_rows):
                square = Rectangle(width=bar.get_width(), height=bar.get_height() / n_rows)
            ...
            cols.add(col)
        squares.shuffle()
        self.play(LaggedStartMap(VFadeInThenOut, squares, lag_ratio=0.005, run_time=3))
        self.remove(squares)
        squares.set_opacity(1)
        self.wait()
        example_col = cols[95]
        self.play(bars.set_opacity, 0.25, FadeIn(example_col, lag_ratio=0.1))
        self.wait()
        ...
        self.play(FadeIn(self.data), FadeOut(example_col), self.bayes[3].set_opacity, 1)
        self.wait()
    ...
    def update_from_data(self):
        ...
        self.play(ReplacementTransform(bars.copy().set_opacity(0), new_bars), bars.set_fill, {'opacity': 0.1}, bars.set_stroke, {'opacity': 0.1}, run_time=2)
        bar95 = VGroup(bars[95].copy(), new_bars[95].copy())
        bar95.save_state()
        ...
        self.play(MoveToTarget(bar95), FadeInFromDown(ex_label), data.shift, LEFT)
        self.wait()
        side_brace = Brace(bar95[1], RIGHT, buff=SMALL_BUFF)
        side_label = side_brace.get_text('0.26', buff=SMALL_BUFF)
        self.play(GrowFromCenter(side_brace), FadeIn(side_label))
        self.wait()
        self.play(FadeOut(side_brace), FadeOut(side_label), FadeOut(ex_label))
        self.play(bar95.restore, bar95.set_opacity, 0)
        ...
        self.play(ShowCreation(rect), ShowCreation(tops))
        self.wait()
        self.play(LaggedStartMap(MoveToTarget, tops, run_time=2, lag_ratio=0.02), MoveToTarget(rect))
        ...
        for a, b in [(5, 2), (7, 1)]:
            ...
            for bar, saved in zip(bar_groups, bar_groups.saved_state):
                ...
                height = max(height, 1e-06)
                bar.target.set_height(height, about_edge=DOWN, stretch=True)
            self.play(LaggedStartMap(MoveToTarget, bar_groups, lag_ratio=0))
            self.wait()
        self.play(Restore(bar_groups))
        self.wait()
        ex_p_label = OldTex('P(s = 0.95 | 00000000) = ', tex_to_color_map={'s = 0.95': YELLOW, '00000000': WHITE})
        ...
        self.remove(data)
        self.play(FadeIn(ex_p_label), VFadeOut(data[0]), data[1:].move_to, template, FadeIn(highlight))
        self.wait()
        ...
        self.play(numer.next_to, h_line, UP, denom.next_to, h_line, DOWN, ShowCreation(h_line))
        self.wait()
        self.play(denom.space_out_submobjects, rate_func=there_and_back)
        self.play(bayes[4].set_opacity, 1, bayes[5].set_opacity, 1, FadeOut(rect))
        self.wait()
        self.play(FadeOut(highlight), FadeOut(ex_p_label), FadeOut(data), FadeOut(h_line), FadeOut(numer), FadeOut(denom), bayes.set_opacity, 1)
        new_bars.unlock_shader_data()
        self.remove(new_bars, *new_bars)
        self.play(new_bars.set_height, 5, {'about_edge': DOWN, 'stretch': True}, new_bars.set_color, MAROON_D)
        self.wait()
...
class UniverseOf95Percent(WhatsTheModel):
    ...
    def construct(self):
        ...
        self.add(label)
        for x in range(4):
            self.play(*self.experience_animations(self.seller, self.buyer, arc=30 * DEGREES, p=self.s))
        self.embed()
...
class OpenAndCloseAsideOnPdfs(Scene):
    ...
    def construct(self):
        labels = VGroup(OldTexText('$\\langle$', 'Aside on', ' pdfs', '$\\rangle$'), OldTexText('$\\langle$/', 'Aside on', ' pdfs', '$\\rangle$'))
...
class BayesRuleWithPdf(ShowLimitToPdf):
    ...
    def construct(self):
        ...
        prior_graph_top = Line(prior_graph.get_corner(UL), prior_graph.get_corner(UR))
        prior_graph_top.set_stroke(YELLOW, 3)
        bayes.save_state()
        bayes.set_opacity(0.2)
        prior.set_opacity(1)
        self.play(Restore(bayes, rate_func=reverse_smooth), FadeIn(prior_graph), ShowCreation(prior_graph_top))
        self.play(FadeOut(prior_graph_top))
        self.wait()
        nh = 1
        nt = 2
        scaled_graph = axes.get_graph(lambda x: scipy.stats.binom(3, x).pmf(1) + 1e-06)
        scaled_graph.set_stroke(GREEN)
        scaled_region = get_region_under_curve(axes, scaled_graph, 0, 1)

        def to_uniform(p, axes=axes):
            return axes.c2p(axes.x_axis.p2n(p), int(axes.y_axis.p2n(p) != 0))
        scaled_region.set_fill(opacity=0.75)
        scaled_region.save_state()
        scaled_region.apply_function(to_uniform)
        self.play(Restore(scaled_region), UpdateFromAlphaFunc(scaled_region, lambda m, a: m.set_opacity(a * 0.75)), likelihood.set_opacity, 1)
        self.wait()
        new_graph = get_beta_graph(axes, nh, nt)
        self.play(ApplyMethod(scaled_region.set_height, new_graph.get_height(), {'about_edge': DOWN, 'stretch': True}, run_time=2), over.set_opacity, 1, p_data.set_opacity, 1)
        self.wait()
        self.play(post.set_opacity, 1, eq.set_opacity, 1)
        self.wait()
        new_bayes = self.get_formula(lowercase=True)
        new_bayes.replace(bayes, dim_to_match=0)
        rects = VGroup(SurroundingRectangle(new_bayes[0][0]), SurroundingRectangle(new_bayes[6][0]))
    ...
    def get_formula(self, lowercase=False):
        ...
        bayes = OldTex(p_sym + '({s} \\,|\\, \\text{data})', '=', '{' + p_sym + '({s})', 'P(\\text{data} \\,|\\, {s})', '\\over', 'P(\\text{data})', tex_to_color_map={'{s}': YELLOW, '\\text{data}': GREEN})
...
class TalkThroughCoinExample(ShowBayesianUpdating):
    ...
    def construct(self):
        ...
        self.play(FadeOut(q_marks), TransformFromCopy(x_label, prob_box_x))
        ...
        last_row = VMobject()
        for row in rows:
            self.play(FadeOut(last_row, DOWN), FadeIn(row, lag_ratio=0.1))
            last_row = row
        self.play(FadeOut(last_row, DOWN))
        region = get_beta_graph(axes, 0, 0)
        graph = Line(region.get_corner(UL), region.get_corner(UR))
        func_label = OldTex('f(x) =', '1')
        func_label.next_to(graph, UP)
        self.play(FadeIn(func_label, lag_ratio=0.1), ShowCreation(graph))
        ...
        self.play(FadeIn(coin, DOWN), GrowArrow(arrow), Write(data_label, run_time=1))
        self.wait()
        bayes = OldTex('p({x} | \\text{data})', '=', 'p({x})', '{P(\\text{data} | {x})', '\\over', 'P(\\text{data})', tex_to_color_map={'{x}': WHITE, '\\text{data}': GREEN})
        ...
        self.play(Write(times_x), Transform(graph, new_graph))
        self.play(region.set_opacity, 0.5, FadeIn(sub_region))
        self.wait()
        low_x = 0.1
        high_x = 0.9
        lines = VGroup()
        ...
        self.play(FadeOut(likelihood_x_label), ReplacementTransform(likelihood_rect, p_data_rect))
        self.wait()
        one = func_label[1]
        two = OldTex('2')
        two.move_to(one, LEFT)
        self.play(FadeOut(region), sub_region.stretch, 2, 1, {'about_edge': DOWN}, sub_region.set_color, BLUE, graph.stretch, 2, 1, {'about_edge': DOWN}, FadeInFromDown(two), FadeOut(one, UP))
        region = sub_region
        ...
        self.play(FadeOut(two, UP), FadeIn(const, DOWN))
        ...
        self.wait()
        coin = coins[1]
        self.play(arrow.next_to, coin, DOWN, SMALL_BUFF, MaintainPositionRelativeTo(data_label, arrow), FadeInFromDown(coin))
        self.wait()
        to_prior_arrow = Arrow(func_label[0][3], bayes[6], max_tip_length_to_length_ratio=0.15, stroke_width=3)
        ...
        self.play(ShowCreation(likelihood_rect), FadeIn(l_1mx, 0.5 * DOWN))
        self.wait()
        self.play(ShowCreationThenFadeOut(Underline(p_label)))
        self.play(Indicate(coins[1]))
        self.wait()
        self.play(TransformFromCopy(l_1mx, eq_1mx), FadeIn(dot, RIGHT), func_label.next_to, dot, LEFT, 0.2)
        ...
        self.play(TransformFromCopy(likelihood_rect, p_data_rect), FadeOut(l_1mx))
        new_graph = get_beta_graph(axes, 1, 1)
        group = VGroup(graph, scaled_region)
        self.play(group.set_height, new_graph.get_height(), {'about_edge': DOWN, 'stretch': True}, group.set_color, BLUE, FadeOut(region))
        ...
        self.play(FadeIn(exp1, DOWN), FadeIn(exp2, DOWN), VGroup(func_label, dot, eq_1mx).shift, shift_vect, bayes.scale, 0.5, bayes.next_to, p_label, DOWN, LARGE_BUFF, {'aligned_edge': RIGHT})
        nh = 1
        nt = 1
        for coin, is_heads in zip(coins[2:10], bool_values[2:10]):
            self.play(arrow.next_to, coin, DOWN, SMALL_BUFF, MaintainPositionRelativeTo(data_label, arrow), FadeIn(coin, DOWN))
            if is_heads:
                nh += 1
                old_exp = exp1
            else:
            ...
            self.play(FadeOut(graph), FadeOut(region), FadeIn(new_graph), FadeIn(new_region), FadeOut(old_exp, MED_SMALL_BUFF * UP), FadeIn(new_exp, MED_SMALL_BUFF * DOWN))
            graph = new_graph
            region = new_region
            self.remove(new_exp)
            self.add(old_exp)
            ...
            if coin is coins[4]:
                ...
                self.play(GrowFromPoint(area_label, const.get_center()))

class PDefectEqualsQmark(Scene):

    def construct(self):
        label = OldTex('P(\\text{Defect}) = ???', tex_to_color_map={'\\text{Defect}': RED})
...
class UpdateOnceWithBinomial(TalkThroughCoinExample):
    ...
    def construct(self):
        ...
        self.add(coins[:10])
        region = get_beta_graph(axes, 0, 0)
        graph = axes.get_graph(lambda x: 1, min_samples=30)
        self.add(region, graph)
        bayes = OldTex('p({x} | \\text{data})', '=', 'p({x})', '{P(\\text{data} | {x})', '\\over', 'P(\\text{data})', tex_to_color_map={'{x}': WHITE, '\\text{data}': GREEN})
        ...
        self.wait()
        self.play(GrowFromCenter(brace), FadeIn(all_data_label, 0.2 * UP))
        self.wait()
        nh = sum(bool_values[:10])
        nt = sum(~bool_values[:10])
        likelihood_brace = Brace(likelihood, UP)
        t2c = {str(nh): BLUE, str(nt): RED}
        binom_formula = OldTex('{10 \\choose ', str(nh), '}', 'x^{', str(nh), '}', '(1-x)^{' + str(nt) + '}', tex_to_color_map=t2c)
        ...
        self.play(TransformFromCopy(brace, likelihood_brace), FadeOut(all_data_label), FadeIn(binom_formula))
        self.wait()
        rhs = OldTex('C \\cdot', 'x^{', str(nh), '}', '(1-x)^{', str(nt), '}', tex_to_color_map=t2c)
        ...
        dist = scipy.stats.beta(nh + 1, nt + 1)
        new_graph = axes.get_graph(dist.pdf)
        new_graph.shift(1e-06 * UP)
        new_graph.set_stroke(WHITE, 1, opacity=0.5)
        new_region = get_region_under_curve(axes, new_graph, 0, 1)
        new_region.match_style(region)
        new_region.set_opacity(0.75)
        self.add(new_region, new_graph, bayes)
        self.play(FadeOut(graph), FadeOut(region), FadeIn(new_graph), FadeIn(new_region), run_time=1)
        self.play(Write(eq), FadeIn(rhs, UP))
        self.wait()