...
CHEAP_AMBIENT_LIGHT_CONFIG = {'num_levels': 5, 'radius': 0.25, 'opacity_function': DEFAULT_OPACITY_FUNCTION}
HIGHT_QUALITY_AMBIENT_LIGHT_CONFIG = {'opacity_function': DEFAULT_OPACITY_FUNCTION, 'num_levels': 100, 'radius': 5, 'max_opacity': 0.8, 'color': PRODUCT_COLOR}
...
class Introduction(Scene):
    ...
    def construct(self):
        n_terms = 10
        number_line = NumberLine(x_min=0, x_max=2, unit_size=5, tick_frequency=0.25, big_tick_numbers=[0, 1, 2], color=GREY_B)
        ...
        def get_arrow_update():
            return ApplyFunction(lambda mob: mob.next_to(number_line.number_to_point(curr_product), UP, SMALL_BUFF), arrow)
        ...
        decimal_anim = ChangingDecimal(decimal, lambda a: number_line.point_to_number(arrow.get_center()), tracked_mobject=arrow)
        ...
        for n in range(1, len(rects)):
            ...
            self.play(get_arrow_update(), decimal_anim, Transform(rect, rects[n]), run_time=0.5)
            self.wait(0.5)
...
class TableOfContents(Scene):
    ...
    def construct(self):
        topics = VGroup(OldTexText('The setup'), OldTexText('Circle geometry with complex polynomials'), OldTexText('Proof of the Wallis product'), OldTexText('Formalities not discussed'), OldTexText('Generalizing this proof to get \\\\ the product formula for sine'))
        for topic in topics:
            dot = Dot(color=BLUE)
        ...
        for i in range(len(topics)):
            self.play(topics[i + 1:].set_fill, {'opacity': 0.25}, topics[:i].set_fill, {'opacity': 0.25}, topics[i].set_fill, {'opacity': 1})
            self.wait(2)
...
class SourcesOfOriginality(TeacherStudentsScene):
    ...
    def mention_excitement(self):
        self.teacher_says('This one came about \\\\ a bit differently...', target_mode='speaking', run_time=1)
    ...
    def break_down_value_of_math_presentations(self):
        ...
        left_rect, right_rect = self.left_rect, self.right_rect = [Rectangle(height=big_rect.get_height() - 2 * SMALL_BUFF, width=0.5 * big_rect.get_width() - 2 * SMALL_BUFF, color=color) for color in (MATH_COLOR, COMMUNICATION_COLOR)]
        right_rect.flip()
        ...
        formula = OldTex('\\sum_{n = 1}^\\infty \\frac{1}{n^2} = \\frac{\\pi^2}{2}')
        formula.scale(0.75)
        ...
        ambient_light = AmbientLight(num_levels=200, radius=5, opacity_function=DEFAULT_OPACITY_FUNCTION)
        ambient_light.move_source_to(lighthouse.get_top())
        big_rect.save_state()
        big_rect.stretch(0, 1)
        big_rect.stretch(0.5, 0)
        big_rect.move_to(title)
        self.play(FadeInFromDown(title), RemovePiCreatureBubble(self.teacher, target_mode='raise_right_hand', look_at=title), self.change_students(*['pondering'] * 3, look_at=title))
        self.play(big_rect.restore)
        self.play(*list(map(ShowCreation, [left_rect, right_rect])))
        self.wait()
        self.play(math.match_color, left_rect, ReplacementTransform(VGroup(math.copy()), underlying_math))
        self.play(FadeIn(formula))
        self.play(presentation.match_color, right_rect, ReplacementTransform(presentation.copy(), communication))
        self.play(FadeIn(communication_parts), FadeIn(lighthouse), SwitchOn(ambient_light))
        self.play(self.teacher.change, 'tease')
        self.wait()
        self.play(FadeIn(based_on_wastlund), self.change_students('sassy', 'erm', 'plain', look_at=based_on_wastlund))
        self.wait()
    ...
    def where_we_fit_in(self):
        ...
        points = [right_rect.get_left() + SMALL_BUFF * RIGHT, right_rect.get_corner(UL), right_rect.get_corner(UR), right_rect.get_right() + SMALL_BUFF * LEFT, right_rect.get_corner(DR), right_rect.get_bottom() + SMALL_BUFF * UP, right_rect.get_corner(DL)]
        added_points = [left_rect.get_bottom(), left_rect.get_corner(DL), left_rect.get_corner(DL) + 1.25 * UP, left_rect.get_bottom() + 1.25 * UP]
        ...
        arrow = Arrow(our_contribution.get_bottom(), right_rect.get_right() + MED_LARGE_BUFF * LEFT, color=BLUE)
        ...
        self.play(DrawBorderThenFill(blob1), Write(our_contribution), GrowArrow(arrow))
        self.wait(2)
        self.play(FadeOut(self.math_content))
        self.play(FadeIn(wallis_product), Write(wallis_product_name, run_time=1))
        self.wait(2)
        self.play(Transform(blob1, blob2, path_arc=-90 * DEGREES), FadeIn(new_proof), self.teacher.change, 'hooray')
...
class SridharWatchingScene(PiCreatureScene):
    CONFIG = {'default_pi_creature_kwargs': {'color': YELLOW_E, 'flip_at_start': False}}
    ...
    def construct(self):
        ...
        basel = OldTex('{1', '\\over', '1^2}', '+{1', '\\over', '2^2}', '+{1', '\\over', '3^2}', '+', '\\cdots', '= \\frac{\\pi^2}{6}')
        ...
        self.play(ReplacementTransform(basel.copy(), wallis), GrowFromPoint(arrow, arrow.get_top()))
        self.wait(4)
        self.play(sridhar.change, 'thinking', wallis)
        self.wait(4)
        self.play(LaggedStartMap(ApplyFunction, VGroup(*list(laptop) + [bubble, basel, arrow, wallis, sridhar]), lambda mob: (lambda m: m.set_color(BLACK).fade(1).scale(0.8), mob), run_time=3))
...
class ShowProduct(Scene):
    ...
    def show_larger_terms(self):
        ...
        for part, line, dot, prod, new_brace in tuples:
            self.play(FadeIn(part), Transform(brace, new_brace), ChangeDecimalToValue(decimal, prod, position_update_func=lambda m: m.next_to(brace, DOWN)), ShowCreation(line), GrowFromCenter(dot, rate_func=squish_rate_func(smooth, 0.5, 1)), run_time=0.5)
            self.wait(0.5)
        N = len(parts)
        self.play(LaggedStartMap(ShowCreation, lines[N - 1:], lag_ratio=0.2), LaggedStartMap(FadeIn, dots[N:], lag_ratio=0.2), brace.stretch, 1.2, 0, {'about_edge': LEFT}, ChangeDecimalToValue(decimal, partial_products[-1], position_update_func=lambda m: m.next_to(brace, DOWN)), run_time=4, rate_func=linear)
        self.play(FadeOut(brace), ChangeDecimalToValue(decimal, partial_products[-1] + 2, position_update_func=lambda m: m.next_to(brace, DOWN)), UpdateFromFunc(decimal, lambda d: d.shift(self.frame_duration * RIGHT)), UpdateFromAlphaFunc(decimal, lambda d, a: d.set_fill(opacity=1 - a)))
        self.remove(decimal)
    ...
    def show_smaller_terms(self):
        ...
        vect = op.sub(smaller_parts.denominators[1].get_left(), smaller_parts.denominators[0].get_left())
        smaller_parts.denominators.shift(vect)
        self.play(larger_parts_mover.move_to, smaller_parts, LEFT, FadeOut(self.graph_to_remove))
        self.play(larger_parts_mover.denominators.shift, -vect, smaller_parts.denominators.shift, -vect, UpdateFromAlphaFunc(larger_parts_mover, lambda m, a: m.set_fill(opacity=1 - a), remover=True), UpdateFromAlphaFunc(smaller_parts, lambda m, a: m.set_fill(opacity=a)))
        ...
        lines = VGroup(*[Line(d1.get_center(), d2.get_center()) for d1, d2 in zip(dots, dots[1:])])
        self.play(ShowCreation(lines), LaggedStartMap(FadeIn, dots, lag_ratio=0.1), run_time=3, rate_func=linear)
        self.wait(2)
    ...
    def interleave_terms(self):
        ...
        for line, dot, new_brace in tuples:
            self.play(Transform(brace, new_brace), ChangeDecimalToValue(decimal, next(partial_products_iter), position_update_func=lambda m: m.next_to(brace, DOWN)), ShowCreation(line), GrowFromCenter(dot, rate_func=squish_rate_func(smooth, 0.5, 1)), run_time=0.5)
            self.wait(0.5)

        def get_decimal_anim():
            return ChangeDecimalToValue(decimal, next(partial_products_iter), run_time=1, rate_func=squish_rate_func(smooth, 0, 0.5))
        self.play(FadeIn(lines[len(parts) - 1:]), FadeIn(dots[len(parts):]), get_decimal_anim())
        for x in range(3):
    ...
    def show_answer(self):
        ...
        randy.change('plain')
        randy.fade(1)
        h_line = DashedLine(axes.coords_to_point(0, np.pi / 2), axes.coords_to_point(20, np.pi / 2), color=RED)
        ...
        self.play(self.get_decimal_anim(), UpdateFromAlphaFunc(decimal, lambda m, a: m.set_fill(opacity=1 - a)), ReplacementTransform(randy, pi_halves[0]), Write(pi_halves[1:]))
        self.remove(decimal)
        self.wait()

    def get_axes(self, unit_size):
        y_max = 7
        axes = Axes(x_min=-1, x_max=12.5, y_min=-0.5, y_max=y_max + 0.25, y_axis_config={'unit_size': unit_size, 'big_tick_numbers': list(range(1, y_max + 1)), 'tick_size': 0.05})
...
class TeacherShowing(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(screen), self.teacher.change, 'raise_right_hand', screen, self.change_students(*['pondering'] * 3, look_at=screen))
        self.wait(5)

class DistanceProductScene(MovingCameraScene):
    CONFIG = {'ambient_light_config': HIGHT_QUALITY_AMBIENT_LIGHT_CONFIG, 'circle_color': BLUE, 'circle_radius': 3, 'num_lighthouses': 6, 'lighthouse_height': 0.5, 'ignored_lighthouse_indices': [], 'observer_config': {'color': MAROON_B, 'mode': 'pondering', 'height': 0.25, 'flip_at_start': True}, 'observer_fraction': 1.0 / 3, 'd_label_height': 0.35, 'numeric_distance_label_height': 0.25, 'default_product_column_top': FRAME_WIDTH * RIGHT / 4 + 1.5 * UP, 'include_lighthouses': True, 'include_distance_labels_background_rectangle': True}

    def setup(self):
        super(DistanceProductScene, self).setup()
        self.circle = Circle(color=self.circle_color, radius=self.circle_radius)
    ...
    def get_lh_points(self):
        return np.array([self.get_circle_point_at_proportion(fdiv(i, self.num_lighthouses)) for i in range(self.num_lighthouses) if i not in self.ignored_lighthouse_indices])
    ...
    def get_numeric_distance_labels(self, lines=None, num_decimal_places=3, show_ellipsis=True):
        ...
        for line in lines:
            label = DecimalNumber(line.get_length() / radius, num_decimal_places=num_decimal_places, show_ellipsis=show_ellipsis, include_background_rectangle=self.include_distance_labels_background_rectangle)
    ...
    def get_distance_product_column(self, column_top=None, labels=None, fraction=None):
        ...
        product_decimal = DecimalNumber(self.get_distance_product(fraction), num_decimal_places=3, show_ellipsis=True, include_background_rectangle=self.include_distance_labels_background_rectangle)
    ...
    def get_fractional_arc(self, fraction, start_fraction=0):
        arc = Arc(angle=fraction * TAU, start_angle=start_fraction * TAU, radius=self.get_radius())
        arc.shift(self.circle.get_center())
        return arc

    def get_halfway_indication_arcs(self):
        fraction = 0.5 / self.num_lighthouses
        arcs = VGroup(self.get_fractional_arc(fraction), self.get_fractional_arc(-fraction, start_fraction=2 * fraction))
...
class IntroduceDistanceProduct(DistanceProductScene):
    CONFIG = {'ambient_light_config': {'color': YELLOW}}
    ...
    def draw_circle_with_points(self):
        ...
        lh_dot_arrows = VGroup(*[Arrow(*[interpolate(circle.get_center(), dot.get_center(), a) for a in (0.6, 0.9)], buff=0) for dot in lh_dots])
        ...
        self.play(LaggedStartMap(ShowCreation, lh_dots), LaggedStartMap(GrowArrow, lh_dot_arrows), Write(evenly_space_dots_label))
        self.wait()
        self.play(special_dot.restore, GrowArrow(special_dot_arrow), Write(special_dot_label, run_time=1), FadeOut(VGroup(lh_dot_arrows, evenly_space_dots_label)))
        self.wait()
    ...
    def turn_into_lighthouses_and_observer(self):
        ...
        self.play(LaggedStartMap(FadeOut, self.lh_dots), LaggedStartMap(FadeIn, lighthouses), LaggedStartMap(SwitchOn, lights))
        self.wait()
    ...
    def show_sum_of_inverse_squares(self):
        ...
        sum_of_inverse_squares = OldTex(*it.chain(*[['{1', '\\over', '(', 'd_%d' % i, ')', '^2}', '+'] for i in range(len(lines))]))
        ...
        basel_sum = OldTex('{1 \\over 1^2} + ', '{1 \\over 2^2} + ', '{1 \\over 3^2} + ', '{1 \\over 4^2} + ', '\\cdots')
        basel_sum.next_to(arrow, DOWN)
        basel_cross = Cross(basel_sum)
        useful_for = OldTexText('Useful for')
        useful_for.next_to(arrow, RIGHT)
        wallis_product = OldTex('{2 \\over 1} \\cdot', '{2 \\over 3} \\cdot', '{4 \\over 3} \\cdot', '{4 \\over 5} \\cdot', '{6 \\over 5} \\cdot', '{6 \\over 7} \\cdot', '\\cdots')
        wallis_product.move_to(basel_sum)
        light_rings = VGroup(*it.chain(*self.lights))
        self.play(LaggedStartMap(ShowCreation, lines), LaggedStartMap(Write, labels))
        circle_group = VGroup(*self.get_top_level_mobjects())
        self.wait()
        self.play(ReplacementTransform(labels[-1].copy(), last_term[3]), Write(VGroup(*it.chain(last_term[:3], last_term[4:]))))
        self.remove(last_term)
        self.add(last_term)
        self.wait()
        self.play(Write(non_d_terms), ReplacementTransform(labels[:-1].copy(), d_terms[:-1]), circle_group.scale, 0.8, {'about_point': FRAME_Y_RADIUS * DOWN})
        self.wait()
        self.play(LaggedStartMap(ApplyMethod, light_rings, lambda m: (m.set_fill, {'opacity': 2 * m.get_fill_opacity()}), rate_func=there_and_back, run_time=3))
        self.wait()
        circle_group.save_state()
        v_point = VectorizedPoint(FRAME_X_RADIUS * LEFT + FRAME_Y_RADIUS * DOWN)
        self.play(circle_group.next_to, v_point, UR, {'submobject_to_align': self.circle}, circle_group.scale, 0.5, {'about_point': v_point.get_center()})
        self.play(GrowFromCenter(brace), Write(brace_text))
        self.wait()
        self.play(FadeOut(brace_text), GrowArrow(arrow), FadeIn(useful_for), FadeIn(basel_sum))
        self.wait()
        self.play(ShowCreation(basel_cross), FadeOut(VGroup(arrow, useful_for, brace)))
        basel_group = VGroup(basel_sum, basel_cross)
        self.play(basel_group.scale, 0.5, basel_group.to_corner, DR)
        self.play(Write(wallis_product))
        self.wait()
        self.play(circle_group.restore, wallis_product.match_width, basel_sum, wallis_product.next_to, basel_sum, UP, {'aligned_edge': RIGHT})
        self.play(d_terms.shift, 0.75 * d_terms.get_height() * UP, d_terms.set_color, PRODUCT_COLOR, light_rings.set_fill, PRODUCT_COLOR, *[FadeOut(mob) for mob in sum_of_inverse_squares if mob not in d_terms and mob not in plusses])
        self.wait()
        self.play(FadeOut(plusses), d_terms.arrange, RIGHT, 0.25 * SMALL_BUFF, d_terms.move_to, sum_of_inverse_squares, DOWN)
        self.wait()
        ...
        self.wait()
        self.play(LaggedStartMap(ApplyFunction, light_rings, lambda mob: (lambda m: m.shift(MED_SMALL_BUFF * UP).set_fill(opacity=2 * m.get_fill_opacity()), mob), rate_func=wiggle, run_time=6))
        self.wait()
    ...
    def transition_to_lemma_1(self):
        ...
        self.clear()
        self.play(MoveToTarget(circle), Transform(lighthouses, new_lighthouses), Transform(lights, new_lights), ApplyMethod(self.observer_dot.move_to, self.get_circle_point_at_proportion(self.observer_fraction / self.num_lighthouses)), MaintainPositionRelativeTo(self.observer, self.observer_dot))

class Lemma1(DistanceProductScene):
    CONFIG = {'circle_radius': 2.5, 'observer_fraction': 0.5, 'lighthouse_height': 0.25, 'lemma_text': 'distance product = 2', 'include_distance_labels_background_rectangle': False}
    ...
    def state_lemma_premise(self):
        ...
        show_arcs = ShowCreationThenDestruction(arcs, lag_ratio=0, run_time=2)
    ...
    def wiggle_observer(self):
        ...
        self.remove(*VGroup(self.observer, self.observer_dot, self.distance_lines, self.numeric_distance_labels, self.distance_product_column).get_family())
        self.play(ApplyMethod(observer_angle_tracker.set_value, observer_angle + 0.05 * TAU, rate_func=wiggle), UpdateFromFunc(self.observer_dot, update_dot), MaintainPositionRelativeTo(self.observer, self.observer_dot), UpdateFromFunc(self.distance_lines, update_distance_lines), UpdateFromFunc(self.numeric_distance_labels, update_numeric_distance_labels), UpdateFromFunc(self.distance_product_column, update_distance_product_column), run_time=5)
        self.distance_product_column[-1].set_color(BLUE).scale(1.05)
        self.wait()

class Lemma1With7Lighthouses(Lemma1):
    CONFIG = {'num_lighthouses': 7}

class Lemma1With8Lighthouses(Lemma1):
    CONFIG = {'num_lighthouses': 8}

class Lemma1With9Lighthouses(Lemma1):
    CONFIG = {'num_lighthouses': 9}

class Lemma2(Lemma1):
    CONFIG = {'lemma_text': 'distance product = \\# Initial lighthouses'}
    ...
    def replace_first_lighthouse(self):
        ...
        self.play(lighthouse_group.shift, 5 * RIGHT, lighthouse_group.fade, 1, run_time=1.5, rate_func=running_start, remover=True)
        self.play(dot.move_to, point, observer_anim, path_arc=-120 * DEGREES)
        self.wait()
...
class FromGeometryToAlgebra(DistanceProductScene):
    ...
    def point_out_evenly_spaced(self):
        ...
        arcs = VGroup(*[VMobject().pointwise_become_partial(circle, a1, a2) for a1, a2 in zip(alpha_range, alpha_range[1:])])
        arcs.set_stroke(YELLOW, 5)
        for arc in arcs[::2]:
            arc.reverse_points()
    ...
    def transition_to_complex_plane(self):
        plane = self.complex_plane = ComplexPlane(unit_size=2, y_radius=6, x_radius=9)
        ...
        for part in roots_of_unity_words:
            part.add_background_rectangle()
        self.play(Write(plane), MoveToTarget(circle_group), FadeOut(self.spacing_words), MoveToTarget(geometric_words), FadeIn(arrow), FadeIn(algebraic_words))
        ...
        complex_decimal = self.complex_decimal = DecimalNumber(values[3], num_decimal_places=3, include_background_rectangle=True)
        ...
        self.play(FadeIn(complex_plane_words), FadeOut(word_group), FadeIn(complex_decimal), FadeIn(outer_arrow))
        self.wait(2)
        self.play(ChangeDecimalToValue(complex_decimal, values[1], tracked_mobject=complex_decimal_rect), complex_decimal_rect.next_to, outer_arrows[1].get_start(), UP, SMALL_BUFF, Transform(outer_arrow, outer_arrows[1]), run_time=1.5)
        self.wait()
        arrows = self.get_arrows()
        arrows.set_color(YELLOW)
        self.play(ReplacementTransform(unit_circle_words, roots_of_unity_words), LaggedStartMap(GrowArrow, arrows))
        self.wait()
        self.play(complex_plane_words.move_to, word_group, LaggedStartMap(FadeOut, VGroup(*it.chain(arrows, roots_of_unity_words))))
    ...
    def discuss_powers(self):
        ...
        angle_arcs = VGroup(*[Arc(angle=alpha * TAU, radius=0.35).shift(origin) for alpha in np.arange(0, 1 + step, step)])
        angle_labels = VGroup()
        ...
        L_labels = self.L_labels = VGroup(*[OldTex('L_%d' % i).move_to(power_label, DOWN).add_background_rectangle(opacity=1) for i, power_label in enumerate(power_labels)])
        ...
        for i in list(range(2, self.num_lighthouses)) + [0]:
            anims = [Transform(angle_arc, angle_arcs[i]), Transform(angle_label, angle_labels[i]), Transform(line, lines[i], path_arc=TAU / self.num_lighthouses)]
            if i == 2:
                anims.append(FadeOut(magnitude_label))
            if i == 3:
                anims.append(FadeOut(question))
            self.play(*anims)
            new_anims = [GrowArrow(outer_arrows[i]), Write(power_labels[i])]
            if i == 2:
        ...
        self.wait()
        self.play(FadeOut(angle_arc), FadeOut(angle_label), *list(map(ShowCreationThenDestruction, lines)))
        self.wait()
    ...
    def raise_everything_to_the_nth(self):
        ...
        for i, light in enumerate(lights):
            rotations.append(Rotating(light, radians=i * TAU - i * TAU / self.num_lighthouses, about_point=origin, rate_func=bezier([0, 0, 1, 1])))
        ...
        self.wait(3)
        self.play(factored_polynomial[0].next_to, polynomial[1], RIGHT, 1.5 * SMALL_BUFF, FadeOut(polynomial[2]), FadeOut(factored_polynomial[1:]))
        ...
        for seven in sevens:
            ...
            n_terms.add(n_term)
        self.play(LaggedStartMap(FadeOut, VGroup(*it.chain(L1_rhs, self.outer_arrows, self.L_labels, self.outer_arrow, self.angle_line, self.ghost_angle_line))))
    ...
    def get_arrows(self):
        return VGroup(*[Arrow(interpolate(self.circle.get_center(), point, 0.6), interpolate(self.circle.get_center(), point, 0.9), buff=0) for point in self.get_lh_points()])

class PlugObserverIntoPolynomial(DistanceProductScene):
    CONFIG = {'num_lighthouses': 7, 'add_lights_in_foreground': True}
    ...
    def add_plane(self):
        plane = self.plane = ComplexPlane(unit_size=2, y_radius=5)
        plane.shift(DOWN)
        plane.add_coordinates()
    ...
    def introduce_observer(self):
        ...
        equals = OldTex('=')
        complex_decimal_animation = ChangingDecimal(complex_decimal, lambda a: self.plane.point_to_number(dot.get_center()), position_update_func=lambda m: m.next_to(equals, RIGHT, SMALL_BUFF))
        ...
        anims_for_dot_movement = self.anims_for_dot_movement = [MaintainPositionRelativeTo(arrow, dot), MaintainPositionRelativeTo(var_name, arrow), MaintainPositionRelativeTo(equals, var_name), complex_decimal_animation, lines_update]
        self.play(FadeIn(observer, direction=-vect), GrowArrow(arrow))
        self.play(Write(full_name))
        self.wait()
        self.play(ReplacementTransform(full_name[0], var_name[0]), ReplacementTransform(full_name[1][0], var_name[1][0]), FadeOut(full_name[1][1:]), ReplacementTransform(observer, dot), FadeIn(equals_decimal))
        self.add_foreground_mobject(dot)
        self.wait()
        self.play(ReplacementTransform(var_name.copy(), O_terms), ReplacementTransform(self.polynomial, new_polynomial))
        self.polynomial = new_polynomial
        self.wait()
        self.play(ShowCreation(rhs_rect))
        self.play(LaggedStartMap(ShowCreation, lines), Animation(dot))
        self.play(Rotating(dot, radians=TAU, rate_func=smooth, about_point=dot.get_center() + MED_LARGE_BUFF * LEFT, run_time=4), *anims_for_dot_movement)
        self.wait()
        ...
        self.play(FadeOut(VGroup(equals, complex_decimal, var_name, arrow)))
    ...
    def raise_observer_to_the_N(self):
        ...
        chord_halves = VGroup(Line(chord.get_center(), chord.get_start()), Line(chord.get_center(), chord.get_end()))
        ...
        numeric_chord_label = DecimalNumber(np.sqrt(3), num_decimal_places=4, include_background_rectangle=True, show_ellipsis=True)
        numeric_chord_label.rotate(chord.get_angle())
        numeric_chord_label.move_to(chord_label)
        self.play(FadeIn(question), ShowCreation(arc))
        for angle in [full_angle - obs_angle, -full_angle, obs_angle]:
            last_angle = angle_of_vector(dot.get_center() - origin)
            self.play(self.lines_update, UpdateFromAlphaFunc(arc, lambda arc, a: Transform(arc, get_arc(last_angle + a * angle)).update(1)), Rotate(dot, angle, about_point=origin), run_time=2)
        self.play(FadeOut(question[0]), FadeOut(question[2]), FadeIn(f_words))
        self.wait()
        self.play(FadeOut(self.lines), FadeOut(self.root_labels))
        self.play(ReplacementTransform(dot.copy(), O_to_N_dot), ReplacementTransform(arc, O_to_N_arc), path_arc=O_to_N_arc.angle - arc.angle)
        self.add_foreground_mobject(O_to_N_dot)
        self.play(FadeIn(O_to_N_label), GrowArrow(O_to_N_arrow))
        self.wait()
        self.play(FadeOut(question[1]), f_words.next_to, around_circle_words, UP, SMALL_BUFF, FadeIn(around_circle_words))
        self.wait()
        self.play(FadeIn(chord_label[0]), ReplacementTransform(self.lhs.copy(), chord_label[1]), ShowCreation(chord))
        self.wait()
        light_rings = VGroup(*it.chain(self.lights))
        self.play(LaggedStartMap(ApplyMethod, light_rings, lambda m: (m.shift, MED_SMALL_BUFF * UP), rate_func=wiggle))
        self.play(FadeOut(around_circle_words), FadeIn(question[1]), ReplacementTransform(f_words, third_words))
        ...
        self.play(third_words.next_to, around_circle_words, UP, SMALL_BUFF, FadeIn(around_circle_words), FadeOut(question[1]))
        self.wait()
        ...
        self.play(FadeOut(chord), FadeOut(numeric_chord_label), FadeOut(O_to_N_group), FadeIn(self.lines), ReplacementTransform(self.lhs_rect.copy(), self.rhs_rect))
        self.wait()
        ...
        self.wait()
        self.play(third_words.next_to, question[1], UP, SMALL_BUFF, FadeOut(around_circle_words), FadeIn(question[1]))
        self.wait()
        chord_group = VGroup(chord, numeric_chord_label[1])
        chord_group.set_color(YELLOW)
        self.add_foreground_mobjects(*chord_group)
        self.play(FadeIn(chord), FadeIn(numeric_chord_label))
        self.wait()

    def get_polynomial_equation(self, var='x', color=None):
        if color is None:
            color = self.observer_config['color']
        equation = OldTex('\\left(', var, '^N', '-', '1', '\\right)', '=', '\\left(', var, '-', 'L_0', '\\right)', '\\left(', var, '-', 'L_1', '\\right)', '\\cdots', '\\left(', var, '-', 'L_{N-1}', '\\right)')
...
class DefineChordF(Scene):
    ...
    def construct(self):
        ...
        circle_group = VGroup(circle, chord, radius_line, one_label, brace, f_label, chord_f, half_chords, height, angle_arc, angle_label)
        formula = OldTex('= 2 \\cdot \\sin\\left(\\frac{f}{2} 2\\pi \\right)', '= 2 \\cdot \\sin\\left(f \\pi \\right)')
        for part in formula:
        ...
        self.play(ReplacementTransform(chord_f.copy(), chord_third), ReplacementTransform(f_label.copy(), third_label))
        chord_copies = VGroup()
        last_chord = chord
        for color in (PINK, BLUE):
            chord_copy = last_chord.copy()
            old_color = chord_copy.get_color()
            self.play(Rotate(chord_copy, -TAU / 6, about_point=last_chord.get_end()), UpdateFromAlphaFunc(chord_copy, lambda m, a: m.set_stroke(interpolate_color(old_color, color, a))))
            chord_copy.reverse_points()
            last_chord = chord_copy
            chord_copies.add(chord_copy)
        self.wait()
        self.play(FadeOut(chord_copies), ReplacementTransform(chord_third, chord_f), ReplacementTransform(third_label, f_label))
        ...
        self.play(ShowCreation(height), FadeIn(half_chords), ShowCreation(angle_arc), Write(angle_label))
        self.wait()
        self.play(MoveToTarget(top_chord_f), circle_group.shift, 1.5 * DOWN)
...
class DistanceProductIsChordF(PlugObserverIntoPolynomial):
    CONFIG = {'include_lighthouses': False, 'num_lighthouses': 8}
    ...
    def show_all_animations(self):
        ...
        f_arc, new_arc = [Arc(angle=TAU * f / self.num_lighthouses, radius=self.get_radius(), color=YELLOW).shift(circle.get_center()) for f in (fraction, 0.5)]
        self.add(f_arc)
        lines = self.lines = self.get_lines()
        ...
        equals_two_terms = VGroup(*[OldTex('=2').next_to(mob, DOWN, SMALL_BUFF) for mob in (chord_half, chord_f_as_product.target)])
        self.play(Write(fraction_words))
        self.wait()
        self.play(LaggedStartMap(ShowCreation, lines), LaggedStartMap(FadeIn, labels))
        self.play(FadeIn(black_rect), ReplacementTransform(labels.copy(), stacked_labels), ShowCreation(h_line), Write(times))
        self.wait(2)
        self.add_foreground_mobjects(chord_f[1], chord, O_dot, O_to_N_dot)
        self.play(FadeOut(labels), ShowCreation(chord), FadeIn(chord_f))
        self.play(MoveToTarget(chord_f_as_product))
        self.wait(2)
        self.play(Transform(lines, new_lines), Transform(f_arc, new_arc), Transform(chord, new_chord), chord_f.rotate, -chord_f.angle, chord_f.move_to, chord_half, MoveToTarget(O_dot), MoveToTarget(O_to_N_dot), MaintainPositionRelativeTo(O_dot.arrow, O_dot), MaintainPositionRelativeTo(O_dot.label, O_dot), MaintainPositionRelativeTo(O_to_N_dot.arrow, O_to_N_dot), MaintainPositionRelativeTo(O_to_N_dot.label, O_to_N_dot), *changing_decimals, path_arc=45 * DEGREES, run_time=2)
        self.play(Transform(chord_f, chord_half), Transform(f_terms, half_terms))
        self.wait()
...
class ProveLemma2(PlugObserverIntoPolynomial):
    ...
    def replace_first_lighthouse(self):
        ...
        self.add_foreground_mobject(dot)
        self.play(dot.move_to, light_to_remove, MaintainPositionRelativeTo(arrow, dot), MaintainPositionRelativeTo(O_label, dot), path_arc=-TAU / 2)
        black_rect = Rectangle(height=6, width=3.5, stroke_width=0, fill_color=BLACK, fill_opacity=1)
        black_rect.to_corner(DL, buff=0)
        ...
        self.play(LaggedStartMap(ShowCreation, lines), LaggedStartMap(FadeIn, labels))
        self.play(FadeIn(black_rect), ShowCreation(h_line), Write(times), ReplacementTransform(labels.copy(), stacked_labels))
        self.wait()
        self.play(ReplacementTransform(stacked_labels.copy(), VGroup(product_decimal)))
        self.wait()
        ...
        self.wait(2)
        self.play(VGroup(light_to_remove, zero_rects[0]).shift, FRAME_WIDTH * RIGHT / 2, path_arc=-60 * DEGREES, rate_func=running_start, remover=True)
        self.play(VGroup(stacked_labels[0], zero_rects[1]).shift, 4 * LEFT, rate_func=running_start, remover=True)
    ...
    def rearrange_polynomial(self):
        ...
        roots_of_unity_circle = VGroup(*[Circle(radius=0.2, color=YELLOW).move_to(point) for point in self.get_lh_points()])
        ...
        self.play(FadeIn(new_lhs_background), polynomial_background.stretch, 0.8, 0, polynomial_background.move_to, frac_line_copy, LEFT, equals_remaining_factors.arrange, RIGHT, SMALL_BUFF, equals_remaining_factors.next_to, frac_line_copy, RIGHT, MED_SMALL_BUFF, ReplacementTransform(first_factor, O_minus_1, path_arc=-90 * DEGREES), ShowCreation(frac_line), Animation(lhs), ReplacementTransform(lhs_rect, new_lhs_rect))
        self.play(roots_of_unity_circle[0].shift, FRAME_WIDTH * RIGHT / 2, path_arc=-60 * DEGREES, rate_func=running_start, remover=True)
        ...
        self.play(polynomial_background.stretch, 1.8, 0, {'about_edge': LEFT}, FadeIn(expanded_rhs), equals_remaining_factors.scale, 0.9, equals_remaining_factors.next_to, expanded_rhs, VGroup(new_lhs_background, lhs, frac_line, O_minus_1, new_lhs_rect).shift, LEFT)
        self.wait()
...
class LocalMathematician(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(Animation(screen), mathy.change, 'pondering', PiCreatureSays(randy, 'Check these \\\\ out!', target_mode='surprised', bubble_config={'height': 3, 'width': 4}, look_at=screen))
        self.play(Animation(screen), RemovePiCreatureBubble(randy, target_mode='raise_right_hand', look_at=screen))
        self.wait(2)
        self.play(PiCreatureSays(mathy, 'Ah yes, consider \\\\ $x^n - 1$ over $\\mathds{C}$...', look_at=randy.eyes), randy.change, 'happy', mathy.eyes)
        self.wait(3)
...
class ArmedWithTwoKeyFacts(TeacherStudentsScene, DistanceProductScene):
    CONFIG = {'num_lighthouses': 6, 'ambient_light_config': {'opacity_function': inverse_power_law(1, 1, 1, 6), 'radius': 1, 'num_levels': 100, 'max_opacity': 1}}
    ...
    def construct(self):
        ...
        for circle, f, dp in ((circle1, 0.5, '2'), (circle2, 0, 'N')):
            ...
            self.play(FadeIn(circle), LaggedStartMap(FadeIn, VGroup(*it.chain(lights))), LaggedStartMap(FadeIn, VGroup(*it.chain(group.family_members_with_points()))), self.teacher.change, 'raise_right_hand', self.change_students(*['pondering'] * 3))
        wallis_product.move_to(labels).to_edge(RIGHT)
        self.play(LaggedStartMap(FadeIn, wallis_product), self.teacher.change_mode, 'hooray', self.change_students(*['thinking'] * 3, look_at=wallis_product))
        self.wait(2)

class Sailor(PiCreature):
    CONFIG = {'flip_at_start': True, 'color': YELLOW_D, 'hat_height_factor': 1.0 / 6}

    def __init__(self, *args, **kwargs):
        PiCreature.__init__(self, *args, **kwargs)
        height = self.get_height() * self.hat_height_factor
        sailor_hat = SVGMobject(file_name='sailor_hat', height=height)
        rhombus = Polygon(UP, UP + 2 * RIGHT, 1.75 * RIGHT + 0.5 * UP, 0.5 * RIGHT + 0.1 * DOWN, 1.25 * LEFT + 0.15 * DOWN)
        ...
        sailor_hat.shift(0.1 * self.eyes.get_width() * RIGHT, 0.1 * self.eyes.get_height() * UP)
        self.add(sailor_hat)

class KeeperAndSailor(DistanceProductScene, PiCreatureScene):
    CONFIG = {'num_lighthouses': 9, 'circle_radius': 2.75, 'add_lights_in_foreground': False, 'text_scale_val': 0.7, 'observer_fraction': 0.5, 'keeper_color': BLUE, 'sailor_color': YELLOW_D, 'include_distance_labels_background_rectangle': False, 'big_circle_center': FRAME_WIDTH * LEFT / 2 + SMALL_BUFF * RIGHT}
    ...
    def place_lighthouses(self):
        ...
        for light in lights:
            ...
            light.add_to_back(dot)
        origin = circle.get_center()
        arrows = VGroup(*[Arrow(0.6 * (p - origin), 0.9 * (p - origin), buff=0).shift(origin) for p in self.get_lh_points()])
        ...
        self.play(LaggedStartMap(FadeIn, VGroup(*it.chain(lights))), LaggedStartMap(FadeIn, lighthouses), LaggedStartMap(GrowArrow, arrows))
    ...
    def introduce_observers(self):
        ...
        self.play(LaggedStartMap(Succession, observers, lambda m: (FadeIn, m, ApplyMethod, m.change, 'wave_1')))
        for pi in observers:
        ...
        for pi in observers:
            self.play(pi.set_height, 0.5, pi.next_to, pi.target_point, RIGHT, SMALL_BUFF, pi.dot.move_to, pi.target_point, pi.dot.set_fill, {'opacity': 1}, pi.title.scale, self.text_scale_val, pi.title.next_to, pi.target_point, RIGHT, {'buff': 0.6})
            if pi is sailor:
    ...
    def write_distance_product_fraction(self):
        fraction = self.distance_product_fraction = OldTex("{\\text{Keeper's distance product}", '\\over', "\\text{Sailor's distance product}}")
        ...
        sailor_dp_column, keeper_dp_column = [self.get_distance_product_column(4 * RIGHT + 1.5 * UP, labels, frac) for labels, frac in [(sailor_line_lengths, 0.5), (keeper_line_lengths, 0)]]
        ...
        result_fraction = self.result_fraction = OldTex('{N', '{\\text{distance} \\choose \\text{between obs.}}', '\\over', '2}')
        ...
        new_keeper_dp_decimal = DecimalNumber(self.num_lighthouses, num_decimal_places=3)
        ...
        self.play(ReplacementTransform(sailor_lines.copy(), VGroup(fraction[2])), ShowCreation(fraction[1]))
        self.wait()
        ...
        self.play(fraction.next_to, equals, LEFT, FadeIn(equals), ShowCreation(frac_line), ReplacementTransform(sailor_dp_decimal.copy(), two), FadeOut(sailor_dp_decimal_rect))
        self.wait()
        self.remove(*list(sailor_lines) + list(sailor_line_lengths))
        self.play(FadeOut(sailor_dp_column), ReplacementTransform(sailor_lines.deepcopy(), keeper_lines), ReplacementTransform(sailor_line_lengths.deepcopy(), keeper_line_lengths))
        ...
        self.wait(2)
        self.play(first_light_group.shift, 0.6 * FRAME_WIDTH * RIGHT, keeper_top_zero_group.shift, 0.4 * FRAME_WIDTH * RIGHT, FadeOut(keeper_dp_decimal), FadeOut(keeper_dp_decimal_rect), path_arc=-30 * DEGREES, rate_func=running_start)
        self.remove(first_light_group, keeper_top_zero_group)
        self.wait()
        self.play(ReplacementTransform(keeper_dp_column[0][1:].copy(), VGroup(new_keeper_dp_decimal)))
        self.wait()
        ...
        self.play(ReplacementTransform(keeper_lines.copy(), sailor_lines), ReplacementTransform(keeper_line_lengths.copy(), sailor_line_lengths), FadeOut(keeper_dp_column[:-1]), FadeOut(new_keeper_dp_decimal))
        self.play(Rotate(sailor_line_lengths[0], 30 * DEGREES, rate_func=wiggle))
        self.wait()
        self.play(ReplacementTransform(sailor_lines[0].copy(), dist), FadeIn(div), MoveToTarget(two))
        self.wait()
        self.play(two.restore, FadeOut(div), dist.restore, N.restore)
        self.play(FadeOut(sailor_lines), FadeOut(sailor_line_lengths))
        self.wait()
    ...
    def break_down_distance_product_by_parts(self):
        ...
        product_parts = self.product_parts = OldTex('{|L_1 - K|', '\\over', '|L_1 - S|}', '\\cdot', '{|L_2 - K|', '\\over', '|L_2 - S|}', '\\cdot', '{|L_3 - K|', '\\over', '|L_3 - S|}', '\\cdots')
        product_parts.set_color_by_tex_to_color_map({'K': BLUE, 'S': YELLOW})
        ...
        sailor_length_braces = VGroup(VMobject())
        keeper_length_braces = VGroup(VMobject())
        triplets = [('S', sailor_length_braces, DOWN), ('K', keeper_length_braces, UP)]
        ...
        self.play(LaggedStartMap(FadeIn, product_parts), LaggedStartMap(FadeIn, sailor_lines, rate_func=there_and_back, remover=True), LaggedStartMap(FadeIn, keeper_lines, rate_func=there_and_back, remover=True))
        ...
        self.play(ShowCreation(keeper_line), GrowFromCenter(keeper_brace))
        self.wait()
        self.play(ShowCreation(sailor_line), GrowFromCenter(sailor_brace))
        self.wait()
        for i in range(2, 4):
            self.play(Transform(keeper_line, keeper_lines[i]), Transform(keeper_brace, keeper_length_braces[i]))
            self.play(Transform(sailor_line, sailor_lines[i]), Transform(sailor_brace, sailor_length_braces[i]))
            self.wait()
        for i in range(4, self.num_lighthouses):
            anims = [Transform(keeper_line, keeper_lines[i]), Transform(sailor_line, sailor_lines[i])]
            if i == 4:
                anims += [FadeOut(sailor_brace), FadeOut(keeper_brace)]
            self.play(*anims)
    ...
    def grow_circle_and_N(self, circle_scale_factor=2, N_multiple=3, added_anims=None):
        ...
        if hasattr(self, 'keeper_line'):
            ...
            anims += [Transform(keeper_line, self.keeper_lines[-1]), Transform(sailor_line, self.sailor_lines[-1])]
    ...
    def show_limit_for_each_fraction(self):
        ...
        def get_angle_mob(p1, p2):
            ...
            arc.shift(center)
            return VGroup(center_dot, Line(center, p1), Line(center, p2), arc)
        ...
        limit_fractions = VGroup(OldTex('{2', '\\over', '1}'), OldTex('{4', '\\over', '3}'), OldTex('{6', '\\over', '5}'))
        limit_arrows = VGroup()
        ...
        self.play(Transform(keeper_line, keeper_lines[1]), Transform(sailor_line, sailor_lines[1]), FadeIn(term_rect), path_arc=-180 * DEGREES)
        self.wait(2)
        self.play(FadeOut(VGroup(keeper_line, sailor_line)), FadeIn(braces[:2]), FadeIn(angle_mob))
        self.wait()
        self.play(Transform(angle_mob, get_angle_mob(lh_points[1], sailor_point)))
        self.wait(2)
        self.play(Write(approx), ReplacementTransform(ratios[0].copy(), limit_fractions[0]), FadeOut(angle_mob))
        self.wait()
        self.play(ReplacementTransform(approx, limit_arrows[0]))
        self.let_N_approach_infinity(braces[:2])
        self.play(Transform(term_rect, term_rects[1]), ReplacementTransform(limit_arrows[0].copy(), limit_arrows[1]), FadeIn(braces[2:4]))
        for group, color in ((braces[:4], self.keeper_color), (braces[1:4], self.sailor_color)):
            self.play(group.scale, 0.95, {'about_point': center}, group.set_color, color, rate_func=there_and_back)
            self.wait(0.5)
        self.play(ReplacementTransform(ratios[1].copy(), limit_fractions[1]))
        self.wait()
        braces[4:6].set_color(YELLOW)
        self.play(Transform(term_rect, term_rects[2]), ReplacementTransform(limit_arrows[1].copy(), limit_arrows[2]), FadeIn(braces[4:6]), braces[1:4].set_color, YELLOW, ReplacementTransform(limit_fractions[1].copy(), limit_fractions[2]))
        self.let_N_approach_infinity(braces[:6])
        self.wait()
        ccw_product_group = VGroup(product_parts, limit_arrows, limit_fractions)
        cw_product_parts = OldTex('\\cdots', '{|L_{-3} - K|', '\\over', '|L_{-3} - S|}', '\\cdot', '{|L_{-2} - K|', '\\over', '|L_{-2} - S|}', '\\cdot', '{|L_{-1} - K|', '\\over', '|L_{-1} - S|}')
        cw_product_parts.match_height(product_parts)
        cw_product_parts.set_color_by_tex_to_color_map({'K': BLUE, 'S': YELLOW})
        ...
        cw_limit_fractions = VGroup(OldTex('{2', '\\over', '3}'), OldTex('{4', '\\over', '5}'), OldTex('{6', '\\over', '7}'))
        ...
        def show_braces(n):
            ...
            for group in (cw_group, VGroup(braces[0], *cw_group)):
                ...
                self.wait(0.5)
        self.play(ccw_product_group.scale, 0.5, {'about_edge': UL}, ccw_product_group.to_corner, UL, FadeOut(term_rect), FadeOut(braces[:6]), cw_product_parts.restore)
        term_rect = cw_term_rects[0].copy()
        self.play(LaggedStartMap(ShowCreationThenDestruction, cw_label_rects))
        self.wait()
        self.play(FadeIn(term_rect), FadeIn(braces[-2:]), FadeIn(braces[0]))
        show_braces(1)
        self.play(GrowArrow(cw_limit_arrows[0]), FadeIn(cw_limit_fractions[0]))
        self.wait()
        self.play(Transform(term_rect, cw_term_rects[1]), ReplacementTransform(cw_limit_arrows[0].copy(), cw_limit_arrows[1]), FadeIn(braces[-4:-2]), Write(cw_limit_fractions[1]))
        show_braces(2)
        self.wait()
        self.play(Transform(term_rect, cw_term_rects[2]), ReplacementTransform(cw_limit_arrows[1].copy(), cw_limit_arrows[2]), FadeIn(braces[-6:-4]), Write(cw_limit_fractions[2]))
        show_braces(3)
        self.let_N_approach_infinity(VGroup(braces[0], *braces[-6:]))
        self.wait()
        fractions = VGroup(*it.chain(*list(zip(limit_fractions, cw_limit_fractions))))
        ...
        self.play(FadeOut(limit_arrows), FadeOut(cw_limit_arrows), FadeOut(braces[-6:]), FadeOut(braces[0]), FadeOut(term_rect))
        self.play(cw_product_parts.scale, 0.5, cw_product_parts.next_to, product_parts, DOWN, {'aligned_edge': LEFT}, MoveToTarget(fractions), Write(dots), run_time=2, path_arc=90 * DEGREES)
        self.wait()
    ...
    def show_limit_of_lhs(self):
        ...
        half_two_pi_over_N = OldTex('{1', '\\over', '2}', '{2', '\\pi', '\\over', 'N}')
        ...
        self.wait()
        self.play(dist.next_to, equals, LEFT, FadeIn(equals), GrowFromCenter(brace))
        self.wait()
        ...
        self.wait()
        self.play(FadeOut(half_two_pi_over_N[:4]), pi.shift, SMALL_BUFF * LEFT)
        self.wait()
        self.play(ReplacementTransform(half_two_pi_over_N[-3:].copy(), pi_over_N[1:4]), FadeIn(pi_over_N[0]), FadeIn(pi_over_N[-1]), N.shift, N_shift * RIGHT, ReplacementTransform(distance_product_equals, approx3))
        self.wait()
        self.play(GrowArrow(pi_halves_arrow), wallis_product.shift, DOWN)
        self.play(Write(pi_halves))
        self.wait(2)
        self.play(wallis_product.next_to, last_equals, LEFT, 2 * SMALL_BUFF, FadeIn(last_equals))
    ...
    def let_N_approach_infinity(self, braces=None, factor=4, run_time=5, zoom_in_after=False):
        ...
        def update_title_heights(*titles):
            for title in titles:
                ...
                title.set_height(min(title.original_height, 0.8 * get_ks_distance()))
            if len(titles) > 1:
                return titles
            else:
                return titles[0]
        ...
        label_update_anim = UpdateFromFunc(labels, lambda ls: self.position_labels_outside_lights(update_title_heights(*ls)))
        sailor_dot_anim = UpdateFromFunc(sailor.dot, lambda d: d.move_to(self.get_circle_point_at_proportion(get_fraction() / 2)))
        sailor_title_anim = UpdateFromFunc(sailor.title, lambda m: update_title_heights(m).next_to(sailor.dot, RIGHT, SMALL_BUFF))
        keeper_title_anim = UpdateFromFunc(keeper.title, lambda m: update_title_heights(m).next_to(keeper.dot, RIGHT, SMALL_BUFF))
        braces_update_anim = UpdateFromFunc(braces, update_braces)
        lights[0].fade(1)
        labels[0].fade(1)
        all_updates = [light_update_anim, label_update_anim, sailor_dot_anim, sailor_title_anim, keeper_title_anim, braces_update_anim]
        self.play(fraction_tracker.set_value, target_fraction, *all_updates, run_time=run_time)
        if zoom_in_after:
            self.play(circle.scale, factor, {'about_point': circle.get_right()}, *all_updates, run_time=1)
            self.wait()
            self.play(circle.scale, 1.0 / factor, {'about_point': circle.get_right()}, *all_updates, run_time=1)
        self.wait()
        self.play(fraction_tracker.set_value, start_fraction, *all_updates, run_time=run_time / 2)
    ...
    def replace_lighthouses_with_labels(self):
        ...
        self.play(FadeOut(self.lighthouses[1:]), FadeIn(lighthouse_labels[1:]))
    ...
    def get_circle_brace(self, f1, f2):
        line = Line(self.get_circle_point_at_proportion(f1), self.get_circle_point_at_proportion(f2))
...
class HowThisArgumentRequiresCommunitingLimits(PiCreatureScene):
    ...
    def construct(self):
        ...
        scale_val = 0.7
        factors = OldTex('{|L_1 - K|', '\\over', '|L_1 - S|}', '\\cdot', '{|L_{-1} - K|', '\\over', '|L_{-1} - S|}', '\\cdot', '{|L_2 - K|', '\\over', '|L_2 - S|}', '\\cdot', '{|L_{-2} - K|', '\\over', '|L_{-2} - S|}', '\\cdot', '{|L_3 - K|', '\\over', '|L_3 - S|}', '\\cdot', '{|L_{-3} - K|', '\\over', '|L_{-3} - S|}', '\\cdots')
        factors.set_color_by_tex_to_color_map({'K': BLUE, 'S': YELLOW})
        equals = OldTex('=')
        ...
        self.play(PiCreatureSays(mathy, 'Whoa whoa whoa \\\\ there buddy', look_at=morty.eyes, target_mode='sassy'), morty.change, 'guilty', mathy.eyes)
        self.wait(2)
        self.play(RemovePiCreatureBubble(mathy, target_mode='raise_right_hand', look_at=factors), morty.change, 'pondering', factors, LaggedStartMap(FadeIn, factors))
        self.wait()
        self.play(FadeIn(equals), Write(result))
        self.wait()
        self.play(LaggedStartMap(GrowArrow, fraction_limit_arrows), LaggedStartMap(FadeInFrom, fraction_limits, direction=UP), run_time=4, lag_ratio=0.25)
        self.wait()
        self.play(LaggedStartMap(FadeIn, mult_signs), FadeIn(lower_equals), mathy.change, 'sassy')
        self.play(GrowArrow(result_limit_arrow), FadeIn(result_limit, direction=UP), morty.change, 'confused')
        self.wait(2)
        limit_rule = OldTex('\\left( \\lim_{N \\to \\infty} a_N^{(1)} \\right)', '\\left( \\lim_{N \\to \\infty} a_N^{(2)} \\right)', '\\cdots', '=', '\\lim_{N \\to \\infty} \\left( a_N^{(1)} a_N^{(2)} \\cdots \\right)')
        ...
        self.play(FadeInFromDown(limit_rule), mathy.change, 'angry', morty.change, 'erm')
        ...
        self.wait(3)
        self.play(Animation(limit_rule), morty.change, 'pondering', mathy.change, 'pondering')
        self.wait(3)
        mover = VGroup(top_line, fraction_limits, fraction_limit_arrows, mult_signs, lower_equals, result_limit, result_limit_arrow)
        self.play(mover.next_to, FRAME_HEIGHT * UP / 2, UP, limit_rule.to_edge, UP)
        ...
        self.play(FadeInFromDown(dominated_convergence), mathy.change, 'raise_right_hand')
...
class NonCommunitingLimitsExample(Scene):
    CONFIG = {'num_terms_per_row': 6, 'num_rows': 5, 'x_spacing': 0.75, 'y_spacing': 1}
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(Write, row_arrows), LaggedStartMap(ReplacementTransform, rows[:-1].copy(), lambda r: (r, next(row_products_iter))))
        self.wait()
        ...
        self.play(LaggedStartMap(Write, column_arrows), LaggedStartMap(ReplacementTransform, columns.copy(), lambda c: (c, next(column_limit_iter))))
        self.wait()
...
class DelicacyInIntermixingSeries(Scene):
    ...
    def construct(self):
        n_terms = 6
        top_product, bottom_product = products = VGroup(*[OldTex(*it.chain(*[['{%d' % (2 * x), '\\over', '%d}' % (2 * x + u), '\\cdot'] for x in range(1, n_terms + 1)])) for u in (-1, 1)])
        ...
        self.play(LaggedStartMap(FadeIn, top_product), LaggedStartMap(FadeIn, bottom_product))
        self.wait()
        for product in products:
        ...
        while True:
            try:
                new_terms = [next(bottom_parts_iter), next(top_parts_iter), next(top_parts_iter)]
        ...
        self.play(FadeOut(q_marks), LaggedStartMap(MoveToTarget, movers1, run_time=5, lag_ratio=0.2))
        self.play(GrowArrow(new_arrow), FadeIn(new_limit, LEFT), bottom_product.parts[3:].fade, 1)
        self.play(FadeIn(randy))
        self.play(randy.change, 'confused', new_limit)
        self.wait()
        self.play(Blink(randy))
        self.wait()
        self.play(LaggedStartMap(Transform, movers2, lambda m: (m, m.final_position), run_time=3, path_arc=TAU / 4))
...
class KeeperAndSailorForSineProduct(KeeperAndSailor):
    CONFIG = {'new_sailor_fraction': 0.7, 'big_circle_center': FRAME_WIDTH * LEFT / 2 + 2.6 * LEFT}
    ...
    def move_sailor_to_new_spot(self):
        ...
        self.wait()
        self.play(sailor.dot.move_to, target_point, sailor.dot.set_color, GREEN, sailor.set_color, GREEN, MaintainPositionRelativeTo(sailor, sailor.dot), MaintainPositionRelativeTo(sailor.title, sailor), ReplacementTransform(brace1, brace2), ReplacementTransform(words1, words2), run_time=1.5)
    ...
    def show_new_distance_product(self):
        ...
        self.play(ShowCreation(sailor_dp_rect), LaggedStartMap(ShowCreation, sailor_lines))
        self.wait()
        self.play(ShowCreation(two_cross))
        self.play(two_group.next_to, chord_f, DOWN, ReplacementTransform(VGroup(sailor_dp[:-6].copy(), sailor_dp[-6:-4].copy(), sailor_dp[-4:-2].copy(), sailor_dp[-2:].copy()), chord_f))
        self.wait()
    ...
    def show_new_limit_of_ratio(self):
        ...
        fraction_words.target[0][:9].next_to(VGroup(fraction_words.target[0][9:], fraction_words.target[1]), UP, SMALL_BUFF)
        fraction_words.target.next_to(fraction_brace.target, LEFT, SMALL_BUFF)
        self.play(LaggedStartMap(FadeIn, group))
        self.grow_circle_and_N(added_anims=[MoveToTarget(fraction_brace), MoveToTarget(fraction_words)])
        self.wait()
        self.play(big_group.next_to, self.distance_product_equals, RIGHT, {'submobject_to_align': frac2}, UpdateFromAlphaFunc(big_group[:-1], lambda m, a: m.set_fill(opacity=1 - a)), Transform(self.distance_product_equals, arrow))
        self.wait()
        self.result_fraction = frac2

    def show_new_infinite_product(self):
        scale_val = 0.7
        fractions = OldTex('\\cdots', '{|L_{-2} - K|', '\\over', '|L_{-2} - S|}', '\\cdot', '{|L_{-1} - K|', '\\over', '|L_{-1} - S|}', '\\cdot', '{|L_1 - K|', '\\over', '|L_1 - S|}', '\\cdot', '{|L_2 - K|', '\\over', '|L_2 - S|}', '\\cdot', '{|L_3 - K|', '\\over', '|L_3 - S|}', '\\cdots')
        fractions.scale(scale_val)
        fractions.move_to(self.observers)
        fractions.to_edge(RIGHT)
        fractions.set_color_by_tex_to_color_map({'K': BLUE, 'S': GREEN})
        keeper_lines = self.get_keeper_lines()
        sailor_lines = self.get_sailor_lines()
        ...
        product_lines = VGroup(DashedLine(full_limits_group.get_corner(DL), product.get_corner(UL)), DashedLine(full_limits_group.get_corner(DR), product.get_corner(UR)))
        product_lines.set_color(YELLOW)
        self.play(LaggedStartMap(FadeIn, fractions), *[LaggedStartMap(FadeIn, VGroup(*list(lines[-10:]) + list(lines[1:10])), rate_func=there_and_back, remover=True, run_time=3, lag_ratio=0.1) for lines in (keeper_lines, sailor_lines)])
        self.wait()
        self.play(LaggedStartMap(GrowArrow, limit_arrows), LaggedStartMap(FadeInFrom, limits, lambda m: (m, UP)), LaggedStartMap(FadeIn, dots))
        self.wait()
        self.play(ShowCreation(product_lines, lag_ratio=0), FadeIn(product))
        self.wait()
        result_fraction = self.result_fraction
        big_group = VGroup(fractions, limit_arrows, full_limits_group, product_lines, product)
        ...
        self.play(MoveToTarget(big_group), FadeIn(equals), MoveToTarget(result_fraction), FadeOut(self.distance_product_fraction), FadeOut(self.distance_product_equals))
        self.wait()
        ...
        self.wait()
        self.play(f_pi.arrange, RIGHT, {'buff': SMALL_BUFF}, f_pi.next_to, over, UP, SMALL_BUFF, FadeOut(dot_two), FadeOut(two_sine_f_pi[0]), sine_f_pi.shift, SMALL_BUFF * LEFT)
        self.wait()
        pairs = VGroup()
        ...
        pairs.add(VGroup(f_pi, sine_f_pi), VGroup(product[1], product[3:]))
        for pair in pairs:
            pair.generate_target()
            pair.target[0].move_to(pair[1], UP)
            pair.target[1].move_to(pair[0], DOWN)
        self.play(LaggedStartMap(MoveToTarget, pairs, path_arc=180 * DEGREES, run_time=3), product_lines[1].scale, 0.9, {'about_point': product_lines[1].get_start()}, product_lines[1].shift, SMALL_BUFF * UP)
        self.wait()
        ...
        self.play(FadeIn(one_minus_f_over_k[0]), FadeIn(one_minus_f_over_k[-1]), FadeOut(product_lines), *[ReplacementTransform(product[i], one_minus_f_over_k[j]) for i, j in [(3, 1), (4, 2), (5, 3), (2, 4), (1, 5)]])
        self.wait()
        ...
        self.play(sine_f_pi.next_to, equals, LEFT, SMALL_BUFF, FadeOut(over), MoveToTarget(f_pi), MoveToTarget(product))
        self.wait()
        rect = SurroundingRectangle(VGroup(sine_f_pi, product))
        rect.set_color(BLUE)
        pi_creatures = VGroup(PiCreature(color=BLUE_E), PiCreature(color=BLUE_C), PiCreature(color=BLUE_D), Mortimer())
        ...
        for pi in pi_creatures:
            ...
            pi.change('plain')
            pi.fade(1)
        self.play(ShowCreation(rect), LaggedStartMap(ApplyMethod, pi_creatures, lambda m: (m.restore,)))
        for x in range(4):
...
class Conclusion(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        sine_formula = OldTex('\\sin(', 'f', '\\pi', ')', '=', 'f', '\\pi', '\\prod_{k \\ne 0}', '\\left(', '1', '-', '{f', '\\over', 'k}', '\\right)')
        ...
        basel_problem = OldTex('\\frac{1}{1^2} + ', '\\frac{1}{2^2} + ', '\\frac{1}{3^2} + ', '\\cdots', '\\frac{\\pi^2}{6}')
        basel_problem.move_to(self.hold_up_spot, DOWN)
        implication = OldTex('\\Rightarrow')
        implication.next_to(basel_problem[0][1], LEFT)
        self.play(self.teacher.change, 'raise_right_hand', FadeInFromDown(wallis_product), self.change_students('thinking', 'hooray', 'surprised'))
        self.wait()
        self.play(self.teacher.change, 'hooray', FadeInFromDown(sine_formula), wallis_product.to_edge, UP, self.change_students('pondering', 'thinking', 'erm'))
        self.wait(3)
        self.play(sine_formula.next_to, implication, LEFT, {'submobject_to_align': sine_formula[-1]}, FadeIn(implication))
        euler.next_to(sine_formula, UP)
        self.play(FadeIn(euler), LaggedStartMap(FadeIn, basel_problem), self.teacher.change, 'happy', self.change_students('sassy', 'confused', 'hesitant'))
        self.wait(2)
        ...
        self.play(ShowCreation(wallis_rect), FadeOut(implication), FadeOut(euler), FadeOut(sine_formula))
        self.play(ShowCreation(basel_rect), self.teacher.change, 'surprised', self.change_students(*['happy'] * 3))
        self.wait(5)
...
class SponsorUnderlay(PiCreatureScene):
    CONFIG = {'default_pi_creature_start_corner': DR}
    ...
    def construct(self):
        ...
        self.play(url.restore, ShowCreation(rect), path_arc=90 * DEGREES)
        self.wait(2)
