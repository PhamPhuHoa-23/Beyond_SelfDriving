...
class Orbiting(VGroup):
    ...
    def update(self, dt):
        ...
        d_prop = 0.001
        ds = get_norm(op.add(ellipse.point_from_proportion((prop + d_prop) % 1), -ellipse.point_from_proportion(prop)))
...
class SunAnimation(Group):
    CONFIG = {'rate': 0.2, 'angle': 60 * DEGREES}
...
class ShowWord(Animation):
    CONFIG = {'time_per_char': 0.06, 'rate_func': linear}
...
class TakeOver(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.pi_creature_says('Muahaha!  All \\\\ mine now.', bubble_config={'fill_opacity': 0.5}, bubble_creation_class=FadeIn, target_mode='conniving', added_anims=[henry.rotate, 5 * DEGREES])
        self.wait(2)

class ShowEmergingEllipse(Scene):
    CONFIG = {'circle_radius': 3, 'circle_color': BLUE, 'num_lines': 150, 'lines_stroke_width': 1, 'eccentricity_vector': 2 * RIGHT, 'ghost_lines_stroke_color': GREY_B, 'ghost_lines_stroke_width': 0.5, 'ellipse_color': PINK}
    ...
    def construct(self):
        ...
        self.wait()
        self.play(LaggedStartMap(ShowCreation, shuffled_lines), Animation(VGroup(e_dot, circle)), FadeOut(eccentric_words))
        self.add(ghost_lines)
        self.add(e_dot, circle)
        self.wait()
        self.play(FadeIn(fade_rect), Animation(line), GrowFromCenter(line_dot), FadeInFromDown(rot_words))
        self.wait()
        self.add(ghost_line)
        self.play(MoveToTarget(line, path_arc=90 * DEGREES), Animation(rot_words), ShowCreation(elbow))
        self.wait()
        self.play(FadeOut(fade_rect), FadeOut(line_dot), FadeOut(rot_words), FadeOut(elbow), Animation(line), Animation(ghost_line))
        self.play(LaggedStartMap(MoveToTarget, lines, run_time=4), Animation(VGroup(e_dot, circle)))
        self.wait()
    ...
    def get_lines(self):
        ...
        e_point = self.get_eccentricity_point()
        lines = VGroup(*[Line(e_point, center + rotate_vector(radius * RIGHT, angle)) for angle in np.linspace(0, TAU, self.num_lines)])
...
class ShowFullStory(Scene):
    ...
    def construct(self):
        ...
        scene_names = ['ShowEmergingEllipse', 'ShowFullStory', 'FeynmanFameStart', 'TheMotionOfPlanets', 'FeynmanElementaryQuote', 'DrawingEllipse', 'ShowEllipseDefiningProperty', 'ProveEllipse', 'KeplersSecondLaw', 'AngularMomentumArgument', 'HistoryOfAngularMomentum', 'FeynmanRecountingNewton', 'IntroduceShapeOfVelocities', 'ShowEqualAngleSlices', 'PonderOverOffCenterDiagram', 'UseVelocityDiagramToDeduceCurve']
        ...
        self.play(images.set_width, FRAME_WIDTH - 1, images.center, run_time=3)
        self.wait()
        self.play(images.shift, -images[2].get_center(), images.scale, FRAME_WIDTH / images[2].get_width(), {'about_point': ORIGIN}, run_time=3)
        self.wait()
...
class FeynmanAndOrbitingPlannetOnEllipseDiagram(ShowEmergingEllipse):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(feynman.restore, MaintainPositionRelativeTo(feynman_rect, feynman), VFadeOut(feynman_rect), group.to_edge, RIGHT)
...
class FeynmanFame(Scene):
    ...
    def construct(self):
        books = VGroup(ImageMobject('Feynman_QED_cover'), ImageMobject('Surely_Youre_Joking_cover'), ImageMobject('Feynman_Lectures_cover'))
        for book in books:
            book.set_height(6)
        ...
        self.wait()
        self.play(self.get_book_intro(books[1]), self.get_book_outro(books[0]), LaggedStartMap(ApplyMethod, fd_parts, lambda m: (m.scale, 0), run_time=1))
        ...
        self.wait(2)
        self.play(self.get_book_intro(books[2]), self.get_book_outro(books[1]), LaggedStartMap(FadeOut, joke, run_time=1), ApplyMethod(feynman_smile.shift, FRAME_HEIGHT * DOWN, remover=True))
    ...
    def get_book_animation(self, book, initial_shift, animated_shift, opacity_func):
        ...
        return AnimationGroup(ApplyMethod(book.shift, animated_shift), UpdateFromAlphaFunc(rect, lambda r, a: r.move_to(book).set_fill(opacity=opacity_func(a)), remover=True))
    ...
    def get_feynman_diagram(self):
        ...
        arrows = VGroup(*[arrow.copy().rotate(angle).next_to(point, vect, buff=0) for angle, point, vect in [(-45 * DEGREES, x_min * RIGHT, UL), (-135 * DEGREES, x_min * RIGHT, DL), (-135 * DEGREES, x_max * RIGHT, UR), (-45 * DEGREES, x_max * RIGHT, DR)]])
        ...
        wave = FunctionGraph(lambda x: 0.2 * np.sin(2 * TAU * x), x_min=x_min, x_max=x_max)
        wave_label = OldTex('\\gamma')
        wave_label.next_to(wave, UP, SMALL_BUFF)
        labels.add(wave_label)
        squiggle = ParametricCurve(lambda t: np.array([t + 0.5 * np.sin(TAU * t), 0.5 * np.cos(TAU * t), 0]), t_min=0, t_max=4)
        squiggle.scale(0.25)
...
class TheMotionOfPlanets(Scene):
    CONFIG = {'camera_config': {'background_opacity': 1}, 'random_seed': 2}
    ...
    def setup_orbits(self):
        ...
        self.wait(10)
        self.play(VGroup(ellipses, sun).shift, 3 * RIGHT, FadeInFromDown(archivist_words), Animation(self.title))
    ...
    def get_planets_ellipses_and_orbits(self, sun):
        planets = VGroup(ImageMobject('mercury'), ImageMobject('venus'), ImageMobject('earth'), ImageMobject('mars'), ImageMobject('comet'))
        ...
        orbits = [Orbiting(planet, sun, circle, rate=0.25 * r ** (2 / 3)) for planet, circle, r in zip(planets, ellipses, orbit_radii)]
...
class AskAboutEllipses(TheMotionOfPlanets):
    CONFIG = {'camera_config': {'background_opacity': 1}, 'sun_height': 0.5, 'sun_center': ORIGIN, 'animate_sun': True, 'a': 3.5, 'b': 2.0, 'ellipse_color': WHITE, 'ellipse_stroke_width': 1, 'comet_height': 0.2}
    ...
    def add_force_labels(self):
        ...
        radius_measurement_kwargs = {'num_decimal_places': 3, 'color': BLUE}
        ...
        force_equation = OldTex('F = {GMm \\over (0.000)^2}', tex_to_color_map={'F': YELLOW, '0.000': BLACK})
        ...
        update_radius_in_denominator = ContinualChangingDecimal(radius_in_denominator, lambda a: radial_line.get_length(), position_update_func=lambda mob: mob.move_to(radius_in_denominator_ref, LEFT))
        ...
        self.play(FadeIn(force_equation), v_fade_in(radius_in_denominator), v_fade_in(radius_measurement))
    ...
    def set_up_differential_equations(self):
        ...
        in_vect = Matrix(np.array(['x(t)', 'y(t)', '\\dot{x}(t)', '\\dot{y}(t)']))
        equals = OldTex('=')
        out_vect = Matrix(np.array(['\\dot{x}(t)', '\\dot{y}(t)', '-x(t) / (x(t)^2 + y(t)^2)^{3/2}', '-y(t) / (x(t)^2 + y(t)^2)^{3/2}']), element_alignment_corner=ORIGIN)
    ...
    def get_ellipse(self):
        ...
        ellipse.set_stroke(self.ellipse_color, self.ellipse_stroke_width)
        ellipse.stretch(fdiv(b, a), dim=1)
        ellipse.move_to(self.sun.get_center() + c * LEFT)
        self.focus_points = [self.sun.get_center(), self.sun.get_center() + 2 * c * LEFT]
        return ellipse
    ...
    def get_force_arrow_and_update(self, comet, scale_factor=1):
        ...
        def update_force_arrow(arrow):
            ...
            target_length = scale_factor / radius
    ...
    def get_radial_line_and_update(self, comet):
        ...
        line_update = Mobject.add_updater(line, lambda l: l.put_start_and_end_on(self.sun.get_center(), comet.get_center()))
        return (line, line_update)

class FeynmanSaysItBest(TeacherStudentsScene):

    def construct(self):
        self.teacher_says('Feynman says \\\\ it best', added_anims=[self.change_students('hooray', 'happy', 'erm')])
        self.wait(3)
...
class FeynmanElementaryQuote(Scene):
    ...
    def construct(self):
        ...
        quote = OldTexText(*quote_parts, tex_to_color_map={'\\emph{elementary}': BLUE, 'elementary': BLUE, 'Elementary': BLUE, 'infinite': YELLOW, 'amount': YELLOW, 'of': YELLOW, 'intelligence': YELLOW, 'very': RED, 'little': RED}, alignment='')
        ...
        for word in quote:
            ...
            self.play(ShowWord(back_word), ShowWord(word))
            self.wait(0.005 * len(word) ** 1.5)
        self.wait()
        images = Group(ImageMobject('Calculus_Thumbnail'), ImageMobject('Fourier_Thumbnail'))
        for image in images:
            image.set_height(3)
        ...
        self.wait()
        self.play(FadeOutAndShiftDown(images[0]), FadeOutAndShiftDown(images[0].rect), FadeOutAndShiftDown(crosses[0]), FadeInFromDown(images[1]), FadeInFromDown(images[1].rect))
...
class LostLecturePicture(TODOStub):
    ...
    def construct(self):
        ...
        picture.fade(0.5)
        self.play(picture.to_corner, DR, {'buff': 0}, picture.shift, 1.5 * DOWN, path_arc=60 * DEGREES, run_time=20, rate_func=bezier([0, 0, 1, 1]))

class AskAboutInfiniteIntelligence(TeacherStudentsScene):

    def construct(self):
        self.student_says('Infinite intelligence?', target_mode='confused')
        self.play(self.change_students('horrified', 'confused', 'sad'), self.teacher.change, 'happy')
        self.wait()
        self.teacher_says("Stay focused, \\\\ go full screen, \\\\ and you'll be fine.", added_anims=[self.change_students(*['happy'] * 3)])
        self.wait()
...
class TableOfContents(Scene):
    ...
    def construct(self):
        items = VGroup(OldTexText('How the ellipse will arise'), OldTexText("Kepler's 2nd law"), OldTexText('The shape of velocities'))
        ...
        scale_factor = 1.2
        self.add(title)
        self.play(LaggedStartMap(FadeIn, items, run_time=1, lag_ratio=0.7))
        self.wait()
        for item in items:
...
class ShowEllipseDefiningProperty(Scene):
    CONFIG = {'camera_config': {'background_opacity': 1}, 'ellipse_color': BLUE, 'a': 4.0, 'b': 3.0, 'distance_labels_scale_factor': 1.0}
    ...
    def add_focal_lines(self):
        push_pins = VGroup(*[SVGMobject(file_name='push_pin', color=GREY_B, fill_opacity=0.8, height=0.5).move_to(point, DR).shift(0.05 * RIGHT) for point in self.get_foci()])
        dot = Dot()
        dot.scale(0.5)
        position_tracker = ValueTracker(0.125)
        dot_update = Mobject.add_updater(dot, lambda d: d.move_to(self.ellipse.point_from_proportion(position_tracker.get_value() % 1)))
        ...
        self.add(dot_update)
        self.play(LaggedStartMap(FadeInFrom, push_pins, lambda m: (m, 2 * UP + LEFT), run_time=1, lag_ratio=0.75))
    ...
    def add_distance_labels(self):
        ...
        sum_expression, numbers, number_updates = self.get_sum_expression_and_update(lines, colors, lambda mob: mob.to_corner(UR))
        ...
        self.play(VFadeIn(distance_labels), FadeOut(sum_expression_fading_rect))
    ...
    def label_foci(self):
        ...
        connecting_lines = VGroup(*[Arrow(foci_word.get_edge_center(-edge), focus_word.get_edge_center(edge), buff=MED_SMALL_BUFF, stroke_width=2) for focus_word, edge in zip(focus_words, [LEFT, RIGHT])])
        ...
        self.wait(2)
        self.play(ReplacementTransform(focus_words.copy(), foci_word))
    ...
    def get_distance_labels_and_update(self, lines, colors):
        distance_labels = VGroup(DecimalNumber(0), DecimalNumber(0))
...
class GeometryProofLand(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, word_outlines, run_time=3, rate_func=there_and_back), Animation(circles))
        self.wait()
...
class ProveEllipse(ShowEmergingEllipse, ShowEllipseDefiningProperty):
    CONFIG = {'eccentricity_vector': 1.5 * RIGHT, 'ellipse_color': PINK, 'distance_labels_scale_factor': 0.7}
    ...
    def setup_ellipse(self):
        ...
        self.play(LaggedStartMap(MoveToTarget, lines), Animation(ep_dot))
        self.play(ShowCreation(ellipse))
        self.wait()
    ...
    def hypothesize_foci(self):
        ...
        self.play(FadeInFromDown(labels[1]), GrowArrow(arrows[1]))
        ...
        self.play(FadeInFromDown(labels[0]), GrowArrow(arrows[0]))
        self.wait()
    ...
    def setup_and_show_focal_sum(self):
        ...
        sum_expression, numbers, number_updates = self.get_sum_expression_and_update(focal_lines, colors, lambda mob: mob.to_edge(RIGHT).shift(UP))
        to_add = self.focal_sum_things_to_add = [focal_lines_update_animation, distance_labels_update_animation, sum_expression] + list(number_updates)
        self.play(ShowCreation(focal_lines), Write(distance_labels), FadeIn(sum_expression), Write(numbers), run_time=1)
        self.wait()
        self.add(*to_add)
        points = [ellipse.get_top(), circle.point_from_proportion(0.2), ellipse.point_from_proportion(0.2), ellipse.point_from_proportion(0.4)]
        for point in points:
            self.play(focal_sum_point.move_to, point)
            self.wait()
        self.remove(*to_add)
        self.play(*list(map(FadeOut, [focal_lines, distance_labels, sum_expression, numbers])))
        self.set_variables_as_attrs(focal_lines, focal_lines_update_animation, focal_sum_point, distance_labels, distance_labels_update_animation, sum_expression, numbers, number_updates)
    ...
    def limit_to_just_one_line(self):
        ...
        self.play(FadeOut(to_fade), ghost_line.set_stroke, YELLOW, 3, line.set_stroke, WHITE, 3, ReplacementTransform(ep_dot.copy(), P_dot))
    ...
    def look_at_perpendicular_bisector(self):
        ...
        def distance_label_shift_update(label):
            ...
            if line.get_end()[0] > line.get_start()[0]:
                ...
                label.shift(-2 * vect)
        distance_label_shift_update_animation = Mobject.add_updater(self.distance_labels[0], distance_label_shift_update)
        ...
        QP_line_update = Mobject.add_updater(QP_line, lambda l: l.put_start_and_end_on(Q_dot.get_center(), P_dot.get_center()))
        QE_line = Line(LEFT, RIGHT)
        QE_line.set_stroke(YELLOW, 3)
        QE_line_update = Mobject.add_updater(QE_line, lambda l: l.put_start_and_end_on(Q_dot.get_center(), self.get_eccentricity_point()))
        triangles = VGroup(*[Polygon(Q_dot.get_center(), line.get_center(), end_point, fill_opacity=1) for end_point in [P_dot.get_center(), self.get_eccentricity_point()]])
        ...
        def distance_label_rotate_update(label):
            ...
            return label
        distance_label_rotate_update_animation = Mobject.add_updater(self.distance_labels[0], distance_label_rotate_update)
        radial_line = DashedLine(ORIGIN, 3 * RIGHT)
        radial_line_update = UpdateFromFunc(radial_line, lambda l: l.put_start_and_end_on(circle.get_center(), P_dot.get_center()))
        ...
        Q_dot.restore()
        ghost_line_update_animation = UpdateFromFunc(ghost_line, lambda l: l.put_start_and_end_on(self.get_eccentricity_point(), P_dot.get_center()))
        ...
        P_dot_movement_updates = [radial_line_update, keep_Q_dot_at_intersection, MaintainPositionRelativeTo(P_label, P_dot), ghost_line_update_animation, perp_bisector_update_animation, elbow_update_animation]
        ...
        self.wait()
        self.play(FadeOut(label), FadeIn(self.focal_lines), FadeIn(self.distance_labels), FadeIn(self.sum_expression), FadeIn(self.numbers), ellipse.set_stroke, {'width': 0.5})
        self.add(*self.focal_sum_things_to_add)
        self.play(FadeInFromDown(Q_label), GrowFromCenter(Q_dot))
        self.wait()
        self.add_foreground_mobjects(Q_dot)
        self.add(Q_label_animation)
        self.play(Q_dot.move_to, line.point_from_proportion(0.05), rate_func=there_and_back, run_time=4)
        self.wait()
        self.play(FadeIn(triangles[0]), ShowCreation(QP_line), Animation(elbow))
        self.add(QP_line_update)
        for i in range(3):
            self.play(FadeIn(triangles[(i + 1) % 2]), FadeOut(triangles[i % 2]), Animation(self.distance_labels), Animation(elbow))
        ...
        self.play(ApplyFunction(distance_label_rotate_update, distance_label_copy, path_arc=angle), Rotate(QE_line, angle, about_point=Q_point))
        ...
        self.wait()
        self.play(Q_dot.move_to, line.point_from_proportion(0), run_time=4, rate_func=there_and_back)
        self.play(ShowCreation(radial_line))
        self.wait()
        self.play(ApplyFunction(put_dot_at_intersection, Q_dot), run_time=3)
        self.wait()
        self.play(Rotating(P_dot, about_point=circle.get_center(), rate_func=bezier([0, 0, 1, 1]), run_time=10), ellipse.restore, *P_dot_movement_updates)
        self.wait()
        self.play(ShowCreation(sum_rect), Write(tangency_comment))
        ...
        self.wait()
        self.remove(*self.focal_sum_things_to_add)
        self.play(*list(map(FadeOut, [radial_line, QP_line, P_dot, P_label, Q_dot, Q_label, elbow, self.distance_labels, self.numbers, self.sum_expression, sum_rect, tangency_comment])))
        self.wait()
        lines.remove(line)
        ...
        for line in lines:
            line.generate_target()
            line.target.rotate(90 * DEGREES)
        self.play(LaggedStartMap(FadeIn, ghost_lines), LaggedStartMap(FadeIn, lines))
...
class KeplersSecondLaw(AskAboutEllipses):
    CONFIG = {'sun_center': 4 * RIGHT + 0.75 * DOWN, 'animate_sun': True, 'a': 5.0, 'b': 3.0, 'ellipse_stroke_width': 2, 'area_color': COBALT, 'area_opacity': 0.75, 'arc_color': YELLOW, 'arc_stroke_width': 3, 'n_sample_sweeps': 5, 'fade_sample_areas': True}
    ...
    def contrast_close_to_far(self):
        ...
        self.play(ShowCreation(radius), Write(radius_words))
        ...
        self.play(ShowCreation(long_radius), Write(long_radius_words))
    ...
    def show_area_sweep(self, time=1.0):
        ...
        area_update = UpdateFromFunc(area, lambda a: Transform(a, self.get_area(start_prop, orbit.proportion)).update(1))
    ...
    def get_area(self, prop1, prop2):
        ...
        result.set_fill(self.area_color, self.area_opacity)
        return result
    ...
    def get_arc(self, prop1, prop2):
        ...
        if prop2 > prop1:
            ...
            arc, arc_extension = [VMobject().pointwise_become_partial(ellipse, p1, p2) for p1, p2 in [(prop1, 1.0), (0.0, prop2)]]
            arc.append_vectorized_mobject(arc_extension)
        else:
            arc = VectorizedPoint(ellipse.point_from_proportion(prop1))
        arc.set_stroke(self.arc_color, self.arc_stroke_width)
        return arc
    ...
    def get_radius_words(self, radius, adjective):
        radius_words = OldTexText('%s radius' % adjective)
...
class NonEllipticalKeplersLaw(KeplersSecondLaw):
    CONFIG = {'animate_sun': True, 'sun_center': 2 * RIGHT, 'n_sample_sweeps': 10}
    ...
    def add_orbit(self):
        ...
        alt_arrow_update = Mobject.add_updater(arrow, lambda a: a.scale(1.0 / a.get_length(), about_point=a.get_start()))
        self.add(arrow_update, alt_arrow_update)
        self.add_foreground_mobjects(comet, arrow)
        self.ellipse = orbit_shape

    def get_ellipse(self):
        orbit_shape = ParametricCurve(lambda t: (1 + 0.2 * np.sin(5 * TAU * t)) * np.array([np.cos(TAU * t), np.sin(TAU * t), 0]))
...
class AngularMomentumArgument(KeplersSecondLaw):
    CONFIG = {'animate_sun': False, 'sun_center': 4 * RIGHT + DOWN, 'comet_start_point': 4 * LEFT, 'comet_end_point': 5 * LEFT + DOWN, 'comet_height': 0.3}
    ...
    def show_small_sweep(self):
        ...
        triangle = Polygon(sun_center, comet_start, comet_end, fill_opacity=1, fill_color=COBALT, stroke_width=0)
        triangle.save_state()
        ...
        small_time_label = OldTexText('Small', 'time', '$\\Delta t$')
        small_time_label.to_edge(UP)
        small = small_time_label.get_part_by_tex('Small')
        small_rect = SurroundingRectangle(small)
        self.add_foreground_mobjects(comet)
        self.play(ShowCreation(triangle, rate_func=lambda t: interpolate(1.0 / 3, 2.0 / 3, t)), MaintainPositionRelativeTo(velocity_vector, comet), MaintainPositionRelativeTo(velocity_vector_label, velocity_vector), ApplyMethod(comet.move_to, comet_end, rate_func=linear), run_time=2)
        self.play(Write(small_time_label), run_time=2)
        self.wait()
        self.play(Transform(triangle, alt_triangle), ShowCreation(small_rect), small.set_color, YELLOW)
        self.wait()
        self.play(Restore(triangle), FadeOut(small_rect), small.set_color, WHITE)
        self.wait()
    ...
    def show_sweep_dimensions(self):
        ...
        area_label = OldTex('\\text{Area}', '=', '\\frac{1}{2}', '\\text{Base}', '\\times', '\\text{Height}')
        area_label.set_color_by_tex_to_color_map({'Base': PINK, 'Height': YELLOW})
        area_label.to_edge(UP)
        equals = area_label.get_part_by_tex('=')
        area_expression = OldTex('=', '\\frac{1}{2}', 'R', '\\times', '\\vec{\\textbf{v}}_\\perp', '\\Delta t')
        area_expression.set_color_by_tex_to_color_map({'R': PINK, 'textbf{v}': YELLOW})
        area_expression.next_to(area_label, DOWN)
        area_expression.align_to(equals, LEFT)
        self.R_v_perp = VGroup(*area_expression[-4:-1])
        self.R_v_perp_rect = SurroundingRectangle(self.R_v_perp, stroke_color=BLUE, fill_color=BLACK, fill_opacity=1)
        ...
        self.play(FadeInFromDown(area_label), self.small_time_label.scale, 0.5, self.small_time_label.to_corner, UL)
        self.wait()
        self.play(ShowCreation(base), Write(radius_label))
        self.wait()
        self.play(ShowCreation(height))
        self.wait()
        self.play(GrowArrow(v_perp), Write(v_perp_label, run_time=1))
        self.wait()
        self.play(MoveToTarget(v_perp_delta_t))
        self.wait()
        self.play(*[ReplacementTransform(area_label.get_part_by_tex(tex).copy(), area_expression.get_part_by_tex(tex)) for tex in ('=', '\\frac{1}{2}', '\\times')])
    ...
    def show_conservation_of_angular_momentum(self):
        ...
        stays_constant_arrow = Arrow(stays_constant.get_left(), R_v_perp_rect.get_bottom(), color=R_v_perp_rect.get_color())
        ...
        self.wait()
        self.play(DrawBorderThenFill(R_v_perp_rect), Animation(R_v_perp), Write(stays_constant, run_time=1), GrowArrow(stays_constant_arrow))
        self.wait()
        foreground = VGroup(*self.get_mobjects())
        self.play(LaggedStartMap(GrowArrow, vector_field), Animation(foreground))
        for x in range(3):
            self.play(LaggedStartMap(ApplyFunction, vector_field, lambda mob: (lambda m: m.scale(1.1).set_fill(opacity=1), mob), rate_func=there_and_back, run_time=1), Animation(foreground))
        self.play(FadeIn(parens), MoveToTarget(R_v_perp))
        self.play(comet.scale, 2, comet.next_to, parens, RIGHT, {'buff': SMALL_BUFF})
        self.wait()
        self.play(FadeOut(parens), R_v_perp.restore, comet.restore)
        self.wait(3)

class KeplersSecondLawImage(KeplersSecondLaw):
    CONFIG = {'animate_sun': False, 'n_sample_sweeps': 8, 'fade_sample_areas': False}
...
class HistoryOfAngularMomentum(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        k2l.add(k2l_image)
        angular_momentum_formula = OldTex('R', '\\times', 'm', '\\vec{\\textbf{v}}_\\perp')
        angular_momentum_formula.set_color_by_tex_to_color_map({'R': PINK, 'v': YELLOW})
        angular_momentum_formula.next_to(am, UP)
        am.add(angular_momentum_formula)
        self.play(self.teacher.change, 'raise_right_hand', FadeInFromDown(group), self.change_students(*3 * ['pondering']))
        self.wait()
        self.play(am.next_to, arrow, RIGHT, {'index_of_submobject_to_align': 0}, k2l.next_to, arrow, LEFT, {'index_of_submobject_to_align': 0}, path_arc=90 * DEGREES, run_time=2)
        self.wait(3)
...
class FeynmanRecountingNewton(Scene):
    ...
    def construct(self):
        ...
        for image in images:
            image.rect = SurroundingRectangle(image, color=WHITE, buff=0)
        self.play(FadeInFromDown(feynman_teaching, run_time=2))
        self.wait()
        self.play(FadeInFromDown(newton), FadeInFromDown(newton.rect))
        self.wait()
...
class IntroduceShapeOfVelocities(AskAboutEllipses, MovingCameraScene):
    CONFIG = {'animate_sun': True, 'sun_center': 2 * RIGHT, 'a': 4.0, 'b': 3.5, 'num_vectors': 25}
    ...
    def reference_inverse_square_law(self):
        ...
        force_arrow, force_arrow_update = self.get_force_arrow_and_update(self.comet, scale_factor=3)
        ...
        self.play(Restore(ellipse), Write(force_equation), UpdateFromAlphaFunc(force_arrow, lambda m, a: m.set_fill(opacity=a)), UpdateFromAlphaFunc(radial_line, lambda m, a: m.set_stroke(width=a)))
        self.wait(10)
    ...
    def show_velocity_vectors(self):
        ...
        self.play(LaggedStartMap(ShowCreation, vectors, lag_ratio=0.2, run_time=3))
        self.wait(5)
    ...
    def collect_velocity_vectors(self):
        ...
        for vector in vectors:
            vector.target = Arrow(root_point, root_point + vector.get_vector(), buff=0, rectangular_stem_width=0.025, tip_length=0.2, color=vector.get_color())
        ...
        rect = SurroundingRectangle(VGroup(circle, velocity_space), buff=MED_LARGE_BUFF, color=WHITE)
        self.play(ApplyMethod(frame.shift, frame_shift, run_time=2), LaggedStartMap(MoveToTarget, vectors, run_time=4), FadeInFromDown(velocity_space), FadeInFromDown(rect))
        self.wait(2)
    ...
    def get_velocity_vector(self, alpha, d_alpha=0.01, scalar=3.0):
        ...
        vector.scale(scalar / (norm(curr_v_perp) * norm(radius_vector)), about_point=vector.get_start())
        vector.set_color(interpolate_color(BLUE, RED, inverse_interpolate(min_length, max_length, vector.get_length())))
    ...
    def get_velocity_vector_and_update(self):
        ...
        def update_moving_vector(vector):
            new_vector = self.get_velocity_vector(self.orbit.proportion)
...
class AskWhy(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('Um...why?', target_mode='confused', index=2, bubble_config={'direction': LEFT})
        self.play(self.teacher.change, 'happy', self.change_students('raise_left_hand', 'sassy', 'confused'))
        self.wait(5)
...
class ShowEqualAngleSlices(IntroduceShapeOfVelocities):
    CONFIG = {'animate_sun': True, 'theta': 30 * DEGREES}
    ...
    def show_equal_angle_slices(self):
        ...
        def get_cos_angle_diff(v1, v2):
            return np.dot(v1 / get_norm(v1), v2 / get_norm(v2))
        lines = VGroup()
        angle_arcs = VGroup()
        thetas = VGroup()
        ...
        for angle in angles:
            ...
            line.prop = prop
            lines.add(line)
            angle_arc = AnnularSector(angle=self.theta, inner_radius=1, outer_radius=1.05)
        ...
        kwargs = {'run_time': 6, 'lag_ratio': 0.2, 'rate_func': there_and_back}
        ...
        self.play(*[LaggedStartMap(ApplyMethod, fader, lambda m: (m.set_fill, {'opacity': 1}), **kwargs) for fader in faders] + [Animation(lines)])
        self.wait()
        self.lines = lines
        self.wedges = wedges
        self.arcs = arcs
    ...
    def ask_about_time_per_slice(self):
        ...
        arrow1 = Arrow(words1.get_bottom(), wedge1.get_center() + wedge1.get_height() * DOWN / 2, color=WHITE)
        arrow2 = Arrow(words2.get_right(), wedge2.get_center() + wedge2.get_height() * UL / 4, color=WHITE)
        foreground = VGroup(self.ellipse, self.angle_arcs, self.lines, comet)
        self.play(Write(words1), wedge1.set_fill, {'opacity': 1}, GrowArrow(arrow1), Animation(foreground), frame.scale, 1.2)
        self.play(MoveAlongPath(comet, arc1, rate_func=linear))
        self.play(Write(words2), wedge2.set_fill, {'opacity': 1}, Write(arrow2), Animation(foreground))
    ...
    def areas_are_proportional_to_radius_squared(self):
        ...
        self.play(Write(expression[:3]), frame.shift, 0.5 * UP, FadeInFromDown(prop_to_words), GrowFromCenter(prop_to_brace))
        self.wait(2)
        ...
        self.play(scaling_group.scale, 0.5, {'about_point': sun_center}, Animation(self.area_question_arrows), Animation(self.comet), rate_func=there_and_back, run_time=4)
        self.wait()
    ...
    def show_inverse_square_law(self):
        ...
        acceleration_expression = OldTex('a', '=', '{\\Delta v', '\\over', '\\Delta t}', '\\propto', '{1 \\over (\\text{Radius})^2}')
        ...
        force_vector, force_vector_update = self.get_force_arrow_and_update(comet, scale_factor=3)
        moving_vector, moving_vector_animation = self.get_velocity_vector_and_update()
        self.play(FadeOut(self.area_questions), FadeOut(self.area_question_arrows), FadeInFromDown(force_expression), frame.shift, 2 * LEFT)
        self.remove(*self.area_questions)
        self.play(randy.change, 'confused', force_expression, VFadeIn(randy))
        self.wait(2)
        self.play(randy.change, 'pondering', force_expression[0], ShowPassingFlashAround(force_expression[:2]))
        self.play(Blink(randy))
        self.play(FadeInFromDown(acceleration_expression), randy.change, 'hooray', force_expression, randy.shift, 2 * DOWN)
        self.wait(2)
        self.play(Blink(randy))
        ...
        self.add_foreground_mobjects(comet)
        self.wait(2)
        self.play(randy.change, 'pondering', force_expression, randy.shift, 2 * DOWN, FadeInFromDown(delta_v_expression))
        self.play(Blink(randy))
        self.wait(2)
        self.play(delta_t_numerator.scale, 1.5, {'about_edge': DOWN}, delta_t_numerator.set_color, YELLOW)
        self.play(ShowCreationThenFadeAround(prop_exp[:-2]))
        self.play(delta_t_numerator.fade, 1, MoveToTarget(moving_R_squared), randy.change, 'happy', delta_v_expression)
        ...
        self.moving_vector = moving_vector
        self.force_expressions = VGroup(force_expression, acceleration_expression, delta_v_expression)
    ...
    def directly_compare_velocity_vectors(self):
        ...
        for vect1, vect2 in adjacent_pairs(vectors):
            diff_vect = Arrow(vect1.target.get_end(), vect2.target.get_end(), buff=0, color=YELLOW, rectangular_stem_width=0.025, tip_length=0.15)
            diff_vect.add_to_back(diff_vect.copy().set_stroke(BLACK, 2))
            difference_vectors.add(diff_vect)
            line = Line(diff_vect.get_start(), diff_vect.get_start() + 2 * diff_vect.get_vector())
        ...
        self.play(MoveToTarget(v1), GrowFromCenter(root_dot), expressions.scale, 0.5, {'about_edge': UL})
        self.wait()
        ...
        self.add(self.orbit)
        self.wait()
        self.play(LaggedStartMap(ShowCreation, external_angle_lines), LaggedStartMap(ShowCreation, external_angle_arcs), Animation(difference_vectors))
...
class ShowEqualAngleSlices15DegreeSlices(ShowEqualAngleSlices):
    CONFIG = {'animate_sun': True, 'theta': 15 * DEGREES}

class ShowEqualAngleSlices5DegreeSlices(ShowEqualAngleSlices):
    CONFIG = {'animate_sun': True, 'theta': 5 * DEGREES}

class IKnowThisIsTricky(TeacherStudentsScene):

    def construct(self):
        self.teacher_says('All you need is \\\\ infinite intelligence', bubble_config={'width': 4, 'height': 3}, added_anims=[self.change_students(*3 * ['horrified'], look_at=self.screen)])
        self.look_at(self.screen)
        self.wait(3)
...
class PonderOverOffCenterDiagram(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(GrowFromCenter, velocity_diagram), randy.change, 'pondering', morty.change, 'confused')
        self.wait(2)
        self.play(ShowCreation(bubble))
        self.wait(2)
        self.play(FadeOut(bubble), randy.change, 'confused', morty.change, 'pondering', ShowCreation(rect))
    ...
    def get_velocity_diagram(self):
        ...
        root_point = interpolate(circle.get_center(), circle.get_bottom(), 0.5)
        dot = Dot(root_point)
        vectors = VGroup()
        for a in np.arange(0, 1, 1.0 / 24):
            end_point = circle.point_from_proportion(a)
            vector = Arrow(root_point, end_point, buff=0)
            vector.set_color(interpolate_color(BLUE, RED, inverse_interpolate(1, 3, vector.get_length())))
            vector.add_to_back(vector.copy().set_stroke(BLACK, 5))
            vectors.add(vector)
...
class UseVelocityDiagramToDeduceCurve(ShowEqualAngleSlices):
    CONFIG = {'animate_sun': True, 'theta': 15 * DEGREES, 'index': 6}
    ...
    def setup_velocity_diagram(self):
        ...
        self.play(LaggedStartMap(MoveToTarget, vectors, lag_ratio=1, run_time=2), GrowFromCenter(root_dot), FadeInFromDown(velocities_word))
    ...
    def show_theta_degrees(self):
        ...
        circle_wedge_start = self.get_wedge(Line(circle.get_top(), circle.get_top()), circle.get_center()).match_style(circle_wedge)
        circle_center_dot = Dot(circle.get_center())
        self.play(FocusOn(comet))
        self.play(ReplacementTransform(ellipse_wedge_start, ellipse_wedge, path_arc=angle), FadeIn(ellipse_arc), ShowCreation(ellipse_angle_arc), Write(ellipse_theta), ReplacementTransform(lines[0].copy(), thick_line, path_arc=angle), MoveAlongPath(comet, ellipse_arc), run_time=2)
        self.wait()
        self.play(ReplacementTransform(circle_wedge_start, circle_wedge, path_arc=angle), ShowCreation(circle_arc), Write(circle_theta), Restore(vector, path_arc=angle), GrowFromCenter(circle_center_dot), FadeIn(horizontal), run_time=2)
        self.wait()
        self.set_variables_as_attrs(index, ellipse_wedge, ellipse_arc, ellipse_angle_arc, ellipse_theta, thick_line, horizontal, circle_wedge, circle_arc, circle_theta, circle_center_dot, highlighted_vector=vector)
    ...
    def match_velocity_vector_to_tangency(self):
        ...
        self.play(ReplacementTransform(vector.copy(), original_vector, run_time=2), Animation(comet))
        self.wait()
        self.play(ShowCreation(tangent_line), Animation(original_vector), Animation(comet))
        self.wait()
        self.set_variables_as_attrs(example_tangent_line=tangent_line, example_tangent_vector=original_vector)
    ...
    def replace_vectors_with_lines(self):
        ...
        self.play(LaggedStartMap(MoveToTarget, vectors), highlighted_vector.scale, 0, {'about_point': root_dot.get_center()}, Animation(highlighted_vector), Animation(self.circle_wedge), Animation(self.circle_arc), Animation(self.circle), Animation(self.circle_center_dot))
        self.remove(vectors, highlighted_vector)
        self.play(LaggedStartMap(ShowCreation, lines), ShowCreation(highlighted_line), Animation(highlighted_vector))
        self.wait()
        self.play(ReplacementTransform(lines.copy(), tangent_lines, run_time=3))
        self.wait()
    ...
    def not_that_velocity_vector_is_theta(self):
        ...
        self.play(ShowCreation(arc), FadeInFromDown(theta_q))
        self.wait()
        self.play(ShowCreation(cross))
        self.wait()
        self.play(*list(map(FadeOut, [v_line, arc, theta_q, cross])))
        self.wait()
        self.play(ReplacementTransform(self.ellipse_theta.copy(), self.circle_theta), ReplacementTransform(self.ellipse_angle_arc.copy(), self.circle_arc), run_time=2)
        self.wait()
        self.play(ReplacementTransform(self.circle.copy(), self.circle_center_dot))
        self.wait()
    ...
    def ask_about_curve(self):
        ...
        self.play(morty.change, 'confused', ellipse, ShowCreationThenDestruction(ellipse.copy().set_stroke(BLUE, 3), run_time=2))
        self.play(Blink(morty), ApplyWave(ellipse))
    ...
    def show_90_degree_rotation(self):
        circle = self.circle
        circle_setup = VGroup(circle, self.eccentric_lines, self.circle_wedge, self.circle_arc, self.highlighted_line, self.circle_center_dot, self.root_dot, self.circle_theta)
        ...
        foci = [self.root_dot.get_center(), circle.get_center()]
        ...
        self.add(ghost_lines)
        self.play(LaggedStartMap(MoveToTarget, lines, lag_ratio=0.1, run_time=8), MoveToTarget(highlighted_line), path_arc=-angle)
        self.play(ShowCreation(little_ellipse))
        self.wait(2)
        self.play(little_ellipse.replace, self.ellipse, run_time=4, rate_func=there_and_back_with_pause)
        self.wait(2)
        self.play(*[Restore(mob, path_arc=angle, run_time=4, rate_func=there_and_back_with_pause) for mob in list(lines) + [highlighted_line]] + [Animation(little_ellipse)])
    ...
    def show_geometry_of_rotated_diagram(self):
        ...
        tangency_point = line_intersection(perp_line.get_start_and_end(), c_line.get_start_and_end())
        ...
        self.play(self.ghost_lines.set_stroke, {'width': 0.5}, self.eccentric_lines.set_stroke, {'width': 0.5}, *list(map(WiggleOutThenIn, [e_line, c_line])))
        for x in range(3):
            self.play(indicate(e_line), indicate(c_line))
        ...
        self.wait(2)
        self.play(perp_line.move_to, velocity_vector, run_time=4, rate_func=there_and_back_with_pause)
        self.wait()
        self.play(little_ellipse.replace, ellipse, run_time=4, rate_func=there_and_back_with_pause)
        self.wait()
...
class ShowSunVectorField(Scene):
    ...
    def construct(self):
        ...
        for x in range(3):
            self.play(LaggedStartMap(MoveToTarget, vector_field, rate_func=there_and_back, lag_ratio=0.5))
...
class PatYourselfOnTheBack(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait(3)
        self.play(RemovePiCreatureBubble(self.teacher, target_mode='raise_right_hand'), self.change_students(*3 * ['pondering'], look_at=self.screen))
        self.look_at(UP)
        self.wait(8)
        ...
        self.wait()
        self.play(RemovePiCreatureBubble(self.teacher, target_mode='happy'), FadeInFromDown(chess))
        self.wait(2)
