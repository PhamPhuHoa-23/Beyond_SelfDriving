...
class AnalyzeZSquared(ComplexTransformationScene, ZoomedScene):
    CONFIG = {'plane_config': {'line_frequency': 0.1}, 'num_anchors_to_add_per_line': 20, 'complex_homotopy': lambda z, t: z ** (1.0 + t), 'zoom_factor': 0.05}
    ...
    def preview_some_numbers(self):
        dots = VGroup(*[Dot().move_to(self.background.number_to_point(z)) for z in [1, 2, complex(0, 1), -1, complex(2, 0.5), complex(-1, -1), complex(3, 0.5)]])
        ...
        for dot in dots:
            ...
            if abs(z - 1) < 0.01:
                arrow = Arc(start_angle=-90 * DEGREES + d_angle, angle=360 * DEGREES - 2 * d_angle, radius=0.25)
                arrow.add_tip(tip_length=0.15)
                arrow.pointwise_become_partial(arrow, 0, 0.9)
                arrow.next_to(dot, UP, buff=0)
            else:
                arrow = Arrow(point, out_point, path_arc=path_arc, buff=SMALL_BUFF)
            ...
            dot.scale(3)
            dot.fade(1)
            dot_group.anim = Succession(ApplyMethod(dot.restore), AnimationGroup(ShowCreation(arrow), ReplacementTransform(dot_copy, out_dot, path_arc=path_arc)))
    ...
    def zoom_in_to_one_plus_half_i(self):
        ...
        self.apply_complex_homotopy(self.complex_homotopy, added_anims=[Write(words), GrowArrow(arrow), MaintainPositionRelativeTo(frame, point_mob)])
        self.wait(2)
    ...
    def get_plane(self):
        top_plane = NumberPlane(y_radius=FRAME_HEIGHT / 2, x_line_frequency=0.1, y_line_frequency=0.1)
