...
class CrossingOneMillion(TeacherStudentsScene):
    ...
    def increment_count(self):
        ...
        self.play(ChangeDecimalToValue(number, 10 ** 6, position_update_func=lambda m: m.move_to(UP, LEFT), rate_func=bezier([0, 0, 0, 1, 1, 1]), run_time=5), LaggedStartMap(ApplyMethod, self.get_pi_creatures(), lambda m: (m.change, 'hooray', number), rate_func=squish_rate_func(smooth, 0, 0.5), run_time=4))
        self.wait()

    def comment_on_real_milestone(self):
        number = self.number
        remainder = Integer(2 ** 20 - 10 ** 6)
        words = OldTexText('Just', '{:,}'.format(remainder.number), 'to go \\\\ before the real milestone')
        self.student_says(words, added_anims=[ApplyMethod(self.teacher.change, 'hesitant'), self.change_students('sassy', 'speaking', 'happy'), number.scale, 0.5, number.center, number.to_edge, UP])
        self.wait()
        self.remove(*self.confetti_spirils)
        remainder.replace(words[1])
        words.submobjects[1] = remainder
        self.play(ChangeDecimalToValue(number, 2 ** 20, run_time=3), ChangeDecimalToValue(remainder, 0.1, run_time=3), self.teacher.change, 'pondering', number, self.change_students(*['pondering'] * 3, look_at=number))
        self.play(FadeOut(self.students[1].bubble), FadeOut(self.students[1].bubble.content))
        self.wait(2)

    def reflect(self):
        bubble = ThoughtBubble(direction=RIGHT, height=4, width=7)
        ...
        self.play(ShowCreation(bubble, rate_func=squish_rate_func(smooth, 0, 0.3)), Write(q_marks), GrowArrow(arrow), MoveToTarget(number), run_time=3)
        self.wait()
...
class ShareWithFriends(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.pi_creature_says(randy, 'Wanna see why \\\\' + '$1 - \\frac{1}{3} + \\frac{1}{5}' + '- \\frac{1}{7} + \\cdots = \\frac{\\pi}{4}$?', target_mode='tease', added_anims=[morty.look, UP])
...
class AllFeaturedCreators(MortyPiCreatureScene):
    ...
    def construct(self):
        ...
        creators = VGroup(*list(map(TexText, ['Think Twice', 'LeiosOS', 'Welch Labs', 'Infinity plus one'])))
        ...
        think_words = VGroup(*list(map(TexText, ['(thinks visually)', '(thinks in terms of communities)', '(thinks in terms of series)', '(thinks playfully)'])))
        ...
        for rect, word, mode in zip(rects, think_words, modes):
            self.play(self.get_rect_removal(rect), morty.change, mode)
            self.wait()
