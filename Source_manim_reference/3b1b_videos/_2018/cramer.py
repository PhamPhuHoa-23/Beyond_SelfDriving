...
class LinearSystem(VGroup):
    CONFIG = {'matrix_config': {'element_to_mobject': Integer}, 'dimensions': 3, 'min_int': -9, 'max_int': 10, 'height': 4}
...
class AltOpeningQuote(OpeningQuote):
    ...
    def construct(self):
        kw = {'tex_to_color_map': {'Jerry': YELLOW, 'Kramer': BLUE}, 'arg_separator': '', 'alignment': ''}
        parts = VGroup(OldTexText("Jerry: Ah, you're crazy!", **kw), OldTexText('Kramer:', '{} Am I? Or am I so sane that\\\\', 'you just blew your mind?', **kw), OldTexText("Jerry: It's impossible!", **kw), OldTexText('Kramer:', '{} Is it?! Or is it so possible\\\\', 'your head is spinning like a top?', **kw))
...
class CramerOpeningQuote(OpeningQuote):
    CONFIG = {'quote': ['Computers are useless. They \\\\ can only give you answers.'], 'author': 'Pablo Picasso'}
...
class LeaveItToComputers(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for i in range(3):
            numer_matrix = get_cramer_matrix(system.matrix_mobject, system.output_vect_mob, index=i)
            color = YELLOW
            VGroup(*numer_matrix.mob_matrix[:, i]).set_color(color)
            VGroup(*numer_matrix.mob_matrix[:, i]).set_stroke(color, 1)
            numer = VGroup(get_det_text(numer_matrix, initial_scale_factor=3), numer_matrix)
            numer.to_corner(UP)
            denom_matrix_mobject = system.matrix_mobject.deepcopy()
            denom = VGroup(get_det_text(denom_matrix_mobject, initial_scale_factor=3), denom_matrix_mobject)
        ...
        self.play(Write(system), self.teacher.change, 'raise_right_hand', self.change_students('pondering', 'thinking', 'hooray'))
        self.wait(2)
        self.play(PiCreatureSays(self.teacher, 'Let the computer \\\\ handle it', target_mode='shruggie'), MoveToTarget(system, path_arc=90 * DEGREES), self.change_students(*['confused'] * 3))
        self.wait(3)
        cg = cramer_groups[0]
        scale_factor = 1.5
        ...
        self.play(RemovePiCreatureBubble(self.teacher, target_mode='raise_right_hand'), ShowCreation(line), ReplacementTransform(system.matrix_mobject.copy(), denom[1]), Write(denom[0]), FadeIn(cg.equals), ReplacementTransform(x_copy, cg.variable))
        ...
        self.look_at(system)
        self.play(ReplacementTransform(system.output_vect_mob.elements.copy(), VGroup(*numer[1].mob_matrix[:, 0]), path_arc=90 * DEGREES), self.teacher.change, 'happy')
        self.remove(denom_mover)
        self.add(cg)
        self.play_all_student_changes('sassy')
        self.wait(2)
        self.play(cramer_groups[0].scale, 1 / scale_factor, cramer_groups[0].next_to, cramer_groups[1], LEFT, MED_LARGE_BUFF, FadeIn(cramer_groups[1]), FadeOut(system), self.change_students(*3 * ['horrified'], look_at=UP))
        self.wait()
        self.play(FadeIn(cramer_groups[2]), cramer_groups[:2].next_to, cramer_groups[2], LEFT, MED_LARGE_BUFF, self.change_students(*3 * ['horrified'], look_at=UP))
        self.wait()
        brace = Brace(cramer_groups, UP)
        rule_text = brace.get_text("``Cramer's rule''")
        self.play(GrowFromCenter(brace), Write(rule_text), self.change_students('pondering', 'erm', 'maybe', look_at=brace))
        self.wait(3)
...
class PrerequisiteKnowledge(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        for image in images:
            self.play(FadeInFromDown(image), FadeInFromDown(image.rect), randy.change, 'pondering')
            self.wait()

class NotTheMostComputationallyEfficient(Scene):
    CONFIG = {'words': 'Not the most \\\\ computationally efficient', 'word_height': 4, 'opacity': 0.7}
...
class GaussTitle(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(title, lag_ratio=0.2), ShowCreation(line))
        self.wait()

class WhyLearnIt(TeacherStudentsScene):

    def construct(self):
        self.student_says('What?!?  Then why \\\\ learn it?', bubble_config={'direction': LEFT}, index=2, target_mode='angry')
        self.play_all_student_changes('angry')
        self.wait()
        self.play(self.teacher.change, 'raise_right_hand', self.change_students('erm', 'happypondering'), RemovePiCreatureBubble(self.students[2], target_mode='pondering'))
        self.look_at(self.screen)
        self.wait(10)

class SetupSimpleSystemOfEquations(LinearTransformationScene):
    CONFIG = {'matrix': [[3, 2], [-1, 2]], 'output_vect': [-4, -2], 'quit_before_final_transformation': False, 'array_scale_factor': 0.75, 'compare_to_big_system': True, 'transition_to_geometric_view': True}
    ...
    def introduce_system(self):
        ...
        unknown_circles = VGroup(*[Circle(color=YELLOW).replace(term).scale(1.5) for term in system.unknowns])
        ...
        self.add(system)
        self.wait()
        self.play(LaggedStartMap(ApplyMethod, unknown_circles, lambda m: (m.restore,), lag_ratio=0.7))
        ...
        if self.compare_to_big_system:
            ...
            self.wait(2)
            cutoff = 3 * dim - 1
            self.play(*[ReplacementTransform(big_system[i][:cutoff], system[i][:cutoff]) for i in range(dim)] + [ReplacementTransform(big_system[i][-2:], system[i][-2:]) for i in range(dim)] + [FadeOut(big_system[i][start:end]) for i in range(big_dim) for start in [cutoff if i < dim else 0] for end in [-2 if i < dim else len(big_system[i])]])
            self.remove(big_system, system)
            self.add(system)

    def from_system_to_matrix(self):
        system_in_lines = self.system
        matrix_system = self.matrix_system = LinearSystem(self.matrix, self.output_vect, height=2)
        matrix_system.center()
        corner_rect = self.corner_rect = SurroundingRectangle(matrix_system, buff=MED_SMALL_BUFF)
        ...
        self.play(ReplacementTransform(VGroup(*list(map(VGroup, system_in_lines_copy.matrix_elements))), matrix_system.matrix_mobject.elements), Write(matrix_system.matrix_mobject.brackets), Write(matrix_system.output_vect_mob.brackets), Write(matrix_system.input_vect_mob.brackets), Write(matrix_system.equals))
        self.play(ReplacementTransform(VGroup(*list(map(VGroup, system_in_lines_copy.output_vect_elements))), matrix_system.output_vect_mob.elements))
        self.play(*[ReplacementTransform(uk, elem) for uk, elem in zip(system_in_lines_copy.unknowns, it.cycle(matrix_system.input_vect_mob.elements))])
        self.wait()
        if self.transition_to_geometric_view:
            self.play(Write(self.background_plane), Write(self.plane), FadeOut(system_in_lines), FadeIn(corner_rect), matrix_system.set_height, corner_rect.get_height() - MED_LARGE_BUFF, matrix_system.move_to, corner_rect)
    ...
    def show_geometry(self):
        ...
        second_half_matrix = np.dot(matrix, np.linalg.inv(first_half_matrix))
        ...
        self.play(GrowArrow(output_vect_mob), MoveToTarget(output_vect_label))
        ...
        for column, color in zip(columns, [X_COLOR, Y_COLOR]):
            ...
            self.play(column.set_color, color, system.input_vect_mob.elements.set_color, WHITE, ShowPassingFlash(rect))
        ...
        for column, column_mob, m in zip(columns, column_mobs, matrices):
            ...
            self.apply_matrix(m, added_anims=[ApplyMethod(column_mob.restore, path_arc=90 * DEGREES)])
        self.wait()
        self.remove_foreground_mobjects(column_mobs)
        self.apply_inverse(self.matrix, run_time=1, added_anims=[ReplacementTransform(output_vect_mob.copy(), input_vect_mob), ReplacementTransform(output_vect_label.elements.copy(), q_marks), FadeOut(column_mobs)])
        self.play(ShowPassingFlash(q_marks_rect))
        self.wait(2)
        if not self.quit_before_final_transformation:
            self.apply_matrix(self.matrix, added_anims=[FadeOut(q_marks), ReplacementTransform(input_vect_mob, output_vect_mob), FadeIn(column_mobs)])
            self.wait()
    ...
    def get_system(self, matrix, output_vect):
        ...
        colors = [color for i, color in zip(list(range(len(matrix))), it.cycle([X_COLOR, Y_COLOR, Z_COLOR, YELLOW, MAROON_B, TEAL]))]
        system = VGroup()
        ...
        for row, num in zip(matrix, output_vect):
            ...
            args.append(str(num))
            line = OldTex(*args)
            line.set_color_by_tex_to_color_map(dict([(char, color) for char, color in zip(chars, colors)]))
            system.add(line)
...
class ShowZeroDeterminantCase(LinearTransformationScene):
    CONFIG = {'show_basis_vectors': True, 'matrix': [[3, -2.0], [1, -2.0 / 3]], 'tex_scale_factor': 1.25, 'det_eq_symbol': '='}
    ...
    def add_equation(self):
        equation = self.equation = OldTex('A', '\\vec{\\textbf{x}}', '=', '\\vec{\\textbf{v}}')
    ...
    def show_det_zero(self):
        ...
        solution_vectors = VGroup(*[self.get_vector(solution + x * null_space_basis, rectangular_stem_width=0.025, tip_length=0.2) for x in np.linspace(-4, 4, 20)])
        ...
        self.play(ShowCreation(vect_off_span_mob.circle), Write(vect_off_span_words))
        self.wait()
        self.play(FadeOut(vect_off_span_mob.circle), ReplacementTransform(vect_off_span_mob, vect_in_span_mob), ReplacementTransform(vect_off_span_words, vect_in_span_words), ReplacementTransform(vect_off_span_mob.circle, vect_in_span_mob.circle))
        self.wait(2)
        self.play(FadeOut(vect_in_span_words), FadeOut(vect_in_span_mob.circle), ApplyMethod(moving_group.restore, run_time=2), ReplacementTransform(VGroup(vect_in_span_mob), solution_vectors, run_time=2))
        self.wait()
    ...
    def show_det_equation(self):
        ...
        self.play(FadeIn(det_rect), Write(det_equation[0]), ReplacementTransform(equation.get_part_by_tex('A').copy(), det_equation.get_part_by_tex('A').copy()), Write(det_equation[2:]))
...
class NonZeroDeterminantCase(ShowZeroDeterminantCase, SetupSimpleSystemOfEquations):
    ...
    def construct(self):
        ...
        self.apply_matrix(matrix, added_anims=[ReplacementTransform(input_vect_mob.copy(), output_vect_mob), ReplacementTransform(input_vect_label.copy(), output_vect_label)], run_time=2)
        self.wait()
        self.remove(output_vect_mob, output_vect_label)
        self.apply_inverse(matrix, added_anims=[ReplacementTransform(output_vect_mob.copy(), input_vect_mob), ReplacementTransform(output_vect_label.copy(), input_vect_label)], run_time=2)
        self.wait()

class ThinkOfPuzzleAsLinearCombination(SetupSimpleSystemOfEquations):
    CONFIG = {'output_vect': [-4, -2]}
    ...
    def rearrange_equation_as_linear_combination(self):
        ...
        column_arrays = VGroup(*[MobjectMatrix(matrix.deepcopy().mob_matrix[:, i]) for i in (0, 1)])
    ...
    def show_linear_combination_of_vectors(self):
        ...
        for basis, scalar in zip(basis_vectors, input_vect):
            ...
            basis.target.put_start_and_end_on(origin, self.plane.coords_to_point(*new_coords))
        dashed_lines = VGroup(*[DashedLine(LEFT, RIGHT) for x in range(2)])

        def update_dashed_lines(lines):
            for i in (0, 1):
                lines[i].put_start_and_end_on(basis_vectors[i].get_start(), basis_vectors[i].get_end())
        ...
        for basis in basis_vectors:
            self.play(MoveToTarget(basis, run_time=2), UpdateFromFunc(dashed_lines, update_dashed_lines))
        self.wait()
...
class LookAtDotProducts(SetupSimpleSystemOfEquations):
    CONFIG = {'quit_before_final_transformation': True, 'equation_scale_factor': 0.7}
    ...
    def remove_corner_system(self):
        ...
        for i in (0, 1):
            basis = [0, 0]
            basis[i] = 1
            equation = VGroup(Matrix(['x', 'y']), OldTex('\\cdot'), IntegerMatrix(basis), OldTex('='), OldTex(['x', 'y'][i]))
        ...
        self.wait()
        self.play(FadeIn(corner_rect), FadeIn(equations[0][:-1]), ShowCreation(v_dashed_line), GrowFromCenter(h_brace))
        self.play(ReplacementTransform(h_brace.copy(), equations[0][-1]))
        self.wait()
        self.play(corner_rect.restore, Animation(equations[0]), FadeIn(equations[1]))
        self.wait()
        self.play(ReplacementTransform(equations[1][-1].copy(), v_brace), ShowCreation(h_dashed_line), GrowFromCenter(v_brace))
        self.wait()
        self.to_fade = VGroup(h_dashed_line, v_dashed_line, h_brace, v_brace, xy_vect_mob, q_marks)
    ...
    def show_dot_products(self):
        ...
        for equation in moving_equations:
            ...
            transformed_equation = VGroup(T1, lp1, xy_vect, rp1, dot, T2, lp2, basis, rp2, equals, coord)
            ...
            transformed_input_rects.add(SurroundingRectangle(transformed_equation[:4], color=OUTPUT_COLOR))
            transformed_basis_rects.add(SurroundingRectangle(transformed_equation[5:5 + 4], color=basis.elements.get_color()))
            for mob in [implies]:
        ...
        self.play(MoveToTarget(corner_rect), Animation(self.equations), FadeOut(self.to_fade), LaggedStartMap(Write, implications))
        self.remove(self.input_vect_mob)
        self.apply_matrix(self.matrix, added_anims=[Animation(VGroup(corner_rect, self.equations, implications)), MoveToTarget(moving_equations[0]), LaggedStartMap(FadeIn, transformed_equations[0].parts_to_write), FadeIn(self.column_mobs), ReplacementTransform(self.input_vect_mob.copy(), self.output_vect_mob)])
        self.play(MoveToTarget(moving_equations[1]), LaggedStartMap(FadeIn, transformed_equations[1].parts_to_write), path_arc=-30 * DEGREES, run_time=2)
        self.wait()
        self.play(LaggedStartMap(ShowCreation, transformed_input_rects, lag_ratio=0.8), ShowCreation(self.output_vect_label.rect))
        for tbr, column_mob in zip(transformed_basis_rects, self.column_mobs):
            self.play(ShowCreation(tbr), ShowCreation(column_mob.rect))
        self.wait()
        self.play(FadeOut(VGroup(transformed_input_rects, transformed_basis_rects, self.output_vect_label.rect, *[cm.rect for cm in self.column_mobs])))
...
class NotAtAllTrue(NotTheMostComputationallyEfficient):
    CONFIG = {'words': 'Not at all \\\\ True', 'word_height': 4}

class ShowDotProductChanging(LinearTransformationScene):
    CONFIG = {'matrix': [[2, -5.0 / 3], [-5.0 / 3, 2]], 'v': [1, 2], 'w': [2, 1], 'v_label': 'v', 'w_label': 'w', 'v_color': YELLOW, 'w_color': MAROON_B, 'rhs1': '> 0', 'rhs2': '< 0', 'foreground_plane_kwargs': {'x_radius': 2 * FRAME_WIDTH, 'y_radius': 2 * FRAME_HEIGHT}, 'equation_scale_factor': 1.5}

    def construct(self):
        v_mob = self.add_vector(self.v, self.v_color, animate=False)
        w_mob = self.add_vector(self.w, self.w_color, animate=False)
        kwargs = {'transformation_name': 'T', 'at_tip': True, 'animate': False}
    ...
    def get_equation(self, v_label, w_label, rhs):
        equation = VGroup(v_label.copy(), OldTex('\\cdot'), w_label.copy(), OldTex(rhs))
        equation.arrange(RIGHT, buff=SMALL_BUFF)
...
class ShowDotProductChangingAwayFromZero(ShowDotProductChanging):
    CONFIG = {'matrix': [[2, 2], [0, -1]], 'v': [1, 0], 'w': [0, 1], 'v_label': 'x', 'w_label': 'y', 'v_color': X_COLOR, 'w_color': Y_COLOR, 'rhs1': '= 0', 'rhs2': '\\ne 0'}
...
class OrthonormalWords(Scene):
    ...
    def construct(self):
        v_tex = '\\vec{\\textbf{v}}'
        w_tex = '\\vec{\\textbf{w}}'
        top_words = OldTex('\\text{If }', 'T(', v_tex, ')', '\\cdot', 'T(', w_tex, ')', '=', v_tex, '\\cdot', w_tex, '\\text{ for all }', v_tex, '\\text{ and }', w_tex)
        top_words.set_color_by_tex_to_color_map({v_tex: YELLOW, w_tex: MAROON_B})
...
class ShowSomeOrthonormalTransformations(LinearTransformationScene):
    CONFIG = {'random_seed': 1, 'n_angles': 7}
...
class SolvingASystemWithOrthonormalMatrix(LinearTransformationScene):
    CONFIG = {'array_scale_factor': 0.6}

    def construct(self):
        angle = TAU / 12
        matrix = np.array([[np.cos(angle), -np.sin(angle)], [np.sin(angle), np.cos(angle)]])
        output_vect = [1, 2]
        symbolic_matrix = [['\\cos(30^\\circ)', '-\\sin(30^\\circ)'], ['\\sin(30^\\circ)', '\\cos(30^\\circ)']]
        system = LinearSystem(matrix=symbolic_matrix, output_vect=output_vect, matrix_config={'h_buff': 2.5, 'element_to_mobject': Tex}, height=1.25)
        system.to_corner(UL)
        ...
        for i, vect in zip(list(range(2)), [DR, DL]):
            ...
            column_mob.next_to(self.get_vector(matrix[:, i]).get_end(), vect, buff=SMALL_BUFF)
            column_mobs.add(column_mob)
        ...
        input_dashed_lines = VGroup(DashedLine(input_vect_mob.get_end(), x_point), DashedLine(input_vect_mob.get_end(), y_point))
        ...
        self.apply_inverse(matrix, added_anims=[FadeOut(column_mobs), ReplacementTransform(output_vect_mob.copy(), input_vect_mob), ReplacementTransform(output_dashed_lines.copy(), input_dashed_lines), FadeIn(input_vect_label)])
        self.wait()
        self.remove(input_dashed_lines, input_vect_mob)
        self.apply_matrix(matrix, added_anims=[FadeOut(input_vect_label), ReplacementTransform(input_vect_mob.copy(), output_vect_mob), ReplacementTransform(input_dashed_lines.copy(), output_dashed_lines), FadeIn(column_mobs)])
        equations = VGroup()
        for i in (0, 1):
            ...
            equation = VGroup(moving_var.generate_target(), OldTex('='), moving_output_vect_label.generate_target(), OldTex('\\cdot'), moving_column_mob.generate_target(use_deepcopy=True))
        ...
        for i, equation in enumerate(equations):
            anims = [FadeIn(equation.background_rectangle), Write(equation.to_write), LaggedStartMap(MoveToTarget, equation.movers, path_arc=60 * DEGREES)]
            if i == 0:
    ...
    def get_column_animations(self, matrix_mobject, column_mobs):
        ...
        def get_kwargs(i):
            return {'rate_func': squish_rate_func(smooth, 0.4 * i, 0.6 + 0.4 * i), 'run_time': 2}
        return list(it.chain(*[[FadeIn(cm[0], **get_kwargs(i)), ReplacementTransform(matrix_mobject.brackets.copy(), cm.brackets, **get_kwargs(i)), ReplacementTransform(VGroup(*matrix_mobject.mob_matrix[:, i]).copy(), cm.elements, **get_kwargs(i))] for i, cm in enumerate(column_mobs)]))

class TransitionToParallelogramIdea(TeacherStudentsScene):

    def construct(self):
        teacher_words = OldTexText('Now ask about \\\\ other geometric \\\\ views of', '$x$', 'and', '$y$')
        teacher_words.set_color_by_tex_to_color_map({'$x$': X_COLOR, '$y$': Y_COLOR})
        self.student_says("But that's a super \\\\ specific case", target_mode='sassy', added_anims=[self.teacher.change, 'guilty'])
        self.play_student_changes('confused', 'sassy', 'angry')
        self.wait()
        self.teacher_says(teacher_words, added_anims=[self.change_students(*['pondering'] * 3)])
        self.wait()

class TransformingAreasYCoord(LinearTransformationScene):
    CONFIG = {'index': 1, 'matrix': [[2, -1], [0, 1]], 'input_vect': [3, 2], 'array_scale_factor': 0.7}
    ...
    def show_coord_parallelogram(self):
        ...
        for basis, char, direction in zip(basis_vectors, chars, directions):
            label = self.get_vector_label(basis, '\\mathbf{\\hat{%s}}' % char, direction=direction)
        ...
        area_arrow = Vector(area_arrow_direction, color=WHITE, rectangular_stem_width=0.025, tip_length=0.2)
        area_arrow.shift(ip.get_center() - area_arrow.get_end() + SMALL_BUFF * DL)
        area_words = OldTex('\\text{Area}', '=', '1', '\\times', ['x', 'y'][index])
        area_words.next_to(area_arrow.get_start(), UL, SMALL_BUFF, submobject_to_align=area_words[0])
        area_words.set_color_by_tex_to_color_map({'Area': YELLOW, 'x': X_COLOR, 'y': Y_COLOR})
        area_words.rect = BackgroundRectangle(area_words)
        origin = self.plane.coords_to_point(0, 0)
        unit_brace = Brace(Line(origin, basis_vectors[non_index].get_end()), [DOWN, LEFT][non_index], buff=SMALL_BUFF)
        ...
        self.play(ShowCreationThenDestruction(SurroundingRectangle(basis_labels[non_index])), GrowArrow(self.basis_vectors[non_index].copy(), remover=True))
        self.play(ShowCreationThenDestruction(SurroundingRectangle(input_vect_label)), GrowArrow(input_vect_mob.copy(), remover=True))
        self.wait()
        self.play(FadeIn(area_words.rect), Write(area_words[:2]), GrowArrow(area_arrow))
        self.wait()
        self.play(FadeOut(basis_labels), GrowFromCenter(unit_brace), FadeIn(one), FadeIn(area_words[2]))
        self.wait()
        self.play(GrowFromCenter(coord_brace), FadeIn(coord_brace.label), FadeIn(area_words[3:]))
        self.wait()
        self.play(area_words.rect.stretch_to_fit_width, area_words[:3].get_width() + SMALL_BUFF, {'about_edge': LEFT}, FadeOut(area_words[2:4]), area_words[4].shift, (area_words[2].get_left()[0] - area_words[4].get_left()[0]) * RIGHT, Animation(area_words[:2]))
        ...
        self.play(PiCreatureSays(morty, 'Run with \\\\ me here...', look_at=randy.eyes), randy.look_at, morty.eyes)
        ...
        self.play(area_words.rect.stretch_to_fit_width, area_words.get_width(), {'about_edge': RIGHT}, Write(signed), Animation(area_words))
        self.play(UpdateFromFunc(ip, lambda m: Transform(m, self.get_input_parallelogram(input_vect_mob)).update(1)), input_vect_mob.rotate, np.pi, {'about_point': origin}, Animation(self.basis_vectors), UpdateFromFunc(brace_group, update_brace_group), UpdateFromFunc(input_vect_label, lambda ivl: self.set_input_vect_label_position(input_vect_mob, ivl)), MaintainPositionRelativeTo(area_arrow, ip), MaintainPositionRelativeTo(area_words.rect, area_arrow), MaintainPositionRelativeTo(area_words, area_arrow), run_time=9, rate_func=there_and_back_with_pause)
        self.play(LaggedStartMap(FadeOut, VGroup(unit_brace, one, coord_brace, coord_brace.label)))
    ...
    def transform_space(self):
        ...
        blobs = VGroup(Circle(radius=0.5).move_to(2 * LEFT + UP), Square(side_length=1).rotate(TAU / 12).move_to(2 * UP + 0.5 * RIGHT), OldTex('\\pi').scale(3).move_to(3 * RIGHT))
        ...
        self.play(Write(apply_words), FadeIn(matrix_mobject), GrowFromCenter(matrix_brace), Write(matrix_label))
        self.add_foreground_mobjects(apply_group)
        self.play(*list(map(FadeOut, [area_words.rect, area_words, area_arrow, input_vect_label])))
        self.apply_matrix(matrix)
        self.wait(2)
        ...
        self.play(ShowPassingFlash(ip_copy), FadeIn(q_marks.rect), Animation(ip), Animation(basis_vectors), Animation(input_vect_mob), Write(q_marks), LaggedStartMap(FadeOut, blobs))
        self.transformable_mobjects.remove(blobs)
        self.play(FadeIn(column_mob.background_rectangle), ReplacementTransform(matrix_mobject.brackets.copy(), column_mob.brackets), ReplacementTransform(VGroup(*matrix_mobject.mob_matrix[:, non_index]).copy(), column_mob.elements))
        self.wait()
        self.play(ReplacementTransform(matrix_label.copy(), transformed_input_vect_label[0]), FadeIn(transformed_input_vect_label[1]))
        self.wait(2)
        self.remove(q_marks.rect)
        self.apply_inverse(matrix, added_anims=[FadeOut(q_marks), FadeOut(transformed_input_vect_label), FadeOut(column_mob), FadeIn(area_words.rect), FadeIn(area_words), FadeIn(area_arrow)])
        self.wait(2)
        self.apply_matrix(matrix, added_anims=[UpdateFromFunc(area_arrow, lambda a: a.put_start_and_end_on(area_arrow.get_start(), ip.get_center() + SMALL_BUFF * DL)), Animation(area_words.rect), Animation(area_words)])
        det_A = area_scale_words.get_part_by_tex('det').copy()
        det_A.generate_target()
        det_A.target.scale(1.0 / 1.5)
        det_A.target.next_to(area_words[1:3], RIGHT, SMALL_BUFF, aligned_edge=DOWN, submobject_to_align=det_A.target[0])
        coord = area_words[-1]
        coord.generate_target()
        coord.target.next_to(det_A.target, RIGHT, SMALL_BUFF)
        self.play(area_words.rect.match_width, VGroup(area_words, coord.target), {'stretch': True, 'about_edge': LEFT}, Animation(area_words), MoveToTarget(det_A), MoveToTarget(coord))
        self.wait(2)
    ...
    def solve_for_coord(self):
        ...
        rhs = VGroup(VGroup(num_det_text, top_matrix_mobject), rhs_h_line, VGroup(matrix_mobject_copy, denom_det_text))
        ...
        system = VGroup(matrix_mobject.target, system_input, system_eq, system_output.target)
        system.arrange(RIGHT, buff=SMALL_BUFF)
        system.to_corner(UL)
        self.play(FadeOut(self.area_scale_words), ShowCreation(h_line), *list(map(MoveToTarget, area_words[1:])), run_time=3)
        self.wait()
        self.play(ReplacementTransform(matrix_mobject.copy(), matrix_mobject_copy), Write(rhs_equals), Write(denom_det_text), ShowCreation(rhs_h_line))
        self.wait(2)
        self.play(FadeIn(output_vect_label.background_rectangle), ReplacementTransform(self.input_vect_mob.copy(), output_vect_label.elements), Write(output_vect_label.brackets))
        self.wait()
        self.play(FadeOut(apply_word), MoveToTarget(matrix_mobject), MaintainPositionRelativeTo(matrix_brace, matrix_mobject), MaintainPositionRelativeTo(matrix_label, matrix_mobject))
        self.play(FadeIn(system_input), FadeIn(system_eq), MoveToTarget(system_output), area_words.rect.stretch_to_fit_width, self.area_words[1:].get_width(), {'about_edge': RIGHT}, Animation(self.area_words[1:]), FadeOut(self.area_words[0]))
        self.wait()
        replaced_column = VGroup(*top_matrix_mobject.mob_matrix[:, self.index])
        replaced_column.set_fill(opacity=0)
        self.play(ReplacementTransform(matrix_mobject_copy.copy(), top_matrix_mobject), ReplacementTransform(denom_det_text.copy(), num_det_text))
        self.wiggle_vector(self.basis_vectors[non_index])
        self.wait()
        self.wiggle_vector(self.input_vect_mob)
        self.remove(replaced_column)
        self.play(Transform(output_vect_label.elements.copy(), replaced_column.copy().set_fill(opacity=1), remover=True))
        replaced_column.set_fill(opacity=1)
        self.wait()
        equation = VGroup(area_words[1:], h_line, rhs_equals, rhs)
    ...
    def get_parallelogram_braces(self, input_parallelogram):
        braces = VGroup(*[Brace(input_parallelogram, vect, buff=SMALL_BUFF, min_num_quads=3, max_num_quads=3) for vect in (DOWN, RIGHT)])
    ...
    def wiggle_vector(self, vector_mob):
        self.play(Rotate(vector_mob, 15 * DEGREES, about_point=ORIGIN, rate_func=wiggle))
...
class ZEqualsVDotK(Scene):
    ...
    def construct(self):
        equation = VGroup(OldTex('z'), OldTex('='), Matrix(['x', 'y', 'z']), OldTex('\\cdot'), IntegerMatrix([0, 0, 1]))
...
class ThreeDCoordinatesAsVolumes(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, VGroup(z, equals, det_text, matrix.brackets, VGroup(*matrix.mob_matrix[:, :2].flatten()), coord_column)))
        self.wait(2)
        coord_column.target.move_to(columns[1])
        coord_column.target.set_color_by_gradient(WHITE, Y_COLOR, WHITE)
        self.play(MoveToTarget(coord_column, path_arc=np.pi), FadeOut(columns[1]), FadeIn(columns[2]), FadeInFromDown(y), z.shift, UP, z.fade, 1)
        self.wait(2)
        coord_column.target.move_to(columns[0])
        coord_column.target.set_color_by_gradient(X_COLOR, WHITE, WHITE)
        self.play(MoveToTarget(coord_column, path_arc=np.pi), FadeOut(columns[0]), FadeIn(columns[1]), FadeInFromDown(x), y.shift, UP, y.fade, 1)
        self.wait(2)
...
class Introduce3DSystem(SetupSimpleSystemOfEquations):
    CONFIG = {'matrix': [[3, 2, -7], [1, 2, -4], [4, 0, 1]], 'output_vect': [4, 2, 5], 'compare_to_big_system': False, 'transition_to_geometric_view': False}
...
class AreYouPausingAndPondering(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says('Are you pausing \\\\ and pondering?', target_mode='sassy', added_anims=[self.change_students(*3 * ['guilty'])])
        self.wait()
        self.play_all_student_changes('thinking', added_anims=[RemovePiCreatureBubble(self.teacher, target_mode='raise_right_hand')], look_at=self.screen)
        self.wait(6)
