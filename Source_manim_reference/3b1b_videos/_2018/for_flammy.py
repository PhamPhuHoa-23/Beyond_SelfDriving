...
class SphereRings(SecondProof):
    ...
    def grow_rings(self):
        ...
        self.wait()
        self.play(FadeOut(sphere), FadeIn(southern_mesh), FadeIn(north_rings))
        self.wait(4)
    ...
    def show_one_ring(self):
        ...
        self.play(to_fade.set_fill, {'opacity': 0.2}, to_fade.set_stroke, {'opacity': 0.0})
        self.move_camera(phi=0, theta=-90 * DEGREES, run_time=2)
        self.stop_ambient_camera_rotation()
        self.play(ShowCreation(circle))
        self.play(FadeOut(circle))
        self.move_camera(phi=70 * DEGREES, theta=-100 * DEGREES, run_time=2)
    ...
    def show_radial_line(self):
        ...
        VGroup(R_label, xy_line, arc, R_label, r_line, r_label).set_shade_in_3d(True)
        self.move_camera(phi=85 * DEGREES, theta=-100 * DEGREES, added_anims=[ring.set_fill, {'opacity': 0.5}, ring.set_stroke, {'opacity': 0.1}, ShowCreation(R_line), FadeIn(R_label, IN)])
        self.wait()
        self.play(FadeIn(xy_line), ShowCreation(arc), Write(theta))
        self.wait()
    ...
    def flash_through_rings(self):
        ...
        for x in range(8):
            self.play(LaggedStartMap(ApplyMethod, rings, lambda m: (m.set_fill, PINK, 0.5), rate_func=there_and_back, lag_ratio=0.1, run_time=2))
...
class IntegralSymbols(Scene):
    ...
    def construct(self):
        ...
        circum_formula = OldTex('2\\pi', 'R\\cos(\\theta)')
        ...
        self.add(int_sign)
        self.play(GrowFromCenter(area_brace), FadeIn(area_text, UP))
        self.wait()
        self.play(FadeInFromDown(circumference))
        self.play(FadeInFromDown(thickness), Write(times))
        self.play(Write(all_rings))
        self.wait()
        self.play(circumference.next_to, circum_brace, UP, MED_SMALL_BUFF, circumference.shift, SMALL_BUFF * UR, GrowFromCenter(circum_brace))
        self.play(FadeIn(circum_formula, UP))
        self.wait()
        self.play(thickness.next_to, circumference, RIGHT, MED_SMALL_BUFF, GrowFromCenter(R_dtheta_brace), area_brace.stretch, 0.84, 0, {'about_edge': LEFT}, MaintainPositionRelativeTo(area_text, area_brace))
        self.play(FadeIn(R_dtheta, UP))
        self.wait()
        self.play(ReplacementTransform(all_rings, bounds))
        self.wait()
        rhs = OldTex('\\displaystyle =', '2\\pi R^2', '\\int_0^{\\pi / 2}', '\\cos(\\theta)', 'd\\theta')
        ...
        self.wait()
        self.play(GrowFromCenter(int_brace), LaggedStartMap(FadeInFrom, q_marks, lambda m: (m, UP)))
        self.wait()
