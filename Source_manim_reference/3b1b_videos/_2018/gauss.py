...
class ShowRowReduction(Scene):
    CONFIG = {'matrices': [[[2, -1, -1], [0, 3, -4], [-3, 2, 1]], [[1, 0, 0], [0, 1, 0], [0, 0, 1]]], 'h_spacing': 2, 'extra_h_spacing': 0.5, 'v_spacing': 1, 'include_separation_lines': True, 'changing_row_color': YELLOW, 'reference_row_color': BLUE}
    ...
    def apply_row_rescaling(self, row_index, scale_factor):
        ...
        label = VGroup(OldTex('r_%d' % (row_index + 1)), OldTex('\\rightarrow'), OldTex('('), FractionMobject(scale_factor), OldTex(')'), OldTex('r_%d' % (row_index + 1)))
        ...
        self.play(FadeIn(label), row.set_color, self.changing_row_color)
    ...
    def add_row_multiple_to_row(self, row1_index, row2_index, scale_factor):
        ...
        label = VGroup(OldTex('r_%d' % (row1_index + 1)), OldTex('\\rightarrow'), OldTex('r_%d' % (row1_index + 1)), OldTex('+'), OldTex('('), FractionMobject(scale_factor), OldTex(')'), OldTex('r_%d' % (row2_index + 1)))
        label.arrange(RIGHT, buff=SMALL_BUFF)
        label.to_edge(UP)
        VGroup(label[0], label[2]).set_color(self.changing_row_color)
        label[-1].set_color(self.reference_row_color)
        self.play(FadeIn(label), row1.set_color, self.changing_row_color, row2.set_color, self.reference_row_color)
        ...
        self.play(FadeIn(row2_parens), Write(scalar))
        self.play(ReplacementTransform(row2.copy(), scaled_row2))
        self.wait()
        for elem, new_elem, s_elem in zip(row1, new_row1, scaled_row2):
            self.play(FadeOut(elem), FadeIn(new_elem), Transform(s_elem, new_elem.copy().fade(1), remover=True))
        self.wait()
        self.play(FadeOut(label), FadeOut(row2_parens), FadeOut(scalar), new_row1.set_color, WHITE, row2.set_color, WHITE)
