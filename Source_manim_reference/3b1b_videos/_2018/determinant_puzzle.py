...
class WorkOutNumerically(Scene):
    CONFIG = {'M1_COLOR': TEAL, 'M2_COLOR': PINK}
    ...
    def add_question(self):
        equation = self.original_equation = OldTex('\\det(', 'M_1', 'M_2', ')', '=', '\\det(', 'M_1', ')', '\\det(', 'M_2', ')')
        equation.set_color_by_tex_to_color_map({'M_1': self.M1_COLOR, 'M_2': self.M2_COLOR})
    ...
    def add_example(self):
        ...
        eq_parts = OldTex('\\det', '\\big(', '\\big)', '=', '\\det', '\\big(', '\\big)', '\\det', '\\big(', '\\big)')
        ...
        equation = self.equation_with_numbers = VGroup(*it.chain(eq_parts[:i1], [M1, M2], eq_parts[i1:i2], [self.M1_copy], eq_parts[i2:i3], [self.M2_copy], eq_parts[i3:]))
    ...
    def compute_rhs(self):
        ...
        line1 = VGroup(OldTex('\\big(', '2', '\\cdot', '2', '-', '(-1)', '\\cdot', '1', '\\big)'), OldTex('\\big(', '-1', '\\cdot', '1', '-', '4', '\\cdot', '1', '\\big)'))
        ...
        for matrix, det in zip([M1, M2], line1):
            ...
            self.play(LaggedStartMap(FadeIn, non_numbers, run_time=1), LaggedStartMap(ReplacementTransform, matrix_numbers, lambda m: (m, next(numbers_iter)), path_arc=TAU / 6))
    ...
    def compute_lhs(self):
        ...
        group = VGroup(det_tex, OldTex('\\downarrow'), OldTex('(-3)(5) - (7)(0)').scale(0.8), OldTex('\\downarrow'), OldTex('-15'))
...
class SuccessiveLinearTransformations(LinearTransformationScene):
    CONFIG = {'matrix_2': [[3, -1], [0, 1]], 'matrix_1': [[2, 3], [-1.0 / 3, 2]]}
    ...
    def scale_area_successively(self, tex2='3', tex1='5', tex_prod='15', reset=True):
        ...
        t2 = '$%s \\, \\cdot $' % tex2
        t3 = 'Area'
        areas = VGroup(OldTexText('', '', t3), OldTexText('', t2, t3), OldTexText(t1, t2, t3))
        areas.scale(0.8)
        ...
        self.wait()
        if reset:
            self.play(FadeOut(VGroup(self.square, area, brace, brace_tex)), Animation(self.plane), Animation(self.basis_vectors))
    ...
    def apply_transformations_successively(self):
        ...
        expression.set_color_by_tex_to_color_map({'M_1': TEAL, 'M_2': PINK})
