...
class CreationDestructionMobject(VMobject):
    CONFIG = {'start_time': 0, 'frequency': 0.25, 'max_ratio_shown': 0.3, 'use_copy': True}
    ...
    def __init__(self, template, **kwargs):
        ...
        def update(mob, dt):
            ...
            mob.shown_mob.pointwise_become_partial(mob.ghost_mob, max(interpolate(-mrs, 1, alpha), 0), min(interpolate(0, 1 + mrs, alpha), 1))
        self.add_updater(update)

class Eddy(VMobject):
    CONFIG = {'cd_mob_config': {'frequency': 0.2, 'max_ratio_shown': 0.3}, 'n_spirils': 5, 'n_layers': 20, 'radius': 1, 'colors': [BLUE_A, BLUE_E]}
    ...
    def get_line(self, r):
        return ParametricCurve(lambda t: r * (t + 1) ** (-1) * np.array([np.cos(TAU * t), np.sin(TAU * t), 0]), t_min=0.1 * random.random(), t_max=self.n_spirils, stroke_width=1, color=interpolate_color(*self.colors, random.random()))

class Chaos(Eddy):
    CONFIG = {'n_lines': 12, 'height': 1, 'width': 2, 'n_midpoints': 4, 'cd_mob_config': {'use_copy': False, 'frequency': 1, 'max_ratio_shown': 0.8}}
    ...
    def get_line(self, y):
        ...
        def update(line, dt):
            ...
            new_anchors = [np.array([x + 1.0 * delta_h * np.cos(f * line.total_time), y0 + y + 1.0 * delta_h * np.cos(f * line.total_time), 0]) for x, f in zip(xs, frequencies)]
            line.set_points_smoothly(new_anchors)
        line.add_updater(update)
        return line

class DoublePendulum(VMobject):
    CONFIG = {'start_angles': [3 * PI / 7, 3 * PI / 4], 'color1': BLUE, 'color2': RED}
    ...
    def __init__(self, **kwargs):
        ...
        m1, m2 = (1, 1)
        g = 9.81
        self.state_vect = np.array([self.start_angles[0], 0, self.start_angles[1], 0])
        self.state_vect += np.random.random(4) * 1e-07
        ...
        def update(group, dt):
            for x in range(2):
                ...
                group.state_vect += 0.5 * dt * np.array([theta1dot, z1dot, theta2dot, z2dot])
...
class DoublePendulums(VGroup):
    ...
    def __init__(self, **kwargs):
        ...
        VGroup.__init__(self, *[DoublePendulum(color1=random.choice(colors), color2=random.choice(colors)) for x in range(5)], **kwargs)
...
class NavierStokesEquations(Tex):
    CONFIG = {'tex_to_color_map': {'\\rho': YELLOW, '\\mu': GREEN, '\\textbf{v}': BLUE, 'p{}': RED}, 'width': 10}

    def __init__(self, **kwargs):
        v_tex = '\\textbf{v}'
        Tex.__init__(self, '\\rho', '\\left({\\partial', v_tex, '\\over', '\\partial', 't}', '+', v_tex, '\\cdot', '\\nabla', v_tex, '\\right)', '=', '-', '\\nabla', 'p{}', '+', '\\mu', '\\nabla^2', v_tex, '+', '\\textbf{F}', '\\qquad\\qquad', '\\nabla', '\\cdot', v_tex, '=', '0', **kwargs)
        self.set_width(self.width)

    def get_labels(self):
        parts = self.get_parts()
        words = ['Analogous to \\\\ mass $\\times$ acceleration', 'Pressure\\\\forces', 'Viscous\\\\forces', 'External\\\\forces']
        result = VGroup()
        braces = VGroup()
        word_mobs = VGroup()
    ...
    def get_parts(self):
        return VGroup(self[:12], self[13:16], self[17:20], self[21:22])
...
class EddyReference(Scene):
    CONFIG = {'radius': 1, 'label': 'Eddy', 'label': ''}
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(ShowCreationThenDestruction, new_eddy), FadeIn(label, rate_func=there_and_back_with_pause), run_time=3)
...
class EddyLabels(Scene):
    ...
    def construct(self):
        labels = VGroup(OldTexText('Large eddy'), OldTexText('Medium eddy'), OldTexText('Small eddy'))
        for label in labels:
...
class SomeTurbulenceEquations(PiCreatureScene):
    ...
    def construct(self):
        ...
        distribution = OldTex('E(k) \\propto k^{-5/3}', tex_to_color_map={'k': GREEN, '-5/3': YELLOW})
        ...
        self.play(Write(navier_stokes), randy.change, 'confused', navier_stokes, morty.change, 'confused', navier_stokes)
        self.wait(3)
        self.play(morty.change, 'raise_right_hand', distribution, randy.look_at, distribution, FadeInFromDown(distribution), navier_stokes.fade, 0.5)
        ...
        self.play(LaggedStartMap(FadeOut, VGroup(randy, morty, navier_stokes)), dist_group.scale, 1.5, dist_group.center, dist_group.to_edge, UP)
        self.wait()
...
class JokeRingEquation(Scene):
    ...
    def construct(self):
        items = VGroup(OldTexText('Container with a lip'), OldTexText('Fill with smoke (or fog)'), OldTexText('Hold awkwardly'))
        ...
        arrow1 = Arrow(items[0].get_left(), point + 0.8 * UP + 0.3 * RIGHT, path_arc=90 * DEGREES)
        arrow1.pointwise_become_partial(arrow1, 0, 0.99)
        arrow2 = Arrow(items[1].get_left(), point)
...
class CarefulWithLasers(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        laser = VGroup(Rectangle(height=0.1, width=0.3, fill_color=GREY_B, fill_opacity=1, stroke_color=GREY_D, stroke_width=1), Line(ORIGIN, 10 * RIGHT, color=GREEN_SCREEN))
        ...
        self.play(ShowCreation(laser), self.change_students('surprised', 'hooray', 'horrified', look_at=laser))
...
class AskAboutTurbulence(TeacherStudentsScene):
    ...
    def pi_creatures_ask(self):
        ...
        self.student_says(words, target_mode='raise_left_hand', added_anims=[morty.change, 'pondering'])
        self.play_student_changes('erm', 'raise_left_hand', 'confused')
        self.wait(3)
        self.play(morty.change, 'raise_right_hand', FadeOut(randy.bubble), ReplacementTransform(VGroup(words[1], words[3]), question), FadeOut(VGroup(words[0], words[2])), self.change_students(*3 * ['pondering'], look_at=question))
        self.play(ShowCreation(h_line), LaggedStartMap(FadeOutAndShiftDown, self.pi_creatures, run_time=1, lag_ratio=0.8))
        self.wait()
        self.question = question
        self.h_line = h_line

    def divide_by_qualitative_quantitative(self):
        v_line = Line(self.h_line.get_center(), FRAME_HEIGHT * DOWN / 2)
        words = VGroup(OldTexText('Features', color=YELLOW), OldTexText('Rigorous definition', color=BLUE))
    ...
    def three_qualitative_descriptors(self):
        words = VGroup(OldTexText('- Eddies'), OldTexText('- Chaos'), OldTexText('- Diffusion'))
...
class LaminarFlowLabel(Scene):
    ...
    def construct(self):
        ...
        words.scale(1.5)
        words.to_edge(UP)
        subwords = OldTexText("`Lamina', in Latin, means \\\\``a thin sheet of material''", tex_to_color_map={'Lamina': YELLOW}, arg_separator='')
...
class HighCurlFieldBreakingLayers(Scene):
    CONFIG = {'flow_anim': move_submobjects_along_vector_field}
...
class VorticitySynonyms(Scene):
    ...
    def construct(self):
        words = VGroup(OldTexText('High', 'vorticity'), OldTex('\\text{a.k.a} \\,', '|\\nabla \\times \\vec{\\textbf{v}}| > 0'), OldTexText('a.k.a', 'high', 'swirly-swirly', 'factor'))
...
class VorticityDoesNotImplyTurbulence(TeacherStudentsScene):
    ...
    def construct(self):
        t_to_v = OldTexText('Turbulence', '$\\Rightarrow$', 'Vorticity')
        v_to_t = OldTexText('Vorticity', '$\\Rightarrow$', 'Turbulence')
        for words in (t_to_v, v_to_t):
            words.move_to(self.hold_up_spot, DR)
            words.set_color_by_tex_to_color_map({'Vorticity': BLUE, 'Turbulence': GREEN})
        ...
        self.play(TransformFromCopy(t_to_v, v_to_t, path_arc=PI / 2), self.change_students('erm', 'confused', 'sassy', run_time=1), ShowCreation(cross, run_time=2))
        self.add(cross)
        self.wait(4)
...
class FeynmanOnTurbulence(Scene):
    ...
    def construct(self):
        ...
        quote = OldTexText('``', 'Turbulence', 'is the most\\\\important', 'unsolved problem\\\\', "of classical physics.''", tex_to_color_map={'Turbulence': BLUE, 'unsolved problem\\\\': YELLOW})
...
class ShowNavierStokesEquations(Scene):
    ...
    def introduce_equations(self):
        ...
        variables = OldTex('&\\textbf{v}', '\\text{ is velocity}\\\\', '&\\rho', '\\text{ is density}\\\\', '&p{}', '\\text{ is pressure}\\\\', '&\\mu', '\\text{ is viscosity}\\\\', tex_to_color_map=NavierStokesEquations.CONFIG['tex_to_color_map'])
        variables.to_corner(DL)
        self.play(FadeInFromDown(equations))
        self.play(Write(name))
        self.play(LaggedStartMap(FadeInFrom, variables, lambda m: (m, RIGHT)))
        self.wait()
        self.play(Write(newtons_second))
        self.wait()
        self.play(FadeInFromDown(labels[0]), newtons_second.next_to, variables, RIGHT, LARGE_BUFF)
    ...
    def ask_about_blowup(self):
        ...
        q_arrow = Arrow(question.get_bottom(), graph.point_from_proportion(0.8), buff=SMALL_BUFF, path_arc=-60 * DEGREES)
        q_arrow.set_stroke(WHITE, 3)
        morty = Mortimer()
        morty.to_corner(DR)
        morty.change('confused', graph)
        self.play(Write(axes, run_time=1), self.reasonable_question.to_edge, LEFT, self.reasonable_question.shift, DOWN)
        self.play(Write(question), ShowCreation(graph), FadeIn(morty))
    ...
    def show_money(self):
        ...
        self.play(ReplacementTransform(self.reasonable_question, problem), pi2.look_at, problem, pi1.look_at, problem, VFadeIn(pi1))
        self.wait()
        self.play(FadeInFromLarge(money))
        self.play(pi1.change, 'hooray', pi2.change, 'hooray')
        self.play(ReplacementTransform(pi1.pupils, pi1.money_eyes), ReplacementTransform(pi2.pupils, pi2.money_eyes))
        self.wait()

    def get_axes_and_graph(self):
        axes = Axes(x_min=-1, x_max=5, y_min=-1, y_max=5)
        ...
        axes.center()
        axes.to_edge(DOWN)
        v_line = DashedLine(axes.coords_to_point(4, 0), axes.coords_to_point(4, 5))
        axes.add(v_line)
        graph = axes.get_graph(lambda x: -1.0 / (x - 4), x_min=0.01, x_max=3.8)
...
class NewtonsSecond(Scene):
    ...
    def construct(self):
        ...
        square.add(label)
        square.save_state()
        arrows = VGroup(Vector(0.5 * UP).next_to(square, UP, buff=0), Vector(RIGHT).next_to(square, RIGHT, buff=0))
...
class FiguresOfFluidDynamics(Scene):
    ...
    def construct(self):
        names = ['Leonhard Euler', 'George Stokes', 'Hermann von Helmholtz', 'Lewis Richardson', 'Geoffrey Taylor', 'Andrey Kolmogorov']
        ...
        self.play(MoveToTarget(to_fade, remover=True), image_groups[-1].set_height, 5, image_groups[-1].center)
        self.wait()
...
class KineticEnergyBreakdown(Scene):
    ...
    def construct(self):
        ...
        left_items = VGroup(OldTexText('- Big moving things'), OldTexText('- Heat'))
...
class Heat(Scene):
    ...
    def construct(self):
        box = Square(side_length=2, stroke_color=WHITE)
    ...
    def get_ball(self, box):
        ...
        ball.move_to(np.array([interpolate(x0, x1, random.random()), interpolate(y0, y1, random.random()), 0]))
...
class SwirlDiameterD(Scene):
    ...
    def construct(self):
        ...
        swirl = VGroup(Arrow(RIGHT, LEFT, **kwargs), Arrow(LEFT, RIGHT, **kwargs))
        swirl.set_stroke(width=5)
        f = 1.5
        swirl.scale(f)
        h_line = DashedLine(f * LEFT, f * RIGHT, color=YELLOW)
        D_label = OldTex('D')
        D_label.scale(2)
        D_label.next_to(h_line, UP, SMALL_BUFF)
        D_label.match_color(h_line)
        self.play(*map(ShowCreation, swirl))
        self.play(GrowFromCenter(h_line), FadeIn(D_label, UP))
        self.wait()

class KolmogorovGraph(Scene):

    def construct(self):
        axes = Axes(x_min=-1, y_min=-1, x_max=7, y_max=9, y_axis_config={'unit_size': 0.7})
        ...
        v_lines = VGroup(*[DashedLine(axes.coords_to_point(x, 0), axes.coords_to_point(x, 9), color=YELLOW, stroke_width=1) for x in [0.5, 5]])
...
class FiveThirds(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(FadeOut(self.teacher.bubble), FadeOut(words[1]), self.teacher.change, 'raise_right_hand', words[0].scale, 1.5, words[0].move_to, self.hold_up_spot)
...
class VortedStretching(ThreeDScene):
    CONFIG = {'n_circles': 200}

    def construct(self):
        axes = ThreeDAxes()
        axes.set_stroke(width=1)
        self.add(axes)
        self.move_camera(phi=70 * DEGREES, theta=-145 * DEGREES, run_time=0)
    ...
    def get_cylinder_circles(self, radius, radius_var, max_z):
        return VGroup(*[ParametricCurve(lambda t: np.array([np.cos(TAU * t) * r, np.sin(TAU * t) * r, z]), **self.get_circle_kwargs()) for z in sorted(max_z * np.random.random(self.n_circles)) for r in [radius + radius_var * random.random()]]).center()
    ...
    def get_torus_circles(self, out_r, in_r, in_r_var):
        ...
        for u in sorted(np.random.random(self.n_circles)):
            ...
            circle = ParametricCurve(lambda t: r * np.array([np.cos(TAU * t), np.sin(TAU * t), 0]), **self.get_circle_kwargs())
    ...
    def get_flow_lines(self, circle_group):
        ...
        def update_circle(circle, dt):
            ...
            circle.pointwise_become_partial(circle.template, max(interpolate(-window, 1, alpha), 0), min(interpolate(0, 1 + window, alpha), 1))
        result = VGroup()
        for template in circle_group:
            circle = template.deepcopy()
            circle.set_stroke(color=interpolate_color(BLUE_A, BLUE_E, random.random()), width=1)
