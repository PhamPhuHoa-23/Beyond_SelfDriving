...
class BrickRow(VMobject):
    CONFIG = {'left_color': COLOR_HEADS, 'right_color': COLOR_TAILS, 'height': 1.0, 'width': 8.0, 'outcome_shrinkage_factor_x': 0.95, 'outcome_shrinkage_factor_y': 0.94}
    ...
    def get_rects_for_level(self, r):
        ...
        for k in range(r + 1):
            ...
            new_rect = Rectangle(width=proportion * self.width, height=self.height, fill_color=graded_color(r, k), fill_opacity=1, stroke_width=0)
    ...
    def get_subdivs_for_level(self, r):
        ...
        for k in range(0, r):
            ...
            subdiv = Line(x * RIGHT + 0.5 * self.height * UP, x * RIGHT + 0.5 * self.height * DOWN)
            subdivs.add(subdiv)
    ...
    def get_sequence_subdivs_for_level(self, r):
        ...
        for k in range(1, 2 ** r):
            proportion = dx
            x += proportion * self.width
            subdiv = DashedLine(x * RIGHT + 0.5 * self.height * UP, x * RIGHT + 0.5 * self.height * DOWN)
            subdivs.add(subdiv)
        subdivs.move_to(self.get_center())
        return subdivs
    ...
    def get_outcome_rects_for_level(self, r, inset=False, with_labels=False):
        ...
        corner_radius = 0.1
        rect = RoundedRectangle(width=outcome_width, height=outcome_height, corner_radius=corner_radius, fill_color=OUTCOME_COLOR, fill_opacity=OUTCOME_OPACITY, stroke_width=0)
        rects = VGroup()
...
class SplitRectsInBrickWall(AnimationGroup):
    ...
    def __init__(self, mobject, **kwargs):
        ...
        for k in range(0, r):
            ...
            subdiv = DashedLine(mobject.get_top() + x * RIGHT, mobject.get_bottom() + x * RIGHT, dash_length=0.05)
            subdivs.add(subdiv)
