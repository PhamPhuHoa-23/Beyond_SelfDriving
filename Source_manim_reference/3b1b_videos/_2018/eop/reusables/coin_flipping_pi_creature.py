...
class PiCreatureCoin(VMobject):
    CONFIG = {'diameter': 0.8, 'thickness': 0.2, 'nb_ridges': 7, 'stroke_color': YELLOW, 'stroke_width': 3, 'fill_color': YELLOW, 'fill_opacity': 0.7}

    def init_points(self):
        outer_rect = Rectangle(width=self.diameter, height=self.thickness, fill_color=self.fill_color, fill_opacity=self.fill_opacity, stroke_color=self.stroke_color, stroke_width=0)
        self.add(outer_rect)
        PI = TAU / 2
        ...
        for x in ridge_positions:
            ridge = Line(x * RIGHT + 0.5 * self.thickness * DOWN, x * RIGHT + 0.5 * self.thickness * UP, stroke_color=ridge_color, stroke_width=self.stroke_width)
            self.add(ridge)
...
class FlipCoin(AnimationGroup):
    CONFIG = {'coin_rate_func': there_and_back, 'pi_rate_func': lambda t: there_and_back_with_pause(t, 1.0 / 4)}

    def __init__(self, pi_creature, **kwargs):
        digest_config(self, kwargs)
        pi_creature_motion = ApplyMethod(pi_creature.flip_coin_up, rate_func=self.pi_rate_func, **kwargs)
        coin_motion = Succession(EmptyAnimation(run_time=1.0), FlipUpAndDown(pi_creature.coin, vector=UP, nb_turns=5, height=pi_creature.flip_height * pi_creature.get_height(), rate_func=self.coin_rate_func, **kwargs))
