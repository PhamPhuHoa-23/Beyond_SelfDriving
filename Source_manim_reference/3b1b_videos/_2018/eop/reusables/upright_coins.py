...
class UprightCoin(Circle):
    CONFIG = {'radius': COIN_RADIUS, 'stroke_width': COIN_STROKE_WIDTH, 'stroke_color': WHITE, 'fill_opacity': 1, 'symbol': '\\euro'}
...
class UprightHeads(UprightCoin):
    CONFIG = {'fill_color': COLOR_HEADS_COIN, 'symbol': 'H'}

class UprightTails(UprightCoin):
    CONFIG = {'fill_color': COLOR_TAILS_COIN, 'symbol': 'T'}

class CoinSequence(VGroup):
    CONFIG = {'sequence': [], 'radius': COIN_RADIUS, 'spacing': COIN_SEQUENCE_SPACING, 'direction': RIGHT}
...
class FlatCoin(UprightCoin):
    CONFIG = {'thickness': COIN_THICKNESS, 'foreshortening': COIN_FORESHORTENING, 'nb_ridges': COIN_NB_RIDGES}
    ...
    def __init__(self, **kwargs):
        ...
        edge_anchors_and_handles += control_points2[::-1]
        edge_anchors_and_handles.append(edge_anchors_and_handles[-1] + self.thickness * UP)
        edge_anchors_and_handles.append(edge_anchors_and_handles[-1] + self.thickness * DOWN)
        ...
        for x in ridge_positions:
            ...
            ridge = Line(x * RIGHT + y * UP, x * RIGHT + y * UP + self.thickness * DOWN, stroke_color=ridge_color, stroke_width=self.stroke_width)
            self.add(ridge)
