...
def get_quiz(*questions):
    q_mobs = VGroup(*list(map(TexText, ['%d. %s' % (i + 1, question) for i, question in enumerate(questions)])))
    q_mobs.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
    content = VGroup(OldTexText('Quiz').scale(1.5), Line(q_mobs.get_left(), q_mobs.get_right()), q_mobs)
...
def get_slot_group(bool_list, buff=MED_LARGE_BUFF, include_qs=True, min_bool_list_len=3):
...
def get_probability_of_slot_group(bool_list, conditioned_list=None):
    ...
    for filler, bl in zip(fillers, [bool_list, conditioned_list]):
        slot_group = get_slot_group(bl, buff=SMALL_BUFF, include_qs=False)
...
class IndependenceOpeningQuote(OpeningQuote):
    CONFIG = {'quote': ['Far better an ', 'approximate', ' answer to the ', ' right question', ', which is often vague, than an ', 'exact', ' answer to the ', 'wrong question', '.'], 'highlighted_quote_terms': {'approximate': GREEN, 'right': GREEN, 'exact': RED, 'wrong': RED}, 'author': 'John Tukey', 'quote_arg_separator': ''}
...
class DangerInProbability(Scene):
    ...
    def construct(self):
        ...
        self.play(warning.next_to, probability, UP, LARGE_BUFF, LaggedStartMap(FadeIn, probability))
        self.wait()
    ...
    def get_warning_sign(self):
        ...
        bang.move_to(interpolate(triangle.get_bottom(), triangle.get_top(), 0.4))
        triangle.add(bang)
        return triangle

class MeaningOfIndependence(SampleSpaceScene):
    CONFIG = {'sample_space_config': {'height': 4, 'width': 4}}
    ...
    def add_labeled_space(self):
        ...
        side_braces_and_labels = self.sample_space.get_side_braces_and_labels(['P(A)', 'P(\\overline A)'])
        top_braces_and_labels, bottom_braces_and_labels = [part.get_subdivision_braces_and_labels(part.vertical_parts, labels=['P(B | %s)' % s, 'P(\\overline B | %s)' % s], direction=vect) for part, s, vect in zip(self.sample_space.horizontal_parts, ['A', '\\overline A'], [UP, DOWN])]
        braces_and_labels_groups = VGroup(side_braces_and_labels, top_braces_and_labels, bottom_braces_and_labels)
        self.add(self.sample_space)
        self.play(Write(braces_and_labels_groups, run_time=4))

    def align_conditionals(self):
        line = Line(*[interpolate(self.sample_space.get_corner(vect + LEFT), self.sample_space.get_corner(vect + RIGHT), 0.7) for vect in (UP, DOWN)])
        line.set_stroke(RED, 8)
        word = OldTexText('Independence')
        word.scale(1.5)
        word.next_to(self.sample_space, RIGHT, buff=LARGE_BUFF)
        word.set_color(RED)
        self.play(*it.chain(self.get_top_conditional_change_anims(0.7), self.get_bottom_conditional_change_anims(0.7)))
    ...
    def relabel(self):
        ...
        ignored_braces, new_top_labels = self.sample_space[0].get_top_braces_and_labels(['P(B)', 'P(\\overline B)'])
        equation = OldTex('P(B | A) = P(B)')
        equation.scale(1.5)
        equation.move_to(self.independence_word)
        self.play(Transform(old_labels, new_top_labels), FadeOut(self.sample_space[1].vertical_parts.labels), FadeOut(self.sample_space[1].vertical_parts.braces))
        self.play(self.independence_word.next_to, equation, UP, MED_LARGE_BUFF, Write(equation))
        self.wait()
    ...
    def assume_independence(self):
        ...
        self.play(FadeIn(morty), everything.scale, 0.5, everything.move_to, bubble.get_bubble_center())
    ...
    def no_independence(self):
        ...
        self.play(*it.chain(self.get_top_conditional_change_anims(0.9), self.get_bottom_conditional_change_anims(0.5), [self.independence_word.fade, 0.7, self.equation.fade, 0.7, self.morty.change, 'confused', self.sample_space, FadeOut(self.independence_line)]))
        self.wait()
...
class IntroduceBinomial(Scene):
    ...
    def add_title(self):
        ...
        title.to_edge(RIGHT)
        title.shift(2 * UP)
        formula = OldTex('P(X=', 'k', ')=', '{n \\choose k}', 'p', '^k', '(1-', 'p', ')', '^{n-', 'k}', arg_separator='')
    ...
    def add_bar_chart(self):
        ...
        chart = BarChart([dist(k) for k in range(n + 1)], bar_names=list(range(n + 1)))
        chart.to_edge(LEFT)
    ...
    def add_p_slider(self):
        ...
        interval.next_to(VGroup(self.bar_chart.x_axis, self.bar_chart.y_axis), UP, MED_LARGE_BUFF)
        interval.add_numbers(0, 1)
        triangle = RegularPolygon(n=3, start_angle=-np.pi / 2, stroke_width=0, fill_color=YELLOW, fill_opacity=1)
    ...
    def shift_weight_to_tails(self):
        ...
        self.play(MoveToTarget(old_bars), ReplacementTransform(old_bars.copy().set_fill(opacity=0), new_bars))
    ...
    def change_p(self, p):
        ...
        self.play(ApplyMethod(self.bar_chart.change_bar_values, [alt_dist(k) for k in range(self.n + 1)]), triangle.move_to, interval.number_to_point(p), DOWN, MaintainPositionRelativeTo(p_label, triangle))
        self.p = p
...
class IntroduceQuiz(PiCreatureScene):
    ...
    def ask_about_probabilities(self):
        probabilities, abbreviated_probabilities = [VGroup(*[OldTex('P(', s_tex, '=', str(score), ')', rhs).set_color_by_tex_to_color_map({str(score): YELLOW, 'text': GREEN}) for score in range(4)]) for s_tex, rhs in [('\\text{Score}', '= \\, ???'), ('\\text{S}', '')]]
        ...
        self.play(LaggedStartMap(FadeIn, probabilities, run_time=3), self.quiz.set_height, 0.7 * self.randy.get_height(), self.quiz.next_to, self.randy, RIGHT, self.randy.change, 'confused', probabilities)
        self.wait()
    ...
    def show_distribution(self):
        ...
        self.play(LaggedStartMap(Write, VGroup(*[m for m in chart if m is not chart.bars])))
        self.play(*[ReplacementTransform(bar.copy().stretch_to_fit_height(0).move_to(bar.get_bottom()), bar) for bar in chart.bars])
        self.play(*[ReplacementTransform(p.copy(), short_p) for p, short_p in zip(self.probabilities, self.abbreviated_probabilities)])
        self.wait()
        self.bar_chart = chart
    ...
    def get_example_quiz(self):
        return get_quiz("Define ``Brachistochrone'' ", "Define ``Tautochrone'' ", "Define ``Cycloid'' ")
...
class BreakDownQuestionPatterns(IntroduceQuiz):
    ...
    def add_parts_from_last_scene(self):
        ...
        self.play(self.single_question_probability.scale, 0.8, self.single_question_probability.to_corner, UP + LEFT, chart_group.scale, 0.7, chart_group.get_top(), chart_group.to_edge, LEFT, FadeOut(self.probabilities))
    ...
    def break_down_possibilities(self):
        ...
        while bool_lists:
            ...
            if len(bool_list) < 3:
                bool_lists += [list(bool_list) + [True], list(bool_list) + [False]]
...
class AssociatePatternsWithScores(BreakDownQuestionPatterns):
    CONFIG = {'score_group_scale_val': 0.8}
    ...
    def add_slot_groups(self):
        self.slot_groups = VGroup(*list(map(self.get_slot_group, it.product(*[[True, False]] * 3))))
    ...
    def show_score_groups(self):
        ...
        for score, score_group in zip(scores, score_groups):
            ...
            self.wait()
            self.play(ReplacementTransform(score_group.copy(), score_group.organized), LaggedStartMap(FadeIn, score, run_time=1))
            self.play(score_group.restore)
        self.wait()

    def think_about_binomial_patterns(self):
        triangle = PascalsTriangle(nrows=5, height=3, width=3)
        ...
        self.play(self.randy.change, 'pondering', LaggedStartMap(FadeIn, triangle, run_time=4))
...
class BeforeCounting(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(triangle.scale, 0.5, triangle.to_corner, UP + RIGHT, self.teacher.change_mode, 'sassy')
...
class TemptingButWrongCalculation(BreakDownQuestionPatterns):
    ...
    def write_simple_product(self):
        ...
        rhs = VGroup(*[OldTex('P(', '\\checkmark' if b else '\\times', ')') for b in slot_group.bool_list])
        rhs.arrange(RIGHT, SMALL_BUFF)
        rhs.next_to(lhs, RIGHT, SMALL_BUFF)
        for part, b in zip(rhs, slot_group.bool_list):
            part.set_color_by_tex_to_color_map({'checkmark': GREEN, 'times': RED})
            brace = Brace(part, UP)
            if b:
                value = OldTex('(0.8)')
            else:
        ...
        self.play(Write(lhs), ShowCreation(slot_group.lines), LaggedStartMap(FadeIn, slot_group.content, run_time=3), self.randy.change, 'pondering')
        self.wait(2)
        for part, mob in zip(rhs, slot_group.content):
            self.play(*[ReplacementTransform(mob.copy(), subpart, path_arc=np.pi / 6) for subpart, mob in zip(part, [p_of, mob, rp])])
...
class ThousandPossibleQuizzes(Scene):
    CONFIG = {'n_quiz_rows': 25, 'n_quiz_cols': 40, 'n_movers': 100, 'quizzes_height': 4}
    ...
    def draw_all_quizzes(self):
        ...
        full_quizzes = VGroup(get_quiz("Define ``Brachistochrone''", "Define ``Tautochrone''", "Define ``Cycloid''"), get_quiz('Define $\\dfrac{df}{dx}$', 'Define $\\displaystyle \\lim_{h \\to 0} f(h)$', 'Prove $\\dfrac{d(x^2)}{dx} = 2x$ '), get_quiz('Find all primes $p$ \\\\ where $p+2$ is prime.', 'Find all primes $p$ \\\\ where $2^{p}-1$ is prime.', 'Solve $\\zeta(s) = 0$'))
        ...
        self.play(LaggedStartMap(FadeIn, quizzes, run_time=3, lag_ratio=0.2), Animation(full_quizzes, remover=True))
        self.wait()
    ...
    def show_division_by_first_question(self):
        ...
        for num, b, split in ((800, True, top_split), (200, False, bottom_split)):
            label = VGroup(OldTex(str(num)), get_slot_group([b], buff=SMALL_BUFF, include_qs=False))
            label.arrange(DOWN)
            label.next_to(split.target, LEFT, buff=LARGE_BUFF)
            labels.add(label)
        self.play(FadeOut(self.title), MoveToTarget(top_split), MoveToTarget(bottom_split))
        for label in labels:
    ...
    def ask_about_second_question(self):
        ...
        sg1, sg2 = slot_groups = VGroup(*[get_slot_group([True, b], include_qs=False, buff=SMALL_BUFF) for b in (True, False)])
        question = VGroup(OldTexText('Where are'), sg1, OldTexText('and'), sg2, OldTexText('?'))
        ...
        little_rects = VGroup(*[SurroundingRectangle(VGroup(sg.lines[1], sg.content[1])) for sg in slot_groups])
        ...
        self.play(FadeOut(big_rect), FadeOut(question))
        self.wait()
    ...
    def show_uncorrelated_division_by_second(self):
        ...
        left_label = VGroup(OldTex('(0.8)', '800 =', '640'), get_slot_group([True, True], buff=SMALL_BUFF, include_qs=False))
        left_label.arrange(RIGHT, buff=MED_LARGE_BUFF)
        left_label.next_to(left_split.target, UP)
        self.play(MoveToTarget(left_split), MaintainPositionRelativeTo(top_label, left_split), MoveToTarget(right_split))
        self.play(FadeIn(left_label))
        self.play(LaggedStartMap(ApplyMethod, left_split, lambda m: (m.set_color, YELLOW), rate_func=there_and_back, lag_ratio=0.2))
        self.wait()
    ...
    def increase_second_correct_slice(self):
        ...
        self.play(MoveToTarget(movers, lag_ratio=0.5, run_time=3), Transform(left_label_equation, new_equation))
        self.wait(2)
    ...
    def second_division_among_first_wrong(self):
        ...
        self.play(bottom_label.shift, LEFT, *list(map(MoveToTarget, [left_split, right_split])))
        self.play(FadeIn(label))
        self.wait()
        self.play(MoveToTarget(movers, lag_ratio=0.5, run_time=3), Transform(equation, alt_equation))
        self.wait()
    ...
    def show_that_second_is_still_80(self):
        ...
        rects = VGroup(*[SurroundingRectangle(mob, buff=SMALL_BUFF) for mob in second_right])
        ...
        self.play(FadeIn(slot_group), Transform(num1, equation[0], rate_func=squish_rate_func(smooth, 0, 0.7)), Transform(num2, equation[2], rate_func=squish_rate_func(smooth, 0.3, 1)), run_time=2)
    ...
    def show_third_question_results(self):
        all_splits = VGroup(self.top_left_split, self.top_right_split, self.bottom_left_split, self.bottom_right_split)
        ...
        self.play(FadeOut(self.top_left_label), FadeOut(self.bottom_left_label))
        for split in all_splits:
            self.play(MoveToTarget(split))
        self.wait()
        self.play(LaggedStartMap(ApplyMethod, all_right, lambda m: (m.set_color, YELLOW), rate_func=there_and_back, lag_ratio=0.2, run_time=2))
        self.wait(2)

    def get_thousand_quizzes(self):
        rows = VGroup()
        for x in range(self.n_quiz_rows):
            quiz = VGroup(*[Rectangle(height=SMALL_BUFF, width=0.5 * SMALL_BUFF) for x in range(3)])
...
class HarderQuizzes(Scene):
    ...
    def construct(self):
        quizzes = VGroup(get_quiz('Find all primes $p$ \\\\ where $p+2$ is prime.', 'Find all primes $p$ \\\\ where $2^{p}-1$ is prime.', 'Solve $\\zeta(s) = 0$'), get_quiz('Find $S$ such that \\\\ $\\#\\mathds{N} < \\#S < \\#\\mathcal{P}(\\mathds{N})$', "Describe ``forcing''", 'Prove from ZFC that $S \\notin S$.'))
...
class AccurateProductRule(SampleSpaceScene, ThreeDScene):
    ...
    def setup_terms(self):
        ...
        slot_group_lists = [[get_slot_group([True, True, False], **kwargs)], [get_slot_group([True], **kwargs)], [get_slot_group([True, True], **kwargs), get_slot_group([True], **kwargs)], [get_slot_group([True, True, False], **kwargs), get_slot_group([True, True], **kwargs)]]
    ...
    def move_to_third_dimension(self):
        ...
        cubes = VGroup(Cube(fill_color=RED_E), Cube(fill_color=GREEN_E))
        ...
        space.add(cubes)
        self.play(space.rotate, 0.9 * np.pi / 2, LEFT, space.rotate, np.pi / 12, UP, space.to_corner, DOWN + RIGHT, LARGE_BUFF)
        space.remove(cubes)
        self.play(cubes[0].set_fill, None, 1, cubes[0].set_stroke, WHITE, 1, cubes[1].set_fill, None, 0.5, cubes[1].set_stroke, WHITE, 1)
        self.wait()
        self.cubes = cubes
...
class ShowAllEightConditionals(Scene):
    ...
    def show_all_conditionals(self):
        ...
        for bool_list in it.product(*[[True, False]] * 3):
            equation = OldTex('P(', filler_tex, ')', '=', 'P(', filler_tex, ')', 'P(', filler_tex, '|', filler_tex, ')', 'P(', filler_tex, '|', filler_tex, ')')
        ...
        rect = SurroundingRectangle(VGroup(*equations[0][7:], *equations[-1][7:]), buff=SMALL_BUFF)
    ...
    def suggest_independence(self):
        ...
        self.play(PiCreatureSays(randy, "Let's just assume \\\\ independence.", target_mode='shruggie'))
        self.play(Blink(randy))
        self.wait()
...
class ShowIndependenceSymbolically(Scene):
    ...
    def construct(self):
        ...
        for part in lhs.get_parts_by_tex(filler_tex):
            slot_group = get_slot_group([True, True, True], buff=SMALL_BUFF, include_qs=False)
            slot_group.replace(part, dim_to_match=0)
        ...
        bool_lists = [[False], [True, False], [False, True], [True]]
        arrow = Arrow(UP, DOWN)
...
class ComputeProbabilityOfOneWrong(Scene):
    CONFIG = {'score': 2, 'final_result_rhs_tex': ['3', '(0.8)', '^2', '(0.2)', '=', '0.384'], 'default_bool': True, 'default_p': '0.8', 'default_q': '0.2'}
    ...
    def show_all_three_patterns(self):
        ...
        self.wait(2)
        for i in range(2):
            self.play(ReplacementTransform(probabilities[i].copy(), probabilities[i + 1]))
        self.wait()
        for group in (point_8s, point_2s):
            self.play(LaggedStartMap(Indicate, group, rate_func=there_and_back, lag_ratio=0.7))
            self.wait()

    def show_final_result(self):
        result = OldTex('P(', '\\text{Score} = %s' % self.score, ')', '=', *self.final_result_rhs_tex)
        result.set_color_by_tex_to_color_map({'0.8': GREEN, '0.2': RED, 'Score': YELLOW})
...
class ComputeProbabilityOfOneRight(ComputeProbabilityOfOneWrong):
    CONFIG = {'score': 1, 'final_result_rhs_tex': ['3', '(0.8)', '(0.2)', '^2', '=', '0.096'], 'default_bool': False, 'default_p': '0.2', 'default_q': '0.8'}
...
class ShowFullDistribution(Scene):
    ...
    def add_scores_one_and_two(self):
        scores = VGroup(OldTex('P(', '\\text{Score} = 0', ')', '=', '(0.2)', '^3', '=', '0.008'), OldTex('P(', '\\text{Score} = 1', ')', '=', '3', '(0.8)', '(0.2)', '^2', '=', '0.096'), OldTex('P(', '\\text{Score} = 2', ')', '=', '3', '(0.8)', '^2', '(0.2)', '=', '0.384'), OldTex('P(', '\\text{Score} = 3', ')', '=', '(0.8)', '^3', '=', '0.512'))
        scores.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        scores.shift(MED_LARGE_BUFF * UP)
        scores.to_edge(LEFT)
        for score in scores:
            score.set_color_by_tex_to_color_map({'0.8': GREEN, '0.2': RED})
    ...
    def add_edge_score(self, index, vect, q_bool):
        ...
        self.play(LaggedStartMap(FadeIn, group, run_time=2, lag_ratio=0.7))
        self.wait(2)
    ...
    def show_bar_chart(self):
        ...
        for term, bar in zip(new_p_terms, chart.bars):
            ...
            term.next_to(bar, UP)
        self.play(ReplacementTransform(value_mobs, chart.bars, lag_ratio=0.5, run_time=2))
        self.play(LaggedStartMap(FadeIn, VGroup(*it.chain(*[submob for submob in chart if submob is not chart.bars]))), Transform(p_terms, new_p_terms), FadeOut(to_fade))
        self.wait(2)
    ...
    def compare_to_binomial_pattern(self):
        ...
        for num, bar in zip(nums, bars):
            num.next_to(bar, UP)
        bars_copy = bars.copy()
        self.play(LaggedStartMap(FadeIn, bars), LaggedStartMap(FadeIn, nums))
        self.wait(2)
...
class ProbablyWrong(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says('Probably wrong!', run_time=1)
...
class ShowTrueDistribution(PiCreatureScene):
    ...
    def show_distributions(self):
        ...
        for bar, old_bar in zip(bars, old_bars):
            for mob, vect in ((bar, RIGHT), (old_bar, LEFT)):
                mob.generate_target()
                mob.target.do_about_point(mob.get_corner(DOWN + vect), mob.target.stretch, 0.5, 0)
    ...
    def imagine_score_0(self):
        ...
        bar_rect = SurroundingRectangle(VGroup(self.bar_chart.bars[0], self.old_bars[0], self.bar_chart.bar_labels[0]))
    ...
    def get_angry(self):
        ...
        self.wait(2)
        self.play(PiCreatureSays(randy, "It's not representative!", target_mode='pleading', bubble_config={'fill_opacity': 1}))
        self.wait(2)
...
class TeacherAssessingLiklihoodOfZero(TeacherStudentsScene):
    ...
    def teacher_reacts(self):
        ...
        self.teacher_says('Highly unlikely', target_mode='sassy', added_anims=[MoveToTarget(ind_group)], run_time=2)
        self.play(randy.change, 'sad')
        self.wait(2)
        self.play(RemovePiCreatureBubble(self.teacher, target_mode='guilty'), PiCreatureSays(randy, 'Wait!', target_mode='surprised'), run_time=1)
        self.wait(1)
...
class CorrelationsWith35Percent(ThousandPossibleQuizzes):
    ...
    def add_top_calculation(self):
        equation = VGroup(get_probability_of_slot_group(3 * [False]), OldTex('='), get_probability_of_slot_group([False]), get_probability_of_slot_group(2 * [False], [False]), get_probability_of_slot_group(3 * [False], 2 * [False]))
    ...
    def show_first_split(self):
        ...
        self.wait()
        self.play(top_part.fade, 0.8, Transform(brace, term_brace), prop.next_to, term_brace, DOWN)
        self.wait()
...
class WeighingIndependenceAssumption(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(randy.change, 'raise_right_hand', randy.look, UP + RIGHT)
        self.wait(2)
...
class NameBinomial(Scene):
    CONFIG = {'flip_indices': [0, 2, 4, 5, 6, 7]}
    ...
    def name_distribution(self):
        ...
        self.play(Write(formula))
        self.wait()
        self.play(formula.scale, 0.7, formula.next_to, charts, DOWN)
        self.chart = charts[1]
    ...
    def add_quiz_questions(self):
        ...
        flipped_arrows, faded_crosses, full_checks = [VGroup(*[group[i] for i in flip_indices]) for group in (arrows, crosses, checkmarks)]
        faded_checkmarks = VGroup(*[m for m in checkmarks if m not in full_checks])
        self.play(*[LaggedStartMap(Write, mob, run_time=3, lag_ratio=0.3) for mob in full_group])
        self.wait()
        self.play(LaggedStartMap(Rotate, flipped_arrows, angle=np.pi, in_place=True, run_time=2, lag_ratio=0.5), faded_crosses.set_fill, None, 0.5, faded_checkmarks.set_fill, None, 0.5)
        self.wait()
    ...
    def change_to_gender(self):
        ...
        girls, boys = [VGroup(*[template.copy().move_to(mob) for mob in group]) for template, group in [(female, self.checkmarks), (male, self.crosses)]]
    ...
    def change_bar_chart_for_gender_example(self):
        ...
        self.play(Transform(checkmark, female), Transform(p_mob, new_p_mob))
        self.play(self.chart.change_bar_values, values)
        self.wait()

    def point_out_example_input(self):
        boy_girl_groups = VGroup(*[VGroup(boy, girl) for boy, girl in zip(self.boys, self.girls)])
        girl_rects = VGroup(*[SurroundingRectangle(boy_girl_groups[i], color=MAROON_B, buff=SMALL_BUFF) for i in sorted(self.flip_indices)])
        chart = self.chart
        n_girls = len(girl_rects)
        chart_rect = SurroundingRectangle(VGroup(chart.bars[n_girls], chart.bar_labels[n_girls]), buff=SMALL_BUFF)
        ...
        self.play(LaggedStartMap(ShowCreation, girl_rects, run_time=2, lag_ratio=0.5))
        self.wait()
        self.play(Write(prob))
        self.play(LaggedStartMap(Indicate, girls, run_time=3, lag_ratio=0.3, rate_func=there_and_back))
    ...
    def write_probability_of_girl(self):
        ...
        probability_copies = VGroup(*[probability.copy().scale(0.7).next_to(girl, LEFT, MED_LARGE_BUFF) for girl in self.girls])
        self.play(FocusOn(probability))
        self.play(Indicate(probability[-1]))
        self.wait()
        self.play(ReplacementTransform(VGroup(probability.copy()), probability_copies), FadeOut(self.children_brace), FadeOut(self.n_children_words))
        self.wait()
...
class CycleThroughPatterns(NameBinomial):
    CONFIG = {'n_patterns_shown': 100, 'pattern_scale_value': 2.7, 'n': 10, 'k': 6}

    def construct(self):
        n = self.n
        k = self.k
        question = OldTexText('How many patterns have \\\\ %d ' % k, '$\\female$', ' and %d ' % (n - k), '$\\male$', '?', arg_separator='')
        ...
        patterns = VGroup(*[self.get_pattern(indicies) for indicies in shown_combinations])
        patterns.to_edge(DOWN, buff=LARGE_BUFF)
        pattern = patterns[0]
        self.add(pattern)
        for new_pattern in ProgressDisplay(patterns[1:]):
            self.play(*[Transform(getattr(pattern, attr), getattr(new_pattern, attr), path_arc=np.pi) for attr in ('boys', 'girls')])
...
class Compute6of10GirlsProbability(CycleThroughPatterns):
    ...
    def write_n_choose_k(self):
        ...
        computation = OldTex('=\\frac{%s}{%s}' % ('\\cdot '.join(map(str, list(range(10, 4, -1)))), '\\cdot '.join(map(str, list(range(1, 7))))))
        ...
        self.wait(2)
        self.play(ten_choose_six.next_to, computation.copy(), LEFT, Write(VGroup(computation, rhs)))
        self.wait()
...
class ProbabilityOfAGivenBoyGirlPattern(CycleThroughPatterns):
    ...
    def write_example_probability(self):
        ...
        self.wait()
        self.play(ReplacementTransform(pattern.copy(), factored, run_time=1.5))
        self.wait(2)
        self.play(ReplacementTransform(factored, factored_in_nums, run_time=2, lag_ratio=0.5))
        self.wait(2)
        for group, tex in ((gp_nums, '0.49'), (bp_nums, '0.51')):
            ...
            self.play(ReplacementTransform(group.copy(), VGroup(VGroup(*part[:-1]))), Write(part[-1]))
            self.wait()
    ...
    def write_total_probability(self):
        ...
        self.play(Write(p_tex, run_time=2), self.final_probability.next_to, ten_choose_six.target, RIGHT)
        self.play(MoveToTarget(ten_choose_six))
        self.wait()

class CycleThroughPatternsForThree(CycleThroughPatterns):
    CONFIG = {'k': 3, 'n_patterns_shown': 20}

class GeneralBinomialDistributionValues(Scene):
    CONFIG = {'n': 10, 'alt_n': 8, 'p': 0.49}
    ...
    def add_chart(self):
        ...
        chart.to_edge(LEFT)
        full_probability = self.get_probability_expression('10', 'k', '(0.49)', '(0.51)')
    ...
    def show_a_few_values(self):
        ...
        for i, bar in enumerate(chart.bars):
            prob = self.get_probability_expression('10', str(i), '(0.49)', '(0.51)', full=False)
        ...
        self.wait()
        self.play(LaggedStartMap(FadeIn, self.full_probability, run_time=4, lag_ratio=0.5))
        self.wait()
        last_k = 6
        for k in (3, 8, 5, 9, 6):
            self.play(Transform(shown_prob, probabilities[k], path_arc=-np.pi / 6 if k > last_k else np.pi / 6))
            self.wait(2)
            last_k = k
    ...
    def compare_to_pascal_row(self):
        ...
        self.play(last_row.set_color_by_gradient, BLUE, YELLOW, Write(ten_choose_ks, run_time=2))
        self.wait()
        self.play(ApplyWave(self.chart.bars, direction=UP))
        self.play(FocusOn(last_row))
        self.play(LaggedStartMap(ApplyMethod, last_row, lambda m: (m.scale, 1.2), rate_func=there_and_back))
        self.wait()
    ...
    def mention_center_concentration(self):
        ...
        self.play(GrowFromCenter(brace), FadeIn(words))
        self.wait(2)
        self.play(bars.restore, *list(map(FadeOut, [brace, words, self.pascals_triangle, self.ten_choose_ks])))
    ...
    def generalize(self):
        ...
        alt_probs = [self.get_probability_expression('n', 'k', '(0.49)', '(0.51)'), self.get_probability_expression('n', 'k', 'p', '(1-p)')]
        for prob in alt_probs:
            prob.move_to(self.full_probability)
        self.play(FocusOn(self.full_probability.get_part_by_tex('choose')))
        self.play(ReplacementTransform(self.chart, alt_chart), Transform(self.full_probability, alt_probs[0]))
    ...
    def play_with_p_value(self):
        ...
        triangle = RegularPolygon(n=3, start_angle=-np.pi / 2, fill_color=MAROON_B, fill_opacity=1, stroke_width=0)
        triangle.set_height(0.25)
        ...
        for new_p in new_p_values:
            ...
            self.play(self.chart.change_bar_values, values, triangle.move_to, interval.number_to_point(p), DOWN)
            self.wait()
    ...
    def get_probability_expression(self, n='n', k='k', p='p', q='(1-p)', full=True):
        ...
        if full:
            args += ['P(', '\\# \\text{Girls}', '=', k, ')', '=']
        args += ['{%s \\choose %s}' % (n, k), p, '^%s' % k, q, '^{%s' % n, '-', '%s}' % k]
        result = OldTex(*args, arg_separator='')
        color_map = {'Girls': MAROON_B, n: WHITE, k: YELLOW, p: MAROON_B, q: BLUE}
...
class PointOutSimplicityOfFormula(TeacherStudentsScene, GeneralBinomialDistributionValues):
    ...
    def construct(self):
        ...
        self.wait()
        self.student_says('Simpler than I feared', target_mode='hooray', index=0, added_anims=[prob.to_corner, UP + RIGHT])
        self.wait()
        self.teacher_says('Due to \\\\ independence')
        self.wait(2)

class CorrectForDependence(NameBinomial):
    CONFIG = {'flip_indices': [3, 6, 8]}
    ...
    def adjust_chart(self):
        ...
        for value, alt_value, bar in zip(values, alt_values, bars):
            ...
            arrows.add(arrow)
        self.play(MoveToTarget(old_bars), MoveToTarget(bars))
        self.wait()
        self.play(*list(map(ShowCreation, arrows)))
        self.play(chart.change_bar_values, alt_values)

    def get_arrow_flip_anims(self, indices):
        checkmarks, arrows, crosses = movers = [VGroup(*[group[i] for i in range(len(group)) if i in indices]) for group in (self.checkmarks, self.arrows, self.crosses)]
        for arrow in arrows:
...
class PermuteQuizQuestions(Scene):
    ...
    def construct(self):
        quiz = get_quiz("Define ``Brachistochrone''", "Define ``Tautochrone''", "Define ``Cycloid''")
...
class AssumeOrderDoesntMatter(Scene):
    ...
    def show_equality(self):
        n = 3
        prob_groups = VGroup(*[VGroup(*list(map(get_probability_of_slot_group, [t for t in it.product(*[[True, False]] * n) if sum(t) == k]))) for k in range(n + 1)])
        ...
        self.wait(2)
        self.play(FadeIn(VGroup(prob_groups[0], *prob_groups[2:]), run_time=3, lag_ratio=0.5))
        self.wait()
    ...
    def coming_soon(self):
        self.play(LaggedStartMap(ApplyMethod, self.assumption_group, lambda m: (m.shift, FRAME_HEIGHT * DOWN), remover=True), ApplyMethod(self.question.center, rate_func=squish_rate_func(smooth, 0.5, 1), run_time=2))
...
class CompareTwoSituations(PiCreatureScene):
    ...
    def construct(self):
        randy = self.randy
        top_left, top_right = screens = [ScreenRectangle(height=3).to_corner(vect) for vect in (UP + LEFT, UP + RIGHT)]
...
class SkepticalOfDistributions(TeacherStudentsScene):
    ...
    def show_binomial(self):
        ...
        self.play(Write(title, run_time=1), FadeIn(binomial, run_time=1, lag_ratio=0.5), self.teacher.change, 'raise_right_hand')
    ...
    def show_alternate_distributions(self):
        ...
        VGroup(poisson, poisson.title).next_to(self.students, UP, LARGE_BUFF).shift(RIGHT)
        gaussian = self.get_gaussian()
        VGroup(gaussian, gaussian.title).next_to(poisson, RIGHT, LARGE_BUFF)
        self.play(FadeIn(poisson, lag_ratio=0.5), RemovePiCreatureBubble(self.students[0]), self.teacher.change, 'raise_right_hand', self.binomial.scale, 0.5, self.binomial.to_corner, UP + LEFT)
        self.play(Write(poisson.title, run_time=1))
        self.play(FadeIn(gaussian, lag_ratio=0.5))
        self.play(Write(gaussian.title, run_time=1))
        self.wait(2)
        self.play_student_changes(*['sassy'] * 3, added_anims=[self.teacher.change, 'plain'])
        self.wait(2)
    ...
    def emphasize_underweighted_tails(self):
        ...
        for arrows in (poisson_arrows, gaussian_arrows):
            self.play(ShowCreation(arrows, lag_ratio=0.5, run_time=2), *[ApplyMethod(pi.change, 'thinking', arrows) for pi in self.pi_creatures])
            self.wait()
        self.wait(2)
    ...
    def get_poisson(self):
        ...
        chart.set_height(self.chart_height)
        title = OldTexText('Poisson distribution \\\\', '$e^{-\\lambda}\\frac{\\lambda^k}{k!}$')
        title.scale(0.75)
    ...
    def get_gaussian(self):
        ...
        graph = FunctionGraph(lambda x: 5 * np.exp(-x ** 2), mark_paths_closed=True, fill_color=BLUE_E, fill_opacity=1, stroke_color=BLUE)
        graph.set_width(axes.get_width())
        graph.move_to(axes[0], DOWN)
        title = OldTexText('Gaussian distribution \\\\ ', '$\\frac{1}{\\sqrt{2\\pi \\sigma^2}} e^{-\\frac{(x-\\mu)^2}{2\\sigma^2}}$')
        title.scale(0.75)
...
class Thumbnail(DangerInProbability):
    ...
    def construct(self):
        ...
        values *= 2
        chart = BarChart(values=values, label_y_axis=False, width=FRAME_WIDTH - 3, height=1.5 * FRAME_Y_RADIUS)
        chart.to_edge(DOWN)
        self.add(chart)
        warning = self.get_warning_sign()
