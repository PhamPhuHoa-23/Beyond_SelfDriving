...
class BayesClassicExampleOpeningQuote(OpeningQuote):
    CONFIG = {'quote': ['When faced with a difficult question, we often answer an easier one instead, usually without noticing the substitution.'], 'author': 'Daniel Kahneman'}
...
class Introduction(TeacherStudentsScene):
    ...
    def hold_up_example(self):
        ...
        self.play(FadeIn(everyone), example.scale, 0.5, example.next_to, self.teacher.get_corner(UP + LEFT), UP)
        self.wait(2)
    ...
    def comment_on_crazy_results(self):
        ...
        self.teacher_says("Who doesn't love \\\\ crazy results?", target_mode='hooray', added_anims=[disease_group.to_corner, UP + LEFT])
        self.wait(2)
    ...
    def put_it_first(self):
        ...
        self.play(disease_group.restore, disease_group.to_edge, LEFT, RemovePiCreatureBubble(self.teacher, target_mode='hesitant'))
        self.play_student_changes(*['pondering'] * 3, look_at=disease_group)
        ...
        bayes_to_intuition = VGroup(*list(map(TexText, ['Bayes', '$\\leftrightarrow$', 'Intuition'])))
        ...
        self.wait(2)
        self.intuitive_examples = VGroup(examples, brace, bayes_to_intuition, check)

    def swap_example_order(self):
        intuitive_examples = self.intuitive_examples
        disease_group = VGroup(self.example, self.bayes_to_intuition)
        self.play(disease_group.next_to, self.teacher.get_corner(UP + LEFT), UP, disease_group.shift, LEFT, intuitive_examples.scale, 0.7, intuitive_examples.to_corner, UP + LEFT, self.teacher.change_mode, 'sassy')
    ...
    def other_culprit(self):
        ...
        self.play_student_changes('erm', 'confused', 'hesitant', added_anims=[self.teacher.change_mode, 'happy'])
        self.wait(3)
    ...
    def get_poker_example(self):
        ...
        probability = OldTex('P(', '\\text{Flush}', '|', '\\text{High bet}', ')')
    ...
    def get_music_example(self):
        ...
        musician.left_arm_range = [0.36, 0.45]
        ...
        probability = OldTex('P(', '\\text{Suck }', '|', '\\text{ Good review}', ')')
...
class OneInOneThousandHaveDisease(Scene):
    ...
    def construct(self):
        ...
        title.to_edge(UP)
        creature = PiCreature()
        all_creatures = VGroup(*[VGroup(*[creature.copy() for y in range(25)]).arrange(DOWN, SMALL_BUFF) for x in range(40)]).arrange(RIGHT, SMALL_BUFF)
        ...
        self.play(Write(title), LaggedStartMap(FadeIn, all_creatures, run_time=3))
        self.wait()
...
class TestNonDiseaseCase(TestScene):
    ...
    def construct(self):
        ...
        accuracy.next_to(VGroup(randy, result), UP)
        accuracy.to_edge(UP)
        all_creatures = VGroup(*[VGroup(*[randy.copy() for y in range(10)]).arrange(DOWN) for y in range(10)]).arrange(RIGHT)
        ...
        self.play(Write(result[1]))
        self.play(Blink(randy))
        self.play(ReplacementTransform(randy, all_creatures[0][0]), LaggedStartMap(FadeIn, all_creatures, run_time=2), FadeOut(result))
        self.play(ShowCreation(rect))
        self.play(last_guy.set_height, 2, last_guy.next_to, all_creatures, RIGHT)
        ...
        arrow = Arrow(false_positive.get_bottom(), result[1].get_top(), buff=SMALL_BUFF)
        self.play(FadeIn(result))
        self.play(FadeIn(false_positive), ShowCreation(arrow), last_guy.change, 'confused', result)
        for x in range(2):
...
class ReceivePositiveResults(TestScene):
    ...
    def construct(self):
        ...
        self.wait(2)
        self.play(randy.change, 'pleading', accuracy, Write(accuracy, run_time=1))
        self.wait()
...
class AskAboutRephrasingQuestion(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says('What if we rephrased \\\\ the question?', run_time=1)
        self.wait(3)

class RephraseQuestion(Scene):

    def construct(self):
        words = VGroup(*list(map(TexText, ['1 in $1{,000}$ chance \\\\ of having disease', '1 in $100$ \\\\ false positive rate.', '\\underline{\\phantom{1 in 10}} chance \\\\\n                of having disease \\\\\n                after testing positive.\n            '])))
        ...
        self.add(words[0])
        self.play(LaggedStartMap(FadeIn, prior), ShowCreation(prior_arrow), run_time=1)
        self.wait()
        self.play(FadeIn(words[1]))
        self.wait()
        self.play(FadeIn(words[2]))
        self.play(LaggedStartMap(FadeIn, posterior), ShowCreation(posterior_arrow), run_time=1)
        self.wait(2)
...
class TryUnitSquareVisual(SampleSpaceScene):
    ...
    def add_prior_division(self):
        ...
        initial_labels, final_labels = [VGroup(OldTex('P(\\text{Disease})', s1), OldTex('P(\\text{Not disease})', s2)).scale(0.7) for s1, s2 in (('', ''), ('= 0.001', '= 0.999'))]
    ...
    def add_conditional_divisions(self):
        ...
        top_label = OldTex('P(', '+', '|', '\\text{Disease}', ')', '=', '1')
        top_label.scale(0.7)
        ...
        bottom_label = OldTex('P(', '+', '|', '\\text{Not disease}', ')', '=', '1')
...
class ShowRestrictedSpace(Scene):
    CONFIG = {'n_rows': 25, 'n_cols': 40, 'n_false_positives': 10, 'false_positive_color': YELLOW_E}
    ...
    def add_all_creatures(self):
        ...
        self.play(sick_one.set_height, 2, sick_one.next_to, sick_one_words, DOWN, sick_one.to_edge, RIGHT)
        self.wait()
        self.play(sick_one.restore)
        self.play(Write(healthy_words), LaggedStartMap(ApplyMethod, healthy_creatures, lambda m: (m.shift, MED_SMALL_BUFF * UP), rate_func=there_and_back, lag_ratio=0.2))
        self.wait()
    ...
    def show_accurate_positive_result(self):
        equation = OldTex('P(', '\\text{Test positive }', '|', '\\text{ sick}', ')', '=', '100\\%')
    ...
    def show_false_positive_conditional(self):
        equation = OldTex('P(', '\\text{Test positive }', '|', '\\text{ healthy}', ')', '=', '1\\%')
        equation.set_color_by_tex('positive', YELLOW)
    ...
    def show_false_positive_individuals(self):
        ...
        false_positives = VGroup(*all_creatures[-1][1:1 + self.n_false_positives])
        ...
        self.play(GrowFromCenter(brace), LaggedStartMap(ApplyMethod, false_positives, lambda pi: (pi.set_color, self.false_positive_color), run_time=1))
    ...
    def fade_out_negative_result_individuals(self):
        to_fade = VGroup(self.healthy_creatures, self.healthy_conditional, self.sick_one_label, self.healthy_ones_label)
        ...
        words.to_edge(UP)
        arrows = [Arrow(words.get_bottom(), movers.target[i].get_top()) for i in (0, -1)]
        self.play(FadeOut(to_fade))
        self.play(MoveToTarget(movers), ReplacementTransform(self.false_positives_brace, brace), self.false_positives_words.next_to, brace, DOWN)
    ...
    def show_posterior_probability(self):
        posterior = OldTex('P(', '\\text{Sick }', '|', '\\text{ Positive test result}', ')', '\\approx \\frac{1}{11}', '\\approx 9\\%')
    ...
    def contrast_with_prior(self):
        prior = OldTex('P(', '\\text{Sick}', ')', '= 0.1\\%')
        prior.set_color_by_tex('Sick', SICKLY_GREEN)
        prior.move_to(self.posterior, UP + RIGHT)
        self.revert_to_original_skipping_status()
        self.play(Write(prior, run_time=1), self.posterior.shift, DOWN)
        arrow = Arrow(prior.get_right(), self.posterior.get_right(), path_arc=-np.pi)
    ...
    def get_all_creatures(self):
        creature = PiCreature()
        all_creatures = VGroup(*[VGroup(*[creature.copy() for y in range(self.n_rows)]).arrange(DOWN, SMALL_BUFF) for x in range(self.n_cols)]).arrange(RIGHT, SMALL_BUFF)
...
class DepressingForMedicalTestDesigners(TestScene):
    ...
    def show_99_percent_accuracy(self):
        ...
        checks = VGroup(*[VGroup(*[OldTex('\\checkmark').set_color(GREEN) for y in range(10)]).arrange(DOWN) for x in range(10)]).arrange(RIGHT)
        cross = OldTex('\\times')
    ...
    def reject_test(self):
        ...
        self.wait()
        self.say("Whatever, I'm 91\\% \\\\ sure that's wrong", target_mode='shruggie')
        self.wait(2)
...
class HowMuchCanYouChangeThisPrior(ShowRestrictedSpace, PiCreatureScene):
    ...
    def show_subgroups(self):
        subgroups = VGroup(*[VGroup(*it.chain(self.pi_creatures[i:i + 5], self.pi_creatures[i + 25:i + 25 + 5])) for i in range(0, 1000) if i % 5 == 0 and i / 25 % 2 == 0])
        ...
        for subgroup in subgroups.target:
            subgroup.stretch_in_place(1.0 / width_factor, 0)
            subgroup.stretch_in_place(1.0 / height_factor, 1)
        self.wait()
        self.play(MoveToTarget(subgroups))
        subgroups.remove(special_group)
        rects = VGroup(*[SurroundingRectangle(group, buff=0, color=GREEN) for group in subgroups])
    ...
    def isolate_special_group(self):
        ...
        self.play(FadeOut(to_fade))
        self.play(FadeOut(self.special_rect), special_group.set_height, 6, special_group.center)
        self.wait()
...
class ShowTheFormula(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        formula = OldTex('P(', sick, '\\,|\\,', positive, ')', '=', '{\\quad P(', sick, '\\text{ and }', positive, ') \\quad', '\\over', 'P(', positive, ')}', '=', '{1/1{,}000', '\\over', '1/1{,}000', '+', '(999/1{,}000)(0.01)}')
        ...
        equals_indices = [formula.index_of_part(equals) for equals in equals_group]
        ...
        alt_numerator = OldTex('P(', sick, ')', 'P(', positive, '\\,|\\,', sick, ')')
        ...
        self.wait()
        self.play(Write(lhs), RemovePiCreatureBubble(self.students[1], target_mode='pondering'), self.teacher.change, 'raise_right_hand', self.students[0].change, 'pondering', self.students[2].change, 'pondering')
        self.wait()
        lhs_copy = lhs.copy()
        self.play(LaggedStartMap(FadeIn, initial_formula, lag_ratio=0.7), Animation(lhs_copy, remover=True))
        self.wait(2)
        ...
        self.wait(3)
        self.play(initial_formula.move_to, initial_formula.saved_state, FadeIn(VGroup(*number_fraction[:3])))
        self.wait(2)
        self.play(LaggedStartMap(FadeIn, VGroup(*number_fraction[3:]), run_time=3, lag_ratio=0.7))
        self.wait(2)
...
class SourceOfConfusion(Scene):
    ...
    def ask_question(self):
        question = OldTexText('Where do math and \\\\ intuition disagree?')
        question.to_corner(UP + LEFT)
        question_arrow = Arrow(question.get_bottom(), self.bayes_rule_words.get_top(), color=WHITE)
    ...
    def write_bayes_rule(self):
        ...
        rule = OldTex('P(', 'S', '|', '+', ')', '=', 'P(', 'S', ')', '{P(', '+', '|', 'S', ')', '\\over', 'P(', '+', ')}')
        ...
        self.play(ReplacementTransform(words_rect, rule_rect), rule.restore, run_time=2)
        self.wait(3)

    def shift_arrow(self):
        new_arrow = Arrow(self.question.get_bottom(), self.progression[0].get_top(), color=WHITE)
...
class StatisticsVsEmpathy(PiCreatureScene):
    ...
    def construct(self):
        ...
        priors = VGroup(*[OldTex('%.1f' % p + '\\%').move_to(ORIGIN, RIGHT) for p in np.arange(0.1, 2.0, 0.1)])
        priors.next_to(randy, UP + LEFT, LARGE_BUFF)
        prior = priors[0]
        prior.save_state()
        self.play(PiCreatureSays(morty, '1 in 1{,}000 people \\\\ have this disease.', look_at=randy.eyes))
        self.play(randy.change, 'pondering', morty.eyes)
        self.wait()
        self.play(Write(prior))
        self.wait()
        self.play(prior.scale, 0.1, prior.set_fill, None, 0, prior.move_to, randy.eyes)
        self.wait()
        self.play(PiCreatureBubbleIntroduction(randy, sick_group, target_mode='guilty', bubble_type=ThoughtBubble, content_introduction_class=FadeIn, look_at=sick_one), RemovePiCreatureBubble(morty))
        self.play(sick_one.change_mode, 'sick', sick_one.set_color, SICKLY_GREEN)
        self.wait()
        ...
        self.pi_creature_thinks(target_sick_group, target_mode='pleading')
        self.wait(2)
...
class PlaneCrashProbability(Scene):
    ...
    def construct(self):
        plane_prob = OldTex('P(\\text{Dying in a }', '\\text{plane}', '\\text{ crash})', '\\approx', '1/', '11{,}000{,}000')
        plane_prob.set_color_by_tex('plane', BLUE)
        car_prob = OldTex('P(\\text{Dying in a }', '\\text{car}', '\\text{ crash})', '\\approx', '1/', '5{,}000')
        car_prob.set_color_by_tex('car', YELLOW)
        plane_prob.shift(UP)
        car_prob.shift(plane_prob.get_part_by_tex('approx').get_center() - car_prob.get_part_by_tex('approx').get_center() + DOWN)
...
class IntroduceTelepathyExample(StatisticsVsEmpathy):
    ...
    def claim_mind_reading_powers(self):
        randy, morty = (self.randy, self.morty)
        self.play(PiCreatureSays(randy, 'I have the gift.', run_time=1, look_at=morty.eyes))
        self.wait()
        self.play(RemovePiCreatureBubble(randy, target_mode='happy', look_at=morty.eyes))
    ...
    def generate_random_number(self):
        ...
        for number in numbers:
            ...
            Scene.wait(self, 0.1)
            self.remove(number)
        self.play(ShowCreation(bubble), number.move_to, bubble.get_bubble_center(), DOWN + LEFT, morty.change, 'pondering')
        self.wait()
        morty.bubble = bubble
        self.number = number
    ...
    def guess_number_correctly(self):
        ...
        self.read_mind(randy, morty)
        self.play(PiCreatureSays(randy, number_copy, target_mode='hooray', look_at=morty.eyes))
        self.wait()

    def ask_about_chances(self):
        probability = OldTex('P(', '\\text{Telepath }', '|', '\\text{ Correct}', ')', '=', '???')
        ...
        self.wait()
        self.play(ReplacementTransform(self.title, VGroup(*it.chain(*probability))))
        self.wait()

    def say_you_probably_got_lucky(self):
        randy, morty = (self.randy, self.morty)
        self.play(PiCreatureSays(morty, 'You probably \\\\ got lucky.', target_mode='sassy', look_at=randy.eyes, bubble_config={'height': 3, 'width': 4}), RemovePiCreatureBubble(randy, target_mode='plain', look_at=morty.eyes))
        self.wait(2)
    ...
    def send_mind_waves(self, pi1, pi2):
        ...
        self.play(LaggedStartMap(ApplyMethod, arcs, lambda m: (m.restore,), lag_ratio=0.7))
        self.remove(arcs)
...
class CompareNumbersInBothExamples(Scene):
    ...
    def construct(self):
        ...
        for word, vect in (('Disease', LEFT), ('Telepathy', RIGHT)):
            ...
            title.to_edge(UP)
            titles.add(title)
        priors = VGroup(*[OldTex('P(', '\\text{%s}' % s, ')', '= 1/1{,}000}') for s in ('Sick', 'Powers')])
        likelihoods = VGroup(*[OldTex('P(', '\\text{%s}' % s1, '|', '\\text{Not }', '\\text{%s}' % s2, ')', '=', '1/100') for s1, s2 in [('+', 'Sick'), ('Correct', 'Powers')]])
        ...
        self.play(LaggedStartMap(FadeIn, titles, lag_ratio=0.7), *list(map(ShowCreation, [h_line, v_line])))
        self.wait()
...
class NonchalantReactionToPositiveTest(TestScene):
    ...
    def construct(self):
        ...
        words.scale(0.8)
        self.pi_creature_says(words, target_mode='shruggie', bubble_config={'direction': RIGHT, 'width': 6, 'height': 3}, content_introduction_class=FadeIn)
        self.wait(4)
...
class ExampleMeasuresDisbeliefInStatistics(Introduction):
    ...
    def write_new_theory(self):
        ...
        statistics_to_belief = OldTexText('Statistics ', '$\\leftrightarrow$', ' Belief')
        ...
        self.play(LaggedStartMap(FadeIn, statistics_to_belief), cross.move_to, arrow)
        self.play_student_changes(*['pondering'] * 3, look_at=statistics_to_belief)
        self.wait(3)
    ...
    def either_way(self):
        ...
        self.play(self.teacher.change_mode, 'raise_left_hand', self.teacher.look, UP + RIGHT, b_to_i.next_to, self.teacher, UP, b_to_i.to_edge, RIGHT, MED_SMALL_BUFF)
        self.wait(2)
        self.play(self.teacher.change_mode, 'raise_right_hand', self.teacher.look, UP + LEFT, s_to_b.next_to, self.teacher, UP, s_to_b.shift, LEFT, b_to_i.shift, 2 * UP)
        self.wait(2)

class AlwaysPictureTheSpaceOfPossibilities(PiCreatureScene):

    def construct(self):
        self.pi_creature_thinks('', bubble_config={'height': 4.5, 'width': 8})
        self.wait(3)
