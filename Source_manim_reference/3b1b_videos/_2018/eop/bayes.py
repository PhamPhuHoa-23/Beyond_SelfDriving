...
class BayesOpeningQuote(OpeningQuote):
    CONFIG = {'quote': ['Inside every non-Bayesian there \\\\ is a Bayesian struggling to get out.'], 'author': 'Dennis V. Lindley'}

class IntroducePokerHand(PiCreatureScene, SampleSpaceScene):
    CONFIG = {'community_cards_center': 1.5 * DOWN, 'community_card_values': ['10S', 'QH', 'AH', '2C', '5H'], 'your_hand_values': ['JS', 'KC']}
    ...
    def add_cards(self):
        ...
        community_cards = VGroup(*list(map(PlayingCard, self.community_card_values)))
    ...
    def indicate_straight(self):
        ...
        selected_community_cards.submobjects.sort(key=lambda c: c.numerical_value)
        selected_community_cards.save_state()
        for card in selected_community_cards:
            card.generate_target()
        straight_cards = VGroup(*it.chain(you.hand.target, [c.target for c in selected_community_cards]))
        ...
        self.play(LaggedStartMap(MoveToTarget, selected_community_cards, run_time=1.5))
        self.play(MoveToTarget(you.hand))
        self.play(LaggedStartMap(ApplyMethod, straight_cards, lambda m: (m.set_color, YELLOW), rate_func=there_and_back, run_time=1.5, lag_ratio=0.5, remover=True))
        self.play(you.change, 'hooray', straight_cards)
        self.wait(2)
        self.play(selected_community_cards.restore, you.hand.restore, you.change_mode, 'happy')
        self.wait()
    ...
    def show_flush_potential(self):
        ...
        self.play(her.change, 'happy', her.glasses.restore)
        self.pi_creatures.remove(her)
        new_suit_pairs = [('clubs', 'diamonds'), ('diamonds', 'spades'), ('spades', 'clubs'), ('hearts', 'hearts')]
        ...
        self.play(heart_cards.restore, her.hand.restore, you.change_mode, 'pondering')
        self.q_marks = q_marks

    def compute_flush_probability(self):
        you, her = (self.you, self.her)
        equation = OldTex('{ {10 \\choose 2}', '\\over', '{45 \\choose 2} }', '=', '{45 \\over 990}', '\\approx', '4.5\\%')
        ...
        num_hearts_arrow = Arrow(num_hearts.get_bottom(), ten.get_right(), color=RED, buff=SMALL_BUFF)
        ...
        num_cards_arrow = Arrow(num_cards, fourty_five, color=BLUE, buff=SMALL_BUFF)
        self.play(LaggedStartMap(FadeIn, equation))
        self.wait(2)
        self.play(FadeIn(num_hearts), ShowCreation(num_hearts_arrow), ten.set_color, RED)
        self.play(FadeIn(num_cards), ShowCreation(num_cards_arrow), fourty_five.set_color, BLUE)
        self.wait(3)
        equation.remove(percentage)
        self.play(*list(map(FadeOut, [equation, num_hearts, num_hearts_arrow, num_cards, num_cards_arrow])))
    ...
    def show_flush_sample_space(self):
        ...
        p = 1.0 / 22
        sample_space.divide_horizontally(p, colors=[SuitSymbol.CONFIG['red'], BLUE_E])
        braces, labels = sample_space.get_side_braces_and_labels([percentage.get_tex(), '95.5\\%'])
        top_label, bottom_label = labels
        self.play(FadeIn(sample_space), ReplacementTransform(percentage, top_label))
    ...
    def talk_through_sample_space(self):
        ...
        flush_hands, non_flush_hands = hand_lists = [[self.get_hand(her, keys) for keys in key_list] for key_list in [[('3H', '8H'), ('4H', '5H'), ('JH', 'KH')], [('AC', '6D'), ('3D', '6S'), ('JH', '4C')]]]
    ...
    def place_high_bet(self):
        you, her = (self.you, self.her)
        pre_money = VGroup(*[VGroup(*[OldTex('\\$') for x in range(10)]).arrange(RIGHT, buff=SMALL_BUFF) for y in range(4)]).arrange(UP, buff=SMALL_BUFF)
        ...
        self.play(LaggedStartMap(ApplyMethod, money, lambda m: (m.restore,), run_time=5))
    ...
    def move_community_cards_out_of_the_way(self):
        ...
        cards.target.arrange(RIGHT, buff=-cards[0].get_width() + MED_SMALL_BUFF)
        cards.target.move_to(self.deck)
        cards.target.to_edge(LEFT)
        self.sample_space.add_braces_and_labels()
        self.play(self.deck.scale, 0.7, self.deck.next_to, cards.target, UP, self.deck.to_edge, LEFT, self.sample_space.shift, 3 * DOWN, MoveToTarget(cards))
    ...
    def name_bayes_rule(self):
        ...
        words = OldTexText('Maybe she really \\\\ does have a flush $\\dots$', alignment='')
        words.scale(0.7)
...
class HowDoesPokerWork(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('Wait, how does \\\\ poker work again?', target_mode='confused', run_time=1)
...
class UpdatePokerPrior(SampleSpaceScene):
    CONFIG = {'double_heart_template': 'HH', 'cash_string': '\\$\\$\\$'}
    ...
    def add_sample_space(self):
        ...
        self.play(LaggedStartMap(FadeIn, sample_space), Write(braces_and_labels))
        self.wait()
    ...
    def add_top_conditionals(self):
        ...
        explanation = OldTexText('Probability of', 'high bet', 'given', 'flush')
        ...
        self.play(FadeIn(top_part.vertical_parts), FadeIn(label), GrowFromCenter(brace))
    ...
    def add_bottom_conditionals(self):
        ...
        explanation = OldTexText('Probability of', 'high bet', 'given', 'no flush')
        explanation.set_color_by_tex('high bet', GREEN)
        ...
        self.play(her.change_mode, 'shruggie', MaintainPositionRelativeTo(her.glasses, her.eyes))
        self.wait()
        self.play(*[ReplacementTransform(VGroup(*label[i1:i2]).copy(), VGroup(explanation[j]), run_time=2, rate_func=squish_rate_func(smooth, a, a + 0.5)) for a, (i1, i2, j) in zip(np.linspace(0, 0.5, 4), [(0, 1, 0), (1, 2, 1), (2, 3, 2), (3, 6, 3)])])
        self.wait()
    ...
    def ask_where_conditionals_come_from(self):
        ...
        numbers.save_state()
        arrows = VGroup(*[Arrow(numbers_word.get_left(), num.get_right(), buff=2 * SMALL_BUFF) for num in numbers])
        questions = VGroup(*list(map(TexText, ['Does she bluff?', 'How much does she have?', 'Does she take risks?', "What's her model of me?", '\\vdots'])))
        ...
        self.play(randy.change_mode, 'confused', ShowCreation(bubble), Write(words, run_time=2))
    ...
    def vary_conditionals(self):
        randy = self.randy
        rects = VGroup(*[SurroundingRectangle(VGroup(explanation), buff=SMALL_BUFF) for explanation, rhs in zip([self.top_explanation, self.bottom_explanation], [self.top_conditional_rhs, self.bottom_conditional_rhs])])
        new_conditionals = [(0.91, 0.4), (0.83, 0.1), (0.99, 0.2), (0.97, 0.3)]
        self.play(*list(map(ShowCreation, rects)))
        self.play(FadeOut(rects))
        for i, value in enumerate(it.chain(*new_conditionals)):
            self.play(randy.look_at, rects[i % 2], *self.get_conditional_change_anims(i % 2, value))
            if i % 2 == 1:
                self.wait()
        self.play(FadeOut(randy))

    def show_restricted_space(self):
        high_bet_space, low_bet_space = [VGroup(*[self.sample_space.horizontal_parts[i].vertical_parts[j] for i in range(2)]) for j in range(2)]
        ...
        arrows = VGroup(*[Arrow(words.get_left(), submob.get_edge_center(vect), color=submob.get_color()) for submob, vect in zip(high_bet_space, [DOWN, RIGHT])])
    ...
    def write_P_flush_given_bet(self):
        posterior_tex = OldTex('P(', self.double_heart_template, '|', self.cash_string, ')')
        ...
        self.wait()
        self.play(*it.chain(list(map(Write, [frac_line, plus])), list(map(MoveToTarget, rects[1:]))))
        self.wait(3)
    ...
    def reshape_rectangles(self):
        ...
        braces, labels = self.get_posterior_rectangle_braces_and_labels(post_rects, [self.posterior_tex.copy()])
        height_rect = SurroundingRectangle(braces)
        self.play(ReplacementTransform(prior_rects.copy(), post_rects, run_time=2))
        self.wait(2)
    ...
    def compare_prior_to_posterior(self):
        ...
        prior_rect, post_rect = [SurroundingRectangle(tex, stroke_width=2) for tex in [prior_tex, post_tex]]
        post_words = OldTexText('Posterior', 'probability')
        post_words.scale(0.8)
        post_words.to_corner(UP + RIGHT)
        post_arrow = Arrow(post_words[0].get_bottom(), post_tex.get_top(), color=WHITE)
    ...
    def preview_tweaks(self):
        ...
        new_value_lists = [(0.85, 0.1, 0.11), (0.97, 0.3, 1.0 / 22)]
        for new_values in new_value_lists:
            for i, value in zip(list(range(2)), new_values):
                self.play(*self.get_conditional_change_anims(i, value, post_rects))
    ...
    def tweak_non_flush_case(self):
        ...
        prior_rects = self.get_prior_rectangles()
        risk_averse_words = OldTexText('Suppose risk \\\\ averse \\dots')
        risk_averse_words.scale(0.7)
        risk_averse_words.next_to(her, DOWN)
        risk_averse_words.shift_onto_screen()
        arrows = VGroup(*[Arrow(ORIGIN, LEFT, tip_length=SMALL_BUFF) for x in range(3)])
        arrows.arrange(DOWN)
        ...
        self.play(*it.chain(self.get_conditional_change_anims(1, 0.1, post_rects), [Animation(arrows)]), run_time=3)
        ...
        self.wait()
        self.play(FadeOut(risk_averse_words), *self.get_conditional_change_anims(1, 0.3, post_rects), run_time=2)

    def tweak_flush_case(self):
        her = self.her
        post_rects = self.post_rects
        self.play(her.change_mode, 'erm', Animation(her.glasses))
        self.play(*self.get_conditional_change_anims(0, 0.47, post_rects), run_time=3)
        self.wait(3)
    ...
    def tweak_prior(self):
        ...
        self.wait(3)
        self.play(*self.get_prior_change_anims(1.0 / 22, post_rects), run_time=2)
        self.play(*list(map(FadeOut, [her, her.glasses])))
    ...
    def compute_posterior(self):
        ...
        fraction = OldTex('{(0.045)', '(0.97)', '\\over', '(0.995)', '(0.3)', '+', '(0.045)', '(0.97)}')
        products = [VGroup(*[fraction.get_parts_by_tex(tex)[i] for tex in tex_list]) for i, tex_list in [(0, ['0.045', '0.97']), (0, ['0.995', '0.3']), (1, ['0.045', '0.97'])]]
        for i in (0, 2):
        ...
        arrow_kwargs = {'color': WHITE, 'tip_length': 0.15}
        rhs = OldTex('\\approx', '0.13')
        rhs.scale(0.8)
        rhs.next_to(post_tex, RIGHT)
        to_rhs_arrow = Arrow(fraction.get_bottom(), rhs.get_top(), **arrow_kwargs)
        pre_top_rect_products = VGroup(prior_rhs_group[0], self.top_conditional_rhs)
        pre_bottom_rect_products = VGroup(prior_rhs_group[1], self.bottom_conditional_rhs)
        self.play(Indicate(prior_rects[0], scale_factor=1))
        self.play(*[ReplacementTransform(mob.copy(), term, run_time=2) for mob, term in zip(pre_top_rect_products, products[0])])
        ...
        self.play(*[ReplacementTransform(mob.copy(), term, run_time=2) for mob, term in zip(pre_bottom_rect_products, products[1])])
        self.wait()
        for pair in zip(pre_bottom_rect_products, products[1]):
            self.play(*list(map(Indicate, pair)))
            self.wait()
        self.play(Write(fraction.get_part_by_tex('+')), ReplacementTransform(products[0].copy(), products[2]))
        self.wait()
    ...
    def get_prior_labels(self, value):
        p_str = '%0.3f' % value
        q_str = '%0.3f' % (1 - value)
        labels = [OldTex('P(', s, self.double_heart_template, ')', '= ', num) for s, num in (('', p_str), ('\\text{not }', q_str))]
        for label in labels:
            label.scale(0.7)
    ...
    def get_conditional_label(self, value, given_flush=True):
        label = OldTex('P(', self.cash_string, '|', '' if given_flush else '\\text{not }', self.double_heart_template, ')', '=', str(value))
        self.insert_double_heart(label)
    ...
    def get_prior_change_anims(self, value, post_rects=None):
        ...
        anims = self.get_horizontal_division_change_animations(value, new_label_kwargs={'labels': self.get_prior_labels(value)})
        if post_rects is not None:
            anims += self.get_posterior_rectangle_change_anims(post_rects)
        return anims

    def get_conditional_change_anims(self, sub_sample_space_index, value, post_rects=None):
        given_flush = sub_sample_space_index == 0
        label = self.get_conditional_label(value, given_flush)
        return SampleSpaceScene.get_conditional_change_anims(self, sub_sample_space_index, value, post_rects, new_label_kwargs={'labels': [label]})
...
class BayesRuleInMemory(Scene):
    ...
    def construct(self):
        ...
        B = '\\text{Belief}'
        D = '\\text{Data}'
        rule = OldTex('P(', B, '|', D, ')', '=', 'P(', 'B', ')', '{P(', D, '|', B, ')', '\\over', 'P(', D, ')}')
        ...
        self.play(Blink(randy))
        self.play(ShowCreation(bubble), MoveToTarget(rule), randy.change, 'pondering')
        self.wait()
        self.play(rule.fade, 0.7, run_time=2)
        self.play(randy.change, 'confused', rule)
        self.play(Blink(randy))
        self.wait(2)
        self.play(FadeOut(VGroup(bubble, rule)), randy.change, 'pondering', screen_rect)
        self.play(randy.look_at, screen_rect.get_right(), ShowCreation(screen_rect))
        self.wait(4)
...
class NextVideoWrapper(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play_student_changes(*['pondering'] * 3, look_at=screen)
...
class BayesianNetworkPreview(Scene):
    ...
    def add_network(self):
        ...
        for i1, i2 in edge_index_pairs:
            ...
            edge = Arrow(n1.get_center(), n2.get_center(), buff=radius, color=WHITE)
        ...
        while curr_nodes:
            ...
            for node in curr_nodes:
                ...
                for next_node in connected_nodes:
                    ...
                    anims.append(Transform(node.fill.copy(), next_node.fill, remover=True))
...
class GeneralizeBayesRule(SampleSpaceScene):
    ...
    def add_sample_space(self):
        sample_space = SampleSpace(full_space_config={'height': 3, 'width': 3, 'fill_opacity': 0})
        sample_space.divide_horizontally(0.4)
        sample_space.horizontal_parts.set_fill(opacity=0)
        labels = [OldTex('P(', 'B', ')'), OldTex('P(\\text{not }', 'B', ')')]
        for label in labels:
            label.scale(0.7)
        ...
        for tup in zip(parts, values, given_strs, color_pairs, vects):
            ...
            label = OldTex('P(', 'I', '|', given_str, 'B', ')')
            label.scale(0.7)
            self.color_label(label)
            part.get_subdivision_braces_and_labels(part.vertical_parts, [label], vect)
            sample_space.add(part.vertical_parts.braces, part.vertical_parts.labels)
        sample_space.to_edge(LEFT)
        self.add(sample_space)
        self.sample_space = sample_space

    def add_title(self):
        title = OldTexText('Updating', 'Beliefs', 'from new', 'Information')
    ...
    def add_posterior_rectangles(self):
        ...
        label.scale(0.7)
        self.color_label(label)
        braces, labels = self.get_posterior_rectangle_braces_and_labels(post_rects, [label])
        self.play(ReplacementTransform(prior_rects.copy(), post_rects, run_time=2))
    ...
    def add_bayes_rule(self):
        rule = OldTex('=', '{P(', 'B', ')', 'P(', 'I', '|', 'B', ')', '\\over', 'P(', 'I', ')}')
    ...
    def talk_through_terms(self):
        ...
        prior_arrow = Arrow(prior_word.get_bottom(), prior.get_top(), tip_length=0.15)
        posterior_arrow = Arrow(posterior_word.get_top(), posterior.get_bottom(), tip_length=0.15)
        self.play(Write(prior_word), ShowCreation(prior_arrow), ShowCreation(prior_rect))
        self.wait()
        self.play(Transform(prior.copy(), prior_target, run_time=2, path_arc=-np.pi / 3, remover=True))
        self.wait()
        parts = self.sample_space[0].vertical_parts
        self.play(Indicate(likelihood), Indicate(parts.labels), Indicate(parts.braces))
        self.wait()
        ...
        self.wait()
        self.play(Write(posterior_word), ShowCreation(posterior_arrow), ShowCreation(posterior_rect))
    ...
    def name_likelihood(self):
        likelihoods = [self.sample_space[0].vertical_parts.labels[0], self.likelihood]
        rects = [SurroundingRectangle(mob, buff=SMALL_BUFF) for mob in likelihoods]
        name = OldTexText('Likelihood')
        name.scale(0.7)
        name.next_to(self.posterior_tex, UP, 1.5 * LARGE_BUFF)
        arrows = [Arrow(name, rect.get_edge_center(vect), tip_length=0.15) for rect, vect in zip(rects, [RIGHT, UP])]
        VGroup(name, *arrows + rects).set_color(YELLOW)
        morty = Mortimer()
        morty.scale(0.5)
        morty.next_to(rects[1], UP, buff=0)
        morty.shift(SMALL_BUFF * RIGHT)
        self.play(self.bayes_rule_words.to_edge, UP, Write(name), *list(map(ShowCreation, arrows + rects)))
        self.wait()
    ...
    def dont_memorize(self):
        ...
        self.play(FadeOut(self.post_rects), FadeOut(self.post_rects.braces), FadeOut(self.post_rects.labels))
    ...
    def show_space_restriction(self):
        ...
        self.play(LaggedStartMap(ApplyMethod, prior_rects, lambda m: (m.set_color, YELLOW), rate_func=there_and_back, lag_ratio=0.7))
        self.wait(2)
        self.play(ReplacementTransform(prior_rects.copy(), post_rects, run_time=2))
        ...
        self.wait()
        self.play(*it.chain(self.get_division_change_animations(self.sample_space, self.sample_space.horizontal_parts, 0.1), self.get_posterior_rectangle_change_anims(post_rects)))
        self.wait(3)
...
class MusicExample(SampleSpaceScene, PiCreatureScene):
    ...
    def introduce_musician(self):
        ...
        self.play(randy.change_mode, 'soulful_musician', path_arc=np.pi / 6)
        self.play(Animation(randy), DrawBorderThenFill(guitar), Animation(randy.arms))
        randy.add(guitar, randy.arms)
        self.wait()
        self.play_notes(guitar)
        self.change_pi_creature_with_guitar('concerned_musician')
        self.wait(2)
        self.play(randy.scale, 0.7, randy.to_corner, UP + LEFT)
        self.play_notes(guitar)
    ...
    def record_track(self):
        randy = self.pi_creature
        friends = VGroup(*[PiCreature(mode='happy', color=color).flip() for color in (BLUE_B, GREY_BROWN, MAROON_E)])
        friends.scale(0.6)
        ...
        self.play(LaggedStartMap(ApplyMethod, friends, lambda pi: (pi.change, 'hooray'), run_time=2))
    ...
    def friend_gives_compliment(self):
        ...
        bubble = SpeechBubble(height=1.25, width=3, direction=RIGHT, fill_opacity=0)
        ...
        self.play(LaggedStartMap(ApplyMethod, friends, lambda pi: (pi.change_mode, 'conniving')))
        self.wait()
        self.play(ShowCreation(bubble), Write(bubble.content, run_time=1), ApplyMethod(friends[0].change_mode, 'hooray'), LaggedStartMap(ApplyMethod, VGroup(*friends[1:]), lambda pi: (pi.change_mode, 'happy')))
        self.wait(2)
    ...
    def false_compliment(self):
        ...
        bubble = SpeechBubble(height=1.25, width=4.5, direction=RIGHT, fill_opacity=0)
        content = OldTexText('The beat was consistent.')
        content.scale(0.75)
        bubble.add_content(content)
        VGroup(bubble, content).next_to(friend, LEFT, SMALL_BUFF)
        VGroup(bubble, content).to_edge(UP, SMALL_BUFF)
        self.play(friend.change_mode, 'maybe', ShowCreation(bubble), Write(content))
    ...
    def get_positive_review(self):
        friends = self.friends
        self.change_pi_creature_with_guitar('soulful_musician', LaggedStartMap(ApplyMethod, friends, lambda pi: (pi.change, 'happy'), run_time=1))
        self.play_notes(self.pi_creature.guitar)

    def restrict_space(self):
        positive_space, negative_space = [VGroup(*[self.sample_space[i][j] for i in range(2)]) for j in range(2)]
        negative_space.save_state()
        self.play(negative_space.fade, 0.8)
        self.play(LaggedStartMap(ApplyMethod, positive_space, lambda m: (m.set_color, YELLOW), rate_func=there_and_back, run_time=2, lag_ratio=0.7))
        self.wait()
    ...
    def show_prior_rectangle_areas(self):
        prior_rects = self.get_prior_rectangles()
        products = VGroup(OldTex('(', '0.8', ')(', '0.9', ')'), OldTex('(', '0.2', ')(', '0.99', ')'))
        ...
        self.play(ReplacementTransform(side_labels[0][-1].copy(), top_product[1]), ReplacementTransform(top_labels[0][-1].copy(), top_product[3]), Write(VGroup(*top_product[::2])))
        self.wait(2)
        self.play(ReplacementTransform(side_labels[1][-1].copy(), bottom_product[1]), ReplacementTransform(bottom_labels[0][-1].copy(), bottom_product[3]), Write(VGroup(*bottom_product[::2])))
        self.wait(2)
    ...
    def show_posterior_probability(self):
        ...
        rhs.scale(0.7)
        rhs.next_to(post_tex, RIGHT)
        ratio = OldTex('{(0.8)(0.9)', '\\over', '(0.8)(0.9)', '+', '(0.2)(0.99)}')
        ratio.scale(0.6)
        ratio.next_to(VGroup(post_tex, rhs), DOWN, LARGE_BUFF)
        ratio.to_edge(RIGHT)
        arrow_kwargs = {'tip_length': 0.15, 'color': WHITE, 'buff': 2 * SMALL_BUFF}
        to_ratio_arrow = Arrow(post_tex.get_bottom(), ratio.get_top(), **arrow_kwargs)
        to_rhs_arrow = Arrow(ratio.get_top(), rhs[1].get_bottom(), **arrow_kwargs)
        prior_rects = self.get_prior_rectangles()
        self.play(ShowCreation(to_ratio_arrow), FadeIn(ratio))
        self.wait(2)
        for mob in (prior_rects, prior_rects[0]):
            self.play(mob.set_color, YELLOW, Animation(self.products), rate_func=there_and_back, run_time=2)
            self.wait()
        self.wait()
        self.play(ShowCreation(to_rhs_arrow))
    ...
    def intuition_of_positive_feedback(self):
        ...
        self.play(ShowCreation(prior_rect))
        self.play(Transform(prior_num_ghost, post_num, remover=True, path_arc=-np.pi / 6, run_time=2))
        self.play(ShowCreation(post_rect))
        self.wait(2)
        for mode, time in (('shruggie', 2), ('hesitant', 0)):
            self.play(LaggedStartMap(ApplyMethod, friends, lambda pi: (pi.change, mode), run_time=2))
            self.wait(time)
        self.play(*list(map(FadeOut, [prior_rect, post_rect, self.ratio_group, self.post_rhs])))
    ...
    def make_friends_honest(self):
        ...
        for value in (0.5, 0.1, 0.9):
            ...
            self.play(*self.get_top_conditional_change_anims(value, post_rects, new_label_kwargs={'labels': [label]}), run_time=2)
            self.wait(2)

    def fade_out_post_rect(self):
        self.play(*list(map(FadeOut, [self.post_rects, self.post_rects.braces, self.post_rects.labels])))
    ...
    def get_negative_feedback(self):
        ...
        post_rects = self.get_posterior_rectangles()
        label = OldTex('P(S | \\text{not } ', '\\checkmark', ')', '\\approx', '0.98')
        label.scale(0.7)
        label.set_color_by_tex('\\checkmark', GREEN)
        braces, labels = self.get_posterior_rectangle_braces_and_labels(post_rects, [label])
        brace = braces[0]
        self.play(old_prior_rects.fade, 0.8)
        self.play(LaggedStartMap(ApplyMethod, friends, lambda pi: (pi.change, 'pondering', post_rects), run_time=1))
        self.wait()
        self.play(ReplacementTransform(new_prior_rects.copy(), post_rects, run_time=2))
        self.play(GrowFromCenter(brace))
        self.wait(2)
        self.play(Write(label))
        self.wait(3)
    ...
    def create_pi_creature(self):
        randy = Randolph()
        randy.left_arm_range = [0.36, 0.45]
        self.randy = randy
        return randy

    def get_conditional_label(self, value, given_suck=True):
        positive_str = '\\checkmark'
        label = OldTex('P(', positive_str, '|', '' if given_suck else '\\text{not }', 'S', ')', '=', str(value))
    ...
    def play_notes(self, guitar):
        ...
        sine_wave.shift(notes.get_center() - sine_wave.point_from_proportion(0))
        self.play(LaggedStartMap(MoveAlongPath, notes, lambda n: (n, sine_wave), path_arc=np.pi / 2, run_time=4, lag_ratio=0.5, rate_func=lambda t: t))
...
class FinalWordsOnRule(SampleSpaceScene):
    ...
    def add_sample_space(self):
        ...
        sample_space[1].divide_vertically(bottom_condional, colors=[GREEN_E, RED_E])
        B = '\\text{Belief}'
        D = '\\text{Data}'
        P_B = OldTex('P(', B, ')')
        P_D_given_B = OldTex('P(', D, '|', B, ')')
        P_D_given_not_B = OldTex('P(', D, '|', '\\text{not }', B, ')')
        ...
        braces.add(*bs)
        bs, ls = self.get_posterior_rectangle_braces_and_labels(post_rects, [P_B_given_D])
        braces.add(*bs)
    ...
    def add_uses(self):
        uses = OldTexText('Machine learning, ', 'scientific inference, $\\dots$')
        uses.to_edge(UP)
        for use in uses:
    ...
    def tweak_values(self):
        post_rects = self.post_rects
        new_value_lists = [(0.85, 0.1, 0.11), (0.3, 0.9, 0.4), (0.97, 0.3, 1.0 / 22)]
        for new_values in new_value_lists:
            for i, value in zip(list(range(2)), new_values):
                self.play(*self.get_conditional_change_anims(i, value, post_rects))
                self.wait()
            self.play(*it.chain(self.get_horizontal_division_change_animations(new_values[-1]), self.get_posterior_rectangle_change_anims(post_rects)))
            self.wait()
        self.wait(2)
...
class Thumbnail(SampleSpaceScene):
    ...
    def construct(self):
        ...
        sample_space.add_braces_and_labels()
        post_rects = self.get_posterior_rectangles()
        group = self.get_posterior_rectangle_braces_and_labels(post_rects, [post_label])
