...
def get_stack(obj1, obj2, n, k, fixed_start=None, fixed_end=None, obj_to_obj_buff=SMALL_BUFF, vertical_buff=MED_SMALL_BUFF):
    stack = VGroup()
    for indices in it.combinations(list(range(n)), k):
        term = VGroup(*[obj1.copy() if i in indices else obj2.copy() for i in range(n)])
        if fixed_start:
...
class Male(Tex):
    CONFIG = {'height': 0.4, 'tex': '\\male', 'color': BLUE}
...
class Female(Male):
    CONFIG = {'tex': '\\female', 'color': MAROON_B}

class PascalsTriangle(VGroup):
    CONFIG = {'n_rows': 9, 'distance': 0.8, 'max_width_to_distance_ratio': 0.7, 'angle': 0.2 * np.pi}
...
class ExperienceProblemSolver(PiCreatureScene):
    ...
    def add_equation(self):
        equation = OldTex('\\frac{x^3 + y^3}{(x+y)^2} + \\frac{3xy}{x+y}')
        equation.to_edge(UP)
    ...
    def jenny_solves(self):
        ...
        self.pi_creature_says(jenny, jenny_words, target_mode='hooray', bubble_config={'height': 2, 'width': 3})
        self.wait()
        ...
        self.wait()
        self.play(*list(map(FadeOut, [jenny.bubble, jenny_words, randy_words])))
    ...
    def no_genius(self):
        ...
        self.play(ShowCreation(cross), jenny.change, 'sassy', cross, randy.change, 'happy')
        self.wait(2)
        self.to_fade = VGroup(lightbulb, cross)

    def think_about_patterns(self):
        randy, jenny = (self.randy, self.jenny)
        rows = PascalsTriangle(n_rows=6, distance=0.6)
        rows.scale(0.8)
        for row in rows:
            for num in row:
                n = float(num.get_tex())
                num.set_color(interpolate_color(BLUE, YELLOW, n / 10.0))
        self.pi_creature_thinks(jenny, '', bubble_config={'width': 5, 'height': 4.2}, added_anims=[FadeOut(self.to_fade), FadeOut(self.equation), randy.change, 'plain'])
        rows.move_to(jenny.bubble.get_bubble_center() + MED_SMALL_BUFF * (UP + LEFT))
        self.play(FadeIn(rows[0]))
        for last_row, curr_row in zip(rows, rows[1:]):
            self.play(*[Transform(last_row.copy(), VGroup(*mobs), remover=True) for mobs in (curr_row[1:], curr_row[:-1])])
            self.add(curr_row)
        self.wait(3)
...
class InitialFiveChooseThreeExample(Scene):
    CONFIG = {'n': 5, 'zero_color': BLUE, 'one_color': PINK}
    ...
    def show_all_stacks(self):
        stacks = get_stacks(self.get_obj1(), self.get_obj2(), self.n, vertical_buff=SMALL_BUFF)
    ...
    def add_title(self):
        ...
        binomials = VGroup(*[OldTex('%d \\choose %d' % (n, k)) for k in range(n + 1)])
        ...
        self.play(LaggedStartMap(MoveToTarget, mover, run_time=3))
        self.remove(mover)
        self.add(binomial_equations)
        self.wait()
        self.set_variables_as_attrs(n_choose_k, n_choose_k_words, binomial_equations)
    ...
    def issolate_single_stack(self):
        ...
        self.play(FadeOut(to_fade), equation.scale, 1.5, equation.get_bottom())
        self.wait()
        for line in stack:
            ones = VGroup(*[mob for mob in line if '1' in mob.get_tex()])
            line.ones = ones
            self.play(LaggedStartMap(ApplyMethod, ones, lambda mob: (mob.set_color, YELLOW), rate_func=there_and_back, lag_ratio=0.7, run_time=1))
    ...
    def count_ways_to_fill_slots(self):
        ...
        for indices in it.combinations(list(range(5)), 3):
            ones = VGroup(*[self.get_obj1().next_to(lines[i], UP) for i in indices])
    ...
    def emphasize_pattern_over_number(self):
        ...
        for x in range(2):
            for line in self.stacks[3]:
                ones = line.ones
                ones.save_state()
                self.play(ones.set_color, YELLOW, last_ones.restore, morty.look_at, ones, run_time=0.25)
                last_ones = ones
            self.wait()
...
class SixChooseThreeExample(InitialFiveChooseThreeExample):
    ...
    def show_stack(self):
        stack = get_stack(self.get_obj1(), self.get_obj2(), self.n, self.k, vertical_buff=SMALL_BUFF)
        stack.set_height(self.stack_height)
        stack.to_edge(DOWN)
        for line in stack:
            line.ones = VGroup(*[mob for mob in line if '1' in mob.get_tex()])
        equation = OldTex('{%d \\choose %d}' % (self.n, self.k), '=', str(choose(self.n, self.k)))
        equation.set_color(YELLOW)
        equation.set_color_by_tex('=', WHITE)
        equation.next_to(stack, RIGHT, LARGE_BUFF)
        self.add(equation)
        self.play(LaggedStartMap(FadeIn, stack, lag_ratio=0.1, run_time=10))
        self.wait()
    ...
    def talk_through_one_line(self):
        ...
        arrows = VGroup(*[Vector(0.5 * UP).next_to(one, DOWN, SMALL_BUFF) for one in line.ones])
        ...
        self.play(LaggedStartMap(GrowArrow, arrows), Write(choose_k, run_time=1))
        self.wait(2)
        self.play(line.restore, *list(map(FadeOut, [brace, n_options, arrows, choose_k])))
    ...
    def count_stack(self):
        ...
        self.add(brace, num)
        self.wait()
        lhs = OldTex('\\frac{6 \\cdot 5 \\cdot 3}{1 \\cdot 2 \\cdot 3} =')
        ...
        self.wait()
        self.play(ApplyMethod(lhs.shift, 0.65 * FRAME_X_RADIUS * (LEFT + UP), path_arc=np.pi / 2, rate_func=running_start, remover=True), *list(map(FadeOut, [brace, num, coming_soon])))
        self.wait()
    ...
    def think_about_pattern(self):
        ...
        for x in range(2):
            for line in self.stack:
                ones = line.ones
                ones.save_state()
                self.play(ones.set_color, YELLOW, ones.set_stroke, RED, 1, last_ones.restore, run_time=0.2)
                last_ones = ones
        self.wait()
...
class SixChooseThreeInOtherContext(Scene):
    ...
    def add_dots(self):
        ...
        self.add(dots)
        self.dots = dots
        self.dot_to_dot_distance = get_norm(dots[1].get_center() - dots[0].get_center())
    ...
    def count_paths_to_three_three(self):
        ...
        for indices in it.combinations(list(range(6)), 3):
            ...
            arrows = VGroup(*[right.deepcopy() if b else up.deepcopy() for b in bools])
            ...
            if last_line is None:
                ...
                self.play(FadeIn(line), ReplacementTransform(last_rights, rights), ReplacementTransform(last_ups, ups))
            last_rights = rights
            last_ups = ups
            last_line = line
        self.wait()
...
class ProbabilityOfKWomenInGroupOfFive(Scene):
    CONFIG = {'random_seed': 0, 'n_people_per_lineup': 5, 'n_examples': 18, 'item_line_width': 0.4}
    ...
    def ask_question(self):
        ...
        self.add(title)
        lineup_point = 1.5 * UP
        prob_words = VGroup(*[OldTexText('Probability of', str(n), 'women?').set_color_by_tex(str(n), YELLOW) for n in range(self.n_people_per_lineup + 1)])
    ...
    def show_all_possibilities(self):
        ...
        vects = [1.5 * UP, 0.65 * UP, 0.25 * UP, 3.5 * RIGHT, 1.5 * RIGHT]
        ...
        n_possibilities = OldTex('2 \\cdot', '2 \\cdot', '2 \\cdot', '2 \\cdot', '2', '\\text{ Possibilities}')
        n_possibilities.next_to(self.title, DOWN)
        twos = VGroup(*n_possibilities[-2::-1])
        twos.set_color(YELLOW)
        two_anims = [ReplacementTransform(VectorizedPoint(twos[0].get_center()), twos[0])] + [ReplacementTransform(t1.copy(), t2) for t1, t2 in zip(twos, twos[1:])]
        curr_lineup_group = lineup_groups[0]
        self.play(ReplacementTransform(self.lineup, curr_lineup_group[0]), FadeOut(self.prob_words))
        for i, lineup_group in enumerate(lineup_groups[1:]):
            ...
            for group in (men, women):
                self.play(LaggedStartMap(ApplyMethod, group, lambda m: (m.shift, MED_SMALL_BUFF * RIGHT), rate_func=there_and_back, lag_ratio=0.9 ** i, run_time=1))
            self.wait()
        ...
        self.play(n_possibilities[-1].next_to, eq_32, RIGHT, twos.next_to, eq_32, LEFT, FadeIn(eq_32))
        self.wait()
        ...
        for k, stack in enumerate(stacks):
            ...
            for lineup in stack:
                ...
                for item in lineup.items:
                    if 'female' in item.get_tex():
                        women.add(item)
            equation = OldTex('{%d \\choose %d}' % (n, k), '=', str(len(stack)))
            ...
            self.play(items.set_fill, None, 1, lines.set_stroke, WHITE, 3, Write(equation, run_time=1))
    ...
    def show_answer_to_question(self):
        stacks = self.stacks
        numbers = self.numbers
        n_possibilities = VGroup(self.n_possibilities[-1], self.n_possibilities[-3])
        n_possibilities_part_to_fade = VGroup(self.n_possibilities[-2], *self.n_possibilities[:-3])
        total = n_possibilities[-1]
        title = self.title
        n = self.n_people_per_lineup
        self.play(FadeOut(title), FadeOut(n_possibilities_part_to_fade), n_possibilities.to_corner, UP + RIGHT)
        for k, stack, num in zip(it.count(), stacks, numbers):
            rect = SurroundingRectangle(stack)
            num.save_state()
            prob_words = OldTex('P(', '\\#', '\\female', '=', str(k), ')=', '{\\quad \\over', '32}', '\\approx', '%0.3f' % (choose(n, k) / 32.0))
            prob_words.set_color_by_tex_to_color_map({'female': MAROON_B, '32': YELLOW})
            frac_line = prob_words.get_parts_by_tex('over')
            prob_words.to_corner(UP + LEFT)
            self.play(num.next_to, frac_line, UP, SMALL_BUFF, FadeIn(prob_words))
            self.play(ShowCreation(rect))
            self.wait(2)
    ...
    def ask_about_pattern(self):
        ...
        question.to_edge(UP)
        numbers = self.numbers
        circles = VGroup(*[Circle().replace(num, dim_to_match=1).scale(1.5) for num in numbers])
    ...
    def get_random_lineup_of_men_and_women(self):
        man, woman = (Male(), Female())
        lineup = self.get_lineup(*[woman if random.choice([True, False]) else man for y in range(self.n_people_per_lineup)])
        return lineup

    def get_lineup(self, *mobjects, **kwargs):
        buff = kwargs.get('buff', MED_SMALL_BUFF)
        lines = VGroup(*[Line(ORIGIN, self.item_line_width * RIGHT) for mob in mobjects])
...
class TeacherHoldingSomething(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play_student_changes(*['pondering'] * 3, look_at=2 * UP + 2 * RIGHT)
        self.wait(6)
...
class BuildFiveFromFour(ProbabilityOfKWomenInGroupOfFive):
    ...
    def show_all_configurations_of_four(self):
        man, woman = (Male(), Female())
        n = 4
        vects = [1.5 * UP, 0.5 * UP, 3.5 * RIGHT, 1.5 * RIGHT]
        lineup_groups = VGroup()
        for k in range(n + 1):
            lineup_group = VGroup()
            for tup in it.product(*[[man, woman]] * k):
                ...
                lineup_group.add(lineup)
            lineup_groups.add(lineup_group)
        n_possibilities = OldTex('2 \\cdot', '2 \\cdot', '2 \\cdot', '2', '\\text{ Possibilities}')
        ...
        self.play(n_possibilities[-1].next_to, eq_16, RIGHT, twos.next_to, eq_16, LEFT, FadeIn(eq_16))
        self.wait()
    ...
    def split_into_two_possibilities(self):
        ...
        self.play(FadeOut(self.n_possibilities), top_group.to_edge, UP, MED_SMALL_BUFF)
        ...
        for stacks, sym in ((top_stacks, Female()), (bottom_stacks, Male())):
            ...
            for stack in stacks:
                new_stack = VGroup()
                for lineup in stack:
                    new_lineup = self.get_lineup(*[Female() if 'female' in item.get_tex() else Male() for item in lineup.items] + [sym], buff=SMALL_BUFF)
        ...
        self.play(top_stacks.shift, dist * RIGHT / 2, top_stacks.numbers.shift, dist * RIGHT / 2, bottom_stacks.shift, dist * LEFT / 2, bottom_stacks.numbers.shift, dist * LEFT / 2)
        self.wait()
    ...
    def combine_stacks(self):
        ...
        for i, top_stack in enumerate(top_stacks[:-1]):
            ...
            new_number = OldTex(str(len(top_stack) + len(bottom_stack) - 2))
            new_number.next_to(expr, UP)
            new_numbers.add(new_number)
            self.play(Write(plus), *list(map(MoveToTarget, movers)))
        self.play(VGroup(top_stacks[-1], top_stacks.numbers[-1]).align_to, bottom_stacks, DOWN)
        self.wait()
        ...
        for x in range(self.n_iterations):
            ...
            for stacks, i in ((low_stacks, 0), (top_stacks, -1)):
                ...
                for num, stack in zip(stacks.numbers.target, new_stacks):
                    num.next_to(stack, UP)
            self.play(*list(map(MoveToTarget, [top_stacks, low_stacks, top_stacks.numbers, low_stacks.numbers])))
            self.wait(wait_time)
            dist = top_stacks[1].get_center()[0] - top_stacks[0].get_center()[0]
            self.play(top_group.shift, dist * RIGHT / 2, low_group.shift, dist * LEFT / 2)
            self.wait(wait_time)
            all_movers = VGroup()
            plusses = VGroup()
            ...
            self.play(*it.chain(list(map(MoveToTarget, all_movers)), list(map(Write, plusses))))
            new_numbers = self.get_numbers(stacks)
            self.play(ReplacementTransform(expressions, VGroup(*list(map(VGroup, new_numbers)))))
            self.wait(wait_time)
            stacks.numbers = new_numbers

    def get_numbers(self, stacks):
        return VGroup(*[OldTex(str(len(stack))).next_to(stack, UP) for stack in stacks])
...
class IntroducePascalsTriangle(Scene):
    ...
    def show_sum_of_two_over_rule(self):
        ...
        self.play(ReplacementTransform(example.rect.copy(), ex_top1.rect), ReplacementTransform(example.rect.copy(), ex_top2.rect))
        self.wait(2)
        self.play(rows_to_fade.fade, 1)
        self.play(top_row.align_to, low_row, LEFT, top_row_copy.next_to, top_row, UP, top_row_copy.align_to, low_row, RIGHT)
        self.play(ShowCreation(h_line), Write(plus))
        self.wait(2)
        for row in (top_row, top_row_copy):
            row.remove(row[-1])
        self.play(rows_to_fade.restore, top_row.restore, Transform(top_row_copy, top_row.saved_state, remover=True), FadeOut(VGroup(h_line, plus)), FadeOut(rects))
        self.wait()
    ...
    def keep_in_mind_what_these_mean(self):
        ...
        self.play(FadeIn(morty))
        self.play(PiCreatureSays(morty, 'Keep in mind \\\\ what these mean.', bubble_config={'width': 3.5, 'height': 2.5}))
        self.play(Blink(morty), LaggedStartMap(Indicate, numbers, rate_func=wiggle, color=PINK))
    ...
    def show_9_choose_4_pattern(self):
        ...
        self.play(rows.shift, 3 * UP, self.nine_choose_four_line.shift, 2.5 * UP)
        ...
        self.wait()
        self.curr_line = line
        expr = OldTex('P(4', '\\female', '\\text{ out of }', '9', ')', '=')
        ...
        nine_choose_four_term.target.scale(1.0 / 1.2)
        ...
        show_random_lines(6)
        self.play(self.nine_choose_four_term.scale, 1.0 / 1.2, self.nine_choose_four_term.set_color, WHITE, *list(map(FadeOut, [expr, nine_choose_four_term, over_512, eq_result, self.curr_line])))
...
class StacksApproachBellCurve(Scene):
    ...
    def construct(self):
        ...
        for x in range(self.n_iterations):
            ...
            self.play(*list(map(MoveToTarget, [bars, bars_copy, numbers, numbers_copy])))
            self.play(bars.shift, distance * LEFT / 2, numbers.shift, distance * LEFT / 2, bars_copy.shift, distance * RIGHT / 2, numbers_copy.shift, distance * RIGHT / 2)
            ...
            for i in range(len(bars) - 1):
                ...
                if new_num.get_width() > top_bar.get_width():
                    min_scale_val = min(min_scale_val, top_bar.get_width() / new_num.get_width())
            ...
            self.play(*[MoveToTarget(mob, lag_ratio=0.5) for mob in (bars_copy, numbers, numbers_copy)])
...
class ChooseThreeFromFive(InitialFiveChooseThreeExample, PiCreatureScene):
    CONFIG = {'n': 5, 'k': 3, 'pi_creature_scale_val': 0.3, 'people_colors': [PURPLE, BLUE, GREEN, GOLD_E, GREY]}
    ...
    def show_binary_strings(self):
        n, k = (self.n, self.k)
        stack = get_stack(self.get_obj1(), self.get_obj2(), n, k, vertical_buff=SMALL_BUFF)
        stack.to_edge(DOWN, buff=LARGE_BUFF)
        equation = OldTex('{%d \\choose %d}' % (n, k), '=', str(choose(n, k)))
        ...
        for i, line in enumerate(stack):
            ...
            self.wait(0.25)
            self.remove(num)
        self.play(Write(VGroup(*equation[:-1])), ReplacementTransform(num, equation[-1]))
        self.wait()
    ...
    def add_people(self):
        ...
        self.play(Write(braces), LaggedStartMap(FadeIn, people), VGroup(self.stack, self.equation).to_edge, RIGHT, LARGE_BUFF)
    ...
    def choose_triplets(self):
        ...
        movers.target.arrange(DOWN, buff=SMALL_BUFF, aligned_edge=LEFT)
        movers.target.next_to(self.people, DOWN, MED_LARGE_BUFF)
        if hasattr(self, 'stack'):
            movers.target.align_to(self.stack, UP)
        self.play(LaggedStartMap(MoveToTarget, movers, lag_ratio=0.2, run_time=4))
        self.wait()
    ...
    def show_association_with_binary(self, index):
        ...
        self.play(LaggedStartMap(ShowCreation, people_rects), MoveToTarget(people), MoveToTarget(names))
        self.wait()
        self.play(ReplacementTransform(line_rect, full_line_rect), triplet.set_color, YELLOW)
        self.wait(2)
        self.play(people.restore, names.restore, triplet.restore, FadeOut(line), FadeOut(full_line_rect), FadeOut(people_rects))

    def order_doesnt_matter(self):
        triplet = self.name_triplets[0].copy()
        triplet.set_fill(opacity=1)
        triplet.next_to(self.name_triplets, RIGHT, buff=LARGE_BUFF, aligned_edge=UP)
        ...
        self.play(ReplacementTransform(triplet.copy(), curr_perm), Write(updownarrow))
        for i in range(8):
            new_perm = permutations[i % (len(permutations) - 1) + 1]
            anims = [Transform(curr_perm, new_perm, path_arc=np.pi)]
            if i == 1:
                self.wait()
            if i == 4:
    ...
    def that_phrase_is_confusing(self):
        ...
        self.play(LaggedStartMap(ShowCreationThenDestruction, odm_words_outline, lag_ratio=0.2, run_time=1), LaggedStartMap(ApplyMethod, self.people, lambda pi: (pi.change, 'confused', odm_words)), LaggedStartMap(FadeIn, q_marks))
        ...
        for x in range(2):
            for i in range(len(word_arrow_groups) + 1):
                ...
                self.play(*anims)
            self.wait()
            word_arrow_groups.submobjects = [word_arrow_groups[j] for j in (1, 2, 0)]
    ...
    def pattern_is_unambiguous(self):
        ...
        for line in self.stack:
            ...
            all_ones.add(ones)
        self.play(LaggedStartMap(FadeIn, all_ones, lag_ratio=0.2, run_time=3, rate_func=there_and_back), LaggedStartMap(ApplyMethod, self.people, lambda pi: (pi.change, 'happy', ones)))
        self.wait()
    ...
    def create_pi_creatures(self):
        people = VGroup(*[PiCreature(color=color).scale(self.pi_creature_scale_val) for color in self.people_colors])
    ...
    def get_names(self, people):
        names = VGroup(*[OldTexText(name + ',') for name in ('Ali', 'Ben', 'Cam', 'Denis', 'Evan')])
...
class SubsetProbabilityExample(ChooseThreeFromFive):
    ...
    def ask_question(self):
        ...
        words = OldTexText('Choose 3 people randomly.\\\\', 'Probability', 'Ali', 'is one of them?')
        ...
        for x in range(4):
            ...
            if ali in group:
                symbol = checkmark
                rect = SurroundingRectangle(group.target[group.submobjects.index(ali)])
                rect.set_stroke(GREEN)
            else:
                symbol = cross
                rect = VGroup()
            run_time = 1
            self.play(MoveToTarget(group), FadeIn(symbol), ShowCreation(rect), run_time=run_time)
            self.wait(0.5)
            self.play(group.restore, FadeOut(symbol), FadeOut(rect), run_time=run_time)
        self.question = words
        self.set_variables_as_attrs(pi_name_groups)

    def show_all_triplets(self):
        self.play(self.question.scale, 0.75, self.question.to_corner, UP + RIGHT, VGroup(self.people, self.names, self.braces).to_edge, LEFT)
        self.choose_triplets()
        brace = Brace(self.name_triplets, RIGHT)
        total_count = brace.get_tex('{5 \\choose 3}', '=', '10', buff=MED_LARGE_BUFF)
        total_count.set_color(BLUE)
        self.play(GrowFromCenter(brace), Write(total_count))
        self.wait()
    ...
    def circle_those_with_ali(self):
        ...
        self.add(arrows)
        self.wait()
        self.play(FadeIn(frac_lines), five_choose_three.next_to, frac_lines[0], DOWN, SMALL_BUFF, ten.next_to, frac_lines[1], DOWN, SMALL_BUFF, Write(four_choose_two))
        self.wait()
...
class StudentsGetConfused(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.add(line)
        self.play(self.get_shuffle_anim(line), PiCreatureSays(pi1, 'Wait \\dots order matters now?', target_mode='confused', look_at=line))
        self.play(self.get_shuffle_anim(line), *[ApplyMethod(pi.change, 'confused', line) for pi in self.pi_creatures])
        for x in range(4):
...
class HowToComputeNChooseK(ChooseThreeFromFive):
    CONFIG = {'n': 5, 'k': 3, 'line_colors': [GREEN, YELLOW], 'n_permutaitons_to_show': 5}
    ...
    def count_possibilities(self):
        ...
        for name, line, count, arrow in zip(chosen_names, lines, choice_counts, arrows):
            self.play(FadeIn(count), LaggedStartMap(FadeIn, name_rects, rate_func=there_and_back, remover=True))
            self.play(name.next_to, line, UP, SMALL_BUFF, GrowArrow(arrow))
            self.wait()
            name_rects.remove(name.rect)
            name_rects.set_stroke(YELLOW, 3)
        choice_numbers = VGroup(*[cc.submobjects.pop(1) for cc in choice_counts])
        ...
        self.play(FadeOut(choice_counts), FadeOut(arrows), MoveToTarget(choice_numbers), Write(dots), GrowFromCenter(chosen_names_brace))
        self.wait()
        self.set_variables_as_attrs(chosen_names, chosen_names_brace, choice_numbers, choice_numbers_dots=dots)
    ...
    def show_permutations_of_ABC(self):
        ...
        for indices in list(it.permutations(list(range(3))))[1:n_perms]:
            self.play(*[ApplyMethod(name.next_to, lines[i], UP, SMALL_BUFF, path_arc=np.pi) for i, name in zip(indices, chosen_names)])
            self.wait(0.5)
    ...
    def count_permutations_of_ABC(self):
        ...
        numerator = VGroup(self.choice_numbers, self.choice_numbers_dots)
        ...
        arrows = self.get_choice_count_arrows(choice_counts)
        self.play(chosen_names.shift, UP, chosen_names.to_edge, LEFT, numerator.next_to, frac_line, UP, SMALL_BUFF, FadeOut(brace))
        ...
        for line, name, count, arrow in zip(lines, shuffled_names, choice_counts, arrows):
            ...
            self.play(name.next_to, line, UP, SMALL_BUFF, path_arc=-np.pi / 3)
            self.wait()
        choice_numbers = VGroup(*[cc.submobjects.pop(1) for cc in choice_counts])
        choice_numbers.generate_target()
        dots = VGroup(*[Tex('\\cdot') for x in range(k - 1)])
        ...
        self.play(FadeOut(choice_counts), FadeOut(arrows), MoveToTarget(choice_numbers), Write(dots), ShowCreation(frac_line))
        self.wait()
        self.fraction = VGroup(numerator, frac_line, VGroup(choice_numbers, dots))

    def reset_stage(self):
        n, k = (self.n, self.k)
        n_choose_k_equals = OldTex('{%d \\choose %d} =' % (n, k))
        n_choose_k_equals.next_to(ORIGIN, RIGHT, LARGE_BUFF)
        n_choose_k_equals.to_edge(UP, LARGE_BUFF)
        self.play(self.names.restore, FadeOut(self.lines), FadeOut(self.place_words))
        self.play(self.people_group.to_edge, LEFT, FadeIn(n_choose_k_equals), self.fraction.next_to, n_choose_k_equals, RIGHT, SMALL_BUFF)
    ...
    def show_whats_being_counted(self):
        ...
        lines.scale(0.8)
        lines.to_edge(DOWN)
        rects = VGroup(*[SurroundingRectangle(line, buff=0, stroke_width=0, fill_color=BLUE, fill_opacity=0.5) for line in lines])
        self.play(Write(VGroup(*rhs[:-1])), LaggedStartMap(ApplyMethod, all_groups, lambda g: (g.restore,), rate_func=lambda t: smooth(1 - t), run_time=4, lag_ratio=0.2))
        self.wait()
    ...
    def get_choice_counts(self, n, k):
        ...
        choice_counts = VGroup(*[OldTexText('(', str(n0), ' choices', ')', arg_separator='') for n0 in range(n, n - k, -1)])
    ...
    def get_choice_count_arrows(self, choice_counts):
        lines = self.lines
        return VGroup(*[Arrow(count.get_bottom(), line.get_center() + MED_LARGE_BUFF * UP, color=line.get_color()) for count, line in zip(choice_counts, lines)])
    ...
    def get_lines_and_place_words(self):
        ...
        place_words = VGroup(*[OldTex('%d^\\text{%s}' % (i + 1, s)) for i, s in zip(list(range(k)), it.chain(['st', 'nd', 'rd'], it.repeat('th')))])
...
class NineChooseFourExample(HowToComputeNChooseK):
    CONFIG = {'random_seed': 2, 'n': 9, 'k': 4, 'line_colors': [RED, MAROON_B], 'n_permutaitons_to_show': 3}
    ...
    def setup_people(self):
        self.remove(self.people)
        self.people = OldTexText(' '.join([chr(ord('A') + i) for i in range(self.n)]))
    ...
    def show_n_choose_k_pattern(self):
        n, k = (self.n, self.k)
        stack = get_stack(OldTex('1').set_color(PINK), OldTex('0').set_color(BLUE), n, k)
        l = len(stack)
        n_stacks = 6
        columns = VGroup(*[VGroup(*stack[i * l / n_stacks:(i + 1) * l / n_stacks]) for i in range(n_stacks)])
    ...
    def choose_k_people(self):
        ...
        self.play(Write(braces), LaggedStartMap(FadeIn, people, run_time=1), FadeIn(n_items))
        self.wait()
        self.play(FadeIn(choose_k), LaggedStartMap(ApplyMethod, chosen_subset, lambda m: (m.shift, MED_LARGE_BUFF * DOWN)))
        self.wait()
        self.play(chosen_subset.shift, MED_LARGE_BUFF * UP, n_items.next_to, n_items.get_center(), LEFT, choose_k.next_to, n_items.get_center(), RIGHT)

    def count_how_to_choose_k(self):
        lines, place_words = self.get_lines_and_place_words()
        self.play(LaggedStartMap(FadeIn, lines), LaggedStartMap(FadeIn, place_words), run_time=1)
        self.count_possibilities()
    ...
    def finish_computation(self):
        ...
        self.play(self.names.restore, FadeOut(self.lines), FadeOut(self.place_words), self.n_choose_k.next_to, equals, LEFT, self.fraction.next_to, equals, RIGHT, FadeIn(equals))
        self.wait()
        ...
        self.play(ShowCreation(six.cross), ShowCreation(two_three.cross))
        self.wait()
        self.play(ShowCreation(eight.cross), ShowCreation(four.cross), FadeIn(two))
        self.wait()
        self.play(Write(rhs))
        self.wait()
...
class WeirdKindOfCancelation(TeacherStudentsScene):
    ...
    def construct(self):
        fraction = OldTex('{5 \\cdot 4 \\cdot 3', '\\text{ ordered}', '\\text{ triplets}', '\\over', '1 \\cdot 2 \\cdot 3', '\\text{ orderings \\;\\qquad}}')
        ...
        names = VGroup(*list(map(TexText, ['Ali', 'Ben', 'Cam', 'Denis', 'Evan'])))
        names.arrange(RIGHT)
        ...
        self.play(RemovePiCreatureBubble(self.teacher, target_mode='raise_right_hand'), LaggedStartMap(FadeIn, fraction, run_time=1), FadeIn(braces), LaggedStartMap(FadeIn, names))
        self.play_student_changes(*['pondering'] * 3, look_at=fraction)
        for num, name in zip(top_numbers[::2], chosen_names):
            rect = SurroundingRectangle(num)
            name.target.set_color(num.get_color())
            self.play(ShowCreationThenDestruction(rect), MoveToTarget(name))
        self.wait(2)
        permutations = list(it.permutations(list(range(3))))[1:]
        self.shuffle(chosen_names, permutations[:2])
        self.play(LaggedStartMap(ShowCreationThenDestruction, VGroup(*list(map(SurroundingRectangle, bottom_numbers[::2])))))
        self.shuffle(chosen_names, permutations[2:])
        self.wait()
        top_cross = Cross(ordered)
        bottom_cross = Cross(orderings)
        self.play(ShowCreation(top_cross), self.teacher.change, 'maybe')
    ...
    def shuffle(self, mobject, permutations):
        for permutation in permutations:
            self.play(*[ApplyMethod(m.move_to, mobject[i].get_center(), path_arc=np.pi) for i, m in zip(permutation, mobject)])
...
class ShowFormula(Scene):
    ...
    def construct(self):
        specific_formula = OldTex('{9 \\choose 4}', '=', '{9 \\cdot 8 \\cdot 7 \\cdot 6', '\\over', '4 \\cdot 3 \\cdot 2 \\cdot 1}')
        general_formula = OldTex('{n \\choose k}', '=', '{n \\cdot (n-1) \\cdots (n-k+1)', '\\over', 'k \\cdot (k-1) \\cdots 2 \\cdot 1}')
...
class SumsToPowerOf2(Scene):
    ...
    def setup_stacks(self):
        stacks = get_stacks(OldTex('1').set_color(PINK), OldTex('0').set_color(BLUE), n=self.n, vertical_buff=SMALL_BUFF)
        stacks.to_corner(DOWN + LEFT)
        numbers = VGroup(*[OldTex(str(choose(self.n, k))) for k in range(self.n + 1)])
        for number, stack in zip(numbers, stacks):
            number.next_to(stack, UP)
        self.play(LaggedStartMap(FadeIn, stacks), LaggedStartMap(FadeIn, numbers))
        self.wait()
    ...
    def count_32(self):
        ...
        numbers.target = VGroup(*[OldTex('{%d \\choose %d}' % (self.n, k)) for k in range(self.n + 1)])
        ...
        self.play(MoveToTarget(sum_result), MoveToTarget(numbers), Write(plusses), Write(rhs))
        self.wait()
        self.set_variables_as_attrs(plusses, sum_result, rhs, n_choose_k_terms=numbers)
    ...
    def show_sum_as_n_choose_k(self):
        ...
        self.play(Transform(n_choose_k_terms, fractions), MoveToTarget(plusses), lag_ratio=0.5, run_time=2)
        self.wait()
    ...
    def show_alternate_sum(self):
        ...
        plusses = VGroup(*[OldTex('+') for f in fractions[:-1]] + [Tex('=')])
        sum_group = VGroup(*it.chain(*list(zip(fractions, plusses))))
        sum_group.arrange(RIGHT)
        sum_group.next_to(self.n_choose_k_terms, DOWN, aligned_edge=LEFT, buff=LARGE_BUFF)
        sum_group.shift(SMALL_BUFF * DOWN)
        rhs = OldTex(str(2 ** self.alt_n), '=', '2^{%d}' % self.alt_n)
        rhs[0].set_color(YELLOW)
        rhs.next_to(sum_group, RIGHT)
        self.play(LaggedStartMap(FadeOut, self.stacks), LaggedStartMap(FadeOut, self.numbers), LaggedStartMap(FadeIn, sum_group))
...
class AskWhyTheyAreCalledBinomial(TeacherStudentsScene):
    ...
    def construct(self):
        example_binomials = VGroup(*[OldTex('(x+y)^%d' % d) for d in range(2, 7)])
        ...
        binomial_word = OldTexText('Bi', 'nomials', arg_separator='')
        ...
        arrows = VGroup(*[Arrow(binomial_word.get_right(), binom.get_left()) for binom in example_binomials])
        ...
        self.student_says("Why are they called \\\\ ``binomial coefficients''?")
        self.play(LaggedStartMap(FadeIn, pascals))
        self.wait()
        self.play(FadeIn(example_binomials[0]), RemovePiCreatureBubble(self.students[1]), self.teacher.change, 'raise_right_hand')
...
class Thumbnail(Scene):
    ...
    def construct(self):
        ...
        stacks = get_stacks(OldTex('1').set_color(PINK), OldTex('0').set_color(BLUE), n=5, vertical_buff=SMALL_BUFF)
        stacks.to_edge(DOWN)
