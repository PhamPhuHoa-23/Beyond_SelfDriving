...
class QuizResult(PiCreatureScene):
    ...
    def construct(self):
        ...
        all_students = PiCreatureClass(width=nb_students_x, height=nb_students_y)
        student_points = []
        grades = []
        grades_count = []
        ...
        self.wait()
        self.play(FadeOut(grades_mob), FadeIn(students_points_mob))
        ...
        self.play(all_other_students.fade, 0.8, all_other_points.fade, 0.8)
        self.wait()
        scale = 1.5
        ...
        self.wait()
        self.play(all_students.restore, students_points_mob.restore)
        self.wait()
        anims = []
        ...
        grade_hist = Histogram(np.ones(4), hist_y_values, mode='widths', x_labels='none', y_label_position='center', bar_stroke_width=0, outline_stroke_width=5)
        ...
        arrows = VGroup(*[Arrow(nb_students_label.get_left(), grade_hist.bars[i].get_center(), color=highlight_color) for i in range(4)])
        self.wait()
        ...
        self.wait()
        self.play(all_students[8].set_color, MAROON_E, ReplacementTransform(percentage_label, prob_label))
        self.wait()
        self.play(FadeOut(prob_label), FadeOut(arrows))
        flash_hist = FlashThroughHistogram(grade_hist, direction='vertical', mode='random', cell_opacity=0.5, run_time=5, rate_func=linear)
        flash_class = FlashThroughClass(all_students, mode='random', highlight_color=MAROON_E, run_time=5, rate_func=linear)
        self.wait()
        for i in range(3):
