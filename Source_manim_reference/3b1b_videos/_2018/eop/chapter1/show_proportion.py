...
class ProbabilityRect(VMobject):
    CONFIG = {'unit_width': 2, 'unit_height': 2, 'alignment': LEFT, 'color': YELLOW, 'opacity': 1.0, 'num_decimal_places': 2, 'use_percent': False}
    ...
    def create_prob_rect(self, p):
        ...
        prob_rect = Rectangle(width=prob_width, height=prob_height, fill_color=self.color, fill_opacity=self.opacity, stroke_color=self.color)
        prob_rect.align_to(self.unit_rect, direction=self.alignment)
        return prob_rect

    def create_prob_label(self, p):
        if self.use_percent:
            prob_label = DecimalNumber(p * 100, color=BLACK, num_decimal_places=self.num_decimal_places, unit='\\%')
        else:
            prob_label = DecimalNumber(p, color=BLACK, num_decimal_places=self.num_decimal_places)
