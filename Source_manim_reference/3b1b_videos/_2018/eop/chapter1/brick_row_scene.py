...
class BrickRowScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(Succession, coin_seqs, lambda m: (FadeIn(m, run_time=0.1), MoveToTarget(m)), run_time=5, lag_ratio=0.5))
        ...
        self.wait()
        self.wait()
        self.play(FadeOut(single_flip_labels), FadeOut(new_heads), FadeOut(single_flip_labels_copy), FadeOut(new_tails))
        self.wait()
        self.tallies = VGroup()
        ...
        prototype_dice = Rectangle(width=dice_width, height=dice_height, stroke_width=2, stroke_color=WHITE, fill_color=WHITE, fill_opacity=0)
        ...
        for dice in all_dice:
            ...
            self.play(ApplyMethod(dice.set_fill, {'opacity': 0.5}, rate_func=there_and_back, run_time=0.05))
        self.wait()
        self.play(FadeOut(outcomes), FadeOut(subdivs), FadeOut(all_dice), FadeOut(table), FadeOut(coin1), FadeOut(coin2), FadeIn(self.tallies))
        self.wait()
        new_row = self.row.copy()
        self.clear()
        self.add(randy, self.row, self.tallies)
        self.bring_to_back(new_row)
        self.play(self.row.shift, 2.5 * UP, self.tallies.shift, 2.5 * UP)
        old_row = self.row
        self.row = new_row
        self.play(FlipCoin(randy))
        self.wait()
        ...
        self.add_foreground_mobject(self.tallies)
        self.add_foreground_mobject(self.tallies_copy)
        self.play(self.row.fade, 0.7, old_row.fade, 0.7, FadeOut(self.tallies), FadeOut(self.tallies_copy))
        ...
        self.play(FadeIn(rect_left), FadeIn(rect_right), FadeIn(tally_left), FadeIn(tally_right))
        ...
        self.row = self.merge_rects_by_subdiv(self.row)
        self.wait()
        self.play(FadeOut(old_row), self.row.fade, 0, FadeOut(new_tally_left), FadeOut(new_tally_right), FadeIn(self.tallies[1]), FadeIn(self.tallies_copy[0]))
        self.play(FadeIn(self.tallies[:1]), FadeIn(self.tallies[2:]), FadeIn(self.tallies_copy[1:]))
        self.wait()
        ...
        self.wait()
        self.play(LaggedStartMap(FadeIn, outcomes, run_time=5))
        self.wait()
        ...
        self.wait()
        self.play(LaggedStartMap(FadeOut, outcomes, run_time=5))
        self.play(FadeIn(self.tallies))
        self.wait()
        self.play(FadeOut(braces), FadeOut(probs))
        self.wait()
        run_time = 5
        self.play(LaggedStartMap(FadeIn, outcomes, run_time=run_time), FadeOut(self.tallies, run_time=run_time))
        self.wait()
        self.play(LaggedStartMap(FadeOut, outcomes, run_time=5), FadeIn(self.tallies, run_time=run_time))
        previous_row = self.row.copy()
        self.add(previous_row)
        v = 1.25 * self.row.height * UP
        self.play(previous_row.shift, v, self.tallies.shift, v)
        self.add_foreground_mobject(self.tallies)
        self.play(SplitRectsInBrickWall(self.row))
        self.wait()
        self.row = self.merge_rects_by_subdiv(self.row)
        ...
        self.play(LaggedStartMap(FadeIn, grouped_outcomes), LaggedStartMap(FadeIn, grouped_outcomes_copy))
        self.wait()
        ...
        for i in range(n + 2):
            ...
            index += size
        old_tally_sizes.append(0)
        for i in range(n + 1):
            self.play(Transform(grouped_outcomes[i][0], grouped_target_outcomes[i][0][old_tally_sizes[i - 1]:]), Transform(grouped_outcomes_copy[i][0], grouped_target_outcomes[i + 1][0][:old_tally_sizes[i]]))
        return
        self.wait()
        ...
        self.play(Transform(grouped_outcomes[k][0], grouped_target_outcomes[k][0][old_tally_sizes[k - 1]:]), Transform(grouped_outcomes_copy[k - 1][0], grouped_target_outcomes[k][0][:old_tally_sizes[k - 1]]))
        self.play(FadeOut(previous_row), FadeOut(self.tallies))
        self.row = self.merge_rects_by_coloring(self.row)
        self.play(FadeIn(new_tallies[0]), FadeIn(new_tallies[2:]))
...
class ShowProbsInBrickRow3(BrickRowScene):
    ...
    def construct(self):
        ...
        for brace, prob in zip(braces, probs):
            prob.next_to(brace, UP)
        self.wait()
        self.play(LaggedStartMap(ShowCreation, braces, run_time=3), LaggedStartMap(Write, probs, run_time=3))
        self.wait()
        self.play(FadeOut(braces), FadeOut(probs))
...
class ShowOutcomesInBrickRow4(BrickRowScene):
    ...
    def construct(self):
        ...
        previous_row = self.row.copy()
        v = 1.25 * self.row.height * UP
        self.play(previous_row.shift, v)
        self.add(self.row)
        self.add(previous_row)
        self.wait()
        ...
        self.play(LaggedStartMap(FadeIn, previous_outcomes), LaggedStartMap(FadeIn, previous_outcomes_copy))
        self.wait()
        new_outcomes = self.row.get_outcome_rects_for_level(4, with_labels=True, inset=True)
        new_outcomes_left = VGroup(new_outcomes[0], new_outcomes[2], new_outcomes[3], new_outcomes[4], new_outcomes[8], new_outcomes[9], new_outcomes[10], new_outcomes[14])
        new_outcomes_right = VGroup(new_outcomes[1], new_outcomes[5], new_outcomes[6], new_outcomes[7], new_outcomes[11], new_outcomes[12], new_outcomes[13], new_outcomes[15])
