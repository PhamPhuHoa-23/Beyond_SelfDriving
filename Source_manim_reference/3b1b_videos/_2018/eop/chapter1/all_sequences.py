...
class ShuffleThroughAllSequences(Scene):
    CONFIG = {'nb_coins': 20, 'run_time': 5, 'fps': int(1.0 / PRODUCTION_QUALITY_FRAME_DURATION), 'coin_size': 0.5, 'coin_spacing': 0.65}
    ...
    def construct(self):
        ...
        idle_heads = CoinSequence(nb_idle_coins * ['H'], radius=self.coin_size * 0.5, spacing=self.coin_spacing)
        idle_tails = CoinSequence(nb_idle_coins * ['T'], radius=self.coin_size * 0.5, spacing=self.coin_spacing)
        idle_tails.fade(0.5)
        ...
        for i in range(2 ** nb_relevant_coins):
            ...
            all_coins = VGroup(left_idle_part, coin_seq)
            all_coins.center()
