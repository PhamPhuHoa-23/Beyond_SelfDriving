...
class GenericMorphBrickRowIntoHistogram(Scene):
    CONFIG = {'level': 3, 'bar_width': 2.0, 'bar_anchor_height': -3.0, 'show_tallies': False, 'show_nb_flips': True}
    ...
    def construct(self):
        ...
        self.wait()
        self.play(self.row.rects.space_out_submobjects, {'factor': 1.3}, FadeOut(tallies))
        self.wait()
        anims = []
...
class MorphBrickRowIntoHistogram3(GenericMorphBrickRowIntoHistogram):
    CONFIG = {'level': 3, 'prob_denominator': 8, 'bar_width': 2.0, 'bar_anchor_height': -3.0, 'show_tallies': True, 'show_nb_flips': False}
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(total_area_text))
        self.wait()
        ...
        for i, rect in enumerate(covering_rects):
            ...
            self.play(FadeIn(rect, rate_func=linear), ChangeDecimalToValue(area_decimal, cumulative_areas[i], rate_func=linear))
            self.wait(0.2)
        self.wait()
        total_area_rect = SurroundingRectangle(total_area_group, buff=MED_SMALL_BUFF, stroke_color=area_color)
...
class MorphBrickRowIntoHistogram20(GenericMorphBrickRowIntoHistogram):
    CONFIG = {'level': 20, 'prob_ticks': 0.05, 'bar_width': 0.5, 'bar_anchor_height': -3.0, 'x_ticks': 5}
