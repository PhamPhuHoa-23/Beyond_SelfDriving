...
class MillionFlips(Scene):
    ...
    def construct(self):
        ...
        hundred_boxes = VGroup(*[Square(stroke_width=1, stroke_color=WHITE, fill_opacity=1, fill_color=interpolate_color(COLOR_HEADS, COLOR_TAILS, prop)) for prop in proportions])
        self.organize_group(hundred_boxes)
        ten_k_proportions = np.random.normal(0.5, 0.5 * 0.01, 100)
        ten_k_boxes = VGroup(*[Square(stroke_width=1, stroke_color=WHITE, fill_opacity=1, fill_color=interpolate_color(COLOR_HEADS, COLOR_TAILS, prop)) for prop in ten_k_proportions])
        ...
        self.play(FadeIn(hundred_boxes[0]), coins.set_stroke, {'width': 0}, coins.replace, hundred_boxes[0])
        ...
        for box, prop in list(zip(hundred_boxes, proportions))[1:]:
            ...
            self.wait(small_wait_time)
        self.play(FadeIn(ten_k_boxes[0]), hundred_boxes.set_stroke, {'width': 0}, hundred_boxes.replace, ten_k_boxes[0])
...
class PropHeadsWithinThousandth(Scene):
    ...
    def construct(self):
        prob = OldTex('P(499{,}000 \\le', '\\# \\text{H}', '\\le 501{,}000)', '\\approx', '0.9545')
...
class PropHeadsWithinHundredth(Scene):
    ...
    def construct(self):
        prob = OldTex('P(490{,}000 \\le', '\\# \\text{H}', '\\le 510{,}000)', '\\approx', '0.99999999\\dots')
        prob[1].set_color(RED)
        prob[-1].set_color(YELLOW)
        self.add(prob)