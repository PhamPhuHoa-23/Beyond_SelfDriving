...
class ParametricDisc(Sphere):
    CONFIG = {'u_min': 0, 'u_max': 1, 'stroke_width': 0, 'checkerboard_colors': [BLUE_D]}

    def func(self, u, v):
        return np.array([u * np.cos(v), u * np.sin(v), 0])

class SphereCylinderScene(SpecialThreeDScene):
    CONFIG = {'cap_config': {'stroke_width': 1, 'stroke_color': WHITE, 'fill_color': BLUE_D, 'fill_opacity': 1}}
    ...
    def get_cylinder_caps(self):
        ...
        caps = VGroup(*[Circle(radius=R, **self.cap_config).shift(R * vect) for vect in [IN, OUT]])
...
class AskAboutShadowRelation(SpecialThreeDScene):
    CONFIG = {'R_color': YELLOW, 'space_out_factor': 1.15}
    ...
    def show_surface_area(self):
        ...
        self.play(Write(sphere, stroke_width=1), FadeInFromDown(sa_equation))
        self.play(LaggedStartMap(UpdateFromAlphaFunc, sphere, lambda mob: (mob, lambda m, a: m.set_fill(color=interpolate_color(BLUE_E, YELLOW, a), opacity=interpolate(0.5, 1, a))), rate_func=there_and_back, lag_ratio=0.1, run_time=4))
        self.play(sphere.scale, 0.75, sphere.shift, 3 * LEFT, sa_equation.shift, 3 * LEFT)
        self.wait(2)
        self.sphere = sphere
    ...
    def show_four_circles(self):
        sphere = self.sphere
        shadow = Circle(radius=sphere.get_width() / 2, stroke_color=WHITE, stroke_width=1, fill_color=BLUE_E, fill_opacity=1)
        ...
        area_labels = VGroup(*[area_label.copy().move_to(interpolate(mob.get_center(), mob.get_top(), 0.5)) for mob in shadows])
    ...
    def show_all_pieces(self):
        ...
        for shadow in shadows:
            pieces = ParametricSurface(func=lambda u, v: np.array([u * np.cos(TAU * v), u * np.sin(TAU * v), 0]), resolution=(12, 24))
    ...
    def show_area_pieces(self, *mobjects):
        ...
        self.play(*[LaggedStartMap(ApplyMethod, mob, lambda m: (m.set_fill, YELLOW, 1), rate_func=there_and_back, lag_ratio=0.25, run_time=1.5) for mob in mobjects])
...
class ButWhy(TeacherStudentsScene):
    CONFIG = {'student_mode': 'raise_right_hand', 'question': 'But why?', 'teacher_mode': 'guilty'}

    def construct(self):
        for student in self.students:
            student.change('pondering', self.screen)
        self.student_says('But why?', index=2, target_mode=self.student_mode, bubble_config={'direction': LEFT})
...
class PreviewTwoMethods(MovingCameraScene):
    ...
    def construct(self):
        thumbnails = Group(ImageMobject('SphereSurfaceProof1'), ImageMobject('SphereSurfaceProof2'))
...
class MapSphereOntoCylinder(SphereCylinderScene):
    ...
    def construct(self):
        ...
        self.move_camera(**self.default_angled_camera_position, run_time=2)
        self.wait(2)
        ...
        stroke_width = 5
        top_line = Line(PI * radius * LEFT + radius * OUT, PI * radius * RIGHT + radius * OUT, color=YELLOW, stroke_width=stroke_width)
        side_line = Line(PI * radius * LEFT + radius * OUT, PI * radius * LEFT + radius * IN, color=RED, stroke_width=stroke_width)
...
class UnfoldCircles(Scene):
    CONFIG = {'circle_style': {'fill_color': GREY_BROWN, 'fill_opacity': 0, 'stroke_color': GREY_BROWN, 'stroke_width': 2}, 'radius': 1.0, 'dr': 0.01}
    ...
    def show_rectangle_with_formula(self):
        ...
        self.play(TransformFromCopy(two_R[0], pre_area_label[0]), TransformFromCopy(two_R[1], pre_area_label[-1]), TransformFromCopy(two_pi_R, pre_area_label[1:-1]))
        self.wait()
        self.play(ReplacementTransform(pre_area_label[:2], area_label[:1]), ReplacementTransform(pre_area_label[2], area_label[1]), ReplacementTransform(pre_area_label[3:], area_label[2:]))
        self.wait()
    ...
    def add_four_circles(self):
        ...
        def get_circle(line):
            return always_redraw(lambda: self.get_unwrapped_circle(radius=radius, dr=self.dr, unwrap_factor=unwrap_factor_tracker.get_value(), center=line.get_top()))
        circles = VGroup(*[get_circle(line) for line in radii_lines])
        circle_copies = circles.copy()
        for mob in circle_copies:
            mob.clear_updaters()
        self.play(LaggedStartMap(Write, circle_copies, lag_ratio=0.7), LaggedStartMap(Write, R_labels), LaggedStartMap(ShowCreation, radii_lines))
        self.remove(circle_copies)
        self.add(circles, radii_lines, R_labels)
        self.wait()
        self.play(radii_lines[2:].shift, 2.9 * RIGHT, R_labels[2:].shift, 2.9 * RIGHT, VGroup(radii_lines[:2], R_labels[:2]).to_edge, LEFT, {'buff': 1.0})
    ...
    def get_unwrapped_circle(self, radius, dr, unwrap_factor=0, center=ORIGIN):
        ...
        for r in radii:
            ...
            ring = Arc(start_angle=-90 * DEGREES, angle=angle, radius=arc_radius, **self.circle_style)
...
class ShowProjection(SphereCylinderScene):
    ...
    def focus_on_one(self):
        ...
        lhs.save_state()
        rhs.save_state()
        self.play(FadeIn(ghost_cylinder), FadeOut(pieces), FadeIn(original_rect))
        ...
        self.play(Restore(lhs), Restore(rhs), FadeInFromDown(equals))
        self.wait(5)
    ...
    def label_sides(self):
        ...
        lat_lines = VGroup(*[ParametricCurve(lambda t: radius * sphere.func(u, t), t_min=sphere.v_min, t_max=sphere.v_max) for u in u_values])
        lon_lines = VGroup(*[ParametricCurve(lambda t: radius * sphere.func(t, v), t_min=sphere.u_min, t_max=sphere.u_max) for v in v_values])
    ...
    def show_length_scaled_up(self):
        ...
        self.stop_ambient_camera_rotation()
        self.move_camera(phi=65 * DEGREES, theta=-80 * DEGREES, added_anims=[ghost_sphere.set_stroke, {'opacity': 0.1}, lines.set_stroke, {'width': 3}])
        ...
        for eg1, eg2 in zip(all_example_groups, all_example_groups[1:]):
            ...
            self.remove(eg1)
            self.play(ReplacementTransform(eg1.deepcopy(), eg2), Transform(equation, self.get_equation(eg2[2])))
            if eg1 is example_group:
                self.move_camera(phi=0, theta=-90 * DEGREES)
            self.show_length_stretch_of_rect(eg2)
        self.play(ReplacementTransform(all_example_groups[-1], example_group), Restore(equation))
    ...
    def show_length_stretch_of_rect(self, example_group):
        ...
        lines.set_shade_in_3d(True)
        proj_lines_to_fade = VGroup(proj_lines[0], proj_lines[3])
        ...
        self.play(FadeOut(lines), FadeIn(rects), FadeIn(proj_lines_to_fade))
    ...
    def show_height_scaled_down(self):
        ...
        equation = self.equation
        r1, lines, r2 = example_group
        to_fade = VGroup(*[mob for mob in it.chain(ghost_sphere, ghost_cylinder) if np.dot(mob.get_center(), [1, 1, 0]) < 0])
        to_fade.save_state()
        new_example_groups = [self.get_example_group([1, -1, z]) for z in [6, 0.25]]
        for eg in new_example_groups:
        ...
        self.move_camera(phi=85 * DEGREES, theta=-135 * DEGREES, added_anims=[lines.set_stroke, {'width': 1}, r1.set_stroke, YELLOW, 2, 1, r2.set_stroke, YELLOW, 2, 1])
        self.show_shadow(example_group)
        for eg1, eg2 in zip(all_example_groups, all_example_groups[1:]):
            self.remove(*eg1.get_family())
            self.play(ReplacementTransform(eg1.deepcopy(), eg2), Transform(equation, self.get_equation(eg2[2])))
            self.show_shadow(eg2)
        self.move_camera(phi=70 * DEGREES, theta=-115 * DEGREES, added_anims=[ReplacementTransform(all_example_groups[-1], example_group), Restore(equation)])
        self.begin_ambient_camera_rotation()
    ...
    def get_projection_lines(self, piece):
        ...
        for corner in piece.get_anchors()[:-1]:
            ...
            kwargs = {'color': YELLOW, 'stroke_width': 0.5}
...
class SlantedShadowSquishing(ShowShadows):
    CONFIG = {'num_reorientations': 4, 'random_seed': 2}
...
class JustifyLengthStretch(ShowProjection):
    CONFIG = {'R_color': RED, 'd_color': WHITE, 'd_ambiguity_iterations': 4}
    ...
    def cut_cross_section(self):
        ...
        self.stop_ambient_camera_rotation()
        self.play(Rotate(to_fade, -PI, axis=OUT, about_point=sphere.get_left(), run_time=2), VFadeOut(to_fade, run_time=2), FadeIn(circle))
    ...
    def label_R(self):
        ...
        self.play(ShowCreation(R_line), FadeIn(R_label, IN))
        self.wait()
        self.R_line = R_line
    ...
    def label_d(self):
        ...
        self.play(ShowCreation(d_line), FadeIn(d_label, IN))
        self.wait()
        for x in range(self.d_ambiguity_iterations):
            to_fade_out = [d_line, alt_d_line][x % 2]
            to_fade_in = [d_line, alt_d_line][(x + 1) % 2]
            self.play(FadeIn(to_fade_in), FadeOut(to_fade_out), d_label.next_to, to_fade_in, IN)
        self.d_line = d_line
    ...
    def show_similar_triangles(self):
        ...
        equation = VGroup(triangle.copy().rotate(-3 * DEGREES), OldTex('\\sim'), big_triangle.copy().rotate(-3 * DEGREES))
        ...
        self.move_camera(phi=40 * DEGREES, theta=-85 * DEGREES, added_anims=[d_label.next_to, d_line, DOWN, SMALL_BUFF, d_line.set_stroke, {'width': 1}, FadeOut(proj_lines[0]), FadeOut(proj_lines[3]), FadeOut(R_label)], run_time=2)
        ...
        self.play(FadeIn(equation[0], 7 * RIGHT + 2.5 * DOWN), FadeIn(equation[1:]), FadeInFromDown(eq_d), FadeInFromDown(eq_R), Animation(self.camera.phi_tracker))
        self.wait()
        for x in range(2):
        ...
        self.play(ReplacementTransform(triangle, big_triangle), FadeIn(R_label), Animation(self.camera.phi_tracker))
        self.wait(3)
        self.move_camera(phi=70 * DEGREES, theta=-70 * DEGREES, added_anims=[big_triangle.set_fill, {'opacity': 0.25}, d_label.next_to, d_line, IN, {'buff': 0.3}])
        self.begin_ambient_camera_rotation()
        ...
        self.play(Rotate(lost_hemisphere, PI, axis=OUT, about_point=left_point, run_time=2), VFadeIn(lost_hemisphere), FadeOut(self.circle), R_line.set_color, self.R_color)
        self.wait(10)
...
class JustifyLengthStretchHigherRes(JustifyLengthStretch):
    CONFIG = {'sphere_config': {'resolution': (2 * 24, 2 * 48)}}

class JustifyLengthStretchHighestRes(JustifyLengthStretch):
    CONFIG = {'sphere_config': {'resolution': (4 * 24, 4 * 48)}}

class ProTipNameThings(Scene):
    CONFIG = {'tip_descriptor': '(Deceptively simple) problem solving tip:', 'tip': 'Start with names'}

    def construct(self):
        words = OldTexText(self.tip_descriptor, self.tip)
...
class TinierAndTinerRectangles(SphereCylinderScene):
    ...
    def construct(self):
        spheres = [self.get_sphere(resolution=(12 * 2 ** n, 24 * 2 ** n), stroke_width=0) for n in range(self.n_iterations)]
        self.set_camera_to_default_position()
        self.add(self.get_axes())
        self.begin_ambient_camera_rotation()
        self.add(spheres[0])
...
class LimitViewToCrossSection(JustifyLengthStretch):
    CONFIG = {'d_ambiguity_iterations': 0}
    ...
    def construct(self):
        ...
        self.label_d()
        self.move_camera(phi=90 * DEGREES, theta=-90 * DEGREES)
        self.play(FadeOut(self.ghost_sphere), FadeOut(self.ghost_cylinder))

class JustifyHeightSquish(MovingCameraScene):
    CONFIG = {'top_phi': 0.5242654422649652, 'low_phi': 0.655081802831207, 'radius': 2, 'R_color': RED, 'd_color': WHITE, 'little_triangle_color': BLUE, 'big_triangle_color': GREY_BROWN, 'alpha_color': WHITE, 'beta_color': WHITE}
    ...
    def recreate_cross_section(self):
        axes = Axes(axis_config={'unit_size': 2})
        circle = Circle(radius=self.radius, stroke_color=PINK, stroke_width=2)
        ...
        s_rect_line, c_rect_line = [Line(*[l.get_end() for l in lines], stroke_color=YELLOW, stroke_width=2) for lines in [d_lines, proj_lines]]
        mobjects = [axes, circle, R_line, d_line, R_label, d_label, proj_lines, s_rect_line, c_rect_line]
        self.add(*mobjects)
    ...
    def show_little_triangle(self):
        ...
        self.play(frame.scale, scale_factor, frame.move_to, lil_tri, self.d_label.scale, d_sf, {'about_edge': UP}, *map(MoveToTarget, stroke_width_changers))
        self.play(DrawBorderThenFill(lil_tri, stroke_width=0.5))
        self.wait()
        self.play(ShowCreation(hyp), LaggedStartMap(DrawBorderThenFill, hyp_word, stroke_width=0.5, run_time=1))
        self.wait()
        self.play(TransformFromCopy(hyp, leg))
        self.play(TransformFromCopy(hyp_word, leg_word, path_arc=-PI / 2))
        self.wait()
        self.play(frame.restore, self.d_label.scale, 1 / d_sf, {'about_edge': UP}, *map(Restore, stroke_width_changers), run_time=3)
    ...
    def show_tangent_to_radius(self):
        ...
        self.wait()
        self.play(Rotate(R_line, 90 * DEGREES - phi, about_point=ORIGIN), R_label.next_to, 0.5 * circle_point, DR, {'buff': 0}, d_label.shift, SMALL_BUFF * UL)
    ...
    def tweak_parameter(self):
        ...
        R_line = self.R_line
        frame = self.camera_frame
        to_fade = VGroup(self.proj_lines, self.s_rect_line, self.c_rect_line, self.hyp_word, self.leg_word, lil_tri.hyp, lil_tri.leg2)
        rad_tangent = VGroup(R_line, self.tangent, self.elbow)
        phi_tracker = ValueTracker(self.low_phi)
        self.play(frame.scale, 0.6, frame.shift, UR, R_label.scale, 0.6, {'about_edge': UL}, d_label.scale, 0.6, {'about_point': d_label.get_top() + SMALL_BUFF * DOWN}, *map(FadeOut, to_fade))
    ...
    def label_angles(self):
        ...
        alpha_arc = Arc(start_angle=90 * DEGREES, angle=-alpha, radius=0.2, stroke_width=2)
        beta_arc = Arc(start_angle=PI, angle=beta, radius=0.2, stroke_width=2)
        ...
        self.play(FadeOut(d_label), FadeOut(R_label), ShowCreation(alpha_arc), ShowCreation(beta_arc))
        self.wait()
        ...
        self.play(ShowCreation(elbow))
        self.wait()
        self.play(LaggedStartMap(MoveToTarget, movers), LaggedStartMap(FadeInFromDown, equation[1:4:2]))
        self.wait()
        ...
        stroke_width_changers = VGroup(*[mob for mob in self.mobjects if mob not in [beta_arc, beta_label, frame, equation]])
        ...
        self.play(ApplyMethod(VGroup(frame, equation).scale, 0.15, {'about_point': circle_point + 0.1 * LEFT}), ApplyMethod(beta_label.scale, 0.3, {'about_point': circle_point}), scaled_arcs.set_stroke, {'width': 0.3}, scaled_arcs.scale, 0.3, {'about_point': circle_point}, *map(MoveToTarget, stroke_width_changers))
        Tex.CONFIG['background_stroke_width'] = 0
        words = VGroup(self.hyp_word, self.leg_word)
        alpha_arc1 = Arc(start_angle=90 * DEGREES + beta, angle=0.95 * alpha, radius=0.3 * 0.2, stroke_width=beta_arc.get_stroke_width()).shift(circle_point)
        alpha_arc2 = Arc(start_angle=0, angle=-0.95 * alpha, radius=0.3 * 0.2, stroke_width=beta_arc.get_stroke_width()).shift(lil_tri.hyp.get_end())
        beta_arc1 = Arc(start_angle=90 * DEGREES, angle=beta, radius=0.3 * 0.2, stroke_width=beta_arc.get_stroke_width()).shift(circle_point)
        ...
        self.play(ShowCreationThenFadeAround(equation, surrounding_rectangle_config={'buff': 0.015, 'stroke_width': 0.5}))
        self.wait()
        self.play(ShowCreation(alpha_arc2), FadeIn(alpha_label2))
        self.play(ShowCreation(beta_arc1), FadeIn(beta_label1))
        self.wait()
    ...
    def get_double_triangle_group(self, phi, d_phi):
        ...
        p2 = np.array(p1)
        p2[0] = p0[0]
        little_triangle = Polygon(p0, p1, p2, stroke_width=0, fill_color=self.little_triangle_color, fill_opacity=1)
        big_triangle = Polygon(p0, ORIGIN, p0 - p0[0] * RIGHT, stroke_width=0, fill_color=self.big_triangle_color, fill_opacity=1)
...
class ProTipTangentRadii(ProTipNameThings):
    CONFIG = {'tip_descriptor': 'Pro tip:', 'tip': "A circle's tangent is perpendicular to its radius"}

class ProTipTweakParameters(ProTipNameThings):
    CONFIG = {'tip_descriptor': 'Pro tip:', 'tip': 'Tweak parameters $\\rightarrow$ make'}
...
class WhyAreWeDoingThis(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('Hang on, what \\\\ are we doing?', index=2, bubble_config={'direction': LEFT}, target_mode='hesitant')
        self.play_student_changes('maybe', 'pondering', 'hesitant', added_anims=[self.teacher.change, 'tease'])
        self.wait(3)
        self.play(RemovePiCreatureBubble(self.students[2]), self.teacher.change, 'raise_right_hand', self.play_student_changes(*2 * ['pondering']))
        self.look_at(self.screen)
        self.wait(2)

class SameEffectAsRotating(Scene):
    CONFIG = {'rectangle_config': {'height': 2, 'width': 1, 'stroke_width': 0, 'fill_color': YELLOW, 'fill_opacity': 1, 'background_stroke_width': 2, 'background_stroke_color': BLACK}, 'x_stretch': 2, 'y_stretch': 0.5}
    ...
    def construct(self):
        ...
        low_brace = always_redraw(lambda: Brace(moving_rect, DOWN, buff=SMALL_BUFF, min_num_quads=2))
        right_brace = always_redraw(lambda: Brace(moving_rect, RIGHT, buff=SMALL_BUFF, min_num_quads=2))
        ...
        self.add(low_brace)
        self.play(moving_rect.match_width, rect2, {'stretch': True}, FadeIn(times_R_over_d))
        self.add(right_brace)
        self.play(moving_rect.match_height, rect2, {'stretch': True}, FadeIn(times_d_over_R))
        self.wait()
...
class NotSameEffectAsRotating(SameEffectAsRotating):
    CONFIG = {'rectangle_config': {'width': 1.5, 'height': 1.5}}
...
class ShowParameterization(Scene):
    ...
    def construct(self):
        u_color = PINK
        v_color = GREEN
        tex_kwargs = {'tex_to_color_map': {'u': u_color, 'v': v_color}}
        vector = Matrix([['\\text{cos}(u)\\text{sin}(v)'], ['\\text{sin}(u)\\text{sin}(v)'], ['\\text{cos}(v)']], element_to_mobject_config=tex_kwargs, element_alignment_corner=DOWN)
        vector.to_edge(UP)
        ranges = VGroup(OldTex('0 \\le u \\le 2\\pi', **tex_kwargs), OldTex('0 \\le v \\le \\pi', **tex_kwargs), OldTexText('Sample $u$ and $v$ with \\\\ the same density', tex_to_color_map={'$u$': u_color, '$v$': v_color}))
        ranges.arrange(DOWN)
...
class RotateAllPiecesWithExpansion(ShowProjection):
    CONFIG = {'sphere_config': {'radius': 1.5}, 'with_expansion': True}
...
class ThinkingCritically(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(randy.change, 'hesitant', 2 * UP)
        self.wait()
...
class RectangulatedSphere(SphereCylinderScene):
    CONFIG = {'sphere_config': {'resolution': (10, 20)}, 'uniform_color': False, 'wait_time': 10}
...
class SmoothSphere(RectangulatedSphere):
    CONFIG = {'sphere_config': {'resolution': (200, 400)}, 'uniform_color': True, 'wait_time': 0}

class SequenceOfSpheres(SphereCylinderScene):

    def construct(self):
        n_shapes = 4
        spheres, cylinders = groups = VGroup(*[VGroup(*[func(resolution=(n, 2 * n)) for k in range(1, n_shapes + 1) for n in [3 * 2 ** k]]) for func in [self.get_sphere, self.get_cylinder]])
        groups.scale(0.5)
        for group in groups:
            for shape in group:
                for piece in shape:
        ...
        final_arrows = VGroup(arrow_groups[0][-1], arrow_groups[1][-1])
        ...
        tilted_final_arrows = VGroup(final_arrows[0].copy().rotate(-45 * DEGREES, axis=DOWN).shift(0.75 * IN), final_arrows[1].copy().rotate(45 * DEGREES, axis=DOWN).shift(0.75 * OUT))
        final_q_mark = q_marks[0].copy()
        final_q_mark.move_to(q_marks)
        self.set_camera_orientation(phi=80 * DEGREES, theta=-90 * DEGREES)
        for i in range(n_shapes):
            anims = [FadeIn(spheres[i], LEFT), FadeIn(cylinders[i], LEFT)]
            if i > 0:
        ...
        self.wait()
        self.play(Transform(final_arrows, tilted_final_arrows), Transform(q_marks, VGroup(final_q_mark)))
        self.wait()
...
class WhatIsSurfaceArea(SpecialThreeDScene):
    ...
    def construct(self):
        ...
        self.set_camera_to_default_position()
        self.begin_ambient_camera_rotation()
        self.play(LaggedStartMap(DrawBorderThenFill, pieces, lag_ratio=0.2))
        self.remove(pieces)
        self.add(surface)
    ...
    def get_surface(self, radius, amplitude, power):
        ...
        return ParametricSurface(lambda u, v: radius * np.array([v * np.cos(TAU * u), v * np.sin(TAU * u), 0]) + amplitude * np.array([0, 0, v ** 2 * alt_pow(np.sin(5 * TAU * u), power)]), resolution=(100, 20), v_min=0.01)

class AltWhatIsSurfaceArea(WhatIsSurfaceArea):
    CONFIG = {'change_power': False}

    def get_surface(self, radius, amplitude, power):
        return ParametricSurface(lambda u, v: radius * (1 - 0.8 * (v ** 2) ** power) * np.array([np.cos(TAU * u) * (1 + 0.5 * v * np.sin(5 * TAU * u)), np.sin(TAU * u) * (1 + 0.5 * v * np.sin(5 * TAU * u)), v]), v_min=-1, v_max=1, resolution=(100, 25))
...
class RoleOfCalculus(SpecialThreeDScene):
    CONFIG = {'camera_config': {'light_source_start_point': [-4, 5, 7]}}
    ...
    def construct(self):
        ...
        self.wait()
        self.play(sphere.next_to, ORIGIN, LEFT, 1.0, arrow.move_to, ORIGIN, LEFT, calc.next_to, ORIGIN, RIGHT, 2.25, FadeOut(cross), path_arc=PI, run_time=2)
        self.wait()

class UnwrappedCircleLogic(UnfoldCircles):
    CONFIG = {'radius': 1.25, 'dr': 0.01}
    ...
    def construct(self):
        ...
        def get_unwrapped_circle():
            result = self.get_unwrapped_circle(radius=radius, dr=dr, unwrap_factor=unwrap_factor_tracker.get_value(), center=center_tracker.get_center())
        ...
        self.play(unwrap_factor_tracker.set_value, 1, run_time=2)
        self.play(center_tracker.move_to, circle.get_right() + (radius + MED_SMALL_BUFF) * RIGHT, circle_group.set_stroke, {'opacity': 1})
        self.wait()
        r_line = always_redraw(lambda: Line(ORIGIN, get_r() * DOWN, stroke_width=2, stroke_color=WHITE).shift(circle.get_center()))
        r_label = OldTex('r')
        r_label.add_updater(lambda m: m.next_to(r_line, LEFT, SMALL_BUFF))
        two_pi_r_label = OldTex('2\\pi r')
        two_pi_r_label.add_updater(lambda m: m.next_to(self.get_submob_from_prop(unwrapped_circle, get_highlight_prop()), DOWN, SMALL_BUFF))
        circle.add_updater(lambda m: m.match_style(unwrapped_circle))
        self.play(ReplacementTransform(R_line, r_line), ReplacementTransform(R_label, r_label), FadeInFromDown(two_pi_r_label.copy().clear_updaters(), remover=True))
        ...
        self.play(ShowCreation(line), Write(line_word))
        self.wait()
        self.play(highligt_prop_tracker.set_value, 1)
        self.wait()
        line.save_state()
        line_word.save_state()
        self.play(Transform(line, curve), Transform(line_word, not_line))
        self.wait()
        self.play(Restore(line), Restore(line_word))
        self.wait()
...
class AskAboutDirectConnection(TeacherStudentsScene, SpecialThreeDScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.change, 'raise_right_hand', self.change_students(*3 * ['pondering'], look_at=group), LaggedStartMap(FadeInFromDown, group), LaggedStartMap(GrowArrow, arrows))
        self.wait()
        self.play(self.teacher.change, 'pondering', self.students[2].change, 'raise_right_hand', GrowArrow(direct_arrow), LaggedStartMap(FadeInFrom, q_marks, lambda m: (m, UP), lag_ratio=0.8, run_time=1.5))
        self.play_student_changes('erm', 'sassy', 'raise_right_hand')
        self.wait(2)
        self.look_at(group)
        self.wait(2)
...
class NobodyLikesHomework(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play_all_student_changes('tired', look_at=8 * RIGHT + 4 * DOWN, added_anims=[self.teacher.change, 'tease'])
        self.wait(2)
...
class SecondProof(SpecialThreeDScene):
    CONFIG = {'sphere_config': {'resolution': (30, 30)}, 'n_random_subsets': 12}
    ...
    def divide_into_rings(self):
        ...
        self.play(rings.space_out_submobjects, 1.5, rate_func=there_and_back_with_pause, run_time=3)
        self.wait(2)
        rings.save_state()
    ...
    def show_shadows(self):
        ...
        for piece in shadows.family_members_with_points():
            piece.set_stroke(piece.get_fill_color(), width=0.5)
        for shadow in shadows:
            shadow.save_state()
        ...
        self.play(Rotating(radial_line, angle=TAU, about_point=ORIGIN, rate_func=smooth, run_time=3))
        self.wait()
    ...
    def correspond_to_every_other_ring(self):
        ...
        self.move_camera(phi=70 * DEGREES, theta=-135 * DEGREES, added_anims=[FadeOut(self.R_label), FadeOut(self.radial_line)], run_time=2)
        shadows_copy = shadows.copy()
        shadows.fade(1)
        self.play(ReplacementTransform(shadows_copy, every_other_ring), FadeOut(self.ghost_rings), run_time=2)
        self.wait(5)
    ...
    def cut_cross_section(self):
        ...
        self.play(FadeIn(back_half), FadeIn(front_half_ghost), FadeIn(circle), FadeOut(shadows), FadeOut(every_other_ring_copy))
        self.wait()
    ...
    def show_theta(self):
        ...
        for alpha in [angle, 0, PI, angle]:
            self.play(theta_tracker.set_value, alpha, theta_mob_opacity_tracker.set_value, 1, Animation(self.camera.phi_tracker), run_time=2)
            self.wait()
        radius = self.sphere_config['radius']
        ...
        brace.rotate(get_theta() + d_theta / 2, axis=UP, about_point=ORIGIN)
        ...
        self.play(TransformFromCopy(theta_group[1], alt_R_line), GrowFromCenter(brace), Animation(self.camera.phi_tracker))
        self.wait()
        self.move_camera(phi=90 * DEGREES, theta=-90 * DEGREES)
        self.wait()
        self.play(FadeIn(brace_label, IN))
        self.play(ShowCreation(radial_line), FadeIn(R_label))
        self.wait()
        self.move_camera(phi=70 * DEGREES, theta=-70 * DEGREES)
        self.wait(3)
        self.set_variables_as_attrs(theta_tracker, lit_ring, theta_group, brace, brace_label, d_theta, alt_R_line, theta_mob_opacity_tracker)
    ...
    def ask_about_ring_circumference(self):
        ...
        self.play(ShowCreation(circle))
        self.wait()
        self.move_camera(phi=70 * DEGREES, theta=-70 * DEGREES, added_anims=[FadeIn(to_fade), FadeOut(circle)])
        self.wait()
    ...
    def ask_about_shadow_area(self):
        ...
        self.play(ReplacementTransform(lit_ring_copy, shadow), FadeOut(self.R_label), FadeOut(self.radial_line), Animation(self.camera.phi_tracker), *map(ShowCreation, dashed_lines), run_time=2)
        self.wait(2)
        self.set_variables_as_attrs(dashed_lines, lit_shadow=shadow)
    ...
    def ask_about_2_to_1_correspondance(self):
        ...
        for angle in [PI, 0, PI]:
            self.play(theta_tracker.set_value, angle, Animation(self.camera.phi_tracker), run_time=3)
    ...
    def show_all_shadow_rings(self):
        ...
        to_fade = VGroup(theta_group_copy, self.alt_R_line, self.brace, self.brace_label, lit_ring_copy, self.lit_shadow, self.slice_circle, self.dashed_lines)
        R_label = self.R_label
        radial_line = self.radial_line
        ...
        self.play(FadeOut(to_fade), LaggedStartMap(FadeIn, shadows), self.theta_mob_opacity_tracker.set_value, 0)
        self.play(LaggedStartMap(Restore, shadows), ApplyMethod(self.camera.phi_tracker.set_value, 60 * DEGREES), ApplyMethod(self.camera.theta_tracker.set_value, -130 * DEGREES), run_time=3)
        self.play(ShowCreation(radial_line), FadeIn(R_label), Animation(self.camera.phi_tracker))
        self.begin_ambient_camera_rotation()
        self.wait()
        rings = self.rings
    ...
    def set_ring_colors(self, rings, colors=[BLUE_E, BLUE_D]):
        for i, ring in enumerate(rings):
            ...
            for piece in ring:
                ...
                piece.set_points([*piece.get_points()[3:-1], *piece.get_points()[:3], piece.get_points()[3]])
        return rings
    ...
    def get_theta_group(self, theta):
        arc = Arc(start_angle=90 * DEGREES, angle=-theta, radius=0.5)
...
class SecondProofHigherRes(SecondProof):
    CONFIG = {'sphere_config': {'resolution': (60, 60)}}

class SecondProofHighestRes(SecondProof):
    CONFIG = {'sphere_config': {'resolution': (120, 120)}}
...
class RangeFrom0To180(Scene):
    ...
    def construct(self):
        ...
        self.add(angle)
        self.wait()
        self.play(ChangeDecimalToValue(angle, 180, run_time=2))
        self.wait()

class Question1(Scene):

    def construct(self):
        kwargs = {'tex_to_color_map': {'circumference': RED}}
        ...
        self.play(FadeOut(question, UP), FadeInFromDown(prompt), question.circum.replace, prompt.circum, run_time=1.5)
        self.wait()

class YouCouldIntegrate(TeacherStudentsScene):

    def construct(self):
        self.student_says('Integrate?', index=2, bubble_config={'direction': LEFT})
...
class Question4(Scene):
    ...
    def construct(self):
        question = OldTexText('Question \\#4:', 'Explain how the shadows relate to\\\\every other ring on the sphere.', tex_to_color_map={'shadows': YELLOW, 'every other ring': BLUE})
        self.add(question[0])
        self.wait()
...
class SpherePatronThanks(Scene):
    ...
    def show_columns(self):
        ...
        self.wait()
        for k in range(4):
            self.play(FadeOut(columns[k]), FadeIn(columns[k + 4]))
        self.wait()
...
class Thumbnail(SpecialThreeDScene):
    CONFIG = {'camera_config': {'light_source_start_point': [-10, 5, 7]}}
