...
def get_fourier_graph(axes, time_func, t_min, t_max, n_samples=NUM_SAMPLES_FOR_FFT, complex_to_real_func=lambda z: z.real, color=RED):
    ...
    graph = VMobject()
    graph.set_points_smoothly([axes.coords_to_point(x, complex_to_real_func(y) / n_samples) for x, y in zip(frequencies, fft_output[:n_samples // 2])])
    graph.set_color(color)
    f_min, f_max = [axes.x_axis.point_to_number(graph.get_points()[i]) for i in (0, -1)]
    graph.underlying_function = lambda f: axes.y_axis.point_to_number(graph.point_from_proportion((f - f_min) / (f_max - f_min)))
    return graph

def get_fourier_transform(func, t_min, t_max, complex_to_real_func=DEFAULT_COMPLEX_TO_REAL_FUNC, use_almost_fourier=USE_ALMOST_FOURIER_BY_DEFAULT, **kwargs):
...
class Introduction(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.student_thinks("What's that?", look_at=title, target_mode='confused', index=1)
        self.play(GrowArrow(arrow), ReplacementTransform(graph.copy(), fourier_graph))
        self.wait(2)
        self.student_thinks('Pssht, I got this', target_mode='tease', index=2, added_anims=[RemovePiCreatureBubble(self.students[1])])
...
class OtherContexts(PiCreatureScene):
    ...
    def construct(self):
        items = VGroup(*list(map(TexText, ['Extracting frequencies from sound', 'Uncertainty principle', 'Riemann Zeta function and primes', 'Differential equations'])))
        ...
        self.add(items)
        for item in items[1:]:
            self.play(LaggedStartMap(ApplyMethod, item, lambda m: (m.set_fill, {'opacity': 1})), morty.change, 'thinking')
            self.wait()

class TODOInsertCosineWrappingAroundCircle(TODOStub):
    CONFIG = {'message': 'Give a picture-in-picture \\\\ of cosine wrapping around circle'}

class AddingPureFrequencies(PiCreatureScene):
    CONFIG = {'A_frequency': 2.1, 'A_color': YELLOW, 'D_color': PINK, 'F_color': TEAL, 'C_color': RED, 'sum_color': GREEN, 'equilibrium_height': 1.5}
    ...
    def play_A440(self):
        ...
        self.broadcast(FadeIn(A_label), Succession(ApplyMethod, randy.change, 'pondering', Animation, randy, Blink, randy))
        self.set_variables_as_attrs(A_label)

    def measure_air_pressure(self):
        randy = self.pi_creature
        axes = Axes(y_min=-2, y_max=2, x_min=0, x_max=10, axis_config={'include_tip': False})
        axes.stretch_to_fit_height(2)
        axes.to_corner(UP + LEFT)
        axes.shift(LARGE_BUFF * DOWN)
        eh = self.equilibrium_height
        equilibrium_line = DashedLine(axes.coords_to_point(0, eh), axes.coords_to_point(axes.x_max, eh), stroke_width=2, stroke_color=GREY_B)
        ...
        n = 10
        brace = Brace(Line(axes.coords_to_point(n / frequency, func(n / frequency)), axes.coords_to_point((n + 1) / frequency, func((n + 1) / frequency))), UP)
        ...
        self.broadcast(ShowCreation(graph, run_time=4, rate_func=linear), ShowCreation(equilibrium_line))
        axes.add(equilibrium_line)
        self.play(randy.change, 'erm', graph, GrowFromCenter(brace), Write(words))
        self.wait()
        graph.save_state()
        self.play(FadeOut(brace), FadeOut(words), VGroup(axes, graph, axes.labels).shift, 0.8 * UP, graph.fade, 0.85, graph.shift, 0.8 * UP)
    ...
    def play_lower_pitch(self):
        ...
        self.play(FadeOut(self.A_label), GrowFromCenter(D_label))
        self.broadcast(ShowCreation(graph, run_time=4, rate_func=linear), randy.change, 'happy', n_circles=6)
    ...
    def play_mix(self):
        ...
        self.broadcast(self.get_broadcast_animation(n_circles=6), self.pi_creature.change, 'thinking', *[ShowCreation(graph, run_time=4, rate_func=linear) for graph in (self.A_graph, self.D_graph)])
        self.wait()
    ...
    def separate_out_parts(self):
        ...
        self.play(*it.chain(list(map(MoveToTarget, movers)), [ApplyMethod(mob.shift, FRAME_Y_RADIUS * DOWN, remover=True) for mob in (randy, speaker)]))
        self.wait()
    ...
    def draw_full_sum(self):
        ...
        D_graph = self.D_graph
        D_axes = self.D_axes
        rect = Rectangle(height=2.5 * FRAME_Y_RADIUS, width=MED_SMALL_BUFF, stroke_width=0, fill_color=YELLOW, fill_opacity=0.4)
        self.play(ReplacementTransform(A_graph.copy(), thin_sum_graph), ReplacementTransform(D_graph.copy(), thin_sum_graph))
        self.play(self.get_graph_line_animation(self.A_axes, self.A_graph), self.get_graph_line_animation(self.D_axes, self.D_graph), self.get_graph_line_animation(axes, sum_graph.deepcopy()), ShowCreation(sum_graph), run_time=15, rate_func=linear)
    ...
    def add_more_notes(self):
        ...
        self.play(MoveToTarget(squish_group), FadeOut(self.lines_to_fade))
        ...
        self.play(*it.chain(list(map(ShowCreation, [F_axes, C_axes, F_graph, C_graph])), list(map(Write, [F_label, C_label])), list(map(FadeOut, [self.sum_graph]))))
        self.play(ReplacementTransform(graphs.copy(), thin_new_sum_graph))
        kwargs = {'rate_func': None, 'run_time': 10}
        self.play(ShowCreation(new_sum_graph.copy(), **kwargs), *[self.get_graph_line_animation(curr_axes, graph, **kwargs) for curr_axes, graph in [(self.A_axes, self.A_graph), (self.D_axes, self.D_graph), (F_axes, F_graph), (C_axes, C_graph), (axes, new_sum_graph)]])
        self.wait()
    ...
    def get_graph_v_line(self, x, axes, graph):
        result = Line(axes.coords_to_point(x, self.equilibrium_height), graph.point_from_proportion(float(x) / axes.x_max), color=WHITE, buff=0)
        return result
...
class BreakApartSum(Scene):
    CONFIG = {'frequencies': [0.5, 1.5, 2, 2.5, 5], 'equilibrium_height': 2.0}
    ...
    def show_initial_sound(self):
        ...
        axes = Axes(x_min=0, x_max=5, y_min=-1, y_max=5, x_axis_config={'include_tip': False, 'unit_size': 2.0}, y_axis_config={'include_tip': False, 'unit_size': 0.5})
        ...
        self.add(axes, graph)
        self.play(v_line.move_to, axes.coords_to_point(5, 0), DOWN, MoveAlongPath(dot, graph), run_time=8, rate_func=linear)
    ...
    def decompose_sound(self):
        ...
        pure_graphs = VGroup(*[axes.get_graph(lambda x: 0.5 * np.cos(2 * np.pi * freq * x), num_graph_points=100) for freq in self.frequencies])
    ...
    def ponder_question(self):
        ...
        self.play(FadeIn(randy), all_graphs.scale, 0.75, all_graphs.to_corner, UP + RIGHT)
        self.play(randy.change, 'pondering', all_graphs)
        self.play(Blink(randy))
        rect = SurroundingRectangle(pure_graphs, color=WHITE)
        self.play(ShowCreation(rect), LaggedStartMap(ApplyFunction, pure_graphs, lambda g: (lambda m: m.shift(SMALL_BUFF * UP).set_color(YELLOW), g), rate_func=wiggle))
        self.play(FadeOut(rect))
        self.play(Blink(randy))
        self.wait()

class Quadrant(VMobject):
    CONFIG = {'radius': 2, 'stroke_width': 0, 'fill_opacity': 1, 'density': 50, 'density_exp': 2.0}

    def init_points(self):
        points = [r * RIGHT for r in np.arange(0, self.radius, 1.0 / self.density)]
        points += [self.radius * (np.cos(theta) * RIGHT + np.sin(theta) * UP) for theta in np.arange(0, TAU / 4, 1.0 / (self.radius * self.density))]
        points += [r * UP for r in np.arange(self.radius, 0, -1.0 / self.density)]
        self.set_points_smoothly(points)

class UnmixMixedPaint(Scene):
    CONFIG = {'colors': [BLUE, RED, YELLOW, GREEN]}

    def construct(self):
        angles = np.arange(4) * np.pi / 2
        quadrants = VGroup(*[Quadrant().rotate(angle, about_point=ORIGIN).set_color(color) for color, angle in zip(self.colors, angles)])
        ...
        self.add(quadrants)
        run_time = 30
        self.play(*[UpdateFromAlphaFunc(quadrant, update_quadrant) for quadrant in quadrants] + [ApplyMethod(mud_circle.restore, rate_func=linear)], run_time=run_time)
...
class MachineThatTreatsOneFrequencyDifferently(Scene):
    ...
    def change_graph_frequency(self, frequency, run_time=2):
        ...
        self.play(UpdateFromAlphaFunc(graph, self.get_signal_update_func(graph, frequency)), ChangingDecimal(frequency_mob, lambda a: 440 * interpolate(curr_frequency, frequency, a)), run_time=run_time)
...
class FourierMachineScene(Scene):
    ...
    def get_frequency_axes(self):
        ...
        box = SurroundingRectangle(frequency_axes, buff=MED_SMALL_BUFF, color=self.frequency_axes_box_color)
        ...
        frequency_label.next_to(frequency_axes.x_axis.get_right(), DOWN, buff=MED_LARGE_BUFF, aligned_edge=RIGHT)
    ...
    def get_fourier_transform_graph(self, time_graph, **kwargs):
        ...
        return self.frequency_axes.get_graph(get_fourier_transform(func, t_min, t_max, **kwargs), color=self.center_of_mass_color, **kwargs)
    ...
    def get_time_graph_y_vector_animation(self, graph, **kwargs):
        ...
        def update_vector(vector, alpha):
            ...
            vector.put_start_and_end_on(self.time_axes.coords_to_point(x, 0), self.time_axes.input_to_graph_point(x, graph_copy))
            return vector
    ...
    def get_vector_animations(self, graph, draw_polarized_graph=True, **kwargs):
        config = dict(self.default_y_vector_animation_config)
        config.update(kwargs)
        anims = [self.get_time_graph_y_vector_animation(graph, **config), self.get_polarized_vector_animation(graph.polarized_mobject, **config)]
    ...
    def animate_time_sweep(self, freq, n_repeats=1, t_max=None, **kwargs):
        ...
        v_line = DashedLine(ctp(0, 0), ctp(0, time_axes.y_max), stroke_width=6)
        v_line.set_color(RED)
        for x in range(n_repeats):
            v_line.move_to(ctp(0, 0), DOWN)
            self.play(ApplyMethod(v_line.move_to, ctp(t_max, 0), DOWN), self.get_polarized_animation(v_line, freq=freq), *added_anims, **config)
    ...
    def get_v_lines_indicating_periods(self, freq, n_lines=None):
        ...
        v_lines = VGroup(*[DashedLine(ORIGIN, 1.5 * UP).move_to(self.time_axes.coords_to_point(n * period, 0), DOWN) for n in range(1, n_lines + 1)])
...
class WrapCosineGraphAroundCircle(FourierMachineScene):
    CONFIG = {'initial_winding_frequency': 0.5, 'signal_frequency': 3.0}
    ...
    def show_initial_signal(self):
        ...
        v_lines = VGroup(*[DashedLine(ORIGIN, 2 * UP, color=RED).move_to(axes.coords_to_point(x, 0), DOWN) for x in (1, 2)])
        ...
        self.play(FadeIn(words), LaggedStartMap(FadeIn, braces), *list(map(ShowCreation, v_lines)))
        self.wait()
        self.play(FadeOut(VGroup(braces, v_lines)), words.restore)
        self.wait()
    ...
    def show_finite_interval(self):
        axes = self.time_axes
        v_line = DashedLine(axes.coords_to_point(0, 0), axes.coords_to_point(0, axes.y_max), color=RED, stroke_width=6)
        h_line = Line(axes.coords_to_point(0, 0), axes.coords_to_point(axes.x_max, 0))
    ...
    def change_wrapping_frequency(self):
        ...
        for target_freq in [1.23, 0.2, 0.79, 1.55, self.signal_frequency]:
            self.play(Transform(v_lines, self.get_v_lines_indicating_periods(target_freq)), ChangeDecimalToValue(freq_label, target_freq), self.get_frequency_change_animation(graph, target_freq), run_time=4)
            self.wait()
            if count == 2:
                self.play(LaggedStartMap(ApplyFunction, v_lines, lambda mob: (lambda m: m.shift(0.25 * UP).set_color(YELLOW), mob), rate_func=there_and_back))
    ...
    def get_peak_braces(self, **kwargs):
        peak_points = [self.time_axes.input_to_graph_point(x, self.graph) for x in np.arange(0, 3.5, 1.0 / self.signal_frequency)]
...
class DrawFrequencyPlot(WrapCosineGraphAroundCircle, PiCreatureScene):
    CONFIG = {'initial_winding_frequency': 3.0, 'center_of_mass_color': RED, 'center_of_mass_multiple': 1}
    ...
    def indicate_weight_of_wire(self):
        ...
        morty.change('plain')
        morty.fade(1)
        self.play(morty.restore, pol_graph.scale, 0.5, pol_graph.next_to, morty.get_corner(UP + LEFT), UP, -SMALL_BUFF)
        self.play(morty.change, 'lower_right_hand', pol_graph.get_bottom(), pol_graph.shift, 0.45 * DOWN, rate_func=there_and_back, run_time=2)
        self.wait()
        metal_wire = pol_graph.copy().set_stroke(GREY_B)
        self.play(ShowCreationThenDestruction(metal_wire), run_time=2)
    ...
    def show_center_of_mass_dot(self):
        ...
        self.play(GrowArrow(arrow), dot.restore)
    ...
    def introduce_frequency_plot(self):
        ...
        flower_path = ParametricCurve(lambda t: self.circle_plane.coords_to_point(np.sin(2 * t) * np.cos(t), np.sin(2 * t) * np.sin(t)), t_min=0, t_max=TAU)
        flower_path.move_to(self.center_of_mass_dot)
        self.play(wps_label.move_to, self.circle_plane.get_top(), com_label.move_to, self.circle_plane, DOWN)
        self.play(LaggedStartMap(FadeIn, frequency_axes))
        self.wait()
        self.play(MoveAlongPath(self.center_of_mass_dot, flower_path, run_time=4))
    ...
    def draw_full_frequency_plot(self):
        ...
        v_line = DashedLine(self.frequency_axes.coords_to_point(0, 0), self.frequency_axes.coords_to_point(0, 1), stroke_width=6, color=fourier_graph.get_color())
        ...
        for freq in [0.2, 1.5, 3.0, 4.0, 5.0]:
            ...
            self.change_frequency(freq, added_anims=[fourier_graph_update], run_time=8)
            self.wait()
        self.fourier_graph = fourier_graph

    def recap_objects_on_screen(self):
        rect = FullScreenFadeRectangle()
        time_group = VGroup(self.graph, self.time_axes, self.beats_per_second_label).copy()
        circle_group = VGroup(self.graph.polarized_mobject, self.circle_plane, self.winding_freq_label, self.center_of_mass_label, self.center_of_mass_dot).copy()
        frequency_group = VGroup(self.fourier_graph, self.frequency_axes, self.x_coord_label).copy()
    ...
    def lower_graph(self):
        ...
        self.wait()
        self.play(time_axes.shift, shift_vect / 2, graph.shift, -shift_vect / 2, self.get_frequency_change_animation(self.graph, new_freq), self.center_of_mass_dot_anim, self.get_period_v_lines_update_anim(), Transform(fourier_graph, new_fourier_graph), self.fourier_graph_dot.move_to, self.frequency_axes.coords_to_point(new_freq, 0), run_time=2)
        self.wait()
        ...
        self.wait()
        self.play(graph.restore, time_axes.restore, self.get_frequency_change_animation(self.graph, 3.0), self.center_of_mass_dot_anim, self.get_period_v_lines_update_anim(), fourier_graph.restore, Animation(self.fourier_graph_dot), run_time=2)
    ...
    def generate_center_of_mass_dot_update_anim(self, multiplier=1):
        ...
        self.center_of_mass_dot_anim = UpdateFromFunc(self.center_of_mass_dot, lambda d: d.move_to(multiplier * (com() - origin) + origin))
...
class AskAboutAlmostFouierName(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.teacher_says("We'll get to the real \\\\ one in a few minutes", added_anims=[self.change_students(*['plain'] * 3)])
        self.wait(2)

class ShowLowerFrequency(DrawFrequencyPlot):
    CONFIG = {'signal_frequency': 2.0, 'higher_signal_frequency': 3.0, 'lower_signal_color': PINK}
    ...
    def show_lower_frequency_signal(self):
        ...
        self.play(start_graph.stretch, ratio, 0, {'about_edge': LEFT}, start_graph.set_color, graph.get_color())
        self.play(FadeOut(start_graph), Animation(graph))
        self.remove(start_graph)
        self.play(Write(bps_label), LaggedStartMap(FadeIn, braces), *list(map(ShowCreation, v_lines)), run_time=1)
        self.wait()
        self.play(FadeOut(v_lines), FadeOut(braces), bps_label.restore)
    ...
    def play_with_lower_frequency_signal(self):
        ...
        self.play(ReplacementTransform(graph.copy(), pol_graph, run_time=2, path_arc=-TAU / 4), FadeIn(wps_label))
        ...
        arrow = Arrow(com_words.get_top(), dot.get_center(), buff=SMALL_BUFF, color=self.center_of_mass_color)
        dot.move_to(arrow.get_start())
        self.generate_center_of_mass_dot_update_anim()
        ...
        x_coord_label.scale(self.text_scale_val)
        x_coord_label.next_to(self.frequency_axes.input_to_graph_point(self.signal_frequency, fourier_graph), UP)
        x_coord_label.set_color(self.center_of_mass_color)
        self.generate_fourier_dot_transform(fourier_graph)
        self.play(Write(x_coord_label))
        self.change_frequency(self.signal_frequency, run_time=10, rate_func=smooth)
        self.wait()
        self.change_frequency(self.frequency_axes.x_max, run_time=15, rate_func=smooth)
        self.wait()
        self.set_variables_as_attrs(fourier_graph, fourier_graph_update)
...
class ShowLinearity(DrawFrequencyPlot):
    CONFIG = {'high_freq_color': YELLOW, 'low_freq_color': PINK, 'sum_color': GREEN, 'low_freq': 3.0, 'high_freq': 4.0, 'circle_plane_config': {'x_radius': 2.5, 'y_radius': 2.7, 'x_unit_size': 0.8, 'y_unit_size': 0.8}}
    ...
    def show_sum_of_signals(self):
        ...
        axes = self.get_time_axes()
        axes_copy = axes.copy()
        low_freq_graph, high_freq_graph = [self.get_cosine_wave(freq=freq, scale_val=0.5, shift_val=0.55) for freq in (low_freq, high_freq)]
        sum_graph = self.get_time_graph(lambda t: sum([low_freq_graph.underlying_function(t), high_freq_graph.underlying_function(t)]))
        ...
        trips = [(low_freq_label, low_freq_graph, self.low_freq_color), (high_freq_label, high_freq_graph, self.high_freq_color), (sum_label, sum_graph, self.sum_color)]
        ...
        self.play(FadeIn(axes_copy), ShowCreation(high_freq_graph))
        self.play(LaggedStartMap(FadeIn, VGroup(high_freq_label, low_freq_label)))
        self.wait()
        self.play(ReplacementTransform(axes_copy, axes), ReplacementTransform(high_freq_graph, sum_graph), ReplacementTransform(low_freq_graph, sum_graph), ReplacementTransform(VGroup(low_freq_label, high_freq_label), sum_label))
        self.wait()
        self.graph = graph
    ...
    def show_winding_with_sum_graph(self):
        ...
        almost_fourier_label = OldTexText("``Almost-Fourier transform''")
        self.generate_fourier_dot_transform(fourier_graph)
        self.play(LaggedStartMap(FadeIn, VGroup(circle_plane, wps_label, frequency_axes, x_coord_label), run_time=1))
        ...
        for freq in freqs:
            self.change_frequency(freq, run_time=8, rate_func=bezier([0, 0, 1, 1]))

    def show_vector_rotation(self):
        self.fourier_graph_drawing_update_anim = Animation(Mobject())
        self.change_frequency(self.low_freq)
        self.play(*self.get_vector_animations(self.graph, draw_polarized_graph=False, run_time=20))
        self.wait()

class ShowCommutativeDiagram(ShowLinearity):
    CONFIG = {'time_axes_config': {'x_max': 1.9, 'y_max': 2.0, 'y_min': -2.0, 'y_axis_config': {'unit_size': 0.5}, 'x_axis_config': {'numbers_to_show': [1]}}, 'time_label_t': 1.5, 'frequency_axes_config': {'x_min': 0.0, 'x_max': 4.0, 'y_min': -0.1, 'y_max': 0.5, 'y_axis_config': {'unit_size': 1.5, 'tick_frequency': 0.5}}}
    ...
    def show_diagram(self):
        ...
        time_axes.scale(0.8)
        ta_group = VGroup(time_axes, time_axes.deepcopy(), time_axes.deepcopy())
        ...
        for ta, fa in zip(ta_group, fa_group):
            ...
            ta.remove(ta.labels)
            fa.remove(fa.label)
        funcs = [lambda t: np.cos(2 * TAU * t), lambda t: np.cos(3 * TAU * t)]
        funcs.append(lambda t: funcs[0](t) + funcs[1](t))
        colors = [self.low_freq_color, self.high_freq_color, self.sum_color]
        labels = [OldTexText('2 Hz'), OldTexText('3 Hz'), VectorizedPoint()]
        for func, color, label, ta, fa in zip(funcs, colors, labels, ta_group, fa_group):
            ...
            arrow = Arrow(ta.x_axis, fa.x_axis, color=WHITE, buff=MED_LARGE_BUFF)
        ...
        def apply_transform(index):
            ...
            fa = fa_group[index]
            anims = [ReplacementTransform(getattr(ta, attr), getattr(fa, attr)) for attr in ('x_axis', 'y_axis', 'graph')]
            anims += [GrowArrow(ta.arrow), Write(ta.arrow.words)]
            if index == 0:
        ...
        apply_transform(1)
        self.wait()
        self.play(GrowArrow(sum_arrows[1]), Write(sum_arrows[1].words), *[ReplacementTransform(fa.copy(), fa_group[2]) for fa in fa_group[:2]])
        self.wait(2)
        self.play(GrowArrow(sum_arrows[0]), Write(sum_arrows[0].words), *[ReplacementTransform(mob.copy(), ta_group[2], run_time=1) for mob in ta_group[:2]])
        self.wait()
        apply_transform(2)
        self.wait()
...
class BeforeGettingToTheFullMath(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(Write(formula), self.teacher.change, 'raise_right_hand', self.change_students(*['confused'] * 3))
        self.wait()
        self.play(ApplyMethod(formula.next_to, FRAME_X_RADIUS * RIGHT, RIGHT, path_arc=TAU / 16, rate_func=running_start), self.change_students(*['pondering'] * 3))
...
class FilterOutHighPitch(AddingPureFrequencies, ShowCommutativeDiagram):
    ...
    def play_sound(self):
        randy = self.pi_creature
        self.play(Succession(ApplyMethod, randy.look_at, self.speaker, Animation, randy, ApplyMethod, randy.change, 'telepath', randy, Animation, randy, Blink, randy, Animation, randy, {'run_time': 2}), *self.get_broadcast_anims(), run_time=7)
        self.play(randy.change, 'angry', self.speaker)
        self.wait()

    def show_intensity_vs_time_graph(self):
        randy = self.pi_creature
        axes = Axes(x_min=0, x_max=12, y_min=-6, y_max=6, y_axis_config={'unit_size': 0.15, 'tick_frequency': 3})
        ...
        axes.labels = labels
        func = lambda t: sum([np.cos(TAU * f * t) for f in (0.5, 0.7, 1.0, 1.2, 3.0)])
        graph = axes.get_graph(func)
        graph.set_color(BLUE)
        self.play(FadeIn(axes), FadeIn(axes.labels), randy.change, 'pondering', axes, ShowCreation(graph, run_time=4, rate_func=bezier([0, 0, 1, 1])), *self.get_broadcast_anims(run_time=6))
        self.wait()
        self.time_axes = axes
    ...
    def take_fourier_transform(self):
        ...
        randy = self.pi_creature
        speaker = self.speaker
        frequency_axes = Axes(x_min=0, x_max=3.5, x_axis_config={'unit_size': 3.5}, y_min=0, y_max=1, y_axis_config={'unit_size': 2})
        frequency_axes.set_color(TEAL)
        ...
        frequency_axes.label = freq_label
        fourier_func = get_fourier_transform(time_graph.underlying_function, t_min=0, t_max=30)
        fourier_graph = frequency_axes.get_graph(fourier_func, num_graph_points=150)
        fourier_graph.set_color(RED)
        ...
        self.play(ReplacementTransform(time_axes.copy(), frequency_axes), ReplacementTransform(time_graph.copy(), fourier_graph), ReplacementTransform(time_axes.labels[0].copy(), freq_label), GrowArrow(arrow), Write(ft_words), VGroup(randy, speaker).shift, FRAME_Y_RADIUS * DOWN)
    ...
    def filter_out_high_pitch(self):
        ...
        self.play(Transform(fourier_graph, new_graph), spike_rect.stretch, 0.01, 1, {'about_point': frequency_axes.coords_to_point(0, 0)}, run_time=2)
        self.wait()
    ...
    def mention_inverse_transform(self):
        ...
        self.wait()
        self.play(time_graph.fade, 0.9, ReplacementTransform(fourier_graph.copy(), filtered_graph))
        self.wait()

    def get_broadcast_anims(self, run_time=7, **kwargs):
        return [self.get_broadcast_animation(n_circles=n, run_time=run_time, big_radius=7, start_stroke_width=5, **kwargs) for n in (5, 7, 10, 12)]
...
class ApplyFourierToFourier(DrawFrequencyPlot):
    CONFIG = {'time_axes_config': {'y_min': -1.5, 'y_max': 1.5, 'x_max': 5, 'x_axis_config': {'numbers_to_show': list(range(1, 5)), 'unit_size': 2.5}}, 'frequency_axes_config': {'y_min': -0.6, 'y_max': 0.6}, 'circle_plane_config': {'x_radius': 1.5, 'y_radius': 1.35, 'x_unit_size': 1.5, 'y_unit_size': 1.5}, 'default_num_v_lines_indicating_periods': 0, 'signal_frequency': 2}
    ...
    def setup_fourier_display(self):
        ...
        VGroup(self.time_axes, self.graph, self.frequency_axes, self.fourier_graph, self.x_coord_label, self.fourier_graph_dot).to_edge(UP, buff=MED_SMALL_BUFF)
        self.revert_to_original_skipping_status()
    ...
    def swap_graphs(self):
        ...
        self.play(ReplacementTransform(fourier_graph.copy(), new_fourier_graph), ApplyMethod(time_graph.shift, 3 * UP + 10 * LEFT, remover=True))
        ...
        self.play(ReplacementTransform(self.graph.copy(), wound_up_graph), ChangeDecimalToValue(self.winding_freq_label[1], 0.0, run_time=0.2))
...
class WriteComplexExponentialExpression(DrawFrequencyPlot):
    CONFIG = {'signal_frequency': 2.0, 'default_num_v_lines_indicating_periods': 0, 'time_axes_scale_val': 0.7, 'initial_winding_frequency': 0.1, 'circle_plane_config': {'unit_size': 2, 'y_radius': FRAME_Y_RADIUS + LARGE_BUFF, 'x_radius': FRAME_X_RADIUS + LARGE_BUFF}}
    ...
    def setup_plane(self):
        ...
        circle_plane.add(circle)
        time_axes = self.get_time_axes()
        time_axes.background_rectangle = BackgroundRectangle(time_axes, fill_opacity=0.9, buff=MED_SMALL_BUFF)
    ...
    def setup_graph(self):
        ...
        graph = self.graph = self.get_cosine_wave(freq=self.signal_frequency, scale_val=0.5, shift_val=0.75)
        freq = self.initial_winding_frequency
    ...
    def show_winding_with_both_coordinates(self):
        ...
        self.change_frequency(2.04, added_anims=[self.center_of_mass_dot_anim], run_time=15, rate_func=bezier([0, 0, 1, 1]))
        self.wait()
    ...
    def show_plane_as_complex_plane(self):
        ...
        number_label = DecimalNumber(0, include_background_rectangle=True)
        number_label_update_anim = ContinualChangingDecimal(number_label, lambda a: plane.point_to_number(dot.get_center()), position_update_func=lambda l: l.next_to(dot, DOWN + RIGHT, buff=SMALL_BUFF))
        number_label_update_anim.update(0)
        flower_path = ParametricCurve(lambda t: plane.coords_to_point(np.sin(2 * t) * np.cos(t), np.sin(2 * t) * np.sin(t)), t_min=0, t_max=TAU)
        flower_path.move_to(self.center_of_mass_dot)
        ...
        self.wait()
        self.play(ShowCreation(self.pol_graph, run_time=3))
    ...
    def show_eulers_formula(self):
        ...
        exp_decimal_update = ContinualChangingDecimal(exp_decimal, lambda a: get_t(), position_update_func=lambda d: d.move_to(exp_base.get_corner(UP + RIGHT), DOWN + LEFT))
        exp_base_update = Mobject.add_updater(exp_base, lambda e: e.move_to(get_circle_point(scalar=1.1, t_shift=0.01 * TAU)))
        vector_update = Mobject.add_updater(vector, lambda v: v.put_start_and_end_on(plane.number_to_point(0), get_circle_point()))
        ...
        distance_label.move_to(plane.number_to_point(1.1 * np.exp(complex(0, 0.4 * get_t()))), DOWN + LEFT)
        self.play(ShowCreation(arc))
        self.play(ReplacementTransform(exp_decimal.copy(), distance_label))
        self.wait()
        self.play(FadeOut(distance_label))
        self.remove(arc)
        self.play(ghost_dot.move_to, TAU * RIGHT, ShowCreation(circle, rate_func=lambda a: interpolate(2.0 / TAU, 1, smooth(a))), run_time=6)
        self.wait()
        ...
        self.add(ghost_dot)
        self.play(Write(time_label), GrowArrow(time_label.arrow))
        self.wait(12.5)
        self.remove(ghost_dot)
        self.play(FadeOut(time_label), FadeIn(frequency_label), t.restore, GrowFromPoint(f, frequency_label.get_center()), ReplacementTransform(time_label.arrow, frequency_label.arrow))
        ghost_dot.move_to(ORIGIN)
        ...
        ghost_dot.clear_updaters()
        always_shift(ghost_dot, rate=-0.1 * TAU)
        self.play(FadeOut(example_frequency), FadeOut(frequency_label.arrow), GrowFromCenter(minus), two_pi_i_f_t_group.restore)
        self.wait(4)
        ghost_dot.clear_updaters()
        self.remove(*updates)
        self.play(*list(map(FadeOut, [update.mobject for update in updates if update.mobject is not vector])))
    ...
    def show_winding_graph_expression(self):
        ...
        self.wait()
        self.play(ReplacementTransform(g_label.copy(), g_scalar), exp_expression.next_to, g_scalar, RIGHT, SMALL_BUFF, exp_expression.shift, 0.5 * SMALL_BUFF * UP)
    ...
    def find_center_of_mass(self):
        ...
        sum_expr = OldTex('{1', '\\over', 'N}', '\\sum', '_{k = 1}', '^N')
        ...
        axes = self.time_axes
        time_interval = Line(axes.coords_to_point(axes.x_min, 0), axes.coords_to_point(axes.x_max, 0))
        ...
        self.wait()
        self.play(Write(sum_expr), MoveToTarget(integrand))
        t1 = integrand[0][1][2]
        t2 = integrand[1][1][-1]
        t_mobs = VGroup(t1, t2)
        t_mobs.save_state()
        ...
        self.play(LaggedStartMap(Indicate, dots[1], rate_func=there_and_back, color=TEAL))
        self.show_center_of_mass_sampling(100)
        dots = self.show_center_of_mass_sampling(500)
        self.wait()
        self.play(FadeOut(dots))
        self.play(ReplacementTransform(sum_expr, integral), FadeIn(dt_mob), t_mobs.restore)
        self.wait()
        ...
        self.play(randy.change, 'thinking', self.pol_graph, GrowArrow(com_arrow), GrowFromCenter(com_dot))
        self.play(Blink(randy))
        self.wait(2)
    ...
    def show_center_of_mass_sampling(self, n_dots):
        ...
        pre_dots = VGroup(*[dot.copy().move_to(axes.coords_to_point(t, 0)) for t in np.linspace(axes.x_min, axes.x_max, n_dots)])
        ...
        for graph in (time_graph, pol_graph):
            ...
            graph.is_faded = True
            graph.dots = VGroup(*[dot.copy().move_to(graph.point_from_proportion(a)) for a in np.linspace(0, 1, n_dots)])
        self.play(ReplacementTransform(pre_dots, time_graph.dots, lag_ratio=0.5, run_time=2), MoveToTarget(time_graph), *time_graph.dot_fade_anims)
        self.play(ReplacementTransform(time_graph.copy(), pol_graph.target), MoveToTarget(pol_graph), ReplacementTransform(time_graph.dots.copy(), pol_graph.dots), *pol_graph.dot_fade_anims, run_time=2)
...
class BuildUpExpressionStepByStep(TeacherStudentsScene):
    ...
    def construct(self):
        expression = OldTex('\\frac{1}{t_2 - t_1}', '\\int_{t_1}^{t_2}', 'g(t)', 'e', '^{-2\\pi i', 'f', 't}', 'dt')
        ...
        t.save_state()
        t.move_to(f, LEFT)
        self.play(self.teacher.change, 'raise_right_hand', FadeIn(e), FadeIn(two_pi_i))
        self.play(self.change_students(*['pondering'] * 3), FadeIn(t))
        self.play(FadeIn(f), t.restore)
        self.wait()
        self.play(FadeIn(g), Blink(self.students[1]))
        self.wait()
        self.play(FadeIn(integral), FadeIn(frac), FadeIn(dt))
        self.wait(3)
        self.teacher_says('Just one final \\\\ distinction.', bubble_config={'height': 2.5, 'width': 3.5}, added_anims=[expression.to_corner, UP + RIGHT])
        self.wait(3)
...
class ScaleUpCenterOfMass(WriteComplexExponentialExpression):
    ...
    def scale_up_center_of_mass(self):
        ...
        graph = self.graph
        short_graph, long_graph = [axes.get_graph(graph.underlying_function, x_min=0, x_max=t_max).match_style(graph) for t_max in (3, 6)]
        for g in (short_graph, long_graph):
            self.get_polarized_mobject(g, freq=self.initial_winding_frequency)
        self.play(FocusOn(circle, run_time=2), Succession(ShowCreation, circle, FadeOut, circle))
        self.play(com_dot.fade, 0.5, FadeIn(vector_to_scale))
        self.wait()
        self.play(vector_to_scale.scale, 4, {'about_point': origin})
        self.wait()
        self.play(FadeOut(vector_to_scale), FadeIn(com_vector))
        self.remove(graph.polarized_mobject)
        self.play(com_dot.move_to, center_of_mass(short_graph.polarized_mobject.get_points()), com_vector_update, time_span.restore, ShowCreation(short_graph.polarized_mobject))
        self.wait()
        ...
        self.wait()
        to_squish = VGroup(axes, graph, time_span)
        to_squish.generate_target()
        squish_factor = 0.75
        to_squish.target.stretch(squish_factor, 0, about_edge=LEFT)
        pairs = list(zip(to_squish.family_members_with_points(), to_squish.target.family_members_with_points()))
        ...
        self.play(com_dot.move_to, center_of_mass(long_graph.polarized_mobject.get_points()), com_vector_update, time_span.stretch, 2, 0, {'about_edge': LEFT}, *[ShowCreation(mob, rate_func=lambda a: interpolate(0.5, 1, smooth(a))) for mob in (long_graph, long_graph.polarized_mobject)], run_time=2)
        ...
        self.play(to_shift.shift, 2 * DOWN, FadeOut(to_fade), axes.background_rectangle.stretch, 2, 0, {'about_edge': LEFT}, Animation(axes), Animation(self.graph), FadeOut(com_vector_copies))
        self.change_frequency(2.0, added_anims=[com_vector_update])
        very_long_graph = axes.get_graph(graph.underlying_function, x_min=0, x_max=12)
        very_long_graph.match_style(graph)
        self.get_polarized_mobject(very_long_graph, freq=2.0)
        self.play(com_dot.move_to, center_of_mass(very_long_graph.polarized_mobject.get_points()), com_vector_update, ShowCreation(very_long_graph, rate_func=lambda a: interpolate(0.5, 1, a)), ShowCreation(very_long_graph.polarized_mobject))
    ...
    def comment_on_current_signal(self):
        ...
        origin = self.circle_plane.coords_to_point(0, 0)
        wps_label = self.winding_freq_label
        new_com_vector_update = UpdateFromFunc(com_vector, lambda v: v.put_start_and_end_on(origin, com_dot.get_center()).scale(12, about_point=origin))
        ...
        self.play(ShowCreation(v_lines, lag_ratio=0), ShowCreation(graph_portion), FadeIn(bps_label))
        self.wait()
        self.play(ReplacementTransform(bps_label[1][0].copy(), wps_label[1]))
        self.wait()
        self.play(com_vector.scale, 0.5, {'about_point': origin}, rate_func=there_and_back, run_time=2)
        self.wait(2)
        self.change_frequency(2.5, added_anims=[new_com_vector_update], run_time=20, rate_func=linear)
        self.wait()
...
class SimpleCosineWrappingAroundCircle(WriteComplexExponentialExpression):
    CONFIG = {'initial_winding_frequency': 0, 'circle_plane_config': {'unit_size': 3}}
...
class SummarizeTheFullTransform(DrawFrequencyPlot):
    CONFIG = {'time_axes_config': {'x_max': 4.5, 'x_axis_config': {'unit_size': 1.2, 'tick_frequency': 0.5}}, 'frequency_axes_config': {'x_max': 5, 'x_axis_config': {'unit_size': 1, 'numbers_to_show': list(range(1, 5))}, 'y_max': 2, 'y_min': -2, 'y_axis_config': {'unit_size': 0.75, 'tick_frequency': 1}}}
    ...
    def show_transform_function(self):
        ...
        fourier_func = get_fourier_transform(func, t_min=time_axes.x_min, t_max=time_axes.x_max, use_almost_fourier=False)
        ...
        g_t.fade(1)
        self.play(morty.change, 'raise_right_hand', g_t.restore)
        self.wait()
        self.play(Write(frequency_axes, run_time=1))
        self.play(ReplacementTransform(graph.copy(), fourier_graph), ReplacementTransform(g_t.copy(), g_hat_f))
        self.wait(2)
        for label in (time_label, frequency_label):
    ...
    def show_winding(self):
        ...
        arrow = Arrow(g_hat_f.target.get_left(), plane.coords_to_point(0, 0), color=self.center_of_mass_color)
        frequency_axes = self.frequency_axes
        imaginary_fourier_graph = frequency_axes.get_graph(get_fourier_transform(graph.underlying_function, t_min=self.time_axes.x_min, t_max=self.time_axes.x_max, real_part=False, use_almost_fourier=False))
        imaginary_fourier_graph.set_color(BLUE)
        imaginary_fourier_graph.shift(frequency_axes.x_axis.get_right() - imaginary_fourier_graph.get_points()[-1])
        ...
        g_hat_f_indication = Succession(Animation, Mobject(), {'run_time': 4}, FocusOn, g_hat_f, ShowCreation, g_hat_f_rect, Animation, Mobject(), Transform, g_hat_f_rect, g_hat_rect, Animation, Mobject(), FadeOut, g_hat_f_rect, Animation, Mobject(), MoveToTarget, g_hat_f, UpdateFromAlphaFunc, com_dot, lambda m, a: m.set_fill(opacity=a), Animation, Mobject(), {'run_time': 2}, GrowArrow, arrow, FadeOut, arrow, Animation, Mobject(), {'run_time': 5}, Write, real_part, {'run_time': 2}, Animation, Mobject(), {'run_time': 3}, ShowCreation, imaginary_fourier_graph, {'run_time': 3}, rate_func=squish_rate_func(lambda x: x, 0, 31.0 / winding_run_time), run_time=winding_run_time)
        self.play(FadeIn(plane), ReplacementTransform(graph.copy(), graph.polarized_mobject), morty.change, 'happy')
        self.generate_center_of_mass_dot_update_anim(multiplier=4.5)
        self.generate_fourier_dot_transform(fourier_graph)
        self.change_frequency(5.0, rate_func=linear, run_time=winding_run_time, added_anims=[g_hat_f_indication, update_pol_graph, Animation(frequency_axes.x_axis.numbers), Animation(self.fourier_graph_dot)])
        self.wait()
...
class SummarizeFormula(Scene):
    ...
    def construct(self):
        ...
        exp_rect, g_exp_rect, int_rect = [SurroundingRectangle(VGroup(expression.get_part_by_tex(p1), expression.get_part_by_tex(p2))) for p1, p2 in [('e', 't}'), ('g({}', 't}'), ('\\int', 'dt')]]
        self.add(expression)
        self.wait()
        self.play(ShowCreation(screen_rect), ShowCreation(exp_rect))
        self.wait(2)
    ...
    def get_expression(self):
        expression = OldTex('\\hat g(', 'f', ')', '=', '\\int', '_{t_1}', '^{t_2}', 'g({}', 't', ')', 'e', '^{-2\\pi i', 'f', 't}', 'dt')
        expression.set_color_by_tex('t', YELLOW, substring=False)
        expression.set_color_by_tex('t}', YELLOW)
        expression.set_color_by_tex('f', RED, substring=False)
...
class BoundsAtInfinity(SummarizeFormula):
    ...
    def construct(self):
        ...
        decimal_updates = [ChangingDecimal(db, get_db_num_update(vect), position_update_func=lambda m: m.move_to(m.bound, LEFT)) for db, vect in zip(decimal_bounds, [LEFT, RIGHT])]
        ...
        self.play(ReplacementTransform(inf_bounds, decimal_bounds), time_interval.restore)
        self.play(VGroup(axes, graph).stretch, 0.05, 0, Transform(time_interval, wide_interval), UpdateFromAlphaFunc(axes.x_axis.numbers, lambda m, a: m.set_fill(opacity=1 - a)), *decimal_updates, run_time=12, rate_func=bezier([0, 0, 1, 1]))
        self.wait()

    def add_graph(self):
        axes = Axes(x_min=-140, x_max=140, y_min=-2, y_max=2, axis_config={'include_tip': False})
        axes.x_axis.add_numbers(*list(filter(lambda x: x != 0, list(range(-8, 10, 2)))))
        axes.shift(DOWN)
        self.add(axes)
...
class MoreToCover(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says('Much more to say...', target_mode='hooray', run_time=1)
        self.wait()
        self.teacher_says('SO MUCH!', target_mode='surprised', added_anims=[self.change_students(*3 * ['happy'])], run_time=0.5)
        self.wait(2)
...
class ShowUncertaintyPrinciple(Scene):
    ...
    def construct(self):
        ...
        self.add(title)
        top_axes = Axes(x_min=-FRAME_X_RADIUS, x_max=FRAME_X_RADIUS, y_min=0, y_max=3, y_axis_config={'unit_size': 0.6, 'include_tip': False})
        ...
        self.play(ReplacementTransform(top_graph.copy(), bottom_graph))
        self.wait(2)
        self.add(*[Mobject.add_updater(graph, get_update_func(axes)) for graph, axes in [(top_graph, top_axes), (bottom_graph, bottom_axes)]])
...
class SubscribeOrBinge(PiCreatureScene):
    ...
    def construct(self):
        ...
        video_anim = LaggedStartMap(Succession, videos, lambda v: (FadeIn, v, ApplyMethod, v.shift, 5 * DOWN, {'run_time': 6}), run_time=10)
        sub_arrow = Arrow(subscribe.get_bottom(), Dot().to_corner(DOWN + RIGHT, buff=LARGE_BUFF), color=RED)
        ...
        self.wait()
        self.play(video_anim, Succession(AnimationGroup(ApplyMethod(binge.restore), ApplyMethod(morty.change, 'raise_right_hand', binge)), Blink, morty, ApplyMethod, morty.change, 'shruggie', videos, Animation, Mobject(), {'run_time': 2}, Blink, morty, Animation, Mobject(), {'run_time': 4}))
...
class PuzzleDescription(Scene):
    ...
    def construct(self):
        lines = VGroup(OldTexText('Convex set', '$C$', 'in $\\mathds{R}^3$'), OldTexText('Boundary', '$B$', '$=$', '$\\partial C$'), OldTexText('$D$', '$=\\{p+q | p, q \\in B\\}$'), OldTexText('Prove that', '$D$', 'is convex'))
        for line in lines:
            line.set_color_by_tex_to_color_map({'$C$': BLUE_D, '\\partial C': BLUE_D, '$B$': BLUE_C, '$D$': YELLOW})
...
class Thumbnail(Scene):
    ...
    def construct(self):
        ...
        for f in np.linspace(1.98, 2.02, 5):
            pol_graph = ParametricCurve(lambda t: complex_to_R3((2 + np.cos(2 * TAU * t) + np.cos(3 * TAU * t)) * np.exp(-complex(0, TAU * f * t))), t_range=(-5, 5, 0.01))
