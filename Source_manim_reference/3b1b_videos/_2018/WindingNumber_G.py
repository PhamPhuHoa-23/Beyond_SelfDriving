...
class IntroSceneWrapper(PiCreatureScene):
    CONFIG = {'default_pi_creature_kwargs': {'color': YELLOW_E, 'flip_at_start': False, 'height': 2}, 'default_pi_creature_start_corner': DOWN + LEFT}
    ...
    def introduce_two_words(self):
        ...
        main_topic, meta_topic = topics = VGroup(OldTexText('Main topic'), OldTexText('Meta topic'))
        topics.next_to(morty, UP)
        topics.shift_onto_screen()
        self.play(morty.change, 'raise_left_hand', FadeInFromDown(main_topic))
        self.wait()
        self.play(morty.change, 'raise_right_hand', main_topic.next_to, meta_topic.get_top(), UP, MED_SMALL_BUFF, FadeInFromDown(meta_topic))
        self.wait()
        self.play(morty.change, 'happy', main_topic.next_to, h_line, UP, meta_topic.set_fill, {'opacity': 0.2})
    ...
    def describe_main_topic(self):
        ...
        self.play(main_topic.move_to, meta_topic, main_topic.set_fill, {'opacity': 0.2}, meta_topic.move_to, main_topic, meta_topic.set_fill, {'opacity': 1}, morty.change, 'hesitant', path_arc=TAU / 8)
...
class Introduce1DFunctionCase(Scene):
    CONFIG = {'search_range_rect_height': 0.15, 'arrow_opacity': 1, 'show_dotted_line_to_f': True, 'arrow_config': {'max_tip_length_to_length_ratio': 0.5}, 'show_midpoint_value': True}
    ...
    def show_axes_one_at_a_time(self):
        axes = Axes(x_min=-1, x_max=3.2, x_axis_config={'unit_size': 3, 'tick_frequency': 0.25, 'big_tick_numbers': list(range(-1, 4))}, y_min=-2, y_max=4.5)
        ...
        self.wait()
        self.play(ShowCreation(axes.y_axis), FadeOut(axes.x_axis.numbers[1], rate_func=squish_rate_func(smooth, 0, 0.2)), Write(outputs))
        self.wait()
        self.axes = axes
    ...
    def show_two_graphs(self):
        ...
        l_v_line, r_v_line, v_line = [DashedLine(axes.coords_to_point(x, 0), axes.coords_to_point(x, f_graph.underlying_function(solution))) for x in (axes.x_min, axes.x_max, solution)]
        ...
        for graph, label in ((f_graph, f_label), (g_graph, g_label)):
            self.play(ShowCreation(graph), Write(label, rate_func=squish_rate_func(smooth, 0.5, 1)), run_time=2)
        self.wait()
        self.play(ReplacementTransform(r_v_line.copy().fade(1), v_line), ReplacementTransform(l_v_line.copy().fade(1), v_line), run_time=2)
        self.play(ReplacementTransform(f_label.copy(), equation[0]), ReplacementTransform(g_label.copy(), equation[2]), Write(equation[1]), GrowArrow(equation_arrow))
        for x in range(4):
            self.play(FadeOut(v_line.copy()), ShowCreation(v_line, rate_func=squish_rate_func(smooth, 0.5, 1)), run_time=1.5)
        self.wait()
        self.play(MoveToTarget(equation, replace_mobject_with_target_in_scene=True), *list(map(FadeOut, [equation_arrow, v_line])))
    ...
    def transition_to_sqrt_2_case(self):
        ...
        two_label.next_to(axes.input_to_graph_point(3, two_graph), UP)
        two_label.match_color(two_graph)
        find_sqrt_2 = self.find_sqrt_2 = OldTexText('(Find $\\sqrt{2}$)')
        find_sqrt_2.next_to(self.equation, DOWN)
        self.play(ReplacementTransform(f_graph, squared_graph), ReplacementTransform(f_label, squared_label))
        self.play(ReplacementTransform(g_graph, two_graph), ReplacementTransform(g_label, two_label), Animation(axes.y_axis.numbers))
        self.wait()
        self.play(Write(find_sqrt_2))
        self.wait()
        self.set_variables_as_attrs(squared_graph, two_graph, squared_label, two_label)

    def show_example_binary_search(self):
        self.binary_search(self.squared_graph, self.two_graph, x0=1, x1=2, n_iterations=8)

    def binary_search(self, f_graph, g_graph, x0, x1, n_iterations, n_iterations_with_sign_mention=0, zoom=False):
        axes = self.axes
        rect = self.rect = Rectangle()
        rect.set_stroke(width=0)
        rect.set_fill(YELLOW, 0.5)
        rect.replace(Line(axes.coords_to_point(x0, 0), axes.coords_to_point(x1, 0)), dim_to_match=0)
        ...
        decimal = DecimalNumber(0, num_decimal_places=3, show_ellipsis=True)
        decimal.scale(0.7)
        decimal_update = ChangingDecimal(decimal, lambda a: axes.x_axis.point_to_number(rect.get_center()), position_update_func=lambda m: m.next_to(midpoint_line, DOWN, SMALL_BUFF, submobject_to_align=decimal[:-1]))
        if not self.show_midpoint_value:
            decimal.set_fill(opacity=0)
            midpoint_line.set_stroke(width=0)
        kwargs = {'mention_signs': False, 'show_decimal': zoom}
        for x in range(n_iterations - 1):
            ...
            self.play(MoveToTarget(rect), midpoint_line_update, decimal_update, Animation(all_leftovers), FadeOut(to_fade), *added_anims)
            if zoom:
    ...
    def compare_graphs_at_x(self, f_graph, g_graph, x, mention_signs=False, show_decimal=False):
        axes = self.axes
        ...
        added_anims = []
        if show_decimal:
            decimal = DecimalNumber(axes.x_axis.point_to_number(arrow.get_start()), num_decimal_places=3)
            height = self.rect.get_height()
            decimal.set_height(height)
            next_to_kwargs = {'buff': height}
            if too_high:
...
class TransitionFromEquationSolverToZeroFinder(Introduce1DFunctionCase):
    CONFIG = {'show_dotted_line_to_f': False, 'arrow_config': {}, 'show_midpoint_value': False}
    ...
    def transition_to_difference_graph(self):
        ...
        new_graph = axes.get_graph(lambda x: f_graph.underlying_function(x) - g_graph.underlying_function(x), color=GREEN)
        ...
        self.play(ReplacementTransform(equation, new_equation, path_arc=TAU / 4), find_sqrt_2.next_to, new_equation, DOWN)
        self.play(MoveToTarget(fg_labels))
        self.play(ReplacementTransform(f_graph, new_graph), ReplacementTransform(g_graph, zero_graph))
        self.play(ReplacementTransform(f_label[0].copy(), new_label[0]), ReplacementTransform(g_label[0].copy(), new_label[2]), Write(new_label[1]))
        self.wait()
    ...
    def show_binary_search_with_signs(self):
        ...
        self.binary_search(self.new_graph, self.zero_graph, 1, 2, n_iterations=9, n_iterations_with_sign_mention=2, zoom=True)

class RewriteEquationWithTeacher(AltTeacherStudentsScene):

    def construct(self):
        root_two_equations = VGroup(OldTex('x^2', '', '=', '2', ''), OldTex('x^2', '-', '2', '=', '0'))
        ...
        self.play(ApplyMethod(to_remove.next_to, FRAME_X_RADIUS * RIGHT, RIGHT, remover=True, rate_func=running_start, path_arc=-TAU / 4), self.teacher.change, 'hesitant', self.change_students(*['erm'] * 3))
        self.teacher_holds_up(two_d_equation)
        self.play_all_student_changes('horrified')
        self.wait()
        self.play(FadeOut(two_d_equation), FadeInFromDown(complex_group))
        self.play_all_student_changes('confused')
        self.wait(3)

class InputOutputScene(Scene):
    CONFIG = {'plane_width': 6, 'plane_height': 6, 'x_shift': FRAME_X_RADIUS / 2, 'y_shift': MED_LARGE_BUFF, 'output_scalar': 10, 'non_renormalized_func': plane_func_by_wind_spec((-2, -1, 2), (1, 1, 1), (2, -2, -1))}
    ...
    def get_colorings(self):
        in_cmos = ColorMappedObjectsScene(func=lambda p: self.non_renormalized_func((p[0] + self.x_shift, p[1] + self.y_shift)))
        scalar = self.output_scalar
        out_cmos = ColorMappedObjectsScene(func=lambda p: (scalar * (p[0] - self.x_shift), scalar * (p[1] + self.y_shift)))
        input_coloring = Rectangle(height=self.plane_height, width=self.plane_width, stroke_width=0, fill_color=WHITE, fill_opacity=1)
    ...
    def get_planes(self):
        input_plane = self.input_plane = NumberPlane(x_radius=self.plane_width / 2.0, y_radius=self.plane_height / 2.0)
...
class IntroduceInputOutputScene(InputOutputScene):
    CONFIG = {'dot_radius': 0.05, 'dot_density': 0.25}
    ...
    def map_single_point_to_point(self):
        ...
        path_arc = -TAU / 4
        curved_arrow = Arrow(in_dot, out_dot, buff=SMALL_BUFF, path_arc=path_arc, color=WHITE)
        ...
        self.wait()
        self.play(ShowCreation(curved_arrow), ReplacementTransform(in_dot.copy(), out_dot, path_arc=path_arc), FadeIn(function_label))
        self.play(GrowArrow(output_label_arrow), FadeIn(output_label))
        self.wait()
        self.play(*list(map(FadeOut, [input_label_arrow, input_label, output_label_arrow, output_label, curved_arrow, function_label])))
        ...
        for coords in ((-2, 2), (-2, -2), (2, -2), (1.5, 1.5)):
            self.play(in_dot.move_to, input_plane.coords_to_point(*coords), path_arc=-TAU / 4, run_time=2)
        self.wait()
    ...
    def get_output_dot_continual_update(self, input_dot, output_dot):
        return Mobject.add_updater(output_dot, lambda od: od.move_to(self.point_function(input_dot.get_center())))

class IntroduceVectorField(IntroduceInputOutputScene):
    CONFIG = {'dot_density': 0.5}
    ...
    def construct(self):
        ...
        in_dot.set_color(YELLOW)
        out_dot.set_color(PINK)
        out_vector = Arrow(LEFT, RIGHT, color=out_dot.get_color())
        out_vector.set_stroke(BLACK, 1)
        continual_out_vector_update = Mobject.add_updater(out_vector, lambda ov: ov.put_start_and_end_on(output_plane.coords_to_point(0, 0), out_dot.get_center()))
        ...
        continual_updates = [out_dot_continual_update, continual_out_vector_update, continual_in_vector_update]
        ...
        path.replace(Line(input_plane.coords_to_point(-1.5, -1.5), input_plane.coords_to_point(1.5, 1.5)), stretch=True)
        in_vectors = VGroup()
        self.add(in_vectors)
        for a in np.linspace(0, 1, 25):
            self.play(in_dot.move_to, path.point_from_proportion(a), run_time=0.2, rate_func=linear)
...
class TwoDScreenInOurThreeDWorld(AltTeacherStudentsScene, ThreeDScene):
    ...
    def ask_about_2d_functions(self):
        ...
        self.play(ShowCreation(arrow), ReplacementTransform(in_plane.copy(), out_plane, path_arc=-TAU / 4))
        self.play(LaggedStartMap(GrowFromCenter, dots, run_time=1), self.change_students(*3 * ['erm']))
    ...
    def show_3d(self):
        ...
        self.move_camera(distance=12, run_time=4, added_anims=[MoveToTarget(everything, run_time=4)])
        always_rotate(everything, axis=UP, rate=3 * DEGREES)
        self.wait(10)

class EveryOutputPointHasAColor(ColorMappedObjectsScene):
    CONFIG = {'func': lambda p: p, 'dot_spacing': 0.1, 'dot_radius': 0.01}
    ...
    def construct(self):
        ...
        while n <= len(dots):
            ...
            n *= m
        self.play(LaggedStartMap(LaggedStartMap, dot_groups, lambda dg: (GrowFromCenter, dg), run_time=8, lag_ratio=0.2))
...
class DotsHoppingToColor(InputOutputScene):
    ...
    def construct(self):
        ...
        right_half_block = Rectangle(height=FRAME_HEIGHT, width=FRAME_X_RADIUS - SMALL_BUFF, stroke_width=0, fill_color=BLACK, fill_opacity=0.8)
        right_half_block.to_edge(RIGHT, buff=0)
        self.add(input_plane, output_plane, v_line)
        self.play(FadeIn(output_coloring), Animation(output_plane), output_plane.white_parts.set_color, BLACK, output_plane.lines_to_fade.set_stroke, {'width': 0})
        self.wait()
        self.play(LaggedStartMap(GrowFromCenter, dots, run_time=3))
        self.wait()
        self.play(LaggedStartMap(MoveToTarget, dots, path_arc=-TAU / 4, run_time=3))
        self.wait()
        self.play(LaggedStartMap(ApplyMethod, dots, lambda d: (d.set_fill, d.target_color)))
        self.wait()
        self.play(LaggedStartMap(ApplyMethod, dots, lambda d: (d.move_to, d.original_position), path_arc=TAU / 4, run_time=3))
        self.wait()
        self.play(FadeIn(input_coloring), Animation(input_plane), input_plane.white_parts.set_color, BLACK, input_plane.lines_to_fade.set_stroke, {'width': 0}, FadeOut(dots))
        self.wait()
        right_half_block.save_state()
        right_half_block.next_to(FRAME_X_RADIUS * RIGHT, RIGHT)
        self.play(right_half_block.restore)
        self.wait()
        inspector = DashedLine(ORIGIN, TAU * UP, dash_length=TAU / 24, fill_opacity=0, stroke_width=3, stroke_color=WHITE)
        ...
        self.play(inspector.move_to, input_plane.coords_to_point(-2.75, 2.75), inspector.set_stroke, {'width': 2})
        pink_points_label.next_to(inspector, RIGHT)
        self.play(Rotating(inspector, about_point=inspector.get_center(), rate_func=smooth, run_time=2), Write(pink_points_label))
        self.wait()
        self.play(right_half_block.next_to, FRAME_X_RADIUS * RIGHT, RIGHT)
        inspector_image_update_anim.update(0)
        self.play(ReplacementTransform(inspector.copy(), inspector_image, path_arc=-TAU / 4))
        self.play(ApplyMethod(inspector.move_to, input_plane.coords_to_point(-2, 0), path_arc=-TAU / 8, run_time=3), inspector_image_update_anim)
        self.play(ApplyMethod(inspector.move_to, input_plane.coords_to_point(-2.75, 2.75), path_arc=TAU / 8, run_time=3), inspector_image_update_anim)
        self.play(FadeOut(pink_points_label))
        zeros = tuple(it.starmap(input_plane.coords_to_point, [(-2.0, -1), (1, 1), (2, -2)]))
        for x in range(2):
            for zero in zeros:
                path = ParametricCurve(bezier([inspector.get_center(), input_plane.coords_to_point(0, 0), zero]), t_min=0, t_max=1)
        ...
        for dot in dots:
            dot.scale(1.5)
        self.play(FadeOut(input_coloring), input_plane.white_parts.set_color, WHITE, LaggedStartMap(GrowFromCenter, dots))
        self.wait()
        random.shuffle(dots.submobjects)
        self.play(LaggedStartMap(FadeOut, dots, lag_ratio=0.05, run_time=10))
        ...
        self.wait()
        quads = [(0, 0.5, 6, 6.25), (1, 1, 0.5, 2), (-1, -1, 3, 4.5), (0, 1.25, 5, 1.7), (-2, -1, 1, 1)]
        for x, y, width, height in quads:
            self.play(square.stretch_to_fit_width, width, square.stretch_to_fit_height, height, square.move_to, input_plane.coords_to_point(x, y))
            self.wait()

class SoWeFoundTheZeros(AltTeacherStudentsScene):

    def construct(self):
        self.student_says('Aha! So we \\\\ found the solutions!', target_mode='hooray', index=2, bubble_config={'direction': LEFT})
        self.wait()
...
class Rearrange2DEquation(AltTeacherStudentsScene):
    ...
    def construct(self):
        ...
        equations = VGroup(OldTex(g_tex, '', '=', h_tex, ''), OldTex(g_tex, '-', h_tex, '=', zero_tex))
        ...
        self.play(Succession(GrowFromCenter(brace), Write(zero_eq, run_time=1)), self.change_students(*['happy'] * 3))
...
class SearchForZerosInInputSpace(ColorMappedObjectsScene):
    CONFIG = {'func': example_plane_func}
...
class OneDRegionBoundary(Scene):
    CONFIG = {'graph_color': BLUE, 'region_rect_height': 0.1}
    ...
    def construct(self):
        ...
        fx0 = self.fx0 = -2
        fx1 = self.fx1 = 2
        axes = self.axes = Axes(x_min=-1, x_max=10, y_min=-3, y_max=3)
        axes.center()
        ...
        alt_graphs = [self.get_graph_part(*points) for points in [(-1, -2), (-1, -1, -1), (1, 1, 1), (-0.75, 0, 1.75), (-3, -2, -1)]]
        line = Line(axes.coords_to_point(x0, 0), axes.coords_to_point(x1, 0))
        region = Rectangle(stroke_width=0, fill_color=YELLOW, fill_opacity=0.5, height=self.region_rect_height)
        ...
        x0_arrow, x1_arrow = arrows = VGroup(*[Arrow(axes.coords_to_point(x, 0), axes.coords_to_point(x, fx), color=color, buff=0) for x, fx, color in [(x0, fx0, RED), (x1, fx1, GREEN)]])
        minus = OldTex('-')
        ...
        self.wait()
        self.play(ShowCreation(graph), FadeOut(region_words))
        self.wait()
    ...
    def get_graph_part(self, *interim_values):
        ...
        result.set_points_smoothly([self.axes.coords_to_point(x, fx) for x, fx in zip(np.linspace(self.x0, self.x1, len(values)), values)])
        return result
...
class DirectionOfA2DFunctionAlongABoundary(InputOutputScene):
    ...
    def construct(self):
        ...
        line = Line(input_plane.coords_to_point(-0.75, 2.5), input_plane.coords_to_point(2.5, -1.5))
        ...
        def get_color():
            return rev_to_color(get_angle() / TAU)
        out_vect = Vector(RIGHT, color=WHITE)
        out_vect_update_anim = UpdateFromFunc(out_vect, lambda ov: ov.put_start_and_end_on(output_plane.coords_to_point(0, 0), rect_image.get_points()[-1]).set_color(get_color()))
        dot = Dot()
        dot.set_stroke(BLACK, 1)
        ...
        self.play(GrowArrow(out_vect), GrowArrow(in_vect), Animation(dot))
        self.play(ShowCreation(rect), ShowCreation(rect_image), out_vect_update_anim, in_vect_update_anim, dot_update_anim, rate_func=bezier([0, 0, 1, 1]), run_time=10)
...
class AskAboutHowToGeneralizeSigns(AltTeacherStudentsScene):
    ...
    def construct(self):
        ...
        dot.fade(1)
        dot.center()
        question = OldTexText('Wait...what would \\\\ positive and negative \\\\ be in 2d?')
        self.student_says(question, target_mode='sassy', index=2, added_anims=[self.teacher.change, 'plain'], bubble_config={'direction': LEFT}, run_time=1)
        self.play(Write(plane, run_time=1), self.students[0].change, 'confused', self.students[1].change, 'confused')
...
class HypothesisAboutFullyColoredBoundary(ColorMappedObjectsScene):
    CONFIG = {'func': plane_func_from_complex_func(lambda z: z ** 3)}

    def construct(self):
        ColorMappedObjectsScene.construct(self)
        square = Square(side_length=4)
        square.color_using_background_image(self.background_image_file)
        hypothesis = OldTexText('Working Hypothesis: \\\\', 'If a 2d function hits outputs of all possible colors \\\\' + 'on the boundary of a 2d region,', 'that region \\\\ contains a zero.', alignment='')
        ...
        n = s.index('colors')
        hypothesis[1][n:n + len('colors')].set_color_by_gradient(BLUE, PINK, YELLOW)
...
class PiCreatureAsksWhatWentWrong(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait(2)
        self.play(Animation(VectorizedPoint(ORIGIN)), question.restore, randy.change, 'confused')
        self.wait(5)

class ForeverNarrowingLoop(InputOutputScene):
    CONFIG = {'target_coords': (1, 1), 'input_plane_corner': UP + RIGHT, 'shrink_time': 20, 'circle_start_radius': 2.25, 'start_around_target': False, 'add_convenient_waits': False}
    ...
    def construct(self):
        ...
        self.play(*it.chain([ApplyMethod(coloring.set_fill, {'opacity': 0.2}) for coloring in colorings], [ApplyMethod(plane.white_parts.set_color, WHITE) for plane in planes]), run_time=2)
        ...
        if self.start_around_target:
            ...
            circle.next_to(input_coloring.get_corner(self.input_plane_corner), -self.input_plane_corner, SMALL_BUFF)
        ...
        def optional_wait():
            if self.add_convenient_waits:
                self.wait()
        optional_wait()
        self.play(ShowCreation(circle), ShowCreation(circle_image), run_time=3, rate_func=bezier([0, 0, 1, 1]))
        optional_wait()
        self.play(circle.scale, 0, circle.move_to, input_plane.coords_to_point(*self.target_coords), circle_image_update_anim, run_time=self.shrink_time, rate_func=bezier([0, 0, 1, 1]))

class AltForeverNarrowingLoop(ForeverNarrowingLoop):
    CONFIG = {'target_coords': (-2, -1), 'input_plane_corner': DOWN + LEFT, 'shrink_time': 3}

class TinyLoop(ForeverNarrowingLoop):
    CONFIG = {'circle_start_radius': 0.5, 'start_around_target': True, 'shrink_time': 1, 'add_convenient_waits': True}

class TinyLoopAroundZero(TinyLoop):
    CONFIG = {'target_coords': (1, 1)}

class TinyLoopAroundBlue(TinyLoop):
    CONFIG = {'target_coords': (2.4, 0)}

class TinyLoopAroundYellow(TinyLoop):
    CONFIG = {'target_coords': (0, -1.3)}

class TinyLoopAroundOrange(TinyLoop):
    CONFIG = {'target_coords': (0, -0.5)}
...
class FailureOfComposition(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        equation = VGroup(no_answers_in_equation[0], plus, no_answers_in_equation[1], equals, yes_answers_in_equation)
        ...
        self.wait()
        self.play(small_squares.arrange, DOWN, {'buff': 0}, small_squares.move_to, big_square, no_answers.space_out_submobjects, 0.9)
        self.add(big_square)
        no_answers_copy = no_answers.copy()
        small_squares.save_state()
        self.play(Transform(no_answers, no_answers_in_equation), Write(plus_equals), small_squares.set_stroke, {'width': 0})
        self.play(Write(yes_answers), Write(yes_answers_in_equation))
        self.play(LaggedStartMap(FadeIn, q_marks, run_time=1, lag_ratio=0.8))
        self.wait(2)
        self.play(small_squares.restore, FadeOut(yes_answers), FadeIn(no_answers_copy))
        self.wait()
        self.play(small_squares.set_stroke, {'width': 0}, FadeOut(no_answers_copy), FadeIn(yes_answers))
        self.wait()
        cross = Cross(question)
        self.play(ShowCreation(cross, run_time=2), FadeOut(equation), FadeOut(no_answers), FadeOut(q_marks), FadeOut(yes_answers))
        ...
        ghost.background_image_file = None
        self.play(small_squares.set_stroke, {'width': 0}, x_plus_y.restore)
        self.play(ShowPassingFlash(ghost))
        self.wait()

class PathContainingZero(InputOutputScene, PiCreatureScene):
    CONFIG = {'default_pi_creature_kwargs': {'flip_at_start': False, 'height': 1.5}, 'default_pi_creature_start_corner': DOWN + LEFT}
    ...
    def draw_path_hitting_zero(self):
        morty = self.pi_creature
        path = self.path = VMobject(stroke_width=5, stroke_color=WHITE, fill_opacity=0)
        path.match_background_image_file(self.input_coloring)
        path.set_points_smoothly(list(it.starmap(self.input_plane.coords_to_point, [(1, 2.5), (2.5, 2.5), (2, 0.5), (1, 1), (0.5, 1), (0.5, 2), (1, 2.5)])))
        ...
        self.play(Flash(VectorizedPoint(self.output_plane.coords_to_point(0, 0)), color=WHITE, flash_radius=0.3, line_length=0.2, num_lines=13, rate_func=squish_rate_func(smooth, 0.5, 0.6)), morty.change, 'pondering', *[ShowCreation(mob, rate_func=bezier([0, 0, 1, 1])) for mob in (path, out_path)], run_time=5)
    ...
    def comment_on_zero(self):
        ...
        new_words = OldTexText('But we want $\\vec{\\textbf{x}}$ \\\\', 'where $f(\\vec{\\textbf{x}}) = 0$')
        new_words.move_to(words)
        self.play(FadeIn(background_rect), Write(words[0]), GrowArrow(arrow))
        self.play(Write(words[1]), morty.change, 'pleading', MoveToTarget(out_circle, run_time=2))
        self.wait()
...
class TransitionFromPathsToBoundaries(ColorMappedObjectsScene):
    CONFIG = {'func': plane_func_by_wind_spec((-2, 0, 2), (2, 0, 1)), 'dot_fill_opacity': 1, 'dot_stroke_width': 1, 'include_walkers': True, 'include_question_mark': True}

    def construct(self):
        ColorMappedObjectsScene.construct(self)
        squares, joint_rect = self.get_squares_and_joint_rect()
        left_square, right_square = squares
        path1, path2 = paths = VGroup(*[Line(square.get_corner(UP + LEFT), square.get_corner(UP + RIGHT)) for square in squares])
        ...
        self.play(MoveAlongPath(dot, joint_path, run_time=3), FadeOut(VGroup(path1.brace, path2.brace)), FadeIn(joint_path.brace))
        self.wait()
        self.play(FadeOut(paths), FadeOut(joint_path.brace), dot.move_to, path1.get_start())
        for square in squares:
            self.position_dot(square.get_points()[0])
            kwargs = {'run_time': 4, 'rate_func': bezier([0, 0, 1, 1])}
            self.play(MoveAlongPath(dot, square.copy(), **kwargs), ShowCreation(square, **kwargs), Write(square.label, run_time=2))
            self.wait()
        self.play(dot.move_to, joint_rect.get_points()[0], FadeOut(squares), FadeIn(joint_rect))
        self.position_dot(joint_rect.get_points()[0])
        self.play(Transform(left_square.label[0], joint_rect.label[0]), Transform(left_square.label[1], joint_rect.label[1][0], path_arc=TAU / 6), FadeIn(joint_rect.label[1][1]), FadeIn(joint_rect.label[1][3:]), FadeOut(right_square.label[0]), Transform(right_square.label[1], joint_rect.label[1][2], path_arc=TAU / 6), MoveAlongPath(dot, joint_rect, run_time=6, rate_func=bezier([0, 0, 1, 1])))
        self.wait()
    ...
    def get_dot_and_add_continual_animations(self):
        ...
        label_upadte = ContinualChangingDecimal(label, get_total_winding, position_update_func=lambda l: l.next_to(dot, UP + LEFT, SMALL_BUFF))
        arrow_length = 0.75
...
class TransitionFromPathsToBoundariesArrowless(TransitionFromPathsToBoundaries):
    CONFIG = {'func': plane_func_by_wind_spec((-2, 0, 2), (2, 0, 1)), 'dot_fill_opacity': 0, 'dot_stroke_width': 0, 'include_walkers': False, 'include_question_mark': False}
...
class BreakDownLoopWithNonzeroWinding(TransitionFromPathsToBoundaries):
    ...
    def construct(self):
        ...
        self.position_dot(joint_rect.get_points()[0])
        self.add(joint_rect)
        self.play(MoveAlongPath(dot, joint_rect, rate_func=bezier([0, 0, 1, 1])), Write(joint_rect.label, rate_func=squish_rate_func(smooth, 0.7, 1)), run_time=4)
        self.wait()
        self.play(ReplacementTransform(joint_rect.label, left_square.label), ReplacementTransform(joint_rect.label.copy(), right_square.label), ReplacementTransform(joint_rect.label[1].copy(), three), FadeIn(left_square), FadeIn(right_square))
        self.play(ReplacementTransform(left_square.label[1].copy(), x), ReplacementTransform(right_square.label[1].copy(), y), FadeIn(plus), FadeIn(equals))
...
class MonomialTerm(PathContainingZero):
    CONFIG = {'non_renormalized_func': plane_func_from_complex_func(lambda z: z ** 5), 'full_func_label': 'f(x) = x^5', 'func_label': 'x^5', 'loop_radius': 1.1, 'label_buff': 0.3, 'label_move_to_corner': ORIGIN, 'should_end_with_rescaling': True}
    ...
    def add_function_label(self):
        ...
        arrow = Arrow(2 * LEFT, 2 * RIGHT, path_arc=-TAU / 3)
    ...
    def show_winding(self):
        ...
        def generate_label_update(label, point_func, origin):
            return UpdateFromFunc(label, lambda m: m.move_to((1 + buff) * point_func() - buff * origin, self.label_move_to_corner))
        x = OldTex('x')
        ...
        morty = self.pi_creature
        kwargs = {'run_time': 15, 'rate_func': None}
        self.play(ShowCreation(loop, **kwargs), ShowCreation(out_loop, **kwargs), update_dot, update_out_dot, update_x, update_fx, ApplyMethod(morty.change, 'pondering', out_dot))
...
class PolynomialTerms(MonomialTerm):
    CONFIG = {'non_renormalized_func': plane_func_from_complex_func(lambda z: z ** 5 - z - 1), 'full_func_label': 'f(x) = x^5 - x - 1', 'func_label': 'x^5 + \\cdots', 'loop_radius': 2.0, 'label_buff': 0.15, 'label_move_to_corner': DOWN + LEFT, 'should_end_with_rescaling': False}
    ...
    def sweep_through_loop_interior(self):
        loop = self.loop
        morty = self.pi_creature
        line, line_target = [Line(loop.get_left(), loop.get_right(), path_arc=u * TAU / 2, n_arc_anchors=40, background_image_file=self.input_coloring.background_image_file, stroke_width=4) for u in (-1, 1)]
        out_line = line.copy()
        update_out_line = UpdateFromFunc(out_line, lambda m: m.set_points(line.get_points()).apply_function(self.point_function))
        self.play(Transform(line, line_target, run_time=10, rate_func=there_and_back), update_out_line, morty.change, 'hooray')
        self.wait()
    ...
    def cinch_loop(self):
        ...
        morty = self.pi_creature
        update_out_loop = UpdateFromFunc(out_loop, lambda m: m.set_points(loop.get_points()).apply_function(self.point_function))
        self.add(loop.copy().set_stroke(width=1), out_loop.copy().set_stroke(width=1))
        self.play(ApplyMethod(loop.scale, 0, {'about_point': self.input_plane.coords_to_point(0.2, 1)}, run_time=12, rate_func=bezier([0, 0, 1, 1])), update_out_loop, morty.change, 'hooray')
        self.wait()

class SearchSpacePerimeterVsArea(EquationSolver2d):
    CONFIG = {'func': example_plane_func, 'num_iterations': 15, 'display_in_parallel': False, 'use_fancy_lines': True}
    ...
    def construct(self):
        ...
        full_rect = SurroundingRectangle(path_parts, stroke_width=0, fill_color=WHITE, fill_opacity=1, background_image_file=path_parts[0].background_image_file)
        full_rect.save_state()
        full_rect.stretch(0, 1, about_edge=UP)
        self.play(FadeOut(non_path_parts), path_parts.set_stroke, {'width': 1})
        self.remove(all_parts)
        for x in range(2):
            alt_path_parts.save_state()
            self.play(LaggedStartMap(FadeIn, alt_path_parts, rate_func=there_and_back, lag_ratio=0.3, run_time=3, remover=True))
            alt_path_parts.restore()
        self.play(full_rect.restore, run_time=2)
        self.wait()
...
class ShowComplexFunction(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ReplacementTransform(x_dot.copy(), fx_dot, path_arc=arrow.path_arc), ShowCreation(arrow, rate_func=squish_rate_func(smooth, 0.2, 1)))
        self.play(FadeIn(fx_label))
        self.wait(2)
        self.play(MoveToTarget(fx_label), *list(map(FadeOut, [title, x_dot, x_label, arrow, fx_dot])))
        self.play(FadeOut(plane.coordinate_labels))
        self.wait()

class WindingNumbersInInputOutputContext(PathContainingZero):
    CONFIG = {'in_loop_center_coords': (-2, -1), 'run_time': 10}
    ...
    def construct(self):
        ...
        out_loop.match_background_image_file(self.output_coloring)
        update_out_loop = Mobject.add_updater(out_loop, lambda m: m.set_points(in_loop.get_points()).apply_function(self.point_function))
        ...
        update_out_arrow = Mobject.add_updater(out_arrow, lambda a: a.put_start_and_end_on(self.output_plane.coords_to_point(0, 0), out_loop.point_from_proportion(1)))
        update_out_arrow_color = Mobject.add_updater(out_arrow, lambda a: a.set_color(rev_to_color(a.get_angle() / TAU)))
        ...
        self.play(ShowCreation(in_loop), ShowCreation(out_loop), ChangeDecimalToValue(decimal, 2), Animation(in_dot), run_time=self.run_time, rate_func=bezier([0, 0, 1, 1]))
...
class MentionQAndA(Scene):
    ...
    def construct(self):
        ...
        title.set_color_by_tex_to_color_map({'Ben': MAROON, 'Sridhar': YELLOW})
        ...
        title.to_edge(UP)
        self.add(title)
        questions = VGroup(*list(map(TexText, ['If you think of the current videos as short stories, \\\\ what is the novel that you want to write?', 'How did you get into mathematics?', 'What motivated you to join 3b1b?', '$\\vdots$'])))
...
class InfiniteListOfTopics(Scene):
    ...
    def construct(self):
        ...
        lines = VGroup(*[OldTexText(words).scale(0.5) for words in ['Winding number', 'Laplace transform', 'Wallis product', 'Quantum information', 'Elliptic curve cryptography', 'Strange attractors', 'Convolutional neural networks', 'Fixed points']] + [Tex('\\vdots')])
...
class ManyIterations(Scene):
    ...
    def construct(self):
        words = VGroup(*[OldTexText(word, alignment='') for word in ['Winding numbers, v1', 'Winding numbers, v2 \\\\ (center on domain coloring)', 'Winding numbers, v3 \\\\ (clarify visuals of 2d functions)', 'Winding numbers, v4 \\\\ (postpone topology examples for part 2)', 'Winding numbers, v5 \\\\ (start down wrong path)']])
...
class MentionFree(PiCreatureScene):
    CONFIG = {'default_pi_creature_kwargs': {'flip_at_start': False}, 'default_pi_creature_start_corner': DOWN}

    def construct(self):
        morty = self.pi_creature
        morty.shift(RIGHT)
        items = VGroup(OldTexText('Movie:', '$>\\$10.00$'), OldTexText('College course:', '$>\\$1{,}000.00$'), OldTexText('YouTube video:', '$=\\$0.00$'))
        ...
        self.wait()
        self.play(FadeInFromDown(items[1]), items[0].shift, UP)
        self.wait()
...
class Thumbnail(SearchSpacePerimeterVsArea):
    CONFIG = {'num_iterations': 18, 'func': plane_func_by_wind_spec((-3, -1.3, 2), (0.1, 0.2, 1), (2.8, -2, 1))}
