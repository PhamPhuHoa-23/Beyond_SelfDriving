...
class Introduction(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(Animation(VectorizedPoint(self.hold_up_spot)), self.teacher.change, 'raise_right_hand')
        ...
        for student in self.students:
            ...
            always_shift(student.center_tracker, direction=DOWN + 3 * LEFT, rate=1.5 * random.random())
            movements.append(student.center_tracker)
        self.add(*movements)
        self.play_student_changes('pondering', 'sad', 'concerned_musician', look_at=10 * LEFT + 2 * DOWN)
        self.teacher_says('Wait, wait, wait!', target_mode='surprised')
        self.remove(*movements)
        self.play(self.change_students(*3 * ['hesitant']), *[Restore(student.center_tracker) for student in self.students])

class StudentsWatching(TeacherStudentsScene):

    def construct(self):
        self.play(self.teacher.change, 'raise_right_hand', self.change_students(*3 * ['thinking'], look_at=self.screen), VFadeIn(self.pi_creatures, run_time=2))
        self.wait(5)
...
class UnexpectedConnection(Scene):
    ...
    def construct(self):
        ...
        circle = Circle(color=YELLOW, stroke_width=1, radius=1.5)
        ...
        arrow.scale(1.5)
        arrow.fade(1)
        formula = OldTex('\\frac{\\pi^2}{6} = \\prod_{p \\text{ prime}}\\frac{1}{1 - p^{-2}}')
        formula.next_to(arrow.get_center(), RIGHT)

        def get_arc():
            angle = radius.get_angle()
            return Arc(start_angle=0, angle=angle, radius=circle.radius, stroke_color=YELLOW, stroke_width=5).shift(center)
        arc = always_redraw(get_arc)
        decimal = DecimalNumber(0)
        decimal.add_updater(lambda d: d.move_to(interpolate(radius.get_start(), radius.get_end(), 1.5)))
        ...
        self.play(Rotate(radius, PI - 1e-07, about_point=center), LaggedStartMap(FadeInFromDown, primes), run_time=4)
        self.remove(decimal)
        self.add(pi)
        self.wait()
        self.play(Restore(arrow), FadeIn(formula, LEFT))
        self.wait()

class MapOfVideo(MovingCameraScene):

    def construct(self):
        images = Group(ImageMobject('NecklaceThumbnail'), ImageMobject('BorsukUlamThumbnail'), ImageMobject('TopologyProofThumbnail'), ImageMobject('ContinuousNecklaceThumbnail'), ImageMobject('NecklaceSphereAssociationThumbnail'))
        for image in images:
        ...
        arrows = VGroup(Arrow(images[0], images[1], buff=SMALL_BUFF), Arrow(images[1].get_corner(DR) + 0.5 * LEFT, images[2].get_top() + 0.5 * LEFT), Arrow(images[2].get_top() + 0.5 * RIGHT, images[3].get_corner(DL) + 0.5 * RIGHT), Arrow(images[3], images[4], buff=SMALL_BUFF))
        ...
        for image in images:
            ...
            group.target.scale(FRAME_WIDTH / image.get_width(), about_point=ORIGIN)
...
class MathIsDeep(PiCreatureScene):
    ...
    def construct(self):
        ...
        top_arrows = VGroup(*[Arrow(c.get_bottom(), n.get_top()) for c, n in zip(math, num_mobs)])
        n_sum = Integer(sum(numbers))
        n_sum.scale(1.5)
        n_sum.next_to(num_mobs, DOWN, buff=1.5)
        low_arrows = VGroup(*[Arrow(n.get_bottom(), n_sum.get_top()) for n in num_mobs])
        ...
        self.play(LaggedStartMap(FadeInFromLarge, words, scale_factor=1.5, run_time=0.6, lag_ratio=0.6), self.pi_creature.change, 'pondering')
        self.play(TransformFromCopy(math, num_mobs), *map(GrowArrow, top_arrows))
        self.wait()
        self.play(TransformFromCopy(num_mobs, VGroup(n_sum)), self.pi_creature.change, 'thinking', *map(GrowArrow, low_arrows))
...
class MinimizeSharding(Scene):
    ...
    def construct(self):
        piece_groups = VGroup(*[VGroup(*[self.get_piece() for x in range(3)]).arrange(RIGHT, buff=SMALL_BUFF) for y in range(4)]).arrange(RIGHT, buff=SMALL_BUFF)
        self.add(piece_groups)
        self.play(*[ApplyMethod(mob.space_out_submobjects, 0.7) for mob in piece_groups])
        self.wait()
        group1 = piece_groups[:2]
        group2 = piece_groups[2:]
        self.play(group1.arrange, DOWN, group1.next_to, ORIGIN, LEFT, LARGE_BUFF, group2.arrange, DOWN, group2.next_to, ORIGIN, RIGHT, LARGE_BUFF)
        self.wait()

    def get_piece(self):
        jagged_spots = [ORIGIN, 2 * UP + RIGHT, 4 * UP + LEFT, 6 * UP]
        corners = list(it.chain(jagged_spots, [6 * UP + 10 * RIGHT], [p + 10 * RIGHT for p in reversed(jagged_spots)], [ORIGIN]))
...
class TopologyWordBreak(Scene):
    ...
    def construct(self):
        ...
        genus_labels = VGroup(*[OldTexText('Genus %d:' % d).scale(1.5).next_to(classes[d], LEFT, MED_LARGE_BUFF) for d in range(3)])
        genus_labels.shift(SMALL_BUFF * UP)
        self.play(Write(word))
        self.play(LaggedStartMap(ApplyMethod, word, lambda m: (m.set_color, m.target_color), run_time=1))
        self.play(LaggedStartMap(MoveToTarget, word), LaggedStartMap(FadeIn, signs), LaggedStartMap(FadeInFromDown, genus_labels))
        self.wait(3)
...
class FunctionGInSymbols(Scene):
    ...
    def construct(self):
        ...
        def color_tex(tex_mob):
            pairs = [(p_tex, YELLOW), (neg_p_tex, RED), ('{g}', GREEN)]
        ...
        self.wait()
        self.play(FadeInFromDown(equals), Write(dec_rhs), FadeOut(f_brace), FadeOut(p_brace))
        self.wait(2)
        self.play(WiggleOutThenIn(f))
        self.wait()
        self.play(FadeOut(dec_rhs, DOWN), FadeInFromDown(f_of_neg_p))
        self.wait()
        ...
        self.play(MoveToTarget(f_of_p, path_arc=PI), MoveToTarget(f_of_neg_p, path_arc=-PI), FadeInFromLarge(minus), FadeIn(zero_zero, LEFT))
        self.wait()
        def_eq = OldTex(':=')
        ...
        seeking_text = OldTex('\\text{Looking for }', p_tex, '\\text{ where}')
        ...
        self.play(FadeInFromLarge(g_of_p), FadeIn(def_eq, LEFT))
        ...
        eq2 = OldTex('=')
        rhs = OldTex('f', '(', neg_p_tex, ')', '-', 'f', '(', p_tex, ')', '=', '-', '{g}', '(', p_tex, ')')
        ...
        self.play(FadeIn(g_of_neg_p), FadeIn(eq2), FadeIn(neg_g_of_p), VGroup(seeking_text, g_equals_zero).shift, 1.5 * DOWN)
        self.wait()
        ...
        self.play(LaggedStartMap(ShowCreationThenDestruction, rects, lag_ratio=0.8))
        self.play(TransformFromCopy(f_of_p, rhs[5:9]), TransformFromCopy(f_of_neg_p, rhs[:4]), FadeIn(rhs[4]), FadeIn(rhs[-6]))
        self.wait()
...
class FunctionGInputSpace(SpecialThreeDScene):
    ...
    def show_input_dot(self):
        ...
        arrow = Arrow(start_point + (LEFT + OUT + UP), start_point, color=BLUE, buff=MED_LARGE_BUFF)
        ...
        self.play(point_mob.move_to, start_point, GrowArrow(arrow), FadeIn(p_label, IN))
        self.wait()
        self.play(arrow.scale, 0, {'about_point': arrow.get_end()}, p_label.next_to, dot, OUT + LEFT, SMALL_BUFF)
    ...
    def show_antipodal_point(self):
        ...
        end_dot = always_redraw(lambda: self.get_dot(path[-1].point_from_proportion(1)).set_color(RED))
        neg_p = OldTex('-\\vec{\\textbf{p}}')
        neg_p.add_updater(lambda p: p.next_to(end_dot, UP + RIGHT + IN))
        neg_p.set_color(RED)
        neg_p.set_shade_in_3d(True)
        self.move_camera(phi=100 * DEGREES, theta=30 * DEGREES, added_anims=[ShowCreation(path)], run_time=4)
        self.wait()
        ...
        self.wait(7)
        self.move_camera(phi=100 * DEGREES, theta=30 * DEGREES, run_time=2)
        self.wait(7)
        self.move_camera(phi=70 * DEGREES, theta=-120 * DEGREES, added_anims=[FadeOut(end_dot), FadeOut(neg_p), FadeOut(path)], run_time=2)
    ...
    def show_equator(self):
        ...
        for x in range(2):
            self.play(Rotate(point_mob, PI, about_point=ORIGIN, axis=OUT), run_time=4)
            self.wait(3)
        self.play(FadeOut(self.dot), FadeOut(self.p_label))
        self.equator = equator

    def deform_towards_north_pole(self):
        equator = self.equator
        self.play(UpdateFromAlphaFunc(equator, lambda m, a: m.become(self.get_lat_line(a * PI / 2)), run_time=16))
        self.wait()

    def init_tracked_point(self):
        self.tracked_point = VectorizedPoint([0, 0, 2])
        self.tracked_point.add_updater(lambda p: p.move_to(2 * normalize(p.get_center())))
    ...
    def get_start_path(self):
        path = ParametricCurve(lambda t: np.array([-np.sin(TAU * t + TAU / 4), np.cos(2 * TAU * t + TAU / 4), 0]), color=RED)
        path.scale(0.5)
        path.shift(0.5 * OUT)
    ...
    def get_antipodal_path(self):
        ...
        path = ParametricCurve(lambda t: 2.03 * np.array([0, np.sin(PI * t), np.cos(PI * t)]), color=YELLOW)
    ...
    def get_lat_line(self, lat=0):
        equator = ParametricCurve(lambda t: 2.03 * np.array([np.cos(lat) * np.sin(TAU * t), np.cos(lat) * -np.cos(TAU * t), np.sin(lat)]))
        equator.rotate(-90 * DEGREES)
        dashed_equator = DashedVMobject(equator, num_dashes=40, color=RED)
        dashed_equator.set_shade_in_3d(True)
        return dashed_equator

    def draw_path(self, path, run_time=4, dot_follow=True, uncreate=False, added_anims=None):
        added_anims = added_anims or []
        point_mob = self.tracked_point
        anims = [ShowCreation(path)]
        if dot_follow:
            anims.append(UpdateFromFunc(point_mob, lambda p: p.move_to(path.point_from_proportion(1))))
...
class FunctionGOutputSpace(FunctionGInputSpace):
    ...
    def setup(self):
        axes = self.axes = Axes(x_min=-2.5, x_max=2.5, y_min=-2.5, y_max=2.5, axis_config={'unit_size': 1.5})
        for axis in axes:
    ...
    def show_input_dot(self):
        ...
        self.wait(3)
        dc = dot.copy()
        self.play(FadeIn(dc, 2 * UP, remover=True), UpdateFromFunc(fp_label, lambda fp: fp.next_to(dc, UL, SMALL_BUFF)))
        self.add(dot)
    ...
    def draw_path(self, path, run_time=4, dot_follow=True, uncreate=False, added_anims=None):
        ...
        anims = [ShowCreation(flat_path), ShowCreation(shadow_path)]
        if dot_follow:
            anims.append(UpdateFromFunc(point_mob, lambda p: p.move_to(shadow_path.point_from_proportion(1))))
    ...
    def show_antipodal_point(self):
        dot = self.dot
        pre_path = VMobject().set_points_smoothly([ORIGIN, DOWN, DOWN + 2 * RIGHT, 3 * RIGHT + 0.5 * UP, 0.5 * RIGHT, ORIGIN])
        ...
        g_neg_p = OldTex('g', '(', '-\\vec{\\textbf{p}}', ')')
        g_neg_p[0].set_color(GREEN)
        g_neg_p[2].set_color(RED)
        g_neg_p.next_to(end_dot, UR, SMALL_BUFF)
        reflection_line = DashedLine(dot.get_center(), end_dot.get_center(), stroke_width=0)
        vector = Vector(dot.get_center())
        self.play(ShowCreation(path, run_time=1))
        self.wait()
        self.play(ShowCreationThenDestruction(reflection_line, run_time=2), TransformFromCopy(dot, end_dot), ReplacementTransform(gp_label.deepcopy().clear_updaters(), g_neg_p))
        self.wait()
        self.play(FadeIn(vector))
        self.play(Rotate(vector, angle=PI, about_point=ORIGIN))
        self.play(FadeOut(vector))
        self.play(FadeOut(end_dot), FadeOut(g_neg_p), FadeOut(path))
    ...
    def show_equator(self):
        ...
        self.play(point_mob.move_to, equator_start, dot.move_to, self.odd_func(equator_start))
        dot.add_updater(lambda m: m.move_to(self.odd_func(point_mob.get_center())))
        self.play(ShowCreation(equator), ShowCreation(flat_eq), run_time=4)
        for x in range(2):
            self.play(Rotate(point_mob, PI, about_point=ORIGIN, axis=OUT), run_time=4)
            self.wait(3)
        self.play(FadeOut(self.dot), FadeOut(self.gp_label))
        self.equator = equator
    ...
    def deform_towards_north_pole(self):
        ...
        self.play(UpdateFromAlphaFunc(equator, lambda m, a: m.become(self.get_lat_line(a * PI / 2)).set_stroke(width=0), run_time=16), UpdateFromFunc(flat_eq, lambda m: m.become(equator.deepcopy().apply_function(self.odd_func).set_stroke(color=RED, width=3))))
        self.wait()

    def func(self, point):
        x, y, z = point
        return 0.5 * self.axes.coords_to_point(2 * x + 0.5 * y + z, 2 * y - 0.5 * np.sin(PI * x) + z ** 2 + 1 - x)
...
class RotationOfEquatorGraphInOuputSpace(FunctionGOutputSpace):
    ...
    def construct(self):
        ...
        self.play(Rotate(VGroup(flat_eq, vector), PI, about_point=ORIGIN, run_time=5))
...
class DivisionOfUnity(Scene):
    ...
    def construct(self):
        ...
        v_lines = VGroup(*[DashedLine(0.2 * UP, 0.2 * DOWN).shift(factor * v) for v in [LEFT, 0.3 * LEFT, 0.1 * RIGHT, RIGHT]])
        upper_braces = VGroup(*[Brace(VGroup(vl1, vl2), UP) for vl1, vl2 in zip(v_lines[:-1], v_lines[1:])])
        ...
        self.play(LaggedStartMap(ShowCreation, v_lines[1:3], lag_ratio=0.8, run_time=1))
...
class ThreeDSpace(ThreeDScene):
    ...
    def construct(self):
        ...
        density = 1
        radius = 3
        lines = VGroup(*[VGroup(*[Line(radius * IN, radius * OUT, stroke_color=WHITE, stroke_width=1, stroke_opacity=0.5).shift(x * RIGHT + y * UP) for x in np.arange(-radius, radius + density, density) for y in np.arange(-radius, radius + density, density)]).rotate(n * 120 * DEGREES, axis=[1, 1, 1]) for n in range(3)])
...
class BorsukEndScreen(PatreonEndScreen):
    CONFIG = {'specific_patrons': ['Ali Yahya', 'Meshal Alshammari', 'Crypticswarm', 'Ankit Agarwal', 'Yu Jun', 'Shelby Doolittle', 'Dave Nicponski', 'Damion Kistler', 'Juan Benet', 'Othman Alikhan', 'Justin Helps', 'Markus Persson', 'Dan Buchoff', 'Derek Dai', 'Joseph John Cox', 'Luc Ritchie', 'Guido Gambardella', 'Jerry Ling', 'Mark Govea', 'Vecht ', 'Jonathan Eppele', 'Shimin Kuang', 'Rish Kundalia', 'Achille Brighton', 'Kirk Werklund', 'Ripta Pasay', 'Felipe Diniz'], 'n_patron_columns': 2}

class Thumbnail(SpecialThreeDScene):

    def construct(self):
        sphere = ParametricSurface(func=lambda u, v: 2 * np.array([np.cos(v) * np.sin(u) + 0.2 * np.cos(3 * u), np.sin(v) * np.sin(u), np.cos(u) + 0.2 * np.sin(4 * v) - 0.3 * np.cos(3 * u)]), resolution=(24, 48), u_min=0.001, u_max=PI - 0.001, v_min=0, v_max=TAU)
