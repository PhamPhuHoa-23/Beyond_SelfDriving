...
class LogoGeneration(LogoGenerationTemplate):
    ...
    def get_logo_animations(self, logo):
        ...
        return [FadeIn(logo.iris_background, rate_func=squish_rate_func(smooth, 0.25, 1), run_time=3), AnimationGroup(*[LaggedStartMap(Restore, layer, run_time=3, path_arc=180 * DEGREES, rate_func=squish_rate_func(smooth, a / 3.0, (a + 0.9) / 3.0), lag_ratio=0.8) for layer, a in zip(layers, [0, 2, 1, 0])]), Animation(logo.pupil)]
...
class ThinkingAboutAProof(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.add(bubble)
        self.play(FadeIn(you, LEFT), GrowArrow(you_arrow))
...
class SumOfIntegersProof(Scene):
    ...
    def construct(self):
        ...
        equation.scale(1.5)
        equation.to_edge(UP)
        one, two, three, dots, n = numbers = VGroup(*[equation.get_part_by_tex(tex, substring=False).copy() for tex in ('1', '2', '3', '\\cdots', 'n')])
...
class HoldUpProof(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        title.to_edge(UP)
        self.add(title)
        self.play(self.teacher.change, 'raise_right_hand', self.screen, self.change_students('pondering', 'confused', 'maybe', look_at=title))
        self.look_at(title)
        self.wait(5)
...
class MultipleDefinitionsOfAnEllipse(Scene):
    ...
    def construct(self):
        ...
        self.add(title)
        definitions = VGroup(OldTexText('1. Stretch a circle'), OldTexText('2. Thumbtack \\\\ \\quad\\, construction'), OldTexText('3. Slice a cone'))
        ...
        for definition in definitions:
            ...
            self.play(definition.set_fill, WHITE, 1, definition.scale, 1.2, {'about_edge': LEFT}, *list(map(Restore, others)))
            self.wait(2)
...
class StretchACircle(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ApplyMethod(plane_circle_group.stretch, 2, 0, run_time=2), LaggedStartMap(GrowArrow, arrows, run_time=1, lag_ratio=1))
        self.play(FadeOut(arrows))
        self.wait()
        self.play(Restore(plane_circle_group))
        self.play(GrowArrow(xy_arrow), Write(xy), FadeIn(start_point, UP))
        self.wait()
        self.add(circle_ghost)
        self.play(circle.stretch, 2, 0, ReplacementTransform(start_point.copy(), end_point), run_time=2)
...
class ShowArrayOfEccentricities(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Write(name), LaggedStartMap(GrowArrow, arrows), LaggedStartMap(Restore, eccentricity_labels))
        self.wait()
        self.play(Write(alt_name), MoveToTarget(name), morty.change, 'hooray', alt_name, VFadeIn(morty))
        ...
        for i in (0, -1):
            ...
            self.play(ShowCreationThenFadeAround(eccentricity_labels[i]), FadeOut(e_copy))
        self.wait()
        circle = ellipses[0]
        group = VGroup(*it.chain(eccentricity_labels, ellipses[1:], arrows, name, alt_name, [morty]))
        self.play(LaggedStartMap(FadeOutAndShiftDown, group), circle.set_height, 5, circle.center)
...
class ShowOrbits(ShowArrayOfEccentricities):
    ...
    def construct(self):
        ...
        comet_orbit.set_width(0.7 * FRAME_WIDTH, about_point=ORIGIN)
        ...
        self.play(earth_parts.set_height, comet_orbit.get_height() / 4.53, earth_parts.shift, 3 * RIGHT)
        comet_orbit.shift(3 * RIGHT)
        comet_orbit.save_state()
        Transform(comet_orbit, earth_orbit).update(1)
        self.play(Restore(comet_orbit, run_time=2), ChangingDecimal(eccentricity_label, lambda a: self.get_eccentricity(comet_orbit)), FadeOut(earth_orbit_words, UP), FadeInFromDown(comet_orbit_words))
...
class EccentricityInThumbtackCase(ShowArrayOfEccentricities):
    ...
    def construct(self):
        ...
        ellipse_target.scale(fdiv(sum(self.get_abc(ellipse)), sum(self.get_abc(ellipse_target))))
        ...
        eccentricity_value_update = self.get_eccentricity_value_update(eccentricity_label, ellipse)
        ...
        focus_distance_update = Mobject.add_updater(focus_distance, lambda m: m.set_width(inner_brace_update.mobject.get_width()).next_to(inner_brace_update.mobject, DOWN, SMALL_BUFF))
        ...
        numerator_update = Mobject.add_updater(numerator, lambda m: m.set_width(focus_distance.get_width()).next_to(fraction[1], UP, MED_SMALL_BUFF))
        fraction_arrow = Arrow(eccentricity_label.get_right(), fraction.get_top() + MED_SMALL_BUFF * UP, path_arc=-60 * DEGREES)
        fraction_arrow.pointwise_become_partial(fraction_arrow, 0, 0.95)
        ellipse_transformation = Transform(ellipse, ellipse_target, rate_func=there_and_back, run_time=8)
        self.add(ellipse)
        ...
        self.play(ellipse_transformation, VFadeIn(inner_brace_update.mobject), VFadeIn(outer_lines), VFadeIn(outer_lines_brace), VFadeIn(fraction), VFadeIn(fraction_arrow), VFadeIn(focus_distance), VFadeIn(diameter))
    ...
    def get_focus_to_focus_line_update(self, ellipse):
        return Mobject.add_updater(Line(LEFT, RIGHT, color=WHITE), lambda m: m.put_start_and_end_on(*self.get_foci(ellipse)))
    ...
    def get_focus_line_to_focus_line_brace_update(self, line):
        ...
        return Mobject.add_updater(brace, lambda b: b.match_width(line, stretch=True).next_to(line, DOWN, buff=SMALL_BUFF))
    ...
    def get_outer_dashed_lines(self, ellipse):
        ...
        return VGroup(line.move_to(ellipse, RIGHT), line.copy().move_to(ellipse, LEFT))
    ...
    def get_foci(self, ellipse):
        ...
        return [ellipse.get_center() + c * RIGHT, ellipse.get_center() + c * LEFT]
...
class EccentricityForSlicedConed(Scene):
    ...
    def construct(self):
        equation = OldTex('\\text{Eccentricity} = ', '{\\sin(', '\\text{angle of plane}', ')', '\\over', '\\sin(', '\\text{angle of cone slant}', ')}')
...
class AskWhyAreTheyTheSame(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        self.student_says('Why on earth \\\\ are these the same?', index=2, target_mode='sassy', bubble_config={'direction': LEFT})
        ...
        self.play(FadeOut(bubble), FadeOut(bubble.content), LaggedStartMap(FadeOutAndShift, self.students, lambda m: (m, 3 * DOWN)), ReplacementTransform(morty, baby_morty, path_arc=30 * DEGREES, run_time=2))
        ...
        self.play(ApplyMethod(bubble_group.shift, FRAME_WIDTH * LEFT, rate_func=running_start), baby_morty.change, 'awe')
...
class TriangleOfEquivalences(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(DrawBorderThenFill, arrows, lag_ratio=0.7, run_time=3))
        self.wait()
...
class ShowMeasurementBook(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(GrowArrow(arrow), FadeIn(words, RIGHT), self.change_students('thinking', 'happy', 'pondering', look_at=arrow))
        self.wait(3)
...
class AskAboutWhyYouWouldAddSpheres(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(Animation(graph_spot), randy.change, 'maybe', Write(why))
        self.wait(3)
        self.play(randy.change, 'pondering')
        self.play(why.to_corner, DR, why.set_fill, GREY_B, 0.5)
        self.wait()
...
class WriteConjecture(Scene):
    CONFIG = {'image_name': 'ConjectureLineEquivalenceBigSphere', 'q_coords': 1.28 * UP + 0.15 * LEFT, 'circle_point_coords': 0.84 * RIGHT + 0.05 * DOWN, 'tangent_point_coords': 0.85 * UP + 1.75 * LEFT, 'plane_line_color': GOLD, 'text_scale_factor': 0.75, 'shift_plane_word_down': False, 'include_image': False}
    ...
    def construct(self):
        ...
        equation = VGroup(OldTex('||'), words[0].in_equation, OldTex('||'), OldTex('='), OldTex('||'), words[1].in_equation, OldTex('||'))
...
class WriteConjectureV2(WriteConjecture):
    CONFIG = {'image_name': 'ConjectureLineEquivalenceSmallSphere', 'q_coords': 2.2 * LEFT + 1.3 * UP, 'circle_point_coords': 1.4 * LEFT + 2.25 * UP, 'tangent_point_coords': 0.95 * LEFT + 1.51 * UP, 'plane_line_color': PINK, 'text_scale_factor': 0.5, 'shift_plane_word_down': True, 'include_image': False}
...
class QuickGeometryProof(Scene):
    ...
    def construct(self):
        ...
        equation = OldTex('\\Delta OP_1Q \\cong \\Delta OP_2Q', tex_to_color_map={'O': BLUE, 'P_1': MAROON_B, 'P_2': MAROON_B, 'Q': YELLOW})
        equation.to_edge(UP)
        self.add(*equation)
        self.add(circle)
        self.add(OldTex('O').next_to(O, LEFT), OldTex('P_1').next_to(p1, UP).set_color(MAROON_B), OldTex('P_2').next_to(p2, DOWN).set_color(MAROON_B), OldTex('Q').next_to(Q, RIGHT).set_color(YELLOW))
        self.add(O_p1, O_p2, p1_Q, p2_Q, O_Q)
        self.add(Dot(O, color=BLUE), Dot(p1, color=MAROON_B), Dot(p2, color=MAROON_B), Dot(Q, color=YELLOW))
        self.add(elbow.copy().rotate(angle + PI, about_point=ORIGIN).shift(p1), elbow.copy().rotate(-angle + PI / 2, about_point=ORIGIN).shift(p2))
        self.add(ticks.copy().rotate(angle).move_to(O_p1), ticks.copy().rotate(-angle).move_to(O_p2))
...
class DetailsLeftAsHomework(PiCreatureScene):
    CONFIG = {'default_pi_creature_kwargs': {'color': GREY_BROWN, 'flip_at_start': False}, 'default_pi_creature_start_corner': DL}
...
class AskWhyYouWouldChooseThisProof(PiCreatureScene):
    ...
    def construct(self):
        ...
        for pi, mode in ((randy, 'hooray'), (other, 'tired')):
            self.play(GrowArrow(pi.arrow), FadeIn(pi.label, RIGHT), pi.change, mode)
        self.play(randy.change, 'raise_right_hand', screen, other.look_at, screen)
        self.wait()
...
class CreativeConstruction(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(lightbulb.next_to, randy, RIGHT, {'buff': LARGE_BUFF, 'aligned_edge': UP}, randy.change, 'pondering', VFadeIn(randy), FadeOut(dandelin, DOWN))
        self.play(self.get_light_shine(lightbulb), Blink(randy))
        self.wait()
        self.play(FadeInFromDown(kant), Write(bubble), Write(kant_words))
    ...
    def get_light_shine(self, lightbulb, max_radius=15.0, delta_r=0.025):
        rings = self.get_rings(lightbulb.get_center(), max_radius=15.0, delta_r=0.025)
...
class LockhartQuote(Scene):
    ...
    def construct(self):
        ...
        quote = OldTexText(*quote_parts, tex_to_color_map={mb_string: BLUE, ml_string: YELLOW}, alignment='')
        ...
        self.wait(2)
        self.play(LaggedStartMap(FadeOutAndShiftDown, quote, lag_ratio=0.2, run_time=5), LaggedStartMap(FadeOutAndShiftDown, pictures, run_time=3))
...
class ShowApollonianCircles(Scene):
    ...
    def construct(self):
        ...
        radii.append(1.0 / k4)
        centers = [ORIGIN, 3 * RIGHT, 4 * UP, 4 * UP + 3 * RIGHT]
...
class EllipseLengthsLinedUp(EccentricityInThumbtackCase):
    ...
    def construct(self):
        ...
        point_movement = cycle_animation(MoveAlongPath(point, ellipse, run_time=5, rate_func=linear))
        arrow = Vector(RIGHT, color=WHITE)
        arrow.to_edge(LEFT)
        q_mark = OldTex('?')
...
class ReactionToGlimpseOfGenius(TeacherStudentsScene, CreativeConstruction):
    ...
    def construct(self):
        ...
        rings = self.get_rings(lightbulb.get_center(), max_radius=15, delta_r=0.1)
        arrow = Vector(RIGHT, color=WHITE)
