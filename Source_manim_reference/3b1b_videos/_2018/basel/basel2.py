...
LIGHT_COLOR = YELLOW
DEGREES = TAU / 360
inverse_power_law = lambda maxint, scale, cutoff, exponent: lambda r: maxint * (cutoff / (r / scale + cutoff)) ** exponent
...
class AngleUpdater(ContinualAnimation):
    ...
    def update_mobject(self, dt):
        new_arc = self.angle_arc.copy().set_bound_angles(start=self.spotlight.start_angle(), stop=self.spotlight.stop_angle())
        new_arc.init_points()
...
class LightIndicator(Mobject):
    CONFIG = {'radius': 0.5, 'reading_height': 0.25, 'intensity': 0, 'opacity_for_unit_intensity': 1, 'fill_color': YELLOW, 'precision': 3, 'show_reading': True, 'measurement_point': ORIGIN, 'light_source': None}
    ...
    def __init__(self, light_sources_mob, factor, about_point=None, **kwargs):
        ...
        for submob in ls_target:
            if type(submob) == LightSource:
                new_sp = submob.source_point.copy()
    ...
    def update(self):
        ...
        for face, (c1, c2) in zip(self, adjacent_pairs(corners)):
            ...
            for a1, a2 in zip(alphas, alphas[1:]):
                face.add(Polygon(interpolate(source_point, c1, a1), interpolate(source_point, c1, a2), interpolate(source_point, c2, a2), interpolate(source_point, c2, a1), fill_color=self.fill_color, fill_opacity=self.ambient_light.opacity_function(a1 * distance), stroke_width=0))
...
class ThinkAboutPondScene(PiCreatureScene):
    CONFIG = {'default_pi_creature_class': Randolph}

    def construct(self):
        randy = self.pi_creature
        randy.to_corner(DOWN + LEFT)
        bubble = ThoughtBubble(width=11, height=8)
        circles = bubble[:3]
        angle = -15 * DEGREES
...
class IntroScene(PiCreatureScene):
    CONFIG = {'rect_height': 0.075, 'duration': 1.0, 'eq_spacing': 3 * MED_LARGE_BUFF, 'n_rects_to_show': 30}
    ...
    def build_up_euler_sum(self):
        ...
        euler_sum = self.euler_sum = OldTex('1', '+', '{1 \\over 4}', '+', '{1 \\over 9}', '+', '{1 \\over 16}', '+', '{1 \\over 25}', '+', '\\cdots', '=', arg_separator=' \\, ')
        ...
        number_line = self.number_line = NumberLine(x_min=0, color=WHITE, number_at_center=1, stroke_width=1, big_tick_numbers=[0, 1, 2, 3], numbers_to_show=np.arange(0, 5), unit_size=5, tick_frequency=0.2, line_to_number_buff=MED_LARGE_BUFF)
        ...
        for i in range(4):
            self.play(FadeIn(term_mobjects[i + 1]), FadeIn(plusses[i]))
            anims = [rect_anims[i + 1], rect_label_anims[i + 1]]
            if i == 0:
                anims += [FadeIn(equals_sign), FadeIn(partial_sum_decimal)]
            elif i <= 5:
                anims += [ChangeDecimalToValue(partial_sum_decimal, series_terms[i + 1], run_time=1, num_decimal_places=6, position_update_func=lambda m: m.next_to(equals_sign, RIGHT))]
            self.play(*anims)
        for i in range(4, len(series_terms) - 2):
            anims = [rect_anims[i + 1], ChangeDecimalToValue(partial_sum_decimal, series_terms[i + 1], num_decimal_places=6)]
            if i == 5:
                anims += [FadeIn(euler_sum[-3]), FadeIn(euler_sum[-2])]
        ...
        self.play(GrowFromCenter(brace), Write(q_marks), ChangeDecimalToValue(partial_sum_decimal, series_terms[-1], num_decimal_places=6), morty.change, 'confused')
        self.wait()
    ...
    def show_history(self):
        ...
        brace = Brace(VGroup(self.euler_sum, self.partial_sum_decimal.target), DOWN, buff=SMALL_BUFF)
        basel_text = brace.get_text('Basel problem', buff=SMALL_BUFF)
        self.number_line_group.save_state()
        self.play(pietro.next_to, ORIGIN, LEFT, LARGE_BUFF, self.number_line_group.next_to, FRAME_Y_RADIUS * DOWN, DOWN, morty.change, 'pondering')
        self.wait(2)
        self.play(euler.next_to, ORIGIN, RIGHT, LARGE_BUFF)
        self.wait(2)
        self.play(ReplacementTransform(self.q_marks, pi_answer), FadeIn(equals_sign), FadeOut(self.brace), MoveToTarget(self.partial_sum_decimal))
        self.wait()
        ...
        self.wait(2)
        self.play(GrowFromCenter(brace), euler.to_edge, DOWN, pietro.to_edge, DOWN, self.number_line_group.restore, self.number_line_group.shift, LARGE_BUFF * RIGHT)
    ...
    def other_pi_formulas(self):
        ...
        self.leibniz_sum = OldTex('1-{1\\over 3}+{1\\over 5}-{1\\over 7}+{1\\over 9}-\\cdots', '=', '{\\pi \\over 4}')
        self.wallis_product = OldTex('{2\\over 1} \\cdot {2\\over 3} \\cdot {4\\over 3} \\cdot {4\\over 5}' + '\\cdot {6\\over 5} \\cdot {6\\over 7} \\cdots', '=', '{\\pi \\over 2}')
        self.leibniz_sum.next_to(self.euler_sum.get_part_by_tex('='), DOWN, buff=self.eq_spacing, submobject_to_align=self.leibniz_sum.get_part_by_tex('='))
        self.wallis_product.next_to(self.leibniz_sum.get_part_by_tex('='), DOWN, buff=self.eq_spacing, submobject_to_align=self.wallis_product.get_part_by_tex('='))
    ...
    def refocus_on_euler_sum(self):
        ...
        self.play(FadeOut(self.leibniz_sum), FadeOut(self.wallis_product), ApplyMethod(self.euler_sum.shift, ORIGIN + 2 * UP - self.euler_sum.get_center()))
        pi_squared = self.euler_sum.get_part_by_tex('\\pi')[-3]
        self.play(ScaleInPlace(pi_squared, 2, rate_func=wiggle))
        q_circle = Circle(stroke_color=YELLOW, fill_color=YELLOW, fill_opacity=0.5, radius=0.4, stroke_width=10.0)
        q_mark = OldTex('?')
...
class MathematicalWebOfConnections(PiCreatureScene):
    ...
    def show_other_pi_formulas(self):
        jerk, randy = self.pi_creatures
        words = self.words
        basel_sum = OldTex('1 + {1 \\over 4} + {1 \\over 9} + {1 \\over 16} + \\cdots', '=', '{\\pi^2 \\over 6}')
        leibniz_sum = OldTex('1-{1\\over 3}+{1\\over 5}-{1\\over 7}+{1\\over 9}-\\cdots', '=', '{\\pi \\over 4}')
        wallis_product = OldTex('{2\\over 1} \\cdot {2\\over 3} \\cdot {4\\over 3} \\cdot {4\\over 5}' + '\\cdot {6\\over 5} \\cdot {6\\over 7} \\cdots', '=', '{\\pi \\over 2}')
        ...
        self.play(jerk.change, 'sassy', randy.change, 'raise_right_hand', FadeOut(jerk.bubble), words.next_to, jerk, UP, FadeIn(basel_sum, lag_ratio=0.5, run_time=3))
    ...
    def question_fundamental(self):
        ...
        self.play(fundamentally.move_to, self.pi_creatures, fundamentally.shift, UP, FadeOut(words), jerk.change, 'pondering', randy.change, 'pondering')
        self.wait()
    ...
    def draw_circle(self):
        ...
        VGroup(radius, semi_circle).move_to(FRAME_X_RADIUS * LEFT / 2 + FRAME_Y_RADIUS * UP / 2)
        ...
        self.play(Rotate(radius, np.pi, about_point=radius.get_start()), ShowCreation(semi_circle), ChangeDecimalToValue(decimal, np.pi, position_update_func=decimal_position_update_func), MaintainPositionRelativeTo(one, radius), run_time=3)
        self.wait(2)
        self.circle_group = VGroup(semi_circle, radius, one, decimal)

    def remove_all_but_basel_sum(self):
        to_shift_down = VGroup(self.circle_group, self.pi_creatures, self.fundamentally, self.formulas[1:])
        to_shift_down.generate_target()
        for part in to_shift_down.target:
            part.move_to(FRAME_HEIGHT * DOWN)
        basel_sum = self.formulas[0]
        self.play(MoveToTarget(to_shift_down), basel_sum.scale, 1.5, basel_sum.move_to, 1.5 * DOWN)
    ...
    def show_web_of_connections(self):
        ...
        dots = VGroup(*[Dot(radius=0.1).move_to((j - 0.5 * (i % 2)) * RIGHT + np.sqrt(3) / 2.0 * i * DOWN + 0.5 * (random.random() * RIGHT + random.random() * UP)) for i in range(4) for j in range(7 + i % 2)])
        dots.set_height(3)
        ...
        for x in range(100):
            ...
            edges.add(edge)
        path_dots = VGroup(dots[-7], dots[-14], dots[9], dots[19], dots[14])
        path_edges = VGroup(*[Line(d1.get_center(), d2.get_center(), color=RED) for d1, d2 in zip(path_dots, path_dots[1:])])
        ...
        self.play(Write(title), LaggedStartMap(ShowCreation, edges, run_time=3), LaggedStartMap(GrowFromCenter, dots, run_time=3))
        ...
        self.play(ShowCreation(circle), Rotate(radius, angle=0.999 * TAU, about_point=radius.get_start()), run_time=2)
        self.wait()
        graph = VGroup(dots, edges, path_edges, title)
        circle.add(radius)
        basel_sum.generate_target()
        basel_sum.target.to_edge(UP)
        arrow = Arrow(UP, DOWN, rectangular_stem_width=0.1, tip_length=0.45, color=RED)
        arrow.next_to(basel_sum.target, DOWN, buff=MED_LARGE_BUFF)
        self.play(MoveToTarget(basel_sum), graph.next_to, basel_sum.target, UP, LARGE_BUFF, circle.next_to, arrow, DOWN, MED_LARGE_BUFF)
    ...
    def show_light(self):
        light = AmbientLight(num_levels=500, radius=13, opacity_function=lambda r: 1.0 / (r + 1))
        pi = self.basel_sum[-1][0]
        pi.set_stroke(BLACK, 0.5)
        light.move_to(pi)
        self.play(SwitchOn(light, run_time=3), Animation(self.arrow), Animation(self.circle), Animation(self.basel_sum))
        self.wait()
...
class FirstLighthouseScene(PiCreatureScene):
    CONFIG = {'num_levels': 100, 'opacity_function': inverse_quadratic(1, 2, 1)}
    ...
    def show_lighthouses_on_number_line(self):
        number_line = self.number_line = NumberLine(x_min=0, color=WHITE, number_at_center=1.6, stroke_width=1, big_tick_numbers=list(range(1, 6)), numbers_to_show=list(range(1, 6)), unit_size=2, tick_frequency=0.2, line_to_number_buff=LARGE_BUFF, label_direction=DOWN)
        number_line.add_numbers()
        self.add(number_line)
        ...
        for i in range(1, NUM_CONES + 1):
            light_source = LightSource(opacity_function=self.opacity_function, num_levels=self.num_levels, radius=12.0)
        ...
        self.play(LaggedStartMap(SwitchOn, VGroup(*[ls.ambient_light for ls in light_sources]), run_time=5, lag_ratio=0.1, rate_func=rush_into), Animation(lighthouses))
        self.wait()
    ...
    def describe_brightness_of_each(self):
        ...
        light_indicator = LightIndicator(radius=INDICATOR_RADIUS, opacity_for_unit_intensity=OPACITY_FOR_UNIT_INTENSITY, color=LIGHT_COLOR)
        ...
        euler_sum_above = OldTex('1', '+', '{1\\over 4}', '+', '{1\\over 9}', '+', '{1\\over 16}', '+', '{1\\over 25}', '+', '{1\\over 36}')
        ...
        ls0.save_state()
        eye = morty.eyes[1]
        triangle = Polygon(number_line.number_to_point(1), eye.get_top(), eye.get_bottom(), stroke_width=0, fill_color=YELLOW, fill_opacity=1)
        ...
        for x in range(4):
            ...
            self.play(FadeOut(triangle.copy()), triangle_anim)
        self.play(FadeOut(triangle), ShowCreation(bubble), FadeIn(light_indicator))
        self.play(UpdateLightIndicator(light_indicator, 1), FadeIn(euler_sum_terms[0]))
        self.wait(2)
        for i in range(1, 5):
            self.play(ApplyMethod(ls0.move_to, light_sources[i], run_time=3), UpdateLightIndicator(light_indicator, 1.0 / (i + 1) ** 2, run_time=3), FadeIn(euler_sum_terms[i], run_time=3, rate_func=squish_rate_func(smooth, 0.5, 1)))
            self.wait()
        self.play(ApplyMethod(ls0.restore), UpdateLightIndicator(light_indicator, 1))
        self.play(LaggedStartMap(FadeIn, lighthouses[1:]), morty.change, 'hooray')
        self.play(LaggedStartMap(SwitchOn, VGroup(*[ls.ambient_light for ls in light_sources[1:]]), run_time=5, rate_func=rush_into), Animation(lighthouses), Animation(euler_sum_above), Write(plusses), UpdateLightIndicator(light_indicator, np.pi ** 2 / 6, run_time=5), morty.change, 'happy')
        self.wait()
        self.play(FadeOut(light_indicator.reading), FadeIn(limit_reading), morty.change, 'confused')
    ...
    def ask_about_rearrangements(self):
        ...
        morty = self.pi_creature
        self.play(LaggedStartMap(Rotate, light_sources, lambda m: (m, (2 * random.random() - 1) * 90 * DEGREES), about_point=origin, rate_func=lambda t: wiggle(t, 4), run_time=10, lag_ratio=0.9), morty.change, 'pondering')
...
class ThatJustSeemsUseless(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('How would \\\\ that help?', target_mode='sassy', index=2, bubble_config={'direction': LEFT})
        self.play(self.teacher.change, 'guilty', self.change_students(*3 * ['sassy']))
        self.wait()

class AskAboutBrightness(TeacherStudentsScene):
    CONFIG = {'num_levels': 200, 'radius': 10}

    def construct(self):
        light_source = LightSource(num_levels=self.num_levels, radius=self.radius, opacity_function=inverse_quadratic(1, 2, 1))
        ...
        self.student_says("What do you mean \\\\ by ``brightness''?", added_anims=[SwitchOn(light_source.ambient_light), Animation(light_source.lighthouse)])
        self.play(self.teacher.change, 'happy')
        self.wait(4)

class IntroduceScreen(Scene):
    CONFIG = {'num_levels': 100, 'radius': 10, 'num_rays': 250, 'min_ray_angle': 0, 'max_ray_angle': TAU, 'source_point': 2.5 * LEFT, 'observer_point': 3.5 * RIGHT, 'screen_height': 2}
    ...
    def setup_elements(self):
        ...
        screen = self.screen = Rectangle(width=0.05, height=self.screen_height, mark_paths_closed=True, fill_color=WHITE, fill_opacity=1.0, stroke_width=0.0)
        screen.next_to(observer_point, LEFT)
        screen_label = OldTexText('Screen')
        screen_label.next_to(screen, UP + LEFT)
        screen_arrow = Arrow(screen_label.get_bottom(), screen.get_center())
        morty = Mortimer()
        ...
        screen.save_state()
        self.play(FadeIn(morty), screen.match_height, morty.eyes, screen.next_to, morty.eyes, LEFT, SMALL_BUFF)
        self.play(Blink(morty))
        self.play(FadeOut(morty), FadeIn(camera), screen.scale, 2, {'about_edge': UP})
        self.wait()
        self.play(FadeOut(camera), screen.restore)
    ...
    def setup_angle(self):
        ...
        self.angle_arc = Arc(radius=3, start_angle=self.light_source.spotlight.start_angle(), angle=self.light_source.spotlight.opening_angle(), tip_length=ARC_TIP_LENGTH)
    ...
    def rotate_screen(self):
        self.add(Mobject.add_updater(self.light_source, lambda m: m.update()))
        self.add(Mobject.add_updater(self.angle_indicator, lambda m: m.set_stroke(width=0).set_fill(opacity=1)))
        self.remove(self.light_source.ambient_light)

        def rotate_screen(angle):
            self.play(Rotate(self.light_source.spotlight.screen, angle), Animation(self.angle_arc), run_time=2)
    ...
    def get_light_source(self):
        light_source = LightSource(opacity_function=inverse_quadratic(1, 2, 1), num_levels=self.num_levels, radius=self.radius, max_opacity_ambient=AMBIENT_FULL)
    ...
    def shoot_rays(self, show_creation_kwargs=None):
        ...
        rays = VGroup(*[Line(ORIGIN, self.radius * rotate_vector(RIGHT, angle)) for angle in np.arange(self.min_ray_angle, self.max_ray_angle, step_size)])
...
class EarthScene(IntroduceScreen):
    CONFIG = {'screen_height': 0.5, 'screen_thickness': 0, 'radius': 100 + FRAME_X_RADIUS, 'source_point': 100 * LEFT, 'min_ray_angle': -1.65 * DEGREES, 'max_ray_angle': 1.65 * DEGREES, 'num_rays': 100}
    ...
    def construct(self):
        ...
        black_rect = Rectangle(height=FRAME_HEIGHT, width=earth_radius + LARGE_BUFF, stroke_width=0, fill_color=BLACK, fill_opacity=1)
        black_rect.move_to(earth.get_center(), LEFT)
        self.add_foreground_mobjects(black_rect, earth)
        screen = self.screen = Line(self.screen_height * UP, ORIGIN, stroke_color=WHITE, stroke_width=self.screen_thickness)
        screen.move_to(earth.get_left())
        screen.generate_target()
        screen.target.rotate(-60 * DEGREES, about_point=earth_circle.get_center())
        equator_arrow = Vector(DOWN + 2 * RIGHT, color=WHITE)
        equator_arrow.next_to(screen.get_center(), UP + LEFT, SMALL_BUFF)
        pole_arrow = Vector(UP + 3 * RIGHT, color=WHITE, path_arc=-60 * DEGREES)
        ...
        sun = sun = LightSource(opacity_function=lambda r: 0.5, max_opacity_ambient=0, max_opacity_spotlight=0.5, num_levels=5, radius=self.radius, screen=screen)
        ...
        self.add(screen)
        self.play(SwitchOn(sunlight, rate_func=squish_rate_func(smooth, 0.7, 0.8)))
        ...
        self.wait()
        self.play(MoveToTarget(screen), Transform(equator_arrow, pole_arrow), Transform(equator_words, pole_words, rate_func=squish_rate_func(smooth, 0.6, 1)), Animation(sunlight), run_time=3)
        self.shoot_rays(show_creation_kwargs={'rate_func': lambda t: interpolate(0.98, 1, smooth(t))})
        self.wait()
...
class ShowLightInThreeDimensions(IntroduceScreen, ThreeDScene):
    ...
    def construct(self):
        ...
        screens = VGroup(Square(), RegularPolygon(8), Circle().insert_n_curves(25))
        ...
        self.move_camera(phi=60 * DEGREES, theta=-155 * DEGREES, run_time=3)
        self.begin_ambient_camera_rotation()
        kwargs = {'run_time': 2}
        self.play(screen.stretch, 0.5, 1, **kwargs)
        self.play(screen.stretch, 2, 2, **kwargs)
        self.play(Rotate(screen, TAU / 4, axis=UP + OUT, rate_func=there_and_back, run_time=3))
...
class InverseSquareLaw(ThreeDScene):
    CONFIG = {'screen_height': 1.0, 'source_point': 5 * LEFT, 'unit_distance': 4, 'num_levels': 100}
    ...
    def move_screen_farther_away(self):
        ...
        light_source = self.light_source = LightSource(opacity_function=lambda r: 1.0 / (r + 1), num_levels=self.num_levels, radius=10, max_opacity=0.2)
        ...
        light_indicator = self.light_indicator = LightIndicator(opacity_for_unit_intensity=0.5)
        ...
        continual_updates = self.continual_updates = [spotlight_update, light_indicator_update, shadow_update]
        ...
        self.shift_by_distance(1, GrowFromPoint(two_arrow, two_arrow.get_left()), Write(two, rate_func=squish_rate_func(smooth, 0.5, 1)))
        self.wait()
        q_marks = OldTexText('???')
        ...
        self.set_variables_as_attrs(ambient_light, spotlight, shadow, lighthouse, morty, arrow_group, *continual_updates)
    ...
    def morph_into_3d(self):
        ...
        morty = self.morty
        new_screen = Square(side_length=self.screen_height, stroke_color=WHITE, stroke_width=1, fill_color=WHITE, fill_opacity=0.5)
        ...
        self.move_camera(phi=60 * DEGREES, theta=-145 * DEGREES, added_anims=[ApplyFunction(lambda m: m.fade(1).shift(1.5 * DOWN), light_indicator, remover=True), FadeOut(morty)], run_time=2)
        self.wait()
        ...
        three_arrow = DoubleArrow(source_point + 4 * DOWN, source_point + 4 * DOWN + 3 * unit_distance * RIGHT, buff=0, color=WHITE)
        three = Integer(3)
        ...
        self.play(new_screen.scale, 2, {'about_edge': IN}, new_screen.shift, unit_distance * RIGHT, get_screen_copy_group_anim(1), run_time=2)
        self.wait()
        self.move_camera(phi=75 * DEGREES, theta=-155 * DEGREES, distance=7, run_time=10)
        self.begin_ambient_camera_rotation(rate=-0.01)
        self.add(ContinualAnimation(screen_copy_groups[2]))
        self.play(new_screen.scale, 3.0 / 2, {'about_edge': IN}, new_screen.shift, unit_distance * RIGHT, get_screen_copy_group_anim(2), GrowFromPoint(three_arrow, three_arrow.get_left()), Write(three, rate_func=squish_rate_func(smooth, 0.5, 1)), run_time=2)
        self.begin_ambient_camera_rotation(rate=-0.01)
        self.play(LaggedStartMap(ApplyMethod, screen_copy_groups[2], lambda m: (m.set_color, RED), run_time=5, rate_func=there_and_back))
        self.wait(2)
        ...
        for n in range(4, len(screen_copy_groups) + 1):
            ...
            self.play(new_screen.scale, float(n) / (n - 1), {'about_edge': IN}, new_screen.shift, unit_distance * RIGHT, get_screen_copy_group_anim(n - 1), last_group.fade, 1)
            last_group = group
            self.wait()

    def shift_by_distance(self, distance, *added_anims):
        anims = [self.screen.shift, self.unit_distance * distance * RIGHT]
...
class OtherInstanceOfInverseSquareLaw(Scene):
    ...
    def construct(self):
        ...
        items = VGroup(*[OldTexText('- %s' % s).scale(1) for s in ['Heat', 'Sound', 'Radio waves', 'Electric fields']])
        ...
        self.play(LaggedStartMap(FadeIn, items, run_time=4, lag_ratio=0.7), Succession(*[get_broadcast() for x in range(2)]))
...
class ScreensIntroWrapper(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play_student_changes('pondering', 'erm', 'confused', look_at=point)
        self.play(self.teacher.look_at, point)
        self.wait(5)

class ManipulateLightsourceSetups(PiCreatureScene):
    CONFIG = {'num_levels': 100, 'radius': 10, 'pi_creature_point': 2 * LEFT + 2 * DOWN}
    ...
    def construct(self):
        ...
        bubble.pin_to(morty)
        light_indicator = LightIndicator(opacity_for_unit_intensity=0.5, fill_color=YELLOW, radius=0.4, reading_height=0.2)
        ...
        self.play(ApplyMethod(light_source.shift, 0.66 * unit_distance * LEFT, rate_func=wiggle, run_time=5), morty.change, 'erm')
        self.play(UpdateFromAlphaFunc(light_source, lambda ls, a: ls.move_to(observer_point + rotate_vector(unit_distance * RIGHT, (1 + 1.0 / 8) * a * TAU)), run_time=6, rate_func=bezier([0, 0, 1, 1])), morty.change, 'pondering', UpdateFromFunc(morty, lambda m: m.look_at(light_source)))
        self.wait()
        plus = OldTex('+')
...
class TwoLightSourcesScene(ManipulateLightsourceSetups):
    CONFIG = {'num_levels': 200, 'radius': 15, 'a': 9, 'b': 5, 'origin_point': 5 * LEFT + 2.5 * DOWN}
    ...
    def construct(self):
        ...
        axes = Axes(x_min=-1, x_max=10.5, y_min=-1, y_max=6.5)
        ...
        xC = C[0]
        yC = C[1]
        prelim_matrix = np.array([[yA - yB, xB - xA], [xA - xB, yA - yB]])
        prelim_vector = np.array([xB * yA - xA * yB, xC * (xA - xB) + yC * (yA - yB)])
        H2 = np.linalg.solve(prelim_matrix, prelim_vector)
        H = np.append(H2, 0.0)
        lsA = LightSource(radius=self.radius, num_levels=self.num_levels, opacity_function=inverse_power_law(2, 1, 1, 1.5))
        lsB = lsA.deepcopy()
        ...
        identical_lighthouses_arrows = VGroup(*[Arrow(identical_lighthouses_words.get_corner(DOWN + LEFT), ls.get_source_point(), buff=SMALL_BUFF, color=WHITE) for ls in (lsA, lsB, lsC)])
        line_a = Line(C, A)
        ...
        perp_mark.shift(H)
        indicator = LightIndicator(color=LIGHT_COLOR, radius=INDICATOR_RADIUS, opacity_for_unit_intensity=OPACITY_FOR_UNIT_INTENSITY, show_reading=True, precision=2)
        ...
        theorem.set_color_by_tex_to_color_map({'a': line_a.get_color(), 'b': line_b.get_color(), 'h': line_h.get_color()})
        ...
        self.play(ShowCreation(axes, run_time=2), MoveToTarget(morty), FadeIn(indicator))
        self.add(lsC)
        indicator_update_anim.update(0)
        intensity = indicator.reading.number
        self.play(SwitchOn(lsC.ambient_light), FadeIn(lsC.lighthouse), UpdateFromAlphaFunc(indicator, lambda i, a: i.set_intensity(a * intensity)))
        self.add(indicator_update_anim)
        self.play(Animation(lsC), run_time=0)
        for point in (axes.coords_to_point(5, 2), H):
            self.play(lsC.move_source_to, point, path_arc=TAU / 4, run_time=1.5)
        self.wait()
        ...
        self.play(FadeOut(lsA), FadeOut(lsB))
        self.play(ShowCreation(rect))
        self.play(FadeOut(rect))
        self.play(FadeOut(lsC))
        self.add(lsA, lsB)
        self.play(FadeIn(lsA), FadeIn(lsB))
        self.play(ShowCreation(rect))
        self.play(FadeOut(rect))
        self.wait(2)
        self.add(lsC)
        self.play(FadeIn(lsC))
        self.play(Write(identical_lighthouses_words), LaggedStartMap(GrowArrow, identical_lighthouses_arrows))
        self.wait()
        self.play(*list(map(FadeOut, [identical_lighthouses_words, identical_lighthouses_arrows])))
        ...
        for ls, part in ((lsA, a_part), (lsB, b_part), (lsC, h_part)):
            self.add(ls)
            self.play(SwitchOn(ls.ambient_light, run_time=2), FadeIn(ls.lighthouse), part.restore)
        self.wait()
...
class SimpleIPTProof(Scene):
    ...
    def construct(self):
        ...
        triangle = VGroup(Line(C, A, color=BLUE), Line(C, B, color=RED), Line(A, B, color=WHITE), Line(C, H, color=GREEN))
        ...
        self.add(triangle)
        argument_lines = VGroup(OldTex('\\text{Area} = ', '{1 \\over 2}', 'a', 'b', '=', '{1 \\over 2}', 'c', 'h'), OldTex('\\Downarrow'), OldTex('a^2', 'b^2', '=', 'c^2', 'h^2'), OldTex('\\Downarrow'), OldTex('a^2', 'b^2', '=', '(', 'a^2', '+', 'b^2', ')', 'h^2'), OldTex('\\Downarrow'), OldTex('{1 \\over ', 'h^2}', '=', '{1 \\over ', 'b^2}', '+', '{1 \\over ', 'a^2}'))
        argument_lines.arrange(DOWN)
        for line in argument_lines:
            line.set_color_by_tex_to_color_map({'a': BLUE, 'b': RED, 'h': GREEN, 'Area': WHITE, 'Downarrow': WHITE})
...
class WeCanHaveMoreFunThanThat(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(RemovePiCreatureBubble(self.teacher, target_mode='raise_right_hand', look_at=point), self.change_students(*3 * ['pondering'], look_at=point))
        self.wait(3)

class IPTScene(TwoLightSourcesScene, ZoomedScene):
    CONFIG = {'max_opacity_ambient': 0.2, 'num_levels': 200}
    ...
    def construct(self):
        ...
        H2 = np.linalg.solve(prelim_matrix, prelim_vector)
        H = np.append(H2, 0.0)
        lsA = LightSource(radius=self.radius, num_levels=self.num_levels, opacity_function=inverse_power_law(2, 1, 1, 1.5), max_opacity_ambient=self.max_opacity_ambient)
        ...
        screen_arrow = Arrow(screen_word.get_bottom(), mini_triangle.target.get_center(), color=WHITE)
        theorem = OldTex('{1 \\over ', 'a^2}', '+', '{1 \\over', 'b^2}', '=', '{1 \\over', 'h^2}')
        theorem.set_color_by_tex_to_color_map({'a': line_a.get_color(), 'b': line_b.get_color(), 'h': line_h.get_color()})
        theorem_name = OldTexText('Inverse Pythagorean Theorem')
        theorem_name.to_corner(UP + RIGHT)
        ...
        def update_spotlight(spotlight):
            ...
            distance = max(get_norm(c1 - source_point), get_norm(c2 - source_point))
            n_parts = np.ceil(distance / dr)
            alphas = np.linspace(0, 1, n_parts + 1)
            for a1, a2 in zip(alphas, alphas[1:]):
                spotlight.add(Polygon(interpolate(source_point, c1, a1), interpolate(source_point, c1, a2), interpolate(source_point, c2, a2), interpolate(source_point, c2, a1), fill_color=YELLOW, fill_opacity=2 * lsC.ambient_light.opacity_function(a1 * distance), stroke_width=0))
        ...
        def get_spotlight_triangle(spotlight):
            ...
            return Polygon(sp, c1, c2, stroke_width=0, fill_color=YELLOW, fill_opacity=0.5)
        spotlights = VGroup(spotlight_a, spotlight_b)
        spotlights_update_anim = Mobject.add_updater(spotlights, update_spotlights)
        self.add(axes, lsA.ambient_light, lsB.ambient_light, lsC.ambient_light, line_c)
        self.add_foreground_mobjects(lsA.lighthouse, A_label, lsB.lighthouse, B_label, lsC.lighthouse, line_h, theorem, theorem_name, theorem_box)
        self.play(ShowCreation(mini_triangle, lag_ratio=0))
        self.play(MoveToTarget(mini_triangle), run_time=2)
        self.add_foreground_mobject(mini_triangle)
        self.play(Write(screen_word), GrowArrow(screen_arrow))
        self.wait()
        spotlights_update_anim.update(0)
        self.play(LaggedStartMap(FadeIn, spotlight_a), LaggedStartMap(FadeIn, spotlight_b), Animation(screen_arrow))
        ...
        def show_key_point(spotlight, new_point):
            ...
            self.play(Transform(screen, screen.alt_version), update_spotlight_anim)
            show_beaming_light(spotlight)
            self.play(screen.restore, update_spotlight_anim)
            self.wait()
            self.play(lsC.move_source_to, new_point, Transform(screen, screen.alt_version), update_spotlight_anim, run_time=2)
            show_beaming_light(spotlight)
            self.wait()
        ...
        self.play(FadeOut(spotlight_b), FadeIn(spotlight_a))
...
class DiameterTheorem(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        def update_triangle(triangle):
            triangle.set_points_as_corners([circle.get_left(), circle.get_right(), point.get_center(), circle.get_left()])
        ...
        self.play(self.teacher.change, 'raise_right_hand', DrawBorderThenFill(triangle), Write(diameter_word))
        ...
        for angle in (0.2 * TAU, -0.4 * TAU, 0.3 * TAU):
            ...
            self.play(MoveToTarget(point), MoveToTarget(perp_mark), path_arc=angle, run_time=3)
...
class InscribedeAngleThreorem(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        def update_shape(shape):
            shape.set_points_as_corners([point.get_center(), circle.point_from_proportion(7.0 / 8), circle.get_center(), circle.point_from_proportion(1.0 / 8), point.get_center()])
        ...
        theta_halves_update = UpdateFromFunc(theta_halves, lambda m: m.move_to(interpolate(point.get_center(), half_angle_mark.point_from_proportion(0.5), 2.5)))
        theta_halves_update.update(0)
        self.play(self.teacher.change, 'raise_right_hand', ShowCreation(shape, rate_func=linear))
        self.play(*list(map(FadeIn, [angle_mark, theta])))
        self.play(ShowCreation(half_angle_mark), Write(theta_halves), self.change_students(*['pondering'] * 3))
        ...
        for angle in (0.25 * TAU, -0.4 * TAU, 0.3 * TAU, -0.35 * TAU):
            ...
            self.play(MoveToTarget(point), MoveToTarget(half_angle_mark), theta_halves_update, path_arc=angle, run_time=3)
...
class PondScene(ThreeDScene):
    ...
    def construct(self):
        ...
        def zoom_out_scene(factor):
            ...
            phi0 = self.camera.get_phi()
            theta0 = self.camera.get_theta()
            ...
            self.play(ApplyMethod(self.camera.rotation_mobject.move_to, camera_target_point), self.zoomable_mobs.shift, self.obs_dot.get_center(), self.unzoomable_mobs.scale, 2, {'about_point': ORIGIN})
        ...
        self.zoomable_mobs.add(baseline)
        ...
        self.unzoomable_mobs.add(self.obs_dot)
        lake0 = Circle(radius=LAKE0_RADIUS, stroke_width=0, fill_color=LAKE_COLOR, fill_opacity=LAKE_OPACITY)
        ...
        arc_left = Arc(-TAU / 2, radius=LAKE0_RADIUS, start_angle=-TAU / 4, stroke_width=LAKE_STROKE_WIDTH, stroke_color=LAKE_STROKE_COLOR)
        ...
        arc_right = Arc(TAU / 2, radius=LAKE0_RADIUS, start_angle=-TAU / 4, stroke_width=LAKE_STROKE_WIDTH, stroke_color=LAKE_STROKE_COLOR)
        ...
        self.play(lake0.restore, lake_word.scale, 0.5, {'about_point': lake0.get_bottom()}, lake_word.fade, 1)
        ...
        self.play(SwitchOn(ls0.ambient_light), Animation(ls0.lighthouse))
        self.wait()
        self.play(morty.move_to, ls0.lighthouse, run_time=3, path_arc=TAU / 2, rate_func=there_and_back)
        self.play(ShowCreation(arc_right), Write(one_right))
        self.play(ShowCreation(arc_left), Write(one_left))
        self.play(lake0.set_stroke, {'color': LAKE_STROKE_COLOR, 'width': LAKE_STROKE_WIDTH})
        self.wait()
        ...
        diameter = DoubleArrow(diameter_start, diameter_stop, buff=0, color=WHITE)
        diameter_text = OldTex('d').scale(TEX_SCALE)
        diameter_text.next_to(diameter, RIGHT)
        self.play(GrowFromCenter(diameter), Write(diameter_text), FadeOut(ls0_dot))
        self.wait()
        ...
        self.play(ReplacementTransform(diameter_text[0].copy(), indicator_reading[2]), FadeIn(indicator_reading))
        self.wait()
        ...
        self.play(FadeOut(indicator_reading), ReplacementTransform(new_diameter_text_copy, new_reading, parth_arc=30 * DEGREES))
        indicator_reading = new_reading
        self.wait(2)
        self.play(FadeOut(one_left), FadeOut(one_right), FadeOut(new_diameter_text), FadeOut(arc_left), FadeOut(arc_right))
        self.add_foreground_mobjects(indicator, indicator_reading)
        self.unzoomable_mobs.add(indicator_reading)

        def indicator_wiggle():
            INDICATOR_WIGGLE_FACTOR = 1.3
            self.play(ScaleInPlace(indicator, INDICATOR_WIGGLE_FACTOR, rate_func=wiggle), ScaleInPlace(indicator_reading, INDICATOR_WIGGLE_FACTOR, rate_func=wiggle))
        ...
        def split_light_source(i, step, show_steps=True, animate=True, run_time=1):
            ...
            if show_steps == True:
                self.play(ShowCreation(leg1, run_time=run_time), ShowCreation(leg2, run_time=run_time))
        ...
        def construction_step(n, show_steps=True, run_time=1, simultaneous_splitting=False):
            if show_steps == True:
                ...
                self.play(FadeOut(self.hypotenuses), FadeOut(self.altitudes), FadeOut(self.inner_lake))
            else:
            ...
            new_outer_lake = Circle(radius=self.lake_radius, stroke_width=LAKE_STROKE_WIDTH, fill_color=LAKE_COLOR, fill_opacity=LAKE_OPACITY, stroke_color=LAKE_STROKE_COLOR)
            ...
            if simultaneous_splitting == False:
                ...
                for i in range(2 ** n):
                    split_light_source(i, step=n, show_steps=show_steps, run_time=run_time, animate=False)
                self.play(ReplacementTransform(old_ls, self.light_sources, run_time=run_time), ReplacementTransform(old_lake, self.outer_lake, run_time=run_time))
            self.legs = VMobject()
            ...
            self.add(self.light_sources, self.inner_lake, self.outer_lake)
            self.zoomable_mobs.add(self.light_sources, self.inner_lake, self.outer_lake)
            if show_steps == True:
                self.add(self.legs, self.hypotenuses, self.altitudes)
        ...
        self.lake_radius = 2 * LAKE0_RADIUS
        self.zoomable_mobs.add(self.inner_lake, self.outer_lake, self.altitudes, self.light_sources)
        self.add(self.inner_lake, self.outer_lake, self.legs, self.altitudes, self.hypotenuses)
        ...
        self.new_hypotenuses = []
        construction_step(0)
        my_triangle = triangle(self.light_sources[0].get_source_point(), OBSERVER_POINT, self.light_sources[1].get_source_point())
        angle_sign1 = right_angle(self.light_sources[0].get_source_point(), OBSERVER_POINT, self.light_sources[1].get_source_point(), size=RIGHT_ANGLE_SIZE)
        self.play(FadeIn(angle_sign1), FadeIn(my_triangle))
        angle_sign2 = right_angle(self.light_sources[1].get_source_point(), self.lake_center, OBSERVER_POINT, size=RIGHT_ANGLE_SIZE)
        ...
        open_sea = Rectangle(width=20 * scale, height=10 * scale, stroke_width=LAKE_STROKE_WIDTH, stroke_color=LAKE_STROKE_COLOR, fill_color=LAKE_COLOR, fill_opacity=LAKE_OPACITY).flip().next_to(origin_point, UP, buff=0)
        self.play(ReplacementTransform(pond_sources, nl_sources), ReplacementTransform(self.outer_lake, open_sea), FadeOut(self.inner_lake))
        self.play(FadeIn(self.number_line))
        self.wait()
        v = 4 * scale * UP
        self.play(nl_sources.shift, v, morty.shift, v, self.number_line.shift, v, indicator.shift, v, indicator_reading.shift, v, open_sea.shift, v, self.obs_dot.shift, v)
        self.number_line_labels.shift(v)
        origin_point = self.number_line.number_to_point(0)
        self.play(indicator.move_to, origin_point + scale * UP, indicator_reading.move_to, origin_point + scale * UP, FadeOut(open_sea), FadeOut(morty), FadeIn(self.number_line_labels))
        two_sided_sum = OldTex('\\dots', '+', '{1\\over (-11)^2}', '+', '{1\\over (-9)^2}', ' + ', '{1\\over (-7)^2}', ' + ', '{1\\over (-5)^2}', ' + ', '{1\\over (-3)^2}', ' + ', '{1\\over (-1)^2}', ' + ', '{1\\over 1^2}', ' + ', '{1\\over 3^2}', ' + ', '{1\\over 5^2}', ' + ', '{1\\over 7^2}', ' + ', '{1\\over 9^2}', ' + ', '{1\\over 11^2}', ' + ', '\\dots')
        ...
        for i, submob in zip(list(range(nb_symbols)), two_sided_sum.submobjects):
            ...
            if i == 0 or i % 2 == 1 or i == nb_symbols - 1:
        ...
        self.wait()
        covering_rectangle = Rectangle(width=FRAME_X_RADIUS * scale, height=2 * FRAME_Y_RADIUS * scale, stroke_width=0, fill_color=BLACK, fill_opacity=1)
        ...
        self.play(FadeIn(covering_rectangle), Transform(indicator_reading, half_indicator_reading), FadeOut(central_plus_sign))
        equals_sign = OldTex('=').scale(TEX_SCALE)
        equals_sign.move_to(central_plus_sign)
        p = 2 * scale * LEFT + central_plus_sign.get_center()[1] * UP
        self.play(indicator.move_to, p, indicator_reading.move_to, p, FadeIn(equals_sign))
...
class YayIPTApplies(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says('Heyo!  The Inverse \\\\ Pythagorean Theorem \\\\ applies!', bubble_config={'width': 5}, target_mode='surprised')
...
class ThinkBackToHowAmazingThisIs(ThreeDScene):
    CONFIG = {'x_radius': 100, 'max_shown_n': 20}
    ...
    def show_sum(self):
        number_line = NumberLine(x_min=-self.x_radius, x_max=self.x_radius, numbers_to_show=list(range(-self.max_shown_n, self.max_shown_n)))
        number_line.add_numbers()
        number_line.shift(2 * DOWN)
        positive_dots, negative_dots = [VGroup(*[Dot(number_line.number_to_point(u * x)) for x in range(1, int(self.x_radius), 2)]) for u in (1, -1)]
        ...
        plusses = VGroup(*[VGroup(*[OldTex('+').next_to(number_line.number_to_point(u * n), UP, buff=1.25) for u in (-1, 1)]) for n in range(0, self.max_shown_n, 2)])
        ...
        for term_mobs, plus_pair, partial_sum in tuples:
            self.play(FadeIn(term_mobs), Write(plus_pair, run_time=1), ChangeDecimalToValue(decimal, partial_sum), run_time=run_time)
            self.wait(run_time)
            run_time *= 0.9
        ...
        self.play(*list(map(FadeOut, it.chain(term_mobjects, plusses, number_line.numbers, [decimal]))))
...
class ButWait(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('But wait!', target_mode='angry', run_time=1)
        self.play_student_changes('sassy', 'angry', 'sassy', added_anims=[self.teacher.change, 'guilty'], run_time=1)
...
class FinalSumManipulationScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait()
        ls_template = LightSource(radius=1, num_levels=10, max_opacity_ambient=0.5, opacity_function=inverse_quadratic(1, 0.75, 1))
        odd_range = np.arange(1, 9, 2)
        even_range = np.arange(2, 16, 2)
        full_range = np.arange(1, 8, 1)
        self.number_line1 = NumberLine(x_min=0, x_max=11, color=LAKE_STROKE_COLOR, number_at_center=0, stroke_width=LAKE_STROKE_WIDTH, stroke_color=LAKE_STROKE_COLOR, number_scale_val=0.5, big_tick_numbers=[], unit_size=unit_length, tick_frequency=1, line_to_number_buff=MED_SMALL_BUFF, include_tip=True, label_direction=UP)
        ...
        for i in odd_range:
            if i == 1:
                term = OldTex('\\phantom{+\\,\\,\\,}{1\\over ' + str(i) + '^2}', fill_color=LIGHT_COLOR, stroke_color=LIGHT_COLOR)
            else:
                term = OldTex('+\\,\\,\\, {1\\over ' + str(i) + '^2}', fill_color=LIGHT_COLOR, stroke_color=LIGHT_COLOR)
        ...
        for ls, term in zip(odd_lights.submobjects, odd_terms.submobjects):
            self.play(FadeIn(ls.lighthouse, run_time=switch_on_time), SwitchOn(ls.ambient_light, run_time=switch_on_time), Write(term, run_time=switch_on_time))
        ...
        self.play(LaggedStartMap(FadeIn, full_lighthouses, lag_ratio=0.2, run_time=3))
        ...
        self.play(ShowCreation(quarter_arrow), Write(quarter_label))
        self.wait()
        ...
        self.play(FadeOut(quarter_label), FadeOut(quarter_arrow), FadeOut(even_lights), FadeOut(even_terms))
        self.wait()
        self.play(ReplacementTransform(three_quarters_arrow, four_thirds_arrow), ReplacementTransform(three_quarters_label, four_thirds_label))
        self.wait()
        full_terms = VMobject()
        for i in range(1, 8):
            if i == 1:
            ...
            full_terms.add(term)
        self.play(FadeOut(self.number_line1), FadeOut(odd_lights), FadeOut(self.number_line2), FadeOut(full_lights), FadeIn(full_terms))
        self.wait()
        v = (sum_vertical_spacing + 0.5) * UP
        self.play(odd_terms.shift, v, result1.shift, v, four_thirds_arrow.shift, v, four_thirds_label.shift, v, odd_terms.shift, v, full_terms.shift, v)
        ...
        self.play(FadeOut(result1), FadeOut(odd_terms), FadeOut(arrow_copy), FadeOut(label_copy), FadeOut(four_thirds_arrow), FadeOut(four_thirds_label), full_terms.shift, LEFT)
        self.wait()
        self.play(equation.shift, -equation.get_center()[1] * UP + UP + 1.5 * LEFT)
        result_box = Rectangle(width=1.1 * equation.get_width(), height=2 * equation.get_height(), color=LIGHT_COLOR3)
    ...
    def construct(self):
        ...
        def flash_arcs(n):
            ...
            for i in range(1, 2 ** n + 1):
                self.play(FadeOut(arcs[i - 1], run_time=FLASH_TIME), FadeIn(arcs[i], run_time=FLASH_TIME))
...
class Credits(Scene):
    ...
    def construct(self):
        credits = VGroup(*[VGroup(*list(map(TexText, pair))) for pair in [('Primary writer and animator:', 'Ben Hambrecht'), ('Editing, advising, narrating:', 'Grant Sanderson'), ('Based on a paper originally by:', 'Johan Wästlund')]])
        ...
        self.wait()
        self.play(credits.next_to, patreon_logo.get_bottom(), DOWN, MED_LARGE_BUFF, DrawBorderThenFill(patreon_logo))
        self.wait()

class Promotion(PiCreatureScene):
    CONFIG = {'seconds_to_blink': 5}
...
class Thumbnail(Scene):
    CONFIG = {'light_source_config': {'num_levels': 250, 'radius': 10.0, 'max_opacity_ambient': 1.0, 'opacity_function': inverse_quadratic(1, 0.25, 1)}}

    def construct(self):
        equation = OldTex('1', '+', '{1\\over 4}', '+', '{1\\over 9}', '+', '{1\\over 16}', '+', '{1\\over 25}', '+', '\\cdots')
        equation.scale(1.8)
        ...
        lake_radius = 6
        lake_center = ORIGIN
        lake = Circle(fill_color=BLUE, fill_opacity=0.15, radius=lake_radius, stroke_color=BLUE_D, stroke_width=3)
