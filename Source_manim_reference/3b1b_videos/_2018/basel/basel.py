...
class LightIndicator(VMobject):
    CONFIG = {'radius': 0.5, 'intensity': 0, 'opacity_for_unit_intensity': 1, 'precision': 3, 'show_reading': True, 'measurement_point': ORIGIN, 'light_source': None}
...
class IntroScene(PiCreatureScene):
    CONFIG = {'rect_height': 0.2, 'duration': 0.5, 'eq_spacing': 6 * MED_LARGE_BUFF}
    ...
    def build_up_euler_sum(self):
        self.euler_sum = OldTex('1', '+', '{1 \\over 4}', '+', '{1 \\over 9}', '+', '{1 \\over 16}', '+', '{1 \\over 25}', '+', '\\cdots', '=', arg_separator=' \\, ')
        ...
        self.partial_sum_decimal = DecimalNumber(partial_results_values[1], num_decimal_places=2)
        self.partial_sum_decimal.next_to(equals_sign, RIGHT)
        for i in range(4):
            FadeIn(self.partial_sum_decimal, run_time=self.duration)
            if i == 0:
                self.play(FadeIn(self.euler_sum[1], run_time=self.duration), FadeIn(self.euler_sum[2], run_time=self.duration), FadeIn(equals_sign, run_time=self.duration), FadeIn(self.partial_sum_decimal, run_time=self.duration))
            else:
                self.play(FadeIn(self.euler_sum[2 * i + 1], run_time=self.duration), FadeIn(self.euler_sum[2 * i + 2], run_time=self.duration), ChangeDecimalToValue(self.partial_sum_decimal, partial_results_values[i + 1], run_time=self.duration, num_decimal_places=6, show_ellipsis=True, position_update_func=lambda m: m.next_to(equals_sign, RIGHT)))
            self.wait()
        self.q_marks = OldTexText('???').set_color(LIGHT_COLOR)
        self.q_marks.move_to(self.partial_sum_decimal)
        self.play(FadeIn(self.euler_sum[-3], run_time=self.duration), FadeIn(self.euler_sum[-2], run_time=self.duration), ReplacementTransform(self.partial_sum_decimal, self.q_marks))
        self.wait()

    def build_up_sum_on_number_line(self):
        self.number_line = NumberLine(x_min=0, color=WHITE, number_at_center=1, stroke_width=1, big_tick_numbers=[0, 1, 2, 3], numbers_to_show=np.arange(0, 5), unit_size=5, tick_frequency=0.2, line_to_number_buff=MED_LARGE_BUFF).shift(LEFT)
        ...
        for i in range(5):
            self.play(GrowFromPoint(self.rects[i], self.euler_sum[2 * i].get_center(), run_time=1))
        ...
        for i in range(5, max_n1):
            if i == 5:
                ...
                self.play(FadeIn(self.ellipsis), GrowFromPoint(self.rects[i], self.euler_sum[10].get_center(), run_time=0.5))
            else:
                self.play(GrowFromPoint(self.rects[i], self.euler_sum[10].get_center(), run_time=0.5))
        for i in range(max_n1, max_n2):
            self.play(GrowFromPoint(self.rects[i], self.euler_sum[10].get_center(), run_time=0.01))
        self.wait()
        PI = TAU / 2
    ...
    def show_pi_answer(self):
        ...
        self.pi_answer.next_to(self.euler_sum[-1], RIGHT, buff=1, submobject_to_align=self.pi_answer[-2])
        self.play(ReplacementTransform(self.q_marks, self.pi_answer))
        self.wait()

    def other_pi_formulas(self):
        self.play(FadeOut(self.rects), FadeOut(self.number_line_labels), FadeOut(self.number_line), FadeOut(self.little_euler_terms), FadeOut(self.ellipsis), FadeOut(self.arrow))
        self.leibniz_sum = OldTex('1-{1\\over 3}+{1\\over 5}-{1\\over 7}+{1\\over 9}-\\cdots', '=', '\\quad\\,\\,{\\pi \\over 4}', arg_separator=' \\, ')
        self.wallis_product = OldTex('{2\\over 1} \\cdot {2\\over 3} \\cdot {4\\over 3} \\cdot {4\\over 5}' + '\\cdot {6\\over 5} \\cdot {6\\over 7} \\cdots', '=', '\\quad\\,\\, {\\pi \\over 2}', arg_separator=' \\, ')
        self.leibniz_sum.next_to(self.euler_sum.get_part_by_tex('='), DOWN, buff=2, submobject_to_align=self.leibniz_sum.get_part_by_tex('='))
        self.wallis_product.next_to(self.leibniz_sum.get_part_by_tex('='), DOWN, buff=2, submobject_to_align=self.wallis_product.get_part_by_tex('='))
    ...
    def refocus_on_euler_sum(self):
        ...
        self.play(WiggleOutThenIn(pi_squared, scale_value=4, angle=0.003 * TAU, run_time=2))
        q_circle = Circle(stroke_color=YELLOW, fill_color=YELLOW, fill_opacity=0.25, radius=0.5, stroke_width=3.0)
        q_mark = OldTex('?')
        q_mark.next_to(q_circle)
        thought = Group(q_circle, q_mark)
        q_mark.set_height(0.6 * q_circle.get_height())
        self.look_at(pi_squared)
        self.pi_creature_thinks(thought, target_mode='confused', bubble_config={'height': 2.5, 'width': 5})
        self.look_at(pi_squared)
        self.wait()
...
class FirstLighthouseScene(PiCreatureScene):
    ...
    def show_lighthouses_on_number_line(self):
        self.number_line = NumberLine(x_min=0, color=WHITE, number_at_center=1.6, stroke_width=1, big_tick_numbers=list(range(1, 5)), numbers_to_show=list(range(1, 5)), unit_size=2, tick_frequency=0.2, line_to_number_buff=LARGE_BUFF, label_direction=UP)
        ...
        self.wait()
        self.play(randy.change, 'wave_2', ShowCreation(bubble), FadeIn(light_indicator))
        light_sources = []
        ...
        for i in range(1, NUM_CONES + 1):
            light_source = LightSource(opacity_function=inverse_quadratic(1, AMBIENT_SCALE, 1), num_levels=NUM_LEVELS, radius=AMBIENT_RADIUS)
        ...
        for i, light_source in zip(list(range(NUM_VISIBLE_CONES)), light_sources[:NUM_VISIBLE_CONES]):
            ...
            indicator_rate_func = squish_rate_func(smooth, indicator_start_time, indicator_stop_time)
            self.play(SwitchOn(light_source.ambient_light), FadeIn(euler_sum_above[2 * i], run_time=SWITCH_ON_RUN_TIME, rate_func=indicator_rate_func), FadeIn(euler_sum_above[2 * i - 1], run_time=SWITCH_ON_RUN_TIME, rate_func=indicator_rate_func), ChangeDecimalToValue(light_indicator.reading, intensities[i], rate_func=indicator_rate_func, run_time=SWITCH_ON_RUN_TIME), ApplyMethod(light_indicator.foreground.set_fill, None, opacities[i], rate_func=indicator_rate_func, run_time=SWITCH_ON_RUN_TIME))
            if i == 0:
                self.wait()
        ...
        for i, light_source in zip(list(range(NUM_VISIBLE_CONES, NUM_CONES)), light_sources[NUM_VISIBLE_CONES:NUM_CONES]):
            ...
            indicator_rate_func = squish_rate_func(smooth, indicator_start_time, indicator_stop_time)
            self.play(SwitchOn(light_source.ambient_light, run_time=FAST_SWITCH_ON_RUN_TIME), ChangeDecimalToValue(light_indicator.reading, intensities[i - 1], rate_func=indicator_rate_func, run_time=FAST_SWITCH_ON_RUN_TIME), ApplyMethod(light_indicator.foreground.set_fill, None, opacities[i - 1]))
        ...
        self.play(FadeOut(light_indicator.reading), FadeIn(limit_reading), FadeIn(equals_sign))
        self.wait()
...
class SingleLighthouseScene(PiCreatureScene):
    ...
    def setup_elements(self):
        ...
        self.light_source = LightSource(opacity_function=inverse_quadratic(1, SPOTLIGHT_SCALE, 1), num_levels=NUM_LEVELS, radius=10, max_opacity_ambient=AMBIENT_FULL, max_opacity_spotlight=SPOTLIGHT_FULL)
        ...
        self.screen = Rectangle(width=0.06, height=2, mark_paths_closed=True, fill_color=WHITE, fill_opacity=1.0, stroke_width=0.0)
        ...
        self.add_foreground_mobject(morty)
        self.play(self.light_source.dim_ambient)
    ...
    def setup_angle(self):
        ...
        self.angle_arc = Arc(radius=5, start_angle=self.light_source.spotlight.start_angle(), angle=self.light_source.spotlight.opening_angle(), tip_length=ARC_TIP_LENGTH)
        self.angle_arc.move_arc_center_to(self.light_source.get_source_point())
        self.angle_indicator = DecimalNumber(arc_angle / DEGREES, num_decimal_places=0, unit='^\\circ', fill_opacity=1.0, fill_color=WHITE)
...
class EarthScene(Scene):
    ...
    def construct(self):
        ...
        self.screen.set_points_as_corners([[0, -self.screen_height / 2, 0], [0, self.screen_height / 2, 0]])
        earth_center_x = 2
        ...
        background_earth = SVGMobject(file_name='earth', width=2 * earth_radius, fill_color=BLUE)
        background_earth.move_to(earth_center)
        morty = Mortimer().scale(0.5).next_to(self.screen, RIGHT, buff=1.5)
        self.add_foreground_mobject(morty)
        sun_position = [-100, 0, 0]
        self.sun = LightSource(opacity_function=lambda r: 0.5, max_opacity_ambient=0, max_opacity_spotlight=0.5, num_levels=NUM_LEVELS, radius=150, screen=self.screen)
        ...
        screen_on_earth.move_to(np.array([earth_center_x - earth_radius * np.cos(theta), earth_radius * np.sin(theta), 0]))
        polar_morty = morty.copy().scale(0.5).next_to(screen_on_earth, DOWN, buff=0.5)
        polar_morty.set_color(BLUE_C)
        self.play(Transform(self.screen, screen_on_earth), Transform(morty, polar_morty))
        self.wait()
        ...
        self.play(Rotate(earth, theta0 + dtheta / 2, run_time=3), MoveToTarget(morty, path_arc=70 * DEGREES, run_time=3))
...
class ScreenShapingScene(ThreeDScene):
    ...
    def setup_elements(self):
        ...
        self.brightness_rect_height = 1.0
        self.screen = Line([3, -self.screen_height / 2, 0], [3, self.screen_height / 2, 0], path_arc=0, num_arc_anchors=10)
        self.light_source = LightSource(opacity_function=inverse_quadratic(1, 5, 1), num_levels=NUM_LEVELS, radius=10, max_opacity=0.2)
    ...
    def create_brightness_rect(self):
        brightness_rect0 = Rectangle(width=0, height=self.screen_height).move_to(self.screen.get_center())
        self.add_foreground_mobject(brightness_rect0)
        self.brightness_rect = Rectangle(width=self.brightness_rect_height, height=self.brightness_rect_height, fill_color=YELLOW, fill_opacity=0.5)
    ...
    def slant_screen(self):
        ...
        lower_slanted_screen_point = interpolate(lower_screen_point, self.spotlight.get_source_point(), SLANTING_AMOUNT)
        upper_slanted_screen_point = interpolate(upper_screen_point, self.spotlight.get_source_point(), -SLANTING_AMOUNT)
        ...
        self.slanted_screen = Line(lower_slanted_screen_point, upper_slanted_screen_point, path_arc=0, num_arc_anchors=10)
        self.slanted_brightness_rect.move_to(self.brightness_rect.get_center())
        self.play(Transform(self.screen, self.slanted_screen), Transform(self.brightness_rect, self.slanted_brightness_rect))

    def unslant_screen(self):
        self.wait()
        self.play(Transform(self.screen, self.unslanted_screen), Transform(self.brightness_rect, self.unslanted_brightness_rect))
    ...
    def left_shift_screen_while_showing_light_indicator(self):
        ...
        self.indicator = LightIndicator(radius=indicator_height / 2, opacity_for_unit_intensity=OPACITY_FOR_UNIT_INTENSITY, color=LIGHT_COLOR, precision=2)
        ...
        self.play(self.screen.shift, [-self.left_shift, 0, 0], self.morty.shift, [-self.left_shift, 0, 0], self.indicator.shift, [-self.left_shift, 0, 0], self.indicator.set_intensity, self.unit_indicator_intensity)
    ...
    def right_shift_screen_while_showing_light_indicator_and_distance_arrow(self):
        self.wait()
        self.play(ReplacementTransform(self.arrow, self.new_arrow), ApplyMethod(self.screen.shift, [self.distance_to_source, 0, 0]), ApplyMethod(self.indicator.shift, [self.left_shift, 0, 0]), ApplyMethod(self.indicator.set_intensity, self.indicator_intensity), ApplyMethod(self.morty.shift, [self.distance_to_source, 0, 0]))

    def left_shift_again(self):
        self.wait()
        self.play(ReplacementTransform(self.new_arrow, self.arrow), ApplyMethod(self.screen.shift, [-self.distance_to_source, 0, 0]), ApplyMethod(self.indicator.set_intensity, self.unit_indicator_intensity), ApplyMethod(self.morty.shift, [-self.distance_to_source, 0, 0]))
    ...
    def morph_into_3d(self):
        ...
        new_screen0 = Rectangle(height=self.screen_height, width=0.1, stroke_color=RED, fill_color=RED, fill_opacity=1)
        ...
        new_screen = Rectangle(height=self.screen_height, width=self.screen_height, stroke_color=RED, fill_color=RED, fill_opacity=1)
        ...
        self.add_foreground_mobject(self.light_source.shadow)
        self.play(ApplyMethod(self.camera.rotation_mobject.move_to, camera_target_point))
        self.remove(self.spotlight)
        self.play(Transform(new_screen0, new_screen))
        self.wait()
        self.unit_screen = new_screen0
    ...
    def prove_inverse_square_law(self):
        ...
        self.play(FadeIn(reading21), FadeIn(reading22), FadeIn(reading23), FadeIn(reading24))
        self.wait()
        self.play(FadeOut(reading21), FadeOut(reading22), FadeOut(reading23), FadeOut(reading24), FadeOut(midline1), FadeOut(midline2))
        ...
        self.play(ShowCreation(midline11), ShowCreation(midline12), ShowCreation(midline21), ShowCreation(midline22))
        self.play(FadeIn(reading31), FadeIn(reading32), FadeIn(reading33), FadeIn(reading34), FadeIn(reading35), FadeIn(reading36), FadeIn(reading37), FadeIn(reading38), FadeIn(reading39))
...
class BackToEulerSumScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        bubble = ThoughtBubble(direction=RIGHT, width=4, height=3, file_name='Bubbles_thought.svg')
        bubble.next_to(randy, LEFT + UP)
        bubble.set_fill(color=BLACK, opacity=1)
        self.play(randy.change, 'wave_2', ShowCreation(bubble))
        euler_sum = OldTex('1', '+', '{1\\over 4}', '+', '{1\\over 9}', '+', '{1\\over 16}', '+', '{1\\over 25}', '+', '\\cdots', ' ')
        ...
        indicator = LightIndicator(color=LIGHT_COLOR, radius=INDICATOR_RADIUS, opacity_for_unit_intensity=OPACITY_FOR_UNIT_INTENSITY, show_reading=False)
        ...
        for i in range(2, NUM_VISIBLE_CONES + 1):
            ...
            self.wait()
            self.play(Transform(bubble_indicator, bubble_indicator_target), collected_indicators.shift, left_shift)
            ...
            self.play(Transform(indicator, indicator_target), new_light.lighthouse.shift, v)
        ...
        for i in range(NUM_VISIBLE_CONES, NUM_CONES):
            ...
            self.play(SwitchOn(ls.ambient_light, run_time=FAST_SWITCH_ON_RUN_TIME))
        sum_indicator = LightIndicator(color=LIGHT_COLOR, radius=INDICATOR_RADIUS, opacity_for_unit_intensity=OPACITY_FOR_UNIT_INTENSITY, show_reading=False)
...
class TwoLightSourcesScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(ls1.lighthouse), FadeIn(ls2.lighthouse), SwitchOn(ls1.ambient_light), SwitchOn(ls2.ambient_light))
        ...
        self.play(SwitchOff(ls1.ambient_light), SwitchOff(ls2.ambient_light), UpdateLightIndicator(new_indicator, 0.0))
        updater = ContinualLightIndicatorUpdate(new_indicator)
        self.add(updater)
        self.play(SwitchOn(ls3.ambient_light), FadeIn(ls3.lighthouse))
        self.wait()
        ...
        buffer = 1.2
        theorem_box = Rectangle(width=buffer * theorem.get_width(), height=buffer * theorem.get_height())
        theorem.move_to(theorem_location)
        theorem_box.move_to(theorem_location)
        theorem_name.next_to(theorem_box, UP)
        self.play(Write(theorem))
        self.play(ShowCreation(theorem_box), Write(theorem_name))
...
class IPTScene1(PiCreatureScene):
    ...
    def construct(self):
        ...
        screen1 = Rectangle(width=screen_width_b, height=screen_thickness_b, stroke_width=0, fill_opacity=1.0)
        ...
        screen2 = Rectangle(width=screen_width_a, height=screen_thickness_a, stroke_width=0, fill_opacity=1.0)
        ...
        self.play(ls2.move_source_to, H, SwitchOff(ls1.ambient_light), Transform(screen2, screen2p))
        self.play(Transform(screen1, screen1pp), Transform(screen2, screen2pp))
...
class IPTScene2(Scene):
    ...
    def construct(self):
        ...
        buffer = 1.5
        box = Rectangle(width=formula.get_width() * buffer, height=formula.get_height() * buffer)
        box.move_to(formula)
...
class InscribedAngleScene(ThreeDScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(arc_left), Write(one_left), ShowCreation(arc_right), Write(one_right))
        self.play(SwitchOn(ls0.ambient_light), lake0.set_stroke, {'color': LAKE_STROKE_COLOR, 'width': LAKE_STROKE_WIDTH})
        self.play(FadeIn(indicator))
        self.play(indicator.set_intensity, 0.5)
        diameter = DoubleArrow(OBSERVER_POINT, ls0.get_source_point(), buff=0, color=WHITE)
        diameter_text = OldTex('d').scale(TEX_SCALE)
        diameter_text.next_to(diameter, RIGHT)
        self.play(ShowCreation(diameter), Write(diameter_text), FadeOut(self.ls0_dot))
        ...
        self.play(FadeOut(one_left), FadeOut(one_right), FadeOut(diameter_text), FadeOut(arc_left), FadeOut(arc_right))
        ...
        def split_light_source(i, step, show_steps=True, run_time=1):
            ...
            if show_animation:
                ...
                self.play(ApplyMethod(ls1.move_source_to, ls_new_loc1, run_time=run_time), ApplyMethod(ls2.move_source_to, ls_new_loc2, run_time=run_time))
            else:
        ...
        inner_angle_arc = Arc(angle=TAU / 4, start_angle=-TAU / 8, radius=arc_radius, stroke_color=ANGLE_COLOR1)
        ...
        outer_angle_arc = Arc(angle=TAU / 8, start_angle=-3 * TAU / 16, radius=arc_radius, stroke_color=ANGLE_COLOR2)
        ...
        self.wait()
        self.play(FadeOut(line1), FadeOut(line2), FadeOut(line3), FadeOut(line4), FadeOut(line5), FadeOut(line6), FadeOut(inner_angle_arc), FadeOut(outer_angle_arc), FadeOut(inner_label), FadeOut(outer_label))
        self.wait()
        inner_lines = VGroup()
        inner_arcs = VGroup()
        for i in range(-2, 2):
            theta = -TAU / 4 + (i + 0.5) * TAU / 4
            ls_point = inner_lake_center + inner_lake_radius * np.array([np.cos(theta), np.sin(theta), 0])
            line = Line(inner_lake_center, ls_point, color=WHITE)
            inner_lines.add(line)
            arc = Arc(angle=TAU / 4, start_angle=theta, radius=arc_radius, stroke_color=ANGLE_COLOR1)
        ...
        for i in range(-2, 2):
            ...
            theta = -TAU / 4 + (i + 0.5) * TAU / 8
            arc = Arc(angle=TAU / 8, start_angle=theta, radius=arc_radius, stroke_color=ANGLE_COLOR2)
        ...
        for i in range(-2, 2):
            theta = -TAU / 4 + (i + 0.5) * TAU / 8
            ls_point = outer_lake_center + outer_lake_radius * np.array([np.cos(theta), np.sin(theta), 0])
            line = Line(outer_lake_center, ls_point, color=WHITE)
        ...
        self.play(ShowCreation(last_arc), ShowCreation(last_arc2))
        self.wait()
        self.play(FadeOut(outer_lines2), FadeOut(outer_lines3), FadeOut(outer_arcs), FadeOut(outer_arcs3), FadeOut(last_arc), FadeOut(last_arc2))
        self.play(FadeOut(inner_ls), FadeOut(outer_ls))
        self.wait()
...
class PondScene(ThreeDScene):
    ...
    def construct(self):
        ...
        open_sea = Rectangle(width=200 * scale, height=100 * scale, stroke_width=LAKE_STROKE_WIDTH, stroke_color=LAKE_STROKE_COLOR, fill_color=LAKE_COLOR, fill_opacity=LAKE_OPACITY).flip().next_to(self.obs_dot.get_center(), UP, buff=0)
        self.revert_to_original_skipping_status()
        self.play(ReplacementTransform(pond_sources, nl_sources), ReplacementTransform(self.outer_lake, open_sea))
        ...
        origin_point = self.number_line.number_to_point(0)
        self.play(indicator.move_to, origin_point + scale * UP + 2 * UP, indicator_reading.move_to, origin_point + scale * UP + 2 * UP, FadeOut(open_sea), FadeOut(morty), FadeIn(self.number_line_labels), FadeIn(self.number_line.tick_marks))
...
class ThumbnailScene(Scene):
    ...
    def construct(self):
        ...
        lake_center = ORIGIN
        op_scale = 0.4
        lake = Circle(fill_color=LAKE_COLOR, fill_opacity=LAKE_OPACITY, radius=lake_radius, stroke_color=LAKE_STROKE_COLOR, stroke_width=LAKE_STROKE_WIDTH)
        lake.move_to(lake_center)
        for i in range(16):
            theta = -TAU / 4 + (i + 0.5) * TAU / 16
            pos = lake_center + lake_radius * np.array([np.cos(theta), np.sin(theta), 0])
            ls = LightSource(radius=15.0, num_levels=150, max_opacity_ambient=1.0, opacity_function=inverse_quadratic(1, op_scale, 1))
            ls.move_source_to(pos)
