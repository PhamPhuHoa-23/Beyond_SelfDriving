...
class GaussianDistributionWrapper(Line):
    ...
    def get_random_points(self, size=1):
        ...
        return np.array([np.array([np.random.normal(mu_coord, sigma_coord) for mu_coord, sigma_coord in zip(mu, sigma)]) for x in range(size)])

class ProbabalisticMobjectCloud(ContinualAnimation):
    CONFIG = {'fill_opacity': 0.25, 'n_copies': 100, 'gaussian_distribution_wrapper_config': {}, 'time_per_change': 1.0 / 60, 'start_up_time': 0}
    ...
    def __init__(self, prototype, **kwargs):
        ...
        self.gaussian_distribution_wrapper = GaussianDistributionWrapper(**self.gaussian_distribution_wrapper_config)
        self.time_since_last_change = np.inf
        group = VGroup(*[prototype.copy().set_fill(opacity=fill_opacity) for x in range(self.n_copies)])
...
class ProbabalisticVectorCloud(ProbabalisticMobjectCloud):
    CONFIG = {'color': RED, 'n_copies': 20, 'fill_opacity': 0.5, 'center_func': lambda: ORIGIN}

    def __init__(self, **kwargs):
        digest_config(self, kwargs)
        vector = Vector(RIGHT, color=self.color, max_tip_length_to_length_ratio=1)
...
class RadarDish(SVGMobject):
    CONFIG = {'file_name': 'radar_dish', 'fill_color': GREY_B, 'stroke_color': WHITE, 'stroke_width': 1, 'height': 1}

class Plane(SVGMobject):
    CONFIG = {'file_name': 'plane', 'color': GREY_B, 'height': 1}
...
class FalconHeavy(SVGMobject):
    CONFIG = {'file_name': 'falcon_heavy', 'color': WHITE, 'logo_color': BLUE_E, 'height': 1.5}
...
class RadarPulseSingleton(ContinualAnimation):
    CONFIG = {'speed': 3.0, 'direction': RIGHT, 'start_up_time': 0, 'fade_in_time': 0.5, 'color': WHITE, 'stroke_width': 3}
    ...
    def __init__(self, radar_dish, target, **kwargs):
        ...
        self.arc = Arc(start_angle=-30 * DEGREES, angle=60 * DEGREES)
...
class RadarPulse(ContinualAnimation):
    CONFIG = {'n_pulse_singletons': 8, 'frequency': 0.05, 'colors': [BLUE, YELLOW]}

    def __init__(self, *args, **kwargs):
        digest_config(self, kwargs)
        colors = color_gradient(self.colors, self.n_pulse_singletons)
        self.pulse_singletons = [RadarPulseSingleton(*args, color=color, **kwargs) for color in colors]
...
class MultipleFlashes(Succession):
    CONFIG = {'run_time_per_flash': 1.0, 'num_flashes': 3}

    def __init__(self, *args, **kwargs):
        digest_config(self, kwargs)
        kwargs['run_time'] = self.run_time_per_flash
        Succession.__init__(self, *[Flash(*args, **kwargs) for x in range(self.num_flashes)])

class TrafficLight(SVGMobject):
    CONFIG = {'file_name': 'traffic_light', 'height': 0.7, 'post_height': 2, 'post_width': 0.05}

    def __init__(self, **kwargs):
        SVGMobject.__init__(self, **kwargs)
        post = Rectangle(height=self.post_height, width=self.post_width, stroke_width=0, fill_color=WHITE, fill_opacity=1)
...
class MentionUncertaintyPrinciple(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        vector_cloud = ProbabalisticVectorCloud(gaussian_distribution_wrapper_config={'sigma_x': 0.2}, center_func=lambda: dot_cloud.gaussian_distribution_wrapper.get_parameters()[0])
        for cloud in (dot_cloud, vector_cloud):
            cloud.gaussian_distribution_wrapper.next_to(title, DOWN, 2 * LARGE_BUFF)
        ...
        dot_brace_anim = get_brace_text_group_update(dot_cloud.gaussian_distribution_wrapper, DOWN, 'position', dot_cloud.color)
        vector_brace_anim = get_brace_text_group_update(vector_cloud.gaussian_distribution_wrapper, UP, 'momentum', vector_cloud.color)
        self.add(title)
        self.add(dot_cloud)
        self.play(Write(title), self.teacher.change, 'raise_right_hand', self.change_students(*['pondering'] * 3))
        self.play(Write(dot_brace_anim.mobject, run_time=1))
        self.add(dot_brace_anim)
        self.wait()
        self.play(dot_cloud.gaussian_distribution_wrapper.change_parameters, {'sigma': 0.1 * RIGHT}, run_time=2)
        self.wait()
        self.add(vector_cloud)
        self.play(FadeIn(vector_brace_anim.mobject))
        self.add(vector_brace_anim)
        self.play(vector_cloud.gaussian_distribution_wrapper.change_parameters, {'sigma': RIGHT}, self.change_students(*3 * ['confused']), run_time=3)
        for x in range(2):
            self.play(dot_cloud.gaussian_distribution_wrapper.change_parameters, {'sigma': 2 * RIGHT}, vector_cloud.gaussian_distribution_wrapper.change_parameters, {'sigma': 0.1 * RIGHT}, run_time=3)
            self.play_student_changes('thinking', 'erm', 'sassy')
            self.play(dot_cloud.gaussian_distribution_wrapper.change_parameters, {'sigma': 0.1 * RIGHT}, vector_cloud.gaussian_distribution_wrapper.change_parameters, {'sigma': 1 * RIGHT}, run_time=3)
            self.wait()

class FourierTradeoff(Scene):
    CONFIG = {'show_text': True, 'complex_to_real_func': lambda z: z.real, 'widths': [6, 0.02, 1]}

    def construct(self):
        time_mean = 4
        time_axes = Axes(x_min=0, x_max=2 * time_mean, x_axis_config={'unit_size': 1.5}, y_min=-2, y_max=2, y_axis_config={'unit_size': 0.5})
        time_label = OldTexText('Time')
        time_label.scale(1.5)
        time_label.next_to(time_axes.x_axis.get_right(), UP + LEFT, buff=MED_SMALL_BUFF)
        time_axes.add(time_label)
        time_axes.center().to_edge(UP)
        time_axes.x_axis.add_numbers(*list(range(1, 2 * time_mean)))
        frequency_axes = Axes(x_min=0, x_max=8, x_axis_config={'unit_size': 1.5}, y_min=-0.025, y_max=0.075, y_axis_config={'unit_size': 30, 'tick_frequency': 0.025}, color=TEAL)
        frequency_label = OldTexText('Frequency')
        frequency_label.scale(1.5)
        frequency_label.next_to(frequency_axes.x_axis.get_right(), UP + LEFT, buff=MED_SMALL_BUFF)
        ...
        def get_wave_packet_function():
            factor = 1.0 / get_width()
            return lambda t: factor ** 0.25 * np.cos(4 * TAU * t) * np.exp(-factor * (t - time_mean) ** 2)

        def get_wave_packet():
            graph = time_axes.get_graph(get_wave_packet_function(), num_graph_points=200)
            graph.set_color(YELLOW)
            return graph
        time_radius = 10

        def get_wave_packet_fourier_transform():
            return get_fourier_graph(frequency_axes, get_wave_packet_function(), t_min=time_mean - time_radius, t_max=time_mean + time_radius, n_samples=2 * time_radius * 17, complex_to_real_func=self.complex_to_real_func, color=FREQUENCY_COLOR)
        wave_packet = get_wave_packet()
        wave_packet_update = UpdateFromFunc(wave_packet, lambda g: Transform(g, get_wave_packet()).update(1))
        fourier_graph = get_wave_packet_fourier_transform()
        fourier_graph_update = UpdateFromFunc(fourier_graph, lambda g: Transform(g, get_wave_packet_fourier_transform()).update(1))
        arrow = Arrow(wave_packet, frequency_axes.coords_to_point(4, frequency_axes.y_max / 2), color=FREQUENCY_COLOR)
        ...
        for width in self.widths:
            self.play(width_tracker.set_value, width, wave_packet_update, fourier_graph_update, run_time=3)
...
class ShowPlan(PiCreatureScene):
    ...
    def get_words(self):
        trips = [('sound waves', '(time vs. frequency)', YELLOW), ('Doppler radar', '(distance vs. velocity)', GREEN), ('quantum particles', '(position vs. momentum)', BLUE)]
        words = VGroup()
    ...
    def play_sound_anims(self, word):
        ...
        wave = FunctionGraph(lambda x: 0.3 * np.sin(15 * x) * np.sin(0.5 * x), x_min=0, x_max=30, step_size=0.001)
        ...
        self.play(Animation(word), wave_fader, morty.change, 'raise_right_hand', word)
        self.wait(2)
        wave_fader.rate_func = lambda a: 1 - smooth(a)
        self.add_foreground_mobjects(checkmark)
        self.play(Write(checkmark), morty.change, 'happy', wave_fader)
    ...
    def play_doppler_anims(self, word):
        ...
        checkmark = self.get_checkmark(word)
        self.add(target)
        self.play(Write(word), DrawBorderThenFill(radar_dish), UpdateFromAlphaFunc(target, lambda m, a: m.set_fill(opacity=a)), morty.change, 'pondering', run_time=1)
        self.add(pulse)
        count = it.count()
        while not pulse.is_finished() and next(count) < 15:
            self.play(morty.look_at, pulse.mobject, run_time=0.5)
        self.play(Write(checkmark), UpdateFromAlphaFunc(target, lambda m, a: m.set_fill(opacity=1 - a)), FadeOut(radar_dish), morty.change, 'happy')
        self.wait()
    ...
    def play_quantum_anims(self, word):
        ...
        self.add(dot_cloud)
        self.play(Write(word), FadeIn(dot_cloud.mobject), morty.change, 'confused')
...
class TwoCarsAtRedLight(Scene):
    ...
    def pull_up_behind(self):
        ...
        red_light = Spotlight(color=RED, source_point=source_point, radius=0.5, screen=screen, num_levels=20, opacity_function=lambda r: 1 / (10 * r ** 2 + 1))
        red_light.fade(0.5)
        ...
        self.add(cars)
        self.play(SwitchOn(red_light, rate_func=squish_rate_func(smooth, 0, 0.3)), Animation(traffic_light), self.get_flashes(car2, num_flashes=3), MoveCar(car1, car1.start_point, run_time=3, rate_func=rush_from))

    def flash_in_sync_short_time(self):
        car1, car2 = cars = self.cars
        axes = Axes(x_min=0, x_max=5, y_min=0, y_max=2, y_axis_config={'tick_frequency': 0.5})
        ...
        graph = axes.get_graph(self.get_multispike_function(list(range(1, 4))), x_min=0.8, x_max=3.8)
        graph.set_color(YELLOW)
        brace = Brace(Line(axes.input_to_graph_point(1, graph), axes.input_to_graph_point(3, graph)), UP)
        ...
        self.play(self.get_flashes(car1, num_flashes=2), self.get_flashes(car2, num_flashes=2), LaggedStartMap(FadeIn, VGroup(axes, time_label, y_title)))
        self.play(self.get_flashes(car1, num_flashes=3), self.get_flashes(car2, num_flashes=3), ShowCreation(graph, rate_func=linear, run_time=3))
        self.play(self.get_flashes(car1, num_flashes=10), self.get_flashes(car2, num_flashes=10, run_time_per_flash=0.98), GrowFromCenter(brace), Write(text))
        self.time_axes = axes
    ...
    def show_low_confidence(self):
        ...
        frequency_axes = Axes(x_min=0, x_max=3, y_min=0, y_max=1.5, y_axis_config={'tick_frequency': 0.5})
        ...
        frequency_axes.add(frequency_label, VectorizedPoint(frequency_axes.y_axis.get_top()))
        frequency_axes.x_axis.add_numbers(1, 2)
        frequency_graph = frequency_axes.get_graph(lambda x: np.exp(-4 * (x - 1) ** 2), x_min=0, x_max=2)
        ...
        self.play(ReplacementTransform(self.time_axes.copy(), frequency_axes), ReplacementTransform(self.time_graph.copy(), frequency_graph))
    ...
    def flash_in_sync_long_time(self):
        ...
        n_spikes = 12
        new_time_graph = time_axes.get_graph(self.get_multispike_function(list(range(1, n_spikes + 1))), x_min=0.8, x_max=n_spikes + 0.8)
        new_time_graph.match_color(time_graph)
        new_frequency_graph = frequency_axes.get_graph(lambda x: np.exp(-500 * (x - 1) ** 2), x_min=0, x_max=2, num_anchors=500)
        ...
        self.play(LaggedStartMap(FadeOut, VGroup(self.time_graph_label, self.frequency_graph_label, self.time_graph)))
        self.play(ApplyMethod(self.time_axes.x_axis.stretch, 2.5, 0, {'about_edge': LEFT}, run_time=4, rate_func=squish_rate_func(smooth, 0.3, 0.6)), UpdateFromFunc(self.time_axes.x_axis.tip, lambda m: m.move_to(self.time_axes.x_axis.get_right(), LEFT)), ShowCreation(new_time_graph, run_time=n_spikes, rate_func=linear), ApplyMethod(frequency_graph.stretch, 0.1, 0, run_time=n_spikes), UpdateFromFunc(frequency_graph, pin_freq_graph_end_points), *[self.get_flashes(car, num_flashes=n_spikes) for car in self.cars])
    ...
    def show_high_confidence(self):
        ...
        self.play(FadeIn(label), GrowArrow(arrow), *list(map(self.get_flashes, self.cars)))
        self.play(GrowFromCenter(brace), Write(text, run_time=1), *list(map(self.get_flashes, self.cars)))
        self.play(*[self.get_flashes(car, num_flashes=10) for car in self.cars])

    def get_flashes(self, car, colors=[YELLOW, RED], num_flashes=1, **kwargs):
        return AnimationGroup(*[MultipleFlashes(light, color, num_flashes=num_flashes, **kwargs) for light, color in zip(car.get_lights(), colors)])
...
class VariousMusicalNotes(Scene):
    ...
    def construct(self):
        ...
        def get_graph():
            ...
            return FunctionGraph(lambda x: np.exp(-a * x ** 2) * np.sin(freq * x) - 0.5, step_size=0.001)
        graph = get_graph()
        ...
        def change_width_anim(width, **kwargs):
            a = 2.0 / width ** 2
            return AnimationGroup(ApplyMethod(graph_width_tracker.set_value, a), graph_update_anim, **kwargs)
        change_width_anim(FRAME_X_RADIUS).update(1)
        graph_update_anim.update(0)
        phrases = [OldTexText(*words.split(' ')) for words in ['Very clear frequency', 'Less clear frequency', 'Extremely unclear frequency']]
        ...
        for width, phrase in zip(widths, phrases):
            ...
            if width is widths[0]:
                ...
                self.play(change_width_anim(width), ReplacementTransform(VGroup(last_phrase, last_brace), VGroup(phrase, brace), rate_func=squish_rate_func(smooth, 0.5, 1)), run_time=2)
            self.wait()
            last_phrase = phrase
            last_brace = brace
        short_signal_words = OldTexText('Short', 'signal', 'correlates', 'with', 'wide range', 'of frequencies')
        long_signal_words = OldTexText('Only', 'wide', 'signals', 'correlate', 'with a', 'short range', 'of frequencies')
        phrases = VGroup(short_signal_words, long_signal_words)
        for phrase in phrases:
            phrase.scale(0.8)
            phrase.set_color_by_tex_to_color_map({'short': RED, 'long': GREEN, 'wide': GREEN}, case_sensitive=False)
        phrases.arrange(DOWN)
        phrases.to_edge(UP)
        long_graph = FunctionGraph(lambda x: 0.5 * np.sin(freq * x), x_min=-FRAME_WIDTH, x_max=FRAME_WIDTH, n_components=0.001)
        long_graph.set_color(BLUE)
        long_graph.next_to(graph, UP, MED_LARGE_BUFF)
        self.play(ShowCreation(long_graph), *list(map(FadeOut, [last_brace, last_phrase])))
        self.play(Write(short_signal_words), change_width_anim(widths[2]))
        self.play(long_graph.stretch, 0.35, 0, long_graph.set_color, GREEN, run_time=5, rate_func=wiggle)
        self.wait()
        self.play(Write(long_signal_words), change_width_anim(widths[0]))
        self.play(long_graph.stretch, 0.95, 0, long_graph.set_color, average_color(GREEN, BLUE), run_time=4, rate_func=wiggle)
        self.wait()

class CrossOutDefinitenessAndCertainty(TeacherStudentsScene):

    def construct(self):
        words = VGroup(OldTexText('Definiteness'), OldTexText('Certainty'))
        words.arrange(DOWN)
        ...
        self.play(self.change_students(*3 * ['erm']), ShowCreation(crosses[1]))
        self.wait(2)
...
class FourierRecapScene(DrawFrequencyPlot):
    CONFIG = {'frequency_axes_config': {'x_max': 10.0, 'x_axis_config': {'unit_size': 0.7, 'numbers_to_show': list(range(1, 10, 1))}}, 'initial_winding_frequency': 0.1}
    ...
    def preview_fourier_plot(self):
        time_graph = self.graph = self.get_time_graph(width=2, num_graph_points=200)
        ...
        self.play(ShowCreation(time_graph, run_time=2), Write(signal_label))
        self.wait()
        self.play(LaggedStartMap(FadeIn, self.frequency_axes), ReplacementTransform(time_graph.copy(), fourier_graph, run_time=2), ReplacementTransform(signal_label.copy(), fourier_label, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)))
        self.wait()
        self.play(LaggedStartMap(Indicate, self.frequency_axes.x_axis.numbers, run_time=4, rate_func=wiggle))
        self.wait()
        self.play(*list(map(FadeOut, [self.frequency_axes, fourier_graph, signal_label, fourier_label])))
    ...
    def wrap_signal_around_circle(self):
        ...
        self.play(Write(circle_plane, run_time=1), ReplacementTransform(time_graph.copy(), pol_graph, path_arc=-TAU / 4, run_time_per_flash=2, run_time=2), FadeIn(winding_freq_label))
        freq = 0.3
    ...
    def match_winding_to_beat_frequency(self):
        ...
        self.play(*self.get_vector_animations(self.time_graph, draw_polarized_graph=False), run_time=10)
        self.wait()
    ...
    def follow_center_of_mass(self):
        ...
        com_dot.fade(1)
        self.play(com_dot.restore, GrowArrow(com_arrow, rate_func=squish_rate_func(smooth, 0.2, 1)), Write(com_words))
        self.wait()
        squished_func = squish_rate_func(smooth, 0, 0.2)
        self.change_frequency(4, added_anims=[FadeOut(com_arrow, rate_func=squished_func), FadeOut(com_words, rate_func=squished_func)], run_time=5)

    def draw_fourier_plot(self):
        frequency_axes = self.frequency_axes
        fourier_label = self.fourier_label
        self.change_frequency(0, run_time=2)
        self.play(FadeIn(frequency_axes), FadeIn(fourier_label))
    ...
    def set_color_spike(self):
        spike_point = self.frequency_axes.input_to_graph_point(5, self.fourier_graph)
    ...
    def get_time_graph(self, frequency=5, width=2, **kwargs):
        graph = self.time_axes.get_graph(lambda x: 1 + 0.9 * np.cos(TAU * frequency * x), x_min=0, x_max=width, **kwargs)
        graph.set_color(YELLOW)
        return graph
...
class CenterOfMassDescription(FourierRecapScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(MoveToTarget(circle_plane), com_dot.move_to, circle_plane.target.coords_to_point(*start_coords))
        self.wait()
        alt_com_dot = com_dot.copy().move_to(circle_plane.coords_to_point(*alt_coords))
        ...
        self.play(GrowFromCenter(line), GrowFromCenter(brace), FadeIn(words))
        self.wait()
        self.play(Transform(com_dot.length_label_group, alt_com_dot.length_label_group), Transform(com_dot, alt_com_dot), rate_func=there_and_back, run_time=4)
        line = com_dot.length_label_group[0]
        com_dot.length_label_group.remove(line)
        angle = line.get_angle()
        arc, alt_arc = [Arc(start_angle=0, angle=factor * angle, radius=0.5) for factor in (1, 2)]
        theta = OldTex('\\theta')
        theta.shift(1.5 * arc.point_from_proportion(0.5))
        self.play(FadeOut(com_dot.length_label_group), Animation(line), ShowCreation(arc), Write(theta))
        self.play(Rotate(VGroup(line, com_dot), angle, about_point=ORIGIN), Transform(arc, alt_arc), theta.move_to, 1.5 * alt_arc.point_from_proportion(0.5), rate_func=there_and_back, run_time=4)
        self.wait()

class AskAboutLongVsShort(TeacherStudentsScene):

    def construct(self):
        self.student_says('What happens if we \\\\ change the length of \\\\ the signal?', index=2)
        self.play(self.teacher.change, 'happy', self.change_students('pondering', 'confused', 'raise_right_hand'))
        self.wait(5)

class LongAndShortSignalsInWindingMachine(FourierRecapScene):
    CONFIG = {'num_fourier_graph_points': 1000}
    ...
    def extend_for_long_time(self):
        ...
        long_time_graph = self.get_time_graph(width=10, num_graph_points=500)
        long_time_graph.set_stroke(width=2)
        new_freq = 5.1
        long_pol_graph = self.get_polarized_mobject(long_time_graph, freq=new_freq)
        fourier_graph = self.fourier_graph
        self.change_frequency(new_freq)
        self.play(FadeOut(self.graph), FadeOut(self.graph.polarized_mobject), FadeOut(fourier_graph))
        self.play(ShowCreation(long_time_graph, rate_func=linear), ShowCreation(long_pol_graph, rate_func=linear), run_time=5)
        self.wait()
        self.time_graph = self.graph = long_time_graph

    def note_sharp_fourier_peak(self):
        fourier_graph = self.get_fourier_transform_graph(self.time_graph, num_graph_points=self.num_fourier_graph_points)
        self.fourier_graph = fourier_graph
    ...
    def very_short_signal(self):
        ...
        new_freq = 5.1
        short_pol_graph = self.get_polarized_mobject(short_time_graph, freq=new_freq)
        self.play(FadeOut(fourier_graph), FadeOut(time_graph), FadeOut(time_graph.polarized_mobject))
        self.play(ShowCreation(short_time_graph), ShowCreation(short_time_graph.polarized_mobject))
        self.graph = self.time_graph = short_time_graph
        self.change_frequency(6.66, run_time=5)

    def note_wide_fourier_peak(self):
        fourier_graph = self.get_fourier_transform_graph(self.graph, num_graph_points=self.num_fourier_graph_points)
        self.fourier_graph = fourier_graph
    ...
    def note_fourier_peak(self, fourier_graph, freq1, freq2):
        ...
        self.change_frequency(freq1, added_anims=[MaintainPositionRelativeTo(arrow, dot), UpdateFromAlphaFunc(arrow, lambda m, a: m.set_fill(opacity=a))], run_time=3)
        self.wait()
        self.change_frequency(freq2, added_anims=[MaintainPositionRelativeTo(arrow, dot)], run_time=3)
        self.wait()
...
class FocusRectangleInsert(FourierRecapScene):
    ...
    def construct(self):
        ...
        self.add(rect)
        self.wait()
        self.play(rect.stretch_to_fit_width, self.target_width, rect.stretch_to_fit_height, 1.5, rect.move_to, point, run_time=2)
        self.wait(3)

class BroadPeakFocusRectangleInsert(FocusRectangleInsert):
    CONFIG = {'target_width': 1.5}

class CleanerFourierTradeoff(FourierTradeoff):
    CONFIG = {'show_text': False, 'complex_to_real_func': lambda z: z.real, 'widths': [0.02, 6, 1]}
...
class MentionDopplerRadar(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        look_at_anims = [Mobject.add_updater(pi, lambda pi: pi.look_at(pulse.mobject)) for pi in self.get_pi_creatures()]
        ...
        self.wait(2)
        self.play(self.teacher.change, 'happy', self.change_students(*['thinking'] * 3))
        self.wait()
        dish.set_stroke(width=0)
        self.play(UpdateFromAlphaFunc(VGroup(plane, dish), lambda m, a: m.set_fill(opacity=1 - a)))

class IntroduceDopplerRadar(Scene):
    CONFIG = {'frequency_spread_factor': 100}
    ...
    def measure_distance_with_time(self):
        ...
        pulse_graph, echo_graph, sum_graph = self.get_pulse_and_echo_graphs(self.get_single_pulse_graph, (1,), (1 + time_diff,))
        ...
        for graph, word in zip([pulse_graph, echo_graph], words):
            ...
            graph.label = label
        double_arrow = DoubleArrow(pulse_graph.peak_point, echo_graph.peak_point, color=WHITE)
        ...
        pulse = RadarPulseSingleton(dish, randy, speed=0.97 * speed)
        graph_draw = turn_animation_into_updater(ShowCreation(sum_graph, rate_func=linear, run_time=0.97 * axes.x_max))
        randy_look_at = Mobject.add_updater(randy, lambda pi: pi.look_at(pulse.mobject))
        axes_anim = ContinualAnimation(axes)
        self.add(randy_look_at, axes_anim, graph_draw)
        self.wait(0.5)
        self.add(pulse)
        self.play(Write(pulse_graph.label), GrowArrow(pulse_graph.arrow), run_time=1)
        self.play(randy.change, 'pondering')
        self.wait(time_diff - 2)
        self.play(Write(echo_graph.label), GrowArrow(echo_graph.arrow), run_time=1)
        self.wait()
        self.play(GrowFromCenter(double_arrow), FadeIn(distance_text))
        self.wait()
        self.remove(graph_draw, pulse, randy_look_at, axes_anim)
        self.add(axes)
        self.play(LaggedStartMap(FadeOut, VGroup(sum_graph, randy, pulse_graph.arrow, pulse_graph.label, echo_graph.arrow, echo_graph.label, double_arrow, distance_text)))
    ...
    def show_frequency_shift(self):
        ...
        plane.to_edge(RIGHT)
        time_diff = 6
        pulse_graph, echo_graph, sum_graph = graphs = self.get_pulse_and_echo_graphs(self.get_frequency_pulse_graph, (1, 25), (1 + time_diff, 50))
        for graph in graphs:
        ...
        graph_draw = turn_animation_into_updater(ShowCreation(sum_graph, run_time=8, rate_func=linear))
        pulse = RadarPulse(dish, plane, n_pulse_singletons=12)
        always_shift(plane, LEFT, 1.5)
        self.add(graph_draw, pulse, plane)
        self.play(UpdateFromAlphaFunc(plane, lambda m, a: m.set_fill(opacity=a)))
        self.play(GrowFromCenter(pulse_brace), FadeIn(pulse_text))
        self.wait(3)
        self.play(GrowFromCenter(echo_brace), GrowFromCenter(echo_text))
        self.play(UpdateFromAlphaFunc(plane, lambda m, a: m.set_fill(opacity=1 - a)))
        ...
        self.time_graph_group = VGroup(axes, pulse_brace, pulse_text, echo_brace, echo_text, echo_subtext, pulse_graph, echo_graph, sum_graph)
    ...
    def show_frequency_shift_in_fourier(self):
        ...
        f_max = 0.02
        frequency_axes = Axes(x_min=0, x_max=20, x_axis_config={'unit_size': 0.5}, y_min=-f_max, y_max=f_max, y_axis_config={'unit_size': 50, 'tick_frequency': 0.01})
        ...
        for graph in (pulse_graph, echo_graph, sum_graph):
            graph.fourier_transform = get_fourier_graph(frequency_axes, graph.underlying_function, frequency_axes.x_min, 25, complex_to_real_func=abs)
        original_fourier_brace = Brace(Line(frequency_axes.coords_to_point(7, 0.9 * f_max), frequency_axes.coords_to_point(9, 0.9 * f_max)), UP).set_color(BLUE)
        echo_fourier_brace = Brace(Line(frequency_axes.coords_to_point(14, 0.4 * f_max), frequency_axes.coords_to_point(18, 0.4 * f_max)), UP).set_color(YELLOW)
        ...
        v_line = DashedLine(frequency_axes.coords_to_point(8, 0), frequency_axes.coords_to_point(8, 1.2 * f_max), color=YELLOW, dash_length=0.025)
        v_line_pair = VGroup(*[v_line.copy().shift(u * 0.6 * RIGHT) for u in (-1, 1)])
        v_line = VGroup(v_line)
        double_arrow = DoubleArrow(frequency_axes.coords_to_point(8, 0.007), frequency_axes.coords_to_point(16, 0.007), buff=0, color=WHITE)
        self.play(self.time_graph_group.to_edge, UP, ApplyMethod(self.dish.shift, 2 * UP, remover=True), FadeIn(frequency_axes))
        self.wait()
        self.play(FadeOut(sum_graph), FadeOut(echo_label), pulse_graph.set_stroke, {'width': 3})
        self.play(ReplacementTransform(pulse_label[0].copy(), original_fourier_brace), ShowCreation(pulse_graph.fourier_transform))
        ...
        self.wait()
        self.play(FadeOut(pulse_graph), FadeIn(sum_graph), ReplacementTransform(pulse_graph.fourier_transform, sum_graph.fourier_transform))
        self.play(FadeIn(echo_label))
        self.play(ReplacementTransform(echo_label[0].copy(), echo_fourier_brace))
        self.wait(2)
    ...
    def get_frequency_pulse_graph(self, x, freq=50, **kwargs):
        return self.get_graph(self.get_frequency_pulse_function(x, freq), num_graph_points=700, **kwargs)

    def get_frequency_pulse_function(self, x, freq):
        factor = self.frequency_spread_factor
        return lambda t: op.mul(2 * np.cos(2 * freq * (t - x)), min(np.exp(-(freq ** 2 / factor) * (t - x) ** 2), 0.5))
    ...
    def get_pulse_and_echo_graphs(self, func, args1, args2):
        ...
        sum_graph = self.axes.get_graph(lambda x: sum([pulse_graph.underlying_function(x), echo_graph.underlying_function(x)]), num_graph_points=echo_graph.get_num_curves(), color=WHITE)
...
class DopplerFormulaInsert(Scene):
    ...
    def construct(self):
        formula = OldTex('f_{\\text{echo}', '=', '\\left(1 + \\frac{v}{c}\\right)', 'f_{\\text{pulse}}')
        ...
        self.add(randy)
        self.play(LaggedStartMap(FadeIn, formula), randy.change, 'pondering', randy.get_bottom())
...
class MentionPRFNuance(TeacherStudentsScene):
    ...
    def construct(self):
        title = OldTexText('Speed of light', '$\\gg$', 'Speed of a plane')
        title.to_edge(UP)
        self.add(title)
        axes = self.axes = Axes(x_min=0, x_max=10, y_min=0, y_max=2)
        ...
        self.play(Animation(pulse_graph), self.teacher.change, 'raise_right_hand', run_time=1)
        self.play(ShowCreation(shift_graph), FadeIn(brace), Write(displayed_doppler_shift, run_time=1), self.change_students(*3 * ['sassy']))
        self.play(UpdateFromAlphaFunc(shift_graph, lambda g, a: Transform(g, self.get_spike_graph(interpolate(shift_x, pulse_x + 0.01, a)).match_style(shift_graph)).update(1)), UpdateFromFunc(brace, lambda b: b.match_width(peak_points, stretch=True).next_to(peak_points, UP, SMALL_BUFF)), Transform(displayed_doppler_shift, actual_doppler_shift, rate_func=squish_rate_func(smooth, 0.3, 0.6)), run_time=3)
        self.wait(2)
        everything = VGroup(title, axes, pulse_graph, shift_graph, brace, displayed_doppler_shift)
        rect = SurroundingRectangle(everything, color=WHITE)
        everything.add(rect)
        self.teacher_says("I'll ignore certain \\\\ nuances for now.", target_mode='shruggie', added_anims=[everything.scale, 0.4, everything.to_corner, UP + LEFT, UpdateFromAlphaFunc(rect, lambda m, a: m.set_stroke(width=2 * a))])
        self.play_student_changes(*3 * ['hesitant'])
        self.wait(2)

    def get_spike_graph(self, x, color=RED, **kwargs):
        graph = self.axes.get_graph(lambda t: np.exp(-10 * (t - x) ** 2) * np.cos(10 * (t - x)), color=color, **kwargs)
...
class TimeAndFrequencyGivePositionAndVelocity(IntroduceDopplerRadar):
    ...
    def construct(self):
        x = 7
        freq = 25
        axes = self.axes = Axes(x_min=0, x_max=10, y_min=-2, y_max=2)
        axes.center()
        ...
        arrow = Arrow(axes.coords_to_point(0, -1.5), axes.coords_to_point(x, -1.5), color=WHITE, buff=SMALL_BUFF)
        time = OldTexText('Time')
        time.next_to(arrow, DOWN, SMALL_BUFF)
        delta_x = 0.7
        brace = Brace(Line(axes.coords_to_point(x - delta_x, 1), axes.coords_to_point(x + delta_x, 1)), UP)
        ...
        self.play(GrowFromCenter(brace), LaggedStartMap(FadeIn, frequency, run_time=1))
        self.wait()
        self.play(GrowFromPoint(time_updown_arrow, time_updown_arrow.get_top()), ReplacementTransform(time.copy().fade(1), distance))
        self.play(GrowFromPoint(freq_updown_arrow, freq_updown_arrow.get_top()), ReplacementTransform(frequency.copy().fade(1), velocity))
        self.wait()
...
class RadarOperatorUncertainty(Scene):
    ...
    def construct(self):
        ...
        bubble = randy.get_bubble(width=7, height=4)
        echo_object = Square()
        ...
        plane_cloud = ProbabalisticMobjectCloud(plane, fill_opacity=0.3, n_copies=10)
        ...
        for i in range(3):
            for plane_factor, vector_factor, freq in ((0.05, 10, 0.01), (20, 0.1, 0.1)):
                ...
                self.play(randy.change, 'pondering', plane, plane_gdw.scale, plane_factor, vector_gdw.scale, vector_factor)
                self.wait(2)

class AmbiguityInLongEchos(IntroduceDopplerRadar, PiCreatureScene):
    CONFIG = {'object_x_coords': [7, 4, 6, 9, 8], 'frequency_spread_factor': 200, 'n_pulse_singletons': 16, 'pulse_frequency': 0.025}
    ...
    def setup_axes(self):
        axes = self.axes = Axes(x_min=0, x_max=10, y_min=-1.5, y_max=1.5)
        time_label = OldTexText('Time')
        time_label.next_to(axes.x_axis.get_right(), UP)
        axes.add(time_label)
        axes.center()
    ...
    def setup_objects(self):
        objects = self.objects = VGroup(Plane().flip(), SVGMobject(file_name='blimp', color=BLUE_C, height=0.5), SVGMobject(file_name='biplane', color=RED_D, height=0.5), SVGMobject(file_name='helicopter', color=GREY_B, height=0.5).rotate(-TAU / 24), FalconHeavy())
        ...
        for x, y, obj in zip(self.object_x_coords, y_shifts, objects):
            ...
            obj.shift(y * UP)
        self.object_velocities = [0.7 * LEFT, 0.1 * RIGHT, 0.4 * LEFT, 0.4 * RIGHT, 0.5 * UP]
    ...
    def send_long_pulse_single_echo(self):
        ...
        brace = Brace(Line(self.axes.coords_to_point(x - 1, 1), self.axes.coords_to_point(x + 1, 1)), UP)
        words = brace.get_text('Spread over time')
        self.add(pulse)
        self.wait()
        squished_rate_func = squish_rate_func(smooth, 0.6, 0.9)
        self.play(ShowCreation(pulse_graph, rate_func=linear), GrowFromCenter(brace, rate_func=squished_rate_func), Write(words, rate_func=squished_rate_func), run_time=3)
        self.remove(pulse)
        self.play(FadeIn(randy))
        self.play(PiCreatureBubbleIntroduction(randy, 'Who cares?', bubble_type=ThoughtBubble, bubble_config={'direction': LEFT, 'width': 2, 'height': 1.5}, target_mode='maybe', look_at=brace))
        self.play(Blink(randy))
    ...
    def introduce_multiple_objects(self):
        ...
        sum_graph = self.axes.get_graph(lambda t: sum([graph.underlying_function(t) for graph in graphs]), num_graph_points=1000)
        noise_function = lambda t: np.sum([0.5 * np.sin(f * t) / f for f in (2, 3, 5, 7, 11, 13)])
        noisy_graph = self.axes.get_graph(lambda t: sum_graph.underlying_function(t) * (1 + noise_function(t)), num_graph_points=1000)
        for graph in (sum_graph, noisy_graph):
            graph.background_image_file = 'blue_yellow_gradient'
        pulses = self.get_pulses()
        self.play(LaggedStartMap(GrowFromCenter, objects[1:]), FadeOut(curr_graph), randy.change, 'pondering')
        self.add(*pulses)
        self.wait(0.5)
        self.play(ShowCreation(sum_graph, rate_func=linear, run_time=3.5), randy.change, 'confused')
        self.remove(*pulses)
        self.play(randy.change, 'pondering')
        self.play(Transform(sum_graph, noisy_graph, rate_func=lambda t: wiggle(t, 4), run_time=3))
        self.wait(2)
    ...
    def use_short_pulse(self):
        ...
        self.add(*pulses)
        self.wait(0.5)
        self.play(ShowCreation(sum_graph, rate_func=linear, run_time=3.5), randy.change, 'happy')
        self.wait()
        self.curr_graph = sum_graph
    ...
    def fourier_transform_of_one_pulse(self):
        frequency_axes = Axes(x_min=0, x_max=20, x_axis_config={'unit_size': 0.5, 'tick_frequency': 2}, y_min=-0.01, y_max=0.01, y_axis_config={'unit_size': 110, 'tick_frequency': 0.006})
        frequency_label = OldTexText('Frequency')
        frequency_label.next_to(frequency_axes.x_axis.get_right(), UP)
        frequency_axes.add(frequency_label)
        first_echo_graph = self.first_echo_graph
        self.play(ApplyMethod(VGroup(self.axes, first_echo_graph).to_edge, UP, {'buff': SMALL_BUFF}, rate_func=squish_rate_func(smooth, 0.5, 1)), LaggedStartMap(FadeOut, self.objects), LaggedStartMap(FadeOut, VGroup(self.curr_graph, self.dish, self.pi_creature)), run_time=2)
        frequency_axes.next_to(self.axes, DOWN, LARGE_BUFF, LEFT)
        fourier_graph = get_fourier_graph(frequency_axes, first_echo_graph.underlying_function, t_min=0, t_max=25, complex_to_real_func=np.abs)
        ...
        f = 8
        v_line = DashedLine(frequency_axes.coords_to_point(f, 0), frequency_axes.coords_to_point(f, frequency_axes.y_max))
        v_lines = VGroup(v_line.copy().shift(2 * LEFT), v_line.copy().shift(2 * RIGHT))
        ...
        self.play(ApplyFunction(lambda m: m.move_to(fourier_graph.saved_state).shift(-h_vect).fade(1), first_echo_graph.copy(), remover=True), fourier_graph.restore)
        self.wait()
        self.play(ShowCreation(v_line))
    ...
    def show_echos_of_moving_objects(self):
        ...
        movements = self.object_movements = [always_shift(obj, direction=v / get_norm(v), rate=get_norm(v)) for v, obj in zip(object_velocities, objects)]
        pulses = self.get_pulses()
        continual_anims = pulses + movements
        self.play(FadeOut(self.axes), FadeOut(self.first_echo_graph), LaggedStartMap(FadeIn, objects), FadeIn(self.dish))
        self.add(*continual_anims)
        self.wait(4)
        self.play(*[UpdateFromAlphaFunc(obj, lambda m, a: m.set_fill(opacity=1 - a)) for obj in objects])
    ...
    def overlapping_frequenies_of_various_objects(self):
        ...
        shifted_graphs.set_color_by_gradient(average_color(color, WHITE), color, average_color(color, BLACK))
        sum_graph = self.get_sum_graph(frequency_axes, shifted_graphs)
        sum_graph.match_style(fourier_graph)
        shifted_graphs.save_state()
        self.play(ReplacementTransform(VGroup(fourier_graph), shifted_graphs, lag_ratio=0.5, run_time=2))
        self.wait()
        self.play(shifted_graphs.arrange, DOWN, shifted_graphs.move_to, fourier_graph, DOWN)
        self.wait()
    ...
    def echos_of_long_pure_signal_in_frequency_space(self):
        ...
        new_fourier_graph = self.frequency_axes.get_graph(lambda x: f_max * np.exp(-100 * (x - 8) ** 2), num_graph_points=1000)
        new_fourier_graph.set_color(PINK)
        self.play(FadeOut(curr_fourier_graph), FadeIn(new_fourier_graph))
    ...
    def concentrated_fourier_requires_long_time(self):
        ...
        self.play(FadeIn(randy))
        self.add(*continual_anims)
        self.play(randy.change, 'angry', *[UpdateFromAlphaFunc(obj, lambda m, a: m.set_fill(opacity=a)) for obj in objects])
        self.play(Blink(randy))
        self.wait(2)
        self.play(Blink(randy))
        self.wait()
        self.play(randy.change, 'plain', *[UpdateFromAlphaFunc(obj, lambda m, a: m.set_fill(opacity=1 - a)) for obj in objects])
        self.wait()

    def get_frequency_pulse_graph(self, x, freq=25, **kwargs):
        graph = IntroduceDopplerRadar.get_frequency_pulse_graph(self, x, freq, **kwargs)
        return graph

    def get_pulse(self, dish, echo_object):
        return RadarPulse(dish, echo_object, n_pulse_singletons=self.n_pulse_singletons, frequency=0.025, speed=5.0)

    def get_pulses(self):
        return [self.get_pulse(self.dish.copy().shift(0.01 * obj.get_center()[0]), obj) for obj in self.objects]
    ...
    def get_shifted_frequency_graphs(self, fourier_graph):
        ...
        def get_func(v):
            return lambda f: fourier_graph.underlying_function(np.clip(f - 5 * v[0], frequency_axes.x_min, frequency_axes.x_max))

        def get_graph(func):
            return frequency_axes.get_graph(func)
        shifted_graphs = VGroup(*list(map(get_graph, list(map(get_func, self.object_velocities)))))
...
class SummarizeFourierTradeoffForDoppler(Scene):
    ...
    def construct(self):
        time_axes = Axes(x_min=0, x_max=12, y_min=-0.5, y_max=1)
        ...
        def get_top_graphs():
            ...
            graphs = VGroup(*[time_axes.get_graph(lambda t: np.exp(-5 * a * (t - x) ** 2)) for x in x_values])
        ...
        def get_bottom_graphs():
            a = np.exp(a_mob.get_center()[0])
            graphs = VGroup(*[frequency_axes.get_graph(lambda t: np.exp(-(5.0 / a) * (t - v) ** 2)) for v in v_values])
            graphs.set_color(RED)
            return graphs
        top_graphs = get_top_graphs()
        bottom_graphs = get_bottom_graphs()
        update_top_graphs = Mobject.add_updater(top_graphs, lambda g: Transform(g, get_top_graphs()).update(1))
        update_bottom_graphs = Mobject.add_updater(bottom_graphs, lambda g: Transform(g, get_bottom_graphs()).update(1))
        self.add(time_axes, frequency_axes)
...
class IntroduceDeBroglie(Scene):
    CONFIG = {'default_wave_frequency': 1, 'wave_colors': [BLUE_D, YELLOW], 'dispersion_factor': 1, 'amplitude': 1}
    ...
    def construct(self):
        ...
        broadcasts = [Broadcast(eiffel_tower.get_top(), big_radius=10, n_circles=10, lag_ratio=0.9, run_time=7, rate_func=squish_rate_func(smooth, a, a + 0.3), color=WHITE) for a in np.linspace(0, 0.7, 3)]
        self.play(*broadcasts)
        self.play(picture.restore)
        self.play(Write(name))
        self.wait()
        time_line = NumberLine(x_min=1900, x_max=1935, tick_frequency=1, big_tick_numbers=list(range(1900, 1941, 10)), color=BLUE_D)
        ...
        update_words(words)
        self.play(FadeIn(time_line), GrowArrow(arrow), Write(words), Write(date), run_time=1)
        self.wait()
        self.play(arrow.next_to, time_line.number_to_point(1924), UP, ChangingDecimal(date, get_year, position_update_func=lambda m: m.next_to(arrow, UP, LARGE_BUFF)), UpdateFromFunc(words, update_words), run_time=3)
        self.wait()
        line = time_line
        self.play(FadeOut(time_line.numbers), VGroup(arrow, words, date).shift, MED_LARGE_BUFF * UP, *[ApplyFunction(lambda m: m.rotate(TAU / 4).set_stroke(width=0), mob, remover=True) for mob in time_line.tick_marks])
        ...
        wave_update_animation = self.get_wave_update_animation(axes, particle, line, im_line)
        for x in range(3):
            particle.move_to(axes.coords_to_point(-10, 0))
            self.play(ApplyMethod(particle.move_to, axes.coords_to_point(22, 0), rate_func=linear), wave_update_animation, run_time=3)
            self.wait()

    def get_wave_update_animation(self, axes, particle, re_line=None, im_line=None):
        line = Line(axes.x_axis.get_left(), axes.x_axis.get_right())
        if re_line is None:
            re_line = line.copy()
    ...
    def get_wave(self, axes, particle, complex_to_real_func=lambda z: z.real, freq=None, **kwargs):
        freq = freq or self.default_wave_frequency
        k0 = 1.0 / freq
        t0 = axes.x_axis.point_to_number(particle.get_center())

        def func(x):
            dispersion = fdiv(1.0, self.dispersion_factor) * np.sqrt(1.0 / (1 + t0 ** 2))
            wave_part = complex_to_real_func(np.exp(complex(0, TAU * freq * (x - dispersion))))
    ...
    def get_wave_pair(self, axes, particle, colors=None, **kwargs):
        ...
        return VGroup(*[self.get_wave(axes, particle, C_to_R, color=color, **kwargs) for C_to_R, color in zip([lambda z: z.imag, lambda z: z.real], colors)])

class ShowMomentumFormula(IntroduceDeBroglie, TeacherStudentsScene):
    CONFIG = {'default_wave_frequency': 2, 'dispersion_factor': 0.25, 'p_color': BLUE, 'xi_color': YELLOW, 'amplitude': 0.5}
    ...
    def introduce_formula(self):
        ...
        p_arrow = Arrow(p_words.get_bottom(), p.get_corner(UP + LEFT), buff=SMALL_BUFF)
        ...
        xi_arrow = Arrow(xi_words.get_bottom(), xi.get_corner(UP + RIGHT), buff=SMALL_BUFF)
        ...
        axes = Axes(x_min=0, x_max=FRAME_WIDTH, y_min=-1, y_max=1)
        ...
        wave_propagation = AnimationGroup(ApplyMethod(particle.move_to, axes.coords_to_point(30, 0)), wave_update_animation, run_time=4, rate_func=linear)
        stopped_wave_propagation = AnimationGroup(ApplyMethod(particle.move_to, xi_words), wave_update_animation, run_time=3, rate_func=linear)
        n_v_lines = 10
        v_lines = VGroup(*[DashedLine(UP, DOWN) for x in range(n_v_lines)])
        v_lines.match_color(xi)
        v_lines.arrange(RIGHT, buff=float(axes.x_axis.unit_size) / self.default_wave_frequency)
        ...
        self.play(self.teacher.change, 'raise_right_hand', GrowArrow(p_arrow), Succession(Write, p_words, ApplyMethod, p_words.next_to, added_p_words, UP), FadeIn(added_p_words, rate_func=squish_rate_func(smooth, 0.5, 1), run_time=2), wave_propagation)
        self.play(Write(xi_words), GrowArrow(xi_arrow), self.change_students('confused', 'erm', 'sassy'), stopped_wave_propagation)
        self.play(FadeIn(added_xi_words), xi_words.next_to, added_xi_words, UP)
        self.play(LaggedStartMap(ShowCreation, v_lines), self.change_students(*['pondering'] * 3))
        self.play(LaggedStartMap(FadeOut, v_lines))
        self.wait()
        self.formula_labels = VGroup(p_words, p_arrow, added_p_words, xi_words, xi_arrow, added_xi_words)
    ...
    def react_to_claim(self):
        ...
        self.student_says('Hang on...', bubble_config={'height': 2, 'width': 2, 'direction': LEFT}, target_mode='sassy', index=2, added_anims=[self.teacher.change, 'plain'])
        student.bubble.add(student.bubble.content)
        self.wait()
        kwargs = {'path_arc': TAU / 4, 'lag_ratio': 0.5, 'lag_ratio': 0.7, 'run_time': 1.5}
        self.play(full_formula.scale, 0, full_formula.move_to, student.eyes.get_bottom() + SMALL_BUFF * DOWN, Animation(student.bubble), **kwargs)
        self.play(full_formula.restore, Animation(student.bubble), **kwargs)
        wave_propagation.update_config(rate_func=lambda a: interpolate(0.35, 1, a))
        self.play(wave_propagation, RemovePiCreatureBubble(student, target_mode='confused'))
        wave_propagation.update_config(rate_func=lambda t: t)
        self.student_says('Physics is \\\\ just weird', bubble_config={'height': 2.5, 'width': 3}, target_mode='shruggie', index=0, added_anims=[ApplyMethod(full_formula.shift, UP)])
        self.wait()
        self.play(wave_propagation, ApplyMethod(full_formula.shift, DOWN), FadeOut(self.students[0].bubble), FadeOut(self.students[0].bubble.content), self.change_students(*3 * ['pondering']), self.teacher.change, 'pondering')
...
class AskPhysicists(PiCreatureScene):
    ...
    def construct(self):
        ...
        formula.set_color_by_tex_to_color_map({'p': BLUE, '\\xi': YELLOW})
        formula.scale(1.5)
        formula.to_edge(UP)
        formula.save_state()
        formula.shift(DOWN)
        ...
        self.wait(2)
        self.play(RemovePiCreatureBubble(morty), PiCreatureSays(physy2, 'Take the Schrödinger equation \\\\ with $H = \\frac{p^2}{2m}+V(x)$', bubble_config={'fill_opacity': 0.9}))
        self.play(PiCreatureSays(physy1, 'Even classically position and \\\\ momentum are conjugate', target_mode='surprised', bubble_config={'fill_opacity': 0.9}))
        self.play(PiCreatureSays(physy3, 'Consider special relativity \\\\ together with $E = hf$', target_mode='hooray', bubble_config={'fill_opacity': 0.9}), morty.change, 'guilty')
        self.wait(2)
    ...
    def create_pi_creatures(self):
        ...
        physies = VGroup(*[PiCreature(color=c).flip() for c in (GREY, GREY_B, GREY_D)])
...
class SortOfDopplerEffect(PiCreatureScene):
    CONFIG = {'omega': np.pi, 'arrow_spacing': 0.25}
    ...
    def construct(self):
        ...
        def get_wave():
            ...
            graph = FunctionGraph(func, x_min=-5 * FRAME_X_RADIUS, x_max=FRAME_X_RADIUS, color=color)
            return VGroup(graph, *[Arrow(x * RIGHT, x * RIGHT + func(x) * UP, color=color) for x in np.arange(-4 * FRAME_X_RADIUS, FRAME_X_RADIUS, self.arrow_spacing)])
            return
        wave = get_wave()
        wave_update = Mobject.add_updater(wave, lambda w: Transform(w, get_wave()).update(1))
        ...
        ref_frame2_follow = Mobject.add_updater(ref_frame2, lambda m: m.next_to(rect, UP))
        ref_frame_1_continual_anim = ContinualAnimation(ref_frame1)
        self.add(t_tracker, wave_update, rect_movement, randy_look_at, ref_frame2_follow, ref_frame_1_continual_anim)
        self.add(ref_frame1)
        self.play(randy.change, 'pondering')
        self.wait(4)
        start_height = screen_rect.get_height()
        start_center = screen_rect.get_center()
        self.play(UpdateFromAlphaFunc(screen_rect, lambda m, a: m.move_to(interpolate(start_center, rect.get_center(), a))), k_tracker.shift, 2 * RIGHT)
        self.play(MaintainPositionRelativeTo(screen_rect, rect, run_time=4))
        self.play(screen_rect.move_to, rect.get_right() + FRAME_X_RADIUS * LEFT, k_tracker.shift, 2 * LEFT)
        ...
        self.play(Transform(time, space), Transform(space, time), lag_ratio=0.5, run_time=1)
...
class HangingWeightsScene(MovingCameraScene):
    CONFIG = {'frequency': 0.5, 'ceiling_radius': 3 * FRAME_X_RADIUS, 'n_springs': 72, 'amplitude': 0.6, 'spring_radius': 0.15}
    ...
    def setup_springs(self):
        ...
        def get_spring(alpha, height=2):
            ...
            s = (height - r) / t_max ** 2
            spring = ParametricCurve(lambda t: op.add(r * (np.sin(TAU * t) * RIGHT + np.cos(TAU * t) * UP), s * (t_max - t) ** 2 * DOWN), t_min=0, t_max=t_max, color=WHITE, stroke_width=2)
            spring.alpha = alpha
    ...
    def setup_weights(self):
        ...
        for spring in self.springs:
            ...
            weight.target_radius = 0.25 * mass
            weight.spring = spring
            weight_anim = Mobject.add_updater(weight, lambda w: w.move_to(w.spring.get_bottom()))
    ...
    def introduce(self):
        ...
        self.play(*[UpdateFromAlphaFunc(weight, lambda w, a: w.set_width(2 * interpolate(w.start_radius, w.target_radius, a)), run_time=2) for weight in self.weights])
        self.play(FadeOut(arrows))
        self.wait(3)

    def show_analogy_with_electron(self):
        words = OldTexText('Analogous to the energy of a particle \\\\', '(in the sense of $E=mc^2$)')
        words.move_to(DOWN)
        self.play(Write(words))
        self.wait(3)
    ...
    def metaphor_for_something(self):
        ...
        self.play(Animation(de_broglie.backdrop, remover=True), FadeIn(de_broglie))
        self.play(Write(words))
        self.wait(7)
        self.play(FadeOut(words), Animation(einstein.backdrop, remover=True), FadeIn(einstein))
        self.wait(2)
    ...
    def moving_reference_frame(self):
        ...
        self.add(rect)
        self.play(Animation(self.de_broglie.backdrop, remover=True), FadeOut(self.de_broglie), Animation(self.einstein.backdrop, remover=True), FadeOut(self.einstein))
        ...
        def zoom_into_reference_frame():
            ...
            self.play(UpdateFromAlphaFunc(camera_frame, lambda c, a: c.set_height(interpolate(original_height, 0.95 * rect.get_height(), a)).move_to(interpolate(original_center, rect.get_center(), a))), ApplyMethod(self.k_tracker.shift, RIGHT))
            self.play(MaintainPositionRelativeTo(camera_frame, rect, run_time=6))
            self.play(camera_frame.set_height, original_height, camera_frame.move_to, original_center, ApplyMethod(self.k_tracker.shift, LEFT))
        zoom_into_reference_frame()
        self.wait()
        self.play(UpdateFromAlphaFunc(rect, lambda m, a: m.set_stroke(width=2 * (1 - a))))
        ...
        self.add(Mobject.add_updater(randy, lambda m: m.next_to(rect.get_corner(DOWN + LEFT), UP + RIGHT, MED_LARGE_BUFF).look_at(weights)), de_broglie)
        self.wait(2)
        zoom_into_reference_frame()
        self.wait(8)
    ...
    def get_peak_flash_anim(self, weight):
        ...
        def update(mob, dt):
            ...
            if different_dy and mob.time_since_last_flash > 0.5:
                mob.curr_anim = Flash(VectorizedPoint(point), flash_radius=0.5, line_length=0.3, run_time=0.2)
...
class WhatDoesTheFourierTradeoffTellUs(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says('So! What does \\\\ the Fourier trade-off \\\\ tell us?', target_mode='surprised', bubble_config={'width': 4, 'height': 3})
        self.play_student_changes(*['thinking'] * 3)
        self.wait(4)

class FourierTransformOfWaveFunction(Scene):
    CONFIG = {'wave_stroke_width': 3, 'wave_color': BLUE}
    ...
    def show_wave_packet(self):
        axes = Axes(x_min=0, x_max=12, y_min=-1, y_max=1, y_axis_config={'tick_frequency': 0.5})
        ...
        wave_update_animation = UpdateFromFunc(wave, lambda w: Transform(w, self.get_wave(axes)).update(1))
        self.add(axes, wave)
        self.play(self.x0_tracker.set_value, 5, wave_update_animation, run_time=3)
        self.wait()
    ...
    def take_fourier_transform(self):
        ...
        frequency_axes = Axes(x_min=0, x_max=3, x_axis_config={'unit_size': 4, 'tick_frequency': 0.25, 'big_tick_numbers': [1, 2]}, y_min=-0.15, y_max=0.15, y_axis_config={'unit_size': 7.5, 'tick_frequency': 0.05})
        ...
        def get_wave_function_fourier_graph():
            return get_fourier_graph(frequency_axes, self.get_wave_func(), t_min=0, t_max=15)
        fourier_graph = get_wave_function_fourier_graph()
        self.fourier_graph_update_animation = UpdateFromFunc(fourier_graph, lambda m: Transform(m, get_wave_function_fourier_graph()).update(1))
        wave_copy = wave.copy()
        ...
        fourier_graph.fade(1)
        arrow = Arrow(self.axes.coords_to_point(5, -1), frequency_axes.coords_to_point(1, 0.1), color=YELLOW)
        fourier_label = OldTexText('Fourier Transform')
        fourier_label.next_to(arrow.get_center(), RIGHT)
        self.play(ReplacementTransform(self.axes.copy(), frequency_axes))
        self.play(MoveToTarget(wave_copy, remover=True), fourier_graph.restore, GrowArrow(arrow), Write(fourier_label, run_time=1))
        self.wait()
        self.frequency_axes = frequency_axes
        self.fourier_graph = fourier_graph
    ...
    def show_correlations_with_pure_frequencies(self):
        frequency_axes = self.frequency_axes
        axes = self.axes
        sinusoid = axes.get_graph(lambda x: 0.5 * np.cos(TAU * x), x_min=-FRAME_X_RADIUS, x_max=3 * FRAME_X_RADIUS)
        ...
        freq_label.next_to(v_line, UP, SMALL_BUFF, submobject_to_align=f_equals[0])
        self.play(ShowCreation(sinusoid), ShowCreation(v_line), Write(freq_label, run_time=1), FadeOut(self.fourier_label))
        last_f = 1
        for f in (1.4, 0.7, 1):
            self.play(sinusoid.stretch, f / last_f, 0, {'about_point': axes.coords_to_point(0, 0)}, v_line.move_to, frequency_axes.coords_to_point(f, 0), DOWN, MaintainPositionRelativeTo(freq_label, v_line), ChangeDecimalToValue(freq_decimal, f), run_time=3)
            last_f = f
    ...
    def this_is_momentum(self):
        ...
        brace.add(words)
        for k in (2, 0.5, 1):
            self.play(self.k_tracker.set_value, k, self.wave_update_animation, self.fourier_graph_update_animation, UpdateFromFunc(brace, lambda b: b.move_to(self.frequency_axes.coords_to_point(self.k_tracker.get_value(), f_max), DOWN)), run_time=2)
        self.wait()
        self.play(*list(map(FadeOut, [brace, words, formula])))

    def show_tradeoff(self):
        for a in (5, 0.1, 0.01, 10, 0.5):
            self.play(ApplyMethod(self.a_tracker.set_value, a, run_time=2), self.wave_update_animation, self.fourier_graph_update_animation)
            self.wait()
    ...
    def get_wave(self, axes):
        return axes.get_graph(self.get_wave_func(), color=self.wave_color, stroke_width=self.wave_stroke_width)
...
class MusicalNote(AddingPureFrequencies):
    ...
    def construct(self):
        ...
        time_label.next_to(axes.x_axis.get_right(), UP)
        axes.add(time_label)
        graph = axes.get_graph(lambda x: op.mul(np.exp(-0.2 * (x - 4) ** 2), 0.3 * (np.cos(2 * TAU * x) + np.cos(3 * TAU * x) + np.cos(5 * TAU * x))))
        graph.set_color(BLUE)
        v_line = DashedLine(ORIGIN, 0.5 * UP)
        v_line_update = UpdateFromFunc(v_line, lambda l: l.put_start_and_end_on_with_projection(graph.get_points()[-1], axes.x_axis.number_to_point(axes.x_axis.point_to_number(graph.get_points()[-1]))))
        self.add(speaker, axes)
        self.play(randy.change, 'pondering', self.get_broadcast_animation(n_circles=6, run_time=5), self.get_broadcast_animation(n_circles=12, run_time=5), ShowCreation(graph, run_time=5, rate_func=linear), v_line_update)
        self.wait(2)

class AskAboutUncertainty(TeacherStudentsScene):

    def construct(self):
        self.student_says("What does this have \\\\ to do with ``certainty''", bubble_config={'direction': LEFT}, index=2)
        self.play(PiCreatureSays(self.students[0], 'What even are \\\\ these waves?', target_mode='confused'))
        self.wait(2)

class ProbabalisticDetection(FourierTransformOfWaveFunction):
    CONFIG = {'wave_stroke_width': 2}
    ...
    def setup_wave(self):
        axes = Axes(x_min=0, x_max=10, y_min=-0.5, y_max=1.5, y_axis_config={'unit_size': 1.5, 'tick_frequency': 0.25})
        ...
        wave = self.get_wave(axes)
        self.wave_update_animation = UpdateFromFunc(wave, lambda w: Transform(w, self.get_wave(axes)).update(1))
        ...
        squared_bell_curve = axes.get_graph(lambda x: bell_curve.underlying_function(x) ** 2).match_style(bell_curve)
        self.set_variables_as_attrs(axes, wave, bell_curve, squared_bell_curve)

    def detect_only_single_points(self):
        particle = ProbabalisticDotCloud(n_copies=100, fill_opacity=0.05, time_per_change=0.05)
        particle.mobject[0].set_fill(BLUE, opacity=1)
        gdw = particle.gaussian_distribution_wrapper
        rect = Rectangle(stroke_width=0, height=0.5, width=2)
        ...
        rect.save_state()
        rect.stretch(0, 0)
        gdw_anim = Mobject.add_updater(gdw, lambda m: m.set_width(2.0 / self.a_tracker.get_value() ** 0.5).move_to(rect))
    ...
    def show_probability_distribution(self):
        ...
        rect = self.rect
        v_lines = VGroup(*[DashedLine(ORIGIN, 3 * UP).move_to(point, DOWN) for point in (rect.get_left(), rect.get_right())])
        self.play(FadeIn(VGroup(axes, wave)), question_group.next_to, v_lines, UP, {'buff': 0}, *list(map(ShowCreation, v_lines)))
        self.wait(10)

    def show_concentration_of_the_wave(self):
        self.play(self.a_tracker.set_value, 5, self.wave_update_animation)
        self.wait(10)
...
class HeisenbergPetPeeve(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.add(particle)
        self.wait()
        self.play(PiCreatureSays(other, '\n            According to the H.U.P., the \\\\\n            universe is unknowable!\n            ', target_mode='speaking'))
        self.play(morty.change, 'angry')
        self.wait(3)
        self.play(PiCreatureSays(morty, 'Well, yes and no', target_mode='sassy'), RemovePiCreatureBubble(other, target_mode='erm'))
        self.wait(4)
...
class OneLevelDeeper(Scene):
    ...
    def construct(self):
        ...
        group.arrange(DOWN)
        randomness = ProbabalisticMobjectCloud(OldTexText('Randomness'), n_copies=5, time_per_change=0.05)
        ...
        self.wait(4)
        self.play(FadeIn(wave_words), GrowArrow(arrow), ApplyMethod(gdw.next_to, wave_words, DOWN, MED_LARGE_BUFF, path_arc=TAU / 2))
        self.wait(6)
...
class BetterTranslation(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        english_term.fade(1)
        self.play(english_term.restore, self.change_students(*['pondering'] * 3))
        self.wait()
        german_word.move_to(english_term)
        to_german_group.next_to(german_word, UP, submobject_to_align=to_german_arrow)
        self.play(self.teacher.change, 'raise_right_hand', english_term.next_to, to_german_arrow, UP)
        self.play(GrowArrow(to_german_arrow), FadeIn(to_german_words), ReplacementTransform(english_term.copy().fade(1), german_word))
        self.wait(2)
        group = VGroup(english_term, to_german_group, german_word)
        translation.move_to(german_word)
        translation_group.next_to(german_word, UP, submobject_to_align=translation_arrow)
        self.play(group.next_to, translation_arrow, UP)
        self.play(GrowArrow(translation_arrow), FadeIn(translation_words), ReplacementTransform(german_word.copy().fade(1), translation))
...
class ThinkOfHeisenbergUncertainty(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.add()
        freq = 1
        continual_anims = [always_shift(time_tracker, direction=RIGHT, rate=1), Mobject.add_updater(dot_gdw, lambda d: d.set_width((np.cos(freq * time_tracker.get_value()) + 1.1) / 2)), Mobject.add_updater(vector_gdw, lambda d: d.set_width((-np.cos(freq * time_tracker.get_value()) + 1.1) / 2)), dot_cloud, vector_cloud]
        self.add(*continual_anims)
        position, momentum, time, frequency = list(map(TexText, ['Position', 'Momentum', 'Time', 'Frequency']))
        ...
        self.play(PiCreatureBubbleIntroduction(morty, 'Heisenberg \\\\ uncertainty \\\\ principle', bubble_type=ThoughtBubble, bubble_config={'height': 4, 'width': 4, 'direction': RIGHT}, target_mode='pondering'))
        self.wait()
        self.play(morty.change, 'confused', dot_gdw)
        self.wait(10)
        self.play(ApplyMethod(VGroup(dot_gdw, vector_gdw).shift, FRAME_X_RADIUS * RIGHT, rate_func=running_start))
        self.remove(*continual_anims)
        self.play(morty.change, 'raise_left_hand', groups, FadeIn(groups, lag_ratio=0.5, run_time=3))
        self.wait(2)
...
class Promotion(PiCreatureScene):
    CONFIG = {'camera_class': ThreeDCamera, 'seconds_to_blink': 5}

    def construct(self):
        aops_logo = AoPSLogo()
        aops_logo.next_to(self.pi_creature, UP + LEFT)
        url = OldTexText('AoPS.com/', '3b1b', arg_separator='')
        ...
        words.to_edge(UP)
        arrow = Arrow(aops_logo.get_top(), morty.get_top(), path_arc=-0.4 * TAU, stroke_width=5, tip_length=0.5)
        arrow.tip.shift(SMALL_BUFF * DOWN)
        self.add(words)
        self.play(self.pi_creature.change_mode, 'raise_right_hand', *[DrawBorderThenFill(submob, run_time=2, rate_func=squish_rate_func(double_smooth, a, a + 0.5)) for submob, a in zip(aops_logo, np.linspace(0, 0.5, len(aops_logo)))])
        self.play(words.scale, 0.75, words.next_to, url, DOWN, LARGE_BUFF, words.shift_onto_screen, Write(url))
        self.wait(2)
        self.play(LaggedStartMap(ApplyFunction, aops_logo, lambda mob: (lambda m: m.shift(0.2 * UP).set_color(YELLOW), mob), rate_func=there_and_back, run_time=1), morty.change, 'thinking')
        self.wait()
        self.play(ShowCreation(arrow))
        self.play(FadeOut(arrow))
        self.wait()
        self.play(morty.change_mode, 'plain', morty.flip, morty.scale, 0.7, morty.next_to, mathy, LEFT, LARGE_BUFF, morty.to_edge, DOWN, FadeIn(mathy))
        self.play(PiCreatureSays(mathy, '', bubble_config={'width': 5}, look_at=morty.eyes), morty.change, 'happy', aops_logo.shift, 1.5 * UP + 0.5 * RIGHT)
        self.play(Blink(mathy))
        self.wait()
        self.play(RemovePiCreatureBubble(mathy, target_mode='raise_right_hand'), aops_logo.to_corner, UP + RIGHT, aops_logo.shift, MED_SMALL_BUFF * DOWN, GrowFromPoint(book, mathy.get_corner(UP + LEFT)))
        self.play(morty.change, 'pondering', book)
        self.wait(3)
        self.play(Blink(mathy))
        self.wait()
        self.play(Animation(BackgroundRectangle(book, fill_opacity=1), remover=True), FadeOut(book))
        print(self.num_plays)
        self.play(FadeOut(words), ShowCreation(rect), morty.restore, morty.change, 'happy', rect, FadeOut(mathy))
        self.wait(10)
        self.play(ShowCreation(url_rect))
        self.play(FadeOut(url_rect), url.get_part_by_tex('3b1b').set_color, BLUE)
        self.wait(15)
...
class Thumbnail(Scene):
    ...
    def construct(self):
        ...
        arrow.scale(4)
        arrow.move_to(Line(uncertainty_principle.get_bottom(), quantum.get_top()))
        cross = Cross(arrow)
        ...
        graphs = VGroup(*[axes.get_graph(get_func(a)) for a in (10, 3, 1, 0.3, 0.1)])
        ...
        fourier_graphs = VGroup(*[get_fourier_graph(frequency_axes, graph.underlying_function, t_min=-10, t_max=10) for graph in graphs])
        ...
        self.add(dot_cloud)
        self.add(uncertainty_principle, quantum)
