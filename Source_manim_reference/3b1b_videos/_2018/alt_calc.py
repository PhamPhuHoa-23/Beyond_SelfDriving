...
class NumberlineTransformationScene(ZoomedScene):
    CONFIG = {'input_line_zero_point': 0.5 * UP + (FRAME_X_RADIUS - 1) * LEFT, 'output_line_zero_point': 2 * DOWN + (FRAME_X_RADIUS - 1) * LEFT, 'number_line_config': {'include_numbers': True, 'x_min': -3.5, 'x_max': 3.5, 'unit_size': 2}, 'input_line_config': {'color': BLUE}, 'output_line_config': {}, 'num_inserted_number_line_curves': 20, 'default_delta_x': 0.1, 'default_sample_dot_radius': 0.07, 'default_sample_dot_colors': [RED, YELLOW], 'default_mapping_animation_config': {'run_time': 3}, 'local_coordinate_num_decimal_places': 2, 'zoom_factor': 0.1, 'zoomed_display_height': 2.5, 'zoomed_display_corner_buff': MED_SMALL_BUFF, 'mini_line_scale_factor': 2, 'default_coordinate_value_dx': 0.05, 'zoomed_camera_background_rectangle_fill_opacity': 1.0}
    ...
    def setup_zoomed_camera_background_rectangle(self):
        ...
        self.zoomed_camera_background_rectangle = BackgroundRectangle(frame, fill_opacity=self.zoomed_camera_background_rectangle_fill_opacity)
        self.zoomed_camera_background_rectangle_anim = UpdateFromFunc(self.zoomed_camera_background_rectangle, lambda m: m.replace(frame, stretch=True))
        self.zoomed_camera_background_rectangle_group = VGroup(self.zoomed_camera_background_rectangle)
    ...
    def get_local_sample_dots(self, x, sample_radius=None, **kwargs):
        ...
        config = {'x_min': x - sample_radius, 'x_max': x + sample_radius, 'delta_x': delta_x, 'dot_radius': dot_radius}
    ...
    def get_line_mapping_animation(self, func, **kwargs):
        ...
        return AnimationGroup(self.get_mapping_animation(func, input_line_copy, apply_function_to_points), self.get_mapping_animation(func, input_line_copy.tick_marks, apply_function_to_submobjects))
    ...
    def get_zoomed_camera_frame_mapping_animation(self, func, x=None, **kwargs):
        ...
        return AnimationGroup(self.get_mapping_animation(func, point_mob), UpdateFromFunc(frame, lambda m: m.move_to(point_mob)))

    def apply_function(self, func, apply_function_to_number_line=True, sample_dots=None, local_sample_dots=None, target_coordinate_values=None, added_anims=None, **kwargs):
    ...
    def zoom_in_on_input(self, x, local_sample_dots=None, local_coordinate_values=None, pop_out=True, first_added_anims=None, first_anim_kwargs=None, second_added_anims=None, second_anim_kwargs=None, zoom_factor=None):
        ...
        if not self.zoom_activated and pop_out:
            ...
            self.play(self.get_zoomed_display_pop_out_animation(), *added_anims, **second_anim_kwargs)
...
class ExampleNumberlineTransformationScene(NumberlineTransformationScene):
    CONFIG = {'number_line_config': {'x_min': 0, 'x_max': 5, 'unit_size': 2.0}, 'output_line_config': {'x_max': 20}}
    ...
    def construct(self):
        ...
        self.zoom_in_on_input(x, local_sample_dots=local_sample_dots, local_coordinate_values=[x - dx, x, x + dx])
        self.wait()
        self.apply_function(func, sample_dots=sample_dots, local_sample_dots=local_sample_dots, target_coordinate_values=[func(x) - dx, func(x), func(x) + dx])
        self.wait()
...
class WriteOpeningWords(Scene):
    ...
    def construct(self):
        ...
        raw_string2 = "You're about to go through your first course. Like " + 'any new topic, it will take some hard work to understand,'
        ...
        for word in words:
            self.play(LaggedStartMap(FadeIn, word, run_time=len(word) * letter_wait, lag_ratio=1.5 / len(word)))
            self.wait(word_wait)
            if word.get_tex()[-1] == ',':
                self.wait(comma_wait)

class StartingCalc101(PiCreatureScene):
    CONFIG = {'camera_config': {'background_opacity': 1}, 'image_frame_width': 3.5, 'image_frame_height': 2.5}
    ...
    def show_images(self):
        ...
        modes = ['pondering', 'pondering', 'hesitant', 'hesitant', 'horrified', 'horrified', 'thinking', 'thinking']
    ...
    def show_mystery_topic(self):
        ...
        randy = self.pi_creature
        mystery_box = Rectangle(width=self.image_frame_width, height=self.image_frame_height, stroke_color=YELLOW, fill_color=GREY_D, fill_opacity=0.5)
    ...
    def get_all_images(self):
        images = VGroup(self.get_hard_work_image(), self.get_neat_example_image(), self.get_not_so_neat_example_image(), self.get_physics_image(), self.get_piles_of_formulas_image(), self.get_getting_stuck_image(), self.get_aha_image(), self.get_graphical_intuition_image())
        colors = color_gradient([BLUE, YELLOW], len(images))
        for i, image, color in zip(it.count(), images, colors):
            self.adjust_size(image)
            frame = Rectangle(width=self.image_frame_width, height=self.image_frame_height, color=color, stroke_width=2)
            frame.move_to(image)
            image.frame = frame
            image.add(frame)
    ...
    def get_neat_example_image(self):
        ...
        unfilled_circle = Circle(stroke_width=3, stroke_color=YELLOW, fill_opacity=0)
        ...
        equation = OldTex('{d (\\pi r^2) \\over dr}  = 2\\pi r', tex_to_color_map={'\\pi r^2': BLUE_D, '2\\pi r': YELLOW})
    ...
    def get_physics_image(self):
        t_max = 6.5
        r = 0.2
        spring = ParametricCurve(lambda t: op.add(r * (np.sin(TAU * t) * RIGHT + np.cos(TAU * t) * UP), t * DOWN), t_min=0, t_max=t_max, color=WHITE, stroke_width=2)
        ...
        group.continual_animations = [t_tracker.add_udpater(lambda tracker, dt: tracker.set_value(tracker.get_value() + dt)), spring.add_updater(lambda s: s.stretch_to_fit_height(1.5 + 0.5 * np.cos(3 * t_tracker.get_value()), about_edge=UP)), weight.add_updater(lambda w: w.move_to(spring.get_points()[-1]))]
    ...
    def get_aha_image(self):
        ...
        group = VGroup(scene.cube, scene.faces, scene.bars, scene.corner_cube)
    ...
    def get_graphical_intuition_image(self):
        gs = GraphScene()
        gs.setup_axes()
        graph = gs.get_graph(lambda x: 0.2 * (x - 3) * (x - 5) * (x - 6) + 4, x_min=2, x_max=8)
        ...
        group.rects = rects
        group.continual_animations = [turn_animation_into_updater(Write(rects)), turn_animation_into_updater(ShowCreation(graph)), turn_animation_into_updater(FadeIn(gs.axes))]
        self.adjust_size(group)
        return group

class GraphicalIntuitions(GraphScene):
    CONFIG = {'func': lambda x: 0.1 * (x - 2) * (x - 5) * (x - 7) + 4, 'x_labeled_nums': list(range(1, 10))}

    def construct(self):
        self.setup_axes()
        axes = self.axes
        graph = self.get_graph(self.func)
        ss_group = self.get_secant_slope_group(x=8, graph=graph, dx=0.01, secant_line_length=6, secant_line_color=RED)
        rects = self.get_riemann_rectangles(graph, x_min=2, x_max=8, dx=0.01, stroke_width=0)
        deriv_text = OldTexText('Derivative $\\rightarrow$ slope', tex_to_color_map={'slope': ss_group.secant_line.get_color()})
        deriv_text.to_edge(UP)
        integral_text = OldTexText('Integral $\\rightarrow$ area', tex_to_color_map={'area': rects[0].get_color()})
        integral_text.next_to(deriv_text, DOWN)
        self.play(Succession(Write(axes), ShowCreation(graph, run_time=2)), self.get_graph_words_anim())
        self.animate_secant_slope_group_change(ss_group, target_x=2, rate_func=smooth, run_time=2.5, added_anims=[Write(deriv_text), VFadeIn(ss_group, run_time=2)])
        self.play(FadeIn(integral_text))
        self.play(LaggedStartMap(GrowFromEdge, rects, lambda r: (r, DOWN)), Animation(axes), Animation(graph))
        self.wait()

    def get_graph_words_anim(self):
        words = VGroup(OldTexText('Graphs,'), OldTexText('graphs,'), OldTexText('non-stop graphs'), OldTexText('all day'), OldTexText('every day'), OldTexText('as if to visualize is to graph'))
        for word in words:
            word.add_background_rectangle()
        words.arrange(DOWN)
        words.to_edge(UP)
        return LaggedStartMap(FadeIn, words, rate_func=there_and_back, run_time=len(words) - 1, lag_ratio=0.6, remover=True)

class Wrapper(Scene):
    CONFIG = {'title': '', 'title_kwargs': {}, 'screen_height': 6, 'wait_time': 2}
...
class DomainColoringWrapper(Wrapper):
    CONFIG = {'title': 'Complex $\\rightarrow$ Complex'}
...
class ExampleMultivariableFunction(LinearTransformationScene):
    CONFIG = {'show_basis_vectors': False, 'show_coordinates': True}
...
class MoreTopics(Scene):
    ...
    def construct(self):
        ...
        others = VGroup(OldTexText('Multivariable calculus'), OldTexText('Complex analysis'), OldTexText('Differential geometry'), OldTexText('$\\vdots$'))
        for word in others:
            word.add_background_rectangle()
        others.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        ...
        self.add(rect)
        self.add(calculus)
        self.play(LaggedStartMap(ShowCreation, lines), LaggedStartMap(Write, others))
        self.wait()
...
class SetTheStage(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ordinary.shift, UP, FadeInFromDown(transformational), self.teacher.change, 'hooray', self.change_students(*3 * ['erm']))
        self.wait(3)
...
class StandardDerivativeVisual(GraphScene):
    ...
    def show_function_graph(self):
        ...
        self.play(DrawBorderThenFill(input_triangle, run_time=1), Write(x_label), ShowCreation(v_line), GrowFromCenter(graph_dot))
        self.add_foreground_mobject(graph_dot)
        self.play(ShowCreation(h_line), Write(output_label), DrawBorderThenFill(output_triangle, run_time=1))
        self.add(input_triangle_update, x_label_update, graph_dot_update, v_line_update, h_line_update, output_triangle_update, output_label_update)
    ...
    def show_slope_of_graph(self):
        ...
        def get_slope_line():
            return self.get_secant_slope_group(x=deriv_input_tracker.get_value(), graph=self.graph, dx=0.01, secant_line_length=4).secant_line
        ...
        self.play(ShowCreation(slope_line), Write(deriv_label), Write(slope_decimal), run_time=1)
        self.wait()
    ...
    def encourage_not_to_think_of_slope_as_definition(self):
        ...
        self.play(PiCreatureSays(morty, "Don't think of \\\\ this as the definition", bubble_config={'height': 2, 'width': 4}))
        self.play(Blink(morty))
        self.wait()
        self.play(RemovePiCreatureBubble(morty), UpdateFromAlphaFunc(morty, lambda m, a: m.set_fill(opacity=1 - a), remover=True))
    ...
    def wiggle_input(self, dx=0.5, run_time=3):
        ...
        x_line = Line(self.coords_to_point(x_min, 0), self.coords_to_point(x_max, 0))
        y_line = Line(self.coords_to_point(0, y_min), self.coords_to_point(0, y_max))
        ...
        self.play(ApplyMethod(input_tracker.set_value, input_tracker.get_value() + dx, rate_func=lambda t: wiggle(t, 6)), FadeIn(rects, rate_func=squish_rate_func(smooth, 0, 0.33), remover=True), run_time=run_time)
...
class IntroduceTransformationView(NumberlineTransformationScene):
    CONFIG = {'func': lambda x: 0.5 * np.sin(2 * x) + x, 'number_line_config': {'x_min': 0, 'x_max': 6, 'unit_size': 2.0}}
    ...
    def show_animation_preview(self):
        ...
        self.play(LaggedStartMap(GrowArrow, arrows, run_time=6, lag_ratio=0.3))
    ...
    def indicate_indicate_point_densities(self):
        ...
        def update_upper_brace(brace):
            x = input_tracker.get_value()
            line = Line(self.get_input_point(x), self.get_input_point(x + dx))
        ...
        def update_lower_brace(brace):
            x = input_tracker.get_value()
            line = Line(self.get_output_point(self.func(x)), self.get_output_point(self.func(x + dx)))
            brace.match_width(line, stretch=True)
            brace.next_to(line, UP, buff=SMALL_BUFF)
            return brace
        ...
        stretch_factor_anim = ChangingDecimal(stretch_factor, lambda a: lower_brace.get_width() / upper_brace.get_width(), position_update_func=lambda m: m.next_to(lower_brace, UP, SMALL_BUFF))
        self.play(GrowFromCenter(upper_brace), FadeOut(self.title), Write(new_title, run_time=2))
        self.title = new_title
        self.play(ReplacementTransform(upper_brace.copy(), lower_brace), GrowFromPoint(stretch_factor, upper_brace.get_center()))
        self.play(input_tracker.set_value, self.input_line.x_max - dx, lower_brace_anim, upper_brace_anim, stretch_factor_anim, run_time=8, rate_func=bezier([0, 0, 1, 1]))
        self.wait()
        new_sample_dots = self.get_sample_dots()
        self.play(FadeOut(VGroup(upper_brace, lower_brace, stretch_factor, self.sample_dots, self.moving_input_line)), FadeIn(new_sample_dots))
        self.sample_dots = new_sample_dots

    def show_zoomed_transformation(self):
        x = 2.75
        local_sample_dots = self.get_local_sample_dots(x)
        self.zoom_in_on_input(x, local_sample_dots=local_sample_dots, local_coordinate_values=self.get_local_coordinate_values(x))
        self.wait()
        self.apply_function(self.func, sample_dots=self.sample_dots, local_sample_dots=local_sample_dots, target_coordinate_values=self.get_local_coordinate_values(self.func(x)))
        self.wait()
...
class TalkThroughXSquaredExample(IntroduceTransformationView):
    CONFIG = {'func': lambda x: x ** 2, 'number_line_config': {'x_min': 0, 'x_max': 5, 'unit_size': 1.25}, 'output_line_config': {'x_max': 25}, 'default_delta_x': 0.2}
    ...
    def show_specific_points_mapping(self):
        ...
        int_arrows = VGroup(*[Arrow(self.get_input_point(x), self.get_output_point(self.func(x)), buff=MED_SMALL_BUFF) for x, num in zip(list(range(1, 6)), self.input_line.numbers[1:])])
        ...
        sample_dots = self.get_sample_dots()
        sample_dot_ghosts = sample_dots.copy().fade(0.5)
        self.play(LaggedStartMap(DrawBorderThenFill, sample_dots), LaggedStartMap(FadeOut, int_arrows))
    ...
    def get_deriv_equation(self, x, rhs, color=RED):
        deriv_equation = self.deriv_equation = OldTex('\\frac{df}{dx}(', str(x), ')', '=', str(rhs), tex_to_color_map={str(x): color, str(rhs): color})
...
class ZoomInOnXSquaredNearOne(TalkThroughXSquaredExample):
    ...
    def construct(self):
        ...
        self.apply_function(self.func, apply_function_to_number_line=False, sample_dots=sample_dot_ghost_copies, local_sample_dots=local_sample_dots, target_coordinate_values=local_coords)
        self.remove(sample_dot_ghost_copies)
        self.wait()
        self.play(frame.restore, self.mini_line.restore, local_sample_dots.restore, zcbr_anim, Animation(zcbr_group))
        self.wait()
        extra_zoom_factor = 0.3
        one_group = VGroup(self.local_coordinates.tick_marks[1], self.local_coordinates.numbers[1])
        all_other_coordinates = VGroup(self.local_coordinates.tick_marks[::2], self.local_coordinates.numbers[::2], self.local_target_coordinates)
        ...
        self.play(Write(new_local_coordinates), Write(new_local_sample_dots), one_group.scale, extra_zoom_factor, {'about_point': self.get_input_point(1)}, FadeOut(all_other_coordinates), *[ApplyMethod(dot.scale, extra_zoom_factor) for dot in local_sample_dots])
        ...
        sample_dot_ghost_copies = self.sample_dot_ghosts.copy()
        self.apply_function(self.func, apply_function_to_number_line=False, sample_dots=sample_dot_ghost_copies, local_sample_dots=new_local_sample_dots, target_coordinate_values=new_coordinate_values, added_anims=[FadeIn(stretch_by_two_words)])
        self.remove(sample_dot_ghost_copies)
        self.wait()
...
class ZoomInOnXSquaredNearThree(ZoomInOnXSquaredNearOne):
    ...
    def construct(self):
        ...
        self.zoom_in_on_input(x, pop_out=False, local_sample_dots=local_sample_dots, local_coordinate_values=local_coordinate_values)
        self.play(Write(zoom_words, run_time=1))
        self.wait()
        self.add_foreground_mobject(stretch_words)
        self.apply_function(self.func, apply_function_to_number_line=False, sample_dots=sample_dot_ghost_copies, local_sample_dots=local_sample_dots, target_coordinate_values=target_coordinate_values, added_anims=[Write(stretch_words)])
        self.wait(2)

class ZoomInOnXSquaredNearOneFourth(ZoomInOnXSquaredNearOne):
    CONFIG = {'zoom_factor': 0.01, 'local_coordinate_num_decimal_places': 4, 'zoomed_display_width': 4, 'default_delta_x': 0.25}

    def construct(self):
        zoom_words = OldTexText('Zoomed view \\\\ near $1/4$')
        zoom_words.next_to(self.zoomed_display, DOWN)
        x = 0.25
        local_sample_dots = self.get_local_sample_dots(x, sample_radius=2.5 * self.zoomed_camera.frame.get_width())
        local_coordinate_values = self.get_local_coordinate_values(x, dx=0.01)
        target_coordinate_values = self.get_local_coordinate_values(self.func(x), dx=0.01)
        color = RED
        sample_dot_ghost_copies = self.sample_dot_ghosts.copy()
        ...
        self.add(deriv_equation)
        self.zoom_in_on_input(x, local_sample_dots=local_sample_dots, local_coordinate_values=local_coordinate_values, pop_out=False, first_added_anims=[FadeIn(one_fourth_label), GrowArrow(one_fourth_arrow)])
        self.play(Write(zoom_words, run_time=1))
        self.wait()
        self.add_foreground_mobject(stretch_words)
        self.apply_function(self.func, apply_function_to_number_line=False, sample_dots=sample_dot_ghost_copies, local_sample_dots=local_sample_dots, target_coordinate_values=target_coordinate_values, added_anims=[Write(stretch_words)])
...
class ZoomInOnXSquaredNearZero(ZoomInOnXSquaredNearOne):
    CONFIG = {'zoom_factor': 0.1, 'zoomed_display_width': 4, 'scale_by_term': '???'}
    ...
    def construct(self):
        ...
        self.wait()
        self.add_foreground_mobject(stretch_words)
        self.apply_function(self.func, apply_function_to_number_line=False, sample_dots=sample_dot_ghost_copies, local_sample_dots=local_sample_dots, added_anims=[Write(stretch_words), MaintainPositionRelativeTo(self.local_coordinates, self.zoomed_camera.frame)])
        self.wait(2)
...
class ZoomInMoreAndMoreToZero(ZoomInOnXSquaredNearZero):
    ...
    def construct(self):
        ...
        for factor in (0.1, 0.01, 0.001, 0.0001):
            ...
            zoom_words = OldTexText('Zoomed', '{:,}x \\\\'.format(int(1.0 / factor)), 'near 0')
            zoom_words.next_to(self.zoomed_display, DOWN)
            sample_dots = self.get_local_sample_dots(x)
            coords = self.get_local_coordinate_values(x, dx=factor)
            frame.restore()
            added_anims = [ApplyMethod(last_sample_dots.fade, 1), ApplyMethod(last_coords.fade, 1)]
            ...
            self.zoom_in_on_input(x, local_sample_dots=sample_dots, local_coordinate_values=coords, pop_out=False, zoom_factor=factor, first_added_anims=added_anims)
            self.wait()
...
class ZoomInOnXSquared1000xZero(ZoomInOnXSquaredNearZero):
    CONFIG = {'zoom_factor': 0.001, 'local_coordinate_num_decimal_places': 3}

class ZoomInOnXSquared10000xZero(ZoomInOnXSquaredNearZero):
    CONFIG = {'zoom_factor': 0.0001, 'local_coordinate_num_decimal_places': 4, 'scale_by_term': '0'}

class XSquaredForNegativeInput(TalkThroughXSquaredExample):
    CONFIG = {'input_line_config': {'x_min': -4, 'x_max': 4}, 'input_line_zero_point': 0.5 * UP + 0 * LEFT, 'output_line_config': {}, 'default_mapping_animation_config': {'path_arc': 30 * DEGREES}, 'zoomed_display_width': 4}
    ...
    def show_full_transformation(self):
        ...
        self.play(LaggedStartMap(ApplyFunction, sample_dots[len(sample_dots) / 2:0:-1], lambda mob: (lambda m: m.scale(2).shift(SMALL_BUFF * UP).set_color(PINK), mob), rate_func=there_and_back))
    ...
    def zoom_in_on_example(self):
        ...
        self.apply_function(self.func, sample_dots=sample_dot_ghost_copies, local_sample_dots=local_sample_dots, target_coordinate_values=target_coordinate_values)
        self.wait()
...
class IntroduceContinuedFractionPuzzle(PiCreatureScene):
    CONFIG = {'remove_initial_rhs': True}
    ...
    def ask_question(self):
        ...
        frac.scale(0.8)
        rhs = DecimalNumber((1 - np.sqrt(5)) / 2.0, num_decimal_places=5, show_ellipsis=True)
        ...
        group.scale(1.5)
        group.to_corner(UR)
        self.play(LaggedStartMap(Write, frac, run_time=15, lag_ratio=0.15), FadeInFromDown(equals), FadeInFromDown(rhs), PiCreatureSays(friend, 'Would this be valid? \\\\ If not, why not?', target_mode='confused', look_at=frac, bubble_config={'direction': RIGHT, 'width': 4, 'height': 3}), morty.change, 'pondering')
        self.wait()
        anims = [RemovePiCreatureBubble(friend, target_mode='pondering', look_at=frac)]
        if self.remove_initial_rhs:
            anims += [Animation(frac), FadeOut(equals), rhs.scale, 0.5, rhs.to_corner, DL]
        self.play(*anims)
    ...
    def set_equal_to_x(self):
        ...
        fixed_point_words = VGroup(OldTexText('Fixed point of'), OldTex('f(x) = 1 + \\frac{1}{x}', tex_to_color_map={'x': YELLOW}))
        fixed_point_words.arrange(DOWN)
        self.play(Write(x), Write(equals))
        self.wait()
        self.play(ShowCreation(inner_frac_rect))
        self.wait()
        self.play(inner_frac_group.scale, 0.75, inner_frac_group.center, inner_frac_group.to_edge, LEFT, ReplacementTransform(x.copy(), new_x, path_arc=-90 * DEGREES))
        self.wait()
        self.play(frac[3].stretch, 0.1, 0, {'about_edge': RIGHT}, MaintainPositionRelativeTo(VGroup(frac[2], new_x), frac[3]), UpdateFromFunc(frac[:2], lambda m: m.next_to(frac[3], LEFT)))
        self.wait()
...
class GraphOnePlusOneOverX(GraphScene):
    CONFIG = {'x_min': -6, 'x_max': 6, 'x_axis_width': 12, 'y_min': -4, 'y_max': 5, 'y_axis_height': 8, 'y_axis_label': None, 'graph_origin': 0.5 * DOWN, 'num_graph_anchor_points': 100, 'func_graph_color': GREEN, 'identity_graph_color': BLUE}
    ...
    def add_title(self):
        title = self.title = OldTex('\\text{Solve: }', '1 + \\frac{1}{x}', '=', 'x')
    ...
    def draw_graphs(self, animate=True):
        lower_func_graph, upper_func_graph = func_graph = VGroup(*[self.get_graph(lambda x: 1.0 + 1.0 / x, x_min=x_min, x_max=x_max, color=self.func_graph_color) for x_min, x_max in [(-10, -0.1), (0.1, 10)]])
        func_graph.label = self.get_graph_label(upper_func_graph, 'y = 1 + \\frac{1}{x}', x_val=6, direction=UP)
        ...
        if animate:
            ...
            self.add(func_graph, func_graph.label, identity_graph, identity_graph.label)
    ...
    def show_solutions(self):
        ...
        for num in (phi, phi_bro):
            line = DashedLine(self.coords_to_point(num, 0), self.coords_to_point(num, num), color=WHITE)
            line_copy = line.copy()
        ...
        decimal_kwargs = {'num_decimal_places': 3, 'show_ellipsis': True, 'color': YELLOW}
        arrow_kwargs = {'buff': SMALL_BUFF, 'color': WHITE, 'tip_length': 0.15, 'rectangular_stem_width': 0.025}
        ...
        brother_words = OldTexText("$\\varphi$'s little brother", tex_to_color_map={'$\\varphi$': YELLOW}, arg_separator='')
...
class ThinkAboutWithRepeatedApplication(IntroduceContinuedFractionPuzzle):
    ...
    def obviously_not(self):
        ...
        self.play(PiCreatureSays(randy, 'Obviously not!', bubble_config={'width': 3, 'height': 2}, target_mode='angry', run_time=1), morty.change, 'guilty', friend.change, 'hesitant')
        self.wait()
        self.play(Animation(frac), RemovePiCreatureBubble(randy, target_mode='sassy'), morty.change, 'confused', friend.change, 'confused')
        self.play(LaggedStartMap(ShowCreationThenDestruction, plus_rects, run_time=2, lag_ratio=0.35))
        self.play(WiggleOutThenIn(rhs))
        self.wait(2)
        self.play(frac.scale, 0.7, frac.to_corner, UL, FadeOut(self.equals), rhs.scale, 0.5, rhs.center, rhs.to_edge, LEFT, FadeOut(randy), morty.change, 'pondering', friend.change, 'pondering')
    ...
    def ask_about_fraction(self):
        ...
        self.play(LaggedStartMap(FadeIn, question, run_time=1), GrowArrow(arrow), LaggedStartMap(ApplyMethod, frac, lambda m: (m.set_color, RED), rate_func=there_and_back, lag_ratio=0.2, run_time=2))
        self.wait()
    ...
    def plug_func_into_self(self, value=1, value_str='1'):
        ...
        for n in range(1, len(lines)):
            ...
            mover, target = [VGroup(line[0][0], line[0][-1], line[1], line[2][:4]) for line in (lines[1], new_line)]
            ...
            self.play(*anims)
            self.wait()
            self.play(ReplacementTransform(last_line[0].copy(), new_line[0][1:-1]), ReplacementTransform(last_line[2].copy(), new_line[2][4:]))
...
class RepeatedApplicationWithPhiBro(ThinkAboutWithRepeatedApplication):
    CONFIG = {'value': (1 - np.sqrt(5)) / 2, 'value_str': '-1/\\varphi'}
...
class ShowRepeatedApplication(Scene):
    ...
    def show_repeated_iteration(self):
        line = VGroup()
        decimal_kwargs = {'num_decimal_places': 3, 'show_ellipsis': True}
        phi = (1 + np.sqrt(5)) / 2
        ...
        brace_texts = VGroup(*[brace.get_text(text).scale(0.75, about_edge=UP) for brace, text in zip(braces, ['Arbitrary \\\\ starting \\\\ value', 'Repeatedly apply $f(x)$', "$\\varphi$ \\\\ ``Golden ratio''"])])
        ...
        self.wait()
        self.play(GrowFromEdge(line[1], LEFT), FadeIn(braces[1]), FadeIn(brace_texts[1]))
        ...
        self.play(GrowFromCenter(braces[2]), FadeIn(brace_texts[2]))
        self.wait()
...
class NumericalPlayOnNumberLineFromOne(Scene):
    CONFIG = {'starting_value': 1, 'n_jumps': 10, 'func': lambda x: 1 + 1.0 / x, 'number_line_config': {'x_min': 0, 'x_max': 2, 'unit_size': 6, 'tick_frequency': 0.25, 'big_tick_numbers': [0, 1, 2]}}
...
class NumericalPlayOnNumberLineFromTau(NumericalPlayOnNumberLineFromOne):
    CONFIG = {'starting_value': TAU, 'number_line_config': {'x_min': 0, 'x_max': 7, 'unit_size': 2}}

class NumericalPlayOnNumberLineFromMinusPhi(NumericalPlayOnNumberLineFromOne):
    CONFIG = {'starting_value': -0.61803, 'number_line_config': {'x_min': -3, 'x_max': 3, 'unit_size': 2, 'tick_frequency': 0.25}, 'n_jumps': 25}
...
class RepeatedApplicationGraphically(GraphOnePlusOneOverX, PiCreatureScene):
    CONFIG = {'starting_value': 1, 'n_jumps': 5, 'n_times_to_show_identity_property': 2}
    ...
    def draw_spider_web(self):
        ...
        for n in range(self.n_jumps):
            ...
            for line in (v_line, h_line):
                ...
                self.play(ShowCreation(line), dot.move_to, line_end, randy.change, self.get_new_randy_mode())
                self.wait()
...
class RepeatedApplicationGraphicallyFromNegPhi(RepeatedApplicationGraphically):
    CONFIG = {'starting_value': -0.61, 'n_jumps': 13, 'n_times_to_show_identity_property': 0}
...
class AnalyzeFunctionWithTransformations(NumberlineTransformationScene):
    CONFIG = {'input_line_zero_point': 0.5 * UP, 'output_line_zero_point': 2 * DOWN, 'func': lambda x: 1 + 1.0 / x, 'num_initial_applications': 10, 'num_repeated_local_applications': 7, 'zoomed_display_width': 3.5, 'zoomed_display_height': 2, 'default_mapping_animation_config': {}}
    ...
    def repeatedly_apply_function(self):
        ...
        sample_dots = self.get_sample_dots(delta_x=0.05, dot_radius=0.05, x_min=-10, x_max=10)
        ...
        for x in range(self.num_initial_applications):
            ...
            self.play(ReplacementTransform(lower_output_line, self.output_line), upper_output_line.shift, shift_vect, upper_output_line.fade, 1, sample_dots.shift, shift_vect)
    ...
    def show_arrows(self, sample_points):
        ...
        arrows, alt_arrows = [VGroup(*[Arrow(point, func(point), buff=SMALL_BUFF, tip_length=0.15) for point in sample_points if get_norm(point - input_zero_point) > 0.3]) for func in (point_func, alt_point_func)]
        ...
        self.wait()
        self.play(LaggedStartMap(ApplyMethod, arrows, lambda a: (a.scale, 0.7), rate_func=there_and_back))
        self.wait()
        self.play(Transform(arrows, alt_arrows), FadeIn(one_plus_rect, remover=True), rate_func=there_and_back_with_pause, run_time=4)
        self.wait()
    ...
    def show_phi_and_phi_bro(self):
        ...
        arrow_kwargs = {'buff': SMALL_BUFF, 'rectangular_stem_width': 0.035, 'tip_length': 0.2}
        ...
        def fade_arrow(arrow):
            ...
            return arrow
        self.play(LaggedStartMap(ApplyFunction, self.all_arrows, lambda a: (fade_arrow, a)), FadeIn(phi_arrow), FadeIn(phi_bro_arrow))
        self.play(Write(phi_label), GrowFromCenter(phi_tick))
        self.play(Write(phi_bro_label), GrowFromCenter(phi_bro_tick))
        self.set_variables_as_attrs(input_phi_point, output_phi_point, input_phi_bro_point, output_phi_bro_point, phi_label, phi_tick, phi_bro_label, phi_bro_tick, phi_arrow, phi_bro_arrow)
    ...
    def zoom_in_on_phi(self):
        ...
        title = self.title
        deriv_text = OldTex('|', '\\frac{df}{dx}(\\varphi)', '|', '< 1', tex_to_color_map={'\\varphi': YELLOW})
        ...
        self.zoom_in_on_input(phi, local_sample_dots=local_sample_dots, local_coordinate_values=local_coordinate_values)
        self.wait()
        self.apply_function(self.func, apply_function_to_number_line=False, local_sample_dots=local_sample_dots, target_coordinate_values=local_coordinate_values)
        self.wait()
        self.play(FadeInFromDown(deriv_text_lhs), FadeInFromDown(deriv_text_rhs), title.to_corner, UL)
        self.wait()
        self.play(deriv_text_lhs.next_to, approx_value, LEFT, deriv_text_rhs.next_to, approx_value, RIGHT, FadeIn(approx_value))
        self.wait()
        for n in range(self.num_repeated_local_applications):
            self.apply_function(self.func, apply_function_to_number_line=False, local_sample_dots=local_sample_dots, path_arc=60 * DEGREES, run_time=2)
    ...
    def zoom_in_on_phi_bro(self):
        ...
        deriv_text = OldTex('\\left| \\frac{df}{dx}\\left(\\frac{-1}{\\varphi}\\right) \\right|', '\\approx |%.2f|' % (-1 / phi_bro ** 2), '> 1')
        deriv_text.move_to(self.deriv_text, UL)
        deriv_text[0][10:14].set_color(YELLOW)
        self.play(zoomed_frame.set_height, 4, zoomed_frame.center, self.deriv_text.fade, 1, run_time=2)
        self.wait()
        zcbr.set_fill(opacity=0)
        self.zoom_in_on_input(phi_bro, local_sample_dots=local_sample_dots, local_coordinate_values=local_coordinate_values, zoom_factor=self.zoom_factor, first_anim_kwargs={'run_time': 2})
        self.wait()
        ...
        self.wait()
        for n in range(self.num_repeated_local_applications):
            self.apply_function(self.func, apply_function_to_number_line=False, local_sample_dots=local_sample_dots, path_arc=20 * DEGREES, run_time=2)

class StabilityAndInstability(AnalyzeFunctionWithTransformations):
    CONFIG = {'num_initial_applications': 0}
    ...
    def label_stability(self):
        ...
        for point in (phi_point, phi_bro_point):
            ...
            arrows.second_anim = LaggedStartMap(ApplyMethod, arrows, lambda m: (m.set_color, YELLOW), rate_func=there_and_back_with_pause, lag_ratio=0.7, run_time=2)
        ...
        self.play(Write(stable_label), phi_arrows.anim)
        self.play(phi_arrows.second_anim)
        self.play(Write(unstable_label), phi_bro_arrows.anim)
    ...
    def write_derivative_fact(self):
        ...
        for arrow_group in arrow_groups:
            ...
            for arrow in arrow_group:
                start_point, end_point = [line.number_to_point(line.point_to_number(p)) for line, p in [(self.input_line, arrow.get_start()), (self.output_line, arrow.get_end())]]
...
class StaticAlgebraicObject(Scene):
    ...
    def construct(self):
        ...
        self.play(frac.scale, 1 / factor, frac.set_color, GREY_B, run_time=0.5, rate_func=lambda t: t ** 5)
        self.wait()
...
class NotBetterThanGraphs(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says("Um, yeah, I'll stick \\\\ with graphs thanks", target_mode='sassy')
        self.play(self.teacher.change, 'guilty', self.change_students('sad', 'sassy', 'hesitant'))
        self.wait(2)
        self.play(RemovePiCreatureBubble(self.students[1]), self.teacher.change, 'raise_right_hand')
        self.play_all_student_changes('confused', look_at=self.screen)
        self.wait(3)
        self.teacher_says('You must flex those \\\\ conceptual muscles', added_anims=[self.change_students(*3 * ['thinking'], look_at=self.teacher.eyes)])
        self.wait(3)
...
class WhatComesAfterWrapper(Wrapper):
    ...
    def construct(self):
        ...
        self.play(FadeInFromDown(new_title), self.title.shift, UP, self.title.fade, 1)
        self.wait(3)
...
class TopicsAfterSingleVariable(PiCreatureScene, MoreTopics):
    ...
    def zero_in_on_complex_analysis(self):
        ...
        self.play(complex_analysis.scale, 1.25, complex_analysis.center, complex_analysis.to_edge, UP, LaggedStartMap(FadeOut, self.other_topics), LaggedStartMap(FadeOut, self.lines), FadeOut(self.calculus), *[ApplyMethod(creature.change, 'pondering') for creature in creatures])
        self.wait(4)

class ShowJacobianZoomedIn(LinearTransformationScene, ZoomedScene):
    CONFIG = {'show_basis_vectors': False, 'show_coordinates': True, 'zoom_factor': 0.05}
    ...
    def construct(self):
        ...
        zd_rect = BackgroundRectangle(zoomed_display, fill_opacity=1, buff=MED_SMALL_BUFF)
        ...
        tiny_grid = NumberPlane(x_radius=2, y_radius=2, color=BLUE_E, secondary_color=GREY_D)
        ...
        self.wait()
        self.apply_nonlinear_transformation(example_function, added_anims=[MaintainPositionRelativeTo(zoomed_camera.frame, tiny_grid)], run_time=5)
        self.wait()
...
class PrinciplesOverlay(PiCreatureScene):
    ...
    def construct(self):
        ...
        for mark in q_marks:
            mark.shift(random.random() * RIGHT, random.random() * UP)
            mark.scale(1.5)
...
class ManyInfiniteExpressions(Scene):
    ...
    def construct(self):
        ...
        self.play(*[LaggedStartMap(GrowFromCenter, group, lag_ratio=0.1, run_time=8) for group in (frac, radical, power_tower)])
        self.wait(2)
