...
class CircleConstants(Scene):
    ...
    def setup_circle(self):
        ...
        self.play(ShowCreation(self.circle_arc), Write(self.pi_equals), Write(self.decimal))
    ...
    def change_arc_length(self, new_angle):
        ...
        self.play(Rotate(self.radius, new_angle - self.circle_arc.angle, about_point=ORIGIN), ArcLengthChange(self.circle_arc, new_angle), ChangeDecimalToValue(self.decimal, new_angle, position_update_func=decimal_position_update_func), MaintainPositionRelativeTo(self.pi_equals, self.decimal), LabelTracksLine(self.one, self.radius, buff=0.5), run_time=3)
        self.wait(2)

class AnalysisQuote(Scene):

    def construct(self):
        text = OldTexText('``We therefore set the radius of \\\\the circle\\dots to be = 1, and \\dots\\\\through approximations the \\\\semicircumference of said circle  \\\\has been found to be $= 3.14159\\dots$,\\\\for which number, for the sake of \\\\brevity, I will write $\\pi$\\dots"', alignment='')
...
class BernoulliQuote(Scene):
    ...
    def construct(self):
        text = OldTexText('``Your most profound investigation of the series \\\\$1+{1\\over 4}+{1\\over 9}+{1\\over 16} + $ etc., which I had found to be \\\\one sixth of the square of $\\pi$ itself\\dots, not only\\\\ gave me the greatest pleasure, but also renown \\\\among the whole Academy of St.\\ Petersburg."')
...
def get_circle_drawing_terms(radius=1, positioning_func=lambda m: m.center()):
    ...
    one = OldTex('1')
    one.scale(0.75)
    one_update = UpdateFromFunc(one, lambda m: m.move_to(radius.get_center() + 0.25 * rotate_vector(radius.get_vector(), TAU / 4)))
    ...
    changing_decimal = ChangingDecimal(decimal, lambda a: a * TAU, position_update_func=reposition_decimal, **kwargs)
    terms = VGroup(circle, radius, one, decimal)
    generate_anims1 = lambda: [ShowCreation(radius), Write(one)]
    generate_anims2 = lambda: [ShowCreation(circle, **kwargs), Rotating(radius, about_point=radius.get_start(), **kwargs), changing_decimal, one_update]
...
class PiTauDebate(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(PiCreatureSays(pi, pi_value, target_mode='angry', look_at=tau.eyes))
        self.play(PiCreatureSays(tau, tau_value, target_mode='angry', look_at=pi.eyes, bubble_config={'width': 3, 'height': 2}))
        self.wait()
        terms, generate_anims1, generate_anims2 = get_circle_drawing_terms(radius=1.25, positioning_func=lambda m: m.to_edge(RIGHT, buff=2).to_edge(UP, buff=1))
        circle, radius, one, decimal = terms
        self.play(RemovePiCreatureBubble(pi), RemovePiCreatureBubble(tau), *generate_anims1())
        ...
        self.play(FadeIn(VGroup(circle, diameter, one)), tau.change, 'confused', pi.change, 'hooray')
    ...
    def create_pi_creatures(self):
        ...
        tau = self.tau = TauCreature(file_name_prefix='TauCreatures', color=GREEN_E).flip()
...
class HartlAndPalais(Scene):
    ...
    def construct(self):
        hartl_rect = ScreenRectangle(color=WHITE, stroke_width=1)
        ...
        self.play(Homotopy(lambda x, y, z, t: (x + 0.1 * np.sin(2 * TAU * t) * np.exp(-10 * (t - 0.5 - 0.5 * (y - 1.85)) ** 2), y, z), three_legged_creature, run_time=2))
        self.wait()

class ManyFormulas(Scene):

    def construct(self):
        formulas = VGroup(OldTex('\\sin(x + \\tau) = \\sin(x)'), OldTex('e^{\\tau i} = 1'), OldTex('n! \\approx \\sqrt{\\tau n} \\left(\\frac{n}{e} \\right)^n'), OldTex('c_n = \\frac{1}{\\tau} \\int_0^\\tau f(x) e^{inx}dx'))
        ...
        for denom in (5, 4, 3, 2):
            ...
            arc = Arc(angle=TAU / denom, stroke_color=RED, stroke_width=4, radius=circle.radius)
        ...
        self.play(circle.set_stroke, {'width': 1}, FadeIn(angle_group))
        self.wait()
...
class HistoryOfOurPeople(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says('Today: The history \\\\ of our people.', bubble_config={'width': 4, 'height': 3})
        self.play_all_student_changes('hooray')
        self.wait()
        self.play(*[ApplyMethod(pi.change, 'happy', self.screen) for pi in self.pi_creatures])
        self.wait(4)
...
class EulerWrites628(Scene):
    ...
    def construct(self):
        ...
        image.fade(0.15)
        rect = Rectangle(width=12, height=0.5, stroke_width=0, fill_opacity=0.3, fill_color=GREEN)
        ...
        words = OldTexText('``Let', '$\\pi$', 'be the', 'circumference', 'of a circle whose', "radius = 1''")
        words.set_color_by_tex_to_color_map({'circumference': YELLOW, 'radius': GREEN})
        words.next_to(image, DOWN)
        pi = words.get_part_by_tex('\\pi').copy()
        terms, generate_anims1, generate_anims2 = get_circle_drawing_terms(radius=1, positioning_func=lambda circ: circ.next_to(words, DOWN, buff=1.25))
        circle, radius, one, decimal = terms
        ...
        perimeter = OldTex('\\pi\\epsilon\\rho\\iota\\mu\\epsilon\\tau\\rho\\text{o}\\varsigma', '\\text{ (perimeter)}', '=')
        ...
        self.wait()
        self.play(GrowFromCenter(brace), circle.set_stroke, YELLOW, 1, ReplacementTransform(circle.copy(), unwrapped_perimeter), decimal.scale, 1.25, decimal.next_to, perimeter[-1].get_right(), RIGHT, ReplacementTransform(pi, perimeter[0][0]), Write(perimeter))
        self.wait()
...
class HeroAndVillain(Scene):
    ...
    def construct(self):
        ...
        self.wait(2)
        self.play(FadeIn(bad_euler), Write(bad_euler_label))
...
class AnalysisQuote(Scene):
    ...
    def construct(self):
        ...
        text = OldTexText('``\\dots set the radius of', 'the circle\\dots to be = 1, \\dots \\\\', 'through approximations the', 'semicircumference \\\\', 'of said circle', 'has been found to be', '$= 3.14159\\dots$,\\\\', 'for which number, for the sake of', 'brevity, \\\\ I will write', "$\\pi$\\dots''", alignment='')
        pi_formula = OldTex('\\pi', '=', '{ \\text{semicircumference}', '\\over', '\\text{radius}}')
        text.set_width(FRAME_X_RADIUS)
        text.next_to(analysis, RIGHT, LARGE_BUFF)
        text.to_edge(UP)
        HIGHLIGHT_COLOR = GREEN
        for mob in (text, pi_formula):
            mob.set_color_by_tex_to_color_map({'semicircumference': HIGHLIGHT_COLOR, '3.14': HIGHLIGHT_COLOR, '\\pi': HIGHLIGHT_COLOR})
        terms, generate_anims1, generate_anims2 = get_circle_drawing_terms(radius=1, positioning_func=lambda circ: circ.next_to(text, DOWN, LARGE_BUFF))
...
class QuarterTurn(Scene):
    ...
    def construct(self):
        terms, generate_anims1, generate_anims2 = get_circle_drawing_terms(radius=2, positioning_func=lambda circ: circ.move_to(4 * RIGHT))
        circle, radius, one, decimal = terms
...
class UsingTheta(Scene):
    ...
    def construct(self):
        ...
        update_theta = Mobject.add_updater(theta, lambda m: m.shift(rotate_vector(0.9 * RIGHT, radius.get_angle() / 2) - m[1][0].get_center()))
        angle = Integer(0, unit='^\\circ')
        update_angle = ContinualChangingDecimal(angle, lambda a: radius.get_angle() * (360 / TAU), position_update_func=lambda a: a.next_to(theta, RIGHT, SMALL_BUFF))
        self.add(update_theta, update_angle)
        last_frac = 0
        for frac in (1.0 / 4, 1.0 / 12, 3.0 / 8, 1.0 / 6, 5.0 / 6):
            self.play(Rotate(radius, angle=(frac - last_frac) * TAU, about_point=ORIGIN), UpdateFromAlphaFunc(arc, lambda m, a: m.pointwise_become_partial(start_arc, 0, interpolate(last_frac, frac, a))), run_time=3)
            last_frac = frac
            self.wait()
...
class ThingsNamedAfterEuler(Scene):
    ...
    def construct(self):
        group = VGroup(*list(map(TexText, ["Euler's formula (Complex analysis)", "Euler's formula (Graph theory)", "Euler's formula (Mechanical engineering)", "Euler's formula (Analytical number theory)", "Euler's formula (Continued fractions)", 'Euler-Lagrance equations (mechanics)', 'Euler number (topology)', 'Euler equations (fluid dynamics)', 'Euler angles (rigid-body mechanics)', 'Euler totient function (number theory)'])))
...
class EulerThinking(Scene):
    ...
    def construct(self):
        ...
        pi_vs_tau = OldTexText('Should $\\pi$ represent \\\\', '3.1415...', 'or', '6.2831...', '?')
        pi_vs_tau.set_color_by_tex_to_color_map({'3.14': GREEN, '6.28': RED})
        pi_vs_tau.move_to(bubble.get_bubble_center())
        question = OldTex('\\frac{1}{1} + \\frac{1}{4} + \\frac{1}{9} + \\frac{1}{16} + \\cdots = ', '\\;???')
        ...
        self.wait()
        self.play(FadeOut(VGroup(pi_vs_tau, cross)), FadeIn(question))
        self.wait()

class WhatIsRight(PiCreatureScene):
    CONFIG = {'default_pi_creature_kwargs': {'color': BLUE_D, 'filp_at_start': False}, 'default_pi_creature_start_corner': DOWN}
    ...
    def construct(self):
        ...
        left_mobs = VGroup(sum_over_N_converges, literal_derivative, divide_by_zero_definable)
        right_mobs = VGroup(sum_over_N_diverges, limit_derivative, divide_by_zero_undefinable)
        ...
        for left, right in zip(left_mobs, right_mobs)[:-1]:
            right.align_to(left)
            self.play(randy.change, 'raise_right_hand', FadeInFromDown(left), last_terms.shift, 1.75 * UP)
            self.wait()
        ...
        self.play(randy.change, 'shruggie', FadeInFromDown(left_mobs[-1]), FadeInFromDown(right_mobs[-1]), last_terms.shift, 1.75 * UP)
        self.wait(4)

class AskPuzzle(TeacherStudentsScene):

    def construct(self):
        series = OldTex('\\frac{1}{1} + \\frac{1}{4} + \\frac{1}{9} + \\cdots + ' + '\\frac{1}{n^2} + \\cdots = ', '\\,???')
        series[0].set_color_by_gradient(BLUE_C, BLUE_B)
        series[1].set_color(YELLOW)
        question = OldTexText('How should we think about\\\\', '$\\displaystyle \\sum_{n=1}^\\infty \\frac{1}{n^s}$', 'for arbitrary $s$?')
        question[1].set_color(BLUE)
        question[0].shift(SMALL_BUFF * UP)
        response = OldTexText('What do you mean by ', '$\\displaystyle \\sum_{n = 1}^{\\infty}$', '?')
        response[1].set_color(BLUE)
        self.teacher_says(series)
        self.play_all_student_changes('pondering', look_at=series)
        self.wait(3)
        self.play(FadeOut(self.teacher.bubble), self.teacher.change, 'happy', series.scale, 0.5, series.to_corner, UP + LEFT, PiCreatureSays(self.students[0], question, target_mode='raise_left_hand'))
        self.play_student_changes(None, 'confused', 'confused', added_anims=[self.students[0].look_at, question])
        self.wait(2)
        self.students[0].bubble.content = VGroup()
        self.play(RemovePiCreatureBubble(self.students[0]), question.scale, 0.5, question.next_to, series, DOWN, MED_LARGE_BUFF, LEFT, PiCreatureSays(self.teacher, response))
...
class ChangeTopic(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.add(title)
        question = OldTexText('Have you ever seen why \\\\', '$\\displaystyle \\frac{2}{1} \\cdot \\frac{2}{3} \\cdots' + '\\frac{4}{3} \\cdot \\frac{4}{5} \\cdot' + '\\frac{6}{5} \\cdot \\frac{6}{7} \\cdots = \\frac{\\pi}{2}$', '?')
        question[0].shift(MED_SMALL_BUFF * UP)
        question[1].set_color_by_gradient(YELLOW, GREEN)
        self.play(PiCreatureSays(tau, 'We should \\emph{really} celebrate \\\\ on 6/28!', target_mode='angry'), pi.change, 'guilty')
        self.wait(2)
        self.play(PiCreatureSays(pi, question), RemovePiCreatureBubble(tau, target_mode='pondering', look_at=question))
...
class SpecialThanks(Scene):
    ...
    def construct(self):
        ...
        people = VGroup(*list(map(TexText, ['Ben Hambrecht', 'University Library Basel', 'Martin Mattmüller', 'Library of the Institut de France'])))
...
class EndScene(PatreonEndScreen):
    ...
    def construct(self):
        ...
        watch_basel = OldTexText('One such actual piece of math', '(quite pretty!)')
        ...
        plushie_words = OldTexText('Plushie pi \\\\ creatures \\\\ now available.', alignment='')
        ...
        self.add(instagram)
        pictures = Group(*[ImageMobject('randy/randy_%s' % name) for name in ['science', 'cooking', 'in_a_can', 'sandwhich', 'lab', 'fractal', 'flowers', 'group', 'labcoat', 'tennis']])
