...
class ComputationalNetwork(MovingCameraScene):
    CONFIG = {'x_color': YELLOW, 'f_color': BLUE, 'g_color': GREEN, 'h_color': RED}
    ...
    def draw_network(self):
        ...
        self.tex_to_color_map = {'x': self.x_color, 'f': self.f_color, 'g': self.g_color, 'h': self.h_color}
        ...
        self.play(Write(x), FadeIn(x.box))
        self.play(ShowCreation(xf_line), ShowCreation(xg_line), ReplacementTransform(x.box.copy(), f_formula.box), ReplacementTransform(x.box.copy(), g_formula.box))
        self.play(Write(f_formula), Write(g_formula))
        self.play(ShowCreation(fh_line), ShowCreation(gh_line), ReplacementTransform(f_formula.box.copy(), h_formula.box), ReplacementTransform(g_formula.box.copy(), h_formula.box))
        self.play(Write(h_formula))
        self.wait()
        network = VGroup(formula_groups, graph_edges)
        self.set_variables_as_attrs(x, f_formula, g_formula, h_formula, xf_line, xg_line, fh_line, gh_line, formulas, formula_groups, graph_edges, network)
    ...
    def walk_through_parts(self):
        ...
        def indicate(mob):
            return ShowCreationThenDestructionAround(mob, surrounding_rectangle_config={'buff': 0.5 * SMALL_BUFF, 'color': mob.get_color()})
    ...
    def feed_forward_input(self):
        ...
        self.play(ReplacementTransform(dh_dx_at_two.get_part_by_tex('2').copy(), two), Write(x_val_label[0]))
        self.wait()
        two_copy1 = two.copy()
        two_copy2 = two.copy()
        four_copy = four.copy()
        ...
        self.play(two_copy1.move_to, x_in_f, DOWN, x_in_f.set_fill, {'opacity': 0})
        self.wait()
        self.play(Write(f_val_label))
        self.wait()
        self.play(two_copy2.move_to, x_in_g, DOWN, x_in_g.set_fill, {'opacity': 0})
        self.wait()
        self.play(Write(g_val_label))
        self.wait()
        self.play(four_copy.move_to, f_in_h, DOWN, f_in_h.set_fill, {'opacity': 0})
        self.play(one_copy.move_to, g_in_h, DOWN, g_in_h.set_fill, {'opacity': 0})
        self.wait()
        self.play(Write(h_val_label))
        self.wait()
        self.value_labels = value_labels
        self.revert_to_formula_animations = [ApplyMethod(term.set_fill, {'opacity': 1}) for term in (x_in_f, x_in_g, f_in_h, g_in_h)] + [FadeOut(term) for term in (two_copy1, two_copy2, four_copy, one_copy)]

    def compare_x_and_h_wiggling(self):
        x_val = self.value_labels[0][1]
        h_val = self.value_labels[3][1]
        x_line = NumberLine(x_min=0, x_max=4, include_numbers=True, numbers_to_show=[0, 2, 4], unit_size=0.75)
        x_line.next_to(x_val, DOWN, LARGE_BUFF, aligned_edge=RIGHT)
        h_line = NumberLine(x_min=0, x_max=32, include_numbers=True, big_tick_numbers=[0, 16, 32], numbers_to_show=[0, 16, 32], tick_frequency=1, tick_size=0.05, unit_size=1.0 / 12)
        ...
        self.play(ShowCreation(x_line), ShowCreation(h_line))
        self.play(GrowArrow(x_arrow), GrowArrow(h_arrow), ReplacementTransform(x_val.copy(), x_dot), ReplacementTransform(h_val.copy(), h_dot))
        self.wait()
        for x in range(2):
            self.play(x_dot.shift, 0.25 * RIGHT, h_dot.shift, 0.35 * RIGHT, rate_func=wiggle, run_time=1)
        self.set_variables_as_attrs(x_line, h_line, x_dot, h_dot, x_arrow, h_arrow)
    ...
    def expand_out_h_as_function_of_x(self):
        ...
        self.play(FadeInFromDown(expanded_formula), deriv.scale, 1.0 / 1.5, deriv.shift, DOWN)
        self.wait()
        self.play(ShowCreation(cross))
        self.wait()
        self.play(FadeOut(VGroup(expanded_formula, cross)), deriv.shift, UP)
    ...
    def show_four_derivatives(self):
        ...
        partial_terms = VGroup(dh_df[0][0], dh_df[2][0], dh_dg[0][0], dh_dg[2][0])
    ...
    def show_chain_rule(self):
        ...
        self.play(frame.shift, 3 * UP, dh_dx.to_edge, UP, dh_dx.shift, 3 * LEFT + 3 * UP, at_two.set_fill, {'opacity': 0})
        for deriv in derivatives:
            deriv.generate_target()
        rhs = VGroup(OldTex('='), df_dx.target, dh_df.target, OldTex('+'), dg_dx.target, dh_dg.target)
        rhs.arrange(RIGHT, buff=2 * SMALL_BUFF)
    ...
    def talk_through_mvcr_parts(self):
        ...
        for term in terms:
            term.rect = SurroundingRectangle(term, buff=0.5 * SMALL_BUFF, stroke_width=0, fill_color=TEAL, fill_opacity=0.5)
        for derivative in derivatives:
        ...
        self.wait()
        self.play(rect.replace, df_dx, {'stretch': True}, rect.scale, 1.1)
        self.wait()
        self.play(Transform(rect, del_f.rect), FadeOut(derivatives[0].rect), FadeIn(derivatives[2].rect))
        self.wait()
        self.play(Transform(rect, del_h1.rect))
        self.wait()
        self.play(ReplacementTransform(del_f[1].copy(), del_h_sub_f, path_arc=PI))
        self.wait()
        self.play(del_h_sub_f.shift, UR, del_h_sub_f.fade, 1, rate_func=running_start, remover=True)
        self.wait()
        self.play(Transform(rect, del_f.rect), ReplacementTransform(rect.copy(), df.rect))
        self.wait()
        for x in range(3):
            self.play(ShowCreationThenDestruction(top_lines, lag_ratio=1))
        self.wait()
        self.play(rect.replace, cr_derivatives[1::2], {'stretch': True}, rect.scale, 1.1, FadeOut(df.rect), FadeOut(derivatives[2].rect), FadeIn(derivatives[1].rect), FadeIn(derivatives[3].rect))
        self.wait()
        self.play(Transform(rect, dg.rect), FadeOut(derivatives[3].rect))
        self.wait()
        self.play(Transform(rect, dx2.rect))
        self.wait()
        self.play(Transform(rect, del_h2.rect), FadeOut(derivatives[1].rect), FadeIn(derivatives[3].rect))
        self.wait()
        self.play(Transform(rect, del_g.rect))
        self.wait()
        self.play(rect.replace, cr_derivatives, {'stretch': True}, rect.scale, 1.1, FadeOut(derivatives[3].rect))
        for x in range(3):
            self.play(*[ShowCreationThenDestruction(group, lag_ratio=1) for group in (top_lines, bottom_lines)])
        self.wait()
        self.play(FadeOut(rect))
        self.remove_foreground_mobject(cr_derivatives)

    def plug_in_expressions(self):
        lhs = VGroup(self.dh_dx_at_two[:3], self.chain_rule_rhs[::3], self.chain_rule_derivatives)
        lhs.generate_target()
        lhs.target.to_edge(LEFT)
        df_dx, dg_dx, dh_df, dh_dg = self.chain_rule_derivatives
        formulas = self.formulas
        x, f_formula, g_formula, h_formula = formulas
        full_derivative = OldTex('=', '(', '2', 'x', ')', '(', '2', 'f', 'g', ')', '+', '(', '-\\sin(', '\\pi', 'x', ')', '\\pi', ')', '(', 'f', '^2', ')')
        ...
        self.play(ReplacementTransform(f_formula[2:].copy(), full_derivative[2:4]), FadeIn(full_derivative[1:5:3]))
        self.wait()
        self.play(f_formula.shift, DOWN, df_dx.shift, 0.5 * UP)
        self.wait()
        self.play(g_formula.shift, 0.75 * UP, dg_dx.shift, 0.5 * DOWN)
        self.play(ReplacementTransform(g_formula[2:].copy(), full_derivative[12:17]), FadeIn(full_derivative[11:18:6]), Write(full_derivative[10]))
        self.wait()
        self.play(g_formula.shift, 0.75 * DOWN, dg_dx.shift, 0.5 * UP)
        self.wait()
        self.play(h_formula.shift, UP, dh_df.shift, 0.5 * DOWN)
        self.wait()
        self.play(ReplacementTransform(h_formula[2:].copy(), full_derivative[6:9]), FadeIn(full_derivative[5:10:4]))
        self.wait()
        self.play(dh_df.shift, 0.5 * UP)
        self.play(dh_dg.shift, 0.5 * DOWN)
        self.wait()
        self.play(ReplacementTransform(h_formula[2:].copy(), full_derivative[19:21]), FadeIn(full_derivative[18:22:3]))
        self.wait()
    ...
    def plug_in_values(self):
        ...
        rhs = OldTex('\n            =\n            (2 \\cdot 2)\n            (2 \\cdot 4 \\cdot 1) +\n            (-\\sin(\\pi 2)\\pi)(4^2)\n            ', tex_to_color_map={'2': self.x_color, '4': self.f_color, '1': self.g_color, '^2': WHITE})
    ...
    def discuss_meaning_of_result(self):
        ...
        h_dot = self.h_dot
        for x in range(3):
            self.play(x_dot.shift, 0.25 * RIGHT, h_dot.shift, RIGHT, run_time=2, rate_func=lambda t: wiggle(t, 4))
            self.wait()