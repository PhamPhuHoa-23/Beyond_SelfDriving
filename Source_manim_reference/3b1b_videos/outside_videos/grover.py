...
class QBitDiagram(InteractiveScene):
    ...
    def construct(self):
        axes = Axes((-1.5, 1.5, 0.5), (-1.5, 1.5, 0.5), height=7, width=7)
        axes.set_height(7)
        x_label = Tex('|0\\rangle')
        y_label = Tex('|1\\rangle')
        ...
        vect_label = VGroup(coefs[0], Tex('|0\\rangle'), Tex('+'), coefs[1], Tex('|1\\rangle'))
        ...
        self.add(vect)
        self.add(vect_label)
        prob_labels = VGroup(self.get_prob_label(coefs[0], '0'), self.get_prob_label(coefs[1], '1'))
    ...
    def get_prob_label(self, coef, bit='0'):
        label = Tex(f'P(\\text{{Measure a }}{bit}) = (0.00)^2 = 0.00', font_size=36)
...
class BlocksToQuantum(InteractiveScene):
    ...
    def construct(self):
        wall_height = 2
        block_height = 1.0
        block_style = dict(stroke_color=WHITE, stroke_width=1, fill_color=BLUE_E, fill_opacity=1, shading=(0.5, 0.25, 0))
        ...
        names = ['David', 'Bob', 'Charlie', 'Eve', 'Alice', 'Joan', 'Kathy', 'Morty', 'Lily', 'Randy']
        ...
        coefs.set_color(RED)
        state_tex = VGroup(coefs[0], Tex(f'|\\text{{{names[0]}}}\\rangle'), Tex('+'), coefs[1], Tex(f'|\\text{{{names[1]}}}\\rangle'), Tex('+'), coefs[2], Tex(f'|\\text{{{names[2]}}}\\rangle'), Tex('+'), Tex('\\cdots'), Tex('+'), coefs[-1], Tex(f'|\\text{{{names[-1]}}}\\rangle'))
        ...
        self.play(all_blocks.animate.shift(dist * LEFT), arrows.animate.shift(dist * LEFT), rate_func=linear, run_time=3)
        ...
        self.play(left_block.animate.shift(0.5 * dist * RIGHT), right_blocks.animate.shift(0.5 * dist * LEFT), arrows[0].animate.shift(0.5 * dist * RIGHT), arrows[1:].animate.shift(0.5 * dist * LEFT), run_time=1.5, rate_func=linear)
        left_block.arrow.flip(UP, about_point=left_block.get_center())
