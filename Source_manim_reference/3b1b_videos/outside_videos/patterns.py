...
class PascalColored(Scene):
    CONFIG = {'colors': [BLUE_E, BLUE_D, BLUE_B], 'dot_radius': 0.16, 'n_layers': 2 * 81, 'rt_reduction_factor': 0.5}
    ...
    def construct(self):
        ...
        for layer in layers[1:]:
            ...
            if triangle.get_height() > max_height:
                self.play(triangle.set_height, 0.5 * max_height, triangle.to_edge, UP, LARGE_BUFF)
...
class TriominoGrid(Scene):
    CONFIG = {'random_seed': 4, 'n': 4}

    def construct(self):
        n = self.n
        grid = VGroup(*[VGroup(*[Square() for x in range(2 ** n)]).arrange(RIGHT, buff=0) for y in range(2 ** n)]).arrange(UP, buff=0)
        for row in grid:
            for square in row:
        ...
        colors = [BLUE_C, BLUE_E, BLUE_D, BLUE_B, MAROON_C, MAROON_E, MAROON_D, MAROON_B, YELLOW, GREY_BROWN, GREY_B, GREEN_C, GREEN_E, GREEN_D, GREEN_B]
