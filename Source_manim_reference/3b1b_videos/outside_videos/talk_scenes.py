...
class DoingMathVsHowMathIsPresented(Scene):
    ...
    def construct(self):
        titles = VGroup(OldTexText('How math is presented'), OldTexText('Actually doing math'))
        for title, vect in zip(titles, [LEFT, RIGHT]):
            title.scale(1.2)
            title.to_edge(UP)
            title.shift(vect * FRAME_WIDTH / 4)
        blocks = VGroup(self.get_block(0.98), self.get_block(0.1))
        self.add(titles)
...
class PiCharts(Scene):
    ...
    def construct(self):
        ...
        pi_equation = VGroup(self.get_pi_chart(10), OldTex('+').scale(2), self.get_pi_chart(5), OldTex('=').scale(2), OldTex('?').scale(2))
    ...
    def get_pi_chart(self, n):
        result = VGroup(*[Sector(start_angle=TAU / 4 - k * TAU / n, angle=-TAU / n, stroke_color=WHITE, stroke_width=2, fill_color=BLUE_E, fill_opacity=1) for k in range(n)])
        result.scale(1.5)
        return result
...
class AskAboutCircleProportion(Scene):
    ...
    def construct(self):
        ...
        arc = Arc(radius=R, start_angle=TAU / 3, angle=TAU / 3)
        ...
        arrow = Arrow(question.get_bottom() + LEFT, arc.point_from_proportion(0.25))
        self.add(circles)
        self.add(dots)
        self.add(arc)
        self.add(question)
...
class BorweinIntegrals(Scene):
    ...
    def construct(self):
        ints = VGroup(OldTex('\\int_0^\\infty', '\\frac{\\sin(x)}{x}', 'dx = \\frac{\\pi}{2}'), OldTex('\\int_0^\\infty', '\\frac{\\sin(x)}{x}', '\\frac{\\sin(x/3)}{x/3}', 'dx = \\frac{\\pi}{2}'), OldTex('\\int_0^\\infty', '\\frac{\\sin(x)}{x}', '\\frac{\\sin(x/3)}{x/3}', '\\frac{\\sin(x/5)}{x/5}', 'dx = \\frac{\\pi}{2}'), OldTex('\\vdots'), OldTex('\\int_0^\\infty', '\\frac{\\sin(x)}{x}', '\\frac{\\sin(x/3)}{x/3}', '\\frac{\\sin(x/5)}{x/5}', '\\dots', '\\frac{\\sin(x/13)}{x/13}', 'dx = \\frac{\\pi}{2}'), OldTex('\\int_0^\\infty', '\\frac{\\sin(x)}{x}', '\\frac{\\sin(x/3)}{x/3}', '\\frac{\\sin(x/5)}{x/5}', '\\dots', '\\frac{\\sin(x/13)}{x/13}', '\\frac{\\sin(x/15)}{x/15}', 'dx = \\frac{\\pi}{2}', '- 0.0000000000231006...'))
