...
class NeuralNetworkTest(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(neuron_copy), FadeIn(prev_connections_copy), FadeIn(prev_layer_copy), layers.animate.set_fill(opacity=0.1).set_stroke(opacity=0.1), connections.animate.set_stroke(opacity=0.1))
        ...
        self.play(FadeIn(dentrite, 2 * DOWN))
        self.play(ShowCreation(den_connections, lag_ratio=0.25), prev_connections_copy.animate.set_stroke(width=0.5))
        self.wait()
        ...
        self.play(layers.animate.set_fill(opacity=0.8).set_stroke(opacity=0.8), connections.animate.set_stroke(opacity=0.8), FadeOut(prev_connections_copy))
        ...
        self.animate_back_prop(connections, to_fade=[den_connections, den_to_neuron_connection])

    def get_layer(self, layer_size, radius=0.3, fill_opacity=0.5, stroke_width=3, buff=1):
        layer = VGroup((Circle(radius=radius) for n in range(layer_size)))
    ...
    def get_connections(self, target, sources, max_stroke_width=4, colors=[BLUE, RED]):
        ...
        for neuron in sources:
            line = Line(neuron.get_center(), target.get_center(), buff=neuron.get_radius(), stroke_width=max_stroke_width * random.random(), stroke_color=random.choice(colors))
            target.connections.add(line)
        return target.connections

    def get_den_connection(self, dentrite, neuron):
        den_connection = DashedLine(dentrite.get_center(), neuron.get_center(), buff=neuron.get_radius(), dash_length=0.1, positive_space_ratio=0.7)
    ...
    def animate_computational_pass(self, connections, run_time=3, lag_ratio=0.05, to_fade=[], fade_amount=0.75, reverse=False):
        ...
        self.play(connections.animate.fade(fade_amount).set_anim_args(rate_func=lambda t: there_and_back_with_pause(t, 0.6)), mobs_to_fade.animate.fade(0.5).set_anim_args(rate_func=lambda t: there_and_back_with_pause(t, 0.6)), LaggedStartMap(ShowCreation, animated_connections, lag_ratio=lag_ratio), run_time=run_time)
