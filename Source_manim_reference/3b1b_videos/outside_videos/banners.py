...
class ShortsBanner(Banner):
    ...
    def get_pis(self):
        pis = VGroup(Randolph(color=BLUE_E, mode='pondering'), Randolph(color=BLUE_D, mode='hooray'), Randolph(color=BLUE_C, mode='tease'), Randolph(color=GREY_BROWN, mode='pondering'))
        height = 0.7
        for pi in pis:
            pi.set_height(1)
