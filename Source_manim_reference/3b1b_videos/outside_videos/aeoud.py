...
class VennDiagram(InteractiveScene):
    ...
    def construct(self):
        ...
        arrow = Arrow(question.get_bottom() + LEFT, arc.pfp(0.3), buff=0.2, stroke_width=8, color=WHITE)
        self.add(question, arrow)
        self.wait()
        hexagon = RegularPolygon(6)
        hexagon.replace(c2, dim_to_match=0)
        hexagon.add(*(Line(v1, v2) for v1, v2 in zip(hexagon.get_vertices()[:3], hexagon.get_vertices()[3:])))
