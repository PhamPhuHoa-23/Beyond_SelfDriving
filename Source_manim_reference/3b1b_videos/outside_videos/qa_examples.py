...
class DemoScene(InteractiveScene):
    ...
    def construct(self):
        ...
        for n in range(2, 100):
            ...
            self.play(ShowCreation(highlights, lag_ratio=0.05), run_time=1 / math.sqrt(n))
            self.wait()
            self.remove(labels)
            self.play(LaggedStartMap(flip, grid[::n], lag_ratio=0.05), FadeOut(highlights), Animation(labels), run_time=1 / math.sqrt(n))
        randy = PiCreature()
