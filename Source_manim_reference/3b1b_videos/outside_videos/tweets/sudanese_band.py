...
def get_full_surface(band_func, x_range):
    surface = ParametricSurface(band_func, x_range, (0, TAU))
    surface.set_color(BLUE_D)
    surface.set_shadow(0.5)
    surface.add_updater(lambda m: m.sort_faces_back_to_front(DOWN))
    m1, m2 = meshes = VGroup(SurfaceMesh(surface, normal_nudge=0.001), SurfaceMesh(surface, normal_nudge=-0.001))
    bound = VGroup(ParametricCurve(lambda t: band_func(x_range[0], t), (0, TAU)), ParametricCurve(lambda t: band_func(x_range[1], t), (0, TAU)))
...
def get_sudanese_band(circle_on_xy_plane=False):
    s_band = get_full_surface(sudanese_band_func, (0, PI))
...
class SudaneseBand(ThreeDScene):
    ...
    def construct(self):
        ...
        self.wait()
        m_band.save_state()
        self.play(Transform(m_band, s_band), run_time=8)
        self.play(frame.animate.reorient(-30, 110), run_time=4)
        self.wait(2)
        frame.clear_updaters()
        self.play(m_band.animate.restore(), frame.animate.reorient(-45, 70), run_time=8)
