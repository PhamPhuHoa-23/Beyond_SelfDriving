...
class PowersOfTwo(Scene):
    ...
    def construct(self):
        ...
        def update_group(group, alpha):
            ...
            group.set_submobjects([self.get_label(n), self.get_dots(n)])
        label = self.get_label(0)
        dots = self.get_dots(0)
        for n in range(1, max_n + 1):
            new_label = self.get_label(n)
            new_dots = self.get_dots(n)
            self.play(FadeTransform(label, new_label), FadeTransform(dots, new_dots[0]), FadeIn(new_dots[1]), run_time=0.4)
            self.remove(dots)
            self.add(new_dots)
            self.wait(0.1)
            label = new_label
