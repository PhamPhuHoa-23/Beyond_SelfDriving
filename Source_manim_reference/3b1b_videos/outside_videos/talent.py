...
class Banner(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart((ShowCreation(line, rate_func=linear) for line in line_groups), lag_ratio=0.1), LaggedStartMap(FadeIn, all_dots, lag_ratio=0.1, time_span=(0, 5)), frame.animate.reorient(0, 0, 0, (4.5, 2, 0), 5.96).set_anim_args(time_span=(7, 15)), run_time=15)
...
class WriteNameToUrl(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(LaggedStart(FadeIn(back_rect), ReplacementTransform(full_name['3b1b'], url['3b1b']), Write(url['.co/'].set_stroke(behind=True), stroke_color=WHITE), ReplacementTransform(full_name['Talent'], url['talent']), lag_ratio=0.25))
        self.wait()
...
class TalentPartnerProperties(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(Blink(randy))
        self.wait()
        others = VGroup(PiCreature(color=BLUE_D, mode='gracious'), PiCreature(color=BLUE_C, mode='tease'), PiCreature(color=BLUE_E, mode='well'))
        ...
        hearts.shuffle()
        self.play(LaggedStartMap(FadeIn, others), randy.change('coin_flip_1', others))
        self.wait()
        self.play(LaggedStartMap(FadeIn, hearts, shift=0.25 * UP, lag_ratio=0.25), *(pi.animate.look_at(company) for pi in others))
        ...
        self.add(morty)
        self.play(FadeOut(randy, time_span=(0, 1)), frame.animate.reorient(0, 0, 0, (-6.75, -1.77, 0.0), 3.94), FadeOut(arrow), morty.change('well', others), FadeOut(hearts, lag_ratio=0.5), *(pi.animate.look_at(morty.eyes) for pi in others), run_time=3)
        bubble = morty.get_bubble('...', bubble_type=SpeechBubble)
        self.play(LaggedStart(Write(bubble), morty.change('speaking', others), others[0].change('tease'), others[1].change('happy'), others[1].change('happy'), lag_ratio=0.5))
        self.play(Blink(morty))
        self.wait()
        content = Text('...', font_size=12)
        new_bubble = others[0].get_bubble(content, bubble_type=SpeechBubble, direction=RIGHT)
        content.scale(3)
        self.play(others[0].change('hooray'), Write(new_bubble), FadeOut(bubble), morty.change('tease'))
        self.wait()
        self.play(Blink(others[0]))
        self.play(others[0].change('well', others[1].eyes), others[1].animate.look_at(others[0].eyes), FadeOut(new_bubble))
        self.play(Blink(others[1]))
        self.wait()
...
class CareerFairBooths(InteractiveScene):
    ...
    def construct(self):
        ...
        job_types = BulletedList('Senior roles', 'New careers', 'Internships', 'Part-time', buff=0.75)
        job_types.move_to(booths).shift(0.5 * UP)
        for n, booth in enumerate(booths):
            booth.generate_target()
            booth.target.shift((-1) ** (n + 1) * RIGHT)
        self.play(LaggedStartMap(FadeIn, job_types, lag_ratio=0.5, run_time=4), LaggedStartMap(MoveToTarget, booths, run_time=4))
        self.wait()

class TalentContactCard(InteractiveScene):

    def construct(self):
        words = VGroup(VGroup(Text('Interested in joining?'), Text('Reach out to explore if your company is a good fit')), VGroup(Text('Find a job through this page?'), Text('Let us know, we’d love to hear your story!')))
        for group in words:
            group[0].scale(1.25)
            group[1].scale(0.75)
        ...
        self.add(words[0][0])
        self.play(FadeIn(words[0][1], lag_ratio=0.1, run_time=1.5), FadeIn(address, 0.5 * UP))
        self.play(LaggedStart(FadeIn(words[1][0]), FadeIn(words[1][1], lag_ratio=0.1, run_time=2)))
        self.wait()
...
class IndustryTypes(InteractiveScene):
    ...
    def construct(self):
        ...
        title.to_corner(UL)
        words = VGroup(Text('Education'), Text('AI Safety'), Text('Agentic code developement'), Text('Social engineering defense'), Text('Finance'), Text('Cryptography research'), Text('Software engineering'), Text('More...'))
