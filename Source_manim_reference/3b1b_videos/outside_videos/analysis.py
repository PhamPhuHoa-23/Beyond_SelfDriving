...
class ExpPlay(Scene):
    ...
    def show_sum_up_to(self, n, shown_n=None, include_dots=False):
        ...
        vectors = always_redraw(lambda: self.get_sum_vectors(t_tracker.get_value(), n, center=plane.n2p(0)))
        path = ParametricCurve(lambda t: plane.n2p(approx_exp(complex(0, t), n)), t_min=0, t_max=TAU)
        path.set_color(WHITE)
        formula = OldTex('e^{it} \\approx', *['{(ti)^{%d} \\over %d!} +' % (k, k) for k in range(shown_n + 1)], '\\cdots' if include_dots else '')
        ...
        number_line = NumberLine(x_min=0, x_max=7.5)
        ...
        tip.rotate(-PI / 2)
        tip.set_color(WHITE)
        tip.add_updater(lambda m: m.move_to(number_line.n2p(t_tracker.get_value()), DOWN))
        t_eq = VGroup(OldTex('t='), DecimalNumber(0))
        ...
        self.play(t_tracker.set_value, TAU, ShowCreation(path), run_time=4, rate_func=bezier([0, 0, 1, 1]))
        self.wait()
    ...
    def get_sum_vectors(self, t, n, center=ORIGIN):
        ...
        for k in range(n + 1):
            ...
            vect.set_stroke(width=max(2, vect.get_stroke_width()))
    ...
    def get_paths(self):
        paths = VGroup(*[ParametricCurve(lambda t: plane.n2p(approx_exp(complex(0, t), n)), t_min=-PI, t_max=PI) for n in range(10)])
...
class ZetaSum(Scene):
    ...
    def construct(self):
        ...
        for k in range(1, N + 1, r):
            c = sum([L ** (-s) * (N - L + 1) / N for L in range(k, k + r)])
            ...
            lines.add(line)
        self.add(lines)
        self.add(Dot(lines[-1].get_end(), color=YELLOW), Dot(center_of_mass([line.get_end() for line in lines]), color=YELLOW))

class ZetaSpiral(Scene):

    def construct(self):
        max_t = 50
        spiral = VGroup(*[ParametricCurve(lambda t: complex_to_R3(zeta(complex(0.5, t))), t_min=t1, t_max=t2 + 0.1) for t1, t2 in zip(it.count(), range(1, max_t))])
        ...
        label = OldTex('\\zeta\\left(0.5 + i{t}\\right)', tex_to_color_map={'{t}': YELLOW}, background_stroke_width=0)
        label.scale(1.5)
...
class SumRotVectors(Scene):
    ...
    def construct(self):
        ...
        self.play(t_tracker.set_value, 1, run_time=10, rate_func=linear)
...
class Spirals(Scene):
    ...
    def construct(self):
        ...
        spiral = always_redraw(lambda: self.get_spiral(get_s()))
        s_dot = always_redraw(lambda: Dot(s_tracker.get_center(), color=YELLOW))
        s_label = always_redraw(lambda: DecimalNumber(get_s()).to_corner(UR))
        ...
        sigma = 0.5
        zero_ts = [14.134725, 21.02204, 25.010858, 30.424876, 32.935062, 37.586178]
        self.wait()
        self.play(s_tracker.set_value, complex(sigma, 1), run_time=3)
        self.wait()
        for zero_t in zero_ts:
            self.play(s_tracker.set_value, complex(sigma, zero_t), run_time=3)
    ...
    def get_spiral(self, s, colors=[RED, BLUE]):
        ...
        for n in range(1, n_lines + 1):
            ...
            line.set_stroke(colors.__next__(), width=3 * abs(z) ** 0.1)
            if len(lines) > 0:
                line.shift(lines[-1].get_end())
            lines.add(line)
        return lines
