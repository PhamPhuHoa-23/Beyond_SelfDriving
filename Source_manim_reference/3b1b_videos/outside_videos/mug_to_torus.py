...
class MugToTorus(ThreeDScene):
    ...
    def construct(self):
        ...
        left_half_torus = ParametricSurface(torus_func, u_range=(-PI / 2, PI + PI / 2), v_range=(0, TAU))
        right_half_torus = ParametricSurface(torus_func, u_range=(PI, TAU), v_range=(0, TAU))
        cylinder = ParametricSurface(cylinder_func, u_range=(PI, TAU), v_range=(0, TAU))
        ...
        self.play(MoveToTarget(disk), MoveToTarget(low_disk), Transform(cylinder, right_half_torus), Restore(left_half_torus, rate_func=squish_rate_func(smooth, 0, 0.75)), frame.animate.reorient(-10, 80), run_time=5)
        self.wait()