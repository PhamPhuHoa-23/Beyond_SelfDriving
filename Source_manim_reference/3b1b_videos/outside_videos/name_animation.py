...
class ComplexMorphingNames(Scene):
    CONFIG = {'patron_name': 'Janel', 'function': lambda z: 0.2 * z ** 3, 'default_apply_complex_function_kwargs': {'run_time': 5}, 'output_directory': os.path.join(get_output_dir(), 'EightDollarPatrons'), 'include_coordinate_labels': False, 'vert_start_color': YELLOW, 'vert_end_color': PINK, 'horiz_start_color': GREEN, 'horiz_end_color': BLUE, 'use_multicolored_plane': True}
    ...
    def construct(self):
        ...
        self.play(self.get_rotation(name_mob), run_time=5)
        self.wait()
        self.add_transformable_mobjects(name_mob)
        self.apply_complex_function(self.function)
        self.wait()
        self.play(self.get_post_transform_rotation(name_mob, name_mob_copy), run_time=10)
        self.wait(3)

    def get_rotation(self, name_mob):
        return UpdateFromAlphaFunc(name_mob, lambda mob, alpha: mob.move_to(rotate_vector(self.start_vect, 2 * np.pi * alpha)))
...
class FlowNameAnimation(Scene):
    ...
    def construct(self):
        ...
        vector_field.submobjects.sort(key=lambda a: a.get_length())
        ...
        self.wait(60)
        self.remove(flow)
        self.play(FadeOut(vector_field), LaggedStartMap(ApplyMethod, name_strokes, lambda m: (m.restore,), lag_ratio=0.2), run_time=5)
        self.wait()

class NameAnimationScene(Scene):
    CONFIG = {'animated_name': 'Test name', 'all_names': ['范英睿'], 'animate_all_names': True, 'linger_after_completion': False}
...
class RotatingNameLetters(NameAnimationScene):
    ...
    def construct(self):
        ...
        times_n_label = VGroup(OldTex('\\times'), Integer(1))
        times_n_label.arrange(RIGHT)
        times_n_label.shift(FRAME_WIDTH * RIGHT / 4)
        times_n_label.to_edge(UP)
        self.play(LaggedStartMap(FadeIn, name_mob, run_time=3), ApplyMethod(randy.change, 'pondering', pos2, run_time=1), FadeIn(times_n_label))
        for n in range(2, nb_letters + 2):
            ...
            self.play(AnimationGroup(*group, run_time=3), UpdateFromFunc(randy, lambda r: r.look_at(name_mob.submobjects[2])), FadeOut(times_n_label[1]), FadeIn(new_n))
        ...
        self.play(ReplacementTransform(name_mob, VGroup(*thank_you)), Transform(randy, new_randy))
...
class ModularMultiplicationNameAnimation(RotatingNameLetters):
    ...
    def construct(self):
        ...
        for char, theta in zip(circle_text, alphas):
            char.move_to(char_radius * np.array([np.sin(theta), np.cos(theta), 0]))
        index_mobs = VGroup()
        ...
        self.wait()
        self.play(Transform(text, circle_text), FadeIn(index_mobs), lag_ratio=0.2, run_time=3)
        self.wait()
        for k in range(2, N + 1):
            ...
            lines = VGroup(*[Line(index_mobs[i], index_mobs[i * k % N], buff=SMALL_BUFF).add_tip(0.1).set_opacity(0 if i * k % N == i else 1) for i in range(N)])
            ...
            kw = {'run_time': 5, 'lag_ratio': 0.5, 'rate_func': lambda t: smooth(t, 2)}
            self.play(MoveToTarget(text, **kw), ShowCreation(lines, **kw), FadeIn(label))
            self.wait()
            text_copy = text.copy()
            self.add(text_copy)
            self.remove(text)
            text.restore()
            self.play(FadeOut(lines), FadeOut(label), FadeOut(text_copy), FadeIn(text))
...
class QuaternionNameAnimation(Scene):
    ...
    def construct(self):
        ...
        axes = self.get_axes()
        self.play(Write(surface), Write(name))
        surface.add(name)
        self.wait()
        self.move_camera(phi=70 * DEGREES, theta=-140 * DEGREES, added_anims=[ApplyPointwiseFunction(self.plane_to_cylinder, surface), FadeIn(axes)], run_time=3)
        self.begin_ambient_camera_rotation(0.01)
        self.wait(2)
        self.play(ApplyPointwiseFunction(self.cylinder_to_sphere, surface), run_time=3)
        self.wait(2)
        for i in range(3):
            axis = np.zeros(3)
            axis[i] = 1
            self.play(Homotopy(self.get_quaternion_homotopy(axis), surface, run_time=10))
        self.wait(5)

    def plane_to_cylinder(self, p):
        x, y, z = p
        R = self.R + z
        return np.array([R * np.cos(x / self.R - 0), R * np.sin(x / self.R - 0), 1.0 * y])
