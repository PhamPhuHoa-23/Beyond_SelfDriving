...
class PodcastIntro(Scene):
    ...
    def construct(self):
        ...
        tuples = zip(rings, np.linspace(max_width, min_width, n_rings), np.linspace(max_opacity, min_opacity, n_rings))
        ...
        self.play(FadeIn(tower[1], scale=10, run_time=1), LaggedStart(*(Restore(ring, rate_func=linear) for ring in reversed(rings)), run_time=4, lag_ratio=0.08))
    ...
    def get_radio_tower(self):
        ...
        for a1, a2 in zip(alphas, alphas[1:]):
            base.add(Line(line1.pfp(a1), line2.pfp(a2)), Line(line2.pfp(a1), line1.pfp(a2)))
