...
class SupportPitch1(Scene):
    CONFIG = {'camera_config': {'background_opacity': 0.85}, 'mode1': 'happy', 'mode2': 'hooray', 'words1': 'So what do\\\\you do?', 'words2': 'Oh, I make\\\\videos about\\\\math.'}
    ...
    def construct(self):
        ...
        b1 = randy.get_bubble(self.words1, bubble_type=SpeechBubble, height=3, width=4)
        b1.add(b1.content)
        b1.shift(0.25 * UP)
        b2 = morty.get_bubble(self.words2, bubble_type=SpeechBubble, height=3, width=4)
        b2.add(b2.content)
        b2.shift(0.25 * DOWN)
        self.add(randy)
        self.add(morty)
...
class SupportPitch2(SupportPitch1):
    CONFIG = {'mode1': 'confused', 'mode2': 'speaking', 'words1': 'Wait, how does\\\\that work?', 'words2': 'People pay\\\\for them.'}

class SupportPitch3(SupportPitch1):
    CONFIG = {'mode1': 'hesitant', 'mode2': 'coin_flip_2', 'words1': 'Oh, so like\\\\a paid course?', 'words2': 'Well, no,\\\\everything\\\\is free.'}

class SupportPitch4(SupportPitch1):
    CONFIG = {'mode1': 'confused', 'mode2': 'hesitant', 'words1': 'Wait, what?', 'words2': "I know,\\\\it's weird..."}
