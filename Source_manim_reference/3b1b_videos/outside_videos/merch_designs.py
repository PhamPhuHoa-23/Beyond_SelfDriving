...
class ShowHilbertCurve(Scene):
    CONFIG = {'FractalClass': HilbertCurve, 'orders': [3, 5, 7], 'stroke_widths': [20, 15, 7]}

    def construct(self):
        curves = VGroup(*[self.FractalClass(order=order).scale(scale_factor) for order, scale_factor in zip(self.orders, np.linspace(1, 2, 3))])
...
class ShowFlowSnake(ShowHilbertCurve):
    CONFIG = {'FractalClass': FlowSnake, 'orders': [2, 3, 4], 'stroke_widths': [20, 15, 10]}
...
class ShowSierpinski(ShowHilbertCurve):
    CONFIG = {'FractalClass': FlippedSierpinski, 'orders': [3, 6, 9], 'stroke_widths': [20, 15, 6]}

class SquareWave(Scene):

    def construct(self):
        L = FRAME_WIDTH / 4
        waves = VGroup(*[FunctionGraph(lambda x: self.approx_square_wave(x, L, n_terms), x_range=[-2 * L, 2 * L, L / 50]) for n_terms in range(1, 5)])
...
class PendulumPhaseSpace(Scene):
    ...
    def construct(self):
        axes = Axes(x_min=-PI, x_max=PI, y_min=-2, y_max=2, x_axis_config={'tick_frequency': PI / 4, 'unit_size': FRAME_WIDTH / TAU, 'include_tip': False}, y_axis_config={'unit_size': 4, 'tick_frequency': 0.25})

        def func(point, mu=0.1, k=0.5):
            theta, theta_dot = axes.p2c(point)
            return axes.c2p(theta_dot, -mu * theta_dot - k * np.sin(theta))
        field = VectorField(func, delta_x=1.5 * FRAME_WIDTH / 12, delta_y=1.5, y_min=-6, y_max=6, length_func=lambda norm: 1.25 * sigmoid(norm), max_magnitude=4, vector_config={'tip_length': 0.75, 'max_tip_length_to_length_ratio': 0.35})
        ...
        line = VMobject()
        line.start_new_path(axes.c2p(-3 * TAU / 4, 1.75))
        dt = 0.1
        t = 0
        total_time = 60
        while t < total_time:
