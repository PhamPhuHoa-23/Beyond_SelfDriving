...
class FakeAreaManipulation(Scene):
    ...
    def construct(self):
        ...
        for target in (group2, group1.copy()):
            ...
            self.wait(2)
        lines = VGroup(Line(group1.get_top(), group1.get_corner(DR)), Line(group1.get_top(), group1.get_corner(DL)))
        lines.set_stroke(YELLOW, 2)
        frame = self.camera.frame
        frame.save_state()
        self.play(ShowCreation(lines, lag_ratio=0))
        self.play(frame.scale, 0.15, frame.move_to, group1[1].get_corner(DR), run_time=4)
        self.wait(3)
        self.play(Restore(frame, run_time=2))
        self.play(*[Transform(sm1, sm2, **kw) for i, sm1, sm2 in zip(it.count(), group1, group2)])
        self.play(frame.scale, 0.15, frame.move_to, group1[1].get_corner(UL), run_time=4)
        self.wait(2)
...
class LogarithmicSpiral(Scene):
    ...
    def construct(self):
        ...
        self.add(group)
        self.play(exp_n_tracker.set_value, 7, run_time=10, rate_func=linear)
        self.wait()
