...
class LinalgThumbnail(ThreeDScene):
    CONFIG = {'camera_config': {'anti_alias_width': 0}}
...
class BaselThumbnail(Scene):
    ...
    def construct(self):
        lake_radius = 6
        lake_center = ORIGIN
        lake = Circle(fill_color=BLUE, fill_opacity=0.0, radius=lake_radius, stroke_color=BLUE_D, stroke_width=3)
...
class Eola1Thumbnail(Scene):
    ...
    def construct(self):
        plane = NumberPlane(x_range=(-2, 2), y_range=(-5, 5))
        plane.set_width(FRAME_WIDTH / 3)
        plane.to_edge(LEFT, buff=0)
        plane.shift(1.5 * DOWN)
        vect = Arrow(plane.get_origin(), plane.c2p(1, 2), buff=0, thickness=0.1)
...
class ODEThumbnail(Scene):
    ...
    def construct(self):
        plane = NumberPlane()
        field = VectorField(pendulum_vector_field_func, plane, step_multiple=0.5, magnitude_range=(0, 5), length_func=lambda norm: 0.35 * sigmoid(norm))
...
class EGraph(InteractiveScene):
    ...
    def construct(self):
        ...
        arrow = Arrow(qmarks.get_corner(UR), equation['2.'].get_corner(DL), stroke_width=10, stroke_color=WHITE)
