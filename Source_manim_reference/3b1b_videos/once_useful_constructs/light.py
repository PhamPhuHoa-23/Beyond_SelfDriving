...
class Lighthouse(SVGMobject):
    CONFIG = {'height': LIGHTHOUSE_HEIGHT, 'fill_color': WHITE, 'fill_opacity': 1.0}
...
class AmbientLight(VMobject):
    CONFIG = {'source_point': VectorizedPoint(location=ORIGIN, stroke_width=0, fill_opacity=0), 'opacity_function': lambda r: 1.0 / (r + 1.0) ** 2, 'color': LIGHT_COLOR, 'max_opacity': 1.0, 'num_levels': NUM_LEVELS, 'radius': 5.0}
    ...
    def init_points(self):
        ...
        for r in np.arange(0, self.radius, dr):
            ...
            annulus = Annulus(inner_radius=r, outer_radius=r + dr, color=self.color, fill_opacity=alpha)
...
class Spotlight(VMobject):
    CONFIG = {'source_point': VectorizedPoint(location=ORIGIN, stroke_width=0, fill_opacity=0), 'opacity_function': lambda r: 1.0 / (r / 2 + 1.0) ** 2, 'color': GREEN, 'max_opacity': 1.0, 'num_levels': 10, 'radius': 10.0, 'screen': None, 'camera_mob': None}
    ...
    def new_sector(self, r, dr, lower_angle, upper_angle):
        alpha = self.max_opacity * self.opacity_function(r)
        annular_sector = AnnularSector(inner_radius=r, outer_radius=r + dr, color=self.color, fill_opacity=alpha, start_angle=lower_angle, angle=upper_angle - lower_angle)
...
class LightSource(VMobject):
    CONFIG = {'source_point': VectorizedPoint(location=ORIGIN, stroke_width=0, fill_opacity=0), 'color': LIGHT_COLOR, 'num_levels': 10, 'radius': 10.0, 'screen': None, 'opacity_function': inverse_quadratic(1, 2, 1), 'max_opacity_ambient': AMBIENT_FULL, 'max_opacity_spotlight': SPOTLIGHT_FULL, 'camera_mob': None}

    def init_points(self):
        self.add(self.source_point)
        self.lighthouse = Lighthouse()
        self.ambient_light = AmbientLight(source_point=VectorizedPoint(location=self.get_source_point()), color=self.color, num_levels=self.num_levels, radius=self.radius, opacity_function=self.opacity_function, max_opacity=self.max_opacity_ambient)
        if self.has_screen():
            self.spotlight = Spotlight(source_point=VectorizedPoint(location=self.get_source_point()), color=self.color, num_levels=self.num_levels, radius=self.radius, screen=self.screen, opacity_function=self.opacity_function, max_opacity=self.max_opacity_spotlight, camera_mob=self.camera_mob)
        else:
    ...
    def set_screen(self, new_screen):
        if self.has_screen():
            ...
            self.spotlight = Spotlight(source_point=VectorizedPoint(location=self.get_source_point()), color=self.color, num_levels=self.num_levels, radius=self.radius, screen=new_screen, camera_mob=self.camera_mob, opacity_function=self.opacity_function, max_opacity=self.max_opacity_spotlight)
            self.spotlight.move_source_to(self.get_source_point())
    ...
    def update_ambient(self):
        new_ambient_light = AmbientLight(source_point=VectorizedPoint(location=ORIGIN), color=self.color, num_levels=self.num_levels, radius=self.radius, opacity_function=self.opacity_function, max_opacity=self.max_opacity_ambient)
    ...
    def rotation_matrix(self):
        ...
        R2 = np.array([[np.cos(theta + TAU / 4), -np.sin(theta + TAU / 4), 0], [np.sin(theta + TAU / 4), np.cos(theta + TAU / 4), 0], [0, 0, 1]])
        R = np.dot(R2, R1)
        return R
    ...
    def update_shadow(self):
        ...
        projected_point_cloud_3d = np.append(projected_screen_points, np.reshape(projected_source, (1, 3)), axis=0)
        rotation_matrix = self.rotation_matrix()
        back_rotation_matrix = rotation_matrix.T
        rotated_point_cloud_3d = np.dot(projected_point_cloud_3d, back_rotation_matrix.T)
        point_cloud_2d = rotated_point_cloud_3d[:, :2]
        hull_2d = ConvexHull(point_cloud_2d)
        hull = []
        ...
        for point in point_cloud_2d[hull_2d.vertices]:
            if np.all(np.abs(point - source_point_2d) < 1e-06):
                source_index = index
                index += 1
                continue
