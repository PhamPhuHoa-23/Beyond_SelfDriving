...
class SEDTest(MovingCameraScene):
    ...
    def construct(self):
        ...
        average_over = 100
        hr_averages = np.array([np.mean(heart_rates[i:i + average_over]) for i in range(len(heart_rates) - average_over)])
        prop = 10
        ...
        axes = Axes(x_min=-1, x_max=12, y_min=0, y_max=130, y_axis_config={'unit_size': 1.0 / 25, 'tick_frequency': 10})
        axes.to_corner(UL)
        ...
        for p1, p2, p3, p4 in zip(points, points[1:], points[2:], points[3:]):
            ...
            line.set_color((point_to_color(p1), point_to_color(p2)))
        ...
        for mark in axes.x_axis.tick_marks:
            ...
            if mark.get_center()[0] < c2p(0, 0)[0]:
                mark.fade(1)
        times_and_words = [(launch_time - 60, 'T-minus\\\\1 hour'), (launch_time, 'Launch!'), (launch_time + 60, '1 hour \\\\ into flight')]
        time_labels = VGroup()
        ...
        time_line_pairs = [(tm4_time, tm4_line), (sep1_time, sep1_line), (sep2_time, sep2_line), (sep3_time, sep3_line)]
        ...
        self.play(ShowCreation(curr_line), rocket.to_edge, UP, UpdateFromAlphaFunc(rocket, lambda r, a: r.set_fill(opacity=min(2 - 2 * a, 1)), remover=True), run_time=2)
        for t, line in time_line_pairs:
            post_t_lines = get_lines_after_value(lines, t).copy()
            flicker = LaggedStartMap(UpdateFromAlphaFunc, post_t_lines, lambda mob: (mob, lambda l, a: l.set_stroke(width=interpolate(2, 10, a), opacity=interpolate(0.5, 1, a))), rate_func=there_and_back, run_time=1.5)
            self.play(flicker, lines.set_stroke, {'opacity': 0.5}, Transform(curr_line, line), FadeInFromDown(line.label), FadeOut(last_label, DOWN))
            for x in range(3):
                self.play(flicker)
