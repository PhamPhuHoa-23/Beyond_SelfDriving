...
class SampleSpaceScene(Scene):
    ...
    def get_division_change_animations(self, sample_space, parts, p_list, dimension=1, new_label_kwargs=None, **kwargs):
        ...
        if hasattr(parts, 'labels') and parts.labels is not None:
            ...
            anims += [Transform(parts.braces, new_braces), Transform(parts.labels, new_labels)]
        return anims

    def get_horizontal_division_change_animations(self, p_list, **kwargs):
        assert hasattr(self.sample_space, 'horizontal_parts')
        return self.get_division_change_animations(self.sample_space, self.sample_space.horizontal_parts, p_list, dimension=1, **kwargs)

    def get_vertical_division_change_animations(self, p_list, **kwargs):
        assert hasattr(self.sample_space, 'vertical_parts')
        return self.get_division_change_animations(self.sample_space, self.sample_space.vertical_parts, p_list, dimension=0, **kwargs)
    ...
    def get_conditional_change_anims(self, sub_sample_space_index, value, post_rects=None, **kwargs):
        ...
        anims = self.get_division_change_animations(sub_sample_space, sub_sample_space.vertical_parts, value, dimension=0, **kwargs)
