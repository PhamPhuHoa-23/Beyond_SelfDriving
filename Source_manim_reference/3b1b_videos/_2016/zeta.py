...
class ZetaTransformationScene(ComplexTransformationScene):
    CONFIG = {'anchor_density': 35, 'min_added_anchors': 10, 'max_added_anchors': 300, 'num_anchors_to_add_per_line': 75, 'post_transformation_stroke_width': 2, 'default_apply_complex_function_kwargs': {'run_time': 5}, 'x_min': 1, 'x_max': int(FRAME_X_RADIUS + 2), 'extra_lines_x_min': -2, 'extra_lines_x_max': 4, 'extra_lines_y_min': -2, 'extra_lines_y_max': 2}
    ...
    def prepare_for_transformation(self, mob):
        for line in mob.family_members_with_points():
            ...
            target_num_curves = np.clip(int(self.anchor_density * np.pi * diameter), self.min_added_anchors, self.max_added_anchors)
    ...
    def get_dense_grid(self, step_size=1.0 / 16):
        epsilon = 0.1
        x_range = np.arange(max(self.x_min, self.extra_lines_x_min), min(self.x_max, self.extra_lines_x_max), step_size)
        y_range = np.arange(max(self.y_min, self.extra_lines_y_min), min(self.y_max, self.extra_lines_y_max), step_size)
        vert_lines = VGroup(*[Line(self.y_min * UP, self.y_max * UP).shift(x * RIGHT) for x in x_range if abs(x - 1) > epsilon])
        vert_lines.set_color_by_gradient(self.vert_start_color, self.vert_end_color)
        horiz_lines = VGroup(*[Line(self.x_min * RIGHT, self.x_max * RIGHT).shift(y * UP) for y in y_range if abs(y) > epsilon])
        horiz_lines.set_color_by_gradient(self.horiz_start_color, self.horiz_end_color)
...
class TestZetaOnHalfPlane(ZetaTransformationScene):
    CONFIG = {'anchor_density': 15}

    def construct(self):
        self.add_transformable_plane()
        self.add_extra_plane_lines_for_zeta()
        self.prepare_for_transformation(self.plane)
        print(sum([mob.get_num_points() for mob in self.plane.family_members_with_points()]))
...
class IntroduceZeta(ZetaTransformationScene):
    CONFIG = {'default_apply_complex_function_kwargs': {'run_time': 8}}

    def construct(self):
        title = OldTexText('Riemann zeta function')
        title.add_background_rectangle()
        title.to_corner(UP + LEFT)
        func_mob = VGroup(OldTex('\\zeta(s) = '), OldTex('\\sum_{n=1}^\\infty \\frac{1}{n^s}'))
        ...
        self.play(randy.change_mode, 'confused', randy.look_at, func_mob)
...
class WhyPeopleMayKnowIt(TeacherStudentsScene):
    ...
    def construct(self):
        title = OldTexText('Riemann zeta function')
        title.to_corner(UP + LEFT)
        func_mob = OldTex('\\zeta(s) = \\sum_{n=1}^\\infty \\frac{1}{n^s}')
        func_mob.next_to(title, DOWN, aligned_edge=LEFT)
        self.add(title, func_mob)
        mercenary_thought = VGroup(OldTex('\\$1{,}000{,}000').set_color_by_gradient(GREEN_B, GREEN_D), OldTex('\\zeta(s) = 0'))
        mercenary_thought.arrange(DOWN)
        divergent_sum = VGroup(OldTex('1+2+3+4+\\cdots = -\\frac{1}{12}'), OldTex('\\zeta(-1) = -\\frac{1}{12}'))
        divergent_sum.arrange(DOWN)
        divergent_sum[0].set_color_by_gradient(YELLOW, MAROON_B)
        divergent_sum[1].set_color(BLACK)
        self.play(*it.chain(*[[pi.change_mode, 'pondering', pi.look_at, func_mob] for pi in self.get_pi_creatures()]))
        self.random_blink()
        self.student_thinks(mercenary_thought, index=2, target_mode='surprised')
        student = self.get_students()[2]
        self.random_blink()
        self.wait(2)
        self.student_thinks(divergent_sum, index=1, added_anims=[student.change_mode, 'plain'])
        student = self.get_students()[1]
        self.play(student.change_mode, 'confused', student.look_at, divergent_sum)
        self.random_blink()
        self.play(*it.chain(*[[pi.change_mode, 'confused', pi.look_at, divergent_sum] for pi in self.get_pi_creatures()]))
        self.wait()
        self.random_blink()
        ...
        self.random_blink()
        self.wait()
        self.student_says(OldTexText('Can you explain \\\\', "``analytic continuation''?"), index=1, target_mode='raise_right_hand')
        self.play_student_changes('raise_left_hand', 'raise_right_hand', 'raise_left_hand')
        self.play(self.get_teacher().change_mode, 'happy', self.get_teacher().look_at, student.eyes)
        self.random_blink()
        self.wait(2)
        self.random_blink()
        self.wait()
...
class ComplexValuedFunctions(ComplexTransformationScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(z_out), Write(f_z))
        self.wait(2)

class PreviewZetaAndContinuation(ZetaTransformationScene):
    CONFIG = {'default_apply_complex_function_kwargs': {'run_time': 4}}

    def construct(self):
        self.add_transformable_plane()
        self.add_extra_plane_lines_for_zeta()
        reflected_plane = self.get_reflected_plane()
        titles = [OldTexText('What does', '%s' % s, 'look like?', alignment='') for s in ['$\\displaystyle \\sum_{n=1}^\\infty \\frac{1}{n^s}$', 'analytic continuation']]
        for mob in titles:
...
class AssumeKnowledgeOfComplexNumbers(ComplexTransformationScene):
    ...
    def construct(self):
        ...
        complex_number_label = OldTex('%d+%di' % (int(z.real), int(z.imag)))
        ...
        text = VGroup(OldTexText('Assumed knowledge:'), OldTexText('1) What complex numbers are.'), OldTexText('2) How to work with them.'), OldTexText('3) Maybe derivatives?'))
        ...
        self.play(ShowCreation(x_line), ShowCreation(y_line), ShowCreation(VGroup(line, dot)), Write(complex_number_label))
...
class DefineForRealS(PiCreatureScene):
    ...
    def initial_definition(self, zeta_def):
        ...
        self.blink()
        pre_zeta_s = VGroup(*self.pi_creature.bubble.content.copy()[-4:])
        pre_zeta_s.add(VectorizedPoint(pre_zeta_s.get_right()))
        self.play(Transform(pre_zeta_s, zeta_s), *self.get_bubble_fade_anims())
        ...
        for count, term in enumerate(sum_terms):
            ...
            if count % 2 == 0:
                self.wait()
        self.play(GrowFromCenter(brace), Write(sigma), self.pi_creature.change_mode, 'pondering')
        self.wait()

    def plug_in_two(self, zeta_def):
        two_def = self.get_definition('2')[0]
        number_line = NumberLine(x_min=0, x_max=3, tick_frequency=0.25, big_tick_numbers=list(range(4)), unit_size=3)
        number_line.add_numbers()
        number_line.next_to(self.pi_creature, LEFT)
        number_line.to_edge(LEFT)
        self.number_line = number_line
        lines, braces, dots, pi_dot = self.get_sum_lines(2)
        fracs = VGroup(*[OldTex('\\frac{1}{%d}' % (d + 1) ** 2).scale(0.7) for d, brace in enumerate(braces)])
        for frac, brace, line in zip(fracs, braces, lines):
            ...
            if frac is fracs[-1]:
                ...
                arrow = Arrow(frac.get_bottom(), brace.get_top(), tip_length=0.1, buff=0.1)
                arrow.set_color(line.get_color())
                frac.add(arrow)
        pi_term = OldTex('= \\frac{\\pi^2}{6}')
        pi_term.next_to(zeta_def[1], RIGHT)
        pi_arrow = Arrow(pi_term[-1].get_bottom(), pi_dot, color=pi_dot.get_color())
        ...
        for frac, brace, line in zip(fracs, braces, lines):
            self.play(Write(frac), GrowFromCenter(brace), ShowCreation(line), run_time=0.7)
            self.wait(0.7)
        self.wait()
        self.play(ShowCreation(VGroup(*lines[4:])), Write(dots))
        self.wait()
        self.play(Write(pi_term), ShowCreation(VGroup(pi_arrow, pi_dot)), self.pi_creature.change_mode, 'hooray')
        self.wait()
        self.play(Write(approx), self.pi_creature.change_mode, 'happy')
        self.wait(3)
        self.play(*list(map(FadeOut, [fracs, pi_arrow, pi_dot, approx])))
        self.lines = lines
        self.braces = braces
        self.dots = dots
    ...
    def plug_in_three_and_four(self, zeta_def):
        ...
        self.wait()
        self.play(ShowCreation(arrow), Write(smaller_words))
    ...
    def plug_in_negative_values(self, zeta_def):
        ...
        self.play(Transform(self.smaller_words, bigger_words), self.pi_creature.change_mode, 'confused')
        new_sum_terms = OldTex(list('1+2+3+4+') + ['\\cdots'])
        ...
        self.play(Transform(sum_terms, new_sum_terms), Transform(brace, new_brace), sigma.next_to, new_brace, UP, MoveToTarget(arrow), Transform(smaller_words, bigger_words), self.final_sum.next_to, new_sum_terms, RIGHT)
        self.wait(3)
        new_sum_terms = OldTex(list('1+4+9+16+') + ['\\cdots'])
        new_sum_terms.move_to(sum_terms, LEFT)
        ...
        self.play(Transform(zeta_s, zeta_minus_two), Transform(sum_terms, new_sum_terms), Transform(sigma, new_sigma), Transform(self.final_sum, new_final_sum), Transform(self.lines, lines), Transform(self.braces, braces))
        self.wait()
    ...
    def get_definition(self, input_string, input_color=YELLOW):
        ...
        sum_terms = OldTex(*it.chain(*list(zip(['\\frac{1}{%d^{%s}}' % (d, input_string) for d in range(1, 1 + num_shown_terms)], it.cycle(['+'])))))
        ...
        sigma = OldTex('\\sum_{n=1}^\\infty \\frac{1}{n^{%s}}' % input_string)
    ...
    def get_sum_lines(self, exponent, line_thickness=6):
        ...
        lines = VGroup(*[Line(self.number_line.number_to_point(s1), self.number_line.number_to_point(s2)) for s1, s2 in zip(power_sums, power_sums[1:])])
        lines.set_stroke(width=line_thickness)
        VGroup(*lines[::2]).set_color(MAROON_B)
        VGroup(*lines[1::2]).set_color(RED)
        braces = VGroup(*[Brace(line, UP) for line in lines[:4]])
        dots = OldTex('...')
        dots.stretch_to_fit_width(0.8 * VGroup(*lines[4:]).get_width())
        dots.next_to(braces, RIGHT, buff=SMALL_BUFF)
        final_dot = Dot(self.number_line.number_to_point(power_sums[-1]), color=GREEN_B)
    ...
    def transition_to_new_input(self, zeta_def, exponent, final_sum):
        ...
        self.play(Transform(zeta_def, new_zeta_def), Transform(self.lines, lines), Transform(self.braces, braces), Transform(self.dots, dots), Transform(self.final_dot, final_dot), Transform(self.final_sum, final_sum), self.pi_creature.change_mode, 'pondering')

class ReadIntoZetaFunction(Scene):
    CONFIG = {'statement': '$\\zeta(-1) = -\\frac{1}{12}$', 'target_mode': 'frustrated'}
    ...
    def construct(self):
        ...
        paper.add(title)
        paragraph_lines = VGroup(Line(LEFT, RIGHT), Line(LEFT, RIGHT).shift(0.2 * DOWN), Line(LEFT, ORIGIN).shift(0.4 * DOWN))
...
class IgnoreNegatives(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        words = OldTexText('Ignore $s \\le 1$ \\dots \\\\', 'For now.')
        words[0][6].set_color(YELLOW)
        words[1].set_color(BLACK)
        self.teacher_says(words)
        self.play(words[1].set_color, WHITE)
        self.play_student_changes(*['happy'] * 3)
        self.play(GrowFromCenter(brace), Write(only_s_gt_1), *it.chain(*[[pi.look_at, definition] for pi in self.get_pi_creatures()]))
        self.random_blink(3)

class RiemannFatherOfComplex(ComplexTransformationScene):

    def construct(self):
        name = OldTexText('Bernhard Riemann $\\rightarrow$ Complex analysis')
        ...
        output_dot = Dot(arc.get_points()[-1] + SMALL_BUFF * (2 * RIGHT + DOWN), color=MAROON_B)
        ...
        self.play(ShowCreation(input_dot), Write(input_dot.label))
        self.play(ShowCreation(arc))
        self.play(ShowCreation(output_dot), Write(output_dot.label))
        self.wait()

class FromRealToComplex(ComplexTransformationScene):
    CONFIG = {'plane_config': {'space_unit_to_x_unit': 2, 'space_unit_to_y_unit': 2}, 'background_label_scale_val': 0.7, 'output_color': GREEN_B, 'num_lines_in_spiril_sum': 1000}
    ...
    def show_real_to_real(self):
        ...
        number_line = NumberLine(unit_size=2, tick_frequency=0.5, big_tick_numbers=list(range(-2, 3)))
        number_line.add_numbers()
        ...
        arc = Arc(2 * np.pi / 3, start_angle=np.pi / 6)
        arc.stretch_to_fit_width((input_dot.get_center() - output_dot.get_center())[0])
        ...
        self.add(sum_term)
        self.wait(2)
        self.play(ShowCreation(self.background, run_time=2), FadeOut(VGroup(arc, output_dot, number_line)), Animation(zeta), Animation(input_dot))
        self.wait(2)
        self.zeta = zeta
    ...
    def transition_to_complex(self):
        ...
        input_dot.target.move_to(self.background.num_pair_to_point((2, 1)))
        ...
        self.wait()
        self.play(input_label.restore, MoveToTarget(input_dot))
        self.wait(2)
    ...
    def single_out_complex_exponent(self):
        ...
        frac = VGroup(VectorizedPoint(self.zeta[2][3].get_left()), self.zeta[2][3], VectorizedPoint(self.zeta[2][3].get_right()), self.zeta[2][4]).copy()
        frac.generate_target()
        frac.target.scale(frac_scale_factor)
        bubble.add_content(frac.target)
        new_frac = OldTex('\\Big(', '\\frac{1}{2}', '\\Big)', '^{2+i}')
        ...
        new_words.scale(1.3)
        self.play(FadeIn(randy))
        self.play(randy.change_mode, 'confused', randy.look_at, bubble, ShowCreation(bubble), MoveToTarget(frac))
        self.play(Blink(randy))
        ...
        for x in range(2):
            self.wait(2)
            self.play(Blink(randy))
        self.play(Transform(words, new_words), randy.change_mode, 'maybe')
        self.wait()
        self.play(Blink(randy))
    ...
    def show_s_equals_two_lines(self):
        ...
        def transform_and_replace(m1, m2):
            ...
            self.remove(m1)
            self.add(m2)
        self.play(self.input_dot.shift, 2 * DOWN, self.input_label.fade, 0.7)
    ...
    def transition_to_spiril_sum(self):
        ...
        self.play(self.input_dot.shift, 2 * UP, self.input_label.restore)
        self.wait()
        self.play(Transform(self.zeta, zeta))
        self.wait()
        self.play(Transform(self.lines, lines), Transform(self.output_dot, output_dot), run_time=2, path_arc=-np.pi / 6)
        self.wait()
    ...
    def vary_complex_input(self):
        ...
        self.wait(2)
        inputs = [complex(1.5, 1.8), complex(1.5, -1), complex(3, -1), complex(1.5, 1.8), complex(1.5, -1.8), complex(1.4, -1.8), complex(1.5, 0), complex(2, 1)]
        for s in inputs:
            input_point = self.z_to_point(s)
            lines, output_dot = self.get_sum_lines(s)
            self.play(self.input_dot.move_to, input_point, Transform(self.lines, lines), Transform(self.output_dot, output_dot), run_time=2)
            self.wait()
        self.wait()

    def show_domain_of_convergence(self, opacity=0.2):
        domain = Rectangle(width=FRAME_X_RADIUS - 2, height=FRAME_HEIGHT, stroke_width=0, fill_color=YELLOW, fill_opacity=opacity)
        domain.to_edge(RIGHT, buff=0)
        anti_domain = Rectangle(width=FRAME_X_RADIUS + 2, height=FRAME_HEIGHT, stroke_width=0, fill_color=RED, fill_opacity=opacity)
        ...
        self.wait(2)
        self.play(*list(map(FadeOut, [anti_domain, anti_domain_words])))
    ...
    def ask_about_visualizing_all(self):
        ...
        self.wait(3)
        self.play(morty.change_mode, 'pondering', morty.look_at, self.input_dot, *list(map(FadeOut, [bubble, bubble.content, self.domain_words])))
    ...
    def get_zeta_definition(self, input_string, output_string, input_color=YELLOW):
        ...
        zeta_s_eq.shift(0.5 * UP)
        inputs.add(*zeta_s_eq[2:2 + n_input_chars])
        raw_sum_terms = OldTex(*['\\frac{1}{%d^{%s}} + ' % (d, input_string) for d in range(1, 1 + num_shown_terms)])
        sum_terms = VGroup(*it.chain(*[[VGroup(*term[:3]), VGroup(*term[3:-1]), term[-1]] for term in raw_sum_terms]))
    ...
    def get_sum_lines(self, exponent, line_thickness=6):
        powers = [0] + [x ** (-exponent) for x in range(1, self.num_lines_in_spiril_sum)]
        power_sums = np.cumsum(powers)
        lines = VGroup(*[Line(*list(map(self.z_to_point, z_pair))) for z_pair in zip(power_sums, power_sums[1:])])
        widths = np.linspace(line_thickness, 0, len(list(lines)))
        for line, width in zip(lines, widths):
            line.set_stroke(width=width)
        VGroup(*lines[::2]).set_color(MAROON_B)
        VGroup(*lines[1::2]).set_color(RED)
        final_dot = Dot(self.z_to_point(zeta(exponent)), color=self.output_color)
...
class TerritoryOfExponents(ComplexTransformationScene):
    ...
    def construct(self):
        ...
        full_plane = Rectangle(width=FRAME_WIDTH, height=FRAME_HEIGHT, fill_color=YELLOW, fill_opacity=0.3)
        self.add(familiar_territory)
        self.play(ShowCreation(arrow1))
        self.play(Transform(arrow1, arrow2), ShowCreation(real_line))
        self.play(FadeOut(arrow1))
        self.play(FadeIn(full_plane), Transform(familiar_territory, extended_realm), Animation(real_line))

    def add_title(self):
        exponent = OldTex('\\left(\\frac{1}{2}\\right)^s')
...
class ComplexExponentiation(Scene):
    ...
    def extract_pure_imaginary_part(self):
        original = OldTex('\\left(\\frac{1}{2}\\right)', '^{2+i}')
        split = OldTex('\\left(\\frac{1}{2}\\right)', '^{2}', '\\left(\\frac{1}{2}\\right)', '^{i}')
        ...
        we_understand = brace.get_text('We understand this')
        VGroup(brace, we_understand).set_color(GREEN_B)
        self.add(original)
        self.wait()
        self.play(*[Transform(*pair) for pair in [(original[0], split[0]), (original[1][0], split[1]), (original[0].copy(), split[2]), (VGroup(*original[1][1:]), split[3])]])
        self.remove(*self.get_mobjects_from_last_animation())
        self.add(real_part, imag_part)
        self.wait()
        self.play(GrowFromCenter(brace), FadeIn(we_understand), real_part.set_color, GREEN_B)
        self.wait()
        self.play(imag_part.move_to, imag_part.get_left(), *list(map(FadeOut, [brace, we_understand, real_part])))
        self.wait()
    ...
    def add_on_planes(self):
        ...
        for plane in (left_plane, right_plane):
            ...
            for label in labels:
                label.scale(0.5)
                label.next_to(label.get_center(), DOWN + RIGHT, buff=SMALL_BUFF)
            plane.add(labels)
        arrow = Arrow(LEFT, RIGHT)
        self.play(ShowCreation(left_plane), Write(left_title), run_time=3)
        self.play(ShowCreation(right_plane), Write(right_title), run_time=3)
        self.play(ShowCreation(arrow))
        self.wait()
    ...
    def show_imaginary_powers(self):
        ...
        def output_dot_update(ouput_dot):
            ...
            output_dot.move_to(self.z_to_point(curr_base ** complex(0, y), is_input=False))
            return output_dot

        def walk_up_and_down():
            for vect in (3 * DOWN, 5 * UP, 5 * DOWN, 2 * UP):
                self.play(input_dot.shift, vect, UpdateFromFunc(output_dot, output_dot_update), run_time=3)
...
class SizeAndRotationBreakdown(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(*[Transform(*pair) for pair in [(original[0], split[0]), (original[1][0], split[1]), (original[0].copy(), split[2]), (VGroup(*original[1][1:]), split[3])]])
        self.play(GrowFromCenter(size_brace), Write(size))
        self.play(GrowFromCenter(rotation_brace), Write(rotation))
        self.wait()
...
class SeeLinksInDescription(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(*it.chain(*[[pi.change_mode, 'hooray', pi.look, DOWN] for pi in self.get_students()]))
        self.random_blink(3)
...
class ShowMultiplicationOfRealAndImaginaryExponentialParts(FromRealToComplex):
    ...
    def show_multiplication(self):
        ...
        full_power_tex = OldTex('\\left(\\frac{1}{2}\\right)', '^{2+i}')
        full_power_tex[-1].set_color(YELLOW)
        full_power_tex.add_background_rectangle()
        full_power_tex.scale(0.7)
        full_power_tex.next_to(0.5 * self.z_to_point(0.5 ** complex(2, 1)), UP + RIGHT)
        self.play(real_part.scale, 0.7, real_part.next_to, brace, UP, SMALL_BUFF, LEFT, ShowCreation(fourth_dot))
        self.play(GrowFromCenter(brace), ShowCreation(fourth_line))
        self.wait()
        self.play(imag_part.scale, 0.7, imag_part.next_to, imag_power_dot, DOWN + RIGHT, SMALL_BUFF, ShowCreation(imag_power_dot))
        self.play(ShowCreation(circle), Animation(imag_power_dot))
        self.play(ShowCreation(imag_power_line))
        self.wait(2)
        self.play(fourth_group.rotate, imag_power_line.get_angle())
...
class ComplexFunctionsAsTransformations(ComplexTransformationScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(input_dots, run_time=2, lag_ratio=0.5))
        for in_dot, out_dot, arrow in zip(input_dots, output_dots, arrows):
            self.play(Transform(in_dot.copy(), out_dot), ShowCreation(arrow))
            self.wait()
        self.wait()
    ...
    def get_dots(self):
        input_points = [RIGHT + 2 * UP, 4 * RIGHT + DOWN, 2 * LEFT + 2 * UP, LEFT + DOWN, 6 * LEFT + DOWN]
        output_nudges = [DOWN + RIGHT, 2 * UP + RIGHT, 2 * RIGHT + 2 * DOWN, 2 * RIGHT + DOWN, RIGHT + 2 * UP]
        input_dots = VGroup(*list(map(Dot, input_points)))
        input_dots.set_color(YELLOW)
        output_dots = VGroup(*[Dot(ip + on) for ip, on in zip(input_points, output_nudges)])
        output_dots.set_color(MAROON_B)
        arrows = VGroup(*[Arrow(in_dot, out_dot, buff=0.1, color=WHITE) for in_dot, out_dot in zip(input_dots, output_dots)])
...
class VisualizingSSquared(ComplexTransformationScene):
    CONFIG = {'num_anchors_to_add_per_line': 100, 'horiz_end_color': GOLD, 'y_min': 0}
    ...
    def plug_in_specific_values(self):
        ...
        input_dots = VGroup(*[Dot(self.z_to_point(z), color=YELLOW) for z in inputs])
        output_dots = VGroup(*[Dot(self.z_to_point(z ** 2), color=BLUE) for z in inputs])
        arrows = VGroup()
        VGroup(*[ParametricCurve(lambda t: self.z_to_point(z ** (1.1 + 0.8 * t))) for z in inputs])
        for z, dot in zip(inputs, input_dots):
            path = ParametricCurve(lambda t: self.z_to_point(z ** (1 + t)))
            dot.path = path
            arrow = ParametricCurve(lambda t: self.z_to_point(z ** (1.1 + 0.8 * t)))
            stand_in_arrow = Arrow(arrow.get_points()[-2], arrow.get_points()[-1], tip_length=0.2)
    ...
    def add_transformable_plane(self, **kwargs):
        ...
        self.plane.add(Line(ORIGIN, FRAME_X_RADIUS * RIGHT, color=self.horiz_end_color), Line(ORIGIN, FRAME_X_RADIUS * LEFT, color=self.horiz_end_color))
        self.add(self.plane)
    ...
    def show_transformation(self):
        ...
        self.wait()
        self.apply_complex_homotopy(lambda z, t: z ** (1 + t), added_anims=[MoveAlongPath(dot, dot.path, run_time=5) for dot in self.input_dots], run_time=5)
        self.wait(2)
...
class ShowZetaOnHalfPlane(ZetaTransformationScene):
    CONFIG = {'x_min': 1, 'x_max': int(FRAME_X_RADIUS + 2)}
    ...
    def add_title(self):
        zeta = OldTex('\\zeta(', 's', ')=', *['\\frac{1}{%d^s} + ' % d for d in range(1, 5)] + ['\\cdots'])
    ...
    def react_to_transformation(self):
        ...
        bubble.pin_to(morty)
        self.play(FadeIn(morty))
        self.play(morty.change_mode, 'surprised', ShowCreation(bubble), Write(bubble.content))
        self.play(Blink(morty))
        self.play(morty.look_at, self.plane.get_top())
        self.wait()
        self.play(morty.look_at, self.plane.get_bottom(), *list(map(FadeOut, [bubble, bubble.content])))
        self.play(Blink(morty))
    ...
    def show_cutoff(self):
        ...
        words.shift(LEFT + UP)
        line = Line(*list(map(self.z_to_point, [complex(np.euler_gamma, u * FRAME_Y_RADIUS) for u in (1, -1)])))
        line.set_color(YELLOW)
        arrows = [Arrow(words.get_right(), point) for point in line.get_start_and_end()]
        self.play(Write(words, run_time=2))
        self.play(ShowCreation(arrows[0]))
        self.play(Transform(*arrows), ShowCreation(line), run_time=2)
        self.play(FadeOut(arrows[0]))
        self.wait(2)
        self.play(*list(map(FadeOut, [words, line])))

    def set_color_i_line(self):
        right_i_lines, left_i_lines = [VGroup(*[Line(vert_vect + RIGHT, vert_vect + (FRAME_X_RADIUS + 1) * horiz_vect) for vert_vect in (UP, DOWN)]) for horiz_vect in (RIGHT, LEFT)]
        ...
        self.play(self.plane.set_stroke, GREY, 1)
    ...
    def show_continuation(self):
        ...
        self.apply_zeta_function()
        self.wait(2)
        self.play(ShowCreation(reflected_plane, run_time=6, rate_func=lambda t: 1 - there_and_back(t)))
        self.wait(2)
    ...
    def restore_mobjects(self, *mobjects):
        self.play(*it.chain(*[[m.restore, m.make_smooth] for m in mobjects]), run_time=2)
        for m in mobjects:
            self.remove(m)
            m.restore()
            self.add(m)
...
class ShowConditionalDefinition(Scene):
    ...
    def construct(self):
        ...
        conditions = VGroup(*[OldTexText('if Re$(s) %s 1$' % s) for s in ('>', '\\le')])
        ...
        self.add(zeta, sigma)
        self.wait()
        self.play(sigma.restore, GrowFromCenter(brace), FadeIn(something_else))
        ...
        self.play(GrowFromCenter(underbrace), Write(question), something_else.set_color, GREEN_B)
        self.wait(2)
...
class SquiggleOnExtensions(ZetaTransformationScene):
    ...
    def show_negative_one(self):
        ...
        self.play(ShowCreation(self.plane, run_time=2), Animation(dot), run_time=2)
        self.remove(thin_plane)
        self.apply_zeta_function(added_anims=[ApplyMethod(dot.move_to, self.z_to_point(-1.0 / 12), run_time=5)])
        dot_to_remove = self.note_point(-1.0 / 12, '-\\frac{1}{12}')
    ...
    def cycle_through_options(self):
        ...
        def shear(point):
            x, y, z = point
            return np.array([x, y + 0.25 * (1 - x) ** 2, 0])
        ...
        def alt_mixed_scalar_func(point):
            x, y, z = point
            scalar = 1 + (gamma - x) / (gamma + FRAME_X_RADIUS)
            return np.array([scalar ** 5 * x, scalar ** 2 * y, 0])

        def sinusoidal_func(point):
            x, y, z = point
            freq = np.pi / gamma
            return np.array([x - 0.2 * np.sin(x * freq) * np.sin(y), y - 0.2 * np.sin(x * freq) * np.sin(y), 0])
        funcs = [shear, mixed_scalar_func, alt_mixed_scalar_func, sinusoidal_func]
        for mob in self.left_plane.family_members_with_points():
            if np.all(np.abs(mob.get_points()[:, 1]) < 0.1):
                self.left_plane.remove(mob)
        new_left_planes = [self.left_plane.copy().apply_function(func) for func in funcs]
        new_dots = [self.dot.copy().move_to(func(self.dot.get_center())) for func in funcs]
        self.left_plane.save_state()
        for plane, dot in zip(new_left_planes, new_dots):
            self.play(Transform(self.left_plane, plane), Transform(self.dot, dot), run_time=3)
            self.wait()
        self.play(FadeOut(self.dot))
        self.wait()
        self.play(FadeOut(self.left_plane))
        self.play(ShowCreation(self.left_plane, run_time=5, rate_func=linear))
        self.wait()

    def lock_into_place(self):
        words = OldTexText('Only one extension\n            has a ', '\\emph{derivative}', 'everywhere', alignment='')
...
class IntroduceAnglePreservation(VisualizingSSquared):
    CONFIG = {'num_anchors_to_add_per_line': 50, 'use_homotopy': True}
    ...
    def talk_about_derivative(self):
        ...
        self.play(randy.change_mode, 'speaking', ShowCreation(randy.bubble), Write(randy.bubble.content))
        self.play(Blink(morty))
        self.wait()
        self.play(morty.change_mode, 'speaking', randy.change_mode, 'pondering', ShowCreation(morty.bubble), Write(morty.bubble.content))
        self.play(Blink(randy))
        self.wait()
        self.play(*list(map(FadeOut, [randy, morty, randy.bubble, randy.bubble.content, morty.bubble, morty.bubble.content])))

    def cycle_through_line_pairs(self):
        line_pairs = [(Line(3 * DOWN + 3 * RIGHT, 2 * UP), Line(DOWN + RIGHT, 3 * UP + 4 * RIGHT)), (Line(RIGHT + 3.5 * DOWN, RIGHT + 2.5 * UP), Line(3 * LEFT + 0.5 * UP, 3 * RIGHT + 0.5 * UP)), (Line(4 * RIGHT + 4 * DOWN, RIGHT + 2 * UP), Line(4 * DOWN + RIGHT, 2 * UP + 2 * RIGHT))]
        for lines in line_pairs:
            self.show_angle_preservation_between_lines(*lines)
            self.reset()

    def note_grid_lines(self):
        intersection_inputs = [complex(x, y) for x in np.arange(-5, 5, 0.5) for y in np.arange(0, 3, 0.5) if not (x <= 0 and y == 0)]
        brackets = VGroup(*list(map(self.get_right_angle_bracket, intersection_inputs)))
        self.apply_function()
        self.wait()
        self.play(ShowCreation(brackets, run_time=5), Animation(self.plane))
        self.wait()
    ...
    def apply_function(self, **kwargs):
        if self.use_homotopy:
            self.apply_complex_homotopy(lambda z, t: z ** (1 + t), run_time=5, **kwargs)
        else:
            self.apply_complex_function(lambda z: z ** 2, **kwargs)

    def show_angle_preservation_between_lines(self, *lines):
        R2_endpoints = [[l.get_start()[:2], l.get_end()[:2]] for l in lines]
    ...
    def get_right_angle_bracket(self, input_z):
        ...
        brackets = VGroup(Line(RIGHT, RIGHT + UP), Line(RIGHT + UP, UP))
        brackets.scale(0.15)
        brackets.set_stroke(width=2)
        brackets.set_color(YELLOW)
        brackets.shift(0.02 * UP)
...
class AngleAtZeroDerivativePoints(IntroduceAnglePreservation):
    ...
    def construct(self):
        ...
        self.show_angle_preservation_between_lines(line, line.copy().rotate(np.pi / 5))
        self.wait()
    ...
    def put_angle_tex_next_to_arc(self, angle_tex, arc):
        IntroduceAnglePreservation.put_angle_tex_next_to_arc(self, angle_tex, arc)
...
class AnglePreservationAtAnyPairOfPoints(IntroduceAnglePreservation):
    ...
    def construct(self):
        ...
        for target_pair in line_pairs[1:]:
            self.play(Transform(line_pair, target_pair, run_time=2, path_arc=np.pi))
            self.wait()
        self.show_angle_preservation_between_lines(*line_pair)
        self.show_example_analytic_functions()

    def get_line_pairs(self):
        return list(it.starmap(VGroup, [(Line(3 * DOWN, 3 * LEFT + 2 * UP), Line(2 * LEFT + DOWN, 3 * UP + RIGHT)), (Line(2 * RIGHT + DOWN, 3 * LEFT + 2 * UP), Line(LEFT + 3 * DOWN, 4 * RIGHT + 3 * UP)), (Line(LEFT + 3 * DOWN, LEFT + 3 * UP), Line(5 * LEFT + UP, 3 * RIGHT + UP)), (Line(4 * RIGHT + 3 * DOWN, RIGHT + 2 * UP), Line(3 * DOWN + RIGHT, 2 * UP + 2 * RIGHT))]))
    ...
    def show_example_analytic_functions(self):
        ...
        functions = OldTexText('$e^x$, ', '$\\sin(x)$, ', 'any polynomial, $\\log(x)$, ', '\\dots')
...
class NoteZetaFunctionAnalyticOnRightHalf(ZetaTransformationScene):
    ...
    def add_title(self):
        title = OldTex('\\zeta(s) = \\sum_{n=1}^\\infty \\frac{1}{n^s}')
    ...
    def note_right_angles(self):
        intersection_inputs = [complex(x, y) for x in np.arange(1 + 2.0 / 16, 1.4, 1.0 / 16) for y in np.arange(-0.5, 0.5, 1.0 / 16) if abs(y) > 1.0 / 16]
        brackets = VGroup(*list(map(self.get_right_angle_bracket, intersection_inputs)))
...
class InfiniteContinuousJigsawPuzzle(ZetaTransformationScene):
    ...
    def set_stage(self):
        ...
        lines_per_piece = 5
        pieces = [VGroup(*left_plane[lines_per_piece * i:lines_per_piece * (i + 1)]) for i in range(len(list(left_plane)) / lines_per_piece)]
...
class ManyIntersectingLinesPreZeta(ZetaTransformationScene):
    CONFIG = {'apply_zeta': False, 'lines_center': RIGHT, 'nudge_size': 0.9, 'function': zeta, 'angle': np.pi / 5, 'arc_scale_factor': 0.3, 'shift_directions': [LEFT, RIGHT]}
    ...
    def construct(self):
        ...
        for vect in self.shift_directions:
            self.play(ApplyMethod(lines.shift, 2 * self.nudge_size * vect, path_arc=np.pi), *added_anims, run_time=3)
...
class ManyIntersectingLinesPreSSquared(ManyIntersectingLinesPreZeta):
    CONFIG = {'x_min': -int(FRAME_X_RADIUS), 'apply_zeta': False, 'lines_center': ORIGIN, 'nudge_size': 0.9, 'function': lambda z: z ** 2, 'shift_directions': [LEFT, RIGHT, UP, DOWN, DOWN + LEFT, UP + RIGHT]}
...
class MathematiciansLookingAtFunctionEquation(Scene):
    ...
    def construct(self):
        equation = OldTex('\\zeta(s)', '= 2^s \\pi ^{s-1}', '\\sin\\left(\\frac{\\pi s}{2}\\right)', '\\Gamma(1-s)', '\\zeta(1-s)')
        equation.shift(UP)
        ...
        self.play(Transform(equation[0].copy(), equation[-1], path_arc=-np.pi / 3, run_time=2))
        for mathy in mathys:
...
class DiscussZeros(ZetaTransformationScene):
    ...
    def ask_about_zeros(self):
        dots = VGroup(*[Dot((2 + np.sin(12 * alpha)) * rotate_vector(RIGHT, alpha + nudge)) for alpha in np.arange(3 * np.pi / 20, 2 * np.pi, 2 * np.pi / 5) for nudge in [random.random() * np.pi / 6]])
        dots.set_color(YELLOW)
        q_marks = VGroup(*[OldTex('?').next_to(dot, UP) for dot in dots])
        arrows = VGroup(*[Arrow(dot, ORIGIN, buff=0.2, tip_length=0.1) for dot in dots])
        ...
        self.play(FadeOut(arrows), FadeOut(q_marks), FadeOut(question), MoveToTarget(dots))
        self.wait()
        self.dots = dots
    ...
    def show_trivial_zeros(self):
        ...
        for dot in self.dots.target:
            dot.move_to(ORIGIN)
        self.apply_zeta_function(added_anims=[MoveToTarget(self.dots, run_time=3)], run_time=3)
        self.wait(3)
        self.play(self.plane.restore, self.plane.make_smooth, self.dots.restore, run_time=2)
        self.remove(*self.get_mobjects_from_last_animation())
        self.plane.restore()
        self.dots.restore()
        ...
        self.wait()
        self.play(FadeIn(randy))
        self.play(randy.change_mode, 'confused', ShowCreation(bubble), Write(bubble.content))
        self.play(Blink(randy))
        self.wait()
        self.play(Blink(randy))
        self.play(*list(map(FadeOut, [randy, bubble, bubble.content, trivial_zero_words])))

    def show_critical_strip(self):
        strip = Rectangle(height=FRAME_HEIGHT, width=1)
        ...
        new_dots = VGroup(*[Dot(0.5 * RIGHT + y * UP) for y in np.linspace(-2.5, 3.2, 5)])
        new_dots.set_color(YELLOW)
        critical_line = Line(0.5 * RIGHT + FRAME_Y_RADIUS * DOWN, 0.5 * RIGHT + FRAME_Y_RADIUS * UP, color=YELLOW)
        self.give_dots_wandering_anims()
        self.play(FadeIn(strip), *self.get_dot_wandering_anims())
        self.play(Write(name, run_time=1), ShowCreation(arrow), *self.get_dot_wandering_anims())
        self.play(*self.get_dot_wandering_anims())
        self.play(FadeIn(primes), *self.get_dot_wandering_anims())
        for x in range(7):
            self.play(*self.get_dot_wandering_anims())
        self.play(GrowFromCenter(photo), FadeOut(name), FadeOut(arrow), *self.get_dot_wandering_anims())
        self.play(Transform(self.dots, new_dots))
        self.play(ShowCreation(critical_line))
        self.wait(3)
        self.play(photo.shift, 7 * LEFT, *list(map(FadeOut, [primes, self.dots, strip])))
        self.remove(photo)
    ...
    def give_dots_wandering_anims(self):
        ...
        for i, dot in enumerate(self.dots):
            ...
            dot.target.move_to(self.wandering_path.point_from_proportion(float(2 + 2 * i) / (4 * len(list(self.dots))) % 1))
    ...
    def get_dot_wandering_anims(self):
        ...
        return [MoveAlongPath(dot, self.wandering_path, rate_func=get_rate_func(i)) for i, dot in enumerate(self.dots)]

    def transform_bit_of_critical_line(self):
        self.play(self.plane.scale, 0.8, self.critical_line.scale, 0.8, rate_func=there_and_back, run_time=2)
        self.wait()
        self.play(self.plane.set_stroke, GREY, 1, Animation(self.critical_line))
        self.plane.add(self.critical_line)
        self.apply_zeta_function()
        self.wait(2)
        self.play(self.plane.fade, Animation(self.critical_line))
    ...
    def extend_transformed_critical_line(self):
        ...
        full_line = VGroup(*[ParametricCurve(func, t_min=t0, t_max=t0 + 1) for t0 in range(100)])
        full_line.set_color_by_gradient(YELLOW, BLUE, GREEN, RED, YELLOW, BLUE, GREEN, RED)
...
class HighlightCriticalLineAgain(DiscussZeros):
    ...
    def construct(self):
        ...
        strip.set_stroke(width=0)
        strip.set_fill(YELLOW, opacity=0.3)
        line = Line(0.5 * RIGHT + FRAME_Y_RADIUS * UP, 0.5 * RIGHT + FRAME_Y_RADIUS * DOWN, color=YELLOW)
        ...
        self.play(randy.change_mode, 'pondering', randy.look_at, line.get_top(), ShowCreation(arrow1), run_time=3)
        self.play(randy.look_at, line.get_bottom(), ShowCreation(line), Transform(arrow1, arrow2))
        self.play(FadeOut(arrow1))
        self.play(Blink(randy))
        self.wait()
...
class DiscussSumOfNaturals(Scene):
    ...
    def construct(self):
        ...
        title.to_corner(UP + LEFT)
        neg_twelfth, eq, zeta_neg_1, sum_naturals = equation = OldTex('-\\frac{1}{12}', '=', '\\zeta(-1)', '= 1 + 2 + 3 + 4 + \\cdots')
        ...
        words = OldTexText('If not equal, at least connected', '\\\\(see links in description)')
        words.next_to(brace, DOWN)
        self.add(neg_twelfth, eq, zeta_neg_1, randy, title)
        self.wait()
        self.play(Write(sum_naturals), Write(q_mark), randy.change_mode, 'confused')
        self.play(Blink(randy))
        self.wait()
        self.play(randy.change_mode, 'angry')
        self.play(ShowCreation(sum_to_zeta), Write(cross))
        self.play(Blink(randy))
        self.wait()
        self.play(Transform(sum_to_zeta, sum_to_ac), FadeOut(cross), Write(analytic_continuation), randy.change_mode, 'pondering', randy.look_at, analytic_continuation)
        self.play(ShowCreation(ac_to_zeta))
        self.play(Blink(randy))
        self.wait()
        self.play(GrowFromCenter(brace), Write(words[0]), randy.look_at, words[0])
        self.wait()
...
class InventingMathPreview(Scene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(rect), Write(title))
...
class FinalAnimationTease(Scene):
    ...
    def construct(self):
        ...
        bubble.write("\n            Want to know what\n            $\\zeta'(s)$ looks like?\n        ")
        self.add(morty)
        self.play(morty.change_mode, 'hooray', morty.look_at, bubble.content, ShowCreation(bubble), Write(bubble.content))
        self.play(Blink(morty))
        self.wait()

class PatreonThanks(Scene):
    CONFIG = {'specific_patrons': ['CrypticSwarm', 'Ali Yahya', 'Damion Kistler', 'Juan Batiz-Benet', 'Yu Jun', 'Othman Alikhan', 'Markus Persson', 'Joseph John Cox', 'Luc Ritchie', 'Shimin Kuang', 'Einar Johansen', 'Rish Kundalia', 'Achille Brighton', 'Kirk Werklund', 'Ripta Pasay', 'Felipe Diniz']}
...
class ZetaThumbnail(Scene):
    ...
    def construct(self):
        plane = ComplexPlane(x_range=(-5, 5), y_range=(-3, 3), background_line_style={'stroke_width': 2, 'stroke_opacity': 0.75})
        plane.set_height(FRAME_HEIGHT)
        plane.scale(3 / 2.5)
        plane.add_coordinate_labels(font_size=12)
        lines = VGroup(*(Line(plane.c2p(-7, y), plane.c2p(7, y)) for y in np.arange(-2, 2, 0.1) if y != 0), *(Line(plane.c2p(x, -4), plane.c2p(x, 4)) for x in np.arange(-2, 2, 0.1) if x != 0))
...
class ZetaPartialSums(ZetaTransformationScene):
    CONFIG = {'anchor_density': 35, 'num_partial_sums': 12}
    ...
    def construct(self):
        ...
        sigma = OldTex('\\sum_{n = 1}^N \\frac{1}{n^s}')
        sigmas = []
        ...
        def get_partial_sum_func(n_terms):
            return lambda s: sum([1.0 / n ** s for n in range(1, n_terms + 1)])
        interim_planes = [self.plane.copy().apply_complex_function(get_partial_sum_func(N)) for N in N_list]
        ...
        for plane, sigma in zip(interim_planes, sigmas):
            self.play(Transform(self.plane, plane), Transform(symbol, sigma))
            self.wait()