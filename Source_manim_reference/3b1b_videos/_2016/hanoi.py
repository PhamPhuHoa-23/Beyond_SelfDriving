...
class CountingScene(Scene):
    CONFIG = {'base': 10, 'power_colors': [YELLOW, MAROON_B, RED, GREEN, BLUE, PURPLE_D], 'counting_dot_starting_position': (FRAME_X_RADIUS - 1) * RIGHT + (FRAME_Y_RADIUS - 1) * UP, 'count_dot_starting_radius': 0.5, 'dot_configuration_height': 2, 'ones_configuration_location': UP + 2 * RIGHT, 'num_scale_factor': 2, 'num_start_location': 2 * DOWN}
    ...
    def get_dot_template(self):
        down_right = 0.5 * RIGHT + np.sqrt(3) / 2 * DOWN
        dots = VGroup(*[Dot(point, radius=0.25, fill_opacity=0, stroke_width=2, stroke_color=WHITE) for point in self.get_template_configuration()])
    ...
    def increment(self, run_time_per_anim=1, added_anims=[], total_run_time=None):
        ...
        moving_dot = Dot(self.counting_dot_starting_position, radius=self.count_dot_starting_radius, color=self.power_colors[0])
        ...
        while continue_rolling_over:
            ...
            if len(self.curr_configurations[place].split()) == self.base:
                ...
                radius = 0.6 * max(full_configuration.get_width(), full_configuration.get_height())
                circle = Circle(radius=radius, stroke_width=0, fill_color=self.power_colors[place], fill_opacity=0.5)
    ...
    def get_digit_increment_animations(self):
        ...
        if self.is_perfect_power():
            ...
            arrow = Arrow(new_number_mob[place].get_top(), self.dot_templates[place].get_bottom(), color=self.power_colors[place])
...
class TowersOfHanoiScene(Scene):
    CONFIG = {'disk_start_and_end_colors': [BLUE_E, BLUE_A], 'num_disks': 5, 'peg_width': 0.25, 'peg_height': 2.5, 'peg_spacing': 4, 'include_peg_labels': True, 'middle_peg_bottom': 0.5 * DOWN, 'disk_height': 0.4, 'disk_min_width': 1, 'disk_max_width': 3, 'default_disk_run_time_off_peg': 1, 'default_disk_run_time_on_peg': 2}
    ...
    def add_pegs(self):
        peg = Rectangle(height=self.peg_height, width=self.peg_width, stroke_width=0, fill_color=GREY_BROWN, fill_opacity=1)
        peg.move_to(self.middle_peg_bottom, DOWN)
        self.pegs = VGroup(*[peg.copy().shift(vect) for vect in (self.peg_spacing * LEFT, ORIGIN, self.peg_spacing * RIGHT)])
        self.add(self.pegs)
    ...
    def add_disks(self):
        self.disks = VGroup(*[Rectangle(height=self.disk_height, width=width, fill_color=color, fill_opacity=0.8, stroke_width=0) for width, color in zip(np.linspace(self.disk_min_width, self.disk_max_width, self.num_disks), color_gradient(self.disk_start_and_end_colors, self.num_disks))])
    ...
    def move_disks_to_peg(self, disk_indices, next_peg_index, run_time=None, stay_on_peg=True, added_anims=[]):
        ...
        if stay_on_peg:
            self.play(Succession(ApplyMethod(disks.next_to, curr_peg, UP, 0), ApplyMethod(disks.next_to, next_peg, UP, 0, path_arc=path_arc), ApplyMethod(disks.move_to, target_bottom_point, DOWN)), *added_anims, run_time=run_time, rate_func=lambda t: smooth(t, 2))
        else:
            self.play(ApplyMethod(disks.move_to, target_bottom_point, DOWN), *added_anims, path_arc=path_arc * 2, run_time=run_time, rate_func=lambda t: smooth(t, 2))
...
class IntroduceKeith(Scene):
    ...
    def construct(self):
        ...
        self.play(keith.change_mode, 'speaking', keith.set_height, morty.get_height(), keith.next_to, morty, LEFT, LARGE_BUFF, run_time=1.5)
        ...
        self.play(keith.change_mode, 'hesitant', Transform(original_content, bubble.content))
        ...
        self.play(randy.change_mode, 'confused', randy.look_at, keith.eyes, keith.change_mode, 'plain', keith.look_at, randy.eyes, morty.change_mode, 'plain', morty.look_at, randy.eyes, FadeOut(bubble), FadeOut(original_content), ShowCreation(randy.bubble), Write(randy.bubble.content))
        self.play(Blink(keith))
...
class IntroduceTowersOfHanoi(TowersOfHanoiScene):
    ...
    def show_setup(self):
        ...
        self.play(ApplyMethod(self.pegs.restore, lag_ratio=0.5, run_time=2), Write(self.peg_labels))
        self.wait()
        self.bring_in_disks()
        self.wait()
...
class KeithShowingBinary(Scene):
    ...
    def construct(self):
        ...
        self.play(keith.change_mode, 'wave_1', keith.look_at, bits, morty.look_at, bits, Write(bits))
        for x in range(2):
        ...
        self.wait()
        self.play(FadeIn(randy), morty.change_mode, 'plain', morty.look_at, randy.eyes, keith.change_mode, 'plain', keith.look_at, randy.eyes)
        self.play(randy.change_mode, 'confused', ShowCreation(randy.bubble), Write(randy.bubble.content))
...
class IntroduceBase10(Scene):
    ...
    def expand_example_number(self):
        ...
        self.play(MoveToTarget(number), ShowCreation(arrows), ApplyMethod(expansion.restore, lag_ratio=0.5), run_time=2)
        self.play(Write(title))
        self.wait()
        self.title = title
...
class RhythmOfDecimalCounting(CountingScene):
    CONFIG = {'ones_configuration_location': 2 * UP + 2 * RIGHT, 'num_start_location': DOWN}
    ...
    def construct(self):
        ...
        self.play(ten_group.scale, 7, ten_group.shift, 2 * (DOWN + LEFT))
        self.wait()
        ...
        self.play(groups[2].to_edge, RIGHT, MaintainPositionRelativeTo(groups[1], groups[2]), MaintainPositionRelativeTo(groups[0], groups[2]), self.number_mob.to_edge, RIGHT, LARGE_BUFF, FadeOut(self.arrows))

class DecimalCountingAtHundredsScale(CountingScene):
    CONFIG = {'power_colors': [RED, GREEN, BLUE, PURPLE_D], 'counting_dot_starting_position': (FRAME_X_RADIUS + 1) * RIGHT + (FRAME_Y_RADIUS - 2) * UP, 'ones_configuration_location': 2 * UP + 5.7 * RIGHT, 'num_start_location': DOWN + 3 * RIGHT}
    ...
    def construct(self):
        ...
        Transform(self.arrows[0], Arrow(self.number_mob, self.dot_templates[0], color=self.power_colors[0])).update(1)
        for x in range(10):
...
class IntroduceBinaryCounting(BinaryCountingScene):
    CONFIG = {'ones_configuration_location': UP + 5 * RIGHT, 'num_start_location': DOWN + 2 * RIGHT}
    ...
    def initial_counting(self):
        ...
        self.play(randy.change_mode, 'hesitant', randy.look_at, self.number_mob, Write(bubble.content))
        self.wait()
        curr_content = bubble.content
        bubble.write('$1 \\! \\cdot \\! 2+$', '$0$')
        bubble.content[0][0].set_color(self.power_colors[1])
        self.play(Transform(curr_content, bubble.content), randy.change_mode, 'pondering', randy.look_at, self.number_mob)
        ...
        self.wait()
        curr_content = bubble.content
        bubble.write('$1 \\!\\cdot\\! 4 + $', '$0 \\!\\cdot\\! 2 + $', '$0$')
        ...
        self.play(Transform(brace, four_brace), Transform(twos_place, fours_place))
        self.play(Blink(randy))
        ...
        self.play(randy.change_mode, 'happy', randy.look_at, self.number_mob, *list(map(MoveToTarget, [brace, twos_place])))
        for x in range(8):
    ...
    def show_self_similarity(self):
        ...
        self.play(big_dot.center, big_dot.set_height, FRAME_HEIGHT - 2, big_dot.to_edge, LEFT, run_time=5)

class BinaryCountingAtEveryScale(Scene):
    CONFIG = {'num_bits': 4, 'show_title': False}

    def construct(self):
        title = OldTexText('Count to %d (which is %s in binary)' % (2 ** self.num_bits - 1, bin(2 ** self.num_bits - 1)[2:]))
        title.to_edge(UP)
        if self.show_title:
            self.add(title)
        ...
        def get_run_through(mobs):
            return Succession(*[Transform(curr_bits, mob, rate_func=squish_rate_func(smooth, 0, 0.5)) for mob in mobs], run_time=1)
        ...
        self.play(Transform(VGroup(*reversed(list(curr_bits))), VGroup(*reversed(list(bit_mobs[2 ** (self.num_bits - 1)]))), lag_ratio=0.5))
        self.wait()
...
class IntroduceSolveByCounting(TowersOfHanoiScene):
    ...
    def construct(self):
        ...
        word_list = [brace.get_text(text) for brace, text in zip(braces, ['Only flip last bit', 'Roll over once', 'Roll over twice', 'Roll over three times'])]
        brace = braces[0].copy()
        ...
        last_bit.save_state()
        self.play(disk.set_fill, YELLOW, disk[1].set_fill, BLACK, last_bit.set_fill, YELLOW)
        self.wait()
        self.move_disk(0, run_time=2)
        self.play(last_bit.restore, disk.set_fill, disk.original_fill_color, self.disks[0][1].set_fill, BLACK)
        self.play(self.get_increment_animation(), Transform(words, word_list[1]), Transform(brace, braces[1]))
        disk = self.disks[1]
        twos_bit = self.curr_bit_mob[1]
        twos_bit.save_state()
        self.play(disk.set_fill, MAROON_B, disk[1].set_fill, BLACK, twos_bit.set_fill, MAROON_B)
        ...
        self.play(disk.set_fill, disk.original_fill_color, disk[1].set_fill, BLACK, twos_bit.restore, Transform(brace, braces[0]), Transform(words, word_list[0]))
        self.move_disk(0, added_anims=[self.get_increment_animation()], run_time=2)
        self.wait()
        self.play(Transform(brace, braces[2]), Transform(words, word_list[2]))
        self.play(self.get_increment_animation())
        disk = self.disks[2]
        ...
        self.play(disk.set_fill, disk.original_fill_color, disk[1].set_fill, BLACK, fours_bit.restore, FadeOut(brace), FadeOut(words))
        self.wait()
        ...
        brace = braces[3]
        words = word_list[3]
        self.play(self.get_increment_animation(), GrowFromCenter(brace), Write(words, run_time=1))
        disk = self.disks[3]
        ...
        self.play(disk.set_fill, disk.original_fill_color, disk[1].set_fill, BLACK, eights_bit.restore)
    ...
    def get_increment_animation(self):
        return Succession(Transform(self.curr_bit_mob, next(self.bit_mobs_iter), lag_ratio=0.5, path_arc=-np.pi / 3), Animation(self.curr_bit_mob))

class SolveSixDisksByCounting(IntroduceSolveByCounting):
    CONFIG = {'num_disks': 6, 'stay_on_peg': False, 'run_time_per_move': 0.5}

    def construct(self):
        self.initialize_bit_mobs()
        for disk_index in get_ruler_sequence(self.num_disks - 1):
            self.play(self.get_increment_animation(), run_time=self.run_time_per_move)
            self.move_disk(disk_index, stay_on_peg=self.stay_on_peg, run_time=self.run_time_per_move)
        self.wait()
...
class RecursionTime(Scene):
    ...
    def construct(self):
        ...
        self.play(Transform(keith, keith_kick), morty.change_mode, 'happy', ShowCreation(bubble), Write(bubble.content, run_time=1))
        self.play(morty.change_mode, 'hooray', Transform(keith, keith_hooray), bubble.content.set_color_by_gradient, BLUE_A, BLUE_E)
        self.play(Blink(morty))
        self.wait()

class RecursiveSolution(TowersOfHanoiScene):
    CONFIG = {'num_disks': 4, 'middle_peg_bottom': 2 * DOWN}

    def construct(self):
        big_disk = self.disks[-1]
        self.eyes = Eyes(big_disk)
        title = OldTexText('Move 4-tower')
        sub_steps = OldTexText('Move 3-tower,', 'Move disk 3,', 'Move 3-tower')
        sub_steps[1].set_color(GREEN)
        sub_step_brace = Brace(sub_steps, UP)
        sub_sub_steps = OldTexText('Move 2-tower,', 'Move disk 2,', 'Move 2-tower')
        ...
        self.blink()
        self.move_subtower_to_peg(3, 1, run_time=2, added_anims=[self.eyes.look_at_anim(self.pegs[1]), FadeIn(sub_step_brace), Write(sub_steps[0], run_time=1)])
        self.wait()
        self.move_disk_to_peg(0, 0, run_time=2, added_anims=[self.eyes.look_at_anim(self.pegs[0].get_top())])
        self.shake(big_disk)
        self.move_disk_to_peg(0, 2, run_time=2, added_anims=[self.eyes.look_at_anim(self.pegs[2].get_bottom())])
        self.change_mode('angry')
        self.move_disk_to_peg(0, 1, run_time=2, added_anims=[self.eyes.look_at_anim(self.disks[1].get_top())])
        self.blink()
        ...
        self.move_subtower_to_peg(3, 2, run_time=2, added_anims=[self.eyes.look_at_anim(self.pegs[2].get_top()), Write(sub_steps[2])])
        ...
        self.shake(big_disk)
        self.blink()
        self.move_subtower_to_peg(2, 0, run_time=2, added_anims=[self.eyes.look_at_anim(self.pegs[0].get_bottom()), Write(sub_sub_steps[0])])
        self.blink()
        self.move_disk_to_peg(2, 2, run_time=2, added_anims=[Write(sub_sub_steps[1])])
        self.look_at(self.disks[0])
        big_disk.remove(self.eyes)
        self.move_subtower_to_peg(2, 2, run_time=2, added_anims=[self.eyes.look_at_anim(self.pegs[2].get_top()), Write(sub_sub_steps[2])])
        self.blink()
        self.look_at(self.disks[-1])
        self.play(FadeOut(self.eyes))
        ...
        big_disk.remove(self.eyes)
        self.move_subtower_to_peg(3, 0, run_time=2, added_anims=[self.eyes.look_at_anim(self.pegs[0].get_top())])
        self.blink()

    def shake(self, mobject, direction=UP, added_anims=[]):
        self.play(mobject.shift, 0.2 * direction, *added_anims, rate_func=wiggle)
...
class CodeThisUp(Scene):
    ...
    def construct(self):
        ...
        self.play(Blink(keith))
        self.wait()
        self.play(keith.change_mode, 'speaking', keith.look_at, morty.eyes, ShowCreation(bubble), Write(bubble.content), morty.change_mode, 'happy', morty.look_at, keith.eyes)
        self.wait()
        self.play(Blink(morty))
        self.wait()
...
class WhyDoesBinaryAchieveThis(Scene):
    ...
    def construct(self):
        ...
        bubble.write('\n            Why does counting\n            in binary work?\n        ')
        self.add(morty, keith)
        self.play(morty.change_mode, 'confused', morty.look_at, keith.eyes, ShowCreation(bubble), Write(bubble.content))
...
class LargeScaleHanoiDecomposition(TowersOfHanoiScene):
    CONFIG = {'num_disks': 8, 'peg_height': 3.5, 'middle_peg_bottom': 2 * DOWN, 'disk_max_width': 4}
...
class SolveTwoDisksByCounting(SolveSixDisksByCounting):
    CONFIG = {'num_disks': 2, 'stay_on_peg': False, 'run_time_per_move': 1, 'disk_max_width': 1.5}
...
class ShowFourDiskFourBitsParallel(IntroduceSolveByCounting):
    CONFIG = {'num_disks': 4, 'subtask_run_time': 1}
    ...
    def construct(self):
        ...
        self.wait()
        self.play(self.get_increment_animation())
        self.move_disk(self.num_disks - 1, stay_on_peg=False)
        self.wait()
        self.counting_subtask()
        self.wait()
        self.disk_subtask()
    ...
    def disk_subtask(self):
        ...
        for disk_index in get_ruler_sequence(self.num_disks - 2):
            self.move_disk(disk_index, run_time=run_time, stay_on_peg=False)

    def counting_subtask(self):
        num_tasks = 2 ** (self.num_disks - 1) - 1
        run_time = float(self.subtask_run_time) / num_tasks
        self.play(Succession(*[self.get_increment_animation() for x in range(num_tasks)]), rate_func=linear, run_time=self.subtask_run_time)

    def get_increment_animation(self):
        return Transform(self.curr_bit_mob, next(self.bit_mobs_iter), path_arc=-np.pi / 3)
...
class CoolRight(Scene):
    ...
    def construct(self):
        ...
        bubble.resize_to_content()
        bubble.pin_to(morty)
        self.play(morty.change_mode, 'surprised', morty.look, OUT, ShowCreation(bubble), Write(bubble.content))
        self.play(Blink(morty))
        self.wait()
        curr_content = bubble.content
        bubble.write('It gets \\\\ better...')
        self.play(Transform(curr_content, bubble.content), morty.change_mode, 'hooray', morty.look, OUT)
        self.wait()
        self.play(Blink(morty))
        self.wait()
...
class MentionLastVideo(Scene):
    ...
    def construct(self):
        ...
        self.add(morty, keith)
        self.play(keith.change_mode, 'raise_right_hand', keith.look_at, point, morty.change_mode, 'pondering', morty.look_at, point)
        self.play(Blink(keith))
...
class IntroduceConstrainedTowersOfHanoi(ConstrainedTowersOfHanoiScene):
    CONFIG = {'middle_peg_bottom': 2 * DOWN}
...
class RecursiveSolutionToConstrained(RecursiveSolution):
    CONFIG = {'middle_peg_bottom': 2 * DOWN, 'num_disks': 4}

    def construct(self):
        big_disk = self.disks[-1]
        self.eyes = Eyes(big_disk)
        title = OldTexText('Move 4-tower')
        subdivisions = [OldTexText('\\tiny Move %d-tower,' % d, 'Move disk %d,' % d, '\\, Move %d-tower, \\,' % d, 'Move disk %d,' % d, 'Move %d-tower' % d).set_color_by_tex('Move disk %d,' % d, color) for d, color in [(3, GREEN), (2, RED), (1, BLUE_C)]]
        ...
        self.shake(big_disk)
        self.wait()
        self.move_subtower_to_peg(self.num_disks - 1, 1, added_anims=[Transform(arrow, new_arrow), self.eyes.look_at_anim(self.pegs[1])])
        ...
        self.move_subtower_to_peg(self.num_disks - 1, 2, added_anims=[FadeOut(blocking), FadeOut(arrow), self.eyes.change_mode_anim('plain', thing_to_look_at=self.pegs[2]), Write(sub_steps[0], run_time=1)])
        self.blink()
        self.move_disk_to_peg(self.num_disks - 1, 1, added_anims=[Write(sub_steps[1], run_time=1)])
        self.wait()
        self.move_subtower_to_peg(self.num_disks - 1, 0, added_anims=[self.eyes.look_at_anim(self.pegs[0]), Write(sub_steps[2], run_time=1)])
        self.blink()
        self.move_disk_to_peg(self.num_disks - 1, 2, added_anims=[Write(sub_steps[3], run_time=1)])
        self.wait()
        big_disk.remove(self.eyes)
        self.move_subtower_to_peg(self.num_disks - 1, 2, added_anims=[self.eyes.look_at_anim(self.pegs[2].get_top()), Write(sub_steps[4], run_time=1)])
        self.blink()
        self.play(FadeOut(self.eyes))
        sub_sub_steps_brace.set_color(WHITE)
        self.move_subtower_to_peg(self.num_disks - 1, 0, added_anims=[steps_to_fade.fade, 0.7, sub_steps[2].set_color, WHITE, sub_steps[2].scale, 1.2, FadeIn(sub_sub_steps_brace)])
        ...
        big_disk.add(self.eyes)
        self.blink()
        self.move_subtower_to_peg(num_disks - 1, 2, added_anims=[self.eyes.look_at_anim(self.pegs[2]), Write(sub_sub_steps[0], run_time=1)])
        self.blink()
        self.move_disk_to_peg(num_disks - 1, 1, added_anims=[Write(sub_sub_steps[1], run_time=1)])
        self.wait()
        self.move_subtower_to_peg(num_disks - 1, 0, added_anims=[self.eyes.look_at_anim(self.pegs[0]), Write(sub_sub_steps[2], run_time=1)])
        self.blink()
        self.move_disk_to_peg(num_disks - 1, 2, added_anims=[Write(sub_sub_steps[3], run_time=1)])
        self.wait()
        big_disk.remove(self.eyes)
        self.move_subtower_to_peg(num_disks - 1, 2, added_anims=[self.eyes.look_at_anim(self.pegs[2].get_top()), Write(sub_sub_steps[4], run_time=1)])
        self.wait()
        ...
        self.play(steps.shift, (FRAME_Y_RADIUS - sub_sub_steps.get_top()[1] - MED_SMALL_BUFF) * UP, self.eyes.look_at_anim(steps))
        self.play(ApplyMethod(VGroup(VGroup(braces[-2], subdivisions[-2])).set_fill, None, 1, run_time=3, lag_ratio=0.5))
        self.blink()
        ...
        self.play(FadeOut(self.eyes))
        self.wait()
        movements = [(0, 1), (0, 0), (1, 1), (0, 1), (0, 2), (1, 0), (0, 1), (0, 0)]
...
class SolveConstrainedByCounting(ConstrainedTowersOfHanoiScene):
    CONFIG = {'num_disks': 5, 'ternary_mob_scale_factor': 2}
    ...
    def increment_animation(self):
        return Succession(Transform(self.curr_ternary_mob, next(self.ternary_mob_iter), lag_ratio=0.5, path_arc=np.pi / 6), Animation(self.curr_ternary_mob))
...
class CompareNumberSystems(Scene):
    ...
    def construct(self):
        ...
        self.play(GrowFromCenter(zero_one_two.brace), Write(zero_one_two.name), keith.look_at, zero_one_two)
        ...
        self.play(Blink(morty))
        self.wait()
        self.play(morty.shrug, morty.look_at, keith.eyes, keith.shrug, keith.look_at, morty.eyes)
        self.wait()
        self.play(morty.change_mode, 'hesitant', morty.look_at, zero_one.name, keith.change_mode, 'erm', keith.look_at, zero_one.name)
        self.play(Blink(morty))
        self.play(Write(dots, run_time=3))
        self.wait()

class IntroduceTernaryCounting(CountingScene):
    CONFIG = {'base': 3, 'counting_dot_starting_position': (FRAME_X_RADIUS - 1) * RIGHT + (FRAME_Y_RADIUS - 1) * UP, 'count_dot_starting_radius': 0.5, 'dot_configuration_height': 1, 'ones_configuration_location': UP + 2 * RIGHT, 'num_scale_factor': 2, 'num_start_location': DOWN + RIGHT}
    ...
    def construct(self):
        ...
        self.play(Transform(brace, new_brace), Transform(threes_place, nines_place))
        self.wait()
        for x in range(9):
            self.increment()

class TernaryCountingSelfSimilarPattern(Scene):
    CONFIG = {'num_trits': 3, 'colors': CountingScene.CONFIG['power_colors'][:3]}
    ...
    def construct(self):
        ...
        steps = VGroup(*list(map(TexText, ['Count to %s,' % ('2' * (self.num_trits - 1)), 'Roll over,', 'Count to %s,' % ('2' * (self.num_trits - 1)), 'Roll over,', 'Count to %s,' % ('2' * (self.num_trits - 1))])))
        steps.arrange(RIGHT)
        for step in steps[::2]:
        ...
        def get_increment():
            return Transform(curr_ternary_mob, next(ternary_mob_iter), lag_ratio=0.5, path_arc=-np.pi / 3)
        self.add(curr_ternary_mob, title)
        self.play(GrowFromCenter(brace))
        for i, step in enumerate(steps):
            self.play(Write(step, run_time=1))
            if i % 2 == 0:
                self.play(Succession(*[get_increment() for x in range(3 ** (self.num_trits - 1) - 1)]), run_time=1)
            else:
                self.play(get_increment())
            self.wait()

class TernaryCountingSelfSimilarPatternFiveTrits(TernaryCountingSelfSimilarPattern):
    CONFIG = {'num_trits': 5, 'colors': color_gradient([YELLOW, PINK, RED], 5)}
...
class SolveConstrainedWithTernaryCounting(ConstrainedTowersOfHanoiScene):
    ...
    def increment_number(self, run_time=1):
        self.play(Transform(self.curr_ternary_mob, next(self.ternary_mob_iter), path_arc=-np.pi / 3, lag_ratio=0.5, run_time=run_time))
...
class DescribeSolutionByCountingToConstrained(SolveConstrainedWithTernaryCounting):
    ...
    def construct(self):
        ...
        words = [brace.get_text(text) for brace, text in zip(braces, ['Only flip last trit', 'Roll over once', 'Roll over twice', 'Roll over three times'])]
        color = YELLOW
        brace = braces[0]
        word = words[0]
        words[0].set_color(color)
        self.increment_number()
        self.play(FadeIn(brace), Write(word, run_time=1), self.curr_ternary_mob[0].set_color, color)
        self.wait()
        self.play(self.disks[0].set_fill, color, self.disks[0].label.set_fill, BLACK)
        ...
        self.increment_number()
        self.play(Transform(brace, braces[1]), Transform(word, words[1]), self.curr_ternary_mob[1].set_color, color)
        self.wait()
        self.play(self.disks[1].set_fill, color, self.disks[1].label.set_fill, BLACK)
        self.move_next_disk(stay_on_peg=True)
        self.wait()
        ...
        self.wait()
        self.increment_number()
        self.play(FadeIn(braces[2]), Write(words[2], run_time=1), self.curr_ternary_mob[2].set_fill, color, self.disks[2].set_fill, color, self.disks[2].label.set_fill, BLACK)
        self.wait()
        self.move_next_disk(stay_on_peg=True)
        self.wait()
...
class AnswerConfigurationsCount(TowersOfHanoiScene):
    CONFIG = {'middle_peg_bottom': 2.5 * DOWN, 'num_disks': 4, 'peg_height': 1.5}
    ...
    def construct(self):
        ...
        for parens, disk in zip(parentheticals, reversed(list(self.disks))):
            ...
            self.play(ApplyMethod(disk.next_to, self.pegs[2], UP, run_time=2), GrowFromCenter(parens.brace), Write(parens.three, run_time=1))
    ...
    def get_parentheticals(self, top_mob):
        parentheticals = VGroup(*reversed([OldTex('\n                \\left(\n                    \\begin{array}{c}\n                        \\text{Choices for} \\\\\n                        \\text{disk %d}\n                    \\end{array}\n                \\right)\n            ' % d) for d in range(self.num_disks)]))
...
class ShowSomeGraph(Scene):
    ...
    def construct(self):
        ...
        title.scale(2)
        title.to_edge(UP)
        nodes = VGroup(*list(map(Dot, [2 * LEFT, UP, DOWN, 2 * RIGHT, 2 * RIGHT + 2 * UP, 2 * RIGHT + 2 * DOWN, 4 * RIGHT + 2 * UP])))
        edge_pairs = [(0, 1), (0, 2), (1, 3), (2, 3), (3, 4), (3, 5), (4, 6)]
        edges = VGroup()
        for i, j in edge_pairs:
            edges.add(Line(nodes[i].get_center(), nodes[j].get_center()))
        self.play(Write(title))
        for mob in (nodes, edges):
            mob.set_color_by_gradient(YELLOW, MAROON_B)
            self.play(ShowCreation(mob, lag_ratio=0.5, run_time=2))
        self.wait()

class SierpinskiGraphScene(Scene):
    CONFIG = {'num_disks': 3, 'towers_config': {'num_disks': 3, 'peg_height': 1.5, 'peg_spacing': 2, 'include_peg_labels': False, 'disk_min_width': 1, 'disk_max_width': 2}, 'preliminary_node_radius': 1, 'center_to_island_length': 2.0, 'include_towers': True, 'start_color': RED, 'end_color': GREEN, 'graph_stroke_width': 2}
    ...
    def get_node_circles(self, order=3):
        ...
        for island, angle in ((islands[0], np.pi / 6), (islands[2], 5 * np.pi / 6)):
            island.rotate(np.pi, rotate_vector(RIGHT, angle), about_point=island.get_center_of_mass())
...
class IntroduceGraphStructure(SierpinskiGraphScene):
    CONFIG = {'include_towers': True, 'graph_stroke_width': 3, 'num_disks': 3}
    ...
    def introduce_nodes(self):
        self.play(FadeIn(self.nodes, run_time=3, lag_ratio=0.5))
        vect = LEFT
    ...
    def define_edges(self):
        ...
        edge = Line(nodes[0].get_right(), nodes[1].get_left(), color=YELLOW, stroke_width=6)
    ...
    def tour_structure(self):
        ...
        self.play(self.disks[0].set_color, YELLOW, *[ApplyMethod(disk.label.set_color, BLACK) for disk in self.disks[0]])
        self.wait()
...
class DescribeTriforcePattern(SierpinskiGraphScene):
    CONFIG = {'index_pairs': [(7, 1), (2, 3), (5, 6)], 'scale': 2, 'disk_color': MAROON_B, 'include_towers': True, 'first_connect_0_and_2_islands': True}

    def construct(self):
        index_pair = self.index_pairs[0]
        self.zoom_into_node(index_pair[0], self.scale)
        self.play(self.disks[self.scale - 1].set_color, self.disk_color, *[ApplyMethod(disk.label.set_color, BLACK) for disk in self.disks[self.scale - 1]])
        ...
        bold_edges = [self.node_indices_to_edge(*pair).copy().set_stroke(self.disk_color, 6) for pair in self.index_pairs]
        self.play(ShowCreation(bold_edges[0]))
        self.wait()
        self.play(*list(map(ShowCreation, bold_edges[1:])))
        self.wait()
        power_of_three = 3 ** (self.scale - 1)
        index_sets = [list(range(0, power_of_three)), list(range(power_of_three, 2 * power_of_three)), list(range(2 * power_of_three, 3 * power_of_three))]
...
class DescribeOrderTwoPattern(DescribeTriforcePattern):
    CONFIG = {'index_pairs': [(8, 9), (17, 18), (4, 22)], 'scale': 3, 'disk_color': RED, 'first_connect_0_and_2_islands': False}
...
class ShowPathThroughGraph(SierpinskiGraphScene):
    ...
    def construct(self):
        arrows = VGroup(*[Arrow(n1.get_center(), n2.get_center(), tip_length=0.15, buff=0.15) for n1, n2 in zip(self.nodes, self.nodes[1:])])
        self.wait()
...
class MentionFinalAnimation(Scene):
    ...
    def construct(self):
        ...
        self.add(morty)
        self.wait()
        self.play(morty.change_mode, 'speaking', morty.look_at, bubble.content, ShowCreation(bubble), Write(bubble.content))
        self.play(Blink(morty))
        self.wait(2)
        self.play(Blink(morty))
        self.wait(2)
...
class PatreonThanks(Scene):
    CONFIG = {'specific_patrons': ['CrypticSwarm', 'Ali Yahya', 'Juan    Batiz-Benet', 'Yu  Jun', 'Othman  Alikhan', 'Joseph  John Cox', 'Luc Ritchie', 'Einar Johansen', 'Rish    Kundalia', 'Achille Brighton', 'Kirk    Werklund', 'Ripta   Pasay', 'Felipe  Diniz']}
...
class ShowSierpinskiCurvesOfIncreasingOrder(Scene):
    CONFIG = {'sierpinski_graph_scene_config': {'include_towers': False}, 'min_order': 2, 'max_order': 7, 'path_stroke_width': 7}

    def construct(self):
        graph_scenes = [SierpinskiGraphScene(num_disks=order, **self.sierpinski_graph_scene_config) for order in range(self.min_order, self.max_order + 1)]
    ...
    def get_path(self, graph_scene):
        ...
        for n1, n2, n3 in zip(nodes, nodes[1:], nodes[2:]):
            segment = VMobject()
            segment.set_points_as_corners([n1.get_center(), n2.get_center(), n3.get_center()])
            path.add(segment)
        path.set_color_by_gradient(graph_scene.start_color, graph_scene.end_color)
...
class Part1Thumbnail(Scene):
    ...
    def construct(self):
        toh_scene = TowersOfHanoiScene(peg_spacing=2, part_number=1)
        ...
        toh.to_edge(DOWN)
        self.add(toh)
        sierpinski_scene = ShowSierpinskiCurvesOfIncreasingOrder(min_order=self.sierpinski_order, max_order=self.sierpinski_order, skip_animations=True)
