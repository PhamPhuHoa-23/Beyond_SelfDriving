...
class TimeLine(Scene):
    ...
    def construct(self):
        dated_events = [{'date': 1696, 'text': 'Johann Bernoulli poses Brachistochrone problem', 'picture': 'Johann_Bernoulli2'}, {'date': 1662, 'text': 'Fermat states his principle of least time', 'picture': 'Pierre_de_Fermat'}]
        speical_dates = [2016] + [obj['date'] for obj in dated_events]
        centuries = list(range(1600, 2100, 100))
        timeline = NumberLine(numerical_radius=300, number_at_center=1800, unit_length_to_spatial_width=FRAME_X_RADIUS / 100, tick_frequency=10, big_tick_numbers=centuries)
...
class StayedUpAllNight(Scene):
    ...
    def construct(self):
        ...
        phil_trans.add(rect)
        newton_complaint = OldTexText(['``I do not love to be', ' \\emph{dunned} ', "and teased by foreigners''"], size='\\small')
        ...
        self.play(Rotating(hour_hand, radians=-2 * np.pi, **kwargs), Rotating(minute_hand, radians=-12 * 2 * np.pi, **kwargs), run_time=5)
        self.wait()
        self.clear()
        self.add(newton)
...
class ThetaTGraph(Scene):
    ...
    def construct(self):
        ...
        squiggle = ParametricCurve(lambda t: t * RIGHT + 0.2 * t * (5 - t) * np.sin(t) ** 2 * UP, start=0, end=5)
        self.play(ShowCreation(t_axis), ShowCreation(theta_axis), ShimmerIn(theta_mob), ShimmerIn(t_mob))
...
class SolutionsToTheBrachistochrone(Scene):
    ...
    def construct(self):
        ...
        lines = Mobject(*[Line(lower_left, lower_left + 5 * r * np.cos(np.arctan(r)) * RIGHT + 2 * r * np.sin(np.arctan(r)) * UP) for r in r_range])
        ...
        self.wait()
        self.play(Transform(cycloids, lines), Transform(x_axis, t_axis), Transform(x_label, t_label), Transform(y_label, theta_label), run_time=2)
        self.wait()
...
class VideoLayout(Scene):
    ...
    def construct(self):
        ...
        left_brace.words = list(map(TexText, ['Problem statement', 'History', "Johann Bernoulli's cleverness"]))
        curr = left_brace
        right_brace.words = list(map(TexText, ['Challenge', "Mark Levi's cleverness"]))
...
class ShortestPathProblem(Scene):
    ...
    def construct(self):
        ...
        for point, char in [(point_a, 'A'), (point_b, 'B')]:
            ...
            dot.add(letter)
            dots.append(dot)
        path = ParametricCurve(lambda t: (t / 2 + np.cos(t)) * RIGHT + np.sin(t) * UP, start=-2 * np.pi, end=2 * np.pi)
...
class TalkedAboutSnellsLaw(Scene):
    ...
    def construct(self):
        ...
        phrases = ["Let's talk about Snell's law", "I love Snell's law", "It's like running from \\\\ a beach into the ocean", "It's like two constant \\\\ tension springs"]
