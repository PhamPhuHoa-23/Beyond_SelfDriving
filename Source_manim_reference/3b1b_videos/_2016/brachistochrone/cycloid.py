...
class CycloidScene(Scene):
    ...
    def draw_cycloid(self, run_time=3, *anims, **kwargs):
        ...
        self.play(RollAlongVector(self.circle, self.cycloid.get_points()[-1] - self.cycloid.get_points()[0], **kwargs), ShowCreation(self.cycloid, **kwargs), *anims)

    def roll_back(self, run_time=3, *anims, **kwargs):
        kwargs['run_time'] = run_time
        self.play(RollAlongVector(self.circle, self.cycloid.get_points()[0] - self.cycloid.get_points()[-1], rotation_vector=IN, **kwargs), ShowCreation(self.cycloid, rate_func=lambda t: smooth(1 - t), **kwargs), *anims)
        self.generate_cycloid()
...
class LeviSolution(CycloidScene):
    CONFIG = {'cycloid_fraction': 0.25}

    def construct(self):
        CycloidScene.construct(self)
        self.add(self.ceiling)
        self.init_points()
        methods = [self.draw_cycloid, self.roll_into_position, self.draw_p_and_c, self.show_pendulum, self.show_diameter, self.show_theta, self.show_similar_triangles, self.show_sin_thetas, self.show_y, self.rearrange]
    ...
    def roll_into_position(self):
        self.play(RollAlongVector(self.circle, (1 - self.cycloid_fraction) * self.radius * 2 * np.pi * LEFT, rotation_vector=IN, run_time=2))

    def draw_p_and_c(self):
        radial_line = self.circle.submobjects[0]
    ...
    def show_pendulum(self, arc_angle=np.pi, arc_color=GREEN):
        ...
        self.wait()
        pairs = [(line_angle, arc_angle / 2), (line_angle + arc_angle / 2, -arc_angle), (line_angle - arc_angle / 2, arc_angle / 2)]
        arcs = []
        for start, angle in pairs:
            arc = Arc(angle=angle, radius=line_length, start_angle=start, color=GREEN)
            arc.shift(self.c_point)
            self.play(ShowCreation(arc), ApplyMethod(line.rotate, angle, path_func=path_along_arc(angle)), run_time=2)
            arcs.append(arc)
        self.wait()
    ...
    def show_diameter(self):
        exceptions = [self.circle, self.tangent_line, self.pc_line, self.right_angle_symbol]
    ...
    def show_theta(self, radius=1):
        arc = Arc(angle=self.tangent_line.get_angle() - np.pi / 2, radius=radius, start_angle=np.pi / 2)
        theta = OldTex('\\theta')
    ...
    def show_similar_triangles(self):
        ...
        new_arc = Arc(angle=self.tangent_line.get_angle() - np.pi / 2, radius=0.5, start_angle=np.pi)
        new_arc.shift(self.c_point)
        new_theta = self.theta.copy()
        new_theta.next_to(new_arc, LEFT)
        new_theta.shift(0.1 * DOWN)
        kwargs = {'stroke_width': 2 * DEFAULT_STROKE_WIDTH}
        ...
        self.play(Transform(self.arc.copy(), new_arc), Transform(self.theta.copy(), new_theta), run_time=3)
        self.wait()
    ...
    def show_sin_thetas(self):
        ...
        mob.ingest_submobjects()
        triplets = [(pc, 'D\\sin(\\theta)', 0.5), (self.y_line, 'D\\sin^2(\\theta)', 0.7)]
    ...
    def show_y(self):
        ...
        y_mob.shift(0.2 * UP)
        self.play(Transform(self.d_sin_squared_theta, temp_expr), ShimmerIn(y_mob), ShowCreation(y_equals))
    ...
    def rearrange(self):
        ...
        self.play(ShimmerIn(y_sqrt), ShimmerIn(d_sqrt), ApplyMethod(squared.shift, 4 * UP), ApplyMethod(theta.shift, 1.5 * squared.get_width() * LEFT))
        self.wait()
        y_sqrt.add(y)
        d_sqrt.add(d)
        sin.add(theta)
        ...
        self.play(Transform(equals, new_eq), ShimmerIn(sin_over), ShimmerIn(one_over), ApplyMethod(d_sqrt.next_to, one_over, DOWN, path_func=path_along_arc(-np.pi)), ApplyMethod(y_sqrt.next_to, sin_over, DOWN, path_func=path_along_arc(-np.pi)), run_time=2)
        self.wait()
...
class SlidingObject(CycloidScene, PathSlidingScene):
    ...
    def construct(self, with_words):
        ...
        self.play_over_time_range(start_time, end_time, RollAlongVector(self.circle, self.cycloid.get_points()[-1] - self.cycloid.get_points()[0], run_time=end_time - start_time, rate_func=linear))
