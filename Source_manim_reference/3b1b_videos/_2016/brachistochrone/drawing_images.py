...
DEFAULT_GAUSS_BLUR_CONFIG = {'ksize': (5, 5), 'sigmaX': 6, 'sigmaY': 6}
DEFAULT_CANNY_CONFIG = {'threshold1': 50, 'threshold2': 100}
...
def sort_by_color(mob):
    indices = np.argsort(np.apply_along_axis(lambda p: -get_norm(p), 1, mob.rgbas))
...
def nearest_neighbor_align(mobject1, mobject2):
    ...
    for n in range(mobject1.get_num_points()):
        ...
        new_mob1.add_points([mobject1.get_points()[n]] * sum(indices))
        new_mob2.add_points(mobject2.get_points()[indices], rgbas=mobject2.rgbas[indices])
    return (new_mob1, new_mob2)

def get_connected_components(image_array, blur_radius=DEFAULT_BLUR_RADIUS, threshold=DEFAULT_CONNECTED_COMPONENT_THRESHOLD):
    ...
    return [image_array * (labels == count) for count in range(1, component_count + 1)]

def color_region(bw_region, colored_image):
    return thicken(bw_region > 0) * colored_image

class TracePicture(Scene):
    args_list = [('Newton',), ('Mark_Levi',), ('Steven_Strogatz',), ('Pierre_de_Fermat',), ('Galileo_Galilei',), ('Jacob_Bernoulli',), ('Johann_Bernoulli2',), ('Old_Newton',)]
    ...
    def construct(self, name):
        ...
        for mob in (edge_mobject, full_picture):
            ...
            mob.show()
        self.play(DelayByOrder(FadeIn(full_picture, run_time=run_time, rate_func=squish_rate_func(smooth, 0.7, 1))), ShowCreation(edge_mobject, run_time=run_time, rate_func=linear))
    ...
    def get_edge_mobject(self, image_array):
        ...
        colored_edges = [color_region(edge, image_array) for edge in individual_edges]
        colored_edge_mobject_list = [MobjectFromPixelArray(colored_edge) for colored_edge in colored_edges]
...
class JohannThinksHeIsBetter(Scene):
    ...
    def construct(self):
        names = ['Johann_Bernoulli2', 'Jacob_Bernoulli', 'Gottfried_Wilhelm_von_Leibniz', 'Newton']
        ...
        self.add(johann)
        self.wait()
        self.play(Transform(johann, pensive_johann), Transform(point, bubble), run_time=2)
        self.remove(point)
        self.add(bubble)
        weakling = guys[1]
        self.play(FadeIn(comparitive_johann), ShowCreation(greater_than), FadeIn(weakling))
        self.wait(2)
        for guy in guys[2:]:
...
class NewtonVsJohann(Scene):
    ...
    def construct(self):
        newton, johann = [ImageMobject(name, invert=False).scale(0.5) for name in ('Newton', 'Johann_Bernoulli2')]
        greater_than = OldTex('>')
        ...
        for i in range(2):
            ...
            self.play(ApplyMethod(newton.replace, johann, **kwargs), ApplyMethod(johann.replace, newton, **kwargs))
            self.wait()

class JohannThinksOfFermat(Scene):

    def construct(self):
        johann, fermat = [ImageMobject(name, invert=False) for name in ('Johann_Bernoulli2', 'Pierre_de_Fermat')]
        johann.scale(0.2)
...
class MathematiciansOfEurope(Scene):
    ...
    def construct(self):
        ...
        mathematicians = [('Newton', [-1.75, -0.75, 0]), ('Jacob_Bernoulli', [-0.75, -1.75, 0]), ('Ehrenfried_von_Tschirnhaus', [0.5, -0.5, 0]), ('Gottfried_Wilhelm_von_Leibniz', [0.2, -1.75, 0]), ("Guillaume_de_L'Hopital", [-1.75, -1.25, 0])]
...
class NewtonConsideredEveryoneBeneathHim(Scene):
    ...
    def construct(self):
        mathematicians = [ImageMobject(name, invert=False) for name in ['Old_Newton', 'Johann_Bernoulli2', 'Jacob_Bernoulli', 'Ehrenfried_von_Tschirnhaus', 'Gottfried_Wilhelm_von_Leibniz', "Guillaume_de_L'Hopital"]]
