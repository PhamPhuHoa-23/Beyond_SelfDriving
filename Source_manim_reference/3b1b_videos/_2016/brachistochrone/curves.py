...
class Cycloid(ParametricCurve):
    CONFIG = {'point_a': 6 * LEFT + 3 * UP, 'radius': 2, 'end_theta': 3 * np.pi / 2, 'density': 5 * DEFAULT_POINT_DENSITY_1D, 'color': YELLOW}
...
class SlideWordDownCycloid(Animation):
    ...
    def interpolate_mobject(self, alpha):
        ...
        cut_offs = [0.1, 0.3, 0.7]
        for letter, time, end_letter in zip(self.mobject.split(), virtual_times, self.end_letters):
            time = max(time, 0)
            time = min(time, 1)
            if time < cut_offs[0]:
                ...
                try:
                    angle = angle_of_vector(self.path.get_points()[index + 1] - self.path.get_points()[index])
                except:
                    angle = letter.angle
            else:
...
class PathSlidingScene(Scene):
    CONFIG = {'gravity': 3, 'delta_t': 0.05, 'wait_and_add': True, 'show_time': True}
...
class TryManyPaths(PathSlidingScene):
    ...
    def construct(self):
        ...
        for path in paths:
            self.play(Transform(curr_path, path, path_func=path_along_arc(np.pi / 2), run_time=3))

    def get_text(self):
        return OldTexText('Which path is fastest?')

    def get_paths(self):
        sharp_corner = Mobject(Line(3 * UP + LEFT, LEFT), Arc(angle=np.pi / 2, start_angle=np.pi), Line(DOWN, DOWN + 3 * RIGHT)).ingest_submobjects().set_color(GREEN)
        paths = [Arc(angle=np.pi / 2, radius=3, start_angle=4), LoopTheLoop(), Line(7 * LEFT, 7 * RIGHT, color=RED_D), sharp_corner, FunctionGraph(lambda x: 0.05 * x ** 2 + 0.1 * np.sin(2 * x)), FunctionGraph(lambda x: x ** 2, x_min=-3, x_max=2, density=3 * DEFAULT_POINT_DENSITY_1D)]
        cycloid = Cycloid()
...
class NotTheCircle(PathSlidingScene):
    ...
    def construct(self):
        ...
        words.shift(UP)
        self.play(ShowCreation(path), ApplyMethod(radius.rotate, angle, path_func=path_along_arc(angle)))
        self.play(FadeOut(radius))
        self.play(ApplyMethod(path.put_start_and_end_on, start, end, path_func=path_along_arc(-angle)), run_time=3)
...
class TransitionAwayFromSlide(PathSlidingScene):
    ...
    def construct(self):
        ...
        arrows = Mobject(*[arrow.copy().shift(vect) for vect in (3 * LEFT, ORIGIN, 3 * RIGHT)])
        arrows.shift(FRAME_WIDTH * RIGHT)
        self.add(arrows)
        self.add(self.cycloid)
        self.slide(randy, self.cycloid)
        everything = Mobject(*self.mobjects)
        self.play(ApplyMethod(everything.shift, 4 * FRAME_X_RADIUS * LEFT, run_time=2, rate_func=rush_into))
...
class MinimalPotentialEnergy(Scene):
    ...
    def construct(self):
        ...
        graph = FunctionGraph(lambda x: 0.4 * (x - 2) * (x + 2) + 3, x_min=-2, x_max=2, density=3 * DEFAULT_POINT_DENSITY_1D)
        ...
        top_words, last_side_words = [list(map(TexText, pair)) for pair in [("Light's travel time", 'Potential energy'), ('path', 'mechanical state')]]
        ...
        self.wait()
        self.play(FadeOut(top_words[0]), FadeOut(last_side_words[0]), GrowFromCenter(top_words[1]), GrowFromCenter(last_side_words[1]))
        self.wait(3)

class WhatGovernsSpeed(PathSlidingScene):
    CONFIG = {'num_pieces': 6, 'wait_and_add': False, 'show_time': False}
    ...
    def construct(self):
        ...
        ceiling = points[0, 1]
        n = len(points)
        broken_points = [points[k * n / self.num_pieces:(k + 1) * n / self.num_pieces] for k in range(self.num_pieces)]
    ...
    def show_geometry(self, slider, vector):
        ...
        vert_brace = Brace(Mobject(Point(ceil_point), Point(slider.get_center())), RIGHT, buff=0.5)
    ...
    def solve_energy(self):
        ...
        self.play(Transform(Point(loss_in_potential.get_left()), loss_in_potential), *list(map(GrowFromCenter, potential.split())))
        self.wait(2)
        ...
        self.wait()
        self.play(Transform(squared, sqrt, path_func=clockwise_path()), Transform(equals, new_eq))
        self.wait(2)
...
class WonkyDefineCurveWithKnob(DefineCurveWithKnob):
    ...
    def get_path(self):
        return ParametricCurve(lambda t: t * RIGHT + (-0.2 * t - np.sin(2 * np.pi * t / 6)) * UP, start=-7, end=10)

class SlowDefineCurveWithKnob(DefineCurveWithKnob):

    def get_path(self):
        return ParametricCurve(lambda t: t * RIGHT + (np.exp(-(t + 2) ** 2) - 0.2 * np.exp(t - 2)), start=-4, end=4)
