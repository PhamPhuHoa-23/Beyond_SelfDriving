...
from from_3b1b.old.brachistochrone.curves import Cycloid, PathSlidingScene, RANDY_SCALE_FACTOR, TryManyPaths

class Lens(Arc):
    CONFIG = {'radius': 2, 'angle': np.pi / 2, 'color': BLUE_B}
...
class MultipathPhotonScene(PhotonScene):
    ...
    def run_along_paths(self, **kwargs):
        ...
        for photon_run, path in zip(photon_runs, paths):
            self.play(photon_run, ShowCreation(path, rate_func=lambda t: 0.9 * smooth(t)), **kwargs)
        self.wait()
...
class PhotonThroughLens(MultipathPhotonScene):
    ...
    def get_paths(self):
        ...
        focal_point = 2 * RIGHT
        return [Mobject(Line(FRAME_X_RADIUS * LEFT + fc[1] * UP, fc), Line(fc, sc), Line(sc, focal_point), Line(focal_point, 6 * focal_point - 5 * sc)).ingest_submobjects() for fc, sc in zip(first_contact, second_contact)]
...
class PhotonOffMirror(MultipathPhotonScene):
    ...
    def get_paths(self):
        ...
        return [Mobject(Line(start_point, anchor_point), Line(anchor_point, end_point)).ingest_submobjects() for anchor_point, end_point in zip(anchor_points, end_points)]
...
class PhotonsInWater(MultipathPhotonScene):
    ...
    def get_paths(self):
        ...
        return [Mobject(Line(start_point, [midpoint, 0, 0]), Line([midpoint, 0, 0], [end_point, -FRAME_Y_RADIUS, 0])).ingest_submobjects() for midpoint, end_point in zip(midpoints, end_points)]
...
class ShowMultiplePathsScene(PhotonScene):
    ...
    def construct(self):
        ...
        self.play(self.photon_run_along_path(curr_path), ShowCreation(curr_path_copy, rate_func=rush_into))
...
class ShowMultiplePathsThroughLens(ShowMultiplePathsScene):
    ...
    def get_paths(self):
        ...
        return [Mobject(Line(self.start_point, a), Line(a, b), Line(b, self.end_point)).set_color(color) for (a, b), color in zip([(upper_left, upper_right), (upper_left, lower_right), (lower_left, lower_right), (lower_left, upper_right)], Color(YELLOW).range_to(WHITE, 4))]
...
class ShowMultiplePathsOffMirror(ShowMultiplePathsScene):
    ...
    def get_paths(self):
        return [Mobject(Line(self.start_point, midpoint), Line(midpoint, self.end_point)).set_color(color) for midpoint, color in zip([2 * UP, 2 * DOWN], Color(YELLOW).range_to(WHITE, 2))]
...
class ShowMultiplePathsInWater(ShowMultiplePathsScene):
    ...
    def get_paths(self):
        self.leftmost, self.rightmost = result = [Mobject(Line(self.start_point, midpoint), Line(midpoint, self.end_point)).set_color(color) for midpoint, color in zip([3 * LEFT, 3 * RIGHT], Color(YELLOW).range_to(WHITE, 2))]
        return result
...
class StraightLinesFastestInConstantMedium(PhotonScene):
    ...
    def get_paths(self):
        squaggle = ParametricCurve(lambda t: (0.5 * t + np.cos(t)) * RIGHT + np.sin(t) * UP, start=-np.pi, end=2 * np.pi)
        squaggle.shift(2 * UP)
...
class GeometryOfGlassSituation(ShowMultiplePathsInWater):
    ...
    def construct(self):
        ...
        kwargs = {'run_time': 4}
        self.play(*[Transform(*pair, **kwargs) for pair in [(top_line, end_top_line), (bottom_line, end_bottom_line), (left_brace, end_brace), (x_mob, end_x_mob)]] + [ShowCreation(graph, **kwargs)])
        self.wait()
...
class SpringSetup(ShowMultiplePathsInWater):
    ...
    def generate_springs(self, ring):
        self.start_springs, self.end_springs = [Mobject(Spring(self.start_point, r.get_top()), Spring(self.end_point, r.get_bottom())) for r in (ring, ring.copy().shift(self.ring_shift_val))]
    ...
    def show_angles(self, ring):
        ...
        top_spring, bottom_spring = self.start_springs.split()
        self.play(Transform(ring, Point(ring_center)), Transform(top_spring, lines[0]), Transform(bottom_spring, lines[1]))
    ...
    def show_equation(self):
        equation = OldTex(['\\left(\\dfrac{1}{\\phantom{v_air}}\\right)', '\\sin(\\theta_1)', '=', '\\left(\\dfrac{1}{\\phantom{v_water}}\\right)', '\\sin(\\theta_2)'])
        ...
        for f, frac in ((f1, frac1), (f2, frac2)):
            ...
            self.add(frac)
        self.wait()
        self.play(FadeOut(frac1), FadeOut(frac2), Transform(v_air, v_air_copy), Transform(v_water, v_water_copy), ShowCreation(bars), Transform(equals, new_eq))
        self.wait()
...
class StateSnellsLaw(PhotonScene):
    ...
    def construct(self):
        ...
        glass = Region(lambda x, y: y < 0, color=BLUE_E)
        self.add(glass)
        equation = OldTex(['\\dfrac{\\sin(\\theta_1)}{v_{\\text{air}}}', '=', '\\dfrac{\\sin(\\theta_2)}{v_{\\text{water}}}'])
        equation.to_corner(UP + RIGHT)
