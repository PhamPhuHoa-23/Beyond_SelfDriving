...
class MultilayeredGlass(PhotonScene, ZoomedScene):
    CONFIG = {'num_discrete_layers': 5, 'num_variables': 3, 'top_color': BLUE_E, 'bottom_color': BLUE_A, 'zoomed_canvas_frame_shape': (5, 5), 'square_color': GREEN_B}
    ...
    def get_continuous_background(self):
        glass = FilledRectangle(height=self.top - self.bottom, width=FRAME_WIDTH)
    ...
    def generate_layer_info(self):
        ...
        self.layer_colors = [Color(rgb=interpolate(top_rgb, bottom_rgb, alpha)) for alpha in np.arange(0, 1 + epsilon, epsilon)]
    ...
    def isolate_bend_points(self):
        ...
        for index in range(3):
            ...
            line = Line(bend_point + DOWN, bend_point + UP, color=WHITE, density=self.zoom_factor * DEFAULT_POINT_DENSITY_1D)
            angle_arcs = []
            ...
            self.play(ApplyMethod(little_square.shift, bend_point - little_square.get_center(), run_time=2))
    ...
    def show_snells(self, index, frame):
        ...
        for x, numerator in [(index, left), (index + 1, right)]:
            v, sqrt_y = [OldTex(text, size='\\Large').next_to(numerator, DOWN) for text in ('v_%d' % x, '\\sqrt{y_%d}' % x)]
            vs.append(v)
            sqrt_ys.append(sqrt_y)
        start, end = [Mobject(left.copy(), mobs[0], equals.copy(), right.copy(), mobs[1]).replace(frame) for mobs in (vs, sqrt_ys)]
        self.add(start)
        self.wait(2)
