...
class Intro(Scene):
    ...
    def construct(self):
        ...
        sort_by_color(logo)
        self.play(Transform(steve, steve_copy), DelayByOrder(Transform(logo, Point())), FadeOut(with_word), FadeOut(name_mob), run_time=3)
...
class IntroduceSteve(Scene):
    ...
    def construct(self):
        ...
        radio_lab, sci_fri, cornell, book2, book3, book4 = [ImageMobject(filename, invert=False, filter_color=WHITE) for filename in ['radio_lab', 'science_friday', 'cornell', 'strogatz_book2', 'strogatz_book3', 'strogatz_book4']]
...
class ShowTweets(Scene):
    ...
    def construct(self):
        ...
        for x in (1, 2):
            self.play(Transform(Point(tweets[x - 1].get_center()), tweets[x]), Animation(tweets[x - 1]))
        self.wait()
...
class CircleOfIdeas(Scene):
    ...
    def construct(self):
        ...
        self.play(*[Transform(word, word.copy().set_color(BLACK).center().scale(0.1), path_func=path_along_arc(np.pi), rate_func=linear, run_time=2) for word in displayed_words] + [GrowFromCenter(brachistochrone)])
        self.wait()
...
class VideoProgression(Scene):
    ...
    def construct(self):
        spacing = 2 * UP
        brachy, optics, light_in_two, snells, multi = words = [OldTexText(text) for text in ['Brachistochrone', 'Optics', 'Light in two media', "Snell's Law", 'Multilayered glass']]
        ...
        self.wait()
        self.play(FadeOut(light_in_two), GrowFromCenter(snells), DelayByOrder(ApplyMethod(arrow2.set_color, BLUE_D)))
        self.wait()
        self.play(FadeOut(optics), GrowFromCenter(multi), DelayByOrder(ApplyMethod(arrow1.set_color, BLUE_D)))
        self.wait()
