...
class MultilayeredScene(Scene):
    CONFIG = {'n_layers': 5, 'top_color': BLUE_E, 'bottom_color': BLUE_A, 'total_glass_height': 5, 'top': 3 * UP, 'RectClass': Rectangle}
    ...
    def get_continuous_glass(self):
        result = self.RectClass(width=FRAME_WIDTH, height=self.total_glass_height)
...
class TwoToMany(MultilayeredScene):
    ...
    def construct(self):
        ...
        self.add(glass)
        self.wait()
        self.play(*[FadeIn(layer, rate_func=squish_rate_func(smooth, x, 1)) for layer, x in zip(layers[1:], it.count(0, 0.2))] + [Transform(glass, layers[0])])
        self.wait()
...
class RaceLightInLayers(MultilayeredScene, PhotonScene):
    ...
    def construct(self):
        ...
        self.play(*[self.photon_run_along_path(line, rate_func=rate_maker(rate), run_time=2) for line, rate in zip(lines, rates)])
...
class ShowLayerVariables(MultilayeredScene, PhotonScene):
    ...
    def construct(self):
        ...
        self.wait()
        starts = [0, 0.3, 0.6]
        self.play(*it.chain(*[[GrowFromCenter(mob, rate_func=squish_rate_func(smooth, start, 1)) for mob, start in zip(mobs, starts)] for mobs in (start_ys, braces)]))
        self.wait()
        triplets = list(zip(v_equations, start_ys, end_ys))
        anims = []
        for v_eq, start_y, end_y in triplets:
            anims += [ShowCreation(v_eq[1]), ShowCreation(v_eq[2]), Transform(start_y.copy(), end_y)]
        self.play(*anims)
        self.wait()
...
class LimitingProcess(MultilayeredScene):
    ...
    def construct(self):
        ...
        glass_sets = [Mobject(*[Mobject(*layer_sets[x][2 ** x * index:2 ** x * (index + 1)]) for index in range(self.n_layers)]).ingest_submobjects() for x in range(num_iterations)]
...
class ContinuouslyObeyingSnellsLaw(MultilayeredScene):
    ...
    def show_equation(self, chopped_cycloid, ref_mob):
        ...
        equation = OldTex(['\\sin(\\theta)', '\\over \\sqrt{y}'])
