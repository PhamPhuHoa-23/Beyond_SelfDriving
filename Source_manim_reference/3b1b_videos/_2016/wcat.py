...
class ClosedLoopScene(Scene):
    ...
    def add_connecting_lines(self, cyclic=False):
        ...
        for d1, d2 in pairs:
            ...
            line.start_dot = d1
            line.end_dot = d2
            line.update_anim = UpdateFromFunc(line, lambda l: l.put_start_and_end_on(l.start_dot.get_center(), l.end_dot.get_center()))
    ...
    def let_dots_wonder(self, run_time=5, random_seed=None, added_anims=[]):
        ...
        anims = [MoveAlongPath(dot, self.loop, rate_func=generate_rate_func(start, rate)) for dot, start, rate in zip(self.dots, start_alphas, alpha_rates)]
    ...
    def move_dots_to_alphas(self, alphas, run_time=3):
        ...
        anims = [MoveAlongPath(dot, self.loop, rate_func=generate_rate_func(sa, a), run_time=run_time) for dot, sa, a in zip(self.dots, start_alphas, alphas)]
        anims += self.get_line_anims()
        self.play(*anims)
    ...
    def transform_loop(self, target_loop, added_anims=[], **kwargs):
        ...
        self.play(Transform(self.loop, target_loop), *dot_anims + self.get_line_anims() + added_anims, **kwargs)
    ...
    def square_scores(self, all_quads):
        midpoint_diffs = np.apply_along_axis(get_norm, 1, 0.5 * (all_quads[:, 0] + all_quads[:, 2]) - 0.5 * (all_quads[:, 1] + all_quads[:, 3]))
...
class WhenIWasAKid(TeacherStudentsScene):
    ...
    def state_excitement(self, children, speaker):
        ...
        me = children[-1]
        self.play(FadeOut(self.get_students()), FadeOut(self.get_teacher().bubble), FadeOut(self.get_teacher().bubble.content), Transform(self.get_teacher(), me))
    ...
    def run_class(self):
        ...
        self.random_blink()
        self.teacher_says('\n            Math! Excitement!\n            You are the future!\n            ', target_mode='hooray')
        self.play(pi1.look_at, pi2.eyes, pi1.change_mode, 'erm', pi2.look_at, pi1.eyes, pi2.change_mode, 'surprised')
        self.play(pi3.look_at, me.eyes, pi3.change_mode, 'sassy', me.look_at, pi3.eyes)
        self.random_blink(2)
        self.play(self.teacher.change_mode, 'speaking', FadeOut(self.teacher.bubble), FadeOut(self.teacher.bubble.content))
        self.play(Write(title))
        self.random_blink()
        self.play(pi1.change_mode, 'raise_right_hand')
        self.random_blink()
        self.play(pi2.change_mode, 'confused', pi3.change_mode, 'happy', pi2.look_at, pi3.eyes, pi3.look_at, pi2.eyes)
        self.random_blink()
        ...
        self.random_blink()
        self.student_says('How is this math?', index=-1, target_mode='pleading', width=5, height=3, direction=RIGHT)
        self.play(pi1.change_mode, 'pondering', pi2.change_mode, 'pondering', pi3.change_mode, 'pondering')
    ...
    def grow_up(self):
        ...
        morty.to_edge(LEFT)
        morty.look(RIGHT)
        self.play(Transform(me, morty), *list(map(FadeOut, [self.students, self.teacher, me.bubble, me.bubble.content])))
        self.remove(me)
        self.add(morty)
...
class DefineInscribedSquareProblem(ClosedLoopScene):
    ...
    def draw_loop(self):
        ...
        self.play(ShowCreation(self.connecting_lines, lag_ratio=0, run_time=2), Animation(self.dots))
        self.wait(2)
    ...
    def cycle_through_shapes(self):
        ...
        randy_anims = [FadeIn(randy), Animation(randy), Blink(randy), Animation(randy), Blink(randy), Animation(randy), Blink(randy, rate_func=smooth)]
    ...
    def ask_about_rectangles(self):
        ...
        for pair, line in zip(dot_pairs, diag_lines):
            self.play(FadeIn(line), pair.set_color, line.get_color())
...
class RectangleProperties(Scene):
    ...
    def construct(self):
        ...
        a, b, c, d = labels = VGroup(*[OldTex(s).next_to(dot, dot.get_center(), buff=SMALL_BUFF) for s, dot in zip('abcd', vertex_dots)])
        midpoint = Dot(ORIGIN, color=RED)
        self.play(ShowCreation(rect))
        self.wait()
        self.play(ShowCreation(vertex_dots), Write(labels))
        self.wait()
        mob_lists = [(a, c, dot_pairs[0]), (b, d, dot_pairs[1])]
...
class PairOfPairBecomeRectangle(Scene):
    ...
    def construct(self):
        dots = VGroup(Dot(4 * RIGHT + 0.5 * DOWN, color=MAROON_B), Dot(5 * RIGHT + 3 * UP, color=MAROON_B), Dot(LEFT + 0.1 * DOWN, color=PURPLE_B), Dot(2 * LEFT + UP, color=PURPLE_B))
        labels = VGroup()
        for dot, char in zip(dots, 'acbd'):
            ...
            labels.add(label)
        lines = [Line(dots[i].get_center(), dots[j].get_center(), color=dots[i].get_color()) for i, j in [(0, 1), (2, 3)]]
        groups = [VGroup(dots[0], dots[1], labels[0], labels[1], lines[0]), VGroup(dots[2], dots[3], labels[2], labels[3], lines[1])]
        midpoint = Dot(LEFT, color=RED)
        words = VGroup(*list(map(TexText, ['Common midpoint', 'Same distance apart', '$\\Downarrow$', 'Rectangle'])))
        words.arrange(DOWN)
        words.to_edge(RIGHT)
...
class SearchForRectangleOnLoop(ClosedLoopScene):
    ...
    def construct(self):
        ...
        self.play(*[ApplyMethod(line.rotate, angle_mean - angle) for line, angle in zip(self.connecting_lines, angles)] + [Animation(midpoint)], rate_func=there_and_back)
...
class PairOfRealsToPlane(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(pair_label.next_to, two_d_point, UP + LEFT, SMALL_BUFF, Rotate(y_axis, np.pi / 2), Rotate(dot2, np.pi / 2), FadeOut(arrows))
...
class RepresentPairInUnitSquare(ClosedLoopScene):
    ...
    def construct(self):
        ...
        right_words = VGroup(*[OldTexText('Pair of\\\\ loop points'), OldTex('\\Downarrow'), OldTexText('Point in \\\\ unit square')])
        ...
        for mob in movers:
            mob.generate_target()
        shift_vals = [RIGHT + DOWN, LEFT + DOWN, LEFT + 2 * UP, 3 * DOWN, 2 * RIGHT + UP, RIGHT + UP, 3 * LEFT + 3 * DOWN]
...
class EdgesOfSquare(Scene):
    ...
    def construct(self):
        ...
        for arrows in arrow_groups:
            self.play(ShowCreation(arrows, run_time=2))
            self.wait()
        self.play(*[ApplyMethod(n.next_to, square.get_corner(vect + LEFT), LEFT, MED_SMALL_BUFF, path_arc=np.pi / 2) for n, vect in zip(self.numbers, [DOWN, UP])])
        self.wait()
    ...
    def get_edges(self, square):
        y_edges = VGroup(*[Line(square.get_corner(vect + LEFT), square.get_corner(vect + RIGHT)) for vect in (DOWN, UP)])
        y_edges.set_color(BLUE)
        x_edges = VGroup(*[Line(square.get_corner(vect + DOWN), square.get_corner(vect + UP)) for vect in (LEFT, RIGHT)])
        x_edges.set_color(MAROON_B)
        return (x_edges, y_edges)

    def get_coordinate_labels(self, square):
        alpha_range = np.arange(0, 1.1, 0.1)
        dot_groups = [VGroup(*[Dot(interpolate(square.get_corner(DOWN + vect), square.get_corner(UP + vect), alpha)) for alpha in alpha_range]) for vect in (LEFT, RIGHT)]
    ...
    def get_arrows(self, x_edges, y_edges):
        ...
        return [VGroup(*[VGroup(*[Arrow(edge.point_from_proportion(a1), edge.point_from_proportion(a2), buff=0) for a1, a2 in zip(alpha_range, alpha_range[1:])]) for edge in edges]).set_color(edges.get_color()) for edges in (x_edges, y_edges)]
...
class EndpointsGluedTogether(ClosedLoopScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.transform_loop(original_loop, added_anims=[ApplyMethod(dot.move_to, original_loop.get_points()[0]) for dot in dots], run_time=3)
        self.wait()
...
class TorusPlaneAnalogy(ClosedLoopScene):
    ...
    def construct(self):
        ...
        number_line.add(Dot(number_line.number_to_point(3), color=GREEN), Dot(number_line.number_to_point(-2), color=RED))
        self.wait()
...
class TrivialPairCollision(ClosedLoopScene):
    ...
    def construct(self):
        ...
        for mob in swappers:
            mob.save_state()
        self.play(a.move_to, b, b.move_to, a, a_label.next_to, b, LEFT, b_label.next_to, a, RIGHT, randy.look_at, a, path_arc=np.pi)
        self.play(ShowCreation(midpoint))
...
class NotHelpful(Scene):
    ...
    def construct(self):
        ...
        self.add(morty)
        self.play(FadeIn(bubble), FadeIn(bubble.content), morty.change_mode, 'angry', morty.look, OUT)
        self.play(Blink(morty))
        self.wait()
...
class FoldUnitSquare(EdgesOfSquare):
    ...
    def add_triangles(self):
        square = self.add_square()
        triangles = VGroup(*[Polygon(*[square.get_corner(vect) for vect in vects]) for vects in [(DOWN + LEFT, UP + RIGHT, UP + LEFT), (DOWN + LEFT, UP + RIGHT, DOWN + RIGHT)]])
        ...
        for a in (0, 1):
            for color in colors:
                ...
                for b1, b2 in zip(b_range, b_range[1:]):
                    arrow = Arrow(self.get_point_from_coords(a, b1), self.get_point_from_coords(a, b2), buff=0, color=color)
                    if color is BLUE:
    ...
    def perform_fold(self):
        diag_line = DashedLine(self.square.get_corner(DOWN + LEFT), self.square.get_corner(UP + RIGHT), color=RED)
        self.play(ShowCreation(diag_line))
        self.wait()
        self.play(Transform(*self.triangles), Transform(self.start_dots, self.end_dots), Transform(self.start_arrows, self.end_arrows))
        self.wait()
    ...
    def show_singleton_pairs(self):
        ...
        for x in xs:
            ...
            if old_label is None:
                ...
                self.play(Transform(old_dot, dot), Transform(old_label, label))
            self.wait()
    ...
    def ask_about_gluing(self):
        ...
        self.play(randy.change_mode, 'pondering', randy.look_at, keepers, ShowCreation(bubble), Write(bubble.content))
        self.play(Blink(randy))
        self.wait()
        self.randy = randy

    def clarify_edge_gluing(self):
        dots = VGroup(*[Dot(self.get_point_from_coords(*coords), radius=0.1) for coords in [(0.1, 0), (1, 0.1), (0.9, 0), (1, 0.9)]])
        dots.scale(0.6)
...
class PrepareForMobiusStrip(Scene):
    ...
    def add_triangles(self):
        triangles = VGroup(Polygon(DOWN + LEFT, DOWN + RIGHT, ORIGIN), Polygon(DOWN + RIGHT, UP + RIGHT, ORIGIN))
        ...
        for tri in triangles:
            ...
            alpha_range = np.linspace(0, 1, 4)
            arrows = VGroup(*[Arrow(interpolate(anchors[0], anchors[1], a), interpolate(anchors[0], anchors[1], b), buff=0, color=arrows_color) for a, b in zip(alpha_range, alpha_range[1:])])
            ...
            cut_arrows = VGroup(*[Arrow(interpolate(start, end, a), interpolate(start, end, b), buff=0, color=YELLOW) for a, b in zip(alpha_range, alpha_range[1:])])
    ...
    def perform_cut(self):
        ...
        self.wait()
        self.play(tri1.shift, (DOWN + LEFT) / 2.0, tri2.shift, (UP + RIGHT) / 2.0)
        self.wait()

    def rearrange_pieces(self):
        tri1, tri2 = self.triangles
        self.play(tri1.rotate, np.pi, UP + RIGHT, tri1.next_to, ORIGIN, RIGHT, tri2.next_to, ORIGIN, LEFT)
        self.wait()
...
class PatreonThanks(Scene):
    CONFIG = {'specific_patrons': ['Loo Yu Jun', 'Tom', 'Othman Alikhan', 'Juan Batiz-Benet', 'Markus Persson', 'Joseph John Cox', 'Achille Brighton', 'Kirk Werklund', 'Luc Ritchie', 'Ripta Pasay', 'PatrickJMT  ', 'Felipe Diniz']}
...
class CreditThree(Scene):
    ...
    def construct(self):
        ...
        domains = VGroup(*list(map(TexText, ['visualnumbertheory.com', 'buymywidgets.com', 'learnwhatilearn.com'])))
        ...
        self.play(Blink(randy))
        self.play(Transform(bubble, VectorizedPoint(randy.get_corner(UP + LEFT))), randy.change_mode, 'sad')
        self.wait()
