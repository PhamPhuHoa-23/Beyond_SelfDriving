...
class UpcomingSeriesOfVidoes(Scene):
    ...
    def construct(self):
        ...
        self.play(*[FadeIn(icon, run_time=5, rate_func=rate_func_creator(offset)) for icon, offset in zip(icons, np.linspace(0, 0.5, len(icons)))])
        self.wait()
...
class AboutLinearAlgebra(Scene):
    ...
    def show_dependencies(self):
        ...
        subjects = list(map(TexText, ['Computer science', 'Physics', 'Electrical engineering', 'Mechanical engineering', 'Statistics', '\\vdots']))
        prev = subjects[0]
    ...
    def to_thought_bubble(self):
        ...
        self.wait()
        topics = [self.get_matrix_multiplication(), self.get_determinant(), self.get_cross_product(), self.get_eigenvalue()]
        questions = [self.get_matrix_multiplication_question(), self.get_cross_product_question(), self.get_eigen_question()]
    ...
    def get_eigen_question(self):
        result = OldTexText("What the heck \\\\ does ``eigen'' mean?")
...
class NumericVsGeometric(Scene):
    ...
    def specifics_concepts(self):
        matrix_vector_product = OldTex(' '.join([matrix_to_tex_string(EXAMPLE_TRANFORM), matrix_to_tex_string(TRANFORMED_VECTOR), '&=', matrix_to_tex_string([['1 \\cdot 1 + 0 \\cdot 2'], ['1 \\cdot 1 + (-1)\\cdot 2']]), '\\\\ &=', matrix_to_tex_string([[1], [-1]])]))
    ...
    def clear_way_for_geometric(self):
        ...
        self.play(Transform(self.vline, new_line), Transform(self.hline, new_line), ApplyMethod(self.numeric.shift, (FRAME_HEIGHT + 1) * DOWN), ApplyMethod(self.matrix_vector_product.shift, (FRAME_HEIGHT + 1) * DOWN), ApplyMethod(self.geometric.to_edge, LEFT))
    ...
    def list_geometric_benefits(self):
        ...
        randys = [Randolph(mode='speaking'), Randolph(mode='surprised'), Randolph(mode='pondering')]
        bulb = SVGMobject('light_bulb')
        bulb.set_height(1)
        bulb.set_color(YELLOW)
        thoughts = [matrix_to_mobject(EXAMPLE_TRANFORM), bulb, OldTexText('So therefore...').scale(0.5)]
...
class LinAlgPyramid(Scene):
    ...
    def list_applications(self, top_mob):
        subjects = [OldTexText(word).to_corner(UP + RIGHT) for word in ['computer science', 'engineering', 'statistics', 'economics', 'pure math']]
...
class ThoughtBubbleTransformation(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.apply_matrix(np.linalg.inv(rotation), path_arc=-np.pi / 3)
        self.apply_matrix(EXAMPLE_TRANFORM)
        self.apply_matrix(rotation, path_arc=np.pi / 3)
        self.wait()
...
class PhysicsExample(Scene):
    ...
    def projectile(self, parabola):
        ...
        self.play(MoveAlongPath(dot, parabola.copy(), **kwargs), ShowCreation(parabola, **kwargs))
        self.wait()
...
class LinearAlgebraIntuitions(Scene):
    ...
    def construct(self):
        ...
        intuitions = ['Matrices transform space', 'Matrix multiplication corresponds to applying ' + 'one transformation after another', 'The determinant gives the factor by which areas change']
...
class MatrixMultiplicationIs(Scene):
    ...
    def construct(self):
        ...
        self.play(Write(matrix1), ShowCreation(arrow), GrowFromCenter(brace1), Write(apply_first), run_time=1)
        self.wait()
...
class ProfessorsTry(Scene):
    ...
    def construct(self):
        ...
        speech_bubble.shift(RIGHT)
        words = OldTexText('It really is beautiful!  I want you to \\\\' + 'see it the way I do...')
        ...
        randy.to_corner()
        self.add(randy, morty)
        self.play(ApplyMethod(morty.change_mode, 'speaking'), FadeIn(speech_bubble), FadeIn(words))
        self.play(Blink(randy))
        self.play(FadeIn(thought_bubble))
...
class TableOfContents(Scene):
    ...
    def construct(self):
        ...
        self.play(ApplyMethod(entry3.set_color, YELLOW), ShowCreation(arrow), Write(added_words), run_time=1)
        self.wait()
    ...
    def series_of_videos(self, chapters):
        ...
        self.play(Transform(chapters, icons, path_arc=np.pi / 2))
        self.clear()
        self.add(icons)
...
class DifferingBackgrounds(Scene):
    ...
    def construct(self):
        words = list(map(TexText, ['Just brushing up', 'Has yet to take the course', 'Supplementing course concurrently']))
