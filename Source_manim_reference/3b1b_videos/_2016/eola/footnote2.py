...
class OpeningQuote(Scene):
    ...
    def construct(self):
        words = OldTexText('``On this quiz, I asked you to find the determinant of a', '2x3 matrix.', "Some of you, to my great amusement, actually tried to do this.''")
...
class ColumnsRepresentBasisVectors(Scene):
    ...
    def put_in_thought_bubble(self):
        ...
        self.play(ApplyFunction(lambda m: bubble.position_mobject_inside(m.set_height(2.5)), everything), ShowCreation(bubble), randy.change_mode, 'pondering')
...
class Symbolic2To3DTransform(Scene):
    ...
    def construct(self):
        ...
        self.play(Transform(mover, Dot().move_to(func)), ShowCreation(in_arrow), rate_func=rush_into)
        self.play(Transform(mover, output_array), ShowCreation(out_arrow), rate_func=rush_from)
...
class DescribeColumnsInSpecificTransformation(Scene):
    ...
    def construct(self):
        ...
        for col, char, vect in zip([i_col, j_col], ['i', 'j'], [UP, DOWN]):
            ...
            col.arrow = Arrow(col.words.get_edge_center(-vect), col.get_edge_center(vect), color=color)
...
class CountRowsAndColumns(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(rows_words[0].copy().move_to, title[0], cols_words[0].copy().move_to, title[2], Write(VMobject(title[1], title[3])))
        self.wait()
...
class MatrixInTheWild(Scene):
    ...
    def construct(self):
        ...
        self.play(GrowFromCenter(top_brace), Write(top_words), matrix.set_column_colors, X_COLOR, Y_COLOR, Z_COLOR)
...
class ThreeDToTwoDOutput(LinearTransformationScene):
    CONFIG = {'show_basis_vectors': False, 'foreground_plane_kwargs': {'color': GREY, 'x_radius': FRAME_X_RADIUS, 'y_radius': FRAME_Y_RADIUS, 'secondary_line_ratio': 0}}
    ...
    def construct(self):
        ...
        self.add(title)
        i, j, k = it.starmap(self.add_vector, [([3, 1], X_COLOR), ([1, 2], Y_COLOR), ([-2, -2], Z_COLOR)])
...
class TwoDTo1DTransform(LinearTransformationScene):
    CONFIG = {'include_background_plane': False, 'foreground_plane_kwargs': {'x_radius': FRAME_X_RADIUS, 'y_radius': FRAME_Y_RADIUS, 'secondary_line_ratio': 1}, 't_matrix': [[1, 0], [2, 0]]}
...
class TwoDTo1DTransformWithDots(TwoDTo1DTransform):
    ...
    def construct(self):
        ...
        self.apply_transposed_matrix(self.t_matrix, added_anims=[Transform(dots, new_dots)])
        self.play(Write(words))
        self.wait()
...
class DotProductPreview(VectorScene):
    CONFIG = {'v_coords': [3, 1], 'w_coords': [2, -1], 'v_color': YELLOW, 'w_color': MAROON_C}
    ...
    def grow_number_line(self):
        ...
        self.play(line.set_color, self.v.get_color(), Animation(self.v), rate_func=there_and_back)
        self.wait()

    def project_w(self):
        dot_product = np.dot(self.v.get_end(), self.w.get_end())
        v_norm, w_norm = [get_norm(vect.get_end()) for vect in (self.v, self.w)]
        projected_w = Vector(self.v.get_end() * dot_product / v_norm ** 2, color=self.w.get_color())
    ...
    def show_scaling(self):
        dot_product = np.dot(self.v.get_end(), self.w.get_end())
        start_brace, interim_brace, final_brace = braces = [Brace(Line(ORIGIN, norm * RIGHT), UP) for norm in (1, self.v.get_length(), dot_product)]
        length_texs = list(it.starmap(Tex, [('1',), ('\\text{Scale by }', '||\\vec{\\textbf{v}}||'), ('\\text{Length of}', '\\text{ scaled projection}')]))
