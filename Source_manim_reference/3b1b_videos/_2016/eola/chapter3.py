...
class OpeningQuote(Scene):
    ...
    def construct(self):
        words = OldTexText(['Unfortunately, no one can be told what the', 'Matrix', 'is. You have to', 'see it for yourself.'])
...
class Introduction(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(ApplyPointwiseFunction(spread_out, everything), ApplyFunction(lambda m: m.center().to_edge(UP), linear_transformations))
...
class WhyConfuseWithTerminology(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.random_blink()
        self.wait()
        statement = OldTexText(['The word', "``transformation''", 'suggests \\\\ that you think using', 'movement'])
...
class ThinkinfOfFunctionsAsGraphs(VectorScene):
    ...
    def construct(self):
        ...
        self.play(ApplyPointwiseFunction(collapse_func, axes), ApplyPointwiseFunction(collapse_func, graph), ApplyMethod(point.shift, 10 * DOWN), ApplyMethod(point_label.shift, 10 * DOWN), ApplyPointwiseFunction(collapse_func, name), run_time=2)
        self.clear()
...
class TransformJustOneVector(VectorScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.remove(v2)
        self.play(Transform(v1.copy(), v2, path_arc=-np.pi / 2, run_time=3), ApplyMethod(v1.fade))
        self.wait()

class TransformManyVectors(LinearTransformationScene):
    CONFIG = {'transposed_matrix': [[2, 1], [1, 2]], 'use_dots': False}

    def construct(self):
        self.lock_in_faded_grid()
        vectors = VMobject(*[Vector([x, y]) for x in np.arange(-int(FRAME_X_RADIUS) + 0.5, int(FRAME_X_RADIUS) + 0.5) for y in np.arange(-int(FRAME_Y_RADIUS) + 0.5, int(FRAME_Y_RADIUS) + 0.5)])
        vectors.set_submobject_colors_by_gradient(PINK, YELLOW)
        t_matrix = self.transposed_matrix
        transformed_vectors = VMobject(*[Vector(np.dot(np.array(t_matrix).transpose(), v.get_end()[:2]), color=v.get_color()) for v in vectors.split()])
...
class TransformInfiniteGrid(LinearTransformationScene):
    CONFIG = {'include_background_plane': False, 'foreground_plane_kwargs': {'x_radius': FRAME_WIDTH, 'y_radius': FRAME_HEIGHT}, 'show_basis_vectors': False}
...
class TransformInfiniteGridWithBackground(TransformInfiniteGrid):
    CONFIG = {'include_background_plane': True, 'foreground_plane_kwargs': {'x_radius': FRAME_WIDTH, 'y_radius': FRAME_HEIGHT, 'secondary_line_ratio': 0}}

class ApplyComplexFunction(LinearTransformationScene):
    CONFIG = {'function': lambda z: 0.5 * z ** 2, 'show_basis_vectors': False, 'foreground_plane_kwargs': {'x_radius': FRAME_X_RADIUS, 'y_radius': FRAME_Y_RADIUS, 'secondary_line_ratio': 0}}

    def construct(self):
        self.setup()
        self.plane.prepare_for_nonlinear_transform(100)
        self.wait()
        self.play(ApplyMethod(self.plane.apply_complex_function, self.function, run_time=5, path_arc=np.pi / 2))
        self.wait()

class ExponentialTransformation(ApplyComplexFunction):
    CONFIG = {'function': np.exp}
...
class LookToWordLinear(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Transform(faded_linear, linear), Transform(transformation, faded_transformation), Transform(transformation_brace, linear_brace), Transform(function, new_sub_word), lag_ratio=0.5)
        self.wait()
...
class SneakyNonlinearTransformationExplained(SneakyNonlinearTransformation):
    ...
    def setup(self):
        LinearTransformationScene.setup(self)
        diag = Line(FRAME_Y_RADIUS * LEFT + FRAME_Y_RADIUS * DOWN, FRAME_Y_RADIUS * RIGHT + FRAME_Y_RADIUS * UP)
...
class GridLinesRemainParallel(SimpleLinearTransformationScene):
    ...
    def construct(self):
        ...
        text = OldTexText(['Grid lines remain', 'parallel', 'and', 'evenly spaced'])
...
class Rotation(SimpleLinearTransformationScene):
    CONFIG = {'angle': np.pi / 3}

    def construct(self):
        self.transposed_matrix = [[np.cos(self.angle), np.sin(self.angle)], [-np.sin(self.angle), np.cos(self.angle)]]
        SimpleLinearTransformationScene.construct(self)

class YetAnotherLinearTransformation(SimpleLinearTransformationScene):
    CONFIG = {'transposed_matrix': [[-1, 1], [3, 2]]}
    ...
    def construct(self):
        ...
        formula = OldTex([matrix_to_tex_string(['x_\\text{in}', 'y_\\text{in}']), '\\rightarrow ???? \\rightarrow', matrix_to_tex_string(['x_\\text{out}', 'y_{\\text{out}}'])])
        formula.add_background_rectangle()
        self.play(Write(words))
        self.wait()
        self.play(ApplyMethod(self.plane.fade, 0.7), ApplyMethod(self.background_plane.fade, 0.7), Write(formula, run_time=2), Animation(words))
        self.wait()
...
class TrackBasisVectorsExample(LinearTransformationScene):
    CONFIG = {'transposed_matrix': [[1, -2], [3, 0]], 'v_coords': [-1, 2], 'v_coord_strings': ['-1', '2'], 'result_coords_string': '\n            =\n            \\left[ \\begin{array}{c}\n                -1(1) + 2(3) \\\\\n                -1(-2) + 2(0)\n            \\end{arary}\\right] \n            = \n            \\left[ \\begin{array}{c}\n                5 \\\\\n                2\n            \\end{arary}\\right] \n        '}
    ...
    def label_bases(self):
        triplets = [(self.i_hat, '\\hat{\\imath}', X_COLOR), (self.j_hat, '\\hat{\\jmath}', Y_COLOR)]
        label_mobs = []
        for vect, label, color in triplets:
            label_mobs.append(self.add_transformable_label(vect, label, '\\text{Transformed } ' + label, color=color, direction='right'))
    ...
    def introduce_vector(self):
        ...
        pre_def = VMobject(VectorizedPoint(coords.get_center()), VMobject(*[mob.copy() for mob in coords.get_mob_matrix().flatten()]))
    ...
    def get_v_definition(self):
        v_def = OldTex(['\\vec{\\textbf{v}}', ' = %s' % self.v_coord_strings[0], '\\hat{\\imath}', '+%s' % self.v_coord_strings[1], '\\hat{\\jmath}'])
    ...
    def write_linear_map_rule(self):
        rule = OldTex(['\\text{Transformed } \\vec{\\textbf{v}}', ' = %s' % self.v_coord_strings[0], '(\\text{Transformed }\\hat{\\imath})', '+%s' % self.v_coord_strings[1], '(\\text{Transformed } \\hat{\\jmath})'])
    ...
    def show_basis_vector_coords(self):
        ...
        calculation = OldTex([' = %s' % self.v_coord_strings[0], matrix_to_tex_string(self.transposed_matrix[0]), '+%s' % self.v_coord_strings[1], matrix_to_tex_string(self.transposed_matrix[1])])
...
class WatchManyVectorsMove(TransformManyVectors):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ApplyMethod(self.plane.fade), FadeOut(vectors), Animation(self.i_hat), Animation(self.j_hat))
        self.wait()
...
class DeduceResultWithGeneralCoordinates(Scene):
    ...
    def construct(self):
        ...
        result.show()
        vect_group = VMobject(vect, rto, x.copy(), i_coords.copy(), plus, y.copy(), j_coords.copy(), equals, result)
...
class MatrixVectorMultiplication(LinearTransformationScene):
    ...
    def build_to_matrix(self):
        ...
        self.play(Transform(VMobject(*abstract_matrix.flatten()), VMobject(*concrete_matrix.get_mob_matrix().flatten())), Transform(i_brackets, matrix_brackets), Transform(j_brackets, matrix_brackets), run_time=2, lag_ratio=0)
        everything = VMobject(*self.get_mobjects())
    ...
    def label_matrix(self, matrix):
        ...
        self.play(Write(i_message, run_time=1.5), ShowCreation(i_arrow))
        self.wait()
        self.play(ShowCreation(j_circle))
        self.play(Write(j_message, run_time=1.5), ShowCreation(j_arrow))
        self.wait()
        self.play(*list(map(FadeOut, [i_message, i_circle, i_arrow, j_message, j_circle, j_arrow])))
    ...
    def multiply_by_vector(self, matrix):
        ...
        formula.center()
        formula_start = VMobject(v1.copy(), VMobject(*matrix.copy().get_mob_matrix()[:, 0]), VectorizedPoint(), v2.copy(), VMobject(*matrix.copy().get_mob_matrix()[:, 1]))
        self.play(FadeOut(brace), FadeOut(words), Transform(formula_start, formula, run_time=2, lag_ratio=0))
        self.wait()
...
class ColumnsToBasisVectors(LinearTransformationScene):
    ...
    def move_matrix_columns(self, transposed_matrix, vector_coords=None):
        ...
        i_coords_end = VMobject(i_label_background, i_coords, i_brackets)
        j_coords_start = VMobject(matrix_background.copy(), col2.copy(), matrix_brackets.copy())
        j_coords_end = VMobject(j_label_background, j_coords, j_brackets)
        transform_matrix1 = np.array(matrix)
        transform_matrix1[:, 1] = [0, 1]
        transform_matrix2 = np.dot(matrix, np.linalg.inv(transform_matrix1))
        self.wait()
        self.apply_transposed_matrix(transform_matrix1.transpose(), added_anims=[Transform(i_coords_start, i_coords_end)], path_arc=np.pi / 2)
        self.add_foreground_mobject(i_coords_start)
        self.apply_transposed_matrix(transform_matrix2.transpose(), added_anims=[Transform(j_coords_start, j_coords_end)], path_arc=np.pi / 2)
    ...
    def scale_and_add(self, vector, vector_coords):
        ...
        self.play(Transform(i_copy, i_target), ApplyMethod(coord1.next_to, i_target.get_center(), DOWN))
        self.play(Transform(j_copy, j_target), ApplyMethod(coord2.next_to, j_target.get_center(), LEFT))
        j_copy.add(coord2)
...
class Describe90DegreeRotation(LinearTransformationScene):
    CONFIG = {'transposed_matrix': [[0, 1], [-1, 0]], 'title': '$90^\\circ$ rotation counterclockwise'}
    ...
    def construct(self):
        ...
        for vect, color, col in [(self.i_hat, X_COLOR, col1), (self.j_hat, Y_COLOR, col2)]:
            ...
            self.wait()
            self.play(ShowCreation(background, rate_func=lambda t: smooth(1 - t)), ApplyMethod(coords.replace, col), FadeOut(brackets))
            self.remove(label)
...
class DescribeShear(Describe90DegreeRotation):
    CONFIG = {'transposed_matrix': [[1, 0], [1, 1]], 'title': "``Shear''"}
...
class TransformationsAreFunctions(Scene):
    ...
    def construct(self):
        title = OldTexText(['Linear transformations are a\n            special kind of', 'function'])
        title_start, function = title.split()
        function.set_color(YELLOW)
        title.to_edge(UP)
        equation = OldTex(['L', '(', '\\vec{\\textbf{v}}', ') = ', '\\vec{\\textbf{w}}'])
...
class UsedToThinkinfOfFunctionsAsGraphs(VectorScene):
    ...
    def show_graph(self):
        ...
        def collapse_func(p):
            return np.dot(p, [RIGHT, RIGHT, OUT]) + (FRAME_Y_RADIUS + 1) * DOWN
        self.play(ApplyPointwiseFunction(collapse_func, axes, lag_ratio=0), ApplyPointwiseFunction(collapse_func, graph), ApplyMethod(point.shift, 10 * DOWN), ApplyMethod(point_label.shift, 10 * DOWN), ApplyFunction(lambda m: m.center().to_edge(UP), name), run_time=1)
        self.clear()
        self.add(name)
        self.wait()
...
class TryingToVisualizeFourDimensions(Scene):
    ...
    def construct(self):
        ...
        self.play(ApplyMethod(randy.change_mode, 'pondering'), ShowCreation(bubble), Write(thought))
...
class ThinkAboutFunctionAsMovingVector(LinearTransformationScene):
    CONFIG = {'show_basis_vectors': False, 'leave_ghost_vectors': True}
...
class AdditivityProperty(LinearTransformationScene):
    CONFIG = {'show_basis_vectors': False, 'give_title': True, 'transposed_matrix': [[2, 0], [1, 1]], 'nonlinear_transformation': None, 'vector_v': [2, 1], 'vector_w': [1, -2], 'proclaim_sum': True}
    ...
    def draw_all_vectors(self):
        ...
        sum_label = self.add_transformable_label(sum_vect, '%s + %s' % (v_label.expression, w_label.expression), rotate=True)
...
class NonlinearLacksAdditivity(AdditivityProperty):
    CONFIG = {'give_title': False, 'nonlinear_transformation': curvy_squish, 'vector_v': [3, 2], 'vector_w': [2, -3], 'proclaim_sum': False}

class SecondAdditivityExample(AdditivityProperty):
    CONFIG = {'give_title': False, 'transposed_matrix': [[1, -1], [2, 1]], 'vector_v': [-2, 2], 'vector_w': [3, 0], 'proclaim_sum': False}
...
class MoveAroundAllVectors(LinearTransformationScene):
    CONFIG = {'show_basis_vectors': False, 'focus_on_one_vector': False, 'include_background_plane': False}
...
class ReasonForThinkingAboutArrows(LinearTransformationScene):
    ...
    def scale_and_add(self, vectors):
        ...
        self.play(ApplyMethod(v.scale, 0.5), ApplyMethod(w.scale, 1.5))
