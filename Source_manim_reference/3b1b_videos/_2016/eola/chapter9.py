...
class Jennifer(PiCreature):
    CONFIG = {'color': PINK, 'start_corner': DOWN + LEFT}

class You(PiCreature):
    CONFIG = {'color': BLUE_E, 'start_corner': DOWN + RIGHT, 'flip_at_start': True}

def get_small_bubble(pi_creature, height=4, width=3):
    pi_center_x = pi_creature.get_center()[0]
    kwargs = {'height': 4, 'bubble_center_adjustment_factor': 1.0 / 6}
...
class OpeningQuote(Scene):
    ...
    def construct(self):
        words = OldTexText('\\centering ``Mathematics is the art of giving the \\\\', 'same name ', 'to ', 'different things', ".''", arg_separator=' ')
...
class LinearCombinationScene(LinearTransformationScene):
    CONFIG = {'include_background_plane': False, 'foreground_plane_kwargs': {'x_radius': FRAME_X_RADIUS, 'y_radius': FRAME_Y_RADIUS, 'secondary_line_ratio': 1}}
    ...
    def show_linear_combination(self, numerical_coords, basis_vectors, coord_mobs, revert_to_original=True, show_sum_vect=False, sum_vect_color=V_COLOR):
        ...
        if revert_to_original:
            self.play(*it.chain([basis.restore for basis in basis_vectors], [basis.label.restore for basis in basis_vectors], [FadeOut(coord) for coord in coord_mobs], [FadeOut(sum_vect) for x in [1] if show_sum_vect]))
        if show_sum_vect:
            return sum_vect
...
class RemindOfCoordinates(LinearCombinationScene):
    ...
    def show_standard_coord_meaning(self, x_coord, y_coord):
        ...
        for line, coord, direction in ((x_line, x_coord, DOWN), (y_line, y_coord, LEFT)):
            self.play(coord.set_color, line.get_color(), coord.next_to, line.get_center(), direction, ShowCreation(line))
            self.wait()
        self.wait()
    ...
    def show_abstract_scalar_idea(self, x_coord, y_coord):
        ...
        self.play(FadeOut(everything), x_coord.scale, 1.5, x_coord.move_to, x_shift + 3 * UP, y_coord.scale, 1.5, y_coord.move_to, y_shift + 3 * UP, Write(words))
    ...
    def scale_basis_vectors(self, x_coord, y_coord):
        ...
        self.show_linear_combination(self.vector_coords, basis_vectors=[self.i_hat, self.j_hat], coord_mobs=[x_coord, y_coord])
    ...
    def list_implicit_assumptions(self, x_coord, y_coord):
        ...
        self.wait()
        self.play(Write(ass1), ApplyFunction(lambda m: m.rotate(np.pi / 6).set_color(X_COLOR), x_coord, rate_func=wiggle))
        self.wait()
        self.play(Write(ass2), ApplyFunction(lambda m: m.rotate(np.pi / 6).set_color(Y_COLOR), y_coord, rate_func=wiggle))
        self.wait()
...
class JenniferScene(LinearCombinationScene):
    CONFIG = {'b1_coords': [2, 1], 'b2_coords': [-1, 1], 'foreground_plane_kwargs': {'x_radius': FRAME_X_RADIUS, 'y_radius': FRAME_X_RADIUS}}
...
class IntroduceJennifer(JenniferScene):
    ...
    def show_v_from_both_perspectives(self):
        ...
        self.wait()
        self.play(*it.chain(list(map(FadeIn, [self.plane, self.i_hat, self.j_hat, self.i_hat.label, self.j_hat.label, you])), list(map(Animation, [jenny, v])), list(map(FadeOut, self.basis_vectors))))
        self.play(ShowCreation(you.bubble), Write(you.coords))
        self.play(you.change_mode, 'speaking')
        self.show_linear_combination(self.v_coords, basis_vectors=[self.i_hat, self.j_hat], coord_mobs=you.coords.get_entries().copy())
        self.play(*it.chain(list(map(FadeOut, [self.plane, self.i_hat, self.j_hat, self.i_hat.label, self.j_hat.label, you.bubble, you.coords])), list(map(FadeIn, [self.jenny_plane, self.basis_vectors])), list(map(Animation, [v, you, jenny]))))
        self.play(ShowCreation(jenny.bubble), Write(jenny.coords), jenny.change_mode, 'speaking')
        self.play(you.change_mode, 'erm')
        self.show_linear_combination(np.dot(self.inv_cob_matrix(), self.v_coords), basis_vectors=[self.b1, self.b2], coord_mobs=jenny.coords.get_entries().copy())
        self.play(FadeOut(v), jenny.change_mode, 'plain')
        self.play(Transform(jenny.coords, new_coords_mob), Blink(jenny))
        self.hacked_show_linear_combination(new_coords, basis_vectors=[self.b1, self.b2], coord_mobs=jenny.coords.get_entries().copy(), show_sum_vect=True)

    def hacked_show_linear_combination(self, numerical_coords, basis_vectors, coord_mobs=None, show_sum_vect=False, sum_vect_color=V_COLOR):
        for coord, basis, scalar in zip(coord_mobs, basis_vectors, numerical_coords):
            basis.save_state()
            basis.label.save_state()
            basis.target = basis.copy().scale(scalar)
        ...
        self.play(Transform(self.jenny_plane, self.plane), Transform(self.jenny.bubble, self.you.bubble), self.you.change_mode, 'speaking', self.jenny.change_mode, 'erm', *list(map(MoveToTarget, [self.jenny.coords, b1, b2, new_label1, new_label2, sum_vect])))
        self.play(Blink(self.you))
        self.wait()
        self.play(*it.chain(list(map(FadeOut, [self.jenny.bubble, self.jenny.coords, coord_mobs, sum_vect])), [ApplyMethod(pi.change_mode, 'plain') for pi in (self.jenny, self.you)], [mob.restore for mob in (b1, b2, b1.label, b2.label)]))
    ...
    def how_we_label_her_basis(self):
        ...
        self.play(b1_coords.mover.next_to, self.b1.get_end(), RIGHT, b1_coords.mover.set_color, X_COLOR)
        self.play(Blink(you))
        self.wait()
        self.play(Transform(b1_coords, b2_coords))
        self.play(b2_coords.mover.next_to, self.b2.get_end(), LEFT, b2_coords.mover.set_color, Y_COLOR)
        ...
        self.play(self.jenny_plane.restore, FadeOut(self.you.bubble), FadeOut(b1_coords), self.jenny.change_mode, 'speaking', self.you.change_mode, 'confused', *list(map(Animation, [self.basis_vectors, b1_coords.mover, b2_coords.mover])))
...
class SpeakingDifferentLanguages(JenniferScene):
    ...
    def construct(self):
        ...
        self.add(you, jenny)
        self.play(Write(title))
        self.play(ShowCreation(vector), you.look_at, vector, jenny.look_at, vector)
        for pi in (you, jenny):
            self.play(pi.change_mode, 'speaking' if pi is you else 'sassy', ShowCreation(pi.bubble), Write(pi.coords))
            self.play(Blink(pi))
        self.wait()
...
class SpaceHasNoGrid(LinearTransformationScene):
    ...
    def construct(self):
        ...
        words.to_edge(UP)
        self.play(Write(words), FadeOut(self.plane), *list(map(Animation, [self.i_hat, self.j_hat])))
        self.wait()
...
class JennysGrid(JenniferScene):
    ...
    def construct(self):
        ...
        for vect in (self.b1, self.b2):
            self.play(ShowCreation(vect), Write(vect.label))
        self.wait()
        self.play(ShowCreation(self.jenny_plane, run_time=3, lag_ratio=0.5), self.jenny.change_mode, 'speaking', self.jenny.look_at, ORIGIN, ShowCreation(bubble), Write(bubble.content), Animation(self.basis_vectors))
        self.play(Blink(self.jenny))
        self.play(FadeOut(bubble.content), FadeIn(coords_mob))
        self.show_linear_combination(numerical_coords=coords, basis_vectors=[self.b1, self.b2], coord_mobs=coords_mob.get_entries().copy(), show_sum_vect=True)
...
class ShowOriginOfGrid(JenniferScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Transform(self.jenny_plane, self.plane), *list(map(Animation, [origin_word, origin_dot, arrow])))
        self.wait()
        self.play(Write(coords))
        self.wait()
        ...
        self.wait()
        self.play(self.jenny_plane.restore, *list(map(Animation, [origin_word, origin_dot, arrow, coords])))
        for vect in (self.b1, self.b2):
            self.play(ShowCreation(vect), Write(vect.label))
        self.wait()
...
class AskAboutTranslation(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('\\centering How do you translate \\\\ between coordinate systems?', target_mode='raise_right_hand')
        self.random_blink(3)
...
class TranslateFromJenny(JenniferScene):
    ...
    def ask_question(self):
        self.play(self.jenny.change_mode, 'pondering', ShowCreation(self.jenny.bubble), Write(self.jenny.coords))
        ...
        sum_vect = self.show_linear_combination(numerical_coords=self.coords, basis_vectors=[self.b1, self.b2], coord_mobs=coord_mobs, revert_to_original=False, show_sum_vect=True)
        self.wait()
        ...
        self.play(self.you.change_mode, 'confused', ShowCreation(self.you.bubble), Write(self.you.coords))
        self.wait()
    ...
    def perform_arithmetic(self):
        ...
        movers = [self.you.coords, equals, jenny_x, self.b1_coords_mob, plus, jenny_y, self.b2_coords_mob, equals2, result]
        for mover in movers:
        ...
        mover_sets = [[jenny_x, self.b1_coords_mob], [plus, jenny_y, self.b2_coords_mob], [self.you.coords, equals]]
        for mover_set in mover_sets:
            self.play(*list(map(MoveToTarget, mover_set)))
            self.wait()
        self.play(MoveToTarget(equals2), Transform(self.b1_coords_mob.copy(), result.target), Transform(self.b2_coords_mob.copy(), result.target))
        ...
        self.play(FadeOut(self.jenny.bubble), FadeOut(self.you.coords), self.jenny.change_mode, 'plain', MoveToTarget(self.jenny.coords), FadeIn(matrix))
        self.wait()
...
class WatchChapter3(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says("\n            You've all watched \n            chapter 3, right?\n        ")
        self.random_blink()
        self.play(self.get_students()[0].look, LEFT, self.get_students()[1].change_mode, 'happy', self.get_students()[2].change_mode, 'happy')
        self.random_blink(2)
...
class TalkThroughChangeOfBasisMatrix(JenniferScene):
    ...
    def construct(self):
        ...
        for coords in (b1_coords, b2_coords):
            coords.scale(0.7)
        basis_coords_pair = VGroup(Matrix([1, 0]).set_color(X_COLOR).scale(0.7), OldTex(','), Matrix([0, 1]).set_color(Y_COLOR).scale(0.7))
        ...
        for vect in (self.i_hat, self.j_hat):
            self.play(ShowCreation(vect), Write(vect.label))
        self.play(self.you.change_mode, 'pondering', ShowCreation(self.you.bubble), Write(basis_coords_pair))
        self.play(Blink(self.you))
        self.wait()
        ...
        self.apply_transposed_matrix(t_matrix1, added_anims=[Transform(self.i_hat, self.b1), Transform(self.i_hat.label, self.b1.label), Transform(matrix_copy.rect, b1_coords.rect), Transform(matrix_copy.get_brackets(), b1_coords.get_brackets()), Transform(VGroup(*matrix_copy.get_mob_matrix()[:, 0]), b1_coords.get_entries())])
        self.remove(matrix_copy)
        self.add_foreground_mobject(b1_coords)
        matrix_copy = matrix.copy()
        matrix_copy.rect.set_fill(opacity=0)
        self.apply_transposed_matrix(t_matrix2, added_anims=[Transform(self.j_hat, self.b2), Transform(self.j_hat.label, self.b2.label), Transform(matrix_copy.rect, b2_coords.rect), Transform(matrix_copy.get_brackets(), b2_coords.get_brackets()), Transform(VGroup(*matrix_copy.get_mob_matrix()[:, 1]), b2_coords.get_entries())])
        self.remove(matrix_copy)
        ...
        self.wait()
        self.play(FadeOut(b1_coords), FadeOut(b2_coords), self.jenny.change_mode, 'speaking', Transform(self.you.bubble, self.jenny.bubble), MoveToTarget(basis_coords_pair))

class ChangeOfBasisExample(JenniferScene):
    CONFIG = {'v_coords': [-1, 2]}

    def construct(self):
        self.add(self.plane, self.i_hat, self.j_hat, self.i_hat.label, self.j_hat.label)
        ...
        self.play(Write(start_group), FadeIn(self.you))
        self.add_foreground_mobject(start_group, self.you)
        self.show_linear_combination(numerical_coords=self.v_coords, basis_vectors=[self.i_hat, self.j_hat], coord_mobs=v_coords.get_entries().copy())
        ...
        self.play(Transform(start_words, end_words), Transform(self.you, self.jenny), *list(map(Write, [self.b1.label, self.b2.label])))
        self.play(Blink(self.you))
        self.show_linear_combination(numerical_coords=self.v_coords, basis_vectors=[self.b1, self.b2], coord_mobs=v_coords.get_entries().copy())

class FeelsBackwards(Scene):

    def construct(self):
        matrix = Matrix(np.array([JenniferScene.CONFIG['b1_coords'], JenniferScene.CONFIG['b2_coords']]).T)
        matrix.set_column_colors(X_COLOR, Y_COLOR)
        matrix.shift(UP)
        ...
        self.play(Swap(jenny_grid, our_grid), top_arrow.scale, 0.8, top_arrow.shift, 0.8 * RIGHT, top_arrow.set_color, BLUE)
        self.play(Swap(jenny_language, our_language), bottom_arrow.scale, 0.8, bottom_arrow.shift, 0.8 * RIGHT)
        self.wait()
...
class SummarizeTranslationProcess(Scene):
    ...
    def show_translation(self):
        ...
        self.wait(2)
        self.play(VGroup(her_vector, equals).next_to, A_inv, LEFT, her_arrow.rotate, -np.pi / 6, her_arrow.shift, MED_SMALL_BUFF * LEFT, Transform(A, A_inv, path_arc=np.pi))
        self.wait()
...
class Prerequisites(Scene):
    ...
    def construct(self):
        ...
        prereqs = list(map(TexText, ['Linear transformations', 'Matrix multiplication']))
...
class RotationExample(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(self.i_hat.rotate, np.pi / 12, self.j_hat.rotate, -np.pi / 12, rate_func=wiggle, run_time=2)
        self.wait()
        ...
        for coords, vect in zip(coord_arrays, [self.i_hat, self.j_hat]):
            ...
            self.play(Write(coords))
            self.wait()
        self.play(Transform(i_coords.rect, matrix.rect), Transform(i_coords.get_brackets(), matrix.get_brackets()), Transform(i_coords.get_entries(), VGroup(*matrix.get_mob_matrix()[:, 0])))
        self.play(FadeOut(j_coords.rect), FadeOut(j_coords.get_brackets()), Transform(j_coords.get_entries(), VGroup(*matrix.get_mob_matrix()[:, 1])))
        self.wait()
    ...
    def add_words(self, matrix):
        ...
        follow_basis.next_to(matrix, LEFT, buff=MED_SMALL_BUFF)
...
class JennyWatchesRotation(JenniferScene):
    ...
    def construct(self):
        ...
        self.play(*it.chain([Rotate(mob, np.pi / 2, run_time=3) for mob in (self.jenny_plane, self.b1, self.b2)], list(map(Animation, [jenny, jenny.bubble, matrix]))))
...
class AksAboutTranslatingColumns(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(Transform(student.bubble, bubble), FadeOut(student.bubble.content), MoveToTarget(matrix.copy()), student.change_mode, 'pondering')
        self.remove(student.bubble)
        student.bubble = None
        self.add(bubble, matrix.target)
        self.random_blink()
        words = OldTexText('\\centering Those columns still \\\\ represent ', 'our basis', ', not ', 'hers', arg_separator='')
...
class HowToTranslateAMatrix(Scene):
    ...
    def construct(self):
        self.add_title()
        arrays = VGroup(*list(map(Matrix, [[['1/3', '-2/3'], ['5/3', '-1/3']], [-1, 2], [[2, -1], [1, 1]], [[0, -1], [1, 0]], [[2, -1], [1, 1]]])))
        ...
        brace_text_pairs = [(her_vector.brace, ('Vector in \\\\', "Jennifer's language")), (her_vector.top_brace, ('',)), (cob_matrix.brace, ('Change of basis \\\\', 'matrix')), (cob_matrix.top_brace, ('Same vector \\\\', 'in', 'our', 'language')), (transform.brace, ('Transformation matrix \\\\', 'in', 'our', 'language')), (transform.top_brace, ('Transformed vector \\\\', 'in', 'our', 'language')), (inv_cob.brace, ('Inverse \\\\', 'change of basis \\\\', 'matrix')), (inv_cob.top_brace, ('Transformed vector \\\\', 'in', 'her', 'language')), (final_top_brace, ('Transformation matrix \\\\', 'in', 'her', 'language'))]
        ...
        def introduce(array):
            self.play(Write(array), Transform(brace, array.brace), Transform(bottom_words, array.brace.text))
            self.wait()

        def echo_introduce(array):
            self.play(Transform(top_brace, array.top_brace), Transform(top_words, array.top_brace.text))
            self.wait()
        ...
        self.play(Transform(her_vector, v), FadeOut(bottom_words), FadeOut(brace))
        self.wait()
        self.play(Transform(top_brace, final_top_brace), Transform(top_brace.text, final_top_brace.text))
        self.wait()
        equals = OldTex('=')
        equals.replace(v)
...
class JennyWatchesRotationWithMatrixAndVector(JenniferScene):
    ...
    def construct(self):
        ...
        self.play(Blink(self.jenny))
        self.play(*it.chain([Rotate(mob, np.pi / 2, run_time=3) for mob in (self.jenny_plane, self.b1, self.b2, vector)], list(map(Animation, [self.jenny, matrix, vector_array]))))
...
class MathematicalEmpathy(TeacherStudentsScene):
    ...
    def construct(self):
        words = OldTexText('\\centering An expression like', '$A^{-1} M A$', '\\\\ suggests a mathematical \\\\', 'sort of empathy')
