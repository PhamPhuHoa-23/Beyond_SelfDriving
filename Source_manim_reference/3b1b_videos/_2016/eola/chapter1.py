...
class ComputerScientist(PiCreature):
    CONFIG = {'color': PURPLE_E, 'flip_at_start': True}

class OpeningQuote(Scene):

    def construct(self):
        words = OldTexText("``The introduction of numbers as \\\\ coordinates is an act of violence.''")
        words.to_edge(UP)
...
class DifferentConceptions(Scene):
    ...
    def intro_vector(self):
        ...
        self.wait(2)
        self.play(FadeOut(plane), FadeOut(labels), ApplyMethod(vector.shift, 4 * LEFT + UP), ApplyMethod(coordinates.shift, 2.5 * RIGHT + 0.5 * DOWN), ApplyMethod(symbol.shift, 0.5 * (UP + LEFT)))
    ...
    def physics_conception(self, creatures, original_vector):
        ...
        random_vectors = VMobject(*[Vector(random.uniform(-2, 2) * RIGHT + random.uniform(-2, 2) * UP).shift(random.uniform(0, 4) * RIGHT + random.uniform(-1, 2) * UP).set_color(random_color()) for x in range(5)])
        self.play(Transform(original_vector, vector), ApplyMethod(physy.change_mode, 'speaking'))
        ...
        self.wait()
        self.play(Write(two_dimensional), ApplyMethod(physy.change_mode, 'pondering'), ShowCreation(random_vectors, lag_ratio=0.5), run_time=1)
        self.wait(2)
    ...
    def cs_conception(self, creatures):
        ...
        title.to_edge(UP)
        vectors = VMobject(*list(map(matrix_to_mobject, [[2, 1], [5, 0, 0, -3], [2.3, -7.1, 0.1]])))
    ...
    def house_example(self, starter_mobject, title):
        ...
        alt_vector = Matrix([OldTexText('300{,}000\\text{ ft}^2').set_color(RED), OldTexText('\\$2{,}600').set_color(GREEN)])
        ...
        self.wait()
        self.play(FadeOut(square_footage_words), FadeOut(price_words), Transform(square_footage, new_square_footage), Transform(price, new_price), Write(brackets), run_time=1)
    ...
    def handle_mathy(self, creatures):
        ...
        self.play(Write(syms), Write(arrays), ShowCreation(arrows), ApplyMethod(mathy.change_mode, 'pondering'), run_time=2)
    ...
    def add_scaling(self, arrows, syms, arrays):
        s_arrows = VMobject(OldTex('2'), Vector([1, 1]).set_color(YELLOW), OldTex('='), Vector([2, 2]).set_color(WHITE))
        s_arrows.arrange(RIGHT)
        s_arrows.scale(0.75)
        s_arrows.next_to(arrows, DOWN)
        s_arrays = VMobject(OldTex('2'), matrix_to_mobject([3, -5]).set_color(YELLOW), OldTexText('='), matrix_to_mobject(['2(3)', '2(-5)']))
    ...
    def restore_creatures(self, creatures):
        self.play(*[ApplyFunction(lambda m: m.change_mode('plain').set_color(m.color), pi) for pi in creatures] + [ApplyMethod(pi.title.set_fill, WHITE, 1.0) for pi in creatures])
...
class HelpsToHaveOneThought(Scene):
    ...
    def construct(self):
        ...
        self.wait(2)
        self.play(FadeOut(speech_bubble), FadeOut(words), ApplyMethod(randy.change_mode, 'pondering'), ShowCreation(thought_bubble), Write(thought))
        self.wait(2)
...
class CoordinateSystemWalkthrough(VectorScene):
    ...
    def show_vector_coordinates(self):
        ...
        self.wait()
        self.play(ApplyFunction(lambda m: m.scale(1.25).set_color(YELLOW), array.get_brackets(), rate_func=there_and_back))
        self.wait()
...
class VectorAddition(VectorScene):
    ...
    def ask_why(self, v1, v2, v_sum):
        ...
        self.wait(2)
        self.play(Transform(v2, new_v2), Transform(v_sum, new_v_sum), Transform(why, why_not_this))
        self.wait(2)
...
class VectorAdditionNumerically(VectorScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ApplyMethod(coords2.next_to, plus, RIGHT), Write(plus, run_time=1), *[ApplyMethod(mob.shift, v1.get_end()) for mob in (v2, x_line2, y_line2)])
        ...
        self.play(GrowFromCenter(one_brace), GrowFromCenter(three_brace), Write(one), Write(three), run_time=1)
        self.wait()
        ...
        self.wait()
        self.play(Transform(two_brace, two_minus_one_brace), Transform(two, two_minus_one), Transform(y_line1, new_y_line), Transform(y_line2, new_y_line))
        self.wait()
        ...
        for i, (var, starter) in enumerate(zip(variables, starters)):
            ...
            var.move_to(starter)
        self.play(Transform(VMobject(*starters[:4]), VMobject(*variables[:4])), FadeOut(sum_x_start), FadeOut(sum_y_start))
...
class ShowScalarMultiplication(VectorScene):
    ...
    def scale_vector(self, v, factor, v_label, v_name='v', factor_tex=None):
        ...
        self.wait()
        self.play(ApplyMethod(v.copy().set_color, GREY_D), ApplyMethod(v_label.copy().set_color, GREY_D), Transform(factor_mob, factor_in_label), Transform(v.copy(), scaled_vector), Transform(v_label.copy(), label_remainder))
        self.wait(2)
        self.clear()
        self.add(*starting_mobjects)
...
class FollowingVideos(UpcomingSeriesOfVidoes):
    ...
    def construct(self):
        v_sum = VMobject(Vector([1, 1], color=YELLOW), Vector([3, 1], color=BLUE).shift(RIGHT + UP), Vector([4, 2], color=GREEN))
        scalar_multiplication = VMobject(OldTex('2 \\cdot '), Vector([1, 1]), OldTex('='), Vector([2, 2], color=WHITE))
        ...
        self.play(ApplyMethod(everything.shift, FRAME_WIDTH * LEFT), Transform(last_video, big_last_video), run_time=2)
...
class ItDoesntMatterWhich(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ApplyMethod(physy.change_mode, 'speaking'), ShowCreation(physy_speech), Write(physy_statement), run_time=1)
        self.play(Blink(compy))
        self.play(ApplyMethod(physy.change_mode, 'sassy'), ApplyMethod(compy.change_mode, 'speaking'), FadeOut(physy_speech), ShowCreation(compy_speech), Transform(physy_statement, compy_statement, path_arc=np.pi))
        self.wait(2)
        self.play(ApplyMethod(physy.change_mode, 'pondering'), ApplyMethod(compy.change_mode, 'pondering'), Transform(compy_speech, VectorizedPoint(compy_speech.get_tip())), Transform(physy_statement, back_and_forth))
        self.wait()
...
class DataAnalyst(Scene):
    ...
    def construct(self):
        plane = NumberPlane()
        ellipse = ParametricCurve(lambda x: 2 * np.cos(x) * (UP + RIGHT) + np.sin(x) * (UP + LEFT), color=PINK, t_max=2 * np.pi)
        ...
        self.add(vects)
        self.wait()
        self.play(ApplyMethod(plane.fade, 0.7), ApplyMethod(vects.set_color, GREY_D), ShowCreation(ellipse))
        self.wait(3)

class ManipulateSpace(LinearTransformationScene):
    CONFIG = {'include_background_plane': False, 'show_basis_vectors': False}
