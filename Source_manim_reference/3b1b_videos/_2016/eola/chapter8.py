...
class ListSteps(Scene):
    ...
    def construct(self):
        ...
        self.play(Blink(randy))
        self.play(Transform(step_1, step_1.copy().restore().fade()), MoveToTarget(step_2), randy.look, LEFT)
...
class SimpleDefine2dCrossProduct(LinearTransformationScene):
    CONFIG = {'show_basis_vectors': False, 'v_coords': [3, 2], 'w_coords': [2, -1]}
    ...
    def add_vectors(self):
        ...
        for vect, name, direction in ((v, 'v', 'left'), (w, 'w', 'right')):
            ...
            vect.label = self.label_vector(vect, name, color=color, direction=direction)
        self.v, self.w = (v, w)

    def show_area(self):
        self.add_unit_square()
        transform = self.get_matrix_transformation(np.array([self.v_coords, self.w_coords]))
    ...
    def write_area_words(self):
        ...
        self.play(FadeIn(cross_rect), Write(times), *[ApplyMethod(vect.label.target.restore, rate_func=lambda t: smooth(1 - t)) for vect in (self.v, self.w)])
        self.wait()
    ...
    def show_sign(self):
        ...
        transform = self.get_matrix_transformation([self.v.target.get_end()[:2], self.w.target.get_end()[:2]])
        self.square.apply_function(transform)
        ...
        self.remove(arc)
        self.play(FadeOut(positive), FadeOut(positive.arrow), *[mob.restore for mob in (square, self.v, self.w)])
        arc = self.get_arc(self.v, self.w, radius=1.5)
        arc.set_color(RED)
        self.play(ShowCreation(arc))
        self.play(Write(negative), ShowCreation(negative.arrow), Write(minus_sign), MoveToTarget(self.area_words))
        self.wait()
    ...
    def swap_v_and_w(self):
        ...
        self.play(FadeOut(self.area_words), Transform(self.cross.copy(), new_cross, path_arc=np.pi / 2))
        self.wait()
        ...
        self.play(MoveToTarget(self.square), Transform(self.v, self.w), Transform(self.w, self.v), rate_func=there_and_back, run_time=3)
        self.wait()

    def get_arc(self, v, w, radius=2):
        v_angle, w_angle = (v.get_angle(), w.get_angle())
        nudge = 0.05
        arc = Arc((1 - 2 * nudge) * (w_angle - v_angle), start_angle=interpolate(v_angle, w_angle, nudge), radius=radius, stroke_width=8)
        arc.add_tip()
        return arc
...
class CrossBasisVectors(LinearTransformationScene):
    ...
    def construct(self):
        self.plane.fade()
        i_label = self.get_vector_label(self.i_hat, '\\hat{\\imath}', direction='right', color=X_COLOR)
        j_label = self.get_vector_label(self.j_hat, '\\hat{\\jmath}', direction='left', color=Y_COLOR)
        ...
        self.play(ShowCreation(cross_rect), MoveToTarget(i_label.copy()), MoveToTarget(j_label.copy()), Write(times))
        self.play(Write(eq))
        self.wait()
...
class VisualExample(SimpleDefine2dCrossProduct):
    CONFIG = {'show_basis_vectors': False, 'v_coords': [3, 1], 'w_coords': [1, -2]}
    ...
    def show_coords(self):
        for vect, edge in ((self.v, DOWN), (self.w, UP)):
            color = vect.get_color()
            vect.coord_array = vector_coordinate_label(vect, color=color)
...
class ContrastDotAndCross(Scene):
    ...
    def add_dot_products(self, max_width=FRAME_X_RADIUS - 1, dims=[2, 5]):
        ...
        for dim in dims:
            ...
            def get_dot():
                ...
                syms.add(dot)
                return dot
            result = VGroup(*it.chain(*list(zip(syms, [VGroup(e1.target, get_dot(), e2.target).arrange() for e1, e2 in zip(m1.get_entries(), m2.get_entries())]))))
        ...
        for dot_prod in dot_products:
            self.play(Write(dot_prod.syms), *[Transform(e.copy(), e.target, path_arc=-np.pi / 6) for e in dot_prod.entries], run_time=2)
        self.wait()
    ...
    def add_cross_product(self):
        ...
        for e1, e1_target, e2, e2_target in movement_sets:
            movements += [e1.copy().move_to, e1_target, e2.copy().move_to, e2_target]
    ...
    def add_2d_cross_product(self):
        ...
        result = VGroup(eq, x1.target, dot1, x4.target, minus, x3.target, dot2, x2.target)
        result.arrange(RIGHT)
        ...
        self.play(Write(VGroup(*syms)), *[Transform(entry.copy(), entry.target) for entry in entries])
        self.wait()
...
class Define2dCrossProduct(LinearTransformationScene):
    CONFIG = {'show_basis_vectors': False, 'v_coords': [3, 1], 'w_coords': [2, -1]}
    ...
    def initial_definition(self):
        ...
        self.play(FadeIn(cross_background), Transform(v.label.copy(), v.label.target), Transform(w.label.copy(), w.label.target), Write(times))
        self.wait()
        ...
        self.wait()
        self.play(Write(equals), Write(det_text), Animation(matrix))
        self.wait()
    ...
    def show_transformation(self):
        ...
        self.play(*list(map(FadeOut, everything)) + [Animation(self.background_plane), self.plane.restore, Animation(matrix)])
        ...
        self.wait()
        self.play(Transform(i_words, j_words), Transform(i_words.arrow, j_words.arrow), col2.set_color, Y_COLOR)
        self.wait()
        ...
        self.wait()
        self.play(FadeOut(self.plane), *list(map(Animation, [self.background_plane, matrix, i_hat, j_hat])))
        self.play(ShowCreation(self.v), ShowCreation(self.w), FadeIn(self.v.label), FadeIn(self.w.label), FadeIn(self.v.coord_array), FadeIn(self.w.coord_array), matrix.set_column_colors, V_COLOR, W_COLOR)
        self.wait()
    ...
    def show_orientation_rule(self):
        ...
        positive_words, negative_words = words_list = [OldTex(v_tex, '\\times', w_tex, '\\text{ is }', word) for word in ('\\text{positive}', '\\text{negative}')]
        ...
        self.play(Transform(self.v, self.v.target), Transform(self.w, self.w.target), rate_func=there_and_back, run_time=2)
        self.wait()
...
class TwoDCrossProductExample(Define2dCrossProduct):
    CONFIG = {'v_coords': [-3, 1], 'w_coords': [2, 1]}
    ...
    def construct(self):
        ...
        matrix = Matrix(np.array([list(v.coords.copy().get_entries()), list(w.coords.copy().get_entries())]).T)
        matrix_background = BackgroundRectangle(matrix)
        ...
        equals, dot1, minus, dot2, equals_result = syms = VGroup(*list(map(Tex, ['=', '\\cdot', '-', '\\cdot', '=%d' % det])))
        equation_end = VGroup(equals, v1.target, dot1, w2.target, minus, w1.target, dot2, v2.target, equals_result)
        ...
        self.play(Write(syms), Transform(VGroup(v1, w2).copy(), VGroup(v1.target, w2.target), rate_func=squish_rate_func(smooth, 0, 1.0 / 3), path_arc=np.pi / 2), Transform(VGroup(v2, w1).copy(), VGroup(v2.target, w1.target), rate_func=squish_rate_func(smooth, 2.0 / 3, 1), path_arc=np.pi / 2), run_time=3)
        self.wait()
    ...
    def show_transformation(self, v, w):
        ...
        self.add_unit_square(animate=True, opacity=0.2)
        self.apply_transposed_matrix([v.get_end()[:2], w.get_end()[:2]], added_anims=[Transform(i_hat, v), Transform(j_hat, w)])
...
class BiggerWhenPerpendicular(LinearTransformationScene):
    ...
    def construct(self):
        ...
        transforms = [self.get_matrix_transformation([v1.get_end()[:2], v2.get_end()[:2]]) for v1, v2 in [(v, w), (v.target, w.target)]]
        ...
        self.wait()
        self.play(Transform(start_square, end_square), Transform(v, v.target), Transform(w, w.target))
...
class ScalingRule(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Transform(v, new_v), Transform(v_tex_mob, three_v))
        self.wait()
        self.play(Transform(square, new_squares), *list(map(Animation, [v, w])), path_arc=-np.pi / 6)
        self.wait()
        self.play(Write(rhs))
        self.wait()
...
class CrossProductRightHandRule(RightHandRule):
    CONFIG = {'flip': False, 'labels_tex': get_cross_product_right_hand_rule_labels(), 'colors': [U_COLOR, W_COLOR, P_COLOR]}
...
class LabelingExampleVectors(Scene):
    ...
    def construct(self):
        ...
        equations = [OldTex(v_tex, '=%s' % matrix_to_tex_string([0, 0, 2])), OldTex(w_tex, '=%s' % matrix_to_tex_string([0, 2, 0])), OldTex(v_tex, '\\times', w_tex, '=%s' % matrix_to_tex_string([-4, 0, 0]))]
...
class ShowCrossProductFormula(Scene):
    ...
    def construct(self):
        ...
        result = Matrix([VGroup(entry_dict['positive'], entry_dict['negative']).arrange() for entry_dict in entry_dicts])
        ...
        for e1, e1_target, e2, e2_target, syms in movement_sets:
            e1.save_state()
            e2.save_state()
            self.play(e1.scale, 1.5, e2.scale, 1.5)
            self.play(Transform(e1.copy(), e1_target), Transform(e2.copy(), e2_target), Write(syms), e1.restore, e2.restore, path_arc=-np.pi / 2)
        self.wait()
...
class DeterminantTrick(Scene):
    ...
    def construct(self):
        ...
        v.set_color(V_COLOR)
        w.set_color(W_COLOR)
        matrix = Matrix(np.array([[OldTex('\\hat{%s}' % s) for s in ('\\imath', '\\jmath', 'k')], list(v.get_entries().copy()), list(w.get_entries().copy())]).T)
        colors = [X_COLOR, Y_COLOR, Z_COLOR]
        ...
        for col, vect in ((col2, v), (col3, w)):
            ...
            self.play(col.restore, path_arc=-np.pi / 2)
        for entry in col1:
        ...
        self.play(equation.to_corner, UP + LEFT, ApplyFunction(lambda r: r.change_mode('plain').look(UP + RIGHT), randy))
        quints = [(i, v2, w3, v3, w2), (j, v3, w1, v1, w3), (k, v1, w2, v2, w1)]
        last_mob = None
        paren_sets = []
        for quint in quints:
            for mob in quint:
            ...
            self.wait()
            self.play(*[Transform(mob.copy(), mob.t) for mob in quint] + [mob.restore for mob in quint] + [Write(syms)], run_time=2)
            self.wait()
...
class ThereIsAReason(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says("\\centering Sure, it's a \\\\", 'notational', 'trick')
        self.random_blink(2)
...
class CrossAndDualWords(Scene):
    ...
    def construct(self):
        ...
        matrix = Matrix(np.array([['x', 'y', 'z'], ['v_1', 'v_2', 'v_3'], ['w_1', 'w_2', 'w_3']]).T)
