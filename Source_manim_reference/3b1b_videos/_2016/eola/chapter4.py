...
class OpeningQuote(Scene):
    ...
    def construct(self):
        words = OldTexText(['It is my experience that proofs involving', 'matrices', 'can be shortened by 50\\% if one', 'throws the matrices out.'])
...
class RecapTime(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(ApplyMethod(everyone.fade, 0.7), ApplyMethod(student.change_mode, 'confused'))
...
class FollowLinearCombination(LinearTransformationScene):
    ...
    def construct(self):
        ...
        t_matrix = np.array([[2, 2], [-2, 1]])
        self.setup()
        i_label = self.add_transformable_label(self.i_hat, '\\hat{\\imath}', animate=False, direction='right', color=X_COLOR)
        j_label = self.add_transformable_label(self.j_hat, '\\hat{\\jmath}', animate=False, direction='right', color=Y_COLOR)
    ...
    def show_scaled_vectors(self, vect_array, vect_coords, i_label, j_label):
        ...
        self.play(Transform(self.i_hat.copy(), scaled_i), Transform(scaled_i_label, scaled_i_label_target))
        scaled_i = self.get_mobjects_from_last_animation()[0]
        self.play(Transform(self.j_hat.copy(), scaled_j), Transform(scaled_j_label, scaled_j_label_target))
        ...
        self.wait()
        self.play(*list(map(FadeOut, [scaled_i, scaled_j, scaled_i_label, scaled_j_label])))
    ...
    def record_basis_coordinates(self, vect_array, vect):
        ...
        self.play(*[ApplyMethod(m.fade) for m in everything] + [ShowCreation(background, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)), Transform(pre_formula.copy(), post_formula, run_time=2), ApplyMethod(vect.set_stroke, width=7)])
        self.wait()
...
class RotationThenShear(LinearTransformationScene):
    CONFIG = {'foreground_plane_kwargs': {'x_radius': FRAME_X_RADIUS, 'y_radius': FRAME_WIDTH, 'secondary_line_ratio': 0}}
...
class IntroduceIdeaOfComposition(RotationThenShear):
    ...
    def show_composition(self):
        ...
        self.play(ApplyMethod(self.plane.fade), Write(words), Animation(self.i_hat), Animation(self.j_hat))
        self.wait()
    ...
    def track_basis_vectors(self):
        ...
        matrix = Matrix(np.append(i_coords.copy().get_mob_matrix(), j_coords.copy().get_mob_matrix(), axis=1))
        ...
        self.wait()
        self.play(Transform(i_background.copy(), matrix_background), Transform(i_coords.copy().get_brackets(), matrix.get_brackets()), ApplyMethod(i_coords.copy().get_entries().move_to, col1))
        self.wait()
    ...
    def show_overall_effect(self, matrix):
        ...
        self.play(ApplyPointwiseFunction(func, self.plane), Transform(bases, new_bases), Animation(matrix), run_time=3)
        self.wait()
...
class ExplainWhyItsMatrixMultiplication(Scene):
    ...
    def construct(self):
        ...
        pairs = [(rot_matrix, 'Rotation'), (shear_matrix, 'Shear'), (comp_matrix, 'Composition')]
        ...
        self.add(vect)
        groups = [[rot_matrix], [l_paren, r_paren, shear_matrix], [equals, comp_matrix, vect_copy]]
        for group in groups:
...
class MoreComplicatedExampleVisually(LinearTransformationScene):
    CONFIG = {'t_matrix1': [[1, 1], [-2, 0]], 't_matrix2': [[0, 1], [2, 0]]}
    ...
    def construct(self):
        ...
        self.wait()
        self.play(*list(map(FadeOut, [self.background_plane, self.plane, self.i_hat, self.j_hat])) + [Animation(m) for m in self.foreground_mobjects])
...
class MoreComplicatedExampleNumerically(MoreComplicatedExampleVisually):
    ...
    def construct(self):
        ...
        for col in (col1, col2):
            ...
            circle.move_to(col)
            col.circle = circle
        triplets = [(col1, 'i', X_COLOR), (col2, 'j', Y_COLOR)]
        for i, (col, char, color) in enumerate(triplets):
            ...
            intermediate = VMobject(OldTex('='), col_vect.copy().get_entries().split()[0], Matrix(new_m2_copy.get_mob_matrix()[:, 0]), OldTex('+'), col_vect.copy().get_entries().split()[1], Matrix(new_m2_copy.get_mob_matrix()[:, 1]), OldTex('='))
            ...
            self.wait()
            self.play(Transform(question, first), ShowCreation(first_arrow), ShowCreation(col.circle), ApplyMethod(col.set_color, col.target_color))
            self.wait()
            self.play(Transform(m2_copy, m2_target, run_time=2), ApplyMethod(col.copy().move_to, col_vect, run_time=2), Write(col_vect.get_brackets()), Transform(first_arrow, second_arrow), Transform(question, second))
            self.wait()
            ...
            self.play(ApplyMethod(comp_col.set_color, BLACK), ApplyMethod(product_entries.move_to, comp_col))
            self.wait()
            start_state.append(product_entries)
            self.play(*[FadeOut(mob) for mob in self.get_mobjects() if mob not in start_state] + [Animation(product_entries)])
            self.wait()
...
class GeneralMultiplication(MoreComplicatedExampleNumerically):
    ...
    def get_matrices(self):
        ...
        m2_entries_target = VMobject(*[OldTex(char).move_to(entry).set_color(entry.get_color()) for entry, char in zip(m2_entries.split(), 'abcd')])
        m1_entries_target = VMobject(*[OldTex(char).move_to(entry).set_color(entry.get_color()) for entry, char in zip(m1_entries.split(), 'efgh')])
        ...
        self.play(Transform(comp.split()[1].get_brackets(), new_comp.get_brackets()), *[ApplyMethod(q_mark.move_to, entry) for q_mark, entry in zip(comp.split()[1].get_entries().split(), new_comp.get_entries().split())])
        self.wait()
...
class MoreComplicatedExampleWithJustJHat(MoreComplicatedExampleWithJustIHat):
    CONFIG = {'v_coords': [0, 1], 'v_color': Y_COLOR}
...
class AskAboutCommutativity(Scene):
    ...
    def construct(self):
        ...
        self.play(Transform(l_m1.copy(), r_m1), Transform(l_m2.copy(), r_m2), path_arc=-np.pi, run_time=2)
...
class ShowRotation(ShowShear):
    CONFIG = {'title': '$90^\\circ$ rotation', 'title_color': YELLOW, 't_matrix': [[0, 1], [-1, 0]]}

class FirstShearThenRotation(LinearTransformationScene):
    CONFIG = {'title': 'First shear then rotation', 't_matrix1': [[1, 0], [1, 1]], 't_matrix2': [[0, 1], [-1, 0]], 'foreground_plane_kwargs': {'x_radius': FRAME_WIDTH, 'y_radius': FRAME_WIDTH, 'secondary_line_ratio': 0}}
...
class RotationThenShear(FirstShearThenRotation):
    CONFIG = {'title': 'First rotation then shear', 't_matrix1': [[0, 1], [-1, 0]], 't_matrix2': [[1, 0], [1, 1]]}
...
class NoticeTheLackOfComputations(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(*[ApplyMethod(pi.change_mode, 'pondering', rate_func=squish_rate_func(smooth, *np.clip(unit + 0.5 * i, 0, 1))) for i, pi in enumerate(students)])
        self.random_blink()
        self.wait()
...
class AskAssociativityQuestion(Scene):
    ...
    def show_full_matrices(self, morty, a, b, c, title):
        ...
        matrices = list(map(matrix_to_mobject, [np.array(list(m)).reshape((2, 2)) for m in ('abcd', 'efgh', 'ijkl')]))
...
class ThreeSuccessiveTransformations(LinearTransformationScene):
    CONFIG = {'t_matrices': [[[2, 1], [1, 2]], [[np.cos(-np.pi / 6), np.sin(-np.pi / 6)], [-np.sin(-np.pi / 6), np.cos(-np.pi / 6)]], [[1, 0], [1, 1]]], 'symbols_str': 'A(BC)', 'include_background_plane': False}
...
class GoPlay(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(bubble), student.look, UP + LEFT)
