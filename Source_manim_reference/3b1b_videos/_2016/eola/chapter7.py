...
class OpeningQuote(Scene):
    ...
    def construct(self):
        ...
        for i in range(3):
            ...
            self.play(Write(quote, run_time=rt))
        self.wait(2)
...
class TraditionalOrdering(RandolphScene):
    ...
    def construct(self):
        ...
        topics = VMobject(*list(map(TexText, ['Topic 1: Vectors', 'Topic 2: Dot products', '\\vdots', '(everything else)', '\\vdots'])))
        topics.arrange(DOWN, aligned_edge=LEFT, buff=SMALL_BUFF)
        self.play(Write(title, run_time=1), FadeIn(topics, run_time=3, lag_ratio=0.5))
...
class ThisSeriesOrdering(RandolphScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(chapters, lag_ratio=0.5, run_time=3), self.randy.change_mode, 'sassy')
...
class OneMustViewThroughTransformations(TeacherStudentsScene):
    ...
    def construct(self):
        words = OldTexText('Only with', 'transformations', '\n can we truly understand')
...
class ShowNumericalDotProduct(Scene):
    CONFIG = {'v1': [2, 7, 1], 'v2': [8, 2, 8], 'write_dot_product_words': True}
    ...
    def construct(self):
        ...
        dot = OldTex('\\cdot')
        products = VGroup(*[VGroup(p1.copy(), dot.copy(), p2.copy()).arrange(RIGHT, buff=SMALL_BUFF) for p1, p2 in pairs])
        ...
        if self.write_dot_product_words:
            self.play(inter_array_dot.set_color, YELLOW, ShowCreation(arrow), Write(dot_product_words, run_time=2))
            self.wait()
        self.play(Transform(VGroup(*it.starmap(Group, pairs)).copy(), products, path_arc=-np.pi / 2, run_time=2))
        self.remove(*self.get_mobjects_from_last_animation())
        self.add(products)
        self.wait()
        self.play(Write(symbols), Transform(products, products.target, path_arc=np.pi / 2))
        self.wait()

class TwoDDotProductExample(ShowNumericalDotProduct):
    CONFIG = {'v1': [1, 2], 'v2': [3, 4]}

class FourDDotProductExample(ShowNumericalDotProduct):
    CONFIG = {'v1': [6, 2, 8, 3], 'v2': [1, 8, 5, 3], 'write_dot_product_words': False}

class GeometricInterpretation(VectorScene):
    CONFIG = {'v_coords': [4, 1], 'w_coords': [2, -1], 'v_color': V_COLOR, 'w_color': W_COLOR, 'project_onto_v': True}
    ...
    def add_vectors(self):
        ...
        for vect, char, direction in zip([self.v, self.w], ['v', 'w'], [DOWN + RIGHT, DOWN]):
    ...
    def project(self):
        ...
        projected = Vector(self.stable_vect.get_end() * dot_product / self.stable_vect.get_length() ** 2, color=self.proj_vect.get_color())
        projection_line = Line(self.proj_vect.get_end(), projected.get_end(), color=GREY)
    ...
    def show_lengths(self):
        ...
        product = OldTexText('=', '(', 'Length of projected $\\vec{\\textbf{%s}}$' % proj_char, ')', '(', 'Length of $\\vec{\\textbf{%s}}$' % stable_char, ')', arg_separator='')
        product.scale(0.9)
        ...
        proj_brace, stable_brace = braces = [Brace(Line(ORIGIN, vect.get_length() * RIGHT * sgn), UP) for vect in (self.proj_vect, self.stable_vect) for sgn in [np.sign(np.dot(vect.get_end(), self.stable_vect.get_end()))]]
        ...
        self.play(GrowFromCenter(stable_brace), Write(stable_words.start, run_time=2), Animation(self.stable_vect))
        self.wait()
    ...
    def handle_possible_negative(self):
        ...
        words.set_color(RED)
        words.next_to(VMobject(*self.product[2:]), DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
        words.add_background_rectangle()
        arrow = Arrow(words.get_left(), neg, color=RED)
        self.play(Write(neg), ShowCreation(arrow), VMobject(*self.product[1:]).next_to, neg, Write(words))
        self.wait()

class GeometricInterpretationNegative(GeometricInterpretation):
    CONFIG = {'v_coords': [3, 1], 'w_coords': [-1, -2], 'v_color': YELLOW, 'w_color': MAROON_B}

class ShowQualitativeDotProductValues(VectorScene):

    def construct(self):
        self.lock_in_faded_grid()
        v_sym, dot, w_sym, comp, zero = ineq = OldTex('\\vec{\\textbf{v}}', '\\cdot', '\\vec{\\textbf{w}}', '>', '0')
        ineq.to_edge(UP)
        ...
        words = list(map(TexText, ['Similar directions', 'Perpendicular', 'Opposing directions']))
        ...
        w.rotate(-np.pi / 6)
        shadow = Vector(v.get_end() * np.dot(v.get_end(), w.get_end()) / v.get_length() ** 2, color=MAROON_E, preserve_tip_size_when_scaling=False)
...
class AskAboutSymmetry(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        question = OldTex('\\text{Why does }', v, '\\cdot', w, '=', w, '\\cdot', v, '\\text{?}')
...
class SymmetricVAndW(VectorScene):
    ...
    def construct(self):
        ...
        self.play(Rotate(w, angle), Rotate(v, -angle), rate_func=there_and_back, run_time=2)
        self.wait()
        self.play(ShowCreation(line_of_symmetry), Write(line_of_symmetry_words, run_time=1))
        self.wait(0.5)
    ...
    def show_doubling(self, v, w):
        ...
        equation = OldTex('(', '2', v_tex, ')', '\\cdot', w_tex, '=', '2(', v_tex, '\\cdot', w_tex, ')')
        ...
        self.play(Transform(*[VGroup(mob, mob.label) for mob in (v, new_v)]), run_time=2)
        last_mob = self.get_mobjects_from_last_animation()[0]
        self.remove(last_mob)
        self.add(*last_mob)
        Transform(VGroup(v.proj, v.proj_line), VGroup(new_v.proj, new_v.proj_line)).update(1)
        self.play(Write(words))
        self.wait()
        two_v_parts = equation[1:3]
        equation.remove(*two_v_parts)
        for full_v, projector, stable in zip([False, True], [w, v], [v, w]):
            self.play(Transform(projector.copy(), projector.proj), ShowCreation(projector.proj_line))
            self.remove(self.get_mobjects_from_last_animation()[0])
            self.add(projector.proj)
            self.wait()
            if equation not in self.get_mobjects():
                self.play(Write(equation), Transform(new_v.label.copy(), VMobject(*two_v_parts)))
                self.wait()
            v_parts = [v]
            if full_v:
                v_parts += [v.proj, v.proj_line]
            self.play(*[v_part.restore for v_part in v_parts], rate_func=there_and_back, run_time=2)
            self.wait()
...
class LurkingQuestion(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(ApplyPointwiseFunction(lambda p: 10 * (p + 2 * DOWN) / get_norm(p + 2 * DOWN), everything))

class TwoDToOneDScene(LinearTransformationScene):
    CONFIG = {'include_background_plane': False, 'foreground_plane_kwargs': {'x_radius': FRAME_X_RADIUS, 'y_radius': FRAME_Y_RADIUS, 'secondary_line_ratio': 1}, 't_matrix': [[2, 0], [1, 0]]}
...
class Introduce2Dto1DLinearTransformations(TwoDToOneDScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(number_line), *[Animation(v) for v in (self.i_hat, self.j_hat)])
...
class Symbolic2To1DTransform(Scene):
    ...
    def construct(self):
        ...
        VMobject(input_array, in_arrow, func, out_arrow, output_array).arrange(RIGHT, buff=SMALL_BUFF)
        ...
        self.play(Transform(mover, Dot().move_to(func)), ShowCreation(in_arrow), rate_func=rush_into, run_time=0.5)
        self.play(Transform(mover, output_array), ShowCreation(out_arrow), rate_func=rush_from, run_time=0.5)
...
class OkayToIgnoreFormalProperties(Scene):
    ...
    def construct(self):
        ...
        self.play(randy.look, LEFT, morty.look, RIGHT)
        self.wait()
...
class AdditivityProperty(TwoDToOneDScene):
    ...
    def get_symbols(self):
        ...
        if self.sum_before:
            ...
            result = VGroup(VectorizedPoint(tex_mob.get_left()), tex_mob, VectorizedPoint(tex_mob.get_right()))
        tex_mob.set_color_by_tex(v_tex, V_COLOR)
        tex_mob.set_color_by_tex(w_tex, W_COLOR)
...
class ScalingProperty(TwoDToOneDScene):
    CONFIG = {'show_basis_vectors': False, 'scale_before': True, 'scalar': 2}
...
class NonLinearFailsDotTest(TwoDTo1DTransformWithDots):
    ...
    def construct(self):
        ...
        offset = LEFT + DOWN
        vect = 2 * RIGHT + UP
        dots = VMobject(*[Dot(offset + a * vect, radius=0.075) for a in np.linspace(-2, 3, 18)])
        dots.set_submobject_colors_by_gradient(YELLOW_B, YELLOW_C)
        func = lambda p: (p[0] ** 2 - p[1] ** 2) * RIGHT
        new_dots = VMobject(*[Dot(func(dot.get_center()), color=dot.get_color(), radius=dot.radius) for dot in dots])
        ...
        self.apply_nonlinear_transformation(func, added_anims=[Transform(dots, new_dots)])
        self.play(Write(words))
        self.wait()
...
class ShowMatrix(TwoDToOneDScene):
    ...
    def show_matrix(self):
        ...
        for i, vect in enumerate([self.i_hat, self.j_hat]):
            self.play(Write(vect.words, run_time=1), ApplyMethod(vect.shift, 0.5 * UP, rate_func=there_and_back))
            self.wait()
            self.play(vect.words[1].copy().move_to, entries[i])
            self.wait()

class FollowVectorViaCoordinates(TwoDToOneDScene):
    CONFIG = {'t_matrix': [[1, 0], [-2, 0]], 'v_coords': [3, 3], 'written_v_coords': ['x', 'y'], 'concrete': False}
    ...
    def construct(self):
        ...
        scaling_anim_tuples = self.get_scaling_anim_tuples(basis_labels, array, [DOWN, RIGHT])
        ...
        for i, anim_tuple in enumerate(scaling_anim_tuples):
            ...
            to_fade += movers[:-1]
            if i == 1:
                self.play(*[ApplyMethod(m.shift, self.v_coords[0] * RIGHT) for m in movers])
            self.wait()
        self.play(*list(map(FadeOut, to_fade)) + [vect.restore for vect in (self.i_hat, self.j_hat)])
        self.apply_transposed_matrix(self.t_matrix)
        self.play(Write(self.number_line.get_numbers(), run_time=1))
        self.play(self.i_hat.shift, 0.5 * UP, self.j_hat.shift, DOWN)
        if self.concrete:
        ...
        self.wait()
        scaling_anim_tuples = self.get_scaling_anim_tuples(new_labels, array, [UP, DOWN])
        for i, anim_tuple in enumerate(scaling_anim_tuples):
            self.play(*anim_tuple)
    ...
    def get_scaling_anim_tuples(self, labels, array, directions):
        scaling_anim_tuples = []
        bases = (self.i_hat, self.j_hat)
        quints = list(zip(bases, self.v_coords, labels, array.get_entries(), directions))
        for basis, scalar, label, entry, direction in quints:
            ...
            scaling_anim_tuples.append((ApplyMethod(label.move_to, label.target), ApplyMethod(entry.copy().move_to, entry.target), Transform(basis, basis.scaled)))
...
class TwoDOneDMatrixMultiplication(Scene):
    CONFIG = {'matrix': [[1, -2]], 'vector': [4, 3], 'order_left_to_right': False}
    ...
    def show_product(self, matrix, vector):
        ...
        pairs = [VMobject(e1.copy(), OldTex('\\cdot'), e2.copy()).arrange(LEFT if self.order_left_to_right else RIGHT) for e1, e2 in starter_pairs]
        ...
        for starter_pair, pair in zip(starter_pairs, pairs):
            self.play(Transform(VMobject(*starter_pair).copy(), pair, path_arc=-np.pi / 2))
        self.wait()
...
class AssociationBetweenMatricesAndVectors(Scene):
    ...
    def construct(self):
        ...
        vectors.save_state()
        self.wait()
        self.play(Transform(vectors, matrices, path_arc=np.pi / 2, lag_ratio=0.5, run_time=2))
        self.wait()
...
class SomeKindOfConnection(Scene):
    ...
    def construct(self):
        ...
        for array in (matrix, v_coords):
            array.add_to_back(BackgroundRectangle(array))
        self.play(*list(map(ShowCreation, [plane, number_line, v_line])) + [Write(numbers, run_time=2)])
        self.play(Write(matrix, run_time=1))
        mover = plane.copy()
        interim = plane.copy().scale(0.8).move_to(number_line)
        for target in (interim, squish_plane):
            self.play(Transform(mover, target), Animation(plane), run_time=1)
        self.wait()
        self.play(ShowCreation(vect))
...
class ProjectOntoUnitVectorNumberline(VectorScene):
    CONFIG = {'randomize_dots': True, 'animate_setup': True, 'zoom_factor': 1, 'u_hat_color': YELLOW, 'tilt_angle': np.pi / 6}
    ...
    def construct(self):
        ...
        self.wait()
        self.play(self.number_line.set_stroke, None, 2, Transform(vectors, proj_dots), Transform(proj_lines, proj_dots), Animation(self.u_hat), lag_ratio=0.5, run_time=2)
        self.wait()
    ...
    def get_dots(self, vectors):
        return VGroup(*[Dot(v.get_end(), color=v.get_color(), radius=0.075) for v in vectors])

    def get_proj_dots(self, dots):
        return VGroup(*[dot.copy().move_to(get_projection(dot.get_center(), self.u_hat.get_end())) for dot in dots])

    def get_proj_lines(self, dots, proj_dots):
        return VGroup(*[DashedLine(d1.get_center(), d2.get_center(), buff=0, color=d1.get_color(), dash_length=0.15) for d1, d2 in zip(dots, proj_dots)])

class ProjectionFunctionSymbol(Scene):

    def construct(self):
        v_tex = '\\vec{\\textbf{v}}'
        equation = OldTex('P(', v_tex, ')=', '\\text{number }', v_tex, '\\text{ lands on}')
...
class ProjectLineOfDots(ProjectOntoUnitVectorNumberline):
    CONFIG = {'randomize_dots': False, 'animate_setup': False}
...
class ProjectSingleVectorOnUHat(ProjectOntoUnitVectorNumberline):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(v), Write(v.label))
        self.wait()
        self.play(ShowCreation(v.proj_line), Transform(v.copy(), v.proj))
        self.wait()
        self.play(FadeOut(v), FadeOut(v.proj_line), FadeOut(v.label), Write(dot_product))
        self.wait()
...
class AskAboutProjectionMatrix(Scene):
    ...
    def construct(self):
        ...
        basis_words = [OldTexText('Where', '$\\hat{\\%smath}$' % char, 'lands') for char in ('i', 'j')]
...
class ProjectBasisVectors(ProjectOntoUnitVectorNumberline):
    CONFIG = {'animate_setup': False, 'zoom_factor': 3, 'u_hat_color': YELLOW, 'tilt_angle': np.pi / 5}
    ...
    def construct(self):
        ...
        for dot in proj_dots:
            dot.scale(0.1)
        proj_basis = VGroup(*[get_vect_mob_projection(vector, self.u_hat) for vector in basis_vectors])
        ...
        VGroup(question, matrix).arrange(DOWN).to_corner(UP + LEFT, buff=MED_SMALL_BUFF / 2)
        matrix_rect = BackgroundRectangle(matrix)
        i_label = OldTex(i_tex[1:-1])
        j_label = OldTex(j_tex[1:-1])
        u_label = OldTex('\\hat{\\textbf{u}}')
        trips = list(zip((i_label, j_label, u_label), (i_hat, j_hat, self.u_hat), (DOWN + RIGHT, UP + LEFT, UP)))
        ...
        self.play(ShowCreation(proj_lines))
        self.play(Write(question), ShowCreation(matrix_rect), Write(matrix.get_brackets()), run_time=2)
        ...
        u_x, u_y = [OldTex('u_%s' % c).set_color(self.u_hat.get_color()) for c in ('x', 'y')]
    ...
    def show_symmetry(self, vect, coord, coord_landing_spot):
        ...
        added_mobjects = [mob for mob in self.get_mobjects() if mob not in starting_mobjects + [final_coord]]
...
class ShowSingleProjection(ProjectBasisVectors):
    ...
    def construct(self):
        ...
        proj_line = DashedLine(vector.get_end(), proj.get_end(), color=vector.get_color())
        coords = Matrix(['x', 'y'])
...
class GeneralTwoDOneDMatrixMultiplication(TwoDOneDMatrixMultiplication):
    CONFIG = {'matrix': [['u_x', 'u_y']], 'vector': ['x', 'y'], 'order_left_to_right': True}
    ...
    def construct(self):
        ...
        words = VGroup(OldTexText('Matrix-vector product'), OldTex('\\Updownarrow'), OldTexText('Dot product'))
...
class UHatIsTransformInDisguise(Scene):
    ...
    def construct(self):
        u_tex = '$\\hat{\\textbf{u}}$'
        words = OldTexText(u_tex, 'is secretly a \\\\', 'transform', 'in disguise')
...
class AskAboutNonUnitVectors(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('What about \\\\ non-unit vectors', target_mode='raise_left_hand')
        self.random_blink(2)

class ScaleUpUHat(ProjectOntoUnitVectorNumberline):
    CONFIG = {'animate_setup': False, 'u_hat_color': YELLOW, 'tilt_angle': np.pi / 6, 'scalar': 3}
    ...
    def scale_u_hat(self):
        ...
        new_u.coords = Matrix(['%du_x' % self.scalar, '%du_y' % self.scalar])
        ...
        self.play(Transform(self.u_hat, new_u), Transform(self.u_hat.coords, new_u.coords))
        self.wait()
    ...
    def transform_basis_vectors(self):
        ...
        for b in bases:
            ...
            self.wait()
            self.play(Transform(mover, b.scaled_proj), Transform(b.proj.label, b.scaled_proj.label))
            self.wait()
        self.play(*list(map(FadeOut, [mob for mob in self.get_mobjects() if mob not in start_state])))
...
class TwoDOneDTransformationSeparateSpace(Scene):
    ...
    def construct(self):
        ...
        for array in (matrix, v_coords):
            array.add_to_back(BackgroundRectangle(array))
        start_words = OldTexText('\\centering Any time you have a \\\\', '2d-to-1d linear transform...')
        end_words = OldTexText("\\centering ...it's associated \\\\", 'with some vector')
        ...
        interim = plane.copy().scale(0.8).move_to(number_line)
        for target in (interim, squish_plane):
            self.play(Transform(mover, target), Animation(plane), Animation(start_words), run_time=1)
        self.wait()
...
class TranslateToTheWorldOfTransformations(TwoDOneDMatrixMultiplication):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(dot.set_fill, BLACK, 0, Transform(v1, matrix))
...
class WhatTheVectorWantsToBe(Scene):
    ...
    def construct(self):
        ...
        self.play(Transform(plane.copy(), squish_plane, run_time=3), *list(map(Animation, [words, matrix, plane, vect, v_coords])))
        self.wait()
