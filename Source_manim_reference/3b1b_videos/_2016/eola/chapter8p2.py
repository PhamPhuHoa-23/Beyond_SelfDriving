...
class OpeningQuote(Scene):
    ...
    def construct(self):
        words = OldTexText('From [Grothendieck], I have also learned not', 'to take glory in the ', 'difficulty of a proof:', 'difficulty means we have not understood.', 'The idea is to be able to ', 'paint a landscape', 'in which the proof is obvious.', arg_separator=' ')
...
class CrossProductSymbols(Scene):
    ...
    def construct(self):
        ...
        equation = OldTex(v_tex, '\\times', w_tex, '=', p_tex)
...
class BruteForceVerification(Scene):
    ...
    def construct(self):
        ...
        def get_term(e1, e2, e3, e4):
            group = VGroup(e1.copy(), e2.copy(), OldTex('-'), e3.copy(), e4.copy())
            group.arrange()
            return group
        cross = Matrix(list(it.starmap(get_term, [(v2, w3, v3, w2), (v3, w1, v1, w3), (v1, w2, v2, w1)])))
        cross_product = VGroup(v.copy(), OldTex('\\times'), w.copy(), OldTex('='), cross.copy())
        ...
        self.play(ShowCreation(h_line), ShowCreation(v_line), Write(cross_product))
        v_tex, w_tex = get_vect_tex(*'vw')
        v_dot, w_dot = [OldTex(tex, '\\cdot', '(', v_tex, '\\times', w_tex, ')', '= 0') for tex in (v_tex, w_tex)]
        theta_def = OldTex('\\theta', '= \\cos^{-1} \\big(', v_tex, '\\cdot', w_tex, '/', '(||', v_tex, '||', '\\cdot', '||', w_tex, '||)', '\\big)')
        length_check = OldTex('||', '(', v_tex, '\\times', w_tex, ')', '|| = ', '(||', v_tex, '||)', '(||', w_tex, '||)', '\\sin(', '\\theta', ')')
...
class Prerequisites(Scene):
    ...
    def construct(self):
        ...
        left_rect, right_rect = [rect.copy().shift(DOWN / 2).to_edge(edge) for edge in (LEFT, RIGHT)]
        chapter5 = OldTexText('\n            \\centering \n            Chapter 5 \n            Determinants\n        ')
...
class DotProductToTransformSymbol(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Write(equals), Write(dot), Transform(matrix.copy(), vector), Transform(right_input.copy(), left_input))
...
class ThreeStepPlan(Scene):
    ...
    def construct(self):
        ...
        v_tex, w_tex = get_vect_tex(*'vw')
        v_text, w_text, cross_text = ['$%s$' % s for s in (v_tex, w_tex, v_tex + '\\times' + w_tex)]
        steps = [OldTexText('1. Define a 3d-to-1d', 'linear \\\\', 'transformation', 'in terms of', v_text, 'and', w_text), OldTexText('2. Find its', 'dual vector'), OldTexText('3. Show that this dual is', cross_text)]
        ...
        VGroup(*steps).arrange(DOWN, aligned_edge=LEFT, buff=LARGE_BUFF).next_to(h_line, DOWN, buff=MED_SMALL_BUFF)
        self.add(title)
        ...
        self.play(Transform(VGroup(linear, transformation), linear_transformation), *list(map(FadeOut, steps)))
        self.wait()
    ...
    def get_det(self):
        matrix = Matrix(np.array([['\\hat{\\imath}', '\\hat{\\jmath}', '\\hat{k}'], ['v_%d' % d for d in range(1, 4)], ['w_%d' % d for d in range(1, 4)]]).T)
...
class DefineDualTransform(Scene):
    ...
    def show_triple_cross_product(self):
        ...
        arrays = [Matrix(['%s_%d' % (s, d) for d in range(1, 4)]) for s in 'uvw']
        defs_equals = VGroup()
        definitions = VGroup()
        ...
        mobs_with_targets = list(it.chain(tex_mobs, *[a.get_entries() for a in arrays]))
        for mob in mobs_with_targets:
            mob.target = mob.copy()
        matrix = Matrix(np.array([[e.target for e in array.get_entries()] for array in arrays]).T)
        det_text = get_det_text(matrix, background_rect=False)
        syms = times1, times2, equals = [OldTex(sym) for sym in ('\\times', '\\times', '=')]
        triple_cross = VGroup(u_tex.target, times1, v_tex.target, times2, w_tex.target, equals)
        ...
        self.wait(2)
        self.play(*[Transform(mob.copy(), mob.target) for mob in tex_mobs] + [Write(times1), Write(times2)])
        triple_cross.add(*self.get_mobjects_from_last_animation()[:3])
        self.play(*[Transform(mob.copy(), mob.target) for mob in mobs_with_targets if mob not in tex_mobs])
    ...
    def write_function(self):
        ...
        func_tex = OldTex('f\\left(%s\\right)' % matrix_to_tex_string(list('xyz')))
        func_tex.scale(0.7)
        ...
        func_tex = VGroup(VGroup(*[func_tex[i] for i in (0, 1, 2, -2, -1)]), func_input)
        func_tex.next_to(self.equals, LEFT)
        self.play(FadeOut(self.title), FadeOut(self.triple_cross), *[Transform(mob, mob.target) for mob in [brace, number_text]])
        self.play(*[Transform(mob, mob.target) for mob in self.u_entries])
        self.play(*[Write(VGroup(vect_brace, vect_brace.tex)) for vect_brace in (v_brace, w_brace)])
        self.wait()
    ...
    def introduce_dual_vector(self):
        ...
        dual_dot = VGroup(dual_vector, OldTex('\\cdot').next_to(dual_vector))
        ...
        matrix_arrow = Arrow(matrix_words.get_top(), matrix.get_bottom(), color=WHITE)
        self.play(Transform(func, matrix), MoveToTarget(func_input), FadeOut(self.variables_text))
        self.wait()
        ...
        self.wait()
        p_coords = VGroup(*list(map(Tex, ['p_%d' % d for d in range(1, 4)])))
    ...
    def expand_dot_product(self):
        ...
        for p, x in zip(p_entries, input_entries):
            self.play(MoveToTarget(p.copy()), MoveToTarget(x.copy()), Write(p.sym), Write(x.sym))
        ...
        quints = [(x, v2, w3, v3, w2), (y, v3, w1, v1, w3), (z, v1, w2, v2, w1)]
        ...
        for quint in quints:
            ...
            self.wait(0.5)
            self.play(*[MoveToTarget(mob) for mob in quint] + [Write(mob.sym) for mob in quint])
            self.wait()
        self.play(ApplyFunction(lambda m: m.arrange(DOWN, buff=MED_SMALL_BUFF + SMALL_BUFF).next_to(cross_components, LEFT), new_ps), *list(map(FadeOut, to_fade)))
        self.play(*[Write(OldTex('=').next_to(p, buff=2 * SMALL_BUFF)) for p in new_ps])
    ...
    def ask_question(self):
        ...
        question = OldTexText('What vector', p_tex, 'has \\\\ the property that')
        question.to_edge(UP)
...
class WhyAreWeDoingThis(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('Um...why are \\\\ we doing this?', target_mode='confused')
        self.random_blink()
...
class NowForTheCoolPart(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play_student_changes(*['happy'] * 3)
        self.random_blink(2)
        self.teacher_says("Let's answer the same question,\\\\", 'but this time geometrically')
...
class DotProductWords(Scene):
    ...
    def construct(self):
        ...
        words = VGroup(*it.starmap(TexText, [('(Length of projection)',), ('(Length of ', p_tex, ')')]))
        times = OldTex('\\times')
...
class GeometricVolumeWords(Scene):
    ...
    def construct(self):
        ...
        words = VGroup(OldTexText('(Area of', 'parallelogram', ')$\\times$'), OldTexText('(Component of $%s$' % matrix_to_tex_string(list('xyz')), 'perpendicular to', v_tex, 'and', w_tex, ')'))
...
class CrossVectorEmphasisWords(Scene):
    ...
    def construct(self):
        v_tex, w_tex = ['$%s$' % s for s in get_vect_tex(*'vw')]
        words = [OldTexText('Perpendicular to', v_tex, 'and', w_tex), OldTexText('Length = (Area of ', 'parallelogram', ')')]
        for word in words:
...
class ChangeOfBasisPreview(LinearTransformationScene):
    CONFIG = {'include_background_plane': False, 'foreground_plane_kwargs': {'x_radius': FRAME_WIDTH, 'y_radius': FRAME_WIDTH, 'secondary_line_ratio': 0}, 't_matrix': [[2, 1], [-1, 1]], 'i_target_color': YELLOW, 'j_target_color': MAROON_B, 'sum_color': PINK, 'vector': [-1, 2]}
    ...
    def construct(self):
        ...
        coords.target.move_to(pinky.get_corner(UP + LEFT), aligned_edge=DOWN + RIGHT)
        self.play(Write(coords), randy.change_mode, 'speaking')
        self.scale_basis_vectors()
        self.apply_transposed_matrix(self.t_matrix, added_anims=[MoveToTarget(coords), ApplyMethod(pinky.change_mode, 'speaking'), ApplyMethod(randy.change_mode, 'plain')])
        self.play(randy.change_mode, 'erm', self.i_hat.set_color, self.i_target_color, self.j_hat.set_color, self.j_target_color)
    ...
    def scale_basis_vectors(self):
        ...
        self.wait(2)
        self.play(FadeOut(sum_vect), self.i_hat.restore, self.j_hat.restore)
        self.wait()