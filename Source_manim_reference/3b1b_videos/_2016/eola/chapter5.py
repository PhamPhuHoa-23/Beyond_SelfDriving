...
class Blob(Circle):
    CONFIG = {'stroke_color': TEAL, 'fill_color': BLUE_E, 'fill_opacity': 1, 'random_seed': 1, 'random_nudge_size': 0.5, 'height': 2}
...
class OpeningQuote(Scene):
    ...
    def construct(self):
        words = OldTexText(['``The purpose of computation is \\\\', 'insight', ', not ', 'numbers.', "''"], arg_separator='')
        words.to_edge(UP)
...
class SquishingTransformation(LinearTransformationScene):
    CONFIG = {'foreground_plane_kwargs': {'x_radius': 3 * FRAME_X_RADIUS, 'y_radius': 3 * FRAME_X_RADIUS, 'secondary_line_ratio': 0}}
...
class DiagonalExample(LinearTransformationScene):
    CONFIG = {'show_square': False, 'show_coordinates': True, 'transposed_matrix': [[3, 0], [0, 2]]}
    ...
    def construct(self):
        ...
        if self.show_square:
            ...
            self.play(ShowCreation(background_rect), Transform(width.copy(), width_target), Transform(height.copy(), height_target), *list(map(Write, [words, times, eq_det])))
            self.wait()
...
class ShearExample(DiagonalExample):
    CONFIG = {'show_square': False, 'show_coordinates': True, 'transposed_matrix': [[1, 0], [1, 1]]}

class ShearExampleWithSquare(DiagonalExample):
    CONFIG = {'show_square': True, 'show_coordinates': True, 'show_coordinates': False, 'transposed_matrix': [[1, 0], [1, 1]]}
...
class WhatHappensToOneSquareHappensToAll(LinearTransformationScene):
    ...
    def construct(self):
        self.setup()
        self.add_unit_square()
        pairs = [(2 * RIGHT + UP, 1), (3 * LEFT, 2), (2 * LEFT + DOWN, 0.5), (3.5 * RIGHT + 2.5 * UP, 1.5), (RIGHT + 2 * DOWN, 0.25), (3 * LEFT + 3 * DOWN, 1)]
        squares = VMobject()
...
class BreakBlobIntoGridSquares(LinearTransformationScene):
    CONFIG = {'square_size': 0.5, 'blob_height': 3}

    def construct(self):
        self.setup()
        blob = Blob(height=self.blob_height, random_seed=5, random_nudge_size=0.2)
        ...
        self.play(ShowCreation(squares, lag_ratio=0.5, run_time=2))
        self.add_transformable_mobject(squares)
        self.wait()
        self.apply_transposed_matrix([[1, -1], [0.5, 1]])
        self.wait()
...
class DeterminantIsOneHalf(NameDeterminant):
    CONFIG = {'t_matrix': [[0.5, -0.5], [0.5, 0.5]], 'foreground_plane_kwargs': {'x_radius': FRAME_WIDTH, 'y_radius': FRAME_WIDTH, 'secondary_line_ratio': 0}}

class DeterminantIsZero(NameDeterminant):
    CONFIG = {'t_matrix': [[4, 2], [2, 1]]}
...
class FlipSpaceOver(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Rotate(plane1, axis=axis, rate_func=lambda t: smooth(t / 2), run_time=1.5, path_arc=np.pi / 2))
        self.remove(plane1)
        self.play(Rotate(plane2, axis=axis, rate_func=lambda t: smooth((t + 1) / 2), run_time=1.5, path_arc=np.pi / 2))
        self.wait()
...
class IHatJHatOrientation(NegativeDeterminantTransformation):
    ...
    def construct(self):
        ...
        arc.add_tip()
        words1 = OldTexText(['$\\hat{\\jmath}$', 'is to the', 'left', 'of', '$\\hat{\\imath}$'])
        ...
        words2 = OldTexText(['$L(\\hat{\\jmath})$', 'is to the \\\\', '\\emph{right}', 'of', '$L(\\hat{\\imath})$'])
        ...
        self.play(ShowCreation(arc), Write(words2, run_time=2))
        self.wait()
    ...
    def construct(self):
        ...
        for mob in det_text.split():
            if isinstance(mob, Tex):
                mob.add_background_rectangle()
        matrix_background = BackgroundRectangle(matrix)
        self.play(ShowCreation(matrix_background), Write(matrix), Write(det_text))
...
class SlowlyRotateIHat(LinearTransformationScene):
    ...
    def construct(self):
        self.setup()
        self.add_unit_square()
        self.apply_transposed_matrix([[-1, 0], [0, 1]], path_arc=np.pi, run_time=30, rate_func=linear)

class DeterminantGraphForRotatingIHat(Scene):

    def construct(self):
        t_axis = NumberLine(big_tick_numbers=[], x_min=0, x_max=10, color=WHITE)
...
class RightHandRule(Scene):
    CONFIG = {'flip': False, 'labels_tex': ['\\hat{\\imath}', '\\hat{\\jmath}', '\\hat{k}'], 'colors': [X_COLOR, Y_COLOR, Z_COLOR]}
    ...
    def construct(self):
        ...
        self.play(ShowCreation(hand.outline, run_time=2, rate_func=linear), FadeIn(hand.inlines))
        self.wait()
...
class TwoDDeterminantFormulaIntuition(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.apply_transposed_matrix([[1, 0], [float(b) / d, 1]], added_anims=[ApplyMethod(m.shift, b * RIGHT) for m in (side_brace, height)])
        self.wait()
...
class FullFormulaExplanation(LinearTransformationScene):
    ...
    def add_polygons(self):
        ...
        shapes_colors_and_tex = [(Polygon(ORIGIN, a, a + c), MAROON, 'ac/2'), (Polygon(ORIGIN, d + b, d, d), TEAL, '\\dfrac{bd}{2}'), (Polygon(a + c, a + b + c, a + b + c, a + b + c + d), TEAL, '\\dfrac{bd}{2}'), (Polygon(b + d, a + b + c + d, b + c + d), MAROON, 'ac/2'), (Polygon(a, a + b, a + b + c, a + c), PINK, 'bc'), (Polygon(d, d + b, d + b + c, d + c), PINK, 'bc')]
        everyone = VMobject()
    ...
    def add_braces(self):
        ...
        c = self.i_hat.get_end()[1] * UP
        d = self.j_hat.get_end()[1] * UP
        quads = [(ORIGIN, a, DOWN, 'a'), (a, a + b, DOWN, 'b'), (a + b, a + b + c, RIGHT, 'c'), (a + b + c, a + b + c + d, RIGHT, 'd'), (a + b + c + d, b + c + d, UP, 'a'), (b + c + d, d + c, UP, 'b'), (d + c, d, LEFT, 'c'), (d, ORIGIN, LEFT, 'd')]
        everyone = VMobject()
...
class ThreeDDetFormula(Scene):
    ...
    def construct(self):
        ...
        parts = it.starmap(VMobject, [[matrix], [Tex('='), a.copy(), m1], [Tex('-'), b.copy(), m2], [Tex('+'), c.copy(), m3]])
        parts = list(parts)
        for part in parts:
