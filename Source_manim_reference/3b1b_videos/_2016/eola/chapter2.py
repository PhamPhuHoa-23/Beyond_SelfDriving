...
class CoordinatesAsScalars(VectorScene):
    ...
    def general_idea_of_scalars(self, array, vector):
        ...
        self.play(Transform(x, new_x), Transform(y, new_y), Write(title))
        ...
        self.play(Transform(x, new_x), Transform(y, new_y), FadeOut(i_hat), FadeOut(j_hat), Write(new_array.get_brackets()), FadeIn(VMobject(*starting_mobjects)), FadeOut(title))
        self.remove(x, y)
        self.add(new_array)
        return new_array
    ...
    def scale_basis_vectors(self, new_array):
        ...
        for coord, v, label, factor, shift_right in [(x, i_hat, i_hat_label, self.vector_coords[0], False), (y, j_hat, j_hat_label, self.vector_coords[1], True)]:
            ...
            self.play(Transform(v.copy(), faded_v), Transform(v, scaled_v), Transform(VMobject(coord_copy, label), scaled_label))
            self.wait()
            if shift_right:
    ...
    def show_symbolic_sum(self, new_array, vector):
        new_mob = OldTex(['(%d)\\hat{\\imath}' % self.vector_coords[0], '+', '(%d)\\hat{\\jmath}' % self.vector_coords[1]])
...
class ShowVaryingLinearCombinations(VectorScene):
    CONFIG = {'vector1': [1, 2], 'vector2': [3, -1], 'vector1_color': MAROON_C, 'vector2_color': BLUE, 'vector1_label': 'v', 'vector2_label': 'w', 'sum_color': PINK, 'scalar_pairs': [(1.5, 0.6), (0.7, 1.3), (-1, -1.5), (1, -1.13), (1.25, 0.5), (-0.8, 1.3)], 'leave_sum_vector_copies': False, 'start_with_non_sum_scaling': True, 'finish_with_standard_basis_comparison': True, 'finish_by_drawing_lines': False}
    ...
    def get_scalar_anims(self, v1, v2, v1_label, v2_label):
        ...
        return [RangingValues(tracked_mobject=label, tracked_mobject_next_to_kwargs={'direction': LEFT, 'buff': 0.1}, scale_factor=0.75, value_function=get_val_func(v)) for v, label in [(v1, v1_label), (v2, v2_label)]]
    ...
    def initial_scaling(self, v1, v2, label_anims, scalar_anims):
        ...
        anims = [ApplyMethod(v.scale, s, rate_func=rf) for v, s, rf in zip([v1, v2], scalar_pair, self.get_rate_func_pair())]
    ...
    def scale_with_sum(self, v1, v2, label_anims, scalar_anims):
        ...
        while self.scalar_pairs:
            scalar_pair = self.scalar_pairs.pop(0)
            anims = [ApplyMethod(v.scale, s / s_old, rate_func=rf) for v, s, s_old, rf in zip([v1, v2], scalar_pair, self.last_scalar_pair, self.get_rate_func_pair())]
    ...
    def get_sum_animations(self, v1, v2):
        ...
        sum_anim = UpdateFromFunc(self.sum_vector, lambda v: v.put_start_and_end_on(v1.get_start(), v2.get_end()))
    ...
    def standard_basis_comparison(self, label_anims, scalar_anims):
        ...
        array = Matrix([mob.copy().set_color(color) for mob, color in zip(alt_coords, [self.vector1_color, self.vector2_color])])
        array.scale(0.8)
        array.to_edge(UP)
        array.shift(RIGHT)
        ...
        self.add(array)
        self.play(FadeOut(everything), Animation(array))
        ...
        self.play(ApplyMethod(x.set_color, X_COLOR), ApplyMethod(y.set_color, Y_COLOR))
        neq = OldTex('\\neq')
        neq.next_to(array)
    ...
    def draw_lines(self, v1, v2, label_anims, scalar_anims):
        ...
        for w1, w2 in ((v2, v1), (v1, v2)):
            ...
            for num in (scale_factor, -1, -1.0 / scale_factor):
                ...
                self.play(ApplyMethod(w2.scale, num), UpdateFromFunc(line, lambda l: l.put_start_and_end_on(curr_tip, self.sum_vector.get_end())), *aux_anims)
                self.add(line, v2)
            self.wait()

class AltShowVaryingLinearCombinations(ShowVaryingLinearCombinations):
    CONFIG = {'scalar_pairs': [(1.5, 0.3), (0.64, 1.3), (-1, -1.5), (1, 1.13), (1.25, 0.5), (-0.8, 1.14)], 'finish_with_standard_basis_comparison': False}
...
class NameLinearCombinations(Scene):
    ...
    def construct(self):
        v_color = MAROON_C
        w_color = BLUE
        words = OldTexText(["``Linear combination'' of", '$\\vec{\\textbf{v}}$', 'and', '$\\vec{\\textbf{w}}$'])
...
class LinearCombinationsDrawLines(ShowVaryingLinearCombinations):
    CONFIG = {'scalar_pairs': [(1.5, 0.6), (0.7, 1.3), (1, 1)], 'start_with_non_sum_scaling': False, 'finish_with_standard_basis_comparison': False, 'finish_by_drawing_lines': True}

class LinearCombinationsWithSumCopies(ShowVaryingLinearCombinations):
    CONFIG = {'scalar_pairs': [(1.5, 0.6), (0.7, 1.3), (-1, -1.5), (1, -1.13), (1.25, 0.5), (-0.8, 1.3), (-0.9, 1.4), (0.9, 2)], 'leave_sum_vector_copies': True, 'start_with_non_sum_scaling': False, 'finish_with_standard_basis_comparison': False, 'finish_by_drawing_lines': False}

class LinearDependentVectors(ShowVaryingLinearCombinations):
    CONFIG = {'vector1': [1, 2], 'vector2': [0.5, 1], 'vector1_color': MAROON_C, 'vector2_color': BLUE, 'vector1_label': 'v', 'vector2_label': 'w', 'sum_color': PINK, 'scalar_pairs': [(1.5, 0.6), (0.7, 1.3), (-1, -1.5), (1.25, 0.5), (-0.8, 1.3), (-0.9, 1.4), (0.9, 2)], 'leave_sum_vector_copies': False, 'start_with_non_sum_scaling': False, 'finish_with_standard_basis_comparison': False, 'finish_by_drawing_lines': False}
...
class WhenVectorsLineUp(LinearDependentVectors):
    CONFIG = {'vector1': [3, 2], 'vector2': [1.5, 1], 'scalar_pairs': [(1.5, 0.6), (0.7, 1.3)]}

class AnimationUnderSpanDefinition(ShowVaryingLinearCombinations):
    CONFIG = {'scalar_pairs': [(1.5, 0.6), (0.7, 1.3), (-1, -1.5), (1.25, 0.5), (0.8, 1.3), (0.93, -1.4), (-2, -0.5)], 'leave_sum_vector_copies': True, 'start_with_non_sum_scaling': False, 'finish_with_standard_basis_comparison': False, 'finish_by_drawing_lines': False}
...
class VectorsToDotsScene(VectorScene):
    CONFIG = {'num_vectors': 16, 'start_color': PINK, 'end_color': BLUE_E}
...
class VectorsOnALine(VectorsToDotsScene):
    ...
    def play_final_animation(self, vectors, rate_functions):
        ...
        self.play(*[Transform(v, mob, rate_func=rf, run_time=2) for v, mob, rf in zip(vectors, line_copies, rate_functions)])

class VectorsInThePlane(VectorsToDotsScene):
    CONFIG = {'num_vectors': 16, 'start_color': PINK, 'end_color': BLUE_E}

    def get_vectors(self):
        return [Vector([x, y]) for x in np.arange(-int(FRAME_X_RADIUS) - 0.5, int(FRAME_X_RADIUS) + 0.5) for y in np.arange(-int(FRAME_Y_RADIUS) - 0.5, int(FRAME_Y_RADIUS) + 0.5)]

    def play_final_animation(self, vectors, rate_functions):
        h_line = Line(FRAME_X_RADIUS * RIGHT, FRAME_X_RADIUS * LEFT, stroke_width=0.5, color=BLUE_E)
        v_line = Line(FRAME_Y_RADIUS * UP, FRAME_Y_RADIUS * DOWN, stroke_width=0.5, color=BLUE_E)
        line_pairs = [VMobject(h_line.copy().shift(y), v_line.copy().shift(x)) for v in vectors for x, y, z in [v.get_center()]]
        plane = NumberPlane()
        self.play(ShowCreation(plane), *[Transform(v, p, rate_func=rf) for v, p, rf in zip(vectors, line_pairs, rate_functions)])
        self.remove(*vectors)
        self.wait()
...
class LinearDependentWords(Scene):
    ...
    def construct(self):
        words1 = OldTexText(['$\\vec{\\textbf{v}}$', 'and', '$\\vec{\\textbf{w}}$', 'are', "``Linearly dependent'' "])
        v, _and, w, are, rest = words1.split()
        v.set_color(MAROON_C)
        w.set_color(BLUE)
        rest.set_color(YELLOW)
        words2 = OldTexText(['$\\vec{\\textbf{v}}$,', '$\\vec{\\textbf{w}}$', 'and', '$\\vec{\\textbf{u}}$', 'are', "``Linearly dependent'' "])
...
class LinearDependentEquations(Scene):
    ...
    def construct(self):
        ...
        title.to_edge(UP)
        self.add(title)
        equation1 = OldTex(['\\vec{\\textbf{w}}', '=', 'a', '\\vec{\\textbf{v}}'])
        ...
        equation2 = OldTex(['\\vec{\\textbf{u}}', '=', 'a', '\\vec{\\textbf{v}}', '+', 'b', '\\vec{\\textbf{w}}'])
        ...
        self.wait()
        self.play(Transform(equation1, equation2), Transform(low_words1, low_words2), Transform(arrow, arrows))
        self.wait(2)
        ...
        self.play(Transform(title, new_title), Transform(equation1, eq1_copy), Transform(arrow, arrow_copy), Transform(low_words1, new_low_words1))
        self.wait()
        self.play(Transform(equation1, eq2_copy), Transform(arrow, arrows), Transform(low_words1, new_low_words2))
        self.wait()

class AlternateDefOfLinearlyDependent(Scene):

    def construct(self):
        title1 = OldTexText(['$\\vec{\\textbf{v}}$,', '$\\vec{\\textbf{w}}$', 'and', '$\\vec{\\textbf{u}}$', 'are', 'linearly dependent', 'if'])
        title2 = OldTexText(['$\\vec{\\textbf{v}}$,', '$\\vec{\\textbf{w}}$', 'and', '$\\vec{\\textbf{u}}$', 'are', 'linearly independent', 'if'])
        ...
        self.wait(2)
        self.play(Transform(title1, title2), Write(subtitle), Transform(added_words1, added_words2))
        self.wait(3)
    ...
    def get_equations(self):
        equation1 = OldTex(['\\vec{\\textbf{u}}', '=', 'a', '\\vec{\\textbf{v}}', '+', 'b', '\\vec{\\textbf{w}}'])
        u = equation1.split()[0]
        equation1.submobjects = list(it.chain([VectorizedPoint(u.get_center())], equation1.submobjects[1:], [VectorizedPoint(u.get_left())], [u]))
        equation2 = OldTex(['\\left[\\begin{array}{c} 0 \\\\ 0 \\\\ 0 \\end{array} \\right]', '=', 'a', '\\vec{\\textbf{v}}', '+', 'b', '\\vec{\\textbf{w}}', '-', '\\vec{\\textbf{u}}'])
        equation3 = OldTex(['\\vec{\\textbf{0}}', '=', 'a', '\\vec{\\textbf{v}}', '+', 'b', '\\vec{\\textbf{w}}', '-', '\\vec{\\textbf{u}}'])
        equation4 = OldTex(['\\vec{\\textbf{0}}', '=', '0', '\\vec{\\textbf{v}}', '+', '0', '\\vec{\\textbf{w}}', '+0', '\\vec{\\textbf{u}}'])
        equation5 = OldTex(['\\vec{\\textbf{0}}', '=', 'a', '\\vec{\\textbf{v}}', '+', 'b', '\\vec{\\textbf{w}}', '+(-1)', '\\vec{\\textbf{u}}'])
        equation5.split()[-2].set_color(YELLOW)
        equation6 = OldTex(['\\vec{\\textbf{0}}', '=', 'a', '\\vec{\\textbf{v}}', '+', 'b', '\\vec{\\textbf{w}}', '+c', '\\vec{\\textbf{u}}'])
...
class TechnicalDefinitionOfBasis(Scene):
    ...
    def construct(self):
        ...
        title.to_edge(UP)
        definition = OldTexText(['The', 'basis', 'of a vector space is a set of', 'linearly independent', 'vectors that', 'span', 'the full space'])
