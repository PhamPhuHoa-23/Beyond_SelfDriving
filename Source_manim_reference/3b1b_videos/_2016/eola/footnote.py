...
class SymbolicThreeDTransform(Scene):
    CONFIG = {'input_coords': [2, 6, -1], 'output_coords': [3, 2, 0], 'title': 'Three-dimensional transformation'}
...
class PutTogether3x3Matrix(Scene):
    CONFIG = {'col1': [1, 0, -1], 'col2': [1, 1, 0], 'col3': [1, 0, 1]}
    ...
    def construct(self):
        ...
        everything = VMobject(i_to, i_array, OldTex('=').set_color(BLACK), j_to, j_array, OldTex('=').set_color(BLACK), k_to, k_array, OldTex('=').set_color(BLACK))
        ...
        matrix = Matrix(reduce(lambda a1, a2: np.append(a1, a2, axis=1), [m.copy().get_mob_matrix() for m in arrays]))
        matrix.to_edge(DOWN)
        ...
        self.wait()
        self.play(FadeOut(everything), Transform(VMobject(*start_entries), VMobject(*target_entries)), Transform(start_brackets, target_bracketes))
        self.wait()
...
class ShowVCoordinateMeaning(Scene):
    CONFIG = {'v_str': '\\vec{\\textbf{v}}', 'i_str': '\\hat{\\imath}', 'j_str': '\\hat{\\jmath}', 'k_str': '\\hat{k}', 'post_transform': False}
    ...
    def construct(self):
        ...
        lin_comb = VMobject(*list(map(Tex, ['x', self.i_str, '+', 'y', self.j_str, '+', 'z', self.k_str])))
        ...
        self.wait()
        self.play(Transform(coords.get_entries().copy(), VMobject(new_x, new_y, new_z), path_arc=-np.pi, lag_ratio=0.5), Write(VMobject(*[eq2, i, j, k] + list(plusses))), run_time=3)
        self.wait()
...
class ShowVCoordinateMeaningAfterTransform(ShowVCoordinateMeaning):
    CONFIG = {'v_str': 'L(\\vec{\\textbf{v}})', 'i_str': 'L(\\hat{\\imath})', 'j_str': 'L(\\hat{\\jmath})', 'k_str': 'L(\\hat{k})', 'post_transform': True}
...
class ShowMatrixVectorMultiplication(Scene):
    ...
    def construct(self):
        ...
        trips = [(matrix, DOWN, 'Transformation'), (vect, UP, 'Input vector'), (result, DOWN, 'Output vector')]
        braces = []
        ...
        sexts = list(zip(matrix.get_mob_matrix().transpose(), columns, vect.get_entries().split(), coords, [eq, plus1, plus2], [X_COLOR, Y_COLOR, Z_COLOR]))
        for o_col, col, start_coord, coord, sym, color in sexts:
            ...
            col.set_color(color)
            self.play(Write(col.get_brackets()), Transform(o_col.copy(), col.get_entries(), path_arc=-np.pi), Transform(start_coord.copy(), coord, path_arc=-np.pi), Write(sym))
            self.wait()
...
class ShowMatrixMultiplication(Scene):
    ...
    def construct(self):
        ...
        left.set_color(PINK)
        trips = [(right, DOWN, 'First transformation'), (left, UP, 'Second transformation')]
        braces = []
        for mob, direction, text in trips:
            brace = Brace(mob, direction)
            words = OldTexText(text)
...
class SymbolicTwoDToThreeDTransform(SymbolicThreeDTransform):
    CONFIG = {'input_coords': [2, 6], 'output_coords': [3, 2, 0], 'title': 'Two dimensions to three dimensions'}

class SymbolicThreeDToTwoDTransform(SymbolicThreeDTransform):
    CONFIG = {'input_coords': [4, -3, 1], 'output_coords': [8, 4], 'title': 'Three dimensions to two dimensions'}
...
class QuestionsToPonder(Scene):
    ...
    def construct(self):
        ...
        self.add(title)
        questions = VMobject(*list(map(TexText, ['1. Can you visualize these transformations?', '2. Can you represent them with matrices?', '3. How many rows and columns?', '4. When can you multiply these matrices?'])))
