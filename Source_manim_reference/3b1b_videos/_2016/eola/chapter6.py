...
class ListTerms(Scene):
    ...
    def construct(self):
        ...
        words = VMobject(*list(map(TexText, ['Inverse matrices', 'Column space', 'Rank', 'Null space'])))
...
class NoComputations(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.random_blink()
        self.teacher_says('Well...uh...no', target_mode='guilty')
        self.play(*[ApplyMethod(student.change_mode, mode) for student, mode in zip(self.get_students(), ['dejected', 'confused', 'angry'])])
        self.random_blink()
        self.wait()
        new_words = self.teacher.bubble.position_mobject_inside(OldTexText(['Search', "``Gaussian elimination'' \\\\", 'and', "``Row echelon form''"]))
        new_words.split()[1].set_color(YELLOW)
        new_words.split()[3].set_color(GREEN)
        self.play(Transform(self.teacher.bubble.content, new_words), self.teacher.change_mode, 'speaking')
...
class SystemOfEquations(Scene):
    ...
    def get_equations(self):
        ...
        for row in mob_matrix:
            equation = VMobject(*it.chain(*list(zip(row, [v.copy() for v in variables], list(map(Tex, list('++=')))))))
    ...
    def show_nonlinearity_examples(self):
        ...
        self.play(Write(squared), Write(words, run_time=1), ShowCreation(arrow))
    ...
    def describe_organization(self, equations):
        ...
        zero_circles = VMobject(*[Circle().replace(mob).scale(1.3) for mob in [VMobject(*equations.split()[i].split()[j:j + 2]) for i, j in [(1, 3), (2, 6)]]])
    ...
    def factor_into_matrix(self, equations):
        ...
        self.play(*it.chain(*[[Transform(m1.copy(), m2, run_time=2, path_arc=-np.pi / 2) for m1, m2 in zip(start_array.flatten(), matrix_mobject.get_entries().split())] for start_array, matrix_mobject in [(coefficients, matrix), (variable_arrays[0], x_array), (variable_arrays[1], x_array), (variable_arrays[2], x_array), (rhs_entries, v_array)]]))
...
class LinearSystemTransformationScene(LinearTransformationScene):
    ...
    def setup(self):
        ...
        equation = OldTex(['A', '\\vec{\\textbf{x}}', '=', '\\vec{\\textbf{v}}'])
        equation.scale(1.5)
...
class ThinkAboutWhatsHappening(Scene):
    ...
    def construct(self):
        ...
        self.play(ApplyFunction(lambda m: m.shift(2 * DOWN).scale(5), everything), bubble.content.set_color, BLACK, run_time=2)
...
class ShowBijectivity(LinearTransformationScene):
    ...
    def construct(self):
        self.setup()
        vectors = VMobject(*[Vector([x, y]) for x, y in it.product(*[np.arange(-int(val) + 0.5, int(val) + 0.5) for val in (FRAME_X_RADIUS, FRAME_Y_RADIUS)])])
        ...
        titles = [OldTexText(['Each vector lands on\\\\', 'exactly one vector']), OldTexText(['Every vector has \\\\', 'been landed on'])]
        for title in titles:
            title.to_edge(UP)
...
class LabeledExample(LinearSystemTransformationScene):
    CONFIG = {'title': '', 't_matrix': [[0, 0], [0, 0]], 'show_square': False}
...
class SquishExmapleWithWords(LabeledExample):
    CONFIG = {'title': '$A$ squishes things to a lower dimension', 't_matrix': [[-2, -1], [2, 1]]}
...
class SquishExmapleDet(SquishExmapleWithWords):
    CONFIG = {'title': '$\\det(A) = 0$', 'show_square': True}

class FullRankExmapleDet(FullRankExmapleWithWords):
    CONFIG = {'title': '$\\det(A) \\ne 0$', 'show_square': True}
...
class DescribeInverse(LinearTransformationScene):
    CONFIG = {'show_actual_inverse': False, 'matrix_label': '$A$', 'inv_label': '$A^{-1}$'}
...
class ClockwiseCounterclockwise(DescribeInverse):
    CONFIG = {'t_matrix': [[0, 1], [-1, 0]], 'show_actual_inverse': True, 'matrix_label': '$90^\\circ$ Couterclockwise', 'inv_label': '$90^\\circ$ Clockwise'}

class ShearInverseShear(DescribeInverse):
    CONFIG = {'t_matrix': [[1, 0], [1, 1]], 'show_actual_inverse': True, 'matrix_label': 'Rightward shear', 'inv_label': 'Leftward shear'}
...
class MultiplyToIdentity(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.apply_transposed_matrix(self.t_matrix)
        self.play(Transform(brace, A_inv.brace), Transform(text, A_inv.text))
        self.apply_inverse_transpose(self.t_matrix)
        self.wait()
        self.play(Transform(brace, product.brace), Transform(text, product.text))
        self.wait()
        self.play(Write(identity.background), Write(identity.get_brackets()), Write(eq), Transform(brace, identity.brace), Transform(text, identity.text))
        self.wait()
...
class SymbolicInversion(Scene):
    ...
    def construct(self):
        ...
        self.play(GrowFromCenter(product.brace), Write(product.words, run_time=1), product.set_color, BLUE)
        self.wait()
...
class PlayInReverseWithSolution(PlayInReverse):
    ...
    def setup(self):
        LinearTransformationScene.setup(self)
        equation = OldTex(['\\vec{\\textbf{x}}', '=', 'A^{-1}', '\\vec{\\textbf{v}}'])
        equation.to_edge(UP)
        equation.add_background_rectangle()
        self.add_foreground_mobject(equation)
        self.equation = equation
...
class OneInputMultipleOutputs(InvertNonInvertable):
    ...
    def construct(self):
        ...
        self.play(grid.restore, *[Transform(output_vector.copy(), input_vector) for input_vector in input_vectors.split()] + list(map(Animation, self.foreground_mobjects)), run_time=3)
...
class DefineColumnSpace(Scene):
    ...
    def construct(self):
        left_words = OldTexText('Set of all possible\\\\', 'outputs', '$A\\vec{\\textbf{v}}$')
...
class ColumnsRepresentBasisVectors(Scene):
    ...
    def construct(self):
        ...
        for i, words in enumerate([i_hat_words, j_hat_words]):
            arrow = Arrow(words.get_bottom(), matrix.get_mob_matrix()[0, i].get_top(), color=words.get_color())
            self.play(Write(words, run_time=1), ShowCreation(arrow), *[ApplyMethod(m.set_color, words.get_color()) for m in matrix.get_mob_matrix()[:, i]])
        self.wait()
...
class TowDColumnsDontSpan(LinearTransformationScene):
    ...
    def construct(self):
        ...
        brace = Brace(matrix)
        words = VMobject(OldTexText('Span', 'of columns'), OldTex('\\Updownarrow'), OldTexText("``Column space''"))
...
class ThreeDColumnsDontSpan(Scene):
    ...
    def construct(self):
        matrix = Matrix(np.array([[1, 1, 0], [0, 1, 1], [-1, -2, -1]]).T)
        matrix.set_column_colors(X_COLOR, Y_COLOR, Z_COLOR)
        brace = Brace(matrix)
...
class NameColumnSpace(Scene):
    ...
    def construct(self):
        ...
        span_text = OldTex('\\text{Span}', '\\Big(', matrix_to_tex_string([1, 2, 3]), ',', matrix_to_tex_string([1, 2, 3]), ',', matrix_to_tex_string([1, 2, 3]), '\\big)')
        for i in (1, -1):
        ...
        self.add(matrix)
        self.wait()
        self.play(*[Transform(VMobject(*matrix.copy().get_mob_matrix()[:, i]), col_arrays[i].get_entries()) for i in range(3)])
        self.play(Write(span_text), *list(map(Animation, self.get_mobjects_from_last_animation())))
        self.play(ShowCreation(arrow), Write(column_space))
        self.wait()
        self.play(FadeOut(matrix))
        self.clear()
        self.add(text)
        words = OldTexText('To solve', '$A\\vec{\\textbf{x}} = \\vec{\\textbf{v}}$,\\\\', '$\\vec{\\textbf{v}}$', 'must be in \\\\ the', 'column space.')
...
class IHatShear(LinearTransformationScene):
    CONFIG = {'foreground_plane_kwargs': {'x_radius': FRAME_WIDTH, 'y_radius': FRAME_WIDTH, 'secondary_line_ratio': 0}}
...
class NameFullRank(Scene):
    ...
    def construct(self):
        ...
        matrix.to_edge(UP)
        brace = Brace(matrix)
        top_words = brace.get_text('When', 'rank', '$=$', 'number of columns')
...
class NameNullSpace(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(vectors.restore, self.plane.restore, *list(map(Animation, self.foreground_mobjects)), run_time=2)
        self.play(Transform(vectors, line, run_time=2, lag_ratio=0.5))
        self.wait()
        for label in (null_space_label, kernel_label):
            self.play(Write(label))
            self.wait()
        self.apply_transposed_matrix(self.t_matrix, added_anims=[Transform(vectors, dot), ApplyMethod(null_space_label.scale, 0), ApplyMethod(kernel_label.scale, 0)], path_arc=0)
        self.wait()
...
class OffsetNullSpace(NameNullSpace):
    ...
    def construct(self):
        ...
        self.play(Transform(vectors, offset_vectors), *[Transform(v, VectorizedPoint(v.get_end())) for v in x_copy])
        self.remove(x_copy)
        self.wait()
...
class ShowAdditivityProperty(LinearTransformationScene):
    CONFIG = {'show_basis_vectors': False, 't_matrix': [[1, 0.5], [-1, 1]], 'include_background_plane': False}
    ...
    def construct(self):
        ...
        form = OldTex('A(', '\\vec{\\textbf{v}}', '+', '\\vec{\\textbf{w}}', ')', '=A', '\\vec{\\textbf{v}}', '+A', '\\vec{\\textbf{w}}')
...
class AddJustOneNullSpaceVector(NameNullSpace):
    ...
    def construct(self):
        ...
        form = OldTex('A(', '\\vec{\\textbf{x}}', '+', '\\vec{\\textbf{n}}', ')', '=A', '\\vec{\\textbf{x}}', '+A', '\\vec{\\textbf{n}}')
        form.to_corner(UP + RIGHT)
        ...
        self.wait()
        words = OldTexText('$\\vec{\\textbf{x}}$', 'and the', '$\\vec{\\textbf{x}} + \\vec{\\textbf{n}}$\\\\', 'land on the same spot')
