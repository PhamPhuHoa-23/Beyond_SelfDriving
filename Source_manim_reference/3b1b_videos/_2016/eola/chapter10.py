...
class OpeningQuote(Scene):
    ...
    def construct(self):
        words = OldTexText('``Last time, I asked: `What does', 'mathematics', " mean to you?', and some people answered: `The \n            manipulation of numbers, the manipulation of structures.' \n            And if I had asked what", 'music', "means to you, would you have answered: `The \n            manipulation of notes?' '' ", enforce_new_line_structure=False, alignment='')
...
class StudentsFindThisConfusing(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(Write(title), *[ApplyMethod(pi.look_at, title) for pi in self.get_pi_creatures()])
        self.play(self.get_teacher().look_at, students[-1].eyes, students[0].change_mode, 'confused', students[1].change_mode, 'tired', students[2].change_mode, 'sassy')
        self.random_blink()
        self.student_says('Why are we doing this?', index=0, run_time=2)
        question1 = students[0].bubble.content.copy()
        self.student_says('What does this actually mean?', index=2, added_anims=[question1.scale, 0.8, question1.to_edge, LEFT, question1.shift, DOWN])
        ...
        equation = OldTex('\\det\\left( %s \\right)=0' % matrix_to_tex_string([['a-\\lambda', 'b'], ['c', 'd-\\lambda']]))
...
class ManyPrerequisites(Scene):
    ...
    def construct(self):
        ...
        words = ['Linear transformations', 'Determinants', 'Linear systems', 'Change of basis']
...
class IntroduceExampleTransformation(ExampleTranformationScene):
    ...
    def construct(self):
        ...
        self.play(*[Transform(*pair) for pair in [(i_coords_copy.rect, self.matrix.rect), (i_coords_copy.get_brackets(), self.matrix.get_brackets()), (i_coords_copy.get_entries(), VGroup(*self.matrix.get_mob_matrix()[:, 0]))]])
        to_remove = self.get_mobjects_from_last_animation()
        self.play(Transform(j_coords.copy().get_entries(), VGroup(*self.matrix.get_mob_matrix()[:, 1])))
        to_remove += self.get_mobjects_from_last_animation()
        self.wait()
...
class VectorKnockedOffSpan(ExampleTranformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ShowCreation(line), Write(span_label), Animation(vector))
...
class IHatAsEigenVector(ExampleTranformationScene):
    ...
    def set_color_x_axis(self):
        ...
        for target, line in zip(targets, lines):
            self.play(ShowCreation(line), Transform(self.i_hat, target))
        self.wait()
        self.remove(*lines)
    ...
    def label_i_hat_landing_spot(self):
        ...
        self.play(Transform(matrix.rect, array.rect), Transform(matrix.get_brackets(), array.get_brackets()), Transform(VGroup(*matrix.get_mob_matrix()[:, 0]), array.get_entries()))
        self.wait()

class AllXAxisVectorsAreEigenvectors(ExampleTranformationScene):

    def construct(self):
        vectors = VGroup(*[self.add_vector(u * x * RIGHT, animate=False) for x in reversed(list(range(1, int(FRAME_X_RADIUS) + 1))) for u in [-1, 1]])
...
class SneakierEigenVector(ExampleTranformationScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(span_line), Animation(vector), Animation(array))
        self.wait()
        self.apply_transposed_matrix(self.t_matrix, added_anims=[MoveToTarget(array), Transform(VectorizedPoint(array.get_left()), two_times)], path_arc=0)
        self.wait()
...
class FullSneakyEigenspace(ExampleTranformationScene):
    ...
    def construct(self):
        self.matrix.shift(-2 * self.matrix.get_center()[0] * RIGHT)
        vectors = VGroup(*[self.add_vector(u * x * (LEFT + UP), animate=False) for x in reversed(np.arange(0.5, 5, 0.5)) for u in [-1, 1]])
...
class NameEigenvectorsAndEigenvalues(ExampleTranformationScene):
    ...
    def construct(self):
        self.remove(self.matrix)
        self.foreground_mobjects.remove(self.matrix)
        x_vectors = VGroup(*[self.add_vector(u * x * RIGHT, animate=False) for x in range(int(FRAME_X_RADIUS) + 1, 0, -1) for u in [-1, 1]])
        x_vectors.set_color_by_gradient(YELLOW, X_COLOR)
        self.remove(x_vectors)
        sneak_vectors = VGroup(*[self.add_vector(u * x * (LEFT + UP), animate=False) for x in np.arange(int(FRAME_Y_RADIUS), 0, -0.5) for u in [-1, 1]])
        ...
        VGroup(sneak_words, sneak_words.target, sneak_words.eigen_val_words).rotate(sneak_vectors[0].get_angle())
        non_eigen = Vector([1, 1], color=PINK)
        non_eigen_span = Line(-FRAME_Y_RADIUS * non_eigen.get_end(), FRAME_Y_RADIUS * non_eigen.get_end(), color=RED)
        ...
        self.wait()
        self.apply_transposed_matrix(self.t_matrix, added_anims=[FadeOut(non_eigen_words)], path_arc=0)
        self.wait(2)
...
class EigenvalueNegativeOneHalf(LinearTransformationScene):
    CONFIG = {'t_matrix': [[0.5, -1], [-1, 0.5]], 'foreground_plane_kwargs': {'x_radius': FRAME_WIDTH, 'y_radius': FRAME_WIDTH, 'secondary_line_ratio': 0}, 'include_background_plane': False}
    ...
    def construct(self):
        ...
        span = Line(-FRAME_Y_RADIUS * vector.get_end(), FRAME_Y_RADIUS * vector.get_end(), color=MAROON_B)
        self.play(Write(words))
        self.play(ShowCreation(span), Animation(vector), Animation(words))
        self.wait()
        self.apply_transposed_matrix(self.t_matrix, added_anims=[FadeOut(words)])
        self.wait()
        self.play(Rotate(span, np.pi / 12, rate_func=wiggle), Animation(vector))
        self.wait()
...
class EigenvectorToAxisOfRotation(Scene):
    ...
    def construct(self):
        words = [OldTexText('Eigenvector'), OldTexText('Axis of rotation')]
        for word in words:
            word.scale(2)
...
class ContrastMatrixUnderstandingWithEigenvalue(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        matrix = Matrix([['\\cos(\\theta)\\cos(\\phi)', '-\\sin(\\phi)', '\\cos(\\theta)\\sin(\\phi)'], ['\\sin(\\theta)\\cos(\\phi)', '\\cos(\\theta)', '\\sin(\\theta)\\sin(\\phi)'], ['-\\sin(\\phi)', '0', '\\cos(\\phi)']])
        matrix.scale(0.7)
        for mob in (axis_and_rotation, matrix):
            mob.to_corner(UP + RIGHT)
        everyone = self.get_pi_creatures()
        self.play(Write(axis_and_rotation), *it.chain(*list(zip([pi.change_mode for pi in everyone], ['hooray'] * 4, [pi.look_at for pi in everyone], [axis_and_rotation] * 4))), run_time=2)
        self.random_blink(2)
        self.play(Transform(axis_and_rotation, matrix), *it.chain(*list(zip([pi.change_mode for pi in everyone], ['confused'] * 4, [pi.look_at for pi in everyone], [matrix] * 4))))
        self.random_blink(3)
...
class WordsOnComputation(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says("I won't cover the full\\\\", 'details of computation...', target_mode='guilty')
...
class SymbolicEigenvectors(Scene):
    ...
    def contrast_multiplication_types(self):
        ...
        self.play(Blink(morty))
        self.wait()
        self.play(*list(map(FadeOut, [left_group.brace, left_group.text, right_group.brace, right_group.text, morty, bubble, solve_text])))
    ...
    def rewrite_righthand_side(self):
        ...
        self.add(scaling_by)
        self.play(Write(VGroup(matrix_multiplication, arrow, matrix.get_brackets(), q_marks), run_time=2))
        self.wait()
        ...
        for mob in (lamb, id_text, v2):
            mob.target = mob.copy()
        VGroup(l_paren, lamb.target, id_text.target, r_paren, v2.target).arrange().next_to(equals).shift(SMALL_BUFF * UP)
    ...
    def reveal_as_linear_system(self):
        ...
        self.play(Write(zero), Write(minus), *list(map(MoveToTarget, movers)), path_arc=np.pi / 3)
        self.wait()
        A.target.next_to(minus, LEFT)
        l_paren.target = l_paren.copy()
        l_paren.target.next_to(A.target, LEFT)
        self.play(MoveToTarget(A), MoveToTarget(l_paren), Transform(v1, v2, path_arc=np.pi / 3))
        self.remove(v1)
        self.wait()
        ...
        matrix = Matrix([['3-\\lambda', '1', '4'], ['1', '5-\\lambda', '9'], ['2', '6', '5-\\lambda']])
        matrix.scale(0.7)
...
class NonZeroSolutionsVisually(LinearTransformationScene):
    ...
    def construct(self):
        ...
        det_equation = OldTex('\\text{Squishification} \\Rightarrow', '\\det', '(A-', '\\lambda', 'I', ')', '=0')
...
class TweakLambda(LinearTransformationScene):
    CONFIG = {'t_matrix': [[2, 1], [2, 3]], 'include_background_plane': False, 'foreground_plane_kwargs': {'x_radius': FRAME_WIDTH, 'y_radius': FRAME_WIDTH, 'secondary_line_ratio': 1}}

    def construct(self):
        matrix = Matrix([['2-0.00', '2'], ['1', '3-0.00']])
        matrix.add_to_back(BackgroundRectangle(matrix))
        ...
        self.plane_mobjects = [self.plane, self.square]
    ...
    def range_lambda(self, start_val, end_val, run_time=3):
        ...
        transformations += [Transform(self.i_hat, Vector(matrix_transform(RIGHT), color=X_COLOR)), Transform(self.j_hat, Vector(matrix_transform(UP), color=Y_COLOR))]
        for alpha in alphas:
            self.clear()
            ...
            new_det = DecimalNumber(np.linalg.det([self.i_hat.get_end()[:2], self.j_hat.get_end()[:2]]))
...
class ShowEigenVectorAfterComputing(LinearTransformationScene):
    CONFIG = {'t_matrix': [[2, 1], [2, 3]], 'v_coords': [2, -1], 'include_background_plane': False, 'foreground_plane_kwargs': {'x_radius': FRAME_WIDTH, 'y_radius': FRAME_WIDTH, 'secondary_line_ratio': 1}}
    ...
    def construct(self):
        ...
        self.add_foreground_mobject(matrix)
        v_label = OldTex('\\vec{\\textbf{v}}', '=', '1', '\\vec{\\textbf{v}}')
...
class LineOfReasoning(Scene):
    ...
    def construct(self):
        v_tex = '\\vec{\\textbf{v}}'
        expressions = VGroup(*it.starmap(Tex, [('A', v_tex, '=', '\\lambda', v_tex), ('A', v_tex, '-', '\\lambda', 'I', v_tex, '=', '0'), ('(', 'A', '-', '\\lambda', 'I)', v_tex, '=', '0'), ('\\det(A-', '\\lambda', 'I)', '=', '0')]))
...
class RevisitExampleTransformation(ExampleTranformationScene):
    ...
    def introduce_matrix(self):
        ...
        self.play(Write(i_coords), Write(j_coords))
        self.wait()
        self.play(*[Transform(*pair) for pair in [(i_coords.rect, self.matrix.rect), (i_coords.get_brackets(), self.matrix.get_brackets()), (i_coords.get_entries(), VGroup(*self.matrix.get_mob_matrix()[:, 0]))]])
        to_remove = self.get_mobjects_from_last_animation()
        self.play(FadeOut(j_coords.get_brackets()), FadeOut(j_coords.rect), Transform(j_coords.get_entries(), VGroup(*self.matrix.get_mob_matrix()[:, 1])))
        to_remove += self.get_mobjects_from_last_animation()
        self.wait()
        self.remove(*to_remove)
    ...
    def get_lambda_to_diag_movements(self, lamb):
        ...
        self.diag_entries = [VGroup(three, minus1, lamb), VGroup(two, minus2, lamb_copy)]
        return result
    ...
    def show_determinant(self, to_fade=None):
        ...
        minus = OldTex('-')
        cdot = OldTex('\\cdot')
        VGroup(lp1, three_minus_lamb.target, rp1, lp2, two_minus_lamb.target, rp2, minus, one.target, cdot, zero.target).arrange().next_to(equals)
        ...
        self.wait()
        self.play(Write(parens), MoveToTarget(three_minus_lamb), MoveToTarget(two_minus_lamb), run_time=2)
        self.wait()
        self.play(FadeIn(new_rect), MoveToTarget(one), MoveToTarget(zero), Write(minus), Write(cdot), run_time=2)
        self.play(GrowFromCenter(brace), Write(brace_text))
        self.wait()
        self.play(*it.chain(list(map(MoveToTarget, [brace, brace_text])), list(map(FadeOut, [one, zero, minus, cdot, new_rect]))))
        self.wait()
        self.play(Write(equals_0))
        self.wait()
        self.play(Transform(brace, final_brace), Transform(brace_text, final_text))
        self.wait()
        faders = [det_text, equals, parens, three_minus_lamb, two_minus_lamb, brace, brace_text, equals_0]
        if to_fade is not None:
            faders.append(to_fade)
        self.play(*it.chain(list(map(FadeOut, faders)), [lambda_equals_two.scale, 1.3, lambda_equals_two.next_to, self.matrix, DOWN]))
    ...
    def show_diagonally_altered_transform(self):
        ...
        self.play(*it.chain([mob.restore for mob in (self.plane, self.i_hat, self.j_hat)], list(map(Animation, self.foreground_mobjects))))
        xy_array = Matrix(['x', 'y'])
        ...
        self.wait()
        vectors = VGroup(*[self.add_vector(u * x * (LEFT + UP), animate=False) for x in range(4, 0, -1) for u in [-1, 1]])
        vectors.set_color_by_gradient(MAROON_B, YELLOW)
        vectors.save_state()
        self.play(ShowCreation(vectors, lag_ratio=0.5, run_time=2), *list(map(Animation, self.foreground_mobjects)))
        self.wait()
        self.apply_transposed_matrix(self.t_matrix - 2 * np.identity(2))
        self.wait()
        self.play(*it.chain([mob.restore for mob in (self.plane, self.i_hat, self.j_hat, vectors)], list(map(FadeOut, [xy_array, equals, zero_array])), list(map(Animation, self.foreground_mobjects))))
    ...
    def show_unaltered_transform(self):
        ...
        self.play(*it.chain(list(map(MoveToTarget, movers)), list(map(FadeOut, faders)), [GrowFromCenter(brace)]))
...
class Rotate90Degrees(LinearTransformationScene):
    CONFIG = {'t_matrix': [[0, 1], [-1, 0]], 'example_vector_coords': None}
    ...
    def construct(self):
        ...
        for entries, sym in ((diag_entries, equals), (off_diag_entries, minus)):
            ...
            for entry in entries:
                entry.target = entry.copy()
            group = VGroup(lp1, entries[0].target, rp1, lp2, entries[1].target, rp2)
            group.arrange()
            group.next_to(sym)
            parens.add_background_rectangle()
            self.play(Write(parens), *[MoveToTarget(entry.copy()) for entry in entries], run_time=2)
            self.wait()
...
class ShearExample(RevisitExampleTransformation):
    CONFIG = {'t_matrix': [[1, 0], [1, 1]], 'include_background_plane': False, 'foreground_plane_kwargs': {'x_radius': FRAME_WIDTH, 'y_radius': FRAME_HEIGHT, 'secondary_line_ratio': 1}}
    ...
    def point_out_eigenvectors(self):
        vectors = VGroup(*[self.add_vector(u * x * RIGHT, animate=False) for x in range(int(FRAME_X_RADIUS) + 1, 0, -1) for u in [-1, 1]])
        vectors.set_color_by_gradient(YELLOW, X_COLOR)
...
class IntroduceEigenbasis(TeacherStudentsScene):
    ...
    def construct(self):
        words1, words2 = list(map(TexText, ["Finish with ``eigenbasis.''", "Make sure you've\n            watched the last video"]))
        ...
        self.play(self.get_teacher().change_mode, 'sassy', Write(words2), Transform(words1, new_words[0]))
        student = self.get_students()[0]
...
class BasisVectorsAreEigenvectors(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Transform(i_coords.rect, matrix.rect), Transform(i_coords.get_brackets(), matrix.get_brackets()), i_coords.get_entries().move_to, VGroup(*matrix.get_mob_matrix()[:, 0]))
        self.play(FadeOut(j_coords.rect), FadeOut(j_coords.get_brackets()), j_coords.get_entries().move_to, VGroup(*matrix.get_mob_matrix()[:, 1]))
        ...
        for entries in (diag_entries, off_diag_entries):
            self.play(entries.scale, 1.3, entries.set_color, YELLOW, run_time=2, rate_func=there_and_back)
        self.wait()
...
class DefineDiagonalMatrix(Scene):
    ...
    def construct(self):
        ...
        off_diag_entries = VGroup(*[matrix.get_mob_matrix()[i, j] for i in range(n_dims) for j in range(n_dims) if i != j])
        title = OldTexText("``Diagonal matrix''")
        title.to_edge(UP)
        self.add(matrix)
        self.wait()
        for entries in (off_diag_entries, diag_entries):
            self.play(entries.scale, 1.1, entries.set_color, YELLOW, rate_func=there_and_back)
        self.wait()
        self.play(Write(title))
        self.wait()
        ...
        self.play(diag_entries.scale, 1.1, rate_func=there_and_back)
        self.wait()
...
class RepeatedMultilpicationOfMatrices(Scene):
    CONFIG = {'matrix': [[3, 0], [0, 2]], 'diagonal': True}
    ...
    def construct(self):
        ...
        matrix.set_column_colors(X_COLOR, Y_COLOR)
        matrices = VGroup(*[matrix.copy(), OldTex('\\dots\\dots'), matrix.copy(), matrix.copy(), matrix.copy()])
        ...
        if self.diagonal:
            ...
            self.play(*it.chain(list(map(FadeOut, [brace, brace_text[1]] + list(matrices[:-1]))), list(map(MoveToTarget, [hundred, hundred_copy, last_matrix]))), run_time=2)
            self.wait()
        else:
...
class RepeatedMultilpicationOfNonDiagonalMatrices(RepeatedMultilpicationOfMatrices):
    CONFIG = {'matrix': [[3, 4], [1, 1]], 'diagonal': False}
...
class ChangeToEigenBasis(ExampleTranformationScene):
    CONFIG = {'show_basis_vectors': False, 'include_background_plane': False, 'foreground_plane_kwargs': {'x_radius': FRAME_WIDTH, 'y_radius': FRAME_HEIGHT, 'secondary_line_ratio': 0}}
    ...
    def introduce_eigenvectors(self):
        x_vectors, v_vectors = [VGroup(*[self.add_vector(u * x * vect, animate=False) for x in range(num, 0, -1) for u in [-1, 1]]) for vect, num in [(RIGHT, 7), (UP + LEFT, 4)]]
        x_vectors.set_color_by_gradient(YELLOW, X_COLOR)
        ...
        self.play(Write(words))
        self.play(FadeOut(x_vectors), FadeOut(v_vectors), Animation(x_vector), Animation(v_vector))
        self.wait()
    ...
    def write_change_of_basis_matrix(self):
        ...
        self.to_fade += copies
        self.add(*copies)
        self.play(Transform(b1.coords.rect, cob_matrix.rect), Transform(b1.coords.get_brackets(), cob_matrix.get_brackets()), MoveToTarget(b1.entries))
        to_remove = self.get_mobjects_from_last_animation()
        ...
        neg_1.next_to(inv_cob.target.get_corner(UP + RIGHT), RIGHT)
        ...
        self.play(FadeIn(self.plane), *list(map(Animation, self.foreground_mobjects + self.moving_vectors)))
