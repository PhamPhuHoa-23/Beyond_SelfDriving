...
class SideGigToFullTime(Scene):
    ...
    def income(self, morty):
        ...
        self.play(morty.change_mode, 'happy', morty.look_at, dollar_signs, dollar_signs.shift, LEFT, dollar_signs.set_color, GREEN)
        for x in range(5):
        ...
        self.play(ApplyMethod(dollar_signs.shift, (FRAME_Y_RADIUS + 1) * DOWN, lag_ratio=0.5), morty.change_mode, 'guilty', morty.look, DOWN + RIGHT)
        self.play(Blink(morty))
...
class GrowingToDoList(Scene):
    ...
    def construct(self):
        ...
        lines = VGroup(*list(map(TexText, ['That one on topology', 'Something with quaternions', 'Solving puzzles with binary counting', 'Tatoos on math', 'Laplace stuffs', 'The role of memorization in math', 'Strangeness of the axiom of choice', 'Tensors', 'Different view of $e^{\\pi i}$', 'Quadratic reciprocity', 'Fourier stuffs', '$1+2+3+\\cdots = -\\frac{1}{12}$', 'Understanding entropy'])))
        lines.scale(0.65)
        ...
        self.play(morty.change_mode, 'pleading', morty.look_at, lines, Write(VGroup(*lines[3:])))
...
class TwoTypesOfVideos(Scene):
    ...
    def construct(self):
        ...
        series_list = VGroup(*[OldTexText('Essence of %s' % s) for s in ['linear algebra', 'calculus', 'probability', 'real analysis', 'complex analysis', 'ODEs']])
        ...
        self.add(morty)
        self.play(Blink(morty))
        self.play(morty.change_mode, 'raise_right_hand', morty.look_at, stand_alone, Write(stand_alone, run_time=2))
        self.play(morty.change_mode, 'raise_left_hand', morty.look_at, series, Write(series, run_time=2))
        self.play(Blink(morty))
        self.wait()
        ...
        self.play(Blink(morty))
        self.wait()
        pairs = [(fridays, 'speaking'), (when_done, 'wave_2'), (unless, 'surprised')]
...
class ClassWatching(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for pi in self.get_students():
            pi.look_at(rect)
        self.random_blink(5)
        self.play_student_changes('raise_left_hand', 'raise_right_hand', 'sassy')
...
class GrowRonaksSierpinski(Scene):
    CONFIG = {'colors': [BLUE, YELLOW, BLUE_C, BLUE_E], 'dot_radius': 0.08, 'n_layers': 64}
    ...
    def construct(self):
        ...
        for n, sierp_layer, dot_layer in zip(it.count(1), sierp_layers, dot_layers[1:]):
            self.play(ShowCreation(sierp_layer, lag_ratio=1), Animation(last_dot_layer), run_time=run_time)
            self.play(ShowCreation(dot_layer, run_time=run_time, lag_ratio=1))
            log2 = np.log2(n)
    ...
    def get_ronaks_sierpinski(self, n_layers):
        ...
        ronaks_sierpinski.set_stroke(width=0)
...
class PythagoreanTransformation(Scene):
    ...
    def construct(self):
        tri1 = VGroup(Line(ORIGIN, 2 * RIGHT, color=BLUE), Line(2 * RIGHT, 3 * UP, color=YELLOW), Line(3 * UP, ORIGIN, color=MAROON_B))
...
class KindWordsOnEoLA(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(Write(title), ShowCreation(rect), *[ApplyMethod(pi.look_at, rect) for pi in self.get_pi_creatures()], run_time=2)
        self.random_blink()
...
class MakeALotOfPiCreaturesHappy(Scene):
    ...
    def construct(self):
        width = 7
        height = 4
        pis = VGroup(*[VGroup(*[Randolph() for x in range(7)]).arrange(RIGHT, buff=MED_LARGE_BUFF) for x in range(4)]).arrange(DOWN, buff=MED_LARGE_BUFF)
        ...
        for pi, color in zip(pis.target, colors):
            ...
            pi.set_color(color)
        self.play(MoveToTarget(pis, run_time=2, lag_ratio=0.5))
        for x in range(10):
...
class IntegrationByParts(Scene):
    ...
    def draw_curve(self, curve):
        ...
        self.play(ShowCreation(curve), UpdateFromFunc(dot, lambda d: d.move_to(curve.get_points()[-1])), MaintainPositionRelativeTo(coords, dot), run_time=5, rate_func=linear)
        self.wait()
