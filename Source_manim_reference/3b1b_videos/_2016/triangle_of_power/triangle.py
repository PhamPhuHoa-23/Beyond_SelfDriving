...
def get_inverse_rules():
    return list(map(Tex, ['x^{\\log_x(z)} = z', '\\log_x\\left(x^y \\right) = y', '\\sqrt[y]{x^y} = x', '\\left(\\sqrt[y]{z}\\right)^y = z', '\\sqrt[\\log_x(z)]{z} = x', '\\log_{\\sqrt[y]{z}}(z) = y']))

def get_top_inverse_rules():
    result = []
    pairs = [(0, 2), (0, 1), (1, 0), (1, 2), (2, 0), (2, 1)]
    for i, j in pairs:
...
class TOP(VMobject):
    CONFIG = {'triangle_height_to_number_height': 3, 'offset_multiple': 1.5, 'radius': 1.5}
    ...
    def init_points(self):
        vertices = [self.radius * rotate_vector(RIGHT, 7 * np.pi / 6 - i * 2 * np.pi / 3) for i in range(3)]
    ...
    def put_in_vertex(self, index, mobject):
        ...
        mobject.center()
        mobject.shift(interpolate(self.get_center(), self.get_vertices()[index], 0.7))
        return mobject

    def get_surrounding_circle(self, color=YELLOW):
        return Circle(radius=1.7 * self.radius, color=color).shift(self.triangle.get_center(), self.triangle.get_height() / 6 * DOWN)
...
class IntroduceNotation(Scene):
    ...
    def construct(self):
        ...
        two, three, eight = [top.put_on_vertex(i, num) for i, num in enumerate([2, 3, 8])]
...
class ShowRule(Scene):
    ...
    def construct(self, index):
        ...
        self.wait()
        self.play(Transform(circle, new_circle), ApplyMethod(new_top.values[index].set_color, circle.color))
        self.wait()
...
class SixDifferentInverses(Scene):
    ...
    def construct(self):
        ...
        vects = it.starmap(op.add, it.product([3 * UP, 0.5 * UP, 2 * DOWN], [2 * LEFT, 2 * RIGHT]))
        ...
        self.play(ApplyMethod(VMobject(*rules[::2]).to_edge, LEFT), ApplyMethod(VMobject(*rules[1::2]).to_edge, RIGHT), GrowFromCenter(general_idea))
        self.wait()
    ...
    def animate_top_rule(self, top_rule):
        ...
        cancel_parts = [VMobject(top.triangle, top.values[symbol_index]) for top in (lil_top, big_top)]
        ...
        self.play(FadeOut(VMobject(*cancel_parts)), Transform(lil_symbol, new_symbol, rate_func=rush_from))
        self.wait()
...
class SixSixSix(Scene):
    ...
    def construct(self):
        ...
        speech_bubble.clear()
        bubble.clear()
        self.play(ApplyMethod(randy.change_mode, 'angry'), Transform(bubble, speech_bubble), Transform(equation, words), FadeOut(VMobject(*sixes)))
        self.wait()
...
class AdditiveProperty(Scene):
    ...
    def construct(self):
        ...
        self.play(ApplyMethod(exp_rule.to_edge, UP), ApplyMethod(log_rule.to_edge, DOWN, 1.5))
        t_exp_rule.next_to(exp_rule, DOWN)
        t_exp_rule.set_color(GREEN)
        t_log_rule.next_to(log_rule, UP)
        t_log_rule.set_color(RED)
        self.play(FadeIn(t_exp_rule), FadeIn(t_log_rule), ApplyMethod(exp_rule.set_color, GREEN), ApplyMethod(log_rule.set_color, RED))
        self.wait()
    ...
    def write_old_style_rules(self):
        ...
        a4, x3, y3, eq2, a5, x4, p2, a6, y4 = np.array(end.split())[[3, 5, 6, 8, 12, 14, 16, 20, 22]]
        start_copy = start.copy()
        self.play(Write(start_copy))
        self.wait()
        self.play(Transform(VMobject(a1, x1, a2, y1, eq1, a3, p1, x2, a3.copy(), y2), VMobject(a4, x3, a4.copy(), y3, eq2, a5, p2, x4, a6, y4)))
        self.play(Write(end))
        self.clear()
    ...
    def get_new_style_rules(self):
        upper_mobs = [TOP('a', 'x', 'R'), Dot(), TOP('a', 'y', 'R'), OldTex('='), TOP('a', 'x+y')]
        lower_mobs = [TOP('a', None, 'xy'), OldTex('='), TOP('a', None, 'x'), OldTex('+'), TOP('a', None, 'y')]
    ...
    def set_color_appropriate_parts(self, t_exp_rule, t_log_rule):
        ...
        mob_lists = [[circle1, top_dot, circle2], [top_plus], [bottom_times], [circle3, bottom_plus, circle4]]
        for mobs in mob_lists:
            copies = VMobject(*mobs).copy()
            self.play(ApplyMethod(copies.set_color, YELLOW, run_time=0.5))
            self.play(ApplyMethod(copies.scale, 1.2, rate_func=there_and_back))
            self.wait()
            self.remove(copies)
...
class ConstantOnTop(Scene):
    ...
    def construct(self):
        ...
        x_cubed.submobjects.reverse()
        cube_root_x = OldTex('\\sqrt[3]{x}').to_edge(UP)
        self.add(top)
        self.play(ShowCreation(three))
        self.play(FadeIn(lower_left_x), Write(x_cubed), run_time=1)
        self.wait()
        self.play(*[Transform(*pair, path_arc=np.pi) for pair in [(lower_left_x, lower_right_x), (x_cubed, cube_root_x)]])
        self.wait(2)
...
class MultiplyWithConstantTop(Scene):
    ...
    def construct(self):
        ...
        self.play(Transform(old_style_exp, old_style_rad), Transform(VMobject(*old_tops), VMobject(*new_tops), path_arc=np.pi / 2))
        self.wait(3)

class RightStaysConstantQ(Scene):

    def construct(self):
        top1, top2, top3 = old_tops = [TOP(None, s, '8') for s in ('x', 'y', OldTex('x?y'))]
        q_mark = OldTex('?').scale(2)
        equation = VMobject(top1, q_mark, top2, OldTex('='), top3)
        equation.arrange(buff=0.7)
        symbols_at_top = VMobject(*[top.values[1] for top in (top1, top2, top3)])
        symbols_at_lower_right = VMobject(*[top.put_on_vertex(0, top.values[1].copy()) for top in (top1, top2, top3)])
        ...
        bubble.clear()
        self.play(Transform(triangle, equation), FadeOut(eight), ApplyPointwiseFunction(lambda p: (p + 2 * DOWN) * 15 / get_norm(p + 2 * DOWN), bubble), FadeIn(old_style_eq1), ApplyMethod(randy.shift, 3 * DOWN + 3 * LEFT), run_time=2)
        self.remove(triangle)
        self.add(equation)
        self.wait(4)
        self.play(Transform(symbols_at_top, symbols_at_lower_right, path_arc=np.pi / 2), Transform(old_style_eq1, old_style_eq2))
        self.wait(2)

class AOplusB(Scene):

    def construct(self):
        self.add(OldTex('a \\oplus b = \\dfrac{1}{\\frac{1}{a} + \\frac{1}{b}}').scale(2))
        self.wait()
...
class ConstantLowerRight(Scene):
    ...
    def construct(self):
        ...
        x, y, xy = [t.put_on_vertex(0, s) for t, s in zip(tops, ['x', 'y', 'xy'])]
        old_style_eq = OldTex('\\dfrac{1}{\\frac{1}{\\log_x(8)} + \\frac{1}{\\log_y(8)}} = \\log_{xy}(8)')
        old_style_eq.to_edge(UP).set_color(RED)
        triple_top_copy = VMobject(*[top.copy() for i in range(3)])
        self.clear()
        self.play(Transform(triple_top_copy, VMobject(*tops)), FadeIn(VMobject(x, y, xy, big_oplus, equals)))
        self.remove(triple_top_copy)
        self.add(*tops)
        self.play(Write(old_style_eq))
        self.wait(3)
        syms = VMobject(x, y, xy)
        new_syms = VMobject(*[t.put_on_vertex(1, s) for t, s in zip(tops, ['x', 'y', 'x \\oplus y'])])
        new_old_style_eq = OldTex('\\sqrt[x]{8} \\sqrt[y]{8} = \\sqrt[X]{8}')
        ...
        self.play(Transform(old_style_eq, new_old_style_eq), Transform(syms, new_syms, path_arc=np.pi / 2), Transform(big_oplus, big_times))
        self.wait(4)
