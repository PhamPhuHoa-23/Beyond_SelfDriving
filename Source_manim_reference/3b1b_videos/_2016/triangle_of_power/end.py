...
class DontLearnFromSymbols(Scene):
    ...
    def construct(self):
        ...
        equation = VMobject(TOP(2, 'x'), OldTex('\\times'), TOP(2, 'y'), OldTex('='), TOP(2, 'x+y'))
        equation.arrange()
        q_marks = OldTexText('???')
        ...
        self.play(FadeOut(top), ApplyMethod(randy.change_mode, 'sassy'), Write(bubble.content), Write(q_marks), run_time=1)
        self.wait(3)
...
class NotationReflectsMath(Scene):
    ...
    def construct(self):
        ...
        self.play(ShowCreationPerSubmobject(arrow, rate_func=lambda t: min(smooth(3 * t), 1)), Write(better_questions), run_time=3)
        self.wait(2)

class AsymmetriesInTheMath(Scene):

    def construct(self):
        assyms_of_top = VMobject(OldTexText('Asymmetries of '), TOP('a', 'b', 'c', radius=0.75).set_color(BLUE)).arrange()
...
class AddedVsOplussed(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Transform(VMobject(left_dot, plus, right_times), VMobject(right_dot, oplus, left_times), path_arc=np.pi / 2))
        self.wait()
...
class ReciprocalTop(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Transform(VMobject(start_two, x), VMobject(end_two, one_over_x)), ApplyMethod(top.rotate, np.pi, UP, path_arc=np.pi / 7))
        self.wait()
...
class NotSymbolicPatterns(Scene):
    ...
    def construct(self):
        ...
        self.play(Write(substantive_reasoning), ApplyMethod(randy.change_mode, 'pondering_looking_left'), run_time=1)
        self.wait(2)
        self.play(ApplyMethod(line.shift, 10 * DOWN), ApplyMethod(substantive_reasoning.shift, 10 * DOWN), ApplyMethod(randy.change_mode, 'sad'), run_time=1)
        self.wait(2)
...
class TriangleOfPowerIsBetter(Scene):
    ...
    def construct(self):
        ...
        top.set_color(BLUE)
        alts = VMobject(*list(map(Tex, ['x^y', '\\log_x(z)', '\\sqrt[y]{z}'])))
...
class InYourOwnNotes(Scene):
    ...
    def construct(self):
        anims = [self.get_log_anim(3 * LEFT), self.get_exp_anim(3 * RIGHT)]
        for anim in anims:
    ...
    def get_exp_anim(self, center):
        ...
        e.submobjects = []
        start = VMobject(equals, VMobject(e, loop), VMobject(pi, i), VMobject(minus, one))
        return Transform(start, top)

class Qwerty(Scene):

    def construct(self):
        qwerty = VMobject(OldTexText(list('QWERTYUIOP')), OldTexText(list('ASDFGHJKL')), OldTexText(list('ZXCVBNM')))
        qwerty.arrange(DOWN)
        dvorak = VMobject(OldTexText(list('PYFGCRL')), OldTexText(list('AOEUIDHTNS')), OldTexText(list('QJKXBMWVZ')))
        dvorak.arrange(DOWN)
...
class ShowLog(Scene):
    ...
    def construct(self):
        equation = VMobject(*[TOP(2, None, 'x'), OldTex('+'), TOP(2, None, 'y'), OldTex('='), TOP(2, None, 'xy')]).arrange()
...
class NoOneWillActuallyDoThis(Scene):
    ...
    def construct(self):
        ...
        self.add(randy)
        self.play(ApplyMethod(randy.change_mode, 'sassy'), ShowCreation(bubble))
        bubble.add_content(words)
        self.play(Write(words), run_time=2)
        self.play(Blink(randy))
        bubble.add_content(tau_v_pi)
        self.play(FadeOut(words), GrowFromCenter(tau_v_pi), ApplyMethod(randy.change_mode, 'speaking'))
        self.remove(words)
        self.play(Blink(randy))
        self.wait(2)
        self.play(FadeOut(bubble), FadeIn(morty), ShowCreation(morty_bubble), ApplyMethod(randy.change_mode, 'plain'))
