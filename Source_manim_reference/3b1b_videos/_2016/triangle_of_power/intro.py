...
class TrigAnimation(Animation):
    CONFIG = {'rate_func': None, 'run_time': 5, 'sin_color': BLUE, 'cos_color': RED, 'tan_color': GREEN}
    ...
    def __init__(self, **kwargs):
        ...
        self.trig_lines = [Line(ORIGIN, RIGHT, color=color) for color in (self.sin_color, self.cos_color, self.tan_color)]
    ...
    def interpolate_mobject(self, alpha):
        ...
        points = [circle_point[0] * RIGHT, circle_point[1] * UP + self.center, (np.sign(np.cos(theta)) * np.sqrt(np.tan(theta) ** 2 - np.sin(theta) ** 2) + np.cos(theta)) * RIGHT + self.center]
...
class Notation(Scene):
    ...
    def introduce_notation(self):
        ...
        self.trigs1 = OldTex([['\\sin', '(x)'], ['\\cos', '(x)'], ['\\tan', '(x)']], next_to_direction=DOWN)
    ...
    def shift_to_good_and_back(self):
        ...
        new_sin, new_cos, new_tan = [VMobject().set_points_as_corners(corners).replace(trig_part.split()[0]) for corners, trig_part in zip([[RIGHT, RIGHT + UP, LEFT], [RIGHT + UP, LEFT, RIGHT], [RIGHT + UP, RIGHT, LEFT]], self.trigs1.split())]
        x1, x2, x3 = [trig_part.split()[1] for trig_part in self.trigs1.split()]
        trigs2 = VMobject(VMobject(new_sin, x1), VMobject(new_cos, x2), VMobject(new_tan, x3))
    ...
    def shift_to_visuals(self):
        ...
        self.play(Transform(trig, new_trig), *it.starmap(ApplyMethod, [(sigma.to_corner, UP + LEFT), (prod.shift, 15 * LEFT), (func.shift, 5 * UP)]))
    ...
    def get_harmonic_sum_lines(self):
        result = VMobject()
        for n in range(1, 8):
            big_line = NumberLine(x_min=0, x_max=1.01, tick_frequency=1.0 / n, big_tick_numbers=[], color=WHITE)
            little_line = Line(big_line.number_to_point(0), big_line.number_to_point(1.0 / n), color=RED)
...
class TwoThreeEightExample(Scene):
    ...
    def construct(self):
        ...
        self.play(Transform(VMobject(two1, dot1, two2, dot2, brace, three), exp_three, path_arc=-np.pi / 2), Transform(two3, base_two))
        self.clear()
        self.add(base_two, exp_three, eq, eight)
        self.wait(3)
        rad_three, rad1, rad2, rad_eight, rad_eq, rad_two = OldTex('\\sqrt[3]{8} = 2').split()
        self.play(*[Transform(*pair, path_arc=np.pi / 2) for pair in [(exp_three, rad_three), (VMobject(), rad1), (VMobject(), rad2), (eight, rad_eight), (eq, rad_eq), (base_two, rad_two)]])
        self.wait()
        self.play(ApplyMethod(VMobject(rad1, rad2).set_color, RED, rate_func=there_and_back, run_time=2))
        self.remove(rad1, rad2)
        self.wait()
        l, o, g, log_two, p1, log_eight, p2, log_eq, log_three = OldTex('\\log_2(8) = 3').split()
        self.clear()
        self.play(*[Transform(*pair, path_arc=np.pi / 2) for pair in [(rad1, l), (rad2, o), (rad2.copy(), g), (rad_two, log_two), (VMobject(), p1), (rad_eight, log_eight), (VMobject(), p2), (rad_eq, log_eq), (rad_three, log_three)]])
        self.wait()
...
class WhatTheHell(Scene):
    ...
    def construct(self):
        ...
        exp, rad, log = list(map(Tex, ['2^3 = 8', '\\sqrt[3]{8} = 2', '\\log_2(8) = 3']))
        ...
        phrases = [OldTexText(['Communicate with', words]).next_to(mob, LEFT, buff=1) for words, mob in [('position', exp), ('a new symbol', rad), ('a word', log)]]
...
class PascalsCollision(Scene):
    ...
    def construct(self):
        ...
        self.play(Write(words))
        self.wait()
        self.play(Transform(pascals_triangle, final_triangle), Transform(n_choose_k, final_triangle), FadeOut(formula), ApplyMethod(to_remove.shift, 5 * DOWN))
        self.wait()
...
class LogarithmProperties(Scene):
    ...
    def construct(self):
        ...
        props = [OldTex('\\log_a(x) = \\dfrac{\\log_b(a)}{\\log_b(x)}'), OldTex('\\log_a(x) = \\dfrac{\\log_b(x)}{\\log_b(a)}'), OldTex('\\log_a(x) = \\log_b(x) - \\log_b(a)'), OldTex('\\log_a(x) = \\log_b(x) + \\log_b(a)'), OldTex('\\log_a(x) = \\dfrac{\\log_b(x)}{\\log_b(a)}')]
        bubble.add_content(props[0])
        words = OldTexText('What was it again?')
        words.set_color(YELLOW)
        words.scale(0.5)
        words.next_to(props[0], UP)
        self.play(ApplyMethod(randy.change_mode, 'confused'), ShowCreation(bubble), Write(words))
        self.show_frame()
...
class HaveToShare(Scene):
    ...
    def construct(self):
        ...
        left_dot, top_dot, bottom_dot = [Dot(point, radius=0.1) for point in (ORIGIN, RIGHT + 0.5 * UP, RIGHT + 0.5 * DOWN)]
