...
from hilbert.curves import TransformOverIncreasingOrders, FlowSnake, HilbertCurve, SnakeCurve, Sierpinski
...
class InfiniteResultsFiniteWorld(Scene):
    ...
    def construct(self):
        ...
        right_overwords = OldTexText('\\substack{                \\text{How computers} \\\\                 \\text{represent $-1$}            }').scale(1.5)
        ...
        self.wait()
        self.finite_analog(Mobject(left_formula, left_words), arrow, Mobject(right_formula, right_words))
    ...
    def finite_analog(self, left_mob, arrow, right_mob):
        ...
        left_arrow, right_arrow = [Arrow(mob1.get_right()[0] * RIGHT, mob2.get_left()[0] * RIGHT, buff=0) for mob1, mob2 in [(new_left, middle), (middle, new_right)]]
        for mob in (ex, middle):
            mob.sort_points(get_norm)
        self.play(GrowFromCenter(ex))
        self.wait()
        self.play(Transform(left_mob, new_left), Transform(arrow.copy(), left_arrow), DelayByOrder(Transform(ex, middle)), Transform(arrow, right_arrow), Transform(right_mob, new_right))
    ...
    def equivalence(self, left_mob, arrow, right_mob):
        ...
        for mob in (arrow, words):
            mob.sort_points(get_norm)
        self.play(ApplyMethod(left_mob.shift, RIGHT), Transform(arrow, words), ApplyMethod(right_mob.to_edge, RIGHT))
        self.wait()
...
class InfiniteObjectsEncapsulateFiniteObjects(Scene):
    ...
    def get_triangles(self):
        ...
        triangles = Mobject(triangle.copy().scale(0.5).shift(LEFT), triangle, triangle.copy().scale(0.3).shift(0.5 * UP + RIGHT))
        triangles.center()
        return triangles

    def construct(self):
        words = [OldTexText(text, size='\\large') for text in ['Truths about infinite objects', ' encapsulate ', 'facts about finite objects']]
        words[0].set_color(RED)
        words[1].next_to(words[0])
        words[2].set_color(GREEN).next_to(words[1])
        Mobject(*words).center().to_edge(UP)
        infinite_objects = [OldTex('\\sum_{n=0}^\\infty', size='\\normal').set_color(RED_E), Sierpinski(order=8).scale(0.3), OldTexText('$\\exists$ something infinite $\\dots$').set_color(RED_B)]
        finite_objects = [OldTex('\\sum_{n=0}^N', size='\\normal').set_color(GREEN_E), self.get_triangles(), OldTexText('$\\forall$ finite somethings $\\dots$').set_color(GREEN_B)]
