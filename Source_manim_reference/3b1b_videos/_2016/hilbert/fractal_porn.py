...
class Intro(TransformOverIncreasingOrders):
    ...
    def construct(self):
        words1 = OldTexText("If you watched my video about Hilbert's space-filling curve\\dots")
        words2 = OldTexText('\\dots you might be curious to see what a few other space-filling curves look like.')
        words2.scale(0.8)
...
class IntroduceKoch(Intro):
    ...
    def construct(self):
        words = list(map(TexText, ['This is another famous fractal.', "The ``Koch Snowflake''", "Let's finish things off by seeing how to turn             this into a space-filling curve"]))
        for text in words:
            text.to_edge(UP)
...
class SharperKoch(StraightKoch):
    CONFIG = {'angle': 0.9 * np.pi / 2}

class DullerKoch(StraightKoch):
    CONFIG = {'angle': np.pi / 6}

class SpaceFillingKoch(StraightKoch):
    CONFIG = {'angle': np.pi / 2}
...
class FromKochToSpaceFilling(Scene):
    ...
    def revisit_koch(self):
        words = list(map(TexText, ['First, look at how one section of this curve is made.', "This pattern of four lines is the ``seed''", 'With each iteration, every straight line is             replaced with an appropriately small copy of the seed']))
        for text in words:
            text.to_edge(UP)
        self.add(words[0])
    ...
    def show_angles(self):
        ...
        words.to_edge(UP)
        koch, sharper_koch, duller_koch = curves = [CurveClass(order=1) for CurveClass in (StraightKoch, SharperKoch, DullerKoch)]
        arcs = [Arc(2 * (np.pi / 2 - curve.angle), radius=r, start_angle=np.pi + curve.angle).shift(curve.get_points()[curve.get_num_points() / 2]) for curve, r in zip(curves, [0.6, 0.7, 0.4])]
        theta = OldTex('\\theta')
        ...
        self.play(Transform(koch, duller_koch), Transform(arcs[0], arcs[2]))
        self.play(Transform(koch, sharper_koch), Transform(arcs[0], arcs[1]))
        self.clear()
    ...
    def show_change_side_by_side(self):
        ...
        words = list(map(TexText, ['A sharper angle results in a richer curve', 'A more obtuse angle gives a sparser curve', 'And as the angle approaches 0\\dots', 'We have a new space-filling curve.']))
        for text in words:
            text.to_edge(UP)
        sharper, duller, space_filling = [CurveClass(order=1).shift(3 * LEFT) for CurveClass in (SharperKoch, DullerKoch, SpaceFillingKoch)]
        shaper_f, duller_f, space_filling_f = [CurveClass(order=self.max_order).shift(3 * RIGHT) for CurveClass in (SharperKoch, DullerKoch, SpaceFillingKoch)]
        self.add(words[0])
        left_curve = SharperKoch(order=1)
        right_curve = SharperKoch(order=1)
        self.play(Transform(left_curve, sharper), ApplyMethod(right_curve.shift, 3 * RIGHT))
        self.play(Transform(right_curve, SharperKoch(order=2).shift(3 * RIGHT)), ShimmerIn(seed), ShimmerIn(fractal))
        for order in range(3, self.max_order):
            self.play(Transform(right_curve, SharperKoch(order=order).shift(3 * RIGHT)))
        self.remove(words[0])
        self.add(words[1])
        kwargs = {'run_time': 4}
        self.play(Transform(left_curve, duller, **kwargs), Transform(right_curve, duller_f, **kwargs))
        self.wait()
        kwargs['run_time'] = 7
        kwargs['rate_func'] = None
        self.remove(words[1])
        self.add(words[2])
        self.play(Transform(left_curve, space_filling, **kwargs), Transform(right_curve, space_filling_f, **kwargs))
        self.remove(words[2])
        self.add(words[3])
        self.wait()