...
from hilbert.curves import TransformOverIncreasingOrders, FlowSnake, HilbertCurve, SnakeCurve, PeanoCurve
...
def get_time_line():
    ...
    year_range = 400
    time_line = NumberLine(numerical_radius=year_range / 2, unit_length_to_spatial_width=length / year_range, tick_frequency=10, leftmost_tick=1720, number_at_center=1870, big_tick_numbers=list(range(1700, 2100, 100)))
    time_line.sort_points(lambda p: p[0])
    time_line.set_color_by_gradient(PeanoCurve.CONFIG['start_color'], PeanoCurve.CONFIG['end_color'])
...
class AskMathematicianFriend(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ApplyMethod(mathy.shift, 3 * (DOWN + LEFT)), ApplyPointwiseFunction(lambda p: 15 * p / get_norm(p), bubble), run_time=3)
...
class TimeLineAboutSpaceFilling(Scene):
    ...
    def construct(self):
        ...
        self.add(time_line)
        self.play(ApplyMethod(time_line.shift, -time_line.number_to_point(1900), run_time=3))
        brace = Brace(Mobject(Point(time_line.number_to_point(1865)), Point(time_line.number_to_point(1888))), UP)
...
class NotPixelatedSpace(Scene):
    ...
    def construct(self):
        ...
        arrows = Mobject(*[Arrow(2 * p, 4 * p) for theta in np.arange(np.pi / 6, 2 * np.pi, np.pi / 3) for p in [rotate_vector(RIGHT, theta)]])
        self.add(grid)
        self.wait()
...
class HistoryOfDiscover(Scene):
    ...
    def construct(self):
        ...
        squares = Mobject(*[Square(side_length=3, color=WHITE).replace(curve) for curve in (hilbert_curve, peano_curve)])
        self.add(time_line)
        self.wait()
        for year, curve, vect, text in [(1890, peano_curve, UP, 'Peano Curve'), (1891, hilbert_curve, DOWN, 'Hilbert Curve')]:
...
class DefinitionOfCurve(Scene):
    ...
    def construct(self):
        start_words = OldTexText(['``', 'Space Filling', "Curve ''"]).to_edge(TOP, buff=0.25)
        ...
        end_words = Mobject(*[quote, space_filling, curve_quote]).center().to_edge(TOP, buff=0.25)
        ...
        dots = Mobject(*[Dot(curve.get_points()[n * curve.get_num_points() / 15], color=YELLOW_C) for n in range(1, 15) if n not in [4, 11]])
...
class DefiningCurves(Scene):
    ...
    def construct(self):
        words = OldTexText(['One does not simply define the limit \\\\             of a sequence of', 'curves', '\\dots'])
        ...
        self.wait()
        self.play(FadeOut(words), ApplyMethod(curves1.replace, curves2), ShimmerIn(top_words.split()[1]))
        self.wait()
...
class PseudoHilbertCurveAsFunctionExample(Scene):
    ...
    def construct(self, order):
        ...
        phc, arg, result = OldTex(['\\text{PHC}_%d' % order, '(0.3)', '= %s' % result_tex]).to_edge(UP).split()
        ...
        self.wait()
        self.remove(arrow1)
        self.play(FadeIn(grid), Transform(line, curve), Transform(dot1, dot2), run_time=2)
        self.wait()
...
class ContinuityRequired(Scene):
    ...
    def construct(self):
        words = OldTexText(['A function must be', '\\emph{continuous}', 'if it is to represent a curve.'])
...
class FormalDefinitionOfContinuity(Scene):
    ...
    def setup(self):
        ...
        self.spiril2 = ParametricCurve(lambda t: 5.5 * RIGHT + UP - 2 * spiril(1 - t), density=5 * DEFAULT_POINT_DENSITY_1D)
    ...
    def get_circles_and_points(self, min_input, max_input):
        input_left, input_right = [self.interval.number_to_point(num) for num in (min_input, max_input)]
        input_circle = Circle(radius=get_norm(input_left - input_right) / 2, color=WHITE)
        ...
        output_points.add_points(self.output.get_points()[int(min_input * n):int(max_input * n)])
    ...
    def label_spaces(self):
        ...
        self.play(ShimmerIn(input_space), ShimmerIn(output_space), ShowCreation(line), ShowCreation(self.interval))
        self.wait()

    def move_dot(self):
        kwargs = {'rate_func': None, 'run_time': 3}
        self.play(Homotopy(self.input_homotopy, self.input_dot, **kwargs), Homotopy(self.output_homotopy, self.output_dot, **kwargs), ShowCreation(self.output, **kwargs))
        self.wait()

    def label_jump(self):
        jump_points = Mobject(Point(self.spiril1.get_points()[-1]), Point(self.spiril2.get_points()[0]))
    ...
    def draw_circles(self):
        ...
        self.play(Homotopy(self.input_homotopy, self.input_dot, **kwargs), Homotopy(self.output_homotopy, self.output_dot, **kwargs))
        ...
        tup = self.get_circles_and_points(input_value - input_radius, input_value + input_radius)
    ...
    def vary_circle_sizes(self):
        input_value = 0.45
        radius = 0.04
        vary_circles = VaryCircles(self, input_value, radius, run_time=5)
    ...
    def discontinuous_point(self):
        ...
        input_value = 0.5
        input_radius = 0.04
        vary_circles = VaryCircles(self, input_value, input_radius, run_time=5)
        for dot in (self.input_dot, self.output_dot):
            dot.center()
        ...
        arrow = Arrow(point_description, self.output_dot, buff=0.05, color=self.output_color)
...
class VaryCircles(Animation):
    ...
    def interpolate_mobject(self, alpha):
        ...
        self.mobject = Mobject(*self.scene.get_circles_and_points(self.input_value - radius, self.input_value + radius)).ingest_submobjects()
...
class WonderfulPropertyOfPseudoHilbertCurves(Scene):
    ...
    def construct(self):
        val = 0.3
        text = OldTexText(['PHC', '$_n', '(', '%3.1f' % val, ')$', ' has a ', 'limit point ', 'as $n \\to \\infty$'])
        ...
        for num in range(2, 9):
            ...
            self.play(Transform(curve, new_curve), Transform(dot, new_dot), Transform(num_str, new_num_str))
            self.wait()
        text.to_edge(UP)
        text_parts = text.split()
        for index in (1, -1):
            text_parts[index].set_color()
        starters = Mobject(*func_parts + [Point(mob.get_center(), stroke_width=1) for mob in text_parts[5:]])
...
class FollowManyPoints(Scene):
    ...
    def construct(self):
        text = OldTexText(['PHC', '_n', '(', 'x', ')$', ' has a limit point ', 'as $n \\to \\infty$', '\\\\ for all $x$'])
        parts = text.split()
        ...
        for num in range(1, 10):
            new_curve = HilbertCurve(order=num)
            new_curve.scale(0.8)
            new_dots = Mobject(*[Dot(new_curve.get_points()[int(val * new_curve.get_num_points())]) for val in vals])
            self.play(Transform(curve, new_curve), Transform(dots, new_dots))

class FormalDefinitionOfHilbertCurve(Scene):

    def construct(self):
        val = 0.7
        text = OldTex(['\\text{HC}(', 'x', ')', '=\\lim_{n \\to \\infty}\\text{PHC}_n(', 'x', ')'])
        text.to_edge(UP)
        x1 = text.split()[1]
        x2 = text.split()[-2]
        x2.set_color(BLUE)
        ...
        self.play(ShowCreation(curve))
        self.play(ApplyMethod(x1.set_color, BLUE), ShowCreation(x_arrow), ShowCreation(dot))
        self.wait()
        self.remove(x_arrow)
...
class ThreeThingsToProve(Scene):
    ...
    def construct(self):
        definition = OldTex(['\\text{HC}(', 'x', ')', '=\\lim_{n \\to \\infty}\\text{PHC}_n(', 'x', ')'])
        ...
        items = OldTexText(['\\begin{enumerate}', '\\item Well-defined: ', 'Points on Pseudo-Hilbert-curves really do converge', '\\item A Curve: ', 'HC is continuous', '\\item Space-filling: ', 'Each point in the unit square is an output of HC', '\\end{enumerate}']).split()
...
class TilingSpace(Scene):
    ...
    def construct(self):
        ...
        squares = Mobject(*[square.copy().shift(coords) for coords in coords_set]).ingest_submobjects()
        self.play(DelayByOrder(FadeIn(squares)), run_time=3)
        curve = HilbertCurve(order=6).scale(1.0 / 6)
        all_curves = Mobject(*[curve.copy().shift(coords) for coords in coords_set]).ingest_submobjects()
...
class ColorIntervals(Scene):
    ...
    def construct(self):
        number_line = NumberLine(numerical_radius=5, number_at_center=5, leftmost_tick=0, density=2 * DEFAULT_POINT_DENSITY_1D)
        ...
        for n in range(0, 10, 2):
            ...
            self.play(ApplyMethod(number_line.set_color, RED, lambda p: p[0] > n - 6.2 and p[0] < n - 4 and (p[1] > -0.4)), brace_anim)