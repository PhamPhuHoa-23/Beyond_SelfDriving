...
from hilbert.curves import TransformOverIncreasingOrders, FlowSnake, HilbertCurve, SnakeCurve
...
def get_freq_line():
    return UnitInterval().shift(2 * DOWN)
...
class AboutSpaceFillingCurves(TransformOverIncreasingOrders):
    ...
    def show_infinite_objects(self):
        sigma, summand, equals, result = OldTex(['\\sum_{n = 1}^{\\infty}', '\\dfrac{1}{n^2}', '=', '\\dfrac{\\pi^2}{6}']).split()
        ...
        local_mobjects = list(filter(lambda m: isinstance(m, Mobject), list(locals().values())))
        for mob in local_mobjects:
            mob.sort_points(get_norm)
        self.play(ShimmerIn(infinity))
        self.wait()
        self.play(ShimmerIn(summand), ShimmerIn(equals), ShimmerIn(result), DelayByOrder(Transform(infinity, sigma)))
        self.wait()
        self.play(Transform(summand, alt_summand), Transform(result, alt_result))
        self.wait()
        self.remove(infinity)
        self.play(*[CounterclockwiseTransform(Mobject(summand, equals, result, sigma), Mobject(rationals, other_equals, naturals))])
        self.wait()
        self.clear()
    ...
    def pose_question(self):
        ...
        self.wait()
        self.play(*[ShimmerIn(mob) for mob in (infinity, rightarrow, N)] + [ApplyMethod(question_mark.next_to, rightarrow, UP)])
        self.wait()
...
class PostponePhilosophizing(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(*[ApplyMethod(word1.replace, word2, path_func=path_along_arc(np.pi / 2)) for word1, word2 in it.permutations([abstract, concrete])])
        self.wait()
...
class ImageDataIsTwoDimensional(Scene):
    ...
    def construct(self):
        ...
        for vect, num in zip([DOWN, RIGHT], [1, 2]):
            ...
            self.play(Transform(Point(brace.get_center()), brace), ShimmerIn(words_mob), run_time=2)
        self.wait()
...
class SoundDataIsOneDimensional(Scene):
    ...
    def construct(self):
        ...
        component_strings = [Vibrate(num_periods=k + 1, overtones=1, color=color, center=2 * DOWN + UP * k) for k, color in zip(list(range(overtones)), Color(BLUE_E).range_to(WHITE, overtones))]
        ...
        self.clear()
        self.play(ShowCreation(freq_line), GrowFromCenter(brace), ShimmerIn(words), *[Transform(dot, dot.copy().scale(2).rotate(-np.pi / 2).shift(floor), path_func=path_along_arc(np.pi / 3)) for dot in dots])
        self.wait(0.5)
...
class GridOfPixels(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(GrowFromCenter(top_brace), GrowFromCenter(side_brace), ShimmerIn(top_words), ShimmerIn(side_words))
        self.wait()
...
class AssociatePixelWithFrequency(Scene):
    ...
    def construct(self):
        ...
        small_grid.center()
        pixel = MobjectFromRegion(region_from_polygon_vertices(*0.2 * np.array([RIGHT + DOWN, RIGHT + UP, LEFT + UP, LEFT + DOWN])))
        ...
        vibration_config = {'overtones': 1, 'spatial_period': 2}
        vibration, loud_vibration, quiet_vibration = [Vibrate(string.copy(), amplitude=a, **vibration_config) for a in [0.5, 1.0, 0.25]]
        self.add(small_grid)
        self.wait()
        ...
        self.play(TransformAnimations(loud_vibration, quiet_vibration), ApplyMethod(dot.fade, 0.9))
        self.clear()
...
class ListenToAllPixels(Scene):
    ...
    def construct(self):
        ...
        sub_vibrations = [Vibrate(string.copy().shift((n - 1) * UP).set_color(colors[n]), overtones=1, spatial_period=6.0 / (n + 1), temporal_period=1.0 / (n + 1), amplitude=0.5 / (n + 1)) for n in range(4)]
        ...
        self.wait()
        self.play(ShimmerIn(words, rate_func=squish_rate_func(smooth, 0, 0.2)), *sub_vibrations, run_time=5)
...
class RandomMapping(Scene):
    ...
    def construct(self):
        ...
        for target, rate_func in zip(targets, alphas):
            self.play(Transform(grid, target, run_time=3, rate_func=rate_func, path_func=path_along_arc(-np.pi / 2)))
        self.wait()
...
class WeaveLineThroughPixels(Scene):
    ...
    def construct(self, order):
        ...
        squares = Mobject(*[Square(side_length=unit, color=WHITE).shift(x * right + y * up) for x, y in it.product(list(range(2 ** order)), list(range(2 ** order)))])
        squares.center()
        targets = Mobject()
...
class TellMathematicianFriend(Scene):
    ...
    def construct(self):
        ...
        bubble.write('Well, no, it \\emph{is} just one thing, but I need \\\\                       to tell you about a certain infinite family first.')
        ...
        self.play(Transform(mathy.mouth, squiggle_mouth), ApplyMethod(mathy.arm.wag, 0.2 * RIGHT, LEFT))
        self.remove(words1)
        self.add(words2)
        self.wait(2)
        self.remove(words2)
        self.add(words3)
        self.wait(2)
        self.play(ApplyPointwiseFunction(lambda p: 15 * p / get_norm(p), bubble), ApplyMethod(mathy.shift, 5 * (DOWN + LEFT)), FadeOut(words3), run_time=3)
...
class Order2PseudoHilbertCurve(Scene):
    ...
    def construct(self):
        ...
        mini_curves = [HilbertCurve(order=1).scale(0.5).shift(1.5 * vect) for vect in [LEFT + DOWN, LEFT + UP, RIGHT + UP, RIGHT + DOWN]]
...
class Order3PseudoHilbertCurve(Scene):
    ...
    def construct(self):
        ...
        mini_curves = [HilbertCurve(order=2).scale(0.5).shift(1.5 * vect) for vect in [LEFT + DOWN, LEFT + UP, RIGHT + UP, RIGHT + DOWN]]
        ...
        self.play(*[ApplyMethod(curve.rotate, np.pi, axis) for curve, axis in [(mini_curves[0], UP + RIGHT), (mini_curves[3], UP + LEFT)]])
...
class GrowToOrder8PseudoHilbertCurve(Scene):
    ...
    def increase_order(self):
        mini_curves = [self.curve.copy().scale(0.5).shift(1.5 * vect) for vect in [LEFT + DOWN, LEFT + UP, RIGHT + UP, RIGHT + DOWN]]
...
class HilbertBetterThanSnakeQ(Scene):
    ...
    def construct(self):
        hilbert_curves, snake_curves = [[CurveClass(order=n) for n in range(2, 7)] for CurveClass in (HilbertCurve, SnakeCurve)]
...
class IncreaseResolution(Scene):
    ...
    def construct(self):
        ...
        self.add(grid)
        self.play(GrowFromCenter(side_brace), GrowFromCenter(top_brace), ShimmerIn(top_words), ShimmerIn(side_words))
        self.wait()
        self.play(DelayByOrder(Transform(*grids)), Transform(top_words, new_top_words), Transform(side_words, new_side_words))
        self.wait()
...
class IncreasingResolutionWithSnakeCurve(Scene):
    ...
    def construct(self):
        ...
        start_dots, end_dots = [Mobject(*[Dot(curve.get_points()[int(x * curve.get_num_points())], color=color) for x, color in [(0.202, GREEN), (0.48, BLUE), (0.7, RED)]]) for curve in (start_curve, end_curve)]
