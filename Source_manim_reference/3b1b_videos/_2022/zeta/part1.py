...
class ShowPrimeDensity(InteractiveScene):
    ...
    def construct(self):
        ...
        for n in range(x_max):
            if sympy.isprime(n):
                t = smooth()
                all_prime_animations.append(self.get_prime_animation(numberline, n, run_time=11, time_span=(t, t + 1)))
        frame.generate_target()
        frame.target.move_to(numberline.n2p(100))
        frame.target.scale(2)
        self.play(LaggedStart(*all_prime_animations, lag_ratio=0.25), MoveToTarget(frame, rate_func=smooth, time_span=(2, 10)), run_time=10)
    ...
    def get_prime_animation(self, numberline, prime, **kwargs):
        ...
        return AnimationGroup(ShowCreation(arrow), MoveToTarget(dot, rate_func=rush_into), numberline.numbers[prime].animate.set_color(PRIME_COLOR), **kwargs)
