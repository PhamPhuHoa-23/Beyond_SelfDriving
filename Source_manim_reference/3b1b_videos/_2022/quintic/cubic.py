...
class CubicFormula(RootCoefScene):
    coefs = [1, -1, 0, 1]
    coef_plane_config = {'x_range': (-2.0, 2.0), 'y_range': (-2.0, 2.0), 'background_line_style': {'stroke_color': GREY, 'stroke_width': 1.0}}
    root_plane_config = {'x_range': (-2.0, 2.0), 'y_range': (-2.0, 2.0), 'background_line_style': {'stroke_color': BLUE_E, 'stroke_width': 1.0}}
    sqrt_plane_config = {'x_range': (-1.0, 1.0), 'y_range': (-1.0, 1.0), 'background_line_style': {'stroke_color': GREY_B, 'stroke_width': 1.0}, 'height': 3.0, 'width': 3.0}
    crt_plane_config = {'x_range': (-2.0, 2.0), 'y_range': (-2.0, 2.0), 'background_line_style': {'stroke_color': BLUE_E, 'stroke_width': 1.0}, 'height': 3.0, 'width': 3.0}
    cf_plane_config = {'x_range': (-2.0, 2.0), 'y_range': (-2.0, 2.0), 'background_line_style': {'stroke_color': BLUE_E, 'stroke_width': 1.0}, 'height': 3.0, 'width': 3.0}
    plane_height = 3.0
    plane_buff = 1.0
    ...
    def add_lower_planes(self):
        ...
        kw = dict(font_size=24, tex_to_color_map={'\\delta_1': GREEN, '\\delta_2': GREEN}, background_stroke_width=3, background_stroke_color=3)
        ...
        crt_labels = VGroup(OldTex('\\cdot', '= \\sqrt[3]{-\\frac{q}{2} + \\delta_1}', **kw), OldTex('\\cdot', '= \\sqrt[3]{-\\frac{q}{2} + \\delta_2}', **kw))
        ...
        cf_label = OldTex('\\sqrt[3]{ -\\frac{q}{2} + \\delta_1 } +', '\\sqrt[3]{ -\\frac{q}{2} + \\delta_2 }', font_size=24)
    ...
    def get_coef_poly(self):
        return OldTex('x^3 + {0}x^2 + {p}x + {q}', tex_to_color_map={'{0}': self.coef_color, '{p}': self.coef_color, '{q}': self.coef_color})
    ...
    def add_cf_lines(self):
        cf_lines = VGroup(Line(stroke_color=self.crt_dot_colors[0]).replicate(9), Line(stroke_color=self.crt_dot_colors[1]).replicate(3))
        cf_lines.set_stroke(width=1)

        def update_cf_lines(cf_lines):
            cube_root_values = self.get_cube_root_values()
            for z1, line in zip(cube_root_values[1], cf_lines[1]):
                line.put_start_and_end_on(self.cf_plane.n2p(0), self.cf_plane.n2p(z1))
            for line, (z1, z2) in zip(cf_lines[0], it.product(*cube_root_values)):
                line.put_start_and_end_on(self.cf_plane.n2p(z2), self.cf_plane.n2p(z1 + z2))
