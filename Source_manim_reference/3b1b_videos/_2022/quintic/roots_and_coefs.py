...
def roots_to_coefficients(roots):
    n = len(list(roots))
    return [(-1) ** (n - k) * sum((np.prod(tup) for tup in it.combinations(roots, n - k))) for k in range(n)] + [1]
...
def find_root(func, dfunc, seed=complex(1, 1), tol=1e-08, max_steps=100):
    last_seed = np.inf
...
def coefficients_to_roots(coefs):
    ...
    for i in range(len(coefs) - 1):
        root = find_root(lambda x: poly(x, coefs), lambda x: dpoly(x, coefs))
        roots.append(root)
...
def get_nth_roots(z, n):
    base_root = z ** (1 / n)
    return [base_root * np.exp(complex(0, k * TAU / n)) for k in range(n)]
...
def factored_poly_tex(roots, prefix='P(x) = ', root_colors=[YELLOW, YELLOW]):
    ...
    t2c = dict(((rt, root_color) for rt, root_color in zip(root_texs, root_colors)))
...
def expanded_poly_tex(roots, vertical=True, root_colors=[YELLOW, YELLOW], abbreviate=False):
    ...
    root_colors = color_gradient(root_colors, len(roots))
    n = len(roots)
    kw = dict(tex_to_color_map=dict(((str(r), root_color) for r, root_color in zip(roots, root_colors))), arg_separator=' ')
    result = VGroup()
    result.add(OldTex(f'x^{{{n}}}'))
    for k in range(1, n + 1):
        sym_poly = sym_poly_tex_args(roots, k, abbreviate=abbreviate)
        line = OldTex('+' if k % 2 == 0 else '-', '\\big(', *sym_poly, '\\big)', x_power_tex(n - k), **kw)
        result.add(line)
    for line in result:
...
def get_symmetric_system(lhss, roots=None, root_colors=[YELLOW, YELLOW], lhs_color=RED_B, abbreviate=False, signed=False):
    lhss = [f'{{{lhs}}}' for lhs in lhss]
    if roots is None:
        roots = [f'r_{{{i}}}' for i in range(len(lhss))]
    root_colors = color_gradient(root_colors, len(roots))
    t2c = dict([(root, root_color) for root, root_color in zip(roots, root_colors)])
    t2c.update(dict([(str(lhs), lhs_color) for lhs in lhss]))
    kw = dict(tex_to_color_map=t2c)
    equations = VGroup(*(OldTex(lhs, '=', '-(' if neg else '', *sym_poly_tex_args(roots, k, abbreviate=abbreviate), ')' if neg else '', **kw) for k, lhs in zip(it.count(1), lhss) for neg in [signed and k % 2 == 1]))
...
def get_quadratic_formula(lhs='', **tex_config):
    return Tex(lhs + '{-{b} \\pm \\sqrt{ {b}^2 - 4{a}{c} } \\over 2{a} }', **tex_config)
...
class RootCoefScene(Scene):
    coefs = [3, 2, 1, 0, -1, 1]
    root_plane_config = {'x_range': (-2.0, 2.0), 'y_range': (-2.0, 2.0), 'background_line_style': {'stroke_color': BLUE_E}}
    coef_plane_config = {'x_range': (-4, 4), 'y_range': (-4, 4), 'background_line_style': {'stroke_color': GREY}}
    plane_height = 5.5
    plane_buff = 1.5
    ...
    root_color = YELLOW
    coef_color = RED_B
    dot_style = {'radius': 0.05, 'stroke_color': BLACK, 'stroke_width': 3, 'stroke_behind': True}
    ...
    def add_planes(self):
        planes = VGroup(ComplexPlane(**self.root_plane_config), ComplexPlane(**self.coef_plane_config))
        for plane in planes:
    ...
    def get_coef_poly(self):
        ...
        return OldTex(f'x^{degree}', *(f' + c_{n} x^{n}' for n in range(degree - 1, 1, -1)), ' + c_{1} x', ' + c_{0}')

    def get_root_poly(self):
        return OldTex(*(f'(x - r_{i})' for i in range(self.get_degree())))
    ...
    def get_random_root(self):
        return complex(interpolate(*self.root_plane.x_range[:2], random.random()), interpolate(*self.root_plane.y_range[:2], random.random()))
    ...
    def set_roots(self, roots):
        self.root_dots.set_submobjects((Dot(self.root_plane.n2p(root), color=self.root_color, **self.dot_style) for root in roots))

    def set_coefs(self, coefs):
        self.coef_dots.set_submobjects((Dot(self.coef_plane.n2p(coef), color=self.coef_color, **self.dot_style) for coef in coefs[:-1]))
    ...
    def get_roots(self):
        return [self.root_plane.p2n(root_dot.get_center()) for root_dot in self.root_dots]

    def get_coefs(self):
        return [self.coef_plane.p2n(coef_dot.get_center()) for coef_dot in self.coef_dots] + [1.0]
    ...
    def get_tracers(self, dots, time_traced=2.0, **kwargs):
        tracers = VGroup()
        for dot in dots:
            dot.tracer = TracingTail(dot, stroke_color=dot.get_fill_color(), time_traced=time_traced, **kwargs)
    ...
    def get_tracking_lines(self, dots, syms, stroke_width=1, stroke_opacity=0.5):
        lines = VGroup(*(Line(stroke_color=root.get_fill_color(), stroke_width=stroke_width, stroke_opacity=stroke_opacity) for root in dots))

        def update_lines(lines):
            for sym, dot, line in zip(syms, dots, lines):
                line.put_start_and_end_on(sym.get_bottom(), dot.get_center())
        lines.add_updater(update_lines)
        return lines

    def add_root_lines(self, **kwargs):
        self.root_lines = self.get_tracking_lines(self.root_dots, self.get_r_symbols(self.root_poly), **kwargs)
        self.add(self.root_lines)

    def add_coef_lines(self, **kwargs):
        self.coef_lines = self.get_tracking_lines(self.coef_dots, self.get_c_symbols(self.coef_poly), **kwargs)
    ...
    def add_r_labels(self):
        self.r_dot_labels = self.add_dot_labels(VGroup(*(OldTex(f'r_{i}') for i in range(self.get_degree()))), self.root_dots)

    def add_c_labels(self):
        self.c_dot_labels = self.add_dot_labels(VGroup(*(OldTex(f'c_{i}') for i in range(self.get_degree()))), self.coef_dots)
    ...
    def get_root_swap_arrows(self, i, j, path_arc=90 * DEGREES, stroke_width=5, stroke_opacity=0.7, buff=0.3, **kwargs):
        ...
        kwargs['buff'] = buff
        return VGroup(Arrow(di, dj, **kwargs), Arrow(dj, di, **kwargs))

    def swap_roots(self, *indices, run_time=2, wait_time=1, **kwargs):
        self.play(CyclicReplace(*(self.root_dots[i] for i in indices), run_time=run_time, **kwargs))
        self.wait(wait_time)

    def rotate_coefs(self, indicies, center_z=0, run_time=5, wait_time=1, **kwargs):
        self.play(*(Rotate(self.coef_dots[i], TAU, about_point=self.coef_plane.n2p(center_z), run_time=run_time, **kwargs) for i in indicies))
        self.wait(wait_time)
    ...
    def carry_out_cycle(self):
        ...
        if self.dot_awaiting_loop is not None:
            self.tie_roots_to_coefs()
            self.play(Rotate(self.dot_awaiting_loop, angle=TAU, about_point=self.mouse_point.get_center().copy(), run_time=8))
            self.remove_dot_aura()
        self.prepare_cycle_interaction()
    ...
    def on_mouse_release(self, point, button, mods):
        ...
        if self.lock_coef_imag or self.window.is_key_pressed(ord('r')):
            ...
            dot.add_updater(lambda d: d.move_to(self.coef_plane.c2p(*d.last_norm * normalize(self.coef_plane.p2c(d.get_center())))))
...
class RadicalScene(RootCoefScene):
    ...
    c = 1.5
    root_plane_config = {'x_range': (-2.0, 2.0), 'y_range': (-2.0, 2.0), 'background_line_style': {'stroke_color': BLUE_E}}
    coef_plane_config = {'x_range': (-2, 2), 'y_range': (-2, 2), 'background_line_style': {'stroke_color': GREY}}
    plane_height = 4.0
    plane_buff = 3.0
    ...
    def get_radical_labels(self):
        ...
        arrow_kw = dict(stroke_width=5, stroke_color=GREY_A, buff=0.5)
        ...
        return VGroup(VGroup(r_arrow, l_arrow), VGroup(r_label, l_label))

    def get_angle_label(self, dot, plane, sym, get_theta):
        line = Line()
        line.set_stroke(dot.get_fill_color(), width=2)
        line.add_updater(lambda m: m.put_start_and_end_on(plane.get_origin(), dot.get_center()))
        arc = always_redraw(lambda: ParametricCurve(lambda t: plane.n2p((0.25 + 0.01 * t) * np.exp(complex(0, t))), t_range=[0, get_theta() + 1e-05, 0.025], stroke_width=2))
    ...
    def add_c_labels(self):
        self.c_dot_labels = self.add_dot_labels(VGroup(OldTex('c')), VGroup(self.coef_dots[0]))
    ...
    def update_coef_dots_by_roots(self, coef_dots):
        controlled_roots = [d for d in self.root_dots if len(d.get_updaters()) > 1]
...
class QuadraticFormula(RootCoefScene):
    coefs = [-1, 0, 1]
    coef_plane_config = {'x_range': (-2.0, 2.0), 'y_range': (-2.0, 2.0), 'background_line_style': {'stroke_color': GREY_B, 'stroke_width': 1.0}}
    sqrt_plane_config = {'x_range': (-2.0, 2.0), 'y_range': (-2.0, 2.0), 'background_line_style': {'stroke_color': BLUE_D, 'stroke_width': 1.0}}
    plane_height = 3.5
    ...
    def add_sqrt_plane(self):
        ...
        label = OldTex('-{b \\over 2} \\pm \\sqrt{{b^2 \\over 4} - c}', font_size=30)[0]
        for i in [1, 7, 12]:
    ...
    def get_coef_poly(self):
        return OldTex('x^2', '+ b x', '+ c')

    def add_c_labels(self):
        self.c_dot_labels = self.add_dot_labels(VGroup(OldTex('c'), OldTex('b')), self.coef_dots)
