...
class ConstructPolynomialWithGivenRoots(Scene):
    ...
    def construct(self):
        axes = self.add_axes()
        challenge = VGroup(Text('Can you construct a cubic polynomial'), OldTex('P(x) = x^3 + c_2 x^2 + c_1 x + c_0', tex_to_color_map={'c_2': RED_B, 'c_1': RED_B, 'c_0': RED_B}), OldTexText('with roots at $x = 1$, $x = 2$, and $x = 4$?', tex_to_color_map={'$x = 1$': self.root_color, '$x = 2$': self.root_color, '$x = 4$': self.root_color}))
        challenge.scale(0.7)
        ...
        self.wait()
        self.play(LaggedStart(*(FadeTransform(x_term.copy(), dot) for x_term, dot in zip(x_terms, root_dots)), lag_ratio=0.7, run_time=3))
        ...
        rects = VGroup(*(SurroundingRectangle(factored[i:i + 5], stroke_width=1, stroke_color=BLUE, buff=0.05) for i in range(1, 12, 5)))
        arrows = VGroup(*(Vector(DOWN).next_to(dot, UP, buff=0) for dot in root_dots))
        zeros_eqs = VGroup(*(OldTex(f'P({r}) = 0', font_size=24).next_to(rect, UP, SMALL_BUFF) for r, rect in zip(roots, rects)))
        self.play(FadeIn(factored, DOWN))
        self.wait()
        to_fade = VGroup()
        for rect, arrow, eq in zip(rects, arrows, zeros_eqs):
            self.play(ShowCreation(rect), FadeIn(eq), ShowCreation(arrow), FadeOut(to_fade))
            self.wait(2)
            to_fade = VGroup(rect, arrow, eq)
        self.play(FadeOut(to_fade))
        x_terms = factored[2::5]
        root_terms = VGroup(*(VGroup(m1, m2) for m1, m2 in zip(factored[3::5], factored[4::5])))
        expanded = OldTex('&x^3 ', '-1x^2', '-2x^2', '-4x^2 \\\\', '&+(-1)(-2)x', '+(-1)(-4)x', '+(-2)(-4)x\\\\', '&+(-1)(-2)(-4)')
        ...
        for k in range(4):
            for tup in it.combinations(range(3), k):
                ...
                expanded_rect = SurroundingRectangle(expanded_term, buff=SMALL_BUFF)
        ...
        self.play(FadeTransform(expanded[0], cleaner_expanded[0]), TransformMatchingShapes(expanded[1:4], cleaner_expanded[1]), expanded[4:].animate.next_to(cleaner_expanded[1], DOWN, aligned_edge=LEFT))
        self.wait()
        self.play(TransformMatchingShapes(expanded[4:7], cleaner_expanded[2]))
        self.wait()
        self.play(TransformMatchingShapes(expanded[7], cleaner_expanded[3]))
        ...
        self.wait()
        answer = OldTex('= x^3 -7x^2 + 14x -8', tex_to_color_map={'-7': RED_B, '14': RED_B, '-8': RED_B})
        answer.scale(0.7)
        ...
        for alt_roots in [(2, 4, 1), (4, 2, 1), (1, 4, 2), (1, 2, 4)]:
            ...
            movers, targets = [VGroup(*(group.get_parts_by_tex(str(root)) for root in alt_roots for group in groups)) for groups in [(factored, *cleaner_expanded), (alt_factored, *alt_expanded)]]
            self.play(TransformMatchingShapes(movers, targets, path_arc=PI / 2, run_time=1.5), randy.animate.look_at(movers))
        ...
        self.play(FadeOut(challenge, UP), MoveToTarget(full_factored), MoveToTarget(coef_poly))
        ...
        self.play(ShowCreation(arrow), FadeIn(new_challenge, 0.5 * RIGHT))
        self.wait()
        ...
        self.wait()
        for i in range(1, 4):
            self.play(cleaner_expanded[0].animate.set_opacity(1), equals.animate.set_opacity(1), FadeTransformPieces(cleaner_expanded[i], gen_expanded[i]), cleaner_expanded[i + 1:].animate.next_to(gen_expanded[i], RIGHT, SMALL_BUFF))
            self.wait()
        ...
        for coef, rhs, eq in zip(coef_terms, rhss, system):
            self.play(FadeTransform(coef.copy(), eq[0]), FadeIn(eq[1]), FadeTransform(rhs.copy(), eq[2:]))
            self.wait()
        cubic_example = VGroup(coef_poly, full_factored, system)
        q_roots = [-1, 1, 2, 4, 6]
        q_coefs = roots_to_coefficients(q_roots)
        q_poly = poly_tex(q_coefs)
        q_poly_factored = factored_poly_tex([f'r_{i}' for i in range(5)], root_colors=[YELLOW, GREEN])
        VGroup(q_poly, q_poly_factored).scale(0.8)
        q_poly.to_corner(UL)
        q_poly_factored.next_to(q_poly, DOWN, MED_LARGE_BUFF, aligned_edge=LEFT)
        self.play(FadeOut(cubic_example, DOWN), FadeOut(VGroup(arrow, new_challenge), DOWN), FadeIn(q_poly, DOWN))
        y_scale_factor = 0.1
        new_graph = axes.get_graph(lambda x: y_scale_factor * poly(x, q_coefs), x_range=(-1.2, 6.2))
        new_root_dots = Group(*(GlowDot(axes.c2p(x, 0)) for x in q_roots))
        new_graph.match_style(graph)
        axes.save_state()
        graph.save_state()
        root_dots.save_state()
        self.play(Transform(graph, new_graph), Transform(root_dots, new_root_dots))
        self.wait()
        root_terms = q_poly_factored.get_parts_by_tex('r_')
        self.play(FadeIn(q_poly_factored, lag_ratio=0.1, run_time=2), LaggedStart(*(FadeTransform(dot.copy(), term, remover=True) for dot, term in zip(root_dots, root_terms)), lag_ratio=0.5, run_time=3))
        self.wait()
        signed_coefs = [(-1) ** k * c for k, c in zip(it.count(1), q_coefs[-2::-1])]
        q_system, q_system_full = [get_symmetric_system(signed_coefs, abbreviate=abbrev, root_colors=[YELLOW, GREEN]) for abbrev in [True, False]]
        for mob in (q_system, q_system_full):
            mob.scale(0.8)
            mob.next_to(q_poly_factored, DOWN, LARGE_BUFF, aligned_edge=LEFT)
        root_tuple_groups = VGroup(*(VGroup(*(VGroup(*tup) for tup in it.combinations(root_terms, k))) for k in range(1, 6)))
        for equation, tuple_group in zip(q_system, root_tuple_groups):
            self.play(FadeIn(equation))
            self.wait(0.25)
            rects_group = VGroup(*(VGroup(*(SurroundingRectangle(term).set_stroke(BLUE, 2) for term in tup)) for tup in tuple_group))
            terms_column = VGroup(*(VGroup(*tup).copy().arrange(RIGHT, buff=SMALL_BUFF) for tup in tuple_group))
            terms_column.arrange(DOWN)
            terms_column.move_to(4 * RIGHT).to_edge(UP)
            anims = [ShowSubmobjectsOneByOne(rects_group, rate_func=linear), ShowIncreasingSubsets(terms_column, rate_func=linear, int_func=np.ceil)]
            if equation is q_system[1]:
                anims.append(Group(axes, graph, root_dots).animate.scale(0.5, about_point=axes.c2p(5, -3)))
        ...
        frame.save_state()
        self.play(frame.animate.replace(q_system_full, dim_to_match=0).scale(1.1), FadeIn(q_system_full, lag_ratio=0.1), FadeOut(q_system), Group(axes, graph, root_dots).animate.shift(2 * DOWN), run_time=2)
        self.wait(2)
        self.play(Restore(axes), Restore(graph), Restore(root_dots), FadeOut(q_system_full, 2 * DOWN), FadeOut(q_poly, 2 * DOWN), FadeOut(q_poly_factored, 2 * DOWN), FadeIn(cubic_example, 2 * DOWN), Restore(frame), run_time=2)
        self.wait()
        ...
        self.play(FadeIn(question), ShowCreation(arrow), FlashAround(gen_factored, run_time=3))
        self.wait()
        ...
        for const in example_constants:
            self.play(ChangeDecimalToValue(const_dec, const), run_time=3)
            self.wait()
        plane = ComplexPlane((-1, 6), (-3, 3))
        ...
        self.play(Uncreate(graph), Write(real_label), MoveToTarget(axes))
        self.wait(2)
        self.add(plane, root_dots, real_label)
        self.play(Restore(plane), FadeOut(axes.x_axis), FadeTransform(real_label, complex_label), run_time=2)
        self.wait(2)
        self.play(VGroup(coef_poly, top_const_dec).animate.next_to(plane, UP), gen_factored.animate.next_to(plane, UP, buff=1.2), FadeOut(equals), FadeOut(gen_expanded), frame.animate.shift(DOWN), run_time=2)
        self.wait()
        eq_zero = OldTex('= 0')
        eq_zero.scale(0.7)
        eq_zero.next_to(top_const_dec, RIGHT, SMALL_BUFF)
        eq_zero.shift(0.2 * LEFT)
        self.play(Write(eq_zero), VGroup(coef_poly, top_const_dec).animate.shift(0.2 * LEFT))
        self.wait()
        ...
        self.play(FlashAround(const_dec), FlashAround(top_const_dec), run_time=2)
        self.play(ChangeDecimalToValue(const_dec, 4), run_time=3)
        self.wait()
        root_eqs = VGroup(*(VGroup(OldTex(f'r_{i} ', '='), DecimalNumber(root, num_decimal_places=3)).arrange(RIGHT) for i, root in enumerate(get_roots())))
        ...
        self.play(FadeIn(root_eqs), VGroup(system, const_dec).animate.next_to(root_eqs, DOWN, LARGE_BUFF))
        self.wait(2)
    ...
    def add_axes(self):
        x_range = (-1, 6)
        y_range = (-3, 11)
        axes = Axes(x_range, y_range, axis_config=dict(include_tip=False, numbers_to_exclude=[]), widith=abs(op.sub(*x_range)), height=abs(op.sub(*y_range)))
...
class FactsAboutRootsToCoefficients(RootCoefScene):
    coefs = [-5, 14, -7, 1]
    coef_plane_config = {'x_range': (-15.0, 15.0, 5.0), 'y_range': (-10, 10, 5), 'background_line_style': {'stroke_color': GREY, 'stroke_width': 1.0}, 'height': 20, 'width': 30}
    root_plane_config = {'x_range': (-1.0, 6.0), 'y_range': (-3.0, 3.0), 'background_line_style': {'stroke_color': BLUE_E, 'stroke_width': 1.0}}
    plane_height = 3.5
    ...
    def construct(self):
        ...
        self.play(FadeOut(self.decimal_poly, DOWN), FadeOut(self.graph_group, DOWN))
        self.add_system()
        self.add_solver_functions()
        self.play(self.coef_system.animate.set_opacity(0.2), self.root_system[1:].animate.set_opacity(0.2))
        self.wait(note='Show loops with c0')
        brace = Brace(self.coef_system, RIGHT)
        properties = VGroup(Text('Continuous'), Text('Symmetric'))
        properties.arrange(DOWN, buff=MED_LARGE_BUFF)
        properties.next_to(brace, RIGHT)
        self.play(GrowFromCenter(brace), self.root_system.animate.set_opacity(0), self.coef_system.animate.set_opacity(1))
        self.wait()
        ...
        self.play(MoveToTarget(brace, path_arc=PI / 2), MoveToTarget(left_group), self.root_system.animate.set_opacity(1))
        self.wait()
        restriction = VGroup(Text('Cannot(!) be both'), Text('Continuous and single-valued', t2c={'Continuous': YELLOW, 'single-valued': BLUE}))
        ...
        symbols = OldTex('+,\\,', '-,\\,', '\\times,\\,', '/,\\,', '\\text{exp}\\\\', '\\sin,\\,', '\\cos,\\,', '| \\cdot |,\\,', '\\dots')
        ...
        self.play(FadeIn(impossibility[0]), FadeIn(arrow), ShowCreation(impossible_rect), MoveToTarget(restriction))
        for symbol in symbols:
            self.wait(0.25)
            self.add(symbol)
        self.wait()
        to_fade = VGroup(restriction[0], restriction[1].get_part_by_text('Continuous and'), arrow, impossibility, impossible_rect)
        to_fade.save_state()
        ...
        t2c = dict([(f'{sym}_{i}', color) for i in range(3) for sym, color in [('r', self.root_color), ('c', self.coef_color)]])
        t2c[f_name] = GREY_A
        mvf = OldTex(f'{f_name}(c_0, c_1, c_2)\\\\', '=\\\\', '\\left\\{r_0, r_1, r_2\\right\\}', tex_to_color_map=t2c)
        ...
        self.play(TransformMatchingShapes(self.root_system, mvf), restriction[1].get_part_by_text('single-valued').animate.fade(0.8))
    ...
    def add_c_labels(self):
        ...
        self.c_dot_labels[2].add_updater(lambda l: l.next_to(l.dot, DL, buff=0))
        return self.c_dot_labels

    def add_constant_decimals(self):
        dummy = '+10.00'
        polynomial = OldTex(f'x^3 {dummy}x^2 {dummy}x {dummy}', isolate=[dummy], font_size=40)
        ...
        VGroup(polynomial[0], decimals[0]).next_to(polynomial[2], LEFT, SMALL_BUFF, aligned_edge=DOWN)
    ...
    def add_graph(self):
        self.decimal_poly
        axes = Axes((0, 6), (-4, 10), axis_config=dict(tick_size=0.025), width=3, height=2)
        axes.set_height(2)
        axes.move_to(self.root_plane)
        axes.to_edge(UP, buff=SMALL_BUFF)
        graph = always_redraw(lambda: axes.get_graph(lambda x: poly(x, self.get_coefs()).real).set_stroke(BLUE, 2))
        root_dots = GlowDot()
        root_dots.add_updater(lambda d: d.set_points([axes.c2p(r.real, 0) for r in self.get_roots() if abs(r.imag) < 1e-05]))
        arrow = Arrow(self.decimal_poly.get_right(), axes)
        graph_group = Group(axes, graph, root_dots)
        self.play(ShowCreation(arrow), FadeIn(graph_group, shift=UR))
        graph_group.add(arrow)
        self.graph_group = graph_group

    def add_system(self):
        c_parts = self.get_c_symbols(self.coef_poly)
        system = get_symmetric_system((f'c_{i}' for i in reversed(range(len(self.coef_dots)))), signed=True)
        system.scale(0.8)
        system.next_to(self.coef_poly, UP, LARGE_BUFF)
        system.align_to(self.coef_plane, LEFT)
        self.add(system)
        kw = dict(lag_ratio=0.8, run_time=2.5)
        self.play(LaggedStart(*(TransformFromCopy(c, line[0]) for c, line in zip(c_parts, system)), **kw), LaggedStart(*(FadeIn(line[1:], lag_ratio=0.1) for line in system), **kw))
        self.add(system)
    ...
    def add_solver_functions(self):
        ...
        t2c = dict(((f'{sym}_{i}', color) for i in range(3) for sym, color in [('c', self.coef_color), ('r', self.root_color), (func_name, GREY_A)]))
        kw = dict(tex_to_color_map=t2c)
        lines = VGroup(*(OldTex(f'r_{i} = {func_name}_{i}(c_0, c_1, c_2)', **kw) for i in range(3)))
        lines.scale(0.8)
        ...
        self.play(LaggedStart(*(TransformFromCopy(r, line[0]) for r, line in zip(self.get_r_symbols(self.root_poly), lines)), **kw), LaggedStart(*(FadeIn(line[1:], lag_ratio=0.1) for line in lines), **kw))
        self.add(lines)
    ...
    def swap_root_symbols(self):
        ...
        for tup in [(1, 2, 0), (2, 0, 1), (0, 1, 2)]:
            ...
            self.play(*(TransformMatchingTex(l1, l2, path_arc=PI / 2, lag_ratio=0.01, run_time=2) for l1, l2 in zip(system, alt_system)))
            self.remove(system)
            system = alt_system
            self.add(system)
            self.wait()
...
class SolvabilityChart(Scene):
    ...
    def construct(self):
        ...
        lines = self.get_lines(rows=VGroup(s_words, *words), cols=VGroup(words, equations, *s_words))
        ...
        graph = always_redraw(lambda: axes.get_graph(lambda x: poly(x, get_coefs()), stroke_color=BLUE, stroke_width=2))
        root_dots = GlowDot()
        root_dots.add_updater(lambda m: m.set_points([axes.c2p(r.real, 0) for r in coefficients_to_roots(get_coefs()) if abs(r.imag) < 1e-05 and abs(r.real) < 5]))
        self.add(axes)
        ...
        self.wait()
        self.play(ShowCreation(graph), FadeIn(root_dots, rate_func=squish_rate_func(smooth, 0.3, 0.4)))
        self.wait()
        self.play(TransformMatchingShapes(equations[0].copy(), lin_solution))
        ...
        self.play(cover_rect.animate.move_to(row_lines[2], UL).shift(LEFT), FadeOut(lin_solution, DOWN))
        self.play(coef_tracker.animate.set_value(new_coefs))
        self.wait()
        self.play(TransformMatchingShapes(equations[1].copy(), quadratic_formula))
        self.play(Write(marks[1][1]))
        self.wait()
        key_to_color = dict([(TransformMatchingShapes.get_mobject_key(OldTex(c)[0][0]), color) for c, color in self.get_tex_to_color_map().items()])
        ...
        self.play(cover_rect.animate.move_to(row_lines[3], UL).shift(LEFT), FadeOut(quadratic_formula, DOWN))
        self.play(coef_tracker.animate.set_value(new_coefs))
        self.wait()
        self.play(TransformMatchingShapes(equations[2].copy(), full_cubic, run_time=2))
        self.wait()
        self.embed()

    def get_words(self, frame):
        words = VGroup(*map(Text, ('Linear', 'Quadratic', 'Cubic', 'Quartic', 'Quintic', 'Sextic')))
    ...
    def get_equations(self, words):
        ...
        equations = VGroup(OldTex('{a}x + {b} = 0', **kw), OldTex('{a}x^2 + {b}x + {c} = 0', **kw), OldTex('{a}x^3 + {b}x^2 + {c}x + {d} = 0', **kw), OldTex('{a}x^4 + \\cdots + {d}x + {e} = 0', **kw), OldTex('{a}x^5 + \\cdots + {e}x + {f} = 0', **kw), OldTex('{a}x^6 + \\cdots + {f}x + {g} = 0', **kw), OldTex('\\vdots', **kw))
    ...
    def get_solvability_words(self, equations):
        ...
        arith, radicals = ('$' + ' ,\\, '.join(operations[s]) + '$' for s in (slice(None, -1), slice(None)))
        s_words = VGroup(OldTexText('Solvable', ' using\\\\', arith), OldTexText('Solvable', ' using\\\\', radicals), OldTexText('Solvable\\\\', 'numerically'))
    ...
    def get_marks(self, equations, solvability_words):
        pre_marks = ['cxxxxxx', 'ccccxxx', 'ccccccc']
        marks = VGroup(*(VGroup(*(Checkmark() if pm == 'c' else Exmark() for pm in pm_list)) for pm_list in pre_marks))
    ...
    def get_tex_to_color_map(self):
        ...
        return dict(((f'{{{char}}}', color) for char, color in zip(chars, colors)))
...
class StudySqrt(RadicalScene):
    ...
    def construct(self):
        ...
        equations = VGroup(OldTex('x^2 - c = 0', **kw), OldTex('x =', '\\sqrt{c}', **kw))
        equations.arrange(DOWN, buff=MED_LARGE_BUFF)
        equations.to_edge(UP)
        self.wait()
        self.play(FadeIn(equations[0], UP))
        self.wait()
        self.play(TransformMatchingShapes(equations[0].copy(), equations[1], path_arc=PI / 2))
        self.wait()
        sqrt_label = equations[1][1:].copy()
        c_label = VGroup(OldTex('c = ', tex_to_color_map={'c': self.coef_color}), DecimalNumber(self.c))
        ...
        r_labels = VGroup(*(VGroup(OldTex(f'r_{i}', '='), DecimalNumber(self.c, include_sign=True)) for i in range(2)))
        ...
        self.play(FadeIn(c_label), ShowCreation(sqrt_arrow), sqrt_label.animate.next_to(sqrt_arrow, UP), FadeOut(equations))
        ...
        self.play(r_labels[0].animate.match_y(c_label), FadeOut(r_labels[1], DOWN), root_dots[1].animate.set_opacity(0.5), root_dots[1].tracer.animate.set_stroke(opacity=0.5), root_labels[1].animate.set_opacity(0.5))
        self.wait()
        r_label = r_labels[0]
        self.show_angle_variation(c_label, r_label, sqrt_arrow)
        self.play(sqrt_arrow.animate.set_width(1.75).match_y(self.root_plane), MaintainPositionRelativeTo(sqrt_label, sqrt_arrow))
        option = VGroup(OldTexText('One option:', color=BLUE), OldTexText('\\\\Make', ' $\\sqrt{\\quad}$', ' single-valued, but discontinuous', font_size=36))
        ...
        self.wait()
        self.remove(root_tracers)
        self.play(self.root_dots.animate.set_opacity(0), FadeOut(self.root_poly), FadeOut(root_labels), FadeIn(np_tex), FadeIn(sqrt_dot), FadeIn(sqrt_label))
        self.wait(note='Show discontinuity')
        taylor_series = OldTex('\\sqrt{x} \\approx', '1', '+ \\frac{1}{2}(x - 1)', '- \\frac{1}{8}(x - 1)^2', '+ \\frac{1}{16}(x - 1)^3', '- \\frac{5}{128}(x - 1)^4', '+ \\cdots', font_size=36)
        ...
        def f(x, n):
            return sum((gen_choose(1 / 2, k) * (x - 1) ** k for k in range(n)))
        ...
        self.play(FadeOut(np_tex), FadeIn(ts_group, UP), FadeOut(option, UP))
        self.wait()
        self.play(GrowFromCenter(brace), FadeIn(upper_f_label, 0.5 * DOWN))
        self.wait()
        self.play(TransformFromCopy(upper_f_label, f_label), GrowFromPoint(f_dot, upper_f_label.get_center()))
        self.wait()
        anims = [brace.animate.become(Brace(taylor_series[1:], DOWN, buff=SMALL_BUFF))]
        ...
        self.play(FadeOut(ts_group, UP), *map(FadeOut, (disc, f_label, f_dot, sqrt_label, sqrt_dot)), FadeIn(root_labels), FadeIn(self.root_poly), root_dots.animate.set_opacity(1))
    ...
    def show_angle_variation(self, c_label, r_label, arrow):
        ...
        left_exp_label, right_exp_label = (self.get_exp_label(get_theta=func, color=angle_color).move_to(label[-1], DL) for label, func in [(c_label, get_theta), (r_label, lambda: get_theta() / self.n)])
        below_arrow_tex = Tex('e^{x} \\rightarrow e^{x /' + str(self.n) + '}', font_size=36, tex_to_color_map={'\\theta': angle_color})
        below_arrow_tex.next_to(arrow, DOWN)
        angle_labels = VGroup(self.get_angle_label(self.coef_dots[0], self.coef_plane, '\\theta', get_theta), self.get_angle_label(self.root_dots[0], self.root_plane, f'\\theta / {self.n}', lambda: get_theta() / self.n))
        self.add(circle, self.coef_dots)
        self.lock_coef_norm = True
        self.tie_roots_to_coefs()
        self.play(FadeIn(circle), FadeIn(angle_labels), self.coef_dots[0].animate.move_to(self.coef_plane.n2p(1)))
        self.wait()
        self.play(FadeOut(c_label[-1], UP), FadeOut(r_label[-1], UP), FadeIn(left_exp_label, UP), FadeIn(right_exp_label, UP))
        self.wait(note='Rotate c a bit')
        self.play(Write(below_arrow_tex))
        self.wait(note='Show full rotation, then two rotations')
        self.play(LaggedStart(*map(FadeOut, (circle, angle_labels, left_exp_label, right_exp_label, c_label[:-1], r_label[:-1], below_arrow_tex))))
...
class CubeRootBehavior(StudySqrt):
    ...
    def construct(self):
        ...
        self.wait()
        self.wait()
        self.play(right_arrow_group.animate.to_edge(UP), *map(FadeIn, (c_label, r_label)))
