...
def get_sphere_slices(radius=1.0, n_slices=20):
    ...
    north_slices = Group(*(ParametricSurface(uv_func=lambda u, v: [radius * math.sin(v) * math.cos(u), radius * math.sin(v) * math.sin(u), radius * math.cos(v)], u_range=[theta, theta + delta_theta], v_range=[0, PI / 2], resolution=(4, 25)) for theta in np.arange(0, TAU, delta_theta)))
...
def get_flattened_slices(radius=1.0, n_slices=20, straightened=True):
    slc = ParametricSurface(lambda u, v: [u * math.sin(v * PI / 2), 1 - v, 0], u_range=[-1, 1], v_range=[0, 1], resolution=(4, 25))
    ...
    color_slices(north_slices)
    equator = Line(north_slices.get_corner(DL), north_slices.get_corner(DR), **EQUATOR_STYLE)
...
class PreviewThreeExamples(InteractiveScene):
    ...
    def construct(self):
        ...
        titles = VGroup(OldTexText('S.A. = $\\pi^2 R^2$'), OldTex('\\pi = 4').scale(1.5), Text('All triangles\nare isosceles'))
        titles.set_width(rects[0].get_width())
        for title, rect in zip(titles, rects):
            title.next_to(rect, UP)
        self.play(LaggedStartMap(DrawBorderThenFill, rects, lag_ratio=0.3), LaggedStartMap(FadeIn, titles, shift=0.25 * UP, lag_ratio=0.3))
        self.wait()
...
class SimpleSphereQuestion(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(Write(text), morty.change('raise_right_hand', text), randy.change('thinking', text))
...
class SphereExample(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(slices), FadeOut(sphere, lag_ratio=0, scale=0.95), FadeOut(mesh, lag_ratio=0, scale=0.95))
        self.play(LaggedStart(*(FadeIn(sh, rate_func=there_and_back) for sh in slice_highlights), lag_ratio=0.35, run_time=1.5))
        ...
        self.play(Transform(slices[0], flat_slices[0]), Transform(slices[2], flat_slices[2]), run_time=2)
        self.wait()
        self.play(Transform(slices[1], flat_slices[1], run_time=2))
        self.wait()
        slc = flat_slices[0][0]
        ...
        def update_width_line(width_line, slc=slc, v_tracker=v_tracker):
            ...
            width_line.set_width(1.2 * slc.get_width() * math.sin(v) + 0.01)
        ...
        vslices = VGroup(*(VGroup(*(tri_template.copy().rotate(rot).replace(slc, stretch=True) for slc in hemi)) for rot, hemi in zip([0, PI], slices)))
        ...
        self.play(Write(circ_label), VShowPassingFlash(vslices[2].copy().set_stroke(YELLOW, 5).insert_n_curves(20), time_width=1.5, run_time=1.5), vslices[2].animate.set_color(YELLOW))
        ...
        self.play(ShowCreation(edge), Write(q_marks))
        ...
        self.play(Write(equation.select_parts('\\text{Area}')), Write(equation.select_parts('=')[0]), Write(equation.select_parts('\\times')), TransformFromCopy(arc_form2, equation.select_parts(arc_tex)), TransformFromCopy(circ_formula, equation.select_parts(circ_tex)))
        self.wait()
        self.play(Write(equation.select_parts('=')[1]), Write(equation.select_parts('\\pi^2 R^2')))
...
class SomethingSomethingLimits(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.says('Something,\nsomething\nlimits!', mode='shruggie'), self.change_students('hesitant', 'angry', 'erm'))
        self.wait()
...
class Proof2Slide(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(circle.animate.set_x(nl.n2p(4)[0]), t_tracker.animate.set_value(1), rate_func=linear, run_time=4)
        self.wait()
...
class CircleExample(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(sectors.animate.scale(0.7).to_corner(UL), question.animate.to_corner(UR))
        ...
        self.play(MoveToTarget(lh, run_time=1.5), MoveToTarget(rh, run_time=1.5, path_arc=PI))
        self.play(lh.animate.shift(UP), rh.animate.shift(DOWN))
        self.wait()
        self.play(*(LaggedStart(*(VShowPassingFlash(piece, time_width=2) for piece in group.copy().set_fill(opacity=0).set_stroke(RED, 5)), lag_ratio=0.02, run_time=4) for group in [laid_sectors, sectors]))
        ulp = lh[0].get_top()
    ...
    def get_sectors(self, circle, n_slices=20, fill_colors=[BLUE_D, BLUE_E]):
        angle = TAU / n_slices
        sectors = VGroup(*(Sector(angle=angle, start_angle=i * angle, fill_color=color, fill_opacity=1) for i, color in zip(range(n_slices), it.cycle(fill_colors))))
...
class SphereSectorAnalysis(Scene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(slices, lag_ratio=0.05), frame.animate.reorient(-10, 70), run_time=2)
        self.add(slices, mesh)
        self.wait()
        self.play(slices[0][1:].animate.set_opacity(0.1), slices[1].animate.set_opacity(0.1))
        self.wait()
        dtheta = TAU / self.n_slices
        ...
        def get_width_line():
            return ParametricCurve(lambda t: radius * np.array([math.sin(t) * math.sin(get_phi()), -math.cos(t) * math.sin(get_phi()), math.cos(get_phi())]), t_range=(0, dtheta), stroke_color=RED, stroke_width=2)
        ...
        width_label.add_updater(lambda m: m.next_to(width_line, OUT, buff=SMALL_BUFF).set_width(1.5 * width_line.get_width() + 0.01))
        ...
        self.play(frame.animate.reorient(-65, 65), MoveToTarget(slices), MoveToTarget(mesh), slices[2].animate.set_stroke(width=0), run_time=2)
        ...
        def get_radial_line():
            return Line(ORIGIN, get_sphere_point(), stroke_color=YELLOW, stroke_width=2)
        ...
        self.play(phi_tracker.animate.set_value(30 * DEGREES), UpdateFromAlphaFunc(angle_label, lambda m, a: m.update().set_opacity(a)))
        self.wait()
        self.play(phi_tracker.animate.set_value(75 * DEGREES), run_time=3)
        ...
        self.wait()
        self.play(FadeTransform(formula1[1], formula2[0]), FadeTransform(formula1[2], formula2[1]), FadeTransform(formula1[3], formula2[2]), FadeOut(formula1[0]), FadeOut(formula1[4:]))
        self.wait()
        axes = Axes((0, PI, PI / 4), (0, 1, 1 / 2), height=1, width=PI)
        ...
        self.play(phi_tracker.animate.set_value(90 * DEGREES), ShowCreation(graph), UpdateFromAlphaFunc(dot, lambda d, a: d.move_to(graph_copy.pfp(a))), run_time=6)
        self.wait()
        self.play(phi_tracker.animate.set_value(45 * DEGREES), UpdateFromAlphaFunc(dot, lambda d, a: d.move_to(graph.pfp(1 - 0.5 * a))), run_time=3)
        self.wait()

class FalseVsTrueSurfaceAreaOverlay(InteractiveScene):

    def construct(self):
        false_answer = VGroup(Text('False answer', color=RED), OldTex('\\pi^2 R^2'))
        true_answer = VGroup(Text('True answer', color=GREEN), OldTex('4\\pi R^2'))
...
class FakeAreaManipulation(InteractiveScene):
    ...
    def construct(self):
        ...
        grid = NumberPlane(x_range=(-30, 30), y_range=(-30, 30), faded_line_ratio=0)
        grid.set_stroke(width=1)
        grid.scale(unit)
        grid.shift(3 * DOWN - grid.c2p(0, 0))
        vertex_dots = VGroup(Dot(group1.get_top()), Dot(group1.get_corner(DR)), Dot(group1.get_corner(DL)))
        self.add(grid)
        self.add(*group1)
        ...
        kw = {'lag_ratio': 0.1, 'run_time': 2, 'rate_func': bezier([0, 0, 1, 1])}
        path_arc_factors = [-1, 1, 0, 0, -1, 1]
        for target in targets:
            self.play(group1.animate.space_out_submobjects(1.2))
            self.play(*[Transform(sm1, sm2, path_arc=path_arc_factors[i] * 60 * DEGREES, **kw) for i, sm1, sm2 in zip(it.count(), group1, target)])
            self.wait(2)
        lines = VGroup(Line(grid.c2p(0, 12), grid.c2p(-5, 0)), Line(grid.c2p(0, 12), grid.c2p(5, 0)))
        lines.set_stroke(YELLOW, 2)
        self.disable_interaction(lines)
        frame = self.camera.frame
        frame.save_state()
        self.play(ShowCreation(lines, lag_ratio=0))
        self.play(frame.animate.scale(0.15).move_to(group1[0].get_corner(UR)), run_time=4)
        self.wait(3)
        ...
        self.wait()
        self.play(frame.animate.scale(0.15).move_to(group1[4].get_corner(UR)), run_time=4)
        self.wait(2)
        self.play(frame.animate.restore(), run_time=2)
        ...
        slope_labels = VGroup(OldTexText('Slope =', ' $5 / 2$'), OldTexText('Slope =', ' $7 / 3$'))
        for line, label in zip(lil_lines, slope_labels):
            ...
            label.add(arrow)
        self.play(FadeOut(lines[0]), ShowCreation(lil_lines))
        for line, label in zip(lil_lines, slope_labels):
            ...
            self.play(FadeIn(label[:2]), ShowCreation(label[2]))
            self.play(TransformFromCopy(line, y_line), FlashAround(label[1][0]))
            self.play(TransformFromCopy(line, x_line), FlashAround(label[1][2]))
            self.wait()
    ...
    def get_diagrams(self):
        ...
        ell = Polygon(ORIGIN, 4 * RIGHT, 4 * RIGHT + 2 * UP, 2 * RIGHT + 2 * UP, 2 * RIGHT + 5 * UP, 5 * UP)
        ell.scale(unit)
...
class ContrastSphericalGeometry(InteractiveScene):
    ...
    def construct(self):
        titles = VGroup(Text('Spherical geometry'), Text('Euclidean geometry'))
        ...
        shapes = VGroup(Polygon(RIGHT, UP, DL).set_fill(BLACK, 1), Square().set_fill(BLUE_C, 1), Circle().set_fill(BLUE_E, 1), RegularPolygon(5).set_fill(GREY_BROWN, 1))
        ...
        t2c = {'\\alpha': BLUE_B, '\\beta': BLUE_C, '\\gamma': BLUE_D}
        kw = dict(tex_to_color_map=t2c)
        sph_tri = VGroup(VMobject().pointwise_become_partial(mesh[16], 0.5, 1.0), VMobject().pointwise_become_partial(mesh[19], 0.5, 1.0).reverse_points(), VMobject().pointwise_become_partial(mesh[26], 16 / 20, 19 / 20).reverse_points())
        ...
        angle_equations = VGroup(Tex('\\alpha + \\beta + \\gamma > 180^\\circ', **kw), Tex('\\alpha + \\beta + \\gamma = 180^\\circ', **kw))
        for eq, title in zip(angle_equations, titles):
            eq.next_to(title, DOWN, buff=1.5)
        sph_tri_angle_labels.suspend_updating()
        self.play(LaggedStartMap(Write, VGroup(pre_tri, pre_tri_arcs, pre_tri_angle_labels, sph_tri, sph_tri_angle_labels)), *(FadeIn(eq, 0.25 * DOWN) for eq in angle_equations))
        sph_tri_angle_labels.resume_updating()
        sphere_group.add(sph_tri, sph_tri_angle_labels)
        sph_tri.deactivate_depth_test()
        self.add(sphere_group)
        self.play()
        self.play(FadeIn(shift_line, shift=shift_line.shift_vect), Rotate(sphere_group, -30 * DEGREES, axis=sphere_axis, run_time=2))
        for i in (0, 1):
            self.play(TransformFromCopy(pre_tri_arcs[i + 1], new_arcs[i]), TransformFromCopy(pre_tri_angle_labels[i + 1], new_labels[i]))
        self.wait()
        pre_tri_group = VGroup(pre_tri, pre_tri_arcs, pre_tri_angle_labels, shift_line, new_arcs, new_labels)
        area_formulas = VGroup(Tex('\\text{Area}(\\Delta) = (\\alpha + \\beta + \\gamma - \\pi) R^2', **kw), Tex('\\text{Area}(\\Delta) = \\frac{1}{2} bh'))
        for title, formula in zip(titles, area_formulas):
            formula.move_to(title).shift(1.5 * DOWN)
        self.play(FadeIn(area_formulas, 0.5 * DOWN), angle_equations.animate.shift(0.75 * DOWN))
        curvature_words = VGroup(OldTexText('Gaussian curvature > 0', color=GREEN_B), OldTexText('Gaussian curvature = 0', color=YELLOW))
        for words, eq in zip(curvature_words, angle_equations):
            words.next_to(eq, DOWN, MED_LARGE_BUFF)
        self.play(FadeIn(curvature_words, 0.7 * DOWN), sphere_group.animate.scale(0.7, about_edge=DOWN), pre_tri_group.animate.scale(0.7, about_edge=DOWN))
        self.wait()
        ...
        self.play(LaggedStart(*(FadeOut(mob, RIGHT) for mob in [area_formulas[1], angle_equations[1], curvature_words[1], pre_tri_group])), TransformFromCopy(sphere, unwrapped, run_time=2))
...
class GiveItAGo(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        ss = self.students
        self.play(morty.says("Can you see\nwhat's happening?"), ss[0].change('pondering', self.screen), ss[1].change('angry', self.screen), ss[2].change('confused', self.screen))
        self.wait()
...
class SquareCircleExample(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(question.animate.to_edge(UP), Transform(circum, unwrapped_circum))
        ...
        self.wait()
        self.play(TransformFromCopy(VGroup(radial_line), new_radii, lag_ratio=0.2, run_time=2), FadeIn(perimeter_label))
        self.wait()
        ...
        for i, curve in enumerate(approx_curves[1:]):
            ...
            if i == 0:
                ...
                self.play(MoveAlongPath(dots[0], square), MoveAlongPath(dots[1], curve), ShowCreation(sc), ShowCreation(cc), rate_func=linear, run_time=6)
        ...
        self.play(Transform(square, approx_curves[0]), frame.animate.move_to(4 * RIGHT), FadeIn(dot), FadeIn(t_label), Write(c_labels[0]), Write(t_axis), Write(t_indicator), run_time=1)
        square.match_points(approx_curves[0])
        self.wait()
        self.play(t_tracker.animate.set_value(1), run_time=7)
        self.wait()
        t_tracker.set_value(0)
        self.play(Transform(square, approx_curves[1]), FadeTransform(c_labels[0], c_labels[1]))
        self.play(t_tracker.animate.set_value(1), run_time=7)
        self.wait()
        t_tracker.set_value(0)
        ...
        for i in range(2, len(approx_curves)):
            ...
            self.play(Transform(square, approx_curves[i]), FadeTransform(c_labels[i - 1], c_labels[i]), run_time=0.5)
            self.wait(0.5)
        ...
        self.wait()
        self.play(FadeTransform(lim_tex_ex, lim_tex[2]), Write(lim_tex[:2]))
        self.wait()
        ...
        t2c = {'\\lim_{n \\to \\infty}': YELLOW, '\\text{len}': RED, 'c_n(t)': WHITE, '\\Big(': WHITE, '\\Big)': WHITE}
        ...
        self.wait()
        self.play(VGroup(lim_tex, lim_len).animate.next_to(frame.get_corner(UR), DL, MED_LARGE_BUFF), FadeOut(top_group, 2 * UR))
...
class ObviouslyWrong(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        ss = self.students
        self.play(LaggedStart(ss[2].says("Obviously those\nperimeters aren't\nthe circle...", mode='sassy', look_at=self.screen, bubble_direction=LEFT), ss[0].change('erm', self.screen), ss[1].change('angry', self.screen), morty.change('guilty')))
        self.wait(5)
...
class UpshotOfLimitExample(InteractiveScene):
    ...
    def construct(self):
        ...
        subtitle = Text("What's true of a sequence may not be true of the limit", t2c={'sequence': BLUE, 'limit': YELLOW})
        subtitle.set_max_width(FRAME_WIDTH - 1)
        subtitle.next_to(title, DOWN, buff=MED_LARGE_BUFF)
        self.add(title, underline, subtitle)
        n = 4
        numbers = VGroup(OldTex('1.4'), OldTex('1.41'), OldTex('1.414'), OldTex('1.4142'), OldTex('\\sqrt{2}'))
        numbers.scale(1.25)
        properties = VGroup(*Text('Rational', font_size=30).replicate(n), Text('Irrational', font_size=30))
        folder = '/Users/grant/Dropbox/3Blue1Brown/videos/2022/visual_proofs/lies/images/'
        rational_example = VGroup(numbers, properties)
        circle_example = Group(Group(*(ImageMobject(os.path.join(folder, 'SquareApprox' + end)).set_width(1.25) for end in ['1', '2', '3', '4', 'Inf'])), VGroup(*TexText('Len = 8').replicate(n), OldTexText('Len = $2\\pi$')))
        fourier_example = Group(Group(*(ImageMobject(os.path.join(folder, 'Fourier' + end)).set_width(1.25) for end in ['1', '2', '3', '4', 'Inf'])), VGroup(*TexText('Continuous').replicate(n), OldTexText('Discontinuous')))
        ...
        for i in range(3):
            ...
            self.play(LaggedStartMap(FadeIn, objs, lag_ratio=0.5, scale=2), LaggedStartMap(FadeIn, descs, lag_ratio=0.5, scale=2), run_time=2)

class WhyWeNeedProofs(TeacherStudentsScene):

    def construct(self):
        phrases = VGroup(Text('Looks can be deceiving'), Text('We need rigor!'), Text('We need proofs!'))
        ...
        morty = self.teacher
        ss = self.students
        self.play(morty.change('raise_right_hand'), FadeIn(phrases[0], UP), ss[0].change('happy'), ss[1].change('tease'), ss[2].change('happy'))
        self.wait()
        self.play(morty.change('hooray'), phrases[0].animate.shift(UP), FadeIn(phrases[1], UP))
        self.play(phrases[0].animate.shift(UP), phrases[1].animate.shift(UP), FadeIn(phrases[2], UP))
        self.wait()
        self.play(morty.change('tease'), FadeIn(euclid, lag_ratio=0.3), self.change_students('pondering', 'pondering', 'thinking', look_at=euclid))
        self.wait(4)
...
class Proof3Slide(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(TransformFromCopy(AB, AC), FadeTransform(eq[0].copy(), eq[2]), Write(eq[1]))
        self.wait()

class SpeakingOfLimits(TeacherStudentsScene):

    def construct(self):
        self.play(self.teacher.says('Speaking of\nlimits...'), self.change_students('happy', 'tease', 'thinking'))
        self.wait(4)
...
class IntegralExample(InteractiveScene):
    ...
    def construct(self):
        ...
        all_rects = VGroup(*(axes.get_riemann_rectangles(graph, (0, 3), dx).set_stroke(BLACK, np.round(4 * dx, 1), behind=False) for dx in [2 ** (-n) for n in range(2, 8)]))
        rects = all_rects[0]
        ...
        self.wait()
        self.play(LaggedStart(*(ApplyMethod(rect.set_color, YELLOW, rate_func=there_and_back) for rect in rects), lag_ratio=0.5, run_time=4))
...
class IntegralError(InteractiveScene):
    ...
    def construct(self):
        ...
        graph = axes.get_graph(lambda x: x ** 2)
        graph.set_stroke(TEAL, 2)
        all_rects = VGroup(*(axes.get_riemann_rectangles(graph, (0, 3), dx).set_stroke(BLACK, np.round(4 * dx, 1), behind=False) for dx in [2 ** (-n) for n in range(2, 9)]))
        rects = all_rects[0]
        last_rects = all_rects[-1].copy()
        last_rects.set_stroke(width=0)
        self.add(axes, graph)
        ...
        error_arrows = VGroup(*(Arrow(error_label, rect.get_top() + 0.2 * UP, stroke_width=2, tip_width_ratio=8) for rect in rects))
        error_arrows.set_color(RED)
        self.add(error, rects)
        self.play(FadeIn(error, lag_ratio=0.5, run_time=2), Write(error_label, stroke_color=RED), LaggedStart(*(ShowCreation(arrow) for arrow in error_arrows), lag_ratio=0.2, run_time=3))
        self.wait()
        ...
        for rg in all_rects:
            ...
            for rect in rg:
                ...
                box.replace(Line(axes.i2gp(axes.x_axis.p2n(rect.get_left()), graph), axes.i2gp(axes.x_axis.p2n(rect.get_right()), graph)), stretch=True)
                boxes.add(box)
            all_error_boxes.add(boxes)
        self.play(FadeIn(all_error_boxes[0], lag_ratio=0.1), error.animate.set_opacity(0.25), FadeOut(error_arrows))
        ...
        self.play(MoveToTarget(error_label), Write(sum_squares[:2]), TransformFromCopy(pre_box_sym, box_sym))
        self.add(rhs)
        self.play(CountInFrom(rhs[1], 0), LaggedStart(*(VFadeInThenOut(r.copy().set_fill(opacity=1), buff=0, color=RED) for r in all_error_boxes[0]), lag_ratio=0.2), run_time=2)
        self.wait()
        for new_rects in all_rects[1:]:
            self.play(Transform(rects, new_rects), ChangeDecimalToValue(rhs[1], rhs[1].get_value() / 4), run_time=3)
            self.wait(0.5)
...
class FalseEuclidProofAnnotation(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Transform(AF.brace, AB.brace, path_arc=45 * DEGREES), Transform(FB.brace, AB.brace, path_arc=45 * DEGREES))
        self.wait()
        self.play(GrowFromCenter(AE.brace), run_time=1)
        self.play(GrowFromCenter(EC.brace), run_time=1)
        self.wait()
        self.play(Transform(AE.brace, AC.brace, path_arc=45 * DEGREES), Transform(EC.brace, AC.brace, path_arc=45 * DEGREES))
        self.wait()
        return
        BP = Line(B, P)
        CP = Line(C, P)
        ...
        self.wait()
        self.play(TransformFromCopy(PF, BF, path_arc=90 * DEGREES), TransformFromCopy(PE, CE, path_arc=-90 * DEGREES))
        self.wait()
        ...
        self.play(AB.animate.set_angle(-90 * DEGREES).next_to(A, RIGHT, aligned_edge=UP, buff=2), AC.animate.set_angle(-90 * DEGREES).next_to(A, RIGHT, aligned_edge=UP, buff=2.5))
        self.wait()
...
class FalseEuclidFollowup(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(tri, labels)
        self.wait()
        self.play(FadeIn(equation), FadeIn(words), ShowCreationThenDestruction(get_side(0, 1), run_time=2), ShowCreationThenDestruction(get_side(0, 2), run_time=2))
        self.play(Transform(tri, tri2), labels[1].animate.shift(1.5 * UP))
        self.wait()
        self.play(CyclicReplace(*labels))
        self.play(ShowCreationThenDestruction(get_side(1, 0), run_time=2), ShowCreationThenDestruction(get_side(1, 2), run_time=2), ShowCreation(iso_cross))
        cross_group = VGroup(iso, iso_cross)
        self.play(cross_group.animate.shift(DOWN), FadeIn(equi, 0.5 * DOWN), Transform(tri, tri3), labels[2].animate.next_to(tri3, UP, SMALL_BUFF))
        self.play(Transform(equation, equation2[:len(equation)]), FadeIn(equation2[len(equation):], RIGHT))
        self.wait()
        words = VGroup(words.select_parts('All triangles are'), equi)
        possibilities = VGroup(Text('1. This is true'), OldTexText("2. Euclid's axioms $\\Rightarrow$ falsehoods")[0], Text('3. This proof has a flaw'))
        ...
        tri.add(labels)
        self.play(FadeOut(cross_group, DL), tri.animate.match_width(words).next_to(words, DOWN, LARGE_BUFF), LaggedStart(*(FadeIn(poss[:2], shift=LEFT) for poss in possibilities)), Write(poss_title))
        self.wait()
...
class TryToFindFault(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(morty.says('I dare you to\nfind a fault', mode='tease'))
        self.play_student_changes('pondering', 'pondering', 'sassy')
        self.wait()
        self.play(morty.debubble(mode='raise_right_hand'), self.change_students('thinking', 'happy', 'tease'), Write(stars))
        self.wait(3)
...
class SideSumTruthiness(InteractiveScene):
    ...
    def construct(self):
        ...
        eq.to_corner(UR)
        labels = VGroup(VGroup(Text('True'), Checkmark()), VGroup(Text('False'), Exmark()))
        labels.scale(60 / 48)
        for label in labels:
...
class PythagoreanProofSketch(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(ReplacementTransform(pre_tri, tris[0]), side_labels.animate.scale(scale_factor, about_point=pre_tri.get_center()).shift(shift_vect))
        self.add(tris[1], side_labels)
        self.play(LaggedStart(TransformFromCopy(tris[0], tris[1], path_arc=PI), TransformFromCopy(tris[0], tris[2]), TransformFromCopy(tris[0], tris[3]), FadeIn(big_square), Animation(side_labels), lag_ratio=0.3))
        self.play(FadeIn(a_square), FadeTransform(side_labels[0], a_square_label), FadeIn(b_square), FadeTransform(side_labels[1], b_square_label))
        self.play(FadeOut(a_square), FadeOut(b_square), TransformFromCopy(a_square_label, equation.select_parts('a^2')), TransformFromCopy(b_square_label, equation.select_parts('b^2')), Write(equation.select_parts('+')))
        self.wait()
        tris.save_state()
        tris2 = tris.copy()
        ...
        self.play(LaggedStart(FadeOut(a_square_label), FadeOut(b_square_label), *(Transform(tris[i], tris2[i]) for i in (0, 2, 3)), lag_ratio=0.5))
        self.wait()
        self.play(FadeIn(c_square), FadeIn(c_square_label))
        self.play(FadeTransform(c_square_label.copy(), equation.select_parts('c^2')), Write(equation.select_parts('=')), c_square.animate.set_fill(opacity=0.5))
        self.wait()
        self.play(FadeOut(c_square))
        self.play(Transform(tris, tris.saved_state, lag_ratio=0.2, run_time=3, path_arc=45 * DEGREES), FadeOut(c_square_label), FadeIn(a_square_label, time_span=(1.5, 2.5)), FadeIn(b_square_label, time_span=(2, 3)))
        self.wait()
        self.play(Transform(tris, tris2, lag_ratio=0.2, run_time=3, path_arc=45 * DEGREES), FadeIn(c_square_label, time_span=(2, 3)), FadeOut(a_square_label, time_span=(0, 1)), FadeOut(b_square_label, time_span=(0.5, 1.5)))
        self.wait()
...
class LastSideBySide(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(squares)
        titles = VGroup(OldTexText('Given examples\\\\like this...'), OldTexText("What's needed to\\\\make this rigorous?"))
...
class EndScreen(PatreonEndScreen):
    CONFIG = {'thanks_words': 'Special thanks to the following patrons'}