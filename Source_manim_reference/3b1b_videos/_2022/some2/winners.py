...
class ExampleChannels(InteractiveScene):
    ...
    def construct(self):
        ...
        channels = [('Eric Rowland', '8.9k', '3gyHKCDq1YA', 423816), ('A Well-Rested Dog', '15k', '5M2RWtD4EzI', 381953), ('polylog', '10k', '-64UT8yikng', 225508), ('A Bit Wiser', '6.5k', '_DaTsI42Wvo', 104138), ('SithDev', '7.3k', '6JxvKfSV9Ns', 134753), ('Another Roof', '11k', 'dKtsjQtigag', 136517), ('Marc Evanstein', '3.1k', '8x374slJGuo', 161853), ('Armando Arredondo', '4.6k', '_lb1AxwXLaM', 272279), ('HexagonVideos', '2.8k', '-vxW42R47bc', 219047), ('SackVideo', '12k', 'UJp4q2D2Nh0', 233851), ('Morphocular', '31k', '2dwQUUDt5Is', 281021)]
        ...
        for tile, channel in zip(tiles, channels):
            ...
            count = VGroup(Integer(views, font_size=30), Text('Views', font_size=30))
            count.arrange(RIGHT, buff=0.1, aligned_edge=UP)
            count.next_to(video, DOWN, buff=0.15)
            tile.add(title, video, count)
        self.add(tiles)
        self.play(LaggedStart(*(UpdateFromAlphaFunc(tile, lambda m, a: m.set_opacity(a)) for tile in tiles), lag_ratio=0.5), LaggedStart(*(CountInFrom(tile[-1][0], 0, run_time=2.5) for tile in tiles), lag_ratio=0.2))
...
class IntroScene(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(title)
        self.play(LaggedStart(FadeIn(logos[0], LEFT), Write(logos[1], run_time=1, lag_ratio=0.01, stroke_width=0.1, stroke_color=BLACK), lag_ratio=0.3))
        self.wait()
        ...
        self.play(FadeOut(to_fade, lag_ratio=0.1, shift=UP, run_time=1.5), MoveToTarget(inits, time_span=(0.25, 1.5)), FadeOut(logos, LEFT))
        two = Text('2')
        ...
        self.play(LaggedStart(*(FadeTransform(init.copy(), two) for init in inits), lag_ratio=0.1), MoveToTarget(inits))
        inits.add(two)
        self.wait()
        count = VGroup(Integer(7000000, edge_to_fix=RIGHT), Text('collective views'))
        count.arrange(RIGHT, aligned_edge=UP)
        count.set_height(0.6)
        count.to_corner(UR)
        self.play(inits.animate.set_height(0.6).to_corner(UL), FadeIn(count[1]), CountInFrom(count[0], 0, run_time=3))
        self.add(count)
...
class PeerReview(InteractiveScene):
    ...
    def construct(self):
        n_peers = 10
        peers = VGroup(*(PiCreature(color=random.choice([BLUE_E, BLUE_D, BLUE_C, BLUE_B])) for x in range(n_peers)))
        peers.arrange(RIGHT)
        ...
        self.play(LaggedStart(*(AnimationGroup(pi.change('raise_right_hand'), FadeIn(video, 0.5 * UP)) for video, pi in pairs)))
        ...
        self.play(FadeIn(entry_box), FadeIn(entry_box_label), FadeOut(copies, lag_ratio=0.1), MoveToTarget(videos), LaggedStart(*(peer.change('tease', ORIGIN) for peer in peers)))
        peers.shuffle()
        for pi in peers:
            pi.watchlist = list(videos.copy())
            random.shuffle(pi.watchlist)
        self.play(LaggedStart(*(AnimationGroup(pi.watchlist[0].animate.next_to(pi.get_top(), UL, buff=0.1).shift(0.025 * RIGHT), pi.watchlist[1].animate.next_to(pi.get_top(), UR, buff=0.1).shift(0.025 * LEFT), pi.change('pondering', pi.get_top() + UP)) for pi in peers), lag_ratio=0.1))
        for x in range(10):
            self.play(LaggedStart(*(AnimationGroup(pi.change(['raise_right_hand', 'raise_left_hand'][1 - bit], pi.watchlist[x + bit]).set_anim_args(run_time=0.5), pi.watchlist[x + bit].animate.scale(1.2), pi.watchlist[x + 1 - bit].animate.scale(0.8)) for pi in peers for bit in [random.randint(0, 1)]), lag_ratio=0.1))
            self.play(LaggedStart(*(AnimationGroup(pi.change('sassy', pi.get_top() + UP).set_anim_args(run_time=0.5), FadeOut(pi.watchlist[x], scale=0.5), pi.watchlist[x + 1].animate.set_height(0.25).next_to(pi.get_top(), UL, buff=0.1).shift(0.025 * RIGHT), pi.watchlist[x + 2].animate.next_to(pi.get_top(), UR, buff=0.1).shift(0.025 * LEFT)) for pi in peers)))

    def get_judgement_animation(self, pi, videos):
        vids = list(videos.copy())
        random.shuffle(vids)
        anims = [Animation(Mobject(), run_time=3 * random.random())]
        anims.append(AnimationGroup(vids[0].animate.next_to(pi.get_top(), UL, buff=0.1), vids[1].animate.next_to(pi.get_top(), UR, buff=0.1), pi.change('pondering', pi.get_top() + UP)))
        for i in range(8):
            anims.append(pi.animate.change(random.choice(['raise_right_hand', 'raise_left_hand'])))
            anims.append(AnimationGroup(FadeOut(vids[i], LEFT), vids[i + 1].animate.next_to(pi.get_top(), UL, buff=0.1), videos[i + 2].animate.next_to(pi.get_top(), UR, buff=0.1)))
        return Succession(*anims)

class Over10KComparisons(InteractiveScene):

    def construct(self):
        count = VGroup(Integer(10000), Text('Comparisons'))
        count.arrange(DOWN)
        count.scale(2)
        plus = Text('+').scale(2)
        plus.next_to(count[0], RIGHT, buff=SMALL_BUFF)
        self.play(CountInFrom(count[0], 0, run_time=3), Write(count[1], run_time=1))
        self.add(plus)
        self.wait()
...
class YTStatement(InteractiveScene):
    ...
    def construct(self):
        words = VGroup(Text('Shared viewer base'), OldTex('\\Downarrow', font_size=96).set_stroke(WHITE, 0), Text('\n                YT is more likely to\n                recommend one video\n                to viewers of another.\n            ', alignment='LEFT'))
        words.arrange(DOWN, buff=MED_LARGE_BUFF)
        self.add(words[0])
        self.play(Write(words[1]), FadeIn(words[2], DOWN))
        self.wait()

class HistogramOfViews(InteractiveScene):
    data = [628083, 396785, 359233, 301850, 245930, 243370, 231732, 212708, 209343, 208586, 167442, 153613, 145692, 135465, 130783, 126159, 101919, 93069, 92110, 89790, 82160, 81311, 79598, 67092, 63373, 63239, 62246, 56242, 52840, 47267, 45942, 43032, 42558, 41618, 41201, 39059, 36675, 36138, 35999, 35999, 35805, 34057, 33137, 33009, 31064, 30855, 30616, 30122, 30016, 28817]

    def construct(self):
        axes = Axes((0, 50, 50), (0, 600000.0, 100000.0), width=12, height=6)
        axes.to_corner(DR)
        y_step = 100000
        axes.add_coordinate_labels(x_values=range(0), y_values=range(y_step, 7 * y_step, y_step))
        self.add(axes)
        bars = VGroup(*(Rectangle(width=axes.x_axis.get_width() / len(self.data), height=value * axes.y_axis.unit_size) for value in self.data))
        ...
        bars.shift(0.01 * UR)
        self.add(bars)
        title = Text('Distribution of SoME2 views (top 50 shown)')
        ...
        self.play(LaggedStart(*(ReplacementTransform(bar.copy().stretch(0, 1, about_edge=DOWN), bar) for bar in bars)), lag_ratio=0.2)
        self.add(bars)
        ...
        self.play(FadeIn(box), Write(some1_words))
        self.wait()
...
class ValueInSharedGoals(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        self.play(morty.says('Never underestimate\ncommunity and\ndeadlines'), self.change_students('happy', 'tease', 'thinking'))
        self.wait(5)
        self.play(morty.debubble(mode='raise_right_hand', look_at=self.screen), self.change_students('pondering', 'thinking', 'erm', look_at=self.screen))
        self.wait(2)
        links = VGroup(Text('explanaria.github.io/crystalgroups'), Text('thenumb.at/Autodiff'), Text('xperimex.com/blog/panorama-homography'), Text('calmcode.io/blog/inverse-turing-test.html'), Text('summbit.com/blog/bezier-curve-guide'), Text('lukelavalva.com/theoryofsliding'), Text('chessengines.org'), OldTex('\\vdots'))
        links.arrange(DOWN, aligned_edge=LEFT)
        links[-1].match_x(links[-2])
        links.set_width(5)
        links.to_corner(UR)
        self.play(morty.change('raise_left_hand', links), self.change_students('tease', 'happy', 'thinking', look_at=links), FadeIn(links, lag_ratio=0.3, run_time=2))
        self.wait(4)
        stds = self.students
        self.play(FadeOut(links, RIGHT), morty.change('guilty'), stds[0].says('But, like, who actually\nwon the contest?'), stds[1].change('erm', stds[0].eyes), stds[2].change('sassy', stds[0].eyes))
        self.wait(3)
...
class OneHundredEntries(InteractiveScene):
    ...
    def construct(self):
        ...
        winners = VGroup(VideoIcon(), self.written_icon(), *(VideoIcon() for x in range(3)))
        ...
        prizes = VGroup(*(VGroup(Text('$1,000 + ', color=GREEN), Randolph(mode='thinking', height=0.5, color=YELLOW)).arrange(RIGHT, buff=MED_SMALL_BUFF).next_to(winner, RIGHT) for winner in winners))
        self.add(title)
        ...
        self.play(LaggedStart(*(ReplacementTransform(vid, random.choice(videos)) for vid in (winners[0], *winners[2:]))), ReplacementTransform(winners[1], random.choice(articles)), LaggedStartMap(FadeIn, videos), LaggedStartMap(FadeIn, articles), FadeOut(title, UP), LaggedStartMap(FadeOut, prizes, shift=UP))
        self.add(videos, articles)
        self.play(Write(video_title), Write(article_title))
        self.wait()
        ...
        frame.save_state()
        judges = VGroup(Text('Alex Kontorovich'), Text('Henry Reich'), Text('Mithuna Yoganathan'), Text('Nicky Case'), Text('Tai-Danae Bradley'), Text('Deniz Ozbay'), Text('Jeffrey Wack'), Text('Fran Herr'))
        ...
        self.play(frame.animate.scale(1.5, about_edge=RIGHT), FadeIn(guest_words), logos.animate.next_to(guest_words, UP), LaggedStartMap(FadeIn, judges, shift=0.5 * RIGHT, lag_ratio=0.5, run_time=6))
        self.play(FadeIn(momath))
        self.wait()
        self.play(frame.animate.restore(), LaggedStart(*(FadeOut(mob, UL) for mob in [video_title, article_title, *logos, guest_words, *judges, momath]), run_time=1))
        self.wait()
...
class FeaturedContentFrame(InteractiveScene):
    ...
    def construct(self):
        ...
        if self.thumbnail:
            ...
            thumbnail = Mobject()
        self.play(Write(title, run_time=1), DrawBorderThenFill(frame), FadeIn(thumbnail))
...
class FourCategories(InteractiveScene):
    ...
    def construct(self):
        ...
        regions.set_submobject_colors_by_gradient(BLUE_D, BLUE_C, BLUE_E, GREY_BROWN)
        titles = VGroup(*(Text(word, font_size=60) for word in ['Motivation', 'Clarity', 'Novelty', 'Memorability']))
        for title, region in zip(titles, regions):
            title.next_to(region.get_top(), DOWN, buff=MED_SMALL_BUFF)
        self.play(LaggedStartMap(FadeIn, regions, lag_ratio=0.5), LaggedStartMap(Write, titles, lag_ratio=0.8), run_time=3)
        self.wait()
        ...
        self.play(LaggedStart(*(FadeIn(tile, scale=0.8) for tile in self.tiles)), run_time=5, lag_ratio=0.1)
        self.wait()
        icons = VGroup(*(SVGMobject(file) for file in ['teaching', 'gem', 'lightbulb', 'memory']))
        ...
        for title, icon in zip(titles, icons):
            ...
            title.icon = icon
            self.play(FadeIn(icon, lag_ratio=0.25), FlashUnder(title))
            title.add(icon)
        self.wait()
        self.play(LaggedStart(regions[:2].animate.stretch(1.4, 1, about_edge=UP), regions[2:].animate.stretch(0.6, 1, about_edge=DOWN), titles[:2].animate.shift(MED_SMALL_BUFF * DOWN), titles[2:].animate.shift(2 * DOWN), regions[0].tiles.animate.shift(DOWN), regions[1].tiles.animate.shift(DOWN), regions[2].tiles.animate.shift(1.4 * DOWN), regions[3].tiles.animate.shift(1.4 * DOWN), lag_ratio=0.025, run_time=2))
        self.wait()
        regions[0].save_state()
        titles[0].save_state()
        self.add(regions[0], regions[0].tiles)
        self.play(regions[0].animate.replace(FullScreenRectangle()).set_opacity(0), titles[0].animate.set_x(0), regions[0].tiles.animate.scale(2).center(), FadeOut(regions[1:]), FadeOut(titles[1:]), *(FadeOut(regions[i].tiles) for i in range(1, 4)))
        ...
        descriptions = VGroup(Text('Have you generated a\ndesire to learn?'), Text('Is each new idea given\na good reason to be there?'))
        descriptions.scale(0.8)
        for desc, st in zip(descriptions, subtitles):
            desc.next_to(st, DOWN, MED_LARGE_BUFF)
        self.play(Write(subregions), titles[0].animate.to_edge(UP, buff=SMALL_BUFF), regions[0].tiles[:7].animate.scale(0.9).arrange_in_grid(3, 3).move_to(subregions[0]).align_to(ORIGIN, UP), regions[0].tiles[7:].animate.scale(0.9).arrange_in_grid(2, 2).move_to(subregions[1]).align_to(ORIGIN, UP), LaggedStartMap(FadeIn, subtitles, shift=0.5 * DOWN))
        self.wait()
        ...
        self.play(frame.animate.scale(1.55).move_to(subregions[1]), FadeIn(titles[1]), FadeIn(c_region), FadeIn(regions[1].tiles))
        self.wait()
        c_region.save_state()
        self.play(c_region.animate.stretch(2, 0, about_edge=RIGHT))
        self.wait()
        randy = Randolph()
        ...
        bucket_arrow = Arrow(bucket_label.get_corner(DL) + 0.7 * RIGHT + 0.1 * DOWN, bucket.get_right(), color=YELLOW)
        focus_dots = Group(*(GlowDot([interpolate(bucket.get_x(LEFT) + 0.1, bucket.get_x(RIGHT) - 0.1, random.random()), interpolate(bucket.get_y(DOWN) + 0.1, bucket.get_y(TOP) - 0.1, random.random()), 0]) for x in range(40)))
        focus_dots.sort(lambda p: p[1])
        self.play(frame.animate.move_to(subregions[1], DOWN).shift(MED_SMALL_BUFF * DOWN), VFadeIn(randy), randy.change('tease'), FadeIn(bucket))
        self.play(FadeIn(bucket_label), ShowCreation(bucket_arrow), FadeIn(focus_dots, lag_ratio=0.5))
        self.wait()
        ...
        self.play(MoveToTarget(randy_copy), MoveToTarget(bucket_copy), focus_dots_copy.animate.move_to(bucket_copy.target).set_opacity(1))
        focus_dots_copy.sort(lambda p: -p[1])
        self.play(LaggedStartMap(FadeOut, focus_dots_copy, shift=0.2 * UP, scale=3, lag_ratio=0.4, run_time=6), randy_copy.change('erm', regions[1].tiles).set_anim_args(time_span=(4, 5)))
        self.wait()
        self.play(Blink(randy_copy))
        self.wait()
        self.play(titles[1].animate.center().to_edge(UP), regions[1].tiles.animate.scale(1.5).center(), c_region.animate.become(FullScreenRectangle(fill_color=BLACK, fill_opacity=0)), frame.animate.set_height(FRAME_HEIGHT).center(), *(FadeOut(mob, shift=LEFT, time_span=(0.05 * i, 0.05 * i + 1)) for i, mob in enumerate([randy, bucket, focus_dots, bucket_label, randy_copy, bucket_copy, titles[0], subregions, subtitles, descriptions, *regions[0].tiles])), run_time=2)
        ...
        self.play(FadeIn(regions), *map(FadeIn, (titles[0], titles[2], titles[3])), *(FadeIn(regions[i].tiles) for i in [0, 2, 3]), regions[1].tiles.animate.scale(regions[0].tiles[0].get_height() / regions[1].tiles[0].get_height()).move_to(regions[1]), titles[1].animate.restore())
        ...
        self.play(regions[2].animate.replace(FullScreenRectangle(), stretch=True).set_opacity(0), titles[2].animate.center().to_edge(UP), *map(FadeOut, [*titles[:2], titles[3], *regions[:2], regions[3], *self.tiles]))
        ...
        subtitles = VGroup(Text('Stylistic\noriginality'), Text('Content\noriginality'))
        ...
        self.play(LaggedStartMap(ShowCreation, lines, lag_ratio=0.5), LaggedStartMap(FadeInFromPoint, subtitles, point=titles[2].get_bottom(), lag_ratio=0.5))
        subtitles.resume_updating()
        self.wait()
        self.add(*rects, *subtitles)
        self.play(*map(FadeIn, rects), FadeOut(lines))
        self.play(rects[0].animate.set_width(rects[0].get_width() - 5, stretch=True, about_edge=LEFT), rects[1].animate.set_width(rects[1].get_width() + 5, stretch=True, about_edge=RIGHT))
        self.wait()
    ...
    def highlight_tile(self, index):
        ...
        self.play(bg.animate.restore(), MoveToTarget(tile), image.animate.replace(tile.target).set_opacity(1.0), Write(title, time_span=(0.5, 2.0)))
        self.play(FadeIn(author, 0.5 * DOWN))
        self.wait()
        tile.replace_submobject(1, image)
        self.play(tile.animate.replace(original_tile), *map(FadeOut, (bg, title, author)))
...
class WhoCares(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(morty.change('raise_right_hand', look_at=equation), Write(equation), self.change_students('pondering', 'pondering', 'pondering', look_at=equation))
        self.wait()
        self.play(self.change_students('sassy', 'confused', 'erm'))
        self.wait()
        self.play(stds[0].says('Why should\nI care?', mode='angry'), morty.change('guilty'))
        self.wait()
        self.play(morty.says('Well...hmmm...', mode='sassy', look_at=equation), stds[1].change('maybe'))
        self.wait(5)
...
class Overphilosophizing(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        rects = VGroup(*(Rectangle(width=w) for w in [2, 9, 2]))
        ...
        def left_shift(x, rects=rects, **kw):
            return (rects[0].animate.set_width(rects[0].get_width() + x, about_edge=LEFT, stretch=True).set_anim_args(**kw), rects[1].animate.set_width(rects[1].get_width() - x, about_edge=RIGHT, stretch=True).set_anim_args(**kw))

        def right_shift(x, rects=rects, **kw):
            return (rects[1].animate.set_width(rects[1].get_width() + x, about_edge=LEFT, stretch=True).set_anim_args(**kw), rects[2].animate.set_width(rects[2].get_width() - x, about_edge=RIGHT, stretch=True).set_anim_args(**kw))
        self.play(morty.change('guilty'), self.change_students('erm', 'erm', 'erm', look_at=rects))
        self.wait()
        self.play(*left_shift(2, run_time=3))
        self.play(self.change_students('tired', 'dejected', 'tired', look_at=rects))
        self.wait()
        self.play(morty.change('raise_right_hand'), self.change_students('thinking', 'thinking', 'thinking', look_at=rects), *left_shift(-3))
        self.wait()
        ...
        self.play(Write(exs), ShowCreation(arrow))
        self.wait(4)
...
class MusicSidenote(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(*(FadeIn(note, 0.2 * LEFT) for note in notes), lag_ratio=0.3), randy.change('thinking', notes))
        self.play(notes.animate.to_edge(RIGHT).set_anim_args(run_time=3), randy.change('tease'))
        ...
        self.play(ShowCreation(outline), randy.change('confused'))
        self.play(Blink(randy))
        self.wait(2)
        self.play(LaggedStart(*(note.copy().animate.set_opacity(0).scale(0.5).move_to(randy.get_top()) for note in notes), lag_ratio=0.2), randy.change('concentrating'))
        self.play(Blink(randy))
        self.wait()
...
class NotAsUnique(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.says("It's not as\nunique these days", mode='shruggie'), self.change_students('happy', 'tease', 'happy'))
        self.wait(3)
...
class Winners(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(title)
        winner_data = [('a-767WnbaCQ', 'Percolation: a Mathematical Phase Transition', 'Spectral Collective'), ('crystalgroups', 'Clear Crystal Conundrums', 'Ex Planaria'), ('5M2RWtD4EzI', "This Is the Calculus They Won't Teach You", 'A Well-Rested Dog'), ('gsZiJeaMO48', 'How Realistic CGI Works (And How To Do It Way Faster)', 'Joshua Maros'), ('6hVPNONm7xw', "The Coolest Hat Puzzle You've Probably Never Heard", 'Going Null')]
        ...
        mystery_tiles = Group(*(get_entry_tile().replace(winner[0]) for winner in winners))
        ...
        self.play(ShowCreation(first_outline), FadeIn(group_label, 0.2 * UP))
        self.wait()
        for tile, winner in zip(mystery_tiles, winners):
            self.play(FadeOut(tile, scale=0.5), FadeIn(winner[0], scale=2), FadeIn(winner[1], DOWN, time_span=(0.5, 1.5)))
            self.add(winner)
            self.wait()
        hm_slugs = ['v_HeaeUUOnc', 'piF6D6CQxUw', 'QC3CjBZLHXs', 'KufsL2VgELo', 'zR_hpai3XkY', 'HeBP3MG-WHg', '2dwQUUDt5Is', '3gyHKCDq1YA', 'nK2jYk37Rlg', 'l7bYY2U5ld8', 'dwNxVpbEVcc', 'CFBa2ezTQJQ', 'gnUYoQ1pwes', 'LUCvSsx6-EU', '-vxW42R47bc', 'inverse-turing-test', 'bezier-curve-guide', 'does-every-game-have-a-winner', 'Autodiff', 'panorama-homography']
        ...
        hm_tiles = Group(*(ImageMobject(os.path.join(DOC_DIR, 'thumbnails', slug)) for slug in hm_slugs))
        ...
        self.play(FadeOut(to_fade, run_time=2, lag_ratio=0.5), MoveToTarget(winner_tiles))
...
class Sponsors(InteractiveScene):
    ...
    def construct(self):
        ...
        contributions = VGroup(Text('$1,000 for each winner').next_to(brilliant, RIGHT, buff=2.0), Text('$500 for each\nhonorable mention').next_to(logos[1:3], RIGHT, buff=2.0), Text('Operations costs').next_to(pl, RIGHT, buff=2.0))
...
class EndScreen(PatreonEndScreen):
    CONFIG = {'title_text': 'Playlist of all entries', 'scroll_time': 30, 'thanks_words': 'Many additional thanks to these Patreon supporters'}