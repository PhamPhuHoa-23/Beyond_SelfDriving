...
class IntersectionAndUnion(InteractiveScene):
    ...
    def construct(self):
        ...
        circle_titles = VGroup(Text('People with\ngood lessons'), Text('People with the\ntime and know-how\nto produce videos,\ninteractives, etc.', alignment='LEFT'))
        ...
        for ct, circ, vect in zip(circle_titles, circles, [LEFT, RIGHT]):
            ...
            ct.match_color(circ)
        self.play(LaggedStartMap(Write, circle_titles, lag_ratio=0.25), LaggedStartMap(DrawBorderThenFill, circles, lag_ratio=0.25))
        self.wait()
        ...
        self.play(VGroup(by_ind, cross).animate.scale(0.5).shift(0.75 * DOWN), FadeIn(new_text, 0.5 * DOWN), MoveToTarget(title_start), FadeOut(inter), circles.animate.set_fill(opacity=0.7), ShowCreation(union))
        self.wait(3)
...
class WinnerCategories(InteractiveScene):
    ...
    def construct(self):
        ...
        cats = VGroup(Text('At least one video entry', **kw), Text('At least one non-video entry', **kw), Text('At least one collaboration', **kw))
...
class ComplainAboutGithub(TeacherStudentsScene):
    ...
    def construct(self):
        pis = self.students
        morty = self.teacher
        self.play(pis[1].says("Aren't GitHub issues\nmeant for, like, tracking\nbugs with code?", mode='sassy'), morty.change('guilty'), pis[0].change('confused'), pis[2].change('erm'))
        self.wait(2)
...
class Triumverate(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(prof_label, RIGHT), FadeIn(prof, RIGHT), morty.change('tease', prof.eyes))
        self.play(prof.change('pondering'))
        self.wait(2)
        self.play(FadeIn(artist, RIGHT), FadeIn(artist_label, RIGHT), morty.change('tease', artist.eyes))
        self.play(artist.change('hooray', morty.eyes), prof.change('thinking', artist.eyes))
        self.wait(2)
        ...
        self.play(Write(logo.iris_background, stroke_width=1), GrowFromCenter(logo.spike_layers, lag_ratio=0.1, run_time=2), Animation(logo.pupil), morty.change('thinking', logo), prof.change('pondering', logo), artist.change('tease', logo))
...
class Winners(InteractiveScene):
    ...
    def construct(self):
        ...
        for c, pi in zip(cash, winners):
            c.next_to(pi, DOWN)
        self.play(LaggedStart(*(pi.change('hooray').set_opacity(1) for pi in winners)), lag_ratio=0.2)
        self.wait()
        self.play(Write(brilliant_logo, run_time=1))
        self.play(LaggedStart(*(pi.animate.look(DOWN) for pi in winners)), LaggedStartMap(FadeIn, cash, shift=DOWN, lag_ratio=0.5))
