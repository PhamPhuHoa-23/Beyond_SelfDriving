...
class FlowerSymmetries(InteractiveScene):
    ...
    def construct(self):
        ...
        for angle in angles:
            ...
            self.play(Rotate(flower, angle * DEGREES), ShowCreation(arrow), FadeIn(label))
            self.wait()
    ...
    def get_arrow_and_label(self, flower, degrees):
        ...
        arc = Arc(start_angle=0, angle=degrees * DEGREES - 3 * DEGREES, radius=radius, arc_center=flower.get_center(), stroke_width=5, color=BLUE)
        arc.add_tip()
