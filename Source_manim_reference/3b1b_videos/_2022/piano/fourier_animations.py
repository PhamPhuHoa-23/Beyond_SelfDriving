...
def get_wave_sum(axes, freqs, amplitudes=None, phases=None):
    ...
    return axes.get_graph(lambda t: sum((amp * math.sin(TAU * freq * (t - phase)) for freq, amp, phase in zip(freqs, amplitudes, phases))))
...
class SumOfWaves(Scene):
    ...
    def construct(self):
        axes = Axes((0, 12), (-1, 1), height=2)
        base_freq = 0.5
        wave = get_wave_sum(axes, [base_freq])
        wave.set_stroke(BLUE, 2)
        x = 4.5
        brace = Brace(Line(axes.i2gp(x, wave), axes.i2gp(x + 1 / base_freq, wave)), UP, buff=SMALL_BUFF)
        brace_label = brace.get_text('220 cycles / sec.', buff=SMALL_BUFF, font_size=36)
        ...
        self.play(ShowCreation(wave, rate_func=linear), GrowFromCenter(brace, rate_func=brace_rf), Write(brace_label, rate_func=label_rf), run_time=3)
        ...
        self.play(FadeOut(VGroup(axes_labels[1], brace), DOWN), ReplacementTransform(brace_label, axes_labels[0]), ReplacementTransform(axes, low_axes_group[0]), ReplacementTransform(wave, waves[0]), *(TransformFromCopy(axes, low_axes) for low_axes in low_axes_group[1:]))
        self.play(LaggedStartMap(ShowCreation, waves[1:], lag_ratio=0.5, rate_func=linear), LaggedStartMap(FadeIn, axes_labels[1:], lag_ratio=0.5), run_time=2)
        self.wait()
        ...
        comp_wave = always_redraw(lambda: get_wave_sum(top_axes, freqs, amplitudes=amp_tracker.get_value()).set_stroke(TEAL, 2))
        self.play(FadeIn(top_rect), FadeIn(top_axes), FadeIn(sum_label), *(Transform(wave.deepcopy(), comp_wave, remover=True) for wave in waves))
        self.add(comp_wave)
        self.wait()
...
class DecomposeAudioSegment(Scene):
    ...
    sample_density = 1 / 5
    n_sine_waves = 5
    signal_graph_style = dict(stroke_color=BLUE, stroke_width=1)
    graph_point = 0.428
    ...
    def add_full_waveform(self, run_time=5):
        ...
        self.add(axes)
        self.play(ShowCreation(graph, rate_func=squish_rate_func(linear, 0.05, 1)), VShowPassingFlash(graph.copy().set_stroke(BLUE_B, 3), time_width=0.1, rate_func=linear), run_time=run_time)
        self.axes = axes
        self.graph = graph
    ...
    def zoom_in_on_segment(self, axes, graph, graph_point, zoom_rect_dims, run_time=4, fade_in_new_axes=True):
        ...
        self.play(*map(MoveToTarget, movers), FadeIn(new_axes), run_time=run_time)
    ...
    def break_down_into_fourier_components(self):
        ...
        f_axes = Axes((0, max_freq / 2, max_freq / len(signal) / 2), (0, 1, 1 / 8), height=t_axes.get_depth(), width=150)
        ...
        for index in range(1, 50):
            ...
            wave = t_axes.get_graph(lambda t: 2 * amp * np.cos(TAU * freq * (t + phase)), x_range=(0, t_max))
        ...
        self.play(FadeIn(f_axes), MoveToTarget(frame, run_time=8), LaggedStart(*(TransformFromCopy(graph, wave) for wave in top_waves), lag_ratio=0.8, run_time=3))
        frame.add_updater(lambda f, dt: f.increment_theta(0.25 * dt * DEGREES))
        self.play(Write(freq_label))
        self.wait(3)
        self.play(FadeIn(sine_waves, lag_ratio=0.1, run_time=3))
        self.wait(3)
        lines = VGroup(*(Line(f_axes.c2p(freqs[i], 0), f_axes.i2gp(freqs[i], fft_graph)) for i in range(1, len(sine_waves))))
        ...
        self.play(LaggedStart(*(TransformFromCopy(wave, line) for wave, line in zip(sine_waves, lines)), lag_ratio=0.1, run_time=8), graph.animate.set_stroke(width=1, opacity=0.5), ShowCreation(fft_graph, run_time=5), Write(fft_label), MoveToTarget(frame, run_time=5))
        self.wait(2)
        self.add(piano, freq_label, fft_graph, lines)
        self.play(Write(piano), frame.animate.set_phi(1.25), run_time=3)
        self.wait()
        glow_keys = VGroup(*(piano[np.argmin([get_norm(k.get_center() - wave.get_left()) for k in piano])] for wave in top_waves))
        peak_dots = GlowDots([lines[np.argmin([get_norm(line.get_start() - wave.get_left()) for line in lines])].get_end() for wave in top_waves])
        self.play(ShowCreation(peak_dots), LaggedStartMap(ShowCreation, top_waves), frame.animate.set_euler_angles(0.72, 1.15).move_to([2.0, 4.0, 1.0]), ApplyMethod(glow_keys.set_fill, RED, 1, rate_func=squish_rate_func(smooth, 0, 0.2)), run_time=6)
        self.wait()
        approx_wave = graph.copy()
        approx_wave.set_points_smoothly(graph.get_points()[::150])
        approx_wave.set_stroke(TEAL, 3, 1.0)
        self.play(frame.animate.reorient(0, 90).move_to(ORIGIN).set_height(10), graph.animate.set_stroke(width=2, opacity=0.5), *(ReplacementTransform(wave, approx_wave) for wave in top_waves), LaggedStartMap(FadeOut, VGroup(fft_graph, lines, fft_label, freq_label, f_axes)), FadeOut(peak_dots), FadeOut(wave_shadows), FadeOut(piano), run_time=3)
        self.wait()
        self.approx_wave = approx_wave

    def back_to_full_signal(self):
        self.play(FadeOut(self.axes), FadeOut(self.approx_wave), self.graph.animate.set_stroke(opacity=1))
        self.camera.frame.reorient(0, 0)
        self.graph.rotate(-PI / 2, RIGHT)
        self.play(Restore(self.original_axes), Restore(self.original_graph), Restore(self.graph), run_time=3)
    ...
    def get_signal_graph(self):
        ...
        axes = Axes((0, len(signal), sample_rate * self.sample_density), (-1, 1, 0.25), height=6, width=15)
        axes.to_edge(LEFT)
...
class SignalsAsVectors(DecomposeAudioSegment):
    ...
    def zoom_in_on_waveform(self):
        ...
        for point in graph.get_anchors():
            if not any(((point == p).all() for p in points)):
                points.append(point)
        axes = Axes((0, len(points) - 1), (-1, 1, 0.25), width=graph.get_width(), height=graph.get_height())
        ...
        graph = new_graph
        self.add(graph)
        self.play(ShowCreation(lines, lag_ratio=0.5), FadeIn(dots), graph.animate.set_stroke(width=1))
        self.wait()
    ...
    def describe_fourier_basis(self):
        ...
        self.play(frame.animate.move_to(axes.get_bottom() + MED_LARGE_BUFF * DOWN, DOWN), LaggedStart(Write(vector.get_brackets()), *(GrowFromPoint(mob, point) for point, mob in zip(dots.get_points()[:3], vector.get_entries()[:3])), Write(vector.get_entries()[3]), *(GrowFromPoint(mob, point) for point, mob in zip(dots.get_points()[-2:], vector.get_entries()[-2:]))), run_time=3)
        self.wait()
        signal_fft = np.fft.fft(values)
        fourier_basis = np.array([np.fft.ifft(basis) for basis in np.identity(values.size)])
        component_values = np.array([part * len(signal_fft) for coef, fb in zip(signal_fft, fourier_basis) for part in (fb.real, fb.imag)])
        xs = list(range(len(signal_fft)))
        component_graphs = VGroup(*(self.get_graph_with_lines_and_dots(axes, axes.c2p(xs, vals))[0] for vals in component_values))
        ...
        self.play(graph_group.animate.set_width(graph_group.get_width() - comp_groups.get_width() - LARGE_BUFF, about_edge=DL), vector.animate.to_edge(LEFT, buff=MED_LARGE_BUFF), LaggedStartMap(FadeIn, comp_groups))
        self.wait()
        c_dots_group = Group()
        ...
        for sym, coef_sym, c_vect in zip(syms, coef_syms, c_vectors):
            ...
            last_vect = c_vect
        for i in range(3):
            self.play(ShowCreation(c_dots_group[i]), ShowCreation(c_lines_group[i]))
            self.play(FadeIn(syms[i]), Write(coef_syms[i]), FadeTransform(c_dots_group[i].copy(), c_vectors[i]))
        self.play(*map(ShowCreation, c_dots_group[3:]), *map(ShowCreation, c_lines_group[3:]), Write(syms[3]), Write(coef_syms[3]))
        self.wait()
        for comp, dots, lines in zip(comp_groups, c_dots_group, c_lines_group):
            c_axes, c_graph = comp
            stretcher = Group(c_graph, dots, lines)
            self.play(stretcher.animate.stretch(random.uniform(-1, 1), dim=1, about_point=c_axes.get_center()))
        self.wait()
...
class SampleRateOverlay(Scene):
    ...
    def construct(self):
        text = VGroup(OldTexText('48,000 samples / sec'), OldTex('\\Downarrow'), OldTexText('20 ms window', ' = 960-dimensional vector'))
        text.arrange(DOWN)
        text.to_edge(UP)
...
class ThreeDChangeOfBasisExample(Scene):
    ...
    def construct(self):
        ...
        frame.shift(OUT)
        frame.add_updater(lambda m: m.set_theta(-20 * math.cos(TAU * self.time / 60) * DEGREES))
        axes = ThreeDAxes(axis_config=dict(tick_size=0.05))
        axes.set_stroke(width=1)
        plane = NumberPlane(faded_line_ratio=0)
        plane.set_stroke(GREY, 1, 0.5)
        basis_mobs = VGroup(Vector(RIGHT, color=RED), Vector(UP, color=GREEN), Vector(OUT, color=BLUE))
    ...
    def get_coef_label(self, vect_mob, basis_mobs, lhs_tex='\\vec{\\textbf{s}}'):
        ...
        label.basis_labels = VGroup(*(OldTex('\\vec{\\textbf{v}}_1', color=RED), OldTex('\\vec{\\textbf{v}}_2', color=GREEN), OldTex('\\vec{\\textbf{v}}_3', color=BLUE)))
        label.add(lhs)
