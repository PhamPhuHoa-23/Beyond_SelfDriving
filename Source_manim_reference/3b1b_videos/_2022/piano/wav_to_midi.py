...
Note = namedtuple('Note', ['value', 'velocity', 'position', 'duration'])
...
def add_notes(track, notes, sec_per_tick):
    ...
    for index, note in enumerate(notes):
        ...
        track.append(mido.Message('note_on', note=int(note.value), velocity=int(note.velocity), time=pos_in_ticks - curr_tick))
        curr_tick = pos_in_ticks
        track.append(mido.Message('note_off', note=int(note.value), time=dur_in_ticks))
...
def midi_to_wav(mid_file):
    ...
    if os.path.exists(wav_file):
        os.remove(wav_file)
    os.system(' '.join(['timidity', mid_file, '-Ow -o -', '|', 'ffmpeg', '-i - -acodec libmp3lame -ab 64k', '-ar 48000', '-hide_banner -loglevel error', mp3_file]))
    os.system(' '.join(['ffmpeg', '-hide_banner -loglevel error', '-i', mp3_file, wav_file]))
    os.remove(mp3_file)
    return wav_file
...
def generate_piano_sample_midi(delay=5, duration=0.025, velocity=100):
    ...
    for n, key in enumerate(piano_midi_range):
        notes.append(Note(key, velocity=velocity, position=n * delay, duration=duration))
...
def load_piano_key_signals(folder='piano_samples', duration=0.5, velocity=50):
    key_signals = []
    for key in piano_midi_range:
        sample_rate, full_signal = wav_data(os.path.join(DATA_DIR, folder, f'{key}_{velocity}.wav'))
...
def get_volume_to_velocity_func_map(folder='true_piano_samples', sampled_velocities=[26, 101]):
    ...
    for key in piano_midi_range:
        volumes = [wav_data(os.path.join(DATA_DIR, folder, f'{key}_{vel}.wav'))[1].max() for vel in vels]
...
def wav_to_midi(sound_file, output_file=None, key_block_time=0.075, key_signal_time=0.075, key_play_time=0.02, step_size=0.001, sample_velocity=100, sample_folder='digital_piano_samples', key_signal_max=0.15, volume_ratio_threshold=1.0, scale_factor_cutoff=0.1, min_velocity=5, max_velocity=60, n_supressed_lower_keys=32, supression_factor=0.25):
    ...
    notes = []
    key_signals = load_piano_key_signals(folder=sample_folder, duration=key_signal_time, velocity=sample_velocity)
    key_signals *= key_signal_max / key_signals.max()
    velocities = []
    full_convs = np.array([fftconvolve(signal, ks[::-1])[len(ks) - 1:] for ks in key_signals])
    ...
    for pos in positions:
        ...
        diff = (signal - new_signal)[opt_pos:opt_pos + len(ks)]
        ...
        if factor > scale_factor_cutoff:
            ...
            notes.append(Note(value=piano_midi_range[key_index], velocity=velocity, position=opt_pos / sample_rate, duration=key_play_time))
...
def extract_pure_keys(key_sample_file, output_folder='true_piano_samples', start=3.9, spacing=3, velocities=[1, 26, 51, 76, 101, 126]):
    ...
    for key in piano_midi_range:
        for vel in velocities:
            wavfile.write(filename=os.path.join(output_folder, f'{key}_{vel}.wav'), rate=sample_rate, data=arr[index:index + spacing_in_samples])
...
def test_midi_file_writing():
    notes = [Note(value=hz_to_midi_value(240 * 2 ** (5 * x % 12 / 12)), velocity=random.randint(20, 64), position=x / 120, duration=1 / 120) for x in range(64) for y in range(10)]
    test_file = os.path.join(DATA_DIR, 'test.mid')
    create_midi_file_with_notes(test_file, notes)
