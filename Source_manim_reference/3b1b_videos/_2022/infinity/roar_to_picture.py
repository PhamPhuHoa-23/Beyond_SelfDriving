...
class DecomposeRoar_SlowFPS(DecomposeAudioSegment):
    ...
    def construct(self):
        ...
        self.zoom_in_on_segment(self.axes, self.graph, self.graph_point, self.zoom_rect_dims, run_time=3)
        self.prepare_for_3d()
        self.show_all_waves()
    ...
    def show_all_waves(self):
        ...
        self.play(FadeIn(f_axes), MoveToTarget(frame), LaggedStart(*(ReplacementTransform(graph.copy().set_opacity(0), wave) for wave in sine_waves), lag_ratio=1 / len(sine_waves)), run_time=4)
        self.wait(3)

class CombineWavesToImage_SlowFPS(FourierCirclesScene):
    CONFIG = {'drawing_height': 6.0, 'n_shown_waves': 5, 'n_vectors': 500, 'slow_factor': 0.1, 'max_circle_stroke_width': 2.0, 'parametric_function_step_size': 0.001, 'drawn_path_color': YELLOW, 'remove_background_waves': False, 'drawing_file': '/Users/grant/Dropbox/3Blue1Brown/videos/2022/infinity/monster-holding-piece-sign-3.svg'}
    ...
    def show_sine_waves(self):
        ...
        all_axes = VGroup(*(Axes(x_range=(0, 8 * PI, PI), y_range=(-max_y, max_y), height=1.0, width=18) for n in range(self.n_shown_waves)))
        ...
        for vect, axes, color in zip(shown_vectors, all_axes, colors):
            ...
            axes.vect = vect
            axes.color = color
            axes.graph = always_redraw(lambda a=axes: a.get_graph(lambda x: a.amp * math.sin(a.vect.freq * x + a.vect.get_angle()), stroke_width=2, stroke_color=a.color))
    ...
    def piece_together_circles(self):
        ...
        self.play(FadeIn(fade_region, lag_ratio=0.1), LaggedStart(*(TransformFromCopy(v1, v2) for v1, v2 in zip(self.shown_vectors, self.vectors[1:])), lag_ratio=0.1), LaggedStart(*(TransformFromCopy(c1, c2) for c1, c2 in zip(self.shown_circles, self.circles[1:])), lag_ratio=0.1), FadeIn(label), Write(terms), run_time=rt)
        ...
        anims = [UpdateFromAlphaFunc(drawn_path, lambda m, a: m.set_stroke(interpolate_color(BLACK, YELLOW, a))), UpdateFromAlphaFunc(dot, lambda m, a: m.set_opacity(a))]
        if self.remove_background_waves:
            self.play(fade_region.animate.set_height(2 * FRAME_WIDTH).set_opacity(0.05), *anims, run_time=2)
            self.remove(self.all_axes, self.graphs, self.shown_vectors, self.shown_circles, self.v2g_lines, self.dots, fade_region)
        else:
            self.play(*anims, run_time=2)
        drawn_paths = dict()
        all_terms = dict()
        drawing_group = VGroup(drawn_path, terms, label, VGroup(), VGroup())
        ...
        self.play(UpdateFromAlphaFunc(drawing_group, update_drawing_group, run_time=15, rate_func=lambda a: a ** 2))
        inf = OldTex('\\infty')
        ...
        self.play(UpdateFromAlphaFunc(drawn_path, lambda m, a: m.set_stroke(interpolate_color(YELLOW, BLACK, a))), VFadeIn(true_path), ChangeDecimalToValue(label[0], 1000), VFadeOut(label[0]), FadeIn(inf), run_time=3)
    ...
    def get_peace_sign(self):
        ...
        parts = [circ, Line(UP, ORIGIN), Line(ORIGIN, DOWN), arc1, Line(arc1.get_end(), ORIGIN), Line(ORIGIN, arc2.get_start()), arc2]
        path = VMobject()
        for part in parts:
