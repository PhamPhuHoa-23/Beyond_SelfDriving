...
def get_pattern_matrix(words1, words2):
    if not PATTERN_GRID_DATA:
        if not os.path.exists(PATTERN_MATRIX_FILE):
            log.info('\n'.join(['Generating pattern matrix. This takes a minute, but', 'the result will be saved to file so that it only', 'needs to be computed once.']))
...
def words_to_max_buckets(possible_words, weights):
    return dict(((word, max_bucket_size(word, possible_words, weights)) for word in ProgressDisplay(possible_words)))
...
def get_average_second_step_entropies(first_guesses, allowed_second_guesses, possible_words, priors):
    ...
    for first_guess, dist in ProgressDisplay(list(zip(first_guesses, distributions)), leave=False, desc='Searching 2nd step entropies'):
        ...
        ents2 = np.array([get_entropies(allowed_words=allowed_second_guesses, possible_words=bucket, weights=get_weights(bucket, priors)).max() for bucket in word_buckets])
...
def get_guess_values_array(allowed_words, possible_words, priors, look_two_ahead=False):
    ...
    if look_two_ahead:
        ...
        ents2[top_indices] = get_average_second_step_entropies(first_guesses=np.array(allowed_words)[top_indices], allowed_second_guesses=allowed_words, possible_words=possible_words, priors=priors)
...
def get_expected_scores(allowed_words, possible_words, priors, look_two_ahead=False, n_top_candidates_for_two_step=25):
    weights = get_weights(possible_words, priors)
    ...
    for i in ProgressDisplay(sorted_indices[:n_top_candidates_for_two_step], leave=False):
        ...
        H2s = [get_entropies([guess2], bucket, get_weights(bucket, priors))[0] for guess2, bucket in zip(second_guesses, buckets)]
        prob = word_to_weight.get(guess, 0)
        expected_scores[i] = sum((1 * prob, 2 * (1 - prob) * sum((p * word_to_weight.get(g2, 0) for p, g2 in zip(dist, second_guesses))), (1 - prob) * (2 + sum((p * (1 - word_to_weight.get(g2, 0)) * entropy_to_expected_score(H0 - H1 - H2) for p, g2, H2 in zip(dist, second_guesses, H2s))))))
    return expected_scores
...
def optimal_guess(allowed_words, possible_words, priors, look_two_ahead=False, optimize_for_uniform_distribution=False, purely_maximize_information=False):
...
def brute_force_optimal_guess(all_words, possible_words, priors, n_top_picks=10, display_progress=False):
    ...
    true_average_scores = []
    if display_progress:
        iterable = ProgressDisplay(top_choices, desc=f'Possibilities: {len(possible_words)}', leave=False)
    else:
...
def find_top_scorers(n_top_candidates=100, quiet=True, file_ext='', **kwargs):
    ...
    for row in ProgressDisplay(double_ents[:n_top_candidates]):
        first_guess = row[0]
        result, decision_map = simulate_games(first_guess, priors=priors, optimize_for_uniform_distribution=True, quiet=quiet, **kwargs)
    ...
    file = os.path.join(get_directories()['data'], 'wordle', 'best_scores' + file_ext + '.json')
...
def find_best_two_step_entropy():
    ...
    ents2 = get_average_second_step_entropies(top_candidates, words, answers, priors)
...
def find_smallest_second_guess_buckets(n_top_picks=100):
    ...
    for j in np.argsort(avg_ts_buckets):
        i = top_indices[j]
        result.append((all_words[i], len(possibilities) * (dists[i] ** 2).sum(), avg_ts_buckets[j]))
    return result
...
def get_optimal_second_guess_map(first_guess, n_top_picks=10, regenerate=False):
    ...
    log.info('\n'.join([f'Generating optimal second guess map for {first_guess}.', 'This involves brute forcing many simulations', 'so can take a little while.']))
    sgm = [''] * 3 ** 5
    all_words = get_word_list()
...
def simulate_games(first_guess=None, priors=None, look_two_ahead=False, optimize_for_uniform_distribution=False, second_guess_map=None, exclude_seen_words=False, test_set=None, shuffle=False, hard_mode=False, purely_maximize_information=False, brute_force_optimize=False, brute_force_depth=10, results_file=None, next_guess_map_file=None, quiet=False):
    ...
    def get_next_guess(guesses, patterns, possibilities):
        phash = ''.join((str(g) + ''.join(map(str, pattern_to_int_list(p))) for g, p in zip(guesses, patterns)))
        ...
        if phash not in next_guess_map:
            ...
            if brute_force_optimize:
                next_guess_map[phash] = brute_force_optimal_guess(choices, possibilities, priors, n_top_picks=brute_force_depth)
            else:
                next_guess_map[phash] = optimal_guess(choices, possibilities, priors, look_two_ahead=look_two_ahead, purely_maximize_information=purely_maximize_information, optimize_for_uniform_distribution=optimize_for_uniform_distribution)
    ...
    for answer in ProgressDisplay(test_set, leave=False, desc=' Trying all wordle answers'):
        ...
        average = scores.mean()
        seen.add(answer)
        game_results.append(dict(score=int(score), answer=answer, guesses=guesses, patterns=list(map(int, patterns)), reductions=possibility_counts))
        if not quiet:
            message = '\n'.join(['', f'Score: {score}', f'Answer: {answer}', f'Guesses: {guesses}', f'Reductions: {possibility_counts}', *patterns_to_string((*patterns, 3 ** 5 - 1)).split('\n'), *' ' * (6 - len(patterns)), f'Distribution: {score_dist}', f'Total guesses: {total_guesses}', f'Average: {average}', *' ' * 2])
            if answer is not test_set[0]:
                ...
                print('\r\x1b[K\n')
            print(message)
    final_result = dict(score_distribution=score_dist, total_guesses=int(total_guesses), average_score=float(scores.mean()), game_results=game_results)
...
if __name__ == '__main__':
    first_guess = 'salet'
    results, decision_map = simulate_games(first_guess=first_guess, priors=get_true_wordle_prior(), optimize_for_uniform_distribution=True)