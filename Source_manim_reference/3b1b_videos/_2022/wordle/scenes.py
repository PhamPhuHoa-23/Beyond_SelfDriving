...
class WordleScene(Scene):
    ...
    grid_center = ORIGIN
    secret_word = None
    color_map = {0: '#797C7E', 1: '#C6B566', 2: GREEN_D}
    uniform_prior = False
    ...
    def shake_word_out(self):
        ...
        self.play(UpdateFromAlphaFunc(VGroup(row, self.grid.pending_word), lambda m, a: m.move_to(c + func(a) * RIGHT), run_time=0.5))
    ...
    def animate_color_change(self, row, word, colors, added_anims=[]):
        ...
        def alpha_func(mob, alpha):
            ...
            mob.set_height(mob.initial_height * max(abs(interpolate(1, -1, alpha)), 1e-06), stretch=True)
            if isinstance(mob, Square) and alpha > 0.5:
                mob.set_fill(mob.future_color, 1)
        mobjects = self.mobjects
        self.play(*(LaggedStart(*(UpdateFromAlphaFunc(sm, alpha_func) for sm in mob), lag_ratio=self.reveal_lag_ratio, run_time=self.reveal_run_time) for mob in (row, word)), *added_anims)
        self.clear()
        self.add(*mobjects)
    ...
    def win_animation(self):
        ...
        mover = VGroup(*(VGroup(square, letter) for square, letter in zip(row, letters)))
        y = row.get_y()
        bf = bezier([0, 0, 1, 1, -1, -1, 0, 0])
        self.play(LaggedStart(*(UpdateFromAlphaFunc(sm, lambda m, a: m.set_y(y + 0.2 * bf(a))) for sm in mover), lag_ratio=0.1, run_time=1.5), LaggedStart(*(Flash(letter, line_length=0.1, flash_radius=0.4) for letter in letters), lag_ratio=0.3, run_time=1.5))
...
class WordleSceneWithAnalysis(WordleScene):
    ...
    def add_guess_value_grid_title(self):
        titles = VGroup(Text('Top picks'), Text('E[Info.]', color=self.entropy_color))
    ...
    def add_count_title(self):
        title = VGroup(Text('# Possibilities'), Text('/'), Text('Uncertainty', color=self.entropy_color))
    ...
    def get_count_label(self):
        ...
        label = VGroup(Integer(len(self.possibilities), edge_to_fix=UR), Text('Pos,'), DecimalNumber(self.get_current_entropy(), edge_to_fix=UR, color=self.entropy_color), Text('Bits', color=self.entropy_color))
        label.arrange(RIGHT, buff=MED_SMALL_BUFF, aligned_edge=UP)
        label.scale(0.6)
    ...
    def reveal_pattern(self, pattern=None, animate=True):
        ...
        if is_valid_guess and (not self.has_won()):
            ...
            self.show_guess_values()
        if self.has_won():
            self.play(FadeOut(self.guess_value_grid, RIGHT), FadeOut(self.guess_value_grid_titles, RIGHT))
    ...
    def show_pattern_information(self, guess, pattern, new_possibilities):
        weights = get_weights(self.possibilities, self.priors)
        prob = sum((weight for word, weight in zip(self.possibilities, weights) if word in new_possibilities))
        info = -math.log2(prob)
    ...
    def isolate_guessed_row(self):
        ...
        if guess in row_words:
            ...
            new_row = self.get_guess_value_row(self.guess_value_grid_lines[0], guess)
            rows.shift(DOWN)
            rows.add(new_row)

    def get_shown_words(self, font_size=24):
        return self.get_grid_of_words(self.possibilities, n_rows=20 - 2 * len(self.grid.words), n_cols=1)

    def get_probability_bars(self, shown_words, max_width=1.0):
        mobs = shown_words
        words = shown_words.words
        probs = [self.priors.get(w, 0) for w in words]
        height = mobs[0].get_height() * 0.7
        bars = VGroup(*(Rectangle(width=prob * max_width, height=height, fill_color=self.prior_color, fill_opacity=0.7, stroke_width=0.5 * (prob > 0), stroke_color=self.prior_color) for prob in probs))
    ...
    def show_possible_words(self):
        ...
        if len(self.grid.words) > 0:
            ...
            count_label.shift((prev_count_label[1].get_right() - count_label[1].get_right())[0] * RIGHT)
            num_rate_func = squish_rate_func(rush_into, 0.3, 1)

            def update_moving_count_label(label, alpha):
                for i in (0, 2):
                    label[i].set_value(interpolate(prev_count_label[i].get_value(), count_label[i].get_value(), num_rate_func(alpha)))
                label.set_y(interpolate(prev_count_label.get_y(), count_label.get_y(), alpha))
                return label
            label_transition = UpdateFromAlphaFunc(prev_count_label.copy(), update_moving_count_label, remover=True)
            ...
            self.play(FadeOut(self.prob_bars, run_time=0.25), FadeOut(self.guess_value_grid, RIGHT), label_transition, MoveToTarget(prev_words, run_time=0.5), LaggedStartMap(Restore, shown_words, run_time=1), LaggedStartMap(Restore, prob_bars, run_time=1), run_time=1)
    ...
    def get_guess_value_grid(self, font_size=36):
        if self.pre_computed_first_guesses and len(self.grid.words) == 0:
            ...
            expected_scores = get_expected_scores(guesses, self.possibilities, self.priors, look_two_ahead=self.look_two_ahead)
            top_indices = np.argsort(expected_scores)[:self.n_top_picks]
        guess_values_array = get_guess_values_array(guesses, self.possibilities, self.priors, look_two_ahead=self.look_two_ahead)
        ...
        guess_value_grid = VGroup(*(self.get_guess_value_row(line, word, *values) for line, word, values in zip(lines, top_words, top_guess_value_parts.T)))
    ...
    def get_guess_value_row(self, line, word, entropy=None, entropy2=None, probability=None, font_size=36):
        titles = self.guess_value_grid_titles
        row = VGroup()
        ...
        if self.look_two_ahead:
            if entropy2 is None:
                entropy2 = get_average_second_step_entropies([word], self.all_words, self.possibilities, self.priors)
    ...
    def get_column_of_numbers(self, values, row_refs, col_ref, num_decimal_places=2, font_size=36):
        mobs = VGroup(*(DecimalNumber(value, num_decimal_places=num_decimal_places, font_size=font_size) for value in values))
...
class WordleDistributions(WordleScene):
    ...
    grid_height = 5
    bar_style = dict(fill_color=TEAL_D, fill_opacity=0.8, stroke_color=WHITE, stroke_width=0.1)
    show_fraction_in_p_label = True

    def get_axes(self, x_max=3 ** 5 / 2, y_max=0.1, width=7.5, height=6):
        axes = Axes((0, x_max), (0, y_max, y_max / 5), height=height, width=width, x_axis_config={'tick_size': 0})
    ...
    def get_total_words_label(self, font_size=36):
        label = VGroup(Integer(len(self.all_words), font_size=font_size, edge_to_fix=UR), Text('Total words', font_size=font_size))
    ...
    def get_dynamic_match_label(self, font_size=36):
        label = VGroup(Integer(len(self.possibilities), font_size=font_size, edge_to_fix=UR), Text('Possible matches', font_size=font_size))
    ...
    def get_distribution_bars(self, axes, guess):
        distribution = get_pattern_distributions([guess], self.possibilities, get_weights(self.possibilities, self.priors))[0]
    ...
    def get_bar_indicator(self, bars, pattern_index):
        ...
        pattern_index_tracker.add_updater(lambda m: m.set_value(clip(self.axes.x_axis.p2n(self.mouse_drag_point.get_center()), 0, 3 ** 5)))
    ...
    def get_information_label(self, p_label, get_bar):
        info_label = self.get_dynamic_bar_label(('I\\left(', '00000', '\\right)', '=', '\\log_2\\left(1 / p)', '=', '0.00'))
    ...
    def get_entropy_label(self, font_size=36):
        ...
        if self.is_valid_guess():
            entropy = get_entropies([guess], self.possibilities, get_weights(self.possibilities, self.priors))[0]
        else:
            entropy = 0
        lhs = OldTex('E[I] = \\sum_x ', 'p(x) \\cdot \\log_2(1 / p(x))', '=', tex_to_color_map={'I': BLUE}, font_size=font_size)
    ...
    def get_grid_of_matches(self, n_rows=20, n_cols=9):
        if self.grid.pending_pattern is not None:
            buckets = get_word_buckets(self.pending_word_as_string(), self.possibilities)
...
class LessonTitleCard(Scene):
    ...
    def construct(self):
        title = VGroup(Text('Lesson today:', font_size=40), Text('Information theory', color=TEAL))
        title.arrange(DOWN)
        title.scale(1.5)
        title.center()
...
class DrawPhone(Scene):
    ...
    def construct(self):
        ...
        bubble.add_content(WordleScene.patterns_to_squares(list(map(pattern_from_string, ['00102', '00110', '22222']))))
        bubble.content.scale(0.7)
        self.add(morty)
        self.add(phone)
        self.play(LaggedStart(morty.change('thinking', phone), ShowCreation(bubble), FadeIn(bubble.content, lag_ratio=0.1)), Write(phone, run_time=3, lag_ratio=0.01))
        self.play(Blink(morty), FadeOut(phone))
        self.wait()

class AskWhatWorldeIs(TeacherStudentsScene):

    def construct(self):
        self.student_says('What is Wordle?', index=0, target_mode='raise_left_hand', added_anims=[self.teacher.change('tease')])
        self.play_student_changes('raise_left_hand', 'hesitant', 'happy', look_at=self.students[0].bubble)
        self.wait(3)
...
class IntroduceGame(WordleScene):
    ...
    def construct(self):
        grid = self.grid
        row_copy = self.get_curr_row().copy()
        secret = VGroup(Text('Secret word'), Vector(0.5 * DOWN), row_copy)
        ...
        self.wait()
        self.wait()
        self.play(Write(secret[0]), ShowCreation(secret[1]), TransformFromCopy(grid[0], secret[2]))
        self.play(ShowSubmobjectsOneByOne(words, run_time=10, rate_func=linear))
        self.wait()
        ...
        titles = VGroup(VGroup(Text('Allowed guesses'), Integer(len(all_words))), VGroup(Text('Possible answers'), Integer(len(answers))))
        for title in titles:
            title.arrange(DOWN)
        titles.scale(0.8)
        ...
        word_columns = VGroup(self.get_grid_of_words(all_words, 20, 5), self.get_grid_of_words(answers, 20, 1))
        word_columns[1].set_color(GREEN)
        for column, title in zip(word_columns, titles):
            column.next_to(title, DOWN, MED_LARGE_BUFF)
        grid.add(numbers)
        self.play(FadeOut(secret), FadeOut(self.q_marks), grid.animate.scale(0.7, about_edge=LEFT), Write(titles[0][0], run_time=1))
        self.play(CountInFrom(titles[0][1], 0), ShowIncreasingSubsets(word_columns[0]))
        self.wait()
        self.play(FadeIn(titles[1][0]), CountInFrom(titles[1][1], 0), ShowIncreasingSubsets(word_columns[1]))
        self.wait()
        ...
        self.play(frame.animate.match_x(word_columns, LEFT).shift(LEFT), ShowCreation(answer_rect), run_time=2)
        self.play(Write(avoid), Restore(morty))
        self.wait()
        ...
        non_s_most_common = self.get_grid_of_words(list(filter(lambda w: w[-1] != 's', sorted_by_freq))[-n:], n, 1)
        ...
        self.play(FadeOut(avoid, DOWN), morty.change('raise_LEFT_hand', most_common), ShowIncreasingSubsets(most_common))
        self.play(FadeIn(label))
        self.play(Blink(morty))
        self.wait()
        self.play(morty.change('pondering', most_common), LaggedStartMap(FadeOut, most_common, shift=RIGHT), LaggedStartMap(FadeIn, non_s_most_common, shift=RIGHT))
        self.wait()
    ...
    def show_pattern(self, *args, **kwargs):
        ...
        if guess == self.secret_word:
            added_anims.append(LaggedStart(*(square.animate.set_fill(GREEN, 1) for square in self.row_copy), lag_ratio=0.7, run_time=2))
            added_anims.append(LaggedStart(*(Transform(q_mark, letter) for q_mark, letter in zip(self.q_marks, letters)), lag_ratio=0.7, run_time=2))
        super().show_pattern(*args, added_anims=added_anims, **kwargs)

class InitialDemo(ExternalPatternEntry):
    secret_word = 'elder'
    pre_computed_first_guesses = ['crane', 'slane', 'slate', 'salet', 'trace', 'reast', 'crate', 'toile', 'torse', 'carse', 'carle', 'trone', 'carte', 'roast']
...
class ShowTonsOfWords(Scene):
    ...
    def construct(self):
        ...
        n_cols = 15
        N = n_rows * n_cols
        grids = VGroup(*(WordleScene.get_grid_of_words(words[N * k:N * (k + 1)], n_rows, n_cols) for k in range(5)))
...
class ChoosingBasedOnLetterFrequencies(IntroduceGame):
    ...
    def construct(self):
        ...
        self.add(grid)
        letters_and_frequencies = [('E', 13), ('T', 9.1), ('A', 8.2), ('O', 7.5), ('I', 7), ('N', 6.7), ('S', 6.3), ('H', 6.1), ('R', 6), ('D', 4.3), ('L', 4), ('U', 2.8), ('C', 2.8), ('M', 2.5), ('W', 2.4), ('F', 2.2), ('G', 2.0), ('Y', 2.0)]
        freq_data = VGroup(*(VGroup(Text(letter, font='Consolas'), Rectangle(height=0.25, width=0.2 * freq, stroke_width=0, fill_color=BLUE if letter in 'AEIOUY' else GREY_B, fill_opacity=1), DecimalNumber(freq, num_decimal_places=1, font_size=24, unit='\\%')).arrange(RIGHT) for letter, freq in letters_and_frequencies))
...
class PreviewGamePlay(WordleSceneWithAnalysis):
    n_games = 10
    pre_computed_first_guesses = ['tares', 'lares', 'rates', 'rales', 'tears', 'tales', 'salet', 'teras', 'arles', 'nares', 'soare', 'saner', 'reals']
...
class IntroduceDistribution(WordleDistributions):
    ...
    def construct(self):
        ...
        for i, pattern_str in enumerate(pattern_strs):
            ...
            match_label.update()
            self.play(CountInFrom(match_label[0], 0), ShowIncreasingSubsets(word_grid, run_time=2))
            self.wait(note=f'Pattern {i} / {len(pattern_strs)}')
            num = match_label[0].get_value()
            denom = total_label[0].get_value()
            prob_label = self.get_dynamic_bar_label(('p\\left(', '0000', '\\right)', '=', '{' + '{:,}'.format(num).replace(',', '{,}'), '\\over ', '{:,}'.format(denom).replace(',', '{,}') + '}', '=', '0.0000'))
            prob_label[-1].set_value(num / denom)
            prob_label.next_to(word_grid, UP)
            prob_label.clear_updaters()
            self.play(LaggedStart(FadeTransform(match_label[0].copy().clear_updaters(), prob_label[4], remover=True), FadeTransform(total_label[0].copy().clear_updaters(), prob_label[6], remover=True), lag_ratio=0.5), FadeIn(VGroup(*prob_label[:4], prob_label[5], prob_label[7:])))
            self.add(prob_label)
            self.wait()
        ...
        standin = OldTex('E[\\text{Information}] = \\sum_{x} p(x) \\cdot (\\text{Something})', tex_to_color_map={'\\text{Something}': GREY_B, '\\text{Information}': BLUE}, font_size=36)
        ...
        self.play(FadeIn(entropy_definition, UP), FadeOut(standin, UP), FadeOut(want, UP))
        ...
        self.add_word('?????')
        self.get_curr_row().set_fill(BLACK, 0)
        eqs = VGroup(OldTex('E[I] = \\log_2\\left({1 \\over 1 / 3^5}\\right) = \\log_2(3^5) \\approx 7.92'), OldTex('E[I] = \\log_2\\left({1 \\over 1 / 16}\\right) = \\log_2(16) = 4.00'), OldTex('E[I] = \\log_2\\left({1 \\over 1 / 64}\\right) = \\log_2(64) = 6.00'))
        eqs.scale(0.7)
        for eq in eqs:
        ...
        self.wait()
        self.play(ApplyMethod(match_label[0].copy().move_to, total_label[0], UR, remover=True), FadeOut(total_label[0], UP))
        ...
        self.play(LaggedStart(*map(FadeOut, faders)), FadeOut(entropy_definition[-1]))
        self.grid.clear_updaters()
    ...
    def set_bars_to_values(self, bars, values, ent_rhs, run_time=3, added_anims=[]):
        ...
        for bar, value in zip(bars.target, values):
            ...
            bar.become(target)
        self.play(MoveToTarget(bars, run_time=run_time), ChangeDecimalToValue(ent_rhs, entropy, run_time=run_time), *added_anims)

    def recalculate_entropy(self, entropy_definition, guess, x_tracker=None):
        dec = entropy_definition[-1]
        dec.set_value(get_entropies([guess], self.possibilities, get_weights(self.possibilities, self.priors))[0])
        dec.set_opacity(1)
        ...
        if x_tracker is not None:
            ...
            anims.append(UpdateFromAlphaFunc(x_tracker, lambda m, a: m.set_value(a * 200), run_time=5))
            run_time = 3
...
class ButTheyreNotEquallyLikely(Scene):
    ...
    def construct(self):
        ...
        self.play(PiCreatureSays(randy, OldTexText("But they're \\emph{not}\\\\equally likely!"), target_mode='angry'), morty.change('guilty', randy.eyes))
        self.play(Blink(randy))
        self.wait()
        self.play(PiCreatureSays(morty, OldTexText("To warm up, let's\\\\assume they are."), target_mode='speaking'), RemovePiCreatureBubble(randy))
        self.play(Blink(morty))
        self.wait()
...
class ExpectedMatchesInsert(Scene):
    ...
    def construct(self):
        tex = OldTex('\\sum_{x} p(x) \\big(\\text{\\# Matches}\\big)', tex_to_color_map={'\\text{\\# Matches}': GREEN})
...
class DescribeBit(TeacherStudentsScene):
    ...
    def construct(self):
        words = OldTexText('Standard unit of\\\\information: The bit', tex_to_color_map={'The bit': YELLOW})
        words.next_to(self.teacher.get_corner(UL), UP, MED_LARGE_BUFF)
        words.to_edge(RIGHT)
        self.play(self.teacher.change('raise_left_hand'), FadeIn(words, UP))
        self.play_student_changes('happy', 'pondering', 'thinking', look_at=words)
        self.wait(4)
        formula = OldTex('I = -\\log_2(p)', tex_to_color_map={'I': YELLOW})
        formula.next_to(self.teacher.get_corner(UL), UP)
        self.play(words.animate.to_edge(UP), FadeIn(formula, UP), self.teacher.change('raise_right_hand', formula), self.students[0].change('erm', formula), self.students[1].change('confused', formula), self.students[2].change('pondering', formula))
        self.wait(5)
...
class DefineInformation(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.wait()
        self.play(ShowCreation(arrow), FadeIn(obs_label, lag_ratio=0.1), FadeTransform(pre_space.copy().set_fill(opacity=0), post_space), FadeIn(post_labels[1], 3 * RIGHT))
        self.wait()
        ...
        self.play(MoveToTarget(word_grid), FadeIn(has_s, 0.5 * DOWN))
        self.wait()
        frame = self.camera.frame
        ...
        self.play(MoveToTarget(mini_group1), FadeOut(has_s), Restore(word_grid), FadeOut(post_space), FadeOut(post_labels), frame.animate.move_to(2 * RIGHT))
        self.play(FadeTransform(pre_space.copy(), post_space2), FadeIn(post_labels2, shift=3 * RIGHT))
        self.wait()
        word_grid.generate_target()
        for word in word_grid.target:
            if 't' not in word.text:
                word.set_opacity(0.1)
        self.play(FadeIn(has_t, 0.5 * DOWN), MoveToTarget(word_grid))
        self.wait()
        self.remove(has_t)
        word_grid.restore()
        ...
        for n in range(3, 7):
            ...
            self.play(LaggedStart(MoveToTarget(new_mini), AnimationGroup(FadeOut(last_posts), FadeIn(new_post_space), FadeIn(new_post_labels)), lag_ratio=0.5))
            self.wait()
        ...
        formulas = VGroup(OldTex('\\left( \\frac{1}{2} \\right)^I = p', **kw), OldTex('2^I = \\frac{1}{p}', **kw), OldTex('I = \\log_2\\left(\\frac{1}{p}\\right)', **kw), OldTex('I = -\\log_2(p)', **kw))
        ...
        self.play(Blink(randy))
        self.wait()
        expr = OldTex('20 \\text{ bits} \\Leftrightarrow p \\approx 0.00000095', tex_to_color_map={'\\text{bits}': YELLOW})
        expr.next_to(group, UP, buff=0.75)
        self.play(FadeOut(randy), FadeOut(formulas[3]), FadeOut(rhs_rect), Write(expr))
        self.wait()
        group = group[:-2]
        self.play(FadeOut(expr), FadeOut(last_posts), group.animate.scale(0.7, about_edge=DL), FadeOut(mini_groups, RIGHT), frame.animate.move_to(RIGHT))
        ...
        ps2.label = self.get_post_space_labels(self.get_post_space(pre_space, 3).replace(ps2), font_size=24)
        self.play(FadeTransform(pre_space.copy().set_opacity(0), ps1), FadeIn(ps1.label, 2 * RIGHT))
        self.wait()
        self.play(FadeTransform(ps1.copy().set_opacity(0), ps2), FadeIn(ps2.label, 2 * RIGHT), ShowCreation(arrow2))
        self.wait()
        brace = Brace(VGroup(ps1.label, ps2.label), UP)
        b_label = brace.get_text('5 bits').set_color(YELLOW)
        self.play(GrowFromCenter(brace), FadeIn(b_label, 0.2 * UP))
        self.wait()
...
class AskForFormulaForI(Scene):
    ...
    def construct(self):
        tex = OldTex('I = ???', tex_to_color_map={'I': YELLOW}, font_size=72)
        tex.to_edge(UP)
        self.play(Write(tex))
        self.wait()
...
class MinusLogExpression(Scene):
    ...
    def construct(self):
        tex = OldTex('I = -\\log_2(p)', tex_to_color_map={'I': YELLOW}, font_size=60)
...
class ShowPatternInformationExamples(WordleDistributions):
    ...
    def construct(self):
        ...
        randy.flip()
        randy.to_corner(DR)
        self.play(PiCreatureSays(randy, OldTexText('I thought this\\\\was a word game'), bubble_config={'width': 4, 'height': 3}, target_mode='pleading'))
        x_tracker.clear_updaters()
        x_tracker.set_value(0)
        self.play(x_tracker.animate.set_value(120), run_time=20, rate_func=linear)
        self.wait()
...
class AddingBitsObservationOverlay(Scene):
    ...
    def construct(self):
        ...
        for i, s in enumerate(phase2_strs):
            s[i + 1] = 't'
        kw = dict(font='Consolas', t2c={'t': RED, 's': YELLOW})
        ...
        obss = VGroup(OldTexText("Has a `t'"), OldTexText("Starts with `s'"))
        obss.scale(0.85)
        bits_labels = VGroup(Text('2 bits'), Text('3 more bits'))
        ...
        self.play(FadeTransform(arrows[0], long_arrow), FadeTransform(arrows[1], long_arrow), Write(full_bits_label, run_time=1))
        self.wait()

class ExpectedInformationLabel(Scene):

    def construct(self):
        eq = OldTex('E[\\text{Information}] = ', '\\sum_x p(x) \\cdot \\text{Information}(x)', tex_to_color_map={'\\text{Information}': YELLOW}, font_size=60)
        eq.to_edge(UP)
        self.play(Write(eq))
        self.wait()
...
class AskAboutPhysicsRelation(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        physics.to_edge(UP)
        self.student_says(OldTexText('What does this have\\\\to do with thermodynamics?'), target_mode='raise_right_hand', index=2, bubble_config=dict(width=5, height=3, direction=LEFT))
        self.play(self.teacher.change('tease'))
        self.play(self.students[0].change('pondering', physics), self.students[1].change('pondering', physics), self.students[2].change('raise_left_hand', physics), Write(physics))
        self.wait(3)
        self.embed()
    ...
    def get_physics_entropy_image(self):
        ...
        labels = VGroup(Text('Low entropy'), Text('High entropy'))
        labels.scale(0.75)
...
class ContrastWearyAndSlate(WordleScene):
    ...
    def construct(self):
        ...
        grids.to_edge(DOWN)
        EI_label = OldTex('E[I]', '= ', '\\sum_{x} p(x) \\log_2\\big(1 / p(x) \\big)', tex_to_color_map={'I': BLUE}, font_size=36)
        EI_label.to_edge(UP)
        EI_rect = SurroundingRectangle(EI_label)
        EI_rect.set_stroke(YELLOW, 2)
        values = VGroup(DecimalNumber(4.9, unit='\\text{ bits}'), DecimalNumber(5.87, unit='\\text{ bits}'))
        arrows = VGroup()
...
class ShowEntropyCalculations(IntroduceDistribution):
    ...
    def construct(self):
        ...
        for word in words:
            ...
            self.play(UpdateFromAlphaFunc(x_tracker, lambda m, a: m.set_value(int(a * (n - 1)))), UpdateFromAlphaFunc(ent_rhs, lambda m, a: m.set_value(sum(ent_summands[:int(a * n)]))), rate_func=linear, run_time=4 * n / 3 ** 5)
            self.wait()
            self.remove(group)
...
class UniformPriorExample(WordleSceneWithAnalysis):
    ...
    show_prior = False
    weight_to_prob = 0
    pre_computed_first_guesses = ['tares', 'lares', 'rales', 'rates', 'teras', 'nares', 'soare', 'tales', 'reais', 'tears', 'arles', 'tores', 'salet']

class MentionUsingWordFrequencies(TeacherStudentsScene):

    def construct(self):
        self.teacher_says(OldTexText('Next step: Integrate\\\\word frequency data'), added_anims=[self.change_students('hooray', 'happy', 'tease')])
        self.wait()
...
class HowThePriorWorks(Scene):
    ...
    def construct(self):
        ...
        col1, col2 = cols = [WordleScene.get_grid_of_words(word_list, 25, 1, dots_index=-12) for word_list in (random.sample(all_words, 100), sorted_words)]
        for col in cols:
            col.set_height(6)
            col.set_x(-1)
            col.to_edge(DOWN, buff=MED_SMALL_BUFF)
        bars1, bars2 = [self.get_freq_bars(col, freq_map, max_width=width, exp=exp) for col, width, exp in zip(cols, (1, 2), (0.3, 1))]
        group1 = VGroup(col1, bars1)
        group2 = VGroup(col2, bars2)
        col1_title = VGroup(Text('Relative frequencies of all words'), Text('From the Google Books English n-gram public dataset', font_size=24, color=GREY_B))
        ...
        self.play(ShowCreation(arrow), Write(arrow_label), group1.animate.next_to(arrow, LEFT))
        group2.next_to(arrow, RIGHT, buff=LARGE_BUFF)
        self.play(LaggedStart(*(FadeInFromPoint(VGroup(word, bar), col1.get_center()) for word, bar in zip(col2, bars2)), lag_ratio=0.1, run_time=3))
        self.wait()
        ...
        self.wait()
        self.play(rect.animate.replace(col2[7], stretch=True).set_opacity(0), col2[7].animate.set_color(YELLOW), ShowIncreasingSubsets(numbers), run_time=0.5)
        self.wait()
        self.remove(rect)
        ...
        which_group, braid_group = comp = VGroup(*(VGroup(Text(word, font='Consolas'), Vector(RIGHT), DecimalNumber(freq_map[word], num_decimal_places=6)).arrange(RIGHT) for word in comp_words))
        ...
        self.play(FadeOut(arrow), FadeOut(arrow_label), FadeOut(group1), FadeTransform(col2[0].copy(), which_group[0]))
        self.play(ShowCreation(which_group[1]), CountInFrom(which_group[2], 0))
        self.wait()
        self.play(FadeTransform(which_group[:2].copy(), braid_group[:2]))
        self.play(CountInFrom(braid_group[2], 0, run_time=0.5))
        self.wait()
        self.play(FadeIn(percentages, 0.75 * DOWN), *map(MoveToTarget, rhss))
        self.wait()
        ...
        for word in col3:
            if word.text in col2_words:
                ...
                word.set_opacity(0)
        (self.remove(col2),)
        self.play(LaggedStart(FadeOut(VGroup(comp, percentages), 2 * LEFT), FadeOut(numbers), FadeOut(bars2), FadeOut(col1_title, UP), MoveToTarget(col3), Write(axes), FadeOut(col2[col2_words.index('...')]), run_time=5))
        self.wait()
        ...
        def update_lines(lines):
            for line, word in zip(lines, col3):
                line.put_start_and_end_on(word.get_top(), axes.input_to_graph_point(axes.x_axis.p2n(word.get_center()), graph))
    ...
    def get_freq_bars(self, words, freq_map, max_width=2, exp=1):
        freqs = [freq_map.get(w.text, 0) ** exp for w in words]
        max_freq = max(freqs)
        bars = VGroup()
        height = np.mean([w.get_height() for w in words]) * 0.8
        for word, freq in zip(words, freqs):
            bar = Rectangle(height=height, width=max_width * freq / max_freq, stroke_color=WHITE, stroke_width=1, fill_color=BLUE, fill_opacity=1)
...
class ShowWordLikelihoods(Scene):
    ...
    def construct(self):
        ...
        for word in word_mobs:
            ...
            decs.add(dec)
            bars.add(bar)
        self.play(LaggedStartMap(Restore, bars, lag_ratio=0.01), LaggedStartMap(CountInFrom, decs, lag_ratio=0.01), run_time=4)
        self.wait()
        self.bars = bars
        self.decs = decs
...
class LookThroughWindowsOfWords(Scene):
    ...
    def construct(self):
        ...
        group_size = 20
        n_groups = 3
        col4 = VGroup(*(WordleScene.get_grid_of_words(sorted_words[base + n * group_size:base + (n + 1) * group_size], group_size, 1) for n in range(n_groups)))
...
class EntropyOfWordDistributionExample(WordleScene):
    ...
    def construct(self):
        ...
        self.add(match_label)
        s_words = get_word_list(short=True)
        col1 = self.get_grid_of_words(sorted(list(set(self.possibilities).intersection(s_words))), 4, 1)
        col1.scale(1.5)
        ...
        for bar in bars1:
            ...
            bar.stretch(0, 0, about_edge=LEFT)
            bar.set_opacity(0)
        self.play(ShowIncreasingSubsets(col1), CountInFrom(match_label[0], 0))
        self.wait()
        ...
        formula = OldTex('H &=', '\\sum_x p(x) \\cdot', '\\log_2\\big(1 / p(x) \\big)\\\\', font_size=36)
        ...
        for word in col2:
            if word.text in col1_words:
                ...
                word.move_to(col1)
                word.set_opacity(0)
        pre_bars2, bars2 = [VGroup(*(self.get_prob_bar(word, 0.246 * self.priors[word.text] + 0.001, num_decimal_places=3) for word in group)) for group in (col2, col2.saved_state)]
        new_brace = Brace(bars2, RIGHT)
        self.play(FadeTransform(col1, col2), FadeTransform(bars1, pre_bars2), LaggedStart(*map(FadeOut, [alt_lhs, info_value, info_label, info_box])))
        self.play(ChangeDecimalToValue(match_label[0], 16, run_time=1), Restore(col2, run_time=2), ReplacementTransform(pre_bars2, bars2, run_time=2), Transform(brace, new_brace))
        self.wait()
        ...
        rhs2 = OldTex('= &4 \\big(0.247 \\cdot \\log_2(1/0.247)\\big) \\\\', '+ &12 \\big(0.001 \\cdot \\log_2(1/0.001)\\big)\\\\ ', '= &2.11', font_size=30)
    ...
    def get_prob_bar(self, word, prob, num_decimal_places=2, height=0.15, width_mult=8.0):
        bar = Rectangle(height=height, width=width_mult * prob, stroke_color=WHITE, stroke_width=1, fill_color=BLUE)
        bar.next_to(word, RIGHT, MED_SMALL_BUFF)
    ...
    def seek_good_examples(self):
        ...
        for answer in swords:
            poss = list(words)
            for guess in ['other', 'nails']:
                poss = get_possible_words(guess, get_pattern(guess, answer), poss)
...
class WhatMakesWordleNice(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says(OldTexText('This is what makes wordle\\\\such a nice example'), added_anims=[self.change_students('pondering', 'thinking', 'erm', look_at=ORIGIN)])
        self.wait(5)
...
class TwoInterpretationsWrapper(Scene):
    ...
    def construct(self):
        ...
        title.to_edge(UP)
        screen_titles = VGroup(Text('Expected information from guess'), Text('Remaining uncertainty'))
...
class WordlePriorExample(WordleSceneWithAnalysis):
    secret_word = 'thump'
    wordle_based_prior = True
    pre_computed_first_guesses = ['soare', 'raise', 'roate', 'raile', 'reast', 'slate', 'crate', 'irate', 'trace', 'salet', 'arise', 'orate', 'stare']
...
class HowToCombineEntropyAndProbability(FreqPriorExample):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(rows[:2].animate.set_opacity(1), rows[2:].animate.set_opacity(0.3))
        self.wait()
        self.play(rows[1].animate.set_opacity(0.3), rows[2].animate.set_opacity(1), rows[3:].animate.set_opacity(0.3))
        self.wait()
        es_eq = OldTex('E[\\text{Score}] = 0.58 \\cdot {4} +', '(1 - 0.58)', ' \\cdot \\big({4} + f(1.44 - 1.27)\\big)', tex_to_color_map={'\\text{Score}': YELLOW, '{4}': YELLOW, '0.58': self.prior_color, '1.44': self.entropy_color, '1.27': self.entropy_color, '=': WHITE})
        ...
        self.play(FadeTransform(rows[0][2].copy(), es_eq.get_part_by_tex('0.58')), FadeIn(es_eq.slice_by_tex('\\cdot', '(1 -')), q_marks.animate.next_to(es_eq[:8], RIGHT, aligned_edge=UP))
        self.remove(es_eq)
        self.add(es_eq[:8])
        self.wait()
        self.play(FadeIn(es_eq[8:11]), q_marks.animate.next_to(es_eq[10], RIGHT), FadeOut(rows[3:]))
        self.wait()
...
class FirstThoughtsOnCombination(Scene):
    ...
    def construct(self):
        ...
        morty.flip()
        morty.to_corner(DL)
        example = VGroup(Text('dorms', font='Consolas'), DecimalNumber(1.08, color=TEAL), DecimalNumber(0.31, color=BLUE))
        word, n1, n2 = example
        example.arrange(RIGHT, buff=1.5)
        example[0].shift(0.5 * RIGHT)
        example.to_corner(UR)
        example.shift(DOWN)
        example_titles = VGroup(OldTex('E[\\text{Info.}]', color=TEAL), OldTex('p(\\text{word})', color=BLUE))
        ...
        self.add(example)
        self.add(morty)
        self.play(PiCreatureBubbleIntroduction(morty, OldTexText('How should I measure\\\\guess quality?', font_size=36), look_at=example, target_mode='pondering', bubble_type=ThoughtBubble, bubble_config={'width': 4, 'height': 3}))
        self.play(Blink(morty))
        self.wait()
        ...
        arrow = OldTex('\\rightarrow')
        plus = OldTex('+')
        group = VGroup(attempt.target[0], arrow, attempt.target[1], plus, attempt.target[2])
        group.arrange(RIGHT, buff=0.2)
        group.next_to(example, DOWN, buff=2)
        self.play(MoveToTarget(attempt), morty.change('shruggie', attempt), FadeIn(arrow), FadeIn(plus))
        self.wait()
        self.play(Blink(morty))
        cross = Cross(group)
        ...
        self.play(ShowCreation(cross))
        self.play(Write(better_words, run_time=1), morty.change('pondering', cross))
        self.play(Blink(morty))
        self.wait()
        self.embed()
...
class EntropyToScoreData(Scene):
    ...
    def construct(self):
        axes = Axes((0, 13), (0, 6), height=6, width=10)
        ...
        for x in bucket_xs:
            ...
            if not indices.any():
                continue
            bar = Rectangle(width=axes.x_axis.unit_size * bucket_size, height=axes.y_axis.unit_size * y)
            bar.set_stroke(WHITE, 1)
            bar.move_to(axes.c2p(x, 0), DL)
        ...
        self.wait()
        bars.save_state()
        self.play(bars[:4].animate.fade(0.7), bars[5:].animate.fade(0.7))
        self.wait()
        self.play(Restore(bars))
        self.play(bars[:16].animate.fade(0.7), bars[17:].animate.fade(0.7))
        self.wait()
        self.play(Restore(bars))
        curve = axes.get_graph(lambda x: 1 + 0.56 * math.log(x + 1) + 0.08 * x)
...
class LookTwoStepsAhead(WordleSceneWithAnalysis):
    look_two_ahead = True
    wordle_based_prior = True
    pre_computed_first_guesses = ['slate', 'salet', 'slane', 'reast', 'trace', 'carse', 'crate', 'torse', 'carle', 'carte', 'toile', 'crane', 'least', 'saint', 'crine', 'roast']
...
class HowLookTwoAheadWorks(Scene):
    ...
    def construct(self):
        ...
        self.add(guess1)
        self.play(ShowCreation(pattern_array1.connecting_lines, lag_ratio=0.1), LaggedStartMap(FadeIn, pattern_array1.pattern_mobs, shift=0.2 * RIGHT, lag_ratio=0.1), run_time=2)
        ...
        self.wait()
        H_eq = OldTex('E[I] = \\sum_{x} p(x) \\cdot \\log_2\\big((1 / p(x)\\big)', font_size=36)
        ...
        self.wait()
        self.play(LaggedStartMap(FadeIn, info_labels[1:], lag_ratio=0.5), H_eq.animate.scale(0.7).to_edge(DOWN), run_time=2)
        self.wait()
        ...
        for i, bar in enumerate(prob_bars1):
            ...
            for j, shown_word in enumerate(shown_words):
                ...
                group = VGroup(arrow, guess2, h2_label, pattern_array2, prob_bars2)
            ...
            self.play(MoveToTarget(h2_label), Uncreate(pattern_array2.connecting_lines, lag_ratio=0.01), LaggedStartMap(FadeOut, pattern_array2.pattern_mobs), LaggedStartMap(FadeOut, pattern_array2.dot_parts), LaggedStartMap(FadeOut, prob_bars2, scale=0.25), run_time=self.transition_time)
            arrows.add(arrow)
        ...
        self.wait()
        self.play(LaggedStartMap(MoveToTarget, sum_parts, run_time=2), label.animate.scale(0.7).to_edge(DOWN))
        self.wait()
    ...
    def get_pattern_array(self, word, possibilities, priors, n_shown=15):
        ...
        connecting_lines = VGroup(*(self.get_connecting_line(word, row) for row in pattern_mobs))
    ...
    def get_connecting_line(self, mob1, mob2):
        ...
        return CubicBezier(mob1.get_right() + SMALL_BUFF * RIGHT, mob1.get_right() + RIGHT * diff / 2, mob2.get_left() + LEFT * diff / 2, mob2.get_left() + SMALL_BUFF * LEFT, stroke_color=WHITE, stroke_width=1)

    def get_prob_bars(self, pattern_mobs, width_scalar=10):
        result = VGroup()
        for pattern_mob in pattern_mobs:
            bar = Rectangle(width=width_scalar * pattern_mob.prob, height=pattern_mob.get_height(), fill_color=self.prob_color, fill_opacity=1, stroke_width=0.5, stroke_color=WHITE)
    ...
    def get_info_label(self, bar):
        result = VGroup(OldTex('\\log_2\\big( 1 / '), DecimalNumber(bar.prob, num_decimal_places=3), OldTex('\\big) = '), DecimalNumber(-math.log2(bar.prob), num_decimal_places=3))
        result.arrange(RIGHT, buff=SMALL_BUFF)
...
class TripleComparisonFrame(Scene):
    ...
    def construct(self):
        ...
        self.add(squares)
        titles = VGroup(OldTexText('V1: Just maximize\\\\entropy'), OldTexText('V2: Incorporate word\\\\frequency data'), OldTexText('V3: Use true wordle list\\\\(plus 1 or 2 other tricks)'))
        titles.scale(0.75)
...
class InformationLimit(WordleScene):
    ...
    def construct(self):
        ...
        brace_label = VGroup(OldTex('\\log_2(2{,}315)', '=', '11.17 \\text{ bits}', **kw), Text('of uncertainty', **kw))
        ...
        self.play(Write(left_title), FadeIn(word_mobs, lag_ratio=0.1, run_time=3))
        self.wait()
        ...
        s_title = OldTexText('Maximum expected information\\\\', 'after first two guesses:', **kw)
        s_title.match_y(left_title)
        s_title.to_edge(RIGHT)
        arrows = VGroup(*(Arrow(grid[i].get_right(), grid[i + 1].get_right(), buff=0, path_arc=-(PI + 0.1), width_to_tip_len=0.005) for i in (0, 1)))
        for arrow in arrows:
            arrow.shift(0.1 * RIGHT)
        arrows.space_out_submobjects(1.2)
        EI_labels = VGroup(OldTex('E[I_1] = 5.77', **kw), OldTex('E[I_2] = 4.24', **kw))
        ...
        self.play(Write(s_title), ShowCreation(arrows))
        self.play(LaggedStart(*(FadeIn(EI_label, 0.25 * RIGHT) for EI_label in EI_labels)), lag_ratio=0.5)
        ...
        self.play(grid.animate.fade(0.75), FadeIn(row3))
        self.wait()
        best_case_words = Text('Best case scenario:\nDown to ~1.16 bits of\nuncertainty, on average', **kw)
        best_case_words.get_part_by_text('~').match_y(best_case_words.get_part_by_text('1.16'))
        best_case_words.next_to(grid[2], DR, MED_LARGE_BUFF)
        self.play(Write(best_case_words))
        self.wait()

class EndScreen(PatreonEndScreen):
    CONFIG = {'scroll_time': 30}

class ShowScoreDistribution(Scene):
    data_file = 'crane_with_wordle_prior.json'
    axes_config = dict(x_range=(0, 9), y_range=(0, 1, 0.1), width=8, height=6)
    ...
    def construct(self):
        ...
        mean_label = VGroup(Text('Average score: '), DecimalNumber(np.mean(scores), num_decimal_places=3))
        ...
        grid.add(grid.words)
        self.add(grid)
        score_label = VGroup(Text('Score: '), Integer(0, edge_to_fix=LEFT))
        ...
        answer_label = VGroup(Text('Answer: '), Text(games[0]['answer'], font='Consolas'))
        ...
        def update_grid(grid, alpha):
            ...
            try:
                grid.words.set_submobjects([Text(guess.upper(), font='Consolas') for guess in (*game['guesses'], game['answer'])])
            except Exception:
                return
        ...
        def update_answer_label(answer_label, alpha):
            ...
            return answer_label
        self.play(UpdateFromAlphaFunc(bars, update_bars), UpdateFromAlphaFunc(mean_label, update_mean_label), UpdateFromAlphaFunc(grid, update_grid), UpdateFromAlphaFunc(score_label, update_score_label), UpdateFromAlphaFunc(answer_label, update_answer_label), run_time=20, rate_func=linear)
        update_bars(bars, 1)
        self.wait()
    ...
    def get_bars(self, axes, scores):
        ...
        bars = VGroup(*(self.get_bar(axes, n + 1, prop) for n, prop in enumerate(props)))
    ...
    def get_bar(self, axes, score, proportion):
        bar = Rectangle(width=axes.x_axis.unit_size, height=axes.y_axis.unit_size * proportion)
        bar.set_fill(BLUE, 1)
        bar.set_stroke(WHITE, 1)
...
class ThinV1Stats(SimulatedGamesUniformPriorDist):
    axes_config = dict(x_range=(0, 8), y_range=(0, 1, 0.1), width=5, height=6)
...
class Thumbnail(Scene):
    ...
    def construct(self):
        ...
        rows = WordleScene.patterns_to_squares(patterns, color_map=[GREY_D, YELLOW, GREEN_E])
        ...
        for guess, row in zip(guesses, rows):
            ...
            for char, square in zip(word, row):
                char.move_to(square)
            words.add(word)
        title = Text('Best opener: CRANE', font_size=100, font='Consolas', t2c={'crane': GREEN})
