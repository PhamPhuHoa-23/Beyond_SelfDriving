...
class HeresTheThing(TeacherStudentsScene):
    ...
    def construct(self):
        thumbnail = ImageMobject(os.path.join(self.file_writer.output_directory, self.file_writer.get_default_module_directory(), os.pardir, 'images', 'Thumbnail'))
        thumbnail = Group(SurroundingRectangle(thumbnail, buff=0, stroke_color=WHITE, stroke_width=2), thumbnail)
        thumbnail.set_height(3)
        thumbnail.move_to(self.hold_up_spot, DOWN)
        self.play(self.teacher.change('raise_right_hand'), FadeIn(thumbnail, UP))
        self.play_student_changes('raise_left_hand', 'pondering', 'erm', look_at=self.teacher.eyes)
        self.wait()
        self.teacher_says(OldTexText("Right, here's\\\\the thing...", font_size=40), target_mode='hesitant', bubble_config=dict(width=3.5, height=2.5), run_time=1, added_anims=[self.change_students('dance_3', 'frustrated', 'sassy'), thumbnail.animate.to_corner(UL)])
...
class Confessions(Scene):
    ...
    def construct(self):
        ...
        phrases = VGroup(OldTexText('Bug in the code', font_size=48), OldTexText('About the opening scene...', font_size=48), OldTexText("Maybe don't use entropy?", font_size=48))
        ...
        for title, phrase in zip(titles, phrases):
            ...
            self.play(FadeOut(last_phrase), title.animate.restore().set_height(0.5).center().to_edge(UP), *map(MoveToTarget, others))
...
class WhatWasTheBug(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.change_students('raise_left_hand', 'pondering', 'raise_right_hand'), self.teacher.change('tired'))
        self.wait(3)
...
class HowWordleColoringWorks(WordleScene):
    ...
    def construct(self):
        ...
        def get_grey_label(i):
            result = VGroup(Vector(0.5 * UP), OldTexText("No 2nd `E'"))
        ...
        self.play(Write(gl))
        self.wait()
        group1 = VGroup(guess_row.copy(), answer_row.copy(), guess_row.word.copy(), answer_row.word.copy(), path, gl)
        answer_row.word.set_submobjects([])
        vect = 1.5 * DOWN
        self.play(Transform(group1, generate_group_target(group1).to_corner(UL)), labels.animate.shift(vect), rows.animate.set_fill(BLACK, 0).shift(vect), guess_row.word.animate.shift(vect))
        ...
        self.wait()
        group2 = VGroup(*rows.copy(), *words.copy(), path1, path2)
        answer_row.word.set_submobjects([])
        group2.target = generate_group_target(group2)
        group2.target.next_to(group1, RIGHT, buff=LARGE_BUFF, aligned_edge=DOWN)
        self.play(guess_row.animate.set_fill(BLACK, 0), MoveToTarget(group2))
        change_answer('steal')
        ...
        self.play(Write(gl))
        self.wait()
        group3 = VGroup(*rows.copy(), *words.copy(), path, gl)
        answer_row.word.set_submobjects([])
        ...
        self.play(FadeTransform(groups.copy(), low_groups), FadeIn(bug_box), Write(bug_label))
    ...
    def write_word_in_row(self, word, row, added_anims=[]):
        ...
        self.play(ShowIncreasingSubsets(word_mob, run_time=0.5), *added_anims)
...
class MatrixOfPatterns(Scene):
    ...
    def construct(self):
        ...
        self.play(ApplyMethod(frame.set_height, 25, dict(about_edge=UL), rate_func=squish_rate_func(smooth, 0.1, 1)), ShowIncreasingSubsets(pattern_mobs), run_time=20)
        self.wait()
...
class SneakAttackInformationTheory(Scene):
    ...
    def construct(self):
        ...
        self.play(randy.change('thinking', phone), FadeIn(phone), Write(phone_outline, stroke_width=1, run_time=2))
        self.play(Blink(randy))
        self.wait()
        words = VGroup(Text('Surprise!'), Text('Information theory!'))
        words.scale(1.25)
        words.arrange(DOWN)
        words.to_corner(UL)
        ...
        self.play(FadeIn(words[1], scale=2), run_time=0.5)
        self.play(Blink(randy))
        self.play(Write(ent_formula), randy.change('erm', ent_formula))
        self.play(Blink(randy))
        self.wait(2)
        self.play(Blink(randy))
        self.wait(2)
...
class HowAreYouFindingTheBest(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says(OldTexText("What exactly is this\\\\``final analysis''?"), index=0)
        self.play(self.students[1].change('pondering'), self.students[2].change('raise_left_hand'), self.teacher.change('happy'))
        self.wait(2)
        self.teacher_says(OldTexText("I'm glad\\\\you asked!"), target_mode='hooray')
        self.wait(3)

class SaletDistThin(SaletBruteForceDistribution):
    axes_config = dict(x_range=(0, 6), y_range=(0, 1, 0.1), width=4.5, height=6)
...
class FromEstimatedProbabilityOfInclusionToExact(DistributionOverWord):
    ...
    def construct(self):
        ...
        for bar, dec, word_mob in zip(bars, decs, word_mobs):
            ...
            anims.extend([bar.animate.set_height(value * height, stretch=True, about_edge=DOWN), ChangeDecimalToValue(dec, 100 * value)])
...
class SaletSecondGuessMap(HowLookTwoAheadWorks):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(ShowCreation, arrows, lag_ratio=0.5), LaggedStartMap(FadeIn, guess2s, lambda m: (m, RIGHT), lag_ratio=0.5), run_time=2)
        self.wait()
        for pm, guess2 in zip(pattern_array1.pattern_mobs, guess2s):
            ...
            ng_group = self.get_next_guess_group(guess2.text, pattern_array2, bucket, prefix=guess1.text + ''.join(map(str, pattern_to_int_list(pm.pattern))))
    ...
    def get_next_guess_group(self, guess, pattern_array, possibilities, prefix=''):
        ...
        for pm in pattern_array.pattern_mobs:
            ...
            if key in self.guess_map:
                ...
                next_guess = optimal_guess(self.all_words, get_possible_words(guess, pm.pattern, possibilities), self.priors, optimize_for_uniform_distribution=True)
...
class RankingUpdates(Scene):
    ...
    def construct(self):
        ...
        titles = VGroup(OldTexText('Highest $E[I]$\\\\(one step)', **kw), OldTexText('Highest $E[I]$\\\\(two steps)', **kw), OldTexText('Lowest average\\\\scores', **kw))
        ...
        with open(os.path.join(get_directories()['data'], 'wordle', 'best_entropies.json')) as fp:
            ...
            for word, ent in json.load(fp)[:n_shown]:
                row = VGroup(Text(word, font='Consolas'), OldTex('\\rightarrow'), DecimalNumber(ent, color=TEAL))
                row.arrange(RIGHT)
                rows1.add(row)
        with open(os.path.join(get_directories()['data'], 'wordle', 'best_double_entropies.json')) as fp:
            ...
            for word, ent1, ent2 in json.load(fp)[:n_shown]:
                row = VGroup(Text(word, font='Consolas'), OldTex('\\rightarrow'), DecimalNumber(ent1, color=TEAL), OldTex('+'), DecimalNumber(ent2, color=TEAL), OldTex('='), DecimalNumber(ent1 + ent2, color=TEAL))
                row.arrange(RIGHT)
                rows2.add(row)
        ...
        with open(os.path.join(get_directories()['data'], 'wordle', 'best_scores_with_entropy_method.json')) as fp:
            ...
            for word, score, dist in json.load(fp)[:n_shown]:
                row = VGroup(Text(word, font='Consolas'), OldTex('\\rightarrow'), DecimalNumber(score / len(answers), num_decimal_places=3, color=BLUE))
                row.arrange(RIGHT)
                rows3.add(row)
        ...
        for title, col in zip(titles[1:], cols[1:]):
            ...
            self.play(FadeIn(title), FadeIn(col.numbers, lag_ratio=0.1), LaggedStart(*mover_anims, run_time=2, lag_ratio=0.2))
...
class WeCanDoBetter(Scene):
    ...
    def construct(self):
        morty = Mortimer()
        morty.to_corner(DR)
        self.play(PiCreatureSays(morty, OldTexText('We can do\\\\better!'), bubble_config=dict(width=3.5, height=2.5, fill_opacity=0.95)))
        for x in range(2):
...
class WhydYouHaveToRuinIt(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says(OldTexText("Why'd you have to\\\\ruin Wordle!"), target_mode='pleading', added_anims=[self.teacher.change('guilty')])
        self.play_student_changes('sassy', 'angry', 'pleading')
        self.wait(3)
        self.teacher_says(OldTexText("But ``salet'' is probably\\\\not the best for us"), added_anims=[self.change_students('confused', 'sassy', 'hesitant')])
        self.wait(3)
...
class ForgetTheBestWord(Scene):
    ...
    def construct(self):
        ...
        self.play(PiCreatureBubbleIntroduction(randy, text, bubble_type=ThoughtBubble, target_mode='confused'))
        self.play(Blink(randy))
        self.wait()
...
class Thumbnail2(Thumbnail):
    ...
    def construct(self):
        ...
        strike = Line(crane.get_corner(DL), crane.get_corner(UR))
