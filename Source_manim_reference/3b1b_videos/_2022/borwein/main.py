...
def get_sinc_tex(k=1):
    ...
    return f'{{\\sin(x {div_k}) \\over x {div_k}}}'

def get_multi_sinc_integral(ks=[1], dots_at=None, rhs='', insertion=''):
    result = OldTex('\\int_{-\\infty}^\\infty', insertion, *(get_sinc_tex(k) if k != dots_at else '\\dots' for k in ks), 'dx', rhs)
    t2c = {'\\sin': BLUE, 'x / 3': TEAL, 'x / 5': GREEN_B, 'x / 7': GREEN_C, 'x / 9': interpolate_color(GREEN, YELLOW, 1 / 3), 'x / 11': interpolate_color(GREEN, YELLOW, 2 / 3), 'x / 13': YELLOW, 'x / 15': RED_B}
...
class ShowIntegrals(InteractiveScene):
    ...
    def construct(self):
        ...
        func_label = Tex('{\\sin(x) \\over x}')
        func_label.move_to(axes, UP).to_edge(LEFT)
        self.add(axes)
        self.play(Write(func_label), ShowCreation(right_sinc, remover=True, run_time=3), ShowCreation(left_sinc, remover=True, run_time=3))
        self.add(graph)
        self.wait()
        sinc_label = OldTex('\\text{sinc}(x)')
        ...
        self.wait()
        self.play(ShowCreation(sine_wave, run_time=2), graph.animate.set_stroke(width=1, opacity=0.5))
        self.wait()
        self.play(ShowCreation(one_over_x_graph), FadeIn(one_over_x_label), Transform(sine_wave, half_sinc))
        self.wait()
        self.play(FadeOut(one_over_x_graph), FadeOut(one_over_x_label), FadeOut(sine_wave), graph.animate.set_stroke(width=3, opacity=1))
        hole = Dot()
        hole.set_stroke(BLUE, 2)
        hole.set_fill(BLACK, 1)
        hole.move_to(axes.c2p(0, 1))
        zero_eq = OldTex('{\\sin(0) \\over 0} = ???')
        zero_eq.next_to(hole, UR)
        lim = OldTex('\\lim_{x \\to 0} {\\sin(x) \\over x} = 1')
        ...
        self.add(dots)
        self.play(x_tracker.animate.set_value(0).set_anim_args(run_time=2), UpdateFromAlphaFunc(dots, lambda m, a: m.set_opacity(a)))
        self.wait()
        ...
        self.play(Write(area, stroke_color=WHITE, lag_ratio=0.01, run_time=4), ReplacementTransform(func_label, int1[1]), Write(int1[::2]), graph.animate.set_stroke(width=1))
        self.add(int1)
        self.wait()
        ...
        low_label = OldTex('{\\sin(x / 3) \\over x / 3}')
        low_label.match_x(int1[1]).shift(1.75 * LEFT).match_y(low_axes.get_top())
        self.play(MoveToTarget(top_group), FadeTransform(axes.copy(), low_axes), FadeIn(low_label), frame.animate.set_height(10), VGroup(int1, rhs).animate.shift(UP + 1.75 * LEFT))
        self.wait()
        ...
        for n in range(2, 9):
            ...
            if n < 8:
                ...
                new_rhs = Tex('\\approx \\pi - 4.62 \\times 10^{-11}')
            ...
            self.play(ReplacementTransform(curr_int[:n], new_int[:n]), TransformFromCopy(low_label[0], new_int[n]), ReplacementTransform(curr_int[-1], new_int[-1]), FadeOut(rhs), FadeOut(area), ReplacementTransform(graph, new_graph), TransformFromCopy(low_graph, new_graph.copy(), remover=True))
            ...
            if n < 8:
                ...
                new_low_label = OldTex(f'{{\\sin(x / {2 * n + 1}) \\over x / {2 * n + 1}}}')
                new_low_label.move_to(low_label)
                if n == 4:
                    new_low_label.shift(0.5 * UP)
                self.play(FadeOut(low_label, UP), FadeIn(new_low_label, UP), ReplacementTransform(low_graph, new_low_graph))
                self.wait(0.25)
            curr_int = new_int
            rhs = new_rhs
            graph = new_graph
        ...
        new_rhs = OldTex(f'= {get_fifteenth_frac_tex()} \\pi')
        new_rhs.next_to(curr_int, DOWN, buff=LARGE_BUFF)
        self.play(FadeOut(VGroup(low_axes, low_graph, low_label), DOWN), VGroup(axes, graph, area).animate.shift(2 * DOWN), Write(new_rhs))
        self.wait()

    def get_axes(self, x_range=(-10 * PI, 10 * PI, PI), y_range=(-0.5, 1, 0.5), width=1.3 * FRAME_WIDTH, height=3.5):
        ...
        if self.add_axis_labels:
            ...
            for u in (-1, 1):
                axes.x_axis.add_numbers(u * np.arange(PI, 15 * PI, PI), unit=PI, unit_tex='\\pi', font_size=20)
        return axes

    def get_area(self, axes, graph, dx=0.01 * PI, fill_opacity=0.5):
        rects = axes.get_riemann_rectangles(graph, dx=dx, colors=(BLUE_D, BLUE_D), negative_color=RED)
    ...
    def get_integral(self, n):
        return OldTex('\\int_{-\\infty}^\\infty', '{\\sin(x) \\over x}', *(f'{{\\sin(x/{k}) \\over x / {k} }}' for k in range(3, 2 * n + 1, 2)), 'dx').to_corner(UL)
...
class SineLimit(InteractiveScene):
    ...
    def construct(self):
        ...
        arc.add_updater(lambda m: m.match_points(Arc(radius=radius, arc_center=origin, start_angle=0, angle=get_x())))
        h_line = Line()
        h_line.set_stroke(RED)
        h_line.add_updater(lambda m: m.put_start_and_end_on(get_point(), get_point()[0] * RIGHT))
        radial_line = Line()
        radial_line.add_updater(lambda m: m.put_start_and_end_on(origin, get_point()))
        one_label = Integer(1, font_size=24)
        one_label.add_updater(lambda m: m.next_to(radial_line.get_center(), normalize(rotate_vector(radial_line.get_vector(), PI / 2)), buff=SMALL_BUFF))
        sine_label = OldTex('\\sin(x)', font_size=24)
        ...
        self.add(sine_label)
        self.add(x_label)
        value_label = VGroup(OldTex('\\frac{\\sin(x)}{x} = '), DecimalNumber(1, num_decimal_places=4))
        ...
        self.play(UpdateFromFunc(frame, lambda m: m.interpolate(m, target_frame, get_alpha()), run_time=15), alpha_tracker.animate.set_value(1 / 30).set_anim_args(run_time=1))

class WriteFullIntegrals(InteractiveScene):

    def construct(self):
        ints = VGroup(*(get_multi_sinc_integral(range(1, n, 2), rhs='= \\pi') for n in range(3, 19, 2)))
...
class WriteMoreFullIntegrals(InteractiveScene):
    ...
    def construct(self):
        ints = VGroup(*(get_multi_sinc_integral(range(1, n, 2), rhs='= \\pi') for n in range(3, 17, 2)))
        ints.add(get_multi_sinc_integral(range(1, 17, 2), rhs='= (0.99999999998529)\\pi'))
        ints.add(get_multi_sinc_integral(range(1, 19, 2), rhs='= (0.99999998807962)\\pi'))
        ints.add(get_multi_sinc_integral(range(1, 21, 2), rhs='= (0.99999990662610)\\pi'))
        ints.add(get_multi_sinc_integral(range(1, 23, 2), rhs='= (0.99999972286332)\\pi'))
        ints.arrange(DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
        ints.center().to_corner(UL)
        for inter in ints:
            inter[-1].scale(1.5, about_edge=LEFT)
...
class WriteOutIntegrals(InteractiveScene):
    ...
    def get_integrals(self):
        return VGroup(get_multi_sinc_integral([1], rhs='= \\pi'), get_multi_sinc_integral([1, 3], rhs='= \\pi'), get_multi_sinc_integral([1, 3, 5], rhs='= \\pi'), OldTex('\\vdots'), get_multi_sinc_integral([1, 3, 5, 7, 13], dots_at=7, rhs='= \\pi'), get_multi_sinc_integral([1, 3, 5, 7, 13, 15], dots_at=7, rhs=f'= ({SUB_ONE_FACTOR}\\dots)\\pi'))

class WriteOutIntegralsWithPi(WriteOutIntegrals):

    def get_integrals(self):
        t2c = {'\\sin': BLUE, 'x/3': TEAL, 'x/5': GREEN_B, 'x/7': GREEN_C, 'x/9': interpolate_color(GREEN, YELLOW, 1 / 3), 'x/11': interpolate_color(GREEN, YELLOW, 2 / 3), 'x/13': YELLOW, 'x/15': RED_B}
        result = VGroup(OldTex('\\int_{-\\infty}^\\infty', '\\frac{\\sin(\\pi x)}{\\pi x}', 'dx = ', '1.0'), OldTex('\\int_{-\\infty}^\\infty', '\\frac{\\sin(\\pi x)}{\\pi x}', '\\frac{\\sin(\\pi x/3)}{\\pi x/3}', 'dx = ', '1.0'), OldTex('\\int_{-\\infty}^\\infty', '\\frac{\\sin(\\pi x)}{\\pi x}', '\\frac{\\sin(\\pi x/3)}{\\pi x/3}', '\\frac{\\sin(\\pi x/5)}{\\pi x/5}', 'dx = ', '1.0'), OldTex('\\vdots'), OldTex('\\int_{-\\infty}^\\infty', '\\frac{\\sin(\\pi x)}{\\pi x}', '\\frac{\\sin(\\pi x/3)}{\\pi x/3}', '\\dots', '\\frac{\\sin(\\pi x/13)}{\\pi x/13}', 'dx = ', '1.0'), OldTex('\\int_{-\\infty}^\\infty', '\\frac{\\sin(\\pi x)}{\\pi x}', '\\frac{\\sin(\\pi x/3)}{\\pi x/3}', '\\dots', '\\frac{\\sin(\\pi x/13)}{\\pi x/13}', '\\frac{\\sin(\\pi x/15)}{\\pi x/15}', 'dx = ', f'{SUB_ONE_FACTOR}\\dots', '.'))
        result[-1][-1].scale(0)
        for mob in result:
            for tex, color in t2c.items():
...
class InsertTwoCos(InteractiveScene):
    ...
    def construct(self):
        ...
        integrals = VGroup(get_multi_sinc_integral([1], rhs='= \\pi'), *(get_multi_sinc_integral(range(1, n, 2), insertion='2\\cos(x)', rhs='= \\pi') for n in n_range))
        ...
        axes.x_axis.add_numbers(unit_tex='\\pi', unit=PI)
        graphs = VGroup(axes.get_graph(sinc), *(axes.get_graph(lambda x, n=n: 2 * math.cos(x) * multi_sinc(x, n + 1), x_range=(-4 * PI, 4 * PI, 0.01)) for n in range(len(n_range))))
        graphs.set_stroke(WHITE, 2)
        areas = VGroup(*(axes.get_riemann_rectangles(graph, dx=self.dx, colors=(BLUE_D, BLUE_D), fill_opacity=1) for graph in graphs))
        self.add(areas[0], axes, graphs[0])
        for i in range(len(integrals) - 1):
            self.play(TransformMatchingTex(integrals[i], integrals[i + 1]), ReplacementTransform(graphs[i], graphs[i + 1]), ReplacementTransform(areas[i], areas[i + 1]))
...
class WriteTwoCosPattern(InteractiveScene):
    ...
    def construct(self):
        integrals = VGroup(*(get_multi_sinc_integral(range(1, n, 2), insertion='2\\cos(x)', rhs='= \\pi') for n in range(1, 7, 2)), OldTex('\\vdots'), get_multi_sinc_integral([1, 3, 5, 111], dots_at=5, insertion='2\\cos(x)', rhs='= \\pi'), get_multi_sinc_integral([1, 3, 5, 111, 113], dots_at=5, insertion='2\\cos(x)', rhs='= \\pi'))
        integrals.scale(0.75)
        for inter in integrals:
            inter[-1].scale(1.5, about_edge=LEFT)
...
class MovingAverages(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(f1_label_group.animate.set_height(0.7, about_edge=UR), VGroup(axes, rect_graph).animate.to_edge(UP), TransformFromCopy(axes, low_axes))
        self.wait()
        ...
        graphs = [rect_graph, *self.get_all_convolution_graphs(xs, rect_func(xs / rs), low_axes, k_range)]
        for graph in graphs:
        ...
        for n in range(1, self.n_iterations):
            ...
            self.play(GrowFromCenter(brace), GrowFromCenter(plateau), Write(brace_label, stroke_color=WHITE), run_time=anim_time)
            ...
            if n < self.n_iterations - 1:
                ...
                shift_value = ORIGIN
                anims = []
            anims.extend([*(mob.animate.shift(shift_value) for mob in [low_axes, graphs[n], fn_label, fn0_label]), FadeOut(VGroup(brace, brace_label), shift_value)])
            self.play(*anims)
            prev_graph = graphs[n]
        ...
        graphs[0].add(Line(graphs[0].axes.c2p(-0.5, 1), graphs[0].axes.c2p(0.5, 1), stroke_color=YELLOW, stroke_width=3.0))
        ...
        self.play(MoveToTarget(graph_groups), FadeOut(graphs[7].func_labels[0]), FadeOut(graphs[6].func_labels[0]))
        self.wait()
    ...
    def get_all_convolution_graphs(self, xs, ys, axes, k_range):
        ...
        for k in k_range:
            ...
            kernel /= sum(kernel)
            func_samples.append(np.convolve(func_samples[-1], kernel, mode='same'))
            new_graph = VMobject()
    ...
    def show_moving_average(self, top_axes, low_axes, low_graph, k):
        rs = self.rect_scalar
        window = Rectangle(width=top_axes.x_axis.unit_size / k, height=top_axes.y_axis.unit_size * 1.5)
        ...
        drawing_dot.add_updater(lambda m, w=window, g=low_graph.copy(): m.move_to(g.quick_point_from_proportion((low_axes.x_axis.p2n(w.get_center()) + rs) / (2 * rs))))
        drawing_dot.set_glow_factor(1)
        drawing_dot.set_radius(0.15)
        self.add(drawing_dot)
        self.play(ShowCreation(low_graph), window.animate.move_to(top_axes.c2p(rs, 0), DOWN), rate_func=bezier([0, 0, 1, 1]), run_time=2.5 if self.quick_transitions else 5)
        self.wait()
    ...
    def get_axes(self, x_range=(-1, 1, 0.25), y_range=(0, 1, 0.25), width=13.0, height=2.5):
        ...
        axes.add_coordinate_labels(x_values=np.arange(*x_range)[::2], y_values=np.arange(0, 1.0, 0.5), num_decimal_places=2, font_size=20)
        return axes
...
class ShowReciprocalSums(InteractiveScene):
    ...
    def construct(self):
        ...
        for equation in equations:
            t2c = dict(zip([f'1 / {2 * k + 1}' for k in range(1, 10)], color_gradient([BLUE, YELLOW, RED], 8)))
...
class LongerReciprocalSums(ShowReciprocalSums):
    ...
    def construct(self):
        ...
        dots.shift(2 * LEFT)
        for eq in equations:
            eq.set_color_by_tex_to_color_map({'1 / 3': BLUE, '1 / 5': interpolate_color(BLUE, GREEN, 1 / 3), '1 / 7': interpolate_color(BLUE, GREEN, 2 / 3), '1 / 9': GREEN, '1 / 109': YELLOW, '1 / 111': interpolate_color(YELLOW, RED, 1 / 2), '1 / 113': RED})
        self.add(equations[0])
        for eq1, eq2 in zip(equations, equations[1:]):
...
class MoreGeneralFact(InteractiveScene):
    ...
    def construct(self):
        inters = VGroup(*(get_multi_sinc_integral(range(1, n, 2), rhs='= \\pi') for n in range(3, 11, 2)))
        inters.scale(0.75)
        inters.to_edge(UP)
        axes = Axes((-4 * PI, 4 * PI, PI), (-0.5, 1, 0.5), width=FRAME_WIDTH + 1, height=4)
        axes.x_axis.add_numbers(unit=PI, unit_tex='\\pi')
        axes.to_edge(DOWN)
        graphs = VGroup(*(axes.get_graph(lambda x, n=n: multi_sinc(x, n)) for n in range(1, 5)))
        graphs.set_stroke(WHITE, 2)
        areas = VGroup(*(axes.get_riemann_rectangles(graph, colors=(BLUE_D, BLUE_D), dx=self.dx, fill_opacity=1) for graph in graphs))
        for area in areas:
            area.sort(lambda p: abs(p[0]))
        self.add(areas[0], axes, graphs[0], inters[0])
        for i in range(3):
            self.play(ReplacementTransform(areas[i], areas[i + 1]), TransformMatchingTex(inters[i], inters[i + 1]), ReplacementTransform(graphs[i], graphs[i + 1]))
        ...
        general_group = OldTex('\\frac{\\sin(a_1 x)}{a_1 x}', '\\cdots', '\\frac{\\sin(a_n x)}{a_n x}')
        ...
        self.play(ShowCreation(rect), GrowFromCenter(brace), FadeIn(brace_text, 0.5 * DOWN))
        self.wait()
        self.play(FadeOut(inner_group, UP), FadeIn(general_group, UP), rect.animate.match_points(SurroundingRectangle(general_group, buff=SMALL_BUFF)), brace.animate.become(Brace(general_group, DOWN, buff=MED_SMALL_BUFF)), FadeOut(brace_text), inters[-1][5:].animate.next_to(general_group, RIGHT, SMALL_BUFF))
        self.wait()
        sum_tex = brace.get_tex('a_1 + \\cdots + a_n < 1')[0]
        lt = OldTex('<')
        eq = inters[-1][-1][0]
        lt.move_to(eq)
        ...
        self.play(FadeOut(eq, 0.5 * UP), FadeIn(lt, 0.5 * UP))
        self.wait()

class WaysToCombineFunctions(InteractiveScene):

    def construct(self):
        axes1, axes2, axes3 = all_axes = VGroup(*(Axes((-5, 5), (0, 2), width=FRAME_WIDTH - 2, height=FRAME_HEIGHT / 3 - 1.0) for x in range(3)))
        ...
        f_label = OldTex('f(x) = \\sin(x) + 1', color=BLUE)[0]
        g_label = OldTex('g(x) = e^{-x^2}', color=YELLOW)[0]
        f_label.move_to(axes1.c2p(-1.5, 1.5))
        g_label.move_to(axes2.c2p(-1.5, 1.5))
        self.play(LaggedStart(FadeIn(f_graph), FadeIn(g_graph)), LaggedStart(FadeIn(f_label), FadeIn(g_label)))
        self.wait()
        ...
        prod_label = OldTex('[f \\cdot g](x)')
        conv_label = OldTex('[f * g](x)')
        ...
        for graph, label, word in zip(graphs, labels, words):
            self.play(Transform(f_graph.copy(), graph.copy(), remover=True), TransformFromCopy(g_graph, graph), TransformMatchingShapes(VGroup(*f_label[:4], *g_label[:4]).copy(), label), FadeIn(word))
            self.wait()
...
class ReplaceXWithPiX(InteractiveScene):
    ...
    def construct(self):
        ...
        dx = 0.01
        sinc_area = axes.get_riemann_rectangles(sinc_graph, dx=dx, colors=(BLUE_E, BLUE_E), negative_color=RED_E, fill_opacity=1.0)
        ...
        sinc_label = Tex('\\int_{-\\infty}^\\infty \\frac{\\sin(x)}{x} dx = \\pi')
        sinc_label.next_to(axes, UP).to_edge(LEFT)
        kw = dict(tex_to_color_map={'\\pi': TEAL})
        sinc_pi_label = Tex('\\int_{-\\infty}^\\infty \\frac{\\sin(\\pi x)}{\\pi x} dx = 1.0', **kw)
        ...
        self.wait()
        x_to_pix = Tex('x \\rightarrow \\pi x', **kw)
        ...
        self.play(LaggedStart(FadeIn(x_to_pix), TransformMatchingShapes(sinc_label.copy(), sinc_pi_label), FadeTransform(instead_of.copy(), focus_on)))
        self.wait()
        self.play(Transform(sinc_graph, sinc_pi_graph), Transform(sinc_area, sinc_pi_area), FadeIn(squish_arrows, scale=0.35), run_time=2)
        self.wait()
        self.play(ShowCreation(one_rect))
        self.wait()
        self.play(ShowCreation(rect))
        self.wait()
        self.play(FadeOut(squish_arrows, scale=3), sinc_area.animate.restore(), sinc_graph.animate.restore(), rect.animate.become(SurroundingRectangle(VGroup(sinc_label, want_to_show), **rect_kw)))
        self.wait()
...
class WhatWeNeedToShow(InteractiveScene):
    ...
    def construct(self):
        ...
        t2c = {'\\mathcal{F}': TEAL, '{t}': BLUE, '{\\omega}': YELLOW, '{k}': RED}
        kw = dict(tex_to_color_map=t2c, font_size=36)
        expressions = VGroup(Tex('\\mathcal{F}\\left[\\frac{\\sin(\\pi {t})}{\\pi {t}} \\right]({\\omega}) = \\text{rect}({\\omega})', **kw), Tex('\\mathcal{F}\\left[\\frac{\\sin(\\pi {t} / {k})}{\\pi {t} / {k}} \\right]({\\omega}) = {k} \\cdot \\text{rect}({k}{\\omega})', **kw), Tex('\\int_{-\\infty}^\\infty f({t}) dt = \\mathcal{F}\\left[ f({t}) \\right](0)', **kw), Tex('\\int_{-\\infty}^\\infty \\frac{\\sin(\\pi {t})}{\\pi {t}} dt = \\text{rect}(0) = 1', **kw), Tex('\\mathcal{F}\\left[ f({t}) \\cdot g({t}) \\right] = \\mathcal{F}[f({t})] * \\mathcal{F}[g({t})]', **kw), Tex('\\mathcal{F}\\left[ \\frac{\\sin(\\pi {t})}{\\pi {t}} \\cdot \\frac{\\sin(\\pi {t} / 3)}{\\pi {t} / 3} \\right]\n                = \\big[ \\text{rect} * \\text{rect}_3 \\big]', **kw))
        ...
        axes2.add(OldTex('\\omega', color=YELLOW, font_size=24).next_to(axes2.x_axis.get_right(), UP, 0.2))
        ...
        def update_graph2(graph):
            k = get_k()
            graph.set_points_as_corners([axes2.c2p(-1, 0), axes2.c2p(-0.5 / k, 0), axes2.c2p(-0.5 / k, k), axes2.c2p(0.5 / k, k), axes2.c2p(0.5 / k, 0), axes2.c2p(1, 0)])
            return graph
        graph2.add_updater(update_graph2)
        graph1_label = Tex('{\\sin(\\pi {t}) \\over \\pi {t} }', **kw)
        graph2_label = Tex('\\text{rect}({\\omega})', **kw)
        ...
        arrow_label = Tex('\\mathcal{F}', color=TEAL)
        ...
        self.wait()
        self.play(ShowCreation(arrow), FadeIn(arrow_label, RIGHT + 0.2 * UP), FadeIn(axes2))
        self.play(Write(graph2_label), ShowCreation(graph2))
        self.wait()
        self.play(TransformFromCopy(arrow, arrow_copy, path_arc=PI / 2), TransformFromCopy(arrow_label, arrow_label_copy, path_arc=PI / 2))
        self.wait()
        self.play(LaggedStart(FadeTransform(arrow_label.copy(), ft_sinc[0]), FadeTransform(graph1_label.copy(), ft_sinc[2:12]), Write(VGroup(ft_sinc[1], ft_sinc[12]))))
        self.wait()
        self.play(Write(ft_sinc[13:17]))
        self.play(FadeTransform(graph2_label.copy(), ft_sinc[17:]))
        self.add(ft_sinc)
        self.wait()
        graph1_gen_label = Tex('{\\sin(\\pi {t} / {k}) \\over \\pi {t} / {k} }', **kw)
        graph2_gen_label = Tex('{k} \\cdot \\text{rect}({k} {\\omega})', **kw)
        graph1_gen_label.move_to(graph1_label)
        graph2_gen_label.move_to(graph2_label)
        ft_sinck.move_to(ft_sinc)
        self.play(LaggedStart(FadeOut(graph1_label, UP), FadeIn(graph1_gen_label, UP), FadeOut(graph2_label, UP), FadeIn(graph2_gen_label, UP)))
        ...
        thick_graph = VGroup(axes1.get_graph(np.sinc, x_range=(-1, 4)), axes1.get_graph(np.sinc, x_range=(-4, 1)).reverse_points())
        thick_graph.set_stroke(YELLOW, 6)
        self.play(FadeIn(dots, DOWN, lag_ratio=0.5, run_time=5))
        self.wait()
        self.play(VShowPassingFlash(thick_graph[0], run_time=4, time_width=1), VShowPassingFlash(thick_graph[1], run_time=4, time_width=1), FadeOut(area))
        self.wait()
        ...
        self.wait()
        self.play(FadeIn(conv_theorem_ex, DOWN), FadeOut(conv_theorem, DOWN))
        self.wait()
        self.play(FadeOut(conv_theorem_ex, UP), FadeIn(conv_theorem, UP))
        ...
        self.play(MoveToTarget(facts), title.animate.next_to(facts.target, LEFT, LARGE_BUFF), Uncreate(underline), FadeOut(conv_theorem_name), Write(bullets))
        self.wait()
...
class ConvolutionTheoremDiagram(InteractiveScene):
    ...
    def construct(self):
        ...
        left_graphs = VGroup(left_axes[0].get_graph(np.sinc, color=BLUE), left_axes[1].get_graph(lambda x: np.sinc(x / 2), color=YELLOW), left_axes[2].get_graph(lambda x: np.sinc(x) * np.sinc(x / 2), color=GREEN))
        left_graphs.set_stroke(width=2)
        right_graphs = VGroup(VMobject().set_points_as_corners([right_axes[0].c2p(x, y) for x, y in [(-1, 0), (-0.5, 0), (-0.5, 1), (0.5, 1), (0.5, 0), (1, 0)]]).set_stroke(BLUE, 2), VMobject().set_points_as_corners([right_axes[1].c2p(x, y) for x, y in [(-1, 0), (-0.5 / 2, 0), (-0.5 / 2, 2), (0.5 / 2, 2), (0.5 / 2, 0), (1, 0)]]).set_stroke(YELLOW, 2), VMobject().set_points_as_corners([right_axes[2].c2p(x, y) for x, y in [(-1, 0), (-0.75, 0), (-0.25, 1), (0.25, 1), (0.75, 0), (1, 0)]]).set_stroke(GREEN, 2))
        ...
        left_labels = VGroup(OldTex('\\frac{\\sin(\\pi x)}{\\pi x}')[0], OldTex('\\frac{\\sin(\\pi x / 2)}{\\pi x / 2}')[0], OldTex('\\frac{\\sin(\\pi x)}{\\pi x} \\cdot \\frac{\\sin(\\pi x / 2)}{\\pi x / 2}')[0])
        right_labels = VGroup(*(OldTex(f'\\mathcal{{F}}\\left[{ll.get_tex()}\\right]', tex_to_color_map={'\\mathcal{F}': TEAL}) for ll in left_labels))
        ...
        ft_arrows = VGroup(*(Arrow(l1.get_right(), l2.get_left(), buff=0.2, path_arc=arc, color=TEAL, stroke_width=3) for l1, l2, arc in zip(left_labels, right_labels, (-0.3, -0.75, -1.0))))
        self.play(LaggedStartMap(FadeIn, left_plots[:2], lag_ratio=0.5), LaggedStartMap(FadeIn, left_labels[:2], lag_ratio=0.5), run_time=1)
        self.wait()
        self.play(Transform(left_plots[0].copy(), left_plots[2].copy(), remover=True), TransformFromCopy(left_plots[1], left_plots[2], remover=True), FadeTransform(left_labels[0].copy(), left_labels[2][:len(left_labels[0])]), FadeTransform(left_labels[1].copy(), left_labels[2][len(left_labels[0]):]))
        self.add(left_plots[2])
        self.wait()
        self.play(ShowCreation(ft_arrows[2]), FadeTransform(left_labels[2].copy(), right_labels[2]), FadeIn(right_plots[2]))
        self.wait()
        for i in range(2):
            self.play(ShowCreation(ft_arrows[i]), FadeTransform(left_labels[i].copy(), right_labels[i]), FadeIn(right_plots[i]))
        self.wait()
        right_labels[2].generate_target()
        equation = VGroup(right_labels[2].target, OldTex('=').scale(0.75), right_labels[0].copy(), OldTex('*').scale(0.75), right_labels[1].copy())
        equation.arrange(RIGHT, buff=SMALL_BUFF)
        equation.next_to(right_axes[2], UP, SMALL_BUFF)
        self.play(LaggedStart(ft_arrows[2].animate.put_start_and_end_on(ft_arrows[2].get_start(), right_labels[2].target.get_left() + SMALL_BUFF * UL), MoveToTarget(right_labels[2]), Write(equation[1]), FadeTransform(right_labels[0].copy(), equation[2]), Write(equation[3]), FadeTransform(right_labels[1].copy(), equation[4]), run_time=2))
        ...
        self.play(rect.animate.move_to(right_axes[0].c2p(-1, 0), DOWN).set_fill(opacity=0.5), FadeIn(dot))
        self.play(MoveAlongPath(dot, right_graphs[2]), UpdateFromFunc(rect, lambda m: m.match_x(dot)), run_time=8)
        self.play(FadeOut(rect), FadeOut(dot))
        area = left_axes[2].get_riemann_rectangles(left_graphs[2], dx=0.01, colors=(BLUE_D, BLUE_D), fill_opacity=1.0)
        o = area.get_center()
        ...
        self.play(MoveAlongPath(dot, right_graphs[2], rate_func=lambda t: smooth(1 - 0.5 * t)), MoveAlongPath(dot.copy(), right_graphs[2], rate_func=lambda t: smooth(0.5 * t)), run_time=1)
...
class MultiplyBigNumbers(InteractiveScene):
    ...
    def construct(self):
        numbers = VGroup(Tex('3{,}141{,}592{,}653{,}589{,}793{,}238'), Tex('2{,}718{,}281{,}828{,}459{,}045{,}235'))
        ...
        d2_label = OldTex('\\mathcal{O}(N^2)', font_size=60, tex_to_color_map={'N': YELLOW})
        dlogd_label = OldTex('\\mathcal{O}({N} \\cdot \\text{log}({N}))', font_size=60, tex_to_color_map={'{N}': YELLOW})
        ...
        for num in numbers:
            ...
            num.digit_highlights = VGroup(*(VHighlight(digit, color_bounds=(YELLOW, YELLOW_E), max_stroke_addition=8) for digit in num.digits))
        self.add(d_label)
        self.play(FadeIn(d2_label, UP), FadeIn(q_marks, UP))
        for dh2 in numbers[1].digit_highlights:
            self.add(dh2, numbers[1])
            self.play(ShowSubmobjectsOneByOne(numbers[0].digit_highlights.copy(), remover=True), run_time=1.0)
            self.remove(dh2)
        self.play(ShowCreation(cross), FadeOut(q_marks), FadeTransform(d2_label.copy(), dlogd_label))
...
class Graph15Case(ShowIntegrals):
    ...
    def construct(self):
        axes = self.get_axes(x_range=(-3 * PI, 3 * PI, PI), y_range=(0, 1), width=1.0 * FRAME_WIDTH)
