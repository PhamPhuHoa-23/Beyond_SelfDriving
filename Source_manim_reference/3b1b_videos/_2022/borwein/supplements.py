...
class ExpressAnger(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.change('raise_left_hand', look_at=3 * RIGHT + UP), self.change_students('pondering', 'confused', 'thinking', look_at=3 * RIGHT + UP))
        self.wait(2)
        self.play(self.teacher.change('tease', look_at=3 * RIGHT + UP), self.change_students('angry', 'erm', 'pleading', look_at=3 * RIGHT + UP))
        self.wait(5)

class LooksCanBeDeceiving(TeacherStudentsScene):

    def construct(self):
        self.play(self.teacher.says('Looks can be\ndeceiving', mode='tease'), self.change_students('angry', 'erm', 'pleading', look_at=self.screen))
        self.wait(3)
...
class AstuteAmongYou(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(stds[2].says(OldTexText('What about\\\\$x=0$?'), bubble_direction=LEFT, mode='raise_right_hand'), stds[0].change('hesitant'), stds[1].change('pondering'), morty.change('happy', stds[2].eyes))
        self.wait(2)
        self.play(stds[0].says('The squeeze theorem!', mode='hooray'), RemovePiCreatureBubble(stds[2]))
        self.look_at(stds[0].get_corner(UR))
        self.wait(3)
        point = stds.get_top() + 2 * UP
        self.play(stds[0].debubble(mode='raise_right_hand', look_at=point), stds[1].change('thinking', point), stds[2].change('erm', point), morty.change('tease', point))
        self.wait(5)
...
class AreaToSignedArea(InteractiveScene):
    ...
    def construct(self):
        ...
        blue_minus_red = OldTexText('= (Blue area) - (Red area)', tex_to_color_map={'(Blue area)': BLUE, '(Red area)': RED})
...
class HoldOffUntilEnd(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.says("I'll mention the\ntrick at the end"), self.change_students('confused', 'hesitant', 'maybe', look_at=self.screen))
        self.wait()
...
class WhatsThePoint(TeacherStudentsScene):
    ...
    def construct(self):
        stds = self.students
        self.play(stds[2].says(OldTexText("So it computes $\\pi$ \\\\ what's the point?"), bubble_direction=LEFT, mode='angry'), stds[1].change('sassy', self.screen), stds[0].change('pondering', self.screen), self.teacher.change('guilty'))
        self.wait(2)
        self.play(self.teacher.says('Look what \n happens next', mode='tease', bubble_direction=RIGHT), stds[2].debubble(), stds[1].change('erm'), stds[0].change('thinking'))
        self.wait(2)
...
class MovingAverageFrames(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(screens)
        titles = VGroup(Text('Step 1:\nAn easier analogy', alignment='LEFT'), Text("Step 2:\nReveal it's more than an analogy", alignment='LEFT'))
...
class XInMovingAverageGraphs(MovingAverages):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeOut(x_label), FadeOut(arrow), GrowFromCenter(brace), FadeIn(brace_text, 0.5 * DOWN))
        self.wait()
...
class ThisConstant(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(same, 0.5 * UP), FadeOut(words1.select_part('This'), 0.5 * UP))
...
class HeavyMachinery(TeacherStudentsScene):
    ...
    def construct(self):
        stds = self.students
        morty = self.teacher
        self.play(stds[2].says('Why on earth are\nthose two related?', bubble_direction=LEFT, look_at=self.screen), morty.change('tease'), stds[0].change('pleading', self.screen), stds[1].change('confused', self.screen))
        self.wait(2)
        ...
        conv_thm = OldTexText('The ', 'Convolution', '\\\\Theorem', **kw)
        for text in (ft, conv, conv_thm):
            text.move_to(self.hold_up_spot, DOWN)
        self.play(morty.change('raise_right_hand'), FadeIn(ft, UP), stds[2].debubble(mode='guilty', look_at=ft), stds[0].change('erm', look_at=ft), stds[1].change('sad', look_at=ft))
        self.wait()
        ...
        ft.save_state()
        conv.save_state()
        self.play(morty.says('First, a high\nlevel overview', bubble_direction=RIGHT), self.change_students('skeptical', 'erm', 'pondering'), VGroup(ft, conv).animate.scale(0.55).to_corner(UR))
        self.wait(3)
        self.play(morty.debubble(mode='raise_right_hand'), conv.animate.restore(), self.change_students('pondering', 'thinking', 'thinking', look_at=self.screen))
        self.wait(4)
        self.play(TransformMatchingTex(conv, conv_thm), morty.change('hooray', conv_thm))
...
class EngineersSinc(InteractiveScene):
    ...
    def construct(self):
        sinc = OldTex('{\\text{sin}(\\pi x) \\over \\pi x}', tex_to_color_map={'\\pi': TEAL})
        sinc.shift(UP)
...
class ThinkAboutMovingAverages(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        self.play(morty.change('raise_right_hand', self.screen), self.change_students('pondering', 'thinking', 'happy', look_at=self.screen))
        self.wait(3)
        self.play(morty.says('Think about\nmoving averages'))
...
class ConceptAndNotationFrames(InteractiveScene):
    ...
    def construct(self):
        ...
        screens.move_to(0.5 * DOWN)
        self.add(screens)
        titles = VGroup(Text('Moving average fact'), Text('In our new notation'), Text('How does this...'), Text('...relate to this'))
...
class SubPiComment(InteractiveScene):
    ...
    def construct(self):
        morty = Mortimer(height=2).flip()
        morty.to_corner(DL)
        self.play(morty.says(OldTex('x \\rightarrow \\pi x', tex_to_color_map={'\\pi': TEAL})))
...
class HowDoYouCompute(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(morty.change('guilty'), stds[0].change('confused', self.screen), stds[1].change('maybe', self.screen))
        self.wait(2)
        self.play(stds[2].debubble(), morty.says("We'll get there"))
...
class TranslateToFourierLand(InteractiveScene):
    ...
    def construct(self):
        ...
        left_graphs = VGroup(left_axes[0].get_graph(lambda x: math.sin(3 * x)), left_axes[1].get_graph(lambda x: math.sin(4 * x)), left_axes[2].get_graph(lambda x: math.sin(3 * x) + math.sin(4 * x)))
        ...
        right_graphs = VGroup(right_axes[0].get_graph(lambda x: bell(x, 3)), right_axes[1].get_graph(lambda x: bell(x, 4)), right_axes[2].get_graph(lambda x: bell(x, 3) + bell(x, 4)))
        right_graphs.match_style(left_graphs)
        right_symbols = left_symbols.copy().match_x(right_rect).shift(0.5 * DOWN)
        self.play(LaggedStartMap(FadeIn, left_axes, lag_ratio=0.7), LaggedStartMap(ShowCreation, left_graphs, lag_ratio=0.7), LaggedStartMap(Write, left_symbols, lag_ratio=0.7), run_time=3)
        self.wait()
        self.play(LaggedStart(TransformFromCopy(left_axes, right_axes), TransformFromCopy(left_graphs, right_graphs), TransformFromCopy(left_symbols, right_symbols)))
        self.wait(3)
        to_fade = VGroup(left_axes, left_graphs, left_symbols, right_axes, right_graphs, right_symbols)
        property_pairs = [VGroup(OldTex('\\int_{-\\infty}^\\infty f(t) dt'), OldTex('\\hat f(0)')), VGroup(OldTex('f(t) \\cdot g(t)'), OldTex('\\int_{-\\infty}^\\infty \\hat f(\\xi) \\hat g(\\omega - \\xi) d\\xi', tex_to_color_map={'\\omega': YELLOW})), VGroup(OldTex('\\sum_{n = -\\infty}^\\infty f(n)'), OldTex('\\sum_{k = -\\infty}^\\infty \\hat f(k)'))]
...
class TranslatedByFourier(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait(2)
        self.play(LaggedStart(GrowArrow(arrow), FadeIn(arrow_label, lag_ratio=0.1), FadeTransform(left_expr.copy(), right_expr, path_arc=-PI / 5), lag_ratio=0.5))
        self.wait(2)
        self.wait()
...
class UnsatisfyingEnd(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        morty = self.teacher
        stds = self.students
        self.play(stds[0].says("But you haven't explained\nwhy any of that is true?", mode='angry'), stds[1].change('hesitant'), stds[2].change('pleading'), morty.change('guilty'))
        stds[0].bubble.insert_n_curves(20)
        self.wait(3)
        self.play(morty.says('Patience.', mode='tease'), stds[0].debubble(), stds[2].change('hesitant'))
        self.wait(2)
