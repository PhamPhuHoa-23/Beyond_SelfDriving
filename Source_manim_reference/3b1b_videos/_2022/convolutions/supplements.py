...
class HoldUpLists(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.change('raise_right_hand', look_at=self.screen), self.change_students('pondering', 'thinking', 'pondering', look_at=self.screen))
        self.wait()
        self.play(self.change_students('thinking', 'thinking', 'pondering', look_at=3 * UR), self.teacher.change('raise_left_hand', look_at=3 * UR))
        self.wait(2)
...
class FunToVisualize(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.says('Boy are they fun\nto visualize!', mode='hooray'), self.change_students('tease', 'happy', 'well'))
        self.wait(3)

class ILearnedSomething(TeacherStudentsScene):

    def construct(self):
        implication = VGroup(Text('Making\nvisuals'), OldTex('\\Rightarrow', font_size=60), Text('Deeper\nunderstanding'))
        implication.arrange(RIGHT, buff=0.4)
        implication.move_to(self.hold_up_spot, DOWN)
        implication.shift_onto_screen()
        self.play(self.teacher.change('raise_right_hand', implication), FadeIn(implication, UP), self.change_students('confused', 'hesitant', 'tease', look_at=implication))
        self.wait(5)
...
class NormalFunctionPreview(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(axes3d), frame.animate.reorient(-10, 70).set_anim_args(run_time=2), FadeIn(surface, time_span=(1, 2)), FadeIn(label3d), FadeOut(label), Rotate(curve, PI, run_time=3))
...
class Intimidation(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(LaggedStart(*(pi.change(mode, look_at=3 * UR) for pi, mode in zip(pis, ('horrified', 'maybe', 'pleading')))))
        for x in range(2):
...
class ThereIsAnother(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.says('There is another'), self.change_students('pondering', 'thinking', 'erm', look_at=self.screen))
        self.wait(2)
...
class SharedInsights(InteractiveScene):
    ...
    def construct(self):
        ...
        arrows = VGroup(Arrow(rects[3].get_top(), rects[1].get_bottom(), **kw), Arrow(rects[3].get_corner(UL), rects[0].get_corner(DR), **kw), Arrow(rects[3].get_left(), rects[2].get_right(), **kw))
        self.wait()
...
class HoldUpImageProcessing(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.change('raise_right_hand'), self.change_students('pleading', 'hesitant', 'horrified', look_at=2 * UP))
        self.wait(2)
...
class OtherVisualizations(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.says('Can you think of\nother visualizations?'), self.change_students('pondering', 'pondering', 'pondering', look_at=self.screen))
        self.wait(2)
        self.play(self.change_students('confused', 'erm', 'thinking'))
        self.wait(4)
        self.play(self.students[2].change('raise_right_hand', self.teacher.eyes), self.teacher.change('tease'))
        self.wait(2)

class Boring(TeacherStudentsScene):

    def construct(self):
        self.play(LaggedStart(self.students[2].says('Boring!', mode='dejected', look_at=self.teacher.eyes, bubble_direction=LEFT), self.teacher.change('hesitant'), self.students[0].change('hesitant', self.screen), self.students[1].change('guilty', self.screen), lag_ratio=0.25))
        self.wait(3)

class AskForExample(TeacherStudentsScene):

    def construct(self):
        self.play(self.teacher.change('raise_right_hand', self.screen), self.change_students('pondering', 'thinking', 'confused', look_at=self.screen))
        self.wait(3)
        self.play(self.students[2].says('Can we go do\na concrete example?', mode='raise_left_hand', bubble_direction=LEFT))
        self.wait(3)
...
class MarioConvolutionLabel(BoxBlurMario):
    ...
    def construct(self):
        ...
        if self.label_conv:
            self.play(Write(conv_label), ShowCreation(arrow))
        self.wait()
...
class ThinkDifferently(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.change('raise_right_hand'), self.change_students('pondering', 'confused', 'thinking', look_at=self.screen))
        self.wait()
        self.play(self.teacher.change('tease'))
...
class ThisIsTheCoolPart(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.says('Now for the\ncool part!', mode='hooray'), self.change_students('happy', 'tease', 'well'))
        self.wait(2)
        self.play(self.change_students('thinking', 'thinking', 'happy', look_at=self.screen), self.teacher.change('tease'))
        self.wait(3)
...
class MentionONSquared(InteractiveScene):
    ...
    def construct(self):
        ...
        bigO = Tex('\\mathcal{O}({N}^2)', **kw)
        explanation = VGroup(Text('# Operations'), OldTex('=').rotate(PI / 2), OldTex('\\text{const}', '\\cdot {N}^2 +', '\\left(\\substack{\\text{stuff asymptotically} \\\\ \\text{smaller than $N^2$}}\\right)', **kw))
        ...
        self.play(FadeIn(explanation, 0.5 * UP), morty.change('raise_left_hand', look_at=explanation))
        for x in range(2):
            self.play(Blink(morty))
            self.wait(2)
...
class DumbIdea(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(morty.change('happy', look_at=self.screen), self.change_students('pondering', 'pondering', 'frustrated', look_at=self.screen))
        self.wait()
        self.student_says('Uh...', target_mode='hesitant', bubble_direction=LEFT, index=2)
        self.play(Blink(stds[2]))
        self.student_says("That's idiotic!", target_mode='angry', index=2, added_anims=[morty.change('guilty'), stds[1].change('erm')])
        self.wait(2)
        self.student_says(OldTexText('Calculating the samples\\\\is already $\\mathcal{O}(N^2)$'), target_mode='concentrating', look_at=self.screen, added_anims=[stds[0].change('erm'), stds[1].change('hesitant')])
        self.wait(5)
        self.student_says('And so is solving\nthe linear system!', target_mode='surprised', look_at=morty.eyes, added_anims=[morty.change('tease'), stds[0].change('sassy'), stds[1].change('hesitant')])
        self.wait()
        self.play(stds[2].change('angry'))
        self.wait(4)
        new_point = 3 * UR
        self.play(stds[2].debubble(mode='raise_right_hand', look_at=new_point), stds[0].change('pondering', look_at=new_point), stds[1].change('pondering', look_at=new_point))
        self.wait(3)
        self.play(self.change_students('erm', 'hesitant', 'angry'))
        self.wait(3)
        self.play(morty.says("But there's\na trick"), self.change_students('tease', 'happy', 'hesitant'))
        self.wait(2)
        fft = Text('FFT')
        fft.next_to(stds[0].get_corner(UR), UP)
        self.play(stds[0].change('raise_right_hand'), FadeIn(fft, UP), morty.change('tease', fft), *(pi.animate.look_at(fft) for pi in self.students[1:]))
        self.wait()
        self.student_says('What...is that?', target_mode='dance_3', index=1)
...
class EnthusiasticAboutRunTime(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.change('raise_right_hand'), FadeIn(run_time, UP), self.change_students('thinking', 'tease', 'happy', look_at=run_time))
        self.wait(2)
        self.play(self.teacher.says('That feels\nlike magic!', mode='hooray'), run_time.animate.to_edge(UP), self.change_students('tease', 'happy', 'tease'))
        self.wait(3)