...
def get_pair_rects(pairs, together=True, buff=SMALL_BUFF, corner_radius=0.1):
    if together:
        ...
        return VGroup(*(VGroup(*(SurroundingRectangle(m, buff=buff).round_corners(corner_radius) for m in pair)) for pair in pairs))
...
def get_lagrange_polynomial(data):
    ...
    def poly(x):
        return sum((y0 * prod((x - x1 for x1, y1 in data if x1 != x0)) / prod((x0 - x1 for x1, y1 in data if x1 != x0)) for x0, y0 in data))
    return poly
...
class WaysToCombine(InteractiveScene):
    ...
    def construct(self):
        ...
        axes1, axes2, axes3 = all_axes = VGroup(*(Axes((-3, 3), (-1, 1), height=2.0, width=FRAME_WIDTH * 0.5 - 1) for x in range(3)))
        ...
        prod_label = Tex('[f \\cdot g](x)', **kw).move_to(axes3.get_corner(UL), UL)
        conv_label = Tex('[f * g](x)', **kw).move_to(axes3.get_corner(UL), UL)
        graph_labels = VGroup(f_label, g_label, sum_label, prod_label, conv_label)
        seq1 = np.array([1, 2, 3, 4])
        seq2 = np.array([5, 6, 7, 8])
        kw = dict(font_size=48, tex_to_color_map={'a': BLUE, 'b': YELLOW}, isolate=[',', '[', ']'])
        seq1_tex, seq2_tex, sum_seq, prod_seq, conv_seq = seqs = VGroup(OldTex(f'a = {list(seq1)}', **kw), OldTex(f'b = {list(seq2)}', **kw), OldTex(f'a + b = {list(seq1 + seq2)}', **kw), OldTex(f'a \\cdot b = {list(seq1 * seq2)}', **kw), OldTex(f'a * b = {list(np.convolve(seq1, seq2))}', **kw))
        ...
        self.play(LaggedStartMap(Write, seqs[:2], **kw), run_time=2)
        self.play(FadeIn(v_line), LaggedStartMap(FadeIn, all_axes[:2], **kw), LaggedStartMap(ShowCreation, VGroup(f_graph, g_graph), **kw), LaggedStartMap(FadeIn, graph_labels[:2], **kw), run_time=2)
        self.wait()
        ...
        self.play(*map(Write, box_arrows), FadeIn(boxes), *(Transform(box.copy().set_stroke(width=0, opacity=0), mystery_box.copy(), path_arc=-PI / 3, remover=True) for box in boxes))
        self.add(mystery_box)
        comb_graph = sum_graph.copy()
        self.play(Write(q_marks), ReplacementTransform(axes1.copy(), axes3), Transform(axes2.copy(), axes3.copy(), remover=True), ReplacementTransform(f_graph.copy(), comb_graph), Transform(g_graph.copy(), comb_graph.copy(), remover=True))
        ...
        tuples = [(sum_seq, sum_label, axes3, sum_graph, op_labels[0]), (prod_seq, prod_label, axes3, prod_graph, op_labels[1])]
        for seq, label, axes, graph, op_label in tuples:
            self.play(LaggedStart(TransformMatchingShapes(VGroup(*seq1_tex[:2], *seq2_tex[:2]).copy(), seq[:4]), TransformMatchingShapes(graph_labels[:2].copy(), label), FadeIn(op_label, DOWN)))
            self.add(axes)
            ...
            self.play(*map(FadeIn, [seq[4], seq[-1]]), *(VFadeInThenOut(rects, lag_ratio=0.5) for rects in value_rects), LaggedStart(*(FadeIn(seq[n:n + 2 if n < 11 else n + 1]) for n in range(5, 12, 2)), lag_ratio=0.5), run_time=2)
            self.play(ShowCreation(graph, rate_func=linear), UpdateFromFunc(dots[2], lambda m: m.move_to(graph.get_end())), *(UpdateFromAlphaFunc(dot, lambda d, a: d.set_opacity(min(10 * a * (1 - a), 1))) for dot in dots), *(MoveAlongPath(dot, graph) for dot, graph in zip(dots, [f_graph, g_graph])), run_time=4)
            self.wait()
            self.play(*map(FadeOut, [seq, label, graph, op_label]))
        self.play(LaggedStart(TransformMatchingShapes(VGroup(*seq1_tex[:2], *seq2_tex[:2]).copy(), conv_seq[:4]), FadeIn(op_labels[2][0], DOWN), TransformMatchingShapes(graph_labels[:2].copy(), conv_label), FadeIn(op_labels[2][1], DOWN)))
...
class DiceExample(InteractiveScene):
    ...
    def construct(self):
        ...
        red_dice.save_state()
        self.play(LaggedStart(FadeIn(blue_dice, lag_ratio=0.1, shift=0.25 * UP, run_time=2, rate_func=overshoot), FadeIn(red_dice, lag_ratio=0.1, shift=0.25 * UP, run_time=2, rate_func=overshoot), lag_ratio=0.2))
        self.wait()
        self.play(Write(grid, run_time=2, stroke_color=YELLOW, stroke_width=4), *(die.animate.set_width(0.6 * square.get_width()).next_to(square, UP, SMALL_BUFF) for die, square in zip(blue_dice, grid[:6])), *(die.animate.set_width(0.6 * square.get_width()).next_to(square, LEFT, SMALL_BUFF) for die, square in zip(red_dice, grid[0::6])))
        mini_dice = VGroup()
        ...
        self.play(LaggedStartMap(MoveToTarget, mini_dice, lag_ratio=0.02, run_time=2), FadeIn(combinations_label[0]), CountInFrom(combinations_label[1], 1, run_time=2), VFadeIn(combinations_label[1]), FadeIn(combinations_label[2], 0))
        self.wait()
        ...
        for n in range(2, 13):
            ...
            num = len(pairs)
            prob_label = self.get_p_sum_expr(n, f'\\frac{{{num}}}{{36}}')
            prob_label.next_to(combinations_label, UP, buff=1.5)
            self.play(FadeIn(prob_label, UP), FadeOut(last_prob_label, UP), to_highlight.animate.set_opacity(1), to_fade.animate.set_opacity(0.2), blue_dice.animate.set_opacity(0.5), red_dice.animate.set_opacity(0.5))
            if n <= 4:
                self.play(LaggedStart(*(FlashAround(pair.copy(), remover=True, time_width=1, run_time=1.5) for pair in pairs), lag_ratio=0.2))
                self.wait()
            last_prob_label = prob_label
            self.wait()
        self.play(FadeOut(grid), FadeOut(mini_dice, lag_ratio=0.01), FadeOut(combinations_label, RIGHT), FadeOut(prob_label, RIGHT), blue_dice.animate.restore(), red_dice.animate.restore())
        self.wait()
        self.play(Rotate(red_dice, PI))
        self.wait()
        last_prob_label = VMobject()
        last_rects = VMobject()
        for n in range(2, 13):
            pairs = self.get_aligned_pairs(blue_dice, red_dice, n)
            prob_label = self.get_p_sum_expr(n, f'\\frac{{{len(pairs)}}}{{36}}')
            prob_label.to_edge(UP)
            self.play(self.get_dice_shift(blue_dice, red_dice, n), FadeOut(last_rects, run_time=0.5))
            rects = get_pair_rects(pairs)
            self.play(FadeOut(last_prob_label, UP), FadeIn(prob_label, UP), LaggedStartMap(ShowCreation, rects, lag_ratio=0.2))
            self.wait()
            last_prob_label = prob_label
            last_rects = rects
        self.play(FadeOut(last_rects), FadeOut(last_prob_label, UP), red_dice.animate.next_to(blue_dice, DOWN, buff=1.5))
        ...
        self.wait()
        self.play(Write(question, run_time=1), FadeOut(sixths, 0.25 * UP, lag_ratio=0.03, run_time=4), FadeIn(new_prob_labels, 0.25 * UP, lag_ratio=0.03, run_time=4), MoveToTarget(all_dice, run_time=3))
        self.wait()
        ...
        while n < 13:
            ...
            self.play(self.get_dice_shift(blue_dice, red_dice, n), FadeOut(last_rects, run_time=0.5), FadeOut(last_prob_label, 0.5 * UP), FadeIn(prob_label, 0.5 * UP))
            rects = self.get_pair_rects(pairs)
            self.play(FadeIn(rects, lag_ratio=0.2))
            self.play(TransformMatchingShapes(VGroup(*(VGroup(blue.prob_label, red.prob_label) for blue, red in pairs)).copy(), rhs))
            self.wait()
            if n == 4 and isinstance(blue_dice[0].prob_label, DecimalNumber):
                ...
                for die, label in zip(all_dice, (*blue_labels, *red_labels)):
                    ...
                    die.add(label)
                self.play(FadeIn(VGroup(*blue_labels, *red_labels), shift=0.25 * UP, lag_ratio=0.04), FadeOut(old_prob_labels, shift=0.25 * UP, lag_ratio=0.04), FadeOut(rhs, time_span=(0, 1)), run_time=4)
            else:
                n += 1
        ...
        self.play(LaggedStartMap(FadeIn, prob_labels[:-1], lag_ratio=0.2, run_time=2), FadeTransform(last_prob_label, prob_labels[-1], time_span=(0.5, 2.0)), MoveToTarget(dice, path_arc=PI / 2), FadeOut(last_rects))
        self.wait()
        ...
        conv_name = OldTexText('Convolution of ', '$(a_i)$', ' and ', '$(b_i)$', tex_to_color_map={'$(a_i)$': BLUE, '$(b_i)$': RED})
        conv_eq = Tex('(a * b)_n = \\sum_{\\substack{i, j \\\\ i + j = n}} a_i \\cdot b_j', isolate=['a_i', '*', 'b_j', '(a * b)_n'])
        conv_label = VGroup(conv_name, conv_eq)
        conv_label.arrange(DOWN, buff=LARGE_BUFF)
        conv_label.scale(0.9)
        conv_label.to_corner(UR)
        self.play(LaggedStartMap(ShowCreationThenFadeOut, VGroup(a_rect, b_rect), lag_ratio=0.5, run_time=3), LaggedStartMap(FadeIn, conv_name[1:5:2], shift=UP, lag_ratio=0.5, run_time=2))
        self.wait()
        ...
        self.play(plusses.animate.set_opacity(0), FadeOut(lhss, LEFT, run_time=0.5, lag_ratio=0.01), MoveToTarget(diagonals), MoveToTarget(blue_dice), MoveToTarget(red_dice), run_time=2)
        ...
        self.play(FadeOut(grid), Restore(rhss), FadeIn(lhss, time_span=(1, 2), lag_ratio=0.01), run_time=2)
        self.wait()
        self.wait(2)
        ...
        self.play(prob_labels[:n].animate.set_opacity(0.35), prob_labels[n + 1:].animate.set_opacity(0.35))
        self.wait()
        self.play(ShowSubmobjectsOneByOne(pair_rects, remover=True, rate_func=linear, run_time=3))
        self.wait()
        alt_rhs = OldTex('\\sum_{i = 1}^6 a_i \\cdot b_{n - i}')
        alt_rhs.scale(0.9)
        alt_rhs.move_to(conv_eq[7], LEFT)
        self.play(FadeIn(alt_rhs, 0.5 * DOWN), conv_eq[7:].animate.shift(1.5 * DOWN).set_opacity(0.5), dice.animate.to_edge(DOWN))
        self.wait()

    def get_p_sum_expr(self, n, rhs=' '):
        raw_expr = Tex(f'P\\big(O + O = {n}\\big) = \\, {rhs}', isolate=('O', str(n), '=', rhs))
...
class SimpleExample(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(TransformMatchingShapes(conv[1:6:2].copy(), top_row), TransformMatchingShapes(conv[9:14:2].copy(), low_row))
        self.wait()
        ...
        self.play(LaggedStart(MoveToTarget(top_row), MoveToTarget(low_row, path_arc=PI), MoveToTarget(conv), Write(VGroup(*rhs[:2], rhs[-1])), FadeOut(question, UP)))
        self.wait()
        c_labels = VGroup()
        for n in range(len(conv_result)):
            ...
            rects = VGroup(*(SurroundingRectangle(lp, buff=0.2).set_stroke(YELLOW, 1).round_corners() for lp in label_pairs))
            self.play(FadeIn(rects, lag_ratio=0.5))
            self.play(LaggedStart(*(TransformFromCopy(lp, nlp) for lp, nlp in zip(label_pairs, new_label_pairs)), lag_ratio=0.5), Write(symbols))
            self.wait()
            anims = [FadeTransform(expr.copy(), rhs[2 * n + 2]), c_labels.animate.set_opacity(0.35), FadeIn(c_label)]
            if n < 4:
        ...
        grid.move_to(DL)
        self.play(Write(grid, time_span=(0.5, 2.0)), LaggedStart(*(value.animate.next_to(square, UP, buff=0.2) for value, square in zip(values1, grid[:3])), *(value.animate.next_to(square, LEFT, buff=0.2) for value, square in zip(values2, grid[0::3])), run_time=2), *(MaintainPositionRelativeTo(block, value) for row, values in [(top_row, values1), (low_row, values2)] for block, value in zip(row, values)), VFadeOut(top_row), VFadeOut(low_row), equation.animate.center().to_edge(UP))
        products = VGroup()
        ...
        self.play(LaggedStart(*(ReplacementTransform(factor.copy(), product) for product in products for factor in product.factors), lag_ratio=0.1))
        self.wait()
        ...
        for n, oval, arrow in zip(it.count(), ovals, arrows):
            self.play(ShowCreation(oval), ovals[:n].animate.set_stroke(opacity=0.25), Transform(curr_arrow, arrow))
            self.wait(0.5)
        self.play(ovals.animate.set_stroke(opacity=0.25), FadeOut(curr_arrow))
        self.wait()
        grid_group = VGroup(grid, values1, values2, products, ovals)
        polynomial_eq = Tex('\\left(1 + 2x + 3x^2\\right)\\left(4 + 5x + 6x^2\\right)={4} + {13}x + {28}x^2 + {27}x^3 + {18}x^4', tex_to_color_map=dict(((f'{{{n}}}', YELLOW) for n in conv_result)))
        ...
        for values, eq_values in [(values1, eq_values1), (values2, eq_values2)]:
            self.play(LaggedStart(*(TransformMatchingShapes(ev.copy(), v.target) for ev, v in zip(eq_values, values))), LaggedStart(*(FadeTransform(v, v.target[0]) for v in values)))
        self.wait()
        old_rects = VGroup()
        for n, prod in enumerate(products):
            new_rects = VGroup(SurroundingRectangle(values1[n % 3].target), SurroundingRectangle(values2[n // 3].target))
            new_rects.set_stroke(GREEN, 2)
            self.play(FadeIn(new_rects), FadeOut(old_rects), FadeTransform(prod, prod.target[:len(prod)]), FadeIn(prod.target[len(prod):], scale=2), FlashAround(prod.target, time_width=1), run_time=1.0)
            old_rects = new_rects
        self.play(FadeOut(old_rects))
        arrows = VGroup(*(Vector(0.5 * UP).next_to(polynomial_eq.select_part(f'{{{n}}}'), DOWN, buff=SMALL_BUFF) for n in conv_result))
        arrows.set_color(YELLOW)
        curr_arrow = arrows[0].copy().shift(DOWN).set_opacity(0)
        for n, oval, arrow in zip(it.count(), ovals, arrows):
            self.play(oval.animate.set_stroke(opacity=1), Transform(curr_arrow, arrow), ovals[:n].animate.set_stroke(opacity=0.25))
            self.wait(0.5)
...
class MovingAverageExample(InteractiveScene):
    ...
    def construct(self):
        ...
        for n, conv_bar in enumerate(conv_bars):
            ...
            rect.set_opacity(0)
            self.play(get_row_shift(top_bars, low_bars, n), FadeOut(last_rects), run_time=self.march_anim_run_time)
            pairs = get_aligned_pairs(top_bars, low_bars, n)
            label_pairs = VGroup(*(VGroup(m1.value_label, m2.value_label) for m1, m2 in pairs))
            rects = VGroup(*(SurroundingRectangle(lp, buff=0.05).set_stroke(YELLOW, 2).round_corners() for lp in label_pairs))
            rects.set_stroke(YELLOW, 2)
            self.play(FadeIn(rects, lag_ratio=0.5), conv_bars[:n].animate.set_opacity(0.5), run_time=self.march_anim_run_time)
            self.play(*(FadeTransform(label.copy(), value) for lp in label_pairs for label in lp), Restore(rect), run_time=self.march_anim_run_time)
            ...
            self.wait(0.5)
            last_rects = rects
        self.play(FadeOut(last_rects), conv_bars.animate.set_opacity(1))
...
class CompareSizes(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(Transform(brace, small_brace), FadeTransform(brace_label, small_brace_label), conv_arr[:2].animate.set_opacity(0.25), conv_arr[-2:].animate.set_opacity(0.25))
        self.wait()
        self.play(Transform(brace, mid_brace), FadeTransform(small_brace_label, mid_brace_label), conv_arr[1].animate.set_opacity(1), conv_arr[-2].animate.set_opacity(1))
        self.wait()
...
class ImageConvolution(InteractiveScene):
    ...
    def set_index(self, value, run_time=8, rate_func=linear):
        self.play(self.index_tracker.animate.set_value(value), run_time=run_time, rate_func=rate_func)

    def zoom_to_kernel(self, run_time=2):
        ka = self.kernel_array
        self.play(self.camera.frame.animate.set_height(1.5 * ka.get_height()).move_to(ka), run_time=run_time)
    ...
    def zoom_to_new_pixel(self, run_time=4):
        ...
        height_func = bezier([frame.get_height(), frame.get_height(), FRAME_HEIGHT, target_height, target_height])
        self.play(UpdateFromAlphaFunc(frame, lambda m, a: m.set_height(height_func(a)).move_to(center_func(a))), run_time=run_time, rate_func=linear)
    ...
    def show_pixel_sum(self, tex=None, convert_to_vect=True, row_len=9):
        ...
        expr[-1].scale(0, about_edge=LEFT)
        ...
        self.play(frame.animate.scale(1.2, about_edge=DOWN), FadeIn(top_bar, 2 * DOWN))
        ...
        self.remove(*ka_copy)
        if convert_to_vect:
            self.play(LaggedStart(*(Transform(lil_pixel, rgb_vect) for lil_pixel, rgb_vect in zip(lil_pixels, rgb_vects))))
        self.wait()
    ...
    def construct(self):
        for index in self.stops:
            ...
            if index == self.stops[0]:
                top_bar = self.show_pixel_sum(tex='\\frac{1}{9}')
            self.wait()
...
class GaussianBluMario(ImageConvolution):
    ...
    def construct(self):
        ...
        frame = self.camera.frame
        gaussian = ParametricSurface(lambda u, v: [u, v, np.exp(-u ** 2 - v ** 2)], u_range=(-3, 3), v_range=(-3, 3), resolution=(101, 101))
        ...
        self.wait()
        self.play(frame.animate.set_height(8).reorient(0, 60).move_to(ORIGIN), FadeOut(top_bar, time_span=(0, 1)), run_time=3)
        self.set_index(len(self.pixel_array), run_time=self.final_run_time)
        self.wait()

    def get_kernel(self):
        return np.array([[0.00296902, 0.0133062, 0.0219382, 0.0133062, 0.00296902], [0.0133062, 0.0596343, 0.0983203, 0.0596343, 0.0133062], [0.0219382, 0.0983203, 0.162103, 0.0983203, 0.0219382], [0.0133062, 0.0596343, 0.0983203, 0.0596343, 0.0133062], [0.00296902, 0.0133062, 0.0219382, 0.0133062, 0.00296902]])
...
class SobelFilter1(ImageConvolution):
    ...
    def construct(self):
        ...
        for square in kernel.target:
            ...
            square.set_fill(rgb_to_color([2 * max(-v, 0), 2 * max(v, 0), 2 * max(v, 0)]), opacity=0.5, recurse=False)
        ...
        for index in self.stops:
            self.set_index(index)
            self.zoom_to_kernel()
            self.play(*(square.animate.set_fill(opacity=0, recurse=False) for square in kernel), rate_func=there_and_back_with_pause, run_time=3)
            self.add(kernel)
            self.wait()
            self.zoom_to_new_pixel()
            self.wait()
    ...
    def get_kernel(self):
        return np.array([[-0.25, 0, 0.25], [-0.5, 0, 0.5], [-0.25, 0, 0.25]])
...
class SharpenFilter(ImageConvolution):
    ...
    def get_kernel(self):
        return np.array([[0.0, 0.0, -0.25, 0.0, 0.0], [0.0, -0.25, -0.5, -0.25, 0.0], [-0.25, -0.5, 5.0, -0.5, -0.25], [0.0, -0.25, -0.5, -0.25, 0.0], [0.0, 0.0, -0.25, 0.0, 0.0]])
...
class ContrastConvolutionToMultiplication(InteractiveScene):
    ...
    def construct(self):
        ...
        prod = arr1 * arr2
        quart = FRAME_WIDTH / 4
        left_arrays, right_arrays = (VGroup(*(self.get_array_mobject(arr, color) for arr, color in [(arr1, BLUE), (arr2, YELLOW)])).arrange(DOWN, buff=1.0).move_to(vect * quart + UP) for vect in [LEFT, RIGHT])
        ...
        self.play(FadeIn(VGroup(conv_array[0], conv_array[-1])), MoveToTarget(low_arr, path_arc=PI))
        for n, elem in enumerate(conv_array.elements):
            pairs = get_aligned_pairs(top_arr.elements, low_arr.elements, n)
            self.play(get_row_shift(top_arr.elements, low_arr.elements, n), MaintainPositionRelativeTo(low_arr[::2], low_arr.elements), run_time=0.25)
        ...
        lines.set_stroke(TEAL, 1)
        self.play(FadeIn(VGroup(prod_array[0], prod_array[-1])), FadeIn(lines))
...
class BigPolynomials(InteractiveScene):
    ...
    def construct(self):
        ...
        for i, row in enumerate(grid):
            if i == N - 2:
                for j, square in enumerate(row):
                    if j == N - 2:
                        self.replace_square(square, OldTex('\\ddots'))
                    else:
                        self.replace_square(square, OldTex('\\vdots'))
            else:
                self.replace_square(row[N - 2], OldTex('\\cdots'))
        self.add(grid)
        a_terms, b_terms = all_terms = [VGroup(*(Tex(f'{letter}_{{{n}}} x^{{{n}}}', font_size=24) for n in (*range(N - 1), 99))) for letter in ['a', 'b']]
        ...
        a_terms[-2].become(OldTex('\\cdots', font_size=24).move_to(a_terms[-2]).shift(0.02 * DOWN))
        b_terms[-2].become(OldTex('\\vdots', font_size=24).move_to(b_terms[-2]))
        ...
        product_count = OldTexText('$100 \\times 100 = 10{,}000$ \\\\ products', font_size=60)
        ...
        self.play(FlashAround(b_terms, run_time=2, time_width=2), FadeIn(b_label), FadeOut(a_label))
        self.play(FadeOut(b_label))
        self.wait()
        self.play(LaggedStart(*(AnimationGroup(TransformFromCopy(term.pre_a, term[0]), TransformFromCopy(term.pre_b, term[1]), FadeIn(term[2], rate_func=squish_rate_func(smooth, 0.5, 1))) for term in prod_terms), lag_ratio=0.1, run_time=5), Write(product_count))
        self.add(prod_terms)
...
class FunctionToCoefficientCommutativeDiagram(InteractiveScene):
    ...
    def construct(self):
        ...
        graphs = VGroup(axes1.get_graph(p1).set_stroke(BLUE), axes2.get_graph(p2).set_stroke(TEAL), axes3.get_graph(lambda x: p1(x) * p2(x)).set_stroke(YELLOW))
        graphs.set_stroke(width=2)
        kw = dict(font_size=30)
        graph_labels = VGroup(*(Tex(tex, **kw).next_to(axes.get_top(), RIGHT, aligned_edge=UP) for tex, axes in zip(['f(x)', 'g(x)', 'f(x) \\cdot g(x)'], all_axes)))
        a_labels, b_labels, conv_label = coef_labels = VGroup(Tex('(a_0, a_1, \\dots, a_n)', **kw), Tex('(b_0, b_1, \\dots, b_m)', **kw), VGroup(Tex('c_0 = a_0 b_0', **kw), Tex('c_1 = a_0 b_1 + a_1 b_0', **kw), Tex('c_2 = a_0 b_2 + a_1 b_1 + a_2 b_0', **kw), Tex('c_3 = a_0 b_3 + a_1 b_2 + a_2 b_1 + a_3 b_0', **kw), Tex('\\vdots', **kw)).arrange(DOWN, aligned_edge=LEFT).scale(0.85))
        v_arrows = VGroup()
        ...
        self.play(LaggedStartMap(ShowCreation, graphs[:2]), LaggedStartMap(FadeIn, graph_labels[:2]), LaggedStartMap(FadeIn, all_axes[:2]))
        self.play(LaggedStart(Write(mult_arrow, run_time=1), TransformFromCopy(graphs[0], graphs[2].copy(), remover=True), TransformFromCopy(graphs[1], graphs[2]), TransformFromCopy(all_axes[0], all_axes[2].copy(), remover=True), TransformFromCopy(all_axes[1], all_axes[2]), TransformFromCopy(graph_labels[0], graph_labels[2][:5]), TransformFromCopy(graph_labels[1], graph_labels[2][5:])), lag_ratio=0.2)
        self.wait()
        self.play(LaggedStartMap(Write, v_arrows[:2], lag_ratio=0.7), LaggedStartMap(FadeIn, coef_labels[:2], shift=DOWN, lag_ratio=0.7))
        self.wait()
        self.play(ShowCreation(coef_rect), FadeIn(conv_arrow))
        self.play(FadeTransformPieces(a_labels.copy(), conv_label), FadeTransformPieces(b_labels.copy(), conv_label))
        all_dots = Group(*(Group(*(GlowDot(axes.i2gp(x, graph), radius=0.1, glow_factor=0.8, color=WHITE) for x in range(-3, 4))) for axes, graph in zip(all_axes, graphs)))
        all_circles = VGroup(*(VGroup(*(Circle(radius=0.1).set_stroke(YELLOW, 2).move_to(dot) for dot in dots)) for dots in all_dots))
        self.play(LaggedStartMap(FadeIn, all_dots[0], scale=0.5, lag_ratio=0.5), LaggedStartMap(FadeIn, all_dots[1], scale=0.5, lag_ratio=0.5))
        self.wait()
        self.play(ShowSubmobjectsOneByOne(all_circles[0]), ShowSubmobjectsOneByOne(all_circles[1]), ShowSubmobjectsOneByOne(all_circles[2]), ShowIncreasingSubsets(all_dots[2]), run_time=4, rate_func=linear)
...
class DataPointsToPolynomial(InteractiveScene):
    ...
    def construct(self):
        ...
        poly_tex = OldTex('a_0', '+', 'a_1 x', '+', *it.chain(*((f'a_{n} x^{n}', '+') for n in range(2, len(data) - 1))))
        poly_tex.to_corner(UR)
        graph = graphs[1].copy()
        self.play(LaggedStartMap(FadeIn, dots[:2], lag_ratio=0.7, run_time=1), LaggedStartMap(ShowCreationThenFadeOut, circles[:2], lag_ratio=0.2, run_time=1))
        self.play(ShowCreation(graph), FadeIn(poly_tex[:3]))
        self.wait()
        for n in range(2, len(data) - 1):
            self.play(ShowCreationThenFadeOut(circles[n], run_time=1.5), FadeIn(dots[n]), Transform(graph, graphs[n]), FadeIn(poly_tex[2 * n - 1:2 * n + 1], time_span=(0, 1), lag_ratio=0.1), run_time=2)
            self.wait()
...
class PolynomialSystem(InteractiveScene):
    ...
    def construct(self):
        frame = self.camera.frame
        N = self.N
        coefs = VGroup(*(Tex(f'c_{{{n}}}') for n in range(N)))
        coefs.set_submobject_colors_by_gradient(BLUE, TEAL)
        x_powers = VGroup(*(Tex(f'x^{{{n}}}') for n in range(N)))
        ...
        arrows = VGroup(*(Arrow(interpolate(words.get_corner(DL) + 0.5 * RIGHT, words.get_corner(DR) + 0.5 * LEFT, n / (N - 1)), coef, color=coef.get_color()) for n, coef in enumerate(poly_x.coefs)))
        self.play(FadeIn(words, lag_ratio=0.1), LaggedStartMap(GrowArrow, arrows), LaggedStart(*(FlashAround(coef, time_width=1) for coef in poly_x.coefs), lag_ratio=0.1, run_time=3))
        self.wait()
        self.play(Write(eq), FadeIn(fg, 0.25 * UP))
        self.wait()
        self.play(LaggedStart(FadeOut(words, UP), FadeOut(arrows, 2 * UP, lag_ratio=0.05), FadeOut(fg, UP), FadeOut(eq, 1.5 * UP), top_eq.animate.to_edge(UP)), frame.animate.set_height(11, about_edge=UR), run_time=2)
        lhss = VGroup(*(Tex(f'h({n})=') for n in range(N)))
        rhss = VGroup(*(self.get_polynomial(coefs.copy(), VGroup(*(Tex(f'{x}^{{{n}}}') for n in range(N)))) for x in range(N)))
        equations = VGroup()
        for lhs, rhs in zip(lhss, rhss):
        ...
        self.play(LaggedStart(*(FadeTransform(top_eq.copy(), eq) for eq in equations), lag_ratio=0.02, run_time=3))
        ...
        self.play(Write(words2), LaggedStart(*(FlashAround(lhs[:4], time_width=1.5) for lhs in lhss), run_time=5, lag_ratio=0.1))
        self.wait()
        ...
        self.play(FadeOut(words2, LEFT, rate_func=running_start), inputs.animate.set_color(RED).shift(2 * LEFT).set_anim_args(run_time=1.5, lag_ratio=0.2, path_arc=PI / 2), Transform(consts, boxes, lag_ratio=0.01, run_time=3))
        self.play(FadeOut(inputs, LEFT, lag_ratio=0.2, rate_func=running_start))
        inputs.restore()
        inputs.set_opacity(0)
        self.add(lhss)
        kw = dict(tex_to_color_map={'\\omega': YELLOW})
        omega_def = OldTex(f'\n            \\text{{Let }} \\omega = e^{{2\\pi i / {N}}} \\qquad\n            \\text{{Notice }} \\omega^{{{N}}} = 1\n            ', font_size=60, **kw)
        omega_def.next_to(lhss, UP, buff=1.5, aligned_edge=LEFT)
        all_omega_powers = [VGroup(*(Tex(f'\\omega^{{{k * n % N}}}', **kw) for n in range(N))) for k in range(0, N)]
        new_rhss = VGroup(*(self.get_polynomial(coefs.copy(), omega_powers) for omega_powers in all_omega_powers))
        new_lhss = VGroup(*(Tex(f'h(\\omega^{n}) = ') for n in range(N)))
        self.play(frame.animate.set_height(13, about_edge=DR), FadeIn(omega_def), top_eq.animate.shift(1.75 * UP))
        for old_lhs, old_rhs, new_lhs, new_rhs in zip(lhss, rhss, new_lhss, new_rhss):
            ...
            self.play(FadeTransformPieces(old_lhs, new_lhs), FadeTransformPieces(old_rhs, new_rhs))
        self.wait()
        brace = Brace(new_lhss, LEFT)
        kw = dict(font_size=60)
        label = OldTexText('Discrete\\\\Fourier\\\\Transform', alignment='\\raggedright', isolate=list('DFT'), **kw)
        ...
        self.play(FadeIn(c_rect), TransformFromCopy(new_rhss[0].coefs, c_list, path_arc=-PI / 3))
        self.play(TransformMatchingTex(label, short_label), sub_label.animate.scale(48 / 60).next_to(short_label, RIGHT), FadeInFromPoint(h_rect, pre_h_list.get_center()), TransformFromCopy(pre_h_list, h_list))
        ...
        for arrow in (top_arrow, low_arrow):
            ...
            run_time = OldTex('\\mathcal{O}\\big(N\\log(N)\\big)')
...
class RootsOfUnity(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(circle), Rotate(Group(root_lines[0], root_dots[0]), TAU, about_point=plane.n2p(0)), run_time=2)
        self.wait()
        kw = dict(tex_to_color_map={'\\omega': YELLOW}, font_size=36)
        max_power = 3 * N
        powers = VGroup(*(OldTex(f'\\omega^{{{k}}}', **kw) for k in range(max_power)))
        ...
        for k in range(max_power - 1):
            ...
            self.play(Transform(moving_power, powers[k + 1], **kw), Transform(root_lines[k % N].copy(), root_lines[(k + 1) % N], remover=True, **kw), Transform(root_dots[k % N].copy(), root_dots[(k + 1) % N], remover=True, **kw), MoveToTarget(shown_powers, **kw))
...
class AlgorithmOutline(InteractiveScene):
    ...
    def construct(self):
        ...
        t2c = {tex: BLUE for tex in ['\\textbf{a}', 'a_0', 'a_1', 'a_2', 'a_{n - 1}']}
        t2c.update({tex: TEAL for tex in ['\\textbf{b}', 'b_0', 'b_1', 'b_2', 'b_{m - 1}']})
        tex_kw = dict(tex_to_color_map=t2c, font_size=36)
        lists = VGroup(Tex('\\textbf{a} = [a_0, a_1, a_2, \\dots, a_{n - 1}]', **tex_kw), Tex('\\textbf{b} = [b_0, b_1, b_2, \\dots, b_{m - 1}]', **tex_kw), Tex('\\textbf{a} * \\textbf{b} = \\left[\\begin{array}{l}\n                    a_0 b_0, \\\\\n                    a_0 b_1 + a_1 b_0, \\\\\n                    a_0 b_2 + a_1 b_1  + a_2 b_0, \\\\\n                    \\quad \\vdots \\\\\n                    a_{n - 1} b_{m - 1}\n            \\end{array}\\right]', **tex_kw))
        ...
        self.play(TransformMatchingShapes(VGroup(*lists[0], *lists[1]).copy(), lists[2]), FadeIn(conv_rect), FadeIn(q_marks))
        self.wait()
        polys = VGroup(Tex('f(x) = a_0 + a_1 x + a_2 x^2 + \\cdots + a_{n - 1}x^{n - 1}', **tex_kw), Tex('g(x) = b_0 + b_1 x + b_2 x^2 + \\cdots + b_{m - 1}x^{m - 1}', **tex_kw))
        ...
        graphs = VGroup(axes[0].get_graph(f, color=BLUE), axes[1].get_graph(g, color=TEAL), axes[2].get_graph(lambda x: f(x) * g(x), color=YELLOW))
        graphs.set_stroke(width=2)
        label_kw = dict(font_size=24)
        graph_labels = VGroup(Tex('f(x)', **label_kw).move_to(axes[0], UL), Tex('g(x)', **label_kw).move_to(axes[1], UL), Tex('f(x) \\cdot g(x)', **label_kw).move_to(axes[2], UL))
        self.play(LaggedStart(*(TransformFromCopy(listy.copy(), poly) for listy, poly in zip(lists, polys))), LaggedStartMap(FadeIn, axes[:2]), LaggedStartMap(ShowCreation, graphs[:2]), LaggedStartMap(FadeIn, graph_labels[:2]), lag_ratio=0.5)
        self.wait()
        x_samples = np.arange(-3, 3.5, 0.5)
        f_points, g_points, fg_points = all_points = [[ax.i2gp(x, graph) for x in x_samples] for ax, graph in zip(axes, graphs)]
        f_dots, g_dots, fg_dots = all_dots = Group(*(Group(*(GlowDot(point, color=WHITE, glow_factor=0.8, radius=0.07) for point in points)) for points in all_points))
        self.play(FadeIn(f_dots, lag_ratio=0.7), FadeIn(g_dots, lag_ratio=0.7), run_time=5)
        self.wait()
        self.play(TransformFromCopy(axes[1], axes[2]), TransformMatchingShapes(graph_labels[:2].copy(), graph_labels[2]), LaggedStart(*(Transform(Group(fd, gd).copy(), Group(fgd), remover=True) for fd, gd, fgd in zip(*all_dots)), lag_ratio=0.5, run_time=5))
        self.add(fg_dots)
        ...
        self.play(LaggedStartMap(ShowCreation, crosses, lag_ratio=0.5, run_time=2), polys[0].animate.scale(0.5, about_edge=DL), polys[1].animate.scale(0.5, about_edge=DL), LaggedStartMap(FadeOut, Group(final_arrow, axes[2], graph_labels[2], fg_dots, graphs[2], q_marks)))
        self.play(LaggedStartMap(FadeOut, Group(axes[0], graphs[0], graph_labels[0], f_dots, crosses[0], axes[1], graphs[1], graph_labels[1], g_dots, crosses[1]), shift=0.2 * RIGHT))
        t2c = {tex: RED for tex in ['\\hat{\\textbf{a}}', '\\hat{a}_0', '\\hat{a}_1', '\\hat{a}_2', '\\hat{a}_{m + n - 1}']}
        t2c.update({tex: MAROON_C for tex in ['\\hat{\\textbf{b}}', '\\hat{b}_0', '\\hat{b}_1', '\\hat{b}_2', '\\hat{b}_{m + n - 1}']})
        fft_kw = dict(tex_to_color_map=t2c, font_size=36, isolate=['='])
        fft_lists = VGroup(Tex('\\hat{\\textbf{a}} = [\\hat{a}_0, \\hat{a}_1, \\hat{a}_2, \\dots, \\hat{a}_{m + n - 1}]', **fft_kw), Tex('\\hat{\\textbf{b}} = [\\hat{b}_0, \\hat{b}_1, \\hat{b}_2, \\dots, \\hat{b}_{m + n - 1}]', **fft_kw), Tex('\\hat{\\textbf{a}} \\cdot \\hat{\\textbf{b}} = [\n                    \\hat{a}_0 \\hat{b}_0,\n                    \\hat{a}_1 \\hat{b}_1,\n                    \\hat{a}_2 \\hat{b}_2,\n                    \\dots,\n            ]', **fft_kw))
        ...
        self.play(title[:4].animate.match_x(title[4:7], RIGHT), FadeOut(title[4:7], 0.1 * DOWN, lag_ratio=0.1))
        self.play(LaggedStartMap(FadeIn, fft_lists[:2], shift=1.5 * RIGHT, **kw), LaggedStartMap(GrowArrow, arrows[:2], **kw), LaggedStartMap(FadeIn, arrow_labels[:2], shift=0.5 * RIGHT, **kw))
        self.wait()
        self.play(LaggedStart(polys[0].animate.scale(1.5, about_edge=DL), polys[1].animate.scale(1.5, about_edge=DL), lag_ratio=0.5))
        self.wait()
        self.play(FadeIn(fft_lists[2], DOWN), GrowArrow(mult_arrow), FadeIn(mult_arrow.label, 0.5 * DOWN))
        self.wait()
        self.play(GrowArrow(arrows[2]), FadeIn(arrow_labels[2], shift=LEFT))
        self.wait()
...
class FourierCoefficients(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(graph)
        exp = '{\\left(e^{2\\pi i \\cdot t}\\right)}'
        kw = dict(tex_to_color_map={'{x}': BLUE, exp: TEAL})
        equations = VGroup(Tex('f(x) = c_0 + c_1 {x} + c_2 {x}^2 + c_3 {x}^3 + \\dots', **kw), Tex(f'f(t) = c_0 + c_1 {exp} + c_2 {exp}^2 + c_3 {exp}^3 + \\dots', **kw), Tex(f'f(t) = \\cdots + c_{{-1}} {exp}^{{-1}} + c_0 + c_1 {exp} + c_2 {exp}^2 + \\dots', **kw))
        equations = VGroup
        last = VMobject()
