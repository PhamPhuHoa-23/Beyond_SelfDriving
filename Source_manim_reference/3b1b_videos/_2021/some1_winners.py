...
def save_thumbnail_locally(slug, rewrite=False):
    ...
    urls = [*(f'https://img.youtube.com/vi/{slug}/{suffix}.jpg' for suffix in suffixes), *(f'https://i.ytimg.com/vi/{slug}/{suffix}.jpg' for suffix in suffixes)]
    for url in urls:
        try:
...
class Introduction(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.add(summer)
        self.wait(0.5)
        self.play(MoveToTarget(this_summer), FadeIn(james_name, lag_ratio=0.1), FadeIn(logos[0], LEFT))
        self.play(Write(logos[1], lag_ratio=0.01, stroke_width=0.05, run_time=1), Animation(logos[1][-1].copy(), remover=True), FadeOut(james_name))
        self.wait()
        self.play(FadeIn(some[len('Summer'):], lag_ratio=0.1), FadeOut(this), MoveToTarget(logos, path_arc=-PI / 3, run_time=2))
        ...
        url[0].shift(0.05 * UP)
        url.to_edge(UP)
        self.play(FadeOut(title, UP), FadeIn(url[0]), ShowIncreasingSubsets(url[1], rate_func=linear, run_time=2))
        self.wait()
        dots = Dot().get_grid(3, 1)
        dots.arrange(DOWN, buff=1.5)
        dots.align_to(url, LEFT)
        dots.set_y(-0.5)
        points = VGroup(Text('Extremely open-ended'), Text('Promise to feature 4 to 5 winners'), Text('Brilliant offered $5k in cash prizes'))
        colors = [GREEN_A, GREEN_B, GREEN_C]
        for dot, point, color in zip(dots, points, colors):
            point.next_to(dot, RIGHT)
            point.set_color(color)
        self.play(LaggedStartMap(FadeInFromPoint, dots, lambda m: (m, url.get_bottom())))
        self.wait()
        for point in points:
            self.play(Write(point))
            if point is points[1]:
                self.play(morty.change('hooray').look(ORIGIN), FadeOut(BackgroundRectangle(morty, buff=0.25, fill_opacity=1)))
        ...
        self.play(LaggedStartMap(FadeOut, VGroup(url, dots, points)), FadeIn(self.background), morty.change('hesitant', winner_word), FadeTransform(pre_winner_word, winner_word[2:9]), LaggedStartMap(FadeIn, self.students))
        self.play(Write(winner_word[:2]), Write(winner_word[-2:]), self.change_students('sassy', 'raise_right_hand', 'raise_left_hand'))
        self.wait(4)
        ...
        self.play(FadeIn(salt, lag_ratio=0.1), ShowCreation(arrow), self.change_students('pondering', 'pondering', 'erm', look_at=salt))
        self.play(Blink(self.students[2]), FadeIn(strange, lag_ratio=0.1))
        ...
        bubble.add_content(OldTex('|fg|_1 \\leq |f|_p |g|_q', tex_to_color_map={'f': GREEN, 'g': TEAL}))
        ...
        video.set_gloss(1)
        self.play(FadeOut(salt, 0.5 * UP), FadeIn(spirit, 0.5 * UP))
        self.play(MoveToTarget(arrow), FadeIn(bubble), self.students[0].change('thinking', bubble), self.students[1].change('pondering', bubble), self.students[2].change('pondering', bubble), self.teacher.change('happy', bubble), winner_word.animate.scale(0.3).set_opacity(0.5).to_corner(UR))
        self.play(self.students[0].change('raise_left_hand', video), FadeIn(video, 0.5 * UP))
        self.wait()
        ...
        self.play(LaggedStart(*(TransformFromCopy(bubble, bc) for bc in bubble_copies), lag_ratio=0.5), LaggedStart(*(student.change('thinking', UR) for student in self.students[1:]), lag_ratio=0.5), run_time=2)
        self.wait(3)
...
class ProsConsOfContext(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(ShowCreation(h_line), ShowCreation(v_line), lag_ratio=0.5), LaggedStart(FadeIn(title, 0.25 * UP), FadeIn(pro_label), FadeIn(con_label), lag_ratio=0.5))
        self.wait()
        pros = VGroup(Text('Clear deadline'), Text('Extra push for quality'))
...
class FiltrationProcess(Scene):
    ...
    def construct(self):
        ...
        peer_subwords = OldTexText('(actually quite interesting\\\\see the blog post)', font_size=24, fill_color=GREY_B)
        ...
        guests = VGroup(Text('Alex Kontorovich'), Text('Henry Reich'), Text('Nicky Case'), Text('Tai-Danae Bradley'), Text('Bukard Polster'), Text('Mithuna Yoganathan'), Text('Steven Strogatz'))
        guests.scale(0.75)
        guests.set_color(GREY_B)
        guests.arrange(DOWN, aligned_edge=LEFT)
        guests.next_to(guest_words, DOWN, aligned_edge=LEFT)
        self.play(FadeIn(guest_words), LaggedStartMap(FadeIn, guests, shift=0.25 * DOWN, lag_ratio=0.5, run_time=4))
        self.wait()
...
class RevealingTiles(Scene):
    ...
    def five_winners(self):
        ...
        self.wait()
        self.play(FadeIn(subtitle, lag_ratio=0.2), LaggedStart(*(ApplyMethod(tile.move_to, tiles[reorder[i]], path_arc=PI / 2) for i, tile in enumerate(tiles)), lag_ratio=0.25, run_time=2))
        self.wait()
    ...
    def honorable_mentions(self):
        ...
        self.play(FadeTransform(self.title, new_title), FadeOut(self.subtitle), MoveToTarget(tiles), LaggedStartMap(FadeInFromPoint, new_tiles[5:], lambda m: (m, ORIGIN), lag_ratio=0.1, run_time=5))
        self.wait()
    ...
    def reveal_tile(self, tile, title, author, image_name):
        ...
        self.play(Rotate(image, PI, RIGHT), Rotate(tile, PI, RIGHT), UpdateFromAlphaFunc(tile, lambda m, a: m.set_opacity(1 if a < 0.5 else 0)), UpdateFromAlphaFunc(image, lambda m, a: m.set_opacity(0 if a < 0.5 else 1)))
        tile.remove(tile[1])
        ...
        self.play(FadeIn(full_rect), MoveToTarget(tile, run_time=2), FadeInFromPoint(title, tile.get_top(), run_time=2))
        self.play(FadeIn(byline, 0.25 * DOWN))
        self.wait()
        self.play(Restore(tile), FadeOut(full_rect), FadeOut(title, 0.75 * UP), FadeOut(byline, 1.0 * UP))
...
class AlmostTooGood(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.teacher_says(OldTexText('Almost \\emph{too} good'), look_at=self.students[2].eyes, added_anims=[self.change_students('happy', 'tease', 'hesitant')])
        self.wait(4)
...
class ViewRect(Scene):
    ...
    def construct(self):
        ...
        number.set_value(0)
        self.play(ShowCreation(arrow), ChangeDecimalToValue(number, self.views, run_time=2), VFadeIn(number), UpdateFromFunc(number, lambda m: m.set_fill(RED)))
        self.wait()
...
class SureSure(TeacherStudentsScene):
    ...
    def construct(self):
        self.students.flip().flip()
        self.teacher_says('The point is \n not the winners', look_at=self.students[2].eyes, bubble_config={'height': 3, 'width': 4})
        self.play(PiCreatureSays(self.students[0], "Yeah, yeah, sure\n it isn't...", bubble_config={'height': 2, 'width': 3}))
...
class Narrative(Scene):
    ...
    def construct(self):
        ...
        self.add(question)
        properties = VGroup(Text('Clarity'), Text('Motivation'), Text('Memorability'), Text('Narrative'))
        ...
        self.play(FadeIn(rect), FlashAround(narrative, run_time=2, time_width=1), narrative.animate.set_color(YELLOW))
        self.wait()

class Traction(Scene):

    def construct(self):
        slug_count_pairs = [('PKjbBQ0PBCQ', '$\\sim$800,000'), ('ACZC_XEyg9U', '$\\sim$100,000'), ('4y_nmpv-9lI', '$\\sim$140,000'), ('cCKOl5li6YM', '$\\sim$400,000')]
        groups = Group()
