...
def get_c_dot_label(dot, get_c, font_size=24, direction=UP):
    c_label = VGroup(OldTex('c = ', font_size=font_size), DecimalNumber(get_c(), font_size=font_size, include_sign=True)).arrange(RIGHT, buff=0.075)
...
def get_iteration_label(font_size=36):
    kw = {'tex_to_color_map': {'z_0': BLUE_C, 'z_1': BLUE_D, 'z_2': GREEN_D, 'z_3': GREEN_E, 'z_{n + 1}': GREEN_D, 'z_n': BLUE, '\\longrightarrow': WHITE}, 'font_size': font_size}
    iterations = OldTex('\n        z_0 \\longrightarrow\n        z_1 \\longrightarrow\n        z_2 \\longrightarrow\n        z_3 \\longrightarrow\n        \\cdots\n        ', **kw)
...
class MandelbrotFractal(NewtonFractal):
    CONFIG = {'shader_folder': 'mandelbrot_fractal', 'data_dtype': [('point', np.float32, (3,))], 'scale_factor': 1.0, 'offset': ORIGIN, 'colors': MANDELBROT_COLORS, 'n_colors': 9, 'parameter': complex(0, 0), 'n_steps': 300, 'mandelbrot': True}
...
class MandelbrotSetPreview(Scene):
    ...
    def construct(self):
        plane = ComplexPlane((-2, 1), (-2, 2), background_line_style={'stroke_color': GREY_B, 'stroke_opacity': 0.5})
        ...
        self.play(mandelbrot.animate.set_n_steps(300), rate_func=lambda a: a ** 3, run_time=10)
        self.wait()
...
class HolomorphicDynamics(Scene):
    ...
    def show_goals(self):
        ...
        frames.to_edge(DOWN)
        goals = VGroup(OldTexText('Goal 1: Other Mandelbrot occurrences'), OldTexText('Goal 2: Tie up loose ends'))
        ...
        self.play(FadeIn(frames[0]), FadeIn(goals[0], 0.5 * UP))
        self.wait()
        self.play(FadeIn(frames[1]), FadeIn(goals[1], 0.5 * UP))
        self.wait()
        rect = SurroundingRectangle(title.get_part_by_text('Holomorphic'))
        rect.set_stroke(YELLOW, 2)
        self.play(ReplacementTransform(underline, rect), FadeOut(background), LaggedStartMap(FadeOut, VGroup(*goals, *frames)))
        self.title_rect = rect

    def complex_functions(self):
        kw = {'tex_to_color_map': {'\\mathds{C}': BLUE, 'z': YELLOW}}
        f_def = VGroup(OldTex('f : \\mathds{C} \\rightarrow \\mathds{C}', **kw), OldTex("f'(z) \\text{ exists}", **kw))
        ...
        for part in f_def:
            ...
            self.wait()
        examples = VGroup(OldTex('f(z) = z^2 + 1', **kw), OldTex('f(z) = e^z', **kw), OldTex('f(z) = \\sin\\left(z\\right)', **kw), OldTex('\\vdots'))
        ...
        self.play(FadeOut(examples, lag_ratio=0.1), FadeOut(f_def, lag_ratio=0.1), Transform(rect, new_rect))
        self.wait()
    ...
    def example_fractals(self):
        ...
        self.play(FadeOut(self.title, UP), LaggedStartMap(Write, rhss), LaggedStartMap(FadeIn, lhss), LaggedStartMap(FadeIn, rects), LaggedStartMap(ShowCreation, arrows))
        self.wait()
...
class HolomorphicPreview(Scene):
    ...
    def construct(self):
        ...
        tiny_plane = ComplexPlane((-2, 2), (-2, 2), height=0.5, width=0.5, axis_config={'stroke_width': 1.0}, background_line_style={'stroke_width': 1.0}, faded_line_ratio=1)
        ...
        self.play(ShowCreation(tiny_plane), MoveToTarget(in_plane))
        self.wait()
        ...
        arrow = Arrow(tiny_plane, tiny_plane_image, path_arc=-PI / 4, stroke_width=5)
        f_label = OldTex('f(z)')
        f_label.next_to(arrow, UP, SMALL_BUFF)
        words = Text('Looks roughly like\nscaling + rotating')
        words.set_width(2.5)
        words.move_to(VGroup(in_plane, out_plane))
        self.play(ShowCreation(arrow), FadeIn(f_label, 0.1 * UP))
        self.play(TransformFromCopy(tiny_plane, tiny_plane_image), MoveToTarget(out_plane), FadeIn(words), run_time=2)
        self.wait(2)
...
class AmbientRepetitionChaos(AmbientRepetition):
    ...
    def func(self, c):
        return complex(random.random() * 4 - 2, random.random() * 4 - 2)
...
class RationalFunctions(Scene):
    ...
    def construct(self):
        equation = OldTex('f(z)', '=', "z - {P(z) \\over P'(z)}", '=', 'z - {z^3 - 1 \\over 3z^2}', '=', '{2z^3 + 1 \\over 3z^2}')
        ...
        self.play(FadeIn(equation[:3]), GrowFromCenter(iter_brace), FadeIn(iter_text, 0.5 * UP))
        self.wait()
        self.play(FadeIn(equation[3:5]), GrowFromCenter(example_brace), FadeIn(example_text, 0.5 * DOWN))
        self.wait()
        self.play(Write(equation[5]), LaggedStart(*(FadeTransform(equation[4][i].copy(), equation[6][j]) for i, j in zip([1, 0, *range(2, 10)], [0, 0, *range(1, 9)])), lag_ratio=0.02))
        self.add(equation[6])
        self.play(LaggedStart(*(FadeTransform(equation[4][i].copy().set_opacity(0), equation[6][j].copy().set_opacity(0)) for i, j in zip([0, 0, 0, 0], [0, 0, 0, 0])), lag_ratio=0.02))
        self.add(equation)
        self.wait()
        ...
        self.play(Write(rational_name), ShowCreation(arrow), ShowCreation(box))
        self.wait()
        self.play(equation[:2].animate.next_to(ORIGIN, LEFT, SMALL_BUFF), equation[2:6].animate.set_opacity(0.5).scale(0.5).to_corner(DR), VGroup(equation[6], box).animate.next_to(ORIGIN, RIGHT, SMALL_BUFF), MaintainPositionRelativeTo(VGroup(rational_name, arrow), box), FadeOut(VGroup(example_brace, example_text, iter_brace, iter_text), shift=2 * RIGHT + DOWN))
        self.wait()
        frame = self.camera.frame
        rhs = equation[6]
        functions = VGroup(OldTex('3z^4 + z^3 + 4 \\over z^5 + 5z + 9'), OldTex('2z^6 + 7z^4 + 1z \\over 8z^3 + 2z^2 + 8'), OldTex('(z^2 + 1)^2 \\over 4z(z^2 - 1)'), OldTex('az + b \\over cz + d'), OldTex('z^2 + az + b \\over z^2 + cz + d'), OldTex('a_n z^n + \\cdots + a_0 \\over b_m z^m + \\cdots + b_0'), OldTex('z^2 + c \\over 1'))
        ...
        for n, function in enumerate(functions):
            self.play(FadeOut(rhs, lag_ratio=0.1), FadeIn(function, lag_ratio=0.1), box.animate.set_opacity(0))
            self.remove(box)
            self.wait()
            if n == 0:
                self.play(rational_name.animate.next_to(box, UP), arrow.animate.scale(0, about_edge=DOWN), ApplyMethod(frame.shift, 2 * DOWN, run_time=2), FadeOut(equation[2:6]))
            else:
                self.wait()
            rhs = function[0]
        self.play(FadeOut(box), FadeOut(rational_name), FadeOut(rhs[4:]), rhs[:4].animate.next_to(ORIGIN, RIGHT, SMALL_BUFF).shift(0.07 * UP), frame.animate.shift(DOWN))
        self.wait()
...
class ShowFatouAndJulia(Scene):
    ...
    def construct(self):
        ...
        figures = Group(get_figure('Pierre_Fatou', 'Pierre Fatou', '1878-1929', **kw), get_figure('Gaston_Julia', 'Gaston Julia', '1893-1978', **kw))
...
class MandelbrotIntro(Scene):
    ...
    def add_process_description(self):
        ...
        terms = self.terms = VGroup(OldTex('z_{n + 1} = z_n^2 + {c}', **kw), OldTex('{c} \\text{ can be changed}', **kw), OldTex('z_0 = 0', **kw))
    ...
    def show_iterations(self):
        ...
        def increase_step(run_time=1.0):
            ...
            self.add(*lines, c_dot, c_label)
            self.play(ShowCreation(lines[-2]), TransformFromCopy(lines[-3], lines[-1]), run_time=run_time)
            self.add(lines, c_dot, c_label)
            lines.resume_updating()
        kw = {'tex_to_color_map': {'c': YELLOW}}
        new_lines = VGroup(OldTex('z_1 = 0^2 + c = c', **kw), OldTex('z_2 = c^2 + c', **kw), OldTex('z_3 = (c^2 + c)^2 + c', **kw), OldTex('z_4 = ((c^2 + c)^2 + c)^2 + c', **kw), OldTex('\\vdots', **kw))
    ...
    def add_mandelbrot_image(self):
        ...
        self.play(FadeIn(mandelbrot_set, run_time=2), self.plane.animate.set_stroke(WHITE, opacity=0.25))
...
class ParameterSpaceVsSeedSpace(Scene):
    ...
    def construct(self):
        ...
        boxes.to_corner(DR)
        self.add(boxes)
        f_labels = VGroup(OldTex('f(z) = z^2 + c'), OldTex("f(z) = z - {P(z) \\over P'(z)}"))
        kw = {'tex_to_color_map': {'function': YELLOW, 'seed': BLUE_D}}
        top_labels = VGroup(OldTexText('One seed\\\\Pixel $\\leftrightarrow$ function', **kw), OldTexText('One function\\\\Pixel $\\leftrightarrow$ seed', **kw))
...
class ClassicJuliaSetDemo(MandelbrotIntro):
    ...
    def construct(self):
        kw = {'background_line_style': {'stroke_width': 0.5}}
        planes = VGroup(ComplexPlane((-2, 1), (-1.6, 1.6), **kw), ComplexPlane((-2, 2), (-2, 2), **kw))
        ...
        kw = {'tex_to_color_map': {'{z_0}': GREY_A, '{c}': YELLOW, '\\text{Pixel}': BLUE_D}}
        title = OldTexText('Iterate\\\\$z^2 + c$')
        title.move_to(Line(*planes))
        title[0][-1].set_color(YELLOW)
        self.add(title)
        labels = VGroup(VGroup(OldTex('{c} \\leftrightarrow \\text{Pixel}', **kw), OldTex('z_0 = 0', **kw)), VGroup(OldTex('{c} = \\text{const.}', **kw), OldTex('z_0 \\leftrightarrow \\text{Pixel}', **kw)))
        for label, plane in zip(labels, planes):
            label.arrange(DOWN, aligned_edge=LEFT)
            label.next_to(plane, UP)
        space_labels = VGroup(OldTex('{c}\\text{-space}', **kw), OldTex('{z_0}\\text{-space}', **kw))
        ...
        self.add(labels[0])
        self.wait(2)
        self.play(TransformFromCopy(labels[0][0][0], labels[1][0][0]), FadeIn(labels[1][0][1]))
        self.wait(2)
        self.play(TransformFromCopy(labels[0][1].get_part_by_tex('z_0'), labels[1][1].get_part_by_tex('z_0')), FadeTransform(labels[0][0][1:].copy(), labels[1][1][1:]))

class AskAboutGeneralTheory(TeacherStudentsScene):

    def construct(self):
        self.teacher_says(OldTexText('Think about constructing\\\\a general theory'), added_anims=[self.change_students('pondering', 'thinking', 'pondering', look_at=UP)])
        self.wait(3)
        self.teacher_says(OldTexText('What questions would\\\\you ask?'), target_mode='tease')
...
class FixedPoints(Scene):
    ...
    def construct(self):
        ...
        t2c = {'z': BLUE}
        kw = {'tex_to_color_map': t2c, 'isolate': ['=', '\\Rightarrow', 'A(', 'B(', ')']}
        equation = OldTex('f(z) = z', **kw)
        equation.next_to(question, DOWN, MED_LARGE_BUFF)
        newton_example = OldTex("z - {P(z) \\over P'(z)} = z", '\\quad \\Leftrightarrow \\quad ', 'P(z) = 0', **kw)
        newton_example.next_to(equation, DOWN, buff=LARGE_BUFF)
        mandelbrot_example = OldTex('\\text{Exercise 1a: Find the fixed points}\\\\', '\\text{of }', 'f(z) = z^2 + c', alignment='\\centering', **kw)
        ...
        fp_arrow = Arrow(fixed_point.get_left(), equation[1].get_bottom(), path_arc=-PI / 4)
        fp_group = VGroup(fixed_point, fp_arrow)
        fp_group.set_color(YELLOW)
        self.play(FadeIn(equation, DOWN))
        self.wait(2)
        self.play(FadeIn(fixed_point), ShowCreation(fp_arrow))
        self.wait()
        self.play(FadeIn(newton_example, shift=0.5 * DOWN), FadeOut(fp_group))
        self.wait(2)
        self.play(FadeOut(newton_example, 0.5 * DOWN), FadeIn(mandelbrot_example, 0.5 * DOWN))
        self.wait(2)
        ...
        rational_parts = VGroup(OldTex('{A(z) \\over B(z)} = z', **kw), OldTex('A(z) = z \\cdot B(z)', **kw), OldTex('A(z) - z \\cdot B(z) = 0', **kw))
        ...
        self.play(FadeOut(rule, UP), MoveToTarget(question_group), MoveToTarget(iterations), FadeOut(mandelbrot_example), FadeIn(rational_parts[0]))
        self.wait()
        for p1, p2 in zip(rational_parts, rational_parts[1:]):
            self.play(TransformMatchingTex(p1.copy(), p2, path_arc=PI / 2, run_time=2, fade_transform_mismatches=True))
            self.wait(2)
        ...
        self.play(ShowCreation(rect), Write(solution_words, run_time=1))
        self.wait()
        example_roots = [-1.5 + 0.5j, -1.5 - 0.5j, -1.0 + 1.2j, -1.0 - 1.2j, 1.0 + 1j, 1.0 - 1j, 0.5, 1.7]
        glow_dots = VGroup(*(glow_dot(plane.n2p(root)) for root in example_roots))
        self.play(LaggedStartMap(FadeIn, glow_dots, scale=0.5, lag_ratio=0.2))
        self.wait()
        ...
        stability_words = VGroup(Text('Attracting'), Text('Repelling'))
        ...
        self.play(FadeOut(question, RIGHT), FadeIn(stability_question, RIGHT), FadeOut(arrow), FadeOut(z_dot), FadeOut(z_label))
        ...
        self.play(LaggedStartMap(FadeOut, VGroup(*rational_parts, rect, solution_words)), VFadeIn(morty), morty.change('tease'))
        self.play(PiCreatureSays(morty, 'Use derivatives!', target_mode='hooray', bubble_config={'height': 2, 'width': 4}))
        self.play(Blink(morty))
        self.wait()
        self.play(RemovePiCreatureBubble(morty, target_mode='happy'))
        for condition, words in zip(conditions, stability_words):
            self.play(Write(condition), MoveToTarget(words))
            self.wait()
        self.play(FadeInFromPoint(deriv_ineq, morty.get_corner(UL)), MoveToTarget(equation), morty.change('raise_right_hand'))
        self.play(Blink(morty))
        newton_case = VGroup(OldTex("f(z) = z - {P(z) \\over P'(z)}", **kw), OldTex("f'(z) = {P(z)P''(z) \\over P'(z)^2}", **kw), OldTex("P(z) = 0 \\quad \\Rightarrow \\quad f'(z) = 0", **kw))
        ...
        self.play(TransformMatchingTex(equation.copy()[:3], newton_case[0]), morty.change('pondering', newton_case[0]))
        self.wait()
        self.play(TransformMatchingTex(deriv_ineq[:-1].copy(), alt_line1[:4]))
        self.wait()
        self.play(FadeIn(alt_line1[4:], lag_ratio=0.1, run_time=1.5), morty.animate.scale(0.8, about_edge=DR).change('sassy', alt_line1))
        self.play(Blink(morty))
        self.wait()
        self.play(TransformMatchingTex(alt_line1[4:], newton_case[1][4:]), morty.change('tease', alt_line1))
        ...
        self.play(LaggedStartMap(FadeOut, VGroup(*conditions, *stability_words, outward_arrows, inward_arrows)), LaggedStartMap(FadeIn, super_arrows, scale=0.5))
        self.play(LaggedStart(*attraction_anims, lag_ratio=0.02), run_time=3)
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(*super_arrows, *newton_case, rect, super_words, morty, glow_dots)))
        part1 = mandelbrot_example
        part1.set_height(0.9)
        part2 = OldTexText('Exercise 1b: Determine when at least\\\\one fixed point is attracting.')
        part3 = OldTexText('Exercise 1c$^{**}$: Show that the set of values\\\\', '$c$ satisfying this form a cardioid.', tex_to_color_map={'$c$': YELLOW})
        ...
        parts.next_to(VGroup(equation, deriv_ineq), DOWN, buff=LARGE_BUFF)
        mandelbrot = MandelbrotFractal(plane)
        R = 0.25
        cardioid = ParametricCurve(lambda t: plane.c2p(2 * R * math.cos(t) - R * math.cos(2 * t), 2 * R * math.sin(t) - R * math.sin(2 * t)), t_range=(0, TAU))
        cardioid.set_stroke(YELLOW, 4)
        self.add(mandelbrot, plane)
        plane.generate_target(use_deepcopy=True)
        plane.target.set_stroke(WHITE, opacity=0.5)
        self.play(MoveToTarget(plane), FadeIn(mandelbrot), FadeIn(part1))
        self.wait()
    ...
    def add_plane(self):
        plane = self.plane = ComplexPlane((-2, 2), (-2, 2), background_line_style={'stroke_width': 1})
        plane.set_height(5)
        plane.to_corner(DL)
    ...
    def get_arrow_loop(self, dot):
        arrow = Line(dot.get_bottom(), dot.get_right(), path_arc=330 * DEGREES, buff=0.05)
...
class UseNewton(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says(OldTexText('You could solve\\\\ $A(z) - z\\cdot B(z) = 0$ \\\\', "using Newton's method"), bubble_config={'width': 4, 'height': 3}, target_mode='hooray', added_anims=[self.change_students('confused', 'erm', 'maybe')])
        self.wait(2)
        self.student_says(Text('Too meta...'), target_mode='sassy', index=2)
        self.wait(3)
...
class DescribeDerivative(Scene):
    ...
    def construct(self):
        ...
        kw = {'tex_to_color_map': {'z': GREY_A, z_str: YELLOW}, 'isolate': ['f', '(', ')', '=', z_str, fz_str, fpz_str]}
        ...
        self.wait()
        self.play(FadeIn(df_label, shift=0.5 * DOWN), FadeOut(df_question), Uncreate(df_arrow))
        density = 10
        dot_radius = 0.025
        dots = DotCloud([plane.c2p(x, y) for x in np.arange(-3.7, 3.7, 1.0 / density) for y in np.arange(-2.0, 2.1, 1.0 / density)])
        ...
        epsilon = 0.005
        tiny_dots = DotCloud([plane.n2p(self.z + epsilon * complex(x, y)) for x in np.arange(-20, 20) for y in np.arange(-10, 10)])
        ...
        self.wait()
        ex_labels = VGroup(OldTex(f'f({z_str}) = {fz_str}', **kw), OldTex(f"f'({z_str}) = {fpz_str}", **kw))
        ...
        corner_rect.target.set_width(VGroup(ex_labels, labels).get_width() + 0.5, about_edge=LEFT, stretch=True)
        self.add(*corner_group)
        self.play(MoveToTarget(corner_rect), FadeIn(ex_labels))
        self.wait()
        fade_anims = []
        if self.zoom_in_frame:
            rect = self.camera.frame
            plane.generate_target()
            fade_anims = [ApplyMethod(dots.set_opacity, 0, rate_func=squish_rate_func(smooth, 0.5, 1.0)), plane.animate.set_stroke(width=0.25)]
        else:
            rect = ScreenRectangle()
            rect.set_height(FRAME_HEIGHT)
            rect.set_stroke(WHITE, 2)
        self.play(rect.animate.replace(tiny_dots, 1).move_to(plane.n2p(self.z)), *fade_anims, FadeIn(tiny_dots), run_time=3)
        self.wait()
        ...
        self.play(Homotopy(homotopy, dots), Homotopy(homotopy, tiny_dots), MoveAlongPath(rect, path), run_time=5)
        self.wait()
...
class Cycles(FixedPoints):
    ...
    def construct(self):
        ...
        arrows = VGroup(Arrow(z0_dot, z1_dot, **kw), Arrow(z1_dot, z0_dot, **kw))
        arrows.set_stroke(opacity=0.75)
        z_dot = z0_dot.copy()
        z_dot.set_color(YELLOW)
        self.add(question)
        self.play(ShowCreation(arrows[0]), TransformFromCopy(z0_dot, z1_dot, path_arc=-PI / 3), TransformFromCopy(z0_label, z1_label, path_arc=-PI / 3))
        self.wait()
        self.play(TransformFromCopy(z1_dot, z_dot, path_arc=-PI / 3), ShowCreation(arrows[1]))
        self.wait()
        for n in range(1, 5):
            self.play(z_dot.move_to, z_dots[n % 2], path_arc=PI / 3)
            self.wait()
        kw = {'tex_to_color_map': {'z': BLUE}, 'isolate': ['f']}
        ...
        self.play(julia_fractal.animate.set_opacity(0.75), Write(z2c))
        self.wait()
        julia_f2_eqs = VGroup(OldTex('(z^2 + c)^2 + c = z', **kw), OldTex('z^4 + 2cz^2 -z + c^2 + c = 0', **kw))
        julia_f2_eqs.arrange(DOWN, buff=0.7, aligned_edge=LEFT)
        julia_f2_eqs.next_to(f2_equation, DOWN, buff=1.0, aligned_edge=LEFT)
        eq_arrows = VGroup(Arrow(f2_equation.get_bottom(), julia_f2_eqs.get_top()), Arrow(z2c.get_bottom(), julia_f2_eqs.get_top()))
        ...
        self.wait()
        fixed_dots = VGroup(Dot(plane.c2p(0.8, -0.5), color=GREY_A), Dot(plane.c2p(-0.9, -0.6), color=GREY_A))
        ...
        for dot, loop in zip(fixed_dots, arrow_loops):
            ...
            self.play(FadeIn(dot, scale=0.3), ShowCreation(loop))
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(eq_arrows, *julia_f2_eqs, *fixed_dots, *arrow_loops)))
        ...
        self.play(TransformMatchingShapes(f2_equation, fn_eq), FadeOut(z2c), ReplacementTransform(arrows, n_arrows), *map(GrowFromCenter, z_dots[2:]))
        self.play(GrowFromCenter(brace), FadeIn(brace_tex, SMALL_BUFF * DOWN), Succession(*dot_anims, run_time=3))
        self.wait()
        morty = Mortimer(height=2)
        morty.to_corner(DR)
        z2c.next_to(fn_eq, UP, buff=MED_LARGE_BUFF)
        self.play(PiCreatureSays(morty, 'How many solutions?', bubble_config={'height': 2, 'width': 4}))
        self.play(Blink(morty))
        self.wait()
        self.play(FadeIn(z2c), RemovePiCreatureBubble(morty, target_mode='pondering', look_at=z2c))
        self.wait()
        million = Integer(1000000.0, font_size=36)
        million.next_to(brace, DOWN)
        million.set_value(0)
        mega_poly = OldTex('z^{2^{1{,}000{,}000}} +', '\\cdots \\text{(nightmare)} \\cdots', '= 0', **kw)
        mega_poly.next_to(million, DOWN, buff=0.75)
        mega_poly.align_to(fn_eq, LEFT)
        expr = self.show_composition(million, morty, **kw)
        self.play(ChangeDecimalToValue(million, 1000000.0, run_time=2), VFadeIn(million), FadeOut(brace_tex), morty.change('raise_right_hand', fn_eq))
        self.play(Blink(morty))
        self.play(FadeOut(expr), Write(mega_poly), morty.animate.set_height(1.8, about_edge=DR).change('horrified', mega_poly))
        ...
        dots.set_opacity(1)
        dots.add_updater(lambda m: m)
        self.play(FadeOut(z_labels), FadeOut(z_dots), FadeOut(n_arrows), morty.change('erm', dots), ShowCreation(dots, run_time=5))
        self.wait()
        self.play(FadeOut(dots), FadeOut(mega_poly), FadeOut(morty), FadeOut(z2c))
        ...
        for arrow, dot in zip(n_arrows, [*z_dots[1:], z_dots[0]]):
            self.play(ShowCreation(arrow), z_dot.animate.move_to(dot), path_arc=PI / 6)
            self.add(dot, z_dot)
        ...
        self.play(Write(new_question, run_time=1), fn_eq.animate.shift(0.5 * DOWN), FadeOut(brace), FadeOut(million))
        ...
        for n in range(3):
            for mult, dot in zip(multipliers, [*z_dots[1:], z_dots[0]]):
                self.play(ApplyMethod(z_dot.move_to, dot, path_arc=PI / 6), h_tracker.animate.set_value(h_tracker.get_value() * mult))
        theorem = OldTexText('Theorem (Fatou 1919): If $f(z)$ has an\\\\', 'attracting cycle, then at least one solution\\\\', "to $f'(z) = 0$ will fall into it.", font_size=36)
        theorem.arrange(DOWN, buff=0.15, aligned_edge=LEFT)
        theorem.next_to(fn_eq, DOWN, LARGE_BUFF, aligned_edge=LEFT)
        self.play(rational.animate.set_height(0.8).next_to(fn_eq, RIGHT, LARGE_BUFF), FadeIn(theorem))
    ...
    def show_composition(self, ref_mob, morty, **kwargs):
        ...
        curr_poly = polys[0]
        self.play(FadeIn(curr_poly), FadeIn(degree), morty.change('tease', curr_poly))
        for n, poly in enumerate(polys[1:]):
            anims = []
            if n == 4:
                anims.append(morty.change('erm'))
            self.play(curr_poly.animate.replace(poly[1]), FadeIn(poly[::2]), UpdateFromAlphaFunc(degree, lambda m, a: m[1].set_value(2 ** (n + 1) if a < 0.5 else 2 ** (n + 2))), *anims, run_time=1 if n < 5 else 0.25)
...
class CyclesHaveSolutions(Scene):
    ...
    def construct(self):
        text = VGroup(OldTex('f^n(z) = z \\text{ has solutions}', tex_to_color_map={'z': BLUE}), OldTex('\\sim D^n \\text{ of them...}'))
        text.arrange(DOWN)
        for part in text:
...
class MandelbrotFunctions(Scene):
    ...
    def construct(self):
        kw = {'tex_to_color_map': {'c': YELLOW}}
        group = VGroup(OldTex('f(z) = z^2 + c', **kw), OldTex("f'(z) = 2z"))
...
class AmbientNewtonRepetition(RepeatedNewton):
    ...
    dot_density = 10.0
    points_scalar = 2.0
    dots_config = {'radius': 0.025, 'color': GREY_A, 'gloss': 0.4, 'shadow': 0.1, 'opacity': 0.5}
...
class AmbientNewtonBoundary(AmbientNewtonRepetition):
    ...
    def construct(self):
        ...
        fractal.set_julia_highlight(0.0001)
...
class CyclicAttractor(RepeatedNewton):
    ...
    def get_original_points(self):
        return [(r * np.cos(theta), r * np.sin(theta), 0) for r in np.linspace(0, self.cluster_radius, 10) for theta in np.linspace(0, TAU, int(50 * r)) + TAU * np.random.random()]
...
class CyclicAttractorSmallRadius(CyclicAttractor):
    ...
    def construct(self):
        super().construct()
        fractal = NewtonFractal(self.plane, coefs=self.coefs, colors=self.colors, black_for_cycles=True)
...
class CyclicExercise(Scene):
    ...
    def construct(self):
        words = OldTexText('Exercise 2: If $f(z) = z - {z^3 - 2z + 2 \\over 3z^2 - 2}$,\\\\', "and $g(z) = f(f(z))$, confirm that $|g'(0)| < 1$.")
        ...
        hint = OldTexText("Hint: Don't expand out $g(z)$. Use\\\\", "the chain rule: $g'(0) = f'(f(0))f'(0)$")
        hint.scale(0.8)
...
class AskHowOftenThisHappensAlt(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says(OldTexText('How often does\\\\this happen?'), bubble_config={'height': 3, 'width': 4}, index=0)
        self.play(self.teacher.change('raise_right_hand', 3 * UR), self.change_students('raise_left_hand', 'pondering', 'pondering', look_at=3 * UR))
        self.play_student_changes('raise_left_hand', 'erm', 'erm', look_at=3 * UR)
        self.wait()
        self.play(self.teacher.change('tease', 3 * UR))
        self.play_student_changes('confused', 'pondering', 'thinking', look_at=3 * UR)
        self.wait(8)
        return
        self.wait(2)
        self.play(PiCreatureSays(self.teacher, "You'll like this", target_mode='tease', run_time=1), self.students[1].change('thinking', self.teacher.eyes), self.students[2].change('thinking', self.teacher.eyes))
        self.wait(2)
...
class WhatDoesBlackMean(Scene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(arrow), Write(lhs))
        self.wait()
...
class ShowCenterOfMassPoint(PlayWithRootsSeekingCycles):
    ...
    def construct(self):
        ...
        self.play(self.root_dots[2].animate.move_to(self.plane.n2p(-3)), rate_func=there_and_back, run_time=10)
...
class CenterOfMassStatement(Scene):
    ...
    def construct(self):
        words = OldTexText("If there's an attracting cycle, the seed\\\\", '$z_0 = (r_1 + r_2 + r_3) / 3$ will fall into it.')
        words.set_stroke(BLACK, 8, background=True)
        words.to_corner(UL)
...
class GenerateCubicParameterPlot(Scene):
    ...
    def construct(self):
        colors = self.colors
        kw = {'tex_to_color_map': {'\\lambda': colors[2]}}
        title = OldTex("z_{n+1} = z_n - {P(z_n) \\over P'(z_n)} \\qquad\\qquad ", 'P(z) = (z - 1)(z + 1)(z - \\lambda)', font_size=30, **kw)
        title.to_edge(UP)
        self.add(title)
        planes = VGroup(*(ComplexPlane((-2, 2), (-2, 2), background_line_style={'stroke_color': GREY_B, 'stroke_opacity': 0.5}) for x in range(2)))
        ...
        lambda_label.add_updater(lambda m: m.next_to(root_dots[2], UR, buff=SMALL_BUFF))
        left_fractal = NewtonFractal(planes[0], coefs=[-1, 0, 0, 1], colors=colors, black_for_cycles=True)
        left_fractal.add_updater(lambda m: m.set_roots([planes[0].p2n(rd.get_center()) for rd in root_dots]))
        right_fractal = MetaNewtonFractal(planes[1], colors=colors, fixed_roots=[-1, 1])
        ...
        plane_titles = VGroup(OldTexText('Pixel $\\leftrightarrow z_0$'), OldTexText('Pixel $\\leftrightarrow$ ', '$\\lambda$'))
        ...
        for rd in root_dots[:2]:
            ...
            pins.add(pin)
            self.play(FadeIn(pin, 0.25 * DR), FlashAround(rd))
        self.wait()
        circle = Circle()
        circle.scale(0.5)
        circle.rotate(PI / 2)
        ...
        for y, back in zip(y_range, it.cycle([False, True])):
            ...
            for x in x_range[::-1] if back else x_range:
                ...
                arrow.put_start_and_end_on(com_dot.get_center(), square.get_center())
                self.wait(1 / 15)
        self.play(FadeOut(arrow), FadeOut(square))
        self.wait()
        frame = self.camera.frame
        self.play(frame.animate.replace(planes[1], 1), run_time=3)
        self.wait()
...
class SmallCircleProperty(Scene):
    ...
    def construct(self):
        rule = OldTexText('For any rational map, color points\\\\based on their limiting behavior...\\\\', '(which limit point, which limit cycle, etc.)', font_size=36)
        rule.to_corner(UL)
        rule[-1].shift(SMALL_BUFF * DOWN)
        rule[-1].set_color(GREY_B)
        r_map = OldTex('z_{n + 1} = A(z_n) / B(z_n)')
        r_map.to_corner(UR)
        colors = [MANDELBROT_COLORS[0], BLUE_C, BLUE_E, ROOT_COLORS_DEEP[1]]
        plane = ComplexPlane((-2, 2), (-2, 2))
        plane.set_height(5)
        plane.to_corner(DR)
        ...
        circle_labels = VGroup(Text('One color'), Text('Some colors'), Text('All colors'))
        ...
        for big, lil in zip(circles[::2], lil_circles):
            ...
            lines.add(VGroup(Line(lil.get_top(), big.get_boundary_point(v1)), Line(lil.get_bottom(), big.get_boundary_point(v2))))
        ...
        self.play(TransformFromCopy(lil_circles[0], circles[0]), *map(ShowCreation, lines[0]), FadeIn(circle_labels[0]))
        self.play(Write(marks[0]))
        self.wait()
        self.add(multi_color_image)
        self.add(lil_circles[1])
        self.play(TransformFromCopy(lil_circles[1], circles[2]), *map(ShowCreation, lines[1]), FadeIn(circle_labels[2]))
        self.play(Write(marks[2]))
        self.wait()
        self.play(FadeIn(circle_labels[1]), FadeIn(circles[1]))
...
class MentionFatouSetsAndJuliaSets(Scene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(f_line), FadeOut(f_name), MoveToTarget(mover))
        self.play(Write(otherwise), FadeIn(j_condition, 0.5 * DOWN))
        self.wait()
        self.play(ShowCreation(j_underline), ShowCreation(arrow), FadeIn(j_group[1]), FadeIn(julia_copy))
    ...
    def get_fatou_condition(self):
        zn = OldTex('z_0', '\\overset{f}{\\longrightarrow}', 'z_1', '\\overset{f}{\\longrightarrow}', 'z_2', '\\overset{f}{\\longrightarrow}', '\\dots', '\\longrightarrow')
        words = VGroup(OldTexText('Stable fixed point'), OldTexText('Stable cycle'), OldTexText('$\\infty$'))
...
class ShowJuliaSetPoint(TwoToMillionPoints):
    ...
    def construct(self):
        ...
        for n in range(self.n_steps):
            ...
            self.play(ApplyMethod(dot.move_to, new_point, path_arc=path_arc), ShowCreation(arrow), *anims)
...
class AboutFatouDisks(Scene):
    disk_style = {'fill_color': YELLOW, 'fill_opacity': 0.5, 'stroke_color': YELLOW, 'stroke_width': 1}
    ...
    def play_disk_progression(self, disks, arrows):
        ...
        for d1, d2, arrow in zip(disks, disks[1:], arrows):
            self.play(TransformFromCopy(d1.copy().fade(1), d2), FadeIn(arrow))

    def get_disks(self):
        radii = [*np.linspace(0.5, 1, 3), *np.linspace(1, 0, 7) ** 2 + 0.05]
...
class AboutFatouDisksJustWords(Scene):
    ...
    def construct(self):
        words = OldTexText('(Small enough) disks around points in the Fatou set...\\\\', '...eventually shrink to 0')
        words.arrange(DOWN, aligned_edge=LEFT)
        words.to_corner(UL)
...
class ShowFatouDiskExample(Scene):
    ...
    def construct(self):
        ...
        for n in range(self.n_steps):
            ...
            self.play(dot.animate.move_to(new_point), disk.animate.apply_function(func), ShowCreation(arrow), path_arc=path_arc)
            self.play(FadeOut(arrow))
        self.embed()
...
class AboutJuliaDisks(AboutFatouDisks):
    ...
    def construct(self):
        ...
        self.play(FadeIn(arrows[-1]), FadeIn(words2, run_time=2, lag_ratio=0.1), VFadeIn(plane, suspend_updating=False))
        self.wait()
        self.play(FadeIn(words3, run_time=2, lag_ratio=0.1))
        self.wait(2)
...
class DescribeChaos(Scene):
    ...
    def construct(self):
        ...
        surrounding_dots = VGroup(*(Dot(radius=0.05).move_to(j_dot.get_center() + buff * vect) for n, buff in [(6, 0.2), (12, 0.4)] for vect in compass_directions(n)))
...
class SimulationOfTinyDisk(RepeatedNewton):
    ...
    def construct(self):
        ...
        julia_set.add_updater(lambda m: m.set_julia_highlight(get_height_ratio() * 0.001))
        ...
        n_radii = 200
        dots.set_points([[cluster_radius * r * math.cos(theta), cluster_radius * r * math.sin(theta), 0] for r in np.linspace(1, 0, n_radii) for theta in np.linspace(0, TAU, int(r * 20)) + random.random() * TAU])
        dots.set_height(0.3)
        dots.set_gloss(0.5)
        dots.set_shadow(0.5)
        ...
        self.play(frame.animate.set_height(target_height).move_to(point), run_time=6, rate_func=lambda a: smooth(a ** 0.5))
...
class SimulationAnnotations(Scene):
    ...
    def construct(self):
        ...
        brace = Brace(Line(dots.get_center(), dots.get_right()), DOWN, buff=0)
...
class LattesExample(TeacherStudentsScene):
    ...
    def construct(self):
        example = VGroup(OldTexText("Lattè's example: "), OldTex('L(z)=\\frac{\\left(z^{2}+1\\right)^{2}}{4 z\\left(z^{2}-1\\right)}'))
        ...
        self.play(self.teacher.change('raise_right_hand', 3 * UR), self.change_students('pondering', 'happy', 'tease', look_at=3 * UR))
        self.wait(3)
        self.play(self.teacher.change('sassy', example), Write(example))
        self.play_student_changes('pondering', 'pondering', 'pondering', look_at=example)
        self.play(example.animate.to_edge(UP), FadeIn(j_fact), self.change_students('erm', 'erm', 'erm', look_at=j_fact), self.teacher.change('raise_right_hand', j_fact))
...
class LinksBelow(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play_student_changes('pondering', 'thinking', 'pondering', look_at=FRAME_HEIGHT * DOWN)
        self.wait(2)
...
class MoreAmbientChaos(TwoToMillionPoints):
    ...
    def construct(self):
        ...
        for n in range(n_steps):
            ...
            lines.set_stroke(WHITE, 1)
            self.play(dots.animate.set_points(new_points), ShowCreation(lines, lag_ratio=0))
...
class AmbientJulia(Scene):
    ...
    def construct(self):
        plane = ComplexPlane((-4, 4), (-2, 2), background_line_style={'stroke_color': GREY_A, 'stroke_width': 1})
