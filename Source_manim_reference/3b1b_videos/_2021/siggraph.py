...
class PendulumAxes(Scene):
    ...
    def construct(self):
        plane = NumberPlane((-2, 2), (-2, 2), height=8, width=8, background_line_style={'stroke_color': WHITE})
...
class InterpolatingOrientations(ThreeDScene):
    ...
    def construct(self):
        ...
        axes = ThreeDAxes((-3, 3), (-3, 3), (-2, 2), width=6, height=6, depth=4)
        ...
        if self.matrix:
            mat_mob = DecimalMatrix(get_matrix(), element_to_mobject_config={'num_decimal_places': 2, 'edge_to_fix': RIGHT})
            ...
            quat_label = VGroup(VGroup(DecimalNumber(1, **kw)), VGroup(DecimalNumber(0, **kw), OldTex('i')), VGroup(DecimalNumber(0, **kw), OldTex('j')), VGroup(DecimalNumber(0, **kw), OldTex('k')))
        ...
        def move_to_quaternion(quaternion, run_time=5):
            self.play(quat_tracker.animate.set_value(quaternion), UpdateFromFunc(quat_tracker, lambda m: m.set_value(normalize(m.get_value()))), run_time=run_time)
            self.wait()
