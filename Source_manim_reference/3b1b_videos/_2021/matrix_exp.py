...
def get_vector_field_and_stream_lines(func, coordinate_system, magnitude_range=(0.5, 4), vector_opacity=0.75, vector_thickness=0.03, color_by_magnitude=False, line_color=GREY_A, line_width=3, line_opacity=0.75, sample_freq=5, n_samples_per_line=10, arc_len=3, time_width=0.3):
    vector_field = VectorField(func, coordinate_system, magnitude_range=magnitude_range, vector_config={'fill_opacity': vector_opacity, 'thickness': vector_thickness})
    stream_lines = StreamLines(func, coordinate_system, step_multiple=1.0 / sample_freq, n_samples_per_line=n_samples_per_line, arc_len=arc_len, magnitude_range=magnitude_range, color_by_magnitude=color_by_magnitude, stroke_color=line_color, stroke_width=line_width, stroke_opacity=line_opacity)
    animated_lines = AnimatedStreamLines(stream_lines, line_anim_config={'time_width': time_width})
...
def get_1d_equation(r='r'):
    return OldTex('{d \\over d{t}} x({t}) = {' + r + '} \\cdot x({t})', tex_to_color_map={'{t}': GREY_B, '{' + r + '}': BLUE, '=': WHITE})

def get_2d_equation(matrix=[['a', 'b'], ['c', 'd']]):
    deriv = OldTex('d \\over dt', tex_to_color_map={'t': GREY_B})
    vect = Matrix([['x(t)'], ['y(t)']], bracket_h_buff=SMALL_BUFF, bracket_v_buff=SMALL_BUFF, element_to_mobject_config={'tex_to_color_map': {'t': GREY_B}, 'isolate': ['(', ')']})
...
class ZoomInOnProblem(Scene):
    ...
    def construct(self):
        ...
        full_rect.append_vectorized_mobject(examples_rect.copy().scale(1e-06))
        ...
        self.play(ShowCreation(prob_rect), FadeIn(full_rect), FadeTransform(prob_rect.copy(), problem), GrowArrow(prob_arrow))
        self.wait()
        ...
        matrices = VGroup(IntegerMatrix([[1, 0], [0, 2]]), IntegerMatrix([[0, 1], [0, 0]]), IntegerMatrix([[0, 1], [-1, 0]]), IntegerMatrix([[0, 1, 0], [0, 0, 1], [0, 0, 0]]))
        ...
        self.play(ShowCreation(examples_rect), FadeIn(examples_label), FadeIn(lhs), FadeIn(matrices[0]))
        self.wait()
...
class LeadToPhysicsAndQM(Scene):
    ...
    def construct(self):
        ...
        self.add(de_words)
        self.play(GrowArrow(de_arrow), FadeIn(mat_exp, shift=DOWN))
        self.wait()
        self.play(GrowArrow(top_arrow), FadeIn(physics_words, RIGHT))
        self.wait()
        self.play(FadeOut(physics_words, UP), FadeIn(qm_words, UP))
        self.play(TransformFromCopy(de_arrow, qm_arrow), FadeTransform(mat_exp.copy(), qm_exp))
        self.wait()
...
class PlanForThisVideo(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(PiCreatureSays(s0, OldTexText('But what\\\\is $e^{M}$?'), target_mode='raise_left_hand'), s1.change('erm', UL), s2.change('pondering', UL))
        self.wait()
        s0.bubble = None
        self.play(PiCreatureSays(s2, OldTexText('And who cares?'), target_mode='sassy', bubble_config={'direction': LEFT}), s1.change('hesitant', UL), self.teacher.change('guilty'))
        self.wait(3)
...
class IntroduceTheComputation(Scene):
    ...
    def construct(self):
        base = OldTex('e')
        base.set_height(1.0)
        matrix = IntegerMatrix([[3, 1, 4], [1, 5, 9], [2, 6, 5]])
        matrix.move_to(base.get_corner(UR), DL)
        ...
        self.add(randy)
        self.play(randy.change('pondering', matrix), Write(matrix.get_brackets()), ShowIncreasingSubsets(matrix.get_entries()))
        self.play(matrix.animate.restore(), Write(base), randy.change('erm', base))
        ...
        bubble = randy.get_bubble(OldTexText("I'm sorry,\\\\what?!").scale(0.75), height=2, width=3, bubble_type=SpeechBubble)
        self.play(TransformMatchingParts(base.copy(), rhs, path_arc=10 * DEGREES, lag_ratio=0.01), GrowFromCenter(brace), ReplacementTransform(matrix.copy(), brace_label[0], path_arc=30 * DEGREES, run_time=2, rate_func=squish_rate_func(smooth, 0.3, 1)), Write(brace_label[1], run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)), randy.change('angry', rhs), ShowCreation(bubble), Write(bubble.content, run_time=1))
        self.wait()
        ...
        randy.bubble = bubble
        self.play(MoveToTarget(false_equation), RemovePiCreatureBubble(randy, target_mode='hesitant'), morty.animate.set_opacity(1).change('raise_right_hand'), ShowCreation(fe_rect))
        self.play(ShowCreation(cross), FadeIn(nonsense))
        self.play(Blink(morty))
        self.wait()
        false_group = VGroup(false_equation, fe_rect, cross, nonsense)
        real_equation = OldTex('e^x = x^0 + x^1 + \\frac{1}{2} x^2 + \\frac{1}{6} x^3 + \\cdots + \\frac{1}{n!} x^n + \\cdots', isolate=['x'])
        ...
        self.play(TransformFromCopy(base, real_equation[0]), FadeTransform(matrix.copy(), real_equation[1]), FadeIn(real_label, UR), GrowArrow(real_arrow), randy.change('thinking', real_label), morty.animate.look_at(real_label))
        self.wait()
        self.play(Write(real_equation[2], lag_ratio=0.2), FadeTransformPieces(xs[:1].copy(), xs[1:], path_arc=20 * DEGREES), LaggedStart(*(FadeIn(part) for part in real_equation[4:] if part not in xs)), randy.change('pondering', real_equation), morty.change('pondering', real_equation))
        self.add(real_equation)
        self.play(Blink(morty))
        self.play(false_group.animate.scale(0.7).to_edge(DOWN), GrowFromCenter(taylor_brace), FadeIn(taylor_label, 0.5 * DOWN))
        self.wait()
        ex_rhs = OldTex('\n            {2}^0 +\n            {2}^1 +\n            { {2}^2 \\over 2} +\n            { {2}^3 \\over 6} +\n            { {2}^4 \\over 24} +\n            { {2}^5 \\over 120} +\n            { {2}^6 \\over 720} +\n            { {2}^7 \\over 5040} +\n            \\cdots\n            ', tex_to_color_map={'{2}': YELLOW, '+': WHITE})
        ex_rhs.next_to(real_equation[3:], DOWN, buff=0.75)
        ex_parts = VGroup(*(ex_rhs[i:j] for i, j in [(0, 2), (3, 5), (6, 8), (9, 11), (12, 14), (15, 17), (18, 20), (21, 23), (24, 25)]))
        ...
        rects.set_width(7)
        rects.to_edge(DOWN)
        self.play(ReplacementTransform(taylor_brace, term_brace), FadeTransform(real_equation[3:].copy(), ex_rhs), FadeOut(false_group, shift=DOWN), FadeOut(taylor_label, shift=DOWN), FadeIn(frac))
        term_values = VGroup()
        for n in range(11):
            rect = rects[n]
            ...
            self.play(term_brace.animate.become(new_brace), FadeTransform(frac, new_frac))
            frac = new_frac
            rect.save_state()
            rect.stretch(0, 1, about_edge=DOWN)
            rect.set_opacity(0)
            value.set_value(0)
            self.play(Restore(rect), ChangeDecimalToValue(value, 2 ** n / math.factorial(n)), UpdateFromAlphaFunc(value, lambda m, a: m.next_to(rect, UP, SMALL_BUFF).set_opacity(a)), randy.animate.look_at(rect), morty.animate.look_at(rect))
        ...
        self.play(term_brace.animate.become(new_brace), randy.change('thinking', sum_value), morty.change('tease', sum_value), *(FadeTransform(dec.copy().set_opacity(0), sum_value) for dec in term_values))
        self.play(Blink(randy))
        lhs = OldTex('e \\cdot e =')
        ...
        for n, rect, value_mob in zip(it.count(0), rects, term_values):
            ...
            value_mob.rect = rect
            anims += [MoveToTarget(rect), ChangeDecimalToValue(value_mob, new_value), UpdateFromFunc(value_mob, lambda m: m.next_to(m.rect, UP, SMALL_BUFF))]
        self.play(FadeOut(twos, 0.5 * UP), FadeIn(threes, 0.5 * UP))
        twos.set_opacity(0)
        self.play(ChangeDecimalToValue(sum_value, math.exp(3)), *anims)
        self.play(FadeOut(lhs, 0.5 * UP), FadeIn(new_lhs, 0.5 * UP))
        self.wait()
        real_lhs = VGroup(real_equation[:3], real_label, real_arrow)
        self.play(LaggedStartMap(FadeOut, VGroup(*new_lhs, *threes, *ex_rhs, term_brace, sum_value, *rects, *term_values)), real_lhs.animate.set_opacity(0.2), randy.change('erm', real_equation), morty.change('thinking', real_equation), run_time=1)
        self.play(Blink(morty))
        rhs_tex = 'X^0 + X^1 + \\frac{1}{2} X^2 + \\frac{1}{6} X^3 + \\cdots + \\frac{1}{n!} X^n + \\cdots'
        pii_rhs = OldTex(rhs_tex.replace('X', '(\\pi i)'), tex_to_color_map={'(\\pi i)': BLUE})
        pii_rhs.match_width(real_rhs)
        mat_tex = '\\left[ \\begin{array}{ccc} 3 & 1 & 4 \\\\ 1 & 5 & 9 \\\\ 2 & 6 & 5 \\end{array} \\right]'
        mat_rhs = OldTex(rhs_tex.replace('X', mat_tex), tex_to_color_map={mat_tex: TEAL})
        mat_rhs.scale(0.5)
        pii_rhs.next_to(real_rhs, DOWN, buff=0.7)
        mat_rhs.next_to(pii_rhs, DOWN, buff=0.7)
        self.play(FlashAround(real_rhs))
        self.wait()
        self.play(morty.change('raise_right_hand', pii_rhs), FadeTransformPieces(real_rhs.copy(), pii_rhs))
        self.play(Blink(randy))
        self.play(FadeTransformPieces(real_rhs.copy(), mat_rhs))
        self.play(randy.change('maybe', mat_rhs))
        self.wait()
        why = Text('Why?', font_size=36)
        why.next_to(randy, UP, aligned_edge=LEFT)
        self.play(randy.change('confused', mat_rhs.get_corner(UL)), Write(why))
        self.play(Blink(randy))
        reassurance = VGroup(Text('I know it looks complicated.', font_size=24), Text("Don't panic.", font_size=24))
        ...
        self.play(MoveToTarget(to_right_group), FadeOut(why), FadeOut(reassurance), randy.change('pondering', mat_rhs), morty.change('tease'))
        ...
        self.play(FadeIn(pii_lhs), randy.change('thinking', pii_lhs), randy.change('tease', pii_lhs))
        self.play(FadeIn(mat_lhs))
        self.play(Blink(randy))
        self.play(LaggedStart(FlashAround(pii_lhs[:3]), FlashAround(mat_lhs[:3]), lag_ratio=0.3, run_time=2), randy.change('raise_left_hand', pii_lhs))
        self.wait()
        ...
        for part, lhs in ((pii_lhs_pi_part, pii_lhs), (mat_lhs_mat_part, mat_lhs)):
            ...
            powers.add(power)
            equals.add(equal)
        self.play(ShowCreation(crosses), randy.change('hesitant', crosses))
        self.play(Blink(randy))
        self.play(real_lhs.animate.set_opacity(1))
        self.play(FadeOut(pii_lhs), FadeOut(mat_lhs), FadeOut(crosses), *(MoveToTarget(power) for power in powers), *(TransformFromCopy(real_equation[0], base) for base in bases), Write(equals), randy.change('sassy', powers))
        self.wait()
        ...
        self.play(FadeOut(randy, DOWN), FadeOut(morty, DOWN), real_part.animate.set_x(0).shift(DOWN), def_parts.animate.set_x(0).to_edge(DOWN))
        ...
        self.play(ShowCreation(real_rect), FadeIn(theorem_label, 0.5 * UP))
        self.wait()
        self.play(ShowCreation(def_rect), FadeIn(def_label, 0.5 * UP))
        self.wait()
        randy2 = Randolph()
        randy2.set_height(1.5)
        randy2.next_to(def_rect, UP, SMALL_BUFF, aligned_edge=LEFT)
        self.play(ApplyMethod(randy2.change, 'angry', mat_rhs), UpdateFromAlphaFunc(randy2, lambda m, a: m.set_opacity(a)))
        ...
        self.play(FadeOut(theorem_label, UP), FadeIn(discovery_label, UP), randy2.change('hesitant', theorem_label))
        self.play(FadeOut(def_label, UP), FadeIn(invention_label, UP))
        self.wait(2)
        to_fade = VGroup(randy2, discovery_label, invention_label, real_rect, def_rect, real_arrow, real_label, real_part, pii_part, bases, powers, equals)
        self.play(mat_rhs.animate.set_width(FRAME_WIDTH - 1).center().to_edge(UP), LaggedStartMap(FadeOut, to_fade), FadeIn(VGroup(randy, morty), run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)))
        mat = mat_rhs[4]
        ...
        self.play(LaggedStart(TransformFromCopy(mat, square_eq[0], path_arc=45 * DEGREES), TransformFromCopy(mat, square_eq[1]), Write(square_eq[2]), Write(result.brackets)), randy.change('pondering', square_eq))
        ...
        mat_squared = result
        mat_cubed = IntegerMatrix(np.dot(matrix, matrix_square), h_buff=1.8, v_buff=0.7, element_alignment_corner=ORIGIN)
        mat_cubed.match_height(mat)
        cube_eq = VGroup(VGroup(mat.copy(), mat.copy(), mat.copy()).arrange(RIGHT, buff=SMALL_BUFF), OldTex('=').rotate(90 * DEGREES), VGroup(mat.copy(), mat_squared.deepcopy()).arrange(RIGHT, buff=SMALL_BUFF), OldTex('=').rotate(90 * DEGREES), mat_cubed)
        cube_eq.arrange(DOWN)
        cube_eq.next_to(mat_brace.target, DOWN)
        self.play(MoveToTarget(mat_brace), ReplacementTransform(square_eq[0], cube_eq[0][1]), ReplacementTransform(square_eq[1], cube_eq[0][2]), ReplacementTransform(square_eq[2], cube_eq[1]), ReplacementTransform(square_eq[3], cube_eq[2][1]), randy.change('happy', cube_eq))
        self.play(LaggedStart(FadeIn(cube_eq[0][0]), FadeIn(cube_eq[2][0]), FadeIn(cube_eq[3]), FadeIn(cube_eq[4].brackets)), randy.change('tease', cube_eq))
        self.show_mat_mult(matrix, matrix_square, cube_eq[2][0][2:11], cube_eq[2][1].get_entries(), cube_eq[4].get_entries(), 0.1, 0.1)
        self.play(Blink(morty))
        example_matrix = Matrix([['a', 'b', 'c'], ['d', 'e', 'f'], ['g', 'h', 'i']])
        example_scaled_matrix = Matrix([['a / n!', 'b / n!', 'c / n!'], ['d / n!', 'e / n!', 'f / n!'], ['g / n!', 'h / n!', 'i / n!']])
        factor = OldTex('1 \\over n!')
        factor.scale(1.5)
        factor.next_to(example_matrix, LEFT, MED_SMALL_BUFF)
        self.play(LaggedStartMap(FadeOut, VGroup(mat_brace, *cube_eq[:-1])), FadeIn(factor), FadeTransformPieces(cube_eq[-1], example_matrix))
        self.wait()
        self.play(TransformMatchingShapes(VGroup(*factor, *example_matrix), example_scaled_matrix), randy.change('pondering', example_scaled_matrix))
        self.wait()
        mat1 = np.array([[2, 7, 1], [8, 2, 8], [1, 8, 2]])
        mat2 = np.array([[8, 4, 5], [9, 0, 4], [5, 2, 3]])
        sum_eq = VGroup(IntegerMatrix(mat1), OldTex('+'), IntegerMatrix(mat2), OldTex('='), Matrix(np.array([f'{m1} + {m2}' for m1, m2 in zip(mat1.flatten(), mat2.flatten())]).reshape((3, 3)), h_buff=1.8))
        sum_eq.set_height(1.5)
        sum_eq.arrange(RIGHT)
        sum_eq.center()
        self.play(FadeOut(example_scaled_matrix, UP), FadeIn(sum_eq[:-1], UP), FadeIn(sum_eq[-1].brackets, UP), morty.change('raise_right_hand', sum_eq), randy.change('thinking', sum_eq))
        last_rects = VGroup()
        ...
        self.play(Write(bubble), Write(bubble.content), FadeOut(sum_eq, UP), randy.change('sassy', mat_rhs), morty.change('guilty', randy.eyes))
        self.play(Blink(randy))
        self.wait()
        self.play(FadeOut(bubble), bubble.content.animate.next_to(randy, RIGHT, aligned_edge=UP), randy.change('pondering', mat_rhs), morty.change('pondering', mat_rhs))
        pi_mat_tex = ''
        pi_mat_tex = '\\left[ \\begin{array}{cc} 0 & -\\pi \\\\ \\pi & 0 \\end{array} \\right]'
        pi_mat_rhs = OldTex(rhs_tex.replace('X', pi_mat_tex), tex_to_color_map={pi_mat_tex: BLUE})
        ...
        self.play(Blink(morty))
        self.play(FadeTransformPieces(mat_rhs, pi_mat_rhs), Transform(VGroup(pi_mat), pi_mat_rhs.get_parts_by_tex(pi_mat_tex), remover=True), morty.change('tease'))
        self.wait()
        ...
        self.play(GrowFromCenter(brace), FadeTransform(mat_parts[0].copy(), curr_sum_mob), randy.change('erm', curr_sum_mob))
        self.wait()
        ...
        for n in range(1, 18):
            ...
            new_sum_mob = DecimalMatrix(curr_sum, element_alignment_corner=RIGHT, element_to_mobject_config={'num_decimal_places': nd, 'font_size': 36}, h_buff=h_buff)
            new_sum_mob.match_height(curr_sum_mob)
            new_sum_mob.next_to(brace.target, DOWN)
            self.play(FadeOut(curr_sum_mob), FadeIn(new_sum_mob), randy.animate.look_at(new_sum_mob), *anims, run_time=1 if n < 5 else 1 / 60)
            self.wait()
            curr_sum_mob = new_sum_mob
            partial_sum_mobs.append(new_sum_mob)
        self.play(FadeOut(last_n_label), randy.change('confused', curr_sum_mob))
        why = Text('Why?')
        ...
        self.play(randy.change('maybe'), FadeIn(why, UP), FadeOut(bubble.content, UP))
        self.wait()
        self.play(morty.change('raise_right_hand'), FadeIn(epii, UP))
        self.play(Blink(morty))
        self.play(Write(later_text, run_time=1), randy.change('hesitant', morty.eyes))
        new_mat_rhs = OldTex(rhs_tex.replace('X', mat_tex), tex_to_color_map={mat_tex: TEAL}, isolate=['+'])
        new_mat_rhs.replace(mat_rhs)
        self.play(FadeOut(pi_mat_rhs), FadeIn(new_mat_rhs), FadeOut(new_sum_mob, DOWN), brace.animate.become(Brace(new_mat_rhs, DOWN)), LaggedStartMap(FadeOut, VGroup(why, epii, later_text), shift=DOWN), randy.change('pondering', new_mat_rhs), morty.change('pondering', new_mat_rhs))
        matrix = np.array([[3, 1, 4], [1, 5, 9], [2, 6, 5]])
        partial_sum_mobs = VGroup()
        curr_matrix = np.identity(3)
        partial_sum = np.array(curr_matrix)
        for n in range(50):
            psm = DecimalMatrix(partial_sum, element_to_mobject_config={'num_decimal_places': 2}, element_alignment_corner=ORIGIN, h_buff=1.5 * DecimalNumber(partial_sum[0, 0]).get_width(), v_buff=1.0)
        ...
        for n in range(6, 50):
            ...
            self.wait(0.1)
        self.play(randy.change('erm'), morty.change('tease'))
        self.play(Blink(morty))
        self.play(Blink(randy))
        self.wait()
...
class ShowHigherMatrixPowers(IntroduceTheComputation):
    ...
    def construct(self):
        ...
        equation = VGroup(mat_mobs[1].deepcopy(), Integer(2, font_size=18), OldTex('='), mat_mobs[1].deepcopy(), mat_mobs[1].deepcopy(), OldTex('='), mat_mobs[2].deepcopy())
        equation.arrange(RIGHT)
        ...
        for n in range(3, N):
            self.show_mat_mult(matrix_powers[1], matrix_powers[n - 2], m1.get_entries(), m2.get_entries(), rhs.get_entries(), between_terms=self.between_terms, per_term=self.per_term)
            self.wait(0.5)
            rhs.generate_target()
            eq.generate_target()
            ...
            self.play(MoveToTarget(rhs, path_arc=PI / 2), MoveToTarget(eq), FadeOut(m2, DOWN), FadeIn(new_rhs.get_brackets()), FadeIn(new_exp, 0.5 * DOWN), FadeOut(exp, 0.5 * DOWN))
            m2, rhs = (rhs, new_rhs)
            exp = new_exp
...
class WhyTortureMatrices(TeacherStudentsScene):
    ...
    def construct(self):
        self.play_student_changes('maybe', 'confused', 'erm', look_at=self.screen)
        q_marks = VGroup()
        ...
        self.wait(2)
        self.student_says(OldTexText('Why...would you\\\\ever want\\\\to do that?'), index=2, added_anims=[FadeOut(q_marks)])
        self.play(self.change_students('confused', 'pondering', 'raise_left_hand', look_at=self.screen), self.teacher.change('tease', self.screen))
        self.wait(2)
...
class DefinitionFirstVsLast(Scene):
    ...
    def construct(self):
        ...
        top_prog = VGroup(OldTexText('Definition', color=BLUE), OldTexText('Theorem'), OldTexText('Proof'), OldTexText('Examples'))
        low_prog = VGroup(OldTexText('Specific\n\nproblem'), OldTexText('General\n\nproblems'), OldTexText('Helpful\n\nconstructs'), OldTexText('Definition', color=BLUE))
        ...
        versus = Text('vs.')
        self.add(top_prog[0])
        self.play(GrowArrow(top_prog.arrows[0]), FadeIn(top_dots, 0.2 * RIGHT, lag_ratio=0.1))
        self.wait()
        kw = {'path_arc': -90 * DEGREES}
        self.play(LaggedStart(TransformFromCopy(top_prog[0], low_prog[-1], **kw), TransformFromCopy(top_prog.arrows[0], low_prog.arrows[-1], **kw), TransformFromCopy(top_dots, low_dots, **kw)), Write(versus))
        self.wait()
        self.play(ShowCreation(top_rect), FadeIn(top_title, 0.25 * UP))
        self.play(FadeOut(top_dots), FadeIn(top_prog[1]))
        for arrow, term in zip(top_prog.arrows[1:], top_prog[2:]):
            self.play(GrowArrow(arrow), FadeIn(term, shift=0.25 * RIGHT))
        self.wait()
        self.play(ShowCreation(low_rect), FadeIn(low_title, 0.25 * UP), versus.animate.move_to(midpoint(low_title.get_top(), top_rect.get_bottom())))
        self.wait()
        self.play(FadeIn(low_prog[0]))
        self.play(GrowArrow(low_prog.arrows[0]), FadeIn(low_prog[1], shift=0.25 * RIGHT))
        self.play(GrowArrow(low_prog.arrows[1]), FadeIn(low_prog[2], shift=0.25 * RIGHT), FadeOut(low_dots))
        self.wait()
        full_rect = FullScreenRectangle()
        ...
        self.wait()
        self.play(love.animate.center().scale(1.5), FadeOut(quantum), FadeOut(sp), full_rect.animate.set_fill(opacity=1))
        self.wait()
...
class RomeoAndJuliet(Scene):
    ...
    def construct(self):
        ...
        for creature in lovers:
            self.play(creature.love_tracker.animate.set_value(2.5), Write(creature.name_label, run_time=1))
        self.wait()
        ...
        scale_labels = VGroup(OldTexText("Juliet's love for Romeo", font_size=30), OldTexText("Romeo's love for Juliet", font_size=30))
        scale_arrows = VGroup()
        ...
        for lover, scale, arrow, label, final_love in zip(reversed(lovers), scales, scale_arrows, scale_labels, [1, -1]):
            ...
            self.play(lover.love_tracker.animate.set_value(5), GrowArrow(arrow), FadeIn(label, 0.5 * UP))
            self.play(lover.love_tracker.animate.set_value(final_love), run_time=2)
            self.wait()
        frame = self.camera.frame
        equations = VGroup(OldTex('{dx \\over dt} {{=}} -{{y(t)}}'), OldTex('{dy \\over dt} {{=}} {{x(t)}}'))
        juliet_eq, romeo_eq = equations
        juliet_eq.next_to(scale_labels[0], UR)
        juliet_eq.shift(0.5 * UP)
        self.play(frame.animate.move_to(0.7 * UP), Write(equations[0]))
        self.wait()
        ...
        big_arrow = Arrow(juliet.scale_mob.number_line.get_bottom(), juliet.scale_mob.number_line.get_top())
        big_arrow.set_color(GREEN)
        big_arrow.next_to(juliet.scale_mob.number_line, LEFT)
        self.play(FadeIn(big_arrow), ApplyMethod(juliet.love_tracker.set_value, 5, run_time=3, rate_func=linear))
        self.wait()
        self.play(romeo.love_tracker.animate.set_value(5))
        self.play(big_arrow.animate.rotate(PI).set_color(RED), path_arc=PI, run_time=0.5)
        ...
        self.play(juliet_eq.animate.to_edge(LEFT), FadeOut(big_arrow))
        ...
        self.wait()
        self.play(big_arrow.animate.rotate(PI).set_color(GREEN), path_arc=PI, run_time=0.5)
        ...
        self.play(VFadeIn(left_arrow), ApplyMethod(big_arrow.scale, 0, remover=True, run_time=3), ApplyMethod(juliet.love_tracker.set_value, 0, run_time=3))
        ps_point = Point(5 * UP)
        curr_time = self.time
        ps_point.add_updater(lambda m: m.move_to([-5 * np.sin(0.5 * (self.time - curr_time)), 5 * np.cos(0.5 * (self.time - curr_time)), 0]))
        ...
        self.play(equations.animate.arrange(RIGHT, buff=LARGE_BUFF).to_edge(UP, buff=0), run_time=2)
        self.wait(5 * TAU)
    ...
    def make_character_dynamic(self, pi_creature, lover, cutoff_values, modes):
        ...
        def update_eyes(heart_eyes):
            ...
            l_alpha = np.clip(inverse_interpolate(cutoff_values[-1] - 0.5, cutoff_values[-1], love), 0, 1)
    ...
    def get_love_scale(self, creature, direction, var_name, color):
        ...
        number_line.add_numbers(range(-4, 6, 2), font_size=18, color=GREY_B, buff=0.1, direction=LEFT)
        dot = Dot(color=color)
...
class DiscussSystem(Scene):
    ...
    def construct(self):
        ...
        self.add(equations)
        self.play(FadeIn(sys_label, 0.5 * UP), ShowCreation(eq_rect))
        style = {'color': BLUE, 'time_width': 3, 'run_time': 2}
        self.play(LaggedStart(FlashAround(sys_label.get_part_by_text('differential'), **style), FlashAround(equations[0].get_part_by_tex('dx'), **style), FlashAround(equations[1].get_part_by_tex('dy'), **style)))
        self.wait()
        solutions = VGroup(OldTex('x(t) {{=}} (\\text{expression with } t)'), OldTex('y(t) {{=}} (\\text{expression with } t)'))
        for solution in solutions:
            solution.set_color_by_tex('expression', GREY_B)
        solutions.arrange(DOWN, buff=0.5)
        solutions.move_to(equations)
        solutions.set_x(3)
        self.play(sys_label.animate.match_width(eq_rect).to_edge(LEFT), VGroup(equations, eq_rect).animate.to_edge(LEFT), LaggedStartMap(FadeIn, solutions, shift=0.5 * UP, lag_ratio=0.3))
        self.wait()
        guess_rhss = VGroup(OldTex('\\cos(t)', color=GREY_B)[0], OldTex('\\sin(t)', color=GREY_B)[0])
        temp_rhss = VGroup()
        ...
        self.play(ShowCreation(bubble), *(TransformMatchingShapes(temp_rhs.copy(), guess_rhs) for temp_rhs, guess_rhs in zip(temp_rhss, guess_rhss)))
        self.wait()
        ...
        self.play(LaggedStartMap(FadeOut, VGroup(*bubble, *not_enough)), *(TransformMatchingShapes(guess_rhs.copy(), temp_rhs) for temp_rhs, guess_rhs in zip(temp_rhss, guess_rhss)))
        solutions.generate_target()
        initial_conditions = VGroup(OldTex('x(0) = x_0'), OldTex('y(0) = y_0'))
        ...
        self.play(MoveToTarget(solutions), LaggedStartMap(FadeIn, initial_conditions, shift=0.1 * UP, lag_ratio=0.3))
        self.wait()
        ic_label = Text('Initial condition', font_size=30)
        ic_label.set_color(BLUE)
        ic_label.next_to(initial_conditions, RIGHT, buff=1.0)
        ic_arrows = VGroup(*(Arrow(ic_label.get_left(), eq.get_right(), buff=0.1, fill_color=BLUE, thickness=0.025) for eq in initial_conditions))
...
class MoreGeneralSystem(Scene):
    ...
    def construct(self):
        kw = {'tex_to_color_map': {'x': RED, 'y': GREEN, 'z': BLUE, '{t}': GREY_B}}
        equations = VGroup(OldTex('{dx \\over d{t} } = a\\cdot x({t}) + b\\cdot y({t}) + c\\cdot z({t})', **kw), OldTex('{dy \\over d{t} } = d\\cdot x({t}) + e\\cdot y({t}) + f\\cdot z({t})', **kw), OldTex('{dz \\over d{t} } = g\\cdot x({t}) + h\\cdot y({t}) + i\\cdot z({t})', **kw))
...
class HowExampleLeadsToMatrixExponents(Scene):
    ...
    def construct(self):
        ...
        double_arrow = VGroup(Arrow(mat_exp.target, screen2), Arrow(screen2, mat_exp.target))
        for mob in double_arrow:
            mob.scale(0.9, about_point=mob.get_end())
        self.play(MoveToTarget(mat_exp), GrowFromCenter(double_arrow), arrow.animate.become(Arrow(screen, screen2)), FadeIn(screen2, DOWN))
        self.wait()
...
class RomeoJulietVectorSpace(RomeoAndJuliet):
    ...
    def construct(self):
        ...
        self.play(Rotate(psp_tracker, 90 * DEGREES, about_point=ORIGIN, run_time=3, rate_func=linear))
        axes = Axes(x_range=(-5, 5), y_range=(-5, 5), height=7, width=7, axis_config={'include_tip': False, 'numbers_to_exclude': []})
        axes.set_x(-3)
        for axis in axes:
        ...
        v_line.add_updater(lambda m: m.put_start_and_end_on(axes.x_axis.n2p(get_psp()[0]), axes.c2p(*get_psp()[:2])))
        h_line.add_updater(lambda m: m.put_start_and_end_on(axes.y_axis.n2p(get_psp()[1]), axes.c2p(*get_psp()[:2])))
        ...
        self.play(ShowCreation(h_line.copy().clear_updaters(), remover=True), ShowCreation(v_line.copy().clear_updaters(), remover=True), ReplacementTransform(romeo.scale_mob.dot, ps_dot), ReplacementTransform(juliet.scale_mob.dot, ps_dot), ChangeDecimalToValue(x_dec, get_psp()[0]), VFadeIn(x_dec), ChangeDecimalToValue(y_dec, get_psp()[1]), VFadeIn(y_dec))
        self.add(h_line, v_line, ps_dot)
        equation = VGroup(Matrix([['x'], ['y']], bracket_h_buff=SMALL_BUFF), OldTex('='), DecimalMatrix(np.reshape(get_psp()[:2], (2, 1)), element_to_mobject_config={'num_decimal_places': 2, 'font_size': 36, 'include_sign': True}))
        equation[0].match_height(equation[2])
        equation.arrange(RIGHT)
        equation.to_corner(UR)
        equation.shift(MED_SMALL_BUFF * LEFT)
        self.play(FadeIn(equation[:2]), FadeIn(equation[2].get_brackets()), TransformFromCopy(x_dec, equation[2].get_entries()[0]), TransformFromCopy(y_dec, equation[2].get_entries()[1]))
        ...
        self.play(LaggedStart(FadeIn(arrow), FadeOut(ps_dot), ShowPassingFlash(arrow_outline, run_time=1, time_width=0.5), lag_ratio=0.5))
        self.wait()
        self.play(LaggedStart(FadeIn(ps_dot), FadeOut(arrow), FlashAround(ps_dot, buff=0.05)))
        self.wait()
        ...
        self.play(FadeTransformPieces(equation[0], new_lhs))
        self.remove(equation[0])
        self.add(new_lhs)
        equation.replace_submobject(0, new_lhs)
        curr_time = self.time
        curr_psp = get_psp()
        psp_tracker.add_updater(lambda m: m.move_to(np.dot(curr_psp, np.transpose(rotation_about_z(0.25 * (self.time - curr_time))))))
        self.wait(5)
        ...
        self.play(FadeIn(deriv_lhs), Write(deriv_label, run_time=1), equation.animate.shift(2.0 * deriv_lhs.get_height() * DOWN))
        self.wait(5)
        deriv_vect = Arrow(fill_color=RED_B)
        deriv_vect.add_updater(lambda m: m.put_start_and_end_on(axes.get_origin(), axes.c2p(-0.5 * get_psp()[1], 0.5 * get_psp()[0])).shift(ps_dot.get_center() - axes.get_origin()))
        ...
        self.add(deriv_vect)
        self.play(UpdateFromAlphaFunc(moving_vect, lambda m, a: m.interpolate(pre_vect, deriv_vect, a).set_fill(opacity=a), remover=True))
        ...
        self.wait()
        for i in range(2):
            self.play(FlashAround(VGroup(deriv_lhs.get_entries()[i], rhs[1].get_entries()[i]), run_time=3, time_width=4))
        self.wait(2)
        deriv_lhs.generate_target()
        new_eq = VGroup(deriv_lhs.target, OldTex('='), IntegerMatrix([[0, -1], [1, 0]], bracket_v_buff=MED_LARGE_BUFF), Matrix([['x(t)'], ['y(t)']], bracket_h_buff=SMALL_BUFF))
        new_eq[2].match_height(new_eq[0])
        new_eq[3].match_height(new_eq[0])
        new_eq.arrange(RIGHT)
        new_eq.to_corner(UR)
        self.play(MoveToTarget(deriv_lhs), MaintainPositionRelativeTo(deriv_label, deriv_lhs), ReplacementTransform(rhs[0], new_eq[1]), ReplacementTransform(rhs[1].get_brackets(), new_eq[3].get_brackets()), FadeIn(new_eq[2], scale=2), FadeTransform(rhs[1].get_entries()[1], new_eq[3].get_entries()[0]), FadeTransform(rhs[1].get_entries()[0], new_eq[3].get_entries()[1]))
        self.wait(3)
        ...
        general_form = OldTex('{d \\over dt}', '\\vec{\\textbf{v} }', '(t)', '=', '\\textbf{M}', '\\vec{\\textbf{v} }', '(t)')
        ...
        self.play(FadeIn(general_form), FadeOut(equation))
        self.wait()
...
class From2DTo1D(Scene):
    ...
    def construct(self):
        ...
        vect_sym.save_state()
        plane = NumberPlane(x_range=(-4, 4), y_range=(-2, 2), height=4, width=8)
        plane.to_edge(DOWN)
        ...
        vector.save_state()
        vector.put_start_and_end_on(static_vect_sym.get_corner(DL), static_vect_sym.get_corner(UR))
        ...
        vector.add_updater(lambda m: m.put_start_and_end_on(plane.get_origin(), point.get_location()))
        deriv_vector = Vector(fill_color=RED, thickness=0.03)
        deriv_vector.add_updater(lambda m: m.put_start_and_end_on(plane.get_origin(), plane.c2p(*func(*plane.p2c(point.get_location())))).shift(vector.get_vector()))
        self.add(point)
        ...
        self.play(Restore(vect_sym), FadeIn(deriv))
        self.wait()
        self.play(ShowCreation(deriv_underline))
        self.play(VFadeIn(deriv_vector, rate_func=squish_rate_func(smooth, 0.8, 1.0)), UpdateFromAlphaFunc(alt_line, lambda m, a: m.put_start_and_end_on(interpolate(deriv_underline.get_start(), deriv_vector.get_start(), a), interpolate(deriv_underline.get_end(), deriv_vector.get_end(), a)), remover=True))
        self.wait(4)
        self.play(LaggedStartMap(FadeIn, equation[2:4], shift=RIGHT, lag_ratio=0.3), TransformFromCopy(equation[1], equation[4], path_arc=-45 * DEGREES, run_time=2, rate_func=squish_rate_func(smooth, 0.3, 1.0)))
        self.wait(8)
        ...
        vector_field = VectorField(func, plane, magnitude_range=(0, 1.2), opacity=0.5, vector_config={'thickness': 0.02})
        ...
        mat_exp = get_matrix_exponential([['a', 'b'], ['c', 'd']], h_buff=0.75, v_buff=0.75)
        mat_exp[1].set_color(TEAL)
        if self.show_solution:
            ...
            solution = VGroup(equation[1].copy(), OldTex('='), mat_exp, Matrix([['x(0)'], ['y(0)']], bracket_h_buff=SMALL_BUFF, bracket_v_buff=SMALL_BUFF))
            ...
            self.play(LaggedStart(Write(implies), ShowCreation(solution_rect), TransformFromCopy(equation[4], solution[0], path_arc=30 * DEGREES)))
            self.wait()
            self.play(LaggedStart(TransformFromCopy(equation[3], solution[2][1]), FadeIn(solution[2][0]), FadeIn(solution[2][2]), FadeIn(solution[1]), FadeIn(solution[3]), lag_ratio=0.1))
            self.wait(10)
            return
        else:
            ...
            self.play(MoveToTarget(equation))
            self.play(FadeIn(mat_exp[0::2]), TransformFromCopy(equation[3], mat_exp[1]), Write(arrow1, run_time=1))
            self.wait(4)
            ...
            self.play(FadeTransformPieces(mat_exp, normal_exp), FadeOut(VGroup(arrow1, arrow2)))
            self.wait(2)
        max_x = 50
        mult = 50
        ...
        nl2.set_opacity(0)
        nl.add(nl2)
        new_equation = OldTex('{d \\over dt}', 'x(t)', '=', 'r \\cdot ', 'x(t)')
        ...
        self.play(normal_exp.animate.scale(0.5).to_corner(UR), vector.animate.put_start_and_end_on(nl.n2p(0), nl.n2p(1)), deriv_vector.animate.put_start_and_end_on(nl.n2p(1), nl.n2p(1.5)), plane.animate.shift(nl.n2p(0) - plane.get_origin()).set_opacity(0), FadeIn(number_line, rate_func=squish_rate_func(smooth, 0.5, 1)), TransformMatchingShapes(equation[0], new_equation[0]), Transform(equation[1].get_entries()[0], new_equation[1]), FadeTransform(equation[2], new_equation[2]), FadeTransform(equation[3], new_equation[3]), FadeTransform(equation[4].get_entries()[0], new_equation[4]), FadeOut(equation[1].get_brackets()), FadeOut(equation[1].get_entries()[1]), FadeOut(equation[4].get_brackets()), FadeOut(equation[4].get_entries()[1]), FadeOut(deriv_underline), run_time=2)
        vt = ValueTracker(1)
        ...
        self.add(vt)
        self.wait(11)
        self.play(number_line.animate.scale(0.3, about_point=nl.n2p(0)))
        self.wait(4)
...
class SchroedingersEquationIntro(Scene):
    ...
    def construct(self):
        ...
        title.to_edge(UP)
        self.add(title)
        t2c = {'|\\psi \\rangle': BLUE, '{H}': GREY_A, '=': WHITE, 'i\\hbar': WHITE}
        original_equation = OldTex('i\\hbar \\frac{\\partial}{\\partial t} |\\psi \\rangle = {H} |\\psi \\rangle', tex_to_color_map=t2c)
        equation = OldTex('\\frac{\\partial}{\\partial t} |\\psi \\rangle = \\frac{1}{i\\hbar} {H} |\\psi \\rangle', tex_to_color_map=t2c)
        ...
        self.play(Blink(randy))
        self.play(randy.change('pondering', state_label), psis.animate.match_color(state_label), FadeIn(state_label, 0.25 * DOWN), *map(GrowArrow, state_arrows))
        self.wait()
        self.play(Blink(randy))
        self.wait()
        self.play(ReplacementTransform(original_equation[1:4], equation[0:3]), Write(equation[3]), ReplacementTransform(original_equation[0], equation[4], path_arc=90 * DEGREES), ReplacementTransform(original_equation[4:], equation[5:]), state_arrows.animate.become(VGroup(*(Arrow(state_label, psi) for psi in equation.get_parts_by_tex('\\psi')))), randy.change('hesitant', equation))
        ...
        self.play(ShowCreation(mat_rect), FadeIn(mat_label, 0.25 * UP))
        self.wait()
        self.play(randy.change('confused', equation))
        self.play(Blink(randy))
        self.wait()
        psi_words = OldTexText('Often this is a function.\\\\', '(But whatever functions are really\\\\just infinite-dimensional vectors)')
        ...
        self.play(LaggedStart(FadeOut(state_label), FadeIn(psi_words), ShowCreation(mat_line), Write(operator_word, run_time=1), GrowArrow(cv_arrow), FadeIn(complex_valued), randy.change('horrified', equation), lag_ratio=0.5))
        self.play(Blink(randy))
        self.wait()
...
class SimpleDerivativeOfExp(TeacherStudentsScene):
    ...
    def construct(self):
        eq = OldTex('{d \\over dt}', 'e^{rt}', '=', 'r', 'e^{rt}')
        ...
        self.play(s2.change('pondering', eq), FadeIn(bubble, lag_ratio=0.2))
        self.play(LaggedStart(s0.change('hesitant', eq), s1.change('erm', eq), morty.change('tease')), Write(eq[:2]))
        self.wait()
        self.play(TransformFromCopy(*eq.get_parts_by_tex('e^{rt}'), path_arc=45 * DEGREES), Write(eq.get_part_by_tex('=')), *(pi.animate.look_at(eq[4]) for pi in self.pi_creatures))
        self.play(FadeTransform(eq[4][1].copy(), eq[3][0], path_arc=90 * DEGREES))
        self.play(LaggedStart(s0.change('thinking'), s1.change('tease')))
        self.wait(2)
        rect = ScreenRectangle(height=3.5)
        rect.set_fill(BLACK, 1)
        rect.set_stroke(BLUE_B, 2)
        rect.to_corner(UR)
        self.play(morty.change('raise_right_hand', rect), self.change_students('pondering', 'pondering', 'pondering', look_at=rect), FadeIn(rect, UP))
        self.wait(10)
...
class GraphAndHistoryOfExponential(Scene):
    ...
    def construct(self):
        axes = Axes(x_range=(0, 23, 1), y_range=(0, 320, 10), height=160, width=13, axis_config={'include_tip': False})
        ...
        self.add(equation)
        curr_time = self.time
        exp_graph.add_updater(lambda m: m.pointwise_become_partial(graph_template, 0, (self.time - curr_time) / 20))
        ...
        vect.add_updater(lambda v: v.put_start_and_end_on(axes.get_origin(), axes.y_axis.get_projection(exp_graph.get_end())))
        h_line = always_redraw(lambda: DashedLine(vect.get_end(), dot.get_left(), stroke_width=1, stroke_color=GREY_B))
        v_line = always_redraw(lambda: DashedLine(axes.x_axis.get_projection(dot.get_bottom()), dot.get_bottom(), stroke_width=1, stroke_color=GREY_B))
        ...
        self.add(words)
        self.play(ApplyMethod(original_words.next_to, axes.c2p(14, 0), UP, SMALL_BUFF, run_time=2), fill_tracker.animate.set_value(1))
        self.remove(original_words, fill_tracker)
        words.clear_updaters()
        self.wait()
        solution_with_number = OldTex('{d \\over dt}', '(2.71828...)^{', 'r', 't', '}', '=', 'r', '\\cdot', '(2.71828...)^{', 'r', 't', '}', tex_to_color_map={'2.71828...': TEAL})
        solution_with_e = OldTex('{d \\over dt}', '{e}^{', 'r', 't', '}', '=', 'r', '\\cdot', '{e}^{', 'r', 't', '}', tex_to_color_map={'{e}': TEAL})
        ...
        self.play(FadeOut(words), FadeIn(dot1), FadeIn(dot2), FadeIn(line))
        self.play(dot2.x_tracker.animate.set_value(15 + 1e-06), run_time=3)
        self.play(dot1.x_tracker.animate.set_value(18), dot2.x_tracker.animate.set_value(18 + 1e-06), run_time=3)
        self.play(FadeOut(dot1), FadeOut(dot2), FadeOut(line))
        self.wait()
        self.play(Restore(lhs))
        self.play(FadeIn(solution_with_number[6]), TransformFromCopy(lhs[1:], solution_with_number[8:], path_arc=30 * DEGREES))
        self.play(TransformFromCopy(solution_with_number[12], solution_with_number[7], path_arc=-90 * DEGREES))
        self.wait()
        ...
        self.wait()
        self.play(LaggedStartMap(FadeOut, b_group, shift=0.5 * UP), LaggedStartMap(FadeIn, e_group, shift=0.5 * UP))
        ...
        self.play(LaggedStartMap(FadeOut, e_group[2:], shift=0.5 * RIGHT), FadeIn(book), FadeIn(date), e_usage.animate.scale(1.4).next_to(book, RIGHT, aligned_edge=UP).shift(0.25 * DOWN))
        self.wait()
...
class BernoullisThoughts(Scene):
    ...
    def construct(self):
        ...
        continuous_money = VGroup(*(dollar.copy().shift(smooth(x) * RIGHT * 2.0 + np.exp(smooth(x) - 1) * UP) for x in np.linspace(0, 1, 300)))
        ...
        self.play(FadeOut(dollars), Write(continuous_money, stroke_color=GREEN, stroke_width=1, run_time=3))
        ...
        limit.scale(1.5)
        self.play(FadeOut(dollars), FadeIn(limit))
        self.wait()
...
class CompoundInterestPopulationAndEpidemic(Scene):
    ...
    def construct(self):
        ...
        dollars, people, viruses = groups = [VGroup(*(mob.copy().move_to(point) for point in points)) for mob in templates]
        ...
        def update_height(group, alpha):
            for mob in group:
                mob.set_height(max(alpha * mob_height, 0.0001))
        for group in groups:
        ...
        self.add(dollars)
        self.wait(3)
        self.play(UpdateFromAlphaFunc(people, update_height), UpdateFromAlphaFunc(dollars, update_height, rate_func=lambda t: smooth(1 - t), remover=True))
        self.wait(4)
        self.play(UpdateFromAlphaFunc(viruses, update_height), UpdateFromAlphaFunc(people, update_height, rate_func=lambda t: smooth(1 - t), remover=True))
        self.wait(4)
...
class Compare1DTo2DEquations(Scene):
    ...
    def construct(self):
        ...
        solutions = VGroup(OldTex('e^{rt}', tex_to_color_map={'r': BLUE}), get_matrix_exponential([['a', 'b'], ['c', 'd']], h_buff=0.75, v_buff=0.5, bracket_h_buff=0.25))
        ...
        self.add(sol0)
        self.wait()
        self.play(Restore(sol0), TransformFromCopy(Arrow(eq1d, sol0, fill_opacity=0), arrows[0]), FadeIn(eq1d))
        self.wait(2)
        self.play(TransformMatchingShapes(sol0.copy(), sol1, fade_transform_mismatches=True))
        self.wait()
        self.play(TransformFromCopy(*arrows), LaggedStart(*(FadeTransform(m1.copy(), m2) for m1, m2 in zip([eq1d[:2], eq1d[2:5], eq1d[5], eq1d[6], eq1d[7:]], eq2d)), lag_ratio=0.05))
        self.wait()
...
class EBaseMisconception(Scene):
    ...
    def construct(self):
        ...
        randy.to_corner(DR)
        self.add(randy)
        self.play(PiCreatureSays(randy, OldTexText('This function is\\\\about about $e$', tex_to_color_map={'$e$': BLUE}), target_mode='thinking', bubble_config={'height': 3, 'width': 4}))
        self.play(Blink(randy))
        self.wait(2)
...
class OneFinalPoint(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says(OldTexText('One final point\\\\about one-dimension'), bubble_config={'height': 3, 'width': 4})
        self.play_student_changes('happy', 'hesitant', 'tease', added_anims=[self.teacher.animate.look_at(self.students[2])])
        self.wait(3)

class ManySolutionsDependingOnInitialCondition(VideoWrapper):

    def construct(self):
        axes = Axes(x_range=(0, 12), y_range=(0, 10), width=12, height=6)
        ...
        solution = OldTex('x({t}) = e^{0.3 {t} } \\cdot x_0', tex_to_color_map={'{t}': GREY_B, '0.3': BLUE, '=': WHITE})
        ...
        group.shift(2 * LEFT)
        labels = VGroup(Text('Differential equation', font_size=30), Text('Solution', font_size=30))
        ...
        alt_rhs = OldTex('&= 2^{(0.4328\\dots) t }\\\\', '&= 3^{(0.2730\\dots) t }\\\\', '&= 4^{(0.2164\\dots) t }\\\\', '&= 5^{(0.1864\\dots) t }\\\\')
        ...
        self.wait()
        self.play(alt_rhs.animate.set_opacity(0.4), FlashAround(solution[4:-1]), Write(choice_words, run_time=1))
        self.wait()
        self.play(FadeTransform(solution[:3].copy(), equation[2:5]), FadeTransform(solution[:3].copy(), equation[7:10]), FadeTransform(solution[3].copy(), equation[5]), FadeIn(equation[:2]), FadeOut(choice_words))
        self.play(FadeTransform(solution[5].copy(), equation[6]))
        self.wait()
        ...
        for label, arrow in zip(labels, arrows):
            self.play(FadeIn(label, 0.5 * RIGHT), GrowArrow(arrow))
            self.wait()
        ...
        group = VGroup(VGroup(one, sol_label.target).arrange(RIGHT, buff=MED_SMALL_BUFF, aligned_edge=DOWN), many)
        ...
        self.play(MoveToTarget(sol_label, rate_func=squish_rate_func(smooth, 0, 0.5)), FadeIn(one), FadeIn(many), ShowIncreasingSubsets(graphs), dot.animate.move_to(axes.c2p(0, 10)), run_time=4)
        self.wait()
        ...
        x0_label = VGroup(OldTex('x_0 = ', font_size=36), DecimalNumber(1, font_size=36))
        ...
        self.play(Write(ic_label), run_time=1)
        self.play(dot.animate.move_to(axes.c2p(0, 1)), graphs.animate.set_stroke(opacity=0.5), run_time=2)
        self.wait()
        ...
        self.play(FadeOut(rect), FadeOut(ic_label), self.camera.frame.animate.shift(0.6 * LEFT))
        self.wait()
        self.play(x0_tracker.animate.set_value(0.1))
        self.wait()
        for x in [5, 9]:
            self.play(x0_tracker.animate.set_value(x), run_time=5)
            self.wait()
            self.play(x0_tracker.animate.set_value(1), run_time=3)
        self.wait()
        new_sol_label = Text('General solution', font_size=36)
        new_sol_label.move_to(sol_label, LEFT)
        self.play(TransformFromCopy(x0_label[0][0][:2], solution[-1]), sol_arrow.animate.become(Arrow(sol_label, solution)))
        self.play(FadeOut(sol_label), FadeIn(new_sol_label))
        self.wait()
        rhs = solution[4:].copy()
        ...
        self.play(MoveToTarget(rhs), MaintainPositionRelativeTo(rhs_rect, rhs), FadeOut(equation[index:]), FadeOut(eq_arrow), FadeOut(eq_label))
        ...
        self.wait()
        self.play(FadeOut(rhs), FadeIn(equation[index:]), FadeIn(eq_arrow), FadeIn(eq_label))
        self.wait()
        ...
        words2 = Text("It's something which\n\nacts on an initial condition\n\nto give a solution", t2s={'acts': ITALIC}, t2c={'acts': YELLOW, 'initial condition': MAROON_B}, font_size=30)
        ...
        self.wait(2)
        self.play(FadeOut(words1), FadeIn(words2))
        self.wait(2)

class ExoticExponentsWithEBase(Scene):

    def construct(self):
        exps = VGroup(OldTex('e', '^{it}'), get_matrix_exponential([[3, 1], [4, 1]]), OldTex('e', '^{((i + j + k) / \\sqrt{3})t}'), OldTex('e', '^{\\left(\\frac{\\partial}{\\partial x}\\right)t}'))
        for i in (0, 2, 3):
            exps[i][0].scale(1.25)
        ...
        labels = VGroup(Text('Complex numbers', font_size=30), Text('Matrices', font_size=30), Text('Quaternions', font_size=30), Text('Operators', font_size=30))
        ...
        for label, exp in zip(labels, exps):
            e_mobs.add(exp[0])
            self.play(FadeIn(label), FadeIn(exp[0]), GrowFromPoint(exp[1:], exp[0].get_center()), exotic_label.animate.set_opacity(1), arrow.animate.set_opacity(1))
        self.wait()
        self.play(LaggedStart(*(FlashAround(e, buff=0.05, time_width=3, run_time=2) for e in e_mobs), lag_ratio=0.1))
        self.wait()
        zeta = OldTex('\\sum_{n = 1}^\\infty \\frac{1}{n^s}')
        zeta.move_to(exps[0], LEFT)
        self.play(FadeOut(exps[0]), FadeIn(zeta))
        self.wait()
        self.play(FadeOut(zeta), FadeIn(exps[0]))
        self.wait()
        n_mob_groups = VGroup()
        ...
        for group in [*n_mob_groups, e_mobs]:
            self.play(LaggedStartMap(FadeOut, last_group, lag_ratio=0.2), LaggedStartMap(FadeIn, group, lag_ratio=0.2), run_time=1.5)
            self.wait()
            last_group = group
        equations = VGroup(OldTex('\\frac{dz}{dt}(t) = i \\cdot z(t)'), get_2d_equation([['3', '1'], ['4', '1']]), OldTex('\\frac{dq}{dt}(t) = \\frac{i + j + k}{ \\sqrt{3} } \\cdot q(t)'), OldTex('{\\partial \\over \\partial t}f(x, t) = {\\partial \\over \\partial x}f(x, t)'))
        for eq, exp in zip(equations, exps):
            eq.match_height(exp)
            eq.move_to(exp, DL)
            eq.align_to(equations[0], LEFT)
        equations.to_edge(RIGHT)
        self.play(FadeOut(arrow, 3 * LEFT), FadeOut(exotic_label, 3 * LEFT), VGroup(labels, exps).animate.to_edge(LEFT), LaggedStartMap(FadeIn, equations, run_time=3, lag_ratio=0.5))
        self.wait()
        self.play(VGroup(labels[0], labels[2:], exps[0], exps[2:], equations[0], equations[2:]).animate.set_opacity(0.3), equations[1].animate.scale(1.5, about_edge=RIGHT), exps[1].animate.scale(1.5), labels[1].animate.scale(1.5, about_edge=LEFT))
        self.wait()
...
class TryToDefineExp(TeacherStudentsScene):
    ...
    def construct(self):
        words = OldTexText('Try to define {{$e^{M}$}} to\\\\make sure this is true!')
        words[1][1].set_color(TEAL)
        self.play(PiCreatureSays(self.teacher, words, target_mode='surprised'), self.change_students('pondering', 'thinking', 'pondering'))
...
class SolutionToRomeoJuliet(Scene):
    ...
    def construct(self):
        mat_exp = get_matrix_exponential([['0', '-1'], ['1', '0']], h_buff=1.0)
        solution = VGroup(Matrix([['x(t)'], ['y(t)']], bracket_h_buff=SMALL_BUFF, bracket_v_buff=SMALL_BUFF), OldTex('='), mat_exp, Matrix([['x(0)'], ['y(0)']], bracket_h_buff=SMALL_BUFF, bracket_v_buff=SMALL_BUFF))
        solution[2].match_height(solution[0])
        solution[3].match_height(solution[0])
...
class ExpRotMatrixComputation(Scene):
    ...
    def construct(self):
        ...
        mat_exp[1].set_color(TEAL)
        equation = OldTex('e^X', ':=', 'X^0 + X^1 + \\frac{1}{2} X^2 + \\frac{1}{6} X^3 + \\cdots + \\frac{1}{n!} X^n + \\cdots', isolate=['X', '+'])
        ...
        mat_rhs = OldTex(rhs_tex.replace('X', mat_tex), tex_to_color_map={mat_tex: TEAL}, isolate=['{t}', '+'])
        mat_rhs.scale(0.5)
        mat_equals = OldTex('=')
        ...
        self.wait()
        self.play(LaggedStart(FadeTransform(equation[:2].copy(), mat_equation[0]), FadeTransform(equation[2].copy(), mat_equation[1]), *(FadeTransform(equation[i].copy(), mat_equation[2][j]) for i, j in zip([3, 4, 3, 4, 5, 6, 7, 6, 7, 8, 9, 10, 11, 10, 11, 12, 13, 14, 15, 14, 15, 16, 17, 18, 19, 20, 21, 20, 21, 22, 23], it.count())), run_time=2, lag_ratio=0.02))
        self.wait()
        ...
        self.play(LaggedStart(TransformFromCopy(mat_equation[0], new_eq[0]), TransformFromCopy(mat_equation[1], new_eq[1]), *(TransformFromCopy(mat_equation[2][i], new_eq[2][i]) for i in range(21) if i not in indices and i - 1 not in indices), lag_ratio=0.01))
        ...
        self.wait()
        lhs = mat_equation[:2]
        self.play(lhs.animate.scale(2).to_corner(UL).shift(DOWN), FadeOut(VGroup(mat_equation[2], rect, equation)))
        rhs = Matrix([['1 - \\frac{t^2}{2!} + \\frac{t^4}{4!} - \\frac{t^6}{6!} + \\cdots', '-t + \\frac{t^3}{3!} - \\frac{t^5}{5!} + \\frac{t^7}{7!} - \\cdots'], ['t - \\frac{t^3}{3!} + \\frac{t^5}{5!} - \\frac{t^7}{7!} + \\cdots', '1 - \\frac{t^2}{2!} + \\frac{t^4}{4!} - \\frac{t^6}{6!} + \\cdots']], h_buff=6, v_buff=2)
        rhs.set_width(10)
        ...
        for entry, per in zip(rhs.get_entries(), power_entry_rects):
            ...
            self.play(ReplacementTransform(last_per, per), FadeIn(entry), FadeIn(rect), FadeOut(last_rect))
            self.wait()
            last_per = per
            last_rect = rect
        ...
        self.play(*anims, FadeOut(low_terms, shift=2 * DOWN), ReplacementTransform(new_eq[:2], new_lhs), TransformFromCopy(rhs.get_brackets(), final_result.get_brackets()))
        self.wait()
        last_rects = VGroup()
        for entry1, entry2 in zip(rhs.get_entries(), final_result.get_entries()):
            ...
            self.play(FadeIn(rect1), FadeIn(rect2), FadeIn(entry2), FadeOut(last_rects))
        ...
        self.play(LaggedStart(FadeOut(high_eq, UP), FadeOut(VGroup(question, arrow)), low_eq.animate.center().to_edge(UP), lag_ratio=0.05))
        self.wait()

class ThatsHorrifying(TeacherStudentsScene):

    def construct(self):
        self.student_says(OldTexText('You want us to\\\\do what?'), index=2, target_mode='pleading', added_anims=[LaggedStart(self.students[0].change('tired'), self.students[1].change('horrified'), self.teacher.change('guilty'), lag_ratio=0.5)])
        ...
        self.play_student_changes('pondering', 'thinking', 'hesitant', look_at=self.screen)
        self.wait(3)
...
class HowBasisVectorMultiplicationPullsOutColumns(Scene):
    ...
    def construct(self):
        ...
        matrix = Matrix([['a', 'b'], ['c', 'd']], h_buff=0.8)
        matrix.to_corner(UL)
        matrix.to_edge(LEFT, buff=MED_SMALL_BUFF)
        matrix.add_to_back(BackgroundRectangle(matrix))
        self.add(matrix)
        basis_vectors = VGroup(Arrow(plane.get_origin(), plane.c2p(1, 0), buff=0, fill_color=GREEN), Arrow(plane.get_origin(), plane.c2p(0, 1), buff=0, fill_color=RED))
        bhb = 0.2
        basis_labels = VGroup(Matrix([['1'], ['0']], bracket_h_buff=bhb), Matrix([['0'], ['1']], bracket_h_buff=bhb))
        ...
        rhss = VGroup(Matrix([['a'], ['c']], bracket_h_buff=bhb), Matrix([['b'], ['d']], bracket_h_buff=bhb))
        colors = [GREEN, RED]
        ...
        def show_basis_product(index, matrix):
            ...
            self.play(FadeIn(basis_labels[index], RIGHT), GrowArrow(basis_vectors[index]), FadeIn(basis_label_copies[index]), FadeIn(equals), FadeIn(rhs_br), FadeIn(rhss[index].get_brackets()))
            ...
            for e1, e2, row_rect in zip(matrix.get_columns()[index], rhss[index].get_entries(), row_rects):
                self.play(col_rect.animate.set_stroke(opacity=1), FadeIn(row_rect), FadeOut(last_row_rect), e1.animate.set_color(colors[index]), FadeIn(e2))
...
class ReadColumnsOfRotationMatrix(Scene):
    ...
    def construct(self):
        ...
        angle = 50 * DEGREES
        plane2 = plane.copy().apply_matrix([[math.cos(angle), 0], [math.sin(angle), 1]], about_point=plane.get_origin())
        plane3 = plane.copy().apply_matrix([[math.cos(angle), -math.sin(angle)], [math.sin(angle), math.cos(angle)]], about_point=plane.get_origin())
        coords = plane.deepcopy().add_coordinate_labels((-2, -1, 1, 2), (-1, 1))
        self.add(b_plane, plane, coords)
        equation = VGroup(get_matrix_exponential([[0, -1], [1, 0]]), OldTex('='), Matrix([['\\cos(t)', '-\\sin(t)'], ['\\sin(t)', '\\cos(t)']], h_buff=2.0))
        ...
        rbl0, rbl1 = rot_basis_labels = VGroup(Matrix([['\\cos(t)'], ['\\sin(t)']]), Matrix([['-\\sin(t)'], ['\\cos(t)']]))
        ...
        arcs = VGroup(Arc(0, angle, arc_center=plane.get_origin(), radius=0.5), Arc(PI / 2, angle, arc_center=plane.get_origin(), radius=0.5))
        ...
        self.play(TransformFromCopy(matrix[0], rbl0[0]), TransformFromCopy(matrix.get_brackets(), rbl0.get_brackets()), TransformFromCopy(matrix.get_columns()[0], rbl0.get_entries()), Transform(plane, plane2, path_arc=angle), Transform(basis_vectors[0], rot_b0, path_arc=angle), Animation(matrix.get_columns()[1]), run_time=2)
        rects = VGroup(*(SurroundingRectangle(entry, color=WHITE, stroke_width=2, buff=0.05) for entry in rbl0.get_entries()))
        self.play(ShowCreation(h_line), FadeIn(rects[0]), FadeIn(cos_label))
        self.play(ShowCreation(v_line), FadeOut(rects[0]), FadeIn(rects[1]), FadeIn(sin_label))
        self.play(FadeOut(rects[1]))
        self.wait()
        self.play(FlashAround(matrix.get_columns()[1], color=RED))
        self.play(TransformFromCopy(matrix[0], rbl1[0]), TransformFromCopy(matrix.get_brackets(), rbl1.get_brackets()), TransformFromCopy(matrix.get_columns()[1], rbl1.get_entries()), Transform(plane, plane3, path_arc=angle), Transform(basis_vectors[1], rot_b1, path_arc=angle), Animation(matrix.get_columns()[0]), run_time=2)
        self.wait()
        self.play(FadeIn(arc_label), *map(ShowCreation, arcs))
        self.wait()
...
class AnalyzeRomeoAndJulietSpace(RomeoJulietVectorSpace):
    ...
    def construct(self):
        ...
        rot_matrix = Matrix([['\\cos(t)', '-\\sin(t)'], ['\\sin(t)', '\\cos(t)']], bracket_h_buff=0.2, h_buff=2.0)
        ...
        self.wait()
        self.play(Write(implies), TransformFromCopy(equation[1], solution[0], path_arc=-20 * DEGREES), TransformFromCopy(equation[2], solution[1]))
        self.play(Write(mat_exp[0]), TransformFromCopy(equation[3], mat_exp[1]))
        self.play(Write(mat_exp[2]))
        self.wait()
        self.play(FadeIn(ic_source), FlashAround(ic_source))
        self.play(TransformFromCopy(ic_source, initial_condition))
        self.wait()
        self.play(*(TransformFromCopy(solution[i], solution2[i]) for i in [0, 1, 3]), FadeTransform(mat_exp.copy(), rot_matrix))
        self.play(GrowFromCenter(rot_matrix_brace), FadeIn(rot_label))
        self.wait()
        ...
        def get_arc():
            curr_time = get_t()
            arc = ParametricCurve(lambda s: axes.c2p(*tuple((1 + 0.2 * s) * np.array([math.cos(start_angle + s * curr_time), math.sin(start_angle + s * curr_time)]))), t_range=(0, 1, 0.01))
    ...
    def get_romeo_juliet_axes(self, height=5):
        romeo, juliet = lovers = self.get_romeo_and_juliet()
        axes = Axes(x_range=(-5, 5), y_range=(-5, 5), height=height, width=height, axis_config={'include_tip': False, 'numbers_to_exclude': []})
        axes.to_edge(LEFT)
        for axis in axes:
    ...
    def get_ps_arrow(self, axes, color=BLUE, add_shadow=False):
        ...
        arrow.add_updater(lambda m: m.put_start_and_end_on(axes.get_origin(), axes.ps_dot.get_center()))
        if add_shadow:
...
class GeometricReasoningForRomeoJuliet(AnalyzeRomeoAndJulietSpace):
    ...
    def construct(self):
        ...
        self.wait()
        curve = VMobject()
        curve.set_points_smoothly([axes.c2p(x, y) for x, y in [(4, 3), (0, 2), (0, -1), (-3, -4), (-3, 2), (3, 1)]], true_smooth=True)
        ...
        arc = Arc(start_angle=ps_arrow.get_angle(), angle=90 * DEGREES, arc_center=ps_arrow.get_end(), radius=0.25)
        elbow = VMobject()
        ...
        self.play(TransformFromCopy(ps_arrow, deriv_vect, path_arc=30 * DEGREES), ShowCreation(deriv_rect))
        self.add(d_line, deriv_vect)
        self.play(ShowCreation(arc), Rotate(deriv_vect, 90 * DEGREES, about_point=ps_arrow.get_end()), run_time=2, rate_func=rush_into)
        self.remove(arc)
        self.add(elbow)
        self.wait()
        ...
        self.play(Rotate(rot_group, angle=TAU - ps_arrow.get_angle(), about_point=axes.get_origin(), run_time=6, rate_func=linear), FadeIn(circle))
        self.wait()
        self.play(deriv_vect.animate.put_start_and_end_on(ps_arrow.get_start(), ps_arrow.get_end()).shift(0.2 * UP), run_time=3, rate_func=there_and_back_with_pause)
        self.wait()
        arc = Arc(0, 1, radius=circle.radius, arc_center=axes.get_origin())
        arc.set_stroke(YELLOW, 3)
        sector = Sector(start_angle=0, angle=1, outer_radius=circle.radius)
        ...
        self.play(Rotate(rot_group, 1, about_point=axes.get_origin()), ShowCreation(arc), ChangeDecimalToValue(t_label[1], 1.0), VFadeIn(t_label), run_time=2, rate_func=linear)
        ...
        self.wait()
        self.play(t_label.animate.shift(UP).scale(2))
        t_label[1].data['font_size'] *= 2
        for x in range(5):
            VGroup(sector, arc).rotate(1, about_point=axes.get_origin())
            self.play(Rotate(rot_group, 1, about_point=axes.get_origin()), ChangeDecimalToValue(t_label[1], t_label[1].get_value() + 1), run_time=2, rate_func=linear)
        self.play(FadeOut(VGroup(sector, arc)), Rotate(rot_group, TAU - 6, about_point=axes.get_origin()), ChangeDecimalToValue(t_label[1], TAU), run_time=2 * (TAU - 6), rate_func=linear)
        self.wait(2)
        t_label[1].set_value(0)
        self.play(Rotate(rot_group, PI, about_point=axes.get_origin()), ChangeDecimalToValue(t_label[1], PI), run_time=2 * PI, rate_func=linear)
        self.wait(2)
...
class Show90DegreeRotation(Scene):
    ...
    def construct(self):
        ...
        vects = VGroup(Arrow(plane.get_origin(), plane.c2p(1, 0), fill_color=GREEN, buff=0, thickness=0.075), Arrow(plane.get_origin(), plane.c2p(0, 1), fill_color=RED, buff=0, thickness=0.075))
        plane.add(*vects)
...
class Show90DegreeRotationColumnByColumn(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Transform(plane, plane2, path_arc=90 * DEGREES), Rotate(vects[0], 90 * DEGREES, about_point=plane.get_origin()), run_time=2)
        self.wait()
        self.play(Transform(plane, plane3, path_arc=90 * DEGREES), Rotate(vects[1], 90 * DEGREES, about_point=plane.get_origin()), run_time=2)
        self.wait()
...
class DistanceOverTimeEquation(Scene):
    ...
    def construct(self):
        equation = OldTex('{\\text{Distance}', ' \\over', ' \\text{Time} }', '=', '\\text{Radius}')
...
class ExplicitSolution(Scene):
    ...
    def construct(self):
        kw = {'tex_to_color_map': {'x_0': BLUE_D, 'y_0': BLUE_B, '{t}': GREY_B}}
        solutions = VGroup(OldTex('x({t}) = \\cos(t) x_0 - \\sin(t) y_0', **kw), OldTex('y({t}) = \\sin(t) x_0 + \\cos(t) y_0', **kw))
...
class ComplexEquation(Scene):
    ...
    def construct(self):
        equation = OldTex('\n            \\frac{d}{d{t} } \\Big[ x(t) + {i}y(t)\\Big] =\n            {i} \\cdot \\Big[ x(t) + {i}y(t) \\Big]\n            ', tex_to_color_map={'x': BLUE_B, 'y': BLUE_D, '{t}': GREY_B, '=': WHITE, '\\cdot': WHITE, '{i}': RED})
        ...
        self.add(equation)
        self.play(*(GrowFromCenter(b) for b in braces), *(FadeIn(b.zt, 0.25 * UP) for b in braces))
        self.wait()
        ...
        self.play(GrowArrow(juliet_arrow), FadeIn(juliet_label))
        self.play(GrowArrow(romeo_arrow), FadeIn(romeo_label))
        self.wait()
        ...
        self.play(GrowArrow(i_arrow), FadeIn(i_label, DR))
        self.wait()
        all_labels = VGroup(romeo_label, juliet_label, i_label)
        solution = OldTex('z(t) = e^{it} z_0', tex_to_color_map={'i': RED, 't': GREY_B, 'z': BLUE, '=': WHITE})
        solution.scale(1.5)
        solution.next_to(all_labels, DOWN, LARGE_BUFF)
        solution[8:].set_color(BLUE_D)
        self.play(TransformFromCopy(braces[0].zt, solution[:4]), Write(solution[4]))
        self.play(Write(solution[5]), TransformFromCopy(equation[9], solution[6]), Write(solution[7]))
...
class JulietChidingRomeo(Scene):
    ...
    def construct(self):
        ...
        self.add(pis)
        self.play(PiCreatureSays(juliet, OldTexText("It just seems like \\\\ your feelings aren't \\\\ real"), bubble_config={'height': 2.5, 'width': 3.5}, target_mode='sassy'), romeo.change('guilty'))
        for x in range(2):
...
class General90DegreeRotationExponents(Scene):
    ...
    def construct(self):
        exps = VGroup(get_matrix_exponential([['0', '-1'], ['1', '0']]), OldTex('e^{it}', tex_to_color_map={'i': RED}), OldTex('e^{(ai + bj + ck)t}', tex_to_color_map={'i': RED, 'j': GREEN, 'k': BLUE}), OldTex('e^{i \\sigma_x t}, \\quad ', 'e^{i \\sigma_y t}, \\quad ', 'e^{i \\sigma_z t}', tex_to_color_map={'\\sigma_x': RED, '\\sigma_y': GREEN, '\\sigma_z': BLUE}))
        exps[0][1].set_color(TEAL)
        exps[0].scale(0.5)
        exps.arrange(DOWN, buff=1.5, aligned_edge=LEFT)
        labels = VGroup(OldTexText('$90^\\circ$ rotation matrix'), OldTexText('Imaginary numbers'), OldTexText('Quaternions'), OldTexText('Pauli matrices'))
        for label, exp in zip(labels, exps):
            label.scale(0.75)
        ...
        for exp, label in zip(exps, labels):
            self.play(FadeIn(exp), FadeIn(label))
        self.wait()
...
class RotationIn3dPlane(Scene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(plane), FadeIn(randy))
        self.play(GrowArrow(p_vect))
        self.wait(2)
        self.play(Rotate(v_vect, PI / 2, axis=normal, about_point=axes.get_origin()), Rotate(randy, PI / 2, axis=normal, about_point=axes.get_origin()), UpdateFromAlphaFunc(v_vect, lambda m, a: m.set_fill(interpolate_color(BLUE, RED, a))), ShowCreation(arc), run_time=2)
        self.wait(3)
        self.play(v_vect.animate.shift(p_vect.get_end() - v_vect.get_start()), FadeOut(arc), FadeOut(randy))
        rot_group = VGroup(grid, p_vect, v_vect)
        rot_group.set_stroke(background=True)
        self.add(circle, rot_group)
        self.play(FadeIn(circle), Rotate(rot_group, 2 * TAU, axis=normal, about_point=axes.get_origin(), run_time=4 * TAU, rate_func=linear), VFadeIn(grid))

class StoryForAnotherTime(TeacherStudentsScene):

    def construct(self):
        self.teacher_says(OldTexText('The full story\\\\takes more time.'), bubble_config={'height': 3, 'width': 3.5}, added_anims=[self.change_students('confused', 'erm', 'hesitant', look_at=self.screen)])
        self.wait(5)

class SchrodingerSum(Scene):

    def construct(self):
        equation_label = VGroup(OldTexText('Schrödinger equation:'), OldTex('{i} \\hbar \\frac{\\partial}{\\partial t} |\\psi(t)\\rangle = \\hat{H} |\\psi(t)\\rangle', tex_to_color_map={'|\\psi(t)\\rangle': BLUE, '{i}': WHITE, '\\frac{\\partial}{\\partial t}': GREY_A}, font_size=36))
        ...
        axes = ThreeDAxes(x_range=x_range, y_range=y_range, z_range=y_range, height=1.25, depth=1.25, width=4, axis_config={'include_tip': False, 'tick_size': 0.05})
        ...
        def get_graph(axes, func, color):
            ...
            result = VGroup()
            graph = always_redraw(lambda: ParametricCurve(lambda x: axes.c2p(*to_xyz(func, x)), t_range=x_range[:2], color=color, stroke_opacity=fade_tracker.get_value(), flat_stroke=False))
            result.add(graph)
            for x in np.linspace(0, 1, 100):
                ...
                line.add_updater(lambda m: m.set_points_as_corners([m.axes.x_axis.pfp(m.x), m.graph.pfp(m.x)]).set_opacity(0.5 * m.fade_tracker.get_value()))
                result.add(line)
            result.fade_tracker = fade_tracker
            return result
        graph0, graph1, graph2, comb_graph = graphs = [get_graph(axes, func, color) for axes, func, color in zip([*lil_axes, axes], [func0, func1, func2, comb_func], [BLUE, TEAL, GREEN, YELLOW])]
        ...
        self.wait(10)
        self.add(*graphs)
        self.play(FadeIn(lil_axes[::2]), FadeIn(axes), Restore(lil_axes[1]), GrowFromCenter(brace), *(UpdateFromAlphaFunc(g.fade_tracker, lambda m, a: m.set_value(a)) for g in (*graphs[::2], comb_graph)))
        self.wait(15)
...
class BasicVectorFieldIdea(Scene):
    ...
    def construct(self):
        v_tex = '\\vec{\\textbf{v} }(t)'
        equation = OldTex('{d \\over dt}', v_tex, '=', 'M', v_tex, tex_to_color_map={'M': GREY_A, v_tex: YELLOW})
        equation.set_height(1.5)
        ...
        self.play(ShowCreation(vel_rect), FadeIn(vel_words, 0.5 * DOWN))
        self.wait()
        self.play(ShowCreation(pos_rect), FadeIn(pos_words, 0.5 * DOWN))
        self.wait()
        ...
        self.play(GrowFromPoint(dot, equation[-1].get_center()), FadeTransform(equation[-1][0].copy(), dot_label))
        self.wait()
        self.play(FadeIn(vector), ReplacementTransform(vector.copy().set_opacity(0), Mv, path_arc=-45 * DEGREES), FadeTransform(equation[3:5].copy(), Mv_label))
        self.wait()
        ...
        for x in range(100):
            ...
            if x < 20:
                ...
                self.wait(0.1)
        self.play(LaggedStartMap(FadeOut, VGroup(Mv_label, dot_label, Mv, vector, dot)), vector_field.animate.set_opacity(1))
        self.wait()

        def get_flow_lines(step_multiple, arc_len):
            return StreamLines(func, plane, step_multiple=step_multiple, magnitude_range=(0, 2), color_by_magnitude=False, stroke_color=GREY_A, stroke_width=2, stroke_opacity=1, arc_len=arc_len)
        dot = Dot()
        vect = Vector()
        vect.set_color(RED)
        ...
        self.play(MoveAlongPath(dot, flow_line, run_time=10, rate_func=linear), ShowCreation(flow_line, run_time=10, rate_func=linear), VFadeIn(dot), VFadeIn(vect))
...
class MoreShakesperianRomeoJuliet(RomeoAndJuliet):
    ...
    def construct(self):
        ...
        def func2(x, y):
            return (y, x)
        vector_fields = VGroup(*(VectorField(func, plane, step_multiple=1, magnitude_range=(0, 8), vector_config={'thickness': 0.025}, length_func=lambda norm: 0.9 * sigmoid(norm)) for func in [func0, func1, func2]))
        equations = VGroup(get_2d_equation([['0', '-1'], ['+1', '0']]), get_2d_equation([['0', '+1'], ['+1', '0']]))
        ...
        self.make_romeo_and_juliet_dynamic(romeo, juliet)
        scales = VGroup(self.get_love_scale(romeo, RIGHT, 'y', BLUE_D), self.get_love_scale(juliet, LEFT, 'x', BLUE_B))
        x0, y0 = (5, 5)
        ...
        self.add(ps_point)
        self.wait()
        self.play(Transform(vf, vector_fields[2]), FadeOut(m1, UP), FadeIn(m2, UP))
        self.wait()
        last_rect = VMobject()
        ...
        def get_flow_line(x0, y0):
            line = ParametricCurve(lambda t: plane.c2p(math.cosh(t) * x0 + math.sinh(t) * y0, math.sinh(t) * x0 + math.cosh(t) * y0), t_range=(0, 4))
            line.set_stroke(WHITE, 3)
            return line

        def move_along_line(line, run_time=8):
            self.add(line, ps_point)
            self.play(MoveAlongPath(ps_point, line.copy()), ShowCreation(line), rate_func=linear, run_time=run_time)
...
class TransitionWrapper(Scene):
    ...
    def construct(self):
        ...
        titles = VGroup(OldTexText('Time: $0$'), OldTexText('Time: $t$'))
...
class DerivativeOfExpMt(Scene):
    ...
    def construct(self):
        ...
        kw = {'tex_to_color_map': {'M': GREY_B, '{t}': YELLOW, v0_tex: BLUE, '=': WHITE, '\\cdots': WHITE, '+': WHITE, '\\left(': WHITE, '\\right)': WHITE}}
        ...
        lines[1].set_y(-1)
        self.play(TransformMatchingTex(VGroup(*(sm for sm in lines[0][5:] if sm.get_tex() not in ['\\left(', '\\right)'])).copy(), lines[1][5:]), TransformFromCopy(lines[0][:5], lines[1][:5]))
        ...
        l1_indices, l2_indices, l3_indices = [[lines[i].index_of_part(part) for part in it.chain(lines[i].get_parts_by_tex('='), lines[i].get_parts_by_tex('+'))] for i in (1, 2, 3)]
        last_rects = VMobject()
        for l1i, l1j, l2i, l2j in zip(l1_indices, l1_indices[1:], l2_indices, l2_indices[1:]):
            ...
            last_rects = rects
        self.play(FadeOut(last_rects))
        self.play(lines[2].animate.set_y(1), FadeOut(lines[1]), FadeIn(lines[3]))
        self.wait()
        last_rects = VMobject()
        ...
        self.wait()
        self.play(LaggedStart(*(FlashUnder(sm, time_width=2, run_time=1) for sm in lines[3].get_parts_by_tex('M')[1:]), lag_ratio=0.2))
        self.wait()
        self.play(TransformMatchingTex(lines[3][:5], lines[4][:5]), FadeTransform(lines[3][5:], lines[4][7:34]), FadeIn(lines[4].get_part_by_tex('\\left(')), FadeIn(lines[4].get_part_by_tex('\\right)')), TransformFromCopy(lines[3].get_parts_by_tex('M')[1:], VGroup(lines[4][5]), path_arc=-45 * DEGREES))
        self.wait()
        match_lines(lines[5], lines[4])
        lines[5].set_y(-3)
        self.play(TransformFromCopy(lines[4][:7], lines[5][:7]), TransformFromCopy(lines[4][34], lines[5][11]), FadeTransform(lines[4][7:34].copy(), lines[5][7:11], stretch=False))
        self.wait()
...
class TracePropertyAndComputation(TeacherStudentsScene):
    ...
    def construct(self):
        trace_eq = OldTex('\\text{Det}\\left(e^{Mt}\\right) = e^{\\text{Tr}(M) t}', tex_to_color_map={'\\text{Det}': GREEN_D, '\\text{Tr}': RED_B})
        trace_eq.move_to(self.hold_up_spot, DOWN)
        self.play(self.teacher.change('raise_right_hand', trace_eq), FadeIn(trace_eq, 0.5 * UP))
        ...
        self.play(trace_eq.animate.shift(UP), FadeIn(text, 0.5 * UP), self.change_students('erm', 'tease', 'maybe'))
        for pi in self.pi_creatures:
        ...
        self.play(FadeIn(exp_deriv, scale=2), topics.animate.scale(0.5).to_corner(UL), self.teacher.change('tease', exp_deriv))
...
class ExponentialPhaseFlow(Scene):
    CONFIG = {'field_config': {'color_by_magnitude': False, 'magnitude_range': (0.5, 5), 'arc_len': 5}, 'plane_config': {'x_range': [-4, 4], 'y_range': [-2, 2], 'height': 8, 'width': 16}, 'matrix': [[1, 0], [0, 1]], 'label_height': 3, 'run_time': 30, 'slow_factor': 0.25}
    ...
    def construct(self):
        ...
        plane.add_coordinate_labels()
        vector_field, animated_lines = get_vector_field_and_stream_lines(self.func, plane, **self.field_config)
        box = Square()
        ...
        basis_vectors = VGroup(Vector(RIGHT, fill_color=GREEN), Vector(UP, fill_color=RED))
    ...
    def get_label(self):
        ...
        equals = OldTex('=')
        rhs = DecimalMatrix(np.zeros((2, 2)), element_to_mobject_config={'num_decimal_places': 3}, h_buff=1.8)
        rhs.match_height(exponential)
        equation = VGroup(exponential, equals, rhs)
        equation.arrange(RIGHT)
        equation.to_corner(UL)
...
class ExponentialEvaluationWithTime(Scene):
    ...
    def construct(self):
        flow_scene_attrs = merge_dicts_recursively(ExponentialPhaseFlow.CONFIG, self.flow_scene_class.CONFIG)
        ...
        equals = OldTex('=')
        rhs = DecimalMatrix(np.zeros((2, 2)), element_to_mobject_config={'num_decimal_places': 2}, element_alignment_corner=ORIGIN, h_buff=2.0)
...
class CircularPhaseFlow(ExponentialPhaseFlow):
    CONFIG = {'field_config': {'magnitude_range': (0.5, 8)}, 'matrix': [[0, -1], [1, 0]]}
...
class EllipticalPhaseFlow(ExponentialPhaseFlow):
    CONFIG = {'field_config': {'magnitude_range': (0.5, 8)}, 'matrix': [[0.5, -3], [1, -0.5]]}
...
class HyperbolicPhaseFlow(ExponentialPhaseFlow):
    CONFIG = {'field_config': {'sample_freq': 8}, 'matrix': [[1, 0], [0, -1]]}
...
class ShearPhaseFlow(ExponentialPhaseFlow):
    CONFIG = {'field_config': {'sample_freq': 2, 'magnitude_range': (0.5, 8)}, 'plane_config': {'x_range': [-8, 8], 'y_range': [-4, 4]}, 'matrix': [[1, 1], [0, 1]], 'slow_factor': 0.1}
...
class HyperbolicTrigFlow(ExponentialPhaseFlow):
    CONFIG = {'field_config': {'sample_freq': 2, 'magnitude_range': (0.5, 7)}, 'plane_config': {'x_range': [-8, 8], 'y_range': [-4, 4]}, 'matrix': [[0, 1], [1, 0]], 'slow_factor': 0.1}
...
class LetsSumUp(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says("Let's review", added_anims=[self.change_students('thinking', 'pondering', 'thinking')])
        self.wait(3)
...
class PrerequisitesWrapper(Scene):
    ...
    def construct(self):
        ...
        topics = VGroup(OldTexText('Basics of $e^x$'), OldTexText('How matrices act\\\\as transformations'))
        ...
        for topic, screen in zip(topics, screens):
            ...
            self.play(FadeIn(topic, 0.5 * UP), FadeIn(screen), VShowPassingFlash(sc, time_width=1.0, run_time=1.5))
            self.wait(2)
...
class PreviewVisualizationWrapper(Scene):
    ...
    def construct(self):
        ...
        screen.to_edge(DOWN)
        self.add(screen)
        titles = VGroup(Text('How to think about matrix exponentiation'), Text('How to visualize matrix exponentiation', t2s={'visualize': ITALIC}), Text('What problems matrix exponentiation solves?'))
        for title in titles:
            title.next_to(screen, UP)
            title.get_parts_by_text('matrix exponentiation').set_color(TEAL)
        self.play(FadeIn(titles[0], 0.5 * UP))
        self.wait(2)
        self.play(*(FadeTransform(titles[0].get_parts_by_text(w1), titles[1].get_parts_by_text(w2)) for w1, w2 in [('How to', 'How to'), ('think about', 'visualize'), ('matrix exponentiation', 'matrix exponentiation')]))
        self.wait(2)
        self.play(*(FadeTransform(titles[1].get_parts_by_text(w1), titles[2].get_parts_by_text(w2)) for w1, w2 in [('How to visualize', 'What problems'), ('matrix exponentiation', 'matrix exponentiation')]), FadeIn(titles[2].get_parts_by_text('solves?')))
        self.wait(2)