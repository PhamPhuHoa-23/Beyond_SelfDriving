...
class TableOfContents(Scene):
    ...
    def construct(self):
        items = VGroup(Text('Summer of Math Exposition'), Text('The Universal Advice'), Text('How to structure math explanations'), Text('Thoughts on animation software'), Text('The 3b1b Podcast/Second channel'))
        ...
        for i in range(len(items)):
            for item in items:
                ...
                if item is items[i]:
                    ...
                    height = 0.3
                    opacity = 0.25
                item.target.scale(height / item[0].get_height(), about_edge=LEFT)
...
class SoME1Name(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(phrase.animate.move_to(UP), MoveToTarget(acronym))
        self.wait()
        self.play(ShowCreation(get_tripple_underline(words[2])), words[2].animate.set_color(BLUE_B))
        self.wait()

class LinkAndDate(Scene):

    def construct(self):
        words = link, date = VGroup(Text('https://3b1b.co/SoME1', font='CMU Serif'), Text('August 22nd', font='CMU Serif'))
        words.set_width(10)
        ...
        self.play(FadeIn(date, scale=1.2, rate_func=squish_rate_func(smooth, 0.3, 0.7)), ShowCreation(lines, lag_ratio=0.25))
        self.wait()
...
class Featuring(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.change('tease'), self.change_students(*3 * ['hooray'], look_at=screen), FadeIn(screen, UP))
        self.play(Write(words))
        self.play(ShowCreation(get_tripple_underline(words)))
        self.wait(5)
        self.student_says(OldTexText('Anything\\\\else?'), target_mode='raise_left_hand', look_at=self.teacher)
...
class Constraints(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says(OldTexText('What are the\\\\constraints?'), added_anims=[self.teacher.change('tease')])
        self.wait(3)

class TopicChoice(TeacherStudentsScene):

    def construct(self):
        self.student_says(OldTexText('What kind of\\\\topics?'), added_anims=[self.teacher.change('tease')], index=1)
        self.wait(3)

class PartialFraction(Scene):

    def construct(self):
        frac = OldTex('\\frac{x+9}{(x-3)(x+5)} = \\frac{?}{x - 3} + \\frac{?}{x + 5}')
        frac.set_width(10)
        self.add(frac)
...
class TrigIdentity(Scene):
    ...
    def construct(self):
        ...
        tan_line = always_redraw(lambda: Line(get_point(), plane.c2p(1 / math.cos(get_theta()), 0), stroke_color=RED))
        sec_line = always_redraw(lambda: Line(plane.c2p(0, 0), tan_line.get_end(), stroke_color=PINK))
        ...
        equation = OldTex('\\tan^2(\\theta) + 1 = \\sec^2(\\theta)', tex_to_color_map={'\\tan': RED, '\\sec': PINK}, font_size=60)
...
class TeacherStudentPairing(Scene):
    ...
    def construct(self):
        ...
        self.play(Write(student_label), GrowArrow(student_arrow), randy.change('hooray', student_label))
        self.wait()
        self.play(Write(teacher_label), GrowArrow(teacher_arrow), morty.change('pondering', teacher_label))
        self.wait()
        for x in range(4):
...
class ButIHaveNoExperience(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says(OldTexText('But I have no\\\\experience!'), index=0, target_mode='pleading')
        self.play_student_changes('pleading', 'pondering', 'pondering', look_at=self.students[0].bubble)
...
class ContentAdvice(Scene):
    ...
    def construct(self):
        ...
        points = VGroup(Text('1) Concrete before abstract'), Text('2) Topic choice > production quality'), Text('3) Be niche'), Text('4) Know your genre'), Text('5) Definitions are not the beginning'))
        points.arrange(DOWN, buff=0.7, aligned_edge=LEFT)
        points.next_to(title, DOWN, buff=1.0)
        self.play(LaggedStartMap(FadeIn, VGroup(*(point[:2] for point in points)), shift=0.25 * RIGHT))
        self.wait()
        for point in points:
        ...
        self.play(VGroup(points[0], *points[2:]).animate.set_opacity(0.25))
        self.play(Write(gt0))
        self.play(ShowCreation(get_tripple_underline(VGroup(points[1].get_part_by_text('production quality'), gt0))))
        self.wait()
...
class LayersOfAbstraction(Scene):
    ...
    def show_pairwise_relations(self):
        ...
        self.play(GrowArrow(down_arrow), FadeIn(down_words))
        self.wait()
        self.play(ReplacementTransform(down_arrow, up_arrow, path_arc=PI), FadeTransform(down_words, up_words))
        self.wait()
    ...
    def get_layers(self):
        ...
        pi_chart = VGroup(*[Sector(start_angle=a, angle=TAU / 3) for a in np.arange(0, TAU, TAU / 3)])
        ...
        numbers = VGroup(OldTex('3'), OldTex('3 \\times 4'), OldTex('1 / 3'))
        ...
        functions = VGroup(OldTex('f(x) = 0'), OldTex('\\frac{df}{dx}'))
        functions.set_height(layers[0].get_height() - 2 * SMALL_BUFF)
        functions.arrange(RIGHT, buff=LARGE_BUFF)
        t2c_map = {'\\textbf{v}': YELLOW, '\\textbf{w}': PINK}
        vector_spaces = VGroup(OldTex('\\textbf{v} + \\textbf{w} =\\textbf{w} + \\textbf{v}', tex_to_color_map=t2c_map), OldTex('s(\\textbf{v} + \\textbf{w}) =s\\textbf{v} + s\\textbf{w}', tex_to_color_map=t2c_map))
        vector_spaces.arrange(DOWN, buff=MED_SMALL_BUFF)
        vector_spaces.set_height(layers[0].get_height() - MED_LARGE_BUFF)
        v, w = vectors = VGroup(Vector([2, 1, 0], color=YELLOW), Vector([1, 2, 0], color=PINK))
        ...
        arrows = VGroup(Arrow(dots[0], dots[1], buff=SMALL_BUFF), Arrow(dots[1], dots[2], buff=SMALL_BUFF), Arrow(dots[0], dots[2], buff=SMALL_BUFF))
        arrows.set_stroke(width=2)
        arrow_labels = VGroup(OldTex('m_1').next_to(arrows[0], UP, SMALL_BUFF), OldTex('m_2').next_to(arrows[1], RIGHT, SMALL_BUFF), OldTex('m_2 \\circ m_1').rotate(-np.arctan(1 / 2)).move_to(arrows[2]).shift(MED_SMALL_BUFF * DL))
        categories = VGroup(dots, arrows, arrow_labels)
        categories.set_height(layers[0].get_height() - MED_SMALL_BUFF)
        all_content = [quantities, numbers, algebra, functions, vector_spaces, categories]
        for layer, content in zip(layers, all_content):
            content.move_to(layer)
            layer.add(content)
            layer.content = content
        layer_titles = VGroup(*map(TexText, ['Quantities', 'Numbers', 'Algebra', 'Functions', 'Vector spaces', 'Categories']))
...
class FractionsExample(Scene):
    ...
    def construct(self):
        ...
        def get_pie(numer, denom, color=BLUE, height=1.5):
            pie = VGroup(*(AnnularSector(angle=TAU / denom, start_angle=i * TAU / denom, inner_radius=0.0, outer_radius=1.0) for i in range(denom)))
            pie.set_stroke(WHITE, 2)
            pie.set_fill(BLACK, 1)
            pie[:numer].set_fill(color)
            pie.set_height(height)
            return pie
        pies = VGroup(get_pie(2, 3, color=BLUE_C), OldTex('+'), get_pie(1, 5, color=BLUE_D))
        pies.arrange(RIGHT)
        pies.move_to(FRAME_HEIGHT * DOWN / 4)
        self.add(expr)
        self.wait()
        self.play(expr.animate.move_to(FRAME_HEIGHT * UP / 4), ShowCreation(h_line), Write(numbers))
        self.play(*(FadeTransform(expr[i].copy(), pies[i]) for i in range(len(pies))), FadeTransform(numbers.copy(), quantities))
        self.wait()
        bottom_eq = OldTex('=')
        bottom_eq.move_to(pies[2])
        self.play(pies.animate.next_to(bottom_eq, LEFT), Write(bottom_eq))
        bottom_rhs = VGroup(*pies[0][:2].copy(), pies[2][0].copy())
        ...
        self.wait()
        rhs = OldTex('=', '{10 \\over 15}', '+', '{3 \\over 15}', '=', '{13 \\over 15}')
        rhs.match_height(expr)
        rhs.next_to(expr, RIGHT)
        expr.generate_target()
        VGroup(expr.target, rhs).set_x(0)
        self.play(MoveToTarget(expr), FadeIn(rhs, lag_ratio=0.1))
        self.wait()
        overlays = VGroup()
...
class CalculusStatement(Scene):
    ...
    def construct(self):
        statement = VGroup(OldTexText('If $f(x)$ has a local maximum or minimum\\\\at $x_0$, and $f$ is differentiable at $x_0$, then'), OldTex('\\frac{df}{dx}(x_0) = 0.'))
...
class ExamplesOfFunctions(Scene):
    ...
    def construct(self):
        ...
        plane.scale(0.5)
        self.add(plane)
        graphs = VGroup(plane.get_graph(lambda x: -x * (x - 4)), plane.get_graph(lambda x: x / 3), plane.get_graph(lambda x: 1 / 3 * x ** 3 - 2 * x + 1), plane.get_graph(lambda x: -5 * x * np.exp(-x ** 2 / 2)))
        graph_labels = VGroup(OldTex('f(x) = ', '-x^2 - 4x'), OldTex('f(x) = ', 'x / 3'), OldTex('f(x) = ', 'x^3 / 3- 2x + 1'), OldTex('f(x) = ', '-(5x) e^{-{1 \\over 2} x^2}'))
        ...
        for graph, color, label in zip(graphs, colors, graph_labels):
            ...
            label.set_stroke(BLACK, 3, background=True)
        self.play(FadeIn(graph_labels[0]), ShowCreation(graphs[0]), run_time=1.5)
        self.wait()
        for i in range(1, len(graphs)):
            self.play(ReplacementTransform(graphs[i - 1], graphs[i]), FadeTransform(graph_labels[i - 1], graph_labels[i]), run_time=0.5)
            self.wait(1.5)
...
class SpecificCases(Scene):
    ...
    def construct(self):
        ...
        labels = VGroup(Text('Profit maximization'), Text('Distance between curves'))
        for label, square in zip(labels, grid):
            label.next_to(square.get_top(), DOWN)
        self.add(grid)
        self.add(labels)
        r_axes = Axes(x_range=(-3, 3), y_range=(-3, 3), width=6, height=4.5, axis_config={'include_tip': False})
        ...
        parabola = r_axes.get_graph(lambda x: 0.2 * (x ** 2 - 4))
        parabola.set_stroke(RED_D)
        smallest_line = Line(circle.pfp(0.83), parabola.pfp(0.76), stroke_width=3)
        self.add(circle, parabola, smallest_line)
        l_axes = Axes((0, 10), (-3, 8), width=6, height=4.5)
        ...
        graph = l_axes.get_graph(lambda x: -0.07 * x * (x - 4) * (x - 9.5))
...
class ConcreteToAbstract(Scene):
    ...
    def construct(self):
        self.add(get_h_line().move_to(FRAME_HEIGHT * UP / 6), get_h_line().move_to(FRAME_HEIGHT * DOWN / 6))
        labels = VGroup(Text('Algebra'), Text('Numbers'), Text('Quantities'))
        for i, label in enumerate(labels):
            label.scale(30 / label.font_size)
            label.to_corner(UL, buff=SMALL_BUFF)
            label.shift(i * FRAME_HEIGHT * DOWN / 3)
        self.add(labels)
        expr = OldTex('x^2 - y^2 = (x + y)(x - y)', tex_to_color_map={'x': BLUE_D, 'y': BLUE_B})
        expr.scale(1.5)
        expr.move_to(FRAME_HEIGHT * UP / 3)
        expr.to_edge(LEFT, buff=LARGE_BUFF)
        self.add(expr)
        tricks = VGroup(OldTex('143 = (12 + 1)(12 - 1) = 13 \\cdot 11'), OldTex('3{,}599 = (60 + 1)(60 - 1) = 61 \\cdot 59'), OldTex('9{,}991 = (100 + 3)(100 - 3) = 103 \\cdot 97'))
        ...
        self.wait()
        arrow = Arrow(ORIGIN, FRAME_HEIGHT * UP / 3, path_arc=60 * DEGREES, thickness=0.15, fill_color=YELLOW, stroke_color=YELLOW)
        ...
        self.play(FadeTransform(top_words, new_top_words), LaggedStartMap(Rotate, up_arrows, angle=PI, axis=RIGHT))
        self.wait()
        self.embed()
...
class DifferenceOfSquares(Scene):
    ...
    def construct(self):
        x = 6
        y = 1
        squares = VGroup(*[VGroup(*[Square() for x in range(x)]).arrange(RIGHT, buff=0) for y in range(x)]).arrange(DOWN, buff=0)
        ...
        self.play(ReplacementTransform(squares[:-y].copy().set_fill(opacity=0), new_squares), ShowCreation(arrow), lag_ratio=0)
        last_row_parts = last_row_parts.copy()
        last_row_parts.save_state()
        last_row_parts.set_fill(opacity=0)
        self.play(last_row_parts.restore, last_row_parts.rotate, -90 * DEGREES, last_row_parts.next_to, new_squares, RIGHT, {'buff': 0}, lag_ratio=0)
...
class AbstractVectorSpace(Scene):
    ...
    def construct(self):
        ...
        columns = VGroup(Matrix([['1'], ['1']], **kw).set_color(BLUE_B), OldTex('+'), Matrix([['-2'], ['3']], **kw).set_color(BLUE_D), OldTex('='), Matrix([['1 - 2'], ['1 + 3']], **kw).set_color(GREEN))
        columns.arrange(RIGHT, buff=SMALL_BUFF)
        columns.next_to(low_title, DOWN, LARGE_BUFF)
        columns.to_edge(LEFT)
        arrows = VGroup(Arrow(ORIGIN, (1, 1), fill_color=BLUE_B, buff=0), Arrow((1, 1), (-1, 4), fill_color=BLUE_D, buff=0), Arrow(ORIGIN, (-1, 4), fill_color=GREEN, buff=0))
        arrows.match_height(columns)
        arrows.scale(1.5)
        arrows.match_y(columns)
        arrows.match_x(low_title)
        funcs = OldTex('(f + g)(x) = f(x) + g(x)', tex_to_color_map={'f': BLUE_B, 'g': BLUE_D})
        ...
        w_tex = '\\vec{\\textbf{w}}'
        v_tex = '\\vec{\\textbf{v}}'
        axioms = VGroup(*it.starmap(Tex, [('1. \\,', u_tex, '+', '(', v_tex, '+', w_tex, ')=(', u_tex, '+', v_tex, ')+', w_tex), ('2. \\,', v_tex, '+', w_tex, '=', w_tex, '+', v_tex), ('3. \\,', '\\textbf{0}+', v_tex, '=', v_tex, '\\text{ for all }', v_tex), ('4. \\,', '\\forall', v_tex, '\\;\\exists', w_tex, '\\text{ s.t. }', v_tex, '+', w_tex, '=\\textbf{0}'), ('5. \\,', 'a', '(', 'b', v_tex, ')=(', 'a', 'b', ')', v_tex), ('6. \\,', '1', v_tex, '=', v_tex), ('7. \\,', 'a', '(', v_tex, '+', w_tex, ')', '=', 'a', v_tex, '+', 'a', w_tex), ('8. \\,', '(', 'a', '+', 'b', ')', v_tex, '=', 'a', v_tex, '+', 'b', v_tex)]))
        for axiom in axioms:
            axiom.set_color_by_tex_to_color_map({u_tex: BLUE_B, w_tex: BLUE_D, v_tex: GREEN})
...
class Nicheness(Scene):
    ...
    def construct(self):
        words = OldTexText('Perceived ', 'nicheness', ' $>$ ', 'Actual ', 'nichness')
        words[:2].set_color(BLUE_B)
        words[3:].set_color(BLUE_D)
        self.add(words[:2])
        self.wait()
        self.play(FadeTransformPieces(words[:2].copy(), words[3:], path_arc=PI / 2), GrowFromCenter(words[2]))
        self.wait()

class TransitionToProductionQuality(TeacherStudentsScene):

    def construct(self):
        self.play(self.students[0].change('pondering'), self.students[2].change('pondering'))
        self.student_says(OldTexText('What parts of\\\\production quality matter?'), index=1)
...
class SurfaceExample(Scene):
    ...
    def construct(self):
        ...
        self.play(Transform(surface, surfaces[2]), frame.animate.increment_phi(-10 * DEGREES), frame.animate.increment_theta(-20 * DEGREES), run_time=3)
...
class BadManimExample(Scene):
    ...
    def construct(self):
        words = OldTexText('Does any of this ', 'need to be ', 'animated?\\\\', 'Much less programatically?')
        ...
        self.wait()
        self.play(Transform(VGroup(*words[0], *words[1], *words[2]).copy(), words[3], lag_ratio=0.03, run_time=1))
        self.wait()

class WhereCanIEngageWithOthers(TeacherStudentsScene):

    def construct(self):
        self.student_says(OldTexText('Where can I find\\\\others joining SoME1?'), index=0, added_anims=[self.students[1].change('pondering', UL), self.students[2].change('pondering', UL)])
