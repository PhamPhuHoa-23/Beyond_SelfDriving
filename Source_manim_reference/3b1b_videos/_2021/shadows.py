...
def get_key_result(solid_name, color=BLUE):
    eq = OldTex('\\text{Area}\\big(\\text{Shadow}(\\text{' + solid_name + '})\\big)', '=', '\\frac{1}{2}', '{c}', '\\cdot', '(\\text{Surface area})', tex_to_color_map={'\\text{Shadow}': GREY_B, f'\\text{{{solid_name}}}': color, '\\text{Solid}': BLUE, '{c}': RED})
...
class ShadowScene(ThreeDScene):
    ...
    plane_dims = (20, 20)
    plane_style = {'stroke_width': 0, 'fill_color': GREY_A, 'fill_opacity': 0.5, 'gloss': 0.5, 'shadow': 0.2}
    limited_plane_extension = 0
    object_style = {'stroke_color': WHITE, 'stroke_width': 0.5, 'fill_color': BLUE_E, 'fill_opacity': 0.7, 'reflectiveness': 0.3, 'gloss': 0.1, 'shadow': 0.5}
    inf_light = False
    glow_radius = 10
    glow_factor = 10
    ...
    def add_plane(self):
        ...
        grid = NumberPlane(x_range=(-width // 2, width // 2, 2), y_range=(-height // 2, height // 2, 2), background_line_style={'stroke_color': GREY_B, 'stroke_width': 1}, faded_line_ratio=4)
    ...
    def setup_light_source(self):
        ...
        if self.inf_light:
            ...
            glow = self.glow = TrueDot(radius=self.glow_radius, glow_factor=self.glow_factor)
    ...
    def randomly_reorient(self, solid=None, about_point=None):
        ...
        solid.rotate(random.uniform(0, TAU), axis=normalize(np.random.uniform(-1, 1, 3)), about_point=about_point)
        return solid
...
class IntroduceShadow(ShadowScene):
    ...
    def construct(self):
        ...
        question = OldTexText("Puzzle: Find the average\\\\area of a cube's shadow", font_size=48)
        ...
        self.shadow.update()
        self.play(FadeIn(question, UP), *(LaggedStartMap(DrawBorderThenFill, mob, lag_ratio=0.1, run_time=3) for mob in (cube, shadow)))
        self.random_toss(run_time=3, angle=TAU)
        outline.update()
        area_label.update()
        self.play(FadeIn(area_label), ShowCreation(outline))
        self.play(cube.animate.scale(0.5), run_time=2, rate_func=there_and_back)
        ...
        self.play(FadeIn(light_comment, 0.5 * UP), light.animate.next_to(cube, OUT, buff=1.5), run_time=2)
        light_lines.update()
        self.play(ShowCreation(light_lines, lag_ratio=0.01, run_time=3))
        self.play(light.animate.shift(1.0 * IN), rate_func=there_and_back, run_time=3)
        self.play(light.animate.shift(4 * RIGHT), run_time=5)
        self.play(Rotate(light, PI, about_point=light.get_z() * OUT), run_time=8)
        self.play(light.animate.shift(4 * RIGHT), run_time=5)
        self.wait()
        self.play(frame.animate.set_height(12).set_z(4), light.animate.set_z(10), run_time=3)
        self.wait()
        self.play(light.animate.move_to(75 * OUT), run_time=3)
        self.wait()
        self.play(frame.animate.set_height(8).set_z(2), LaggedStart(*map(FadeOut, (question, subquestion, light_comment))), run_time=2)
        ...
        self.play(Restore(vert_dots), Write(xyz_label))
        self.wait()
        self.play(TransformFromCopy(cube.deepcopy().clear_updaters().set_opacity(0.5), shadow.deepcopy().clear_updaters().set_opacity(0), remover=True), TransformFromCopy(vert_dots, proj_dots), TransformFromCopy(xyz_label, xy0_label), *map(ShowCreation, vert_proj_lines))
        self.wait(3)
        self.play(LaggedStart(*map(FadeOut, (vert_dots, vert_proj_lines, proj_dots, xyz_label, xy0_label))))
        ...
        edge_lines = VGroup(Line(corner, cube.get_corner(DR + OUT)), Line(corner, cube.get_corner(UL + OUT)), Line(corner, cube.get_corner(DL + IN)))
        ...
        self.play(LaggedStartMap(ShowCreation, edge_lines), LaggedStartMap(FadeIn, s_labels, scale=2), MoveToTarget(frame, run_time=3))
        self.wait()
        self.play(TransformFromCopy(s_labels[:2], s_squared))
        self.wait(2)
        ...
        self.wait()
        self.play(LaggedStart(*map(FadeOut, (rect, s_eq, *edge_lines, *s_labels, s_squared))))
        self.wait()
        axis = UL
        angle = np.arccos(1 / math.sqrt(3))
        area_label.suspend_updating()
        self.play(Rotate(cube, -angle, axis), frame.animate.reorient(-10, 70), ChangeDecimalToValue(area_label[1], math.sqrt(3)), UpdateFromFunc(area_label[1], lambda m: m.fix_in_frame()), run_time=2)
        self.add(area_label)
        ...
        self.play(frame.animate.reorient(0, 0), cube.animate.rotate(-angle).set_opacity(0.2), run_time=3)
        ...
        self.play(ShowCreationThenFadeOut(outline_copy), Write(title, run_time=1))
        self.play(FadeOut(title), Restore(frame), cube.animate.set_opacity(cube_opacity).rotate(angle), run_time=3)
        ...
        self.begin_ambient_rotation(cube)
        self.play(FadeOut(light_lines), FadeIn(question, 0.5 * UP), ApplyMethod(frame.set_height, 8, run_time=2))
        ...
        self.play(GrowFromCenter(brace), FadeIn(text), Write(v_dots))
        self.wait()
        for x in range(10):
            self.random_toss()
            self.wait()
...
class AskAboutAveraging(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        sts = self.students
        tch = self.teacher
        self.play_student_changes('maybe', 'thinking', 'erm', look_at=self.screen, added_anims=[self.teacher.change('raise_right_hand', self.screen)])
        self.wait(3)
        self.play(PiCreatureBubbleIntroduction(sts[2], OldTexText('What does that\\\\mean, exactly?'), target_mode='hesitant', look_at=self.screen, bubble_config={'direction': LEFT}), LaggedStart(sts[0].change('confused', self.screen), sts[1].change('pondering', self.screen), tch.change('tease', sts[2].eyes)))
        self.wait(4)
        self.student_says('Can we do an experiment?', target_mode='raise_left_hand', index=1)
        self.wait(4)
        self.student_says(OldTexText("But what defines a\\\\``random'' toss?"), look_at=self.screen, target_mode='hesitant', index=2, added_anims=[self.teacher.change('guilty'), self.students[0].change('erm')])
        self.wait(4)
        self.play(LaggedStart(self.students[0].change('pondering', self.screen), self.students[1].change('maybe', self.screen), self.teacher.change('tease', self.screen)))
        self.wait(2)
        self.teacher_says(OldTexText('Hold off until\\\\the end'))
        self.wait(3)
        self.play_student_changes('thinking', 'tease', 'pondering', look_at=self.screen, added_anims=[self.teacher.change('tease', self.students)])
        self.wait(4)

class MeanCalculation(Scene):

    def construct(self):
        values = [1.55, 1.33, 1.46, 1.34, 1.5, 1.26, 1.42, 1.54, 1.51]
...
class DescribeSO3(ShadowScene):
    ...
    def construct(self):
        ...
        def get_matrix():
            return np.array([x_point.get_center(), y_point.get_center(), z_point.get_center()]).T

        def get_mat_mob():
            matrix = DecimalMatrix(get_matrix(), element_to_mobject_config=dict(num_decimal_places=2, edge_to_fix=LEFT, include_sign=True), h_buff=2.0, element_alignment_corner=LEFT)
            matrix.fix_in_frame()
        ...
        self.wait(5)
        new_question = Text('What probability distribution are we placing\non the space of all orientations?', t2c={'probability distribution': YELLOW}, t2s={'probability distribution': ITALIC})
        ...
        self.play(FadeTransform(question[-n:], new_question[-n:]), FadeOut(question[:-n]), FadeIn(new_question[:-n]))
        self.wait()
        cube.clear_updaters()
        N = 15
        ...
        self.play(FadeTransform(cube, cube_field[0]), LaggedStartMap(FadeIn, cube_field, run_time=15, lag_ratio=0.1))
        self.add(cube_field)
        self.wait()
...
class PauseAndPonder(TeacherStudentsScene):
    ...
    def construct(self):
        self.remove(self.background)
        self.teacher_says(OldTexText('The goal is\\\\not speed.'), added_anims=[self.change_students('tease', 'well', 'pondering', look_at=self.screen)])
        self.wait(2)
        self.play(RemovePiCreatureBubble(self.teacher, target_mode='tease'), PiCreatureBubbleIntroduction(self.students[2], Lightbulb(), bubble_type=ThoughtBubble, bubble_creation_class=lambda m: FadeIn(m, lag_ratio=0.1), bubble_config=dict(height=3, width=3, direction=LEFT), target_mode='thinking', look_at=self.screen))
        self.wait(3)
        self.teacher_says('Pause and ponder!', target_mode='well', added_anims=[self.change_students('pondering', 'tease', 'thinking')], run_time=1)
        self.wait(5)
        self.embed()
...
class StartSimple(Scene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(line), Write(title, run_time=1))
        self.wait()
        ...
        self.wait()
        self.play(LaggedStart(*map(DrawBorderThenFill, cube)), ShowCreation(arrow), TransformFromCopy(cube[-1], face))
        self.wait(3)
...
class FocusOnOneFace(ShadowScene):
    ...
    def construct(self):
        ...
        shadow = self.shadow
        frame = self.camera.frame
        words = VGroup(Text('Just one orientation'), Text('Just one face'))
        words.fix_in_frame()
        ...
        self.play(FadeIn(words[0], 0.75 * UP), FadeOut(average_words, 0.75 * UP), run_time=0.5)
        self.wait()
        cube.update()
        ...
        self.play(MoveToTarget(cube), FadeIn(words[1]))
        self.play(frame.animate.reorient(-10, 65), FlashAround(words[1], rate_func=squish_rate_func(smooth, 0.2, 0.5)), FlashAround(words[0], rate_func=squish_rate_func(smooth, 0.5, 0.8)), run_time=5)
        frame.add_updater(lambda f, dt: f.increment_theta(0.01 * dt))
        self.solid = face
        self.remove(shadow)
        self.add_shadow()
        ...
        self.play(frame.animate.set_phi(70 * DEGREES), FadeIn(light_lines, lag_ratio=0.5), TransformFromCopy(face, face.deepcopy().set_opacity(0).set_z(0), remover=True), run_time=3)
        self.wait(3)
        self.play(Rotate(face, PI / 2, UP), FadeOut(area_q, scale=0), run_time=3)
        self.wait(3)
        self.play(Rotate(face, -PI / 3, UP), UpdateFromAlphaFunc(light_lines, lambda m, a: m.set_opacity(0.5 * (1 - a)), remover=True), run_time=2)
        z_axis = VGroup(Line(ORIGIN, face.get_center()), Line(face.get_center(), 10 * OUT))
        ...
        normal_vect.add_updater(lambda v: v.put_start_and_end_on(get_fc(), get_fc() + get_un()))
        arc = always_redraw(lambda: Arc(start_angle=PI / 2, angle=-get_theta(), radius=0.5, stroke_width=2).rotate(PI / 2, RIGHT, about_point=ORIGIN).shift(get_fc()))
        ...
        self.play(FadeIn(theta, 0.5 * OUT), ShowCreation(arc))
        frame.reorient(2)
        face.rotate(-35 * DEGREES, get_un(), about_point=face.get_center())
        self.play(Rotate(face, 50 * DEGREES, UP), rate_func=there_and_back, run_time=8)
        axes = Axes((0, 180, 22.5), (0, 1, 0.25), width=5, height=2, axis_config={'include_tip': False, 'tick_size': 0.05, 'numbers_to_exclude': []})
        ...
        axes.fix_in_frame()
        graph = axes.get_graph(lambda x: math.cos(x * DEGREES), x_range=(0, 90))
        ...
        dot.fix_in_frame()
        self.play(FadeIn(axes), Rotate(face, -get_theta(), UP, run_time=2))
        self.play(FadeIn(dot, shift=2 * UP + RIGHT))
        self.wait(2)
        self.add(graph, axes)
        self.play(UpdateFromFunc(dot, lambda d: d.move_to(graph.get_end())), ShowCreation(graph), Rotate(face, PI / 2, UP), run_time=5)
        ...
        self.play(LaggedStartMap(FadeOut, VGroup(*words, graph, axes, dot)), Restore(plane, run_time=3))
        ...
        self.play(frame.animate.set_euler_angles(PI / 2, get_theta()), FadeOut(VGroup(theta, arc)), run_time=2)
        ...
        self.play(MoveToTarget(frame), *map(FadeIn, (theta, arc)), FadeOut(plane), FadeOut(slices), FadeOut(self.shadow), FadeIn(line), run_time=2)
        self.wait()
        shadow = line.copy()
        ...
        self.play(TransformFromCopy(line, shadow), FadeIn(shadow_label, 0.5 * IN))
        self.wait()
        self.play(ShowCreation(vert_line))
        self.wait()
        top_theta_group = VGroup(z_axis[1].copy(), arc.copy().clear_updaters(), theta.copy().clear_updaters(), Line(*normal_vect.get_start_and_end()).match_style(z_axis[1].copy()))
        ...
        dr_arc = Arc(radius=0.4, start_angle=PI, angle=-get_theta())
        ...
        self.play(ShowCreation(ul_arc), FadeTransform(top_theta_group[2].copy(), supl))
        self.play(TransformFromCopy(ul_arc, dr_arc), TransformFromCopy(supl[0][4].copy().set_stroke(width=0), dr_theta[0][0]))
        self.wait()
        rect = Rectangle(0.8, 0.5)
        rect.set_stroke(YELLOW, 2)
        rect.rotate(PI / 2, RIGHT)
        rect.move_to(dr_theta, LEFT).shift(SMALL_BUFF * LEFT)
        self.play(ShowCreation(rect), top_theta_group.animate.fade(0.8), ul_angle_group.animate.fade(0.8))
        self.wait()
        cos_formula = OldTex('\\cos(\\theta)', '=', '{\\text{Length of }', '\\text{shadow}', '\\over', '\\text{Length of }', '\\text{slice}}')
        ...
        self.wait()
        self.play(TransformMatchingTex(VGroup(*(cos_formula[i].copy() for i in [0, 1, 3, 6])), lower_formula, path_arc=PI / 4))
        self.wait()
        frame.generate_target()
        ...
        self.play(MoveToTarget(frame, run_time=5), FadeIn(face, run_time=3), FadeIn(self.shadow, run_time=3), FadeIn(line_shadow, run_time=3), LaggedStart(*map(FadeOut, [top_theta_group, ul_angle_group, rect, dr_theta, dr_arc, vert_line, shadow, shadow_label]), run_time=4))
        frame.add_updater(lambda f, dt: f.increment_theta(0.01 * dt))
        self.wait(2)
        perp = Line(face.pfp(binary_search(lambda a: face.pfp(a)[2], face.get_center()[2], 0, 0.5)), face.pfp(binary_search(lambda a: face.pfp(a)[2], face.get_center()[2], 0.5, 1.0)))
        perp.set_stroke(RED, 3)
        perp_shadow = get_shadow(perp)
        perp_shadow.set_stroke(RED_E, 3, opacity=0.2)
        self.add(perp, normal_vect, arc)
        self.play(ShowCreation(perp), ShowCreation(perp_shadow))
        face.add(line)
        ...
        final_formula = OldTex('\\text{Area}(', '\\text{shadow}', ')', '=', '|', '\\cos(\\theta)', '|', 's^2')
        ...
        self.wait(10)
        face.remove(line)
        self.play(frame.animate.shift(0.5 * DOWN + RIGHT).reorient(10), LaggedStart(*map(FadeOut, [cos_formula, lower_formula])), FadeIn(graph), FadeIn(axes), FadeOut(line), FadeOut(line_shadow), FadeOut(perp), FadeOut(perp_shadow), final_formula.animate.shift(2 * DOWN), run_time=2)
        self.play(Rotate(face, PI / 2 - get_theta(), UP), run_time=2)
        new_graph = axes.get_graph(lambda x: math.cos(x * DEGREES), (90, 180))
        new_graph.match_style(graph)
        new_graph.fix_in_frame()
        self.play(Rotate(face, PI / 2, UP), ShowCreation(new_graph), run_time=5)
        self.play(Rotate(face, -PI / 4, UP), run_time=2)
        self.wait(3)
        ...
        self.play(MoveToTarget(new_graph), final_formula.get_parts_by_tex('|').animate.set_opacity(1))
...
class NotQuiteRight(TeacherStudentsScene):
    ...
    def construct(self):
        self.remove(self.background)
        self.teacher_says('Not quite right...', target_mode='hesitant', bubble_config={'height': 3, 'width': 4}, added_anims=[self.change_students('pondering', 'thinking', 'erm', look_at=self.screen)])
        self.wait(4)
...
class DiscussLinearity(Scene):
    ...
    def construct(self):
        ...
        panels.center()
        self.add(panels)
        arrows = VGroup(*(Arrow(p1.get_top(), p2.get_top(), path_arc=-0.6 * PI).scale(0.75, about_edge=DOWN) for p1, p2 in zip(panels, panels[1:])))
        arrows.space_out_submobjects(0.8)
        arrows.rotate(PI, RIGHT, about_point=panels.get_center())
        arrow_labels = VGroup(Text('Rotation', font_size=30), Text('Flat projection', font_size=30))
        arrow_labels.set_backstroke()
        for arrow, label in zip(arrows, arrow_labels):
            label.next_to(arrow.pfp(0.5), UP, buff=0.35)
        shape_labels = VGroup(Text('Some shape'), Text('Any shape'))
        ...
        self.play(TransformFromCopy(lin_part.replicate(2), lin_copies), LaggedStart(FadeOut(lin_text, lag_ratio=0.1), *map(FadeOut, uniform_words)))
        area_labels = VGroup(Text('Area(shape)', t2c={'shape': BLUE}), Text('Area(shadow)', t2c={'shadow': BLUE_E}))
        area_exprs = VGroup(OldTex('A').set_color(BLUE), OldTex('(\\text{some factor})', '\\cdot ', 'A'))
        ...
        self.play(*(FadeIn(eq, 0.5 * DOWN) for eq in equals), *(FadeIn(expr, DOWN) for expr in area_exprs))
        self.wait()
        ...
        prod.set_height(0.8)
        det = OldTex('\\text{det}', '\\Big(', '\\Big)', tex_to_color_map={'\\text{det}': YELLOW}, font_size=36)
        ...
        self.play(ShowCreation(rect))
        self.play(FadeTransform(rect.copy(), det_rect), FadeTransform(factor.copy(), det), ShowCreation(arrow))
        self.wait()
        self.play(FadeOut(det_rect), GrowFromCenter(rot_brace), FadeIn(details))
        self.wait()
        ...
        self.play(arrow.animate.match_points(Arrow(factor.get_corner(UL), ind_words.get_corner(DR))), Write(ind_words, run_time=1))
        self.wait()
...
class Matrices(Scene):
    ...
    def construct(self):
        ...
        kw = {'v_buff': 0.7, 'bracket_v_buff': 0.15, 'bracket_h_buff': 0.15}
        matrices = VGroup(Matrix([['v_1', 'w_1'], ['v_2', 'w_2'], ['v_3', 'w_3']], h_buff=1.0, **kw), Matrix([['1', '0', '0'], ['0', '1', '0']], h_buff=0.6, **kw))
        ...
        vectors = VGroup(Matrix([['x_0'], ['y_0']], **kw), Matrix([['x_1'], ['y_1'], ['z_1']], **kw), Matrix([['x_2'], ['y_2']], **kw))
        for vect, x in zip(vectors, [-6, 0, 6]):
            vect.set_x(x)
            vect.set_y(2.5)
        arrows = VGroup(Arrow(vectors[0], vectors[1]), Arrow(vectors[1], vectors[2]), Arrow(vectors[0], vectors[2]))
        for mat, arrow in zip((*matrices[:2], mat_product), arrows):
            mat.next_to(arrow, UP, SMALL_BUFF)
        self.add(vectors[0])
        for i in range(2):
            self.play(FadeTransform(vectors[i].copy(), vectors[i + 1]), ShowCreation(arrows[i]), FadeIn(matrices[i], 0.5 * RIGHT))
            self.wait()
        self.play(Transform(arrows[0], arrows[2]), Transform(arrows[1], arrows[2]), Transform(matrices, mat_product), FadeOut(vectors[1], scale=0))
...
class DefineDeterminant(Scene):
    ...
    def construct(self):
        ...
        titles = VGroup(Text('Input'), Text('Output'))
        ...
        self.play(DrawBorderThenFill(square), FadeIn(area_label, 0.25 * UP, rate_func=squish_rate_func(smooth, 0.5, 1)))
        self.wait()
        arrow = Arrow(*planes)
        ...
        self.play(ReplacementTransform(planes[0].copy().fade(1), planes[0].output, run_time=2), ReplacementTransform(square.copy().fade(1), square.output, run_time=2), ShowCreation(arrow), FadeIn(arrow_label, 0.25 * RIGHT), FadeIn(mat_mob, 0.25 * RIGHT))
        self.wait()
        new_area_label = OldTex('\\text{Area} = ', '{c}', '\\cdot', '{A}', tex_to_color_map={'{c}': RED, '{A}': YELLOW})
        ...
        self.wait()
        self.play(FadeTransform(mat_mob.copy(), det), FadeTransform(new_area_label.get_part_by_tex('c'), det_name), new_area_label[1].animate.next_to(det, LEFT, SMALL_BUFF).match_y(new_area_label[1]))
        self.wait()

class AmbientShapeRotationPreimage(ShadowScene):
    inf_light = False
    display_mode = 'preimage_only'
    rotate_in_3d = True
    ...
    def construct(self):
        ...
        def update_shadow(shadow):
            shadow.set_points(np.apply_along_axis(lambda p: project_to_xy_plane(self.light.get_center(), p), 1, rotated.get_points()))
        ...
        if display_mode == 'full_3d':
            ...
            orientation_arrows = VGroup(Vector(RIGHT, stroke_color=RED_D), Vector(UP, stroke_color=GREEN_D), Vector(OUT, stroke_color=BLUE_D))
            orientation_arrows.set_stroke(opacity=0.85)
            orientation_arrows.shift(fc)
            orientation_arrows.save_state()
            orientation_arrows.add_updater(lambda m: m.restore().rotate(rotated.angle_tracker.get_value(), rotated.axis_tracker.get_location()))
            ...
            proj_lines = always_redraw(lambda: VGroup(*(Line(rotated.pfp(a), flat_project(rotated.pfp(a))).set_stroke(WHITE, 0.5, 0.2) for a in np.linspace(0, 1, 100))))
            self.add(proj_lines)
        ...
        preimage.become(cat)
        self.wait(2)
        self.play(preimage.animate.scale(2), rate_func=there_and_back, run_time=3)
        self.play(preimage.animate.become(dog), path_arc=PI, rate_func=there_and_back_with_pause, run_time=5)
        self.wait(6)
        self.play(rotated.rot_speed_tracker.animate.set_value(0))
        ...
        if self.display_mode == 'full_3d':
            ...
            anims += [UpdateFromAlphaFunc(proj_lines, lambda m, a: m.set_stroke(opacity=pso * (1 - a))), UpdateFromAlphaFunc(light_lines, lambda m, a: m.set_stroke(opacity=lso * a)), angle_anim, frame.animate.reorient(20, 70).set_height(8)]
            frame.clear_updaters()
        if self.display_mode == 'shadow_only':
            anims += [frame.animate.set_height(10), angle_anim]
        ...
        self.play(preimage.animate.stretch(1.5, 0), rate_func=there_and_back, run_time=5)
        anims = [rotated.axis_tracker.animate.move_to(RIGHT)]
        if self.display_mode == 'full_3d':
            anims.append(frame.animate.reorient(-20, 70))
        self.play(*anims, run_time=2)
        self.play(preimage.animate.stretch(2, 1), rate_func=there_and_back, run_time=7)
        self.play(rotated.rot_speed_tracker.animate.set_value(0.1))
        self.wait(30)
...
class IsntThatObvious(TeacherStudentsScene):
    ...
    def construct(self):
        self.remove(self.background)
        self.student_says(OldTexText("Isn't that obvious?"), bubble_config={'height': 3, 'width': 4, 'direction': LEFT}, target_mode='angry', look_at=self.screen, added_anims=[LaggedStart(self.teacher.change('guilty'), self.students[0].change('pondering', self.screen), self.students[1].change('erm', self.screen))])
        self.wait(2)
        self.play(self.students[0].change('hesitant'))
        self.wait(2)
...
class StretchLabel(Scene):
    ...
    def construct(self):
        ...
        self.play(*map(ShowCreation, label[::2]), Write(label[1]))
        self.wait()
...
class WonderAboutAverage(Scene):
    ...
    def construct(self):
        ...
        randy.to_edge(DOWN)
        randy.look(RIGHT)
        self.play(PiCreatureBubbleIntroduction(randy, OldTexText('How do you think\\\\about this average'), target_mode='confused', run_time=2))
        for x in range(2):
...
class SingleFaceRandomRotation(ShadowScene):
    ...
    def setup(self):
        ...
        arrows = VGroup(Line(ORIGIN, RIGHT, color=RED_D), Line(ORIGIN, UP, color=GREEN_D), VGroup(Vector(OUT, stroke_width=4, stroke_color=BLACK), Vector(OUT, stroke_width=3, stroke_color=BLUE_D)))
    ...
    def construct(self):
        ...
        for x in range(self.n_rotations):
            self.random_toss(face, about_point=fc, angle=3 * PI, run_time=8, rate_func=smooth)
            self.wait()
...
class AverageFaceShadow(SingleFaceRandomRotation):
    ...
    def construct(self):
        ...
        for n in range(n_samples):
            ...
            if n == n_samples - 1:
                normal = next((sm.get_unit_normal() for sm in face.family_members_with_points() if isinstance(sm, VMobject) and sm.get_fill_opacity() > 0))
        ...
        self.wait()
        self.play(face.animate.scale(0.5, about_point=point), shadows.animate.scale(0.5, about_point=ORIGIN), run_time=3, rate_func=there_and_back)
        for axis in [0, 1]:
            self.play(face.animate.stretch(2, axis, about_point=point), shadows.animate.stretch(2, axis, about_point=ORIGIN), run_time=3, rate_func=there_and_back)
        self.wait()
        self.play(self.camera.frame.animate.reorient(-10).shift(2 * LEFT))
        self.add(shadow)
        for n in range(100):
            self.randomly_reorient(face, about_point=point)
            self.wait(0.2)
...
class AverageShadowAnnotation(Scene):
    ...
    def construct(self):
        ...
        self.wait(2)
        shape_name = 'Square'
        t2c = {'Shadow': GREY_B, shape_name: BLUE, '$c$': RED}
        formula = VGroup(OldTexText(f'Area(Shadow({shape_name}))', tex_to_color_map=t2c), OldTex('=').rotate(PI / 2), OldTexText('$c$', ' $\\cdot$', f'(Area({shape_name}))', tex_to_color_map=t2c))
        ...
        self.wait()
        self.play(VShowPassingFlash(overline.copy().insert_n_curves(100).set_stroke(YELLOW, 5), time_width=0.75, run_time=2))
        self.wait()
        ...
        self.play(FadeOut(c, 0.5 * UP), FadeIn(half, 0.5 * UP))
        self.wait()
...
class AlicesFaceAverage(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        font_size = 30
        fra_labels = VGroup(*(OldTex(f'f(R_{i})', '\\cdot ', 'A', tex_to_color_map={'A': BLUE}, font_size=font_size) for i in indices))
        DARK_BLUE = interpolate_color(BLUE_D, BLUE_E, 0.5)
        area_shadow_labels = VGroup(*(OldTex('\\text{Area}(', '\\text{Shadow}_' + i, ')', tex_to_color_map={'\\text{Shadow}_' + i: DARK_BLUE}, font_size=font_size) for i in indices))
        s_labels = VGroup(*(OldTex(f'S_{i}', '=', tex_to_color_map={f'S_{i}': DARK_BLUE}, font_size=font_size) for i in indices))
        label_arrows = VGroup()
        ...
        lr = 0.2
        self.play(LaggedStartMap(FadeIn, area_shadow_labels, lag_ratio=lr), LaggedStartMap(ShowCreation, label_arrows, lag_ratio=lr), LaggedStartMap(FadeIn, fra_labels, shift=DOWN, lag_ratio=lr))
        self.wait()
        self.play(LaggedStart(*(FadeTransform(area, area_s) for area, area_s in zip(area_shadow_labels, s_labels)), lag_ratio=lr), LaggedStartMap(MoveToTarget, fra_labels, lag_ratio=lr), LaggedStartMap(Uncreate, label_arrows, lag_ratio=lr))
        sample_average = OldTex('\\text{Sample average}', '=', '\\frac{1}{n}', '\\left(', 'f(R_1)', '\\cdot ', 'A', '+', 'f(R_2)', '\\cdot ', 'A', '+', 'f(R_3)', '\\cdot ', 'A', '+', '\\cdots ', 'f(R_n)', '\\cdot ', 'A', '\\right)', font_size=font_size)
        ...
        self.play(LaggedStart(*(FlashAround(mob, time_width=3) for mob in sample_average.get_parts_by_tex('A')), lag_ratio=0.1, run_time=2))
        ...
        self.play(LaggedStart(*map(FadeOut, [*fra_labels, *s_labels, *panels, dots, *rot_labels, rot_words])))
        self.wait()
        rect = SurroundingRectangle(VGroup(new_pieces[4:], brace, lim), buff=SMALL_BUFF)
...
class ManyShadows(SingleFaceRandomRotation):
    ...
    def construct(self):
        ...
        planes.to_edge(DOWN)
        planes.update()
        self.play(LaggedStart(*(FadeIn(plane, scale=1.1) for plane in planes), lag_ratio=0.6, run_time=10))
        self.wait()
        self.embed()
...
class AllPossibleOrientations(ShadowScene):
    ...
    def construct(self):
        ...
        sphere_points = np.array([sphere.uv_func(phi, theta + theta_step * (phi / TAU)) for theta in np.arange(0, PI, theta_step) for phi in np.linspace(0, TAU, int(2 * n_lat_lines * math.sin(theta)) + 1)])
        sphere_points[:, 2] *= -1
        ...
        self.play(ShowCreation(sphere_dots), ShowIncreasingSubsets(sphere_lines), UpdateFromAlphaFunc(face, lambda m, a: m.apply_matrix(rotation_between_vectors(normal_vect.get_vector(), original_sphere_points[int(a * (N - 1))]), about_point=fc)), run_time=30, rate_func=smooth)
        self.play(FadeOut(sphere_words, UP), FadeIn(uniform_words, UP))
        last_dot = Mobject()
        for x in range(20):
            point = random.choice(sphere_points)
            dot = TrueDot(point, radius=1, glow_factor=10, color=YELLOW)
            self.add(dot)
            self.play(face.animate.apply_matrix(rotation_between_vectors(normal_vect.get_vector(), point - fc), about_point=fc), FadeOut(last_dot, run_time=0.25), FadeIn(dot), run_time=0.5)
            self.wait(0.25)
            last_dot = dot
        ...
        v2 = DR + OUT
        frame.reorient(-5)
        self.play(Rotate(face, angle_between_vectors(v1, v2), axis=normalize(cross(v1, v2))), UpdateFromAlphaFunc(self.plane, lambda m, a: square.scale(0.9).set_opacity(0.5 - a * 0.5)))
        self.play(ShowCreation(sphere_mesh, lag_ratio=0.5), FadeIn(sphere), sphere_dots.animate.set_radius(0), FadeOut(sphere_lines), frame.animate.reorient(0), run_time=3)
        self.remove(sphere_dots)

        def get_patch(u, v, delta_u=0.05, delta_v=0.1):
            patch = ParametricSurface(sphere.uv_func, u_range=(u * TAU, (u + delta_u) * TAU), v_range=(v * PI, (v + delta_v) * PI))
            patch.shift(fc)
        ...
        self.play(ShowCreation(patch), frame.animate.reorient(10, 75), run_time=2)
        ...
        for i in (4, 6):
            ...
            self.wait()
        patches = Group(get_patch(0.65, 0.5), get_patch(0.55, 0.8), get_patch(0.85, 0.8), get_patch(0.75, 0.4, 0.1, 0.2))
        patch.deactivate_depth_test()
        self.add(sphere, patch)
        for new_patch in patches:
            self.play(Transform(patch, new_patch))
            self.wait()
        self.play(LaggedStart(*map(FadeOut, (sphere, sphere_mesh, patch, *prob, uniform_words))))
        self.play(square.animate.set_fill(opacity=0.5), frame.animate.reorient(-30), run_time=3)
        self.play(Rotate(square, TAU, normal_vect.get_vector()), run_time=8)
        self.wait()
        ...
        self.play(ShowCreation(z_axis), ShowCreation(arc), FadeIn(theta, 0.5 * OUT))
        self.wait()
        ...
        self.play(Write(shadow_area, run_time=3), Rotate(face, TAU, normal_vect.get_vector(), run_time=10))
        self.wait(4)
        ...
        self.play(GrowFromCenter(brace), MoveToTarget(shadow_area[1]), FadeOut(shadow_area[0]), square.animate.set_fill(opacity=0))
        face.generate_target()
        face.target[1].set_length(0.98, about_point=fc)
        sphere.set_opacity(0.35)
        sphere_mesh.set_stroke(width=0.5)
        self.play(MoveToTarget(face), FadeIn(brace_text, 0.5 * DOWN), Write(sphere_mesh, run_time=2, stroke_width=1), FadeIn(sphere))

        def update_theta_ring(ring):
            theta = get_theta()
            phi = angle_of_vector([*get_normal()[:2], 0])
            ring.set_width(max(2 * 1.01 * math.sin(theta), 0.001))
        ...
        loose_sum = OldTex('\\sum_{\\theta \\in [0, \\pi]}', 'P(\\theta)', '\\cdot ', '|\\cos(\\theta)|s^2')
        ...
        for d_theta in (*[-0.2] * 10, *[0.2] * 10):
            ...
            self.wait(0.25)
        self.play(Write(loose_sum.get_part_by_tex('P(\\theta)')), FadeIn(prob_words, 0.5 * DOWN), FadeOut(finite_words), ApplyMethod(frame.set_x, 1, run_time=2))
        update_theta_ring(theta_ring)
        self.add(theta_ring, sphere)
        self.play(Rotate(face, TAU, OUT, about_point=fc, run_time=4), ShowCreation(theta_ring, run_time=4))
        theta_ring.add_updater(update_theta_ring)
        self.wait()
        self.play(FadeTransform(shadow_area[1].copy(), loose_sum.get_part_by_tex('cos')), Write(loose_sum.get_part_by_tex('\\cdot')), FadeOut(prob_words, 0.5 * DOWN))
        self.wait(2)
        self.play(Write(loose_sum[0], run_time=2), run_time=3)
        ...
        face.clear_updaters()
        face.add_updater(lambda f, dt: f.rotate(angle=0.25 * dt * math.cos(0.1 * (self.time - f.start_time)), axis=np.cross(get_normal(), OUT), about_point=fc).shift(fc - f[1].get_start()))
        self.play(GrowFromCenter(sum_brace), FadeIn(continuum, 0.5 * DOWN))
        self.wait(4)
        self.play(ShowCreation(zero_arrow), GrowFromPoint(zero, zero_arrow.get_start()))
        self.wait(2)
        inf_sum_group = VGroup(nonsense_brace, nonsense, sum_brace, continuum, zero_arrow, zero, loose_sum)
        top_part = inf_sum_group[:2]
        top_part.set_opacity(0)
        self.play(inf_sum_group.animate.to_corner(UR), FadeOut(VGroup(brace, brace_text, shadow_area[1])), run_time=2)
        top_part.set_fill(opacity=1)
        self.play(GrowFromCenter(nonsense_brace), Write(nonsense))
        self.wait(10)
        integral = OldTex('\\int_0^\\pi ', 'p(\\theta)', '\\cdot ', '|\\cos(\\theta)| s^2', 'd\\theta')
        ...
        self.play(TransformMatchingShapes(loose_sum[0], integral[0], fade_transform_mismatches=True))
        ...
        self.play(FadeOut(sphere), sphere_mesh.animate.set_stroke(opacity=0.25), FadeIn(circle), theta_ring.animate.set_stroke(width=1), frame.animate.reorient(-6, 87).set_height(4), integral.animate.set_height(0.5).set_opacity(0).to_corner(UR), run_time=2)
        self.remove(integral)

        def get_tick_marks(theta_samples, tl=0.05):
            return VGroup(*(Line((1 - tl / 2) * p, (1 + tl / 2) * p).shift(fc) for theta in theta_samples for p in [np.array([math.sin(theta), 0, math.cos(theta)])])).set_stroke(YELLOW, 1)
        factor = 1
        ...
        self.play(ShowIncreasingSubsets(tick_marks[:-1]), UpdateFromAlphaFunc(face, lambda f, a: set_theta(face, theta_samples[int(a * (len(theta_samples) - 2))])), run_time=4)
        self.add(tick_marks)
        self.wait(2)
        tsi = factor * 6
        ...
        self.play(Write(dt_brace), Write(dt_label), run_time=1)
        sphere.set_opacity(0.1)
        self.play(frame.animate.reorient(10, 70), Rotate(face, -get_theta() + theta_samples[tsi], UP, about_point=fc), sphere_mesh.animate.set_stroke(opacity=0.5), FadeIn(sphere), run_time=3)
        frame.add_updater(update_frame)
        self.wait()

        def get_band(index):
            band = Sphere(u_range=(0, TAU), v_range=theta_samples[index:index + 2], prefered_creation_axis=1)
        ...
        self.play(ShowCreation(band), Rotate(face, dtheta, UP, about_point=fc), run_time=3)
        ...
        for dot in glow_dots:
            face.apply_matrix(rotation_between_vectors(get_normal(), dot.get_center() - fc), about_point=fc)
            self.add(dot)
        ...
        self.wait()
        frame.clear_updaters()
        self.play(frame.animate.reorient(-7.5, 78), sphere_mesh.animate.set_stroke(opacity=0.2), band.animate.set_opacity(0.2))
        ...
        self.play(Write(one))
        self.wait()
        self.play(TransformFromCopy(normal_vect, radial_line), FadeTransform(one.copy(), r_label))
        self.wait()
        ...
        self.play(frame.animate.reorient(10, 60))
        theta_ring.update()
        self.play(ShowCreation(theta_ring), Rotate(face, TAU, OUT, about_point=fc), FadeIn(circ_label, 0.5 * DOWN, rate_func=squish_rate_func(smooth, 0, 0.5)), GrowFromCenter(circ_brace), Write(circ_words), run_time=3)
        self.wait()
        ...
        self.wait(3)
        prob = OldTex('P(\\text{Vector} \\text{ in } \\text{Band})', '=', '{2\\pi \\sin(\\theta) \\Delta\\theta', '\\over', ' 4\\pi}', tex_to_color_map={'\\text{Vector}': GREY_B, '\\text{Band}': YELLOW})
        prob.fix_in_frame()
        prob.to_edge(RIGHT)
        prob.set_y(1)
        prob.set_backstroke()
        ...
        self.play(FadeTransform(band_area, prob.get_part_by_tex('sin'), remover=True), MoveToTarget(area_question), FadeIn(prob[:i]), FadeIn(prob[i + 1:]), FadeIn(numer_rect), *map(FadeOut, [approx, circ_brace, circ_words]), frame.animate.set_x(1.5))
        self.add(prob)
        self.remove(band_area)
        self.wait()
        self.play(ShowCreation(denom_rect), FadeIn(denom_label, 0.5 * DOWN))
        sc = sphere.copy().flip(UP).scale(1.01).set_color(BLUE, 0.5)
        self.add(sc, sphere_mesh)
        self.play(ShowCreation(sc), run_time=3)
        self.play(FadeOut(sc))
        self.wait()
        sphere_group = Group(sphere, sphere_mesh, theta_ring, band, circle, radial_line, sin_label, one, tick_marks, dt_brace, dt_label)
        average_eq = OldTex('\\text{Average shadow} \\\\', '\\sum_{\\theta}', '{2\\pi', '\\sin(\\theta)', ' \\Delta\\theta', '\\over', ' 4\\pi}', '\\cdot', '|\\cos(\\theta)|', 's^2')
        ...
        self.play(FadeIn(average_eq[:1]), FadeIn(prob_rect), prob[:5].animate.match_width(prob_rect).next_to(prob_rect, DOWN, buff=0.15), FadeTransform(prob[-3:], new_prob), *map(FadeOut, [prob[5], numer_rect, denom_rect, area_question, denom_label]))
        self.wait()
        self.play(FadeOut(sphere_group), FadeIn(average_eq[-3:]), UpdateFromAlphaFunc(face, lambda f, a: f[0].set_fill(opacity=0.5 * a)))
        self.wait()
        ...
        self.play(FadeIn(average_eq[1]), UpdateFromAlphaFunc(face, lambda f, a: f[0].set_fill(opacity=0.5 * (1 - a))), FadeIn(sphere), FadeIn(tick_marks), FadeIn(sphere_mesh), LaggedStartMap(FadeIn, bands, rate_func=there_and_back, lag_ratio=0.5, run_time=8, remover=True))
        average2 = OldTex('{2\\pi', '\\over', '4\\pi}', 's^2', '\\sum_{\\theta}', '\\sin(\\theta)', '\\Delta\\theta', '\\cdot', '|\\cos(\\theta)|')
        ...
        self.play(FadeOut(prob_rect), FadeOut(prob[:5]), *(FadeTransform(average_eq[i], average2[j], path_arc=10 * DEGREES) for i, j in [(1, 4), (2, 0), (3, 5), (4, 6), (5, 1), (6, 2), (7, 7), (8, 8), (9, 3)]), run_time=2)
        self.play(ShowCreation(half_rect))
        self.play(FadeTransform(pre_half, half), FadeOut(half_rect))
        ...
        self.play(MoveToTarget(tail, path_arc=PI / 2))
        self.wait(2)
        ...
        self.play(VGroup(half, average2[3]).animate.next_to(integral, LEFT, SMALL_BUFF), FadeTransform(average2[4], integral), FadeTransform(tail[-1], dtheta), average_copy.animate.shift(2.5 * DOWN), frame.animate.set_phi(80 * DEGREES))
        self.wait()
        self.play(LaggedStart(ShowCreationThenFadeOut(SurroundingRectangle(average_copy[1][-3]).fix_in_frame()), ShowCreationThenFadeOut(SurroundingRectangle(dtheta).fix_in_frame()), lag_ratio=0.5))
        self.wait()
        ...
        self.play(GrowFromCenter(brace), ShowCreation(arrow), FadeIn(brace_text, lag_ratio=0.1))
        for n in range(1, 4):
            new_ticks = get_tick_marks(np.linspace(0, PI, sphere_mesh.resolution[0] * 2 ** n), tl=0.05 / n)
            self.play(ShowCreation(new_ticks), FadeOut(tick_marks), run_time=2)
            self.wait()
        ...
        self.play(LaggedStart(*map(FadeOut, [arrow, brace_text, brace, average_copy])), GrowFromCenter(face[0], remover=True), frame.animate.set_height(6).set_x(3.5), FadeIn(rect), FadeOut(tick_marks), top_line.animate.set_width(4).to_edge(UP).to_edge(RIGHT, buff=LARGE_BUFF), FadeOut(average_eq[0], UP), run_time=2)
        self.add(face, sphere)
        self.begin_ambient_rotation(face, about_point=fc, speed=0.1)
        new_lines = VGroup(OldTex('{1 \\over 2} s^2 \\cdot 2 \\int_0^{\\pi / 2} \\cos(\\theta)\\sin(\\theta)\\,d\\theta'), OldTex('{1 \\over 2} s^2 \\cdot \\int_0^{\\pi / 2} \\sin(2\\theta)\\,d\\theta'), OldTex('{1 \\over 2} s^2 \\cdot \\left[ -\\frac{1}{2} \\cos(2\\theta) \\right]_0^{\\pi / 2}'), OldTex('{1 \\over 2} s^2 \\cdot \\left(-\\left(-\\frac{1}{2}\\right) - \\left(-\\frac{1}{2}\\right)\\right)'), OldTex('{1 \\over 2} s^2'))
        ...
        annotations = VGroup(OldTexText('To avoid the annoying absolute value, just\\\\cover the northern hemisphere and double it.'), OldTexText('Trig identity: $\\sin(2\\theta) = 2\\cos(\\theta)\\sin(\\theta)$'), OldTexText('Antiderivative'), OldTexText('Try not to get lost in\\\\the sea of negatives...'), OldTexText('Whoa, that turned out nice!'))
        ...
        self.play(LaggedStartMap(FadeIn, new_lines, lag_ratio=0.7), LaggedStartMap(FadeIn, annotations, lag_ratio=0.7), run_time=5)
        self.wait(20)
        self.play(new_lines[:-1].animate.set_opacity(0.5), annotations[:-1].animate.set_opacity(0.5), ShowCreation(rect))
        self.wait(10)
...
class AskAboutAverageCosValue(AllPossibleOrientations):
    ...
    def construct(self):
        ...
        def randomly_place_vect():
            ...
            point = fc + np.array([math.sin(theta) * math.cos(phi), math.sin(theta) * math.sin(phi), math.cos(theta)])
...
class ThreeCamps(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(*(student.change('pondering', image.target) for student in students), run_time=2, lag_ratio=0.2), teacher.change('tease'))
        self.wait()
        phrases = [Text('How fun!', font_size=40), Text('Wait, what?', font_size=40), Text('Okay give\nme a sec...', font_size=35)]
        ...
        for student, phrase, mode, height in zip(reversed(students), phrases, modes, heights):
            self.play(PiCreatureSays(student, phrase, target_mode=mode, look_at=image, bubble_config={'direction': LEFT, 'width': 3, 'height': height}, bubble_type=ThoughtBubble, run_time=2))
        self.wait(4)
        ...
        self.play(LaggedStart(*(FadeOut(VGroup(student.bubble, student.bubble.content)) for student in reversed(students))), LaggedStart(*(student.change('pondering', integral) for student in students)), FadeIn(integral, UP), teacher.change('raise_right_hand', integral))
        self.play(GrowFromCenter(brace), Write(words))
        self.wait(2)
        self.play(words.animate.shift(0.75 * UP).set_opacity(0.5), FadeIn(words2, 0.2 * UP), LaggedStart(self.teacher.change('shruggie'), self.students[0].change('sassy', words2), self.students[1].change('thinking', words2), self.students[2].change('well', words2)))
        self.wait(2)
...
class ParticularValuesUnhelpfulOverlay(Scene):
    ...
    def construct(self):
        ...
        self.play(GrowFromCenter(brace), FadeIn(words, shift=0.2 * UP))
        self.wait()
        ...
        self.play(FadeIn(question), ShowCreation(arrow))
        self.wait()
        range_expr = OldTex('P(\\pi / 4 < \\theta < \\pi / 4 + \\Delta\\theta) > 0', tex_to_color_map={'\\pi / 4': YELLOW, '\\Delta\\theta': GREY_A}, font_size=40)
        ...
        self.play(FadeOut(question), FadeOut(arrow), TransformMatchingShapes(expr, range_expr), FadeTransform(brace, new_brace), FadeTransform(words, new_words))
        self.wait()
...
class SurfaceAreaOfSphere(Scene):
    ...
    def construct(self):
        ...
        equation = OldTex('\\text{Surface area} = 4\\pi R^2', tex_to_color_map={'R': YELLOW, '\\text{Surface area}': BLUE})
        equation.to_edge(UP)
        self.add(equation, sphere, sphere_mesh)
        self.play(Write(sphere_mesh, lag_ratio=0.02, stroke_width=1), ShowCreation(sphere, rate_func=squish_rate_func(smooth, 0.25, 1)), run_time=5)
        self.wait()
...
class IntegralOverlay(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(*(FlashAround(sm, time_width=3) for sm in integral[0][:0:-1]), lag_ratio=0.5, run_time=3))
        self.wait()
...
class AlicesInsights(Scene):
    ...
    def construct(self):
        ...
        kw = dict(t2c={'double cover': YELLOW, 'mean': RED, 'means': RED, 'sum': BLUE, 'Sum': BLUE, 'Average': RED})
        insights = VGroup(Text('1. The face shadows double cover the cube shadow', **kw), Text('2. Average(Sum(Face shadows)) = Sum(Average(Face shadow))', **kw), Text('3. Use a sphere to deduce the unknown constant', **kw))
...
class HalfBathedInLight(ShadowScene):
    ...
    def construct(self):
        ...
        for s, color in zip([slice(3, None), slice(0, 3)], [WHITE, GREY_D]):
            ...
            self.play(MoveToTarget(cube), rate_func=there_and_back_with_pause, run_time=3)
        self.wait(5)
        self.embed()
...
class TwoToOneCover(ShadowScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(lhs[:2]), FadeIn(lhs[-1]), Write(outline), VShowPassingFlash(outline.copy().set_stroke(YELLOW, 4), time_width=1.5))
        self.wait()
        cube.save_state()
        faces, face_shadows = self.get_faces_and_face_shadows()
        faces[:3].set_opacity(0.1)
        face_shadow_lines = VGroup(*(VGroup(*(Line(v1, v2) for v1, v2 in zip(f.get_vertices(), fs.get_vertices()))) for f, fs in zip(faces, face_shadows)))
        face_shadow_lines.set_stroke(YELLOW, 0.5, 0.5)
        self.play(Restore(lhs), FadeIn(af_label, shift=0.5 * RIGHT))
        self.play(*(LaggedStart(*(VFadeInThenOut(sm) for sm in reversed(mobject)), lag_ratio=0.5, run_time=6) for mobject in [faces, face_shadows, face_shadow_lines]))
        self.play(ApplyMethod(cube.space_out_submobjects, 1.7, rate_func=there_and_back_with_pause, run_time=8), ApplyMethod(frame.reorient, 20, run_time=8), Write(lt), Write(rhs[0]))
        self.wait(2)
        ...
        self.add(fp_shadow)
        self.play(FadeOut(cube), *map(VFadeOut, shadow), FadeOut(outline), *map(Write, face_pair))
        self.wait(1)
        self.play(Rotate(face_pair, PI / 2, DOWN, about_point=cube.get_center(), run_time=3))
        self.wait()
        self.random_toss(face_pair, about_point=cube.get_center(), run_time=6)
        fp_shadow.clear_updaters()
        self.play(FadeIn(cube), *map(VFadeIn, shadow), FadeOut(face_pair, scale=0), FadeOut(fp_shadow, scale=0))
        self.add(shadow)
        new_expression = self.get_top_expression(' = ', '$\\displaystyle \\frac{1}{2}$')
        new_expression.fix_in_frame()
        eq_half = VGroup(new_expression.get_part_by_tex('='), new_expression.get_part_by_tex('frac'))
        cube.save_state()
        cube.generate_target(use_deepcopy=True)
        cube.target.clear_updaters()
        ...
        self.play(FadeOut(lt, UP), Write(eq_half), ReplacementTransform(rhs, new_expression[-len(rhs):]), ReplacementTransform(lhs, new_expression[:len(lhs)]))
        self.remove(ineq)
        ...
        for part in z_sorted_faces:
            ...
            anims.append(AnimationGroup(VFadeInThenOut(pc), VFadeInThenOut(pc_shadow), VFadeInThenOut(lines)))
        ...
        self.wait()
        self.play(Rotate(dot, TAU, about_point=ORIGIN, run_time=6))
        self.wait()
        for arrow in arrows:
        ...
        outline.update()
        line.clear_updaters()
        self.play(FadeOut(dot), FadeOut(line), ShowCreation(outline), VFadeIn(area_label))
        ...
        cube.save_state()
        self.remove(cube)
        self.play(*(f.animate.set_fill(opacity=0) for f in cube), FadeOut(outline), FadeOut(area_label), Write(face), FadeIn(face_shadow), FadeIn(face_shadow_area))
        self.wait(2)
        self.play(Restore(cube), *map(FadeOut, (face, face_shadow, face_shadow_area)), *map(FadeIn, (outline, area_label)))
        for x in range(2):
        ...
        cube.save_state()
        sff = 1.5
        self.play(VFadeOut(outline), VFadeOut(area_label), cube.animate.space_out_submobjects(sff))
        for x in range(3):
        ...
        def isolate_face_anims(i, color=YELLOW):
            return (shadow.animate.set_fill(interpolate_color(color, BLACK, 0.75)), *(f.animate.set_fill(color if f is cube[i] else BLUE, 0.75 if f is cube[i] else 0) for f in cube))
        ...
        for i, color in ((4, GREEN), (3, RED)):
            self.play(*isolate_face_anims(i, color))
            tour_orientations()
        cube.update()
        self.play(*(f.animate.set_style(**cube_style) for f in cube), shadow.animate.set_fill(interpolate_color(BLUE, BLACK, 0.85)), VFadeIn(outline), VFadeIn(area_label))
        self.add(cube)
        self.add(cube)
        self.begin_ambient_rotation(cube, speed=0.4)
        self.wait(20)
    ...
    def get_top_expression(self, *mid_tex, n_faces=6):
        t2c = {'Shadow': GREY_B, 'Cube': BLUE_D, 'Face$_j$': YELLOW}
        ineq = OldTexText('Area(Shadow(Cube))', *mid_tex, ' $\\displaystyle \\sum_{j=1}^' + f'{{{n_faces}}}' + '$ ', 'Area(Shadow(Face$_j$))', tex_to_color_map=t2c, isolate=['(', ')'])
...
class ConvexityPrelude(Scene):
    ...
    def construct(self):
        ...
        beam = VGroup(Line(points[0] + 3 * UP, points[0], stroke_width=3), Line(points[0], points[1], stroke_width=2), Line(points[1], points[1] + 3 * DOWN, stroke_width=1))
        ...
        self.add(square)
        self.add(words)
        self.play(ShowCreation(beam), FadeIn(arrows, lag_ratio=0.7), FadeIn(dots, lag_ratio=0.7))
        self.wait()
        ...
        convex_shapes = VGroup(square.target, RegularPolygon(5, color=TEAL), Rectangle(2, 1, color=TEAL_E), RegularPolygon(6, color=GREEN), Circle(color=GREEN_B))
        ...
        self.play(MoveToTarget(square), FadeIn(convex_shapes[1:], lag_ratio=0.5), FadeTransform(beam, v_line), ShowCreation(h_line), FadeIn(convex_title), LaggedStart(*map(FadeOut, (dots, arrows, words, question1, question2))))
        self.wait()
        ...
        self.play(Write(non_convex_title, run_time=1), LaggedStartMap(FadeIn, nc_shapes))
        self.wait()
        self.embed()
...
class DefineConvexity(Scene):
    ...
    def construct(self):
        ...
        set_color = BLUE
        line_color = GREEN
        title = Text(definition, t2c={'convex': set_color, 'line connecting any\ntwo points': line_color}, t2s={'convex': ITALIC})
        title.to_edge(UP)
        ...
        self.add(title)
        self.play(title.get_part_by_text('A set is convex').animate.set_opacity(1), Write(shape))
        self.wait()
        ...
        dots.set_radius(0)
        dots.set_opacity(0)
        self.play(title[definition.index('if'):definition.index('is', 16)].animate.set_opacity(1), Restore(dots))
        self.play(ShowCreation(line))
        self.wait()
        self.play(title[definition.index('is', 16):].animate.set_opacity(1))
        dots.add_updater(lambda m: m.set_points(line.get_start_and_end()))

        def show_sample_lines(n=5):
            for x in range(5):
                self.play(line.animate.put_start_and_end_on(shape.pfp(random.random()), shape.pfp(random.random())).scale(random.random(), about_point=shape.get_center()))
                self.wait(0.5)
        show_sample_lines()
        ...
        self.play(Transform(shape, pi, path_arc=PI / 2), MoveToTarget(line), run_time=2)
        self.play(FadeIn(not_convex_label, scale=2))
        self.wait()
        shape.insert_n_curves(80)
        self.play(Transform(shape, letter_c), line.animate.put_start_and_end_on(*(letter_c.pfp(a) for a in (0, 0.5))), run_time=2)
        self.play(UpdateFromAlphaFunc(line, lambda l, a: l.put_start_and_end_on(letter_c.pfp(0.4 * a), l.get_end()), run_time=6))
        self.wait()
        ...
        self.play(FadeTransform(shape, polygon), line.animate.put_start_and_end_on(polygon.get_vertices()[1], polygon.get_vertices()[-1]), TransformMatchingShapes(not_convex_label.copy(), convex_label), run_time=2)
        self.wait()
        ...
        self.play(MoveToTarget(polygon), TransformMatchingShapes(convex_label, not_convex_label))
        self.wait()
        ...
        self.play(ShowCreation(beam), VShowPassingFlash(flash_line), run_time=1)
        self.wait()
...
class NonConvexDoughnut(ShadowScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(dot), ShowCreation(line), ShowCreation(line_shadow), ApplyMethod(frame.reorient, 20, run_time=7))
        self.wait()
...
class ShowGridSum(TwoToOneCover):
    ...
    def construct(self):
        ...
        t2c = {f'\\text{{{shape_name}}}': BLUE, '\\text{F}_j': YELLOW, '\\text{Face}': YELLOW, '\\text{Total}': GREEN, 'S': GREY_B, '{c}': RED}
        ...
        def get_s_f_term(i='i', j='j'):
            result = OldTex(f'S\\big(R_{{{i}}}', '(', '\\text{F}_{' + str(j) + '}', ')', '\\big)', **kw)
        ...
        for mob, vect in ((lhs, RIGHT), (summand, LEFT)):
            ...
            self.play(GrowFromCenter(mob.brace), FadeIn(mob.alt1, shift=0.5 * DOWN))
            self.wait()
            self.play(FadeOut(mob.brace, scale=0.5), FadeOut(mob, shift=0.5 * UP), mob.alt1.animate.move_to(mob, vect))
            mob.alt2.move_to(mob, vect)
        self.wait()
        for mob in (lhs, summand):
            self.play(TransformMatchingShapes(mob.alt1, mob.alt2))
            self.wait()
        lhss = VGroup(get_s_cube_term(1), get_s_cube_term(2), get_s_cube_term(3), OldTex('\\vdots'), get_s_cube_term('n'))
        buff = 0.6
        lhss.arrange(DOWN, buff=buff)
        lhss.move_to(lhs.alt2, UP)
        self.remove(lhs.alt2)
        self.play(LaggedStart(*(ReplacementTransform(lhs.alt2.copy(), target) for target in lhss)), frame.animate.set_height(10, about_edge=UP), run_time=2)
        ...
        total.match_x(lhss)
        mean_sa = OldTex(f'S(\\text{{{shape_name}}})', '=', '\\frac{1}{n}', '\\sum_{i=1}^n S\\big(R_i(' + f'\\text{{{shape_name}}})\\big)', **kw)
        ...
        self.play(Write(lhss_rect), Write(sigma))
        self.wait()
        self.add(corner_rect, sigma)
        self.play(FadeIn(corner_rect), *(FadeTransform(term.copy(), sum_part[1:]) for term in lhss), Restore(sigma))
        self.play(Write(mean_sa.get_part_by_tex('frac')))
        self.wait()
        self.play(FadeIn(mean_sa.slice_by_tex(None, 'frac')))
        self.wait()
        sf = get_s_f_term
        grid_terms = [[sf(1, 1), sf(1, 2), OldTex('\\dots'), sf(1, n_faces)], [sf(2, 1), sf(2, 2), OldTex('\\dots'), sf(2, n_faces)], [sf(3, 1), sf(3, 2), OldTex('\\dots'), sf(3, n_faces)], [Tex('\\vdots'), OldTex('\\vdots'), OldTex('\\ddots'), OldTex('\\vdots')], [sf('n', 1), sf('n', 2), OldTex('\\dots'), sf('n', n_faces)]]
        ...
        self.wait()
        self.play(ShowCreation(arrows[0]), FadeIn(arrow_labels[0]), FadeIn(mean_face_words, DR))
        self.wait()
        self.play(mean_face_words.animate.scale(0.7).next_to(mean_face, DOWN, MED_LARGE_BUFF), FadeIn(mean_face, scale=2))
        self.wait()
        for i in [1, 2]:
            self.play(FadeOut(cols[i - 1]), FadeIn(cols[i]), *(ReplacementTransform(group[i - 1], group[i]) for group in (col_rects, arrows, arrow_labels)))
            self.wait()
        self.wait()
        frame.generate_target()
        ...
        self.play(LaggedStart(VGroup(corner_rect, mean_sa).animate.scale(1.25).move_to(frame.target, UL).shift(0.1 * DR), MoveToTarget(frame), FadeOut(mean_face_words), FadeOut(mean_face), grid.animate.set_opacity(1), *(FadeOut(group[-1]) for group in (cols, col_rects, arrows, arrow_labels)), run_time=3))
        mean_sa.refresh_bounding_box()
        rhss = VGroup(OldTex('=', '\\frac{1}{2}', '\\sum_{j=1}^' + f'{{{n_faces}}}', ' S(\\text{F}_j})', **kw), OldTex('=', '\\frac{1}{2}', '\\sum_{j=1}^' + f'{{{n_faces}}}', ' {c}', '\\cdot ', 'A(\\text{F}_j)', **kw), OldTex('=', '\\frac{1}{2}', '{c}', '\\cdot ', '(\\text{Surface area})', **kw))
        ...
        corner_rect.target.set_width(frame.get_width() - 0.2, stretch=True, about_edge=LEFT)
        grid_rect = SurroundingRectangle(grid, buff=SMALL_BUFF)
        grid_rect.set_stroke(YELLOW, 1)
        grid_rect.set_fill(YELLOW, 0.25)
        rects = VGroup(SurroundingRectangle(mean_sa.slice_by_tex('frac')), SurroundingRectangle(rhss[0][1:]))
        for rect in rects:
        ...
        self.wait()
        self.play(Write(rects[0]), Write(rows_first))
        self.wait()
        self.play(Write(rects[1]), Write(cols_first))
        self.wait()
        self.play(LaggedStart(*map(FadeOut, (*rects, rows_first, cols_first))))
        self.play(TransformMatchingShapes(rhss[0].copy(), rhss[1]))
        self.wait()
        self.play(FadeTransform(rhss[1].copy(), rhss[2]))
        self.wait()
        key_part = rhss[2][1:]
        final_rect = SurroundingRectangle(key_part)
        final_rect.set_stroke(YELLOW, 1)
        self.play(corner_rect.animate.set_stroke(width=0).scale(1.1), FlashAround(key_part, time_width=1.5), FadeIn(final_rect), run_time=2)
...
class ComplainAboutProgress(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says(OldTexText("Wait, is that all\\\\we've accomplished?"), target_mode='angry')
        self.play_student_changes('guilty', 'erm', look_at=self.students[2].eyes, added_anims=[self.teacher.change('guilty')])
        self.wait(4)
...
class SupposedlyObviousProportionality(ShadowScene):
    ...
    def construct(self):
        ...
        c_arrow = Arrow(equation.get_part_by_tex('{c}'), question.get_corner(UL))
        ...
        self.play(TransformMatchingShapes(question, obvious_words), ReplacementTransform(c_arrow, obvious_arrow))
        self.wait()
        cube.clear_updaters()
        ...
        for face in cube[3:]:
            ...
            face.label = lc
            face_labels.add(lc)
        self.play(Write(shadow_label), Write(face_labels), run_time=2)
        self.wait()
        scalars = [cube, face_labels, shadow_label]
        self.play(*(mob.animate.scale(0.5) for mob in scalars), rate_func=there_and_back, run_time=3)
        self.play(*(mob.animate.stretch(2, 0) for mob in scalars), rate_func=there_and_back, run_time=3)
        self.wait()
        ...
        self.play(FadeIn(no_words, 0.25 * DOWN), FadeOut(shadow_label), FadeOut(face_labels))
        self.play(light.animate.next_to(cube, OUT + RIGHT, 2), run_time=4, rate_func=rush_from)
        for s in (1.5, 0.25, 2 / 0.75):
            self.play(cube.animate.scale(s), run_time=2)
        cube.add_updater(lambda m: self.sort_to_camera(m))
        self.begin_ambient_rotation(cube)
        self.play(light.animate.next_to(cube, OUT, 50), LaggedStart(*map(FadeOut, (no_words, obvious_words, obvious_arrow))), run_time=3)
        self.wait(33)
...
class WhatIsC(Scene):
    ...
    def construct(self):
        words = OldTexText('What is $c$?!', tex_to_color_map={'$c$': RED}, font_size=72)
        self.play(Write(words))
...
class BobsFinalAnswer(Scene):
    ...
    def construct(self):
        answer = OldTex('S(\\text{Cube})', '=', '\\frac{1}{2}', '\\cdot', '{\\frac{1}{2}}', '(\\text{Surface area})', '=', '\\frac{1}{4} \\big(6s^2\\big)', '=', '\\frac{3}{2} s^2', tex_to_color_map={'\\text{Cube}': BLUE, '{\\frac{1}{2}}': RED})
        ...
        for m1, m2 in ((eq1, eq2), (eq2, eq3)):
            self.play(FadeIn(m2[len(m1):]), m1.animate.move_to(m2, LEFT))
            self.remove(m1)
            self.add(m2)
            self.wait()
        rect = SurroundingRectangle(eq3[-1])
        rect.set_stroke(YELLOW, 2)
        self.play(ShowCreation(rect), FlashAround(eq3[-1], stroke_width=5, time_width=1.5, run_time=1.5))
        self.wait()
...
class ShowSeveralConvexShapes(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(*(LaggedStartMap(Write, ex, run_time=1) if isinstance(ex[0], VMobject) else ShowCreation(ex[0]) for ex in examples), lag_ratio=0.5, run_time=8))
        self.wait()
...
class KeyResult(Scene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(cube_underline), FadeIn(general_words), ShowCreation(general_arrow), FadeTransformPieces(eq1, eq2))
        self.wait()
        const_rect = SurroundingRectangle(VGroup(eq.get_part_by_tex('frac'), eq.get_part_by_tex('{c}')), buff=SMALL_BUFF)
        ...
        self.play(ShowCreation(const_rect), FadeIn(const_words), ShowCreation(const_arrow))
        self.wait()
...
class ShadowsOfDodecahedron(ShadowScene):
    ...
    def construct(self):
        ...
        self.play(VFadeIn(outline), VFadeIn(area))
        dot = GlowDot(0.5 * DR)
        ...
        for n in range(8):
            self.play(dot.animate.move_to(midpoint(outline.get_center(), outline.pfp(random.random()))), run_time=5)
...
class DodecahedronFaceSum(Scene):
    ...
    def construct(self):
        expr = OldTexText('Area(Shadow(Dodecahedron))', '=', '$\\displaystyle \\frac{1}{2}$', ' $\\displaystyle \\sum_{j=1}^{12}$ ', 'Area(Shadow(Face$_j$))', tex_to_color_map={'Shadow': GREY_B, 'Dodecahedron': BLUE_D, 'Face$_j$': YELLOW})
...
class SphereShadow(ShadowScene):
    ...
    def get_solid(self):
        ep = 0.001
        sphere = Sphere(radius=1.5, u_range=(ep, TAU - ep), v_range=(ep, PI - ep))
...
class SwapConstantForFourth(Scene):
    ...
    def construct(self):
        ...
        self.add(eq)
        self.wait()
        self.play(FadeOut(parts, UP), FadeIn(fourth, UP), eq.slice_by_tex('\\cdot').animate.next_to(fourth, RIGHT))
        self.wait()

class ButSpheresAreSmooth(TeacherStudentsScene):

    def construct(self):
        self.student_says(OldTexText("But spheres don't\\\\have flat faces!"), target_mode='angry', index=2, added_anims=[self.teacher.change('guilty')])
        self.play_student_changes('erm', 'hesitant', 'angry', look_at=self.screen)
        self.wait(6)

class RepeatedRelation(Scene):

    def construct(self):
        relation = VGroup(Text('Average shadow'), OldTex('=').rotate(PI / 2), OldTex('\\frac{1}{2}', 'c', '\\cdot (', '\\text{Surface area}', ')'))
        ...
        self.wait()
        limit = OldTex('\\lim_{|F| \\to 0}', '\\left(', '{\\text{Average shadow}', '\\over ', '\\text{Surface area}}', '\\right)', '=', '\\frac{1}{2}', '{c}')
        ...
        self.wait()
        self.play(limit.slice_by_tex('=').animate.next_to(new_rhs, RIGHT), GrowFromCenter(new_rhs))
        self.wait()

class SimpleCross(Scene):

    def construct(self):
        lines = VGroup(Line(UP, DOWN).set_height(FRAME_HEIGHT), Line(LEFT, RIGHT).set_width(FRAME_WIDTH))
...
class PopularizaitonVsDoing(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(TransformFromCopy(popular.get_part_by_text('math'), doing.get_part_by_text('math')), Write(doing[:len('Doing')]))
        self.wait()
        width = 8
        ...
        bar_group = VGroup(bar, left_bar, right_bar, left_brace, right_brace, left_label, right_label)

        def set_bar_alpha(alpha, **kwargs):
            self.play(left_bar.animate.set_width(alpha * width, about_edge=LEFT, stretch=True), right_bar.animate.set_width((1 - alpha) * width, about_edge=RIGHT, stretch=True), **kwargs)
...
class MultipleMathematicalBackgrounds(TeacherStudentsScene):
    ...
    def construct(self):
        self.remove(self.background)
        labels = VGroup(OldTexText('$\\le$ High school'), OldTexText('$\\approx$ Undergrad'), OldTexText('$\\ge$ Ph.D.'))
        ...
        self.add(words)
        self.play(self.teacher.change('raise_right_hand'), self.change_students('pondering', 'thinking', 'pondering', look_at=self.teacher.eyes))
        self.play(LaggedStartMap(ShowCreation, lines, lag_ratio=0.5), LaggedStartMap(FadeIn, labels, lag_ratio=0.5))
        self.wait(3)
        self.play(self.teacher.change('dejected').look(UP), self.change_students('hesitant', 'well', 'thinking'), LaggedStartMap(FadeOut, lines, scale=0.5), FadeOut(words, DOWN))
        self.wait(4)
        kw = {'font_size': 30}
        methods = VGroup(OldTexText('Calculus\\\\primer', **kw), OldTexText('Quickly show\\\\key steps', **kw), OldTexText('Describe as a\\\\measure on SO(3)', **kw))
        new_lines = VGroup()
        ...
        for method, label, color in zip(methods, labels, colors):
            ...
            new_lines.add(line)
        self.play(self.teacher.change('raise_right_hand'), self.change_students('erm', 'pondering', 'thinking', look_at=self.students.get_center() + 4 * UP), LaggedStartMap(FadeIn, methods, lag_ratio=0.5), LaggedStartMap(ShowCreation, new_lines, lag_ratio=0.5))
        self.wait(4)
...
class CleverProofExample(Scene):
    ...
    def construct(self):
        ...
        self.play(MoveToTarget(initial_sum), FadeIn(eq), MoveToTarget(tripple_tris), FadeTransform(tripple_tris.copy(), final_tri))
        self.wait()
        ...
        self.play(MoveToTarget(final_tri), Write(final1))
        self.wait()
        self.play(FadeOut(final1, UP), FadeIn(final2, UP))
        self.wait()
        self.play(FadeOut(final2, UP), FadeIn(final3, UP))
    ...
    def get_triangle_sums(self):
        ...
        np1 = OldTex('(2n + 1)')
        dots = OldTex('\\dots')
        tri1 = VGroup(Integer(1).replicate(1), Integer(2).replicate(2), Integer(3).replicate(3), VGroup(dl_dots, blank.copy(), blank.copy(), dr_dots), VGroup(n.copy(), n.copy(), dots, n.copy(), n.copy()))
        tri2 = VGroup(n.replicate(1), VGroup(dl_dots, n).copy(), VGroup(Integer(3), blank, dr_dots).copy(), VGroup(Integer(2), Integer(3), dots, n).copy(), VGroup(Integer(1), Integer(2), Integer(3), dots, n).copy())
        tri3 = VGroup(n.replicate(1), VGroup(n, dr_dots).copy(), VGroup(dl_dots, blank, Integer(3)).copy(), VGroup(n, dots, Integer(3), Integer(2)).copy(), VGroup(n, dots, Integer(3), Integer(2), Integer(1)).copy())
        sum_tri = VGroup(np1.replicate(1), np1.replicate(2), np1.replicate(3), VGroup(dl_dots, *blank.replicate(6), dr_dots).copy(), VGroup(np1.copy(), np1.copy(), dots.copy(), np1.copy(), np1.copy()))
        ...
        tris.set_width(6)
        plusses = VGroup(OldTex('+').move_to(tris[:2]), OldTex('+').move_to(tris[1:]))
        parens = OldTex('()')[0]
        parens.stretch(2, 1)
...
class ListernerEmail(Scene):
    ...
    def construct(self):
        ...
        for highlight, phrase in zip(highlights, phrases):
            hlines.add(VGroup(DashedLine(highlight.get_corner(UR), phrase.get_corner(UL), buff=0.1), DashedLine(highlight.get_corner(DR), phrase.get_corner(DL), buff=0.1)))
        hlines.set_stroke(YELLOW, 1)
        for i in range(3):
            self.play(FadeIn(highlights[i]), *map(ShowCreation, hlines[i]), GrowFromPoint(phrases[i], highlights[i].get_right()))
            self.wait(2)
        self.embed()
...
class FamousMathematicians(Scene):
    ...
    def construct(self):
        images = Group(ImageMobject('Newton'), ImageMobject('Euler'), ImageMobject('Gauss'), ImageMobject('Fourier'), ImageMobject('Riemann_cropped'), ImageMobject('Cauchy'), ImageMobject('Noether'), ImageMobject('Ramanujan'))
        names = VGroup(Text('Isaac Newton'), Text('Leonhard Euler'), Text('Carl Friedrich Gauss'), Text('Joseph Fourier'), Text('Bernhard Riemann'), Text('Augustin Cauchy'), Text('Emmy Noether'), Text('Srinivasa Ramanujan'))
        im_groups = Group()
        ...
        self.wait()
        self.play(im_groups[0].animate.set_height(6).center().to_edge(LEFT), LaggedStart(*(FadeOut(mob, DR) for mob in (*im_groups[1:], dots)), lag_ratio=0.25), run_time=2)
        self.wait()
...
class AmbientHourglass(ShadowScene):
    ...
    def construct(self):
        ...
        for x in range(30):
            ...
            self.wait()
        self.begin_ambient_rotation(solid_group, speed=0.5, initial_axis=[1, 0, 1])
        self.wait(35)
...
class QuantifyConvexity(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(question, 0.5 * DOWN), ShowCreation(arrow))
        self.wait()
        ...
        self.play(nonconvex.animate.next_to(double_arrow, LEFT), Write(double_arrow), FadeIn(convex, shift=0.25 * RIGHT), Uncreate(arrow), FadeOut(question, 0.5 * DOWN))
        self.wait()
        interval = UnitInterval(width=7)
        interval.add_numbers()
        interval.to_corner(UL, buff=LARGE_BUFF)
        self.play(FadeTransform(double_arrow, interval), convex.animate.scale(0.5).next_to(interval.n2p(1), UP), nonconvex.animate.scale(0.5).next_to(interval.n2p(0), UP))
        self.wait()
...
class GoalsOfMath(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        aq.set_color(BLUE)
        dni = Text('develop new ideas', t2c={'new ideas': YELLOW}, t2s={'new ideas': ITALIC})
        dni.move_to(aq, LEFT)
        self.play(self.teacher.change('raise_right_hand', words), self.change_students(*3 * ['pondering'], look_at=words), Write(words))
        self.wait(2)
        self.add(aq, self.teacher)
        self.play(aq.animate.shift(0.5 * DOWN).set_opacity(0.2), Write(dni), self.teacher.change('well', words), self.change_students(*3 * ['thinking'], look_at=words))
        self.wait(3)

class InfatuationWithGenerality(TeacherStudentsScene):

    def construct(self):
        self.student_says(OldTexText('Why are mathematicians\\\\obsessed with abstractions?'), index=0, added_anims=[self.students[1].change('tease'), self.students[2].change('pondering')])
        self.play(self.teacher.change('well'))
        self.wait(6)
...
class ByLine(Scene):
    ...
    def construct(self):
        lines = VGroup(OldTexText('Artwork by\\\\', 'Kurt Bruns'), OldTexText('Music by\\\\', 'Vince Rubinetti'), OldTexText('Other stuff\\\\', 'Grant Sanderson'))
        for line in lines:
