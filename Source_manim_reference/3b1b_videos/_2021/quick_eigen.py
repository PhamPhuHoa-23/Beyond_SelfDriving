...
def det_path_anim(matrix, run_time=2):
    path = VMobject()
    path.set_points_smoothly([matrix.get_corner(UL), *[matrix.get_entries()[i].get_center() for i in [0, 3, 1, 2]], matrix.get_corner(DL)])
...
def get_mod_mat(matrix, h_buff=1.3, v_buff=0.8, t2c={}):
    ...
    mod_mat = Matrix([[matrix[0][0] + ' - \\lambda', matrix[0][1]], [matrix[1][0], matrix[1][1] + ' - \\lambda']], element_to_mobject_config={'tex_to_color_map': t2c}, h_buff=h_buff, v_buff=v_buff)
    return mod_mat
...
class Thumbnail(Scene):
    ...
    def construct(self):
        ...
        rect2.move_to(rect)
        dashed_rects = VGroup(DashedVMobject(rect.insert_n_curves(100), num_dashes=50), DashedVMobject(rect2.insert_n_curves(100), num_dashes=50))
        self.add(dashed_rects)
        answer = OldTex('\\lambda_1, \\lambda_2 = {3} \\pm \\sqrt{\\,{3}^2 - {5}} = 5, 1', tex_to_color_map={'{3}': MEAN_COLOR, '{p}': PROD_COLOR})
        ...
        self.add(not_this)
        kw = {'tex_to_color_map': {'\\lambda_1': EIGEN_COLORS[0], '\\lambda_2': EIGEN_COLORS[1], 'M': YELLOW}}
        facts = VGroup(OldTex('\\text{det}(M) = \\,\\, \\lambda_1 \\cdot \\lambda_2', **kw), OldTex('\\text{tr}(M) = \\lambda_1 + \\lambda_2', **kw))
...
class Assumptions(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(PiCreatureSays(self.teacher, OldTexText("I'm assuming you know\\\\ what eigenvalues are.")), self.change_students('erm', 'happy', 'tease', look_at=ORIGIN), run_time=2)
        ...
        self.play(RemovePiCreatureBubble(self.teacher, target_mode='raise_right_hand'), FadeIn(eigen_expression, UP), self.students[1].change('hesitant'), self.students[2].change('sassy'))
        self.play(ShowCreation(cross), FadeIn(words, 0.25 * UP), self.teacher.change('tease', cross), self.students[1].change('pondering', cross), self.students[2].change('hesitant', cross))
        self.wait(3)
...
class PreviousVideoWrapper(Scene):
    ...
    def construct(self):
        ...
        self.wait(2)
        self.play(FadeOut(title, UP), screen.animate.set_height(7).center())
        self.wait()

class GoalOfRediscovery(TeacherStudentsScene):

    def construct(self):
        self.play(PiCreatureSays(self.teacher, OldTexText('The goal is\\\\rediscovery')), self.change_students('happy', 'tease', 'hooray', run_time=1.5))
        self.wait(2)
        recap_words = Text('Quick recap')
        recap_words.move_to(self.screen, UP)
        self.play(RemovePiCreatureBubble(self.teacher, target_mode='raise_right_hand', look_at=recap_words), self.change_students('pondering', 'hesitant', 'pondering', look_at=recap_words), GrowFromPoint(recap_words, self.teacher.get_corner(UL)))
        self.wait(4)
...
class VisualizeEigenvector(Scene):
    ...
    def construct(self):
        ...
        bases = VGroup(Vector(RIGHT, fill_color=GREEN_E), Vector(UP, fill_color=RED_E))
        ...
        self.wait()
        self.play(plane.animate.apply_matrix(matrix), bases[0].animate.put_start_and_end_on(ORIGIN, plane.c2p(3, 0)), bases[1].animate.put_start_and_end_on(ORIGIN, plane.c2p(1, 2)), vector.animate.scale(2, about_point=ORIGIN), MoveToTarget(array), GrowFromPoint(two_times, array.get_left() + SMALL_BUFF * LEFT), run_time=3, path_arc=0)
        self.wait()
        self.remove(mat_mob)
        self.remove(array)
...
class EigenvalueEquationRearranging(Scene):
    ...
    def construct(self):
        ...
        kw = {'tex_to_color_map': {'A': BLUE, '\\lambda': EIGEN_COLORS[1], v_tex: YELLOW, '=': WHITE}}
        lines = VGroup(OldTex('A', v_tex, '=', '\\lambda ', v_tex, **kw), OldTex('A', v_tex, '=', '\\lambda ', ' I ', v_tex, **kw), OldTex('A', v_tex, '-', '\\lambda ', ' I ', v_tex, '=', zero_tex, **kw), OldTex('(A', '-', '\\lambda ', 'I)', v_tex, '=', zero_tex, **kw), OldTex('\\text{det}', '(A', '-', '\\lambda ', 'I)', '=', '0', **kw))
        for line in lines:
        ...
        self.add(lines[0])
        self.play(LaggedStart(GrowFromCenter(mat_prod_brace), GrowFromCenter(scalar_prod_brace), lag_ratio=0.3), LaggedStart(FadeIn(mat_prod_label, 0.25 * DOWN), FadeIn(scalar_prod_label, 0.25 * DOWN), lag_ratio=0.3))
        self.wait()
        ...
        self.play(TransformMatchingTex(lines[0], lines[1]), ReplacementTransform(scalar_prod_brace, id_brace), FadeTransform(scalar_prod_label, id_label), VGroup(mat_prod_label, mat_prod_brace).animate.shift(lines[1].get_left() - lines[0].get_left()))
        self.wait()
        for shift_value, line in zip(it.count(1), lines[2:5]):
            line.shift(shift_value * 0.75 * DOWN)
        self.play(TransformMatchingTex(lines[1].copy(), lines[2], path_arc=-45 * DEGREES), FadeOut(VGroup(mat_prod_label, mat_prod_brace, id_brace, id_label), 0.5 * DOWN))
        self.wait()
        self.play(TransformMatchingShapes(lines[2].copy(), lines[3]))
        self.wait()
        ...
        self.wait()
        self.play(TransformMatchingShapes(lines[3].copy(), lines[4]), FadeOut(nz_label), FadeOut(arrow))
        self.wait()
...
class TypicalComputation(Scene):
    ...
    def construct(self):
        words, mat = task = VGroup(Text('Find the eigenvalues of ', t2c={'eigenvalues': TEAL}), IntegerMatrix([[3, 1], [4, 1]]).set_height(1))
        ...
        t0, t1, t2, t3 = terms = VGroup(det_expression[5:8], det_expression[8:9], det_expression[9:10], det_expression[10:13]).copy()
        ...
        self.play(TransformFromCopy(mat.get_brackets(), VGroup(*(det_expression[i] for i in [4, 13]))), TransformFromCopy(mat.get_entries(), VGroup(*(det_expression[i] for i in [5, 8, 9, 10]))), FadeTransform(mat.get_brackets().copy().set_opacity(0), VGroup(*(det_expression[i] for i in [0, 1, 2, 3, 14]))), run_time=1)
        self.play(Write(VGroup(*(det_expression[i] for i in [6, 7, 11, 12]))), Restore(movers))
        self.wait()
        self.add(det_expression)
        self.play(FadeIn(eq), TransformFromCopy(VGroup(t0, t3), rhs[:2]))
        ...
        top_terms = VGroup(VGroup(rhs[0][0], rhs[0][2]), VGroup(rhs[1][0], rhs[1][2]))
        ...
        self.play(LaggedStart(TransformFromCopy(eq2, eq3, **kw), Transform(rhs2[1].copy(), rhs3[6].copy(), remover=True, **kw), TransformFromCopy(rhs2[9:11], rhs3[5:], **kw), TransformFromCopy(rhs2[2:5], rhs3[2:5], **kw), TransformFromCopy(rhs2[6:8], rhs3[0:2], **kw), run_time=1.5, lag_ratio=0.02))
        self.wait()
        brace = Brace(rhs3, DOWN)
        char_poly = VGroup(Text('Characteristic polynomial of', font_size=30, fill_color=BLUE), mat.copy())
        char_poly.arrange(RIGHT)
        char_poly.next_to(brace, DOWN)
        char_poly.shift_onto_screen()
        self.play(GrowFromCenter(brace), FadeIn(char_poly, DOWN))
        self.wait()
        equals_zero = OldTex('= 0')
        equals_zero.next_to(rhs3, RIGHT)
        root_words = OldTex('\\lambda_1, \\lambda_2 \\,=\\, \\text{roots}', tex_to_color_map={'\\lambda_1': TEAL_C, '\\lambda_2': TEAL_B, '=': WHITE})
        new_rhs3 = VGroup(rhs3, equals_zero)
        root_words.next_to(brace, DOWN)
        root_words.match_x(new_rhs3)
        self.play(FadeIn(root_words, DOWN), FadeOut(char_poly, DOWN), brace.animate.become(Brace(new_rhs3)), Write(equals_zero))
        self.wait()
        ...
        self.play(TransformMatchingShapes(rhs3.copy(), formula), FadeOut(root_words[-1]), root_words[:-1].animate.shift(0.5 * DL))
        self.wait()
        solution = VGroup(root_words[:-1], formula)
        self.play(solution.animate.shift(formula2.get_width() * LEFT))
        formula2.next_to(formula, RIGHT)
        self.play(FadeIn(formula2))
        self.wait()
        solution.add(formula2)
        self.play(solution.animate.shift(formula3.get_width() * LEFT))
...
class OutlineThreeFacts(Scene):
    ...
    def construct(self):
        ...
        self.add(mat)
        self.play(GrowArrow(arrow), Write(arrow_label, run_time=1), LaggedStart(*(AnimationGroup(*(Transform(entry, lambdas[i]) for entry in mat.get_entries().deepcopy())) for i in [0, 2]), lag_ratio=0.3), FadeIn(lambdas[1]))
        self.clear()
        ...
        self.play(LaggedStartMap(FadeIn, indices, shift=0.25 * UP, lag_ratio=0.3), FadeOut(arrow_label), MoveToTarget(group))
        self.wait()
        tr_mat = mat.deepcopy()
        ...
        tr_rects = VGroup(SurroundingRectangle(tr_mat.get_entries()[0]), SurroundingRectangle(tr_mat.get_entries()[3]))
        tr_rects.set_color(BLUE_C)
        moving_tr_rects = tr_rects.copy()
        moving_tr_rects.generate_target()
        tex_kw = {'tex_to_color_map': {'a': COL_COLORS[0], 'b': COL_COLORS[1], 'c': COL_COLORS[0], 'd': COL_COLORS[1], '=': WHITE, '\\lambda_1': EIGEN_COLORS[0], '\\lambda_2': EIGEN_COLORS[1]}}
        ...
        for term, rect in zip(tr_rhs[1:4:2], moving_tr_rects.target):
            rect.move_to(term)
        self.play(TransformFromCopy(mat, tr_mat), Write(VGroup(*tr[:2], tr[-1])))
        self.play(LaggedStart(*map(ShowCreation, tr_rects)))
        tr.add(tr_rects)
        self.play(MoveToTarget(moving_tr_rects), TransformFromCopy(tr_mat.get_entries()[0], tr_rhs.get_part_by_tex('a')), TransformFromCopy(tr_mat.get_entries()[3], tr_rhs.get_part_by_tex('d')), FadeIn(tr_rhs[0:4:2]))
        self.play(FadeOut(moving_tr_rects))
        self.wait()
        self.play(TransformMatchingShapes(lambdas.copy(), tr_rhs[4:]))
        self.wait()
        ...
        self.play(GrowFromCenter(half), MoveToTarget(tr), TransformMatchingShapes(tr_rhs, new_tr_rhs))
        self.wait()
        ...
        self.play(TransformFromCopy(mat, det_mat), Write(VGroup(*det[:2], det[-1])))
        self.play(det_path_anim(det_mat), LaggedStart(Animation(Mobject(), remover=True), FadeIn(det_rhs[:3]), FadeIn(det_rhs[3:6]), lag_ratio=0.7))
        self.wait()
        ...
        randy.to_edge(DOWN)
        self.play(FadeIn(third_point_placeholder), VFadeIn(randy), randy.change('erm', third_point_placeholder))
        self.play(Blink(randy))
        self.wait()
        self.play(randy.change('thinking', eq_m), Write(eq_m))
        self.play(randy.animate.look_at(eq_p), Write(eq_p), mat.animate.set_height(1, about_edge=DOWN))
        self.play(Blink(randy))
        self.wait()
        ...
        diag_rects = VGroup(SurroundingRectangle(ex_mat.get_entries()[0]), SurroundingRectangle(ex_mat.get_entries()[3]))
        off_diag_rects = VGroup(SurroundingRectangle(ex_mat.get_entries()[1]), SurroundingRectangle(ex_mat.get_entries()[2]))
        ...
        self.play(randy.change('raise_right_hand', ex_mat), FadeIn(ex_mat, RIGHT), FadeOut(group, RIGHT))
        self.play(Blink(randy))
        self.wait()
        self.play(FadeIn(tr_rect), randy.change('pondering', tr_rhs))
        self.play(Write(m_eq[:2]), LaggedStartMap(ShowCreation, diag_rects, lag_ratio=0.5, run_time=1), randy.animate.look_at(m_eq))
        self.wait()
        self.remove(diag_rects)
        self.play(TransformFromCopy(diag_rects, mean_rect), FadeTransform(ex_mat.get_entries()[0].copy(), m_eq[2]), FadeTransform(ex_mat.get_entries()[3].copy(), m_eq[2]))
        self.play(Blink(randy))
        self.wait()
        self.play(FadeOut(tr_rect), FadeIn(det_rect), randy.animate.look_at(tr_rhs))
        self.play(Write(p_eq1[:2]), randy.change('hesitant', p_eq1), FadeOut(mean_rect))
        self.play(det_path_anim(ex_mat))
        self.wait()
        self.play(FadeIn(diag_rects), FadeIn(p_eq1[2]))
        self.play(FadeOut(diag_rects), FadeIn(off_diag_rects), FadeIn(p_eq1[3:]))
        self.play(FadeOut(off_diag_rects))
        self.play(randy.change('tease', p_eq2), FadeOut(p_eq1), FadeIn(p_eq2))
        ...
        self.play(TransformMatchingShapes(VGroup(m_eq[0], p_eq2[0]).copy(), form_rhs))
...
class MeansMatch(Scene):
    ...
    def construct(self):
        t2c = {'{a}': COL_COLORS[0], '{d}': COL_COLORS[1], '\\lambda_1': EIGEN_COLORS[0], '\\lambda_2': EIGEN_COLORS[1], '=': WHITE}
        equation = OldTex('{{a} + {d} \\over 2} = {\\lambda_1 + \\lambda_2 \\over 2}', tex_to_color_map=t2c)
        mean_eq = OldTex('\\text{mean}({a}, {d}) = \\text{mean}(\\lambda_1, \\lambda_2)', tex_to_color_map=t2c)
...
class ShowSquishingAndStretching(Scene):
    ...
    def construct(self):
        ...
        plane = NumberPlane((-20, 20), (-20, 20), background_line_style={'stroke_width': 3}, faded_line_ratio=0)
        plane.axes.set_stroke(BLUE, 3)
        back_plane = NumberPlane(faded_line_ratio=0)
        ...
        bases = VGroup(Vector(RIGHT, fill_color=GREEN, thickness=0.05), Vector(UP, fill_color=RED, thickness=0.05))
        ...
        self.wait()
        labels = VGroup(*(OldTex('\\text{Stretch by }', f'\\lambda_{i}', color=color, font_size=30) for i, color in zip((1, 2), EIGEN_COLORS)))
        for label, vect in zip(labels, eigenvect_mobs):
            label.next_to(vect.get_center(), rotate_vector(normalize(vect.get_vector()), 90 * DEGREES), buff=0.1, index_of_submobject_to_align=1)
...
class MeanProductExample(Scene):
    ...
    def construct(self):
        ...
        l1_arrow.add_updater(lambda m: m.set_points_by_ends(m_label.get_left() + SMALL_BUFF * LEFT, l1_label.get_right() + SMALL_BUFF * RIGHT))
        l2_arrow.add_updater(lambda m: m.set_points_by_ends(m_label.get_right() + SMALL_BUFF * RIGHT, l2_label.get_left() + SMALL_BUFF * LEFT))
        minus_d = OldTex('-d')
        plus_d = OldTex('+d')
        ...
        self.add(d_tracker)
        self.wait(20)
        self.play(UpdateFromAlphaFunc(Mobject(), lambda m, a: VGroup(plus_d, minus_d).set_opacity(a), remover=True), UpdateFromAlphaFunc(Mobject(), lambda m, a: VGroup(plus_qm, minus_qm).set_opacity(1 - a), remover=True))
        self.remove(plus_qm, minus_qm)
        self.wait(5)
        kw = {'tex_to_color_map': {'\\,7': MEAN_COLOR, 'd': GREY_A, '=': WHITE, '-': WHITE}}
        texs = VGroup(*(OldTex(tex, **kw) for tex in ['(\\,7 + d\\,)(\\,7 - d\\,)', '\\,7^2 - d^2 = ', '40 =', 'd^2 = \\,7^2 - 40', 'd^2 = 9', 'd = 3']))
        texs[:3].arrange(LEFT)
        ...
        self.wait(3)
        self.play(LaggedStart(TransformFromCopy(texs[2][0], texs[3][6], path_arc=-45 * DEGREES), TransformFromCopy(texs[2][1], texs[3][2]), TransformFromCopy(texs[1][:3], texs[3][3:6]), TransformFromCopy(texs[1][3:5], texs[3][0:2], path_arc=45 * DEGREES), lag_ratio=0.1))
        self.wait(2)
        ...
        self.play(LaggedStart(FadeOut(minus_d, 0.25 * UP), FadeOut(plus_d, 0.25 * UP), lag_ratio=0.25), LaggedStart(FadeIn(minus_3, 0.25 * UP), FadeIn(plus_3, 0.25 * UP), lag_ratio=0.25))
        self.wait()
        rects = VGroup(SurroundingRectangle(VGroup(l1_label, nl.numbers[4])), SurroundingRectangle(VGroup(l2_label, nl.numbers[10])))
        ...
        for g1, g2 in ((sevens, ms), (fourties, ps)):
            self.play(LaggedStartMap(FadeOut, g1, shift=0.25 * UP, lag_ratio=0.3), LaggedStartMap(FadeIn, g2, shift=0.25 * UP, lag_ratio=0.3), texs[-2:].animate.set_opacity(0))
        self.remove(texs[-2:])
        self.wait()
        self.play(FlashUnder(texs[3]))
        self.wait()
        plus_form, minus_form = [OldTex(c + '\\sqrt{\\,m^2 - p}', tex_to_color_map={'m': MEAN_COLOR, 'p': PROD_COLOR}, font_size=24).move_to(d, DOWN) for c, d in [('+', plus_d), ('-', minus_d)]]
        pre_group = VGroup(*texs[3][4:6], ms[-1], ps[-1])
        self.play(TransformMatchingShapes(pre_group.copy(), minus_form), TransformMatchingShapes(pre_group.copy(), plus_form), FadeOut(VGroup(minus_3, plus_3), shift=0.25 * UP))
        self.wait()
...
class AskIfThatsBetter(Scene):
    ...
    def construct(self):
        ...
        self.play(PiCreatureSays(randy, 'Is that better?', target_mode='sassy', bubble_config={'direction': LEFT, 'width': 4, 'height': 2}, look_at=UL))
        self.play(Blink(randy))
        self.wait()
...
class OutstandingChannel(Scene):
    ...
    def construct(self):
        morty = Mortimer()
        morty.to_corner(DR)
        self.play(PiCreatureSays(morty, OldTexText('Outstanding\\\\channel'), target_mode='hooray', bubble_config={'height': 3, 'width': 4}))
...
class JingleAnimation(Scene):
    ...
    def construct(self):
        ...
        m.refresh_bounding_box()
        p.refresh_bounding_box()
        ...
        self.wait(0.2)
        self.play(Restore(pm, rate_func=smooth, run_time=0.5), FadeOut(VGroup(plus_word, minus_word, mean), run_time=0.5))
        ...
        self.play(FadeIn(sqrt, rate_func=squish_rate_func(smooth, 0.25, 0.75)), VShowPassingFlash(sqrt_outline, time_width=2), Restore(root, rate_func=squish_rate_func(snap, 0.25, 1)), run_time=1)
        self.play(TransformFromCopy(m, m2, path_arc=-120 * DEGREES, rate_func=smooth, run_time=0.6))
        ...
        self.wait(0.2)
        self.play(FadeIn(p, scale=0.5, rate_func=snap, run_time=0.5))
        self.add(product)
        self.wait(0.4)
...
class Example1(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(Write(mat.get_entries(), lag_ratio=0.1, run_time=2), self.change_students('pondering', 'pondering', 'thinking', look_at=mat))
        self.wait()
        ...
        formula = OldTex('\\lambda_1, \\, \\lambda_2 = {2} \\pm \\sqrt{\\,{2}^2 - (-1)}', tex_to_color_map={'\\lambda_1': EIGEN_COLORS[0], '\\lambda_2': EIGEN_COLORS[1], '(-1)': PROD_COLOR, '{2}': MEAN_COLOR})
        ...
        self.play(FadeOut(bubble), mat.animate.shift(shift_vect), FadeIn(arrow, shift_vect), FadeIn(formula), FadeIn(form_rects), self.change_students('pondering', 'pondering', 'pondering', look_at=ORIGIN))
        self.play(self.teacher.change('tease', formula), *(pi.animate.look_at(formula) for pi in self.students))
        self.wait()
        m_eq = OldTex('m', '=', '2', tex_to_color_map={'m': MEAN_COLOR, '2': MEAN_COLOR})
        m_eq.next_to(mat, UP, LARGE_BUFF)
        t2c = {'\\lambda_1': EIGEN_COLORS[0], '\\lambda_2': EIGEN_COLORS[1], '{m}': MEAN_COLOR, '{p}': PROD_COLOR, '=': WHITE}
        ...
        self.play(ShowCreation(diag_rects), self.teacher.change('tease', diag_rects), *(pi.animate.look_at(diag_rects) for pi in self.students))
        self.wait()
        self.play(FadeTransform(diag_rects[0].copy(), m_eq[2], remover=True), FadeTransform(diag_rects[1].copy(), m_eq[2], remover=True), *(pi.animate.look_at(two_rect) for pi in self.pi_creatures))
        self.wait(2)
        self.remove(twos, min1)
        VGroup(twos, min1).set_opacity(1)
        for i in (0, 1):
            self.play(TransformFromCopy(m_eq[2], twos[i]), FadeOut(m_rects[i]), self.teacher.change('raise_right_hand', twos), *(pi.change('thinking', twos) for pi in self.students))
        self.wait()
        prod_eq = OldTex('{p} = {3} - {4} = -1', tex_to_color_map={'{3}': COL_COLORS[0], '{4}': COL_COLORS[0], **t2c})
        prod_eq.next_to(mat, UP, LARGE_BUFF)
        prod_eq.set_x(mat.get_center()[0], LEFT)
        lhs = prod_eq[:1]
        self.play(m_eq.animate.scale(0.7).next_to(mat, LEFT).to_edge(LEFT), FadeIn(lhs), FadeOut(diag_rects), self.teacher.change('happy'), self.change_students('erm', 'hesitant', 'thinking', look_at=prod_eq))
        self.play(det_path_anim(mat, run_time=1))
        self.play(FadeIn(prod_eq[1]), FadeTransform(mat.get_entries()[0].copy(), prod_eq[2]), FadeTransform(mat.get_entries()[3].copy(), prod_eq[2]), VFadeInThenOut(VGroup(*(SurroundingRectangle(e) for e in mat.get_entries()[0::3]))), self.teacher.change('coin_flip_1'))
        self.play(FadeIn(prod_eq[3]), FadeTransform(mat.get_entries()[1].copy(), prod_eq[4]), FadeTransform(mat.get_entries()[2].copy(), prod_eq[4]), VFadeInThenOut(VGroup(*(SurroundingRectangle(e) for e in mat.get_entries()[1:3]))))
        self.wait()
        self.play(Write(prod_eq[-2:]), self.change_students('tease', 'tease', 'happy', look_at=prod_eq.get_right()))
        self.play(FadeOut(p_rect), FadeTransform(prod_eq[-1].copy(), min1), self.teacher.change('raise_left_hand', min1), *(pi.animate.look_at(min1) for pi in self.students))
        self.wait()
        self.play(prod_eq.animate.scale(0.7).next_to(m_eq, UP).to_edge(LEFT))
        self.wait(2)
        rhs = OldTex('2\\pm\\sqrt{5}')
        rhs.move_to(formula[4], LEFT)
        self.play(self.teacher.change('tease', rhs), TransformMatchingShapes(formula[4:6].copy(), rhs), formula[4:].animate.shift(UP))
        self.wait()
...
class GeneralExample(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeTransform(diag_rects[0].copy(), mean_eq[2]), FadeTransform(diag_rects[1].copy(), mean_eq[2]))
        self.play(FadeOut(diag_rects))
        last_part = '(' + prod_str + ')}' if prod < 0 else prod_str + '}'
        formula = OldTex(mean_str, '\\pm \\sqrt{\\,', mean_str, '^2 - ', last_part, font_size=60, tex_to_color_map={mean_str: MEAN_COLOR, prod_str: PROD_COLOR})
        formula.next_to(mat, RIGHT, buff=1.5)
        self.play(Write(formula[1]), Write(formula[3]), FadeTransform(mean_eq[2].copy(), formula[0]), FadeTransform(mean_eq[2].copy(), formula[2]))
        self.wait()
        prod_eq = OldTex(f'p = {a * d} - {b * c} = {prod}', tex_to_color_map={str(prod): PROD_COLOR, str(a * d): COL_COLORS[0], str(b * c): COL_COLORS[0], '=': WHITE})
        prod_eq.move_to(mean_eq)
        self.play(FadeIn(prod_eq[:2]), mean_eq.animate.shift(UP))
        self.play(FadeIn(prod_eq[2]), VFadeInThenOut(get_diag_rects(mat, color=YELLOW), run_time=1.5))
        self.play(FadeIn(prod_eq[3:5]), VFadeInThenOut(get_diag_rects(mat, color=YELLOW, off_diagonal=True), run_time=1.5))
        self.wait()
...
class PauliMatrices(Scene):
    ...
    def construct(self):
        ...
        axes.set_height(1.7)
        axes.labels = VGroup(*(OldTex(ch, font_size=24).next_to(axis.get_end(), vect, buff=SMALL_BUFF) for ch, axis, vect in zip('xyz', axes.get_axes(), [RIGHT, UP, UP])))
        ...
        for i, matrix, color, (v1, v2) in zip(it.count(), matrices, colors, [(UP, DOWN), (LEFT, RIGHT), (RIGHT, RIGHT)]):
            ...
            axis = ac.get_axes()[i]
            vp1, vm1 = (Arrow(axis.n2p(0), axis.n2p(n), buff=0.05, fill_color=color, thickness=0.05) for n in (2, -2))
        ...
        self.play(FadeOut(title), Write(acx, stroke_width=0.5))
        self.wait()
        ...
        mean_eqs = VGroup(*(OldTex('m = 0', tex_to_color_map={'m': MEAN_COLOR, '0': WHITE}).next_to(matrix, RIGHT, buff=MED_LARGE_BUFF) for matrix in matrices))
        kw = {'tex_to_color_map': {'0': MEAN_COLOR, '{p}': PROD_COLOR}}
        forms = VGroup(*(OldTex('0 \\pm \\sqrt{\\,0^2 - {p}}', **kw).next_to(matrix, RIGHT).to_edge(RIGHT) for matrix in matrices))
        ...
        for me, mr, form in zip(mean_eqs, means_rects, forms):
            self.play(FadeIn(mr), Write(me[:2], run_time=1))
            self.play(TransformFromCopy(mr, me[2], run_time=0.7))
        self.wait()
        self.play(LaggedStart(*(FadeIn(VGroup(form[1], form[3], form[4])) for form in forms), lag_ratio=0.2), LaggedStart(*(TransformFromCopy(me.get_parts_by_tex('0'), form.get_parts_by_tex('0')) for me, form in zip(mean_eqs, forms)), lag_ratio=0.2))
        self.wait()
        self.play(LaggedStart(*(TransformMatchingShapes(form, simple_form) for form, simple_form in zip(forms, simple_forms))))
        self.wait()
        prod_eqs = VGroup(*(OldTex('p = -1', tex_to_color_map={'p': PROD_COLOR, '-1': WHITE}).next_to(matrix, RIGHT, buff=MED_LARGE_BUFF).shift(0.5 * DOWN) for matrix in matrices))
        self.play(mean_eqs.animate.shift(0.5 * UP), LaggedStart(*(FadeIn(pe[:2]) for pe in prod_eqs)), FadeOut(means_rects))
        ...
        self.play(LaggedStart(*(FadeTransform(pe[2].copy(), ff) for pe, ff in zip(prod_eqs, final_forms)), lag_ratio=0.3), LaggedStart(*(sf.animate.next_to(ff, LEFT) for sf, ff in zip(simple_forms, final_forms))))
        self.wait()
        axes_copies.shift(2 * RIGHT)
        self.play(FadeOut(simple_forms), FadeOut(final_forms), FadeIn(acx))
    ...
    def get_lhss_and_matrices(self, colors):
        ...
        matrices = VGroup(Matrix([['0', '1'], ['1', '0']], **kw), Matrix([['0', '-i'], ['i', '0']], **kw), Matrix([['1', '0'], ['0', '-1']], **kw))
...
class SpinMeasurements(ThreeDScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(screen), FadeIn(axes), FadeIn(machine, lag_ratio=0.05))
        self.wait()
        ...
        def hit_anim(spin=1):
            ...
            return AnimationGroup(Transform(pre_circ, circ, remover=True), VFadeInThenOut(tex))
        for x in range(20):
            if random.random() < 0.5:
                beam = high_beam
                spin = +1
                ...
                beam = low_beam
                spin = -1
            self.play(LaggedStart(VShowPassingFlash(beam, time_width=0.5), hit_anim(spin), lag_ratio=0.6))

class HeresTheThing(TeacherStudentsScene):

    def construct(self):
        self.student_says('Wait, why?', target_mode='raise_right_hand', look_at=self.screen, index=2)
        self.play(self.change_students('maybe', 'confused', 'raise_right_hand', look_at=self.screen), self.teacher.change('happy'))
        self.wait(3)
        self.teacher_says(OldTexText("Here's the thing..."), added_anims=[self.change_students('sassy', 'plain', 'hesitant')], target_mode='hesitant')
        self.wait(2)
        ...
        words.shift(RIGHT)
        self.play(self.teacher.change('guilty'), self.change_students('sassy', 'hesitant', 'sassy'), Write(words))
        self.wait(4)

class NotAllHopeIsLost(TeacherStudentsScene):

    def construct(self):
        self.teacher_says(OldTexText("There's still a\\\\good example here"), target_mode='speaking', bubble_config={'height': 3, 'width': 4}, added_anims=[self.change_students('erm', 'sassy', 'hesitant')])
        self.wait(2)
...
class PauliMatricesWithCharacteristicPolynomial(PauliMatrices):
    ...
    def construct(self):
        ...
        group.to_edge(LEFT)
        kw = {'element_to_mobject_config': {'tex_to_color_map': {'-\\lambda': EIGEN_COLORS[1]}}, 'v_buff': 0.7, 'h_buff': 1.3}
        new_matrices = VGroup(Matrix([['-\\lambda', '1'], ['1', '-\\lambda']], **kw), Matrix([['-\\lambda', '-i'], ['i', '-\\lambda']], **kw), Matrix([['1 -\\lambda', '0'], ['0', '-1 -\\lambda']], **kw))
        arrows = VGroup()
        det_terms = VGroup()
        ...
        self.wait()
        self.play(LaggedStart(*(TransformMatchingShapes(mat.copy(), det_term) for mat, det_term in zip(matrices, det_terms)), lag_ratio=0.8), LaggedStartMap(GrowArrow, arrows, lag_ratio=0.8))
        self.wait()
        kw = {'tex_to_color_map': {'\\lambda': EIGEN_COLORS[1], '=': WHITE}}
        rhss = VGroup(OldTex('= \\lambda^2 - 1 = 0', **kw), OldTex('= \\lambda^2 - 1 = 0', **kw), OldTex('= (1 - \\lambda)(-1 - \\lambda) = 0', **kw))
        lpm1s = VGroup()
        for rhs, det_term in zip(rhss, det_terms):
            rhs.next_to(det_term, RIGHT, SMALL_BUFF, submobject_to_align=rhs[0])
            self.play(LaggedStart(det_path_anim(det_term[-1]), TransformMatchingShapes(det_term[-1].get_entries().copy(), rhs), lag_ratio=0.5))
            if rhs is rhss[-1]:
                continue
        ...
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(*arrows, *det_terms, *rhss, *lpm1s, rect, words, diag_rects)))
        colors = [YELLOW_B, YELLOW_C, YELLOW_D]

        def get_combo(triplet):
            group = VGroup(OldTex('a'), triplet[0].copy(), OldTex('+'), OldTex('b'), triplet[1].copy(), OldTex('+'), OldTex('c'), triplet[2].copy())
        ...
        kw = {'tex_to_color_map': {'a': colors[0], 'b': colors[1], 'c': colors[2], '\\lambda': EIGEN_COLORS[1]}}
        normalized_eq = OldTex('\\left(a^2 + b^2 + c^2 = 1\\right)', **kw)
        normalized_eq.next_to(row2, DOWN, LARGE_BUFF)
        normalized_eq.to_edge(DOWN)
        full_mat = Matrix([['c', 'a - bi'], ['a + bi', '-c']], element_to_mobject_config=kw, h_buff=1.5)
        ...
        self.play(Blink(randy))
        self.wait()
        self.play(FadeIn(p_eq, 0.25 * UP), m_eq.animate.next_to(full_mat, RIGHT, aligned_edge=UP), randy.change('hesitant', full_mat), FadeOut(diag_rects))
        self.play(det_path_anim(full_mat))
        self.play(Blink(randy))
        self.wait()
        self.play(FadeOut(bubble), randy.change('tease', p_eq2), Transform(p_eq, p_eq2))
        self.play(Blink(randy))
        self.wait()
        mod_mat = Matrix([['c - \\lambda', 'a - bi'], ['a + bi', '-c - \\lambda']], element_to_mobject_config=kw, h_buff=2.0)
        parens = OldTex('(', ')')
        parens.stretch(2, 1)
        parens.match_height(mod_mat)
        parens[0].next_to(mod_mat, LEFT, SMALL_BUFF)
        ...
        self.play(TransformMatchingShapes(full_mat.copy(), char_poly, run_time=2), randy.change('raise_left_hand', char_poly))
...
class ThreeSpinExamples(Scene):
    ...
    def construct(self):
        ...
        axes = ThreeDAxes((-1, 1), (-1, 1), (-1, 1), axis_config={'tick_size': 0, 'include_tip': False})
...
class GeneralDirection(Scene):
    ...
    def construct(self):
        ...
        frame.add_updater(lambda m, dt: m.increment_theta(0.01 * dt))
        self.add(frame)
        axes = ThreeDAxes((-2, 2), (-2, 2), (-4, 4), axis_config={'include_tip': False}, depth=6)
        ...
        self.play(GrowArrow(vector), UpdateFromAlphaFunc(sphere, lambda m, a: m.set_opacity(a)), Write(mesh))
        self.play(Write(label))
        self.wait(6)
        self.play(vector.animate.scale(0.5, about_point=ORIGIN), label.animate.shift(-0.5 * vect))
        self.wait(24)
...
class TwoValuesEvenlySpaceAroundZero(Scene):
    ...
    def construct(self):
        ...
        prod_label = VGroup(labels.copy().clear_updaters().arrange(RIGHT, buff=SMALL_BUFF), OldTex('='), DecimalNumber(-4))
...
class MPIsSolvingCharPoly(TeacherStudentsScene):
    ...
    def construct(self):
        formula = OldTex('{m} \\pm \\sqrt{\\,{m}^2 - {p}}', tex_to_color_map={'{m}': MEAN_COLOR, '{p}': PROD_COLOR}, font_size=72)
        ...
        self.wait()
        self.play(self.teacher.change('hooray', char_poly), formula.animate.next_to(arrow, LEFT), FadeIn(char_poly, 0.5 * UP))
        self.play_student_changes('erm', 'pondering', 'pondering', look_at=char_poly, added_anims=[GrowArrow(arrow)])
        self.wait(3)

class QuadraticPolynomials(Scene):

    def construct(self):
        kw = {'tex_to_color_map': {'\\lambda_1': EIGEN_COLORS[0], '\\lambda_2': EIGEN_COLORS[1], '=': WHITE}}
        equation = VGroup(OldTex('x^2 - 10x + 9', **kw)[0], OldTex('=').rotate(PI / 2), OldTex('x^2 - (\\lambda_1 + \\lambda_2)x + \\lambda_1 \\lambda_2', **kw), OldTex('=').rotate(PI / 2), OldTex('(x - \\lambda_1)(x - \\lambda_2)', **kw))
        ...
        line3.save_state()
        line3.move_to(line2)
        axes = Axes((-5, 10), (-20, 20), height=7, width=FRAME_WIDTH / 2)
        ...
        self.play(LaggedStartMap(GrowFromCenter, root_dots), LaggedStart(GrowFromPoint(root_labels[0], root_dots[0].get_center()), GrowFromPoint(root_labels[1], root_dots[1].get_center())))
        self.wait()
        ...
        self.play(TransformFromCopy(xs, line2[:2]))
        self.play(LaggedStart(AnimationGroup(TransformFromCopy(line3[1], line2[10].copy()), TransformFromCopy(line3[9:11], line2[7:9]), FadeIn(line2[2])), AnimationGroup(TransformFromCopy(line3[7], line2[10].copy(), remover=True), TransformFromCopy(line3[3:5], line2[4:6]), FadeIn(VGroup(line2[3], line2[6], line2[9])))))
        self.play(TransformFromCopy(VGroup(*line3[3:5], *line3[9:11]), VGroup(*line2[12:16])), FadeIn(line2[11]), FadeIn(eq2))
        self.wait()
        for i, j, k, l in [(3, 5, 3, 10), (7, 8, 12, 16)]:
            self.play(FadeIn(SurroundingRectangle(line1[i:j])), FadeIn(SurroundingRectangle(line2[k:l])), run_time=2, rate_func=there_and_back_with_pause, remover=True)
        ...
        m_eq = OldTex('{\\lambda_1 + \\lambda_2 \\over 2}', '=', '5', tex_to_color_map={'\\lambda_1': EIGEN_COLORS[0], '\\lambda_2': EIGEN_COLORS[1]})
        ...
        self.wait()
        self.play(GrowArrow(mean_arrow), FadeIn(times_half, shift=0.25 * UP, scale=2), FadeIn(m_eq[:-1]))
        self.play(Write(m_eq[-1]))
        self.play(FadeTransform(m_eq[-1].copy(), m_dot), FadeTransform(m_eq[-1].copy(), m_label))
        self.wait()
        ...
        my = m_label.get_y()
        arrows = VGroup(Arrow(m_label.get_left(), [root_dots[0].get_left()[0], my, 0], buff=0.1), Arrow(m_label.get_right(), [root_dots[1].get_right()[0], my, 0], buff=0.1))
        for d_term, arrow in zip(d_terms, arrows):
            d_term.next_to(arrow, UP, buff=0)
        self.play(FadeOut(p_rects))
        self.play(TransformMatchingShapes(equation[0][-2:].copy(), d_terms), FadeOut(root_labels), *map(GrowArrow, arrows))
        self.wait()
        self.play(FadeOut(fade_rect))
        self.wait()

class SimplerQuadraticFormula(Scene):

    def construct(self):
        form1 = OldTex('{m} \\pm \\sqrt{\\,{m}^2 - {p}}', tex_to_color_map={'{m}': MEAN_COLOR, '{p}': PROD_COLOR}, font_size=72)
        words = OldTexText('takes less to\\\\remember than')
        form2 = OldTex('{-b \\pm \\sqrt{\\,b^2 - 4ac} \\over 2a}', tex_to_color_map={'a': MAROON_A, 'b': MAROON_B, 'c': MAROON_C}, font_size=72)
...
class SkipTheMiddleStep(Scene):
    ...
    def construct(self):
        ...
        mp_formula = OldTex('{2} \\pm \\sqrt{\\,{2}^2 - (-1)}', tex_to_color_map={'{2}': MEAN_COLOR, '(-1)': PROD_COLOR})
        group = VGroup(matrix, char_poly, mp_formula)
        group.arrange(RIGHT, buff=2)
        mat_label, char_poly_label, eigen_label = labels = VGroup(OldTexText('Matrix'), OldTexText('Characteristic\\\\polynomial'), OldTexText('Eigenvalues'))
        ...
        arc = -90 * DEGREES
        arrows = VGroup(Arrow(matrix.get_corner(UR), char_poly.get_top(), path_arc=arc), Arrow(char_poly.get_top(), mp_formula.get_top(), path_arc=arc), Arrow(matrix.get_corner(UR), mp_formula.get_top(), path_arc=0.8 * arc))
        arrows.shift(0.5 * UP)
        self.add(matrix, mat_label)
        self.play(DrawBorderThenFill(arrows[0]), GrowFromPoint(char_poly, matrix.get_top(), path_arc=arc), FadeTransform(mat_label.copy(), char_poly_label))
        self.wait()
        self.play(DrawBorderThenFill(arrows[1]), TransformFromCopy(char_poly, mp_formula), FadeTransform(char_poly_label.copy(), eigen_label))
        self.wait()
        self.play(VFadeInThenOut(get_diag_rects(matrix), run_time=2), LaggedStart(*(ShowCreationThenFadeOut(SurroundingRectangle(sm, buff=0.1, color=WHITE), run_time=2) for sm in mp_formula.get_parts_by_tex('{2}'))))
        self.play(det_path_anim(matrix), ShowCreationThenFadeOut(SurroundingRectangle(mp_formula.get_parts_by_tex('(-1)'), buff=0.1, color=WHITE), run_time=2))
        self.wait()
        self.play(Transform(arrows[0], arrows[2]), Transform(arrows[1], arrows[2]), VGroup(char_poly, char_poly_label).animate.set_opacity(0.15))
        self.wait()
        frame = self.camera.frame
        quad_form = OldTex('{-b \\pm \\sqrt{\\,b^2 - 4ac} \\over 2a}', tex_to_color_map={'a': BLUE_B, 'b': BLUE_C, 'c': BLUE_D})
        ...
        self.play(frame.animate.set_y(1), FadeIn(quad_form, DOWN))
        self.wait()
...
class GeneralCharPoly(Scene):
    ...
    def construct(self):
        t2c = {'a': COL_COLORS[0], 'b': COL_COLORS[1], 'c': COL_COLORS[0], 'd': COL_COLORS[1], '\\lambda': EIGEN_COLORS[1], '-': WHITE, '+': WHITE, '=': WHITE}
        ...
        self.add(det)
        self.wait()
        self.play(TransformMatchingShapes(det[-1].get_entries()[0::3].copy(), rhs1[:10]))
        self.play(TransformMatchingShapes(det[-1].get_entries()[1:3].copy(), rhs1[10:], path_arc=45 * DEGREES))
        self.wait()
