...
class RandomChordScene(Scene):
    ...
    def construct(self):
        ...
        for s, rt in ((slice(0, 16), 8), (slice(18, None), self.run_time)):
            ...
            self.play(ShowIncreasingSubsets(chords[s]), ShowSubmobjectsOneByOne(flash_chords[s]), ShowSubmobjectsOneByOne(indicators[s]), Animation(triangle), fraction.alpha_update, rate_func=linear, run_time=rt)
    ...
    def get_fraction(self, data):
        ...
        nl1 = Integer(100, edge_to_fix=ORIGIN)
        nl2 = Integer(100, edge_to_fix=RIGHT)
        ns = Integer(100, edge_to_fix=LEFT)
        ratio = DecimalNumber(0, num_decimal_places=4)
        fraction = VGroup(nl1.move_to(tex[0]), tex[1], nl2.move_to(tex[2]), tex[3], ns.move_to(tex[4]), tex[5], ratio.move_to(tex[6], LEFT))
...
class PairOfPoints(RandomChordScene):
    ...
    def get_random_chord(circle):
        return Line(circle.pfp(random.random()), circle.pfp(random.random()))

    @staticmethod
    def get_method_indicator(chord):
        dots = DotCloud([chord.get_start(), chord.get_end()])
...
class CenterPoint(RandomChordScene):
    ...
    def chord_from_xy(x, y, circle):
        ...
        line = Line([temp_x, -temp_y, 0], [temp_x, temp_y, 0])
...
class RadialPoint(CenterPoint):
    ...
    def get_random_chord(circle):
        ...
        return CenterPoint.chord_from_xy(dist * math.cos(angle), dist * math.sin(angle), circle)
...
class CompareFirstTwoMethods(RandomChordScene):
    ...
    def construct(self):
        ...
        titles = VGroup(Text('Choose a pair of points'), Text('Chose a center point'))
        for title, circle in zip(titles, circles):
            title.next_to(circle, UP, MED_LARGE_BUFF)
        chord_groups = VGroup(self.get_chords(circles[0], PairOfPoints.get_random_chord), self.get_chords(circles[1], CenterPoint.get_random_chord))
        flash_chord_groups = chord_groups.copy()
        flash_chord_groups.set_stroke(width=3, opacity=1)
        indicator_groups = Group(Group(*(PairOfPoints.get_method_indicator(c) for c in chord_groups[0])), Group(*(CenterPoint.get_method_indicator(c) for c in chord_groups[1])))
        self.add(circles, titles)
        self.play(*map(ShowIncreasingSubsets, chord_groups), *map(ShowSubmobjectsOneByOne, flash_chord_groups), *map(ShowSubmobjectsOneByOne, indicator_groups), rate_func=linear, run_time=15)
        self.play(*map(FadeOut, (flash_chord_groups[0][-1], flash_chord_groups[1][-1], indicator_groups[0][-1], indicator_groups[1][-1])))
        self.wait()
...
class SparseWords(Scene):
    ...
    def construct(self):
        ...
        arrow = Arrow(words.get_top(), [3.5, -0.5, 0])
...
class PortionOfRadialLineInTriangle(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ShowCreation(half_line), FadeIn(half_label, 0.2 * DOWN))
        dot = TrueDot()
        ...
        for alpha in (0.5, 0.01, 0.4):
            self.play(dot.animate.move_to(radial_line.pfp(alpha)), run_time=4)
        self.add(dot)
        self.embed()
...
class RandomPointsFromVariousSpaces(Scene):
    ...
    def construct(self):
        ...
        subwords = OldTexText('($^{*}$Implicitly: According to a \\emph{uniform} distribution)', color=GREY_A, tex_to_color_map={'\\emph{uniform}': YELLOW}, font_size=36)
        ...
        self.play(MoveToTarget(dots, run_time=1), TransformFromCopy(Line(interval.n2p(0), interval.n2p(1)).set_stroke(GREY_B, 1), circle, run_time=1), FadeOut(interval), FadeOut(top_words[1], UP), FadeIn(circle_words, UP), MoveToTarget(top_words[0]))
        ...
        self.play(FadeOut(dots), FadeOut(circle, run_time=2), ShowCreation(sphere, run_time=2), ShowCreation(mesh, lag_ratio=0.1, run_time=2), FadeOut(circle_words, UP), FadeIn(sphere_words, UP))
        ...
        arrow.set_color(YELLOW)
        turn_animation_into_updater(ShowCreation(dots, run_time=10, rate_func=lambda t: 0.5 + 0.5 * t))
        dots.clear_updaters()
        dots.set_points(dots.get_points()[12:16])
        dots.set_radius(0.5)
        dots.add_updater(lambda m: m)
        self.play(ShowCreation(underline), ShowCreation(arrow), FadeIn(question, shift=0.5 * DR), ShowCreation(dots, run_time=4, rate_func=linear))
        self.wait()
...
class CoinFlips(Scene):
    ...
    def construct(self):
        ...
        coins.to_edge(DOWN)
        eq = OldTex('{\\# \\text{Heads} \\over \\# \\text{Flips}} = ', '{Num \\over Den}', '=', '0.500', isolate={'Num', 'Den', '\\# \\text{Heads}'})
        ...
        self.play(ShowIncreasingSubsets(coins, rate_func=linear), UpdateFromAlphaFunc(Mobject(), update_eq, rate_func=linear), Write(words, rate_func=srf), ShowCreation(arrow, rate_func=srf), run_time=18)
        self.wait(2)
...
class ChordsInSpaceWithCircle(RandomChordScene):
    ...
    def construct(self):
        ...
        lines = VGroup(*(RadialPoint.get_random_chord(big_circle) for x in range(n_lines)))
        ...
        triangle = VGroup(triangle.copy().set_stroke(BLACK, 8), triangle.copy())
        ...
        self.play(ShowCreation(circle), VFadeIn(chords))
        self.wait()
        self.play(ShowCreation(triangle, lag_ratio=0))
        circle.add(triangle)
        self.wait()
        key = OldTexText('Blue $\\Rightarrow$ Chord > \\text{Triangle side}\\\\Red $\\Rightarrow$ Chord < \\text{Triangle side}\\\\', tex_to_color_map={'Blue': BLUE, 'Red': RED})
        key.to_edge(UP)
        key.set_backstroke(width=5)
        key[len(key) // 2:].align_to(key[:len(key) // 2], RIGHT)
        self.play(Restore(chords), FadeIn(key))
        self.wait(2)
        ...
        self.wait()
        self.play(circle.animate.set_height(7, about_edge=LEFT), FadeOut(key, rate_func=squish_rate_func(smooth, 0, 0.5)), run_time=4)
        ...
        self.play(FadeOut(chords), FadeOut(lines), FadeIn(big_circle), FadeIn(title), circle.animate.set_height(0.5).to_corner(DR))
        triangle.set_stroke(width=2)
        n_lines = 1000
        lines.become(VGroup(*(CenterPoint.get_random_chord(big_circle) for x in range(n_lines))))
        ...
        self.play(ShowIncreasingSubsets(lines), ShowSubmobjectsOneByOne(flash_lines), ShowSubmobjectsOneByOne(indicators), rate_func=linear, run_time=8)
        self.play(FadeOut(indicators[-1]), FadeOut(flash_lines[-1]))
        self.wait()
        ...
        self.play(circle.animate.move_to(big_circle, RIGHT), run_time=10, rate_func=there_and_back)
        self.wait()
    ...
    def line_to_chord(self, line, circle, stroke_width=1):
        ...
        d2 = np.sqrt(r ** 2 - d1 ** 2)
        chord = Line(center + d1 * normal - d2 * tangent, center + d1 * normal + d2 * tangent)
        chord.set_stroke(BLUE if chord.get_length() > math.sqrt(3) * r else RED, width=stroke_width)
        return chord
...
class TransitiveSymmetries(Scene):
    ...
    def construct(self):
        ...
        dots.add_updater(lambda m: m)
        self.play(Rotate(Group(circle, dot), TAU, about_point=circle.get_center()), ShowCreation(dots, rate_func=lambda a: smooth(a * (1 - 1 / 48) + 1 / 48)), Write(low_words, rate_func=squish_rate_func(smooth, 0.3, 0.5)), run_time=10)
...
class NonTransitive(Scene):
    ...
    def construct(self):
        circle = Circle(radius=3)
        circle.set_stroke(GREY_B, 2)
        words = OldTexText('Rotational symmetries do \\emph{not} act\\\\transitively on the space of all chords', tex_to_color_map={'\\emph{not}': RED})
        ...
        self.play(FadeOut(chords), ShowCreation(chord1_shadow), ShowCreation(chord1), FadeIn(left_words), ShowCreation(left_arrow))
        group = VGroup(circle, chord1)
        self.add(group, left_arrow)
        self.play(Rotate(group, TAU, about_point=circle.get_center()), FadeIn(VGroup(right_words, right_arrow, chord2), rate_func=squish_rate_func(smooth, 0.3, 0.4)), run_time=10)
        self.embed()
...
class RandomSpherePoint(Scene):
    ...
    def construct(self):
        ...
        grid = NumberPlane((-6, 6), (-4, 4), background_line_style={'stroke_color': GREY_B, 'stroke_width': 1, 'stroke_opacity': 0.5}, axis_config={'stroke_width': 1})
        grid.scale(2)
        plane = Rectangle()
        ...
        words.fix_in_frame()
        words.to_corner(UL)
        technicality = OldTexText("$^{*}$From a distribution that's\\\\", 'invariant under rotational symmetries.', font_size=30)
        ...
        def show_dots_in_patch(n=2000):
            ...
            dots.set_opacity([0.25 if point[1] > 0 else 1.0 for point in dots.get_points()])
            dots.add_updater(lambda m: m)
        ...
        show_dots_in_patch()
        self.wait()
        self.play(*(Rotate(sm, PI / 3, axis=OUT + RIGHT, about_point=sphere.get_center(), run_time=2) for sm in (*sphere, patch)))
        show_dots_in_patch()
        self.wait()
        self.embed()
...
class CorrectionInsert(Scene):
    ...
    def construct(self):
        ...
        kw = dict(t2c={'compact': YELLOW, 'transitively': BLUE}, t2s={'compact': ITALIC})
        conditions = VGroup(Text('1) Find a symmetry which acts transitively', **kw), Text('2) The space must be compact', **kw))
        ...
        sphere_dots = DotCloud([sphere.pfp(math.acos(random.random()) / PI) for n in range(300)])
        ...
        for rt, r, color in zip(rect_titles, rects, [YELLOW, RED]):
            ...
            rt.set_color(color)
        compact_spaces = Group(Group(interval, interval_dots), Group(circle, circle_dots), Group(*sphere_group, sphere_dots))
        ...
        for cs, cst in zip(compact_spaces, compact_spaces.target):
            for m, mt in zip(cs, cst):
                ...
                m.target.replace(mt)
                movers.append(m)
        self.play(*map(MoveToTarget, movers), *map(ShowCreation, rects), *map(FadeIn, rect_titles))
        self.wait()
        ...
        arrows = VGroup(*(Arrow(v, 2 * v, stroke_width=2) for v in compass_directions(8)))
        ...
        self.wait()
        self.play(GrowFromCenter(plane), *map(ShowCreation, arrows))
        self.wait(3)