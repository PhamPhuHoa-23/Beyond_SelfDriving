...
def get_boundary_volume_texs():
    return ['0', '2', '2\\pi r', '4\\pi r^2', '2\\pi^2 r^3', '{8 \\over 3}\\pi^2 r^4', '\\pi^3 r^5', '{16 \\pi^3 \\over 15} r^6', '{\\pi^4 \\over 3} r^7', '{32 \\pi^4 \\over 105} r^8']

def get_volume_texs():
    return ['1', '2 r', '\\pi r^2', '{4 \\over 3} \\pi r^3', '{\\pi^2 \\over 2} r^4', '{8 \\over 15} \\pi^2 r^5', '{\\pi^3 \\over 6} r^6', '{16 \\pi^3 \\over 105} r^7', '{\\pi^4 \\over 24} r^8', '{32 \\pi^4 \\over 945} r^9', '{\\pi^5 \\over 120} r^10']

class CircumferenceToArea(InteractiveScene):

    def construct(self):
        t2c = {'{r}': BLUE}
        circum_formula = Tex('\\text{Circumference} = 2 \\pi {r}', t2c=t2c)
        radius = 3
        circle = Circle(radius=radius)
        circle.set_stroke(YELLOW, 5)
        radius_line = Line(ORIGIN, radius * RIGHT)
        radius_line.set_stroke(WHITE, 3)
        r_label = Tex('r', font_size=72)
        ...
        self.play(ShowCreation(circle), Rotate(r_group, TAU, about_point=ORIGIN), run_time=3)
        self.wait()
        circles = VGroup((circle.copy().set_width(a * circle.get_width()) for a in np.linspace(1, 0, 100)))
        circles.set_stroke(YELLOW, 3, 0.25)
        self.play(ReplacementTransform(circle.replicate(len(circles)).set_stroke(width=0, opacity=0), circles, lag_ratio=0.1), run_time=3)
        self.wait()
...
class SurfaceAreaToVolume(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(sphere), Write(mesh), run_time=2)
        self.wait()
        inner_spheres = Group((sphere.copy().set_width(a * sphere.get_width()) for a in np.linspace(0, 1, 30)))
        inner_spheres.set_color(BLUE, 0.05)
        self.remove(sphere)
        self.add(inner_spheres, mesh)
        self.play(ShowCreation(inner_spheres, lag_ratio=0.1, run_time=20))
...
class VolumeGrid(InteractiveScene):
    ...
    def construct(self):
        ...
        labels = VGroup((VGroup(*pair) for pair in zip(boundary_labels, volume_labels)))
        for label_pair, col in zip(labels, grid):
            for label, square in zip(label_pair, col):
                label.move_to(square)
                square.label = label
        self.add(grid[1:])
        row_labels = VGroup(Tex('\\partial B^n'), Tex('B^n'))
        ...
        for d, n in it.product([2, 3], [0, 1]):
            self.play(highlight_cell(n, d), Write(labels[d][n]), run_time=1)
        ...
        self.wait()
        self.play(rect.animate.surround(row_labels[0]), TransformMatchingTex(row_labels[1].copy(), row_labels[0]), run_time=1)
        self.wait()
        self.play(frame.animate.set_x(-3), FadeIn(boundary_word, lag_ratio=0.1), rect.animate.surround(row_labels[0][0]))
        self.wait()
        self.play(frame.animate.set_x(0), FadeOut(rect), FadeOut(boundary_word))
        self.wait()
        arrows = VGroup((Arrow(stack[1].get_right(), stack[0].get_corner(DR), buff=SMALL_BUFF, thickness=5, path_arc=180 * DEG) for stack in labels[2:4]))
        ...
        self.play(highlight_cell(1, 1), FadeIn(volume_labels[1]))
        self.play(highlight_cell(0, 1), FadeIn(boundary_labels[1]))
        q_marks = VGroup((VGroup((Tex('?', font_size=72).move_to(cell) for cell in col)) for col in grid[4:]))
        q_marks.set_fill(YELLOW)
        self.play(Write(q_marks, lag_ratio=0.05), Write(grid[4:], lag_ratio=0.05), Write(col_labels[4:], lag_ratio=0.05), cols_label.animate.match_x(col_labels))
        ...
        self.wait()
        for d in range(4, 10):
            self.play(knight_group.animate.move_to(grid[d - 2][1], DL), FadeOut(q_marks[d - 4][0]))
            label_copy = boundary_labels[d].copy()
            self.play(TransformMatchingTex(boundary_labels[2].copy(), label_copy), TransformMatchingTex(volume_labels[d - 2].copy(), boundary_labels[d]), run_time=1)
            self.remove(label_copy)
            self.wait()
            self.show_derivative_and_integral(grid, d, int_added_anims=[FadeOut(q_marks[d - 4][1]), TransformMatchingTex(boundary_labels[d].copy(), volume_labels[d])], skip_derivative=True)
            self.wait()
        self.play(FadeOut(knight_group), circle_cell.animate.set_fill(opacity=0))
        t2c = {'b_n': YELLOW, 'b_{n - 2}': YELLOW}
        gen_formula = Tex('V(B^n) = b_n r^n', t2c=t2c, font_size=72)
        gen_formula['r'].set_color(BLUE)
        gen_formula.to_edge(DOWN)
        gen_b_part = gen_formula['b_n'][0]
        kw = dict(font_size=48)
        c_formulas = VGroup(Tex('b_0 = 1', **kw), Tex('b_1 = 2', **kw), Tex('b_2 = \\pi', **kw), Tex('b_3 = {4 \\over 3} \\pi', **kw), Tex('b_4 = {\\pi^2 \\over 2}', **kw), Tex('b_5 = {8 \\over 15} \\pi^2', **kw), Tex('b_6 = {\\pi^3 \\over 6}', **kw), Tex('b_7 = {16 \\over 105} \\pi^3', **kw), Tex('b_8 = {\\pi^4 \\over 24}', **kw))
        ...
        for col, b_formula, label in zip(grid[1:], c_formulas[1:], volume_labels[1:]):
            ...
            b_part = b_formula[re.compile('b_.')][0]
            ...
            self.play(FadeOut(last_highlight), FadeOut(last_b_formula), FadeIn(highlight), TransformFromCopy(gen_b_part, b_part), FadeTransform(label[:len(b_formula) - 3].copy(), b_formula[3:], time_span=(0.25, 1)), Write(b_formula[2], time_span=(0.25, 1.0)))
            self.wait()
            last_highlight = highlight
            last_b_formula = b_formula
        self.play(FadeOut(last_highlight), FadeOut(last_b_formula))
        recursion_formula = Tex('b_n = {2\\pi \\over n} b_{n - 2}', t2c=t2c, font_size=72)
        alt_recursion_formula = Tex('b_n = {\\pi \\over n / 2} b_{n - 2}', t2c=t2c, font_size=72)
        recursion_formula.to_corner(DR)
        alt_recursion_formula.move_to(recursion_formula)
        self.play(gen_formula.animate.match_y(recursion_formula).to_edge(LEFT, buff=LARGE_BUFF), TransformFromCopy(gen_formula['b_n'], recursion_formula['b_n']))
        ...
        self.play(grid_group.animate.set_height(3.0, about_edge=UP), formula_group.animate.arrange(DOWN, buff=0.5, aligned_edge=LEFT).set_max_height(2).to_corner(DL))
        self.wait()
        stages = VGroup(Tex('b_8 = {\\pi \\over 4} b_6'), Tex('b_8 = {\\pi \\over 4} {\\pi \\over 3} b_4'), Tex('b_8 = {\\pi \\over 4} {\\pi \\over 3} {\\pi \\over 2} b_2'), Tex('b_8 = {\\pi \\over 4} {\\pi \\over 3} {\\pi \\over 2} {\\pi \\over 1} b_0'))
        stages.set_height(1.2)
        stages.next_to(formula_group, RIGHT, buff=2.5)
        for stage in stages:
            stage.align_to(stages[-1], LEFT)
            stage[re.compile('b_.')].set_color(YELLOW)
        mult_arrows = VGroup((Arrow(col1.get_bottom(), col2.get_bottom(), path_arc=120 * DEG) for col1, col2 in zip(grid[0::2], grid[2::2])))
        arrow_label_texs = ['\\times \\pi / 1', '\\times \\pi / 2', '\\times \\pi / 3', '\\times \\pi / 4']
        ...
        self.play(FadeOut(boundary_labels[4:]), FadeOut(volume_labels[4:]))
        self.play(Write(stages[0]))
        self.wait()
        self.play(Write(mult_arrows[-1]), FadeIn(highlights[-2:]))
        self.wait()
        self.play(TransformMatchingTex(stages[0], stages[1], key_map={'b_6': 'b_4'}, run_time=1), Write(mult_arrows[-2]), FadeIn(highlights[-3]))
        self.wait()
        self.play(TransformMatchingTex(stages[1], stages[2], key_map={'b_4': 'b_2'}, run_time=1), Write(mult_arrows[-3]), FadeIn(highlights[-4]))
        self.wait()
        self.play(TransformMatchingTex(stages[2], stages[3], key_map={'b_2': 'b_0'}, run_time=1), Write(mult_arrows[-4]), FadeIn(highlights[-5]), grid[0].animate.set_stroke(opacity=1), Write(col_labels[0]), row_labels.animate.next_to(grid, LEFT, MED_SMALL_BUFF))
        self.wait()
        ...
        self.wait()
        b8_form = stages[-1]
        gen_b_form = Tex('b_n = {\\pi^{n / 2} \\over (n / 2)!}', t2c=t2c, font_size=48)
        ...
        pis = b8_form[3:-2]
        pis_target = gen_b_form['{\\pi^{n / 2} \\over (n / 2)!}'][0]
        pis_rect = SurroundingRectangle(pis, buff=SMALL_BUFF)
        pis_rect.set_stroke(TEAL, 3)
        self.play(ShowCreation(pis_rect))
        self.wait()
        self.play(TransformMatchingTex(b8_form, gen_b_form, key_map={'b_8': 'b_n'}, matched_keys=['\\pi', '\\over']), pis_rect.animate.surround(pis_target), MoveToTarget(small_b8_form), run_time=1)
        self.play(FadeOut(pis_rect))
        self.wait()
        final_formula = Tex('V(B^n) = {\\pi^{n / 2} \\over (n / 2)!} {r}^n', t2c={'{r}': BLUE}, font_size=72)
        final_formula.next_to(grid, DOWN, buff=2.25)
        final_formula.to_edge(LEFT)
        bn_parts = VGroup((formula['{\\pi^{n / 2} \\over (n / 2)!}'] for formula in [gen_b_form, final_formula]))
        bn_rect = SurroundingRectangle(bn_parts[0])
        bn_rect.set_stroke(YELLOW, 1)
        self.play(FadeOut(small_b8_form), FadeOut(alt_recursion_formula))
        self.play(ShowCreation(bn_rect))
        self.play(TransformFromCopy(*bn_parts), TransformFromCopy(gen_formula['V(B^n) = '].copy(), final_formula['V(B^n) = ']), TransformFromCopy(gen_formula['r^n'], final_formula['{r}^n']), FadeOut(gen_b_form), FadeOut(gen_formula), bn_rect.animate.surround(bn_parts[1], buff=0.05))
        self.wait()
        self.play(bn_rect.animate.surround(final_formula, buff=0.25).set_stroke(width=2))
        self.add(final_formula)
        self.wait()

        def fill_every_other_label_from(n=2):
            for vl1, vl2 in zip(volume_labels[n::2], volume_labels[n + 2:-1:2]):
                self.play(TransformMatchingTex(vl1.copy(), vl2, path_arc=60 * DEG), run_time=1)
            self.wait()
            self.play(LaggedStart(*(TransformMatchingTex(vl.copy(), bl) for vl, bl in zip(volume_labels[n + 2::2], boundary_labels[n + 2::2])), run_time=1.5, lag_ratio=0.2))
        ...
        new_arrow_texs = ['\\times {\\pi \\over 3 / 2}', '\\times {\\pi \\over 5 / 2}', '\\times {\\pi \\over 7 / 2}', '\\times {\\pi \\over 9 / 2}']
        alt_new_arrow_texs = ['\\times {2 \\pi \\over 3}', '\\times {2 \\pi \\over 5}', '\\times {2 \\pi \\over 7}', '\\times {2 \\pi \\over 9}']
        ...
        self.play(MoveToTarget(pure_mult_arrows, lag_ratio=0.2), LaggedStart(*(TransformMatchingTex(l1, l2) for l1, l2 in zip(mult_arrow_labels, new_arrow_labels)), lag_ratio=0.2), run_time=1.5)
        self.wait()
        fill_every_other_label_from(3)
        self.play(LaggedStart((TransformMatchingTex(l1, l2, rate_func=there_and_back_with_pause) for l1, l2 in zip(new_arrow_labels, alt_new_arrow_labels)), lag_ratio=0.05, run_time=5))
        self.remove(alt_new_arrow_labels)
        self.add(new_arrow_labels)
        d1_form = Tex('V(B^1) = {\\pi^{1/2} \\over (1/2)!} {r} = 2{r}', t2c={'{r}': BLUE})
        alt_d1_form = Tex('V(B^1) = {\\sqrt{\\pi} \\over (1/2)!} {r} = 2{r}', t2c={'{r}': BLUE})
        for form in (d1_form, alt_d1_form):
            form.next_to(final_formula, RIGHT, buff=1.0, aligned_edge=DOWN)
        self.play(VGroup(final_formula, bn_rect).animate.scale(0.7, about_edge=DL), FadeTransform(final_formula.copy(), d1_form))
        self.wait()
        self.play(TransformMatchingTex(d1_form, alt_d1_form, key_map={'^{1/2}': '\\sqrt'}, matched_keys=['\\pi'], run_time=1))
        self.wait()
        half_fact = Tex('(1/2)! = {\\sqrt{\\pi} \\over 2}')
    ...
    def get_volume_labels(self):
        config = dict(t2c=self.tex_to_color, font_size=36)
        return VGroup((VGroup((Tex(tex, **config) for tex in texs)) for texs in [get_boundary_volume_texs(), get_volume_texs()]))

    def show_derivative_and_integral(self, grid, dim, upper_buff=1.25, deriv_added_anims=[], int_added_anims=[], skip_derivative=False):
        top_cell = grid[dim][0]
        low_cell = grid[dim][1]
        right_point = VGroup(top_cell, low_cell).get_right()
        down_arrow = Arrow(top_cell.get_right(), low_cell.get_right(), buff=SMALL_BUFF, thickness=5, path_arc=-180 * DEG)
        down_arrow.scale(0.8, about_point=right_point)
        up_arrow = down_arrow.copy().flip(RIGHT)
        deriv_label = Tex('{d / dr}', t2c=self.tex_to_color)
        deriv_label.next_to(up_arrow, RIGHT, SMALL_BUFF)
        int_label = Tex('\\int \\dots dr', t2c=self.tex_to_color)
        ...
        cover_rect.next_to(right_point, RIGHT, buff=0.005)
        cover_rect.shift(0.01 * RIGHT)
        if skip_derivative:
            self.play(FadeIn(cover_rect), Write(down_arrow), Write(int_label), *int_added_anims)
            self.wait()
        else:
            self.play(LaggedStart(FadeIn(cover_rect), Write(up_arrow), Write(deriv_label), *deriv_added_anims, lag_ratio=0.5))
            self.wait()
            self.play(TransformMatchingTex(deriv_label, int_label, run_time=1), ReplacementTransform(up_arrow, down_arrow), *int_added_anims)
            self.wait()
        self.play(FadeOut(down_arrow), FadeOut(int_label), FadeOut(cover_rect))
...
class CircleDerivativeFormula(InteractiveScene):
    ...
    def construct(self):
        formulas = VGroup((Tex(tex, t2c={'dA': RED_D, 'dr': BLUE}) for tex in ['dA = (2 \\pi r) dr', '{dA \\over dr} = 2 \\pi r']))
        formulas.scale(3)
        self.add(formulas[0])
...
class BuildCircleWithCombinedAnnulusses(ShowCircleAreaDerivative):
    ...
    def construct(self):
        dr = 0.1
        radius = 3.9
        rings = VGroup((Annulus(r, r + dr) for r in np.arange(0, radius, dr)))
...
class ShowSphereVolumeDerivative(ShowCircleAreaDerivative):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(42, -15, 0, ORIGIN, 8.73), inner_sphere.animate.set_clip_plane(IN, 0), run_time=3)
...
class SphereDerivativeFormula(InteractiveScene):
    ...
    def construct(self):
        formulas = VGroup((Tex(tex, t2c={'dV': RED_D, '{r}': BLUE}) for tex in ['dV = (4 \\pi {r}^2) d{r}', '{dV \\over d{r}} = 4 \\pi {r}^2']))
        formulas.scale(3)
        self.add(formulas[0])
        self.play(TransformMatchingTex(*formulas, path_arc=-90 * DEG))
        self.wait()
...
class SimpleLineWithEndPoints(InteractiveScene):
    ...
    def construct(self):
        ...
        end_points = Group((Group(Dot(), GlowDot()).move_to(point) for point in line.get_start_and_end()))
...
class ZAxisWithCircle(InteractiveScene):
    ...
    def construct(self):
        ...
        def update_shadow(shadow):
            if len(shadow) > 0 and abs(shadow[-1].get_z() - circle.get_z()) < 0.005:
                return
...
class SeparateRingsOfLatitude(InteractiveScene):
    ...
    def construct(self):
        ...
        sphere.always_sort_to_camera(self.camera)
        n_rings = 50
        rings = VGroup((Circle(radius=math.sqrt(1 - z ** 2)).move_to(z * OUT) for z in np.linspace(-1, 1, n_rings)))
        rings.set_stroke(BLUE, 2, 0.5)
        frame.reorient(4, 78, 0, (-0.03, 0.01, 0.03), 2.88)
        self.add(sphere)
        self.play(sphere.animate.set_opacity(0.2), LaggedStartMap(FadeIn, rings), run_time=3)
        self.wait()
        self.play(rings[n_rings // 2].animate.set_stroke(YELLOW, 3, 1), rings[:n_rings // 2].animate.set_stroke(opacity=0.25), rings[n_rings // 2 + 1:].animate.set_stroke(opacity=0.25))
        self.wait()
        z_tracker = ValueTracker(0)
        get_z = z_tracker.get_value
        equator = rings[n_rings // 2]
        equator.add_updater(lambda m: m.set_width(2 * math.sqrt(1 - get_z() ** 2)).move_to(get_z() * OUT))
...
class CrossLineWithCircle(InteractiveScene):
    ...
    def construct(self):
        ...
        group = Group(sphere, Tex('=', font_size=120), line, Tex('\\times', font_size=120), circle)
        group.arrange(RIGHT, buff=LARGE_BUFF)
        self.add(group)
        sphere_3d_form = Tex('x^2 + y^2 + z^2 = 1')
        sphere_3d_form.next_to(group[0], DOWN, buff=MED_LARGE_BUFF)
        line_form = Tex('z^2 \\le 1')
        line_form.match_x(line).match_y(sphere_3d_form)
        circle_form = Tex('x^2 + y^2 = 1 - z^2')
        ...
        self.wait()
        self.play(TransformMatchingTex(sphere_3d_form.copy(), circle_form, path_arc=45 * DEG))
        self.wait()
        ...
        self.play(FadeIn(z_dot), FadeIn(interval))
        ...
        self.play(FadeOut(circle_form[-3:]), circle_form[:-3].animate.shift(0.5 * RIGHT))
        self.wait()
        self.play(FadeOut(rect), FadeOut(z_dot), FadeOut(interval))
        fade_rects = VGroup((BackgroundRectangle(mob, buff=SMALL_BUFF, fill_opacity=0.5) for mob in group[0::2]))
        ...
        for label in hyper_sphere_labels[1:]:
            label.scale(0.8)
        self.play(LaggedStartMap(FadeOut, sphere_labels, shift=0.5 * UP, lag_ratio=0.5, run_time=1), FadeIn(db4_label, shift=0.5 * UP), FadeOut(group[0], LEFT), group[1].animate.move_to(midpoint(db4_label.get_right(), disk.get_left())), group[3].animate.move_to(VGroup(disk, circle)), ReplacementTransform(line, disk), FadeOut(VGroup(sphere_3d_form, circle_form, line_form)), *map(FadeOut, fade_rects))
        labels4d = VGroup(Tex('x^2 + y^2 + z^2 + w^2 = 1').next_to(db4_label, DOWN), Tex('z^2 + w^2 \\le 1').next_to(disk, DOWN), Tex('x^2 + y^2 = 1 - z^2 - w^2').next_to(circle, DOWN), Tex('x^2 + y^2 = 1').next_to(circle, DOWN))
        ...
        self.play(FadeIn(axes), FadeIn(z_dot))
        ...
        self.play(ShowCreation(rect))
        self.play(TransformMatchingTex(labels4d[2], labels4d[3]), rect.animate.surround(labels4d[3]), run_time=2)
        self.wait()
        self.play(FadeOut(rect))
        db5_label = Tex('\\partial B^5', font_size=120)
        db5_label.move_to(db4_label)
        sphere = pure_sphere
        sphere.replace(disk)
        self.play(LaggedStart(FadeOut(labels4d[:2]), FadeOut(labels4d[3]), FadeOut(disk, 0.5 * UP), FadeIn(sphere, 0.5 * UP), FadeOut(db4_label, 0.5 * UP), FadeIn(db5_label, 0.5 * UP)))
        self.wait()
        labels5d = VGroup(Tex('x^2 + y^2 + z^2 + w^2 + v^2 = 1').next_to(db5_label, DOWN), Tex('z^2 + w^2 + v^2 \\le 1').next_to(sphere, DOWN), Tex('x^2 + y^2 = 1').next_to(circle, DOWN))
        ...
        self.wait()
        self.play(TransformFromCopy(labels5d[0]['x^2 + y^2'][0], labels5d[2]['x^2 + y^2'][0], path_arc=30 * DEG), TransformFromCopy(labels5d[0]['= 1'][0], labels5d[2]['= 1'][0], path_arc=30 * DEG))
        self.wait()

    def get_sphere_labels(self, n, group):
        labels = VGroup((Tex(tex, font_size=120) for tex in [f'\\partial B^{n}', f'B^{n - 2}', f'\\partial B^2']))
...
class ShowNumericalValues(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(axes)
        formulas = VGroup((Tex(s.split(' r')[0]) for s in get_volume_texs()))
        v_lines = VGroup((axes.get_v_line_to_graph(x, graph, line_func=Line) for x in range(len(formulas))))
        ...
        for n, formula, dot in zip(it.count(), formulas, dots):
            ...
            approx = VGroup(Tex('\\approx'), DecimalNumber(func(n)))
        ...
        gen_formula = Tex('\\pi^{n/2} \\over (n/2)!')
        gen_formula.next_to(axes.i2gp(11, graph), UR)
        self.play(ShowCreation(graph), v_lines.animate.set_stroke(opacity=0.25), dots.animate.set_fill(opacity=0.25), FadeOut(last_expression[1:]), FadeTransform(last_expression[0], gen_formula), run_time=2)
        self.wait()
        if False:
            ...
            self.play(self.frame.animate.to_default_state().set_anim_args(run_time=8), ShowCreation(graph_segment, time_span=(2, 12), run_time=15), LaggedStartMap(ShowCreation, v_lines, lag_ratio=0.75, run_time=12), LaggedStartMap(FadeIn, dots, lag_ratio=0.75, run_time=12), Write(gen_formula, time_span=(7, 9)))
            self.wait()
            return
        recurrence = Tex('V_n = {2\\pi \\over n} V_{n - 2}', font_size=72, t2c={'V_n': YELLOW, 'V_{n - 2}': YELLOW})
        ...
        for n in range(1, 9):
            ...
            arrow = Arrow(dot1.get_center(), dot2.get_center(), path_arc=-75 * DEG, thickness=5, fill_color=YELLOW, buff=0.1)
            label = Tex(f'2\\pi / {n + 2}')
            label.next_to(arrow.get_center(), rotate_vector(arrow.get_vector(), 90 * DEG), buff=0.15)
            group = VGroup(dots, arrow, label)
            self.add(group)
            self.wait()
            self.remove(group)
        self.play(self.frame.animate.reorient(0, 0, 0, (5.6, 0.15, 0.0), 14.77), run_time=3)
        self.wait()

class WriteB100Volume(InteractiveScene):

    def construct(self):
        formula = Tex('B^{100} \\rightarrow {\\pi^{50} \\over 50!} \\approx 2.37 \\times 10^{-40}')
        self.add(formula)
