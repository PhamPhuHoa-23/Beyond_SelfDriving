...
class IntroSphereAnimation(InteractiveScene):
    ...
    def construct(self):
        ...
        lattitude_lines = VGroup((Circle(radius=math.sqrt(1 - z ** 2)).set_z(z) for z in np.linspace(-0.99, 0.99, 50)))
        lattitude_lines.set_stroke(TEAL, 1, 0.5)
        self.play(ShowCreation(circle))
        self.remove(circle)
        self.play(LaggedStart((TransformFromCopy(circle, lat_line) for lat_line in lattitude_lines), lag_ratio=0.05), frame.animate.reorient(-2, 56, 0, (0.02, -0.09, -0.07), 2.83), run_time=6)
        ...
        self.wait()
        self.play(LaggedStart((ReplacementTransform(sphere.copy().scale(0.99).set_opacity(0), lat_sphere) for lat_sphere in lattitude_spheres), lag_ratio=0.05), FadeOut(sphere), angle_tracker.animate.set_value(80 * DEG), run_time=6)
        self.wait()
        self.play(angle_tracker.animate.set_value(360 * DEG), run_time=12)
        self.wait()

class IntroText(InteractiveScene):

    def construct(self):
        group = VGroup(Text('Exploring high-dimensional spheres', font_size=90), Text('Delivered at UC Santa Cruz on February 17, 2026'))
        group[1].set_color(GREY_A)
        group.arrange(DOWN, buff=0.5)
        group.set_width(FRAME_WIDTH - 3)
        self.play(LaggedStart(Write(group[0], lag_ratio=0.1), FadeIn(group[1], 0.25 * DOWN), lag_ratio=0.6))
        self.wait()
...
class CornerDistance(InteractiveScene):
    ...
    def construct(self):
        ...
        pythag = Tex('\\sqrt{1^2 + 1^2 + \\cdots + 1^2 + 1^2}')
        sqrt3 = Tex('= \\sqrt{N}')
        r_eq = Tex('r = \\sqrt{N} - 1')
        words.move_to(2 * UP)
...
class Hypercube(InteractiveScene):
    ...
    def construct(self):
        ...
        hyp_label = Tex('\\sqrt{3 + 1}', font_size=24)
        hyp_label.rotate(90 * DEG, RIGHT)
        hyp_label.next_to(hyp.get_center(), OUT, buff=SMALL_BUFF)
        self.play(hypercube.animate.set_stroke(WHITE, 1, 0.5), ShowCreation(leg1), FadeIn(root3_label))
        self.play(ShowCreation(leg2), FadeIn(one_label))
        self.wait()
        self.play(FadeTransformPieces(VGroup(root3_label, one_label).copy(), hyp_label), ShowCreation(hyp))
        self.wait(12)
    ...
    def get_cube(self, bases, stroke_color=WHITE, stroke_width=2):
        ...
        for bits in it.product(*n * [[0, 1]]):
            base_point = sum([bit * basis for bit, basis in zip(bits, bases)])
...
class REqExample(InteractiveScene):
    ...
    def construct(self):
        tex = Tex('r = \\sqrt{10} - 1 \\approx 2.162')
        self.add(tex)

class HyperbolaSquare(InteractiveScene):

    def construct(self):
        square = VGroup(Line(UL, UR), Line(UR, DR), Line(DR, DL), Line(DL, UL))
        square.set_height(4)
        square_shadows = VGroup((square.copy().scale(0.95 ** n).shift(0.05 * DR * n).set_stroke(opacity=0.5 / n) for n in range(1, 20)))
        center_dot = Dot()
        ...
        diag_label = Tex('\\sqrt{N}')
        diag_label.next_to(diag_line.get_center(), UL, SMALL_BUFF)
        diag_label.set_backstroke(BLACK, 5)
        self.add(square)
        self.add(square_shadows)
        self.play(GrowFromCenter(center_dot), ShowCreation(side_line), FadeIn(side_label, 0.5 * RIGHT))
        self.wait()
        self.play(ShowCreation(diag_line), FadeIn(diag_label, 0.5 * UR))
        self.wait()
        ...
        self.wait()
        self.play(Transform(square, warp_square), Transform(square_shadows, warp_square_shadows), MoveToTarget(side_line), MoveToTarget(diag_line), side_label.animate.next_to(side_line.target, DOWN, SMALL_BUFF), diag_label.animate.shift(0.4 * DOWN + 0.2 * LEFT), run_time=3)
        self.wait()
        frame = self.frame
        lil_radius = 0.6
        ...
        self.play(LaggedStartMap(GrowFromCenter, circles), frame.animate.set_height(9))
        self.wait()
        ...
        alt_groups = VGroup((group.copy().rotate(theta, about_point=ORIGIN) for theta in angles))
        alt_groups.fade(0.75)
        corner_label = Tex('2^N \\text{ corners}', font_size=60)
        corner_label.to_corner(UR)
        corner_label.fix_in_frame()
        self.play(FadeIn(corner_label), LaggedStart((TransformFromCopy(group.copy().set_fill(opacity=0), alt_group, path_arc=angle) for angle, alt_group in zip(angles, alt_groups)), lag_ratio=0.01, run_time=3), frame.animate.set_height(11), run_time=3)
        self.wait()

    def get_warp_square(self, square, scale_factor=1.7):
        hyper = FunctionGraph(lambda x: math.sqrt(1 + x ** 2), x_range=(-2, 2, 0.02))
        warp_square = VGroup((hyper.copy().put_start_and_end_on(*side.get_start_and_end()) for side in square))
...
class AreaCircleOverAreaSquareThenVolume(InteractiveScene):
    ...
    def construct(self):
        ...
        c_tex = '{CC \\over CC}'
        s_tex = '{SS \\over SS}'
        frac = Tex(f'{{\\text{{Area}}\\left( {c_tex} \\right) \\over \\text{{Area}}\\left( {s_tex} \\right)}}', font_size=font_size)
        area_words = frac['\\text{Area}']
        ...
        self.wait(0.1)
        self.play(Write(frac, lag_ratio=0.1), LaggedStart(Write(circle), Write(square), lag_ratio=0.5))
        self.wait()
        equals = Tex('=', font_size=90)
        equals.rotate(90 * DEG)
        equals.next_to(frac, DOWN)
        approx = Tex('\\approx', font_size=font_size)
        value = DecimalNumber(PI / 4, num_decimal_places=3, font_size=font_size)
        rhs = Tex('{\\pi (1)^2 \\over 2 \\times 2}', font_size=font_size, t2c={'\\pi (1)^2': circle_color, '2 \\times 2': square_color})
        ...
        new_frac = Tex('{(4/3) \\pi (1)^3 \\over 2 \\times 2 \\times 2}', t2c={'(4/3) \\pi (1)^3': circle_color, '2 \\times 2 \\times 2': square_color}, font_size=font_size)
        new_frac.next_to(equals, DOWN)
        self.play(Write(sphere_mesh, lag_ratio=0.01), FadeOut(VGroup(equals, rhs, approx, value)), FadeTransform(circle, sphere), FadeTransform(square, cube), *(FadeTransformPieces(a_word, v_word) for v_word, a_word in zip(volume_words, area_words)))
        self.play(Write(equals), FadeIn(new_frac, DOWN))
        self.wait()
...
class BigVolumeRatio(InteractiveScene):
    ...
    def construct(self):
        fraction = Tex('\\text{Vol}\\Big(\\text{100D Unit Ball}\\Big) \\over 2^{100}')
        fraction['2^{100}'].scale(1.5, about_edge=UP).shift(SMALL_BUFF * DOWN)
        numerator = fraction['\\text{Vol}\\Big(\\text{100D Unit Ball}\\Big)']
        ...
        self.wait(3)
        self.play(VFadeIn(randy), randy.change('confused'), ShowCreation(numerator_rect))
        self.play(Blink(randy))
        self.wait()
        new_numerator = Tex('\\pi^{50} / 50!', font_size=72)
        new_numerator.move_to(numerator, DOWN)
        self.play(FadeOut(numerator_rect), fraction['\\over'].animate.match_width(new_numerator, stretch=True), FadeTransform(numerator, new_numerator), randy.change('tease'))
        self.play(Blink(randy))
        self.wait()
...
class Derivatives(InteractiveScene):
    ...
    def construct(self):
        kw = dict(t2c={'r': BLUE})
        power_rules = VGroup(Tex('\\frac{d}{dr} r = 1', **kw), Tex('\\frac{d}{dr} r^2 = 2r', **kw), Tex('\\frac{d}{dr} r^3 = 3r^2', **kw), Tex('\\frac{d}{dr} r^4 = 4r^3', **kw), Tex('\\vdots', **kw))
...
class Integrals(InteractiveScene):
    ...
    def construct(self):
        ...
        inv_power_rules = VGroup(Tex('\\int_0^R 1 \\, dr = R', **kw), Tex('\\int_0^R r \\, dr = \\frac{1}{2} R^2', **kw), Tex('\\int_0^R r^2 \\, dr = \\frac{1}{3} R^3', **kw), Tex('\\int_0^R r^3 \\, dr = \\frac{1}{4} R^4', **kw), Tex('\\vdots', **kw))
...
class DrawTorus(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(circles)
        self.wait()
        self.play(frame.animate.reorient(19, 63, 0, (0.05, -0.15, -0.75), 9.57), circle1.animate.rotate(90 * DEG, LEFT).move_to(r1 * RIGHT), circle2.animate.set_width(2 * r1).move_to(ORIGIN), run_time=2)
        torus = Torus(r1=r1, r2=r2)
        partial_torus = Torus()
        partial_torus.set_color(GREY, 0.5)
        partial_torus.always_sort_to_camera(self.camera)
        self.play(UpdateFromAlphaFunc(partial_torus, lambda m, a: m.pointwise_become_partial(torus, 0, a, axis=0)), Rotate(circle1, TAU, about_point=ORIGIN), run_time=5)
        self.wait()
...
class CommentOnGeneralFormula(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(morty.says('Beautiful!', mode='hooray'), randy.change('hesitant', morty.eyes))
        self.play(Blink(morty))
        self.wait()
        self.play(morty.debubble(mode='guilty'), randy.says('What about\nodd n?', mode='confused', look_at=3 * LEFT + DOWN))
        self.play(Blink(randy))
        self.wait()
...
class SquarePyramid(InteractiveScene):
    ...
    def construct(self):
        ...
        n_squares = 40
        max_side_length = 2
        squares = VGroup((Square(side_length) for side_length in np.linspace(max_side_length, 0.01, n_squares)))
        ...
        self.play(LaggedStartMap(FadeIn, squares, shift=RIGHT, lag_ratio=0.35, run_time=8))
        label = Tex('\\text{Vol.} = \\frac{1}{3} \\text{B} \\times \\text{H}', font_size=72)
        label.fix_in_frame()
        label.to_edge(DOWN)
...
class ShellTimesThickness(InteractiveScene):
    ...
    def construct(self):
        equation = Tex('V(\\partial B^n) \\times {1 \\over n} = V(B^n)', font_size=72)
        shell_rect = SurroundingRectangle(equation['V(\\partial B^n)']).set_stroke(TEAL, 2)
        thickness_rect = SurroundingRectangle(equation['{1 \\over n}']).set_stroke(YELLOW, 2)
        volume_rect = SurroundingRectangle(equation['V(B^n)']).set_stroke(BLUE, 2)
...
class VolumeRatio(InteractiveScene):
    ...
    def construct(self):
        group = VGroup(Tex('{8  \\cdot \\big({4 \\over 3} \\pi \\big) \\over 2 \\times 2 \\times 2}'), Tex('='), Tex('{4 \\over 3} \\pi'))
        group.arrange(RIGHT, buff=0.25)
        self.add(group)
        self.clear()
        group = VGroup(Tex('{2^{100}  \\cdot \\big(\\pi^{50} / 50! \\big) \\over 2^{100}}'), Tex('='), Tex('{\\pi^{50} \\over 50!}'))
