...
class IntervalWithSample(Group):
    ...
    def __init__(self, width=4, numbers_font_size=36, x_range=(-1, 1, 0.25), label_step=0.5, marker_length=0.25, marker_width=(0, 15), marker_color=BLUE_B, dec_label_font_size=30):
        self.number_line = NumberLine(x_range, width=width)
        self.number_line.add_numbers((-1, 0, 1), font_size=numbers_font_size, num_decimal_places=0)
        ...
        self.dec_label.add_updater(lambda m: m.next_to(self.marker.get_end(), normalize(self.marker.get_vector()), SMALL_BUFF))
        super().__init__(self.x_tracker, self.number_line, self.marker, self.dec_label)
...
class RandomSumsOfSquares(InteractiveScene):
    ...
    def get_labeled_intervals(self, label_texs, font_size=48, buff=MED_LARGE_BUFF):
        ...
        labels = VGroup((Tex(tex, font_size=font_size, t2c=self.tex_to_color) for tex in label_texs))
    ...
    def get_probability_question(self, sum_tex='x^2 + y^2'):
        question = Tex(f'P\\left({sum_tex} \\le 1 \\right)', t2c=self.tex_to_color)
    ...
    def get_evaluation_object(self, intervals):
        ...
        rhs.add_updater(lambda m: m.set_value(sum([interval.x_tracker.get_value() ** 2 for interval in intervals])))
...
class SumOfTwoSquares(RandomSumsOfSquares):
    ...
    def construct(self):
        ...
        self.play(frame.animate.to_default_state(), FadeIn(question), run_time=2)
        self.wait()
        evaluation.update()
        ...
        xy_coord_label = Tex('(x, y)', t2c={'x': BLUE, 'y': YELLOW}, font_size=24)
        xy_coord_label.add_updater(lambda m: m.next_to(xy_dot.get_center(), UR, buff=SMALL_BUFF))
        self.play(question.animate.set_height(0.7).to_corner(UR), FadeOut(evaluation.clear_updaters()), Transform(intervals[0].number_line, axes.x_axis), Transform(intervals[1].number_line, axes.y_axis), intervals[0].dec_label.animate.set_opacity(0), intervals[1].dec_label.animate.set_opacity(0), intervals[0].marker.animate.set_opacity(0), intervals[1].marker.animate.set_opacity(0), FadeIn(x_dot), FadeIn(y_dot), intervals[0].label.animate.next_to(axes.x_axis.get_end(), RIGHT, SMALL_BUFF), intervals[1].label.animate.next_to(axes.y_axis.get_end(), UP, SMALL_BUFF), run_time=2)
        VGroup(h_line, v_line).update()
        self.play(ShowCreation(h_line, suspend_mobject_updating=True), ShowCreation(v_line, suspend_mobject_updating=True), TransformFromCopy(x_dot, xy_dot, suspend_mobject_updating=True), TransformFromCopy(y_dot, xy_dot, suspend_mobject_updating=True), FadeIn(xy_coord_label[0::2]), TransformFromCopy(intervals[0].label, xy_coord_label[1]), TransformFromCopy(intervals[1].label, xy_coord_label[3]))
        ...
        self.play(self.randomize_intervals(intervals, frequency=0.2, run_time=10), FadeIn(square))
        self.wait(3)
        ...
        underline = Underline(question['x^2 + y^2 \\le 1'], buff=0, stretch_factor=1)
        underline.match_style(circle)
        self.play(ShowCreation(underline))
        self.wait()
        self.play(ReplacementTransform(underline, circle, run_time=1.5))
        self.wait()
        self.play(intervals[0].x_tracker.animate.set_value(0.6), intervals[1].x_tracker.animate.set_value(0.8))
        ...
        pythag = Tex('x^2 + y^2 = {r}^2', t2c=self.tex_to_color, font_size=72)
        pythag.to_edge(RIGHT, buff=LARGE_BUFF)
        self.play(ShowCreation(r_line), FadeIn(r_label, 0.25 * r_line.get_vector()))
        self.play(Write(pythag))
        self.wait()
        self.play(FadeOut(r_line), FadeOut(r_label), FadeOut(pythag), Restore(h_line), Restore(v_line))
        ...
        xyz_coord_label = Tex('(x, y, z)', t2c=self.tex_to_color, font_size=30)
        ...
        self.play(frame.animate.reorient(-26, 79, 0, (-0.13, 0.08, 0.47), 10.09), Write(z_interval.number_line), Write(z_axis_label), run_time=2)
        self.play(TransformFromCopy(xy_dot, xyz_dot, suspend_mobject_updating=True), xy_dot.animate.set_opacity(0), TransformMatchingTex(xy_coord_label, xyz_coord_label), ShowCreation(z_line, suspend_mobject_updating=True), run_time=1)
        ...
        self.remove(square)
        self.play(Restore(cube), Write(sphere_mesh, lag_ratio=0.001, time_span=(1, 3)), FadeOut(circle, time_span=(1, 3)), frame.animate.reorient(-41, 69, 0, (-0.34, -0.78, -0.59), 12.47).set_anim_args(run_time=4))
        self.wait()
        dot_history = DotHistory(xyz_dot.get_center)
        self.add(dot_history, xyz_dot, z_line)
        intervals.add(z_interval)
        self.play(self.randomize_intervals(intervals, frequency=0.2, run_time=20), frame.animate.reorient(14, 62, 0, (1.36, 0.96, 0.63), 12.1), run_time=20)
...
class AskAboutLargeSumOfSquares(RandomSumsOfSquares):
    ...
    def construct(self):
        intervals = self.get_intervals(8, width=6, numbers_font_size=24, dec_label_font_size=24, marker_length=0.15)
        intervals.set_height(7)
        intervals.to_edge(LEFT, buff=LARGE_BUFF)
        to_remove = intervals[-3:-1]
        dots = Tex('\\vdots', font_size=90)
        dots.move_to(to_remove)
        intervals.remove(*to_remove)
        labels = VGroup((Tex(f'x_{{{int(n)}}}', font_size=36) for n in [*range(1, 6), 100]))
        labels.set_submobject_colors_by_gradient(BLUE, YELLOW)
        for label, interval in zip(labels, intervals):
            label.next_to(interval.number_line.get_start(), LEFT, MED_SMALL_BUFF)
        question = Tex('P\\left(x_1^2 + x_2^2 + \\cdots + x_{100}^2 \\le 1 \\right)', t2c={'x_1': labels[0].get_color(), 'x_2': labels[1].get_color(), 'x_{100}': labels[-1].get_color()})
        question.to_corner(UR)
        self.add(intervals)
        self.add(labels)
        self.add(dots)
...
class DotProductOfUnitVectors(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(ShowCreation(dashed_line), FadeIn(proj_line), GrowFromCenter(proj_brace), AnimationGroup(TransformFromCopy(v1.label, label[0]), TransformFromCopy(v2.label, label[1]), Write(label[2])), lag_ratio=0.5))
        self.wait()
        proj_group.resume_updating()
        self.add(proj_group)
        self.play(v1.angle_tracker.animate.set_value(0.2), run_time=3)
        self.wait()
        self.play(v2.angle_tracker.animate.set_value(0.25), run_time=3)
        self.wait()
        for value in [-0.23, 0.2]:
            self.play(v1.angle_tracker.animate.set_value(value), run_time=5)
            self.wait()
    ...
    def get_projection_group(self, v1, v2):
        ...
        proj_line = Line(v2.get_start(), interpolate(v2.get_start(), v2.get_end(), dp))
        proj_line.set_stroke(WHITE, 8)
        orientation = cross(v1.get_vector(), v2.get_vector())[2]
        proj_brace = LineBrace(proj_line, buff=SMALL_BUFF, direction=UP if dp > 0 else DOWN)
        label = VGroup(v1.label, v2.label).copy()
        label.scale(0.5)
        label.arrange(RIGHT, buff=0.25)
        label.add(Tex('\\cdot').move_to(label))
...
class Random3DVectors(InteractiveScene):
    ...
    def construct(self):
        ...
        dp_label = Tex('\\vec{\\textbf{v}} \\cdot \\vec{\\textbf{w}} = 0.00')
        dp_label['\\vec{\\textbf{v}}'].set_color(PINK)
        dp_label['\\vec{\\textbf{w}}'].set_color(YELLOW)
...
class Distributions(InteractiveScene):
    ...
    def construct(self):
        axes = Axes((-1, 1, 0.2), (0, 1), width=6, height=4)
