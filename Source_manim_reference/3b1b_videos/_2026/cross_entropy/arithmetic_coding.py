...
class ArithmeticCodingDiagram(Group):
    ...
    def __init__(self, width=12, buff_to_bars=0.25, show_decimal_labels=True, interval_subdivisions=10, char_alphabet=CHAR_ALPHABET, highlight_colors=(GREEN_E, GREEN_D), context=' '):
        self.layers = VGroup()
    ...
    def populate_intervals(self, x_min, x_max):
        ...
        for scale in range(-1, min_scale - 1, -1):
            ...
            for lower_bound_int in range(int(x_min / step) - 1, int(x_max / step) + 1):
                ...
                interval = DynamicInterval(x_range=(low, high), width=get_norm(self.unit_interval.n2p(high) - self.unit_interval.n2p(low)), subdivisions=self.interval_subdivisions, include_numbers=self.show_decimal_labels, include_endpoint_numbers=False, number_font_size=24 + scale * 2)
    ...
    def get_new_layer(self, buff=0.1):
        ...
        layer = StackedProbDistribution(distribution, labels=self.char_labels_template.copy(), width=mob_above.get_width())
    ...
    def renormalize_animation(self, x_min, x_max, run_time=3, center=ORIGIN, center_curr_text=False, **kwargs):
