...
def get_log_image(log_image_path, scale_factor, resolution=(51, 101)):
    log_image = TexturedSurface(Square3D(resolution=resolution), log_image_path)
...
def get_image_slice(image: TexturedSurface, delta_u: float, u_min: float):
    ...
    image_slice = TexturedSurface(Square3D(resolution=(int(delta_u * nu) + 1, nv)), image.texture_paths['LightTexture'])
    image_slice.set_shading(0, 0, 0)
    image_slice.deactivate_depth_test()
    image_slice.set_image_coords_by_uv_func(lambda u, v: (u_min + delta_u * u, v))
...
def get_nested_square_grid(n_rows=8, n_recursions=6, height=4, stroke_color=WHITE, stroke_width=2, stroke_width_decay_factor=0.85, scale_factor=2):
    ...
    grid.remove(*[square for square in grid if np.all(np.abs(square.get_center()) < height / 2 / scale_factor)])
    result = VGroup(grid)
...
class PrintGalleryZoomStages(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(exp_tracker.animate.set_value(-5), run_time=10)
...
class ZoomWithColors(InteractiveScene):
    ...
    def construct(self):
        ...
        dec.to_corner(DL)
        dec.f_always.set_value(exp_tracker.get_value)
        root = Path(self.file_writer.get_output_file_rootname().parent.parent, 'Paul Segments/SVGs for Print Gallery Outlines/Straight')
        person_outline, person_mask, frame, building, arcade = images = Group((ImageMobject(str(Path(root, name + '.png'))) for name in ['PersonOutline', 'PersonMask', 'Frame', 'Building', 'Arcade']))
        person = Group(person_mask.set_opacity(0.2), person_outline)
        person.set_height(8.8).move_to(np.array([-2.41928692, -5.89659999, 0.0]))
        frame.set_height(7.59924).move_to(np.array([-0.65032366, -0.76692351, 0.0]))
        building.set_height(1.2807975113391876).move_to(np.array([-0.75733024, 0.36813092, 0.0]))
        arcade.set_height(0.24800574034452438).move_to(np.array([0.0865169, 0.01073422, 0.0]))
        ...
        self.play(exp_tracker.animate.set_value(1 - 1 * math.log(256)).set_anim_args(rate_func=linear), FadeOut(person, time_span=(0, 2)), FadeIn(frame, time_span=(0, 5), rate_func=there_and_back_with_pause), FadeIn(building, time_span=(3, 10), rate_func=there_and_back_with_pause), FadeIn(arcade, time_span=(7, 12), rate_func=there_and_back_with_pause), FadeIn(small_person, time_span=(9, 12)), run_time=12)
...
class NoteThe256(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(box_exp_tracker.animate.set_value(math.log(256)).set_anim_args(time_span=(0, 10)), frame_exp_tracker.animate.set_value(math.log(256) - 1.5).set_anim_args(time_span=(4, 12)))
        self.wait()
...
class CreatePiHouseLog(InteractiveScene):
    ...
    def construct(self):
        piece = TexturedSurface(Square3D(resolution=(1000, 1000)), Path(get_texture_folder(), 'UltraHighResPiHouse.png'))
...
class PalleteOfFunctions(InteractiveScene):
    ...
    def construct(self):
        frame = self.frame
        frame.set_height(9)
        examples = VGroup(self.get_input_output_grid('{c} \\cdot z', lambda z: (1.5 + 0.8j) * z), self.get_input_output_grid('z^2', lambda z: z ** 2), self.get_input_output_grid('z^3', lambda z: z ** 3), self.get_input_output_grid('e^z', np.exp, in_x_range=(-7, 7), out_x_range=(-7, 7), in_grid_dims=(25, 16), in_grid_height=6.25, in_grid_z0=-2), (log_example := self.get_input_output_grid('\\ln(z)', np.log, in_x_range=(-2, 2), out_x_range=(-4, 4), in_grid_dims=(16, 16), in_grid_height=4, in_grid_z0=-2 - 2j)), self.get_input_output_grid('\\cos(z)', np.cos, in_grid_dims=(8, 16), in_grid_z0=-2), self.get_input_output_grid('\\sin(z)', np.sin, in_grid_dims=(8, 16), in_grid_z0=-2), self.get_input_output_grid('\\zeta(z)', lambda z: z), self.get_input_output_grid('\\wp(z)', lambda z: z), self.get_input_output_grid('j(\\tau)', lambda z: z))
        ...
        for example in examples[3:]:
            ...
            q_marks = VGroup((Tex('?').set_height(0.5 * planes[0].get_height()).move_to(piece) for piece in planes))
            cover_rects.add(rect)
            all_q_marks.add(q_marks)
        self.add(cover_rects)
        self.add(all_q_marks)
        self.play(frame.animate.reorient(0, 0, 0, (7, -3, 0.0), 15.5), run_time=2)
        exp_example, log_example = exp_log = examples[3:5]
        rect = SurroundingRectangle(exp_log, buff=0.5)
        self.play(ShowCreation(rect), FadeOut(cover_rects[:2]), FadeOut(all_q_marks[:2]))
        self.play(frame.animate.reorient(0, 0, 0, (0.64, -7.71, 0.0), 7.26), examples[:3].animate.fade(0.8), FadeOut(examples[5:]), FadeOut(cover_rects[2:], time_span=(3, 4)), FadeOut(all_q_marks[2:], time_span=(3, 4)), rect.animate.set_stroke(width=1), run_time=4)
        self.wait()
        self.remove(out_grid)
        self.play(FadeOut(out_grid.copy(), time_span=(0, 1)), TransformFromCopy(log_grid, out_grid), run_time=4)
        self.wait()
        return
        self.play(rect.animate.surround(exp_example, buff=0.5), log_example.animate.fade(0.8), frame.animate.match_y(exp_example).set_height(6.5), run_time=2)
        self.wait()

    def get_input_output_grid(self, tex_label, func, in_x_range=(-2, 2), out_x_range=(-4, 4), in_grid_dims=(8, 8), in_grid_height=2, in_grid_z0=0, plane_height=2, in_grid_color=BLUE, out_grid_color=PINK):
        func_label = Tex(tex_label, font_size=72)
        in_plane, out_plane = planes = VGroup(ComplexPlane(in_x_range, in_x_range, faded_line_ratio=0), ComplexPlane(out_x_range, out_x_range, faded_line_ratio=0))
        for plane in planes:
...
class TheExponential(InteractiveScene):
    ...
    def construct(self):
        ...
        func_label = Tex('e^{x}', font_size=72)
        ...
        self.play(GrowArrow(func_arrow), FadeIn(func_label, 0.25 * func_arrow.get_vector()), TransformFromCopy(in_dots, out_dots, lag_ratio=0.01, run_time=2))
        self.wait()
        frame = self.frame
        ...
        active_func_arrow.add_updater(lambda m: m.set_points_by_ends(in_line.n2p(get_x()), out_line.n2p(get_y()), buff=0.5))
        active_func_label = Tex('e^{+0.00} = 1.000')
        ...
        output_labels = VGroup((Tex(f'e^{{{n}}}', font_size=36).next_to(out_line.n2p(np.exp(n)), UP, MED_SMALL_BUFF) for n in range(4)))
        ...
        self.play(MoveToTarget(all_dots), ReplacementTransform(func_arrow, active_func_arrow), FadeTransformPieces(func_label, active_func_label), FadeIn(in_dot), FadeIn(out_dot))
        self.wait()
        ...
        self.wait()
        self.play(x_tracker.animate.set_value(1), frame.animate.set_x(5), run_time=5)
        self.add(output_labels[1])
        self.wait()
        self.play(x_tracker.animate.set_value(2), frame.animate.set_x(5))
        self.add(output_labels[2])
        self.wait()
        self.play(x_tracker.animate.set_value(3), frame.animate.set_height(16).set_x(17), run_time=2)
        self.add(output_labels[3])
        self.wait()
        self.play(x_tracker.animate.set_value(-1), frame.animate.set_height(8).move_to(ORIGIN), run_time=8)
        ...
        x_max = 7
        in_plane, out_plane = [ComplexPlane((-x_max, x_max), (-x_max, x_max), unit_size=0.4, faded_line_ratio=0, background_line_style=dict(stroke_color=BLUE_D, stroke_width=1), axis_config=dict(stroke_color=GREY_A, stroke_width=1)) for _ in range(2)]
        ...
        self.play(LaggedStart(AnimationGroup(MoveToTarget(in_line), Restore(in_plane)), AnimationGroup(MoveToTarget(out_line), Restore(out_plane)), lag_ratio=0.3), ReplacementTransform(active_func_arrow, func_arrow), FadeIn(func_label, time_span=(1.5, 2)), run_time=2)
        self.wait()
        ...
        z_label = VGroup(Tex('z = '), DecimalNumber(complex(1, 1)))
        ...
        self.play(FadeIn(in_dot), FadeIn(out_dot), FadeIn(z_label), in_plane.background_lines.animate.set_stroke(opacity=0.5), out_plane.background_lines.animate.set_stroke(opacity=0.5))

        def apply_exp_to_input_shape(mob):
            mob.apply_points_function(lambda ps: np.array([out_plane.n2p(np.exp(in_plane.p2n(p))) for p in ps]), about_point=ORIGIN)
            return mob
        ...
        for x in x_values:
            ...
            self.play(z_tracker.animate.increment_value(complex(0, TAU)), ShowCreation(v_line), ShowCreation(out_circle), run_time=6)
            self.wait()
        ...
        left_rect.stretch(0.01, 1, about_edge=DOWN)
        rad_label = Text('+1 Radian', font_size=30)
        rad_label.add_updater(lambda m: m.move_to(out_plane.n2p(np.exp(z_tracker.get_value() - 0.5j - 0.3))))
        rad_label.set_backstroke(BLACK, 3)
        sector = always_redraw(lambda: apply_func_between_planes(left_rect.copy(), np.exp, in_plane, out_plane))
        two_pi_brace = Brace(v_lines, RIGHT, SMALL_BUFF)
        two_pi_label = two_pi_brace.get_tex('+2\\pi i')
        self.play(z_tracker.animate.set_value(2), frame.animate.reorient(0, 0, 0, (-2.7, 0.42, 0.0), 2.48), run_time=1)
        self.play(Restore(left_rect), GrowFromPoint(brace, brace.get_bottom()), FadeIn(brace_label, 0.1 * UP), z_tracker.animate.increment_value(1j), run_time=2)
        self.wait()
        sector.update()
        rad_label.update()
        rad_label.suspend_updating()
        self.play(TransformFromCopy(left_rect, sector), FadeTransformPieces(brace_label, rad_label), frame.animate.reorient(0, 0, 0, (5.8, 1.11, 0.0), 3.22), run_time=3)
        self.wait()
        rad_label.resume_updating()
        left_group = VGroup(left_rect, brace, brace_label)
        for n in range(5):
            self.play(frame.animate.to_default_state(), z_tracker.animate.increment_value(1j), left_group.animate.shift(z_unit * UP), run_time=1)
            self.wait()
        self.play(z_tracker.animate.increment_value((TAU - 6) * 1j), FadeOut(left_rect), FadeOut(sector), FadeOut(rad_label), ReplacementTransform(brace, two_pi_brace), FadeTransform(brace_label, two_pi_label))
        self.wait()
        self.play(FadeOut(two_pi_brace), FadeOut(two_pi_label))
        brace = Brace(v_lines, LEFT, buff=SMALL_BUFF)
        brace_label = brace.get_tex('2\\pi')
        ...
        self.play(FadeOut(out_circles), LaggedStartMap(ShowCreation, v_lines, lag_ratio=0.1), run_time=2)
        self.play(LaggedStart((TransformFromCopy(line, circle, path_arc=(0, 0.3 * PI)) for line, circle in zip(v_lines, out_circles)), lag_ratio=0.25, run_time=5))
        self.wait()
        top_arrows = VGroup((Arrow(vl1.get_top(), vl2.get_top(), path_arc=-180 * DEG, thickness=2, buff=0.1, fill_color=TEAL) for vl1, vl2 in zip(v_lines, v_lines[1:])))
        top_labels = VGroup((Tex('+1', font_size=20).next_to(arrow, UP, buff=0.05) for arrow in top_arrows))
        self.play(LaggedStartMap(Write, top_arrows, lag_ratio=0.25), LaggedStartMap(FadeIn, top_labels, shift=0.1 * UP, lag_ratio=0.25))
        self.wait()
        out_arrows = VGroup()
        scale_labels = VGroup()
        for c1, c2 in zip(out_circles[2:], out_circles[3:]):
            arrows = VGroup((Arrow(c1.pfp(a), c2.pfp(a), buff=0.1 * c1.get_width(), thickness=3) for a in np.arange(0, 1, 1.0 / 8)))
            arrows.set_fill(YELLOW_E)
            arrows.set_backstroke(BLACK, 3)
            out_arrows.add(arrows)
            scale_label = Tex('\\times e')
        ...
        self.play(FadeOut(out_circles[:2]), FadeOut(out_circles[3:]), FadeOut(v_lines[:2]), FadeOut(v_lines[3:]), z_tracker.animate.set_value(0))
        self.wait()
        for n in [0, 1]:
            self.play(LaggedStartMap(GrowArrow, out_arrows[n], lag_ratio=0.01), FadeIn(scale_labels[n], shift=0.1 * RIGHT, scale=2), TransformFromCopy(out_circles[n + 2], out_circles[n + 3]), TransformFromCopy(v_lines[n + 2], v_lines[n + 3]), z_tracker.animate.set_value(n + 1), run_time=3)
            self.wait()
        self.play(FadeOut(VGroup(out_arrows, scale_labels, brace, brace_label, top_arrows, top_labels), lag_ratio=0.02), FadeOut(Group(in_dot, out_dot, z_label), lag_ratio=0.1), FadeOut(v_line_ghosts), FadeOut(circle_ghosts), v_lines.animate.set_stroke(opacity=1), out_circles.animate.set_stroke(opacity=1))
        grid_density = 4
        ...
        self.play(Write(in_grid), out_circles.animate.set_stroke(WHITE, 1), v_lines.animate.set_stroke(WHITE, 1))
        self.wait()
        self.remove(in_grid)
        self.add(in_grid_ghost)
        self.play(out_plane.background_lines.animate.set_stroke(opacity=0.25), *(ReplacementTransform(in_square.copy(), out_square, path_arc=0.35 * z.imag + 0.05 * (z.real + 2)) for in_square, out_square in zip(in_grid, out_grid) for z in [in_plane.p2n(in_square.get_center())]), run_time=3)
        self.wait()
        ...
        self.play(ShowSubmobjectsOneByOne(solid_in_grid), ShowSubmobjectsOneByOne(solid_out_grid), run_time=10, rate_func=lambda a: 0.5 + 0.5 * a)
        self.remove(solid_in_grid, solid_out_grid)
        self.wait()
        v_lines.set_stroke(BLUE, 3)
        out_circles.set_stroke(YELLOW, 3)
        self.play(*(LaggedStartMap(ShowCreation, group, lag_ratio=0.5, run_time=3) for group in [v_lines, out_circles]), in_grid_ghost.animate.set_stroke(opacity=0.25), out_grid.animate.set_stroke(opacity=0.25))
        self.play(LaggedStart((TransformFromCopy(line, circle, path_arc=(0, 0.25 * PI)) for line, circle in zip(v_lines, out_circles)), lag_ratio=0.1, run_time=3))
        self.wait()
        ...
        full_v_lines = VGroup((Line(in_plane.c2p(x, -x_max), in_plane.c2p(x, x_max)).insert_n_curves(100) for x in range(-x_max, x_max + 1)))
        full_v_lines.set_stroke(RED, 3)
        full_v_lines.set_submobject_colors_by_gradient(YELLOW, RED)
        full_v_lines.apply_depth_test()
        self.play(*map(FadeOut, [in_grid_ghost, v_lines, out_grid, out_circles]), ShowCreation(full_v_lines, lag_ratio=0.25, run_time=5))
        self.wait()
        self.play(FadeIn(full_grid, lag_ratio=0.001, run_time=1))
        self.wait()
        ...
        self.play(frame.animate.reorient(58, 70, 0, (-1.74, 2.71, 0.02), 6.4), UpdateFromFunc(rolled_cylinder, update_rolled_cylinder), UpdateFromFunc(rolled_v_lines, update_rolled_cylinder), threshold_tracker.animate.set_value(x_max).set_anim_args(time_span=(2, 7)), run_time=8)
        self.wait()
        ...
        def update_circum_dec(circum_dec):
            circum_dec.restore()
            if circum_tracker.get_value() >= TAU - 1e-05:
                circum_dec.become(Tex('2\\pi', font_size=36))
            else:
        ...
        self.add(circum_dec)
        self.play(ShowCreation(circle), circum_tracker.animate.set_value(TAU), run_time=3)
        circum_dec.update()
        ...
        self.play(MoveToTarget(cylinder), frame.animate.reorient(-1, 72, 0, (3.42, 0.82, 1.64), 11.83), run_time=8)
        ...
        self.play(frame.animate.to_default_state(), cylinder.animate.set_clip_plane(RIGHT, -1).set_anim_args(time_span=(0, 1.5)), FadeIn(out_plane), FadeIn(full_grid, time_span=(2, 3)), FadeIn(full_v_lines, time_span=(2, 3)), FadeIn(in_plane), run_time=3)
        self.wait()
        self.play(ShowCreation(full_v_lines, lag_ratio=0.1), ShowCreation(rolled_v_lines, lag_ratio=0.1), run_time=3)
        self.wait()
        self.play(FadeOut(Group(full_grid, full_v_lines, cylinder)), in_plane.background_lines.animate.set_stroke(BLUE, 1, 0.5), out_plane.background_lines.animate.set_stroke(BLUE, 1, 0.5))
        self.wait()
        z_range = [-TAU * 1j, 0, TAU * 1j] + [unit * n * TAU * 1j for n in range(2, int(200 / TAU)) for unit in [-1, 1]]
        input_dots = VGroup((Dot(in_plane.n2p(z)) for z in z_range))
        in_dot_labels = VGroup(Tex('-2\\pi i'), Tex('0'), Tex('2 \\pi i'))
        ...
        in_plane_label = Tex('z').next_to(in_plane, UP)
        out_plane_label = Tex('e^{z}').next_to(out_plane, UP)
        dot_arrows = VGroup((Arrow(input_dot, out_dot_one, thickness=4, fill_color=RED, fill_opacity=0.75) for input_dot in input_dots))
        dot_arrows.set_fill(opacity=0.75)
        self.play(LaggedStartMap(FadeIn, input_dots), LaggedStartMap(FadeIn, in_dot_labels), in_plane.coordinate_labels[14:].animate.set_opacity(0), ReplacementTransform(func_label, out_plane_label), TransformFromCopy(func_label[1], in_plane_label[0]), func_arrow.animate.match_y(out_plane_label))
        self.wait()
        self.play(LaggedStartMap(GrowArrow, dot_arrows[:3], lag_ratio=0.25), LaggedStart((TransformFromCopy(input_dot, out_dot_one) for input_dot in input_dots[:3]), lag_ratio=0.25))
        self.wait()
        self.play(*(Transform(arrow, arrow.copy().rotate(PI), rate_func=there_and_back_with_pause, run_time=3, path_arc=45 * DEG) for arrow in [*dot_arrows[:3], func_arrow]))
        self.wait()
        equations = VGroup(Tex('e^{-2\\pi i} = 1'), Tex('e^{0} = 1'), Tex('e^{2\\pi i} = 1'))
        z_tracker.set_value(0)
        for equation, y in zip(equations, [-2, 0.35, 2]):
            equation.move_to(y * UP)
        self.play(FadeIn(equations[1]), FadeIn(in_dot), FadeIn(out_dot), dot_arrows[0].animate.set_opacity(0.1), dot_arrows[2].animate.set_opacity(0.1))
        self.wait()
        self.play(z_tracker.animate.set_value(complex(0, TAU)), LaggedStart(FadeIn(equations[2], UP), FadeOut(equations[1], UP), lag_ratio=0.1), dot_arrows[1].animate.set_opacity(0.1), dot_arrows[2].animate.set_opacity(1), run_time=2)
        self.wait()
        self.play(z_tracker.animate.set_value(complex(0, -TAU)), LaggedStart(FadeIn(equations[0], DOWN), FadeOut(equations[2], DOWN), lag_ratio=0.1), dot_arrows[2].animate.set_opacity(0.1), dot_arrows[0].animate.set_opacity(1))
        self.wait()
        ...
        new_in_dot_labels = VGroup((Tex(f'{n} \\pi i').next_to(dot, LEFT) for dot, n in zip(input_dots[4::2], range(4, 30, 2))))
        dot_arrows[3:].set_opacity(0)
        corner_eq = Tex('e^{2\\pi i n} = 1', font_size=60)
        corner_eq.fix_in_frame()
        corner_eq.to_corner(UR)
        self.play(dot_arrows.animate.set_opacity(0.5).set_anim_args(lag_ratio=0.01, run_time=3), FadeIn(new_in_dot_labels, lag_ratio=0.1, time_span=(1, 3)), FadeIn(corner_eq), FadeOut(equations[0]), FadeIn(big_in_plane), FadeOut(in_plane_label), FadeOut(func_arrow), FadeOut(in_plane, time_span=(0.5, 1)), frame.animate.reorient(6, 62, 0, (-0.01, -0.01, -0.01), 8.0).set_anim_args(run_time=3))
        self.wait()
        for _ in range(10):
            self.play(z_tracker.animate.increment_value(TAU * 1j), dot_arrows.animate.set_opacity(0.2), run_time=2)
        self.play(FadeOut(corner_eq))
        exp_arrow = Arrow(in_plane, out_plane, thickness=6)
        exp_label = Tex('e^{z}', font_size=72, t2c={'z': BLUE})
        log_label = Tex('\\ln(w)', font_size=72, t2c={'w': PINK})
        ...
        self.wait(5)
        self.play(Rotate(exp_arrow, PI, time_span=(1, 2)), ReplacementTransform(exp_label, log_label, path_arc=PI, time_span=(1, 2)), LaggedStart((arrow.animate.rotate(PI).set_opacity(0.4).set_anim_args(path_arc=30 * DEG) for arrow in dot_arrows), lag_ratio=1.0 / len(dot_arrows)), frame.animate.reorient(0, 51, 0, (-0.01, -0.01, -0.01)), run_time=3)
        self.play(frame.animate.reorient.reorient(0, 62, 0, (-0.01, -0.01, -0.01), 8.22), run_time=12)
        self.wait(12)

class TheNaturalLog(InteractiveScene):

    def construct(self):
        plane_style = dict(background_line_style=dict(stroke_color=BLUE, stroke_width=1, stroke_opacity=0.5), faded_line_style=dict(stroke_color=BLUE, stroke_width=1, stroke_opacity=0.1))
        ...
        v_lines = VGroup((Line(z_plane.n2p(x), z_plane.n2p(x + TAU * 1j)) for x in np.linspace(1, -1, 9)))
        ...
        left_arrow.set_y(-1)
        right_arrow_label = Tex('z \\rightarrow e^z', t2c={'z': BLUE})
        left_arrow_label = Tex('\\ln(w) \\leftarrow w', t2c={'w': PINK})
        ...
        self.wait()
        self.play(TransformFromCopy(right_arrow, left_arrow, path_arc=-PI / 2), FadeTransformPieces(VGroup(*reversed(right_arrow_label.copy())), left_arrow_label, path_arc=-PI / 2), FadeOut(v_lines))
        self.play(TransformFromCopy(circles, v_lines, lag_ratio=0.1, run_time=3, path_arc=(0, -0.8 * PI)))
        self.wait()
        frame = self.frame
        ...
        self.play(FadeOut(circles), FadeOut(v_lines), FadeIn(droste_image), MoveToTarget(w_plane))
        self.wait()
        self.play(frame.animate.scale(1 / 16, about_point=w_plane.get_origin()), run_time=7, rate_func=lambda t: there_and_back_with_pause(t, 1.0 / 7))
        delta_u = 0.025
        n_log_slice_repetitions = 2
        log_image_slices = Group((get_image_slice(log_image, delta_u, u) for n in range(n_log_slice_repetitions) for u in np.arange(0, 1, delta_u)[::-1]))
        ...
        self.play(FadeIn(z_dot), FadeIn(w_dot), droste_image.animate.set_opacity(0.25))
        ...
        self.play(ShowCreation(droste_ring), FadeOut(traced_paths[1]))
        self.wait()
        self.play(TransformFromCopy(droste_ring, log_slice, path_func=path_func), FadeOut(traced_paths[0], time_span=(0, 1)), FadeOut(z_dot), FadeOut(w_dot), run_time=3)
        self.wait()
        ...
        in_arrows = VGroup((Arrow(3 * v, np.exp(-1) * 3 * v, thickness=4, fill_color=WHITE) for v in compass_directions(8)))
        in_arrows.replace(circle).scale(0.9)
        in_arrows.set_z_index(2)
        one_over_e_label = Tex('\\times 1 / e', font_size=36)
        one_over_e_label.next_to(in_arrows[0], UP, buff=0)
        traced_paths.set_stroke(WHITE, 2)
        self.play(line.animate.shift(z_plane.get_unit_size() * LEFT), circle.animate.scale(np.exp(-1), about_point=w_plane.get_origin()), Write(one_over_e_label), *map(GrowArrow, in_arrows))
        self.play(ShowCreation(small_ring), FadeOut(circle))
        self.play(TransformFromCopy(small_ring, small_log_slice, path_func=path_func), FadeOut(line, time_span=(2, 3)), run_time=3)
        one_arrow = Arrow(log_slice.get_top(), line.get_top(), path_arc=120 * DEG, buff=0.1)
        one_arrow_label = Tex('-1', font_size=36).next_to(one_arrow, UP, SMALL_BUFF)
        self.play(GrowArrow(one_arrow), FadeIn(one_arrow_label, 0.1 * UP))
        self.wait()
        self.play(FadeOut(VGroup(one_arrow, one_arrow_label, in_arrows, one_over_e_label)))
        droste_image_rings.set_opacity(1)
        v_lines = VGroup((Line(piece.get_corner(DL), piece.get_corner(UL)).set_stroke(BLACK, 1, 0.5) for piece in log_image_slices))
        black_circles = VGroup((Circle().replace(ring).set_stroke(BLACK, width=np.clip(circle.get_width(), 0, 1), opacity=0.5) for ring in droste_image_rings))
        ...
        self.play(LaggedStartMap(FadeIn, droste_image_rings, lag_ratio=0.5), LaggedStartMap(FadeIn, log_image_slices, lag_ratio=0.5), LaggedStartMap(FadeIn, v_lines, lag_ratio=0.5), LaggedStartMap(FadeIn, black_circles, lag_ratio=0.5), FadeOut(small_ring, time_span=(2, 4)), FadeOut(small_log_slice, time_span=(2, 4)), run_time=25)
        ...
        self.play(v_lines.animate.set_stroke(opacity=0.25), black_circles.animate.set_stroke(opacity=0.25))
        self.wait()
        self.play(frame.animate.set_x(-4.8), v_lines.animate.set_opacity(0), run_time=7, rate_func=there_and_back_with_pause)
        z_tracker.set_value(1.0)
        z_label = VGroup(Tex('z = '), DecimalNumber(complex(0)))
        z_label.scale(0.5)
        ...
        self.play(FadeIn(z_dot), FadeIn(z_label))
        self.play(TransformFromCopy(z_dot, w_dot, suspend_mobject_updating=True), TransformFromCopy(z_label[0], exp_z_label))
        self.add(traced_paths)
        self.play(z_tracker.animate.increment_value(TAU * 1j), run_time=3)
        ...
        self.play(z_tracker.animate.increment_value(TAU * 1j), frame.animate.reorient(0, 0, 0, (0, 2, 0.0), 13), FadeOut(v_lines), FadeOut(black_circles), FadeOut(z_plane, time_span=(2, 4)), FadeIn(big_z_plane, time_span=(2, 4)), run_time=12)
        traced_paths[1].add_updater(lambda m: m.set_stroke(width=1.5))
        traced_paths.suspend_updating()
        self.play(ShowCreation(circle_points, lag_ratio=0), droste_image.animate.set_opacity(0.5))
        self.play(Restore(circle_points, run_time=2), traced_paths[0].animate.set_stroke(width=0.5))
        self.wait()
        self.play(ShowCreation(log_image_slices, lag_ratio=0.03, run_time=5), FadeOut(circle_points, time_span=(1, 2)), traced_paths[0].animate.set_stroke(width=2), droste_image.animate.set_opacity(1))
        ...
        self.wait()
        self.play(frame.animate.set_y(-1), z_tracker.animate.increment_value(-3 * TAU * 1j), run_time=12)
        self.wait()
        ...
        self.play(z_tracker.animate.set_value(complex(0.5, -2.3)), frame.animate.set_y(0), run_time=3)
        self.play(FlashAround(z_label[0]))
        self.play(w_plane.animate.scale(2, about_point=w_plane.get_left()), droste_image.animate.scale(2, about_point=w_plane.get_left()), run_time=2)
        self.wait()
        ...
        log_randy.apply_points_function(lambda ps: np.array([z_plane.n2p(np.log(w_plane.p2n(p))) for p in ps]), about_point=ORIGIN)
        z_unit = z_plane.get_unit_size()
        log_randys = VGroup(log_randy, log_randy.copy().shift(TAU * z_unit * UP), log_randy.copy().shift(2 * TAU * z_unit * UP))
        brace_group = VGroup(brace, brace_label)
        log_randy_rect = SurroundingRectangle(log_randys[0]).set_stroke(YELLOW, 3)
        self.play(FadeOut(z_label), FadeOut(exp_z_label), FadeOut(z_dot), FadeOut(w_dot), v_line.animate.set_stroke(width=1), pink_circle.animate.set_stroke(width=1), log_image_tiles.animate.set_opacity(0.25), droste_image.animate.set_opacity(0.25), w_plane.axes.animate.set_stroke(WHITE), w_plane.coordinate_labels.animate.set_fill(WHITE), FadeIn(log_randy), FadeIn(droste_randy))
        ...
        self.wait()
        self.play(TransformFromCopy(log_randy, log_randys[1]), log_randy_rect.animate.surround(log_randys[1]), brace_group.animate.set_y(log_randy.get_y(), DOWN))
        self.play(FlashAround(log_randys[1], run_time=2, stroke_width=6))
        self.play(TransformFromCopy(log_randys[1], log_randys[2]), log_randy_rect.animate.surround(log_randys[2]))
        self.play(FlashAround(log_randys[2], run_time=2, stroke_width=6))
        self.wait()
        self.play(FadeOut(log_randy_rect))
        self.play(LaggedStart((TransformFromCopy(lr, droste_randy) for lr in reversed(log_randys)), lag_ratio=0.25, run_time=4))
        self.wait()
        ...
        self.wait()
        self.play(limit_box.animate.shift(z_unit * PI * DOWN), run_time=5, rate_func=there_and_back)
        self.play(limit_box.animate.scale(5), run_time=6)
        self.remove(limit_box)
        self.play(FadeOut(log_image_row))
        self.wait()
        long_log_randys = VGroup(*log_randys, log_randy.copy().shift(z_unit * TAU * DOWN), log_randy.copy().shift(2 * z_unit * TAU * DOWN))
        ...
        self.play(FadeIn(w_dot), FadeIn(z_dot), FlashAround(w_dot))
        for n in range(2):
            added_anims = [ShowCreation(circle)] if n == 0 else []
            self.play(z_tracker.animate.increment_value(TAU * 1j), Rotate(droste_randy, TAU, about_point=w_plane.get_origin()), long_log_randys.animate.shift(z_unit * TAU * UP), *added_anims, run_time=3)
        ...
        self.play(droste_image.animate.set_opacity(0.75), log_image_tiles.animate.set_opacity(0.75))
        ...
        self.play(log_randys.animate.shift(math.log(16) * z_plane.get_unit_size() * LEFT), UpdateFromFunc(small_droste_randy, lambda m: m.become(apply_func_to_z_space(log_randy.copy(), np.exp))), z_tracker.animate.increment_value(-math.log(16)), FadeOut(brace_group, time_span=(0, 1)), frame.animate.reorient(0, 0, 0, (0, 0.75, 0.0), 11), run_time=10)
        self.wait()
        self.play(frame.animate.reorient(0, 0, 0, (6.59, -0.04, 0.0), 0.38).set_anim_args(rate_func=lambda t: there_and_back_with_pause(t, 1 / 7), run_time=7), FadeOut(left_arrow))
        self.play(droste_image.animate.set_opacity(0.25), log_image_tiles.animate.set_opacity(0.25))
        ...
        exp_rule = Tex('e^{z_1 + z_2} = e^{z_1} e^{z_2}', t2c=t2c)
        log_rule = Tex('\\ln(w_1 w_2) = \\ln(w_1) + \\ln(w_2)', t2c=t2c, font_size=40)
        ...
        self.play(func_labels.animate.arrange(DOWN).align_to(w_plane, UP), Write(exp_rule, time_span=(0.5, 1.5)), Write(log_rule, time_span=(1.0, 2.0)))
        self.play(ShowCreation(rect))
        self.wait()
        self.play(rect.animate.surround(exp_rule['e^{z_1} e^{z_2}']))
        self.wait()
        self.play(rect.animate.surround(log_rule['w_1 w_2']))
        self.wait()
        self.play(rect.animate.surround(log_rule['\\ln(w_1) + \\ln(w_2)']))
        self.wait()
        ...
        self.play(frame.animate.reorient(0, 0, 0, (6.08, -0.13, 0.0), 1.37), FadeIn(w_dot, 0.1 * UR, time_span=(1, 2)), FadeIn(w_label, 0.1 * UR, time_span=(1, 2)), FadeOut(v_line), FadeOut(rules), run_time=2)
        self.wait()
        w_plane.coordinate_labels[1].set_opacity(0)
        self.play(frame.animate.reorient(0, 0, 0, (0.7, 0.69, 0.0), 11.37), Restore(w_dot), Restore(w_label), TransformFromCopy(w_dot, w16_dot), TransformMatchingTex(w_label.copy(), w16_label), TransformFromCopy(small_droste_randy, droste_randy), GrowArrow(scale_arrow), FadeIn(times_16_label, time_span=(1, 2), shift=0.25 * scale_arrow.get_vector()), run_time=3)
        self.wait()
        log_w_dot = Dot()
        log_w_dot.match_style(w_dot)
        log_w_dot.move_to(z_plane.n2p(np.log(w) + TAU * 1j))
        log_w_label = Tex('\\ln(w)')
        ...
        shift_log_label = Tex('\\ln(w) + \\ln(16)')
        shift_log_label['\\ln(w)'].set_fill(RED)
        ...
        self.play(TransformFromCopy(w_dot, log_w_dot, path_arc=-15 * DEG), TransformMatchingTex(w_label.copy(), log_w_label, path_arc=-15 * DEG), run_time=2)
        self.wait()
        self.play(TransformFromCopy(log_w_dot, shifted_log_dot), TransformFromCopy(log_randys, shifted_log_randys), FadeTransform(log_w_label.copy(), shift_log_label['\\ln(w)'][0]), FadeIn(shift_log_label['+ \\ln(16)'][0], shift=shift_vect), TransformFromCopy(small_droste_randy, droste_randy), TransformFromCopy(w_dot, w16_dot), GrowArrow(shift_arrow, time_span=(1, 2)), FadeIn(shift_arrow_label, shift=0.25 * RIGHT, time_span=(1, 2)), run_time=3)
        self.wait()
        ...
        self.play(FadeOut(VGroup(w_dot, w_label, w16_dot, w16_label, scale_arrow, times_16_label)), FadeOut(VGroup(log_w_dot, log_w_label, shifted_log_dot, shift_log_label, shift_arrow)), FadeOut(VGroup(small_droste_randy, droste_randy, log_randys, shifted_log_randys)), MoveToTarget(shift_arrow_label), GrowFromCenter(top_brace), FadeIn(brace_group), FadeOut(big_z_plane.coordinate_labels[-15:]), FadeIn(log_image), run_time=2)
        self.play(ShowCreation(fundamental_rect))
        self.wait()
        self.play(ShowCreation(log_image), ShowCreation(annulus), w_plane.coordinate_labels.animate.set_opacity(0), w_plane.axes.animate.set_stroke(WHITE, 1, 0.25), run_time=3)
        self.wait()
        log_image_group.save_state()
        for n in range(3):
            self.play(log_image_group.animate.shift(-shift_vect), annulus.animate.scale(1 / 16, about_point=w_plane.get_origin()), frame.animate.reorient(0, 0, 0, (0.67, 0.14, 0.0), 13.59), run_time=3)
            self.wait()
        self.play(Restore(log_image_group), Restore(annulus), frame.animate.reorient(0, 0, 0, (1.72, 0.49, 0.0), 13.42), run_time=2)
        colors = color_gradient([RED, YELLOW], 4)
        v_lines = VGroup((Line(log_image.get_corner(DR), log_image.get_corner(UR)).shift(-0.25 * n * shift_vect) for n in range(20)))
        ...
        self.play(FadeOut(VGroup(top_brace, shift_arrow_label, brace_group)), log_image_tiles.animate.set_opacity(0.9), droste_image.animate.set_opacity(0.9), FadeOut(Group(log_image, annulus), time_span=(0.9, 1)), FadeOut(fundamental_rect))
        self.play(indic_arrow.animate.shift(v_lines[0].get_vector()), ShowCreation(v_lines[0]), ShowCreation(circles[0]), run_time=3)
        self.wait()
        self.play(LaggedStartMap(ShowCreation, v_lines[1:], lag_ratio=0.1, run_time=2), LaggedStartMap(ShowCreation, circles[1:], lag_ratio=0.1, run_time=2))
        self.wait()
        ...
        def update_droste(mob):
            ...
            return mob
        self.play(Group(log_image_tiles, v_lines).animate.shift(4 * RIGHT), UpdateFromFunc(Group(droste_image, circles), update_droste), run_time=10)
        self.play(FadeOut(v_lines), FadeOut(circles))
        self.wait()
        self.play(log_image_tiles.animate.shift(TAU * z_plane.get_unit_size() * UP), Rotate(droste_image, TAU, about_point=w_plane.get_origin()), run_time=6)
        log_image_tiles.shift(shift_vect * RIGHT)
        self.play(log_image_tiles.animate.shift(shift_vect * LEFT), UpdateFromFunc(droste_image, update_droste), run_time=6)
        self.wait()
    ...
    def get_traced_paths(self, dots, colors=None, stroke_width=3):
        ...
        return VGroup((TracedPath(dot.get_center, stroke_color=color, stroke_width=stroke_width) for dot, color in zip(dots, colors)))
...
class CreatingTheSpiral(InteractiveScene):
    ...
    def construct(self):
        ...
        fz, eq, q_marks = question = VGroup(Tex('f(z)'), Tex('='), Tex('???'))
        ...
        self.play(FadeIn(droste_image), FadeIn(dark_a_plane), FadeOut(a_plane))
        self.play(droste_image.animate.scale(16), run_time=4)
        droste_image.restore()
        self.play(FadeIn(final_image), FadeIn(dark_d_plane), FadeOut(d_plane), FadeIn(blank_spot))
        self.play(UpdateFromFunc(final_image, lambda m: m.become(get_final_image())), var_const.animate.set_value(const), run_time=5)
        self.wait()
        var_const.set_value(1)
        ...
        self.play(ReplacementTransform(func_arrow, log_arrow, time_span=(0, 1)), Write(log_label), FadeIn(b_plane), frame.animate.reorient(0, 0, 0, (0.01, 1.96, 0.0), 5.04), FadeOut(question, time_span=(1, 2)), run_time=2)
        self.play(FadeOut(droste_image.copy()), ShowCreation(droste_image, lag_ratio=0), ShowCreation(log_image_tiles, lag_ratio=0), FadeOut(b_plane), FadeIn(dark_b_plane), run_time=3)
        self.wait()
        self.play(FadeIn(rot_log_tiles), FadeIn(dark_c_plane), Write(mult_arrow), FadeIn(rot_label, 0.25 * DOWN), frame.animate.to_default_state(), run_time=2)
        self.play(var_const.animate.set_value(const), UpdateFromFunc(rot_log_tiles, lambda m: m.become(get_rot_tiles())), run_time=4)
        self.wait()
        ...
        self.play(GrowArrow(exp_arrow, time_span=(0, 1)), Write(exp_label, time_span=(0, 1)), Transform(exp_mover, apply_func_between_planes(exp_mover.copy(), np.exp, c_plane, d_plane), time_span=(1, 3)), box.animate.move_to(d_plane).set_anim_args(time_span=(1, 3)), run_time=3)
        self.remove(exp_mover)
        self.wait()
        plane_covers = VGroup((BackgroundRectangle(plane, buff=0.05).set_fill(BLACK, 0.9) for plane in planes))
        ...
        box_lines = VGroup((Line(randy_box.get_corner(corner), small_box.get_corner(corner)).match_style(small_box) for corner in [UL, DR]))
        randy_line = Line(randy.get_center(), small_randy.get_center())
        randy_line.set_stroke([RED, RED_E], width=[10, 3])
        randy_line_dots = VGroup(Dot(randy_line.get_start(), radius=0.05).set_fill(RED), Dot(randy_line.get_end(), radius=0.01).set_fill(RED_E))
        self.play(frame.animate.reorient(0, 0, 0, (2.52, 1.16, 0.0), 1.74), run_time=3)
        self.play(FadeIn(randy), ShowCreation(randy_box), droste_image.animate.set_opacity(0.5))
        self.wait()
        self.play(TransformFromCopy(randy_group, small_group), ShowCreation(box_lines, lag_ratio=0), run_time=2)
        self.wait()
        self.play(GrowFromCenter(randy_line_dots[0]))
        self.play(FadeOut(randy_box), FadeOut(small_box), FadeOut(box_lines), TransformFromCopy(*randy_line_dots), ShowCreation(randy_line))
        self.wait()
        ...
        final_randy = apply_func_between_planes(randy.copy(), lambda z: np.exp(rot_func(np.log(z) + TAU * 1j)), a_plane, d_plane)
        ...
        self.play(frame.animate.reorient(0, 0, 0, (4.43, -0.86, 0.0), 6.44), final_image.animate.set_opacity(0.1), TransformFromCopy(randy, final_randy), Animation(randy_line), Animation(randy_line_dots), run_time=2)
        self.play(TransformFromCopy(randy_line, loop, run_time=3, path_arc=[0, -0.5 * PI]), TransformFromCopy(small_randy, final_randy, run_time=3, path_arc=-0.5 * PI))
        self.wait()
        self.play(ShowCreation(loop), ShowCreation(randy_line), run_time=2)
        self.wait()
        self.play(TransformFromCopy(loop, randy_line), TransformFromCopy(final_randy, randy), TransformFromCopy(final_randy, small_randy), rate_func=there_and_back_with_pause, run_time=6)
        self.add(randy_line)
        self.wait()
        b_unit = b_plane.get_unit_size()
        log_randy = apply_func_between_planes(randy.copy(), np.log, a_plane, b_plane)
        log_randys = VGroup((log_randy.copy().shift(n * b_unit * TAU * UP) for n in range(5)))
        ...
        self.play(frame.animate.reorient(0, 0, 0, (-0.32, 1.82, 0.0), 4.54), log_arrow_group.animate.set_opacity(1), FadeOut(plane_covers[1]), log_image_tiles.animate.set_opacity(0.5), run_time=2)
        self.play(TransformFromCopy(randy.replicate(len(log_randys)), log_randys), log_image_tiles.animate.set_opacity(0.25), run_time=2)
        self.play(ShowCreation(big_box), Write(big_box_label))
        self.wait()
        self.play(TransformFromCopy(randy, small_randy), TransformFromCopy(log_randys, small_log_randys), TransformFromCopy(big_box, small_box), run_time=2)
        self.play(Write(small_box_label))
        self.wait()
        ...
        log_lines = VGroup((Line(br.get_center(), sr.get_center()).set_stroke(RED, 3) for br, sr in zip(log_randys, small_log_randys)))
        ...
        log_indicators = Group((GlowDot(color=RED).move_to(lr) for lr in log_randys))
        line_indicators = Group(randy_line_indicator, log_indicators)
        self.play(UpdateFromFunc(randy_line_indicator, lambda m: m.move_to(randy_line.get_end())), log_indicators.animate.move_to(log_lines, LEFT), ShowCreation(log_lines, lag_ratio=0), ShowCreation(randy_line, lag_ratio=0), run_time=2)
        self.play(FadeOut(line_indicators))
        self.wait()
        ...
        diag_lines = VGroup(diag_line.copy().shift(-v_shift), diag_line, diag_line.copy().shift(v_shift))
        ...
        self.play(ReplacementTransform(log_lines, diag_lines, run_time=3), UpdateFromFunc(randy_line, lambda m: m.match_points(get_true_randy_line())))
        self.wait()
        self.play(big_box.animate.surround(log_randys[1]))
        self.wait()
        self.play(small_box.animate.surround(small_log_randys[0]), small_box_label.animate.next_to(small_log_randys[0], DOWN, buff=0.2))
        self.wait()
        ...
        self.play(log_randy.animate.move_to(small_log_randys[0]), UpdateFromFunc(exp_log_randy, lambda m: m.become(get_exp_log_randy())), run_time=3)
        ...
        v_line_label = v_line_brace.get_tex('2\\pi')
        ...
        diag_arrow, down_arrow = red_arrows = VGroup((Arrow(line.get_start(), line.get_end(), buff=0, thickness=4, fill_color=RED) for line in [diag_line, v_line]))
        self.play(frame.animate.to_default_state(), run_time=2)
        self.play(ShowCreation(plane_rect))
        self.wait()
        self.play(plane_rect.animate.surround(c_plane), TransformFromCopy(loop, v_line), TransformFromCopy(final_randy.replicate(2), rot_log_randys), exp_arrow_group.animate.set_fill(opacity=1), FadeOut(plane_covers[2]), rot_log_tiles.animate.set_opacity(0.5), run_time=2)
        self.play(GrowFromCenter(v_line_brace), Write(v_line_label, stroke_color=WHITE))
        self.wait()
        self.play(plane_rect.animate.surround(b_plane).stretch(1.05, 1, about_edge=DOWN), FadeOut(diag_lines), FadeIn(diag_arrow), FadeOut(VGroup(log_randys[0], small_log_randys[1])), log_image_tiles.animate.set_opacity(0.5))
        self.wait()
        self.play(plane_rect.animate.surround(VGroup(b_plane, c_plane)).stretch(1.02, 1, about_edge=DOWN), TransformFromCopy(diag_arrow, down_arrow), mult_arrow_group.animate.set_fill(opacity=1), run_time=2)
        self.wait()
        kw = dict(t2c={'{z}': BLUE, '{c}': YELLOW})
        func_label = VGroup(Tex('{z}', **kw), Tex('\\downarrow'), Tex('{c} \\cdot {z}', **kw))
        func_label.arrange(DOWN)
        func_label.next_to(mult_arrow, LEFT)
        true_output = Tex('{c} \\cdot ({z} - z_0) + z_0', **kw)
        true_output.move_to(func_label[2], RIGHT)
        pivot_dot = Dot(b_plane.n2p(fixed_point)).set_color(TEAL)
        pivot_dot.set_stroke(WHITE, 1)
        pivot_dot_label = Tex('z_0')
        pivot_dot_label.set_backstroke(BLACK, 3)
        pivot_dot_label.next_to(pivot_dot, RIGHT, SMALL_BUFF)
        self.play(frame.animate.set_x(-2), FadeOut(rot_label, 0.5 * DOWN), FadeIn(func_label, 0.5 * DOWN))
        self.wait()
        self.play(plane_rect.animate.surround(randy))
        self.wait()
        self.play(plane_rect.animate.surround(log_randys[1]))
        self.wait()
        self.play(FadeOut(VGroup(log_randys[1], small_log_randys[0], big_box, big_box_label, small_box, small_box_label, plane_rect)), diag_arrow.animate.put_start_and_end_on(pivot_dot.get_center(), diag_arrow.get_end()), FadeIn(pivot_dot))
        self.play(log_image_tiles.animate.rotate(70 * DEG, about_point=diag_arrow.get_start()), diag_arrow.animate.rotate(70 * DEG, about_point=diag_arrow.get_start()), rate_func=there_and_back, run_time=4)
        self.play(Write(pivot_dot_label, stroke_color=WHITE))
        self.wait()
        self.play(TransformMatchingTex(func_label[2], true_output), TransformFromCopy(pivot_dot_label.replicate(2), true_output['z_0']), func_label[:2].animate.match_x(true_output))
        self.wait()
        func_label = VGroup(*func_label[:2], true_output)
        self.add(func_label)
        c_value = Tex('{c} = \\frac{2\\pi i}{\\ln(16) + 2 \\pi i}', **kw)
        ...
        log_16_label = Tex('\\ln(16)', font_size=24)
        log_16_label.next_to(log16_line, DOWN, SMALL_BUFF)
        two_pi_i_label = Tex('2\\pi i', font_size=24)
        two_pi_i_label.next_to(two_pi_i_line, RIGHT, SMALL_BUFF)
        VGroup(two_pi_i_label, log_16_label).set_backstroke(BLACK, 3)
        self.play(ShowCreation(log16_line), FadeIn(log_16_label))
        self.play(ShowCreation(two_pi_i_line), FadeIn(two_pi_i_label))
        self.wait()
        self.play(func_label.animate.shift(0.7 * UP), FadeIn(c_value, 0.5 * DOWN))
        self.wait()
        faders = VGroup(randy, small_randy, randy_line, log16_line, two_pi_i_line, log_16_label, two_pi_i_label, rot_log_randys, plane_rect, diag_arrow, v_line, down_arrow, v_line_brace_group, loop, final_randy)
        self.remove(blank_spot)
        self.play(LaggedStartMap(FadeOut, faders, lag_ratio=0.1), droste_image.animate.set_opacity(1).set_anim_args(time_span=(0, 1)), log_image_tiles.animate.set_opacity(1).set_anim_args(time_span=(0.25, 1.25)), rot_log_tiles.animate.set_opacity(1).set_anim_args(time_span=(0.5, 1.5)), final_image.animate.set_opacity(1).set_anim_args(time_span=(0.75, 1.75)), FadeIn(blank_spot), run_time=2)
        self.wait()
        mult_plane, c_group = self.get_mult_plane(c_plane)
        mult_plane.match_x(func_label)
        c_dot, c_vect, c_label = c_group
        self.play(FadeOut(c_value, DOWN), FadeIn(mult_plane, DOWN), FadeIn(c_group, DOWN), func_label.animate.shift(0.25 * UP))
        ...
        self.play(ShowCreation(plane_rect), droste_image.animate.set_opacity(0.5), FadeIn(plane_covers[:3]))
        self.wait()
        self.play(plane_rect.animate.surround(blank_spot[1]), frame.animate.reorient(0, 0, 0, (2.21, -1.99, 0.0), 3.48).set_anim_args(run_time=2))
        self.wait()
        self.play(FadeOut(blank_spot))
        self.wait()
        self.play(FadeOut(plane_rect), FadeOut(plane_covers[2]), frame.animate.reorient(0, 0, 0, (-0.07, -1.85, 0.0), 4.89))
        self.wait()
        self.play(rot_log_tiles.animate.shift(c_plane.n2p(4 * const * math.log(16)) - c_plane.get_origin()), self.pre_transform_log.animate.shift(4 * b_unit * math.log(16) * RIGHT), UpdateFromFunc(final_image, lambda m: m.become(get_final_image())), run_time=12)
        self.wait()
        self.pre_transform_log.restore()
        final_image.restore()
        rot_log_tiles.restore()
        self.play(droste_image.animate.set_opacity(1), FadeOut(plane_covers[:2]), frame.animate.to_default_state().set_x(-2), run_time=2)
        self.wait()
        self.play(ShowCreation(old_randy_line), ShowCreation(old_log_lines))
        self.wait()
        var_const.clear_updaters().add_updater(lambda m: m.set_value(mult_plane.p2n(c_dot.get_center())))
        rot_log_tiles.clear_updaters().add_updater(lambda m: m.become(get_rot_tiles()))
        final_image.clear_updaters().add_updater(lambda m: m.become(get_final_image()))
        self.play(c_dot.animate.move_to(mult_plane.n2p(complex(0, TAU / math.log(16)))), run_time=3)
        rot_log_tiles.clear_updaters()
        final_image.clear_updaters()
        for factor, time in [(0.5, 2), (5e-07, 8)]:
            self.play(final_image.animate.scale(factor, about_point=d_plane.get_origin()), rot_log_tiles.animate.shift(b_unit * math.log(factor) * RIGHT), run_time=time)
        self.wait()

    def get_log_image_path(self):
        return get_pi_house_log_image_path()

    def get_four_planes(self, axes_color=WHITE, line_color=BLUE, line_width=1, line_opacity=1, faded_line_ratio=4, faded_line_opacity=0.25, h_buff=2.0, v_buff=0.75, include_coordinates=True):
        kw = dict(faded_line_ratio=faded_line_ratio)
        planes = VGroup(ComplexPlane(self.droste_plane_x_range, self.droste_plane_x_range, **kw), ComplexPlane(self.log_plane_x_range, self.log_plane_x_range, **kw), ComplexPlane(self.log_plane_x_range, self.log_plane_x_range, **kw), ComplexPlane(self.droste_plane_x_range, self.droste_plane_x_range, **kw))
        for plane in planes:
    ...
    def get_four_dark_planes(self):
        result = self.get_four_planes(axes_color=BLACK, line_color=BLACK, line_opacity=0.25, faded_line_ratio=0, include_coordinates=False)
    ...
    def get_log_exp_arrow_groups(self):
        ...
        log_label = Tex('\\ln(w) \\leftarrow w', **tex_kw)
        log_label.next_to(log_arrow, UP, buff=SMALL_BUFF)
        exp_label = Tex('z \\to e^z', **tex_kw)
    ...
    def get_rot_tiles(self):
        ...
        result = apply_func_between_planes(self.log_image_tiles.copy(), self.rot_func, b_plane, c_plane)
    ...
    def get_final_image(self):
        ...
        result = apply_func_between_planes(self.pre_transform_log.copy(), lambda z: np.exp(self.rot_func(z)), b_plane, d_plane)
    ...
    def get_mult_arrow_group(self):
        ...
        mult_arrow = Arrow(b_plane.get_left(), c_plane.get_left(), path_arc=90 * DEG, thickness=5)
        kw = dict(t2c={'{z}': BLUE, 'c': YELLOW})
        mult_label = VGroup(Tex('{z}', **kw), Tex('\\downarrow', **kw), Tex('{c} \\cdot ({z} - z_0) + z_0', **kw))
    ...
    def get_mult_plane(self, ref_plane, x_range=(-2, 2)):
        ...
        c_label = Tex('c').set_color(YELLOW)
...
class FourPannelsWithPrintGallery(CreatingTheSpiral):
    ...
    def construct(self):
        ...
        self.add(droste_image)
        self.play(UpdateFromAlphaFunc(droste_image, lambda m, a: m.set_width(start_width * np.exp(a * math.log(50)))), droste_image.animate.scale(50), run_time=5)
        self.play(Restore(droste_image), FadeIn(a_plane), frame.animate.set_height(4).move_to(a_plane), run_time=2)
        self.add(planes)
        self.play(droste_image.animate.scale(1.0 / self.droste_scale_factor, about_point=a_plane.get_origin()), rate_func=lambda t: np.log(np.exp(smooth(t))), run_time=5)
        droste_image.restore()
        self.play(droste_image.animate.set_opacity(1), FadeOut(a_plane), FadeIn(dark_a_plane))
        self.wait()
        frame = self.frame
        self.play(GrowArrow(log_arrow), FadeIn(log_label, LEFT), FadeIn(log_image_tiles), FadeIn(dark_b_plane), FadeOut(b_plane), frame.animate.reorient(0, 0, 0, (-0.09, 2.03, 0.0), 4.9).set_anim_args(run_time=2))
        self.play(ShowCreation(droste_image, lag_ratio=0), ShowCreation(log_image_tiles, lag_ratio=0), FadeOut(droste_image.copy(), rate_func=rush_from), FadeOut(log_image_tiles.copy(), rate_func=rush_from), run_time=5)
        self.wait()
        b_unit = b_plane.get_unit_size()
        ...
        tile_box_labels = VGroup(Tex('2\\pi', font_size=24).next_to(tile_box, LEFT, SMALL_BUFF), Tex('\\ln(256)', font_size=24).next_to(tile_box, UP, SMALL_BUFF))
        ...
        self.play(ShowCreation(tile_box), log_image_tiles.animate.set_opacity(0.2), Write(tile_box_labels))
        self.wait()
        self.play(VGroup(tile_box, tile_box_labels).animate.shift(tile_box.get_width() * LEFT))
        self.wait()
        self.play(FadeOut(tile_box), FadeOut(tile_box_labels), FadeOut(highlighted_tiles), log_image_tiles.animate.set_opacity(1))
        self.wait()
        ...
        self.play(frame.animate.set_height(FRAME_HEIGHT).move_to(2 * LEFT), LaggedStart(FadeIn(mult_arrow), FadeIn(mult_label), FadeIn(rot_log_tiles), FadeOut(c_plane), FadeIn(dark_c_plane), lag_ratio=0.1), run_time=2)
        self.wait()
        self.play(FadeIn(mult_plane, 0.5 * DOWN), FadeIn(c_group, 0.5 * DOWN), mult_label.animate.next_to(mult_plane, UP))
        self.play(var_const.animate.set_value(1), UpdateFromFunc(rot_log_tiles, lambda m: m.become(self.get_rot_tiles())), run_time=7, rate_func=lambda t: there_and_back_with_pause(t, 1 / 7))
        self.wait()
        ...
        self.play(GrowArrow(exp_arrow), FadeIn(exp_label, 0.5 * RIGHT), ShowCreation(final_image, lag_ratio=0, run_time=5), ShowCreation(rot_log_tiles, lag_ratio=0, run_time=5), FadeOut(rot_log_tiles.copy()), FadeOut(d_plane), FadeIn(dark_d_plane))
        self.wait()
        self.play(var_const.animate.set_value(1), UpdateFromFunc(rot_log_tiles, lambda m: m.become(self.get_rot_tiles())), UpdateFromFunc(final_image, lambda m: m.become(self.get_final_image())), run_time=7, rate_func=lambda t: there_and_back_with_pause(t, 1 / 7))
        self.play(frame.animate.set_height(3.5).move_to(d_plane), dark_d_plane.animate.set_stroke(opacity=0.25), run_time=2)
        self.wait()
        rot_log_tiles.become(self.get_rot_tiles())
        final_image.become(get_final_image())
        b_unit = b_plane.get_unit_size()
        const = var_const.get_value()
        self.play(frame.animate.set_height(4.9).move_to(VGroup(c_plane, d_plane)), dark_d_plane.animate.set_stroke(opacity=0.25), run_time=2)
        self.wait()
        self.play(rot_log_tiles.animate.shift(c_plane.n2p(1 * const * math.log(256)) - c_plane.get_origin()), self.pre_transform_log.animate.shift(1 * b_unit * math.log(256) * RIGHT), UpdateFromFunc(final_image, lambda m: m.become(get_final_image())), run_time=5, rate_func=linear)
        return
        ...
        self.play(exp_tracker.animate.set_value(-9.5), run_time=15)
...
class TransformPrintGalleryFromStraightToLoop(FourPannelsWithPrintGallery):
    ...
    def construct(self):
        ...
        self.play(self.var_const.animate.set_value(const), run_time=8)
...
class FourStepsWithGrid(CreatingTheSpiral):
    ...
    def construct(self):
        ...
        frame = self.frame
        grid = self.get_grid()
        self.play(frame.animate.set_height(4).move_to(a_plane), run_time=2)
        ...
        self.play(MoveToTarget(log_grid), box.animate.move_to(b_plane), frame.animate.set_width(ab_plane.get_width() + 1).move_to(ab_plane), run_time=3)
        ...
        log_tiles = VGroup((log_grid.copy().shift(x * x_shift + y * y_shift) for x in [1, 0] for y in range(-1, 2)))
        for tile in log_tiles[:3]:
            tile.set_submobject_colors_by_gradient(RED, BLUE)
        log_grid.clip_to_box(b_plane)
        log_tiles.clip_to_box(b_plane)
        self.remove(log_grid)
        self.play(LaggedStart((TransformFromCopy(log_grid.copy().set_stroke(opacity=0.1), tile) for tile in log_tiles), lag_ratio=0.1, run_time=2))
        self.wait()
        ...
        self.play(box.animate.replace(c_plane), ReplacementTransform(mover, rot_log_tiles), frame.animate.move_to(cd_plane), run_time=2)
        self.wait()
        rot_log_tiles_subset = VGroup((piece for piece in rot_log_tiles.family_members_with_points() if -PI - 0.01 < c_plane.y_axis.p2n(piece.get_center()) < PI + 0.01))
        ...
        self.play(LaggedStartMap(FadeIn, final_grid, **kw), LaggedStartMap(VFadeInThenOut, final_grid_highlight, **kw), LaggedStartMap(VFadeInThenOut, rot_log_tiles_subset_highlight, **kw))
        self.play(frame.animate.to_default_state(), run_time=2)
        self.wait()
        for plane in [a_plane, d_plane]:
            ...
            self.wait()
        frame = self.frame
        squares = VGroup(grid[2][20], grid[1][10], grid[3][0]).copy()
        ...
        self.play(FadeIn(squares), VGroup(grid, log_tiles, rot_log_tiles, final_grid).animate.set_stroke(WHITE), frame.animate.set_height(3.8).move_to(a_plane), run_time=2)
        self.wait()
        self.play(TransformFromCopy(squares, squares_image, path_arc=15 * DEG), frame.animate.move_to(d_plane), run_time=3)
        self.wait()
        ...
        lower_squares.remove(*(s for s in lower_squares if np.max(np.abs(d_plane.p2c(s.get_center()))) > 0.8))
        lower_squares.set_fill(TEAL, 1)
        samples = VGroup(*random.sample(list(lower_squares), n_samples))
        square_labels = VGroup((Text('square').replace(square, 0).scale(0.6) for square in samples))
        square_labels.set_backstroke(BLACK, 2)
        self.play(LaggedStartMap(VFadeInThenOut, samples, lag_ratio=0.2), LaggedStartMap(VFadeInThenOut, square_labels, lag_ratio=0.2), frame.animate.set_height(2).move_to(d_plane.get_origin()), run_time=8)
        self.wait()

    def get_four_planes(self, *args, faded_line_ratio=3, **kwargs):
        return super().get_four_planes(*args, faded_line_ratio=faded_line_ratio, **kwargs)
...
class ModifyFinalGrid(FourStepsWithGrid):
    ...
    def construct(self):
        ...
        for scale_factor in [256, 16, 81]:
            self.play(scale_factor_log_tracker.animate.set_value(math.log(scale_factor)), run_time=5)
            self.wait()
...
class CreateMeshWarp(FourStepsWithGrid):
    ...
    def construct(self):
        ...
        self.play(FadeIn(flat_grid, lag_ratio=0.5, run_time=10), FadeIn(log_grid, lag_ratio=0.5, run_time=10), FadeIn(rot_log_grid, lag_ratio=0.5, run_time=10), FadeIn(warp_grid, lag_ratio=0.5, run_time=10))
...
class TileToDoughnut(InteractiveScene):
    ...
    def construct(self):
        ...
        borders = VGroup((SurroundingRectangle(piece, buff=0) for piece in tiles))
        ...
        torus = TexturedSurface(ParametricSurface(uv_func, resolution=(101, 101), u_range=(0, TAU), v_range=(0, TAU)), file)
        torus.set_z(3)
        frame.set_height(12)
        self.add(tiles, borders)
        self.play(tiles.animate.set_opacity(0.5), FadeIn(tile))
        self.play(tiles.animate.set_opacity(0.5).set_anim_args(run_time=4), ReplacementTransform(tile, torus, time_span=(1, 5)), frame.animate.reorient(16, 45, 0, (0.01, 0.08, 0.14), 13.11).set_anim_args(run_time=6))
        self.play(frame.animate.reorient(-39, 47, 0, (0.01, 0.08, 0.14), 13.11), run_time=8)
        self.wait()