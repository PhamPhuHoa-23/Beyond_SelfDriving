...
LOOP_RAINBOW = [RED, YELLOW, GREEN, TEAL, BLUE, '#7744FF', PURPLE, PINK, MAROON]
...
class Strings(InteractiveScene):
    ...
    def construct(self):
        ...
        for i in range(n):
            ...
            for m in [strings[i], end_dots[2 * i], end_dots[2 * i + 1]]:
                drop_anims.append(m.animate.restore().set_anim_args(time_span=span, rate_func=rush_into))
        self.play(*drop_anims, frame.animate.reorient(-10, 65), run_time=0.6 + (n - 1) * 0.15)
        ...
        piece_map = {}
        for i in range(n):
            piece = {'mob': strings[i], 'start_eid': 2 * i, 'end_eid': 2 * i + 1, 'size': 1, 'dots': {2 * i: end_dots[2 * i], 2 * i + 1: end_dots[2 * i + 1]}}
            piece_map[2 * i] = piece
        ...
        for k in range(n_box_joins):
            ...
            for piece, eid in pieces_to_lift:
                ...
                lift_anims.append(self.make_pull_lift_anim(mob, eid, piece, lift_height))
            ...
            if forms_loop:
                ...
                self.play(Transform(source, loop_mob), run_time=1.2)
                d1.clear_updaters()
                d2.clear_updaters()
                self.remove(source)
                self.add(loop_mob)
                ...
                self.play(loop_mob.animate.shift(drop_vec), run_time=0.8, rate_func=rush_into)
                self.wait(0.5)
                ...
                self.play(Flash(mid, color=YELLOW, line_length=0.12, flash_radius=0.25), FadeOut(d1), FadeOut(d2), run_time=0.5)
                drop_vec = (-bh - connected.get_center()[2]) * OUT
                merged = {'mob': connected, 'start_eid': free_a, 'end_eid': free_b, 'size': pa['size'] + pb['size'], 'dots': {free_a: da, free_b: db}}
                dot_updaters = pin_dots_to_endpoints(merged)
                self.play(connected.animate.shift(drop_vec), run_time=0.8, rate_func=rush_into)
                remove_updaters(dot_updaters)
                self.wait(0.5)
        ...
        for lm in box_loops:
            all_box_mobs.add(lm)
        self.play(FadeOut(all_box_mobs), frame.animate.reorient(0, 0).set_height(8), run_time=1.5)
        chains_data = self.get_chain_state(n, join_seq[:n_box_joins])
        self.init_organized(chains_data, base_height=0.5)
        self.play(LaggedStartMap(FadeIn, VGroup(*(c['line'] for c in self.org_chains)), lag_ratio=0.08), LaggedStartMap(FadeIn, VGroup(*(d for c in self.org_chains for d in c['dots'].values())), lag_ratio=0.04), run_time=1.5)
        ...
        self.wait(2)
        old_mobs = VGroup(*(c['line'] for c in self.org_chains), *(d for c in self.org_chains for d in c['dots'].values()), *self.org_loops, counter_group)
        self.play(FadeOut(old_mobs), run_time=1.0)
        n2 = 50
        join_seq_50 = self.compute_join_sequence(n2)
        chains_50 = self.get_chain_state(n2, [])
        self.init_organized(chains_50, base_height=0.08, x_range=(-6.5, 6.5), dot_radius=0.035, stroke_width=2)
        ...
        cg_50.to_corner(UR)
        self.play(FadeIn(all_50_lines), FadeIn(all_50_dots), FadeIn(cg_50), run_time=1.0)
        for k in range(n2):
            e1, e2, forms_loop, _ = join_seq_50[k]
            self.animate_organized_join(e1, e2, forms_loop, counter_50, fast=False)
        self.wait(3)

    def init_organized(self, chains_data, base_height=0.5, x_range=(-5, 5), y_center=1.0, loop_y=-2.0, dot_radius=0.07, stroke_width=4):
        self.org_chains = []
        self.org_end_map = {}
        self.org_loops = []
        ...
        for i, cd in enumerate(chains_data):
            ...
            top = self.org_y + h / 2
            bot = self.org_y - h / 2
            line = Line(np.array([x, bot, 0]), np.array([x, top, 0])).set_stroke(ORANGE, stroke_width)
            ...
            chain = {'line': line, 'ends': [cd['ends'][0], cd['ends'][1]], 'dots': {cd['ends'][0]: td, cd['ends'][1]: bd}, 'size': cd['size']}
    ...
    def animate_organized_join(self, e1, e2, forms_loop, counter, fast=False):
        ...
        if forms_loop:
            ...
            if not fast:
                ...
                self.play(ReplacementTransform(ca['line'], circle_raised), run_time=0.5)
                d1.clear_updaters()
                d2.clear_updaters()
                self.play(FadeOut(d1), FadeOut(d2), run_time=0.2)
                anims = [circle_raised.animate.move_to(circle.get_center()).set_anim_args(path_arc=-PI / 3), counter.animate.set_value(self.org_loop_count)]
                self._add_redistribute_anims(anims, xs)
                self.play(*anims, run_time=main_rt)
                self.org_loops.append(circle_raised)
            else:
                anims = [ReplacementTransform(ca['line'], circle), FadeOut(d1), FadeOut(d2), counter.animate.set_value(self.org_loop_count)]
                self._add_redistribute_anims(anims, xs)
                self.play(*anims, run_time=main_rt)
            ...
            nc = {'line': None, 'ends': [oa, ob], 'dots': {oa: da, ob: db}, 'size': new_size}
            ...
            tn = self.org_y + hn / 2
            bn = self.org_y - hn / 2
            new_line = Line(np.array([xn, bn, 0]), np.array([xn, tn, 0])).set_stroke(ORANGE, self.org_sw)
            nc['line'] = new_line
            del self.org_end_map[e1]
            del self.org_end_map[e2]
            ...
            if not fast:
                ...
                ca['line'].target.put_start_and_end_on(np.array([mid_x, a_bot, 0]), np.array([mid_x, a_top, 0]))
                cb['line'].generate_target()
                cb['line'].target.put_start_and_end_on(np.array([mid_x, b_bot, 0]), np.array([mid_x, b_top, 0]))
                self.play(MoveToTarget(ca['line']), MoveToTarget(cb['line']), d1.animate.move_to(meet_point), d2.animate.move_to(meet_point), da.animate.move_to(np.array([mid_x, da_y, 0])), db.animate.move_to(np.array([mid_x, db_y, 0])), run_time=0.4)
                connected = Line(np.array([mid_x, b_bot, 0]), np.array([mid_x, a_top, 0])).set_stroke(ORANGE, self.org_sw)
                self.remove(ca['line'], cb['line'])
                self.add(connected)
                self.play(Flash(meet_point, color=YELLOW, line_length=0.1, flash_radius=0.2), FadeOut(d1), FadeOut(d2), run_time=0.25)
                connected.generate_target()
                connected.target.put_start_and_end_on(np.array([xn, bn, 0]), np.array([xn, tn, 0]))
                anims = [MoveToTarget(connected), da.animate.move_to(np.array([xn, tn, 0])), db.animate.move_to(np.array([xn, bn, 0]))]
                self._add_redistribute_anims(anims, xs, skip=nc)
                self.play(*anims, run_time=main_rt)
                nc['line'] = connected
            else:
                anims = [ReplacementTransform(ca['line'], new_line), FadeOut(cb['line']), FadeOut(d1), FadeOut(d2), da.animate.move_to(np.array([xn, tn, 0])), db.animate.move_to(np.array([xn, bn, 0]))]
                self._add_redistribute_anims(anims, xs, skip=nc)
                self.play(*anims, run_time=main_rt)
    ...
    def _add_redistribute_anims(self, anims, xs, skip=None):
        for i, chain in enumerate(self.org_chains):
            ...
            chain['line'].target.put_start_and_end_on(np.array([x, b, 0]), np.array([x, t, 0]))
    ...
    def get_chain_state(self, n, partial):
        ...
        return [{'ends': ends, 'size': len(root_strings[root])} for root, ends in root_ends.items() if len(ends) == 2]
    ...
    def highlight_endpoints(self, d1, d2):
        ...
        self.play(d1.animate.set_color(YELLOW), d2.animate.set_color(YELLOW), anim1, anim2, run_time=0.5)
    ...
    def get_open_box(self):
        ...
        w, h = (bw / 2, bd / 2)
        top = [np.array([-w, -h, 0.0]), np.array([w, -h, 0.0]), np.array([w, h, 0.0]), np.array([-w, h, 0.0])]
    ...
    def get_floor_strings(self, n):
        ...
        for i in range(n):
            ...
            for j in range(na):
                t = j / (na - 1)
                p = p1 + t * (p2 - p1)
                wave = amp_flat * (np.sin(f1 * PI * t + phase) + 0.5 * np.sin(f2 * PI * t)) * perp
            ...
            for j in range(na):
                ...
                offset = np.array([amp_3d * np.sin(f3 * PI * t + phase), amp_3d * np.sin(f4 * PI * t + phase2), amp_3d * np.sin(f5 * PI * t + phase3)])
