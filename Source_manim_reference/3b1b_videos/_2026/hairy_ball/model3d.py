...
def get_position_vectors(trajectory, prop, d_prop=0.0001, use_curvature=False):
    if use_curvature:
        d_prop = 0.01
...
class RadioTower(VGroup):
    ...
    def __init__(self, height=4, stroke_color=GREY_A, stroke_width=2, **kwargs):
        ...
        self.struts = VGroup((self.get_struts(leg1, leg2) for leg1, leg2 in adjacent_pairs(self.legs)))
    ...
    def get_legs(self):
        return VGroup((Line(point, 4 * OUT) for point in compass_directions(4, UR)))

    def get_struts(self, leg1, leg2, n_crosses=4):
        points1, points2 = [[leg.pfp(a) for a in np.linspace(0, 1, n_crosses + 1)] for leg in [leg1, leg2]]
        return VGroup(*(Line(*pair) for pair in zip(points1, points2[1:])), *(Line(*pair) for pair in zip(points1[1:], points2)))
...
class OrientAModel(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(GrowFromPoint(plane, ORIGIN, time_span=(0, 2)), frame.animate.reorient(-33, 79, 0, (-0.16, 0.01, 0.38), 8.16), run_time=4)
        frame.add_ambient_rotation(1 * DEG)
        traj1 = ParametricCurve(lambda t: 2 * np.array([math.cos(t), math.sin(t), 0.04 * t ** 1.5 - 1]), t_range=(0, 15, 0.2))
        ...
        self.play(plane.animate.place_on_path(traj1, prop_tracker.get_value()), ShowCreation(traj1), FadeIn(axes), FadeIn(xy_plane))
        self.play(prop_tracker.animate.set_value(1), UpdateFromFunc(plane, update_plane), rate_func=linear, run_time=10)
        traj2 = ParametricCurve(lambda t: t * RIGHT + np.exp(-0.1 * t ** 2) * (math.cos(2 * t) * UP + math.sin(2 * t) * OUT) + 2 * OUT, t_range=(-4, 4, 0.1))
        traj2.match_style(traj1)
        prop_tracker.set_value(0)
        self.play(plane.animate.place_on_path(traj2, 0), Transform(traj1, traj2), run_time=2)
        self.play(prop_tracker.animate.set_value(1), UpdateFromFunc(plane, update_plane), frame.animate.reorient(41, 64, 0, (-0.17, 0.0, 0.38), 8.16), rate_func=linear, run_time=10)
        ...
        vel_vector, wing_vector, center_dot = vect_group = Group(Vector(RIGHT, thickness=2).set_color(RED), Vector(RIGHT, thickness=2).set_color(PINK), TrueDot(radius=0.025, color=BLUE).make_3d())
        ...
        self.add(pre_traj)
        self.play(plane.animate.reposition(center_dot.get_center(), RIGHT, UP).set_opacity(0.5), frame.animate.reorient(-7, 66, 0, (-1.99, -0.11, 1.79), 3.67), traj1.animate.set_stroke(GREY, 1, 0.5), FadeIn(center_dot), run_time=3)
        self.wait()
        ...
        self.wait()
        self.play(Rotate(plane, TAU, axis=vel_vector.get_vector(), about_point=center_dot.get_center(), run_time=5), Rotate(wing_vector, TAU, axis=vel_vector.get_vector(), about_point=center_dot.get_center(), run_time=5))
        self.wait()
        self.play(traj1.animate.set_stroke(WHITE, 1), FadeOut(pre_traj))
        self.play(UpdateFromFunc(plane, update_plane), UpdateFromFunc(vect_group, update_vect_group), prop_tracker.animate.set_value(1).set_anim_args(rate_func=linear), frame.animate.reorient(27, 69, 0, (-0.05, -0.3, 0.71), 8.18), run_time=10)
        traj3 = ParametricCurve(lambda t: 3 * math.cos(t) * RIGHT + 3 * math.sin(t) * UP + math.cos(3 * t) * OUT, t_range=(0, TAU, 0.01))
        traj3.insert_n_curves(1000)
        traj3.set_stroke(WHITE, 1)
        center, vel, wing_dir = get_position_vectors(traj3, 0, use_curvature=False)
        prop_tracker.set_value(0)
        self.play(traj1.animate.become(traj3), plane.animate.place_on_path(traj3, 0).set_opacity(1), center_dot.animate.move_to(center), vel_vector.animate.put_start_and_end_on(center, center + vel), wing_vector.animate.put_start_and_end_on(center, center + wing_dir), frame.animate.reorient(-3, 47, 0, (-0.36, -0.81, -0.05), 8.18), run_time=2)
        self.play(UpdateFromFunc(plane, update_plane), UpdateFromFunc(vect_group, update_vect_group), prop_tracker.animate.set_value(1).set_anim_args(rate_func=linear), frame.animate.reorient(-49, 71, 0, (0.47, 0.37, 0.28), 6.45), run_time=10)
        prop_tracker.set_value(0)
        self.play(prop_tracker.animate.set_value(1).set_anim_args(rate_func=linear), UpdateFromFunc(plane, update_plane), UpdateFromFunc(vect_group, update_vect_group), run_time=10)
        v_tracker = Point(normalize(vel_vector.get_vector()))
        self.play(plane.animate.reposition(ORIGIN, v_tracker.get_center(), faulty_perp(v_tracker.get_center())).set_opacity(0.3), center_dot.animate.move_to(ORIGIN), vel_vector.animate.put_start_and_end_on(ORIGIN, v_tracker.get_center()), wing_vector.animate.put_start_and_end_on(ORIGIN, faulty_perp(v_tracker.get_center())), FadeOut(traj1), frame.animate.reorient(-42, 74, 0, (-0.14, 0.44, -0.03), 3.68), run_time=5)
        ...
        self.add(wing_path)
        wing_rotation.set_value(0)
        self.play(wing_rotation.animate.set_value(TAU).set_anim_args(run_time=6), frame.animate.reorient(135, 75, 0, (-0.14, 0.44, -0.03), 3.68).set_anim_args(run_time=10))
        wing_path.clear_updaters()
        self.play(Rotate(Group(v_tracker, wing_path), TAU, axis=UR, about_point=ORIGIN), run_time=10)
        self.wait()
        ...
        self.play(tangent_plane.animate.shift(1.01 * v_tracker.get_center()).set_fill(opacity=0.15), wing_path.animate.shift(1.01 * v_tracker.get_center()), wing_vector_offset.animate.set_value(1), run_time=2)
        self.play(frame.animate.reorient(73, 72, 0, (-0.14, 0.44, -0.03), 3.68), wing_rotation.animate.set_value(0), run_time=6)
        frame.clear_updaters()
        frame.add_ambient_rotation(-2 * DEG)
        points = fibonacci_sphere(1000)
        field = VectorField(lambda ps: np.array([faulty_perp(p) for p in ps]), axes, sample_coords=1.01 * points, color=PINK, max_vect_len_to_step_size=2, density=1, tip_width_ratio=4, tip_len_to_width=0.005)
        ...
        v_tracker.suspend_updating()
        frame.clear_updaters()
        self.play(frame.animate.reorient(-22, 41, 0, (-0.1, -0.02, 0.13), 2.89), v_tracker.animate.move_to(normalize(LEFT + OUT)), plane.animate.set_opacity(0.5), run_time=2)
        self.play(Rotate(v_tracker, TAU, axis=UP, about_point=ORIGIN), run_time=10)

    def alternate_trajectories(self):
        traj1 = ParametricCurve(lambda t: 2 * np.array([math.sin(t), 0, -math.cos(t)]), t_range=(0, TAU, 0.1))
        traj1.set_stroke(YELLOW, 3, 0.5)
        traj1.set_z_index(1)
        prop_tracker = ValueTracker(0)
        ...
        def update_plane(s3, use_curvature=False):
            s3.place_on_path(traj1, prop_tracker.get_value(), use_curvature=use_curvature)
        self.play(plane.animate.place_on_path(traj1, prop_tracker.get_value()), ShowCreation(traj1), FadeIn(axes), FadeIn(xy_plane), frame.animate.reorient(-52, 71, 0, (-0.29, 0.06, 0.48), 5.98))
        self.play(prop_tracker.animate.set_value(1), UpdateFromFunc(plane, update_plane), rate_func=linear, run_time=8)
...
class RadioBroadcast(InteractiveScene):
    ...
    def construct(self):
        ...
        def update_shells(shells):
            alpha = 0.001 + time_tracker.get_value() % 1
        ...
        for theta in np.linspace(0, PI, n_characters):
            character = PiCreature(height=1.0, mode=random.choice(modes), color=random.choice([BLUE_A, BLUE_B, BLUE_C, BLUE_D, BLUE_D]))
        ...
        frame.clear_updaters()
        self.play(LaggedStartMap(FadeIn, characters, lag_ratio=0.2, run_time=3), frame.animate.set_theta(0))
        ...
        E_wave, B_wave = [TimeVaryingVectorField(func, axes, sample_coords=sample_points, color=color, max_vect_len_to_step_size=np.inf, stroke_width=3) for color, func in zip([RED, TEAL], [E_wave_func, B_wave_func])]
        points = sample_points
        self.play(VFadeIn(E_wave, time_span=(0, 1)), VFadeIn(B_wave, time_span=(0, 1)), FadeOut(characters, time_span=(0, 1)), frame.animate.reorient(49, 69, 0, (4.48, -0.2, -0.1), 2.71), rate_tracker.animate.set_value(0.5), run_time=3)
        self.play(frame.animate.reorient(122, 73, 0, (4.48, -0.2, -0.1), 2.71), run_time=11)
        radius = 5
        ...
        def get_sample_vects(point):
            ...
            result = VGroup((Vector(func(point.reshape(1, -1), curr_time).flatten(), fill_color=color) for color, func in zip([RED, TEAL], [E_wave_func, B_wave_func])))
            result.shift(point)
            return result
        sample_vects = always_redraw(lambda: get_sample_vects(sample_point))
        self.play(GrowArrow(prop_vect), E_wave.animate.set_stroke(opacity=0.1), B_wave.animate.set_stroke(opacity=0.1), VFadeIn(sample_vects))
        self.wait(6)
        ...
        fields = VGroup((VectorField(lambda p: func(p, time_tracker.get_value()), axes, sample_coords=sample_coords, stroke_width=3, max_vect_len_to_step_size=np.inf, max_vect_len=np.inf, color=color) for func, color in zip([E_wave_func, B_wave_func], [RED, TEAL])))
        ...
        self.play(FadeIn(sphere, time_span=(0, 2)), ShowCreation(fields, lag_ratio=0), frame.animate.reorient(36, 39, 0, (0.11, -0.24, 0.76), 11.24), run_time=12)
