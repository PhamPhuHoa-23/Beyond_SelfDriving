...
class WhyDoWeCare(TeacherStudentsScene):
    ...
    def construct(self):
        stds = self.students
        morty = self.teacher
        self.play(self.change_students('confused', 'erm', 'concentrating', look_at=self.screen))
        self.wait(3)
        self.play(stds[2].change('erm', stds[1].eyes), stds[1].says('I’m sorry, why\ndo we care?', mode='sassy'), stds[0].change('thinking', self.screen), morty.change('well'))
        self.wait(2)
        self.play(self.change_students('pondering', 'maybe', 'pondering', look_at=self.screen))
        self.play(morty.says('Topology has\nmore subtle utility', mode='tease'), stds[0].animate.look_at(morty.eyes), stds[1].debubble(), stds[2].change('hesitant', morty.eyes))
        self.wait(3)

class RenameTheorem(InteractiveScene):

    def construct(self):
        name1, name2 = names = VGroup(Text('Hairy Ball Theorem'), Text('Sphere Vector Field Theorem'))
        names.scale(1.25)
        ...
        self.add(name1)
        self.wait()
        self.play(ShowCreation(lines[1]), name1['Ball'].animate.set_opacity(0.5), FadeTransformPieces(name1['Ball'].copy(), name2['Sphere']))
        self.play(ShowCreation(lines[0]), name1['Hairy'].animate.set_opacity(0.5), FadeTransformPieces(name1['Hairy'].copy(), name2['Vector Field']))
        self.play(TransformFromCopy(name1['Theorem'], name2['Theorem']))
        self.wait()
...
class CommentOnForce(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        equation = Tex("m x''(t) = \\text{Lift} + \\text{Gravity}", t2c={"x''(t)": RED, '\\text{Lift}': PINK, '\\text{Gravity}': BLUE})
        equation.move_to(self.hold_up_spot, DOWN)
        equation.shift_onto_screen()
        self.play(morty.change('tease'), self.change_students('thinking', 'erm', 'concentrating', look_at=self.screen))
        self.wait(2)
        self.play(morty.change('raise_right_hand', equation), self.change_students('pondering', 'confused', 'hesitant', look_at=equation), Write(equation))
        self.wait(3)
...
class StatementOfTheorem(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(Write(statement, run_time=3, lag_ratio=0.1))
        self.wait()
        statement.set_backstroke(BLACK, 5)
        for text, color in [('continuous', BLUE), ('one null vector', YELLOW)]:
            self.play(FlashUnder(statement[text], time_width=1.5, run_time=2, color=color), statement[text].animate.set_fill(color))
            self.wait()
...
class PedanticStudent(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(LaggedStart(stds[2].says('But atmosphere\nis 3D!', mode='angry', look_at=morty.eyes, bubble_direction=LEFT), morty.change('guilty'), stds[0].change('hesitant', look_at=stds[2].eyes), stds[1].change('hesitant', look_at=stds[2].eyes)))
        self.wait(2)
...
class YouAsAMathematician(InteractiveScene):
    ...
    def construct(self):
        ...
        label = VGroup(Text('You', font_size=72), Text('The mathematician').set_color(GREY_B))
        label.arrange(DOWN)
...
class ThreeCases(InteractiveScene):
    ...
    def construct(self):
        titles = VGroup(VGroup(Text('2 null points'), Text('Obvious')), VGroup(Text('1 null point'), Text('Clever')), VGroup(Text('0 null points'), Text('Very clever')))
        for title in titles:
...
class ProofOutline(InteractiveScene):
    ...
    def construct(self):
        ...
        implies = Tex('\\Longrightarrow', font_size=120)
        ...
        self.wait()
        self.play(DrawBorderThenFill(frames[1]), impossibility.animate.scale(0.5).next_to(frames[1], UP))
        self.wait()
        ...
        self.play(FadeOut(implies), FadeOut(frames[1]))
        self.play(GrowFromCenter(brace), FadeIn(question, lag_ratio=0.1))
        self.wait()
...
class AimingForRediscovery(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(morty.change('tease'), self.change_students('pondering', 'happy', 'hooray', look_at=self.screen))
        self.wait(3)
        self.play(FadeIn(goal, shift=UP), morty.change('raise_right_hand'), self.change_students('pondering', 'pondering', 'pondering', look_at=goal))
        self.wait(2)
...
class TwoFactsForEachPoint(InteractiveScene):
    ...
    def construct(self):
        features = VGroup(Tex('\\text{1)  } p \\rightarrow -p', font_size=72), Tex('\\text{2)  } &\\text{Motion varies }\\\\ &\\text{continuously with } p', font_size=72))
...
class TwoKeyFeatures(InteractiveScene):
    ...
    def construct(self):
        features = VGroup(Text('1) Inside out'), Text('2) Avoids the origin'))
        ...
        for feature in features:
            ...
            self.wait()
        self.play(features[0].animate.scale(1.25, about_edge=LEFT), features[1].animate.scale(0.75, about_edge=LEFT).set_fill(opacity=0.5).shift(DOWN))
        self.wait()
        ...
        net_flow_p1 = TexText('Final flux = $+1.0$', t2c={'+1.0': GREEN}, font_size=60)
        net_flow_p1.next_to(implies1, RIGHT)
        self.play(ShowCreation(rect1), FadeIn(implies1, scale=2, shift=0.25 * RIGHT))
        self.play(FadeIn(net_flow_p1, lag_ratio=0.1))
        self.wait()
        contra = Tex('\\bot', font_size=90)
        contra.to_corner(DR)
...
class DumbQuestion(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(stds[2].says('Isn’t it\nobvious?', mode='confused', look_at=self.screen, bubble_direction=LEFT), stds[1].change('angry', look_at=morty.eyes), stds[0].change('erm', look_at=morty.eyes), morty.change('guilty'))
        self.wait(2)
        self.play(stds[0].change('confused', self.screen), stds[1].change('sassy', self.screen), stds[2].change('maybe', morty.eyes))
        self.wait(4)
...
class InsideOutsideQuestion(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(outside, lag_ratio=0.1), FadeOut(inside, lag_ratio=0.1))
        self.wait()
...
class WhatIsInsideAndOutside(TeacherStudentsScene):
    ...
    def construct(self):
        stds = self.students
        morty = self.teacher
        self.play(stds[2].says('Hang on, what\ndo you mean\n“paint the outside”?', mode='maybe', bubble_direction=LEFT), stds[1].change('erm', self.screen), stds[0].change('pondering', self.screen), morty.change('tease'))
        self.wait(5)

class PToNegP(InteractiveScene):

    def construct(self):
        p, to, neg_p = expression = VGroup(Tex('p'), Tex('\\longrightarrow'), Tex('-p'))
...
class SimplerInsideOutProgression(InteractiveScene):
    ...
    def construct(self):
        parts = VGroup(Tex('(x, y, z)'), Vector(0.75 * DOWN), Tex('(-x, -y, z)'), Vector(0.75 * DOWN), Tex('(-x, -y, -z)'))
        parts.arrange(DOWN, buff=MED_SMALL_BUFF)
        self.add(parts[0])
        self.wait()
        for i in [0, 2]:
            src, arrow, trg = parts[i:i + 3]
            self.play(TransformMatchingStrings(src.copy(), trg), GrowArrow(arrow), run_time=1)
            self.wait()
...
class ReferenceInsideOutMovie(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(stds[0].change('pondering', self.screen), stds[1].change('erm', self.screen), stds[2].says(Text('Huh? I thought\nyou can turn a\nsphere inside out!', t2s={'can': ITALIC}, font_size=42), mode='confused', look_at=self.screen, bubble_direction=LEFT), morty.change('guilty'))
        self.wait(2)
...
class FluxDecimals(InteractiveScene):
    ...
    def construct(self):
        ...
        def update_color(dec, epsilon=0.0001):
            value = dec.get_value()
            if value > epsilon:
                dec.set_color(GREEN)
...
class DivergenceTheorem(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.change_students('pondering', 'confused', 'tease', look_at=self.screen), morty.change('tease'))
        self.wait(3)
        self.play(morty.change('raise_right_hand', div_theorem), FadeIn(div_theorem, shift=UP), self.change_students('thinking', 'confused', 'happy', look_at=div_theorem))
        self.wait()
...
class ThinkAboutOrigin(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        self.play(morty.says(Text('Think about why\ncrossing the origin\nis significant', font_size=36)), self.change_students('thinking', 'tease', 'pondering', look_at=self.screen))
        self.wait(5)
...
class FrameIntuitionVsExamples(InteractiveScene):
    ...
    def construct(self):
        titles = VGroup(Text('Intuitive idea'), Text('Counterexample'), Text('Clever proof'))
        ...
        ideas = VGroup(Text('Turning a sphere\ninside-out must crease it'), Text('All closed loops\nhave inscribed rectangles'))
        for idea in ideas:
        ...
        self.play(FadeIn(titles[1], lag_ratio=0.1))
        self.wait()
        self.play(FadeOut(ideas[0]), FadeIn(ideas[1]), FadeOut(titles[1], lag_ratio=0.1), FadeIn(titles[2], lag_ratio=0.1))
        self.wait()

class DimensionGeneralization(InteractiveScene):

    def construct(self):
        row_labels = VGroup(Text('Dimension'), Text('Can you\ncomb a ball?'))
        n_cols = 15
        ...
        self.play(LaggedStartMap(FadeIn, cells[3:], lag_ratio=0.5), LaggedStartMap(FadeIn, dim_labels[2:], lag_ratio=0.5), run_time=3)
        self.wait()
        self.play(Write(mark_labels[0]))
        self.wait()
        frame = self.frame
        for i in [0, 1]:
            self.play(LaggedStart((TransformFromCopy(mark_labels[i], mark_label, path_arc=30 * DEG) for mark_label in mark_labels[i + 2::2]), lag_ratio=0.25), frame.animate.reorient(0, 0, 0, (4.15, -2.53, 0.0), 12.34), run_time=3)
            self.wait()
        last_det = VGroup()
        ...
        for dim in range(2, 12):
            ...
            self.play(Transform(highlight_rect, rect), FadeIn(det_tex), FadeOut(last_det))
            self.wait()
            last_det = det_tex
...
class MoreRigorNeeded(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.add(words)
        self.play(morty.change('hesitant'), self.change_students('confused', 'sassy', 'erm', look_at=self.screen), Write(words))
...
class AskAboutHomology(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        chain = Tex('\\cdots C_{2} \\xrightarrow{\\partial_2} C_1 \\xrightarrow{\\partial_1} C_0 \\xrightarrow{\\partial_0} 0')
        chain.next_to(stds[2], UP, buff=MED_LARGE_BUFF)
        chain.align_to(stds[2].get_center(), RIGHT)
        self.play(morty.change('well', stds[2].eyes), stds[2].says('What about\nusing homology?', mode='tease', bubble_direction=LEFT, look_at=morty.eyes))
        self.play(stds[2].change('raise_left_hand', chain), self.change_students('confused', 'erm'), Write(chain))
        self.wait(3)
...
class RotationIn2D(InteractiveScene):
    ...
    def construct(self):
        ...
        basis_vectors = VGroup(Vector(RIGHT).set_color(GREEN), Vector(UP).set_color(RED))
        self.frame.set_height(4)
        self.add(back_grid, grid, basis_vectors)
        self.wait()
        self.play(Rotate(basis_vectors, PI, about_point=ORIGIN), Rotate(grid, PI, about_point=ORIGIN), run_time=5)
        self.wait()
...
class InversionIn3d(InteractiveScene):
    ...
    def construct(self):
        ...
        cubes = VGroup((VCube(side_length=1).move_to([x, y, z], DL + IN) for x, y, z in it.product(*3 * [coord_range])))
        cubes.set_fill(opacity=0)
        cubes.set_stroke(WHITE, 1, 0.25)
        basis_vectors = VGroup(Vector(RIGHT).set_color(GREEN), Vector(UP).set_color(RED), Vector(OUT).set_color(BLUE))
        frame.reorient(29, 72, 0, ORIGIN, 5)
        self.add(axes, cubes, basis_vectors)
        rot_group = VGroup(cubes, basis_vectors)
        self.wait()
        self.play(Rotate(rot_group, PI, about_point=ORIGIN, run_time=3))
        self.play(rot_group.animate.stretch(-1, 2, about_point=ORIGIN), run_time=2)
        self.wait(3)

class HypersphereWords(InteractiveScene):

    def construct(self):
        words = VGroup(Text('Hairs on a neatly-combed 4d hypersphere', font_size=60), Text('Represented via streographic projection into 3d space', font_size=48).set_color(GREY_A))
