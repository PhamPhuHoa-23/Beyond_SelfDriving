...
def inv_streographic_proj(points2d):
    ...
    return np.array([2 * u / denom, 2 * v / denom, (norm_squared - 1) / denom]).T
...
def get_sphereical_vector_field(v_func, axes, points, color=BLUE, stroke_width=1, mvltss=1.0, tip_width_ratio=4, tip_len_to_width=0.01):
    field = VectorField(v_func, axes, sample_coords=1.01 * points, max_vect_len_to_step_size=mvltss, density=1, stroke_width=stroke_width, tip_width_ratio=tip_width_ratio, tip_len_to_width=tip_len_to_width)
...
class TeddyHeadSwirl(InteractiveScene):
    ...
    def construct(self):
        ...
        top_sample_points = np.array([point for point in fibonacci_sphere(10000) if point[2] > 0.95])
        full_sample_points = fibonacci_sphere(1000)
        top_lines, full_lines = lines = VGroup(*(SphereStreamLines(v_func, axes, sample_coords=samples, arc_len=0.25, dt=0.05, max_time_steps=10) for samples in [top_sample_points, full_sample_points]))
...
class IntroduceVectorField(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(30, 65, 0), ShowCreation(sphere), Write(mesh, time_span=(1, 3), lag_ratio=0.01), run_time=3)
        v_tracker = Point()
        ...
        self.play(VFadeIn(v_dot), v_tracker.animate.move_to(radius * normalize(RIGHT + OUT)), run_time=2)
        self.wait()
        plane.update()
        ...
        def vector_field_3d(points3d):
            x, y, z = points3d.T
            return np.array([np.cos(3 * x) * np.sin(y), np.cos(5 * z) * np.sin(x), -z ** 2 + x]).T
        ...
        self.wait(4)
        self.play(FadeOut(plane), FadeOut(tangent_vect), FadeOut(v_dot), frame.animate.reorient(0, 59, 0, (-0.03, 0.15, -0.08), 6.8), run_time=4)
        self.wait(5)
        dots, dense_dots = [DotCloud(fibonacci_sphere(num), radius=0.01) for num in [4000, 400000]]
        for mob in [dots, dense_dots]:
            mob.make_3d()
            mob.set_color(WHITE)
            mob.scale(radius * 1.01)
        dense_field = self.get_vector_field(axes, v_func, 50000, mvltss=5.0)
        dense_field.set_stroke(opacity=0.35)
        dots.set_radius(0.02)
        dense_dots.set_radius(0.01)
        self.play(ShowCreation(dots, run_time=3))
        self.wait()
        self.play(FadeOut(dots, time_span=(1.5, 2.5)), FadeOut(field, time_span=(2.5, 3)), ShowCreation(dense_field), run_time=3)
        frame.clear_updaters()
        self.play(frame.animate.reorient(-66, 52, 0, (-0.51, 0.27, 0.2), 2.81), run_time=4)
        self.wait()
        field.save_state()
        field.set_stroke(width=1e-06)
        self.play(dense_field.animate.set_stroke(width=1e-06), Restore(field, time_span=(1, 3)), frame.animate.reorient(48, 54, 0, (-2.74, -2.49, -0.1), 11.93), VFadeIn(plane), VFadeIn(tangent_vect), run_time=5)
        self.wait(5)
        v_tracker.clear_updaters()
        null_point = 3 * normalize(np.array([-1, -0.25, 0.8]))
        self.play(v_tracker.animate.move_to(null_point), frame.animate.reorient(-43, 55, 0, (-2.42, 2.76, -0.55), 9.22), run_time=3)
        self.remove(tangent_vect)
        v_dot.update()
        self.play(FadeIn(v_dot, scale=0.25), FadeOut(plane, scale=0.25))
        self.wait()
        static_stream_lines = SphereStreamLines(lambda p: v_func(np.array(p).reshape(-1, 3)).flatten(), axes, density=400, stroke_width=2, magnitude_range=(0, 10), solution_time=1)
        ...
        self.play(FadeOut(v_dot), FadeIn(earth, time_span=(0, 3)), field.animate.set_stroke(opacity=0.25).set_anim_args(time_span=(0, 2)), frame.animate.reorient(-90, 74, 0, (-1.37, 0.04, 0.37), 5.68))
        self.wait(10)
        frame.clear_updaters()
        dense_field = self.get_vector_field(axes, v_func, 10000)
        dense_field.set_stroke(opacity=0.35)
        self.play(FadeOut(field, run_time=2), FadeIn(dense_field, run_time=2), frame.animate.reorient(-69, 54, 0, (-0.01, 0.19, -0.04), 4.81), run_time=5)
        self.wait(3)
        self.play(frame.animate.reorient(129, 70, 0, (-0.18, 0.18, -0.1), 8.22), run_time=20)
        self.wait(10)
    ...
    def get_vector_field(self, axes, v_func, n_points, start_point=None, mvltss=1.0, random_order=False):
        ...
        alpha = clip(inverse_interpolate(10000, 1000, n_points), 0, 1)
    ...
    def old(self):
        ...
        field.add_updater(lambda m: m.become(self.get_vector_field(int(density_tracker.get_value()), axes, v_func)))
        self.add(field)
        field.resume_updating()
        frame.suspend_updating()
        self.play(density_tracker.animate.set_value(50000), frame.animate.reorient(-30, 50, 0, (-0.07, 0.02, 0.36), 3.01), run_time=5)
...
class StereographicProjection(InteractiveScene):
    ...
    def construct(self):
        ...
        n_dots = 20
        sample_points = np.array([math.cos(theta) * OUT + math.sin(theta) * RIGHT for theta in np.linspace(0, TAU, n_dots + 2)[1:-1]])
        sphere_dots, plane_dots, proj_lines = self.get_dots_and_lines(sample_points)
        self.play(sphere.animate.set_clip_plane(UP, 0), frame.animate.reorient(-43, 74, 0, (0.0, 0.0, -0.0), 3.5), FadeIn(sphere_dots, time_span=(1, 2)), ShowCreation(proj_lines, lag_ratio=0, time_span=(1, 2)), run_time=2)
        frame.add_ambient_rotation(2 * DEG)
        sphere_dot_ghosts = sphere_dots.copy().set_opacity(0.5)
        self.remove(sphere_dots)
        self.add(sphere_dot_ghosts)
        self.play(TransformFromCopy(sphere_dots, plane_dots, lag_ratio=0.5, run_time=10))
        self.wait(3)
        planar_group = Group(sphere_dot_ghosts, plane_dots, proj_lines)
        sample_points = fibonacci_sphere(200)
        sphere_dots, plane_dots, proj_lines = self.get_dots_and_lines(sample_points)
        self.play(sphere.animate.set_clip_plane(UP, -1), frame.animate.reorient(-65, 73, 0, (-0.09, -0.01, -0.15), 5.08), ShowCreation(proj_lines, lag_ratio=0), FadeOut(planar_group), run_time=2)
        self.wait(4)
        ...
        self.remove(sphere_dots)
        self.add(sphere_dot_ghosts)
        self.play(TransformFromCopy(sphere_dots, plane_dots, run_time=3))
        self.wait(3)
        ...
        self.play(Transform(plane_dots, sphere_dot_ghosts), FadeOut(sphere_dot_ghosts, scale=0.9), Transform(plane, proj_plane), proj_lines.animate.set_stroke(opacity=0.2), run_time=4)
        self.play(frame.animate.reorient(-20, 38, 0, (-0.04, -0.03, 0.13), 3.54), run_time=5)
        self.wait(5)
        self.play(frame.animate.reorient(-27, 73, 0, (-0.03, 0.03, 0.04), 5.27), Restore(plane), FadeOut(plane_dots), run_time=5)
        self.wait(2)
        xy_field = VectorField(lambda ps: np.array([RIGHT for p in ps]), plane)
        xy_field.set_stroke(BLUE)
        xy_field.save_state()
        xy_field.set_stroke(width=1e-06)
        ...
        self.play(Transform(plane, proj_plane), Transform(xy_field, proj_field), run_time=5)
        self.play(frame.animate.reorient(-35, 31, 0, (0.05, 0.22, 0.22), 1.59), FadeOut(proj_lines), run_time=10)
        self.wait(8)
        proto_stream_lines = VGroup((Line([x, y, 0], [x + 20, y, 0]).insert_n_curves(25) for x in range(-100, 100, 10) for y in np.arange(-100, 100, 0.25)))
        ...
        self.play(FadeIn(sphere), FadeOut(xy_field), plane.animate.fade(0.25), xy_field.animate.set_stroke(opacity=0.5), frame.animate.reorient(-30, 29, 0, ORIGIN, 3.0), run_time=3)
        self.wait(30)
        return
    ...
    def get_dots_and_lines(self, sample_points, color=YELLOW, radius=0.025, stroke_opacity=0.35):
        ...
        proj_lines = VGroup((VGroup((Line(OUT, dot.get_center()) for dot in dots)) for dots in [plane_dots, sphere_dots]))
    ...
    def flow_with_projection_insertion(self):
        ...
        field = get_sphereical_vector_field(lambda p3d: stereographic_vector_field(p3d, right_func), axes, fibonacci_sphere(1000), stroke_width=0.5)
        field.set_stroke(WHITE, opacity=0.5)
        self.play(proj_lines.animate.set_opacity(0.4))
        self.play(particle_opacity_tracker.animate.set_value(0.15), Transform(moving_particles, proj_particles), moving_particles_opacity_tracker.animate.set_value(1), run_time=5)
        self.remove(moving_particles)
        self.add(sphere, particles, proj_particles)
        self.play(proj_lines.animate.set_opacity(0.1), frame.animate.reorient(-30, 29, 0, ORIGIN, 3.0), proj_particle_radius_tracker.animate.set_value(0.0075), FadeIn(field), run_time=3)
        self.wait(15)
...
class SimpleRightwardFlow(InteractiveScene):
    ...
    def construct(self):
        ...
        xy_field.set_stroke(BLUE)
        self.add(xy_field)
        proto_stream_lines = VGroup((Line([x, y, 0], [x + 1, y, 0]).insert_n_curves(20) for x in np.arange(-10, 10, 0.5) for y in np.arange(-10, 10, 0.1)))
        for line in proto_stream_lines:
            line.virtual_time = 1
        proto_stream_lines.set_stroke(WHITE, 2, 0.8)
...
class SingleNullPointHairyBall(InteractiveScene):
    ...
    def construct(self):
        ...
        points = np.array([normalize(sphere.uv_func(TAU * random.random(), math.acos(pre_v))) for pre_v in np.random.uniform(*v_range, n_points)])
        pre_field, field = fields = [get_sphereical_vector_field(func, axes, points, stroke_width=3, mvltss=3) for func in [out_func, v_func]]
        for lines in fields:
        ...
        self.play(ReplacementTransform(pre_field, field, run_time=2), frame.animate.reorient(-92, 11, 0).set_anim_args(run_time=7), FadeIn(top_q, time_span=(3.5, 5)))
        self.play(frame.animate.reorient(-168, 127, 0), FadeOut(top_q, time_span=(4.2, 5)), run_time=10)
...
class PairsOfNullPoints(InteractiveScene):
    ...
    def construct(self):
        ...
        frame.add_ambient_rotation(1 * DEG)
        new_params = [(rotation_matrix_transpose(90 * DEG, UP), -5), (rotation_matrix_transpose(30 * DEG, DOWN), -2), (rotation_matrix_transpose(120 * DEG, LEFT), 3), (rotation, -1)]
        for new_rot, offset in new_params:
            new_field = self.get_spherical_field(axes, lambda ps: twirl_func(ps, offset), rotation=new_rot)
            self.play(Transform(field, new_field, run_time=2))
        self.play(offset_tracker.animate.set_value(-3), UpdateFromFunc(field, lambda m: m.become(self.get_spherical_field(axes, lambda ps: twirl_func(ps, offset_tracker.get_value()), rotation=rotation))), run_time=4)
    ...
    def get_streamlines(self, field, axes, radius, density=100):
        streamlines = SphereStreamLines(lambda p: field.func(np.array(p).reshape(-1, 3)).flatten(), axes, density=density, solution_time=1.0, dt=0.05)
    ...
    def get_spherical_field(self, axes, plane_func, n_sample_points=2000, stroke_width=2, rotation=np.identity(3)):
        ...
        field = get_sphereical_vector_field(v_func, axes, sample_points, stroke_width=stroke_width)
...
class AskAboutOutside(InteractiveScene):
    ...
    def construct(self):
        ...
        sphere.set_clip_plane(UP, 2)
        self.play(FadeIn(morty, time_span=(0, 1)), sphere.animate.set_opacity(0.25), frame.animate.reorient(-5, 83, 0, (1.09, 0.51, 1.62), 1.15), run_time=3)
        self.play(GrowArrow(out_arrow))
        self.wait()
        self.play(TransformFromCopy(morty, mirror_morty), GrowArrow(in_arrow))
        self.wait()
        out_arrows, in_arrows = all_arrows = VGroup((VGroup((Arrow(radius * a1 * point, radius * a2 * point, fill_color=color, thickness=4, buff=0) for point in compass_directions(24))) for a1, a2, color in [(1.1, 1.5, BLUE), (0.9, 0.7, RED)]))
        all_arrows.rotate(90 * DEG, RIGHT, about_point=ORIGIN)
        self.play(frame.animate.reorient(-4, 82, 0, (0.15, -0.01, 0.02), 8.08), LaggedStartMap(GrowArrow, out_arrows, lag_ratio=0.01, time_span=(3, 6)), run_time=6)
        self.wait()
        self.play(FadeOut(out_arrows), FadeOut(morty_group), sphere.animate.set_opacity(1))
        ...
        self.play(Transform(sphere_group, twisted_sphere), frame.animate.reorient(-1, 73, 0, (-0.23, 0.06, -0.09), 6.0), run_time=5)
        self.play(frame.animate.reorient(12, 73, 0, (-0.09, 0.1, -0.06), 4.71), Transform(sphere_group, squish_sphere), run_time=5)
        index = 9200
        ...
        dot.set_radius(0.01)
        dot.set_z_index(1)
        in_vect, out_vect = vects = VGroup((Vector(sign * 0.2 * normal, thickness=0.5, fill_color=color).shift(point) for sign, color in zip([1, -1], [RED, BLUE])))
        for vect in vects:
            vect.set_perpendicular_to_camera(frame)
        self.add(dot)
        self.play(FadeIn(dot), frame.animate.reorient(-47, 81, 0, (-0.55, 0.17, 0.13), 0.65), run_time=3)
        ...
        self.play(FadeOut(vects, time_span=(0, 1)), FadeOut(dot, time_span=(0, 1)), Homotopy(homotopy, sphere_group), frame.animate.reorient(-44, 78, 0, (-0.16, 0.02, 0.31), 4.18), run_time=6)
...
class InsideOut(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(dashed_semi_circle), MoveAlongPath(neg_p_dot, semi_circle, rate_func=linear), TransformFromCopy(p_label, neg_p_label, time_span=(2, 3)), Rotate(p_label, 90 * DEG, OUT), frame.animate.reorient(59, 87, 0), run_time=3)
        frame.add_ambient_rotation(-2 * DEG)
        self.wait(2)
        self.play(ReplacementTransform(dashed_semi_circle, dashed_line))
        self.wait(3)
        angles = np.linspace(0, 90 * DEG, 15)[1:]
        top_dots, low_dots, lines = groups = [Group((template.copy().rotate(angle, axis=UP, about_point=ORIGIN) for angle in angles)) for template in [p_dot, neg_p_dot, dashed_line]]
        for group in groups:
            group.set_submobject_colors_by_gradient(YELLOW, BLUE, interp_by_hsl=True)
        self.play(FadeIn(top_dots, lag_ratio=0.1))
        frame.clear_updaters()
        self.play(LaggedStartMap(ShowCreation, lines, lag_ratio=0.25), LaggedStart((TransformFromCopy(top_dot, low_dot, rate_func=linear) for top_dot, low_dot in zip(top_dots, low_dots)), lag_ratio=0.25, group_type=Group), frame.animate.reorient(-49, 60, 0), run_time=7)
        cap = self.get_colored_sphere(radius, inner_scale, v_range=(0.75 * PI, PI))
        frame.clear_updaters()
        self.add(cap, sphere, mesh)
        self.play(FadeOut(p_label), FadeOut(neg_p_label), FadeOut(top_dots), FadeOut(lines), FadeOut(low_dots), FadeOut(p_dot), FadeOut(neg_p_dot), FadeOut(dashed_line), FadeOut(sphere, 0.1 * UP), FadeIn(cap))
        self.wait()
        self.play(frame.animate.reorient(-48, 93, 0), run_time=3)
        self.wait()
        uv_samples = np.array([[(u + v) % TAU, v] for v in np.linspace(0.75 * PI, 0.95 * PI, 10) for u in np.linspace(0, TAU, 20)])
        normal_vectors = VGroup((VMobject().set_points_as_corners([ORIGIN, RIGHT, RIGHT, 2 * RIGHT]) for sample in uv_samples))
        ...
        rot_arcs = VGroup(Arrow(4 * RIGHT, 4 * LEFT, path_arc=180 * DEG, thickness=5), Arrow(4 * LEFT, 4 * RIGHT, path_arc=180 * DEG, thickness=5))
        flip_arrows = VGroup(Arrow(3 * IN, 3.2 * OUT, thickness=5), Arrow(3 * OUT, 3.2 * IN, thickness=5)).rotate(90 * DEG).shift(4 * RIGHT)
        self.play(ShowCreation(antipode_lines, lag_ratio=0, suspend_mobject_updating=True), frame.animate.reorient(8, 79, 0, (0.0, 0.02, 0.0)), run_time=4)
        self.wait()
        normal_vectors.add_updater(update_normal_vectors)
        self.play(Write(rot_arcs, lag_ratio=0, run_time=1), Rotate(cap, PI, axis=OUT, run_time=3, about_point=ORIGIN), Rotate(mesh, PI, axis=OUT, run_time=3, about_point=ORIGIN))
        self.play(FadeOut(rot_arcs))
        self.play(FadeIn(flip_arrows, time_span=(0, 1)), Transform(cap, anti_cap), mesh.animate.stretch(-1, 2, about_point=ORIGIN), VFadeOut(antipode_lines), run_time=3)
        self.play(FadeOut(flip_arrows))
        self.play(frame.animate.reorient(-9, 96, 0, (0.0, 0.02, 0.0)), run_time=5)
        self.wait()

    def get_colored_sphere(self, radius=3, inner_scale=0.999, outer_color=BLUE_E, inner_color=GREY_BROWN, u_range=(0, TAU), v_range=(0, PI)):
    ...
    def old_homotopy(self):
        ...
        antipode_lines = VGroup((Line(point, -point) for point in random.sample(list(cap[0].get_points()), 100)))
        antipode_lines.set_stroke(YELLOW, 1, 0.25)
        self.play(Homotopy(lambda x, y, z, t: homotopy(x, y, z, t, -inner_scale), cap[0]), Homotopy(lambda x, y, z, t: homotopy(x, y, z, t, -1.0 / inner_scale), cap[1]), ShowCreation(antipode_lines, lag_ratio=0), frame.animate.reorient(-65, 68, 0), run_time=3)
...
class UnitNormals(InteractiveScene):
    ...
    def construct(self):
        ...
        surface = Torus()
        surface.set_color(GREY_D)
        uv_samples = np.array([[u, v] for v in np.linspace(0, TAU, 25) for u in np.linspace(0, TAU, 50)])
        ...
        update_normal_vectors(normal_vectors)
        frame.reorient(51, 57, 0, (0.39, 0.01, -0.5), 8.0)
...
class DefineOrientation(InsideOut):
    ...
    def construct(self):
        ...
        dot.set_z_index(2)
        lat_label, lon_label = lat_lon_labels = VGroup(Tex('\\text{Lat: }\\, 10^\\circ'), Tex('\\text{Lon: }\\, 10^\\circ'))
        lat_lon_labels.arrange(DOWN, aligned_edge=LEFT)
        lat_lon_labels.fix_in_frame()
        lat_lon_labels.to_corner(UL)
        lat_label.make_number_changeable('10', edge_to_fix=RIGHT).add_updater(lambda m: m.set_value(np.round(self.get_lat_lon(*uv_tracker.get_value())[0])))
        lon_label.make_number_changeable('10', edge_to_fix=RIGHT).add_updater(lambda m: m.set_value(np.round(self.get_lat_lon(*uv_tracker.get_value())[1])))
        ...
        label_template = Tex('(10^\\circ, 10^\\circ)', isolate=['10'])
        ...
        u_radius = 50 * DEG
        v_radius = 30 * DEG
        all_labels = VGroup((get_lat_lon_label(sub_u, sub_v) for sub_u in np.arange(u - u_radius, u + u_radius, 10 * DEG) for sub_v in np.arange(v - v_radius, v + v_radius, 5 * DEG)))
        all_labels.sort(lambda p: get_norm(p - dot.get_center()))
        self.play(frame.animate.reorient(-39, 60, 0, (-0.64, 0.45, 0.06), 4.33), run_time=3)
        self.play(FadeIn(all_labels, lag_ratio=0.001, run_time=3), dot.animate.set_opacity(0.25))
        self.wait()
        ...
        for vect in tangent_vects:
            vect.set_perpendicular_to_camera(frame)
        self.play(dot.animate.set_opacity(1).scale(0.5), all_labels.animate.set_stroke(width=0).set_fill(opacity=0.25))
        self.wait()
        lat_line2 = TracedPath(dot.get_center, stroke_color=TEAL, stroke_width=1)
        lat_line2.set_scale_stroke_with_zoom(True)
        self.add(lat_line2)
        self.play(uv_tracker.animate.increment_value([30 * DEG, 0]).set_anim_args(rate_func=wiggle), FadeOut(lon_line), run_time=3)
        ...
        self.add(lon_line2)
        self.play(tangent_vects[0].animate.set_fill(opacity=0.5).set_anim_args(time_span=(0, 1)), uv_tracker.animate.increment_value([0, 30 * DEG]).set_anim_args(rate_func=wiggle), FadeOut(lat_line), run_time=3)
        lon_line2.clear_updaters()
        self.play(GrowArrow(tangent_vects[1]))
        self.play(tangent_vects.animate.set_fill(opacity=1))
        self.wait()
        normal_vect = Arrow(point, point + 0.5 * np.cross(u_step, v_step), thickness=2, buff=0)
        normal_vect.set_fill(BLUE, 0.8)
        normal_vect.rotate(90 * DEG, axis=normal_vect.get_vector())
        self.play(GrowArrow(normal_vect, time_span=(2, 4)), FadeOut(lat_lon_labels, time_span=(0, 2)), frame.animate.reorient(-96, 54, 0, (-0.23, -1.08, 0.25), 3.67), run_time=8)
        normal_field = get_sphereical_vector_field(lambda p: p, ThreeDAxes(), points=np.array([sphere.uv_func(u, v) for u in np.arange(0, TAU, TAU / 60) for v in np.arange(0, PI, PI / 30)]))
        normal_field.save_state()
        normal_field.set_stroke(width=1e-06)
        self.play(Restore(normal_field, time_span=(2, 5)), frame.animate.reorient(-103, 62, 0, (-0.09, 0.25, 0.23), 7.26), run_time=10)
        self.wait()
        ...
        self.play(FadeOut(normal_field), FadeOut(normal_vect), Restore(light), all_labels.animate.set_fill(opacity=0.5), frame.animate.reorient(-63, 66, 0, (-0.09, 0.25, 0.23), 7.26), run_time=3)
        dot.suspend_updating()
        ...
        self.play(MoveToTarget(group), frame.animate.reorient(-94, 60, 0, (0.5, 0.84, 0.91), 1.06), run_time=8)
        self.wait()
        self.play(MoveAlongPath(dot, lat_line2, run_time=5))
        self.play(MoveAlongPath(dot, lon_line2, run_time=5))
        new_normal_vector = Vector(0.35 * normalize(np.cross(tangent_vects[0].get_vector(), tangent_vects[1].get_vector())), fill_color=BLUE, thickness=1)
        new_normal_vector.shift(tangent_vects[0].get_start())
        new_normal_vector.set_perpendicular_to_camera(self.frame)
        self.play(GrowArrow(new_normal_vector), frame.animate.reorient(-125, 52, 0, (0.5, 0.84, 0.91), 1.06), run_time=2)
        self.wait()
        self.play(FadeOut(new_normal_vector, time_span=(0, 1)), Restore(group), frame.animate.reorient(-78, 66, 0, (-0.13, 0.12, 0.13), 6.54), run_time=5)
        ...
        antipode_lines = VGroup((Line(p1, p2) for index in [0, 1] for p1, p2 in zip(group[index].get_points(), anti_group[index].get_points())))
        antipode_lines.set_stroke(YELLOW, 1, 0.1)
        self.play(FadeOut(sphere, scale=0.9), FadeOut(all_labels), VGroup(lat_line2, lon_line2).animate.set_stroke(width=1), FadeIn(normal_vect), frame.animate.reorient(-93, 55, 0, (-0.26, -0.85, 0.24), 4.27), run_time=2)
        self.play(TransformFromCopy(group, anti_group), frame.animate.reorient(-194, 105, 0, (0.7, -0.22, -0.61), 4.64), ShowCreation(antipode_lines, lag_ratio=0), run_time=5)
        ...
        self.wait()
        self.play(MoveAlongPath(dot, anti_group[0], run_time=5), anti_group[1].animate.set_stroke(opacity=0.25), anti_group[2][1].animate.set_fill(opacity=0.25))
        self.play(MoveAlongPath(dot, anti_group[1], run_time=5), anti_group[1].animate.set_stroke(opacity=1), anti_group[2][1].animate.set_fill(opacity=1), anti_group[0].animate.set_stroke(opacity=0.25), anti_group[2][0].animate.set_fill(opacity=0.25))
        self.wait()
        self.play(anti_group[0].animate.set_stroke(opacity=1), anti_group[2][0].animate.set_fill(opacity=1))
        new_normal = normal_vect.copy()
        new_normal.shift(-2 * point)
        self.play(GrowArrow(new_normal), frame.animate.reorient(-197, 90, 0, (1.16, -0.3, -0.99), 4.64), run_time=3)
        self.wait()
        anti_normal_field = get_sphereical_vector_field(lambda p: -p, ThreeDAxes(), points=np.array([sphere.uv_func(u, v) for u in np.arange(0, TAU, TAU / 60) for v in np.arange(0, PI, PI / 30)]))
        frame.reorient(162, 89, 0, (1.16, -0.3, -0.99), 4.64)
        self.play(FadeOut(group), FadeOut(antipode_lines), FadeIn(anti_normal_field, time_span=(0, 2)), frame.animate.reorient(360 - 177, 82, 0, (-0.03, -0.11, 0.86), 9.51), run_time=10)
    ...
    def get_lat_lon(self, u, v):
        return np.array([v / DEG - 90, u / DEG - 180])
...
class FlowingWater(InteractiveScene):
    ...
    def construct(self):
        ...
        frame.add_ambient_rotation(2 * DEG)
        self.add(axes)
        water = self.get_water(sigma0=0.2, n_droplets=1000000, opacity=0.1, refresh_ratio=0.015)
        ...
        self.play(water.radius_tracker.animate.set_value(0.02), water.opacity_tracker.animate.set_value(0.1), water.refresh_sigma_tracker.animate.set_value(2.5), frame.animate.reorient(-120, 80, 0, ORIGIN, 8.0), run_time=10)
        self.wait(20)
        sphere = Sphere(radius=radius)
        ...
        def get_unit_normal_field(u_range, v_range):
            return get_sphereical_vector_field(lambda p: p, ThreeDAxes(), points=np.array([sphere.uv_func(u, v) for u in u_range for v in v_range]))
        normal_field = get_unit_normal_field(np.arange(0, TAU, TAU / 60), np.arange(0, PI, PI / 30))
        self.play(ShowCreation(sphere), Write(mesh, lag_ratio=0.001), run_time=2)
        self.wait(2)
        self.play(FadeIn(normal_field))
        self.wait(7.2)
        u_range_params = (0 * DEG, 15 * DEG, 5 * DEG)
        v_range_params = (120 * DEG, 130 * DEG, 5 * DEG)
        patch = Sphere(radius=radius, u_range=u_range_params[:2], v_range=v_range_params[:2])
        patch.set_color(WHITE, 0.6)
        patch_normals = get_unit_normal_field(np.arange(*u_range_params), np.arange(*v_range_params))
        self.play(FadeOut(sphere, time_span=(0, 1)), FadeOut(normal_field, time_span=(0, 1)), FadeIn(patch, time_span=(0, 1)), FadeIn(patch_normals, time_span=(0, 1)), mesh.animate.set_stroke(opacity=0.1), frame.animate.reorient(19, 55, 0, (1.65, 0.28, 0.98), 2.32), water.opacity_tracker.animate.set_value(0.125), water.radius_tracker.animate.set_value(0.01), run_time=5)
        frame.clear_updaters()
        self.wait(8)
        patch_group = Group(patch, patch_normals)
        self.play(Rotate(patch_group, PI, axis=UP), run_time=2)
        self.wait(2.5)
        self.play(Rotate(patch_group, PI, axis=UP, time_span=(0, 1)), FadeIn(sphere), FadeIn(normal_field), frame.animate.reorient(30, 78, 0, (0.19, -0.03, 0.09), 5.67), water.opacity_tracker.animate.set_value(0.15), water.radius_tracker.animate.set_value(0.015), run_time=5)
        self.play(FadeOut(patch_group))
        frame.add_ambient_rotation(-1 * DEG)
        self.wait(3)
        self.play(FadeOut(normal_field), sphere.animate.set_shading(1, 1, 1))
        self.wait(5)
        sphere_group = Group(sphere, mesh)

        def alt_uv_func(u, v, params, wiggle_size=1.0, max_freq=4):
            x, y, z = sphere.uv_func(u, v)
            return (x + wiggle_size * params[0] * np.cos(max_freq * params[1] * y), y + wiggle_size * params[4] * np.cos(max_freq * params[5] * z), z + wiggle_size * params[2] * np.cos(max_freq * params[3] * x))
        np.random.seed(3)
        for n in range(20):
            params = np.random.random(6)
            new_sphere = ParametricSurface(lambda u, v: alt_uv_func(u, v, params), u_range=sphere.u_range, v_range=sphere.v_range, resolution=sphere.resolution)
    ...
    def get_water(self, n_droplets=500000, radius=0.02, opacity=0.2, sigma0=3, refresh_sigma=2.5, velocity=10, refresh_ratio=0.01):
    ...
    def rotate_patch(self):
        ...
        self.wait(5)
        self.play(MoveAlongPath(patch_group, path), frame.animate.reorient(-25, 92, 0, (-0.75, -0.96, -0.16), 3.16), run_time=5)
        self.wait(5)
...
class SurfaceFoldedOverSelf(FlowingWater):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(-25, 76, 0, (-0.13, 0.29, 0.85), 3.38).set_anim_args(time_span=(2, 5)), Transform(sphere_group, sphere_target), water.radius_tracker.animate.set_value(0.01), water.opacity_tracker.animate.set_value(0.1), run_time=5)
        ...
        self.play(ShowCreation(dashed_line, run_time=5), ShowCreation(dashed_line_ghost, run_time=5))
        self.wait()
        self.play(GrowArrow(vect1), GrowArrow(vect3))
        self.wait(3)
        self.play(GrowArrow(vect2))
        self.wait(4)
        self.play(FadeOut(VGroup(vect1, vect2, vect3)), FadeOut(dashed_line), FadeOut(dashed_line_ghost))
        frame.clear_updaters()
        self.play(Restore(sphere_group), frame.animate.reorient(10, 78, 0, ORIGIN, 7), run_time=7)
...
class InsideOutWithNormalField(InteractiveScene):
    ...
    def construct(self):
        ...
        def get_unit_normal_field(u_range, v_range, sign=1):
            return get_sphereical_vector_field(lambda p: sign * p, axes, points=np.array([sphere.uv_func(u, v) for u in u_range for v in v_range]), color=BLUE_D, stroke_width=3)
        ...
        self.play(Homotopy(homotopy, sphere_group), sign_tracker.animate.set_value(-1), run_time=5)
        self.wait(3)
...
class ProjectedCombedHypersphere(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(axes)
        flow_lines = VGroup((self.get_flow_line_from_point(normalize(np.random.normal(0, 1, 4))) for n in range(2000)))
        ...
        self.play(LaggedStartMap(VShowPassingFlash, flow_lines, lag_ratio=3 / len(flow_lines), run_time=45, time_width=0.7, rate_func=linear))
    ...
    def get_hypersphere_circle_points(self, point4d, n_samples=100):
        ...
        return np.array([math.cos(a) * point4d + math.sin(a) * perp for a in np.linspace(0, TAU, n_samples)])
