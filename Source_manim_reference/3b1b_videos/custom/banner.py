...
class Banner(Scene):
    ...
    def construct(self):
        ...
        plane = NumberPlane((-7, 7), (-5, 5), height=10 * 1.5, width=14 * 1.5, faded_line_ratio=4)
    ...
    def get_pis(self):
        return VGroup(Randolph(color=BLUE_E, mode='pondering'), Randolph(color=BLUE_D, mode='hooray'), Randolph(color=BLUE_C, mode='tease'), Mortimer(color=GREY_BROWN, mode='thinking'))
    ...
    def get_date_message(self):
        return Text(self.pre_date_text, self.date, t2c={self.date: YELLOW})

    def get_supporter_note(self):
        return OldTexText('(Available to supporters for review now)', color='#F96854')
