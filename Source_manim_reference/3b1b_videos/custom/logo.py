...
class Logo(VMobject):
    ...
    blue_spike_colors: list[ManimColor] = ['#528EA3', '#3E6576', '#224C5B', BLACK]
    brown_spike_colors: list[ManimColor] = ['#754C24', '#603813', '#42210b', BLACK]
    ...
    def add_iris_back(self):
        blue_iris_back = AnnularSector(inner_radius=self.pupil_radius, outer_radius=self.outer_radius, angle=270 * DEGREES, start_angle=180 * DEGREES, fill_color=self.iris_background_blue, fill_opacity=1, stroke_width=0)
        brown_iris_back = AnnularSector(inner_radius=self.pupil_radius, outer_radius=self.outer_radius, angle=90 * DEGREES, start_angle=90 * DEGREES, fill_color=self.iris_background_brown, fill_opacity=1, stroke_width=0)
        self.iris_background = VGroup(blue_iris_back, brown_iris_back)
        self.add(self.iris_background)

    def add_spikes(self):
        layers = VGroup()
        radii = np.linspace(self.outer_radius, self.pupil_radius, self.n_spike_layers, endpoint=False)
        radii[:2] = radii[1::-1]
        ...
        for radius in radii:
            ...
            triangle, right_half_triangle = [Polygon(radius * UP, half_base * RIGHT, vertex3, fill_opacity=1, stroke_width=0) for vertex3 in (half_base * LEFT, ORIGIN)]
            left_half_triangle = right_half_triangle.copy()
            left_half_triangle.flip(UP, about_point=ORIGIN)
            n_spikes = self.n_spikes
            full_spikes = [triangle.copy().rotate(-angle, about_point=ORIGIN) for angle in np.linspace(0, TAU, n_spikes, endpoint=False)]
            index = 3 * n_spikes // 4
            if radius == radii[0]:
                ...
                half_spikes = [right_half_triangle.copy(), left_half_triangle.copy().rotate(90 * DEGREES, about_point=ORIGIN), right_half_triangle.copy().rotate(90 * DEGREES, about_point=ORIGIN), left_half_triangle.copy()]
                layer = VGroup(*it.chain(half_spikes[:1], full_spikes[1:index], half_spikes[1:3], full_spikes[index + 1:], half_spikes[3:]))
    ...
    def add_pupil(self):
        self.pupil = Circle(radius=self.pupil_radius, fill_color=BLACK, fill_opacity=1, stroke_width=0, stroke_color=BLACK, sheen=0.0)
    ...
    def get_blue_part_and_brown_part(self):
        if len(self.pupil) == 1:
            self.cut_pupil()
        blue_part = VGroup(self.iris_background[0], *[layer[:layer.brown_index] for layer in self.spike_layers], self.pupil[0])
        brown_part = VGroup(self.iris_background[1], *[layer[layer.brown_index:] for layer in self.spike_layers], self.pupil[1])
...
class LogoGeneration(LogoGenerationTemplate):
    ...
    def construct(self):
        ...
        self.play(Restore(logo.iris_background, rate_func=squish_rate_func(smooth, 1.0 / 3, 1), run_time=2), AnimationGroup(*(LaggedStartMap(get_spike_animation, layer, run_time=2, lag_ratio=2 / len(layer), path_arc=-90 * DEGREES) for layer, a in zip(layers, [0, 2, 1, 0]))), Animation(logo.pupil), Write(alt_name), Write(name), run_time=3)
        self.wait(0.25)
        self.play(Transform(wrong_spike, wrong_spike.real_saved_state), Animation(self.logo), run_time=0.75)
...
class SortingLogoGeneration(LogoGenerationTemplate):
    ...
    def get_logo_animations(self, logo):
        ...
        return [Restore(logo.iris_background, rate_func=squish_rate_func(smooth, 2.0 / 3, 1), run_time=3), AnimationGroup(*[LaggedStartMap(get_spike_animation, layer, run_time=2, lag_ratio=0.2) for layer, a in zip(layers, [0, 2, 1, 0])]), Animation(logo.pupil)]
...
class LogoTest(Scene):
    ...
    def construct(self):
        ...
        for n, denom in zip(n_range, np.linspace(14, 28, len(n_range))):
            logo = Logo(**{'iris_background_blue': '#78C0E3', 'iris_background_brown': '#8C6239', 'blue_spike_colors': ['#528EA3', '#3E6576', '#224C5B', BLACK], 'brown_spike_colors': ['#754C24', '#603813', '#42210b', BLACK], 'n_spike_layers': 4, 'n_spikes': n, 'spike_angle': TAU / denom})
            self.add(logo)
            self.wait()
            self.clear()
        self.add(logo)
...
class LogoGenerationFlurry(LogoGenerationTemplate):
    ...
    def get_logo_animations(self, logo):
        ...
        return [Restore(logo.iris_background, run_time=3), AnimationGroup(*[LaggedStartMap(Restore, layer, run_time=3, path_arc=180 * DEGREES, lag_ratio=0.8) for layer, a in zip(layers, [0, 0.2, 0.1, 0])]), Animation(logo.pupil)]

class WrittenLogo(LogoGenerationTemplate):

    def get_logo_animations(self, logo):
        return [Write(logo, stroke_color=None, stroke_width=2, run_time=3, lag_ratio=0.005)]
    ...
    def construct(self):
        ...
        for layer in spike_layers:
            for n, spike in enumerate(layer):
                ...
                anims.append(Rotate(spike, -angle, about_point=ORIGIN, run_time=5))
                self.add(spike)
        self.add(pupil)
        ...
        name.flip().flip()
        self.play(*anims, VFadeIn(iris, run_time=3), UpdateFromAlphaFunc(Mobject(), lambda m, a: update(a), run_time=3), Write(name, run_time=3))
        self.wait(2)

class Vertical3B1B(Scene):

    def construct(self):
        words = OldTexText('3', 'Blue', '1', 'Brown')
        words.scale(2)
        words[::2].scale(1.2)
        buff = 0.2
        words.arrange(DOWN, buff=buff, aligned_edge=LEFT)
