...
class Car(SVGMobject):
    ...
    def get_front_line(self):
        return DashedLine(self.get_corner(UP + RIGHT), self.get_corner(DOWN + RIGHT), color=DISTANCE_COLOR, dash_length=0.05)
...
class MoveCar(ApplyMethod):
    ...
    def begin(self):
        super().begin()
        car = self.mobject
        distance = get_norm(op.sub(self.target_mobject.get_right(), self.starting_mobject.get_right()))
...
class Guitar(SVGMobject):
    ...
    def __init__(self, height=2.5, fill_color=GREY_D, fill_opacity=1, stroke_color=WHITE, stroke_width=0.5):
        super().__init__(height=height, fill_color=fill_color, fill_opacity=fill_opacity, stroke_color=stroke_color, stroke_width=stroke_width)
...
class DeckOfCards(VGroup):
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(*(PlayingCard(value=value, suit=suit, **kwargs) for value in possible_values for suit in possible_suits))

class PlayingCard(VGroup):

    def __init__(self, key=None, value=None, suit=None, height=2, height_to_width=3.5 / 2.5, card_height_to_symbol_height=7, card_width_to_corner_num_width=10, card_height_to_corner_num_height=10, color=GREY_A, turned_over=False, possible_suits=['hearts', 'diamonds', 'spades', 'clubs'], possible_values=list(map(str, list(range(2, 11)))) + ['J', 'Q', 'K', 'A'], **kwargs):
        self.key = key
        self.value = value
        self.suit = suit
        self.height = height
        ...
        super().__init__(**kwargs)
        self.key = key
        self.add(Rectangle(height=self.height, width=self.height / self.height_to_width, stroke_color=WHITE, stroke_width=2, fill_color=self.color, fill_opacity=1))
        if self.turned_over:
    ...
    def get_face_card_design(self, value, symbol):
        ...
        sub_rect = Rectangle(stroke_color=BLACK, fill_opacity=0, height=0.9 * self.get_height(), width=0.6 * self.get_width())
        sub_rect.move_to(self)
        pi_color = symbol.get_color()
        if Color(pi_color) == Color(BLACK):
            pi_color = GREY_D
        pi_mode = {'J': 'plain', 'Q': 'thinking', 'K': 'hooray'}[value]
        pi_creature = PiCreature(mode=pi_mode, color=pi_color)
...
class AoPSLogo(SVGMobject):
    ...
    def __init__(self, **kwargs):
        ...
        index_lists = [(10, 11, 12, 13, 14, 21, 22, 23, 24, 27, 28, 29, 30), (0, 1, 2, 3, 4, 15, 16, 17, 26), (5, 6, 7, 8, 9, 18, 19, 20, 25)]
