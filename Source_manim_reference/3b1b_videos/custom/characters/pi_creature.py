...
class PiCreature(SVGMobject):
    ...
    def __init__(self, mode: str='plain', color: ManimColor=BLUE_E, stroke_width: float=0.0, stroke_color: ManimColor=BLACK, fill_opacity: float=1.0, height: float=3, flip_at_start: bool=False, start_corner: Vect3 | None=None, **kwargs):
        self.mode = mode
        self.bubble = None
        self.body = VMobject()
        super().__init__(file_name=self.get_svg_file_path(mode), color=None, stroke_width=stroke_width, stroke_color=stroke_color, fill_opacity=fill_opacity, height=height, **kwargs)
        self.init_structure()
    ...
    def get_svg_file_path(self, mode):
        ...
        if os.path.exists(path):
            ...
            logging.log(logging.WARNING, f'No design with mode {mode}')
            folder = get_directories()['pi_creature_images']
            return os.path.join(folder, 'plain.svg')

    def init_structure(self):
        parts = self.submobjects
        self.eyes: VGroup = self.draw_eyes(original_irises=VGroup(parts[2], parts[6]), original_pupils=VGroup(parts[8], parts[9]))
    ...
    def get_arm_copies(self):
        body = self.body
        return VGroup(*[body.copy().pointwise_become_partial(body, *alpha_range) for alpha_range in (self.right_arm_range, self.left_arm_range)])
    ...
    def says(self, content, mode='speaking', look_at=None, **kwargs) -> Animation:
        ...
        return PiCreatureBubbleIntroduction(self, content, target_mode=mode, look_at=look_at, bubble_type=SpeechBubble, **kwargs)

    def thinks(self, content, mode='thinking', look_at=None, **kwargs) -> Animation:
        from custom.characters.pi_creature_animations import PiCreatureBubbleIntroduction
        return PiCreatureBubbleIntroduction(self, content, target_mode=mode, look_at=look_at, bubble_type=ThoughtBubble, **kwargs)
    ...
    def debubble(self, mode='plain', look_at=None, **kwargs):
        if self.bubble is None:
            logging.log(logging.WARNING, f'Calling debubble on PiCreature with no bubble')
            return self.change(mode, look_at)
...
class Mortimer(PiCreature):
    ...
    def __init__(self, mode: str='plain', color: ManimColor=GREY_BROWN, flip_at_start: bool=True, **kwargs):
...
class BabyPiCreature(PiCreature):
    ...
    def __init__(self, mode: str='plain', height: float=1.5, eye_scale_factor: float=1.2, pupil_scale_factor: float=1.3, **kwargs):
