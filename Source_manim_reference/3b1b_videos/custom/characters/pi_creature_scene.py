...
class PiCreatureScene(InteractiveScene):
    ...
    default_pi_creature_kwargs: dict = dict(color=BLUE, flip_at_start=False)
    ...
    def pi_changes(self, *modes, look_at=None, lag_ratio=0.5, run_time=1):
        return LaggedStart(*(pi.change(mode, look_at) for pi, mode in zip(self.pi_creatures, modes) if mode is not None), lag_ratio=lag_ratio, run_time=1)

    def introduce_bubble(self, pi_creature, content, bubble_type=SpeechBubble, target_mode=None, look_at=None, bubble_config=dict(), bubble_removal_kwargs=dict(), added_anims=[], **kwargs):
        ...
        if pi_creature in pi_creatures_with_bubbles:
            ...
            anims += [ReplacementTransform(old_bubble, bubble), FadeTransform(old_bubble.content, bubble.content), pi_creature.change(target_mode, look_at)]
        else:
            anims.append(PiCreatureBubbleIntroduction(pi_creature, content, target_mode=target_mode, bubble_type=bubble_type, bubble_config=bubble_config, **kwargs))
    ...
    def anims_from_play_args(self, *args, **kwargs):
        ...
        non_pi_creature_anims = [anim for anim in animations if len(set(anim.mobject.get_family()).intersection(pi_creatures)) == 0]
...
class MortyPiCreatureScene(PiCreatureScene):
    default_pi_creature_kwargs: dict = dict(color=GREY_BROWN, flip_at_start=True)
...
class TeacherStudentsScene(PiCreatureScene):
    ...
    def setup(self):
        ...
        self.screen = ScreenRectangle(height=self.screen_height, fill_color=BLACK, fill_opacity=1.0)
    ...
    def add_background(self, color: ManimColor):
        self.background = FullScreenFadeRectangle(fill_color=color, fill_opacity=1)
    ...
    def student_says(self, content, target_mode=None, bubble_direction=LEFT, bubble_config=None, index=2, **kwargs):
        if target_mode is None:
        ...
        return self.pi_creature_says(self.get_students()[index], content, target_mode=target_mode, bubble_config=bubble_config, **kwargs)
    ...
    def change_students(self, *modes, look_at=None, lag_ratio=0.5, run_time=1):
        return LaggedStart(*(student.change(mode, look_at) for student, mode in zip(self.get_students(), modes) if mode is not None), lag_ratio=lag_ratio, run_time=1)
