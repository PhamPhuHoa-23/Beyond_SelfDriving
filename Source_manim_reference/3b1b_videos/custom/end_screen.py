...
class PatreonEndScreen(InteractiveScene):
    ...
    def construct(self):
        black_rect = Rectangle(fill_color=BLACK, fill_opacity=1, stroke_width=3, stroke_color=BLACK, width=FRAME_WIDTH, height=FRAME_HEIGHT)
    ...
    def get_names(self):
        ...
        names = [*patron_path.read_text().split('\n'), *hardcoded_patron_path.read_text().split('\n')]
        substitution_path = Path(data_path, 'patron_name_replacements.json')

        def normalize_name(name):
            return name.strip().lower()
        substitution_map = dict(((normalize_name(key), value) for key, value in json.loads(substitution_path.read_text()).items()))
        names = [substitution_map.get(normalize_name(name), name) for name in names]
        names = list(dict.fromkeys(names))
        if 'Anonymous' in names:
            names.remove('Anonymous')
        if self.capitalize:
            names = [' '.join([s.capitalize() for s in name.split(' ')]) for name in names]
    ...
    def get_name_mobjects(self, x=0, max_width=FRAME_WIDTH - 1):
        ...
        name_labels = VGroup(*map(Text, ProgressDisplay(names, leave=False, desc='Writing names')))
        name_labels.scale(self.patron_scale_val)
        for label in name_labels:
            label.set_max_width(self.max_patron_width)
        name_labels.arrange_in_grid(n_cols=self.n_patron_columns, aligned_edge=LEFT)
    ...
    def scroll(self, name_labels, *added_anims):
        ...
        frame_shift = ApplyMethod(self.frame.shift, distance * DOWN, run_time=self.scroll_time, rate_func=linear)
...
class SideScrollEndScreen(PatreonEndScreen):
    ...
    thanks_style = dict(fill_color=YELLOW, font_size=36)
    ...
    def construct(self):
        ...
        self.scroll(self.name_labels, LaggedStartMap(VShowPassingFlash, self.elements.copy().set_stroke(WHITE, 5).insert_n_curves(100), time_width=2, run_time=3, lag_ratio=0.25), FadeIn(self.elements, time_span=(1, 2)), Write(self.link_text), self.get_early_view_comment())
    ...
    def add_background_rectangle(self):
        ...
        panels = VGroup(ScreenRectangle().stretch(2 / 5, 0), ScreenRectangle().stretch(3 / 5, 0))
    ...
    def add_element_outlines(self):
        screen, channel, link = elements = VGroup(ScreenRectangle().set_width(self.video_element_width), Circle().set_width(self.link_element_width), Square().set_width(self.link_element_width))
    ...
    def get_early_view_comment(self):
        ...
        blank = VMobject()
        bubble = randy.get_bubble(Text(self.early_view_text, font_size=20), SpeechBubble, direction=RIGHT)
        bubble.fix_in_frame()
        return Succession(AnimationGroup(Write(bubble), randy.change('tease', self.link_text)), Blink(randy), Animation(blank, run_time=1), bubble.animate.set_opacity(0), randy.change('raise_left_hand', look_at=self.link_text).set_anim_args(run_time=1), Animation(blank, run_time=2), Blink(randy), Animation(blank, run_time=3), Blink(randy), Animation(blank, run_time=2), Blink(randy), Animation(blank, run_time=3), Blink(randy))
    ...
    def add_thanks_and_names(self):
        ...
        thanks_panel.set_height(thanks.get_height() + 2 * self.thanks_panel_margin, stretch=True, about_edge=UP)
        thanks.move_to(thanks_panel)
        h_line = Line(LEFT, RIGHT)
        h_line.match_width(thanks_panel)
        h_line.move_to(thanks_panel, DOWN)
        h_line.set_stroke(WHITE, 1)
        name_labels = self.get_name_mobjects(x=thanks_panel.get_x(), max_width=thanks_panel.get_width() - 1)
        name_labels.set_z_index(-1)
