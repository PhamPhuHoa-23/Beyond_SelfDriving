...
class LayersOfAbstraction(EnumerableSaveScene):
    ...
    def get_layers(self):
        ...
        arrow_labels = VGroup(OldTex('m_1').next_to(arrows[0], UP, SMALL_BUFF), OldTex('m_2').next_to(arrows[1], RIGHT, SMALL_BUFF), OldTex('m_2 \\circ m_1').rotate(-45 * DEGREES).move_to(arrows[2]).shift(MED_SMALL_BUFF * DL))
...
class DifferenceOfSquares(Scene):
    ...
    def construct(self):
        squares = VGroup(*[VGroup(*[Square() for x in range(8)]).arrange(RIGHT, buff=0) for y in range(8)]).arrange(DOWN, buff=0)
        ...
        self.play(ReplacementTransform(squares[:-3].copy().set_fill(opacity=0), new_squares), ShowCreation(arrow), lag_ratio=0)
...
class Lightbulbs(EnumerableSaveScene):
    ...
    def construct(self):
        ...
        for d1, d2 in it.product(dots, dots):
            line = Line(d1.get_center(), d2.get_center(), path_arc=30 * DEGREES, buff=dot_radius, color=YELLOW)
            connections.add(line)
        lower_dots = dots[:3].copy()
        lower_dots.next_to(dots, DOWN, buff=2)
        lower_lines = VGroup(*[Line(d.get_center(), ld.get_center(), buff=dot_radius) for d, ld in it.product(dots, lower_dots[1:])])
        lower_lines.match_style(connections)
        top_dot = dots[0].copy()
        top_dot.next_to(dots, UP, buff=2)
        top_lines = VGroup(*[Line(d.get_center(), top_dot.get_center(), buff=dot_radius) for d in dots])
    ...
    def construct(self):
        ...
        for l1, l2 in zip(layers, layers[1:]):
            for d1, d2 in it.product(l1, l2):
                ...
                line = Line(d1.get_center(), d2.get_center(), buff=d1.get_width() / 2, color=color, stroke_width=2 * np.random.random())
                lines.add(line)
