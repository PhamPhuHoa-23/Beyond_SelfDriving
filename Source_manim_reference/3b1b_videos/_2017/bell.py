...
class PhotonPassesCompletelyOrNotAtAll(DirectionOfPolarizationScene):
    CONFIG = {'pol_filter_configs': [{'include_arrow_label': False, 'label_tex': '\\text{Filter}'}], 'EMWave_config': {'wave_number': 0, 'A_vect': [0, 1, 1], 'start_point': FRAME_X_RADIUS * LEFT + DOWN + 1.5 * OUT}, 'start_theta': -0.9 * np.pi, 'target_theta': -0.6 * np.pi, 'apply_filter': True, 'lower_portion_shift': 3 * IN, 'show_M_vects': True}
    ...
    def construct(self):
        ...
        green_flash = ApplyMethod(pol_filter.set_fill, GREEN, rate_func=squish_rate_func(there_and_back, 0.4, 0.6), run_time=passing_photon.run_time)
        self.play(DrawBorderThenFill(pol_filter), Write(pol_filter.label, run_time=2))
        self.move_camera(theta=self.target_theta)
        self.play(FadeIn(passing_words), passing_photon, green_flash)
        self.play(lower_filter.restore, lower_filter.shift, self.lower_portion_shift, FadeIn(filtered_words))
        red_flash = ApplyMethod(lower_filter.set_fill, RED, rate_func=squish_rate_func(there_and_back, 0.4, 0.6), run_time=filtered_photon.run_time)
        for x in range(4):
            self.play(passing_photon, filtered_photon, green_flash, red_flash)
            self.wait()

class PhotonPassesCompletelyOrNotAtAllForWavesVideo(PhotonPassesCompletelyOrNotAtAll):
    CONFIG = {'show_M_vects': False}
...
class PhotonsThroughPerpendicularFilters(PhotonPassesCompletelyOrNotAtAll):
    CONFIG = {'filter_x_coordinates': [-2, 2], 'pol_filter_configs': [{'filter_angle': 0}, {'filter_angle': np.pi / 2}], 'start_theta': -0.9 * np.pi, 'target_theta': -0.6 * np.pi, 'EMWave_config': {'A_vect': [0, 0, 1], 'start_point': FRAME_X_RADIUS * LEFT + DOWN + OUT}, 'apply_filter': False}
    ...
    def construct(self):
        ...
        self.move_camera(theta=self.target_theta, added_anims=[ApplyFunction(self.reposition_filter_label, zero_filter)])
    ...
    def get_photons(self):
        ...
        self.update_mobjects()
        return [WavePacket(filter_distance=FRAME_X_RADIUS + x, get_filtered=True, em_wave=self.em_wave.copy(), run_time=1) for x in (-2, 2, 10)]

    def get_probability_text(self, prob=0):
        prob_text = OldTex('P(', '\\substack', '{\\text{photons that make it} \\\\ ', ' \\text{here } ', '\\text{make it}', ' \\text{ here} }', ')', '=', str(int(prob * 100)), '\\%', arg_separator='')
        ...
        arrows = [Arrow(here.get_edge_center(IN), DOWN + OUT + x * RIGHT, color=here.get_color(), normal_vector=DOWN + OUT) for here, x in ((here1, 0), (here2, 4))]
        prob_text.add(*arrows)
        return prob_text

class MoreFiltersMoreLight(FilterScene):
    CONFIG = {'filter_x_coordinates': list(range(-2, 3)), 'pol_filter_configs': [{'include_arrow_label': False, 'filter_angle': angle} for angle in np.linspace(0, np.pi / 2, 5)], 'ambient_rotation_rate': 0, 'arrow_rgb': (0, 0, 0), 'background_rgb': (245, 245, 245)}
    ...
    def construct(self):
        ...
        self.move_camera(phi=0.9 * np.pi / 2, theta=-0.95 * np.pi)
        self.play(Animation(pfs[0]), pfs[2].shift, 3 * OUT, Animation(pfs[4]))
        self.wait()
        self.play(Animation(pfs[0]), pfs[2].shift, 3 * IN, Animation(pfs[4]))
        pfs[1].shift(8 * OUT)
        self.play(Animation(pfs[0]), pfs[1].shift, 8 * IN, Animation(VGroup(pfs[2], pfs[4])), run_time=2)
        self.wait()
        pfs[3].shift(8 * OUT)
        self.play(Animation(VGroup(*pfs[:3])), pfs[3].shift, 8 * IN, Animation(VGroup(*pfs[4:])), run_time=2)
        self.wait()
    ...
    def get_frame(self):
        ...
        bool_arrays = [(frame[:, :, 0] == r) & (frame[:, :, 1] == g) & (frame[:, :, 2] == b) for r, g, b in self.original_rgbas]
...
class MoreFiltersMoreLightBlackBackground(MoreFiltersMoreLight):
    CONFIG = {'arrow_rgb': (255, 255, 255), 'background_rgb': (0, 0, 0)}

class ConfusedPiCreature(Scene):

    def construct(self):
        randy = Randolph()
        self.play(randy.change, 'confused', 3 * (UP + RIGHT))
...
class AngryPiCreature(PiCreatureScene):
    ...
    def construct(self):
        self.pi_creature_says('No, \\emph{locality} \\\\ must be wrong!', target_mode='angry', look_at=2 * RIGHT, run_time=1)
        self.wait(3)
...
class ShowALittleMath(TeacherStudentsScene):
    ...
    def construct(self):
        exp1 = OldTex('|', '\\psi', '\\rangle = ', '\\alpha', '|\\uparrow\\rangle', '+', '\\beta', '|\\rightarrow\\rangle')
        exp2 = OldTex('|| \\langle', '\\psi', '|', '\\psi', '\\rangle ||^2', '= ', '\\alpha', '^2', '+', '\\beta', '^2')
        ...
        self.play(*[ReplacementTransform(exp1.get_parts_by_tex(tex).copy(), exp2.get_parts_by_tex(tex).copy()) for tex in list(color_map.keys())] + [Write(exp2, run_time=2)])
...
class BasicsOfPolarization(DirectionOfPolarizationScene):
    ...
    def show_continual_wave(self):
        ...
        self.play(FadeOut(em_wave.mobject), FadeOut(self.rectangles))
        self.remove(em_wave)
        self.reference_line.put_start_and_end_on(ORIGIN, RIGHT)

    def show_photons(self):
        quantum_left_words = OldTexText('Quantum', '$\\Rightarrow$')
        quantum_left_words.next_to(ORIGIN, UP + RIGHT)
        quantum_left_words.shift(UP)
        quantum_right_words = OldTexText('Completely through', 'or \\\\', 'Completely blocked')
        ...
        prob_eq = OldTex('&P(', '\\text{Pass}', ')', '=', 'p\\\\', '&P(', '\\text{Blocked}', ')', '=', '1-p')
        prob_eq.set_color_by_tex_to_color_map({'Pass': GREEN, 'Blocked': RED})
        prob_eq.next_to(ORIGIN, DOWN + RIGHT)
        prob_eq.shift(RIGHT)
        prob_eq.rotate(np.pi / 2, RIGHT)
        config = dict(self.EMWave_config)
        config.update({'wave_number': 0, 'A_vect': [0, 1, -1]})
        self.em_wave = EMWave(**config)
        self.update_mobjects()
        passing_photon = WavePacket(em_wave=self.em_wave.copy(), run_time=1.5)
        filtered_photon = WavePacket(em_wave=self.em_wave.copy(), get_filtered=True, run_time=1.5)
        self.play(FadeIn(quantum_words, run_time=2, lag_ratio=0.5))
        anim_sets = [[passing_photon], [filtered_photon, self.get_filter_absorption_animation(self.pol_filter, filtered_photon)]]
        for index in (0, 1):
...
class AngleToProbabilityChart(Scene):
    ...
    def construct(self):
        ...
        right_title = OldTexText('Probability that photons passing \\\\', 'through the first pass through the second')
...
class ShowVariousFilterPairsWithPhotonsOverTime(PhotonsThroughPerpendicularFilters):
    CONFIG = {'filter_x_coordinates': [-2, 2, 2, 2, 2], 'pol_filter_configs': [{'filter_angle': angle} for angle in (0, 0, np.pi / 2, np.pi / 4, np.pi / 8)], 'apply_filter': False}
    ...
    def add_filters(self):
        ...
        self.move_camera(theta=-0.65 * np.pi, added_anims=list(it.chain(*[[pf.arrow_label.rotate, np.pi / 2, OUT, pf.arrow_label.next_to, pf.arrow, RIGHT] for pf in self.pol_filters[:2]])))
    ...
    def show_photons(self, n_photons=5):
        ...
        blocked_photon.rate_func = squish_rate_func(lambda x: x, 0, 0.5)
        first_absorption = self.get_filter_absorption_animation(self.pol_filters[0], blocked_photon)
        first_absorption.rate_func = squish_rate_func(first_absorption.rate_func, 0, 0.5)
...
class ShowVariousFilterPairs(ShowVariousFilterPairsWithPhotonsOverTime):
    CONFIG = {'filter_x_coordinates': [], 'filter_z_coordinates': [2.5, 0, -2.5], 'angles': [0, np.pi / 4, np.pi / 2], 'n_lines': 20, 'new_group_shift_val': 2.5 * IN, 'prev_group_shift_val': 1.75 * IN, 'ambient_rotation_rate': 0.015, 'line_start_length': 16, 'line_end_length': 16, 'lines_depth': 1.2, 'lines_shift_vect': SMALL_BUFF * OUT}
    ...
    def show_light(self, filter_pair):
        ...
        self.play(ShowCreation(lines_to_pf2, **kwargs), Animation(VGroup(pf1, lines_to_pf1)), run_time=1.0 / 6)
        self.play(ShowCreation(lines_from_pf2, **kwargs), Animation(VGroup(pf2, lines_to_pf2, pf1, lines_to_pf1)), run_time=2.0 / 3)
    ...
    def get_lines(self, filter1=None, filter2=None, ratio=1.0, remove_from_bottom=False):
        n = self.n_lines
...
class ForgetPreviousActions(ShowVariousFilterPairs):
    CONFIG = {'filter_x_coordinates': [-6, -2, 2, 2, 2], 'pol_filter_configs': [{'filter_angle': angle} for angle in (np.pi / 4, 0, np.pi / 4, np.pi / 3, np.pi / 6)], 'start_theta': -0.6 * np.pi, 'EMWave_config': {'wave_number': 0, 'start_point': FRAME_X_RADIUS * LEFT + DOWN}, 'apply_filter': False}
    ...
    def construct(self):
        ...
        self.play(ShowCreation(rect2), Write(ignore_words, run_time=1), FadeIn(front_filter), run_time=1.5)
        self.shoot_photon()
        for index in (0, 1, 2):
            self.play(Transform(second_filter, possible_second_filters[index]))
            self.shoot_photon()
    ...
    def shoot_photon(self):
        ...
        added_anims = []
        if photon.filter_distance == FRAME_X_RADIUS + 2:
            added_anims.append(ApplyMethod(self.second_filter.set_color, RED, rate_func=squish_rate_func(there_and_back, 0.5, 0.7)))
        self.play(photon, *added_anims, run_time=1.5)

class IntroduceLabeledFilters(ShowVariousFilterPairs):
    CONFIG = {'filter_x_coordinates': [-5, -2, 1], 'pol_filter_configs': [{'filter_angle': angle} for angle in [0, np.pi / 8, np.pi / 4]], 'start_phi': 0.9 * np.pi / 2, 'start_theta': -0.85 * np.pi, 'target_theta': -0.65 * np.pi, 'lines_depth': 1.7, 'lines_shift_vect': MED_SMALL_BUFF * OUT, 'line_start_length': 3, 'line_end_length': 9, 'ambient_rotation_rate': 0.005}
    ...
    def reposition_camera(self):
        self.move_camera(theta=self.target_theta, added_anims=list(it.chain(*[[pf.arrow_label.rotate, np.pi / 2, OUT, pf.arrow_label.next_to, pf.arrow, RIGHT] for pf in self.pol_filters])))

    def separate_cases(self):
        self.lower_pol_filters = VGroup(self.A_filter.deepcopy(), self.C_filter.deepcopy())
        self.lower_pol_filters.save_state()
        self.lower_pol_filters.fade(1)
        self.play(self.lower_pol_filters.restore, self.lower_pol_filters.shift, 3 * IN, self.pol_filters.shift, 1.5 * OUT)
        self.wait()
    ...
    def show_bottom_lines(self):
        ...
        kwargs = {'rate_func': None, 'lag_ratio': 0, 'run_time': 1.0 / 3}
        ...
        self.play(ShowCreation(lines_from_C), Animation(VGroup(C, lines_to_C, A, lines_to_A)), **kwargs)
    ...
    def comment_on_half_blocked_by_C(self):
        arrow = Arrow(ORIGIN, 3.5 * RIGHT, path_arc=-0.9 * np.pi, color=BLUE, stroke_width=5)
    ...
    def show_top_lines(self):
        ...
        kwargs = {'rate_func': None, 'lag_ratio': 0, 'run_time': 1.0 / 5}
        self.play(ShowCreation(lines_to_A), **kwargs)
        self.play(ShowCreation(lines_to_B), Animation(VGroup(A, lines_to_A)), **kwargs)
        self.play(ShowCreation(lines_to_C), Animation(VGroup(B, lines_to_B, A, lines_to_A)), **kwargs)
        kwargs['run_time'] = 3 * kwargs['run_time']
        self.play(ShowCreation(lines_from_C), Animation(VGroup(C, lines_to_C, B, lines_to_B, A, lines_to_A)), **kwargs)
    ...
    def comment_on_those_blocked_by_B(self):
        arrow0 = Arrow(2 * LEFT, 0.5 * (UP + RIGHT), path_arc=0.8 * np.pi, color=WHITE, stroke_width=5, buff=0)
        ...
        self.wait()
        self.play(ReplacementTransform(words0, words1), ReplacementTransform(arrow0, arrow1))
        self.wait()
...
class IntroduceLabeledFiltersNoRotation(IntroduceLabeledFilters):
    CONFIG = {'ambient_rotation_rate': 0, 'target_theta': -0.55 * np.pi}
...
class RemoveBFromLabeledFilters(IntroduceLabeledFiltersNoRotation):
    ...
    def show_filter_B_removal(self):
        ...
        self.play(top_lines_group.shift, 2 * OUT, mover.restore, mover.shift, 2.5 * IN)
        self.wait()
        self.play(ApplyMethod(sB.shift, 4 * IN, rate_func=running_start), FadeOut(sl1), Animation(sC), FadeOut(sl2))
        self.play(ShowCreation(tl2, run_time=0.25, **kwargs))
        self.play(ShowCreation(tl1, run_time=0.5, **kwargs), Animation(sC), Animation(tl2))
        self.wait()

    def fade_in_labels(self):
        self.play(*list(map(FadeIn, [self.blocked_at_B_label_group, self.blocked_at_C_label_group])))
        self.wait()
...
class VennDiagramProofByContradiction(Scene):
    ...
    def draw_venn_diagram(self, send_to_corner=True):
        A, B, C = venn_diagram = VGroup(*[Circle(radius=3, stroke_width=3, stroke_color=c, fill_opacity=0.2, fill_color=c).shift(vect) for c, vect in zip(self.circle_colors, compass_directions(3, UP))])
        ...
        for circle, char, prop, angle in zip(venn_diagram, 'ABC', props, angles):
            label, alt_label = [OldTexText('%s \\\\' % start, 'through', char + '$\\! \\uparrow$').set_color_by_tex(char, circle.get_color()) for start in ('Would pass', 'Pass')]
        ...
        for circle in venn_diagram:
            ...
            self.play(DrawBorderThenFill(circle, run_time=2), Write(circle.label, run_time=2), *added_anims)
            last_circle = circle
    ...
    def show_100_photons(self):
        photon = FunctionGraph(lambda x: -np.cos(3 * np.pi * x) * np.exp(-x * x), x_min=-2, x_max=2, color=YELLOW, stroke_width=2)
        ...
        self.play(Write(hundred), Write(s), ReplacementTransform(VGroup(photon), photons, lag_ratio=0.5))
    ...
    def show_one_photon_answering_questions(self):
        ...
        answers = OldTexText('Pass through A?', 'Yes\\\\', 'Pass through B?', 'No\\\\', 'Pass through C?', 'No\\\\')
        answers.set_color_by_tex_to_color_map({'Yes': GREEN, 'No': RED})
        ...
        self.play(group.restore, group.scale, 4, group.to_corner, DOWN + RIGHT)
        self.play(photon.eyes.blink_anim())
        self.wait()
        self.play(FadeOut(bubble_group), photon.restore)
    ...
    def put_all_photons_in_A(self):
        ...
        self.play(B_group.fade, 1, C_group.fade, 1, MoveToTarget(A), MoveToTarget(A.label), FadeOut(self.photon_words), self.photons.set_height, 0.85 * A.target.get_height(), self.photons.space_out_submobjects, 0.8, self.photons.move_to, A.target)
        self.wait()
    ...
    def separate_by_B(self):
        ...
        self.play(MoveToTarget(B), MoveToTarget(B.label), in_B.shift, 0.5 * DOWN + 0.2 * LEFT, out_of_B.scale, 1.0 / 0.8, out_of_B.shift, 0.15 * (UP + RIGHT))
        words1 = OldTexText('85 also \\\\', 'pass ', 'B')
        words1.set_color_by_tex('B', GREEN)
        words1.scale(0.8)
        words1.next_to(A_group, LEFT, LARGE_BUFF).shift(UP)
        arrow1 = Arrow(words1.get_right(), in_B.get_corner(UP + LEFT) + MED_LARGE_BUFF * (DOWN + RIGHT))
        ...
        self.play(Write(words1, run_time=1), ShowCreation(arrow1), self.in_A_in_B.set_fill, GREEN, 0.5, Animation(in_B))
        self.wait()
        self.play(ReplacementTransform(words1, words2), ReplacementTransform(arrow1, arrow2), self.in_A_in_B.set_fill, None, 0, Animation(in_B), self.in_A_out_B.set_fill, RED, 0.5, Animation(out_of_B))
        self.wait()
        self.play(ApplyMethod(VGroup(self.in_A_out_B, out_of_B).shift, MED_LARGE_BUFF * UP, rate_func=wiggle, run_time=1.5))
        self.wait(0.5)
        self.in_B = in_B
    ...
    def separate_by_C(self):
        ...
        words = OldTexText('$15\\%$', 'passing', 'B \\\\', 'get blocked by ', 'C')
        words.scale(0.8)
        words.set_color_by_tex_to_color_map({'B': GREEN, 'C': BLUE})
        ...
        self.play(self.in_A_in_B.set_fill, GREEN, 0.5, rate_func=there_and_back)
        self.play(MoveToTarget(C), MoveToTarget(C.label), in_C.shift, 0.2 * DOWN + 0.15 * RIGHT, out_of_C.shift, SMALL_BUFF * (UP + LEFT), in_C_out_B.shift, 0.3 * DOWN)
        self.play(self.in_A_in_B_out_C.set_fill, GREEN, 0.5, Write(words, run_time=1), ShowCreation(arrow), Animation(out_of_C))
        self.play(ApplyMethod(VGroup(self.in_A_in_B_out_C, out_of_C).shift, MED_LARGE_BUFF * UP, rate_func=wiggle))
        self.wait()
        C.save_state()
    ...
    def show_two_relevant_subsets(self):
        A, B, C = self.venn_diagram
        all_out_of_C = VGroup(*it.chain(self.out_of_B[6:], self.out_of_C))
        ...
        self.play(*[ApplyMethod(m.scale, 0.7, method_kwargs={'about_point': FRAME_Y_RADIUS * DOWN}) for m in everything])
        terms = VGroup(OldTex('N(', 'A', '\\checkmark', ',', 'C', ')', '\\le'), OldTex('N(', 'A', '\\checkmark', ',', 'B', '\\checkmark', ',', 'C', ')'), OldTex('+\\, N(', 'A', '\\checkmark', ',', 'B', ')'))
        terms.arrange(RIGHT)
        terms.to_edge(UP)
        ...
        cross.set_color('#ff0000')
        cross.set_stroke(width=8)
        tweaser_group = VGroup(self.in_A_in_B_out_C.copy(), self.in_A_out_B.copy())
        tweaser_group.set_fill(TEAL, 1)
        tweaser_group.set_stroke(TEAL, 5)
        faders = VGroup(B, B.label, self.out_of_B_words, self.out_of_C_words, self.out_of_B_arrow, self.out_of_C_arrow, *regions[1:])
        faders.save_state()
        ...
        for term, group, region, num in zip(terms, photon_groups, regions, nums)[1:]:
            ...
            self.wait()
            self.play(ApplyMethod(VGroup(region, group).shift, 0.5 * UP, rate_func=wiggle, run_time=1.5))
        ...
        self.play(self.in_A_out_B.set_fill, YELLOW, 0.5, self.in_A_in_B_out_C.set_fill, YELLOW, 0.5, Animation(self.photons))
        self.wait()
        self.play(FadeOut(rects[0]), self.in_A_out_B.restore, self.in_A_in_B_out_C.restore, Animation(self.in_A_out_C), Animation(self.photons))
        self.wait()
    ...
    def photon_thinks_in_A_out_C(self):
        ...
        bubble.fade()
        self.play(MoveToTarget(photon), bubble.restore)
        self.play(photon.eyes.blink_anim())
        self.play(photon.restore, FadeOut(bubble))

    def setup_venn_diagram_sections(self):
        in_A_out_B, in_A_in_B_out_C, in_A_out_C, in_A_in_B = segments = VGroup(*[SVGMobject(file_name='VennDiagram_' + s, stroke_width=0, fill_opacity=0.5, fill_color=YELLOW) for s in ('in_A_out_B', 'in_A_in_B_out_C', 'in_A_out_C', 'in_A_in_B')])
...
class ReEmphasizeVennDiagram(VennDiagramProofByContradiction):
    ...
    def rescale_diagram(self):
        ...
        self.play(group.scale, 0.7, group.to_edge, DOWN, MED_SMALL_BUFF)
        self.clear()
    ...
    def setup_faded_circles(self):
        ...
        self.filled_circles = VGroup(*[circ.copy().set_stroke(width=0).set_fill(circ.get_color(), 1) for circ in self.circles])
    ...
    def shift_B_circle(self):
        ...
        self.play(B0.shift, vect, B.shift, vect, B.label.shift, vect, run_time=2, rate_func=running_start)
        B1.shift(vect)
        self.wait()
    ...
    def shift_C_circle(self):
        ...
        self.play(FadeOut(A1), FadeOut(B0), self.in_A_out_B_words.fade, 1, self.in_A_out_B_arrow.fade, 1, FadeIn(B1), FadeIn(words, lag_ratio=0.5), ShowCreation(arrow))
        self.play(FadeIn(C0))
        self.wait(2)
        vect = 0.5 * (B.get_center() - C.get_center())
        self.play(C0.shift, vect, C.shift, vect, C.label.shift, vect, run_time=2, rate_func=running_start)
        C1.shift(vect)
        self.wait()
        for mob in (words, arrow):
            mob.save_state()
    ...
    def show_A_not_C_region(self):
        ...
        self.play(FadeOut(B1), FadeOut(C0), self.in_B_out_C_words.fade, 1, self.in_B_out_C_arrow.fade, 1, FadeIn(A1_yellow_copy))
        self.play(FadeIn(C0))
        self.wait()
    ...
    def shorten_labels(self):
        ...
        self.play(A.pre_label.fade, 1, A.letter.scale, 2, A.letter.move_to, A.pre_label, LEFT, B.pre_label.fade, 1, B.letter.scale, 2, B.letter.get_right(), C.pre_label.fade, 1, C.letter.scale, 2, C.letter.move_to, C.pre_label, LEFT, C.letter.shift, DOWN + 0.5 * LEFT)
        for circle in (A, B, C):
    ...
    def show_inequality_with_circle(self):
        ...
        inequality = VGroup(OldTex('N(', 'A', '\\checkmark', ',', 'C', ')'), OldTex('N(', 'B', '\\checkmark', ',', 'C', ')'), OldTex('N(', 'A', '\\checkmark', ',', 'B', ')'))
        ...
        groups = VGroup(*[VGroup(m2.copy(), m1.copy(), VGroup(*self.circles).copy()) for m1, m2 in [(C0, A1_yellow_copy), (C0, B1), (B0, A1)]])
        ...
        self.clear()
        self.play(groups[0].scale, 0.5, groups[0].shift, 5 * LEFT + UP, Write(inequality[0], run_time=1), FadeIn(big_le))
        self.wait()
        self.play(FadeIn(groups[1]))
        self.play(groups[1].restore, FadeIn(inequality[1]), FadeIn(self.in_B_out_C_words), FadeIn(big_plus))
        self.play(FadeIn(groups[2]))
        self.play(groups[2].restore, FadeIn(inequality[2]), FadeIn(self.in_A_out_B_words))
        self.wait(2)
        self.groups = groups
    ...
    def emphasize_containment(self):
        ...
        self.play(ApplyMethod(c1.shift, 4 * LEFT, path_arc=-np.pi / 2), Animation(foreground))
        self.play(ApplyMethod(c2.shift, 8 * LEFT, path_arc=-np.pi / 2), Animation(c1), Animation(foreground), run_time=1.5)
    ...
    def adjust_circles(self):
        groups = self.groups
        A_group = VGroup(groups[0][0], groups[2][0], groups[0][2][0], groups[1][2][0], groups[2][2][0])
        B_group = VGroup(groups[1][0], groups[2][1], groups[0][2][1], groups[1][2][1], groups[2][2][1])
        C_group = VGroup(groups[0][1], groups[1][1], groups[0][2][2], groups[1][2][2], groups[2][2][2])
...
class NoFirstMeasurementPreferenceBasedOnDirection(ShowVariousFilterPairs):
    CONFIG = {'filter_x_coordinates': [0, 0, 0], 'pol_filter_configs': [{'filter_angle': angle} for angle in (0, np.pi / 8, np.pi / 4)], 'lines_depth': 1.2, 'lines_shift_vect': SMALL_BUFF * OUT, 'n_lines': 30}
    ...
    def reposition_camera(self):
        ...
        words.rotate(np.pi / 2, RIGHT)
        self.move_camera(theta=-0.6 * np.pi, added_anims=list(it.chain(*[[pf.arrow_label.rotate, np.pi / 2, OUT, pf.arrow_label.next_to, pf.arrow, OUT + RIGHT, SMALL_BUFF] for pf in self.pol_filters] + [[FadeIn(words)]])))
    ...
    def show_lines(self):
        ...
        self.play(ShowCreation(all_post_lines, **kwargs), Animation(self.pol_filters), Animation(all_pre_lines))
