...
class SimpleVelocityGraph(GraphScene):
    CONFIG = {'data_files': ['data07.txt', 'data13.txt'], 'colors': [WHITE, BLUE, YELLOW, GREEN, MAROON_B], 'x_axis_label': '$t$', 'y_axis_label': '$v$', 'x_min': 0, 'x_max': 1.8, 'x_tick_frequency': 0.1, 'x_labeled_nums': np.arange(0, 1.8, 0.2), 'y_tick_frequency': 100, 'y_min': 0, 'y_max': 1000, 'y_labeled_nums': list(range(0, 1000, 200)), 'x_axis_width': 12, 'graph_origin': 2.5 * DOWN + 5 * LEFT, 'trailing_average_length': 20, 'include_domino_thickness': False}
    ...
    def construct(self):
        ...
        for data_file, color in zip(self.data_files, self.colors):
            ...
            self.add_label_to_last_dot('%s %s %.2f' % (data_file[4:6], 'hard' if self.friction == 'low' else 'felt', self.domino_spacing), color)
    ...
    def init_data(self, data_file):
        file_name = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'dominos', data_file)
        ...
        self.velocities = np.array([np.mean(self.velocities[max(0, i - n):i]) for i in range(len(self.velocities))])
...
class ContrastTwoGraphs(SimpleVelocityGraph):
    ...
    def construct(self):
        ...
        for g, sg in ((graph1, smoothed_graph1), (graph2, smoothed_graph2)):
            ...
            for m, t in zip(mover, target):
                m.target = t
            self.play(LaggedStartMap(MoveToTarget, mover, rate_func=lambda t: 0.3 * wiggle(t), run_time=3, lag_ratio=0.2))
        ...
        self.play(graph1.scale, 2, graph1.move_to, lower_left, DOWN + LEFT, graph2.scale, 2, graph2.move_to, lower_left, DOWN + LEFT, FadeOut(words))
        self.wait()
        return
        self.play(FadeOut(graph2))
        dots = graph1.dots
        dot1, dot2 = (dots[21], dots[41])
        rect = Rectangle(width=dot2.get_center()[0] - dot1.get_center()[0], height=FRAME_Y_RADIUS - self.x_axis.get_center()[1], stroke_width=0, fill_color=YELLOW, fill_opacity=0.5)
        ...
        self.play(dot2.shift, MED_SMALL_BUFF * UP, dot2.set_color, PINK, rate_func=wiggle)
        self.wait()
        self.play(FadeOut(arrow), ReplacementTransform(pre_rect, rect))
        self.wait()
        ...
        self.play(LaggedStartMap(FadeIn, felt), LaggedStartMap(FadeIn, hardwood), run_time=1)
        self.wait()
        ...
        for x in range(2):
            self.play(LaggedStartMap(ApplyMethod, dot_group1, lambda m: (m.scale, 0.5), rate_func=wiggle, lag_ratio=0.05, run_time=3))
        self.wait()
        self.play(ReplacementTransform(rect1, rect2))
        for x in range(2):
            self.play(LaggedStartMap(ApplyMethod, dot_group2, lambda m: (m.scale, 0.5), rate_func=wiggle, lag_ratio=0.05, run_time=3))
        self.wait()
        self.play(ReplacementTransform(rect1, rect3))
        for x in range(2):
            self.play(LaggedStartMap(ApplyMethod, dot_group3, lambda m: (m.scale, 0.5), rate_func=wiggle, lag_ratio=0.05, run_time=3))
        self.wait()
    ...
    def get_smoothed_velocity_graph(self, velocities):
        ...
        smoothed_vs = np.array([np.mean(velocities[max(0, i - n):i]) for i in range(len(velocities))])
    ...
    def get_velocity_graph(self, velocities):
        ...
        for x in range(1, n):
            ...
            lines.add(Line(dots[-2].get_center(), dots[-1].get_center()))
...
class ShowAllSteadyStateVelocities(SimpleVelocityGraph):
    CONFIG = {'x_axis_label': '\\text{Domino spacing}', 'x_min': 0, 'x_max': 40, 'x_axis_width': 9, 'x_tick_frequency': 5, 'x_labeled_nums': list(range(0, 50, 10)), 'y_min': 0, 'y_max': 400, 'y_labeled_nums': []}
    ...
    def construct(self):
        ...
        for index in range(1, 20):
            ...
            dot = Dot(color=color)
            dot.scale(0.5)
            dot.move_to(self.coords_to_point(self.domino_spacing, self.velocities[-1] - 400))
...
class Test(Scene):
    ...
    def construct(self):
        shift_val = 1.5
        domino1 = Rectangle(width=0.5, height=3, stroke_width=0, fill_color=GREEN, fill_opacity=1)
        domino1.shift(LEFT)
        ...
        for domino in dominos:
            ...
            domino.add(line, dot)
        arc1 = Arc(radius=domino1.get_height(), start_angle=np.pi / 2, angle=-np.arcsin(spacing / domino1.get_height()))
        arc1.shift(domino1.get_corner(DOWN + RIGHT))
        arc2 = Arc(radius=domino1.get_height() / 2, start_angle=np.pi / 2, angle=-np.arcsin(2 * spacing / domino1.get_height()))
        ...
        for arc, vect in zip(arcs, [DOWN + RIGHT, RIGHT]):
            ...
            arc_copy.set_fill(arc.get_stroke_color(), 0.2)
            self.add(arc_copy)
        domino1_ghost = domino1.copy()
        domino1_ghost.fade(0.8)
        self.add(domino1_ghost, dominos, arcs)
        self.play(Rotate(domino1, angle=arc1.angle, about_point=domino1.get_corner(DOWN + RIGHT), rate_func=there_and_back, run_time=3))
        self.play(Rotate(domino1, angle=arc2.angle, about_point=domino1.get_right(), rate_func=there_and_back, run_time=3))
