...
def get_composite_rotation_angle_and_axis(angles, axes):
    ...
    for angle2, axis2 in zip(angles, axes):
        ...
        axis3 = np.sin(angle2 / 2) * np.cos(angle1 / 2) * axis2 + np.cos(angle2 / 2) * np.sin(angle1 / 2) * axis1 + np.sin(angle2 / 2) * np.sin(angle1 / 2) * cross
...
class ConfettiSpiril(Animation):
    CONFIG = {'x_start': 0, 'spiril_radius': 0.5, 'num_spirils': 4, 'run_time': 10, 'rate_func': None}
...
def get_confetti_animations(num_confetti_squares):
    ...
    confetti_squares = [Square(side_length=0.2, stroke_width=0, fill_opacity=0.75, fill_color=random.choice(colors)) for x in range(num_confetti_squares)]
    confetti_spirils = [ConfettiSpiril(square, x_start=2 * random.random() * FRAME_X_RADIUS - FRAME_X_RADIUS, rate_func=squish_rate_func(rush_from, a, a + 0.5)) for a, square in zip(np.linspace(0, 0.5, num_confetti_squares), confetti_squares)]
    return confetti_spirils

class Anniversary(TeacherStudentsScene):
    CONFIG = {'num_confetti_squares': 50, 'message': '2 year Anniversary!'}
    ...
    def celebrate(self):
        ...
        title.scale(1.5)
        title.to_edge(UP)
        first_video = Rectangle(height=2, width=2 * (16.0 / 9), stroke_color=WHITE, fill_color='#111111', fill_opacity=0.75)
    ...
    def get_party_hats(self):
        ...
        max_angle = np.pi / 6
        for hat in hats:
            hat.rotate(random.random() * 2 * max_angle - max_angle, about_point=hat.get_bottom())
        return hats
...
class JustGiveMeAQuickExplanation(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says("I ain't got \\\\ time for this!", target_mode='angry')
        self.play(*it.chain(*[[pi.change_mode, 'hesitant', pi.look_at, self.get_students()[1].eyes] for pi in self.get_students()[::2]]))
        self.wait(2)
...
class QuickExplanation(ComplexTransformationScene):
    CONFIG = {'plane_config': {'x_line_frequency': 1, 'y_line_frequency': 1, 'secondary_line_ratio': 1, 'space_unit_to_x_unit': 1.5, 'space_unit_to_y_unit': 1.5}, 'background_fade_factor': 0.2, 'background_label_scale_val': 0.7, 'velocity_color': RED, 'position_color': YELLOW}
    ...
    def add_equation(self):
        ...
        equation.add(brace)
        brace_text = OldTexText('Velocity vector', 'is a', '$90^\\circ$ \\\\ rotation', 'of', 'position vector')
    ...
    def add_vectors(self):
        ...
        kwargs = {'rate_func': None, 'run_time': 5}
...
class SymmetriesOfSquare(ThreeDScene):
    CONFIG = {'square_config': {'side_length': 2, 'stroke_width': 0, 'fill_color': BLUE, 'fill_opacity': 0.75}, 'dashed_line_config': {}}
    ...
    def confused_by_lack_of_labels(self):
        ...
        for axis in (OUT, RIGHT, UP):
            self.rotate_square(angle=np.pi, axis=axis, added_anims=[randy.look_at, self.square.get_points()[0]])
    ...
    def add_labels(self):
        ...
        self.play(FadeIn(self.square.randy), self.randy.change_mode, 'happy', self.randy.look_at, self.square.randy.eyes)
    ...
    def show_top_actions(self):
        ...
        self.wait()
        self.play(*[Rotate(square, rate_func=lambda t: -there_and_back(t), run_time=3, about_point=square.get_center(), **square.rotation_kwargs) for square in all_squares[1:4]])
        self.wait()

    def show_bottom_actions(self):
        for square in self.all_squares[4:]:
            self.rotate_square(square=square, rate_func=there_and_back, run_time=2, **square.rotation_kwargs)
        self.wait()
    ...
    def get_rotation_arcs(self, square, angle, angle_buff=SMALL_BUFF):
        ...
        arc = Arc(radius=square_radius + SMALL_BUFF, start_angle=np.pi / 4 + np.sign(angle) * angle_buff, angle=angle - np.sign(angle) * 2 * angle_buff, color=YELLOW)
        arc.add_tip()
...
class CircleSymmetries(Scene):
    ...
    def show_range_of_angles(self):
        ...
        for rate_func in (smooth, lambda t: smooth(1 - t)):
            self.play(Rotate(self.circle, 2 * np.pi - 0.001), ShowCreation(arc_circle), UpdateFromAlphaFunc(theta_value, theta_value_update), run_time=7, rate_func=rate_func)
            self.wait()
    ...
    def associate_rotations_with_points(self):
        ...
        for alpha in (0.2, 0.6, 0.4, 0.8):
            ...
            self.wait()
            self.play(Rotate(self.circle, -angle, run_time=2), FadeOut(dot), FadeOut(arrow))
            self.wait()
...
class GroupOfCubeSymmetries(ThreeDScene):
    CONFIG = {'cube_opacity': 0.5, 'cube_colors': [BLUE], 'put_randy_on_cube': True}
...
class HowDoSymmetriesPlayWithEachOther(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says('How do symmetries \\\\ play with each other?', target_mode='hesitant')
...
class AddSquareSymmetries(SymmetriesOfSquare):
    ...
    def construct(self):
        ...
        equation = VGroup(equation_square, OldTex('+'), equation_square.copy(), OldTex('='), equation_square.copy())
        equation[0].add(self.get_rotation_arcs(equation[0], np.pi / 2))
        ...
        self.flip_square(square=alt_square, axis=UP + RIGHT, added_anims=list(map(FadeIn, equation[4:])))
        self.wait(2)
...
class AddCircleSymmetries(CircleSymmetries):
    ...
    def construct(self):
        ...
        equation = OldTex('270^\\circ', '+', '120^\\circ', '=', '30^\\circ')
        equation.to_edge(UP)
        ...
        def rotate(circle, angle, arc, terms):
            self.play(Rotate(circle, angle, in_place=True), ShowCreation(arc, rate_func=lambda t: angle / (2 * np.pi) * smooth(t)), Write(VGroup(*terms)), run_time=2)
...
class AddCubeSymmetries(GroupOfCubeSymmetries):
    CONFIG = {'angle_axis_pairs': [(np.pi / 2, RIGHT), (np.pi / 2, UP)], 'cube_opacity': 0.5, 'cube_colors': [BLUE]}
    ...
    def construct(self):
        ...
        self.play(cube_copy.move_to, cube2, cube_copy.set_fill, None, self.cube_opacity, Write(plus))
        ...
        for cube, (angle, axis) in zip([cube1, cube_copy, cube3], angle_axis_pairs):
            self.rotate_cube(cube, -angle, axis, add_arrows=False, rate_func=there_and_back, run_time=1.5)
        self.wait()
    ...
    def rotate_cube(self, cube, angle, axis, add_arrows=True, **kwargs):
        ...
        anims = []
        if add_arrows:
            arrows = VGroup(*[Arc(start_angle=np.pi / 12 + a, angle=5 * np.pi / 6, color=YELLOW).add_tip() for a in (0, np.pi)])
...
class DihedralGroupStructure(SymmetriesOfSquare):
    CONFIG = {'dashed_line_config': {'dash_length': 0.1}, 'filed_sum_scale_factor': 0.4, 'num_rows': 5}

    def construct(self):
        angle_axis_pairs = [(np.pi / 2, OUT), (np.pi, OUT), (-np.pi / 2, OUT), (np.pi, UP), (np.pi, UP + LEFT)]
    ...
    def demonstrate_sum(self, angle_axis_pairs):
        angle_axis_pairs = list(angle_axis_pairs) + [get_composite_rotation_angle_and_axis(*list(zip(*angle_axis_pairs)))]
        ...
        expression = s1, plus, s2, equals, s3 = VGroup(prototype_square, OldTex('+').scale(2), prototype_square.copy(), OldTex('=').scale(2), prototype_square.copy())
        final_expression = VGroup()
        for square, (angle, axis) in zip([s1, s2, s3], angle_axis_pairs):
            if np.cos(angle) > 0.5:
                square.action_illustration = VectorizedPoint()
            elif np.argmax(np.abs(axis)) == 2:
            ...
            square.rotation_kwargs = {'square': square, 'angle': angle, 'axis': axis}
        expression.arrange()
        ...
        s1_copy = s1.copy()
        self.play(s1_copy.move_to, s2, Write(plus))
...
class ThisIsAVeryGeneralIdea(Scene):
    ...
    def construct(self):
        ...
        examples = VGroup(*list(map(TexText, ['Square matrices \\\\ \\small (Where $\\det(M) \\ne 0$)', 'Molecular \\\\ symmetry', 'Cryptography', 'Numbers'])))
        numbers = examples[-1]
        ...
        self.wait()
        self.play(VGroup(*examples[:-1]).fade, 0.7, VGroup(*lines[:-1]).fade, 0.7)
        self.play(numbers.scale, 1.2, numbers.get_corner(UP + RIGHT))
        self.wait(2)
        sub_categories = VGroup(*list(map(TexText, ['Numbers \\\\ (Additive)', 'Numbers \\\\ (Multiplicative)'])))
...
class NumbersAsActionsQ(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('Numbers are actions?', target_mode='confused')
        self.play_student_changes('pondering', 'confused', 'erm')
        self.play(self.get_teacher().change_mode, 'happy')
        self.wait(3)

class AdditiveGroupOfReals(Scene):
    CONFIG = {'number_line_center': UP, 'shadow_line_center': DOWN, 'zero_color': GREEN_B, 'x_min': -FRAME_WIDTH, 'x_max': FRAME_WIDTH}
    ...
    def add_number_line(self):
        number_line = NumberLine(x_min=self.x_min, x_max=self.x_max)
    ...
    def show_example_slides_labeled(self, *nums, **kwargs):
        for num in nums:
            line = DashedLine(self.number_line.number_to_point(num) + MED_LARGE_BUFF * UP, self.shadow_line.number_to_point(num) + MED_LARGE_BUFF * DOWN)
    ...
    def comment_on_zero_as_identity(self):
        line = DashedLine(self.number_line.number_to_point(0) + MED_LARGE_BUFF * UP, self.shadow_line.number_to_point(0) + MED_LARGE_BUFF * DOWN)
...
class AdditiveGroupOfComplexNumbers(ComplexTransformationScene):
    CONFIG = {'x_min': -2 * int(FRAME_X_RADIUS), 'x_max': 2 * int(FRAME_X_RADIUS), 'y_min': -FRAME_HEIGHT, 'y_max': FRAME_HEIGHT, 'example_points': [complex(3, 2), complex(1, -3)]}
    ...
    def show_vertical_slide(self):
        ...
        self.play(self.plane.shift, 6 * DOWN, run_time=3)
        self.wait()
    ...
    def show_example_addition(self):
        ...
        self.wait()
        real_arrow, imag_arrow = component_arrows = [Vector(self.z_to_point(z), color=ADDER_COLOR) for z in [z1.real + z2.real, complex(0, z1.imag + z2.imag)]]
        imag_arrow.shift(real_arrow.get_end())
        plus = OldTex('+').next_to(real_arrow.get_center(), UP + RIGHT)
        plus.add_background_rectangle()
        rp1, rp2, ip1, ip2 = label_parts = [VGroup(label1[1][0].copy()), VGroup(label2[1][0].copy()), VGroup(*label1[1][2:]).copy(), VGroup(*label2[1][1:]).copy()]
        ...
        self.play(ShowCreation(real_arrow), ShowCreation(real_background_rect, rate_func=squish_rate_func(smooth, 0.75, 1)), Write(plus), *list(map(MoveToTarget, [rp1, rp2])))
        self.wait()
        self.play(ShowCreation(imag_arrow), ShowCreation(imag_background_rect, rate_func=squish_rate_func(smooth, 0.75, 1)), *list(map(MoveToTarget, [ip1, ip2])))
        self.wait(2)
        to_remove = [arrow1, dot1, label1, arrow2, dot2, label2, real_background_rect, imag_background_rect, plus] + label_parts + component_arrows
    ...
    def show_some_random_slides(self):
        example_slides = [complex(3), complex(0, 2), complex(-4, -1), complex(-2, -1), complex(4, 2)]
...
class SchizophrenicNumbers(Scene):
    ...
    def construct(self):
        ...
        numbers = VGroup(Randolph(mode='happy').scale(0.2), OldTex('3').shift(UP + LEFT), OldTex('5.83').shift(UP + RIGHT), OldTex('\\sqrt{2}').shift(DOWN + LEFT), OldTex('2-i').shift(DOWN + RIGHT))
        ...
        self.play(*[MoveToTarget(number, rate_func=squish_rate_func(smooth, alpha, alpha + 0.5), run_time=2) for number, alpha in zip(numbers, np.linspace(0, 0.5, len(numbers)))])
        self.wait()
        self.blink_numbers(numbers)
        self.wait()

    def blink_numbers(self, numbers):
        self.play(*[num.eyes.blink_anim(rate_func=squish_rate_func(there_and_back, alpha, alpha + 0.2)) for num, alpha in zip(numbers[1:], 0.8 * np.random.random(len(numbers)))])

class MultiplicativeGroupOfReals(AdditiveGroupOfReals):
    CONFIG = {'number_line_center': 0.5 * UP, 'shadow_line_center': 1.5 * DOWN, 'x_min': -3 * FRAME_X_RADIUS, 'x_max': 3 * FRAME_X_RADIUS, 'positive_reals_color': MAROON_B}
    ...
    def follow_one(self):
        ...
        three_line, half_line = [DashedLine(self.number_line.number_to_point(num), self.shadow_line.number_to_point(num)) for num in (3, 0.5)]
        three_mob = [m for m in self.shadow_line.numbers if m.get_tex() == '3'][0]
        half_point = self.shadow_line.number_to_point(0.5)
        half_arrow = Arrow(half_point + UP + LEFT, half_point, buff=SMALL_BUFF, tip_length=0.15)
        ...
        self.wait()
        self.play(self.number_line.restore, *list(map(FadeOut, [half_label, half_arrow, half_line, dot_copy])))
        self.wait()
        self.one_dot = dot

    def every_positive_number_association(self):
        positive_reals_line = Line(self.shadow_line.number_to_point(0), self.shadow_line.number_to_point(FRAME_X_RADIUS), color=self.positive_reals_color)
        ...
        third_line, one_line = [DashedLine(self.number_line.number_to_point(num), self.shadow_line.number_to_point(num)) for num in (0.33, 1)]
        ...
        self.play(ShowCreation(positive_reals_line), Write(positive_reals_words))
        self.wait()
        self.play(ReplacementTransform(third_line, one_line), self.number_line.restore, Animation(positive_reals_words), run_time=2)
        self.number_line.add_to_back(one_line)
        self.number_line.save_state()
        self.stretch(7, run_time=10, rate_func=there_and_back, added_anims=[Animation(positive_reals_words)])
        self.wait()

    def compose_actions(self, num1, num2):
        words = VGroup(*[OldTexText('(%s by %s)' % (word, str(num))) for num in (num1, num2, num1 * num2) for word in ['Stretch' if num > 1 else 'Squish']])
        ...
        self.wait(2)
        self.play(ApplyMethod(self.number_line.restore, run_time=2), FadeOut(words))
        self.wait()
    ...
    def stretch(self, factor, run_time=2, **kwargs):
        ...
        self.play(Transform(self.number_line, target, **kwargs), *kwargs.get('added_anims', []))
...
class MultiplicativeGroupOfComplexNumbers(AdditiveGroupOfComplexNumbers):
    CONFIG = {'dot_radius': Dot.CONFIG['radius'], 'y_min': -3 * FRAME_Y_RADIUS, 'y_max': 3 * FRAME_Y_RADIUS}
    ...
    def add_plane(self):
        ...
        one_dot = Dot(self.z_to_point(1), color=MULTIPLIER_COLOR, radius=self.dot_radius)
    ...
    def fix_zero_and_move_one(self):
        ...
        one_arrow = Arrow(one_point + UP + 1.25 * RIGHT, one_point, buff=2 * self.dot_radius, color=MULTIPLIER_COLOR)
        ...
        self.play(Write(zero_words, run_time=2), ShowCreation(zero_arrow), Indicate(self.plane.zero_dot, color=RED))
        self.play(Write(one_words, run_time=2), ShowCreation(one_arrow), Indicate(self.plane.one_dot, color=RED))
        self.wait(2)
        self.play(*list(map(FadeOut, [zero_words, zero_arrow, one_words, one_arrow])))

    def show_example_actions(self):
        z_list = [complex(2), complex(0.5), complex(2, 1), complex(-2, 2)]
    ...
    def show_action_at_i(self):
        ...
        arc = Arc(start_angle=np.pi / 24, angle=10 * np.pi / 24, radius=self.z_to_point(1)[0], num_anchors=20)
    ...
    def show_action_at_i_again(self):
        ...
        half_turn_arc = Arc(start_angle=np.pi / 12, angle=10 * np.pi / 12, color=self.turn_arrow.get_color())
    ...
    def show_i_squared_is_negative_one(self):
        ...
        self.wait()
        for term in terms[:2]:
            self.multiply_by_z(complex(0, 1), added_anims=[Animation(equation), Indicate(term, color=RED, run_time=2)])
            self.wait()
    ...
    def talk_through_specific_example(self):
        ...
        self.wait()
        self.reset_plane()
        self.multiply_by_z(np.exp(complex(0, 1) * angle), added_anims=[ShowCreation(arc, run_time=2), Write(angle_label)])
        ...
        self.wait(2)
        to_remove = [label, dot, brace, brace_text, arc, angle_label]
        for mob in to_remove:
            self.foreground_mobjects.remove(mob)
    ...
    def example_actions_broken_down(self):
        z_list = [complex(2, -1), complex(-2, -3), complex(0.5, 0.5)]
        for z in z_list:
    ...
    def multiply_by_z(self, z, run_time=2, **kwargs):
        ...
        self.play(Transform(self.plane, target, run_time=run_time, **kwargs), *kwargs.get('added_anims', []))
...
class ExponentsAsRepeatedMultiplication(TeacherStudentsScene):
    ...
    def show_repeated_multiplication(self):
        ...
        self.play(GrowFromCenter(three_twos.brace), Write(three_twos.exp), self.get_teacher().change_mode, 'raise_right_hand')
        for mob in three_twos:
            self.play(Write(mob, run_time=1))
        self.play_student_changes(*['pondering'] * 3)
        self.wait(2)
        self.play(FadeIn(five_twos.brace), FadeIn(five_twos.exp), three_twos.center, three_twos.to_edge, UP, 2 * LARGE_BUFF, three_twos.brace_anim)
        ...
        self.play(MoveToTarget(three_twos), three_twos.brace_anim, MoveToTarget(five_twos), five_twos.brace_anim, Write(cdot), self.get_teacher().change_mode, 'happy')
        self.wait()
        self.play(Write(lhs))
        self.wait()
        ...
        self.play(*[ReplacementTransform(mob.copy(), term, run_time=2) for term, mob in zip(general_equation, [lhs, three_twos.exp, five_twos.exp])])
        self.wait(2)
        self.exponential_rule = general_equation
        self.expanded_exponential_rule = VGroup(lhs, three_twos, three_twos.brace, three_twos.exp, cdot, five_twos, five_twos.brace, five_twos.exp)

    def show_non_counting_exponents(self):
        self.play(self.expanded_exponential_rule.scale, 0.5, self.expanded_exponential_rule.to_corner, UP + LEFT)
        half_power, neg_power, imag_power = alt_powers = VGroup(OldTex('2^{1/2}'), OldTex('2^{-1}'), OldTex('2^{i}'))
        alt_powers.arrange(RIGHT, buff=LARGE_BUFF)
        alt_powers.next_to(self.get_students(), UP, buff=LARGE_BUFF)
        self.play(Write(half_power, run_time=2), *[ApplyMethod(pi.change_mode, 'pondering') for pi in self.get_pi_creatures()])
        ...
        self.play(Transform(half_power, half_expression[1]), Write(half_expression), RemovePiCreatureBubble(self.get_teacher()))
        self.wait()
        self.play(Transform(neg_power, neg_one_expression[1]), Write(neg_one_expression))
        self.wait(2)
        self.play(self.exponential_rule.next_to, self.get_teacher().get_corner(UP + LEFT), UP, MED_LARGE_BUFF, self.get_teacher().change_mode, 'raise_right_hand')
        self.wait(2)
        self.play(imag_power.move_to, UP, imag_power.scale, 1.5, imag_power.set_color, BLUE, self.exponential_rule.to_edge, RIGHT, self.get_teacher().change_mode, 'speaking')
...
class ExponentsAsHomomorphism(Scene):
    CONFIG = {'top_line_center': 2.5 * UP, 'top_line_config': {'x_min': -16, 'x_max': 16}, 'bottom_line_center': 2.5 * DOWN, 'bottom_line_config': {'x_min': -FRAME_WIDTH, 'x_max': FRAME_WIDTH}}
    ...
    def show_adders(self):
        ...
        self.play(adders.scale, 1.5, adders.center, adders.space_out_submobjects, 2, adders.to_edge, UP, adders.set_color, GREEN_B, FadeOut(self.lhs_brace_group), Write(top_line))
        self.wait()
        for x in (3, 5, -8):
    ...
    def show_multipliers(self):
        ...
        self.play(multipliers.space_out_submobjects, 4, multipliers.next_to, self.bottom_line_center, UP, MED_LARGE_BUFF, multipliers.set_color, YELLOW, FadeOut(self.rhs_brace_group), Write(bottom_line))
        stretch_kwargs = {}
    ...
    def confused_at_mapping(self):
        arrow = Arrow(self.top_line.get_bottom()[1] * UP, self.bottom_line.get_top()[1] * UP, color=WHITE)
        randy = Randolph(mode='confused')
        randy.scale(0.75)
        randy.flip()
        ...
        self.wait()
        for x in (1, -2, 3, 1, -3):
            self.play(self.get_stretch_anim(self.bottom_line, 2 ** x), self.top_line.shift, x * RIGHT, randy.look_at, self.top_line, run_time=2)
    ...
    def talk_through_composition(self):
        ...
        for mob, target_tex in zip(terms, target_texs):
            ...
            if mob in self.adders:
                target.to_edge(UP)
            mob.target = target
        self.play(self.equation.next_to, ORIGIN, LEFT, MED_LARGE_BUFF, randy.change_mode, 'pondering', randy.look_at, self.equation)
        self.wait()
        self.play(randy.look_at, self.top_line)
        self.show_composition(*inputs, parallel_anims=list(map(MoveToTarget, self.adders)))
        self.play(FocusOn(self.bottom_line_center), randy.look_at, self.bottom_line_center)
    ...
    def add_quote(self):
        ...
        self.play(GrowFromCenter(brace), Write(quote), self.randy.look_at, quote)
    ...
    def show_composition(self, *inputs, **kwargs):
        ...
        for x, parallel_anim in zip(inputs, parallel_anims):
            anims = [ApplyMethod(self.top_line.shift, x * RIGHT), self.get_stretch_anim(self.bottom_line, 2 ** x)]
            for anim in anims:
                anim.set_run_time(2)
    ...
    def get_stretch_anim(self, bottom_line, x):
        ...
        target.stretch_about_point(x, 0, self.bottom_line_center)
        for number in target.numbers:
...
class DihedralCubeHomomorphism(GroupOfCubeSymmetries, SymmetriesOfSquare):
    ...
    def construct(self):
        angle_axis_pairs = [(np.pi / 2, OUT), (np.pi, RIGHT), (np.pi, OUT), (np.pi, UP + RIGHT), (-np.pi / 2, OUT), (np.pi, UP + LEFT)]
        angle_axis_pairs *= 3
        ...
        for i, (angle, raw_axis) in enumerate(angle_axis_pairs):
            ...
            self.play(*[Rotate(mob, angle=angle, axis=axis, in_place=True, run_time=abs(angle / (np.pi / 2))) for mob, axis in [(square, raw_axis), (cube, posed_axis)]])
            self.wait()
            if i == 2:
...
class ComplexExponentiationAbstract:
    CONFIG = {'start_base': 2, 'new_base': 5, 'group_type': None, 'color': None, 'vect': None}
...
class ComplexExponentiationAdderHalf(ComplexExponentiationAbstract, AdditiveGroupOfComplexNumbers):
    CONFIG = {'group_type': 'Additive', 'color': GREEN_B, 'vect': LEFT}
    ...
    def apply_action(self, z, run_time=2, **kwargs):
        kwargs['run_time'] = run_time
        self.play(ApplyMethod(self.plane.shift, self.z_to_point(z), **kwargs), *kwargs.get('added_anims', []))

    def set_color_vertical_line(self):
        line = VGroup(Line(ORIGIN, FRAME_Y_RADIUS * UP), Line(ORIGIN, FRAME_Y_RADIUS * DOWN))
        line.set_color(YELLOW)
        self.play(FadeOut(self.real_line), *list(map(ShowCreation, line)))
        self.foreground_mobjects.remove(self.real_line)
        self.play(line.rotate, np.pi / 24, rate_func=wiggle)
        self.wait()
    ...
    def set_color_unit_circle(self):
        ...
        for submob in line:
            ...
            submob.make_smooth()
        circle = VGroup(Circle(), Circle().flip(RIGHT))
    ...
    def take_steps_for_e(self):
        ...
        labels = list(map(TexText, ['1 unit', '2 units', '3 units', '$\\pi$ units']))
        ...
        for slide_value, label, brace in zip(slide_values, labels, braces):
            ...
            if curr_brace is None:
                ...
                self.play(Transform(curr_brace, brace), Transform(curr_label, label))
            self.wait()
            self.wait(4)

class ComplexExponentiationMultiplierHalf(ComplexExponentiationAbstract, MultiplicativeGroupOfComplexNumbers):
    CONFIG = {'group_type': 'Multiplicative', 'color': MULTIPLIER_COLOR, 'vect': RIGHT}
    ...
    def take_steps_for_e(self):
        ...
        for last_angle, angle in zip([0] + angles, angles):
            ...
            if curr_brace is None:
                ...
                curr_arc = arc
                self.play(Transform(curr_brace, curved_brace), Transform(curr_label, label))
            self.wait()
            self.wait()
...
class ExpComplexHomomorphismPreviewMultiplierHalf(ExpComplexHomomorphismPreviewAbstract, ComplexExponentiationMultiplierHalf):
...
class EfvgtPatreonThanks(PatreonEndScreen):
    CONFIG = {'specific_patrons': ['Ali Yahya', 'Meshal  Alshammari', 'CrypticSwarm    ', 'Justin Helps', 'Ankit Agarwal', 'Yu  Jun', 'Shelby  Doolittle', 'Dave    Nicponski', 'Damion  Kistler', 'Juan    Benet', 'Othman  Alikhan', 'Markus  Persson', 'Dan Buchoff', 'Derek Dai', 'Joseph  John Cox', 'Luc Ritchie', 'Nils Schneider', 'Mathew Bramson', 'Guido   Gambardella', 'Jerry   Ling', 'Mark    Govea', 'Vecht', 'Shimin Kuang', 'Rish    Kundalia', 'Achille Brighton', 'Kirk    Werklund', 'Ripta   Pasay', 'Felipe  Diniz']}

class EmeraldLogo(SVGMobject):
    CONFIG = {'file_name': 'emerald_logo', 'stroke_width': 0, 'fill_opacity': 1, 'helix_color': GREEN_E}
...
class ExpTransformation(ComplexTransformationScene):
    ...
    def construct(self):
        ...
        cylinder = self.plane.copy().apply_function(lambda x_y_z: np.array([x_y_z[0], np.sin(x_y_z[1]), -np.cos(x_y_z[1])]))
        ...
        title.to_edge(UP, buff=MED_SMALL_BUFF)
        self.add_foreground_mobjects(title)
        self.play(Transform(self.plane, cylinder, run_time=3, path_arc_axis=RIGHT, path_arc=np.pi))
