...
class IntroduceTriples(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for a, b, c in triples:
            triangle = Polygon(ORIGIN, a * RIGHT, a * RIGHT + b * UP, stroke_width=0, fill_color=WHITE, fill_opacity=0.5)
            ...
            not_i_list = list(filter(lambda i: i not in i_list, list(range(c ** 2))))
            c_square_parts = [VGroup(*[c_square[i] for i in i_list]), VGroup(*[c_square[i] for i in not_i_list])]
            ...
            self.play_student_changes(*['pondering'] * 3, look_at=triangle, added_anims=[LaggedStartMap(FadeIn, b_square)])
...
class CompareToFermatsLastTheorem(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(ReplacementTransform(square_expression.copy(), low_expression), self.teacher.change_mode, 'raise_right_hand', *[ApplyMethod(pi.change, 'confused', expressions[1]) for pi in self.get_students()])
        self.wait()
        self.play(Transform(low_expression, expressions[2]))
        self.play(GrowFromCenter(low_brace), FadeIn(low_text))
        self.play_student_changes('sassy', 'angry', 'erm', look_at=low_expression, added_anims=[Transform(low_expression, expressions[3])])
...
class ShowManyTriples(Scene):
    ...
    def construct(self):
        triples = [(u ** 2 - v ** 2, 2 * u * v, u ** 2 + v ** 2) for u in range(1, 15) for v in range(1, u) if fractions.gcd(u, v) == 1 and (not u % 2 == v % 2)][:40]
        triangles = VGroup()
        titles = VGroup()
        for i, (a, b, c) in enumerate(triples):
            ...
            title.target.move_to((-FRAME_X_RADIUS + MED_LARGE_BUFF + 2.7 * (i // 8)) * RIGHT + (FRAME_Y_RADIUS - MED_LARGE_BUFF - i % 8) * UP, UP + LEFT)
            triangles.add(triangle)
            titles.add(title)
        triangle = triangles[0]
        title = titles[0]
        self.play(Write(triangle), Write(title), run_time=2)
        self.wait()
...
class BabylonianTablets(Scene):
    ...
    def construct(self):
        ...
        ac_pairs = [(119, 169), (3367, 4825), (4601, 6649), (12709, 18541), (65, 97), (319, 481), (2291, 3541), (799, 1249), (481, 769), (4961, 8161), (45, 75), (1679, 2929), (161, 289), (1771, 3229), (56, 106)]
        triples = VGroup()
        for a, c in ac_pairs:
...
class PythagoreanProof(Scene):
    ...
    def show_proof(self):
        triangle = Polygon(ORIGIN, 5 * RIGHT, 5 * RIGHT + 12 * UP, stroke_color=WHITE, stroke_width=2, fill_color=WHITE, fill_opacity=0.5)
        ...
        self.wait()
        self.play(VGroup(triangle_copy, a_square, b_square).move_to, 4 * LEFT + 2 * DOWN, DOWN, VGroup(triangle, c_square).move_to, 4 * RIGHT + 2 * DOWN, DOWN, run_time=2, path_arc=np.pi / 2)
        self.wait()
        self.add_new_triangles(triangle, self.get_added_triangles_to_c_square(triangle, c_square))
        self.wait()
        self.add_new_triangles(triangle_copy, self.get_added_triangles_to_ab_squares(triangle_copy, a_square))
        self.wait()
        ...
        self.play(FadeIn(big_squares), Write(negative_space_words), ShowCreation(double_arrow), *list(map(FadeOut, squares)))
        self.wait(2)
        self.play(*it.chain(list(map(FadeIn, squares)), list(map(Animation, big_squares))))
        self.wait(2)
    ...
    def add_new_triangles(self, triangle, added_triangles):
        ...
        self.play(ReplacementTransform(VGroup(triangle.copy().set_fill(opacity=0)), added_triangles, run_time=2))
    ...
    def get_abc_squares(self, triangle):
        a_square, b_square, c_square = squares = [Square(stroke_color=color, fill_color=color, fill_opacity=0.5) for color in SIDE_COLORS]
        a_square.set_width(triangle.get_width())
        a_square.move_to(triangle.get_bottom(), UP)
        b_square.set_height(triangle.get_height())
        b_square.move_to(triangle.get_right(), LEFT)
        hyp_line = Line(triangle.get_corner(UP + RIGHT), triangle.get_corner(DOWN + LEFT))
...
class ReframeOnLattice(PiCreatureScene):
    CONFIG = {'initial_plane_center': 3 * LEFT + DOWN, 'new_plane_center': ORIGIN, 'initial_unit_size': 0.5, 'new_unit_size': 0.8, 'dot_radius': 0.075, 'dot_color': YELLOW}
    ...
    def add_plane(self):
        plane = ComplexPlane(center_point=self.initial_plane_center, unit_size=self.initial_unit_size, stroke_width=2, secondary_line_ratio=0)
    ...
    def wander_over_lattice_points(self):
        ...
        for new_group in dot_tuple_groups[1:len(initial_examples)]:
            ...
            self.wait()
        self.play(LaggedStartMap(FadeIn, all_dots, rate_func=there_and_back, run_time=3, lag_ratio=0.2))
        self.wait()
    ...
    def show_whole_distance_examples(self):
        ...
        for dot_tuple_group in dot_tuple_groups:
            ...
            p1 = dot.get_center()
            triangle = Polygon(p0, p1[0] * RIGHT + p0[1] * UP, p1, stroke_width=0, fill_color=BLUE, fill_opacity=0.75)
        ...
        for new_group in dot_tuple_groups[1:]:
            self.play(Transform(group, new_group), Transform(group.triangle, new_group.triangle), Transform(group.line, new_group.line))
            self.wait(2)
        self.play(*list(map(FadeOut, [group, group.triangle, group.line])))

    def resize_plane(self):
        new_plane = ComplexPlane(plane_center=self.new_plane_center, unit_size=self.new_unit_size, y_radius=8, x_radius=11, stroke_width=2, secondary_line_ratio=0)
        ...
        self.play(MoveToTarget(self.plane), *list(map(MoveToTarget, self.plane.coordinate_labels)), run_time=2)
    ...
    def show_root_example(self):
        ...
        for tex in ('2^2 + 1^2', '5'):
            ...
            label = VGroup(rect, VGroup(*pre_label[:2]), VGroup(*pre_label[2:]))
            label.scale(0.8)
        ...
        self.play(ShowCreation(line), DrawBorderThenFill(dot, stroke_width=3, stroke_color=PINK))
    ...
    def view_as_complex_number(self):
        ...
        new_label.next_to(self.example_dot, DOWN + RIGHT, buff=0)
    ...
    def mention_squaring_it(self):
        ...
        arrow = Arrow(self.plane.coords_to_point(2, 1), self.plane.coords_to_point(3, 4), path_arc=np.pi / 3, color=MAROON_B)
        ...
        self.play(PiCreatureSays(morty, 'Try squaring \\\\ it!', target_mode='hooray', bubble_config={'width': 4, 'height': 3}))
    ...
    def work_out_square_algebraically(self):
        ...
        self.play(FadeIn(rect), ReplacementTransform(VGroup(self.example_label[1].copy()), top_line), run_time=2)
        self.wait()
        index_alignment_lists = [[(0, 1, 0), (1, 1, 1)], [(0, 2, 2), (0, 1, 3), (1, 3, 4)], [(0, 2, 5), (1, 1, 6), (0, 3, 7)], [(0, 2, 8), (0, 3, 9), (1, 3, 10)]]
        for index_alignment in index_alignment_lists:
            self.play(*[ReplacementTransform(top_line[i][j].copy(), second_line[k]) for i, j, k in index_alignment])
        self.wait(2)
        index_alignment_lists = [[(0, 0), (1, 0), (9, 0), (10, 0)], [(2, 1), (3, 2), (4, 3), (6, 2), (7, 3)]]
        for index_alignment in index_alignment_lists:
            self.play(*[ReplacementTransform(second_line[i].copy(), final_line[j], run_time=1.5) for i, j in index_alignment])
            self.wait()
        result_label = final_line.copy()
        result_label.add_background_rectangle()
        self.play(result_label.next_to, result_dot, UP + RIGHT, SMALL_BUFF, Animation(final_line), run_time=2)
    ...
    def walk_through_square_geometrically(self):
        ...
        double_arc = Arc(angle=2 * line.get_angle(), radius=0.8, color=RED)
        theta = OldTex('\\theta')
        ...
        method_args_list = [(self.plane.rotate, (line.get_angle(),)), (self.plane.scale, (np.sqrt(5),)), (self.plane.restore, ())]
        for method, args in method_args_list:
            self.play(Animation(ghost_plane), ApplyMethod(method, *args), Animation(everything), run_time=1.5)
            self.wait()
        ghost_arc = arc.copy().fade()
        ghost_line = line.copy().fade()
        ghots_dot = dot.copy().fade()
        self.add(ghost_arc, ghost_line, ghots_dot)
        self.play(VGroup(line, dot, distance_label).rotate, line.get_angle(), Transform(arc, double_arc), Transform(theta, two_theta))
        self.wait()
        ...
        self.play(FadeOut(distance_label), *list(map(MoveToTarget, [five, line, dot])), run_time=2)
        self.wait(2)

    def get_all_plane_dots(self):
        x_min, y_min = list(map(int, self.plane.point_to_coords(FRAME_X_RADIUS * LEFT + FRAME_Y_RADIUS * DOWN)))
        x_max, y_max = list(map(int, self.plane.point_to_coords(FRAME_X_RADIUS * RIGHT + FRAME_Y_RADIUS * UP)))
        result = VGroup(*[Dot(self.plane.coords_to_point(x, y), radius=self.dot_radius, color=self.dot_color) for x in range(int(x_min), int(x_max) + 1) for y in range(int(y_min), int(y_max) + 1)])
...
class OneMoreExample(Scene):
    CONFIG = {'unit_size': 0.5, 'plane_center': 3 * LEFT + 3 * DOWN, 'dot_color': YELLOW, 'x_label_range': list(range(-6, 25, 3)), 'y_label_range': list(range(3, 13, 3))}
    ...
    def add_plane(self):
        plane = ComplexPlane(unit_size=self.unit_size, center_point=self.plane_center, stroke_width=2)
        plane.axes.set_stroke(width=4)
    ...
    def add_point(self):
        ...
        for tex in ('3^2 + 2^2', '13'):
            pre_label = OldTex('\\sqrt{%s}' % tex)
            label = VGroup(BackgroundRectangle(pre_label), VGroup(*pre_label[:2]), VGroup(*pre_label[2:]))
            label.scale(0.75)
            label.next_to(line.get_center(), UP, SMALL_BUFF)
            label.rotate(line.get_angle(), about_point=line.get_center())
            distance_labels.add(label)
    ...
    def square_algebraically(self):
        rect = Rectangle(height=3.5, width=7, stroke_color=WHITE, stroke_width=2, fill_color=BLACK, fill_opacity=0.8)
        ...
        self.wait()
        index_alignment_lists = [[(0, 0, 0), (0, 1, 1), (1, 1, 2), (1, 5, 9)], [(0, 2, 3), (1, 3, 4), (0, 3, 5), (0, 4, 6), (1, 4, 7), (1, 3, 8)], [(0, 2, 10), (0, 0, 11), (0, 1, 12), (1, 3, 13), (1, 3, 14), (1, 5, 19), (0, 4, 20), (1, 4, 20)], [(0, 2, 15), (0, 3, 16), (1, 1, 17), (1, 1, 18)]]
        for index_alignment in index_alignment_lists[:2]:
            self.play(*[ReplacementTransform(top_line[i][j].copy(), second_line[k], run_time=1.5) for i, j, k in index_alignment])
            self.wait()
        self.play(Transform(second_line[3], minus), FadeOut(VGroup(*[second_line[i] for i in (4, 6, 7)])), second_line[5].shift, 0.35 * RIGHT)
    ...
    def plot_result(self):
        ...
        self.play(result_label.next_to, dot, UP + LEFT, SMALL_BUFF, Animation(self.final_line), DrawBorderThenFill(dot))
        self.wait()
        self.play(*[ReplacementTransform(m1.copy(), m2) for m1, m2 in [(self.line, line), (self.distance_label, distance_label)]])
        self.wait()
    ...
    def show_triangle(self):
        ...
        self.play(FadeIn(triangle), Animation(VGroup(self.line, self.dot, self.number_label[1], *self.distance_label[1:])), run_time=2)
        self.wait(2)
...
class GeneralExample(OneMoreExample):
    CONFIG = {'number': complex(4, 1), 'square_color': MAROON_B, 'result_label_vect': UP + LEFT}
    ...
    def square_point(self):
        ...
        arrow = Arrow(z_point, square_point, path_arc=np.pi / 2)
        ...
        z_to_z_squared.add_to_back(BackgroundRectangle(VGroup(z_to_z_squared[2][0], *z_to_z_squared[:-1])), BackgroundRectangle(z_to_z_squared[2][1]))
        self.play(Write(label), ShowCreation(line), DrawBorderThenFill(dot))
        self.wait()
        self.play(ShowCreation(arrow), FadeIn(z_to_z_squared), Animation(label))
        self.play(*[ReplacementTransform(start.copy(), target, path_arc=np.pi / 2, run_time=1.5) for start, target in [(dot, square_dot), (line, square_line), (label, square_label)]])
        self.wait()
...
class BoringExample(GeneralExample):
    CONFIG = {'number': complex(2, 2), 'result_label_vect': RIGHT}
...
class FiveTwoExample(GeneralExample):
    CONFIG = {'number': complex(5, 2), 'unit_size': 0.25, 'x_label_range': list(range(-10, 40, 5)), 'y_label_range': list(range(0, 30, 5))}

class WriteGeneralFormula(GeneralExample):
    CONFIG = {'plane_center': 2 * RIGHT, 'x_label_range': [], 'y_label_range': [], 'unit_size': 0.7, 'number': complex(2, 1)}
    ...
    def show_squaring(self):
        ...
        self.play(*[ReplacementTransform(start.copy(), target, run_time=1.5, path_arc=np.pi / 2) for start, target in [(dot, square_dot), (line, square_line), (label, square_label)]])
        self.example_label = label
        self.square_label = square_label

    def expand_square(self):
        rect = Rectangle(height=2.5, width=7, stroke_width=0, fill_color=BLACK, fill_opacity=0.8)
        rect.to_corner(UP + LEFT, buff=0)
        ...
        self.wait()
        self.play(real_part.copy().next_to, real_part_line.copy(), DOWN, SMALL_BUFF, ShowCreation(real_part_line))
        self.wait()
        self.play(FadeOut(VGroup(self.example_label, self.example_dot, self.example_line, self.z_to_z_squared, self.z_to_z_squared_arrow)), imag_part.copy().next_to, imag_part_line.copy(), RIGHT, SMALL_BUFF, ShowCreation(imag_part_line))
        self.wait()
    ...
    def draw_triangle(self):
        ...
        triangle = Polygon(ORIGIN, RIGHT, RIGHT + UP, stroke_width=0, fill_color=MAROON_B, fill_opacity=0.5)
...
class VisualizeZSquared(Scene):
    CONFIG = {'initial_unit_size': 0.4, 'final_unit_size': 0.1, 'plane_center': 3 * LEFT + 2 * DOWN, 'x_label_range': list(range(-12, 24, 4)), 'y_label_range': list(range(-4, 24, 4)), 'dot_color': YELLOW, 'square_color': MAROON_B, 'big_dot_radius': 0.075, 'dot_radius': 0.05}
    ...
    def add_plane(self):
        ...
        background_plane = ComplexPlane(x_radius=width, y_radius=height, stroke_width=2, stroke_color=BLUE_E, secondary_line_ratio=0)
    ...
    def draw_arrows(self):
        z_list = [complex(2, 1), complex(3, 2), complex(0, 1), complex(-1, 0)]
        arrows = VGroup()
        dots = VGroup()
        for z in z_list:
            ...
            angle *= 2
            arrow = Arrow(z_point, square_point, path_arc=angle, color=WHITE, tip_length=0.15, buff=SMALL_BUFF)
            z_dot, square_dot = [Dot(point, color=color, radius=self.big_dot_radius) for point, color in [(z_point, self.dot_color), (square_point, self.square_color)]]
    ...
    def draw_dots(self):
        min_corner, max_corner = [self.background_plane.point_to_coords(u * FRAME_X_RADIUS * RIGHT + u * FRAME_Y_RADIUS * UP) for u in (-1, 1)]
        x_min, y_min = list(map(int, min_corner[:2]))
        x_max, y_max = list(map(int, max_corner[:2]))
        dots = VGroup(*[Dot(self.background_plane.coords_to_point(x, y), color=self.dot_color, radius=self.dot_radius) for x in range(x_min, x_max + 1) for y in range(y_min, y_max + 1)])
        dots.sort(lambda p: np.dot(p, UP + RIGHT))
        self.add_foreground_mobject(self.coordinate_labels)
        self.play(LaggedStartMap(DrawBorderThenFill, dots, stroke_width=3, stroke_color=PINK, run_time=3, lag_ratio=0.2))
        self.wait()
        self.dots = dots

    def add_colored_grid(self):
        color_grid = self.get_color_grid()
        self.play(self.background_planes.set_stroke, None, 1, LaggedStartMap(FadeIn, color_grid, run_time=2), Animation(self.dots))
        self.wait()
    ...
    def apply_transformation(self):
        ...
        def update_dot(dot, alpha):
            ...
            return dot
        self.play(Homotopy(self.homotopy, self.color_grid), *[UpdateFromAlphaFunc(dot, update_dot) for dot in self.dots], run_time=3)
        self.wait(2)
        self.play(self.color_grid.set_stroke, None, 3)
        self.wait()
        scale_factor = self.big_dot_radius / self.dot_radius
        self.play(LaggedStartMap(ApplyMethod, self.dots, lambda d: (d.scale, scale_factor), rate_func=there_and_back, run_time=3))
        self.wait()
    ...
    def zoom_out(self):
        ...
        movers = [self.background_plane, self.color_grid, self.dots, self.coordinate_labels]
    ...
    def get_color_grid(self):
        ...
        color_grid = ComplexPlane(x_radius=width, y_radius=int(height), secondary_line_ratio=0, stroke_width=2)
    ...
    def get_triangles(self, z_list):
        ...
        for z in z_list:
            ...
            triangle = Polygon(ORIGIN, RIGHT, RIGHT + UP, stroke_color=BLUE, stroke_width=2, fill_color=BLUE, fill_opacity=0.5)
            triangle.replace(line, stretch=True)
            a = int(z.real)
            b = int(z.imag)
            c = int(abs(z))
...
class PointsWeMiss(VisualizeZSquared):
    CONFIG = {'final_unit_size': 0.4, 'plane_center': 2 * LEFT + 2 * DOWN, 'dot_x_range': list(range(-5, 6)), 'dot_y_range': list(range(-4, 4))}
    ...
    def add_dots(self):
        ...
        dots = VGroup(*[Dot(self.background_plane.number_to_point(z), color=self.dot_color, radius=self.big_dot_radius) for z in z_list])
        dots.sort(get_norm)
        self.add(dots)
        self.dots = dots
...
class DrawSingleRadialLine(PointsWeMiss):
    ...
    def draw_line(self):
        ...
        line = Line(self.plane_center, self.background_plane.coords_to_point(15, 20), color=WHITE)
        ...
        self.wait()
        self.play(LaggedStartMap(DrawBorderThenFill, added_dots, stroke_color=PINK, stroke_width=4, run_time=3))
        self.wait()

class DrawRadialLines(PointsWeMiss):
    CONFIG = {'final_unit_size': 0.2, 'dot_x_range': list(range(-4, 10)), 'dot_y_range': list(range(-4, 10)), 'x_label_range': list(range(-12, 40, 4)), 'y_label_range': list(range(-4, 32, 4)), 'big_dot_radius': 0.05}
    ...
    def show_single_line(self):
        ...
        self.wait()
        self.play(LaggedStartMap(DrawBorderThenFill, line.new_dots, stroke_width=4, stroke_color=PINK, run_time=3))
        self.wait()
    ...
    def show_all_lines(self):
        ...
        self.play(LaggedStartMap(DrawBorderThenFill, new_dots, stroke_width=4, stroke_color=PINK, run_time=3))
        self.wait()
        self.new_dots = new_dots

    def show_triangles(self):
        z_list = [complex(9, 12), complex(7, 24), complex(8, 15), complex(21, 20), complex(36, 15)]
        triangles = self.get_triangles(z_list)
        triangle = triangles[0]
        self.play(FadeIn(triangle))
        self.wait(2)
...
class RationalPointsOnUnitCircle(DrawRadialLines):
    CONFIG = {'initial_unit_size': 1.2, 'final_unit_size': 0.4, 'plane_center': 1.5 * DOWN}
    ...
    def show_rational_points_on_unit_circle(self):
        ...
        for x, y in coord_list:
            ...
            line = Line(self.plane_center, point)
            line.set_color(dot.get_color())
            label = OldTex('{' + str(x), '\\over', str(int(norm)) + '}', '+', '{' + str(y), '\\over', str(int(norm)) + '}', 'i')
    ...
    def divide_by_c_squared(self):
        ...
        second_line = OldTex('\\left(', '{a', '\\over', 'c}', '\\right)', '^2', '+', '\\left(', '{b', '\\over', 'c}', '\\right)', '^2', '=', '1')
        ...
        circle_arrow = Arrow(circle_label.get_bottom(), self.unit_circle.point_from_proportion(0.45), color=GREEN)
        self.play(FadeIn(top_rect), FadeIn(top_line))
        self.wait()
        self.play(*[ReplacementTransform(top_rect, second_rect)] + [ReplacementTransform(top_line.get_parts_by_tex(tex, substring=False), second_line.get_parts_by_tex(tex), run_time=2, path_arc=-np.pi / 3) for tex in ('a', 'b', 'c', '^2', '+', '=')] + [ReplacementTransform(top_line.get_parts_by_tex('1'), second_line.get_parts_by_tex('1'), run_time=2)] + [Write(second_line.get_parts_by_tex(tex), run_time=2, rate_func=squish_rate_func(smooth, 0, 0.5)) for tex in ('(', ')', 'over')])
        self.wait(2)
        self.play(Write(circle_label))
        self.play(ShowCreation(circle_arrow))
        self.wait(2)
        self.play(FadeOut(circle_arrow))
        self.algebra = VGroup(second_rect, second_line, circle_label)
    ...
    def from_rational_point_to_triple(self):
        ...
        self.wait(2)
        self.play(*[ApplyMethod(mob.scale_about_point, scale_factor, self.plane_center) for mob in [self.background_plane, self.coordinate_labels, self.unit_circle, rational_point_group]] + [Animation(self.algebra)])
...
class ProjectPointsOntoUnitCircle(DrawRadialLines):
    ...
    def zoom_in(self):
        ...
        group = VGroup(self.background_plane, self.coordinate_labels, self.color_grid, self.lines, self.unit_circle, self.dots)
...
class FinalProof(RationalPointsOnUnitCircle):
    ...
    def show_slope_is_rational(self):
        ...
        self.wait(3)
        self.play(*list(map(FadeOut, [self.example_dot, self.example_label, self.secant_line, h_line, v_line, run_brace, rise_brace, run_text, rise_text])))
    ...
    def show_all_rational_slopes(self):
        ...
        for u in range(2, 7):
            for v in range(1, u):
                ...
                dot = Dot(point, color=YELLOW)
                line = Line(self.background_plane.number_to_point(-1), point, color=self.neg_one_dot.get_color())
                line.add(dot)
        ...
        for new_line, new_label in zip(lines, labels)[1:]:
            self.play(Transform(line, new_line), Transform(label, new_label))
            self.wait()
    ...
    def square_example_point(self):
        ...
        two_theta = OldTex('2\\theta')
        two_theta.next_to(self.background_plane.coords_to_point(0, 1), UP + RIGHT, SMALL_BUFF)
        two_theta_arrow = Arrow(two_theta.get_right(), square_arc.point_from_proportion(0.75), tip_length=0.15, path_arc=-np.pi / 2, color=WHITE, buff=SMALL_BUFF)
    ...
    def project_onto_circle(self):
        ...
        dot.generate_target()
        dot.target.move_to(line.target.get_end())
        self.play(MoveToTarget(line), MoveToTarget(dot))
        self.wait()
    ...
    def show_same_slope(self):
        line = Line(self.neg_one_dot.get_center(), self.rational_point_dot.get_center(), color=self.neg_one_dot.get_color())
    ...
    def write_v_over_u_slope(self):
        ...
        self.wait()
        self.play(*[ReplacementTransform(self.uv_label.get_part_by_tex(tex).copy(), rhs.get_part_by_tex(tex), run_time=2) for tex in ('u', 'v')] + [Write(rhs.get_part_by_tex(tex)) for tex in ('=', 'over')])
        self.wait(2)
        self.play(ShowCreation(rect), FadeIn(morty))
        self.play(PiCreatureSays(morty, 'Free to choose!', bubble_config={'height': 1.5, 'width': 3}, target_mode='hooray', look_at=rect))
        self.play(Blink(morty))
        self.wait(2)
...
class BitOfCircleGeometry(Scene):
    ...
    def construct(self):
        ...
        for point, tex, color in ((O, '2', MAROON_B), (p2, '', RED)):
            ...
            arc = Arc(angle=line2.get_angle() - line1.get_angle(), start_angle=line1.get_angle(), radius=0.75, color=WHITE)
...
class Thumbnail(DrawRadialLines):
    ...
    def construct(self):
        ...
        self.show_single_line()
        self.show_all_lines()
        rect = Rectangle(height=4.3, width=4.2, stroke_width=3, stroke_color=WHITE, fill_color=BLACK, fill_opacity=1)
        rect.to_corner(UP + RIGHT, buff=0.01)
        triples = VGroup(*list(map(Tex, ['3^2 + 4^2 = 5^2', '5^2 + 12^2 = 13^2', '8^2 + 15^2 = 17^2', '\\vdots'])))
...
class Poster(DrawRadialLines):
    CONFIG = {'final_unit_size': 0.1, 'plane_center': ORIGIN}
