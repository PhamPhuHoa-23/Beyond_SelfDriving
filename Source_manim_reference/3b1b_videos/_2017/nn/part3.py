...
class LayOutPlan(Scene):
    ...
    def construct(self):
        ...
        items = BulletedList('Recap', 'Intuitive walkthrough', 'Derivatives in \\\\ computational graphs')
...
class InterpretGradientComponents(GradientNudging):
    CONFIG = {'network_mob_config': {'layer_to_layer_buff': 3}, 'stroke_width_exp': 2, 'n_decimals': 6, 'n_steps': 3, 'start_cost': 3.48, 'delta_cost': -0.21}
    ...
    def ask_about_high_dimensions(self):
        ...
        self.play(Write(words, run_time=2), GrowArrow(arrow))
    ...
    def isolate_particular_weights(self):
        ...
        new_vect_contents = VGroup(OldTex('\\vdots'), d1, OldTex('\\vdots'), d2, OldTex('\\vdots'))
        ...
        self.play(FadeOut(vect_contents), FadeIn(new_vect_contents), FadeOut(w_terms), FadeIn(new_w_terms), edges.set_stroke, GREY_B, 0.35)
    ...
    def tweak_individual_weights(self):
        ...
        for i in range(2):
            ...
            added_anims = []
            if i == 0:
                added_anims = [GrowFromCenter(cost.brace), ShowCreation(cost.number_line), cost.dot.restore]
            self.play(group1.restore, group2.fade, 0.7, GrowFromCenter(w.brace), ShowCreation(w.number_line), w.dot.restore, Write(w.words, run_time=1), *added_anims)
            for x in range(2):
                func = lambda a: interpolate(cost_num, cost_num - change, a)
                self.play(ChangingDecimal(cost, func), cost.dot.shift, change * RIGHT, w.dot.shift, 0.25 * RIGHT, edge.set_stroke, None, 8, rate_func=lambda t: wiggle(t, 4), run_time=2)
                self.wait()
    ...
    def move_grad_terms_into_position(self, grad_terms, *added_anims):
        ...
        self.play(Transform(grad_terms, points, remover=True, lag_ratio=0.5, run_time=1), FadeOut(words), *added_anims)

    def get_weight_adjustment_anims(self, edges, cost):
        start_cost = cost.number
        target_cost = start_cost + self.delta_cost
        w_terms = self.cost_expression[1]
        return [self.get_edge_change_anim(edges), LaggedStartMap(Indicate, w_terms, rate_func=there_and_back, run_time=1.5), ChangingDecimal(cost, lambda a: interpolate(start_cost, target_cost, a), run_time=1.5)]

class GetLostInNotation(PiCreatureScene):

    def construct(self):
        morty = self.pi_creature
        equations = VGroup(OldTex('\\delta', '^L', '=', '\\nabla_a', 'C', "\\odot \\sigma'(", 'z', '^L)'), OldTex('\\delta', '^l = ((', 'w', '^{l+1})^T', '\\delta', "^{l+1}) \\odot \\sigma'(", 'z', '^l)'), OldTex('{\\partial', 'C', '\\over \\partial', 'b', '_j^l} =', '\\delta', '_j^l'), OldTex('{\\partial', 'C', ' \\over \\partial', 'w', '_{jk}^l} = ', 'a', '_k^{l-1}', '\\delta', '_j^l'))
        for equation in equations:
            equation.set_color_by_tex_to_color_map({'\\delta': YELLOW, 'C': RED, 'b': MAROON_B, 'w': BLUE, 'z': TEAL})
        ...
        self.play(LaggedStartMap(MoveToTarget, movers, run_time=2), morty.change, 'pondering')
        self.wait()
...
class ShowAveragingCost(PreviewLearning):
    CONFIG = {'network_scale_val': 0.8, 'stroke_width_exp': 1, 'start_examples_time': 5, 'examples_per_adjustment_time': 2, 'n_adjustments': 5, 'time_per_example': 1.0 / 15, 'image_height': 1.2}
    ...
    def show_many_examples(self):
        ...
        self.play(LaggedStartMap(ApplyFunction, edges, lambda edge: (lambda m: m.rotate(np.pi / 12).set_color(YELLOW), edge), rate_func=lambda t: wiggle(t, 4), run_time=3))
        words = OldTexText('Each step \\\\ uses every \\\\ example\\\\', '$\\dots$theoretically', alignment='')
        words.set_color(YELLOW)
        words.scale(0.8)
        ...
        for x in range(self.n_adjustments):
            ...
            for train_in, train_out in training_data[:n_examples_per_adjustment]:
                self.show_one_example(train_in, train_out)
                self.wait(self.time_per_example)
            self.play(LaggedStartMap(ApplyMethod, reversed_edges, lambda m: (m.rotate, np.pi), run_time=1, lag_ratio=0.2))
            if x >= 2:
                self.wait()
...
class WalkThroughTwoExample(ShowAveragingCost):
    ...
    def single_example_influencing_weights(self):
        ...
        def adjust_edge_group_anim(edge_group):
            return LaggedStartMap(ApplyFunction, edge_group, lambda edge: (lambda m: m.rotate(np.pi / 12).set_color(YELLOW), edge), rate_func=wiggle, run_time=1)
        self.play(two.next_to, edge_groups[0].get_corner(DOWN + RIGHT), DOWN, adjust_edge_group_anim(edge_groups[0]))
        self.play(ApplyMethod(two.next_to, edge_groups[1].get_corner(UP + RIGHT), UP, path_arc=np.pi / 6), adjust_edge_group_anim(VGroup(*reversed(edge_groups[1]))))
        self.play(ApplyMethod(two.next_to, edge_groups[2].get_corner(DOWN + RIGHT), DOWN, path_arc=-np.pi / 6), adjust_edge_group_anim(edge_groups[2]))
    ...
    def cannot_directly_affect_activations(self):
        ...
        for edge in edges:
            ...
            edge.target.set_stroke(random.choice([BLUE, RED]), 2 * random.random() ** 2)
        self.play(LaggedStartMap(Transform, edges, lambda e: (e, e.target), run_time=4, rate_func=there_and_back), Write(words, run_time=2))
    ...
    def show_desired_activation_nudges(self, neurons, output_labels, alt_output_labels):
        ...
        for i, neuron, label in zip(it.count(), neurons, alt_output_labels):
            ...
            arrow = Arrow(ORIGIN, diff * neuron.get_height() * DOWN, color=RED)
            ...
            if i == 2:
                ...
                rect.set_color(RED)
            rects.add(rect)
        self.play(output_labels.shift, SMALL_BUFF * RIGHT, LaggedStartMap(GrowArrow, arrows, run_time=1))
        self.wait()
        anims = []
        ...
        for i in range(10):
            ...
            anims.append(ChangingDecimal(decimal, get_decimal_update(decimal.number, target), num_decimal_places=1))
            anims.append(UpdateFromFunc(self.decimals[i], lambda m: m.set_fill(WHITE if m.number < 0.8 else BLACK)))
        ...
        self.play(LaggedStartMap(ApplyFunction, arrows, lambda arrow: (lambda m: m.scale(0.5).set_color(YELLOW), arrow), rate_func=wiggle))
        self.play(ShowCreation(two_rect))
        self.wait()
    ...
    def focus_on_one_neuron(self, expanded_mobjects):
        ...
        to_fade = VGroup(*it.chain(network_mob.layers[:2], network_mob.edge_groups[:2], expanded_mobjects, self.decimals, self.arrows))
        ...
        self.set_variables_as_attrs(two_neuron, two_label, two_arrow, two_decimal)

    def show_activation_formula(self):
        rhs = OldTex('=', '\\sigma(', 'w_0', 'a_0', '+', 'w_1', 'a_1', '+', '\\cdots', '+', 'w_{n-1}', 'a_{n-1}', '+', 'b', ')')
        equals = rhs[0]
        ...
        neuron_copy = VGroup(self.two_neuron.copy(), self.two_decimal.copy())
        self.play(neuron_copy.next_to, equals.get_left(), LEFT, self.curr_image.to_corner, UP + LEFT, Write(equals))
        self.play(ReplacementTransform(edges.copy(), w_terms), Write(VGroup(*plus_terms[:-1])), Write(dots), run_time=1.5)
        self.wait()
        ...
        for mob in (b, w_terms, a_terms):
            self.play(mob.shift, MED_SMALL_BUFF * DOWN, rate_func=there_and_back, lag_ratio=0.5, run_time=1.5)
        self.wait()
    ...
    def three_ways_to_increase(self):
        ...
        b = self.b
        increase_words = VGroup(OldTexText('Increase', '$b$'), OldTexText('Increase', '$w_i$'), OldTexText('Change', '$a_i$'))
        for words in increase_words:
            words.set_color_by_tex_to_color_map({'b': b.get_color(), 'w_': w_terms.get_color(), 'a_': a_terms.get_color()})
    ...
    def note_connections_to_brightest_neurons(self):
        ...
        self.play(LaggedStartMap(ApplyFunction, edges, lambda edge: (lambda m: m.rotate(np.pi / 12).set_stroke(YELLOW), edge), rate_func=wiggle))
        self.wait()
        self.play(ShowCreation(bright_edges), ShowCreation(bright_neurons))
        self.play(LaggedStartMap(ApplyMethod, bright_neurons, lambda m: (m.shift, MED_LARGE_BUFF * LEFT), rate_func=there_and_back))
        self.wait()
        self.play(ReplacementTransform(bright_edges[0].copy(), w_terms[0]), ReplacementTransform(bright_neurons[0].copy(), a_terms[0]), ShowCreation(terms_rect))
        self.wait()
        for x in range(2):
            self.play(LaggedStartMap(ShowCreationThenDestruction, bright_edges))
        self.play(LaggedStartMap(ShowCreation, bright_edges))
        self.play(LaggedStartMap(ApplyMethod, dim_neurons, lambda m: (m.shift, MED_LARGE_BUFF * LEFT), rate_func=there_and_back))
    ...
    def fire_together_wire_together(self):
        ...
        self.play(LaggedStartMap(GrowArrow, neuron_arrows), get_edge_animation())
        self.play(GrowArrow(two_neuron_arrow), morty.change, 'raise_right_hand', two_neuron)
        self.play(ApplyMethod(two_neuron.set_fill, WHITE, 1), ChangingDecimal(two_decimal, lambda a: interpolate(two_activation, 1, a), num_decimal_places=1), UpdateFromFunc(two_decimal, lambda m: m.set_color(WHITE if m.number < 0.8 else BLACK)), LaggedStartMap(ShowCreation, bright_edges), run_time=2)
        self.wait()
        self.play(LaggedStartMap(ShowCreation, neuron_rects), Write(seeing_words, run_time=2), morty.change, 'thinking', seeing_words)
        self.wait()
        self.play(ShowCreation(two_neuron_rect), Write(thinking_words, run_time=2), morty.look_at, thinking_words)
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(neuron_rects, two_neuron_rect, seeing_words, thinking_words, words, morty, neuron_arrows, two_neuron_arrow, bright_edges)))
        self.play(ApplyMethod(two_neuron.set_fill, WHITE, two_activation), ChangingDecimal(two_decimal, lambda a: interpolate(1, two_activation, a), num_decimal_places=1), UpdateFromFunc(two_decimal, lambda m: m.set_color(WHITE if m.number < 0.8 else BLACK)))
    ...
    def show_desired_increase_to_previous_neurons(self):
        ...
        self.play(LaggedStartMap(ApplyFunction, prev_neurons, lambda neuron: (lambda m: m.scale(0.5).set_color(YELLOW), neuron), rate_func=wiggle))
        self.wait()
        for positive in [True, False]:
            ...
            self.play(ApplyMethod(neurons.set_fill, color, 1, rate_func=there_and_back))
        self.wait()
        self.play(two_neuron.set_fill, None, 0.8, ChangingDecimal(two_decimal, lambda a: two_neuron.get_fill_opacity()), run_time=3, rate_func=there_and_back)
        self.wait()
        self.play(*[ApplyMethod(edge.set_stroke, None, 3 * edge.get_stroke_width(), rate_func=there_and_back, run_time=2) for edge in edges])
        self.wait()
        self.play(Write(added_words, run_time=1))
        self.play(prev_neurons.set_stroke, WHITE, 2)
        self.set_variables_as_attrs(in_proportion_to_w=added_words, prev_neuron_arrows=all_arrows)
    ...
    def show_other_output_neurons(self):
        ...
        self.play(two_neuron.restore, two_decimal.scale, 0.5, two_decimal.move_to, two_neuron.saved_state, two_arrow.scale, 0.5, two_arrow.next_to, two_neuron.saved_state, RIGHT, 0.5 * SMALL_BUFF, two_label.scale, 0.5, two_label.next_to, two_neuron.saved_state, RIGHT, 1.5 * SMALL_BUFF, FadeOut(VGroup(self.lhs, self.rhs)), *[e.restore for e in two_edges])
        for neuron, decimal, arrow, label in quads[:2] + quads[2:5]:
            ...
            self.play(FadeIn(VGroup(neuron, decimal, arrow, label)), LaggedStartMap(ShowCreation, neuron.edges_in))
            self.play(ReplacementTransform(neuron.edges_in.copy(), new_arrows), Write(plusses, run_time=2))
        ...
        self.play(LaggedStartMap(FadeIn, VGroup(*it.starmap(VGroup, quads[5:]))), LaggedStartMap(FadeIn, VGroup(*[n.edges_in for n in neurons[5:]])), Write(all_dots_plus), run_time=3)
        self.wait(2)
        ...
        self.wait()
        self.play(FadeOut(self.network_mob.edge_groups[-1]), FadeIn(rect), ReplacementTransform(arrows_to_fade, VGroup(target_arrows)))
        self.prev_neuron_arrows = target_arrows

    def show_recursion(self):
        network_mob = self.network_mob
        words_to_fade = VGroup(self.increase_words, self.in_proportion_to_w, self.in_proportion_to_a)
        ...
        self.play(FadeOut(words_to_fade), FadeIn(prev_neurons), LaggedStartMap(ShowCreation, edges))
        self.wait()
        for neuron, arrow in zip(neurons, self.prev_neuron_arrows):
            ...
            for edge in edge_copies:
                ...
                edge.rotate(np.pi)
            self.play(edges.set_stroke, None, 0.15, neuron.edges_in.restore)
...
class NotANeuroScientist(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.add(group)
        self.add(ContinualEdgeUpdate(network, stroke_width_exp=0.5, color=[BLUE, RED]))
        ...
        self.play(Write(words), GrowArrow(arrow), self.teacher.change, 'guilty', words, run_time=1)
...
class ConstructGradientFromAllTrainingExamples(Scene):
    CONFIG = {'image_height': 0.9, 'eyes_height': 0.25, 'n_examples': 6, 'change_scale_val': 0.8}
    ...
    def show_two_requesting_changes(self):
        ...
        for i in list(range(3)) + [4]:
            ...
            self.play(MoveToTarget(weight), two.eyes.look_at_anim(weight.target), FadeIn(bubble), Write(change, run_time=1))
            if random.random() < 0.5:
                self.play(two.eyes.blink_anim())
            else:
                self.wait()
            if i == 0:
                added_anims = [FadeOut(self.brace), FadeOut(self.weights_words)]
            elif i == 4:
                ...
                added_anims = []
            self.play(FadeOut(bubble), weight.restore, two.eyes.look_at_anim(weight.saved_state), change.restore, change.scale, self.change_scale_val, change.move_to, self.get_grid_position(i, 0), *added_anims)
    ...
    def show_all_examples_requesting_changes(self):
        ...
        more_v_dots = VGroup(*[self.dots.copy().move_to(self.get_grid_position(3, j)) for j in range(1, self.n_examples)])
        changes = VGroup(*[self.get_random_decimal().move_to(self.get_grid_position(i, j)) for i in list(range(3)) + [4] for j in range(1, self.n_examples)])
        for change in changes:
            change.scale(self.change_scale_val)
        self.play(LaggedStartMap(FadeIn, examples), LaggedStartMap(ShowCreation, self.h_lines), LaggedStartMap(ShowCreation, self.v_lines), Write(h_dots, run_time=2, rate_func=squish_rate_func(smooth, 0.7, 1)))
        self.play(Write(changes), Write(more_v_dots), Write(more_h_dots), *[example.eyes.look_at_anim(random.choice(changes)) for example in examples])
        for x in range(2):
            self.play(random.choice(examples).eyes.blink_anim())
        k = self.n_examples - 1
        self.change_rows = VGroup(*[VGroup(two_change, *changes[k * i:k * (i + 1)]) for i, two_change in enumerate(self.two_changes)])
        for i in list(range(3)) + [-1]:
            self.change_rows[i].add(more_h_dots[i])
        self.all_eyes = VGroup(*[m.eyes for m in [self.two] + list(examples)])
        self.set_variables_as_attrs(more_h_dots, more_v_dots, h_dots, changes)
    ...
    def average_together(self):
        ...
        self.play(ReplacementTransform(rects[0].copy(), arrows[0]), rects[0].set_stroke, WHITE, 1, ReplacementTransform(self.change_rows[0].copy(), self.change_rows[0].target), *look_at_anims(averages[0]))
        self.play(GrowArrow(arrow_to_averages))
        self.play(LaggedStartMap(ShowCreation, VGroup(*rects[1:])), *look_at_anims(rects[1]))
        self.play(LaggedStartMap(ReplacementTransform, VGroup(*rects[1:]).copy(), lambda m: (m, m.arrow), lag_ratio=0.7), VGroup(*rects[1:]).set_stroke, WHITE, 1, LaggedStartMap(ReplacementTransform, VGroup(*self.change_rows[1:]).copy(), lambda m: (m, m.target), lag_ratio=0.7), Write(dots), *look_at_anims(averages[1]))
        self.blink(3)
        self.wait()
        averages.add(dots)
    ...
    def collapse_into_gradient_vector(self):
        ...
        to_fade = VGroup(*it.chain(self.h_lines, self.v_lines, self.more_h_dots, self.more_v_dots, self.change_rows, self.first_column_dots, self.rects, self.arrows))
        arrow = self.arrow_to_averages
        self.play(LaggedStartMap(FadeOut, to_fade))
        self.play(brackets.shift, shift_vect, brackets.set_fill, WHITE, 1, averages.shift, shift_vect, Transform(arrow, Arrow(arrow.get_start(), arrow.get_end() + shift_vect, buff=0, color=arrow.get_color())), FadeIn(VGroup(*lhs[:3])), FadeIn(VGroup(*lhs[-2:])), *self.get_look_at_anims(lhs))
        self.play(ReplacementTransform(self.weights, w_terms), ReplacementTransform(self.dots, dots_term), *self.get_look_at_anims(w_terms))
        self.blink(2)
...
class WatchPreviousScene(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.change, 'raise_right_hand', screen, self.change_students(*['thinking'] * 3, look_at=screen), ShowCreation(screen))
        self.wait(10)
...
class OrganizeDataIntoMiniBatches(Scene):
    CONFIG = {'n_rows': 5, 'n_cols': 12, 'example_height': 1, 'random_seed': 0}
    ...
    def shuffle_examples(self):
        self.play(LaggedStartMap(ApplyMethod, self.examples, lambda m: (m.restore,), lag_ratio=0.3, run_time=3, path_arc=np.pi / 3))
        self.wait()
    ...
    def divide_into_minibatches(self):
        ...
        self.play(rows.space_out_submobjects, 1.5, rows.to_edge, UP, 1.5, Write(mini_batches_words, run_time=1))
        rects = VGroup(*[SurroundingRectangle(row, stroke_width=0, fill_color=YELLOW, fill_opacity=0.25) for row in rows])
    ...
    def one_step_per_batch(self):
        ...
        def indicate_row(row):
            ...
            return LaggedStartMap(ApplyFunction, row, lambda row: (lambda m: m.scale(0.75).set_color(YELLOW), row), rate_func=wiggle)
        self.play(FadeOut(self.mini_batches_words), GrowFromCenter(brace), Write(text, run_time=2))
    ...
    def get_examples(self):
        ...
        training_data, validation_data, test_data = load_data_wrapper()
        return Group(*[MNistMobject(t[0], rect_kwargs={'stroke_width': 2}).set_height(height) for t in training_data[:n_examples]])
...
class SwimmingInTerms(TeacherStudentsScene):
    ...
    def construct(self):
        terms = VGroup(OldTexText('Cost surface'), OldTexText('Stochastic gradient descent'), OldTexText('Mini-batches'), OldTexText('Backpropagation'))
        terms.arrange(DOWN)
        terms.to_edge(UP)
        self.play(LaggedStartMap(FadeIn, terms), self.change_students(*['horrified'] * 3))
        self.wait()
        self.play(terms[-1].next_to, self.teacher.get_corner(UP + LEFT), UP, FadeOut(VGroup(*terms[:-1])), self.teacher.change, 'raise_right_hand', self.change_students(*['pondering'] * 3))
        self.wait()
...
class CannotFollowCode(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says("I...er...can't follow\\\\ that code at all.", target_mode='confused', index=1)
        ...
        self.wait(2)
        self.teacher_says("Let's get to the \\\\ calculus then", target_mode='hooray', added_anims=[self.change_students(*3 * ['plain'])], run_time=1)
        self.wait(2)
...
class SimplestNetworkExample(PreviewLearning):
    CONFIG = {'random_seed': 6, 'z_color': GREEN, 'cost_color': RED, 'desired_output_color': YELLOW, 'derivative_scale_val': 0.85}
    ...
    def collapse_ordinary_network(self):
        ...
        config.update({'edge_stroke_width': 3, 'edge_propogation_color': YELLOW, 'edge_propogation_time': 1, 'neuron_radius': 0.3})
        ...
        def edge_collapse_anims(edges, left_attachment_target):
            return [ApplyMethod(e.put_start_and_end_on_with_projection, left_attachment_target.get_right(), e.get_end()) for e in edges]
        neuron = simple_network_mob.layers[0].neurons[0]
        self.play(ReplacementTransform(network_mob.layers[0], neuron), *edge_collapse_anims(network_mob.edge_groups[0], neuron))
        for i, layer in enumerate(network_mob.layers[1:]):
            ...
            self.play(ReplacementTransform(layer, neuron), ReplacementTransform(prev_edges, prev_edge_target), *added_anims)
    ...
    def show_weights_and_biases(self):
        ...
        expression = OldTex('C', '(', 'w_1', ',', 'b_1', ',', 'w_2', ',', 'b_2', ',', 'w_3', ',', 'b_3', ')')
        ...
        for x in range(2):
            self.play(LaggedStartMap(Indicate, variables, rate_func=wiggle, run_time=4))
        self.wait()
        self.play(FadeOut(other_terms), ReplacementTransform(w_terms, edges), ReplacementTransform(b_terms, neurons))
        self.remove(expression)

    def focus_just_on_last_two_layers(self):
        to_fade = VGroup(*it.chain(*list(zip(self.network_mob.layers[:2], self.network_mob.edge_groups[:2]))))
        for mob in to_fade:
            mob.save_state()
    ...
    def label_neurons(self):
        ...
        for neuron, superscript in zip(neurons, superscripts):
            ...
            self.play(Write(label, run_time=1), GrowArrow(arrow))
            self.play(decimal.restore)
            opacity = neuron.get_fill_opacity()
            self.play(neuron.set_fill, None, 0, ChangingDecimal(decimal, lambda a: interpolate(opacity, 0.01, a)), UpdateFromFunc(decimal, lambda d: d.set_fill(WHITE if d.number < 0.8 else BLACK)), run_time=2, rate_func=there_and_back)
            self.wait()
        ...
        self.play(LaggedStartMap(ShowCreation, superscript_rects, lag_ratio=0.8, run_time=1.5), Write(not_exponents, run_time=2))
        self.wait()
    ...
    def show_desired_output(self):
        ...
        self.wait()
        self.set_variables_as_attrs(y_label, y_label_arrow, desired_output_neuron=neuron, desired_output_decimal=decimal, desired_output_rect=rect, desired_output_words=words)
    ...
    def show_cost(self):
        ...
        self.play(ReplacementTransform(pre_a, a), ReplacementTransform(pre_y, y))
    ...
    def introduce_z(self):
        ...
        self.play(ReplacementTransform(z_label.copy(), z_label2), MoveToTarget(aL_start), MoveToTarget(rp))
        self.wait()
    ...
    def break_into_computational_graph(self):
        ...
        top_lines = VGroup(*[Line(term.get_bottom(), zL.target.get_top(), buff=SMALL_BUFF) for term in top_terms.target])
        z_to_a_line, a_to_c_line, y_to_c_line = all_lines = [Line(m1.target.get_bottom(), m2.target.get_top(), buff=SMALL_BUFF) for m1, m2 in [(zL, aL), (aL, C0), (y, C0)]]
    ...
    def show_preceding_layer_in_computational_graph(self):
        ...
        new_edges = VGroup(comp_graph.top_lines.copy(), comp_graph.z_to_a_line.copy())
        ...
        self.play(ReplacementTransform(VGroup(prev_neuron, prev_edge).copy(), new_subgraph), UpdateFromAlphaFunc(new_terms, lambda m, a: m.set_fill(opacity=a)), MoveToTarget(comp_graph), rect.shift, shift_vect)
        self.wait(2)
        self.play(FadeOut(new_subgraph), FadeOut(prev_neuron), FadeOut(prev_edge), comp_graph.restore, rect.shift, -shift_vect, rect.set_stroke, BLACK, 0)
    ...
    def show_number_lines(self):
        ...
        C0.val = (aL.val - 1) ** 2
        number_line = UnitInterval(unit_size=2, stroke_width=2, tick_size=0.075, color=GREY_B)
        for mob in (wL, zL, aL, C0):
            ...
            if mob is C0:
                ...
                tip_length = 0.2
            mob.arrow = Arrow(mob_spot, dot_spot, path_arc=path_arc, tip_length=tip_length, buff=SMALL_BUFF)
            mob.arrow.set_color(mob.get_color())
            mob.arrow.set_stroke(width=5)
            self.play(ShowCreation(mob.number_line, lag_ratio=0.5))
            self.play(ShowCreation(mob.arrow), ReplacementTransform(mob.copy(), mob.dot, path_arc=path_arc))
        self.wait()
    ...
    def ask_about_w_sensitivity(self):
        ...
        def generate_dot_update(term, val_func):
            ...
            def update_dot(dot):
                ...
                return dot
            return update_dot
        dot_update_anims = [UpdateFromFunc(term.dot, generate_dot_update(term, val_func)) for term, val_func in [(zL, get_zL_val), (aL, get_aL_val), (C0, get_C0_val)]]

        def shake_dot(run_time=2, rate_func=there_and_back):
            self.play(ApplyMethod(wL.dot.shift, LEFT, rate_func=rate_func, run_time=run_time), *dot_update_anims)
        ...
        for sym in (del_wL, del_C0):
            ...
            shake_dot()
        self.wait()
        self.set_variables_as_attrs(shake_dot, del_wL, del_C0)
    ...
    def show_derivative_wrt_w(self):
        ...
        self.play(ReplacementTransform(del_C0.copy(), dC_dw[0]), ReplacementTransform(del_wL.copy(), dC_dw[2]), Write(dC_dw[1], run_time=1))
        self.play(FadeOut(cost_word), FadeOut(cost_arrow), ShowCreation(full_rect), Write(words, run_time=1))
        self.wait(2)
    ...
    def show_chain_of_events(self):
        ...
        for sym in (del_zL, del_aL):
            ...
            sym.brace.stretch_about_point(0.5, 1, sym.brace.get_top())
        ...
        for s1, s2 in zip(syms, syms[1:]):
            self.play(ReplacementTransform(s1.copy(), s2), ReplacementTransform(s1.brace.copy(), s2.brace))
    ...
    def show_chain_rule(self):
        ...
        for frac, top_sym, bot_sym in zip(group[1:], del_syms[1:], del_syms):
            ...
            self.play(ReplacementTransform(top_sym.copy(), frac[0]), FadeIn(frac[1]))
    ...
    def name_chain_rule(self):
        ...
        self.play(ShowCreation(rect))
        self.play(MoveToTarget(group), Write(words, run_time=1), graph_parts.scale, 0.7, graph_parts.get_bottom())
        self.wait(2)
    ...
    def prepare_for_derivatives(self):
        ...
        self.play(FadeOut(self.all_comp_graph_parts), FadeOut(self.desired_output_words), MoveToTarget(az_formulas), MoveToTarget(cost_equation))
    ...
    def compute_derivatives(self):
        ...
        self.wait()
        self.play(ReplacementTransform(cost_rhs.copy(), dC_da.rhs, path_arc=np.pi / 2))
        self.wait()
        self.play(cost_equation.restore)
        self.wait()
        neuron = self.last_neurons[0]
        decimal = self.decimals[0]
        double_arrow = DoubleArrow(neuron.get_right(), self.desired_output_neuron.get_left(), buff=SMALL_BUFF, color=RED)
        ...
        moving_decimals.target.next_to(dC_da.rhs, DOWN, buff=MED_LARGE_BUFF, aligned_edge=RIGHT)
        ...
        for target_o in (0, opacity):
            self.wait(2)
            self.play(neuron.set_fill, None, target_o, *[ChangingDecimal(d, lambda a: neuron.get_fill_opacity()) for d in (decimal, moving_decimals[0])])
        ...
        self.wait()
        self.play(ReplacementTransform(a_rhs.copy(), da_dz.rhs))
        self.wait()
        ...
        self.play(zL_formula.next_to, dz_dw.rhs, DOWN, MED_LARGE_BUFF, LEFT)
        self.wait()
        rect = SurroundingRectangle(VGroup(*zL_formula[2:4]))
        self.play(ShowCreation(rect))
        self.play(FadeOut(rect))
        self.play(ReplacementTransform(z_rhs[1].copy(), dz_dw.rhs))
        self.wait()
    ...
    def fire_together_wire_together(self):
        ...
        def get_decimal_anims():
            return [ChangingDecimal(decimal, lambda a: neuron.get_fill_opacity()), UpdateFromFunc(decimal, lambda m: m.set_color(WHITE if neuron.get_fill_opacity() < 0.8 else BLACK))]
        self.play(ShowCreation(rect))
        self.play(FadeOut(rect))
        self.play(del_wL.next_to, edge, UP, SMALL_BUFF)
        self.play(edge.set_stroke, None, 10, rate_func=wiggle, run_time=3)
        self.wait()
        ...
        self.play(neuron.set_fill, None, 0, *get_decimal_anims(), run_time=3, rate_func=there_and_back)
        self.wait()
        opacity = neuron.get_fill_opacity()
    ...
    def organize_chain_rule_rhs(self):
        ...
        right_group = VGroup(self.cost_equation, self.zL_formula, self.aL_formula, self.network_mob, self.decimals, self.a_labels, self.a_label_arrows, self.y_label, self.y_label_arrow, self.desired_output_neuron, self.desired_output_rect, self.desired_output_decimal)
        self.play(MoveToTarget(rhs_group, path_arc=np.pi / 2), Write(equals), FadeOut(fracs), FadeOut(equals_group), right_group.to_corner, DOWN + RIGHT)
        self.wait()
    ...
    def show_average_derivative(self):
        ...
        full_derivative = OldTex('{\\partial C', '\\over', '\\partial w^{(L)}}', '=', '\\frac{1}{n}', '\\sum_{k=0}^{n-1}', '{\\partial C_k', '\\over', '\\partial w^{(L)}}')
        full_derivative.set_color_by_tex_to_color_map({'partial C': self.cost_color, 'partial w': self.del_wL.get_color()})
        ...
        self.play(*list(map(FadeOut, [VGroup(*full_derivative[3:]), lhs_brace, lhs_text, rhs_brace, rhs_text])))
        self.dC_dw = lhs
    ...
    def show_gradient(self):
        ...
        terms = VGroup(OldTex('{\\partial C', '\\over', '\\partial w^{(1)}'), OldTex('{\\partial C', '\\over', '\\partial b^{(1)}'), OldTex('\\vdots'), dC_dw.target, OldTex('{\\partial C', '\\over', '\\partial b^{(L)}'))
        for term in terms:
            if isinstance(term, Tex):
                term.set_color_by_tex_to_color_map({'partial C': RED, 'partial w': BLUE, 'partial b': MAROON_B})
    ...
    def transition_to_derivative_wrt_b(self):
        ...
        for x in range(2):
            self.play(LaggedStartMap(Indicate, path_to_C, rate_func=there_and_back, run_time=1.5, lag_ratio=0.7))
        self.wait()
    ...
    def show_derivative_wrt_b(self):
        ...
        arrow = Arrow(dz_db.get_bottom(), one.get_bottom(), path_arc=np.pi / 2, color=WHITE)
        arrow.set_stroke(width=2)
        wL, bL, zL, aL, C0 = [getattr(comp_graph, attr) for attr in ['wL', 'bL', 'zL', 'aL', 'C0']]
        path_to_C = VGroup(bL, zL, aL, C0)

        def get_path_animation():
            return LaggedStartMap(Indicate, path_to_C, rate_func=there_and_back, run_time=1.5, lag_ratio=0.7)
        zL_formula = self.zL_formula
        ...
        self.play(left_term_group.shift, DOWN, left_term_group.fade, 1)
        ...
        self.play(ShowCreation(arrow), ReplacementTransform(dz_db.copy(), one, path_arc=arrow.path_arc))
        self.wait(2)
    ...
    def show_derivative_wrt_a(self):
        ...
        arrow = Arrow(dz_daLm1.get_bottom(), wL.get_bottom(), path_arc=np.pi / 2, color=WHITE)
        comp_graph = self.comp_graph
        ...
        self.play(FadeIn(dz_daLm1), VGroup(*self.chain_rule_equation[-2:]).shift, SMALL_BUFF * RIGHT)
        self.wait()
        ...
        self.play(ShowCreation(arrow), ReplacementTransform(dz_daLm1.copy(), wL, path_arc=arrow.path_arc))
        self.wait(2)
    ...
    def show_previous_weight_and_bias(self):
        ...
        prev_layer = VGroup(self.network_mob.layers[1], self.network_mob.edge_groups[1])
        ...
        self.play(ShowCreation(prev_comp_subgraph, run_time=1), self.chain_rule_equation.to_edge, RIGHT)
        ...
        self.play(Write(a_label, run_time=1), Write(decimal, run_time=1), GrowArrow(arrow))

    def animate_long_path(self):
        comp_graph = self.comp_graph
        path_to_C = VGroup(self.wLm1, self.zLm1, *[getattr(comp_graph, attr) for attr in ['aLm1', 'zL', 'aL', 'C0']])
        for x in range(2):
            self.play(LaggedStartMap(Indicate, path_to_C, rate_func=there_and_back, run_time=1.5, lag_ratio=0.4))
        self.wait(2)
...
class GeneralFormulas(SimplestNetworkExample):
    CONFIG = {'layer_sizes': [3, 3, 2], 'network_mob_config': {'include_output_labels': False, 'neuron_to_neuron_buff': LARGE_BUFF, 'neuron_radius': 0.3}, 'edge_stroke_width': 4, 'stroke_width_exp': 0.2, 'random_seed': 9}
    ...
    def show_all_a_labels(self):
        ...
        start_labels, start_arrows = [VGroup(*list(map(VGroup, [group[i][0] for i in (0, 1)]))).copy() for group in (all_labels, all_arrows)]
        ...
        self.wait()
        self.play(ReplacementTransform(start_labels, all_labels), ReplacementTransform(start_arrows, all_arrows))
        self.play(LaggedStartMap(ShowCreationThenDestruction, VGroup(*all_subscript_rects.family_members_with_points()), lag_ratio=0.7))
        self.wait()
        self.set_variables_as_attrs(L_neurons, Lm1_neurons, all_arrows, all_labels, all_decimals, all_subscript_rects)
    ...
    def only_show_abstract_a_labels(self):
        ...
        self.play(LaggedStartMap(FadeOut, labels_to_fade), LaggedStartMap(FadeOut, arrows_to_fade), run_time=1)
    ...
    def add_desired_output(self):
        ...
        self.play(FadeIn(rect), FadeIn(words), ReplacementTransform(layer.copy(), desired_output), FadeIn(labels), *[ReplacementTransform(n1.decimal.copy(), n2.decimal) for n1, n2 in zip(layer.neurons, desired_output.neurons)] + list(map(GrowArrow, arrows)))
        self.wait()
        self.set_variables_as_attrs(desired_output, desired_output_decimals, desired_output_rect=rect, desired_output_words=words)

    def show_cost(self):
        aj = self.chosen_neurons[1].label.copy()
        yj = self.desired_output.neurons[0].label.copy()
        cost_equation = OldTex('C_0', '=', '\\sum_{j = 0}^{n_L - 1}', '(', 'a^{(L)}_j', '-', 'y_j', ')', '^2')
        cost_equation.to_corner(UP + RIGHT)
        ...
        self.wait(2)
        self.play(LaggedStartMap(Indicate, sum_part, rate_func=wiggle))
        self.wait()
    ...
    def show_example_weight(self):
        ...
        self.play(LaggedStartMap(ApplyFunction, edges, lambda e: (lambda m: m.rotate(np.pi / 12).set_color(YELLOW), e), rate_func=wiggle))
    ...
    def show_values_between_weight_and_cost(self):
        z_formula = OldTex('z^{(L)}_j', '=', 'w^{(L)}_{j0}', 'a^{(L-1)}_0', '+', 'w^{(L)}_{j1}', 'a^{(L-1)}_1', '+', 'w^{(L)}_{j2}', 'a^{(L-1)}_2', '+', 'b^{(L)}_j')
        compact_z_formula = OldTex('z^{(L)}_j', '=', '\\cdots', '', '+w^{(L)}_{jk}', 'a^{(L-1)}_k', '+', '\\cdots', '', '', '')
        for expression in (z_formula, compact_z_formula):
            expression.to_corner(UP + RIGHT)
            expression.set_color_by_tex_to_color_map({'z^': self.z_color, 'w^': self.w_label.get_color(), 'b^': MAROON_B})
        ...
        aL_part = a_formula[0]
        to_fade = VGroup(self.desired_output, self.desired_output_decimals, self.desired_output_rect, self.desired_output_words, *[VGroup(n.arrow, n.label) for n in self.desired_output.neurons])
        self.play(FadeOut(to_fade), self.cost_equation.next_to, a_formula, DOWN, MED_LARGE_BUFF, self.cost_equation.to_edge, RIGHT, ReplacementTransform(self.w_label[1].copy(), w_part), ReplacementTransform(self.chosen_neurons[0].label.copy(), aLm1_part))
        ...
        self.play(Write(VGroup(*a_formula[1:3] + [a_formula[-1]])), ReplacementTransform(z_formula[0].copy(), a_formula.get_part_by_tex('z^')))
        self.wait()
        self.set_variables_as_attrs(z_formula, compact_z_formula, a_formula)

    def show_weight_chain_rule(self):
        chain_rule = self.get_chain_rule('{\\partial C_0', '\\over', '\\partial w^{(L)}_{jk}}', '=', '{\\partial z^{(L)}_j', '\\over', '\\partial w^{(L)}_{jk}}', '{\\partial a^{(L)}_j', '\\over', '\\partial z^{(L)}_j}', '{\\partial C_0', '\\over', '\\partial a^{(L)}_j}')
    ...
    def show_derivative_wrt_prev_activation(self):
        chain_rule = self.get_chain_rule('{\\partial C_0', '\\over', '\\partial a^{(L-1)}_k}', '=', '\\sum_{j=0}^{n_L - 1}', '{\\partial z^{(L)}_j', '\\over', '\\partial a^{(L-1)}_k}', '{\\partial a^{(L)}_j', '\\over', '\\partial z^{(L)}_j}', '{\\partial C_0', '\\over', '\\partial a^{(L)}_j}')
    ...
    def show_multiple_paths_from_prev_layer_neuron(self):
        neurons = self.network_mob.layers[-1].neurons
        labels, arrows, decimals = [VGroup(*[getattr(n, attr) for n in neurons]) for attr in ('label', 'arrow', 'decimal')]
        edges = VGroup(*[n.edges_in[1] for n in neurons])
        labels[0].generate_target()
        self.replace_subscript(labels[0].target, '0')
        paths = [VGroup(self.chosen_neurons[0].label, self.chosen_neurons[0].arrow, self.chosen_neurons[0], self.chosen_neurons[0].decimal, edges[i], neurons[i], decimals[i], arrows[i], labels[i]) for i in range(2)]
        path_lines = VGroup()
        for path in paths:
        ...
        self.play(MoveToTarget(labels[0]), FadeIn(labels[1]), GrowArrow(arrows[1]), edges[1].restore, FadeOut(self.w_label))
        for x in range(5):
            anims = [ShowCreationThenDestruction(path_line, run_time=1.5, time_width=0.5) for path_line in path_lines]
            if x == 2:
        ...
        for path, path_line in zip(paths, path_lines):
            label = path[-1]
            self.play(LaggedStartMap(Indicate, path, rate_func=wiggle, run_time=1), ShowCreation(path_line), Animation(label))
            self.wait()
    ...
    def show_previous_layer(self):
        ...
        to_fade = VGroup(self.chosen_neurons[0].label, self.chosen_neurons[0].arrow)
        for neuron in layer.neurons:
            neuron.add(self.get_neuron_activation_decimal(neuron))
        all_edges_out = VGroup(*[VGroup(*[n.edges_in[i] for n in mid_neurons]).copy() for i in range(len(layer.neurons))])
        ...
        def get_neurons_decimal_anims(neuron):
            return [ChangingDecimal(neuron.decimal, lambda a: neuron.get_fill_opacity()), UpdateFromFunc(neuron.decimal, lambda m: m.set_fill(WHITE if neuron.get_fill_opacity() < 0.8 else BLACK))]
        self.play(ShowCreation(deriv_rect))
        self.play(LaggedStartMap(ShowCreationThenDestruction, mid_neuron_outlines))
        self.play(*it.chain(*[[ApplyMethod(n.set_fill, None, random.random())] + get_neurons_decimal_anims(n) for n in mid_neurons]), run_time=4, rate_func=there_and_back)
        self.play(faded_edges.restore)
        self.play(LaggedStartMap(GrowFromCenter, layer.neurons, run_time=1), LaggedStartMap(ShowCreation, edges), FadeOut(to_fade))
        for x in range(3):
    ...
    def get_chain_rule(self, *tex):
        ...
        chain_rule.set_color_by_tex_to_color_map({'C_0': self.cost_color, 'z^': self.z_color, 'w^': self.w_label.get_color()})
        return chain_rule

class ThatsPrettyMuchIt(TeacherStudentsScene):

    def construct(self):
        self.teacher_says("That's pretty \\\\ much it!", target_mode='hooray', run_time=1)
        self.wait(2)
...
class LayersOfComplexity(Scene):
    ...
    def get_chain_rule_equations(self):
        w_deriv = OldTex('{\\partial C', '\\over', '\\partial w^{(l)}_{jk}}', '=', 'a^{(l-1)}_k', "\\sigma'(z^{(l)}_j)", '{\\partial C', '\\over', '\\partial a^{(l)}_j}')
        lil_rect = SurroundingRectangle(VGroup(*w_deriv[-3:]), buff=0.5 * SMALL_BUFF)
        a_deriv = OldTex('\\sum_{j = 0}^{n_{l+1} - 1}', 'w^{(l+1)}_{jk}', "\\sigma'(z^{(l+1)}_j)", '{\\partial C', '\\over', '\\partial a^{(l+1)}_j}')
        ...
        arrow = Arrow(lil_rect.get_corner(DOWN + LEFT), big_rect.get_top())
        result = VGroup(w_deriv, lil_rect, arrow, big_rect, a_deriv, or_word, last_a_deriv)
        for expression in (w_deriv, a_deriv, last_a_deriv):
            expression.set_color_by_tex_to_color_map({'C': RED, 'z^': GREEN, 'w^': BLUE, 'b^': MAROON_B})
        return result
...
class SponsorFrame(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(morty.change, 'hooray', t_shirt_words, LaggedStartMap(ShowCreation, t_shirt_words_outline))
...
class Thumbnail(PreviewLearning):
    ...
    def construct(self):
        ...
        for edge in it.chain(*network_mob.edge_groups):
            arrow = Arrow(edge.get_end(), edge.get_start(), buff=0, tip_length=0.1, color=edge.get_color())
            network_mob.add(arrow.tip)
        arrow = Vector(3 * LEFT, tip_length=0.75, rectangular_stem_width=0.2, color=BLUE)
...
class SupplementThumbnail(Thumbnail):
    CONFIG = {'title': 'Backpropagation \\\\ calculus', 'network_scale_val': 0.7}
