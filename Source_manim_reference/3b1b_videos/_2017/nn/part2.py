...
class ShowLastVideo(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        assumption_words = OldTexText("I assume you've\\\\ watched this")
        ...
        self.play(ShowCreation(frame), self.teacher.change, 'raise_right_hand')
        self.play(Write(title), self.change_students(*['thinking'] * 3))
        self.play(Animation(title), GrowArrow(arrow), FadeIn(assumption_words))
        self.wait(5)
...
class ShowPlan(Scene):
    ...
    def construct(self):
        ...
        items = VGroup(*[OldTexText('$\\cdot$ %s' % s) for s in ['Recap', 'Gradient descent', 'Analyze this network', 'Where to learn more', 'Research corner']])
        ...
        for item in items[1:]:
            ...
            self.play(to_fade.set_fill, None, 0.5, item.set_fill, None, 1)
            self.wait()

class BeginAndEndRecap(Scene):

    def construct(self):
        recap = OldTex('\\langle', '\\text{Recap}', '\\rangle')
...
class PreviewLearning(NetworkScene):
    ...
    def show_training(self):
        ...
        for train_in, train_out in training_data[:self.n_examples]:
            ...
            self.backprop_one_example(train_in, train_out, FadeOut(image), self.network_mob.layers.restore)
    ...
    def activate_network(self, train_in, *added_anims, **kwargs):
        ...
        active_layers = [self.network_mob.get_active_layer(i, vect) for i, vect in enumerate(activations)]
        all_edges = VGroup(*it.chain(*network_mob.edge_groups))
        run_time = kwargs.get('run_time', self.default_activate_run_time)
        edge_animation = LaggedStartMap(ShowCreationThenDestruction, all_edges.copy().set_fill(YELLOW), run_time=run_time, lag_ratio=0.3, remover=True)
        layer_animation = Transform(VGroup(*layers), VGroup(*active_layers), run_time=run_time, lag_ratio=0.5, rate_func=linear)
    ...
    def backprop_one_example(self, train_in, train_out, *added_outro_anims):
        ...
        neuron_groups = VGroup(*[layer.neurons for layer in network_mob.layers[1:]])
        delta_neuron_groups = neuron_groups.copy()
        edge_groups = network_mob.edge_groups
        delta_edge_groups = VGroup(*[edge_group.copy() for edge_group in edge_groups])
        tups = list(zip(it.count(), nabla_b, nabla_w, delta_neuron_groups, neuron_groups, delta_edge_groups, edge_groups))
        ...
        for i, nb, nw, delta_neurons, neurons, delta_edges, edges in reversed(tups):
            ...
            for edge, w in zip(delta_edges.split(), shown_nw.T.flatten()):
                edge.set_stroke(nc_color if w > 0 else pc_color, 3 * abs(w) / max_w)
                edge.rotate(np.pi)
            if i == 2:
                delta_edges.submobjects = [delta_edges[j] for j in np.argsort(shown_nw.T.flatten())]
            network = self.network
            network.weights[i] -= self.eta * nw
            network.biases[i] -= self.eta * nb
            self.play(ShowCreation(delta_edges, lag_ratio=0), FadeIn(delta_neurons), run_time=0.5)
        edge_groups.save_state()
        self.color_network_edges()
        self.remove(edge_groups)
        self.play(*it.chain([ReplacementTransform(edge_groups.saved_state, edge_groups)], list(map(FadeOut, [delta_edge_groups, delta_neuron_groups])), added_outro_anims))
    ...
    def get_edge_animation(self):
        ...
        return LaggedStartMap(ApplyFunction, edges, lambda mob: (lambda m: m.rotate(np.pi / 12).set_color(YELLOW), mob), rate_func=wiggle)
...
class TrainingVsTestData(Scene):
    CONFIG = {'n_examples': 10, 'n_new_examples_shown': 10}
    ...
    def get_examples(self):
        ...
        group = Group(*it.starmap(get_training_image_group, self.data[ci:ci + self.n_examples]))
        group.arrange(DOWN)
        group.scale(0.5)
        return group

    def introduce_all_data(self):
        training_examples, test_examples = [self.get_examples() for x in range(2)]
        training_examples.next_to(ORIGIN, LEFT)
        test_examples.next_to(ORIGIN, RIGHT)
        self.play(LaggedStartMap(FadeIn, training_examples), LaggedStartMap(FadeIn, test_examples))
    ...
    def subdivide_into_training_and_testing(self):
        ...
        for is_correct, test_example in zip(bools, test_examples.target):
            ...
            marks.add(mark)
        self.play(MoveToTarget(training_examples), GrowFromCenter(train_brace), FadeIn(train_words))
        self.wait()
        self.play(MoveToTarget(test_examples), GrowFromCenter(test_brace), FadeIn(test_words))
...
class MNistDescription(Scene):
    CONFIG = {'n_grids': 5, 'n_rows_per_grid': 10, 'n_cols_per_grid': 8, 'time_per_example': 1.0 / 120}
    ...
    def construct(self):
        ...
        self.play(Write(link_words, run_time=2), LaggedStartMap(GrowArrow, arrows))
        self.wait()
        training_data, validation_data, test_data = load_data_wrapper()
        epc = self.n_rows_per_grid * self.n_cols_per_grid
        training_data_groups = [training_data[i * epc:(i + 1) * epc] for i in range(self.n_grids)]
        for i, td_group in enumerate(training_data_groups):
            print(i)
            group = Group(*[self.get_digit_pair(v_in, v_out) for v_in, v_out in td_group])
            group.arrange_in_grid(n_rows=self.n_rows_per_grid)
            group.set_height(FRAME_HEIGHT - 1)
            if i == 0:
                self.play(LaggedStartMap(FadeIn, group), FadeOut(word_group))
            else:
...
class NotSciFi(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.student_says('Machines learning?!?', index=0, target_mode='pleading', run_time=1)
        bubble = students[0].bubble
        students[0].bubble = None
        self.student_says('Should we \\\\ be worried?', index=2, target_mode='confused', bubble_config={'direction': LEFT}, run_time=1)
        self.wait()
        students[0].bubble = bubble
        self.teacher_says("It's actually \\\\ just calculus.", run_time=1)
        self.teacher.bubble = None
        self.wait()
        self.student_says('Even worse!', target_mode='horrified', bubble_config={'direction': LEFT, 'width': 3, 'height': 2})
        self.wait(2)

class FunctionMinmization(GraphScene):
    CONFIG = {'x_labeled_nums': list(range(-1, 10))}
    ...
    def construct(self):
        ...
        dots = VGroup(*[Dot().move_to(self.input_to_graph_point(x, graph)) for x in range(10)])
        ...
        self.add(*[Mobject.add_updater(dot, update_dot) for dot in dots])
        self.wait(10)
...
class ChangingDecimalWithColor(ChangingDecimal):
    ...
    def interpolate_mobject(self, alpha):
        ...
        self.decimal_number.set_fill(interpolate_color(BLACK, WHITE, 0.5 + num * 0.5), opacity=1)

class IntroduceCostFunction(PreviewLearning):
    CONFIG = {'max_stroke_width': 2, 'full_edges_exp': 5, 'n_training_examples': 100, 'bias_color': MAROON_B}
    ...
    def isolate_one_neuron(self):
        network_mob = self.network_mob
        neurons = VGroup(*it.chain(*[layer.neurons for layer in network_mob.layers[1:]]))
        ...
        self.play(FadeOut(edges, **kwargs), FadeOut(neurons, **kwargs), FadeOut(output_labels, **kwargs), Animation(neuron), neuron.edges_in.set_stroke, None, 2)
        self.neuron = neuron

    def reminder_of_weights_and_bias(self):
        neuron = self.neuron
        layer0 = self.network_mob.layers[0]
        active_layer0 = self.network_mob.get_active_layer(0, np.random.random(len(layer0.neurons)))
        prev_neurons = layer0.neurons
        weighted_edges = VGroup(*[self.color_edge_randomly(edge.copy(), exp=1) for edge in neuron.edges_in])
        formula = OldTex('=', '\\sigma(', 'w_1', 'a_1', '+', 'w_2', 'a_2', '+', '\\cdots', '+', 'w_n', 'a_n', '+', 'b', ')')
        w_labels = formula.get_parts_by_tex('w_')
        a_labels = formula.get_parts_by_tex('a_')
        b = formula.get_part_by_tex('b')
        sigma = VGroup(formula.get_part_by_tex('\\sigma'), formula.get_part_by_tex(')'))
        symbols = VGroup(*[formula.get_parts_by_tex(tex) for tex in ('=', '+', 'dots')])
        ...
        weights_arrow_to_edges = Arrow(weights_word.get_bottom(), neuron.edges_in[0].get_center(), color=self.positive_edge_color)
        weights_arrow_to_syms = VGroup(*[Arrow(weights_word.get_bottom(), w_label.get_top(), color=self.positive_edge_color) for w_label in w_labels])
        ...
        self.play(Transform(layer0, active_layer0), neuron.set_fill, None, 0.5, FadeIn(formula), run_time=2, lag_ratio=0.5)
        self.play(LaggedStartMap(ShowCreationThenDestruction, neuron.edges_in.copy().set_stroke(YELLOW, 3), run_time=1.5, lag_ratio=0.7, remover=True))
        self.play(Write(weights_word), *list(map(GrowArrow, weights_arrow_to_syms)), run_time=1)
        self.wait()
        self.play(ReplacementTransform(w_labels.copy(), weighted_edges, remover=True), Transform(neuron.edges_in, weighted_edges), ReplacementTransform(weights_arrow_to_syms, VGroup(weights_arrow_to_edges)))
        self.wait()
        self.play(Write(bias_word), GrowArrow(bias_arrow), run_time=1)
        self.wait(2)
        ...
        self.play(Transform(weights_word, w_random), Transform(bias_word, b_random), *[ApplyFunction(self.color_edge_randomly, edge) for edge in neuron.edges_in])
        self.play(LaggedStartMap(ApplyMethod, neuron.edges_in, lambda m: (m.rotate, np.pi / 12), rate_func=wiggle, run_time=2))
        self.play(*list(map(FadeOut, [weights_word, weights_arrow_to_edges, bias_word, bias_arrow, formula])))
    ...
    def bring_back_rest_of_network(self):
        ...
        self.play(*[LaggedStartMap(FadeIn, group, run_time=3) for group in (neurons, edges)])
    ...
    def feed_in_example(self):
        ...
        neurons = VGroup(*[Circle(stroke_width=1, stroke_color=WHITE, fill_opacity=pixel.fill_rgb[0], fill_color=WHITE, radius=pixel.get_height() / 2).move_to(pixel) for pixel in image])
        layer0 = self.network_mob.layers[0]
        n = self.network_mob.max_shown_neurons
        neurons.target = VGroup(*it.chain(VGroup(*layer0.neurons[:n / 2]).set_fill(opacity=0), [VectorizedPoint(layer0.dots.get_center()) for x in range(len(neurons) - n)], VGroup(*layer0.neurons[-n / 2:]).set_fill(opacity=0)))
        self.play(self.network_mob.shift, 0.5 * RIGHT, ShowCreation(rect), LaggedStartMap(DrawBorderThenFill, image), LaggedStartMap(DrawBorderThenFill, neurons), run_time=1)
        self.play(MoveToTarget(neurons, lag_ratio=0.5, remover=True), layer0.neurons.set_fill, None, 0)
    ...
    def need_a_cost_function(self):
        ...
        words = OldTexText("What's the", "``cost''\\\\", 'of this difference?')
        words.set_color_by_tex('cost', RED)
        words.next_to(layers, UP)
        words.to_edge(UP)
        words.shift_onto_screen()
        double_arrow = DoubleArrow(layer.get_right(), desired_layer.get_left(), color=RED)
    ...
    def fade_all_but_last_layer(self):
        network_mob = self.network_mob
        to_fade = VGroup(*it.chain(*list(zip(network_mob.layers[:-1], network_mob.edge_groups))))
    ...
    def break_down_cost_function(self):
        ...
        decimal_groups = VGroup(*[self.num_vect_to_decimals(self.layer_to_num_vect(l)) for l in (layer, desired_layer)])
        terms = VGroup()
        symbols = VGroup()
        for d1, d2 in zip(*decimal_groups):
            term = OldTex('(', '0.00', '-', '0.00', ')^2', '+')
        ...
        terms.arrange(DOWN, buff=SMALL_BUFF, aligned_edge=LEFT)
        ...
        self.play(*[ReplacementTransform(VGroup(*l.neurons[:10]).copy(), dg) for l, dg in zip([layer, desired_layer], decimal_groups)])
        self.play(FadeIn(symbols), MoveToTarget(image_group), FadeIn(cost_of), GrowFromCenter(brace))
        self.wait()
    ...
    def show_small_cost_output(self):
        ...
        arrows = VGroup(*[Arrow(ORIGIN, LEFT).next_to(d, LEFT, MED_LARGE_BUFF) for d in decimals])
        ...
        for arrow, d1, d2 in zip(arrows, *self.decimal_groups):
            ...
            terms.add(term)
            term_updates.append(ChangingDecimalWithColor(term, term.num_update_func, num_decimal_places=4))
        ...
        def sum_update(alpha):
            return sum([(d1.number - d2.number) ** 2 for d1, d2 in zip(*self.decimal_groups)])
        ...
        update_decimals = [ChangingDecimalWithColor(decimal, generate_decimal_update_func(decimal.number, t)) for decimal, t in zip(decimals, target_vect)]
        self.play(cost_group.scale, 0.5, cost_group.to_corner, UP + LEFT, MoveToTarget(brace), LaggedStartMap(GrowArrow, arrows), LaggedStartMap(FadeIn, terms), FadeIn(sum_term), Animation(decimals))
        self.play(*it.chain(update_decimals, term_updates, [ApplyMethod(neuron.set_fill, None, t) for neuron, t in zip(neurons, target_vect)]))
        self.wait()
        ...
        self.play(*it.chain(update_decimals, term_updates, [neurons.restore]), run_time=2)
        self.wait()
        self.play(cost_group.restore, brace.restore, FadeOut(VGroup(terms, sum_term, arrows)))
    ...
    def average_over_all_training_data(self):
        ...
        wait_times = iter(it.chain(4 * [0.5], 4 * [0.25], 8 * [0.125], it.repeat(0.1)))
        ...
        for in_vect, out_vect in training_data[:self.n_training_examples]:
            ...
            new_decimal_groups = VGroup(*[self.num_vect_to_decimals(v) for v in (random_v, out_vect)])
    ...
    def color_edge_randomly(self, edge, exp=1):
        ...
        edge.set_stroke(color=pc if r > 0 else nc, width=abs(r))
        return edge

    def layer_to_num_vect(self, layer, n_terms=10):
        return [n.get_fill_opacity() for n in layer.neurons][:n_terms]

    def num_vect_to_decimals(self, num_vect):
        return VGroup(*[DecimalNumber(n).set_fill(opacity=0.5 * n + 0.5) for n in num_vect])

    def num_vect_to_column_vector(self, num_vect, height):
        decimals = VGroup(*[DecimalNumber(n).set_fill(opacity=0.5 * n + 0.5) for n in num_vect])
...
class YellAtNetwork(PiCreatureScene, PreviewLearning):
    ...
    def construct(self):
        ...
        self.play(PiCreatureBubbleIntroduction(randy, 'Bad network!', target_mode='angry', look_at=eyes, run_time=1), eyes.look_at_anim(randy.eyes))
        self.play(eyes.change_mode_anim('sad'))
        self.play(eyes.look_at_anim(3 * DOWN + 3 * RIGHT))
        self.play(FadeIn(desired), RemovePiCreatureBubble(randy, target_mode='sassy', look_at=desired), eyes.look_at_anim(desired))
        self.play(eyes.blink_anim())
        rect = SurroundingRectangle(VGroup(layer_copy.neurons[3], layer_copy[-1][3]))
        self.play(ShowCreation(rect))
        layer_copy.add(rect)
        self.wait()
        self.play(layer.copy().replace, layer_copy, 1, Write(q_marks, run_time=1), layer_copy.shift, 1.5 * RIGHT, randy.change, 'angry', eyes)
...
class ThisIsVeryComplicated(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says('Very complicated!', target_mode='surprised', run_time=1)
        self.play_student_changes(*3 * ['guilty'])
        self.wait(2)

class EmphasizeComplexityOfCostFunction(IntroduceCostFunction):
    CONFIG = {'stroke_width_exp': 3, 'n_examples': 32}
    ...
    def show_cost_function(self):
        ...
        training_data, validation_data, test_data = load_data_wrapper()
        training_examples = Group(*list(map(self.get_training_pair_mob, training_data[:self.n_examples])))
        training_examples.next_to(parameter_words, DOWN, buff=LARGE_BUFF)
        self.play(FadeOut(to_fade), FadeOut(input_words[1]), FadeOut(output_words[1]), MoveToTarget(network_mob), FadeIn(rect), FadeIn(network_label), Transform(self.title, new_title), self.underline.stretch_to_fit_width, new_title.get_width())
        self.play(ApplyMethod(parameter_words[1].move_to, input_words[1], LEFT, path_arc=np.pi), self.get_edge_animation())
        self.wait()
        self.play(GrowArrow(arrow), Write(cost, run_time=1))
        self.play(Write(new_output_word, run_time=1))
        self.wait()
        self.play(FadeIn(new_parameter_word), FadeIn(training_examples[0]))
        self.wait(0.5)
        for last_ex, ex in zip(training_examples, training_examples[1:]):
            activations = self.network.get_activation_of_all_layers(ex.in_vect)
...
class NetworkGrowthMindset(YellAtNetwork):
    ...
    def construct(self):
        ...
        edge_update = ContinualEdgeUpdate(network_mob, colors=[BLUE, RED])
        self.play(PiCreatureSays(randy, 'Awful, just awful!', target_mode='angry', look_at=eyes, run_time=1), eyes.change_mode_anim('concerned_musician'))
        self.wait()
        self.add(edge_update)
        self.pi_creature_says('But we can do better! \\\\ Growth mindset!', target_mode='hooray')
        self.play(eyes.change_mode_anim('happy'))
        self.wait(3)

class SingleVariableCostFunction(GraphScene):
    CONFIG = {'x_axis_label': '$w$', 'y_axis_label': '', 'x_min': -5, 'x_max': 7, 'x_axis_width': 12, 'graph_origin': 2.5 * DOWN + LEFT, 'tangent_line_color': YELLOW}
    ...
    def reduce_full_function_to_single_variable(self):
        ...
        big_brace, lil_brace = [Brace(cf[1], DOWN) for cf in (cf1, cf2)]
        ...
        name.set_color(RED)
        self.add(name, cf1)
        self.play(GrowFromCenter(big_brace), FadeIn(big_brace_text))
        self.wait()
        self.play(ReplacementTransform(big_brace, lil_brace), ReplacementTransform(big_brace_text, lil_brace_text), ReplacementTransform(cf1, cf2))
    ...
    def show_graph(self):
        ...
        self.setup_axes()
        graph = self.get_graph(lambda x: 0.5 * (x - 3) ** 2 + 2, color=RED)
        self.play(FadeOut(self.to_fade), Write(self.axes), Animation(function_label), run_time=1)
        self.play(function_label.next_to, self.input_to_graph_point(5, graph), RIGHT, ShowCreation(graph))
        self.wait()
        self.graph = graph
    ...
    def find_exact_solution(self):
        ...
        w_min[1].fade(1)
        x = 3
        dot = Dot(self.input_to_graph_point(x, graph), color=YELLOW)
        line = self.get_vertical_line_to_graph(x, graph, line_class=DashedLine, color=YELLOW)
        formula = OldTex('\\frac{dC}{dw}(w) = 0')
        formula.next_to(dot, UP, buff=2)
        formula.shift(LEFT)
        arrow = Arrow(formula.get_bottom(), dot.get_center())
        self.play(w_min.shift, line.get_bottom() - w_min[0].get_top(), MED_SMALL_BUFF * DOWN, w_min.set_fill, WHITE, 1)
    ...
    def make_function_more_complicated(self):
        ...
        graph = self.get_graph(lambda x: 0.05 * ((x + 2) * (x - 1) * (x - 3)) ** 2 + 2 + 0.3 * (x - 3), color=RED)
        self.play(ReplacementTransform(self.graph, graph), function_label.shift, 2 * UP + 1.9 * LEFT, FadeOut(brace_group), Animation(dot))
        self.graph = graph
        self.play(Write(words, run_time=1), GrowFromCenter(brace))
        self.wait(2)
    ...
    def take_steps(self):
        ...
        def update_line(line):
            ...
            line.put_start_and_end_on_with_projection(self.coords_to_point(x, 0), self.input_to_graph_point(x, graph))
            return line
        ...
        for vect, color in ((LEFT, BLUE), (RIGHT, GREEN)):
            ...
            q_marks.add(q_mark)
        self.play(w_mob.next_to, point, DOWN, FadeOut(min_mob), line_update_anim, dot_update_anim)
        self.wait()
        self.play(*it.chain(list(map(GrowArrow, arrows)), list(map(FadeIn, q_marks))))
        self.wait()
    ...
    def take_steps_based_on_slope(self):
        ...
        self.play(ShowCreation(tangent_line), Animation(dot))
        self.play(VGroup(arrows[1], q_marks).set_fill, None, 0)
        self.play(w_mob.shift, MED_SMALL_BUFF * LEFT, MaintainPositionRelativeTo(arrow_group, w_mob), line_update_anim, dot_update_anim)
        self.wait()
        new_x = 0.3
        new_point = self.coords_to_point(new_x, 0)
        new_tangent_line = self.get_tangent_line(new_x, arrows[1].get_color())
        self.play(FadeOut(tangent_line), w_mob.next_to, new_point, DOWN, arrow_group.next_to, new_point, UP, SMALL_BUFF, arrow_group.set_fill, None, 1, dot_update_anim, line_update_anim)
        self.play(ShowCreation(new_tangent_line), Animation(dot), Animation(arrow_group))
        self.wait()
        self.play(VGroup(arrows[0], q_marks).set_fill, None, 0)
        self.play(w_mob.shift, MED_SMALL_BUFF * RIGHT, MaintainPositionRelativeTo(arrow_group, w_mob), line_update_anim, dot_update_anim)
        self.play(FadeOut(VGroup(new_tangent_line, arrow_group)), Animation(dot))
        self.wait()
        for x in (0.8, 1.1, 0.95):
            self.play(w_mob.next_to, self.coords_to_point(x, 0), DOWN, line_update_anim, dot_update_anim)
        self.wait()
    ...
    def ball_rolling_down_hill(self):
        ...
        self.play(point.move_to, w_mob, UpdateFromFunc(ball, update_ball), run_time=3)
        self.wait(2)
        points = [VectorizedPoint(self.coords_to_point(x, 0)) for x in np.linspace(-2.7, 3.7, 11)]
        balls = VGroup()
        updates = []
        for point in points:
            new_ball = ball.copy()
            ...
            balls.add(new_ball)
            updates += [Mobject.add_updater(point, update_point), Mobject.add_updater(new_ball, update_ball)]
    ...
    def note_step_sizes(self):
        ...
        def update_line(line):
            ...
            return line
        self.play(ShowCreation(line), w_mob.next_to, point, DOWN, line_update_anim)
        for n in range(6):
            x = self.x_axis.point_to_number(point.get_center())
            new_x = interpolate(x, target_x, 0.5)
            self.play(point.move_to, self.coords_to_point(new_x, 0), MaintainPositionRelativeTo(w_mob, point), line_update_anim, UpdateFromFunc(line, update_line))
            self.wait(0.5)
        self.wait()
...
class TwoVariableInputSpace(Scene):
    ...
    def add_plane(self):
        plane = NumberPlane(x_radius=FRAME_X_RADIUS / 2)
        ...
        self.play(*list(map(Write, [plane, name, x, y])), run_time=1)
        self.wait()
        self.plane = plane
    ...
    def ask_about_direction(self):
        ...
        dot.fade(1)
        arrows = VGroup(*[Arrow(ORIGIN, vect).shift(point) for vect in compass_directions(8)])
        arrows.set_color(WHITE)
        question = OldTexText('Which direction decreases \\\\', '$C(x, y)$', 'most quickly?')
        question.scale(0.7)
        ...
        self.play(FadeIn(question), LaggedStartMap(GrowArrow, arrows))
        self.wait()
        self.arrows = arrows
        self.dot = dot
    ...
    def show_gradient(self):
        ...
        arrow = arrows[3]
        new_arrow = Arrow(dot.get_center(), arrow.get_end(), buff=0, color=GREEN)
        new_arrow.set_color(GREEN)
        arrow.save_state()
        gradient = OldTex('\\nabla C(x, y)')
        gradient.add_background_rectangle()
        gradient.next_to(arrow.get_end(), UP, SMALL_BUFF)
        gradient_words = OldTexText("``Gradient'', the direction\\\\ of", 'steepest increase')
        ...
        self.play(Transform(arrow, new_arrow), Animation(dot), *[FadeOut(a) for a in arrows if a is not arrow])
        self.play(FadeIn(gradient))
        self.play(Write(gradient_words, run_time=2))
        self.wait(2)
        self.play(arrow.fade, ReplacementTransform(arrow.copy(), anti_arrow))
        self.wait(2)
...
class GradientDescentAlgorithm(Scene):
    ...
    def construct(self):
        words = VGroup(OldTexText('Compute', '$\\nabla C$'), OldTexText('Small step in', '$-\\nabla C$', 'direction'), OldTexText('Repeat.'))
        words.arrange(DOWN, aligned_edge=LEFT)
        words.set_width(FRAME_WIDTH - 1)
...
class ShowFullCostFunctionGradient(PreviewLearning):
    ...
    def organize_weights_as_column_vector(self):
        ...
        decimals = VGroup(*[DecimalNumber(num).set_color(BLUE_D if num > 0 else RED) for num in nums])
        dots = OldTex('\\vdots')
        ...
        edges_target = VGroup(*it.chain(decimals[:3], [dots] * (len(edges) - 6), decimals[-3:]))
        ...
        self.play(FadeOut(layers), edges.space_out_submobjects, 1.2)
        self.play(ReplacementTransform(edges, edges_target, run_time=2, lag_ratio=0.5), LaggedStartMap(FadeIn, words))
    ...
    def show_gradient(self):
        ...
        rhs = VGroup(column_vect[0].copy(), new_decimals, column_vect[2].copy())
        ...
        for od, nd in zip(old_decimals, new_decimals):
            ...
            od_num = od.number
            nd_num = nd.number
            self.play(nd.move_to, od, nd.shift, 1.5 * RIGHT)
            self.play(Transform(nd, VectorizedPoint(od.get_center()), lag_ratio=0.5, remover=True), ChangingDecimal(od, lambda a: interpolate(od_num, od_num + nd_num, a)))
        self.wait()
...
class HowMinimizingCostMeansBetterTrainingPerformance(IntroduceCostFunction):
    ...
    def improve_last_layer(self):
        ...
        updates = [generate_update(n1, n2) for n1, n2 in zip(values, target_values)]
        self.play(LaggedStartMap(FadeIn, words, run_time=1))
        self.play(*[ChangingDecimal(d, update) for d, update in zip(decimals, updates)] + [UpdateFromFunc(d, lambda mob: mob.set_fill(interpolate_color(BLACK, WHITE, 0.5 + 0.5 * mob.number), opacity=1)) for d in decimals] + [ApplyMethod(neuron.set_fill, WHITE, target_value) for neuron, target_value in zip(neurons, target_values)], run_time=3)
        self.wait()
...
class ConfusedAboutHighDimension(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('13{,}002-dimensional \\\\ nudge?', target_mode='confused')
        self.play_student_changes(*['confused'] * 3)
        self.wait(2)
        self.teacher_thinks('', bubble_config={'width': 6, 'height': 4}, added_anims=[self.change_students(*['plain'] * 3)])
        self.zoom_in_on_thought_bubble()

class NonSpatialGradientIntuition(Scene):
    CONFIG = {'w_color': YELLOW, 'positive_color': BLUE, 'negative_color': RED, 'vect_height': FRAME_Y_RADIUS - MED_LARGE_BUFF, 'text_scale_value': 0.7}
    ...
    def add_vector(self):
        ...
        lhs.to_edge(LEFT)
        ws = VGroup(*[VGroup(OldTex(tex)) for tex in it.chain(['w_%d' % d for d in range(3)], ['\\vdots'], ['w_{13{,}00%d}' % d for d in range(3)])])
    ...
    def add_gradient(self):
        ...
        for num, w in zip(nums, ws):
            ...
            decimals.add(decimal)
        new_dots = dots.copy()
        grad_content = VGroup(*it.chain(decimals[:3], new_dots, decimals[3:]))
        ...
        self.wait()
        self.play(ReplacementTransform(self.top_lhs.copy(), lhs), ReplacementTransform(self.vect.copy(), grad_vect), GrowFromCenter(brace), FadeIn(words))
        self.wait()
    ...
    def show_sign_interpretation(self):
        ...
        self.play(LaggedStartMap(MoveToTarget, ws), LaggedStartMap(FadeIn, direction_phrases))
        self.wait(2)
    ...
    def show_magnitude_interpretation(self):
        ...
        for phrase, decimal in zip(direction_phrases, decimals):
            ...
            rect = SurroundingRectangle(VGroup(*decimal[-4:]), buff=SMALL_BUFF, color=GREY_B)
            rect.target = words
            rects.add(rect)
...
class SomeConnectionsMatterMoreThanOthers(PreviewLearning):
    ...
    def fade_edges(self):
        edges = VGroup(*it.chain(*self.network_mob.edge_groups))
        self.play(*[ApplyMethod(edge.set_stroke, BLACK, 0, rate_func=lambda a: 0.5 * smooth(a)) for edge in edges])
    ...
    def show_important_connection(self):
        ...
        self.play(ShowCreation(edge), GrowArrow(arrow), FadeIn(words))
        self.wait()
...
class SpinningVectorWithLabel(Scene):
    ...
    def construct(self):
        ...
        cd1 = ChangingDecimal(decimals[0], lambda a: np.cos(vector.get_angle()), tracked_mobject=label.decimals[0])
        cd2 = ChangingDecimal(decimals[1], lambda a: np.sin(vector.get_angle()), tracked_mobject=label.decimals[1])
        self.play(Rotate(vector, 0.999 * np.pi, in_place=False, run_time=8, rate_func=there_and_back), UpdateFromFunc(label, lambda m: m.next_to(vector.get_end(), UP + RIGHT)), cd1, cd2)
        self.wait()
...
class TwoGradientInterpretationsIn2D(Scene):
    ...
    def add_function_definitions(self):
        func = OldTex('C(', 'x, y', ')', '=', '\\frac{3}{2}x^2', '+', '\\frac{1}{2}y^2')
        func.shift(FRAME_X_RADIUS * LEFT / 2).to_edge(UP)
        grad = OldTex('\\nabla', 'C(', '1, 1', ')', '=')
        vect = OldTex('\\left[\\begin{array}{c} 3 \\\\ 1 \\end{array}\\right]')
    ...
    def point_out_direction(self):
        ...
        arrow = Arrow(self.plane.coords_to_point(1, 1), self.plane.coords_to_point(4, 2), buff=0, color=vect.get_color())
        ...
        self.play(ReplacementTransform(vect.copy(), arrow), Animation(dot))
    ...
    def point_out_relative_importance(self):
        ...
        self.play(Write(x_words, run_time=2), ShowCreation(x_rect))
        self.wait()
        self.play(Write(y_words, run_time=1), ShowCreation(y_rect))
        self.wait(2)

    def wiggle_in_neighborhood(self):
        dot = self.dot
        steepest_words = self.steepest_words
        neighborhood = Circle(fill_color=BLUE, fill_opacity=0.25, stroke_width=0, radius=0.5)
        neighborhood.move_to(dot)
        self.revert_to_original_skipping_status()
        self.play(FadeOut(steepest_words), GrowFromCenter(neighborhood))
        self.wait()
        for vect in (RIGHT, UP, 0.3 * (3 * RIGHT + UP)):
            self.play(dot.shift, 0.5 * vect, rate_func=lambda t: wiggle(t, 4), run_time=3)
            self.wait()
...
class TODOInsertEmphasizeComplexityOfCostFunctionCopy(TODOStub):
    CONFIG = {'message': 'Insert EmphasizeComplexityOfCostFunction copy'}

class GradientNudging(PreviewLearning):
    CONFIG = {'n_steps': 10, 'n_decimals': 8}
    ...
    def add_gradient(self):
        ...
        self.add(*group)
        self.set_variables_as_attrs(grad_lhs=lhs, grad_vect=vect, grad_arg_words=words, grad_arg_brace=brace)

    def change_weights_repeatedly(self):
        network_mob = self.network_mob
        edges = VGroup(*reversed(list(it.chain(*network_mob.edge_groups))))
        decimals = self.grad_vect.decimals
        words = OldTexText('Change by some small\\\\', 'multiple of $-\\nabla C(\\dots)$')
        words.next_to(network_mob, UP).to_edge(UP)
        arrows = VGroup(*[Arrow(words.get_bottom(), edge_group.get_top(), color=WHITE) for edge_group in network_mob.edge_groups])
        ...
        self.play(ReplacementTransform(mover, target), FadeIn(words), LaggedStartMap(GrowArrow, arrows, run_time=1))
    ...
    def get_edge_change_anim(self, edges):
        ...
        for edge, target_num in zip(edges.target, target_nums):
            ...
            edge.rotate(np.pi)
        return MoveToTarget(edges, lag_ratio=0.5, run_time=1.5)
    ...
    def get_decimal_change_anims(self, decimals):
        ...
        return [ChangingDecimal(decimal, generate_change_func(decimal.number, change)) for decimal, change in zip(decimals, changes)] + [FadeIn(words, rate_func=wrf, run_time=1.5, remover=True)]
...
class TODOInsertCostSurfaceSteps(TODOStub):
    CONFIG = {'message': 'Insert CostSurfaceSteps'}
...
class ContinuouslyRangingNeuron(PreviewLearning):
    ...
    def construct(self):
        ...
        for x in (0.7, 0.35, 0.97, 0.23, 0.54):
            ...
            self.play(neuron.set_fill, None, x, ChangingDecimal(decimal, lambda a: interpolate(curr_num, x, a)), decimal_color_anim)
            self.wait()

class AskHowItDoes(TeacherStudentsScene):

    def construct(self):
        self.student_says('How well \\\\ does it do?', index=0)
        self.wait(5)

class TestPerformance(PreviewLearning):
    CONFIG = {'n_examples': 300, 'time_per_example': 0.1, 'wrong_wait_time': 0.5, 'stroke_width_exp': 2, 'decimal_kwargs': {'num_decimal_places': 3}}
    ...
    def add_fraction(self):
        ...
        word_frac = OldTex('{\\text{Number correct}', '\\over', '\\text{total}}', '=')
        word_frac[0].set_color(GREEN)
        self.frac = self.get_frac()
        self.equals = OldTex('=')
        fracs = VGroup(word_frac, self.frac, self.equals, self.decimal)
        fracs.arrange(RIGHT)
    ...
    def run_through_examples(self):
        title = self.title
        rects = [SurroundingRectangle(VGroup(neuron, label), buff=0.5 * SMALL_BUFF) for neuron, label in zip(self.network_mob.layers[-1].neurons, self.network_mob.output_labels)]
        ...
        for test_in, test_out in ProgressDisplay(list(self.test_data)):
            ...
            group.arrange(RIGHT)
            group.shift(self.title.get_bottom() + MED_SMALL_BUFF * DOWN - image.get_top())
            ...
            new_frac.shift(self.frac[1].get_left() - new_frac[1].get_left())
            self.remove(self.frac)
            self.add(new_frac)
            self.frac = new_frac
            self.equals.next_to(new_frac, RIGHT)
            new_decimal = DecimalNumber(float(self.n_correct) / self.total, **self.decimal_kwargs)
...
class ReactToPerformance(TeacherStudentsScene):
    ...
    def construct(self):
        title = VGroup(OldTexText('Play with network structure'), Arrow(LEFT, RIGHT, color=WHITE), OldTexText('98\\%', 'testing accuracy'))
        title.arrange(RIGHT)
        title.to_edge(UP)
        title[-1][0].set_color(GREEN)
        self.play(Write(title, run_time=2))
        last_words = OldTexText('State of the art \\\\ is', '99.79\\%')
        last_words[-1].set_color(GREEN)
        self.teacher_says("That's pretty", 'good!', target_mode='surprised', run_time=1)
        self.play_student_changes(*['hooray'] * 3)
        self.wait()
        self.teacher_says(last_words, target_mode='hesitant')
        self.play_student_changes(*['pondering'] * 3, look_at=self.teacher.bubble)
        self.wait()

class NoticeWhereItMessesUp(TeacherStudentsScene):

    def construct(self):
        self.teacher_says('Look where it \\\\ messes up', run_time=1)
        self.wait(2)

class WrongExamples(TestPerformance):
    CONFIG = {'time_per_example': 0}

class TODOBreakUpNineByPatterns(TODOStub):
    CONFIG = {'message': 'Insert the scene with 9 \\\\ broken up by patterns'}
...
class NotAtAll(TeacherStudentsScene, PreviewLearning):
    ...
    def construct(self):
        ...
        self.color_network_edges()
        self.teacher_says(words, target_mode='guilty', run_time=1)
        self.play_student_changes(*['sassy'] * 3)
        self.play(self.teacher.change, 'concerned_musician', words[1].set_color, WHITE)
        self.wait(2)

class InterpretFirstWeightMatrixRows(TestPerformance):
    CONFIG = {'stroke_width_exp': 1}
    ...
    def slide_network_to_side(self):
        network_mob = self.network_mob
        network_mob.generate_target()
        to_fade = VGroup(*it.chain(network_mob.edge_groups[1:], network_mob.layers[2:], network_mob.output_labels))
        to_keep = VGroup(*it.chain(network_mob.edge_groups[0], network_mob.layers[:2]))
        shift_val = FRAME_X_RADIUS * LEFT + MED_LARGE_BUFF * RIGHT - to_keep.get_left()
        self.play(to_fade.shift, shift_val, to_fade.fade, 1, to_keep.shift, shift_val)
        self.remove(to_fade)
    ...
    def show_all_pixel_array(self):
        ...
        self.play(VGroup(*neurons[1:]).set_stroke, None, 0.5, FadeIn(self.words), neurons[0].edges_in.set_stroke, None, 2, *[ApplyMethod(edge.set_stroke, None, 0.25) for edge in edges if edge not in neurons[0].edges_in])
        self.wait()
        last_neuron = None
        for neuron, pixel_array in zip(neurons, self.pixel_arrays):
            if last_neuron:
                self.play(last_neuron.edges_in.set_stroke, None, 0.25, last_neuron.set_stroke, None, 0.5, neuron.set_stroke, None, 3, neuron.edges_in.set_stroke, None, 2)
            self.play(ReplacementTransform(neuron.edges_in.copy().set_fill(opacity=0), pixel_array))
...
class InputRandomData(TestPerformance):
    ...
    def show_random_image(self):
        ...
        arrow = Arrow(rect.get_top(), self.network_mob.layers[0].neurons.get_top(), path_arc=-2 * np.pi / 3)
        arrow.tip.set_stroke(width=3)
        self.play(ShowCreation(rect), LaggedStartMap(DrawBorderThenFill, image, stroke_width=0.5))
    ...
    def show_expected_outcomes(self):
        ...
        arrow = Arrow(words.get_bottom(), neurons.get_top(), color=WHITE)
        self.play(Write(words, run_time=1), GrowArrow(arrow))
        ...
        for vect in vects:
            ...
            self.play(LaggedStartMap(MoveToTarget, neurons, run_time=1))
            self.wait()
    ...
    def feed_in_random_data(self):
        ...
        image.target = VGroup(*it.chain(target_neurons[:len(neurons) / 2], [point] * (len(image) - len(neurons)), target_neurons[-len(neurons) / 2:]))
        self.play(MoveToTarget(image, run_time=2, lag_ratio=0.5))
        self.activate_network(rand_vect, FadeOut(image))
        neurons = self.network_mob.layers[-1].neurons
        choice = np.argmax([n.get_fill_opacity() for n in neurons])
        rect = SurroundingRectangle(VGroup(neurons[choice], output_labels[choice]))
    ...
    def network_speaks(self):
        ...
        eyes = Eyes(network_mob.target.edge_groups[1], height=0.45)
        eyes.shift(0.5 * SMALL_BUFF * UP)
        bubble = SpeechBubble(height=3, width=5, direction=LEFT)
        bubble.pin_to(network_mob.target.edge_groups[-1])
        bubble.write('Looks like a \\\\ %d to me!' % self.choice)
        self.play(MoveToTarget(network_mob), FadeIn(eyes))
        self.play(eyes.look_at_anim(self.image))
        self.play(ShowCreation(bubble), Write(bubble.content, run_time=1))
        self.play(eyes.blink_anim())
        self.wait()
...
class CannotDraw(PreviewLearning):
    ...
    def construct(self):
        ...
        self.add(eyes)
        bubble = SpeechBubble(height=3, width=4, direction=RIGHT)
        bubble.pin_to(network_mob.edge_groups[0])
        bubble.write("Uh...I'm really \\\\ more of a multiple \\\\ choice guy")
        randy = Randolph()
        randy.to_corner(DOWN + LEFT)
        eyes.look_at_anim(randy.eyes).update(1)
        self.play(PiCreatureSays(randy, 'Draw a \\\\ 5 for me', look_at=eyes, run_time=1))
        self.play(eyes.change_mode_anim('concerned_musician'))
        self.play(ShowCreation(bubble), Write(bubble.content), eyes.look_at_anim(network_mob.get_corner(DOWN + RIGHT)))
        self.play(eyes.blink_anim())
        self.play(Blink(randy))
        self.wait()

class TODOShowCostFunctionDef(TODOStub):
    CONFIG = {'message': 'Insert cost function averaging portion'}

class TODOBreakUpNineByPatterns2(TODOBreakUpNineByPatterns):
    pass

class SomethingToImproveUpon(PiCreatureScene, TestPerformance):
    CONFIG = {'n_examples': 15, 'time_per_example': 0.15}

    def setup(self):
        self.setup_bases()
        self.color_network_edges()
        self.network_mob.to_edge(LEFT)
        edge_update = ContinualEdgeUpdate(self.network_mob, colors=[BLUE, BLUE, RED])
    ...
    def show_path(self):
        ...
        line.shift(UP)
        dots = VGroup(*[Dot(line.point_from_proportion(a)) for a in np.linspace(0, 1, 5)])
        dots.set_color_by_gradient(BLUE, YELLOW)
        path = VGroup(line, dots)
        words = OldTexText('This series')
        words.next_to(line, DOWN)
        self.play(network_mob.scale, 0.25, network_mob.next_to, path.get_right(), UP, ShowCreation(path), Write(words, run_time=1), morty.change, 'sassy')
        self.wait(2)
        self.play(ApplyMethod(network_mob.next_to, path.get_left(), UP, path_arc=np.pi / 2), Rotate(path, np.pi, in_place=True), morty.change, 'raise_right_hand')
        self.wait(3)
        self.line = line
        self.path = path
    ...
    def old_news(self):
        ...
        self.play(Write(words, run_time=1), GrowArrow(arrow), morty.change_mode, 'hesitant')
        self.play(Write(name))
        self.wait()
        self.play(FadeIn(modern_variants), FadeOut(VGroup(words, arrow, name)), morty.change, 'thinking')
        self.wait(2)
    ...
    def hidden_layers(self):
        morty = self.pi_creature
        network_mob = self.network_mob
        self.play(network_mob.scale, 4, network_mob.center, network_mob.to_edge, LEFT, FadeOut(VGroup(self.path, self.modern_variants, self.this_series)), morty.change, 'confused')
...
class ShiftingFocus(Scene):
    ...
    def construct(self):
        how, do, networks, learn = words = OldTexText('How', 'do', 'neural networks', 'learn?')
        ...
        self.play(VGroup(networks, cross).shift, DOWN, Write(viewers, run_time=1))
        self.wait(2)
        self.play(FadeOut(how), Transform(do, cap_do))
        self.wait(2)
...
class PauseAndPonder(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.teacher_says('Pause and \\\\ ponder!', target_mode='hooray', run_time=1)
        self.play(ShowCreation(screen), self.change_students(*['pondering'] * 3))
        self.wait(6)
...
class ConvolutionalNetworkPreview(Scene):
    ...
    def construct(self):
        ...
        self.add(image)
        kernels = [PixelsFromVect(np.zeros(16)) for x in range(2)]
        ...
        for kernel in kernels:
            ...
            kernel.add(SurroundingRectangle(kernel, color=YELLOW, buff=0))
            self.add(kernel)
...
class RandomlyLabeledImageData(Scene):
    CONFIG = {'image_label_pairs': [('lion', 'Lion'), ('Newton', 'Genius'), ('Fork', 'Fork'), ('Trilobite', 'Trilobite'), ('Puppy', 'Puppy'), ('Astrolabe', 'Astrolabe'), ('Adele', 'Songbird of \\\\ our generation'), ('Cow', 'Cow'), ('Sculling', 'Sculling'), ('Pierre_de_Fermat', 'Tease')]}
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, groups, run_time=3, lag_ratio=0.3))
        self.wait()
        self.play(LaggedStartMap(MoveToTarget, labels, run_time=4, lag_ratio=0.5, path_arc=np.pi / 3))
        self.wait()
...
class TrainOnImages(PreviewLearning, RandomlyLabeledImageData):
    CONFIG = {'layer_sizes': [17, 17, 17, 17, 17, 17], 'network_mob_config': {'brace_for_large_layers': False, 'include_output_labels': False}}
    ...
    def construct(self):
        ...
        groups = [self.get_training_group(image_name, label_name) for image_name, label_name in zip(image_names, label_names)]
        ...
        for group in groups:
            ...
            group.to_edge(UP)
            self.add(group)
            self.play(LaggedStartMap(MoveToTarget, edges, lag_ratio=0.4, run_time=2))
            self.remove(group)
...
class AskNetworkAboutMemorizing(YellAtNetwork):
    ...
    def construct(self):
        ...
        eyes.look_at_anim(randy.eyes).update(1)
        self.add(eyes)
        self.pi_creature_says('Are you just \\\\ memorizing?', target_mode='sassy', look_at=eyes, run_time=2)
        self.wait()
        self.play(eyes.change_mode_anim('sad'))
        self.play(eyes.blink_anim())
        self.wait()
...
class CompareLearningCurves(GraphScene):
    CONFIG = {'x_min': 0, 'y_axis_label': 'Value of \\\\ cost function', 'x_axis_label': 'Number of gradient \\\\ descent steps', 'graph_origin': 2 * DOWN + 3.5 * LEFT}
    ...
    def construct(self):
        ...
        slow_decrease = self.get_graph(lambda x: 9 - 0.25 * x)
        faster_decrease = self.get_graph(lambda x: 4.3 * sigmoid(5 * (2 - x)) + 3 + 0.5 * ReLU(3 - x))
        ...
        self.play(ShowCreation(slow_decrease, run_time=12, rate_func=linear))
        self.play(FadeIn(fast_label), ShowCreation(fast_line))
        self.play(ShowCreation(faster_decrease, run_time=12, rate_func=linear))
        self.wait()
        line = Line(self.coords_to_point(1, 2), self.coords_to_point(3, 9))
        rect = Rectangle()
...
class ManyMinimaWords(Scene):
    ...
    def construct(self):
        words = OldTexText('Many local minima,\\\\', 'roughly equal quality')
        words.set_width(FRAME_WIDTH - 1)
        words.to_edge(UP)
        self.play(Write(words))
        self.wait()
