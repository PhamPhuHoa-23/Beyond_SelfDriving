...
class PixelsAsSquares(VGroup):
    ...
    def __init__(self, image_mobject, **kwargs):
        ...
        for row in image_mobject.pixel_array:
            for rgba in row:
                square = Square(stroke_width=0, fill_opacity=rgba[3] / 255.0, fill_color=rgba_to_color(rgba / 255.0))
                self.add(square)
...
class NetworkMobject(VGroup):
    CONFIG = {'neuron_radius': 0.15, 'neuron_to_neuron_buff': MED_SMALL_BUFF, 'layer_to_layer_buff': LARGE_BUFF, 'neuron_stroke_color': BLUE, 'neuron_stroke_width': 3, 'neuron_fill_color': GREEN, 'edge_color': GREY_B, 'edge_stroke_width': 2, 'edge_propogation_color': YELLOW, 'edge_propogation_time': 1, 'max_shown_neurons': 16, 'brace_for_large_layers': True, 'average_shown_activation_of_large_layer': True, 'include_output_labels': False}
    ...
    def get_layer(self, size):
        ...
        neurons = VGroup(*[Circle(radius=self.neuron_radius, stroke_color=self.neuron_stroke_color, stroke_width=self.neuron_stroke_width, fill_color=self.neuron_fill_color, fill_opacity=0) for x in range(n_neurons)])
    ...
    def get_edge(self, neuron1, neuron2):
        return Line(neuron1.get_center(), neuron2.get_center(), buff=self.neuron_radius, stroke_color=self.edge_color, stroke_width=self.edge_stroke_width)
...
class MNistNetworkMobject(NetworkMobject):
    CONFIG = {'neuron_to_neuron_buff': SMALL_BUFF, 'layer_to_layer_buff': 1.5, 'edge_stroke_width': 1, 'include_output_labels': True}
...
class NetworkScene(Scene):
    CONFIG = {'layer_sizes': [8, 6, 6, 4], 'network_mob_config': {}}
...
class ExampleThrees(PiCreatureScene):
    ...
    def show_initial_three(self):
        ...
        self.wait()
        self.play(LaggedStartMap(DrawBorderThenFill, three_mob_copy, run_time=3, stroke_color=WHITE, remover=True), randy.change, 'sassy', *it.chain(list(map(GrowFromCenter, braces)), list(map(FadeIn, brace_labels))))
        self.wait()
        self.play(ShowCreation(bubble), MoveToTarget(three_mob), FadeOut(braces), FadeOut(brace_labels), randy.change, 'pondering')
    ...
    def show_alternate_threes(self):
        ...
        for alt_three in alt_threes:
            self.add(alt_three)
            self.wait(0.5)
        self.play(randy.change, 'plain', *list(map(FadeOut, [self.bubble, self.arrow, self.real_three])) + [MoveToTarget(three)])
        ...
        for moving_three in (three, alt_threes[1]):
            ...
            self.play(Animation(randy), moving_three.replace, randy.eyes[1], moving_three.scale, 0.7, run_time=2, lag_ratio=0.5)
    ...
    def resolve_remaining_threes(self):
        ...
        self.play(randy.change, 'thinking', ShowCreation(self.bubble), MoveToTarget(left_three), MoveToTarget(right_three), Write(equals))
        self.wait()
        self.equals = equals
    ...
    def show_alternate_digits(self):
        ...
        alt_mobs = [WrappedImage(PixelsAsSquares(ImageMobject(layer_to_image_array(arr))), color=GREY_B, buff=0).replace(three) for arr in arrays]
...
class WriteAProgram(Scene):
    ...
    def construct(self):
        ...
        choice.add(rect)
        self.play(choice.scale, 1.5, choice.next_to, arrow, RIGHT, FadeOut(choices), FadeOut(q_mark))
        self.wait(2)
...
class LayOutPlan(TeacherStudentsScene, NetworkScene):
    ...
    def show_words(self):
        words = VGroup(OldTexText('Machine', 'learning').set_color(GREEN), OldTexText('Neural network').set_color(BLUE))
        words.next_to(self.teacher.get_corner(UP + LEFT), UP)
        words[0].save_state()
        words[0].shift(DOWN)
        words[0].fade(1)
        self.play(words[0].restore, self.teacher.change, 'raise_right_hand', self.change_students('pondering', 'erm', 'sassy'))
    ...
    def show_network(self):
        ...
        self.play(ReplacementTransform(VGroup(self.words[1].copy()), network_mob.layers), self.change_students(*['confused'] * 3, lag_ratio=0), self.teacher.change, 'plain', run_time=1)
        self.play(ShowCreation(network_mob.edge_groups, lag_ratio=0.5, run_time=2, rate_func=linear))
        in_vect = np.random.random(self.network.sizes[0])
        self.feed_forward(in_vect)

    def show_math(self):
        equation = OldTex('\\textbf{a}_{l+1}', '=', '\\sigma(', 'W_l', '\\textbf{a}_l', '+', 'b_l', ')')
    ...
    def show_videos(self):
        ...
        self.play(DrawBorderThenFill(videos[0]), FadeIn(structure), self.change_students(*['pondering'] * 3))
        self.wait()
...
class PreviewMNistNetwork(NetworkScene):
    ...
    def feed_in_image(self, in_vect):
        ...
        self.feed_forward(in_vect, added_anims=[self.get_image_to_layer_one_animation(image, start_neurons)])
        n = np.argmax([neuron.get_fill_opacity() for neuron in self.network_mob.layers[-1].neurons])
        rect = SurroundingRectangle(VGroup(self.network_mob.layers[-1].neurons[n], self.network_mob.output_labels[n]))
        self.play(ShowCreation(rect))
        self.reset_display(rect, image, image_rect)

    def reset_display(self, answer_rect, image, image_rect):
        self.play(FadeOut(answer_rect))
        self.play(FadeOut(image), FadeOut(image_rect), self.network_mob.deactivate_layers)
...
class AlternateNeuralNetworks(PiCreatureScene):
    ...
    def construct(self):
        ...
        examples = VGroup(VGroup(OldTexText('Convolutional neural network'), OldTexText('Good for image recognition')), VGroup(OldTexText('Long short-term memory network'), OldTexText('Good for speech recognition')))
        for ex in examples:
            arrow = Arrow(LEFT, RIGHT, color=BLUE)
        ...
        self.wait()
        self.play(examples[0].shift, MED_LARGE_BUFF * UP, FadeIn(examples[1], lag_ratio=0.5))
        self.wait()
...
class BreakDownName(Scene):
    ...
    def ask_questions(self):
        ...
        self.play(brain.set_height, 2, brain.shift, 2 * UP, brain.set_fill, None, 1, randy.look, UP)
        brain_outline = brain.copy()
        brain_outline.set_fill(opacity=0)
        brain_outline.set_stroke(BLUE_B, 3)
        self.play(ShowPassingFlash(brain_outline, time_width=0.5, run_time=2))
        ...
        self.wait()
        self.play(Write(q1, run_time=1), ShowCreation(a1), name[0].set_color, q1.get_color())
...
class IntroduceEachLayer(PreviewMNistNetwork):
    CONFIG = {'network_mob_config': {'neuron_stroke_color': WHITE, 'neuron_stroke_width': 2, 'neuron_fill_color': WHITE, 'average_shown_activation_of_large_layer': False, 'edge_propogation_color': YELLOW, 'edge_propogation_time': 2}}
    ...
    def break_up_image_as_neurons(self):
        ...
        self.wait()
        self.play(ReplacementTransform(image_mob, neurons), FadeOut(rect), FadeIn(braces), FadeIn(labels))
        self.wait()
        self.play(ReplacementTransform(labels[0].copy(), equation[0]), Write(equation[1]), ReplacementTransform(labels[1].copy(), equation[2]), Write(equation[3]), Write(equation[4]))
        self.wait()
    ...
    def show_activation_of_one_neuron(self):
        ...
        def change_activation(num):
            self.play(example_neuron.set_fill, None, num, ChangingDecimal(example_num, lambda a: example_neuron.get_fill_opacity()), UpdateFromFunc(example_num, lambda m: m.set_fill(BLACK if example_neuron.get_fill_opacity() > 0.8 else WHITE)))
        ...
        self.wait()
        self.play(example_neuron.restore, example_num.restore, FadeOut(activation), FadeOut(rect))
    ...
    def transform_into_full_network(self):
        ...
        self.play(FadeOut(self.braces), FadeOut(self.brace_labels), FadeOut(VGroup(*self.num_pixels_equation[:-1])))
        self.play(rows.space_out_submobjects, 1.2)
        self.play(rows.arrange, RIGHT, buff=SMALL_BUFF, path_arc=np.pi / 2, run_time=2)
        self.play(ReplacementTransform(VGroup(*neurons[:n]), VGroup(*layer.neurons[:n])), ReplacementTransform(VGroup(*neurons[n:-n]), layer.dots), ReplacementTransform(VGroup(*neurons[-n:]), VGroup(*layer.neurons[-n:])))
        self.play(ReplacementTransform(self.num_pixels_equation[-1], layer.brace_label), FadeIn(layer.brace))
        self.play(layer.restore, FadeIn(self.corner_image))
        self.wait()
        for edge_group, layer in zip(network_mob.edge_groups, network_mob.layers[1:]):
            self.play(LaggedStartMap(FadeIn, layer, run_time=1), ShowCreation(edge_group))
        self.wait()
    ...
    def show_output_layer(self):
        ...
        def change_activation(num):
            self.play(neuron.set_fill, None, num, ChangingDecimal(activation, lambda a: neuron.get_fill_opacity()), UpdateFromFunc(activation, lambda m: m.set_fill(BLACK if neuron.get_fill_opacity() > 0.8 else WHITE)))
        self.play(ShowCreation(rect))
        ...
        for num in (0.5, 0.38, 0.97):
            change_activation(num)
            self.wait()
        self.play(neuron.restore, neuron.set_fill, None, 1, label.restore, FadeOut(activation), FadeOut(rect))
        self.wait()
...
class DiscussChoiceForHiddenLayers(TeacherStudentsScene):
    ...
    def construct(self):
        network_mob = MNistNetworkMobject(layer_to_layer_buff=2.5, neuron_stroke_color=WHITE)
        ...
        neurons_anim = LaggedStartMap(Indicate, VGroup(*it.chain(*[layer.neurons for layer in layers])), rate_func=there_and_back, scale_factor=2, color=MAROON_B)
        self.play(ShowCreation(rects), Write(two_words, run_time=1), self.teacher.change, 'raise_right_hand')
        self.wait()
        ...
        self.wait()
        self.student_says('Why 2 \\\\ layers?', index=1, bubble_config={'direction': RIGHT}, run_time=1, target_mode='raise_left_hand')
        self.play(self.teacher.change, 'happy')
        self.wait()
        self.student_says('Why 16?', index=0, run_time=1)
        self.play(neurons_anim, run_time=3)
        self.play(self.teacher.change, 'shruggie', RemovePiCreatureBubble(self.students[0]))
        self.wait()
...
class MoreHonestMNistNetworkPreview(IntroduceEachLayer):
    ...
    def get_image_to_layer_one_animation(self, image, start_neurons):
        neurons = VGroup()
        for pixel in image:
            neuron = Circle(radius=pixel.get_width() / 2, stroke_width=1, stroke_color=WHITE, fill_color=WHITE, fill_opacity=pixel.fill_rgb[0])
            neuron.move_to(pixel)
            neurons.add(neuron)
        neurons.scale(1.2)
        ...
        target = VGroup(*it.chain(start_neurons[:n / 2], [point.copy() for x in range(len(neurons) - n)], start_neurons[n / 2:]))
        mover = image.copy()
...
class AskAboutLayers(PreviewMNistNetwork):
    ...
    def construct(self):
        ...
        self.play(Write(question, run_time=1), LaggedStartMap(GrowFromPoint, arrows, lambda a: (a, a.get_start()), run_time=2))
        self.wait()
        self.play(*list(map(ShowCreation, rects)))
        self.wait()

class BreakUpMacroPatterns(IntroduceEachLayer):
    CONFIG = {'camera_config': {'background_opacity': 1}, 'prefixes': ['nine', 'eight', 'four', 'upper_loop', 'right_line', 'lower_loop', 'horizontal_line', 'upper_left_line']}
    ...
    def setup_needed_patterns(self):
        ...
        vects = [np.array(Image.open(get_full_raster_image_path('handwritten_' + p)))[:, :, 0].flatten() / 255.0 for p in prefixes]
    ...
    def show_second_to_last_layer(self):
        everything = VGroup(*it.chain(self.nine_equation, self.eight_equation, self.four_equation))
        patterns = VGroup(self.upper_loop, self.lower_loop, self.right_line, self.upper_left_line, self.horizontal_line, *self.added_patterns[:11])
        ...
        self.play(FadeIn(network_mob, lag_ratio=0.5, run_time=3), MoveToTarget(patterns))
        self.wait(2)
    ...
    def show_upper_loop_activation(self):
        ...
        self.remove_random_edges(0.7)
        self.network.get_activation_of_all_layers = lambda v: [np.zeros(784), sigmoid(6 * (np.random.random(16) - 0.5)), np.array([1, 0, 1] + 13 * [0]), np.array(9 * [0] + [1])]
        self.play(FadeIn(nine))
    ...
    def get_equation(self, *mobs):
        equation = VGroup(mobs[0], OldTex('=').scale(2), *list(it.chain(*[[m, OldTex('+').scale(2)] for m in mobs[1:-1]])) + [mobs[-1]])
...
class BreakUpMicroPatterns(BreakUpMacroPatterns):
    CONFIG = {'prefixes': ['loop', 'loop_edge1', 'loop_edge2', 'loop_edge3', 'loop_edge4', 'loop_edge5', 'right_line', 'right_line_edge1', 'right_line_edge2', 'right_line_edge3']}
    ...
    def break_down_loop(self):
        ...
        self.wait()
        self.play(MoveToTarget(loop, run_time=2), MoveToTarget(edges, run_time=2), Write(symbols), randy.change, 'happy', equation)
        self.wait()
    ...
    def break_down_long_line(self):
        ...
        self.wait()
        self.play(LaggedStartMap(FadeIn, Group(*equation[1:])), randy.change, 'pondering', equation)
        self.wait(3)

class SecondLayerIsLittleEdgeLayer(IntroduceEachLayer):
    CONFIG = {'camera_config': {'background_opacity': 1}, 'network_mob_config': {'layer_to_layer_buff': 2, 'edge_propogation_color': YELLOW}}
    ...
    def setup_activations_and_nines(self):
        ...
        nine_im, loop_im, line_im = images = [Image.open(get_full_raster_image_path('handwritten_%s' % s)) for s in ('nine', 'upper_loop', 'right_line')]
    ...
    def show_propogation(self):
        ...
        neurons = VGroup()
        for pixel in v_nine:
            neuron = Circle(radius=pixel.get_width() / 2, stroke_color=WHITE, stroke_width=1, fill_color=WHITE, fill_opacity=pixel.get_fill_opacity())
        ...
        def activate_layer(i):
            self.play(ShowCreationThenDestruction(edge_groups[i - 1], run_time=2, lag_ratio=0.5), FadeIn(active_layers[i]))
        self.play(FadeIn(nine))
    ...
    def separate_parts(self, image_group):
        ...
        self.play(MoveToTarget(image_group, rate_func=there_and_back, lag_ratio=0.5, run_time=2))
...
class ManyTasksBreakDownLikeThis(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.bubble.content.fade, 1, ShowCreation(audio), pre_word.shift, MED_SMALL_BUFF, DOWN)
        self.wait(2)
        self.play(RemovePiCreatureBubble(self.teacher), audio.restore, FadeIn(audio_label), *[ReplacementTransform(m1, m2) for m1, m2 in zip(pre_word, letters)])
        self.play(GrowFromPoint(arrows[0], arrows[0].get_start()))
        self.wait()
        self.play(GrowFromPoint(arrows[1], arrows[1].get_start()), LaggedStartMap(FadeIn, syllables, run_time=1))
        self.wait()
        self.play(GrowFromPoint(arrows[2], arrows[2].get_start()), LaggedStartMap(FadeIn, word, run_time=1))
        self.wait()
...
class IntroduceWeights(IntroduceEachLayer):
    ...
    def zoom_in_on_one_neuron(self):
        ...
        self.play(FadeOut(edge_groups), FadeOut(VGroup(*layers[1:])), FadeOut(self.network_mob.output_labels), Animation(neuron), neuron.edges_in.set_stroke, None, 2, lag_ratio=0.5, run_time=2)
        self.neuron = neuron
    ...
    def ask_about_parameters(self):
        ...
        arrow = Arrow(parameter_word.get_bottom(), neuron.edges_in[0].get_center(), color=self.weights_color)
        p_labels = VGroup(*[OldTex('p_%d\\!:' % (i + 1)).set_color(self.weights_color) for i in range(8)] + [Tex('\\vdots')])
        ...
        self.play(Write(question, run_time=2), GrowFromPoint(arrow, arrow.get_start()), pixels_group.set_height, 3, pixels_group.to_edge, RIGHT, LaggedStartMap(FadeIn, p_labels), LaggedStartMap(FadeIn, decimals))
        self.wait()
    ...
    def show_weights(self):
        ...
        self.play(FadeOut(question), ReplacementTransform(parameter_word, weights_word), ReplacementTransform(p_labels, w_labels))
        self.play(MoveToTarget(edges), *[ChangingDecimal(decimal, get_alpha_func(r)) for decimal, r in zip(decimals, random_numbers)])
        self.play(LaggedStartMap(ApplyMethod, edges, lambda m: (m.rotate, np.pi / 24), rate_func=wiggle, run_time=2))
        self.wait()
    ...
    def show_weighted_sum(self):
        ...
        weighted_sum.add(OldTex('\\cdots'), OldTex('+'), OldTex('w_n').set_color(self.weights_color), OldTex('a_n'))
        weighted_sum.arrange(RIGHT, buff=SMALL_BUFF)
        weighted_sum.to_edge(UP)
        self.play(Transform(layer, active_layer))
        self.play(FadeOut(weights_word), FadeOut(weight_arrow), *[ReplacementTransform(n.copy(), a) for n, a in zip(layer.neurons, a_labels)] + [ReplacementTransform(n.copy(), weighted_sum[-4]) for n in layer.neurons[4:-1]] + [ReplacementTransform(layer.neurons[-1].copy(), weighted_sum[-1])] + [Write(weighted_sum[i]) for i in list(range(2, 12, 3)) + [-4, -3]], run_time=1.5)
        self.wait()
        self.play(*[ReplacementTransform(w1.copy(), w2) for w1, w2 in zip(self.w_labels, w_labels)[:4]] + [ReplacementTransform(w.copy(), weighted_sum[-4]) for w in self.w_labels[4:-1]] + [ReplacementTransform(self.w_labels[-1].copy(), weighted_sum[-2])], run_time=2)
        self.wait(2)
    ...
    def organize_weights_as_grid(self):
        ...
        self.play(FadeIn(VGroup(*weight_grid[len(decimals):]), lag_ratio=0.5, run_time=3), *[ReplacementTransform(decimal, pixel) for decimal, pixel in zip(decimals, weight_grid)])
        self.wait()
    ...
    def make_most_weights_0(self):
        ...
        self.wait()
        self.play(ReplacementTransform(self.pixels_to_detect.copy(), self.weighted_sum, run_time=3, lag_ratio=0.5), Animation(weight_grid))
        self.wait()
    ...
    def add_negative_weights_around_the_edge(self):
        ...
        self.play(*[ApplyMethod(weight_grid[28 * y + x].set_fill, self.negative_weights_color, 0.5) for y in (6, 10) for x in range(14 - 4, 14 + 4)])
        self.wait(2)
        self.play(weight_grid.move_to, pixels)
        self.wait(2)
...
class MotivateSquishing(Scene):
    ...
    def add_weighted_sum(self):
        weighted_sum = OldTex(*it.chain(*[['w_%d' % d, 'a_%d' % d, '+'] for d in range(1, 5)] + [['\\cdots', '+', 'w_n', 'a_n']]))
    ...
    def show_real_number_line(self):
        ...
        arrow1, arrow2 = [Arrow(weighted_sum.get_bottom(), number_line.number_to_point(n)) for n in (-3, 3)]
    ...
    def show_interval(self):
        ...
        interval = Line(lower_number_line.number_to_point(0), lower_number_line.number_to_point(1), color=YELLOW, stroke_width=5)
...
class IntroduceSigmoid(GraphScene):
    CONFIG = {'x_min': -5, 'x_max': 5, 'x_axis_width': 12, 'y_min': -1, 'y_max': 2, 'y_axis_label': '', 'graph_origin': DOWN, 'x_labeled_nums': list(range(-4, 5)), 'y_labeled_nums': list(range(-1, 3))}
    ...
    def show_part(self, x_min, x_max, color):
        line, graph_part = [self.get_graph(func, x_min=x_min, x_max=x_max, color=color).set_stroke(width=4) for func in (lambda x: 0, sigmoid)]
...
class IncludeBias(IntroduceWeights):
    ...
    def setup_start(self):
        ...
        Transform(self.network_mob.layers[0], self.network_mob.get_active_layer(0, np.random.random(16))).update(1)
    ...
    def add_sigmoid_label(self):
        ...
        arrow = Arrow(name.get_bottom(), sigma.get_top(), buff=SMALL_BUFF, use_rectangular_stem=False, max_tip_length_to_length_ratio=0.3)
    ...
    def comment_on_need_for_bias(self):
        neuron = self.neuron
        weight_grid = self.weight_grid
        colored_pixels = VGroup(self.get_pixels_to_detect(weight_grid), self.get_surrounding_pixels_for_edge(weight_grid))
        ...
        self.play(Write(words, run_time=2))
        self.play(ApplyMethod(colored_pixels.shift, MED_LARGE_BUFF * UP, rate_func=there_and_back, run_time=2, lag_ratio=0.5))
        self.wait()
        self.gt_ten = words[-1]
    ...
    def add_bias(self):
        ...
        self.play(ReplacementTransform(self.gt_ten.copy(), bias, run_time=2), MoveToTarget(rp))
        self.wait(2)
        self.play(ShowCreation(rect), Write(name))
        self.wait(2)
        self.bias_name = name
    ...
    def summarize_weights_and_biases(self):
        ...
        self.play(LaggedStartMap(ApplyMethod, weight_grid, lambda p: (p.set_fill, random.choice([GREEN, GREEN, RED]), random.random()), rate_func=there_and_back, lag_ratio=0.4, run_time=4))
        self.wait()
...
class ContinualEdgeUpdate(VGroup):
    CONFIG = {'max_stroke_width': 3, 'stroke_width_exp': 7, 'n_cycles': 5, 'colors': [GREEN, GREEN, GREEN, RED]}
...
class ShowRemainingNetwork(IntroduceWeights):
    ...
    def show_all_of_second_layer(self):
        ...
        for neuron in neurons[:7]:
            ...
            self.play(ShowCreation(edges, lag_ratio=0.5), FadeIn(neuron), *added_anims, run_time=1.5)
            last_edges = edges
        self.play(LaggedStartMap(ShowCreation, VGroup(*[n.edges_in for n in neurons[7:]]), run_time=3), LaggedStartMap(FadeIn, VGroup(*neurons[7:]), run_time=3), VGroup(*last_edges[1:]).set_stroke, None, 1)
        self.wait()
    ...
    def count_in_biases(self):
        ...
        self.play(FadeIn(words), LaggedStartMap(GrowArrow, arrows, run_time=3, lag_ratio=0.3))
        self.wait()
    ...
    def compute_layer_two_of_weights_and_biases_count(self):
        ...
        self.wait()
        self.play(ReplacementTransform(times_16.copy(), bias_count[0]), FadeOut(bb1), ReplacementTransform(bb2, bias_count[1]), FadeOut(bb3), LaggedStartMap(FadeOut, bias_arrows))
        self.wait()
    ...
    def show_remaining_layers(self):
        ...
        self.play(MoveToTarget(weights_count), MoveToTarget(bias_count), Write(added_weights, run_time=1), Write(added_biases, run_time=1), LaggedStartMap(ShowCreation, edges, run_time=4, lag_ratio=0.3), LaggedStartMap(FadeIn, neurons, run_time=4, lag_ratio=0.3))
        self.wait(2)
    ...
    def get_edge_weight_wandering_anim(self, edges):
        for edge in edges:
            edge.generate_target()
            edge.target.set_stroke(color=random.choice([GREEN, GREEN, GREEN, RED]), width=3 * random.random() ** 7)
        self.play(LaggedStartMap(MoveToTarget, edges, lag_ratio=0.6, run_time=2), *added_anims)
...
class WhenTheNetworkFails(MoreHonestMNistNetworkPreview):
    ...
    def black_box(self):
        network_mob = self.network_mob
        layers = VGroup(*network_mob.layers[1:3])
        box = SurroundingRectangle(layers, stroke_color=WHITE, fill_color=BLACK, fill_opacity=0.8)
...
class IntroduceWeightMatrix(NetworkScene):
    CONFIG = {'network_mob_config': {'neuron_stroke_color': WHITE, 'neuron_fill_color': WHITE, 'neuron_radius': 0.35, 'layer_to_layer_buff': 2}, 'layer_sizes': [8, 6]}
    ...
    def fade_many_neurons(self):
        ...
        for neuron in neurons[1:]:
            ...
            anims += [neuron.fade, 0.8, neuron.set_fill, None, 0, neuron.edges_in.fade, 0.8]
    ...
    def show_first_neuron_weighted_sum(self):
        ...
        for a_label, w_label in zip(a_labels.target, w_labels):
            ...
            symbols.add(plus)
        weighted_sum.add(OldTex('\\cdots'), OldTex('+'), OldTex('w_{0, n}'), OldTex('a^{(0)}_n'))
    ...
    def add_sigmoid(self):
        ...
        self.play(FadeIn(mob), MoveToTarget(weighted_sum), MaintainPositionRelativeTo(self.bias_name, self.bias))
    ...
    def organize_weights_as_matrix(self):
        ...
        lwb.align_to(rwb, UP)
        row_1, row_k = [VGroup(*list(map(Tex, ['w_{%s, 0}' % i, 'w_{%s, 1}' % i, '\\cdots', 'w_{%s, n}' % i]))) for i in ('1', 'k')]
    ...
    def show_meaning_of_lower_rows(self, arrow, brace, row_rect, result_terms):
        ...
        def show_edges(neuron):
            self.play(LaggedStartMap(ShowCreationThenDestruction, neuron.edges_in.copy().set_stroke(GREEN, 5), lag_ratio=0.7, run_time=1))
        self.play(row_rect.move_to, row2, n1.fade, n1.set_fill, None, 0, n1.edges_in.set_stroke, None, 1, n2.set_stroke, WHITE, 3, n2.edges_in.set_stroke, None, 3, ReplacementTransform(arrow, rect2), FadeOut(brace))
        show_edges(n2)
        self.play(row_rect.move_to, rowk, n2.restore, n2.edges_in.restore, nk.set_stroke, WHITE, 3, nk.edges_in.set_stroke, None, 3, ReplacementTransform(rect2, rectk))
        show_edges(nk)
        self.play(n1.restore, n1.edges_in.restore, nk.restore, nk.edges_in.restore, FadeOut(rectk), FadeOut(row_rect))
    ...
    def add_bias_vector(self):
        ...
        b_column = VGroup(*list(map(Tex, ['b_0', 'b_1', '\\vdots', 'b_n'])))
        b_column.scale(0.85)
        ...
        self.play(Write(plus), Write(b_brackets), Transform(self.bias[1].copy(), b_column[0]), run_time=1)
    ...
    def apply_sigmoid(self):
        ...
        self.play(sigma.scale, 2, sigma.next_to, big_lp, LEFT, SMALL_BUFF, Transform(slp, big_lp), Transform(srp, big_rp))
        self.wait(2)
        self.big_sigma_group = VGroup(VGroup(sigma), slp, srp)

    def write_clean_final_expression(self):
        self.fade_weighted_sum()
        expression = OldTex('\\textbf{a}^{(1)}', '=', '\\sigma', '\\big(', '\\textbf{W}', '\\textbf{a}^{(0)}', '+', '\\textbf{b}', '\\big)')
        ...
        self.play(ReplacementTransform(VGroup(self.top_matrix_row, self.lower_matrix_rows, self.matrix_brackets).copy(), VGroup(W)))
        self.play(ReplacementTransform(VGroup(self.a_column, self.a_column_brackets).copy(), VGroup(VGroup(a0))))
        self.play(ReplacementTransform(VGroup(self.b_column, self.b_brackets).copy(), VGroup(VGroup(b))), ReplacementTransform(self.bias_plus.copy(), plus))
    ...
    def fade_weighted_sum(self):
        self.play(*list(map(FadeOut, [self.a1_label, self.a1_equals, self.sigma, self.sigma_parens, self.weighted_sum, self.bias_name, self.sigmoid_name])))
...
class SigmoidAppliedToVector(Scene):
    ...
    def construct(self):
        ...
        tex.to_edge(DOWN)
        indices = it.chain([0], list(range(1, 5)), list(range(16, 16 + 4)), list(range(25, 25 + 2)), [25 + 3], list(range(29, 29 + 2)), [29 + 3], list(range(33, 33 + 2)), [33 + 3])
        for i in indices:
...
class NeuronIsFunction(MoreHonestMNistNetworkPreview):
    ...
    def write_neuron_holds_a_number(self):
        ...
        self.play(neuron_group.restore, neuron_group.scale, 3, neuron_group.next_to, neuron_word, LEFT, FadeIn(neuron_word), GrowArrow(arrow), FadeIn(thing_words, run_time=2, rate_func=squish_rate_func(smooth, 0.3, 1)))
        self.wait()
    ...
    def show_neuron_as_function(self):
        ...
        self.play(FadeOut(self.network_mob), *list(map(Animation, [neuron, edges, prev_layer])))
        self.play(LaggedStartMap(ShowCreationThenDestruction, edges.copy().set_stroke(YELLOW, 4)))
    ...
    def fade_network_back_in(self):
        anims = [FadeIn(mob, run_time=2, lag_ratio=0.5) for mob in (self.network_mob.layers, self.network_mob.edge_groups)]
    ...
    def network_is_a_function(self):
        ...
        func_tex = OldTex('f(a_0, \\dots, a_{783}) = ', '\\left[\n                \\begin{array}{c} \n                    y_0 \\\\ \\vdots \\\\ y_{9} \n                \\end{array}\n            \\right]')
        func_tex.to_edge(UP)
...
class ComplicationIsReassuring(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('It kind of has to \\\\ be complicated, right?', target_mode='speaking', index=0)
        self.play(self.teacher.change, 'happy')
        self.wait(4)

class NextVideo(MoreHonestMNistNetworkPreview, PiCreatureScene):
    CONFIG = {'network_mob_config': {'neuron_stroke_color': WHITE, 'layer_to_layer_buff': 2.5, 'brace_for_large_layers': False}}
    ...
    def show_next_video(self):
        ...
        self.play(content.set_height, 0.8 * video.get_height(), content.move_to, video, morty.change, 'raise_right_hand', FadeIn(rect), FadeIn(video))
    ...
    def talk_about_subscription(self):
        ...
        self.play(Write(subscribe_word), self.video.shift, 3 * LEFT, MoveToTarget(morty))
    ...
    def show_video_neural_network(self):
        ...
        self.play(MoveToTarget(network_mob), MoveToTarget(video), FadeOut(words), FadeOut(rect), morty.change, 'raise_left_hand')
...
class IntroduceReLU(IntroduceSigmoid):
    ...
    def old_school(self):
        ...
        self.wait(2)
        self.play(ApplyMethod(VGroup(cross, sigmoid_title).shift, FRAME_X_RADIUS * RIGHT, rate_func=running_start), FadeOut(old_school), FadeOut(arrow))
        self.play(ShowCreation(self.sigmoid_graph, rate_func=lambda t: smooth(1 - t), remover=True))

    def show_ReLU(self):
        graph = VGroup(Line(self.coords_to_point(-7, 0), self.coords_to_point(0, 0)), Line(self.coords_to_point(0, 0), self.coords_to_point(4, 4)))
...
class CompareSigmoidReLUOnDeepNetworks(PiCreatureScene):
    ...
    def construct(self):
        ...
        relu_graph = VGroup(Line(2 * LEFT, ORIGIN), Line(ORIGIN, np.sqrt(2) * (RIGHT + UP)))
        ...
        self.play(FadeIn(sigmoid_name), ShowCreation(sigmoid_graph), lisha.change, 'raise_left_hand', morty.change, 'pondering')
        self.play(Write(slow_learner, run_time=1), GrowArrow(slow_arrow))
        self.wait()
        self.play(FadeIn(relu_name), ShowCreation(relu_graph), lisha.change, 'raise_right_hand', morty.change, 'thinking')
...
class Thumbnail(NetworkScene):
    CONFIG = {'network_mob_config': {'neuron_stroke_color': WHITE, 'layer_to_layer_buff': 1.25}}
    ...
    def construct(self):
        ...
        subtitle = OldTexText('From the\\\\', 'ground up\\\\')
        subtitle.set_color(YELLOW)
        subtitle.set_height(2.75)
        subtitle.next_to(network_mob, RIGHT, buff=MED_LARGE_BUFF)
        edge_update = ContinualEdgeUpdate(network_mob, max_stroke_width=10, stroke_width_exp=4)
