...
class IntroducePutnam(Scene):
    ...
    def construct(self):
        ...
        question_groups = VGroup(*[VGroup(*[OldTexText('%s%d)' % (c, i)) for i in range(1, 7)]).arrange(DOWN, buff=MED_LARGE_BUFF) for c in ('A', 'B')]).arrange(RIGHT, buff=FRAME_X_RADIUS - MED_SMALL_BUFF)
        ...
        difficulty_strings = ['Pretty hard', 'Hard', 'Harder', 'Very hard', 'Ughhh', 'Can I go home?']
        ...
        self.play(LaggedStartMap(GrowFromCenter, flat_questions, run_time=3))
        self.play(ReplacementTransform(six_hours, three_hours), ReplacementTransform(six_hours.copy(), three_hours_copy), *list(map(ShowCreation, rects)))
        self.wait()
        self.play(LaggedStartMap(DrawBorderThenFill, out_of_tens, run_time=3, stroke_color=YELLOW))
        self.wait()
        self.play(ReplacementTransform(out_of_tens.copy(), VGroup(out_of_120), lag_ratio=0.5, run_time=2))
        self.wait()
        self.play(title.next_to, median_words.copy(), LEFT, LARGE_BUFF, MoveToTarget(out_of_120), Write(median_words))
...
class NatureOf5sAnd6s(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(test.scale, self.test_scale_val, test.to_corner, UP + LEFT, FadeIn(self.teacher), self.change_students(*['horrified'] * 3, look_at=test))
        self.wait()
        mover = VGroup(test.questions[-1].copy(), test.difficulties[-1].copy())
        mover.generate_target()
        mover.target.scale(1.0 / self.test_scale_val)
        mover.target.next_to(self.teacher.get_corner(UP + LEFT), UP)
        ...
        self.play(MoveToTarget(mover), self.teacher.change, 'raise_right_hand')
...
class OtherVideoClips(Scene):
    ...
    def construct(self):
        ...
        rect.center()
        rect.to_edge(DOWN)
        titles = list(map(TexText, ['Essence of calculus, chapter 1', 'Pi hiding in prime regularities', 'How do cryptocurrencies work?']))
        self.add(rect)
        last_title = None
        for title in titles:
            title.to_edge(UP, buff=MED_SMALL_BUFF)
...
class HowDoYouStart(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('How do you even start?', target_mode='raise_left_hand')
...
class TwoDCase(Scene):
    CONFIG = {'center': ORIGIN, 'random_seed': 4, 'radius': 2.5, 'center_color': BLUE, 'point_color': YELLOW, 'positive_triangle_color': BLUE, 'negative_triangle_color': RED, 'triangle_fill_opacity': 0.25, 'n_initial_random_choices': 9, 'n_p3_random_moves': 4}
    ...
    def add_circle(self):
        ...
        self.play(ShowCreation(circle), Rotating(radius, angle=2 * np.pi, about_point=self.center), rate_func=smooth, run_time=2)
        self.play(ShowCreation(radius, rate_func=lambda t: smooth(1 - t), remover=True))
        self.wait()
    ...
    def choose_three_random_points(self):
        ...
        self.update_animations = [self.triangle_update, self.point_labels_update]
        for anim in self.update_animations:
            anim.update(0)
        question = OldTexText('Probability that \\\\ this triangle \\\\', 'contains the center', '?', arg_separator='')
    ...
    def simplify_further(self):
        ...
        self.play(morty.change, 'hooray', ShowCreation(bubble), Write(bubble.content))
        self.play(Blink(morty))
        self.wait()
        self.play(morty.change, 'happy', morty.fade, 1, *list(map(FadeOut, [bubble, bubble.content])))
        self.remove(morty)
    ...
    def fix_two_points_in_place(self):
        ...
        self.play(LaggedStartMap(ApplyMethod, push_pins, lambda mob: (mob.restore,)))
    ...
    def note_special_region(self):
        ...
        self.change_point_mobs([0, 0, arc.angle], ShowCreation(arc, run_time=2))
    ...
    def draw_lines_through_center(self):
        ...
        self.wait()
        self.play(all_arcs.space_out_submobjects, 1.5, Animation(point_mobs), rate_func=there_and_back, run_time=1.5)
        self.wait()
    ...
    def ask_about_probability_p3_lands_in_this_arc(self):
        ...
        answer = OldTex('{\\text{Length of arc}', '\\over', '\\text{Circumference}}')
        ...
        self.play(question.next_to, answer, UP, LARGE_BUFF, Write(equals), FadeIn(answer))
        self.have_p3_jump_around_randomly(4)
        angles = self.get_point_mob_angles()
        self.change_point_mobs([0, 0, 1.35 * np.pi - angles[2]], run_time=0)
        self.wait()
        question.add(equals)
    ...
    def various_arc_sizes_for_p1_p2_placements(self):
        ...
        self.play(*list(map(FadeOut, [self.push_pins, self.triangle, self.arc_lines])))
        self.update_animations.remove(self.triangle_update)
        self.update_animations += [self.get_center_lines_update(self.point_mobs, self.center_lines), self.get_arcs_update(self.all_arcs)]
        self.change_point_mobs_to_angles([np.pi / 2, np.pi], run_time=1)
        elbow = VGroup(Line(DOWN, DOWN + RIGHT), Line(DOWN + RIGHT, RIGHT))
        elbow.scale(0.25)
        ...
        self.wait()
        self.play(ApplyMethod(arc.rotate, np.pi / 12, rate_func=wiggle))
        self.play(LaggedStartMap(FadeIn, proportion, run_time=1))
        self.wait()
        angle_pairs = [(0.26 * np.pi, 1.24 * np.pi), (0.73 * np.pi, 0.78 * np.pi), (0.5 * np.pi, np.pi)]
        self.update_animations.append(proportion_update)
        for angle_pair in angle_pairs:
            self.change_point_mobs_to_angles(angle_pair, VGroup(elbow, ninety_degrees).fade, 1)
    ...
    def fix_p1_in_place(self):
        ...
        for angle in [0.89999 * np.pi, -0.09999 * np.pi, 0.4 * np.pi]:
            self.change_point_mobs_to_angles([0.9 * np.pi, angle], run_time=4)
    ...
    def overall_probability(self):
        ...
        self.play(ReplacementTransform(self.proportion.copy(), VGroup(one_fourth)))
        self.have_p3_jump_around_randomly(32, wait_time=0.25)

    def get_point_mobs(self):
        points = np.array([self.center + rotate_vector(self.radius * RIGHT, theta) for theta in 2 * np.pi * np.random.random(3)])
        for index in (0, 1, 0):
            ...
            points[index] *= -1
            points[index] += self.center
        point_mobs = self.point_mobs = VGroup(*[Dot().move_to(point) for point in points])
        point_mobs.set_color(self.point_color)
        return point_mobs

    def get_point_mob_labels(self):
        point_labels = VGroup(*[OldTex('P_%d' % (i + 1)) for i in range(len(self.point_mobs))])
    ...
    def get_center_lines_update(self, point_mobs, center_lines):
        ...
        def update_lines(center_lines):
            for point_mob, line in zip(point_mobs, center_lines):
                ...
                line.rotate(angle_of_vector(point) - line.get_angle())
    ...
    def get_all_arcs(self):
        ...
        for da0, da1 in it.product(*[[0, np.pi]] * 2):
            ...
            arc = Arc(start_angle=angles[0] + da0, angle=arc_angle, radius=self.radius, stroke_width=5)
    ...
    def get_point_mob_theta_change_anim(self, point_mob, d_theta):
        ...
        def update_point(point_mob, alpha):
            ...
            point_mob.move_to(self.center + self.radius * (np.cos(theta) * RIGHT + np.sin(theta) * UP))
            return point_mob
        return UpdateFromAlphaFunc(point_mob, update_point, run_time=2)

    def change_point_mobs(self, d_thetas, *added_anims, **kwargs):
        anims = it.chain(self.update_animations, [self.get_point_mob_theta_change_anim(pm, dt) for pm, dt in zip(self.point_mobs, d_thetas)], added_anims)
    ...
    def change_point_mobs_to_angles(self, target_angles, *added_anims, **kwargs):
        ...
        self.change_point_mobs([ta - a for a, ta in zip(angles, target_angles)], *added_anims, **kwargs)
    ...
    def have_p3_jump_around_randomly(self, n_jumps, wait_time=0.75, run_time=0):
        for x in range(n_jumps):
            self.change_point_mobs([0, 0, 2 * np.pi * random.random()], run_time=run_time)
            self.wait(wait_time)
...
class AverageSizeOfSphericalTriangleSectionSupplement(Scene):
    ...
    def construct(self):
        words = OldTexText('Average size of \\\\', 'this section', '?', arg_separator='')
...
class TryASurfaceIntegral(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.teacher_says('I mean...you can \\emph{try}', target_mode='sassy')
        self.wait(2)

class RevisitTwoDCase(TwoDCase):
    CONFIG = {'random_seed': 4, 'center': 3 * LEFT + 0.5 * DOWN, 'radius': 2, 'n_random_trials': 200}
    ...
    def show_probability(self):
        title = OldTex('P(\\text{triangle contains the center})', '=', '1/4')
        title.to_edge(UP, buff=MED_SMALL_BUFF)
        ...
        self.n_in = 0
        self.n_out = 0
        frac = OldTex('{0', '\\over', '\\quad 0', '+', '0 \\quad}', '=')
        ...
        def place_random_triangles(n, wait_time):
            for x in range(n):
                ...
                contain_center = self.points_contain_center([pm.get_center() for pm in self.point_mobs])
                if contain_center:
                    self.n_in += 1
                else:
                    self.n_out += 1
    ...
    def add_lines_and_comment_on_them(self):
        ...
        self.play(LaggedStartMap(FadeIn, arcs, run_time=1), Animation(self.point_mobs))
        self.wait()
        ...
        self.wait(2)
        self.play(center_lines.restore, center_lines.fade, 1, *list(map(FadeOut, [rect, words2, center_line_shadows, self.triangle, arcs, self.point_mobs, self.point_labels])))
    ...
    def rewrite_random_procedure(self):
        ...
        words = VGroup(*list(map(TexText, ['Choose 3 random points', 'Choose 2 random lines', 'Flip coin for each line \\\\ to get $P_1$ and $P_2$', 'Choose $P_3$ at random'])))
        words.scale(0.8)
        ...
        self.center_lines_update = self.get_center_lines_update(point_mobs, center_lines)
        ...
        self.play(LaggedStartMap(ApplyMethod, point_label_groups, lambda mob: (mob.restore,)))
        self.play(ShowCreation(cross), point_label_groups.fade, 1)
        self.wait()
        self.center_lines_update.update(1)
        self.play(FadeIn(words[1]), LaggedStartMap(GrowFromCenter, center_lines))
        for x in range(3):
        ...
        self.play(PiCreatureSays(randy, 'Seems \\\\ convoluted', bubble_config={'height': 2, 'width': 2}, target_mode='confused'))
        choose_p1_and_p2()
        self.play(FadeOut(randy.bubble), FadeOut(randy.bubble.content), randy.change, 'pondering')
    ...
    def four_possibilities_for_coin_flips(self):
        ...
        def do_the_rounds():
            for index in (0, 1, 0, 1):
                ...
                point_mob.target.rotate(np.pi, about_point=self.center)
                self.play(MoveToTarget(point_mob), *self.update_animations, run_time=np.sqrt(2) / 4)
                self.wait()
...
class ThisIsWhereItGetsGood(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says('This is where \\\\ things get good', target_mode='hooray')
        self.play_student_changes(*['hooray'] * 3)
        self.wait(2)

class ContrastTwoRandomProcesses(TwoDCase):
    CONFIG = {'radius': 1.5, 'random_seed': 0}
    ...
    def construct(self):
        ...
        center_lines = self.get_center_lines()
        point_label_groups = VGroup(*[VGroup(*pair) for pair in zip(point_mobs, labels)])
        right_circles = VGroup(*[VGroup(circle, *point_label_groups[:i + 1]).copy() for i in range(3)])
        left_circles = VGroup(VGroup(circle, center_lines).copy(), VGroup(circle, center_lines, point_label_groups[2]).copy(), VGroup(circle, center_lines, *point_label_groups[2::-1]).copy())
    ...
    def show_creation_of_circle_group(self, group):
        circles = group[:3]
        arrows = group[3:]
        self.play(ShowCreation(circles[0][0]), FadeIn(VGroup(*circles[0][1:])))
        for c1, c2, arrow in zip(circles, circles[1:], arrows):
            self.play(GrowArrow(arrow), ApplyMethod(c1.copy().shift, c2[0].get_center() - c1[0].get_center(), remover=True))
            self.add(c2)
            n = len(c2) - len(c1)
...
class Rewrite3DRandomProcedure(Scene):
    ...
    def construct(self):
        ...
        words = VGroup(*list(map(TexText, ['Choose 4 random points', 'Choose 3 random lines', 'Choose $P_4$ at random', 'Flip coin for each line \\\\ to get $P_1$, $P_2$, $P_3$'])))
        words.scale(0.8)
        words.arrange(DOWN, buff=MED_LARGE_BUFF)
        words.next_to(underline, DOWN)
        words[1].set_color(YELLOW)
...
class ThreeDAnswer(Scene):
    ...
    def construct(self):
        words = OldTexText('Probability that the tetrahedron contains center:', '$\\frac{1}{8}$')
        words.set_width(FRAME_WIDTH - 1)
        words.to_edge(DOWN)
...
class Formality(TeacherStudentsScene):
    ...
    def construct(self):
        words = OldTexText('Write-up by Ralph Howard and Paul Sisson (link below)')
        words.scale(0.7)
        words.to_corner(UP + LEFT, buff=MED_SMALL_BUFF)
        self.student_says('How would you \\\\ write that down?', target_mode='sassy')
        self.play_student_changes('confused', 'sassy', 'erm')
        self.wait()
        self.play(Write(words), FadeOut(self.students[1].bubble), FadeOut(self.students[1].bubble.content), self.teacher.change, 'raise_right_hand')
...
class ProblemSolvingTakeaways(Scene):
    ...
    def construct(self):
        ...
        group.to_corner(UP + LEFT)
        points = VGroup(*[OldTexText(string, alignment='') for string in ['Ask a simpler version \\\\ of the question', 'Try reframing the question \\\\ around new constructs']])
...
class BrilliantPuzzle(PiCreatureScene):
    ...
    def construct(self):
        ...
        for s1, s2 in adjacent_pairs(self.students):
            arrow = Arrow(s1.get_center(), s2.get_center(), path_arc=np.pi / 2, buff=0.8)
        ...
        self.play(LaggedStartMap(Write, tests), LaggedStartMap(ApplyMethod, students, lambda m: (m.change, 'horrified', m.test)))
        self.wait()
        self.play(LaggedStartMap(ApplyMethod, students, lambda m: (m.change, 'conniving')))
    ...
    def create_pi_creatures(self):
        self.students = VGroup(*[PiCreature(color=random.choice([BLUE_C, BLUE_D, BLUE_E, GREY_BROWN])).scale(0.25).move_to(3 * vect) for vect in compass_directions(8)])
    ...
    def get_arrow_swap_anim(self, arrow):
        ...
        arrow.target.rotate(target_angle, about_point=arrow.student.get_center())
...
class AddedPromoWords(Scene):
    ...
    def construct(self):
        words = OldTexText('First', '$2^8$', 'vistors get', '$(e^\\pi - \\pi)\\%$', 'off')
        words.set_width(FRAME_WIDTH - 1)
        words.to_edge(DOWN)
