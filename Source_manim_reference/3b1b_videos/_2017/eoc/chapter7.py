...
class Chapter7OpeningQuote(OpeningQuote):
    CONFIG = {'quote': [' Calculus required ', 'continuity', ', and ', 'continuity ', 'was supposed to require the ', 'infinitely little', '; but nobody could discover what the ', 'infinitely little', ' might be. '], 'quote_arg_separator': '', 'highlighted_quote_terms': {'continuity': BLUE, 'infinitely': GREEN}, 'author': 'Bertrand Russell'}
...
class ThisVideo(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        tex_mobs = [OldTex(*args) for args in [('{df ', ' \\over \\, ', ' dx}'), ('\\lim_{h \\to 0}',), ('\\int ', 'f(x)', '\\,dx')]]
        ...
        for index in (0, 2):
            ...
            self.play(ApplyWave(videos, direction=DOWN))
            self.play(GrowFromCenter(brace), Write(tex_mob, run_time=2))
        self.play(this_video.set_color, YELLOW, GrowFromCenter(this_brace), self.get_teacher().change_mode, 'raise_right_hand', self.get_teacher().look_at, this_video)
...
class LimitJustMeansApproach(PiCreatureScene):
    CONFIG = {'dx_color': GREEN, 'max_num_zeros': 7}
    ...
    def construct(self):
        ...
        for next_expression in evaluated_expressions:
            ...
            self.play(Transform(expression, next_expression, lag_ratio=0.5), self.pi_creature.look_at, next_expression[-1])
    ...
    def get_expression(self, dx):
        result = OldTex('{(2 + ', str(dx), ')^3 - 2^3 \\over', str(dx))
...
class Goals(Scene):
    ...
    def construct(self):
        goals = [OldTexText('Goal %d:' % d, s) for d, s in [(1, 'Formal definition of derivatives'), (2, '$(\\epsilon, \\delta)$ definition of a limit'), (3, "L'Hôpital's rule")]]
        for goal in goals:
            goal.scale(1.3)
...
class RefreshOnDerivativeDefinition(GraphScene):
    CONFIG = {'start_x': 2, 'start_dx': 0.7, 'df_color': YELLOW, 'dx_color': GREEN, 'secant_line_color': MAROON_B}
    ...
    def construct(self):
        ...
        start_x_v_line, nudged_x_v_line = [self.get_vertical_line_to_graph(self.start_x + nudge, graph, line_class=DashedLine, color=RED) for nudge in (0, self.start_dx)]
        nudged_x_v_line.save_state()
        ss_group = self.get_secant_slope_group(self.start_x, graph, dx=self.start_dx, dx_label='dx', df_label='df', df_line_color=self.df_color, dx_line_color=self.dx_color, secant_line_color=self.secant_line_color)
        derivative = OldTex('{df', '\\over \\,', 'dx}', '(', str(self.start_x), ')')
        ...
        self.play(input_copy.next_to, self.coords_to_point(self.start_x, 0), DOWN)
        self.play(ShowCreation(start_x_v_line))
        self.wait()
        self.play(ShowCreation(ss_group.dx_line), ShowCreation(ss_group.dx_label))
        self.wait()
        self.play(ShowCreation(ss_group.df_line))
        self.play(Write(ss_group.df_label))
        self.wait(2)
        self.play(ReplacementTransform(ss_group.dx_label.copy(), dx), ReplacementTransform(ss_group.df_label.copy(), df), run_time=2)
        self.play(ShowCreation(ss_group.secant_line))
        self.wait()
        self.play(GrowFromCenter(deriv_brace), Write(dx_to_0))
        self.animate_secant_slope_group_change(ss_group, target_dx=0.01, run_time=5)
        self.wait()
        new_deriv = OldTex('{f', '(', str(self.start_x), '+', 'dx', ')', '-', 'f', '(', str(self.start_x), ')', '\\over \\,', 'dx')
        ...
        deriv_to_new_deriv = dict([(VGroup(derivative.get_part_by_tex(s)), VGroup(*new_deriv.get_parts_by_tex(s))) for s in ['f', 'over', 'dx', '(', str(self.start_x), ')']])
        ...
        self.animate_secant_slope_group_change(ss_group, target_dx=self.start_dx, run_time=2)
        self.play(ShowCreation(nudged_x_v_line))
        self.wait()
        self.play(*[ReplacementTransform(*pair, run_time=2) for pair in list(deriv_to_new_deriv.items())] + [Transform(deriv_brace, new_brace), dx_to_0.next_to, new_brace, DOWN])
        ...
        self.play(ReplacementTransform(deriv_brace, lim), MoveToTarget(dx_to_0), new_deriv.next_to, lim_group, RIGHT, run_time=2)
        for sf, color in ((1.2, YELLOW), (1 / 1.2, WHITE)):
            self.play(lim.scale, sf, lim.set_color, color, lag_ratio=0.5)
        self.wait(2)
        self.animate_secant_slope_group_change(ss_group, target_dx=0.01, run_time=5, added_anims=[Transform(nudged_x_v_line, start_x_v_line, run_time=5)])
        self.wait(2)
        digest_locals(self)
...
class RantOpenAndClose(Scene):
    ...
    def construct(self):
        opening, closing = [OldTexText(start, 'Rant on infinitesimals', '$>$', arg_separator='') for start in ('$<$', '$<$/')]
...
class DiscussLowercaseDs(RefreshOnDerivativeDefinition, PiCreatureScene, ZoomedScene):
    ...
    def skip_superclass_anims(self):
        ...
        self.animate_secant_slope_group_change(self.ss_group, target_dx=self.start_dx, added_anims=[self.nudged_x_v_line.restore, Animation(self.ss_group.df_line)], run_time=1)
        everything = self.get_top_level_mobjects()
        everything.remove(self.derivative)
        self.play(*[ApplyMethod(mob.shift, 2.5 * LEFT) for mob in everything] + [FadeIn(self.pi_creature)])
    ...
    def replace_dx_terms(self):
        ...
        self.play(mover.scale, mover_scale_val, mover.next_to, self.pi_creature.get_corner(UP + LEFT), UP, MED_SMALL_BUFF, self.pi_creature.change_mode, 'sassy', path_arc=np.pi / 2)
        self.blink()
        self.wait()
        for tex in ('\\Delta x', 'h'):
            dx_list_replacement = [OldTex(tex).set_color(self.dx_color).move_to(dx, DOWN) for dx in dx_list]
            self.play(Transform(VGroup(*dx_list), VGroup(*dx_list_replacement)), self.pi_creature.change_mode, 'raise_right_hand')
            self.wait()
        self.play(mover.scale, 0.9, mover.move_to, mover.initial_right, RIGHT, self.pi_creature.change_mode, 'happy')
        self.play(self.dx_to_0.next_to, self.lim, DOWN, SMALL_BUFF)
        self.wait()
    ...
    def compare_rhs_and_lhs(self):
        ...
        d_circles = VGroup(*[Circle(color=BLUE_B).replace(lhs.get_part_by_tex(tex)[0], stretch=True).scale(1.5).rotate(-np.pi / 12) for tex in ('df', 'dx')])
        d_words = OldTexText('\n            Limit idea is\n            built in\n        ')
        d_words.next_to(d_circles, DOWN)
        d_words.set_color(d_circles[0].get_color())
        lhs_rect, rhs_rect = rects = [Rectangle(color=GREEN_B).replace(mob, stretch=True) for mob in (lhs, rhs.target)]
        for rect in rects:
        ...
        self.play(MoveToTarget(rhs), Write(lhs), Write(equals))
        self.play(ShowCreation(d_circles, run_time=2), self.pi_creature.change_mode, 'pondering')
        self.play(Write(d_words))
        self.animate_secant_slope_group_change(self.ss_group, target_dx=0.01, added_anims=[Transform(self.nudged_x_v_line, self.start_x_v_line, run_time=3)])
        self.change_mode('thinking')
        self.wait(2)
        self.play(ShowCreation(lhs_rect), FadeOut(d_circles), FadeOut(d_words))
        self.wait(2)
        self.play(ReplacementTransform(lhs_rect, rhs_rect), self.pi_creature.change_mode, 'raise_right_hand')
        self.wait(2)
        self.play(ReplacementTransform(df.copy(), df_target, path_arc=-np.pi / 2, run_time=2))
        self.wait(2)
        self.play(Indicate(VGroup(*rhs[:2]), run_time=2))
        self.wait()
        self.play(Write(formal_definition_words))
        self.play(self.pi_creature.change_mode, 'happy', self.pi_creature.look_at, formal_definition_words)
        self.wait(2)
        lhs.add_background_rectangle()
        self.add(rhs_rect, rhs)
        self.definition_group = VGroup(lhs, equals, rhs_rect, rhs, formal_definition_words)
        self.lhs, self.rhs, self.rhs_rect = (lhs, rhs, rhs_rect)

    def h_is_finite(self):
        self.play(FadeOut(self.graph_label), self.definition_group.center, self.definition_group.to_corner, UP + RIGHT, self.pi_creature.change_mode, 'sassy', self.pi_creature.look_at, 4 * UP)
        self.wait()
        words = OldTexText("No ``infinitely small''")
        words.next_to(self.definition_group, DOWN, buff=LARGE_BUFF)
        arrow = Arrow(words.get_top(), self.rhs_rect.get_bottom())
        arrow.set_color(WHITE)
        h_group = VGroup(self.rhs[1].get_part_by_tex('dx'), *self.rhs[2].get_parts_by_tex('dx'))
        moving_h = h_group[0]
        ...
        self.wait(2)
        self.play(moving_h.next_to, self.pi_creature.get_corner(UP + RIGHT), UP, self.pi_creature.change_mode, 'raise_left_hand')
        self.wait()
        moving_dx.move_to(moving_h)
        h_group.save_state()
        self.play(Transform(h_group, dx_group, path_arc=np.pi))
        self.wait(2)
        self.play(h_group.restore, path_arc=np.pi)
        self.play(moving_h.move_to, moving_h.original_center, self.pi_creature.change_mode, 'plain')
        self.wait()
        ...
        self.wait()
        self.add(self.rhs)
        self.play(lil_rect.set_width, self.ss_group.dx_line.get_width() * 4, run_time=4)
        self.wait()
        ...
        for target_dx in (3, 0.01, -2, 0.01):
            self.animate_secant_slope_group_change(self.ss_group, target_dx=target_dx, run_time=4)
            self.wait()

class OtherViewsOfDx(TeacherStudentsScene):

    def construct(self):
        definition = OldTex('{df', '\\over \\,', 'dx}', '(', '2', ')', '=', '\\lim', '_{h', '\\to', '0}', '{f', '(', '2', '+', 'h', ')', '-', 'f', '(', '2', ')', '\\over \\,', 'h}')
        tex_to_color = {'df': YELLOW, 'f': YELLOW, 'dx': GREEN, 'h': GREEN, '2': RED}
        ...
        self.add(definition)
        statements = [OldTexText(*args) for args in [('Why the new \\\\ variable', '$h$', '?'), ('$dx$', 'is more $\\dots$ contentious.'), ('$dx$', 'is infinitely small'), ('$dx$', 'is nothing more \\\\ than a symbol')]]
        for statement in statements:
            statement.h, statement.dx = [VGroup(*statement.get_parts_by_tex(tex, substring=False)).set_color(GREEN) for tex in ('$h$', '$dx$')]
        self.student_says(statements[0], index=1, target_mode='confused')
        self.play(ReplacementTransform(statements[0].h.copy(), h_group, run_time=2, lag_ratio=0.5))
        self.wait()
        self.teacher_says(statements[1], target_mode='hesitant', bubble_creation_class=FadeIn)
        self.play(ReplacementTransform(statements[1].dx.copy(), dx_group, run_time=2))
        self.wait()
        moving_dx = dx_group.copy()
        bubble_intro = PiCreatureBubbleIntroduction(self.get_students()[2], statements[2], target_mode='hooray', bubble_creation_class=FadeIn)
        bubble_intro.update(1)
        dx_movement = Transform(moving_dx, statements[2].dx, run_time=2)
        bubble_intro.update(0)
        self.play(bubble_intro, dx_movement, RemovePiCreatureBubble(self.get_teacher()))
        self.play(self.get_teacher().change_mode, 'erm')
        self.wait()
        bubble_intro = PiCreatureBubbleIntroduction(self.get_students()[0], statements[3], target_mode='maybe', look_at=3 * UP, bubble_creation_class=FadeIn)
        bubble_intro.update(1)
        dx_movement = Transform(moving_dx, statements[3].dx, run_time=2)
        bubble_intro.update(0)
        last_bubble = self.get_students()[2].bubble
        self.play(bubble_intro, dx_movement, FadeOut(last_bubble), FadeOut(last_bubble.content), *it.chain(*[[pi.change_mode, 'pondering', pi.look_at, bubble_intro.mobject] for pi in self.get_students()[1:]]))
        self.wait(3)

class GoalsListed(Scene):

    def construct(self):
        goals = VGroup(*[OldTexText('Goal %d: %s' % (d, s)) for d, s in zip(it.count(1), ['Formal definition of a derivative', '$(\\epsilon, \\delta)$ definition of limits', "L'Hôpital's rule"])])
...
class GraphLimitExpression(GraphScene):
    CONFIG = {'start_x': 2, 'h_color': GREEN, 'f_color': YELLOW, 'two_color': RED, 'graph_origin': 3 * DOWN + LEFT, 'x_min': -8, 'x_max': 5, 'x_axis_label': '$h$', 'x_labeled_nums': list(range(-8, 6, 2)), 'y_min': 0, 'y_max': 20, 'y_tick_frequency': 1, 'y_labeled_nums': list(range(5, 25, 5)), 'y_axis_label': '', 'big_delta': 0.7, 'small_delta': 0.01}
    ...
    def introduce_function(self):
        expression = OldTex('{(', '2', '+', 'h', ')', '^3', '-', '(', '2', ')', '^3', '\\over \\,', 'h}', arg_separator='')
        limit = OldTex('\\lim', '_{h', '\\to 0}')
        derivative = OldTex('{d(x^3)', '\\over \\,', 'dx}', '(', '2', ')')
        tex_to_color = {'h': self.h_color, 'dx': self.h_color, '2': self.two_color}
        ...
        for i, j, func in zip(indices, indices[1:], funcs):
            anims = [FadeIn(VGroup(*expression[i:j]), lag_ratio=0.5)]
            ...
            self.wait()
        self.wait()
        self.play(MoveToTarget(expression), FadeIn(limit, lag_ratio=0.5), GrowFromCenter(brace))
        self.play(Write(derivative))
        self.wait(2)
        self.play(expression.restore, *list(map(FadeOut, [derivative, brace, limit])))
        self.wait()
    ...
    def emphasize_non_definedness_at_0(self):
        ...
        self.plug_in_zero_group = VGroup(new_expression, rhs, ud_brace, undefined)

    def draw_limit_point_hole(self):
        dx = 0.07
        color = self.graph.get_color()
        circle = Circle(radius=dx, stroke_color=color, fill_color=BLACK, fill_opacity=1)
        ...
        self.play(ShowCreation(circle.copy().set_fill(opacity=0), remover=True))
        self.remove(colored_circle)
        self.play(circle.scale, 0.3, run_time=2, rate_func=wiggle)
        self.wait()
    ...
    def show_limit(self):
        ...
        self.play(FadeOut(self.plug_in_zero_group), FadeOut(VGroup(*self.point_to_zero_group[:-1])))
        self.play(GrowFromCenter(brace), Write(brace_text), ReplacementTransform(dot, ed_group.input_range))
        self.add(h_zero_hole)
        self.wait()
        self.play(ReplacementTransform(ed_group.input_range.copy(), ed_group.output_range, run_time=2))
        ...
        for v_line, h_line in ((right_v_line, top_h_line), (left_v_line, bottom_h_line)):
            ...
            self.wait()
            self.play(v_line.move_to, self.coords_to_point(0, 0), DOWN, h_line.move_to, self.coords_to_point(0, self.func(0)), run_time=3)
            self.play(VGroup(h_line, v_line).set_stroke, GREY, 2)
        self.wait()
        limit = self.limit
    ...
    def skeptic_asks(self):
        ...
        self.play(PiCreatureSays(randy, "\n                What \\emph{exactly} do you\n                mean by ``approach''\n            ", bubble_config={'height': 3, 'width': 5, 'fill_opacity': 1, 'direction': LEFT}, target_mode='sassy'))
        self.remove(self.twelve_copy)
        self.play(randy.look, OUT)
        self.play(Blink(randy))
        self.wait()
        self.play(RemovePiCreatureBubble(randy, target_mode='pondering', look_at=self.limit_point_hole))
        self.play(self.ed_group.delta_lines.restore, self.ed_group.epsilon_lines.restore, Animation(randy), rate_func=there_and_back, run_time=5)
        self.play(Blink(randy))
        self.play(FadeOut(randy))

    def show_epsilon_delta_intuition(self):
        self.play(FadeOut(self.ed_group.epsilon_lines), FadeIn(self.input_range_brace_group))
        self.ed_group.epsilon_lines.restore()
        self.wait()
        self.play(self.ed_group.delta_lines.restore, Animation(self.input_range_brace_group), run_time=2)
        self.play(FadeOut(self.input_range_brace_group))
        self.play(ReplacementTransform(self.ed_group.input_range.copy(), self.ed_group.output_range, run_time=2))
        self.wait()
        self.play(*list(map(GrowFromCenter, self.ed_group.epsilon_lines)))
        self.play(*[ApplyMethod(line.copy().set_stroke(GREY, 2).move_to, self.coords_to_point(0, self.func(0)), run_time=3, rate_func=there_and_back, remover=True) for line in self.ed_group.epsilon_lines])
        self.wait()
        holes = VGroup(self.ed_group.input_range.submobjects.pop(), self.ed_group.output_range.submobjects.pop())
        holes.save_state()
        self.animate_epsilon_delta_group_change(self.ed_group, target_delta=self.small_delta, run_time=8, rate_func=lambda t: smooth(t, 2), added_anims=[ApplyMethod(hole.scale, 0.5, run_time=8) for hole in holes])
        self.wait()
        self.holes = holes

    def get_epsilon_delta_group(self, delta, limit_x=0, dashed_line_stroke_width=3, dashed_line_length=FRAME_HEIGHT, input_range_color=YELLOW, input_range_stroke_width=6):
        kwargs = dict(locals())
        result = VGroup()
        kwargs.pop('self')
        ...
        dashed_line = DashedLine(ORIGIN, dashed_line_length * RIGHT, stroke_width=dashed_line_stroke_width)
        x_values = [limit_x - delta, limit_x + delta]
        x_axis_points = [self.coords_to_point(x, 0) for x in x_values]
        result.delta_lines = VGroup(*[dashed_line.copy().rotate(np.pi / 2).move_to(point, DOWN) for point in x_axis_points])
        if self.func(limit_x) < 0:
            result.delta_lines.rotate(np.pi, RIGHT, about_point=result.delta_lines.get_bottom())
        basically_zero = 1e-05
        result.input_range, result.output_range = [VGroup(*[self.get_graph(func, color=input_range_color, x_min=x_min, x_max=x_max) for x_min, x_max in [(limit_x - delta, limit_x - basically_zero), (limit_x + basically_zero, limit_x + delta)]]).set_stroke(width=input_range_stroke_width) for func in (lambda h: 0, self.func)]
        result.epsilon_lines = VGroup(*[dashed_line.copy().move_to(self.coords_to_point(limit_x, 0)[0] * RIGHT + result.output_range.get_edge_center(vect)[1] * UP) for vect in (DOWN, UP)])
        result.digest_mobject_attrs()
        return result

    def animate_epsilon_delta_group_change(self, epsilon_delta_group, target_delta, **kwargs):
        ...
        def update_ed_group(ed_group, alpha):
            ...
            new_group = self.get_epsilon_delta_group(delta, limit_x=limit_x, **ed_group_kwargs)
            Transform(ed_group, new_group).update(1)
            return ed_group
        self.play(UpdateFromAlphaFunc(epsilon_delta_group, update_ed_group, **kwargs), *added_anims)

class LimitCounterExample(GraphLimitExpression):
    CONFIG = {'x_min': -8, 'x_max': 8, 'x_labeled_nums': list(range(-8, 10, 2)), 'x_axis_width': FRAME_WIDTH - LARGE_BUFF, 'y_min': -4, 'y_max': 4, 'y_labeled_nums': list(range(-2, 4, 1)), 'y_axis_height': FRAME_HEIGHT + 2 * LARGE_BUFF, 'graph_origin': DOWN, 'graph_color': BLUE, 'hole_radius': 0.075, 'smaller_hole_radius': 0.04, 'big_delta': 1.5, 'small_delta': 0.05}
    ...
    def draw_graph(self):
        epsilon = 0.1
        left_graph, right_graph = [self.get_graph(self.func, color=self.graph_color, x_min=x_min, x_max=x_max) for x_min, x_max in [(self.x_min, -epsilon), (epsilon, self.x_max)]]
        ...
        self.wait()
        self.play(ReplacementTransform(left_hole.copy().set_stroke(YELLOW), right_hole))
        self.wait()
        self.graph = graph
    ...
    def show_epsilon_delta_intuition(self):
        ...
        self.play(ed_group.delta_lines.restore, ed_group.epsilon_lines.restore)
        ...
        self.wait()
        self.animate_epsilon_delta_group_change(ed_group, target_delta=self.small_delta, run_time=6)
        ...
        self.wait()
        run_time_rate_func_pairs = [(3, lambda t: 1 - there_and_back(t)), (1, lambda t: 1 - 0.2 * there_and_back(3 * t % 1)), (1, lambda t: 1 - 0.2 * there_and_back(5 * t % 1))]
        for run_time, rate_func in run_time_rate_func_pairs:
            self.animate_epsilon_delta_group_change(ed_group, target_delta=self.small_delta, run_time=run_time, rate_func=rate_func)
            self.wait()
    ...
    def get_epsilon_delta_group(self, delta, **kwargs):
        ...
        pairs = [(ed_group.input_range[0], (0, 0)), (ed_group.input_range[1], (0, 0)), (ed_group.output_range[0], (0, 1)), (ed_group.output_range[1], (0, 2))]
    ...
    def get_hole(self, *coords, **kwargs):
        ...
        return Circle(radius=radius, stroke_color=color, fill_color=BLACK, fill_opacity=1).move_to(self.coords_to_point(*coords))
...
class PrefaceToEpsilonDeltaDefinition(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(title.restore, self.get_teacher().change_mode, 'raise_right_hand')
        self.play_student_changes(*['confused'] * 3)
        self.wait()
        self.student_says("Isn't that pretty \\\\ technical?", target_mode='guilty', added_anims=[title.to_edge, UP, self.get_teacher().change_mode, 'plain', self.get_teacher().look_at, self.get_students()[1].eyes])
        self.look_at(self.get_teacher().eyes, self.get_students())
        self.wait()
        self.teacher_says('', bubble_config={'stroke_width': 0})
        self.play_student_changes(*['pondering'] * 3, look_at=UP + LEFT, added_anims=[self.get_teacher().look_at, UP + LEFT])
        self.wait(3)
        words = OldTexText("It's a glimpse of\\\\", 'real analysis')
        words.set_color_by_tex('real', YELLOW)
        self.teacher_says(words, bubble_config={'height': 3, 'width': 6})
        self.play_student_changes(*['happy'] * 3)
        self.wait(6)

class EpsilonDeltaExample(GraphLimitExpression, ZoomedScene):
    CONFIG = {'epsilon_list': [2, 1, 0.5], 'zoomed_canvas_corner': DOWN + RIGHT}
    ...
    def skip_superclass_anims(self):
        ...
        self.animate_epsilon_delta_group_change(self.ed_group, target_delta=self.big_delta)
        self.holes.restore()
        self.add(self.holes)
        self.revert_to_original_skipping_status()

    def introduce_epsilon(self):
        epsilon_group, small_epsilon_group = list(map(self.get_epsilon_group, self.epsilon_list[:2]))
        ...
        arrows = VGroup(*[Arrow(distance.get_top(), label.get_right()) for label in epsilon_group.labels])
        self.play(ShowCreation(twelve_line))
        self.play(Write(twelve_copy))
        self.play(ReplacementTransform(twelve_copy, twelve))
        self.wait()
        self.play(*it.chain([ReplacementTransform(twelve_line.copy(), line) for line in epsilon_group.epsilon_lines], list(map(GrowFromCenter, epsilon_group.braces))))
    ...
    def match_epsilon(self):
        self.animate_epsilon_delta_group_change(self.ed_group, target_delta=self.delta_list[1], run_time=2, added_anims=[ApplyMethod(hole.scale, 0.25, run_time=2) for hole in self.holes])
    ...
    def zoom_in(self):
        ...
        self.add(self.holes)
        self.wait()
        self.play(lil_rect.scale, 1.0 / self.zoom_factor)
        self.wait()
    ...
    def introduce_delta(self):
        ...
        self.wait()
        self.play(ReplacementTransform(self.ed_group.input_range.copy(), self.ed_group.output_range, run_time=2), Animation(self.holes))
        self.play(ApplyWave(VGroup(self.ed_group.output_range, self.holes[1]), direction=RIGHT))
        self.wait(2)
    ...
    def smaller_epsilon(self):
        ...
        self.play(Transform(self.epsilon_group, self.get_epsilon_group(new_epsilon)))
        self.wait()
        self.animate_epsilon_delta_group_change(self.ed_group, target_delta=new_delta, added_anims=[Transform(self.delta_group, self.get_delta_group(new_delta))] + [ApplyMethod(hole.scale, 0.5) for hole in self.holes])
        self.ed_group.input_range.make_jagged()
        self.wait(2)
    ...
    def get_epsilon_group(self, epsilon, limit_value=12):
        ...
        lines = [Line(ORIGIN, line_length * RIGHT).move_to(self.coords_to_point(0, limit_value + nudge)) for nudge in (0, -epsilon, epsilon)]
        ...
        result.braces = VGroup(*[brace.copy().set_height(group.get_height()).next_to(group, RIGHT, SMALL_BUFF) for i in (1, 2) for group in [VGroup(lines[0], lines[i])]])
        result.labels = VGroup(*[brace.get_text('\\Big $\\epsilon$', buff=SMALL_BUFF) for brace in result.braces])
    ...
    def get_delta_group(self, delta):
        ...
        brace.set_width((self.coords_to_point(delta, 0) - self.graph_origin)[0])
        result.braces = VGroup(*[brace.copy().move_to(self.coords_to_point(x, 0)) for x in (-delta / 2, delta / 2)])
        result.braces.shift(self.holes[0].get_height() * DOWN)
        result.labels = VGroup(*[OldTex('\\delta').scale(1.0 / self.zoom_factor) for brace in result.braces])
        for label, brace in zip(result.labels, result.braces):
...
class EpsilonDeltaCounterExample(LimitCounterExample, EpsilonDeltaExample):
    ...
    def introduce_epsilon(self):
        ...
        self.play(*it.chain([ReplacementTransform(epsilon_group.limit_line.copy(), line) for line in epsilon_group.epsilon_lines], list(map(GrowFromCenter, epsilon_group.braces))))
        self.play(*list(map(Write, epsilon_group.labels)))
        self.play(Write(rhs))
        self.wait()
        self.epsilon_group = epsilon_group
    ...
    def introduce_epsilon_delta_group(self):
        ...
        self.wait(2)
        self.animate_epsilon_delta_group_change(ed_group, target_delta=self.small_delta, run_time=3)
        ed_group.delta = self.small_delta
        self.wait()
        self.ed_group = ed_group

    def move_epsilon_group_up_and_down(self):
        vects = [self.coords_to_point(0, 2) - self.coords_to_point(0, 1.5), self.coords_to_point(0, 1) - self.coords_to_point(0, 2), self.coords_to_point(0, 1.5) - self.coords_to_point(0, 1)]
        for vect in vects:
    ...
    def shake_ed_group(self):
        self.animate_epsilon_delta_group_change(self.ed_group, target_delta=self.big_delta, rate_func=lambda t: 0.2 * there_and_back(2 * t % 1))

class TheoryHeavy(TeacherStudentsScene):

    def construct(self):
        lhs = OldTex('{df', '\\over \\,', 'dx}', '(', 'x', ')')
        equals = OldTex('=')
        rhs = OldTex('\\lim', '_{h', '\\to 0}', '{f', '(', 'x', '+', 'h', ')', '-', 'f', '(', 'x', ')', '\\over \\,', 'h}')
        ...
        self.student_says('Too much theory!', target_mode='angry', content_introduction_kwargs={'run_time': 2})
        self.wait()
        student = self.get_students()[1]
        Scene.play(self, Write(lhs), FadeOut(student.bubble), FadeOut(student.bubble.content), *[ApplyFunction(lambda pi: pi.change_mode('pondering').look_at(epsilon_delta), pi) for pi in self.get_pi_creatures()])
        student.bubble = None
        part_tex_pairs = [('df', 'f'), ('over', '+'), ('over', '-'), ('over', 'to'), ('over', 'over'), ('dx', 'h'), ('(', '('), ('x', 'x'), (')', ')')]
        self.play(Write(equals), Write(lim), *[ReplacementTransform(VGroup(*lhs.get_parts_by_tex(t1)).copy(), VGroup(*rhs.get_parts_by_tex(t2)), run_time=2, rate_func=squish_rate_func(smooth, alpha, alpha + 0.5)) for (t1, t2), alpha in zip(part_tex_pairs, np.linspace(0, 0.5, len(part_tex_pairs)))])
        self.wait(2)
        self.play(Write(epsilon_delta), ShowCreation(arrow))
        self.wait(3)
        derivative.add(epsilon_delta, arrow)
        self.student_says('How do you \\\\ compute limits?', index=2, added_anims=[derivative.scale, 0.8, derivative.to_corner, UP + LEFT])
        self.play(self.get_teacher().change_mode, 'happy')
        self.wait(2)

class LHopitalExample(LimitCounterExample, PiCreatureScene, ZoomedScene, ReconfigurableScene):
    CONFIG = {'graph_origin': ORIGIN, 'x_axis_width': FRAME_WIDTH, 'x_min': -5, 'x_max': 5, 'x_labeled_nums': list(range(-6, 8, 2)), 'x_axis_label': '$x$', 'y_axis_height': FRAME_HEIGHT, 'y_min': -3.1, 'y_max': 3.1, 'y_bottom_tick': -4, 'y_labeled_nums': list(range(-2, 4, 2)), 'y_axis_label': '', 'x_color': RED, 'hole_radius': 0.07, 'big_delta': 0.5, 'small_delta': 0.01, 'dx': 0.06, 'dx_color': WHITE, 'tex_scale_value': 0.9, 'sin_color': GREEN, 'parabola_color': YELLOW, 'zoomed_canvas_corner': DOWN + LEFT, 'zoom_factor': 10, 'zoomed_canvas_frame_shape': (5, 5), 'zoomed_canvas_corner_buff': MED_SMALL_BUFF, 'zoomed_rect_center_coords': (1 + 0.1, -0.03)}
    ...
    def introduce_function(self):
        ...
        self.play(Write(func_label), Transform(x_copy, VGroup(*func_label.get_parts_by_tex('x')), remover=True))
    ...
    def show_non_definedness_at_one(self):
        ...
        v_line, alt_v_line = [self.get_vertical_line_to_graph(x, self.graph, line_class=DashedLine, color=self.x_color) for x in (1, -1)]
        hole, alt_hole = [self.get_hole(x, self.func(x)) for x in (1, -1)]
        ed_group = self.get_epsilon_delta_group(self.big_delta, limit_x=1)
        ...
        self.play(RemovePiCreatureBubble(morty, target_mode='pondering', look_at=func_1), ReplacementTransform(self.func_label.copy(), func_1))
        self.wait(2)
        ...
        self.play(ReplacementTransform(ed_group.input_range.copy(), ed_group.output_range), *list(map(ShowCreation, ed_group.delta_lines)))
        self.play(*list(map(GrowFromCenter, ed_group.epsilon_lines)))
        self.play(morty.change_mode, 'thinking')
        self.animate_epsilon_delta_group_change(ed_group, target_delta=self.small_delta, run_time=4)
        self.wait()
        self.play(Write(lim), MoveToTarget(self.func_label), Write(equals_q), morty.change_mode, 'confused', morty.look_at, lim)
        self.wait(2)
        self.play(func_1_group.to_corner, UP + LEFT, *list(map(FadeOut, [morty, ed_group])))
        self.wait()
    ...
    def ask_about_systematic_process(self):
        ...
        self.pi_creature_says(morty, 'Is there a \\\\ better way?', bubble_config={'height': 3, 'width': 4})
        self.wait(2)
        self.play(RemovePiCreatureBubble(morty, target_mode='raise_left_hand', look_at=self.func_1_group), self.func_1_group.scale, self.tex_scale_value, self.func_1_group.move_to, morty.get_corner(UP + LEFT), DOWN + LEFT, self.func_1_group.shift, MED_LARGE_BUFF * UP)
        self.wait(2)
        self.play(morty.change_mode, 'raise_right_hand', morty.look, UP + RIGHT, FadeOut(self.approx_group), self.func_1_group.restore, self.lim_group.next_to, morty.get_corner(UP + RIGHT), RIGHT)
        self.wait(2)
        self.play(FadeOut(self.func_1_group), self.lim_group.scale, self.tex_scale_value, self.lim_group.to_corner, UP + LEFT, morty.change_mode, 'plain')
        self.wait()
        self.play(FadeOut(morty))

    def show_graph_of_numerator_and_denominator(self):
        sine_graph = self.get_graph(lambda x: np.sin(np.pi * x), color=self.sin_color)
        sine_label = self.get_graph_label(sine_graph, '\\sin(\\pi x)', x_val=4.5, direction=UP)
        parabola = self.get_graph(lambda x: x ** 2 - 1, color=self.parabola_color)
        parabola_label = self.get_graph_label(parabola, 'x^2 - 1')
        fader = VGroup(*[Rectangle(width=FRAME_WIDTH, height=FRAME_HEIGHT, stroke_width=0, fill_opacity=0.75, fill_color=BLACK).next_to(self.coords_to_point(1, 0), vect, MED_LARGE_BUFF) for vect in (LEFT, RIGHT)])
        self.play(ShowCreation(sine_graph, run_time=2), Animation(self.lim_group))
    ...
    def talk_through_sizes_of_nudges(self):
        ...
        dx_arrow = Arrow(self.coords_to_point(1, 0), self.coords_to_point(1 + self.dx, 0), tip_length=arrow_tip_length, color=WHITE)
        dx_label = OldTex('dx')
        dx_label.scale(zoom_tex_scale_factor)
        dx_label.next_to(dx_arrow, UP, buff=SMALL_BUFF / self.zoom_factor)
        d_sine_arrow, d_parabola_arrow = [Arrow(self.coords_to_point(1 + self.dx, 0), self.coords_to_point(1 + self.dx, graph.underlying_function(1 + self.dx)), tip_length=arrow_tip_length, color=graph.get_color()) for graph in (self.sine_graph, self.parabola)]
        tex_arrow_pairs = [[('d\\big(', '\\sin(', '\\pi', 'x', ')', '\\big)'), d_sine_arrow], [('d\\big(', 'x', '^2', '-1', '\\big)'), d_parabola_arrow], [('\\cos(', '\\pi', 'x', ')', '\\pi ', '\\, dx'), d_sine_arrow], [('2', 'x', '\\, dx'), d_parabola_arrow]]
        d_labels = []
        ...
        self.wait()
        self.play(ReplacementTransform(VGroup(dx_label[1].copy()), d_sine_label, run_time=2))
        self.wait(2)
        self.play(d_sine_label.shift, d_sine_label.get_height() * UP)
        tex_pair_lists = [[('sin', 'cos'), ('pi', 'pi'), ('x', 'x'), (')', ')')], [('pi', '\\pi ')], [('d\\big(', 'dx'), ('\\big)', 'dx')]]
        for tex_pairs in tex_pair_lists:
            self.play(*[ReplacementTransform(d_sine_label.get_part_by_tex(t1).copy(), cos_dx.get_part_by_tex(t2)) for t1, t2 in tex_pairs])
            self.wait()
        ...
        self.wait(2)
        self.play(brace.set_width, cos.get_width(), brace.next_to, cos, UP, SMALL_BUFF / self.zoom_factor, FadeOut(brace_text))
        ...
        self.play(neg_pi_dx.next_to, d_sine_arrow, RIGHT, SMALL_BUFF / self.zoom_factor)
        self.wait()
        point = VectorizedPoint(self.coords_to_point(1, 0))
        self.play(ReplacementTransform(point, d_parabola_arrow))
        self.play(ReplacementTransform(VGroup(dx_label[1].copy()), d_parabola_label, run_time=2))
        self.wait(2)
        self.play(d_parabola_label.shift, d_parabola_label.get_height() * UP)
        tex_pair_lists = [[('2', '2'), ('x', 'x')], [('d\\big(', 'dx'), ('\\big)', 'dx')]]
        for tex_pairs in tex_pair_lists:
            self.play(*[ReplacementTransform(d_parabola_label.get_part_by_tex(t1).copy(), two_x_dx.get_part_by_tex(t2)) for t1, t2 in tex_pairs])
        self.wait()
    ...
    def substitute_x_equals_1(self, tex_mob, zoom_tex_scale_factor):
        ...
        equation.next_to(x, UP, buff=MED_SMALL_BUFF / self.zoom_factor, aligned_edge=LEFT)
        ...
        self.wait()
        self.play(ReplacementTransform(x.copy(), eq_x), Transform(x, one), Write(equals))
        self.wait()
    ...
    def show_final_ratio(self):
        ...
        rhs = OldTex('\\approx', '{-\\pi', '\\, dx', '\\over \\,', '2', '\\, dx}')
        ...
        circles = VGroup(*[Circle(color=GREEN).replace(dx).scale(1.3) for dx in dxs])
        ...
        self.wait()
        self.transition_to_alt_config(transformation_kwargs={'run_time': 2}, dx=self.dx / 10)
        self.wait()
    ...
    def show_final_height(self):
        ...
        self.disactivate_zooming()
        self.play(*list(map(FadeOut, [self.sine_graph, self.sine_label, self.parabola, self.parabola_label])) + [Animation(self.final_ratio)])
        self.play(GrowFromCenter(brace))
    ...
    def get_func_label(self, argument='x'):
        ...
        result = OldTex('{\\sin(\\pi ', in_tex, ')', ' \\over \\,', in_tex, '^2 - 1}')
    ...
    def get_epsilon_delta_group(self, delta, **kwargs):
        ...
        color = ed_group.kwargs['input_range_color']
        radius = min(delta / 2, self.hole_radius)
        pairs = [(ed_group.input_range[1], (1, 0)), (ed_group.output_range[1], (1, self.func(1)))]
        for mob, coords in pairs:
            mob.add(self.get_hole(*coords, color=color, radius=radius))
        return ed_group
...
class GeneralLHoptial(LHopitalExample):
    CONFIG = {'f_color': BLUE, 'g_color': YELLOW, 'a_value': 2.5, 'zoomed_rect_center_coords': (2.55, 0), 'zoom_factor': 15, 'image_height': 3}
    ...
    def add_graphs(self):
        ...
        f_label = self.get_graph_label(f_graph, 'f(x)', x_val=3, direction=RIGHT)
        g_graph = ParametricCurve(lambda y: self.coords_to_point(np.exp(y) + self.a_value - 1, y), t_min=self.y_min, t_max=self.y_max, color=self.g_color)
        ...
        self.wait()
        self.play(Write(a_label), ShowCreation(a_arrow), ShowCreation(a_dot))
        self.wait()
    ...
    def zoom_in(self):
        ...
        lil_rect.move_to(self.coords_to_point(*self.zoomed_rect_center_coords))
        self.wait()
        self.play(lil_rect.scale, 1.0 / self.zoom_factor, self.a_dot.scale, 1.0 / self.zoom_factor, run_time=3)
        self.wait()
    ...
    def show_limit_in_symbols(self):
        ...
        equals_zero_over_zero = OldTex('=', '{\\, 0 \\,', '\\over \\,', '0 \\,}')
        equals_q = OldTex(*'=???')
        ...
        self.play(ReplacementTransform(VGroup(*self.f_label).copy(), VGroup(frac_a.numerator)), ReplacementTransform(VGroup(*self.g_label).copy(), VGroup(frac_a.denominator)), Write(frac_a.over), run_time=2)
        self.wait()
        self.play(Write(equals_zero_over_zero))
        self.wait(2)
        self.play(ReplacementTransform(VGroup(*frac_a.get_parts_by_tex('a')), VGroup(lim.get_part_by_tex('a'))))
        self.play(Write(VGroup(*lim[:-1])))
        self.play(ReplacementTransform(VGroup(*lim.get_parts_by_tex('x')).copy(), VGroup(*frac_x.get_parts_by_tex('x'))))
    ...
    def show_tiny_nudge(self):
        ...
        dx_arrow = Arrow(self.coords_to_point(self.a_value, 0), self.coords_to_point(self.a_value + self.dx, 0), tip_length=arrow_tip_length, color=WHITE)
        dx_label = OldTex('dx')
        dx_label.scale(zoom_tex_scale_factor)
        dx_label.next_to(dx_arrow, UP, buff=z_small_buff)
        dx_label.shift(z_small_buff * RIGHT)
        df_arrow, dg_arrow = [Arrow(self.coords_to_point(self.a_value + self.dx, 0), self.coords_to_point(self.a_value + self.dx, graph.underlying_function(self.a_value + self.dx)), tip_length=arrow_tip_length, color=graph.get_color()) for graph in (self.f_graph, self.g_graph)]
        v_labels = []
        for char, arrow in (('f', df_arrow), ('g', dg_arrow)):
            label = OldTex('\\frac{d%s}{dx}' % char, '(', 'a', ')', '\\,dx')
            label.scale(zoom_tex_scale_factor)
    ...
    def show_derivative_ratio(self):
        ...
        deriv_ratio = OldTex('{ \\frac{df}{dx}', '(', 'a', ')', '\\,dx', '\\over \\,', '\\frac{dg}{dx}', '(', 'a', ')', '\\,dx}')
    ...
    def show_example(self):
        lhs = OldTex('\\lim', '_{x \\to', '0}', '{\\sin(', 'x', ')', '\\over \\,', 'x}')
        rhs = OldTex('=', '{\\cos(', '0', ')', '\\over \\,', '1}', '=', '1')
        ...
        brace_text.add_background_rectangle()
        name = OldTexText('``', "L'Hôpital's", ' rule', "''", arg_separator='')
        ...
        self.play(GrowFromCenter(brace), Write(brace_text))
        self.wait()
        self.play(Write(rhs[0]), ReplacementTransform(VGroup(*lhs[3:6]).copy(), VGroup(*rhs[1:4])))
        self.wait()
        self.play(ReplacementTransform(VGroup(*lhs[6:8]).copy(), VGroup(*rhs[4:6])))
        self.wait()
        self.play(Write(VGroup(*rhs[6:])))
        self.wait(2)
        example = VGroup(lhs, rhs, brace, brace_text)
        self.play(example.scale, 0.7, example.to_corner, DOWN + RIGHT, SMALL_BUFF, path_arc=7 * np.pi / 6)
    ...
    def show_bernoulli_and_lHopital(self):
        ...
        strike = Line(lhopital_name.get_left(), lhopital_name.get_right(), color=RED)
        ...
        self.wait()
        self.play(FadeIn(lhopital_image), bernoulli_image.next_to, arrow, DOWN, SMALL_BUFF, ShowCreation(arrow), FadeIn(dollars))
        for x in range(10):
            dollars.restore()
            self.play(*[Transform(*pair) for pair in zip(dollars, dollars[1:])] + [FadeOut(dollars[-1])])
    ...
    def get_frac(self, input_tex, color=WHITE):
        result = OldTex('{f', '(', input_tex, ')', '\\over \\,', 'g', '(', input_tex, ')}')
...
class CannotUseLHopital(TeacherStudentsScene):
    ...
    def construct(self):
        deriv = OldTex('{d(e^x)', '\\over \\,', 'dx}', '(', 'x', ')', '=', '\\lim', '_{h', '\\to 0}', '{e^{', 'x', '+', 'h}', '-', 'e^', 'x', '\\over \\,', 'h}')
        deriv.to_edge(UP)
        deriv.set_color_by_tex('x', RED)
        deriv.set_color_by_tex('dx', GREEN)
        deriv.set_color_by_tex('h', GREEN)
        deriv.set_color_by_tex('e^', YELLOW)
        self.play(Write(deriv), *it.chain(*[[pi.change_mode, 'pondering', pi.look_at, deriv] for pi in self.get_pi_creatures()]))
        self.wait()
        self.student_says("Use L'Hôpital's rule!", target_mode='hooray')
        self.wait()
        answer = OldTex('\\text{That requires knowing }', '{d(e^x)', '\\over \\,', 'dx}')
        answer.set_color_by_tex('e^', YELLOW)
        answer.set_color_by_tex('dx', GREEN)
        self.teacher_says(answer, bubble_config={'height': 2.5}, target_mode='hesitant')
        self.play_student_changes(*['confused'] * 3)
        self.wait(3)
...
class NextVideo(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        ftc = OldTex('F(b)', '-', 'F(a)', '=', '\\int_a^b', '{dF', '\\over \\,', 'dx}', '(x)', 'dx')
        for tex_mob in (integral, ftc):
            ...
            tex_mob.next_to(brace, DOWN)
        self.add(series)
        self.play(GrowFromCenter(brace), next_video.set_color, YELLOW, self.get_teacher().change_mode, 'raise_right_hand', self.get_teacher().look_at, next_video)
        self.play(Write(integral))
        self.wait(2)
        self.play(*[ReplacementTransform(VGroup(*integral.get_parts_by_tex(p1)), VGroup(*ftc.get_parts_by_tex(p2)), run_time=2, path_arc=np.pi / 2, rate_func=squish_rate_func(smooth, alpha, alpha + 0.5)) for alpha, (p1, p2) in zip(np.linspace(0, 0.5, 3), [('int', 'int'), ('f', 'F'), ('dx', 'dx')])] + [Write(VGroup(*ftc.get_parts_by_tex(part))) for part in ('-', '=', 'over', '(x)')])
        self.play_student_changes(*['pondering'] * 3)
        self.wait(3)

class Chapter7PatreonThanks(PatreonThanks):
    CONFIG = {'specific_patrons': ['Ali  Yahya', 'Meshal  Alshammari', 'CrypticSwarm    ', 'Kaustuv DeBiswas', 'Kathryn Schmiedicke', 'Nathan Pellegrin', 'Karan Bhargava', 'Ankit   Agarwal', 'Yu  Jun', 'Dave    Nicponski', 'Damion  Kistler', 'Juan    Benet', 'Othman  Alikhan', 'Justin Helps', 'Markus  Persson', 'Dan Buchoff', 'Derek   Dai', 'Joseph  John Cox', 'Luc Ritchie', 'Daan Smedinga', 'Jonathan Eppele', 'Albert Nguyen', 'Nils Schneider', 'Mustafa Mahdi', 'Mathew Bramson', 'Guido   Gambardella', 'Jerry   Ling', 'Mark    Govea', 'Vecht', 'Shimin Kuang', 'Rish    Kundalia', 'Achille Brighton', 'Ripta   Pasay', 'Felipe  Diniz']}
