...
from _2017.eoc.chapter2 import Car, MoveCar, ShowSpeedometer, IncrementNumber, GraphCarTrajectory, SecantLineToTangentLine, VELOCITY_COLOR, TIME_COLOR, DISTANCE_COLOR
...
class Chapter8OpeningQuote(OpeningQuote, PiCreatureScene):
    CONFIG = {'quote': [' One should never try to prove anything that \\\\ is not ', 'almost obvious', '. '], 'quote_arg_separator': '', 'highlighted_quote_terms': {'almost obvious': BLUE}, 'author': 'Alexander Grothendieck'}
    ...
    def construct(self):
        ...
        self.play(author.next_to, self.pi_creature.get_corner(UP + LEFT), UP, self.pi_creature.change_mode, 'raise_right_hand')
        self.wait(3)
        self.play(author.restore, self.pi_creature.change_mode, 'plain')
        self.play(words_copy.next_to, self.pi_creature, LEFT, MED_SMALL_BUFF, UP, self.pi_creature.change_mode, 'thinking')
        self.wait(2)
        self.play(Write(formula), self.pi_creature.change_mode, 'confused')
        self.wait()

    def get_formula(self):
        result = OldTex('{d(\\sin(\\theta)) \\over \\,', 'd\\theta}', '=', '\\lim_{', 'h', ' \\to 0}', '{\\sin(\\theta+', 'h', ') - \\sin(\\theta) \\over', ' h}', '=', '\\lim_{', 'h', ' \\to 0}', '{\\big[ \\sin(\\theta)\\cos(', 'h', ') + ', '\\sin(', 'h', ')\\cos(\\theta)\\big] - \\sin(\\theta) \\over', 'h}', '= \\dots')
...
class ThisVideo(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        next_video = series[8]
        deriv, integral, v_t, dt, equals, v_T = formula = OldTex('\\frac{d}{dT}', '\\int_0^T', 'v(t)', '\\,dt', '=', 'v(T)')
        ...
        self.play(this_video.shift, this_video.get_height() * DOWN / 2, this_video.set_color, YELLOW, self.teacher.change_mode, 'raise_right_hand')
        ...
        self.wait(3)
        self.play(this_video.restore, next_video.shift, next_video.get_height() * DOWN / 2, next_video.set_color, YELLOW, integral[0].copy().next_to, next_video, DOWN, MED_LARGE_BUFF, FadeOut(formula), *it.chain(*[[pi.change_mode, 'plain', pi.look_at, next_video] for pi in self.pi_creatures]))
        self.wait(2)

class InCarRestrictedView(ShowSpeedometer):
    CONFIG = {'speedometer_title_text': 'Your view'}
    ...
    def construct(self):
        ...
        self.play(MoveCar(car, self.point_B, rate_func=s_rate_func), IncrementNumber(time_label[1], run_time=8), MaintainPositionRelativeTo(window, car), *self.get_added_movement_anims(rate_func=v_rate_func, radians=-(16.0 / 70) * 4 * np.pi / 3), run_time=8)
...
class GraphDistanceVsTime(GraphCarTrajectory):
    CONFIG = {'y_min': 0, 'y_max': 100, 'y_axis_height': 6, 'y_tick_frequency': 10, 'y_labeled_nums': list(range(10, 100, 10)), 'y_axis_label': 'Distance (in meters)', 'x_min': -1, 'x_max': 9, 'x_axis_width': 9, 'x_tick_frequency': 1, 'x_leftmost_tick': None, 'x_labeled_nums': list(range(1, 9)), 'x_axis_label': '$t$', 'time_of_journey': 8, 'care_movement_rate_func': s_rate_func, 'num_graph_anchor_points': 100}

    def construct(self):
        self.setup_axes()
        graph = self.get_graph(s_func, color=DISTANCE_COLOR, x_min=0, x_max=8)
...
class PlotVelocity(GraphScene):
    CONFIG = {'x_min': -1, 'x_max': 9, 'x_axis_width': 9, 'x_tick_frequency': 1, 'x_labeled_nums': list(range(1, 9)), 'x_axis_label': '$t$', 'y_min': 0, 'y_max': 25, 'y_axis_height': 6, 'y_tick_frequency': 5, 'y_labeled_nums': list(range(5, 30, 5)), 'y_axis_label': 'Velocity in $\\frac{\\text{meters}}{\\text{second}}$', 'num_graph_anchor_points': 50}
    ...
    def add_speedometer(self):
        ...
        self.play(DrawBorderThenFill(speedometer, lag_ratio=0.5, rate_func=linear))
    ...
    def plot_points(self):
        ...
        for time, dot in zip(times, dots):
            ...
            dot_intro_anims += [ApplyMethod(pre_dot.set_fill, YELLOW, 1, run_time=0.1), ReplacementTransform(pre_dot, dot, run_time=0.9)]
        self.speedometer.move_needle_to_velocity(0)
        self.play(Succession(*dot_intro_anims, rate_func=linear), ApplyMethod(self.speedometer.move_needle_to_velocity, v_func(4), rate_func=squish_rate_func(lambda t: 1 - v_rate_func(t), 0, 0.95)), run_time=5)
        self.wait()
...
class Chapter2Wrapper(Scene):
    CONFIG = {'title': 'Chapter 2: The paradox of the derivative'}
...
class AskAboutAntiderivative(PlotVelocity):
    ...
    def write_s_formula(self):
        ...
        arrow = Arrow(ds_dt.get_bottom(), v_t.get_top(), color=WHITE)
    ...
    def write_antiderivative(self):
        ...
        self.play(PiCreatureSays(randy, words, target_mode='confused', bubble_config={'height': 3, 'width': 4}))
...
class Antiderivative(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ShowCreation(bottom_arc), Write(antiderivative), self.pi_creature.change_mode, 'raise_right_hand')
        self.wait(2)
        for pair in reversed(list(zip(functions, alt_functions))):
            self.play(Transform(*pair), self.pi_creature.change_mode, 'pondering')
        self.wait(2)
        self.pi_creature_says('But first!', target_mode='surprised', look_at=50 * OUT, added_anims=[group.to_edge, LEFT], run_time=1)
        self.wait()
...
class AreaUnderVGraph(PlotVelocity):
    ...
    def show_rects(self):
        rect_list = self.get_riemann_rectangles_list(self.v_graph, 7, max_dx=1.0, x_min=0, x_max=8)
...
class ConstantVelocityCar(Scene):
    ...
    def construct(self):
        ...
        self.add(car)
        self.wait()
        self.play(MoveCar(car, 7 * RIGHT + 3 * DOWN, run_time=5, rate_func=linear))
        self.wait()

class ConstantVelocityPlot(PlotVelocity):
    CONFIG = {'x_axis_label': 'Time', 'units_of_area_color': BLUE_E}
    ...
    def show_product(self):
        ...
        v_label = right_brace.get_text('$10 \\frac{\\text{meters}}{\\text{second}}$')
        ...
        self.play(GrowFromCenter(right_brace), Write(v_label))
        self.play(GrowFromCenter(top_brace), Write(t_label))
    ...
    def comment_on_area_wierdness(self):
        randy = Randolph()
        randy.to_corner(DOWN + LEFT)
        bubble = randy.get_bubble('Distance \\\\ is area?', bubble_type=ThoughtBubble, height=3, width=4, fill_opacity=1)
        bubble.content.scale(0.8)
        bubble.content.shift(SMALL_BUFF * UP)
        VGroup(bubble[-1], bubble.content).shift(1.5 * LEFT)
        self.play(FadeIn(randy))
        self.play(randy.change_mode, 'pondering')
        self.play(self.area_rect.set_color, YELLOW, *list(map(Animation, self.get_mobjects())), rate_func=there_and_back)
        self.play(Blink(randy))
        self.play(randy.change_mode, 'confused', randy.look_at, randy.bubble, ShowCreation(bubble), Write(bubble.content))
        self.wait()
        self.play(Blink(randy))
        self.wait()
        self.play(randy.change_mode, 'pondering', FadeOut(bubble), FadeOut(bubble.content))
        self.randy = randy

    def note_units(self):
        x_line, y_line = lines = VGroup(*[axis.copy() for axis in (self.x_axis, self.y_axis)])
        lines.set_color(TIME_COLOR)
        square = Square(stroke_color=BLACK, stroke_width=1, fill_color=self.units_of_area_color, fill_opacity=1)
        square.replace(VGroup(*[VectorizedPoint(self.coords_to_point(i, i)) for i in (0, 1)]), stretch=True)
        units_of_area = VGroup(*[square.copy().move_to(self.coords_to_point(x, y), DOWN + LEFT) for x in range(8) for y in range(10)])
        ...
        for FadeClass in (FadeIn, FadeOut):
            self.play(FadeClass(units_of_area, lag_ratio=0.5, run_time=3), Animation(self.s_label), self.randy.look_at, self.area_rect)
...
class PiecewiseConstantPlot(PlotVelocity):
    CONFIG = {'y_axis_label': '', 'min_graph_proportion': 0.1, 'max_graph_proportion': 0.8, 'num_riemann_approximations': 7, 'riemann_rect_fill_opacity': 0.75, 'tick_size': 0.2}
    ...
    def compute_distance_on_each_interval(self):
        rect_list = self.get_riemann_rectangles_list(self.v_graph, self.num_riemann_approximations, max_dx=1, x_min=0, x_max=8)
        ...
        for brace in (right_brace, top_brace):
            self.play(GrowFromCenter(brace), Write(brace.label, run_time=1))
            brace.add(brace.label)
            self.wait()
        self.play(ReplacementTransform(flat_rects, rects, run_time=2, lag_ratio=0.5), Animation(right_brace))
    ...
    def approximate_original_curve(self):
        rects = self.rects
        self.play(FadeOut(self.pw_constant_graph), *[m.restore for m in (self.v_graph, self.v_graph_label)] + [Animation(self.rects)])
    ...
    def revert_to_specific_approximation(self):
        ...
        self.wait()
        self.play(FadeIn(example_text, run_time=2, lag_ratio=0.5), ReplacementTransform(dt_label.copy(), example_text.get_part_by_tex('dt')))
        self.wait()
    ...
    def show_specific_rectangle(self):
        rects = self.rects
        rect = rects[4].copy()
        rect_top = Line(rect.get_corner(UP + LEFT), rect.get_corner(UP + RIGHT), color=self.v_graph.get_color())
        t_vals = [1, 1.25]
        ...
        for vect, label in zip([LEFT, RIGHT], t_labels):
            ...
            label.add(Arrow(label.get_edge_center(-vect), rect.get_corner(DOWN + vect), buff=SMALL_BUFF, tip_length=0.15, color=WHITE))
        v_lines = VGroup()
        h_lines = VGroup()
        ...
        self.play(t_label_copy.scale, 1.0 / 0.7, t_label_copy.next_to, self.v_graph_label, DOWN + LEFT, 0)
        self.wait()
        ...
        self.wait()
        for x in range(2):
            self.play(rect.stretch_to_fit_height, v_lines[1].get_height(), rect.move_to, rect.get_bottom(), DOWN, Animation(v_lines), run_time=4, rate_func=there_and_back)
        self.play(*list(map(FadeOut, [group[1] for group in (v_lines, h_lines, height_labels)])))
        self.play(v_lines[0].set_color, RED, rate_func=there_and_back)
        self.wait()
        ...
        self.wait(2)
        self.play(*list(map(FadeOut, [area, arrow, v_lines[0], h_lines[0], height_labels[0], rect, t_labels])))
    ...
    def show_v_dt_for_all_rectangles(self):
        ...
        for rect in rects_subset:
            ...
            anims = [rect.set_fill, None, 1, dt_brace_group.next_to, rect, DOWN, SMALL_BUFF]
            if last_rect is not None:
                anims += [last_rect.set_fill, None, 0.25, ReplacementTransform(last_brace, brace), ReplacementTransform(last_v_t, v_t)]
            else:
    ...
    def write_integral_symbol(self):
        ...
        for mob, tex in ((self.v_t, 'v(t)'), (self.dt_label, 'dt')):
            self.play(ReplacementTransform(mob.copy().set_color(YELLOW), integral.get_part_by_tex(tex), run_time=2))
        self.wait()
    ...
    def roles_of_dt(self):
        ...
        self.play(morty.change_mode, 'raise_right_hand', morty.look, UP + RIGHT, dt_copy.next_to, morty.get_corner(UP + RIGHT), UP, dt_copy.set_color, YELLOW)
        self.play(Blink(morty))
        self.play(ReplacementTransform(dt_copy.copy(), int_dt, run_time=2), morty.look_at, int_dt)
        self.wait(2)
        self.play(ReplacementTransform(dt_copy.copy(), self.dt_label), morty.look_at, self.dt_label)
        self.play(*[ApplyMethod(tick.shift, tick.get_height() * UP / 2, run_time=2, rate_func=squish_rate_func(there_and_back, alpha, alpha + 0.2)) for tick, alpha in zip(self.ticks, np.linspace(0, 0.8, len(self.ticks)))])
        self.wait()
        self.play(morty.change_mode, 'pondering', rects.set_fill, None, 0.75, *list(map(FadeOut, [dt_copy, self.v_t, self.v_t_brace])))
        rects.align_family(next_rects)
        for every_other_rect in rects[::2]:
            every_other_rect.set_fill(opacity=0)
        self.play(self.dt_brace.stretch, 0.5, 0, self.dt_brace.move_to, self.dt_brace, LEFT, ReplacementTransform(rects, next_rects, run_time=2, lag_ratio=0.5), Transform(self.ticks, self.get_ticks(next_rects), run_time=2, lag_ratio=0.5))
        self.rects = rects = next_rects
        self.wait()
        self.play(Blink(morty))
        self.play(*[ApplyFunction(lambda r: r.shift(0.2 * UP).set_fill(None, 1), rect, run_time=2, rate_func=squish_rate_func(there_and_back, alpha, alpha + 0.2)) for rect, alpha in zip(rects, np.linspace(0, 0.8, len(rects)))] + [morty.change_mode, 'thinking'])
        self.wait()
        self.morty = morty
    ...
    def what_does_sum_approach(self):
        ...
        self.wait()
        self.play(RemovePiCreatureBubble(morty, target_mode='plain'), *list(map(FadeOut, [cross, self.sum_word, self.ticks, self.dt_brace, self.dt_label])))
        self.play(FadeIn(brace), FadeIn(dt_to_0))
        for new_rects in self.rect_list[4:]:
            rects.align_family(new_rects)
            for every_other_rect in rects[::2]:
                every_other_rect.set_fill(opacity=0)
            self.play(Transform(rects, new_rects, run_time=2, lag_ratio=0.5), morty.look_at, rects)
            self.wait()
        self.play(Write(distance_words), ShowCreation(arrow), morty.change_mode, 'pondering', morty.look_at, distance_words)
        self.wait()
        self.play(Blink(morty))
        self.wait()
    ...
    def label_integral(self):
        ...
        self.wait()
        self.play(*[ApplyFunction(lambda r: r.shift(0.2 * UP).set_fill(None, 1), rect, run_time=3, rate_func=squish_rate_func(there_and_back, alpha, alpha + 0.2)) for rect, alpha in zip(self.rects, np.linspace(0, 0.8, len(self.rects)))] + [Animation(self.area_arrow), self.morty.change_mode, 'happy', self.morty.look_at, self.rects])
        self.wait()
    ...
    def get_ticks(self, rects):
        ticks = VGroup(*[Line(point + self.tick_size * UP / 2, point + self.tick_size * DOWN / 2) for t in np.linspace(0, 8, len(rects) + 1) for point in [self.coords_to_point(t, 0)]])
...
class CarJourneyApproximation(Scene):
    CONFIG = {'n_jumps': 5, 'bottom_words': 'Approximated motion (5 jumps)'}

    def construct(self):
        points = [5 * LEFT + v for v in (UP, 2 * DOWN)]
        cars = [Car().move_to(point) for point in points]
        h_line = Line(LEFT, RIGHT).scale(FRAME_X_RADIUS)
        words = [OldTexText('Real motion (smooth)').shift(3 * UP), OldTexText(self.bottom_words).shift(0.5 * DOWN)]
        words[1].set_color(GREEN)
        self.add(h_line, *cars + words)
        self.wait()
        self.play(*[MoveCar(car, point + 10 * RIGHT, run_time=5, rate_func=rf) for car, point, rf in zip(cars, points, [s_rate_func, self.get_approximated_rate_func(self.n_jumps)])])
        self.wait()
...
class LessWrongCarJourneyApproximation(CarJourneyApproximation):
    CONFIG = {'n_jumps': 20, 'bottom_words': 'Better approximation (20 jumps)'}
...
class HowDoesThisHelp(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play_student_changes('confused', 'angry', 'confused')
        self.wait(2)
...
class AltAreaUnderCurve(AreaUnderACurve):
    CONFIG = {'graph_origin': 2 * DOWN, 'x_min': -3, 'x_max': 3, 'x_axis_width': 12, 'y_max': 2, 'y_axis_height': 4}

    def func(self, x):
        return np.exp(-x ** 2)

class Chapter1Wrapper(Chapter2Wrapper):
    CONFIG = {'title': 'Essence of calculus, chapter 1'}

class AreaIsDerivative(PlotVelocity, ReconfigurableScene):
    CONFIG = {'y_axis_label': '', 'num_rects': 400, 'dT': 0.25, 'variable_point_label': 'T', 'area_opacity': 0.8}
    ...
    def write_integral(self):
        ...
        self.wait()
        self.change_area_bounds(new_t_max=8, rate_func=there_and_back, run_time=3)
        ...
        self.change_area_bounds(new_t_max=8, rate_func=linear, run_time=7.9)
        self.wait()
    ...
    def nudge_input(self):
        ...
        rect.replace(VGroup(VectorizedPoint(self.coords_to_point(new_T, 0)), self.right_v_line), stretch=True)
    ...
    def show_rectangle_approximation(self):
        ...
        self.wait()
        self.play(*[ReplacementTransform(mob, formula1.get_part_by_tex(tex), run_time=2) for mob, tex in [(self.ds_label, 'ds'), (self.ds_arrow, '='), (self.v_T_label_group[0].copy(), 'v(T)'), (self.dT_label_group[0].copy(), 'dT')]])
        self.wait()
        self.transition_to_alt_config(dT=self.dT / 5.0, transformation_kwargs={'run_time': 2})
        self.wait()
        self.play(*[ReplacementTransform(formula1.get_part_by_tex(tex), formula2.get_part_by_tex(tex)) for tex in ('ds', '=', 'v(T)', 'dT')] + [Write(formula2.get_part_by_tex('over'))])
        self.wait()
    ...
    def get_area(self, t_min, t_max):
        ...
        return self.get_riemann_rectangles(self.v_graph, x_min=t_min, x_max=t_max, dx=dx, stroke_width=0).set_fill(opacity=self.area_opacity)
    ...
    def change_area_bounds(self, new_t_min=None, new_t_max=None, **kwargs):
        ...
        def update_group(group, alpha):
            ...
            return group
        self.play(UpdateFromAlphaFunc(group, update_group), *list(map(Animation, self.foreground_mobjects)), **kwargs)
...
class DirectInterpretationOfDsDt(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for words, part in ((s_words, ds), (t_words, dt)):
            self.play(FadeIn(words, run_time=2, lag_ratio=0.5), self.students[1].change_mode, 'raise_right_hand')
...
class FindAntiderivative(Antiderivative):
    ...
    def introduce(self):
        ...
        self.wait(2)
        self.play(*[ReplacementTransform(rhs[i], expanded_rhs[j], run_time=2, path_arc=np.pi) for i, j in enumerate([1, 4, 0, 2, 3, 4])] + [self.pi_creature.change_mode, 'hesitant'])
        self.wait()
    ...
    def first_part(self):
        ...
        self.wait(2)
        self.play(FadeOut(self.q_marks), FadeIn(t_squared), self.v_part1.shift, DOWN + RIGHT)
        self.play(*[ReplacementTransform(t_squared[i].copy(), two_t[1 - i], run_time=2, path_arc=-np.pi / 6.0) for i in (0, 1)])
    ...
    def second_part(self):
        ...
        self.play(part1_group.scale, 0.5, part1_group.to_corner, UP + LEFT, self.pi_creature.change_mode, 'plain')
        ...
        self.play(*[ReplacementTransform(t_cubed[i].copy(), three_t_squared[j], path_arc=-np.pi / 6, run_time=2) for i, j in [(0, 1), (1, 0), (1, 2)]])
        self.wait()
    ...
    def combine(self):
        self.play(self.v_part1.restore, self.v_part2.restore, self.s_part1.scale, 2, self.s_part1.next_to, self.s_part2, LEFT, FadeOut(self.arcs_copy), FadeOut(self.words_copy), run_time=2)
    ...
    def add_plus_C(self):
        ...
        self.wait()
        self.play(s_group.next_to, plus_C.copy(), LEFT, GrowFromCenter(plus_C))
        self.wait()
...
class GraphSPlusC(GraphDistanceVsTime):
    ...
    def construct(self):
        ...
        graph.add(tangent)
        self.play(ShowCreation(v_line))
        self.play(graph.shift, 2 * DOWN, run_time=4, rate_func=there_and_back)
        self.play(Write(plus_C))
        self.play(graph.shift, 2 * UP, rate_func=there_and_back, run_time=4)
        self.wait()
...
class LowerBound(AreaIsDerivative):
    ...
    def mention_lower_bound(self):
        ...
        self.play(zero_label.scale, 1.5, zero_label.next_to, self.graph_origin, DOWN, MED_LARGE_BUFF, FadeOut(circle))
        self.wait()
    ...
    def write_antiderivative_difference(self):
        ...
        self.wait()
        self.play(ReplacementTransform(at_T.brace, big_brace), ReplacementTransform(antideriv_text, cancel_text))
        self.change_area_bounds(0, 0, run_time=4)
        self.wait()
        self.play(ReplacementTransform(big_brace, at_zero.brace), ReplacementTransform(cancel_text, happens_to_be_zero))
        self.wait(2)
        self.change_area_bounds(0, 8, run_time=2)
        self.play(Transform(self.integral, integral_at_eight), Transform(antideriv_diff, antideriv_diff_at_eight), MaintainPositionRelativeTo(at_zero.brace, at_zero), MaintainPositionRelativeTo(happens_to_be_zero, at_zero.brace))
        self.play(GrowFromCenter(at_eight.brace), Write(value_at_eight))
        self.wait(2)
        self.play(*list(map(FadeOut, [at_eight.brace, value_at_eight, at_zero.brace, happens_to_be_zero])))
    ...
    def show_alternate_antiderivative_difference(self):
        ...
        self.play(Transform(self.integral, new_integral), Transform(self.antideriv_diff, new_antideriv_diff))
        self.wait(3)
    ...
    def get_antiderivative_difference(self, lower_bound, upper_bound):
        ...
        for s in strings:
            ...
            parts.append(part)
        result = VGroup(OldTex('='), parts[0], OldTex('-'), parts[1])
...
class FundamentalTheorem(GraphScene):
    CONFIG = {'lower_bound': 1, 'upper_bound': 7, 'lower_bound_color': RED, 'upper_bound_color': GREEN, 'n_riemann_iterations': 6}
    ...
    def add_graph_and_integral(self):
        ...
        graph = self.get_graph(lambda x: -0.01 * x * (x - 3) * (x - 6) * (x - 12) + 3)
    ...
    def show_f_dx_sum(self):
        kwargs = {'x_min': self.lower_bound, 'x_max': self.upper_bound, 'fill_opacity': 0.75, 'stroke_width': 0.25}
        low_opacity = 0.25
        start_rect_index = 3
        ...
        self.play(*it.chain(list(map(GrowFromCenter, [f_brace, dx_brace])), list(map(Write, [f_brace.label, dx_brace.label]))))
        self.wait()
        for i in range(start_rect_index + 1, last_rect_index):
            self.play(rects[i - 1].set_fill, None, low_opacity, rects[i].set_fill, None, 1, f_brace.set_height, rects[i].get_height(), f_brace.next_to, rects[i], LEFT, 0, dx_brace.next_to, rects[i], DOWN, 0, *[MaintainPositionRelativeTo(brace.label, brace) for brace in (f_brace, dx_brace)])
        self.wait()
        self.play(*it.chain(list(map(FadeOut, [f_brace, dx_brace, f_brace.label, dx_brace.label])), [rects.set_fill, None, kwargs['fill_opacity']]))
    ...
    def write_antiderivative(self):
        ...
        self.wait(2)
        self.play(*it.chain([ReplacementTransform(deriv_F.copy(), part) for part in rhs.get_parts_by_tex('F')], [Write(VGroup(*rhs.get_parts_by_tex(tex))) for tex in '=()-']))
        for tex in ('b', 'a'):
    ...
    def show_integral_considering_continuum(self):
        ...
        self.wait()
        for x in range(2):
            self.play(*[ApplyFunction(lambda m: m.shift(MED_SMALL_BUFF * UP).set_fill(opacity=1), rect, run_time=3, rate_func=squish_rate_func(there_and_back, alpha, alpha + 0.2)) for rect, alpha in zip(self.rects, np.linspace(0, 0.8, len(self.rects)))])
        self.wait()

    def show_antiderivative_considering_bounds(self):
        self.play(self.integral.set_fill, None, 0.5, self.deriv.set_fill, None, 1, self.rhs.set_fill, None, 1)
...
class LetsRecap(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says("Let's recap", target_mode='hesitant')
        self.play_student_changes(*['happy'] * 3)
        self.wait(3)

class NegativeArea(GraphScene):
    CONFIG = {'x_axis_label': 'Time', 'y_axis_label': 'Velocity', 'graph_origin': 1.5 * DOWN + 5 * LEFT, 'y_min': -3, 'y_max': 7, 'small_dx': 0.01, 'sample_input': 5}
    ...
    def add_graph_and_area(self):
        graph = self.get_graph(lambda x: -0.02 * (x + 1) * (x - 3) * (x - 7) * (x - 10), x_min=0, x_max=8, color=VELOCITY_COLOR)
        area = self.get_riemann_rectangles(graph, x_min=0, x_max=8, dx=self.small_dx, start_color=BLUE_D, end_color=BLUE_D, fill_opacity=0.75, stroke_width=0)
        self.play(ShowCreation(graph), FadeIn(area, run_time=2, lag_ratio=0.5))
        self.graph = graph
        self.area = area
    ...
    def write_negative_area(self):
        ...
        words.set_color(RED)
        words.next_to(self.coords_to_point(7, -2), RIGHT)
        arrow = Arrow(words, self.coords_to_point(self.sample_input, -1))
    ...
    def write_v_dt(self):
        ...
        ds_arrow, v_arrow = arrows = VGroup(*[Arrow(negative.get_bottom(), equation.get_part_by_tex(tex).get_top(), color=RED) for tex in ('ds', 'v(t)')])
    ...
    def show_rectangle(self):
        rect_list = self.get_riemann_rectangles_list(self.graph, x_min=0, x_max=8, n_iterations=6, start_color=BLUE_D, end_color=BLUE_D, fill_opacity=0.75)
        rects = rect_list[0]
        ...
        self.play(GrowFromCenter(dt_brace), GrowFromCenter(v_brace), Write(dt_label), Write(v_label))
        self.wait(2)
        self.play(*it.chain([FadeIn(r) for r in rects if r is not rect], list(map(FadeOut, [dt_brace, v_brace, dt_label, v_label]))))
        self.wait()
        for new_rects in rect_list[1:]:
            self.transform_between_riemann_rects(rects, new_rects)
        self.wait()
    ...
    def write_signed_area(self):
        ...
        symbols = VGroup(*[OldTex(sym).move_to(self.coords_to_point(*coords)) for sym, coords in [('+', (1, 2)), ('-', (5, -1)), ('+', (7.6, 0.5))]])
        self.play(Write(words))
...
class NextVideo(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(series, lag_ratio=0.5))
        self.play(next_video.set_color, YELLOW, next_video.shift, next_video.get_height() * DOWN / 2, self.teacher.change_mode, 'raise_right_hand')
        self.play(Write(integral))
        self.wait(5)
...
class Thumbnail(Chapter1Thumbnail):
    CONFIG = {'x_axis_label': '', 'y_axis_label': '', 'graph_origin': 1.5 * DOWN + 4 * LEFT, 'y_axis_height': 5, 'x_max': 5, 'x_axis_width': 11}
    ...
    def construct(self):
        ...
        graph = self.get_graph(self.func)
        rects = self.get_riemann_rectangles(graph, x_min=0, x_max=4, dx=0.25)
