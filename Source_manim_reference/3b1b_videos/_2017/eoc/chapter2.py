...
class IncrementNumber(Succession):
    CONFIG = {'start_num': 0, 'changes_per_second': 1, 'run_time': 11}

    def __init__(self, num_mob, **kwargs):
        digest_config(self, kwargs)
        n_iterations = int(self.run_time * self.changes_per_second)
        new_num_mobs = [OldTex(str(num)).move_to(num_mob, LEFT) for num in range(self.start_num, self.start_num + n_iterations)]
        transforms = [Transform(num_mob, new_num_mob, run_time=1.0 / self.changes_per_second, rate_func=squish_rate_func(smooth, 0, 0.5)) for new_num_mob in new_num_mobs]
        Succession.__init__(self, *transforms, **{'rate_func': None, 'run_time': self.run_time})
...
class Chapter2OpeningQuote(OpeningQuote):
    CONFIG = {'quote': ['So far as the theories of mathematics are about', 'reality,', 'they are not', 'certain;', 'so far as they are', 'certain,', 'they are not about', 'reality.'], 'highlighted_quote_terms': {'reality,': BLUE, 'certain;': GREEN, 'certain,': GREEN, 'reality.': BLUE}, 'author': 'Albert Einstein'}
...
class Introduction(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.student_says('Instantaneous rate of change', '?', index=0)
        self.wait()
        bubble = self.get_students()[0].bubble
        phrase = bubble.content[0]
        bubble.content.remove(phrase)
        self.play(FadeOut(bubble), FadeOut(bubble.content), FadeOut(goals), phrase.center, phrase.scale, 1.5, phrase.to_edge, UP, *it.chain(*[[pi.change_mode, mode, pi.look_at, FRAME_Y_RADIUS * UP] for pi, mode in zip(self.get_pi_creatures(), ['speaking', 'pondering', 'confused', 'confused'])]))
        self.wait()
        ...
        def get_clock_anim(run_time=3):
            return ClockPassesTime(clock, hours_passed=0.4 * run_time, run_time=run_time)
        self.play(FadeIn(clock))
        self.play(change.set_color_by_gradient, BLUE, YELLOW, GrowFromCenter(change_brace), Write(change_description), get_clock_anim())
        self.play(get_clock_anim(1))
        stopped_clock = clock.copy()
        stopped_clock.next_to(instantaneous_description, DOWN)
        self.play(instantaneous.set_color, BLUE, GrowFromCenter(instantaneous_brace), Transform(change_description.copy(), instantaneous_description), clock.copy().next_to, instantaneous_description, DOWN, get_clock_anim(3))
        self.play(get_clock_anim(12))

class FathersOfCalculus(Scene):
    CONFIG = {'names': ['Barrow', 'Newton', 'Leibniz', 'Cauchy', 'Weierstrass'], 'picture_height': 2.5}
...
class IntroduceCar(Scene):
    CONFIG = {'should_transition_to_graph': True, 'show_distance': True, 'point_A': DOWN + 4 * LEFT, 'point_B': DOWN + 5 * RIGHT}
    ...
    def construct(self):
        ...
        self.play(MoveCar(car, point_B, run_time=10), IncrementNumber(time_label[1], run_time=11), *self.get_added_movement_anims())
        front_line = car.get_front_line()
        self.play(ShowCreation(front_line))
        self.play(FadeOut(front_line))
        ...
        if self.should_transition_to_graph:
            self.play(car.move_to, point_A, FadeOut(time_label), FadeOut(distance_brace), FadeOut(distance))
...
class GraphCarTrajectory(GraphScene):
    CONFIG = {'x_min': 0, 'x_max': 10, 'x_labeled_nums': list(range(1, 11)), 'x_axis_label': 'Time (seconds)', 'y_min': 0, 'y_max': 110, 'y_tick_frequency': 10, 'y_labeled_nums': list(range(10, 110, 10)), 'y_axis_label': 'Distance traveled \\\\ (meters)', 'graph_origin': 2.5 * DOWN + 5 * LEFT, 'default_graph_colors': [DISTANCE_COLOR, VELOCITY_COLOR], 'default_derivative_color': VELOCITY_COLOR, 'time_of_journey': 10, 'care_movement_rate_func': smooth}
    ...
    def introduce_graph(self, graph, origin):
        ...
        self.add(car)
        self.play(ShowCreation(graph, rate_func=linear), MoveCar(car, car_target, rate_func=self.care_movement_rate_func), UpdateFromFunc(h_line, h_update), UpdateFromFunc(v_line, v_update), run_time=self.time_of_journey)
        self.wait()
    ...
    def comment_on_slope(self, graph, origin):
        ...
        for x in range(1, 10):
            ...
            self.play(rect.move_to, self.coords_to_point(curr_time, 0), DOWN + LEFT, Transform(change_lines, new_change_lines))
            if curr_time == 5:
    ...
    def show_velocity_graph(self):
        ...
        def get_velocity_label(v_graph):
            result = self.get_graph_label(v_graph, label='v(t)', direction=UP + RIGHT, x_val=5, buff=SMALL_BUFF)
            self.remove(result)
            return result
        ...
        graph_funcs = [shallow_slope, steep_slope, double_smooth_graph_function]
        for graph_func in graph_funcs:
            new_graph = self.get_graph(graph_func, color=DISTANCE_COLOR)
            ...
            self.play(Transform(velocity_graph, new_velocity_graph), Transform(label, new_velocity_label))
            self.wait(2)
        self.play(self.s_graph.restore)
        self.play(velocity_graph.restore, label.restore)
        self.wait(2)
...
class ShowSpeedometer(IntroduceCar):
    CONFIG = {'num_ticks': 8, 'tick_length': 0.2, 'needle_width': 0.1, 'needle_height': 0.8, 'should_transition_to_graph': False, 'show_distance': False, 'speedometer_title_text': 'Speedometer'}
    ...
    def get_added_movement_anims(self, **kwargs):
        ...
        default_kwargs = {'about_point': center, 'radians': -np.pi / 2, 'run_time': 10, 'rate_func': there_and_back}
...
class SnapshotOfACar(Scene):
    ...
    def construct(self):
        ...
        flash_box = Rectangle(width=FRAME_WIDTH, height=FRAME_HEIGHT, stroke_width=0, fill_color=WHITE, fill_opacity=1)
        speed_lines = VGroup(*[Line(point, point + 0.5 * LEFT) for point in [0.5 * UP + 0.25 * RIGHT, ORIGIN, 0.5 * DOWN + 0.25 * RIGHT]])
        question = OldTexText('\n            How fast is\n            this car going?\n        ')
...
class CompareTwoTimes(Scene):
    CONFIG = {'start_distance': 30, 'start_time': 4, 'end_distance': 50, 'end_time': 5, 'fade_at_the_end': True}
    ...
    def introduce_states(self):
        ...
        dividers = VGroup(Line(FRAME_X_RADIUS * LEFT, RIGHT), Line(RIGHT + FRAME_Y_RADIUS * UP, RIGHT + FRAME_Y_RADIUS * DOWN))
    ...
    def show_equation(self):
        ...
        formula = OldTex('\\frac{(%s - %s) \\text{ meters}}{(%s - %s) \\text{ seconds}}' % (str(self.end_distance), str(self.start_distance), str(self.end_time), str(self.start_time)))
        ...
        VGroup(VGroup(*formula[1:1 + ed_len]), VGroup(*formula[2 + ed_len:2 + ed_len + sd_len])).set_color(DISTANCE_COLOR)
        VGroup(VGroup(*formula[-2 - seconds_len - et_len - st_len:-2 - seconds_len - st_len]), VGroup(*formula[-1 - seconds_len - st_len:-1 - seconds_len])).set_color(TIME_COLOR)
        down_arrow1 = OldTex('\\Downarrow')
        down_arrow2 = OldTex('\\Downarrow')
        group = VGroup(velocity, down_arrow1, change_over_change, down_arrow2, formula)
        group.arrange(DOWN)
...
class VelocityAtIndividualPointsVsPairs(GraphCarTrajectory):
    CONFIG = {'start_time': 6.5, 'end_time': 3, 'dt': 1.0}
    ...
    def construct(self):
        ...
        self.play(ShowCreation(velocity_graph))
        velocity_label = self.label_graph(velocity_graph, label='v(t)', proportion=self.start_time / 10.0, direction=UP, buff=MED_SMALL_BUFF)
    ...
    def show_individual_times_to_velocity(self, velocity_graph):
        ...
        def line_update(line, alpha):
            ...
            line.put_start_and_end_on(self.coords_to_point(time, 0), self.input_to_graph_point(time, graph=velocity_graph))
...
class CompareTwoVerySimilarTimes(CompareTwoTimes):
    CONFIG = {'start_distance': 20, 'start_time': 3, 'end_distance': 20.21, 'end_time': 3.01, 'fade_at_the_end': False}

    def construct(self):
        CompareTwoTimes.construct(self)
        formula = self.formula
        ds_symbols, dt_symbols = [VGroup(*[mob for mob in formula if mob.get_color() == Color(color)]) for color in (DISTANCE_COLOR, TIME_COLOR)]
...
class DsOverDtGraphically(GraphCarTrajectory, ZoomedScene):
    CONFIG = {'dt': 0.1, 'zoom_factor': 4, 'start_time': 3, 'end_time': 7}

    def construct(self):
        self.setup_axes(animate=False)
        distance_graph = self.graph_function(lambda t: 100 * smooth(t / 10.0), animate=False)
        distance_label = self.label_graph(distance_graph, label='s(t)', proportion=0.9, direction=UP + LEFT, buff=SMALL_BUFF)
        input_point_line = self.get_vertical_line_to_graph(self.start_time, line_kwargs={'dash_length': 0.02, 'stroke_width': 4, 'color': WHITE})
        ...
        velocity_graph = self.get_derivative_graph()
        t_tick_marks = VGroup(*[Line(UP, DOWN, color=TIME_COLOR, stroke_width=3).scale(0.1).move_to(self.coords_to_point(t, 0)) for t in np.linspace(0, 10, 75)])
        v_line_at_t, v_line_at_t_plus_dt = [self.get_vertical_line_to_graph(time, line_class=Line, line_kwargs={'color': MAROON_B}) for time in (self.end_time, self.end_time + self.dt)]
        ...
        def input_point_line_update(line, alpha):
            ...
            line.put_start_and_end_on(self.coords_to_point(time, 0), self.input_to_graph_point(time))

        def ds_dt_group_update(group, alpha):
            time = interpolate(self.start_time, self.end_time, alpha)
            new_group = get_ds_dt_group(time)
            Transform(group, new_group).update(1)
        self.play(UpdateFromAlphaFunc(input_point_line, input_point_line_update), UpdateFromAlphaFunc(ds_dt_group, ds_dt_group_update), UpdateFromFunc(self.little_rectangle, align_little_rectangle_on_ds_dt_group), run_time=6)
        ...
        self.wait(2)
        self.play(ShowCreation(velocity_graph))
        velocity_label = self.label_graph(velocity_graph, label='v(t)', proportion=0.6, direction=DOWN + LEFT, buff=SMALL_BUFF)
        self.wait(2)
        ...
        def double_smooth_graph_function(t):
            if t < 5:
                ...
                return 50 * (1 + smooth((t - 5) / 5.0))
        graph_funcs = [steep_slope, sin_wiggle, double_smooth_graph_function]
        for graph_func in graph_funcs:
        ...
        self.play(velocity_graph.restore, velocity_label.restore)
        randy = Randolph()
...
class DefineTrueDerivative(Scene):
    ...
    def get_fraction(self, dt_string):
        ...
        part_lengths = [0, len('s(t+'), 1, len(dt_string) - 1, len(')-s(t)_'), 1, len(dt_string) - 1]
...
class SecantLineToTangentLine(GraphCarTrajectory, DefineTrueDerivative):
    CONFIG = {'start_time': 6, 'end_time': 2, 'alt_end_time': 10, 'start_dt': 2, 'end_dt': 0.01, 'secant_line_length': 10}
    ...
    def get_ds_dt_group(self, dt, animate=False):
        ...
        if animate:
            self.play(ShowCreation(dt_line), Write(dt), ShowCreation(dots[0]))
            self.play(ShowCreation(ds_line), Write(ds), ShowCreation(dots[1]))
    ...
    def add_graph(self):
        ...
        self.graph_label = self.get_graph_label(self.graph, 's(t)', x_val=self.x_max, direction=DOWN + RIGHT, buff=SMALL_BUFF)
    ...
    def draw_axes(self):
        ...
        self.play(Write(VGroup(self.x_axis, self.y_axis, self.graph, self.graph_label), run_time=4))
        self.wait()
    ...
    def show_tangent_line(self):
        ...
        def update_as_tangent_line(ds_dt_group, alpha):
            ...
            Transform(ds_dt_group, new_group).update(1)
        self.play(UpdateFromAlphaFunc(ds_dt_group, update_as_tangent_line), run_time=8, rate_func=there_and_back)
        self.wait()
        what_dt_is_not_text = self.what_this_is_not_saying()
        self.wait()
        self.play(UpdateFromAlphaFunc(ds_dt_group, update_ds_dt_group), run_time=8, rate_func=lambda t: 1 - there_and_back(t))
        self.wait()
        self.play(FadeOut(what_dt_is_not_text))
        v_line = self.get_vertical_line_to_graph(self.curr_time, self.graph, line_class=Line, line_kwargs={'color': MAROON_B, 'stroke_width': 3})

        def v_line_update(v_line):
            v_line.put_start_and_end_on(self.coords_to_point(self.curr_time, 0), self.input_to_graph_point(self.curr_time, self.graph))
            return v_line
        ...
        for end_time in (self.alt_end_time, original_end_time, self.start_time):
            ...
            self.play(UpdateFromAlphaFunc(ds_dt_group, update_as_tangent_line), UpdateFromFunc(v_line, v_line_update), run_time=abs(self.curr_time - self.end_time))
...
class LeadIntoASpecificExample(TeacherStudentsScene, SecantLineToTangentLine):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Write(deriv_def), *it.chain(*[[pi.change_mode, 'thinking', pi.look_at, dot] for pi in self.get_students()]))
        self.random_blink(3)

class TCubedExample(SecantLineToTangentLine):
    CONFIG = {'y_axis_label': 'Distance', 'y_min': 0, 'y_max': 16, 'y_tick_frequency': 1, 'y_labeled_nums': list(range(0, 17, 2)), 'x_min': 0, 'x_max': 4, 'x_labeled_nums': list(range(1, 5)), 'graph_origin': 2.5 * DOWN + 6 * LEFT, 'start_time': 2, 'end_time': 0.5, 'start_dt': 0.25, 'end_dt': 0.001, 'secant_line_length': 0.01}
    ...
    def add_ds_dt_group(self):
        ...
        self.wait()
        self.play(ShowCreation(arrow), Write(nonzero_size))
        self.wait(2)
    ...
    def get_vertical_lines(self):
        return VGroup(*[self.get_vertical_line_to_graph(time, line_class=DashedLine, line_kwargs={'color': WHITE, 'dash_length': 0.05}) for time in (self.start_time, self.start_time + self.start_dt)])
    ...
    def show_expansion(self):
        ...
        term_lens = [len('23+'), len('3(2)2dt'), len('+3(2)(dt)2+'), len('(dt)3'), len('-23'), len('_'), len('dt')]
        terms = [VGroup(*expression[i1:i2]) for i1, i2 in zip([0] + list(np.cumsum(term_lens)), np.cumsum(term_lens))]
        dts = [VGroup(*terms[1][-2:]), VGroup(*terms[2][6:8]), VGroup(*terms[3][1:3]), terms[-1]]
        VGroup(*dts).set_color(TIME_COLOR)
        two_cubed_terms = (terms[0], terms[4])
        for term in terms:
            self.play(FadeIn(term))
            self.wait()
        self.play(*it.chain(*[[tc.scale, 1.3, tc.get_corner(vect), tc.set_color, RED] for tc, vect in zip(two_cubed_terms, [DOWN + RIGHT, DOWN + LEFT])]))
        self.play(*list(map(FadeOut, two_cubed_terms)))
        numerator = VGroup(*terms[1:4])
        self.play(numerator.scale, 1.4, numerator.get_bottom(), terms[-1].scale, 1.4, terms[-1].get_top())
        self.wait(2)
        faders = VGroup(terms[-1], VGroup(*terms[1][-2:]), terms[2][-2], terms[3][-1])
        new_exp = OldTex('2').replace(faders[-1], dim_to_match=1)
        self.play(faders.set_color, BLACK, FadeIn(new_exp), run_time=2)
        self.wait()
        terms[3].add(new_exp)
        shift_val = 0.4 * DOWN
        self.play(FadeOut(terms[-2]), terms[1].shift, shift_val + 0.45 * RIGHT, terms[2].shift, shift_val, terms[3].shift, shift_val)
        arrow = Arrow(self.lhs[4].get_bottom(), terms[1][2].get_top(), color=WHITE, buff=MED_SMALL_BUFF)
        ...
        self.play(UpdateFromAlphaFunc(self.ds_dt_group, ds_dt_group_update), faders.fade, 0.7, run_time=5)
        self.wait(2)
        ...
        self.play(FadeOut(arrow), FadeOut(faders), MoveToTarget(deriv_term), MoveToTarget(lhs_copy))
        arrow = Arrow(self.rhs.get_bottom(), deriv_term.target.get_top(), buff=MED_SMALL_BUFF, color=WHITE)
        ...
        def update_as_tangent_line(group, alpha):
            ...
            new_group = self.get_ds_dt_group(self.end_dt)
            Transform(group, new_group).update(1)
        self.play(UpdateFromAlphaFunc(self.ds_dt_group, update_as_tangent_line), run_time=5, rate_func=there_and_back)
        self.wait(2)
    ...
    def react_to_simplicity(self):
        ...
        self.play(Blink(morty))
        self.play(morty.change_mode, 'happy', *list(map(FadeOut, [morty.bubble, morty.bubble.content])))
        ...
        for mob in (numerator, denominator, self.approach_text, self.deriv_term):
            ...
            self.play(MoveToTarget(mob, rate_func=there_and_back, run_time=1.5), morty.look_at, mob)
            self.wait()
        self.play(Blink(morty))
        self.wait()
...
class YouWouldntDoThisEveryTime(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(this_video.restore, next_video.set_color, YELLOW, next_video.shift, 0.5 * DOWN)
        self.wait(2)
...
class ContrastConcreteDtWithLimit(Scene):
    ...
    def construct(self):
        ...
        for title, vect in ((l_title, LEFT), (r_title, RIGHT)):
            ...
            self.add(title)
        l_formula = OldTex('\n            \\frac{d(t^3)}{dt} = \n            \\frac{\n                t^3+\n                3t^2 \\, dt + \n                3t \\, (dt)^2 + \n                (dt)^3\n                - t^3\n            }{dt}\n        ')
        VGroup(*it.chain(l_formula[6:8], l_formula[15:17], l_formula[21:23], l_formula[27:29], l_formula[35:37])).set_color(TIME_COLOR)
        ...
        triplets = [(l_formula, l_brace, l_text), (r_formula, r_brace, r_text)]
...
class TimeForAnActualParadox(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(teacher.change_mode, 'raise_right_hand', teacher.look_at, words, Write(words))
        self.play(*list(map(Write, [arrow, paradoxes])))
        self.play(*it.chain(*[[pi.change_mode, mode, pi.look_at, words] for pi, mode in zip(self.get_students(), ['pondering', 'happy', 'hesitant'])]))
        self.wait(4)

class ParadoxAtTEquals0(TCubedExample):
    CONFIG = {'tangent_line_length': 20}
    ...
    def draw_graph(self):
        ...
        self.play(MoveCar(car, car_target_point, rate_func=lambda t: (t * graph_x_max) ** 3), ShowCreation(graph, rate_func=linear), UpdateFromFunc(h_line, h_line_update), UpdateFromFunc(v_line, v_line_update), run_time=5)
    ...
    def show_derivative_text(self):
        derivative = OldTex('\\frac{ds}{dt}(t) = 3t^2', '= 3(0)^2', '= 0', '\\frac{\\text{m}}{\\text{s}}')
    ...
    def show_tangent_line(self):
        ...
        self.play(UpdateFromAlphaFunc(line, line_update), UpdateFromAlphaFunc(dot, dot_update), run_time=4)
        self.wait(2)
    ...
    def if_not_then_when(self):
        ...
        self.play(Blink(morty))
        self.play(MoveCar(self.car, self.coords_to_point(0, 1), rate_func=lambda t: (3 * t) ** 3, run_time=5))
        self.play(morty.change_mode, 'pondering', FadeOut(morty.bubble), FadeOut(morty.bubble.content))
    ...
    def single_out_question(self):
        ...
        self.play(everything.fade, 0.8, question.center, morty.change_mode, 'confused', morty.look_at, ORIGIN)
        self.play(Blink(morty))
        for word in (change_word, moment_word):
            self.play(word.scale, 1.2, word.set_color, YELLOW, rate_func=there_and_back, run_time=1.5)
        self.wait(2)
        self.play(everything.restore, FadeOut(question), morty.change_mode, 'raise_right_hand', morty.look_at, self.derivative)
...
class TinyMovement(ZoomedScene):
    CONFIG = {'distance': 0.05, 'distance_label': '(0.1)^3 = 0.001', 'time_label': '0.1'}
    ...
    def get_ratio(self, power=1):
        ...
        lengths = [0, len('('), len(dt), len(')3meters_'), len(dt), len('seconds='), len(ds_dt), len('meters_second')]
        result = VGroup(*[VGroup(*expression[i1:i2]) for i1, i2 in zip(np.cumsum(lengths), np.cumsum(lengths)[1:])])
...
class Promotion(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(self.pi_creature.change_mode, 'raise_right_hand', *[DrawBorderThenFill(submob, run_time=3, rate_func=squish_rate_func(double_smooth, a, a + 0.5)) for submob, a in zip(aops_logo, np.linspace(0, 0.5, len(aops_logo)))])
        self.play(Write(url))
        ...
        self.play(PiCreatureSays(mathy, '', bubble_config={'width': 5}, look_at=morty.eyes), aops_logo.shift, 1.5 * UP + 0.5 * RIGHT)
        self.change_mode('happy')
        self.wait(2)
        self.play(Blink(mathy))
        self.wait()
        self.play(RemovePiCreatureBubble(mathy, target_mode='happy'), aops_logo.to_corner, UP + RIGHT, aops_logo.shift, MED_SMALL_BUFF * DOWN)
        self.play(mathy.look_at, morty.eyes, morty.look_at, mathy.eyes)
        self.wait(2)
        self.play(Animation(VectorizedPoint(book_spot)), mathy.change, 'raise_right_hand', book_spot, morty.change, 'pondering')
        self.wait(3)
        self.play(Blink(mathy))
        self.wait(7)
        self.play(ShowCreation(rect), morty.restore, morty.change, 'happy', rect, FadeOut(mathy))
        self.wait(10)
        self.play(ShowCreation(url_rect))
        self.play(FadeOut(url_rect), url.get_part_by_tex('3blue1brown').set_color, BLUE)
        self.wait(3)
