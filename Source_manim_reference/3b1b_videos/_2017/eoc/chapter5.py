...
class ExpFootnoteOpeningQuote(OpeningQuote):
    CONFIG = {'quote': ['Who has not been amazed to learn that the function', '$y = e^x$,', 'like a phoenix rising again from its own', 'ashes, is its own derivative?'], 'highlighted_quote_terms': {'$y = e^x$': MAROON_B}, 'author': 'Francois le Lionnais'}
...
class LastVideo(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        this_video = series[3]
        known_formulas = VGroup(*list(map(Tex, ['\\frac{d(x^n)}{dx} = nx^{n-1}', '\\frac{d(\\sin(x))}{dx} = \\cos(x)'])))
        known_formulas.arrange(DOWN, buff=MED_LARGE_BUFF)
        ...
        self.add(series)
        self.play(last_video_brace.restore, last_video.set_color, YELLOW, self.get_teacher().change_mode, 'raise_right_hand')
        self.play(Write(known_formulas))
        self.wait()
        self.student_says(exp_question, index=1, added_anims=[self.get_teacher().change_mode, 'pondering'])
        self.wait(3)
        e_to_x = exp_question.get_part_by_tex('e^x')
        self.play(self.teacher.change_mode, 'raise_right_hand', e_to_x.scale, 1.5, e_to_x.set_color, YELLOW, e_to_x.next_to, self.teacher.get_corner(UP + LEFT), UP)
        self.wait(2)
...
class DoublingPopulation(PiCreatureScene):
    CONFIG = {'time_color': YELLOW, 'pi_creature_grid_dimensions': (8, 8), 'pi_creature_grid_height': 6}
    ...
    def introduce_pi_creatures(self):
        ...
        self.wait()
        self.play(Transform(self.function, self.mass_function), Transform(self.pop_brace, self.mass_brace), Transform(self.pop_brace.text, self.mass_brace.text))
        self.wait()
    ...
    def ask_about_dM_dt(self):
        ...
        self.play(FadeOut(self.pop_brace), FadeOut(self.pop_brace.text), Write(dM_dt_question))
        self.wait(3)
        for mob in (dM, dt):
    ...
    def growth_per_day(self):
        ...
        self.play(FadeOut(self.dM_dt_question), FadeOut(self.population_size_descriptor), FadeIn(day_to_day))
    ...
    def let_day_pass_and_highlight_new_creatures(self, frac):
        ...
        rect = Rectangle(color=GREEN, fill_color=BLUE, fill_opacity=0.3)
    ...
    def relate_growth_rate_to_pop_size(self):
        ...
        VGroup(false_deriv[0][3], false_deriv[2][-1], difference_eq[0][1], difference_eq[0][-2], difference_eq[2][-1], difference_eq[2][-1], real_deriv[0][1], real_deriv[0][-2]).set_color(YELLOW)
        VGroup(difference_eq[0][3], difference_eq[1][-1], real_deriv[0][3], real_deriv[0][4], real_deriv[1][-2], real_deriv[1][-1]).set_color(GREEN)
        expressions = [false_deriv, difference_eq, real_deriv]
        text_arg_list = [('Tempting', '...'), ('Rate of change', '\\\\ over one full day'), ('Rate of change', '\\\\ in a small time')]
        for expression, text_args in zip(expressions, text_arg_list):
            expression.next_to(self.function, DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
        ...
        self.play(*list(map(FadeIn, [false_deriv, false_deriv.brace, false_deriv.brace_text])))
        self.wait()
        ...
        for expression in (difference_eq, real_deriv):
            ...
            self.play(Transform(false_deriv, expression), Transform(false_deriv.brace, expression.brace), Transform(false_deriv.brace_text, expression.brace_text))
            self.wait(3)
        self.reset()
        ...
        self.play(*[ApplyFunction(lambda pi: pi.change_mode('pondering').look_at(real_deriv), pi, run_time=2, rate_func=squish_rate_func(smooth, a, a + 0.5)) for pi in self.get_pi_creatures() for a in [0.5 * random.random()]])
        self.wait(3)

    def create_pi_creatures(self):
        width, height = self.pi_creature_grid_dimensions
        creature_array = VGroup(*[VGroup(*[PiCreature(mode='plain') for y in range(height)]).arrange(UP, buff=MED_LARGE_BUFF) for x in range(width)]).arrange(RIGHT, buff=MED_LARGE_BUFF)
    ...
    def let_one_day_pass(self, run_time=2):
        ...
        for old_pi, pi in zip(on_screen_creatures, new_creatures):
            ...
            rate_func = squish_rate_func(smooth, alpha * prop, alpha * prop + (1 - alpha))
    ...
    def get_population_size_descriptor(self):
        ...
        label = brace.get_text('$2^%d$' % int(np.log2(n)), '$=%d$' % n)
        brace.add(label)
        return brace
    ...
    def get_from_day_to_day_label(self):
        ...
        top_words.next_to(self.function, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        top_words[1].set_color(GREEN)
        bottom_words = OldTex(str(2 ** curr_day), '\\text{ creatures}', '\\over {1 \\text{ day}}')
...
class GraphOfTwoToT(GraphScene):
    CONFIG = {'x_axis_label': '$t$', 'y_axis_label': '$M$', 'x_labeled_nums': list(range(1, 7)), 'y_labeled_nums': list(range(8, 40, 8)), 'x_max': 6, 'y_min': 0, 'y_max': 32, 'y_tick_frequency': 2, 'graph_origin': 2.5 * DOWN + 5 * LEFT}
    ...
    def construct(self):
        ...
        self.graph = graph
        graph_label = self.get_graph_label(graph, 'M(t) = 2^t', direction=LEFT)
        ...
        self.wait()
        self.play(FadeIn(slope_label, run_time=4, lag_ratio=0.5), ReplacementTransform(height_label.copy(), slope_label.get_part_by_tex('2^')))
        self.wait()
        threes = VGroup(height_label[1], slope_label[2][1])
        ts = VGroup(*[OldTex('t').set_color(YELLOW).scale(0.75).move_to(three) for three in threes])
        ...
        def update_label_group(group, alpha):
            ...
            return group
        self.play(UpdateFromAlphaFunc(label_group, update_label_group, run_time=3))
        self.play(UpdateFromAlphaFunc(label_group, update_label_group, run_time=3, rate_func=lambda t: 1 - 1.5 * smooth(t)))

    def get_label_group(self, t):
        graph = self.graph
        v_line = self.get_vertical_line_to_graph(t, graph, color=YELLOW)
        brace = Brace(v_line, RIGHT)
        height_label = brace.get_text('$2^%d$' % t)
        ss_group = self.get_secant_slope_group(t, graph, dx=0.01, df_label='dM', dx_label='dt', dx_line_color=GREEN, secant_line_color=RED)
        slope_label = OldTex('\\text{Slope}', '=', '2^%d' % t, '(%.7f\\dots)' % np.log(2))
        slope_label.next_to(ss_group.secant_line.point_from_proportion(0.65), DOWN + RIGHT, buff=0)
...
class SimpleGraphOfTwoToT(GraphOfTwoToT):
    CONFIG = {'x_axis_label': '', 'y_axis_label': ''}
    ...
    def construct(self):
        ...
        for x in (1, 2, 3, 4, 5):
            ...
            line_pairs.add(VGroup(DashedLine(x_axis_point, point), DashedLine(y_axis_point, point)))
...
class FakeDiagram(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        gs.setup_axes()
        background_graph, foreground_graph = graphs = VGroup(*[gs.get_graph(lambda t: np.log(2) * 2 ** t, x_min=-8, x_max=2 + dx) for dx in (0.25, 0)])
        for graph in graphs:
        ...
        diagram = VGroup(graphs, two_to_t, dt_brace, dt, side_brace, deriv, circle, words, arrow)
        self.play(self.teacher.change_mode, 'raise_right_hand')
        self.play(Animation(VectorizedPoint(graphs.get_right())), DrawBorderThenFill(foreground_graph), Write(two_to_t))
        self.wait()
        self.play(ReplacementTransform(foreground_graph.copy(), background_graph), Animation(foreground_graph), Animation(two_to_t), GrowFromCenter(dt_brace), Write(dt))
        ...
        self.play(Write(words), ShowCreation(arrow), self.teacher.change_mode, 'shruggie')
        self.wait(3)
        self.play(FadeOut(diagram), *[ApplyMethod(pi.change_mode, 'plain') for pi in self.get_pi_creatures()])
        self.teacher_says('More numerical \\\\ than visual...')
        self.wait(2)
        self.diagram = diagram

class AnalyzeExponentRatio(PiCreatureScene):
    CONFIG = {'base': 2, 'base_str': '2'}
    ...
    def construct(self):
        ...
        self.add(func_def)
        ratio = OldTex('{ {%s^' % base_str, '{t', '+', 'dt}', '-', '%s^' % base_str, 't}', '\\over \\,', 'dt}')
        ratio.shift(UP + LEFT)
        ...
        self.play(Write(ratio), Write(lhs), self.pi_creature.change_mode, 'raise_right_hand')
        self.wait(2)
        self.play(two_to_t_plus_dt.next_to, exp_prop_brace, UP, self.pi_creature.change_mode, 'pondering')
        self.play(ReplacementTransform(two_to_t_plus_dt.copy(), two_to_t_two_to_dt, run_time=2, path_arc=np.pi), FadeIn(exp_prop_brace))
        self.wait(2)
        add_exp_rect, mult_rect = rects = [Rectangle(stroke_color=BLUE, stroke_width=2).replace(mob).scale(1.1) for mob in [VGroup(*two_to_t_plus_dt[1:]), two_to_t_two_to_dt]]
        words = VGroup(*[OldTexText(s, ' ideas') for s in ('Additive', 'Multiplicative')])
        words[0].move_to(words[1], LEFT)
        words.set_color(BLUE)
        words.next_to(two_to_t_plus_dt, RIGHT, buff=1.5 * LARGE_BUFF)
        arrows = VGroup(*[Arrow(word.get_left(), rect, color=words.get_color()) for word, rect in zip(words, rects)])
        ...
        self.wait()
        self.play(Transform(*words), Transform(*arrows))
        self.wait(2)
        self.play(*list(map(FadeOut, [words[0], arrows[0], add_exp_rect, mult_rect, two_to_t_plus_dt, exp_prop_brace])))
        self.play(*[FadeIn(mob, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)) for mob in (one, lp, rp)] + [ReplacementTransform(mob, extracted_two_to_t, path_arc=np.pi / 2, run_time=2) for mob in (two_to_t, VGroup(*two_to_t_two_to_dt[:2]))] + [lhs.next_to, extracted_two_to_t, LEFT])
        ...
        randy.scale(0.7)
        randy.to_edge(DOWN)
        self.play(FadeIn(randy), self.pi_creature.change_mode, 'plain')
        self.play(PiCreatureSays(randy, 'Who cares?', bubble_config={'direction': LEFT}, target_mode='angry'))
        self.wait(2)
        self.play(RemovePiCreatureBubble(randy), FadeOut(randy), self.pi_creature.change_mode, 'hooray', self.pi_creature.look_at, parens)
        self.play(Transform(brace, alt_brace), dt_to_zero.next_to, alt_brace, DOWN)
        self.wait()
        rects = [Rectangle(stroke_color=color, stroke_width=2).replace(mob, stretch=True).scale(1.1) for mob, color in [(VGroup(parens, dt_to_zero), GREEN), (extracted_two_to_t, YELLOW)]]
        ...
        limit_term = VGroup(brace, dt_to_zero, ratio[4], one, rects[0], *ratio[7:] + two_to_t_two_to_dt[2:])
        self.play(FadeIn(rects[0]))
        self.play(limit_term.to_corner, DOWN + LEFT)
        self.play(lp.stretch, 0.5, 1, lp.stretch, 0.8, 0, lp.next_to, extracted_two_to_t[0], RIGHT, rp.stretch, 0.5, 1, rp.stretch, 0.8, 0, rp.next_to, lp, RIGHT, SMALL_BUFF, rp.shift, constant.get_width() * RIGHT, constant.next_to, extracted_two_to_t[0], RIGHT, MED_LARGE_BUFF)
        self.wait()
        ...
        M = lhs[0][1]
        self.play(FadeOut(M), func_def_rhs.move_to, M, LEFT, func_def_rhs.set_fill, None, 1)
        lhs[0].submobjects[1] = func_def_rhs
        self.wait()
        self.play(derivative.next_to, self.pi_creature, UP, derivative.to_edge, RIGHT, self.pi_creature.change_mode, 'raise_right_hand')
        self.wait(2)
    ...
    def try_specific_dt_values(self):
        ...
        for num_zeros in [1, 2, 4, 7]:
            ...
            expression = OldTex('{%s^' % self.base_str, '{%s}' % dt_str, '-1', '\\over \\,', '%s}' % dt_str, '=', output_str)
...
class CompareTwoConstantToEightConstant(PiCreatureScene):
    ...
    def construct(self):
        two_deriv, eight_deriv = derivs = VGroup(*[self.get_derivative_expression(base) for base in (2, 8)])
        ...
        self.wait(3)
        self.play(Animation(derivs), Write(why), self.pi_creature.change, 'confused', derivs)
        self.wait()
        for deriv in derivs:
            for index in (-5, -2):
    ...
    def get_derivative_expression(self, base):
        ...
        result = OldTex('{d(', base_str, '^t', ')', '\\over', 'dt}', '=', base_str, '^t', '(', const_str, ')')
...
class AskAboutConstantOne(TeacherStudentsScene):
    ...
    def construct(self):
        note = OldTex('{ d(a^', 't', ')', '\\over \\,', 'dt}', '=', 'a^', 't', '(\\text{Some constant})')
        ...
        self.play_student_changes('pondering', 'raise_right_hand', 'thinking')
        self.wait(2)
...
class WhyPi(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(Write(pi), Write(why), self.pi_creature.change_mode, 'confused')
        self.wait(3)
...
class GraphOfExp(GraphScene):
    CONFIG = {'x_min': -3, 'x_max': 3, 'x_tick_frequency': 1, 'x_axis_label': 't', 'x_labeled_nums': list(range(-3, 4)), 'x_axis_width': 11, 'graph_origin': 2 * DOWN + LEFT, 'example_inputs': [1, 2], 'small_dx': 0.01}
    ...
    def show_slopes(self):
        ...
        ss_group = self.get_secant_slope_group(start_input, graph, dx=self.small_dx, dx_label='dt', df_label='d(e^t)', secant_line_color=YELLOW)
        v_lines = [self.get_vertical_line_to_graph(x, graph, color=WHITE) for x in self.example_inputs]
        height_labels = [OldTex('e^%d' % x).next_to(vl, RIGHT, SMALL_BUFF) for vl, x in zip(v_lines, self.example_inputs)]
        slope_labels = [OldTexText('Slope = $e^%d$' % x).next_to(vl.get_top(), UP + RIGHT).shift(0.7 * RIGHT / x) for vl, x in zip(v_lines, self.example_inputs)]
        self.play(ShowCreation(graph, run_time=2), Write(graph_label, rate_func=squish_rate_func(smooth, 0.5, 1)))
        self.wait()
        ...
        self.wait(2)
        self.animate_secant_slope_group_change(ss_group, target_x=target_input, run_time=2, added_anims=[Transform(*pair, path_arc=np.pi / 6, run_time=2) for pair in [slope_labels, v_lines, height_labels]])
        self.wait(2)
        self.graph = graph
...
class ApplyChainRule(TeacherStudentsScene):
    ...
    def construct(self):
        deriv_equation = OldTex('{d(', 'e^', '{3', 't}', ')', '\\over', 'dt}', '=', '3', 'e^', '{3', 't}')
        ...
        self.play(Write(deriv), RemovePiCreatureBubble(self.teacher, target_mode='raise_right_hand'))
        self.wait(2)
        self.play(*[Transform(*deriv_equation.get_parts_by_tex(tex, substring=False).copy()[:2], path_arc=-np.pi, run_time=2) for tex in ('e^', '{3', 't}')] + [Write(deriv_equation.get_part_by_tex('='))])
...
class ChainRuleIntuition(ThreeLinesChainRule):
    CONFIG = {'line_configs': [{'func': lambda t: t, 'func_label': 't', 'triangle_color': WHITE, 'center_y': 3, 'x_min': 0, 'x_max': 3, 'numbers_to_show': list(range(4)), 'big_tick_numbers': list(range(4)), 'tick_frequency': 1}, {'func': lambda t: 3 * t, 'func_label': '3t', 'triangle_color': GREEN, 'center_y': 0.5, 'x_min': 0, 'x_max': 3, 'numbers_to_show': list(range(0, 4)), 'big_tick_numbers': list(range(4)), 'tick_frequency': 1}, {'func': lambda t: np.exp(3 * t), 'func_label': 'e^{3t}', 'triangle_color': BLUE, 'center_y': -2, 'x_min': 0, 'x_max': 10, 'numbers_to_show': list(range(0, 11, 3)), 'big_tick_numbers': list(range(11)), 'tick_frequency': 1}], 'example_x': 0.4, 'start_x': 0.4, 'max_x': 0.6, 'min_x': 0.2}
...
class WhyNaturalLogOf2ShowsUp(TeacherStudentsScene):
    ...
    def add_e_to_the_three_t(self):
        ...
        self.play(FadeIn(exp_c, run_time=2, lag_ratio=0.5), self.teacher.change, 'raise_right_hand')
        self.wait()
    ...
    def show_e_to_log_2(self):
        ...
        log_words_arrow = Arrow(log_words.get_right(), equation.get_part_by_tex('ln(2)').get_corner(UP + LEFT), color=BLUE)
        derivative = OldTex('\\ln(2)', '2', '^t', '=', '\\ln(2)', 'e^', '{\\ln(2)', 't}')
        ...
        self.play(Write(non_t_group), self.exp_c.next_to, equation, LEFT, 2 * LARGE_BUFF, self.exp_c.to_edge, UP)
        self.play_student_changes('confused', 'sassy', 'erm')
        self.play(Write(log_words), ShowCreation(log_words_arrow, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)))
        self.play_student_changes(*['pondering'] * 3, look_at=log_words)
        self.wait(2)
        t_group.save_state()
        t_group.shift(UP)
        t_group.set_fill(opacity=0)
        self.play(ApplyMethod(t_group.restore, run_time=2, lag_ratio=0.5), self.teacher.change_mode, 'speaking')
        self.wait(2)
        self.play(FocusOn(self.exp_c))
        self.play(Indicate(self.exp_c, scale_factor=1.05))
        self.wait(2)
        self.play(equation.next_to, derivative_arrow, UP, equation.shift, MED_SMALL_BUFF * RIGHT, FadeOut(VGroup(log_words, log_words_arrow)), self.teacher.change_mode, 'raise_right_hand')
        self.play(ShowCreation(derivative_arrow), Write(derivative_symbol), Write(derivative))
        self.wait(3)
        self.play(self.teacher.change_mode, 'happy')
        self.wait(2)
        ...
        self.play(ln.next_to, student, UP + LEFT, MED_LARGE_BUFF, student.change_mode, 'raise_left_hand')
    ...
    def get_exp_C(self, C):
        C_str = str(C)
        result = OldTex('{d(', 'e^', '{%s' % C_str, 't}', ')', '\\over', 'dt}', '=', C_str, 'e^', '{%s' % C_str, 't}')
    ...
    def get_a_to_t(self, a):
        ...
        result = OldTex('{d(', a_str, '^t', ')', '\\over', 'dt}', '=', log_str, a_str, '^t')
...
class CompareWaysToWriteExponentials(GraphScene):
    CONFIG = {'y_max': 50, 'y_tick_frequency': 5, 'x_max': 7}
    ...
    def construct(self):
        ...
        for base in bases:
            ...
            equation = OldTex(str(base), '^t', '=', 'e^', '{(%s)' % log_str, 't}')
            equation.set_color_by_tex(log_str, BLUE)
            equation.scale(1.2)
            equations.add(equation)
        equation = equations[0]
        equations.next_to(e_to_ct, DOWN, LARGE_BUFF, LEFT)
        self.play(ShowCreation(graph), Write(a_to_t, rate_func=squish_rate_func(smooth, 0.5, 1)), run_time=2)
...
class ManyExponentialForms(TeacherStudentsScene):
    ...
    def construct(self):
        lhs = OldTex('2', '^t')
        rhs_list = [OldTex('=', '%s^' % tex, '{(%.5f\\dots)' % log, 't}') for tex, log in [('e', np.log(2)), ('\\pi', np.log(2) / np.log(np.pi)), ('42', np.log(2) / np.log(42))]]
        ...
        for rhs in rhs_list[1:]:
            self.play(FadeIn(rhs, run_time=2, lag_ratio=0.5))
            self.wait(2)
        self.wait()
        self.play(FadeIn(rect), exp.next_to, self.teacher, UP + LEFT, self.teacher.change, 'raise_right_hand')
        self.play(*[ApplyFunction(lambda m: m.shift(SMALL_BUFF * UP).set_color(RED), part, run_time=2, rate_func=squish_rate_func(there_and_back, a, a + 0.3)) for part, a in zip(exp[1], np.linspace(0, 0.7, len(exp[1])))])
...
class TemperatureOverTimeOfWarmWater(GraphScene):
    CONFIG = {'x_min': 0, 'x_axis_label': '$t$', 'y_axis_label': 'Temperature', 'T_room': 4, 'include_solution': False}

    def construct(self):
        self.setup_axes()
        graph = self.get_graph(lambda t: 3 * np.exp(-0.3 * t) + self.T_room, color=RED)
        h_line = DashedLine(*[self.coords_to_point(x, self.T_room) for x in (self.x_min, self.x_max)])
        ...
        def update_delta_T_group(group):
            brace, label = group
            v_line = Line(graph.get_points()[-1], graph.get_points()[-1][0] * RIGHT + h_line.get_center()[1] * UP)
        ...
        self.play(GrowFromCenter(delta_T_brace), Write(delta_T_label))
...
class InvestedMoney(Scene):
    ...
    def construct(self):
        ...
        cash = OldTex('\\$')
        cash_pile = VGroup(*[cash.copy().shift(x * (1 + MED_SMALL_BUFF) * cash.get_width() * RIGHT + y * (1 + MED_SMALL_BUFF) * cash.get_height() * UP) for x in range(40) for y in range(8)])
        ...
        for i, cash in enumerate(cash_pile):
            ...
            rate_func = squish_rate_func(smooth, np.clip(prop - 0.5 / run_time, 0, 1), np.clip(prop + 0.5 / run_time, 0, 1))
            anims.append(GrowFromCenter(cash, rate_func=rate_func))
        self.add(equation)
...
class NaturalLog(Scene):
    ...
    def show_natural_loggedness(self, expression):
        ...
        VGroup(exp_base, exp_equals).next_to(VGroup(e, exp_constant), RIGHT, aligned_edge=DOWN)
        exp_expression = VGroup(e, exp_constant, exp_equals, exp_base)
        for group, vect in ((log_expression, UP), (exp_expression, DOWN)):
            group.to_edge(RIGHT)
            group.shift(vect)
        self.play(ReplacementTransform(base.copy(), log_base), ReplacementTransform(constant.copy(), log_constant), run_time=2)
        self.play(Write(ln), Write(log_equals))
        self.wait()
        self.play(ReplacementTransform(log_expression.copy(), exp_expression, run_time=2))
        self.wait(2)
        ln_a = expression[-1]
        self.play(FadeOut(expression[-2]), FadeOut(constant), ln_a.move_to, constant, LEFT, ln_a.set_color, BLUE)
        self.wait()
        self.play(*list(map(FadeOut, [log_expression, exp_expression])))
        self.wait()

    def get_expression(self, base):
        expression = OldTex('{d(', '%d^' % base, 't', ')', '\\over \\,', 'dt}', '=', '%d^' % base, 't', '(%.4f\\dots)' % np.log(base))
        expression.set_color_by_tex('t', YELLOW)
        expression.set_color_by_tex('dt', GREEN)
...
class NextVideo(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        next_tex = VGroup(*list(map(TexText, ['Chain rule', 'Product rule', '$\\vdots$'])))
        next_tex.arrange(DOWN)
        next_tex.next_to(brace, DOWN)
        next_tex.shift(next_video.get_center()[0] * RIGHT - next_tex.get_center()[0] * RIGHT)
        ...
        self.wait()
        self.play(Write(this_tex[4]), ReplacementTransform(this_tex[3][0].copy(), this_tex[4][3], path_arc=np.pi, remover=True))
        self.wait(2)
        self.play(this_tex.replace, this_video)
        self.play(brace.next_to, next_video, DOWN, this_video.restore, Animation(this_tex), next_video.set_color, YELLOW, Write(next_tex), self.get_teacher().change_mode, 'raise_right_hand')
        self.play_student_changes(*['pondering'] * 3, look_at=next_tex)
        self.wait(3)

class ExpPatreonThanks(PatreonThanks):
    CONFIG = {'specific_patrons': ['Ali  Yahya', 'Meshal  Alshammari', 'CrypticSwarm    ', 'Kathryn Schmiedicke', 'Nathan Pellegrin', 'Karan Bhargava', 'Justin Helps', 'Ankit   Agarwal', 'Yu  Jun', 'Dave    Nicponski', 'Damion  Kistler', 'Juan    Benet', 'Othman  Alikhan', 'Justin Helps', 'Markus  Persson', 'Dan Buchoff', 'Derek   Dai', 'Joseph  John Cox', 'Luc Ritchie', 'Mustafa Mahdi', 'Daan Smedinga', 'Jonathan Eppele', 'Albert Nguyen', 'Nils Schneider', 'Mustafa Mahdi', 'Mathew Bramson', 'Guido   Gambardella', 'Jerry   Ling', 'Mark    Govea', 'Vecht', 'Shimin Kuang', 'Rish    Kundalia', 'Achille Brighton', 'Kirk    Werklund', 'Ripta   Pasay', 'Felipe  Diniz']}

class Thumbnail(GraphOfTwoToT):
    CONFIG = {'x_axis_label': '', 'y_axis_label': '', 'x_labeled_nums': None, 'y_labeled_nums': None, 'y_max': 32, 'y_tick_frequency': 4, 'graph_origin': 3 * DOWN + 5 * LEFT, 'x_axis_width': 12}
