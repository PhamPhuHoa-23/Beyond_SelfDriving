...
class Eoc1Thumbnail(GraphScene):
    ...
    def construct(self):
        title = OldTexText('The Essence of\\\\Calculus', tex_to_color_map={'\\emph{you}': YELLOW})
        ...
        axes = Axes(x_range=(-1, 5), y_range=(-1, 5), y_axis_config={'include_tip': False}, x_axis_config={'unit_size': 2})
        ...
        rects = self.get_riemann_rectangles(graph, x_min=0, x_max=4, dx=0.2)
...
class CircleScene(PiCreatureScene):
    CONFIG = {'radius': 1.5, 'stroke_color': WHITE, 'fill_color': BLUE_E, 'fill_opacity': 0.75, 'radial_line_color': MAROON_B, 'outer_ring_color': GREEN_E, 'ring_colors': [BLUE, GREEN], 'dR': 0.1, 'dR_color': YELLOW, 'unwrapped_tip': ORIGIN, 'include_pi_creature': False, 'circle_corner': UP + LEFT}

    def setup(self):
        PiCreatureScene.setup(self)
        self.circle = Circle(radius=self.radius, stroke_color=self.stroke_color, fill_color=self.fill_color, fill_opacity=self.fill_opacity)
    ...
    def introduce_circle(self, added_anims=[]):
        ...
        self.play(ShowCreation(self.radius_line), GrowFromCenter(self.radius_brace), Write(self.radius_label))
        self.circle.set_fill(opacity=0)
        self.play(Rotate(self.radius_line, 2 * np.pi - 0.001, about_point=self.circle.get_center()), ShowCreation(self.circle), *added_anims, run_time=2)
        self.play(self.circle.set_fill, self.fill_color, self.fill_opacity, Animation(self.radius_line), Animation(self.radius_brace), Animation(self.radius_label))
    ...
    def increase_radius(self, numerical_dr=True, run_time=2):
        ...
        nudge_arrow = Arrow(nudge_line.get_center() + 0.5 * RIGHT + DOWN, nudge_line.get_center(), color=YELLOW, buff=SMALL_BUFF, tip_length=0.2)
        if numerical_dr:
        ...
        self.play(FadeIn(outer_ring), ShowCreation(nudge_line), ShowCreation(nudge_arrow), Write(nudge_label), run_time=run_time / 2.0)
    ...
    def unwrap_rings(self, *rings, **kwargs):
        ...
        self.play(rings.rotate, np.pi / 2, rings.next_to, unwrapped.get_bottom(), UP, run_time=2, path_arc=np.pi / 2)
    ...
    def get_unwrapped(self, ring, to_edge=LEFT, **kwargs):
        ...
        result = VMobject()
        result.set_points_as_corners([interpolate(np.pi * R_plus_dr * LEFT, np.pi * R_plus_dr * RIGHT, a) for a in np.linspace(0, 1, n_anchors / 2)] + [interpolate(np.pi * R * RIGHT + ring.dR * UP, np.pi * R * LEFT + ring.dR * UP, a) for a in np.linspace(0, 1, n_anchors / 2)])
        result.set_style_data(stroke_color=ring.get_stroke_color(), stroke_width=ring.get_stroke_width(), fill_color=ring.get_fill_color(), fill_opacity=ring.get_fill_opacity())
...
class Chapter1OpeningQuote(OpeningQuote):
    CONFIG = {'quote': ['The art of doing mathematics is finding\n            that ', 'special case', 'that contains all the \n            germs of generality.'], 'quote_arg_separator': ' ', 'highlighted_quote_terms': {'special case': BLUE}, 'author': 'David Hilbert'}
...
class Introduction(TeacherStudentsScene):
    ...
    def show_series(self):
        ...
        self.play_student_changes(*['hooray'] * 3, look_at=series[1].get_left(), added_anims=[ApplyMethod(this_video.restore, run_time=3)])
        self.play(*[ApplyMethod(video.shift, 0.5 * video.get_height() * DOWN, run_time=3, rate_func=squish_rate_func(there_and_back, alpha, alpha + 0.3)) for video, alpha in zip(series, np.linspace(0, 0.7, len(series)))] + [Animation(self.teacher.bubble), Animation(self.teacher.bubble.content)])
        essence_words = words.get_part_by_tex('Essence').copy()
        self.play(FadeOut(self.teacher.bubble), FadeOut(self.teacher.bubble.content), essence_words.next_to, series, DOWN, *[ApplyMethod(pi.change_mode, 'pondering') for pi in self.get_pi_creatures()])
        self.wait(3)
        self.series = series
        self.essence_words = essence_words

    def show_many_facts(self):
        rules = list(it.starmap(Tex, [('{d(', 'x', '^2)', '\\over \\,', 'dx}', '=', '2', 'x'), ('d(', 'f', 'g', ')', '=', 'f', 'dg', '+', 'g', 'df'), ('F(x)', '=', '\\int_0^x', '\\frac{dF}{dg}(t)\\,', 'dt'), ('f(x)', '=', '\\sum_{n = 0}^\\infty', 'f^{(n)}(a)', '\\frac{(x-a)^n}{n!}')]))
        video_indices = [2, 3, 7, 10]
        tex_to_color = [('x', BLUE), ('f', BLUE), ('df', BLUE), ('g', YELLOW), ('dg', YELLOW), ('f(x)', BLUE), ('f^{(n)}(a)', BLUE)]
        for rule in rules:
        ...
        self.play_student_changes('pondering', 'sassy', 'pondering', look_at=self.teacher.eyes, added_anims=[self.teacher.change_mode, 'plain'])
        self.wait(2)
        self.play(Write(rules[0]), self.teacher.change_mode, 'raise_right_hand')
        self.wait()
        ...
        for last_rule, rule, video_index in zip(rules, alt_rules_list, video_indices):
            ...
            self.play(last_rule.replace, video, FadeIn(rule))
    ...
    def invent_calculus(self):
        ...
        fader = Rectangle(width=FRAME_WIDTH, height=FRAME_HEIGHT, stroke_width=0, fill_color=BLACK, fill_opacity=0.5)
        self.play(FadeIn(fader), Animation(student), Animation(calculus))
        self.play(Write(invent), MoveToTarget(calculus), student.change_mode, 'erm', student.look_at, calculus)
...
class PreviewFrame(Scene):
    ...
    def construct(self):
        ...
        titles = [OldTexText('Chapter %d:' % d, s).to_edge(UP).set_color(next(colors)) for d, s in [(3, 'Derivative formulas through geometry'), (4, 'Chain rule, product rule, etc.'), (7, 'Limits')]]
        title = titles[0]
...
class ProductRuleDiagram(Scene):
    ...
    def construct(self):
        df = 0.4
        dg = 0.2
        rect_kwargs = {'stroke_width': 0, 'fill_color': BLUE, 'fill_opacity': 0.6}
        ...
        corner_pairs = [(DOWN + RIGHT, UP + RIGHT), (DOWN + RIGHT, DOWN + LEFT), (DOWN + RIGHT, DOWN + RIGHT)]
        ...
        self.add(rect)
        self.play(GrowFromCenter(f_brace), GrowFromCenter(g_brace), Write(f_label), Write(g_label))
        self.play(MoveToTarget(fg_group))
        self.play(*[mob.restore for mob in (df_brace, df_label, dg_brace, dg_label)] + [ReplacementTransform(d_rect.line, d_rect) for d_rect in d_rects])
        self.wait()
        self.play(d_rects.space_out_submobjects, 1.2, MaintainPositionRelativeTo(VGroup(df_brace, df_label), df_rect), MaintainPositionRelativeTo(VGroup(dg_brace, dg_label), dg_rect))
        self.wait()
        deriv = OldTex('d(', 'fg', ')', '=', 'f', '\\cdot', 'dg', '+', 'g', '\\cdot', 'df')
        deriv.to_edge(UP)
        alpha_iter = iter(np.linspace(0, 0.5, 5))
        self.play(*[ApplyMethod(mob.copy().move_to, deriv.get_part_by_tex(tex, substring=False), rate_func=squish_rate_func(smooth, alpha, alpha + 0.5)) for mob, tex in [(fg_group, 'fg'), (f_label, 'f'), (dg_label, 'dg'), (g_label, 'g'), (df_label, 'df')] for alpha in [next(alpha_iter)]] + [Write(VGroup(*it.chain(*[deriv.get_parts_by_tex(tex, substring=False) for tex in ('d(', ')', '=', '\\cdot', '+')])))], run_time=3)
        self.wait()
...
class IntroduceCircle(CircleScene):
    ...
    def introduce_area(self):
        ...
        self.play(self.pi_creature.change_mode, 'pondering', self.pi_creature.look_at, self.circle)
        self.introduce_circle()
        self.wait()
        R_copy = self.radius_label.copy()
        self.play(self.pi_creature.change_mode, 'raise_right_hand', self.pi_creature.look_at, area, Transform(R_copy, area.get_part_by_tex('R')))
    ...
    def question_area(self):
        ...
        self.play(Animation(self.area), Write(q_marks), self.pi_creature.change_mode, 'confused', self.pi_creature.look_at, self.area)
        self.wait()
        self.play(FadeIn(rings, **ring_anim_kwargs), Animation(self.radius_group), FadeOut(q_marks), self.pi_creature.change_mode, 'thinking')
        self.wait()
        self.play(rings.rotate, np.pi / 2, rings.move_to, unwrapped_rings.get_top(), Animation(self.radius_group), path_arc=np.pi / 2, **ring_anim_kwargs)
    ...
    def show_calculus_symbols(self):
        ...
        ftc.shift(2 * UP)
        self.play(ReplacementTransform(self.area.get_part_by_tex('R').copy(), ftc.get_part_by_tex('int')), self.pi_creature.change_mode, 'plain')
        self.wait()
        self.play(ReplacementTransform(self.area.get_part_by_tex('Area').copy(), ftc.get_part_by_tex('frac')), ReplacementTransform(self.area.get_part_by_tex('R').copy(), ftc.get_part_by_tex('\\,dr')))
        self.wait()
        self.play(Write(VGroup(*ftc[-2:])))
        self.wait(2)

class ApproximateOneRing(CircleScene, ReconfigurableScene):
    CONFIG = {'num_lines': 24, 'ring_index_proportion': 0.6, 'ring_shift_val': 6 * RIGHT, 'ring_colors': [BLUE, GREEN_E], 'unwrapped_tip': 2 * RIGHT + 0.5 * UP}
    ...
    def try_to_understand_area(self):
        line_sets = [VGroup(*[Line(self.circle.point_from_proportion(alpha), self.circle.point_from_proportion(func(alpha))) for alpha in np.linspace(0, 1, self.num_lines)]) for func in [lambda alpha: 1 - alpha, lambda alpha: (0.5 - alpha) % 1, lambda alpha: (alpha + 0.4) % 1, lambda alpha: (alpha + 0.5) % 1]]
        for lines in line_sets:
            lines.set_stroke(BLACK, 2)
        lines = line_sets[0]
        self.play(ShowCreation(lines, run_time=2, lag_ratio=0.5), Animation(self.radius_group), self.pi_creature.change_mode, 'maybe')
        self.wait(2)
    ...
    def slice_into_rings(self):
        ...
        self.play(FadeIn(rings, lag_ratio=0.5, run_time=3), Animation(self.radius_group), self.pi_creature.change_mode, 'pondering', self.pi_creature.look_at, self.circle)
        self.wait(2)
        for x in range(2):
            self.play(Rotate(rings, np.pi, in_place=True, run_time=2), Animation(self.radius_group), self.pi_creature.change_mode, 'happy')
        self.wait(2)
        self.rings = rings
    ...
    def isolate_one_ring(self):
        ...
        self.play(ring.shift, self.ring_shift_val, original_ring.set_fill, None, 0.25, Animation(self.radius_group))
        ...
        self.wait()
        self.play(*[ApplyMethod(r.set_fill, YELLOW, rate_func=squish_rate_func(there_and_back, alpha, alpha + 0.15), run_time=3) for r, alpha in zip(rings, np.linspace(0, 0.85, len(rings)))] + [Animation(self.radius_group)])
        self.wait()
    ...
    def straighten_ring_out(self):
        ...
        for text in (trapezoid, rectangle_ish):
            text.next_to(self.pi_creature.get_corner(UP + RIGHT), DOWN + RIGHT, buff=MED_LARGE_BUFF)
        ...
        strike = Line(trapezoid.get_left(), trapezoid.get_right(), stroke_color=RED, stroke_width=8)
        self.play(Write(rectangle_ish), ShowCreation(strike), self.pi_creature.change_mode, 'happy')
        self.wait()
        self.play(*list(map(FadeOut, [trapezoid, strike])))
        self.unwrapped_ring = ring

    def approximate_as_rectangle(self):
        top_brace, side_brace = [Brace(self.unwrapped_ring, vect, buff=SMALL_BUFF, min_num_quads=2) for vect in (UP, LEFT)]
        ...
        approx.next_to(self.area_q.get_part_by_tex('Area'), RIGHT, align_using_submobjects=True)
        ...
        self.play(Write(width_label.get_part_by_tex('pi')), ReplacementTransform(self.ring_radius_group[1].copy(), width_label.get_part_by_tex('r')))
        self.wait()
        ...
        self.play(ReplacementTransform(side_brace.copy(), alt_side_brace), ReplacementTransform(dr_label.copy(), alt_dr_label), run_time=2)
        self.wait()
        self.play(dr_label.next_to, concrete_dr.copy(), LEFT, SMALL_BUFF, DOWN, Write(concrete_dr, run_time=2), self.pi_creature.change_mode, 'pondering')
        self.wait(2)
        self.play(MoveToTarget(two_pi_r_dr), FadeIn(approx), self.area_q.get_part_by_tex('?').fade, 1)
        self.wait()
        self.play(FadeOut(concrete_dr), dr_label.restore)
        self.show_alternate_width(40, transformation_kwargs={'run_time': 4}, return_to_original_configuration=False)
        self.wait(2)
        self.look_at(self.circle)
        self.play(ApplyWave(self.rings, amplitude=0.1), Animation(self.radius_group), Animation(alt_side_brace), Animation(alt_dr_label), run_time=3, lag_ratio=0.5)
        self.wait(2)
...
class GraphRectangles(CircleScene, GraphScene):
    CONFIG = {'graph_origin': 3.25 * LEFT + 2.5 * DOWN, 'x_min': 0, 'x_max': 4, 'x_axis_width': 7, 'x_labeled_nums': list(range(5)), 'x_axis_label': '$r$', 'y_min': 0, 'y_max': 20, 'y_tick_frequency': 2.5, 'y_labeled_nums': list(range(5, 25, 5)), 'y_axis_label': '', 'exclude_zero_label': False, 'num_rings_in_ring_sum_start': 3, 'tick_height': 0.2}
    ...
    def draw_ring_sum(self):
        ...
        arrows = VGroup(*[Arrow(arrow_start, ring.target.get_boundary_point(arrow_start - ring.target.get_center()), color=ring.get_color()) for ring in rings if ring.target.get_fill_opacity() > 0])
    ...
    def draw_r_values(self):
        ...
        r_ticks = VGroup(*[Line(self.coords_to_point(r, -self.tick_height), self.coords_to_point(r, self.tick_height), color=YELLOW) for r in np.arange(0, 3, 0.1)])
        arrows = VGroup(*[Arrow(values_of_r.get_part_by_tex('r').get_bottom(), tick.get_top(), buff=SMALL_BUFF, color=YELLOW, tip_length=0.15) for tick in (r_ticks[0], r_ticks[-1])])
        ...
        self.play(FadeIn(values_of_r), FadeIn(self.x_axis))
        self.play(ShowCreation(moving_arrow), ShowCreation(first_tick))
        self.play(Indicate(self.rings[0]))
        self.wait()
        self.play(Transform(moving_arrow, arrows[-1]), ShowCreation(r_ticks, lag_ratio=0.5), run_time=2)
    ...
    def unwrap_rings_onto_graph(self):
        ...
        rects, flat_rects = [self.get_riemann_rectangles(g, x_min=0, x_max=3, dx=self.dR, start_color=self.rings[0].get_fill_color(), end_color=self.rings[-1].get_fill_color()) for g in (graph, flat_graph)]
        ...
        for index, rect, r in zip(it.count(), rects, np.arange(0, 3, 0.1)):
            ...
            n_anchors = ring.get_num_curves()
            target = VMobject()
            target.set_points_as_corners([interpolate(ORIGIN, DOWN, a) for a in np.linspace(0, 1, n_anchors / 2)] + [interpolate(DOWN + RIGHT, RIGHT, a) for a in np.linspace(0, 1, n_anchors / 2)])
        ...
        self.play(MoveToTarget(example_ring, path_arc=-np.pi / 2, run_time=2), Animation(self.x_axis))
        self.wait(2)
        self.play(*[MoveToTarget(ring, path_arc=-np.pi / 2, run_time=4, rate_func=squish_rate_func(smooth, alpha, alpha + 0.25)) for ring, alpha in zip(transformed_rings, np.linspace(0, 0.75, len(transformed_rings)))] + foreground_animations)
        self.wait()
        ...
        self.play(transformed_rings.set_fill, None, 0.2, Animation(highlighted_ring), *foreground_animations)
        self.play(self.dr_group.arrange, DOWN, self.dr_group.next_to, highlighted_ring, DOWN, SMALL_BUFF)
        self.wait()
        ...
        self.wait()
        self.play(transformed_rings.set_fill, None, 1, FadeOut(side_brace), FadeOut(height_label), *foreground_animations)
        self.remove(highlighted_ring)
        self.wait()
        self.play(*[ApplyMethod(ring.replace, ring.rect, method_kwargs={'stretch': True}) for ring in transformed_rings] + [Write(self.y_axis), FadeOut(self.area_label)] + foreground_animations)
    ...
    def draw_graph(self):
        ...
        self.wait()
        self.play(*[Transform(rect, flat_rect, run_time=2, rate_func=squish_rate_func(lambda t: 0.1 * there_and_back(t), alpha, alpha + 0.5), lag_ratio=0.5) for rect, flat_rect, alpha in zip(self.rects, self.flat_rects, np.linspace(0, 0.5, len(self.rects)))] + list(map(Animation, [self.x_axis, self.graph])))
        self.wait(2)
    ...
    def point_out_approximation(self):
        ...
        group.arrange(RIGHT)
        group.to_edge(RIGHT)
        self.play(MoveToTarget(rect), Write(approximation))
        self.wait(2)
        self.play(rect.restore, FadeOut(approximation))
        self.wait()

    def let_dr_approah_zero(self):
        thinner_rects_list = [self.get_riemann_rectangles(self.graph, x_min=0, x_max=3, dx=1.0 / (10 * 2 ** n), stroke_width=1.0 / 2 ** n, start_color=self.rects[0].get_fill_color(), end_color=self.rects[-1].get_fill_color()) for n in range(1, 5)]
        ...
        for new_rects in thinner_rects_list:
            self.play(Transform(self.rects, new_rects, lag_ratio=0.5, run_time=2), Animation(self.axes), Animation(self.graph))
            self.wait()
        self.play(ApplyWave(self.rects, direction=RIGHT, run_time=2, lag_ratio=0.5))
        self.wait()
    ...
    def compute_area_under_graph(self):
        ...
        self.wait()
        self.play(Transform(formula, formula_with_R), Transform(height_label, height_label_with_R))
        self.wait(2)
    ...
    def show_circle_unwrapping(self):
        ...
        unwrapped.apply_function(lambda p: np.dot(p, np.array([[1, 0, 0], [-1, 1, 0], [0, 0, 1]])), maintain_smoothness=False)
        unwrapped.rotate(np.pi / 2)
        unwrapped.replace(self.rects, stretch=True)
        self.play(self.rects.fade, 0.8)
        self.play(Transform(rings, unwrapped, run_time=5, lag_ratio=0.5), Animation(self.radius_group))
        self.wait()
    ...
    def get_ring_sum(self, rings):
        ...
        draw_ring_sum_anims += [MoveToTarget(ring, run_time=3, path_arc=-np.pi / 3, rate_func=squish_rate_func(smooth, alpha, alpha + 0.8)) for ring, alpha in zip(rings, np.linspace(0, 0.2, len(rings)))]
    ...
    def get_area_formula(self, R):
        formula = OldTex('\\text{Area}', '&= \\frac{1}{2}', 'b', 'h', '\\\\ &=', '\\frac{1}{2}', '(%s)' % R, '(2\\pi \\cdot %s)' % R, '\\\\ &=', '\\pi ', '%s' % R, '^2')
...
class ThinkLikeAMathematician(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait(2)
        self.play_student_changes(*['pondering'] * 3, look_at=self.teacher.eyes, added_anims=[PiCreatureSays(self.teacher, 'But why did \\\\ that work?')])
...
class TwoThingsToNotice(TeacherStudentsScene):
    ...
    def construct(self):
        words = OldTexText('Two things to \\\\ note about', '$dr$')
...
class RecapCircleSolution(GraphRectangles, ReconfigurableScene):
    ...
    def show_sum(self):
        ...
        arrows = VGroup(*[Arrow(area_label.get_left(), approximation.get_right(), color=WHITE) for approximation in approximations])
        self.play(Write(area_label))
        self.play(ShowCreation(arrows, lag_ratio=0), FadeIn(radial_lines), *[ReplacementTransform(area_label.copy(), VGroup(*approximation[1:])) for approximation in approximations])
        self.wait()
    ...
    def dr_indicates_spacing(self):
        r_ticks = VGroup(*[Line(self.coords_to_point(r, -self.tick_height), self.coords_to_point(r, self.tick_height), color=YELLOW) for r in np.arange(0, 3, self.dR)])
        ...
        self.play(ReplacementTransform(dr.copy(), dr_num, run_time=2, path_arc=np.pi / 2))
        self.wait()
    ...
    def show_riemann_sum(self):
        ...
        self.play(Write(self.y_axis, run_time=2), *list(map(FadeOut, [self.approximations, self.area_label, self.area_arrows, self.dr_brace_group, self.r_ticks])))
        self.play(ReplacementTransform(self.rings.copy(), rects, run_time=2, lag_ratio=0.5), Animation(self.x_axis))
    ...
    def limiting_riemann_sum(self):
        thinner_rects_list = [self.get_riemann_rectangles(self.graph, x_min=0, x_max=3, dx=1.0 / (10 * 2 ** n), stroke_width=1.0 / 2 ** n, start_color=self.rects[0].get_fill_color(), end_color=self.rects[-1].get_fill_color()) for n in range(1, 4)]
        for new_rects in thinner_rects_list:
            self.play(Transform(self.rects, new_rects, lag_ratio=0.5, run_time=2), Animation(self.axes), Animation(self.graph))
            self.wait()
...
class ExampleIntegralProblems(PiCreatureScene, GraphScene):
    CONFIG = {'dt': 0.2, 't_max': 7, 'x_max': 8, 'y_axis_height': 5.5, 'x_axis_label': '$t$', 'y_axis_label': '', 'graph_origin': 3 * DOWN + 4.5 * LEFT}
    ...
    def show_car(self):
        ...
        ticks = VGroup(*[Line(p + tick_height * UP / 2, p + tick_height * DOWN / 2, color=YELLOW, stroke_width=2) for t in np.arange(0, self.t_max, self.dt) for p in [line.point_from_proportion(smooth(t / self.t_max))]])
        ...
        self.play(MoveCar(car, end), FadeIn(ticks, lag_ratio=1, rate_func=linear), ShowCreation(line), FadeIn(v_dt_brace_group, rate_func=squish_rate_func(smooth, 0.6, 0.8)), run_time=self.t_max)
        self.wait()
        for mob in v_dt:
    ...
    def show_graph(self):
        ...
        self.play(ReplacementTransform(pre_rects, rects, run_time=3, lag_ratio=0.5), Animation(self.ticks), Write(self.axes, run_time=1))
        self.play(ShowCreation(v_graph))
        self.change_mode('pondering')
        self.wait()
        self.play(v_t.next_to, brace, LEFT, SMALL_BUFF, dt.next_to, special_rect, DOWN, special_rect.set_fill, None, 1, ReplacementTransform(start_brace, brace))
        self.wait(3)
    ...
    def let_dt_approach_zero(self):
        thinner_rects_list = [self.get_riemann_rectangles(self.v_graph, x_min=0, x_max=self.t_max, dx=self.dt / 2 ** n, stroke_width=1.0 / 2 ** n) for n in range(1, 4)]
        self.play(self.rects.set_fill, None, 1, Animation(self.x_axis), FadeOut(self.v_dt_brace_group_copy))
        self.change_mode('thinking')
        self.wait()
    ...
    def show_confusion(self):
        ...
        self.play(self.pi_creature.change_mode, 'confused', self.pi_creature.look_at, randy.eyes)
...
class MathematicianPonderingAreaUnderDifferentCurves(PiCreatureScene):
    ...
    def construct(self):
        self.play(self.pi_creature.change_mode, 'raise_left_hand', self.pi_creature.look, UP + LEFT)
        self.wait(4)
        self.play(self.pi_creature.change_mode, 'raise_right_hand', self.pi_creature.look, UP + RIGHT)
        self.wait(4)
...
class AreaUnderParabola(GraphScene):
    CONFIG = {'x_max': 4, 'x_labeled_nums': list(range(-1, 5)), 'y_min': 0, 'y_max': 15, 'y_tick_frequency': 2.5, 'y_labeled_nums': list(range(5, 20, 5)), 'n_rect_iterations': 6, 'default_right_x': 3, 'func': lambda x: x ** 2, 'graph_label_tex': 'x^2', 'graph_label_x_val': 3.8}
    ...
    def show_graph(self):
        graph = self.get_graph(self.func)
        graph_label = self.get_graph_label(graph, self.graph_label_tex, direction=LEFT, x_val=self.graph_label_x_val)
        self.play(ShowCreation(graph))
        self.play(Write(graph_label))
        self.wait()
        self.graph = graph
    ...
    def show_area(self):
        ...
        rect_lists = [self.get_riemann_rectangles(self.graph, x_min=0, x_max=self.default_right_x, dx=dx, stroke_width=4 * dx) for dx in dx_list]
        rects = rect_lists[0]
        foreground_mobjects = [self.axes, self.graph]
        self.play(DrawBorderThenFill(rects, run_time=2, rate_func=smooth, lag_ratio=0.5), *list(map(Animation, foreground_mobjects)))
        self.wait()
        for new_rects in rect_lists[1:]:
            self.play(Transform(rects, new_rects, lag_ratio=0.5), *list(map(Animation, foreground_mobjects)))
        self.wait()
        self.rects = rects
        self.dx = dx_list[-1]
    ...
    def ask_about_area(self):
        ...
        v_lines = VGroup(*[DashedLine(FRAME_HEIGHT * UP, ORIGIN, color=RED).move_to(self.coords_to_point(x, 0), DOWN) for x in (0, self.default_right_x)])
    ...
    def show_confusion(self):
        ...
        self.play(morty.change_mode, 'confused', morty.look_at, self.question)
    ...
    def show_variable_endpoint(self):
        triangle = RegularPolygon(n=3, start_angle=np.pi / 2, stroke_width=0, fill_color=WHITE, fill_opacity=1)
    ...
    def move_right_point_to(self, target_x, **kwargs):
        ...
        def update_group(group, alpha):
            ...
            new_rects = self.get_riemann_rectangles(self.graph, x_min=0, x_max=new_x, dx=self.dx * new_x / 3.0, stroke_width=rects[0].get_stroke_width())
        ...
        self.play(UpdateFromAlphaFunc(group, update_group, **kwargs), *list(map(Animation, self.foreground_mobjects)))

class WhoCaresAboutArea(TeacherStudentsScene):

    def construct(self):
        point = 2 * RIGHT + 3 * UP
        self.student_says('Who cares!?!', target_mode='angry')
        self.play(self.teacher.change_mode, 'guilty')
        self.wait()
        self.play(RemovePiCreatureBubble(self.students[1]), self.teacher.change_mode, 'raise_right_hand', self.teacher.look_at, point)
...
class PlayingTowardsDADX(AreaUnderParabola, ReconfigurableScene):
    CONFIG = {'n_rect_iterations': 6, 'deriv_dx': 0.2, 'graph_origin': 2.5 * DOWN + 6 * LEFT}
    ...
    def describe_sliver(self):
        ...
        dA_rect = Rectangle(width=self.right_v_lines.get_width(), height=self.shadow_rects[-1].get_height(), stroke_width=0, fill_color=YELLOW, fill_opacity=0.5).move_to(self.right_v_lines, DOWN)
        dA_label = OldTex('d', 'A')
        dA_label.next_to(dA_rect, RIGHT, MED_LARGE_BUFF, UP)
        dA_label.set_color(GREEN)
        dA_arrow = Arrow(dA_label.get_bottom() + MED_SMALL_BUFF * DOWN, dA_rect.get_center(), buff=0, color=WHITE)
        ...
        self.wait()
        self.play(ReplacementTransform(dA_label[0].copy(), difference_in_area[0]), ReplacementTransform(dA_label[1].copy(), difference_in_area[2]), *list(map(FadeIn, [difference_in_area[1], difference_in_area[3]])))
        self.wait(2)
        self.play(FadeIn(dA_rect), Animation(dA_arrow))
        self.play(GrowFromCenter(side_brace))
        self.play(graph_label_copy.set_color, WHITE, graph_label_copy.next_to, side_brace, LEFT, SMALL_BUFF)
        self.wait()
    ...
    def write_dA_dx(self):
        ...
        dx = self.dx_group[1]
        self.play(*[ReplacementTransform(mob.copy(), equation.get_part_by_tex(tex), run_time=2) for mob, tex in [(x_squared, f_tex), (dx, 'dx'), (dA, 'dA')]])
        ...
        for tex, mob in ((f_tex, x_squared), ('dx', dx)):
            ...
            self.wait(2)
        self.play(*[ReplacementTransform(equation.get_part_by_tex(tex), deriv_equation.get_part_by_tex(tex), run_time=2) for tex in ('dA', 'approx', f_tex, 'dx')] + [Write(deriv_equation.get_part_by_tex('over'))])
        self.wait(2)
    ...
    def dA_remains_a_mystery(self):
        ...
        A_circle, dA_circle = [Circle(color=color).replace(mob, stretch=True).scale(1.5) for mob, color in [(self.A_func, RED), (self.deriv_equation, GREEN)]]
        q_marks = OldTex('???')
        ...
        self.play(Blink(randy))
        self.wait()
        self.play(randy.change_mode, 'surprised', randy.look_at, dA_circle, ReplacementTransform(A_circle, dA_circle))
        self.play(Blink(randy))
        self.wait()
    ...
    def show_dA_dx_in_detail(self):
        d = self.default_right_x
        expression = OldTex('{A(', '%d.001' % d, ') ', '-A(', '%d' % d, ')', '\\over \\,', '0.001}', '\\approx', '%d' % d, '^2')
        expression.scale(0.9)
        expression.next_to(self.deriv_equation, DOWN, MED_LARGE_BUFF)
        expression.to_edge(RIGHT)
        self.play(ReplacementTransform(self.three_plus_dx.get_part_by_tex('%d.001' % d).copy(), expression.get_part_by_tex('%d.001' % d)), Write(VGroup(expression.get_part_by_tex('A('), expression.get_part_by_tex(')'))))
        self.wait()
        self.play(ReplacementTransform(self.three.get_part_by_tex('%d' % d).copy(), expression.get_part_by_tex('%d' % d, substring=False)), Write(VGroup(expression.get_part_by_tex('-A('), expression.get_parts_by_tex(')')[1])))
        self.wait(2)
        self.play(Write(expression.get_part_by_tex('over')), ReplacementTransform(expression.get_part_by_tex('%d.001' % d).copy(), expression.get_part_by_tex('0.001')))
        self.wait()
        self.play(Write(expression.get_part_by_tex('approx')), ReplacementTransform(self.graph_label_copy.copy(), VGroup(*expression[-2:]), run_time=2))
        self.wait()
...
class AlternateAreaUnderCurve(PlayingTowardsDADX):
    CONFIG = {'func': lambda x: (x - 2) ** 3 - 3 * (x - 2) + 6, 'graph_label_tex': 'f(x)', 'deriv_dx': 0.1, 'x_max': 5, 'x_axis_width': 11, 'graph_label_x_val': 4.5}
    ...
    def approximation_improves_for_smaller_dx(self):
        ...
        self.play(approx.set_color, color, ShowCreation(arrow), FadeIn(dx_to_zero_words))
        self.wait()
    ...
    def name_derivative(self):
        ...
        moving_group = VGroup(self.deriv_equation, self.dx_to_zero_words, self.dx_to_zero_words_arrow)
...
class NextVideoWrapper(Scene):
    ...
    def construct(self):
        ...
        titles = [OldTexText('Chapter %d:' % d, s) for d, s in [(2, 'The paradox of the derivative'), (3, 'Derivative formulas through geometry')]]
        for title in titles:
            title.to_edge(UP)
...
class NextVideos(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(RemovePiCreatureBubble(self.teacher, target_mode='raise_right_hand', look_at=this_video), *it.chain(*[[pi.change_mode, 'pondering', pi.look_at, this_video] for pi in self.get_students()]))
        self.play(*[ApplyMethod(pi.look_at, series) for pi in self.get_pi_creatures()])
        self.play(*[ApplyMethod(video.shift, 0.5 * video.get_height() * DOWN, run_time=3, rate_func=squish_rate_func(there_and_back, alpha, alpha + 0.3)) for video, alpha in zip(series, np.linspace(0, 0.7, len(series)))])
        self.wait()
        student = self.get_students()[1]
        self.remove(student)
        ...
        self.play(Write(words), student.look_at, words)
...
class Chapter1PatreonThanks(PatreonThanks):
    CONFIG = {'specific_patrons': ['Ali Yahya', 'CrypticSwarm', 'Juan    Benet', 'Yu  Jun', 'Othman  Alikhan', 'Markus  Persson', 'Joseph  John Cox', 'Luc Ritchie', 'Einar Johansen', 'Rish    Kundalia', 'Achille Brighton', 'Kirk Werklund', 'Ripta   Pasay', 'Felipe  Diniz'], 'patron_scale_val': 0.9}

class EndScreen(PiCreatureScene):
    CONFIG = {'seconds_to_blink': 3}
    ...
    def construct(self):
        ...
        words.to_edge(UP)
        self.play(FadeIn(words, run_time=2, lag_ratio=0.5), self.pi_creature.change_mode, 'hooray')
        self.wait()
        mode_point_pairs = [('raise_left_hand', 5 * LEFT + 3 * UP), ('raise_right_hand', 5 * RIGHT + 3 * UP), ('thinking', 5 * LEFT + 2 * DOWN), ('thinking', 5 * RIGHT + 2 * DOWN), ('thinking', 5 * RIGHT + 2 * DOWN), ('happy', 5 * LEFT + 3 * UP), ('raise_right_hand', 5 * RIGHT + 3 * UP)]
...
class Thumbnail(AlternateAreaUnderCurve):
    CONFIG = {'x_axis_label': '', 'y_axis_label': '', 'graph_origin': 2.4 * DOWN + 3 * LEFT}
    ...
    def construct(self):
        ...
        graph = self.get_graph(self.func)
        rects = self.get_riemann_rectangles(graph, x_min=0, x_max=4, dx=0.25, start_color=BLUE_E)
