...
class ThisWasConfusing(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.get_teacher().change_mode, 'raise_right_hand', words.set_fill, None, 1, words.shift, 0.5 * UP)
        self.play_student_changes(*['confused'] * 3, look_at=words, added_anims=[Animation(self.get_teacher())])
        self.wait()
        self.play(self.get_teacher().change_mode, 'confused', self.get_teacher().look_at, words)
        self.wait(3)
...
class SlopeOfCircleExample(ZoomedScene):
    ...
    def introduce_circle(self):
        circle = Circle(radius=self.circle_radius * SPACE_UNIT_TO_PLANE_UNIT, color=self.circle_color)
        equation = OldTex('x^2 + y^2 = 5^2')
        equation.add_background_rectangle()
        equation.next_to(circle.point_from_proportion(1.0 / 8), UP + RIGHT)
    ...
    def talk_through_pythagorean_theorem(self):
        ...
        for line, tex in zip(lines, '5xy'):
            ...
            label.next_to(line.get_center(), rotate_vector(UP, line.get_angle()), buff=SMALL_BUFF)
        ...
        def update_full_group(group, alpha):
            ...
            for line, label in zip(lines, labels):
                label.next_to(line.get_center(), rotate_vector(UP, line.get_angle()), buff=SMALL_BUFF)
            return group
        self.play(UpdateFromAlphaFunc(full_group, update_full_group, rate_func=there_and_back, run_time=5))
        self.wait(2)
        movers = labels.copy()
        pairs = list(zip([movers[1], movers[2], movers[0]], self.circle_equation[1][0:-1:3]))
    ...
    def show_perpendicular_radius(self):
        ...
        perp_mark = VGroup(Line(UP, UP + RIGHT), Line(UP + RIGHT, RIGHT))
        perp_mark.scale(0.2)
        ...
        self.play(PiCreatureBubbleIntroduction(morty, "Suppose you \\\\ don't know this."))
    ...
    def show_dx_and_dy(self):
        ...
        step_length = 1.0 / self.zoom_factor
        ...
        step_group.scale(1.0 / self.zoom_factor)
        ...
        for line, tex in ((dx_line, 'dx'), (dy_line, 'dy')):
            label = OldTex(tex)
            label.scale(1.0 / self.zoom_factor)
            next_to_vect = np.round(rotate_vector(DOWN, line.get_angle()))
            label.next_to(line, next_to_vect, buff=MED_SMALL_BUFF / self.zoom_factor)
            label.set_color(line.get_color())
            line.label = label
        ...
        self.wait()
        self.play(self.little_rectangle.restore, dot.scale, 1.0 / self.zoom_factor, run_time=2)
        self.wait()
    ...
    def perform_implicit_derivative(self):
        ...
        rect = Rectangle(width=FRAME_X_RADIUS - SMALL_BUFF, height=FRAME_Y_RADIUS - SMALL_BUFF, stroke_width=0, fill_color=BLACK, fill_opacity=0.8)
        ...
        dx.set_color(GREEN)
        dy.set_color(RED)
        self.play(FadeIn(rect), FadeIn(morty), equation.next_to, ORIGIN, DOWN, MED_LARGE_BUFF, equation.shift, FRAME_X_RADIUS * RIGHT / 2)
        self.play(morty.change_mode, 'confused', morty.look_at, equation)
        self.play(Blink(morty))
        derivative.next_to(equation, DOWN)
        derivative.shift(equation[1][-3].get_center()[0] * RIGHT - derivative[-2].get_center()[0] * RIGHT)
        self.play(morty.look_at, derivative[0], *[ReplacementTransform(equation[1][i].copy(), derivative[j]) for i, j in ((1, 0), (0, 1))])
        self.play(Write(dx, run_time=1))
        self.wait()
        self.play(*[ReplacementTransform(equation[1][i].copy(), derivative[j]) for i, j in ((2, 4), (3, 6), (4, 5))])
        self.play(Write(dy, run_time=1))
        self.play(Blink(morty))
        self.play(*[ReplacementTransform(equation[1][i].copy(), derivative[j]) for i, j in ((-3, -2), (-2, -1), (-1, -1))])
        self.wait()
        ...
        self.play(Blink(morty))
        self.play(morty.change_mode, 'pondering', morty.look_at, derivative)
        x, y, eq = np.array(derivative)[[1, 6, 9]]
        final_form = OldTex('\\frac{dy}{dx} = \\frac{-x}{y}')
        ...
        self.play(*[ReplacementTransform(mover.copy(), target, run_time=2, path_arc=np.pi / 2) for mover, target in [(dy, new_dy), (dx, new_dx), (eq, new_eq), (x, new_x), (y, new_y)]] + [morty.look_at, final_form])
        self.wait(2)
        self.morty = morty
    ...
    def show_final_slope(self):
        ...
        self.play(FadeIn(rect), MoveToTarget(frac), Write(eq), morty.look_at, rect, run_time=2)
        self.wait()
        self.play(FocusOn(coords), morty.look_at, coords)
        self.play(Indicate(coords))
        scale_factor = 1.4
        self.play(x.scale, scale_factor, x.set_color, GREEN, x.move_to, frac[1], FadeOut(frac[1]), y.scale, scale_factor, y.set_color, RED, y.move_to, frac[3], DOWN, y.shift, SMALL_BUFF * UP, FadeOut(frac[3]), morty.look_at, frac, run_time=2)
        self.wait()
        self.play(Blink(morty))
...
class NameImplicitDifferentation(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.get_teacher().change_mode, 'raise_right_hand', ShowCreation(arrow))
        self.play_student_changes(*['confused'] * 3, look_at=derivative, added_anims=[ReplacementTransform(equation.copy(), derivative)])
        self.wait(2)
        self.teacher_says("Don't worry...", added_anims=[group.scale, 0.7, group.to_corner, UP + LEFT])
        self.play_student_changes(*['happy'] * 3)
        self.wait(3)

class Ladder(VMobject):
    CONFIG = {'height': 4, 'width': 1, 'n_rungs': 7}

    def init_points(self):
        left_line, right_line = [Line(ORIGIN, self.height * UP).shift(self.width * vect / 2.0) for vect in (LEFT, RIGHT)]
        rungs = [Line(left_line.point_from_proportion(a), right_line.point_from_proportion(a)) for a in np.linspace(0, 1, 2 * self.n_rungs + 1)[1:-1:2]]
        self.add(left_line, right_line, *rungs)
        self.center()

class RelatedRatesExample(ThreeDScene):
    CONFIG = {'start_x': 3.0, 'start_y': 4.0, 'wall_dimensions': [0.3, 5, 5], 'wall_color': color_gradient([GREY_BROWN, BLACK], 4)[1], 'wall_center': 1.5 * LEFT + 0.5 * UP}
    ...
    def introduce_ladder(self):
        ...
        wall = Prism(dimensions=self.wall_dimensions, fill_color=self.wall_color, fill_opacity=1)
    ...
    def slide_ladder(self):
        ...
        x_line, y_line = x_and_y_lines
        down_arrow, left_arrow = [Arrow(ORIGIN, vect, color=YELLOW, buff=0) for vect in (DOWN, LEFT)]
        ...
        added_anims = [UpdateFromFunc(brace, self.update_brace), UpdateFromFunc(x_and_y_lines, self.update_x_and_y_lines), Animation(down_arrow)]
    ...
    def ponder_question(self):
        ...
        self.wait()
        self.play(randy.change_mode, 'confused', randy.look_at, self.ladder.get_top())
    ...
    def write_equation(self):
        ...
        self.wait()
        self.let_ladder_fall(self.ladder, y_label.shift, self.start_y * DOWN / 2, *self.get_added_anims_for_ladder_fall()[:-1], rate_func=lambda t: 0.2 * there_and_back(t), run_time=3)
        self.play(FocusOn(x_label))
        self.play(Write(x_label))
        self.wait(2)
        self.play(ReplacementTransform(x_label.copy(), equation[0]), ReplacementTransform(y_label.copy(), equation[3]), Write(VGroup(*np.array(equation)[[1, 2, 4, 5, 6]])))
        self.wait(2)
        self.let_ladder_fall(self.ladder, *self.get_added_anims_for_ladder_fall(), rate_func=there_and_back, run_time=6)
        self.wait()
        self.equation = equation

    def isolate_x_of_t(self):
        alt_equation = OldTex('x(t)', '=', '\\big(5^2', '-', 'y(t)', '^2 \\big)', '^{1/2}')
        ...
        randy = Randolph()
        randy.next_to(alt_equation, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        ...
        self.play(FadeIn(randy))
        self.play(randy.change_mode, 'raise_right_hand', randy.look_at, alt_equation, *[ReplacementTransform(self.equation[i].copy(), alt_equation[j], path_arc=np.pi / 2, run_time=3, rate_func=squish_rate_func(smooth, j / 12.0, (j + 6) / 12.0)) for i, j in enumerate([0, 6, 3, 4, 5, 1, 2])])
        self.play(Blink(randy))
        self.wait()
        self.play(Write(find_dx_dt), randy.change_mode, 'pondering', randy.look_at, find_dx_dt)
        self.let_ladder_fall(self.ladder, *self.get_added_anims_for_ladder_fall(), run_time=8, rate_func=there_and_back)
    ...
    def discuss_lhs_as_function(self):
        ...
        constant_words = OldTexText('that happens to\n            be constant')
        constant_words.set_color(YELLOW)
        constant_words.next_to(function_of_time, DOWN)
        derivative = OldTex('\\frac{d\\left(x(t)^2 + y(t)^2 \\right)}{dt}')
        derivative.next_to(equation, DOWN, buff=MED_LARGE_BUFF)
        derivative.shift(equation[0][0].get_center()[0] * RIGHT - derivative[2].get_center()[0] * RIGHT)
        derivative_interior = lhs.copy()
        derivative_interior.move_to(VGroup(*derivative[2:13]))
        derivative_scaffold = VGroup(*list(derivative[:2]) + list(derivative[13:]))
        self.play(GrowFromCenter(brace), Write(function_of_time))
        self.wait()
        self.play(Write(constant_words))
        self.let_ladder_fall(self.ladder, *self.get_added_anims_for_ladder_fall(), run_time=6, rate_func=lambda t: 0.5 * there_and_back(t))
        self.wait()
        self.play(*list(map(FadeOut, [brace, constant_words, function_of_time])))
        self.play(ReplacementTransform(lhs.copy(), derivative_interior), Write(derivative_scaffold))
        self.wait()
    ...
    def let_dt_pass(self):
        ...
        self.play(*list(map(FadeOut, [self.dy_arrow, self.dy_label, self.dx_arrow, self.dx_label])))
        self.add(shadow_ladder)
        self.let_ladder_fall(self.ladder, *self.get_added_anims_for_ladder_fall(), rate_func=lambda t: 0.1 * smooth(t), run_time=1)
        ...
        for line, brace, label in ((dy_line, dy_brace, dy_label), (dx_line, dx_brace, dx_label)):
            self.play(ShowCreation(line), GrowFromCenter(brace), Write(label), run_time=1)
        self.wait()
    ...
    def take_derivative_of_rhs(self):
        ...
        self.wait()
        self.reset_ladder(self.ladder, *self.get_added_anims_for_ladder_fall() + [Animation(self.dy_group), Animation(self.dx_group)], rate_func=there_and_back, run_time=3)
        self.wait()
    ...
    def take_derivative_of_lhs(self):
        ...
        lhs_derivative = OldTex('2', 'x(t)', '\\frac{dx}{dt}', '+', '2', 'y(t)', '\\frac{dy}{dt}')
        ...
        lhs_derivative.next_to(derivative_scaffold, DOWN, aligned_edge=RIGHT, buff=MED_LARGE_BUFF)
        equals_zero_copy.next_to(lhs_derivative, RIGHT)
        pairs = [(0, 1), (1, 0), (2, 3), (3, 5), (4, 4)]

        def perform_replacement(index_pairs):
            self.play(*[ReplacementTransform(equation[i].copy(), lhs_derivative[j], path_arc=np.pi / 2, run_time=2) for i, j in index_pairs])
        perform_replacement(pairs[:2])
        self.play(Write(lhs_derivative[2]))
        self.wait()
        self.play(Indicate(VGroup(*list(lhs_derivative[:2]) + list(lhs_derivative[2][:2])), run_time=2))
        ...
        self.play(ReplacementTransform(self.equals_zero.copy(), equals_zero_copy))
        self.wait(2)
    ...
    def bring_back_velocity_arrows(self):
        ...
        self.reset_ladder(*ladder_fall_args + [FadeOut(dx_dy_group), FadeOut(self.derivative), FadeOut(self.equals_zero), self.lhs_derivative.shift, 2 * UP])
    ...
    def replace_terms_in_final_form(self):
        ...
        new_lhs_derivative = OldTex('2', '(%d)' % int(self.start_x), '\\frac{dx}{dt}', '+', '2', '(%d)' % int(self.start_y), '(-1)', '= 0')
        new_lhs_derivative[1].set_color(GREEN)
        VGroup(*new_lhs_derivative[2][:2]).set_color(GREEN)
        new_lhs_derivative[5].set_color(RED)
        new_lhs_derivative.next_to(self.lhs_derivative, DOWN, buff=MED_LARGE_BUFF, aligned_edge=RIGHT)

        def fill_in_equation_part(*indices):
            self.play(*[ReplacementTransform(self.lhs_derivative[i].copy(), new_lhs_derivative[i], run_time=2) for i in indices])
    ...
    def write_final_solution(self):
        solution = OldTex('\\frac{dx}{dt} = \\frac{4}{3}')
        for i in (0, 1, -1):
            solution[i].set_color(GREEN)
        solution[-3].set_color(RED)
        solution.next_to(self.new_lhs_derivative, DOWN, buff=MED_LARGE_BUFF, aligned_edge=RIGHT)
    ...
    def get_added_anims_for_ladder_fall(self):
        return [UpdateFromFunc(self.ladder_brace, self.update_brace), UpdateFromFunc(self.x_and_y_lines, self.update_x_and_y_lines), UpdateFromFunc(self.x_and_y_labels, self.update_x_and_y_labels)]
...
class LightweightCircleExample(SlopeOfCircleExample):
    CONFIG = {'skip_animations': True, 'plane_kwargs': {'x_radius': 5, 'y_radius': 5, 'space_unit_to_x_unit': SPACE_UNIT_TO_PLANE_UNIT, 'space_unit_to_y_unit': SPACE_UNIT_TO_PLANE_UNIT}}
    ...
    def construct(self):
        ...
        self.example_point_coords_mob.scale(1.5, about_point=self.example_point_coords_mob.get_corner(UP + RIGHT))
...
class CompareLadderAndCircle(PiCreatureScene, ThreeDScene):
    ...
    def introduce_both_scenes(self):
        ...
        for mobs, vect in ((ladder_mobs, LEFT), (circle_mobs, RIGHT)):
            ...
            mobs.next_to(self.pi_creature.get_corner(UP + vect), UP, buff=SMALL_BUFF, aligned_edge=-vect)
        ladder_mobs.save_state()
        ladder_mobs.fade(1)
        ladder_mobs.rotate(np.pi / 3, UP)
        self.play(self.pi_creature.change_mode, 'raise_right_hand', self.pi_creature.look_at, ladder_mobs, ApplyMethod(ladder_mobs.restore, run_time=2))
        self.play(self.pi_creature.change_mode, 'raise_left_hand', self.pi_creature.look_at, circle_mobs, Write(circle_mobs, run_time=2))
        self.wait(2)
        self.play(circle_mobs.to_edge, RIGHT, ladder_mobs.to_edge, LEFT)

    def show_derivatives(self):
        equation = OldTex('x', '^2', '+', 'y', '^2', '= 5^2')
        derivative = OldTex('2', 'x', 'dx', '+', '2', 'y', 'dy', '=0')
        self.color_equations(equation, derivative)
        equation.to_edge(UP)
        equation.shift(MED_LARGE_BUFF * LEFT)
        derivative.next_to(equation, DOWN, buff=MED_LARGE_BUFF)
        self.play(Write(equation), self.pi_creature.change_mode, 'plain', self.pi_creature.look_at, equation)
        self.wait()
        self.play(*[ReplacementTransform(equation[i].copy(), derivative[j], path_arc=np.pi / 2) for i, j in enumerate([1, 0, 3, 5, 4, 7])] + [Write(derivative[j]) for j in (2, 6)])
        self.play(self.pi_creature.change_mode, 'pondering', self.pi_creature.look_at, derivative)
        self.wait()
        self.equation = equation
    ...
    def comment_on_ladder_derivative(self):
        equation = self.equation
        derivative = self.derivative
        time_equation = OldTex('x(t)', '^2', '+', 'y(t)', '^2', '= 5^2')
        time_derivative = OldTex('2', 'x(t)', '\\frac{dx}{dt}', '+', '2', 'y(t)', '\\frac{dy}{dt}', '=0')
        ...
        self.wait(2)
        self.play(equation.restore, derivative.restore, FadeOut(brace), FadeOut(brace_text), self.pi_creature.change_mode, 'confused')
        self.wait()
    ...
    def comment_on_circle_derivative(self):
        ...
        for mob in (dx, dy):
            ...
            self.wait()
        self.play(self.pi_creature.change_mode, 'pondering', self.pi_creature.look, DOWN + LEFT)
        self.wait(2)
...
class TwoVariableFunctionAndDerivative(SlopeOfCircleExample):
    CONFIG = {'zoomed_canvas_corner': DOWN + RIGHT, 'zoomed_canvas_frame_shape': (3, 4)}
    ...
    def show_example_point(self):
        ...
        circle = Circle(radius=self.circle_radius * self.plane.space_unit_to_x_unit, color=self.circle_color)
        self.play(ReplacementTransform(self.s_expression.copy(), new_s_expression), ShowCreation(dot))
    ...
    def shift_example_point(self, coords):
        ...
        self.play(self.example_point_dot.move_to, point, Transform(self.example_point_label, point_label))
        self.wait(2)

    def take_derivative_symbolically(self):
        equation = self.equation
        derivative = OldTex('dS =', '2', 'x', '\\,dx', '+', '2', 'y', '\\,dy')
        derivative[2].set_color(GREEN)
        derivative[6].set_color(RED)
        derivative.next_to(equation, DOWN, buff=MED_LARGE_BUFF)
        derivative.add_background_rectangle()
        derivative.to_edge(LEFT)
        self.play(*[FadeIn(derivative[0])] + [ReplacementTransform(self.s_expression[1][0].copy(), derivative[1][0])] + [Write(derivative[1][j]) for j in (3, 7)])
        self.play(*[ReplacementTransform(equation[1][i].copy(), derivative[1][j], path_arc=np.pi / 2, run_time=2, rate_func=squish_rate_func(smooth, (j - 1) / 12.0, (j + 6) / 12.0)) for i, j in enumerate([2, 1, 4, 6, 5])])
        self.wait(2)
        self.derivative = derivative
    ...
    def show_dx_dy_step(self):
        ...
        for line, tex, vect in ((dx_line, 'dx', UP), (dy_line, 'dy', LEFT)):
            ...
            label.add_background_rectangle()
            label.scale(1.0 / self.zoom_factor, about_point=line.get_center())
            line.label = label
        self.activate_zooming()
        ...
        self.wait()
        self.play(FadeOut(rhs), dot.scale, 1.0 / self.zoom_factor, point, s_label.scale, 1.0 / self.zoom_factor, point, lil_rect.restore, run_time=2)
        self.wait()
        ...
        new_s_label = self.get_s_expression('%d + dx' % int(self.example_point[0]), '%d + dy' % int(self.example_point[1]))
        new_dot.set_height(dot.get_height())
        new_dot.move_to(new_point)
        new_s_label.set_height(s_label.get_height())
        new_s_label.scale(0.8)
        new_s_label.next_to(new_dot, DOWN, buff=SMALL_BUFF / self.zoom_factor, aligned_edge=LEFT)
        ...
        self.wait()
        self.tiny_step_group = VGroup(dx_line, dx_line.label, dy_line, dy_line.label, s_label, new_s_label, new_dot)

    def plug_in_example_values(self):
        deriv_example = OldTex('dS =', '2', '(3)', '\\,(-0.02)', '+', '2', '(4)', '\\,(-0.01)')
        ...
        def add_example_parts(*indices):
            self.play(*[ReplacementTransform(self.derivative[1][i].copy(), deriv_example[1][i], run_time=2) for i in indices])
        ...
        randy.to_edge(DOWN)
        self.play(FadeIn(randy))
        self.play(randy.change_mode, 'pondering', randy.look_at, deriv_example.get_left())
        self.play(Blink(randy))
        self.play(randy.look_at, deriv_example.get_right())
        self.wait()
        self.play(Indicate(self.equation), randy.look_at, self.equation)
        self.play(Blink(randy))
        self.play(randy.change_mode, 'thinking', randy.look_at, self.big_rectangle)
        self.wait(2)
    ...
    def ask_about_equalling_zero(self):
        ...
        self.play(Write(equals_zero), randy.change_mode, 'confused', randy.look_at, equals_zero)
        self.wait()
        self.play(Blink(randy))
        self.play(randy.change_mode, 'plain', randy.look_at, self.big_rectangle)
        self.play(FadeOut(self.tiny_step_group), self.little_rectangle.move_to, dot)
        self.wait()
        self.equals_zero = equals_zero
    ...
    def show_tangent_step(self):
        ...
        s_label.scale(1.0 / self.zoom_factor, about_point=point)
        arrow1, arrow2 = [Arrow(s_label.get_top(), mob, preserve_tip_size_when_scaling=False, color=self.example_color, buff=SMALL_BUFF / self.zoom_factor, tip_length=0.15 / self.zoom_factor) for mob in (dot, new_dot)]
        ...
        self.play(ShowCreation(new_dot))
        self.play(randy.change_mode, 'pondering', randy.look_at, self.big_rectangle)
        self.play(Blink(randy))
        self.wait()
    ...
    def show_tangent_line(self):
        ...
        line.move_to(point)
        self.play(PiCreatureSays(randy, 'Approximately...'))
        self.play(Blink(randy))
        self.play(RemovePiCreatureBubble(randy))
        self.play(GrowFromCenter(line), randy.look_at, line)
        self.wait(2)
        self.play(self.little_rectangle.scale, self.zoom_factor / 2, run_time=4, rate_func=there_and_back)
        self.wait(2)
...
class TryOtherExamples(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        formula.next_to(self.get_teacher().get_corner(UP + LEFT), UP, buff=MED_LARGE_BUFF, aligned_edge=RIGHT)
...
class AlternateExample(ZoomedScene):
    CONFIG = {'example_color': MAROON_B, 'zoom_factor': 10, 'zoomed_canvas_corner': DOWN + RIGHT, 'zoomed_canvas_frame_shape': (3, 4)}
    ...
    def add_plane(self):
        ...
        plane = NumberPlane(space_unit_to_x_unit=0.75, x_radius=FRAME_WIDTH)
        plane.fade()
    ...
    def draw_graph(self):
        graphs = VGroup(*[FunctionGraph(lambda x: u * np.sqrt(x / np.sin(x)), num_steps=200, x_min=x_min + 0.1, x_max=x_max - 0.1) for u in [-1, 1] for x_min, x_max in [(-4 * np.pi, -2 * np.pi), (-np.pi, np.pi), (2 * np.pi, 4 * np.pi)]])
        graphs.stretch(self.plane.space_unit_to_x_unit, dim=0)
        self.play(ShowCreation(graphs, run_time=3, lag_ratio=0), Animation(self.formula))
        self.wait()
        self.graphs = graphs
    ...
    def emphasize_meaning_of_points(self):
        ...
        self.play(UpdateFromAlphaFunc(dot, update_dot), UpdateFromFunc(label, update_label), run_time=3)
        self.wait()
        self.play(*[ApplyMethod(label[1][i].copy().move_to, self.formula[1][j], run_time=3, rate_func=squish_rate_func(smooth, count / 6.0, count / 6.0 + 2.0 / 3)) for count, (i, j) in enumerate([(1, 4), (1, 9), (3, 6)])])
        movers = self.get_mobjects_from_last_animation()
        self.wait()
        self.play(UpdateFromAlphaFunc(dot, update_dot), UpdateFromFunc(label, update_label), run_time=3, rate_func=lambda t: 1 - smooth(t))
        self.wait()
        self.play(*[ApplyMethod(mover.set_fill, None, 0, remover=True) for mover in movers])
        self.dot = dot
        self.label = label
    ...
    def zoom_in(self):
        ...
        for mob in (VGroup(dot, label), self.little_rectangle):
            self.play(ApplyMethod(mob.scale, 1.0 / self.zoom_factor, method_kwargs={'about_point': dot.get_center()}, run_time=1))
        self.wait()
    ...
    def show_tiny_step(self):
        ...
        for line, tex, vect in ((dx_line, 'dx', DOWN), (dy_line, 'dy', LEFT)):
            ...
            label.add_background_rectangle()
            line.label = label
        arrow = Arrow(point, new_point, buff=0, tip_length=0.15 / self.zoom_factor, color=WHITE)
        self.play(ShowCreation(arrow))
        for line in (dx_line, dy_line):
            self.play(ShowCreation(line), Animation(arrow))
            self.play(Write(line.label, run_time=1))
        self.wait()
        self.step_group = VGroup(arrow, dx_line, dx_line.label, dy_line, dy_line.label)
    ...
    def ask_about_derivatives(self):
        ...
        word.add_background_rectangle()
        word.next_to(Line(lhs.get_center(), rhs.get_center()), DOWN, buff=1.5 * LARGE_BUFF)
    ...
    def differentiate_lhs(self):
        ...
        mnemonic = OldTexText('``', 'Left', ' d-Right', ' + ', 'Right', " d-Left''", arg_separator='')
        ...
        derivative = OldTex('\\sin(x)', '(2y\\,dy)', '+', 'y^2', '(\\cos(x)\\,dx)')
        ...
        derivative_rects = [BackgroundRectangle(VGroup(*subset)) for subset in (derivative[:2], derivative[2:])]
        derivative_rects[1].stretch(1.05, dim=0)
        self.play(GrowFromCenter(brace))
        self.play(Write(mnemonic))
        self.wait()
        pairs = [(sine_rect, derivative_rects[0]), (sine_x, derivative[0]), (y_squared, derivative[1]), (sine_rect, derivative_rects[1]), (y_squared, derivative[2]), (y_squared, derivative[3]), (sine_x, derivative[4])]
        for pairs_subset in (pairs[:3], pairs[3:]):
            self.play(*[ReplacementTransform(m1.copy(), m2, path_arc=np.pi / 2) for m1, m2 in pairs_subset])
            self.wait()
    ...
    def put_step_on_curve(self):
        ...
        graph_points = [graph.point_from_proportion(alpha) for alpha in np.linspace(0, 1, 1000)]
        distances = np.apply_along_axis(lambda p: np.abs(p[0] - arrow_end[0]), 1, graph_points)
        index = np.argmin(distances)
        new_end_point = graph_points[index]
        lil_rect = self.little_rectangle
        self.play(arrow.put_start_and_end_on, point, new_end_point, dy_line.put_start_and_end_on, dy_line.get_start(), new_end_point, MaintainPositionRelativeTo(dy_line.label, dy_line), lil_rect.shift, lil_rect.get_height() * DOWN / 3, run_time=2)
        self.wait(2)
    ...
    def manipulate_to_find_dy_dx(self):
        ...
        words = brace.get_text('Commonly, solve for', '$dy/dx$', buff=SMALL_BUFF)
...
class AskAboutNaturalLog(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.teacher_says('We can find\n            new derivatives', target_mode='hooray')
        self.play_student_changes(*['happy'] * 3)
        self.play(RemovePiCreatureBubble(self.get_teacher(), target_mode='raise_right_hand'))
        self.play(Write(exp_deriv))
        self.wait()
        self.play(Write(log_deriv), exp_deriv.next_to, log_deriv, UP, LARGE_BUFF, *[ApplyMethod(pi.change_mode, 'confused') for pi in self.get_pi_creatures()])
        self.wait(3)

class DerivativeOfNaturalLog(ZoomedScene):
    CONFIG = {'zoom_factor': 10, 'zoomed_canvas_corner': DOWN + RIGHT, 'example_color': MAROON_B}
    ...
    def draw_graph(self):
        graph = FunctionGraph(np.log, x_min=0.01, x_max=FRAME_X_RADIUS, num_steps=100)
    ...
    def describe_as_implicit_curve(self):
        ...
        self.play(Transform(xy_start, xy_end, run_time=2))
        self.wait()
        self.play(UpdateFromAlphaFunc(dot, update_dot), UpdateFromFunc(label, update_label), run_time=3, rate_func=lambda t: 1 - 0.6 * smooth(t))
    ...
    def slope_gives_derivative(self):
        ...
        dy_line = Line(point, interim_point, color=RED)
        dx_line = Line(interim_point, new_point, color=GREEN)
        equation = OldTex('\\text{Slope} = ', '\\frac{dy}{dx} = ', '\\frac{d(\\ln(x))}{dx}')
    ...
    def rearrange_equation(self):
        ...
        new_formula.next_to(formula, DOWN, buff=MED_LARGE_BUFF)
        ...
        self.play(FadeIn(rect), Animation(VGroup(e, new_y, new_eq)), ReplacementTransform(x.copy(), new_x))
        self.wait(2)
    ...
    def take_derivative(self):
        ...
        dx.set_color(GREEN)
        dy.set_color(RED)
        pairs = [(VGroup(e, y), VGroup(new_e, new_y)), (new_y, dy), (eq, new_eq), (x, dx)]
    ...
    def show_tiny_nudge(self):
        ...
        self.play(group.scale, 1.0 / self.zoom_factor, point)
        for line, tex, vect in ((dx_line, 'dx', UP), (dy_line, 'dy', LEFT)):
            label = OldTex(tex)
            label.add_background_rectangle()
            label.next_to(line, vect, buff=SMALL_BUFF)
    ...
    def solve_for_dy_dx(self):
        ...
        rearranged = OldTex('{dy \\over ', ' dx}', '=', '{1 \\over ', 'e', '^y}')
        ...
        self.play(equals_one_over_x.next_to, self.slope_equation, RIGHT, 0, run_time=2)
        self.wait()
    ...
    def show_slope_above_x(self):
        ...
        target_x = 0.2
        graph = FunctionGraph(lambda x: 1.0 / x, x_min=0.1, x_max=FRAME_X_RADIUS, num_steps=100, color=PINK)

        def update_line(line, alpha):
            x = interpolate(start_x, target_x, alpha)
            point = x * RIGHT + np.log(x) * UP
            angle = np.arctan(1.0 / x)
            line.rotate(angle - line.get_angle())
            line.move_to(point)
        self.play(UpdateFromAlphaFunc(line, update_line, rate_func=there_and_back, run_time=6))
        self.wait()
...
class FinalWords(TeacherStudentsScene):
    ...
    def construct(self):
        words = OldTexText('This is a peek into \\\\', 'Multivariable', 'calculus')
        mvc = VGroup(*words[1:])
        ...
        self.play(FadeOut(self.teacher.bubble), FadeOut(VGroup(*words[:1])), mvc.to_corner, UP + LEFT, self.teacher.change_mode, 'raise_right_hand', self.teacher.look_at, formula, Write(formula, run_time=2))
        ...
        self.play(FadeOut(mvc), FadeOut(formula), self.teacher.change_mode, 'plain', FadeIn(series, run_time=2, lag_ratio=0.5))
        self.play(video.set_color, YELLOW, video.shift, video.get_height() * DOWN / 2)
        lim.next_to(video, DOWN)
        self.play(Write(lim), *it.chain(*[[pi.change_mode, 'pondering', pi.look_at, lim] for pi in self.get_pi_creatures()]))
        self.wait(3)

class Chapter6PatreonThanks(PatreonThanks):
    CONFIG = {'specific_patrons': ['Ali  Yahya', 'Meshal  Alshammari', 'CrypticSwarm    ', 'Nathan Pellegrin', 'Karan Bhargava', 'Justin Helps', 'Ankit   Agarwal', 'Yu  Jun', 'Dave    Nicponski', 'Damion  Kistler', 'Juan    Benet', 'Othman  Alikhan', 'Justin Helps', 'Markus  Persson', 'Dan Buchoff', 'Derek   Dai', 'Joseph  John Cox', 'Luc Ritchie', 'Daan Smedinga', 'Jonathan Eppele', 'Nils Schneider', 'Albert Nguyen', 'Mustafa Mahdi', 'Mathew Bramson', 'Guido   Gambardella', 'Jerry   Ling', 'Mark    Govea', 'Vecht', 'Shimin Kuang', 'Rish    Kundalia', 'Achille Brighton', 'Kirk    Werklund', 'Ripta   Pasay', 'Felipe  Diniz']}

class Thumbnail(AlternateExample):

    def construct(self):
        title = VGroup(*list(map(TexText, ['Implicit', 'Differentiation'])))
        title.arrange(DOWN)
        title.scale(3)
