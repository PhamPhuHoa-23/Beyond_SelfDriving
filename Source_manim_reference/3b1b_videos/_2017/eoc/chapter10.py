...
class Chapter10OpeningQuote(OpeningQuote):
    CONFIG = {'quote': ['For me, mathematics is a collection of ', 'examples', '; a ', 'theorem', ' is a statement about a collection of ', 'examples', ' and the purpose of proving ', 'theorems', ' is to classify and explain the ', 'examples', '.'], 'quote_arg_separator': '', 'highlighted_quote_terms': {'examples': BLUE}, 'author': 'John B. Conway', 'fade_in_kwargs': {'run_time': 7}}

class ExampleApproximation(GraphScene):
    CONFIG = {'function': lambda x: np.exp(-x ** 2), 'function_tex': 'e^{-x^2}', 'function_color': BLUE, 'order_sequence': [0, 2, 4], 'center_point': 0, 'approximation_terms': ['1 ', '-x^2', '+\\frac{1}{2}x^4'], 'approximation_color': GREEN, 'x_min': -3, 'x_max': 3, 'y_min': -1, 'y_max': 2, 'graph_origin': DOWN + 2 * LEFT}

    def construct(self):
        self.setup_axes()
        func_graph = self.get_graph(self.function, self.function_color)
        approx_graphs = [self.get_graph(taylor_approximation(self.function, n), self.approximation_color) for n in self.order_sequence]
        ...
        approx_terms.set_fill(self.approximation_color, opacity=0)
        equation.add_background_rectangle()
        approx_graph = VectorizedPoint(self.input_to_graph_point(self.center_point, func_graph))
        self.play(ShowCreation(func_graph, run_time=2), Animation(equation), Animation(near_text))
        for graph, term in zip(approx_graphs, approx_terms):
            self.play(Transform(approx_graph, graph, run_time=2), Animation(equation), Animation(near_text), term.set_fill, None, 1)
            self.wait()
        self.wait(2)

class ExampleApproximationWithSine(ExampleApproximation):
    CONFIG = {'function': np.sin, 'function_tex': '\\sin(x)', 'order_sequence': [1, 3, 5], 'center_point': 0, 'approximation_terms': ['x', '-\\frac{1}{6}x^3', '+\\frac{1}{120}x^5'], 'approximation_color': GREEN, 'x_min': -2 * np.pi, 'x_max': 2 * np.pi, 'x_tick_frequency': np.pi / 2, 'y_min': -2, 'y_max': 2, 'graph_origin': DOWN + 2 * LEFT}

class ExampleApproximationWithExp(ExampleApproximation):
    CONFIG = {'function': np.exp, 'function_tex': 'e^x', 'order_sequence': [1, 2, 3, 4], 'center_point': 0, 'approximation_terms': ['1 + x', '+\\frac{1}{2}x^2', '+\\frac{1}{6}x^3', '+\\frac{1}{24}x^4'], 'approximation_color': GREEN, 'x_min': -3, 'x_max': 4, 'y_min': -1, 'y_max': 10, 'graph_origin': 2 * DOWN + 3 * LEFT}

class Pendulum(ReconfigurableScene):
    CONFIG = {'anchor_point': 3 * UP + 4 * LEFT, 'radius': 4, 'weight_radius': 0.2, 'angle': np.pi / 6, 'approx_tex': ['\\approx 1 - ', '{\\theta', '^2', '\\over', '2}'], 'leave_original_cosine': False, 'perform_substitution': True}
    ...
    def show_oscillation(self):
        ...
        self.play(ShowCreation(trajectory_dots, rate_func=linear, run_time=kwargs['run_time']), Rotate(self.pendulum, -2 * self.angle, **kwargs))
        for m in (2, -2, 2):
    ...
    def show_height(self):
        ...
        self.play(ShowCreation(v_line), ShowCreation(arc), Write(theta))
        self.play(GrowFromCenter(radius_brace))
        self.wait(2)
        self.play(Write(to_write), ReplacementTransform(radius_brace[-1].copy(), height_tex_R), ReplacementTransform(theta.copy(), height_tex_theta), run_time=2)
        self.wait(2)
        self.arc = arc
        self.theta = theta
    ...
    def get_angry_at_cosine(self):
        ...
        self.play(MoveToTarget(cosine), morty.change, 'angry', cosine.target)
        self.wait()
    ...
    def substitute_approximation(self):
        ...
        kwargs = self.get_swing_kwargs()
        self.play(FadeIn(cosine_approx, run_time=2, lag_ratio=0.5), MoveToTarget(cosine), morty.change, 'pondering', cosine_approx)
        self.wait()
        if not self.perform_substitution:
            return
        self.play(ApplyMethod(cosine_approx.theta_squared_over_two.copy().next_to, self.height_tex_R, run_time=2), FadeOut(self.one_minus), morty.look_at, self.height_tex_R)
        self.play(morty.change, 'thinking', self.height_tex_R)
        self.transition_to_alt_config(angle=np.pi / 12, transformation_kwargs={'run_time': 2})
    ...
    def get_pendulum(self):
        ...
        weight = Circle(radius=self.weight_radius, fill_color=GREY, fill_opacity=1, stroke_width=0)
    ...
    def get_trajectory_dots(self, n_dots=40, color=YELLOW):
        ...
        dots = VGroup(*[Dot(angle_to_point(angle), radius=0.005) for angle in angles])
    ...
    def get_height_brace(self):
        ...
        line = Line(h_line.get_end(), h_line.get_end() + height * UP)
    ...
    def get_swing_kwargs(self):
        return {'about_point': self.anchor_point, 'run_time': 1.7, 'rate_func': self.swing_rate_func}

    def swing_rate_func(self, t):
        return (1 - np.cos(np.pi * t)) / 2.0

class PendulumWithBetterApprox(Pendulum):
    CONFIG = {'approx_tex': ['\\approx 1 - ', '{\\theta', '^2', '\\over', '2}', '+', '{\\theta', '^4', '\\over', '24}'], 'leave_original_cosine': True, 'perform_substitution': False}

    def show_confusion(self):
        pass

class ExampleApproximationWithCos(ExampleApproximationWithSine):
    CONFIG = {'function': np.cos, 'function_tex': '\\cos(\\theta)', 'order_sequence': [0, 2], 'approximation_terms': ['1', '-\\frac{1}{2} \\theta ^2'], 'x_axis_label': '$\\theta$', 'y_axis_label': '', 'x_axis_width': 13, 'graph_origin': DOWN}
    ...
    def construct(self):
        ...
        v_lines, alt_v_lines = [VGroup(*[self.get_vertical_line_to_graph(u * dx, high_graph, line_class=DashedLine, color=YELLOW) for u in (-1, 1)]) for dx in (0.01, 0.7)]
        self.play(*list(map(ShowCreation, v_lines)), run_time=2)
        self.play(Transform(v_lines, alt_v_lines, run_time=2))
        self.play(FadeIn(randy))
        self.play(PiCreatureBubbleIntroduction(randy, 'How...?', bubble_type=ThoughtBubble, look_at=self.graph_origin, target_mode='confused'))
        self.wait(2)
        self.play(Blink(randy))
        self.wait()
    ...
    def setup_axes(self):
        ...
        x_val_label_pairs = [(-np.pi, '-\\pi'), (np.pi, '\\pi'), (2 * np.pi, '2\\pi')]
...
class ConstructQuadraticApproximation(ExampleApproximationWithCos):
    CONFIG = {'x_axis_label': '$x$', 'colors': [BLUE, YELLOW, GREEN]}
    ...
    def introduce_quadratic_constants(self):
        ...
        arrows = VGroup(*[Arrow(free_to_change.get_top(), const.get_bottom(), tip_length=0.75 * Arrow.CONFIG['tip_length'], color=const.get_color()) for const in const_terms])
        alt_consts_list = [(0, -1, -0.25), (1, -1, -0.25), (1, 0, -0.25), ()]
        self.play(FadeIn(quadratic_tex, run_time=3, lag_ratio=0.5))
        self.play(FadeIn(free_to_change), *list(map(ShowCreation, arrows)))
        self.play(*[ApplyMethod(const.scale, 0.8, run_time=2, rate_func=squish_rate_func(there_and_back, a, a + 0.75)) for const, a in zip(const_terms, np.linspace(0, 0.25, len(const_terms)))])
    ...
    def set_c0_to_one(self):
        ...
        equals_c0.next_to(poly_at_zero.get_part_by_tex('='), DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        poly_group = VGroup(equals_c0, poly_at_zero, self.quadratic_tex)
        poly_group_target = VGroup(OldTex('=', '1', '+0').set_color_by_tex('1', self.colors[0]), self.get_quadratic_tex('1', 'c_1', 'c_2', arg='0'), self.get_quadratic_tex('1', 'c_1', 'c_2'))
    ...
    def let_c1_and_c2_vary(self):
        alt_consts_list = [(1, 1, -0.25), (1, -1, -0.25), (1, -1, 0.25), (1, 1, -0.1)]
        for alt_consts in alt_consts_list:
            self.change_quadratic_graph(self.quadratic_graph, *alt_consts)
            self.wait()
    ...
    def compute_cosine_derivative(self):
        ...
        self.wait()
        self.play(Rotate(self.tangent_line, np.pi / 12, in_place=True, run_time=3, rate_func=wiggle))
        self.wait()
    ...
    def compute_polynomial_derivative(self):
        ...
        equals_c1.next_to(derivative_at_zero.get_part_by_tex('='), DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        equals_c1.set_color_by_tex('c_1', self.colors[1])
        poly_group = VGroup(equals_c1, derivative, self.quadratic_tex)
        poly_group_target = VGroup(OldTex('=', '0', '+0').set_color_by_tex('0', self.colors[1], substring=False), self.get_quadratic_derivative('0', 'c_2', arg='0'), self.get_quadratic_tex('1', '0', 'c_2'))
        for start, target in zip(poly_group, poly_group_target):
            target.move_to(start)
        self.play(FadeOut(self.free_to_change_group))
    ...
    def show_matching_curvature(self):
        alt_consts_list = [(1, 1, -0.2), (1, 0, -0.2), (1, 0, -0.5)]
        for alt_consts in alt_consts_list:
            self.change_quadratic_graph(self.quadratic_graph, *alt_consts)
            self.wait()
    ...
    def show_matching_tangent_lines(self):
        ...
        tangent_change_anims = [self.get_tangent_line_change_anim(line, np.pi / 2, graph, run_time=6, rate_func=there_and_back) for line, graph in zip(tangent_lines, graphs)]
    ...
    def get_cosine_derivative(self):
        ...
        derivative = OldTex('{d(', '\\cos', ')', '\\over', 'dx}', '(0)')
    ...
    def get_cosine_second_derivative(self):
        ...
        second_deriv = OldTex('{d^2(', '\\cos', ')', '\\over', 'dx^2}', '(', '0', ')')
...
class ReflectOnQuadraticApproximation(TeacherStudentsScene):
    ...
    def show_example_approximation(self):
        approx_at_x, approx_at_point = [OldTex('\\cos(', s, ')', '\\approx', '1 - \\frac{1}{2}', '(', s, ')', '^2').next_to(self.get_students(), UP, 2) for s in ('x', '0.1')]
        ...
        self.wait(2)
        self.play(ReplacementTransform(approx_at_x, approx_at_point))
        self.wait()
        self.play(Write(approx_rhs))
        self.wait(2)
        self.play(real_result.shift, 1.5 * DOWN, real_result.set_fill, None, 1)
        self.play_student_changes(*['hooray'] * 3)
        self.wait(2)
        self.play_student_changes(*['plain'] * 3, added_anims=list(map(FadeOut, [approx_at_point, approx_rhs, real_result])), look_at=approx_at_x)
    ...
    def add_polynomial(self):
        ...
        self.wait(2)
        self.play(*[ApplyMethod(const.shift, MED_LARGE_BUFF * UP, run_time=2, rate_func=squish_rate_func(there_and_back, a, a + 0.7)) for const, a in zip(const_terms, np.linspace(0, 0.3, len(const_terms)))])
        self.wait()
...
class SimilarityOfChangeBehavior(ConstructQuadraticApproximation):
    ...
    def construct(self):
        ...
        tangent_line_movements = [self.get_tangent_line_change_anim(line, max_x, graph, run_time=5) for line, graph in zip(tangent_lines, graphs)]
...
class MoreTerms(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says('More terms!', target_mode='surprised')
        self.play_student_changes(*['hooray'] * 3)
        self.wait(3)

class CubicAndQuarticApproximations(ConstructQuadraticApproximation):
    CONFIG = {'colors': [BLUE, YELLOW, GREEN, RED, MAROON_B]}
    ...
    def add_background(self):
        ...
        self.big_rect = Rectangle(height=FRAME_HEIGHT, width=FRAME_WIDTH, stroke_width=0, fill_color=BLACK, fill_opacity=0.5)
    ...
    def take_third_derivative_of_cubic(self):
        ...
        third_derivative.shift(brace.get_bottom() + MED_SMALL_BUFF * DOWN - third_derivative.get_part_by_tex('0').get_top())
    ...
    def show_third_derivative_of_cosine(self):
        ...
        self.play(dot.move_to, self.polynomial_third_derivative.get_right(), dot.set_fill, None, 0)
        self.wait()

    def set_c3_to_zero(self):
        c3s = VGroup(self.polynomial_third_derivative[-1][-1], self.plus_cubic_term.get_part_by_tex('c_3'))
    ...
    def show_cubic_curves(self):
        ...
        graph.save_state()
        alt_graphs = [self.get_graph(func, color=real_graph.get_color()) for func in [lambda x: x * (x - 1) * (x + 1), lambda x: 1 - 0.5 * x ** 2 + 0.2 * x ** 3]]
    ...
    def add_quartic_term(self):
        ...
        self.play(*list(map(FadeOut, [self.plus_cubic_term, self.polynomial_third_derivative, self.polynomial_third_derivative_brace])))
    ...
    def take_fourth_derivative_of_quartic(self):
        ...
        alt_rhs.next_to(fourth_deriv_lhs.get_part_by_tex('='), DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
    ...
    def solve_for_c4(self):
        c4s = VGroup(self.fourth_deriv_alt_rhs.get_part_by_tex('c_4'), self.fourth_deriv_rhs[-1], self.plus_quartic_term.get_part_by_tex('c_4'))
        ...
        self.play(Transform(c4s, fractions, run_time=3, lag_ratio=0.5), MoveToTarget(x_to_4, run_time=2))
        self.wait(3)

    def show_quartic_approximation(self):
        real_graph = self.quadratic_graph
        graph = real_graph.copy()
        quartic_graph = self.get_graph(lambda x: 1 - x ** 2 / 2.0 + x ** 4 / 24.0, color=graph.get_color())
        tex_mobs = VGroup(*[self.polynomial, self.fourth_deriv_rhs, self.fourth_deriv_alt_rhs, self.cosine_label, self.cosine_derivative, self.cosine_second_derivative, self.cosine_third_derivative[1]])
        for tex_mob in tex_mobs:
            tex_mob.add_to_back(BackgroundRectangle(tex_mob))
        self.play(FadeIn(graph))
        real_graph.set_stroke(width=0)
        self.play(Transform(graph, quartic_graph, run_time=3), Animation(tex_mobs))
        self.wait(3)
    ...
    def take_derivatives_of_monomial(self, term, *added_anims):
        ...
        for k in range(n, 0, -1):
            ...
            try:
                ...
                pass
            self.play(ApplyMethod(exponent_copy.replace, front_num[0], path_arc=np.pi), Write(front_num[1], rate_func=squish_rate_func(smooth, 0.5, 1)), MoveToTarget(curr_term), *possibly_added_anims, run_time=2)
    ...
    def get_cosine_third_derivative(self):
        ...
        third_deriv = OldTex('{d^3(', '\\cos', ')', '\\over', 'dx^3}', '(', '0', ')')
    ...
    def get_cosine_fourth_derivative(self):
        ...
        fourth_deriv = OldTex('{d^4(', '\\cos', ')', '\\over', 'dx^4}', '(', '0', ')')
...
class FactorialTerms(CubicAndQuarticApproximations):
    ...
    def construct(self):
        lhs_list = [OldTex('{d%s' % s, '\\over', 'dx%s}' % s, '(', 'c_8', 'x^8', ')=') for i in range(9) for s in ['^%d' % i if i > 1 else '']]
        for lhs in lhs_list:
            lhs.set_color_by_tex('c_8', YELLOW)
            lhs.next_to(ORIGIN, LEFT)
        lhs_list[0].set_fill(opacity=0)
        added_anims = [ReplacementTransform(start_lhs, target_lhs, rate_func=squish_rate_func(smooth, 0, 0.5)) for start_lhs, target_lhs in zip(lhs_list, lhs_list[1:])]
        term = OldTex('c_8', 'x^8')
        ...
        self.play(ReplacementTransform(coefficient.copy(), words.get_part_by_tex('c_8')), Write(words))
        self.wait(2)

class HigherTermsDontMessUpLowerTerms(Scene):
    CONFIG = {'colors': CubicAndQuarticApproximations.CONFIG['colors'][::2]}
    ...
    def add_polynomial(self):
        ...
        c4_tex = 'c_4'
        polynomial = OldTex('P(x) = ', c0_tex, '+', c2_tex, 'x^2', '+', c4_tex, 'x^4')
        ...
        arrows = VGroup(*[Arrow(c4.get_top(), c.get_top(), path_arc=arc, color=c.get_color()) for c, arc in [(c2, 0.9 * np.pi), (c0, np.pi)]])
    ...
    def show_second_derivative(self):
        second_deriv = OldTex('{d^2 P \\over dx^2}(', '0', ')', '=', '2', self.c2_tex, '+', '3 \\cdot 4', self.c4_tex, '(', '0', ')', '^2')
...
class EachTermControlsOneDerivative(Scene):
    ...
    def construct(self):
        ...
        polynomial = OldTex('P(x) = ', 'c_0', '+', 'c_1', 'x', *it.chain(*[['+', 'c_%d' % n, 'x^%d' % n] for n in range(2, 5)]))
        consts = polynomial.get_parts_by_tex('c')
        deriv_words = VGroup(*[OldTexText('Controls \\\\ $%s(0)$' % tex) for tex in ['P', '\\frac{dP}{dx}'] + ['\\frac{d^%d P}{dx^%d}' % (n, n) for n in range(2, 5)]])
        ...
        for const, deriv, color in zip(consts, deriv_words, colors):
            for mob in (const, deriv):
                mob.set_color(color)
            arrow = Arrow(const.get_top(), deriv.get_bottom(), color=color)
            deriv.arrow = arrow
        self.add(polynomial)
...
class ApproximateNearNewPoint(CubicAndQuarticApproximations):
    CONFIG = {'target_approx_centers': [np.pi / 2, np.pi]}
    ...
    def shift_approximation_center(self):
        ...
        v_line = self.get_vertical_line_to_graph(self.target_approx_centers[-1], self.cosine_graph, line_class=DashedLine, color=YELLOW)
        pi = self.x_axis_labels[1]
        pi.add_background_rectangle()
        self.play(ReplacementTransform(self.cosine_graph.copy(), quartic_graph), DrawBorderThenFill(dot, run_time=1))
    ...
    def change_approximation_center(self, graph, dot, target, **kwargs):
        ...
        self.play(UpdateFromAlphaFunc(graph, update_quartic), UpdateFromAlphaFunc(dot, update_dot), **kwargs)
    ...
    def get_polynomial(self, arg, center_tex):
        result = OldTex('P_{%s}(x)' % center_tex, '=', 'c_0', *it.chain(*[['+', 'c_%d' % d, '%s^%d' % (arg, d)] for d in range(1, 5)]))
...
class TranslationOfInformation(CubicAndQuarticApproximations):
    ...
    def mention_information_exchange(self):
        ...
        self.play(ReplacementTransform(self.cosine_graph.copy(), quadratic_graph), Animation(dot))
        for x in (-1, 1, 0):
            start_x = self.x_axis.point_to_number(dot.get_center())
            self.play(UpdateFromAlphaFunc(dot, lambda d, a: d.move_to(self.input_to_graph_point(interpolate(start_x, x, a), self.cosine_graph)), run_time=2))
        self.wait()
    ...
    def show_derivative_pattern(self):
        derivs_at_x, derivs_at_zero = [VGroup(*[OldTex(tex, '(', arg, ')') for tex in ['\\cos', '-\\sin', '-\\cos', '\\sin', '\\cos']]) for arg in ('x', '0')]
        ...
        for dx, d0, color in zip(derivs_at_x, derivs_at_zero, self.colors):
            for d in (dx, d0):
                d.set_color(color)
            d0.replace(dx)
        rhs_group = VGroup(*[OldTex('=', '%d' % d).scale(0.7).next_to(deriv, RIGHT) for deriv, d in zip(derivs_at_zero, [1, 0, -1, 0, 1])])
        ...
        self.wait()
        self.play(*[ReplacementTransform(zero.copy(), self.dot, run_time=3, rate_func=squish_rate_func(smooth, a, a + 0.4)) for zero, a in zip(zeros, np.linspace(0, 0.6, len(zeros)))])
        self.wait()
    ...
    def show_polynomial(self):
        ...
        self.play(*[Transform(dv, pc, run_time=2, path_arc=np.pi / 2) for dv, pc, a in zip(derivative_values, polynomial.coefficients, np.linspace(0, 0.6, len(derivative_values)))])
    ...
    def name_taylor_polynomial(self):
        ...
        quartic_graph = self.get_graph(lambda x: 1 - x ** 2 / 2.0 + x ** 4 / 24.0, color=GREEN, x_min=-3.2, x_max=3.2)
        quartic_graph.set_color(self.colors[4])
        self.play(GrowFromCenter(brace))
        self.play(Write(name))
        self.wait()
        self.play(Transform(self.quadratic_graph, quartic_graph, run_time=2), Animation(self.dot))
        self.wait(2)
    ...
    def draw_new_function_graph(self):
        ...
        self.play(*list(map(FadeOut, [self.cosine_derivative_group, self.cosine_graph, self.quadratic_graph, self.v_line, self.dot])))
        self.play(ShowCreation(graph))
        self.graph = graph

    def write_general_function_derivative(self):
        derivs_at_x, derivs_at_zero, derivs_at_a = deriv_lists = [VGroup(*[OldTex('\\text{$%s$}' % args[0], *args[1:]) for args in [('f', '(', arg, ')'), ('\\frac{df}{dx}', '(', arg, ')'), ('\\frac{d^2 f}{dx^2}', '(', arg, ')'), ('\\frac{d^3 f}{dx^3}', '(', arg, ')'), ('\\frac{d^4 f}{dx^4}', '(', arg, ')')]]) for arg in ('x', '0', 'a')]
    ...
    def replace_coefficients_in_generality(self):
        ...
        self.play(ReplacementTransform(self.polynomial, new_polynomial, run_time=2, lag_ratio=0.5), FadeOut(polynomial_fourth_term), FadeOut(self.taylor_name_group))
    ...
    def walk_through_terms(self):
        ...
        for new_brace, new_graph in zip(braces[1:], approx_graphs[1:]):
            ...
            self.play(Transform(approx_graph, new_graph, run_time=2), Animation(self.polynomial), Animation(self.dot))
            self.wait()
    ...
    def show_polynomial_around_a(self):
        ...
        def get_update_function(target_x):
            ...
            def update(group, alpha):
                ...
                line.put_start_and_end_on(self.coords_to_point(x, 0), graph_point)
                new_approx_graph = self.get_graph(taylor_approximation(self.graph.underlying_function, self.approx_order, center_point=x), color=graph.get_color())
                Transform(graph, new_approx_graph).update(1)
                return VGroup(graph, dot, line)
            return update
        self.play(UpdateFromAlphaFunc(group, get_update_function(1), run_time=2), Animation(self.polynomial), Animation(polynomial_third_term))
        self.wait()
        ...
        self.wait()
        for x in (-1, np.pi / 6):
            self.play(UpdateFromAlphaFunc(group, get_update_function(x)), Animation(self.polynomial), run_time=4)
            self.wait()

    def get_polynomial(self, arg, *coefficients):
        result = OldTex('P(x) = ', str(coefficients[0]), *list(it.chain(*[['+', str(c), '{%s' % arg, '^%d' % n, '\\over', '%d!}' % n] for n, c in zip(it.count(1), coefficients[1:])])) + ['+ \\cdots'])
        result.scale(0.8)
...
class ExpPolynomial(TranslationOfInformation, ExampleApproximationWithExp):
    CONFIG = {'x_tick_frequency': 1, 'x_leftmost_tick': -3, 'graph_origin': 2 * (DOWN + LEFT), 'y_axis_label': ''}
    ...
    def add_graph(self):
        ...
        self.play(ShowCreation(graph), Write(e_to_x, rate_func=squish_rate_func(smooth, 0.5, 1)), run_time=2)
        self.wait()
        self.graph = graph
        self.e_to_x = e_to_x
    ...
    def show_derivatives(self):
        ...
        derivs_at_x, derivs_at_zero = [VGroup(*[OldTex('e^%s' % s).set_color(c) for c in self.colors]) for s in ('x', '0')]
        derivs_at_x.submobjects[0] = self.e_to_x.target
        arrows = VGroup(*[Arrow(UP, ORIGIN, color=WHITE, buff=SMALL_BUFF, tip_length=0.2) for d in derivs_at_x])
        group = VGroup(*it.chain(*list(zip(derivs_at_x, arrows))))
        group.add(OldTex('\\vdots'))
    ...
    def show_polynomial(self):
        ...
        self.play(*[Transform(dv, pc, run_time=2, path_arc=np.pi / 2) for dv, pc in zip(derivative_values, polynomial.coefficients)])
...
class ShowSecondTerm(TeacherStudentsScene):
    ...
    def construct(self):
        colors = CubicAndQuarticApproximations.CONFIG['colors']
        polynomial = OldTex('f(a)', '+', '\\frac{df}{dx}(a)', '(x - a)', '+', '\\frac{d^2 f}{dx^2}(a)', '(x - a)^2')
        ...
        self.wait(2)
        self.play(RemovePiCreatureBubble(self.teacher, target_mode='raise_right_hand'), Write(polynomial))
        self.play(ShowCreation(box), FadeIn(words))
        self.play_student_changes(*['pondering'] * 3)
        self.wait(3)

class SecondTermIntuition(AreaIsDerivative):
    CONFIG = {'func': lambda x: x * (x - 1) * (x - 2) + 2, 'num_rects': 300, 't_max': 2.3, 'x_max': 4, 'x_labeled_nums': None, 'x_axis_label': '', 'y_labeled_nums': None, 'y_axis_label': '', 'y_min': -1, 'y_max': 5, 'y_tick_frequency': 1, 'variable_point_label': 'x', 'area_opacity': 1, 'default_riemann_start_color': BLUE_E, 'dx': 0.15, 'skip_reconfiguration': False}
    ...
    def introduce_area(self):
        graph = self.v_graph = self.get_graph(self.func, color=WHITE)
        ...
        if not self.skip_animations:
            for target in (1.6, 2.7, self.t_max):
                self.change_area_bounds(new_t_max=target, run_time=3)
    ...
    def nudge_end_point(self):
        ...
        rect.replace(VGroup(VectorizedPoint(self.coords_to_point(new_x, 0)), self.right_v_line), stretch=True)
        ...
        self.play(FadeOut(self.T_label_group), FadeIn(dx_label_group), FadeIn(rect), FadeIn(height_brace))
        self.wait()
        if not self.skip_reconfiguration:
            self.transition_to_alt_config(dx=self.dx / 10.0, run_time=3)
    ...
    def point_out_triangle(self):
        ...
        triangle.replace(Line(self.rect.get_corner(UP + LEFT), self.right_v_line.get_top()), stretch=True)
    ...
    def relabel_start_and_end(self):
        ...
        for vect, tex in ((LEFT, 'a'), (RIGHT, 'x')):
            ...
            arrow = Arrow(label.get_corner(UP - vect), point, buff=SMALL_BUFF, tip_length=0.2, color=WHITE)
            labels.add(label)
            arrows.add(arrow)
    ...
    def compute_triangle_area(self):
        ...
        base_arrow = Arrow(base_label.get_left(), base_line.get_center(), buff=SMALL_BUFF, color=base_line.get_color(), tip_length=0.2)
        height_brace = Brace(triangle, RIGHT, buff=SMALL_BUFF)
        height_labels = [OldTex('\\text{Height} = ', s, '(x-a)') for s in ['(\\text{Slope})', '\\frac{d^2 f_{\\text{area}}}{dx^2}(a)']]
    ...
    def walk_through_taylor_terms(self):
        ...
        geometric_taylor = VGroup(OldTex('f(x) \\approx '), mini_area, OldTex('+'), mini_rect, OldTex('+'), mini_triangle)
        geometric_taylor.arrange(RIGHT, buff=MED_SMALL_BUFF)
        geometric_taylor.to_corner(UP + LEFT)
        analytic_taylor = OldTex('f(x) \\approx', 'f(a)', '+', '\\frac{df}{dx}(a)(x-a)', '+', '\\frac{1}{2}\\frac{d^2 f}{dx^2}(a)(x-a)^2')
        ...
        self.wait()
        self.play(FadeIn(VGroup(*analytic_taylor[:3])), self.dark_area.set_fill, BLUE_E, 1, Transform(self.__name__, new_func_name))
        self.wait()
...
class EachTermHasMeaning(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.teacher_says('Each term \\\\ has meaning!', target_mode='hooray', bubble_config={'height': 3, 'width': 4})
...
class AskAboutInfiniteSum(TeacherStudentsScene):
    ...
    def ask_question(self):
        big_rect = Rectangle(width=FRAME_WIDTH, height=FRAME_HEIGHT, stroke_width=0, fill_color=BLACK, fill_opacity=0.7)
        randy = self.get_students()[1]
        series = OldTex('\\cos(x)', '\\approx', '1 - \\frac{x^2}{2!} + \\frac{x^4}{4!}', ' - \\frac{x^6}{6!}', '+\\cdots')
        ...
        self.wait(3)
        self.play(RemovePiCreatureBubble(self.teacher), self.get_students()[0].change_mode, 'plain', self.get_students()[2].change_mode, 'plain', FadeIn(big_rect), randy.change_mode, 'pondering')
    ...
    def name_taylor_series(self):
        ...
        self.wait(2)
        self.play(series_def.scale, 0.7, series_def.to_corner, UP + RIGHT, FadeIn(taylor_series))
    ...
    def be_careful(self):
        ...
        self.teacher_says('Be careful', bubble_config={'width': 3, 'height': 2}, added_anims=[self.randy.change, 'hesitant'])
        self.wait(2)
...
class ConvergenceExample(Scene):
    ...
    def construct(self):
        ...
        series = OldTex(*list(it.chain(*[['\\frac{1}{%d}' % 3 ** n, '+'] for n in range(1, max_shown_power)])) + ['\\cdots'])
    ...
    def get_partial_sum_braces(self, series, partial_sums):
        ...
        last_brace = braces[-1]
        braces += [braces[-1].copy().stretch_in_place(1 + 0.1 + 0.02 * (n + 1), dim=0).move_to(last_brace, LEFT) for n in range(len(partial_sums) - len(braces))]
...
class ExpConvergenceExample(ConvergenceExample):
    ...
    def get_series(self, arg, n_terms=5):
        series = OldTex('1', '+', *list(it.chain(*[['\\frac{%s^%d}{%d!}' % (arg, n, n), '+'] for n in range(1, n_terms + 1)])) + ['\\cdots'])
...
class ExpGraphConvergence(ExpPolynomial, ExpConvergenceExample):
    CONFIG = {'example_input': 2, 'graph_origin': 3 * DOWN + LEFT, 'n_iterations': 8, 'y_max': 20, 'num_graph_anchor_points': 50, 'func': np.exp}
    ...
    def construct(self):
        ...
        for new_brace, new_graph in zip(self.braces[2:], approx_graphs[2:]):
            self.play(Transform(brace, new_brace), Transform(approx_graph, new_graph), Animation(self.series))
            self.wait()
...
class SecondExpGraphConvergence(ExpGraphConvergence):
    CONFIG = {'example_input': 3, 'n_iterations': 12}

class BoundedRadiusOfConvergence(CubicAndQuarticApproximations):
    CONFIG = {'num_graph_anchor_points': 100}

    def construct(self):
        self.setup_axes()
        func = lambda x: (np.sin(x ** 2 + x) + 0.5) * (np.log(x + 1.01) + 1) * np.exp(-x)
        graph = self.get_graph(func, color=self.colors[0], x_min=-0.99, x_max=self.x_max)
        ...
        outer_v_lines = VGroup(*[DashedLine(self.coords_to_point(x, -2), self.coords_to_point(x, 2), color=WHITE) for x in (-1, 1)])
...
class RadiusOfConvergenceForLnX(ExpGraphConvergence):
    CONFIG = {'x_min': -1, 'x_leftmost_tick': None, 'x_max': 5, 'y_min': -2, 'y_max': 3, 'graph_origin': DOWN + 2 * LEFT, 'func': np.log, 'num_graph_anchor_points': 100, 'initial_n_iterations': 7, 'n_iterations': 11, 'convergent_example': 1.5, 'divergent_example': 2.5}
    ...
    def add_series(self):
        series = OldTex('\\ln(x) \\rightarrow', '(x-1)', '-', '\\frac{(x-1)^2}{2}', '+', '\\frac{(x-1)^3}{3}', '-', '\\frac{(x-1)^4}{4}', '+', '\\cdots')
        lhs = VGroup(*series[1:])
        series.add_background_rectangle()
        series.scale(0.8)
    ...
    def show_bounds(self):
        ...
        self.play(dot.move_to, self.coords_to_point(1, 0), dot.set_fill, YELLOW, 1)
        self.wait()
    ...
    def show_converging_point(self):
        approx_graphs = [self.get_graph(taylor_approximation(self.func, n, 1), color=WHITE) for n in range(1, self.n_iterations + 1)]
        ...
        self.play(*it.chain(list(map(FadeIn, [approx_graph, brace])), list(map(Animation, self.foreground_mobjects))))
        self.wait()
        new_graphs = approx_graphs[1:self.initial_n_iterations]
        for new_graph, new_brace in zip(new_graphs, self.braces[1:]):
            self.play(Transform(approx_graph, new_graph), Transform(brace, new_brace), *list(map(Animation, self.foreground_mobjects)))
            self.wait()
        approx_graph.remove(approx_dot)
        self.play(approx_dot.move_to, self.coords_to_point(self.divergent_example, 0), *it.chain(list(map(FadeOut, [approx_graph, brace])), list(map(Animation, self.foreground_mobjects))))
        self.wait()
    ...
    def show_diverging_point(self):
        ...
        brace = self.braces[0].copy()
        self.play(ReplacementTransform(self.approx_dot, approx_graph.dot), FadeIn(approx_graph[0]), FadeIn(brace), *list(map(Animation, self.foreground_mobjects)))
    ...
    def write_divergence(self):
        ...
        for new_graph in new_graphs:
            self.play(Transform(self.approx_graph, new_graph), *list(map(Animation, self.foreground_mobjects)))
            self.wait()
...
class MoreToBeSaid(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        randy = self.get_students()[1]
        self.teacher_says("There's still \\\\ more to learn!", target_mode='surprised', bubble_config={'height': 3, 'width': 4})
        for word in words:
