...
class Chapter3OpeningQuote(OpeningQuote):
    CONFIG = {'quote': ['You know, for a mathematician, he did not have \\\\ enough', 'imagination.', 'But he has become a poet and \\\\ now he is fine.'], 'highlighted_quote_terms': {'imagination.': BLUE}, 'author': 'David Hilbert'}
...
class PoseAbstractDerivative(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.student_says('Why?', target_mode='sassy', added_anims=[content_copy.scale, 0.8, content_copy.to_corner, UP + LEFT])
...
class ContrastAbstractAndConcrete(Scene):
    ...
    def construct(self):
        ...
        functions = VGroup(*list(map(Tex, ['f(x) = 2x^2 - x^3', 'f(x) = \\sin(x)', 'f(x) = e^x', '\\v_dots'])))
        ...
        self.wait()
        anims = [method(func_mob) for func_mob, method in zip(functions, [self.get_car_anim, self.get_spring_anim, self.get_population_anim])]
        for anim in anims:
    ...
    def get_car_anim(self, alignement_mob):
        ...
        car.move_to(point)
        return MoveCar(car, target_point, run_time=5)

    def get_spring_anim(self, alignement_mob):
        compact_spring, extended_spring = [ParametricCurve(lambda t: t / denom * RIGHT + np.sin(t) * UP + np.cos(t) * OUT, t_max=12 * np.pi) for denom in (12.0, 4.0)]
        for spring in (compact_spring, extended_spring):
            ...
            weight = Square(side_length=0.5, stroke_width=0, fill_color=GREY_B, fill_opacity=1)
            weight.move_to(spring.get_points()[-1])
            spring.add(weight)
        return Transform(compact_spring, extended_spring, rate_func=lambda t: 1 + np.sin(6 * np.pi * t), run_time=5)

    def get_population_anim(self, alignement_mob):
        colors = color_gradient([BLUE_B, BLUE_E], 12)
        pis = VGroup(*[Randolph(mode='happy', color=random.choice(colors)).shift(4 * x * RIGHT + 4 * y * UP + 2 * random.random() * RIGHT + 2 * random.random() * UP) for x in range(20) for y in range(10)])
        pis.set_height(3)
        pis.center()
        ...
        while 2 ** exp < len(pis):
            ...
            anims.append(Transform(VGroup(*[pi.parents for pi in these_pis]), VGroup(*[VGroup(pi, pi.copy()) for pi in these_pis]), lag_ratio=0.5, run_time=2))
            exp += 1
...
class ListOfRules(PiCreatureScene):
    CONFIG = {'use_morty': False}

    def construct(self):
        rules = VGroup(*list(map(Tex, ['\\frac{d}{dx} x^n = nx^{n-1}', '\\frac{d}{dx} \\sin(x) = \\cos(x)', '\\frac{d}{dx} \\cos(x) = -\\sin(x)', '\\frac{d}{dx} a^x = \\ln(a) a^x', '\\vdots'])))
        rules.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        rules[-1].shift(MED_LARGE_BUFF * RIGHT)
        rules.set_height(FRAME_HEIGHT - 1)
        rules.next_to(self.pi_creature, RIGHT)
        rules.to_edge(DOWN)
        self.play(Write(rules), self.pi_creature.change_mode, 'pleading')
        self.change_mode('tired')
        self.wait()

class DerivativeOfXSquaredAsGraph(GraphScene, ZoomedScene, PiCreatureScene):
    CONFIG = {'start_x': 2, 'big_x': 3, 'dx': 0.1, 'x_min': -9, 'x_labeled_nums': list(range(-8, 0, 2)) + list(range(2, 10, 2)), 'y_labeled_nums': list(range(2, 12, 2)), 'little_rect_nudge': 0.5 * (1.5 * UP + RIGHT), 'graph_origin': 2.5 * DOWN + LEFT, 'zoomed_canvas_corner': UP + LEFT, 'zoomed_canvas_frame_shape': (4, 4)}
    ...
    def draw_graph(self):
        ...
        label = self.get_graph_label(graph, 'f(x) = x^2')
    ...
    def ask_about_df_dx(self):
        ss_group = self.get_secant_slope_group(self.start_x, self.graph, dx=self.dx, dx_label='dx', df_label='df')
        secant_line = ss_group.secant_line
        ss_group.remove(secant_line)
        v_line, nudged_v_line = [self.get_vertical_line_to_graph(x, self.graph, line_class=DashedLine, color=RED, dash_length=0.025) for x in (self.start_x, self.start_x + self.dx)]
        ...
        df_dx.next_to(self.input_to_graph_point(self.start_x, self.graph), DOWN + RIGHT, buff=MED_SMALL_BUFF)
        ...
        self.play(FadeIn(self.little_rectangle), FadeIn(self.big_rectangle))
        self.play(ApplyFunction(lambda r: self.position_little_rectangle(r, ss_group), self.little_rectangle), self.pi_creature.change_mode, 'pondering', self.pi_creature.look_at, ss_group)
        self.play(ShowCreation(ss_group.dx_line), Write(ss_group.dx_label))
        self.wait()
        self.play(ShowCreation(ss_group.df_line), Write(ss_group.df_label))
        self.wait()
        self.play(Write(df_dx))
        self.wait()
        self.play(*list(map(FadeOut, [v_line, nudged_v_line])))
    ...
    def show_differing_slopes(self):
        ...
        for target_x in (self.big_x, -self.dx / 2, 1, 2):
            self.animate_secant_slope_group_change(ss_group, target_x=target_x, added_anims=[UpdateFromFunc(self.little_rectangle, rect_update)])
            self.wait()
    ...
    def mention_alternate_view(self):
        ...
        self.play(ApplyMethod(everything.shift, FRAME_WIDTH * LEFT, rate_func=lambda t: running_start(t, -0.1)), self.pi_creature.change_mode, 'happy')
        self.say("Let's try \\\\ another view.", target_mode='speaking')
        self.wait(2)

class NudgeSideLengthOfSquare(PiCreatureScene):
    CONFIG = {'square_width': 3, 'alt_square_width': 5, 'dx': 0.25, 'alt_dx': 0.01, 'square_color': GREEN, 'square_fill_opacity': 0.75, 'three_color': GREEN, 'dx_color': BLUE_B, 'is_recursing_on_dx': False, 'is_recursing_on_square_width': False}
    ...
    def introduce_square(self):
        square = Square(side_length=self.square_width, stroke_width=0, fill_opacity=self.square_fill_opacity, fill_color=self.square_color)
    ...
    def increase_area(self):
        color_kwargs = {'fill_color': YELLOW, 'fill_opacity': self.square_fill_opacity, 'stroke_width': 0}
        ...
        right_line = Line(self.square.get_corner(UP + RIGHT), self.square.get_corner(DOWN + RIGHT), stroke_width=0)
        bottom_line = Line(self.square.get_corner(DOWN + RIGHT), self.square.get_corner(DOWN + LEFT), stroke_width=0)
        corner_point = VectorizedPoint(self.square.get_corner(DOWN + RIGHT))
        little_braces = VGroup()
        for vect in (RIGHT, DOWN):
            brace = Brace(corner_square, vect, buff=SMALL_BUFF)
        ...
        self.play(Transform(right_line, right_rect), Transform(bottom_line, bottom_rect), Transform(corner_point, corner_square), right_brace.next_to, right_rect, RIGHT, SMALL_BUFF, bottom_brace.next_to, bottom_rect, DOWN, SMALL_BUFF)
        ...
        self.wait()
        self.play(*it.chain(*[[mob.shift, vect * SMALL_BUFF] for mob, vect in [(right_rect, RIGHT), (bottom_rect, DOWN), (corner_square, DOWN + RIGHT), (right_brace, RIGHT), (bottom_brace, DOWN), (little_braces, DOWN + RIGHT)]]))
    ...
    def write_df_equation(self):
        ...
        df_equation = VGroup(OldTex('df').set_color(right_rect.get_color()), OldTex('='), right_rect.copy(), OldTexText('+'), right_rect.copy(), OldTex('+'), corner_square.copy())
        df_equation.arrange()
        df_equation.next_to(self.function_label, DOWN, aligned_edge=LEFT, buff=SMALL_BUFF)
        df, equals, r1, plus1, r2, plus2, s = df_equation
        pairs = [(df, self.function_label[0]), (r1, right_rect), (r2, bottom_rect), (s, corner_square)]
        for mover, origin in pairs:
            mover.save_state()
            Transform(mover, origin).update(1)
        self.play(df.restore)
        self.wait()
        self.play(*[mob.restore for mob in (r1, r2, s)] + [Write(symbol) for symbol in (equals, plus1, plus2)], run_time=2)
    ...
    def set_color_shapes(self):
        ...
        for tup in tups:
            self.play(*it.chain(*[[m.scale, 1.2, m.set_color, RED] for m in tup]), rate_func=there_and_back)
            self.wait()
    ...
    def show_smaller_dx(self):
        ...
        for mob in self.get_mobjects():
            mob.save_state()
        self.play(*[Transform(*pair) for pair in zip(self.get_mobjects(), alt_scene.mobjects_at_start_of_show_smaller_dx)])
        self.wait()
    ...
    def rule_of_thumb(self):
        ...
        self.play(Write(safe_to_ignore, run_time=2), self.pi_creature.change_mode, 'raise_right_hand')
        self.play(FadeOut(circle), FadeOut(safe_to_ignore), dx_squared_group.fade, 0.5, dx_squared_group.to_corner, UP + RIGHT, self.pi_creature.change_mode, 'plain')
        self.wait()
    ...
    def write_out_derivative(self):
        ...
        self.play(Transform(r1, self.right_rect), Transform(r2, self.bottom_rect), FadeOut(plus1), FadeOut(self.thin_rect_brace))
        self.play(self.thin_rect_area.next_to, VGroup(df, equals), RIGHT, MED_SMALL_BUFF, UP, self.pi_creature.change_mode, 'thinking')
        self.wait(2)
        self.play(ApplyMethod(df.next_to, frac_line, UP, SMALL_BUFF), ApplyMethod(dx.next_to, frac_line, DOWN, SMALL_BUFF), Write(frac_line), path_arc=-np.pi)
        self.wait()
        ...
        if self.is_recursing_on_square_width or self.is_recursing_on_dx:
            return
        alt_scene = self.__class__(skip_animations=True, square_width=self.alt_square_width, is_recursing_on_square_width=True)
        self.play(*[Transform(*pair) for pair in zip(self.mobjects_at_end_of_write_out_derivative, alt_scene.mobjects_at_end_of_write_out_derivative)])
        self.change_mode('happy')
        self.wait(2)
...
class NudgeSideLengthOfCube(Scene):
    CONFIG = {'x_color': BLUE, 'dx_color': GREEN, 'df_color': YELLOW, 'use_morty': False, 'x': 3, 'dx': 0.2, 'alt_dx': 0.02, 'offset_vect': OUT, 'pose_angle': np.pi / 12, 'pose_axis': UP + RIGHT, 'small_piece_scaling_factor': 0.7, 'allow_recursion': True}

    def construct(self):
        self.states = dict()
        if self.allow_recursion:
            self.alt_scene = self.__class__(skip_animations=True, allow_recursion=False, dx=self.alt_dx)
        self.add_title()
        self.introduce_cube()
        self.write_df_equation()
    ...
    def introduce_cube(self):
        ...
        dx_brace = Brace(dv_pieces[1], LEFT, buff=SMALL_BUFF)
        ...
        self.play(*[ApplyMethod(piece.shift, 0.5 * (piece.get_center() - cube.get_center())) for piece in dv_pieces] + [ApplyMethod(dx_brace.shift, 0.7 * UP)])
        self.wait()
        self.cube = cube
        self.dx_brace = dx_brace
        self.faces, self.bars, self.corner_cube = [VGroup(*[piece for piece in dv_pieces if piece.type == target_type]) for target_type in ('face', 'bar', 'corner_cube')]

    def write_df_equation(self):
        df_equation = VGroup(OldTex('df'), OldTex('='), self.organize_faces(self.faces.copy()), OldTex('+'), self.organize_bars(self.bars.copy()), OldTex('+'), self.corner_cube.copy())
        ...
        for submob, sides in zip(x_squared_dx, [face[0], VGroup(*face[1:])]):
            ...
            self.wait()
        self.play(face.restore, Transform(x_squared_dx, derivative, replace_mobject_with_target_in_scene=True), GrowFromCenter(faces_brace))
        self.wait()
    ...
    def write_derivative(self):
        ...
        self.play(MoveToTarget(derivative), df_equals.next_to, derivative.target[0], LEFT, df_equals.shift, 0.07 * DOWN)
        ...
        self.play(FadeIn(frac_line), FadeIn(frac_line2), df.next_to, frac_line, UP, SMALL_BUFF, dx.next_to, frac_line, DOWN, SMALL_BUFF, MoveToTarget(extra_stuff), Write(dx_below_dx_squared), path_arc=-np.pi)
        self.wait()
        inner_dx = VGroup(*dx_squared[:-1])
        self.play(FadeOut(frac_line2), FadeOut(dx_below_dx_squared), dx_squared[-1].set_color, BLACK, inner_dx.next_to, extra_stuff[0], RIGHT, SMALL_BUFF)
        self.wait()
    ...
    def get_prism(self, width, height, depth):
        color_kwargs = {'fill_color': YELLOW, 'fill_opacity': 0.4, 'stroke_color': WHITE, 'stroke_width': 0.1}
        ...
        for vect in (LEFT, RIGHT, UP, DOWN):
            ...
            side.rotate(np.pi / 2, rotate_vector(vect, -np.pi / 2), about_point=front.get_edge_center(vect))
            face.add(side)
        return face
...
class GraphOfXCubed(GraphScene):
    CONFIG = {'x_min': -6, 'x_max': 6, 'x_axis_width': FRAME_WIDTH, 'x_labeled_nums': list(range(-6, 7)), 'y_min': -35, 'y_max': 35, 'y_axis_height': FRAME_HEIGHT, 'y_tick_frequency': 5, 'y_labeled_nums': list(range(-30, 40, 10)), 'graph_origin': ORIGIN, 'dx': 0.2, 'deriv_x_min': -3, 'deriv_x_max': 3}

    def construct(self):
        self.setup_axes(animate=False)
        graph = self.get_graph(lambda x: x ** 3)
        label = self.get_graph_label(graph, 'f(x) = x^3', direction=LEFT)
        deriv_graph, full_deriv_graph = [self.get_derivative_graph(graph, color=DERIVATIVE_COLOR, x_min=low_x, x_max=high_x) for low_x, high_x in [(self.deriv_x_min, self.deriv_x_max), (self.x_min, self.x_max)]]
        deriv_label = self.get_graph_label(deriv_graph, '\\frac{df}{dx}(x) = 3x^2', x_val=-3, direction=LEFT)
        deriv_label.shift(0.5 * DOWN)
        ss_group = self.get_secant_slope_group(self.deriv_x_min, graph, dx=self.dx, dx_line_color=WHITE, df_line_color=WHITE, secant_line_color=YELLOW)
        self.play(ShowCreation(graph))
        self.play(Write(label, run_time=1))
        self.wait()
        self.play(Write(deriv_label, run_time=1))
        self.play(ShowCreation(ss_group, lag_ratio=0))
        self.animate_secant_slope_group_change(ss_group, target_x=self.deriv_x_max, run_time=10, added_anims=[ShowCreation(deriv_graph, run_time=10)])
...
class PatternForPowerRule(PiCreatureScene):
    ...
    def introduce_pattern(self):
        ...
        for exponent, color in zip(exp_range, colors):
            derivative = OldTex('\\frac{d(x^%d)}{dx} = ' % exponent, '%d x^{%d}' % (exponent, exponent - 1))
        ...
        for derivative in derivatives[-2:]:
            ...
            self.play(derivative.scale, 2, derivative.next_to, derivative, RIGHT, SMALL_BUFF, DOWN)
            self.wait(2)
    ...
    def generalize_pattern(self):
        ...
        lines = VGroup(*[Line(deriv.get_right(), power_rule.get_left(), buff=MED_SMALL_BUFF, color=deriv[0][2].get_color()) for deriv in derivatives])
        self.play(Transform(VGroup(*[d[0].copy() for d in derivatives]), VGroup(power_rule[0]), replace_mobject_with_target_in_scene=True), ShowCreation(lines), lag_ratio=0.5, run_time=2)
        self.wait()
    ...
    def show_hopping(self):
        ...
        for exp, color in zip(exp_range, self.colors[1:]):
            ...
            self.wait()
            self.play(ApplyMethod(lhs_copy[1].replace, form[3], path_arc=np.pi, rate_func=running_start), FadeIn(form[5], rate_func=squish_rate_func(smooth, 0.5, 1)))
            self.wait()
            self.play(self.pi_creature.change_mode, 'hesitant', self.pi_creature.look_at, lhs_copy)
            self.play(*list(map(FadeOut, [form, lhs_copy])))

class PowerRuleAlgebra(Scene):
    CONFIG = {'dx_color': YELLOW, 'x_color': BLUE}
    ...
    def construct(self):
        ...
        self.play(FadeIn(x_dx_to_n), GrowFromCenter(nudge_brace), GrowFromCenter(nudged_output))
        self.wait()
        self.play(Write(VGroup(equals, full_product)), GrowFromCenter(product_brace, rate_func=squish_rate_func(smooth, 0.6, 1)), run_time=3)
        self.wait()
    ...
    def workout_product(self, equals, full_product):
        ...
        xs, dxs = [VGroup(*[submob for submob, tex in product_part_tex_pairs if tex == target_tex]) for target_tex in ('x', 'dx')]
        x_to_n = OldTex('x^n')
        ...
        circle.scale(1.4)
        circle.rotate(Line(derivative_term.get_corner(DOWN + LEFT), derivative_term.get_corner(UP + RIGHT)).get_angle())
    ...
    def pull_out_linear_terms(self, x_to_n, product_part_tex_pairs, xs, dxs):
        ...
        for dx_index, dx in enumerate(dxs):
            if dx is dxs[-1]:
                ...
                self.play(ReplacementTransform(h_dots_copy, v_dots))
                last_group.add(v_dots)
                all_linear_terms.add(v_dots)
            dx_copy = dx.copy()
            xs_copy = xs.copy()
            xs_copy.remove(xs_copy[dx_index])
            self.play(dx_copy.set_color, self.dx_color, xs_copy.set_color, self.x_color, rate_func=squish_rate_func(smooth, 0, 0.5))
        ...
        VGroup(brace, all_linear_terms.target).shift(x_to_n[0].get_right() + MED_LARGE_BUFF * RIGHT - compact[0].get_left())
...
class OneOverX(PiCreatureScene, GraphScene):
    ...
    def introduce_function(self):
        ...
        self.wait()
        self.play(recip_copy.next_to, self.pi_creature, UP + LEFT, self.pi_creature.change_mode, 'raise_right_hand')
        x_to_neg_one.move_to(recip_copy)
        neg_two.replace(neg_one)
        self.play(ReplacementTransform(recip_copy, x_to_neg_one))
        self.wait()
        self.play(neg_one.scale, 1.5, neg_one.next_to, x_to_neg_one, LEFT, SMALL_BUFF, DOWN, rate_func=running_start, path_arc=np.pi)
        self.play(FadeIn(neg_two))
        self.wait()
        self.say('More geometry!', target_mode='hooray', added_anims=[FadeOut(x_to_neg_one), FadeOut(neg_two)], run_time=2)
        self.wait()
    ...
    def introduce_puddle(self):
        ...
        self.play(DrawBorderThenFill(rect_group.rectangle), Write(rect_group.area_label), self.pi_creature.change_mode, 'thinking')
        self.play(GrowFromCenter(rect_group.x_brace), Write(rect_group.x_label))
        self.wait()
        self.play(GrowFromCenter(rect_group.recip_brace), Write(rect_group.recip_label))
        self.setup_axes(animate=True)
        self.wait()
        for d in (2, 3):
            self.change_rectangle_group(rect_group, d, target_group_kwargs={'x_label': str(d), 'one_over_x_label': '\\frac{1}{%d}' % d}, run_time=2)
            self.wait()
    ...
    def introduce_graph(self):
        ...
        self.change_rectangle_group(rect_group, 0.01, added_anims=[ShowCreation(graph)], run_time=5)
        self.change_mode('happy')
    ...
    def perform_nudge(self):
        ...
        recip_brace.target.next_to(new_rect, RIGHT, buff=SMALL_BUFF, aligned_edge=DOWN)
        recip_label = rect_group.recip_label
        recip_label.generate_target()
        recip_label.target.next_to(recip_brace.target, RIGHT)
        h_lines = VGroup(*[DashedLine(ORIGIN, (rect_copy.get_width() + LARGE_BUFF) * RIGHT, color=self.df_color, stroke_width=2).move_to(rect.get_corner(UP + LEFT), LEFT) for rect in (rect_group.rectangle, new_rect)])
        v_lines = VGroup(*[DashedLine(ORIGIN, (rect_copy.get_height() + MED_LARGE_BUFF) * UP, color=self.dx_color, stroke_width=2).move_to(rect.get_corner(DOWN + RIGHT), DOWN) for rect in (rect_group.rectangle, new_rect)])
        ...
        area_changes = VGroup()
        point_pairs = [(new_rect.get_corner(UP + RIGHT), rect_copy.get_corner(DOWN + RIGHT)), (new_rect.get_corner(UP + LEFT), rect_copy.get_corner(UP + RIGHT))]
        ...
        question.next_to(self.pi_creature.get_corner(UP + LEFT), UP, buff=MED_SMALL_BUFF)
        self.play(FadeOut(rect_group.area_label), ReplacementTransform(rect_copy, new_rect), MoveToTarget(recip_brace), MoveToTarget(recip_label), self.pi_creature.change_mode, 'pondering')
        ...
        self.wait()
        self.play(FadeIn(area_gained), Write(area_gained_label, run_time=2), ShowCreation(area_gained_arrow))
        self.wait()
        self.play(FadeIn(area_lost), Write(area_lost_label, run_time=2), ShowCreation(area_lost_arrow))
        self.wait()
    ...
    def draw_graph(self):
        ...
        self.add(rect_group)
        self.wait()
        self.change_rectangle_group(rect_group, 2, target_group_kwargs={'x_label': '2', 'one_over_x_label': '\\frac{1}{2}'}, added_anims=[ShowCreation(graph)])
        self.wait()
    ...
    def get_rectangle_group(self, x, x_label='x', one_over_x_label='\\frac{1}{x}'):
        ...
        result.add(result.rectangle, result.x_brace, result.recip_brace, result.labels, result.area_label)
        return result
    ...
    def get_rectangle(self, x):
        try:
            ...
            y = 100
        rectangle = Rectangle(width=x * self.unit_length, height=y * self.unit_length, **self.rectangle_color_kwargs)
        rectangle.move_to(self.graph_origin, DOWN + LEFT)
        return rectangle

    def change_rectangle_group(self, rect_group, target_x, target_group_kwargs=None, added_anims=[], **anim_kwargs):
        ...
        def update_rect_group(group, alpha):
            ...
            for l1, l2 in zip(rect_group.labels, new_group.labels):
                l1.move_to(l2)
            return rect_group
        self.play(UpdateFromAlphaFunc(rect_group, update_rect_group), *added_anims, **anim_kwargs)
...
class SquareRootOfX(Scene):
    CONFIG = {'square_color_kwargs': {'stroke_color': WHITE, 'stroke_width': 1, 'fill_color': BLUE_E, 'fill_opacity': 1}, 'bigger_square_color_kwargs': {'stroke_color': WHITE, 'stroke_width': 1, 'fill_color': YELLOW, 'fill_opacity': 0.7}, 'square_corner': 6 * LEFT + 3 * UP, 'square_width': 3, 'd_sqrt_x': 0.3}
    ...
    def nudge_square(self):
        ...
        bigger_square = Square(side_length=self.square_width + self.d_sqrt_x, **self.bigger_square_color_kwargs)
        bigger_square.move_to(self.square_corner, UP + LEFT)
        square_copy = square.copy()
        lines = VGroup(*[DashedLine(ORIGIN, (self.square_width + MED_LARGE_BUFF) * vect, color=WHITE, stroke_width=3).shift(s.get_corner(corner)) for corner, vect in [(DOWN + LEFT, RIGHT), (UP + RIGHT, DOWN)] for s in [square, bigger_square]])
        little_braces = VGroup(*[Brace(VGroup(*line_pair), vect, buff=0) for line_pair, vect in [(lines[:2], RIGHT), (lines[2:], DOWN)]])
        ...
        self.play(Transform(square_copy, bigger_square), Animation(square), Animation(area_label), bottom_brace.next_to, bigger_square, DOWN, SMALL_BUFF, LEFT, right_brace.next_to, bigger_square, RIGHT, SMALL_BUFF, UP)
...
class IntroduceUnitCircleWithSine(GraphScene):
    ...
    def introduce_unit_circle(self):
        ...
        self.play(ShowCreation(circle), Rotate(radial_line, 2 * np.pi), run_time=2)
        self.wait()
        self.circle = circle
    ...
    def draw_example_radians(self):
        ...
        line = Line(ORIGIN, self.example_radians * self.unit_length * UP, color=YELLOW)
        line.shift(FRAME_X_RADIUS * RIGHT / 3).to_edge(UP)
        line.insert_n_curves(10)
        line.make_smooth()
        arc = Arc(self.example_radians, radius=self.unit_length, color=line.get_color())
        ...
        self.play(GrowFromCenter(line), GrowFromCenter(brace))
    ...
    def walk_around_circle(self):
        ...
        self.play(Rotate(radial_line, filler_portion * 2 * np.pi), ShowCreation(filler_arc), UpdateFromFunc(v_line, v_line_update), run_time=filler_portion / self.rotations_per_second, rate_func=linear)
        for x in range(5):
            self.play(Rotate(radial_line, 2 * np.pi), UpdateFromFunc(v_line, v_line_update), run_time=1.0 / self.rotations_per_second, rate_func=linear)
...
class DerivativeIntuitionFromSineGraph(GraphScene):
    CONFIG = {'graph_origin': 6 * LEFT, 'x_axis_width': 11, 'x_min': 0, 'x_max': 4 * np.pi, 'x_labeled_nums': np.arange(0, 4 * np.pi, np.pi), 'x_tick_frequency': np.pi / 4, 'x_axis_label': '$\\theta$', 'y_axis_height': 6, 'y_min': -2, 'y_max': 2, 'y_tick_frequency': 0.5, 'y_axis_label': ''}
    ...
    def draw_sine_graph(self):
        ...
        self.play(ShowCreation(graph), UpdateFromFunc(v_line, lambda v: self.v_line_update(v, graph)), run_time=2.0 / rps, rate_func=linear)
        self.wait()
        self.graph = graph

    def draw_derivative_from_slopes(self):
        ss_group = self.get_secant_slope_group(0, self.graph, dx=0.01, secant_line_color=RED)
        ...
        while last_theta < self.x_max:
            ...
            self.animate_secant_slope_group_change(ss_group, target_x=next_theta, added_anims=[ShowCreation(deriv_copy, rate_func=lambda t: interpolate(last_theta / self.x_max, next_theta / self.x_max, smooth(t)), run_time=3), UpdateFromFunc(v_line, lambda v: self.v_line_update(v, deriv_copy), run_time=3)])
            self.wait()
    ...
    def alter_derivative_graph(self):
        func_list = [lambda x: 0.5 * (np.cos(x) ** 3 + np.cos(x)), lambda x: 0.75 * (np.sign(np.cos(x)) * np.cos(x) ** 2 + np.cos(x)), lambda x: 2 * np.cos(x), lambda x: np.cos(x)]
...
class DerivativeFromZoomingInOnSine(IntroduceUnitCircleWithSine, ZoomedScene):
    CONFIG = {'zoom_factor': 10, 'zoomed_canvas_frame_shape': (3, 4.5), 'include_radial_line_dot': False, 'remove_angle_label': False, 'theta_label': '', 'line_class': Line, 'example_radians': 1.0, 'zoomed_canvas_corner_buff': SMALL_BUFF, 'd_theta': 0.05}
    ...
    def zoom_in(self):
        ...
        self.play(self.little_rectangle.move_to, self.radial_line.get_end(), DOWN + RIGHT, self.little_rectangle.shift, SMALL_BUFF * (DOWN + RIGHT))
        self.wait()

    def perform_nudge(self):
        d_theta_arc = Arc(start_angle=self.example_radians, angle=self.d_theta, radius=self.unit_length, color=MAROON_B, stroke_width=6)
    ...
    def show_similar_triangles(self):
        ...
        big_triangle = Polygon(self.graph_origin, self.radial_line.get_end(), self.radial_line.get_end()[0] * RIGHT)
        ...
        little_triangle_lines = VGroup(*[Line(*list(map(little_triangle.get_corner, pair))) for pair in [(DOWN + RIGHT, UP + LEFT), (UP + LEFT, DOWN + LEFT)]])
        little_triangle_lines.set_color(little_triangle.get_color())
        self.play(DrawBorderThenFill(little_triangle))
        self.play(little_triangle.scale, 2, little_triangle.get_corner(DOWN + RIGHT), little_triangle.set_color, YELLOW, rate_func=there_and_back)
        self.wait()
        groups = [self.d_theta_group, self.d_sine_group]
        for group, line in zip(groups, little_triangle_lines):
            self.play(ApplyMethod(line.rotate, np.pi / 12, rate_func=wiggle, remover=True))
            self.play(group.scale, 1.2, group.get_corner(DOWN + RIGHT), group.set_color, YELLOW, rate_func=there_and_back)
            self.wait()
        self.play(ReplacementTransform(little_triangle.copy().set_fill(opacity=0), big_triangle_copy, path_arc=np.pi / 2, run_time=2))
        self.wait()
        self.play(ReplacementTransform(big_triangle_copy, big_triangle), Animation(self.angle_label))
        self.wait()
        self.play(self.radial_line.rotate, np.pi / 12, Animation(big_triangle), rate_func=wiggle)
        self.wait()
        self.play(ReplacementTransform(big_triangle.copy().set_fill(opacity=0), little_triangle, path_arc=-np.pi / 2, run_time=3), ReplacementTransform(self.angle_label.copy(), new_angle_label, path_arc=-np.pi / 2, run_time=3))
        self.play(new_angle_label.scale, 2, new_angle_label.set_color, RED, rate_func=there_and_back)
        self.wait()
...
class NextVideo(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(series, lag_ratio=0.5, run_time=3), *[ApplyMethod(pi.look_at, next_video) for pi in self.get_pi_creatures()])
        self.play(next_video.set_color, YELLOW, next_video.shift, MED_LARGE_BUFF * DOWN)
        self.wait()
        for mob in group:
            self.play(Write(mob, run_time=1), *[ApplyMethod(pi.look_at, mob) for pi in self.get_pi_creatures()])
        self.wait(3)

class Chapter3PatreonThanks(PatreonThanks):
    CONFIG = {'specific_patrons': ['Ali Yahya', 'CrypticSwarm    ', 'Yu  Jun', 'Shelby  Doolittle', 'Dave    Nicponski', 'Damion  Kistler', 'Juan    Benet', 'Othman  Alikhan', 'Markus  Persson', 'Dan Buchoff', 'Derek   Dai', 'Joseph  John Cox', 'Luc Ritchie', 'Guido   Gambardella', 'Jerry   Ling', 'Mark    Govea', 'Vecht   ', 'Jonathan    Eppele', 'Shimin Kuang', 'Rish    Kundalia', 'Achille Brighton', 'Kirk    Werklund', 'Ripta   Pasay', 'Felipe  Diniz']}
