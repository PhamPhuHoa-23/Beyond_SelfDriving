...
class CircleScene(PiCreatureScene):
    CONFIG = {'radius': 1.5, 'stroke_color': WHITE, 'fill_color': BLUE_E, 'fill_opacity': 0.5, 'radial_line_color': MAROON_B, 'outer_ring_color': GREEN_E, 'dR': 0.1, 'dR_color': YELLOW, 'unwrapped_tip': ORIGIN, 'include_pi_creature': False, 'circle_corner': UP + LEFT}
    ...
    def setup(self):
        ...
        self.add(self.circle, self.radius_line, self.radius_brace, self.radius_label)
...
class Introduction(TeacherStudentsScene):
    ...
    def show_series(self):
        ...
        self.play(ApplyMethod(this_video.restore, run_time=3), *[ApplyFunction(lambda p: p.change_mode('hooray').look_at(series[1]), pi) for pi in self.get_pi_creatures()])
        ...
        self.play(Homotopy(homotopy, series, apply_function_kwargs={'maintain_smoothness': False}), *[ApplyMethod(pi.look_at, series[-1]) for pi in self.get_pi_creatures()], run_time=5)
        self.play(FadeOut(self.teacher.bubble), FadeOut(self.teacher.bubble.content), *[ApplyMethod(pi.change_mode, 'happy') for pi in self.get_pi_creatures()])
    ...
    def look_to_center(self):
        ...
        self.play(*anims)
        self.random_blink(6)
        self.play(*[ApplyMethod(pi.change_mode, 'happy') for pi in self.get_pi_creatures()])
    ...
    def go_through_students(self):
        ...
        remembered_symbols = VGroup(OldTex('\\int_0^1 \\frac{1}{1-x^2}\\,dx').shift(UP + LEFT), OldTex('\\frac{d}{dx} e^x = e^x').shift(DOWN + RIGHT))
        cant_wait = OldTexText("I literally \\\\ can't wait")
        big_derivative = OldTex('\n            \\frac{d}{dx} \\left( \\sin(x^2)2^{\\sqrt{x}} \\right)\n        ')
        self.play(pi1.change_mode, 'confused', pi1.look_at, bubble.get_right(), ShowCreation(bubble), pi2.fade, pi3.fade)
        bubble.add_content(remembered_symbols)
        self.play(Write(remembered_symbols))
        self.play(ApplyMethod(remembered_symbols.fade, 0.7, lag_ratio=0.5, run_time=3))
        self.play(pi1.restore, pi1.fade, pi2.restore, pi2.change_mode, 'hooray', pi2.look_at, bubble.get_right(), bubble.pin_to, pi2, FadeOut(remembered_symbols))
        bubble.add_content(cant_wait)
        self.play(Write(cant_wait, run_time=2))
        self.play(Blink(pi2))
        self.play(pi2.restore, pi2.fade, pi3.restore, pi3.change_mode, 'pleading', pi3.look_at, bubble.get_right(), bubble.pin_to, pi3, FadeOut(cant_wait))
...
class IntroduceCircle(Scene):
    ...
    def construct(self):
        ...
        area, circumference = words = VGroup(*list(map(TexText, ['Area =', 'Circumference ='])))
        area.set_color(BLUE)
        ...
        derivative = OldTex('\\frac{d}{dR}', '\\pi R^2', '=', '2\\pi R')
        integral = OldTex('\\int_0^R', '2\\pi r', '\\, dR = ', '\\pi R^2')
        ...
        self.play(Rotate(radius, 2 * np.pi, about_point=circle.get_center()), ShowCreation(circle))
        self.play(FadeIn(area), Write(area_form), circle.set_fill, area.get_color(), 0.5, Animation(radius), Animation(R))
        self.wait()
        self.play(circle.set_stroke, circumference.get_color(), FadeIn(circumference), Animation(radius), Animation(R))
        self.play(Transform(area_form.copy(), circum_form, path_arc=-np.pi / 2, run_time=3))
        self.wait()
        self.play(area_form.copy().replace, derivative[1], circum_form.copy().replace, derivative[3], Write(derivative[0]), Write(derivative[2]), run_time=1)
        self.wait()
        self.play(area_form.copy().replace, integral[3], Transform(circum_form.copy(), integral[1]), Write(integral[0]), Write(integral[2]), run_time=1)
        self.wait()
...
class HeartOfCalculus(GraphScene):
    CONFIG = {'x_labeled_nums': [], 'y_labeled_nums': []}

    def construct(self):
        self.setup_axes()
        self.graph_function(lambda x: 3 * np.sin(x / 2) + x)
        rect_sets = [self.get_riemann_rectangles(0, self.x_max, 1.0 / 2 ** n, stroke_width=1.0 / (n + 1)) for n in range(6)]
...
class PragmatismToArt(Scene):
    ...
    def construct(self):
        ...
        circle1 = Circle(radius=2, fill_opacity=1, fill_color=BLUE_E, stroke_width=0)
        ...
        self.play(morty.change_mode, 'raise_right_hand', morty.look_at, pragmatism, Write(pragmatism, run_time=1))
        self.play(Blink(morty))
        self.play(morty.change_mode, 'raise_left_hand', morty.look_at, art, Transform(VectorizedPoint(morty.get_corner(UP + RIGHT)), art), pragmatism.fade, 0.7, pragmatism.rotate, np.pi / 4, pragmatism.shift, DOWN + LEFT)
        self.play(Blink(morty))
...
class IntroduceTinyChangeInArea(CircleScene):
    CONFIG = {'include_pi_creature': True}

    def construct(self):
        new_area_form, minus, area_form = expression = OldTex('\\pi (R + 0.1)^2', '-', '\\pi R^2')
        ...
        group = VGroup(expression, expression_brace, change_in_area, area_brace, area_word, new_area_brace, new_area_word)
        ...
        self.introduce_circle(added_anims=[self.pi_creature.change_mode, 'speaking'])
        self.play(Write(area_group))
        self.change_mode('happy')
        outer_ring = self.increase_radius()
        self.wait()
        self.play(area_group.restore, GrowFromCenter(expression_brace), Write(new_area_form), Write(minus), Write(change_in_area), self.pi_creature.change_mode, 'confused')
        self.play(Write(new_area_word), GrowFromCenter(new_area_brace))
        self.wait(2)
        self.play(group.fade, 0.7, self.pi_creature.change_mode, 'happy')
        self.wait()
        self.play(outer_ring.set_color, YELLOW, Animation(self.nudge_arrow), Animation(self.nudge_line), rate_func=there_and_back)
        ...
        self.wait()
        big_rect = Rectangle(width=FRAME_WIDTH, height=FRAME_HEIGHT, fill_color=BLACK, fill_opacity=0.85, stroke_width=0)
        self.play(FadeIn(big_rect), area_form.set_color, BLUE, self.two_pi_R.set_color, GREEN, self.pi_creature.change_mode, 'happy')

    def show_unwrapping(self, outer_ring):
        almost_rect = outer_ring.copy()
        self.unwrap_ring(almost_rect, added_anims=[self.pi_creature.change_mode, 'pondering'])
        ...
        dR.generate_target()
        lp, rp = OldTex('()')
        change_in_area = OldTexText('Change in area $\\approx$')
        final_area = VGroup(change_in_area, two_pi_R.target, lp, dR.target.scale(1.0 / 0.7), rp)
        ...
        self.wait()
        self.play(MoveToTarget(two_pi_R.copy()), MoveToTarget(dR.copy()), Write(change_in_area, run_time=1), Write(lp), Write(rp))
        self.remove(*self.get_mobjects_from_last_animation())
        self.add(final_area)
        self.play(self.pi_creature.change_mode, 'happy', self.pi_creature.look_at, final_area)
        self.wait()
        group = VGroup(almost_rect, final_area, two_pi_R, dR, circum_brace, dR_brace)
    ...
    def work_out_expression(self, expression_group):
        ...
        expanded = OldTex('\\pi R^2', '+', '2\\pi R (0.1)', '+', '\\pi (0.1)^2', '-', '\\pi R^2')
        ...
        expanded.shift(LEFT / 2.0)
        faders = [area_brace, area_word, new_area_brace, new_area_word]
        self.play(*list(map(FadeOut, faders)))
        trips = [([0, 2, 8], pi_R_squared, plus), ([8, 0, 2, 1, 4, 5, 6, 7], two_pi_R_dR, plus2), ([0, 1, 4, 5, 6, 7, 8], pi_dR_squared, VGroup())]
        to_remove = []
        for subset, target, writer in trips:
            ...
            self.wait()
        self.play(Transform(minus.copy(), minus2), Transform(area_form.copy(), pi_R_squared2))
        to_remove += self.get_mobjects_from_last_animation()
        self.remove(*to_remove)
        self.add(self.pi_creature, *expanded)
        self.wait(2)
        self.play(*[ApplyMethod(mob.set_color, RED) for mob in (pi_R_squared, pi_R_squared2)])
        self.wait()
        self.play(*[ApplyMethod(mob.fade, 0.7) for mob in (plus, pi_R_squared, pi_R_squared2, minus2)])
        self.wait()
        ...
        self.play(GrowFromCenter(approx_brace), self.pi_creature.change_mode, 'hooray')
        self.wait()
        self.play(GrowFromCenter(error_brace), Write(error_words), self.pi_creature.change_mode, 'confused')
        self.wait()
        self.two_pi_R = VGroup(*two_pi_R_dR[:3])

    def second_unwrapping(self, outer_ring):
        almost_rect = outer_ring.copy()
        rect = Rectangle(width=2 * np.pi * self.radius, height=self.dR, fill_color=self.outer_ring_color, fill_opacity=1, stroke_width=0)
        self.play(almost_rect.set_color, YELLOW, self.pi_creature.change_mode, 'pondering')
...
class BuildToDADR(CircleScene):
    ...
    def write_initial_terms(self):
        ...
        equals, two_pi_R, dR, plus, pi, dR2, squared = rhs = OldTex('=', '2 \\pi R', '(0.1)', '+', '\\pi', '(0.1)', '^2')
        ...
        self.play(Write(change, run_time=1), self.pi_creature.change_mode, 'pondering')
        self.wait()
        self.play(*it.chain(list(map(Write, [equals, two_pi_R, dR])), list(map(FadeIn, [circum_text, circum_brace]))))
        self.wait()
        self.play(*it.chain(list(map(Write, [plus, pi, dR2, squared])), list(map(FadeIn, [error_brace, error_text]))))
        self.wait(2)
        self.change = change
    ...
    def show_fractions(self):
        ...
        self.play(*it.chain([Write(term.frac_line) for term in terms], list(map(MoveToTarget, [self.equals, self.plus]))))
        self.play(*[term.denom.restore for term in terms])
        self.wait(2)
        self.play(self.outer_ring.set_color, YELLOW, rate_func=there_and_back)
        self.play(self.nudge_label.scale, 2, rate_func=there_and_back)
        self.wait(2)
        ...
        self.play(self.circum_term[0].move_to, self.circum_term.frac_line, LEFT, self.circum_term[0].shift, 0.1 * UP, FadeOut(self.circum_term.frac_line), MaintainPositionRelativeTo(self.circum_term.label, self.circum_term[0]))
        ...
        self.play(self.error_term.move_to, self.error_term.frac_line, self.error_term.shift, 0.3 * LEFT + 0.15 * UP, FadeOut(self.error_term.frac_line), self.plus.shift, 0.7 * LEFT + 0.1 * UP, MaintainPositionRelativeTo(self.error_term.label, self.error_term))
        self.wait()

    def transition_to_dR(self):
        dRs = VGroup(self.nudge_label, self.change.denom, self.error_term[1])
        error_brace, error_text = self.error_term.label
        for s, width in (('(0.01)', 0.05), ('(0.001)', 0.03), ('dR', 0.03)):
            new_dRs = VGroup(*[OldTex(s).move_to(mob, LEFT) for mob in dRs])
            ...
            error_brace.target.stretch_to_fit_width(VGroup(self.error_term[0], new_dRs[-1]).get_width())
            error_brace.target.move_to(error_brace, LEFT)
            self.play(MoveToTarget(error_brace), Transform(self.outer_ring, new_outer_ring), Transform(self.nudge_line, new_nudge_line), *[Transform(*pair) for pair in zip(dRs, new_dRs)])
            self.wait()
            if s == '(0.001)':
                self.plus.generate_target()
                ...
                self.play(*list(map(MoveToTarget, [error_brace, error_text, self.plus, self.error_term])))
                self.wait()
        difference_text = OldTexText('``Tiny ', 'd', 'ifference in ', '$R$', "''", arg_separator='')
        ...
        self.play(Write(difference_text, run_time=2), ShowCreation(difference_arrow), self.pi_creature.change_mode, 'speaking')
        self.wait()
        dA = OldTex('dA')
        ...
        self.play(Transform(self.change, dA), MoveToTarget(frac_line), MoveToTarget(self.change.denom), Transform(difference_text[3], A), difference_text[1].set_color, dA.get_color(), MoveToTarget(difference_arrow))
        self.wait(2)
        self.play(*list(map(FadeOut, [difference_text, difference_arrow])))

    def elaborate_on_d(self):
        arc = Arc(-np.pi, start_angle=-np.pi / 2)
        arc.set_height(self.change.get_center()[1] - self.change.denom.get_center()[1])
        ...
        d_group.arrange()
        d_group.next_to(self.pi_creature.get_corner(UP + RIGHT), buff=LARGE_BUFF)
        ...
        self.play(GrowFromCenter(equality_brace), Write(equal_word, run_time=1))
        self.wait(2)
        ...
        self.play(Transform(self.outer_ring, big_outer_ring), Transform(self.nudge_line, big_nudge_line), Transform(self.nudge_arrow, new_nudge_arrow))
        self.play(*[mob.restore for mob in [self.outer_ring, self.nudge_line, self.nudge_arrow]], rate_func=linear, run_time=7)
    ...
    def not_infinitely_small(self):
        ...
        self.play(randy.change_mode, 'speaking', ShowCreation(bubble), Write(bubble.content), self.pi_creature.change_mode, 'confused')
        self.wait()
        ...
        self.wait()
        self.play(*it.chain([mob.restore for mob in to_infs], list(map(FadeOut, [bubble, bubble.content])), [randy.change_mode, 'erm'], [self.pi_creature.change_mode, 'happy']))
        for n in range(7):
            ...
            self.play(Transform(self.nudge_label, target), MoveToTarget(self.outer_ring), self.nudge_line.stretch_to_fit_width, 0.1 / (n + 1))
        self.wait()
        bubble.write('Wrong!')
        bubble.resize_to_content()
        bubble.stretch(0.7, 1)
        bubble.pin_to(randy)
        bubble.add_content(bubble.content)
        self.play(FadeIn(bubble), Write(bubble.content, run_time=1), randy.change_mode, 'angry')
        ...
        self.play(FadeOut(bubble), FadeOut(bubble.content), ShowCreation(new_bubble), Write(new_bubble.content), randy.change_mode, 'erm', randy.set_color, BLUE_E, self.pi_creature.change_mode, 'shruggie')
        self.wait(2)
...
class NameDerivative(IntroduceTinyChangeInArea):
    ...
    def name_derivative_for_cricle(self):
        dA_dR, equals, d_formula_dR, equals2, two_pi_R = dArea_fom = OldTex('\\frac{dA}{dR}', '=', '\\frac{d(\\pi R^2)}{dR}', '=', '2\\pi R')
        ...
        words = OldTexText("``Derivative'' of $A$\\\\", 'with respect to $R$')
        ...
        self.play(Transform(self.nudge_line.copy(), dR, run_time=2), Write(frac_line))
        self.wait()
        self.play(ShowCreation(dA_dR_circle), ShowCreation(arrow), Write(words))
        self.wait()
    ...
    def interpret_geometrically(self):
        target_formula = OldTex('\\frac{d \\quad}{dR} = ')
        ...
        self.play(Write(target_formula), MoveToTarget(area_circle), MoveToTarget(circum_circle, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)), self.pi_creature.change_mode, 'hooray')
        self.wait()
    ...
    def show_limiting_process(self):
        ...
        for frac in fracs[:len(sample_dRs)] + [fracs[-1]]:
            ...
            if frac is fracs[-1]:
                ...
                arrow.add(cross)
            arrows.add(arrow)
        self.play(Transform(ring_group, big_group), self.pi_creature.change_mode, 'sassy')
        for n, frac in enumerate(fracs):
            anims = [FadeIn(frac)]
            num_fracs = len(sample_dRs)
            if n < num_fracs:
                anims.append(ShowCreation(arrows[n]))
                anims.append(Transform(ring_group, small_group, rate_func=lambda t: t * (1.0 / (num_fracs - n)), run_time=2))
            elif n > num_fracs:
                anims.append(ShowCreation(arrows[-1]))
            self.play(*anims)
        self.wait(2)
        self.play(FadeOut(arrows), ring_group.restore, self.pi_creature.change_mode, 'happy')
        self.wait()
    ...
    def emphasize_equality(self):
        ...
        self.play(equals.scale, 1.5, equals.set_color, GREEN, rate_func=there_and_back, run_time=2)
        self.play(self.two_pi_R.set_stroke, YELLOW, 3, rate_func=there_and_back, run_time=2)
        self.wait()
        new_words = OldTexText('Systematically\\\\', 'ignore error')
        new_words.move_to(self.words)
        self.play(Transform(self.words, new_words))
        self.wait()

class DerivativeAsTangentLine(ZoomedScene):
    CONFIG = {'zoomed_canvas_frame_shape': (4, 4), 'zoom_factor': 10, 'R_min': 0, 'R_max': 2.5, 'R_to_zoom_in_on': 2, 'little_rect_nudge': 0.075 * (UP + RIGHT)}
    ...
    def setup_axes(self):
        x_axis = NumberLine(x_min=-0.25, x_max=4, unit_size=2, tick_frequency=0.25, leftmost_tick=-0.25, big_tick_numbers=[0, 1, 2, 3, 4], color=GREY)
        ...
        y_axis = NumberLine(x_min=-2, x_max=20, unit_size=0.3, tick_frequency=2.5, leftmost_tick=0, longer_tick_multiple=-2, big_tick_numbers=[0, 5, 10, 15, 20], color=GREY)
    ...
    def show_zoomed_in_steps(self):
        ...
        self.play(self.little_rectangle.move_to, graph_point + self.little_rect_nudge)
        ...
        for line, vect, char in ((dR_line, DOWN, 'R'), (dA_line, RIGHT, 'A')):
            ...
            line.brace.scale(1.0 / self.zoom_factor)
            ...
            line.text.scale(1.0 / self.zoom_factor)
            ...
            self.wait()
        deriv_is_slope = OldTex('\\frac{dA}{dR} =', '\\text{Slope}')
        ...
        self.wait()
        self.dot = dot
        self.small_step_group = VGroup(dR_line, dR_line.brace, dR_line.text, dA_line, dA_line.brace, dA_line.text)
    ...
    def show_tangent_lines(self):
        ...
        self.last_alpha = alphas[np.argmin([get_norm(point - curr_graph_point) for point in graph_points])]

        def shift_everything_to_alpha(alpha, run_time=3):
            self.play(MoveAlongPath(self.dot, self.graph, rate_func=lambda t: interpolate(self.last_alpha, alpha, smooth(t))), UpdateFromFunc(line, line_update_func), UpdateFromFunc(self.small_step_group, self.update_small_step_group), UpdateFromFunc(self.little_rectangle, update_little_rect), run_time=run_time)
    ...
    def state_commonality(self):
        ...
        self.play(ShowCreation(bubble), Write(bubble.content), morty.change_mode, 'surprised')
        self.play(Blink(morty))
        self.wait()
        new_words = OldTexText('Which is...fine...')
        new_words.move_to(bubble.content, RIGHT)
        self.play(bubble.stretch_to_fit_width, 5, bubble.shift, RIGHT, Transform(bubble.content, new_words), morty.change_mode, 'hesitant')
        self.play(Blink(morty))
        self.wait()
...
class IntroduceConcentricRings(CircleScene):
    CONFIG = {'radius': 2.5, 'special_ring_index': 10, 'include_pi_creature': True}
    ...
    def build_up_rings(self):
        self.circle.set_fill(opacity=0)
        rings = VGroup(*[self.get_ring(r, self.dR) for r in np.arange(0, self.radius, self.dR)])
        ...
        self.play(Write(dr_text), ShowCreation(dr_arrow), ShowCreation(dr_line))
        foreground_group.add(dr_line, dr_arrow, dr_text)
        self.change_mode('pondering')
        self.wait()
        self.play(FadeIn(VGroup(*rings[:-1]), lag_ratio=1, run_time=5), Animation(foreground_group))
        self.wait()
    ...
    def add_up_areas(self):
        ...
        area_sum = VGroup(*it.chain(*list(zip([ring for ring in moving_rings.target], plusses))))
        ...
        self.play(MoveToTarget(moving_rings, lag_ratio=0.5), Write(VGroup(plusses, dots_equals_area), rate_func=squish_rate_func(smooth, 0.5, 1)), Animation(self.foreground_group), run_time=5)
        self.wait()
    ...
    def unwrap_special_ring(self):
        ...
        radius = (special_ring.get_width() - 2 * self.dR) / 2.0
        ...
        self.play(Write(think_concrete), ShowCreation(arrow), self.pi_creature.change_mode, 'speaking')
        self.wait()
        ...
        self.to_fade = VGroup(brace, left_brace, two_pi_r, dr, think_concrete, arrow, less_wrong)
    ...
    def write_integral(self):
        ...
        integral_expression = VGroup(def_int, self.two_pi_r.target, self.dr.target, equals_pi_R_squared)
        integral_expression.arrange()
        integral_expression.next_to(brace, DOWN)
        self.integral_expression = VGroup(*integral_expression[:-1])
        self.play(GrowFromCenter(brace), Write(formula_q), self.pi_creature.change_mode, 'pondering')
        self.wait(2)
        last = VMobject()
        last.save_state()
        for ring in self.rings:
            ...
            self.play(last.restore, Transform(ring, target), Animation(self.foreground_group), run_time=0.5)
            last = ring
        ...
        self.play(MoveToTarget(self.rings, rate_func=there_and_back), Animation(self.foreground_group))
        self.wait()
        self.grow_and_shrink_r_line(zero, R)
        self.wait()
        self.play(MoveToTarget(self.two_pi_r), MoveToTarget(self.dr), run_time=2)
        self.wait()
        self.play(FadeOut(self.to_fade), ApplyMethod(self.rings.restore, run_time=2), Animation(self.foreground_group))
        self.wait()
        self.play(Write(equals_pi_R_squared))
        self.wait()
        self.equals = equals_pi_R_squared[0]
        self.integral_terms = VGroup(self.integral_expression[1], self.integral_expression[2], self.int_lower_bound, self.int_upper_bound, VGroup(*equals_pi_R_squared[1:]))
    ...
    def grow_and_shrink_r_line(self, zero_target, R_target):
        ...
        self.radial_line.target.scale(0.1 / self.radial_line.get_length())
        ...
        self.play(MoveToTarget(self.radial_line), MoveToTarget(self.r_label), GrowFromCenter(equals_0))
        self.play(equals_0[-1].copy().replace, zero_target)
        self.remove(self.get_mobjects_from_last_animation()[0])
        self.add(zero_target)
        self.wait()
        self.radial_line.target.scale(self.radius / self.radial_line.get_length())
        equals_0.target = OldTex('=R')
        equals_0.target.next_to(self.radial_line.target.get_center_of_mass(), UP + LEFT, buff=SMALL_BUFF)
        self.r_label.target.next_to(equals_0.target, LEFT, buff=SMALL_BUFF)
        self.play(MoveToTarget(self.radial_line), MoveToTarget(self.r_label), MoveToTarget(equals_0))
        self.play(equals_0[-1].copy().replace, R_target)
        self.remove(self.get_mobjects_from_last_animation()[0])
        self.add(R_target)
        self.wait()
        self.play(self.radial_line.restore, self.r_label.restore, FadeOut(equals_0))
    ...
    def ask_about_approx(self):
        ...
        question = OldTexText('Should this be\\\\', 'an approximation?')
        ...
        self.play(Transform(self.equals, approx), Write(question), ShowCreation(arrow), self.pi_creature.change_mode, 'confused')
        self.wait(2)
        self.play(*[ApplyMethod(ring.set_stroke, ring.get_color(), width=1) for ring in self.rings] + [FadeOut(self.dr_group), Animation(self.foreground_group)])
        self.wait()
        self.play(Transform(question, approach_words), Transform(arrow, Arrow(approach_words, approx)), self.equals.restore, self.pi_creature.change_mode, 'happy')
        self.wait(2)
        self.play(self.integral_expression.set_color_by_gradient, BLUE, GREEN, GrowFromCenter(int_brace), Write(integral_word))
        self.wait()
...
class GraphIntegral(GraphScene):
    CONFIG = {'x_min': -0.25, 'x_max': 4, 'x_tick_frequency': 0.25, 'x_leftmost_tick': -0.25, 'x_labeled_nums': list(range(1, 5)), 'x_axis_label': 'r', 'y_min': -2, 'y_max': 25, 'y_tick_frequency': 2.5, 'y_bottom_tick': 0, 'y_labeled_nums': list(range(5, 30, 5)), 'y_axis_label': '', 'dr': 0.125, 'R': 3.5}
    ...
    def show_horizontal_axis(self):
        arrows = [Arrow(self.little_r, self.coords_to_point(*coords)) for coords in ((0, 0), (self.x_max, 0))]
    ...
    def add_rectangles(self):
        ...
        ticks = VGroup(*[Line(UP, DOWN).move_to(self.coords_to_point(x, 0)) for x in np.arange(0, self.R + self.dr, self.dr)])
        ...
        arrows = VGroup(*[Arrow(values_words.get_bottom(), tick.get_center(), tip_length=0.15) for tick in ticks])
        dr_brace = Brace(VGroup(*ticks[special_tick_index:special_tick_index + 2]), buff=SMALL_BUFF)
        ...
        self.wait()
        self.play(*[ApplyMethod(rect.fade, 0.7) for rect in rectangles if rect is not special_rect] + [Animation(ticks)])
        self.play(GrowFromCenter(left_brace), Write(height_label))
        self.wait()
        graph = self.graph_function(lambda r: 2 * np.pi * r, animate=False)
        graph_label = self.label_graph(self.graph, 'f(r) = 2\\pi r', proportion=0.5, direction=LEFT, animate=False)
        self.play(rectangles.restore, Animation(ticks), FadeOut(left_brace), Transform(height_label, graph_label), ShowCreation(graph))
        self.wait(3)
    ...
    def thinner_rectangles(self):
        for x in range(2, 8):
            new_rects = self.get_rectangles(dr=self.dr / x, stroke_width=1.0 / x)
    ...
    def get_rectangles(self, dr, stroke_width=1):
        return self.get_riemann_rectangles(0, self.R, dr, stroke_width=stroke_width)

class MoreOnThisLater(TeacherStudentsScene):

    def construct(self):
        self.teacher_says('\n            More details on\n            integrals later\n        ')
        self.play_student_changes('raise_right_hand', 'raise_left_hand', 'raise_left_hand')
        self.random_blink(2)
        self.teacher_says('\n            This is just\n            a preview\n        ')
        self.random_blink(2)

class FundamentalTheorem(CircleScene):
    CONFIG = {'circle_corner': ORIGIN, 'radius': 1.5, 'area_color': BLUE, 'circum_color': WHITE, 'unwrapped_tip': 2.5 * UP, 'include_pi_creature': False}

    def setup(self):
        CircleScene.setup(self)
        group = VGroup(self.circle, self.radius_line, self.radius_brace, self.radius_label)
        self.remove(*group)
        group.shift(DOWN)
        self.foreground_group = VGroup(self.radius_line, self.radius_brace, self.radius_label)
    ...
    def add_derivative_terms(self):
        symbolic = OldTex('\\frac{d(\\pi R^2)}{dR} =', '2\\pi R')
        ...
        area_circle = Circle(stroke_width=0, fill_color=self.area_color, fill_opacity=0.5, radius=radius)
        ...
        self.play(Write(self.derivative_terms), self.pi_creature.change_mode, 'hooray')
        self.wait()

    def add_integral_terms(self):
        symbolic = OldTex('\\int_0^R', '2\\pi r', '\\cdot', 'dr', '=', '\\pi R^2')
        ...
        area_circle = Circle(radius=geometric[-1].get_width() / 2, stroke_width=0, fill_color=self.area_color, fill_opacity=0.5)
        area_circle.move_to(geometric[-1])
        circum_circle = Circle(radius=geometric[1].get_width() / 2, color=self.circum_color)
    ...
    def bring_in_circle(self):
        self.play(FadeOut(self.derivative_terms[0]), FadeOut(self.integral_terms[0]), self.derivative_terms[1].to_corner, UP + LEFT, MED_LARGE_BUFF, self.integral_terms[1].to_corner, UP + RIGHT, MED_LARGE_BUFF, self.pi_creature.change_mode, 'speaking')
    ...
    def show_outer_ring(self):
        ...
        self.play(GrowFromCenter(brace), Write(deriv), self.pi_creature.change_mode, 'happy')
        self.to_fade = VGroup(deriv, brace)
        self.to_restore = ring_copy

    def show_all_rings(self):
        rings = VGroup(*[self.get_ring(radius=r, dR=self.dR) for r in np.arange(0, self.radius, self.dR)])
        rings.set_color_by_gradient(BLUE_E, GREEN_E)
        rings.save_state()
        integrand = self.integral_terms[1][1]
        for ring in rings:
            Transform(ring, integrand).update(1)
        self.play(ApplyMethod(rings.restore, lag_ratio=0.5, run_time=5), Animation(self.foreground_group))

    def emphasize_oposites(self):
        self.play(FadeOut(self.to_fade), self.to_restore.restore, Animation(self.foreground_group), run_time=2)
        arrow = DoubleArrow(self.derivative_terms[1], self.integral_terms[1])
...
class NameTheFundamentalTheorem(TeacherStudentsScene):
    ...
    def construct(self):
        symbols = OldTex('\\frac{d}{dx} \\int_0^x f(t)dt = f(x)')
        symbols.to_corner(UP + LEFT)
        brace = Brace(symbols)
        abstract = brace.get_text('Abstract version')
        self.add(symbols)
        self.play(GrowFromCenter(brace), Write(abstract), *[ApplyMethod(pi.look_at, symbols) for pi in self.get_pi_creatures()])
...
class CalculusInANutshell(CircleScene):
    CONFIG = {'circle_corner': ORIGIN, 'radius': 3}
    ...
    def morph_word(self):
        ...
        rings = VGroup(*[self.get_ring(rad, 0.95 * dR) for rad in np.arange(0, self.radius, dR)])
        for ring in rings:
    ...
    def show_remainder_of_series(self):
        ...
        self.play(FadeIn(VGroup(*series[1:]), lag_ratio=0.5, run_time=2))
        self.wait()
...
class Thumbnail(CircleScene):
    ...
    def construct(self):
        ...
        title.scale(2)
        title.to_edge(UP)
        area_circle = Circle(fill_color=BLUE, fill_opacity=0.5, stroke_width=0)
