...
class Chapter4OpeningQuote(OpeningQuote):
    CONFIG = {'quote': ['Using the chain rule is like peeling an onion: ', 'you have to deal with each layer at a time, and ', 'if it is too big you will start crying.'], 'author': '(Anonymous professor)'}

class TransitionFromLastVideo(TeacherStudentsScene):

    def construct(self):
        simple_rules = VGroup(*list(map(Tex, ['\\frac{d(x^3)}{dx} = 3x^2', '\\frac{d(\\sin(x))}{dx} = \\cos(x)', '\\frac{d(1/x)}{dx} = -\\frac{1}{x^2}'])))
        combination_rules = VGroup(*[OldTex('\\frac{d}{dx}\\left(%s\\right)' % tex) for tex in ['\\sin(x) + x^2', '\\sin(x)(x^2)', '\\sin(x^2)']])
        ...
        for rule in simple_rules:
            self.play(Write(rule, run_time=2), self.get_teacher().change_mode, 'raise_right_hand', *[ApplyMethod(pi.change_mode, 'pondering') for pi in self.get_students()])
            self.wait()
        self.wait(2)
        self.play(simple_rules.replace, last_video)
        self.play(last_video.restore, Animation(simple_rules), brace.next_to, this_video, DOWN, this_video.set_color, YELLOW)
        self.play(Write(combination_rules), *[ApplyMethod(pi.change_mode, 'confused') for pi in self.get_students()])
        self.wait(2)
        for rule in combination_rules:
            ...
            if rule is combination_rules[-1]:
                ...
                added_anims = [inner_func.shift, 0.5 * DOWN, inner_func.set_color, YELLOW]
        ...
        division = VGroup(VGroup(*pre_division[:6]), VGroup(*pre_division[6:7]), VGroup(*pre_division[7:]))
        pre_decomposed_division = OldTex('\\sin(x)\\cdot\\frac{1}{x^2}')
        decomposed_division = VGroup(VGroup(*pre_decomposed_division[:6]), VGroup(*pre_decomposed_division[6:9]), VGroup(*pre_decomposed_division[9:]))
        for mob in (subtraction, decomposed_subtraction, division, decomposed_division):
            mob.next_to(VGroup(self.get_teacher(), self.get_students()[-1]), UP, buff=MED_LARGE_BUFF)
        top_group = VGroup(series, simple_rules, brace)
        combination_rules.save_state()
        self.play(top_group.next_to, FRAME_Y_RADIUS * UP, UP, combination_rules.to_edge, UP)
        pairs = [(subtraction, decomposed_subtraction), (division, decomposed_division)]
        for question, answer in pairs:
            self.play(Write(question), combination_rules.fade, 0.2, self.get_students()[2].change_mode, 'raise_right_hand', self.get_teacher().change_mode, 'plain')
            self.wait()
            answer[1].set_color(GREEN)
            self.play(Transform(question, answer), self.get_teacher().change_mode, 'hooray', self.get_students()[2].change_mode, 'plain')
            self.wait()
            self.play(FadeOut(question))
        monster = OldTex('\\Big(', 'e^{\\sin(x)} \\cdot', '\\cos\\Big(', '\\frac{1}{x^3}', ' + x^3', '\\Big)', '\\Big)^4')
        monster.next_to(self.get_pi_creatures(), UP)
        parts = [VGroup(*monster[3][2:]), VGroup(*monster[3][:2]), monster[4], VGroup(monster[2], monster[5]), monster[1], VGroup(monster[0], monster[6])]
        modes = 3 * ['erm'] + 3 * ['pleading']
        for part, mode in zip(parts, modes):
            self.play(FadeIn(part, lag_ratio=0.5), self.get_teacher().change_mode, 'raise_right_hand', *[ApplyMethod(pi.change_mode, mode) for pi in self.get_students()])
        self.wait()
        self.play_student_changes(*['happy'] * 3)
        words = list(map(TexText, ['composition', 'product', 'composition', 'sum', 'composition']))
        for word, part in zip(words, reversed(parts)):
            word.set_color(YELLOW)
            word.next_to(monster, UP)
            self.play(FadeIn(word), part.scale, 1.2, part.set_color, YELLOW)
            self.wait()
            self.play(*list(map(FadeOut, [word, part])))
        self.play(FadeOut(parts[0]))
        self.play(combination_rules.restore, *[ApplyMethod(pi_creature.change_mode, 'pondering') for pi_creature in self.get_pi_creatures()])
        self.wait(2)

class DampenedSpring(Scene):

    def construct(self):
        compact_spring, extended_spring = [ParametricCurve(lambda t: t / denom * RIGHT + np.sin(t) * UP + np.cos(t) * OUT, t_max=12 * np.pi, color=GREY).shift(3 * LEFT) for denom in (12.0, 2.0)]
        ...
        def update_spring(spring, a):
            spring.interpolate(compact_spring, extended_spring, 0.5 * (np.exp(-4 * a) * np.cos(40 * a) + 1))
        equation = OldTex('\\text{Length} = 2 + e^{-4t}\\cos(20t)')
        equation.to_edge(UP)
        self.add(moving_spring, equation)
        self.play(UpdateFromAlphaFunc(moving_spring, update_spring, run_time=10, rate_func=linear))
        self.wait()
...
class PreSumRuleDiscussion(Scene):
    ...
    def construct(self):
        ...
        title.to_edge(UP)
        self.add(title)
        specific = OldTex('\\frac{d}{dx}(', '\\sin(x)', '+', 'x^2', ')', '=', '\\cos(x)', '+', '2x')
        general = OldTex('\\frac{d}{dx}(', 'g(x)', '+', 'h(x)', ')', '=', '\\frac{dg}{dx}', '+', '\\frac{dh}{dx}')
        ...
        for indices in [(1, 2, 3), (6,), (7, 8)]:
            self.play(*[ReplacementTransform(specific[i].copy(), general[i]) for i in indices])
            self.wait()
        for i in (1, 3, -1, 6, 8):
            if i < 0:
                self.wait()
                ...
                part = specific[i]
                self.play(part.set_color, YELLOW, part.scale, 1.2, rate_func=there_and_back)
        self.wait()

class SumRule(GraphScene):
    CONFIG = {'x_labeled_nums': [], 'y_labeled_nums': [], 'y_axis_label': '', 'x_max': 4, 'x_axis_width': FRAME_WIDTH, 'y_max': 3, 'graph_origin': 2.5 * DOWN + 2.5 * LEFT, 'graph_label_x_value': 1.5, 'example_input': 0.5, 'example_input_string': '0.5', 'dx': 0.05, 'v_lines_x_min': -1, 'v_lines_x_max': 2, 'graph_scale_factor': 2, 'tex_scale_factor': 0.8}
    ...
    def add_graphs(self):
        ...
        sine_label = self.get_graph_label(sine_graph, '\\sin(x)', x_val=self.graph_label_x_value, direction=UP + RIGHT, buff=0)
        sine_label.scale(self.tex_scale_factor)
        parabola_label = self.get_graph_label(parabola, 'x^2', x_val=self.graph_label_x_value)
        ...
        sine_v_lines, parabox_v_lines = v_line_sets = [self.get_vertical_lines_to_graph(graph, x_min=self.v_lines_x_min, x_max=self.v_lines_x_max, num_lines=num_lines, stroke_width=2) for graph in graphs]
        sine_v_lines.shift(0.02 * RIGHT)
        for v_lines in v_line_sets:
            self.play(ShowCreation(v_lines), Animation(labels))
        self.wait()
        self.play(*it.chain([ApplyMethod(l2.move_to, l1.get_top(), DOWN) for l1, l2 in zip(*v_line_sets)], [graph.fade for graph in graphs], [Animation(labels)]))
        self.wait()
    ...
    def zoom_in_on_graph(self):
        graph_parts = VGroup(self.axes, self.sine_graph, self.parabola, self.sum_graph, *self.v_line_sets)
        ...
        for mob in (graph_parts, self.graph_labels):
            mob.target.scale(self.graph_scale_factor, about_point=self.graph_origin)
        for mob in self.graph_labels.target:
            mob.scale(1.0 / self.graph_scale_factor, about_point=mob.get_bottom())
    ...
    def show_example_stacking(self):
        ...
        example_v_lines, nudged_v_lines = [VGroup(*[v_lines[index] for v_lines in v_line_sets]) for index in (num_lines / 2, num_lines / 2 + 1)]
        for line in nudged_v_lines:
            line.save_state()
        sine_lines, parabola_lines = [VGroup(example_v_lines[i], nudged_v_lines[i]) for i in (0, 1)]
        ...
        for line, tex, vect in zip(sine_lines, ['', '+dx'], [LEFT, RIGHT]):
            ...
            label = OldTex('x=' + str(self.example_input) + tex)
            label.next_to(dot, DOWN + vect, buff=MED_LARGE_BUFF)
            arrow = Arrow(label.get_corner(UP - vect), dot, buff=SMALL_BUFF, color=WHITE, tip_length=0.1)
            label_groups.append(VGroup(label, arrow, dot))
        line_tex_direction_triplets = [(sine_lines[0], '\\sin(0.5)', LEFT), (sine_lines[1], '\\sin(0.5+dx)', RIGHT), (parabola_lines[0], '(0.5)^2', LEFT), (parabola_lines[1], '(0.5+dx)^2', RIGHT)]
        for line, tex, direction in line_tex_direction_triplets:
            line.brace = Brace(line, direction, buff=SMALL_BUFF, min_num_quads=2)
            line.brace.set_color(line.get_color())
            line.brace.add_background_rectangle()
            line.brace_text = line.brace.get_text('$%s$' % tex)
            line.brace_text.scale(self.tex_scale_factor, about_point=line.brace_text.get_edge_center(-direction))
            line.brace_text.add_background_rectangle()
            line.brace_anim = MaintainPositionRelativeTo(VGroup(line.brace, line.brace_text), line)
        self.play(example_v_lines.set_stroke, None, 4, faders.fade, Animation(self.graph_labels), Write(label_groups[0]))
        for line in example_v_lines:
            line.save_state()
        self.wait()
        self.play(GrowFromCenter(sine_lines[0].brace), Write(sine_lines[0].brace_text))
        self.wait()
        self.play(sine_lines[0].shift, UP + 4 * LEFT, sine_lines[0].brace_anim, parabola_lines[0].move_to, sine_lines[0], DOWN)
        self.wait()
        parabola_lines[0].brace_anim.update(1)
        self.play(GrowFromCenter(parabola_lines[0].brace), Write(parabola_lines[0].brace_text))
        self.wait()
        self.play(*it.chain(*[[line.restore, line.brace_anim] for line in example_v_lines]))
        self.play(Write(label_groups[1]), *it.chain(*[[line.restore, line.set_stroke, None, 4] for line in nudged_v_lines]))
        self.wait()
        for line in nudged_v_lines:
    ...
    def show_df(self):
        ...
        df, equals, d_sine, plus, d_x_squared = deriv_mob = OldTex('df', '=', 'd(\\sin(x))', '+', 'd(x^2)')
        ...
        deriv_mob.next_to(self.func_mob, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        ...
        self.play(parabola_lines[0].shift, (parabola_lines[1].get_bottom()[1] - parabola_lines[0].get_bottom()[1]) * UP, parabola_lines[0].brace_anim)
    ...
    def show_difference(self, v_lines, target_tex, added_tex):
        ...
        h_lines = VGroup(*[DashedLine(ORIGIN, 2 * RIGHT, stroke_width=3) for x in range(2)])
        ...
        self.wait()
        self.play(ReplacementTransform(brace_text.copy(), target_tex), Write(added_tex))
        return VGroup(h_lines, brace, brace_text)

    def expand_derivative(self):
        expanded_deriv = OldTex('df', '=', '\\cos(x)', '\\,dx', '+', '2x', '\\,dx')
        ...
        expanded_deriv.next_to(self.deriv_mob, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        background_rect = BackgroundRectangle(expanded_deriv)
        rearranged_deriv = OldTex('{df \\over', 'dx}', '=', '\\cos(x)', '+', '2x')
        ...
        self.play(FadeIn(background_rect, rate_func=squish_rate_func(smooth, 0.6, 1)), Write(expanded_deriv))
        self.wait()
        tex_group_pairs = [('\\cos(0.5)dx', self.d_sine_lines), ('2(0.5)dx', self.d_x_squared_lines)]

        def indicate(mob):
            self.play(mob.set_color, YELLOW, mob.scale, 1.2, rate_func=there_and_back)
        for tex, group in tex_group_pairs:
            ...
            self.wait()
        self.wait()
        self.play(*[Transform(expanded_deriv[i], rearranged_deriv[j], path_arc=-np.pi / 2) for i, j in enumerate(deriv_target_indices)])
        self.wait()

class DiscussProducts(TeacherStudentsScene):

    def construct(self):
        wrong_product_rule = OldTex('\\frac{d(\\sin(x)x^2)}{dx}', '\\ne', '\\left(\\frac{d(\\sin(x))}{dx}\\right)', '\\left(\\frac{d(x^2)}{dx}\\right)')
        not_equals = wrong_product_rule[1]
        wrong_product_rule[2].set_color(SINE_COLOR)
        wrong_product_rule[3].set_color(X_SQUARED_COLOR)
        wrong_product_rule.next_to(self.get_teacher().get_corner(UP + LEFT), UP, buff=MED_LARGE_BUFF).shift_onto_screen()
        self.teacher_says('Products are a bit different', target_mode='sassy')
        self.wait(2)
        self.play(RemovePiCreatureBubble(self.get_teacher(), target_mode='raise_right_hand'))
        self.play(Write(wrong_product_rule))
        self.play_student_changes('pondering', 'confused', 'erm', added_anims=[not_equals.scale, 1.3, not_equals.set_color, RED])
        self.wait()
        self.teacher_says('Think about the \\\\ underlying meaning', bubble_config={'height': 3}, added_anims=[wrong_product_rule.scale, 0.7, wrong_product_rule.to_corner, UP + LEFT])
        self.play_student_changes(*['pondering'] * 3)
        self.wait(2)

class NotGraphsForProducts(GraphScene):
    CONFIG = {'y_max': 25, 'x_max': 7}

    def construct(self):
        self.setup_axes()
        sine_graph = self.get_graph(np.sin, color=SINE_COLOR)
        sine_graph.label = self.get_graph_label(sine_graph, '\\sin(x)', x_val=3 * np.pi / 2, direction=DOWN)
        parabola = self.get_graph(lambda x: x ** 2, color=X_SQUARED_COLOR)
        parabola.label = self.get_graph_label(parabola, 'x^2', x_val=2.5, direction=UP + LEFT)
        product_graph = self.get_graph(lambda x: np.sin(x) * x ** 2, color=PRODUCT_COLOR)
        product_graph.label = self.get_graph_label(product_graph, '\\sin(x)x^2', x_val=2.8, direction=UP + RIGHT, buff=0)
...
class IntroduceProductAsArea(ReconfigurableScene):
    CONFIG = {'top_func': np.sin, 'top_func_label': '\\sin(x)', 'top_func_nudge_label': 'd(\\sin(x))', 'top_func_derivative': '\\cos(x)', 'side_func': lambda x: x ** 2, 'side_func_label': 'x^2', 'side_func_nudge_label': 'd(x^2)', 'side_func_derivative': '2x', 'x_unit_to_space_unit': 3, 'box_kwargs': {'fill_color': YELLOW, 'fill_opacity': 0.75, 'stroke_width': 1}, 'df_box_kwargs': {'fill_color': GREEN, 'fill_opacity': 0.75, 'stroke_width': 0}, 'box_corner_location': 6 * LEFT + 2.5 * UP, 'slider_center': 3.5 * RIGHT + 2 * DOWN, 'slider_width': 6, 'slider_x_max': 3, 'x_slider_handle_height': 0.25, 'slider_handle_color': BLUE, 'default_x': 0.75, 'dx': 0.1, 'tiny_dx': 0.01}
    ...
    def talk_though_sine(self):
        ...
        self.play(Rotate(y_axis, np.pi / 2, about_point=y_axis.get_left()), Animation(x_axis))
        self.play(ShowCreation(graph), Write(label, run_time=1))
        self.play(ShowCreation(v_line))
        for x, rt in zip([0.25, np.pi / 2, 3, self.default_x], [2, 4, 4, 2]):
            self.animate_x_change(x, run_time=rt, added_anims=[UpdateFromFunc(v_line, v_line_update)])
            self.wait()
        self.play(*it.chain(list(map(FadeOut, [y_axis, graph, label, v_line])), [Animation(x_axis)]))
        self.wait()
    ...
    def define_f_of_x(self):
        f_def = OldTex('f(x)', '=', self.top_func_label, self.side_func_label, '=', '\\text{Area}')
        ...
        self.play(Transform(box.copy().set_fill(opacity=0), f_def[-1], run_time=1.5))
        self.wait()
        self.f_def = f_def
    ...
    def nudge_x(self):
        ...
        self.animate_x_change(self.default_x + self.dx, rate_func=there_and_back, run_time=2, added_anims=[Animation(nudge_label_group)])
        self.wait()
        self.play(ReplacementTransform(thin_df_boxes, df_boxes), VGroup(*labels[1]).shift, right_box.get_width() * RIGHT)
        self.play(df_boxes.space_out_submobjects, 1.1, df_boxes.move_to, box, UP + LEFT)
        self.wait()
    ...
    def get_nudge_label_group(self):
        ...
        dx_line = Line(*[line.number_to_point(self.x_slider.x_val + num) for num in (0, self.dx)])
    ...
    def get_df_boxes(self):
        ...
        df_boxes = VGroup(*[Rectangle(height=height, width=width, **self.df_box_kwargs).next_to(box, vect, buff=0) for (height, width), vect in zip(heights_and_widths, vects)])
        return df_boxes

    def get_df_box_labels(self, df_boxes):
        bottom_box, corner_box, right_box = df_boxes
        result = VGroup()
        quads = [(right_box, UP, self.top_func_nudge_label, LEFT), (corner_box, RIGHT, self.side_func_nudge_label, ORIGIN)]
        for box, vect, label_tex, aligned_edge in quads:
            brace = Brace(box, vect)
            label = OldTex(label_tex)
            label.next_to(brace, vect, aligned_edge=aligned_edge, buff=SMALL_BUFF)
            label.set_color(df_boxes[0].get_color())
            result.add(VGroup(brace, label))
        return result

    def write_df(self):
        deriv = OldTex('df', '=', self.top_func_label, self.side_func_nudge_label, '+', self.side_func_label, self.top_func_nudge_label)
        deriv.scale(0.9)
        ...
        df_boxes_copy = VGroup(bottom_box.copy(), plus, right_box.copy(), plus.copy(), corner_box.copy())
        self.deriv = deriv
        self.df_boxes_copy = df_boxes_copy
        box, labels = self.box_label_group
        self.full_box_parts = VGroup(*it.chain([box], self.df_boxes, labels, self.df_box_labels))
        self.play(Write(VGroup(*deriv[:2])))
        self.play(df_boxes_copy.arrange, df_boxes_copy.set_fill, None, self.df_box_kwargs['fill_opacity'], df_boxes_copy.next_to, deriv[1])
    ...
    def set_color_boxes_and_label(self, boxes, label):
        ...
        self.play(boxes.set_color, RED, label.set_color, RED)
        self.play(label[1].scale, 1.1, rate_func=there_and_back)
        self.play(boxes.restore, label.restore)
        self.wait()

    def set_color_right_boxes(self):
        self.set_color_boxes_and_label(VGroup(*self.df_boxes[1:]), self.df_box_labels[0])

    def set_color_bottom_boxes(self):
        self.set_color_boxes_and_label(VGroup(*self.df_boxes[:-1]), self.df_box_labels[1])
    ...
    def describe_bottom_box(self, bottom_box_area):
        ...
        self.play(ReplacementTransform(bottom_box_copy, bottom_box_area), other_box_copies.next_to, bottom_box_area, RIGHT)
        self.wait()
    ...
    def describe_right_box(self, right_box_area):
        ...
        self.play(ReplacementTransform(right_box_copy, right_box_area), other_box_copies.next_to, right_box_area, DOWN, MED_SMALL_BUFF, RIGHT)
        self.wait()
        self.play(faders.restore)
    ...
    def ignore_corner(self):
        ...
        self.play(corner.set_color, RED, corner_copy.set_color, RED)
        self.wait()
    ...
    def expand_derivative(self):
        expanded_deriv = OldTex('df', '=', self.top_func_label, self.side_func_derivative, '\\,dx', '+', self.side_func_label, self.top_func_derivative, '\\,dx')
        final_deriv = OldTex('{df \\over ', 'dx}', '=', self.top_func_label, self.side_func_derivative, '+', self.side_func_label, self.top_func_derivative)
        ...
        def indicate(mob):
            self.play(mob.scale, 1.2, mob.set_color, YELLOW, rate_func=there_and_back)
        for index in (6, 3):
        ...
        self.play(ReplacementTransform(top_part, bottom_part))
        self.wait()
        self.play(*[ReplacementTransform(expanded_deriv[i], final_deriv[j], path_arc=-np.pi / 2) for i, j in [(0, 0), (1, 2), (2, 3), (3, 4), (4, 1), (5, 5), (6, 6), (7, 7), (8, 1)]])
        self.wait()
    ...
    def write_derivative_abstractly(self):
        self.transition_to_alt_config(return_to_original_configuration=False, top_func_label='g(x)', top_func_nudge_label='dg', top_func_derivative='\\frac{dg}{dx}', side_func_label='h(x)', side_func_nudge_label='dh', side_func_derivative='\\frac{dh}{dx}')
        self.wait()
    ...
    def write_mneumonic(self):
        ...
        morty.to_edge(DOWN)
        morty.shift(2 * LEFT)
        words = OldTexText('``Left ', 'd(Right) ', '+', ' Right ', 'd(Left)', "''", arg_separator='')
    ...
    def animate_x_change(self, target_x, box_label_group=None, x_slider=None, **kwargs):
        ...
        self.play(UpdateFromAlphaFunc(box_label_group, update_box_label_group), UpdateFromAlphaFunc(x_slider, update_x_slider), *added_anims, **kwargs)
        x_slider.x_val = target_x

    def get_x_slider(self, x):
        numbers = list(range(int(self.slider_x_max) + 1))
        line = NumberLine(x_min=0, x_max=self.slider_x_max, unit_size=float(self.slider_width) / self.slider_x_max, color=GREY, big_tick_numbers=numbers, tick_frequency=0.25)
        ...
        triangle = RegularPolygon(3, start_angle=np.pi / 2, fill_color=self.slider_handle_color, fill_opacity=0.8, stroke_width=0)
    ...
    def get_box(self, x):
        box = Rectangle(width=self.x_unit_to_space_unit * self.top_func(x), height=self.x_unit_to_space_unit * self.side_func(x), **self.box_kwargs)
...
class MneumonicExample(TeacherStudentsScene):
    ...
    def construct(self):
        d, left, right, rp = deriv_q = OldTex('\\frac{d}{dx}(', '\\sin(x)', 'x^2', ')')
        deriv_q.to_edge(UP)
        words = OldTexText('Left ', 'd(Right) ', '+', ' Right ', 'd(Left)', arg_separator='')
        ...
        self.add(words)
        self.play(Write(deriv_q), self.get_teacher().change_mode, 'raise_right_hand')
        self.play_student_changes(*['pondering'] * 3)
...
class ConstantMultiplication(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.student_says(answer, target_mode='hooray', added_anims=[question.copy().to_edge, UP])
        self.play(self.get_teacher().change_mode, 'happy')
        self.play_student_changes('pondering', 'hooray', 'pondering')
        self.wait(3)

class ConstantMultiplicationFigure(IntroduceProductAsArea):
    CONFIG = {'side_func': lambda x: 1, 'side_func_label': '\\text{Constant}', 'side_func_nudge_label': '', 'side_func_derivative': '', 'x_unit_to_space_unit': 3, 'default_x': 0.5, 'dx': 0.1}
...
class ShoveXSquaredInSine(Scene):
    ...
    def construct(self):
        ...
        group.shift(UP)
        composition = OldTex('g(', 'h(x)', ')', '=', '\\sin(', 'x^2', ')')
        for i in (0, 2, 4, 6):
        ...
        self.play(Write(title))
        self.wait()
        triplets = [[sine, (0, 2), (0, 2)], [x_squared, (0,), (1,)], [sine, (3, 4, 6), (3, 4, 6)], [x_squared, (2,), (5,)]]
        for premob, pre_indices, comp_indicies in triplets:
            self.play(*[ReplacementTransform(premob[i].copy(), composition[j]) for i, j in zip(pre_indices, comp_indicies)])
            self.wait()
        self.wait()
...
class ThreeLinesChainRule(ReconfigurableScene):
    CONFIG = {'start_x': 0.5, 'max_x': 1, 'min_x': 0, 'top_x': 3, 'example_x': 1.5, 'dx': 0.1, 'line_configs': [{'func': lambda x: x, 'func_label': 'x', 'triangle_color': WHITE, 'center_y': 3, 'x_min': 0, 'x_max': 3, 'numbers_to_show': list(range(4)), 'big_tick_numbers': list(range(4)), 'tick_frequency': 0.25}, {'func': lambda x: x ** 2, 'func_label': 'x^2', 'triangle_color': X_SQUARED_COLOR, 'center_y': 0.5, 'x_min': 0, 'x_max': 10, 'numbers_to_show': list(range(0, 11)), 'big_tick_numbers': list(range(0, 11, 1)), 'tick_frequency': 0.25}, {'func': lambda x: np.sin(x ** 2), 'func_label': '\\sin(x^2)', 'triangle_color': SINE_COLOR, 'center_y': -2, 'x_min': -2, 'x_max': 2, 'numbers_to_show': list(range(-2, 3)), 'big_tick_numbers': list(range(-2, 3)), 'tick_frequency': 0.25}], 'line_width': 8, 'triangle_height': 0.25}
    ...
    def draw_function_arrows(self):
        ...
        labels = VGroup(*[OldTex('(\\dots)^2').set_color(X_SQUARED_COLOR), OldTex('\\sin(\\dots)').set_color(SINE_COLOR)])
        arrows = VGroup()
    ...
    def talk_through_movement(self):
        ...
        sine_arrow = Arrow(sine_text.get_top(), labels[-1][0].get_bottom(), buff=SMALL_BUFF)
    ...
    def nudge_x(self):
        ...
        dx_squared_value = OldTex('= 2x\\,dx ', '\\\\ = 2(%s)dx' % x_value)
        dx_squared_value.shift(dx_squared_brace.text.get_right() + MED_SMALL_BUFF * RIGHT - dx_squared_value[0].get_left())
        dsine_value = OldTexText('$=\\cos(%s)$' % self.line_configs[1]['func_label'], dx_squared_brace.text.get_tex())
        dsine_value.next_to(dsine_brace.text)
        less_than_zero = OldTex('<0')
        less_than_zero.next_to(dsine_brace.text)
        all_x_squared_relevant_labels = VGroup(dx_squared_brace, dsine_brace, labels[1], labels[2], dsine_value)
        all_x_squared_relevant_labels.save_state()
        self.play(FadeOut(numbers))
        self.animate_x_change(self.example_x + self.dx, run_time=1, added_anims=it.chain([GrowFromCenter(dx_brace)], list(map(ShowCreation, nudge_lines))))
        ...
        self.wait()
        self.play(all_x_squared_relevant_labels.restore, lag_ratio=0.5, run_time=3)
        ...
        self.play(two_x_dx_copy.move_to, dx_squared, LEFT, dx_squared.next_to, dx_squared, UP, run_time=2)
    ...
    def replace_x_squared_with_h(self):
        ...
        self.transition_to_alt_config(return_to_original_configuration=False, **new_config)
    ...
    def animate_x_change(self, target_x, **kwargs):
        ...
        self.play(UpdateFromAlphaFunc(self.line_group, update), *added_anims, **kwargs)
    ...
    def get_line_label(self, number_line, x, func, func_label, triangle_color, **spillover_kwargs):
        triangle = RegularPolygon(n=3, start_angle=-np.pi / 2, fill_color=triangle_color, fill_opacity=0.75, stroke_width=0)
...
class GeneralizeChainRule(Scene):
    ...
    def construct(self):
        example = OldTex('\\frac{d}{dx}', '\\sin(', 'x^2', ')', '=', '\\cos(', 'x^2', ')', '\\,2x')
        general = OldTex('\\frac{d}{dx}', 'g(', 'h(x)', ')', '=', '{dg \\over ', ' dh}', '(', 'h(x)', ')', '{dh \\over', ' dx}', '(x)')
        ...
        self.add(example)
        braces = VGroup(example_outer_brace, example_inner_brace, d_example_outer_brace)
        for brace in braces:
            self.play(GrowFromCenter(brace))
            self.play(Write(brace.text, run_time=1))
            self.wait()
        self.wait()
        self.play(*it.chain(*[[mob.scale, 1.2, mob.set_color, YELLOW] for mob in (example_inner, d_example_inner)]), rate_func=there_and_back)
        self.play(Transform(example_inner.copy(), d_example_inner, path_arc=-np.pi / 2, remover=True))
        self.wait()
        self.play(GrowFromCenter(d_example_d_inner_brace), Write(d_example_d_inner_brace.text))
        self.play(Transform(VGroup(*reversed(example_inner.copy())), d_example_d_inner, path_arc=-np.pi / 2, run_time=2, remover=True))
        self.wait()
        self.play(*list(map(FadeIn, general[:5])))
        self.wait()
        self.play(Transform(example_outer_brace, general_outer_brace), Transform(example_outer_brace.text, general_outer_brace.text), Transform(example_inner_brace, general_inner_brace), Transform(example_inner_brace.text, general_inner_brace.text))
        self.wait()
        self.play(Transform(d_example_outer_brace, d_general_outer_brace), Transform(d_example_outer_brace.text, d_general_outer_brace.text))
        self.play(Write(d_general_outer))
        self.wait(2)
        self.play(Transform(d_example_d_inner_brace, d_general_d_inner_brace), Transform(d_example_d_inner_brace.text, d_general_d_inner_brace.text))
        ...
        self.play(morty.set_fill, None, 1, morty.change_mode, 'raise_left_hand', morty.look, UP + LEFT, d_general_outer_copy.next_to, morty.get_corner(UP + LEFT), UP, MED_LARGE_BUFF, d_general_outer_copy.shift_onto_screen)
        self.wait()
        ...
        self.play(morty.change_mode, 'hooray', morty.look_at, inner, inner.shift, UP)
        ...
        braces = [d_example_d_inner_brace, d_example_outer_brace, example_inner_brace, example_outer_brace]
        ...
        points = VGroup(*list(map(VectorizedPoint, [m.get_left() for m in to_collapse])))
        self.play(Transform(to_collapse, points), dh_dx.next_to, dg_dh, morty.look_at, dg_dh)
        self.wait()
        ...
        self.play(Blink(morty))
        self.wait(2)
        self.play(PiCreatureSays(morty, '\n                This is more than a\n                notational trick\n            '), VGroup(dg_dh, dh_dx, equals_dg_dx, strikes, *general[:5]).shift, DOWN, FadeOut(example))
        self.wait()
        self.play(Blink(morty))
        self.wait()
...
class WatchingVideo(PiCreatureScene):
    ...
    def construct(self):
        ...
        formulas = VGroup(*[OldTex('\\frac{d}{dx}\\left( %s \\right)' % s) for s in ['e^x \\sin(x)', '\\sin(x) \\cdot  \\frac{1}{\\cos(x)}', '\\cos(3x)^2', 'e^x(x^2 + 3x + 2)']])
        ...
        for formula in formulas:
            self.play(Write(formula, run_time=2), randy.change_mode, 'thinking')
            self.wait()
...
class NextVideo(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        series.to_edge(UP)
        next_video = series[4]
        pre_expression = OldTex('x', '^2', '+', 'y', '^2', '=', '1')
        d_expression = OldTex('2', 'x', '\\,dx', '+', '2', 'y', '\\,dy', '=', '0')
        ...
        for mob in (expression, d_expression):
            mob.scale(1.2)
            mob.next_to(self.get_teacher().get_corner(UP + LEFT), UP, buff=MED_LARGE_BUFF)
            mob.shift_onto_screen()
        ...
        self.add(series)
        self.play(next_video.shift, 0.5 * DOWN, next_video.set_color, YELLOW, self.get_teacher().change_mode, 'raise_right_hand')
        self.wait()
        self.play(Write(expression), *[ApplyMethod(pi.change_mode, 'pondering') for pi in self.get_students()])
        self.play(FadeIn(axes))
        self.wait()
        self.remove(expression)
        self.play(Transform(expression, d_expression, path_arc=np.pi / 2))
        self.wait()
        self.play(Rotate(line, np.pi / 4, about_point=axes.get_center(), rate_func=wiggle, run_time=3))
        self.wait(2)

class Chapter4Thanks(PatreonThanks):
    CONFIG = {'specific_patrons': ['Ali  Yahya', 'Meshal  Alshammari', 'CrypticSwarm    ', 'Ankit   Agarwal', 'Yu  Jun', 'Shelby  Doolittle', 'Dave    Nicponski', 'Damion  Kistler', 'Juan    Benet', 'Othman  Alikhan', 'Justin Helps', 'Markus  Persson', 'Dan Buchoff', 'Derek   Dai', 'Joseph  John Cox', 'Luc Ritchie', 'Nils Schneider', 'Mathew Bramson', 'Guido   Gambardella', 'Jerry   Ling', 'Mark    Govea', 'Vecht', 'Shimin Kuang', 'Rish    Kundalia', 'Achille Brighton', 'Kirk    Werklund', 'Ripta   Pasay', 'Felipe  Diniz'], 'patron_group_size': 8}
