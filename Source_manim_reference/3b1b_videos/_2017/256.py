...
class BreakUp2To256(PiCreatureScene):
    ...
    def add_number(self):
        ...
        self.play(self.pi_creature.change, 'raise_right_hand', GrowFromCenter(brace), Write(expression, run_time=1))
        self.wait()
    ...
    def break_up_as_powers_of_two(self):
        ...
        self.play(FadeOut(self.bits_brace), ReplacementTransform(VGroup(self.expression), subexpressions), MoveToTarget(bits))
    ...
    def break_up_as_four_billions(self):
        ...
        self.play(Transform(self.subexpressions, new_subexpressions, run_time=2, lag_ratio=0.5), FadeOut(self.pi_creature))
        self.wait(3)
    ...
    def reorganize_four_billions(self):
        ...
        self.play(self.bits.to_corner, DOWN + LEFT, Transform(four_billions, target_four_billions), LaggedStartMap(FadeIn, parens), FadeOut(to_fade))
        self.wait()
...
class MainBreakdown(Scene):
    CONFIG = {'n_group_rows': 8, 'n_group_cols': 8}
    ...
    def show_gpu(self):
        gpu = SVGMobject(file_name='gpu', height=1, fill_color=GREY_B)
        ...
        for hash_name in hash_names:
            ...
            path.set_points([start_point, start_point + RIGHT, hash_name.get_left() + LEFT, hash_name.get_left(), hash_name.get_left(), hash_name.get_right(), hash_name.get_right(), hash_name.get_right() + RIGHT, end_point + LEFT, end_point])
            paths.add(path)
        paths.set_stroke(width=3)
        paths.set_color_by_gradient(BLUE, GREEN)

        def get_passing_flash():
            return ShowPassingFlash(paths, lag_ratio=0, time_width=0.7, run_time=2)
    ...
    def cram_computer_with_gpus(self):
        ...
        self.play(gpus.arrange, RIGHT, SMALL_BUFF, gpus.next_to, rate_words, UP, gpus.to_edge, LEFT)
        self.play(Transform(four_billion.copy(), new_rate_words[0], remover=True), Transform(rate_words, new_rate_words))
        self.wait()
        self.play(LaggedStartMap(ApplyFunction, gpus, lambda g: (lambda m: m.scale(0.01).move_to(laptop), g), remover=True))
        self.wait()
        self.play(Transform(rate_words[0], four_billion.copy().set_color(BLUE), remover=True), four_billion.set_color, BLUE, Transform(rate_words[1], hps))
        self.play(Transform(laptop, target_laptop), ShowCreation(h_line))
        self.wait()
    ...
    def kilo_google(self):
        ...
        self.wait(2)
        self.play(LaggedStartMap(Indicate, self.group_of_four_billion_things, run_time=4, rate_func=there_and_back, lag_ratio=0.25))
        ...
        self.play(Transform(kilogoogle, target_kilogoogle), FadeIn(laptop), FadeIn(h_line))
        self.wait()
    ...
    def half_all_people_on_earth(self):
        ...
        group.shift(RIGHT)
        kg, h_line, target_earth = self.get_fraction(2, self.get_kilogoogle(), self.get_earth())
        self.play(GrowFromCenter(earth), Write(people))
        self.wait()
        self.create_four_billion_copies(2, self.get_kilogoogle())
        self.wait()
        self.play(self.four_billions[2].restore, Transform(earth, target_earth), FadeIn(h_line), FadeIn(kg), FadeOut(self.group_of_four_billion_things), FadeOut(people))
        self.wait()
    ...
    def four_billion_earths(self):
        ...
        self.wait()
        self.play(LaggedStartMap(Indicate, self.group_of_four_billion_things, rate_func=there_and_back, lag_ratio=0.2, run_time=3))
        self.wait()
        self.play(ReplacementTransform(self.group_of_four_billion_things, VGroup(earth)), ShowCreation(h_line), FadeIn(denom), self.four_billions[3].restore, FadeOut(milky_way), FadeOut(n_stars_estimate))
        self.wait()
    ...
    def four_billion_galxies(self):
        ...
        self.wait()
        self.play(self.four_billions[4].restore, ReplacementTransform(self.group_of_four_billion_things, VGroup(num), run_time=2, lag_ratio=0.5), ShowCreation(h_line), ReplacementTransform(name, denom))
        self.wait()
    ...
    def show_time_scale(self):
        ...
        years_to_eons = OldTexText('$\\times$ 126.8 years', '$\\approx$ 507 Billion years')
        years_to_eons.next_to(seconds_to_years, DOWN, aligned_edge=LEFT)
    ...
    def create_four_billion_copies(self, index, mobject):
        ...
        group = VGroup(*[VGroup(*[mobject.copy().set_height(0.25) for x in range(self.n_group_rows)]).arrange(DOWN, buff=SMALL_BUFF) for y in range(self.n_group_cols - 1)])
        dots = OldTex('\\dots')
        group.add(dots)
        ...
        self.play(four_billion.scale, 2, four_billion.next_to, brace, LEFT, GrowFromCenter(brace), LaggedStartMap(FadeIn, group, run_time=3, lag_ratio=0.2))
        self.wait()
    ...
    def get_galaxy(self):
        return SVGMobject(file_name='galaxy', fill_opacity=0, stroke_width=3, stroke_color=WHITE, height=1)
...
class StateOfBitcoin(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        asic.to_edge(UP)
        asic.shift(LEFT)
        circuit = SVGMobject(file_name='circuit', height=asic.get_height(), fill_color=WHITE)
        ...
        self.play(self.teacher.change, 'raise_right_hand', rate.restore)
        self.play_student_changes(*['pondering'] * 3)
        self.play(LaggedStartMap(FadeIn, kilogoogle))
        self.play_student_changes(*['surprised'] * 3)
        self.wait()
        self.play_student_changes(*['plain'] * 3, added_anims=[all_text.to_edge, LEFT, self.teacher.change_mode, 'happy'], look_at=gpu)
        self.play(Write(gpu_name), DrawBorderThenFill(gpu))
        self.play(ShowCreation(cross))
        self.wait()
        self.play(Write(asic), gpu_group.to_edge, DOWN, self.teacher.change, 'raise_right_hand')
        self.play_student_changes(*['pondering'] * 3, added_anims=[Write(asic_rate)])
        self.play(LaggedStartMap(FadeIn, circuit, run_time=3, lag_ratio=0.2))
        self.wait()
...
class QAndA(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(q_and_a.restore, self.pi_creature.change, 'raise_right_hand')
        self.wait()
    ...
    def show_powers_of_two(self):
        rows = 16
        cols = 64
        dots = VGroup(*[VGroup(*[Dot() for x in range(rows)]).arrange(DOWN, buff=SMALL_BUFF) for y in range(cols)]).arrange(RIGHT, buff=SMALL_BUFF)
        ...
        for i, power_of_two in enumerate(powers_of_two):
            ...
            self.play(FadeIn(VGroup(*dots[2 ** (i - 1):2 ** i])), Transform(curr_power, power_of_two, rate_func=squish_rate_func(smooth, 0, 0.5)))
        self.wait()
...
class Thumbnail(Scene):
    ...
    def construct(self):
        ...
        lock = SVGMobject(file_name='shield_locked', fill_color=WHITE)
        lock.set_height(6)
