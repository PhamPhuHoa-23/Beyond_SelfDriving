...
class ShowSimpleMultivariableFunction(Scene):
    ...
    def construct(self):
        scale_val = 1.5
        func_tex = OldTex('C(', 'x_1,', 'x_2,', '\\dots,', 'x_n', ')', '=')
        ...
        for tex in (func_tex, alt_func_tex):
            tex.set_color_by_tex_to_color_map({'C(': RED, ')': RED})
        ...
        self.wait(3)
        self.play(ReplacementTransform(inputs, many_inputs[:len(inputs)]), LaggedStartMap(FadeIn, many_inputs[len(inputs):]), FadeOut(inputs_brace), FadeOut(inputs_brace_text))
        self.wait()
        self.play(ReplacementTransform(func_tex[0], alt_func_tex[0]), Write(alt_func_tex[1:3]), LaggedStartMap(FadeOutAndShiftDown, many_inputs))
        self.wait(3)
...
class ShowFunction(Scene):
    ...
    def construct(self):
        func = OldTex('f(x, y) = e^{-x^2 + \\cos(2y)}', tex_to_color_map={'x': BLUE, 'y': RED})
        func.scale(1.5)
...
class ShowGradient(Scene):
    ...
    def construct(self):
        ...
        vector = Matrix([['\\partial f / \\partial x'], ['\\partial f / \\partial y']], v_buff=1)
        ...
        self.play(LaggedStartMap(FadeIn, gradient), LaggedStartMap(FadeIn, background_rects, rate_func=squish_rate_func(smooth, 0.5, 1)))
        self.wait()
...
class TakePartialDerivatives(Scene):
    ...
    def construct(self):
        ...
        func_tex = OldTex('f', '(', 'x', ',', 'y', ')', '=', 'e^{', '-x^2', '+ \\cos(2y)}', tex_to_color_map=tex_to_color_map)
        partial_x = OldTex('{\\partial', 'f', '\\over', '\\partial', 'x}', '=', '\\left(', 'e^', '{-x^2', '+ \\cos(2y)}', '\\right)', '(', '-2', 'x', ')', tex_to_color_map=tex_to_color_map)
        partial_y = OldTex('{\\partial', 'f', '\\over', '\\partial', 'y}', '=', '\\left(', 'e^', '{-x^2', '+ \\cos(', '2', 'y)}', '\\right)', '(', '-\\sin(', '2', 'y)', '\\cdot 2', ')', tex_to_color_map=tex_to_color_map)
        ...
        self.wait()
        self.play(ReplacementTransform(func_tex[0].copy(), partial_x[1]), Write(partial_x[0]), Write(partial_x[2:4]), Write(partial_x[6]))
        self.play(ReplacementTransform(func_tex[2].copy(), partial_x[4]))
        self.wait()
        self.play(LaggedStartMap(ShowCreation, ys.rects))
        self.play(LaggedStartMap(GrowArrow, ys.arrows, lag_ratio=0.8), Write(treat_as_constant))
        self.wait(2)
        self.play(FadeIn(exp_rect), Animation(func_tex))
        self.wait()
        pxi1 = 8
        pxi2 = 15
        self.play(ReplacementTransform(func_tex[7:].copy(), partial_x[pxi1:pxi2]), FadeIn(partial_x[pxi1 - 1:pxi1]), FadeIn(partial_x[pxi2]))
        self.wait(2)
        self.play(ReplacementTransform(partial_x[10:12].copy(), partial_x[pxi2 + 2:pxi2 + 4], path_arc=-(TAU / 4)), FadeIn(partial_x[pxi2 + 1]), FadeIn(partial_x[-1]))
        self.wait(2)
        self.play(FadeOutAndShiftDown(partial_x), FadeOut(ys.rects), FadeOut(ys.arrows), FadeOut(treat_as_constant), FadeOut(exp_rect), Animation(func_tex))
        self.play(FadeInFromDown(partial_y[:7]))
        self.wait()
        treat_as_constant.next_to(xs.arrows[1], UP, SMALL_BUFF)
        self.play(LaggedStartMap(ShowCreation, xs.rects), LaggedStartMap(GrowArrow, xs.arrows), Write(treat_as_constant), lag_ratio=0.8)
        self.wait()
        self.play(ReplacementTransform(func_tex[7:].copy(), partial_x[pxi1:pxi2]), FadeIn(partial_x[pxi1 - 2:pxi1]), FadeIn(partial_x[pxi2]))
        self.wait()
        self.play(ReplacementTransform(partial_y[12:16].copy(), partial_y[pxi2 + 3:pxi2 + 7], path_arc=-(TAU / 4)), FadeIn(partial_y[pxi2 + 2]), FadeIn(partial_y[-1]))
        self.wait()
...
class ShowDerivativeAtExamplePoint(Scene):
    ...
    def construct(self):
        tex_to_color_map = {'x': BLUE, 'y': RED}
        func_tex = OldTex('f', '(', 'x', ',', 'y', ')', '=', 'e^{', '-x^2', '+ \\cos(2y)}', tex_to_color_map=tex_to_color_map)
        gradient_tex = OldTex('\\nabla', 'f', '(', 'x', ',', 'y', ')', '=', tex_to_color_map=tex_to_color_map)
        partial_vect = Matrix([['{\\partial f / \\partial x}'], ['{\\partial f / \\partial y}']])
        ...
        example_lhs = OldTex('\\nabla', 'f', '(', '1', ',', '3', ')', '=', tex_to_color_map={'1': BLUE, '3': RED})
        example_result_vector = self.get_result_vector('1', '3')
        example_rhs = DecimalMatrix([[-1.92], [0.54]])
        example = VGroup(example_lhs, example_result_vector, OldTex('='), example_rhs)
        example.arrange(RIGHT, buff=SMALL_BUFF)
        example.next_to(gradient, DOWN, LARGE_BUFF)
        self.add(func_tex, gradient)
        self.wait()
        self.play(ReplacementTransform(gradient_tex.copy(), example_lhs), ReplacementTransform(result_vector.copy(), example_result_vector))
        self.wait()
        self.play(Write(example[2:]))
        self.wait()

    def get_result_vector(self, x, y):
        result_vector = Matrix([['e^{-%s^2 + \\cos(2\\cdot %s)} (-2\\cdot %s)' % (x, y, x)], ['e^{-%s^2 + \\cos(2\\cdot %s)} \\big(-\\sin(2\\cdot %s) \\cdot 2\\big)' % (x, y, y)]], v_buff=1.2, element_alignment_corner=ORIGIN)
        x_terms = VGroup(result_vector.get_mob_matrix()[0, 0][2], result_vector.get_mob_matrix()[1, 0][2], result_vector.get_mob_matrix()[0, 0][-2])
        y_terms = VGroup(result_vector.get_mob_matrix()[0, 0][11], result_vector.get_mob_matrix()[1, 0][11], result_vector.get_mob_matrix()[1, 0][-5])
