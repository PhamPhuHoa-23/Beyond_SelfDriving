...
class EthereumLogo(SVGMobject):
    CONFIG = {'file_name': 'ethereum_logo', 'stroke_width': 0, 'fill_opacity': 1, 'color_chars': '8B8B48', 'height': 0.5}
...
class LitecoinLogo(SVGMobject):
    CONFIG = {'file_name': 'litecoin_logo', 'stroke_width': 0, 'fill_opacity': 1, 'fill_color': GREY_B, 'height': 0.5}

class TenDollarBill(VGroup):
    CONFIG = {'color': GREEN, 'height': 0.5, 'mark_paths_closed': False}

    def __init__(self, **kwargs):
        VGroup.__init__(self, **kwargs)
        rect = Rectangle(height=2.61, width=6.14, color=self.color, mark_paths_closed=False, fill_color=BLACK, fill_opacity=1)
...
class AskQuestion(Scene):
    ...
    def construct(self):
        ...
        for word, part in zip(strings, question):
            ...
            self.play(LaggedStartMap(FadeIn, part), run_time=self.time_per_char * len(word), rate_func=squish_rate_func(smooth, 0, 0.5))
...
class ListOfAttributes(Scene):
    ...
    def construct(self):
        ...
        government, bank = buildings = [SVGMobject(file_name='%s_building' % word, height=2, fill_color=GREY_B, fill_opacity=1, stroke_width=0) for word in ('government', 'bank')]
...
class DisectQuestion(TeacherStudentsScene):
    ...
    def list_topics(self):
        topics = OldTexText('Digital signatures, ', 'Proof of work, ', 'Cryptographic hash functions, \\dots')
        ...
        for word in topics:
            ...
            self.play_student_changes(*['confused'] * 3, added_anims=anims, look_at=word)
...
class ListRecentCurrencies(Scene):
    ...
    def construct(self):
        ...
        self.add(footnote)
        logos = VGroup(BitcoinLogo(), EthereumLogo(), ImageMobject('ripple_logo'), LitecoinLogo(), EthereumLogo().set_color_by_gradient(GREEN_B, GREEN_D))
        for logo in logos:
            logo.set_height(0.75)
        logos.arrange(DOWN, buff=MED_LARGE_BUFF)
        logos.shift(LEFT)
        logos.to_edge(UP)
        names = list(map(TexText, ['Bitcoin', 'Ethereum', 'Ripple', 'Litecoin', 'Ethereum Classic']))
...
class Hype(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        phrases = list(map(TexText, ['I want some!', "I'll get rich, right?", 'Buy them all!']))
        ...
        for student, phrase, mode in zip(self.students, phrases, modes):
            ...
            bubble.add(phrase)
            self.play(student.change_mode, mode, FadeIn(bubble))
        self.wait(3)

class NoCommentOnSpeculation(TeacherStudentsScene):

    def construct(self):
        axes = VGroup(Line(0.25 * LEFT, 4 * RIGHT), Line(0.25 * DOWN, 3 * UP))
        times = np.arange(0, 4.25, 0.25)
        prices = [0.1, 0.5, 1.75, 1.5, 2.75, 2.2, 1.3, 0.8, 1.1, 1.3, 1.2, 1.4, 1.5, 1.7, 1.2, 1.3]
        graph = VMobject()
        graph.set_points_as_corners([time * RIGHT + price * UP for time, price in zip(times, prices)])
        ...
        axes.shift(DOWN)
        axes.fade(1)
        self.play(self.teacher.change, 'sassy', axes.restore)
        ...
        self.wait(2)
        self.play(FadeIn(mining_graphic), RemovePiCreatureBubble(self.students[0]), self.teacher.change, 'sassy')
...
class LedgerScene(PiCreatureScene):
    CONFIG = {'ledger_width': 6, 'ledger_height': 7, 'denomination': 'USD', 'ledger_line_height': 0.4, 'sign_transactions': False, 'enumerate_lines': False, 'line_number_color': YELLOW}
    ...
    def add_payment_line_to_ledger(self, from_name, to_name, amount):
        ...
        line_tex_parts = [from_name.capitalize(), 'pays' if from_name.lower() != 'you' else 'pay', to_name.capitalize(), amount_str]
        ...
        amount_color = {'USD': GREEN, 'BTC': YELLOW, 'LD': YELLOW}.get(self.denomination, WHITE)
    ...
    def get_network(self):
        ...
        lines = VGroup(*[Line(VGroup(pi1, pi1.label), VGroup(pi2, pi2.label), buff=MED_SMALL_BUFF, stroke_width=2) for pi1, pi2 in it.combinations(creatures, 2)])
    ...
    def create_pi_creatures(self):
        creatures = VGroup(*[PiCreature(color=color, height=1).shift(2 * vect) for color, vect in zip([BLUE_C, MAROON_D, GREY_BROWN, BLUE_E], [UP + LEFT, UP + RIGHT, DOWN + LEFT, DOWN + RIGHT])])
...
class LayOutPlan(LedgerScene):
    ...
    def show_ledger(self):
        ...
        payments = [('Alice', 'Bob', 20), ('Bob', 'Charlie', 40), ('Alice', 'You', 50)]
    ...
    def become_skeptical(self):
        ...
        for k in range(3):
            self.play(*[ApplyMethod(creatures[i].look_at, creatures[k * (i + 1) % 4]) for i in range(4)])
        self.wait(2)
...
class UnderlyingSystemVsUserFacing(Scene):
    ...
    def construct(self):
        ...
        phone = SVGMobject(file_name='phone', fill_color=WHITE, fill_opacity=1, height=1, stroke_width=0)
        phone.next_to(protocol, UP, LARGE_BUFF)
        card = SVGMobject(file_name='credit_card', fill_color=GREY_B, fill_opacity=1, stroke_width=0, height=1)
...
class IntroduceLedgerSystem(LedgerScene):
    CONFIG = {'payments': [('Alice', 'Bob', 20), ('Bob', 'Charlie', 40), ('Charlie', 'You', 30), ('You', 'Alice', 10)]}
    ...
    def add_ledger(self):
        ledger = self.get_ledger()
        self.play(Write(ledger), *[ApplyMethod(pi.change, 'pondering', ledger) for pi in self.pi_creatures])
    ...
    def tally_it_all_up(self):
        ...
        self.play(LaggedStartMap(FadeIn, results), *[ApplyMethod(pi.change, 'happy') for pi in creditors] + [ApplyMethod(pi.change, 'plain') for pi in debtors])
        self.wait()
        debtor_cash, creditor_cash = [VGroup(*it.chain(*[pi.cash for pi in group])) for group in (debtors, creditors)]
        self.play(FadeIn(debtor_cash))
        self.play(debtor_cash.arrange, RIGHT, SMALL_BUFF, debtor_cash.move_to, self.pi_creatures)
        self.wait()
...
class InitialProtocol(Scene):
    ...
    def show_first_two_items(self):
        items = VGroup(*list(map(self.get_new_item, ['Anyone can add lines to the Ledger', 'Settle up with real money each month'])))
        for item in items:
            self.wait()
...
class AddFraudulentLine(LedgerScene):
    ...
    def anyone_can_add_a_line(self):
        ...
        arrow = Arrow(words.get_left(), self.ledger.content.get_center() + DOWN)
        self.play(Write(words, run_time=1))
        self.play(ShowCreation(arrow))
        self.wait()
        self.play(FadeOut(words), FadeOut(arrow), FocusOn(self.bob))
    ...
    def alice_reacts(self):
        bubble = SpeechBubble(height=1.5, width=2, direction=LEFT)
        bubble.next_to(self.alice, UP + RIGHT, buff=0)
        bubble.write('Hey!')
        self.play(Animation(self.bob.pupils), self.alice.change, 'angry', FadeIn(bubble), Write(bubble.content, run_time=1))
        self.wait(3)
        self.play(FadeOut(bubble), FadeOut(bubble.content), self.alice.change_mode, 'pondering')
...
class IntroduceSignatures(LedgerScene):
    CONFIG = {'payments': [('Alice', 'Bob', 100), ('Charlie', 'You', 20), ('Bob', 'You', 30)]}
    ...
    def add_transactions(self):
        transactions = VGroup(*[self.add_payment_line_to_ledger(*payment) for payment in self.payments])
        self.play(LaggedStartMap(FadeIn, transactions))
        self.wait()

    def add_signatures(self):
        signatures = VGroup(*[get_cursive_name(payments[0].capitalize()) for payments in self.payments])
...
class AskHowDigitalSignaturesArePossible(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.student_says("Couldn't you just \\\\ copy the signature?", target_mode='confused', run_time=1)
...
class DescribeDigitalSignatures(LedgerScene):
    CONFIG = {'public_color': GREEN, 'private_color': RED, 'signature_color': BLUE_C}
    ...
    def reorganize_pi_creatures(self):
        ...
        creature_groups = VGroup(*[VGroup(pi, pi.label).scale(1.7) for pi in self.pi_creatures])
    ...
    def generate_key_pairs(self):
        ...
        public_key_strings = [bin(256 + ord(name[0].capitalize()))[3:] for name in names]
        private_key_strings = [bin(hash(name))[2:10] for name in names]
        public_keys, private_keys = [VGroup(*[OldTexText(key_name + ':', ' $%s\\dots$' % key) for key in keys]) for key_name, keys in [('pk', public_key_strings), ('sk', private_key_strings)]]
        key_pairs = [VGroup(*pair).arrange(DOWN, aligned_edge=LEFT) for pair in zip(public_keys, private_keys)]
    ...
    def keep_secret_key_secret(self):
        ...
        lock = SVGMobject(file_name='lock', height=rects.get_height(), fill_color=GREY_B, fill_opacity=1, stroke_width=0)
    ...
    def show_digital_signatures(self):
        rect = SurroundingRectangle(VGroup(self.public_keys[0], self.private_key_rects[0], self.locks[0]))
        ...
        arrows = VGroup(*[Arrow(rect.get_corner(UP), sig.get_bottom(), tip_length=0.15, color=WHITE) for sig in digital_signatures])
        words = VGroup(*list(map(TexText, ['Different messages', 'Completely different signatures'])))
        ...
        self.play(*[ApplyMethod(pi.change, 'pondering', digital_signatures) for pi in self.pi_creatures])
        for word in words:
    ...
    def show_signing_functions(self):
        sign = OldTexText('Sign(', 'Message', ', ', 'sk', ') = ', 'Signature', arg_separator='')
        sign.to_corner(UP + RIGHT)
        verify = OldTexText('Verify(', 'Message', ', ', 'Signature', ', ', 'pk', ') = ', 'T/F', arg_separator='')
        for mob in (sign, verify):
            mob.set_color_by_tex('sk', self.private_color)
            mob.set_color_by_tex('pk', self.public_color)
            mob.set_color_by_tex('Signature', self.signature_color)
        ...
        self.play(*[FadeIn(part) for part in sign if part not in [sign.message, sign.sk, sign.signature]])
        self.play(ReplacementTransform(message.copy(), VGroup(sign.message)))
        self.wait()
        self.play(ReplacementTransform(private_key.copy(), sign.sk))
        self.wait()
        self.play(ReplacementTransform(VGroup(sign.sk, sign.message).copy(), VGroup(sign.signature)))
        self.wait()
        self.play(Indicate(sign.sk))
        self.wait()
        self.play(Indicate(sign.message))
        self.wait()
        self.play(*[FadeIn(part) for part in verify if part not in [verify.message, verify.signature, verify.pk, verify[-1]]])
        self.wait()
        self.play(ReplacementTransform(sign.message.copy(), verify.message), ReplacementTransform(sign.signature.copy(), verify.signature))
        self.wait()
...
class TryGuessingDigitalSignature(Scene):
    ...
    def construct(self):
        verify = OldTexText('Verify(', 'Message', ', ', '256 bit Signature', ', ', 'pk', ')', arg_separator='')
        verify.scale(1.5)
        verify.shift(DOWN)
...
class FeelConfidentWithVerification(PiCreatureScene):
    ...
    def show_verification(self):
        ...
        lock = SVGMobject(file_name='lock', fill_color=GREY_B, height=0.5)
        lock.next_to(lock_box, LEFT, SMALL_BUFF)
        self.add(verify)
        self.play(GrowFromCenter(brace), Write(signature), self.pi_creature.change, 'pondering')
...
class IncludeTransactionNumber(LedgerScene):
    ...
    def fail_to_sign_new_transaction(self):
        ...
        self.play(self.bob.change, 'raise_right_hand', payment.restore)
    ...
    def copy_payment_many_times(self):
        ...
        self.play(LaggedStartMap(FadeIn, copies, run_time=3), self.bob.change, 'conniving')
    ...
    def add_ids(self):
        ...
        self.play(ReplacementTransform(lines.copy(), new_signatures, lag_ratio=0.5, run_time=2))
...
class SignedLedgerScene(LedgerScene):
    CONFIG = {'sign_transactions': True, 'enumerate_lines': True, 'ledger_width': 7.5}

class CharlieRacksUpDebt(SignedLedgerScene):
    CONFIG = {'payments': [('Charlie', 'Alice', 100), ('Charlie', 'Bob', 200), ('Charlie', 'You', 800), ('Charlie', 'Bob', 600), ('Charlie', 'Alice', 900)]}

    def construct(self):
        self.add_ledger_and_network()
        lines = VGroup(*[self.add_payment_line_to_ledger(*payment) for payment in self.payments])
        self.play(LaggedStartMap(FadeIn, lines, run_time=3, lag_ratio=0.25))
        self.play(*[ApplyMethod(pi.change, 'sassy', self.charlie) for pi in self.pi_creatures if pi is not self.charlie])
        self.play(self.charlie.shift, FRAME_X_RADIUS * RIGHT, rate_func=running_start)
        self.play(*[ApplyMethod(pi.change, 'angry', self.charlie) for pi in self.get_pi_creatures()])
        self.wait()
...
class ThinkAboutSettlingUp(Scene):
    ...
    def construct(self):
        ...
        self.play(PiCreatureBubbleIntroduction(randy, "You don't \\emph{actually} \\\\" + 'need to settle up $\\dots$', bubble_type=ThoughtBubble, target_mode='thinking'))
...
class DontAllowOverdrawing(InitialProtocol):
    ...
    def construct(self):
        self.add_title()
        lines = list(map(self.get_new_item, ['Anyone can add lines to the Ledger \\,', 'Only signed transactions are valid \\,', 'No overspending']))
...
class LedgerWithInitialBuyIn(SignedLedgerScene):
    ...
    def everyone_buys_in(self):
        ...
        self.play(LaggedStartMap(ApplyMethod, moneys, lambda m: (m.restore,)), self.charlie.change, 'raise_right_hand', self.you.change, 'raise_right_hand')
        self.network.add(moneys)

    def add_initial_lines(self):
        lines = VGroup()
        for name in self.get_names():
            new_line = OldTexText(name.capitalize(), 'get' if name == 'you' else 'gets', '\\$100')
        ...
        lines[-1].add(line)
        self.play(LaggedStartMap(FadeIn, lines), *[ApplyMethod(pi.change, 'thinking', self.ledger) for pi in self.pi_creatures])
        self.wait()

    def add_charlie_payments(self):
        payments = [('Charlie', 'Alice', 50), ('Charlie', 'Bob', 50), ('Charlie', 'You', 20)]
        new_lines = VGroup(*[self.add_payment_line_to_ledger(*payment) for payment in payments])
        for line in new_lines:
            self.play(Write(line, run_time=1))
            self.wait()

    def point_out_charlie_is_broke(self):
        charlie_lines = VGroup(*[VGroup(*self.ledger.content[i][1:5]) for i in (3, 5, 6, 7)])
    ...
    def running_balance(self):
        ...
        self.play(LaggedStartMap(FadeIn, lines, run_time=3), LaggedStartMap(ShowCreation, arrows, run_time=3))
        self.wait()
...
class RemovedConnectionBetweenLedgerAndCash(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play_student_changes(*['pondering'] * 3, look_at=ledger, added_anims=[self.teacher.change, 'happy'])
        self.wait(3)

class RenameToLedgerDollars(LedgerScene):
    CONFIG = {'payments': [('Alice', 'Bob', 20), ('Charlie', 'You', 80), ('Bob', 'Charlie', 60), ('Bob', 'Alice', 30), ('Alice', 'You', 100)], 'enumerate_lines': True, 'line_number_color': WHITE}
    ...
    def rewrite_as_ledger_dollars(self):
        ...
        self.play(Write(ledger_dollars), FadeOut(self.bubble), self.pi_creature.change, 'thinking', ledger_dollars)
...
class ExchangeCashForLedgerDollars(LedgerScene):
    CONFIG = {'sign_transactions': True, 'denomination': 'LD', 'ledger_width': 7.5, 'ledger_height': 6}
    ...
    def give_ten_dollar_bill(self):
        ...
        self.play(ReplacementTransform(small_bill, bill), self.alice.change, 'raise_right_hand')
        self.play(ShowCreation(arrow))
    ...
    def everyone_thinks(self):
        self.play(*[ApplyMethod(pi.change, 'thinking', self.ledger) for pi in self.pi_creatures])
        self.wait(4)
...
class BitcoinIsALedger(Scene):
    ...
    def add_currency_to_tx_history(self):
        ...
        equation.shift(self.btc_to_ledger[1].get_center() - equation[1].get_center() + 2 * UP)
...
class BigDifferenceBetweenLDAndCryptocurrencies(Scene):
    ...
    def construct(self):
        ...
        self.add(logos)
        self.wait()
        self.play(cryptos.next_to, decentralized, DOWN, LARGE_BUFF, ld.next_to, centralized, DOWN, LARGE_BUFF)
...
class TransitionToDistributedLedger(DistributedLedgerScene):
    CONFIG = {'sign_transactions': True, 'ledger_width': 7.5, 'ledger_height': 6, 'enumerate_lines': True, 'denomination': 'LD', 'line_number_color': WHITE, 'ledger_line_height': 0.35}
    ...
    def add_various_payements(self):
        payments = VGroup(*[self.add_payment_line_to_ledger(*payment) for payment in [('Alice', 'Bob', 20), ('Charlie', 'You', 100), ('You', 'Alice', 50), ('Bob', 'You', 30)]])
        for payment in payments:
    ...
    def ask_who_controls_ledger(self):
        ...
        self.play(*[ApplyMethod(pi.change, 'confused', new_question) for pi in self.pi_creatures])
        self.wait(2)
    ...
    def distribute_ledger(self):
        ...
        self.play(*[ApplyMethod(pi.change, 'pondering', pi.ledger) for pi in self.pi_creatures])
    ...
    def broadcast_transaction(self):
        ...
        payment_copies = VGroup(*[payment.copy().next_to(pi, UP) for pi in self.pi_creatures])
        payment = payment_copies[0]
        self.play(self.alice.change, 'raise_right_hand', payment, Write(payment, run_time=2))
        self.wait()
        self.play(ReplacementTransform(VGroup(payment), payment_copies, run_time=3, rate_func=squish_rate_func(smooth, 0.5, 1)), Broadcast(self.alice.get_corner(UP + RIGHT)), self.alice.change, 'happy')
        self.wait()
        pairs = list(zip(payment_copies, self.pi_creatures))
        Scene.play(self, *it.chain(*[[pi.look_at, pi.ledger[-1], line.scale, 0.2, line.next_to, pi.ledger[-1], DOWN, SMALL_BUFF] for line, pi in pairs]))
        self.wait(3)
    ...
    def ask_about_ledger_consistency(self):
        ...
        self.play(FadeOut(self.network), ledgers.scale, 2, ledgers.arrange, RIGHT, ledgers.space_out_submobjects)
        question = OldTexText('Are these the same?')
        question.scale(1.5)
        question.to_edge(UP)
        arrows = VGroup(*[Arrow(question.get_bottom(), ledger.get_top()) for ledger in ledgers])
        self.play(*list(map(ShowCreation, arrows)))
        self.play(Write(question))
        self.wait()
...
class BobDoubtsBroadcastTransaction(DistributedLedgerScene):
    ...
    def bob_receives_transaction(self):
        ...
        self.play(Broadcast(corner), ApplyMethod(payment.next_to, bob, UP, run_time=3, rate_func=squish_rate_func(smooth, 0.3, 1)))
    ...
    def bob_tries_to_pay_charlie(self):
        ...
        self.play(PiCreatureSays(bob, 'Did you hear that?', target_mode='sassy'), FadeIn(chralie_group))
...
class YouListeningToBroadcasts(LedgerScene):
    ...
    def construct(self):
        ledger = self.get_ledger()
        payments = VGroup(*[self.add_payment_line_to_ledger(*payment) for payment in [('Alice', 'You', 20), ('Bob', 'You', 50), ('Charlie', 'You', 30)]])
        self.remove(self.ledger)
        corners = [FRAME_X_RADIUS * RIGHT * u1 + FRAME_Y_RADIUS * UP * u2 for u1, u2 in [(-1, 1), (1, 1), (-1, -1)]]
        you = self.you
        you.scale(2)
        you.center()
        self.add(you)
        for payment, corner in zip(payments, corners):
            ...
            self.play(Broadcast(corner), ApplyMethod(payment.next_to, you, vect, run_time=3, rate_func=squish_rate_func(smooth, 0.3, 1)), you.change_mode, 'pondering')
        self.wait()
...
class AskWhatToAddToProtocol(InitialProtocol):
    ...
    def construct(self):
        self.add_title()
        items = VGroup(*list(map(self.get_new_item, ['Broadcast transactions', 'Only accept signed transactions', 'No overspending'] + [''] * 6)))
...
class ThisIsWellIntoTheWeeds(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.teacher_says("We're well into \\\\ the weeds now", target_mode='sassy', added_anims=[ApplyMethod(pi.change, mode) for pi, mode in zip(self.students, ['hooray', 'sad', 'erm'])])
        self.wait()
        self.play(idea.restore, RemovePiCreatureBubble(self.teacher, target_mode='hooray', look_at=lightbulb))
        self.play_student_changes(*['pondering'] * 3, added_anims=[LaggedStartMap(FadeIn, lightbulb)])
        self.play(LaggedStartMap(ApplyMethod, lightbulb, lambda b: (b.set_color, YELLOW_A), rate_func=there_and_back))
        self.wait(2)
...
class IntroduceSHA256(Scene):
    ...
    def introduce_evaluation(self):
        messages = ['3Blue1Brown', '3Blue1Crown', 'Mathologer', 'Infinite Series', 'Numberphile', 'Welch Labs', '3Blue1Brown']
        groups = VGroup()
        ...
        brace_text_pairs = [(sha_brace, sha_text), (message_brace, message_text), (digest_brace, digest_text)]
        ...
        for mob in (digest, message):
            self.play(LaggedStartMap(ApplyMethod, mob, lambda m: (m.set_color, YELLOW), rate_func=there_and_back, run_time=1))
        self.wait()
        ...
        for new_group in groups[1:]:
            ...
            self.play(Transform(lhs, new_lhs), message_brace.stretch_to_fit_width, new_message.get_width(), message_brace.next_to, new_message, DOWN, MaintainPositionRelativeTo(message_text, message_brace), MaintainPositionRelativeTo(sha_brace, lhs[0]), MaintainPositionRelativeTo(sha_text, sha_brace))
            self.play(Transform(digest, new_digest, run_time=2, lag_ratio=0.5, path_arc=np.pi / 2))
    ...
    def issue_challenge(self):
        ...
        self.play(Transform(self.digest, new_digest, run_time=2, lag_ratio=0.5, path_arc=np.pi / 2), Transform(self.digest_text, desired_output_text))
    ...
    def guess_and_check(self):
        ...
        for new_group in groups[1:]:
            self.play(Transform(group[0], new_group[0], run_time=0.5))
...
class IntroduceNonceOnTrasactions(LedgerScene):
    CONFIG = {'denomination': 'LD', 'ledger_width': 5, 'ledger_line_height': 0.3}
    ...
    def hash_with_nonce(self):
        ledger = self.ledger
        self.add(*[self.add_payment_line_to_ledger(*payment) for payment in [('Alice', 'Bob', 20), ('Alice', 'You', 30), ('Charlie', 'You', 100)]])
        ...
        digest = sha256_tex_mob("Man, you're reading this deeply into\n            the code behind videos?  I'm touched,\n            really touched.  Keeping loving math, my\n            friend. ", n_forced_start_zeros=30)
        ...
        self.wait()
        self.play(nonce.next_to, ledger.content, DOWN, MED_SMALL_BUFF, LEFT, FadeOut(special_word), FadeOut(nonce_brace))
        ...
        self.play(LaggedStartMap(ApplyMethod, decomposed_ledger, lambda m: (m.set_color, YELLOW), rate_func=there_and_back))
        point = VectorizedPoint(sha.get_center())
        point.set_fill(opacity=1)
        self.play(LaggedStartMap(Transform, decomposed_ledger.copy(), lambda m: (m, point), run_time=1))
        bit_iter = iter(digest)
        self.play(LaggedStartMap(ReplacementTransform, VGroup(*[point.copy() for x in range(256)]), lambda m: (m, next(bit_iter))))
    ...
    def write_probability(self):
        probability = OldTexText('Probability: $\\frac{1}{2^{30}}$', '$\\approx \\frac{1}{1{,}000{,}000{,}000}$')
    ...
    def name_proof_of_work(self):
        ...
        arrow = Arrow(words.get_top(), self.nonce.get_bottom(), color=WHITE, tip_length=0.15)
    ...
    def change_ledger(self):
        ...
        self.play(Transform(self.digest, new_digest, lag_ratio=0.5))
    ...
    def construct(self):
        ...
        for pi in self.pi_creatures:
            ...
            for line in outgoing_lines:
                ...
                dots.add(dot)
            self.play(Animation(point), Broadcast(pi), *[Succession(FadeIn(dot), MoveToTarget(dot, run_time=2)) for dot in dots])
            self.play(*it.chain(*[[dot.move_to, dot.ledger, dot.set_fill, None, 0] for dot in dots]))

class IntroduceBlockChain(Scene):
    CONFIG = {'transaction_color': YELLOW, 'proof_of_work_color': GREEN, 'prev_hash_color': BLUE, 'block_width': 3, 'block_height': 3.5, 'n_transaction_lines': 8, 'payment_height_to_block_height': 0.15}
    ...
    def divide_ledger_into_blocks(self):
        ...
        arrows = VGroup(*[Arrow(blocks_word.get_top(), block.get_bottom(), buff=MED_LARGE_BUFF, color=WHITE) for block in blocks])
        self.play(LaggedStartMap(FadeIn, blocks))
        self.play(Write(blocks_word), LaggedStartMap(ShowCreation, arrows, run_time=1))
        self.wait()
        for group in (all_payments, all_proofs_of_work):
            self.play(LaggedStartMap(Indicate, group, rate_func=there_and_back, scale_factor=1.1))
    ...
    def show_proofs_of_work(self):
        ...
        for group in (signatures, proofs_of_work):
            self.play(LaggedStartMap(Indicate, group, run_time=2, rate_func=there_and_back))
            self.wait()
    ...
    def chain_blocks_together(self):
        ...
        new_arrows = VGroup()
        for block in blocks:
            end = np.array([block.get_left()[0] + block_spacing[0], block.prev_hash.get_center()[1], 0])
        ...
        for i in range(3):
            self.play(ReplacementTransform(digests[i], prev_hashes[i + 1]), Transform(sha_arrows[i], new_arrows[i]))
    ...
    def redo_proof_of_work(self):
        ...
        for proof_of_work, prev_hash in zip(proofs_of_work, hashes):
            num_pow_group = VGroup(*[Integer(random.randint(10 ** 9, 10 ** 10)) for x in range(50)])
    ...
    def get_block(self):
        block = VGroup()
        rect = Rectangle(color=WHITE, height=self.block_height, width=self.block_width)
        h_line1, h_line2 = [Line(rect.get_left(), rect.get_right()).shift(0.3 * rect.get_height() * vect) for vect in (UP, DOWN)]
        payments = VGroup()
        if not hasattr(self, 'transaction_counter'):
            self.transaction_counter = 0
        for x in range(2):
            hashes = [hash('%d %d' % (seed, self.transaction_counter)) for seed in range(3)]
            payment = OldTexText(self.names[hashes[0] % 3], 'pays', self.names[hashes[1] % 4], '%d0 LD' % (hashes[2] % 9 + 1))
            payment.set_color_by_tex('LD', YELLOW)
...
class DistributedBlockChainScene(DistributedLedgerScene):
    CONFIG = {'block_height': 0.5, 'block_width': 0.5, 'n_blocks': 3}
    ...
    def get_block_chain(self):
        ...
        for b1, b2 in zip(blocks, blocks[1:]):
            arrow = Arrow(LEFT, RIGHT, preserve_tip_size_when_scaling=False, tip_length=0.15)
    ...
    def get_block(self):
        block = Rectangle(color=WHITE, height=self.block_height, width=self.block_width)
        for vect in (UP, DOWN):
    ...
    def create_pi_creatures(self):
        ...
        VGroup(self.alice, self.alice.label, self.charlie, self.charlie.label).shift(LEFT)
        return creatures

class FromBankToDecentralizedSystem(DistributedBlockChainScene):
    CONFIG = {'n_blocks': 5, 'ledger_height': 3}
    ...
    def remove_bank(self):
        bank = SVGMobject(file_name='bank_building', color=GREY_B, height=3)
        cross = Cross(bank)
        ...
        self.play(ShowCreation(cross))
        self.wait()
        self.play(group.next_to, FRAME_X_RADIUS * RIGHT, RIGHT, rate_func=running_start, path_arc=-np.pi / 6)
        self.wait()
        self.remove(group)
    ...
    def show_block_chains(self):
        ...
        self.play(LaggedStartMap(FadeIn, center_chain.blocks, run_time=1), ShowCreation(center_chain.arrows))
        self.wait()
        self.play(ReplacementTransform(VGroup(center_chain), chains), *[ApplyMethod(pi.change, 'pondering', pi.ledger) for pi in creatures])
        self.wait()

    def add_crypto_terms(self):
        terms = OldTexText('Digital signatures \\\\', 'Cryptographic hash functions')
    ...
    def set_aside_everything(self):
        ...
        LedgerScene.add_payment_line_to_ledger(self, 'Alice', 'Bob', '40')
        LedgerScene.add_payment_line_to_ledger(self, 'Charlie', 'Alice', '60')
        ledger.next_to(ORIGIN, LEFT)
        self.remove(digital_signature)
        everything = VGroup(*self.get_top_level_mobjects())
        self.play(Animation(digital_signature), everything.scale, 0.1, everything.to_corner, DOWN + LEFT)
        self.play(Write(ledger), digital_signature.next_to, ORIGIN, RIGHT)
        self.wait(2)

class IntroduceBlockCreator(DistributedBlockChainScene):
    CONFIG = {'n_block_creators': 3, 'n_pow_guesses': 60}
    ...
    def broadcast_transactions(self):
        payment_parts = [('Alice', 'Bob', 20), ('Bob', 'Charlie', 10), ('Charlie', 'You', 50), ('You', 'Alice', 30)]
        payments = VGroup()
        ...
        for from_name, to_name, amount in payment_parts:
            ...
            payment.set_color_by_tex('LD', YELLOW)
            for name in self.get_names():
                payment.set_color_by_tex(name.capitalize(), self.get_color_from_name(name))
            payment.scale(0.7)
        ...
        for pi, payment, alpha in zip(self.pi_creatures, payments, alpha_range):
            ...
            anims.append(Broadcast(pi, rate_func=rf1, big_radius=3))
    ...
    def collect_transactions(self):
        ...
        block.add(prev_hash)
        self.play(FadeIn(block), MoveToTarget(payments), creator.change, 'raise_right_hand')
        self.wait()
        block.add(payments)
        self.block = block
    ...
    def add_block_reward(self):
        payments = self.payments
        new_transaction = OldTexText(self.block_creator_labels[0].get_tex(), 'gets', '10 LD')
    ...
    def comment_on_block_reward(self):
        ...
        comments = VGroup(*list(map(TexText, ["- ``Block reward''", '- No sender/signature', '- Adds to total money supply'])))
    ...
    def broadcast_block(self):
        ...
        for old_chain, new_chain in zip(old_chains, new_chains):
            for attr in ('blocks', 'arrows'):
                pairs = list(zip(getattr(old_chain, attr), getattr(new_chain, attr)))
                for m1, m2 in pairs:
        ...
        self.play(*it.chain(arrow_creations, [ApplyMethod(pi.change, 'hooray', pi.block_chain.get_right()) for pi in self.pi_creatures]))
        self.wait(3)

class MiningIsALottery(IntroduceBlockCreator):
    CONFIG = {'n_miners': 3, 'denomination': 'LD', 'n_guesses': 90, 'n_nonce_digits': 15}
    ...
    def make_guesses(self):
        ...
        self.play(winner.change, 'hooray', *[ApplyMethod(VGroup(self.blocks[i], self.arrows[i], nonces[i], digests[i]).fade, 0.7) for i in range(len(self.blocks)) if i is not winner_index])
    ...
    def get_payments(self, miner):
        ...
        lines.add(top_line)
        payments = [('Alice', 'Bob', 20), ('Charlie', 'You', 50)]
        for payment in payments:
...
class TwoBlockChains(DistributedBlockChainScene):
    ...
    def listen_for_new_blocks(self):
        ...
        corners = [u1 * FRAME_X_RADIUS * RIGHT + u2 * FRAME_Y_RADIUS * UP for u1, u2 in it.product(*[[-1, 1]] * 2)]
        ...
        for corner, block, arrow in zip(corners, moving_blocks, chain.arrows):
            ...
            self.play(ApplyMethod(block.restore, rate_func=squish_rate_func(smooth, 0.3, 0.8), run_time=3), Broadcast(corner, run_time=3), ShowCreation(arrow, rate_func=squish_rate_func(smooth, 0.8, 1), run_time=3))
        self.wait()
    ...
    def defer_to_longer(self):
        ...
        arrows = VGroup(*[Arrow(conflicting.get_bottom(), block_chain.get_top(), color=RED, buff=MED_LARGE_BUFF) for block_chain in block_chains])
        ...
        self.play(randy.change, 'confused', chain, Broadcast(corner), ApplyMethod(chain.restore, rate_func=squish_rate_func(smooth, 0.3, 0.7), run_time=3))
    ...
    def break_tie(self):
        ...
        self.play(randy.change, 'confused', block_chains, FadeIn(arrow_block))
        self.wait()
        arrow_block = arrow_block.copy()
        arrow_block.next_to(FRAME_X_RADIUS * RIGHT, RIGHT)
        self.play(ApplyMethod(arrow_block.next_to, block_chains[0], RIGHT, 0, run_time=3, rate_func=squish_rate_func(smooth, 0.3, 0.8)), Broadcast(arrow_block))
        ...
        checkmark.set_color(GREEN)
        self.play(ShowCreation(rect), Write(checkmark), randy.change, 'happy', arrow_block)
        self.wait(2)
...
class AskAboutTrustingWork(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.student_says('Is trusting work \\\\ really enough?', target_mode=mode)
...
class DoubleSpendingAttack(DistributedBlockChainScene):
    ...
    def show_fraudulent_block(self):
        ...
        self.play(ShowCreation(block), Write(block.content), self.alice.change, 'raise_left_hand')
    ...
    def send_to_bob(self):
        ...
        VGroup(arrow, block.target).next_to(self.bob.block_chain.blocks[-1], RIGHT, buff=0)
    ...
    def dont_send_to_rest_of_network(self):
        ...
        self.play(self.charlie.change, 'shruggie', self.you.change, 'shruggie')
    ...
    def get_fraud_block(self):
        ...
        content = VGroup()
        tuples = [('Prev hash', UP, BLUE), ('Proof of work', DOWN, GREEN)]
        for word, vect, color in tuples:
            mob = OldTexText(word)
            mob.set_color(color)
            mob.set_height(0.07 * block.get_height())
            mob.next_to(block.get_edge_center(vect), -vect, buff=0.06 * block.get_height())
            content.add(mob)
...
class AliceRacesOtherMiners(DoubleSpendingAttack):
    CONFIG = {'n_frames_per_pow': 3, 'fraud_block_height': 2, 'n_miners': 3, 'n_proof_of_work_digits': 11, 'proof_of_work_update_counter': 0}
    ...
    def initialize_characters(self):
        ...
        for miner in miners:
            ...
            for mob, vect in ((proof_of_work, DOWN), (prev_hash, UP)):
                ...
                mob.next_to(block.get_edge_center(vect), -vect, buff=0.05 * block.get_height())
                block.add(mob)
            block.proof_of_work = proof_of_work
            block.prev_hash = prev_hash
    ...
    def receive_broadcast_from_other_miners(self):
        ...
        self.play(Broadcast(block), *[MoveToTarget(mover, run_time=3, rate_func=squish_rate_func(smooth, 0.3, 0.8)) for mover in movers])
        self.play(FadeIn(winner.block), winner.change_mode, 'plain', self.alice.change, 'sassy')
    ...
    def race_between_alice_and_miners(self):
        ...
        chain = self.block_chain
        winners = ['Alice', 'Alice', 'Miners', 'Miners', 'Miners', 'Alice', 'Miners', 'Miners', 'Miners', 'Alice', 'Miners', 'Miners']
        self.revert_to_original_skipping_status()
        for winner in winners:
            ...
            anims = [MoveToTarget(block_copy, run_time=2), ShowCreation(arrow, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)), FadeIn(block)]
            if winner == 'Alice':
...
class WhenToTrustANewBlock(DistributedBlockChainScene):
    ...
    def construct(self):
        ...
        chain.to_edge(LEFT)
        self.add(chain)
        words = list(map(TexText, ["Don't trust yet", "Still don't trust", '...a little more...', 'Maybe trust', 'Probably safe', "Alright, you're good."]))
        ...
        for new_word in words[1:]:
            ...
            self.add_new_block(Transform(word, new_word, **kwargs), ApplyMethod(arrow.set_color, new_word.get_color(), **kwargs))
        self.wait(2)
    ...
    def add_new_block(self, *added_anims):
        ...
        self.play(Broadcast(block), ApplyMethod(block.restore, run_time=3, rate_func=squish_rate_func(smooth, 0.3, 0.8)), ShowCreation(arrow, run_time=3, rate_func=squish_rate_func(smooth, 0.7, 1)), *added_anims)
        arrows.add(arrow)
        blocks.add(block)
...
class MainIdeas(Scene):
    ...
    def construct(self):
        ...
        ideas = VGroup(*[OldTexText('$\\cdot$ ' + words) for words in ['Digital signatures', 'The ledger is the currency', 'Decentralize', 'Proof of work', 'Block chain']])
...
class VariableProofOfWork(WhenToTrustANewBlock):
    CONFIG = {'block_height': 3, 'block_width': 3, 'n_guesses': 60, 'n_proof_of_work_digits': 11, 'n_miners': 6}
    ...
    def change_challenge(self, n_zeros):
        ...
        self.play(Transform(n_zeros_mob, new_n_zeros_mob), Transform(digest, new_digest, lag_ratio=0.5), Transform(proof_of_work, new_pow))
...
class CompareBlockTimes(Scene):
    ...
    def construct(self):
        ...
        examples = VGroup(OldTexText('BTC: ', '10 minutes'), OldTexText('ETH: ', '15 Seconds'), OldTexText('XRP: ', '3.5 Seconds'), OldTexText('LTC: ', '2.5 Minutes'))
        examples.arrange(DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
        examples.next_to(h_line, DOWN)
        logos = VGroup(BitcoinLogo(), EthereumLogo(), ImageMobject('ripple_logo'), LitecoinLogo())
        ...
        self.wait()
        self.play(*[LaggedStartMap(FadeIn, VGroup(*group[1:])) for group in (examples, logos)])
        self.wait(2)
...
class BlockRewards(Scene):
    ...
    def construct(self):
        ...
        rewards = VGroup(OldTexText('Jan 2009 - Nov 2012:', '50', 'BTC'), OldTexText('Nov 2012 - Jul 2016:', '25', 'BTC'), OldTexText('Jul 2016 - Feb 2020$^*$:', '12.5', 'BTC'), OldTexText('Feb 2020$^*$ - Sep 2023$^*$:', '6.25', 'BTC'))
...
class ShowGeometricSum(Scene):
    ...
    def construct(self):
        equation = OldTex('210{,}000', '(', '50', '+', '25', '+', '12.5', '+', '6.25', '+\\cdots', ')', '=', '21{,}000{,}000')
...
class TransactionFeeExample(PiCreatureScene):
    ...
    def construct(self):
        ...
        payment = OldTexText('Alice', 'pays', 'Bob', '0.42 BTC')
        ...
        self.wait()
        self.play(alice.change, 'raise_right_hand', payment, fee.restore)
        self.play(Write(signature))
        self.play(ShowCreation(rect), alice.change_mode, 'happy')
        self.wait()
        self.play(Write(incentive_words), ShowCreation(arrow), alice.change, 'pondering')
        self.wait(2)
...
class ShowBitcoinBlockSize(LedgerScene):
    ...
    def construct(self):
        block = VGroup()
        ledger = self.get_ledger()
        payments = VGroup(*[self.add_payment_line_to_ledger(*args) for args in [('Alice', 'Bob', '0.42'), ('You', 'Charlie', '3.14'), ('Bob', 'You', '2.72'), ('Alice', 'Charlie', '4.67')]])
        dots = OldTex('\\vdots')
        ...
        tuples = [('Prev hash', UP, BLUE_C), ('Proof of work', DOWN, GREEN)]
        for word, vect, color in tuples:
            mob = OldTexText(word)
            mob.set_color(color)
        ...
        self.wait()
        visa_logo = SVGMobject(file_name='visa_logo', height=0.5, stroke_width=0, fill_color=BLUE_D, fill_opacity=1)
...
class CurrentAverageFees(Scene):
    ...
    def construct(self):
        fees = OldTexText('Current average fees: ', '$\\sim 0.0013$ BTC', '$\\approx$', '\\$3.39')
...
class TopicsNotCovered(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        topics = VGroup(*list(map(TexText, ['Merkle trees', 'Alternatives to proof of work', 'Scripting', '$\\vdots$', '(See links in description)'])))
        ...
        for topic in topics:
            self.play_student_changes('confused', 'thinking', 'pondering', look_at=topic, added_anims=[LaggedStartMap(FadeIn, topic)])
            self.wait()

class Exchange(Animation):
    CONFIG = {'rate_func': None}

    def __init__(self, exchange, **kwargs):
        self.swap = Swap(exchange.left, exchange.right)
...
class ShowManyExchanges(Scene):
    CONFIG = {'n_rows': 2, 'n_cols': 8, 'shift_radius': 0.5, 'run_time': 30}
    ...
    def construct(self):
        ...
        exchange = VGroup(*[Arrow(p1, p2, path_arc=np.pi, buff=MED_LARGE_BUFF) for p1, p2 in [(LEFT, RIGHT), (RIGHT, LEFT)]]).set_color(WHITE)
        exchanges = VGroup(*[VGroup(*[exchange.copy() for x in range(3)]).arrange(RIGHT, buff=2 * LARGE_BUFF) for y in range(3)]).arrange(DOWN, buff=MED_LARGE_BUFF)
    ...
    def get_cryptocurrencies(self):
        return [BitcoinLogo(), BitcoinLogo(), BitcoinLogo(), EthereumLogo(), LitecoinLogo()]

    def get_currencies(self):
        return [OldTex('\\$').set_color(GREEN), OldTex('\\$').set_color(GREEN), OldTex('\\$').set_color(GREEN), SVGMobject(file_name='euro_symbol', stroke_width=0, fill_opacity=1, fill_color=BLUE), SVGMobject(file_name='yen_symbol', stroke_width=0, fill_opacity=1, fill_color=RED)]

class ShowLDAndOtherCurrencyExchanges(ShowManyExchanges):
    CONFIG = {'run_time': 15}

    def get_cryptocurrencies(self):
        euro = SVGMobject(file_name='euro_symbol', stroke_width=0, fill_opacity=1, fill_color=BLUE)
        return [OldTexText('LD').set_color(YELLOW), OldTexText('LD').set_color(YELLOW), euro, euro.copy(), SVGMobject(file_name='yen_symbol', stroke_width=0, fill_opacity=1, fill_color=RED), BitcoinLogo()]

    def get_currencies(self):
        return [Tex('\\$').set_color(GREEN)]

class CryptoPatreonThanks(PatreonThanks):
    CONFIG = {'specific_patrons': ['Ali Yahya', 'Desmos', 'Burt Humburg', 'CrypticSwarm', 'Juan Benet', 'Samantha D. Suplee', 'James Park', 'Erik Sundell', 'Yana Chernobilsky', 'Kaustuv DeBiswas', 'Kathryn Schmiedicke', 'Karan Bhargava', 'Yu Jun', 'Dave Nicponski', 'Damion Kistler', 'Markus Persson', 'Yoni Nazarathy', 'Ed Kellett', 'Joseph John Cox', 'Dan Buchoff', 'Luc Ritchie', 'Andrew Busey', 'Michael McGuffin', 'John Haley', 'Mourits de Beer', 'Ankalagon', 'Eric Lavault', 'Tomohiro Furusawa', 'Boris Veselinovich', 'Julian Pulgarin', 'Jeff Linse', 'Cooper Jones', 'Ryan Dahl', 'Mark Govea', 'Robert Teed', 'Jason Hise', 'Meshal Alshammari', 'Bernd Sing', 'Nils Schneider', 'James Thornton', 'Mustafa Mahdi', 'Mathew Bramson', 'Jerry Ling', 'Vecht', 'Shimin Kuang', 'Rish Kundalia', 'Achille Brighton', 'Ripta Pasay']}
...
class ProtocolLabs(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(DrawBorderThenFill(logo[0]), LaggedStartMap(FadeIn, logo[1]), morty.change, 'raise_right_hand')
        self.wait()
    ...
    def get_logo(self):
        logo = SVGMobject(file_name='protocol_labs_logo', height=1.5, fill_color=WHITE)
        name = SVGMobject(file_name='protocol_labs_name', height=0.5 * logo.get_height(), fill_color=GREY_B)
