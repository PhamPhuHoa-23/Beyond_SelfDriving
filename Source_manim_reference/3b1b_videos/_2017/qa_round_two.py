...
class Announcements(PiCreatureScene):
    ...
    def construct(self):
        ...
        announcements = VGroup(*[OldTexText('$\\cdot$ %s' % s) for s in ['Q\\&A Round 2', 'The case against Net Neutrality?']])
        announcements.arrange(DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
        announcements.next_to(underline, DOWN, LARGE_BUFF, aligned_edge=LEFT)
        announcements.set_color_by_gradient(GREEN, YELLOW)
        self.play(Write(title), LaggedStartMap(FadeIn, announcements), ShowCreation(underline), self.pi_creature.change, 'hooray', underline)
...
class PowersOfTwo(Scene):
    ...
    def construct(self):
        ...
        for i, vect, po2 in zip(it.count(), vects, powers_of_2[1:]):
            ...
            pa = np.pi / 3
            self.play(Transform(curr_po2, po2), ReplacementTransform(mob, m1, path_arc=pa), ReplacementTransform(mob.copy(), m2, path_arc=pa))
            mob = VGroup(*it.chain(m1, m2))
        self.play(mob.space_out_submobjects, 1.1, mob.to_edge, RIGHT)
        two_to_ten.to_edge(LEFT)
        lines = VGroup(*[Line(two_to_ten.get_corner(vect + RIGHT), mob[0].get_corner(vect + LEFT)) for vect in (UP, DOWN)])
        ...
        self.play(LaggedStartMap(FadeIn, curr_po2_outline, rate_func=lambda t: wiggle(t, 8), run_time=2, lag_ratio=0.75), *get_confetti_animations(50))
...
class PiHoldingScreen(PiCreatureScene):
    ...
    def construct(self):
        ...
        screen.to_edge(LEFT)
        words = VGroup(OldTexText('Ben Eater'), OldTexText('The Case Against Net Neutrality?'))
