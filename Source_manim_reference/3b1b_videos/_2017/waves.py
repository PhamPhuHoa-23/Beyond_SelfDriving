...
class OscillatingVector(ContinualAnimation):
    CONFIG = {'tail': ORIGIN, 'frequency': 1, 'A_vect': [1, 0, 0], 'phi_vect': [0, 0, 0], 'vector_to_be_added_to': None}
    ...
    def update_mobject(self, dt):
        ...
        angle = 2 * np.pi * f * t
        vect = np.array([A * np.exp(complex(0, angle + phi)) for A, phi in zip(self.A_vect, self.phi_vect)]).real
        self.update_tail()
...
class OscillatingVectorComponents(ContinualAnimationGroup):
    CONFIG = {'tip_to_tail': False}

    def __init__(self, oscillating_vector, **kwargs):
        digest_config(self, kwargs)
        vx = Vector(UP, color=GREEN).fade()
        vy = Vector(UP, color=RED).fade()
        kwargs = {'frequency': oscillating_vector.frequency, 'tail': oscillating_vector.tail}
        ovx = OscillatingVector(vx, A_x=oscillating_vector.A_x, phi_x=oscillating_vector.phi_x, A_y=0, phi_y=0, **kwargs)
        ovy = OscillatingVector(vy, A_x=0, phi_x=0, A_y=oscillating_vector.A_y, phi_y=oscillating_vector.phi_y, **kwargs)
        components = [ovx, ovy]
        ...
        if self.tip_to_tail:
            ...
            for ov1, ov2 in ((ovx, ovy), (ovy, ovx)):
                ov_line = ov1.copy()
                ov_line.mobject = ov_line.vector = DashedLine(UP, DOWN, color=ov1.vector.get_color())
...
class EMWave(ContinualAnimationGroup):
    CONFIG = {'wave_number': 1, 'frequency': 0.25, 'n_vectors': 40, 'propogation_direction': RIGHT, 'start_point': FRAME_X_RADIUS * LEFT + DOWN + OUT, 'length': FRAME_WIDTH, 'amplitude': 1, 'rotation': 0, 'A_vect': [0, 0, 1], 'phi_vect': [0, 0, 0], 'requires_start_up': False}
    ...
    def update_mobject(self, dt):
        if self.requires_start_up:
            ...
            middle_alpha = interpolate(0.4, 1.4, self.external_time / prop_time)
            new_smooth = squish_rate_func(smooth, 0.4, 0.6)
            ovs = self.continual_animations
            for ov, alpha in zip(ovs, np.linspace(0, 1, len(ovs))):
                epsilon = 0.0001
                new_amplitude = np.clip(new_smooth(middle_alpha - alpha), epsilon, 1)
...
class WavePacket(Animation):
    CONFIG = {'EMWave_config': {'wave_number': 0, 'start_point': FRAME_X_RADIUS * LEFT, 'phi_vect': np.ones(3) * np.pi / 4}, 'em_wave': None, 'run_time': 4, 'rate_func': None, 'packet_width': 6, 'include_E_vects': True, 'include_M_vects': True, 'filter_distance': FRAME_X_RADIUS, 'get_filtered': False, 'remover': True, 'width': 2 * np.pi}
    ...
    def __init__(self, **kwargs):
        ...
        self.wave_packet_start, self.wave_packet_end = [em_wave.start_point - u * self.packet_width / 2, em_wave.start_point + u * (em_wave.length + self.packet_width / 2)]
        Animation.__init__(self, self.vects, **kwargs)

    def interpolate_mobject(self, alpha):
        packet_center = interpolate(self.wave_packet_start, self.wave_packet_end, alpha)
        em_wave = self.em_wave
        for vect in self.vects:
            tail = vect.get_start()
            distance_from_packet = np.dot(tail - packet_center, em_wave.propogation_direction)
...
class PolarizingFilter(Circle):
    CONFIG = {'stroke_color': GREY_D, 'fill_color': GREY_B, 'fill_opacity': 0.5, 'label_tex': None, 'filter_angle': 0, 'include_arrow_label': True, 'arrow_length': 0.7}
    ...
    def __init__(self, **kwargs):
        ...
        if self.label_tex:
            ...
            self.add(self.label)
        arrow = Arrow(ORIGIN, self.arrow_length * UP, color=WHITE, buff=0)
        ...
        if self.include_arrow_label:
            arrow_label = OldTex('%.1f^\\circ' % (self.filter_angle * 180 / np.pi))
...
class FilterScene(ThreeDScene):
    CONFIG = {'filter_x_coordinates': [0], 'pol_filter_configs': [{}], 'EMWave_config': {'start_point': FRAME_X_RADIUS * LEFT + DOWN + OUT}, 'axes_config': {}, 'start_phi': 0.8 * np.pi / 2, 'start_theta': -0.6 * np.pi, 'ambient_rotation_rate': 0.01}
    ...
    def setup(self):
        ...
        self.pol_filters = VGroup(*[PolarizingFilter(**config) for config in self.pol_filter_configs])
    ...
    def get_filter_absorption_animation(self, pol_filter, photon):
        ...
        return ApplyMethod(pol_filter.set_fill, RED, run_time=photon.run_time, rate_func=squish_rate_func(there_and_back, alpha - 0.1, alpha + 0.1))

class DirectionOfPolarizationScene(FilterScene):
    CONFIG = {'pol_filter_configs': [{'include_arrow_label': False}], 'target_theta': -0.97 * np.pi, 'target_phi': 0.9 * np.pi / 2, 'ambient_rotation_rate': 0.005, 'apply_filter': False, 'quantum': False}
    ...
    def change_polarization_direction(self, angle, **kwargs):
        ...
        self.play(ApplyMethod(self.reference_line.rotate, angle, **kwargs), *added_anims)

    def setup_rectangles(self):
        rect1 = Rectangle(height=2 * self.em_wave.amplitude, width=FRAME_X_RADIUS + 0.25, stroke_color=BLUE, fill_color=BLUE, fill_opacity=0.2)
    ...
    def apply_filters(self):
        ...
        filters = sorted(self.pol_filters, lambda pf1, pf2: cmp(pf1.get_center()[0], pf2.get_center()[0]))
            ...
            for vect_group, angle in zip(vect_groups, [0, -np.pi / 2]):
                ...
                for vect_mob in vect_group:
                    ...
                    if start[0] > filter_x:
                        ...
                        if not self.quantum:
                            vect_mob.scale(np.cos(angle_diff), about_point=start)
    ...
    def update_rectangles(self):
        ...
        target_depth = max(2 * self.em_wave.amplitude * abs(np.cos(target_angle)), epsilon)
        r2.stretch_in_place(target_depth / curr_depth, 2)

class WantToLearnQM(TeacherStudentsScene):

    def construct(self):
        question1 = OldTex('\\text{What does }\\qquad \\\\', '|\\!\\psi \\rangle', '=', '\\frac{1}{\\sqrt{2}}', '|\\!\\uparrow \\rangle', '+', '\\frac{1}{\\sqrt{2}}', '|\\!\\downarrow \\rangle \\\\', '\\text{mean?}\\qquad\\quad')
        question1.set_color_by_tex_to_color_map({'psi': BLUE, 'uparrow': GREEN, 'downarrow': RED})
        question2 = OldTexText('Why are complex \\\\ numbers involved?')
        question3 = OldTexText('How do you compute \\\\ quantum probabilities?')
        questions = [question1, question2, question3]
        bubbles = VGroup()
        for i, question in zip([1, 2, 0], questions):
            self.student_says(question, content_introduction_kwargs={'run_time': 2}, index=i, bubble_config={'fill_opacity': 1}, bubble_creation_class=FadeIn)
        ...
        self.wait(2)
        self.teacher_says('First, lots and lots \\\\ of linear algebra', added_anims=list(map(FadeOut, bubbles)))
        self.wait()
...
class Goal(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(VGroup(weirdness, cross).shift, DOWN, Write(foundations, run_time=2), randy.change, 'happy')
        self.wait(2)
...
class IntroduceElectricField(PiCreatureScene):
    CONFIG = {'vector_field_colors': [BLUE_B, BLUE_D], 'max_vector_length': 0.9}
    ...
    def write_title(self):
        ...
        title = OldTexText('Electro', 'magnetic', ' field', arg_separator='')
        ...
        title.shift(DOWN)
        title.fade(1)
        self.play(title.restore, morty.change, 'raise_right_hand')
        self.play(title[0].set_color, BLUE, title[1].set_color, YELLOW)
        self.wait()
    ...
    def draw_field(self):
        ...
        self.play(LaggedStartMap(ShowCreation, vector_field, run_time=3), self.title.center, self.title.scale, 1.5, self.title.to_edge, UP, morty.change, 'happy', ORIGIN)
        self.wait()
    ...
    def add_particle(self):
        ...
        particle.fade(1)
        self.play(particle.restore, morty.change, 'raise_right_hand')
        self.play(morty.change, 'thinking', particle)
        self.play(ShowCreation(vector), Write(force, run_time=1))
        self.wait(2)
    ...
    def let_particle_wander(self):
        ...
        particles = VGroup(self.particle, *[self.particle.copy().move_to(point) for point in points])
        for particle in particles:
            particle.velocity = np.zeros(3)
        self.play(FadeOut(self.force_vector), LaggedStartMap(FadeIn, VGroup(*particles[1:])))
    ...
    def get_vector_field(self):
        result = VGroup(*[self.get_vector(point) for x in np.arange(-9, 9) for y in np.arange(-5, 5) for point in [x * RIGHT + y * UP]])
...
class IntroduceMagneticField(IntroduceElectricField, ThreeDScene):
    ...
    def add_vector_field(self):
        vector_field = self.get_vector_field()
        self.play(LaggedStartMap(ShowCreation, vector_field, run_time=3), Animation(self.title))
        self.wait()
    ...
    def introduce_moving_charge(self):
        ...
        self.play(particle.restore, run_time=2, rate_func=linear)
        self.add(velocity)
    ...
    def show_force(self):
        ...
        F_vect = Vector(3 * np.cross(self.field_function(point), RIGHT), color=GREEN)
        F_vect.shift(point)
        F_word = OldTexText('Force')
        F_word.rotate(np.pi / 2, RIGHT)
        F_word.next_to(F_vect, OUT)
        F_word.set_color(F_vect.get_color())
        F_eq = OldTex('=', 'q', '\\textbf{v}', '\\times', '\\textbf{B}')
        F_eq.set_color_by_tex_to_color_map({'q': RED, 'B': YELLOW})
...
class CurlRelationBetweenFields(ThreeDScene):
    ...
    def loop_in_E(self):
        E_vects = VGroup(*[Vector(0.5 * rotate_vector(vect, np.pi / 2)).shift(vect) for vect in compass_directions(8)])
    ...
    def loop_in_M(self):
        M_vects = VGroup(*[Vector(rotate_vector(vect, np.pi / 2), normal_vector=IN, color=M_COLOR).shift(vect) for vect in compass_directions(8, LEFT)[1:]])
        M_vects.rotate(np.pi / 2, RIGHT)
        new_point = self.E_circle_center + RIGHT
        M_vects.shift(new_point)
        E_vect = self.E_vects[0]
        self.play(ShowCreation(M_vects, run_time=2), *list(map(FadeOut, self.E_vects[1:])))
        self.wait()
        self.play(E_vect.rotate, np.pi, RIGHT, [], new_point, E_vect.scale_about_point, 3, new_point, run_time=4, rate_func=linear)
        self.wait()
    ...
    def second_loop_in_E(self):
        E_vects = VGroup(*[Vector(1.5 * rotate_vector(vect, np.pi / 2)).shift(vect) for vect in compass_directions(8, LEFT)[1:]])
        E_vects.set_color(E_COLOR)
        ...
        self.play(M_vect.rotate, np.pi, RIGHT, [], point, run_time=5, rate_func=linear)
        self.wait(3)
...
class WriteCurlEquations(Scene):
    ...
    def construct(self):
        ...
        eqs.to_edge(LEFT)
        for eq in eqs:
            eq.set_color_by_tex_to_color_map({'E': E_COLOR, 'B': M_COLOR})
...
class IntroduceEMWave(ThreeDScene):
    ...
    def construct(self):
        words = OldTexText('Electro', 'magnetic', ' radiation', arg_separator='')
        words.set_color_by_tex_to_color_map({'Electro': E_COLOR, 'magnetic': M_COLOR})
...
class ListRelevantWaveIdeas(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        topics = VGroup(*list(map(TexText, ['- Superposition', '- Amplitudes', '- How phase influences addition'])))
        topics.scale(0.8)
        ...
        self.play(Animation(VectorizedPoint(wave_point)), self.teacher.change, 'raise_right_hand')
        self.wait(2)
        self.play(Write(title, run_time=2), ShowCreation(h_line))
        self.play_student_changes(*['pondering'] * 3, added_anims=[LaggedStartMap(FadeIn, topics, run_time=3)], look_at=title)
        self.play(Animation(title), self.teacher.change, 'happy')
        self.play(title[0].next_to, quantum.copy(), UP, MED_SMALL_BUFF, LEFT, title[0].fade, 0.5, title[1].next_to, quantum.copy(), RIGHT, 2 * SMALL_BUFF, Write(quantum))
        self.wait(5)

class DirectWaveOutOfScreen(IntroduceEMWave):
    CONFIG = {'EMWave_config': {'requires_start_up': False, 'amplitude': 2, 'start_point': FRAME_X_RADIUS * LEFT, 'A_vect': [0, 1, 0], 'start_up_time': 0}}
    ...
    def move_into_position(self):
        ...
        faded_vectors = VGroup(*[ov.vector for ov in self.em_wave.continual_animations[:-2]])
        self.move_camera(0.99 * np.pi / 2, -0.01, run_time=2, added_anims=[faded_vectors.set_fill, None, 0.5])
        self.stop_ambient_camera_rotation()
        self.move_camera(np.pi / 2, 0, added_anims=[faded_vectors.set_fill, None, 0.05], run_time=2)
...
class ShowVectorEquation(Scene):
    CONFIG = {'f_color': RED, 'phi_color': MAROON_B, 'A_color': GREEN}
    ...
    def add_vector(self):
        ...
        self.oscillating_vector = OscillatingVector(self.vector, A_vect=[2, 0, 0], frequency=0.25)
        self.add(self.oscillating_vector)
        self.wait(3)

    def add_plane(self):
        xy_plane = NumberPlane(axes_color=GREY_B, color=GREY_D, secondary_color=GREY_D, x_unit_size=2, y_unit_size=2)
    ...
    def write_horizontally_polarized(self):
        words = OldTexText('``', 'Horizontally', ' polarized', "''", arg_separator='')
    ...
    def write_components(self):
        x, y = components = VGroup(OldTex('\\cos(', '2\\pi', 'f_x', 't', '+ ', '\\phi_x', ')'), OldTex('0', ''))
        ...
        E, equals = E_equals = OldTex('\\vec{\\textbf{E}}', '=')
        E.set_color(E_COLOR)
        ...
        group.next_to(self.horizontally_polarized_words, DOWN, MED_LARGE_BUFF, RIGHT)
        x_without_phi = OldTex('\\cos(', '2\\pi', 'f_x', 't', ')')
        x_without_phi.move_to(x)
        for mob in (x, x_without_phi):
            mob.set_color_by_tex_to_color_map({'f_x': self.f_color, 'phi_x': self.phi_color})
        ...
        def update_brace(brace):
            ...
            return brace
        moving_brace = Mobject.add_updater(Brace(Line(LEFT, RIGHT), DOWN), update_brace)
        moving_x_without_phi = Mobject.add_updater(x_without_phi.copy().add_background_rectangle(), lambda m: m.next_to(moving_brace.mobject, DOWN, SMALL_BUFF))
        ...
        self.play(FadeIn(moving_brace.mobject), FadeIn(x_without_phi), FadeIn(moving_x_without_phi.mobject), lag_ratio=0.5, run_time=2)
        self.wait(3)
        self.play(FadeOut(moving_brace.mobject), FadeOut(moving_x_without_phi.mobject))
    ...
    def show_graph(self):
        axes = Axes(x_min=-0.5, x_max=5.2, y_min=-1.5, y_max=1.5)
        ...
        high_f_graph, low_f_graph = graphs = VGroup(*[FunctionGraph(lambda x: np.cos(2 * np.pi * f * x), color=E_COLOR, x_min=0, x_max=4 / f, num_steps=20 / f) for f in (1, 0.25)])
        group = VGroup(axes, t, cos, high_f_graph, *fx_group)
        rect = SurroundingRectangle(group, buff=MED_LARGE_BUFF, stroke_color=WHITE, stroke_width=3, fill_color=BLACK, fill_opacity=0.9)
        ...
        new_ov = OscillatingVector(Vector(RIGHT, color=E_COLOR), A_vect=[2, 0, 0], frequency=1, start_up_time=0)
        self.play(FadeIn(group))
        self.play(ReplacementTransform(self.components[0].get_part_by_tex('f_x').copy(), fx_equals_1))
        self.wait(4 - self.oscillating_vector.internal_time % 4)
        self.remove(self.oscillating_vector)
        self.add(new_ov)
        self.play(ShowCreation(high_f_graph, run_time=4, rate_func=linear))
        self.wait()
        self.play(FadeOut(new_ov.vector))
        self.remove(new_ov)
        self.add(self.oscillating_vector)
        self.play(ReplacementTransform(*fx_group), ReplacementTransform(*graphs), FadeOut(new_ov.vector), FadeIn(self.vector))
        self.wait(4)
    ...
    def add_phi(self):
        ...
        self.play(ReplacementTransform(VGroup(*x_without_phi[:-1]), VGroup(*x[:-3])), ReplacementTransform(x_without_phi[-1], x[-1]), Write(VGroup(*x[-3:-1])), corner_cos[-1].next_to, corner_phi.copy(), RIGHT, SMALL_BUFF, Write(corner_phi), FadeOut(self.fx_equals_fourth))
    ...
    def add_amplitude(self):
        ...
        v_brace = Brace(Line(graph_y_axis.number_to_point(0), graph_y_axis.number_to_point(1)), LEFT, buff=SMALL_BUFF)
        ...
        def update_vect(vect):
            ...
            return vect
        self.play(GrowFromCenter(h_brace), GrowFromCenter(v_brace))
        self.wait(2)
        self.play(x.next_to, A, RIGHT, SMALL_BUFF, corner_cos.next_to, corner_A, RIGHT, SMALL_BUFF, FadeIn(all_As))
        x.add(A)
        corner_cos.add(corner_A)
        self.wait()
        factor = 0.5
        self.play(v_brace.stretch_in_place, factor, 1, v_brace.move_to, v_brace.copy(), DOWN, MaintainPositionRelativeTo(v_brace.A, v_brace), h_brace.stretch_in_place, factor, 0, h_brace.move_to, h_brace.copy(), LEFT, MaintainPositionRelativeTo(h_brace.A, h_brace), UpdateFromFunc(self.vector, update_vect), graph.stretch_in_place, factor, 1)
        self.wait(4)
    ...
    def add_kets(self):
        ...
        plus = OldTexText('+')
        group = VGroup(E_equals.target, x.target, right_ket, plus, y.target, up_ket)
        group.arrange(RIGHT)
        ...
        arrows = VGroup(*[Arrow(kets_word.get_top(), ket, color=ket.get_color()) for ket in kets])
        ...
        self.play(FadeOut(self.brackets), *list(map(MoveToTarget, [E_equals, x, y])))
        self.play(*list(map(Write, [right_ket, plus, up_ket])), run_time=1)
        self.play(Write(kets_word), LaggedStartMap(ShowCreation, arrows, lag_ratio=0.7), run_time=2)
        self.wait()
    ...
    def switch_to_vertically_polarized_light(self):
        ...
        y.target = OldTex('A_y', '\\cos(', '2\\pi', 'f_y', 't', '+', '\\phi_y', ')')
        y.target.set_color_by_tex_to_color_map({'A': self.A_color, 'f': self.f_color, 'phi': self.phi_color})
        ...
        def update_vect(vect, alpha):
            self.oscillating_vector.A_vect = rotate_vector(vector_A_vect, alpha * np.pi / 2)
            return vect
        new_h_brace = Brace(Line(ORIGIN, UP), RIGHT)
        words = OldTexText('``', 'Vertically', ' polarized', "''", arg_separator='')
        words.add_background_rectangle()
        words.move_to(self.horizontally_polarized_words)
        self.play(UpdateFromAlphaFunc(self.vector, update_vect), Transform(self.h_brace, new_h_brace), self.h_brace.A.next_to, new_h_brace, RIGHT, SMALL_BUFF, Transform(self.horizontally_polarized_words, words), *list(map(FadeOut, [self.corner_group, self.v_brace, self.v_brace.A, self.low_f_graph])))
        self.play(*list(map(MoveToTarget, movers)))
        self.wait(5)

class ChangeFromHorizontalToVerticallyPolarized(DirectionOfPolarizationScene):
    CONFIG = {'filter_x_coordinates': [], 'EMWave_config': {'start_point': FRAME_X_RADIUS * LEFT, 'A_vect': [0, 2, 0]}}
...
class SumOfTwoWaves(ChangeFromHorizontalToVerticallyPolarized):
    CONFIG = {'axes_config': {'y_max': 1.5, 'y_min': -1.5, 'z_max': 1.5, 'z_min': -1.5}, 'EMWave_config': {'A_vect': [0, 0, 1]}, 'ambient_rotation_rate': 0}
...
class ShowTipToTailSum(ShowVectorEquation):
    ...
    def add_vertial_vector(self):
        ...
        self.d_oscillating_vector = Mobject.add_updater(Vector(UP + RIGHT, color=E_COLOR), lambda v: v.put_start_and_end_on(ORIGIN, self.v_vector.get_end() + self.h_vector.get_end()))
    ...
    def add_kets(self):
        h_ket, v_ket = kets = VGroup(*[OldTex('\\cos(', '2\\pi', 'f', 't', ')', '|\\!\\%sarrow\\rangle' % s) for s in ('right', 'up')])
        for ket in kets:
            ket.set_color_by_tex_to_color_map({'f': self.f_color, 'rangle': YELLOW})
    ...
    def show_vector_sum(self):
        ...
        h_line.update = self.generate_dashed_line_update(self.h_vector, self.v_vector)
        v_line.update = self.generate_dashed_line_update(self.v_vector, self.h_vector)
        ...
        self.play(*it.chain(list(map(MoveToTarget, self.kets)), [Write(plus)], list(map(ShowCreation, [h_line, v_line]))))
        blue_black = average_color(BLUE, BLACK)
        self.play(GrowFromPoint(self.d_vector, ORIGIN), self.h_vector.set_fill, blue_black, self.v_vector.set_fill, blue_black)
        self.wait()
        self.add(self.h_oscillating_vector, self.v_oscillating_vector, self.d_oscillating_vector, Mobject.add_updater(h_line, h_line.update), Mobject.add_updater(v_line, v_line.update))
        self.wait(4)
        self.ket_sum = VGroup(h_ket, plus, v_ket)

    def write_superposition(self):
        superposition_words = OldTexText("``Superposition''", 'of', '$|\\!\\rightarrow\\rangle$', 'and', '$|\\!\\uparrow\\rangle$')
        ...
        self.play(Write(superposition_words, run_time=2), MoveToTarget(ket_sum))
        self.wait(2)
        self.play(FadeIn(sum_word), superposition_words.shift, MED_LARGE_BUFF * DOWN, ket_sum.shift, MED_LARGE_BUFF * DOWN)
        self.wait()
    ...
    def add_amplitudes(self):
        ...
        for mob in self.ket_sum:
            mob.generate_target()
        h_A, v_A = (2, 0.5)
        h_A_mob, v_A_mob = A_mobs = VGroup(*[OldTex(str(A)).add_background_rectangle() for A in [h_A, v_A]])
        A_mobs.scale(0.8)
        ...
        self.play(*it.chain(list(map(MoveToTarget, self.ket_sum)), list(map(Write, A_mobs)), [UpdateFromAlphaFunc(ov.vector, self.generate_A_update(ov, A * np.array(ov.A_vect), np.array(ov.A_vect))) for ov, A in [(h_ov, h_A), (v_ov, v_A)]]))
        self.wait(4)
        self.A_mobs = A_mobs
    ...
    def add_phase_shift(self):
        ...
        self.play(MoveToTarget(v_ket), Write(plus_phi), UpdateFromAlphaFunc(v_ov.vector, self.generate_phi_update(v_ov, np.array([0, np.pi / 2, 0]), np.array(v_ov.phi_vect))))
        self.play(FadeIn(ellipse))
        self.wait(5)
        self.play(UpdateFromAlphaFunc(h_ov.vector, self.generate_A_update(h_ov, 0.25 * np.array(h_ov.A_vect), np.array(h_ov.A_vect))), ellipse.stretch, 0.25, 0, Transform(h_A_mob, new_h_A_mob))
        self.wait(8)
...
class FromBracketFootnote(Scene):
    ...
    def construct(self):
        words = OldTexText('From, ``Bra', 'ket', "''", arg_separator='')
...
class CircularlyPolarizedLight(SumOfTwoWaves):
    CONFIG = {'EMWave_config': {'phi_vect': [0, np.pi / 2, 0]}}
...
class AlternateBasis(ShowTipToTailSum):
    ...
    def add_superposition_text(self):
        self.hv_superposition, self.da_superposition = superpositions = [OldTex('\\vec{\\textbf{E}}', '=', '(\\dots)', '|\\!\\%sarrow\\rangle' % s1, '+', '(\\dots)', '|\\!\\%sarrow\\rangle' % s2) for s1, s2 in [('right', 'up'), ('ne', 'nw')]]
    ...
    def rotate_plane(self):
        new_plane = NumberPlane(x_unit_size=2, y_unit_size=2, y_radius=FRAME_X_RADIUS, secondary_line_ratio=0)
        ...
        a = (LEFT + UP) / np.sqrt(2)
        self.wait(4)
        self.play(self.xy_plane.fade, 0.5, self.xy_plane.coordinate_labels.fade, 1, new_plane.restore, new_plane.rotate, np.pi / 4, UpdateFromAlphaFunc(self.h_vector, self.generate_A_update(self.h_oscillating_vector, 2 * d * np.dot(0.5 * RIGHT + UP, d), np.array(self.h_oscillating_vector.A_vect))), UpdateFromAlphaFunc(self.v_vector, self.generate_A_update(self.v_oscillating_vector, 2 * a * np.dot(0.5 * RIGHT + UP, a), np.array(self.v_oscillating_vector.A_vect))), Transform(self.hv_superposition, self.da_superposition), run_time=2)
        self.wait(4)

    def show_vertically_polarized(self):
        self.play(UpdateFromAlphaFunc(self.h_vector, self.generate_A_update(self.h_oscillating_vector, np.array([0.7, 0.7, 0]), np.array(self.h_oscillating_vector.A_vect))), UpdateFromAlphaFunc(self.v_vector, self.generate_A_update(self.v_oscillating_vector, np.array([-0.7, 0.7, 0]), np.array(self.v_oscillating_vector.A_vect))))
        self.wait(8)
...
class ShowPolarizingFilter(DirectionOfPolarizationScene):
    CONFIG = {'EMWave_config': {'start_point': FRAME_X_RADIUS * LEFT}, 'apply_filter': True}
    ...
    def mention_energy_absorption(self):
        ...
        lines = VGroup(*[Line(np.sin(a) * RIGHT + np.cos(a) * UP, np.sin(a) * LEFT + np.cos(a) * UP, color=RED, stroke_width=2) for a in np.linspace(0, np.pi, 15)])
    ...
    def write_as_superposition(self):
        ...
        for angle in (np.pi / 4, -np.pi / 6):
            ...
            self.wait(3)
        self.move_camera(theta=-0.6 * np.pi, added_anims=[Rotate(superposition, -0.6 * np.pi, axis=OUT)])
        rect.set_stroke(YELLOW, 3)
        self.play(ShowCreation(rect))
        arrow = Arrow(rect.get_nadir(), 3 * RIGHT + 0.5 * OUT, normal_vector=DOWN)
        self.play(ShowCreation(arrow))
        for angle in (np.pi / 3, -np.pi / 3, np.pi / 6):
            self.change_polarization_direction(angle)
            self.wait(2)
        self.play(FadeOut(superposition), FadeOut(arrow), *[FadeOut(cu.mobject) for cu in continual_updates])
    ...
    def diagonal_filter(self):
        ...
        self.play(Rotate(self.pol_filter, np.pi / 4, axis=LEFT), UpdateFromAlphaFunc(self.pol_filter, update_filter_angle), Animation(self.em_wave.mobject))
    ...
    def get_superposition_tex(self, angle, s1, s2):
        superposition = OldTex('0.00', '\\cos(', '2\\pi', 'f', 't', ')', '|\\! \\%sarrow \\rangle' % s1, '+', '1.00', '\\cos(', '2\\pi', 'f', 't', ')', '|\\! \\%sarrow \\rangle' % s2)
        A_x = DecimalNumber(0)
        A_y = DecimalNumber(1)
        ...
        def generate_decimal_update(trig_func):
            ...
            def update_decimal(decimal):
                new_decimal = DecimalNumber(abs(trig_func(self.reference_line.get_angle() - angle)))
                ...
                families = [mob.family_members_with_points() for mob in (decimal, new_decimal)]
                for sm1, sm2 in zip(*families):
                    sm1.interpolate(sm1, sm2, 1)
                return decimal
            return update_decimal
        continual_updates = [Mobject.add_updater(A_x, generate_decimal_update(np.sin)), Mobject.add_updater(A_y, generate_decimal_update(np.cos))]
...
class EnergyOfWavesWavePortion(DirectWaveOutOfScreen):
    CONFIG = {'EMWave_config': {'A_vect': [0, 1, 1], 'amplitude': 4, 'start_point': FRAME_X_RADIUS * LEFT + 2 * DOWN}}
    ...
    def add_components(self):
        ...
        period = 1.0 / self.em_wave.frequency
        self.add(h_wave, v_wave)
        self.play(FadeIn(h_wave.mobject), FadeIn(v_wave.mobject), self.brace.A.move_to, self.brace, self.brace.A.shift, SMALL_BUFF * (2 * UP + IN), ReplacementTransform(self.brace, h_brace), Write(h_brace.A))
        self.wait(6)
        self.play(ReplacementTransform(h_brace.copy(), v_brace), Write(v_brace.A))
        self.wait(6)
    ...
    def scale_wave(self, factor):
        ...
        self.play(MoveToTarget(h_brace), MoveToTarget(v_brace), *[UpdateFromAlphaFunc(ov.vector, generate_vect_update(ov)) for ov in it.chain(self.em_wave.continual_animations, self.h_wave.continual_animations, self.v_wave.continual_animations)])
...
class EnergyOfWavesTeacherPortion(TeacherStudentsScene):
    ...
    def show_energy_equation(self):
        ...
        dot.fade(1)
        self.dot = dot
        energy = OldTex('\\frac{\\text{Energy}}{\\text{Volume}}', '=', '\\epsilon_0', 'A', '^2')
        energy.set_color_by_tex('A', GREEN)
        energy.to_corner(UP + LEFT)
        component_energy = OldTex('=', '\\epsilon_0', 'A_x', '^2', '+', '\\epsilon_0', 'A_y', '^2')
        for i in (2, 6):
        ...
        self.play(Animation(dot), self.teacher.change, 'raise_right_hand', dot)
        self.play_student_changes(*['pondering'] * 3, look_at=dot)
        self.wait(2)
        self.play(Write(energy))
        self.play(self.teacher.change, 'happy')
        self.wait(3)
        self.play(ReplacementTransform(VGroup(*energy[-4:]).copy(), VGroup(*component_energy[:4])), ReplacementTransform(VGroup(*energy[-4:]).copy(), VGroup(*component_energy[4:])))
    ...
    def show_both_ways_of_thinking_about_it(self):
        ...
        for b, s in ((b1, s1), (b2, s2)):
            self.play(ShowCreation(b), Write(b.content, run_time=2), s.change, 'thinking')
            self.wait(2)
        self.play_student_changes(*['plain'] * 3, look_at=self.dot, added_anims=[self.teacher.change, 'raise_right_hand', self.dot])
        self.play(self.teacher.look_at, self.dot)
        self.wait(5)

class DescribePhoton(ThreeDScene):
    CONFIG = {'x_color': RED, 'y_color': GREEN}

    def setup(self):
        self.axes = ThreeDAxes()
        self.add(self.axes)
        self.set_camera_orientation(phi=0.8 * np.pi / 2, theta=-np.pi / 4)
        em_wave = EMWave(start_point=FRAME_X_RADIUS * LEFT, A_vect=[0, 1, 1], wave_number=0, amplitude=3)
        for ov in em_wave.continual_animations:
            ov.vector.normal_vector = RIGHT
        ...
        em_wave.update(0)
        photon = WavePacket(em_wave=em_wave, run_time=2)
        self.photon = photon
        self.em_wave = em_wave
    ...
    def add_ket_equation(self):
        equation = OldTex('|\\!\\psi\\rangle', '=', '\\alpha', '|\\!\\rightarrow \\rangle', '+', '\\beta', '|\\!\\uparrow \\rangle')
        equation.to_edge(UP)
    ...
    def reposition_to_face_photon_head_on(self):
        plane = NumberPlane(color=GREY_B, secondary_color=GREY_D, x_unit_size=2, y_unit_size=2, y_radius=FRAME_X_RADIUS)
        plane.add_coordinates(x_vals=list(range(-3, 4)), y_vals=[])
        plane.rotate(np.pi / 2, RIGHT)
        plane.rotate(np.pi / 2, OUT)
        self.play(self.em_wave.M_vects.set_fill, None, 0)
        self.move_camera(phi=np.pi / 2, theta=0, added_anims=[Rotate(self.equation, -self.camera.get_theta())])
        self.play(Write(plane, run_time=1), Animation(self.equation))
        self.xy_plane = plane

    def show_components(self):
        h_arrow, v_arrow = [Vector(1.38 * direction, color=color, normal_vector=RIGHT) for color, direction in [(self.x_color, UP), (self.y_color, OUT)]]
        v_arrow.move_to(h_arrow.get_end(), IN)
        h_part = VGroup(*self.equation[1][2:4]).copy()
        v_part = VGroup(*self.equation[1][5:7]).copy()
        self.play(self.equation.rect.set_stroke, BLUE, 4, self.equation.words.set_fill, WHITE, 1)
    ...
    def show_amplitude_and_phase(self):
        ...
        rhs = OldTex('=', 'A_x', 'e', '^{i', '(2\\pi', 'f', 't', '+', '\\phi_x)}')
    ...
    def change_basis(self):
        ...
        axes = self.axes
        movers = [plane, axes, h_arrow, v_arrow, h_part, v_part, self.equation, superposition]
        for mob in movers:
            mob.save_state()
        superposition.target = OldTex('\\gamma', '|\\! \\nearrow \\rangle', '+', '\\delta', '|\\! \\nwarrow \\rangle')
        ...
        A = 1.9
        h_arrow.target = Vector(A * np.cos(np.pi / 12) * rotate_vector(UP, np.pi / 6, RIGHT), normal_vector=RIGHT, color=TEAL)
        v_arrow.target = Vector(A * np.sin(np.pi / 12) * rotate_vector(OUT, np.pi / 6, RIGHT), normal_vector=RIGHT, color=MAROON)
    ...
    def write_different_meaning(self):
        ...
        self.play(Transform(morty, blinked, rate_func=squish_rate_func(there_and_back)))
        self.wait()
        self.play(*list(map(FadeOut, [morty, words, rect, self.equation.rect, self.equation.words])))
    ...
    def write_components(self):
        ...
        self.wait()
        self.play(FadeOut(self.h_part_tex), FadeOut(self.v_part_tex), GrowFromCenter(h_brace), GrowFromCenter(v_brace))
        self.play(Write(h_brace.label), Write(v_brace.label))
        self.wait()
    ...
    def describe_via_energy(self):
        energy = OldTex('&\\text{Energy}', '=', '(hf)', '(', '1', ')^2\\\\', '&=', '(hf)', '\\left(', '\\sqrt{1/2}', '\\right)^2', '+', '(hf)', '\\left(', '\\sqrt{1/2}', '\\right)^2')
        energy.scale(0.8)
        ...
        parts = VGroup(*[VGroup(*energy[i1:i2]) for i1, i2 in zip(indices, indices[1:])])
        for part in parts:
            bg_rect = BackgroundRectangle(part)
        ...
        self.play(Transform(self.d_brace.label.copy(), one.copy(), remover=True), Write(parts[1], run_time=1))
        self.wait()
        self.play(Transform(self.h_brace.label[1].copy(), halves[0].copy(), remover=True, rate_func=squish_rate_func(smooth, 0, 0.75)), Transform(self.v_brace.label[1].copy(), halves[1].copy(), remover=True, rate_func=squish_rate_func(smooth, 0.25, 1)), Write(parts[2]), run_time=2)
        self.wait()
    ...
    def components_not_possible_in_isolation(self):
        ...
        self.play(randy.rotate, np.pi / 2, IN, randy.rotate, np.pi / 2, LEFT, randy.change, 'maybe', randy.rotate, np.pi / 2, RIGHT, randy.rotate, np.pi / 2, OUT)
        self.wait()
    ...
    def ask_what_they_mean(self):
        ...
        self.play(FadeIn(morty), component.next_to, morty, DOWN, OUT, component.shift, MED_LARGE_BUFF * (DOWN + OUT))
        ...
        self.play(ShowCreation(bubble), Write(bubble.content), morty.look_at, component)
        self.wait()
    ...
    def change_camera(self):
        ...
        self.play(self.photon, rate_func=lambda x: min(x + 0.55, 1), run_time=2)
        self.photon.rate_func = lambda x: x
        self.play(self.photon)
        self.wait()
...
class ShootPhotonThroughFilter(DirectionOfPolarizationScene):
    CONFIG = {'EMWave_config': {'wave_number': 0, 'A_vect': [0, 1, 1], 'start_point': FRAME_X_RADIUS * LEFT, 'amplitude': np.sqrt(2)}, 'pol_filter_configs': [{'label_tex': '\\text{Filter}', 'include_arrow_label': False}], 'apply_filter': True, 'quantum': True, 'pre_filter_alpha': 0.35, 'ambient_rotation_rate': 0}
    ...
    def add_superposition_tex(self):
        superposition_tex = OldTex('|\\!\\nearrow\\rangle', '=', '(\\sqrt{1/2})', '|\\!\\rightarrow \\rangle', '+', '(\\sqrt{1/2})', '|\\!\\uparrow \\rangle')
    ...
    def ask_what_would_happen(self):
        photon = self.get_photon(rate_func=lambda t: self.pre_filter_alpha * t, remover=False, run_time=0.6)
        ...
        self.pol_filter.add(self.pol_filter.arrow.copy().rotate(np.pi / 2, OUT))
    ...
    def expect_half_energy_to_be_absorbed(self):
        ...
        lines.rotate(np.pi / 2, RIGHT)
        lines.rotate(np.pi / 2, OUT)
        self.move_camera(phi=np.pi / 2, theta=0, added_anims=[Rotate(self.superposition_tex, np.pi / 2)] + [ApplyMethod(v.rotate, -np.pi / 2, method_kwargs={'axis': v.get_vector()}) for v in self.frozen_photon.mobject])
        self.play(Write(words, run_time=2), self.superposition_tex.h_rect.set_stroke, RED, 3, *list(map(GrowFromCenter, lines)) + [Animation(self.pol_filter), Animation(self.frozen_photon.mobject)])
        self.wait(2)
        self.move_camera(phi=0.8 * np.pi / 2, theta=-0.7 * np.pi, added_anims=[FadeOut(words), Animation(lines), Rotate(self.superposition_tex, -np.pi / 2)] + [ApplyMethod(v.rotate, np.pi / 2, method_kwargs={'axis': v.get_vector()}) for v in self.frozen_photon.mobject])
        self.play(FadeOut(lines), FadeOut(self.question), self.superposition_tex.h_rect.fade, 1, Animation(self.pol_filter))
        self.wait()
        self.absorption_words = words

    def probabalistic_passing_and_blocking(self):
        absorption = self.get_filter_absorption_animation(self.pol_filter, self.get_blocked_photon())
        ...
        self.play(self.get_photon(), rate_func=lambda t: min(t + self.pre_filter_alpha, 1))
        ...
        for should_pass in bools:
            if should_pass:
                ...
                self.play(self.get_blocked_photon(), Animation(self.axes), absorption, run_time=1)
        self.play(FadeOut(prob))

    def note_change_in_polarization(self):
        words = OldTexText("``Collapses'' \\\\ from", '$|\\!\\nearrow\\rangle$', 'to', '$|\\!\\uparrow\\rangle$')
...
class YouCanSeeTheCollapse(TeacherStudentsScene):
    ...
    def construct(self):
        self.teacher_says('You can literally \\\\ \\emph{see} the collapse', target_mode='hooray')
        self.play_student_changes('confused', 'hooray', 'erm')
        self.wait(3)

class ThreeFilters(ShootPhotonThroughFilter):
    CONFIG = {'filter_x_coordinates': [-4, 0, 4], 'pol_filter_configs': [{'filter_angle': 0}, {'filter_angle': np.pi / 4}, {'filter_angle': np.pi / 2}], 'EMWave_config': {'A_vect': [0, 0, 1], 'amplitude': 1.5, 'n_vectors': 60}, 'line_start_length': 8, 'line_end_length': 8, 'n_lines': 20, 'lines_depth': 1.8, 'lines_shift_vect': SMALL_BUFF * OUT, 'random_seed': 6}
    ...
    def setup_lines(self):
        lines_group = VGroup(*[self.get_lines(pf1, pf2, ratio) for pf1, pf2, ratio in zip([None] + list(self.pol_filters), list(self.pol_filters) + [None], [1, 1, 0.5, 0.25])])
        lines = lines_group[0]
        spacing = lines[1].get_start() - lines[0].get_start()
        lines.add(lines.copy().shift(spacing / 2))
        self.lines_group = lines_group
        self.A_to_C_lines = self.get_lines(self.pol_filters[0], self.pol_filters[2])
    ...
    def fifty_percent_pass_second(self):
        arrow = Arrow(ORIGIN, 3 * RIGHT, use_rectangular_stem=False, path_arc=-0.8 * np.pi)
        label = OldTex('50\\%')
        ...
        kwargs = {'lag_ratio': 0, 'rate_func': None}
        self.play(ShowCreation(l1, run_time=1, **kwargs))
        self.play(ShowCreation(l2, **kwargs), Animation(VGroup(pf1, l1)), ShowCreation(arrow), run_time=0.5)
        self.play(ShowCreation(l3, **kwargs), Animation(VGroup(pf2, l2, pf1, l1)), FadeIn(label), run_time=0.5)
        self.wait(2)
        self.play(FadeOut(l3), Animation(pf2), FadeOut(l2), Animation(pf1), FadeOut(l1))
        self.fifty_percent_arrow_group = group

    def show_changed_to_diagonal(self):
        photon = self.get_photon(run_time=2, rate_func=lambda x: 0.6 * x, remover=False)
        ...
        kwargs = {'run_time': 3, 'rate_func': there_and_back_with_pause}
        self.move_camera(phi=np.pi / 2, theta=0, added_anims=[Animation(VGroup(*self.pol_filters[:2]))] + [Rotate(v, np.pi / 2, axis=v.get_vector(), in_place=True, **kwargs) for v in photon.mobject] + [Animation(self.pol_filters[2]), Rotate(label, np.pi / 2, axis=OUT, in_place=True, **kwargs)], **kwargs)
        self.wait()
        self.photon = photon
    ...
    def fifty_percent_to_pass_third(self):
        ...
        new_photon = self.get_photon(rate_func=lambda x: (1 - a) * x + a, run_time=1)
        self.revert_to_original_skipping_status()
    ...
    def show_lines_with_middle(self):
        ...
        pf1, pf2, pf3 = self.pol_filters
        self.play(FadeIn(l4), Animation(pf3), FadeIn(l3), Animation(pf2), FadeIn(l2), Animation(pf1), FadeIn(l1), FadeOut(self.brace_group))
        self.wait(2)
    ...
    def remove_middle_then_put_back(self):
        ...
        mover = VGroup(pf2, self.fifty_percent_arrow_group, self.second_fifty_percent_arrow_group)
        arrow = Arrow(ORIGIN, 7 * RIGHT, path_arc=0.5 * np.pi)
        ...
        self.remove(l2, l3)
        self.play(FadeOut(l4), Animation(pf3), FadeOut(l3), ApplyMethod(mover.shift, 3 * OUT, rate_func=running_start), ReplacementTransform(l2.copy(), mid_lines), Animation(pf1), Animation(l1))
        self.play(ShowCreation(arrow), Write(labels[0], run_time=1))
        self.wait(2)
        self.play(FadeIn(l4), Animation(pf3), FadeOut(mid_lines), FadeIn(l3), mover.shift, 3 * IN, FadeIn(l2), Animation(pf1), Animation(l1))
...
class PhotonAtSlightAngle(ThreeFilters):
    CONFIG = {'filter_x_coordinates': [3], 'pol_filter_configs': [{'label_tex': '', 'include_arrow_label': False, 'radius': 1.4}], 'EMWave_config': {'wave_number': 0, 'A_vect': [0, np.sin(np.pi / 8), np.cos(np.pi / 8)], 'start_point': FRAME_X_RADIUS * LEFT, 'amplitude': 2}, 'axes_config': {'z_max': 2.5}, 'radius': 1.3, 'lines_depth': 2.5, 'line_start_length': 12}
    ...
    def shoot_photon(self):
        photon = self.get_photon(rate_func=lambda x: 0.5 * x, remover=False)
        self.play(photon)
        self.photon = photon

    def reposition_camera_to_head_on(self):
        self.move_camera(phi=np.pi / 2, theta=0, added_anims=list(it.chain(*[[v.rotate, np.pi / 2, v.get_vector(), v.set_fill, None, 0.7] for v in self.photon.mobject])) + [Animation(self.pol_filter)])

    def write_angle(self):
        arc = Arc(start_angle=np.pi / 2, angle=-np.pi / 8, radius=self.pol_filter.radius)
        label = OldTex('22.5^\\circ')
        label.next_to(arc.get_center(), UP + RIGHT, SMALL_BUFF)
        group = VGroup(arc, label)
        group.rotate(np.pi / 2, RIGHT)
        group.rotate(np.pi / 2, OUT)
        self.play(FadeOut(self.pol_filter), ShowCreation(arc), Write(label, run_time=1))
        self.wait()
        self.arc = arc
    ...
    def write_components(self):
        ...
        h_arrow = Vector(self.radius * np.sin(np.pi / 8) * RIGHT, color=RED)
        ...
        state = OldTex('|\\!\\psi\\rangle', '=', '\\sin(22.5^\\circ)', '|\\!\\rightarrow\\rangle', '+', '\\cos(22.5^\\circ)', '|\\!\\uparrow\\rangle')
        state.set_color_by_tex_to_color_map({'psi': BLUE, 'rightarrow': RED, 'uparrow': GREEN})
        state.to_edge(UP)
        ...
        group = VGroup(d_brace, d_brace.label, h_arrow, h_label, v_arrow, v_label, state, sin_brace, sin_brace.label, cos_brace, cos_brace.label)
        group.rotate(np.pi / 2, RIGHT)
        group.rotate(np.pi / 2, OUT)
        self.play(GrowFromCenter(d_brace), Write(d_brace.label))
        self.wait()
        self.play(GrowFromPoint(h_arrow, ORIGIN), Write(h_label, run_time=1))
        self.play(Write(VGroup(*state[:2])), ReplacementTransform(h_label.copy(), state.get_part_by_tex('sin')), ReplacementTransform(h_arrow.copy(), state.get_part_by_tex('rightarrow')), Write(state.get_part_by_tex('+')))
        self.play(GrowFromCenter(sin_brace), Write(sin_brace.label, run_time=1))
        self.wait()
        self.play(GrowFromPoint(v_arrow, h_arrow.get_end()), Write(v_label, run_time=1))
        self.play(ReplacementTransform(v_label.copy(), state.get_part_by_tex('cos')), ReplacementTransform(v_arrow.copy(), state.get_part_by_tex('uparrow')))
        self.play(GrowFromCenter(cos_brace), Write(cos_brace.label, run_time=1))
        self.wait()
        self.d_brace = d_brace
        self.state_equation = state
        self.state_equation.add(sin_brace, sin_brace.label, cos_brace, cos_brace.label)
    ...
    def classical_energy_conception(self):
        ...
        h_content = OldTex('0.38', '^2', '= 0.15', '\\text{ energy}\\\\', '\\text{in the }', '\\rightarrow', '\\text{ direction}')
        alt_h_content = OldTex('0.38', '^2', '=& 15\\%', '\\text{ of energy}\\\\', '&\\text{absorbed}', '', '')
        h_content.set_color_by_tex('rightarrow', RED)
        alt_h_content.set_color_by_tex('rightarrow', RED)
        alt_h_content.scale(0.8)
        v_content = OldTex('0.92', '^2', '= 0.85', '\\text{ energy}\\\\', '\\text{in the }', '\\uparrow', '\\text{ direction}')
        ...
        self.play(FadeIn(randy), FadeIn(classically), ShowCreation(bubble))
        self.play(ReplacementTransform(self.sin_brace.label.copy(), h_content[0]), ReplacementTransform(self.state_equation.get_part_by_tex('rightarrow').copy(), h_content.get_part_by_tex('rightarrow')))
        self.play(Write(VGroup(*h_content[1:5])), Write(h_content.get_part_by_tex('direction')), run_time=2)
        self.wait(2)
        self.play(h_content.shift, 2 * IN)
        self.play(ReplacementTransform(self.cos_brace.label.copy(), v_content[0]), ReplacementTransform(self.state_equation.get_part_by_tex('uparrow').copy(), v_content.get_part_by_tex('uparrow')))
        self.play(Write(VGroup(*v_content[1:5])), Write(v_content.get_part_by_tex('direction')), run_time=2)
        self.wait(2)
        self.play(FadeOut(randy), FadeOut(bubble), FadeOut(v_content), Transform(h_content, alt_h_content), FadeIn(self.pol_filter), Animation(self.arc))
    ...
    def reposition_camera_back(self):
        self.move_camera(phi=0.8 * np.pi / 2, theta=-0.6 * np.pi, added_anims=[FadeOut(self.h_arrow), FadeOut(self.h_label), FadeOut(self.v_arrow), FadeOut(self.v_label), FadeOut(self.d_brace), FadeOut(self.d_brace.label), FadeOut(self.arc), FadeOut(self.angle_label), Rotate(self.state_equation, np.pi / 2, IN), Rotate(self.classically, np.pi / 2, IN)] + [Rotate(v, np.pi / 2, axis=v.get_vector(), in_place=True) for v in self.photon.mobject], run_time=1.5)
    ...
    def rewrite_15_percent_meaning(self):
        ...
        new_conception = OldTexText('$0.38^2 = 15\\%$ chance of \\\\ getting blocked')
        new_conception.scale(0.8)
        new_conception.rotate(np.pi / 2, RIGHT)
        new_conception.move_to(self.classically, OUT)
        a = self.photon.rate_func(1)
        finish_photon = self.get_blocked_photon(rate_func=lambda t: a + (1 - a) * t)
        finish_photon.mobject.set_fill(opacity=0.7)
        self.play(ShowCreation(cross))
        self.classically.add(cross)
        self.play(self.classically.shift, 4 * IN, FadeIn(new_conception))
        self.remove(self.photon.mobject)
        self.revert_to_original_skipping_status()
        self.play(finish_photon, ApplyMethod(self.pol_filter.set_color, RED, rate_func=squish_rate_func(there_and_back, 0, 0.3), run_time=finish_photon.run_time))
    ...
    def probabalistic_passing(self):
        ...
        arrow = Arrow(2 * LEFT, 2 * RIGHT, path_arc=0.8 * np.pi)
        ...
        self.play(ShowCreation(arrow), Write(label, run_time=1), ShowCreation(l1, **kwargs))
        self.play(ShowCreation(l2, run_time=0.5, **kwargs), Animation(self.pol_filter), Animation(l1))
        self.wait()
    ...
    def get_blocked_photon(self, **kwargs):
        return self.get_photon(filter_distance=FRAME_X_RADIUS + 3, get_filtered=True, **kwargs)
...
class CompareWaveEquations(TeacherStudentsScene):
    ...
    def add_equation(self):
        equation = OldTex('|\\!\\psi\\rangle', '=', '\\alpha', '|\\!\\rightarrow\\rangle', '+', '\\beta', '|\\!\\uparrow\\rangle')
        equation.set_color_by_tex_to_color_map({'psi': BLUE, 'rightarrow': RED, 'uparrow': GREEN})
        ...
        self.play(equation.restore, self.teacher.change, 'raise_right_hand')
        self.play_student_changes(*['pondering'] * 3, look_at=psi_rect, added_anims=[ShowCreation(psi_rect), Write(state_words, run_time=1)], run_time=1)
        self.play(FadeOut(psi_rect))
        self.equation = equation
        self.state_words = state_words

    def show_complex_plane(self):
        new_alpha, new_beta = terms = [self.equation.get_part_by_tex(tex).copy() for tex in ('alpha', 'beta')]
        for term in terms:
            term.save_state()
            term.generate_target()
            term.target.scale(0.7)
        plane = ComplexPlane(x_radius=1.5, y_radius=1.5)
        plane.add_coordinates()
        plane.scale(1.3)
        plane.next_to(ORIGIN, RIGHT, MED_LARGE_BUFF)
        plane.to_edge(UP)
        alpha_dot, beta_dot = [Dot(plane.coords_to_point(x, 0.5), radius=0.05, color=color) for x, color in [(-0.5, RED), (0.5, GREEN)]]
        ...
        rhs = OldTex('=', 'A_y', 'e', '^{i(', '2\\pi', 'f', 't', '+', '\\phi_y', ')}')
        rhs.scale(0.7)
        rhs.next_to(new_beta.target, RIGHT, SMALL_BUFF)
        rhs.shift(0.5 * SMALL_BUFF * UP)
        rhs.set_color_by_tex_to_color_map({'A_y': GREEN, 'phi': MAROON_B})
        A_copy = rhs.get_part_by_tex('A_y').copy()
        phi_copy = rhs.get_part_by_tex('phi_y').copy()
        A_line = Line(plane.coords_to_point(0, 0), plane.coords_to_point(0.5, 0.5), color=GREEN, stroke_width=2)
        arc = Arc(angle=np.pi / 4, radius=0.5)
        arc.shift(plane.get_center())
        self.play(Write(plane, run_time=2), MoveToTarget(new_alpha), MoveToTarget(new_beta), DrawBorderThenFill(alpha_dot, run_time=1), DrawBorderThenFill(beta_dot, run_time=1))
        self.play(Write(rhs), ShowCreation(A_line), ShowCreation(arc))
        self.play(phi_copy.next_to, arc, RIGHT, SMALL_BUFF, phi_copy.shift, 0.5 * SMALL_BUFF * UP)
        self.play(A_copy.next_to, A_line.get_center(), UP, SMALL_BUFF, A_copy.shift, 0.5 * SMALL_BUFF * (UP + LEFT))
        self.wait()

    def show_interpretations(self):
        c_words = OldTex('\\text{Classically: }', '&|\\beta|^2', '\\rightarrow', '\\text{Component of} \\\\', '&\\text{energy in }', '|\\!\\uparrow\\rangle', '\\text{ direction}')
        qm_words = OldTex('\\text{Quantum: }', '&|\\beta|^2', '\\rightarrow', '\\text{Probability that}', '\\text{ \\emph{all}} \\\\', '&\\text{energy is measured in }', '|\\!\\uparrow\\rangle', '\\text{ direction}')
        for words in (c_words, qm_words):
            words.set_color_by_tex_to_color_map({'Classically': YELLOW, 'Quantum': BLUE, '{all}': BLUE, 'beta': GREEN, 'uparrow': GREEN})
            words.scale(0.7)
        c_words.to_edge(LEFT)
        c_words.shift(2 * UP)
        qm_words.next_to(c_words, DOWN, MED_LARGE_BUFF, LEFT)
        self.play(FadeOut(self.state_words), Write(c_words), self.teacher.change, 'happy')
        self.play_student_changes(*['happy'] * 3, look_at=c_words)
        self.play(Write(qm_words))
        self.play_student_changes('erm', 'confused', 'pondering', look_at=qm_words)
        self.wait()

class CircularPhotons(ShootPhotonThroughFilter):
    CONFIG = {'EMWave_config': {'phi_vect': [0, -np.pi / 2, 0], 'wave_number': 1, 'start_point': 10 * LEFT, 'length': 20, 'n_vectors': 60}, 'apply_filter': False}
    ...
    def setup_filter(self):
        ...
        arrows = VGroup(*[Arrow(v1, v2, color=WHITE, path_arc=np.pi) for v1, v2 in [(LEFT, RIGHT), (RIGHT, LEFT)]])
        arrows.scale(0.7)
    ...
    def show_phase_difference(self):
        equation = OldTex('|\\!\\circlearrowright\\rangle', '=', '\\frac{1}{\\sqrt{2}}', '|\\!\\rightarrow\\rangle', '+', '\\frac{i}{\\sqrt{2}}', '|\\!\\uparrow\\rangle')
        equation.set_color_by_tex_to_color_map({'circlearrowright': BLUE, 'rightarrow': RED, 'uparrow': GREEN})
    ...
    def show_filter(self):
        ...
        self.play(self.get_circular_photon(), Animation(self.circ_equation_group))
        self.play(FadeOut(self.circ_equation_group))

    def show_vertically_polarized_light(self):
        equation = OldTex('|\\!\\uparrow \\rangle', '=', '\\frac{i}{\\sqrt{2}}', '|\\!\\circlearrowleft \\rangle', '+', '\\frac{-i}{\\sqrt{2}}', '|\\!\\circlearrowright \\rangle')
        equation.set_color_by_tex_to_color_map({'circlearrowright': BLUE, 'frac{-i}': BLUE, 'circlearrowleft': YELLOW, 'frac{i}': YELLOW, 'uparrow': GREEN})
        equation.next_to(ORIGIN, LEFT, LARGE_BUFF)
        equation.to_edge(UP)
        prob = OldTex('P(', '\\text{passing}', ')', '=', '\\left(', '\\frac{-i}{\\sqrt{2}}', '\\right)^2')
        prob.set_color_by_tex('sqrt{2}', BLUE)
        prob.next_to(equation, DOWN)
        group = VGroup(equation, prob)
        group.rotate(np.pi / 2, RIGHT)
        group.rotate(np.pi / 4, IN)
        em_wave = EMWave(wave_number=0, amplitude=2, start_point=10 * LEFT, length=20)
        v_photon = WavePacket(em_wave=em_wave, include_M_vects=False, run_time=2)
        ...
        for should_pass in bools:
            if should_pass:
                ...
                self.play(blocked_v_photon, self.get_filter_absorption_animation(self.pol_filter, blocked_v_photon))
        self.wait()
...
class ClockwisePhotonInsert(Scene):
    ...
    def construct(self):
        eq = OldTex('\\left| \\frac{-i}{\\sqrt{2}} \\right|^2')
        eq.set_color(BLUE)
