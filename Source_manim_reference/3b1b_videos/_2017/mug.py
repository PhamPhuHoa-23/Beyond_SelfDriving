...
class HappyHolidays(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play_student_changes(*['hooray'] * 3, look_at=FRAME_Y_RADIUS * UP, added_anims=[self.teacher.change, 'hooray'])
        self.play(LaggedStartMap(DrawBorderThenFill, hats), Animation(self.pi_creatures))
        self.play_student_changes('happy', 'wave_2', 'wave_1', look_at=FRAME_Y_RADIUS * UP)
    ...
    def get_snowflakes(self, n_flakes=50):
        snowflakes = VGroup(*[SVGMobject(file_name='snowflake', height=0.5, stroke_width=0, fill_opacity=0.75, fill_color=WHITE).rotate(np.pi / 12, RIGHT) for x in range(n_flakes)])

        def random_confetti_spiral(mob, **kwargs):
            return ConfettiSpiril(mob, x_start=2 * random.random() * FRAME_X_RADIUS - FRAME_X_RADIUS, **kwargs)
        snowflake_spirils = LaggedStartMap(random_confetti_spiral, snowflakes, run_time=10, rate_func=lambda x: x)
        return turn_animation_into_updater(snowflake_spirils)

class UtilitiesPuzzleScene(Scene):
    CONFIG = {'object_height': 0.75, 'h_distance': 2, 'v_distance': 2, 'line_width': 4}
    ...
    def setup_configuration(self):
        ...
        utilities = VGroup(*[self.get_utility(u, c).move_to(x * self.h_distance * RIGHT) for x, u, c in zip(it.count(), ['fire', 'electricity', 'water'], [RED_D, YELLOW_C, BLUE_D])])
        utilities.move_to(self.v_distance * DOWN / 2)
        objects = VGroup(houses, utilities)
        bounding_box = SurroundingRectangle(objects, buff=MED_LARGE_BUFF, stroke_width=0)
    ...
    def get_utility(self, name, color):
        circle = Circle(fill_color=color, fill_opacity=1, stroke_width=0)
        utility = SVGMobject(file_name=name, height=0.65 * circle.get_height(), fill_color=WHITE)
    ...
    def get_line(self, utility_index, house_index, *midpoints, **kwargs):
        ...
        line = Line(points[0], points[-1], color=utility[0].get_color(), stroke_width=self.line_width)
    ...
    def get_almost_solution_lines(self):
        ...
        return VGroup(VGroup(self.get_line(0, 0), self.get_line(0, 1), self.get_line(0, 2, bb.get_top())), VGroup(self.get_line(1, 0, bb.get_corner(DOWN + LEFT)), self.get_line(1, 1), self.get_line(1, 2, bb.get_corner(DOWN + RIGHT))), VGroup(self.get_line(2, 0, bb.get_top()), self.get_line(2, 1), self.get_line(2, 2)))

    def get_straight_lines(self):
        return VGroup(*[VGroup(*[self.get_line(i, j) for j in range(3)]) for i in range(3)])
...
class AboutToyPuzzles(UtilitiesPuzzleScene, TeacherStudentsScene, ThreeDScene):
    ...
    def lay_out_puzzle(self):
        ...
        self.play(ReplacementTransform(VGroup(houses[1], houses[1]).copy().fade(1), VGroup(houses[0], houses[2]), rate_func=squish_rate_func(smooth, 0.35, 1), run_time=2), FadeIn(houses[1]), LaggedStartMap(DrawBorderThenFill, utilities, run_time=2))
        self.play(LaggedStartMap(ShowCreation, lines, run_time=3), Animation(self.objects))
        self.play(Write(no_crossing_words[0]), GrowArrow(no_crossing_words[1]))
        self.wait()
    ...
    def point_to_abstractions(self):
        ...
        objects.target.move_to((FRAME_Y_RADIUS * DOWN + FRAME_X_RADIUS * LEFT) / 2)
        eulers = OldTex(*'V-E+F=2')
        eulers.set_color_by_tex_to_color_map({'V': RED, 'E': GREEN, 'F': BLUE})
        ...
        arrow_from_eulers = Arrow(eulers.get_bottom(), tda.get_corner(UP + LEFT), color=WHITE)
        self.play(objects.scale, 0.5, objects.move_to, DOWN + 4 * LEFT)
        arrow_to_eulers = Arrow(self.houses[2].get_corner(UP + LEFT), eulers.get_bottom(), color=WHITE)
        ...
        self.wait(2)
        self.set_variables_as_attrs(eulers, cube, tda, arrows=VGroup(arrow_to_eulers, arrow_from_eulers))

    def show_this_video(self):
        screen_rect = FullScreenFadeRectangle(stroke_color=WHITE, stroke_width=2, fill_opacity=0)
        everything = VGroup(self.objects, self.arrows, self.eulers, self.cube, self.tda, screen_rect)
        self.teacher.save_state()
        self.teacher.fade(1)
        self.play(everything.scale, 0.5, everything.next_to, self.teacher, UP + LEFT, self.teacher.restore, self.teacher.change, 'raise_right_hand', UP + LEFT, LaggedStartMap(FadeIn, self.students))
        self.play_student_changes(*['pondering'] * 3, look_at=everything)
        self.wait(5)
...
class ThisPuzzleIsHard(UtilitiesPuzzleScene, PiCreatureScene):
    ...
    def introduce_components(self):
        ...
        self.play(LaggedStartMap(DrawBorderThenFill, houses), LaggedStartMap(GrowFromCenter, utilities), try_it.set_width, house.get_width(), try_it.fade, 1, try_it.move_to, house, self.pi_creature.change, 'happy')
    ...
    def failed_attempts(self):
        ...
        houses = self.houses
        randy = self.pi_creature
        line_sets = [[self.get_line(0, 0), self.get_line(1, 1), self.get_line(2, 2), self.get_line(0, 1), self.get_line(2, 1), self.get_line(0, 2, bb.get_corner(UP + LEFT)), self.get_line(2, 0, bb.get_corner(DOWN + LEFT), prop=0.85), self.get_line(2, 0, bb.get_corner(UP + RIGHT), bb.get_top(), prop=0.73)], [self.get_line(0, 0), self.get_line(1, 1), self.get_line(2, 2), self.get_line(0, 1), self.get_line(1, 2), self.get_line(2, 0, bb.get_bottom(), bb.get_corner(DOWN + LEFT)), self.get_line(0, 2, bb.get_top()), self.get_line(2, 1, utilities[1].get_bottom() + MED_SMALL_BUFF * DOWN, utilities[1].get_left() + MED_SMALL_BUFF * LEFT), self.get_line(1, 0, houses[2].get_corner(UP + LEFT) + MED_LARGE_BUFF * LEFT, prop=0.49), self.get_line(1, 2, bb.get_right(), prop=0.25)], [self.get_line(0, 0), self.get_line(0, 1), self.get_line(0, 2, bb.get_top()), self.get_line(1, 0, bb.get_corner(DOWN + LEFT)), self.get_line(1, 1), self.get_line(1, 2, bb.get_corner(DOWN + RIGHT)), self.get_line(2, 1), self.get_line(2, 2), self.get_line(2, 0, bb.get_top(), prop=0.45), self.get_line(2, 0, prop=0.45)]]
        ...
        for line_set, mode in zip(line_sets, modes):
            ...
            for bl in bad_lines:
                self.play(ShowCreation(bl, rate_func=bezier([0, 0, 0, 1, 1, 1, 1, 1, 0.3, 1, 1]), run_time=1.5), randy.change, mode)
                self.play(ShowCreation(bl, rate_func=lambda t: smooth(1 - t)))
                self.remove(bl)
            self.play(LaggedStartMap(FadeOut, good_lines))

    def ask_meta_puzzle(self):
        randy = self.pi_creature
        group = VGroup(self.puzzle_words, self.objects)
        ...
        self.play(LaggedStartMap(ShowCreation, straight_lines, run_time=2, lag_ratio=0.8), Blink(randy))
        self.play(Transform(straight_lines, almost_solution_lines, run_time=3, lag_ratio=0.5))
        self.wait()
...
class IntroduceGraph(PiCreatureScene):
    ...
    def construct(self):
        ...
        dots = VGroup(*[Dot(color=pi.get_color()).scale(2).move_to(pi) for pi in pi_creatures])
        lines = VGroup(*[Line(pi1.get_center(), pi2.get_center()) for pi1, pi2 in it.combinations(pi_creatures, 2)])
        ...
        vertex_arrows = VGroup(*[Arrow(vertices_word.get_left(), dot) for dot in dots[-2:]])
        ...
        self.play(LaggedStartMap(ShowCreation, lines), LaggedStartMap(ApplyMethod, pi_creatures, lambda pi: (pi.change, 'pondering', lines)))
        self.play(Write(graph_word))
        self.play(ReplacementTransform(pi_creatures, dots, run_time=2, lag_ratio=0.5))
        self.add_foreground_mobjects(dots)
        self.play(FadeIn(vertex_arrows), FadeIn(vertices_word))
        self.wait()
        self.play(LaggedStartMap(ApplyMethod, lines, lambda l: (l.rotate, np.pi / 12), rate_func=wiggle))
        self.play(FadeIn(edge_word), GrowArrow(edge_arrow))
        self.wait(2)
        line = lines[2]
        self.play(line.set_points_smoothly, [line.get_start(), dots.get_left() + MED_SMALL_BUFF * LEFT, dots.get_corner(DOWN + LEFT) + MED_SMALL_BUFF * (DOWN + LEFT), dots.get_bottom() + MED_SMALL_BUFF * DOWN, line.get_end()], VGroup(edge_word, edge_arrow).shift, MED_LARGE_BUFF * LEFT)
        self.wait()
        self.play(ReplacementTransform(graph_word, planar_graph_word))
        self.wait(2)

    def create_pi_creatures(self):
        pis = VGroup(PiCreature(color=BLUE_D), PiCreature(color=GREY_BROWN), PiCreature(color=BLUE_C).flip(), PiCreature(color=BLUE_E).flip())
        pis.scale(0.5)
...
class IsK33Planar(UtilitiesPuzzleScene):
    ...
    def construct(self):
        ...
        fancy_name = brace.get_text("``Complete bipartite graph $K_{3, 3}$''", buff=SMALL_BUFF)
        fancy_name.set_color(YELLOW)
        self.add(question)
        self.convert_objects_to_dots()
        self.play(LaggedStartMap(ShowCreation, straight_lines))
        self.play(GrowFromCenter(brace), LaggedStartMap(FadeIn, fancy_name))
        self.play(ReplacementTransform(straight_lines, almost_solution_lines, run_time=3, lag_ratio=0.5))
        self.wait(2)
...
class TwoKindsOfViewers(PiCreatureScene, UtilitiesPuzzleScene):
    ...
    def construct(self):
        ...
        words = OldTexText('$(V-E+F)$', 'kinds of viewers')
        words.to_edge(UP)
        ...
        self.add(words)
        self.wait()
        self.play(pi1.shift, 2 * LEFT, pi2.shift, 2 * RIGHT)
        ...
        dont.set_color(RED)
        self.play(FadeIn(know_eulers), pi1.change, 'hooray')
        self.play(FadeIn(dont), pi2.change, 'maybe', eulers)
        self.wait()
        self.pi_creature_thinks(pi1, '', bubble_config={'width': 3, 'height': 2}, target_mode='thinking')
        self.play(pi2.change, 'confused', eulers)
        self.wait()
        self.play(*list(map(FadeOut, [non_eulers, pi1, pi2, pi1.bubble, know_eulers, dont])))
        self.play(eulers.next_to, ORIGIN, LEFT, LARGE_BUFF)
        arrow = Arrow(eulers, objects, color=WHITE)
        self.play(GrowArrow(arrow), LaggedStartMap(DrawBorderThenFill, VGroup(*it.chain(*objects))))
        self.wait()
        self.play(objects.move_to, eulers, RIGHT, eulers.move_to, objects, LEFT, path_arc=np.pi, run_time=1.5)
        self.wait(2)
...
class IntroduceRegions(UtilitiesPuzzleScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(back_region), Animation(front_regions))
        self.wait()
        self.play(FadeOut(regions))
        paint_bucket = SVGMobject(file_name='paint_bucket', height=0.5)
        paint_bucket.flip()
        paint_bucket.move_to(8 * LEFT + 5 * UP)

        def click(region):
            self.play(UpdateFromAlphaFunc(region, lambda m, a: m.set_fill(opacity=int(2 * a))), ApplyMethod(paint_bucket.scale, 0.5, rate_func=there_and_back), run_time=0.25)
        ...
        self.wait()
        self.play(FadeOut(back_region), FadeOut(front_regions[0]), FadeOut(paint_bucket), *list(map(Animation, front_regions[1:])))
        point_sets = [[VGroup(*houses[1:]).get_center(), houses[2].get_top() + MED_SMALL_BUFF * UP], [houses[1].get_top() + SMALL_BUFF * UP, utilities[0].get_center()], [VGroup(houses[1], utilities[1]).get_center()], [utilities[2].get_center() + 0.75 * (DOWN + RIGHT)]]
        escape_lines = VGroup(*[Line(LEFT, RIGHT).set_points_smoothly([utilities[2].get_center()] + point_set) for point_set in point_sets])
        self.wait()
    ...
    def get_lines_line_groups_and_regions(self):
        ...
        line_groups = [VGroup(*[lines[i][j] for i, j in ij_set]) for ij_set in [[(0, 0), (1, 0), (1, 1), (0, 1)], [(1, 1), (2, 1), (2, 2), (1, 2)], [(0, 2), (2, 2), (2, 1), (0, 1)], [(0, 0), (1, 0), (1, 2), (0, 2)]]]
        regions = VGroup(*[self.get_region(*line_group) for line_group in line_groups])
...
class AskAboutRegions(IntroduceRegions):
    ...
    def construct(self):
        ...
        back_region = regions[0]
        front_regions = VGroup(*regions[1:])
        missing_lines = VGroup(self.get_line(2, 0, self.objects.get_top()), self.get_line(2, 0, self.objects.get_bottom() + DOWN, self.objects.get_corner(DOWN + LEFT) + DOWN + LEFT))
        ...
        self.play(LaggedStartMap(ApplyMethod, front_regions, lambda m: (m.rotate, np.pi / 12), rate_func=wiggle, lag_ratio=0.75, run_time=1))
        self.play(front_regions.restore)
        self.wait()
        for line in missing_lines:
            self.play(ShowCreation(line, rate_func=there_and_back, run_time=2))
        count = OldTex('1')
        count.scale(1.5)
        count.to_edge(UP)
        ...
        def show_lines(line_group):
            ...
            self.play(LaggedStartMap(FadeIn, lg_copy, run_time=3, rate_func=there_and_back, lag_ratio=0.4, remover=True))
        self.play(FadeIn(fade_rect), Animation(region), Animation(line_group))
        ...
        for i in range(1, 3):
            ...
            self.play(FadeOut(last_region), FadeOut(last_line_group), FadeIn(region), FadeIn(line_group))
            show_lines(line_group)
            last_line_group = line_group
            last_region = region
        self.play(FadeOut(fade_rect), FadeOut(last_region), FadeOut(last_line_group))
        self.wait()
...
class NewRegionClosedOnlyForNodesWithEdges(UtilitiesPuzzleScene):
    ...
    def construct(self):
        ...
        houses, utilities = (self.houses, self.utilities)
        bb = self.bounding_box
        lines = VGroup(self.get_line(2, 1), self.get_line(0, 1), self.get_line(0, 2, bb.get_corner(UP + LEFT), bb.get_top() + MED_LARGE_BUFF * UP), self.get_line(2, 2))
        ...
        self.wait()
        self.remove(region)
        self.play(ShowCreation(lines[-1], rate_func=lambda t: smooth(1 - 2 * t * (1 - t))))
        self.add(region, lines, objects)
        self.wait()
        self.remove(region)
        self.play(ShowCreation(lines[-1], rate_func=lambda t: smooth(1 - 0.5 * t)), FadeIn(words), GrowArrow(arrow))
        for x in range(2):
...
class LightUpNodes(IntroduceRegions):
    CONFIG = {'vertices_word': 'Lit vertices'}
    ...
    def setup_regions(self):
        ...
        back_region = regions[0]
        front_regions = VGroup(*regions[1:])
        self.set_variables_as_attrs(lines, line_groups, regions, back_region, front_regions)

    def setup_counters(self):
        titles = [OldTexText('\\# %s' % self.vertices_word), OldTexText('\\# Edges'), OldTexText('\\# Regions')]
        ...
        self.v_count, self.e_count, self.f_count = self.counts = list(map(Integer, [1, 0, 1]))
    ...
    def describe_one_as_lit(self):
        ...
        self.play(vertices.set_fill, None, 0, vertices.set_stroke, None, 1)
        ...
        self.wait()
        self.play(*list(map(FadeOut, [lit_word, lit_arrow, dim_word, dim_arrows])))
    ...
    def show_rule_for_lighting(self):
        ...
        self.play(*it.chain(self.get_lit_vertex_animations(houses[0]), self.get_count_change_animations(1, 0, 0)))
        self.wait()
        for line, vertex in ((lines[1], houses[1]), (lines[4], utilities[1])):
            self.play(ShowCreation(line), *self.get_count_change_animations(0, 1, 0))
            self.play(*it.chain(self.get_lit_vertex_animations(vertex), self.get_count_change_animations(1, 0, 0)))
        self.wait()
        self.play(ShowCreation(lines[3], run_time=2), *self.get_count_change_animations(0, 1, 0))
        ...
        for line, vertex in ((lines[2], houses[2]), (lines[6], utilities[2])):
            self.play(ShowCreation(line), *it.chain(self.get_lit_vertex_animations(vertex, run_time=2, squish_range=(0.5, 1)), self.get_count_change_animations(1, 1, 0)))
        self.wait()
        self.play(ShowCreation(lines[7]), *self.get_count_change_animations(0, 1, 1))
    ...
    def get_count_change_animations(self, *changes):
        ...
        for change, count in zip(changes, self.counts):
            ...
            anims.append(Transform(count, new_count, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)))
            count.number += 1
    ...
    def get_plus_one_anim(self, count):
        ...
        return UpdateFromAlphaFunc(plus_one, lambda m, a: (grow if a < 0.5 else move).update(2 * a % 1), remover=True, rate_func=double_smooth, run_time=2)

    def get_lit_vertex_animations(self, vertex, run_time=1, squish_range=(0, 1)):
        line = Line(LEFT, RIGHT, stroke_width=0, stroke_color=BLACK)
        line.set_width(0.5 * vertex.get_width())
        line.next_to(ORIGIN, buff=0.75 * vertex.get_width())
        lines = VGroup(*[line.copy().rotate(angle) for angle in np.arange(0, 2 * np.pi, np.pi / 4)])
        lines.move_to(vertex)
        random.shuffle(lines.submobjects)
        return [LaggedStartMap(ApplyMethod, lines, lambda l: (l.set_stroke, YELLOW, 4), rate_func=squish_rate_func(there_and_back, *squish_range), lag_ratio=0.75, remover=True, run_time=run_time), ApplyMethod(vertex.set_fill, None, 1, run_time=run_time, rate_func=squish_rate_func(smooth, *squish_range))]
...
class ShowRule(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        arrows = VGroup(*[Arrow(new_edge.get_right(), mob.get_left(), color=WHITE, buff=SMALL_BUFF) for mob in (new_vertex, new_region)])
        ...
        self.play_student_changes(*['confused'] * 3, look_at=rule)
        self.wait(2)
        self.play(Write(nine_total), self.teacher.change, 'happy')
        self.play_student_changes(*['thinking'] * 3, look_at=rule)
        self.wait(3)
...
class ConcludeFiveRegions(LightUpNodes):
    ...
    def describe_start_setup(self):
        ...
        full_screen_rect = FullScreenFadeRectangle(fill_color=GREY_B, fill_opacity=0.25)
        self.play(Indicate(self.v_count), *self.get_lit_vertex_animations(self.utilities[0]))
        self.play(FadeIn(full_screen_rect, rate_func=there_and_back, remover=True), Indicate(self.f_count), *list(map(Animation, self.mobjects)))
        self.wait()
    ...
    def show_five_to_lit_up_nodes(self):
        ...
        for li, vi in zip(line_indices, vertex_indices):
            ...
            self.play(*it.chain(self.get_count_change_animations(1, 1, 0), self.get_lit_vertex_animations(vertex)))

    def relate_four_lines_to_regions(self):
        f_rect = SurroundingRectangle(VGroup(self.count_titles[-1], self.f_count))
    ...
    def conclude_about_five_regions(self):
        ...
        for region, line_group, li in zip(regions, line_groups, line_indices):
            ...
            self.play(FadeIn(region), Animation(line_group), Animation(objects), *self.get_count_change_animations(0, 1, 1))
        self.wait()
...
class WhatsWrongWithFive(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says("What's wrong with \\\\ 5 regions?", target_mode='maybe')
        self.wait(2)
...
class CyclesHaveAtLeastFour(UtilitiesPuzzleScene):
    ...
    def construct(self):
        self.setup_configuration()
        houses, utilities = (self.houses, self.utilities)
        vertices = VGroup(houses[0], utilities[0], houses[1], utilities[1], houses[0])
        lines = [VectorizedPoint(), self.get_line(0, 0), self.get_line(0, 1), self.get_line(1, 1), self.get_line(1, 0, self.objects.get_corner(DOWN + LEFT))]
        ...
        for vertex in vertices:
            ...
            vertex.arrow = arrow
            arrows.add(arrow)
        word_strings = ['Start at a house', 'Go to a utility', 'Go to another house', 'Go to another utility', 'Back to the start']
        words = VGroup()
        ...
        for line, word, arrow in zip(lines, words, arrows):
            ...
            new_count = Integer(count.number + 1)
            new_count.move_to(count)
            anims += [FadeIn(word), GrowArrow(arrow), ShowCreation(line), FadeOut(count), FadeIn(new_count)]
            self.play(*anims)
            self.wait()
            last_word = word
            last_arrow = arrow
...
class FiveRegionsFourEdgesEachGraph(Scene):
    CONFIG = {'v_color': WHITE, 'e_color': YELLOW, 'f_colors': (BLUE, RED_E, BLUE_E), 'n_edge_double_count_examples': 6, 'random_seed': 1}
    ...
    def draw_squares(self):
        words = VGroup(OldTexText('5', 'regions'), OldTexText('4', 'edges each'))
        words.arrange(DOWN)
        words.to_edge(UP)
        ...
        for square in squares:
            ...
            square.edges = VGroup(*[Line(c1, c2, color=self.e_color) for c1, c2 in adjacent_pairs(corners)])
            square.vertices = VGroup(*[Dot(color=self.v_color).move_to(c) for c in corners])
            all_edges.add(*square.edges)
            all_vertices.add(*square.vertices)
        self.play(FadeIn(words[0]), LaggedStartMap(FadeIn, squares, run_time=1.5))
        self.play(FadeIn(words[1]), LaggedStartMap(ShowCreation, all_edges), LaggedStartMap(GrowFromCenter, all_vertices))
        self.wait()
    ...
    def transition_to_graph(self):
        ...
        words = self.words
        points = np.array([UP + LEFT, UP + RIGHT, DOWN + RIGHT, DOWN + LEFT, 3 * (UP + RIGHT), 3 * (DOWN + LEFT), 3 * (DOWN + RIGHT)])
        points *= 0.75
        regions = VGroup(*[Square().set_points_as_corners(points[indices]) for indices in [[0, 1, 2, 3], [0, 4, 2, 1], [5, 0, 3, 2], [5, 2, 4, 6], [6, 4, 0, 5]]])
        ...
        for region, square in zip(regions, squares):
            ...
            region.edges = VGroup(*[Line(c1, c2, color=self.e_color) for c1, c2 in adjacent_pairs(corners)])
            all_edges.add(*region.edges)
            region.vertices = VGroup(*[Dot(color=self.v_color).move_to(c) for c in corners])
            mover = VGroup(square, square.edges, square.vertices)
        ...
        self.play(FadeIn(back_region, rate_func=squish_rate_func(smooth, 0.7, 1), run_time=3), LaggedStartMap(MoveToTarget, all_movers, run_time=3, replace_mobject_with_target_in_scene=True))
        self.wait(2)
        self.set_variables_as_attrs(regions, all_edges, back_region, graph=VGroup(*[m.target for m in all_movers]))
    ...
    def count_edges_per_region(self):
        ...
        for region in list(regions[:-1]) + [back_region]:
            self.play(FadeIn(region), Animation(ghost_edges), FadeOut(last_region), Animation(count), Animation(all_vertices))
            for edge in region.edges:
                new_count = Integer(count.number + 1)
                new_count.replace(count, dim_to_match=1)
                new_count.set_color(count.get_color())
                self.play(ShowCreation(edge), FadeOut(count), FadeIn(new_count), run_time=0.5)
                count = new_count
            last_region = VGroup(region, region.edges)
        self.wait()
        self.add_foreground_mobjects(count)
        self.play(FadeOut(last_region), Animation(ghost_edges), Animation(all_vertices))
    ...
    def ten_total_edges(self):
        ...
        regions = self.regions
        edges = VGroup(*it.chain(regions[0].edges, regions[-1].edges, [regions[1].edges[1]], [regions[2].edges[3]]))
        count = Integer(0)
        count.scale(2)
...
class EulersFormulaForGeneralPlanarGraph(LightUpNodes, ThreeDScene):
    ...
    def show_creation_of_graph(self):
        points = np.array([UP + LEFT, UP + RIGHT, DOWN + RIGHT, DOWN + LEFT, 3 * (UP + LEFT), 3 * (UP + RIGHT), 3 * (DOWN + RIGHT), 3 * (DOWN + LEFT)])
        points *= 0.75
        points += DOWN
        vertices = VGroup(*list(map(Dot, points)))
        vertices.set_color(YELLOW)
        edges = VGroup(*[VGroup(*[Line(p1, p2, color=WHITE) for p2 in points]) for p1 in points])
        regions = self.get_cube_faces(points)
        regions.set_stroke(width=0)
        regions.set_fill(opacity=1)
        regions.set_color_by_gradient(GREEN, RED, BLUE_E)
        regions[-1].set_fill(opacity=0)
        pairs = [(edges[0][1], vertices[1]), (edges[1][2], vertices[2]), (edges[2][6], vertices[6]), (edges[6][5], vertices[5]), (edges[5][1], regions[2]), (edges[0][4], vertices[4]), (edges[4][5], regions[1]), (edges[0][3], vertices[3]), (edges[3][2], regions[0]), (edges[4][7], vertices[7]), (edges[7][3], regions[4]), (edges[7][6], regions[3])]
    ...
    def show_formula(self):
        ...
        twos = VGroup(*[OldTex('2').next_to(group.symbols, RIGHT) for group in groups])
    ...
    def transform_into_cube(self):
        regions = self.regions
        points = np.array([UP + LEFT, UP + RIGHT, DOWN + RIGHT, DOWN + LEFT, UP + LEFT + 2 * IN, UP + RIGHT + 2 * IN, DOWN + RIGHT + 2 * IN, DOWN + LEFT + 2 * IN])
    ...
    def get_cube_faces(self, eight_points):
        return VGroup(*[Square().set_points_as_corners(eight_points[indices]) for indices in [[0, 1, 2, 3], [0, 4, 5, 1], [1, 5, 6, 2], [2, 6, 7, 3], [3, 7, 4, 0], [4, 5, 6, 7]]])

class YouGaveFriendsAnImpossiblePuzzle(TeacherStudentsScene):

    def construct(self):
        self.student_says('You gave friends \\\\ an impossible puzzle?', target_mode='sassy')
        self.play_student_changes('angry', 'sassy', 'angry', added_anims=[self.teacher.change, 'happy'])
        self.wait(2)
...
class FunnyStory(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play_student_changes(*['happy'] * 3, added_anims=[RemovePiCreatureBubble(self.teacher, target_mode='raise_right_hand')], look_at=UP + 2 * RIGHT)
        self.wait(5)

class QuestionWrapper(Scene):

    def construct(self):
        question = OldTexText('Where', '\\emph{specifically}', 'does\\\\', 'this proof break down?')
        question.to_edge(UP)
...
class NewMugThumbnail(Scene):
    ...
    def construct(self):
        title = OldTex('V - E + F = 0', tex_to_color_map={'0': YELLOW})
        title.scale(3)
        title.to_edge(UP)
...
class Thumbnail3(UtilitiesPuzzleScene):
    ...
    def construct(self):
        ...
        words.to_corner(UL)
        arrow = Arrow(words.get_corner(UR) + 0.1 * DOWN, group.get_top() + 0.35 * DOWN + 0.15 * RIGHT, path_arc=-75 * DEGREES, thickness=0.1)
        arrow.set_fill(RED)
