...
class Jewel(VMobject):
    CONFIG = {'color': WHITE, 'fill_opacity': 0.75, 'stroke_width': 0, 'height': 0.5, 'num_equator_points': 5, 'sun_vect': OUT + LEFT + UP}
...
class Necklace(VMobject):
    CONFIG = {'width': FRAME_WIDTH - 1, 'jewel_buff': MED_SMALL_BUFF, 'chain_color': GREY, 'default_colors': [(4, BLUE), (6, WHITE), (4, GREEN)]}
    ...
    def init_points(self):
        ...
        chain = Line(jewels[0].get_center() + j_to_j_dist * LEFT / 2, jewels[-1].get_center() + j_to_j_dist * RIGHT / 2, color=self.chain_color)
...
class CheckOutMathologer(PiCreatureScene):
    CONFIG = {'logo_height': 1.5, 'screen_height': 5, 'channel_name': 'Mathologer', 'logo_file': 'mathologer_logo', 'logo_color': None}
    ...
    def construct(self):
        ...
        rect.to_edge(LEFT)
        self.play(self.get_logo_intro_animation(logo), self.pi_creature.change_mode, 'hooray')
...
class IntroduceStolenNecklaceProblem(ThreeDScene):
    CONFIG = {'jewel_colors': [BLUE, GREEN, WHITE, RED], 'num_per_jewel': [8, 10, 4, 6], 'num_shuffles': 1, 'necklace_center': UP, 'random_seed': 2, 'forced_binary_choices': (0, 1, 0, 1, 0)}
    ...
    def write_title(self):
        ...
        title.to_edge(UP)
        self.play(Write(title), *[ApplyMethod(pi.look_at, title) for pi in self.thieves])
        self.title = title
    ...
    def introduce_necklace(self):
        ...
        self.play(FadeIn(necklace, lag_ratio=0.5, run_time=3), *it.chain(*[[pi.change_mode, 'conniving', pi.look_at, necklace] for pi in self.thieves]))
        self.play(*[ApplyMethod(jewel.rotate, np.pi / 6, UP, rate_func=there_and_back) for jewel in jewels])
    ...
    def divvy_by_cutting_all(self):
        ...
        for type_index, jewel_type in enumerate(jewel_types):
            ...
            halves = [VGroup(*jewel_type_copy[:n_jewels / 2]), VGroup(*jewel_type_copy[n_jewels / 2:])]
            for half, thief, vect in zip(halves, thieves, [RIGHT, LEFT]):
                half.arrange(DOWN)
                half.next_to(thief, vect, buff=SMALL_BUFF + type_index * half.get_width(), aligned_edge=DOWN)
            self.play(Transform(jewel_type, jewel_type_copy), *[ApplyMethod(thief.look_at, jewel_type_copy) for thief in thieves])
        self.play(*it.chain(*[[thief.change_mode, 'happy', thief.look_at, necklace] for thief in thieves]))
        self.wait()
        self.play(*[jewel_type.restore for jewel_type in jewel_types])
        self.play(*it.chain(*[[thief.change_mode, 'confused', thief.look_at, necklace] for thief in thieves]))

    def divvy_with_n_cuts(self, with_thieves=True, highlight_groups=True, show_matching_after_divvying=True):
        necklace = self.necklace
        jewel_types = self.jewel_types
        jewels = sorted(necklace.jewels, lambda m1, m2: cmp(m1.get_center()[0], m2.get_center()[0]))
        slice_indices, binary_choices = self.find_slice_indices(jewels, jewel_types)
        subgroups = [VGroup(*jewels[i1:i2]) for i1, i2 in zip(slice_indices, slice_indices[1:])]
        buff = (jewels[1].get_left()[0] - jewels[0].get_right()[0]) / 2
        v_lines = VGroup(*[DashedLine(UP, DOWN).next_to(group, RIGHT, buff=buff) for group in subgroups[:-1]])
        strand_groups = [VGroup(), VGroup()]
        for group, choice in zip(subgroups, binary_choices):
            strand = Line(group[0].get_center(), group[-1].get_center(), color=necklace.chain.get_color())
            strand.add(*group)
            strand_groups[choice].add(strand)
            self.add(strand)
        self.play(ShowCreation(v_lines))
        self.play(FadeOut(necklace.chain), *it.chain(*[list(map(Animation, group)) for group in strand_groups]))
        for group in strand_groups:
            group.save_state()
        self.play(strand_groups[0].shift, UP / 2.0, strand_groups[1].shift, DOWN / 2.0)
        if with_thieves:
            self.play(*it.chain(*[[thief.change_mode, 'happy', thief.look_at, self.necklace] for thief in self.thieves]))
            self.play(Blink(self.thieves[1]))
        else:
            self.wait()
        if highlight_groups:
            for group in strand_groups:
                box = Rectangle(width=group.get_width() + 2 * SMALL_BUFF, height=group.get_height() + 2 * SMALL_BUFF, stroke_width=0, fill_color=YELLOW, fill_opacity=0.3)
                box.move_to(group)
        ...
        if show_matching_after_divvying:
            for jewel_type in jewel_types:
                self.play(*[ApplyMethod(jewel.scale, 1.5) for jewel in jewel_type], rate_func=there_and_back, run_time=2)
            self.wait()
        self.play(FadeOut(v_lines), FadeIn(necklace.chain), *[group.restore for group in strand_groups])
        self.remove(*strand_groups)
        self.add(necklace)

    def get_necklace(self, **kwargs):
        colors = reduce(op.add, [num * [color] for num, color in zip(self.num_per_jewel, self.jewel_colors)])
    ...
    def get_jewels_organized_by_type(self, jewels):
        return [VGroup(*[m for m in jewels if m.get_color() == color]) for color in map(Color, self.jewel_colors)]

    def shuffle_jewels(self, jewels, run_time=2, path_arc=np.pi / 2, **kwargs):
        shuffled_indices = list(range(len(jewels)))
        random.shuffle(shuffled_indices)
        target_group = VGroup(*[jewel.copy().move_to(jewels[shuffled_indices[i]]) for i, jewel in enumerate(jewels)])
        self.play(Transform(jewels, target_group, run_time=run_time, path_arc=path_arc, **kwargs))
    ...
    def find_slice_indices(self, jewels, jewel_types):
        ...
        for slice_indices in it.combinations(list(range(1, len(jewels))), n_types):
            ...
            for binary_choices in all_binary_choices:
                subsets = [type_numbers[i1:i2] for i1, i2 in zip(slice_indices, slice_indices[1:])]
                left_sets, right_sets = [[subset for subset, index in zip(subsets, binary_choices) if index == target_index] for target_index in range(2)]
                flat_left_set = np.array(list(it.chain(*left_sets)))
                flat_right_set = np.array(list(it.chain(*right_sets)))
                match_array = [sum(flat_left_set == n) == sum(flat_right_set == n) for n in range(n_types)]
...
class ThingToProve(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(Write(top_words, run_time=2), self.pi_creature.change_mode, 'raise_right_hand')
        self.play(ShowCreation(arrow))
        self.play(Write(bottom_words, run_time=2), self.pi_creature.change_mode, 'pondering')
        self.wait(3)

class FiveJewelCase(IntroduceStolenNecklaceProblem):
    CONFIG = {'jewel_colors': [BLUE, GREEN, WHITE, RED, YELLOW], 'num_per_jewel': [6, 4, 4, 2, 8], 'forced_binary_choices': (0, 1, 0, 1, 0, 1)}
...
class SixJewelCase(FiveJewelCase):
    CONFIG = {'jewel_colors': [BLUE, GREEN, WHITE, RED, YELLOW, MAROON_B], 'num_per_jewel': [6, 4, 4, 2, 2, 6], 'forced_binary_choices': (0, 1, 0, 1, 0, 1, 0)}
...
class ThreeJewelCase(FiveJewelCase):
    CONFIG = {'jewel_colors': [BLUE, GREEN, WHITE], 'num_per_jewel': [6, 4, 8], 'forced_binary_choices': (0, 1, 0, 1)}

class RepeatedShuffling(IntroduceStolenNecklaceProblem):
    CONFIG = {'num_shuffles': 5, 'random_seed': 3, 'show_matching_after_divvying': False}
...
class WriteNotAllowed(WriteWords):
    CONFIG = {'words': 'Not allowed', 'color': RED}
...
class WriteBorsukUlam(WriteWords):
    CONFIG = {'words': 'Borsuk-Ulam Theorem'}

class WriteAntipodal(WriteWords):
    CONFIG = {'words': "``Antipodal''", 'color': MAROON_B}
...
class TemperaturePressurePlane(GraphScene):
    CONFIG = {'x_labeled_nums': [], 'y_labeled_nums': [], 'x_axis_label': 'Temperature', 'y_axis_label': 'Pressure', 'graph_origin': 2.5 * DOWN + 2 * LEFT, 'corner_square_width': 4, 'example_point_coords': (2, 5)}
...
class PointOutVSauce(CheckOutMathologer):
    CONFIG = {'channel_name': '', 'logo_file': 'Vsauce_logo', 'logo_height': 1, 'logo_color': GREY}
    ...
    def get_logo_intro_animation(self, logo):
        return DrawBorderThenFill(logo, run_time=2)

class WalkEquatorPostTransform(GraphScene):
    CONFIG = {'x_labeled_nums': [], 'y_labeled_nums': [], 'graph_origin': 2.5 * DOWN + 2 * LEFT, 'curved_arrow_color': WHITE, 'curved_arrow_radius': 3, 'num_great_arcs': 10}
    ...
    def add_curved_arrow(self):
        arc = Arc(start_angle=2 * np.pi / 3, angle=-np.pi / 2, radius=self.curved_arrow_radius, color=self.curved_arrow_color)
        arc.add_tip()
    ...
    def walk_equator(self):
        ...
        self.play(self.get_arc_walk_dot_movement(equator, dots, rate_func=lambda t: 2 * proportion * smooth(t)))
    ...
    def walk_tilted_equator(self):
        ...
        self.play(self.get_arc_walk_dot_movement(tilted_eq, dots, rate_func=lambda t: 2 * proportion * smooth(t)))
        v_line = DashedLine(FRAME_Y_RADIUS * UP, FRAME_Y_RADIUS * DOWN)
        v_line.shift(dots.get_center()[0] * RIGHT)
        self.play(ShowCreation(v_line))
        self.wait()
    ...
    def draw_transverse_curve(self):
        ...
        equator_transform = Succession(*[Transform(equator, arc, rate_func=linear) for arc in target_arcs])
        dots_transform = Succession(*[Transform(dots, target, rate_func=linear) for target in target_dots])
        self.play(ShowCreation(transverse_curve, lag_ratio=0), equator_transform, dots_transform, run_time=10, rate_func=linear)
        self.wait(2)
    ...
    def walk_transverse_curve(self):
        ...
        self.play(UpdateFromAlphaFunc(dots, dot_update), run_time=4, rate_func=lambda t: 0.455 * smooth(t))
    ...
    def get_great_arc_images(self):
        curves = VGroup(*[ParametricCurve(lambda t: self.sphere_point(t, s)).apply_function(self.sphere_to_plane) for s in np.arange(0, 1, 1.0 / self.num_great_arcs)])
        curves.set_color(YELLOW)
        curves[0].set_color(RED)
        for curve in curves:
            antipodal_x_diff = lambda x: curve.point_from_proportion(x + 0.5)[0] - curve.point_from_proportion(x)[0]
            last_x = 0
            ...
            for x in np.linspace(0, 0.5, 100):
                ...
                if sign != last_sign:
                    ...
                    curve.x_collision_points = [curve.point_from_proportion(mean), curve.point_from_proportion(mean + 0.5)]
    ...
    def get_transverse_curve(self, gerat_arc_images):
        points = list(it.chain(*[[curve.x_collision_points[i] for curve in gerat_arc_images] for i in (0, 1)]))
...
class MentionGenerality(TeacherStudentsScene, ThreeDScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.change_mode, 'raise_right_hand', self.teacher.look_at, arrow)
        self.play(FadeIn(necklace, run_time=2, lag_ratio=0.5), Write(arrow), *[ApplyMethod(pi.look_at, arrow) for pi in self.get_pi_creatures()])
...
class PointsIn3D(Scene):
    CONFIG = {'colors': color_gradient([GREEN, BLUE], 3)}

    def construct(self):
        sphere_def = OldTexText('\\doublespacing Sphere in 3D: All', '$(x_1, x_2, x_3)$\\\\', 'such that', '$x_1^2 + x_2^2 + x_3^2 = 1$', alignment='')
        ...
        point_ex = OldTexText('For example, ', '(', '0.41', ', ', '-0.58', ', ', '0.71', ')', arg_separator='')
        ...
        self.wait()
        self.play(Animation(sphere_def[1].copy(), remover=True), Write(sphere_def))
        self.wait()
...
class WritePointCoordinates(Scene):
    CONFIG = {'colors': color_gradient([GREEN, BLUE], 3), 'corner': DOWN + RIGHT}
    ...
    def construct(self):
        ...
        arrow.next_to(coords.get_edge_center(y_component), y_component, aligned_edge=-x_component, buff=MED_SMALL_BUFF)
...
class WriteAntipodalCoordinates(WritePointCoordinates):
    CONFIG = {'corner': UP + LEFT, 'sign_color': RED}
...
class GeneralizeBorsukUlam(Scene):
    CONFIG = {'n_dims': 3, 'boundary_colors': [GREEN_B, BLUE], 'output_boundary_color': [MAROON_B, YELLOW], 'negative_color': RED}
...
class MakeTwoJewelCaseContinuous(IntroduceStolenNecklaceProblem):
    CONFIG = {'jewel_colors': [BLUE, GREEN], 'num_per_jewel': [8, 10], 'random_seed': 2, 'forced_binary_choices': (0, 1, 0), 'show_matching_after_divvying': True, 'necklace_center': ORIGIN, 'necklace_width': FRAME_WIDTH - 3, 'random_seed': 0, 'num_continuous_division_searches': 4}
    ...
    def divvy_with_n_cuts(self):
        IntroduceStolenNecklaceProblem.divvy_with_n_cuts(self, with_thieves=False, highlight_groups=False, show_matching_after_divvying=True)

    def identify_necklace_with_unit_interval(self):
        interval = UnitInterval(tick_frequency=1.0 / sum(self.num_per_jewel), tick_size=0.2, big_tick_numbers=[])
    ...
    def color_necklace(self):
        ...
        segment_types = VGroup(*[VGroup(*[m for m in segments if m.get_color() == Color(color)]) for color in self.jewel_colors])
        for group in segment_types:
            length_tex = OldTex('\\frac{%d}{%d}' % (len(group), len(jewels)))
            length_tex.next_to(group, UP)
            length_tex.shift(UP)
            self.play(group.shift, UP, Write(length_tex, run_time=1))
            self.wait()
    ...
    def color_necklace_by_indices(self, *indices):
        ...
        segments = VGroup(*[Line(chain.point_from_proportion(index / float(n_jewels)), chain.point_from_proportion((index + 1) / float(n_jewels)), color=jewel.get_color()) for index, jewel in zip(indices, jewels)])
        for jewel in jewels:
            jewel.save_state()
    ...
    def find_continuous_fair_division(self):
        chain = self.necklace.chain
        n_jewels = len(self.necklace.jewels)
        slice_indices, ignore = self.find_slice_indices(self.necklace.jewels, self.jewel_types)
        cut_proportions = [sorted([random.random(), random.random()]) for x in range(self.num_continuous_division_searches)]
        cut_proportions.append([float(i) / n_jewels for i in slice_indices[1:-1]])
        cut_points = [list(map(chain.point_from_proportion, pair)) for pair in cut_proportions]
        ...
        for target_points in cut_points[1:]:
            self.play(*[ApplyMethod(line.move_to, point) for line, point in zip(v_lines, target_points)])
            self.wait()
    ...
    def show_continuous_fair_division(self):
        ...
        groups = [VGroup(VGroup(*self.segments[i1:i2]), VGroup(*self.tick_marks[i1:i2])) for i1, i2 in zip(slice_indices, slice_indices[1:])]
        groups[-1].add(self.tick_marks[-1])
        vects = [[UP, DOWN][i] for i in self.forced_binary_choices]
        self.play(*[ApplyMethod(group.shift, 0.5 * vect) for group, vect in zip(groups, vects)])
        self.wait()
        self.groups = groups
    ...
    def set_color_continuous_groups(self):
        ...
        for group in (top_group, bottom_group):
            box = Rectangle(width=FRAME_WIDTH - 2, height=group.get_height() + SMALL_BUFF, stroke_width=0, fill_color=WHITE, fill_opacity=0.25)
            box.shift(group.get_center()[1] * UP)
            boxes.add(box)
        weight_description = VGroup(*[VGroup(OldTex('\\frac{%d}{%d}' % (len(jewel_type) / 2, len(self.segments))), Jewel(color=jewel_type[0].get_color())).arrange() for jewel_type in self.jewel_types])
    ...
    def mention_equivalence_to_discrete_case(self):
        ...
        self.play(PiCreatureSays(morty, 'This is equivalent to\n            the discrete case. ', bubble_config={'height': 3, 'direction': LEFT}))
    ...
    def shift_divide_off_tick_marks(self):
        ...
        for mob in (left_segment, right_segment):
            mob.parts = VGroup(mob.copy().pointwise_become_partial(mob, 0, 0.5), mob.copy().pointwise_become_partial(mob, 0.5, 1))
            self.remove(mob)
            self.add(mob.parts)
        restorers = [left_segment.parts, left_tick, right_segment.parts, right_tick]
        for mob in restorers:
            mob.save_state()
        emerald_segments = VGroup(*[segment for segment in list(groups[0][0]) + list(groups[2][0]) if segment.get_color() == Color(self.jewel_colors[1]) if segment is not right_segment])
        emerald_segments.add(left_segment.parts[0], right_segment.parts[1])
        emerald_segments.sort()
        self.play(v_lines.shift, segment_width * RIGHT / 2)
        self.play(*[ApplyMethod(mob.shift, vect) for mob, vect in [(left_segment.parts[0], UP), (left_tick, UP), (right_segment.parts[0], DOWN), (right_tick, DOWN)]])
        self.wait()
        ...
        label = VGroup(OldTex('5\\left( 1/18 \\right)'), Jewel(color=self.jewel_colors[1])).arrange()
        ...
        self.play(broken_pair[0].shift, -vect / 2, broken_pair[1].shift, vect / 2)
        self.wait()
...
class ThinkAboutTheChoices(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play_student_changes(*['pondering'] * 3, look_at=FRAME_X_RADIUS * RIGHT + FRAME_Y_RADIUS * DOWN)
        self.wait(3)

class ChoicesInNecklaceCutting(ReconfigurableScene):
    CONFIG = {'num_continuous_division_searches': 4, 'denoms': [6, 3, 2], 'necklace_center': DOWN, 'thief_box_offset': 1.2}
    ...
    def add_necklace(self):
        width, colors, num_per_color = [MakeTwoJewelCaseContinuous.CONFIG[key] for key in ['necklace_width', 'jewel_colors', 'num_per_jewel']]
        color_list = list(it.chain(*[num * [color] for num, color in zip(num_per_color, colors)]))
        random.shuffle(color_list)
        interval = UnitInterval(tick_frequency=1.0 / sum(num_per_color), tick_size=0.2, big_tick_numbers=[])
    ...
    def choose_places_to_cut(self):
        ...
        num_pairs = [sorted([random.random(), random.random()]) for x in range(self.num_continuous_division_searches)] + [final_num_pair]
        point_pairs = [list(map(self.interval.number_to_point, num_pair)) for num_pair in num_pairs]
        for line, point in zip(v_lines, point_pairs[0]):
            line.move_to(point)
        self.play(ShowCreation(v_lines))
        for point_pair in point_pairs[1:]:
            self.wait()
            self.play(*[ApplyMethod(line.move_to, point) for line, point in zip(v_lines, point_pair)])
        self.wait()
        self.division_points = list(it.chain([self.interval.get_left()], point_pairs[-1], [self.interval.get_right()]))
        self.v_lines = v_lines

    def show_three_numbers_adding_to_one(self):
        points = self.division_points
        braces = [Brace(Line(p1 + SMALL_BUFF * RIGHT / 2, p2 + SMALL_BUFF * LEFT / 2)) for p1, p2 in zip(points, points[1:])]
        ...
        self.play(*it.chain(*[[GrowFromCenter(brace), Write(brace.label)] for brace in braces]))
        self.play(Write(words))
        self.wait(2)
        self.play(*[ReplacementTransform(brace.label, brace.concrete_label) for brace in braces])
        self.wait()
        self.wiggle_v_lines()
        self.wait()
        self.transition_to_alt_config(denoms=[3, 3, 3])
        self.wait()
        self.play(*list(map(FadeOut, list(braces) + [brace.concrete_label for brace in braces])))
    ...
    def make_binary_choice(self):
        ...
        words.next_to(self.choice_one_words, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        self.play(Write(words))
        self.play(*list(map(FadeIn, [boxes, labels])))
        for binary_choices in it.product(*[[0, 1]] * 3):
            self.play(*[ApplyMethod(group.move_to, group.target_points[choice]) for group, choice in zip(groups, binary_choices)] + [Transform(curr_arrow, arrow_pair[choice], path_arc=np.pi) for curr_arrow, arrow_pair, choice in zip(curr_arrows, arrow_pairs, binary_choices)])
            self.wait()
    ...
    def get_groups(self, indices=None):
        ...
        groups = [VGroup(VGroup(*segments[i1:i2]), VGroup(*tick_marks[i1:i2])) for i1, i2 in zip(indices, indices[1:])]
        for group, index in zip(groups, indices[1:]):
            group[1].add(tick_marks[index].copy())
        groups[-1][1].add(tick_marks[-1])
        for group in groups:
            group.target_points = [group.get_center() + self.thief_box_offset * vect for vect in (UP, DOWN)]
        return groups

    def get_boxes_and_labels(self):
        box = Rectangle(height=self.necklace.get_height() + SMALL_BUFF, width=self.necklace.get_width() + 2 * SMALL_BUFF, stroke_width=0, fill_color=WHITE, fill_opacity=0.25)
        box.move_to(self.necklace)
        boxes = VGroup(*[box.copy().shift(self.thief_box_offset * vect) for vect in (UP, DOWN)])
        labels = VGroup(*[OldTexText('Thief %d' % (i + 1)).next_to(box, UP, aligned_edge=RIGHT) for i, box in enumerate(boxes)])
        return (boxes, labels)
    ...
    def wiggle_v_lines(self):
        self.play(*it.chain(*[[line.rotate, np.pi / 12, vect, line.set_color, RED] for line, vect in zip(self.v_lines, [OUT, IN])]), rate_func=wiggle)
...
class ChoicesForSpherePoint(GeneralizeBorsukUlam):
    ...
    def initialize_words(self):
        choice_one_words = OldTexText('1) Choose', '$x^2$, $y^2$, $z^2$', 'so that', '$x^2+y^2+z^2 = 1$')
        for i in (1, 3):
        ...
        choice_two_words.next_to(choice_one_words, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        self.choice_one_words = choice_one_words
        self.choice_two_words = choice_two_words

    def show_example_choices(self):
        choices = VGroup(*[OldTex(*tex).set_color(color) for color, tex in zip(self.colors, [('x', '^2 = ', '1/6'), ('y', '^2 = ', '1/3'), ('z', '^2 = ', '1/2')])])
    ...
    def show_binary_choices(self):
        for choice in self.choices:
            ...
            sqrts = VGroup(*[OldTex(var_tex + '=' + sign + '\\sqrt{%s}' % frac_tex) for sign in ['+', '-']])
            for sqrt in sqrts:
                sqrt.scale(0.6)
            sqrts.arrange(DOWN)
            sqrts.next_to(choice, RIGHT, buff=LARGE_BUFF)
            sqrts.set_color(choice.get_color())
            arrows = VGroup(*[Arrow(choice.get_right(), sqrt.get_left(), color=WHITE, tip_length=0.1, buff=SMALL_BUFF) for sqrt in sqrts])
...
class NecklaceDivisionSphereAssociation(ChoicesInNecklaceCutting):
    CONFIG = {'xyz_colors': color_gradient([GREEN_B, BLUE], 3), 'necklace_center': DOWN, 'thief_box_offset': 1.6, 'denoms': [6, 3, 2]}
    ...
    def add_braces(self):
        points = self.division_points
        braces = [Brace(Line(p1 + SMALL_BUFF * RIGHT / 2, p2 + SMALL_BUFF * LEFT / 2), UP) for p1, p2 in zip(points, points[1:])]
        for char, brace, color, denom in zip('xyz', braces, self.xyz_colors, self.denoms):
            brace.label = brace.get_text('$%s^2$' % char, '$= 1/%d$' % denom, buff=SMALL_BUFF)
        ...
        self.play(*it.chain(list(map(GrowFromCenter, braces)), [Write(brace.label) for brace in braces]))
        self.wait()
        self.braces = braces
    ...
    def show_binary_choice_association(self):
        ...
        for brace, denom, group, final_choice in quads:
            ...
            choices = [OldTex(char, '=', sign, '\\sqrt{\\frac{1}{%d}}' % denom) for sign in ('+', '-')]
            ...
            self.play(ReplacementTransform(VGroup(brace.label[0], brace, brace.label[1]), choices[0]), group.move_to, group.target_points[0])
            self.wait()
            self.play(Transform(*choices), group.move_to, group.target_points[1])
            self.wait()
            if final_choice == 0:
                self.play(Transform(choices[0], original_choices[0]), group.move_to, group.target_points[0])
            self.swapping_anims += [Transform(choices[0], original_choices[1 - final_choice]), group.move_to, group.target_points[1 - final_choice]]
...
class TotalLengthOfEachJewelEquals(NecklaceDivisionSphereAssociation, ThreeDScene):
    CONFIG = {'random_seed': 1, 'thief_box_offset': 1.2}
    ...
    def find_fair_division(self):
        ...
        self.wait()
        self.play(*[ApplyMethod(group.move_to, group.target_points[choice]) for group, choice in zip(groups, binary_choice)])
        self.wait()
        self.groups = groups
        self.v_lines = v_lines
    ...
    def get_fair_division_indices(self, colors):
        ...
        for i1, i2 in it.combinations(list(range(1, len(colors) - 1)), 2):
            bools = [sum(colors[i1:i2] == color) == type_to_count[color] / 2 for color in color_types]
            if np.all(bools):
                return (i1, i2)
        raise Exception('No fair division found')
    ...
    def demonstrate_fair_division(self):
        segments, tick_marks = self.necklace
        color_types = list(map(Color, set([segment.get_color().get_hex_l() for segment in segments])))
        top_segments = VGroup(*it.chain(self.groups[0][0], self.groups[2][0]))
        bottom_segments = self.groups[1][0]
        for color in color_types:
            monochrome_groups = [VGroup(*[segment for segment in segment_group if segment.get_color() == color]) for segment_group in (top_segments, bottom_segments)]
            labels = VGroup()
            for i, group in enumerate(monochrome_groups):
                ...
                label = VGroup(OldTexText('Thief %d' % (i + 1)), Jewel(color=group[0].get_color()))
                label.arrange()
            ...
            for group, label in zip(monochrome_groups, labels):
                self.play(MoveToTarget(group), FadeIn(label))
                self.wait()
    ...
    def perform_antipodal_swap(self):
        ...
        for binary_choices in binary_choices_list:
            self.play(*[ApplyMethod(group.move_to, group.target_points[choice]) for group, choice in zip(self.groups, binary_choices)])
            self.wait()
...
class ExclaimBorsukUlam(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(*[ApplyMethod(pi.change_mode, 'hooray') for pi in self.get_pi_creatures()])
        self.wait(3)

class ShowFunctionDiagram(TotalLengthOfEachJewelEquals, ReconfigurableScene):
    CONFIG = {'necklace_center': ORIGIN, 'camera_class': ThreeDCamera, 'thief_box_offset': 0.3, 'make_up_fair_division_indices': False}
    ...
    def add_number_pair(self):
        plane_classes = [JewelPairPlane(skip_animations=True, thief_number=x) for x in (1, 2)]
        t1_plane, t2_plane = planes = VGroup(*[VGroup(*plane_class.get_top_level_mobjects()) for plane_class in plane_classes])
        planes.set_width(FRAME_X_RADIUS)
        planes.to_edge(RIGHT)
        self.example_coords = plane_classes[0].example_coords[0]
        arrow = Arrow(self.necklace.get_corner(DOWN + RIGHT), self.example_coords, color=YELLOW)
        self.play(ShowCreation(arrow))
    ...
    def swap_necklace_allocation(self):
        for choices in [(1, 0, 1), (0, 1, 0)]:
            self.play(*[ApplyMethod(group.move_to, group.target_points[i]) for group, i in zip(self.groups, choices)])
            self.wait()
    ...
    def add_sphere_arrow(self):
        ...
        to_plane_arrow = Arrow(up_down_arrow.get_bottom() + DOWN + RIGHT, self.example_coords, color=YELLOW)
...
class JewelPairPlane(GraphScene):
    CONFIG = {'camera_class': ThreeDCamera, 'x_labeled_nums': [], 'y_labeled_nums': [], 'thief_number': 1, 'colors': [BLUE, GREEN]}

    def construct(self):
        self.setup_axes()
        point = self.coords_to_point(4, 5)
        dot = Dot(point, color=WHITE)
        coord_pair = OldTex('\\big(', '\\text{Thief %d }' % self.thief_number, 'X', ',', '\\text{Thief %d }' % self.thief_number, 'X', '\\big)')
...
class WhatAboutGeneralCase(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.teacher_says('Use Borsuk-Ulam for\n            higher-dimensional spheres ', target_mode='hooray')
...
class FourDBorsukUlam(GeneralizeBorsukUlam, PiCreatureScene):
    CONFIG = {'n_dims': 4, 'use_morty': False}
...
class BorsukPatreonThanks(PatreonThanks):
    CONFIG = {'specific_patrons': ['Ali  Yahya', 'Meshal  Alshammari', 'CrypticSwarm    ', 'Ankit   Agarwal', 'Yu  Jun', 'Shelby  Doolittle', 'Dave    Nicponski', 'Damion  Kistler', 'Juan    Benet', 'Othman  Alikhan', 'Justin Helps', 'Markus  Persson', 'Dan Buchoff', 'Derek   Dai', 'Joseph  John Cox', 'Luc Ritchie', 'Guido   Gambardella', 'Jerry   Ling', 'Mark    Govea', 'Vecht', 'Jonathan    Eppele', 'Shimin Kuang', 'Rish    Kundalia', 'Achille Brighton', 'Kirk    Werklund', 'Ripta   Pasay', 'Felipe  Diniz']}
...
class RotatingThreeDSphereProjection(Scene):
    ...
    def construct(self):
        sphere = VGroup(*[Circle(radius=np.sin(t)).shift(np.cos(t) * OUT) for t in np.linspace(0, np.pi, 20)])
