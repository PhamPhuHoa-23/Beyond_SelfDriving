...
class TrigRepresentationsScene(Scene):
    CONFIG = {'unit_length': 1.5, 'arc_radius': 0.5, 'axes_color': WHITE, 'circle_color': RED, 'theta_color': YELLOW, 'theta_height': 0.3, 'theta_value': np.pi / 5, 'x_line_colors': MAROON_B, 'y_line_colors': BLUE}
...
class ReactionsToTattoo(PiCreatureScene):
    ...
    def construct(self):
        modes = ['horrified', 'hesitant', 'pondering', 'thinking', 'sassy']
        ...
        for mode in modes:
            ...
            self.wait(2)
        self.play(Write(tattoo_on_math), self.pi_creature.change_mode, 'hooray', self.pi_creature.look, UP)
        self.wait()
...
class IntroduceCSC(TrigRepresentationsScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(self.axes), ShowCreation(self.circle), GrowFromCenter(brace), csc_of_theta.rotate, np.pi / 2, csc_of_theta.next_to, brace, LEFT, path_arc=np.pi / 2)
        self.play(Write(self.theta_group, run_time=1))
        self.play(ShowCreation(cot_line))
        self.play(ShowCreation(csc_line), csc.set_color, csc_line.get_color())
        self.wait(3)

class TeachObscureTrigFunctions(TeacherStudentsScene):

    def construct(self):
        self.teacher_says('$\\sec(\\theta)$, ', '$\\csc(\\theta)$, ', '$\\cot(\\theta)$')
        content = self.teacher.bubble.content.copy()
        self.play_student_changes(*['confused'] * 3)
        self.student_says('But why?', target_mode='pleading', added_anims=[content.to_corner, UP + RIGHT])
        self.wait()
...
class ExplainTrigFunctionDistances(TrigRepresentationsScene, PiCreatureScene):
    CONFIG = {'use_morty': False, 'alt_theta_val': 2 * np.pi / 5}
    ...
    def introduce_angle(self):
        ...
        self.play(ShowCreation(line), ShowCreation(arc))
        self.play(Write(theta))
        self.play(self.pi_creature.change_mode, 'pondering')
        self.play(ShowCreation(self.circle), Rotating(line, rate_func=smooth, in_place=False), run_time=2)
    ...
    def show_sine_and_cosine(self):
        ...
        self.play(GrowFromCenter(sin_brace), Write(sin_text))
        self.play(self.pi_creature.change_mode, 'happy')
        self.play(ShowCreation(cos_line))
        self.play(GrowFromCenter(cos_brace), Write(cos_text))
        self.wait()
        self.change_mode('well')
        mover = VGroup(sin_group, cos_group, self.theta_group)
        thetas = np.linspace(self.theta_value, self.alt_theta_val, 100)
        targets = []
        for theta in thetas:
            self.theta_value = theta
            targets.append(VGroup(self.get_line_brace_text('sin'), self.get_line_brace_text('cos'), self.get_theta_group()))
        self.play(Succession(*[Transform(mover, target, rate_func=linear) for target in targets], run_time=5, rate_func=there_and_back))
    ...
    def show_tangent_and_cotangent(self):
        ...
        self.wait()
        self.play(self.theta_group[-1].set_color, YELLOW, ShowCreation(line), self.pi_creature.change_mode, 'pondering')
        small_lines = VGroup()
        for group in (tan_group, cot_group):
            small_line, brace, text = group
            self.play(ShowCreation(small_line), GrowFromCenter(brace), text.restore)
            self.wait()
            small_lines.add(small_line)
        self.play(FadeOut(line), Animation(small_lines))
        mover = VGroup(tan_group, cot_group, self.theta_group)
        thetas = np.linspace(self.theta_value, self.alt_theta_val, 100)
        targets = []
        for theta in thetas:
            self.theta_value = theta
            targets.append(VGroup(self.get_line_brace_text('tan'), self.get_line_brace_text('cot'), self.get_theta_group()))
        self.play(Succession(*[Transform(mover, target, rate_func=linear) for target in targets], run_time=5, rate_func=there_and_back))
        self.theta_value = thetas[0]
        self.change_mode('happy')
        self.wait(2)
        ...
        self.play(*it.chain(*[list(map(FadeOut, [tan_group, cot_group])), [Animation(self.theta_group[-1])]]))
    ...
    def show_secant_and_cosecant(self):
        ...
        for group in (sec_group, csc_group):
            ...
            self.wait()
            self.play(ShowCreation(line), GrowFromCenter(brace), text.restore, self.pi_creature.change_mode, 'pondering')
            self.wait()
        mover = VGroup(sec_group, csc_group, self.theta_group, self.tangent_line)
        thetas = np.linspace(self.theta_value, self.alt_theta_val, 100)
        targets = []
        for theta in thetas:
            self.theta_value = theta
            ...
            targets.append(VGroup(new_sec_group, new_csc_group, self.get_theta_group(), self.get_tangent_line()))
        self.play(Succession(*[Transform(mover, target, rate_func=linear) for target in targets], run_time=5, rate_func=there_and_back))
        self.theta_value = thetas[0]
        self.change_mode('confused')
        self.wait(2)
    ...
    def explain_cosecant(self):
        ...
        tri1 = Polygon(ORIGIN, radial_line.get_end(), sin_line.get_end(), color=GREEN, stroke_width=8)
        tri2 = Polygon(csc_line.get_end(), ORIGIN, radial_line.get_end(), color=GREEN, stroke_width=8)
        ...
        self.play(ApplyMethod(tri1.rotate, np.pi / 12, rate_func=wiggle), self.pi_creature.change_mode, 'thinking')
        self.wait()
        tri1.save_state()
        ...
        self.wait(2)
        self.play(Write(question), ShowCreation(arrow), self.pi_creature.change_mode, 'confused')
        self.wait(2)
        ...
        self.play(*list(map(FadeOut, [one, self.csc_group, tri1, frac1, frac2, self.csc_frac, arc_theta])))
    ...
    def reciprocate(self, frac1, frac2):
        ...
        self.play(group.scale, 1 / 0.7, group.to_corner, UP + RIGHT)
        self.play(num1.move_to, dem1, dem1.move_to, num1, num2.move_to, dem2, dem2.move_to, num2, path_arc=np.pi)
        self.wait()
    ...
    def summarize_full_group(self):
        ...
        self.play(*it.chain(*[[mob.scale, scale_factor] for mob in [self.circle, self.axes, theta_subgroup, self.tangent_line]]))
    ...
    def get_line_brace_text(self, func_name='sin'):
        ...
        if (vect[1] < 0) ^ (func_name is 'sec'):
            vect = -vect
            angle += np.pi
        brace = Brace(Line(line.get_length() * LEFT / 2, line.get_length() * RIGHT / 2), UP)
        brace.rotate(angle)
    ...
    def get_tangent_line(self):
        return Line(self.unit_length * (1.0 / np.sin(self.theta_value)) * UP, self.unit_length * (1.0 / np.cos(self.theta_value)) * RIGHT, color=GREY)

class RenameAllInTermsOfSine(Scene):

    def construct(self):
        texs = ['\\sin(\\theta)', '\\cos(\\theta)', '\\tan(\\theta)', '\\csc(\\theta)', '\\sec(\\theta)', '\\cot(\\theta)']
        shift_vals = [4 * LEFT + 3 * UP, 4 * LEFT + UP, 4 * LEFT + DOWN, 4 * RIGHT + 3 * UP, 4 * RIGHT + UP, 4 * RIGHT + DOWN]
        equivs = ['', '= \\sin(90^\\circ - \\theta)', '= \\frac{\\sin(\\theta)}{\\sin(90^\\circ - \\theta)}', '= \\frac{1}{\\sin(\\theta)}', '= \\frac{1}{\\sin(90^\\circ - \\theta)}', '= \\frac{\\sin(90^\\circ - \\theta)}{\\sin(\\theta)}']
        ...
        for mob, rhs_mob in list(zip(mobs, rhs_mobs))[1:3]:
            anims += [FadeOut(rhs_mob), mob.restore, mob.set_color, mob.target_color]
        self.play(*anims)
        self.wait()
        new_rhs_mobs = [OldTex('=\\frac{1}{\\%s(\\theta)}' % s).set_color(color) for s, color in [('cos', cos.target_color), ('tan', tan.target_color)]]
        anims = []
...
class MisMatchOfCoPrefix(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        eqs = VGroup(eq1, eq2)
        self.play(self.get_teacher().change_mode, 'speaking', Write(eqs), *[ApplyMethod(pi.look_at, eqs) for pi in self.get_students()])
        self.random_blink()
        self.play(VGroup(*eq1[-9:-7]).set_color, YELLOW, VGroup(*eq2[:2]).set_color, YELLOW, *[ApplyMethod(pi.change_mode, 'confused') for pi in self.get_students()])
        self.random_blink(2)
...
class Credit(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(headphones), morty.change_mode, 'thinking', FadeOut(morty.bubble), FadeOut(morty.bubble.content))
        self.play(Write(url))
