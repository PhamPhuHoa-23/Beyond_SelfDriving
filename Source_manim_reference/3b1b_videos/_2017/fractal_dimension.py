...
class Britain(SVGMobject):
    CONFIG = {'file_name': 'Britain.svg', 'stroke_width': 0, 'fill_color': BLUE_D, 'fill_opacity': 1, 'height': 5, 'mark_paths_closed': True}
...
class SierpinskiTest(Scene):
    ...
    def construct(self):
        sierp = Sierpinski(order=5)
        self.play(FadeIn(sierp, run_time=5, lag_ratio=0.5))
        self.wait()

class ZoomInOnFractal(PiCreatureScene):
    CONFIG = {'fractal_order': 6, 'num_zooms': 5, 'fractal_class': DiamondFractal, 'index_to_replace': 0}
    ...
    def introduce_fractal(self):
        ...
        for order in range(1, self.fractal_order + 1):
            ...
            self.play(Transform(fractal, new_fractal, run_time=2), self.pi_creature.change_mode, 'hooray')
        return fractal
    ...
    def zoom_in(self, fractal):
        ...
        for x in range(self.num_zooms):
            ...
            everything = VGroup(*[submob for submob in fractal.family_members_with_points() if not submob.is_off_screen() if submob not in grower_family])
            ...
            self.play(Transform(grower, grower_target), MoveToTarget(everything), self.pi_creature.change_mode, 'thinking', run_time=2)
            self.wait()
...
class WhatAreFractals(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(Write(name, run_time=2), FadeIn(picture), *[ApplyMethod(pi.look_at, name) for pi in self.get_pi_creatures()])
        self.wait(2)
...
class IntroduceVonKochCurve(Scene):
    ...
    def get_snowflake(self):
        ...
        curves = VGroup(*[KochCurve(order=self.order, stroke_width=self.stroke_width) for x in range(3)])
    ...
    def isolate_one_curve(self, snowflake):
        self.play(*[ApplyMethod(curve.shift, curve.get_center() / 2) for curve in snowflake])
        self.wait()
    ...
    def zoom_in_on(self, curve):
        ...
        larger_curve.set_color_by_gradient(curve[0].get_color(), curve[-1].get_color())
...
class IntroduceSierpinskiTriangle(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(self.pi_creature.change_mode, 'pondering', *[ApplyMethod(submob.shift, submob.get_center()) for submob in sierp])
        self.wait()
        for submob in sierp:
...
class SelfSimilarFractalsAsSubset(Scene):
    ...
    def add_self_similar_fractals(self):
        fractals = VGroup(DiamondFractal(order=5), KochSnowFlake(order=3), Sierpinski(order=5))
    ...
    def add_general_fractals(self):
        big_rectangle = Rectangle(width=FRAME_WIDTH - MED_LARGE_BUFF, height=FRAME_HEIGHT - MED_LARGE_BUFF)
        title = OldTexText('Fractals')
        title.scale(1.5)
        title.next_to(ORIGIN, RIGHT, buff=LARGE_BUFF)
        title.to_edge(UP, buff=MED_LARGE_BUFF)
        britain = Britain(fill_opacity=0, stroke_width=2, stroke_color=WHITE)
        ...
        self.play(ShowCreation(big_rectangle), Write(title))
...
class ConstrastSmoothAndFractal(Scene):
    CONFIG = {'britain_zoom_point_proportion': 0.45, 'scale_factor': 50, 'fractalification_order': 2, 'fractal_dimension': 1.21}
    ...
    def construct(self):
        ...
        background_rectangle = Rectangle(height=FRAME_HEIGHT, width=FRAME_X_RADIUS)
        ...
        britain = Britain(fill_opacity=0, stroke_width=2, stroke_color=WHITE)[0]
        ...
        smooth_britain.zoom_point = smooth_britain.point_from_proportion(self.britain_zoom_point_proportion)
        britain.shift(FRAME_X_RADIUS * RIGHT / 2)
        britain.zoom_point = britain.point_from_proportion(self.britain_zoom_point_proportion)
        fractalify(britain, order=self.fractalification_order, dimension=self.fractal_dimension)
        britains = VGroup(britain, smooth_britain)
        self.play(*[ShowCreation(mob, run_time=3) for mob in britains])
        self.play(britains.set_fill, BLUE, 1, britains.set_stroke, None, 0)
        self.wait()
        self.play(ApplyMethod(smooth_britain.scale, self.scale_factor, smooth_britain.zoom_point), Animation(v_line), Animation(background_rectangle), ApplyMethod(britain.scale, self.scale_factor, britain.zoom_point), Animation(smooth), Animation(fractal), run_time=10)
        self.wait(2)

class InfiniteKochZoom(Scene):
    CONFIG = {'order': 6, 'left_point': 3 * LEFT}
    ...
    def get_curve(self, order):
        koch_curve = KochCurve(monochromatic=True, order=order, color=BLUE, stroke_width=2)
...
class ShowIdealizations(Scene):
    ...
    def construct(self):
        ...
        VGroup(smooth_britain, britain, koch_snowflake).set_color_by_gradient(BLUE_B, BLUE_D)
...
class ExamplesOfDimension(Scene):
    ...
    def construct(self):
        labels = VGroup(*[OldTexText('%s-dimensional' % s) for s in ('1.585', '1.262', '1.21')])
        fractals = VGroup(*[Sierpinski(order=7), KochSnowFlake(order=5), Britain(stroke_width=2, fill_opacity=0)])
...
class FractalDimensionIsNonsense(Scene):
    ...
    def construct(self):
        ...
        self.play(PiCreatureSays(mathy, "It's 1.585-dimensional!", target_mode='hooray'), morty.change_mode, 'hesitant')
        self.play(Blink(morty))
        self.play(PiCreatureSays(morty, 'Nonsense!', target_mode='angry'), FadeOut(mathy.bubble), FadeOut(mathy.bubble.content), mathy.change_mode, 'guilty')
...
class FourSelfSimilarShapes(Scene):
    CONFIG = {'shape_width': 2, 'sierpinski_order': 6}
    ...
    def get_shapes(self, titles):
        ...
        square = VGroup(*[Square().next_to(ORIGIN, vect, buff=0) for vect in compass_directions(start_vect=DOWN + LEFT)])
    ...
    def mention_measurements(self):
        ...
        labels = list(map(TexText, ['$1/2$ length', '$1/4$ area', '$1/8$ volume', "You'll see..."]))
...
class GeneralWordForMeasurement(Scene):
    ...
    def construct(self):
        ...
        lines = VGroup(*[Line(measure.get_bottom(), word.get_top(), color=word.get_color(), buff=MED_SMALL_BUFF) for word in words])
        for word in words:
...
class ImagineShapesAsMetal(FourSelfSimilarShapes):
    ...
    def construct(self):
        ...
        shapes = self.get_shapes(titles)
        shapes.shift(DOWN)
        descriptions = VGroup(*[OldTexText(*words, arg_separator='\\\\') for shape, words in zip(shapes, [['Thin', 'wire'], ['Flat', 'sheet'], ['Solid', 'cube'], ['Sierpinski', 'mesh']])])
        ...
        self.wait()
        for shape in shapes:
            self.play(shape.scale, 0.5, shape.get_top(), run_time=3, rate_func=there_and_back)
        self.wait()

class ScaledLineMass(Scene):
    CONFIG = {'title': 'Line', 'mass_scaling_factor': '\\frac{1}{2}', 'shape_width': 2, 'break_up_factor': 1.3, 'vert_distance': 2, 'brace_direction': DOWN, 'shape_to_shape_buff': 2 * LARGE_BUFF}
    ...
    def construct(self):
        ...
        mass_scaling_label = OldTexText('Mass scaling factor:', '$%s$' % self.mass_scaling_factor)
        mass_scaling_label[1].set_color(GREEN)
        mass_scaling_label.next_to(scaling_factor_label, DOWN, aligned_edge=LEFT, buff=LARGE_BUFF)
...
class ScaledSquareMass(ScaledLineMass):
    CONFIG = {'title': 'Square', 'mass_scaling_factor': '\\frac{1}{4} = \\left( \\frac{1}{2} \\right)^2', 'brace_direction': LEFT}

    def get_shape(self):
        return VGroup(*[Square(stroke_width=0, fill_color=BLUE, fill_opacity=0.7).shift(vect) for vect in compass_directions(start_vect=DOWN + LEFT)])

class ScaledCubeMass(ScaledLineMass):
    CONFIG = {'title': 'Cube', 'mass_scaling_factor': '\\frac{1}{8} = \\left( \\frac{1}{2} \\right)^3'}
...
class ScaledSierpinskiMass(ScaledLineMass):
    CONFIG = {'title': 'Sierpinski', 'mass_scaling_factor': '\\frac{1}{3}', 'vert_distance': 2.5, 'shape_to_shape_buff': 1.5 * LARGE_BUFF}

    def get_shape(self):
        return Sierpinski(order=6)

class DefineTwoDimensional(PiCreatureScene):
    CONFIG = {'dimension': '2', 'length_color': GREEN, 'dimension_color': YELLOW, 'shape_width': 2, 'scale_factor': 0.5, 'bottom_shape_buff': MED_SMALL_BUFF, 'scalar': 's'}
    ...
    def add_width_mass_labels(self):
        ...
        bottom_length = OldTexText('Length: ', '$%s$' % self.scalar, '$L$', arg_separator='')
        bottom_mass = OldTexText('Mass: ', '$%s^%s$' % (self.scalar, self.dimension), '$M$', arg_separator='')
    ...
    def perform_scaling(self):
        ...
        shape.generate_target()
        shape.target.scale(self.scale_factor)
    ...
    def show_dimension(self):
        ...
        self.play(Transform(top_dimension, self.dimension_in_exp, run_time=2))
        self.wait(3)

    def get_shape(self):
        return Square(stroke_width=0, fill_color=BLUE, fill_opacity=0.7)
...
class DefineSierpinskiDimension(DefineTwoDimensional):
    CONFIG = {'dimension': 'D', 'scalar': '\\left( \\frac{1}{2} \\right)', 'sierpinski_order': 6, 'equation_scale_factor': 1.3}
    ...
    def add_one_third(self):
        equation = OldTexText('$= \\left(\\frac{1}{3}\\right)$', '$M$', arg_separator='')
    ...
    def isolate_equation(self):
        ...
        self.play(*[ApplyMethod(mob.fade, 0.5) for mob in self.get_mobjects()] + [Animation(mob) for mob in keepers_copies])
        self.remove(*keepers_copies)
        for mob in keepers:
            ApplyMethod(mob.restore).update(1)
        self.add(*keepers)
        self.play(self.pi_creature.change_mode, 'confused', self.pi_creature.look_at, self.equation)
        self.wait()
        equation = self.distilled_equation
        self.play(equation.arrange, RIGHT, equation.scale, self.equation_scale_factor, equation.to_corner, UP + RIGHT, run_time=2)
        self.wait(2)
        ...
        self.play(self.pi_creature.change_mode, 'hooray', self.pi_creature.look_at, log_expression)
        self.wait(2)

    def get_shape(self):
        return Sierpinski(order=self.sierpinski_order, color=RED)
...
class LengthAndAreaOfSierpinski(ShowSierpinskiCurve):
    CONFIG = {'curve_start_order': 5, 'sierpinski_start_order': 4, 'n_iterations': 3}
    ...
    def construct(self):
        ...
        for x in range(self.n_iterations):
            ...
            self.play(Transform(curve, new_curve), Transform(sierp, new_sierp), run_time=2)
...
class DimensionOfKoch(Scene):
    CONFIG = {'scaling_factor_color': YELLOW, 'mass_scaling_color': BLUE, 'dimension_color': GREEN_A, 'curve_class': KochCurve, 'scaling_factor': 3, 'mass_scaling_factor': 4, 'num_subparts': 4, 'koch_curve_order': 5, 'koch_curve_width': 5, 'break_up_factor': 1.5, 'down_shift': 3 * DOWN, 'dimension_rhs': '\\approx 1.262'}
    ...
    def add_labels(self):
        scaling_factor = OldTexText('Scaling factor:', '$\\frac{1}{%d}$' % self.scaling_factor)
        ...
        mass_scaling = OldTexText('Mass scaling factor:', '$\\frac{1}{%d}$' % self.mass_scaling_factor)
    ...
    def break_up_curve(self):
        ...
        broken_curve = VGroup(*[VGroup(*curve_copy[i * length / n_parts:(i + 1) * length / n_parts]) for i in range(n_parts)])
    ...
    def compare_sizes(self):
        ...
        self.play(GrowFromCenter(big_brace), GrowFromCenter(little_brace), Write(one), Write(one_third))
        self.wait()
    ...
    def show_dimension(self):
        ...
        formula = VGroup(VGroup(*raw_formula[:5]), VGroup(raw_formula[5]), VGroup(raw_formula[6]), VGroup(*raw_formula[7:]))
        formula.to_corner(UP + LEFT)
        simpler_formula = OldTex(str(self.scaling_factor), '^D', '=', str(self.mass_scaling_factor))
        ...
        log_expression = OldTex('D = \\log_%d(%d) %s' % (self.scaling_factor, self.mass_scaling_factor, self.dimension_rhs))
        ...
        log_expression.next_to(simpler_formula, DOWN, aligned_edge=LEFT, buff=MED_LARGE_BUFF)
        ...
        self.play(Transform(third, formula[0]), Transform(fourth, formula[-1]))
...
class DimensionOfQuadraticKoch(DimensionOfKoch):
    CONFIG = {'curve_class': QuadraticKoch, 'scaling_factor': 4, 'mass_scaling_factor': 8, 'num_subparts': 8, 'koch_curve_order': 4, 'koch_curve_width': 4, 'break_up_factor': 1.7, 'down_shift': 4 * DOWN, 'dimension_rhs': '= \\frac{3}{2} = 1.5'}
    ...
    def add_curve(self):
        ...
        self.wait()
        self.play(curve.next_to, resulting_fractal, DOWN, MED_LARGE_BUFF, Write(resulting_fractal, run_time=1))
    ...
    def set_color_curve_subparts(self):
        ...
        broken_curve = VGroup(*[VGroup(*colored_curve[i * length / n_parts:(i + 1) * length / n_parts]) for i in range(n_parts)])
        ...
        self.play(ApplyFunction(lambda m: break_up(m, self.break_up_factor), broken_curve, rate_func=there_and_back, run_time=2))
        self.wait()
        self.play(Write(self.scaling_factor_mob))
        self.play(Write(self.mass_scaling_factor_mob))
        self.wait(2)
...
class ShowSeveralSelfSimilarityDimensions(Scene):
    ...
    def construct(self):
        vects = [4 * LEFT, ORIGIN, 4 * RIGHT]
        fractal_classes = [PentagonalFractal, QuadraticKoch, DiamondFractal]
        max_orders = [4, 4, 5]
        dimensions = [1.668, 1.5, 1.843]
...
class SeparateFractals(Scene):
    ...
    def construct(self):
        ...
        new_quad_koch = VGroup(*[VGroup(*quad_koch[i * length / 8:(i + 1) * length / 8]) for i in range(8)])
...
class ShowDiskScaling(Scene):
    ...
    def show_non_self_similar_shapes(self):
        ...
        disk = Circle()
        blob = VMobject().set_points_smoothly([RIGHT, RIGHT + UP, ORIGIN, RIGHT + DOWN, LEFT, UP, RIGHT])
        britain = Britain()
    ...
    def isolate_disk(self):
        disk = self.disk
        self.play(FadeOut(self.to_fade), disk.set_width, 2, disk.next_to, ORIGIN, LEFT, 2, disk.set_fill, BLUE_D, 0.7)
    ...
    def try_fitting_small_disks(self):
        ...
        foursome = VGroup(*[disk.copy().next_to(ORIGIN, vect, buff=0) for vect in compass_directions(start_vect=UP + RIGHT)])
        foursome.move_to(self.scaled_disk)
        self.play(Transform(disk, foursome))
        self.remove(*self.get_mobjects_from_last_animation())
        self.add(foursome)
        self.wait()
        self.play(ApplyFunction(lambda m: break_up(m, 0.2), foursome, rate_func=there_and_back, run_time=4))
        self.wait()
...
class BoxCountingScene(Scene):
    CONFIG = {'box_width': 0.25, 'box_color': YELLOW, 'box_opacity': 0.5, 'num_boundary_check_points': 200, 'corner_rect_left_extension': 0}
    ...
    def get_grid(self):
        v_line = Line(UP, DOWN).scale(FRAME_Y_RADIUS)
        v_lines = VGroup(*[v_line.copy().shift(u * x * self.box_width * RIGHT) for x in range(self.num_cols / 2 + 1) for u in [-1, 1]])
        h_line = Line(LEFT, RIGHT).scale(FRAME_X_RADIUS)
        h_lines = VGroup(*[h_line.copy().shift(u * y * self.box_width * UP) for y in range(self.num_rows / 2 + 1) for u in [-1, 1]])
    ...
    def get_box_centers(self):
        bottom_left = reduce(op.add, [self.box_width * (self.num_cols / 2) * LEFT, self.box_width * (self.num_rows / 2) * DOWN, self.box_width * RIGHT / 2, self.box_width * UP / 2])
        return np.array([bottom_left + (x * RIGHT + y * UP) * self.box_width for x in range(self.num_cols) for y in range(self.num_rows)])
    ...
    def get_boxes(self, points):
        ...
        return VGroup(*[Square(side_length=self.box_width, stroke_width=0, fill_color=self.box_color, fill_opacity=self.box_opacity).move_to(point, DOWN + LEFT) for point in unique_rounded_points])

    def get_corner_rect(self):
        rect = Rectangle(height=FRAME_Y_RADIUS / 2, width=FRAME_X_RADIUS + self.corner_rect_left_extension, stroke_width=0, fill_color=BLACK, fill_opacity=0.8)
    ...
    def count_boxes(self, boxes):
        ...
        self.play(Write(num))
        self.play(num.next_to, self.counting_num_reference, RIGHT, MED_SMALL_BUFF, DOWN, num.set_color, YELLOW)
        return num

class BoxCountingWithDisk(BoxCountingScene):
    CONFIG = {'box_width': 0.25, 'num_boundary_check_points': 200, 'corner_rect_left_extension': 2, 'disk_opacity': 0.5, 'disk_stroke_width': 0.5, 'decimal_string': '= %.2f'}
    ...
    def construct(self):
        ...
        self.add(disk, one)
        self.play(ShowCreation(grid), Animation(disk))
        self.wait()
        ...
        self.play(MoveToTarget(disk), Transform(one, two), FadeOut(boxes))
        ...
        self.play(FadeIn(randy))
        self.play(PiCreatureSays(randy, 'Is it?', target_mode='sassy', bubble_config={'direction': LEFT}))
        self.play(Blink(randy))
        self.wait()

class FinerBoxCountingWithDisk(BoxCountingWithDisk):
    CONFIG = {'box_width': 0.03, 'num_boundary_check_points': 1000, 'disk_stroke_width': 0.5, 'decimal_string': '= %.2f'}

class PlotDiskBoxCounting(GraphScene):
    CONFIG = {'x_axis_label': 'Scaling factor', 'y_axis_label': 'Number of boxes \\\\ touched', 'x_labeled_nums': [], 'y_labeled_nums': [], 'x_min': 0, 'y_min': 0, 'y_max': 30, 'func': lambda x: 0.5 * x ** 2, 'func_label': 'f(x) = cx^2'}
    ...
    def plot_points(self):
        ...
        data_points = [self.input_to_graph_point(x) + (random.random() - 0.5) / x * UP for x in np.arange(2, 10, 0.5)]
        data_dots = VGroup(*[Dot(point, radius=0.05, color=YELLOW) for point in data_points])
        self.play(ShowCreation(data_dots))
        self.wait()
        self.play(ShowCreation(self.graph))
        self.label_graph(self.graph, self.func_label, direction=RIGHT + DOWN, buff=SMALL_BUFF, color=WHITE)
        self.wait()
...
class FineGridSameAsLargeScaling(BoxCountingScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(grid.scale, 1.0 / self.scale_factor, disk.scale, 1.0 / self.scale_factor, disk.set_stroke, None, 0.5)
        self.wait()
        boxes = self.get_highlighted_boxes(disk)
        self.play(ShowCreation(boxes, run_time=3))
        self.wait(2)
...
class BoxCountingSierpinski(BoxCountingScene):
    CONFIG = {'box_width': 0.1, 'sierpinski_order': 7, 'sierpinski_width': 3, 'num_boundary_check_points': 6, 'corner_rect_left_extension': 2}
    ...
    def construct(self):
        ...
        counting_mob = self.count_boxes(boxes)
        self.wait()
        self.play(FadeOut(boxes), sierp.scale, 2, sierp.get_corner(DOWN + RIGHT))
        self.play(counting_mob.next_to, counting_mob, DOWN)
        boxes = self.get_highlighted_boxes(sierp)
        new_counting_mob = self.count_boxes(boxes)
        self.wait()
...
class PlotSierpinskiBoxCounting(PlotDiskBoxCounting):
    CONFIG = {'func': lambda x: 0.5 * x ** 1.585, 'func_label': 'f(x) = cx^{1.585}'}

    def construct(self):
        self.plot_points()

class BoxCountingWithBritain(BoxCountingScene):
    CONFIG = {'box_width': 0.1, 'num_boundary_check_points': 5000, 'corner_rect_left_extension': 1}
    ...
    def show_formula(self):
        ...
        simpler_eq.target[-1].next_to(simpler_eq.target[-2].get_corner(UP + RIGHT), RIGHT, buff=SMALL_BUFF)
        ...
        self.wait(2)
        log_expression1 = OldTex('\\log(', 'N', ')', '=', '\\log(', 'c', 's', '^{1.21}', ')')
        log_expression2 = OldTex('\\log(', 'N', ')', '=', '\\log(', 'c', ')', '+', '1.21', '\\log(', 's', ')')
        for log_expression in (log_expression1, log_expression2):
            ...
            log_expression.set_color_by_tex('1.21', dim.get_color())
        rewired_log_expression1 = VGroup(*[log_expression1[index].copy() for index in [0, 1, 2, 3, 4, 5, 8, 8, 7, 4, 6, 8]])
...
class GiveShapeAndPonder(Scene):
    ...
    def construct(self):
        ...
        self.play(morty.change_mode, 'raise_right_hand', morty.look_at, norway, randy.look_at, norway, ShowCreation(norway))
        self.play(Blink(morty))
...
class ConfusedAtParabolicData(PlotDiskBoxCounting):
    CONFIG = {'func': lambda x: 0.5 * x ** 1.6, 'func_label': 'f(x) = cx^{1.21}'}
...
class IntroduceLogLogPlot(GraphScene):
    CONFIG = {'x_axis_label': '\\log(s)', 'y_axis_label': '\\log(N)', 'x_labeled_nums': [], 'y_labeled_nums': [], 'graph_origin': 2.5 * DOWN + 6 * LEFT, 'dimension': 1.21, 'y_intercept': 2, 'x_max': 16}

    def construct(self):
        last_scene = CheapBoxCountingWithBritain()
        expression = last_scene.final_expression
        box = Rectangle(stroke_color=WHITE, fill_color=BLACK, fill_opacity=0.7)
        ...
        self.wait()
        data_points = [self.input_to_graph_point(x) + (random.random() - 0.5) / x * UP for x in np.arange(1, 8, 0.7)]
        data_dots = VGroup(*[Dot(point, radius=0.05, color=YELLOW) for point in data_points])
...
class ManyBritainCounts(BoxCountingWithBritain):
    CONFIG = {'box_width': 0.1, 'num_boundary_check_points': 10000, 'corner_rect_left_extension': 1}
...
class DifferentSlopesAtDifferentScales(IntroduceLogLogPlot):
    ...
    def construct(self):
        ...
        words = OldTexText('\n            Different slopes\n            at different scales\n        ')
        words.to_edge(RIGHT)
        arrows = VGroup(*[Arrow(words.get_left(), self.input_to_graph_point(x)) for x in (1, 7, 12)])
        data_points = [self.input_to_graph_point(x) + 0.3 * (random.random() - 0.5) * UP for x in np.arange(1, self.x_max, 0.7)]
        data_dots = VGroup(*[Dot(point, radius=0.05, color=YELLOW) for point in data_points])
...
class HoldUpCoilExample(TeacherStudentsScene):
    ...
    def construct(self):
        point = UP + RIGHT
        self.play(self.get_teacher().change_mode, 'raise_right_hand', self.get_teacher().look_at, point)
...
class SmoothHilbertZoom(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ApplyMethod(hilbert.scale, 100, hilbert.point_from_proportion(0.3)), Transform(two_d_title, one_d_title, rate_func=squish_rate_func(smooth)), run_time=3)
        self.wait()
...
class ListDimensionTypes(PiCreatureScene):
    ...
    def construct(self):
        types = VGroup(*list(map(TexText, ['Box counting dimension', 'Information dimension', 'Hausdorff dimension', 'Packing dimension'])))
        types.arrange(DOWN, aligned_edge=LEFT)
        for text in types:
            self.play(Write(text, run_time=1), self.pi_creature.change_mode, 'pondering')
        self.wait(3)
...
class ZoomInOnBritain(Scene):
    ...
    def construct(self):
        ...
        britain.set_points_as_corners(reduce(lambda a1, a2: np.append(a1, a2, axis=0), [anchors[:key_value - num_neighbors_kept:thinning_factor, :], anchors[key_value - num_neighbors_kept:key_value + num_neighbors_kept, :], anchors[key_value + num_neighbors_kept::thinning_factor, :]]))
        self.add(britain)
        self.wait()
...
class FromHandwavyToQuantitative(Scene):
    ...
    def construct(self):
        ...
        self.play(Blink(morty))
        self.play(PiCreatureSays(morty, 'We can make \\\\ that quantitative!', target_mode='hooray'), FadeOut(randy.bubble), FadeOut(randy.bubble.content), randy.change_mode, 'happy')
        self.play(Blink(randy))
        self.wait()

class WhatSlopeDoesLogLogPlotApproach(IntroduceLogLogPlot):
    CONFIG = {'words': 'What slope does \\\\ this approach?', 'x_max': 20, 'y_max': 15}
    ...
    def construct(self):
        ...
        randomness = [0.5 * np.exp(-x / 2) + spacing * (0.8 + random.random() / x ** 0.5) for x in x_range]
        cum_sums = np.cumsum(randomness)
        data_points = [self.coords_to_point(x, cum_sum) for x, cum_sum in zip(x_range, cum_sums)]
        data_dots = VGroup(*[Dot(point, radius=0.025, color=YELLOW) for point in data_points])
        words = OldTexText(self.words)
        p1, p2 = [data_dots[int(alpha * len(data_dots))].get_center() for alpha in (0.3, 0.5)]
...
class SmoothBritainLogLogPlot(IntroduceLogLogPlot):
    ...
    def construct(self):
        ...
        data_points = [self.input_to_graph_point(x) + (random.random() - 0.5) / x * UP for x in np.arange(1, 12, 0.7)]
        data_dots = VGroup(*[Dot(point, radius=0.05, color=YELLOW) for point in data_points])
        self.play(ShowCreation(data_dots, run_time=3))
        self.play(ShowCreation(self.graph))
        self.wait()
...
class SlopeAlwaysAboveOne(WhatSlopeDoesLogLogPlotApproach):
    CONFIG = {'words': 'Slope always $> 1$', 'x_max': 20}
...
class CompareBritainAndNorway(Scene):
    ...
    def construct(self):
        norway = Norway(fill_opacity=0, stroke_width=2)
        ...
        self.wait()
        self.play(*it.chain(*[[mob.set_stroke, None, 0, mob.set_fill, BLUE, 1] for mob in (britain, norway)]))
        self.wait(2)
...
class ShowPiCreatureFractalCreation(FractalCreation):
    CONFIG = {'fractal_class': PentagonalPiCreatureFractal, 'max_order': 4}

class FractalPatreonThanks(PatreonThanks):
    CONFIG = {'specific_patrons': ['Meshal  Alshammari', 'Ali Yahya', 'CrypticSwarm    ', 'Yu  Jun', 'Shelby  Doolittle', 'Dave    Nicponski', 'Damion  Kistler', 'Juan    Batiz-Benet', 'Othman  Alikhan', 'Markus  Persson', 'Dan Buchoff', 'Derek   Dai', 'Joseph  John Cox', 'Luc Ritchie', 'Jerry   Ling', 'Mark    Govea', 'Guido   Gambardella', 'Vecht   ', 'Jonathan Eppele', 'Shimin Kuang', 'Rish    Kundalia', 'Achille Brighton', 'Kirk    Werklund', 'Ripta   Pasay', 'Felipe  Diniz']}

class AffirmLogo(SVGMobject):
    CONFIG = {'fill_color': '#0FA0EA', 'fill_opacity': 1, 'stroke_color': '#0FA0EA', 'stroke_width': 0, 'file_name': 'affirm_logo', 'width': 3}
...
class MortyLookingAtRectangle(Scene):
    ...
    def construct(self):
        ...
        self.play(ApplyMethod(affirm_logo.restore, run_time=2), morty.look_at, affirm_logo)
