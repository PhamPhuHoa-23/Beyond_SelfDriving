...
class StateTracker(ValueTracker):
    ...
    def __init__(self, blocks, initial_positions=[8, 5], initial_velocities=[-1, 0]):
        ...
        self.state0 = np.array([*np.array(initial_positions) * self.sqrt_mass_vect, *np.array(initial_velocities) * self.sqrt_mass_vect])
...
class Blocks(InteractiveScene):
    ...
    def construct(self):
        ...
        ke_equation = Tex('\\frac{1}{2} m_1 (v_1)^2 + \\frac{1}{2}m_2 (v_2)^2 = \\text{E}', **kw)
        p_equation = Tex('m_1 v_1 + m_2 v_2 = \\text{P}', **kw)
        ...
        stop_time = 12
        self.play(frame.animate.to_default_state(), FadeIn(ke_equation, shift=0.5 * UP, time_span=(7, 8)), FadeIn(p_equation, shift=0.5 * UP, time_span=(10, 11)), time_tracker.animate.set_value(stop_time).set_anim_args(rate_func=linear), run_time=stop_time)
        self.wait()
        self.add(equations)
        ...
        velocity_vectors = VGroup(self.get_velocity_vector(blocks[0], lambda: marked_velocity.get_value()[0]), self.get_velocity_vector(blocks[1], lambda: marked_velocity.get_value()[1]))
        velocity_vectors.update()
        velocity_vectors.set_z_index(1)
        self.play(frame.animate.reorient(-12, 3, 0, (-3.41, -1.48, 0.01), 4.63), blocks[0].mass_label.animate.set_opacity(1), blocks[1].mass_label.animate.set_opacity(1), VFadeIn(velocity_vectors), FadeOut(p_equation, time_span=(1, 2)), run_time=3)
        self.wait()
        tex = ['m_1', 'v_1', 'm_2', 'v_2']
        labels = [blocks[0][2], velocity_vectors[0][1], blocks[1][2], velocity_vectors[1][1]]
        last_rects = VGroup()
        for tex, label in zip(tex, labels):
            rects = VGroup(SurroundingRectangle(ke_equation[tex], buff=0.05), SurroundingRectangle(label, buff=0.05))
            rects[0].set_stroke(YELLOW, 5)
            rects[1].set_stroke(YELLOW, 3)
            self.play(FadeOut(last_rects), ShowCreation(rects, lag_ratio=0))
            self.wait()
            last_rects = rects
        self.play(FadeOut(last_rects))
        self.wait()
        dec_equation = Tex('\\frac{1}{2}(10)(+0.00)^2 + \\frac{1}{2}(1)(+0.00)^2 = +0.00', font_size=42)
        ...
        self.play(VFadeIn(dec_equation, time_span=(1.5, 2)), Transform(VGroup(*labels).copy().clear_updaters(), label_targets, remover=True, run_time=2, lag_ratio=0.001))
        self.wait()
        self.play(time_tracker.animate.increment_value(1), rate_func=there_and_back, run_time=6)
        self.wait()
        self.play(time_tracker.animate.increment_value(10).set_anim_args(rate_func=linear), frame.animate.reorient(-1, 3, 0, (-1.59, -0.79, 0.17), 6.46), run_time=10, rate_func=linear)
        dec_equation.clear_updaters()
        self.play(time_tracker.animate.set_value(12), frame.animate.reorient(-12, 3, 0, (-3.41, -1.48, 0.01), 4.63), FadeOut(dec_equation), run_time=2)
        self.wait()
        self.play(ke_equation.animate.set_opacity(0.2), FadeIn(p_equation), time_tracker.animate.increment_value(-1))
        self.wait()
        p_dec_equation = Tex('(10)(+0.00) + (1)(+0.00) = +0.00', font_size=42)
        ...
        label_targets.clear_updaters()
        self.play(frame.animate.reorient(-2, 2, 0, (-2.82, -0.66, 0.11), 5.96), VFadeIn(p_dec_equation), Transform(VGroup(*labels).copy().clear_updaters(), label_targets, remover=True), run_time=2)
        self.play(time_tracker.animate.increment_value(8).set_anim_args(rate_func=linear), frame.animate.reorient(6, -1, 0, (-0.7, -0.25, -0.17), 7.75), run_time=16)
        self.wait()
        ...
        self.play(FadeOut(p_dec_equation, 2 * LEFT), frame.animate.to_default_state(), equations.animate.set_width(4.5).arrange(DOWN, buff=0.5).set_opacity(1).to_corner(UL), run_time=2)
        self.wait()
        v_terms = VGroup((eq[f'v_{n}'] for eq in equations for n in [1, 2]))
        ...
        self.play(LaggedStartMap(FlashAround, v_terms, lag_ratio=0.1, time_width=1.0, time_span=(0, 2)), MoveToTarget(v_point, path_arc=PI), UpdateFromFunc(marked_velocity, lambda m: m.set_value(v_point.get_center()[:2])), run_time=3)
        self.wait()
        ...
        state_point = Group(TrueDot(radius=0.05).make_3d(), GlowDot(radius=0.2))
        state_point.set_color(RED)
        state_point.add_updater(lambda m: m.move_to(plane.c2p(*marked_velocity.get_value())))
        kw['font_size'] = 36
        x_label, y_label = axis_labels = VGroup(Tex('x = v_1', **kw), Tex('y = v_2', **kw))
        ...
        self.play(ShowCreation(plane, lag_ratio=0.02))
        self.play(Write(x_label[:2]), TransformFromCopy(ke_equation['v_1'][0], x_label['v_1'][0]), FadeIn(state_point))
        self.add(x_label)
        self.play(marked_velocity.animate.set_value([3, 0]), run_time=3, rate_func=there_and_back)
        self.wait()
        self.play(Write(y_label[:2]), TransformFromCopy(ke_equation['v_2'][0], y_label['v_2'][0]))
        self.add(y_label)
        self.play(marked_velocity.animate.set_value([-1, 3]), run_time=5, rate_func=wiggle)
        self.wait()
        ...
        coord_label = Tex('(+0.00, +0.00)', font_size=36)
        ...
        self.play(LaggedStart(TransformFromCopy(velocity_vectors[0][1].copy().clear_updaters(), terms[0]), TransformFromCopy(velocity_vectors[1][1].copy().clear_updaters(), terms[1]), FadeIn(coord_label[0::2]), lag_ratio=0.5))
        self.wait()
        if False:
            ...
            for _ in range(15):
                ...
                self.wait()
        self.play(FadeOut(coord_label), time_tracker.animate.set_value(0).set_anim_args(run_time=2))
        traced_path = self.get_traced_path(state_point)
        self.add(traced_path)
        self.play(time_tracker.animate.set_value(45), run_time=25, rate_func=linear)
        ...
        self.play(FadeOut(p_equation), ShowCreation(ke_rect), Write(question, run_time=1))
        self.wait()
        time_tracker.set_value(0)
        blocks.update()
        xy_equation = Tex('5.00 {x}^2 + 0.50 {y}^2 = \\text{E}', font_size=36)
        ...
        self.wait()
        self.play(traced_path.animate.set_stroke(WHITE, 1, 0.25), ShowCreation(ellipse), run_time=3)
        self.wait()
        self.play(FadeOut(state_point), FadeOut(traced_path), FadeOut(ke_rect), FadeOut(question), VGroup(ke_equation, xy_equation).animate.arrange(DOWN, buff=0.5).next_to(plane, LEFT, buff=LARGE_BUFF))
        underlines = VGroup(Underline(x_coef, buff=SMALL_BUFF), Underline(ke_equation['m_1'], buff=SMALL_BUFF))
        ...
        self.play(LaggedStartMap(GrowArrow, squish_arrows, lag_ratio=0.01, run_time=1))
        self.play(ShowCreation(underlines, lag_ratio=0))
        self.wait()
        self.play(FadeOut(squish_arrows), FadeOut(underlines))
        marked_velocity.suspend_updating()
        E_rects = VGroup(SurroundingRectangle(ke_equation['\\text{E}'], buff=SMALL_BUFF), SurroundingRectangle(xy_equation['\\text{E}'], buff=SMALL_BUFF))
        E_rects.set_stroke(YELLOW, 1)
        self.play(ShowCreation(E_rects, lag_ratio=0))
        self.play(marked_velocity.animate.set_value([-0.5, 0]), ellipse.animate.scale(0.25), rate_func=there_and_back, run_time=8)
        ...
        new_ke_eq = Tex('\\left(\\sqrt{m_1} \\cdot v_1\\right)^2 + \\left(\\sqrt{m_2} \\cdot v_2\\right)^2 = 2 \\text{E}', **kw)
        new_xy_eq = Tex('x^2 + y^2 = 2\\text{E}', **kw)
        new_xy_eq.scale(48 / 36)
        ke_equation.target = ke_equation.generate_target()
        stack = VGroup(ke_equation.target, new_ke_eq, new_xy_eq)
        stack.arrange(DOWN, buff=0.5)
        stack.next_to(plane, LEFT, buff=1.0)
        self.play(Group(ellipse, traced_path, state_point).animate.stretch(math.sqrt(10), 0, about_point=plane.get_origin()), FadeOut(axis_labels, time_span=(0, 1)), LaggedStart(MoveToTarget(ke_equation), FadeOut(xy_equation, DOWN), FadeIn(new_ke_eq), FadeIn(new_xy_eq), lag_ratio=0.5), run_time=4)
        self.wait()
        ...
        self.wait()
        x_axis_label, y_axis_label = axis_labels = VGroup(Tex('x = \\sqrt{m_1} \\cdot v_1', **kw), Tex('y = \\sqrt{m_2} \\cdot v_2', **kw))
        ...
        x_rhs_tex = '\\sqrt{m_1} \\cdot v_1'
        y_rhs_tex = '\\sqrt{m_2} \\cdot v_2'
        rect2 = SurroundingRectangle(new_ke_eq['\\left(\\sqrt{m_1} \\cdot v_1\\right)^2'])
        rect2.match_style(rect)
        self.play(rect.animate.surround(new_xy_eq['x^2']), FadeIn(rect2), FadeOut(words))
        self.wait()
        self.play(TransformFromCopy(new_xy_eq['x'][0], x_axis_label['x'][0]), TransformFromCopy(new_ke_eq[x_rhs_tex][0], x_axis_label[x_rhs_tex][0]), Write(x_axis_label['='][0]), run_time=2)
        self.wait()
        self.play(rect.animate.surround(new_xy_eq['y^2']), rect2.animate.surround(new_ke_eq['\\left(\\sqrt{m_2} \\cdot v_2\\right)^2']))
        self.wait()
        self.play(TransformFromCopy(new_xy_eq['y'][0], y_axis_label['y'][0]), TransformFromCopy(new_ke_eq[y_rhs_tex][0], y_axis_label[y_rhs_tex][0]), Write(y_axis_label['='][0]), run_time=2)
        self.wait()
        self.play(rect.animate.surround(new_xy_eq), rect2.animate.surround(VGroup(ke_equation, new_ke_eq)))
        self.wait()
        ...
        self.play(time_tracker.animate.set_value(0), FadeOut(new_ke_eq), FadeOut(new_xy_eq), FadeOut(traced_path), run_time=4)
        if False:
        ...
        region.save_state()
        region.stretch(1e-05, 0, about_edge=RIGHT)
        ...
        big_block_rect, lil_block_rect = rects = VGroup((SurroundingRectangle(Group(blocks[i], velocity_vectors[i]), buff=buff) for i, buff in zip([0, 1], [0.25, 0.1])))
        ...
        state_point.clear_updaters()
        marked_velocity.clear_updaters()
        marked_velocity.add_updater(lambda m: m.set_value(np.array(plane.p2c(state_point.get_center())) / state_tracker.sqrt_mass_vect))
        ...
        self.play(ReplacementTransform(x_rect, arc), ReplacementTransform(y_rect, arc))
        self.play(Rotate(state_point, 25 * DEG, about_point=plane.get_center(), rate_func=lambda t: wiggle(t, 5), run_time=8))
        self.wait()
        p_equation.next_to(ke_equation, DOWN)
        new_p_eq = Tex('\\sqrt{m_1}\\left(\\sqrt{m_1} \\cdot v_1\\right) + \\sqrt{m_2}\\left(\\sqrt{m_2} \\cdot v_2\\right) = P', **kw)
        new_p_eq.next_to(p_equation, DOWN, buff=0.75)
        p_xy_eq = Tex('\\sqrt{m_1} x + \\sqrt{m_2} y = P', **kw)
        p_xy_eq.next_to(new_p_eq, DOWN, buff=0.75)
        p1_rects, p2_rects = p_rects = VGroup((VGroup(SurroundingRectangle(p_equation[f'm_{i} v_{i}']), SurroundingRectangle(p_xy_eq[f'\\sqrt{{m_{i}}} {x}'])) for i, x in [(1, 'x'), (2, 'y')]))
        p_rects.set_stroke(GREEN, 2)
        self.play(ke_equation.animate.to_edge(UP).set_opacity(0.33), FadeIn(p_equation))
        self.wait()
        self.play(TransformMatchingTex(p_equation.copy(), new_p_eq))
        self.play(TransformMatchingTex(new_p_eq.copy(), p_xy_eq, key_map={'\\left(\\sqrt{m_1} \\cdot v_1\\right)': 'x', '\\left(\\sqrt{m_2} \\cdot v_2\\right)': 'y'}))
        self.wait()
        ...
        slope_eq = Tex('\\text{Slope} = -\\sqrt{m_1 \\over m_2}', font_size=36)
        slope_eq.next_to(p_line.pfp(0.6), RIGHT)
        self.play(ShowCreation(p_line), FadeOut(arc))
        self.wait()
        self.play(Write(slope_eq))
        self.wait()
        self.play(FadeOut(slope_eq))
        self.play(Group(p_line, state_point).animate.shift(4 * RIGHT), run_time=8, rate_func=there_and_back)
        self.wait()
        ...
        intersection_arrows = VGroup((Vector(0.5 * DL).next_to(point, UR, buff=SMALL_BUFF) for point in [point1, point2]))
        self.play(state_point.animate.move_to(p_line.get_start()), rate_func=lambda t: wiggle(t, 5), run_time=8)
        self.wait()
        ...
        self.wait()
        self.play(FadeOut(intersection_arrows), FadeOut(new_p_eq), FadeOut(p_xy_eq), ke_equation.animate.set_opacity(1))
        traced_path.resume_updating()
        self.play(time_tracker.animate.set_value(7), run_time=5, rate_func=linear)
        traced_path.suspend_updating()
        self.wait()
        state_point.suspend_updating()
        self.play(state_point.animate.move_to(point2), rate_func=there_and_back, run_time=4)
        self.wait()
        self.play(p_line.animate.move_to(plane.c2p(state_tracker.get_momentum() / math.sqrt(self.masses[0]))), run_time=2)
        self.wait()
        p_line.add_updater(lambda m: m.move_to(plane.c2p(state_tracker.get_momentum() / math.sqrt(self.masses[0]))))
        traced_path.resume_updating()
        state_point.resume_updating()
        self.play(time_tracker.animate.set_value(25), run_time=25 - time_tracker.get_value(), rate_func=linear)
        endzone_line = Line(plane.get_origin(), plane.c2p(*state_tracker.sqrt_mass_vect * 4))
        endzone_line.set_stroke(WHITE, 2)
        endzone = Polygon(endzone_line.get_end(), plane.get_origin(), plane.c2p(10, 0))
        ...
        endzone_line_label = Tex('v_1 = v_2', **kw)
        endzone_line_label.next_to(ORIGIN, UP, buff=SMALL_BUFF)
        slope_label = Tex('y = x \\cdot \\sqrt{m_2 / m_1}', **kw)
        slope_label.scale(20 / kw['font_size'])
        slope_label.next_to(ORIGIN, DOWN, SMALL_BUFF)
        VGroup(endzone_line_label, slope_label).rotate(state_tracker.theta, about_point=ORIGIN).shift(endzone_line.pfp(0.45))
        time_tracker.clear_updaters()
        time_tracker.add_updater(lambda m, dt: m.increment_value(dt))
        self.wait(5)
        self.play(FadeOut(p_line), FadeIn(ur_quadrant))
        self.wait(5)
        self.play(FadeOut(ur_quadrant), FadeIn(endzone))
        self.wait(4)
    ...
    def get_floor_and_wall_2d(self, width=13, stroke_width=2, height=2, tick_spacing=0.5, tick_vect=0.25 * DL, buff_to_bottom=0.75):
    ...
    def get_floor_and_wall_3d(self, buff_to_bottom=0.75, color=GREY_D, shading=(0.2, 0.2, 0.2)):
        ...
        for part, vect in zip(result, [OUT, RIGHT]):
            part.data['d_normal_point'] = part.data['point'] + 0.001 * vect
        return result

    def get_block_pair(self, floor, three_d=False):
        return Group((self.get_block(mass, color, width, floor, three_d=three_d) for mass, color, width in zip(self.masses, self.colors, self.widths)))

    def get_block(self, mass, color, width, floor, font_size=24, three_d=False, stroke_width=2, shading=None, floor_buff=0.01):
        if shading is None:
        ...
        block.mass = mass
        mass_label = Tex('10 \\, \\text{kg}', font_size=font_size)
    ...
    def get_velocity_vector(self, block, vel_function, thickness=2, scale_factor=0.5, color=RED, backstroke_width=1, label_backstroke_width=1, num_decimal_places=2, max_width=1.0, font_size=18):
        ...
        label = DecimalNumber(0, num_decimal_places=num_decimal_places, font_size=font_size)
        label.set_fill(RED)
        label.set_backstroke(BLACK, label_backstroke_width)
        label.add_updater(lambda m: m.set_value(vel_function()).next_to(vector.get_start(), UP, buff=0.1))
...
class BasicBlockCount(Blocks):
    ...
    def construct(self):
        ...
        self.play(self.time_tracker.animate.set_value(self.total_time), run_time=self.total_time, rate_func=linear)

    def add_count_label(self):
        count_label = Tex('\\# \\text{Collisions} = 0')
...
class PreviewClip(BasicBlockCount):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(-46, -4, 0, (-0.78, -2.2, -0.17), 5.41), run_time=8)
        self.play(frame.animate.reorient(-4, -4, 0, (-2.38, -1.95, -0.99), 6.58), run_time=12)
...
class SlowMoBlockCount(BasicBlockCount1e4):
    ...
    def setup(self):
        ...
        slowdown_label = Tex('100 \\text{x } \\text{Slower}')
    ...
    def construct(self):
        self.play(self.frame.animate.reorient(-1, 0, 0, (-5.94, -2.36, -1.0), 3.8).set_field_of_view(25 * DEG), run_time=self.time_to_slomo)
        ...
        self.play(self.frame.animate.to_default_state().set_field_of_view(15 * DEG), run_time=8)
        self.wait(30)
...
class SlowMoBlockCount1e6(SlowMoBlockCount):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(-9, 3, 0, (-4.25, -1.33, -0.98), 5.7).set_field_of_view(25 * DEG), run_time=7.9)
        self.clack_sound = 'slow_clack'
        self.add(self.slowdown_label)
        rate_tracker.set_value(0.01)
        self.wait(8)
        rate_tracker.set_value(0.001)
        self.clack_sound = 'super_slow_clack'
        self.wait(7)
        rate_tracker.set_value(0.0001)
...
class SlowMoBlockCount1e8(SlowMoBlockCount):
    ...
    def construct(self):
        frame = self.frame
        rate_tracker = self.rate_tracker
        self.play(frame.animate.reorient(-8, -6, 0, (-4.15, -1.22, -0.96), 6.18).set_field_of_view(25 * DEG), run_time=7.98)
        self.clack_sound = 'slow_clack'
        self.add(self.slowdown_label)
        rate_tracker.set_value(0.01)
        self.wait()
...
class IntroduceSetup(BasicBlockCount):
    ...
    def construct(self):
        ...
        self.play(Write(floor_label), Restore(alt_floor), run_time=2)
        self.wait()
        ...
        self.play(FadeIn(lil_block_label), ShowCreation(lil_block_rect), GrowArrow(lil_block_arrow))
        self.wait()
        self.play(lil_block_rect.animate.surround(blocks[0]).set_stroke(opacity=0), FadeOut(lil_block_label, shift=2 * RIGHT), ReplacementTransform(lil_block_arrow, big_block_vect))
        self.wait()
        ...
        self.play(VFadeOut(big_block_vect), VFadeIn(self.count_label))
        self.wait(10)
...
class ThumbnailShot(BasicBlockCount):
    ...
    def construct(self):
        frame = self.frame
        frame.reorient(-40, -5, 0, (-3.45, -2.54, -0.34), 3.0)
        blocks = self.blocks
...
class MovementOfWall(Blocks):
    ...
    def construct(self):
        ...
        nudge = 0.25 * LEFT
        brace = Brace(Line(ORIGIN, nudge), UP, tex_string='\\underbrace{\\quad\\qquad}')
        brace.next_to(wall, UP, SMALL_BUFF, aligned_edge=RIGHT)
        brace.save_state()
        brace.stretch(0, 0, about_edge=RIGHT).set_fill(opacity=0)
        self.wait(3)
        self.add(ghost_wall)
        self.play(wall.animate.shift(nudge), Restore(brace), rate_func=rush_from)
        self.wait(2)
        time_tracker.clear_updaters()
        block = Cube()
        ...
        self.play(FadeOut(Group(wall, ghost_wall, brace)), FadeIn(block))
...
class CirclePuzzle(InteractiveScene):
    ...
    def construct(self):
        ...
        axes.set_height(7)
        kw = dict(t2c={f'v_1': RED, f'v_2': RED, f'm_1': BLUE, f'm_2': BLUE}, font_size=24)
        axis_labels = VGroup(Tex('x = \\sqrt{m_1} \\cdot v_1', **kw).next_to(x_axis.get_right(), DOWN), Tex('y = \\sqrt{m_2} \\cdot v_2', **kw).next_to(y_axis.get_top(), RIGHT))
        ...
        self.wait()
        slope_label = Tex('\\text{Slope} = -4.0', font_size=36)
        slope_label.next_to(lines[0].get_center(), RIGHT)
        slope_dec = slope_label.make_number_changeable('-4.0')
        count_label = Tex('\\# \\text{Lines} = 1', font_size=36)
        count = count_label.make_number_changeable('1')
        count_label.to_corner(UL)
        right_shift = (slope_dec.get_width() - count.get_width()) * RIGHT
        self.play(ShowCreation(lines[0]), state_point.animate.move_to(lines[0].get_end()), FadeIn(slope_label))
        self.wait()
        self.play(slope_label.animate.next_to(count_label, DOWN, aligned_edge=RIGHT).shift(right_shift), VFadeIn(count_label), ChangeDecimalToValue(count, 2), ShowCreation(lines[1]), state_point.animate.move_to(lines[1].get_end()))
        self.wait()
        for line in lines[2:]:
            self.play(ShowCreation(line), state_point.animate.move_to(line.get_end()), ChangeDecimalToValue(count, count.get_value() + 1))
        self.wait()
        self.add(lines)
        ...
        self.play(LaggedStart((line.animate.set_stroke(PINK, 5).set_anim_args(rate_func=there_and_back) for line in lines), lag_ratio=0.5, run_time=6), ShowCreation(count_rect))
        self.add(lines)
        ...
        slope_rect.add_updater(lambda m: m.set_width(slope_label.get_width() + 2 * SMALL_BUFF, stretch=True, about_edge=LEFT))
        self.play(ReplacementTransform(count_rect, slope_rect, suspend_mobject_updating=True), FadeOut(state_point))
        self.play(slope_tracker.animate.set_value(-10), rate_func=there_and_back, run_time=10)
        for piece in updated_pieces:
            piece.suspend_updating()
        rhs = Tex('= -\\sqrt{m_1 / m_2}', **kw)
        rhs.scale(1.5)
        rhs.next_to(slope_label, RIGHT)
        mass_equations = VGroup(Tex('m_1 = 16', **kw), Tex('m_2 = 1', **kw))
        ...
        self.play(Write(rhs), FadeOut(slope_rect))
        self.wait()
        self.play(TransformFromCopy(rhs['m_1'][0], mass_equations[0]['m_1'][0]), TransformFromCopy(rhs['m_2'][0], mass_equations[1]['m_2'][0]), FadeIn(mass_equations[0][2:]), FadeIn(mass_equations[1][2:]))
        self.wait()
        ...
        self.play(mass_tracker.animate.set_value(100), rate_func=lambda t: there_and_back_with_pause(t, 0.1), run_time=12)
        for piece in updated_pieces:
            piece.suspend_updating()
        ...
        stacked_arcs = VGroup((arc.copy().rotate(PI - angle_of_vector(arc.pfp(0.5))) for arc in arcs))
        stacked_arcs.arrange(RIGHT, buff=0.1)
        stacked_arcs.to_corner(DL)
        self.remove(circle)
        self.play(arcs.animate.set_stroke(opacity=0.25), TransformFromCopy(arcs, stacked_arcs, lag_ratio=0.35, run_time=4))
        self.wait()
        self.play(Transform(stacked_arcs, arcs.copy().set_stroke(opacity=1), remover=True, lag_ratio=0.1), arcs.animate.set_stroke(opacity=1), run_time=3)
        self.wait()
        self.play(lines[:3].animate.set_stroke(opacity=0.15), lines[5:].animate.set_stroke(opacity=0.15), arcs[:4].animate.set_stroke(opacity=0.15), arcs[5:].animate.set_stroke(opacity=0.15), FadeOut(dots))
        self.wait()
        ...
        new_lines = VGroup(Line(points[3], ORIGIN), Line(ORIGIN, points[5]))
        ...
        self.play(ShowCreation(lil_arc), Write(theta_label))
        self.wait()
        self.play(TransformFromCopy(lines[3:5], new_lines), TransformFromCopy(lil_arc, lil_two_arc), TransformFromCopy(theta_label, two_theta_label))
        self.wait()
        self.play(ReplacementTransform(lil_two_arc, arcs[4]), two_theta_label.animate.next_to(arcs[4].pfp(0.5), DOWN, SMALL_BUFF), new_lines.animate.set_stroke(WHITE, 1))
        self.wait()
        theta_group = VGroup(lil_arc, theta_label)
        for n in range(6, len(lines), 2):
            self.play(lines[n - 3:n - 1].animate.set_stroke(opacity=0.15), lines[n - 1:n + 1].animate.set_stroke(opacity=1), theta_group.animate.shift(points[n] - points[n - 2]), Rotate(new_lines, 2 * theta, about_point=ORIGIN), two_theta_label.animate.next_to(arcs[n].pfp(0.5), arcs[n].pfp(0.5), SMALL_BUFF), arcs[n - 2].animate.set_stroke(opacity=0.25), arcs[n].animate.set_stroke(opacity=1))
            self.wait()
        self.play(theta_group.animate.rotate(PI, about_point=points[10]).shift(points[1] - points[10]).scale(0.75, about_point=points[1]), Rotate(new_lines, -12 * theta, about_point=ORIGIN), two_theta_label.animate.next_to(arcs[1].pfp(0.5), arcs[1].pfp(0.5), buff=0.05), lines[9:11].animate.set_stroke(opacity=0.15), lines[0:2].animate.set_stroke(opacity=1), arcs[10].animate.set_stroke(opacity=0.15), arcs[1].animate.set_stroke(opacity=1), FadeOut(axis_labels), run_time=1)
        self.wait()
        for n in range(3, len(lines), 2):
            self.play(lines[n - 3:n - 1].animate.set_stroke(opacity=0.15), lines[n - 1:n + 1].animate.set_stroke(opacity=1), theta_group.animate.shift(points[n] - points[n - 2]), Rotate(new_lines, -2 * theta, about_point=ORIGIN), two_theta_label.animate.next_to(arcs[n].pfp(0.5), arcs[n].pfp(0.5), SMALL_BUFF), arcs[n - 2].animate.set_stroke(opacity=0.25), arcs[n].animate.set_stroke(opacity=1))
            self.wait()
        state_point.move_to(lines[0].get_start())
        self.play(FadeOut(two_theta_label), FadeOut(new_lines), FadeOut(theta_group), arcs[-1].animate.set_stroke(opacity=0.15), lines[-2:].animate.set_stroke(opacity=0.15), FadeIn(state_point))
        ...
        for line, arc in zip(lines, arcs):
            ...
            two_theta_label = Tex('2\\theta', font_size=36)
            ...
            self.play(Animation(ghost_line, remover=True), Animation(ghost_arc, remover=True), ShowCreation(line), state_point.animate.move_to(line.get_end()), Restore(arc), FadeIn(two_theta_label, arc.saved_state.pfp(0.5) - arc.pfp(0.5)))
        self.wait()
        max_x = 8
        ...
        self.play(FadeIn(full_screen_rect), ShowCreation(endzone_line))
        self.wait()
        self.play(FadeOut(full_screen_rect), FadeOut(bold_lines), FadeOut(endzone_line))
        wedge = arcs[-1].copy()
        ...
        self.play(FadeIn(circle), FadeOut(two_theta_labels, lag_ratio=0.1), FadeOut(arcs, lag_ratio=0.1), FadeOut(past_state_points, lag_ratio=0.1), FadeOut(state_point))
        self.play(ShowCreation(slope_label_rect))
        self.play(FadeIn(rhs), FadeIn(mass_equations))
        self.wait()
        self.play(slope_label_rect.animate.surround(mass_equations).stretch(1.2, 0, about_edge=LEFT))
        self.add(lines)
        for piece in updated_pieces:
            piece.resume_updating()
        self.play(mass_tracker.animate.set_value(100), rhs.animate.shift(0.1 * RIGHT), run_time=5)
        for piece in updated_pieces:
            piece.suspend_updating()
        frame = self.frame
        ...
        self.play(FadeOut(slope_label_rect, shift=shift_value), lines[:line_index + 1].animate.set_stroke(opacity=0.1), lines[line_index + 2:].animate.set_stroke(opacity=0.1), frame.animate.shift(shift_value), movers.animate.shift(shift_value))
        dy_line = lines[line_index].copy().rotate(PI)
        dy_line.set_stroke(PINK, 3, 1)
        dx_line = Line(dy_line.get_end(), find_intersection(dy_line.get_end(), RIGHT, lines[line_index + 1].get_start(), lines[line_index + 1].get_vector()))
        dx_line.set_stroke(RED, 5)
        dy_label = Tex('\\Delta y', font_size=36).match_color(dy_line)
        dx_label = Tex('\\Delta x', font_size=36).match_color(dx_line)
        dy_label.next_to(dy_line, LEFT, SMALL_BUFF).shift(0.25 * DOWN)
        dx_label.next_to(dx_line, UP, SMALL_BUFF)
        slope_tex_kw = dict(t2c={'\\Delta y': PINK, '\\Delta x': RED}, font_size=36)
        new_slope_eq = Tex('\\text{Slope} = {\\Delta y \\over \\Delta x}', **slope_tex_kw)
        new_slope_eq.next_to(slope_label, DOWN, buff=0.5, aligned_edge=LEFT)
        self.play(FadeIn(new_slope_eq, DOWN), mass_equations.animate.to_edge(DOWN))
        self.wait()
        self.play(ShowCreation(dy_line), TransformFromCopy(new_slope_eq['\\Delta y'][0], dy_label))
        self.wait()
        self.play(ShowCreation(dx_line), TransformFromCopy(new_slope_eq['\\Delta x'][0], dx_label))
        self.wait()
        theta = math.atan(-1 / get_slope())
        point = dy_line.get_start()
        arc = Arc(-90 * DEG, theta, radius=2.25, arc_center=point)
        theta_label = Tex('\\theta', font_size=36)
        theta_label.next_to(arc, DOWN, SMALL_BUFF)
        tan_eq = Tex('\\tan(\\theta) = {\\Delta x \\over -\\Delta y}', **slope_tex_kw)
        tan_eq.next_to(new_slope_eq, DOWN, buff=0.5, aligned_edge=LEFT)
        self.play(ShowCreation(arc), Write(theta_label))
        self.play(TransformFromCopy(theta_label, tan_eq['\\theta']), Write(tan_eq['\\tan(']), Write(tan_eq[') = ']))
        self.wait()
        self.play(TransformFromCopy(dx_label, tan_eq['\\Delta x'][0]))
        self.play(TransformFromCopy(dy_label, tan_eq['\\Delta y'][0]), Write(tan_eq['\\over']), Write(tan_eq['-']))
        self.wait()
        kw['font_size'] = 36
        new_rhs = Tex('= \\sqrt{m_2 / m_1}', **kw)
        ...
        arctan_eq = Tex('\\theta = \\arctan\\left(\\sqrt{m_2 / m_1}\\right)', **kw)
        arctan_eq.next_to(tan_eq, DOWN, buff=0.5, aligned_edge=LEFT)
        example_arctan = Tex('\\theta = \\arctan(0.1)', font_size=36)
        example_arctan.next_to(arctan_eq, DOWN, buff=0.5, aligned_edge=LEFT)
        example_arctan_rect = SurroundingRectangle(example_arctan)
        self.play(LaggedStart(TransformFromCopy(new_slope_eq['\\Delta y'], tan_eq['\\Delta y'], path_arc=-90 * DEG), TransformFromCopy(new_slope_eq['\\Delta x'], tan_eq['\\Delta x'], path_arc=-90 * DEG), run_time=2))
        self.wait()
        self.play(TransformMatchingTex(rhs.copy(), new_rhs, path_arc=-90 * DEG, run_time=2, lag_ratio=0.01, matched_keys=['m_1', 'm_2', '\\sqrt']))
        self.wait()
        self.play(GrowFromCenter(new_rhs_brace), TransformFromCopy(mass_equations.copy().clear_updaters(), ratio_example))
        self.wait()
        self.play(LaggedStart(TransformFromCopy(tan_eq['\\tan'], arctan_eq['\\arctan']), TransformFromCopy(tan_eq['('], arctan_eq['\\left(']), TransformFromCopy(tan_eq[')'], arctan_eq['\\right)']), TransformFromCopy(tan_eq['\\theta'], arctan_eq['\\theta']), TransformFromCopy(tan_eq['='], arctan_eq['=']), TransformFromCopy(new_rhs['\\sqrt{m_2 / m_1}'], arctan_eq['\\sqrt{m_2 / m_1}']), lag_ratio=0.1, run_time=2))
        self.wait()
    ...
    def get_end_zone(self, slope, max_x=8, color=GREEN, opacity=0.25):
        zone = Polygon(max_x * RIGHT + slope * max_x * UP, ORIGIN, max_x * RIGHT)
    ...
    def get_arcs(self, points, slope):
        ...
        for i in range(-1, len(points) - 2):
            arc = Line(points[max(i, 0)], points[i + 2], path_arc=-2 * theta * (-1) ** (i % 2))
