...
class GroverPreview(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(blocks)
        d = 100
        wires = VGroup(*Line(UP, DOWN).set_height(1).replicate(3), Tex('\\dots'), *Line(UP, DOWN).set_height(1).replicate(3), Tex('\\dots'), *Line(UP, DOWN).set_height(1).replicate(3))
        wires.arrange(RIGHT)
        ...
        kets = VGroup((Tex(f'|{sym}\\rangle') for sym in syms))
        ...
        self.add(wires)
        self.add(kets)
        amplitudes = VGroup((DecimalNumber(0.1, font_size=24, include_sign=True, num_decimal_places=3).next_to(wire, LEFT, buff=0.1) for wire in true_wires))
        ...
        for block, axis in zip(blocks, it.cycle([RIGHT, diag_axis])):
            ...
            self.play(FadeOutToPoint(curr_amplitudes.copy(), block[0].get_center(), lag_ratio=0.0025, time_span=(0, 2)), FadeInFromPoint(new_amplitudes, block[0].get_center(), lag_ratio=0.0025, time_span=(1, 3.0)), frame.animate.set_y(min(block.wires.get_y() + 1, 1)), run_time=3)
...
class ClassicalSearch(InteractiveScene):
    ...
    def construct(self):
        ...
        for n in range(key + 1):
            item = items[n]
            self.play(items.animate.shift((box.get_y() - item.get_y()) * UP), FadeOut(last_sym))
            sym = Checkmark().set_color(GREEN) if n == key else Exmark().set_color(RED)
            sym.set_height(0.75)
            sym.next_to(box, RIGHT, LARGE_BUFF)
            self.play(FadeOutToPoint(item.copy(), box.get_right(), time_span=(0, 0.7)), FadeInFromPoint(sym, box.get_left(), time_span=(0.3, 1)))
            last_sym = sym
        rect = SurroundingRectangle(items[key])
        rect.set_stroke(GREEN, 3)
        self.play(items[:n].animate.set_opacity(0.5), ShowCreation(rect), items[n + 1:].animate.set_opacity(0.5))