...
class ShowPastVideos(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(Write(title), VShowPassingFlash(screen.copy().set_fill(opacity=0).set_stroke(BLUE, 4).insert_n_curves(20), time_width=1.5, run_time=3))
        self.wait()
        versions = VGroup(Text('2019 video: '), Text('Adapted as a short:\n(separate channel) ', alignment='LEFT'), Text('Re-posted to this channel: '))
        versions[1]['(separate channel)'].set_fill(GREY_B)
        versions.arrange(DOWN, buff=1.0, aligned_edge=LEFT)
        versions.to_corner(UL)
        counts = VGroup(Text('13.8M Views').set_color(YELLOW), Text('42.6M Views').set_color(ORANGE), Text('62.0M Views').set_color(RED))
        for version, count in zip(versions, counts):
            count.next_to(version, RIGHT, aligned_edge=UP)
        explanation = VGroup(Text('My only reason for making shorts at all'), Text('is to pique the curiosity of people in the shorts feed and'), Text("encourage them to pop out for a full explanation. Originally I didn't"), Text('want to trash up this channel with shorts, but then the only way to link to'), Text('link to a long-form videos was if the short lived on the same channel.  ¯\\_(ツ)_/¯'))
        ...
        self.play(Transform(title, versions[0]), FadeIn(counts[0]), Transform(screen, vertical_frame))
        for i in [1, 2]:
            self.play(FadeIn(versions[i]), FadeIn(counts[i], lag_ratio=0.1))
...
class Confetti(InteractiveScene):
    ...
    def construct(self):
        ...
        confetti_spirils = [ConfettiSpiril(square, x_start=2 * random.random() * FRAME_X_RADIUS - FRAME_X_RADIUS, num_spirals=np.random.uniform(-5, 5)) for square in confetti_squares]
        self.play(LaggedStart(*confetti_spirils, lag_ratio=0.01, run_time=10))
...
class HappyPiDay(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(Write(title), morty.change('hooray', look_at=title), self.change_students('hooray', 'surprised', 'jamming', look_at=title))
        self.wait()
        self.play(morty.change('tease', 2 * UP), self.change_students('tease', 'surprised', 'well', look_at=3 * UR))
        self.wait(4)

class Leftrightarrow(InteractiveScene):

    def construct(self):
        arrow = Tex('\\longleftrightarrow', font_size=90)
...
class GroversAlgorithmLabel(InteractiveScene):
    ...
    def construct(self):
        labels = VGroup(TexText('Quantum Computing'), TexText("Grover's Algorithm"))
...
class CommentOnElastic(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(morty.says('Assume perfectly\nelastic collisions'), self.change_students('pondering', 'erm', 'tease', look_at=self.screen))
        self.wait(2)
        self.play(stds[2].says('Then there should\nbe no sound', mode='sassy', bubble_direction=LEFT, look_at=morty.eyes), morty.debubble('guilty'))
...
class ShowMassRatioToCountChart(InteractiveScene):
    ...
    def construct(self):
        ...
        titles = VGroup(Text('Mass ratio'), Text('#Collisions'))
...
class StateThePuzzle(InteractiveScene):
    ...
    def construct(self):
        question = TexText('Given $m_1$ and $m_2$, how \\\\ many collisions take place?', t2c={'$m_1$': BLUE, '$m_2$': BLUE})
        question.to_edge(UP)
...
class EnergyAndMomentumLaws(InteractiveScene):
    ...
    def construct(self):
        laws = VGroup(Text('Conservation of Energy'), Text('Conservation of Momentum'))
        laws.arrange(DOWN, buff=0.75)
        laws.move_to(3 * UP, UP)
        self.play(FadeIn(laws[0], UP))
        self.wait()
        self.play(TransformMatchingStrings(*laws, key_map={'Energy': 'Momentum'}, mismatch_animation=FadeTransform, run_time=1))
        self.wait()
...
class ProblemSolvingPrinciplesWithPis(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        items = BulletedList('Try a simpler version of the problem', 'Use the defining features of the problem', 'List any equations that might be relevant', 'Seek symmetry', 'Compute something (anything) to build intuition', 'Run simulations (if possible) to build intuition', 'Draw pictures!')
        ...
        self.play(morty.change('raise_right_hand', underline), self.change_students('pondering', 'pondering', 'pondering', look_at=underline), FadeIn(title, lag_ratio=0.1), ShowCreation(underline))
        self.play(morty.change('tease', stds))
        self.wait()
        index_mode_pairs = [(0, 'raise_right_hand'), (2, 'raise_right_hand'), (1, 'raise_right_hand'), (2, 'raise_left_hand'), (0, 'raise_left_hand'), (1, 'raise_left_hand'), (2, 'raise_right_hand')]
        for item, pair in zip(items, index_mode_pairs):
            index, mode = pair
            if mode == 'raise_left_hand':
                item.shift(2 * LEFT).shift_onto_screen()
            self.play(stds[index].change(mode, item), FadeIn(item, scale=3, shift=item.get_center() - stds[index].get_center()))
        self.wait(3)
        self.play(Restore(items, run_time=2, lag_ratio=0.001), FadeOut(self.pi_creatures), FadeOut(self.background))
        self.wait()
        for index in [2, 6, 3]:
            self.play(items.animate.fade_all_but(index))
            self.wait()
...
class StaysConstant(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(unchanged_label), GrowArrow(unchanged_arrow))
        self.wait()
...
class KeyStep(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.says('This is the\nkey step'), self.change_students('pondering', 'thinking', 'erm', look_at=self.screen))
        self.wait()
...
class HoldUpEllipseVsCircle(InteractiveScene):
    ...
    def construct(self):
        ...
        radial_label = Tex('1', font_size=48)
        radial_label.add_updater(lambda m: m.move_to(radial_line.get_center() + 0.1 * rotate_vector(radial_line.get_vector(), -90 * DEG)))
        ...
        self.play(MoveToTarget(circle_group), VFadeIn(morty))
...
class AskWhy(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.students[2].says('...why?', mode='confused', bubble_direction=LEFT, look_at=self.screen), self.teacher.change('tease'), self.change_students('pondering', 'thinking', look_at=self.screen))
        self.wait()
        self.play(self.teacher.says('Symmetry breeds\ninsight!', mode='hooray'), self.students[2].debubble(look_at=self.teacher.eyes))
...
class PiTime1e5(InteractiveScene):
    ...
    def construct(self):
        value = Integer(PI * 100000.0, font_size=60)
...
class HighlightTheSlope(TeacherStudentsScene):
    ...
    def construct(self):
        slope = Tex('-\\sqrt{m_1 \\over m_2}', t2c={'m_1': BLUE, 'm_2': BLUE}, font_size=60)
        slope.next_to(self.hold_up_spot, UP)
        rect = SurroundingRectangle(slope)
        sqrt_highlight = slope['\\sqrt'][0].copy()
        sqrt_highlight.set_stroke(YELLOW, 4).set_fill(opacity=0)
        sqrt_highlight.insert_n_curves(100)
        self.play(self.teacher.change('raise_right_hand', slope), self.change_students('erm', 'sassy', 'well', look_at=slope), FadeIn(slope, UP))
        self.wait()
        self.play(FlashAround(slope), ShowCreation(rect), self.change_students('pondering', 'pondering', 'thinking', look_at=slope))
        self.wait()
        self.remove(rect)
...
class StareAtDiagram(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(self.change_students('well', 'happy', look_at=self.screen), stds[2].says('I can see\nmyself there', mode='tease', look_at=self.screen, bubble_direction=LEFT), morty.change('tease'))
        self.wait(4)
        self.play(stds[2].debubble(mode='pondering', look_at=3 * UP), self.change_students('pondering', 'pondering', look_at=3 * UP), morty.change('raise_right_hand'), FadeOut(self.background))
        self.wait()
        self.play(stds[1].change('concentrating', look_at=3 * UP))
        self.wait()
        self.play(LaggedStart(stds[1].change('hooray'), stds[0].change('erm', look_at=stds[1].eyes), stds[2].change('erm', look_at=stds[1].eyes), morty.change('tease', look_at=stds[1].eyes), lag_ratio=0.25))
        self.wait()
...
class ISeeWhereThisIsGoing(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for pi in self.pi_creatures:
            pi.body.insert_n_curves(100)
        self.play(morty.change('raise_right_hand'), self.change_students('pondering', 'pondering', 'pondering', look_at=self.screen))
        self.wait()
        self.play(std.says('I see!', mode='hooray'))
        self.play(morty.change('hesitant'))
        mass_terms = VGroup(Tex('m_1 = 100^n', t2c={'m_1': BLUE}), Tex('m_2 = 1', t2c={'m_2': BLUE}))
        mass_terms.arrange(DOWN, aligned_edge=LEFT)
        mass_terms.next_to(std.get_corner(UL), UP, buff=0.5)
        implies = Tex('\\Longrightarrow', font_size=72)
        implies.next_to(mass_terms, RIGHT)
        q_mark = Tex('?').next_to(implies, UP, SMALL_BUFF)
        theta_eq = Tex('\\theta=(0.1)^n')
        theta_eq.next_to(implies, RIGHT)
        group = VGroup(mass_terms, implies, q_mark, theta_eq)
        self.play(std.debubble(mode='raise_left_hand', look_at=mass_terms), FadeIn(mass_terms, UL, scale=2), stds[0].change('hesitant', mass_terms), stds[2].change('erm', mass_terms))
        self.wait()
        self.play(std.change('raise_right_hand', look_at=theta_eq), Write(implies), Write(q_mark), FadeTransform(mass_terms[0].copy(), theta_eq), stds[0].animate.look_at(theta_eq), stds[2].animate.look_at(theta_eq))
        self.wait()
        self.play(std.change('hooray', group), group.animate.set_height(0.75).to_edge(UP))
        self.wait(3)
        self.play(morty.says('Almost', mode='well'), self.change_students('pondering', 'angry', 'confused', look_at=morty.eyes))
        self.wait(3)
        self.play(FadeOut(morty.bubble), morty.says('One final bit\nof reasoning', mode='speaking'), self.change_students('thinking', 'hesitant', 'happy'))
        self.wait(3)
...
class ReferenceSmallAngleApproximations(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for pi in self.pi_creatures:
            pi.body.insert_n_curves(100)
        equation1 = Tex('\\arctan(x) \\approx x')
        equation1['\\arctan'].set_color(GREEN)
        equation2 = Tex('x \\approx \\tan(x)')
        equation2['\\tan'].set_color(YELLOW)
        for mob in (equation1, equation2):
            mob.move_to(self.hold_up_spot, DOWN)
        self.play(FadeIn(equation1, UP), self.teacher.change('raise_right_hand'), self.change_students('erm', 'sassy', 'confused'))
        self.look_at(3 * UL)
        self.play(equation1.animate.shift(UP))
        self.play(TransformMatchingTex(equation1.copy(), equation2, lag_ratio=0.01, key_map={'\\arctan': '\\tan'}))
...
class ExplainSmallAngleApprox(InteractiveScene):
    ...
    def construct(self):
        frame = self.frame
        height = 7.5
        axes = Axes((-2, 2), (-2, 2), height=height, width=height, axis_config=dict(include_tip=True))
        ...
        radial_line = Line()
        radial_line.set_stroke(BLUE, 3)
        radial_line.f_always.put_start_and_end_on(axes.get_origin, point.get_center)
        radial_label = Tex('1', font_size=36)
        radial_label.add_updater(lambda m: m.move_to(radial_line.get_center() + 0.15 * rotate_vector(radial_line.get_vector(), -90 * DEG)))
        self.add(axes)
        self.add(radial_line)
        self.add(radial_label)
        self.play(ShowCreation(circle), UpdateFromFunc(point, lambda m: m.move_to(circle.get_end())), frame.animate.set_height(5), run_time=3)
        self.wait()
        theta_color = YELLOW
        x_color = RED
        y_color = GREEN
        ...
        theta_label.f_always.set_height(lambda: clip(arc.get_height(), 0.01, 0.3))
        theta_label.add_updater(lambda m: m.next_to(arc.pfp(0.65), RIGHT, buff=0.075))
        tan_eq = Tex('\\tan(\\theta) = {{y} \\over {x}} \\approx {\\theta \\over 1}', t2c={'\\theta': theta_color, '{x}': x_color, '{y}': y_color})
        tan_eq.fix_in_frame()
        tan_eq.to_corner(UR).shift(LEFT)
        self.add(h_radial_line)
        self.add(arc)
        self.add(theta_label)
        self.play(theta_tracker.animate.set_value(30 * DEG), radial_line.animate.set_stroke(WHITE, 3), run_time=1)
        self.play(TransformFromCopy(theta_label.copy().clear_updaters(), tan_eq['\\theta'][0]), Write(tan_eq['\\tan(']), Write(tan_eq[') =']))
        self.wait()
        ...
        y_line.add_updater(lambda m: m.put_start_and_end_on(axes.c2p(math.cos(get_theta()), 0), axes.c2p(math.cos(get_theta()), math.sin(get_theta()))))
        x_label = Tex('x', font_size=30).set_color(x_color)
        y_label = Tex('y', font_size=30).set_color(y_color)
        ...
        self.play(LaggedStart(ShowCreation(y_line, suspend_mobject_updating=True), VFadeIn(y_label), FadeOut(h_radial_line), FadeOut(radial_label), ShowCreation(x_line, suspend_mobject_updating=True), VFadeIn(x_label), lag_ratio=0.15))
        self.play(LaggedStart(TransformFromCopy(y_label.copy().clear_updaters(), tan_eq['{y}'][0]), Write(tan_eq['\\over'][0]), TransformFromCopy(x_label.copy().clear_updaters(), tan_eq['{x}'][0]), lag_ratio=0.5))
        self.wait()
        self.play(theta_tracker.animate.set_value(0.1), run_time=3)
        self.wait()
        lil_rect = SurroundingRectangle(tan_eq['{x}'])
        self.play(Write(tan_eq['\\approx']), TransformFromCopy(tan_eq['\\over'][0], tan_eq['\\over'][1]), ShowCreation(lil_rect))
        self.play(TransformFromCopy(tan_eq['{x}'][0], tan_eq['1'][0]), lil_rect.animate.surround(tan_eq['1']))
        self.wait()
        self.play(lil_rect.animate.surround(tan_eq['{y}']), y_label.animate.scale(0.5), x_label.animate.scale(0.5), point.animate.set_radius(0.25 * point.get_radius()), frame.animate.reorient(0, 0, 0, (1.46, -0.0, 0.0), 1.87).set_anim_args(run_time=3))
        self.play(TransformFromCopy(tan_eq['{y}'][0], tan_eq['\\theta'][1]), lil_rect.animate.surround(tan_eq['\\theta'][1]))
        self.play(FadeOut(lil_rect))
        self.play(theta_tracker.animate.set_value(0.01), run_time=8)

class AngryStudents(TeacherStudentsScene):

    def construct(self):
        self.play(self.change_students('angry', 'confused', 'sassy'), self.teacher.change('guilty'))
        self.wait(3)
...
class DigitsOfPi(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FadeOut(one_line_pi[index:]), FadeIn(nines, lag_ratio=0.25, run_time=3))
        self.wait()
        self.play(LaggedStart(GrowFromCenter(prefix_brace), Write(prefix_label), GrowFromCenter(nines_brace), Write(nines_label), lag_ratio=0.25))
        self.wait()
        frame = self.frame
        pi_tex = pi_text[2:].replace('\n', '\\\\&')
        full_pi = Tex('\\pi = 3.&' + pi_tex)
        full_pi.to_edge(UP)
        ...
        self.play(ReplacementTransform(one_line_pi[:index], full_pi[:index]), FadeIn(full_pi[index:], lag_ratio=0.001, time_span=(0.5, 3)), FadeOut(VGroup(prefix_brace, prefix_label, nines_brace, nines_label, nines)), FadeIn(dots, time_span=(2, 3)))
        self.wait()
        self.play(frame.animate.scale(1.8, about_edge=UP), FadeIn(big_nines, lag_ratio=0.01), run_time=6)
        self.wait()
...
class WriteExactSolution(InteractiveScene):
    ...
    def construct(self):
        ...
        kw = dict(t2c={'\\theta': YELLOW, 'm_1': BLUE, 'm_2': BLUE})
        solution = VGroup(Tex('\\#\\text{Collisions} = \\lceil \\pi / \\theta - 1 \\rceil', **kw), Tex('\\text{Where } \\; \\theta = \\text{arctan}\\left(\\sqrt{m_2 / m_1}\\right)', **kw))
...
class ExactSolutionForMatt(InteractiveScene):
    ...
    def construct(self):
        kw = dict(t2c={'\\theta': YELLOW, 'm_1': BLUE, 'm_2': BLUE, '\\pi': WHITE})
        equations = VGroup(Tex('\\#\\text{Collisions} = {\\pi \\over \\text{arctan}\\left(\\sqrt{m_2 / m_1}\\right)}', **kw), Tex('\\#\\text{Collisions} \\approx {\\pi \\over \\sqrt{m_2 / m_1}}', **kw), Tex('\\#\\text{Collisions} \\approx \\pi \\cdot \\sqrt{m_1 / m_2}', **kw))
        for eq in equations[:2]:
            eq['\\pi'].scale(2, about_edge=DOWN)
        equations[2]['\\pi'].scale(1.5, about_edge=DOWN).shift(0.05 * RIGHT)
        rect = SurroundingRectangle(equations[0][len('#Collisions='):])
        rect.set_stroke(YELLOW, 2)
        words = VGroup(Text('If fractional, round down').set_color(YELLOW), TexText('If whole...').set_color(GREEN))
        for word in words:
            word.next_to(rect, UP)
        minus_1 = Tex('-1', font_size=60)
        minus_1.next_to(equations[0]['\\over'], RIGHT)
        self.play(FadeIn(equations[0], lag_ratio=0.25))
        self.wait()
        self.play(ShowCreation(rect), FadeIn(words[0], 0.25 * UP))
        self.wait()
        self.play(rect.animate.set_color(GREEN), FadeTransformPieces(words[0], words[1], run_time=1))
        self.play(Write(minus_1))
        self.wait()
        self.play(LaggedStart(FadeOut(rect), FadeOut(words[1]), FadeOut(minus_1)))
        randy = Randolph()
        pi = equations[0]['\\pi'][0][0]
        ...
        exmark = Cross(equations[0]['\\text{arctan}'])
        self.play(ShowCreation(exmark))
        self.wait()
        self.play(FadeOut(exmark), TransformMatchingTex(equations[0], equations[1]))
        self.wait()
        self.play(TransformMatchingTex(equations[1], equations[2], matched_keys=['m_1', 'm_2', '\\sqrt'], path_arc=45 * DEG))
        self.wait()
...
class WhoCares(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(stds[2].says('Question...', mode='dance_3'), self.change_students('pondering', 'pondering', look_at=stds[2].eyes), morty.change('tease'))
        self.wait()
        old_bubble = stds[2].bubble
        bubble = stds[2].get_bubble('Who cares?', bubble_type=SpeechBubble)
        self.play(LaggedStart(FadeTransformPieces(old_bubble, bubble), stds[2].change('angry'), self.change_students('guilty', 'hesitant'), morty.change('hesitant'), lag_ratio=0.25))
...
class SimplifyingMessiness(InteractiveScene):
    ...
    def construct(self):
        ...
        arrows = VGroup((Arrow(rough_circle.get_right(), circle.get_left(), path_arc=-sgn * 90 * DEG, thickness=5).shift(sgn * UP) for sgn in [+1, -1]))
        mid_arrow = Arrow(rough_circle, circle, buff=0.25, thickness=5, path_arc=1 * DEG)
        self.play(ShowCreation(rough_circle))
        self.wait()
        self.play(LaggedStartMap(GrowArrow, arrows, lag_ratio=0.7), TransformFromCopy(rough_circle, circle, run_time=2))
        self.wait()
        self.play(ReplacementTransform(arrows[0], mid_arrow))
        self.wait()
        self.play(ReplacementTransform(mid_arrow, arrows[1]))
        self.play(circle.animate.center(), FadeOut(rough_circle, 2 * LEFT), FadeOut(arrows[1], 2 * LEFT))
        self.wait()
...
class HiddenConnections(InteractiveScene):
    ...
    def construct(self):
        ...
        q_marks = VGroup((Tex('???', font_size=72).move_to(screen) for screen in screens))
        connections = VGroup()
        ...
        self.remove(circle)
        self.play(FadeIn(background), ReplacementTransform(circle.replicate(len(connections)), connections, lag_ratio=0.1), LaggedStartMap(FadeIn, screens, scale=1.25), Write(q_marks[1:], time_span=(1, 3)), run_time=3)
        self.wait()
        self.play(FadeOut(q_marks[1]), screens[1].animate.set_fill(BLACK, 1), connections[1:].animate.set_stroke(width=1, opacity=0.5))
        self.wait()
        self.play(FadeOut(q_marks[2]), screens[2].animate.set_fill(BLACK, 1), connections[1:4:2].animate.set_stroke(width=3, opacity=1))
        self.wait()
        self.play(FadeOut(q_marks[3]), screens[3].animate.set_fill(BLACK, 1), connections.animate.set_stroke(width=3, opacity=1))
        self.wait()
    ...
    def construct(self):
        ...
        for p1, p2 in it.combinations(self.dots.get_points(), 2):
            if random.random() < np.exp(-0.2 * get_norm(p1 - p2)):
                line = Line(p1, p2, stroke_color=WHITE, stroke_width=0.5, stroke_opacity=random.random() ** 10)
                dense_web.add(line)
