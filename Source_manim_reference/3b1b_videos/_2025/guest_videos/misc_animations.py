...
class WelchLabsIntroCard(InteractiveScene):
    ...
    def construct(self):
        group = Group(VGroup(Text(self.title_words, font_size=48), Text(self.subtitle_words, font_size=36, fill_color=GREY_A)).arrange(DOWN), ImageMobject(self.banner_file).set_width(0.7 * FRAME_WIDTH), Text(self.bottom_words, font_size=36))
        group.arrange(DOWN, buff=MED_LARGE_BUFF)
        group[2].scale(1.1, about_edge=DOWN)
        group[-1].to_edge(DOWN, buff=MED_LARGE_BUFF)
        self.play(LaggedStart(Write(group[0], run_time=1, lag_ratio=0.05), FadeIn(group[1], scale=1.25), FadeIn(group[2], lag_ratio=0.01), lag_ratio=0.05))
        self.wait(2)
        self.play(LaggedStartMap(FadeOut, group, shift=1.0 * DOWN, lag_ratio=0.1))
...
class SubManifolds(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(FadeIn(prompt_words, lag_ratio=0.1), Write(arrow), TransformFromCopy(big_circle, middle, run_time=2)))
        self.wait()
        ...
        def get_training_data_examples(n_samples, min_scale=0.2):
            training_data = DotCloud(np.array([big_circle.pfp(random.random()) * random.uniform(min_scale, 1) for n in range(n_samples)]))
        ...
        self.play(ShowCreation(training_data, run_time=3), Write(no_training_words, run_time=1))
        self.wait()
        blobs.set_fill(opacity=0.25)
        more_data = get_training_data_examples(1000)
        blob_words = VGroup((Text(word, font_size=24) for word in ['cats', 'horses', 'astronauts', 'transformation', 'on the\nmoon']))
        for word, blob in zip(blob_words, blobs):
            word.move_to(blob.get_center() * 1.5)
        blob_words.set_backstroke(BLACK, 2)
        self.play(ShowCreation(more_data, run_time=6), LaggedStartMap(Write, blobs), LaggedStartMap(FadeIn, blob_words), no_training_words.animate.scale(0.5).move_to(middle).set_backstroke(BLACK, 0).set_fill(BLACK), prompt_words.animate.set_backstroke(BLACK, 10))
        self.wait()
...
class IMOGoldOrganizations(InteractiveScene):
    ...
    def construct(self):
        orgs = VGroup(Text('Google DeepMind'), Text('OpenAI'), Text('Harmonic'), Text('ByteDance'))
        orgs.scale(1.5)
        ...
        self.wait()
        self.play(LaggedStart(GrowFromCenter(top_brace), FadeIn(top_text, RIGHT), orgs[:2].animate.match_color(top_text), GrowFromCenter(low_brace), FadeIn(low_text, RIGHT), orgs[2:].animate.match_color(low_text)))
        self.wait()
...
class PhaseChangeEndScreen(PatreonEndScreen):
    ...
    def construct(self):
        ...
        elements = VGroup(ScreenRectangle(height=3), Circle(radius=1))
        ...
        thanks = Text('This channel is funded via Patreon\nSpecial thanks to these supporters', alignment='LEFT', font_size=32, fill_color=text_color)
        ...
        self.add(thanks)
        self.add(names)
        self.play(names.animate.to_edge(DOWN, buff=1.5).set_anim_args(run_time=25, rate_func=linear), LaggedStartMap(VShowPassingFlash, elements.copy().set_stroke(WHITE, 5).insert_n_curves(1000), time_width=2, run_time=3), FadeIn(elements, time_span=(1, 2)))
...
class SeriesOfFiveVideos(InteractiveScene):
    ...
    def construct(self):
        ...
        pure_images = Group((ImageMobject(filename) for filename in ['diffusion_TN', 'alpha_geometry_TN', 'phase_change_TN', 'incomplete_cubes_TN', 'euclid_TN']))
        borders = VGroup((SurroundingRectangle(image, buff=0) for image in pure_images))
        borders.set_stroke(WHITE, 3)
        images = Group((Group(border, image) for border, image in zip(borders, pure_images)))
        images.set_width(4)
        images.arrange_in_grid(n_cols=3, buff=1)
        images[3:].match_x(images[:3]).shift(MED_LARGE_BUFF * DOWN)
        images.set_width(FRAME_WIDTH - 1)
        names = VGroup((Text(f'Guest video by {text}', font_size=30) for text in ['Welch Labs', 'Aleph0', 'Vilas Winstein', 'Paul Dancstep', 'Ben Syversen']))
        ...
        self.add(images[-1])
        self.wait()
        self.play(frame.animate.to_default_state(), LaggedStartMap(FadeIn, images[3::-1], lag_ratio=0.25), run_time=3)
        self.wait()
        titles = VGroup(Text('Diffusion models'), Text('AlphaGeometry'), Text('Statistical Mechanics'), Text('Group theory'), Text('Euclid'))
        titles.scale(0.8)
        for title, name, image in zip(titles, names, images):
            title.move_to(name, DOWN)
            self.play(LaggedStart(FadeIn(title, 0.25 * UP), FadeOut(name, 0.25 * UP), lag_ratio=0.2), pure_images.animate.set_opacity(0.25), borders.animate.set_stroke(opacity=0.25))
        self.wait()