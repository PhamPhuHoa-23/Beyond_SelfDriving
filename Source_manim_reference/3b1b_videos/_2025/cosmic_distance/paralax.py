...
class SimpleDotsParalax(InteractiveScene):
    ...
    def construct(self):
        ...
        if self.show_pi_perspective:
            self.play(frame.animate.reorient(-89, -4, 0, (0.01, 0.21, 0.0), 3.05), randy.animate.set_opacity(0), cube.animate.set_stroke(width=5).set_anti_alias_width(10), run_time=3)
        ...
        self.play(ShowCreation(triangle), FadeIn(red_stars))
...
class ParalxInSolarSystem(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(stars), frame.animate.reorient(0, 71, 0, (2.29, -0.66, 0.52), 43.56), run_time=4)
        self.wait(3)
        self.play(frame.animate.set_euler_angles(-10 * DEG, 86 * DEG, 0).set_height(0.25), UpdateFromAlphaFunc(sun, lambda m, a: frame.move_to(interpolate(frame.get_center(), earth.get_center(), min(2 * a, 1)))), run_time=5)
        frame.clear_updaters()
        frame.add_updater(lambda m: m.move_to(earth))
        self.wait(30)
        self.play(frame.animate.reorient(7, 74, 0, ORIGIN, 120), stars.animate.set_radii(2 * radii), run_time=3)
        new_points = 1000 * normalize_along_axis(stars.get_points(), 1)
        self.play(stars.animate.set_points(new_points).set_radii(20 * radii), run_time=4)
...
class ParalaxMeasurmentFromEarth(InteractiveScene):
    ...
    def construct(self):
        ...
        pi_height = 0.25
        randy, morty = pis = VGroup(Randolph(height=2, mode='hesitant').look_at(10 * RIGHT), Mortimer(height=2, mode='pondering').look_at(10 * RIGHT))
        angles = [45 * DEG, -55 * DEG]
        labels = VGroup(Text('Observer 1', font_size=36), Text('Observer 2', font_size=36))
        ...
        self.play(LaggedStartMap(FadeIn, pis, shift=0.5 * UP, lag_ratio=0.5), LaggedStartMap(FadeIn, labels, shift=0.25 * UP, lag_ratio=0.5))
        self.play(LaggedStartMap(Blink, pis, lag_ratio=0.25))
        self.play(LaggedStartMap(MoveToTarget, pis, lag_ratio=0.7), LaggedStartMap(MoveToTarget, labels, lag_ratio=0.7), FadeIn(obs_dots, time_span=(0.75, 1.25)))
        self.wait()
        frame = self.frame
        obj = GlowDot(12 * RIGHT, color=TEAL)
        obs_lines = VGroup(DashedLine(obs_points[0], obj.get_center()), DashedLine(obs_points[1], obj.get_center()))
        obs_lines.set_stroke(WHITE, 2)
        self.play(frame.animate.set_width(20, about_edge=LEFT), *map(ShowCreation, obs_lines), FadeIn(obj), run_time=2)
        self.wait()
        eyes = Randolph().eyes
        eyes.set_height(1)
        eyes.set_z_index(-1)
        ...
        frame.save_state()
        self.play(FadeIn(celestial_sphere), low_obs_group.animate.fade(0.75), frame.animate.set_height(20, about_edge=LEFT).shift(2 * RIGHT))
        self.play(Rotate(Group(pis[0], obs_dots[0]), angles[1] - angles[0], about_point=earth.get_center()), MaintainPositionRelativeTo(labels[0], pis[0]), run_time=8, rate_func=there_and_back)
        self.play(Restore(low_obs_group), Restore(frame))
        line_between = Line(*obs_points)
        line_between.set_stroke(YELLOW, 3)
        brace_between = LineBrace(line_between, DOWN)
        self.play(ShowCreation(line_between), earth.animate.set_fill(opacity=0.35).set_stroke(width=2, opacity=1), earth_pattern.animate.set_fill(opacity=0.75))
        self.wait()
        ...
        for angle_label in angle_labels:
            ...
            self.wait()
        tip_arc = Arc(obs_lines[0].get_angle() + PI, obs_lines[1].get_angle() - obs_lines[0].get_angle(), arc_center=obj.get_center(), radius=1)
        tip_arc_label = Tex('180^\\circ - \\alpha - \\beta', t2c={'\\alpha': colors[0], '\\beta': colors[1]})
        tip_arc_label.next_to(tip_arc, LEFT, MED_SMALL_BUFF)
        self.play(LaggedStart(ShowCreation(tip_arc), FadeTransform(angle_labels[0][1].copy(), tip_arc_label['\\alpha'][0]), FadeTransform(angle_labels[1][1].copy(), tip_arc_label['\\beta'][0]), Write(tip_arc_label['180^\\circ']), Write(tip_arc_label['-']), run_time=2))
        self.wait()
        ...
        self.play(FadeIn(true_earth), FadeOut(earth_back), FadeOut(earth), FadeOut(earth_pattern), FadeOut(tip_arc_label), FadeOut(tip_arc), FadeOut(obs_lines), FadeIn(new_obs_lines))
        self.wait()
        ...
        for line, dot in zip(obs_lines, obs_dots):
            line.dot = dot
            line.add_updater(lambda m: m.put_start_and_end_on(m.dot.get_center(), obj.get_center()))
        angle_labels.add_updater(lambda m: m.become(self.get_angle_labels(obs_lines, obs_points=[obs_dots[0].get_center(), obs_dots[1].get_center()], line_between=line_between, arc_props=[0.75, 0.5])))
        ...
        self.play(obj.animate.move_to(moon), frame.animate.set_height(1.5 * moon_orbit.get_width()).move_to(moon_orbit.get_right()).set_field_of_view(35 * DEG), FadeIn(moon_orbit), run_time=5)
        self.wait(5)
        ...
        self.add(sun)
        self.play(frame.animate.set_height(0.4 * earth_orbit.get_width()).move_to(interpolate(venus_orbit.get_left(), sun.get_center(), 0.25)), FadeIn(earth_orbit, time_span=(2, 4)), FadeIn(venus_orbit, time_span=(2, 4)), obj.animate.move_to(venus_orbit.get_left()), run_time=8)
        self.wait(4)
        frame.save_state()
        frame.clear_updaters()
        if False:
            ...
            self.play(frame.animate.reorient(-62, -2, 0, (4.64, 1.98, 2.86), 15.8), FadeOut(moon_orbit, time_span=(3, 4)), FadeOut(moon, time_span=(3, 4)), FadeOut(earth_orbit, time_span=(3, 4)), run_time=5, rate_func=lambda t: smooth(rush_from(t)))
            ...
            self.wait()
        self.play(frame.animate.reorient(0, 1, 0, (3.02, 0.82, -0.03), 15.8), FadeOut(moon_orbit, time_span=(3, 4)), FadeOut(moon, time_span=(3, 4)), FadeOut(earth_orbit, time_span=(3, 4)), run_time=4)
        self.wait()
        self.add(angle_labels, obs_lines)
        self.play(Rotate(Group(pis[0], obs_dots[0]), 90 * DEG - angles[0], about_point=earth.get_center()), Rotate(Group(pis[1], obs_dots[1]), -90 * DEG - angles[1], about_point=earth.get_center()), MaintainPositionRelativeTo(labels[0], pis[0]), MaintainPositionRelativeTo(labels[1], pis[1]), UpdateFromFunc(line_between, lambda m: m.put_start_and_end_on(obs_dots[0].get_center(), obs_dots[1].get_center())), run_time=3)
        self.wait()
        self.play(frame.animate.reorient(-33, -9, 0, (20739.48, 3596.8, 5435.71), 33171.78), FadeIn(earth_orbit, time_span=(10, 12)), run_time=30, rate_func=lambda t: smooth(smooth(t)))
        new_orbits = VGroup((Circle(radius=r * conversion_factor) for r in [MERCURY_ORBIT_RADIUS, MARS_ORBIT_RADIUS, JUPITER_ORBIT_RADIUS, SATURN_ORBIT_RADIUS]))
        ...
        self.play(FadeIn(new_orbits, time_span=(0, 3)), frame.animate.reorient(-29, -41, 0, ORIGIN, 753807.38), celestial_sphere.animate.set_width(20 * JUPITER_ORBIT_RADIUS * conversion_factor), run_time=20)

    def get_angle_labels(self, obs_lines, obs_points, line_between, arc_props=[0.5, 0.5], arc_radius=0.5, colors=[BLUE, RED], backstroke_width=4):
        arc_radius = 0.5
        angle_syms = Tex('\\alpha \\beta')
...
class NearbyStars(InteractiveScene):
    ...
    def construct(self):
        ...
        dist_label.f_always.move_to(lambda: dist_line.get_center() + 0.5 * normalize(rotate_vector(dist_line.get_vector(), 90 * DEG)))
        self.play(FadeIn(dist_line, time_span=(0, 1)), FadeIn(dist_label, time_span=(0, 1)), Rotate(orbit, TAU, about_point=ORIGIN, rate_func=linear, run_time=10))
        self.wait()
        ...
        self.play(LaggedStart(*(ReplacementTransform(dist_label[t2][0], au_label[t1][i]) for t1, t2, i in zip('A.U.', ['A', 'stronomical', 'U', 'nit'], [0, 0, 0, 1])), lag_ratio=0.2))
        self.add(au_label)
        frame = self.frame
        self.play(Rotate(orbit, 90 * DEG), frame.animate.reorient(0, 0, 0, 7 * RIGHT, 14), run_time=2)
        frame.save_state()
        ...
        self.play(UpdateFromAlphaFunc(frame, lambda m, a: m.reorient(*interpolate(np.array([-74, 79, 0]), np.zeros(3), a), interpolate(earth.get_center(), 7 * RIGHT, srf(a)), np.exp(interpolate(np.log(2 * earth.get_height()), np.log(14), smooth(a)))), run_time=5), FadeIn(earth_glow, time_span=(2.5, 4.5)), FadeIn(orbit, time_span=(1, 4)), FadeIn(dist_line, time_span=(1, 4)), FadeIn(au_label, time_span=(4, 5)), FadeOut(earth), run_time=5)
        star = Group(ImageMobject('StarFourPoints').set_height(0.8).center(), GlowDot(color=WHITE).center())
        star[1].add_updater(lambda m: m.set_width(0.4 * (1 + math.sin(1.5 * self.time))))
        star.move_to(50 * RIGHT)
        obs_points = Group((TrueDot(point, radius=0.1).set_color(GREEN).make_3d() for point in [orbit.get_top(), orbit.get_bottom()]))
        obs_lines = VGroup((self.get_obs_line(obs_point, star) for obs_point in obs_points))
        obs_lines.set_stroke(WHITE, 2)
        ...
        self.add(star)
        self.play(ShowCreation(obs_lines[0], suspend_mobject_updating=True), FadeIn(obs_labels[0], 0.25 * UP), FadeIn(obs_points[0]))
        self.wait()
        self.play(Rotate(orbit, PI), run_time=2)
        self.play(ShowCreation(obs_lines[1], suspend_mobject_updating=True), FadeIn(obs_labels[1], DOWN), FadeIn(obs_points[1]))
        self.wait()
        ...
        self.wait()
        self.play(FadeOut(sample_obs_line), obs_lines.animate.set_stroke(opacity=1))
        ...
        self.play(UpdateFromAlphaFunc(star, lambda m, a: m.move_to(RIGHT * orbit_radius / math.tan(interpolate(curr_angle, 1e-05, there_and_back_with_pause(a)) / 2))), run_time=6)
        ...
        arc_label = Tex('\\theta / 2', font_size=60)
        arc_label.next_to(arc, LEFT, buff=SMALL_BUFF)
        self.play(ShowCreation(line_to_star), obs_lines.animate.set_stroke(width=1), FadeIn(dist_label, RIGHT))
        self.wait()
        self.play(ShowCreation(arc), Write(arc_label))
        self.play(FlashAround(arc_label, run_time=2))
        self.wait()
        self.play(Transform(obs_lines[0].copy().clear_updaters(), obs_lines[1].copy(), remover=True), run_time=2)
        self.wait()
        kw = dict(t2c={'\\text{A.U.}': BLUE, 'D': RED}, font_size=72)
        eq1, eq2 = equations = VGroup(Tex('\\tan\\left(\\theta / 2\\right) = {\\text{A.U.} \\over D}', **kw), Tex('\\theta = 2 \\cdot \\tan^{-1}\\left({\\text{A.U.} \\over D}\\right)', **kw))
        equations.arrange(DOWN, buff=LARGE_BUFF)
        equations.next_to(frame.get_top(), DOWN, buff=-0.5)
        equations.align_to(dist_label, LEFT)
        self.play(LaggedStart(frame.animate.shift(UP), Write(eq1['\\tan\\left(']), FadeTransform(arc_label.copy(), eq1['\\theta / 2'][0]), Write(eq1['\\right) = ']), FadeTransform(au_label.copy().clear_updaters(), eq1['A.U.'][0]), Write(eq1['\\over']), FadeTransform(dist_label.copy(), eq1['D'][0]), lag_ratio=0.25, run_time=3))
        self.wait()
        self.play(TransformMatchingTex(eq1.copy(), eq2, path_arc=90 * DEG, run_time=2))
        self.wait()
        ac_labels = VGroup((Text(text, font_size=60, t2c={'D': RED, 'A.U.': BLUE}) for text in ['Proxima Centauri', 'D = 40.17 trillion km', 'D = 268,553 A.U.']))
        ...
        rhs = Tex('= 2 \\cdot \\tan^{-1}\\left(1 \\over 268{,}553 \\right)', font_size=72)
        rhs.next_to(eq2, RIGHT)
        rhs.shift(shift_value)
        answer = Tex('=0.000413^\\circ', font_size=72)
        answer.next_to(rhs, RIGHT)
        answer_in_arc_seconds = Tex('\\approx 1.5 \\text{ arc-seconds}', font_size=72)
        ...
        self.play(LaggedStart(equations.animate.shift(shift_value), frame.animate.move_to(11 * RIGHT + 3 * UP).set_height(16), *(TransformFromCopy(eq2[tex][0], rhs[tex][0]) for tex in ['2 \\cdot \\tan^{-1}\\left(', '\\right)']), FadeIn(rhs['1 \\over']), FadeIn(rhs['=']), FadeTransform(ac_labels[2]['268,553'].copy(), rhs['268{,}553'].copy()), run_time=2, lag_ratio=0.1))
        self.wait()
        self.play(Write(answer))
        self.wait()
        self.play(FadeIn(answer_in_arc_seconds, DOWN))
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(line_to_star, dist_label, arc, arc_label, *ac_labels), shift=0.1 * DOWN, lag_ratio=0.25))
        obs_lines.resume_updating()
        self.play(star.animate.move_to(1000 * RIGHT), rate_func=lambda t: t ** 4, run_time=5)
