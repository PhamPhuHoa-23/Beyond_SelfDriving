...
SUN_RADIUS = 695700
MERCURY_ORBIT_RADIUS = 68050000.0
VENUS_ORBIT_RADIUS = 108200000.0
EARTH_ORBIT_RADIUS = 147300000.0
MARS_ORBIT_RADIUS = 228000000.0
CERES_ORBIT_RADIUS = 413000000.0
JUPITER_ORBIT_RADIUS = 761300000.0
SATURN_ORBIT_RADIUS = 1439000000.0
...
def get_sun(radius=1.0, near_glow_ratio=2.0, near_glow_factor=2, big_glow_ratio=4, big_glow_factor=1, big_glow_opacity=0.35):
...
def get_celestial_sphere(radius=1000, constellation_opacity=0.1):
    sphere = Group(TexturedSurface(Sphere(radius=radius, clockwise=True), 'hiptyc_2020_8k'), TexturedSurface(Sphere(radius=0.99 * radius, clockwise=True), 'constellation_figures'))
...
def get_planet_symbols(text, font_size=48):
    return Tex(f'\\{text}', additional_preamble='\\usepackage{wasysym}', font_size=font_size)
...
class PerspectivesOnEarth(InteractiveScene):
    ...
    def construct(self):
        ...
        label = brace.get_tex(f'R_E', font_size=24)
        VGroup(brace, label).rotate(90 * DEG, RIGHT, about_point=brace.get_bottom())
        dashed_lines = VGroup(DashedLine(brace.get_corner(OUT + RIGHT), earth.get_zenith(), dash_length=0.02), DashedLine(brace.get_corner(IN + RIGHT), earth.get_center(), dash_length=0.02))
        dashed_lines.set_stroke(WHITE, 2)
        frame.reorient(0, 90, 0, ORIGIN, 3.42)
        self.play(GrowFromCenter(brace), Write(label), *map(ShowCreation, dashed_lines))
        self.wait(4)
        ...
        earth.save_state()
        flat_earth = TexturedSurface(ParametricSurface(lambda u, v: ((1 - v) * math.cos(u), (1 - v) * math.sin(u), 0), u_range=(0, TAU), v_range=(0, 1), resolution=earth.resolution), 'EarthTextureMap', 'NightEarthTextureMap')
        rot = rotation_matrix_transpose(40 * DEG, (-1, 0, -0.5)).T
        flat_earth = earth.copy().apply_matrix(rot).stretch(0.01, 2)
        flat_earth.data['d_normal_point'] = flat_earth.get_points() + 0.001 * OUT
        self.play(Transform(earth, flat_earth, run_time=4), frame.animate.reorient(0, 74, 0, (-0.03, -0.14, 0.04), 3.42), light.animate.shift(10 * OUT + 10 * LEFT), run_time=2)
        ...
        self.add(orbit, moon)
        self.play(Restore(earth, time_span=(0, 3)), frame.animate.reorient(0, 0, 0, ORIGIN, 1.1 * orbit.get_height()), run_time=4)
        ...
        self.play(frame.animate.reorient(-67, 60, 0).set_height(3), interp_factor.animate.set_value(0.2), run_time=8)
        self.wait(4)
...
class SphericalEarthVsFlat(InteractiveScene):
    ...
    def construct(self):
        ...
        flat_earth = day_earth.copy().set_depth(0.01, stretch=True)
        flat_earth.data['d_normal_point'] = flat_earth.data['point'] + 0.001 * OUT
...
class SizeOfEarthRenewed(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(earth)
        earth.save_state()
        earth.stretch(0.001, 0)
        earth.data['d_normal_point'] = earth.get_points() + 0.001 * RIGHT
        earth.note_changed_data()
        frame.reorient(5, 0, -90, 2 * RIGHT)
        self.play(frame.animate.reorient(0, 0, -90, RIGHT), Restore(earth), run_time=3)
        ...
        def update_rays(rays):
            ...
            for ray, y in zip(rays, ys):
                ray.put_start_and_end_on(sun.get_center(), [math.sqrt(abs(self.radius ** 2 - y ** 2)), y, 0])
        rays.add_updater(update_rays)
        rays.set_z_index(-1)
        self.play(FadeIn(rays, shift=0.5 * LEFT, lag_ratio=0.02), run_time=2)
        self.play(frame.animate.reorient(-67, -7, 0, (0.92, 2.13, 3.94), 20.25), sun.animate.move_to(50 * RIGHT), run_time=3)
        self.play(sun.animate.move_to(1000 * RIGHT).set_anim_args(time_span=(0, 3)), frame.animate.reorient(-130, -14, 0, (2.66, 0.14, -0.69), 8.18).set_anim_args(time_span=(2, 7)), run_time=7)
        ...
        self.play(slice_tracker.animate.set_value(0), FadeIn(center_dot, scale=0.25), run_time=2)
        self.play(ShowCreation(h_line, rate_func=rush_into, run_time=2), FadeOut(rays, run_time=2), frame.animate.reorient(-205, -11, 0, (3.0, -0.0, 0.0), 6.5).set_anim_args(run_time=10))
        ...
        self.play(Rotate(earth_group, 2 * TAU, axis=earth_axis), FadeIn(axis_line, time_span=(0, 1)), FadeIn(rays, remover=True, time_span=(4, 15), rate_func=lambda t: there_and_back_with_pause(t, 9 / 11)), frame.animate.reorient(-91, -36, 0, (4.91, -1.44, 0.39), 11.01).set_anim_args(time_span=(4, 15), rate_func=lambda t: there_and_back_with_pause(t, 0.2)), run_time=15)
        ...
        cancer_label.move_to(op.add(math.cos(EARTH_TILT_ANGLE) * self.radius * IN, (math.sin(EARTH_TILT_ANGLE) * self.radius + 0.15) * earth_axis))
        ...
        self.play(Rotate(earth_group, TAU, axis=earth_axis), ShowCreation(tropic_of_cancer), axis_line.animate.set_stroke(opacity=0.5).set_anim_args(time_span=(0, 1)), run_time=12)
        self.play(frame.animate.reorient(-172, 0, 0, (3.05, -0.01, -0.01), 6.69), Write(cancer_label, time_span=(1, 3)), run_time=3)
        self.wait()
        self.play(Write(syene_name), GrowArrow(syene_arrow))
        self.play(frame.animate.reorient(-225, -8, 0, (3.05, -0.01, -0.01), 6.69), FadeOut(cancer_label, time_span=(0, 2)), run_time=4)
        self.wait()
        self.play(ShowCreation(d_line), frame.animate.reorient(-189, 6, 0, (3.05, -0.01, -0.01), 6.69), run_time=3)
        self.play(Write(alex_name), GrowArrow(alex_arrow))
        self.wait()
        ...
        arc_labels = VGroup()
        for tex in ['\\theta', '7^{\\circ}']:
            arc_label = Tex(tex, font_size=36)
            arc_label.flip()
            arc_label.next_to(arc, RIGHT, SMALL_BUFF)
            arc_labels.add(arc_label)
        arc_label = arc_labels[0]
        self.play(FadeIn(upper_ray), ShowCreation(arc), FadeIn(arc_label))
        self.wait()
        self.play(Transform(arc_label, arc_labels[1]))
        self.wait()
        self.play(arc.animate.shift(earth_point - arc.get_end()).shift(0.01 * LEFT).set_stroke(RED), arc_label.animate.next_to(earth_point, DR, buff=0.05), upper_ray.animate.set_stroke(width=1, opacity=0.5), run_time=2)
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(alex_name, alex_arrow, syene_name, syene_arrow)))
        self.play(frame.animate.reorient(-173, 0, 0, (2.91, 0.1, -0.01), 7.2), ShowCreation(circle), run_time=4)
        circle.apply_depth_test()
        self.play(FadeOut(arc_label, time_span=(3, 5)), frame.animate.reorient(110 - 360, -10, 0, (-0.41, -0.16, 3.6), 7.86), run_time=24)
    ...
    def old_material(self):
        ...
        arc_label = Tex('23.5^\\circ', font_size=36)
        arc_label.next_to(arc.pfp(0.65), UP, buff=0.15)
        self.play(Rotate(earth_group, -EARTH_TILT_ANGLE, axis=OUT), Rotate(axis_line, -EARTH_TILT_ANGLE, axis=OUT), Rotate(d_line, -EARTH_TILT_ANGLE, axis=OUT), FadeIn(v_line), VFadeIn(d_line), ShowCreation(arc), Write(arc_label), run_time=2)
        self.wait()
        self.add(rays, axis_line, earth_group)
        self.play(FadeOut(VGroup(v_line, d_line, arc, arc_label), time_span=(0, 1)), frame.animate.reorient(-138, -25, 0, (4.72, -0.66, -0.11), 10.77), run_time=12)
...
class AlBiruniEarthMeasurement(InteractiveScene):
    ...
    def construct(self):
        ...
        theta_label = Tex('\\theta', font_size=24)
        theta_label.next_to(arc, RIGHT, SMALL_BUFF)
        self.play(ShowCreation(line_of_sight))
        self.wait()
        self.play(ShowCreation(horizontal))
        self.play(Rotate(horizontal_copy, -theta, about_point=mountain_tip), ShowCreation(arc), Write(theta_label))
        ...
        self.play(earth_pattern.animate.set_fill(opacity=0.5), earth.animate.set_fill(opacity=0.2), ShowCreation(radius_line), Write(R_label))
        self.wait()
        ...
        self.play(ShowCreation(elbow), ShowCreation(hyp))
        self.wait()
        self.play(GrowFromCenter(hyp_brace), Write(hyp_label))
        self.wait()
        ...
        self.play(TransformFromCopy(arc, low_arc), TransformFromCopy(theta_label, low_theta_label))
        self.wait()
        frame = self.frame
        equation = Tex('R = (R + h)\\cos(\\theta)', font_size=36)
        equation.to_edge(UP, buff=0)
        self.play(frame.animate.shift(UP), LaggedStart(TransformFromCopy(R_label, equation['R'][0]), Write(equation['= ('][0]), TransformFromCopy(hyp_label, equation['R + h'][0]), Write(equation[')\\cos('][0]), TransformFromCopy(low_theta_label, equation['\\theta'][0]), Write(equation[')'][-1]), lag_ratio=0.25))
        self.wait()
        eq2 = Tex('R - R \\cos(\\theta) = h \\cos(\\theta)', font_size=36)
        eq3 = Tex('R = {h\\cos(\\theta) \\over 1 - \\cos(\\theta)}', font_size=36)
...
class LuarEclipse(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(earth)
        brace = Brace(earth, LEFT)
        label = brace.get_tex(f'2 R_E')
        re_label = Integer(2 * EARTH_RADIUS, unit=' km', font_size=36)
        re_label.next_to(label, DOWN, aligned_edge=RIGHT)
        dashed_lines = VGroup(DashedLine(brace.get_corner(UR), earth.get_top() + RIGHT, dash_length=0.03), DashedLine(brace.get_corner(DR), earth.get_bottom() + RIGHT, dash_length=0.03))
        dashed_lines.set_stroke(GREY, 2)
        frame.reorient(-2, 59, 0, (-0.08, 0.07, -0.08), 2.76)
        self.play(frame.animate.to_default_state(), LaggedStart(Animation(Mobject(), remover=True), Animation(Mobject(), remover=True), Animation(Mobject(), remover=True), FadeIn(brace, scale=100), FadeIn(label), FadeIn(dashed_lines), lag_ratio=0.5), run_time=3)
        self.wait()
        ...
        dist_question = TexText('Distance to the moon \\\\ $\\approx$ 385,000km $\\pm$ 21,000km', font_size=600)
        dist_question.add_updater(lambda m: m.next_to(radial_line.get_center(), DR, buff=1))
        trg_height = 1.2 * orbit.get_height()
        self.add(orbit, moon)
        self.play(frame.animate.set_height(trg_height), FadeIn(radial_line, time_span=(2, 3)), FadeIn(dist_question, time_span=(2, 3)), run_time=4)
        self.wait(20)
        ...
        shadow = Rectangle(width=3 * MOON_ORBIT_RADIUS * conversion_factor, height=2 * EARTH_RADIUS * conversion_factor)
        ...
        self.play(Rotate(orbit, -angle_of_vector(orbit.get_start()) - 1.25 * DEG), frame.animate.set_height(6 * EARTH_RADIUS * conversion_factor).move_to(MOON_ORBIT_RADIUS * conversion_factor * RIGHT), FadeOut(radial_line, time_span=(0, 2)), FadeOut(dist_question, time_span=(0, 2)), sunlight.animate.set_opacity(0.35), run_time=5)
        self.play(Rotate(orbit, 2.5 * DEG, about_point=ORIGIN), Rotate(moon, 2.5 * DEG, about_point=ORIGIN), run_time=12, rate_func=linear)
        self.wait()
        ...
        self.wait()
        time_label = Tex('~4 \\text{ hours}', font_size=60)
        time_label.next_to(brace_copy, RIGHT)
        self.play(FadeOut(label_copy, lag_ratio=0.1), FadeIn(time_label, lag_ratio=0.1))
        self.wait()
        ...
        month_arrow = Arc(5 * DEG, 350 * DEG, arc_center=2.0 * earth.get_right(), radius=0.9 * MOON_ORBIT_RADIUS * conversion_factor)
        ...
        self.play(frame.animate.move_to(MOON_ORBIT_RADIUS * conversion_factor * LEFT).set_height(1.35 * orbit.get_height()).set_anim_args(run_time=4), orbit_rate.animate.set_value(15).set_anim_args(run_time=4), time_label.animate.set_height(large_text_height, about_point=brace_copy.get_right()).set_anim_args(run_time=4), ShowCreation(month_arrow, time_span=(1, 5)), Write(month_label, time_span=(2, 3)))
        self.wait(32)

    def show_circles(self):
        radii = [10000.0, 100000.0]
        circles = VGroup()
    ...
    def get_red_moon(self):
        red_moon = TexturedSurface(Sphere(radius=MOON_RADIUS * conversion_factor), 'RedMoonTexture', 'DarkMoonTexture').set_opacity(0.8)
        return red_moon
...
class PenumbraAndUmbra(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(penumbra), FadeIn(penumbral_lines), FadeIn(penumbrum_word))
        self.add(*shadow_group, earth, umbrum_word, penumbrum_word)
        self.play(FadeIn(umbral_lines, lag_ratio=0), FadeIn(umbra, time_span=(0.5, 1)), FadeIn(umbrum_word, time_span=(0.5, 1)))
        self.wait()
        self.add(shadow_group, earth, umbrum_word, penumbrum_word)
        shift_vect = 100 * LEFT
        self.play(sun[:2].animate.shift(shift_vect), sun[2].animate.shift(5 * LEFT).scale(2), frame.animate.reorient(0, 0, 0, (4.21, 0.56, 0.0), 13.04), umbrum_word.animate.shift(2 * RIGHT), penumbrum_word.animate.scale(0.5, about_edge=DOWN).shift(2 * RIGHT), UpdateFromFunc(shadow_group, lambda m: m.become(self.get_umbral_lines(sun[0], earth))), run_time=8)
    ...
    def get_umbral_lines(self, circle1, circle2):
        ...
        umbral_lines = VGroup(Line(c1 + r1 * v1, cs1), Line(c1 + r1 * v2, cs1))
        umbral_lines.set_stroke(WHITE, 0.5)
        umbra = Polygon(cs1, c2 + r2 * v1, c2, c2 + r2 * v2)
        umbra.set_fill(BLACK, 1).set_stroke(width=0)
        penumbral_lines = VGroup(Line(cs2, c2 + r2 * v3).scale(10), Line(cs2, c2 + r2 * v4).scale(10))
        penumbral_lines.set_stroke(WHITE, 0.5)
        penumbra = Polygon(c2 + r2 * v3, penumbral_lines[0].get_end(), penumbral_lines[1].get_end(), c2 + r2 * v4)
...
class LineOfSight(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(frame.animate.set_height(MOON_ORBIT_RADIUS * conversion_factor * 2.25), ShowCreation(line), FadeIn(words, lag_ratio=0.1, time_span=(2, 4)), run_time=5)
        self.play(frame.animate.reorient(0, 0, 0, (-59.4, 0.01, 0.0), 2.83), line.animate.set_stroke(width=2), run_time=3)
        self.play(Rotate(line, -2 * angle, about_point=line.get_start()), run_time=2)
        self.wait()
        self.play(frame.animate.set_height(MOON_ORBIT_RADIUS * conversion_factor * 2.25).center(), line.animate.set_stroke(width=1), run_time=3)
        ...
        self.play(VFadeIn(timer, time_span=(0, 1)), VFadeIn(units, time_span=(0, 1)), ChangeDecimalToValue(timer, 24), Rotate(earth, -TAU, about_point=ORIGIN), Rotate(line, -TAU, about_point=ORIGIN), Rotate(words, -TAU, about_point=ORIGIN), Rotate(moon, TAU / 28, about_point=ORIGIN), Rotate(orbit, TAU / 28, about_point=ORIGIN), run_time=5)
        self.wait()
        self.play(*map(FadeOut, [timer, units, words, line]))
        self.play(Rotate(orbit, 90 * DEG, about_point=ORIGIN, rate_func=linear), Rotate(moon, 90 * DEG, about_point=ORIGIN, rate_func=linear), run_time=12)
        moon.f_always.move_to(orbit.get_end)
        orbit_ghost = orbit.copy().set_stroke(opacity=0.5)
        self.add(orbit_ghost)
        self.play(orbit.animate.stretch(0.9, 1).stretch(1.1, 0).shift(3 * earth.get_width() * LEFT), rate_func=there_and_back, run_time=5)
        self.play(frame.animate.reorient(80, 82, 0, moon.get_center(), 0.8), run_time=5)
        self.wait()
        self.play(frame.animate.reorient(0, 0, 0, ORIGIN, MOON_ORBIT_RADIUS * conversion_factor * 2.25), run_time=5)
        self.wait()
        ...
        for a in np.arange(0, 1, 1 / ratio):
            ...
            moons.add(lil_moon)
        self.play(FadeOut(orbit, run_time=3), FadeIn(moons, lag_ratio=0.05, run_time=5))
        self.wait()
        n_moons = 32
        ...
        self.remove(moon)
        self.play(LaggedStartMap(Restore, inner_moons, lag_ratio=0), FadeOut(moons), frame.animate.set_height(1.5 * small_orbit.get_height()).center().set_anim_args(time_span=(1, 5)), run_time=5)
...
class DistanceToSun(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(GrowFromCenter(sun_brace), Write(sun_radius_label))
        self.wait()
        self.play(ShowCreation(dist_line), Write(dist_label))
        self.wait()
        ratio = Tex('{R_S \\over D_S}')
        ratio.to_corner(UL)
        ratio.set_color(YELLOW)
        self.play(TransformFromCopy(sun_radius_label, ratio['R_S'][0]), TransformFromCopy(dist_label, ratio['D_S'][0]), Write(ratio['\\over'][0]))
        self.wait()
        orbit_radius = 2
        ...
        self.add(orbit, moon)
        self.play(FadeIn(orbit), FadeIn(moon), dist_line.animate.set_stroke(opacity=0), frame.animate.reorient(0, 0, 0, (-4.94, 0.05, 0.0), 1.83), run_time=4)
        self.wait()
        ...
        moon_ratio = Tex('{R_M \\over D_M}', font_size=10)
        moon_ratio.set_color(GREY_B)
        moon_ratio.next_to(frame.get_corner(UL), DR, buff=SMALL_BUFF)
        self.play(FadeIn(moon_ratio))
        self.play(GrowFromCenter(moon_brace), TransformFromCopy(moon_ratio['R_M'][0], moon_radius_label))
        self.play(ShowCreation(moon_dist_line), TransformFromCopy(moon_ratio['D_M'][0], moon_dist_label))
        self.wait()
        equals = Tex('=')
        ...
        self.play(frame.animate.reorient(-87.85, 89.126, 0, [-5.6279674, -0.0808167, 0.00590339], 0.03), sun[2].animate.set_opacity(0.15), *map(FadeOut, [moon_ratio, moon_dist_line, moon_dist_label, moon_brace, moon_radius_label, orbit]), run_time=5)
        self.wait()
        self.play(frame.animate.to_default_state(), FadeIn(orbit), run_time=3)
        VGroup(sun_radius_label, dist_label).set_color(YELLOW)
        self.play(GrowFromCenter(sun_brace), GrowFromCenter(sun_radius_label))
        (dist_line.set_stroke(opacity=1),)
        self.play(FadeIn(dist_line), FadeIn(dist_label, shift=0.25 * UP))
        self.wait()
        self.play(orbit.animate.stretch(0.8, 0).stretch(1.2, 1), UpdateFromFunc(Group(moon), lambda m: m.move_to(orbit.get_end())), rate_func=wiggle, run_time=4)
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(dist_line, dist_label, sun_brace, sun_radius_label), scale=0.5))
        sun.clear_updaters()
        ...
        self.play(MoveToTarget(sun), frame.animate.reorient(0, 0, 0, (365.67, 14.24, 0.0), 485.35), run_time=5)
        ...
        self.play(MoveToTarget(sun, time_span=(0, 2)), frame.animate.reorient(0, 0, 0, (-3.89, 0.03, 0.0), 4.73), run_time=4)
        ...
        self.play(FadeIn(words1, lag_ratio=0.1), Write(arrow1))
        self.wait()
        self.play(FadeIn(words2, lag_ratio=0.1), Write(arrow2), ShowCreation(our_half))
        self.wait()
        self.play(FadeIn(sub_words, 0.03 * DOWN))
        self.play(FadeOut(VGroup(words1, arrow1, words2, arrow2, sub_words)), frame.animate.reorient(0, 0, 0, (-6.23, 0.02, 0.0), 4.73).set_anim_args(run_time=2))
        ...
        moon.target.rotate(-45 * DEG)
        moon.target.data['d_normal_point'] = moon.target.data['point'] + 0.001 * DR
        self.play(MoveToTarget(moon))
        self.play(frame.animate.reorient(3, 63, 0, (-6.08, 1.44, -0.78), 2.88), run_time=3)
        self.play(Rotate(orbit_group, -TAU / 4, about_point=earth.get_center(), run_time=8, rate_func=there_and_back))
        self.play(Restore(moon, time_span=(0, 1)), frame.animate.reorient(0, 0, 0, (-4.8, 0, 0.0), 4.22), run_time=2)
        ...
        self.play(Rotate(orbit_group, 3 * TAU / 8, about_point=earth.get_center(), run_time=8), VFadeIn(question, time_span=(1, 3)))
        ...
        lines1 = VGroup(Line(sun[0].get_center(), earth.get_center()), Line(earth.get_center(), orbit.get_top()))
        lines2 = VGroup(Line(sun[0].get_center(), half_moon_point), Line(half_moon_point, earth.get_center()))
        ...
        self.play(FadeOut(question), FadeIn(not_here, scale=0.75), FadeIn(lines1), FadeIn(elbow1))
        self.wait()
        self.play(ReplacementTransform(lines1, lines2, time_span=(2, 3)), ReplacementTransform(elbow1, elbow2, time_span=(2, 3)), Rotate(orbit_group, get_half_moon_angle() - 90 * DEG, about_point=earth.get_center()), FadeOut(not_here, time_span=(0, 1)), run_time=3)
        half_moon_label.always.next_to(moon, UR, buff=0.025)
        self.play(FadeIn(half_moon_label))
        self.wait()
        self.play(frame.animate.reorient(-30, 87, 0, (-5.02, 1.67, 0.0), 0.35), FadeOut(lines2), FadeOut(elbow2), FadeOut(half_moon_label), run_time=3)
        self.wait()
        self.play(frame.animate.reorient(3, 0, 0, (-5.02, 1.71, 0.01), 0.36), our_half.animate.set_opacity(0.35), run_time=2)
        self.wait()
        lit_half = our_half.copy().set_opacity(0)
        lit_half.shift(0.01 * OUT)
        self.play(Transform(lit_half, lit_half.copy().rotate(90 * DEG, about_point=half_moon_point).set_color(YELLOW, 0.35), path_arc=90 * DEG), run_time=1)
        self.wait()
        self.play(FadeOut(lit_half), our_half.animate.set_opacity(0), FadeIn(half_moon_label), frame.animate.reorient(0, 0, 0, (-2.89, 0.13, 0.0), 6.83), FadeIn(elbow2), FadeIn(lines2), run_time=5)
        ...
        def get_diff_arc(radius=0.75, color=WHITE, stroke_width=3):
            return Arc(90 * DEG, get_half_moon_angle() - 90 * DEG, radius=radius, arc_center=earth.get_center()).set_stroke(color, stroke_width)
        ...
        self.play(ShowCreation(v_line), FadeIn(arc), FadeIn(theta_label))
        self.wait()
        self.play(Rotate(orbit_group, 10 * DEG, about_point=earth.get_center()), run_time=8)
        dist_eq = Tex('D_S = {D_M \\over \\sin(\\theta)}', t2c={'D_S': YELLOW, 'D_M': GREY_B})
        dist_eq.to_edge(UP)
        dist_eq.fix_in_frame()
        dist_line = Line(earth.get_center(), sun[0].get_center())
        dist_line.set_stroke(YELLOW, 2)
        dist_label = Tex('D_S', font_size=72)
        dist_label.next_to(dist_line, DOWN)
        self.play(frame.animate.reorient(0, 0, 0, (4.7, 0.17, 0.0), 14.93), ShowCreation(dist_line, time_span=(3, 5)), FadeIn(dist_label, RIGHT, time_span=(3, 5)), FadeIn(dist_eq, time_span=(3, 5)), run_time=5)
        self.wait()
        self.play(FadeOut(dist_eq, time_span=(0, 2)), frame.animate.reorient(0, 0, 0, (-6.0, 1.05, 0.0), 2.52), run_time=5)
        ...
        arrow = Arrow(est_words['6 hours'].get_bottom() + 0.05 * DOWN, disc_arc.get_center(), buff=0.025, path_arc=90 * DEG, thickness=1.5)
        arrow.set_fill(BLUE)
        self.play(FadeIn(question), FadeIn(arrow), ShowCreation(disc_arc))
        self.wait()
        self.play(FadeOut(question, 0.25 * UP), FadeIn(est_words, 0.25 * UP))
        self.wait()
        true_answer = TexText('True answer: $\\sim30$ minutes', font_size=15)
        true_answer.set_color(TEAL)
        true_answer.move_to(est_words, DL)
        self.play(FadeOut(est_words, 0.25 * LEFT, time_span=(1, 2)), FadeIn(true_answer, 0.25 * LEFT, time_span=(1.25, 2.25)), Rotate(orbit_group, 4 * DEG, about_point=earth.get_center()), UpdateFromAlphaFunc(disc_arc, lambda m, a: m.become(get_diff_arc(radius=orbit_radius, color=interpolate_color(BLUE, TEAL, a), stroke_width=5))), arrow.animate.set_color(TEAL).shift(0.05 * LEFT), run_time=3)
        self.wait()
        ...
        v_line_copies = VGroup((v_line.copy().align_to(brace_copy, RIGHT) for brace_copy in brace_copies))
        v_line_copies.stretch(0.5, 1, about_edge=DOWN)
        self.play(FadeOut(true_answer), FadeOut(arrow), FadeOut(disc_arc), FadeOut(dist_line), FadeOut(dist_label), Rotate(orbit_group, 1 * DEG - math.asin(1 / 20), about_point=earth.get_center()))
        self.play(frame.animate.reorient(0, 0, 0, (13.31, -0.69, 0.0), 26.92), FadeIn(brace, time_span=(0, 1)), LaggedStart((TransformFromCopy(brace, brace2, path_arc=45 * DEG) for brace2 in brace_copies), lag_ratio=0.1, time_span=(1, 5)), LaggedStartMap(FadeIn, v_line_copies, lag_ratio=0.1, time_span=(1.5, 5)), sun[2].animate.set_radius(50).set_glow_factor(2), run_time=5)
        self.wait()
        self.play(Rotate(orbit_group, math.asin(1 / 20) - math.asin(1 / 383), about_point=earth.get_center()), frame.animate.reorient(0, 0, 0, (364.45, -1.75, 0.0), 480.47), run_time=6)
        self.wait()
...
class HowManyEarthsInsideSun(InteractiveScene):
    ...
    def construct(self):
        ...
        stack.move_to(sun)
        self.play(ShowCreation(circle))
        self.play(LaggedStart((TransformFromCopy(circle, circle2) for circle2 in stack), lag_ratio=0.05))
        self.wait()
        ...
        self.play(Transform(sun, big_sun), frame.animate.reorient(0, 0, 0, (8.71, 0.01, 0.0), 89.7), circle.animate.set_stroke(width=1), Transform(stack, big_stack, lag_ratio=0.001), run_time=3)
...
class EarthAroundSun(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait(2)
        self.play(Rotate(sun, TAU, about_point=earth.get_center()), frame.animate.move_to(earth).set_height(16).set_anim_args(time_span=(0, 4)), run_time=10)
        self.play(FadeOut(sun_orbit))
        self.wait()
        self.play(frame.animate.move_to(sun).set_anim_args(time_span=(0, 4)), Rotate(earth, TAU, about_point=sun.get_center()), run_time=10)
        self.wait(3)
...
class NearestPlanets(InteractiveScene):
    ...
    def construct(self):
        ...
        radii = radius_conversion * np.array([MERCURY_ORBIT_RADIUS, VENUS_ORBIT_RADIUS, EARTH_ORBIT_RADIUS, MARS_ORBIT_RADIUS, JUPITER_ORBIT_RADIUS, SATURN_ORBIT_RADIUS])
        periods = [MERCURY_ORBIT_PERIOD, VENUS_ORBIT_PERIOD, EARTH_ORBIT_PERIOD, MARS_ORBIT_PERIOD, JUPITER_ORBIT_PERIOD, SATURN_ORBIT_PERIOD]
        ...
        for radius, period, color in zip(radii, periods, colors):
            ...
            orbits.add(orbit)
        self.add(*orbits)
        symbol_texs = ['\\mercury', '\\venus', '\\earth', '\\mars', '\\jupiter', '\\saturn']
        symbols = Tex(''.join(symbol_texs), additional_preamble='\\usepackage{wasysym}', font_size=16)
        ...
        self.play(frame.animate.reorient(0, 0, 0, ORIGIN, 15), run_time=30)
        if self.linger:
            self.wait(30)
        braces = VGroup()
        for orbit, symbol_tex, angle in zip(orbits, symbol_texs, np.linspace(90 * DEG, 0, len(symbol_texs))):
            ...
            sym = Tex(f'R{symbol_tex}', additional_preamble='\\usepackage{wasysym}', font_size=24)
            sym[1].scale(0.5)
        ...
        self.play(Succession(GrowFromCenter(main_brace), *(Transform(main_brace, b, rate_func=lambda t: smooth(clip(1.5 * t, 0, 1))) for b in braces[1:])), frame.animate.reorient(0, 61, 0, ORIGIN, 2.5).set_field_of_view(20 * DEG), run_time=6)
        self.wait()
        ...
        solids.next_to(rotate_vector(frame.get_corner(UL), frame.get_phi(), RIGHT), IN + RIGHT, buff=SMALL_BUFF)
        self.play(LaggedStartMap(FadeIn, solids, shift=0.25 * OUT, run_time=2, lag_ratio=0.25), FadeOut(symbols))
        self.wait(3)
        ...
        self.play(FadeIn(spheres[0]), ReplacementTransform(solids[0], target_solids[0]), frame.animate.reorient(-8, 72, 0, (0.0, 0.0, 0.0), 2.5), run_time=2)
        drop_sphere(1, run_time=2)
        self.wait(2)
        self.play(target_solids[0].animate.set_stroke(opacity=0.25), ReplacementTransform(solids[1], target_solids[1]), frame.animate.reorient(-2, 69, 0, ORIGIN, 2.92), solids[2:].animate.shift(LEFT + 0.1 * OUT), run_time=2)
        drop_sphere(2)
        self.play(target_solids[1].animate.set_stroke(opacity=0.25), ReplacementTransform(solids[2], target_solids[2]), frame.animate.reorient(-16, 69, 0, ORIGIN, 4), solids[3:].animate.shift(LEFT + 0.5 * OUT), run_time=2)
        drop_sphere(3)
        self.play(target_solids[2].animate.set_stroke(opacity=0.25), ReplacementTransform(solids[3], target_solids[3]), frame.animate.reorient(-31, 68, 0, ORIGIN, 9), solids[4:].animate.scale(5).shift(3 * LEFT + 2 * OUT), run_time=2)
        drop_sphere(4)
        self.play(target_solids[3].animate.set_stroke(opacity=0.25), ReplacementTransform(solids[4], target_solids[4]), frame.animate.reorient(-9, 69, 0, ORIGIN, 15), run_time=2)
        drop_sphere(5)
        self.play(frame.animate.reorient(-19, 72, 0, ORIGIN, 20), run_time=3)
        frame.clear_updaters()
        frame.add_ambient_rotation(3 * DEG)
        self.play(LaggedStart(*(sphere.clip_tracker.animate.set_value(0) for sphere in spheres[::-1]), lag_ratio=0.25, run_time=6))
        last_solid = target_solids[-1]
        for solid in (*target_solids, *target_solids, *target_solids):
            self.play(solid.animate.set_stroke(WHITE, 2, 1), last_solid.animate.set_stroke(WHITE, 1, 0.25))
            last_solid = solid
        frame.clear_updaters()
        orbits.suspend_updating()
        self.add(*spheres)
        self.play(*(mob.animate.scale(1.1).set_anim_args(rate_func=lambda t: wiggle(t, 5), time_span=(random.random(), 4 + random.random())) for mob in (*spheres, *orbits, *target_solids)))
        self.play(frame.animate.reorient(0, 0, 0, ORIGIN, 12), target_solids.animate.set_stroke(opacity=0.2), FadeOut(spheres), run_time=2)
        factors = np.random.uniform(0.8, 1.2, len(orbits))
        angles = np.random.uniform(0, TAU, len(orbits))
        self.play(*(orbit.animate.rotate(angle).stretch(factor, 0).stretch(1 / factor, 1).rotate(-angle).set_anim_args(rate_func=lambda t: wiggle(t, 7), time_span=(random.random(), 6 + random.random())) for orbit, factor, angle in zip(orbits, factors, angles)), FadeOut(target_solids))
        self.camera.light_source.move_to(ORIGIN)
        small_radius = 0.01
        planets = Group((Sphere(radius=small_radius * math.sqrt(orbit.get_radius()), color=orbit.get_color()).set_shading(0.25, 0.25, 1) for orbit in orbits))
        ...
        self.play(FadeOut(fading_orbits, shift=2 * LEFT, lag_ratio=0.2), FadeIn(planets), FadeIn(symbols), celestial_sphere.animate.set_opacity(0.25), frame.animate.set_height(4), run_time=3)
        ...
        self.play(frame.animate.set_height(6).move_to(DOWN), FadeIn(lines, time_span=(0, 1)), run_time=12)
        orbits.suspend_updating()
        earth = planets[2]
        self.play(frame.animate.reorient(82, 87, 0, earth.get_center(), 0.05), FadeOut(symbols), *(planet.animate.scale(0.1) for planet in non_earth_planets), celestial_sphere.animate.set_opacity(1), run_time=3)
        self.wait()
    ...
    def get_platonic_solids(self):
        ...
        tetra_verts = vertices = np.array([[0, 0, 1], [np.sqrt(8 / 9), 0, -1 / 3], [-np.sqrt(2 / 9), np.sqrt(2 / 3), -1 / 3], [-np.sqrt(2 / 9), -np.sqrt(2 / 3), -1 / 3]])
    ...
    def wireframe_from_points(self, points, n_neighbors):
        ...
        for point in points:
            ...
            lines.add(*(Line(point, points[index]) for index in indices[1:1 + n_neighbors]))
        return lines
...
class KeplersMethod(InteractiveScene):
    ...
    def construct(self):
        ...
        symbols = Tex('\\earth \\mars', additional_preamble='\\usepackage{wasysym}')
        symbols.scale(0.75)
        ...
        self.play(frame.animate.to_default_state().set_anim_args(time_span=(1.5, 6)), ShowCreation(line_to_mars, suspend_mobject_updating=True), mars.animate.scale(5), mars_glow.animate.scale(3), earth.animate.scale(5).set_anim_args(time_span=(4, 6)), FadeIn(earth_glow, time_span=(4, 5)), FadeIn(symbols, time_span=(4, 5)), run_time=6)
        self.wait()
        self.play(Rotate(earth, TAU, about_point=earth.get_center() + 0.4 * DOWN), MaintainPositionRelativeTo(mars, earth), frame.animate.set_height(9), run_time=4)
        self.play(mars.animate.move_to(interpolate(mars.get_center(), earth.get_center(), 0.5)), run_time=4, rate_func=wiggle)
        line_to_sun = self.get_line_between_bodies(earth, sun, YELLOW)
        line_to_sun.set_stroke(width=1)
        self.play(ShowCreation(line_to_sun, suspend_mobject_updating=True, run_time=2), line_to_mars.animate.set_stroke(width=1))
        self.wait()
        self.play(Rotate(earth, TAU, about_point=sun.get_center()), Rotate(mars, EARTH_ORBIT_PERIOD / MARS_ORBIT_PERIOD * TAU, about_point=sun.get_center()), run_time=10)
        ...
        self.play(LaggedStart(GrowFromPoint(brace, sun.get_center(), suspend_mobject_updating=True), FadeIn(q_marks, shift=0.25 * DOWN, suspend_mobject_updating=True)))
        self.play(earth.animate.shift(0.5 * LEFT), MaintainPositionRelativeTo(mars, earth), rate_func=lambda t: wiggle(t, 6), run_time=8)
        ...
        self.play(Rotate(earth, angle, about_point=sun.get_center()), Rotate(mars, EARTH_ORBIT_PERIOD / MARS_ORBIT_PERIOD * angle, about_point=sun.get_center()), FadeIn(earth_annulus, time_span=(3, 5)), FadeIn(mars_annulus, time_span=(4, 6)), run_time=12)
        self.play(FadeOut(earth_annulus), FadeOut(mars_annulus), celestial_sphere[0].animate.set_opacity(0.5), celestial_sphere[1].animate.set_opacity(0))
        ...
        self.play(FadeIn(pin, shift=0.5 * DR, rate_func=rush_into))
        self.wait()
        ...
        for sample_point in sample_points:
            self.play(earth.animate.move_to(sample_point), ghost_lines.animate.set_stroke(opacity=0.2), run_time=0.5)
        ...
        self.play(mars.animate.scale(1.2, about_point=ORIGIN), rate_func=wiggle, run_time=5)
        earth.clear_updaters()
        ghost_earths.clear_updaters()
        self.play(FadeOut(pin, UL), FadeOut(ghost_earths, lag_ratio=0.1, run_time=1), FadeOut(ghost_lines, lag_ratio=0.1, run_time=1), Rotate(mars, TAU, about_point=sun.get_center(), run_time=8))
        self.wait()
        ...
        self.play(FadeOut(ghost_lines), ghost_earths.animate.set_color(GREEN), FadeIn(mars_copy))
        self.play(Rotate(mars, 5 * DEG, about_point=sun.get_center()), Rotate(earth, MARS_ORBIT_PERIOD / EARTH_ORBIT_PERIOD * 5 * DEG, about_point=sun.get_center()), FadeIn(earth_glow))
        self.wait()
        ...
        self.play(Rotate(mars, 5 * DEG, about_point=ORIGIN, run_time=5, rate_func=wiggle))
        self.play(mars.animate.shift(LEFT), run_time=5, rate_func=wiggle)
        self.play(*map(FadeOut, [ghost_lines, ghost_earths, og_ghost_earths, earth_glow, mars_copy]))
        ...
        for n in range(30):
            ...
            self.play(ShowIncreasingSubsets(cluster), ShowIncreasingSubsets(mars_lines), *(dot.animate.set_color(BLUE).set_radius(0.15) for dot in last_cluster))
            self.play(Restore(rigid_group))
            last_cluster = cluster
            last_mars_lines = mars_lines
        earth.move_to(earth_orbit.get_start())
        self.play(FadeOut(last_mars_lines), FadeOut(last_cluster), FadeOut(clusters), FadeIn(earth), FadeIn(earth_glow), FadeIn(earth_symbol), ShowCreation(earth_orbit))
        mars.set_x(4.8)
        ...
        for n in range(5):
            ...
            if n == 0:
                self.play(mars.animate.move_to(mars_line.pfp(0.75)).set_anim_args(rate_func=wiggle), frame.animate.set_height(10), run_time=4)
                self.play(mars.animate.set_opacity(0), mars_glow.animate.set_opacity(0), mars_symbol.animate.set_opacity(0))
            if n < 4:
                self.play(Rotate(mars, TAU, about_point=sun.get_center()), earth_location_tracker.animate.increment_value(MARS_ORBIT_PERIOD / EARTH_ORBIT_PERIOD), run_time=2)
            else:
                self.play(mars.animate.set_opacity(1), mars_glow.animate.set_opacity(1), mars_symbol.animate.set_opacity(1))
    ...
    def show_time_series_measurments(self, earth, mars, sun, earth_glow, n_measurements=5, rotation_time=2):
        ghost_lines = VGroup()
        ghost_earths = Group()
        corner_counter = VGroup(Integer(687, include_sign=True, edge_to_fix=RIGHT), Text('Days'))
        ...
        for n in range(n_measurements):
            ...
            if n < n_measurements - 1:
                ...
                self.play(Rotate(mars, TAU, about_point=sun.get_center()), Rotate(earth, MARS_ORBIT_PERIOD / EARTH_ORBIT_PERIOD * TAU, about_point=sun.get_center()), ChangeDecimalToValue(corner_counter[0], 687), ghost_lines.animate.set_stroke(opacity=0.2).set_anim_args(time_span=(0, 1)), run_time=rotation_time)
            else:
                self.play(ghost_lines.animate.set_stroke(opacity=0.2), FadeOut(corner_counter))
        return (ghost_lines, ghost_earths)

    def show_earth_location_with_intersection(self, earth, mars, sun):
        lines = VGroup(self.get_line_between_bodies(mars, earth, RED, 1, 2), self.get_line_between_bodies(sun, earth, YELLOW, 1, 2))
    ...
    def bind_ghost_lines_to_mars(self, ghost_lines, ghost_earths, mars):
        ...
        for pair, dot in zip(ghost_lines, ghost_earths):
            ...
            dot.add_updater(lambda m: m.move_to(find_intersection(m.sun_line.get_start(), m.sun_line.get_vector(), m.mars_line.get_start(), m.mars_line.get_vector())))

    def get_rigid_lines_from_mars(self, mars, ghost_earths):
        mars_lines = VGroup((self.get_line_between_bodies(mars, dot, RED, 1, 1) for dot in ghost_earths))
...
class ShowCreationOfAllOrbits(KeplersMethod):
    ...
    def construct(self):
        ...
        for radius, period, color, ecc in zip(radii, periods, colors, eccentricities):
            ...
            orbits.add(orbit)
        symbol_texs = ['\\mercury', '\\venus', '\\earth', '\\mars', '\\jupiter', '\\saturn']
        symbols = Tex(''.join(symbol_texs), additional_preamble='\\usepackage{wasysym}', font_size=32)
        planet_dots = Group()
        symbols[-2:].scale(3)
        ...
        self.draw_orbit(earth_orbit, orbits[4], JUPITER_ORBIT_PERIOD, 2, run_time=10, added_anims=[frame.animate.set_height(18)])
    ...
    def draw_orbit(self, earth_orbit, orbit, period, n_samples=3, run_time=4, added_anims=[]):
        ...
        for n in range(n_samples):
            ...
            (lines.add(line),)
        self.play(FadeIn(earth_dots), ShowCreation(lines, lag_ratio=0.1, suspend_mobject_updating=True), FadeIn(orbit.dot), FadeIn(orbit.symbol))
        self.play(ShowCreation(orbit), UpdateFromFunc(orbit.dot, lambda m: m.move_to(orbit.get_end())), earth_year_tracker.animate.set_value(period / EARTH_ORBIT_PERIOD), *added_anims, run_time=run_time)
        self.play(FadeOut(lines), FadeOut(earth_dots), FadeOut(orbit.dot), FadeOut(orbit.symbol), orbit.animate.set_stroke(width=1))
...
class LightFromEarthToMoon(InteractiveScene):
    ...
    def get_pulse(self, velocity, low_x=-np.inf, high_x=np.inf, radius=0.2, max_stroke_width=6, tail_time=0.25, label_text=''):
...
class LightFromSunToEarth(LightFromEarthToMoon):
    ...
    def construct(self):
        ...
        orbits = VGroup((Circle(radius=radius * conversion_factor) for radius, color in zip(radii, colors)))
        glows = Group()
        np.random.seed(1)
        ...
        for orbit, angle, color in zip(orbits, angles, colors):
            ...
            glows.add(glow)
        symbol_texs = ['\\sun', '\\mercury', '\\venus', '\\earth']
        labels = Tex(''.join(symbol_texs), additional_preamble='\\usepackage{wasysym}', font_size=40)
        ...
        pulse = self.get_pulse(SPEED_OF_LIGHT * conversion_factor * RIGHT, low_x=sun.get_x(RIGHT), high_x=earth.get_x(LEFT), tail_time=20, label_text='Light')
...
class LightAcrossEarth(LightFromEarthToMoon):
    ...
    def construct(self):
        ...
        pulse = self.get_pulse(0.1 * SPEED_OF_LIGHT * conversion_factor * RIGHT, low_x=earth.get_x(LEFT) + 0.025, high_x=earth.get_x(RIGHT), tail_time=0.4)
        pulse.move_to(earth.get_corner(DL) + 0.5 * DOWN)
        v_lines = Group((DashedLine(earth.get_corner(UP + vect) + UP, earth.get_corner(DOWN + vect) + DOWN).set_stroke(WHITE, 1) for vect in [LEFT, RIGHT]))
...
class EarthAndVenus(InteractiveScene):
    ...
    def construct(self):
        ...
        orbits = VGroup((Circle(radius=radius).set_stroke(color, (0, 5)) for radius, color in zip(radii, colors)))
        ...
        self.play(*(Rotate(orbit, angle) for orbit, angle in zip(orbits, angles)), VFadeIn(line_of_sight, time_span=(8, 10)), run_time=20, rate_func=lambda t: 0.999 * t)
        line_of_sight.clear_updaters()
        self.play(frame.animate.reorient(-89, 82, 0, (-0.0, 0.01, -0.01), 0.49), run_time=5)
        self.play(line_of_sight.animate.rotate(-0.6 * DEG, about_point=earth.get_center()), run_time=6)
        self.wait()
...
class RadarToVenus(LightFromEarthToMoon):
    ...
    def construct(self):
        ...
        orbits = VGroup((Circle(radius=radius).rotate(angle).set_stroke(color, (0, 5)) for radius, color, angle in zip(radii, colors, angles)))
        glows = Group((GlowDot(orbit.get_start(), color=orbit.get_color()) for orbit in orbits))
        symbols = VGroup(*map(get_planet_symbols, ['venus', 'earth']))
        ...
        for _ in range(4):
            pulse = self.get_pulse(velocity=0.15 * (venus_point - earth_point), radius=0.05, max_stroke_width=2, low_x=earth_point[0], high_x=venus_point[0], tail_time=0.5)
