...
class TerenceLabel(InteractiveScene):
    ...
    def construct(self):
        ...
        subtitle = VGroup(Text('Professor of Mathematics at UCLA', font_size=42))
...
class IntroducingTao(InteractiveScene):
    ...
    def construct(self):
        ...
        labels = VGroup(Text('Age 10, with Paul Erdős'), Text('Age 13, Gold medal at the\nInternationalMath Olympiad'), Text('Age 31, Receiving the\n2006 Fields Medal'))
        for image, label in zip(images, labels):
            label.scale(0.6)
            label.next_to(image, DOWN)
            self.play(FadeIn(image, 0.25 * UP, scale=1.2), FadeIn(label, lag_ratio=0.1))
            self.wait(0.5)
        self.wait()
...
class AskingTaoForTopics(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Transform(tau, TauCreature('hooray').move_to(tau)), FadeIn(tau.get_bubble('How about the\nCosmic Distance Ladder?', bubble_type=SpeechBubble), lag_ratio=0.1, run_time=2), morty.change('tease', look_at=ORIGIN))
        self.play(Blink(morty))
        self.play(tau[:4].animate.stretch(0, 1, about_edge=DOWN), rate_func=squish_rate_func(there_and_back, 0.4, 0.6))
...
class TableOfContents(InteractiveScene):
    ...
    def construct(self):
        items = VGroup(Text('Rung 1: Earth'), Text('Rung 2: Moon'), Text('Rung 3: Sun'), Text('Rung 4: Shapes of orbits'), Text('Rung 5: Distances to planets'), Text('Rung 6: Speed of light'), Text('Rung 7: Nearby stars'), Text('Rung 8: Milky way'), Text('Rung 9: Nearby Galaxies'), Text('Rung 10: Distant galaxies'))
        ...
        self.play(*(item.highlight_point.animate.set_y(items[0].get_y()) for item in items), run_time=2)
        self.play(*(item.highlight_point.animate.set_y(-4) for item in items), run_time=20, rate_func=linear)
        self.play(*(item.highlight_point.animate.match_y(item) for item in items), run_time=2)
        self.wait()
        brace1 = Brace(items[:4], RIGHT)
        brace2 = Brace(items[4:], RIGHT)
        self.play(GrowFromCenter(brace1), *(item.highlight_point.animate.shift(DOWN) for item in items[4:]))
        self.wait()
        self.play(*(item.highlight_point.animate.shift(DOWN) for item in items[:4]), *(item.highlight_point.animate.shift(UP) for item in items[4:]), ReplacementTransform(brace1, brace2))
        self.wait()
...
class MainCharacterTimeline(InteractiveScene):
    ...
    def construct(self):
        frame = self.frame
        timeline = NumberLine((-500, 2000, 10), tick_size=0.05, longer_tick_multiple=2, big_tick_spacing=100, unit_size=1 / 50)
        numbers = timeline.add_numbers(range(-500, 2100, 100), group_with_commas=False, font_size=20, buff=0.15)
        ...
        characters = [('Aristotle', -384, -322, 0.2, BLUE_D), ('Eratosthenes', -276, -194, 0.2, BLUE_B), ('Aristarchus', -310, -230, 0.5, BLUE_C), ('Kepler', 1571, 1630, 0.2, RED_C), ('Copernicus', 1473, 1543, 0.2, RED_A), ('Brahe', 1546, 1601, 0.5, RED_E), ('James Cook', 1728, 1779, 0.2, GREEN_E), ('Edmond Halley', 1656, 1742, 0.5, GREEN_C), ('Ole Rømer', 1644, 1710, 0.2, BLUE_D), ('Huygens', 1629, 1695, 0.5, BLUE_C), ('Friedrich Bessel', 1784, 1846, 0.2, GREEN_B), ('Henrietta Leavitt', 1868, 1921, 0.5, RED_D), ('Edwin Hubble', 1889, 1953, 0.2, RED_C)]
        ...
        for name, start, end, offset, color in characters:
            ...
            dashes = VGroup(DashedLine(line.get_start(), timeline.n2p(start), dash_length=0.01), DashedLine(line.get_end(), timeline.n2p(end), dash_length=0.01))
            dashes.set_stroke(color, 1.5)
            line_group = VGroup(line, name_mob, dashes)
            character_labels.add(line_group)
        images = Group(ImageMobject('Head_of_Aristotle'), ImageMobject('Eratosthenes'), Square().set_opacity(0), ImageMobject('Kepler'), ImageMobject('Copernicus'), ImageMobject('TychoBrahe'), ImageMobject('JamesCook'), ImageMobject('EdmondHalley'), ImageMobject('OleRomer'), ImageMobject('ChristiaanHuygens'), ImageMobject('FriedrichBessel'), ImageMobject('HenriettaLeavitt'), ImageMobject('EdwinHubble'))
        ...
        frame.set_y(0.5)
        self.play(FadeIn(character_labels[0], lag_ratio=0.1))
        self.wait()
        self.play(FadeIn(character_labels[1], lag_ratio=0.1), FadeIn(images[1], 0.5 * UP), frame.animate.set_height(6).match_x(timeline.n2p(-175)).set_anim_args(run_time=3))
        self.wait()
        self.play(FadeIn(character_labels[2], lag_ratio=0.1), FadeOut(images[1], 0.5 * RIGHT))
        self.wait()
        kepler_label, copernicus_label, brahe_label = character_labels[3:6]
        kepler_image, copernicus_image, brahe_image = images[3:6]
        self.play(frame.animate.match_x(timeline.n2p(1600)), UpdateFromAlphaFunc(frame, lambda m, a: m.set_height(interpolate(6, 12, there_and_back(a)))), run_time=3)
        self.play(FadeIn(kepler_label, lag_ratio=0.1), FadeIn(kepler_image, 0.5 * UP))
        self.wait()
        self.play(FadeIn(copernicus_label, lag_ratio=0.1), FadeIn(copernicus_image, 0.5 * UP))
        self.wait()
        self.play(FadeIn(brahe_label, lag_ratio=0.1), FadeIn(brahe_image, 0.5 * UP), copernicus_image.animate.scale(0.5, about_edge=DL).shift(0.25 * RIGHT), kepler_image.animate.scale(0.5, about_edge=DR).shift(0.25 * DR))
        self.wait()
        self.play(brahe_image.animate.scale(0.5, about_edge=DOWN), kepler_image.animate.shift(0.2 * LEFT))
        ...
        self.play(frame.animate.match_x(cook_label).set_anim_args(run_time=2), FadeIn(cook_label, lag_ratio=0.1), FadeIn(cook_image, 0.5 * UP), images[3:6].animate.set_opacity(0.25), character_labels[3:6].animate.set_opacity(0.25))
        self.wait()
        self.play(FadeIn(halley_label, lag_ratio=0.1), FadeIn(halley_image, 0.5 * UP), cook_image.animate.scale(0.5, about_point=cook_label.get_corner(UR)).shift(0.1 * LEFT))
        self.wait()
        ...
        self.play(FadeIn(romer_label, lag_ratio=0.1), FadeIn(romer_image, 0.5 * UP), FadeOut(halley_image), MoveToTarget(halley_label), cook_image.animate.set_opacity(0.25).match_x(cook_label))
        self.wait()
        self.play(FadeIn(huygens_label, lag_ratio=0.1), FadeIn(huygens_image, 0.5 * UP), romer_image.animate.set_height(1.5).next_to(romer_label, UR, 0).shift(0.25 * LEFT), FadeOut(kepler_image), FadeOut(cook_image), FadeOut(cook_label))
        self.wait()
        ...
        venus_arrows = VGroup((Arrow(venus_words['r'][0].get_top(), dot.get_center(), buff=0.1, thickness=1.5).set_color(YELLOW) for dot in venus_points))
        venus_words.shift(0.1 * DL)
        self.play(Write(venus_words), FadeIn(venus_points), ShowCreation(venus_arrows))
        self.wait()
        ...
        self.play(frame.animate.match_x(bessel_label), FadeOut(venus_words), FadeOut(venus_arrows), venus_points.animate.set_opacity(0.5), Write(star_words), ShowCreation(star_arrow), FadeIn(star_point), huygens_image.animate.set_opacity(0.5), FadeOut(romer_image))
        self.play(FadeIn(bessel_label, lag_ratio=0.1), FadeIn(bessel_image, 0.5 * UP))
        self.wait()
        ...
        self.play(bessel_image.animate.set_opacity(0.25).scale(0.5, about_edge=DOWN), FadeOut(star_words), FadeOut(star_arrow), FadeIn(leavitt_label, lag_ratio=0.1), FadeIn(leavitt_image, 0.5 * UP), frame.animate.match_x(leavitt_label).set_height(5.0).set_anim_args(run_time=2))
        self.wait()
        self.play(FadeIn(hubble_label, lag_ratio=0.1), FadeIn(hubble_image, 0.5 * UP))
        self.wait()

class WhatVsHow(InteractiveScene):

    def construct(self):
        titles = VGroup(Text('What'), Text('How'))
        titles.scale(1.5)
        for title, vect in zip(titles, [LEFT, RIGHT]):
            title.move_to(vect * FRAME_WIDTH / 4)
            title.to_edge(UP, buff=0.25)
        v_line = Line(UP, DOWN).set_height(FRAME_HEIGHT)
        self.play(FadeIn(titles[0], 0.5 * UP), ShowCreation(v_line))
        self.wait()
...
class ProjectionTheorem(InteractiveScene):
    ...
    def construct(self):
        ...
        grid = NumberPlane((-width, width), (-width, width))
        ...
        frame.reorient(-30, 70, 0, 1.5 * OUT, 8.0)
        ...
        for _ in range(10):
            ...
            self.play(Rotate(sphere_group, random.uniform(0, TAU), axis=normalize(np.random.random(3))), FadeOut(projection))

    def show_projection(self, sphere):
        projection = sphere.copy()
        projection.stretch(0.05, 2)
        projection.set_z(0.01)
        circle = Circle(radius=1)
        circle.set_stroke(TEAL, 2)
        z = sphere.get_z()
        v_lines = VGroup((Line(circle.pfp(a) + z * OUT, circle.pfp(a)) for a in np.arange(0, 1, 1.0 / 12.0)))
        v_lines.set_stroke(WHITE, 1, 0.25)
        v_lines.apply_depth_test()
        self.play(TransformFromCopy(sphere, projection), *map(ShowCreationThenFadeOut, v_lines), run_time=1)
        self.add(circle)
        self.wait()
        self.remove(circle)
        return projection
...
class RouleauxTriangle(InteractiveScene):
    ...
    def construct(self):
        arcs = VGroup((Arc(30 * DEG + x * 120 * DEG, 60 * DEG, radius=1) for x in range(3)))
        ...
        self.play(*map(ShowCreation, v_lines), TransformFromCopy(shape, projection), run_time=2)
        self.wait()
        shape_center = np.array([0, shape.get_y(), 0])
        shape.add_updater(lambda m: m.move_to(shape_center))
        self.play(Rotate(shape, PI, run_time=6), run_time=6)
...
class WellOfSyene(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(Homotopy(wave, image, rate_func=linear), frame.animate.to_default_state(), GrowFromCenter(sun_spot, time_span=(5.5, 6.5)), run_time=12)
...
class NileLabels(InteractiveScene):
    ...
    def construct(self):
        ...
        line_label = TexText('$\\approx$ 5,000 Stadia')
        ...
        self.play(FadeIn(alex_point), FadeIn(alex_label, lag_ratio=0.1))
        self.play(ShowCreation(line, run_time=2), FadeIn(syene_point, time_span=(1, 2)), FadeIn(syene_label, lag_ratio=0.1, time_span=(1, 2)))
        self.wait()
...
class EarthSizeRatios(InteractiveScene):
    ...
    def construct(self):
        ...
        dist_term = equation['\\text{dist}(\\text{Alexandria}, \\text{Syene})'][0]
        self.play(FadeIn(equation['7^\\circ'], UL))
        self.play(Write(equation['\\over 360^\\circ}']))
        self.wait()
        self.play(LaggedStart(Write(equation['='][0]), FadeTransformPieces(equation['7^\\circ'][0].copy(), dist_term), FadeTransformPieces(equation['\\over'][0].copy(), equation['\\over'][1]), FadeTransformPieces(equation['360^\\circ'][0].copy(), equation['\\text{Circumference of Earth}'][0]), lag_ratio=0.1))
        self.wait()
...
class AccuracyLabel(InteractiveScene):
    ...
    def construct(self):
        label = VGroup(TexText('Estimate: $R_E = ...$'), TexText('True value: $R_E = 6{,}378$ km'))

class MoonOrbitCalculation(InteractiveScene):

    def construct(self):
        ratio_tex = '{\\text{28 days} \\over \\text{4 hours}}'
        new_ratio_tex = '{\\text{28 days} \\over \\text{3.5 hours}}'
        eq1, eq2 = (Tex(ratio_tex + '= {2 \\pi D_M \\over 2 R_\\text{E}}'), Tex(new_ratio_tex + '= {2 \\pi D_M \\over 2 R_\\text{E}}'))
        for eq in [eq1, eq2]:
            eq.to_corner(UL)
        ...
        self.add(eq1)
        self.wait()
        self.play(RMO.animate.set_color(TEAL), ShowCreation(RMO_rect), FadeIn(RMO_words, lag_ratio=0.1))
        self.wait()
        self.play(Transform(eq1['28 days'][0], eq2['28 days'][0]), Transform(eq1['\\over'][0], eq2['\\over'][0]), Transform(eq1['4'], eq2['3.5']))
        self.wait()
        eq3 = Tex('\\frac{1}{\\pi} \\left( ' + new_ratio_tex + '\\right) R_\\text{E} = D_M')
        eq3.next_to(eq2, DOWN, LARGE_BUFF, aligned_edge=LEFT)
        RMO2 = eq3['D_M']
        RMO2.set_color(TEAL)
        self.play(LaggedStart(TransformFromCopy(eq2[new_ratio_tex], eq3[new_ratio_tex]), FadeIn(eq3['\\left(']), FadeIn(eq3['\\right)']), TransformFromCopy(eq2['\\pi'], eq3['\\frac{1}{\\pi}']), TransformFromCopy(eq2['R_\\text{E}'], eq3['R_\\text{E}']), TransformFromCopy(eq2['='], eq3['=']), TransformFromCopy(eq2['D_M'], eq3['D_M']), lag_ratio=0.3, run_time=3))
        self.wait()
        frac = eq3['\\frac{1}{\\pi} \\left( ' + new_ratio_tex + '\\right)']
...
class FullLunarEclipseDistance(InteractiveScene):
    ...
    def construct(self):
        ...
        diam_brace.add(diam_brace.get_tex('2R_E', font_size=36))
        R_m_brace = Brace(Line(diam_line.get_top(), end_dot.get_center()), RIGHT)
        R_m_brace.add(R_m_brace.get_tex('2 R_M', font_size=36))
        ...
        self.play(FadeIn(start_words, lag_ratio=0.1), FadeIn(start_dot))
        self.wait(2)
        self.play(FadeIn(diam_brace))
        self.wait()
        self.play(FadeIn(end_words, lag_ratio=0.1), FadeIn(end_dot))
...
class ShowFourLittleCircles(InteractiveScene):
    ...
    def construct(self):
        ...
        lil_circle.move_to([-1.0, -0.167, 0.0])
        ...
        four_circles.move_to([1.503, -0.043, 0.0])
        self.play(ShowCreation(lil_circle))
        self.wait()
        self.play(LaggedStart((TransformFromCopy(lil_circle, circ) for circ in four_circles), lag_ratio=0.25, run_time=3))
        self.wait()
...
class MoonSizeCalculation(InteractiveScene):
    ...
    def construct(self):
        eq = Tex('{2 \\text{ minutes} \\over 24 \\text{ hours}} = {2 R_M\\over 2 \\pi D_M}', t2c={'R_M': GREY_B, 'D_M': GREY_B})
        eq.to_corner(UL)
        eq.set_backstroke(BLACK, 2)
        self.play(FadeIn(eq['{2 \\text{ minutes} \\over 24 \\text{ hours}}']))
        self.wait()
        self.play(LaggedStart(FadeIn(eq['='][0]), TransformFromCopy(eq['2 \\text{ minutes}'][0], eq['2 R_M'][0]), TransformFromCopy(eq['\\over'][0], eq['\\over'][1]), TransformFromCopy(eq['24 \\text{ hours}'][0], eq['2 \\pi D_M'][0]), lag_ratio=0.35))
        self.wait()
        ...
        self.play(ShowCreation(top_rect), FadeIn(top_label, lag_ratio=0.1))
        self.wait()
        self.play(ReplacementTransform(top_rect, low_rect), FadeOut(top_label), FadeIn(low_label, lag_ratio=0.1))
        self.wait()
...
class MoonSunRatios(InteractiveScene):
    ...
    def construct(self):
        equation = Tex('{R_M \\over D_M} = {R_S \\over D_S} \\approx \\frac{1}{220}', t2c={'R_M': GREY_B, 'D_M': GREY_B, 'R_S': YELLOW, 'D_S': YELLOW})
        equation.to_corner(UL)
        self.add(equation['{R_M \\over D_M}'])
        self.wait()
        self.play(LaggedStart(Write(equation['='][0]), TransformFromCopy(equation['R_M'][0], equation['R_S'][0]), TransformFromCopy(equation['\\over'][0], equation['\\over'][1]), TransformFromCopy(equation['D_M'][0], equation['D_S'][0]), lag_ratio=0.35))
        self.wait()
        self.play(Write(equation['\\approx \\frac{1}{220}']))
        self.wait()
...
class AristarchusDistanceEstimate(InteractiveScene):
    ...
    def construct(self):
        ...
        truth = TexText('True answer: $D_S \\approx 370 D_M$', t2c=t2c)
        guess.to_edge(UP)
...
class AristarchusSunSizeEstimate(InteractiveScene):
    ...
    def construct(self):
        ...
        truth = TexText('True answer: $R_S \\approx 109 R_E$', t2c=t2c)
        guess.to_edge(UP)
        truth.next_to(guess, DOWN)
...
class OtherGreeks(InteractiveScene):
    ...
    def construct(self):
        ...
        sub_images = Group((ImageMobject(Path(folder, 'stencil-luma-masks', f'figure-{n}-masked.png')) for n in range(1, 10)))
        for image in [base_image, *sub_images]:
            image.set_height(FRAME_HEIGHT)
        base_image.set_opacity(0.25)
        self.add(base_image)
        self.add(sub_images[0])
        self.play(*(FadeIn(im, rate_func=there_and_back_with_pause, time_span=(0.25 * n, 0.25 * n + 7)) for n, im in enumerate(sub_images)), run_time=8)
...
class EqualAreas1(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(UpdateFromAlphaFunc(arc, lambda m, a: m.become(self.get_slice(a * self.angle))), run_time=self.run_time)
        self.wait()
...
class EvenWithMathRight(InteractiveScene):
    ...
    def construct(self):
        words = TexText("Even when you have the math right\\\\you don't necessarily get to the truth", font_size=36)
        words.to_corner(UL)
...
class CopernicusConclusions(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(title)
        facts = BulletedList('proposed the planets\\\\go around the Sun', 'assumed orbits\\\\were circular', 'calculated all the\\\\orbital periods', buff=0.75, font_size=36)
        ...
        for fact in facts:
            ...
            self.wait()
        self.play(self.frame.animate.reorient(0, 0, 0, (0.78, -0.58, 0.0), 9.0), facts[:2].animate.scale(0.75, about_edge=UL).set_opacity(0.5), facts[2].animate.scale(1.25, about_edge=UL).shift(0.5 * UP))
        self.wait()
        periods = VGroup(Text('Mercury: 88 days'), Text('Venus: 225 days'), Text('Earth: 365 days'), Text('Mars: 687 days'), Text('Jupiter: 4,333 days'), Text('Saturn: 10,755 days'))
        periods.scale(0.75)
        periods.arrange(DOWN, aligned_edge=LEFT)
        periods.next_to(facts, DOWN, buff=MED_SMALL_BUFF, aligned_edge=LEFT)
        periods.shift(RIGHT)
        self.play(LaggedStartMap(FadeIn, periods, shift=0.35 * RIGHT, lag_ratio=0.15))
        self.add(periods)
        arrow = Vector(LEFT)
        arrow.set_fill(WHITE, 0)
        arrow.next_to(periods[0], RIGHT)
        for i, color in zip(range(2, 5), [BLUE, RED, ORANGE]):
            self.play(periods[:i].animate.set_fill(WHITE, 0.5), periods[i + 1:].animate.set_fill(WHITE, 0.5), periods[i].animate.set_fill(color, 1), arrow.animate.set_fill(color, 1).next_to(periods[i], RIGHT, SMALL_BUFF))
            self.wait()
...
class UniversalProblemSolvingTip(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(title)
        words = TexText("If you can't solve a problem, try \\\\ to solve a simpler problem instead", font_size=60, isolate='simpler problem ')
        words.next_to(title, DOWN, buff=1.5)
        self.play(FadeIn(words, lag_ratio=0.1, run_time=2))
        self.wait()
        self.play(FlashUnder(words['simpler problem'], buff=-0.025), words['simpler problem'].animate.set_color(YELLOW))
        self.wait()
...
class CountUpDates(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(label)
        self.play(day_tracker.animate.set_value(365), run_time=10, rate_func=smooth)
    ...
    def day_to_date_label(self, day_number, buff=MED_SMALL_BUFF):
        ...
        while days_remaining > month_days[month]:
            ...
            month += 1
        result = VGroup(Text(month_names[month]), Integer(days_remaining))
...
class ScaleIndicator(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(indicator)
        max_tim = 27
        self.play(power_tracker.animate.set_value(max_tim), run_time=3 * max_tim, rate_func=linear)

    def get_indicator(self, exponent, ref_width=2.0):
        low_power = int(exponent)
        high_power = int(exponent + 1)
        pieces = VGroup(self.get_piece(width=10 ** (low_power - exponent) * ref_width), self.get_piece(width=10 ** (high_power - exponent) * ref_width))
        labels = VGroup(self.get_label(low_power), self.get_label(high_power))
        opacities = [self.get_opacity(low_power - exponent), self.get_opacity(high_power - exponent)]
...
class Subscribe(InteractiveScene):
    ...
    def construct(self):
        ...
        logos = Group(SVGMobject('youtube_logo'), SVGMobject('email').set_fill(GREY_A), SVGMobject('x_logo').set_fill(GREY_C), ImageMobject('instagram_logo'), SVGMobject('bluesky_logo'), ImageMobject('facebook_logo'), SVGMobject('rss_logo'), Tex('\\dots', font_size=72))
        for logo in logos[:-1]:
        ...
        self.play(GrowArrow(arrow), FadeIn(url, DOWN), morty.change('tease'))
        self.play(Blink(morty))
        self.wait()
        n = len('Follow@3blue1brown')
        self.play(LaggedStart(FadeOut(arrow, LEFT), FadeOut(url, LEFT)), logos.animate.arrange_in_grid(2, 4, h_buff=0.25, v_buff=0.5).set_height(1.0).next_to(title, DOWN, 0.5).to_edge(LEFT, buff=0.5), title[:n].animate.to_edge(LEFT), FadeOut(title[n:], LEFT), run_time=2)
        self.wait()
