...
class StatsToVenus(InteractiveScene):
    ...
    def construct(self):
        title = TexText('Nearest distance to Venus $\\approx$ 39,000,000 km $\\approx 6{,}200 \\times R_E$')
        title.to_edge(UP)
        title.set_backstroke(BLACK, 3)
        self.play(Write(title['Nearest distance to Venus ']))
        self.wait()
        self.play(Write(title['$\\approx$ 39,000,000 km']))
        self.wait()
        self.play(Write(title['$\\approx 6{,}200 \\times R_E$']))
        self.wait()

class AngleDeviationForVenusParallax(InteractiveScene):

    def construct(self):
        title = TexText('Angle deviation $ = 2 \\tan^{-1}(1 / 6200) \\approx$ 1 arc-minute = $\\displaystyle \\frac{1}{60} \\cdot 1^\\circ$')
        title.to_edge(UP)
...
class SunInTheSky(InteractiveScene):
    ...
    def construct(self):
        ...
        deg_labels = VGroup((Tex(f'{n}^\\circ', font_size=2).next_to(protractor.n2p(60 * n), RIGHT, buff=0.01) for n in range(5)))
        protractor.add(deg_labels)
        self.add(sky_image)
        self.add(clean_sun)
        self.add(protractor)
        self.play(frame.animate.set_height(1.75 * pure_sun.get_height()).move_to(pure_sun).shift([0.1, 0.08, 0]), FadeIn(protractor, time_span=(2.5, 3.5)), FadeIn(pure_sun, time_span=(3, 4)), FadeIn(clean_sun, time_span=(4, 5)), FadeOut(sky_image, time_span=(3, 4)), run_time=5)
        self.wait()
        ...
        dashed_lines = VGroup(dashed_line.copy().align_to(protractor.n2p(0), DOWN), dashed_line)
        self.play(FadeIn(sun_label), ShowCreation(dashed_lines))
        self.wait()
        ...
        self.play(Restore(venus), frame.animate.reorient(0, 0, 0, (-0.5, 1.37, 0.0), 0.29), FadeIn(venus_line, time_span=(3, 4)), run_time=4)
        self.wait()
        labels = VGroup(Text('Northern Hemisphere\nObservation'), Text('Sourthern Hemisphere\nObservation'))
        labels.scale(1 / 36)
        ...
        for n in range(4):
            index = n % 2
            sign = -(2 * index - 1)
            self.play(FadeOut(labels[index]), FadeIn(labels[1 - index]), VGroup(venus, venus_line).animate.shift(sign * shift_value))
            self.wait()
        prop1 = 312.9 / 360
        prop2 = prop1 + 3 / 360
        p1 = sun_disk.pfp(prop1)
        p2 = sun_disk.pfp(prop2)
        lines = VGroup((DashedLine(point + 0.675 * sun_disk.get_width() * RIGHT, [protractor.n2p(0)[0], point[1], 0], dash_length=0.5 * dash_length).set_stroke(TEAL, 1) for point in [p1, p2]))
        self.play(FadeOut(pure_sun), FadeOut(clean_sun), FadeOut(venus_line), FadeOut(venus), FadeOut(labels), FadeIn(lines), sun_label.animate.scale(0.5, about_edge=LEFT), deg_labels[0].animate.scale(0.5, about_edge=LEFT))
        self.wait()
...
class ShowDuration(InteractiveScene):
    ...
    def construct(self):
        clock = VGroup(Integer(0, min_total_width=2), Text('h'), Integer(0, min_total_width=2), Text('m'), Integer(0, min_total_width=2), Text('s'))
        ...
        def update_clock(clock):
            time = int(time_tracker.get_value())
            clock[4].set_value(time % 60)
            clock[2].set_value(time // 60 % 60)
            clock[0].set_value(time // 3600)
...
class VenusTransitTimeline(InteractiveScene):
    ...
    def construct(self):
        frame = self.frame
        timeline = NumberLine((1500, 2000, 10), tick_size=0.05, longer_tick_multiple=2, big_tick_spacing=100, unit_size=1 / 25)
        ...
        self.play(GrowArrow(arrow), Write(year_label), FadeIn(dots[0], UP), frame.animate.set_height(6).move_to(dots[0].get_center() + 0.5 * UP).set_anim_args(run_time=2))
        self.wait()
        next_arrows = VGroup((Arrow(d1.get_center(), d2.get_center(), path_arc=angle, buff=0.05) for d1, d2, angle in zip(dots, dots[1:], [-PI, -90 * DEG])))
        next_labels = VGroup((Text(f'+{n} years', font_size=16).next_to(na, UP, SMALL_BUFF) for na, n in zip(next_arrows, [8, 105])))
        for na, label, d1, d2 in zip(next_arrows, next_labels, dots, dots[1:]):
            self.play(FadeIn(na), FadeIn(label, lag_ratio=0.1), TransformFromCopy(d1, d2, path_arc=-90 * DEG))
            self.wait()
...
class LabelIo(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(label)
        for _ in range(3):
            self.play(MoveAlongPath(point, orbit, run_time=7, rate_func=linear))

class TwoAU(InteractiveScene):

    def construct(self):
        braces = VGroup(Brace(Line(ORIGIN, 3 * RIGHT), UP), Brace(Line(3 * LEFT, ORIGIN), UP))
        labels = VGroup((brace.get_text('A.U.') for brace in braces))
        self.play(LaggedStartMap(GrowFromCenter, braces, lag_ratio=0.5), LaggedStartMap(FadeIn, labels, shift=0.25 * UP, lag_ratio=0.5))
        self.wait()
...
class CompareLightSpeedEstimates(InteractiveScene):
    ...
    def construct(self):
        words = VGroup(TexText("Huygens' Estimate: 212,000 km/s"), TexText('True speed: 299,792 km/s'))
...
class DemonstrateAnArcSecond(InteractiveScene):
    ...
    def construct(self):
        ...
        deg30_labels = VGroup((self.get_tick_label(tick, number) for tick, number in zip(deg10_ticks[0::3], range(0, 360, 30))))
        deg10_labels = VGroup((self.get_tick_label(tick, number, visibility_range=(60, 30)) for tick, number in zip([*deg10_ticks[-2:], *deg10_ticks[1:3]], [340, 350, 10, 20])))
        deg_labels = VGroup((self.get_tick_label(tick, number, visibility_range=(12, 6)) for tick, number in zip(deg_ticks[1:10], range(1, 10))))
        self.add(circle, deg_ticks, deg10_ticks)
        self.add(deg30_labels, deg10_labels, deg_labels)
        arc_minute_ticks = VGroup((self.get_tick_mark(circle, n * DEG / 60, 0.025, max_aparent_length=0.75) for n in range(0, 60)))
        arc_minute_labels = VGroup((self.get_tick_label(tick, number, unit='arc-minutes', frame_prop=0.025, visibility_range=(0.35 * deg_height, 0.15 * deg_height)) for tick, number in zip(arc_minute_ticks[1:20], range(1, 20))))
        self.add(arc_minute_ticks)
        self.add(arc_minute_labels)
        arc_second_ticks = VGroup((self.get_tick_mark(circle, n * DEG / 60 / 60, 0.00035, max_aparent_length=0.5) for n in range(0, 60)))
        arc_second_ticks.rotate(0.25 * DEG, about_point=zero_point)
        am_height = deg_height / 60
        arc_second_labels = VGroup((self.get_tick_label(tick, number, unit='arc-seconds', frame_prop=0.025, visibility_range=(0.35 * am_height, 0.15 * am_height), font_size=0.1) for tick, number in zip(arc_second_ticks[1:20], range(1, 20))))
        ...
        self.play(frame.animate.set_height(1.5 * deg_height).move_to(circle.pfp(0.5 * DEG / TAU)), run_time=8)
        self.wait(2)
        if False:
        ...
        self.play(frame.animate.set_height(1.5 * deg_height / 60).move_to(circle.pfp(0.5 * DEG / TAU / 60)), run_time=4)
        self.wait()
        op_tracker = ValueTracker(0)
        arc_second_labels.add_updater(lambda m: m.set_opacity(op_tracker.get_value()))
        self.play(VGroup(circle, arc_minute_ticks, arc_second_ticks).animate.scale(20, about_point=zero_point), op_tracker.animate.set_value(1).set_anim_args(time_span=(1, 3)), run_time=4)
        self.wait()

    def get_tick_mark(self, circle, angle, tick_length, max_aparent_length=0.5, stroke_color=WHITE, stroke_width=2):
    ...
    def get_tick_label(self, tick, number, unit='^\\circ', buff_ratio=0.5, font_size=360, visibility_range=(1000, 900), frame_prop=0.05):
...
class CompareTwoStars(InteractiveScene):
    ...
    def construct(self):
        frame = self.frame
        stars = Group(GlowDot(UR, color=WHITE, radius=0.2), GlowDot(DR, color=WHITE, radius=0.8))
        ...
        self.add(stars)
        self.wait()
        self.play(frame.animate.to_default_state(), FadeIn(randy, time_span=(1, 2)), run_time=3)
        lines = always_redraw(lambda: VGroup(DashedLine(randy.eyes[1].get_right(), stars[0].get_center()), DashedLine(randy.eyes[1].get_right(), stars[1].get_center())).set_stroke(WHITE, 1))
...
class InverseSquareLaw(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(op_tracker.animate.set_value(0), FadeIn(sphere), FadeIn(mesh))
        self.remove(radiation)
        self.wait()
        patch = ParametricSurface(lambda u, v: sphere.uv_func(u, v), u_range=(0 * DEG, 9 * DEG), v_range=(90 * DEG, 99 * DEG))
        patch.set_color(WHITE)
        beam, op_tracker = self.beaming_effect(patch)
        op_tracker.set_value(0)
        self.add(beam, sphere, mesh)
        self.play(op_tracker.animate.set_value(1), sphere.animate.set_opacity(0.1), mesh.animate.set_stroke(opacity=0.1), FadeIn(patch))
        self.wait(2)
        ...
        division = VGroup(ParametricCurve(lambda t: 2 * sphere.uv_func(t, 94.5 * DEG), t_range=(0 * DEG, 9 * DEG, DEG)), ParametricCurve(lambda t: 2 * sphere.uv_func(4.5 * DEG, t), t_range=(90 * DEG, 99 * DEG, DEG)))
        ...
        self.play(sphere_group.animate.scale(2), MoveToTarget(patch), frame.animate.reorient(65, 78, 0, ORIGIN, 8).set_anim_args(run_time=3), run_time=2)
        self.play(FadeIn(division))
        self.wait(6)
        self.play(patch.animate.scale(0.5, about_edge=IN + DOWN).set_opacity(0.5), FadeOut(division), run_time=2)
        self.play(frame.animate.reorient(103, 78, 0, (-0.0, 0.0, 0.0), 8.0), run_time=18)
...
class WriteInverseSquareLaw(InteractiveScene):
    ...
    def construct(self):
        eq = Tex('\\text{Apparent brightness} = \\text{const} \\cdot {\\text{Absolute brightness} \\over (\\text{distance})^2}')
        eq.to_edge(UP)
        eq.set_color(GREY_B)
        self.add(eq)
        self.wait()
...
class MeasuringNearbyStars(InteractiveScene):
    ...
    def construct(self):
        ...
        for n in range(10):
            ...
            self.play(ShowCreation(line), FadeIn(label, shift=0.25 * OUT), FadeOut(last_group))
            self.wait()
        ...
        self.play(FadeIn(axes), frame.animate.to_default_state(), stars.animate.set_points(random_points).set_radii(new_radii).set_glow_factor(0).make_3d().set_opacity(0.5), FadeOut(sun), run_time=3)
        self.play(stars.animate.set_points(sorted_by_color).set_anim_args(run_time=5, path_arc=30 * DEG), Write(x_axis_label, run_time=2))
        self.wait()
        self.play(stars.animate.set_points(fully_sorted).set_anim_args(run_time=5).set_opacity(0.25), Write(y_axis_label, run_time=2))
        self.wait()
        ...
        self.play(frame.animate.reorient(0, 0, 0, (-0.75, -1.45, 0.0), 5.71).set_anim_args(run_time=3), ShowCreation(ms_circle), Write(ms_label))
        self.wait()
        name = Text('Hertzsprung–Russell diagram')
        name.center().to_edge(UP)
        name.fix_in_frame()
        self.play(Write(name), frame.animate.reorient(0, 0, 0, 0.5 * UP, 9), run_time=2)
        self.wait()
        ...
        self.play(FadeOut(ms_label), FadeOut(ms_circle))
        self.play(stars.animate.set_opacity(0.02), FadeIn(example_star), FadeIn(glow))
        self.wait()
        self.play(example_star.animate.set_x(1).set_anim_args(run_time=2, rate_func=there_and_back), FlashAround(x_axis_label, run_time=1.5, color=RED, time_width=1.0))
        self.play(example_star.animate.set_y(2).set_anim_args(run_time=2, rate_func=there_and_back), FlashAround(y_axis_label, run_time=1.5, color=YELLOW, time_width=1.0))
        ...
        self.play(stars.animate.set_opacity(opacities), ShowCreationThenFadeOut(ms_circle), FadeIn(ms_label), example_star.animate.move_to(axes.c2p(0.25, 0.37)), run_time=2)
        self.wait(5)
        ...
        self.wait(6)
        self.play(FadeOut(ms_arrow), FadeOut(ms_label), FadeOut(example_star), FadeOut(glows), stars.animate.set_opacity(0.25))
        self.wait()
...
class LineToDistantStar(InteractiveScene):
    ...
    def construct(self):
        ...
        line_label = TexText('$\\sim$60,000 Light Years', font_size=36)
        ...
        self.play(ShowCreation(line), FadeIn(line_label, lag_ratio=0.1, time_span=(1, 3)), galaxy.animate.set_opacity(0.75), FadeIn(star), run_time=3)
        self.wait(12)
...
class SolarSpectrum(InteractiveScene):
    ...
    def construct(self):
        ...
        x_coords = VGroup((Integer(n, font_size=24).next_to(axes.x_axis.n2p(n), DOWN, MED_SMALL_BUFF) for n in range(0, 2800, 400)))
        x_label.set_z_index(1)
        self.add(axes)
        self.add(x_label)
        self.add(x_coords)
        lines = self.get_spectral_lines(axes, wavelength, avg_intensity)
        self.play(LaggedStart((Write(line, stroke_color=WHITE, stroke_width=0.1) for line in lines), lag_ratio=0.005, run_time=5))
        ...
        lump = axes.get_graph(func, x_range=(0.001, 2400, 12))
        ...
        origin = axes.c2p(0, 0)
        self.play(lines.animate.stretch(2, 0, about_point=origin).set_stroke(width=1), axes.x_axis.animate.stretch(2, 0, about_point=origin), *(coord.animate.stretch(2, 0, about_point=origin).stretch(0.5, 0) for coord in x_coords), run_time=3)
        gap_points = np.array([(-3.77, -1.1, 0.0), (-2.96, 1.09, 0.0), (-2.58, 2.51, 0.0), (-1.74, 3.0, 0.0)])
        ...
        self.wait()
        spectral_wavelengths = np.array([122, 103, 97, 656, 486, 430, 410, 1875, 1282, 1094])
        ...
        self.play(LaggedStartMap(ShowCreation, spectral_lines), FadeOut(arrows), lines.animate.set_stroke(opacity=0.2), self.frame.animate.reorient(0, 0, 0, (3.42, 0.49, 0.0), 12.0).set_anim_args(run_time=2), Write(hyd_words, run_time=3), x_label.animate.set_x(12.5))
        self.wait()
        ...
        self.play(GrowArrow(arrow), TransformFromCopy(spectral_lines, new_lines), spectral_lines.animate.set_stroke(width=1, opacity=0.5))
        self.wait()
        brace = Brace(VGroup(spectral_lines[3], new_lines[3]), UP, SMALL_BUFF)
        self.play(GrowFromCenter(brace), FadeOut(arrow))
        self.play(brace.animate.stretch(3, 0, about_edge=LEFT), new_lines.animate.shift(2 * shift_value), rate_func=there_and_back, run_time=5)

    def bucket_data(self, arr, bucket_size=10):
    ...
    def get_spectral_lines(self, axes, wavelengths, intensities, stroke_width=1):
        ...
        return VGroup((Line(axes.c2p(lam, 0), axes.c2p(lam, y), stroke_color=self.color_alpha_to_color(ca), stroke_width=stroke_width) for lam, y, ca in zip(wavelengths, rel_intensities, color_alpha)))

class LeavittLabel(InteractiveScene):

    def construct(self):
        text = TexText("From Leavitt's\\\\1912 Paper")
        text.to_corner(UL)
...
class GalacticSurveyData(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(-12, 129, 0, (-18.76, 7.73, -104.01), 439.75), rad_factor.animate.set_value(0.0025), run_time=12)
        self.wait()
        self.play(frame.animate.reorient(-115, 86, 0, ORIGIN, 1200), rad_factor.animate.set_value(0.002), run_time=10)
        self.play(frame.animate.reorient(107, 87, 0), run_time=15)
        self.play(frame.animate.reorient(17, 100, 0, (-69.76, -40.26, -81.8), 186.35), rad_factor.animate.set_value(0.003), run_time=10)
        self.play(frame.animate.reorient(-84, 85, 0, ORIGIN, 1100), rad_factor.animate.set_value(0.002), run_time=10)

class RungsUpToGalaxies(InteractiveScene):

    def construct(self):
        rungs = VGroup(Text('Distance to the sun'), Text('Stellar parallax'), Text('Main sequence fitting'), Text('Cepheid periods'), Text('Red shift'))
...
class ShrinkADime(InteractiveScene):
    ...
    def construct(self):
        frame = self.frame
        plane = NumberPlane((0, 1000, 1), (-4, 4), background_line_style=dict(stroke_color=BLUE, stroke_width=1), faded_line_style=dict(stroke_color=BLUE, stroke_width=0.5, stroke_opacity=0.25), faded_line_ratio=0)
        ...
        self.add(plane)
        self.play(frame.animate.reorient(-90, 84, 0, (-0.04, 0.01, -0.01), 8.0), run_time=2)
