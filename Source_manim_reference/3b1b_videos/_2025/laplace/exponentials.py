...
def get_exp_graph_icon(s, t_range=(0, 7), y_max=4, pos_real_scalar=0.1, neg_real_scalar=0.2, width=1, height=1):
    axes = Axes(t_range, (-y_max, y_max), width=width, height=height, axis_config=dict(tick_size=0.035, stroke_width=1))
...
class IntroduceEulersFormula(InteractiveScene):
    ...
    def construct(self):
        plane = ComplexPlane((-2, 2), (-2, 2), width=6, height=6)
        plane.background_lines.set_stroke(BLUE, 1)
        plane.faded_lines.set_stroke(BLUE, 0.5, 0.5)
        plane.to_edge(LEFT)
        plane.add_coordinate_labels([1, -1])
        i_labels = VGroup(Tex('i', font_size=36).next_to(plane.n2p(1j), UL, SMALL_BUFF), Tex('-i', font_size=36).next_to(plane.n2p(-1j), DL, SMALL_BUFF))
        plane.add(i_labels)
        self.add(plane)
        pi_color = RED
        ...
        self.play(ShowCreation(arc), t_tracker.animate.set_value(PI), VFadeIn(t_dec, time_span=(0, 1)), run_time=2)
        pi.move_to(t_dec, DR)
        self.play(FadeOut(t_dec), FadeIn(pi), run_time=0.5)
        formula = Tex('e^{\\pi i} = -1', font_size=90, t2c={'\\pi': RED, 'i': BLUE})
        ...
        self.play(LaggedStart(TransformFromCopy(pi, formula['\\pi'][0]), FadeTransform(i_labels[0].copy(), formula['i'][0]), Write(formula['='][0]), TransformFromCopy(plane.coordinate_labels[1], formula['-1'][0]), Write(formula['e'][0]), lag_ratio=0.2, run_time=3))
        self.wait()
        self.play(LaggedStart(*(FadeTransform(formula[c1][0].copy(), cliche[c2][0]) for c1, c2 in zip(['e', '1', 'i', 'e', '\\pi', 'e', '1'], 'Cliché?')), lag_ratio=0.1, run_time=3), VFadeIn(randy), randy.says('This again?', mode='sassy', bubble_direction=LEFT))
        self.play(Blink(randy))
        self.add(cliche)
        plane_group = VGroup(plane, arc, pi)
        plane_group.set_z_index(-1)
        thumbnails = Group((Group(ImageMobject(f'https://img.youtube.com/vi/{slug}/maxresdefault.jpg')) for slug in ['-dhHrg-KbJ0', 'f8CXG7dS-D0', 'ZxYOEwM6Wbk', 'LE2uwd9V5vw', 'CRj-sbi2i2I', 'v0YEaeIClKY', 'sKtloBAuP74', 'IUTGFQpKaPU']))
        ...
        self.play(FadeOut(randy.bubble, time_span=(0, 1)), randy.change('raise_left_hand', thumbnails).set_anim_args(time_span=(0, 1)), plane_group.animate.set_width(3.5).next_to(formula, DOWN, MED_LARGE_BUFF).set_anim_args(time_span=(0, 2)), FadeOut(cliche, 3 * RIGHT, lag_ratio=-0.02, time_span=(0.5, 2.0)), LaggedStartMap(FadeIn, thumbnails, shift=UP, lag_ratio=0.5), run_time=3)
        ...
        q_marks = Tex('???', font_size=90)
        VGroup(formula.target, q_marks).arrange(DOWN, buff=MED_LARGE_BUFF).center()
        self.play(MoveToTarget(thumbnails, lag_ratio=0.01, run_time=2), FadeOut(randy, DOWN), FadeOut(plane_group, DOWN), MoveToTarget(formula), Write(q_marks))
        self.wait()
        ...
        radius_label = Tex('1', font_size=24)
        radius_label.next_to(radius, DOWN, SMALL_BUFF)
        pi_label = Tex('\\pi').set_color(pi_color)
        pi_label.next_to(circle, UP, buff=SMALL_BUFF)
        circle_group = VGroup(circle, arc, radius_label, radius, pi_label)
        i_eq = Tex('i^2 = -1', t2c={'i': BLUE}, font_size=90)
        i_eq.move_to(circle).set_x(5)
        self.play(formula.animate.shift(DOWN), FadeOut(e_copy, 3 * UP + 5 * LEFT), FadeOut(q_marks, DOWN))
        self.play(TransformFromCopy(formula['\\pi'][0], pi_label), LaggedStartMap(FadeIn, VGroup(circle, radius, radius_label)), ShowCreation(arc))
        self.play(FadeTransform(formula['i'][0].copy(), i_eq['i'][0]), Write(i_eq[1:], time_span=(0.75, 1.75)))
        self.wait()
        i_rect = SurroundingRectangle(formula['i'], buff=0.05)
        i_rect.set_stroke(YELLOW, 2)
        q_marks = Tex('???', font_size=24)
        q_marks.match_color(i_rect)
        q_marks.next_to(i_rect, UP, SMALL_BUFF)
        self.play(ShowCreation(i_rect), FadeIn(q_marks, 0.25 * UP, lag_ratio=0.25))
        self.wait()
        frame = self.frame
        back_rect = FullScreenRectangle()
        back_rect.fix_in_frame()
        (back_rect.set_z_index(-1),)
        self.play(LaggedStartMap(FadeOut, VGroup(circle_group, i_eq, VGroup(i_rect, q_marks))), FadeOut(circle_group), frame.animate.set_y(-3.5), FadeIn(back_rect), formula.animate.set_fill(WHITE), run_time=2)
        self.wait()
...
class ExpGraph(InteractiveScene):
    ...
    def construct(self):
        ...
        title = Tex('\\frac{d}{dt} e^t = e^t', t2c={'t': GREY_B}, font_size=60)
        title.to_edge(UP)
        ...
        v_line = always_redraw(lambda: axes.get_v_line_to_graph(get_t(), graph, line_func=Line).set_stroke(RED, 3))
        height_label = Tex('e^t', font_size=42)
        height_label.always.next_to(v_line, RIGHT, SMALL_BUFF)
        height_label_height = height_label.get_height()
        height_label.add_updater(lambda m: m.set_height(min(height_label_height, 0.7 * v_line.get_height())))
        self.play(ShowCreation(v_line, suspend_mobject_updating=True), FadeIn(height_label, UP, suspend_mobject_updating=True))
        self.wait()
        tangent_line = always_redraw(lambda: axes.get_tangent_line(get_t(), graph, length=10).set_stroke(BLUE_A, 1))
        ...
        tan_v_line = always_redraw(lambda: v_line.copy().shift(v_line.get_vector() + unit_size * RIGHT))
        deriv_label = Tex('\\frac{d}{dt} e^t = e^t', font_size=42)
        deriv_label['\\frac{d}{dt}'].scale(0.75, about_edge=RIGHT)
        deriv_label_height = deriv_label.get_height()
        deriv_label.add_updater(lambda m: m.set_height(min(deriv_label_height, 0.8 * v_line.get_height())))
        deriv_label.always.next_to(tan_v_line, RIGHT, SMALL_BUFF)
        self.play(ShowCreation(tangent_line, suspend_mobject_updating=True))
        self.play(VFadeIn(unit_line), VFadeIn(unit_label), VFadeIn(tan_v_line, suspend_mobject_updating=True), TransformFromCopy(title, deriv_label))
        self.play(ReplacementTransform(v_line.copy().clear_updaters(), tan_v_line, path_arc=45 * DEG), FadeTransform(height_label.copy(), deriv_label['e^t'][1], path_arc=45 * DEG, remover=True))
        self.wait()
        for t in [2.35, 0, 1, 2]:
...
class DefiningPropertyOfExp(InteractiveScene):
    ...
    def construct(self):
        ...
        equation = Tex('\\frac{d}{d{t}} e^{t} = e^{t}', font_size=90, **tex_kw)
        exp_parts = equation['e^{t}']
        ddt = equation['\\frac{d}{d{t}}']
        self.play(Write(exp_parts[0]))
        self.wait()
        self.play(FadeIn(ddt, scale=2))
        self.play(Write(equation['=']), TransformFromCopy(*exp_parts, path_arc=PI / 2))
        self.wait()
        ode = Tex("x'(t) = x(t)", font_size=72, **tex_kw)
        ...
        self.play(FadeTransform(equation.copy(), ode), FadeIn(ode_label))
        self.wait()
        frame = self.frame
        abs_ic = Tex('x(0) = 1', font_size=72, **tex_kw)
        exp_ic = Tex('e^{0} = 1', font_size=90, t2c={'0': GREY_B})
        ...
        self.play(FadeIn(abs_ic, RIGHT), FadeIn(exp_ic, RIGHT), frame.animate.set_x(2), Write(ic_label))
        self.wait()
...
class ImaginaryInputsToTheTaylorSeries(InteractiveScene):
    ...
    def construct(self):
        plane = ComplexPlane((-6, 6), (-4, 4), background_line_style=dict(stroke_color=BLUE, stroke_width=1), faded_line_style=dict(stroke_color=BLUE, stroke_width=0.5, stroke_opacity=0.5))
        plane.set_height(5)
        ...
        pi_i_label = Tex('\\pi i', font_size=30).set_color(YELLOW)
        pi_i_label.next_to(dot, RIGHT, buff=-0.1).align_to(plane.n2p(3j), DOWN)
        self.add(dot, pi_i_label)
        false_eq = Tex('e^x = e \\cdot e \\cdots e \\cdot e', t2c={'x': BLUE}, font_size=60)
        false_eq.to_edge(UP).shift(2 * LEFT)
        brace = Brace(false_eq[3:], DOWN)
        brace_tex = brace.get_tex('x \\text{ times}')
        brace_tex[0].set_color(BLUE)
        nonsense = TexText('Nonsense if $x$ \\\\ is complex')
        ...
        epii = self.get_series('\\pi i', use_parens=True, in_tex_color=YELLOW)
        epii.next_to(gen_poly, DOWN, aligned_edge=LEFT)
        self.remove(false_eq)
        self.play(TransformFromCopy(false_eq[:2], gen_poly[0]), FadeOut(false_eq[2:], 0.5 * DOWN, lag_ratio=0.05), FadeOut(nonsense), FadeOut(brace, 0.5 * DOWN), FadeOut(brace_tex, 0.25 * DOWN), Write(gen_poly[1:]))
        self.wait()
        vectors = self.get_spiral_vectors(plane, PI)
        buff = 0.5 * SMALL_BUFF
        labels = VGroup(Tex('\\pi i', font_size=30).next_to(vectors[1], RIGHT, buff), Tex('(\\pi^2 / 2) \\cdot i^2', font_size=30).next_to(vectors[2], UP, buff), Tex('(\\pi^3 / 6) \\cdot i^3', font_size=30).next_to(vectors[3], LEFT, buff), Tex('(\\pi^4 / 24) \\cdot i^4', font_size=30).next_to(vectors[4], DOWN, buff))
        labels.set_color(YELLOW)
        labels.set_backstroke(BLACK, 5)
        for n in range(0, len(gen_poly), 2):
            anims = [LaggedStart(TransformMatchingTex(gen_poly[n].copy(), epii[n], run_time=1), TransformFromCopy(gen_poly[n + 1], epii[n + 1]), gen_poly[n + 2:].animate.align_to(epii[n + 2:], LEFT), lag_ratio=0.05)]
            k = (n - 1) // 2
            if k >= 0:
        ...
        for n in range(5):
            ...
            self.play(FadeIn(rect1), self.fade_all_but(epii, 2 * n + 2), self.fade_all_but(vectors, n), self.fade_all_but(labels, n), dot.animate.set_opacity(0.1))
            self.play(Transform(rect1, rect2))
            self.play(FadeOut(rect1))
        self.play(*(mob.animate.set_fill(opacity=1) for mob in [epii, vectors, labels]))
        self.wait()
        ...
        get_t = t_tracker.get_value
        t_label = Tex('t = 3.14', t2c={'t': GREEN})
        ...
        self.play(ReplacementTransform(epii, e_to_it, lag_ratio=0.01, run_time=2), FadeOut(labels), FadeIn(t_label))
        self.add(vectors)
        self.play(t_tracker.animate.set_value(0), run_time=5)
        self.play(t_tracker.animate.set_value(max_theta), ShowCreation(semi_circle), run_time=12)
    ...
    def get_series(self, in_tex='x', use_parens=False, in_tex_color=BLUE, buff=0.2):
        ...
        terms = VGroup(Tex(f'e^{{{in_tex}}}', **kw), Tex('='), Tex(f'1'), Tex('+'), Tex(f'{in_tex}', **kw), Tex('+'), Tex(f'\\frac{{1}}{{2}} {paren_tex}^2', **kw), Tex('+'), Tex(f'\\frac{{1}}{{6}} {paren_tex}^3', **kw), Tex('+'), Tex(f'\\frac{{1}}{{24}} {paren_tex}^4', **kw), Tex('+'), Tex('\\cdots', **kw), Tex('+'), Tex(f'\\frac{{1}}{{n!}} {paren_tex}^n', **kw), Tex('+'))
        terms.arrange(RIGHT, buff=buff)
        terms[0].scale(1.25, about_edge=DR)
        return terms

    def get_spiral_vectors(self, plane, t, n_terms=10, colors=[GREEN_E, GREEN_C, GREEN_B, GREEN_A]):
        values = [(t * 1j) ** n / math.factorial(n) for n in range(n_terms)]
        vectors = VGroup((Arrow(plane.n2p(0), plane.n2p(value), buff=0, fill_color=color) for value, color in zip(values, it.cycle(colors))))
...
class ComplexExpGraph(InteractiveScene):
    ...
    def construct(self):
        ...
        vector = Vector()
        vector.add_updater(lambda m: m.put_start_and_end_on(z_to_point(0), z_to_point(get_z())))
        ...
        self.play(frame.animate.reorient(*self.orientation2), t_tracker.animate.set_value(12).set_anim_args(rate_func=linear), VFadeIn(t_axis, time_span=(0, 1)), run_time=12)
        self.play(frame.animate.reorient(0, -89, -90, (0.06, -0.62, 5.27), 7.42).set_field_of_view(1 * DEG), FadeOut(plane), FadeOut(moving_plane), FadeOut(t_label), FadeOut(point), FadeOut(vector), run_time=3)
...
class SPlane(InteractiveScene):
    tex_to_color_map = {'s': YELLOW, 't': BLUE, '\\omega': PINK}
    s_plane_x_range = (-2, 2)
    s_label_font_size = 36
    s_label_config = dict(hide_zero_components_on_complex=True, include_sign=True, num_decimal_places=1)
    ...
    def construct(self):
        ...
        self.wait()
        frame = self.frame
        self.play_time_forward(3 * TAU, added_anims=[frame.animate.set_x(3).set_height(12).set_anim_args(time_span=(6, 15))])
        self.wait()
        self.play(t_tracker.animate.set_value(0), frame.animate.set_x(1.5).set_height(10), run_time=3)
        t2c = {'2i': YELLOW, **self.tex_to_color_map}
        exp_2it_label = Tex('e^{2i t}', t2c=t2c)
        exp_2it_label.move_to(exp_plane_label, RIGHT)
        self.play(s_tracker.animate.set_value(2j), run_time=2)
        self.play(FadeOut(exp_plane_label, 0.5 * UP), FadeIn(exp_2it_label, 0.5 * UP))
        self.play_time_forward(TAU)
        exp_plane.target = exp_plane.generate_target()
        exp_plane.target.align_to(axes.c2p(0, 0), LEFT)
        deriv_expression = Tex('\\frac{d}{dt} e^{2i t} = 2i \\cdot e^{2i t}', t2c=t2c)
        deriv_expression.next_to(exp_plane.target, RIGHT, aligned_edge=UP)
        self.play(LaggedStart(ReplacementTransform(exp_2it_label, deriv_expression['e^{2i t}'][0], path_arc=-90 * DEG), MoveToTarget(exp_plane), Write(deriv_expression['\\frac{d}{dt}']), lag_ratio=0.2), frame.animate.reorient(0, 0, 0, (1, 0, 0.0), 9.25))
        self.play(LaggedStart(Write(deriv_expression['=']), TransformFromCopy(*deriv_expression['e^{2i t}'], path_arc=-90 * DEGREES), FadeTransform(deriv_expression['e^{2i t}'][0][1:3].copy(), deriv_expression['2i'][1], path_arc=-90 * DEG), Write(deriv_expression['\\cdot']), lag_ratio=0.25, run_time=1.5))
        self.add(deriv_expression)
        self.wait()
        v_part, p_part, i_part, two_part = parts = VGroup(deriv_expression['\\frac{d}{dt} e^{2i t}'][0], deriv_expression['e^{2i t}'][1], deriv_expression['i'][1], deriv_expression['2'][1])
        colors = [GREEN, BLUE, YELLOW, YELLOW]
        labels = VGroup(Text('Velocity'), Text('Position'), Tex('90^{\\circ}'), Text('Stretch'))
        ...
        self.play(ShowCreation(v_part.rect), FadeIn(v_part.label), GrowArrow(v_vect))
        self.wait()
        self.play(ReplacementTransform(v_part.rect, p_part.rect), FadeTransform(v_part.label, p_part.label), GrowArrow(p_vect))
        self.wait()
        self.play(ReplacementTransform(p_part.rect, i_part.rect), FadeTransform(p_part.label, i_part.label), p_vect_copy.animate.rotate(90 * DEG, about_point=exp_plane.n2p(0)).shift(p_vect.get_vector()))
        self.wait()
        self.play(ReplacementTransform(i_part.rect, two_part.rect), FadeTransform(i_part.label, two_part.label), Transform(p_vect_copy, v_vect, remover=True))
        self.wait()
        self.play(FadeOut(two_part.rect), FadeOut(two_part.label))
        vectors.resume_updating()
        self.play_time_forward(TAU)
        imag_exp = Tex('e^{i \\omega t}', t2c=self.tex_to_color_map, font_size=60)
        imag_exp.move_to(deriv_expression, LEFT)
        self.play(FadeOut(deriv_expression, 0.5 * UP), FadeIn(imag_exp, 0.5 * UP))
        ...
        t_tracker.set_value(0)
        self.play(FadeOut(imag_exp, time_span=(0, 1)), s_tracker.animate.set_value(-0.2 + 1.5j), run_time=2)
        ...
        lines = VGroup(Tex('e^{(-0.5 + i)t}', t2c=t2c), Tex('\\left(e^{-0.5t} \\right) \\left(e^{it} \\right)', t2c=t2c))
        lines.arrange(DOWN, MED_LARGE_BUFF, aligned_edge=LEFT)
        lines.next_to(exp_plane, RIGHT, LARGE_BUFF, aligned_edge=UP)
        dec_brace = Brace(lines[1]['\\left(e^{-0.5t} \\right)'], DOWN, SMALL_BUFF)
        rot_brace = Brace(lines[1]['\\left(e^{it} \\right)'], DOWN, SMALL_BUFF)
        dec_label = dec_brace.get_text('Decay')
        rot_label = rot_brace.get_text('Rotation')
        self.play(FadeIn(lines[0], time_span=(0.5, 1)), FadeTransform(s_label[-1].copy(), lines[0]['-0.5 + i']))
        self.wait()
        self.play(TransformMatchingTex(lines[0].copy(), lines[1], run_time=1, lag_ratio=0.01))
        self.wait()
        self.play(GrowFromCenter(dec_brace), FadeIn(dec_label))
        self.wait()
        self.play(ReplacementTransform(dec_brace, rot_brace), FadeTransform(dec_label, rot_label))
        self.wait()
        self.play(FadeOut(rot_brace), FadeOut(rot_label), t_tracker.animate.set_value(0).set_anim_args(run_time=3))
        ...
        arc = Line(s_plane.n2p(0.3), s_plane.n2p(0.3 * get_s()), path_arc=s_vect.get_angle(), buff=0.1)
        arc.add_tip(length=0.25, width=0.25)
        times_s_label = Tex('\\times s')
        times_s_label.next_to(arc.pfp(0.5), UR, SMALL_BUFF)
        self.play(LaggedStart(FadeIn(one_vect), FadeIn(arc), FadeIn(times_s_label), FadeIn(s_vect)))
        self.wait()
        self.play(TransformFromCopy(one_vect, s_vect, path_arc=s_vect.get_angle()), run_time=2)
        self.wait()
        self.play(TransformFromCopy(one_vect, p_vect), TransformFromCopy(s_vect, v_vect), run_time=2)
        ...
        self.add(s_vect)
        values = [-0.1 + 2j, -0.1 - 2j, -0.1 + 0.5j, +0.05 + 0.5j, -0.5 + 0.5j, -0.1 + 0.5j]
        for value in values:
    ...
    def get_s_dot_and_label(self, s_plane, get_s):
        s_dot = Group(Dot(radius=0.05, fill_color=YELLOW), GlowDot(color=YELLOW))
        s_dot.add_updater(lambda m: m.move_to(s_plane.n2p(get_s())))
        s_label = Tex('s = +0.5', font_size=self.s_label_font_size)
    ...
    def get_exp_plane_label(self, exp_plane, font_size=60):
        label = Tex('e^{st}', font_size=font_size, t2c=self.tex_to_color_map)
    ...
    def get_output_dot_and_label(self, exp_plane, get_s, get_t, label_direction=UR, s_tex='s'):
        output_dot = Group(TrueDot(color=GREEN), GlowDot(color=GREEN))
        output_dot.add_updater(lambda m: m.move_to(exp_plane.n2p(np.exp(get_s() * get_t()))))
        output_label = Tex(f'e^{{{s_tex} \\cdot 0.00}}', font_size=36, t2c=self.tex_to_color_map)
    ...
    def get_graph_axes(self, x_range=(0, 24), y_range=(-2, 2), width=15, height=2):
        axes = Axes(x_range, y_range, width=width, height=height)
        x_axis_label = Tex('t', font_size=36, t2c=self.tex_to_color_map)
        y_axis_label = Tex('\\text{Re}\\left[e^{st}\\right]', font_size=36, t2c=self.tex_to_color_map)
    ...
    def get_graph_v_line(self, axes, get_t, get_s):
        ...
        v_line.f_always.put_start_and_end_on(lambda: axes.c2p(get_t(), 0), lambda: axes.c2p(get_t(), np.exp(get_s() * get_t()).real))
        return v_line
    ...
    def play_time_forward(self, time, added_anims=[]):
        self.t_tracker.set_value(0)
        self.play(self.t_tracker.animate.set_value(time).set_anim_args(rate_func=linear), *added_anims, run_time=time)
    ...
    def get_pv_vectors(self, exp_plane, get_t, get_s, thickness=3, colors=[BLUE, YELLOW]):
        ...
        p_vect.add_updater(lambda m: m.put_start_and_end_on(exp_plane.n2p(0), exp_plane.n2p(np.exp(get_t() * get_s()))))
        v_vect.add_updater(lambda m: m.put_start_and_end_on(exp_plane.n2p(0), exp_plane.n2p(get_s() * np.exp(get_t() * get_s()))).shift(p_vect.get_vector()))
    ...
    def setup_for_square_frame(self):
        ...
        self.frame.reorient(0, 0, 0, (1.0, 2.68, 0.0), 16.0)

    def old_material(self):
        output_dot = GlowDot(color=GREEN)
        output_dot.move_to(axes.x_axis.n2p(1))
        output_label = Tex('e^{s \\cdot 0.00}', **tex_kw, font_size=36)
        t_tracker.set_value(0)
        t_label = output_label.make_number_changeable('0.00')
        t_label.set_color(BLUE)
        ...
        self.remove(axes)
        self.play(LaggedStart(FadeOut(VGroup(axes.x_axis, x_axis_label, graph)), AnimationGroup(Rotate(axes.y_axis, -90 * DEG), TransformMatchingTex(y_axis_label, output_label, run_time=1.5)), FadeIn(output_dot), lag_ratio=0.5))
...
class FamilyOfRealExp(InteractiveScene):
    ...
    def construct(self):
        ...
        label = Tex('e^{st}', font_size=90)
        label.move_to(UP)
        label['s'].set_color(YELLOW)
        self.add(axes, label)
        self.play(ShowCreation(graph, suspend_mobject_updating=True))
        self.play(s_tracker.animate.set_value(-1), graph.animate.set_color(YELLOW), run_time=4)
        self.wait()

class ForcedOscillatorSolutionForm(InteractiveScene):

    def construct(self):
        exp_texs = [f'e^{{s_{n} t}}' for n in range(1, 5)]
        const_texs = [f'c_{n}' for n in range(1, 5)]
        ...
        solution[re.compile('s_\\w+')].set_color(YELLOW)
        solution[re.compile('c_\\w+')].set_color(BLUE)
        ...
        number_lines = VGroup((NumberLine((-3, 3), width=2).rotate(90 * DEG).next_to(solution[c_tex], DOWN) for c_tex in const_texs[:2]))
        ...
        def random_tuning_animation(run_time=2, lag_ratio=0.25):
            return LaggedStart(*(tracker.animate.set_value(random.uniform(-3, 3)) for tracker in c_trackers), lag_ratio=lag_ratio, run_time=run_time)
        self.play(FadeIn(number_lines), VFadeIn(tips), VFadeIn(c_labels), random_tuning_animation())
        for _ in range(6):
        ...
        s_values = [1.5j, -1.5j, -0.3 + 1j, -0.3 - 1j]
        s_dots = Group((GlowDot(plane.n2p(s)) for s in s_values))
        s_labels = VGroup((Tex(f's_{n}', font_size=24).set_color(YELLOW).next_to(dot, vect, buff=-0.1) for n, dot, vect in zip(it.count(1), s_dots, [RIGHT, RIGHT, LEFT, LEFT])))
        self.play(LaggedStart(FadeOut(number_lines, lag_ratio=0.1), FadeOut(tips, lag_ratio=0.1), FadeOut(c_labels, lag_ratio=0.1), FadeOut(VGroup(ex_mark, nope), LEFT), FadeIn(VGroup(checkmark, actually), LEFT), Restore(first_two), FadeIn(last_two, LEFT), run_time=2))
        self.play(FadeIn(plane), LaggedStartMap(FadeIn, s_dots), LaggedStart(*(FadeTransform(solution[f's_{n + 1}'].copy(), s_labels[n]) for n in range(4))))
        self.wait()
        const_rects = VGroup((SurroundingRectangle(solution[c_tex], buff=0.075) for c_tex in const_texs))
        const_rects.set_stroke(BLUE, 2)
        underlines = VGroup((Line(c1.get_bottom(), c2.get_bottom(), path_arc=40 * DEG) for c1, c2 in it.combinations(const_rects, 2)))
        underlines.set_stroke(TEAL, 2)
        underlines.insert_n_curves(10)
        underlines = VGroup((Vector(0.75 * UP, thickness=4).next_to(rect, DOWN, buff=0) for rect in const_rects))
        ...
        self.play(FadeIn(constraint_words, lag_ratio=0.1), FadeOut(checkmark), FadeOut(actually), LaggedStartMap(ShowCreation, const_rects, lag_ratio=0.25), LaggedStartMap(GrowArrow, underlines))
        ...
        if False:
            ...
            self.wait(24)
        term_rects = VGroup((SurroundingRectangle(solution[term], buff=0.1).set_stroke(TEAL, 2) for term in terms))
        s_rects = VGroup((SurroundingRectangle(solution[exp_tex][0][1:3], buff=0.05).set_stroke(YELLOW, 2) for exp_tex in exp_texs))
        moving_rects = const_rects.copy()
        self.remove(const_rects)
        anim_kw = dict(lag_ratio=0.25, run_time=1.5)
        self.play(FadeOut(constraint_words), FadeOut(underlines), Transform(moving_rects, term_rects, **anim_kw))
        self.wait()
    ...
    def get_exponential_diagram(self, term, s, c=1.0, color=PINK):
        ...
        vector.add_updater(lambda m: m.put_start_and_end_on(plane.n2p(0), plane.n2p(c * np.exp(s * get_t()))))
...
class BreakingDownFunctions(ForcedOscillatorSolutionForm):
    ...
    def construct(self):
        s_plane, out_plane = planes = VGroup(ComplexPlane((-2, 2), (-2, 2)), ComplexPlane((-2, 2), (-2, 2)))
        for plane in planes:
            plane.set_height(5)
            plane.add_coordinate_labels(font_size=16)
        ...
        exp_texs = [f'e^{{s_{n} t}}' for n in n_range]
        const_texs = [f'c_{n}' for n in n_range]
        terms = [' '.join(pair) for pair in zip(const_texs, exp_texs)]
        solution = Tex('x(t) = ' + ' + '.join(terms), isolate=[*exp_texs, *const_texs])
        solution[re.compile('s_\\w+')].set_color(YELLOW)
        solution[re.compile('c_\\w+')].set_color(BLUE)
        ...
        compact_equation = Tex('x(t) = \\sum_{n=1}^{N} c_n e^{{s_n} t}', t2c={'c_n': BLUE, 's_n': YELLOW}, isolate=['n=1', 'N'])
        ...
        s_plane_title = Text('S-plane', font_size=60)
        ...
        def update_exp_graph(exp_graph):
            ...
            anchors = np.array([out_plane.n2p(np.exp(s * t)) for t in np.arange(0, 100, 0.1)])
            exp_graph.set_points_as_corners(anchors)
        exp_graph.add_updater(update_exp_graph)
        self.play(LaggedStart(MoveToTarget(out_plane), LaggedStart(*(exp_diagram.animate.scale(scale_factor).move_to(out_plane.target) for exp_diagram in exp_diagrams), lag_ratio=0.01), AnimationGroup(*(ReplacementTransform(solution[t1], compact_equation[t2]) for t1, t2 in [('x(t) = ', 'x(t) = '), (re.compile('c_\\w+'), 'c_n'), (re.compile('s_\\w+'), 's_n'), ('e', 'e'), ('t', 't'), ('+', '\\sum_{n=1}^{N}')])), lag_ratio=0.2, run_time=2))
        exp_graph.update()
        self.play(FadeIn(s_plane), TransformFromCopy(compact_equation['s_n'][0], s_label), FadeTransform(compact_equation['s_n'][0].copy(), s_dot), Write(s_plane_title), FadeOut(exp_diagrams[2]), FadeIn(exp_graph, suspend_mobject_updating=True), compact_equation_start.animate.set_opacity(0.4))
        self.remove(exp_diagrams)
        self.wait()
        arrows = VGroup((Arrow(s_plane.n2p(0), s_plane.n2p(z), thickness=4, fill_color=GREY_A) for z in [2, -2, 2j, -2j]))
        arrows.set_fill(GREY_A, 1)
        arrow_labels = VGroup(Text('Growth', font_size=36).next_to(arrows[0], UP, buff=0), Text('Decay', font_size=36).next_to(arrows[1], UP, buff=0), Text('Oscillation', font_size=36).rotate(-90 * DEG).next_to(arrows[3], RIGHT, buff=SMALL_BUFF))
        ...
        self.play(s_dot.animate.shift(0.2 * RIGHT).set_anim_args(run_time=1), GrowArrow(arrows[0]), FadeIn(arrow_labels[0]))
        self.play(s_dot.animate.shift(0.2 * LEFT).set_anim_args(run_time=1), GrowArrow(arrows[1]), FadeIn(arrow_labels[1]))
        self.play(GrowArrow(arrows[2]), GrowArrow(arrows[3]), FadeIn(arrow_labels[2]), VFadeIn(rot_vect))
        self.wait()
        ...
        self.play(FadeOut(arrows), FadeOut(arrow_labels), FadeOut(out_plane), FadeOut(exp_graph), FadeIn(s_dots, lag_ratio=0.5), FadeOut(s_dot), FadeOut(s_label), compact_equation.animate.set_height(2.0).set_opacity(1).next_to(s_plane, RIGHT, LARGE_BUFF), frame.animate.match_y(s_plane))
        self.wait()
        inf = Tex('\\infty', font_size=60)
        N = compact_equation['N'][0]
        inf.move_to(N)
        dot_line = Group((GlowDot(s_plane.n2p(complex(-0.5, b))) for b in np.linspace(-2, 2, 25)))
        self.play(FlashAround(N, time_width=1), FadeTransform(N, inf, path_arc=20 * DEG), ShowIncreasingSubsets(dot_line, run_time=5, rate_func=linear), ReplacementTransform(s_dots, dot_line[:len(s_dots)].copy().set_opacity(0)))
        integral_eq = Tex('x(t) = \\int_{\\gamma} c(s) e^{st} ds', t2c={'s': YELLOW, '\\gamma': YELLOW})
        ...
        self.play(LaggedStart(*(FadeTransform(compact_equation[t1], integral_eq[t2]) for t1, t2 in [('x(t) = ', 'x(t) ='), ('\\sum_{n=1}^{N}', '\\int_{\\gamma}'), ('n=1', '\\gamma'), ('c_n', 'c(s)'), ('e^{{s_n} t}', 'e^{st}')])), FadeIn(integral_eq['ds']), LaggedStartMap(FadeOut, dot_line, lag_ratio=0.5, scale=0.25, time_span=(0.3, 2)), VShowPassingFlash(thick_line, run_time=2), ShowCreation(line, run_time=2))
        self.wait()
...
class Thumbnail(InteractiveScene):
    ...
    def construct(self):
        ...
        curve = ParametricCurve(lambda t: plane.n2p(np.exp(s * t)), t_range=(0, 40, 0.1))
        partial_curve = ParametricCurve(lambda t: plane.n2p(np.exp(s * t)), t_range=(0, max_t, 0.1))
        ...
        self.add(dot)
        vectors = VGroup((self.get_vector(plane, s, t, color=BLUE) for t in np.linspace(0, max_t, 10)))
        self.add(vectors)
    ...
    def get_formula(self):
        formula = Tex('e^{st}', t2c={'s': YELLOW, 't': BLUE}, font_size=200)
...
class Thumbnail2(InteractiveScene):
    ...
    def construct(self):
        ...
        arrow = Arrow(plane.n2p(1), plane.n2p(z), buff=0, path_arc=theta, thickness=12, fill_color=YELLOW)
        ...
        n_vects = 8
        vectors = VGroup((Vector(2.5 * UP, thickness=8).set_fill(vect_color, opacity ** 2).put_start_on(plane.n2p(1)).rotate(phi, about_point=plane.n2p(0)) for phi, opacity in zip(np.linspace(0, theta, n_vects), np.linspace(0.5, 1, n_vects))))
        ...
        theta_label = Tex('\\theta', font_size=72)
...
class RecapSPlane(SPlane):
    ...
    def construct(self):
        ...
        axes_background.align_to(s_plane.get_right(), LEFT).shift(0.01 * RIGHT)
...
class DefineSPlane(SPlane):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(0, 0, 0, (0.49, 0.34, 0.0), 9.65).set_anim_args(time_span=(0, 2)), FlashAround(s_plane, time_width=1.5, buff=MED_SMALL_BUFF, stroke_width=5), run_time=4)
        self.wait()
        ...
        exp_pieces = VGroup((get_exp_graph_icon(s).move_to(s_plane.n2p(0.85 * s)) for s in s_samples))
        dots = VGroup((Dot(radius=0.1).move_to(piece) for piece in exp_pieces))
        dots.set_color(YELLOW)
        self.play(LaggedStartMap(FadeIn, dots, scale=5, lag_ratio=0.05, run_time=2), VFadeOut(output_label), FadeOut(output_dot), VFadeOut(s_label), FadeOut(s_dot))
        t_tracker.clear_updaters()
        self.wait()
        self.play(LaggedStart((FadeTransform(dot, piece) for dot, piece in zip(dots, exp_pieces)), lag_ratio=0.05, run_time=3, group_type=Group))
        self.wait()
        ...
        self.play(rows[2].animate.fade(0.7), Restore(rows[1]), Restore(rows[3]))
        self.play(Restore(rows[0]), Restore(rows[4]), rows[1].animate.fade(0.7), rows[3].animate.fade(0.7))
        self.wait()
        ...
        for col in cols:
            self.play(last.animate.fade(0.7), Restore(col))
            last = col
...
class BreakingDownCosine(ShowFamilyOfComplexSolutions):
    ...
    def construct(self):
        left_planes, left_plane_labels = self.get_left_planes(label_texs=['e^{+it}', 'e^{-it}'])
        ...
        for i in [0, 1]:
            ...
            self.wait(3)
        self.wait(15)
        self.play(*(vs.coef_tracker.animate.set_value(0.5) for vs in vect_sum), LaggedStartMap(GrowArrow, inner_arrows, lag_ratio=0.01, run_time=1))
