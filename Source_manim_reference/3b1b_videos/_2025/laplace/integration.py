...
def get_complex_graph(s_plane, func, min_real=None, pole_buff=0.001, color_by_phase=True, opacity=0.7, shading=(0.1, 0.1, 0.1), resolution=(301, 301), saturation=0.5, luminance=0.5, face_sort_direction=UP, mesh_resolution=(61, 61), mesh_stroke_style=dict(color=WHITE, width=1, opacity=0.15)):
    ...
    graph = ParametricSurface(lambda u, v: [*s_plane.c2p(u, v)[:2], unit_size * abs(func(complex(u, v)))], u_range=u_range, v_range=v_range, resolution=resolution)
    graph.set_shading(*shading)
    if color_by_phase:
        graph.color_by_uv_function(lambda u, v: z_to_color(func(complex(u, v)), sat=saturation, lum=luminance))
...
class IntegrateConstant(InteractiveScene):
    ...
    def construct(self):
        ...
        integral = Tex('\\int^{0.01}_0 1 \\, dt = 0.01', font_size=72)
        ...
        rect.add_updater(lambda m: m.set_shape(get_t() * x_unit, y_unit).move_to(origin, DL))
        integral.fix_in_frame()
        self.add(rect, integral, t_tracker)
        v_tracker.set_value(1)
        self.play(v_tracker.animate.set_value(12), self.frame.animate.reorient(0, 0, 0, (42.64, 14.41, 0.0), 58.35).set_anim_args(time_span=(6, 15)), run_time=20)

class IntegrateRealExponential(InteractiveScene):

    def construct(self):
        t2c = {'{s}': YELLOW}
        integral = Tex('\\int^\\infty_0 e^{\\minus {s}t} dt', t2c=t2c)
        integral.set_x(1)
        integral.to_edge(UP)
        integral.save_state()
        ...
        graph_label = Tex('e^{\\minus {s}t}', t2c=t2c, font_size=72)
        graph_label.next_to(axes.y_axis.get_top(), UR).shift(0.5 * RIGHT)
        self.play(FadeIn(axes), TransformFromCopy(integral['e^{\\minus {s}t}'], graph_label), ShowCreation(graph, suspend_mobject_updating=True, run_time=3), Restore(integral))
        ...
        for value in [5, 0.25, 1]:
            ...
            self.wait()
        equals = Tex('=', font_size=72).rotate(90 * DEG)
        ...
        self.play(LaggedStart(Animation(graph.copy(), remover=True), Write(equals), FadeIn(area_word, DOWN), GrowArrow(arrow), UpdateFromFunc(area, update_area), lag_ratio=0.25), ShowCreation(graph, suspend_mobject_updating=True, run_time=3))
        self.wait()
        frame = self.frame
        area.clear_updaters()
        area.add_updater(update_area)
        self.play(s_tracker.animate.set_value(-0.2), frame.animate.reorient(0, 0, 0, (16.79, 7.82, 0.0), 28.01).set_anim_args(time_span=(2, 5)), run_time=5)
        simple_integral = Tex('\\int^\\infty_0 e^{\\minus t} dt')
        simple_integral.move_to(integral)
        simple_exp = Tex('e^{\\minus t}')
        simple_exp.move_to(graph_label)
        anti_deriv = Tex('=\\big[\\minus e^{\\minus t} \\big]^\\infty_0')
        simple_rhs = Tex('=0 - (\\minus 1)')
        anti_deriv.next_to(simple_integral, RIGHT)
        simple_rhs.next_to(anti_deriv, RIGHT)
        equals_one = Tex('= 1', font_size=60)
        equals_one.next_to(area_word)
        area_one_label = Tex('1', font_size=60)
        area_one_label.move_to(axes.c2p(0.35, 0.35))
        area_one_label.set_z_index(1)
        self.remove(integral, graph_label)
        self.play(TransformMatchingTex(integral.copy(), simple_integral), TransformMatchingTex(graph_label.copy(), simple_exp), run_time=1)
        self.play(TransformMatchingTex(simple_integral.copy(), anti_deriv, run_time=1.5, path_arc=30 * DEG))
        rect_kw = dict(buff=0.05, stroke_width=1.5)
        self.play(FadeIn(simple_rhs[:2], time_span=(0, 1)), VFadeInThenOut(SurroundingRectangle(anti_deriv['\\minus e^{\\minus t}'], **rect_kw)), VFadeInThenOut(SurroundingRectangle(anti_deriv['\\infty'], **rect_kw)), VFadeInThenOut(SurroundingRectangle(simple_rhs[:2], **rect_kw)), run_time=1.5)
        self.play(FadeIn(simple_rhs[2:], time_span=(0, 1)), VFadeInThenOut(SurroundingRectangle(anti_deriv['\\minus e^{\\minus t}'], **rect_kw)), VFadeInThenOut(SurroundingRectangle(anti_deriv['0'], **rect_kw)), VFadeInThenOut(SurroundingRectangle(simple_rhs[2:], **rect_kw)), run_time=1.5)
        self.wait()
        ...
        self.wait()
        square = Polygon(axes.c2p(0, 1), axes.c2p(1, 1), axes.c2p(1, 0), axes.c2p(0, 0))
        ...
        squishy_area = always_redraw(lambda: axes.get_area_under_graph(axes.get_graph(lambda t: np.exp(-area_s_tracker.get_value() * t), x_range=(0, area_x_max_tracker.get_value()))))
        ...
        self.wait()
        self.play(ShowCreation(square), FadeIn(corner_area), area_one_label.animate.move_to(square))
        self.wait()
        self.play(FadeOut(square), FadeOut(corner_area), FadeOut(tail_area), Restore(area_one_label), FadeOut(anti_deriv), FadeOut(simple_rhs))
        self.wait()
        stretch_label = VGroup(TexText('Squish by $\\frac{1}{s}$', t2c=t2c), Vector(2 * LEFT, thickness=5, fill_color=YELLOW))
        ...
        rhs = Tex('= \\frac{1}{s}', t2c=t2c, font_size=60)
        rhs.next_to(area_word.target, RIGHT)
        area.set_z_index(-1)
        area.add_updater(update_area)
        self.play(LaggedStart(TransformMatchingTex(simple_integral, integral, run_time=1), FadeIn(graph_label, 0.5 * DOWN), FadeOut(simple_exp, 0.5 * DOWN), FadeOut(equals_one, 0.5 * DOWN), FadeOut(area_one_label), lag_ratio=0.1))
        self.play(s_tracker.animate.set_value(5).set_anim_args(run_time=8), FadeIn(stretch_label, 1.5 * LEFT, time_span=(1, 5)), FadeOut(arrow))
        self.wait()
        self.play(LaggedStart(FadeOut(stretch_label), MoveToTarget(area_word), FadeTransform(stretch_label[0][-3:].copy(), rhs[1:]), FadeTransform(stretch_label[1].copy(), rhs[0]), FadeOut(equals), run_time=2, lag_ratio=0.1))
        dec_rhs = Tex('= 1.00', font_size=60)
        dec_rhs.make_number_changeable('1.00').add_updater(lambda m: m.set_value(1 / get_s()))
        dec_rhs.always.next_to(rhs, RIGHT)
        self.play(VFadeIn(dec_rhs), s_tracker.animate.set_value(0.01).set_anim_args(run_time=12, rate_func=bezier([0, 1, 1, 1])), self.frame.animate.reorient(0, 0, 0, (15.36, 0, 0.0), 30).set_anim_args(time_span=(6, 11)))
        self.wait()
        self.play(VGroup(area_word, rhs).animate.next_to(axes.c2p(0, 0), UR, MED_LARGE_BUFF).set_anim_args(time_span=(1, 3)), s_tracker.animate.set_value(0.75), VFadeOut(dec_rhs, time_span=(2, 4)), self.frame.animate.to_default_state(), run_time=4)
        self.wait()
        v_lines = VGroup(DashedLine(axes.c2p(0, 0), axes.c2p(0, 1.25)), DashedLine(axes.c2p(1, 0), axes.c2p(1, 1.25)))
        v_lines.set_stroke(WHITE, 1)
        unit_int = Tex('\\int^1_0 e^{\\minus {s}t} dt', t2c=t2c, font_size=60)
        unit_int.move_to(v_lines, UP)
        graph_for_unit_area = axes.get_graph(exp_func)
        graph_for_unit_area.set_stroke(width=0)
        unit_int_area = always_redraw(lambda: axes.get_area_under_graph(graph_for_unit_area, (0, 1)))
        ...
        area.clear_updaters()
        self.play(*map(FadeOut, [area_word, rhs, graph_label, s_slider]), *map(ShowCreation, v_lines), TransformMatchingTex(integral.copy(), unit_int), FadeOut(area), FadeIn(unit_int_area, time_span=(0.5, 1), suspend_mobject_updating=True))
        self.wait()
        self.play(Transform(graph_for_unit_area, axes.get_graph(lambda t: avg_value).set_stroke(width=0), run_time=3, rate_func=slosh_rate_func))
        ...
        self.play(LaggedStart(GrowFromCenter(brace), ShowCreation(unit_line), FadeIn(unit_label, 0.25 * DOWN), frame.animate.set_y(-1.5), run_time=1.5))
        self.wait()
        height_line = Line(unit_int_area.get_corner(DL), unit_int_area.get_corner(UL))
        height_line.set_stroke(RED, 5)
        area_eq_height = TexText('= Area = Width $\\times$ Height', t2c={'Area': BLUE, 'Height': RED, 'Width': YELLOW})
        ...
        self.play(FadeIn(fade_rect), FadeIn(area_eq_height), ShowCreation(height_line))
        self.wait()
        self.play(area_eq_height['Width $\\times$'].animate.set_opacity(0), area_eq_height['Height'].animate.move_to(area_eq_height['Width'], UL))
        self.wait()
        ...
        unit_int2 = Tex('\\int^2_1 e^{\\minus {s}t} dt', t2c=t2c, font_size=60)
        ...
        avg_label2 = self.get_avg_label(pile2.copy().set_height(axes.y_axis.get_unit_size() * avg_value2, stretch=True, about_edge=DOWN), 1, 2)
        self.add(v_lines[0])
        self.play(TransformFromCopy(v_lines, v_lines2, path_arc=-20 * DEG), TransformMatchingTex(unit_int.copy(), unit_int2, path_arc=-20 * DEG, run_time=1, key_map={'0': '1', '1': 2}), FadeIn(pile2, suspend_mobject_updating=True), unit_group.animate.match_x(pile2))
        self.play(area2_graph.animate.stretch(0, 1).match_y(axes.y_axis.n2p(avg_value2)).set_anim_args(rate_func=slosh_rate_func, run_time=3), FadeIn(avg_label2))
        ...
        for n in range(2, 6):
            ...
            new_unit_int = Tex(f'\\int^{n + 1}_{n} ' + 'e^{\\minus {s}t} dt', t2c=t2c, font_size=60)
            if n == 5:
                new_unit_int = Tex('\\cdots', font_size=90)
            new_unit_int.match_y(unit_int)
            new_unit_int.match_x(new_v_lines)
            s = get_s()
            avg_y = np.mean([np.exp(-s * t) for t in np.arange(n, n + 1, 0.001)])
        ...
        self.play(self.frame.animate.reorient(0, 0, 0, (5.34, 2.63, 0.0), 13.89), LaggedStartMap(FadeIn, new_groups, lag_ratio=0.75), FadeOut(unit_group, time_span=(0, 2)), run_time=4)
        self.play(GrowFromCenter(big_brace), FadeIn(integral))
        self.wait()
        height_vects = VGroup((Arrow(pile.get_bottom(), pile.get_top(), buff=0, thickness=6, fill_color=RED) for pile in piles))
        equals = Tex('=', font_size=120)
        ...
        self.play(LaggedStartMap(FadeOut, avg_labels, lag_ratio=0.2), LaggedStartMap(FadeIn, height_vects, lag_ratio=0.2), run_time=1)
        self.wait()
        self.play(TransformFromCopy(height_vects, stacked_vects, lag_ratio=0.05, run_time=2), integral.animate.next_to(equals, LEFT, LARGE_BUFF), Write(equals))
        self.wait()
    ...
    def get_avg_label(self, pile, start=0, end=1, font_size=30):
        ...
        arrows = VGroup(Arrow(word.get_top() + buff * UP, pile.get_top(), buff=0), Arrow(word.get_bottom() + buff * DOWN, pile.get_bottom(), buff=0))
...
class IntegrateComplexExponential(SPlane):
    ...
    t_max = 100
    initial_s = 0.2 + 1j
    s_label_config = dict(hide_zero_components_on_complex=False, include_sign=True, num_decimal_places=1)
    ...
    def construct(self):
        ...
        exp_plane_label = Tex('e^{\\minus st}', font_size=72, t2c=t2c)
        exp_plane_label.next_to(exp_plane, UP)
        output_dot, output_label = self.get_output_dot_and_label(exp_plane, lambda: -get_s(), get_t, label_direction=DR, s_tex='\\minus s')
        ...
        self.play(ShowCreation(path, suspend_mobject_updating=True), t_tracker.animate.set_value(path.t_range[1]), rate_func=linear, run_time=10)
        self.play(s_tracker.animate.set_value(0.2 - 1j), rate_func=there_and_back, run_time=6)
        self.play(s_tracker.animate.set_value(-0.2 + 1j), rate_func=there_and_back, run_time=6)
        ...
        int_tex0 = Tex('\\int^1_0 e^{\\minus st} dt', t2c=t2c, font_size=48)
        int_tex0.next_to(exp_plane.n2p(1), UP, MED_SMALL_BUFF)
        int_tex0.set_backstroke(BLACK, 5)
        self.play(FadeIn(int_tex0, 0.25 * UP))
        self.play(t_tracker.animate.set_value(1), ShowCreation(subpath_0), rate_func=linear, run_time=2)
        self.wait()
        self.play(ShowCreation(many_points))
        self.wait()
        self.play(ReplacementTransform(many_points, avg_dot0))
        self.wait()
        self.play(GrowArrow(avg_vect0), int_tex0.animate.set_height(0.75).next_to(avg_vect0.get_center(), UR, buff=SMALL_BUFF))
        ...
        int_plane_label = Tex('\\int^\\infty_0 e^{\\minus st} dt', t2c=t2c, font_size=48)
        ...
        self.play(exp_plane.animate.move_to(lil_exp_plane.saved_state).set_opacity(0), FadeIn(lil_exp_plane, to_upper), VGroup(path_ghost, subpath_0, avg_vect0).animate.shift(to_upper), int_tex0.animate.scale(0.5, about_point=exp_plane.n2p(0)).shift(to_lower), TransformFromCopy(avg_vect0, lower_avg_vect0), FadeIn(int_plane), exp_plane_label.animate.next_to(lil_exp_plane, LEFT, aligned_edge=UP))
        self.wait()
        avg_vects = VGroup(avg_vect0)
        lower_avg_vects = VGroup(lower_avg_vect0)
        subpaths = VGroup(subpath_0)
        int_texs = VGroup(int_tex0)
        shifts = [0.8 * LEFT + 0.1 * DOWN, LEFT, 0.2 * LEFT + 0.4 * UP, 0.4 * UP]
        ...
        for n in range(1, 5):
            ...
            avg_vect, lower_avg_vect = [self.get_mean_vector(plane, exp_func, n, n + 1, thickness=thickness) for plane, thickness in [(exp_plane, 3), (int_plane, 1)]]
            ...
            int_tex = Tex(f'\\int_{n}^{n + 1} e^{{\\minus s t}} dt', t2c=t2c)
            ...
            self.play(t_tracker.animate.set_value(n + 1), ShowCreation(subpath_n), output_label_added_shift.animate.move_to(shifts[n - 1]), ShowCreation(sample_points), run_time=2, rate_func=linear)
            self.play(avg_vects[-1].animate.set_fill(opacity=0.5).set_stroke(width=0), GrowArrow(avg_vect), Transform(sample_points, avg_dot))
            self.play(FadeOut(sample_points), TransformFromCopy(avg_vect, lower_avg_vect), FadeIn(int_tex))
        ...
        for n in range(5, 20):
            ...
            lower_avg_vect.set_stroke(width=0)
            lower_avg_vect.put_start_on(lower_avg_vects[-1].get_end())
            anims = [ShowCreation(subpath, rate_func=linear), t_tracker.animate.set_value(n + 1).set_anim_args(rate_func=linear), FadeIn(avg_vect)]
            if n == 5:
        ...
        self.play(LaggedStart((FadeTransform(int_tex, int_plane_label) for int_tex in int_texs), lag_ratio=0.025, group_type=Group), *map(FadeOut, [output_label, output_dot, avg_vects]))
        self.play(ShowCreation(int_rect))
        self.wait()
        self.play(int_rect.animate.surround(int_dot[0], buff=0), FadeIn(int_dot))
        ...
        self.wait()
        frame = self.frame
        self.play(s_tracker.animate.set_value(0.1).set_anim_args(rate_func=bezier([0, 1, 1, 1])), frame.animate.reorient(0, 0, 0, (2.75, 0, 0.0), 11).set_anim_args(time_span=(2, 8)), run_time=8)
        self.wait()
        self.play(s_tracker.animate.set_value(0.1 + 2j), frame.animate.to_default_state(), run_time=10)
        ...
        self.wait()
        self.play(rect.animate.surround(int_dot[0], buff=0), int_pieces.animate.set_fill(opacity=0.5))
        self.wait()
        self.play(GrowArrow(int_vect), FadeOut(rect), FadeOut(int_dot))
        self.wait()
        frame = self.frame
        ...
        unit_size = s_plane.x_axis.get_unit_size()
        z_line.add_updater(lambda m: m.put_start_and_end_on(s_plane.n2p(get_s()), s_plane.n2p(get_s()) + unit_size * get_output_magnitude(get_s()) * OUT))
        ...
        traced_graph.update()
        int_vect.add_updater(lambda m: m.put_start_and_end_on(int_plane.n2p(0), int_plane.n2p(1.0 / get_s())))
        ...
        self.play(frame.animate.reorient(-3, 68, 0, (1.43, 0.51, -0.28), 6.5), s_plane_group.animate.center(), ReplacementTransform(pre_z_line, z_line), FadeOut(exp_plane_label, time_span=(1, 2)), FadeIn(new_exp_plane_label, time_span=(1, 2)), FadeOut(int_plane_label, time_span=(1, 2)), FadeIn(new_int_plane_label, time_span=(1, 2)), VFadeIn(v_line, time_span=(2, 3)), run_time=3)
        self.play(FadeIn(out_dot, time_span=(0, 1)), frame.animate.reorient(8, 64, 0, (1.43, 0.51, -0.28), 6.5), run_time=3)
        self.add(traced_graph)
        self.play(s_tracker.animate.set_value(0.2 - 2j), frame.animate.reorient(25, 87, 0, (3.18, 1.73, 2.63), 11.08).set_anim_args(time_span=(1, 7.5)), run_time=10)
        self.play(s_tracker.animate.set_value(0.4 - 2j))
        self.play(s_tracker.animate.set_value(0.4 + 2j), frame.animate.reorient(44, 81, 0, (3.16, 1.71, 2.63), 11.08), run_time=5)
        self.play(s_tracker.animate.set_value(0.6 + 2j))
        self.play(s_tracker.animate.set_value(0.6 - 2j), frame.animate.reorient(66, 89, 0, (3.31, 2.07, 2.96), 9.69), run_time=5)
        self.play(s_tracker.animate.set_value(0.8 - 2j))
        self.play(s_tracker.animate.set_value(0.8 + 2j), frame.animate.reorient(22, 84, 0, (2.56, 1.53, 1.72), 10.14), run_time=5)
        self.play(s_tracker.animate.set_value(1.0 + 2j))
        self.play(s_tracker.animate.set_value(1.0 - 2j), frame.animate.reorient(22, 84, 0, (2.56, 1.53, 1.72), 10.14), run_time=5)
        self.wait()
        ...
        for line in mesh:
            if line.get_z(IN) < 0:
                mesh.remove(line)
        self.play(ShowCreation(graph), ShowCreation(mesh, lag_ratio=0.01, time_span=(1, 4)), VFadeOut(traced_graph), frame.animate.reorient(34, 74, 0, (2.02, 1.44, 1.88), 9.87), out_dot.animate.set_color(WHITE), run_time=4)
        self.play(s_tracker.animate.set_value(0.1 - 0.1j), frame.animate.reorient(38, 82, 0, (2.84, 2.62, 4.72), 14.21).set_anim_args(time_span=(0, 6)), run_time=10)
        self.wait()
        ...
        self.wait()
        self.play(s_tracker.animate.set_value(0.5 - 1j), run_time=3)
        self.wait()
        ...
        self.wait()
        self.play(Restore(graph, time_span=(0, 1)), frame.animate.reorient(29, 72, 0, (1.79, 1.9, 1.65), 11.21), run_time=4)
        ...
        self.play(frame.animate.reorient(-38, 70, 5, (3.17, 1.96, -0.36), 9.16), FadeOut(hue_circle), Restore(left_plane, time_span=(3, 5)), run_time=5)
        self.wait()
        self.play(s_tracker.animate.set_value(-0.5), VFadeOut(z_line, time_span=(0, 1.5)), FadeOut(out_dot, time_span=(0, 1.5)), VFadeOut(int_vect, time_span=(0, 1.0)), run_time=3)
        self.play(s_tracker.animate.set_value(-0.1 - 1j).set_anim_args(run_time=3))
        self.play(*(plane.animate.scale(0.1, about_point=plane.n2p(0)) for plane in [exp_plane, lil_exp_plane, int_plane]))
        self.play(ShowCreation(staggered_path, suspend_mobject_updating=True), ShowIncreasingSubsets(int_pieces, suspend_mobject_updating=True), run_time=4)
        self.play(UpdateFromAlphaFunc(s_tracker, lambda m, a: m.set_value(complex(-0.1 - math.sin(PI * a), interpolate(-1, 1, a)))), run_time=8)
        self.play(*(plane.animate.scale(10, about_point=plane.n2p(0)).set_anim_args(time_span=(2, 4)) for plane in [exp_plane, lil_exp_plane, int_plane]), s_tracker.animate.set_value(1j), run_time=4)
        ...
        self.play(FadeIn(output_dot), VFadeIn(output_label), left_plane.animate.set_fill(opacity=0.2))
        max_n = 25
        self.play(ShowCreation(staggered_path[:max_n].set_z_index(2)), ShowIncreasingSubsets(int_pieces[:max_n].set_z_index(2), int_func=np.floor), t_tracker.animate.set_value(max_n).set_anim_args(time_span=(max_n / (max_n + 1), max_n)), rate_func=linear, run_time=max_n)
        self.play(VFadeOut(output_label), FadeOut(output_dot), t_tracker.animate.set_value(max_n + 1).set_anim_args(rate_func=linear), ShowIncreasingSubsets(int_pieces[max_n:].set_z_index(2)), FadeIn(staggered_path[max_n:].set_z_index(2)))
        self.add(staggered_path, int_pieces)
        self.add(int_vect)
        self.play(s_tracker.animate.set_value(0.1 + 1j), int_pieces.animate.set_fill(opacity=0.75), VFadeIn(int_vect), FadeOut(left_plane), run_time=2)
        rect = SurroundingRectangle(int_vect).set_z_index(2)
        self.play(ShowCreation(rect))
        self.play(FadeOut(rect))
        self.wait()
        self.play(s_tracker.animate.set_value(1j), run_time=5)
        self.wait()
        ...
        int_equation = Tex('\\int^\\infty_0 e^{\\minus st} dt = \\frac{1}{s}', t2c=t2c)
        int_equation.to_edge(UP).shift(LEFT)
        int_equation.fix_in_frame()
        lhs_mover = new_int_plane_label.copy()
        self.play(frame.animate.reorient(0, 85, 1, (5.32, 3.46, 3.69), 13.49), s_tracker.animate.set_value(0.2 + 1j), run_time=3)
        self.play(lhs_mover.animate.replace(int_equation[:-4]), Write(int_equation[-4:], time_span=(0.75, 1.75)))
        ...
        self.play(s_tracker.animate.set_value(0.2 - 1j), run_time=6)
        self.play(s_tracker.animate.set_value(0.5 + 1j), frame.animate.reorient(-49, 79, 0, (6.24, 2.45, -0.13), 10.06), run_time=4)
        self.play(GrowArrow(left_arrow, time_span=(0, 1)), VShowPassingFlash(int_plane_rect, time_width=1.5, time_span=(4, 7)), Write(i_eq, time_span=(10, 12)), s_tracker.animate.set_value(1j), run_time=15)
        self.wait()
        self.play(frame.animate.reorient(0, 85, 1, (5.32, 3.46, 3.69), 13.49), FadeOut(i_eq), FadeOut(left_arrow), run_time=3)
        ...
        self.play(Restore(left_plane), frame.animate.reorient(-36, 79, 4, (7.71, 2.4, 1.86), 15.36), run_time=3)
        self.wait()
        self.play(ShowCreation(equation_rect))
        self.wait()
        self.play(equation_rect.animate.surround(int_equation['\\frac{1}{s}'], buff=SMALL_BUFF))
        self.wait()
        self.play(ReplacementTransform(left_plane, full_plane))
        self.wait()
        ...
        extended_mesh.shift(0.01 * OUT)
        self.add(extended_graph, graph, extended_mesh, mesh)
        self.play(FadeOut(graph, 0.01 * IN), FadeIn(extended_graph), FadeOut(mesh), FadeIn(extended_mesh), frame.animate.reorient(0, 85, 0, (5.32, 3.46, 3.69), 13.49).set_anim_args(run_time=5), FadeOut(full_plane))
        self.play(frame.animate.reorient(-17, 62, 9, (6.29, -0.4, 3.75), 13.87), FadeOut(equation_rect), run_time=5)
        curr_s = complex(get_s())
        self.play(UpdateFromAlphaFunc(s_tracker, lambda m, a: m.set_value(curr_s * np.exp(a * TAU * 1j))), run_time=30)
        self.wait()
        self.play(frame.animate.reorient(-36, 83, 1, (5.84, -1.06, 3.19), 10.63), s_tracker.animate.set_value(0.1 + 0.1j), run_time=20)
        self.play(frame.animate.reorient(0, 0, 0, (1.95, 0.95, -0.0), 15.34), s_tracker.animate.set_value(0.001).set_anim_args(time_span=(0, 5)), extended_mesh.animate.set_stroke(opacity=0), VFadeOut(int_vect, time_span=(0, 7)), run_time=10)
        self.play(frame.animate.reorient(0, 83, 0, (5.18, 4.04, 3.15), 14.5), run_time=10)
    ...
    def get_s_group(self):
        ...
        s_plane_label = Tex('s', t2c=self.tex_to_color_map, font_size=72)
        s_plane_label.next_to(s_plane, UP)
        s_group = Group(s_plane, s_dot, s_label, s_plane_label)
        return s_group

    def get_output_path(self, plane, func, t_min=0, t_max=20, stroke_color=TEAL, stroke_width=2, step_size=0.1):
        return ParametricCurve(lambda t: plane.n2p(func(t)), t_range=[t_min, t_max, step_size], stroke_color=stroke_color, stroke_width=stroke_width)
    ...
    def get_sample_dots(self, subpath, n_samples=20, radius=0.05, glow_factor=0.5, color=RED):
        return DotCloud([subpath.pfp(a) for a in np.linspace(0, 1, n_samples)], radius=radius, glow_factor=glow_factor, color=color)

    def get_dynamic_output_path(self, plane, func, stroke_width=3, step_size=0.01):
    ...
    def get_dynamic_vector_sum(self, plane, func, thickness=1, backstroke_width=0, n_samples=100, border_width=0.5):
        t_range = list(range(0, self.t_max))
        vects = VGroup((self.get_vector(plane, 1, thickness=thickness, backstroke_width=backstroke_width, fill_color=color) for t, color in zip(t_range, it.cycle(self.staggered_path_colors))))
...
class BreakDownLaplaceTransform(IntegrateComplexExponential):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(20, 68, 0, (-0.45, 0.24, 0.17), 6.28), s_tracker.animate.set_value(1.6 - 1j), ShowCreation(int_rect, time_span=(4, 5)), run_time=8)
        self.play(int_rect.animate.surround(exp_plane.label))
        self.draw_upper_plot(draw_time=8)
        self.play(s_tracker.animate.set_value(1.6), frame.animate.reorient(24, 65, 0, (-0.35, 1.85, 1.61), 11.2), run_time=5)
        self.play(s_tracker.animate.set_value(1.501), run_time=5)
        self.clear()
        ...
        self.play(frame.animate.reorient(0, 43, 0, (-0.42, 0.6, 2.1), 9.91), self.s_tracker.animate.set_value(1.5001), run_time=10)
    ...
    def add_core_pieces(self):
        ...
        self.add(s_group, graph_tracer, graph, right_rect, output_planes, output_path, vect_sum, integral_vect)
    ...
    def get_integral_vect(self, int_plane):
        ...
        vect.add_updater(lambda m: m.put_start_and_end_on(origin, int_plane.n2p(self.transformed_func(self.s_tracker.get_value()))))
    ...
    def get_output_planes(self, width=3, edge_buff=2.5):
        ...
        exp_plane.label = Tex(self.func_tex, 'e^{\\minus {s} {t}}', **kw)
        exp_plane.label.next_to(exp_plane, RIGHT, aligned_edge=UP)
        int_plane.label = Tex('\\int^\\infty_0 ' + self.func_tex + 'e^{\\minus {s} {t}} d{t}', **kw)
    ...
    def draw_upper_plot(self, draw_time=12, rate_multiple=2):
        ...
        tracing_vect.add_updater(lambda m: m.put_start_and_end_on(exp_plane.n2p(0), path_copy.get_end()))
        ...
        self.play(ShowCreation(path_copy), ShowIncreasingSubsets(self.vect_sum[:n_pieces], int_func=np.ceil), run_time=draw_time, rate_func=linear)
    ...
    def show_simple_pole(self):
        ...
        self.play(frame.animate.reorient(0, 0, 0, (-5.17, -0.19, 0.0), 9.88).set_field_of_view(20 * DEG), self.s_tracker.animate.set_value(self.pole_value + 1e-05), run_time=10)
        self.wait()

class LaplaceTransformOfCos(BreakDownLaplaceTransform):
    func_tex = '\\cos(t)'
    s_label_config = dict(hide_zero_components_on_complex=True, include_sign=True, num_decimal_places=2)
    t_max = 200
    ...
    def construct(self):
        ...
        frame = self.frame
        s_tracker = self.s_tracker
        frame.reorient(0, 0, 0, (-0.44, -0.47, 0.0), 8.0)
        self.play(frame.animate.reorient(28, 69, 0, (-0.42, 0.29, 1.36), 7.45), s_tracker.animate.set_value(0.02 + 1j), run_time=10)
        self.play(frame.animate.reorient(-38, 81, 0, (-0.24, 0.38, 1.05), 8.77), s_tracker.animate.set_value(0.02 - 1j), run_time=10)
        self.play(frame.animate.reorient(13, 72, 0, (-1.41, 0.26, 1.11), 7.37), s_tracker.animate.set_value(0.1 + 0.57j), run_time=8)
        self.play(frame.animate.reorient(0, 31, 0, (-1.08, -0.52, 0.3), 7.37), s_tracker.animate.set_value(0.01 + 1j), run_time=4)
        self.play(s_tracker.animate.set_value(0.2 - 1j), run_time=5)
        self.play(frame.animate.reorient(21, 71, 0, (-1.48, -0.28, 0.8), 5.07), s_tracker.animate.set_value(0.5 + 1j), run_time=20)
        exp_plane, int_plane = self.output_planes
        ...
        self.play(s_tracker.animate.set_value(0.2j), self.graph[0].animate.set_opacity(0.5), self.graph[1].animate.set_stroke(opacity=0.05), frame.animate.reorient(0, 15, 0, (-1.05, -1.03, 0.59), 4.66), run_time=4)
        t_tracker = self.t_tracker
        t_tracker.set_value(0)
        get_t = t_tracker.get_value
        get_s = s_tracker.get_value
        ...
        exp_vect.add_updater(lambda m: m.put_start_and_end_on(exp_plane.n2p(0), exp_plane.n2p(np.exp(-get_s() * get_t()))))
        full_vect = exp_vect.copy().clear_updaters()
        full_vect.add_updater(lambda m: m.put_start_and_end_on(exp_plane.n2p(0), exp_plane.n2p(self.func(get_t()) * np.exp(-get_s() * get_t()))))
        t_label = Tex('{t} = 0.0', t2c=self.tex_to_color_map)
        ...
        partial_path.add_updater(lambda m: m.pointwise_become_partial(output_path_copy, 0, get_t() / self.t_max))
        ...
        self.play(int_rect.animate.surround(exp_plane.label[-4:], SMALL_BUFF), FadeOut(output_path, suspend_mobject_updating=True), FadeOut(vect_sum, suspend_mobject_updating=True), FadeOut(int_vect, suspend_mobject_updating=True), FadeIn(exp_vect), FadeIn(t_label))
        self.add(t_tracker)
        self.wait(12)
        self.add(partial_path)
        self.play(int_rect.animate.surround(exp_plane.label, SMALL_BUFF), exp_vect.animate.set_fill(opacity=0.25), VFadeIn(full_vect))
        self.wait(12)
        self.add(growing_vect_sum)
        self.play(int_rect.animate.surround(int_plane.label, buff=SMALL_BUFF))
        self.wait(20)
        self.play(frame.animate.reorient(25, 55, 0, (-0.91, -0.76, 0.83), 5.93), self.graph[0].animate.set_opacity(0.7), self.graph[1].animate.set_stroke(opacity=0.1), run_time=5)
        self.wait(5)
        growing_vect_sum.clear_updaters()
        self.play(VFadeOut(growing_vect_sum), VFadeOut(partial_path), VFadeOut(t_label), VFadeOut(exp_vect), VFadeOut(full_vect), VFadeOut(int_rect), VFadeIn(output_path), VFadeIn(vect_sum), FadeIn(int_vect))
        self.play(s_tracker.animate.increment_value(0.05), run_time=3)
        self.draw_upper_plot(draw_time=12, rate_multiple=6)
        self.play(s_tracker.animate.increment_value(0.8j), run_time=20)
        self.draw_upper_plot(draw_time=12, rate_multiple=4)
        self.play(s_tracker.animate.increment_value(-0.03), frame.animate.reorient(29, 78, 0, (-0.48, 0.16, 2.64), 10.0), run_time=7)
        self.play(s_tracker.animate.increment_value(0.05), run_time=3)
        self.wait()
