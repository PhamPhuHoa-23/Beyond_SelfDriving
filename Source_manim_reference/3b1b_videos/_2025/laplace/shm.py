...
def get_coef_colors(n_coefs=3):
    return [interpolate_color_by_hsl(TEAL, RED, a) for a in np.linspace(0, 1, n_coefs)]

class SrpingMassSystem(VGroup):

    def __init__(self, x0=0, v0=0, k=3, mu=0.1, equilibrium_length=7, equilibrium_position=ORIGIN, direction=RIGHT, spring_stroke_color=GREY_B, spring_stroke_width=2, spring_radius=0.25, n_spring_curls=8, mass_width=1.0, mass_color=BLUE_E, mass_label='m', external_force=None):
        super().__init__()
    ...
    def get_spring(self, stroke_color, stroke_width, n_curls, radius):
        spring = ParametricCurve(lambda t: [t, -radius * math.sin(TAU * t), radius * math.cos(TAU * t)], t_range=(0, n_curls, 0.01), stroke_color=stroke_color, stroke_width=stroke_width)
    ...
    def time_step(self, delta_t, dt_size=0.001):
    ...
    def get_velocity_vector(self, scale_factor=0.5, thickness=3.0, v_offset=-0.25, color=GREEN):
        ...
        v_shift = v_offset * UP
        vector.add_updater(lambda m: m.put_start_and_end_on(self.mass.get_center() + v_shift, self.mass.get_center() + v_shift + scale_factor * self.velocity * RIGHT))
        return vector

    def get_force_vector(self, scale_factor=0.5, thickness=3.0, v_offset=-0.25, color=RED):
        vector = Vector(RIGHT, fill_color=color)
        v_shift = v_offset * UP
        vector.add_updater(lambda m: m.put_start_and_end_on(self.mass.get_center() + v_shift, self.mass.get_center() + v_shift + scale_factor * self.get_force(self.get_x(), self.velocity) * RIGHT))
        return vector
...
class BasicSpringScene(InteractiveScene):
    ...
    def construct(self):
        spring = SrpingMassSystem(x0=2, mu=0.1, k=3, equilibrium_position=2 * LEFT, equilibrium_length=5)
        self.add(spring)
        ...
        dashed_line = DashedLine(spring.mass.get_bottom(), number_line.n2p(spring.get_x()), stroke_color=GREY, stroke_width=2)
        ...
        self.wait(2)
        self.play(VFadeIn(number_line), VFadeIn(dashed_line), VFadeIn(arrow_tip), VFadeIn(x_label))
        self.wait(7)
        if False:
            ...
            self.play(VFadeIn(x_label_arrow, time_span=(1, 2)), x_label.animate.scale(2), run_time=2)
            self.wait(8)
        ...
        self.play(UpdateFromAlphaFunc(spring, lambda m, a: m.set_x(4 * math.cos(2 * TAU * a)), rate_func=linear, run_time=8))
        spring.unpause()
        spring.set_mu(0.25)
        self.wait(15)
        frame = self.frame
        ...
        axes = Axes(x_range=(0, 20, 1), y_range=(-2, 2, 1), width=12, height=3, axis_config={'stroke_color': GREY})
        ...
        tracking_point.add_updater(lambda p: p.move_to(axes.c2p(time_tracker.get_value(), spring.get_x())))
        position_graph = TracedPath(tracking_point.get_center, stroke_color=BLUE, stroke_width=3)
        spring.pause()
        spring.set_velocity(0)
        self.play(frame.animate.reorient(0, 0, 0, (2.88, 1.88, 0.0), 12.48), FadeIn(axes), VFadeOut(a_vect), spring.animate.set_x(2))
...
class DampingForceDemo(InteractiveScene):
    ...
    def construct(self):
        spring_system = SrpingMassSystem(x0=-4, v0=2, k=0, mu=0.3, equilibrium_position=ORIGIN, equilibrium_length=6, mass_width=0.8, mass_color=BLUE_E, mass_label='m')
        ...
        velocity_label = Tex('\\vec{\\textbf{v}}', font_size=24)
        ...
        damping_label = Tex('-\\mu v', fill_color=RED, font_size=24)
...
class SolveDampedSpringEquation(InteractiveScene):
    ...
    def construct(self):
        pos, vel, acc = funcs = VGroup(Tex('x(t)'), Tex("x'(t)"), Tex("x''(t)"))
        funcs.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        labels = VGroup(Text('Position').set_color(BLUE), Text('Velocity').set_color(RED), Text('Acceleration').set_color(YELLOW))
        ...
        for l1, l2 in zip(funcs, funcs[1:]):
            ...
            ddt = Tex('\\frac{d}{dt}', font_size=30)
        ...
        for func1, func2, label1, label2, arrow in zip(funcs, funcs[1:], labels, labels[1:], arrows):
            self.play(LaggedStart(GrowFromPoint(arrow, arrow.get_corner(UR), path_arc=30 * DEG), TransformFromCopy(func1, func2, path_arc=30 * DEG), FadeTransform(label1.copy(), label2), lag_ratio=0.1))
            self.wait()
        deriv_group = VGroup(funcs, labels, arrows)
        t2c = {'x(t)': colors[0], "x'(t)": colors[1], "x''(t)": colors[2]}
        equation1 = Tex("{m} x''(t) = -k x(t) - \\mu x'(t)", t2c=t2c)
        equation1.to_corner(UL)
        ma = equation1["{m} x''(t)"][0]
        kx = equation1['-k x(t)'][0]
        mu_v = equation1["- \\mu x'(t)"][0]
        rhs = VGroup(kx, mu_v)
        ma_brace, kx_brace, mu_v_brace = braces = VGroup((Brace(part, DOWN, buff=SMALL_BUFF) for part in [ma, kx, mu_v]))
        label_texs = ['\\textbf{F}', '\\text{Spring force}', '\\text{Damping}']
        ...
        self.play(LaggedStart(GrowFromCenter(ma_brace), Write(ma[0]), run_time=1, lag_ratio=0.1))
        self.wait()
        self.play(LaggedStart(Write(equation1['= -k'][0]), FadeTransformPieces(ma_brace, kx_brace), TransformFromCopy(pos, equation1['x(t)'][0], path_arc=-45 * DEG)))
        self.wait()
        self.play(LaggedStart(FadeTransformPieces(kx_brace, mu_v_brace), Write(equation1['- \\mu'][0]), TransformFromCopy(vel, equation1["x'(t)"][0], path_arc=-45 * DEG)))
        self.wait()
        self.play(FadeOut(mu_v_brace))
        equation2 = Tex("{m} x''(t) + \\mu x'(t) + k x(t) = 0", t2c=t2c)
        equation2.move_to(equation1, UL)
        self.play(TransformMatchingTex(equation1, equation2, path_arc=45 * DEG))
        self.wait()
        t2c = {'s': YELLOW, 'x(t)': TEAL}
        hyp_word, hyp_tex = hypothesis = VGroup(Text('Hypothesis: '), Tex('x(t) = e^{st}', t2c=t2c))
        hypothesis.arrange(RIGHT)
        hypothesis.to_corner(UR)
        sub_hyp_word = TexText('(For some $s$)', t2c={'$s$': YELLOW}, font_size=36, fill_color=GREY_B)
        sub_hyp_word.next_to(hyp_tex, DOWN)
        self.play(LaggedStart(FadeTransform(pos.copy(), hyp_tex[:4], path_arc=45 * DEG, remover=True), FadeOut(deriv_group), Write(hyp_word, run_time=1), Write(hyp_tex[4:], time_span=(0.5, 1.5))))
        self.add(hypothesis)
        self.wait()
        self.play(FadeIn(sub_hyp_word, 0.25 * DOWN))
        self.wait()
        t2c['s'] = YELLOW
        equation3 = Tex('{m} s^2 e^{st} + \\mu s e^{st} + k e^{st} = 0', t2c=t2c)
        ...
        pos_rect, vel_rect, acc_rect = rects = VGroup((SurroundingRectangle(group[0], buff=0.05).set_stroke(group[0][0].get_color(), 1) for group in matched_parts))
        pos_arrow, vel_arrow, acc_arrow = arrows = VGroup((Arrow(*pair, buff=0.1) for pair in matched_parts))
        for rect, arrow, pair in zip(rects, arrows, matched_parts):
            self.play(ShowCreation(rect))
            self.play(GrowArrow(arrow), FadeTransform(pair[0].copy(), pair[1]), rect.animate.surround(pair[1]))
            self.wait()
        self.play(LaggedStart((TransformFromCopy(equation2[tex], equation3[tex]) for tex in ['{m}', '+', 'k', '\\mu', '=', '0']), lag_ratio=0.05))
        self.wait()
        self.play(FadeOut(arrows, lag_ratio=0.1), FadeOut(rects, lag_ratio=0.1))
        key_syms = ['s', 'm', '\\mu', 'k']
        equation4, equation5, equation6 = new_equations = VGroup(Tex('e^{st} \\left( ms^2 + \\mu s + k \\right) = 0', t2c=t2c), Tex('ms^2 + \\mu s + k = 0', t2c=t2c), Tex('{s} = {{-\\mu \\pm \\sqrt{\\mu^2 - 4mk}} \\over 2m}', isolate=key_syms))
        rhs = equation6[2:]
        ...
        exp_rect = SurroundingRectangle(equation4['e^{st}'])
        exp_rect.set_stroke(YELLOW, 2)
        ne_0 = VGroup(Tex('\\ne').rotate(90 * DEG), Integer(0))
        ne_0.arrange(DOWN).next_to(exp_rect, DOWN)
        self.play(TransformMatchingTex(equation3, equation4, matched_keys=['e^{st}'], run_time=1.5, path_arc=30 * DEG))
        self.wait(0.5)
        ...
        self.play(*(TransformFromCopy(equation4[key], equation5[key]) for key in ['ms^2 + \\mu s + k', '= 0']), FadeOut(exp_rect))
        self.wait()
        self.play(TransformMatchingTex(equation5.copy(), equation2.copy(), key_map={'s^2': "x''(t)", '\\mu s': '\\mu x(t)', 'k': 'k x(t)'}, remover=True, rate_func=there_and_back_with_pause, run_time=6), equation4.animate.set_fill(opacity=0.25))
        self.play(equation4.animate.set_fill(opacity=1))
        self.wait()
        boxes = VGroup((SurroundingRectangle(mob) for mob in [equation2["+ \\mu x'(t)"], equation4['+ \\mu s'], equation5['+ \\mu s']]))
        ...
        self.wait()
        simple_answer = Tex('s = \\pm i \\sqrt{k / m}', t2c=t2c)
        simple_answer.next_to(equation5, DOWN, LARGE_BUFF, aligned_edge=RIGHT)
        omega_brace = Brace(simple_answer['\\sqrt{k / m}'], DOWN, SMALL_BUFF)
        omega = omega_brace.get_tex('\\omega')
        ...
        self.wait()
        self.play(s_rect.animate.surround(hyp_tex['e^{st}']).set_anim_args(path_arc=-60 * DEG), FadeTransform(s_copy, hyp_tex['s'], path_arc=-60 * DEG), run_time=2)
        self.wait()
        self.play(FadeOut(s_rect))
        frame = self.frame
        self.play(frame.animate.scale(1.5, about_edge=LEFT), hypothesis.animate.next_to(equation2, UP, LARGE_BUFF, aligned_edge=LEFT), FadeOut(sub_hyp_word), run_time=1.5)
        self.wait()
        qf_arrow = Arrow(equation5.get_right(), equation6.get_corner(UR) + 0.5 * LEFT, path_arc=-150 * DEG)
        ...
        self.play(FadeOut(simple_answer, DOWN), boxes.animate.set_fill(opacity=0).set_stroke(width=2, opacity=1))
        self.play(FadeOut(boxes))
        self.wait()
        self.play(TransformFromCopy(equation5['s'], equation6['s']), Write(equation6['=']), GrowFromPoint(qf_arrow, qf_arrow.get_corner(UL)), FadeIn(qf_words, shift=0.5 * DOWN))
        self.play(LaggedStart(*(TransformFromCopy(equation5[sym], equation6[sym], time_span=(0.5, 1.5)) for sym in key_syms[1:]), lag_ratio=0.1), Write(naked_equation))
        self.wait()
    ...
    def old_material(self):
        ...
        consolidated_lines = VGroup(hypothesis, equation2, equation4, final_equation)
        ...
        implies = Tex('\\Longrightarrow', font_size=60)
        implies.next_to(consolidated_lines.target[0], RIGHT, buff=0.75)
        t2c = {'x(t)': TEAL, '\\omega': PINK}
        imag_exps = VGroup(Tex('x(t) = e^{+i \\omega t}', t2c=t2c), Tex('x(t) = e^{-i \\omega t}', t2c=t2c))
        imag_exps.arrange(RIGHT, buff=2.0)
        imag_exps.next_to(implies, RIGHT, buff=0.75)
        self.remove(final_equation)
        self.play(LaggedStart(FadeOut(arrows), FadeOut(equation3, 0.5 * UP), FadeOut(sub_hyp_word), MoveToTarget(consolidated_lines), Write(implies)))
        for imag_exp, sgn in zip(imag_exps, '+-'):
            self.play(TransformFromCopy(hyp_tex['x(t) ='][0], imag_exp['x(t) ='][0]), TransformFromCopy(hyp_tex['e'][0], imag_exp['e'][0]), TransformFromCopy(hyp_tex['t'][-1], imag_exp['t'][-1]), FadeTransform(final_equation['\\pm i'][0].copy(), imag_exp[f'{sgn}i'][0]), FadeTransform(final_equation['\\sqrt{k/m}'][0].copy(), imag_exp['\\omega'][0]))
        omega_brace = Brace(final_equation['\\sqrt{k/m}'], DOWN, SMALL_BUFF)
        omega_label = omega_brace.get_tex('\\omega').set_color(PINK)
        self.play(GrowFromCenter(omega_brace), Write(omega_label))
        self.wait()
        cos_equation = Tex('e^{+i \\omega t} + e^{-i \\omega t} = 2\\cos(\\omega t)', t2c={'\\omega': PINK})
        cos_equation.move_to(imag_exps)
        omega_brace2 = omega_brace.copy()
        omega_brace2.stretch(0.5, 0).match_width(cos_equation['\\omega'][-1])
        omega_brace2.next_to(cos_equation['\\omega'][-1], DOWN, SMALL_BUFF)
        omega_brace2_tex = omega_brace2.get_tex('\\sqrt{k / m}', buff=SMALL_BUFF, font_size=24)
        self.remove(imag_exps)
        self.play(TransformFromCopy(imag_exps[0]['e^{+i \\omega t}'], cos_equation['e^{+i \\omega t}']), TransformFromCopy(imag_exps[1]['e^{-i \\omega t}'], cos_equation['e^{-i \\omega t}']), FadeOut(imag_exps[0]['x(t) =']), FadeOut(imag_exps[1]['x(t) =']), Write(cos_equation['+'][1]))
        self.wait()
        self.play(Write(cos_equation['= 2\\cos(\\omega t)']))
        self.wait()
        self.play(GrowFromCenter(omega_brace2), Write(omega_brace2_tex))
        self.play(LaggedStart(FadeOut(implies), FadeOut(cos_equation), FadeOut(omega_brace2), FadeOut(omega_brace2_tex), FadeOut(consolidated_lines[2:]), FadeOut(omega_brace), FadeOut(omega_label), lag_ratio=0.1))
        t2c = {"x''(t)": colors[2], "x'(t)": colors[1], 'x(t)': colors[0], '{s}': YELLOW}
        new_lines = VGroup(Tex("m x''(t) + \\mu x'(t) + k x(t) = 0", t2c=t2c), Tex('m ({s}^2 e^{{s}t}) + \\mu ({s} e^{{s}t}) + k (e^{{s}t}) = 0', t2c=t2c), Tex('e^{{s}t}\\left(m {s}^2 + \\mu {s} + k \\right) = 0', t2c=t2c), Tex('m {s}^2 + \\mu {s} + k = 0', t2c=t2c), Tex('{s} = {{-\\mu \\pm \\sqrt{\\mu^2 - 4mk}} \\over 2m}', t2c=t2c))
        new_lines.scale(0.7)
        new_lines.arrange(DOWN, aligned_edge=LEFT, buff=MED_LARGE_BUFF)
        new_lines.move_to(equation2, UL)
        self.play(TransformMatchingTex(equation2, new_lines[0], matched_keys=t2c.keys(), run_time=1))
        self.wait()
        for line1, line2 in zip(new_lines, new_lines[1:]):
            if line1 is new_lines[0]:
                key_map = {"x''(t)": '({s}^2 e^{{s}t})', "x'(t)": '({s} e^{{s}t})', 'x(t)': '(e^{{s}t})'}
            else:
                key_map = dict()
...
class DampedSpringSolutionsOnSPlane(InteractiveScene):
    ...
    def construct(self):
        ...
        chars = ['m', '\\mu', 'k']
        m_slider, mu_slider, k_slider = sliders = VGroup((self.get_slider(char, color) for char, color in zip(chars, colors)))
        m_tracker, mu_tracker, k_tracker = trackers = Group((slider.value_tracker for slider in sliders))
        ...
        self.wait()
        self.play(s_rhs_point.animate.move_to(rect_edge_point), VFadeOut(lines), VFadeIn(sliders[1]))
        self.wait()
        ...
        graph_label = Tex('\\text{Re}[e^{st}]', t2c={'s': YELLOW}, font_size=72)
        graph_label.next_to(axes.get_corner(UL), DL)
        self.play(frame.animate.set_height(12, about_point=4 * UP + 2 * LEFT), FadeIn(axes, time_span=(1.5, 3)), ShowCreation(graph, suspend_mobject_updating=True, time_span=(1.5, 3)), Write(graph_label), run_time=3)
        self.wait()
        ...
        self.wait()
        self.play(mu_tracker.animate.set_value(0.5), m_tracker.animate.set_value(3), run_time=3)
        self.wait()
    ...
    def get_slider(self, char_name, color=WHITE, x_range=(0, 5), height=1.5, font_size=36):
        ...
        label = Tex(f'{char_name} = 0.00', font_size=font_size)
...
class RotatingExponentials(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(t_tracker)
        left_plane, right_plane = planes = VGroup((ComplexPlane((-2, 2), (-2, 2), background_line_style=dict(stroke_color=BLUE, stroke_width=1)) for _ in range(2)))
        for plane in planes:
        ...
        self.add(planes)
        t2c = {'\\omega': PINK}
        left_title, right_title = titles = VGroup((Tex(tex, t2c=t2c, font_size=48) for tex in ['e^{+i \\omega t}', 'e^{-i \\omega t}']))
        ...
        left_tail, right_tail = tails = VGroup((TracingTail(vect.get_end, stroke_color=vect.get_color(), time_traced=2) for vect in vectors))
        self.add(Point())
        ...
        self.wait(12)
        spring = SrpingMassSystem(equilibrium_position=planes[0].get_bottom() + DOWN, equilibrium_length=3, n_spring_curls=8, mass_width=0.5, spring_radius=0.2)
        spring.pause()
        ...
        self.wait(20)
        self.play(VFadeOut(spring), VFadeOut(v_line), VFadeOut(tails))
        ...
        sum_expr = VGroup(titles[0], Tex('+'), titles[1])
        ...
        self.play(planes[0].animate.move_to(new_plane_center), planes[1].animate.move_to(new_plane_center).set_opacity(0), MoveToTarget(sum_expr), run_time=2)
        self.play(shift_factor.animate.set_value(1))
        self.play(FadeIn(result_dot))
        self.wait(4)
        spring = SrpingMassSystem(equilibrium_position=planes[0].get_bottom() + DOWN, equilibrium_length=5, n_spring_curls=8, mass_width=0.5, spring_radius=0.2)
        spring.pause()
        unit_size = planes[0].x_axis.get_unit_size()
        ...
        self.play(VFadeIn(spring), VFadeIn(v_line))
        self.wait(2)
        rhs = Tex('= 2 \\cos(\\omega t)', t2c={'\\omega': PINK})
...
class SimpleSolutionSummary(InteractiveScene):
    ...
    def construct(self):
        t2c = {'m': RED, 'k': TEAL, '{s}': YELLOW, '\\omega': PINK}
        kw = dict(t2c=t2c, font_size=36)
        arrow = Vector(1.5 * RIGHT)
        top_eq = VGroup(Tex("m x''(t) + k x(t) = 0", **kw), arrow, Tex('e^{{s}t}\\left(m{s}^2 + k\\right) = 0', **kw), Tex('\\Longrightarrow', **kw), Tex('{s} = \\pm i \\underbrace{\\sqrt{k / m}}_{\\omega}', **kw))
        top_eq.arrange(RIGHT)
        top_eq[-1].align_to(top_eq[2], UP)
        guess = Tex('\\text{Guess } e^{{s}t}', **kw)
        guess.scale(0.75)
...
class ShowFamilyOfComplexSolutions(RotatingExponentials):
    tex_to_color_map = {'\\omega': PINK}
    plane_config = dict(background_line_style=dict(stroke_color=BLUE, stroke_width=1), faded_line_style=dict(stroke_color=BLUE, stroke_width=0.5, stroke_opacity=0.25))
    ...
    def construct(self):
        ...
        equation = Tex("m x''(t) + k x(t) = 0", t2c=t2c, font_size=42)
        ...
        strategy_words = VGroup(Text('“Strategy”'), TexText('Guess $e^{{s}t}$', t2c={'{s}': YELLOW}, font_size=36, fill_color=GREY_A))
        strategy_words.arrange(DOWN)
        strategy_words.next_to(arrow, UP, MED_SMALL_BUFF)
        self.add(equation)
        self.play(GrowArrow(arrow), FadeIn(strategy_words, lag_ratio=0.1))
        self.wait()
        t2c = self.tex_to_color_map
        left_planes, left_plane_labels = self.get_left_planes(label_texs=['e^{+i\\omega t}', 'e^{-i\\omega t}'])
        ...
        self.add(t_tracker)
        self.play(GrowFromCenter(left_planes_brace), FadeIn(left_planes), FadeTransform(strategy_words[1]['e^{{s}t}'].copy(), left_plane_labels[0]), FadeTransform(strategy_words[1]['e^{{s}t}'].copy(), left_plane_labels[1]), VFadeIn(rot_vects))
        self.wait(3)
        self.wait(8)
        right_plane = self.get_right_plane()
        right_plane.next_to(left_planes, RIGHT, buff=1.5)
        scaled_solution = Tex('c_1 e^{+i\\omega t} + c_2 e^{-i\\omega t}', t2c={'\\omega': PINK, 'c_1': BLUE, 'c_2': BLUE})
        scaled_solution.next_to(right_plane, UP)
        vect1, vect2 = right_rot_vects = self.get_rot_vect_sum(right_plane, t_tracker)
        c1_eq, c2_eq = coef_eqs = VGroup((VGroup(Tex(f'c_{n} = '), DecimalNumber(1)) for n in [1, 2]))
        coef_eqs.scale(0.85)
        ...
        self.play(FadeIn(right_plane), FadeOut(left_planes_brace), frame.animate.center(), run_time=2)
        self.play(LaggedStart(FadeTransform(left_plane_labels[0].copy(), scaled_solution['e^{+i\\omega t}']), FadeIn(scaled_solution['c_1']), TransformFromCopy(rot_vects[0], right_rot_vects[0], suspend_mobject_updating=True), FadeTransform(left_plane_labels[1].copy(), scaled_solution['e^{-i\\omega t}']), FadeIn(scaled_solution['+'][1]), FadeIn(scaled_solution['c_2']), TransformFromCopy(rot_vects[1], right_rot_vects[1], suspend_mobject_updating=True)))
        self.play(LaggedStart(FadeTransformPieces(scaled_solution['c_1'].copy(), c1_eq), FadeTransformPieces(scaled_solution['c_2'].copy(), c2_eq)))
        self.play(LaggedStart(vect1.coef_tracker.animate.set_value(2), vect2.coef_tracker.animate.set_value(0.5), lag_ratio=0.5))
        ...
        self.wait(6)
        self.play(LaggedStart(vect1.coef_tracker.animate.set_value(complex(1.5, 1)), vect2.coef_tracker.animate.set_value(complex(0.5, -1.25))))
        self.wait(7)
        t_tracker.suspend_updating()
        self.play(FadeOut(comb_tail, suspend_mobject_updating=True), LaggedStart(vect1.coef_tracker.animate.set_value(complex(0.31, -0.41)), vect2.coef_tracker.animate.set_value(complex(2.71, -0.82))))
        self.wait()
        self.play(LaggedStart(vect1.coef_tracker.animate.set_value(complex(-1.03, 0.5)), vect2.coef_tracker.animate.set_value(complex(1.5, 0.35))))
        self.add(comb_tail)
        self.wait(2)
        ...
        self.play(LaggedStart(vect1.coef_tracker.animate.set_value(1), vect2.coef_tracker.animate.set_value(1)))
        self.wait(6)
        initial_conditions = VGroup(Tex('x_0 = 0.00'), Tex('v_0 = 0.00'))
        ...
        self.play(FadeIn(initial_conditions), Write(implies), coef_eqs.animate.next_to(implies, DOWN).align_to(initial_conditions, LEFT))
        self.remove(comb_tail)
        self.play(vect1.coef_tracker.animate.set_value(1), vect2.coef_tracker.animate.set_value(1), t_tracker.animate.set_value(0))
        self.wait()
        self.remove(comb_tail)
        t_tracker.resume_updating()
        ...
        self.wait(4)
        self.play(vect1.coef_tracker.animate.set_value(1.5), vect2.coef_tracker.animate.set_value(1.5), ChangeDecimalToValue(x0_value, 3))
        self.wait(12)

    def get_left_planes(self, label_texs: list[str]):
        planes = VGroup((ComplexPlane((-1, 1), (-1, 1), **self.plane_config) for _ in range(2)))
    ...
    def get_rot_vects(self, planes):
        t_tracker = ValueTracker(0)
        t_tracker.add_updater(lambda m, dt: m.increment_value(dt))
        rot_vects = VGroup((self.get_rotating_vector(plane, u * 1j * self.rotation_frequency, t_tracker, color) for plane, u, color in zip(planes, [+1, -1], self.vect_colors)))
        tails = VGroup((TracingTail(vect.get_end, stroke_color=vect.get_color(), time_traced=2) for vect in rot_vects))
        return Group(rot_vects, tails, t_tracker)

    def get_rot_vect_sum(self, plane, t_tracker):
        vect1, vect2 = vect_sum = VGroup((self.get_rotating_vector(plane, u * 1j * self.rotation_frequency, t_tracker, color) for u, color in zip([+1, -1], self.vect_colors)))
...
class GuessSine(InteractiveScene):
    ...
    def construct(self):
        ...
        func_tex = f'{func_name}(\\omega t)'
        t2c = {'\\omega': PINK, 'x(t)': TEAL, "x''(t)": RED}
        equation = Tex("m x''(t) + k x(t) = 0", t2c=t2c)
        equation.to_edge(LEFT)
        guess_words = TexText(f'Try $x(t) = {func_tex}$', t2c=t2c, font_size=36)
        ...
        sub = Tex(f'-m \\omega^2 {func_tex}  + k {func_tex} = 0', t2c=t2c)
        sub.next_to(arrow, RIGHT)
        simple_sub = Tex(f'\\left(-m \\omega^2 + k\\right) {func_tex} = 0', t2c=t2c)
        simple_sub.next_to(arrow, RIGHT)
        implies = Tex('\\Rightarrow', font_size=72)
        implies.next_to(simple_sub, RIGHT)
        t2c['\\ding{51}'] = GREEN
        t2c['Valid'] = GREEN
        result = TexText('\\ding{51} Valid if $\\omega = \\sqrt{k / m}$', t2c=t2c, font_size=36)
        result.next_to(simple_sub, DOWN)
        simple_sub.shift(0.05 * UP)
        blank_sub = sub.copy()
        blank_sub[func_tex].set_opacity(0)
        func_parts = sub[func_tex]
        self.play(LaggedStart(TransformMatchingTex(equation.copy(), blank_sub, path_arc=30 * DEG, run_time=2), FadeTransform(guess_words[func_tex][0].copy(), func_parts[0]), FadeTransform(guess_words[func_tex][0].copy(), func_parts[1]), lag_ratio=0.25))
        self.wait()
        self.play(TransformMatchingTex(blank_sub, simple_sub), Transform(func_parts[0], simple_sub[func_tex][0], path_arc=-30 * DEG), Transform(func_parts[1], simple_sub[func_tex][0], path_arc=-30 * DEG), run_time=1)
        self.wait()
...
class ShowFamilyOfRealSolutions(InteractiveScene):
    ...
    def construct(self):
        cos_graph, sin_graph = small_graphs = VGroup(self.get_small_graph(math.cos, '\\cos(\\omega t)', BLUE), self.get_small_graph(math.sin, '\\sin(\\omega t)', RED))
        ...
        axes.to_edge(DOWN)
        graph_label = Tex('+1.00 \\cos(\\omega t) +1.00 \\sin(\\omega t)', t2c=self.t2c)
        ...
        for c1, c2 in [(-0.5, 2), (1.5, 0), (0.25, -2)]:
            self.play(LaggedStart(coef_trackers[0].animate.set_value(c1), coef_trackers[1].animate.set_value(c2), lag_ratio=0.5))
            self.wait()
    ...
    def show_spring(self, axes, graph, func):
        ...
        spring = SrpingMassSystem(equilibrium_length=4, equilibrium_position=axes.get_right() + RIGHT, direction=UP, mass_width=0.75, spring_radius=0.2)
        spring.add_updater(lambda m: m.set_x(axes.y_axis.get_unit_size() * axes.y_axis.p2n(graph_copy.get_end())))
        h_line = Line()
        h_line.set_stroke(WHITE, 1)
        h_line.f_always.put_start_and_end_on(spring.mass.get_left, graph_copy.get_end)
        self.play(graph.animate.set_stroke(opacity=0.2), ShowCreation(graph_copy, rate_func=linear, run_time=self.x_max), VFadeIn(spring, run_time=1), VFadeIn(h_line, run_time=1))
...
class SetOfInitialConditions(InteractiveScene):
    ...
    def construct(self):
        ...
        x0_labels = VGroup((Tex(f'x_0 = {x0}', font_size=48).next_to(box, UP, SMALL_BUFF) for x0, box in zip(range(-2, 3), mid_row)))
        mid_row_springs = VGroup((box.spring for box in mid_row))
        mid_row_solutions = VGroup((self.get_solution_graph(box, x0=x0) for box, x0 in zip(mid_row, range(-2, 3))))
        ...
        self.play(GrowFromPoint(brace, brace.get_left()), last_spring.animate.set_x(last_x), Write(x0_labels[-1]))
        self.wait()
        last_spring.unpause()
        last_spring.v_vect.set_opacity(0)
        self.play(frame.animate.reorient(0, 0, 0, (6.7, -0.61, 0.0), 6.16).set_anim_args(run_time=2), FadeIn(last_solution[0]), ShowCreation(last_solution[1], rate_func=linear, run_time=self.graph_time))
        last_spring.pause()
        self.play(frame.animate.center().set_width(box_grid.get_width() + 2).set_anim_args(time_span=(0, 1.5)), last_spring.animate.set_x(1).set_anim_args(time_span=(0, 1)), LaggedStartMap(FadeIn, VGroup(*reversed(mid_row[:-1])), lag_ratio=0.75), LaggedStartMap(FadeIn, VGroup(*reversed(mid_row_springs[:-1])), lag_ratio=0.75), LaggedStartMap(FadeIn, VGroup(*reversed(x0_labels[:-1])), lag_ratio=0.75), LaggedStartMap(FadeIn, VGroup(*reversed(mid_row_solutions[:-1])), lag_ratio=0.75), run_time=3)
        self.wait()
        ...
        self.play(ShowCreation(graphs, lag_ratio=0, run_time=self.graph_time, rate_func=linear))
        self.wait()
        ...
        for spring in mid_row_springs:
            ...
            spring.v_vect.set_opacity(0)
            spring.set_velocity(0)
        self.play(FadeOut(mid_row_solutions), *(spring.animate.set_x(spring.x0) for spring in mid_row_springs))
        v0_labels = VGroup((Tex(f'v_0 = {v0}', font_size=48).next_to(row, LEFT).set_fill(TEAL) for v0, row in zip(range(2, -3, -1), box_grid)))
        other_indices = [0, 1, 3, 4]
        row_springs = VGroup((VGroup((box.spring for box in row)) for row in box_grid))
        self.play(frame.animate.set_width(box_grid.get_width() + 3, about_edge=DR), Write(v0_labels[2]))
        self.wait()
        self.play(LaggedStartMap(FadeIn, VGroup((box_grid[i] for i in other_indices)), lag_ratio=0.5, run_time=3), LaggedStartMap(FadeIn, VGroup((v0_labels[i] for i in other_indices)), lag_ratio=0.5, run_time=3), LaggedStartMap(FadeIn, VGroup((row_springs[i] for i in other_indices)), lag_ratio=0.5, run_time=3), x0_labels.animate.next_to(box_grid, UP, SMALL_BUFF).set_anim_args(run_time=1), FadeOut(brace))
        self.wait()
        for row in box_grid:
            for box in row:
                box.spring.unpause()
        self.wait(20)
        all_solutions = VGroup((self.get_solution_graph(box, x0=x0, v0=v0, graph_color=YELLOW).move_to(box).scale(0.8) for row, v0 in zip(box_grid, range(2, -3, -1)) for box, x0 in zip(row, range(-2, 3))))
        ...
        self.add(all_axes)
        self.play(ShowCreation(all_graphs, lag_ratio=0.1, run_time=3))
        self.wait()
    ...
    def get_spring_in_a_box(self, box, x0=0, v0=0, k=9, mu=0.5):
        ...
        spring = SrpingMassSystem(x0=x0, v0=v0, k=k, mu=mu, mass_width=0.1 * box_width, equilibrium_length=0.5 * box_width, equilibrium_position=box.get_center(), spring_radius=0.035 * box_width)
    ...
    def get_solution_graph(self, box, x0=2, v0=0, k=9, mu=0.5, width_factor=0.8, graph_color=TEAL):
        axes = Axes(x_range=(0, self.graph_time, 1), y_range=(-2, 2), width=width_factor * box.get_width(), height=box.get_height())
        axes.set_stroke(GREY, 1)
        axes.next_to(box, DOWN)
        s = 0.5 * (-mu + 1j * math.sqrt(4 * k - mu ** 2))
        z0 = complex(x0, (s.real * x0 - v0) / s.imag)
        graph = axes.get_graph(lambda t: (z0 * np.exp(s * t)).real)
...
class SpringInTheWind(InteractiveScene):
    ...
    def setup(self):
        super().setup()
        plane = NumberPlane()
        wind = TimeVaryingVectorField(lambda p, t: np.tile(self.external_force(t) * RIGHT, (len(p), 1)), plane, density=1, color=WHITE, stroke_width=6, stroke_opacity=0.5)
        spring = SrpingMassSystem(k=self.k, mu=self.mu, external_force=lambda: self.external_force(wind.time))
...
class ShowSpringInWindGraph(SpringInTheWind):
    ...
    def construct(self):
        ...
        axes.add(axis_label)
        graph = TracedPath(lambda: axes.c2p(self.time, spring.get_x()), stroke_color=BLUE, stroke_width=3)
        self.add(graph_block)
        self.add(axes)
        self.add(graph)
        self.wait(t_max)
        graph.clear_updaters()
        self.play(VFadeOut(spring), VFadeOut(wind))
        ...
        self.play(LaggedStart(Write(equals), FadeIn(axes1), FadeIn(shm_graph), Write(plus), FadeIn(axes2), FadeIn(cos_graph), lag_ratio=0.15))
        self.wait()
        colors = color_gradient([TEAL, RED], 3, interp_by_hsl=True)
        shm_eq = VGroup(Text('Solution to', font_size=36), Tex("mx''(t) + \\mu x'(t) + k x(t) = 0", t2c={'x(t)': colors[0], "x'(t)": colors[1], "x''(t)": colors[2]}, font_size=36, alignment=''))
        shm_eq.arrange(DOWN)
        shm_eq.next_to(axes1.x_axis, UP, buff=0.35)
        shm_eq.set_x(-2.5)
        self.add(shm_graph.copy().set_stroke(opacity=0.25))
        self.play(FadeIn(shm_eq, lag_ratio=0.1), ShowCreation(shm_graph, run_time=3, rate_func=linear))
        self.wait()
        self.play(left_highlight.animate.set_height(8, stretch=True, about_edge=UP).shift(0.15 * UP), shm_eq.animate.shift(5 * RIGHT), run_time=2)
        self.wait()
        self.add(graph.copy().set_stroke(opacity=0.25))
        self.add(cos_graph.copy().set_stroke(opacity=0.25))
        self.play(left_highlight.animate.set_fill(opacity=0.1), *(ShowCreation(mob, run_time=15, rate_func=linear) for mob in [graph, shm_graph, cos_graph]))