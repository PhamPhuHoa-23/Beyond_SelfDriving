...
class DrivenHarmonicOscillatorEquation(InteractiveScene):
    ...
    def construct(self):
        ...
        equation = Tex("m x''(t) + \\mu x'(t) + k x(t) = F_0 \\cos(\\omega t)", t2c={'x(t)': colors[0], "x'(t)": colors[1], "x''(t)": colors[2], '\\omega': PINK})
        self.add(equation)
...
class SimpleCosGraph(InteractiveScene):
    ...
    def construct(self):
        ...
        x_axis = axes.x_axis
        x_axis.add(VGroup((Tex(tex, font_size=20).next_to(x_axis.n2p(n), DOWN) for n, tex in zip(it.count(1, 2), self.get_pi_frac_texs()))))
        ...
        cos_label = Tex('\\cos(t)', font_size=60)
    ...
    def get_pi_frac_texs(self):
        return ['\\pi / 2', '\\pi', '3 \\pi / 2', '2\\pi', '5\\pi / 2', '3\\pi', '7 \\pi / 2', '4\\pi', '9\\pi / 2', '5\\pi']

class BreakUpCosineTex(InteractiveScene):

    def construct(self):
        pure_cos = Tex('\\cos(t)', font_size=72)
        pure_cos.to_edge(UP)
        tex_pieces = ['\\cos(t)', '=', '\\frac{1}{2}', 'e^{+it}', '{+}', '{1 \\over 2}', 'e^{-it}']
        sum_tex = Tex(' '.join(tex_pieces), t2c={'+i': YELLOW, '-i': YELLOW})
        sum_tex.to_edge(UP, buff=MED_SMALL_BUFF).shift(0.5 * RIGHT)
        cos, equals, half1, eit, plus, half2, enit = pieces = VGroup((sum_tex[tex][0] for tex in remove_list_redundancies(tex_pieces)))
        self.add(pure_cos)
        self.wait()
        self.play(FadeTransform(pure_cos, cos), Write(pieces[1:]))
        self.wait()
...
class TranslateToNewLanguage(InteractiveScene):
    ...
    show_integral = True
    label_config = dict(font_size=72, t2c={'{t}': BLUE, '{s}': YELLOW})

    def construct(self):
        full_s_samples = self.get_s_samples()
        func_s_samples = [complex(-2, 2), complex(-2, -2), complex(0, 1), complex(-1, 0), complex(0, -1)]
        func_weights = [-1, -1, 1j, 2, -1j]

        def func(t):
            return sum([(weight * np.exp(complex(0.1 * s.real, s.imag) * t)).real for s, weight in zip(func_s_samples, func_weights)])
        ...
        self.play(LaggedStart(FadeIn(axes), ShowCreation(graph), FadeIn(graph_label), FadeIn(s_plane_name, lag_ratio=0.1), LaggedStartMap(FadeIn, exp_pieces, lag_ratio=0.1)))
        self.wait()
        ...
        weight_labels = VGroup((Tex(f'\\times {w}', font_size=24).next_to(piece.get_top(), DOWN, SMALL_BUFF) for w, piece in zip(['\\minus 1', '\\minus 1', '\\minus i', '2', 'i'], key_pieces)))
        self.play(MoveToTarget(exp_pieces), LaggedStartMap(FadeIn, weight_labels))
        self.play(LaggedStart((Transform(graph.copy(), piece[-1].copy().insert_n_curves(100), remover=True) for piece in key_pieces), lag_ratio=0.1, group_type=Group, run_time=2))
        self.wait()
        frame = self.frame
        ...
        def Func(s):
            result = sum([np.divide(w, s - s0) for s0, w in zip(func_s_samples, func_weights)])
        ...
        self.play(FadeOut(s_plane_name), GrowArrow(arrow), Write(fancy_L), Restore(Fs_label, time_span=(1, 2), path_arc=-10 * DEG), FadeIn(s_plane), MoveToTarget(exp_pieces, lag_ratio=0.001), FadeOut(weight_labels), Restore(lt_graph, time_span=(1.5, 3)), frame.animate.reorient(70, 86, 0, (-4.94, -2.45, 3.51), 19.43), run_time=3)
        if self.show_integral:
            integral = Tex('= \\int^\\infty_0 f({t})e^{\\minus{s}{t}}d{t}', t2c=self.label_config['t2c'])
            ...
            self.play(FadeIn(rect), Write(integral, run_time=1))
            self.wait()
            lt_graph.set_clip_plane(RIGHT, s_plane.get_left()[0])
            self.play(frame.animate.reorient(-10, 85, 0, (-2.18, 0.45, 3.12), 11.52), lt_graph.animate.set_clip_plane(RIGHT, -s_plane.n2p(0)[0]), run_time=3)
            self.play(frame.animate.reorient(33, 85, 0, (-1.81, 0.13, 2.44), 12.25), run_time=6)
            self.wait()
            self.add(graph_copy, rect, Fs_label, integral)
            self.play(FadeOut(rect), ShowCreation(graph_copy), frame.animate.reorient(-2, 67, 0, (-0.95, -0.06, 1.91), 9.45), run_time=8)
            for piece in key_pieces:
                piece.set_height(1)
            self.add(key_pieces, graph_copy, graph)
            self.play(frame.animate.reorient(0, 0, 0, (-1.23, 1.49, 1.9), 9.36), FadeIn(key_pieces), exp_pieces.animate.fade(0.5), run_time=3)
        ...
        pole_lines = VGroup((Line(s_plane.n2p(s), s_plane.n2p(s) + 20 * OUT) for s in func_s_samples))
        ...
        self.play(ShowCreation(pole_lines, lag_ratio=0.1))
        self.play(frame.animate.reorient(0, 0, 0, (-0.98, 0.82, 0.0), 10.0), lt_graph[0].animate.set_opacity(0.2), lt_graph[1].animate.set_opacity(0.05), pole_lines.animate.stretch(0, 2, about_edge=IN), MoveToTarget(key_pieces), FadeIn(weight_labels), run_time=3)
        self.wait()
        ...
        self.play(Restore(top_rect), ShowCreation(h_line, time_span=(1, 2)), VGroup(axes, graph).animate.shift(2 * DOWN), VGroup(graph_label, arrow, fancy_L, Fs_label).animate.shift(3 * DOWN), frame.animate.reorient(-17, 90, 0, (-2.86, 1.57, 3.14), 10.95), run_time=2)
        self.play(frame.animate.reorient(39, 92, 0, (-4.35, 0.64, 3.03), 14.99), run_time=20)

    def get_graph_group(self, func, func_tex='f({t})'):
    ...
    def get_s_plane_and_exp_pieces(self, s_samples):
        ...
        exp_pieces = VGroup((self.get_exp_graph(s).move_to(s_plane.n2p(s)) for s in s_samples))
    ...
    def get_arrow_to_Fs(self, graph_label):
        ...
        fancy_L = Tex('\\mathcal{L}', font_size=60)
        fancy_L.next_to(arrow, UP, buff=0)
        fancy_L.fix_in_frame()
        Fs_label = Tex('F({s})', **self.label_config)
...
class TranslateDifferentialEquationAndInvert(InteractiveScene):
    ...
    def construct(self):
        colors = color_gradient([TEAL, RED], 3, interp_by_hsl=True)
        t2c = {'{s}': YELLOW, 'x(t)': colors[0], "x'(t)": colors[1], "x''(t)": colors[2], '\\omega': PINK}
        kw = dict(t2c=t2c, font_size=30)
        lhs = Tex("m x''(t) + \\mu x'(t) + k x(t) = F_0 \\cos(\\omega t)", **kw)
        rhs1 = Tex('m {s}^2 X({s}) + \\mu {s} X({s}) + k X({s}) = \\frac{F_0 {s}}{({s}^2 + \\omega^2)}', **kw)
        rhs2 = Tex('X({s}) \\left( m {s}^2 + \\mu {s} + k \\right) = \\frac{F_0 {s}}{({s}^2 + \\omega^2)}', **kw)
        rhs3 = Tex('X({s}) = \\frac{F_0 {s}}{\\left({s}^2 + \\omega^2\\right) \\left( m {s}^2 + \\mu {s} + k \\right)}', **kw)
        ...
        arrow_label = Tex('\\mathcal{L}', font_size=72)
        ...
        self.play(FadeIn(lhs, lag_ratio=0.1), FadeIn(ode_word, lag_ratio=0.1))
        self.wait()
        self.play(GrowArrow(arrow), FadeIn(arrow_label, DOWN))
        self.play(TransformMatchingTex(lhs.copy(), rhs1, path_arc=-10 * DEG, lag_ratio=0.03, key_map={'F_0 \\cos(\\omega t)': '\\frac{F_0 {s}}{({s}^2 + \\omega^2)}', 'x(t) = ': 'X({s}) = ', "x'(t)": '{s} X({s})', "x''(t)": '{s}^2 X({s})', '(t)': '({s})'}))
        self.wait()
        self.play(FadeIn(algebra_word, 0.5 * DOWN), TransformMatchingTex(rhs1, rhs2, path_arc=-30 * DEG))
        self.play(TransformMatchingTex(rhs2, rhs3, path_arc=-10 * DEG, matched_keys=['\\left( m {s}^2 \\mu s + k \\right)', 'X({s})']))
        self.wait()
        inv_L = Tex('\\mathcal{L}^{-1}', font_size=72)
        inv_L.next_to(arrow, LEFT, buff=0)
        xt = Text('Solution', font_size=72)
        xt.next_to(arrow, UP, MED_LARGE_BUFF)
        self.play(LaggedStart(Rotate(arrow, PI), ReplacementTransform(arrow_label, inv_L), lhs.animate.scale(0.5).to_corner(UR)))
...
class DesiredMachine(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FadeOutToPoint(in_func, machine.get_bottom(), lag_ratio=0.025))
        self.wait()
        if show_ode:
            ode = Tex("m x''({t}) + \\mu x'({t}) + k x(t) = F_0 \\cos(\\omega{t})", t2c=t2c, font_size=60)
            ode.next_to(machine, UP, MED_LARGE_BUFF)
            ode_ghost = ode.copy().set_fill(opacity=0.5)
            self.play(in_func_ghost.animate.to_edge(UP, buff=MED_SMALL_BUFF), FadeIn(ode, lag_ratio=0.1))
            self.wait()
            self.add(ode_ghost)
            self.play(LaggedStart((FadeOutToPoint(piece, machine.get_top() + 0.5 * DOWN, path_arc=arc) for piece, arc in zip(ode, np.linspace(-70 * DEG, 70 * DEG, len(ode)))), lag_ratio=0.05, run_time=2))
            self.wait()
        out_func = Tex('x({t}) = c_1 e^{s_1 {t}} + c_2 e^{s_2 {t}} + c_3 e^{s_3 {t}} + c_4 e^{s_4 {t}}', t2c=t2c, font_size=72)
        ...
        self.play(LaggedStart((FadeInFromPoint(piece, machine.get_bottom() + 0.5 * UP, path_arc=arc) for piece, arc in zip(out_func, np.linspace(-70 * DEG, 70 * DEG, len(out_func)))), lag_ratio=0.05, run_time=2))
        if False:
            text = Text('Laplace Transform', font_size=72)
            machine.set_z_index(0)
            self.play(LaggedStart(FadeOut(in_func_ghost, UP), FadeOut(ode_ghost, 2 * UP), FadeOut(out_func, DOWN), FadeTransform(fancy_L[0], text[0]), FadeIn(text[1:], lag_ratio=0.1), run_time=2, lag_ratio=0.2), FadeOut(machine, scale=3, run_time=2))
            self.wait()
        ...
        self.play(Write(s_rects), out_func.animate.set_opacity(0.75))
        self.wait()
        self.play(ReplacementTransform(s_rects, c_rects, lag_ratio=0.1), FadeOut(s_part_copies), FadeIn(c_part_copies))
        self.wait()
        ...
        exp_piece = Tex('e^{{s}{t}}', t2c=t2c, font_size=90)
        exp_piece.next_to(randy, UR, LARGE_BUFF).shift(0.5 * DOWN)
        exp_piece.insert_submobject(2, VectorizedPoint(exp_piece[2].get_right()))
        self.play(LaggedStartMap(FadeOut, mobs, run_time=2), TransformFromCopy(out_func['e^{s_1 {t}}'][0], exp_piece, run_time=2), VFadeIn(randy, time_span=(0.5, 2.0)), randy.change('confused', exp_piece).set_anim_args(run_time=2))
...
class ExpDeriv(InteractiveScene):
    ...
    def construct(self):
        t2c = {'{t}': BLUE, '{s}': YELLOW}
        lhs, rhs = terms = VGroup((Tex(tex, t2c=t2c, font_size=90) for tex in ['e^{{s}{t}}', '{s} e^{{s}{t}}']))
        terms.arrange(RIGHT, buff=4)
        arrows = VGroup((Arrow(lhs.get_corner(sign * UP + RIGHT), rhs.get_corner(sign * UP + LEFT), path_arc=-sign * 75 * DEG, thickness=4) for sign in [1, -1]))
        arrows.set_fill(border_width=2)
        arrows[1].shift(0.25 * DOWN)
        arrow_labels = VGroup((Tex(tex, t2c=t2c, font_size=60).next_to(arrow, vect) for arrow, tex, vect in zip(arrows, ['d / d{t}', '\\times {s}'], [UP, DOWN])))
        self.add(terms[0])
        self.play(TransformFromCopy(terms[0].copy(), terms[1][1:], path_arc=-75 * DEG), Write(arrows[0], time_span=(0.5, 1.5)), FadeIn(arrow_labels[0], lag_ratio=0.1), run_time=1.5)
        self.play(FadeTransform(terms[1][2].copy(), terms[1][0], path_arc=90 * DEG, run_time=0.75))
        self.play(LaggedStart(TransformFromCopy(*arrows), TransformFromCopy(*arrow_labels)))
        self.wait(2)
...
class IntroduceTransform(SPlane):
    ...
    def construct(self):
        ...
        q_marks = Tex('???', font_size=72).space_out_submobjects(1.5)
        q_marks.next_to(transform_word_rect, UP)
        q_marks.set_color(YELLOW)
        self.add(laplace_transform_word)
        self.wait()
        self.play(LaggedStart(VFadeIn(randy), randy.change('confused', transform_word), ShowCreation(transform_word_rect), laplace_word.animate.set_opacity(0.5), LaggedStartMap(FadeIn, q_marks, shift=0.5 * UP, lag_ratio=0.25), lag_ratio=0.1))
        self.wait()
        t2c = {'{t}': BLUE, '{s}': YELLOW}
        ...
        mapsto = Tex('\\xmapsto{\\qquad}', additional_preamble='\\usepackage{mathtools}')
        mapsto.rotate(-90 * DEG)
        mapsto.set_fill(border_width=2)
        t_mob, _, ft_mob = func_group = VGroup(Tex('{t}', t2c=t2c), mapsto, Tex('f({t})', t2c=t2c))
        ...
        self.play(MoveToTarget(name_group), randy.change('pondering', func_group), FadeIn(func_name, lag_ratio=0.1))
        self.play(Restore(mapsto), FadeIn(t_mob, 0.1 * UP))
        self.play(LaggedStart(TransformFromCopy(t_mob, ft_mob[2], path_arc=45 * DEG, run_time=1), FadeTransform(func_name[0].copy(), ft_mob[0], path_arc=45 * DEG, run_time=1), Write(ft_mob[1::2]), lag_ratio=0.25))
        self.wait()
        func_group.generate_target()
        func_group.target.arrange(DOWN)
        func_group.target.move_to(4 * LEFT)
        braces = VGroup((Brace(func_group.target, direction, SMALL_BUFF).scale(1.25, about_point=func_group.target.get_center()) for direction in [LEFT, RIGHT]))
        braces.save_state()
        braces.set_opacity(0)
        for brace in braces:
            brace.replace(func_group, dim_to_match=1)
        short_func_name = Tex('f')
        ...
        self.play(LaggedStart(MoveToTarget(func_group), Restore(braces), LaggedStart([FadeTransform(func_name[0], short_func_name)] + [char.animate.set_opacity(0).replace(short_func_name) for char in func_name[1:]], lag_ratio=0.02, remover=True), GrowArrow(right_arrow), lag_ratio=0.05))
        ...
        F_braces, s_mob, F_mob, right_mapsto, Fs_mob = F_group = VGroup(braces.copy(), Tex('{s}', t2c=t2c), Tex('F'), mapsto.copy(), Tex('F({s})', t2c=t2c))
        for mob1, mob2 in zip(f_group, F_group):
            mob2.move_to(mob1)
        F_group[2].shift(SMALL_BUFF * LEFT)
        F_group.next_to(right_arrow, RIGHT, MED_LARGE_BUFF)
        self.play(LaggedStart((TransformFromCopy(*pair, path_arc=-60 * DEG) for pair in zip(f_group, F_group)), lag_ratio=0.025, run_time=8), randy.change('tease').scale(0.75).to_corner(DR))
        self.wait()
        ...
        self.play(ShowCreation(rect), GrowArrow(vect), randy.change('pondering', rect))
        self.wait()
        self.play(rect.animate.surround(f_group[-1][0], buff=0.05), MaintainPositionRelativeTo(vect, rect))
        self.wait()
        self.play(Blink(randy))
        self.play(rect.animate.surround(f_group[1], buff=0.05).set_anim_args(path_arc=-90 * DEG), vect.animate.rotate(PI).next_to(f_group[1], UP, SMALL_BUFF).set_anim_args(path_arc=-90 * DEG))
        self.wait()
        self.play(rect.animate.surround(F_group[1], buff=0.05).set_anim_args(path_arc=-90 * DEG), MaintainPositionRelativeTo(vect, rect), randy.animate.look_at(F_group))
        self.play(Blink(randy))
        self.wait()
        self.play(FadeOut(vect), FadeOut(rect))
        lt_def = Tex('\\int^\\infty_0 f({t}) e^{\\minus {s}{t}} d{t}', t2c=t2c)
        ...
        q_marks = Tex('???').set_color(YELLOW)
        q_marks.next_to(randy, UP, SMALL_BUFF).shift(0.25 * RIGHT + 0.5 * DOWN)
        self.play(FadeOut(Fs_mob), Write(lt_def), randy.change('pleading').scale(0.75, about_edge=DR), FadeIn(q_marks, 0.1 * UP, lag_ratio=0.1), MoveToTarget(F_braces), new_interior[:3].animate.match_x(lt_def).shift(0.1 * UP))
        self.play(Blink(randy))
        self.wait()
        ...
        self.play(ShowCreation(rect), randy.change('confused'))
        self.wait()
        self.play(Transform(rect, s_vect.copy().match_style(rect)), FadeIn(s_vect, time_span=(0.75, 1)))
        self.play(randy.change('pondering', s_vect), FadeOut(q_marks), FadeOut(rect))
        self.play(Blink(randy))
        self.wait()
        F_lhs = Tex('F({s}) = ', t2c=t2c)
        F_lhs.move_to(lt_def, LEFT)
        right_shift = F_lhs.get_width() * RIGHT
        self.play(TransformFromCopy(s_mob, F_lhs['{s}'][0]), TransformFromCopy(F_mob, F_lhs['F'][0]), Write(F_lhs[re.compile('\\(|\\)|=')]), lt_def.animate.shift(right_shift), MaintainPositionRelativeTo(s_vect, lt_def), F_braces[1].animate.shift(right_shift), new_interior[:3].animate.shift(right_shift + 0.1 * UP), randy.animate.shift(0.5 * DOWN), self.frame.animate.shift(0.5 * DOWN))
        F_rect = SurroundingRectangle(F_lhs[0], buff=0.05)
        F_rect.set_stroke(RED, 2)
        self.play(ShowCreation(F_rect), s_vect.animate.next_to(F_rect, UP, SMALL_BUFF).set_color(RED))
        self.wait()
        self.play(F_rect.animate.surround(ft_mob[0], buff=0.05), s_vect.animate.next_to(ft_mob[0], UP, SMALL_BUFF), randy.animate.look_at(ft_mob))
        self.play(Blink(randy))
        self.wait()
        ...
        self.play(LaggedStartMap(FadeOut, everything, shift=0.1 * DOWN, lag_ratio=0.1), lt_group.animate.center().set_height(2), self.frame.animate.to_default_state(), run_time=2)
        outer_part = VGroup(lt_def['\\int^\\infty_0'][0], lt_def['d{t}'][0])
        inner_part = lt_def['f({t}) e^{\\minus {s}{t}}'][0]
        ...
        self.play(ShowCreation(rect), outer_part.animate.set_fill(opacity=0.2), F_lhs.animate.set_fill(opacity=0.2))
        self.wait()
        self.play(rect.animate.surround(outer_part, buff=SMALL_BUFF), outer_part.animate.set_fill(opacity=1), inner_part.animate.set_fill(opacity=0.5))
        self.wait()
        ...
        self.play(MoveToTarget(lt_group), rect.animate.surround(lt_group.target).set_stroke(width=0))
        self.wait()
        self.play(rect.animate.surround(lt_def['s'][0], buff=0.05).set_stroke(YELLOW))
        self.wait()
        ...
        s_label = Tex('s').set_color(YELLOW)
        s_label.always.next_to(s_dot[0], UR, SMALL_BUFF)
        self.play(Write(plane, lag_ratio=0.01), TransformFromCopy(lt_def['s'][0], s_label), FadeInFromPoint(s_dot, rect.get_center()), FadeOut(rect))
        self.wait()
        final_z = -2 + 1j
        n_iterations = 12
        for n in range(12):
        ...
        lt_of_cos = Tex(lt_def.get_tex().replace('f({t})', '\\cos({t})'), t2c=t2c)
        lt_of_cos.move_to(lt_def, LEFT)
        cos_lt_group = VGroup(F_lhs, VGroup(lt_of_cos['\\int^\\infty_0'][0], lt_of_cos['d{t}'][0]), lt_of_cos['\\cos({t}) e^{\\minus {s}{t}}'][0])
        cos_lt_group[1].set_fill(opacity=0.2)
        imag_circles = VGroup((Dot(plane.n2p(z)).set_stroke(YELLOW, 1).set_fill(opacity=0.25) for z in [1j, -1j]))
        f_term = lt_def['f'][-1]
        cos_term = lt_of_cos['\\cos'][0]
        f_rect = SurroundingRectangle(f_term, buff=0.05)
        f_rect.set_stroke(PINK, 2)
        self.play(ShowCreation(f_rect))
        self.play(TransformMatchingTex(lt_def, lt_of_cos, matched_pairs=[(f_term, cos_term)], run_time=1), f_rect.animate.surround(cos_term, buff=0.05), F_lhs.animate.shift((lt_of_cos.get_width() - lt_def.get_width()) * 0.5 * LEFT), *map(ShowCreation, imag_circles))
        ...
        graph = get_complex_graph(plane, lambda s: s / (s ** 2 + 1), resolution=(501, 501), mesh_resolution=(31, 31))
        graph.stretch(0.25, 2, about_point=plane.n2p(0))
        self.play(cos_lt_group.animate.to_edge(LEFT, buff=MED_SMALL_BUFF).set_fill(opacity=1), ShowCreation(graph[0]), Write(graph[1], stroke_width=1, time_span=(2, 4)), frame.animate.reorient(-28, 74, 0, OUT, 8.37), run_time=4)
        ...
        if self.long_ambient_graph_display:
            ...
            s_plane.restore()
            self.play(frame.animate.to_default_state(), MoveToTarget(s_plane), MoveToTarget(s_dot), run_time=2)
        ...
        self.add(interior)
        self.play(ShowCreation(interior_rect), cos_lt_group[:2].animate.set_fill(opacity=0.2))
        self.wait()
        arrow = Vector(DOWN)
        arrow.match_color(interior_rect)
        arrow.next_to(interior_rect, DOWN, SMALL_BUFF)
        expanded = Tex('\\frac{1}{2} \\left(e^{i{t}} + e^{\\minus i{t}} \\right) e^{\\minus{s}{t}}', t2c={'i': WHITE, '-i': WHITE, **t2c})
        expanded.next_to(arrow, DOWN, MED_LARGE_BUFF)
        expanded_brace = Brace(expanded, UP, SMALL_BUFF)
        expanded_brace.set_color(TEAL)
        index = -4
        self.play(FadeTransform(interior[:index].copy(), expanded[:index]), TransformFromCopy(interior[index:], expanded[index:]), GrowArrow(arrow), FadeInFromPoint(expanded_brace, arrow.get_start()))
        self.wait()
        eit, est = pair = VGroup(expanded['e^{i{t}}'][0], expanded['e^{\\minus{s}{t}}'][0])
        pair_rects = VGroup((SurroundingRectangle(p, buff=0.05) for p in pair))
        pair_rects.set_stroke(BLUE, 2)
        pair_copy = pair.copy()
        (self.add(pair_copy),)
        self.play(expanded.animate.set_fill(opacity=0.5), ShowCreation(pair_rects, lag_ratio=0.5))
        self.wait()
        combined_term = Tex('e^{(i - {s}){t}}', t2c=t2c)
        ...
        self.play(TransformMatchingShapes(pair.copy(), combined_term), *map(GrowArrow, comb_arrows), run_time=1)
        self.wait()
        s_plane = plane
        s_plane.set_height(3.5)
        s_plane.to_corner(UR, buff=MED_SMALL_BUFF).shift(LEFT)
        ...
        output_label = self.get_output_dot_and_label(exp_plane, get_s=get_ims, get_t=get_t, s_tex='(i - {s})')
        output_path = self.get_output_path(exp_plane, get_t, get_ims)
        self.play(FadeIn(s_plane), FadeIn(s_dot), FadeIn(s_label))
        self.play(FadeIn(exp_plane), FadeTransform(combined_term.copy(), output_label[1]), FadeIn(output_label[0]))
        ...
        self.play(FadeOut(eq_i_rhs), FadeOut(output_label))
        self.play()
        for _ in range(6):
            self.play(s_tracker.animate.set_value(complex(random.uniform(-0.1, 0.5), random.uniform(-3, 3))), run_time=2)
            self.wait()
        ...
        self.play(ReplacementTransform(pair_rects, VGroup(big_rect)), LaggedStartMap(FadeOut, VGroup(comb_arrows, combined_term, down_arrow, arrow_label, const, eq_1)), FadeOut(Group(exp_plane, s_plane, s_label, s_dot)), expanded.animate.set_fill(opacity=1))
        self.play(FadeIn(goal, lag_ratio=0.1))
        self.wait()
        self.play(FadeOut(goal), big_rect.animate.surround(expanded))
        self.wait()
        self.add(expanded)
        self.remove(pair_copy)
        self.play(LaggedStartMap(FadeOut, VGroup(interior_rect, arrow, expanded_brace, expanded, big_rect)), cos_lt_group.animate.set_fill(opacity=1))
        self.wait()
...
class SimpleToComplex(InteractiveScene):
    ...
    def construct(self):
        t2c = {'{t}': BLUE, '{s}': YELLOW}
        terms = VGroup(Tex('f({t}) = 1', t2c=t2c), Tex('f({t}) = e^{at}', t2c=t2c), Tex('f({t}) = \\sum_{n=1}^N c_n e^{s_n {t}}', t2c=t2c))
        terms.scale(1.5)
        terms.arrange(DOWN, buff=1.5)
        arrows = VGroup((Arrow(*pair, buff=0.25, thickness=5) for pair in zip(terms, terms[1:])))
        terms[2].shift(SMALL_BUFF * DOWN)
        self.add(terms[0])
        for arrow, term1, term2 in zip(arrows, terms, terms[1:]):
            self.play(TransformMatchingTex(term1.copy(), term2, key_map={'1': 'e^{at}', 'a': 's_n'}), GrowArrow(arrow), run_time=1)
            self.wait()
        lt_left_x = terms.get_x(RIGHT) + 3
        lt_arrows = VGroup((Arrow(term.get_right(), lt_left_x * RIGHT + term.get_y() * UP, buff=0.5, thickness=6) for term in terms))
        lt_arrows.set_color(GREY_A)
        lt_arrow_labels = VGroup((Tex('\\mathcal{L}').next_to(arrow, UP, buff=0) for arrow in lt_arrows))
        lt_integrals = VGroup((Tex('\\int^\\infty_0 ' + tex + '\\cdot e^{\\minus{s}{t}}d{t}', t2c=t2c) for tex in ['1', 'e^{a{t}}', '\\sum_{n=1}^N c_n e^{s_n {t}}']))
        lt_rhss = VGroup(Tex('= \\frac{1}{{s}}', t2c=t2c), Tex('= \\frac{1}{{s} - a}', t2c=t2c), Tex('= \\sum_{n=1}^N \\frac{c_n}{{s} - s_n}', t2c=t2c))
        for integral, arrow, rhs in zip(lt_integrals, lt_arrows, lt_rhss):
            integral.next_to(arrow, RIGHT)
            rhs.scale(1.25)
            rhs.next_to(integral, RIGHT)
        frame = self.frame
        self.play(LaggedStart(GrowArrow(lt_arrows[0]), Write(lt_arrow_labels[0]), FadeIn(lt_integrals[0], RIGHT), lag_ratio=0.3), terms[1:].animate.set_fill(opacity=0.1), arrows.animate.set_fill(opacity=0.1), frame.animate.set_x(4), run_time=2)
        self.wait()
        self.play(Write(lt_rhss[0]))
        self.wait()
        for index in [1, 2]:
            self.play(arrows[index - 1].animate.set_fill(opacity=1), terms[index].animate.set_fill(opacity=1))
            self.play(LaggedStart(GrowArrow(lt_arrows[index]), Write(lt_arrow_labels[index]), FadeIn(lt_integrals[index], RIGHT), lag_ratio=0.3))
            self.wait()
...
class SetSToMinus1(InteractiveScene):
    ...
    def construct(self):
        eq = Tex('\\frac{1}{\\minus 1} = \\minus 1')
        eq['\\minus 1'][0].set_color(YELLOW)
        self.play(Write(eq))
        self.wait()
...
class RealExtension(InteractiveScene):
    ...
    def construct(self):
        ...
        limited_domain_line = Line(axes.c2p(limited_domain[0], 0), axes.c2p(limited_domain[1], 0))
        ...
        self.add(axes)
        self.play(ShowCreation(partial_graph), Write(f_label))
        self.play(ShowCreation(limited_domain_line), FadeIn(limited_domain_words, lag_ratio=0.1))
        self.wait()
        ...
        def get_extension(nudge_size=0):
            ...
            result = VGroup(self.get_extension(axes, points[3::-1], pre_xs, func, nudge_size=nudge_size), self.get_extension(axes, points[-4:], post_xs, func, nudge_size=nudge_size))
        ...
        tan_line = always_redraw(lambda: axes.get_tangent_line(x_tracker.get_value(), partial_graph, length=2).set_stroke(WHITE, 3))
        ...
        group.set_z_index(-1)
        self.play(FadeOut(extension), FadeIn(full_graph))
        self.play(full_graph.animate.become(modifed_graph), rate_func=lambda t: wiggle(t, 5), run_time=8)
...
class ComplexExtension(InteractiveScene):
    ...
    def construct(self):
        input_plane, output_plane = planes = VGroup((ComplexPlane((-3, 3), (-3, 3)) for n in range(2)))
        planes.set_height(5)
        ...
        def point_func(points):
            return np.array([output_plane.n2p(func(input_plane.p2n(p))) for p in points])
        mapped_domain = domain.copy().apply_points_function(point_func, about_edge=None)
        mapped_domain.set_z_index(2)
        arrow = Arrow(domain.get_top(), mapped_domain.get_top(), path_arc=-90 * DEG, fill_color=TEAL, thickness=5)
        arrow.set_z_index(1)
        func_label = Tex('f(z)')
        func_label.next_to(arrow, UP, SMALL_BUFF)
        func_label.set_backstroke()
        self.play(Write(arrow, time_span=(0, 1)), Write(func_label, time_span=(0.5, 1.5)), ReplacementTransform(domain.copy().set_fill(opacity=0), mapped_domain, path_arc=-90 * DEG, run_time=3))
        ...
        self.play(Write(extension, run_time=1, lag_ratio=0.01))
        self.wait()
        self.play(TransformFromCopy(extension.copy().set_fill(opacity=0), mapped_extension, path_arc=-90 * DEG), run_time=3)
        self.wait()
        frame = self.frame
        possibilities = VGroup(Text('1) There is no such extension', font_size=72), Text('2) There is only one extension', font_size=72))
        ...
        self.play(Write(possibilities[0][:2]), Write(possibilities[1][:2]), frame.animate.reorient(0, 0, 0, (0, 1.5, 0.0), 9).set_anim_args(time_span=(0, 1)), run_time=2)
        self.wait()
        self.play(FadeIn(possibilities[0][2:], lag_ratio=0.1))
        self.wait()
        self.play(FadeIn(possibilities[1][2:], lag_ratio=0.1))
        self.play(ShowCreation(underline), only_one.animate.set_fill(YELLOW))
        self.wait()
        ...
        self.play(FadeOut(possibilities, lag_ratio=0.05), FadeOut(underline), Write(ac_words, time_span=(0.5, 2.0)))
        self.play(Write(mapped_big_domain, lag_ratio=0.05, stroke_color=RED, stroke_width=0.5), Write(big_domain, lag_ratio=0.05, stroke_color=RED, stroke_width=0.5), run_time=8)
        self.wait()
    ...
    def get_rect_group(self, width, height, plane, corner_value=0, square_density=5, color=BLUE_E):
        ...
        rect_lines = VGroup(Line(DOWN, UP).get_grid(1, width * line_density + 1, buff=SMALL_BUFF), Line(LEFT, RIGHT).get_grid(height * line_density + 1, 1, buff=SMALL_BUFF))
...
class ZetaFunctionPlot(InteractiveScene):
    ...
    def construct(self):
        ...
        graph = get_complex_graph(s_plane, zeta_log_deriv, resolution=self.resolution)
        graph.set_clip_plane(RIGHT, -1)
        graph.set_opacity(0.6)
        self.add(graph)
        frame = self.frame
        frame.reorient(24, 82, 0, (0.39, 0.58, 0.49), 4.02)
        self.play(frame.animate.reorient(-14, 79, 0, (-0.62, -0.08, 1.41), 8.12), run_time=8)
        self.play(graph.animate.set_clip_plane(RIGHT, x_max), frame.animate.reorient(-13, 80, 0, (-0.28, -0.01, 2.17), 12.57), run_time=5)
        self.play(frame.animate.reorient(32, 81, 0, (-0.98, -2.23, 5.19), 32.31), run_time=20)
        self.play(frame.animate.reorient(91, 84, 0, (-0.25, -1.54, 7.01), 32.31), run_time=15)

class WriteZetaPrimeFact(InteractiveScene):

    def construct(self):
        formula = Tex("\\frac{\\zeta'({s})}{\\zeta({s})} = \\sum_{spacer} \\sum_{k=1}^\\infty \\frac{1}{k} \\frac{1}{p^{s}}", t2c={'{s}': YELLOW})
...
class SimpleExpToPole(InteractiveScene):
    ...
    def construct(self):
        ...
        lhs, arrow, rhs = group = VGroup(Tex('e^{a{t}}', **kw), Vector(1.5 * RIGHT, thickness=5), Tex('{1 \\over {s} - a}', **kw))
        group.arrange(RIGHT, buff=MED_LARGE_BUFF)
        group.to_edge(UP, MED_LARGE_BUFF)
        fancy_L = Tex('\\mathcal{L}')
        fancy_L.next_to(arrow, UP, SMALL_BUFF)
        self.play(FadeIn(lhs))
        self.play(GrowArrow(arrow), Write(fancy_L), run_time=1)
        self.play(Write(rhs[:-1]), TransformFromCopy(lhs['a'], rhs['a']))
        self.wait()

class Linearity(InteractiveScene):

    def construct(self):
        t2c = {'f': GREEN, 'g': BLUE, '\\mathcal{L}': GREY_A, '\\big\\{': WHITE, '\\big\\}': WHITE, 'a': GREEN_A, 'b': BLUE_A}
        lhs = Tex('\\mathcal{L}\\big\\{a \\cdot f(t) + b \\cdot g(t) \\big\\}', t2c=t2c)
        rhs = Tex('a \\cdot \\mathcal{L} \\big\\{ f(t) \\big\\} + b \\cdot \\mathcal{L} \\big\\{ g(t) \\big\\}', t2c=t2c)
        arrow = Vector(RIGHT, thickness=4)
        group = VGroup(lhs, arrow, rhs)
        group.arrange(RIGHT)
        self.add(lhs)
        self.wait()
        self.play(GrowArrow(arrow), LaggedStart(AnimationGroup(*(TransformFromCopy(lhs[tex][0], rhs[tex][0], path_arc=45 * DEG) for tex in ['\\mathcal{L}', '\\big\\{', 'a \\cdot', 'f(t)', '\\big\\}'])), AnimationGroup(*(TransformFromCopy(lhs[tex][0], rhs[tex][-1], path_arc=-45 * DEG) for tex in ['+', '\\mathcal{L}', '\\big\\{', 'b \\cdot', 'g(t)', '\\big\\}'])), lag_ratio=0.75, run_time=3))
        self.wait()
...
class LaplaceTransformOfCosineSymbolically(InteractiveScene):
    ...
    def construct(self):
        frame = self.frame
        t2c = {'{t}': BLUE, '{s}': YELLOW, '\\omega': PINK, 'int^\\infty_0': WHITE}
        key_strings = ['int^\\infty_0', 'e^{\\minus{s}{t}} d{t}', '+', '\\frac{1}{2}', 'e^{i{t}}', 'e^{\\minus i{t}}']
        ...
        def lt_string(interior):
            return f'\\int^\\infty_0 ' + interior + 'e^{\\minus{s}{t}} d{t}'
        lt_def = Tex(lt_string('\\cos({t})'), **kw)
        lt_def.next_to(arrow, RIGHT)
        self.add(cos_t)
        self.play(LaggedStart(GrowArrow(arrow), Write(fancy_L), Write(lt_def['\\int^\\infty_0']), TransformFromCopy(cos_t, lt_def['\\cos({t})'][0], path_arc=45 * DEG), Write(lt_def['e^{\\minus{s}{t}} d{t}']), lag_ratio=0.2))
        spilt_cos_str = '\\left( \\frac{1}{2} e^{i{t}} + \\frac{1}{2} e^{\\minus i{t}} \\right)'
        split_inside = Tex('=' + lt_string(spilt_cos_str), **kw)
        split_inside.next_to(lt_def, RIGHT)
        cos_rect = SurroundingRectangle(lt_def['\\cos({t})'])
        cos_rect.set_stroke(TEAL, 2)
        self.play(ShowCreation(cos_rect))
        self.play(TransformMatchingTex(lt_def.copy(), split_inside, key_map={'\\cos({t})': spilt_cos_str}, path_arc=30 * DEG, mismatch_animation=FadeTransform), cos_rect.animate.surround(split_inside[spilt_cos_str]).set_anim_args(path_arc=30 * DEG), run_time=1.5)
        ...
        self.wait()
        self.play(FadeOut(cos_rect))
        split_tex = ' '.join(['\\frac{1}{2}', lt_string('e^{i{t}}'), '\\, + \\,', '\\frac{1}{2}', lt_string('e^{\\minus i{t}}')])
        split_outside = Tex(split_tex, **kw)
        side_eq = Tex('=', font_size=72).rotate(90 * DEG)
        ...
        for tex in key_strings:
            ...
            srcs.add(src)
            trgs.add(trg)
        self.play(Write(side_eq), LaggedStart((TransformFromCopy(*pair) for pair in zip(srcs[:3], trgs[:3])), lag_ratio=0.01, run_time=2))
        self.wait()
        self.play(TransformFromCopy(srcs[3][0], trgs[3][0]), TransformFromCopy(srcs[4][0], trgs[4][0]))
        self.wait()
        self.play(TransformFromCopy(srcs[3][1], trgs[3][1]), TransformFromCopy(srcs[5][0], trgs[5][0]))
        self.wait()
        exp_transform_parts = VGroup(split_outside[lt_string('e^{i{t}}')], split_outside[lt_string('e^{\\minus i{t}}')])
        pole_strings = ['\\frac{1}{{s} - i}', '\\frac{1}{{s} \\, + \\, i}']
        half_string = '\\frac{1}{2}'
        pole_sum = Tex(' \\, '.join([half_string, pole_strings[0], '+', half_string, pole_strings[1]]), **kw)
        pole_sum.scale(1.25)
        pole_sum.move_to(split_outside).shift(0.2 * LEFT)
        split_inside_rect = SurroundingRectangle(split_inside[spilt_cos_str])
        exp_transform_rects = VGroup((SurroundingRectangle(part, buff=SMALL_BUFF) for part in exp_transform_parts))
        pole_rects = VGroup((SurroundingRectangle(pole_sum[tex], buff=SMALL_BUFF) for tex in pole_strings))
        VGroup(split_inside_rect, exp_transform_rects, pole_rects).set_stroke(TEAL, 2)
        self.play(ShowCreation(split_inside_rect))
        self.wait()
        self.play(LaggedStart(*(TransformFromCopy(split_inside_rect, rect) for rect in exp_transform_rects)))
        self.play(FadeOut(split_inside_rect))
        self.wait()
        for i, tex in enumerate(['e^{i{t}}', 'e^{\\minus i{t}}']):
            self.play(ReplacementTransform(exp_transform_rects[i], pole_rects[i]), ReplacementTransform(split_outside[half_string][i], pole_sum[half_string][i]), FadeTransform(split_outside[lt_string(tex)], pole_sum[pole_strings[i]]), Transform(split_outside['+'][0], pole_sum['+'][0]))
        ...
        pole_words = VGroup((TexText(f'Pole at \\\\ $s = {value}$', font_size=60, t2c={'Pole at': YELLOW, 's': YELLOW}) for value in ['i', '-i']))
        last_group = VGroup()
        for word, rect in zip(pole_words, pole_rects):
            word.next_to(rect, DOWN, MED_LARGE_BUFF)
            self.play(FadeIn(word, lag_ratio=0.1), ShowCreation(rect), FadeOut(last_group))
            self.wait()
            last_group = VGroup(word, rect)
        self.play(FadeOut(last_group))
        old_group = VGroup(cos_t, lt_def, split_inside, pole_sum)
        new_group = VGroup(Tex('\\cos(\\omega{t})', **kw), Tex(lt_string('\\cos(\\omega{t})'), **kw), Tex('=' + lt_string('\\left(\\frac{1}{2} e^{i\\omega{t}} + \\frac{1}{2}e^{\\minus i \\omega {t}} \\right)'), **kw), Tex(' \\, '.join([half_string, '\\frac{1}{{s} - \\omega i}', '+', half_string, '\\frac{1}{{s} \\, + \\, \\omega i}']), **kw))
        ...
        self.play(ShowCreation(cos_omega_rect), TransformMatchingTex(cos_t, cos_omega), run_time=1)
        self.wait()
        self.play(LaggedStart((TransformMatchingTex(old, new) for new, old in zip(new_group[1:], old_group[1:])), lag_ratio=0.05, run_time=1), TransformFromCopy(omegas[0].replicate(len(omega_copies) - 1), omega_copies[1:], path_arc=30 * DEG, lag_ratio=0.1, run_time=2))
        self.remove(omega_copies)
        omegas.set_fill(opacity=1)
        self.add(new_group)
        self.play(FadeOut(cos_omega_rect))
        self.wait()
        lower_arrow = Tex('\\longleftarrow', font_size=60)
        lower_arrow.next_to(pole_sum, LEFT)
        transform_kw = dict(matched_keys=['{s}^2 \\,+\\, \\omega^2', '{s} \\,+\\, \\omega i', '{s} - \\omega i', '\\over'], key_map={'({s} - \\omega i)({s} + \\omega i)': '{s}^2 \\,+\\, \\omega^2'})
        steps = VGroup(Tex('\n                \\frac{1}{2}\\left(\n                {{s} \\,+\\, \\omega i \\over ({s} - \\omega i)({s} + \\omega i)} +\n                {{s} - \\omega i \\over ({s} - \\omega i)({s} + \\omega i)}\n                \\right)\n            ', **kw), Tex('\n                \\frac{1}{2}\\left(\n                {{s} \\,+\\, \\omega i \\over {s}^2 \\,+\\, \\omega^2} +\n                {{s} - \\omega i \\over {s}^2 \\,+\\, \\omega^2}\n                \\right)\n            ', **kw), Tex('\n                \\frac{1}{2} {{s} \\,+\\, \\omega i \\,+\\, {s} - \\omega i \\over {s}^2 \\,+\\, \\omega^2}\n            ', **kw), Tex('\n                \\frac{1}{2} {2{s} \\over {s}^2 \\,+\\, \\omega^2}\n            ', **kw), Tex('{{s} \\over {s}^2 \\,+\\, \\omega^2}', **kw))
        for step in steps:
            step.next_to(lower_arrow, LEFT)
        self.play(Write(lower_arrow), FadeTransform(pole_sum.copy(), steps[0]), frame.animate.set_height(8.5, about_edge=DR), run_time=2)
        for step1, step2 in zip(steps, steps[1:]):
            self.play(TransformMatchingTex(step1, step2, **transform_kw))
            self.wait()
        answer = steps[-1]
        ...
        self.play(ShowCreation(answer_rect), MoveToTarget(answer))
        self.wait()
        lt_def, int_of_expanded, imag_result = new_group[-3:]
        direct_equals = Tex('=', font_size=90)
        ...
        self.play(lower_arrow.animate.set_fill(opacity=0), to_fade.animate.set_fill(opacity=0.2), answer_rect.animate.surround(VGroup(lt_def, answer)), Write(direct_equals))
        self.wait()
        self.play(lower_arrow.animate.set_fill(opacity=1).rotate(PI).set_anim_args(path_arc=PI), answer_rect.animate.surround(VGroup(answer, imag_result)), imag_result.animate.set_fill(opacity=1))
        self.wait()
...
class SimplePolesOverImaginaryLine(InteractiveScene):
    ...
    def construct(self):
        ...
        omega = 2
        graph = get_complex_graph(s_plane, lambda s: s ** 2 / (s ** 2 + omega ** 2 + 1e-06), resolution=(301, 301))
...
class IntegrationByParts(InteractiveScene):
    ...
    def construct(self):
        t2c = {'{s}': YELLOW, '{t}': BLUE, '\\omega': PINK}
        steps = VGroup(Tex('X = \\int^\\infty_0 \\cos(\\omega{t}) e^{\\minus {s}{t}}d{t}', t2c=t2c), Tex('X = \\left[\\frac{1}{\\omega} \\sin(\\omega{t}) e^{\\minus {s}{t}} \\right]_0^\\infty - \\int^\\infty_0 \\frac{1}{\\omega} \\sin(\\omega {t}) \\left(\\minus {s} e^{\\minus{s}{t}} \\right) d{t}', t2c=t2c), Tex('X = \\frac{s}{\\omega} \\int^\\infty_0 \\sin(\\omega{t}) e^{\\minus{s}{t}} d{t}', t2c=t2c), Tex('X = \\frac{s}{\\omega} \\left(\\left[\\frac{\\minus 1}{\\omega} \\cos(\\omega{t}) e^{\\minus {s}{t}} \\right]_0^\\infty - \\int^\\infty_0 \\frac{\\minus 1}{\\omega} \\cos(\\omega{t}) \\left(\\minus {s} e^{\\minus{s}{t}} \\right) d{t} \\right)', t2c=t2c), Tex('X = \\frac{s}{\\omega} \\left(\\frac{1}{\\omega} - \\frac{s}{\\omega} \\int^\\infty_0 \\cos(\\omega{t}) e^{\\minus{s}{t}} d{t} \\right)', t2c=t2c), Tex('X = \\frac{s}{\\omega^2} \\left(1 - {s} X \\right)', t2c=t2c), Tex('X\\left(\\omega^2 + {s}^2 \\right) = {s}', t2c=t2c), Tex('X = \\frac{s}{\\omega^2 + {s}^2}', t2c=t2c))
        ...
        ibp = Tex('\\int u \\, dv = uv - \\int v \\, du', t2c={'u': RED, 'v': PINK})
        ibp.next_to(randy, UR, MED_LARGE_BUFF)
        self.add(randy, steps[0])
        self.play(Blink(randy))
        self.play(randy.change('raise_right_hand', ibp), FadeIn(ibp, UP))
        self.wait()
        self.play(self.frame.animate.set_height(steps.get_height() + 3, about_edge=LEFT), Restore(steps[0]), Write(steps[1:], run_time=2), randy.change('pondering', 5 * UL).shift(6 * RIGHT + 2 * DOWN), ibp.animate.shift(6 * RIGHT + 3 * DOWN))
...
class AlternateBreakDown(TranslateToNewLanguage):
    ...
    def construct(self):
        axes, graph, graph_label = self.get_graph_group(lambda t: 0.35 * t ** 2, func_tex='f({t}) = {t}^2')
        ...
        ne.set_color(RED)
        sum_tex = Tex('\\sum_{n=1}^N c_n e^{s_n t}', t2c={'s_n': YELLOW, 'c_n': GREY_A}, font_size=72)
        sum_tex.next_to(ne, RIGHT)
        ne_rhs = VGroup(ne, sum_tex)
        self.play(LaggedStart(FadeIn(ne, scale=2), Write(sum_tex), exp_pieces.animate.scale(0.7, about_edge=DR), lag_ratio=0.2))
        self.wait()
        self.play(LaggedStart(FadeOut(graph_label[4:]), graph_label[:4].animate.next_to(arrow, LEFT), GrowArrow(arrow), Write(fancy_L), TransformFromCopy(graph_label, Fs_label, path_arc=20 * DEG), lag_ratio=0.05), FadeOut(ne_rhs, DOWN, scale=0.5))
        self.wait()
        inv_lt = Tex('f({t}) = \\frac{1}{2\\pi i} \\int_\\gamma F({s}) e^{{s}{t}} d{s}', t2c=self.label_config['t2c'])
        ...
        line.insert_n_curves(20)
        self.play(TransformFromCopy(graph_label[:4], inv_lt[:4]), TransformFromCopy(Fs_label, inv_lt['F({s})'][0]), Write(inv_lt[4:]))
        self.play(FadeIn(s_plane), LaggedStartMap(FadeOut, exp_pieces, scale=0.1))
        self.play(VShowPassingFlash(line.copy()), ShowCreation(line), run_time=5)
        self.wait()