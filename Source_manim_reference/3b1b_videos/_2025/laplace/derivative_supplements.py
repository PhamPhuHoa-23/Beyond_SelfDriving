...
def get_lt_group(src, trg, arrow_length=1.5, arrow_thickness=4, buff=MED_SMALL_BUFF, label_font_size=48):
    ...
    label = Tex('\\mathcal{L}', font_size=label_font_size)
...
class AnnotateIntro(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(randy.change('maybe', look_at=left_section), GrowFromCenter(left_brace))
        self.play(Blink(randy))
        self.wait()
        self.play(randy.change('hesitant', look_at=left_section), FadeTransform(left_brace, length_question))
        self.play(Blink(randy))
        self.wait()
        ...
        self.play(Restore(right_brace, **kw), arrows.animate.rotate(90 * DEG).set_opacity(0).replace(right_brace.saved_state, stretch=True).set_anim_args(**kw), q_marks.animate.next_to(right_brace.saved_state, RIGHT).set_anim_args(**kw), randy.change('pondering', right_section))
...
class SimpleEToST(InteractiveScene):
    ...
    def construct(self):
        tex = Tex('e^{st}', t2c={'s': YELLOW, 't': GREY_A}, font_size=120)
        self.add(tex)
...
class TitleCard(InteractiveScene):
    ...
    def construct(self):
        ...
        tex = Tex('F({s}) = \\int^\\infty_0 f({t})e^{\\minus{s}{t}} d{t}', t2c=tex_to_color, font_size=60)
...
class KeyProperties(InteractiveScene):
    ...
    def construct(self):
        ...
        number_labels = VGroup((Tex(f'{n})', font_size=72) for n in range(1, 4)))
        ...
        properties = VGroup(get_lt_group(Tex('e^{a{t}}', t2c=t2c, font_size=60), Tex('{1 \\over {s} - a}', t2c=t2c)), get_lt_group(Tex('a \\cdot f({t}) + b \\cdot g({t})', t2c=t2c), Tex('a \\cdot F({s}) + b \\cdot G({s})', t2c=t2c)), get_lt_group(Tex("f'({t})", t2c=t2c), Tex('{s} F({s}) - f(0)', t2c=t2c)))
        ...
        self.play(LaggedStartMap(FadeIn, number_labels[:2], shift=UP, lag_ratio=0.25), ShowCreation(underline))
        self.wait()
        self.play(Write(exp_prop[0]))
        self.play(LaggedStart(GrowArrow(exp_prop[1]), FadeIn(exp_prop[2], 0.25 * RIGHT), Transform(exp_prop[0]['a'][0].copy(), exp_prop[3]['a'][0].copy(), remover=True), Write(exp_prop[3]), lag_ratio=0.5))
        self.wait()
        f_rects = VGroup(SurroundingRectangle(lin_prop[0]['f({t})'], buff=SMALL_BUFF), SurroundingRectangle(lin_prop[3]['F({s})'], buff=SMALL_BUFF))
        g_rects = VGroup(SurroundingRectangle(lin_prop[0]['g({t})'], buff=SMALL_BUFF), SurroundingRectangle(lin_prop[3]['G({s})'], buff=SMALL_BUFF))
        VGroup(f_rects, g_rects).set_stroke(TEAL, 2)
        self.play(Write(lin_prop[0]))
        self.play(LaggedStart(GrowArrow(lin_prop[1]), FadeIn(lin_prop[2], 0.25 * RIGHT), TransformMatchingTex(lin_prop[0].copy(), lin_prop[3], key_map={'{t}': '{s}', 'f': 'F', 'g': 'G'}, path_arc=45 * DEG, lag_ratio=0.01)))
        self.wait()
        ...
        self.play(VFadeIn(morty), morty.change('raise_left_hand', number_labels[2]), properties[:2].animate.set_fill(opacity=0.5), number_labels[:2].animate.set_fill(opacity=0.5), Write(number_labels[2]), frame.animate.set_height(12, about_edge=UP))
        self.wait()
        self.play(LaggedStart(Write(deriv_prop[0]), GrowArrow(deriv_prop[1]), FadeIn(deriv_prop[2], 0.25 * RIGHT), morty.change('pondering', deriv_prop[0])))
        self.play(Blink(morty))
        self.wait()
        morty.body.insert_n_curves(500)
        self.play(Write(deriv_prop[3]), morty.change('raise_right_hand', deriv_prop[3]))
        self.play(Blink(morty))
        self.wait()
...
class SimpleLTArrow(InteractiveScene):
    ...
    def insertion(self):
        ...
        self.play(GrowArrow(group[1]), FadeIn(group[2], RIGHT))
        self.wait()
...
class CombinationOfExponentials(InteractiveScene):
    ...
    def construct(self):
        ...
        combination = Tex('\\sum_n c_n e^{a_n{t}}', **kw)
        result = Tex('\\sum_n {c_n \\over {s} - a_n}', **kw)
        group = get_lt_group(combination, result, arrow_length=4, label_font_size=60, arrow_thickness=6)
        group.center()
        self.play(FadeIn(combination, lag_ratio=0.1))
        self.wait()
        self.play(LaggedStart(GrowArrow(group[1]), FadeIn(group[2], RIGHT), *(Transform(combination[tex][0].copy(), result[tex][0].copy(), remover=True) for tex in ['c_n', 'a_n', '\\sum_n']), Write(result['\\over {s} - '][0]), lag_ratio=0.2, run_time=2))
        self.add(group)
        self.wait()
...
class StepByStep(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(morty.change('raise_right_hand'), self.change_students('confused', 'maybe', 'erm', look_at=self.screen))
        self.wait()
        self.play(morty.change('tease'), self.change_students('pondering', 'thinking', 'pondering', look_at=morty.eyes))
        self.wait()
...
class PolesAtOmegaI(InteractiveScene):
    ...
    def construct(self):
        t2c = {'{s}': YELLOW, '\\omega': PINK}
        pole_words = VGroup(Tex('\\text{Pole at } {s} = +\\omega i', t2c=t2c), Tex('\\text{Pole at } {s} = -\\omega i', t2c=t2c))
...
class WhatIsTheAnswer(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(stds[1].says('Okay, okay, okay', mode='sassy'), stds[0].change('erm', stds[1].eyes), stds[2].change('hesitant', stds[1].eyes), morty.change('tease'))
        self.wait(2)
        self.play(FadeOut(stds[1].bubble), stds[1].says('But...what is\nthe actual answer?', mode='maybe'), morty.change('guilty'))
        self.wait()
        self.play(self.change_students('sassy', 'erm', 'confused', look_at=self.screen))
        self.wait(3)
        self.wait(2)
        self.play(FadeOut(stds[1].bubble), morty.change('raise_right_hand'), self.change_students('pondering', 'pondering', 'pondering', look_at=4 * UR))
        self.wait(4)
...
class InvertArrow(InteractiveScene):
    ...
    def construct(self):
        ...
        label = Tex('\\mathcal{L}', font_size=48)
        label.next_to(arrow, UP, SMALL_BUFF)
        inv_label = Tex('\\mathcal{L}^{-1}')
        inv_label.next_to(arrow, DOWN, buff=0)
        self.play(GrowArrow(arrow), FadeIn(label, 0.5 * RIGHT))
        self.wait()
        self.play(Rotate(arrow, PI), ReplacementTransform(label, inv_label))
        self.wait()
...
class ReferenceHomework(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        self.play(morty.change('raise_right_hand'), self.change_students('tease', 'pondering', 'thinking', look_at=self.screen))
        self.wait(3)
...
class ODEToAlgebra(InteractiveScene):
    ...
    def construct(self):
        ...
        ode_group = get_lt_group(Text('Differential\nEquation'), Text('Algebra'), **arrow_config)
        ode_group.center()
        deriv_group = get_lt_group(Tex('d / dt', t2c={'t': BLUE}, font_size=72), Tex('\\times s', t2c={'s': YELLOW}, font_size=72), **arrow_config)
    ...
    def animate_group(self, group):
        ...
        self.play(LaggedStart(GrowArrow(arrow), FadeIn(label, 0.5 * arrow.get_vector()), FadeIn(trg, arrow.get_vector())))

class ThreeExplanations(InteractiveScene):

    def construct(self):
        t2c = {'{t}': BLUE, '{s}': YELLOW}
        title = Tex("\\text{Why } \\mathcal{L}\\big\\{f'({t})\\big\\} = {s} F({s}) - f(0)", t2c=t2c)
        title.to_edge(UP)
        ...
        by_parts = Tex('\\text{Integration by parts}', t2c=t2c)
        inversion = Tex('\\frac{d}{d{t}} \\Big(\\text{Inverse Laplace Transform}\\Big)', t2c=t2c)
        descriptors = VGroup(Text('[Elementary, but limited]'), Text('[General, but opaque]'), Text('[My favorite, but theoretical]'))
        descriptors.set_fill(GREY_B)
        explanations = VGroup(Tex('\\text{Start with } e^{a{t}}', t2c=t2c), by_parts, inversion)
        ...
        for i, desc, expl in zip(it.count(), descriptors, explanations):
            self.play(FadeOut(desc, 0.5 * UP), FadeIn(expl, 0.5 * UP), num_mobs[:i].animate.set_opacity(0.25), explanations[:i].animate.set_opacity(0.25))
            self.wait()
        num_mobs.set_fill(opacity=1)
        explanations.set_fill(opacity=1)
        self.play(LaggedStartMap(FadeIn, num_mobs, shift=UP, lag_ratio=0.25), LaggedStartMap(FadeIn, explanations, shift=UP, lag_ratio=0.25))
        self.wait()
...
class ComplainAboutSpecificity(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(stds[1].says('But, that’s just\none example!', mode='angry'), stds[0].change('pondering', self.screen), stds[2].change('erm', self.screen), morty.change('guilty'))
        self.wait(3)
        self.play(stds[1].debubble('pondering'), morty.says('One with the\nseeds of generality'), stds[2].change('thinking'))
        self.wait(3)
...
class ExplanationOneTitle(InteractiveScene):
    ...
    def construct(self):
        t2c = dict(tex_to_color)
        title = VGroup(Text('Explanation #1: Try it for'), Tex('e^{a{t}}', t2c=t2c))
        title.arrange(RIGHT)
        title[1].shift(SMALL_BUFF * UP)
        title.to_corner(UL)
        alt_formula = Tex('c_1 e^{a_1 {t}} + c_2 e^{a_2 {t}} + \\cdots + c_n e^{a_n {t}}', t2c=t2c)
        alt_formula[re.compile('c_.')].set_color(TEAL_D)
        ...
        self.wait()
        self.play(TransformMatchingTex(title[1], alt_formula), FadeOut(ponder_words, lag_ratio=0.1))
        self.wait()
...
class IntuitionEvaporating(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait()
        y0 = words.get_y()
        self.play(self.change_students('tired', 'tired', 'tired', look_at=self.screen), morty.change('guilty'), LaggedStart(*(UpdateFromAlphaFunc(word, lambda m, a: m.set_y(y0 + 2 * a).set_fill(opacity=there_and_back(a))) for word in words), lag_ratio=0.1, run_time=6))
...
class ContourIntegralReference(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(inv_rect)
        self.play(Write(contour_words), ShowCreation(integral_rect), VShowPassingFlash(integral_rect.copy().insert_n_curves(100).set_stroke(YELLOW, 3)), run_time=2)
        self.wait()
