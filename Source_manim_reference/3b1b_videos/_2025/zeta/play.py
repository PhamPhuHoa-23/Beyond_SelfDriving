...
def get_first_n_primes(n=100000):
    pass
...
class ZetaSum(InteractiveScene):
    ...
    max_N = 100000
    show_c = True
    sum_tex = '\\sum_{n=1}^\\infty \\frac{1}{n^{s}}'
    exp_tex = '\\cdot e^{\\minus {c} n}'
    ...
    def add_planes(self):
        planes = VGroup(ComplexPlane((-1, 3), (-40, 40)), ComplexPlane((-3, 3), (-3, 3)))
    ...
    def add_sum_label(self):
        ...
        sum_label = Tex(self.sum_tex + (self.exp_tex if self.show_c else ''), t2c={'{s}': YELLOW, '{c}': RED}, font_size=42)
        sum_label.next_to(out_plane, UP, aligned_edge=LEFT)
        self.add(sum_label)
        if self.show_c:
            exp_const_label = Tex('c = 0.0000', t2c={'c': RED})
    ...
    def get_vect_sum(self, path, vect_colors=[TEAL, GREEN], thickness=3):
        vects = VGroup((Vector(RIGHT, thickness=thickness, fill_color=color) for n, color in zip(range(self.n_vects), it.cycle(vect_colors))))
...
class ZetaLogDerivSum(ZetaSum):
    sum_tex = "-\\frac{\\zeta'(s)}{\\zeta(s)} = \\sum_{\\substack{p \\, \\text{prime} \\\\ k \\in \\mathds{N}}} \\log(p) \\left(\\frac{1}{p^{k}} \\right)^s"
    exp_tex = '\\cdot e^{\\minus {c} p^k}'
    show_c = False
    max_N = 1000000
