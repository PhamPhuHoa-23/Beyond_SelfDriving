...
class BeamSplitter(InteractiveScene):
    ...
    def construct(self):
        ...
        def set_theta(target_angle, run_time=2):
            ...
            return AnimationGroup(theta_tracker.animate.set_value(target_angle), Rotate(pointer, curr_angle - target_angle, axis=RIGHT), run_time=run_time)
        frame.reorient(-121, 76, 0, (-3.29, -0.25, -0.34), 4.8)
        self.add(pointer)
        ...
        self.play(frame.animate.reorient(-57, 74, 0, (-3.29, -0.18, -0.18), 4.8), run_time=8)
        ...
        self.play(FadeIn(plane_in_situ), TransformFromCopy(plane_in_situ, corner_plane, run_time=2))
        self.play(VFadeIn(corner_vector))
        self.play(frame.animate.reorient(-80, 79, 0, (-3.36, 0.1, -0.46), 4.8), run_time=12)
        split_point_dist = 6
        ...
        self.play(FadeIn(splitter), FadeOut(wave), FadeIn(short_wave), FadeIn(top_wave), FadeIn(low_wave), FadeOut(beam), FadeIn(short_beam), FadeIn(top_beam), FadeIn(low_beam), plane_in_situ.animate.fade(0.5), frame.animate.reorient(-76, 62, 0, (-2.7, 0.08, -0.8), 6.22).set_anim_args(run_time=4))
        self.wait(3)
        ...
        rot_arrows = VGroup(Arrow(RIGHT, LEFT, path_arc=PI), Arrow(LEFT, RIGHT, path_arc=PI))
        ...
        self.play(frame.animate.reorient(-90, 82, 0, (-1.73, 0.07, 1.0), 8.0), run_time=6)
        self.play(set_theta(60 * DEG, run_time=4))
        self.wait(4)
        eq, plus = signs = VGroup(Tex('='), Tex('+'))
        signs.scale(1.5)
        signs.fix_in_frame()
        for sign, plane1, plane2 in zip(signs, fixed_planes, fixed_planes[1:]):
            sign.move_to(midpoint(plane1.get_right(), plane2.get_left()))
        coords = VGroup(DecimalNumber(0, unit='\\times'), DecimalNumber(0, unit='\\times'))
        coords.fix_in_frame()
        coords.scale(0.75)
        ...
        self.play(VFadeIn(h_plane), VFadeIn(v_plane), VFadeIn(soft_h_vect), VFadeIn(soft_v_vect))
        self.wait()
        self.play(Restore(h_plane, path_arc=30 * DEG), VFadeIn(h_vect), Write(eq))
        self.play(Restore(v_plane, path_arc=30 * DEG), VFadeIn(v_vect), Write(plus))
        self.wait(5)
        curr_angle = 60 * DEG
        for target_angle in [90 * DEG, 0, 60 * DEG]:
            self.play(set_theta(target_angle))
            self.wait(3)
        arc = always_redraw(lambda: Arc(0, theta_tracker.get_value(), radius=0.5, arc_center=corner_plane.get_center()).fix_in_frame())
        ...
        for mob in movers:
            mob.generate_target()
        cos_term = Tex('\\cos(\\theta) \\, \\cdot ').fix_in_frame()
        sin_term = Tex('\\sin(\\theta) \\, \\cdot ').fix_in_frame()
        ...
        self.play(VFadeIn(arc), Write(theta_label))
        h_vect.force_unit = True
        v_vect.force_unit = True
        self.play(LaggedStartMap(MoveToTarget, movers), LaggedStartMap(FadeIn, VGroup(cos_term, sin_term)), FadeTransform(theta_label.copy().clear_updaters(), cos_term['\\theta'][0], time_span=(0.25, 1.25)), FadeTransform(theta_label.copy().clear_updaters(), sin_term['\\theta'][0], time_span=(0.5, 1.5)), run_time=1.5)
        self.wait(4)
        sin_part = VGroup(sin_term, v_plane)
        cos_part = VGroup(cos_term, h_plane)
        self.play(sin_part.animate.scale(0.5).rotate(5 * DEG).rotate(45 * DEGREES, UP).shift(3 * DOWN), rate_func=there_and_back_with_pause, run_time=4)
        self.play(cos_part.animate.scale(0.5).rotate(-5 * DEG).rotate(45 * DEGREES, DOWN).shift(3 * DOWN + 2 * LEFT), rate_func=there_and_back_with_pause, run_time=4)
        self.wait(3)
        for target_angle in [80 * DEG, 10 * DEG, 60 * DEG]:
            self.play(set_theta(target_angle))
            self.wait()
        e_expr = Tex('E = k \\cdot (\\text{Amplitude})^2', font_size=36)
        e_expr.fix_in_frame()
        ...
        self.play(Write(e_expr), frame.animate.reorient(-77, 80, 0, (-1.83, 2.76, 0.89), 8.0))
        self.wait(5)
        self.play(GrowFromCenter(amp_brace), Write(one_label), VFadeOut(soft_h_vect), VFadeOut(soft_v_vect))
        self.wait()
        eq_60 = Tex('= 60^\\circ')
    ...
    def get_laser_pointer(self):
        ...
        cone = ParametricSurface(lambda u, v: np.array([u, u * math.cos(-TAU * v), u * math.sin(TAU * v)]))
        cone.stretch(5, 0)
        cone.set_width(0.25)
    ...
    def get_beam(self, start, end, color=GREEN_SCREEN, stroke_width=2, opacity=1.0, anti_alias_width=25):
        beam = Line(start, end)
        beam.set_stroke(color, stroke_width, opacity)
        beam.set_anti_alias_width(anti_alias_width)
        return beam

    def get_wave(self, theta_tracker, start_point=ORIGIN, max_x=20, refraction_angle=0 * DEG, wave_number=4.0, freq=0.5, amplitude=0.25, color=BLUE, stroke_opacity=0.5, vector_density=0.1, max_vect_len=1.0, project_vertical=False, project_horizontal=False):
        axes = ThreeDAxes()
        ...
        wave = TimeVaryingVectorField(field_func, axes, sample_coords=sample_coords, max_vect_len=max_vect_len, color=color, stroke_opacity=stroke_opacity)
