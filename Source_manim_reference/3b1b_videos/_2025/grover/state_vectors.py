...
class RandomSampling(Animation):
    ...
    def __init__(self, mobject: Mobject, samples: list, weights: list[float] | None=None, **kwargs):
...
class ContrstClassicalAndQuantum(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeOut(v_line, shift=2 * RIGHT), FadeOut(quantum, shift=RIGHT), ReplacementTransform(short_boxed_bits, boxed_bits), frame.animate.match_x(classical))
        self.wait()
        ...
        layer_names = VGroup(Text('Hardware'), Text('Bits'), Text('Data types'))
        ...
        self.play(LaggedStartMap(FadeIn, layers, lag_ratio=0.25, run_time=1), FadeIn(layers_name, lag_ratio=0.01), Write(layer_names[1]))
        self.play(LaggedStart((TransformFromCopy(bit, num_mob) for bit in bits), lag_ratio=0.02), FadeIn(layer_names[2], UP))
        self.wait()
        ...
        self.play(LaggedStart(frame.animate.set_x(0), MoveToTarget(layer_names), FadeIn(quantum, RIGHT), ShowCreation(v_line), Transform(layers_name, new_layers_name), Group(num_mob, character, boxed_bits).animate.shift(RIGHT), circuitry.animate.scale(0.8).shift(RIGHT)), MoveToTarget(layers, lag_ratio=0.01))
        self.wait()
        ...
        ion = Group(GlowDot(color=RED, radius=0.5), Dot(radius=0.1).set_fill(RED, 0.5), Tex('+', font_size=14).set_fill(border_width=1))
        ...
        for value in [ord('C'), ord('Q')]:
            self.play(value_tracker.animate.set_value(value), UpdateFromFunc(qunit_num, lambda m: m.set_value(int(value_tracker.get_value()))), UpdateFromFunc(qubit_string, lambda m: m.set_value(int(value_tracker.get_value()))), rate_func=linear, run_time=1.0)
            self.wait(0.25)
        lasers = VGroup()
        ...
        for value in [*np.random.randint(0, 2 ** 8, 4), ord('Q')]:
            ...
            self.play(LaggedStartMap(VShowPassingFlash, lasers, lag_ratio=0.1, time_width=2.0, run_time=2), MoveToTarget(trapped_ions, lag_ratio=0.1, time_span=(0.5, 2.0)), MoveToTarget(qubit_string, lag_ratio=0.1, time_span=(0.5, 2.0)), MoveToTarget(qunit_num, time_span=(1.0, 1.25)), Transform(qunit_ket, Ket(qunit_num.target), time_span=(1.0, 1.5)))
        ...
        self.play(frame.animate.reorient(0, 0, 0, (4.66, -2.55, 0.0), 13.19), morty.change('raise_right_hand', big_ket), VFadeIn(morty), *(TransformFromCopy(src, big_ket) for src in [qubit_ket, qunit_ket]))
        self.play(Write(big_ket_name, time_span=(0.75, 2.0)), FlashAround(big_ket, time_width=1.5, run_time=2))
        self.wait()
        self.remove(big_ket)
        self.play(LaggedStart(FadeOut(VGroup(morty, big_ket, big_ket_name)), TransformFromCopy(big_ket, qubit_ket), TransformFromCopy(big_ket, qunit_ket), frame.animate.to_default_state()))
        mid_layer = layers[1]
        ...
        target_y = -1.0
        self.play(FadeOut(VGroup(layers[2], num_mob, character, qunit, layer_names[2]), UP), FadeOut(Group(layers[0], circuitry, trapped_ions, layer_names[0]), DOWN), FadeOut(layers_name, DOWN), qubits.animate.set_y(target_y), boxed_bits.animate.match_x(classical).set_y(target_y), layer_names[1].animate.set_y(target_y), MoveToTarget(mid_layer, time_span=(0.5, 2.0)), run_time=2)
        self.play(FadeOut(layers[1]), FadeOut(layer_names[1]), run_time=3)
        contrast = VGroup(Text('State'), Tex('=', font_size=72), Text('What you see'))
        ...
        self.play(FadeIn(contrast[::2]), MoveToTarget(boxed_bits))
        self.play(Write(contrast[1]), TransformFromCopy(boxed_bits, boxed_bits_copy, path_arc=30 * DEG))
        self.wait()
        self.play(*(LaggedStart((bit.animate.set_stroke(YELLOW, 3).set_anim_args(rate_func=there_and_back) for bit in group[1]), lag_ratio=0.25, run_time=4) for group in [boxed_bits, boxed_bits_copy]))
        self.wait()
        ...
        self.play(LaggedStart(TransformFromCopy(contrast, q_contrast, path_arc=-45 * DEG), MoveToTarget(qubits), GrowArrow(state_vector)))
        self.wait()
        self.play(moving_rect.animate.surround(qubits).set_stroke(YELLOW, 3, 1))
        self.play(FadeOut(moving_rect))
        self.play(value_tracker.animate.set_value(0).set_anim_args(rate_func=there_and_back, run_time=4), UpdateFromFunc(qubit_string, lambda m: m.set_value(int(value_tracker.get_value()))))
        self.wait()
        qubit_samples = list()
        for n in range(2 ** 8):
            sample = qubits.copy()
        ...
        self.play(FadeIn(labels), RandomSampling(qubits, qubit_samples))
        self.wait()
        for _ in range(8):
...
class AmbientStateVector(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(sphere), Write(sphere_mesh, lag_ratio=0.001))
        self.wait(10)

    def get_plane_and_axes(self, scale_factor=2.0):
        frame = self.frame
        axes = ThreeDAxes((-1, 1), (-1, 1), (-1, 1))
        plane = NumberPlane((-1, 1 - 1e-05), (-1, 1 - 1e-05), faded_line_ratio=5)
...
class FlipsToCertainDirection(AmbientStateVector):
    ...
    def construct(self):
        ...
        for n in range(1, 14):
            ...
            self.play(MoveToTarget(ghosts), Rotate(vector, n * theta, axis=axis, about_point=ORIGIN))
...
class DisectAQuantumComputer(InteractiveScene):
    ...
    def construct(self):
        ...
        wires.shift(LEFT)
        gates = VGroup(self.get_labeled_box(wires[0], 0.1), self.get_cnot(wires[1], wires[0], 0.2), self.get_cnot(wires[3], wires[1], 0.3), self.get_cnot(wires[2], wires[3], 0.4), self.get_labeled_box(wires[3], 0.5, 'X'), self.get_labeled_box(wires[1], 0.5, 'Z'), self.get_cnot(wires[2], wires[1], 0.6), self.get_labeled_box(wires[1], 0.7), self.get_cnot(wires[3], wires[1], 0.8), self.get_cnot(wires[0], wires[1], 0.9))
        mes_gates = VGroup(self.get_measurement(wires[0], 1), self.get_measurement(wires[1], 1), self.get_measurement(wires[2], 1), self.get_measurement(wires[3], 1))
        ...
        all_qubits.set_y(0)
        all_qubit_rects = VGroup((SurroundingRectangle(qubits, buff=0.05).set_stroke(YELLOW, 1, 0.5) for qubits in all_qubits))
        ...
        self.play(machine.animate.to_edge(LEFT), TransformFromCopy(VGroup(output), all_qubits), TransformFromCopy(VGroup(output_rect), all_qubit_rects))
        ...
        dists = [np.random.randint(1, 8, 16).astype(float), np.random.random(16) + 10 * np.eye(16)[10], np.random.random(16) + 5, np.random.randint(1, 8, 16).astype(float)]
        for dist in dists:
            dist /= dist.sum()
        all_dist_rects = VGroup((self.get_dist_bars(dist, all_qubits, width_ratio=6) for dist in dists))
        dist_rects = all_dist_rects[0]
        dist_rect_labels = VGroup((Integer(100 * prob, unit='\\%', font_size=24, num_decimal_places=1).next_to(rect, RIGHT, SMALL_BUFF) for prob, rect in zip(dists[0], dist_rects)))
        self.play(FadeOut(machine_rect, DOWN), FadeOut(circuit, DOWN), MoveToTarget(qc_label))
        self.play(GrowArrow(comp_to_dist_arrow), ReplacementTransform(all_qubit_rects, dist_rects, lag_ratio=0.1), FadeOut(brace, RIGHT), FadeOut(brace_label, RIGHT))
        self.wait()
        ...
        if False:
            ...
            wigglers = Superposition(VGroup((VGroup(qb, bar) for qb, bar in zip(all_qubits, dist_rects))).copy(), max_rot_vel=8, glow_stroke_opacity=0)
            index = 7
            ...
            for _ in range(2):
                self.play(glass.animate.shift(qc_label.get_center() - glass[0].get_center()).set_anim_args(path_arc=-45 * DEG), Transform(dist_rects, new_bars, time_span=(1.25, 1.5)), FadeIn(choice_rect, time_span=(1.25, 1.5)), run_time=2)
                self.wait()
                self.play(Restore(glass, path_arc=45 * DEG), FadeOut(choice_rect, time_span=(0.25, 0.5)), run_time=1.5)
                self.wait()
        ...
        self.play(ShowCreation(underline, time_span=(1, 2)), Write(title, run_time=2))
        self.wait()
        last_sym = title['4'][0]
        ...
        for k in [5, 6, 'k']:
            ...
            self.play(FadeOut(last_sym, 0.25 * UP), FadeIn(k_sym, 0.25 * UP), FadeOut(last_qubits), FadeOut(last_dist_rects), FadeIn(new_qubits), FadeIn(big_dist_rects))
            self.wait()
            last_sym = k_sym
            last_qubits = new_qubits
            last_dist_rects = big_dist_rects
        brace = Brace(last_dist_rects, RIGHT, SMALL_BUFF)
        brace_label = Tex('2^k', t2c={'k': YELLOW})
        ...
        self.play(GrowFromCenter(brace), TransformFromCopy(last_sym, brace_label))
        self.play(TransformFromCopy(last_qubits[70], longer_example))
        self.play(GrowFromCenter(down_brace), TransformFromCopy(brace_label[1], down_brace_label))
        self.wait()
        ...
        self.play(FadeOut(title), FadeOut(word_rect), FadeOut(last_sym), LaggedStartMap(FadeOut, VGroup(brace, brace_label, longer_example, down_brace, down_brace_label)), FadeOut(last_qubits), FadeOut(last_dist_rects), FadeIn(all_qubits), FadeIn(dist_rects))
        self.wait()
        frame = self.frame
        ...
        self.play(LaggedStart(frame.animate.reorient(0, 0, 0, (4.2, 0.7, 0.0), 9.86), FadeIn(dist_rect), FadeIn(dist_words, lag_ratio=0.1), FadeIn(output), FadeIn(output_words, lag_ratio=0.1), FadeIn(sample_rects[sample]), lag_ratio=0.2))
        ...
        self.wait(6)
        self.play(FadeOut(superposition), FadeOut(big_cross))
        n_samples = 8
        ...
        self.play(FadeOut(selection), Transform(dist_rects, new_rects), dist_rect.animate.stretch(1.2, 0, about_edge=LEFT), UpdateFromFunc(dist_words, lambda m: m.match_x(dist_rect)), output_words.animate.shift(RIGHT))
        self.wait()
        ...
        self.play(dist_rect.animate.surround(dist_group), FadeTransformPieces(dist_words, q_marks))
        self.wait()
        ...
        self.play(LaggedStart(ShowCreation(vect_lines, lag_ratio=0), FadeInFromPoint(state_vector, qc_label.get_right()), MoveToTarget(comp_to_dist_arrow), dist_group.animate.next_to(comp_to_dist_arrow.target, RIGHT), MaintainPositionRelativeTo(sample_rects[num], dist_group), MaintainPositionRelativeTo(q_marks, dist_group), VGroup(output, output_words).animate.shift(0.5 * RIGHT)))
        ...
        for bits, entry in zip(indices, state_vector.get_entries()):
            ...
            bits.match_y(entry)
        self.play(VGroup(qc_label, vect_lines).animate.shift(0.75 * LEFT), LaggedStart((TransformFromCopy(pre_bits, bits, path_arc=30 * DEG) for pre_bits, bits in zip(pre_indices, indices)), lag_ratio=0.25, run_time=6))
        self.wait()
        ne_sign = Tex('\\ne', font_size=120)
        ne_sign.move_to(comp_to_dist_arrow)
        ne_sign.set_color(RED)
        rule = Tex('x \\rightarrow |x|^2', font_size=42)
        rule.next_to(comp_to_dist_arrow, UP, buff=SMALL_BUFF)
        template_eq = Tex('(+0.50)^2 = 0.25', font_size=20)
        ...
        self.play(FadeOut(comp_to_dist_arrow), FadeIn(ne_sign))
        self.wait()
        self.play(FadeIn(comp_to_dist_arrow), FadeOut(ne_sign))
        self.play(ReplacementTransform(q_marks, rule, path_arc=90 * DEG, run_time=1.5))
        self.wait()
        self.play(dist_rect.animate.stretch(1.5, 0, about_edge=LEFT).set_anim_args(time_span=(1, 2)), FadeOut(sample_rects[num]), LaggedStart((TransformFromCopy(entry, value[1], path_arc=-30 * DEG) for entry, value in zip(vector_entries, bar_values)), lag_ratio=0.5), LaggedStart((FadeIn(value[0]) for value in bar_values), lag_ratio=0.5, time_span=(2, 6)), LaggedStart((FadeIn(value[2:]) for value in bar_values), lag_ratio=0.5, time_span=(2, 6)), frame.animate.reorient(0, 0, 0, (2.15, 0.04, 0.0), 7.64), run_time=6)
        ...
        comp_lines = VGroup((Line(label_line.pfp(alpha), entry.get_center()).insert_n_curves(20).set_stroke(color=random_bright_color(hue_range=(0.3, 0.5)), width=(0, 3, 3, 3, 0)) for alpha, entry in zip(np.linspace(0, 1, len(entries)), entries)))
        comp_lines.shuffle()
        self.play(FlashAround(rule, time_width=1.5, run_time=2))
        self.play(FadeOut(bar_values))
        for new_state in [*alt_states, state]:
            comp_lines.shuffle()
            self.play(LaggedStartMap(VShowPassingFlash, comp_lines, time_width=2.0, lag_ratio=0.02), Transform(dist_rects, self.get_dist_bars(new_state ** 2, all_qubits), time_span=(0.75, 1.75)), *(ChangeDecimalToValue(entry, value, time_span=(0.75, 1.75)) for entry, value in zip(entries, new_state)), run_time=2)
        ...
        for index in [point5_index, point1_index]:
            self.play(frame.animate.reorient(0, 0, 0, (3.36, 0.87, 0.0), 5.6), dist_rect.animate.set_stroke(opacity=0), highlight_rect.animate.set_stroke(opacity=1).match_y(all_qubits[index]), *(group[slc].animate.set_opacity(0.25) for group in groups for slc in [slice(0, index), slice(index + 1, None)]), *(group[index].animate.set_opacity(1) for group in groups), run_time=2)
            self.play(FlashAround(indices[index], buff=SMALL_BUFF, color=RED, time_width=1.5), WiggleOutThenIn(indices[index]), run_time=2)
            self.wait()
            ...
            self.wait()
        self.play(frame.animate.reorient(0, 0, 0, (2.13, 1.33, 0.0), 10.48), FadeOut(highlight_rect, time_span=(0, 1)), FadeIn(vector_title), *(group.animate.set_opacity(1) for group in groups), FadeOut(bar_values, lag_ratio=0.01), run_time=4)
        if False:
            og_state = state
            frame.reorient(0, 0, 0, (0.94, 0.34, 0.0), 9.2)
            ...
            for alpha in [0.1, 0.1, 0.2, 0.25, 0.25, 0.25, 0.25, 1.0]:
                ...
                self.play(Transform(dist_rects, self.get_dist_bars(new_state ** 2, all_qubits, width_ratio=4), time_span=(0.5, 1.5)), LaggedStartMap(VShowPassingFlash, comp_lines, time_width=2.0, lag_ratio=0.01, run_time=2.0), *(ChangeDecimalToValue(entry, value, time_span=(0.5, 1.5)) for entry, value in zip(state_vector.elements, new_state)))
                state = new_state
            rect = SurroundingRectangle(all_qubits[key])
            self.play(ShowCreation(rect, run_time=2))
        sum_expr = Tex('x_{0}^2 + x_{1}^2 + x_{2}^2 + x_{3}^2 + \\cdots + x_{14}^2 + x_{15}^2 = 1', font_size=60)
        sum_expr.next_to(vector_title, UP, LARGE_BUFF)
        sum_expr.match_x(frame)
        self.play(LaggedStart((FadeTransform(vector_entries[n].copy(), sum_expr[f'x_{{{n}}}^2']) for n in [0, 1, 2, 3, 14, 15]), lag_ratio=0.05, run_time=2), FadeTransformPieces(vector_entries[4:14].copy(), sum_expr['\\cdots'][0], time_span=(0.5, 2.0)), Write(sum_expr['+'], time_span=(0.5, 1.5)), Write(sum_expr['= 1'], time_span=(1, 2)))
        self.wait()
        self.play(FadeOut(sum_expr))
        top_eq = Tex('(+0.50)^2 = 0.25', font_size=60)
        ...
        lines = VGroup(Line(entry_rect.get_corner(UL), value_rect.get_corner(DL)), Line(entry_rect.get_corner(UR), value_rect.get_corner(DR)))
        lines.set_stroke(YELLOW, 1)
        self.play(ShowCreation(entry_rect))
        self.play(TransformFromCopy(entry_rect, value_rect), TransformFromCopy(entry, top_value), ShowCreation(lines, lag_ratio=0), FadeOut(vector_title))
        self.wait()
        self.play(entry.animate.set_value(-0.5), top_value.animate.set_value(-0.5))
        self.wait()
        self.play(Write(top_eq[0]), Write(top_eq[2:]), FadeOut(value_rect))
        self.wait()
        self.play(entry.animate.set_value(0.5), top_value.animate.set_value(0.5))
        self.wait()
        self.play(Uncreate(lines, lag_ratio=0), ReplacementTransform(top_value, entry), FadeOut(top_eq[0]), FadeOut(top_eq[2:]), FadeOut(entry_rect), FadeIn(vector_title), frame.animate.reorient(0, 0, 0, (0.45, 0.46, 0.0), 8.7))
        ...
        for n in range(3):
            self.play(LaggedStart(*(Transform(sign, random.choice(choice), path_arc=PI) for sign, choice in zip(signs, sign_choices)), lag_ratio=0.15, run_time=2))
        self.play(Restore(vector_entries))
        ...
        self.play(LaggedStart(FadeOut(indices, scale=0.25), TransformFromCopy(vect_lines, small_vect_lines), TransformFromCopy(state_vector.get_brackets(), small_state_vector.get_brackets()), TransformFromCopy(state_vector.get_entries(), small_state_vector.get_entries()), vector_title.animate.next_to(small_state_vector, UP, MED_LARGE_BUFF), FadeTransformPieces(all_qubits.copy(), qubits), TransformFromCopy(dist_rects, small_dist_bars), lag_ratio=0.1, run_time=3))
        self.add(small_state_vector)
        self.wait()
        self.play(FadeOut(VGroup(small_vect_lines, small_state_vector, qubits, small_dist_bars)), FadeIn(VGroup(vect_lines, indices, state_vector, all_qubits, dist_rects)), vector_title.animate.next_to(state_vector, UP, MED_LARGE_BUFF))
        self.play(frame.animate.reorient(0, 0, 0, (2, 0.45, 0.0), 9), qc_label[1].animate.set_color(BLUE))
        key = 12
        ...
        for n, state in enumerate(new_states):
            anims = [Transform(dist_rects, self.get_dist_bars(state ** 2, all_qubits, width_ratio=4.0), time_span=(1, 2))]
            if n == 1:
                entry_rects = VGroup((SurroundingRectangle(e, buff=0.05) for e in vector_entries))
                qubit_rects = VGroup((SurroundingRectangle(VGroup(*pair), buff=0.05) for pair in zip(all_qubits, dist_rects)))
                VGroup(entry_rects, qubit_rects).set_stroke(YELLOW, 2)
                self.play(LaggedStartMap(VFadeInThenOut, entry_rects, lag_ratio=0.1), LaggedStartMap(VFadeInThenOut, qubit_rects, lag_ratio=0.1), run_time=6)
                self.wait()
                bits = indices[key]
                bits.save_state()
                ...
                self.wait()
                self.play(Restore(bits), key_icon.animate.scale(0.5).next_to(bits.saved_state, LEFT, buff=SMALL_BUFF))
            if n % 2 == 1:
                neg1 = Tex('\\times -1')
                ...
                anims.append(LaggedStartMap(VShowPassingFlash, comp_lines, time_width=2.0, lag_ratio=0.005, run_time=2))
                anims.extend([ChangeDecimalToValue(entry, value, time_span=(1, 2)) for entry, value in zip(vector_entries, state)])
            self.play(*anims)
            self.wait()
        glass = get_magnifying_glass()
        ...
        self.play(FadeIn(glass))
        self.play(glass.animate.shift(qc_label.get_center() - glass[0].get_center()).set_anim_args(path_arc=-45 * DEG), rate_func=there_and_back_with_pause, run_time=6)
        self.wait()
    ...
    def get_labeled_box(self, wire, alpha, label='H', size=0.5):
        ...
        arrow = VGroup(Vector(size * vect, thickness=1).center(), Vector(-size * vect, thickness=1).center())
        arrow.move_to(box)
        label = Tex(label)
    ...
    def get_connecting_lines(self, from_mob, to_mob, from_buff=0, to_buff=0, stroke_color=TEAL_A, stroke_width=2):
        ...
        lines = VGroup(CubicBezier(l_ur, l_ur + RIGHT, v_ul + LEFT, v_ul), CubicBezier(l_dr, l_dr + RIGHT, v_dl + LEFT, v_dl))
        lines.set_stroke(TEAL_A, 2)
        return lines

    def get_dist_bars(self, dist, objs, height_ratio=0.8, width_ratio=8, fill_colors=(BLUE_D, GREEN), stroke_color=WHITE, stroke_width=1):
...
class Qubit(DisectAQuantumComputer):
    ...
    def construct(self):
        ...
        prob_rule = VGroup(Tex('P(0) = x^2'), Tex('P(1) = y^2'))
        ...
        dist_arrow_label = Tex('c \\rightarrow c^2', font_size=24)
        dist_arrow_label.next_to(dist_arrow, UP, SMALL_BUFF)
        bar_labels = VGroup(Integer(25, unit='\\%', font_size=24), Integer(75, unit='\\%', font_size=24))
        ...
        self.play(LaggedStart(TransformFromCopy(vector_label.copy().clear_updaters(), var_vect), frame.animate.center(), GrowArrow(var_arrow), FadeIn(prob_rule), run_time=2, lag_ratio=0.1))
        self.play(ShowCreation(top_rule_rect))
        self.wait()
        self.play(LaggedStart(Transform(var_vect.copy(), coord_vect.copy().clear_updaters(), remover=True), TransformFromCopy(var_arrow, dist_arrow), FadeIn(dist_arrow_label, DOWN), TransformFromCopy(VGroup((pr[1:4] for pr in prob_rule)).copy(), qubits), FadeTransformPieces(VGroup((pr[5:] for pr in prob_rule)).copy(), dist_bars), TransformFromCopy(top_rule_rect, bar_rect), lag_ratio=0.01))
        self.add(coord_vect)
        ...
        self.wait()
        self.play(top_rule_rect.animate.match_y(prob_rule[1]), bar_rect.animate.match_y(qubits[1]))
        ...
        pythag = Tex('x^2 + y^2 = 1')
        pythag.match_x(var_group)
        pythag.to_edge(UP)
        self.play(LaggedStart(var_group.animate.shift(1.5 * DOWN), TransformFromCopy(prob_rule[0]['x^2'][0], pythag['x^2'][0]), Write(pythag['+'][0]), TransformFromCopy(prob_rule[1]['y^2'][0], pythag['y^2'][0])))
        self.play(Write(pythag['= 1'][0]), run_time=1)
        self.add(pythag)
        self.wait()
        brace = LineBrace(vector, DOWN, buff=0)
        brace_label = brace.get_tex('\\sqrt{x^2 + y^2} = 1', font_size=36)
        ...
        self.play(GrowFromCenter(brace), TransformMatchingTex(pythag.copy(), brace_label, run_time=1))
        self.wait()
        self.add(vector_ghost)
        self.play(ShowCreation(circle), theta_tracker.animate.set_value(theta_tracker.get_value() + TAU), run_time=6)
        self.remove(vector_ghost)
        self.wait()
        self.play(FadeOut(brace), FadeOut(brace_label), circle.animate.set_stroke(width=1, opacity=0.5))
        ...
        self.play(FlashAround(zero_label, run_time=2, time_width=1.5, color=BLUE), FadeIn(zero_vect))
        self.wait()
        ...
        self.play(FlashAround(one_label, run_time=2, time_width=1.5, color=GREEN), FadeIn(one_vect))
        self.wait()
        ...
        weighted_sum = Tex('x|0\\rangle + y|1\\rangle')
        ...
        self.wait()
        self.play(FadeOut(red_cross), FadeTransform(var_vect_copy.get_entries()[0].copy(), weighted_sum['x']), FadeTransform(var_vect_copy.get_entries()[1].copy(), weighted_sum['y']), FadeOut(var_vect_copy), FadeTransform(zero_label.copy(), weighted_sum['|0\\rangle']), FadeTransform(one_label.copy(), weighted_sum['|1\\rangle']), FadeIn(weighted_sum['+']))
        self.wait()
        ...
        matrix_tex = Tex('\\frac{1}{\\sqrt{2}} \\left[\\begin{array}{cc} 1 & 1 \\\\ 1 & -1 \\end{array}\\right]', font_size=24)
        matrix_tex.set_fill(GREY_B)
        matrix_tex.next_to(arrow, DOWN, SMALL_BUFF)
        self.play(frame.animate.set_width(planes.get_width() + LARGE_BUFF).move_to(planes), LaggedStartMap(FadeOut, faders, shift=0.5 * DOWN, lag_ratio=0.25), zero_vect.animate.set_fill(opacity=1), one_vect.animate.set_fill(opacity=1), FadeIn(plane2, shift=RIGHT), GrowArrow(arrow), FadeIn(arrow_label, lag_ratio=0.1), FadeIn(matrix_tex), run_time=2)
        ...
        labels_image = VGroup(Tex('\\text{H}|0\\rangle').scale(0.75).rotate(45 * DEG).next_to(movers_image[1].get_center(), UL, buff=-0.05), Tex('\\text{H}|1\\rangle').scale(0.75).rotate(-45 * DEG).next_to(movers_image[2].get_center(), DL, buff=-0.05), Tex('\\text{H}\\big(x|0\\rangle + y|1\\rangle\\big)').scale(0.75).next_to(movers_image[3].get_end(), RIGHT, SMALL_BUFF))
        labels_image.set_backstroke(BLACK, 7)
        self.play(TransformFromCopy(movers, movers_image, path_arc=-30 * DEG), TransformFromCopy(labels, labels_image, path_arc=-30 * DEG), run_time=2)
        self.wait()
        ...
        for index in range(2):
            ...
            self.play(MoveToTarget(faders), ShowCreation(rect))
            self.play(TransformFromCopy(movers[index + 1], movers_image[index + 1], path_arc=-30 * DEG), TransformFromCopy(labels[index], labels_image[index], path_arc=-30 * DEG), rect.animate.surround(labels_image[index]).set_anim_args(path_arc=-30 * DEG), run_time=2)
    ...
    def get_vector(self, plane, x=1, y=0, fill_color=TEAL, thickness=6):
        return Arrow(plane.c2p(0, 0), plane.c2p(x, y), buff=0, thickness=thickness, fill_color=fill_color)
    ...
    def z_filp_insert(self):
        ...
        self.play(Rotate(flipper, PI, axis=RIGHT, about_point=plane.c2p(0, 0), run_time=6, rate_func=there_and_back_with_pause))
...
class ShowAFewFlips(Qubit):
    ...
    def construct(self):
        ...
        for i in [1, 0, 2, 1, 2, 1, 0, 1]:
            self.play(LaggedStart(AnimationGroup(FadeIn(lines[i]), FadeIn(gate_labels[i])), Rotate(vector, PI, axis=lines[i].get_vector(), about_point=ORIGIN), lag_ratio=0.5))
            self.play(FadeOut(lines[i]), FadeOut(gate_labels[i]))

class ExponentiallyGrowingState(InteractiveScene):

    def construct(self):
        label = TexText('State of a\\\\ 1 qubit computer')
        ...
        for n in range(2, 9):
            ...
            n_label.set_value(n)
            self.play(ReplacementTransform(vect[0], new_vect[0]), FadeTransform(vect[1], new_vect[1]), FadeTransformPieces(vect[2], new_vect[2]), FadeTransform(brace[0], new_brace[0]), FadeTransform(brace[1], new_brace[1]))
            self.wait()
            vect = new_vect
            brace = new_brace
        frame = self.frame
        new_brace_label = Tex('2^{100}')
        new_brace_label.set_color(YELLOW)
        new_brace_label.next_to(brace[0], RIGHT).shift(SMALL_BUFF * UR)
        top_eq = Tex('2^{100} = ' + '{:,}'.format(2 ** 100))
        top_eq.next_to(vect, UP, MED_LARGE_BUFF).set_x(0)
        self.play(FadeTransform(brace[1], new_brace_label), ChangeDecimalToValue(n_label, 100), frame.animate.set_height(9, about_edge=DOWN), FadeIn(top_eq), FadeTransform(vect, self.get_state_vector(n + 2).move_to(vect, RIGHT)))
        self.wait()
...
class InvisibleStateValues(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(ShowCreation, rects, lag_ratio=0.25), frame.animate.to_default_state(), run_time=5)
...
class ThreeDSample(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(GrowArrow(vector), VFadeIn(coord_array))
        ...
        self.play(FadeIn(key_icon, 0.25 * LEFT), symbols[2].animate.set_fill(YELLOW))
        self.wait(3)
        ...
        self.play(self.set_vect_anim(vector, balanced_state, run_time=4), FadeIn(balance_name, lag_ratio=0.1, time_span=(3, 4)))
        ...
        self.add(tail)
        self.play(frame.animate.reorient(43, 79, 0, (-0.26, 0.37, 0.15), 4.97), rot_vel_tracker.animate.set_value(-2 * DEG), FadeIn(v_slice), plane.animate.fade(0.75), axes.animate.set_stroke(opacity=0.5), run_time=3)
        self.add(b_vect_ghost)
        self.play(Rotate(vector, TAU, axis=np.cross(vector.get_end(), OUT), run_time=8, about_point=ORIGIN))
        self.play(frame.animate.reorient(-1, 79, 0, (-0.13, 0.55, 1.08), 7.67), coord_array.animate.set_x(0), run_time=2)
        tail.clear_updaters()
        ...
        self.wait(4)
        self.play(GrowFromCenter(xy_line), self.set_vect_anim(vector, normalize(UR)))
        self.wait(10)
        self.play(self.set_vect_anim(vector, balanced_state))
        self.wait(20)

    def set_vect_anim(self, vector, trg_coords, run_time=1, **kwargs):
        return Rotate(vector, angle_between_vectors(vector.get_end(), trg_coords), axis=np.cross(vector.get_end(), trg_coords), about_point=ORIGIN, run_time=run_time, **kwargs)
    ...
    def flip_along_key_axis(self):
        ...
        vector.clear_updaters()
        for _ in range(4):
            self.play(Group(mesh, vector, key_vect).animate.stretch(-1, 2, about_point=ORIGIN), run_time=2)
            self.wait()

class GroversAlgorithm(InteractiveScene):

    def construct(self):
        x_range = y_range = (-1, 1 - 1e-06)
        plane = NumberPlane(x_range, y_range, faded_line_ratio=5)
        plane.set_height(6)
        ...
        x_label_example, x_label_general = x_labels = VGroup(Tex('\\frac{1}{\\sqrt{2}}\\big(|0\\rangle + |1\\rangle \\big)', font_size=36), Tex('\\frac{1}{\\sqrt{N - 1}} \\sum_{n \\ne k} |n \\rangle', font_size=24))
        for label in x_labels:
            label.next_to(x_vect.get_end(), RIGHT)
        x_label_general.shift(SMALL_BUFF * DL)
        self.play(TransformFromCopy(key_vect, x_vect, path_arc=-90 * DEG), FadeIn(x_label_general, time_span=(0.5, 1.5)))
        self.wait()
        x_label_general.save_state()
        self.play(FadeIn(x_label_example, DOWN), x_label_general.animate.fade(0.5).shift(1.25 * DOWN))
        self.wait()
        rhs = Tex('= \\frac{1}{\\sqrt{3}}\\big(|0\\rangle + |1\\rangle + |2\\rangle \\big)', font_size=36)
        ...
        self.wait()
        self.play(TransformFromCopy(b_vect, b_vect_proj), ShowCreation(dashed_line))
        self.wait()
        self.play(FlashAround(rhs['|2\\rangle'], time_width=1.5, run_time=3), rhs['|2\\rangle'].animate.set_color(YELLOW))
        self.wait()
        N_eq = Tex('N = 3')
        dim = N_eq.make_number_changeable('3', edge_to_fix=LEFT)
        N_eq.next_to(plane, UR, LARGE_BUFF).shift_onto_screen()
        self.play(LaggedStart(FadeOut(rhs), Restore(x_label_general), FadeOut(x_label_example, 0.5 * UP)), Write(N_eq))
        ...
        self.play(N_tracker.animate.set_value(100).set_anim_args(rate_func=rush_into), UpdateFromFunc(dim, lambda m: m.set_value(int(get_N()))), UpdateFromFunc(VGroup(b_vect, b_vect_proj), update_b_vects), UpdateFromFunc(dashed_line, lambda m: m.become(DashedLine(b_vect.get_end(), b_vect_proj.get_end()))), UpdateFromFunc(b_label, lambda m: m.next_to(b_vect.get_end(), UR, SMALL_BUFF)), run_time=12)
        self.wait()
        ...
        self.play(FadeOut(b_vect_proj), FadeOut(dashed_line), ShowCreation(arc), FadeIn(alpha_label))
        ...
        frame = self.frame
        b_comp_tex = '1 / \\sqrt{N}'
        b_array = TexMatrix(np.array([*2 * [b_comp_tex], '\\vdots', *3 * [b_comp_tex], '\\vdots', *2 * [b_comp_tex]]).reshape(-1, 1))
        k_array = TexMatrix(np.array([*2 * ['0'], '\\vdots', '0', '1', '0', '\\vdots', *2 * ['0']]).reshape(-1, 1))
        ...
        self.play(GrowFromPoint(b_array, b_label.get_center()), TransformFromCopy(b_label, b_array_label), N_eq.animate.to_edge(UP, MED_SMALL_BUFF).set_x(2), frame.animate.set_x(4), FadeOut(x_label_general), FadeOut(x_vect), run_time=2)
        self.play(GrowFromPoint(k_array, key_label.get_center()), TransformFromCopy(key_label, k_array_label), Write(dot), run_time=2)
        self.wait()
        elem_rects = VGroup((SurroundingRectangle(VGroup(*elems), buff=0.05).set_width(2.5, stretch=True) for elems in zip(b_array.elements, k_array.elements)))
        for rect in elem_rects:
            rect.set_stroke(WHITE, 2)
            rect.align_to(elem_rects[0], RIGHT)
        equals = Tex('=').next_to(arrays, RIGHT)
        rhs = Tex('1 / \\sqrt{N}')
        rhs.next_to(equals, RIGHT)
        self.play(Write(equals))
        self.play(LaggedStartMap(VFadeInThenOut, elem_rects, lag_ratio=0.2, run_time=5), FadeIn(rhs, time_span=(1.75, 2.25)))
        self.wait()
        cos_expr = Tex('\\cos(\\alpha) = 1 / \\sqrt{N}', font_size=36)
        cos_expr.next_to(alpha_label, UP, MED_LARGE_BUFF, aligned_edge=LEFT)
        self.play(LaggedStart(FadeOut(lt_90), Write(cos_expr['\\cos(']), Write(cos_expr[') =']), TransformFromCopy(alpha_label, cos_expr['\\alpha'][0]), TransformFromCopy(rhs, cos_expr['1 / \\sqrt{N}'][0]), lag_ratio=0.1))
        self.wait()
        target_thickness = 3
        ...
        sin_expr = Tex('\\sin(\\theta) = 1 / \\sqrt{N}', font_size=36)
        sin_expr.move_to(cos_expr).set_y(-0.5)
        theta_approx = Tex('\\theta \\approx 1 / \\sqrt{N}', font_size=36)
        theta_approx.next_to(sin_expr, DOWN, aligned_edge=RIGHT)
        cos_group = VGroup(arc, alpha_label, cos_expr)
        self.play(MoveToTarget(b_vect), MoveToTarget(key_vect), LaggedStart(TransformFromCopy(arc, theta_arc), TransformFromCopy(alpha_label, theta_label), TransformFromCopy(cos_expr, sin_expr), lag_ratio=0.25), cos_group.animate.fade(0.5))
        self.wait()
        self.play(TransformMatchingTex(sin_expr.copy(), theta_approx, matched_keys=['1 / \\sqrt{N}'], key_map={'=': '\\approx'}))
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(array_labels, arrays, dot, equals, rhs, cos_group, sin_expr), lag_ratio=0.25), frame.animate.set_x(-2), N_eq.animate.set_x(-2), theta_approx.animate.to_corner(UR, buff=MED_SMALL_BUFF).shift(2 * LEFT), run_time=2)
        self.wait()
        vector = b_vect.copy()
        ...
        def right_filp(run_time=2):
            self.play(Rotate(flipper, PI, RIGHT, about_point=ORIGIN), vect_ghosts.animate.set_fill(opacity=0.25), run_time=run_time)

        def diag_flip(run_time=2, draw_time=0.5):
            self.play(ShowCreation(diag_line, run_time=draw_time))
            self.play(flipper.animate.flip(axis=diag_line.get_vector(), about_point=ORIGIN), vect_ghosts.animate.set_fill(opacity=0.25), UpdateFromFunc(bars, update_bars), run_time=run_time)
        ...
        if False:
            ...
            self.play(FadeOut(diag_line), TransformFromCopy(vector, v_part), ShowCreation(h_line))
            self.wait()
        ...
        arc_labels = VGroup((Tex('2\\theta', font_size=24).next_to(arc.pfp(0.5), normalize(arc.pfp(0.5)), SMALL_BUFF) for arc in arcs))
        self.play(Rotate(vector, -angle_between_vectors(vector.get_vector(), b_vect.get_vector()), about_point=ORIGIN), FadeOut(b_label))
        self.wait()
        right_filp()
        diag_flip(draw_time=0.25)
        self.wait()
        self.play(TransformFromCopy(vect_ghosts[0], vector, path_arc=theta), ShowCreation(arcs[0]))
        self.play(TransformFromCopy(theta_label, arc_labels[0]))
        self.wait()
        for arc, label in zip(arcs[1:], arc_labels[1:]):
            self.play(Rotate(vector, 2 * theta, about_point=ORIGIN), ShowCreation(arc), FadeIn(label, shift=0.5 * (arc.get_end() - arc.get_start())))
            self.wait()
        quarter_arc = Arc(0, 90 * DEG)
        ninety_label = Tex('90^\\circ')
        pi_halves_label = Tex('\\pi / 2')
        for label in [ninety_label, pi_halves_label]:
            label.set_backstroke(BLACK, 5)
            label.next_to(quarter_arc.pfp(0.5), UR, buff=0.05)
        self.play(ShowCreation(quarter_arc), Write(ninety_label))
        self.wait()
        self.play(FadeTransform(ninety_label, pi_halves_label))
        self.wait()
        lhs = Text('# Repetitions')
        lhs.next_to(plane, RIGHT, buff=1.0).set_y(2)
        rhs_terms = VGroup(Tex('\\approx {\\pi / 2 \\over 2 \\theta}'), Tex('= {\\pi \\over 4} \\cdot {1 \\over \\theta}'), Tex('= {\\pi \\over 4} \\sqrt{N}'))
        ...
        self.play(frame.animate.set_x(3), FadeOut(vect_coords), FadeOut(bars), FadeOut(key_icon2), Write(lhs), Write(rhs_terms[0]))
        self.wait()
        self.play(LaggedStartMap(FadeIn, rhs_terms[1:], shift=0.5 * DOWN, lag_ratio=0.5))
        self.wait()
        self.play(LaggedStart(Rotate(vector, -angle_between_vectors(vector.get_vector(), b_vect.get_vector()), about_point=ORIGIN), FadeOut(arcs), FadeOut(arc_labels), FadeOut(vect_ghosts), FadeOut(b_vect), FadeOut(theta_label), FadeOut(theta_arc), frame.animate.set_y(0.5), N_eq.animate.shift(0.5 * UP), theta_approx.animate.align_to(plane, RIGHT), VGroup(lhs, rhs_terms).animate.shift(1.5 * UP)))
        new_theta = math.asin(2 ** (-10))
        dim.set_value(100)
        step_count = Tex('\\frac{\\pi}{4}\\sqrt{2^{20}} = 804.248...')
        step_count.next_to(rhs_terms, DOWN, LARGE_BUFF)
        step_count.to_edge(RIGHT).shift(frame.get_x() * RIGHT)
        eq_two_twenty = Tex('=2^{20}')
        eq_two_twenty.next_to(N_eq['='], DOWN, aligned_edge=LEFT)
        self.play(ChangeDecimalToValue(dim, int(2 ** 20)), Rotate(vector, new_theta - theta, about_point=ORIGIN), FadeIn(eq_two_twenty, time_span=(0, 1)), run_time=3)
        self.wait()
        self.play(TransformFromCopy(rhs_terms[-1][1:6], step_count[:5]), TransformFromCopy(eq_two_twenty[1:], step_count['2^{20}'][0]), run_time=2)
        self.play(Write(step_count['= 804.248...'][0]))
        self.wait()
        step_tracker = ValueTracker(0)
        radius = 0.5 * plane.get_width()
        step_label = Tex('\\#\\text{Reps} = 0', font_size=36)
        ...
        self.play(step_tracker.animate.set_value(2 * 804), UpdateFromFunc(vector, update_vector), frame.animate.scale(1.4, about_edge=RIGHT).set_anim_args(time_span=(0, 8)), run_time=20, rate_func=linear)
...
class TwoFlipsEqualsRotation(InteractiveScene):
    ...
    def construct(self):
        ...
        for line in flip_lines:
            ...
            self.play(randy.animate.flip(axis=line.get_vector(), about_point=plane.get_origin()), plane.animate.flip(axis=line.get_vector(), about_point=plane.get_origin()))
        self.wait()
        arcs = VGroup(Arc(0, 90 * DEG, radius=1), Arc(180 * DEG, 90 * DEG, radius=1))
        for arc, vect in zip(arcs, [UR, DL]):
            arc.set_stroke(WHITE, 5)
            arc.move_to(plane2.get_corner(vect))
            arc.add_tip()
        self.play(self.frame.animate.move_to(midpoint(plane.get_center(), plane2.get_center())), FadeIn(ghost_plane2), FadeIn(plane2), FadeIn(randy2))
        self.play(Rotate(Group(plane2, Point(), randy2), 2 * theta, about_point=plane2.get_center()), *map(FadeIn, arcs))
        self.wait()
        arc = Arc(0, theta, radius=1.0)
        theta_label = Tex('\\theta', font_size=30)
        ...
        self.play(TransformFromCopy(arc, rot_arc), TransformFromCopy(theta_label, two_theta_label))
        self.wait()
...
class ComplexComponents(InteractiveScene):
    ...
    def construct(self):
        ...
        for index, elem in zip(indices, dec_vect.elements):
            ...
            index.match_y(elem)
        x_vect, z_vect = var_vects = [TexMatrix(np.array([f'{char}_{n}' for n in range(4)]).reshape(-1, 1)) for char in 'xz']
        for vect in var_vects:
            vect.match_height(dec_vect)
            vect.move_to(dec_vect, LEFT)
        self.add(dec_vect, indices)
        self.wait()
        number_lines = VGroup((NumberLine((-1, 1, 0.1), big_tick_spacing=1, width=3, tick_size=0.05, stroke_color=GREY_C) for n in range(4)))
        ...
        R_labels, C_labels = [VGroup((Tex(f'\\mathds{{{char}}}', font_size=36).next_to(mob.get_right(), RIGHT) for mob in group)) for char, group in zip('RC', [number_lines, complex_planes.target])]
        ...
        self.play(LaggedStartMap(FadeIn, number_lines, lag_ratio=0.25), LaggedStartMap(FadeIn, R_labels, lag_ratio=0.25), LaggedStart((FadeTransform(elem, dot) for elem, dot in zip(dec_vect.elements, dots)), lag_ratio=0.05, group_type=Group), LaggedStart((FadeTransform(elem.copy(), x) for elem, x in zip(dec_vect.elements, x_vect.elements)), lag_ratio=0.05), LaggedStartMap(ShowCreation, dot_lines), ReplacementTransform(dec_vect.get_brackets(), x_vect.get_brackets(), time_span=(1, 2)), run_time=2)
        self.add(dots, dot_lines)
        dot_lines.add_updater(update_dot_lines)
        self.play(state_tracker.animate.set_value(normalize(np.random.uniform(-1, 1, 4))), run_time=4)
        number_lines.generate_target()
        for line, plane in zip(number_lines.target, complex_planes.target):
            line.replace(plane, 0)
            line.set_opacity(0)
        self.play(MoveToTarget(complex_planes), MoveToTarget(number_lines, remover=True), *(FadeTransform(R, C) for R, C in zip(R_labels, C_labels)), ReplacementTransform(x_vect, z_vect))
        self.wait()
        for n in range(4):
        ...
        self.play(frame.animate.set_height(1.6).move_to(plane), FadeIn(c_dot), FadeOut(dots), FadeOut(dot_lines), run_time=2)
        ...
        mag_label = Tex('|z_0|', font_size=12)
        (mag_label.next_to(brace.get_center(), DR, buff=0.05),)
        self.play(ShowCreation(c_line), GrowFromCenter(brace), FadeIn(mag_label))
        arc = always_redraw(lambda: Arc(0, c_line.get_angle() % TAU, radius=0.1, arc_center=plane.c2p(0), stroke_color=MAROON_B))
        arc.update()
        arc.suspend_updating()
        phi_label = Tex('\\varphi', font_size=12)
        ...
        self.wait()
        prob_eq = Tex('\\text{Prob} = |z_0|^2', font_size=12)
        prob_eq.move_to(mag_label)
        prob_eq.align_to(C_labels[0], RIGHT).shift(0.2 * RIGHT)
        self.play(TransformFromCopy(mag_label, prob_eq['|z_0|'][0], path_arc=45 * DEG), Write(prob_eq['\\text{Prob} ='][0]), Write(prob_eq['2'][0]))
        self.wait()
        ...
        for angle in [-(1 - 1e-05) * phi, PI, phi - PI]:
            self.play(Rotate(c_line, angle, about_point=plane.n2p(0), run_time=6))
            self.wait()
        ...
        self.play(frame.animate.to_default_state(), FadeOut(fader), FadeIn(dots), FadeIn(dot_lines), run_time=4)
        self.random_state_change(state_tracker)
        dots.suspend_updating()
        self.play(*(Rotate(dot, random.random() * 2 * TAU, about_point=plane.c2p(0), run_time=10, rate_func=there_and_back) for dot, plane in zip(dots, complex_planes)))
