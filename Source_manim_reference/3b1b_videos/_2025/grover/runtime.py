...
class Quiz(InteractiveScene):
    ...
    def construct(self):
        choices = VGroup(TexText('A) $\\mathcal{O}\\big(\\sqrt{N}\\big)$'), TexText('B) $\\mathcal{O}\\big(\\log(N)\\big)$'), TexText('C) $\\mathcal{O}\\big(\\log(\\log(N))\\big)$'), TexText('D) $\\mathcal{O}(1)$'))
        choices.arrange(DOWN, buff=0.5, aligned_edge=LEFT)
        choices.to_edge(LEFT, buff=1.0)
        covers = VGroup((SurroundingRectangle(choice[2:], buff=SMALL_BUFF) for choice in choices))
        ...
        self.play(LaggedStartMap(Restore, covers, lag_ratio=0.25), LaggedStartMap(FadeIn, choices, lag_ratio=0.25))
        self.wait()
        ...
        symbol_height = 0.35
        symbols = [Exmark().set_color(RED).set_height(symbol_height), Checkmark().set_color(GREEN).set_height(symbol_height)]
        all_symbols = VGroup()
        for pi in pis:
        ...
        self.play(LaggedStartMap(MoveToTarget, pis, lag_ratio=0.01), LaggedStartMap(Write, all_symbols, lag_ratio=0.01))
        pis.shuffle()
        self.play(LaggedStartMap(Blink, pis[::4]))
        self.wait()
        self.play(FadeOut(pis, lag_ratio=0.001), FadeOut(all_symbols, lag_ratio=0.001))
        frame = self.frame
        question = VGroup(get_quantum_computer_symbol(), Clock(), Tex('?'))
        for mob in question:
            mob.set_height(1.5)
        ...
        self.play(VFadeIn(question, suspend_mobject_updating=True, lag_ratio=0.01), VGroup(choices, covers).animate.shift(DOWN).set_anim_args(run_time=2), frame.animate.match_x(choices).set_anim_args(run_time=2))
        self.add(choices, Point(), covers)
        choices.set_opacity(1)
        self.play(LaggedStart((cover.animate.stretch(0, 0, about_edge=RIGHT).set_opacity(0) for cover in covers), lag_ratio=0.25))
        self.wait(16)
        question.clear_updaters()
        dists = [np.array([18, 20, 8, 54], dtype=float), np.array([51, 55, 37, 65], dtype=float), np.array([17, 25, 5, 39], dtype=float)]
        for dist in dists:
            dist[:] = dist / dist.sum()
        max_bar_width = 5.0
        ...
        for bar in prob_bars:
            label = Integer(100, font_size=36, unit='\\%')
            label.bar = bar
        ...
        self.play(LaggedStart((GrowFromPoint(bar, bar.get_right()) for bar in prob_bars), lag_ratio=0.2), VFadeIn(prob_labels))
        self.wait()
        for index in [1, 2, 0]:
            self.play(Transform(prob_bars, prob_bar_group[index]))
            self.wait()
        covers = VGroup((SurroundingRectangle(VGroup(bar, label, choice)) for bar, label, choice in zip(prob_bars, prob_labels, choices)))
...
class ShowOptionGraphs(InteractiveScene):
    ...
    def construct(self):
        ...
        axes.set_height(7)
        self.add(axes)
        graphs = VGroup((axes.get_graph(func, x_range=(0.01, x_max)) for func in [lambda n: n, lambda n: math.sqrt(n), lambda n: 0.8 * math.log(n + 1), lambda n: math.log(math.log(n + 1) + 1), lambda n: 1]))
        graphs.set_submobject_colors_by_gradient(YELLOW, ORANGE, RED, RED_E, BLUE)
        labels = VGroup((Tex(sym, font_size=30).match_color(graph).next_to(graph.get_end(), RIGHT, SMALL_BUFF) for graph, sym in zip(graphs, ['\\mathcal{O}\\left(N\\right)', '\\mathcal{O}\\left(\\sqrt{N}\\right)', '\\mathcal{O}\\left(\\log(N)\\right)', '\\mathcal{O}\\left(\\log(\\log(N))\\right)', '\\mathcal{O}\\left(1\\right)'])))
        labels[-1].shift(2 * SMALL_BUFF * DOWN)
        for graph, label in zip(graphs, labels):
            vect = label.get_center() - graph.get_end()
            self.play(ShowCreation(graph), VFadeIn(label), UpdateFromFunc(label, lambda m: m.move_to(graph.get_end() + vect)))
        self.wait()
...
class NeedleInAHaystackProblem(InteractiveScene):
    ...
    def construct(self):
        ...
        number_set = Tex(''.join(['\\{', *[str(n) + ',' for n in shown_numbers], '\\dots N - 1\\}']), isolate=number_strs)
        ...
        self.play(FlashAround(key_input), key_input.animate.set_color(YELLOW), FadeIn(key_icon, 0.25 * DOWN))
        self.wait()
        ...
        self.play(FadeOut(in_mob, DOWN), FadeOut(machine.output_group, DOWN), MoveToTarget(other_inputs, lag_ratio=0.01))
        machine.output_group.clear()
        self.play(LaggedStart((self.evaluation_animation(mob, machine) for mob in other_inputs), lag_ratio=0.2))
        self.wait()
        self.play(FadeOut(other_inputs, shift=0.25 * DOWN, lag_ratio=0.01), FadeOut(machine.output_group, 0.25 * DOWN))
        machine.output_group.clear()
        self.wait()
        innards = Code('\n            def f(n):\n                return (n == 12)\n        ', font_size=16)
        ...
        self.wait()
        self.play(FadeOut(innards), machine.animate.set_fill(opacity=1), FadeIn(q_marks, shift=0.25 * UP, lag_ratio=0.25))
        self.play(FadeOut(q_marks))
        self.wait()
        last_group = VGroup()
        for n, in_mob in enumerate(number_mobs[:key_number + 1].copy()):
            self.play(FadeOut(last_group), in_mob.animate.scale(1.5).next_to(machine, LEFT, MED_LARGE_BUFF))
            output = n == key_number
        ...
        self.play(MoveToTarget(superposition, run_time=2))
        answers = VGroup((Text('True').set_color(GREEN) if n == key_number else Text('False').set_color(RED) for n, piece in enumerate(superposition.pieces)))
        ...
        self.play(LaggedStart(LaggedStart((FadeOutToPoint(glow.copy(), machine.get_left() + 0.5 * RIGHT) for glow in superposition.glows), lag_ratio=0.1), LaggedStart((FadeInFromPoint(answer, machine.get_right() + 0.5 * LEFT) for answer in answer_superposition.pieces), lag_ratio=0.05), lag_ratio=0.5))
        self.play(answer_superposition.animate.set_offset_multiple(0.025).set_glow_opacity(0.01))
        self.wait(10)
    ...
    def evaluation_animation(self, input_mob, machine, output=False, run_time=1.0):
        ...
        return AnimationGroup(FadeOutToPoint(moving_input, in_point, time_span=(0, 0.75 * run_time)), FadeInFromPoint(out_mob, machine.get_left(), time_span=(0.25 * run_time, run_time)))
...
class WriteClassicalBigO(InteractiveScene):
    ...
    def construct(self):
        ...
        avg = TexText('Avg: $\\displaystyle \\frac{1}{2} N$')
        arrow = Tex('\\longrightarrow')
        big_o = Tex('\\mathcal{O}(N)')
        ...
        self.play(Write(arrow), TransformFromCopy(avg[-1], big_o[2], path_arc=45 * DEG), Write(big_o[:2]), Write(big_o[3:]))
        self.wait()
...
class ReferenceNeedleInAHaystack(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(fader), ShowCreation(rect))
...
class SuperpositionAsParallelization(InteractiveScene):
    ...
    def construct(self):
        classical, quantum = symbols = VGroup(get_classical_computer_symbol(), get_quantum_computer_symbol())
        ...
        self.play(value_tracker.animate.set_value(12), UpdateFromFunc(bit_mobs, lambda m: match_value(m, value_tracker.get_value())), run_time=3, rate_func=linear)
        self.wait()
        bit_strings = VGroup()
        ...
        superposition.set_glow_opacity(0)
        superposition.update()
        superposition_name = TexText("``Superposition''")
        superposition_name.set_color(TEAL)
        superposition_name.next_to(superposition, RIGHT, aligned_edge=UP).shift(LEFT)
        self.play(LaggedStart((TransformFromCopy(bit_mobs, bit_string) for bit_string in bit_strings), lag_ratio=0.05))
        self.play(superposition.animate.set_offset_multiple(0.1).set_glow_opacity(0.05).shift(1.5 * LEFT), Write(superposition_name, run_time=1))
        self.wait(15)
        mini_classical = VGroup((classical.copy().set_height(0.25).move_to(point).match_x(quantum) for point in superposition.center_points))
        lines = VGroup((VGroup(Line(mc.get_left() + 1.1 * LEFT, mc.get_left(), buff=0.1), Line(mc.get_right(), mc.get_right() + 1.1 * RIGHT, buff=0.1)) for mc in mini_classical))
        lines.set_stroke(GREY, 2)
        outputs = VGroup((Integer(int(n == 12), font_size=24).next_to(line, RIGHT) for n, line in enumerate(lines)))
        self.play(superposition.animate.set_offset_multiple(0.025), FadeOut(superposition_name), LaggedStart((TransformFromCopy(classical, mc) for mc in mini_classical), lag_ratio=0.05), LaggedStartMap(ShowCreation, lines))
        self.wait()
        self.play(LaggedStart((TransformFromCopy(piece, output) for piece, output in zip(superposition.pieces, outputs)), lag_ratio=0.01, run_time=3))
        self.wait(15)
...
class LogTable(InteractiveScene):
    ...
    def construct(self):
        ...
        N_title, logN_title = titles = VGroup(Tex('N'), Tex('\\log_2(N)'))
        titles.scale(1.25)
        ...
        for index in range(len(N_values) - 1):
            self.play(TransformMatchingShapes(N_values[index].copy(), N_values[index + 1]), FadeIn(logN_values[index + 1], shift=0.5 * DOWN), run_time=1)
        self.add(N_values)
        self.add(logN_values)
        all_arrows = VGroup()
        for line in h_lines[1:-1]:
            arrow = Arrow(logN_values[0].get_right(), logN_values[1].get_right(), path_arc=-PI, buff=0)
            arrow.scale(0.8)
            arrow.next_to(line, RIGHT, SMALL_BUFF)
            plus_label = Tex('+\\log_2(10)', font_size=24)
...
class SecondMisconception(InteractiveScene):
    ...
    def construct(self):
        ...
        words = Text('Quantum Computers would make\n' + 'everything exponentially faster', font_size=60)
        ...
        self.wait()
        self.play(LaggedStart(ShowCreation(red_cross), FadeIn(new_words, lag_ratio=0.1), FadeOut(title), lag_ratio=0.35))
        self.wait()
        ...
        lines = VGroup((Line(product.get_bottom(), factor.get_top(), buff=0.2) for factor in factors))
        ...
        self.play(LaggedStartMap(ShowCreation, lines, lag_ratio=0.25), LaggedStart((TransformFromCopy(product, factor) for factor in factors), lag_ratio=0.25), Write(times, time_span=(0.5, 1.5)))
        self.wait()

class GroverTimeline(InteractiveScene):

    def construct(self):
        timeline = NumberLine((1990, 2025, 1), big_tick_spacing=5, width=FRAME_WIDTH - 1)
        timeline.set_y(-2)
        timeline.add_numbers(range(1990, 2030, 5), group_with_commas=False, font_size=24)
        self.add(timeline)
        bbbv_statement = VGroup(Text('Quantum Search', font_size=36), Tex('\\ge', font_size=42), Tex('\\mathcal{O}(\\sqrt{N})', font_size=36))
        ...
        bbbv_dots = VGroup(Dot(timeline.n2p(1994)), Dot().next_to(bbbv_attribution, LEFT, SMALL_BUFF))
        bbbv_dots.set_color(RED_B)
        arc = -45 * DEG
        bbbv_line = Line(bbbv_dots[0].get_center(), bbbv_dots[1].get_center(), path_arc=arc)
        bbbv_line.set_stroke(RED_B)
        self.play(GrowFromCenter(bbbv_dots[0]), TransformFromCopy(*bbbv_dots, path_arc=arc), ShowCreation(bbbv_line), FadeIn(bbbv_attribution, UP), FadeIn(footnote))
        ...
        eq = Tex('=').replace(grover_statement[1], dim_to_match=0)
        ...
        grover_dots = VGroup(Dot(timeline.n2p(1996)), Dot().next_to(grover_name, LEFT, SMALL_BUFF))
        arc = -35 * DEG
        grover_line = Line(grover_dots[0].get_center(), grover_dots[1].get_center(), path_arc=arc)
        VGroup(grover_dots, grover_line).set_color(BLUE_B)
        self.play(TransformFromCopy(bbbv_dots[0], grover_dots[0], path_arc=-PI))
        self.play(TransformFromCopy(*grover_dots, path_arc=arc), ShowCreation(grover_line), FadeIn(grover_name, UP))
        self.wait()
        self.play(TransformFromCopy(bbbv_statement, grover_statement))
        self.wait()
        examples = VGroup()
        for n in [6, 12]:
            n_eq = VGroup(Tex('N = '), Integer(10 ** n))
            n_eq.arrange(RIGHT, SMALL_BUFF)
            n_eq.to_corner(UR, buff=MED_LARGE_BUFF)
            steps = VGroup(Tex('\\sim'), Integer(10 ** (n / 2)), Dot().set_fill(opacity=0), Text('Steps'))
        ...
        for n, example in enumerate(examples):
            if n == 0:
                self.play(CountInFrom(example[0][1], 0), VFadeIn(example[0]))
            elif n == 1:
                self.play(ReplacementTransform(examples[0][0], examples[1][0]), FadeOut(examples[0][1:]))
            sqrt_rect = SurroundingRectangle(sqrt_N, buff=SMALL_BUFF)
            sqrt_rect.set_stroke(WHITE, 2)
            self.play(ShowCreation(sqrt_rect))
            self.play(GrowArrow(example[1]), sqrt_rect.animate.surround(example[2][1]).set_stroke(opacity=0), FadeTransform(sqrt_N.copy(), example[2][1]), FadeIn(example[2][0]), FadeIn(example[2][-1]))
            self.add(example)
            self.wait()
        big_O = grover_statement[2]
        runtime = Tex('\\left\\lceil \\frac{\\pi}{4} \\right\\rceil \\sqrt{N}', font_size=36)
...
class NPProblemExamples(InteractiveScene):
    ...
    def construct(self):
        example_images = Group(self.get_sudoku(), ImageMobject('US_color_graph'), Square().set_opacity(0))
        for img in example_images:
            img.set_height(2)
        example_images.arrange_in_grid(2, 2, buff=1.5)
        example_images[2].match_x(example_images[:2])
        example_names = VGroup(Text('Sudoku'), Text('Graph Coloring'), Text('Reversing Cryptographic\nHash Functions'))
        examples = Group()
        ...
        self.play(LaggedStart(ShowCreation(big_rect), Write(name), lag_ratio=0.2))
        self.wait()
...
class ShowSha256(InteractiveScene):
    ...
    def construct(self):
        ...
        output_text = Text('')
        lhs = VGroup(Text('SHA256('), input_int, Text(')'))
        ...
        def update_hash(text_mob):
            ...
            new_text = '\n'.join((''.join(row) for row in np.array(list(hash_hex)).reshape((4, 16))))
...
class ContrastTwoAlgorithmsFrame(DisectAQuantumComputer):
    ...
    def construct(self):
        ...
        self.add(background)
        self.add(screens)
        titles = VGroup(VGroup(get_classical_computer_symbol(), Tex('\\mathcal{O}(N)')), VGroup(get_quantum_computer_symbol(), Tex('\\mathcal{O}(\\sqrt{N})')))
        ...
        values = VGroup((Integer(n, font_size=12).replace(box, dim_to_match=1) for n, box in enumerate(boxes)))
        dist = np.ones(100)
        width_ratio = 5
        ...
        lines = VGroup((Line(point, box.get_center()) for point, box in zip(q_dots.get_points(), boxes)))
        for line in lines:
        ...
        for n in range(1, 10):
            ...
            width_ratio *= 0.9
            lines.shuffle
            self.play(LaggedStartMap(VShowPassingFlash, lines, time_width=1.5, lag_ratio=0.002), Transform(bars, self.get_dist_bars(dist, boxes, width_ratio=width_ratio), time_span=(0.5, 1)))
...
class QuantumCompilation(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(machine.animate.scale(2, about_point=ORIGIN).set_fill(GREY_E), FadeOut(label, scale=2))
        ...
        self.play(number.animate.next_to(bit_string, UP, MED_LARGE_BUFF), TransformFromCopy(number.replicate(5), bit_string, lag_ratio=0.01))
        self.wait()
        self.play(FadeOut(bit_string.copy(), 2 * RIGHT, lag_ratio=0.05, path_arc=45 * DEG), FadeIn(output, RIGHT, time_span=(0.75, 1.5)))
        self.play(ChangeDecimalToValue(number, 5), UpdateFromFunc(bit_string, lambda m: m.set_value(number.get_value())), run_time=1)
        output.set_value(1)
        self.wait()
        ...
        self.play(MoveToTarget(c_machine), bit_string.animate.next_to(c_machine.target, LEFT), output.animate.next_to(c_machine.target, RIGHT, MED_LARGE_BUFF), FadeOut(number, UP))
        self.play(GrowArrow(arrow))
        self.play(FadeTransform(c_machine[0].copy(), q_machine), TransformFromCopy(c_machine[1], q_label, lag_ratio=0.01, run_time=2))
        self.wait()
        q_input = KetGroup(bit_string.copy())
        q_input.next_to(q_machine, LEFT)
        q_output = q_input.copy()
        neg = Tex('-').next_to(q_output, LEFT, SMALL_BUFF)
        q_output.add(neg)
        ...
        self.wait()
        self.play(input_rect.animate.surround(q_input), TransformFromCopy(VGroup(VectorizedPoint(bit_string.get_center()), bit_string), q_input))
        self.play(FadeOut(input_rect), FadeOut(q_input.copy(), 3 * RIGHT), FadeIn(q_output, 3 * RIGHT, time_span=(0.5, 1.5)))
        self.wait()
        ...
        self.play(input_value_tracker.animate.set_value(13).set_anim_args(rate_func=linear), UpdateFromFunc(bit_string, lambda m: m.set_value(int(input_value_tracker.get_value()))), UpdateFromFunc(q_input[1], lambda m: m.set_value(int(input_value_tracker.get_value()))), run_time=2)
        self.wait()
        ...
        self.wait()
        combined_input = VGroup(q_input.copy(), Tex('+'), flipped_input)
        ...
        combined_output = VGroup(q_output2.copy(), Tex('+'), flipped_output)
        combined_output.arrange(DOWN, buff=SMALL_BUFF)
        combined_output.next_to(q_machine, RIGHT)
        self.play(ReplacementTransform(q_input, combined_input[0]), Write(combined_input[1:]), ReplacementTransform(q_output2, combined_output[0]), Write(combined_output[1:]), FadeIn(key_icon))
        self.play(input_value_tracker.animate.set_value(5).set_anim_args(rate_func=linear), UpdateFromFunc(bit_string, lambda m: m.set_value(int(input_value_tracker.get_value()))))
        output.set_value(1)
        self.remove(ex)
        self.add(check)
        self.wait()