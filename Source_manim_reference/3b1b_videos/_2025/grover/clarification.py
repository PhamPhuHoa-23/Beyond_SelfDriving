...
default_sudoku_values = [[7, 4, 3, 6, 5, 1, 9, 8, 2], [1, 8, 9, 2, 3, 4, 7, 6, 5], [6, 2, 5, 9, 8, 7, 4, 3, 1], [3, 2, 5, 8, 6, 9, 4, 1, 7], [9, 7, 6, 3, 4, 1, 5, 2, 8], [1, 4, 8, 7, 5, 2, 3, 6, 9], [2, 3, 8, 5, 7, 6, 1, 9, 4], [4, 1, 7, 8, 9, 3, 6, 5, 2], [5, 9, 6, 2, 1, 4, 8, 7, 3]]
default_sudoku_locked_cells = [[3, 7, 8], [1, 6, 7, 8], [3, 5], [1, 3, 6, 8], [5], [1, 3, 4, 8], [5, 6], [5], [4, 9]]

class Sudoku(VGroup):

    def __init__(self, values=default_sudoku_values, locked_cells=default_sudoku_locked_cells, height=4, big_square_stroke_width=3, little_square_stroke_width=0.5, locked_number_color=BLUE_B, num_to_square_height_ratio=0.5):
        self.big_grid = self.get_square_grid(height, big_square_stroke_width)
        self.little_grids = VGroup((self.get_square_grid(height / 3, little_square_stroke_width).move_to(square) for square in self.big_grid))
        self.numbers = VGroup((VGroup((Integer(num).replace(square, 1).scale(num_to_square_height_ratio) for square, num in zip(grid, arr))) for grid, arr in zip(self.little_grids, values)))
    ...
    def get_square_grid(self, height, stroke_width, stroke_color=WHITE, fill_color=GREY_E):
        square = Square()
...
class Intro(InteractiveScene):
    ...
    def construct(self):
        ...
        icon.center()
        self.play(Write(icon, run_time=3, lag_ratio=0.01, stroke_color=TEAL))
        self.wait()
        ...
        comments = Group((Group(ImageMobject(folder / name)) for name in os.listdir(folder)))
        ...
        self.play(FadeOut(icon, time_span=(0, 2)), LaggedStartMap(FadeIn, comments, lag_ratio=0.6, shift=0.25 * UP, run_time=8))
        self.wait()
...
class HowDoYouKnowWhichAxis(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(morty.change('guilty'), stds[2].says("Doesn't this assumes\nknowing the key value?", mode='angry', bubble_direction=LEFT), stds[0].change('confused', look_at=self.screen), stds[1].change('pleading', look_at=self.screen))
        self.wait()
...
class SudokuChecker(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(Write(sudoku, run_time=3, lag_ratio=0.01))
        ...
        self.wait()
        row_rects, col_rects, square_rects = rect_groups = VGroup((VGroup((VGroup((SurroundingRectangle(num) for num in sg)) for sg in group)) for group in [sudoku.get_rows(), sudoku.get_columns(), sudoku.get_number_squares()]))
        rect_groups.set_stroke(YELLOW, 3)
        for group in rect_groups:
            self.play(LaggedStart(*(VFadeInThenOut(sg, lag_ratio=0.025) for sg in group), lag_ratio=0.5, run_time=5))
        unlocked_numbers = sudoku.unlocked_numbers
        q_marks = VGroup((Text('?').replace(num, 1) for num in sudoku.unlocked_numbers))
        q_marks.set_color(RED)
        unlocked_numbers.save_state()
        self.play(Transform(unlocked_numbers, q_marks, lag_ratio=0.01, run_time=2))
        self.wait()
...
class ArrowToQC(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(GrowArrow(arrow), FadeIn(icon, DOWN))
        self.wait()
...
class CompiledSudokuVerifier(InteractiveScene):
    ...
    def construct(self):
        ...
        sudoku.to_edge(LEFT)
        machine = get_blackbox_machine(height=3, label_tex='\\text{Verifier}')
        machine.next_to(sudoku, RIGHT, LARGE_BUFF)
        machine[-1].scale(0.65).set_color(YELLOW)
        self.add(sudoku)
        self.add(machine)
        gates = VGroup(SVGMobject('and_gate'), SVGMobject('or_gate'), SVGMobject('not_gate'))
        ...
        self.play(MoveToTarget(pile_of_gates, lag_ratio=3.0 / len(pile_of_gates), run_time=8))
        all_numbers = VGroup((num for grid in sudoku.numbers for num in grid))
        bit_groups = VGroup((BitString(num.get_value()).replace(num, 1).scale(0.35) for num in all_numbers))
        self.play(ReplacementTransform(all_numbers, bit_groups, lag_ratio=0.001), sudoku.big_grid.animate.set_fill(opacity=0.1).set_stroke(opacity=0.25), sudoku.little_grids.animate.set_fill(opacity=0.1).set_stroke(opacity=0.25))
        self.wait()
        ...
        self.play(TransformFromCopy(bit_groups, target_bits, lag_ratio=0.001, run_time=2))
        self.wait()
...
class StateVectorsAsABasis(InteractiveScene):
    ...
    def construct(self):
        ...
        axes.set_height(4)
        basis_vectors = VGroup((Vector(2 * vect, thickness=4) for vect in np.identity(3)))
        ...
        self.add(axes)
        two_qubits = VGroup((KetGroup(BitString(n, length=2)) for n in range(4)))
        four_qubits = VGroup((KetGroup(BitString(n, length=4)) for n in range(16)))
        ...
        basis_labels = VGroup(two_qubits[0].copy().scale(0.3).rotate(90 * DEG, RIGHT).next_to(basis_vectors[0].get_end(), OUT, SMALL_BUFF), two_qubits[1].copy().scale(0.3).rotate(90 * DEG, RIGHT).next_to(basis_vectors[1].get_end(), OUT, SMALL_BUFF), two_qubits[2].copy().scale(0.3).rotate(90 * DEG, RIGHT).next_to(basis_vectors[2].get_end(), RIGHT, SMALL_BUFF))
        ...
        for src, trg, vect in zip(two_qubits, basis_labels, basis_vectors):
            self.play(TransformFromCopy(src, trg), GrowArrow(vect))
        self.wait(2)
        basis_name = TexText("``Basis vectors''")
        ...
        new_basis_labels = VGroup(four_qubits[0].copy().scale(0.3).rotate(90 * DEG, RIGHT).next_to(basis_vectors[0].get_end(), OUT, SMALL_BUFF), four_qubits[1].copy().scale(0.3).rotate(90 * DEG, RIGHT).next_to(basis_vectors[1].get_end(), OUT, SMALL_BUFF), four_qubits[2].copy().scale(0.3).rotate(90 * DEG, RIGHT).next_to(basis_vectors[2].get_end(), RIGHT, SMALL_BUFF))
        new_basis_labels.unfix_from_frame()
        self.play(FadeOut(two_qubits), FadeOut(basis_labels), LaggedStartMap(FadeIn, four_qubits, lag_ratio=0.25))
...
class OperationsOnQC(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(FadeOut(in_ket.copy(), shift=2 * RIGHT, scale=0.5), FadeIn(bad_output, shift=2 * RIGHT, scale=2), lag_ratio=0.5))
...
class TwoByTwoGrid(InteractiveScene):
    ...
    def construct(self):
        ...
        good_sudoku, bad_sudoku = sudokus = VGroup((Sudoku(big_square_stroke_width=2, little_square_stroke_width=0.25) for x in range(2)))
        ...
        self.add(sudokus)
        for sudoku, verifier, output, mark in zip(sudokus, verifiers, classical_outputs, marks):
            self.play(LaggedStart(FadeOutToPoint(sudoku.numbers.copy(), verifier.get_center(), lag_ratio=0.001), FadeInFromPoint(output, verifier.get_center()), lag_ratio=0.5, run_time=2))
            self.play(Write(mark))
        self.wait()
        ...
        mult_neg_1_words = TexText('Multiply\\\\by $-1$', font_size=36)
        ...
        self.wait()
        self.play(LaggedStart(FadeOutToPoint(good_ket.copy(), verifiers[2].get_center(), lag_ratio=0.01), FadeInFromPoint(good_ket_out, verifiers[2].get_center(), lag_ratio=0.01), lag_ratio=0.5), FadeIn(mult_neg_1_words, RIGHT))
        self.wait()
        ...
        mult_pos_1_words = TexText('Multiply\\\\by $+1$', font_size=36)
        ...
        self.wait()
        self.play(LaggedStart(FadeOutToPoint(bad_ket.copy(), verifiers[3].get_center(), lag_ratio=0.01), FadeInFromPoint(bad_ket_out, verifiers[3].get_center(), lag_ratio=0.01), lag_ratio=0.5), FadeIn(mult_pos_1_words, RIGHT))
        self.wait()
        gate_groups = VGroup(SVGMobject('and_gate').replicate(2), SVGMobject('not_gate').replicate(2), SVGMobject('or_gate').replicate(2))
    ...
    def turn_into_bits(self, sudoku):
        bits = VGroup((BitString(num.get_value()).replace(num, 1).scale(0.35) for grid in sudoku.numbers for num in grid))
        sudoku.save_state()
        in_group = VGroup(bits[0].copy().set_width(0.25), Tex('\\cdots', font_size=20), bits[-1].copy().set_width(0.25))
        ...
        self.play(sudoku.animate.fade(0.9), FadeIn(bits, lag_ratio=0.001))
        self.play(LaggedStart(ReplacementTransform(bits[0], in_group[0]), *(ReplacementTransform(bs, in_group[1]) for bs in bits[1:-1]), ReplacementTransform(bits[-1], in_group[-1]), Restore(sudoku), lag_ratio=0.01))
        return in_group
...
class AskWhyThatsTrue(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(stds[1].says('How does that work?', mode='maybe'), stds[0].change('erm', look_at=self.screen), stds[2].change('confused', look_at=self.screen), morty.change('hesitant'))
        self.look_at(self.screen)
        self.wait(5)
        mapping = VGroup(VGroup(Text('True').set_color(GREEN), Vector(DOWN), Tex('\\times -1')).arrange(DOWN), VGroup(Text('False').set_color(RED), Vector(DOWN), Tex('\\times +1')).arrange(DOWN))
        for part in mapping:
            tex = part[2]
        ...
        self.play(morty.change('raise_right_hand'), FadeIn(mapping, UP), stds[1].debubble(mode='pondering'), stds[0].change('sassy', mapping), stds[2].change('hesitant', mapping))
        self.wait()
...
class ListOfConfusions(InteractiveScene):
    ...
    def construct(self):
        items = BulletedList('Insufficient detail', 'Bad framing', 'Glossing over linearity', buff=1.0)
        rects = VGroup((SurroundingRectangle(item[1:]) for item in items))
        rects.set_stroke(width=0)
        rects.set_fill(GREY_D, 1)
        self.add(items)
        self.add(rects[1:])
        self.wait()
        self.play(items[0].animate.fade(0.5), rects[1].animate.stretch(0, 0, about_edge=RIGHT))
        self.wait()
        self.play(items[1].animate.fade(0.5), rects[2].animate.stretch(0, 0, about_edge=RIGHT))
        self.wait()

class SolveSHAWord(InteractiveScene):

    def construct(self):
        words = VGroup(TexText('Solve for ${x}$'), Tex('\\text{SHA256}({x}) = 0'))
...
class ThatsOnMe(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(morty.says("That's on me", mode='guilty'), self.change_students('pondering', 'well', 'hesitant', look_at=self.screen))
        self.wait(3)
...
class ShowSuperposition(InteractiveScene):
    ...
    def construct(self):
        ...
        basis_vectors = VGroup((Vector(unit_size * vect, thickness=5) for vect in np.identity(3)))
        basis_vectors.set_submobject_colors_by_gradient(BLUE_D, BLUE_B)
        ...
        basis_labels = VGroup((KetGroup(BitString(n, length=2)).set_height(0.35) for n in range(3)))
        ...
        vect_label = VGroup(DecimalNumber(vect_coords[0], **dec_kw), KetGroup(BitString(0, 2)), DecimalNumber(vect_coords[1], **dec_kw), KetGroup(BitString(1, 2)), DecimalNumber(vect_coords[2], **dec_kw), KetGroup(BitString(2, 2)), DecimalNumber(0, **dec_kw), KetGroup(BitString(3, 2)))
        ...
        self.play(GrowArrow(vector), frame.animate.set_x(1))
        ...
        self.play(FadeIn(col.get_brackets()), TransformFromCopy(vect_label[0::2], col.get_entries(), run_time=2), FadeIn(eq), vect_label.animate.next_to(eq, RIGHT, SMALL_BUFF))
        rects = VGroup((SurroundingRectangle(mob.copy().rotate(90 * DEG, LEFT)).rotate(90 * DEG, RIGHT) for mob in [col, vect_label]))
...
class NorthEastTraveler(InteractiveScene):
    ...
    def construct(self):
        ...
        components = VGroup(Vector(math.sqrt(2) * UP).shift(math.sqrt(2) * RIGHT).set_fill(GREEN), Vector(math.sqrt(2) * RIGHT).set_fill(RED))
        components.shift(vel_vect.get_start() - components[1].get_start())
        labels = VGroup(Text('North').next_to(components[0], RIGHT, buff=-0.05), Text('East').next_to(components[1], DOWN, SMALL_BUFF))
        for component, label in zip(components, labels):
            label.scale(0.75)
            label.match_color(component)
            self.play(GrowArrow(component), FadeIn(label))
        self.wait()
        sum_expr = VGroup(labels[0].copy(), Tex('+', font_size=24), labels[1].copy())
        sum_expr.arrange(RIGHT, buff=SMALL_BUFF)
        sum_expr.next_to(vel_vect.get_end(), UP)
        self.play(TransformFromCopy(labels, sum_expr[0::2]), Write(sum_expr[1]))
        self.wait()
        north_group = VGroup(components[0], labels[0])
        east_group = VGroup(components[1], labels[1])
        self.play(FadeOut(sum_expr), north_group.animate.shift(DR).fade(0.5), east_group.animate.shift(DR).fade(0.5))
        self.wait()
        self.add(travler.copy().fade(0.75))
        t_rot_marks = self.show_90_degree_rotation(travler, 45 * DEG, about_point=vel_vect.get_start())
        self.wait()
        self.play(north_group.animate.set_fill(opacity=1).shift(UR), FadeOut(t_rot_marks))
        n_rot_marks = self.show_90_degree_rotation(components[0], 90 * DEG, about_point=components[0].get_start())
        self.wait()
        self.play(east_group.animate.set_fill(opacity=1).shift(DR + DOWN))
        ...
        self.wait()
        self.play(LaggedStart(*(Rotate(mob, -90 * DEG, about_point=vel_vect.get_start(), run_time=6, rate_func=there_and_back_with_pause) for mob in [travler, *components]), lag_ratio=0.05))

    def get_compass(self):
        spike = Triangle()
        spike.set_shape(0.25, 1)
        spike.move_to(ORIGIN, DOWN)
        spikes = VGroup((spike.copy().rotate(x * TAU / 4, about_point=ORIGIN) for x in range(4)))
    ...
    def show_90_degree_rotation(self, mobject, start_angle, about_point, radius=1.5):
        ...
        self.play(LaggedStart(ShowCreation(arc), FadeIn(label), Rotate(mobject, 90 * DEG, about_point=about_point), lag_ratio=0.5))
...
class ShowLinearityExample(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        kets = VGroup((KetGroup(BitString(n, 2)) for n in range(4)))
        ...
        components = VGroup((DecimalNumber(n, include_sign=True, font_size=42) for n in normalize([1, -2, 3, -4])))
        ...
        self.wait()
        self.play(FadeOutToPoint(in_group.copy(), machine.get_left() + RIGHT, lag_ratio=0.01, run_time=2))
        out_kets = VGroup()
        for ket in kets:
            f_group = Tex('f()', font_size=60)
        ...
        self.play(FadeInFromPoint(out_group, machine.get_right() + LEFT, lag_ratio=0.01, run_time=2))
        self.wait()
        in_rects = VGroup(map(SurroundingRectangle, components))
        out_rects = VGroup(map(SurroundingRectangle, out_components))
        self.play(LaggedStartMap(VFadeInThenOut, in_rects, lag_ratio=0.5), LaggedStartMap(VFadeInThenOut, out_rects, lag_ratio=0.5), run_time=5)
        self.wait()
...
class ZGateExample(InteractiveScene):
    ...
    def construct(self):
        ...
        for gate in z_gates:
            ...
            gate[-1].scale(1.5)
        zero, one = kets = VGroup(KetGroup(Integer(0)), KetGroup(Integer(1)))
        gen_input = VGroup(Tex('x'), zero.copy(), Tex('+'), Tex('y'), one.copy())
        ...
        outputs = VGroup(zero.copy().next_to(z_gates[0], RIGHT), VGroup(Tex('-'), one.copy()).arrange(RIGHT, buff=SMALL_BUFF).next_to(z_gates[1], RIGHT))
        self.play(FadeIn(z_gates[0]))
        self.wait()
        self.play(FadeIn(zero, RIGHT))
        self.play(LaggedStart(FadeOutToPoint(zero.copy(), z_gates[0].get_center(), lag_ratio=0.05), FadeInFromPoint(outputs[0], z_gates[0].get_center(), lag_ratio=0.05), lag_ratio=0.5))
        self.wait()
        self.play(TransformFromCopy(*z_gates[:2]), TransformFromCopy(*kets))
        self.play(LaggedStart(FadeOutToPoint(one.copy(), z_gates[1].get_center(), lag_ratio=0.05), FadeInFromPoint(outputs[1], z_gates[1].get_center(), lag_ratio=0.05), lag_ratio=0.5))
        self.wait()
        self.play(TransformFromCopy(*z_gates[1:3]), TransformFromCopy(kets, gen_input[1::3]), *(FadeIn(gen_input[i]) for i in [0, 2, 3]))
        self.wait()
        rhss = VGroup(Tex('xZ|0\\rangle + y Z|1\\rangle ', t2c={'Z': TEAL}), Tex('x |0\\rangle - y |1\\rangle'))
        rhss[0].next_to(z_gates[2], RIGHT)
        rhss[1].next_to(rhss[0], DOWN, MED_LARGE_BUFF, aligned_edge=LEFT)
        self.play(LaggedStart(FadeOutToPoint(gen_input.copy(), z_gates[2].get_center(), lag_ratio=0.01), FadeInFromPoint(rhss[0], z_gates[2].get_center(), lag_ratio=0.01), lag_ratio=0.5))
        self.wait()
...
class PassSuperpositionIntoVerifier(InteractiveScene):
    ...
    def construct(self):
        ...
        basis_kets = VGroup(KetGroup(Tex('b_0')), KetGroup(Tex('b_1')), Tex('\\vdots'), KetGroup(Tex('b_k')), Tex('\\vdots'), KetGroup(Tex('b_{n-2}')), KetGroup(Tex('b_{n-1}')))
        ...
        components = VGroup(Tex('x_0'), Tex('+x_1'), VectorizedPoint(), Tex('+x_k'), VectorizedPoint(), Tex('+x_{n-2}'), Tex('+x_{n-1}'))
        ...
        self.wait()
        self.play(Transform(bit_ket, basis_kets[3]), basis_label.animate.next_to(basis_kets[3], LEFT))
        self.wait()
        self.play(FadeTransform(basis_label, superposition_word), GrowFromCenter(brace), FadeIn(basis_kets, lag_ratio=0.1, run_time=2), FadeIn(components, lag_ratio=0.1, run_time=2))
        self.remove(bit_ket)
        self.wait()
        out_kets = basis_kets.copy()
        out_Vs = VGroup((Tex('V').set_color(TEAL).next_to(ket, LEFT, SMALL_BUFF) for ket in out_kets))
        ...
        self.play(LaggedStart(FadeOutToPoint(in_group.copy(), machine.get_left() + RIGHT, lag_ratio=0.01), FadeInFromPoint(out_group, machine.get_right() + LEFT, lag_ratio=0.01), lag_ratio=0.5, run_time=2), frame.animate.center(), FadeOut(brace), FadeOut(superposition_word))
        self.wait()
        ...
        for n in [0, 1, -2, -1]:
            ...
            self.play(LaggedStart(FadeOut(last_annotation), FadeIn(annotation), FadeOut(V), comp.animate.shift(V.get_width() * RIGHT)))
        ...
        out_rect = SurroundingRectangle(VGroup(out_comps[sol_index], out_Vs[sol_index], out_kets[sol_index]))
        out_rect.set_stroke(YELLOW)
        flip_word = Text('Flip!')
        flip_word.set_color(YELLOW)
        flip_word.next_to(out_rect, RIGHT)
        new_out_comp = Tex('-x_k')
        new_out_comp.next_to(out_kets[sol_index], LEFT, SMALL_BUFF)
        self.play(FadeIn(out_rect), FadeIn(flip_word), Transform(out_comps[sol_index], new_out_comp), FadeOut(out_Vs[sol_index], scale=0.25))
        self.play(Write(solution_label), GrowArrow(solution_arrow))
        self.wait()
        lines = VGroup((Line(basis_kets[n].get_right(), out_comps[n].get_left(), buff=SMALL_BUFF) for n in range(7)))
        ...
        self.play(Transform(pre_machine, new_machines), LaggedStartMap(ShowCreation, lines))
        self.wait()
        self.play(FadeOut(lines), FadeOut(pre_machine), FadeIn(machine))
        self.wait()
        in_col = TexMatrix(np.array(['x_0', 'x_1', '\\vdots', 'x_k', '\\vdots', 'x_{n-2}', 'x_{n-1}']).reshape(-1, 1))
        out_col = TexMatrix(np.array(['x_0', 'x_1', '\\vdots', '-x_k', '\\vdots', 'x_{n-2}', 'x_{n-1}']).reshape(-1, 1))
        for col, vect in zip([in_col, out_col], [LEFT, RIGHT]):
            col.match_height(basis_kets)
            col.next_to(machine, vect)
        self.play(FadeOut(solution_label), FadeOut(solution_arrow), FadeOut(basis_kets), FadeOut(out_kets), out_rect.animate.surround(out_col.get_entries()[3]).set_stroke(width=1), FadeOut(flip_word), ReplacementTransform(components, in_col.get_entries()), ReplacementTransform(out_comps, out_col.get_entries()), Write(in_col.get_brackets()), Write(out_col.get_brackets()))
        self.wait()
...
class OverOrUnderExplain(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(morty.change('shruggie'), self.change_students('pondering', 'pondering', 'pondering', look_at=self.screen))
        self.wait(2)
        self.play(LaggedStart(stds[2].change('erm', look_at=morty.eyes), morty.change('hesitant', look_at=self.students)))
        self.wait(2)
        self.play(morty.change('raise_right_hand'), self.change_students('hesitant', 'well', 'confused', look_at=3 * UR))
        self.wait(5)
...
class IsThisUseful(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(morty.change('hesitant', self.screen), self.change_students('pondering', 'pondering', 'pondering', look_at=self.screen))
        self.wait(4)
        self.play(stds[1].says('Is this...useful?', mode='confused'), stds[0].change('hesitant', look_at=stds[1].eyes), stds[2].change('well', look_at=stds[1].eyes), morty.change('guilty', look_at=stds[1].eyes))
        self.wait(4)
...
class SudokuBruteForce(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(group)
        self.play(GrowFromCenter(brace), FadeIn(number, lag_ratio=0.1))
        self.wait()
        grover_words = TexText("Using Grover's Algorithm:")
        grover_words.move_to(words, RIGHT)
        grover_steps = TexText('$\\displaystyle \\left\\lceil\\frac{\\pi}{4} 9^{30}\\right\\rceil$ Steps', isolate=['30'])
        ...
        self.play(FadeTransformPieces(words, grover_words), FadeTransform(steps, grover_steps), Transform(brace, new_brace), FadeTransformPieces(number, new_number))
        self.wait()

class ShaInversionCounts(InteractiveScene):

    def construct(self):
        classical = VGroup(get_classical_computer_symbol(height=1), TexText('$2^{256}$ Steps'))
        quantum = VGroup(get_quantum_computer_symbol(height=1), TexText('$\\displaystyle \\left\\lceil \\frac{\\pi}{4} 2^{128} \\right\\rceil$ Steps'))
        ...
        self.wait()
        self.play(FadeTransformPieces(classical.copy(), quantum, lag_ratio=0.0001))
        self.wait()
...
class SkepticalPiCreature(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(randy.says('Quantum Computing will\nchange everything!', mode='surprised'), morty.change('hesitant')))
        self.play(Blink(morty))
        self.wait(2)
        self.play(Blink(randy))
        self.add(Point())
        self.play(morty.says('...will it?', mode='sassy'), randy.change('guilty'))
        self.play(Blink(morty))
        self.wait()
...
class FactoringNumbers(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(LaggedStart(FadeOutToPoint(product.copy(), icon.get_center(), lag_ratio=0.02, path_arc=45 * DEG), FadeInFromPoint(factor_group, icon.get_center(), lag_ratio=0.02, path_arc=45 * DEG), lag_ratio=0.3))
        self.wait()
...
class FourBitAdder(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(Write(circuit, lag_ratio=0.01, run_time=3))
        self.wait()
