...
def get_quantum_computer_symbol(height=2, color=GREY_B, symbol_tex='|Q\\rangle', symbol_color=TEAL):
...
def get_classical_computer_symbol(height=2, color=GREY_B, symbol_tex='\\mathcal{C}', symbol_color=YELLOW):
...
class Superposition(Group):
    ...
    def __init__(self, pieces, offset_multiple=0.2, max_rot_vel=3, glow_color=TEAL, glow_stroke_range=(1, 22, 4), glow_stroke_opacity=0.05):
        self.pieces = pieces
...
class WriteQuantumComputingTitle(InteractiveScene):
    ...
    def construct(self):
        ...
        title.move_to(UP)
        subtitle = Tex('\\frac{1}{\\sqrt{2}}\\big(|0\\rangle + |1\\rangle \\big)', font_size=90)
...
class MentionQuiz(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        to_confusion = VGroup(Vector(1.5 * RIGHT, thickness=6), Text('???', font_size=90).space_out_submobjects(1.5))
        ...
        self.play(morty.change('angry', self.screen), self.change_students('thinking', 'tease', 'confused', look_at=self.screen))
        self.wait(2)
        self.play(morty.change('raise_right_hand', to_confusion), self.change_students('erm', 'guilty', 'hesitant'), FadeInFromPoint(to_confusion, morty.get_corner(UL), lag_ratio=0.1))
        self.wait()
...
class QuizMarks(InteractiveScene):
    ...
    def construct(self):
        marks = VGroup(Exmark().set_color(RED), Exmark().set_color(RED), Checkmark().set_color(GREEN), Exmark().set_color(RED))
...
class SimpleScreenReference(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        self.play(morty.change('hesitant'), self.change_students('erm', 'concentrating', 'sassy', look_at=self.screen))
        self.wait(5)
...
class StudentsCommentOnQuiz(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(morty.change('raise_right_hand'), FadeIn(qc, UP), self.change_students('confused', 'horrified', 'angry'), LaggedStartMap(FadeIn, q_marks, shift=0.25 * UP))
        self.play(VShowPassingFlash(qc_outline, time_width=2, run_time=3), Blink(morty))
        self.wait(4)
        std = self.students[2]
        machine = get_blackbox_machine(height=1.0)
        machine.next_to(std.get_corner(UR), UP)
        arrow = Tex('\\updownarrow').set_height(1)
        arrow.next_to(machine, UP)
        self.play(LaggedStart(FadeOut(q_marks[:2], lag_ratio=0.1), self.change_students('pondering', 'erm', 'raise_right_hand'), morty.change('tease'), qc.animate.next_to(arrow, UP), FadeIn(machine), q_marks[2].animate.next_to(arrow, LEFT), Write(arrow)))
        self.wait(5)
        bubble = ThoughtBubble().pin_to(std)
        self.play(self.change_students('pondering', 'pondering', 'thinking', look_at=self.screen, lag_ratio=0.1), FadeIn(bubble, lag_ratio=0.2), LaggedStartMap(FadeOut, VGroup(machine, arrow, q_marks[2], qc), shift=LEFT, lag_ratio=0.1, run_time=1))
        self.wait(3)
...
class Wrong(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(TransformMatchingStrings(old_bubble.content, new_bubble.content, key_map={'Wrong': 'wrong'}, run_time=1), Transform(old_bubble[0], new_bubble[0]), morty.change('tease'))
        self.wait()
...
class WriteCheckMark(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(Write(check), Flash(check.get_left() + 1.0 * LEFT, flash_radius=0.8, line_length=0.2, color=GREEN))
        self.wait()
...
class TwoThirdsDivision(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(rects)
        self.wait()
        self.play(rects[0].animate.stretch_to_fit_width(8, about_edge=LEFT), rects[1].animate.stretch_to_fit_width(4, about_edge=RIGHT), run_time=2)
        self.wait()
...
class MoreDimensionsNote(InteractiveScene):
    ...
    def construct(self):
        words = TexText('(Except $2^k$ dimensions instead of 3)')
...
class QuestionsOnTheStateVector(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        morty = self.teacher
        stds = self.students
        self.play(morty.change('hesitant'), self.change_students('pleading', 'confused', 'erm', look_at=self.screen))
        self.wait(3)
        ...
        self.wait(3)
        expr = Tex('\\mathds{R}^n \\text{ vs. } \\mathds{C}^n')
        expr.move_to(self.hold_up_spot, DOWN)
        self.play(morty.change('raise_right_hand'), FadeIn(expr, UP), stds[0].debubble(mode='sassy'), stds[1].debubble(mode='erm'), stds[2].change('confused', look_at=expr))
        self.wait(3)
...
class MagnifyingGlassOverComputer(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(comp, glass)
        self.play(glass.animate.shift(-glass[0].get_center()).set_anim_args(path_arc=-60 * DEG), rate_func=there_and_back_with_pause, run_time=6)
        self.wait()

class SimpleSampleValue(InteractiveScene):

    def construct(self):
        value = Tex('|0011\\rangle', font_size=60)
...
class ReferenceQubitComplexity(TeacherStudentsScene):
    ...
    def construct(self):
        for pi in self.pi_creatures:
            pi.body.insert_n_curves(1000)
        expr = Tex('\\mathds{R}^2 \\text{ vs. } \\mathds{C}^2', font_size=60)
        expr.move_to(self.hold_up_spot, DOWN)
        self.play(self.teacher.change('raise_right_hand'), self.change_students('erm', 'confused', 'tease', look_at=expr), FadeIn(expr, UP))
        self.wait(2)
        self.play(self.teacher.change('tease'), FadeOut(expr, shift=3 * RIGHT, path_arc=-90 * DEG), self.change_students('pondering', 'pondering', 'pondering', look_at=self.screen))
        self.wait(4)
...
class ConfusionAtPresmises(TeacherStudentsScene):
    ...
    def construct(self):
        morty = self.teacher
        stds = self.students
        self.play(morty.change('guilty'), stds[2].says('Okay...\nbut why?', mode='sassy', bubble_direction=LEFT), stds[1].change('confused', self.screen), stds[0].change('maybe', self.screen))
        self.wait(2)
...
class BitExamples(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(Write(circuit, lag_ratio=0.01), Write(switch, lag_ratio=0.01), FadeIn(coins))
        switch.flip(RIGHT)
        self.play(Rotate(coins, PI, axis=RIGHT), op_tracker.animate.set_value(1))
        self.wait()
...
class KetDefinition(InteractiveScene):
    ...
    def construct(self):
        ...
        ket_name = TexText("``ket''", font_size=60)
        ket_name.next_to(ket, UP, buff=MED_LARGE_BUFF)
        self.add(ket)
        self.play(Write(ket_name))
        self.wait()
        arrow = Tex('\\longrightarrow', font_size=90)
        ...
        self.play(ket.animate.next_to(arrow, LEFT, MED_LARGE_BUFF), MaintainPositionRelativeTo(ket_name, ket), Write(arrow))
        self.play(GrowArrow(vector))
        self.play(GrowFromCenter(brace), FadeIn(brace_label))
        self.wait()
        examples = VGroup((Tex(tex) for tex in ['0', '1', '+z', '-z', '\\updownarrow', 'E', '\\text{Dead}', '\\text{Alive}', '\\psi']))
        last = dots
        for example in examples:
            example.scale(2)
            example.set_max_width(dots.get_width())
            example.move_to(last)
            self.play(FadeOut(last, 0.5 * UP), FadeIn(example, 0.5 * UP), rate_func=linear, run_time=0.5)
            last = example
...
class ClassicalGates(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(Write, gates, lag_ratio=0.5), LaggedStartMap(Write, gate_names, lag_ratio=0.5))
        ...
        self.play(LaggedStart(*(FadeOut(in_bit.copy(), RIGHT) for in_bit in inputs), *(FadeIn(out_bit, 0.5 * RIGHT) for out_bit in outputs), lag_ratio=0.2))
        self.add(inputs, outputs)
        gate_groups = VGroup((VGroup(gate, name, in_bits, out_bits) for gate, name, in_bits, out_bits in zip(gates, gate_names, inputs, outputs)))
        ...
        self.play(gate_groups.animate.scale(0.35).space_out_submobjects(0.9).next_to(title, DOWN, MED_LARGE_BUFF), Write(circuit, run_time=3))
        self.wait()
...
class ShowQubitThroughTwoHadamardGates(InteractiveScene):
    ...
    def construct(self):
        ...
        symbols = VGroup(Tex('|0\\rangle', font_size=60), Tex('\\frac{1}{\\sqrt{2}}\\left(|0\\rangle + |1\\rangle\\right)'), Tex('|0\\rangle', font_size=60))
        ...
        mid_sym_lines = VGroup((Line(line.pfp(0.5 + 0.01 * u), mid_sym_rect.get_corner(DOWN + u * RIGHT)) for u in [-1, 1]))
        ...
        self.play(ShowCreation(mid_sym_lines, lag_ratio=0), GrowFromPoint(mid_sym_rect, dot.get_center()), GrowFromPoint(symbols[1], dot.get_center()))
        self.wait()
...
class ReferencePreview(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for pi in self.pi_creatures:
            pi.body.insert_n_curves(1000)
        self.play(morty.says('First, a preview'), self.change_students('happy', 'tease', 'happy', look_at=morty.eyes))
        self.wait(4)
        self.play(morty.debubble(mode='raise_right_hand'), self.change_students('pondering', 'thinking', 'pondering', look_at=2 * UR))
        self.look_at(3 * UR)
        self.wait(4)
        stds = self.students
        self.play(stds[2].says('Why can you flip the\nkey sign like that?', mode='raise_left_hand', look_at=self.teacher.eyes), stds[1].change('sassy', look_at=3 * UR), stds[0].change('maybe', look_at=3 * UR), morty.change('tease'))
        self.wait(5)
...
class GroverPreviewBox(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.add(rect)
        self.play(morty.change('raise_right_hand', rect), self.change_students('pondering', 'pondering', 'pondering', look_at=rect))
        self.wait(4)
...
class SimpleMagnifyingGlass(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(glass)
        self.wait()
        self.play(MoveToTarget(glass, path_arc=-45 * DEG), run_time=6, rate_func=there_and_back_with_pause)
        self.wait()

class ShowAbstractionArrows(InteractiveScene):

    def construct(self):
        arrows = VGroup((Arrow(point + 4 * LEFT, ORIGIN + 0.35 * point, buff=1.0, thickness=5) for point in np.linspace(2.5 * DOWN, 2.5 * UP, 3)))
...
class StareAtPicture(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for pi in self.pi_creatures:
            pi.body.insert_n_curves(100)
        self.play(morty.change('raise_right_hand', self.screen), self.change_students('pondering', 'pondering', 'pondering', look_at=self.screen))
        self.wait()
        self.play(morty.change('speaking', look_at=self.students), self.change_students('tease', 'well', 'coin_flip_2'))
        self.wait(5)
...
class TryingToDescribeComputing(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(randy.change('raise_right_hand', laptop), FadeIn(laptop, UP), buddy.change('erm', laptop), lag_ratio=0.5))
        self.play(Blink(buddy))
        self.wait()
        self.play(LaggedStart(randy.change('well', buddy.eyes), FadeIn(chip, UP), laptop.animate.shift(1.5 * UP), buddy.change('confused'), lag_ratio=0.25))
        self.play(Blink(randy))
        factors = Tex('91 = 7 \\times 13')
        factors.next_to(randy, UL, MED_LARGE_BUFF)
        factors.shift_onto_screen(buff=LARGE_BUFF)
        self.play(randy.change('raise_left_hand', factors), FadeInFromPoint(factors, chip.get_center(), lag_ratio=0.05))
        self.play(Blink(buddy))
        self.wait(2)
        c_label = chip[1]
        chip.remove(c_label)
        seven = factors['7'][0].copy()
        product = factors['7 \\times 13'][0].copy()
        self.play(randy.change('raise_right_hand', chip), chip.animate.scale(2, about_edge=DOWN), FadeOut(c_label, 0.5 * UP), FadeOut(laptop, UP), buddy.change('pondering', chip))
        self.play(seven.animate.move_to(chip).scale(1.5))
        self.play(Blink(buddy))
        self.wait()
        product.move_to(chip)
        self.play(ReplacementTransform(seven, product[0]), Write(product[1:]), buddy.change('hesitant', chip))
        result = Tex('91')
        result.scale(1.5)
        result.move_to(chip)
        self.play(TransformFromCopy(product, result, lag_ratio=0.2), product.animate.set_opacity(0.25))
        self.play(Blink(randy))
        self.wait()
        ...
        self.play(randy.change('dance_3', gates), Write(gates, run_time=2), FadeOut(factors, DOWN), buddy.change('awe', gates))
        self.play(Blink(buddy))
        self.play(Blink(randy))
        self.wait()
        self.play(randy.change('tease'), FadeOut(gates, 3 * LEFT, rate_func=running_start, path_arc=30 * DEG))
        self.wait()

class ProbForMillionDim(InteractiveScene):

    def construct(self):
        context = VGroup(Tex('N = 2^{20}'), TexText('\\# Reps: 804'))
        context.arrange(DOWN, aligned_edge=LEFT)
        context.to_corner(UL)
        theta = math.asin(2 ** (-10))
        prob = math.sin((2 * 804 + 1) * theta) ** 2
        chance_lhs = Tex('P(k) = ', t2c={'k': YELLOW})
        ...
        equation = brace.get_tex('\\sin((2 \\cdot 804 + 1) \\theta)^2')
        equation.set_color(GREY_C)
        equation.scale(0.7, about_edge=UP)
        self.play(Write(chance))
        self.play(GrowFromCenter(brace), FadeIn(equation))
        self.wait()
...
class AskAreYouSure(InteractiveScene):
    ...
    def construct(self):
        ...
        number = Integer(random.randint(0, int(1000000.0)))
        ...
        self.play(randy.debubble('pondering', machine), FadeOut(answer[0]), number.animate.scale(1.5).next_to(machine, LEFT).set_anim_args(path_arc=-45 * DEG), FadeIn(machine))
        self.add(number.copy().set_opacity(0.5))
        self.play(FadeOutToPoint(number, machine.get_center(), path_arc=-45 * DEG, lag_ratio=0.01))
        self.play(FadeIn(output, 2 * RIGHT), randy.change('well', output))
        self.play(Blink(randy))
        self.wait(2)
...
class WrapUpList(InteractiveScene):
    ...
    def construct(self):
        morty = Mortimer()
        morty.to_corner(DR)
        items = BulletedList('The Lie', 'Why $\\sqrt{\\quad}$', 'A suprising analogy', buff=0.75)
        ...
        self.play(morty.change('raise_right_hand', items), FadeInFromPoint(items[0], morty.get_corner(UL), lag_ratio=0.01))
        self.play(Blink(morty))
        self.wait()
        self.play(morty.change('pondering', items[1]), Write(items[1]))
        self.wait()
        self.play(morty.change('tease', 5 * UR), FadeIn(items[2], DOWN))
        self.play(Blink(morty))
        self.wait()
        for n, mode in enumerate(['guilty', 'tease', 'surprised']):
            self.play(items.animate.fade_all_but(n, scale_factor=0.5), morty.change(mode, items[n]))
            self.play(Blink(morty))
            self.wait()
...
class OneWordSummary(InteractiveScene):
    ...
    def construct(self):
        ...
        prompt.move_to(2 * UP)
        words = VGroup(Text('Parallelism').set_color(TEAL), Text('Pythagoras').set_color(YELLOW))
        words.scale(2)
        ...
        self.play(FadeIn(words[1], 0.5 * DOWN), words[0].animate.scale(0.75).shift(2 * DOWN).set_opacity(0.25), strike.animate.scale(0.75).shift(2 * DOWN))
        self.wait()
...
class PythagoreanIntuition(InteractiveScene):
    ...
    def construct(self):
        ...
        side_lines = VGroup(Line(axes.get_origin(), axes.c2p(1, 0, 0)), Line(axes.c2p(1, 0, 0), axes.c2p(1, 1, 0)), Line(axes.c2p(1, 1, 0), axes.c2p(1, 1, 1)))
        side_lines.set_stroke(RED, 4)
        ones = VGroup((Tex('1', font_size=36).next_to(line, vect, SMALL_BUFF) for line, vect in zip(side_lines, [DOWN, RIGHT, RIGHT])))
        ...
        for line, one in zip(side_lines[:2], ones):
            self.play(ShowCreation(line), FadeIn(one, 0.5 * line.get_vector()), dot.animate.move_to(line.get_end()))
        self.wait()
        ...
        sqrts = VGroup(Tex('\\sqrt{1^2 + 1^2}', font_size=24), Tex('\\sqrt{2}', font_size=36))
        for sqrt in sqrts:
            sqrt.next_to(diag.pfp(0.5), UL, buff=0.05)
        self.play(ShowCreation(diag), dot.animate.move_to(square.get_corner(UR)), TransformFromCopy(ones[:2], sqrts[0]['1'], time_span=(1, 2)), *(Write(sqrts[0][tex], time_span=(1, 2)) for tex in ['\\sqrt', '+', '2']), run_time=2)
        self.wait()
        ...
        self.play(FadeIn(axes_group), ShowCreation(cube, lag_ratio=0.1, time_span=(0.5, 2.0)), frame.animate.reorient(-16, 68, 0, (0.45, 0.98, 1.05), 4.36), run_time=2)
        frame.add_ambient_rotation(DEG)
        line = side_lines[2]
        self.play(ShowCreation(line), FadeIn(ones[2], 0.5 * line.get_vector()), dot.animate.move_to(line.get_end()))
        self.wait(2)
        diag3 = Line(axes.c2p(0, 0, 0), axes.c2p(1, 1, 1))
        diag3.set_stroke(YELLOW, 3)
        new_sqrts = VGroup(Tex('\\sqrt{\\sqrt{2}^2 + 1^2}', font_size=24), Tex('\\sqrt{3}', font_size=36))
        ...
        self.play(TransformFromCopy(ones[2], new_sqrts[0]['1'][0], time_span=(1, 2)), TransformFromCopy(sqrts[1], new_sqrts[0]['\\sqrt{2}'][0], time_span=(1, 2)), *(Write(new_sqrts[0][tex], time_span=(1, 2)) for tex in ['\\sqrt', '+', '2']), run_time=2)
        self.wait()
        self.play(TransformMatchingTex(*new_sqrts, key_map={'\\sqrt{2}^2 + 1^2': '3'}, match_animation=FadeTransform, run_time=1))
        self.wait(6)
        symbols = VGroup(ones, sqrts[1], new_sqrts[1])
        wireframe = VGroup(cube, side_lines, diag, diag3)
        basis_vectors = VGroup((Vector(2 * v, thickness=4, fill_color=color) for v, color in zip(np.identity(3), [BLUE_E, BLUE_D, BLUE_C])))
        basis_vectors.set_z_index(1)
        for vector in basis_vectors:
            vector.always.set_perpendicular_to_camera(frame)
        obs_labels = VGroup((KetGroup(Text(f'Obs {n}', font_size=30), height_scale_factor=1.5, buff=0.05) for n in range(1, 4)))
        ...
        self.play(LaggedStartMap(GrowArrow, basis_vectors, lag_ratio=0.25), FadeOut(symbols), FadeOut(dot), wireframe.animate.set_stroke(opacity=0.2), frame.animate.reorient(13, 67, 0, (-0.04, 0.76, 0.87), 4.84))
        ...
        self.play(Write(new_cube, stroke_width=5, lag_ratio=0.1, run_time=3))
        self.play(FadeOut(new_cube))
        self.wait(4)
        diag_vects = VGroup((Vector(2 * normalize(np.array(tup))) for tup in it.product(*3 * [[-1, 0, 1]]) if get_norm(tup) > 0))
        for vect in diag_vects:
            vect.set_perpendicular_to_camera(frame)
            color = random_bright_color(hue_range=(0.4, 0.5), saturation_range=(0.5, 0.7), luminance_range=(0.5, 0.6))
            vect.set_color(color)
        self.play(FadeOut(obs_labels), LaggedStartMap(GrowArrow, diag_vects), frame.animate.reorient(-19, 61, 0, (-0.17, 0.22, -0.25), 6.96), run_time=3)
        self.wait(10)
...
class GrainOfSalt(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for pi in self.pi_creatures:
            pi.body.insert_n_curves(100)
        self.play(morty.change('coin_flip_2'), self.change_students('hesitant', 'sassy', 'tease', look_at=self.screen))
        self.wait(2)
        self.play(morty.says('Take this with\na grain of salt', mode='hesitant'), self.change_students('pondering', 'pondering', 'pondering', look_at=morty.eyes))
        self.wait(3)
        self.play(morty.debubble(mode='raise_right_hand', look_at=3 * UP), self.change_students('hesitant', 'erm', 'hesitant', look_at=2 * UP))
        self.wait(4)
        self.play(morty.change('tease'), self.change_students('pondering', 'thinking', 'thinking', look_at=2 * UP))
        self.wait(5)
...
class ConstructQRCode2(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(frame.animate.to_default_state(), ShowCreation(union_pieces, lag_ratio=0), FadeIn(background, time_span=(20, 25)), Write(squares, stroke_color=BLUE, stroke_width=1, time_span=(12, 25)), Write(corner_pieces, time_span=(20, 25)), run_time=25)
        self.play(FadeOut(union_pieces), squares.animate.set_fill(BLACK, 1))
        squares.shuffle()
        self.play(LaggedStart(*(Rotate(square, 90 * DEG) for square in squares), lag_ratio=0.02, run_time=10))
        return
        squares.save_state()
        ...
        self.play(Restore(background), Restore(squares, lag_ratio=0.1), Write(corner_pieces, time_span=(18, 23), lag_ratio=0.25), run_time=35)