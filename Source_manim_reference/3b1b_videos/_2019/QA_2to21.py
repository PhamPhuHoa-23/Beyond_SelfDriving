...
class MathematicianPlusX(Scene):
    ...
    def construct(self):
        text = OldTexText('Side note:\\\\', "``The Mathematician + X''\\\\", 'would make a great band name.')
...
class NoClearCutPath(Scene):
    ...
    def construct(self):
        ...
        path1.append_points([ORIGIN, 2.5 * RIGHT, 2.5 * RIGHT + 3 * UP, 5 * RIGHT + 3 * UP])
        path2 = path1.copy()
        path2.rotate(PI, axis=RIGHT, about_point=ORIGIN)
        paths = VGroup(path1, path2)
        paths.to_edge(LEFT)
        paths.set_stroke(WHITE, 2)
        labels = VGroup(OldTexText('Pure mathematicians'), OldTexText('Applied mathematicians'))
        ...
        for x in range(n_animations):
            ...
            anim = Succession(MoveAlongPath(dot, path, run_time=4, rate_func=lambda t: smooth(t, 2)), FadeOut(dot))
        ...
        alt_dot_anim = MoveAlongPath(alt_dot, alt_path, run_time=10, rate_func=linear)
        self.add(paths)
        self.add(labels)
        self.play(LaggedStart(*animations, run_time=10, lag_ratio=1 / n_animations), ShowCreation(dashed_path, rate_func=linear, run_time=10), alt_dot_anim)
        self.wait()
...
class Cumulative(Scene):
    ...
    def construct(self):
        ...
        rects = VGroup(*[Rectangle(height=0.3, width=4, fill_color=random.choice(colors), fill_opacity=1, stroke_color=WHITE, stroke_width=1) for i, color in zip(range(20), it.cycle(colors))])
        rects.arrange(UP, buff=0)
        check = OldTex('\\checkmark').set_color(GREEN)
        cross = OldTex('\\times').set_color(RED)
        checks, crosses = [VGroup(*[mob.copy().next_to(rect, RIGHT, SMALL_BUFF) for rect in rects]) for mob in [check, cross]]
        rects.set_fill(opacity=0)
        self.add(rects)
        for i in range(7):
            self.play(rects[i].set_fill, None, 1.0, Write(checks[i]), run_time=0.5)
        self.play(FadeOut(rects[7], 2 * LEFT))
        self.play(LaggedStartMap(Write, crosses[8:]), LaggedStartMap(ApplyMethod, rects[8:], lambda m: (m.set_fill, None, 0.2)))
        self.wait()
        rects[7].set_opacity(1)
        self.play(FadeIn(rects[7], 2 * LEFT), FadeIn(checks[7], 2 * LEFT))
        self.play(LaggedStartMap(ApplyMethod, rects[8:], lambda m: (m.set_fill, None, 1.0)), LaggedStart(*[ReplacementTransform(cross, check) for cross, check in zip(crosses[8:], checks[8:])]))
        self.wait()
