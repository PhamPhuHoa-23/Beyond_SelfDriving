...
def read_in_primes(max_N=None):
    if max_N is None:
        max_N = int(10000000.0)
    if max_N < 100000.0:
        file = 'primes_1e5.json'
    elif max_N < 1000000.0:
...
class SpiralScene(Scene):
    CONFIG = {'axes_config': {'axis_config': {'stroke_width': 1.5}}, 'default_dot_color': TEAL, 'p_spiral_width': 6}
    ...
    def get_v_spiral(self, sequence, axes=None, box_width=None):
        ...
        if box_width is None:
            ...
            box_width = max(0.2 / (-np.log10(unit) + 1), 0.02)
        return VGroup(*[Square(side_length=box_width, fill_color=self.default_dot_color, fill_opacity=1, stroke_width=0).move_to(self.get_polar_point(n, n, axes)) for n in sequence])
    ...
    def set_scale(self, scale, axes=None, spiral=None, to_shrink=None, min_box_width=0.05, target_p_spiral_width=None, added_anims=[], run_time=3):
        if axes is None:
            axes = self.axes
        ...
        if run_time == 0:
            ...
            self.play(*anims, run_time=run_time, rate_func=lambda t: interpolate(smooth(t), smooth(t) ** sf ** 0.5, t))
    ...
    def get_labels(self, sequence, scale_func=np.sqrt):
        ...
        for n in sequence:
            ...
            label.next_to(self.get_polar_point(n, n), UP, buff=0.5 * label.get_height())
            labels.add(label)
        return labels
...
class AltTitle(Scene):
    ...
    def construct(self):
        ...
        self.add(image, rect)
        for word in title:
            self.play(FadeIn(word, run_time=0.05 * len(word), lag_ratio=0.4))
        self.wait()
...
class MathExchangeNames(Scene):
    ...
    def construct(self):
        names = VGroup(OldTexText('dwymark'), OldTexText('Greg Martin'))
...
class PrimesAndPi(Scene):
    ...
    def show_primes(self):
        n_rows = 10
        n_cols = 10
        matrix = IntegerMatrix([[n_cols * x + y for y in range(n_cols)] for x in range(n_rows)])
        numbers = matrix.get_entries()
        primes = VGroup(*filter(lambda m: is_prime(m.get_value()), numbers))
        non_primes = VGroup(*filter(lambda m: not is_prime(m.get_value()), numbers))
        self.add(numbers)
        self.play(LaggedStart(*[ApplyFunction(lambda m: m.set_color(TEAL).scale(1.2), prime) for prime in primes]), non_primes.set_opacity, 0.25, run_time=2)
        self.wait()
        self.numbers = numbers

    def show_rational_approximations(self):
        numbers = self.numbers
        approxs = OldTex('{22 \\over 7} &=', '{:.12}\\dots\\\\'.format(22 / 7), '{355 \\over 113} &=', '{:.12}\\dots\\\\'.format(355 / 113), '\\pi &=', '{:.12}\\dots\\\\'.format(PI))
        ...
        self.play(LaggedStartMap(FadeOutAndShiftDown, numbers), LaggedStartMap(FadeIn, approxs), FadeIn(randy))
        self.play(Blink(randy))
        self.play(randy.change, 'pondering', UR)
        self.wait()

class RefresherOnPolarCoordinates(Scene):
    CONFIG = {'x_color': GREEN, 'y_color': RED, 'r_color': YELLOW, 'theta_color': LIGHT_PINK}
    ...
    def show_xy_coordinates(self):
        ...
        self.play(ShowCreation(x_line), ChangeDecimalToValue(x_coord, x), UpdateFromFunc(dot, lambda d: d.move_to(x_line.get_end())), run_time=2)
        self.play(ShowCreation(y_line), ChangeDecimalToValue(y_coord, y), UpdateFromFunc(dot, lambda d: d.move_to(y_line.get_end())), run_time=2)
        self.wait()
        self.xy_coord_mobjects = VGroup(x_line, y_line, coord_label, x_brace, y_brace)
        self.plane = plane
        self.dot = dot
    ...
    def transition_to_polar_grid(self):
        ...
        self.play(FadeOut(self.xy_coord_mobjects), FadeOut(self.plane), ShowCreation(self.polar_grid, run_time=2))
        self.wait()
    ...
    def show_polar_coordinates(self):
        ...
        theta_coord.add_updater(lambda d: d.set_value(angle_of_vector(dot.get_center()) % TAU))
        coord_label[-1].add_updater(lambda m: m.next_to(theta_coord, RIGHT, SMALL_BUFF))
        non_coord_parts = VGroup(*[part for part in coord_label if part not in [r_coord, theta_coord]])
        r_label = OldTex('r')
        ...
        self.play(ShowCreation(r_line, run_time=2), ChangeDecimalToValue(r_coord_copy, r_value, run_time=2), VFadeIn(r_coord_copy, run_time=0.5))
        r_coord.set_value(r_value)
        self.add(non_coord_parts, r_coord_copy)
        self.play(FadeIn(non_coord_parts), ReplacementTransform(r_coord_copy, r_coord), FadeInFromDown(r_label))
        self.wait()
        degree_label.next_to(arc.get_start(), UR, SMALL_BUFF)
        line = r_line.copy()
        line.rotate(-theta_value, about_point=ORIGIN)
        line.set_color(theta_color)
        self.play(ShowCreation(arc), Rotate(line, theta_value, about_point=ORIGIN), VFadeInThenOut(line), ChangeDecimalToValue(degree_label, theta_value / DEGREES))
        self.play(degree_label.scale, 0.9, degree_label.move_to, theta_coord, FadeInFromDown(theta_label))
        self.wait()
        ...
        radians_word.add_updater(lambda m: m.next_to(theta_label, RIGHT, aligned_edge=DOWN))
        self.play(ShowCreation(degree_cross))
        self.play(FadeOut(VGroup(degree_label, degree_cross), DOWN), FadeIn(theta_coord))
        ...
        for angle in [PI - theta_value, PI - 0.001, -TAU + 0.002]:
            self.play(Rotate(dot, angle, about_point=ORIGIN), run_time=3)
            self.wait()
        self.play(FadeOut(coord_label), FadeOut(r_label), FadeOut(theta_label), FadeOut(radians_word), FadeOut(r_line), FadeOut(arc), FadeOut(dot))
        self.dot = dot
        self.r_line = r_line
        self.arc = arc
    ...
    def show_all_nn_tuples(self):
        ...
        self.play(ApplyMethod(frame.scale, 2), LaggedStartMap(FadeInFromDown, pp_points), run_time=2)
        self.wait()
        ...
        self.wait()
        self.play(ReplacementTransform(Line(*r_line.get_start_and_end()), arc), ReplacementTransform(one_r_rect, one_theta_rect))
        self.add(arc, one, one_theta_rect)
        self.play(FadeOut(one_theta_rect))
        self.wait()
        for n in [2, 3, 4]:
            self.play(Rotate(dot, 1, about_point=ORIGIN), theta_tracker.set_value, n)
            self.wait()
            self.play(dot.move_to, self.get_polar_point(n, n))
            self.wait()
        big_anim = Succession(*3 * [Animation(Mobject())], *it.chain(*[[AnimationGroup(Rotate(dot, 1, about_point=ORIGIN), ApplyMethod(theta_tracker.set_value, n)), ApplyMethod(dot.move_to, self.get_polar_point(n, n))] for n in [5, 6, 7, 8, 9]]))
        spiral = ParametricCurve(lambda t: self.get_polar_point(t, t), t_min=0, t_max=25, stroke_width=1.5)
        self.polar_grid.generate_target()
        for mob in self.polar_grid:
            if not isinstance(mob[0], Integer):
                mob.set_stroke(width=1)
        self.play(frame.scale, 3, big_anim, run_time=10)
        self.play(ShowCreation(spiral, run_time=4), FadeOut(r_line), FadeOut(arc), FadeOut(dot))
        self.wait()

    def get_nn_point(self, n):
        point = self.get_polar_point(n, n)
        dot = Dot(point)
        coord_label = self.get_coord_label(n, n, include_background_rectangle=False, num_decimal_places=0)
    ...
    def get_polar_grid(self, radius=25):
        plane = self.plane
        axes = VGroup(Line(radius * DOWN, radius * UP), Line(radius * LEFT, radius * RIGHT))
        axes.set_stroke(width=2)
        circles = VGroup(*[Circle(color=BLUE, stroke_width=1, radius=r) for r in range(1, int(radius))])
        rays = VGroup(*[Line(ORIGIN, radius * RIGHT, color=BLUE, stroke_width=1).rotate(angle, about_point=ORIGIN) for angle in np.arange(0, TAU, TAU / 16)])
        labels = VGroup(*[Integer(n).scale(0.5).next_to(plane.c2p(n, 0), DR, SMALL_BUFF) for n in range(1, int(radius))])
        return VGroup(circles, rays, labels, axes)
    ...
    def get_arc(self, theta, r=1, color=None):
        ...
        return ParametricCurve(lambda t: self.get_polar_point(1 + 0.025 * t, t), t_min=0, t_max=theta, dt=0.25, color=color, stroke_width=3)
...
class IntroducePolarPlot(RefresherOnPolarCoordinates):
    ...
    def construct(self):
        ...
        title.to_edge(UP)
        self.add(grid, title)
        self.play(ShowCreation(grid, lag_ratio=0.1), run_time=3)

class ReplacePolarCoordinatesWithPrimes(RefresherOnPolarCoordinates):

    def construct(self):
        coords, p_coords = [self.get_coord_label(*pair, x_color=self.r_color, y_color=self.theta_color).scale(2) for pair in [('r', '\\theta'), ('p', 'p')]]
        ...
        arrows = VGroup(*[Arrow(some_prime.get_top(), coord.get_bottom(), stroke_width=5, tip_length=0.4) for coord in [p_coords.x_coord, p_coords.y_coord]])
        equals = OldTex('=')
        equals.next_to(p_coords, LEFT)
        self.add(coords)
        self.wait()
        self.play(coords.next_to, equals, LEFT, FadeIn(equals), FadeIn(p_coords))
        self.play(FadeInFromDown(some_prime), ShowCreation(arrows))
        self.wait()

class IntroducePrimePatterns(SpiralScene):
    CONFIG = {'small_n_primes': 25000, 'big_n_primes': 1000000, 'axes_config': {'x_min': -25, 'x_max': 25, 'y_min': -25, 'y_max': 25}, 'spiral_scale': 3000.0, 'ray_scale': 100000.0}
    ...
    def slowly_zoom_out(self):
        ...
        self.wait()
        self.set_scale(self.spiral_scale, spiral=prime_spiral, target_p_spiral_width=8, run_time=zoom_time)
        self.wait()
        self.remove(prime_spiral)
        prime_spiral = self.get_prime_p_spiral(self.big_n_primes)
        prime_spiral.set_stroke_width(8)
        self.set_scale(self.ray_scale, spiral=prime_spiral, target_p_spiral_width=4, run_time=zoom_time)
        self.wait()
    ...
    def show_clumps_of_four(self):
        ...
        self.play(FadeOut(line_groups[4]), LaggedStartMap(VFadeInThenOut, line_groups[4:], lag_ratio=0.5, run_time=5))
        self.wait()

    def get_arithmetic_sequence_line(self, N, r, max_val, skip_factor=5):
        line = VMobject()
        line.set_points_smoothly([self.get_polar_point(x, x) for x in range(r, max_val, skip_factor * N)])
        return line
...
class AskWhat(TeacherStudentsScene):
    ...
    def construct(self):
        screen = self.screen
        self.student_says("I'm sorry,\\\\what?!?", target_mode='angry', look_at=screen, index=2, added_anims=[self.teacher.change, 'happy', screen, self.students[0].change, 'confused', screen, self.students[1].change, 'confused', screen])
        self.wait(3)

class CountSpirals(IntroducePrimePatterns):
    CONFIG = {'count_sound': 'pen_click.wav'}

    def construct(self):
        prime_spiral = self.get_prime_p_spiral(self.small_n_primes)
        self.add(prime_spiral)
        self.set_scale(self.spiral_scale, spiral=prime_spiral, run_time=0)
        spiral_lines = self.get_all_primitive_arithmetic_lines(44, self.small_n_primes, INV_7_MOD_44)
        ...
        for n, spiral in zip(it.count(1), spiral_lines):
            ...
            counts.add(count)
        run_time = 3
        self.play(ShowIncreasingSubsets(spiral_lines), ShowSubmobjectsOneByOne(counts), run_time=run_time, rate_func=linear)
        self.add_count_clicks(len(spiral_lines), run_time)
        self.play(counts[-1].scale, 3, counts[-1].set_stroke, BLACK, 5, {'background': True})
        self.wait()
    ...
    def get_all_primitive_arithmetic_lines(self, N, max_val, mult_factor):
        ...
        for r in range(1, N):
            if get_gcd(N, r) == 1:
                lines.add(self.get_arithmetic_sequence_line(N, mult_factor * r % N, max_val))
        return lines
    ...
    def add_count_clicks(self, N, time, rate_func=linear):
        ...
        if rate_func is linear:
            ...
            delays = time * np.array([binary_search(rate_func, alpha, 0, 1) for alpha in alphas])
        for delay in delays:
            self.add_sound(self.count_sound, time_offset=-delay, gain=-15)
...
class CountRays(CountSpirals):
    ...
    def construct(self):
        prime_spiral = self.get_prime_p_spiral(self.big_n_primes)
        self.add(prime_spiral)
        self.set_scale(self.ray_scale, spiral=prime_spiral, run_time=0)
        spiral_lines = self.get_all_primitive_arithmetic_lines(710, self.big_n_primes, INV_113_MOD_710)
        spiral_lines.set_stroke(YELLOW, 2, opacity=0.5)
        counts = VGroup()
        for n, spiral in zip(it.count(1), spiral_lines):
            count = Integer(n)
            ...
            counts.add(count)
        run_time = 6
        self.play(ShowIncreasingSubsets(spiral_lines), ShowSubmobjectsOneByOne(counts), run_time=run_time, rate_func=smooth)
...
class AskAboutRelationToPrimes(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.change_students(*3 * ['maybe'], look_at=numbers), self.teacher.change, 'maybe', numbers, ShowCreation(arrow), FadeIn(numbers, RIGHT))
        self.play(FadeIn(primes, LEFT))
...
class ZoomOutOnPrimesWithNumbers(IntroducePrimePatterns):
    CONFIG = {'n_labeled_primes': 1000, 'big_n_primes': int(5000000.0), 'thicknesses': [8, 3, 2], 'thicker_target': False}
    ...
    def construct(self):
        ...
        scales = [self.spiral_scale, self.ray_scale, 500000.0]
        thicknesses = self.thicknesses
        for scale, tp in zip(scales, thicknesses):
            kwargs = {'spiral': prime_spiral, 'to_shrink': prime_labels, 'run_time': zoom_time, 'target_p_spiral_width': tp}
...
class HighlightGapsInSpirals(IntroducePrimePatterns):
    ...
    def construct(self):
        ...
        gaps = always_redraw(lambda: VGroup(*[self.get_highlighted_gap(n - 1, n + 1, get_max_n()) for n in [11, 33]]))
        self.add(gaps)
    ...
    def setup_spiral(self):
        ...
        self.add(p_spiral)
        self.set_scale(scale=self.spiral_scale, spiral=p_spiral, target_p_spiral_width=8, run_time=0)

    def get_highlighted_gap(self, n1, n2, max_n):
        l1, l2 = [[self.get_polar_point(k, k) for k in range(INV_7_MOD_44 * n, int(max_n), 5 * 44)] for n in (n1, n2)]
...
class QuestionIsMisleading(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('Whoa, is this some\\\\divine hidden structure\\\\in the primes?', target_mode='surprised', index=0, added_anims=[self.students[1].change, 'pondering', self.students[2].change, 'pondering'])
        self.wait(2)
        self.students[0].bubble = None
        self.teacher_says('Er...not exactly', bubble_config={'width': 3, 'height': 2}, target_mode='guilty')
        self.wait(3)
...
class DirichletComingUp(Scene):
    ...
    def construct(self):
        ...
        image.set_height(3)
        words = OldTexText('Coming up: \\\\', "Dirichlet's theorem", alignment='')
        ...
        self.play(FadeIn(image, RIGHT), FadeIn(words, LEFT))
        self.wait()
...
class ImagineYouFoundIt(TeacherStudentsScene):
    ...
    def construct(self):
        you = self.students[1]
        others = VGroup(self.students[0], self.students[2], self.teacher)
        ...
        self.play(you.change, 'hesitant', you_label, FadeInFromDown(you_label), GrowArrow(arrow), others.set_opacity, 0.25)
        self.play(Blink(you))
        self.play(FadeIn(bubble), FadeOut(you_label), FadeOut(arrow), you.change, 'pondering')
...
class ShowSpiralsForWholeNumbers(CountSpirals):
    CONFIG = {'max_prime': 10000, 'scale_44': 1000.0, 'scale_6': 10, 'n_labels': 100, 'axes_config': {'x_min': -50, 'x_max': 50, 'y_min': -50, 'y_max': 50}}
    ...
    def zoom_out_with_whole_numbers(self):
        ...
        self.set_scale(self.scale_44, spiral=spiral, to_shrink=labels, target_p_spiral_width=6, run_time=10)
        self.wait(2)
        self.spiral = spiral
        self.labels = labels
    ...
    def count_44_spirals(self):
        ...
        new_spirals = PGroup(*[self.get_p_spiral(range(INV_7_MOD_44 * k % 44, self.max_prime, 44)) for k in range(44)])
        ...
        run_time = 3
        self.play(ShowIncreasingSubsets(new_spirals), ShowSubmobjectsOneByOne(counts), run_time=run_time, rate_func=linear)
        self.add_count_clicks(44, run_time)
        self.play(counts[-1].scale, 2, {'about_edge': DL}, counts[-1].set_stroke, BLACK, 5, {'background': True})
        self.wait()
        self.play(FadeOut(counts[-1]), FadeOut(new_spirals), FadeIn(curr_spiral))

    def zoom_back_in_to_6(self):
        spiral = self.spiral
        self.rescale_labels(self.labels)
        self.set_scale(self.scale_6, spiral=spiral, to_shrink=self.labels, target_p_spiral_width=15, run_time=6)
        self.wait()

    def rescale_labels(self, labels):
        for i, label in zip(it.count(1), labels):
            height = label.get_height()
            label.set_height(3 * height / i ** 0.25, about_point=label.get_bottom() + 0.5 * label.get_height() * DOWN)
...
class PrimeSpiralsAtScale1000(SpiralScene):
    ...
    def construct(self):
        ...
        self.add(spiral)
        self.set_scale(scale=1000, spiral=spiral, target_p_spiral_width=15, run_time=0)
...
class SeparateIntoTwoQuestions(Scene):
    ...
    def construct(self):
        ...
        q2.to_edge(RIGHT)
        q2.set_color(TEAL)
        v_line = DashedLine(top_q.get_bottom() + MED_SMALL_BUFF * DOWN, FRAME_HEIGHT * DOWN / 2)
        self.add(top_q)
        self.wait()
        for q, text in [(q1, 'spirals'), (q2, 'primes')]:
            self.play(top_q.get_part_by_tex(text).set_color, q.get_color(), TransformFromCopy(top_q.get_part_by_tex(text), q.get_part_by_tex(text)), LaggedStartMap(FadeIn, filter(lambda m: m is not q.get_part_by_tex(text), q)))
        self.wait()
...
class ExplainSixSpirals(ShowSpiralsForWholeNumbers):
    ...
    def add_spirals_and_labels(self):
        ...
        self.add(spiral, labels)
        self.set_scale(spiral=spiral, scale=self.scale_6, to_shrink=labels, min_box_width=0.08, run_time=0)
    ...
    def comment_on_arms(self):
        ...
        six_groups = VGroup(*[VGroup(sg, lg) for sg, lg in zip(spiral_groups, label_groups)])
        rect_groups = VGroup(*[VGroup(*[SurroundingRectangle(label, stroke_width=2, buff=0.05) for label in group]) for group in label_groups])
        ...
        self.play(LaggedStartMap(ShowCreation, rect_groups[0]), FadeInFromDown(formula_rect), FadeInFromDown(formula[0]), *[ApplyMethod(group.set_opacity, 0.25) for group in six_groups[1:]], run_time=2)
        self.play(LaggedStartMap(FadeOut, rect_groups[0], run_time=1))
        self.wait()
        self.play(six_groups[0].set_opacity, 0.25, six_groups[1].set_opacity, 1, FadeIn(formula[1:]))
        self.wait(2)
        for m in [2, 3, 4, 5]:
            self.play(six_groups[m - 1].set_opacity, 0.25, six_groups[m].set_opacity, 1, ChangeDecimalToValue(formula[2], m))
            self.wait()
        self.play(six_groups[5].set_opacity, 0.25, six_groups[0].set_opacity, 1, formula[1:].set_opacity, 0)
        self.wait()
    ...
    def talk_though_multiples_of_six(self):
        ...
        self.set_scale(4.5, spiral=spiral, to_shrink=labels, run_time=2)
        self.wait()
        ...
        def show_steps(start, stop, added_anims=None, run_time=2):
            if added_anims is None:
                added_anims = []
            self.play(*[ShowSubmobjectsOneByOne(group[start:stop + 1]) for group in [arcs, boxes, lines]], *added_anims, rate_func=linear, run_time=run_time)
        ...
        show_steps(24, 30)
        self.wait(2)
        self.play(FadeOut(trash), FadeOut(rhs), FadeOut(tau_value), spiral.set_opacity, 1, labels.set_opacity, 1, formula[1].set_opacity, 0)
    ...
    def limit_to_primes(self):
        ...
        self.play(FadeIn(prime_label), non_primes.set_opacity, 0.25)
        frame.add(prime_label)
        self.play(frame.scale, 1.5, run_time=2)
        self.wait(2)
        cross_groups = VGroup()
        ...
        for r in [0, 2, 4, 3]:
            ...
            if r == 0:
                ...
                m_sym.set_value(2)
                anims += [plus.set_opacity, 1, m_sym.set_opacity, 1]
            else:
            ...
            self.wait()
        to_fade = VGroup(*[VGroup(six_groups[r], cross_groups[r]) for r in (0, 2, 3, 4)])
        self.add(to_fade, frame)
        self.play(to_fade.set_opacity, 0.25, VGroup(six_k, plus, m_sym).set_opacity, 0, prime_label.set_opacity, 1)
        self.wait()
    ...
    def get_arc(self, n):
        if n == 0:
            return VectorizedPoint()
        return ParametricCurve(self.arc_func, t_min=0, t_max=n, step_size=0.1, stroke_width=2, stroke_color=PINK)
...
class IntroduceResidueClassTerminology(Scene):
    ...
    def add_title(self):
        ...
        self.play(ReplacementTransform(pre_title[0], title[1]), FadeIn(title[0], RIGHT), GrowFromCenter(underline))
        self.play(title[0].set_color, WHITE, underline.set_color, WHITE)
        self.wait()
        title.add(underline)
        self.add(title)
        self.title = title
    ...
    def add_sequences(self):
        ...
        self.add(labels)
        self.play(LaggedStart(*[LaggedStartMap(FadeInFrom, sequence, lambda m: (m, LEFT)) for sequence in sequences], lag_ratio=0.3))
        self.wait()
    ...
    def add_terms(self):
        ...
        arrows = VGroup(*[Arrow(terms.get_left(), sequence.get_right()) for sequence in sequences])
        arrows.set_color(YELLOW)
        self.play(FadeIn(terms), LaggedStartMap(ShowCreation, arrows))
        self.wait()
        self.play(GrowFromCenter(res_brace), FadeInFromDown(remainder))
        self.wait()
        self.play(GrowFromCenter(mod_brace))
        self.play(Write(mod_def))
        self.wait()
        self.terminology = VGroup(terms, res_brace, remainder, mod_brace, mod_def, arrows)
    ...
    def highlight_example(self):
        ...
        self.wait()
        self.play(ShowCreationThenFadeOut(self.underline.copy().set_color(PINK)))
    ...
    def simple_english(self):
        ...
        self.play(FadeOut(terminology), FadeIn(new_phrase), VFadeIn(randy), randy.change, 'sassy')
        self.wait()
...
class SimpleLongDivision(Scene):
    ...
    def construct(self):
        ...
        self.play(TransformFromCopy(divisor, to_subtract.copy()), TransformFromCopy(quotient, to_subtract))
...
class Explain44Spirals(ExplainSixSpirals):
    CONFIG = {'max_N': 3000, 'initial_scale': 10, 'zoom_factor_1': 7, 'zoom_factor_2': 3, 'n_labels': 80}
    ...
    def add_spirals_and_labels(self):
        ...
        self.add(spiral, labels)
        self.set_scale(spiral=spiral, scale=self.initial_scale, to_shrink=labels, target_p_spiral_width=10, run_time=0)
        self.rescale_labels(labels)
        self.spiral = spiral
        self.labels = labels
    ...
    def show_44_steps(self):
        ...
        def show_steps(start, stop, added_anims=None, run_time=2):
            ...
            def rate_func(t):
                return smooth(t, 2)
            self.play(*[ShowSubmobjectsOneByOne(group[start:stop + 1]) for group in [arcs, opaque_labels, lines]], *added_anims, rate_func=rate_func, run_time=run_time)
    ...
    def show_top_right_arithmetic(self):
        ...
        unit_conversion = OldTex('/\\,', '\\left(', '2\\pi', '{\\text{radians}', '\\over', '\\text{rotations}}', '\\right)')
    ...
    def show_pi_approx_arithmetic(self):
        ...
        lines = VGroup(OldTex('{44', '\\over', '2\\pi}', '\\approx', '7'), OldTex('\\Leftrightarrow'), OldTex('{44', '\\over', '7}', '\\approx', '2\\pi'), OldTex('{22', '\\over', '7}', '\\approx', '\\pi'))
        ...
        self.play(FadeIn(rect), LaggedStart(TransformFromCopy(ra_l2[1:4], lines[0][:3]), FadeIn(lines[0].get_part_by_tex('approx')), TransformFromCopy(ra_value[0], lines[0].get_part_by_tex('7')), run_time=2))
        self.wait()
        l0_copy = lines[0].copy()
        self.play(l0_copy.move_to, lines[2], Write(lines[1]))
        self.play(LaggedStart(*[ReplacementTransform(l0_copy.get_part_by_tex(tex), lines[2].get_part_by_tex(tex), path_arc=60 * DEGREES) for tex in ['44', '\\over', '7', 'approx', '2\\pi']], lag_ratio=0.1, run_time=2))
        self.wait()
        self.play(Transform(lines[2], lines[3]))
        self.wait()
        left_arithmetic = VGroup(rect, lines[:3])
        self.play(LaggedStart(FadeOut(self.spiral_group[0]), FadeOut(left_arithmetic), FadeOut(self.right_arithmetic)))
    ...
    def count_by_44(self):
        ...
        l_height_ratio_tracker = ValueTracker(ff_label.get_height() / frame.get_height())
        get_l_height_ratio = l_height_ratio_tracker.get_value
        n_labels = 10
        labels = VGroup(*[Integer(n) for n in values[:n_labels]])
        for label, point in zip(labels, points):
            label.point = point
            label.add_updater(lambda l: l.set_height(frame.get_height() * get_l_height_ratio()))
            label.add_updater(lambda l: l.move_to(l.point + get_p2l() * UP, DOWN))
        ...
        self.play(frame.scale, self.zoom_factor_1, p2l_tracker.set_value, 1, l_height_ratio_tracker.set_value, 0.025, FadeOut(ff_label, rate_func=squish_rate_func(smooth, 0, 1 / 8)), LaggedStart(*2 * [Animation(Group())], *map(FadeIn, labels), lag_ratio=0.5), LaggedStart(*2 * [Animation(Group())], *map(ShowCreation, lines[:len(labels)]), lag_ratio=1), run_time=8)
        self.play(frame.scale, self.zoom_factor_2, l_height_ratio_tracker.set_value, 0.01, ShowCreation(lines[len(labels):]), run_time=8)
...
class Label44Spirals(Explain44Spirals):
    ...
    def setup_spirals(self):
        ...
        for r in range(mod):
            ...
            spirals.add(spiral)
        self.add(spirals)
        scale = np.prod([self.initial_scale, self.zoom_factor_1, self.zoom_factor_2])
    ...
    def enumerate_spirals(self):
        ...
        self.play(spirals[1:].set_opacity, 0.25, FadeIn(labels[0]))
        self.wait()
        for n in range(10):
            arc = Arc(start_angle=n + 0.2, angle=0.9, radius=1.5)
            arc.add_tip()
            ...
            self.play(ShowCreation(arc), FadeIn(r_label), spirals[n + 1].set_opacity, 1, TransformFromCopy(labels[n], labels[n + 1]))
            self.play(FadeOut(arc), FadeOut(r_label), spirals[n].set_opacity, 0.25, FadeOut(labels[n]))

    def get_spiral_arm_labels(self, spirals, index=15):
        mod = 44
        labels = VGroup(*[VGroup(*Tex('44k', '+'), Integer(n)).arrange(RIGHT, buff=SMALL_BUFF) for n in range(mod)])
...
class EliminateNonPrimativeResidueClassesOf44(Label44Spirals):
    ...
    def eliminate_classes(self):
        spirals = self.spirals
        labels = self.get_spiral_arm_labels(spirals)
        self.play(spirals[1:].set_opacity, 0.5, FadeIn(labels[0]))
        self.wait()
        self.play(FadeOut(spirals[0]), FadeOut(labels[0]))
        for n in range(2, 8, 2):
            self.play(FadeIn(labels[n]), spirals[n].set_opacity, 1)
            self.play(FadeOut(VGroup(labels[n], spirals[n])))
        words = OldTexText('All even numbers')
        words.scale(1.5)
        words.to_corner(UL)
        self.play(LaggedStart(*[ApplyMethod(spiral.set_opacity, 1) for spiral in spirals[8::2]], lag_ratio=0.01), FadeIn(words))
        self.play(FadeOut(words), FadeOut(spirals[8::2]))
        self.wait()
        for k in [11, 33]:
            self.play(spirals[k].set_opacity, 1, FadeIn(labels[k]))
            self.wait()
            self.play(FadeOut(spirals[k]), FadeOut(labels[k]))
        admissible_spirals = VGroup(*[spiral for n, spiral in enumerate(spirals) if n % 2 != 0 and n % 11 != 0])
    ...
    def zoom_out(self):
        ...
        for spiral in admissible_spirals.target:
            for box in spiral:
                box.scale(3)
        self.play(frame.scale, 4, MoveToTarget(admissible_spirals), run_time=3)

    def filter_to_primes(self):
        admissible_spirals = self.admissible_spirals
        frame = self.camera_frame
        primes = read_in_primes(self.max_N)
        to_fade = VGroup(*[box for spiral in admissible_spirals for box in spiral if box.n not in primes])
        words = OldTexText('Just the primes')
        words.set_height(0.1 * frame.get_height())
        words.next_to(frame.get_corner(UL), DR, LARGE_BUFF)
        self.play(LaggedStartMap(FadeOut, to_fade, lag_ratio=2 / len(to_fade)), FadeIn(words))
        self.wait()
...
class IntroduceTotientJargon(TeacherStudentsScene):
    ...
    def add_title(self):
        self.teacher_says('More jargon!', target_mode='hooray')
        ...
        underline.scale(1.2)
        self.play(MoveToTarget(words), FadeOut(self.teacher.bubble), LaggedStart(*[FadeOut(pi, 4 * DOWN) for pi in self.pi_creatures]), ShowCreation(underline))

    def eliminate_non_coprimes(self):
        number_grid = VGroup(*[VGroup(*[Integer(n) for n in range(11 * k, 11 * (k + 1))]).arrange(DOWN) for k in range(4)]).arrange(RIGHT, buff=1)
        ...
        coprimes = VGroup(*filter(lambda nm: nm not in evens and nm not in div11, numbers))
        words = OldTexText('Which ones ', "don't\\\\", 'share any factors\\\\', 'with ', '44', alignment='')
        words.scale(1.5)
        ...
        for group in (evens, div11):
            rects = VGroup(*[SurroundingRectangle(number, color=RED) for number in group])
            self.play(LaggedStartMap(ShowCreation, rects, run_time=1))
            self.play(LaggedStart(*[ApplyMethod(number.set_opacity, 0.2) for number in group]), LaggedStartMap(FadeOut, rects), run_time=1)
            self.wait()
        dsf = words[1:3]
        dsf.generate_target()
        ...
        num_pair = VGroup(ff.target, OldTexText('and').scale(1.5), example.target)
        ...
        self.play(FadeOut(words[0]), FadeOut(words[3]), MoveToTarget(dsf), MoveToTarget(ff), GrowFromCenter(num_pair[1]))
        self.play(MoveToTarget(example, path_arc=30 * DEGREES))
        self.wait()
        self.play(dsf.next_to, phrase1.arrow, DOWN, SMALL_BUFF, GrowFromEdge(phrase1.arrow, UP), GrowFromCenter(phrase1), ShowCreation(phrase1.rect))
        self.play(FadeOut(phrase1.rect))
        self.wait()
        self.play(VGroup(dsf, phrase1, phrase1.arrow).next_to, phrase2.arrow, DOWN, SMALL_BUFF, GrowFromEdge(phrase2.arrow, UP), GrowFromCenter(phrase2), ShowCreation(phrase2.rect))
        self.play(FadeOut(phrase2.rect))
        self.wait()
        coprime_rects = VGroup(*map(SurroundingRectangle, coprimes))
        coprime_rects.set_stroke(BLUE, 2)
        example_anim = UpdateFromFunc(example, lambda m: m.set_value(coprimes[len(coprime_rects) - 1].get_value()))
        self.play(ShowIncreasingSubsets(coprime_rects, int_func=np.ceil), example_anim, run_time=3, rate_func=linear)
        self.wait()
        ...
        self.play(words_to_keep.to_edge, DOWN, MaintainPositionRelativeTo(to_fade, words_to_keep), VFadeOut(to_fade))
        ...
        self.wait()
        self.play(ShowIncreasingSubsets(coprime_rects), UpdateFromFunc(rhs, lambda m: m.set_value(len(coprime_rects))), example_anim, rate_func=linear, run_time=3)
        self.wait()
        totient_group = VGroup(totient, brace, etf)
        for cp, rect in zip(coprimes, coprime_rects):
            cp.add(rect)
        self.play(coprimes.arrange, RIGHT, {'buff': SMALL_BUFF}, coprimes.set_width, FRAME_WIDTH - 1, coprimes.move_to, 2 * UP, FadeOut(evens), FadeOut(div11[1::2]), FadeOutAndShiftDown(words_to_keep), totient_group.center, totient_group.to_edge, DOWN)
        totatives = OldTexText("``Totatives''")
        totatives.scale(2)
        totatives.set_color(BLUE)
        totatives.move_to(ORIGIN)
        arrows = VGroup(*[Arrow(totatives.get_top(), coprime.get_bottom()) for coprime in coprimes])
        arrows.set_color(WHITE)
        self.play(FadeIn(totatives), LaggedStartMap(VFadeInThenOut, arrows, run_time=4, lag_ratio=0.05))
        self.wait(2)
...
class TwoUnrelatedFacts(Scene):
    ...
    def show_columns(self):
        h_line = self.h_line
        v_line = Line(h_line.get_center() + MED_SMALL_BUFF * DOWN, FRAME_HEIGHT * DOWN / 2)
        v_line.match_style(h_line)
        approx = OldTex('{44 \\over 7} \\approx 2\\pi')
        approx.scale(1.5)
        approx.next_to(h_line.point_from_proportion(0.25), DOWN, MED_LARGE_BUFF)
        mod = 44
        n_terms = 9
        ...
        def get_line(row):
            return Line(row.get_left(), row.get_right(), stroke_color=RED, stroke_width=4)
        ...
        self.wait()
        self.play(primes.set_color, TEAL, non_primes.set_opacity, 0.25, even_lines.set_opacity, 0.25, eleven_line.set_opacity, 0.25)
        self.wait()

class ExplainRays(Explain44Spirals):
    CONFIG = {'max_N': int(500000.0), 'axes_config': {'x_min': -1000, 'x_max': 1000, 'y_min': -1000, 'y_max': 1000, 'axis_config': {'tick_frequency': 50}}}
    ...
    def show_710th_point(self):
        ...
        self.set_scale(scale=120, spiral=spiral, to_shrink=labels)
        ...
        arrow = Arrow(ORIGIN, DOWN, stroke_width=6, max_tip_length_to_length_ratio=0.35, max_stroke_width_to_length_ratio=10, tip_length=0.35)
        ...
        self.play(FadeIn(fade_rect), FadeOut(labels), FadeInFromLarge(box_710), FadeIn(label_710, DOWN), ShowCreation(arrow))
        self.wait()
    ...
    def show_arithmetic(self):
        ...
        equation = OldTex('710', '\\text{ radians}', '=', '(710 / 2\\pi)', '\\text{ rotations}')
        ...
        values = VGroup(*[value[0][:n].deepcopy().next_to(brace, DOWN, SMALL_BUFF) for n in [3, *range(5, 13)]])
        group = VGroup(equation, brace, value)
        rect = SurroundingRectangle(group, buff=MED_SMALL_BUFF)
        rect.set_stroke(WHITE, 2)
        rect.set_fill(GREY_D, 1)
        approx = OldTex('{710', '\\over', '113}', '\\approx', '2\\pi')
        approx.next_to(rect, DOWN)
        approx.align_to(equation, LEFT)
        approx2 = OldTex('{355', '\\over', '113}', '\\approx', '\\pi')
        approx2.next_to(approx, RIGHT, LARGE_BUFF)
        self.play(FadeIn(rect), TransformFromCopy(label_710, equation[0]), FadeIn(equation[1:3]))
        self.play(FadeIn(equation[3:], LEFT))
        self.play(GrowFromCenter(brace))
        self.play(ShowSubmobjectsOneByOne(values), run_time=3, rate_func=linear)
        self.wait()
        self.play(rect.stretch, 2, 1, {'about_edge': UP}, LaggedStart(TransformFromCopy(equation[3][1:4], approx[0]), FadeIn(approx[1][0]), TransformFromCopy(values[-1][:3], approx[2]), FadeIn(approx[3]), TransformFromCopy(equation[3][5:7], approx[4]), run_time=2))
        self.wait()
        self.play(TransformFromCopy(approx, approx2))
        self.wait()
        self.play(FadeOut(VGroup(rect, equation, brace, values[-1], approx, approx2)), self.fade_rect.set_opacity, 0.25)
    ...
    def zoom_and_count(self):
        ...
        def update_arrow(arrow):
            ...
            arrow.put_start_and_end_on(label.get_bottom() + SMALL_BUFF * DOWN, point + SMALL_BUFF * UP)
        ...
        k_label.add_updater(lambda d: d.set_value(get_k()).next_to(times, LEFT, SMALL_BUFF))
        self.remove(box)
        self.add(times, k_label, boxes)
        self.set_scale(scale=10000, spiral=self.spiral, run_time=8, target_p_spiral_width=2, added_anims=[box_height_tracker.set_value, 0.035])
        self.wait()
        ...
        self.play(FadeOut(times), ReplacementTransform(label, new_label[0]), ReplacementTransform(k_label, new_label[1]), FadeOut(arrow))
        ...
        for r in range(1, 12):
            ...
            for box in new_boxes:
                box.rotate(-1)
            self.play(FadeOut(boxes), LaggedStartMap(FadeIn, new_boxes, lag_ratio=0.01), new_label.set_opacity, 1, new_label.next_to, point, vect, r_label.set_opacity, 1, ChangeDecimalToValue(r_label, r), run_time=1)
            self.remove(boxes)
            boxes = new_boxes
            self.add(boxes)
            self.wait()
        self.play(FadeOut(boxes), FadeOut(new_label), FadeOut(r_label), FadeOut(self.fade_rect))
        self.set_scale(30000, spiral=spiral, run_time=4)
        self.wait()
        self.remove(spiral)
        self.add(spiral[1])
        self.wait()
...
class CompareTauToApprox(Scene):
    ...
    def construct(self):
        eqs = VGroup(OldTex('2\\pi', '=', '{:.10}\\dots'.format(TAU)), OldTex('\\frac{710}{113}', '=', '{:.10}\\dots'.format(710 / 113)))
        ...
        self.play(LaggedStart(FadeIn(eqs[0], DOWN), FadeIn(eqs[1], UP)))
...
class ShowClassesOfPrimeRays(SpiralScene):
    CONFIG = {'max_N': int(1000000.0), 'scale': 100000.0}
    ...
    def setup_rays(self):
        ...
        self.add(spiral)
        self.set_scale(scale=self.scale, spiral=spiral, target_p_spiral_width=3, run_time=0)
    ...
    def show_classes(self):
        ...
        for i in range(40):
            ...
            self.play(rect.set_opacity, 0.5, ShowCreation(ray), LaggedStartMap(FadeOut, last_ray), ReplacementTransform(last_label[:2], label[:2]), Restore(label[2]), last_label[2].move_to, label[2].saved_state, last_label[2].set_opacity, 0)
...
class ShowFactorsOf710(Scene):
    ...
    def construct(self):
        equation = OldTex('710', '=', '71', '\\cdot', '5', '\\cdot', '2')
        equation.scale(1.5)
        equation.to_corner(UL)
        ten = OldTex('10')
        ...
        self.add(equation[0])
        self.wait()
        self.play(TransformFromCopy(equation[0][:2], equation[2]), FadeIn(equation[1]), FadeIn(equation[3]), TransformFromCopy(equation[0][2:], ten))
        self.wait()
        self.remove(ten)
...
class LookAtRemainderMod710(Scene):
    ...
    def construct(self):
        ...
        equation = OldTex('n', '=', '710', 'k', '+', 'r', tex_to_color_map=t2c)
        equation.scale(1.5)
        ...
        self.add(equation)
        self.play(FadeIn(n_label, UP), ShowCreation(n_arrow))
        self.wait()
        self.play(FadeIn(r_label, DOWN), ShowCreation(r_arrow))
        self.wait()

class EliminateNonPrimative710Residues(ShowClassesOfPrimeRays):
    CONFIG = {'max_N': int(500000.0), 'scale': 50000.0}
    ...
    def setup_rays(self):
        mod = 710
        rays = PGroup(*[self.get_p_spiral(range(r, self.max_N, mod)) for r in range(mod)])
        rays.set_color(YELLOW)
        self.add(rays)
        self.set_scale(scale=self.scale, spiral=rays, target_p_spiral_width=1, run_time=0)
        self.rays = rays
    ...
    def eliminate_classes(self):
        ...
        r_label.group = odds
        r_label.add_updater(lambda m: m.set_value(m.group[-1].r if len(m.group) > 0 else 1))
        self.remove(rays)
        odds.set_stroke_width(3)
        self.add(odds, label)
        self.play(ShowIncreasingSubsets(odds, int_func=np.ceil), run_time=10)
        ...
        for i, group in [(1, mult5), (2, mult71)]:
            ...
            self.play(ShowIncreasingSubsets(group_copy, int_func=np.ceil, run_time=10))
            self.play(FadeOut(label))
            self.wait()
...
class Show280Computation(Scene):
    ...
    def construct(self):
        equation = OldTex('\\phi(710) = ', '710', '\\left({1 \\over 2}\\right)', '\\left({4 \\over 5}\\right)', '\\left({70 \\over 71}\\right)', '=', '280')
        equation.set_width(FRAME_WIDTH - 1)
        equation.move_to(UP)
        words = VGroup(OldTexText('Filter out\\\\evens'), OldTexText('Filter out\\\\multiples of 5'), OldTexText('Filter out\\\\multiples of 71'))
        vects = [DOWN, UP, DOWN]
        ...
        self.play(FadeIn(rect), FadeInFromDown(equation), FadeIn(etf_label), GrowArrow(arrow))
        self.wait()
        for word in words:
            self.play(FadeIn(word), GrowFromCenter(word.brace))
            self.wait()

class TeacherHoldUp(TeacherStudentsScene):

    def construct(self):
        self.play_all_student_changes('pondering', look_at=2 * UP, added_anims=[self.teacher.change, 'raise_right_hand'])
        self.wait(8)
...
class DiscussPrimesMod10(Scene):
    ...
    def construct(self):
        ...
        sequences = VGroup(*[VGroup(*[Integer(n).shift(n // 10 * RIGHT) for n in range(r, 100 + r, 10)]) for r in range(10)])
        ...
        self.wait()
        self.play(LaggedStart(*[LaggedStart(*[FadeIn(item, LEFT) for item in sequence]) for sequence in sequences]))
        self.wait()
        ...
        primes = VGroup(*[item for seq in sequences for item in seq if int(item.get_value()) in prime_numbers])
        non_primes = VGroup(*[item for seq in sequences for item in seq if reduce(op.and_, [int(item.get_value()) not in prime_numbers, item.get_value() % 2 != 0, item.get_value() % 5 != 0])])
        for prime, group in [(two, evens), (five, div5)]:
            self.play(ShowCreationThenFadeAround(prime))
            self.play(LaggedStart(*[ApplyMethod(item.set_opacity, 0.2) for item in group]))
            self.wait()
        self.play(LaggedStart(*[ApplyFunction(lambda m: m.scale(1.2).set_color(TEAL), prime) for prime in primes]), LaggedStart(*[ApplyFunction(lambda m: m.scale(0.8).set_opacity(0.8), non_prime) for non_prime in non_primes]))
        self.wait()
        rects = VGroup(*[SurroundingRectangle(VGroup(labels[r], sequences[r])) for r in [1, 3, 7, 9]])
        for rect in rects:
            rect.reverse_points()
        fade_rect = FullScreenFadeRectangle()
        fade_rect.scale(1.1)
        ...
        self.wait()
        self.play(FadeOut(fade_rect), FadeIn(new_fade_rect))
        self.wait()

class BucketPrimesByLastDigit(Scene):
    CONFIG = {'bar_colors': [YELLOW, BLUE], 'mod': 10, 'max_n': 10000, 'n_to_animate': 20, 'n_to_show': 1000, 'x_label_scale_factor': 1, 'x_axis_label': 'Last digit', 'bar_width': 0.5}
    ...
    def add_axes(self):
        mod = self.mod
        axes = Axes(x_min=0, x_max=mod + 0.5, x_axis_config={'unit_size': 10 / mod, 'include_tip': False}, y_min=0, y_max=100, y_axis_config={'unit_size': 0.055, 'tick_frequency': 12.5, 'include_tip': False})
        x_labels = VGroup()
        for x in range(mod):
            digit = Integer(x)
    ...
    def add_bars(self):
        ...
        bars = VGroup()
        for x in range(mod):
            bar = Rectangle(height=1, width=self.bar_width, fill_opacity=1)
        ...
        def update_bars(bars):
            ...
            for bar, prop in zip(bars, props):
                bar.set_height(max(1e-05, 100 * prop * axes.y_axis.unit_size), stretch=True)
    ...
    def bucket_primes(self):
        ...
        mod = self.mod
        primes = VGroup(*[Integer(prime).scale(2).to_edge(UP, buff=LARGE_BUFF) for prime in read_in_primes(max_n)])
        ...
        for i, p, np in zip(it.count(), primes[:n_to_show], primes[1:]):
            ...
            self.add(rects[d])
            if i < n_to_animate:
                self.play(p.scale, 0.5, p.move_to, bars[d].get_top(), p.set_opacity, 0, FadeIn(np), count_trackers[d].increment_value, 1)
                self.remove(p)
            else:
...
class PhraseDirichletsTheoremFor10(TeacherStudentsScene):
    ...
    def construct(self):
        expression = OldTex('\\lim_{x \\to \\infty}', '\\left(', '{\\text{\\# of primes $p$ where $p \\le x$} \\text{ and $p \\equiv 1$ mod 10}', '\\over', '\\text{\\# of primes $p$ where $p \\le x$}}', '\\right)', '=', '\\frac{1}{4}')
        ...
        x_example = VGroup(OldTexText('Think, for example, $x = $'), Integer(int(1000000.0)))
        x_example.arrange(RIGHT)
        x_example.scale(1.5)
        x_example.to_edge(UP)
        teacher = self.teacher
        students = self.students
        self.play(FadeInFromDown(denom), teacher.change, 'raise_right_hand', self.change_students(*['pondering'] * 3))
        self.wait()
        self.play(FadeInFromDown(x_example))
        self.wait()
        self.play(Restore(denom), teacher.change, 'thinking')
        self.play(TransformFromCopy(denom, num[:len(denom)]), Write(over))
        self.play(Write(num[len(denom):]), students[0].change, 'confused', students[2].change, 'erm')
        self.wait(2)
        self.play(Write(lp), Write(rp), Write(eq))
        self.play(FadeIn(fourth, LEFT))
        self.play(FadeIn(lim, RIGHT))
        self.play(ChangeDecimalToValue(x_example[1], int(10000000.0), run_time=8, rate_func=linear), VFadeOut(x_example, run_time=8), self.change_students(*['thinking'] * 3), Blink(teacher, run_time=4, rate_func=squish_rate_func(there_and_back, 0.65, 0.7)))
...
class BucketPrimesBy44(BucketPrimesByLastDigit):
    CONFIG = {'mod': 44, 'n_to_animate': 5, 'x_label_scale_factor': 0.5, 'x_axis_label': 'r mod 44', 'bar_width': 0.1}

    def modify_x_labels(self, labels):
        labels[::2].set_opacity(0)

class BucketPrimesBy9(BucketPrimesByLastDigit):
    CONFIG = {'mod': 9, 'n_to_animate': 5, 'x_label_scale_factor': 1, 'x_axis_label': 'r mod 9', 'bar_width': 1}
...
class DirichletIn1837(Scene):
    ...
    def construct(self):
        ...
        timeline = NumberLine(x_min=1700, x_max=2020, tick_frequency=1, big_tick_numbers=dates, unit_size=0.2, stroke_color=GREY, stroke_width=2)
        timeline.add_numbers(*dates, group_with_commas=False)
        timeline.numbers.shift(SMALL_BUFF * DOWN)
        timeline.to_edge(RIGHT)
        d_arrow, rh_arrow, pnt_arrow = arrows = VGroup(*[Vector(DOWN).next_to(timeline.n2p(date), UP) for date in [1837, 1859, 1896]])
        d_label, rh_label, pnt_label = labels = VGroup(*[OldTexText(text).next_to(arrow, UP) for arrow, text in zip(arrows, ["Dirichlet's\\\\theorem\\\\1837", 'Riemann\\\\hypothesis\\\\1859', 'Prime number\\\\theorem\\\\1896'])])
        frame = self.camera_frame
        self.add(timeline, arrows, labels)
        self.play(frame.move_to, timeline.n2p(1837), run_time=4)
        self.wait()
        image = ImageMobject('Dirichlet')
        image.set_height(3)
        ...
        self.wait()
        self.play(Flash(d_label.get_center(), num_lines=12, line_length=0.25, flash_radius=1.5, line_stroke_width=3, lag_ratio=0.05))
        self.wait()
        title, underline = self.get_title_and_underline()
        n = len(title[0])
        self.play(ReplacementTransform(d_label[0][:n], title[0][:n]), FadeOut(d_label[0][n:]), LaggedStartMap(FadeOutAndShiftDown, Group(image, d_arrow, rh_label, rh_arrow, pnt_label, pnt_arrow, *timeline)))
...
class PhraseDirichletsTheorem(DirichletIn1837):
    ...
    def construct(self):
        ...
        phi_N_label = OldTex('\\phi({10}) = ', '\\#\\{1, 3, 7, 9\\} = 4', tex_to_color_map={'{10}': N.get_color()})
        phi_N_label[-1][2:9:2].set_color(r.get_color())
        phi_N_label.next_to(r_label, DOWN, MED_LARGE_BUFF)
        self.play(LaggedStart(*[FadeIn(denom), ShowCreation(over), FadeIn(num), Write(VGroup(lp, rp)), FadeIn(lim), Write(VGroup(eq, fourth))]), run_time=3, lag_ratio=0.7)
        self.wait()
        for mob in (denom, num):
            self.play(ShowCreationThenFadeAround(mob))
            self.wait()
        self.play(FadeIn(r, DOWN), FadeOut(one, UP))
        self.play(FadeIn(N, DOWN), FadeOut(ten, UP))
        self.wait()
        self.play(TransformFromCopy(N, N_label[0]), FadeIn(N_label[1:], DOWN))
        self.wait()
        self.play(FadeIn(r_label[1:-1], DOWN), TransformFromCopy(r, r_label[0]))
        self.play(TransformFromCopy(N_label[0], r_label[-1]))
        self.wait()
        self.play(ShowCreationThenFadeAround(fourth))
        self.play(FadeIn(one_over_phi_N[2:], LEFT), FadeOut(four, RIGHT), ReplacementTransform(fourth[0], one_over_phi_N[0][0]), ReplacementTransform(fourth[1], one_over_phi_N[1][0]))
        self.play(FadeIn(phi_N_label, DOWN))
        self.wait()
        new_expression = OldTex('\\lim_{x \\to \\infty}', '\\left(', '{\\pi(x; {N}, {r})', '\\over', '\\pi(x)}', '\\right)', '=', '\\frac{1}{\\phi({N})}', tex_to_color_map={'{N}': N.get_color(), '{r}': r.get_color(), '\\pi': WHITE})
        ...
        self.play(Blink(randy))
        self.wait()
        self.play(FadeIn(new_expression), MoveToTarget(label_group), phi_N_label.to_edge, DOWN, MED_LARGE_BUFF, randy.change, 'horrified', new_expression)
        self.play(ShowCreation(ne_rect))
        self.play(randy.change, 'confused')
        self.play(Blink(randy))
        self.wait()
        self.play(LaggedStartMap(ShowCreationThenFadeAround, pis), randy.change, 'angry', new_expression)
        self.wait()
        self.play(Blink(randy))
        self.wait()
...
class MoreModestDirichlet(Scene):
    ...
    def construct(self):
        ed = OldTexText('Each (coprime) residue class ', 'is equally dense with ', 'primes.')
        inf = OldTexText('Each (coprime) residue class ', 'has infinitely many ', 'primes.')
        ed[1].set_color(BLUE)
        ...
        self.wait()
        self.play(ShowCreation(cross))
        self.play(c_group.shift, DOWN, c_group.set_opacity, 0.5, ReplacementTransform(ed[::2], inf[::2]), FadeIn(inf[1]))
        self.wait()
        self.remove(*inf)
        self.play(inf[1].shift, UP, Restore(ed[0]), Restore(ed[1]), Restore(ed[2]), FadeOut(cross))
        self.wait()
...
class TalkAboutProof(TeacherStudentsScene):
    ...
    def construct(self):
        teacher = self.teacher
        students = self.students
        self.student_says("So how'd he\\\\prove it?", index=0)
        bubble = students[0].bubble
        students[0].bubble = None
        self.play(teacher.change, 'hesitant', students[1].change, 'happy', students[2].change, 'happy')
        self.wait()
        self.teacher_says("...er...it's a\\\\bit complicated", target_mode='guilty')
        self.play_all_student_changes('tired', look_at=teacher.bubble, lag_ratio=0.1)
        self.play(FadeOut(bubble), FadeOut(bubble.content))
        self.wait(3)
        ca = OldTexText('Complex ', 'Analysis')
        ca.move_to(self.hold_up_spot, DOWN)
        self.play(teacher.change, 'raise_right_hand', FadeInFromDown(ca), FadeOut(teacher.bubble), FadeOut(teacher.bubble.content), self.change_students(*['pondering'] * 3))
        self.wait()
        self.play(ca.scale, 2, ca.center, ca.to_edge, UP, teacher.change, 'happy')
        self.play(ca[1].set_color, GREEN)
        self.wait(2)
        self.play(ca[0].set_color, YELLOW)
        self.wait(2)
        self.play_all_student_changes('confused', look_at=ca)
        self.wait(4)
...
class HighlightTwinPrimes(Scene):
    ...
    def add_paper_titles(self):
        gpy = OldTexText('Goldston, Pintz, Yildirim\\\\', '2005')
        zhang = OldTexText('Zhang\\\\2014')
        gpy.move_to(FRAME_WIDTH * LEFT / 4)
        gpy.to_edge(UP)
        zhang.move_to(FRAME_WIDTH * RIGHT / 4)
        zhang.to_edge(UP)
        self.play(LaggedStartMap(FadeInFromDown, VGroup(gpy, zhang), lag_ratio=0.3))

    def show_twin_primes(self):
        max_x = 300
        line = NumberLine(x_min=0, x_max=max_x, unit_size=0.5, big_tick_numbers=range(10, max_x, 10))
        ...
        for pm, npm in zip(prime_mobs, prime_mobs[1:]):
            ...
            if np - p == 2:
                angle = 30 * DEGREES
                arc = Arc(start_angle=angle, angle=PI - 2 * angle, color=RED)
        ...
        line.add(dots)
        self.play(FadeIn(line, lag_ratio=0.9), LaggedStartMap(FadeInFromDown, prime_mobs), run_time=2)
        line.add(prime_mobs)
        self.wait()
        self.play(FadeIn(arcs))
        self.play(line.shift, 100 * LEFT, arcs.shift, 100 * LEFT, run_time=20, rate_func=lambda t: smooth(t, 5))
        self.wait()

class RandomToImportant(PiCreatureScene):
    CONFIG = {'default_pi_creature_kwargs': {'color': GREY_BROWN}, 'camera_config': {'background_color': GREY_E}}
    ...
    def construct(self):
        ...
        arrow = Arrow(left_comment.get_right(), right_comment.get_left(), buff=0.5)
        self.play(morty.change, 'raise_left_hand', left_comment, FadeInFromDown(left_comment))
        self.wait(2)
        self.play(morty.change, 'raise_right_hand', right_comment, FadeInFromDown(right_comment))
        self.play(ShowCreation(arrow), morty.look_at, right_comment)
        self.wait(2)

class RandomWalkOfTopics(Scene):
    CONFIG = {'n_dots': 30, 'n_edge_range': [2, 4], 'super_dot_n_edges': 20, 'isolated_threashold': 0.5}
    ...
    def define_important(self):
        ...
        title = VGroup(OldTexText('Important'), OldTex('\\Leftrightarrow'), OldTexText('Many connections'))
        title.scale(1.5)
        ...
        title[0].set_x(0)
        self.add(title[0])
        self.play(FadeInFromLarge(sd), title[0].set_color, RED)
        title[0].saved_state.set_color(RED)
        self.play(Restore(title[0]), GrowFromCenter(title[1]), FadeIn(arrow_words), FadeIn(title[2], LEFT), LaggedStartMap(ShowCreation, sd.edges, run_time=3), LaggedStartMap(GrowFromPoint, sd.neighbors, lambda m: (m, sd.get_center()), run_time=3))
        self.wait()
    ...
    def perform_walk(self):
        ...
        for d1, d2 in zip(path, path[1:]):
            ...
            self.play(ShowCreationThenDestruction(Line(d1.get_center(), d2.get_center(), color=YELLOW)))
        self.play(Flash(sd))
        self.play(LaggedStart(*[ApplyMethod(edge.set_stroke, YELLOW, 5, rate_func=there_and_back) for edge in sd.edges]))
        self.wait()

class DeadEnds(RandomWalkOfTopics):
    CONFIG = {'n_dots': 20, 'n_edge_range': [2, 4], 'super_dot_n_edges': 2, 'random_seed': 1}
    ...
    def construct(self):
        ...
        edges = self.edges
        self.add(dots, edges)
        VGroup(edges[3], edges[4], edges[7], edges[10], edges[12], edges[15], edges[27], edges[30], edges[33]).set_opacity(0)
...
class Rediscovery(Scene):
    ...
    def construct(self):
        ...
        rings = self.get_rings(lightbulb.get_center(), max_radius=10.0, delta_r=0.1)
        bubble = ThoughtBubble()
        bubble.pin_to(randy)
        self.play(randy.change, 'pondering', ShowCreation(bubble), FadeInFromDown(lightbulb))
        self.add(rings, bubble)
        self.play(randy.change, 'thinking', LaggedStartMap(VFadeInThenOut, rings, lag_ratio=0.002, run_time=3))
        self.wait(4)

    def get_rings(self, center, max_radius, delta_r):
        radii = np.arange(0, max_radius, delta_r)
        rings = VGroup(*[Annulus(inner_radius=r1, outer_radius=r2, fill_opacity=0.75 * (1 - fdiv(r1, max_radius)), fill_color=YELLOW) for r1, r2 in zip(radii, radii[1:])])
...
class Thumbnail(SpiralScene):
    ...
    def add_dots(self):
        self.set_scale(scale=1000.0)
        p_spiral = self.get_prime_p_spiral(self.max_N)
        dots = VGroup(*[Dot(point, radius=interpolate(0.01, 0.07, min(0.5 * get_norm(point), 1)), fill_color=TEAL) for point in p_spiral.get_points()])
        dots.set_fill([TEAL_E, TEAL_A])
        dots.set_stroke(BLACK, 1)
        label = OldTexText('($p$, $p$) for all primes $p$,\\\\', 'in polar coordinates', tex_to_color_map={'$p$': YELLOW})
        label.scale(2)
