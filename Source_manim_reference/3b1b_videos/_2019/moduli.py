...
class TriangleModuliSpace(Scene):
    CONFIG = {'camera_config': {'background_image': 'chalkboard'}, 'degen_color': GREEN_D, 'x1_color': GREEN_B, 'y1_color': RED, 'x_eq_y_color': YELLOW, 'right_color': TEAL, 'obtuse_color': PINK, 'acute_color': GREEN, 'triangle_fill_opacity': 0.5, 'random_seed': 0, 'example_triangle_width': 6}

    def setup(self):
        self.plane = NumberPlane(axis_config={'unit_size': 2})
    ...
    def show_meaning_of_similar(self):
        ...
        self.wait()
        self.play(ReplacementTransform(question.get_part_by_tex('space'), title.get_part_by_tex('Space')), FadeOut(question[:-2]), FadeOut(question[-1]), FadeIn(title[1:]), LaggedStartMap(DrawBorderThenFill, all_triangles, rate_func=bezier([0, 0, 1.5, 1, 1]), run_time=5, lag_ratio=0.05))
        self.wait()
        ...
        rects = VGroup(*[Rectangle(height=height, width=max_width + 0.25, stroke_width=2).move_to(tri_class, UP) for tri_class in tri_classes.target])
        ...
        per_class_dots = VGroup(*[OldTex('\\vdots').move_to(tri_class).set_y(rects.get_bottom()[1] + 0.4) for tri_class in tri_classes.target])
        all_class_dots = OldTex('\\dots').next_to(rects, RIGHT, MED_SMALL_BUFF)
        self.play(FadeInFromDown(subtitle), MoveToTarget(tri_classes))
        self.play(LaggedStartMap(FadeIn, rects), Write(per_class_dots), Write(all_class_dots))
        self.wait(2)
        ...
        self.play(FadeOut(VGroup(title, subtitle), UP), tri1.next_to, sim_sign, LEFT, 0.75, tri2.next_to, sim_sign, RIGHT, 0.75)
        self.play(FadeInFromDown(sim_sign), Write(similar_word, run_time=1))
        self.wait()
        tri1_copy = tri1.copy()
        self.play(tri1_copy.next_to, tri2, RIGHT, LARGE_BUFF, path_arc=90 * DEGREES)
        self.play(Rotate(tri1_copy, tri2.angle - tri1.angle))
        self.play(tri1_copy.scale, tri2.scalar / tri1.scalar)
        self.play(tri1_copy.move_to, tri2)
        tri1_copy.set_color(YELLOW)
        self.play(FadeOut(tri1_copy), rate_func=rush_from)
        self.wait(2)
        ...
        tri1_copy = tri1.copy()
        self.play(Restore(tri2), MoveToTarget(tri3))
        self.play(ReplacementTransform(similar_word[0], not_similar_word[1]), GrowFromCenter(not_similar_word[0]), ShowCreation(sim_cross))
        self.play(tri1_copy.move_to, tri3)
        self.play(Rotate(tri1_copy, 90 * DEGREES))
        self.play(tri1_copy.match_height, tri3, tri1_copy.move_to, tri3, RIGHT)
        ...
        self.play(Restore(tri1), Restore(tri2), Restore(tri3), FadeOut(not_similar_word), FadeOut(sim_sign), FadeOut(sim_cross), FadeIn(new_title[1], UP))
        self.play(ShowCreationThenDestruction(new_title_underline), LaggedStartMap(ApplyMethod, rects, lambda m: (m.set_stroke, YELLOW, 5), rate_func=there_and_back, run_time=1))
        self.wait()
        ...
        blob.set_height(2)
        blob.to_corner(UR)
        dots = VGroup(*[Dot(color=tri.get_color()).move_to(self.get_triangle_x(tri) * RIGHT + self.get_triangle_y(tri) * UP) for tri_class in tri_classes for tri in tri_class[0]])
        dots.space_out_submobjects(2)
        dots.move_to(blob)
        self.play(DrawBorderThenFill(blob), new_title.shift, LEFT)
        self.play(LaggedStart(*[ReplacementTransform(tri_class.copy().set_fill(opacity=0), dot) for tri_class, dot in zip(tri_classes, dots)], run_time=3, lag_ratio=0.3))
        tri = tri_classes[0][0]
        verts = tri.get_vertices()
        angle = PI + angle_of_vector(verts[1] - verts[2])
        self.play(tri.rotate, -angle, tri.set_width, self.example_triangle_width, tri.center, FadeOut(tri_classes[0][1:]), FadeOut(tri_classes[1:]), FadeOut(rects), FadeOut(per_class_dots), FadeOut(all_class_dots), FadeOut(blob), FadeOut(dots), FadeOut(new_title))
        self.triangle = tri
    ...
    def show_xy_rule(self):
        ...
        for var, num, vect in zip(xy_labels, [b_label, a_label], [DR, DL]):
            ...
            self.play(FadeIn(var, DOWN), Write(var.brace, run_time=1))
            self.wait()
        to_move = VGroup(triangle, side_labels, x_label, x_label.brace, y_label, y_label.brace)
        axes = Axes(x_min=-0.25, x_max=1.5, y_min=-0.25, y_max=1.5, axis_config={'tick_frequency': 0.25, 'unit_size': 3})
        x_axis = axes.x_axis
        y_axis = axes.y_axis
        ...
        for axis, vect in [(x_axis, DOWN), (y_axis, LEFT)]:
            axis.add_numbers(0.5, 1.0, direction=vect, num_decimal_places=1)
        axes.to_corner(DR, buff=LARGE_BUFF)
        self.play(to_move.to_corner, UL, {'buff': LARGE_BUFF}, to_move.shift, MED_LARGE_BUFF * DOWN, Write(axes))
        ...
        dot = always_redraw(lambda: triangle.copy().set_width(0.1).move_to(get_ms_point()))
        y_line = always_redraw(lambda: DashedLine(x_axis.n2p(x_coord.get_value()), get_ms_point(), color=y_color, stroke_width=1))
        x_line = always_redraw(lambda: DashedLine(y_axis.n2p(y_coord.get_value()), get_ms_point(), color=x_color, stroke_width=1))
        ...
        self.play(MoveToTarget(x_coord), MoveToTarget(y_coord), FadeIn(coord_label), ReplacementTransform(triangle.copy().set_fill(opacity=0), dot))
        ...
        box = Square(fill_color=GREY_D, fill_opacity=0.75, stroke_color=GREY_B, stroke_width=2)
        ...
        self.wait()
        region = Polygon(axes.c2p(0, 0), axes.c2p(1, 0), axes.c2p(1, 1), fill_color=GREY_BROWN, fill_opacity=0.75, stroke_color=GREY_BROWN, stroke_width=2)
        ...
        self.play(FadeIn(region), ShowCreation(x_eq_y_line), Transform(ineq1[:2], ineq[:2], remover=True), Transform(ineq1[2:], ineq[4:], remover=True), Transform(ineq2[:4], ineq[:4], remover=True), Transform(ineq2[4:], ineq[6:], remover=True))
        self.add(ineq)
        ...
        ms_region = Polygon(axes.c2p(1, 0), axes.c2p(0.5, 0.5), axes.c2p(1, 1), fill_color=BLUE_E, fill_opacity=0.75, stroke_width=0)
        ...
        self.play(ShowCreation(xpy1_line), FadeIn(xpy1_ineq, UP))
        self.wait()
        self.play(tip_tracker.set_y, triangle.get_bottom()[1] + 0.01, FadeIn(tt_line))
        self.wait()
        self.add(ms_region, axes, x_line, y_line, coord_label, dot)
        self.play(FadeIn(ms_region), region.set_fill, GREY_D)
        self.wait()
        self.play(tip_tracker.shift, UP + RIGHT, FadeOut(tt_line))
        self.wait()
        ...
        self.play(tip_tracker.move_to, equilateral_point, run_time=2)
        self.wait()
        ms_words = OldTexText('Moduli\\\\', 'Space')
        ms_words.scale(1.5)
        ms_words.next_to(ms_region, RIGHT, buff=0.35)
        ms_arrow = Arrow(ms_words[1].get_corner(DL), ms_region.get_center(), path_arc=-90 * DEGREES, buff=0.1)
        ms_arrow.shift(0.1 * RIGHT)
        ms_arrow.scale(0.95)
        self.play(FadeIn(ms_words, LEFT))
        ...
        self.play(tip_tracker.move_to, new_tip, FadeOut(ms_words), FadeOut(ms_arrow))
        self.play(ShowCreation(elbow), FadeIn(right_words, UP), FadeOut(ineqs, DOWN))
        ...
        arc = Arc(start_angle=90 * DEGREES, angle=-90 * DEGREES, color=self.right_color)
        arc.replace(box)
        self.play(FadeIn(pythag_eq, UP))
        self.add(arc, arc)
        ...
        self.play(tip_tracker.shift, 0.5 * UP, coord_label.set_opacity, 0, FadeOut(elbow), FadeIn(acute_region), FadeOut(right_words, UP), FadeOut(eq, UP), FadeIn(acute_words, DOWN), FadeIn(gt, DOWN))
        self.wait()
        self.play(tip_tracker.shift, 0.5 * RIGHT)
        self.wait()
        self.add(obtuse_region, coord_label, x_line, y_line, xpy1_line, x_eq_y_line, dot)
        self.play(tip_tracker.shift, 1.5 * DOWN, FadeIn(obtuse_region), FadeOut(acute_words, DOWN), FadeOut(gt, DOWN), FadeIn(obtuse_words, UP), FadeIn(lt, UP))
        self.wait()
        ...
        self.wait()
        self.play(FadeOut(obtuse_words), FadeOut(pythag_eq[:-2]), FadeOut(pythag_eq[-1]), FadeOut(lt))
        self.play(tip_tracker.move_to, equilateral_point + 0.25 * DL, path_arc=30 * DEGREES, run_time=8)

    def get_triangles_and_classes(self):
        original_triangles = VGroup(*[self.get_random_triangle() for x in range(5)])
        original_triangles.submobjects[4] = self.get_random_triangle()
    ...
    def get_triangle(self, x, y):
        ...
        return Polygon(plane.c2p(0, 0), plane.c2p(1, 0), plane.c2p(*tip_point[:2]), color=color, fill_opacity=self.triangle_fill_opacity)
...
class Credits(Scene):
    ...
    def construct(self):
        items = VGroup(OldTexText('Written by\\\\Jayadev Athreya'), OldTexText('Illustrated and Narrated by\\\\Grant Sanderson'), OldTexText('3Blue1Brown\\\\', '\\copyright {} Copyright 2019\\\\', 'www.3blue1brown.com\\\\'))
