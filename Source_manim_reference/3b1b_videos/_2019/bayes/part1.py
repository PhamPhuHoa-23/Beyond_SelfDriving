...
def get_bayes_formula(expand_denominator=False):
    t2c = {'{H}': HYPOTHESIS_COLOR, '{\\neg H}': NOT_HYPOTHESIS_COLOR, '{E}': EVIDENCE_COLOR1}
    isolate = ['P', '\\over', '=', '\\cdot', '+']
    ...
    if expand_denominator:
        ...
        tex += 'P({E})}'
    formula = OldTex(tex, tex_to_color_map=t2c, isolate=isolate)
...
class BayesDiagram(VGroup):
    CONFIG = {'height': 2, 'square_style': {'fill_color': GREY_D, 'fill_opacity': 1, 'stroke_color': WHITE, 'stroke_width': 2}, 'rect_style': {'stroke_color': WHITE, 'stroke_width': 1, 'fill_opacity': 1}, 'hypothesis_color': HYPOTHESIS_COLOR, 'not_hypothesis_color': NOT_HYPOTHESIS_COLOR, 'evidence_color1': EVIDENCE_COLOR1, 'evidence_color2': EVIDENCE_COLOR2, 'not_evidence_color1': NOT_EVIDENCE_COLOR1, 'not_evidence_color2': NOT_EVIDENCE_COLOR2, 'prior_rect_direction': DOWN}
    ...
    def __init__(self, prior, likelihood, antilikelihood, **kwargs):
        ...
        h_rect, nh_rect, he_rect, nhe_rect, hne_rect, nhne_rect = [square.copy().set_style(**self.rect_style) for x in range(6)]
        self.square = square
        self.h_rect = h_rect
    ...
    def add_brace_attrs(self, buff=SMALL_BUFF):
        ...
        attrs = ['h_brace', 'nh_brace', 'he_brace', 'hne_brace', 'nhe_brace', 'nhne_brace']
        for brace, attr in zip(braces, attrs):
            setattr(self, attr, brace)
        return self

    def create_braces(self, buff=SMALL_BUFF):
        kw = {'buff': buff, 'min_num_quads': 1}
        return VGroup(Brace(self.h_rect, self.prior_rect_direction, **kw), Brace(self.nh_rect, self.prior_rect_direction, **kw), Brace(self.he_rect, LEFT, **kw), Brace(self.hne_rect, LEFT, **kw), Brace(self.nhe_rect, RIGHT, **kw), Brace(self.nhne_rect, RIGHT, **kw))
    ...
    def set_prior(self, new_prior):
        ...
        for group, vect, value in [(left_rects, LEFT, p), (right_rects, RIGHT, q)]:
            for rect in group:
                rect.set_width(value * full_width, stretch=True, about_edge=vect)
        self.refresh_braces()
        return self

    def general_set_likelihood(self, new_likelihood, low_rect, high_rect):
        height = self.square.get_height()
        low_rect.set_height(new_likelihood * height, stretch=True, about_edge=DOWN)
        high_rect.set_height((1 - new_likelihood) * height, stretch=True, about_edge=UP)
        self.refresh_braces()
        return self

    def set_likelihood(self, new_likelihood):
        self.general_set_likelihood(new_likelihood, self.he_rect, self.hne_rect)
        return self

    def set_antilikelihood(self, new_antilikelihood):
        self.general_set_likelihood(new_antilikelihood, self.nhe_rect, self.nhne_rect)
        return self
...
class ProbabilityBar(VGroup):
    CONFIG = {'color1': BLUE_D, 'color2': GREY_BROWN, 'height': 0.5, 'width': 6, 'rect_style': {'stroke_width': 1, 'stroke_color': WHITE, 'fill_opacity': 1}, 'include_braces': False, 'brace_direction': UP, 'include_percentages': True, 'percentage_background_stroke_width': 2}
    ...
    def get_braces(self):
        return VGroup(*[Brace(bar, self.brace_direction, min_num_quads=1, buff=SMALL_BUFF) for bar in self.bars])

    def get_percentages(self):
        p = self.p_tracker.get_value()
        labels = VGroup(*[Integer(value, unit='\\%') for value in [np.floor(p * 100), 100 - np.floor(p * 100)]])
...
class Steve(SVGMobject):
    CONFIG = {'file_name': 'steve', 'fill_color': GREY, 'sheen_factor': 0.5, 'sheen_direction': UL, 'stroke_width': 0, 'height': 3, 'include_name': True, 'name': 'Steve'}
...
class LibrarianIcon(SVGMobject):
    CONFIG = {'file_name': 'book', 'stroke_width': 0, 'fill_color': GREY_B, 'sheen_factor': 0.5, 'sheen_direction': UL, 'height': 0.75}

class FarmerIcon(SVGMobject):
    CONFIG = {'file_name': 'farming', 'stroke_width': 0, 'fill_color': GREEN_E, 'sheen_factor': 0.5, 'sheen_direction': UL, 'height': 1.5}

class PitchforkIcon(SVGMobject):
    CONFIG = {'file_name': 'pitch_fork_and_roll', 'stroke_width': 0, 'fill_color': GREY_B, 'sheen_factor': 0.5, 'sheen_direction': UL, 'height': 1.5}

class Person(SVGMobject):
    CONFIG = {'file_name': 'person', 'height': 1.5, 'stroke_width': 0, 'fill_opacity': 1, 'fill_color': GREY_B}

class Librarian(Person):
    CONFIG = {'IconClass': LibrarianIcon, 'icon_style': {'background_stroke_width': 5, 'background_stroke_color': BLACK}}
...
class Farmer(Librarian):
    CONFIG = {'IconClass': FarmerIcon, 'icon_style': {'background_stroke_width': 2}, 'fill_color': GREEN}
...
class IntroduceFormula(Scene):
    ...
    def construct(self):
        ...
        self.add(formula[:6])
        self.play(FadeIn(hyp_label, DOWN), GrowArrow(hyp_arrow), FadeIn(evid_label, UP), GrowArrow(evid_arrow))
        self.wait()
        self.play(ShowCreation(formula.get_part_by_tex('=')), TransformFromCopy(get_formula_slice(0, 1, 2, 5), get_formula_slice(8, 9, 10, 11)))
        lhs_copy = formula[:6].copy()
        likelihood = formula[12:18]
        run_time = 1
        self.play(lhs_copy.next_to, likelihood, UP, run_time=run_time)
        self.play(Swap(lhs_copy[2], lhs_copy[4]), run_time=run_time)
        self.play(lhs_copy.move_to, likelihood, run_time=run_time)
        self.play(ShowCreation(formula.get_part_by_tex('\\over')), TransformFromCopy(get_formula_slice(0, 1, 4, 5), get_formula_slice(19, 20, 21, 22)))
        self.wait()
        self.clear()
        self.play(formula.restore, formula.scale, 1.5, formula.to_edge, UP, FadeOut(VGroup(hyp_arrow, hyp_label, evid_arrow, evid_label)))

class StateGoal(PiCreatureScene, Scene):
    CONFIG = {'default_pi_creature_kwargs': {'color': BLUE_B, 'height': 2}}
    ...
    def construct(self):
        ...
        self.add(you)
        self.play(FadeIn(you_label, LEFT), GrowArrow(you_arrow), you.change, 'pondering')
        self.wait()
        you_label.add(you_arrow)
        self.play(you.change, 'horrified', you.look, DOWN, you.next_to, line.n2p(10), UP, MaintainPositionRelativeTo(you_label, you), FadeInFromPoint(later_label, now_label.get_center()))
        self.wait()
        bubble = you.get_bubble(height=4, width=6)
        bubble.set_fill(opacity=0)
        formula = get_bayes_formula()
        bubble.position_mobject_inside(formula)
        self.play(you.change, 'confused', bubble, ShowCreation(bubble))
        self.play(FadeIn(formula))
        self.play(you.change, 'hooray', formula)
        self.wait(2)
        icons = VGroup(SVGMobject(file_name='science'), SVGMobject(file_name='robot'))
        for icon in icons:
        ...
        icons.to_corner(UL)
        for icon in icons[:2]:
            self.play(Write(icon, run_time=2), you.change, 'thinking', icon)
        self.play(Blink(you), FadeOut(VGroup(line, now_label, later_label, you_label, you_arrow)))
        self.play(FadeIn(gold, LEFT), you.change, 'erm', gold)
        self.play(Blink(you))
        words = VGroup(OldTexText('1988').scale(1.5), OldTexText('Tommy Thompson\\\\and friends'))
        ...
        self.play(FadeIn(words[0], LEFT), you.change, 'tease', words, FadeOut(icons[:2]))
        self.play(FadeIn(words[1], UP))
        self.wait()
        self.add(ship, gold)
        self.play(FadeIn(ship), gold.scale, 0.2, gold.move_to, ship)
        ...
        gold_copy = gold.copy()
        self.play(gold_copy.scale, 3, gold_copy.next_to, amount, LEFT, FadeIn(amount))
        self.play(Blink(you))
        self.wait()
        self.play(LaggedStartMap(FadeOutAndShift, Group(*words, ship_title, ship, gold, gold_copy, amount)))
        ...
        self.remove(bubble)
        self.play(formula.to_corner, UR, FadeOut(you), *[ReplacementTransform(bubble.copy(), point) for point in level_points])
        self.play(Write(title, run_time=1))
        self.wait()
        level_labels = VGroup(OldTexText('What is it saying?'), OldTexText('Why is it true?'), OldTexText('When is it useful?'))
        for lp, ll in zip(level_points, level_labels):
            ll.scale(1.25)
            ll.match_color(lp)
            ll.next_to(lp, RIGHT)
        formula_parts = VGroup(formula.prior, formula.likelihood, formula.p_evidence, formula.posterior).copy()
        ...
        kw = {'tex_to_color_map': {'hypothesis': HYPOTHESIS_COLOR, 'evidence': EVIDENCE_COLOR1}, 'alignment': ''}
        meanings = VGroup(OldTexText('Probability a hypothesis is true\\\\(before any evidence)', **kw), OldTexText('Probability of seeing the evidence \\quad \\\\if the hypothesis is true', **kw), OldTexText('Probability of seeing the evidence', **kw), OldTexText('Probability a hypothesis is true\\\\given some evidence', **kw))
        for meaning, equals in zip(meanings, equal_signs):
            meaning.scale(0.5)
            meaning.next_to(equals, RIGHT)
        self.play(FadeIn(level_labels[0], lag_ratio=0.1), MoveToTarget(formula_parts), LaggedStartMap(FadeInFrom, equal_signs, lambda m: (m, RIGHT)), LaggedStartMap(FadeIn, meanings))
        self.wait()
        ...
        phe_picture = VGroup(diagram.he_rect.copy(), Line().match_width(pe_picture), pe_picture.copy())
        ...
        self.play(FadeOut(meanings), FadeOut(equal_signs[:2]), MoveToTarget(formula_parts), FadeIn(diagram), LaggedStartMap(GrowFromCenter, braces), FadeIn(level_labels[1], lag_ratio=0.1), level_labels[0].set_opacity, 0.5)
        self.play(TransformFromCopy(diagram.he_rect, pe_picture[0]), TransformFromCopy(diagram.nhe_rect, pe_picture[2]), FadeIn(pe_picture[1]))
        self.play(TransformFromCopy(pe_picture, phe_picture[2]), TransformFromCopy(pe_picture[0], phe_picture[0]), ShowCreation(phe_picture[1]))
        self.wait()
        ...
        self.play(LaggedStartMap(FadeOutAndShift, VGroup(VGroup(diagram, braces, formula_parts[:2]), VGroup(formula_parts[2], equal_signs[2], pe_picture), VGroup(formula_parts[3], equal_signs[3], phe_picture)), lambda m: (m, 3 * RIGHT)), FadeIn(level_labels[2], lag_ratio=0.1), level_labels[1].set_opacity, 0.5)
        self.wait()
        self.play(GrowArrow(arrow), level_points.shift, 0.5 * RIGHT, level_labels.shift, 0.5 * RIGHT, level_labels.set_opacity, 1)
        self.wait()
        self.play(Write(steve, run_time=3))
        self.wait()
        self.play(steve.to_corner, UR, Uncreate(arrow), LaggedStartMap(FadeOutAndShift, VGroup(title, formula, *level_points, *level_labels), lambda m: (m, DOWN)))
        self.wait()
...
class DescriptionOfSteve(Scene):
    ...
    def write_description(self):
        ...
        mt_parts = VGroup(description.get_part_by_tex('meek'), description.get_part_by_tex('soul'))
        mt_parts.set_color(WHITE)
        self.add(steve)
        self.play(FadeIn(description), run_time=3, lag_ratio=0.01, rate_func=linear)
        self.wait(3)
        lines = VGroup(*[Line(mob.get_corner(DL), mob.get_corner(DR), color=YELLOW) for mob in mt_parts])
        self.play(ShowCreation(lines), mt_parts.set_color, YELLOW)
        self.play(FadeOut(lines))
        self.wait()

    def compare_probabilities(self):
        bar = ProbabilityBar(0.5, width=10, include_braces=True, percentage_background_stroke_width=2)
        ...
        self.wait()
        self.play(bar.p_tracker.set_value, 0.9, bar.icons[0].set_opacity, 1)
        self.wait()
        self.play(bar.p_tracker.set_value, 0.1, bar.icons[1].set_opacity, 1)
        self.play(LaggedStartMap(FadeInFrom, q_marks, lambda m: (m, UP), run_time=2), ApplyMethod(bar.p_tracker.set_value, 0.7, run_time=8))
        for value in (0.3, 0.7):
            self.play(bar.p_tracker.set_value, 0.3, run_time=7)

    def get_description(self):
        return OldTexText('\n            Steve is very shy and withdrawn,\\\\\n            invariably helpful but with very\\\\\n            little interest in people or in the\\\\\n            world of reality. A meek and tidy\\\\\n            soul, he has a need for order and\\\\\n            structure, and a passion for detail.\\\\\n            ', tex_to_color_map={'shy and withdrawn': BLUE, 'meek and tidy': YELLOW, 'soul': YELLOW}, alignment='')

class IntroduceKahnemanAndTversky(DescriptionOfSteve):

    def construct(self):
        images = Group(ImageMobject('kahneman'), ImageMobject('tversky'))
        danny, amos = images
        images.set_height(3.5)
        images.arrange(DOWN, buff=0.5)
        images.to_edge(LEFT, buff=MED_LARGE_BUFF)
        names = VGroup(OldTexText('Daniel\\\\Kahneman', alignment=''), OldTexText('Amos\\\\Tversky', alignment=''))
        ...
        books = Group(ImageMobject('thinking_fast_and_slow'), ImageMobject('undoing_project'))
        books.set_height(5)
        books.arrange(RIGHT, buff=0.5)
        books.to_edge(RIGHT, buff=MED_LARGE_BUFF)
        self.play(FadeIn(danny, DOWN), FadeIn(danny.name, LEFT))
        self.play(FadeIn(amos, UP), FadeIn(amos.name, LEFT))
        self.wait()
        ...
        bubble = ThoughtBubble(width=7, height=4)
        ...
        lil_bubble[:-1].rotate(PI, axis=UR, about_point=lil_bubble[:-1].get_corner(UL))
        ...
        bar.add_icons(LibrarianIcon(height=1), FarmerIcon(height=1))
        bar.scale(0.5)
        bar.next_to(randy, RIGHT, buff=0.75)
        bar.update()
        self.play(LaggedStartMap(MoveToTarget, images), LaggedStartMap(FadeOutAndShiftDown, books), LaggedStartMap(FadeOut, Group(prize, *names)))
        self.play(DrawBorderThenFill(bubble), FadeIn(randy, UR, rate_func=squish_rate_func(smooth, 0.5, 1), run_time=2))
        self.play(DrawBorderThenFill(lil_bubble), randy.change, 'pondering')
        self.play(Blink(randy))
        self.play(Write(librarian))
        self.add(bar, lil_bubble, librarian)
        self.play(FadeIn(bar))
        self.play(bar.p_tracker.set_value, 1 / 6, randy.change, 'thinking', bar)
        self.play(Blink(randy))
        self.wait()
        description = self.get_description()
        ...
        self.play(frame.scale, 0.5, frame.move_to, bubble[-1], DOWN, frame.shift, 0.75 * LEFT, FadeOut(bubble), FadeOut(images), FadeOut(lil_bubble), FadeOut(librarian), FadeIn(description, lag_ratio=0.05), randy.change, 'pondering', description, run_time=6)
        self.play(randy.change, 'happy', description)
        self.play(description.get_part_by_tex('shy').set_color, BLUE, lag_ratio=0.1)
        self.play(description.get_part_by_tex('meek').set_color, YELLOW, description.get_part_by_tex('soul').set_color, YELLOW, lag_ratio=0.1)
        self.play(bar.p_tracker.set_value, 0.9, FadeIn(lil_bubble), Write(librarian))
        self.play(ShowCreationThenFadeOut(book_border))
        self.play(Blink(randy))
        self.play(FadeInFromDown(steve), randy.look_at, steve)
        self.play(randy.change, 'tease', steve, FadeOut(librarian), FadeIn(seems_bookish))
        lil_bubble.add(seems_bookish)
        self.wait()
        self.play(Blink(randy))
        self.play(LaggedStartMap(FadeOutAndShift, lil_bubble, lambda m: (m, LEFT), run_time=1), bar.p_tracker.set_value, 1 / 6, randy.change, 'confused', bar)
        ...
        self.play(TransformFromCopy(steve, steve_copy), LaggedStartMap(MoveToTarget, VGroup(steve, book, farm)), LaggedStartMap(FadeOutAndShift, description, lambda m: (m, LEFT)), FadeOut(randy, LEFT), FadeOut(bar, LEFT))
...
class CorrectViewOfFarmersAndLibrarians(Scene):
    ...
    def construct(self):
        ...
        words = VGroup(OldTexText('Stereotype'), OldTexText('Unexpected'))
        for arrow, word, vect in zip(arrows, words, [LEFT, RIGHT]):
            word.scale(1.5)
            word.next_to(arrow, vect)
            self.play(GrowArrow(arrow), FadeIn(word, UP))
        self.wait()
        librarian = Librarian()
        farmer = Farmer()
        ...
        self.remove(*icons)
        self.play(LaggedStartMap(FadeOut, VGroup(steves, arrows, words)), Restore(librarian), Restore(farmer), run_time=1)
        self.play(LaggedStartMap(FadeIn, farmers[1:]))
        self.wait()
        self.add(farmer_count)
        self.play(ShowIncreasingSubsets(farmer_outlines), int_func=np.ceil, rate_func=linear, run_time=2)
        self.play(FadeOut(farmer_outlines))
        self.wait()
        farmers.save_state()
        farmers.generate_target()
        farmers.target.scale(0.5, about_edge=UL)
        new_farmers = VGroup(*it.chain(*[farmers.target.copy().next_to(farmers.target, vect, buff=SMALL_BUFF) for vect in [RIGHT, DOWN]]))
        new_farmers[-10:].align_to(new_farmers, RIGHT)
        new_farmers[-10:].align_to(new_farmers[-20], UP)
        farmer_count.clear_updaters()
        self.play(MoveToTarget(farmers), ShowIncreasingSubsets(new_farmers), ChangeDecimalToValue(farmer_count, 60))
        self.wait()
        self.play(FadeOut(new_farmers), Restore(farmers), ChangeDecimalToValue(farmer_count, 20))
        self.wait()
        farmers.generate_target()
        ...
        self.play(MoveToTarget(librarian), MoveToTarget(farmers), ReplacementTransform(farmer_count, f_brace[-1]), GrowFromCenter(f_brace[:-1]))
...
class ComplainAboutNotKnowingTheStats(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play_student_changes('sassy', 'sassy')
        self.play(self.teacher.change, 'hesitant')
        self.look_at(self.screen)
        self.wait(3)
        self.teacher_says('No, but did you\\\\think to estimate it?', bubble_config={'width': 4.5, 'height': 3.5})
...
class ReasonByRepresentativeSample(CorrectViewOfFarmersAndLibrarians):
    ...
    def construct(self):
        ...
        for brace in braces:
            ...
            brace.count.add_updater(lambda c: c.set_value(c.brace.count_tracker.get_value()).next_to(c.brace, UP, SMALL_BUFF))
        ...
        self.play(*[FadeIn(new_rows, lag_ratio=0.1) for new_rows in new_people], *[ApplyMethod(brace.next_to, new_rows, UP, {'buff': SMALL_BUFF}) for brace, new_rows in zip(braces, new_people)], *[ApplyMethod(brace.count_tracker.set_value, 10 * brace.count_tracker.get_value()) for brace in braces])
        farmers = VGroup(farmers, *new_farmers)
        librarians = VGroup(librarians, *new_librarians)
        everyone = VGroup(farmers, librarians)
        big_rect = SurroundingRectangle(everyone, buff=0.05, stroke_width=1, stroke_color=WHITE, fill_opacity=1, fill_color=GREY_E)
        ...
        for rect, color in zip([dl_rect, dr_rect], colors):
            rect.set_fill(color)
        all_rects = VGroup(left_rect, right_rect, dl_rect, ul_rect, dr_rect, ur_rect)
        all_rects.set_opacity(0)
        self.add(all_rects, everyone)
        self.play(left_rect.set_opacity, 1, right_rect.set_opacity, 1)
        self.wait()
        ...
        for rect, to_fade in [(dl_rect, librarians[4:]), (dr_rect, farmers[1:])]:
            ...
            self.play(Restore(rect), Restore(rect.brace), to_fade.set_opacity, 0.1)
            self.wait()
        ...
        self.play(l_brace.next_to, librarians[:4], UP, SMALL_BUFF, l_brace.count_tracker.set_value, 4, ShowIncreasingSubsets(highlighted_librarians))
        self.play(FadeOut(highlighted_librarians))
        self.play(f_brace.next_to, farmers[:1], UP, SMALL_BUFF, f_brace.count_tracker.set_value, 20, ShowIncreasingSubsets(highlighted_farmers), run_time=2)
        self.play(FadeOut(highlighted_farmers))
        self.wait()
        equation = OldTex('P\\left(', '\\text{Librarian }', '\\text{given }', '\\text{description}', '\\right)', '=', '{4', '\\over', ' 4', '+', '20}', '\\approx', '16.7\\%')
        equation.set_color_by_tex_to_color_map({'Librarian': HYPOTHESIS_COLOR, 'description': EVIDENCE_COLOR1})
        ...
        self.wait()
        self.play(TransformFromCopy(l_brace.count, equation.get_parts_by_tex('4')[0]), Write(equation.get_part_by_tex('\\over')))
        self.play(Write(equation.get_part_by_tex('+')), TransformFromCopy(f_brace.count, equation.get_part_by_tex('20')), TransformFromCopy(l_brace.count, equation.get_parts_by_tex('4')[1]))
        self.wait()
        self.play(FadeIn(equation[-2:]))
        self.wait()
        axes = Axes(x_min=0, x_max=10, y_min=0, y_max=100, y_axis_config={'unit_size': 0.07, 'tick_frequency': 10}, axis_config={'include_tip': False})
        ...
        title.shift(RIGHT)
        axes.add(title)
        lines = VGroup(Line(axes.c2p(3, 0), axes.c2p(3, 40)), Line(axes.c2p(7, 0), axes.c2p(7, 10)))
        ...
        self.play(*[FadeOut(mob) for mob in everything], FadeIn(axes), FadeIn(icons), *[TransformFromCopy(pr, r) for pr, r in zip(pre_rects, rects)], FadeOut(people_copy))
        self.play(*[ShowCreation(rect.line) for rect in rects])
        self.wait()
        self.play(FadeOut(axes), FadeOut(rects[0].line), FadeOut(rects[1].line), FadeOut(icons), *[ReplacementTransform(r, pr) for pr, r in zip(pre_rects, rects)], *[FadeIn(mob) for mob in everything])
        self.remove(*pre_rects)
        self.wait()
        prior_equation = OldTex('P\\left(', '\\text{Librarian}', '\\right)', '=', '{1', '\\over', '21}', '\\approx', '4.8\\%')
        prior_equation.set_color_by_tex_to_color_map({'Librarian': HYPOTHESIS_COLOR})
        ...
        group.shift((equation.get_part_by_tex('\\over').get_x() - prior_equation.get_part_by_tex('\\over').get_x()) * RIGHT)
        ...
        prior_arrow = Arrow(prior_label.get_right(), prior_equation.get_left(), buff=SMALL_BUFF)
        self.play(VGroup(equation_rect, equation).shift, prior_rect.get_height() * DOWN, FadeIn(prior_rect), FadeIn(prior_equation), FadeIn(prior_label, RIGHT), GrowArrow(prior_arrow))
        self.wait()
...
class NewEvidenceUpdatesPriorBeliefs(DescriptionOfSteve):
    ...
    def construct(self):
        ...
        for icon in icons:
            icon.set_height(0.5)
        kw = {'include_braces': True, 'width': 11, 'height': 0.75}
        ...
        self.play(FadeOut(cross), Transform(arrow, new_arrow), evid.scale, 0.6, evid.move_to, new_arrow, ReplacementTransform(librarian, low_bar.icons[0]), FadeIn(bars))
...
class HeartOfBayesTheorem(Scene):
    ...
    def construct(self):
        ...
        title.to_edge(UP)
        diagram = self.get_diagram(prior=0.1, likelihood=0.4, antilikelihood=0.1)
        ...
        label2 = OldTexText('All possibilities\\\\', 'fitting the evidence', tex_to_color_map={'evidence': EVIDENCE_COLOR1})
        ...
        frac_box = always_redraw(lambda: DashedVMobject(SurroundingRectangle(fraction, buff=MED_SMALL_BUFF, stroke_width=2, stroke_color=WHITE), num_dashes=100))
        prob = OldTex('P\\left(', '{\\text{Librarian }', '\\text{given}', '\\over', '\\text{the evidence}}', '\\right)')
        ...
        self.play(FadeIn(diagram), FadeInFromDown(label1))
        self.play(TransformFromCopy(diagram, diagram_copy), TransformFromCopy(label1[0], label2[0]))
        self.play(Restore(label2), ReplacementTransform(diagram_copy, restricted_diagram))
        self.wait()
        self.play(TransformFromCopy(restricted_diagram.he_rect, fraction[0]), TransformFromCopy(restricted_diagram.he_rect, fraction[2][0]), TransformFromCopy(restricted_diagram.nhe_rect, fraction[2][2]), ShowCreation(fraction[1]), Write(fraction[2][1]))
        self.add(fraction)
    ...
    def get_diagram(self, prior, likelihood, antilikelihood):
        diagram = BayesDiagram(prior, likelihood, antilikelihood, not_evidence_color1=GREY, not_evidence_color2=GREEN_E)
    ...
    def get_geometric_fraction(self, diagram):
        fraction = VGroup(diagram.he_rect.copy(), Line(LEFT, RIGHT), VGroup(diagram.he_rect.copy(), OldTex('+'), diagram.nhe_rect.copy()).arrange(RIGHT, buff=SMALL_BUFF))
...
class WhenDoesBayesApply(DescriptionOfSteve):
    ...
    def construct(self):
        ...
        title.to_edge(UP)
        self.add(title)
        all_words = VGroup(OldTexText('You have a\\\\', 'hypothesis'), OldTexText("You've observed\\\\some ", 'evidence'), OldTex('\\text{You want}\\\\', 'P', '(', 'H', '|', 'E', ')\\\\', 'P', '\\left(', '\\substack{\\text{Hypothesis} \\\\', '\\textbf{given} \\\\', '\\, \\text{the evidence} \\,}', '\\right)'))
        for words in all_words:
            words.set_color_by_tex_to_color_map({'hypothesis': HYPOTHESIS_COLOR, 'H': HYPOTHESIS_COLOR, 'evidence': EVIDENCE_COLOR1, 'E': EVIDENCE_COLOR1})
        goal = all_words[2]
        prob = goal[1:7]
        big_prob = goal[7:]
        ...
        self.play(LaggedStart(FadeIn(hypothesis_icon[0], DOWN), Write(hypothesis_icon[1]), FadeIn(hypothesis_icon[2], UP), run_time=1))
        self.wait()
        self.play(FadeInFromDown(all_words[1]))
        self.play(FadeIn(evidence_icon), lag_ratio=0.1, run_time=2)
        self.wait()
        self.play(FadeInFromDown(VGroup(goal[0], big_prob)))
        self.wait()
        self.play(Restore(big_prob), *[TransformFromCopy(big_prob[i], prob[i]) for i in [0, 1, -1]])
        self.play(LaggedStart(*[TransformFromCopy(big_prob[i][j], prob[i]) for i, j in [(2, 0), (3, 1), (4, 3)]]))
        self.wait()
        rects = VGroup(*[SurroundingRectangle(goal.get_part_by_tex(tex), buff=0.05, stroke_width=2, stroke_color=RED) for tex in ('|', 'given')])
        ...
        self.play(LaggedStartMap(FadeOut, everything, run_time=2), prob.copy().to_corner, UR)

    def get_hypothesis_icon(self):
        group = VGroup(Steve().set_height(1.5), OldTex('\\updownarrow'), LibrarianIcon().set_color(YELLOW_D))
        group.arrange(DOWN)
        return group
...
class CreateFormulaFromDiagram(Scene):
    CONFIG = {'tex_to_color_map': {'P': WHITE, 'H': HYPOTHESIS_COLOR, 'E': EVIDENCE_COLOR1, '\\neg': RED}, 'bayes_diagram_config': {'prior': 1 / 21, 'likelihood': 0.4, 'antilikelihood': 0.1, 'not_evidence_color1': GREY, 'not_evidence_color2': GREY_D, 'prior_rect_direction': UP}}
    ...
    def construct(self):
        ...
        self.wait()
        self.play(diagram.hypothesis_split.set_opacity, 1, FadeIn(diagram.h_brace), FadeInFromDown(prior_label))
        self.wait()
        self.play(FadeIn(prior_example))
        self.play(LaggedStartMap(Indicate, diagram.people[::10], color=BLUE))
        self.wait()
        self.play(FadeIn(prior_word, RIGHT), GrowArrow(prior_arrow))
        self.wait()
        prior_group = VGroup(prior_word, prior_arrow, prior_label, prior_example, diagram.h_brace)
        ...
        like_arrow = Arrow(like_word.get_bottom(), like_label.get_top(), buff=0.2)
        ...
        self.play(diagram.he_rect.set_opacity, 1, diagram.hne_rect.set_opacity, 1, MoveToTarget(hne_people), GrowFromCenter(diagram.he_brace), FadeIn(like_label, RIGHT), FadeIn(like_example, RIGHT))
        self.wait()
        self.play(ShowCreationThenFadeAround(like_label.get_part_by_tex('E'), surrounding_rectangle_config={'color': EVIDENCE_COLOR1}))
        self.play(WiggleOutThenIn(like_label.get_part_by_tex('|')))
        self.play(ShowCreationThenFadeAround(like_label.get_part_by_tex('H')))
        self.wait()
        self.play(diagram.people[10:].set_opacity, 0.2, diagram.nh_rect.set_opacity, 0.2, FadeIn(limit_word, DOWN), GrowArrow(limit_arrow), rate_func=there_and_back_with_pause, run_time=6)
        self.wait()
        self.play(Write(like_word, run_time=1), GrowArrow(like_arrow))
        self.wait()
        ...
        self.play(Restore(diagram.nhe_rect), GrowFromCenter(diagram.nhe_brace), MoveToTarget(nhne_people), FadeIn(anti_label, LEFT), FadeIn(anti_example, LEFT))
        diagram.nhne_rect.set_opacity(1)
        self.wait()
        self.play(ShowCreationThenFadeAround(anti_label[2], surrounding_rectangle_config={'color': EVIDENCE_COLOR1}))
        self.play(ShowCreationThenFadeAround(anti_label[4:6], surrounding_rectangle_config={'color': RED}))
        self.wait()
        ...
        denom = VGroup(he_group.copy(), OldTex('+'), nhe_group.copy())
        denom.arrange(RIGHT)
        answer = VGroup(he_group.copy(), Underline(denom, stroke_width=2), denom)
        answer.arrange(DOWN)
        answer.scale(0.5)
        ...
        post_arrow = Arrow(post_word.get_bottom(), posterior.target[1].get_top(), buff=0.2)
        ...
        diagram.add(diagram.h_brace, diagram.he_brace, diagram.nhe_brace)
        ...
        self.play(ShowCreation(dividing_line), MoveToTarget(diagram), MaintainPositionRelativeTo(like_group, like_label), MaintainPositionRelativeTo(VGroup(prior_word, prior_arrow), prior_label), MoveToTarget(posterior), ReplacementTransform(posterior_words, equals))
        ...
        self.play(LaggedStart(TransformFromCopy(he_group, answer[2][0]), GrowFromCenter(answer[2][1]), TransformFromCopy(nhe_group, answer[2][2])))
        self.wait()
        formula = OldTex('=', '{P(H) P(E | H)', '\\over', 'P(H) P(E | H) + P(\\neg H) P(E | \\neg H)}', tex_to_color_map=t2c)
        formula.scale(0.9)
        ...
        h_rect = Rectangle(stroke_color=YELLOW, stroke_width=3, fill_color=YELLOW, fill_opacity=0.25)
        h_rect.replace(diagram.square, stretch=True)
        s_rect = SurroundingRectangle(answer[0])
        diagram.refresh_braces()
        nh_group = VGroup(diagram.nh_brace, OldTex('P(\\neg H)', tex_to_color_map=t2c), OldTex('= 20 / 21'))
        nh_group[1].next_to(nh_group[0], UP, SMALL_BUFF)
        nh_group[2].next_to(nh_group[1], RIGHT, SMALL_BUFF)
        self.play(Write(formula.get_part_by_tex('=')), Write(formula.get_part_by_tex('\\over')), run_time=1)
        self.play(ShowCreation(s_rect))
        self.wait()
        self.play(FadeIn(np1), FadeIn(top_brace), FadeIn(top_count))
        self.wait()
        self.play(h_rect.replace, diagram.h_rect, {'stretch': True})
        self.play(TransformFromCopy(prior_label, ph), top_brace.become, Brace(VGroup(np1, ph), UP, buff=SMALL_BUFF), ChangeDecimalToValue(top_count, 10))
        self.wait()
        self.play(h_rect.replace, diagram.he_rect, {'stretch': True})
        self.play(TransformFromCopy(like_label, peh), top_brace.become, Brace(VGroup(np1, peh), UP, buff=SMALL_BUFF), ChangeDecimalToValue(top_count, 4))
        self.wait()
        self.play(s_rect.move_to, answer[2][0], TransformFromCopy(np1, np2), TransformFromCopy(ph, ph2), TransformFromCopy(peh, peh2))
        self.wait()
        self.play(FadeOut(h_rect), s_rect.become, SurroundingRectangle(answer[2][2]), FadeOut(prior_group), FadeIn(nh_group))
        self.wait()
        h_rect.replace(diagram.square, stretch=True)
        self.play(FadeIn(np3), FadeIn(low_brace), FadeIn(low_count))
        self.play(h_rect.replace, diagram.nh_rect, {'stretch': True})
        self.play(TransformFromCopy(nh_group[1], pnh), low_brace.become, Brace(VGroup(np3, pnh), DOWN, buff=SMALL_BUFF), ChangeDecimalToValue(low_count, 200))
        self.play(h_rect.replace, diagram.nhe_rect, {'stretch': True})
        self.play(TransformFromCopy(anti_label, penh), low_brace.become, Brace(VGroup(np3, penh), DOWN, buff=SMALL_BUFF), ChangeDecimalToValue(low_count, 20))
        self.wait()
        self.play(FadeOut(nh_group), FadeOut(s_rect), FadeOut(h_rect), FadeIn(prior_group))
        self.wait()
        self.play(ShowCreation(crosses), FadeOut(low_brace), FadeOut(top_brace), FadeOut(low_count), FadeOut(top_count))
        self.wait()
        ...
        self.wait()
        self.play(Restore(pe), ReplacementTransform(big_denom_rect, lil_denom_rect), Transform(formula_rect, SurroundingRectangle(simpler_form, color=TEAL)), bayes_words.match_x, simpler_form)
        self.remove(pe)
        ...
        self.play(copies.arrange, RIGHT, {'buff': LARGE_BUFF}, copies.move_to, DOWN, copies.to_edge, RIGHT, LARGE_BUFF)
        self.wait()
        self.play(he_group_copy.next_to, VGroup(ph2, peh2), DOWN)
        self.play(nhe_group_copy.next_to, VGroup(pnh, penh), DOWN)
        self.wait()
        self.play(FadeOut(copies), FadeOut(lil_denom_rect))
        self.wait()
        self.play(GrowArrow(post_arrow), FadeIn(post_word, RIGHT), FadeOut(formula_rect), FadeOut(bayes_words))
        self.wait()
        randy = Randolph()
        randy.flip()
        ...
        self.play(Blink(randy))
        self.play(ReplacementTransform(prior_rect, post_rect), randy.look_at, post_rect)
        ...
        everything = Group(*self.get_mobjects())
        self.play(LaggedStartMap(FadeOutAndShiftDown, everything), MoveToTarget(to_move, rate_func=squish_rate_func(smooth, 0.5, 1)), run_time=2)
...
class DiscussFormulaAndAreaModel(CreateFormulaFromDiagram):
    CONFIG = {'bayes_diagram_config': {'prior_rect_direction': DOWN}}

    def construct(self):
        t2c = self.tex_to_color_map
        formula = OldTex('P(H | E)', '=', '{P(H) P(E | H)', '\\over', 'P(H) P(E | H) + P(\\neg H) P(E | \\neg H)}', tex_to_color_map=t2c)
        ...
        alt_rhs = OldTex('{P(H)P(E|H)', '\\over', 'P(E)}', '=', tex_to_color_map=t2c)
        ...
        self.play(VGroup(rhs, s_rect).next_to, alt_rhs, RIGHT, SMALL_BUFF, GrowFromCenter(alt_rhs))
        ...
        diagram.refresh_braces()
        diagram.add(diagram.h_brace, diagram.he_brace, diagram.nhe_brace)
        h_label, he_label, nhe_label = labels = [OldTex(tex, tex_to_color_map=t2c) for tex in ['P(H)', 'P(E|H)', 'P(E|\\neg H)']]
        ...
        self.play(FadeIn(diagram), ReplacementTransform(VGroup(s_rect, s_rect.copy()), VGroup(he_rect_copy, nhe_rect_copy)))
        self.wait()
        self.play(LaggedStart(ApplyMethod(he_rect_copy.next_to, rhs[12:22], DOWN), ApplyMethod(nhe_rect_copy.next_to, rhs[23:], DOWN), lag_ratio=0.7, run_time=1.5))
        self.wait()
        ...
        self.wait()
        self.play(ReplacementTransform(arrow1, arrow2), FadeOut(words1), FadeIn(words2), big_rect.set_stroke, WHITE, 0, big_rect.set_fill, BLACK, 0.7)
        self.wait()
        to_fade = VGroup(diagram.evidence_split, diagram.he_brace, diagram.nhe_brace, he_label, nhe_label)
        to_fade.save_state()
        h_part = VGroup(diagram.hypothesis_split, diagram.h_brace, h_label)
        people = self.get_diagram().people
        people.set_width(diagram.square.get_width() - 0.05)
        people.move_to(diagram.square)
        sides = VGroup(DashedLine(diagram.square.get_corner(UL), diagram.square.get_corner(UR)), DashedLine(diagram.square.get_corner(UL), diagram.square.get_corner(DL)))
        sides.set_stroke(YELLOW, 4)
        ones = VGroup(OldTex('1').next_to(diagram.square, UP), OldTex('1').next_to(diagram.square, LEFT))
        self.play(to_fade.set_opacity, 0, h_part.set_opacity, 0, diagram.square.set_opacity, 1, ShowIncreasingSubsets(people))
        self.play(FadeOut(people))
        self.play(LaggedStartMap(ShowCreation, sides, lag_ratio=0.8), LaggedStartMap(FadeIn, ones, lag_ratio=0.8))
        self.wait()
        self.play(h_part.set_opacity, 1)
        diagram.square.set_opacity(0)
        self.wait()
        self.play(FadeOut(sides), FadeOut(ones))
        self.wait()
        VGroup(to_fade[1:], to_fade[0][::2]).stretch(0, 1, about_edge=DOWN)
        self.play(Restore(to_fade), diagram.hypothesis_split.set_opacity, 0, diagram.hne_rect.set_opacity, 0.2, diagram.nhne_rect.set_opacity, 0.2)
        self.wait()
        post_bar = always_redraw(lambda: self.get_posterior_bar(diagram.he_rect, diagram.nhe_rect))
        post_label = OldTex('P(H|E)', tex_to_color_map=t2c)
        post_label.add_updater(lambda m: m.next_to(post_bar.brace, DOWN))
        self.play(FadeOut(words2), FadeOut(arrow2), TransformFromCopy(diagram.he_rect, post_bar.rects[0]), TransformFromCopy(diagram.nhe_rect, post_bar.rects[1]), FadeIn(post_bar.brace), FadeIn(post_label))
        self.add(post_bar)
        self.wait()
        self.play(diagram.set_likelihood, 0.8, rate_func=there_and_back, run_time=4)
        self.wait()
        self.play(diagram.set_antilikelihood, 0.4)
        self.wait()
        self.play(diagram.set_likelihood, 0.8, diagram.set_antilikelihood, 0.05)
        self.wait()
        self.play(diagram.set_likelihood, 0.4, diagram.set_antilikelihood, 0.1)
        self.wait()
        self.play(big_rect.set_width, rhs.get_width() + 0.7, {'about_edge': RIGHT, 'stretch': True})
        self.wait()
    ...
    def get_posterior_bar(self, he_rect, nhe_rect):
        ...
        new_he_rect = Rectangle(width=new_he_width, height=new_he_height).match_style(he_rect)
        new_nhe_rect = Rectangle(width=new_nhe_width, height=new_nhe_height).match_style(nhe_rect)
        rects = VGroup(new_he_rect, new_nhe_rect)
        rects.arrange(RIGHT, buff=0)
        brace = Brace(new_he_rect, DOWN, buff=SMALL_BUFF, min_num_quads=2)
...
class RandomShapes(Scene):
    ...
    def construct(self):
        ...
        events = VGroup(e_part, h_rect, circle, pi, tri)
        last = VMobject()
        for event in events:
...
class BigArrow(Scene):
    ...
    def construct(self):
        arrow = Arrow(3 * DOWN + 4 * LEFT, 3 * RIGHT + DOWN, path_arc=50 * DEGREES)
...
class UsesOfBayesTheorem(Scene):
    ...
    def construct(self):
        ...
        bar = ProbabilityBar(0.1, percentage_background_stroke_width=1, include_braces=False)
        ...
        for person in [scientist, programmer]:
            ...
            rhs_copy = rhs.copy()
            rhs_copy.add_to_back(SurroundingRectangle(rhs_copy, fill_color=BLACK, fill_opacity=0.8, stroke_color=WHITE, stroke_width=2))
        ...
        self.play(bubble_group.scale, 1.4, bubble_group.move_to, randy.get_corner(UL), DR, randy.look_at, bubble, FadeOut(arrow))
        self.wait()
        self.play(Blink(randy))
        ...
        self.play(Blink(randy))
        self.wait()
        self.play(LaggedStartMap(FadeOutAndShift, VGroup(*people, bubble_group), lambda m: (m, DOWN), lag_ratio=0.15))
...
class AskAboutWhenProbabilityIsIntuitive(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.change, 'speaking', self.change_students('pondering', 'sassy', 'happy', look_at=self.screen))
        self.wait(2)
        self.play(self.teacher.change, 'raise_right_hand', FadeIn(words, DOWN), self.change_students('erm', 'pondering', 'confused'))
        self.wait(2)
        self.play(words.scale, 2, words.center, words.to_edge, UP, self.teacher.change, 'pondering', 3 * UP, self.change_students('pondering', 'thinking', 'thinking', look_at=3 * UP))
        self.wait(6)
...
class IntroduceLinda(DescriptionOfSteve):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeInFrom, images, lambda m: (m, LEFT), lag_ratio=0.3, run_time=3))
        self.wait()
        steve = Steve()
        steve.set_height(3)
        ...
        self.play(LaggedStart(FadeIn(steve, LEFT), FadeIn(steve_words, LEFT)))
        self.wait()
        linda = Linda()
        ...
        self.play(LaggedStart(FadeOut(steve_words, 2 * RIGHT), FadeOut(steve, 2 * RIGHT), FadeIn(linda, 2 * LEFT), lag_ratio=0.15))
        self.wait()
        self.play(FadeIn(linda_words), lag_ratio=0.1, run_time=6, rate_func=linear)
        self.wait()
        options = VGroup(OldTexText('1) Linda is a bank teller.'), OldTexText('2) Linda is a bank teller and is active\\\\', '\\phantom{2)} in the feminist movement.', alignment=''))
        options.arrange(DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
        options.to_edge(DOWN, buff=LARGE_BUFF)
        self.play(linda.match_height, linda_words, linda.next_to, linda_words, LEFT, LARGE_BUFF, LaggedStartMap(FadeOutAndShift, images, lambda m: (m, 2 * LEFT)))
        ...
        self.play(MoveToTarget(options), MoveToTarget(rect), VFadeIn(rect), FadeIn(result, LEFT), GrowArrow(arrow))
        self.wait()
        ...
        self.play(FadeOut(linda_words), TransformFromCopy(rect, fb_set), ReplacementTransform(VectorizedPoint(rect.get_center()), fb_words))
        self.add(fb_set, fb_words)
        self.wait()
        self.add(b_set, fb_set, fb_words)
        self.play(DrawBorderThenFill(b_set), TransformFromCopy(options[0][0][10:], b_words[0]))
        sets_group = VGroup(b_set, b_words, fb_set, fb_words)
        self.add(sets_group)
        self.wait()
        number = result[0]
        self.play(LaggedStart(FadeOut(arrow), FadeOut(result[1]), FadeOut(rect), FadeOut(options[0]), FadeOut(options[1])), number.scale, 1.5, number.move_to, DOWN)
        self.play(ChangeDecimalToValue(number, 0), UpdateFromAlphaFunc(number, lambda m, a: m.set_color(interpolate_color(RED, GREEN, a))), run_time=2)
        self.wait()
        self.play(FadeOut(number), FadeOut(sets_group), FadeIn(linda_words))
        ...
        new_options = VGroup(OldTexText('1) Bank tellers? \\underline{\\qquad} of 100', **kw), OldTexText('2) Bank tellers and active in the', ' feminist movement? \\underline{\\qquad} of 100', **kw))
    ...
    def get_images(self):
        ...
        images.arrange(DOWN, buff=0.5)
        images.to_edge(LEFT, buff=MED_LARGE_BUFF)
        names = VGroup(OldTexText('Kahneman', alignment=''), OldTexText('Tversky', alignment=''))
        for name, image in zip(names, images):
...
class LindaDescription(IntroduceLinda):
    ...
    def construct(self):
        ...
        highlighted_part = VGroup(*words.get_part_by_tex('deeply'), *words.get_part_by_tex('discrimination'))
        self.add(words)
        self.play(FadeIn(words), run_time=3, lag_ratio=0.01, rate_func=linear)
        self.wait(1)
...
class AlternatePhrasings(PiCreatureScene):
    ...
    def construct(self):
        ...
        phrases = VGroup(OldTexText('40 out of 100'), OldTex('40\\%'), OldTex('0.4'), OldTexText("What's more likely$\\dots$"))
        ...
        self.play(FadeIn(phrases[0], UP), randy.change, 'pondering')
        self.play(DrawBorderThenFill(bubble, lag_ratio=0.1), FadeIn(people, lag_ratio=0.1), randy.change, 'thinking', people)
        self.wait()
        self.play(FadeIn(phrases[1], UP), randy.change, 'confused', phrases[1], FadeOut(people), ApplyFunction(push_down, phrases[0]), FadeIn(bar))
        self.play(bar.p_tracker.set_value, 0.4)
        bar.clear_updaters()
        self.play(FadeIn(phrases[2], UP), ApplyFunction(push_down, phrases[:2]), FadeOut(bar.percentages), randy.change, 'guilty')
        self.wait()
        bar.remove(bar.percentages)
        self.play(FadeIn(phrases[3], UP), ApplyFunction(push_down, phrases[:3]), FadeOut(bar), FadeIn(stereotypes), randy.change, 'shruggie', stereotypes)
        self.wait(6)
...
class WhenDiscreteChunksArentSoClean(Scene):
    ...
    def construct(self):
        ...
        target_p = 0.3612
        rain, sun = icon_templates = VGroup(SVGMobject('rain_cloud'), SVGMobject('sunny'))
        ...
        for i, square in enumerate(squares):
            ...
            q_mark.scale(0.8)
            q_marks.add(q_mark)
        p_label = VGroup(OldTex('P', '(', '\\text{Rain}', ')', '='), DecimalNumber(40, unit='\\%', num_decimal_places=2))
        ...
        self.add(percentage)
        self.play(ChangeDecimalToValue(percentage, 100 * target_p), FadeIn(partial_rects[30:40]), FadeIn(q_marks[30:40], lag_ratio=0.3))
        self.wait(2)
        ...
        self.play(FadeIn(rects), MoveToTarget(rain), MoveToTarget(sun), GrowFromCenter(brace), p_label.shift, brace.get_top() + MED_SMALL_BUFF * UP - percentage.get_bottom())
        self.wait()
        full_width = rects.get_width()
        rain.add_updater(lambda m: m.move_to(l_rect))
        sun.add_updater(lambda m: m.move_to(r_rect))
        r_rect.add_updater(lambda m: m.set_width(full_width - l_rect.get_width(), about_edge=RIGHT, stretch=True))
        brace.add_updater(lambda m: m.match_width(l_rect, stretch=True))
        brace.add_updater(lambda m: m.next_to(l_rect, UP, SMALL_BUFF))
        percentage.add_updater(lambda m: m.set_value(100 * l_rect.get_width() / full_width))
        percentage.add_updater(lambda m: m.next_to(brace, UP, MED_SMALL_BUFF))
        self.play(MaintainPositionRelativeTo(p_label[0], percentage), l_rect.stretch, 2, 0, {'about_edge': LEFT}, run_time=8, rate_func=there_and_back)
...
class RandomnessVsProportions(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(unc_word, LEFT), Write(arrows[1]))
        ...
        self.play(grid_copy.set_width, 0.8 * example[1].get_width(), grid_copy.next_to, example[1], DOWN)
    ...
    def get_die_faces(self):
        ...
        edge_groups = [(ORIGIN,), (UL, DR), (UL, ORIGIN, DR), (UL, UR, DL, DR), (UL, UR, ORIGIN, DL, DR), (UL, UR, LEFT, RIGHT, DL, DR)]
        arrangements = VGroup(*[VGroup(*[dot.copy().move_to(square.get_bounding_box_point(ec)) for ec in edge_group]) for edge_group in edge_groups])
        square.set_width(1)
    ...
    def get_dice_diagram(self):
        grid = VGroup(*[VGroup(*[Square() for x in range(6)]).arrange(RIGHT, buff=0) for y in range(6)]).arrange(DOWN, buff=0)
...
class BayesTheoremOnProportions(Scene):
    ...
    def construct(self):
        ...
        self.add(group)
        self.wait()
        self.play(group.to_edge, LEFT, MaintainPositionRelativeTo(equals, group), VFadeIn(equals), MaintainPositionRelativeTo(h_line, group), VFadeIn(h_line))
        ...
        self.play(FadeOut(people), FadeOut(num_people), FadeIn(diagram), FadeIn(num_diagram))
        self.wait()
        ...
        E_words = OldTexText('...among cases where\\\\$E$ is True', tex_to_color_map={'$E$': BLUE})
        E_words.next_to(E_arrow, DOWN)
        H_arrow = Vector(DOWN, color=YELLOW)
        H_arrow.next_to(H_part[0], UP)
        H_words = OldTexText('How often is\\\\$H$ True...', tex_to_color_map={'$H$': YELLOW})
        ...
        self.play(formula.set_opacity, 0.5, ApplyMethod(E_part.set_stroke, YELLOW, 3, {'background': True}, rate_func=there_and_back), FadeIn(denom_rect), ShowCreation(E_arrow), FadeIn(E_words, UP), low_diagram_rects.set_stroke, TEAL, 3)
        self.wait()
        self.play(FadeOut(E_part), FadeIn(H_part), FadeOut(denom_rect), FadeIn(numer_rect), ShowCreation(H_arrow), FadeIn(H_words, DOWN), FadeOut(title, UP), low_diagram_rects.set_stroke, WHITE, 1, top_diagram_rects.set_stroke, YELLOW, 3)
        self.wait()
...
class GlimpseOfNextVideo(GraphScene):
    CONFIG = {'x_axis_label': '', 'y_axis_label': '', 'x_min': 0, 'x_max': 15, 'x_axis_width': 12, 'y_min': 0, 'y_max': 1.0, 'y_axis_height': 6, 'y_tick_frequency': 0.125, 'add_x_coords': True, 'formula_position': ORIGIN, 'dx': 0.2}

    def setup(self):
        super().setup()
        self.setup_axes()
        self.y_axis.add_numbers(0.25, 0.5, 0.75, 1, num_decimal_places=2, direction=LEFT)
    ...
    def construct(self):
        ...
        self.play(FadeInFromDown(formula[4:10]), Transform(rects1, rects2))
        self.wait()
        self.play(rects1.stretch, 1 / pe, 1, {'about_edge': DOWN}, Write(formula[10:], run_time=1))
        self.wait()

    def get_formula(self):
        formula = OldTex('p(H) p(E|H) \\over p(E)', tex_to_color_map={'H': HYPOTHESIS_COLOR, 'E': EVIDENCE_COLOR1}, isolate=list('p(|)'))
...
class QuestionSteveConclusion(HeartOfBayesTheorem, DescriptionOfSteve):
    ...
    def construct(self):
        ...
        steve.shift(UP)
        self.add(steve)
        kt = Group(ImageMobject('kahneman'), ImageMobject('tversky'))
        kt.arrange(DOWN)
        kt.set_height(6)
        randy = Randolph()
        ...
        self.play(steve.scale, 0.5, steve.to_corner, DL, FadeIn(randy), FadeInFromDown(kt, lag_ratio=0.3))
        self.play(randy.change, 'sassy')
        self.wait()
        self.play(FadeIn(people, RIGHT, lag_ratio=0.01), randy.change, 'raise_left_hand', people)
        self.wait()
        self.play(ShowCreation(cross), randy.change, 'angry')
        self.wait()
        people.add(cross)
        self.play(people.scale, 0.3, people.to_corner, UL, steve.scale, 1.5, steve.next_to, randy.get_corner(UL), LEFT, randy.change, 'pondering', steve)
        ...
        group.arrange(RIGHT)
        group.to_edge(RIGHT)
        self.play(MoveToTarget(kt), MoveToTarget(steve), randy.shift, 2 * LEFT, randy.change, 'erm', kt.target, FadeOut(people, 2 * LEFT))
        ...
        self.play(LaggedStartMap(FadeOutAndShift, kt, lambda m: (m, 3 * UR)), MoveToTarget(steve), randy.to_edge, DOWN, randy.change, 'happy', steve.target, FadeIn(jessy), FadeIn(morty))
        ...
        self.play(FadeIn(diagram), *[ApplyMethod(pi.change, 'pondering', diagram) for pi in pis])
        self.play(diagram.set_prior, 0.5)
        self.play(Blink(jessy))
        self.wait()
        self.play(Blink(morty))
        self.play(morty.change, 'raise_right_hand', diagram, ApplyMethod(diagram.set_prior, 0.9, run_time=2))
        self.play(Blink(randy))
        self.wait()
        description = self.get_description()
        description.scale(0.5)
        description.to_corner(UL)
        self.play(FadeIn(description), *[ApplyMethod(pi.change, 'sassy', description) for pi in pis])
        self.play(diagram.set_likelihood, 0.2, run_time=2)
        self.play(diagram.set_antilikelihood, 0.5, run_time=2)
        self.play(Blink(jessy))
        self.play(Blink(randy))
        self.wait()
        ...
        self.play(FadeInFromDown(formula), LaggedStart(*[ApplyMethod(pi.change, 'thinking', formula) for pi in pis]))
        self.play(Blink(randy))
        self.play(LaggedStartMap(FadeOutAndShiftDown, VGroup(description, *pis, steve)), MoveToTarget(diagram, run_time=3), ApplyMethod(formula.scale, 1.5, {'about_edge': UR}, run_time=2.5))
        self.wait()
        kw = {'run_time': 2}
...
class ReprogrammingThought(Scene):
    ...
    def construct(self):
        ...
        self.play(GrowFromCenter(arrow), LaggedStartMap(FadeInFromDown, q_marks[0]), run_time=1)
        ...
        self.play(DrawBorderThenFill(VGroup(*reversed(bubble))), group.shift, 2 * DOWN)
        self.play(FadeIn(diagram))
        self.wait()
        self.play(q_marks.scale, 1.5, q_marks.space_out_submobjects, 1.5, q_marks.set_opacity, 0)
        self.remove(q_marks)
        self.wait()
        ...
        self.play(FadeIn(like_outline), ApplyMethod(diagram.set_likelihood, 0.2, run_time=2))
        self.play(FadeOut(like_outline))
        self.wait()

class MassOfEarthEstimates(GlimpseOfNextVideo):
    CONFIG = {'add_x_coords': False, 'formula_position': 2 * UP + 0.5 * RIGHT, 'dx': 0.05}

    def setup(self):
        super().setup()
        earth = SVGMobject(file_name='earth', height=1.5, fill_color=BLACK)
        earth.set_stroke(width=0)
        circle = Circle(stroke_width=3, stroke_color=GREEN, fill_opacity=1, fill_color=BLUE_C)
    ...
    def get_formula(self):
        formula = OldTex('p(M) p(\\text{data}|M) \\over p(\\text{data})', tex_to_color_map={'M': HYPOTHESIS_COLOR, '\\text{data}': EVIDENCE_COLOR1}, isolate=list('p(|)'))
        formula.move_to(self.formula_position)
        return formula
...
class Thumbnail(Scene):
    ...
    def construct(self):
        ...
        diagram.set_height(3)
        diagram.add_brace_attrs()
        braces = VGroup(diagram.h_brace, diagram.he_brace, diagram.nhe_brace)
        diagram.add(*braces)
        kw = {'tex_to_color_map': {'H': YELLOW, 'E': BLUE, '\\neg': RED}}
        labels = VGroup(OldTex('P(H)', **kw), OldTex('P(E|H)', **kw), OldTex('P(E|\\neg H)', **kw))
        labels.scale(1)
        ...
        formula.scale(1.5)
        formula.to_corner(UL)
        frac = VGroup(diagram.he_rect.copy(), Line(ORIGIN, 4 * RIGHT).set_stroke(WHITE, 3), VGroup(diagram.he_rect.copy(), OldTex('+'), diagram.nhe_rect.copy()).arrange(RIGHT))
        frac.arrange(DOWN)
        equals = OldTex('=')
        equals.next_to(formula, RIGHT)
...
class Thumbnail2(Scene):
    ...
    def construct(self):
        ...
        self.add(diagram)
        frac = VGroup(diagram.he_rect.copy(), Line(LEFT, RIGHT).set_width(7), VGroup(diagram.he_rect.copy(), OldTex('+', font_size=120), diagram.nhe_rect.copy()).arrange(RIGHT)).arrange(DOWN, buff=0.5)
        frac.set_width(5)
        ...
        words.to_edge(UP)
        arrow = Arrow(words, frac, thickness=0.1)
