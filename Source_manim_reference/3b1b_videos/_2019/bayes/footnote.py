...
def get_bayes_formula():
    return OldTex('P(A|B) = {P(A)P(B|A) \\over P(B)}', tex_to_color_map={'A': YELLOW, 'B': BLUE}, isolate=list('P(|)'))
...
class ThisIsAFootnote(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        over = formula[17]
        pb = formula[18:23]
        self.play(FadeInFromDown(image_group), self.change_students('pondering', 'hooray', 'tease', look_at=image))
        self.play(Write(asterisk), self.teacher.change, 'speaking')
        self.play(self.change_students('thinking', 'erm', 'thinking'))
        self.wait(3)
        self.play(self.teacher.change, 'raise_right_hand', FadeInFromDown(formula), self.change_students(*3 * ['pondering']))
        self.wait()
        parts = VGroup(pb, pab, eq, pa, pba)
        parts.generate_target()
        parts.target.arrange(RIGHT, buff=SMALL_BUFF)
        parts.target.move_to(self.hold_up_spot)
        self.play(MoveToTarget(parts, path_arc=-30 * DEGREES), FadeOut(over), self.teacher.change, 'pondering')
        self.wait()
        p_both = OldTex('P(A \\text{ and } B)', tex_to_color_map={'A': YELLOW, 'B': BLUE})
        eq2 = OldTex('=')
        ...
        p_both.move_to(eq)
        eq2.move_to(eq)
        self.play(MoveToTarget(full_equation), FadeOut(image_group, 2 * LEFT), FadeOut(asterisk, 2 * LEFT), self.teacher.look_at, 4 * UP, self.change_students('thinking', 'erm', 'confused', look_at=4 * UP))
        self.wait(2)

class ShowTwoPerspectives(Scene):
    CONFIG = {'pa': 1 / 3, 'pb': 1 / 4, 'p_both': 1 / 6, 'diagram_height': 4}

    def construct(self):
        formula = self.get_formula()
        venn_diagram = self.get_venn_diagram()
        venn_diagram.next_to(formula, DOWN, LARGE_BUFF)
        arrow = Arrow(formula[3].get_bottom(), venn_diagram.get_center())
        self.add(formula)
        self.play(formula[:3].set_opacity, 0.2, formula[-3:].set_opacity, 0.2)
        for i in (0, 1):
            self.play(FadeIn(venn_diagram[0][i]), Write(venn_diagram[1][i]), run_time=1)
        ...
        d1_line = DashedLine(diagram1.h_rect.get_corner(UR), diagram1.h_rect.get_corner(DR))
        ...
        self.play(FadeOut(venn_diagram[0][1]), FadeOut(venn_diagram[1][1]), FadeOut(arrow), formula[4:6].set_opacity, 1)
        diagram1.pa_label.update()
        self.play(FadeIn(diagram1.nh_rect), ReplacementTransform(venn_diagram[0][0], diagram1.h_rect), ReplacementTransform(venn_diagram[1][0], diagram1.pa_label.get_part_by_tex('A')), FadeIn(diagram1.h_brace), FadeIn(diagram1.pa_label[0]), FadeIn(diagram1.pa_label[2]), ShowCreation(d1_line))
        self.add(diagram1.pa_label)
        self.wait()
        self.play(FadeIn(space_words), ShowCreation(space_outline))
        self.play(FadeOut(space_words), FadeOut(space_outline))
        self.wait()
        ...
        self.play(Restore(diagram1), Restore(diagram1.he_brace), VFadeIn(diagram1.he_brace), VFadeIn(diagram1.pba_label), formula.pba.set_opacity, 1)
        self.wait()
        diagram1_copy = diagram1.deepcopy()
        diagram2 = self.get_diagram2()
        d2_line = DashedLine(diagram2.b_rect.get_corner(UL), diagram2.b_rect.get_corner(UR))
        d2_line.set_stroke(BLACK, 2)
        for rect in [diagram2.ba_rect, diagram2.nba_rect]:
            rect.save_state()
            rect.stretch(0.001, 0, about_edge=LEFT)
        self.play(diagram1_copy.move_to, diagram2, formula.pb.set_opacity, 1)
        self.play(diagram1_copy.set_likelihood, self.pb, diagram1_copy.set_antilikelihood, self.pb, VFadeOut(diagram1_copy), FadeIn(diagram2), TransformFromCopy(formula.pb, diagram2.pb_label), FadeIn(diagram2.pb_brace), ShowCreation(d2_line))
        self.wait()
        self.play(formula.pab.set_opacity, 1, formula.eq1.set_opacity, 1)
        self.play(TransformFromCopy(formula.pab, diagram2.pab_label), FadeIn(diagram2.pab_brace), Restore(diagram2.ba_rect), Restore(diagram2.nba_rect))
        self.wait()

    def get_formula(self):
        kw = {'tex_to_color_map': {'A': YELLOW, 'B': BLUE}}
        parts = VGroup(*[OldTex(tex, **kw) for tex in ['P(B)', 'P(A|B)', '=', 'P(A \\text{ and } B)', '=', 'P(A)', 'P(B|A)']])
    ...
    def get_venn_diagram(self):
        c1 = Circle(radius=2.5, stroke_width=2, stroke_color=YELLOW, fill_opacity=0.5, fill_color=YELLOW)
        c1.flip(RIGHT)
        c1.rotate(3 * TAU / 8)
        c2 = c1.copy()
        c2.set_color(BLUE)
        ...
        titles = VGroup(OldTex('A'), OldTex('B'))
    ...
    def get_diagram1(self):
        ...
        diagram.pa_label.add_updater(lambda m: m.next_to(diagram.h_brace, DOWN, SMALL_BUFF))
        diagram.pba_label.add_updater(lambda m: m.next_to(diagram.he_brace, LEFT, SMALL_BUFF))
        return diagram
    ...
    def get_diagram2(self):
        ...
        result = VGroup(square.set_opacity(0), b_rect, nb_rect, ba_rect, nba_rect)
        ...
        VGroup(result, pb_brace, pab_brace, pb_label, pab_label).to_edge(LEFT)
        return result
...
class Rearrange(ShowTwoPerspectives):
    ...
    def construct(self):
        ...
        eq3 = eq1.copy()
        new_line = VGroup(formula.pb, formula.pab, eq3, formula.pa, formula.pba)
        ...
        self.add(formula)
        self.play(MoveToTarget(new_line), MoveToTarget(eq1), MoveToTarget(eq2))
        self.wait()
        over.move_to(VGroup(pa, pba))
        self.play(ApplyMethod(pb.next_to, over, DOWN, path_arc=30 * DEGREES), VGroup(pa, pba).next_to, over, UP, ShowCreation(over), FadeOut(VGroup(eq1, eq2)))
        self.wait(2)
        ...
        self.play(LaggedStart(MoveToTarget(over, path_arc=-30 * DEGREES), MoveToTarget(numer, path_arc=-30 * DEGREES), ApplyMethod(pa.next_to, over.target, DOWN), ApplyMethod(pba.next_to, eq3, RIGHT), lag_ratio=0.3))
        self.wait(2)
        ...
        self.play(LaggedStartMap(GrowFromCenter, braces, lag_ratio=0.3), LaggedStartMap(GrowFromCenter, values, lag_ratio=0.3), FadeOut(p_both))
        self.wait()
...
class ClassLooking(TeacherStudentsScene):
    ...
    def construct(self):
        self.play(self.teacher.change, 'pondering', self.change_students('pondering', 'confused', 'sassy', look_at=self.screen))
        self.wait(5)
        self.play(self.teacher.change, 'raise_right_hand')
        self.play(self.change_students('thinking', 'pondering', 'pondering', look_at=self.hold_up_spot + 2 * UP))
        self.wait(3)
...
class LandscapeOfTools(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        bayes = group[0].copy()
        self.play(self.teacher.change, 'raise_right_hand', self.change_students(*3 * ['confused'], look_at=group), FadeInFromDown(bayes))
        self.remove(bayes)
        ...
        self.play(bayes.move_to, bubble.get_bubble_center(), DrawBorderThenFill(bubble), self.teacher.change, 'happy', self.change_students('pondering', 'erm', 'erm', look_at=bubble))
        self.play_all_student_changes('thinking', look_at=bayes)
        self.wait()
        self.play(FadeOut(bayes), bubble.set_fill, BLACK, 0.2, bubble.set_stroke, WHITE, 1, self.change_students('pleading', 'guilty', 'guilty'), self.teacher.change, 'hesitant')
        self.wait(2)

    def get_formulas(self):
        group = VGroup(get_bayes_formula(), OldTex('P(X = k) = {\\lambda^k \\over k!}', 'e^{-\\lambda}', tex_to_color_map={'k': YELLOW, '\\lambda': GREEN}), OldTex('{1 \\over \\sigma\\sqrt{2\\pi}}', 'e^{\\frac{1}{2}\\left({(x - \\mu) \\over \\sigma}\\right)^2}', tex_to_color_map={'\\sigma': GREEN, '\\mu': BLUE}), OldTex('P(X = k) =', '\\left({n \\over k}\\right)', 'p^k(1-p)^{n-k}', tex_to_color_map={'\\over': BLACK, 'p': WHITE, 'k': YELLOW, 'n': BLUE, 'k': GREEN}), OldTex('E[X + Y] = E[x] + E[y]'), OldTex('\\text{Var}(X + Y) = \\text{Var}(x) + \\text{Var}(y) + 2\\text{Cov}(X, Y)'), OldTex('H = \\sum_{i} -p_i \\log', '(p_i)', tex_to_color_map={'p_i': YELLOW}), OldTex('{n \\choose k}', '{B(k + \\alpha, n -k + \\beta) \\over B(\\alpha, \\beta)}', tex_to_color_map={'\\alpha': BLUE, '\\beta': YELLOW}), OldTex('P(d) = \\log_{10}\\left(1 + {1 \\over d}\\right)', tex_to_color_map={'d': BLUE}), OldTex('\\text{Cov}(X, Y) = \\sum_{i, j} p({x}_i, {y}_j)({x}_i - \\mu_{x})({y}_j - \\mu_{y})', tex_to_color_map={'{x}': BLUE, '{y}': RED}))
...
class TemptingFormula(ShowTwoPerspectives, RandomnessVsProportions):
    ...
    def construct(self):
        kw = {'tex_to_color_map': TEX_TO_COLOR_MAP, 'isolate': list('P()')}
        formula = VGroup(OldTex('P(A \\text{ and } B)', **kw), OldTex('='), OldTex('P(A)P(B)', '\\,', '\\,', **kw))
        ...
        self.add(formula)
        for i in (0, 1):
            self.play(DrawBorderThenFill(venn[0][i]), FadeIn(venn[1][i]))
        self.play(and_part.scale, 0.5, and_part.move_to, venn)
        ...
        prob = OldTex('P(', 'OO', ')', '= \\frac{1}{4} \\cdot \\frac{1}{4} = \\frac{1}{16}')
        pair.move_to(prob[1])
        prob.submobjects[1] = pair
        prob.scale(1.5)
        prob.next_to(grid, RIGHT, LARGE_BUFF)
        self.play(FadeOut(venn), LaggedStartMap(FadeInFromDown, people))
        self.play(WiggleOutThenIn(heart))
        self.wait()
        self.play(MoveToTarget(people), TransformFromCopy(people, left_people), Write(grid), FadeIn(prob[:3]), run_time=1)
        self.add(both_square, pairs)
        self.play(LaggedStartMap(MoveToTarget, pairs, path_arc=30 * DEGREES), both_square.set_stroke, YELLOW, 5, both_square.set_fill, YELLOW, 0.25)
        ...
        ht_prob = OldTex('P(\\text{TT}) = \\frac{1}{2} \\cdot \\frac{1}{2} = \\frac{1}{4}', tex_to_color_map={'\\text{TT}': RED})
        ht_prob.scale(1.5)
        ht_prob.next_to(ht_grid, RIGHT, LARGE_BUFF)
        ht_grid_group = VGroup(ht_grid, ht_labels, left_ht_labels, ht_both_square, ht_pairs)
        self.play(FadeOut(grid_group), FadeOut(prob), FadeIn(ht_grid_group), FadeIn(ht_prob))
        self.wait()
        ...
        dice_prob = OldTex('P(', 'OO', ') = \\frac{1}{6} \\cdot \\frac{1}{6} = \\frac{1}{36}')
        ...
        dice_grid_group = VGroup(dice_grid, dice_labels, left_dice_labels, dice_both_square, dice_pairs)
        self.play(FadeOut(ht_grid_group), FadeOut(ht_prob), FadeIn(dice_grid_group), FadeIn(dice_prob))
        self.wait()
        self.play(FadeOut(dice_grid_group), FadeOut(dice_prob), FadeIn(prob), FadeIn(grid_group))
        self.wait()
        cross = Cross(prob[3])
        for pair in pairs:
        ...
        self.play(FadeOut(prob), FadeOut(cross))
        self.play(ShowCreation(row_rect))
        self.wait()
        self.play(grid[0][0].stretch, 2, 0, {'about_edge': LEFT}, grid[0][1:].stretch, 2 / 3, 0, {'about_edge': RIGHT}, both_square.stretch, 2, 0, {'about_edge': LEFT}, *[ApplyMethod(grid[i][0].stretch, 2 / 3, 0, {'about_edge': LEFT}) for i in range(1, 4)], *[ApplyMethod(grid[i][1:].stretch, 10 / 9, 0, {'about_edge': RIGHT}) for i in range(1, 4)])
        self.wait()
        ...
        self.play(ShowCreationThenFadeAround(formula), FadeOut(q_marks))
        self.play(ShowCreation(cross))
        self.wait()
        self.play(TransformFromCopy(formula, real_formula), grid_group.scale, 0.7, grid_group.to_corner, DL)
        self.play(FadeIn(real_rect), FadeIn(check, RIGHT))
        self.wait()
        ...
        B_rect = SurroundingRectangle(Bs, stroke_color=BLUE, buff=0)
        B_rect.scale(1.1, about_edge=LEFT)
        B_rect.set_fill(BLUE, 0.5)
        B_rect.set_stroke(width=0)
        big_rect = SurroundingRectangle(VGroup(ht_grid_group, dice_grid_group), buff=MED_LARGE_BUFF, color=BLUE)
        self.play(FadeIn(ht_grid_group))
        self.play(FadeIn(dice_grid_group))
        self.wait()
        self.add(B_rect, Bs.copy())
        self.play(FadeIn(B_rect), FadeIn(big_rect), Transform(cross, small_cross), FadeOut(real_rect))
        self.wait()

    def get_grid(self, n, m, height=4):
        grid = VGroup(*[VGroup(*[Square() for x in range(m)]).arrange(RIGHT, buff=0) for y in range(n)]).arrange(DOWN, buff=0)
...
class DiseaseBayes(Scene):
    ...
    def construct(self):
        formula = OldTex('P(D | +) = {P(D) P(+ | D) \\over P(+)}', tex_to_color_map={'D': YELLOW, '+': BLUE}, isolate=list('P(|)='))
        formula.scale(2.5)
        ...
        lhs.center()
        sicky = lhs[2]
        sick_words = OldTexText('You are sick', tex_to_color_map={'sick': SICKLY_GREEN})
        ...
        arrow_groups = VGroup(sick_words, sick_arrow, positive_words, positive_arrow)
        sicky.save_state()
        sicky.change('happy')
        sicky.set_color(BLUE)
        self.play(FadeInFromDown(lhs))
        self.play(Restore(sicky), GrowArrow(sick_arrow), FadeInFromDown(sick_words))
        self.play(GrowArrow(positive_arrow), FadeIn(positive_words, UP))
        self.wait(2)
        self.play(Restore(lhs), MaintainPositionRelativeTo(arrow_groups, lhs), FadeIn(formula[6]))

        def get_formula_slice(*indices):
            return VGroup(*[formula[i] for i in indices])
        self.play(TransformFromCopy(get_formula_slice(0, 1, 2, 5), get_formula_slice(8, 9, 10, 11)))
        ...
        self.play(ShowCreation(formula.get_part_by_tex('\\over')), TransformFromCopy(get_formula_slice(12, 13, 14, 17), get_formula_slice(19, 20, 21, 22)))
        self.wait()
...
class EndScreen(Scene):
    ...
    def construct(self):
        ...
        video_rect = Rectangle(width=width, height=height, fill_color=BLACK, fill_opacity=1)
        video_rect.shift(UP)
