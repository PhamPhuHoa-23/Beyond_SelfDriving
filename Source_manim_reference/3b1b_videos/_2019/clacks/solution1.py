...
class FromPuzzleToSolution(MovingCameraScene):
    ...
    def construct(self):
        ...
        titles = VGroup(OldTexText('Puzzle'), OldTexText('Solution'))
        images = Group(ImageMobject('BlocksAndWallExampleMass16'), ImageMobject('AnalyzeCircleGeometry'))
        ...
        self.play(frame.replace, images[1], run_time=3)
        self.wait()

class BlocksAndWallExampleMass16(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 16, 'velocity': -1.5}}, 'wait_time': 25}

class Mass16WithElasticLabel(Mass1e1WithElasticLabel):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 16}}}

class BlocksAndWallExampleMass64(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 64, 'velocity': -1.5}}, 'wait_time': 25}

class BlocksAndWallExampleMass1e4(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 10000.0, 'velocity': -1.5}}, 'wait_time': 25}

class BlocksAndWallExampleMassMillion(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 1000000.0, 'velocity': -0.9, 'label_text': '$100^{3}$ kg'}}, 'wait_time': 30, 'million_fade_time': 4, 'min_time_between_sounds': 0.002}
...
class BlocksAndWallExampleMassTrillion(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 1000000000000.0, 'velocity': -1}}, 'wait_time': 30, 'min_time_between_sounds': 0.001}
...
class V1EqualsV2Line(Scene):
    ...
    def construct(self):
        ...
        self.play(Write(eq, run_time=1), ShowCreation(line))
        self.wait()
...
class AskAboutFindingNewVelocities(Scene):
    CONFIG = {'floor_y': -3, 'wall_x': -6.5, 'wall_height': 7, 'block1_config': {'mass': 10, 'fill_color': BLUE_E, 'velocity': -1}, 'block2_config': {'mass': 1}, 'block1_start_x': 7, 'block2_start_x': 3, 'v_arrow_scale_value': 1.0, 'is_halted': False}
    ...
    def add_floor(self):
        floor = self.floor = Line(self.wall_x * RIGHT, FRAME_WIDTH / 2 * RIGHT)
        floor.shift(self.floor_y * UP)
        self.add(floor)

    def add_wall(self):
        wall = self.wall = Wall(height=self.wall_height)
        wall.move_to(self.wall_x * RIGHT + self.floor_y * UP, DR)
        self.add(wall)
    ...
    def add_blocks(self):
        ...
        for block in blocks:
            ...
            block.arrow = arrow
            block.add(arrow)
            block.v_label = DecimalNumber(block.velocity, num_decimal_places=2, background_stroke_width=2)
            block.v_label.set_color(RED)
            block.add(block.v_label)
        blocks.add_updater(self.update_blocks)
        self.add(blocks, block2.arrow, block1.arrow, block2.v_label, block1.v_label)

    def add_velocity_phase_space_point(self):
        self.vps_point = VectorizedPoint([np.sqrt(self.block1.mass) * self.block1.velocity, np.sqrt(self.block2.mass) * self.block2.velocity, 0])
    ...
    def ask_about_transfer(self):
        energy_expression, momentum_expression = self.get_energy_and_momentum_expressions()
        ...
        momentum_words.next_to(energy_words, DOWN, buff=0.7)
        ...
        self.play(randy.restore, randy.change, 'pondering', velocity_labels[0])
        self.halt()
        ...
        self.play(Blink(randy))
        self.wait()
        self.play(FadeIn(energy_words, RIGHT), FadeInFromDown(energy_expression), FadeOut(randy))
        self.wait()
        self.play(FadeIn(momentum_words, RIGHT), FadeInFromDown(momentum_expression))
        self.wait()
    ...
    def show_ms_and_vs(self):
        ...
        m1_labels = VGroup(block1.label, energy_expression.get_part_by_tex('m_1'), momentum_expression.get_part_by_tex('m_1'))
        m2_labels = VGroup(block2.label, energy_expression.get_part_by_tex('m_2'), momentum_expression.get_part_by_tex('m_2'))
        v1_labels = VGroup(block1.v_label, energy_expression.get_part_by_tex('v_1'), momentum_expression.get_part_by_tex('v_1'))
        v2_labels = VGroup(block2.v_label, energy_expression.get_part_by_tex('v_2'), momentum_expression.get_part_by_tex('v_2'))
        label_groups = VGroup(m1_labels, m2_labels, v1_labels, v2_labels)
        for group in label_groups:
            group.rects = VGroup(*map(SurroundingRectangle, group))
        for group in label_groups:
            self.play(LaggedStartMap(ShowCreation, group.rects, lag_ratio=0.8, run_time=1))
    ...
    def show_value_on_equations(self):
        ...
        block1.save_state()
        block2.save_state()
        v_terms, momentum_v_terms = [VGroup(*[expr.get_part_by_tex('v_{}'.format(d)) for d in [1, 2]]) for expr in [energy_expression, momentum_expression]]
        v_braces = VGroup(*[Brace(term, UP, buff=SMALL_BUFF) for term in v_terms])
        ...
        energy_const_brace, momentum_const_brace = [Brace(expr.get_part_by_tex('const'), UP, buff=SMALL_BUFF) for expr in [energy_expression, momentum_expression]]
        sqrt_m_vect = np.array([np.sqrt(self.block1.mass), np.sqrt(self.block2.mass), 0])
        ...
        VGroup(energy_const_brace, energy_decimal, momentum_const_brace, momentum_decimal).set_color(YELLOW)
        self.play(ShowCreationThenFadeAround(energy_expression), momentum_text.set_fill, {'opacity': 0.25}, FadeOut(self.all_velocity_labels))
        self.play(*[*map(GrowFromCenter, v_braces), *map(VFadeIn, v_decimals), GrowFromCenter(energy_const_brace), FadeIn(energy_decimal)])
        ...
        self.halt()
        self.play(*[momentum_text.set_fill, {'opacity': 1}, FadeOut(energy_text), FadeOut(energy_const_brace), FadeOut(energy_decimal), GrowFromCenter(momentum_const_brace), FadeIn(momentum_decimal), *[ApplyMethod(b.next_to, vt, UP, SMALL_BUFF) for b, vt in zip(v_braces, momentum_v_terms)]])
        self.unhalt()
        self.vps_point.restore()
        momentum_decimal.add_updater(lambda m: m.set_value(get_momentum()))
        momentum_decimal.add_updater(lambda m: m.next_to(momentum_const_brace, UP, SMALL_BUFF))
        for x in range(9):
            self.go_through_next_collision()
        self.wait(10)

    def get_energy_and_momentum_expressions(self):
        tex_to_color_map = {'v_1': RED_B, 'v_2': RED_B, 'm_1': BLUE_C, 'm_2': BLUE_C}
        energy_expression = OldTex('\\frac{1}{2} m_1 (v_1)^2 + ', '\\frac{1}{2} m_2 (v_2)^2 = ', '\\text{const.}', tex_to_color_map=tex_to_color_map)
        momentum_expression = OldTex('m_1 v_1 + m_2 v_2 =', '\\text{const.}', tex_to_color_map=tex_to_color_map)
        return VGroup(energy_expression, momentum_expression)
    ...
    def get_next_velocity_labels_animation(self):
        return FadeIn(self.get_next_velocity_labels(), LEFT, run_time=0.5)
    ...
    def get_velocity_labels(self, v1=None, v2=None):
        ...
        labels = VGroup(OldTex('v_1 = {:.2f}'.format(v1)), OldTex('v_2 = {:.2f}'.format(v2)))
        labels.arrange(DOWN, buff=MED_SMALL_BUFF, aligned_edge=LEFT)
        labels.scale(0.9)
        for label in labels:
    ...
    def update_blocks(self, blocks, dt):
        for block, velocity in zip(blocks, self.get_velocities()):
            ...
            block.arrow.put_start_and_end_on(center, center + self.block_to_v_vector(block))
            max_height = 0.25
            block.v_label.set_value(block.velocity)
            if block.v_label.get_height() > max_height:
                block.v_label.set_height(max_height)
            block.v_label.next_to(block.arrow.get_start(), UP, buff=SMALL_BUFF)
        return blocks
    ...
    def get_velocities(self):
        ...
        return [vps_coords[0] / np.sqrt(m1), vps_coords[1] / np.sqrt(m2)]
...
class IntroduceVelocityPhaseSpace(AskAboutFindingNewVelocities):
    CONFIG = {'wall_height': 1.5, 'floor_y': -3.5, 'block1_start_x': 5, 'block2_start_x': 0, 'axes_config': {'x_axis_config': {'x_min': -5.5, 'x_max': 6}, 'y_axis_config': {'x_min': -3.5, 'x_max': 4}, 'axis_config': {'unit_size': 0.7}}, 'momentum_line_scale_factor': 4}
    ...
    def show_two_equations(self):
        ...
        equations.shift(UP)
        v1_terms, v2_terms = v_terms = VGroup(*[VGroup(*[expr.get_parts_by_tex(tex) for expr in equations]) for tex in ('v_1', 'v_2')])
        for eq in equations:
            eq.highlighted_copy = eq.copy()
            eq.highlighted_copy.set_fill(opacity=0)
            eq.highlighted_copy.set_stroke(YELLOW, 3)
        self.add(equations)
        self.play(ShowCreation(equations[0].highlighted_copy), run_time=0.75)
        self.play(FadeOut(equations[0].highlighted_copy), ShowCreation(equations[1].highlighted_copy), run_time=0.75)
        self.play(FadeOut(equations[1].highlighted_copy), run_time=0.75)
        self.play(LaggedStartMap(Indicate, v_terms, lag_ratio=0.75, rate_func=there_and_back))
        self.wait()
    ...
    def draw_axes(self):
        ...
        x_axis_labels = VGroup(OldTex('x = ', 'v_1'), OldTex('x = ', '\\sqrt{m_1}', '\\cdot', 'v_1'))
        y_axis_labels = VGroup(OldTex('y = ', 'v_2'), OldTex('y = ', '\\sqrt{m_2}', '\\cdot', 'v_2'))
        ...
        self.play(equations.scale, 0.8, equations.to_corner, UL, {'buff': MED_SMALL_BUFF}, Write(axes))
        self.wait()
        self.play(momentum_expression.set_fill, {'opacity': 0.2}, Indicate(energy_expression, scale_factor=1.05))
        self.wait()
        for n in range(2):
            tex = 'v_{}'.format(n + 1)
            self.play(TransformFromCopy(energy_expression.get_part_by_tex(tex), axis_labels[n][0].get_part_by_tex(tex)), FadeInFromDown(axis_labels[n][0][0]))
        vps_dot = self.vps_dot = Dot(color=RED)
        vps_dot.set_stroke(BLACK, 2, background=True)
        vps_dot.add_updater(lambda m: m.move_to(axes.coords_to_point(*self.get_velocities())))
        vps_point = self.vps_point
        vps_point.save_state()
        kwargs = {'path_arc': PI / 3, 'run_time': 2}
        target_locations = [6 * RIGHT + 2 * UP, 6 * RIGHT + 2 * DOWN, 6 * LEFT + 1 * UP]
        self.add(vps_dot)
        for target_location in target_locations:
            self.play(vps_point.move_to, target_location, **kwargs)
    ...
    def draw_ellipse(self):
        ...
        ellipse.replace(Polygon(*[axes.coords_to_point(x, y * np.sqrt(mass_ratio)) for x, y in [(1, 0), (0, 1), (-1, 0), (0, -1)]]), stretch=True)
        self.play(Indicate(energy_expression, scale_factor=1.05))
        self.add(ellipse, vps_dot)
        self.play(ShowCreation(ellipse), Rotating(vps_point, about_point=ORIGIN), run_time=6, rate_func=lambda t: smooth(t, 3))
        self.wait()
    ...
    def rescale_axes(self):
        ...
        xy_equation = self.xy_equation = OldTex('\\frac{1}{2}', '\\left(', 'x^2', '+', 'y^2', '\\right)', '=', '\\text{const.}')
        ...
        for i, labels, block in zip(it.count(), axis_labels, self.blocks):
            ...
            self.play(ReplacementTransform(labels[0][0], labels[1][0]), ReplacementTransform(labels[0][-1], labels[1][-1]), FadeInFromDown(labels[1][1:-1]), ellipse.stretch, np.sqrt(block.mass), i)
            self.wait()
        vps_dot.clear_updaters()
        vps_dot.add_updater(lambda m: m.move_to(self.axes.coords_to_point(*self.vps_point.get_location()[:2])))
        self.play(FadeIn(xy_equation, UP), FadeOut(equations[1]))
        self.wait()
        curr_x = vps_point.get_location()[0]
        for x in [0.5 * curr_x, 2 * curr_x, curr_x]:
            axes_center = self.axes.coords_to_point(0, 0)
            self.play(vps_point.move_to, x * RIGHT, UpdateFromFunc(ellipse, lambda m: m.set_width(2 * get_norm(vps_dot.get_center() - axes_center)).move_to(axes_center)), run_time=2)
        self.wait()
    ...
    def ask_about_where_to_land(self):
        self.play(Rotating(self.vps_point, about_point=ORIGIN, run_time=6, rate_func=lambda t: smooth(t, 3)))
        self.wait(2)
    ...
    def show_conservation_of_momentum_equation(self):
        ...
        momentum_xy_equation = self.momentum_xy_equation = OldTex('\\sqrt{m_1}', 'x', '+', '\\sqrt{m_2}', 'y', '=', '\\text{const.}')
        momentum_xy_equation.set_color_by_tex('m_', BLUE)
        momentum_xy_equation.scale(0.8)
        momentum_xy_equation.next_to(momentum_expression, DOWN, buff=MED_LARGE_BUFF, aligned_edge=RIGHT)
        self.play(FadeOut(xy_equation), energy_expression.set_fill, {'opacity': 0.2}, FadeInFromDown(momentum_expression))
    ...
    def show_momentum_line(self):
        ...
        slope_label = OldTex('\\text{Slope =}', '-\\sqrt{\\frac{m_1}{m_2}}')
        slope_label.scale(0.8)
        slope_label.next_to(vps_dot, LEFT, LARGE_BUFF)
        slope_arrow = Arrow(slope_label.get_right(), line.point_from_proportion(0.45), buff=SMALL_BUFF)
        slope_group = VGroup(line, slope_label, slope_arrow)
        foreground_mobs = VGroup(self.equations[1], self.momentum_xy_equation, self.blocks, self.vps_dot)
        for mob in foreground_mobs:
            if isinstance(mob, Tex):
                mob.set_stroke(BLACK, 3, background=True)
        self.add(line, *foreground_mobs)
        self.play(ShowCreation(line))
        self.play(FadeIn(slope_label, RIGHT), GrowArrow(slope_arrow))
        self.wait()
        ...
        self.play(slope_group.shift, RIGHT, run_time=1, rate_func=lambda t: t ** 4)
        self.wait()
    ...
    def reiterate_meaning_of_line_and_circle(self):
        ...
        self.wait()
        self.play(Rotating(vps_point, about_point=ORIGIN, rate_func=lambda t: smooth(t, 3)))
        self.wait()
    ...
    def show_reflection_about_x(self):
        ...
        self.wait()
        group = VGroup(self.ellipse, self.lines[-1], self.vps_dot.copy().clear_updaters())
        for x in range(2):
            self.play(Rotate(group, PI, RIGHT, about_point=self.axes.coords_to_point(0, 0)))
        self.remove(group[-1])
    ...
    def show_remaining_collisions(self):
        ...
        end_region = Polygon(axes.coords_to_point(0, 0), axes.coords_to_point(10, 0), axes.coords_to_point(10, slope * 10), stroke_width=0, fill_color=GREEN, fill_opacity=0.3)
        self.unhalt()
        for x in range(7):
            self.go_through_next_collision()
            if x == 0:
...
class IntroduceVelocityPhaseSpaceWith16(IntroduceVelocityPhaseSpace):
    CONFIG = {'block1_config': {'mass': 16, 'velocity': -0.5}, 'momentum_line_scale_factor': 0}
...
class HuntForPi(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('Hunt for $\\pi$!', bubble_config={'direction': LEFT}, target_mode='hooray')
        self.play_all_student_changes('hooray', added_anims=[self.teacher.change, 'happy'])
        self.wait()
...
class StretchBySqrt10(Scene):
    ...
    def construct(self):
        ...
        arrow.save_state()
        arrow.stretch(0, 0)
        self.play(Restore(arrow), Write(value, run_time=1))
        self.wait()
...
class CircleDiagramFromSlidingBlocks(Scene):
    CONFIG = {'BlocksAndWallSceneClass': BlocksAndWallExampleMass1e1, 'circle_config': {'radius': 2, 'stroke_color': YELLOW, 'stroke_width': 3}, 'lines_style': {'stroke_color': WHITE, 'stroke_width': 2}, 'axes_config': {'style': {'stroke_color': GREY_B, 'stroke_width': 1}, 'width': 5, 'height': 4.5}, 'end_zone_style': {'stroke_width': 0, 'fill_color': GREEN, 'fill_opacity': 0.3}, 'show_dot': True, 'show_vector': False}

    def construct(self):
        sliding_blocks_scene = self.BlocksAndWallSceneClass(show_flash_animations=False, write_to_movie=False, wait_time=0, file_writer_config={'output_directory': '.'})
        blocks = sliding_blocks_scene.blocks
        times = [pair[1] for pair in blocks.clack_data]
        self.mass_ratio = 1 / blocks.mass_ratio
        self.show_circle_lines(times=times, slope=-1 / np.sqrt(blocks.mass_ratio))
    ...
    def show_circle_lines(self, times, slope):
        ...
        vector.add_updater(lambda v: v.put_start_and_end_on(ORIGIN, dot.get_center()))
    ...
    def get_axes(self):
        ...
        axes = VGroup(Line(LEFT, RIGHT).set_width(config['width']), Line(DOWN, UP).set_height(config['height']))
    ...
    def get_end_zone(self):
        ...
        zone = Polygon(ORIGIN, x * RIGHT, x * RIGHT + slope * x * UP)
...
class AnnouncePhaseDiagram(CircleDiagramFromSlidingBlocks):
    ...
    def construct(self):
        ...
        simple_arrow = Arrow(simple_words.get_bottom(), pd_words.get_top(), color=simple_words.get_color())
        self.play(self.teacher.change, 'raise_right_hand', FadeInFromDown(pd_words))
        self.play_student_changes('pondering', 'thinking', 'pondering', added_anims=[ShowCreationThenFadeOut(pd_words_border)])
        self.wait()
        self.play(FadeIn(simple_words, RIGHT), GrowArrow(simple_arrow), self.teacher.change, 'hooray')
        self.play_student_changes('thinking', 'happy', 'thinking')
        self.wait(3)

class AnalyzeCircleGeometry(CircleDiagramFromSlidingBlocks, MovingCameraScene):
    CONFIG = {'mass_ratio': 16, 'circle_config': {'radius': 3}, 'axes_config': {'width': FRAME_WIDTH, 'height': FRAME_HEIGHT}, 'lines_style': {'stroke_width': 2}}
    ...
    def add_mass_ratio_label(self, mass_ratio=None):
        ...
        mass_ratio_label = OldTexText('Mass ratio =', '{:,} : 1'.format(mass_ratio))
    ...
    def add_circle_with_lines(self):
        ...
        lines = self.get_lines(radius=circle.radius, slope=slope)
        end_zone = self.get_end_zone()
        ...
        self.add(axes, axes_labels, circle, end_zone, end_zone_words)
        self.play(ShowCreation(lines, run_time=3, rate_func=linear))
        self.wait()
        self.set_variables_as_attrs(circle, axes, lines, end_zone, end_zone_words)
    ...
    def show_equal_arc_lengths(self):
        ...
        lower_arcs = VGroup(*[Arc(start_angle=PI + n * 2 * theta, angle=2 * theta, radius=radius) for n in range(n_arcs + 1)])
        ...
        equal_signs = VGroup(*[OldTex('=') for x in range(len(lower_arcs))])
        ...
        self.play(Restore(all_arcs, lag_ratio=0.5, run_time=2))
        self.wait()
    ...
    def use_arc_lengths_to_count(self):
        ...
        self.play(FadeOut(lines), FadeOut(all_arcs), FadeOut(self.arcs_equation))
        self.play(ShowIncreasingSubsets(all_arcs), ShowIncreasingSubsets(lines), ShowIncreasingSubsets(arc_counts), run_time=5, rate_func=bezier([0, 0, 1, 1]))
        self.wait()
        ...
        kwargs = {'rate_func': there_and_back, 'run_time': 3}
        self.play(LaggedStartMap(MoveToTarget, all_arcs, **kwargs), LaggedStartMap(MoveToTarget, arc_counts, **kwargs))
    ...
    def focus_on_three_points(self):
        ...
        three_points = [line_pair[0].get_start(), line_pair[1].get_start(), line_pair[1].get_end()]
        ...
        center_lines = VGroup(Line(three_points[0], ORIGIN), Line(ORIGIN, three_points[2]))
        center_lines.match_style(line_pair)
        two_theta_arc = Arc(radius=1, start_angle=center_lines[0].get_angle() + PI, angle=2 * theta)
        ...
        self.play(LaggedStartMap(FadeOut, VGroup(*to_fade.family_members_with_points())))
        ...
        self.wait()
        self.play(ShowCreation(theta_arc), FadeIn(theta_label, UP))
        self.wait()
        self.play(line_pair.set_stroke, WHITE, 1, TransformFromCopy(line_pair, center_lines), TransformFromCopy(theta_arc, two_theta_arc), TransformFromCopy(theta_label, two_theta_label))
        self.wait()
        self.play(TransformFromCopy(two_theta_arc, arc), two_theta_label.move_to, 2.7 * arc_center)
        self.wait()
        self.three_dots = three_dots
        self.theta_group = VGroup(theta_arc, theta_label)
        self.center_lines_group = VGroup(center_lines, two_theta_arc)
    ...
    def show_likewise_for_all_jumps(self):
        ...
        self.play(Restore(lines, lag_ratio=0.5, run_time=2), FadeOut(self.center_lines_group), FadeOut(self.three_dots))
        self.play(LaggedStartMap(ApplyFunction, every_other_line, lambda line: (lambda l: l.scale(10 / l.get_length()).set_stroke(BLUE, 3), line)))
        ...
        for line, arc in zip(lines[5:10:2], arcs[6:11:2]):
            ...
            self.play(theta_group_copy.shift, new_point - last_point, two_theta_label.move_to, 1.1 * arc_point, FadeIn(arc), FadeOut(last_arc))
            self.wait()
            last_point = new_point
            last_arc = arc
        self.play(FadeOut(theta_group_copy), FadeOut(two_theta_label), FadeOut(last_arc))
        self.wait()
    ...
    def drop_arc_for_each_hop(self):
        ...
        for line, arc, label, wedge in zip(lines, arcs, two_theta_labels, wedges):
            self.play(ShowCreation(line), FadeIn(arc, normalize(arc.get_center())), FadeIn(label, normalize(arc.get_center())), FadeIn(wedge))
        self.wedges = wedges
    ...
    def try_adding_one_more_arc(self):
        ...
        for angle in [-2 * theta, 4 * DEGREES, -2 * DEGREES]:
            ...
            self.wait()
        self.play(new_wedge.shift, 10 * RIGHT, rate_func=running_start, path_arc=-30 * DEGREES)
        self.remove(new_wedge)

    def zoom_out(self):
        frame = self.camera_frame
        self.play(frame.scale, 2, {'about_point': TOP + RIGHT_SIDE}, run_time=3)
        self.wait()

    def get_axes_labels(self, axes):
        axes_labels = VGroup(OldTex('x = ', '\\sqrt{m_1}', '\\cdot', 'v_1'), OldTex('y = ', '\\sqrt{m_2}', '\\cdot', 'v_2'))
...
class InscribedAngleTheorem(Scene):
    ...
    def show_circle(self):
        circle = self.circle = Circle(color=BLUE, radius=2)
        center_dot = Dot(circle.get_center(), color=WHITE)
        self.add(circle, center_dot)
        angle_trackers = self.angle_trackers = VGroup(ValueTracker(TAU / 4), ValueTracker(PI), ValueTracker(-TAU / 4))
        ...
        def get_center_lines():
            points = [dots[0].get_center(), circle.get_center(), dots[2].get_center()]
            lines = VGroup(*[Line(p1, p2) for p1, p2 in zip(points, points[1:])])
        ...
        def get_angle_label(lines, tex, reduce_angle=True):
            ...
            arc = Arc(start_angle=a1, angle=diff, radius=0.5)
            arc.shift(point)
        ...
        labels = VGroup(*[OldTex('P_{}'.format(n + 1)) for n in range(3)])
        update_labels(labels)
        theta_label = get_theta_label()
        two_theta_label = get_2theta_label()
        self.play(FadeInFromDown(labels[0]), FadeInFromLarge(dots[0]))
        self.play(TransformFromCopy(*labels[:2]), TransformFromCopy(*dots[:2]), ShowCreation(lines[0]))
        self.play(ShowCreation(lines[1]), TransformFromCopy(*labels[1:3]), TransformFromCopy(*dots[1:3]), Write(theta_label))
        self.wait()
        ...
        self.play(angle_trackers[0].set_value, TAU / 8)
        self.play(angle_trackers[2].set_value, -TAU / 8)
        self.wait()
        center_lines.update()
        two_theta_label.update()
        self.play(TransformFromCopy(lines.copy().clear_updaters(), center_lines), TransformFromCopy(theta_label.copy().clear_updaters(), two_theta_label))
        self.wait()
...
class SimpleSlopeLabel(Scene):
    ...
    def construct(self):
        label = OldTex('\\text{Slope}', '=', '-\\frac{\\sqrt{m_1}}{\\sqrt{m_2}}')
        vector = Vector(DOWN + 2 * LEFT, color=WHITE)
        vector.move_to(label[0].get_bottom(), UR)
        vector.shift(SMALL_BUFF * DOWN)
        self.play(Write(label), GrowArrow(vector))
        self.wait()

class AddTwoThetaManyTimes(Scene):

    def construct(self):
        expression = OldTex('2\\theta', '+', '2\\theta', '+', '2\\theta', '+', '\\cdots', '+', '2\\theta', '<', '2\\pi')
        ...
        for n in range(5):
            ...
            self.wait(0.25)
        self.play(FadeIn(expression[-2:], LEFT), GrowFromCenter(brace), FadeIn(question, UP))
        self.wait(3)
        self.play(TransformFromCopy(expression[:-2], simplified[:3]), TransformFromCopy(expression[-2:], simplified[3:]), TransformFromCopy(brace, lil_brace))
    ...
    def get_central_question(self, brace_vect=DOWN):
        expression = OldTex('N', '\\cdot', '\\theta', '<', '\\pi')
        N = expression[0]
        N.set_color(BLUE)
    ...
    def show_example(self):
        ...
        self.play(TransformFromCopy(expression[0], N_mob), TransformFromCopy(expression[1:4], dot_theta_eq), TransformFromCopy(expression[4], rhs), TransformFromCopy(expression[4], comp_pi))
        self.wait()
...
class AskAboutTheta(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('But what is $\\theta$?', target_mode='raise_left_hand')
        self.play_student_changes('confused', 'sassy', 'raise_left_hand', added_anims=[self.teacher.change, 'happy'])
        self.wait(3)

class ComputeThetaFor1e4(AnalyzeCircleGeometry):
    CONFIG = {'mass_ratio': 100}
    ...
    def add_circle_with_three_lines(self):
        ...
        arc = Arc(start_angle=-90 * DEGREES, angle=theta, radius=2, arc_center=two_lines[0].get_end())
        theta_label = OldTex('\\theta')
        ...
        self.play(lines_to_fade.set_stroke, WHITE, 1, 0.3, ShowCreation(arc), FadeIn(theta_label, UP))
    ...
    def write_slope(self):
        ...
        slope_label = OldTex('\\text{Slope}', '=', '\\frac{\\text{rise}}{\\text{run}}', '=', '\\frac{-\\sqrt{m_1}}{\\sqrt{m_2}}', '=', '-10')
        for mob in slope_label:
            mob.add_to_back(mob.copy().set_stroke(BLACK, 6))
        slope_label.next_to(line.get_center(), UR, buff=1)
        slope_arrow = Arrow(slope_label[0].get_bottom(), line.point_from_proportion(0.45), color=RED, buff=SMALL_BUFF)
        new_line = line.copy().set_color(RED)
        self.play(FadeInFromDown(slope_label[:3]), ShowCreation(new_line), GrowArrow(slope_arrow))
        self.remove(new_line)
        line.match_style(new_line)
        self.play(FadeIn(slope_label[3:5], LEFT))
        self.wait()
        self.play(Write(slope_label[5]), TransformFromCopy(self.mass_ratio_label[1][:3], slope_label[6]))
        self.wait()
    ...
    def show_tangent(self):
        ...
        tan_equation = OldTex('\\tan', '(', '\\theta', ')', '=', '{\\text{run}', '\\over', '-\\text{rise}}', '=', '\\frac{1}{10}')
        ...
        arctan_equation = OldTex('\\theta', '=', '\\arctan', '(', '1 / 10', ')')
        arctan_equation.next_to(tan_equation, DOWN, MED_LARGE_BUFF)
        self.play(FadeInFromDown(tan_equation[:8]))
        self.play(TransformFromCopy(theta_label_group[1], opposite), run_word.set_color, opposite.get_color())
        self.play(WiggleOutThenIn(run_word))
        self.play(TransformFromCopy(opposite, adjacent), rise_word.set_color, adjacent.get_color())
        self.play(WiggleOutThenIn(rise_word))
        self.wait()
        self.play(TransformFromCopy(self.slope_label[-1], tan_equation[-2:]))
        self.wait(2)
        indices = [2, 4, 0, 1, -1, 3]
        ...
        sm[-1], sm[-2] = (sm[-2], sm[-1])
        self.play(LaggedStartMap(Transform, movers[:-1], lambda m: (m, m.target), lag_ratio=1, run_time=1, path_arc=PI / 6))
...
class ThetaChart(Scene):
    ...
    def create_columns(self):
        titles = VGroup(*[OldTexText('Mass ratio'), OldTexText('$\\theta$ formula'), OldTexText('$\\theta$ value')])
        titles.scale(1.5)
        ...
        self.play(LaggedStartMap(FadeInFromDown, titles), LaggedStartMap(ShowCreation, lines, lag_ratio=0.8))
        self.h_line = h_line
        self.titles = titles
    ...
    def populate_columns(self):
        ...
        entries = [('$m_1$ : $m_2$', '$\\arctan(\\sqrt{m_2} / \\sqrt{m_1})$', '')] + [('{:,} : 1'.format(10 ** (2 * exp)), '$\\arctan(1 / {:,})$'.format(10 ** exp), self.get_theta_decimal(exp)) for exp in [1, 2, 3, 4, 5]]
        ...
        for entry in entries:
            ...
            h_lines.add(h_line)
        self.play(LaggedStartMap(FadeInFromDown, VGroup(*[em[:2] for em in entry_mobs])), LaggedStartMap(ShowCreation, h_lines[1:]), lag_ratio=0.1, run_time=5)
    ...
    def show_values(self):
        ...
        for value in values:
            self.play(LaggedStartMap(FadeIn, value, lag_ratio=0.1, run_time=0.5))
            self.wait(0.5)
    ...
    def highlight_example(self, exp):
        ...
        self.play(other_entries.set_fill, {'opacity': 0.25}, example.set_fill, {'opacity': 1}, ShowCreationThenFadeAround(example))
        self.wait()
        self.play(MoveToTarget(value), Write(rhs))
        self.wait()
        value.add(rhs)

    def get_theta_decimal(self, exp):
        theta = np.arctan(10 ** (-exp))
        rounded_theta = np.floor(10000000000.0 * theta) / 10000000000.0
...
class CentralQuestionFor1e2(AddTwoThetaManyTimes):
    ...
    def construct(self):
        ...
        self.add(question)
        b10_equation = self.get_changable_equation(10 ** (-exp), n_decimal_places=exp)
        b10_equation.next_to(question, DOWN, buff=1.5)
        arctan_equation = self.get_changable_equation(np.arctan(10 ** (-exp)), n_decimal_places=10)
        arctan_equation.next_to(b10_equation, DOWN, MED_LARGE_BUFF)
        eq_centers = [eq[1][2].get_center() for eq in [b10_equation, arctan_equation]]
        arctan_equation.shift((eq_centers[1][0] - eq_centers[1][0]) * RIGHT)
        arctan_brace = Brace(arctan_equation[1][1][1:-1], DOWN)
        arctan_tex = arctan_brace.get_tex('\\theta = \\arctan(1 / {:,})'.format(10 ** exp))
        int_mobs = (b10_equation[0], arctan_equation[0])
        self.add(*b10_equation, *arctan_equation)
        self.add(arctan_brace, arctan_tex)
        self.wait()
        self.play(*[ChangeDecimalToValue(int_mob, max_count, run_time=8) for int_mob in int_mobs])
        self.wait()
        self.play(*[ChangeDecimalToValue(int_mob, max_count + 1, run_time=1) for int_mob in int_mobs])
        self.wait()
        self.play(*[ChangeDecimalToValue(int_mob, max_count, run_time=1) for int_mob in int_mobs])
...
class AskAboutArctanOfSmallValues(TeacherStudentsScene):
    ...
    def construct(self):
        self.add_title()
        equation1 = OldTex('\\arctan', '(', 'x', ')', '\\approx', 'x')
        equation1.set_color_by_tex('arctan', YELLOW)
        equation2 = OldTex('x', '\\approx', '\\tan', '(', 'x', ')')
        equation2.set_color_by_tex('tan', BLUE)
        for mob in (equation1, equation2):
            mob.move_to(self.hold_up_spot, DOWN)
        self.play(FadeInFromDown(equation1), self.teacher.change, 'raise_right_hand', self.change_students('erm', 'sassy', 'confused'))
        self.look_at(3 * UL)
        self.play(equation1.shift, UP)
        self.play(TransformFromCopy(VGroup(*[equation1[i] for i in (2, 4, 5)]), VGroup(*[equation2[i] for i in (0, 1, 4)])))
        self.play(TransformFromCopy(VGroup(*[equation1[i] for i in (0, 1, 3)]), VGroup(*[equation2[i] for i in (2, 3, 5)])), self.change_students('confused', 'erm', 'sassy'))
        self.look_at(3 * UL)
        self.wait(3)
...
class ActanAndTanGraphs(GraphScene):
    CONFIG = {'x_min': -PI / 8, 'x_max': 5 * PI / 8, 'y_min': -PI / 8, 'y_max': 4 * PI / 8, 'x_tick_frequency': PI / 8, 'x_leftmost_tick': -PI / 8, 'y_tick_frequency': PI / 8, 'y_leftmost_tick': -PI / 8, 'x_axis_width': 10, 'y_axis_height': 7, 'graph_origin': 2.5 * DOWN + 5 * LEFT, 'num_graph_anchor_points': 500}

    def construct(self):
        self.setup_axes()
        axes = self.axes
        labels = VGroup(OldTex('\\pi / 8'), OldTex('\\pi / 4'), OldTex('3\\pi / 8'), OldTex('\\pi / 2'))
        ...
        arctan_v_line = always_redraw(lambda: self.get_vertical_line_to_graph(arctan_x_tracker.get_value(), arctan_graph, line_class=DashedLine, color=WHITE))
        tan_x_tracker = ValueTracker(2 * PI / 8)
        tan_v_line = always_redraw(lambda: self.get_vertical_line_to_graph(tan_x_tracker.get_value(), tan_graph, line_class=DashedLine, color=WHITE))
        self.add(axes)
        ...
        self.wait()
        self.play(TransformFromCopy(arctan_graph, tan_graph), TransformFromCopy(arctan_label, tan_label))
        self.add(tan_v_line)
...
class UnitCircleIntuition(Scene):
    ...
    def draw_unit_circle(self):
        unit_size = 2.5
        axes = Axes(axis_config={'unit_size': unit_size}, x_min=-2.5, x_max=2.5, y_min=-1.5, y_max=1.5)
        axes.set_stroke(width=1)
        self.add(axes)
        radius_line = Line(ORIGIN, axes.coords_to_point(1, 0))
        radius_line.set_color(BLUE)
        r_label = OldTex('1')
        r_label.add_updater(lambda m: m.next_to(radius_line.get_center(), DOWN, SMALL_BUFF))
        circle = Circle(radius=unit_size, color=WHITE)
        self.add(radius_line, r_label)
        self.play(Rotating(radius_line, about_point=ORIGIN), ShowCreation(circle), run_time=2, rate_func=smooth)
    ...
    def show_angle(self):
        circle = self.circle
        tan_eq = OldTex('\\tan', '(', '\\theta', ')', '=', tex_to_color_map={'\\theta': RED})
        ...
        def get_r_line():
            return Line(circle.get_center(), circle.point_at_angle(get_theta()))
        ...
        def get_arc(radius=None, **kwargs):
            ...
            return Arc(radius=radius, start_angle=0, angle=get_theta(), arc_center=circle.get_center(), **kwargs)
        arc = always_redraw(get_arc)
        self.circle_arc = always_redraw(lambda: get_arc(radius=circle.radius, color=RED))
        ...
        def get_height_line():
            ...
            return Line(p1, p2, stroke_color=YELLOW, stroke_width=3)
        ...
        def get_width_line():
            ...
            p1[1] = p2[1]
            return Line(p1, p2, stroke_color=PINK, stroke_width=3)
        ...
        self.play(FadeInFromDown(tan_eq), theta_tracker.set_value, 20 * DEGREES)
        self.wait()
        self.tan_eq = tan_eq
    ...
    def show_fraction(self):
        ...
        tan_eq = self.tan_eq
        rhs = OldTex('{\\text{height}', '\\over', '\\text{width}}')
        ...
        for mob in [height_line, width_line, h_label, w_label]:
            mob.update()
        self.play(ShowCreation(height_line.copy().clear_updaters(), remover=True), FadeIn(h_label.copy().clear_updaters(), RIGHT, remover=True), Write(rhs[:2]))
        self.add(height_line, h_label)
        self.play(ShowCreation(width_line.copy().clear_updaters(), remover=True), FadeIn(w_label.copy().clear_updaters(), UP, remover=True), self.r_label.fade, 1, Write(rhs[2]))
    ...
    def show_fraction_approximation(self):
        ...
        approx_rhs = OldTex('\\approx', '{\\theta', '\\over', '1}')
...
class TangentTaylorSeries(TeacherStudentsScene):
    ...
    def construct(self):
        series = OldTex('\\tan', '(', '\\theta', ')', '=', '\\theta', '+', '\\frac{1}{3}', '\\theta', '^3', '+', '\\frac{2}{15}', '\\theta', '^5', '+', '\\cdots', tex_to_color_map={'\\theta': YELLOW})
        series.move_to(2 * UP)
        series.move_to(self.hold_up_spot, DOWN)
        series_error = series[7:]
        series_error_rect = SurroundingRectangle(series_error)
        example = OldTex('\\tan', '\\left(', '\\frac{1}{100}', '\\right)', '=', '\\frac{1}{100}', '+', '\\frac{1}{3}', '\\left(', '\\frac{1}{1{,}000{,}000}', '\\right)', '+', '\\frac{2}{15}', '\\left(', '\\frac{1}{10{,}000{,}000{,}000}', '\\right)', '+', '\\cdots')
        ...
        error_decimal = DecimalNumber(np.tan(0.01) - 0.01, num_decimal_places=15)
        ...
        self.play(FadeInFromDown(series), self.teacher.change, 'raise_right_hand')
        self.play(ShowCreation(series_error_rect), self.change_students(*3 * ['pondering']))
        self.play(FadeOut(series_error_rect))
        self.play(series.center, series.to_edge, UP)
        self.look_at(series)
        self.play(TransformFromCopy(series[:8], example[:8]), TransformFromCopy(series[8], example[9]), TransformFromCopy(series[10:12], example[11:13]), TransformFromCopy(series[12], example[14]), TransformFromCopy(series[14:], example[16:]), *map(GrowFromCenter, [example[i] for i in (8, 10, 13, 15)]))
...
class SumUpWrapper(Scene):
    ...
    def construct(self):
        ...
        self.add(FullScreenFadeRectangle(fill_color=GREY_D, fill_opacity=0.5))
        self.play(FadeInFromDown(title), FadeIn(screen_rect))
        self.wait()

class ConservationLawSummary(Scene):

    def construct(self):
        energy_eq = OldTex('\\frac{1}{2}', 'm_1', '(', 'v_1', ')', '^2', '+', '\\frac{1}{2}', 'm_2', '(', 'v_2', ')', '^2', '=', '\\text{const.}')
        energy_word = OldTexText('Energy')
        energy_word.scale(2)
        circle = Circle(color=YELLOW, radius=2)
        energy_group = VGroup(energy_word, energy_eq, circle)
        momentum_eq = OldTex('m_1', 'v_1', '+', 'm_2', 'v_2', '=', '\\text{const.}')
        ...
        for group in (energy_group, momentum_group):
            arrow = Arrow(LEFT, 2 * RIGHT, rectangular_stem_width=0.1, tip_length=0.5, color=WHITE)
        ...
        self.wait()
        self.play(LaggedStartMap(ShowCreationThenDestruction, red_energy_word))
...
class FinalCommentsOnPhaseSpace(Scene):
    ...
    def show_related_fields(self):
        title = self.title
        images = Group(ImageMobject('ClacksThumbnail'), ImageMobject('PictoralODE'), ImageMobject('MobiusStrip'))
        colors = [BLUE_D, GREY_BROWN, BLUE_C]
        for image, color in zip(images, colors):
            image.set_height(2.5)
            image.add(SurroundingRectangle(image, color=color, stroke_width=5, buff=0))
        images.arrange(RIGHT)
        images.move_to(DOWN)
        arrows = VGroup(*[Arrow(title.get_bottom(), image.get_top(), color=WHITE) for image in images])
        for image, arrow in zip(images, arrows):
            self.play(GrowArrow(arrow), GrowFromPoint(image, title.get_bottom()))
            self.wait()
        self.wait()
    ...
    def state_to_point(self):
        ...
        arrow = Arrow(2 * LEFT, 2 * RIGHT, color=WHITE, rectangular_stem_width=0.1, tip_length=0.5)
        ...
        self.play(FadeOut(self.to_fade, UP), FadeIn(state, UP))
        self.play(GrowArrow(arrow), FadeIn(point, LEFT))
        self.wait(2)
        for w1, w2 in [(state, dynamics), (point, geometry)]:
            self.play(FadeOut(w1, UP), FadeIn(w2, DOWN))
            self.wait()
        self.wait()
