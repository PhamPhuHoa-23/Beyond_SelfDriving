...
class Block(Square):
    CONFIG = {'mass': 1, 'velocity': 0, 'width': None, 'label_text': None, 'label_scale_value': 0.8, 'fill_opacity': 1, 'stroke_width': 3, 'stroke_color': WHITE, 'fill_color': None, 'sheen_direction': UL, 'sheen_factor': 0.5, 'sheen_direction': UL}
    ...
    def mass_to_color(self, mass):
        colors = [GREY_B, BLUE_D, BLUE_D, BLUE_E, BLUE_E, GREY_D, GREY_D, BLACK]
...
class SlidingBlocks(VGroup):
    CONFIG = {'block1_config': {'distance': 7, 'mass': 1000000.0, 'velocity': -2}, 'block2_config': {'distance': 3, 'mass': 1, 'velocity': 0}, 'collect_clack_data': True}
    ...
    def __init__(self, scene, **kwargs):
        ...
        self.add(self.block1, self.block2, self.phase_space_point_tracker)
    ...
    def get_block(self, distance, **kwargs):
        block = Block(**kwargs)
        block.move_to(self.floor.get_top()[1] * UP + (self.wall.get_right()[0] + distance) * RIGHT, DL)
        return block
    ...
    def get_phase_space_point_tracker(self):
        ...
        result = VectorizedPoint([s1 * np.sqrt(block1.mass), s2 * np.sqrt(block2.mass), 0])
        result.velocity = np.array([np.sqrt(block1.mass) * block1.velocity, np.sqrt(block2.mass) * block2.velocity, 0])
        return result

    def update_positions(self, dt):
        self.phase_space_point_tracker.shift(self.phase_space_point_tracker.velocity * dt)
    ...
    def update_blocks_from_phase_space_point_tracker(self):
        ...
        block1.move_to((shadow_wall_x + s1) * RIGHT + floor_y * UP, DL)
        block2.move_to((shadow_wall_x + s2) * RIGHT + floor_y * UP, DR)
    ...
    def get_clack_data(self):
        ...
        if ps_velocity[1] != 0:
            raise Exception("Haven't implemented anything to gather clack data from a start state with block2 moving")
        y = ps_point[1]
        theta = np.arctan(np.sqrt(self.mass_ratio))
        clack_data = []
        for k in range(1, int(PI / theta) + 1):
            ...
            block2 = self.block2
            s2 = reflected_point[1] / np.sqrt(block2.mass)
            location = np.array([self.wall.get_right()[0] + s2, block2.get_center()[1], 0])
            if k % 2 == 1:
...
class ClackFlashes(VGroup):
    CONFIG = {'flash_config': {'run_time': 0.5, 'line_length': 0.1, 'flash_radius': 0.2}, 'start_up_time': 0, 'min_time_between_flashes': 1 / 30}
    ...
    def update(self, dt):
        ...
        for flash in self.flashes:
            ...
            elif flash.mobject in self.submobjects:
                self.remove(flash.mobject)

class Wall(Line):
    CONFIG = {'tick_spacing': 0.5, 'tick_length': 0.25, 'tick_style': {'stroke_width': 1, 'stroke_color': WHITE}}
    ...
    def get_ticks(self):
        ...
        lines = VGroup(*[Line(ORIGIN, self.tick_length * UR).shift(n * self.tick_spacing * UP) for n in range(n_lines)])
        lines.set_style(**self.tick_style)
        lines.move_to(self, DR)
        return lines

class BlocksAndWallScene(Scene):
    CONFIG = {'include_sound': True, 'collision_sound': 'clack.wav', 'count_clacks': True, 'counter_group_shift_vect': LEFT, 'sliding_blocks_config': {}, 'floor_y': -2, 'wall_x': -6, 'n_wall_ticks': 15, 'counter_label': '\\# Collisions: ', 'show_flash_animations': True, 'min_time_between_sounds': 0.004}
    ...
    def add_counter(self):
        ...
        counter_mob.next_to(counter_label[-1], RIGHT)
        counter_mob.align_to(counter_label[-1][-1], DOWN)
        counter_group = VGroup(counter_label, counter_mob)
    ...
    def add_clack_sounds(self, clack_data):
        ...
        for time in times:
            ...
            if d_time < self.min_time_between_sounds:
                continue
            last_time = time
            self.add_sound(clack_file, time_offset=time - total_time, gain=-20)
        return self
...
class NameIntro(Scene):
    ...
    def construct(self):
        ...
        flash_time = 0.75
        self.add(blue, brown)
        self.play(VFadeIn(blue), VFadeIn(brown), Restore(brown, rate_func=linear))
        self.play(Flash(blue.get_right(), run_time=flash_time), ApplyMethod(blue.to_edge, LEFT, {'buff': 0}, rate_func=linear))
        self.play(Flash(blue.get_left(), run_time=flash_time), Restore(blue, rate_func=linear))
        self.play(Flash(blue.get_right(), run_time=flash_time), ApplyMethod(brown.to_edge, RIGHT, {'buff': 0}, rate_func=linear))
        self.play(Flash(brown.get_right(), run_time=flash_time), Restore(brown, rate_func=linear))
...
class MathAndPhysicsConspiring(Scene):
    ...
    def construct(self):
        ...
        for title, vect in ((math_title, LEFT), (physics_title, RIGHT)):
            ...
            title.to_edge(UP)
        math_stuffs = VGroup(OldTex('\\pi = {:.16}\\dots'.format(PI)), self.get_tangent_image())
        ...
        self.play(LaggedStartMap(FadeInFromDown, to_fade, lag_ratio=0.7, run_time=3), Restore(v_line, run_time=2, path_arc=PI / 2))
        self.wait()

    def get_tangent_image(self):
        axes = Axes(x_min=-1.5, x_max=1.5, y_min=-1.5, y_max=1.5)
        circle = Circle()
        ...
        result = VGroup(axes, circle, line, one, arc, theta_label, tan_line, tan_text)
        result.set_height(4)
        return result

class LightBouncing(MovingCameraScene):
    CONFIG = {'theta': np.arctan(0.15), 'show_fanning': False, 'mirror_shift_vect': 5 * LEFT, 'mirror_length': 10, 'beam_start_x': 12, 'beam_height': 1}
    ...
    def construct(self):
        ...
        points = [start_point] + [np.array([beam_height / np.tan(k * theta), beam_height, 0]) for k in range(1, int(PI / theta))] + [rotate(start_point, PI, UP)]
        reflected_points = []
    ...
    def get_beam_anim(self, beam):
        ...
        return AnimationGroup(ShowPassingFlash(beam, run_time=5, rate_func=lambda t: smooth(t, 5), time_width=0.05), UpdateFromFunc(dot, lambda m: m.move_to(beam.get_points()[-1])))

class BlocksAndWallExample(BlocksAndWallScene):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 1.0, 'velocity': -2}}, 'wait_time': 15}

    def construct(self):
        self.wait(self.wait_time)

class BlocksAndWallExampleMass1e1(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 10.0, 'velocity': -1.5}}, 'wait_time': 20}
...
class TwoBlocksLabel(Scene):
    ...
    def construct(self):
        ...
        self.play(Write(label), LaggedStartMap(GrowArrow, arrows, lag_ratio=0.7), run_time=1)
        self.wait()
...
class Mass1e1WithElasticLabel(BlocksAndWallExampleMass1e1):
    ...
    def get_arrow(self, label, clack_flashes, flash):
        arrow = Arrow(label.get_bottom(), flash.mobject.get_center() + 0.0 * UP)
...
class AskAboutSoundlessness(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait(2)
        self.teacher_says('Focus on \\\\ collisions', target_mode='speaking', added_anims=[self.change_students('pondering', 'confused', 'thinking')])
...
class AskWhatWillHappen(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.pi_creature_says('What will\\\\happen?', target_mode='maybe', look_at=4 * DR)
        self.wait(3)

class BlocksAndWallExampleMass1e2(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 100.0, 'velocity': -0.6}}, 'wait_time': 35}

class BlocksAndWallExampleMassSameSpeedAtEnd(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 1 / np.tan(PI / 5) ** 2, 'velocity': -1, 'label_text': '$\\frac{1}{\\tan(\\pi / 5)^2}$ kg'}}, 'wait_time': 25}
...
class BlocksAndWallExampleMass1e4SlowMo(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 10000.0, 'velocity': -0.1, 'distance': 4.1}}, 'wait_time': 50, 'collision_sound': 'slow_clack.wav'}
...
class BlocksAndWallExampleMass1e6(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 1000000.0, 'velocity': -1}}, 'wait_time': 20}

class BlocksAndWallExampleMass1e6SlowMo(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 1000000.0, 'velocity': -0.1, 'distance': 4.1}}, 'wait_time': 60, 'collision_sound': 'slow_clack.wav'}

class BlocksAndWallExampleMass1e8(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 100000000.0, 'velocity': -1}}, 'wait_time': 25}

class BlocksAndWallExampleMass1e10(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 10000000000.0, 'velocity': -1}}, 'wait_time': 25}
...
class DigitsOfPi(Scene):
    ...
    def construct(self):
        ...
        self.play(ShowIncreasingSubsets(equation[2:], rate_func=linear, run_time=1))
...
class PiComputingAlgorithmsAxes(Scene):
    ...
    def setup_axes(self):
        axes = Axes(x_min=0, y_min=0, x_max=9, y_max=5, axis_config={'tick_frequency': 100, 'big_tick_numbers': []})
    ...
    def add_methods(self):
        method_location_list = [(self.get_machin_like_formula(), (1, 4.5)), (self.get_viete(), (3, 3.5)), (self.get_measuring_tape(), (0.5, 1)), (self.get_monte_carlo(), (2, 0.2)), (self.get_basel(), (6, 1)), (self.get_blocks_image(), (8, 0.1))]
        algorithms = VGroup()
        ...
        self.play(LaggedStartMap(FadeInFromDown, algorithms, run_time=4, lag_ratio=0.4))
        self.wait()
        self.play(ShowCreationThenFadeAround(algorithms[-1][0]))

    def get_machin_like_formula(self):
        formula = OldTex('\\frac{\\pi}{4} = 12\\arctan\\left(\\frac{1}{49}\\right) + 32\\arctan\\left(\\frac{1}{57}\\right) - 5\\arctan\\left(\\frac{1}{239}\\right) + 12\\arctan\\left(\\frac{1}{110{,}443}\\right)')
        formula.scale(0.5)
        return formula

    def get_viete(self):
        formula = OldTex('\\frac{2}{\\pi} = \\frac{\\sqrt{2}}{2} \\cdot\\frac{\\sqrt{2 + \\sqrt{2}}}{2} \\cdot\\frac{\\sqrt{2 + \\sqrt{2 + \\sqrt{2}}}}{2} \\cdots')
        formula.scale(0.5)
        return formula
    ...
    def get_blocks_image(self):
        scene = BlocksAndWallScene(write_to_movie=False, skip_animations=True, count_clacks=False, floor_y=1, wall_x=0, n_wall_ticks=6, sliding_blocks_config={'block1_config': {'mass': 100.0, 'velocity': -0.01, 'distance': 3.5}, 'block2_config': {'distance': 1, 'velocity': 0}})
        group = VGroup(scene.wall, scene.floor, scene.blocks.block1, scene.blocks.block2)
        group.set_width(3)
        return group
...
class StepsOfTheAlgorithm(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for step in steps:
            ...
            self.play(Write(step[1], run_time=2), self.change_students(*['pondering'] * 3, look_at=step))
            self.wait()
        self.play_student_changes('sassy', 'erm', 'confused', look_at=steps, added_anims=[self.teacher.change, 'happy'])
        self.wait(3)

    def get_steps(self):
        return VGroup(OldTexText('Step 1:', 'Implement a physics engine'), OldTexText('Step 2:', 'Choose the number of digits, $d$,\\\\of $\\pi$ that you want to compute'), OldTexText('Step 3:', 'Set one mass to $100^{d - 1}$,\\\\the other to $1$'), OldTexText('Step 4:', 'Count collisions'))
...
class StepsOfTheAlgorithmJustTitles(StepsOfTheAlgorithm):
    ...
    def construct(self):
        ...
        for title in titles:
            self.play(FadeInFromDown(title), FadeOut(last_title, UP))
            self.wait()
            last_title = title

class BlocksAndWallExampleToShowWithSteps(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 1e+22, 'velocity': -1, 'label_text': '$100^{(12 - 1)}$\\,kg', 'width': 2}, 'collect_clack_data': False}, 'wait_time': 25, 'counter_group_shift_vect': 5 * LEFT, 'count_clacks': True, 'include_sound': False, 'show_flash_animations': False}
...
class CompareToGalacticMass(Scene):
    ...
    def add_digits_of_pi(self):
        ...
        digits.to_edge(UP)
        highlighted_digits = VGroup(*[d.copy().set_background_stroke(color=BLUE, width=5) for d in [digits[0], *digits[2:-3]]])
        counter = Integer(0)
        counter.scale(1.5)
        ...
        group.arrange(RIGHT, index_of_submobject_to_align=0, aligned_edge=DOWN, buff=0.7)
        group.next_to(brace, DOWN)
        self.play(MoveToTarget(counter), FadeIn(digits_word, LEFT))
        self.wait()
        self.pi_digits_group = VGroup(digits, brace, counter, digits_word)

    def show_mass(self):
        bw_scene = BlocksAndWallExample(write_to_movie=False, skip_animations=True, count_clacks=False, show_flash_animations=False, floor_y=0, wall_x=-2, n_wall_ticks=8, sliding_blocks_config={'block1_config': {'mass': 1000000.0, 'velocity': -0.01, 'distance': 4.5, 'label_text': '$100^{(20 - 1)}$ kg', 'fill_color': BLACK}, 'block2_config': {'distance': 1, 'velocity': 0}})
        ...
        number_words = OldTexText('100', *['billion'] * 4)
        ...
        self.play(GrowFromCenter(brace), FadeInFromDown(last_word))
    ...
    def show_galactic_black_holes(self):
        ...
        self.add(words)
        self.play(Write(equals), Write(words), LaggedStartMap(Restore, black_holes, run_time=3))
        self.wait()
    ...
    def show_total_count(self):
        ...
        self.play(ReplacementTransform(VGroup(*filter(lambda m: m not in dots, digits)), VGroup(*filter(lambda m: m not in commas, number))), ReplacementTransform(dots, commas, lag_ratio=0.5, run_time=2))
...
class BlocksAndWallExampleGalacticMass(BlocksAndWallExample):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 10000000000.0, 'velocity': -1, 'label_text': '$100^{(20 - 1)}$\\,kg', 'width': 2}}, 'wait_time': 25, 'counter_group_shift_vect': 5 * LEFT, 'count_clacks': False}
...
class RealPhysicsVsThis(Scene):
    ...
    def construct(self):
        ...
        self.play(this.shift, FRAME_WIDTH * RIGHT, rate_func=rush_into, run_time=3)
        self.wait()
...
class CompareAlgorithmToPhysics(PiCreatureScene):
    ...
    def construct(self):
        morty = self.pi_creature
        right_pic = ImageMobject(self.get_image_file_path().replace(str(self), 'PiComputingAlgorithmsAxes'))
        ...
        self.play(FadeInFromDown(right_pic), morty.change, 'raise_right_hand')
        self.wait()
        self.play(FadeInFromDown(left_rect), morty.change, 'raise_left_hand')
        self.wait()
        digits = OldTex('3.141592653589793238462643383279502884197...')
        digits.set_width(FRAME_WIDTH - 1)
        digits.to_edge(UP)
        self.play(FadeOut(right_pic, 5 * RIGHT), FadeOut(left_rect), PiCreatureBubbleIntroduction(morty, "This doesn't seem \\\\ like me...", bubble_type=ThoughtBubble, bubble_config={'direction': LEFT}, target_mode='pondering', look_at=left_rect), LaggedStartMap(FadeInFrom, digits, lambda m: (m, LEFT), run_time=5, lag_ratio=0.2))
        self.blink()
        self.wait()
...
class AskAboutWhy(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        ke_conservation = OldTex('\\frac{1}{2}m_1 v_1^2 + \\frac{1}{2}m_2 v_2^2 = \\text{const.}')
        ke_conservation.move_to(circle)
        self.student_says('But why?')
        self.play_student_changes('erm', 'raise_left_hand', 'sassy', added_anims=[self.teacher.change, 'happy'])
        self.wait()
        self.play(ShowCreation(circle), RemovePiCreatureBubble(self.students[1]), self.teacher.change, 'raise_right_hand')
        self.play_all_student_changes('pondering', look_at=circle)
        self.wait(2)
        self.play(Write(ke_conservation), circle.stretch, 1.5, 0)
        self.play_all_student_changes('confused')
        self.look_at(circle)
        self.wait(3)

class LightBouncingNoFanning(LightBouncing):
    CONFIG = {'mirror_shift_vect': 2 * DOWN, 'mirror_length': 6, 'beam_start_x': 8, 'beam_height': 0.5}

class LightBouncingFanning(LightBouncingNoFanning):
    CONFIG = {'show_fanning': True}
...
class NextVideo(Scene):
    ...
    def construct(self):
        ...
        titles = VGroup(OldTexText('Here and now'), OldTexText('Solution'))
        ...
        self.wait()
        self.play(LaggedStartMap(FadeInFrom, mid_words, lambda m: (m, UP), lag_ratio=0.8), randy.change, 'pondering', VFadeIn(randy), videos.space_out_submobjects, 1.3)
        self.play(ShowCreation(thought_bubble))
        self.play(Blink(randy))
        self.play(Uncreate(thought_bubble), ShowCreation(speech_bubble), Write(speech_bubble.content), randy.change, 'maybe', friends[0].eyes, LaggedStartMap(FadeInFromDown, friends), videos.space_out_submobjects, 1.6)
        self.play(LaggedStartMap(ApplyMethod, friends, lambda m: (m.change, 'pondering'), run_time=1, lag_ratio=0.7))
...
class EndScreen(Scene):
    ...
    def construct(self):
        ...
        video_rect = Rectangle(width=width, height=height)
        video_rect.shift(UP)
        video_rects = VGroup(*[video_rect.copy().set_color(color) for color in [BLUE_E, BLUE_C, BLUE_D, GREY_BROWN]])
        for rect in video_rects[1::2]:
            rect.reverse_points()
        video_rect.set_fill(GREY_D, 0.5)
        video_rect.set_stroke(GREY_BROWN, 0.5)
        date = OldTexText('Solution will be\\\\posted', '1/20/19')
        ...
        for n in range(10):
            self.play(FadeOut(video_rects[(n - 1) % 4]), ShowCreation(video_rects[n % 4]), run_time=2)

class Thumbnail(BlocksAndWallExample, MovingCameraScene):
    CONFIG = {'sliding_blocks_config': {'block1_config': {'mass': 10000.0, 'velocity': -1.5}, 'collect_clack_data': False}, 'wait_time': 0, 'count_clacks': False, 'show_flash_animations': False, 'floor_y': -3.0, 'include_sound': False}
    ...
    def add_vector(self):
        blocks = self.blocks
        arrow = self.arrow = Vector(2.5 * LEFT, color=RED, rectangular_stem_width=1.5, tip_length=0.5)
