...
class ConnectionToOptics(Scene):
    ...
    def construct(self):
        ...
        for k in [0, 2]:
            ...
            rects.add(rect)
        titles = VGroup(OldTexText('Kinematics'), OldTexText('Optics'))
        titles.scale(1.5)
        ...
        self.wait(4)
        for k in range(2):
            anims = [ShowCreation(rects[k]), FadeInFromDown(titles[k])]
            if k == 0:
    ...
    def get_kinematics_groups(self):
        tex_to_color_map = {'m_1': BLUE, 'm_2': BLUE, 'v_1': RED, 'v_2': RED}
        energy_eq = OldTex('\\frac{1}{2} m_1 (v_1)^2 + \\frac{1}{2} m_2 (v_2)^2 = \\text{const.}', tex_to_color_map=tex_to_color_map)
        energy_eq.scale(0.8)
    ...
    def get_angle_group(self):
        title = VGroup(*map(TexText, ['Angle of\\\\Incidence', '=', 'Angle of\\\\Reflection'])).arrange(RIGHT)
        title.set_color(YELLOW)
        h_line = Line(LEFT, RIGHT)
        h_line.match_width(title)
        h_line.set_stroke(GREY_B)
        h_line.set_sheen(1, UL)
        points = [h_line.get_left() + UP, h_line.get_center(), h_line.get_right() + UP]
        ...
        arcs = VGroup(Arc(start_angle=0, angle=theta), Arc(start_angle=PI, angle=-theta))
...
class RearrangeMomentumEquation(ShowMomentumConservation):
    ...
    def construct(self):
        self.add(FullScreenFadeRectangle(fill_color=BLACK, fill_opacity=0.95))
    ...
    def show_initial_dot_product(self):
        ...
        self.add(equation)
        self.play(FadeInFromDown(VGroup(m_array.get_brackets(), dot, v_array.get_brackets(), rhs)))
        self.play(TransformFromCopy(equation.get_parts_by_tex('m_'), m_array.get_entries()))
        self.play(TransformFromCopy(equation.get_parts_by_tex('v_'), v_array.get_entries()))
        self.wait()
    ...
    def show_with_x_and_y(self):
        ...
        new_equation = OldTex('\\sqrt{m_1}', '\\left(', '\\sqrt{m_1}', 'v_1', '\\right)', '+', '\\sqrt{m_2}', '\\left(', '\\sqrt{m_2}', 'v_2', '\\right)', '=', '\\text{const.}')
        new_equation.set_color_by_tex_to_color_map({'m_': BLUE, 'v_': RED})
        ...
        self.play(FadeIn(new_equation, UP), simple_dot_product.to_edge, DOWN, LARGE_BUFF)
        self.wait()
        self.play(GrowFromCenter(x_brace), GrowFromCenter(y_brace), FadeIn(dx_dt, UP), FadeIn(dy_dt, UP))
        self.wait()
        self.play(FadeIn(VGroup(m_array.get_brackets(), dot, d_array.get_brackets(), rhs)), MoveToTarget(new_eq_group))
        self.play(TransformFromCopy(VGroup(VGroup(new_equation[0]), VGroup(new_equation[6])), m_array.get_entries()))
        self.play(TransformFromCopy(VGroup(dx_dt, dy_dt), d_array.get_entries()))
        self.wait()
