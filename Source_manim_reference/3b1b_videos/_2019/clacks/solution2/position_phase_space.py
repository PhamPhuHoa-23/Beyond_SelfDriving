...
class PositionPhaseSpaceScene(Scene):
    ...
    def ds_to_point(self, d1, d2):
        return self.axes.coords_to_point(self.d1_to_x(d1), self.d2_to_y(d2))
    ...
    def time_to_ds(self, time):
        ...
        def ps_point_to_ds(point):
            return (point[0] / np.sqrt(m1), point[1] / np.sqrt(m2))
        ...
        if n % 2 == 0:
            ...
            ps_point = rotate_vector(ps_point, -(n + 1) * theta)
    ...
    def get_clack_data(self):
        ...
        for k in range(1, int(PI / theta) + 1):
            ...
            reflected_point = rotate_vector(clack_ps_point, -2 * np.ceil((k - 1) / 2) * theta)
    ...
    def add_clack_flashes(self):
        if hasattr(self, 'flash_anims'):
            ...
            self.block_flashes = ClackFlashes([(loc1, time) for time, loc1, loc2 in clack_data])
            self.ps_flashes = ClackFlashes([(loc2, time) for time, loc1, loc2 in clack_data])
    ...
    def get_continually_building_trajectory(self):
        ...
        def update_trajectory(trajectory):
            ...
            angle = angle_between_vectors(p2 - p1, new_point - p2)
            if angle > epsilon:
    ...
    def get_ps_point_change_anim(self, d1, d2, **added_kwargs):
        ...
        kwargs = {'run_time': distance / ps_speed, 'rate_func': linear}
        kwargs.update(added_kwargs)
        return ApplyMethod(self.ps_point.move_to, self.ds_to_point(d1, d2), **kwargs)

    def get_floor(self):
        floor = self.floor = Line(self.wall_x * RIGHT, FRAME_WIDTH * RIGHT / 2, stroke_color=WHITE, stroke_width=3)
    ...
    def get_added_axes_lines(self, axes):
        ...
        y_lines = VGroup(*[Line(c2p(0, 0), c2p(0, axes.y_max * y_mult + 1)).move_to(c2p(x, 0), DOWN) for x in np.arange(0, axes.x_max) * x_mult])
        x_lines = VGroup(*[Line(c2p(0, 0), c2p(axes.x_max * x_mult, 0)).move_to(c2p(0, y), LEFT) for y in np.arange(0, axes.y_max) * y_mult])
    ...
    def get_phase_space_point(self):
        ...
        ps_point.move_to(self.ds_to_point(self.block1.distance, self.block2.distance + self.block2.width))
    ...
    def get_phase_space_x_line(self):
        ...
        def get_x_line():
            ...
            return DashedLine(y_axis_point, point, **self.ps_x_line_config)
    ...
    def get_phase_space_y_line(self):
        ...
        def get_y_line():
            ...
            return DashedLine(x_axis_point, point, **self.ps_y_line_config)
    ...
    def get_d_label(self, n, get_d):
        label = VGroup(OldTex('d_{}'.format(n), '='), DecimalNumber())
        ...
        def update_value(label):
            lhs, rhs = label
            rhs.set_value(get_d())
            rhs.next_to(lhs, RIGHT, SMALL_BUFF, aligned_edge=DOWN)
    ...
    def get_phase_space_d1_label(self):
        self.ps_d1_label = self.get_phase_space_d_label(1, self.get_d1, self.x_line, UP)
        return self.ps_d1_label

    def get_phase_space_d2_label(self):
        self.ps_d2_label = self.get_phase_space_d_label(2, self.get_d2, self.y_line, self.ps_d2_label_vect)
    ...
    def get_d1_label(self):
        self.d1_label = self.get_brace_d_label(1, self.get_d1, self.d1_brace, UP, SMALL_BUFF)
    ...
    def get_ps_velocity_vector(self, trajectory):
        vector = Vector(self.ps_velocity_vector_length * LEFT, **self.ps_velocity_vector_config)
    ...
    def get_block_velocity_vectors(self, ps_vect):
        blocks = self.blocks
        vectors = VGroup(*[Vector(LEFT, **self.block_velocity_vector_config) for x in range(2)])
...
class IntroducePositionPhaseSpace(PositionPhaseSpaceScene):
    CONFIG = {'rescale_coordinates': False, 'd1_eq_d2_tex': 'x = y', 'block1_config': {'velocity': 1.5}, 'slide_wait_time': 12}

    def setup(self):
        super().setup()
        self.add(self.floor, self.wall, self.blocks, self.axes)
    ...
    def show_coordinates(self):
        ...
        self.play(ShowCreation(self.x_line), GrowFromPoint(self.d1_brace, self.d1_brace.get_left()), Indicate(axes.labels[0]))
        self.play(FadeInFromDown(self.ps_d1_label), FadeInFromDown(self.d1_label))
        self.play(ps_point.shift, 0.5 * LEFT)
        self.play(ps_point.shift, 0.5 * RIGHT)
        self.wait()
        self.play(ShowCreation(self.y_line), GrowFromPoint(self.d2_brace, self.d2_brace.get_left()), Indicate(axes.labels[1]))
        self.play(FadeInFromDown(self.ps_d2_label), FadeInFromDown(self.d2_label))
        self.play(ps_point.shift, 0.5 * UP)
        self.play(ps_point.shift, 0.5 * DOWN)
        self.wait()
        self.play(Rotating(ps_point, about_point=ps_point.get_location() + 0.5 * RIGHT, run_time=3, rate_func=smooth))
        self.wait()
    ...
    def show_xy_line(self):
        ...
        self.play(ShowCreation(xy_line), Write(xy_label))
...
class EqualMassCase(PositionPhaseSpaceScene):
    CONFIG = {'block1_config': {'mass': 1, 'width': 1, 'velocity': 1.5}, 'rescale_coordinates': False, 'd1_eq_d2_tex': 'x = y'}

    def setup(self):
        super().setup()
        self.add(self.floor, self.wall, self.blocks, self.axes, self.d1_eq_d2_line, self.d1_eq_d2_label, self.d2_eq_w2_line, self.d2_eq_w2_label, self.ps_dot, self.x_line, self.y_line, self.ps_d1_label, self.ps_d2_label)
    ...
    def show_same_mass(self):
        blocks = self.blocks
        self.play(LaggedStartMap(Indicate, blocks, lag_ratio=0.8, run_time=1))
    ...
    def show_first_point(self):
        ...
        self.play(ShowCreationThenFadeOut(Circle(color=RED).replace(ps_dot).scale(2), run_time=1))
        self.wait()
        self.play(ps_point.move_to, self.ds_to_point(d1 - 1, d2), rate_func=wiggle, run_time=3)
        self.wait()
    ...
    def up_to_first_collision(self):
        ...
        self.play(GrowArrow(block_arrow), GrowArrow(ps_arrow))
        self.add(trajectory)
        ...
        self.play(Flash(ps_point), Flash(block1.get_left()), self.get_ps_point_change_anim(d2, d2 - 1))
        self.play(ShowPassingFlash(xy_line.copy().set_stroke(YELLOW, 3)), Indicate(xy_line_label))
    ...
    def up_to_second_collision(self):
        ...
        self.add_sound(self.clack_sound)
        self.play(Flash(self.block2.get_left()), Flash(ps_point), self.get_ps_point_change_anim(d1, w2 + 1))
        trajectory.suspend_updating()
        self.wait()
    ...
    def up_to_third_collision(self):
        ...
        ps_arrow.next_to(ps_point, DR, SMALL_BUFF)
        self.add_sound(self.clack_sound)
        self.play(Flash(self.block2.get_left()), Flash(ps_point.get_location()), self.get_ps_point_change_anim(d1 + 10, d1))
        trajectory.suspend_updating()

    def fade_distance_indicators(self):
        trajectory = self.trajectory
        self.play(trajectory.set_stroke, {'width': 1}, *map(FadeOut, [self.ps_arrow, self.block_arrow, self.x_line, self.y_line, self.ps_d1_label, self.ps_d2_label]))
    ...
    def show_beam_bouncing(self):
        ...
        trajectory = VMobject()
        trajectory.set_points_as_corners(points)
        flashes = [SpecialShowPassingFlash(trajectory.copy().set_stroke(YELLOW, width=6 - n), time_width=0.01 * n, max_time_width=0.05, remover=True) for n in np.arange(0, 6, 0.25)]
        flash_mob = flashes[0].mobject
        ...
        for line in lines:
            ...
            self.play(Write(line.word), line.set_sheen, 1, RIGHT, line.set_stroke, {'width': 2}, run_time=1)
        for x in range(3):
            self.play(UpdateFromFunc(ps_point, update_ps_point_from_flas_mob), *flashes, run_time=3, rate_func=linear)
            self.wait()

class FailedAngleRelation(PositionPhaseSpaceScene):
    CONFIG = {'block1_config': {'distance': 10, 'velocity': -1.5}, 'block2_config': {'distance': 5}, 'rescale_coordinates': False, 'trajectory_style': {'stroke_width': 2}}

    def setup(self):
        super().setup()
        self.add(self.floor, self.wall, self.blocks, self.axes, self.ps_dot, self.x_line, self.y_line, self.d1_eq_d2_line, self.d1_eq_d2_label, self.d2_eq_w2_line, self.d2_eq_w2_label)
    ...
    def show_angles(self):
        ...
        equation.next_to(trajectory.get_points()[0], UR, MED_SMALL_BUFF, index_of_submobject_to_align=0)
        for arc in arcs:
            ...
            arc.line = line
        arc1, arc2 = arcs
        arc1.arrow = Arrow(equation[0].get_left(), arc1.get_right(), buff=SMALL_BUFF, color=WHITE, path_arc=0)
        arc2.arrow = Arrow(equation[2].get_corner(DL), arc2.get_left(), path_arc=-120 * DEGREES, buff=SMALL_BUFF)
        arc2.arrow.pointwise_become_partial(arc.arrow, 0, 0.95)
        arc1.word = equation[0]
        arc2.word = equation[1:]
        for arc in arcs:
            self.play(FadeIn(arc.word, LEFT), GrowArrow(arc.arrow, path_arc=arc.arrow.path_arc))
            self.play(ShowCreation(arc), arc.line.rotate, arc.angle, {'about_point': arc.line.get_start()}, UpdateFromAlphaFunc(arc.line, lambda m, a: m.set_stroke(opacity=there_and_back(a) ** 0.5)))

    def get_arcs(self, trajectory):
        p0, p1, p2 = trajectory.get_anchors()[1:4]
        arc_config = {'stroke_color': WHITE, 'stroke_width': 2, 'radius': 0.5, 'arc_center': p1}
    ...
    def get_word_equation(self):
        result = VGroup(OldTexText('Angle of incidence'), OldTex('\\ne').rotate(90 * DEGREES), OldTexText('Angle of reflection'))
        result.arrange(DOWN)
        result.set_stroke(BLACK, 5, background=True)
        return result

class UnscaledPositionPhaseSpaceMass10(FailedAngleRelation):
    CONFIG = {'block1_config': {'mass': 10}, 'wait_time': 25}
...
class RescaleCoordinates(PositionPhaseSpaceScene, MovingCameraScene):
    CONFIG = {'rescale_coordinates': False, 'ps_d2_label_vect': LEFT, 'axes_center': 6 * LEFT + 0.65 * DOWN, 'block1_config': {'distance': 7}, 'wait_time': 30}

    def setup(self):
        PositionPhaseSpaceScene.setup(self)
        MovingCameraScene.setup(self)
        self.add(self.floor, self.wall, self.blocks, self.axes, self.d1_eq_d2_line, self.d1_eq_d2_label, self.d2_eq_w2_line, self.ps_dot, self.x_line, self.y_line, self.ps_d1_label, self.ps_d2_label, self.d1_brace, self.d2_brace, self.d1_label, self.d2_label)
    ...
    def show_rescaling(self):
        axes = self.axes
        blocks = self.blocks
        to_stretch = VGroup(axes.added_lines, self.d1_eq_d2_line, self.ps_point)
        m1 = self.block1.mass
        ...
        def show_label(index, block, vect):
            ...
            self.play(Transform(axes.labels[index], VGroup(*new_axes_labels[index][:2], new_axes_labels[index][3])), GrowFromCenter(new_axes_labels[index][2]))
            group = VGroup(new_axes_labels[index][2][-2:].copy(), OldTex('='), block.label.copy())
            ...
            self.play(MoveToTarget(group, rate_func=there_and_back_with_pause, run_time=3))
            self.remove(group)
            self.wait()
        show_label(0, self.block1, RIGHT)
        blocks.suspend_updating()
        self.play(ApplyMethod(to_stretch.stretch, np.sqrt(m1), 0, {'about_point': axes.coords_to_point(0, 0)}), self.d1_eq_d2_label.shift, 6 * RIGHT, run_time=2)
        self.rescale_coordinates = True
        blocks.resume_updating()
        self.wait()
        d1, d2 = self.get_ds()
        for new_d1 in [d1 - 2, d1]:
            self.play(self.get_ps_point_change_anim(new_d1, d2, rate_func=smooth, run_time=2))
        self.wait()
    ...
    def comment_on_ugliness(self):
        ...
        self.play(PiCreatureSays(randy, 'Hideous!', bubble_config={'height': 1.5, 'width': 2}, target_mode='angry', look_at=axes.labels[0]))
    ...
    def put_into_frame(self):
        ...
        self.begin_sliding()
        self.add(rect)
        self.play(frame.scale, 1.5, {'about_point': frame.get_bottom() + UP}, run_time=2)
    ...
    def get_ds(self):
        if self.rescale_coordinates:
            return super().get_ds()
        return (self.block1_config['distance'], self.block2_config['distance'])

class RescaleCoordinatesMass16(RescaleCoordinates):
    CONFIG = {'block1_config': {'mass': 16, 'distance': 10}, 'rescale_coordinates': True, 'wait_time': 20}

    def construct(self):
        self.put_into_frame()

class RescaleCoordinatesMass64(RescaleCoordinatesMass16):
    CONFIG = {'block1_config': {'mass': 64, 'distance': 6}, 'block2_config': {'distance': 3}}

class RescaleCoordinatesMass100(RescaleCoordinatesMass16):
    CONFIG = {'block1_config': {'mass': 100, 'distance': 6, 'velocity': 0.5}, 'block2_config': {'distance': 2}, 'wait_time': 25}

class IntroduceVelocityVector(PositionPhaseSpaceScene, MovingCameraScene):
    CONFIG = {'zoom': True, 'ps_x_line_config': {'color': WHITE, 'stroke_width': 1, 'stroke_opacity': 0.5}, 'ps_y_line_config': {'color': WHITE, 'stroke_width': 1, 'stroke_opacity': 0.5}, 'axes_center': 6 * LEFT + 0.65 * DOWN, 'slide_time': 20, 'new_vect_config': {'tip_length': 0.1, 'rectangular_stem_width': 0.02}}

    def setup(self):
        MovingCameraScene.setup(self)
        PositionPhaseSpaceScene.setup(self)
        self.add(self.floor, self.wall, self.blocks, self.axes, self.d1_eq_d2_line, self.d1_eq_d2_label, self.d2_eq_w2_line, self.x_line, self.y_line, self.ps_dot)
    ...
    def contrast_with_physical_velocities(self):
        ...
        self.play(Rotating(ps_vect, angle=TAU, about_point=ps_vect.get_start(), run_time=5, rate_func=smooth))
        self.wait()
    ...
    def zoom_in_on_vector(self):
        if not self.zoom:
            ...
            return
        ps_vect = self.ps_velocity_vector
        new_vect = Arrow(ps_vect.get_start(), ps_vect.get_end(), buff=0, **self.new_vect_config)
        new_vect.match_style(ps_vect)
        camera_frame = self.camera_frame
        camera_frame.save_state()
        point = self.ps_point.get_location()
        point += MED_SMALL_BUFF * DOWN
        self.play(camera_frame.scale, 0.25, {'about_point': point}, Transform(ps_vect, new_vect), run_time=2)
        self.wait()
    ...
    def break_down_components(self):
        ...
        x_line, y_line = [DashedLine(ps_vect.get_end(), vect.get_end(), dash_length=0.01, color=vect.get_color()) for vect in (x_vect, y_vect)]
        ...
        self.play(TransformFromCopy(ps_vect, x_vect), ShowCreation(x_line))
        self.play(FadeIn(dx_label, 0.25 * DOWN))
        self.wait()
        self.play(TransformFromCopy(ps_vect, y_vect), ShowCreation(y_line))
    ...
    def zoom_out(self):
        ...
        self.play(Restore(self.camera_frame), ApplyFunction(self.grow_labels, labels), run_time=2)
    ...
    def relate_x_dot_y_dot_to_v1_v2(self):
        ...
        x_eq = x_label[1]
        dx_eq = OldTex('=')
        dx_eq.next_to(x_eq, DOWN, buff=LARGE_BUFF, aligned_edge=RIGHT)
        ...
        self.wait()
        self.play(VGroup(block1, m_part).shift, SMALL_BUFF * UP, rate_func=wiggle)
        self.wait()
        self.d1_brace.update()
        self.d1_label.update()
        self.play(ShowCreationThenFadeAround(x_label[3]), FadeIn(self.d1_brace), FadeIn(self.d1_label))
        self.wait()
        self.play(TransformFromCopy(x_label[3], dx_rhs[1]), TransformFromCopy(x_label[2], VGroup(dx_rhs[0])))
        block_vectors.suspend_updating()
        self.play(alt_v1.next_to, block_vectors[0], UP, SMALL_BUFF)
        self.play(Rotate(block_vectors[0], 10 * DEGREES, about_point=block_vectors[0].get_start(), rate_func=wiggle, run_time=1))
        ...
        self.play(Write(dy_eq), Write(dy_rhs))
        self.play(alt_v2.next_to, block_vectors[1], UP, SMALL_BUFF)
        self.wait()
        self.play(FadeOut(alt_v2))
        self.wait()
        self.derivative_equations = VGroup(VGroup(dx_label, dx_eq, dx_rhs), VGroup(dy_label, dy_eq, dy_rhs))

    def calculate_magnitude(self):
        corner_rect = Rectangle(stroke_color=WHITE, stroke_width=1, fill_color=BLACK, fill_opacity=1, height=2.5, width=8.5)
        corner_rect.to_corner(UR, buff=0)
        ps_vect = self.ps_velocity_vector
        big_ps_vect = Arrow(ps_vect.get_start(), ps_vect.get_end(), buff=0)
        ...
        rhs_scale_val = 0.8
        rhs = OldTex('=\\sqrt{\\left( dx/dt \\right)^2 + \\left( dy/dt \\right)^2}')
        ...
        new_rhs = OldTex('=', '\\sqrt', '{m_1(v_1)^2 + m_2(v_2)^2}', tex_to_color_map={'m_1': BLUE, 'm_2': BLUE, 'v_1': GREEN, 'v_2': RED})
    ...
    def let_process_play_out(self):
        self.play(*map(FadeOut, [self.projection_lines, self.derivative_labels, self.component_vectors, self.d1_brace, self.d1_label]))
...
class ShowMomentumConservation(IntroduceVelocityVector):
    CONFIG = {'ps_velocity_vector_length': 1.25, 'block_velocity_vector_length_multiple': 1, 'block1_config': {'distance': 7}, 'axes_config': {'y_max': 11}, 'axes_center': 6.5 * LEFT + 1.2 * DOWN, 'floor_y': -3.75, 'wait_time': 15}
    ...
    def add_velocity_vectors(self):
        ...
        self.play(GrowArrow(self.ps_vect), LaggedStartMap(GrowArrow, self.block_vectors, run_time=1))
    ...
    def contrast_d1_d2_line_with_xy_line(self):
        ...
        self.play(ShowPassingFlash(line.copy().set_stroke(YELLOW, 4)), Write(label))
    ...
    def rearrange_for_slope(self):
        ...
        new_eqs = VGroup(OldTex('{y', '\\over', '\\sqrt{m_2}}', '=', 'd_2'), OldTex('{x', '\\over', '\\sqrt{m_1}}', '=', 'd_1'))
        ...
        for eq in eqs:
            eq.generate_target()
        VGroup(x_eq.target[4], x_eq.target[3], x_eq.target[:3]).arrange(RIGHT)
        ...
        self.play(MoveToTarget(y_eq), MoveToTarget(x_eq, path_arc=90 * DEGREES), GrowFromCenter(equals))
        self.wait()
        final_eq = OldTex('y', '=', '{\\sqrt{m_2}', '\\over', '\\sqrt{m_1}}', 'x')
        for part in final_eq.get_parts_by_tex('sqrt'):
            part[2:].set_color(BLUE)
        m_part = final_eq[2:5]
        final_eq.next_to(group, DOWN)
        final_eq.shift(0.4 * UP)
        movers = VGroup(y_eq[0], equals.submobjects[0], y_eq[2], y_eq[1], x_eq[2], x_eq[0]).copy()
        for mover, part in zip(movers, final_eq):
            mover.target = part
        self.play(LaggedStartMap(MoveToTarget, movers, path_arc=30 * DEGREES, lag_ratio=0.9), VGroup(x_eq, equals, y_eq).scale, 0.7, {'about_edge': UP})
        self.remove(movers)
        self.add(final_eq)
        self.wait()
        ...
        bubble = ThoughtBubble(height=1.3, width=1.3, direction=RIGHT)
        bubble.pin_to(randy)
        ...
        self.play(Blink(randy))
        self.thinking_on_slope_group = VGroup(randy, bubble, slope)
        self.to_fade = VGroup(eqs, equals, final_eq, self.thinking_on_slope_group, self.xy_group)
    ...
    def show_conservation_of_momentum(self):
        ...
        x_label, y_label = self.axes.labels
        self.play(FadeOut(self.whats_next_question), FadeIn(dot_product), Transform(x_label[2].copy(), sqrty_m_array.get_entries()[0], remover=True), Transform(y_label[2].copy(), sqrty_m_array.get_entries()[1], remover=True))
    ...
    def show_rate_of_change_vector(self):
        ...
        d_array.scale(0.75)
        d_array.add_updater(lambda m: m.next_to(ps_vect.get_end(), np.sign(ps_vect.get_vector()[0]) * RIGHT, SMALL_BUFF))
    ...
    def show_sqrty_m_vector(self):
        ...
        sqrty_m_vector = Arrow(self.ds_to_point(0, 0), self.ds_to_point(2, 2), buff=0, color=YELLOW)
        ...
        rise = DashedLine(sqrty_m_vector.get_end(), sqrty_m_vector.get_corner(DR), color=RED)
        run = DashedLine(sqrty_m_vector.get_corner(DR), sqrty_m_vector.get_start(), color=GREEN)
        ...
        self.play(TransformFromCopy(original_sqrty_m_array, sqrty_m_array))
        self.play(FadeIn(randy_group))
        self.play(ShowCreation(rise), TransformFromCopy(sqrty_m_array.get_entries()[1], rise_label))
        self.add(run, randy_group)
        self.play(ShowCreation(run), TransformFromCopy(sqrty_m_array.get_entries()[0], run_label))
        self.wait()
        self.play(FadeOut(randy_group))
        self.play(FadeOut(VGroup(rise, run, rise_label, run_label)))
        point = self.ps_point.get_location()
        sqrty_m_vector.generate_target()
        sqrty_m_vector.target.shift(point - sqrty_m_vector.get_start())
        sqrty_m_array.generate_target()
        sqrty_m_array.target.next_to(sqrty_m_vector.target.get_end(), RIGHT, SMALL_BUFF)
    ...
    def show_dot_product(self):
        ...
        self.play(ShowCreationThenFadeAround(big_d_array), ShowCreationThenFadeAround(d_array))
        self.play(ShowCreationThenFadeAround(big_m_array), ShowCreationThenFadeAround(m_array))
        ps_vect = self.ps_velocity_vector
        ...
        for x in range(2):
            for u in [-1, 1]:
                ...
                self.wait()
        ps_vect.resume_updating()
        circle = Circle(radius=ps_vect.get_length(), arc_center=ps_vect.get_start(), color=RED, stroke_width=1)
        self.play(Rotating(ps_vect, about_point=ps_vect.get_start(), run_time=5, rate_func=lambda t: smooth(t, 3)), FadeIn(circle))
        self.wait()
    ...
    def show_same_angles(self):
        ...
        theta = self.theta
        ghost_ps_vect.rotate(-2 * theta, about_point=ghost_ps_vect.get_start())
        arc1 = Arc(start_angle=PI, angle=theta, arc_center=point, radius=0.5, color=WHITE)
        arc2 = arc1.copy()
        arc2.rotate(theta, about_point=point)
        arc3 = arc1.copy()
        arc3.rotate(PI, about_point=point)
        arc1.set_color(BLUE)
        line_pair = VGroup(*[Line(point, point + LEFT).rotate(angle, about_point=point) for angle in [0, theta]])
        ...
        self.wait(0.5)
        self.end_sliding()
        self.play(TransformFromCopy(arc1, arc3, path_arc=-PI), Rotate(line_pair, -PI, about_point=point), UpdateFromAlphaFunc(line_pair, lambda m, a: m.set_stroke(width=there_and_back(a) ** 0.5)))
    ...
    def show_horizontal_bounce(self):
        ...
        point = self.ps_point.get_location()
        theta = self.theta
        arc1 = Arc(start_angle=0, angle=2 * theta, radius=0.5, arc_center=point)
        arc2 = arc1.copy()
        arc2.rotate(PI - 2 * theta, about_point=point)
        arcs = VGroup(arc1, arc2)
        self.slide(0.5)
        self.play(LaggedStartMap(FadeInFromLarge, arcs, lag_ratio=0.75))
        self.show_trajectory_beam_bounce()
    ...
    def show_trajectory_beam_bounce(self, n_times=2):
        ...
        for x in range(n_times):
            self.play(ShowPassingFlash(beam, run_time=2, rate_func=bezier([0, 0, 1, 1])))

    def get_momentum_equation(self):
        equation = OldTex('m_1', 'v_1', '+', 'm_2', 'v_2', '=', '\\text{const.}', tex_to_color_map={'m_1': BLUE, 'm_2': BLUE, 'v_1': RED, 'v_2': RED})
        equation.to_edge(UP, buff=MED_SMALL_BUFF)
        return equation

    def get_dot_product(self, m1='\\sqrt{m_1}', m2='\\sqrt{m_2}', d1='dx/dt', d2='dy/dt'):
...
class JustTheProcessNew(PositionPhaseSpaceScene):
    CONFIG = {'block1_config': {'mass': 16, 'velocity': -2}, 'wait_time': 10}

    def setup(self):
        super().setup()
        self.add(self.floor, self.wall, self.blocks, self.axes, self.d1_eq_d2_line, self.d2_eq_w2_line)
