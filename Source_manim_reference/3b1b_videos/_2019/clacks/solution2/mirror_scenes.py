...
class MirrorScene(Scene):
    CONFIG = {'center': DOWN + 3 * LEFT, 'line_length': FRAME_WIDTH, 'start_theta': np.arctan(0.25), 'start_y_offset': 0.5, 'start_x_offset': 8, 'arc_config': {'radius': 1, 'stroke_color': WHITE, 'stroke_width': 2}, 'trajectory_point_spacing': 0.1, 'trajectory_style': {'stroke_color': YELLOW, 'stroke_width': 2}, 'ghost_lines_style': {'stroke_color': WHITE, 'stroke_width': 1, 'stroke_opacity': 0.5}, 'reflect_sound': 'pen_click'}
    ...
    def setup(self):
        ...
        self.add(self.mirrors, self.arc, self.theta_symbol, self.theta_display, self.count_display_word, self.count_display_number)
    ...
    def get_mirror(self):
        mirror = VGroup(Line(ORIGIN, 2 * RIGHT), Line(ORIGIN, 2 * RIGHT), Line(ORIGIN, (self.line_length - 4) * RIGHT))
    ...
    def get_arc(self, radius=0.5):
        return always_redraw(lambda: Arc(start_angle=0, angle=self.get_theta(), arc_center=self.get_center(), **self.arc_config))
    ...
    def get_ghost_collision_points(self):
        ...
        points += [np.array([x, y, 0]) for k in range(1, int(PI / theta) + 1) for x in [y / np.tan(k * theta)] if abs(x) < FRAME_WIDTH]
    ...
    def get_count_display_number(self, count_display_word=None, ghost_beam_point=None):
        ...
        result = Integer()
        result.next_to(count_display_word[-1], RIGHT, aligned_edge=DOWN)
        result.set_color(YELLOW)
    ...
    def update_mobjects(self, dt):
        ...
        if self.get_count() != self.last_count:
            ...
            if self.is_sound_allowed:
                self.add_sound(self.reflect_sound, gain=-20)
    ...
    def get_special_flash(self, mobject, stroke_width, time_width, rate_func=linear, **kwargs):
        ...
        return UpdateFromAlphaFunc(mob_copy, lambda m, a: m.pointwise_become_partial(mobject, max(a - (1 - a) * m.time_width, 0), a), **kwargs)

    def get_shooting_beam_anims(self, trajectory, ghost_trajectory=None, update_beam_point=True, num_flashes=20, min_time_width=0.01, max_time_width=0.5, min_stroke_width=0.01, max_stroke_width=6, fade_trajectory=True, faded_trajectory_width=0.25, faded_trajectory_time_exp=0.2):
        result = [self.get_special_flash(trajectory, stroke_width, time_width) for stroke_width, time_width in zip(np.linspace(max_stroke_width, min_stroke_width, num_flashes), np.linspace(min_time_width, max_time_width, num_flashes))]
        if update_beam_point:
            smallest_flash = result[0]
            result.append(UpdateFromFunc(self.beam_point, lambda m: m.move_to(smallest_flash.mobject.get_points()[-1])))
        if ghost_trajectory:
            ghost_flash = self.get_special_flash(ghost_trajectory, 0, min_time_width)
            ghost_beam_point_update = UpdateFromFunc(self.ghost_beam_point, lambda m: m.move_to(ghost_flash.mobject.get_points()[-1]))
            result += [ghost_flash, ghost_beam_point_update]
        if fade_trajectory:
            ftte = faded_trajectory_time_exp
            result.append(ApplyMethod(trajectory.set_stroke, {'width': faded_trajectory_width}, rate_func=lambda t: there_and_back(t) ** ftte))
        return result
...
class ShowTrajectoryWithChangingTheta(MirrorScene):
    ...
    def construct(self):
        ...
        for angle, y in zip(angles, ys):
            ...
            self.play(self.theta_tracker.set_value, angle, self.start_y_offset_tracker.set_value, y, FadeIn(rect, rate_func=there_and_back, remover=True), UpdateFromFunc(trajectory, lambda m: m.become(self.get_trajectory())), run_time=2)
            self.show_bouncing()
        self.wait(2)

class ReflectWorldThroughMirrorNew(MirrorScene):
    CONFIG = {'start_y_offset': 1.25, 'center': DOWN, 'randy_height': 1, 'partial_trajectory_values': [0, 0.22, 0.28, 0.315, 1]}
    ...
    def shift_displays(self):
        VGroup(self.theta_display, self.count_display_word, self.count_display_number).to_edge(DOWN)
    ...
    def create_reflected_worlds(self):
        ...
        world = self.world = VGroup(triangle, mirrors, self.arc, self.theta_symbol, self.randy)
    ...
    def first_reflection(self):
        ...
        self.play(TransformFromCopy(world, reflected_world), TransformFromCopy(trajectory, reflected_trajectory), run_time=2)
        beam_anims = self.get_shooting_beam_anims(ghost_trajectory, fade_trajectory=False)
        self.play(*[ApplyMethod(m.set_stroke, GREY, 1) for m in (trajectory, reflected_trajectory)] + beam_anims, run_time=2)
        for x in range(2):
    ...
    def next_reflection(self, index=2):
        i = index
        self.play(*self.get_both_partial_trajectory_anims(*self.partial_trajectory_values[i - 1:i + 1]), UpdateFromFunc(VMobject(), lambda m: self.reflected_trajectories.update(), remover=True))
        anims = [TransformFromCopy(*reflections[i - 2:i]) for reflections in [self.reflected_worlds, self.reflected_trajectories]]
    ...
    def unfold_all_reflected_worlds(self):
        ...
        pairs = [(VGroup(w1, t1), VGroup(w2, t2)) for w1, w2, t1, t2 in zip(worlds[2:], worlds[3:], trajectories[2:], trajectories[3:])]
        new_worlds = VGroup()
        for m1, m2 in pairs:
    ...
    def show_all_trajectories(self):
        ...
        self.play(FadeOut(ghost_trajectory), trajectories.set_stroke, YELLOW, 0.5, all_mirrors.set_stroke, {'width': 1})
        flash_groups = [self.get_shooting_beam_anims(mob, fade_trajectory=False) for mob in trajectories]
        ...
        red_ghost_beam = self.get_shooting_beam_anims(red_ghost, fade_trajectory=False)
        num_repeats = 3
    ...
    def focus_on_two_important_trajectories(self):
        ...
        self.play(FadeOut(self.reflected_trajectories), FadeIn(self.ghost_trajectory), self.trajectory.set_stroke, YELLOW, 1)
    ...
    def get_reflected_worlds(self, world, n_reflections=None):
        ...
        for n in range(n_reflections):
            ...
            reflected_world.rotate(PI, axis=vect, about_point=center)
    ...
    def get_partial_trajectory_anims(self, trajectory, a, b, **kwargs):
        ...
        return UpdateFromAlphaFunc(trajectory, lambda m, alpha: m.pointwise_become_partial(m.full_self, 0, interpolate(a, b, alpha)), **kwargs)

    def get_both_partial_trajectory_anims(self, a, b, run_time=2, **kwargs):
        kwargs['run_time'] = run_time
        return [self.get_partial_trajectory_anims(mob, a, b, **kwargs) for mob in (self.trajectory, self.ghost_trajectory)]

    def add_flashing_windows(self):
        theta = self.get_theta()
        center = self.get_center()
        windows = self.windows = VGroup(*[Line(center, center + rotate_vector(10 * RIGHT, k * theta), color=BLUE, stroke_width=0) for k in range(0, self.get_count() + 1)])
...
class ReflectWorldThroughMirrorThetaPoint2(ReflectWorldThroughMirrorNew):
    CONFIG = {'start_theta': 0.2, 'randy_height': 0.8}

class ReflectWorldThroughMirrorThetaPoint1(ReflectWorldThroughMirrorNew):
    CONFIG = {'start_theta': 0.1, 'randy_height': 0.5, 'start_y_offset': 0.5, 'arc_config': {'radius': 0.5}}

class MirrorAndWiresOverlay(MirrorScene):
    CONFIG = {'wire_pixel_points': [(355, 574), (846, 438), (839, 629), (845, 288), (1273, 314)], 'max_x_pixel': 1440, 'max_y_pixel': 1440}
    ...
    def get_shooting_beam_anims(self, mobject, **new_kwargs):
        kwargs = {'update_beam_point': False, 'fade_trajectory': False, 'max_stroke_width': 10}
    ...
    def add_wires(self):
        ...
        points = self.wire_points = [ul_corner + np.array([x / self.max_x_pixel * FRAME_HEIGHT, -y / self.max_y_pixel * FRAME_HEIGHT, 0]) for x, y in self.wire_pixel_points]
        wires = self.wires = VGroup(Line(points[0], points[1]), Line(points[1], points[2]), Line(points[1], points[3]), Line(points[1], points[4]))
    ...
    def add_diagram(self):
        ...
        rect = diagram.rect = Rectangle(height=4, width=5, stroke_color=WHITE, stroke_width=1, fill_color=BLACK, fill_opacity=0.9)
        rect.to_corner(UR)
        diagram.add(rect)
        center = rect.get_center()
        mirror = diagram.mirror = VGroup(Line(rect.get_left(), center + 1.5 * LEFT), Line(center + 1.5 * LEFT, rect.get_right()))
        mirror.scale(0.8)
    ...
    def introduce_wires(self):
        ...
        def get_rect(wire):
            rect = Rectangle(width=wire.get_length(), height=0.25, color=YELLOW)
        ...
        self.play(FadeIn(diagram.rect), ShowCreation(diagram.mirror), LaggedStartMap(ShowCreation, diagram.wires), run_time=1)
        self.remove(diagram)
    ...
    def show_illusion(self):
        ...
        d_trajectory = self.d_trajectory = Line(self.diagram.wires[0].get_start(), self.diagram.wires[3].get_start())
        d_trajectory.match_style(g_trajectory)
        g_trajectory.get_points()[0] += 0.2 * RIGHT + 0.1 * DOWN
        g_trajectory.make_jagged()
        for x in range(3):
            self.play(*self.get_shooting_beam_anims(g_trajectory), *self.get_shooting_beam_anims(d_trajectory))
            self.wait()
    ...
    def show_angles(self):
        ...
        arc_config = {'radius': 0.5, 'arc_center': center}

        def get_dl_arc():
            return Arc(start_angle=PI, angle=dl_wire.get_angle(), **arc_config)
        dl_arc = always_redraw(get_dl_arc)

        def get_dr_arc():
            return Arc(start_angle=0, angle=dr_wire.get_angle() - PI, **arc_config)
        ...
        self.add(incidence)
        self.play(ShowCreation(dl_arc), UpdateFromAlphaFunc(VMobject(), lambda m, a: incidence.set_fill(opacity=a), remover=True))
        self.wait()
        self.add(reflection)
        self.play(ShowCreation(dr_arc), UpdateFromAlphaFunc(VMobject(), lambda m, a: reflection.set_fill(opacity=a), remover=True))
        self.wait()
        dr_wire.clear_updaters()
        for angle in [20 * DEGREES, -20 * DEGREES]:
            self.play(Rotate(dr_wire, angle, about_point=dr_wire.get_end(), run_time=2))
            self.play(*self.get_shooting_beam_anims(self.ghost_trajectory), *self.get_shooting_beam_anims(self.d_trajectory))
            self.wait()

    def show_reflecting_beam(self):
        self.play(*self.get_shooting_beam_anims(self.trajectory), *self.get_shooting_beam_anims(self.ghost_trajectory))
        self.wait()