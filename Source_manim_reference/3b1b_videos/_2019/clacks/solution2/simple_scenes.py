...
class WrapperScene(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(big_rect), FadeIn(title, DOWN), FadeIn(screen_rect, UP))
        self.wait()
...
class LeftEdge(Scene):
    CONFIG = {'text': 'Left edge', 'vect': LEFT}
...
class RightEdge(LeftEdge):
    CONFIG = {'text': 'Right edge', 'vect': RIGHT}

class NoteOnEnergyLostToSound(Scene):

    def construct(self):
        self.add(OldTexText("Yeah yeah, the clack sound\\\\would require energy, but\\\\don't let accuracy get in the\\\\way of delight!", alignment=''))
...
class ShowDotProductMeaning(Scene):
    ...
    def construct(self):
        ...
        title = OldTex('\\vec{\\textbf{w}}', '\\cdot', '\\vec{\\textbf{v}}', '=', '||', '\\vec{\\textbf{w}}', '||', '\\cdot', '||', '\\vec{\\textbf{v}}', '||', '\\cdot', '\\cos(\\theta)')
        title.set_color_by_tex_to_color_map({'textbf{v}': v_vect.get_color(), 'textbf{w}': w_vect.get_color()})
        title.to_edge(UP)

        def get_w_line():
            center = dot.get_center()
            direction = w_vect.get_vector()
            return Line(center - 3 * direction, center + 3 * direction, stroke_color=GREY_B, stroke_width=1)
        ...
        def get_proj_line():
            return DashedLine(v_vect.get_end(), proj_v.get_end(), stroke_width=1, dash_length=0.025)
        ...
        def get_arc():
            ...
            a1 = w_vect.get_angle()
            a2 = v_vect.get_angle()
            arc = Arc(start_angle=a1, angle=a2 - a1, radius=0.5, arc_center=center)
            theta = OldTex('\\theta')
            p = arc.point_from_proportion(0.5)
            theta.move_to(center + 1.5 * (p - center))
            return VGroup(arc, theta)
        arc = always_redraw(get_arc)
        self.add(title[:3], w_vect, v_vect, dot, w_label, v_label)
        self.play(v_vect.angle_tracker.set_value, 170 * DEGREES, w_vect.angle_tracker.set_value, 45 * DEGREES, run_time=2)
        self.wait()
        w_brace.update()
        ...
        proj_v.update()
        self.play(ShowCreation(proj_line), TransformFromCopy(v_vect, proj_v))
...
class FinalComment(Scene):
    ...
    def construct(self):
        self.add(OldTexText('Thoughts on what ending should go here?\\\\See the Patreon post.'))

class FourtyFiveDegreeLine(Scene):
    CONFIG = {'angle': 45 * DEGREES, 'label_config': {'num_decimal_places': 0, 'unit': '^\\circ', 'label_height': 0.3}, 'degrees': True}
    ...
    def construct(self):
        ...
        self.play(ChangeDecimalToValue(label, target_value), ShowCreation(arc), Rotate(line2, angle, about_point=ORIGIN))
        self.wait()

class ArctanSqrtPoint1Angle(FourtyFiveDegreeLine):
    CONFIG = {'angle': np.arctan(np.sqrt(0.1))}

class AskAboutAddingThetaToItself(Scene):
    CONFIG = {'theta': np.arctan(0.25), 'wait_time': 0.25, 'wedge_radius': 3, 'theta_symbol_scale_val': 0.5, 'number_height': 0.2}
    ...
    def get_group(self, theta):
        ...
        wedge = VGroup(Line(ORIGIN, wedge_radius * RIGHT), Line(ORIGIN, wedge_radius * RIGHT).rotate(theta, about_point=ORIGIN))
    ...
    def get_groups(self, theta):
        ...
        groups = VGroup(*[group.copy().rotate(angle, about_point=ORIGIN) for angle in angles])
        ...
        for n, angle, group, color in zip(it.count(1), angles, groups, colors):
            ...
            number.move_to(center_of_mass([wedge[0].get_end(), wedge[1].get_end()]))
            group.add(number)
    ...
    def get_semicircle(self):
        return Arc(start_angle=0, angle=PI, radius=self.wedge_radius / 2, color=YELLOW, stroke_width=4)
...
class AskAboutAddingThetaToItselfThetaPoint1(AskAboutAddingThetaToItself):
    CONFIG = {'theta': 0.1, 'wait_time': 0.1, 'theta_symbol_scale_val': 0.25, 'number_height': 0.15}

class AskAboutAddingThetaToItselfThetaPoint2(AskAboutAddingThetaToItself):
    CONFIG = {'theta': 0.2, 'wait_time': 0.1}

class FinalFormula(Scene):

    def construct(self):
        text = OldTexText('Final answer: ')
        t2c_map = {'\\theta': BLUE, 'm_1': GREEN, 'm_2': RED}
        formula = OldTex('\\left\\lfloor', '{\\pi', '\\over', '\\theta}', '\\right\\rfloor')
        ...
        self.wait()
        theta_eq = OldTex('\\theta', '=', '\\arctan', '\\left(', '\\sqrt', '{{m_2', '\\over', 'm_1}}', '\\right)')
        theta_eq.set_color_by_tex_to_color_map(t2c_map)
        theta_eq.scale(1.5)
        theta_eq.next_to(group, DOWN, MED_LARGE_BUFF)
        self.play(TransformFromCopy(formula.get_part_by_tex('\\theta'), theta_eq.get_part_by_tex('\\theta')))
...
class TwoSolutionsWrapper(WrapperScene):
    ...
    def construct(self):
        ...
        pis = VGroup(Randolph(color=BLUE_D), Randolph(color=BLUE_E), Randolph(color=BLUE_B), Mortimer().scale(1.2))
        pis.set_height(2)
        pis.arrange(RIGHT, buff=MED_LARGE_BUFF)
        pis.to_edge(DOWN, buff=SMALL_BUFF)
        self.add(big_rect, title, pis)
        self.play(LaggedStartMap(ShowCreation, screen_rects.copy().set_fill(opacity=0), lag_ratio=0.8), LaggedStartMap(FadeIn, screen_rects, lag_ratio=0.8), LaggedStartMap(ApplyMethod, pis, lambda pi: (pi.change, 'pondering', screen_rects[0])))
        self.play(Blink(random.choice(pis)))
        self.play(LaggedStartMap(ApplyMethod, pis, lambda pi: (pi.change, 'thinking', screen_rects[1])))
        self.play(Blink(random.choice(pis)))
        self.wait()
...
class ClacksSolution2Thumbnail(Scene):
    ...
    def construct(self):
        self.add_scene1()
        self.add_scene2()
        arrow = Arrow(self.block_word.get_top(), self.light_dot.get_bottom(), tip_length=0.75, rectangular_stem_width=0.2, color=RED, buff=0.5)
    ...
    def add_scene1(self):
        scene1 = Thumbnail(sliding_blocks_config={'block1_config': {'label_text': '$100^{d}$ kg', 'distance': 8}})
    ...
    def add_scene2(self):
        scene2 = ReflectWorldThroughMirrorNew(skip_animations=True, file_writer_config={'write_to_movie': False}, end_at_animation_number=18, center=ORIGIN)
        ...
        beams1, beams2 = [scene2.get_shooting_beam_anims(path, max_stroke_width=20, max_time_width=1, num_flashes=50) for path in [scene2.trajectory, scene2.ghost_trajectory]]
        beams = beams1 + beams2
        beams = beams2
        flashes = VGroup()
        for beam in beams:
        ...
        word.scale(3.5)
        word.set_color(YELLOW)
        word.add_to_back(word.copy().set_stroke(BLACK, 25))
        word.next_to(dot, UR)
