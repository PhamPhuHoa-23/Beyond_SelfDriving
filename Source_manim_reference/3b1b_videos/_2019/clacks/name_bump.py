...
class NameBump(BlocksAndWallExample):
    CONFIG = {'name': 'Grant Sanderson', 'sliding_blocks_config': {'block1_config': {'mass': 1000000.0, 'velocity': -0.5, 'distance': 7}, 'block2_config': {}}, 'wait_time': 25}
    ...
    def setup(self):
        ...
        if n == 1:
            ...
            names = [' '.join(names[:n // 2]), ' '.join(names[n // 2:])]
        ...
        name_mobs.scale(2)
        configs = [self.sliding_blocks_config['block1_config'], self.sliding_blocks_config['block2_config']]
