...
class HyperdartScene(MovingCameraScene):
    CONFIG = {'square_width': 6, 'square_style': {'stroke_width': 2, 'fill_color': BLUE, 'fill_opacity': 0.5}, 'circle_style': {'fill_color': RED_E, 'fill_opacity': 1, 'stroke_width': 0}, 'circle_center_dot_radius': 0.025, 'default_line_style': {'stroke_width': 2, 'stroke_color': WHITE}, 'default_dot_config': {'fill_color': WHITE, 'background_stroke_width': 1, 'background_stroke_color': BLACK, 'radius': 0.5 * DEFAULT_DOT_RADIUS}, 'dart_sound': 'dart_low', 'default_bullseye_shadow_opacity': 0.35}
    ...
    def get_circle_center_dot(self, circle=None):
        ...
        return Dot(circle.get_center(), radius=self.circle_center_dot_radius, fill_color=BLACK)

    def get_number_plane(self):
        square = self.square
        unit_size = square.get_width() / 2
        plane = NumberPlane(axis_config={'unit_size': unit_size})
    ...
    def get_radii_to_chord(self, chord, circle=None):
        circle = circle or self.circle
        center = circle.get_center()
        radii = VGroup(*[DashedLine(center, point) for point in chord.get_start_and_end()])
    ...
    def get_new_circle_from_point(self, point, circle=None):
        return self.get_new_circle(self.get_new_radius(point, circle), circle)

    def get_new_circle_from_chord(self, chord, circle=None):
        return self.get_new_circle(chord.get_length() / 2, circle)
    ...
    def add_dart_sound(self, time_offset=0, gain=-20, **kwargs):
        self.add_sound(self.dart_sound, time_offset=time_offset, gain=-20, **kwargs)
    ...
    def show_hits_with_darts(self, points, run_time=0.5, added_anims=None):
        ...
        n_points = len(points)
        self.play(ShowIncreasingSubsets(dots, rate_func=squish_rate_func(linear, 0.5, 1)), LaggedStart(*[Restore(dart, path_arc=-20 * DEGREES, rate_func=linear, run_time=run_time) for dart in darts], lag_ratio=1 / n_points), *added_anims, run_time=run_time)
    ...
    def show_hit(self, point, pace='slow', added_anims=None):
        ...
        if pace == 'slow':
            self.play(FadeInFromLarge(dot, rate_func=rush_into), *added_anims, run_time=0.5)
        elif pace == 'fast':
            self.add(dot)
        return dot
    ...
    def show_geometry(self, point, pace='slow'):
        ...
        if pace == 'slow':
            self.play(ShowCreation(h_line), GrowFromCenter(chord))
            self.play(ShowCreation(elbow))
        elif pace == 'fast':
            self.play(ShowCreation(h_line), GrowFromCenter(chord), ShowCreation(elbow), run_time=0.5)
        return lines
    ...
    def show_circle_shrink(self, chord, pace='slow', shadow_opacity=None):
        ...
        outline = VGroup(*[VMobject().pointwise_become_partial(new_circle, a, b) for a, b in [(0, 0.5), (0.5, 1)]])
        outline.rotate(chord.get_angle())
        outline.set_fill(opacity=0)
        outline.set_stroke(YELLOW, 2)
        assert pace in ['slow', 'fast']
        if pace == 'slow':
            self.play(chord_copy.move_to, circle.get_center(), circle.set_opacity, 0.5)
            self.play(Rotating(chord_copy, radians=PI), ShowCreation(outline, lag_ratio=0), run_time=1, rate_func=smooth)
            self.play(Transform(circle, new_circle), FadeOut(outline))
        elif pace == 'fast':
            ...
            self.play(chord_copy.move_to, circle, Transform(circle, new_circle))
        return to_fade
    ...
    def show_miss(self, point, with_dart=True):
        ...
        if with_dart:
            ...
            dot = self.show_hit(point)
            to_fade.add(dot)
        self.play(ApplyMethod(square.set_color, YELLOW, rate_func=lambda t: 1 - t), GrowFromCenter(miss), run_time=0.25)
        return to_fade
    ...
    def show_game_over(self):
        ...
        self.play(FadeIn(rect), FadeInFromLarge(game_over))
...
class TableOfContents(Scene):
    ...
    def construct(self):
        ...
        self.add(rect)
        parts = VGroup(OldTexText('The game'), OldTexText('The puzzle'), OldTexText('The micropuzzles'), OldTexText('The answer'))
        parts.scale(1.5)
        ...
        for part in parts:
            part.save_state()
            self.play(part.scale, 1.5, {'about_edge': LEFT}, part.set_opacity, 1, Restore(last_part))
            self.wait()
            last_part = part
...
class ShowGiantBullseye(HyperdartScene):
    ...
    def construct(self):
        ...
        self.play(FadeInFromDown(label[1]), ShowCreation(arrow))
        self.play(bullseye.match_width, board, ApplyMethod(arrow.scale, 0.4, {'about_point': arrow.get_start()}), run_time=2)
...
class ShowExampleHit(HyperdartScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ShowCreation(chord), ShowCreation(elbow), Write(chord_word, run_time=1))
        self.wait()
        chord_copy = chord.copy()
        ...
        outline = VGroup(Arc(start_angle=0, angle=PI), Arc(start_angle=PI, angle=PI))
        outline.set_stroke(YELLOW, 3)
        outline.set_fill(opacity=0)
        outline.replace(new_circle)
        self.play(circle.set_color, GREY_D, TransformFromCopy(chord, chord_copy), FadeIn(new_diam_word, UP))
        self.play(Rotate(chord_copy, PI), ShowCreation(outline, lag_ratio=0))
        self.play()
        self.remove(lines)
        point_tracker = VectorizedPoint(point)
        self.remove(lines, *lines)
        lines = always_redraw(lambda: self.get_all_hit_lines(point_tracker.get_location()))
        ...
        new_diam_word.add_updater(lambda m: m.set_width(min(ndw_width, chord_copy.get_width())).next_to(chord_copy, DOWN, SMALL_BUFF))
        ...
        self.wait()
        self.play(point_tracker.shift, 2.1 * UP, run_time=9, rate_func=there_and_back_with_pause)
...
class SimulateRealGame(HyperdartScene):
    ...
    def construct(self):
        ...
        points = [0.5 * UP, 2.0 * UP, 1.9 * LEFT + 0.4 * DOWN]
        for point in points:
...
class ContrastDistributions(HyperdartScene):
    ...
    def construct(self):
        ...
        right_points = np.random.multivariate_normal(mean=board_copy.get_center(), cov=0.6 * np.identity(3), size=n_points)
        left_dots, right_dots = [VGroup(*[Dot(p, radius=0.02) for p in points]) for points in [left_points, right_points]]
        ...
        self.wait()
        self.play(FadeOut(left_rect), FadeIn(right_rect))
...
class ChooseXThenYUniformly(Scene):
    ...
    def construct(self):
        unit_size = 3
        axes = Axes(x_min=-1.25, x_max=1.25, y_min=-1.25, y_max=1.25, axis_config={'tick_frequency': 0.25, 'unit_size': unit_size})
        numbers = [-1, -0.5, 0.5, 1]
        num_config = {'num_decimal_places': 1, 'background_stroke_width': 3}
        axes.x_axis.add_numbers(*numbers, **num_config)
        axes.y_axis.add_numbers(*numbers, **num_config, direction=LEFT)
        ...
        self.add(x_tip)
        self.add(x_eq)
        self.play(x_tracker.set_value, 1, run_time=2)
        self.play(x_tracker.set_value, np.random.random(), run_time=1)
        self.play(FadeIn(y_tip), FadeIn(y_eq))
        self.play(y_tracker.set_value, 1, run_time=2)
        self.play(y_tracker.set_value, np.random.random(), run_time=1)
        ...
        self.play(LaggedStartMap(FadeInFromLarge, dots), run_time=3)
        self.wait()

class ShowDistributionOfScores(Scene):
    CONFIG = {'axes_config': {'x_min': -1, 'x_max': 10, 'x_axis_config': {'unit_size': 1.2, 'tick_frequency': 1}, 'y_min': 0, 'y_max': 100, 'y_axis_config': {'unit_size': 0.065, 'tick_frequency': 10, 'include_tip': False}}, 'random_seed': 1}
    ...
    def construct(self):
        ...
        bars.add_updater(lambda b: self.set_histogram_bars(b, scores[:get_index()], axes))
        self.add(bars)
        ...
        score_label[1].add_updater(lambda m: m.set_fill(bars[scores[get_index() - 1]].get_fill_color()))
        n_trials_label = VGroup(OldTexText('\\# Games: '), Integer(0))
        ...
        score_label.next_to(n_trials_label, DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
        ...
        curr_score_arrow.add_updater(lambda m: m.next_to(bars[scores[get_index() - 1] - 1], UP, SMALL_BUFF))
        ...
        mean_label = VGroup(OldTexText('Mean = '), DecimalNumber(num_decimal_places=3))
        ...
        for value in [10, 100, 1000, 10000]:
            anims = [ApplyMethod(index_tracker.set_value, value, rate_func=linear, run_time=5)]
            if value == 10:
                anims.append(FadeIn(VGroup(mean_line, mean_label), rate_func=squish_rate_func(smooth, 0.5, 1), run_time=2))
            self.play(*anims)
        self.wait()
    ...
    def set_histogram_bars(self, bars, scores, axes):
        ...
        for bar in bars:
            ...
            bar.set_height(prop * axes.y_axis.unit_size * 100, stretch=True, about_edge=DOWN)
...
class ShowProbabilityForFirstShot(HyperdartScene):
    ...
    def construct(self):
        ...
        square_frac = VGroup(circle.copy().set_height(0.5), Line(LEFT, RIGHT).set_width(0.7), square.copy().set_height(0.5).set_stroke(width=0))
        ...
        self.play(LaggedStartMap(FadeInFromLarge, dots), run_time=5)
        self.play(ReplacementTransform(circle.copy().set_fill(opacity=0).set_stroke(WHITE, 1), square_frac[0]))
        self.play(ReplacementTransform(square.copy().set_fill(opacity=0), square_frac[2]))
        self.wait(2)
        x = np.random.random()
        y = np.sqrt(1 - x ** 2)
        ...
        dart, dot = self.show_hit_with_dart(point)
        self.remove(dot)
        self.play(frame.scale, 0.05, frame.move_to, point, run_time=5)

class SamplingFourRandomNumbers(Scene):
    CONFIG = {'n_terms': 4, 'title_tex': 'P\\left(x_0{}^2 + y_0{}^2 + x_1{}^2 + y_1{}^2 < 1\\right) = \\, ???', 'nl_to_nl_buff': 0.75, 'to_floor_buff': 0.5, 'tip_scale_factor': 0.75, 'include_half_labels': True, 'include_title': True}
    ...
    def construct(self):
        ...
        number_lines = VGroup(*[NumberLine(x_min=-1, x_max=1, tick_frequency=0.25, unit_size=3) for x in range(self.n_terms)])
        for line in number_lines:
            line.add_numbers(-1, 0, 1)
            if self.include_half_labels:
                line.add_numbers(-0.5, 0.5, num_decimal_places=1)
        ...
        tips = VGroup(*[ArrowTip(start_angle=-PI / 2, color=color).scale(self.tip_scale_factor) for color in colors])
        labels = VGroup(*[OldTex(tex) for tex in texs])
        for tip, tracker, line, label in zip(tips, trackers, number_lines, labels):
            tip.line = line
            tip.tracker = tracker
            tip.add_updater(lambda t: t.move_to(t.line.n2p(t.tracker.get_value()), DOWN))
            label.tip = tip
        ...
        summands = VGroup(*[OldTex('\\big(', '+0.00', '\\big)^2').set_color(color) for color in colors])
        ...
        total.add_updater(lambda d: d.set_value(np.sum([t.get_value() ** 2 for t in trackers])))
        VGroup(summands, h_line, total).shift_onto_screen()
        self.add(summands, h_line, total)
        lt, gt = signs = VGroup(OldTex('< 1 \\quad \\checkmark'), OldTex('\\ge 1 \\quad'))
        for sign in signs:
            sign.scale(1.5)
        ...
        self.wait()
        self.play(LaggedStart(*[ShowCreationThenFadeAround(summand) for summand in summands[1:]]))
...
class SamplingTwoRandomNumbers(SamplingFourRandomNumbers):
    CONFIG = {'n_terms': 2, 'title_tex': 'P\\left(x_0{}^2 + y_0{}^2 < 1\\right) = \\, ???', 'nl_to_nl_buff': 1, 'to_floor_buff': 2, 'random_seed': 1}

class SamplingSixRandomNumbers(SamplingFourRandomNumbers):
    CONFIG = {'n_terms': 6, 'nl_to_nl_buff': 0.5, 'include_half_labels': False, 'include_title': False, 'tip_scale_factor': 0.5}
...
class SamplePointIn3d(SpecialThreeDScene):
    ...
    def construct(self):
        ...
        dots = VGroup(*[Dot(point, radius=0.01, shade_in_3d=True) for point in points])
...
class OverlayToPointIn3d(Scene):
    ...
    def construct(self):
        t2c = {'{x}': GREEN, '{y}': RED, '{z}': BLUE}
        ineq = OldTex('{x}^2 + {y}^2 + {z}^2 < 1', tex_to_color_map=t2c)
        ineq.scale(1.5)
...
class TwoDPlusTwoDEqualsFourD(HyperdartScene):
    ...
    def construct(self):
        ...
        unit_size = 1.5
        axes = Axes(x_min=-1.25, x_max=1.25, y_min=-1.25, y_max=1.25, axis_config={'unit_size': unit_size, 'tick_frequency': 0.5, 'include_tip': False})
        ...
        board.to_edge(LEFT)
        self.add(board)
        kw = {'tex_to_color_map': {'x_0': WHITE, 'y_0': WHITE, 'x_1': WHITE, 'y_1': WHITE}}
        title1 = VGroup(OldTexText('First shot'), OldTex('(x_0, y_0)', **kw))
        title2 = VGroup(OldTexText('Second shot'), OldTex('(x_1, y_1)', **kw))
        title3 = VGroup(OldTexText('Point in 4d space'), OldTex('(x_0, y_0, x_1, y_1)', **kw))
...
class ExpectedValueComputation(Scene):
    ...
    def construct(self):
        t2c = {'0': MAROON_C, '1': BLUE, '2': GREEN, '3': YELLOW, '4': RED}
        line1 = OldTex('E[S]', '=', '1 \\cdot', 'P(S = 1)', '+', '2 \\cdot', 'P(S = 2)', '+', '3 \\cdot', 'P(S = 3)', '+', '\\cdots', tex_to_color_map=t2c)
        line2 = OldTex('=&\\phantom{-}', '1 \\cdot', '\\big(', 'P(S > 0)', '-', 'P(S > 1)', '\\big)', '\\\\&+', '2 \\cdot', '\\big(', 'P(S > 1)', '-', 'P(S > 2)', '\\big)', '\\\\&+', '3 \\cdot', '\\big(', 'P(S > 2)', '-', 'P(S > 3)', '\\big)', '\\\\&+', '\\cdots', tex_to_color_map=t2c)
        line2[1:12].align_to(line2[13], LEFT)
        line3 = OldTex('=', 'P(S > 0)', '+', 'P(S > 1)', '+', 'P(S > 2)', '+', 'P(S > 3)', '+', '\\cdots', tex_to_color_map=t2c)
        line1.to_corner(UL)
        ...
        self.play(LaggedStart(TransformFromCopy(line1.get_part_by_tex('='), line2.get_part_by_tex('='), **kw), TransformFromCopy(line1.get_parts_by_tex('\\cdot'), line2.get_parts_by_tex('\\cdot'), **kw), TransformFromCopy(line1.get_parts_by_tex('+'), line2.get_parts_by_tex('+'), **kw), TransformFromCopy(line1.get_part_by_tex('1'), line2.get_part_by_tex('1'), **kw), TransformFromCopy(line1.get_part_by_tex('2'), line2.get_part_by_tex('2'), **kw), TransformFromCopy(line1.get_part_by_tex('3'), line2.get_part_by_tex('3'), **kw), run_time=3, lag_ratio=0), LaggedStart(*[GrowFromCenter(bigs[i:i + 2]) for i in range(0, len(bigs), 2)]))
        self.wait()
        for n in range(3):
            i = 6 * n
            j = 12 * n
            ...
            self.play(TransformFromCopy(line1[i + 4:i + 7], line2[j + 4:j + 7]), TransformFromCopy(line1[i + 4:i + 7], line2[j + 8:j + 11]), FadeIn(line2[j + 7]), ReplacementTransform(rect1, rect2))
        ...
        self.wait()
        self.play(TransformFromCopy(line2[0], line3[0]), TransformFromCopy(line2[4:7], line3[1:4]))
        self.wait()
        line2.target.set_opacity(0.2)
        VGroup(line2.target[1:4], line2.target[7:12], line2.target[12:19], line2.target[23]).set_opacity(1)
        self.play(MoveToTarget(line2))
        self.wait()
        self.play(TransformFromCopy(line2[12], line3[4]), TransformFromCopy(line2[16:19], line3[5:8]))
        self.wait()
        n = 12
        line2.target.set_opacity(0.2)
        VGroup(line2.target[n + 1:n + 4], line2.target[n + 7:n + 12], line2.target[n + 12:n + 19], line2.target[n + 23]).set_opacity(1)
        self.play(MoveToTarget(line2))
        self.wait()
        self.play(TransformFromCopy(line2[n + 12], line3[8]), TransformFromCopy(line2[n + 16:n + 19], line3[9:12]))
        self.wait()
...
class SubtractHistogramParts(ShowDistributionOfScores):
    ...
    def construct(self):
        ...
        self.add(axes)
        self.add(bars)
        p2_arrow = Vector(0.75 * DOWN, max_stroke_width_to_length_ratio=10, max_tip_length_to_length_ratio=0.35)
        p2_arrow.next_to(bars[1], UP, SMALL_BUFF)
        p2_arrow = VGroup(p2_arrow.copy().set_stroke(BLACK, 9), p2_arrow)
        p2_label = OldTex('P(S = 2)')
        p2_label.next_to(p2_arrow, UP, SMALL_BUFF)
        p2_label.set_color(bars[1].get_fill_color())
        self.play(GrowFromPoint(p2_arrow, p2_arrow.get_top()), FadeInFromDown(p2_label), bars[0].set_opacity, 0.1, bars[2:].set_opacity, 0.1)
        self.wait()
        ...
        self.play(FadeIn(rhs), bars[2:].set_opacity, 1, GrowFromPoint(brace1, rhs[1].get_bottom()), p2_arrow.set_opacity, 0.5)
        self.wait()
        self.play(rhs[:2].set_opacity, 0.2, brace1.set_opacity, 0.2, rhs[2:].set_opacity, 1, bars[1].set_opacity, 0.1, GrowFromCenter(brace2))
        self.wait()
        self.play(bars[2:].set_opacity, 0.1, bars[1].set_opacity, 1, rhs.set_opacity, 1, brace1.set_opacity, 1, p2_arrow.set_opacity, 1)
        self.wait()

class GameWithSpecifiedScore(HyperdartScene):
    CONFIG = {'score': 1, 'random_seed': 1}
    ...
    def construct(self):
        ...
        while True:
            ...
            if want_to_continue:
                ...
                lines = self.show_geometry(point, pace)
                chord_and_shadow = self.show_circle_shrink(lines[1], pace=pace)
                self.play(FadeOut(VGroup(dart, dot, lines, chord_and_shadow)), run_time=0.5)
                score += 1
            else:
...
class HistogramScene(ShowDistributionOfScores):
    CONFIG = {'n_scores': 10000, 'mean_line_height': 4}
    ...
    def get_mean_label(self):
        ...
        mean_label = VGroup(*TexText('E[S]', '='), DecimalNumber(mean, num_decimal_places=3))
        mean_label.arrange(RIGHT)
        mean_label.match_color(mean_line)
        mean_label.next_to(mean_line.get_end(), UP, SMALL_BUFF, index_of_submobject_to_align=0)
...
class ExpectedValueFromBars(HistogramScene):
    ...
    def construct(self):
        ...
        equation = OldTex('P(S = 1)', '\\cdot', '1', '+', 'P(S = 2)', '\\cdot', '2', '+', 'P(S = 3)', '\\cdot', '3', '+', '\\cdots')
        equation.scale(0.9)
        ...
        for i in range(3):
            ...
            self.play(bars[:i + 1].set_opacity, 1, bars[i + 1:].set_opacity, 0.2, equation[:4 * (i + 1)].set_opacity, 1, FadeInFromDown(p_part_copy), Animation(mean_label[0]))
            kw = {'surrounding_rectangle_config': {'color': bar.get_fill_color(), 'buff': 0.5 * SMALL_BUFF}}
            self.play(LaggedStart(AnimationGroup(ShowCreationThenFadeAround(p_part, **kw), ShowCreationThenFadeAround(p_part_copy, **kw)), AnimationGroup(ShowCreationThenFadeAround(s_part, **kw), ShowCreationThenFadeAround(num, **kw)), lag_ratio=0.5))
            self.wait()
            p_parts.add(p_part)
...
class ProbabilitySGtOne(HistogramScene):
    ...
    def construct(self):
        ...
        self.play(GrowFromEdge(brace, LEFT), bars[0].set_opacity, 0.2, bars[1:].set_opacity, 0.8, ShowCreationThenFadeOut(outlines), FadeIn(label, LEFT))
        self.wait()
        square = Square()
        ...
        new_brace.next_to(bars[2], UP, buff=SMALL_BUFF, aligned_edge=LEFT)
        self.add(new_brace)
        new_label = OldTex('P(S > 2)', '=', '\\,???')
        new_label.next_to(new_brace[0][2], UP)
        self.play(bars[1].set_opacity, 0.2, label.set_opacity, 0.5, rhs.set_opacity, 0.5, brace.set_opacity, 0.5, GrowFromEdge(new_brace, LEFT), ReplacementTransform(new_label.copy().fade(1).move_to(label, LEFT), new_label))
        self.wait()
        new_rhs = OldTex('{\\text{4d ball}', ' \\over', ' \\text{4d cube}}')
        ...
        final_brace.next_to(bars[3], UP, buff=SMALL_BUFF, aligned_edge=LEFT)
        self.add(final_brace)
        final_label = OldTex('P(S > 3)')
        final_label.next_to(final_brace[0][2], UP, SMALL_BUFF)
        self.play(bars[2].set_opacity, 0.2, new_label[:-1].set_opacity, 0.5, new_rhs.set_opacity, 0.5, new_brace.set_opacity, 0.5, GrowFromEdge(final_brace, LEFT), ReplacementTransform(final_label.copy().fade(1).move_to(new_label, LEFT), final_label), axes.x_axis[-1].set_opacity, 0)
        self.wait()

class VolumsOfNBalls(Scene):

    def construct(self):
        title, alt_title = [OldTexText('Volumes of ' + tex + '-dimensional balls', tex_to_color_map={tex: YELLOW}) for tex in ['$N$', '$2n$']]
        for mob in [title, alt_title]:
            mob.scale(1.5)
            mob.to_edge(UP)
        formulas = VGroup(*[OldTex(tex, tex_to_color_map={'R': WHITE}) for tex in ['2R', '\\pi R^2', '\\frac{4}{3} \\pi R^3', '\\frac{1}{2} \\pi^2 R^4', '\\frac{8}{15} \\pi^2 R^5', '\\frac{1}{6} \\pi^3 R^6', '\\frac{16}{105} \\pi^3 R^7', '\\frac{1}{24} \\pi^4 R^8', '\\frac{32}{945} \\pi^4 R^9', '\\frac{1}{120} \\pi^5 R^{10}', '\\frac{64}{10{,}395} \\pi^5 R^{11}', '\\frac{1}{720} \\pi^6 R^{12}']])
        ...
        arrows = VGroup(*[Arrow(formulas[i].get_bottom(), formulas[i + 1].get_bottom(), path_arc=150 * DEGREES) for i in (1, 2)])
        for arrow in arrows:
            self.play(ShowCreation(arrow))
        self.wait()
        self.play(FadeOut(arrows), FadeIn(formulas[4:]))
        ...
        self.play(formulas[::2].set_opacity, 0.25, ReplacementTransform(title, alt_title))
        for brace in braces[:3]:
            self.play(GrowFromCenter(brace))
        self.wait()
        self.play(FadeOut(braces[:3]), FadeIn(gen_form, UP))
        self.wait()
...
class IntroduceGame(HyperdartScene):
    CONFIG = {'random_seed': 0, 'square_width': 5, 'num_darts_in_initial_flurry': 5}
    ...
    def show_flurry_of_points(self):
        ...
        self.add(board)
        darts, dots = self.show_hits_with_darts(points, added_anims=[FadeInFromDown(title)])
        self.wait()
        ...
        def func(p):
            ...
            p *= 1 / np.cos(theta)
            return p
        self.play(*[ApplyPointwiseFunction(func, pieces, run_time=1) for pieces in [*board[:3], *dots]], *[MaintainPositionRelativeTo(dart, dot) for dart, dot in zip(darts, dots)])
    ...
    def show_board_dimensions(self):
        square = self.square
        labels = VGroup(*[OldTexText('2 ft').next_to(square.get_edge_center(vect), vect) for vect in [DOWN, RIGHT]])
        labels.set_color(YELLOW)
        h_line, v_line = lines = VGroup(*[DashedLine(square.get_edge_center(v1), square.get_edge_center(-v1)).next_to(label, v2) for label, v1, v2 in zip(labels, [LEFT, UP], [UP, LEFT])])
        lines.match_color(labels)
        self.play(LaggedStartMap(ShowCreation, lines), LaggedStartMap(FadeInFromDown, labels), lag_ratio=0.5)
        self.wait()
    ...
    def introduce_bullseye(self):
        ...
        radius = DashedLine(square.get_center(), square.get_left(), stroke_width=2)
        radius_label = OldTexText('1 ft')
        radius_label.next_to(radius, DOWN, SMALL_BUFF)
        self.add(circle, self.square_dimensions)
        self.play(FadeInFromLarge(circle), FadeInFromDown(label), ShowCreation(arrow), LaggedStartMap(FadeOut, self.flurry_dots, run_time=1), LaggedStartMap(FadeOut, self.darts, run_time=1))
        self.wait()
        self.add(square, board, arrow, circle)
        self.play(Restore(circle), ApplyMethod(arrow.scale, 0.4, {'about_point': arrow.get_start()}))
        self.add(radius, self.circle_center_dot)
        self.play(ShowCreation(radius), FadeIn(radius_label, RIGHT), FadeIn(self.circle_center_dot))
        self.play(FadeOut(label), Uncreate(arrow), FadeOut(board))
        self.wait()
        s_lines, s_labels = self.square_dimensions
        self.play(FadeOut(s_lines), FadeOut(radius), FadeOut(radius_label), FadeOut(self.title))
        self.circle_dimensions = VGroup(radius, radius_label)
    ...
    def show_shrink_rule(self):
        ...
        self.play(FadeInFromDown(new_label), FadeOut(label, UP))
        self.show_full_hit_process(point)
        self.wait()
...
class ShowScoring(HyperdartScene):
    ...
    def comment_on_score(self):
        ...
        comment.set_x(midpoint(self.square.get_left(), LEFT_SIDE)[0])
        arrow = Arrow(comment.get_top(), score_label[1].get_bottom(), buff=0.2)
        arrow.match_color(comment)
        self.play(FadeInFromDown(comment), GrowArrow(arrow))
    ...
    def increment_score(self):
        ...
        self.play(FadeOut(score, UP), FadeIn(new_score, DOWN), run_time=1)
...
class ShowSeveralRounds(ShowScoring):
    ...
    def show_single_round(self, pace='fast'):
        while True:
            ...
            if self.is_inside(point):
                ...
                self.wait(0.5)
                self.play(ShowCreationThenFadeAround(self.score_label), FadeOut(to_fade))
                return
    ...
    def reset_board(self):
        ...
        self.play(self.circle.match_width, self.square, FadeOut(score, UP), FadeIn(new_score, DOWN))
        score.set_value(0)
        self.add(score)
        self.remove(new_score)
...
class ShowUniformDistribution(HyperdartScene):
    CONFIG = {'dart_sound': 'dart_high', 'n_points': 1000}
    ...
    def show_random_points(self):
        ...
        run_time = 5
        self.play(LaggedStartMap(FadeInFromLarge, dots, run_time=run_time))
        for x in range(1000):
            self.add_dart_sound(time_offset=-run_time * np.random.random(), gain=-10, gain_to_background=-5)
        self.wait()

    def exchange_titles(self):
        self.play(FadeInFromDown(self.new_title), FadeOut(self.title, UP))

class ExpectedScoreEqualsQMark(Scene):

    def construct(self):
        equation = OldTexText('\\textbf{E}[Score] = ???', tex_to_color_map={'???': YELLOW})
