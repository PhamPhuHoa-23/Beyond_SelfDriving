...
Lg_formula_config = {'tex_to_color_map': {'\\theta_0': WHITE, '{L}': BLUE, '{g}': YELLOW}}

class You(PiCreature):
    CONFIG = {'color': BLUE_C}

def get_ode():
    tex_config = {'tex_to_color_map': {'{\\theta}': BLUE, '{\\dot\\theta}': RED, '{\\ddot\\theta}': YELLOW, '{t}': WHITE, '{\\mu}': WHITE}}
    ode = OldTex('{\\ddot\\theta}({t})', '=', '-{\\mu} {\\dot\\theta}({t})', '-{g \\over L} \\sin\\big({\\theta}({t})\\big)', **tex_config)
    return ode

def get_period_formula():
    return OldTex('2\\pi', '\\sqrt{\\,', 'L', '/', 'g', '}', tex_to_color_map={'L': BLUE, 'g': YELLOW})
...
def get_vector_symbol(*texs, **kwargs):
    config = {'include_background_rectangle': True, 'bracket_h_buff': SMALL_BUFF, 'bracket_v_buff': SMALL_BUFF, 'element_alignment_corner': ORIGIN}
...
def get_love_equation1():
    equation = VGroup(get_heart_var_deriv(1), OldTex('=').scale(2), OldTex('a').scale(2), get_heart_var(2))
    equation.arrange(RIGHT)
    equation[-1].shift(SMALL_BUFF * DL)
    return equation

def get_love_equation2():
    equation = VGroup(get_heart_var_deriv(2), OldTex('=').scale(2), OldTex('-b').scale(2), get_heart_var(1))
    equation.arrange(RIGHT)
    equation[-1].shift(SMALL_BUFF * DL)
    return equation