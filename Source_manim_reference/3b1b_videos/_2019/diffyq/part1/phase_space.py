...
class VisualizeStates(Scene):
    CONFIG = {'coordinate_plane_config': {'y_line_frequency': PI / 2, 'x_line_frequency': 1, 'y_axis_config': {'unit_size': 1}, 'y_max': 4, 'faded_line_ratio': 4, 'background_line_style': {'stroke_width': 1}}, 'little_pendulum_config': {'length': 1, 'gravity': 4.9, 'weight_diameter': 0.3, 'include_theta_label': False, 'include_velocity_vector': True, 'angle_arc_config': {'radius': 0.2}, 'velocity_vector_config': {'max_tip_length_to_length_ratio': 0.35, 'max_stroke_width_to_length_ratio': 6}, 'velocity_vector_multiple': 0.25, 'max_velocity_vector_length_to_length_ratio': 0.8}, 'big_pendulum_config': {'length': 1.6, 'gravity': 4.9, 'damping': 0.2, 'weight_diameter': 0.3, 'include_velocity_vector': True, 'angle_arc_config': {'radius': 0.5}, 'initial_theta': 80 * DEGREES, 'omega': -1, 'set_theta_label_height_cap': True}, 'n_thetas': 11, 'n_omegas': 7, 'initial_grid_wait_time': 15}
    ...
    def initialize_grid_of_states(self):
        ...
        for omega in omegas:
            row = VGroup()
            for theta in thetas:
                rect = Rectangle(height=3, width=3, stroke_color=WHITE, stroke_width=2, fill_color=GREY_E, fill_opacity=1)
                pendulum = Pendulum(initial_theta=theta, omega=omega, top_point=rect.get_center(), **self.little_pendulum_config)
    ...
    def initialize_plane(self):
        ...
        plane.axis_labels = VGroup(plane.get_x_axis_label('\\theta', RIGHT, UL, buff=SMALL_BUFF), plane.get_y_axis_label('\\dot \\theta', UP, DR, buff=SMALL_BUFF).set_color(YELLOW))
    ...
    def show_grid_of_states_creation(self):
        ...
        for state in it.chain(middle_row_copy, right_column_copy):
            place_at_top(state)
        self.add(title)
        self.play(ShowIncreasingSubsets(middle_row), ShowIncreasingSubsets(middle_row_copy), run_time=2, rate_func=linear)
        self.wait()
        self.play(ShowIncreasingSubsets(state_grid), ShowIncreasingSubsets(right_column_copy), run_time=2, rate_func=linear)
    ...
    def collapse_grid_into_points(self):
        ...
        for row in state_grid:
            for state in row:
                dot = Dot(self.get_state_point(state.pendulum), radius=0.05, color=YELLOW, background_stroke_width=3, background_stroke_color=BLACK)
                dot.state = state
                dots.add(dot)
        self.add(plane)
        ...
        self.clear()
        self.play(ShowCreation(plane), FadeOut(self.all_states_title), LaggedStart(*[TransformFromCopy(m1, m2) for m1, m2 in zip(flat_state_group, flat_dot_group)], lag_ratio=0.1, run_time=4))
        self.clear()
        self.add(plane, dots)
        self.wait()
    ...
    def show_state_with_pair_of_numbers(self):
        ...
        for vect in (LEFT, 3 * UP):
            self.play(ApplyMethod(dot.shift, vect, rate_func=there_and_back, run_time=2))
        self.wait()
    ...
    def show_evolution_from_a_start_state(self):
        state = self.state
        dot = self.state_dot
        self.play(Rotating(dot, about_point=dot.get_center() + UR, rate_func=smooth))
        self.wait()
        ...
        self.wait()
        self.play(ShowCreation(sub_traj), UpdateFromFunc(dot, lambda d: d.move_to(sub_traj.get_end())), run_time=6)
        self.wait()
        ...
        self.play(ApplyMethod(self.plane.set_stroke, YELLOW, 0.5, rate_func=there_and_back, lag_ratio=0.2), FadeInFromDown(abstract), Animation(state))
        self.wait()
    ...
    def get_flexible_state_picture(self):
        ...
        rect = Square(side_length=height, stroke_color=WHITE, stroke_width=2, fill_color='#111111', fill_opacity=1)
    ...
    def get_tracking_line(self, point, axis, color=WHITE):
        ...
        return DashedLine(axis_point, point, dash_length=0.025, color=color)

    def get_tracking_h_line(self, point):
        return self.get_tracking_line(point, self.plane.y_axis, WHITE)

    def get_tracking_v_line(self, point):
        return self.get_tracking_line(point, self.plane.x_axis, YELLOW)
...
class IntroduceVectorField(VisualizeStates):
    CONFIG = {'vector_field_config': {'max_magnitude': 3}, 'big_pendulum_config': {'initial_theta': -80 * DEGREES, 'omega': 1}}
    ...
    def initialize_vector_field(self):
        self.vector_field = VectorField(self.vector_field_func, **self.vector_field_config)
    ...
    def add_equation(self):
        ...
        self.play(FadeIn(ode, 0.5 * DOWN), FadeIn(ode_word, 0.5 * UP))
        self.ode = ode
        self.ode_word = ode_word

    def preview_vector_field(self):
        vector_field = self.vector_field
        growth = LaggedStartMap(GrowArrow, vector_field, run_time=3, lag_ratio=0.01)
    ...
    def write_vector_derivative(self):
        ...
        vect_sym, d_vect_sym = [self.get_vector_symbol('{' + a + '\\theta}(t)', '{' + b + '\\theta}(t)') for a, b in [('', '\\dot'), ('\\dot', '\\ddot')]]
        ...
        d_vect = always_redraw(lambda: Vector(rotate_vector(mag_tracker.get_value() * RIGHT, angle_tracker.get_value()), color=WHITE).shift(dot.get_center()))
        d_vect_magnitude_factor_tracker = ValueTracker(2)
        real_d_vect = always_redraw(lambda: self.vector_field.get_vector(dot.get_center()).scale(d_vect_magnitude_factor_tracker.get_value(), about_point=dot.get_center()))
        self.play(TransformFromCopy(state[1], vect))
        self.play(FadeInFromDown(vect_sym))
        self.wait()
        self.play(ReplacementTransform(vect, dot))
        self.wait()
        self.play(LaggedStartMap(ShowCreationThenFadeAround, time_inputs, lag_ratio=0.1))
        self.wait()
        self.play(Write(ddt))
        self.play(plane.y_axis.numbers.fade, 1, FadeIn(equals, LEFT), TransformFromCopy(vect_sym, d_vect_sym))
        self.wait()
        equation_group = VGroup(ddt, vect_sym, equals, d_vect_sym)
        self.play(equation_group.to_edge, RIGHT, LARGE_BUFF, GrowArrow(d_vect))
        self.wait()
        ...
        self.wait()
        self.play(TransformFromCopy(d_vect_sym, real_d_vect), dot.set_color, WHITE)
        self.wait()
        trajectory = VMobject()
        ...
        self.tie_state_to_dot_position(state, dot)
        self.play(MoveAlongPath(dot, trajectory), run_time=5, rate_func=bezier([0, 0, 1, 1]))
        self.state_dot = dot
    ...
    def interpret_first_coordinate(self):
        ...
        dot = self.state_dot
        first_components_copy = VGroup(vect_sym.get_entries()[0], d_vect_sym.get_entries()[0]).copy()
        rect = SurroundingRectangle(first_components_copy)
        rect.set_stroke(YELLOW, 2)
        equation.save_state()
        self.play(ShowCreation(rect), equation.fade, 0.5, Animation(first_components_copy))
        self.wait()
        dot.save_state()
        ...
        self.wait()
        self.play(equation.restore, FadeOut(rect))
        self.remove(first_components_copy)

    def interpret_second_coordinate(self):
        equation = self.equation_group
        ddt, vect_sym, equals, d_vect_sym = equation
        second_components = VGroup(vect_sym.get_entries()[1], d_vect_sym.get_entries()[1])
        rect = SurroundingRectangle(second_components)
        rect.set_stroke(YELLOW, 2)
        expanded_derivative = self.get_vector_symbol('{\\dot\\theta}(t)', '-\\mu {\\dot\\theta}(t)' + '-(g / L) \\sin\\big({\\theta}(t)\\big)')
        ...
        self.wait()
        self.play(FadeIn(expanded_derivative, LEFT), FadeIn(equals2), equation.next_to, equals2, LEFT, SMALL_BUFF, MaintainPositionRelativeTo(rect, equation), VFadeOut(rect))
        self.wait()
        self.full_equation = VGroup(*equation, equals2, expanded_derivative)
    ...
    def show_full_vector_field(self):
        ...
        growth = LaggedStartMap(GrowArrow, vector_field, run_time=3, lag_ratio=0.01)
        self.add(growth.mobject, state, ode, ode_word, equation_rect, equation, dot, d_vect)
        self.play(growth, FadeIn(equation_rect), MoveToTarget(equation), self.d_vect_magnitude_factor_tracker.set_value, 1)
    ...
    def get_vector_symbol(self, tex1, tex2):
        t2c = {'{\\theta}': BLUE, '{\\dot\\theta}': YELLOW, '{\\omega}': YELLOW, '{\\ddot\\theta}': RED}
        return get_vector_symbol(tex1, tex2, element_to_mobject_config={'tex_to_color_map': t2c}).scale(0.9)
    ...
    def ask_about_change(self):
        ...
        q_mark = always_redraw(lambda: OldTex('?').move_to(d_vect.get_end() + 0.4 * rotate_vector(d_vect.get_vector(), 90 * DEGREES)))
        self.play(TransformFromCopy(state[1], dot))
        self.tie_state_to_dot_position(state, dot)
        self.play(GrowArrow(d_vect), FadeInFromDown(q_mark))
        for x in range(4):
            angle = 90 * DEGREES
            self.play(Rotate(d_vect, angle, about_point=d_vect.get_start()))
            self.play(dot.shift, 0.3 * d_vect.get_vector(), rate_func=there_and_back)

class ShowPendulumPhaseFlow(IntroduceVectorField):
    CONFIG = {'coordinate_plane_config': {'x_axis_config': {'unit_size': 0.8}, 'x_max': 9, 'x_min': -9}, 'flow_time': 20}
    ...
    def construct(self):
        ...
        stream_lines = StreamLines(field.func, delta_x=0.3, delta_y=0.3)
...
class ShowHighVelocityCase(ShowPendulumPhaseFlow, MovingCameraScene):
    CONFIG = {'coordinate_plane_config': {'x_max': 15, 'x_min': -15}, 'vector_field_config': {'x_max': 15}, 'big_pendulum_config': {'max_velocity_vector_length_to_length_ratio': 1}, 'run_time': 25, 'initial_theta': 0, 'initial_theta_dot': 4, 'frame_shift_vect': TAU * RIGHT}
    ...
    def add_flexible_state(self):
        ...
        start_point = plane.coords_to_point(self.initial_theta, self.initial_theta_dot)
        dot = self.get_state_controlling_dot(state)
    ...
    def show_high_vector(self):
        ...
        self.play(ShowCreationThenFadeOut(top_vectors, run_time=2, lag_ratio=0.01))
    ...
    def show_trajectory(self):
        ...
        self.add(traj, dot)
        anims = [ShowCreation(traj, rate_func=linear), UpdateFromFunc(dot, lambda d: d.move_to(traj.get_points()[-1]))]
        if get_norm(self.frame_shift_vect) > 0:
            anims += [ApplyMethod(frame.shift, self.frame_shift_vect, rate_func=squish_rate_func(smooth, 0, 0.3)), MaintainPositionRelativeTo(state.rect, frame)]
...
class TweakMuInFormula(Scene):
    ...
    def construct(self):
        ...
        arrow = Arrow(lil_rect.get_top(), big_rect_seed.point_from_proportion(0.65), buff=SMALL_BUFF)
        arrow.match_color(lil_rect)
        mu_tracker = ValueTracker(0.1)
        get_mu = mu_tracker.get_value
        triangle = Triangle(start_angle=-90 * DEGREES, stroke_width=0, fill_opacity=1, fill_color=WHITE)
        triangle.set_height(0.2)
        triangle.add_updater(lambda t: t.next_to(interval.number_to_point(get_mu()), UP, buff=0))
        equation = VGroup(OldTex('\\mu = '), DecimalNumber())
        equation.add_updater(lambda e: e.arrange(RIGHT).next_to(triangle, UP, SMALL_BUFF).shift(0.4 * RIGHT))
        equation[-1].add_updater(lambda d: d.set_value(get_mu()).shift(0.05 * UL))
        self.add(ode)
        self.play(ShowCreation(lil_rect))
        self.play(GrowFromPoint(interval, mu.get_center()), GrowFromPoint(triangle, mu.get_center()), GrowFromPoint(equation, mu.get_center()), TransformFromCopy(lil_rect, big_rect), ShowCreation(arrow))
        self.wait()
...
class TweakMuInVectorField(ShowPendulumPhaseFlow):
    ...
    def construct(self):
        ...
        def get_vector_field():
            return VectorField(vector_field_func, **self.vector_field_config)
...
class HighAmplitudePendulum(ShowHighVelocityCase):
    CONFIG = {'big_pendulum_config': {'damping': 0.02}, 'initial_theta': 175 * DEGREES, 'initial_theta_dot': 0, 'frame_shift_vect': 0 * RIGHT}
...
class SpectrumOfStartingStates(ShowHighVelocityCase):
    ...
    def show_many_trajectories(self):
        ...
        delta_y = 0.5
        n = 20
        start_points = [plane.coords_to_point(x, y) for x in np.linspace(PI - delta_x, PI + delta_x, n) for y in np.linspace(-delta_y, delta_y, n)]
        ...
        self.play(LaggedStartMap(FadeInFromLarge, dots, lambda m: (m, 10), run_time=2), FadeInFromDown(words))
        self.wait()
        trajs = VGroup()
        for sp in start_points:
            trajs.add(self.get_trajectory(sp, time, added_steps=10))
...
class AskAboutStability(ShowHighVelocityCase):
    CONFIG = {'initial_theta': 60 * DEGREES, 'initial_theta_dot': 1}
    ...
    def show_fixed_points(self):
        ...
        self.play(dot1.move_to, fp1, run_time=3)
        self.wait()
        self.play(FadeIn(state2))
        self.play(dot2.move_to, fp2, path_arc=-30 * DEGREES, run_time=2)
        self.wait()
        self.state1 = state1
        self.state2 = state2
        self.dot1 = dot1
    ...
    def label_fixed_points(self):
        ...
        arrows = VGroup(*[Arrow(label.get_bottom(), dot.get_center(), color=dot.get_color()) for dot in dots])
    ...
    def show_nudges(self):
        dots = VGroup(self.dot1, self.dot2)
        time = 20
        self.play(*[ApplyMethod(dot.shift, 0.1 * UL, rate_func=rush_from) for dot in dots])
        trajs = VGroup()
        for dot in dots:
            traj = self.get_trajectory(dot.get_center(), time)
...
class LovePhaseSpace(ShowHighVelocityCase):
    CONFIG = {'vector_field_config': {'max_magnitude': 4}, 'a': 0.5, 'b': 0.3, 'mu': 0.2}
    ...
    def add_equations(self):
        equations = VGroup(get_love_equation1(), get_love_equation2())
        equations.scale(0.5)
    ...
    def show_vector_field(self):
        field = VectorField(lambda p: np.array([self.a * p[1], -self.b * p[0], 0]), **self.vector_field_config)
        field.sort(get_norm)
        ...
        self.play(axis_arrows.restore, ShowCreation(field), run_time=3)
    ...
    def show_example_trajectories(self):
        n_points = 20
        total_time = 30
        start_points = self.start_points = [2.5 * np.random.random() * rotate_vector(RIGHT, TAU * np.random.random()) for x in range(n_points)]
        ...
        self.play(LaggedStartMap(FadeInFromLarge, dots, lambda m: (m, 5)), FadeInFromDown(words))
        trajs = VGroup(*[self.get_trajectory(sp, total_time, added_steps=10) for sp in start_points])
        ...
        dots.add_updater(update_dots)
        self.add(trajs, dots)
        self.play(ShowCreation(trajs, lag_ratio=0, run_time=10, rate_func=linear))
        self.trajs = trajs
        self.dots = dots

    def add_resistance_term(self):
        added_term = VGroup(OldTex('-\\mu'), get_heart_var(2).scale(0.5))
        ...
        VGroup(neg_b, h1).next_to(added_term, RIGHT, SMALL_BUFF, aligned_edge=DOWN)
        ...
        self.play(MoveToTarget(equation2), FadeInFromDown(added_term), GrowFromCenter(brace), Write(words))
    ...
    def show_new_trajectories(self):
        ...
        trajs = self.trajs
        field = self.field
        new_field = VectorField(lambda p: np.array([self.a * p[1], -self.mu * p[1] - self.b * p[0], 0]), **self.vector_field_config)
        ...
        total_time = 30
        new_trajs = VGroup(*[self.get_trajectory(sp, total_time, added_steps=10) for sp in self.start_points])
        new_trajs.set_color_by_gradient(BLUE, WHITE)
        dots.trajs = new_trajs
        self.add(new_trajs, dots)
        self.play(ShowCreation(new_trajs, lag_ratio=0, run_time=10, rate_func=linear))
        self.wait()

class TakeManyTinySteps(IntroduceVectorField):
    CONFIG = {'initial_theta': 60 * DEGREES, 'initial_theta_dot': 0, 'initial_theta_tex': '\\pi / 3', 'initial_theta_dot_tex': '0'}
    ...
    def take_many_time_steps(self):
        ...
        traj = always_redraw(lambda: self.get_time_step_trajectory(get_delta_t(), get_t(), self.initial_theta, self.initial_theta_dot))
        vectors = always_redraw(lambda: self.get_path_vectors(get_delta_t(), get_t(), self.initial_theta, self.initial_theta_dot))
        ...
        theta_t_label.add_updater(lambda m: m.next_to(vectors[-1].get_end(), vectors[-1].get_vector(), SMALL_BUFF))
        self.add(traj, vectors, init_labels, labels)
        time_tracker.set_value(0)
        target_time = 10
        self.play(VFadeIn(theta_t_label), ApplyMethod(time_tracker.set_value, target_time, run_time=5, rate_func=linear))
        self.wait()
        t_label[-1].clear_updaters()
        self.remove(theta_t_label)
        target_delta_t = 0.01
        self.play(delta_t_tracker.set_value, target_delta_t, run_time=7)
        self.wait()
        traj.clear_updaters()
        ...
        def update_scaled_vectors(group):
            group.set_opacity(0)
            group[min(int(count.get_value()), len(group) - 1)].set_opacity(1)
        scaled_vectors.add_updater(update_scaled_vectors)
        self.add(count, count_label, scaled_vectors)
        self.play(ApplyMethod(count_tracker.set_value, int(target_time / target_delta_t), rate_func=linear), run_time=5)
    ...
    def get_labels(self, get_t, get_delta_t):
        t_label, dt_label = labels = VGroup(*[VGroup(OldTex('{} = '.format(s)), DecimalNumber(0)).arrange(RIGHT, aligned_edge=DOWN) for s in ('t', '{\\Delta t}')])
        ...
        init_labels = VGroup(OldTex('\\theta_0', '=', self.initial_theta_tex, tex_to_color_map={'\\theta': BLUE}), OldTex('{\\dot\\theta}_0 =', self.initial_theta_dot_tex, tex_to_color_map={'{\\dot\\theta}': YELLOW}))
    ...
    def get_time_step_points(self, delta_t, total_time, theta_0, theta_dot_0):
        plane = self.plane
        field = self.vector_field
        curr_point = plane.coords_to_point(theta_0, theta_dot_0)
        points = [curr_point]
        t = 0
        while t < total_time:
    ...
    def get_time_step_trajectory(self, delta_t, total_time, theta_0, theta_dot_0):
        traj = VMobject()
        traj.set_points_as_corners(self.get_time_step_points(delta_t, total_time, theta_0, theta_dot_0))
        traj.set_stroke(WHITE, 2)
        return traj

    def get_path_vectors(self, delta_t, total_time, theta_0, theta_dot_0):
        corners = self.get_time_step_points(delta_t, total_time, theta_0, theta_dot_0)
        result = VGroup()
        for a1, a2 in zip(corners, corners[1:]):
            vector = Arrow(a1, a2, buff=0)
...
class SetupToTakingManyTinySteps(TakeManyTinySteps):
    ...
    def show_step(self):
        ...
        field = self.vector_field
        point = self.plane.coords_to_point(self.initial_theta, self.initial_theta_dot)
        ...
        self.play(ReplacementTransform(init_labels_rect, dot))
        self.wait()
        self.add(vector, dot)
        self.play(ShowCreation(vector), FadeIn(v_label, RIGHT))
        ...
        self.play(ReplacementTransform(vector, scaled_vector), FadeIn(rect), MoveToTarget(v_label), MoveToTarget(dt_part))
        self.add(scaled_vector, dot)
        self.wait()
        self.play(LaggedStart(*[Transform(sm1, sm2, rate_func=there_and_back_with_pause) for sm1, sm2 in zip(field, real_field)], lag_ratio=0.001, run_time=3))
        self.wait()
...
class ManyStepsFromDifferentStartingPoints(TakeManyTinySteps):
    CONFIG = {'initial_thetas': np.linspace(0.1, PI - 0.1, 10), 'initial_theta_dot': 0}
    ...
    def take_many_time_steps(self):
        ...
        get_t = time_tracker.get_value
        vectors = always_redraw(lambda: VGroup(*[self.get_path_vectors(get_delta_t(), get_t(), theta, self.initial_theta_dot) for theta in self.initial_thetas]))
        self.add(vectors)
        time_tracker.set_value(0)
        self.play(time_tracker.set_value, 5, run_time=5, rate_func=linear)

class Thumbnail(IntroduceVectorField):
    CONFIG = {'vector_field_config': {'delta_x': 1, 'delta_y': 1, 'max_magnitude': 5, 'length_func': lambda norm: 0.9 * sigmoid(norm)}, 'big_pendulum_config': {'damping': 0.4}}
