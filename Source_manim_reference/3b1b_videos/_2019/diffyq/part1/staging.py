...
class TourOfDifferentialEquations(MovingCameraScene):
    CONFIG = {'screen_rect_style': {'stroke_width': 2, 'stroke_color': WHITE, 'fill_opacity': 1, 'fill_color': BLACK}, 'camera_config': {'background_color': GREY_E}, 'zoomed_thumbnail_index': 0}
    ...
    def show_thumbnails(self):
        thumbnails = self.thumbnails = Group(Group(ScreenRectangle(**self.screen_rect_style)), Group(ScreenRectangle(**self.screen_rect_style)), Group(ScreenRectangle(**self.screen_rect_style)), Group(ScreenRectangle(**self.screen_rect_style)), Group(ScreenRectangle(**self.screen_rect_style)))
        n = len(thumbnails)
        thumbnails.set_height(1.5)
        line = self.line = CubicBezier([-5, 3, 0], [3, 3, 0], [-3, -3, 0], [5, -3, 0])
        ...
        self.play(ShowCreation(line, rate_func=lambda t: np.clip(t * (n + 1) / n, 0, 1)), LaggedStart(*[GrowFromCenter(thumbnail, rate_func=squish_rate_func(smooth, 0, 0.7)) for thumbnail in thumbnails], lag_ratio=1), run_time=5)
        self.play(Write(dots))
        self.wait()
        self.thumbnails = thumbnails

    def zoom_in_to_one_thumbnail(self):
        self.play(self.camera_frame.replace, self.thumbnails[self.zoomed_thumbnail_index], run_time=3)
        self.wait()

    def show_words(self):
        words = VGroup(OldTexText('Generalize'), OldTexText('Put in context'), OldTexText('Modify'))
        words.scale(1.5)
        words.to_corner(UR)
        ...
        for word1, word2 in zip(words, words[1:]):
            self.play(FadeInFromDown(word2), FadeOut(word1, UP))
            self.wait()
        self.play(ReplacementTransform(VGroup(self.thumbnails).copy().fade(1), diffEq, lag_ratio=0.01))
        self.wait()
    ...
    def add_matrix_exponent(self, thumbnail):
        matrix = IntegerMatrix([[3, 1], [4, 1]], v_buff=MED_LARGE_BUFF, h_buff=MED_LARGE_BUFF, bracket_h_buff=SMALL_BUFF, bracket_v_buff=SMALL_BUFF)
        e = OldTex('e')
        t = OldTex('t')
        t.scale(1.5)
    ...
    def get_square_wave_approx(self, N, color):
        return FunctionGraph(lambda x: sum([1 / n * np.sin(n * PI * x) for n in range(1, 2 * N + 3, 2)]), x_min=0, x_max=2, color=color)
...
class ShowGravityAcceleration(Scene):
    CONFIG = {'flash': True, 'add_ball_copies': True}
    ...
    def add_gravity_field(self):
        gravity_field = self.gravity_field = VectorField(lambda p: DOWN)
        gravity_field.set_opacity(0.5)
        gravity_field.sort(lambda p: -p[1])
    ...
    def add_title(self):
        ...
        title.scale(1.5)
        title.to_edge(UP)
        title.add_background_rectangle(buff=0.05, opacity=1)
        self.play(FadeInFromDown(title))

    def pulse_gravity_down(self):
        field = self.gravity_field
        self.play(LaggedStart(*[ApplyFunction(lambda v: v.set_opacity(1).scale(1.2), vector, rate_func=there_and_back) for vector in field]), run_time=2, lag_ratio=0.001)
        self.add(self.title)
    ...
    def show_g_value(self):
        ...
        self.wait(0.25)
        self.play(LaggedStart(GrowFromPoint(per, per.get_left()), FadeIn(s, 0.5 * UP), lag_ratio=0.7, run_time=0.75))
        self.wait()
        self.play(g_eq.scale, 0.5, g_eq.next_to, title, DOWN)
    ...
    def show_trajectory(self):
        ...
        v0 = 2.8 * UP + 1.5 * RIGHT
        g = 0.9 * DOWN
        graph = ParametricCurve(lambda t: p0 + v0 * t + 0.5 * g * t ** 2, t_min=0, t_max=total_time)
        ...
        velocity_graph = ParametricCurve(lambda t: v0 + g * t, t_min=0, t_max=total_time)
        v_point = VectorizedPoint()
        v_point.move_to(velocity_graph.get_start())

        def get_v_vect():
            result = Vector(v_point.get_location(), color=RED, tip_length=0.2)
        ...
        for t1, t2 in zip(times, times[1:]):
            ...
            anims = [ShowCreation(dashed_graph, **kw), MoveAlongPath(ball, graph, **kw), MoveAlongPath(v_point, velocity_graph, **kw), ApplyMethod(time_tracker.increment_value, 1, rate_func=linear)]
            if self.flash:
                anims.append(flash)
    ...
    def combine_v_vects(self):
        ...
        delta_vects = VGroup(*[Arrow(v1.get_end(), v2.get_end(), buff=0.01, color=YELLOW).set_opacity(0.5) for v1, v2 in zip(v_vects, v_vects[1:])])
        brace = Brace(Line(ORIGIN, UP), RIGHT)
        braces = VGroup(*[brace.copy().match_height(arrow).next_to(arrow, RIGHT, buff=0.2 * SMALL_BUFF) for arrow in delta_vects])
        amounts = VGroup(*[OldTexText('9.8 m/s').scale(0.5).next_to(brace, RIGHT, SMALL_BUFF) for brace in braces])
        self.play(FadeOut(self.gravity_field), FadeIn(delta_vects, lag_ratio=0.1))
        self.play(LaggedStartMap(GrowFromCenter, braces), LaggedStartMap(FadeInFrom, amounts, lambda m: (m, LEFT)))
        self.wait()
    ...
    def show_g_symbol(self):
        ...
        g.next_to(brace, UP)
        g.set_color(YELLOW)
        self.play(FadeOut(self.title), GrowFromCenter(brace), FadeIn(g, UP))
        self.wait()

    def get_ball(self):
        ball = Circle(stroke_width=1, stroke_color=WHITE, fill_color=GREY, fill_opacity=1, sheen_factor=1, sheen_direction=UL, radius=0.25)
...
class SimpleProjectileEquation(ShowGravityAcceleration):
    CONFIG = {'y0': 0, 'g': 9.8, 'axes_config': {'x_min': 0, 'x_max': 6, 'x_axis_config': {'unit_size': 1.5, 'tip_width': 0.15}, 'y_min': -30, 'y_max': 35, 'y_axis_config': {'unit_size': 0.1, 'big_tick_numbers': range(-30, 35, 10), 'tick_size': 0.05, 'numbers_to_show': range(-30, 31, 10), 'tip_width': 0.15}, 'center_point': 2 * LEFT}}
    ...
    def setup_trajectory(self):
        ...
        h_line = always_redraw(lambda: DashedLine(get_ball_point(), axes.input_to_graph_point(get_t(), graph_template), stroke_width=1))
        y_label = OldTex('y', '(t)')
        y_label.set_color_by_tex('y', BLUE)
        y_label.add_updater(lambda m: m.next_to(graph.get_last_point(), UR, SMALL_BUFF))

        def v_func(t):
            return -g * t + v0

        def get_v_vect():
            return Vector(axes.y_axis.unit_size * v_func(get_t()) * UP, color=RED)
        v_vect = always_redraw(lambda: get_v_vect().shift(get_ball_point()))
        v_brace = always_redraw(lambda: Brace(v_vect, LEFT))
        dy_dt_label = OldTex('{d', 'y', '\\over dt}', '(t)')
        ...
        for label in (dy_dt_label, y_dot_label):
            label.add_updater(lambda m: m.next_to(v_brace, LEFT, SMALL_BUFF))
        graphed_v_vect = always_redraw(lambda: get_v_vect().shift(axes.coords_to_point(get_t(), 0)))
        v_graph_template = axes.get_graph(v_func, x_max=total_time)
        v_graph = VMobject()
        v_graph.set_stroke(RED, 2)
        v_graph.add_updater(lambda m: m.pointwise_become_partial(v_graph_template, 0, get_t() / total_time))

        def get_a_vect():
            return Vector(axes.y_axis.unit_size * g * DOWN)
        a_vect = get_a_vect()
        a_vect.add_updater(lambda a: a.move_to(get_ball_point(), UP))
        ...
        d2y_dt2_label.set_color_by_tex('y', BLUE)
        ...
        a_graph = axes.get_graph(lambda t: -g, x_max=total_time)
        a_graph.set_stroke(YELLOW, 2)
        graphed_a_vect = get_a_vect()
        graphed_a_vect.add_updater(lambda a: a.move_to(axes.coords_to_point(get_t(), 0), UP))
        self.set_variables_as_attrs(t_tracker, graph, y_label, traj, h_line, v_vect, v_brace, dy_dt_label, y_dot_label, ball, graphed_v_vect, v_graph, a_vect, a_brace, d2y_dt2_label, y_ddot_label, a_graph, graphed_a_vect)

    def show_trajectory(self):
        self.add(self.h_line, self.traj, self.ball, self.graph, self.y_label)
        self.play_trajectory()
        self.wait()
        self.add(self.v_vect, self.v_brace, self.dy_dt_label, self.ball, self.graphed_v_vect, self.v_graph)
        self.play_trajectory()
        self.wait()
        self.add(self.a_vect, self.ball, self.a_brace, self.d2y_dt2_label, self.a_graph, self.graphed_a_vect)
        self.play_trajectory()
        self.wait()
        self.play(ReplacementTransform(self.dy_dt_label, self.y_dot_label), ShowCreationThenFadeAround(self.y_dot_label))
        self.play(ReplacementTransform(self.d2y_dt2_label, self.y_ddot_label), ShowCreationThenFadeAround(self.y_ddot_label))

    def show_equation(self):
        y_ddot = self.y_ddot_label
        new_y_ddot = y_ddot.deepcopy()
        new_y_ddot.clear_updaters()
        equation = VGroup(new_y_ddot, *Tex('=', '-g', tex_to_color_map={'-g': YELLOW}))
        new_y_ddot.next_to(equation[1], LEFT, SMALL_BUFF)
        equation.move_to(self.axes)
        equation.to_edge(UP)
        self.play(TransformFromCopy(y_ddot, new_y_ddot), Write(equation[1:]), FadeOut(self.graph), FadeOut(self.y_label), FadeOut(self.h_line), FadeOut(self.v_graph), FadeOut(self.graphed_v_vect), FadeOut(self.graphed_a_vect))
    ...
    def solve_for_velocity(self):
        ...
        tex_question, answer1, answer2 = derivs = [OldTex('{d', '(', *term, ')', '\\over', 'dt}', '(t)', '=', '-g', tex_to_color_map={'-g': YELLOW, 'v_0': RED, '?': RED}) for term in [('?', '?', '?', '?'), ('-g', 't'), ('-g', 't', '+', 'v_0')]]
        for x in range(2):
            answer1.submobjects.insert(4, VectorizedPoint(answer1[4].get_left()))
        for deriv in derivs:
            deriv.next_to(equation, DOWN, MED_LARGE_BUFF)
        question = OldTexText('What function has slope $-g$?', tex_to_color_map={'$-g$': YELLOW})
        ...
        y_dot_equation = OldTex('{\\dot y}', '(t)', '=', '-g', 't', '+', 'v_0', tex_to_color_map={'{\\dot y}': RED, '-g': YELLOW, 'v_0': RED})
        y_dot_equation.to_corner(UR)
        self.play(FadeIn(tex_question, DOWN), FadeIn(question, UP))
        self.wait()
        self.add(v_graph, question)
        self.play(ReplacementTransform(tex_question, answer1), ShowCreation(v_graph))
        self.wait()
        self.play(ReplacementTransform(answer1, answer2), v_graph.shift, offset)
        self.play(FadeInFromLarge(v0_dot), FadeInFromDown(v0_label))
        self.wait()
        self.play(TransformFromCopy(answer2[2:6], y_dot_equation[3:]), Write(y_dot_equation[:3]), equation.shift, LEFT)
        self.play(FadeOut(question), FadeOut(answer2))
        self.remove(v_graph)
    ...
    def solve_for_position(self):
        ...
        graph = self.graph
        all_terms = [('?', '?', '?', '?'), ('-', '(1/2)', 'g', 't^2', '+', 'v_0', 't'), ('-', '(1/2)', 'g', 't^2', '+', 'v_0', 't', '+', 'y_0')]
        tex_question, answer1, answer2 = derivs = [OldTex('{d', '(', *term, ')', '\\over', 'dt}', '(t)', '=', '-g', 't', '+', 'v_0', tex_to_color_map={'g': YELLOW, 'v_0': RED, '?': BLUE, 'y_0': BLUE}) for term in all_terms]
        answer1.scale(0.8)
        answer2.scale(0.8)
        ...
        y_equation = OldTex('y', '(t)', '=', '-', '(1/2)', 'g', 't^2', '+', 'v_0', 't', '+', 'y_0', tex_to_color_map={'y': BLUE, 'g': YELLOW, 'v_0': RED})
        y_equation.next_to(VGroup(equation, y_dot_equation), DOWN, MED_LARGE_BUFF)
        self.play(FadeIn(tex_question, DOWN))
        self.wait()
        self.add(graph, tex_question)
        self.play(ReplacementTransform(tex_question, answer1), ShowCreation(graph))
        ...
        self.play(graph.shift, 2 * DOWN, rate_func=there_and_back, run_time=2)
        graph.add_updater(g_updaters[0])
        self.wait()
        br = BackgroundRectangle(y_equation)
        self.play(FadeIn(br), ReplacementTransform(answer2[2:11], y_equation[3:]), FadeIn(y_equation[:3]), FadeOut(answer2[:2]), FadeOut(answer2[11:]))
        self.play(ShowCreationThenFadeAround(y_equation))
        self.play_trajectory()

    def play_trajectory(self, *added_anims, **kwargs):
        self.t_tracker.set_value(0)
        self.play(ApplyMethod(self.t_tracker.set_value, 5, rate_func=linear, run_time=self.total_time), *added_anims)
        self.wait()
...
class UniversalGravityLawSymbols(Scene):
    ...
    def construct(self):
        ...
        law = OldTex('F_1', '=', 'm_1', a1_tex, '=', 'G', 'm_1', 'm_2', '\\left({', x2_tex, '-', x1_tex, '\\over', '||', x2_tex, '-', x1_tex, '||', '}\\right)', '\\left({', '1', '\\over', '||', x2_tex, '-', x1_tex, '||^2', '}\\right)', tex_to_color_map={x1_tex: BLUE_C, 'm_1': BLUE_C, x2_tex: new_brown, 'm_2': new_brown, a1_tex: YELLOW})
        law.to_edge(UP)
        force = law[:4]
        constants = law[4:8]
        unit_vect = law[8:19]
        ...
        words = VGroup(OldTexText('Force on\\\\mass 1'), OldTexText('Unit vector\\\\towards mass 2'), OldTexText('Inverse square\\\\law'))
        self.add(law)
        braces = VGroup()
        rects = VGroup()
        for part, word in zip(parts, words):
            ...
            braces.add(brace)
            rects.add(rect)
        self.play(ShowCreationThenFadeOut(rects[0]), GrowFromCenter(braces[0]), FadeIn(words[0], UP))
        self.wait()
        self.play(ShowCreationThenFadeOut(rects[1]), GrowFromCenter(braces[1]), FadeIn(words[1], UP))
        self.wait()
        self.play(ShowCreationThenFadeOut(rects[2]), TransformFromCopy(*braces[1:3]), FadeIn(words[2], UP))
        self.wait()
        v1_tex = '\\vec{\\textbf{v}}_1'
        kw = {'tex_to_color_map': {x1_tex: BLUE_C, v1_tex: RED}}
        ...
        self.play(FadeInFromDown(x_deriv), MoveToTarget(law), braces[1:].align_to, lt, RIGHT, MaintainPositionRelativeTo(words[1:], braces[1:]), FadeOut(words[0]), FadeOut(braces[0]))
        self.play(ShowCreationThenFadeAround(x_deriv))
        self.play(TransformFromCopy(x_deriv.get_part_by_tex(v1_tex), v_deriv.get_part_by_tex(v1_tex)), Write(VGroup(*filter(lambda m: m is not v_deriv.get_part_by_tex(v1_tex), v_deriv))))
        x_parts = law.get_parts_by_tex(x1_tex)
        self.play(TransformFromCopy(x_deriv.get_parts_by_tex(x1_tex), x_parts.copy(), remover=True, path_arc=30 * DEGREES))
...
class ExampleTypicalODE(TeacherStudentsScene):
    ...
    def construct(self):
        examples = VGroup(OldTex('{\\dot x}(t) = k{x}(t)', tex_to_color_map={'{\\dot x}': BLUE, '{x}': BLUE}), get_ode(), OldTex('{\\partial T', '\\over', '\\partial t} = ', '{\\partial^2 T', '\\over', '\\partial x^2}', '+', '{\\partial^2 T', '\\over', '\\partial y^2}', '+', '{\\partial^2 T', '\\over', '\\partial z^2}', tex_to_color_map={'T': RED}))
        examples[1].get_parts_by_tex('theta').set_color(GREEN)
        examples.arrange(DOWN, buff=MED_LARGE_BUFF)
        examples.to_edge(UP)
        self.play(FadeIn(examples[0], UP), self.teacher.change, 'raise_right_hand')
        self.play(FadeIn(examples[1], UP), self.change_students(*3 * ['pondering'], look_at=examples))
...
class ShowDerivativeVideo(Scene):
    ...
    def construct(self):
        ...
        rect = rect.copy()
        rect.set_style(fill_opacity=1, fill_color=BLACK, stroke_width=0)
...
class DefineODE(Scene):
    CONFIG = {'pendulum_config': {'length': 2, 'top_point': 5 * RIGHT + 2 * UP, 'initial_theta': 150 * DEGREES, 'mu': 0.3}, 'axes_config': {'y_axis_config': {'unit_size': 0.75}, 'y_max': PI, 'y_min': -PI, 'x_max': 10, 'x_axis_config': {'numbers_to_show': range(2, 10, 2), 'unit_size': 1}}}
    ...
    def write_differential_equation(self):
        ...
        self.add(equation)
        self.wait(5)
        self.play(ShowPassingFlashWithThinningStrokeWidth(lines, lag_ratio=0.002, run_time=1.5, time_width=0.9, n_segments=5))
        self.play(FadeInFromDown(de_word))
        self.wait(2)
        self.play(LaggedStartMap(ApplyMethod, thetas, lambda m: (m.shift, 0.25 * DOWN), rate_func=there_and_back))
        self.wait()
    ...
    def dont_know_the_value(self):
        ...
        self.play(VFadeOut(graph), FadeOut(pendulum), LaggedStart(*[UpdateFromAlphaFunc(q_mark, lambda m, a: m.set_height(0.5 * (1 + a)).set_fill(opacity=there_and_back(a))) for q_mark in q_marks], lag_ratio=0.01, run_time=2))
        self.remove(q_marks)

    def show_value_slope_curvature(self):
        axes = self.axes
        p = self.pendulum
        graph = axes.get_graph(lambda t: p.initial_theta * np.cos(np.sqrt(p.gravity / p.length) * t) * np.exp(-p.mu * t / 2))
        tex_config = {'tex_to_color_map': {'{\\theta}': BLUE, '{\\dot\\theta}': RED, '{\\ddot\\theta}': YELLOW}, 'height': 0.5}
        ...
        def get_v_line():
            ...
            return DashedLine(x_point, point, dash_length=0.025, stroke_color=BLUE, stroke_width=2)

        def get_tangent_line(curve, alpha):
            line = Line(ORIGIN, 1.5 * RIGHT, color=RED, stroke_width=1.5)
            da = 0.0001
        ...
        words = VGroup(OldTexText('= Height').set_color(BLUE), OldTexText('= Slope').set_color(RED), OldTexText("= ``Curvature''").set_color(YELLOW))
        words.scale(0.75)
        for word, sym in zip(words, thetas):
            word.next_to(sym, RIGHT, buff=2 * SMALL_BUFF)
            sym.word = word
        self.play(ShowCreation(v_line), FadeInFromPoint(dot, v_line.get_start()), FadeIn(theta, DOWN), FadeIn(theta.word, DOWN))
        self.add(slope_line, dot)
        self.play(ShowCreation(slope_line), FadeIn(d_theta, LEFT), FadeIn(d_theta.word, LEFT))
        a_tracker = ValueTracker(0)
        curve_copy = curve.copy()
        changing_slope = always_redraw(lambda: get_tangent_line(curve_copy, a_tracker.get_value()).set_stroke(opacity=there_and_back(a_tracker.get_value())))
        self.add(curve, dot)
        self.play(ShowCreation(curve), FadeIn(dd_theta, LEFT), FadeIn(dd_theta.word, LEFT))
        self.add(changing_slope)
        self.play(a_tracker.set_value, 1, run_time=3)
    ...
    def write_ode(self):
        ...
        t_rects = VGroup(*map(SurroundingRectangle, ts))
        x_axis = axes.x_axis
        x_axis_line = Line(x_axis.get_start(), x_axis.get_end(), stroke_color=YELLOW, stroke_width=5)
        ...
        self.play(ShowCreationThenFadeOut(t_rects, lag_ratio=0.8), ShowCreationThenFadeOut(x_axis_line))
        self.play(MoveToTarget(de_word), FadeIn(ordinary, RIGHT), GrowFromCenter(ordinary_underline))
        ...
        for mob in ode_remaining_letters.target:
            mob.shift(0.2 * UP)
            mob.fade(1)
        self.play(MoveToTarget(ode_initials), MoveToTarget(ode_remaining_letters, lag_ratio=0.05))
        self.wait()
    ...
    def show_second_order(self):
        ...
        self.play(Write(so), MoveToTarget(ode))
        self.wait()
    ...
    def show_higher_order_examples(self):
        ...
        example3 = VGroup(OldTexText('Third order ODE'), OldTex('\\dddot {x}(t) + \\dot {x}(t)^2 = 0', **tex_config))
        example4 = VGroup(OldTexText('Fourth order ODE'), OldTex('\\ddddot {x}(t) +', 'a\\dddot {x}(t) \\dot {x}(t) + ', 'b \\ddot {x}(t) {x}(t)', '= 1', **tex_config))
        for example in [example3, example4]:
            example[0].scale(1.2)
            example.arrange(DOWN, buff=MED_LARGE_BUFF)
            example.to_edge(UP, buff=MED_SMALL_BUFF)
        self.play(FadeOut(main_example), FadeIn(example3))
        self.wait(2)
        self.play(FadeOut(example3), FadeIn(example4))
        self.wait(2)
        self.play(FadeOut(example4), FadeIn(main_example))
        self.wait(2)
    ...
    def show_changing_curvature_group(self):
        ...
        graph = VMobject()
        graph.pointwise_become_partial(self.fake_graph, t_tracker.get_value() / self.axes.x_max, 1)
        dashed_graph = DashedVMobject(graph, num_dashes=100)
        dashed_graph.set_stroke(GREEN, 1)
        self.play(FadeOut(labels))
        self.add(dashed_graph, curvature_group)
        self.play(t_tracker.set_value, 10, ShowCreation(dashed_graph), run_time=15, rate_func=linear)
        self.wait()
...
class SecondOrderEquationExample(DefineODE):
    ...
    def add_graph(self):
        axes = self.axes = Axes(x_min=0, x_max=10.5, y_min=-2.5, y_max=2.5)
        axes.center()
        axes.to_edge(DOWN)
        x_t = OldTex('x', '(t)')
    ...
    def write_differential_equation(self):
        ...
        equation = OldTex('{\\ddot x}(t)', '=', '-\\mu', '{\\dot x}(t)', '-', '\\omega', '{x}(t)', tex_to_color_map={'{x}': BLUE, '{\\dot x}': RED, '{\\ddot x}': YELLOW})
        equation.next_to(de_word, DOWN)
        dd_x_part = equation[:2]
        dd_x_rect = SurroundingRectangle(dd_x_part)
        self.add(de_word, equation)
        self.play(MoveToTarget(de_word), FadeIn(so_word, RIGHT), GrowFromCenter(so_line))
    ...
    def show_value_slope_curvature(self):
        ...
        tex_config = {'tex_to_color_map': {'{x}': BLUE, '{\\dot x}': RED, '{\\ddot x}': YELLOW}, 'height': 0.5}
        ...
        for word, sym in zip(words, xs):
            word.next_to(sym, RIGHT, buff=2 * SMALL_BUFF)
            sym.word = word
        self.play(ShowCreation(v_line), FadeInFromPoint(dot, v_line.get_start()), FadeIn(x, DOWN), FadeIn(x.word, DOWN))
        self.add(slope_line, dot)
        self.play(ShowCreation(slope_line), FadeIn(d_x, LEFT), FadeIn(d_x.word, LEFT))
        a_tracker = ValueTracker(0)
        curve_copy = curve.copy()
        changing_slope = always_redraw(lambda: get_tangent_line(curve_copy, a_tracker.get_value()).set_stroke(opacity=there_and_back(a_tracker.get_value())))
        self.add(curve, dot)
        self.play(ShowCreation(curve), FadeIn(dd_x, LEFT), FadeIn(dd_x.word, LEFT))
        self.add(changing_slope)
        self.play(a_tracker.set_value, 1, run_time=3)
        self.remove(changing_slope, a_tracker)
        self.t_tracker = t_tracker
    ...
    def get_main_example(self):
        return VGroup(self.equation, self.title)

class ODEvsPDEinFrames(Scene):
    CONFIG = {'camera_config': {'background_color': GREY_E}}

    def construct(self):
        frames = VGroup(*[ScreenRectangle(height=3.5, fill_opacity=1, fill_color=BLACK, stroke_width=0) for x in range(2)])
        frames.arrange(RIGHT, buff=LARGE_BUFF)
        frames.shift(0.5 * DOWN)
        animated_frames = VGroup(*[AnimatedBoundary(frame, cycle_rate=0.2, max_stroke_width=1) for frame in frames])
        titles = VGroup(OldTexText('Ordinary', 'Differential', 'Equations'), OldTexText('Partial', 'Differential', 'Equations'))
        ...
        o_arrows = VGroup(*[Arrow(one_input.get_top(), t.get_bottom(), buff=0.2, color=WHITE, max_tip_length_to_length_ratio=0.075, path_arc=pa) for t, pa in zip(ts, [-0.7, 0, 0.7])])
        ...
        p_arrows = VGroup(*[Arrow(multi_input.get_top(), mob.get_bottom(), buff=0.2, color=WHITE, max_tip_length_to_length_ratio=0.075, path_arc=pa) for mob, pa in zip(inputs, [-0.7, 0, 0.7])])
        ...
        self.wait(7)
        self.play(TransformFromCopy(*titles), TransformFromCopy(*frames))
...
class BreakingSecondOrderIntoTwoFirstOrder(IntroduceVectorField):
    ...
    def construct(self):
        ode = OldTex('{\\ddot\\theta}', '(t)', '=', '-\\mu', '{\\dot\\theta}', '(t)-(g / L)\\sin\\big(', '{\\theta}', '(t)\\big)', tex_to_color_map={'{\\ddot\\theta}': RED, '{\\dot\\theta}': YELLOW, '{\\theta}': BLUE})
        ...
        theta_dots = VGroup(*filter(lambda m: isinstance(m, SingleStringTex) and '{\\dot\\theta}' == m.get_tex(), system1.get_family()))
        self.add(ode)
        self.play(FadeIn(so_word, 0.5 * DOWN))
        self.wait()
        self.play(TransformFromCopy(ode[3:], system1[3].get_entries()[1]), TransformFromCopy(ode[2], system1[2]), TransformFromCopy(ode[:2], VGroup(system1[0], system1[1].get_entries()[1])))
        self.play(FadeIn(system1[1].get_brackets()), FadeIn(system1[1].get_entries()[0]), FadeIn(system1[3].get_brackets()), FadeIn(system1[3].get_entries()[0]))
        self.play(FadeInFromDown(sys_word))
        self.wait()
        self.play(LaggedStartMap(ShowCreationThenFadeAround, theta_dots, surrounding_rectangle_config={'color': PINK}))
        self.play(ReplacementTransform(system1, system2))
        self.wait()

    def get_system(self, tex1, tex2):
        system = VGroup(OldTex('d \\over dt'), self.get_vector_symbol(tex1 + '(t)', tex2 + '(t)'), OldTex('='), self.get_vector_symbol(tex2 + '(t)', ''.join(['-\\mu', tex2, '(t)', '-(g / L) \\sin\\big(', tex1, '(t)', '\\big)'])))
...
class FromODEToVectorField(Scene):
    ...
    def construct(self):
        matrix_config = {'bracket_v_buff': 2 * SMALL_BUFF, 'element_to_mobject_config': {'tex_to_color_map': {'x': GREEN, 'y': RED, 'z': BLUE}}}
        vect = get_vector_symbol('x(t)', 'y(t)', 'z(t)', **matrix_config)
        d_vect = get_vector_symbol('\\sigma\\big(y(t) - x(t)\\big)', 'x(t)\\big(\\rho - z(t)\\big) - y(t)', 'x(t)y(t) - \\beta z(t)', **matrix_config)
...
class ThreeBodiesInSpace(SpecialThreeDScene):
    CONFIG = {'masses': [1, 6, 3], 'colors': [RED_E, GREEN_E, BLUE_E], 'G': 0.5, 'play_time': 60}
    ...
    def add_bodies(self):
        ...
        for mass, color, center in zip(masses, colors, centers):
            body = self.get_sphere(checkerboard_colors=[color, color], color=color, stroke_width=0.1)
    ...
    def get_velocity_vector_mob(self, body):
        ...
        def draw_vector():
            ...
            vect = Arrow(center, center + body.velocity, buff=0, color=RED)
...
class AltThreeBodiesInSpace(ThreeBodiesInSpace):
    CONFIG = {'random_seed': 6, 'masses': [1, 2, 6]}

class TwoBodiesInSpace(ThreeBodiesInSpace):
    CONFIG = {'colors': [GREY, BLUE], 'masses': [6, 36], 'play_time': 60}
    ...
    def get_velocity_vector(self, body):
        ...
        def create_vector(b):
            v = Vector(b.velocity, color=RED, max_stroke_width_to_length_ratio=3)
            v.set_stroke(width=3)
            v.shift(b.point + b.radius * normalize(b.velocity) - v.get_start())
    ...
    def get_force_vector(self, body1, body2):
        ...
        def create_vector(b1, b2):
            r = b2.point - b1.point
            F = r / get_norm(r) ** 3
            v = Vector(4 * F, color=YELLOW, max_stroke_width_to_length_ratio=3)
            v.set_stroke(width=3)
...
class LoveExample(PiCreatureScene):
    ...
    def show_hearts(self):
        ...
        self.add(hearts)
        self.play(LaggedStartMap(UpdateFromAlphaFunc, hearts, lambda heart: (heart, lambda h, a: h.set_opacity(there_and_back(a)).shift(0.02 * UP)), lag_ratio=0.01, run_time=3, suspend_mobject_updating=False), ApplyMethod(you.change, 'hooray', tau.eyes, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)), ApplyMethod(tau.change, 'hooray', you.eyes, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)))
        self.remove(hearts)
        self.wait()
    ...
    def add_love_trackers(self):
        ...
        self.play(Rotating(self.ps_point, radians=-7 * TAU / 8, about_point=ORIGIN, run_time=10, rate_func=linear))
        self.wait()
    ...
    def break_down_your_rule(self):
        ...
        self.play(self.tau.love_eyes.scale, 1.25, self.tau.love_eyes.set_color, BLUE_C, rate_func=there_and_back)
        self.play(ps_point.shift, 6 * RIGHT, run_time=2)
        self.wait()
        ps_point.shift(13 * DOWN)
        self.play(FadeOut(up_arrow), GrowArrow(down_arrow))
        self.play(ps_point.shift, 11 * LEFT, run_time=3)
        self.wait()
        equation = get_love_equation1()
        equation.shift(0.5 * UP)
        deriv, equals, a, h2 = equation
        self.play(Write(deriv[:-1]), Write(equals), Write(a), TransformFromCopy(label1[0], deriv.heart), TransformFromCopy(label2[0], h2))
        self.wait()
    ...
    def break_down_their_rule(self):
        ...
        self.play(Write(deriv[:-1]), Write(equals), Write(mb), TransformFromCopy(label1[0], h1), TransformFromCopy(label2[0], deriv.heart))
        self.play(GrowArrow(up_arrow))
        self.play(ps_point.shift, 13 * UP, run_time=3)
        self.wait()
        self.play(ps_point.shift, 11 * RIGHT)
        self.play(FadeOut(up_arrow), GrowArrow(down_arrow))
        self.play(ps_point.shift, 13 * DOWN, run_time=3)
        self.wait()
    ...
    def add_love_decimals(self):
        self.love_1_label = self.add_love_decimal(1, self.get_love1, self.you.get_color(), -3)
        self.love_2_label = self.add_love_decimal(2, self.get_love2, self.tau.get_color(), 3)
    ...
    def add_love_number_lines(self):
        nl1 = NumberLine(x_min=-8, x_max=8, unit_size=0.25, tick_frequency=2, number_scale_val=0.25)
...
class ComparePhysicsToLove(Scene):
    ...
    def construct(self):
        ...
        love = VGroup(get_love_equation1(), get_love_equation2())
        love.scale(0.5)
        ...
        self.wait()
        self.play(LaggedStartMap(ShowCreationThenFadeAround, thetas))
        self.play(LaggedStartMap(ShowCreationThenFadeAround, hearts))
        self.wait()
        self.play(ShowCreation(arrow))
        self.wait()
...
class FramesComparingPhysicsToLove(Scene):
    ...
    def construct(self):
        ...
        love.scale(0.5)
        love.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        frames = VGroup(*[ScreenRectangle(height=3.5, fill_color=BLACK, fill_opacity=1, stroke_width=0) for x in range(2)])
        frames.arrange(RIGHT, buff=LARGE_BUFF)
        frames.shift(DOWN)
...
class PassageOfTime(Scene):
    ...
    def construct(self):
        ...
        clock.scale(0.8)
        clock.to_corner(UL)
        passage = ClockPassesTime(clock, hours_passed=48)
...
class InaccurateComputation(Scene):
    ...
    def construct(self):
        ...
        words = VGroup(OldTexText('Real number'), OldTexText('IEEE 754\\\\representation'), OldTexText('Error'))
        ...
        numbers = VGroup(OldTex('\\pi').scale(2), OldTex('e^{\\sqrt{163}\\pi}').scale(1.5))
        numbers.set_color(YELLOW)
        numbers.set_stroke(width=0, background=True)
        bit_strings = VGroup(OldTex('01000000', '01001001', '00001111', '11011011'), OldTex('01011100', '01101001', '00101110', '00011001'))
        ...
        errors = VGroup(OldTex('\\approx 8.7422 \\times 10^{-8}'), OldTex('\\approx 5{,}289{,}803{,}032.00'))
        ...
        self.wait()
        self.play(TransformFromCopy(numbers, bit_strings), lag_ratio=0.01, run_time=2)
        self.wait()
