...
class Pendulum(VGroup):
    ...
    def create_angle_arc(self):
        self.angle_arc = always_redraw(lambda: Arc(arc_center=self.get_fixed_point(), start_angle=-90 * DEGREES, angle=self.get_arc_angle_theta(), **self.angle_arc_config))
    ...
    def add_velocity_vector(self):
        ...
        def make_vector():
            ...
            vector = Vector(multiple * RIGHT, **self.velocity_vector_config)
    ...
    def update_by_gravity(self, dt):
        ...
        for x in range(nspf):
            ...
            d_omega = op.add(-self.damping * omega, -(self.gravity / self.length) * np.sin(theta)) * dt / nspf
            theta += d_theta
            omega += d_omega
...
class GravityVector(Vector):
    CONFIG = {'color': YELLOW, 'length_multiple': 1 / 9.8}
    ...
    def attach_to_pendulum(self, pendulum):
        self.add_updater(lambda m: m.shift(pendulum.weight.get_center() - self.get_start()))
    ...
    def create_component_lines(self):
        ...
        return VGroup(DashedLine(base, corner, **kw), DashedLine(corner, tip, **kw))
...
class ThetaVsTAxes(Axes):
    CONFIG = {'x_min': 0, 'x_max': 8, 'y_min': -PI / 2, 'y_max': PI / 2, 'y_axis_config': {'tick_frequency': PI / 8, 'unit_size': 1.5}, 'axis_config': {'color': '#EEEEEE', 'stroke_width': 2, 'include_tip': False}, 'graph_style': {'stroke_color': GREEN, 'stroke_width': 3, 'fill_opacity': 0}}
    ...
    def get_live_drawn_graph(self, pendulum, t_max=None, t_step=1.0 / 60, **style):
        ...
        def update_graph(graph, dt):
            ...
            points = [self.coords_to_point(*coords) for coords in [*graph.all_coords, new_coords]]
            graph.set_points_smoothly(points)
        graph.add_updater(update_graph)
        return graph

class IntroducePendulum(PiCreatureScene, MovingCameraScene):
    CONFIG = {'pendulum_config': {'length': 3, 'top_point': 4 * RIGHT, 'weight_diameter': 0.35, 'gravity': 20}, 'theta_vs_t_axes_config': {'y_max': PI / 4, 'y_min': -PI / 4, 'y_axis_config': {'tick_frequency': PI / 16, 'unit_size': 2, 'tip_length': 0.3}, 'x_max': 12, 'axis_config': {'stroke_width': 2}}}
    ...
    def label_pi_creatures(self):
        ...
        morty.labels = VGroup(morty_label, morty_label.arrow)
        self.play(FadeInFromDown(randy_label), GrowArrow(randy_label.arrow), randy.change, 'hooray')
        self.play(Animation(self.pendulum.fixed_point_tracker), TransformFromCopy(randy_label[0], morty_label[0]), FadeIn(morty_label[1]), GrowArrow(morty_label.arrow), morty.change, 'raise_right_hand')
        self.wait(2)
    ...
    def add_graph(self):
        ...
        axes.to_corner(UL)
        self.play(Restore(self.camera_frame, rate_func=squish_rate_func(smooth, 0, 0.9)), DrawBorderThenFill(axes, rate_func=squish_rate_func(smooth, 0.5, 1), lag_ratio=0.9), Transform(self.pendulum.theta_label.copy().clear_updaters(), axes.y_axis.label.copy(), remover=True, rate_func=squish_rate_func(smooth, 0, 0.8)), run_time=3)
        self.wait(1.5)
    ...
    def label_function(self):
        ...
        hm_word.scale(1.25)
        hm_word.to_edge(UP)
        formula = OldTex('=\\theta_0 \\cos(\\sqrt{g / L} t)')
    ...
    def show_graph_period(self):
        pendulum = self.pendulum
        axes = self.axes
        period = self.period = TAU * np.sqrt(pendulum.length / pendulum.gravity)
        amplitude = pendulum.initial_theta
        line = Line(axes.coords_to_point(0, amplitude), axes.coords_to_point(period, amplitude))
        ...
        self.play(GrowFromCenter(brace), FadeInFromDown(formula))
        self.wait(2)
    ...
    def show_length_and_gravity(self):
        ...
        new_rod.add_updater(lambda r: r.put_start_and_end_on(*rod.get_start_and_end()))
        g_vect = GravityVector(self.pendulum, length_multiple=0.5 / 9.8)
        down_vectors = self.get_down_vectors()
        down_vectors.set_color(YELLOW)
        down_vectors.set_opacity(0.5)
        self.play(ShowCreationThenDestructionAround(L), ShowCreation(new_rod))
        self.play(FadeOut(new_rod))
        self.play(ShowCreationThenDestructionAround(g), GrowArrow(g_vect))
    ...
    def tweak_length_and_gravity(self):
        ...
        amplitude = pendulum.initial_theta
        graph2, graph3 = [axes.get_graph(lambda t: amplitude * np.cos(TAU * t / p), color=RED) for p in (period2, period3)]
        ...
        self.play(ReplacementTransform(pendulum, new_pendulum), morty.change, 'horrified', morty.shift, 3 * RIGHT, morty.labels.shift, 3 * RIGHT)
        self.remove(morty, morty.labels)
        g_vect.attach_to_pendulum(new_pendulum)
        new_pendulum.start_swinging()
        self.play(ReplacementTransform(graph, graph2), brace.stretch, np.sqrt(2), 0, {'about_edge': LEFT})
        self.add(g_vect)
        self.wait(3)
        new_pendulum.gravity *= 4
        g_vect.scale(2)
        self.play(FadeOut(graph2), self.get_down_vectors_animation(down_vectors))
        self.play(FadeIn(graph3), brace.stretch, 0.5, 0, {'about_edge': LEFT})
        self.wait(6)
    ...
    def get_down_vectors_animation(self, down_vectors):
        return LaggedStart(*[GrowArrow(v, rate_func=there_and_back) for v in down_vectors], lag_ratio=0.0005, run_time=2, remover=True)

class MultiplePendulumsOverlayed(Scene):
    CONFIG = {'initial_thetas': [150 * DEGREES, 90 * DEGREES, 60 * DEGREES, 30 * DEGREES, 10 * DEGREES], 'weight_colors': [PINK, RED, GREEN, BLUE, GREY], 'pendulum_config': {'top_point': ORIGIN, 'length': 3}}

    def construct(self):
        pendulums = VGroup(*[Pendulum(initial_theta=theta, weight_style={'fill_color': wc, 'fill_opacity': 0.5}, **self.pendulum_config) for theta, wc in zip(self.initial_thetas, self.weight_colors)])
        ...
        axes = ThetaVsTAxes(x_max=20, y_axis_config={'unit_size': 0.5, 'tip_length': 0.3})
        axes.to_corner(UL)
        graphs = VGroup(*[axes.get_live_drawn_graph(pendulum, stroke_color=pendulum.weight.get_color(), stroke_width=1) for pendulum in pendulums])
        self.add(pendulums)
...
class LowAnglePendulum(Scene):
    CONFIG = {'pendulum_config': {'initial_theta': 20 * DEGREES, 'length': 2.0, 'damping': 0, 'top_point': ORIGIN}, 'axes_config': {'y_axis_config': {'unit_size': 0.75}, 'x_axis_config': {'unit_size': 0.5, 'numbers_to_show': range(2, 25, 2), 'number_scale_val': 0.5}, 'x_max': 25, 'axis_config': {'tip_length': 0.3, 'stroke_width': 2}}, 'axes_corner': UL}
    ...
    def construct(self):
        ...
        prediction = axes.get_graph(lambda t: theta0 * np.cos(t * np.sqrt(g / L)))
        dashed_prediction = DashedVMobject(prediction, num_dashes=300)
        dashed_prediction.set_stroke(WHITE, 1)
        prediction_formula = OldTex('\\theta_0', '\\cos(\\sqrt{g / L} \\cdot t)')
        prediction_formula.scale(0.75)
        prediction_formula.next_to(dashed_prediction, UP, SMALL_BUFF)
        ...
        theta0_label = Integer(pendulum.initial_theta * 180 / PI, unit='^\\circ')
        ...
        self.play(ShowCreation(dashed_prediction, run_time=2), FadeInFromDown(prediction_formula), FadeInFromDown(theta0_brace), FadeInFromDown(theta0_label))
        self.play(ShowCreationThenFadeAround(theta0_label), ShowCreationThenFadeAround(pendulum.theta_label))
        self.wait()
        pendulum.start_swinging()
        self.add(graph)
        self.wait(30)
...
class ApproxWordsLowAnglePendulum(Scene):
    ...
    def construct(self):
        period = OldTex('\\text{Period}', '\\approx', '2\\pi \\sqrt{\\,{L} / {g}}', **Lg_formula_config)
...
class MediumAnglePendulum(LowAnglePendulum):
    CONFIG = {'pendulum_config': {'initial_theta': 50 * DEGREES, 'n_steps_per_frame': 1000}, 'axes_config': {'y_axis_config': {'unit_size': 0.75}, 'y_max': PI / 2, 'y_min': -PI / 2, 'axis_config': {'tip_length': 0.3, 'stroke_width': 2}}, 'pendulum_shift_vect': 1 * RIGHT}

class MediumHighAnglePendulum(MediumAnglePendulum):
    CONFIG = {'pendulum_config': {'initial_theta': 90 * DEGREES, 'n_steps_per_frame': 1000}}

class HighAnglePendulum(LowAnglePendulum):
    CONFIG = {'pendulum_config': {'initial_theta': 175 * DEGREES, 'n_steps_per_frame': 1000, 'top_point': 1.5 * DOWN, 'length': 2}, 'axes_config': {'y_axis_config': {'unit_size': 0.5}, 'y_max': PI, 'y_min': -PI, 'axis_config': {'tip_length': 0.3, 'stroke_width': 2}}, 'pendulum_shift_vect': 1 * RIGHT}

class VeryLowAnglePendulum(LowAnglePendulum):
    CONFIG = {'pendulum_config': {'initial_theta': 10 * DEGREES, 'n_steps_per_frame': 1000, 'top_point': ORIGIN, 'length': 3}, 'axes_config': {'y_axis_config': {'unit_size': 2}, 'y_max': PI / 4, 'y_min': -PI / 4, 'axis_config': {'tip_length': 0.3, 'stroke_width': 2}}, 'pendulum_shift_vect': 1 * RIGHT}

class WherePendulumLeads(PiCreatureScene):

    def construct(self):
        pendulum = Pendulum(top_point=UP, length=3, gravity=20)
        pendulum.start_swinging()
        ...
        spring = always_redraw(lambda: ParametricCurve(lambda t: np.array([np.cos(TAU * t) + (1.4 + get_theta()) * t, np.sin(TAU * t) - 0.5, 0]), t_min=-0.5, t_max=7, color=GREY, sheen_factor=1, sheen_direction=UL).scale(0.2).to_edge(LEFT, buff=0))
        spring_rect = SurroundingRectangle(spring, buff=MED_LARGE_BUFF, stroke_width=0, fill_color=BLACK, fill_opacity=0)
        weight = Dot(radius=0.25)
        weight.add_updater(lambda m: m.move_to(spring.get_points()[-1]))
        weight.set_color(BLUE)
        weight.set_sheen(1, UL)
        spring_system = VGroup(spring, weight)
        linear_formula = OldTex('\\frac{d \\vec{\\textbf{x}}}{dt}=A\\vec{\\textbf{x}}')
        ...
        self.play(randy.change, 'pondering', pendulum, FadeInFromDown(q_marks, lag_ratio=0.3))
        self.play(randy.look_at, pendulum)
        self.wait(5)
        self.play(Animation(VectorizedPoint(pendulum.get_top())), FadeOut(q_marks, UP, lag_ratio=0.3))
        self.add(spring_system)
        self.play(FadeOut(spring_rect), FadeIn(linear_formula, UP), FadeInFromDown(l_title))
...
class AnalyzePendulumForce(MovingCameraScene):
    CONFIG = {'pendulum_config': {'length': 5, 'top_point': 3.5 * UP, 'initial_theta': 60 * DEGREES, 'set_theta_label_height_cap': True}, 'g_vect_config': {'length_multiple': 0.25}, 'tan_line_color': BLUE, 'perp_line_color': PINK}
    ...
    def show_arc_length(self):
        ...
        line.move_to(top, UP)
        arc = always_redraw(lambda: Arc(start_angle=-90 * DEGREES, angle=pendulum.get_theta(), arc_center=pendulum.get_fixed_point(), radius=pendulum.length, stroke_color=GREEN))
        ...
        brace.move_to(arc)
        brace.shift(MED_SMALL_BUFF * normalize(arc.point_from_proportion(0.5) - top))
        x_sym = OldTex('x')
        ...
        line_L = OldTex('L')
        line_L.next_to(pendulum.rod.get_center(), UR, SMALL_BUFF)
        self.play(ShowCreation(arc), Rotate(line, angle, about_point=top), UpdateFromAlphaFunc(line, lambda m, a: m.set_stroke(width=2 * there_and_back(a))), GrowFromPoint(brace, line.get_bottom(), path_arc=angle))
        self.play(FadeIn(x_sym, UP))
        self.wait()
        line.set_stroke(BLUE, 5)
        self.play(ShowCreationThenFadeOut(line), FadeInFromDown(line_L))
        self.play(TransformFromCopy(line_L, rhs.get_part_by_tex('L')), Write(rhs.get_part_by_tex('=')))
        self.play(TransformFromCopy(pendulum.theta_label, rhs.get_parts_by_tex('\\theta')))
        self.add(rhs)
        x_eq = VGroup(x_sym, rhs)
        self.play(FadeOut(brace), x_eq.rotate, angle / 2, x_eq.next_to, arc.point_from_proportion(0.5), UL, {'buff': -MED_SMALL_BUFF})
        self.x_eq = x_eq
        self.arc = arc
        self.line_L = line_L
    ...
    def add_g_vect(self):
        pendulum = self.pendulum
        g_vect = self.g_vect = GravityVector(pendulum, **self.g_vect_config)
        g_word = self.g_word = OldTexText('Gravity')
        g_word.rotate(-90 * DEGREES)
        g_word.scale(0.75)
        g_word.add_updater(lambda m: m.next_to(g_vect, RIGHT, buff=-SMALL_BUFF))
        self.play(GrowArrow(g_vect), FadeIn(g_word, UP, lag_ratio=0.1))
        self.wait()
    ...
    def show_constraint(self):
        ...
        for u in [-1, 2, -1]:
            ...
            arc = Arc(start_angle=pendulum.get_theta() - 90 * DEGREES, angle=d_theta, radius=pendulum.length, arc_center=pendulum.get_fixed_point(), stroke_width=2, stroke_color=YELLOW, stroke_opacity=0.5)
            self.play(self.theta_tracker.increment_value, d_theta, ShowCreation(arc))
            arcs.add(arc)
    ...
    def break_g_vect_into_components(self):
        ...
        g_vect.tangent = always_redraw(lambda: Arrow(tan_line.get_start(), tan_line.get_end(), buff=0, color=self.tan_line_color))
        g_vect.perp = always_redraw(lambda: Arrow(perp_line.get_start(), perp_line.get_end(), buff=0, color=self.perp_line_color))
    ...
    def show_angle_geometry(self):
        g_vect = self.g_vect
        arc = Arc(start_angle=90 * DEGREES, angle=self.pendulum.get_theta(), radius=0.5, arc_center=g_vect.get_end())
        q_mark = OldTex('?')
        ...
        self.play(TransformFromCopy(self.pendulum.theta_label, theta_label), FadeOut(q_mark))
        self.wait()
        self.play(WiggleOutThenIn(g_vect.tangent))
        self.play(WiggleOutThenIn(Line(*g_vect.get_start_and_end(), buff=0).add_tip().match_style(g_vect), remover=True))
        self.wait()
        self.play(FadeOut(arc), FadeOut(theta_label))
    ...
    def show_gsin_formula(self):
        ...
        g_term.add_updater(lambda m: m.next_to(g_vect, RIGHT if self.pendulum.get_theta() >= 0 else LEFT, SMALL_BUFF))
        ...
        def create_vect_label(vect, tex, direction):
            ...
            return label
        g_sin_label = always_redraw(lambda: create_vect_label(g_vect.tangent, '-g\\sin(\\theta)', UP))
        g_cos_label = always_redraw(lambda: create_vect_label(g_vect.perp, '-g\\cos(\\theta)', DOWN))
        self.play(ReplacementTransform(g_word[0][0], g_term[0][1]), FadeOut(g_word[0][1:]), Write(g_term[0][0]))
        self.add(g_term)
        self.wait()
        for label in (g_sin_label, g_cos_label):
            self.play(GrowFromPoint(label[0], g_term.get_center()), TransformFromCopy(g_term, label[1][:2]), GrowFromPoint(label[1][2:], g_term.get_center()), remover=True)
            self.add(label)
            self.wait()
    ...
    def show_sign(self):
        ...
        theta_decimal.add_updater(lambda m: m.next_to(self.pendulum.theta_label, DOWN))
        theta_decimal.add_updater(lambda m: m.set_color(GREEN if get_theta() > 0 else RED))
        self.play(FadeIn(theta_decimal, UP), FadeOut(self.x_eq), FadeOut(self.line_L))
        self.set_theta(-60 * DEGREES, run_time=4)
        self.set_theta(60 * DEGREES, run_time=4)
        self.play(FadeOut(theta_decimal), FadeIn(self.x_eq))

    def show_acceleration_formula(self):
        x_eq = self.x_eq
        g_sin_theta = self.g_sin_label
        equation = OldTex('a', '=', '\\ddot', 'x', '=', '-', 'g', '\\sin\\big(', '\\theta', '\\big)')
        ...
        self.play(TransformFromCopy(x_eq[0], x_part), Write(equation[:3]))
        self.wait()
        self.play(Write(eq2), TransformFromCopy(g_sin_theta, rhs))
        self.wait()
        self.show_acceleration_at_different_angles()
        self.play(FadeInFromDown(second_deriv_L_form), Write(eq3), second_deriv.next_to, eq3, UP, a_eq.shift, SMALL_BUFF * LEFT, eq2.shift, SMALL_BUFF * RIGHT, rhs.shift, SMALL_BUFF * RIGHT)
        self.wait()
        self.wait()
        self.play(FadeOut(a_eq), FadeOut(second_deriv), FadeOut(eq3), ReplacementTransform(second_deriv_L_form.get_part_by_tex('L'), g_L_frac.get_part_by_tex('L')), ReplacementTransform(equation.get_part_by_tex('-'), g_L_frac.get_part_by_tex('-')), ReplacementTransform(equation.get_part_by_tex('g'), g_L_frac.get_part_by_tex('g')), Write(g_L_frac.get_part_by_tex('\\over')), rhs[2:].next_to, g_L_frac, RIGHT, {'buff': SMALL_BUFF})
        self.wait()
        self.play(GrowFromCenter(mu_term), VGroup(eq2, second_deriv_L_form[1:]).next_to, mu_term, LEFT)
        self.play(GrowFromCenter(mu_brace), FadeInFromDown(mu_word))

    def show_acceleration_at_different_angles(self):
        to_fade = VGroup(self.g_cos_label, self.g_vect.perp)
        new_comp_line_sytle = {'stroke_width': 0.5, 'stroke_opacity': 0.25}
        self.play(FadeOut(self.x_eq), to_fade.set_opacity, 0.25, self.g_vect.component_lines.set_style, new_comp_line_sytle)
        ...
        self.wait(2)
        self.set_theta(60 * DEGREES, FadeIn(self.x_eq), run_time=2)
        self.wait()
    ...
    def ask_about_what_to_do(self):
        ...
        randy = You()
        randy.to_corner(DL)
        bubble = randy.get_bubble(height=2, width=3.5)
        ...
        self.play(ShowCreationThenFadeOut(rect))
        self.play(ShowCreation(bubble), Write(a_eq), MoveToTarget(g_sin_copy), randy.look_at, bubble)
        thought_term.remove(g_sin_copy.target)
        thought_term.add(g_sin_copy)
        self.play(Blink(randy))
        self.wait()
        self.play(ShowCreationThenDestruction(thought_term.copy().set_style(stroke_color=YELLOW, stroke_width=2, fill_opacity=0), run_time=2, lag_ratio=0.2), randy.change, 'confused', thought_term)
        self.play(Blink(randy))
        self.play(FadeOut(randy), FadeOut(bubble), thought_term.next_to, self.pendulum, DOWN, LARGE_BUFF)
    ...
    def set_theta(self, value, *added_anims, **kwargs):
        ...
        self.play(self.theta_tracker.set_value, value, *added_anims, **kwargs)

class BuildUpEquation(Scene):
    CONFIG = {'tex_config': {'tex_to_color_map': {'{a}': YELLOW, '{v}': RED, '{x}': GREEN, '\\theta': BLUE, '{L}': WHITE}}}
    ...
    def show_derivatives(self):
        a_eq = OldTex('{a}', '=', '{d{v} \\over dt}', **self.tex_config)
        v_eq = OldTex('{v}', '=', '{d{x} \\over dt}', **self.tex_config)
        x_eq = OldTex('{x} = {L} \\theta', **self.tex_config)
        eqs = VGroup(a_eq, v_eq, x_eq)
        eqs.arrange(DOWN, buff=LARGE_BUFF)
        eqs.to_corner(UL)
        v_rhs = OldTex('={L}{d\\theta \\over dt}', '=', '{L}\\dot{\\theta}', **self.tex_config)
        v_rhs.next_to(v_eq, RIGHT, SMALL_BUFF)
        v_rhs.shift(UP * (v_eq[1].get_bottom()[1] - v_rhs[0].get_bottom()[1]))
        a_rhs = OldTex('={L}{d', '\\dot{\\theta}', '\\over dt}', '=', '{L}\\ddot{\\theta}', **self.tex_config)
        a_rhs.next_to(a_eq, RIGHT, SMALL_BUFF)
        a_rhs.shift(UP * (a_eq[1].get_bottom()[1] - a_rhs[0].get_bottom()[1]))
        self.play(Write(a_eq))
        self.wait()
        self.play(TransformFromCopy(a_eq.get_part_by_tex('{v}'), v_eq.get_part_by_tex('{v}')))
        self.play(TransformFromCopy(v_eq[:1], v_eq[1:]))
        self.wait()
        self.play(TransformFromCopy(v_eq.get_part_by_tex('{x}'), x_eq.get_part_by_tex('{x}')))
        self.play(Write(x_eq[1:]))
        self.wait()
        for tex in ('L', '\\theta'):
            self.play(ShowCreationThenFadeAround(x_eq.get_part_by_tex(tex)))
        self.wait()
        self.play(*[TransformFromCopy(x_eq.get_part_by_tex(tex), v_rhs.get_part_by_tex(tex)) for tex in ('=', '{L}', '\\theta')])
        self.play(TransformFromCopy(v_eq[-3], v_rhs[2]), TransformFromCopy(v_eq[-1], v_rhs[4]))
        self.wait()
        self.play(Write(v_rhs[-5]), TransformFromCopy(*v_rhs.get_parts_by_tex('{L}')), TransformFromCopy(v_rhs[3:4], v_rhs[-3:]))
        self.wait()
        self.play(ShowCreationThenFadeAround(v_rhs[2:4]))
        self.play(ShowCreationThenFadeAround(v_rhs[4]))
        self.wait()
        self.play(*[TransformFromCopy(v_rhs.get_parts_by_tex(tex)[-1], a_rhs.get_part_by_tex(tex)) for tex in ('=', '{L}', '\\theta', '\\dot')])
        self.play(TransformFromCopy(a_eq[-3], a_rhs[2]), TransformFromCopy(a_eq[-1], a_rhs[6]))
        self.wait()
        self.play(Write(a_rhs[-5]), TransformFromCopy(*a_rhs.get_parts_by_tex('{L}')), TransformFromCopy(a_rhs[3:4], a_rhs[-3:]))
        self.wait()
        self.equations = VGroup(a_eq, v_eq, x_eq, v_rhs, a_rhs)
    ...
    def show_theta_double_dot_equation(self):
        ...
        g_sin_eq = OldTex('-', 'g', '\\sin', '(', '\\theta', ')', **self.tex_config)
        g_sin_eq.next_to(equals, UP, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        g_sin_eq.to_edge(LEFT)
        g_sin_eq.shift(shift_vect)
        shift_vect += (g_sin_eq[1].get_center() - a_deriv[0].get_center())[0] * RIGHT
        equals.shift(shift_vect)
        a_rhs.shift(shift_vect)
        self.play(equations.shift, shift_vect, Write(equals), GrowFromPoint(g_sin_eq, 2 * RIGHT + 3 * DOWN))
        self.wait()
        self.play(a_rhs.next_to, g_sin_eq, RIGHT, a_rhs.shift, SMALL_BUFF * UP)
        self.wait()
        self.play(FadeOut(equals), equations.shift, DOWN, equations.fade, 0.5)
        ...
        self.play(L_dd_theta.next_to, equals, LEFT, L_dd_theta.shift, SMALL_BUFF * UP, g_sin_eq.next_to, equals, RIGHT, path_arc=PI / 2)
        self.play(ReplacementTransform(g, g2), ReplacementTransform(minus, m2), ReplacementTransform(L, L2), Write(over), g_sin_eq[2:].next_to, over, RIGHT, SMALL_BUFF)
        self.wait()
        rect = SurroundingRectangle(VGroup(g_sin_eq, frac, ddot))
        rect.stretch(1.1, 0)
        dashed_rect = DashedVMobject(rect, num_dashes=50, positive_space_ratio=1)
        dashed_rect.shuffle()
        ...
        self.wait()
        self.main_equation = VGroup(ddot, theta, equals, m2, L2, over, g2, sin, lp, theta2, rp)
    ...
    def talk_about_sine_component(self):
        ...
        sin.save_state()
        self.play(morty.change, 'angry', sin.next_to, morty, LEFT, {'aligned_edge': UP})
        self.play(Blink(morty))
        morty.clear_updaters()
        self.play(morty.change, 'concerned_musician', morty.look, DR)
        ...
        self.play(FadeIn(word, UP), GrowArrow(arrow))
        self.play(ShowCreationThenDestruction(theta.copy().set_style(fill_opacity=0, stroke_width=2, stroke_color=YELLOW), lag_ratio=0.1))
    ...
    def add_air_resistance(self):
        ...
        self.play(FadeInFromDown(new_term), tdd_eq.next_to, new_term, LEFT, tdd_eq.align_to, tdd_eq, UP, rhs.next_to, new_term, RIGHT, rhs.align_to, rhs, UP)
...
class SimpleDampenedPendulum(Scene):
    ...
    def construct(self):
        pendulum = Pendulum(top_point=ORIGIN, initial_theta=150 * DEGREES, mu=0.5)
        self.add(pendulum)
        pendulum.start_swinging()
        self.wait(20)
