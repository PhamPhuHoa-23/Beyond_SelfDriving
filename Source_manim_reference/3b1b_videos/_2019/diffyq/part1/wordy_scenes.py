...
class SmallAngleApproximationTex(Scene):
    ...
    def construct(self):
        approx = OldTex('\\sin', '(', '\\theta', ') \\approx \\theta', tex_to_color_map={'\\theta': RED}, arg_separator='')
        ...
        approx_words = OldTexText('For small $\\theta$', tex_to_color_map={'$\\theta$': RED})
...
class StrogatzQuote(Scene):
    ...
    def construct(self):
        ...
        for mover in movers:
            ...
            disc.move_to(mover)
            mover.become(disc)
        self.play(FadeIn(quote.author_part, LEFT), LaggedStartMap(Restore, movers, lag_ratio=0.005, run_time=2))
        self.wait()
        self.play(Write(quote.law_part.copy().set_color(YELLOW)), run_time=1)
        self.wait()
        self.play(Write(quote.language_part.copy().set_color(BLUE)), run_time=1.5)
        self.wait(2)
...
class XEqLThetaToCorner(Scene):
    ...
    def construct(self):
        equation = OldTex('x = L\\theta', tex_to_color_map={'x': GREEN, '\\theta': BLUE})
...
class ReallyHardToSolve(Scene):
    ...
    def construct(self):
        words = OldTexText("They're", 'really\\\\', 'freaking', 'hard\\\\', 'to', 'solve\\\\')
        words.set_height(6)
        self.wait()
        for word in words:
...
class ReasonForSolution(Scene):
    ...
    def construct(self):
        ...
        arrows = VGroup(Arrow(eq_word.get_right(), s_word.get_left()), Arrow(s_word.get_right(), u_word.get_left()), Arrow(s_word.get_right(), c_word.get_left()))
        arrows.set_color(GREY_B)
        new_arrows = VGroup(Arrow(eq_word.get_corner(UR), u_word.get_left(), path_arc=-60 * DEGREES), Arrow(eq_word.get_corner(DR), c_word.get_left(), path_arc=60 * DEGREES))
        new_arrows.set_color(BLUE)
        t2c = {'{x}': BLUE, '{\\dot x}': RED}
        equation = OldTex('{\\dot x}(t) = k {x}(t)', tex_to_color_map=t2c)
        equation.next_to(eq_word, DOWN)
        solution = OldTex('{x}(t) = x_0 e^{kt}', tex_to_color_map=t2c)
        ...
        computation = OldTex('e^x = \\sum_{n=0}^\\infty \\frac{x^n}{n!}')
        computation.next_to(c_word, DOWN)
        first_examples = VGroup(equation, solution, graph, computation)
        ode = get_ode()
        ode.scale(0.75)
        second_examples = VGroup(ode, OldTex('???').set_color(GREY_B), ScreenRectangle(height=2, stroke_width=1))
        ...
        self.add(eq_word)
        self.add(equation)
        self.play(FadeIn(s_word, LEFT), GrowArrow(arrows[0]), TransformFromCopy(equation, solution))
        self.wait()
        ...
        self.wait()
        self.play(arrows.fade, 0.75, s_word.fade, 0.75, second_examples[1].fade, 0.75, ShowCreation(new_arrows[0]), FadeIn(second_examples[2]))
        self.play(ShowCreation(new_arrows[1]), Animation(second_examples))
        self.wait()
...
class GleickQuote(Scene):
    ...
    def construct(self):
        quote = OldTexText('``[Phase space is] one of the most\\\\', 'powerful inventions', "of modern science.''\\\\")
        ...
        gleick.to_edge(DOWN)
        quote.to_edge(UP)
        self.play(FadeIn(book, RIGHT), FadeIn(gleick, LEFT))
        self.wait()
...
class ShowSineValues(Scene):
    ...
    def construct(self):
        ...
        self.add(formula)
        self.play(angle_tracker.set_value, 0, run_time=3)
        self.wait()
        self.play(angle_tracker.set_value, 90 * DEGREES, run_time=3)
        self.wait()
...
class SetAsideSeekingSolution(Scene):
    ...
    def construct(self):
        ...
        q1.set_color(YELLOW)
        q2 = OldTex('\\text{What is }', '\\theta', '(t)', "\\text{'s personality?}", tex_to_color_map={'\\theta': BLUE}, arg_separator='')
        ...
        self.add(q1)
        self.wait()
        self.play(q1.scale, 0.3, q1.to_corner, UR, MED_SMALL_BUFF)
        self.play(FadeIn(q2, DOWN))
        self.play(eyes.blink, rate_func=lambda t: smooth(1 - t))
        ...
        self.wait()
        self.play(eyes.change_mode, 'confused', eyes.look_at, ode.get_left())
        self.play(eyes.blink, rate_func=squish_rate_func(there_and_back))
...
class ThreeBodySymbols(Scene):
    ...
    def init_coord_groups(self):
        ...
        velocities = VGroup(*[get_vector_symbol(*['p^{}_{}'.format(s, i) for s in 'xyz'], **kwargs) for i in range(1, 4)])
    ...
    def introduce_coord_groups(self):
        ...
        rect_groups = VGroup()
        for group in groups:
            rect_group = VGroup(*[SurroundingRectangle(VGroup(*[tm.coords[i] for tm in group]), color=WHITE, stroke_width=2) for i in range(3)])
            rect_groups.add(rect_group)
        self.play(*[LaggedStartMap(FadeInFrom, group, lambda m: (m, UP), run_time=1) for group in groups], *map(FadeInFromDown, words))
        for rect_group in rect_groups:
            self.play(ShowCreationThenFadeOut(rect_group, lag_ratio=0.5))
        self.wait()
    ...
    def count_coords(self):
        ...
        count_group.arrange(RIGHT, aligned_edge=DOWN)
        count_group.scale(1.5)
        count_group.next_to(self.coord_groups, RIGHT, aligned_edge=DOWN)
        ...
        self.play(ShowIncreasingSubsets(coord_copies), run_time=1.5, rate_func=linear)
...
class ThreeBodyEquation(Scene):
    ...
    def construct(self):
        ...
        x2 = '\\vec{\\textbf{x}}_2'
        x3 = '\\vec{\\textbf{x}}_3'
        kw = {'tex_to_color_map': {x1: RED, x2: GREEN, x3: BLUE}}
        equations = VGroup(*[OldTex('{d^2', t1, '\\over dt^2}', '=', 'G', '\\left({' + m2, '(', t2, '-', t1, ')\\over||', t2, '-', t1, '||^3}', '+', '{' + m3, '(', t3, '-', t1, ')\\over||', t3, '-', t1, '||^3}', '\\right)', **kw) for t1, t2, t3, m1, m2, m3 in [(x1, x2, x3, 'm_1', 'm_2', 'm_3'), (x2, x3, x1, 'm_2', 'm_3', 'm_1'), (x3, x1, x2, 'm_3', 'm_1', 'm_2')]])
        equations.arrange(DOWN, buff=LARGE_BUFF)
        self.play(LaggedStartMap(FadeInFrom, equations, lambda m: (m, UP), lag_ratio=0.2))
        self.wait()
...
class JumpToThisPoint(Scene):
    ...
    def construct(self):
        ...
        words = OldTexText('Jump directly to\\\\', 'this point?')
        words.add_background_rectangle_to_submobjects()
        words.next_to(arrow.get_start(), UP, SMALL_BUFF)
        self.play(FadeInFromLarge(dot, 20), rate_func=rush_into)
        self.play(GrowArrow(arrow), FadeInFromDown(words))
