...
class GeneralizeToComplexFunctions(Scene):
    CONFIG = {'axes_config': {'x_min': 0, 'x_max': 10, 'x_axis_config': {'stroke_width': 2}, 'y_min': -2.5, 'y_max': 2.5, 'y_axis_config': {'tick_frequency': 0.25, 'unit_size': 1.5, 'include_tip': False, 'stroke_width': 2}}, 'complex_plane_config': {'axis_config': {'unit_size': 2}}}
    ...
    def show_cosine_wave(self):
        ...
        self.add(h_line)
        self.wait(2)
        self.play(FadeIn(real_words, RIGHT), FadeIn(real_arrow))
        self.wait(5)
    ...
    def add_rotating_vectors_making_cos(self):
        ...
        formula = OldTex('2\\cos(x) =', 'e^{ix}', '+', 'e^{-ix}', tex_to_color_map={'e^{ix}': v1.get_color(), 'e^{-ix}': v2.get_color()})
...
class ClarifyInputAndOutput(GeneralizeToComplexFunctions):
    CONFIG = {'input_space_rect_config': {'stroke_color': WHITE, 'stroke_width': 1, 'fill_color': GREY_E, 'fill_opacity': 1, 'width': 6, 'height': 2}}
    ...
    def setup_input_space(self):
        ...
        input_words.next_to(rect.get_bottom(), UP, SMALL_BUFF)
        self.add(rect)
        self.add(input_line)
    ...
    def describe_input(self):
        ...
        self.play(FadeInFromLarge(self.input_dot), FadeIn(self.input_decimal))
        for value in (1, 0):
    ...
    def describe_output(self):
        ...
        self.play(FadeOut(real_line), FadeOut(real_words))
        self.play(FadeIn(plane.sublabel, UP))
        self.play(FadeIn(output_decimal), TransformFromCopy(input_dot, output_dot))
        kw = {'run_time': 10, 'rate_func': lambda t: smooth(t, 1)}
        self.play(ApplyMethod(input_tracker.set_value, 1, **kw), ShowCreation(path.copy(), remover=True, **kw))
        self.add(path)
        self.add(output_dot)
        self.wait()
        ...
        self.play(FadeIn(real_function_word), MoveToTarget(path))
...
class GraphForFlattenedPi(ClarifyInputAndOutput):
    ...
    def construct(self):
        ...
        path = self.get_path()
        axes = Axes(x_min=0, x_max=1, x_axis_config={'unit_size': 7, 'include_tip': False, 'tick_frequency': 0.1}, y_min=-1.5, y_max=1.5, y_axis_config={'include_tip': False, 'unit_size': 2.5, 'tick_frequency': 0.5})
        axes.set_width(FRAME_WIDTH - 1)
        axes.set_height(FRAME_HEIGHT - 1, stretch=True)
        axes.center()
        axes.x_axis.add_numbers(0.5, 1.0, num_decimal_places=1)
        ...
        v_line = always_redraw(lambda: Line(axes.x_axis.n2p(axes.x_axis.p2n(graph.get_end())), graph.get_end(), stroke_width=1))
        self.add(axes)
        self.add(v_line)
        kw = {'run_time': 10, 'rate_func': lambda t: smooth(t, 1)}
...
class SimpleComplexExponentExample(ClarifyInputAndOutput):
    CONFIG = {'input_space_rect_config': {'width': 14, 'height': 1.5}, 'input_line_config': {'unit_size': 0.5, 'x_min': 0, 'x_max': 25, 'stroke_width': 2}, 'input_numbers': range(0, 30, 5), 'input_tex_args': ['t', '=']}
    ...
    def construct(self):
        ...
        self.play(self.input_tracker.set_value, time, run_time=time, rate_func=linear)
    ...
    def setup_input_trackers(self):
        ...
        f_always(input_tip.move_to, lambda: input_line.n2p(get_input()), lambda: DOWN)
        input_label = VGroup(OldTex(*self.input_tex_args), DecimalNumber())
    ...
    def setup_output_trackers(self):
        ...
        output_label, static_output_label = [OldTex('e^{i t}' + s, tex_to_color_map={'t': PINK}, background_stroke_width=3) for s in ['', '\\approx']]
        output_label.scale(1.2)
        output_label.add_updater(lambda m: m.shift(-m.get_bottom() + get_output_point() + rotate_vector(0.35 * RIGHT, get_input())))
        output_vector = Vector()
        output_vector.set_opacity(0.75)
        output_vector.add_updater(lambda m: m.put_start_and_end_on(plane.n2p(0), get_output_point()))
        t_max = 40
        ...
        output_path.add_updater(lambda m: m.pointwise_become_partial(full_output_path, 0, get_input() / t_max))
        ...
        output_decimal.add_updater(lambda m: m.next_to(static_output_label, RIGHT, 2 * SMALL_BUFF, aligned_edge=DOWN))
    ...
    def get_input_line(self, input_rect):
        ...
        input_line.move_to(input_rect)
        input_line.set_width(input_rect.get_width() - 1.5, stretch=True)
        input_line.add_numbers(*self.input_numbers)
        return input_line

class TRangingFrom0To1(SimpleComplexExponentExample):
    CONFIG = {'input_space_rect_config': {'width': 6, 'height': 2}}
