...
class StepFunctionExample(BringTwoRodsTogether, FourierSeriesIllustraiton):
    CONFIG = {'axes_config': {'y_min': -1.5, 'y_max': 1.5, 'y_axis_config': {'unit_size': 2.5, 'tick_frequency': 0.5}, 'x_min': 0, 'x_max': 1, 'x_axis_config': {'unit_size': 8, 'tick_frequency': 0.1, 'include_tip': False}}, 'y_labels': [-1, 1], 'graph_x_min': 0, 'graph_x_max': 1, 'midpoint': 0.5, 'min_temp': -1, 'max_temp': 1, 'alpha': 0.25, 'step_size': 0.01, 'n_range': range(1, 41, 2)}
    ...
    def bring_rods_together(self):
        rods = VGroup(self.get_rod(0, 0.5), self.get_rod(0.5, 1))
        rods.add_updater(self.update_rods)
        arrows = VGroup(Vector(RIGHT).next_to(rods[0], UP), Vector(LEFT).next_to(rods[1], UP))
        words = VGroup(OldTexText('Hot').next_to(rods[0], DOWN), OldTexText('Cold').next_to(rods[1], DOWN))
        for pair in (rods, words):
            pair.save_state()
            pair.space_out_submobjects(1.2)
        black_rects = VGroup(*[Square(side_length=1, fill_color=BLACK, fill_opacity=1, stroke_width=0).move_to(self.axes.c2p(0, u)) for u in [1, -1]])
        ...
        self.add(self.axes, self.graph, self.clock)
        self.add(rods, words)
        self.add(black_rects)
        kw = {'run_time': 2, 'rate_func': rush_into}
    ...
    def let_evolve_for_a_bit(self):
        ...
        self.play(self.get_clock_anim(-4, run_time=1, rate_func=smooth), graph.restore, time_label.set_value, 0)
        rods.clear_updaters()
        self.wait()
    ...
    def add_labels(self):
        ...
        x_numbers = x_axis.get_number_mobjects(*np.arange(0.2, 1.2, 0.2), num_decimal_places=1)
        ...
        self.play(LaggedStartMap(FadeInFrom, x_numbers, lambda m: (m, UP)), self.rods.set_opacity, 0.8)
        self.wait()
    ...
    def compare_to_sine_wave(self):
        ...
        sine_wave = always_redraw(lambda: self.axes.get_graph(lambda x: get_A() * np.sin(get_k() * x - get_phi()), x_min=self.graph_x_min, x_max=self.graph_x_max).color_using_background_image('VerticalTempGradient'))
    ...
    def sum_of_sine_waves(self):
        ...
        self.play(FadeOut(curr_sine_wave), FadeIn(curr_partial_sum), FadeOut(self.rods))
        ...
        for sine_graph, partial_sum in pairs:
            ...
            anims2 = [curr_partial_sum.set_stroke, {'width': 1, 'opacity': 0.25}, curr_partial_sum.set_stroke, {'width': 0, 'background': True}, ReplacementTransform(sine_graph, partial_sum, remover=True)]
            self.play(*anims1)
            self.play(*anims2)
...
class BreakDownStepFunction(StepFunctionExample):
    CONFIG = {'axes_config': {'x_axis_config': {'stroke_width': 2}, 'y_axis_config': {'tick_frequency': 0.25, 'stroke_width': 2}, 'y_min': -1.25, 'y_max': 1.25}, 'alpha': 0.1, 'wait_time': 30}
    ...
    def setup_axes(self):
        ...
        self.scale_factor = fdiv(mini_axes[0].get_width(), axes.get_width())
        dots = OldTex('\\vdots')
    ...
    def setup_graph(self):
        ...
        for axes, u, n in zip(mini_axes, it.cycle([1, -1]), it.count(1, 2)):
            mini_graph = axes.get_graph(lambda x: 4 / PI * (u / 1) * np.cos(PI * n * x))
