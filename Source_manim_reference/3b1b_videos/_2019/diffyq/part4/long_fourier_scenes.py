...
class FourierSeriesExampleWithRectForZoom(ComplexFourierSeriesExample):
    CONFIG = {'n_vectors': 100, 'slow_factor': 0.01, 'rect_scale_factor': 0.1, 'start_drawn': True, 'drawing_height': 7, 'rect_stroke_width': 1}
    ...
    def get_rect(self):
        return ScreenRectangle(color=BLUE, stroke_width=self.rect_stroke_width)

class ZoomedInFourierSeriesExample(FourierSeriesExampleWithRectForZoom, MovingCameraScene):
    CONFIG = {'vector_config': {'max_tip_length_to_length_ratio': 0.15, 'tip_length': 0.05}, 'parametric_function_step_size': 0.001}
...
class ZoomedInFourierSeriesExample100x(ZoomedInFourierSeriesExample):
    CONFIG = {'vector_config': {'max_tip_length_to_length_ratio': 0.15 * 0.4, 'tip_length': 0.05 * 0.2, 'max_stroke_width_to_length_ratio': 80, 'stroke_width': 3}, 'max_circle_stroke_width': 0.5, 'rect_scale_factor': 0.01}

    def get_rect_center(self):
        return self.vectors[-1].get_end()

class TrebleClefFourierSeriesExampleWithRectForZoom(FourierSeriesExampleWithRectForZoom):
    CONFIG = {'file_name': 'TrebleClef', 'drawn_path_stroke_width': 10}

class TrebleClefZoomedInFourierSeriesExample(ZoomedInFourierSeriesExample):
    CONFIG = {'file_name': 'TrebleClef'}

class NailAndGearFourierSeriesExampleWithRectForZoom(FourierSeriesExampleWithRectForZoom):
    CONFIG = {'file_name': 'Nail_And_Gear', 'n_vectors': 200, 'drawn_path_color': '#39FF14'}
...
class SigmaFourierSeriesExampleWithRectForZoom(FourierSeriesExampleWithRectForZoom):
    CONFIG = {'n_vectors': 200, 'drawn_path_color': PINK, 'rect_stroke_width': 0}
...
class FourierOfFourier(FourierSeriesExampleWithRectForZoom):
    CONFIG = {'file_name': 'FourierOneLine', 'n_vectors': 300, 'rect_stroke_width': 1}

class FourierOfFourierZoomedIn(ZoomedInFourierSeriesExample):
    CONFIG = {'file_name': 'FourierOneLine', 'max_circle_stroke_width': 0.3, 'n_vectors': 300}

class FourierOfFourier100xZoom(ZoomedInFourierSeriesExample100x):
    CONFIG = {'file_name': 'FourierOneLine', 'max_circle_stroke_width': 0.3, 'n_vectors': 300, 'slow_factor': 0.001}
...
class FourierOfHilbert(FourierSeriesExampleWithRectForZoom):
    CONFIG = {'n_vectors': 300, 'rect_stroke_width': 1, 'drawn_path_stroke_width': 4, 'drawn_path_color': BLUE}
...
class FourierOfBritain(FourierSeriesExampleWithRectForZoom):
    CONFIG = {'file_name': 'Britain', 'n_vectors': 500, 'drawn_path_color': RED}
...
class FourierOfSeattle(FourierSeriesExampleWithRectForZoom):
    CONFIG = {'file_name': 'SeattleSkyline', 'drawing_height': 7, 'n_vectors': 400, 'drawn_path_color': TEAL, 'drawn_path_stroke_width': 5}

class FourierOfSeattleZoomedIn(ZoomedInFourierSeriesExample):
    CONFIG = {'file_name': 'SeattleSkyline', 'drawing_height': 7, 'n_vectors': 400, 'drawn_path_color': TEAL, 'drawn_path_stroke_width': 5, 'max_circle_stroke_width': 0.3}
