...
class ComplexFourierSeriesExample(FourierOfTrebleClef):
    ...
    def highlight_vectors_one_by_one(self):
        ...
        for vector, circle, label in zip(self.vectors, self.circles, labels):
            ...
            self.play(circle.set_stroke, RED, 3, FadeIn(rect), *next_anims)
            self.wait()
            next_anims = [circle.set_stroke, c_color, c_stroke_width, FadeOut(rect)]
    ...
    def get_top_row(self, vectors, circles, max_freq=3):
        ...
        for vector, circle in zip(vectors, circles):
            ...
            vcopy.center_point = op.add(self.top_row_center, vector.freq * self.top_row_x_spacing * RIGHT)
        ...
        dots = VGroup(*[OldTex('\\dots').next_to(circle_copies, direction, MED_LARGE_BUFF) for direction in [LEFT, RIGHT]])
        labels = self.get_top_row_labels(vector_copies)
        return VGroup(vector_copies, circle_copies, dots, labels)
    ...
    def setup_plane(self):
        ...
        top_rect = Rectangle(width=FRAME_WIDTH, fill_color=BLACK, fill_opacity=1, stroke_width=0, height=self.top_rect_height)
    ...
    def get_path_end(self, vectors, stroke_width=None, **kwargs):
        ...
        def update_path(p):
            ...
            p.pointwise_become_partial(full_path, np.clip(alpha - 0.01, 0, 1), np.clip(alpha, 0, 1))
    ...
    def get_drawn_path(self, vectors, stroke_width=2, **kwargs):
        ...
        return VGroup(odp, self.get_path_end(vectors, stroke_width, **kwargs))
...
class PiFourierSeries(ComplexFourierSeriesExample):
    CONFIG = {'tex': '\\pi', 'n_vectors': 101, 'path_height': 3.5, 'max_circle_stroke_width': 1, 'top_row_copy_scale_factor': 0.6}
...
class RealValuedFunctionFourierSeries(PiFourierSeries):
    CONFIG = {'n_vectors': 101, 'start_drawn': True}
    ...
    def focus_on_vector_pair(self):
        ...
        rects1, rects2, rects3 = [VGroup(*[SurroundingRectangle(VGroup(top_circles[i], labels[i])) for i in pair]).set_stroke(GREY_B, 2) for pair in [(1, 2), (3, 4), (5, 6)]]

        def get_opacity_animation(i1, i2, alpha_func):
            v_group = vectors[i1:i2]
            c_group = circles[i1:i2]
            return AnimationGroup(UpdateFromAlphaFunc(VectorizedPoint(), lambda m, a: v_group.set_opacity(alpha_func(a))), UpdateFromAlphaFunc(VectorizedPoint(), lambda m, a: c_group.set_stroke(opacity=alpha_func(a))))
        self.remove(self.path, self.drawn_path)
        self.play(get_opacity_animation(3, len(vectors), lambda a: smooth(1 - a)), ShowCreation(rects1, lag_ratio=0.3))
        traced_path2 = self.get_vertically_falling_tracing(vectors[2], GREEN)
        self.add(traced_path2)
        for n in range(3):
            self.run_one_cycle()
        self.play(get_opacity_animation(3, 5, smooth), get_opacity_animation(0, 3, lambda a: 1 - 0.75 * smooth(a)), ReplacementTransform(rects1, rects2))
        traced_path2.set_stroke(width=1)
        traced_path4 = self.get_vertically_falling_tracing(vectors[4], YELLOW)
        self.add(traced_path4)
        self.run_one_cycle()
        self.play(get_opacity_animation(5, 7, smooth), get_opacity_animation(3, 5, lambda a: 1 - 0.75 * smooth(a)), ReplacementTransform(rects2, rects3))
        traced_path2.set_stroke(width=1)
...
class DemonstrateAddingArrows(PiFourierSeries):
    CONFIG = {'tex': '\\leftarrow', 'n_arrows': 21, 'parametric_function_step_size': 0.1}
    ...
    def construct(self):
        ...
        vectors = VGroup(*[Vector(**self.vector_config).put_start_and_end_on(*v.get_start_and_end()) for v in original_vectors])
        original_top_vectors = self.top_row[0]
        top_vectors = VGroup(*[Vector(**self.vector_config).put_start_and_end_on(*v.get_start_and_end()) for v in original_top_vectors])
        ...
        self.play(LaggedStart(*anims1), run_time=3, lag_ratio=0.2)
        self.play(LaggedStart(*anims2), lag_ratio=0.1, run_time=5)

class LabelRotatingVectors(PiFourierSeries):
    CONFIG = {'n_vectors': 6, 'center_point': 1.5 * DOWN, 'top_rect_height': 3, 'plane_config': {'axis_config': {'unit_size': 1.75, 'stroke_color': GREY_B}}, 'top_row_x_spacing': 1.9, 'top_row_center': 3 * UP + 0.2 * LEFT}
    ...
    def ask_about_labels(self):
        ...
        self.play(LaggedStartMap(FadeInFrom, q_marks, lambda m: (m, UP), lag_ratio=0.2, run_time=3))
        self.wait(3)
    ...
    def initialize_at_one(self):
        ...
        self.play(vector_clock.set_value, 0, run_time=2)
        zero_vect = Vector()
        ...
        self.play(zero_vect.put_start_and_end_on, plane.n2p(0), plane.n2p(1))
        vector_clock.add_updater(vc_updater)
        self.wait()
        self.play(FadeOut(q_marks[0], UP), FadeIn(one_label, DOWN))
        self.wait(4)
    ...
    def show_complex_exponents(self):
        ...
        v1_rect = SurroundingRectangle(VGroup(circles[1], q_marks[1], labels[1]), stroke_color=GREY, stroke_width=2)
        ...
        vg1 = self.get_vector_in_plane_group(vectors[1], circles[1])
        vg1_copy = vg1.copy()
        ...
        arclen_decimal = DecimalNumber(num_decimal_places=3, show_ellipsis=True)
        arclen_tracker = ValueTracker(0)
        arclen_decimal.add_updater(lambda m: m.next_to(circle_copy.get_end(), UR, SMALL_BUFF))
        ...
        self.wait(2)
        self.play(FadeOut(q_marks[1], UP), FadeIn(f1_exp, DOWN))
        self.wait(2)
        self.play(ShowCreationThenFadeAround(f1_exp.get_part_by_tex('2\\pi')))
        (self.add(arclen_decimal),)
        self.play(ShowCreation(circle_copy), arclen_tracker.set_value, TAU, run_time=3)
        self.wait()
        self.play(FadeOut(circle_copy), FadeOut(arclen_decimal))
        self.wait(8)
        self.play(v1_rect.move_to, circles[2], v1_rect.match_y, v1_rect, FadeOut(vg1), FadeOut(cps_1))
        vgm1 = self.get_vector_in_plane_group(vectors[2], circles[2])
        vgm1_copy = vgm1.copy()
        ...
        self.wait(2)
        self.play(FadeOut(q_marks[2], UP), FadeInFromDown(fm1_exp), v1_rect.stretch, 1.4, 0)
        self.wait(5)
        self.play(v1_rect.move_to, circles[3], v1_rect.match_y, v1_rect, FadeOut(vgm1), FadeOut(cps_m1))
        vg2 = self.get_vector_in_plane_group(vectors[3], circles[3])
        vg2_copy = vg2.copy()
        ...
        self.wait()
        self.play(FadeOut(q_marks[3], UP), FadeInFromDown(f2_exp))
        self.wait(3)
        ...
        self.wait(3)
        self.play(FadeOut(circle_copy), FadeOut(arclen_decimal))
        self.play(FadeOut(vg2), FadeOut(cps_2), FadeOut(v1_rect))
        ...
        self.play(FadeOut(q_marks[4:]), FadeOut(f1_exp), FadeIn(f1_exp_new), FadeInFromDown(fm2_exp), FadeInFromDown(f3_exp), FadeIn(v_lines, lag_ratio=0.2))
        self.play(FadeIn(f_exp_general, UP))
        self.play(ShowCreationThenFadeAround(f_exp_general))
        self.wait(3)
        self.play(FadeOut(one_label, UP), TransformFromCopy(f_exp_general, f0_exp))
        self.wait(5)
        self.f_exp_labels = VGroup(f0_exp, f1_exp_new, fm1_exp, f2_exp, fm2_exp, f3_exp)
        self.f_exp_general = f_exp_general

    def show_complex_exponents_temp(self):
        self.f_exp_labels = VGroup(*[self.get_exp_tex(n).move_to(qm, DOWN) for n, qm in zip([0, 1, -1, 2, -2, 3], self.q_marks)])
    ...
    def tweak_initial_states(self):
        ...
        new_coefs = [0.5, np.exp(complex(0, TAU / 8)), 0.7 * np.exp(-complex(0, TAU / 8)), 0.6 * np.exp(complex(0, TAU / 3)), 1.1 * np.exp(-complex(0, TAU / 12)), 0.3 * np.exp(complex(0, TAU / 12))]
        ...
        self.play(LaggedStartMap(MoveToTarget, VGroup(f_exp_general, *f_exp_labels)), LaggedStartMap(FadeInFromDown, VGroup(cn_general, *cn_terms)), UpdateFromAlphaFunc(VectorizedPoint(), lambda m, a: update_vectors(a)), run_time=2)
        self.wait()
        self.play(LaggedStart(*[ShowCreationThenFadeAround(cn_term, surrounding_rectangle_config={'buff': 0.05, 'stroke_width': 2}) for cn_term in cn_terms]))
    ...
    def constant_examples(self):
        ...
        self.play(vip_group0_copy.scale, 0.5, GrowFromCenter(c0_brace), GrowFromCenter(c0_label))
        self.wait(2)
        self.play(FadeOut(c0_brace), FadeOut(c0_label), FadeOut(vip_group0_copy))
        c1_brace = Brace(cn_terms[1], DOWN, buff=SMALL_BUFF)
        c1_label = OldTex('e^{(\\pi / 4)i}')
        c1_label.next_to(c1_brace, DOWN, SMALL_BUFF)
        c1_decimal = DecimalNumber(np.exp(np.complex(0, PI / 4)), num_decimal_places=3)
        ...
        scalar.next_to(c1_label, LEFT, SMALL_BUFF, aligned_edge=DOWN)
        ...
        self.play(Rotate(vip_group1_copy, PI / 4))
        self.play(GrowFromCenter(c1_brace), FadeIn(c1_label))
        self.play(Write(approx), Write(c1_decimal), run_time=1)
        self.wait(2)

        def update_v1(alpha):
            vectors[1].coefficient = 0.5 * interpolate(np.exp(complex(0, PI / 4)), 0.3 * np.exp(complex(0, PI / 4)), alpha)
        self.play(FadeIn(scalar), c1_decimal.set_value, scalar.get_value() * c1_decimal.get_value(), vip_group1_copy.scale, scalar.get_value(), UpdateFromAlphaFunc(VMobject(), lambda m, a: update_v1(a)))
        self.wait()
        self.play(FadeOut(c1_brace), FadeOut(c1_label), FadeOut(approx), FadeOut(c1_decimal), FadeOut(scalar), FadeOut(vip_group1_copy))
        fade_anims = []
    ...
    def get_vector_in_plane_group(self, top_vector, top_circle):
        ...
        origin = plane.n2p(0)
        vector = Vector()
        vector.add_updater(lambda v: v.put_start_and_end_on(origin, plane.n2p(2 * top_vector.coefficient)).set_angle(top_vector.get_angle()))
        circle = Circle()
    ...
    def get_exp_tex(self, freq=None):
        ...
        result = OldTex('e^{', freq_str, '2\\pi i {t}}', tex_to_color_map={'2\\pi': WHITE, '{t}': PINK, freq_str: YELLOW})
        result.scale(0.9)
        return result
    ...
    def get_cps_label(self, n):
        ...
        result = OldTex(n_str, frac_tex, tex_to_color_map={n_str: YELLOW})
...
class IntegralTrick(LabelRotatingVectors, TRangingFrom0To1):
    ...
    def setup_input_space(self):
        ...
        self.input_line.next_to(self.input_rect.get_bottom(), UP)
        group = VGroup(self.input_rect, self.input_line)
        group.move_to(self.plane.n2p(0))
        group.to_edge(LEFT)

    def setup_top_row(self):
        top_row = self.get_top_row(self.vectors, self.circles, max_freq=2)
    ...
    def setup_sum(self):
        ...
        for t1, t2 in zip(o_terms, o_terms[1:]):
            plus = OldTex('+')
            plus.scale(0.7)
            plus.move_to(midpoint(t1.get_right(), t2.get_left()))
            plusses.add(plus)
    ...
    def introduce_sum(self):
        self.remove(self.vector_clock, self.vectors, self.circles, self.drawn_path)
        ft = self.ft_eq[0]
        terms = self.terms
        path = self.path
        ...
        self.play(ShowCreation(path), input_tracker.set_value, 1, run_time=3, rate_func=lambda t: smooth(t, 1))
        self.wait()
        input_tracker.add_updater(lambda m: m.set_value(self.vector_clock.get_value() % 1))
        self.add(self.vector_clock, self.vectors, self.circles)
        self.play(FadeOut(path), FadeOut(dot), FadeIn(self.drawn_path))
    ...
    def issolate_c0(self):
        ...
        v0.add_updater(lambda v: v.put_start_and_end_on(origin, v0_point.get_location()))
        self.play(MaintainPositionRelativeTo(path, v0_point), ApplyMethod(v0_point.shift, 1.5 * LEFT, run_time=4, rate_func=there_and_back, path_arc=60 * DEGREES))
        v0.updaters.pop()
        self.opacity_tracker = opacity_tracker

    def show_center_of_mass(self):
        dot_sets = VGroup(*[self.get_sample_dots(dt=dt, radius=radius) for dt, radius in [(0.05, 0.04), (0.01, 0.03), (0.0025, 0.02)]])
        ...
        self.play(LaggedStartMap(FadeInFromLarge, input_dots, lambda m: (m, 5), run_time=2, lag_ratio=0.5))
        self.wait()
        ...
        for ds1, ds2 in zip(dot_sets, dot_sets[1:]):
            ...
            self.play(FadeOut(ind1), LaggedStartMap(FadeInFrom, ind2, lambda m: (m, UP), lag_ratio=4 / len(ind2), run_time=2))
            self.play(TransformFromCopy(ind2, outd2), FadeOut(outd1), run_time=2)
            self.play(FadeOut(v0_dot), *[Transform(od.copy(), v0_dot.copy(), remover=True) for od in outd2])
            v0_dot = new_v0_dot
            self.add(v0_dot)
        self.wait()
    ...
    def write_integral(self):
        ...
        path = self.path
        expression = OldTex('c_{0}', '=\\int_0^1 f({t}) d{t}', tex_to_color_map={'{t}': PINK, '{0}': YELLOW})
        ...
        self.play(FadeInFromDown(expression), GrowFromCenter(brace), FadeIn(average))
        t_tracker.clear_updaters()
        t_tracker.set_value(0)
        self.add(path)
        self.play(t_tracker.set_value, 0.999, ShowCreation(path), run_time=8, rate_func=lambda t: smooth(t, 1))
        self.wait()
    ...
    def get_sample_dots(self, dt, radius):
        ...
        dot.set_stroke(RED, 1, opacity=0.8, background=True)
        input_dots = VGroup()
        output_dots = VGroup()
        for t in t_values:
            in_dot = dot.copy()
...
class IncreaseOrderOfApproximation(ComplexFourierSeriesExample):
    CONFIG = {'file_name': 'FourierOneLine', 'drawing_height': 6, 'n_vectors': 250, 'parametric_function_step_size': 0.001, 'run_time': 10, 'slow_factor': 0.05}
    ...
    def construct(self):
        ...
        n_label = VGroup(OldTexText('Approximation using'), Integer(100).set_color(YELLOW), OldTexText('vectors'))
        ...
        self.play(n_tracker.set_value, self.n_vectors, rate_func=smooth, run_time=self.run_time)
        self.wait(5)

class ShowStepFunctionIn2dView(SimpleComplexExponentExample, ComplexFourierSeriesExample):
    CONFIG = {'input_space_rect_config': {'width': 5, 'height': 2}, 'input_line_config': {'unit_size': 3, 'x_min': 0, 'x_max': 1, 'tick_frequency': 0.1, 'stroke_width': 2, 'decimal_number_config': {'num_decimal_places': 1}}, 'input_numbers': [0, 0.5, 1], 'input_tex_args': [], 'n_vectors': 2}
    ...
    def transition_from_step_function(self):
        ...
        y_axis = NumberLine(unit_size=2, x_min=-1.5, x_max=1.5, tick_frequency=0.5, stroke_color=GREY_B, stroke_width=2)
        y_axis.rotate(90 * DEGREES)
        y_axis.shift(x_axis.n2p(0) - y_axis.n2p(0))
        y_axis.add_numbers(-1, 0, 1, direction=LEFT)
        axes = Axes()
        axes.x_axis = x_axis
        axes.y_axis = y_axis
        axes.axes = VGroup(x_axis, y_axis)
        graph = VGroup(Line(axes.c2p(0, 1), axes.c2p(0.5, 1), color=RED), Line(axes.c2p(0.5, -1), axes.c2p(1, -1), color=BLUE))
        dot1 = Dot(color=RED)
        dot2 = Dot(color=BLUE)
        ...
        self.play(self.input_tracker.set_value, 1, ShowCreation(graph), run_time=3, rate_func=lambda t: smooth(t, 1))
        self.wait()
        self.add(plane, input_rect, input_space_label, x_axis, input_tip, input_label)
        self.play(FadeIn(input_rect), FadeIn(input_space_label), Restore(x_axis))
        ...
        plane.set_opacity(1)
        self.play(MoveToTarget(y_axis), ShowCreation(plane))
    ...
    def show_output(self):
        ...
        tip.add_updater(lambda m: m.move_to(get_output_point(), DOWN))
        output_label = OldTexText('Output')
        output_label.add_background_rectangle()
        output_label.add_updater(lambda m: m.next_to(tip, UP, SMALL_BUFF))
        self.play(FadeIn(tip), FadeIn(output_label))
    ...
    def show_fourier_series(self):
        ...
        self.play(plane.axes.set_stroke, WHITE, 1, plane.background_lines.set_stroke, GREY_B, 0.5, plane.faded_lines.set_stroke, GREY_B, 0.25, 0.5)
        self.vector_clock.set_value(0)
        ...
        self.run_one_cycle()
        path = self.get_vertically_falling_tracing(self.vectors[1], GREEN, rate=0.5)
        self.add(path)
        for x in range(3):
            self.run_one_cycle()
    ...
    def get_freqs(self):
        n = self.n_vectors
        all_freqs = [*range(1, n + 1 // 2, 2), *range(-1, -n + 1 // 2, -2)]
...
class AddVectorsOneByOne(IntegralTrick):
    CONFIG = {'file_name': 'TrebleClef', 'n_vectors': 101, 'path_height': 5}
...
class DE4Thumbnail(ComplexFourierSeriesExample):
    CONFIG = {'file_name': 'FourierOneLine', 'start_drawn': True, 'n_vectors': 300, 'parametric_function_step_size': 0.0025, 'drawn_path_stroke_width': 7, 'drawing_height': 6}
