...
class FourierSeriesFormula(Scene):
    ...
    def construct(self):
        formula = OldTex('c_{n} = \\int_0^1 e^{-2\\pi i {n} {t}}f({t}){dt}', tex_to_color_map={'{n}': YELLOW, '{t}': PINK})
...
class RelationToOtherVideos(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeInFrom, videos, lambda m: (m, LEFT), lag_ratio=0.4, run_time=2), GrowFromCenter(brace), FadeInFromDown(text))
        self.wait()
        ...
        bubble = ThoughtBubble(direction=RIGHT, width=3, height=2, fill_opacity=0.5, stroke_color=WHITE)
        ...
        self.play(fourier.match_x, new_brace, fourier.to_edge, UP, MoveToTarget(first_two), Transform(brace, new_brace), text.scale, 0.7, text.next_to, new_brace, UP, FadeOut(bubble, LEFT))
        self.play(videos[2].scale, 1.7, videos[2].to_corner, UR)
        self.wait()

    def get_video_thumbnails(self):
        thumbnails = Group(ImageMobject('diffyq_part2_thumbnail'), ImageMobject('diffyq_part3_thumbnail'), ImageMobject('diffyq_part4_thumbnail'))
...
class FourierGainsImmortality(Scene):
    CONFIG = {'mathematicians': ['Pythagoras', 'Euclid', 'Archimedes', 'Fermat', 'Newton', 'Leibniz', 'Johann_Bernoulli2', 'Euler', 'Joseph Fourier', 'Gauss', 'Riemann', 'Cantor', 'Noether', 'Ramanujan', 'Godel', 'Turing']}
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, immortals, lag_ratio=0.1, run_time=2))
    ...
    def get_immortals(self):
        ...
        for image, name in zip(images, self.mathematicians):
            setattr(result, name.split(' ')[-1].lower(), image)
        return result

class WhichWavesAreAvailable(Scene):
    CONFIG = {'axes_config': {'x_min': 0, 'x_max': TAU, 'y_min': -1.5, 'y_max': 1.5, 'x_axis_config': {'tick_frequency': PI / 2, 'unit_size': 1, 'include_tip': False}, 'y_axis_config': {'unit_size': 1, 'tick_frequency': 0.5, 'include_tip': False}}, 'n_axes': 5, 'trig_func': np.cos, 'trig_func_tex': '\\cos', 'bc_words': 'Restricted by\\\\boundary conditions'}
    ...
    def show_cosine_waves(self):
        ...
        for k, axes, color in zip(it.count(1), axes_group, colors):
            ...
            graphs.add(graph)
            func_label = OldTex(self.trig_func_tex + '\\left(', str(k), '(\\pi / L)x\\right)', tex_to_color_map={str(k): color})
...
class AlternateBoundaryConditions(WhichWavesAreAvailable):
    CONFIG = {'trig_func': np.sin, 'trig_func_tex': '\\sin', 'bc_words': 'Alternate\\\\boundary condition'}

class AskQuestionOfGraph(PiCreatureScene):

    def construct(self):
        randy = self.pi_creature
        self.pi_creature_says(randy, 'What sine waves\\\\are you made of?', target_mode='sassy')
        self.wait(2)
...
class CommentOnFouriersImmortality(FourierGainsImmortality):
    ...
    def construct(self):
        ...
        self.add(immortals)
        immortals.remove(immortals.fourier)
        self.play(fourier.set_height, 6, fourier.next_to, name, UP, FadeInFromDown(name))
        self.play(FadeOut(immortals))
        self.wait(2)
        self.play(LaggedStartMap(FadeInFrom, fourier_things, lambda m: (m, UP), run_time=5, lag_ratio=0.3))
        self.wait()

class ShowInfiniteSum(FourierSeriesIllustraiton):
    CONFIG = {'n_range': range(1, 101, 2), 'colors': [BLUE, GREEN, RED, YELLOW, PINK], 'axes_config': {'y_max': 1.5, 'y_min': -1.5, 'y_axis_config': {'tick_frequency': 0.5, 'unit_size': 1.0, 'stroke_width': 2}, 'x_axis_config': {'tick_frequency': 0.1, 'stroke_width': 2}}}
    ...
    def walk_through_constants(self):
        ...
        self.play(FadeOut(partial_sums[1:]), FadeIn(curr_rect), FadeIn(curr_partial_sum))
        for sg, ps, rect in zip(sine_graphs[1:], partial_sums[1:], rects[1:]):
            self.play(FadeOut(curr_rect), FadeIn(rect), FadeIn(sg))
            curr_rect.become(rect)
            self.remove(rect)
            self.add(curr_rect)
            ps.set_stroke(width=3)
            self.play(curr_partial_sum.set_stroke, {'width': 0.5}, ReplacementTransform(sg, ps))
            curr_partial_sum = ps
        self.play(Transform(curr_rect, dots_rect), curr_partial_sum.set_stroke, {'width': 0.5}, LaggedStart(*[UpdateFromAlphaFunc(ps, lambda m, a: m.set_stroke(width=3 * there_and_back(a) + 0.5 * a)) for ps in partial_sums[4:]], run_time=3, lag_ratio=0.2))
        self.play(partial_sums[-1].set_stroke, {'width': 2})
        self.play(FadeOut(curr_rect), ShowCreationThenFadeAround(inf_sum.four_over_pi))
        self.wait()
    ...
    def show_inf_sum_of_numbers(self):
        ...
        number_line = NumberLine(x_min=0, x_max=1, tick_frequency=0.1, big_tick_numbers=[0, 0.5, 1], unit_size=8)
        ...
        num_inf_sum = OldTex('{1 \\over 1}', '-{1 \\over 3}', '+{1 \\over 5}', '-{1 \\over 7}', '+{1 \\over 9}', '-{1 \\over 11}', '+\\cdots', '= {\\pi \\over 4}')
        num_inf_sum.move_to(UP)
        self.play(FadeOut(graph_group, DOWN), FadeIn(number_line, UP), FadeOut(self.inf_words), *[TransformFromCopy(t1[-1:], t2) for t1, t2 in zip(inf_sum.terms, num_inf_sum)], TransformFromCopy(inf_sum.dots, num_inf_sum[-2]), TransformFromCopy(inf_sum.dots, num_inf_sum[-4:-2]), self.equation.set_opacity, 0.5)
        ...
        tip.add_updater(lambda t: t.move_to(number_line.n2p(get_value()), DOWN))
        n_braces = 7
        ...
        pi_fourths_tip.move_to(number_line.n2p(PI / 4), UP)
        ...
        pi_fourths_label.next_to(pi_fourths_tip, DOWN, MED_SMALL_BUFF, aligned_edge=LEFT)
        self.play(LaggedStartMap(FadeIn, VGroup(brace, tip, decimal, term_count)), FadeIn(pi_fourths_tip), FadeIn(pi_fourths_label))
        for ps, new_brace in zip(partial_sums[1:], braces[1:]):
            term_count_tracker.increment_value(1)
            self.play(Transform(brace, new_brace), value_tracker.set_value, ps)
        ...
        term_count_tracker.increment_value(1)
        self.play(UpdateFromAlphaFunc(value_tracker, lambda m, a: m.set_value(partial_sums[integer_interpolate(num_moving_values, len(partial_sums) - 1, a)[0]])), ShowIncreasingSubsets(new_marks), term_count_tracker.set_value, len(partial_sums), run_time=10, rate_func=smooth)
        self.play(LaggedStartMap(FadeOut, VGroup(num_inf_sum, brace, decimal, term_count, tip, number_line, new_marks, pi_fourths_tip, pi_fourths_label), lag_ratio=0.3))
        self.play(FadeInFromDown(graph_group), self.equation.set_opacity, 1)
    ...
    def show_many_inputs_in_parallel(self):
        ...
        n_iterations = 100
        values = np.array([[4 / PI * (u / n) * np.cos(n * PI * x) for x in inputs] for u, n in zip(it.cycle([1, -1]), range(1, 2 * n_iterations + 1, 2))])
        ...
        self.play(FadeOut(partial_sums), FadeIn(dots), FadeIn(lines))
        self.play(n_tracker.set_value, len(p_sums) - 1, run_time=5)
        dots.clear_updaters()
        index = 4
    ...
    def follow_single_inputs(self):
        ...
        brace_value_labels = VGroup(*[get_brace_value_label(brace, u, n) for brace, u, n in zip(braces, it.cycle([1, -1]), it.count(1, 2))])
        bv_rects = VGroup(*[SurroundingRectangle(brace_value_labels[:n], color=BLUE, stroke_width=1) for n in range(1, len(braces) + 1)])
        bv_rect = bv_rects[0].copy()
        partial_sum = partial_sums[0].copy()
        dot.add_updater(lambda d: d.move_to(partial_sum.point_from_proportion(inverse_interpolate(x_axis.x_min, x_axis.x_max, get_input()))))
        ...
        self.play(FadeIn(input_label, LEFT), dot.scale, 1.5)
        self.wait()
        self.play(LaggedStartMap(GrowFromCenter, braces), LaggedStartMap(GrowFromCenter, brace_value_labels), *[TransformFromCopy(input_label[0][0], term[n][1]) for i, term in enumerate(inf_sum.terms) for n in [2 if i == 0 else 4]])
        self.wait()
        self.add(partial_sum, dot)
        dot.set_stroke(BLACK, 1, background=True)
        self.play(FadeOut(input_label), FadeIn(n_waves_label), FadeIn(bv_rect), FadeIn(partial_sum))
        self.wait()
        ps_iter = iter(partial_sums[1:])

        def get_ps_anim():
            return AnimationGroup(Transform(partial_sum, next(ps_iter)), ApplyMethod(n_tracker.increment_value, 1))
        for i in range(1, 4):
            self.play(Transform(bv_rect, bv_rects[i]), get_ps_anim())
            self.wait()
        self.play(FadeOut(bv_rect), get_ps_anim())
        for x in range(3):
            self.play(get_ps_anim())
        self.play(input_tracker.set_value, 0.7)
        for x in range(6):
            self.play(get_ps_anim())
        self.play(input_tracker.set_value, 0.5)
        for x in range(40):
            self.play(get_ps_anim(), run_time=0.5)
    ...
    def get_infinite_sum(self):
        colors = self.colors
        inf_sum = OldTex('{4 \\over \\pi}', '\\left(', '{\\cos\\left({1}\\pi x\\right) \\over {1}}', '-', '{\\cos\\left({3}\\pi x\\right) \\over {3}}', '+', '{\\cos\\left({5}\\pi x\\right) \\over {5}}', '-', '{\\cos\\left({7}\\pi x\\right) \\over {7}}', '+', '\\cdots', '\\right)', tex_to_color_map={'{1}': colors[0], '{-1 \\over 3}': colors[1], '{3}': colors[1], '{1 \\over 5}': colors[2], '{5}': colors[2], '{-1 \\over 7}': colors[3], '{7}': colors[3], '{1 \\over 9}': colors[4], '{9}': colors[4]})
        ...
        inf_sum.terms = VGroup(inf_sum[2:6], inf_sum[6:12], inf_sum[12:18], inf_sum[18:24])
    ...
    def get_step_func_definition(self):
        ...
        conditions = VGroup(*map(TexText, ['if $x < 0.5$', 'if $x = 0.5$', 'if $x > 0.5$']))
...
class StepFunctionSolutionFormla(WriteHeatEquationTemplate):
    CONFIG = {'tex_mobject_config': {'tex_to_color_map': {'{1}': WHITE, '{3}': WHITE, '{5}': WHITE, '{7}': WHITE}}}

    def construct(self):
        formula = OldTex('T({x}, {t}) = ', '{4 \\over \\pi}', '\\left(', '{\\cos\\left({1}\\pi {x}\\right) \\over {1}}', 'e^{-\\alpha {1}^2 {t} }', '-{\\cos\\left({3}\\pi {x}\\right) \\over {3}}', 'e^{-\\alpha {3}^2 {t} }', '+', '{\\cos\\left({5}\\pi {x}\\right) \\over {5}}', 'e^{-\\alpha {5}^2 {t} }', '- \\cdots', '\\right)', **self.tex_mobject_config)
...
class TechnicalNuances(Scene):
    ...
    def construct(self):
        ...
        questions = VGroup(*map(TexText, ['Does the value at $0.5$ matter?', 'What does it mean to solve a PDE with\\\\a discontinuous initial condition?', 'Is the limit of a sequence of solutions\\\\also a solution?', 'Do all functions have a Fourier series?']))
        self.play(Write(title), ShowCreation(line))
        title.add(line)
...
class SineWaveResidue(Scene):
    ...
    def construct(self):
        f = 2
        wave = FunctionGraph(lambda x: np.cos(-0.1 * f * TAU * x), x_min=0, x_max=20, color=YELLOW)
        wave.next_to(LEFT_SIDE, LEFT, buff=0)
        time = 10
        self.play(wave.shift, time * RIGHT, run_time=f * time, rate_func=linear)
...
class AskAboutComplexNotVector(Scene):
    ...
    def construct(self):
        ...
        i_part = c_ex[-1]
        v_ex = IntegerMatrix([[2], [1]], bracket_h_buff=SMALL_BUFF, bracket_v_buff=SMALL_BUFF)
        group = VGroup(v_ex, c_ex)
        group.scale(1.5)
        group.arrange(DOWN, buff=0.9)
        group.to_corner(UL, buff=0.2)
        q1, q2 = questions = VGroup(OldTexText('Why not this?').set_color(BLUE), OldTexText('Instead of this?').set_color(YELLOW))
        questions.scale(1.5)
        ...
        vect = Vector([4, 2, 0])
        self.play(ShowCreation(plane, lag_ratio=0.1), FadeIn(vect))
        for q, ex in zip(questions, group):
...
class SwapIntegralAndSum(Scene):
    ...
    def perform_swap(self):
        ...
        tex_config = self.tex_config = {'tex_to_color_map': {'=': WHITE, '\\int_0^1': WHITE, '+': WHITE, '\\cdots': WHITE, '{t}': PINK, '{dt}': light_pink, '{-2}': YELLOW, '{\\cdot 1}': YELLOW, '{0}': YELLOW, '{1}': YELLOW, '{2}': YELLOW, '{n}': YELLOW}}
        ...
        aos_label = OldTexText('Average of a sum', tex_to_color_map={'Average': light_pink, 'sum': YELLOW})
        soa_label = OldTexText('Sum over averages', tex_to_color_map={'averages': light_pink, 'Sum': YELLOW})
        aos_label.next_to(ORIGIN, RIGHT, buff=2)
        aos_label.to_edge(UP)
        soa_label.move_to(2.5 * DOWN)
        aos_arrow = Arrow(aos_label.get_corner(DL), int_sum.get_corner(TOP) + 2 * RIGHT, buff=0.3)
        soa_arrow = Arrow(soa_label.get_top(), sum_int.get_bottom(), buff=0.3)
        self.add(int_ft)
        self.play(FadeIn(breakdown_label, LEFT), GrowArrow(arrow))
        self.wait()
        self.play(FadeOut(breakdown_label), FadeOut(arrow), FadeIn(int_sum.get_parts_by_tex('=')), FadeIn(int_sum.get_parts_by_tex('+')), FadeIn(int_sum.get_parts_by_tex('\\cdots')), FadeIn(int_sum.get_parts_by_tex('(')), FadeIn(int_sum.get_parts_by_tex(')')), *[TransformFromCopy(int_ft.get_part_by_tex(tex), int_sum.get_part_by_tex(tex)) for tex in ['\\int_0^1', '{dt}']])
        self.play(LaggedStart(*[GrowFromPoint(term, int_ft.get_part_by_tex('{t}').get_center()) for term in int_sum.exp_terms]))
        self.wait()
        self.play(FadeIn(aos_label), GrowArrow(aos_arrow))
        self.play(*[TransformFromCopy(int_sum.get_parts_by_tex(tex), sum_int.get_parts_by_tex(tex), run_time=2) for tex in ['\\cdots', '+']], TransformFromCopy(int_sum.exp_terms, sum_int.exp_terms, run_time=2), FadeIn(soa_label), GrowArrow(soa_arrow))
        self.play(*[TransformFromCopy(int_sum.get_part_by_tex('\\int_0^1'), part, run_time=2) for part in sum_int.get_parts_by_tex('\\int_0^1')], FadeIn(sum_int.get_parts_by_tex('{dt}')))
        self.wait()
        self.play(FadeOut(soa_arrow), soa_label.to_edge, DOWN)
    ...
    def ask_about_c2(self):
        ...
        self.play(ShowCreation(c2_rect), FadeOut(self.aos_arrow), FadeInFromDown(q_marks))
    ...
    def multiply_f_by_exp_neg_2(self):
        ...
        new_exp = OldTex('e^{{-2} \\cdot 2\\pi i {t}}', **self.tex_config)
        ...
        self.play(FadeOut(eq_c0), Write(something, run_time=1), dt_part.next_to, new_exp, RIGHT, SMALL_BUFF, dt_part.align_to, dt_part, DOWN)
        self.wait()
        self.play(*[Rotating(diagram[2], radians=n * TAU, about_point=diagram[0].n2p(0), run_time=3, rate_func=lambda t: smooth(t, 1)) for n, diagram in zip(it.count(-3), diagrams)])
        self.wait()
        self.play(FadeOut(something, UP), FadeIn(new_exp, DOWN))
        ...
        quads = zip(it.count(-3), int_sum.exp_terms, sum_int.exp_terms, diagrams)
        for n, c_exp1, c_exp2, diagram in quads:
            ...
            if n < 0:
                ...
                n_str = '{' + str(n) + '}'
            replacement1 = OldTex('e^{', n_str, '\\cdot 2\\pi i', '{t}', tex_to_color_map={'{t}': PINK, n_str: YELLOW})
            ...
            self.play(TransformFromCopy(moving_exp[1], replacement1[1]), FadeOut(exp1[1], DOWN), Transform(exp1[0], replacement1[0]), Transform(exp1[2:], replacement1[2:]))
            self.play(TransformFromCopy(replacement1, replacement2), FadeOut(exp2, DOWN), FadeOut(diagram))
    ...
    def show_individual_average(self, brace, n, cn_tex=None):
        ...
        result.next_to(brace, DOWN, SMALL_BUFF)
        coord_max = 1.25
        plane = ComplexPlane(x_min=-coord_max, x_max=coord_max, y_min=-coord_max, y_max=coord_max, axis_config={'stroke_color': GREY_B, 'stroke_width': 1, 'unit_size': 0.75}, background_line_style={'stroke_color': GREY_B, 'stroke_width': 1})
        plane.next_to(result, DOWN, SMALL_BUFF)
        vector = Arrow(plane.n2p(0), plane.n2p((0.2 + 0.8 * np.random.random()) * np.exp(complex(0, TAU * np.random.random()))), buff=0, color=WHITE)
        circle = Circle()
        circle.set_stroke(BLUE, 2)
        circle.rotate(vector.get_angle())
        circle.set_width(2 * vector.get_length())
        circle.move_to(plane.n2p(0))
        dots = VGroup(*[Dot(circle.point_from_proportion(n * a % 1), radius=0.04, color=PINK, fill_opacity=0.75) for a in np.arange(0, 1, 1 / 25)])
        dot_center = center_of_mass([d.get_center() for d in dots])
        self.play(FadeIn(plane), FadeIn(circle), FadeIn(vector))
        self.play(Rotating(vector, radians=n * TAU, about_point=plane.n2p(0)), ShowIncreasingSubsets(dots, int_func=np.ceil), rate_func=lambda t: smooth(t, 1), run_time=3)
        ...
        self.wait()
        self.all_average_diagrams.add(VGroup(plane, circle, vector, dots, dot_copies, result))

    def fix_minuses(self, tex_mob):
        for mob in tex_mob.get_parts_by_tex('{\\cdot'):
            minus = OldTex('-')
            minus.match_style(mob[0])
            minus.set_width(3 * mob[0].get_width(), stretch=True)
...
class ShowRangeOfCnFormulas(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeInFrom, formulas, lambda m: (m, UP), lag_ratio=0.2, run_time=4))
        self.wait()
    ...
    def get_formula(self, n):
        ...
        n_str = '{' + str(n) + '}'
        neg_n_str = '{' + str(-n) + '}'
        expression = OldTex('c_' + n_str, '=', '\\int_0^1 f({t})', 'e^{', neg_n_str, '\\cdot 2\\pi i {t}}{dt}', tex_to_color_map={'{t}': light_pink, '{dt}': light_pink, n_str: YELLOW, neg_n_str: YELLOW})
        return expression
...
class DescribeSVG(Scene):
    ...
    def construct(self):
        ...
        self.play(UpdateFromAlphaFunc(dot, lambda d, a: d.move_to(path.point_from_proportion(a)), run_time=5, rate_func=lambda t: 0.3 * there_and_back(t)))
        self.wait()
        self.play(FadeIn(answer, UP))
        self.play(FadeOut(path), FadeOut(dot), FadeIn(path_parts))
        ...
        self.play(ShowCreation(indicators, lag_ratio=0.5, run_time=3))
        self.wait()
        self.play(FadeOut(indicators), path_parts.restore)
    ...
    def get_bezier_point_indicators(self, path):
        ...
        for part in path.family_members_with_points():
            for tup in part.get_cubic_bezier_tuples():
                a1, h1, h2, a2 = tup
                dots.add(Dot(a1), Dot(a2), Dot(h2, color=YELLOW), Dot(h1, color=YELLOW))
                lines.add(Line(a1, h1), Line(a2, h2))
        for dot in dots:
            dot.set_width(0.05)
...
class NumericalIntegration(Scene):
    CONFIG = {'tex_config': {'tex_to_color_map': {'{t}': LIGHT_PINK, '{dt}': LIGHT_PINK, '{n}': YELLOW, '{0.01}': WHITE}}}
    ...
    def add_integral(self):
        integral = OldTex('c_{n}', '=', '\\int_0^1 f({t})e^{-{n} \\cdot 2\\pi i {t}}{dt}', **self.tex_config)
        integral.to_edge(LEFT)
        sum_tex = OldTex(' \\approx \\text{sum([} \\dots', 'f({0.01}) e^{-{n} \\cdot 2\\pi i ({0.01})}({0.01})\\dots \\text{])}', **self.tex_config)
        sum_tex.next_to(integral.get_part_by_tex('='), DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
        ...
        dots = VGroup(*[Dot(input_line.n2p(t), color=PINK, radius=0.03).stretch(2, 1) for t in np.arange(0, 1, 0.01)])
        self.add(integral)
        self.add(sum_tex)
        self.add(decimals)
        ...
        self.add(input_line)
        time = 15
        self.play(t_tracker.set_value, 0.99, ShowIncreasingSubsets(dots), run_time=time, rate_func=lambda t: smooth(t, 1))
        self.wait()
...
class StepFunctionIntegral(Scene):
    ...
    def construct(self):
        light_pink = interpolate_color(PINK, WHITE, 0.25)
        tex_config = {'tex_to_color_map': {'{t}': light_pink, '{dt}': light_pink, '{n}': YELLOW}}
        ...
        group.set_width(FRAME_WIDTH - 1)
        group.to_corner(UL)
        words1 = OldTex('\\text{Challenge 1: Show that }c_{n} = {2 \\over {n} \\pi i}', '\\text{ for odd }', '{n}', '\\text{ and 0 otherwise}', **tex_config)
        words2 = OldTex('\\text{Challenge 2: }', '&\\text{Using }', '\\sin(x) = (e^{ix} - e^{-ix}) / 2i,', '\\text{ show that}\\\\&\\text{step}({t}) = \\sum_{n = -\\infty}^{\\infty}', 'c_{n} e^{{n} \\cdot 2\\pi i {t}}', '=', '\\sum_{n = 1, 3, 5, \\dots}', '{4 \\over {n} \\pi}', '\\sin\\left({n} \\cdot 2\\pi {t}\\right)', **tex_config)
        words3 = OldTexText('Challenge 3: How can you turn this into an expansion with \\\\ \\phantom{Challenge 3:} cosines? (Hint: Draw the sine waves over $[0.25, 0.75]$)', alignment='')
...
class GeneralChallenge(Scene):
    ...
    def construct(self):
        ...
        light_pink = interpolate_color(PINK, WHITE, 0.25)
        tex_config = {'tex_to_color_map': {'{t}': light_pink, '{dt}': light_pink, '{n}': YELLOW}}
        words1 = OldTexText('In some texts, for a function $f$ on the input range $[0, 1]$,\\\\', 'its Fourier series is presented like this:', alignment='')
        formula1 = OldTex('f(t) = {a_{0} \\over 2} + \\sum_{n = 1}^{\\infty} \\big(a_{n} \\cos\\left({n} \\cdot 2\\pi {t}\\right) + b_{n} \\sin\\left({n} \\cdot 2\\pi {t}\\right) \\big)', **tex_config)
        formula1[0][6].set_color(YELLOW)
        words2 = OldTexText('where')
        formula2 = OldTex('a_{n} = 2\\int_0^1 f({t})\\cos\\left({n} \\cdot 2\\pi {t}\\right) {dt}\\\\b_{n} = 2\\int_0^1 f({t})\\sin\\left({n} \\cdot 2\\pi {t}\\right) {dt}', **tex_config)
...
class HintToGeneralChallenge(Scene):
    ...
    def construct(self):
        ...
        formula1 = OldTex('c_{n} =', '{', 'a_{n} \\over 2} +', '{', 'b_{n} \\over 2} i', tex_to_color_map={'{n}': YELLOW})
        words2 = OldTexText('and')
        formula2 = OldTex('e^{i{x}} =', '\\cos\\left({x}\\right) +', 'i\\sin\\left({x}\\right)', tex_to_color_map={'{x}': GREEN})
...
class OtherResources(Scene):
    ...
    def construct(self):
        thumbnails = Group(ImageMobject('MathologerFourier'), ImageMobject('CodingTrainFourier'))
        names = VGroup(OldTexText('Mathologer'), OldTexText('The Coding Train'))
        ...
        url.to_edge(UP)
        self.play(thumbnails.arrange, DOWN, {'buff': LARGE_BUFF}, thumbnails.set_height, FRAME_HEIGHT - 2, thumbnails.to_edge, LEFT)
...
class ExponentialsMoreBroadly(Scene):
    ...
    def write_fourier_series(self):
        formula = OldTex('f({t}) = \\sum_{n = -\\infty}^\\infty', 'c_{n}', 'e^{{n} \\cdot 2\\pi i {t}}', tex_to_color_map={'{t}': LIGHT_PINK, '{n}': YELLOW})
    ...
    def pull_out_complex_exponent(self):
        ...
        self.play(c_exp.set_opacity, 1, c_exp.scale, 1.5, c_exp.next_to, dot, UR, SMALL_BUFF, GrowFromCenter(dot))
    ...
    def show_matrix_exponent(self):
        ...
        circle = self.circle
        dot = self.dot
        m_exp = OldTex('\\text{exp}\\left(\\left[ \\begin{array}{cc}0 & -1 \\\\ 1 & 0 \\end{array} \\right]', '{t} \\right)', '=', '\\left[ \\begin{array}{cc}\\cos(t) & -\\sin(t) \\\\ \\sin(t) & \\cos(t)\\end{array} \\right]')
        VGroup(m_exp[1][0], m_exp[3][5], m_exp[3][12], m_exp[3][18], m_exp[3][24]).set_color(LIGHT_PINK)
        m_exp.to_corner(UL)
        m_exp.add_background_rectangle_to_submobjects()
        vector_field = VectorField(lambda p: rotate_vector(p, TAU / 4), max_magnitude=7, delta_x=0.5, delta_y=0.5, length_func=lambda norm: 0.35 * sigmoid(norm))
