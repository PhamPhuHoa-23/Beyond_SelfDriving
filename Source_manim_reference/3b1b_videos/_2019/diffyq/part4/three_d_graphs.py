...
class ShowLinearity(WriteHeatEquationTemplate, TemperatureGraphScene):
    CONFIG = {'temp_text': 'Temp', 'alpha': 0.1, 'axes_config': {'z_max': 2, 'z_min': -2, 'z_axis_config': {'tick_frequency': 0.5, 'unit_size': 1.5}}, 'default_surface_config': {'resolution': (16, 16)}, 'freqs': [2, 5]}
    ...
    def add_three_graphs(self):
        ...
        freqs = self.freqs
        scalar_trackers = Group(ValueTracker(1), ValueTracker(1))
        graphs = VGroup(self.get_graph(axes0, [freqs[0]], [scalar_trackers[0]]), self.get_graph(axes1, [freqs[1]], [scalar_trackers[1]]), self.get_graph(axes2, freqs, scalar_trackers))
        plus = OldTex('+').scale(2)
        equals = OldTex('=').scale(2)
        plus.move_to(midpoint(axes0.get_right(), axes1.get_left()))
        equals.move_to(midpoint(axes1.get_right(), axes2.get_left()))
        self.add(axes_group)
        self.add(graphs)
        self.add(plus)
        self.add(equals)
    ...
    def add_function_labels(self):
        ...
        kw = {'tex_to_color_map': {'T_1': BLUE, 'T_2': GREEN}}
        T1 = OldTex('a', 'T_1', **kw)
        T2 = OldTex('b', 'T_2', **kw)
        ...
        self.play(FadeIn(T1[1], DOWN), FadeIn(solution_labels[0], UP))
        self.play(FadeIn(T2[1], DOWN), FadeIn(solution_labels[1], UP))
        self.wait()
        self.play(TransformFromCopy(T1[1], T_sum[0]), TransformFromCopy(T2[1], T_sum[2]), TransformFromCopy(self.plus, T_sum[1]), *[Transform(graph.copy().set_fill(opacity=0), graphs[2].copy().set_fill(opacity=0), remover=True) for graph in graphs[:2]])
        self.wait()
        self.play(FadeIn(solution_labels[2], UP))
        self.wait()
        self.play(FadeIn(T1[0]), FadeIn(T2[0]), FadeIn(a_brace), FadeIn(b_brace), FadeIn(s1_decimal), FadeIn(s2_decimal), FadeOut(T_sum), FadeIn(T_sum_with_scalars))
    ...
    def get_graph(self, axes, freqs, scalar_trackers):
        ...
        def func(x, t):
            ...
            return np.sum([s * np.cos(k * x) * np.exp(-a * k ** 2 * t) for freq, s in zip(freqs, scalars) for k in [freq * PI / L]])

        def get_surface_graph_group():
            return VGroup(self.get_surface(axes, func), self.get_time_slice_graph(axes, func, t=0))
...
class CombineSeveralSolutions(ShowLinearity):
    CONFIG = {'default_surface_config': {'resolution': (16, 16)}, 'n_top_graphs': 5, 'axes_config': {'y_max': 15}, 'target_scalars': [0.81, -0.53, 0.41, 0.62, -0.95], 'final_run_time': 14}
    ...
    def add_all_axes(self):
        top_axes_group = VGroup(*[self.get_axes(z_min=-1.25, z_max=1.25, z_axis_config={'unit_size': 2, 'tick_frequency': 0.5}) for x in range(self.n_top_graphs)])
    ...
    def setup_all_graphs(self):
        ...
        freqs += 1
        self.top_graphs = VGroup(*[self.get_graph(axes, [n], [st]) for axes, n, st in zip(self.top_axes_group, freqs, scalar_trackers)])
    ...
    def show_infinite_family(self):
        ...
        self.play(LaggedStart(*[AnimationGroup(Write(graph[0]), FadeIn(graph[1])) for graph in top_graphs]))
        self.wait()
        self.play(FadeIn(decimals))
        for graph in top_graphs:
            graph.resume_updating()
        self.play(LaggedStart(*[ApplyMethod(st.set_value, value) for st, value in zip(scalar_trackers, self.target_scalars)]), run_time=3)
        self.wait()
    ...
    def show_sum(self):
        ...
        low_graph.update()
        self.play(LaggedStart(*[Transform(top_graph.copy().set_fill(opacity=0), low_graph.copy().set_fill(opacity=0), remover=True) for top_graph in top_graphs]), FadeIn(low_graph, rate_func=squish_rate_func(smooth, 0.7, 1)), run_time=3)
        self.wait()
    ...
    def show_time_passing(self):
        ...
        self.play(time_tracker.increment_value, run_time, run_time=run_time, rate_func=linear)
...
class CycleThroughManyLinearCombinations(CombineSeveralSolutions):
    CONFIG = {'default_surface_config': {'resolution': (16, 16)}, 'n_cycles': 10}
    ...
    def cycle_through_superpositions(self):
        ...
        for x in range(self.n_cycles):
            self.play(LaggedStart(*[ApplyMethod(st.set_value, value) for st, value in zip(scalar_trackers, 3 * np.random.random(nst) - 1.5)]), run_time=3)
            self.wait()