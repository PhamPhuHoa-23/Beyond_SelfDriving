...
class FourierNameIntro(Scene):
    ...
    def show_two_titles(self):
        ...
        lt_variants = VGroup(OldTexText('Complex', 'Fourier Series'), OldTexText('Discrete', 'Fourier Series'))
        rt_variants = VGroup(OldTexText('Discrete', 'Fourier Transform'), OldTexText('Fast', 'Fourier Transform'), OldTexText('Quantum', 'Fourier Transform'))
        titles = VGroup(lt, rt)
        titles.scale(1.5)
        ...
        for title, variants in [(rt, rt_variants)]:
            ...
            for v1, v2 in zip(variants, variants[1:]):
                self.play(FadeOut(v1[0], UP), FadeIn(v2[0], DOWN), run_time=0.5)
                self.wait(0.5)
    ...
    def transition_to_image(self):
        ...
        self.play(ReplacementTransform(v_line, frame), FadeIn(image), FadeIn(name[0]), *[ReplacementTransform(title[0].deepcopy(), name[1]) for title in titles], titles.scale, 0.65, titles.arrange, DOWN, titles.next_to, image, UP)
        self.wait()
        big_F = F_sym.copy()
        big_F.set_fill(opacity=0)
        big_F.set_stroke(WHITE, 2)
        big_F.set_height(3)
        big_F.move_to(midpoint(image.get_right(), RIGHT_SIDE))
        big_F.shift(DOWN)
        equivalence = VGroup(fourier_part.copy().scale(1.25), OldTex('\\Leftrightarrow').scale(1.5), OldTexText('Break down into\\\\pure frequencies'))
        equivalence.arrange(RIGHT)
        equivalence.move_to(big_F)
        equivalence.to_edge(UP)
        self.play(FadeIn(big_F), TransformFromCopy(fourier_part, equivalence[0]), Write(equivalence[1:]))
        self.wait(3)
    ...
    def show_paper(self):
        ...
        date_arrow = Arrow(date_rect.get_bottom(), date.get_top(), buff=SMALL_BUFF, color=date_rect.get_color())
        ...
        heat_arrow = Arrow(heat_rect.get_top(), heat_word.get_left(), buff=0.1, path_arc=-60 * DEGREES, color=heat_rect.get_color())
        self.play(FadeIn(paper, LEFT))
        self.play(ShowCreation(date_rect))
        self.play(GrowFromPoint(date, date_arrow.get_start()), ShowCreation(date_arrow))
        self.wait(3)
        self.play(ShowCreation(heat_rect))
        self.play(GrowFromPoint(heat_word, heat_arrow.get_start()), ShowCreation(heat_arrow))
        self.wait(3)

class ManyCousinsOfFourierThings(Scene):

    def construct(self):
        series_variants = VGroup(OldTexText('Complex', 'Fourier Series'), OldTexText('Discrete', 'Fourier Series'))
        transform_variants = VGroup(OldTexText('Discrete', 'Fourier Transform'), OldTexText('Fast', 'Fourier Transform'), OldTexText('Quantum', 'Fourier Transform'))
...
class FourierSeriesIllustraiton(Scene):
    CONFIG = {'n_range': range(1, 31, 2), 'axes_config': {'axis_config': {'include_tip': False}, 'x_axis_config': {'tick_frequency': 1 / 4, 'unit_size': 4}, 'x_min': 0, 'x_max': 1, 'y_min': -1, 'y_max': 1}, 'colors': [BLUE, GREEN, RED, YELLOW, PINK]}
    ...
    def construct(self):
        ...
        range_words.next_to(VGroup(sum_tex, target_func_tex), DOWN)
        rects = it.chain([SurroundingRectangle(piece) for piece in self.get_sum_tex_pieces(sum_tex)], it.cycle([None]))
        ...
        for sine_graph, partial_sum, rect in zip(sine_graphs, partial_sums, rects):
            ...
            anims2 = [curr_partial_sum.set_stroke, {'width': 1, 'opacity': 0.5}, curr_partial_sum.set_stroke, {'width': 0, 'background': True}, ReplacementTransform(sine_graph, partial_sum, remover=True)]
            if rect:
    ...
    def get_axes_arrow_axes(self):
        ...
        axes1.y_axis.add_numbers(-1, 1, direction=LEFT, num_decimal_places=1)
    ...
    def get_sine_graphs(self, axes):
        sine_graphs = VGroup(*[axes.get_graph(self.generate_nth_func(n)) for n in self.n_range])
    ...
    def get_partial_sums(self, axes, sine_graphs):
        partial_sums = VGroup(*[axes.get_graph(self.generate_kth_partial_sum_func(k + 1)) for k in range(len(self.n_range))])
        partial_sums.match_style(sine_graphs)
        return partial_sums

    def get_sum_tex(self):
        return OldTex('\\frac{4}{\\pi} \\left(', '\\frac{\\cos(\\pi x)}{1}', '-\\frac{\\cos(3\\pi x)}{3}', '+\\frac{\\cos(5\\pi x)}{5}', '- \\cdots \\right)').scale(0.75)
    ...
    def get_target_func_graph(self, axes):
        step_func = axes.get_graph(lambda x: 1 if x < 0.5 else -1, discontinuities=[0.5], color=YELLOW, stroke_width=3)
    ...
    def generate_nth_func(self, n):
        return lambda x: np.prod([4 / PI, 1 / n * (-1) ** ((n - 1) / 2), np.cos(PI * n * x)])

    def generate_kth_partial_sum_func(self, k):
        return lambda x: np.sum([self.generate_nth_func(n)(x) for n in self.n_range[:k]])

class FourierSeriesOfLineIllustration(FourierSeriesIllustraiton):
    CONFIG = {'n_range': range(1, 31, 2), 'axes_config': {'y_axis_config': {'unit_size': 2, 'tick_frequency': 0.25, 'big_tick_numbers': [-1, 1]}}}

    def get_sum_tex(self):
        return OldTex('\\frac{8}{\\pi^2} \\left(', '\\frac{\\cos(\\pi x)}{1^2}', '+\\frac{\\cos(3\\pi x)}{3^2}', '+\\frac{\\cos(5\\pi x)}{5^2}', '+ \\cdots \\right)').scale(0.75)
    ...
    def get_target_func_graph(self, axes):
        return axes.get_graph(lambda x: 1 - 2 * x, color=YELLOW, stroke_width=3)

    def generate_nth_func(self, n):
        return lambda x: np.prod([8 / PI ** 2, 1 / n ** 2, np.cos(PI * n * x)])

class CircleAnimationOfF(FourierOfTrebleClef):
    CONFIG = {'height': 3, 'n_circles': 200, 'run_time': 10, 'arrow_config': {'tip_length': 0.1, 'stroke_width': 2}}
...
class ExponentialDecay(PiCreatureScene):
    ...
    def construct(self):
        ...
        randy.to_edge(DOWN)
        bubble = ThoughtBubble(direction=LEFT, height=3.5, width=3)
        bubble.pin_to(randy)
        bubble.set_fill(GREY_E)
        exp = OldTex('Ce^{', mk_tex, 't}', tex_to_color_map=t2c)
        exp.move_to(bubble.get_bubble_center())
        axes = Axes(x_min=0, x_max=13, y_min=-4, y_max=4)
        ...
        axes.x_axis.add(OldTexText('Time').next_to(axes.x_axis.get_end(), DR))
        axes.y_axis.add(OldTex('f').next_to(axes.y_axis.get_corner(UR), RIGHT).set_color(YELLOW))
        axes.x_axis.set_opacity(0)
        y_tracker = ValueTracker(3)
        x_tracker = ValueTracker(0)
        dydt_tracker = ValueTracker()
        dxdt_tracker = ValueTracker(0)
        self.add(y_tracker, x_tracker, dydt_tracker, dxdt_tracker)
        ...
        decimal.add_updater(lambda d: d.next_to(tip, RIGHT, SMALL_BUFF))
        arrow = Vector(DOWN, color=RED, max_stroke_width_to_length_ratio=50, max_tip_length_to_length_ratio=0.2)
        ...
        ode = OldTex('{d{f} \\over dt}(t)', '=', mk_tex, '\\cdot {f}(t)', tex_to_color_map={'{f}': YELLOW, '=': WHITE, mk_tex: mk_tex_color})
        ode.to_edge(UP)
        dfdt = ode[:3]
        ft = ode[-2:]
        self.add(axes)
        ...
        self.wait(3)
        self.play(DrawBorderThenFill(bubble), Write(exp), FadeOut(rect), randy.change, 'thinking')
        axes.x_axis.set_opacity(1)
        self.play(y_tracker.set_value, 3, ShowCreation(axes.x_axis))
        ...
        self.play(MoveToTarget(exp), FadeOut(bubble), FadeIn(d_dt), FadeIn(eq))
        self.wait(2)
        self.play(ApplyMethod(exp[1].copy().replace, const[0]))
        self.wait()
...
class InvestmentGrowth(Scene):
    CONFIG = {'output_tex': '{M}', 'output_color': GREEN, 'initial_value': 1, 'initial_value_tex': '{M_0}', 'k': 0.05, 'k_tex': '0.05', 'total_time': 43, 'time_rate': 3}

    def construct(self):
        axes = Axes(x_min=0, x_max=self.total_time, y_min=0, y_max=6, x_axis_config={'unit_size': 0.3, 'tick_size': 0.05, 'big_tick_numbers': range(0, self.total_time, 5)})
        ...
        money_label.next_to(axes.y_axis.get_top(), UP)
        ...
        graph.add_updater(lambda m: m.pointwise_become_partial(full_graph, 0, np.clip(time_tracker.get_value() / self.total_time, 0, 1)))
        tex_kwargs = {'tex_to_color_map': {self.output_tex: self.output_color, self.initial_value_tex: BLUE}}
        ode = OldTex('{d', '\\over dt}', self.output_tex, '(t)', '=', self.k_tex, '\\cdot', self.output_tex, '(t)', **tex_kwargs)
        ode.to_edge(UP)
        exp = OldTex(self.output_tex, '(t) =', self.initial_value_tex, 'e^{', self.k_tex, 't}', **tex_kwargs)
        ...
        self.wait(2)
        self.play(Restore(exp_part), M0_part.set_opacity, 1)
...
class CarbonDecayCurve(InvestmentGrowth):
    CONFIG = {'output_tex': '{^{14}C}', 'output_color': GOLD, 'initial_value': 4, 'initial_value_tex': '{^{14}C_0}', 'k': -0.1, 'k_tex': '-k', 'time_rate': 6}
...
class CarbonDecayingInMammoth(Scene):
    ...
    def get_atom(self, body):
        ...
        wn = 0
        while wn == 0:
            point = np.array([interpolate(dl[0], ur[0], np.random.random()), interpolate(dl[1], ur[1], np.random.random()), 0])
            wn = get_winding_number([body.point_from_proportion(a) - point for a in np.linspace(0, 1, 300)])
            wn = int(np.round(wn))
        atom.move_to(point)
        return atom
...
class BoundaryConditionInterlude(Scene):
    ...
    def construct(self):
        ...
        bc_image = self.get_labeled_image('Boundary\\\\conditions', 'boundary_condition_thumbnail')
        bc_image.next_to(storyline[1], UP)
        new_arrow0 = Arrow(storyline.arrows[0].get_start() + v_shift, bc_image.get_left() + SMALL_BUFF * LEFT, path_arc=-90 * DEGREES, buff=0)
        new_mid_arrow = Arrow(bc_image.get_bottom(), storyline[1].get_top() + v_shift, buff=SMALL_BUFF, path_arc=60 * DEGREES)
        self.add(background)
        self.add(storyline[0])
        for im1, im2, arrow in zip(storyline, storyline[1:], storyline.arrows):
            self.add(im2, im1)
            self.play(FadeIn(im2, -im_to_im), ShowCreation(arrow))
        self.wait()
        self.play(GrowFromCenter(bc_image), MoveToTarget(storyline), Transform(storyline.arrows[0], new_arrow0), MaintainPositionRelativeTo(storyline.arrows[1], storyline))
        ...
        self.play(LaggedStart(Transform(rect1, rect2, **kw), lag_ratio=0.4))
        ...
        arrows = VGroup(storyline.arrows, new_mid_arrow)
        self.play(LaggedStart(MoveToTarget(l_group), MoveToTarget(r_group), lag_ratio=0.3), FadeOut(arrows))
        self.play(GrowFromCenter(brace), FadeIn(nv_text, RIGHT))
        self.wait()

    def get_main_storyline(self):
        images = Group(self.get_sine_curve_image(), self.get_linearity_image(), self.get_fourier_series_image())
        for image in images:
            image.set_height(3)
        ...
        for im1, im2 in zip(images, images[1:]):
            arrow = Arrow(im1.get_top(), im2.get_top(), color=WHITE, buff=MED_SMALL_BUFF, path_arc=-120 * DEGREES, rectangular_stem_width=0.025)
    ...
    def get_sine_curve_image(self):
        return self.get_labeled_image('Sine curves', 'sine_curve_temp_graph')

    def get_linearity_image(self):
        return self.get_labeled_image('Linearity', 'linearity_thumbnail')

    def get_fourier_series_image(self):
        return self.get_labeled_image('Fourier series', 'fourier_series_thumbnail')
...
class GiantCross(Scene):
    ...
    def construct(self):
        ...
        words.set_color(RED)
        words.to_edge(UP)
        self.play(FadeInFromDown(words), ShowCreation(cross))
        self.wait()
