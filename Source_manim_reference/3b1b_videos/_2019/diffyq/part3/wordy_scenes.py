...
class ThreeMainObservations(Scene):
    ...
    def construct(self):
        ...
        fourier.shift(LEFT)
        bubble = ThoughtBubble(direction=RIGHT, height=3, width=4)
        bubble.move_tip_to(fourier.get_corner(UL) + 0.5 * DR)
        observations = VGroup(OldTexText('1)', 'Sine = Nice'), OldTexText('2)', 'Linearity'), OldTexText('3)', 'Fourier series'))
        observations.arrange(DOWN, aligned_edge=LEFT, buff=2 * LARGE_BUFF)
        observations.set_height(FRAME_HEIGHT - 2)
        observations.to_corner(UL, buff=LARGE_BUFF)
        self.add(fourier)
        self.play(ShowCreation(bubble))
        self.wait()
        self.play(LaggedStart(*[TransformFromCopy(bubble, observation[0]) for observation in observations], lag_ratio=0.2))
        self.play(FadeOut(fourier), FadeOut(bubble))
        self.wait()
...
class LastChapterWrapper(Scene):
    ...
    def construct(self):
        full_rect = FullScreenFadeRectangle(fill_color=GREY_D, fill_opacity=1)
        ...
        self.add(full_rect)
        self.play(FadeIn(rect), Write(title, run_time=2))
        self.wait()
...
class ThreeConstraints(WriteHeatEquationTemplate):
    ...
    def show_three_conditions(self):
        ...
        title = OldTex('\\text{Constraints }T({x}, {t})\\text{ must satisfy:}', **self.tex_mobject_config)
        title.to_edge(UP)
        items = VGroup(OldTexText('1)', 'The PDE'), OldTexText('2)', 'Boundary condition'), OldTexText('3)', 'Initial condition'))
        items.scale(0.7)
        ...
        self.play(FadeInFromDown(title), FadeOut(to_remove, UP), equation.scale, 0.6, equation.next_to, items[0], DOWN, equation.shift_onto_screen, LaggedStartMap(FadeIn, [items[0], items[1][0], items[2][0]]))
        self.wait()
...
class SeekIdealized(Scene):
    ...
    def construct(self):
        phrases = VGroup(*[OldTexText('Seek', text, 'problems', tex_to_color_map={'realistic': GREEN, '{idealized}': YELLOW, 'over-idealized': YELLOW, 'general': BLUE}) for text in ['realistic', '{idealized}', 'over-idealized', 'general']])
        phrases.scale(2)
        words = VGroup()
        ...
        self.wait()
        words[0].save_state()
        self.play(words[0].to_edge, DOWN, words[0].set_opacity, 0.5, Transform(phrase, phrases[1]), FadeIn(words[1], UP))
        self.wait()
        self.play(words[1].next_to, arrow, UP, ShowCreation(arrow), MaintainPositionRelativeTo(phrase, words[1]), FadeIn(solutions, LEFT), FadeIn(words[3]))
        self.wait()
...
class SecondDerivativeOfSine(Scene):
    ...
    def construct(self):
        equation = OldTex('{d^2 \\over d{x}^2}', '\\cos\\left({x}\\right) =', '-\\cos\\left({x}\\right)', tex_to_color_map={'{x}': GREEN})
        self.add(equation)
...
class EquationAboveSineAnalysis(WriteHeatEquationTemplate):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeOut(t_terms, UP), FadeIn(zeros, DOWN))
        t_terms.fade(1)
        self.wait()
        self.play(FadeIn(new_rhs))
        self.wait()
        self.play(VGroup(lhs[6:], eq, rhs, new_rhs[0], new_rhs[-3:], zeros).fade, 0.5)
...
class ExpVideoWrapper(Scene):
    ...
    def construct(self):
        ...
        screen.center()
        self.play(FadeIn(screen, DOWN))
        self.wait()

class ShowSinExpDerivatives(WriteHeatEquationTemplate):
    CONFIG = {'tex_mobject_config': {'tex_to_color_map': {'{0}': WHITE, '\\partial': WHITE, '=': WHITE}}}

    def construct(self):
        pde = self.get_d1_equation_without_inputs()
        pde.to_edge(UP)
        pde.generate_target()
        new_rhs = OldTex('=- \\alpha \\cdot T', **self.tex_mobject_config)
        ...
        equation2 = OldTex('T({x}, {t}) = \\sin\\left({x}\\right)', 'e^{-\\alpha{t}}', **self.tex_mobject_config)
        ...
        self.wait()
        self.play(ReplacementTransform(equation1, eq2_part1), FadeIn(eq2_part2))
        ...
        ddx_label1 = OldTex('\\partial \\over \\partial {x}', **self.tex_mobject_config)
        ddx_label1.scale(0.7)
        ddx_label1.next_to(arrow1.point_from_proportion(0.8), UR, SMALL_BUFF)
        pde_ddx = VGroup(*pde.get_parts_by_tex('\\partial')[2:], pde.get_parts_by_tex('\\over')[1], pde.get_part_by_tex('{x}'))
        ...
        self.wait()
        self.play(LaggedStart(GrowArrow(arrow1), GrowArrow(arrow2)), TransformFromCopy(pde_ddx[0], ddx_label1), TransformFromCopy(pde_ddx[0], ddx_label2))
        self.wait()
        ...
        dt_label = OldTex('\\partial \\over \\partial {t}', **self.tex_mobject_config)
        dt_label.scale(1)
        dt_label.next_to(dt_arrow.get_center(), UL, SMALL_BUFF)
        rhs_copy = rhs.copy()
        ...
        dot = OldTex('\\cdot')
        dot.move_to(midpoint(minus_alpha.get_right(), rhs_copy.get_left()))
        self.play(TransformFromCopy(pde_ddx_rect, pde_ddt_rect))
        self.play(GrowArrow(dt_arrow), TransformFromCopy(pde_ddt, dt_label))
        self.wait()
        self.play(TransformFromCopy(rhs, rhs_copy))
        self.play(FadeIn(minus_alpha_in_exp))
        self.play(ApplyMethod(minus_alpha_in_exp.replace, minus_alpha, path_arc=TAU / 4), FadeIn(dot))
        self.play(FadeIn(minus_alpha), FadeOut(minus_alpha_in_exp))
        self.wait()
...
class DerivativesOfLinearFunction(WriteHeatEquationTemplate):
    CONFIG = {'tex_mobject_config': {'tex_to_color_map': {'{c}': WHITE}}}
    ...
    def construct(self):
        ...
        dx_group = VGroup(func.target, dx_arrows[0], dx_T, dx_arrows[1], ddx_T)
        ...
        for arrow, char, vect in zip(arrows, 'xxt', [UP, UP, RIGHT]):
            ...
            arrow.add(label)
        dt_arrow.shift(func.target[-3:].get_bottom() + MED_SMALL_BUFF * DOWN - dt_arrow.get_start())
        dt_T.next_to(dt_arrow.get_end(), DOWN)
        self.play(FadeInFromDown(func))
        self.wait()
        self.play(MoveToTarget(func), LaggedStartMap(Write, dx_arrows), run_time=1)
        self.play(TransformFromCopy(func[-3:], dx_T), path_arc=-TAU / 4)
        self.play(TransformFromCopy(dx_T, ddx_T), path_arc=-TAU / 4)
        self.wait()
...
class FlatAtBoundaryWords(Scene):
    ...
    def get_bc_words(self):
        return OldTexText('Flat at boundary\\\\for all', '${t}$', '$> 0$')
...
class WriteOutBoundaryCondition(FlatAtBoundaryWords, ThreeConstraints, MovingCameraScene):
    ...
    def write_bc_words(self):
        ...
        self.play(FadeOut(bc_paren, UP), FadeIn(bc_words, DOWN))
        self.wait()
        self.bc_words = bc_words

    def write_bc_equation(self):
        bc_words = self.bc_words
        equation = OldTex('{\\partial {T} \\over \\partial {x}}(0, {t}) = ', '{\\partial {T} \\over \\partial {x}}(L, {t}) = ', '0', **self.tex_mobject_config)
        equation.next_to(bc_words, DOWN, MED_LARGE_BUFF)
        self.play(self.camera_frame.shift, 0.8 * DOWN)
...
class HeatEquationFrame(WriteHeatEquationTemplate):
    ...
    def construct(self):
        ...
        dt = equation[:11]
        full_rect = FullScreenFadeRectangle(fill_color=GREY_D, fill_opacity=1)
        smaller_rect = ScreenRectangle(height=6, fill_color=BLACK, fill_opacity=1, stroke_color=WHITE, stroke_width=2)
        smaller_rect.next_to(equation, DOWN)
        self.add(full_rect)
        self.add(smaller_rect)
        self.add(equation)
        self.wait()
        self.play(ShowCreationThenFadeAround(ddx, surrounding_rectangle_config={'stroke_color': GREEN}))
        self.wait()
...
class CompareFreqDecays1to2(Scene):
    ...
    def construct(self):
        ...
        screens = VGroup(*[ScreenRectangle(height=4, fill_color=BLACK, fill_opacity=1, stroke_width=1, stroke_color=WHITE) for x in range(2)])
    ...
    def get_formula(self, freq):
        f_str = str(freq)
        return OldTex('\\cos\\left(%s \\cdot {x}\\right)' % f_str, 'e^{-\\alpha \\cdot %s^2 \\cdot {t}}' % f_str, tex_to_color_map={'{x}': GREEN, '{t}': YELLOW, f_str: MAROON_B})
...
class WorryAboutGenerality(TeacherStudentsScene, WriteHeatEquationTemplate):
    ...
    def construct(self):
        ...
        self.play(self.change_students('sad', 'tired', 'pleading'), self.teacher.change, 'raise_right_hand', FadeInFromDown(eq))
        self.play(Write(group[1:]))
        self.wait(2)
        self.play(ShowCreation(arrow), self.change_students(*3 * ['pondering']))
        self.play(FadeIn(themes, UP), self.change_students(*3 * ['thinking']), self.teacher.change, 'happy')
        self.wait(4)
