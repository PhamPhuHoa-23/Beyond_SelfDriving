...
class ShowNewRuleAtDiscreteBoundary(DiscreteSetup):
    CONFIG = {'axes_config': {'x_min': 0, 'stroke_width': 1, 'x_axis_config': {'include_tip': False}}, 'freq_amplitude_pairs': [(1, 0.5), (2, 1), (3, 0.5), (4, 0.3)], 'v_line_class': DashedLine, 'v_line_config': {}, 'step_size': 1, 'wait_time': 15, 'alpha': 0.25}
    ...
    def show_boundary_point_influenced_by_neighbor(self):
        ...
        self.play(LaggedStartMap(ShowCreation, arrows), LaggedStart(*[FadeIn(q_mark, -arrow.get_vector()) for q_mark, arrow in zip(q_marks, arrows)]), run_time=1.5)
        self.wait()
        new_arrows = VGroup(*[Arrow(d1.get_center(), VGroup(d1, d2).get_center(), buff=0).match_style(l_arrow) for d1, d2 in [(ld, ld_in), (rd, rd_in)]])
        ...
        self.play(ReplacementTransform(arrows, new_arrows), FadeOut(q_marks), ReplacementTransform(*blocking_rects))
        self.wait()
        self.play(FadeIn(braces, UP))
        self.wait()
        self.play(FadeOut(new_arrows), FadeOut(blocking_rects[1]), FadeOut(braces))
    ...
    def let_evolve(self):
        ...
        self.play(ClockPassesTime(self.clock, run_time=wait_time, hours_passed=wait_time))
    ...
    def get_v_line(self, dot):
        ...
        return self.v_line_class(proj_point, bottom, **self.v_line_config)
...
class DiscreteEvolutionPoint25(ShowNewRuleAtDiscreteBoundary):
    CONFIG = {'step_size': 0.25, 'alpha': 0.5, 'wait_time': 30}
...
class DiscreteEvolutionPoint1(DiscreteEvolutionPoint25):
    CONFIG = {'step_size': 0.1, 'v_line_config': {'stroke_width': 1}, 'wait_time': 30}

class FlatEdgesForDiscreteEvolution(DiscreteEvolutionPoint1):
    CONFIG = {'wait_time': 20, 'step_size': 0.1}
