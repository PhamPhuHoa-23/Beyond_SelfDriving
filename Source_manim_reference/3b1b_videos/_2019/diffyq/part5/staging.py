...
POSITION_COLOR = BLUE
CONST_COLOR = YELLOW
tex_config = {'tex_to_color_map': {'{t}': T_COLOR, '{0}': T_COLOR, 'e^': WHITE, '=': WHITE}}

class IntroductionOfExp(Scene):

    def construct(self):
        questions = VGroup(OldTexText('What', 'is', '$e^{t}$', '?'), OldTexText('What', 'properties', 'does', '$e^{t}$', 'have?'), OldTexText('What', 'property', 'defines', '$e^{t}$', '?', tex_to_color_map={'defines': BLUE}))
        questions.scale(2)
        ...
        self.wait()
        self.play(ShowCreation(top_cross), LaggedStart(*[TransformFromCopy(questions[0].get_part_by_tex(tex), questions[1].get_part_by_tex(tex)) for tex in ('What', 'e^{t}')], *[FadeIn(questions[1][i]) for i in [1, 2, 4]]))
        self.wait()
        self.play(TransformFromCopy(questions[1].get_part_by_tex('$e^{t}$'), derivative[3:7]), FadeIn(derivative[:2]), FadeIn(derivative.get_part_by_tex('=')))
        self.play(ReplacementTransform(questions[1][0], questions[2][0]), FadeOut(questions[1][1]), FadeIn(questions[2][1]), FadeOut(questions[1][2]), FadeIn(questions[2][2]), ReplacementTransform(questions[1][3], questions[2][3]), FadeOut(questions[1][4]), FadeIn(questions[2][4]))
        self.wait()
        self.play(TransformFromCopy(derivative[3:7:3], derivative[-5:], path_arc=-60 * DEGREES))
        self.wait()
        self.play(Restore(derivative), FadeIn(ic, LEFT))
        self.wait()
        self.play(FadeOut(questions[0]), FadeOut(top_cross), FadeOut(questions[2]), Restore(deriv_ic))
        self.wait()
    ...
    def get_deriv_and_ic(self, const=None):
        if const:
            ...
            const_str = '{}'
            mult_str = '{}'
        args = ['{d \\over d{t}}', 'e^{', const_str, '{t}}', '=', const_str, mult_str, 'e^{', const_str, '{t}}']
...
class IntroducePhysicalModel(IntroductionOfExp):
    CONFIG = {'number_line_config': {'x_min': 0, 'x_max': 100, 'unit_size': 1.5, 'big_tick_numbers': [0], 'numbers_to_show': list(range(0, 15)), 'label_direction': UP}, 'number_line_y': -2, 'const': 1, 'const_str': '', 'num_decimal_places': 2, 'output_label_config': {'show_ellipsis': True}}
    ...
    def setup_tip_label(self):
        ...
        tip_label.add_updater(lambda m: m.next_to(tip, UP, buff=2 * SMALL_BUFF, index_of_submobject_to_align=0))
    ...
    def setup_output_label(self):
        ...
        label.add_updater(lambda m: m.next_to(self.tip_label, RIGHT, buff=SMALL_BUFF, aligned_edge=DOWN))
    ...
    def setup_vectors(self):
        ...
        position_vect.add_updater(lambda m: m.put_start_and_end_on(number_line.cn2p(0), number_line.cn2p(self.get_output())))

        def update_velocity_vect(vect):
            vect.put_start_and_end_on(number_line.cn2p(0), number_line.cn2p(self.const * self.get_output()))
    ...
    def setup_vector_labels(self):
        ...
        def update_label(label):
            label.set_width(min(max_width, 0.8 * label.vect.get_length()))
            direction = normalize(label.vect.get_vector())
            perp_direction = rotate_vector(direction, -TAU / 4)
            label.next_to(label.vect.get_center(), np.round(perp_direction), buff=MED_SMALL_BUFF)
    ...
    def show_number_line(self):
        number_line = self.number_line
        self.play(LaggedStartMap(FadeIn, number_line.numbers.copy(), remover=True), Write(number_line), run_time=2)
        self.wait()
    ...
    def show_formulas(self):
        ...
        self.add(deriv, ic)
        self.play(ShowCreation(rhs_rect), FadeIn(rhs_word, UP), ShowCreation(self.position_vect))
        self.add(self.tip, self.number_line, self.position_vect)
        self.number_line.numbers.set_stroke(BLACK, 3, background=True)
        self.play(Transform(rhs.copy(), self.tip_label.copy().clear_updaters(), remover=True), GrowFromPoint(self.tip, rhs.get_bottom()), TransformFromCopy(rhs_word, self.position_label))
        self.add(self.tip_label)
        self.wait()
        self.play(ShowCreation(lhs_rect), FadeIn(lhs_word, UP))
        self.wait()
        self.play(TransformFromCopy(self.position_vect, self.velocity_vect, path_arc=PI))
    ...
    def let_time_pass(self):
        ...
        self.wait(5)
        self.play(nl.scale, 0.1, {'about_point': zero_point}, run_time=1)
        self.wait(6)
        self.play(nl.scale, 0.1, {'about_point': zero_point}, run_time=1)
        self.wait(8)
        self.play(nl.scale, 0.1, {'about_point': zero_point}, run_time=1)
...
class ConstantEquals2(IntroducePhysicalModel):
    CONFIG = {'const': 2, 'const_str': '2', 'const_text': 'Double'}
    ...
    def point_out_chain_rule(self):
        ...
        self.play(ShowCreationThenFadeAround(v_part[-2], surrounding_rectangle_config={'color': CONST_COLOR}))
        self.wait()
        self.play(Restore(p_part, path_arc=-TAU / 6), FadeIn(p_rect), FadeIn(p_word))
        self.play(Restore(c_part, path_arc=TAU / 6), FadeIn(dot))
        self.play(FadeIn(c_rect), FadeIn(c_word))
        self.wait()
        self.v_word = v_word
    ...
    def show_double_vector(self):
        ...
        self.play(MoveToTarget(p_copy), FadeIn(times_2))
        self.play(p_copy.scale, 2, {'about_edge': LEFT}, p_copy.match_color, v_vect)
        self.play(FadeOut(times_2))
        self.play(ReplacementTransform(p_copy, v_vect), TransformFromCopy(self.v_word, self.velocity_label))
        self.wait()
    ...
    def let_time_pass(self):
        ...
        xs2 = range(200, 1000, 200)
        nl.add(*[nl.get_tick(x, size=15) for x in xs2])
        nl.add_numbers(*xs2, buff=20, height=50)
        xs3 = range(2000, 10000, 2000)
        nl.add(*[nl.get_tick(x, size=150) for x in xs3])
        nl.add_numbers(*xs3, buff=200, height=300)
        for n in nl.numbers:
            n[1].scale(0.5)
        xs3 = range(20000, 100000, 20000)
        nl.add(*[nl.get_tick(x, size=1500) for x in xs3])
        nl.add_numbers(*xs3, buff=2000, height=3000)
        for n in nl.numbers:
            n[2].scale(0.5)
        self.add(self.tip)
        self.add(self.output_label)
...
class NegativeConstant(ConstantEquals2):
    CONFIG = {'const': -0.5, 'const_str': '-0.5', 'const_text': 'Flip and squish', 'number_line_config': {'unit_size': 2.5}}
    ...
    def show_vector_manipulation(self):
        ...
        self.velocity_label.add_updater(lambda m: m.next_to(self.velocity_vect, UP, buff=SMALL_BUFF))
        v_vect_back = v_vect.copy()
        v_vect_back.set_stroke(BLACK, 3)
        self.play(MoveToTarget(p_copy), FadeIn(times_neg1))
        self.play(Rotate(p_copy, angle=PI, about_point=p_copy.get_start()))
        self.wait()
        self.play(p_copy.scale, 0.5, {'about_point': p_copy.get_start()}, p_copy.match_color, v_vect, FadeIn(times_point5))
        self.wait()
        self.play(ReplacementTransform(p_copy, v_vect), FadeOut(terms), TransformFromCopy(self.v_word, self.velocity_label))
    ...
    def let_time_pass(self):
        ...
        self.play(MoveToTarget(nl), new_ticks.stretch, scale_factor, 0, {'about_point': zero_point}, VFadeIn(new_ticks))
        self.wait()
        rate = 0.25
        t_tracker.add_updater(lambda m, dt: m.increment_value(rate * dt))
        self.play(VFadeIn(self.output_label))
...
class ImaginaryConstant(ConstantEquals2):
    CONFIG = {'const': complex(0, 1), 'const_str': 'i', 'const_text': 'Rotate $90^\\circ$', 'number_line_config': {'x_min': -5, 'unit_size': 2.5}, 'output_label_config': {'show_ellipsis': False}, 'plane_unit_size': 2.5}
    ...
    def add_initial_objects(self):
        ...
        self.tip_label.add_updater(lambda m: m.next_to(self.position_vect.get_end(), UP, buff=0, index_of_submobject_to_align=0).shift(m.shift_vect.get_location()))
        eq = deriv.get_part_by_tex('=')
        dot = deriv.get_part_by_tex('\\cdot')
        eq_index = deriv.index_of_part(eq)
        dot_index = deriv.index_of_part(dot)
        ...
        self.play(FadeInFromDown(p_term), VFadeIn(randy))
        self.play(randy.change, 'confused')
        self.play(ShowCreation(p_rect), FadeIn(p_label, UP))
        self.add(self.number_line, self.position_vect)
        self.play(Transform(p_label.copy(), self.tip_label.copy().clear_updaters(), remover=True), point.move_to, self.position_vect.get_end(), FadeIn(ic))
        self.add(self.tip_label)
        self.play(Blink(randy))
        self.play(FadeOut(randy))
        self.play(LaggedStartMap(FadeIn, [v_term, eq, v_rect, v_label]), run_time=1)
        c_term.save_state()
        c_term.replace(p_term[2])
        self.play(Restore(c_term, path_arc=TAU / 4), FadeIn(dot))
        c_rect.stretch(1.2, 1, about_edge=DOWN)
    ...
    def show_vector_manipulation(self):
        ...
        arc = Arc(angle=TAU / 4, radius=0.5, arc_center=p_copy.target.get_start())
        ...
        self.play(MoveToTarget(p_copy), FadeIn(times_i))
        self.play(FadeIn(self.c_rect), FadeInFromDown(self.c_label))
        self.play(ShowCreation(arc), FadeIn(angle_label), ApplyMethod(times_i.next_to, rot_p, RIGHT, {'buff': 0}, path_arc=TAU / 4), TransformFromCopy(p_copy, rot_p, path_arc=-TAU / 4))
        self.wait()
        temp_rot_v_label = rot_v_label.copy()
        temp_rot_v_label.clear_updaters()
        temp_rot_v_label.replace(self.v_label, dim_to_match=0)
        self.play(TransformFromCopy(rot_p, v_vect), TransformFromCopy(temp_rot_v_label, rot_v_label), self.tip_label.shift_vect.move_to, 0.1 * UP + 0.2 * RIGHT)
        self.play(FadeOut(VGroup(p_copy, arc, angle_label, times_i, rot_p)))
        self.velocity_label = rot_v_label

    def transition_to_complex_plane(self):
        nl = self.number_line
        background_rects = VGroup(*[BackgroundRectangle(rect) for rect in self.term_rects])
        self.ic.add_background_rectangle()
        for label in self.term_labels:
            label.add_background_rectangle()
        corner_group = VGroup(background_rects, self.deriv, self.term_rects, self.term_labels, self.ic)
        corner_group.generate_target()
        corner_group.target.to_corner(UL)
        corner_group.target[-1].next_to(corner_group.target, DOWN, buff=0.75, aligned_edge=LEFT)
        plane = ComplexPlane()
        ...
        self.add(plane, corner_group, self.position_vect, self.velocity_vect, self.position_label, self.velocity_label, self.tip_label)
        plane.save_state()
        plane.set_opacity(0)
        self.play(VFadeIn(background_rects), MoveToTarget(corner_group), MoveToTarget(nl))
    ...
    def show_vector_field(self):
        plane = self.plane
        temp_faders = VGroup(self.position_vect, self.position_label, self.velocity_vect, self.velocity_label, self.tip_label)
        foreground = VGroup(self.corner_group)
        n = 12
        zs = [np.exp(complex(0, k * TAU / n)) for k in range(n)]
        points = map(plane.n2p, zs)
        vects = VGroup(*[Arrow(plane.n2p(0), point, buff=0, color=POSITION_COLOR) for point in points])
        vects.set_opacity(0.75)
        attached_vects = VGroup(*[vect.copy().shift(vect.get_vector()) for vect in vects])
        attached_vects.set_color(VELOCITY_COLOR)
        v_vects = VGroup(*[av.copy().rotate(90 * DEGREES, about_point=av.get_start()) for av in attached_vects])
        self.play(LaggedStartMap(GrowArrow, vects), FadeOut(temp_faders))
        self.wait()
        self.play(TransformFromCopy(vects, attached_vects, path_arc=20 * DEGREES))
        self.play(ReplacementTransform(attached_vects, v_vects, path_arc=90 * DEGREES))
        self.wait()
        ...
        vf_config = {'max_magnitude': 8, 'opacity': 0.5}
        vf0 = VectorField(lambda p: np.array(p) - zero_point, length_func=lambda x: x, **vf_config)
        vf1 = VectorField(func, length_func=lambda x: x, **vf_config)
        vf2 = VectorField(func, **vf_config)
        for vf in [vf0, vf1, vf2]:
            vf.submobjects.sort(key=lambda m: get_norm(m.get_start()))
        self.play(FadeIn(vf0, lag_ratio=0.01, run_time=3), FadeOut(vects), FadeOut(v_vects), FadeOut(foreground))
        self.play(Transform(vf0, vf1, path_arc=90 * DEGREES))
        self.play(Transform(vf0, vf2, lag_ratio=0.001, run_time=2))
    ...
    def show_circle(self):
        ...
        circle = Circle(radius=get_norm(plane.n2p(1) - plane.n2p(0)), stroke_color=TEAL)
        circle.move_to(plane.n2p(0))
        epii, eti = results = [OldTex('e^{{i}' + s1 + '} = ' + s2, tex_to_color_map={'{i}': CONST_COLOR, '\\pi': T_COLOR, '\\tau': T_COLOR, '-1': POSITION_COLOR, '1': POSITION_COLOR}) for s1, s2 in [('\\pi', '-1'), ('\\tau', '1')]]
        ...
        self.play(Transform(ic[-1].copy(), output_label.deepcopy().clear_updaters(), remover=True), FadeOut(self.position_label), FadeOut(self.velocity_label))
        self.add(output_label)
        self.wait()
        circle_copy = circle.copy()
        circle_copy.save_state()
        self.play(ShowCreation(circle), t_tracker.set_value, TAU, run_time=TAU, rate_func=linear)
        self.wait()
        self.play(t_tracker.set_value, 0, circle.set_stroke, {'width': 1})
        self.wait()
        self.add(circle_copy, self.tip_label, self.output_label)
        self.play(ApplyMethod(t_tracker.set_value, PI, rate_func=linear), tip_label.shift_vect.move_to, 0.25 * UR, ShowCreation(circle_copy, rate_func=lambda t: 0.5 * t), run_time=PI)
        self.wait()
        self.play(Restore(epii, run_time=2))
        self.wait()
        circle_copy.restore()
        self.play(ApplyMethod(t_tracker.set_value, TAU, rate_func=linear), tip_label.shift_vect.move_to, 0.1 * UR, ShowCreation(circle_copy, rate_func=lambda t: 0.5 + 0.5 * t), run_time=PI)
        self.wait()
...
class PiMinuteMark(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeInFromDown(rect), FadeInFromDown(text))
        self.wait()
...
class ReferenceWhatItMeans(PiCreatureScene):
    ...
    def construct(self):
        ...
        tex_config = {'tex_to_color_map': {'{i}': CONST_COLOR, '\\pi': T_COLOR, '-1': POSITION_COLOR}}
        ...
        series = OldTex('{({i} \\pi )^0 \\over 0!} + ', '{({i} \\pi )^1 \\over 1!} + ', '{({i} \\pi )^2 \\over 2!} + ', '{({i} \\pi )^3 \\over 3!} + ', '{({i} \\pi )^4 \\over 4!} + ', '\\cdots = -1', **tex_config)
        series.set_width(FRAME_WIDTH - 1)
        series.next_to(down_arrow, DOWN, LARGE_BUFF)
        series.set_x(0)
        self.add(epii)
        self.play(FadeIn(arrow, LEFT), FadeIn(repeated_mult, 2 * LEFT), randy.change, 'maybe')
        self.wait()
        self.play(randy.change, 'confused')
        self.play(FadeOut(arrow, DOWN), FadeIn(does_not_mean, UP))
        self.play(Write(nonsense))
        self.play(randy.change, 'angry')
        self.wait()
        self.play(FadeOut(randy, DOWN), FadeIn(down_arrow, UP), FadeIn(actually_means, LEFT), FadeIn(series, lag_ratio=0.1, run_time=2))
        self.wait()
...
class VideoWrapper(Scene):
    ...
    def construct(self):
        ...
        screen_rects = VGroup(ScreenRectangle(), ScreenRectangle())
        ...
        titles = VGroup(OldTexText('Want more?'), OldTexText('Learn calculus'))
        titles.scale(1.5)
...
class Thumbnail(Scene):
    ...
    def construct(self):
        epii = OldTex('e^{ {i} \\pi} = -1', tex_to_color_map={'{i}': YELLOW, '\\pi': BLUE, '-1': GREY_A})
        epii.set_width(8)
        epii.to_edge(UP)
        epii.set_stroke(BLACK, 50, background=True)
        words = VGroup(OldTexText('in'), OldTexText('in', '3.14', ' minutes'))
        ...
        circle = Circle(radius=unit_size, color=YELLOW)
        ...
        p_vect = Arrow(plane.n2p(0), plane.n2p(1), buff=0, fill_color=GREY_A, thickness=0.1)
        v_vect = Arrow(plane.n2p(1), plane.n2p(complex(1, 1)), buff=0, fill_color=YELLOW, thickness=0.1)
