...
class WriteHeatEquationTemplate(Scene):
    CONFIG = {'tex_mobject_config': {'tex_to_color_map': {'{T}': WHITE, '{t}': YELLOW, '{x}': GREEN, '{y}': RED, '{z}': BLUE, '\\partial': WHITE, '2': WHITE}}}

    def get_d1_equation(self):
        return OldTex('{\\partial {T} \\over \\partial {t}}({x}, {t})', '=', '\\alpha \\cdot', '{\\partial^2 {T} \\over \\partial {x}^2} ({x}, {t})', **self.tex_mobject_config)

    def get_d1_equation_without_inputs(self):
        return OldTex('{\\partial {T} \\over \\partial {t}}', '=', '\\alpha \\cdot', '{\\partial^2 {T} \\over \\partial {x}^2}', **self.tex_mobject_config)

    def get_d3_equation(self):
        return OldTex('{\\partial {T} \\over \\partial {t}}', '=', '\\alpha \\left(', '{\\partial^2 {T} \\over \\partial {x}^2} + ', '{\\partial^2 {T} \\over \\partial {y}^2} + ', '{\\partial^2 {T} \\over \\partial {z}^2}', '\\right)', **self.tex_mobject_config)

    def get_general_equation(self):
        return OldTex('{\\partial {T} \\over \\partial {t}}', '=', '\\alpha', '\\nabla^2 {T}', **self.tex_mobject_config)

    def get_d3_equation_with_inputs(self):
        return OldTex('{\\partial {T} \\over \\partial {t}}', '({x}, {y}, {z}, {t})', '=', '\\alpha \\left(', '{\\partial^2 {T} \\over \\partial {x}^2}', '({x}, {y}, {z}, {t}) + ', '{\\partial^2 {T} \\over \\partial {y}^2}', '({x}, {y}, {z}, {t}) + ', '{\\partial^2 {T} \\over \\partial {z}^2}', '({x}, {y}, {z}, {t})', '\\right)', **self.tex_mobject_config)
    ...
    def get_d1_group(self):
        group = VGroup(self.get_d1_words(), self.get_d1_equation())
        group.arrange(DOWN, buff=MED_LARGE_BUFF)
        return group

    def get_d3_group(self):
        group = VGroup(self.get_d3_words(), self.get_d3_equation())
        group.arrange(DOWN, buff=MED_LARGE_BUFF)
        return group
...
class HeatEquationIntroTitle(WriteHeatEquationTemplate):
    ...
    def construct(self):
        ...
        self.play(FadeIn(title, DOWN), FadeIn(equation, UP))
        self.wait()
...
class BringTogether(Scene):
    ...
    def construct(self):
        ...
        self.play(VFadeIn(words), Restore(words), arrows.arrange, RIGHT, {'buff': SMALL_BUFF}, VFadeIn(arrows))
...
class FourierSeriesIntro(WriteHeatEquationTemplate):
    ...
    def construct(self):
        ...
        title = OldTexText('Fourier ', 'Series')
        ...
        def update_heat_colors(heat):
            ...
            for letter, vertex in zip(letters, vertices):
                ...
                letter.set_color(interpolate_color(colors[i], colors[i + 1], alpha))
        ...
        bubble = ThoughtBubble(height=2, width=2.5, direction=RIGHT)
        ...
        self.add(title)
        self.play(FadeInFromDown(image), TransformFromCopy(title.get_part_by_tex('Fourier'), name.get_part_by_tex('Fourier'), path_arc=90 * DEGREES), FadeIn(name.get_part_by_tex('Joseph')))
        self.play(Write(details_coming, run_time=1))
        self.play(LaggedStartMap(FadeOut, details_coming[0], run_time=1))
        self.wait()
        self.add(rot_square)
        self.play(FadeIn(physics, RIGHT), GrowArrow(arrow2), FadeIn(heat, RIGHT), GrowArrow(arrow1), MoveToTarget(title))
...
class ShowPartialDerivativeSymbols(Scene):
    ...
    def construct(self):
        t2c = {'{x}': GREEN, '{t}': YELLOW}
        d_derivs, del_derivs = VGroup(*[VGroup(*[OldTex('{' + sym, 'T', '\\over', sym, var + '}', '(', '{x}', ',', '{t}', ')').set_color_by_tex_to_color_map(t2c) for var in ('{x}', '{t}')]) for sym in ('d', '\\partial')])
        dTdx, dTdt = d_derivs
        ...
        self.wait()
        dTdt.move_to(dTdx)
        self.play(dTdx.next_to, dTdt, RIGHT, {'buff': 1.5}, dTdx.set_opacity, 0.5, FadeInFromDown(dTdt))
        self.wait()
        ...
        self.play(Write(pd_words), dTdx.set_opacity, 1, run_time=1)
        self.wait()
        self.play(ReplacementTransform(d_derivs, del_derivs))
        self.play(LaggedStartMap(ShowCreationThenFadeAround, dels, surrounding_rectangle_config={'color': BLUE, 'buff': 0.5 * SMALL_BUFF, 'stroke_width': 2}))
        self.wait()
        num_words = VGroup(*[OldTexText('Change in $T$\\\\caused by {}', '$\\partial$', '${}$'.format(var), arg_separator='').set_color_by_tex_to_color_map(t2c) for var in ('{x}', '{t}')])
        num_words.scale(0.8)
        for word, deriv in zip(num_words, del_derivs):
            num = deriv[:2]
            word.move_to(num, UP)
            word.to_edge(UP, buff=MED_SMALL_BUFF)
            deriv.rect = SurroundingRectangle(num, buff=SMALL_BUFF, stroke_width=2, color=word[-1].get_color())
            deriv.rect.mob = num
            deriv.rect.add_updater(lambda r: r.move_to(r.mob))
        self.play(Write(num_words[1]), VGroup(del_derivs, pd_words).shift, DOWN, ShowCreation(del_derivs[1].rect))
        self.play(Write(num_words[0]), ShowCreation(del_derivs[0].rect))
        self.wait()
...
class WriteHeatEquation(WriteHeatEquationTemplate):
    ...
    def construct(self):
        ...
        self.play(FadeIn(title, 0.5 * DOWN), FadeIn(equation, 0.5 * UP))
        self.wait()
        ...
        self.wait(2)
        self.play(ShowCreationThenDestruction(del_outlines, lag_ratio=0.1))
        self.play(FadeOut(title, UP), FadeIn(pde, DOWN), FadeOut(dt_rect), FadeOut(dx_rect))
        self.wait()
...
class Show3DEquation(WriteHeatEquationTemplate):
    ...
    def construct(self):
        ...
        equation.to_edge(UP)
        self.add(equation)
        self.play(LaggedStartMap(ShowCreationThenFadeAround, inputs, surrounding_rectangle_config={'buff': 0.05, 'stroke_width': 2}))
        self.wait()
...
class Show1DAnd3DEquations(WriteHeatEquationTemplate):
    ...
    def construct(self):
        ...
        self.play(Write(d1_words), FadeIn(d1_equation, UP), run_time=1)
        self.wait(2)
        self.play(Restore(d1_group), FadeIn(d3_group, LEFT))
        self.wait()
        self.play(GrowFromCenter(d3_brace), Write(nabla_words), TransformFromCopy(d3_rhs, nabla_exp), run_time=1)
        self.wait()
...
class AltHeatRHS(Scene):
    ...
    def construct(self):
        formula = OldTex('{\\alpha \\over 2}', '\\Big(', 'T({x} - 1, {t}) + T({x} + 1, {t})\\Big)', tex_to_color_map={'{x}': GREEN, '{t}': YELLOW})
        self.add(formula)

class CompareInputsOfGeneralCaseTo1D(WriteHeatEquation):

    def construct(self):
        three_d_expr, one_d_expr = [OldTex('{T}(' + inputs + ', {t})', **self.tex_mobject_config) for inputs in ['{x}, {y}, {z}', '{x}']]
        ...
        low = 3
        high = -3
        self.play(ReplacementTransform(three_d_expr[:low], one_d_expr[:low]), ReplacementTransform(three_d_expr[high:], one_d_expr[high:]), three_d_expr[low:high].scale, 0)
        self.wait()
...
class ShowLaplacian(WriteHeatEquation):
    ...
    def construct(self):
        ...
        for part in parts[1:]:
            self.play(rp.become, part.rp, FadeIn(part, LEFT), Write(part.plus), ShowCreation(part.rect))
...
class AskAboutActuallySolving(WriteHeatEquationTemplate):
    ...
    def construct(self):
        ...
        formula = OldTex('T({x}, {t}) = \\sin\\big(a{x}\\big) e^{-\\alpha \\cdot a^2 {t}}', tex_to_color_map={'{x}': GREEN, '{t}': YELLOW})
