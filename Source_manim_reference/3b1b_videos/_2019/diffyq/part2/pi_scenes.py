...
class ReactionsToInitialHeatEquation(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(randy.change, 'confused', point.next_to, randy, UR, LARGE_BUFF)
        self.wait(2)
...
class ContrastPDEToODE(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        pde, ode = words = VGroup(*[OldTexText(text + '\\\\', 'Differential\\\\', 'Equation') for text in ('Partial', 'Ordinary')])
        ...
        self.play(FadeInFromDown(VGroup(words, lt)), student.change, 'raise_right_hand')
        self.play(self.change_students('pondering', 'pondering', 'hooray'), self.teacher.change, 'happy')
        self.wait(3)
        self.play(Swap(ode, pde), self.teacher.change, 'raise_right_hand', self.change_students('erm', 'sassy', 'confused'))
        self.look_at(words)
        self.play_student_changes('thinking', 'thinking', 'tease')
        self.wait(3)
...
class AskAboutWhereEquationComesFrom(TeacherStudentsScene, WriteHeatEquationTemplate):
    ...
    def construct(self):
        ...
        self.student_says('Um...why?', target_mode='sassy', index=2, bubble_config={'direction': RIGHT})
        self.play_student_changes('confused', 'confused', 'sassy')
        self.wait()
...
class AskWhyRewriteIt(TeacherStudentsScene):
    ...
    def construct(self):
        self.student_says('Why?', index=1, bubble_config={'height': 2, 'width': 2})
