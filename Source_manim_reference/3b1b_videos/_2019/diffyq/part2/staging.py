...
class PartTwoOfTour(TourOfDifferentialEquations):
    CONFIG = {'zoomed_thumbnail_index': 1}
    ...
    def zoom_in_to_one_thumbnail(self):
        ...
        self.add(ode)
        frame.save_state()
        self.play(frame.replace, thumbnails[0], run_time=1)
        self.play(Restore(frame, run_time=3))
        self.play(TransformFromCopy(ode, pde))
        self.play(ApplyMethod(frame.replace, thumbnails[1], path_arc=-30 * DEGREES, run_time=3))
        self.wait()

class BrownianMotion(Scene):
    CONFIG = {'wait_time': 60, 'L': 3, 'n_particles': 100, 'm1': 1, 'm2': 100, 'r1': 0.05, 'r2': 0.5, 'max_v': 5, 'random_seed': 2}
    ...
    def add_particles(self):
        ...
        lil_particles = VGroup(*[self.get_particle(m1, r1, L, max_v) for k in range(n_particles)])
    ...
    def get_particle(self, m, r, L, max_v):
        ...
        dot.mass = m
        dot.radius = r
        dot.center = op.add(np.random.uniform(-L + r, L - r) * RIGHT, np.random.uniform(-L + r, L - r) * UP)
        dot.move_to(dot.center)
        dot.velocity = rotate_vector(np.random.uniform(0, max_v) * RIGHT, np.random.uniform(0, TAU))
        return dot
        ...
        for p1 in particles:
            ...
            for i in [0, 1]:
                if abs(c1[i]) + r1 > self.L:
                    ...
                    p1.velocity[i] *= -1 * op.mul(np.sign(p1.velocity[i]), np.sign(c1[i]))
        for p in particles:
            p.move_to(p.center)
        return particles

class AltBrownianMotion(BrownianMotion):
    CONFIG = {'wait_time': 20, 'n_particles': 100, 'm2': 10}
...
class BlackScholes(AltBrownianMotion):
    ...
    def add_title(self):
        ...
        equation = OldTex('{\\partial V \\over \\partial t}', '+', '\\frac{1}{2} \\sigma^2 S^2', '{\\partial^2 V \\over \\partial S^2}', '+', 'rS', '{\\partial V \\over \\partial S}', '-rV', '=', '0')
        equation.scale(0.8)
    ...
    def add_graph(self):
        axes = Axes(x_min=-1, x_max=20, y_min=0, y_max=10, axis_config={'unit_size': 0.5})
        axes.set_height(4)
        axes.move_to(DOWN)
...
class ContrastChapters1And2(Scene):
    ...
    def construct(self):
        ...
        c1_title, c2_title = titles = VGroup(OldTexText('Chapter 1'), OldTexText('Chapter 2'))
        titles.scale(1.5)
        ode, pde = des = VGroup(OldTexText('Ordinary', 'Differential Equations\\\\', 'ODEs'), OldTexText('Partial', 'Differential Equations\\\\', 'PDEs'))
        ode[0].set_color(BLUE)
        pde[0].set_color(YELLOW)
        for de in des:
        ...
        c1_words = OldTexText("They're", 'really\\\\', '{}', 'freaking', 'hard\\\\', 'to', 'solve\\\\')
        c1_words.set_height(0.5 * c1_frame.get_height())
        c1_words.move_to(c1_frame)
        c2_words = OldTexText("They're", 'really', '\\emph{really}\\\\', 'freaking', 'hard\\\\', 'to', 'solve\\\\')
        c2_words.set_color_by_tex('\\emph', YELLOW)
        c2_words.move_to(c2_frame)
        edit_shift = MED_LARGE_BUFF * RIGHT
        c2_edits = VGroup(OldTexText('sometimes').next_to(c2_words[1:3], UP, aligned_edge=LEFT), Line(c2_words[1].get_left(), c2_words[2].get_right(), stroke_width=8), OldTexText('not too').next_to(c2_words[3], LEFT), Line(c2_words[3].get_left(), c2_words[3].get_right(), stroke_width=8))
        ...
        self.add(des)
        self.wait()
        self.play(LaggedStartMap(FadeInFromDown, c1_words, lag_ratio=0.1))
        self.wait()
        self.play(TransformFromCopy(c1_words, c2_words), Write(lt))
        self.wait()
        self.play(Write(c2_edits[:2], run_time=1))
        self.play(c2_words[3:5].shift, edit_shift, Write(c2_edits[2:]), run_time=1)
        self.wait()

class ShowCubeFormation(ThreeDScene):
    CONFIG = {'camera_config': {'shading_factor': 1.0}, 'color': False}

    def construct(self):
        light_source = self.camera.light_source
        light_source.move_to(np.array([-6, -3, 6]))
        cube = Cube(side_length=4, fill_color=GREY, stroke_color=WHITE, stroke_width=0.5)
        ...
        cube.become(cube_0d)
        self.set_camera_orientation(phi=70 * DEGREES, theta=-145 * DEGREES)
...
class TwoBodyEquations(Scene):
    ...
    def construct(self):
        kw = {'tex_to_color_map': {'x_1': GREY_B, 'y_1': GREY_B, 'x_2': BLUE, 'y_2': BLUE, '=': WHITE}}
        equations = VGroup(OldTex('{d^2 x_1 \\over dt^2}', '=', '{x_2 - x_1 \\over m_1 \\left(', '(x_2 - x_1)^2 + (y_2 - y_1)^2', '\\right)^{3/2}', **kw), OldTex('{d^2 y_1 \\over dt^2}', '=', '{y_2 - y_1 \\over m_1 \\left(', '(x_2 - x_1)^2 + (y_2 - y_1)^2', '\\right)^{3/2}', **kw), OldTex('{d^2 x_2 \\over dt^2}', '=', '{x_1 - x_2 \\over m_2 \\left(', '(x_2 - x_1)^2 + (y_2 - y_1)^2', '\\right)^{3/2}', **kw), OldTex('{d^2 y_2 \\over dt^2}', '=', '{y_1 - y_2 \\over m_2 \\left(', '(x_2 - x_1)^2 + (y_2 - y_1)^2', '\\right)^{3/2}', **kw))
        ...
        self.wait()
        self.play(LaggedStartMap(Restore, variables), FadeIn(lhss_copy, remover=True, lag_ratio=0.1, run_time=2))
        self.add(lhss)
        self.wait()
        ...
        self.wait()
        self.play(LaggedStartMap(ShowCreationThenFadeAround, rhss))
        self.wait()

class LaplacianIntuition(SpecialThreeDScene):
    CONFIG = {'three_d_axes_config': {'x_min': -5, 'x_max': 5, 'y_min': -5, 'y_max': 5}, 'surface_resolution': 32}
    ...
    def construct(self):
        ...
        surface_config = {'u_min': -5, 'u_max': 5, 'v_min': -5, 'v_max': 5, 'resolution': self.surface_resolution}
        plane = Square(side_length=10, stroke_width=0, fill_color=WHITE, fill_opacity=0.1)
        plane.center()
        ...
        line = always_redraw(lambda: DashedLine(point.get_location(), func(*point.get_location()[:2]), background_image_file='VerticalTempGradient'))
        ...
        surface_circle = always_redraw(lambda: circle.copy().apply_function(lambda p: func(*p[:2])).shift(0.02 * IN).color_using_background_image('VerticalTempGradient'))
        self.play(FadeInFromLarge(dot))
        self.play(ShowCreation(line))
        self.play(TransformFromCopy(dot, circle))
        self.play(Transform(circle.copy(), surface_circle.copy().clear_updaters(), remover=True))
        self.add(surface_circle)
        self.wait()
        for vect in [4 * LEFT, DOWN, 4 * RIGHT, UP]:
            self.play(point.shift, vect, run_time=3)
...
class StrogatzMention(PiCreatureScene):
    ...
    def show_book(self):
        ...
        self.wait()
        self.play(FadeIn(book, DOWN), steve.shift, 4 * RIGHT, RemovePiCreatureBubble(morty, target_mode='thinking'))
        self.wait(3)
        self.play(FadeOut(steve), FadeOut(morty))
        self.book = book

    def show_motives(self):
        motives = VGroup(OldTexText('1) Scratch and itch'), OldTexText('2) Make people love math'))
        motives.scale(1.5)
        motives.arrange(DOWN, LARGE_BUFF, aligned_edge=LEFT)
...
class Thumbnail(Scene):
    ...
    def construct(self):
        ...
        self.add(image)
        equation = OldTex('{\\partial {T} \\over \\partial {t}}', '=', '\\alpha', '\\nabla^2 {T}', tex_to_color_map={'{t}': YELLOW, '{T}': RED})
        equation.scale(2)
        equation.to_edge(UP)
        self.add(equation)
        ...
        arrow = Arrow(question.get_top(), equation.get_left(), buff=0.5, path_arc=-90 * DEGREES)
