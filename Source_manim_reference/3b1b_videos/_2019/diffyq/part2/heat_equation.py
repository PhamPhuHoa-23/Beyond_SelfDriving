...
class TwoDBodyWithManyTemperatures(ThreeDScene):
    CONFIG = {'cells_per_side': 20, 'body_height': 6}
    ...
    def introduce_body(self):
        ...
        plate = Square(stroke_width=0, fill_color=GREY_D, sheen_direction=UL, sheen_factor=1, fill_opacity=1)
        plate.replace(body)
...
class BringTwoRodsTogether(Scene):
    CONFIG = {'step_size': 0.05, 'axes_config': {'x_min': -1, 'x_max': 11, 'y_min': -10, 'y_max': 100, 'y_axis_config': {'unit_size': 0.06, 'tick_frequency': 10}}, 'y_labels': range(20, 100, 20), 'graph_x_min': 0, 'graph_x_max': 10, 'midpoint': 5, 'max_temp': 90, 'min_temp': 10, 'wait_time': 30, 'default_n_rod_pieces': 20, 'alpha': 1.0}
    ...
    def setup_graph(self):
        graph = self.axes.get_graph(self.initial_function, x_min=self.graph_x_min, x_max=self.graph_x_max, step_size=self.step_size, discontinuities=[self.midpoint])
    ...
    def setup_clock(self):
        ...
        time_label = DecimalNumber(0, num_decimal_places=2)
        time_rhs = OldTexText('s')
        time_group = VGroup(time_lhs, time_label)
    ...
    def show_rods(self):
        rod1, rod2 = rods = VGroup(self.get_rod(0, 5), self.get_rod(5, 10))
        rod1.set_color(rod1[0].get_color())
        rod2.set_color(rod2[-1].get_color())
        rods.save_state()
        rods.space_out_submobjects(1.5)
        rods.center()
        labels = VGroup(OldTex('90^\\circ'), OldTex('10^\\circ'))
        for rod, label in zip(rods, labels):
            label.next_to(rod, DOWN)
            rod.label = label
        self.play(FadeIn(rod1, UP), Write(rod1.label))
        self.play(FadeIn(rod2, DOWN), Write(rod2.label))
        self.wait()
        self.rods = rods
    ...
    def show_equilibration(self):
        ...
        self.play(Write(axes), rods.restore, rods.space_out_submobjects, 1.1, FadeIn(self.time_group), FadeIn(self.clock), *[MaintainPositionRelativeTo(rod.label, rod) for rod in rods])
        ...
        self.play(ShowCreation(graph), labels[0].align_to, axes.c2p(0, 87), UP, labels[1].align_to, axes.c2p(0, 13), DOWN)
        self.play()
        self.play(rods.restore, rate_func=rush_into)
        self.remove(br1, br2)
    ...
    def get_clock_anim(self, time, **kwargs):
        config = {'run_time': time, 'hours_passed': time}
    ...
    def update_graph(self, graph, dt, alpha=None, n_mini_steps=500):
        if alpha is None:
            alpha = self.alpha
        points = np.append(graph.get_start_anchors(), [graph.get_last_point()], axis=0)
    ...
    def get_second_derivative(self, x, dx=0.001):
        ...
        ly, y, ry = [graph.point_from_proportion(inverse_interpolate(x_min, x_max, alt_x))[1] for alt_x in (x - dx, x, x + dx)]
        if x == x_max:
            return (ly - y) / dx
        elif x == x_min:
            return (ry - y) / dx
    ...
    def color_rod_by_graph(self, rod):
        for piece in rod:
            piece.set_color(color=[self.rod_point_to_color(piece.get_left()), self.rod_point_to_color(piece.get_right())])

    def rod_point_to_graph_y(self, point):
        axes = self.axes
        x = axes.x_axis.p2n(point)
        graph = self.graph
        alpha = inverse_interpolate(self.graph_x_min, self.graph_x_max, x)
...
class ShowEvolvingTempGraphWithArrows(BringTwoRodsTogether):
    CONFIG = {'alpha': 0.1, 'arrow_xs': np.linspace(0, 10, 22)[1:-1], 'arrow_scale_factor': 0.5, 'max_magnitude': 1.5, 'wait_time': 30, 'freq_amplitude_pairs': [(1, 0.5), (2, 1), (3, 0.5), (4, 0.3), (5, 0.3), (7, 0.2), (21, 0.1), (41, 0.05)]}
    ...
    def add_rod(self):
        rod = self.rod = self.get_rod(self.graph_x_min, self.graph_x_max)
        self.add(rod)
    ...
    def add_arrows(self):
        ...
        def update_arrows(arrows):
            for x, arrow in zip(xs, arrows):
                ...
                mag = np.clip(mag, -self.max_magnitude, self.max_magnitude)
                arrow.put_start_and_end_on(ORIGIN, mag * UP)
                point = graph.point_from_proportion(inverse_interpolate(x_min, x_max, x))
    ...
    def run_clock(self, time):
        self.play(ClockPassesTime(self.clock, run_time=time, hours_passed=time))

    def temp_func(self, x, t):
        new_x = TAU * x / 10
        return 50 + 20 * np.sum([amp * np.sin(freq * new_x) * np.exp(-(self.alpha * freq ** 2) * t) for freq, amp in self.freq_amplitude_pairs])
...
class TalkThrough1DHeatGraph(ShowEvolvingTempGraphWithArrows, SpecialThreeDScene):
    CONFIG = {'freq_amplitude_pairs': [(1, 0.5), (2, 1), (3, 0.5), (4, 0.3), (5, 0.3), (7, 0.2)], 'surface_resolution': 20, 'graph_slice_step': 10 / 20}
    ...
    def emphasize_graph(self):
        graph = self.graph
        q_marks = VGroup(*[OldTex('?').move_to(graph.point_from_proportion(a), UP).set_stroke(BLACK, 3, background=True) for a in np.linspace(0, 1, 20)])
        self.play(LaggedStart(*[Succession(FadeInFromLarge(q_mark), FadeOut(q_mark, DOWN)) for q_mark in q_marks]))
        self.wait()
    ...
    def emphasize_rod(self):
        ...
        self.play(LaggedStart(*[Rotating(piece, rate_func=smooth) for piece in alt_rod], run_time=2, lag_ratio=0.01), Write(word))
        self.remove(*alt_rod)
        self.wait()
        self.rod_word = word
    ...
    def show_x_axis(self):
        ...
        self.play(rod.set_opacity, 0.5, FadeIn(x_axis_label, UL), LaggedStartMap(FadeInFrom, x_numbers, lambda m: (m, UP)))
        self.wait()
        triangle = ArrowTip(start_angle=-90 * DEGREES, color=GREY_B)
        ...
        def get_graph_point():
            return graph.point_from_proportion(inverse_interpolate(self.graph_x_min, self.graph_x_max, get_x()))
        triangle.add_updater(lambda t: t.next_to(get_x_point(), UP))
        x_label = VGroup(OldTex('x'), OldTex('='), DecimalNumber(0, num_decimal_places=3, include_background_rectangle=True).scale(0.9))
        ...
        rod_piece = always_redraw(lambda: self.get_rod(get_x() - 0.05, get_x() + 0.05, n_pieces=1))
        self.play(FadeIn(triangle, UP), FadeIn(x_label), FadeIn(rod_piece), FadeOut(self.rod_word))
        for value in [np.exp(2), (np.sqrt(5) + 1) / 2]:
            self.play(x_tracker.set_value, value, run_time=2)
            self.wait()
        v_line = always_redraw(lambda: DashedLine(get_x_point(), get_graph_point(), color=self.rod_point_to_color(get_x_point())))
        graph_dot = Dot()
        graph_dot.add_updater(lambda m: m.set_color(self.rod_point_to_color(m.get_center())))
        ...
        self.play(TransformFromCopy(x_label[0], t_label[1]), FadeIn(t_label[0::2]), ShowCreation(v_line), GrowFromPoint(graph_dot, get_x_point()))
        self.add(t_label)
        self.wait()
        self.play(x_tracker.set_value, TAU, run_time=5)
        self.x_tracker = x_tracker
        self.graph_label_group = VGroup(v_line, rod_piece, triangle, x_label, graph_dot, t_label)
    ...
    def show_changes_over_time(self):
        ...
        time = 5
        self.play(FadeIn(clock), FadeIn(time_group))
        self.play(self.get_graph_time_change_animation(graph, time), ClockPassesTime(clock), ChangeDecimalToValue(time_label, time, rate_func=linear), ReplacementTransform(t_label, new_t_label, rate_func=squish_rate_func(smooth, 0.5, 0.7)), run_time=time)
        self.play(ShowCreationThenFadeAround(new_t_label.get_part_by_tex('t')))
        self.wait()
        self.play(FadeOut(clock), ChangeDecimalToValue(time_label, 0), VFadeOut(time_group), self.get_graph_time_change_animation(graph, new_time=0), run_time=1, rate_func=smooth)
    ...
    def show_surface(self):
        ...
        t_axis = NumberLine(x_min=t_min, x_max=t_max)
        origin = axes.c2p(0, 0)
        t_axis.shift(origin - t_axis.n2p(0))
        t_axis.add_numbers(*range(1, t_max + 1), direction=UP)
        ...
        graph_slices = VGroup(*[self.get_graph(time=t).shift(t * IN) for t in np.arange(0, t_max + step, step)])
        graph_slices.set_stroke(width=1)
        graph_slices.set_shade_in_3d(True)
        x_axis = self.axes.x_axis
        y = axes.c2p(0, 0)[1]
        surface_config = {'u_min': self.graph_x_min, 'u_max': self.graph_x_max, 'v_min': t_min, 'v_max': t_max, 'resolution': self.surface_resolution}
        input_plane = ParametricSurface(lambda x, t: np.array([x_axis.n2p(x)[0], y, t_axis.n2p(t)[2]]), **surface_config)
        input_plane.set_style(fill_opacity=0.5, fill_color=BLUE_B, stroke_width=0.5, stroke_color=WHITE)
        y_axis = axes.y_axis
        surface = ParametricSurface(lambda x, t: np.array([x_axis.n2p(x)[0], y_axis.n2p(self.temp_func(x, t))[1], t_axis.n2p(t)[2]]), **surface_config)
        surface.set_style(fill_opacity=0.1, fill_color=GREY_B, stroke_width=0.5, stroke_color=WHITE, stroke_opacity=0.5)
        curr_group = Group(*self.get_mobjects())
        curr_group.clear_updaters()
        self.set_camera_orientation(phi=90 * DEGREES)
        mobs = [curr_group, graph_slices, t_axis, input_plane, surface]
        for mob in mobs:
            self.orient_mobject_for_3d(mob)
        self.x_label.set_stroke(BLACK, 2, background=True)
        self.x_label[-1][0].fade(1)
        self.move_camera(phi=80 * DEGREES, theta=-85 * DEGREES, added_anims=[Write(input_plane), Write(t_axis), FadeOut(self.graph_label_group), self.rod.set_opacity, 1])
        self.begin_ambient_camera_rotation()
        self.add(*graph_slices, *self.get_mobjects())
        self.play(FadeIn(surface), LaggedStart(*[TransformFromCopy(graph, graph_slice) for graph_slice in graph_slices], lag_ratio=0.02))
        self.wait(4)
        self.axes = axes_copy
        slicing_plane = Rectangle(stroke_width=0, fill_color=WHITE, fill_opacity=0.2)
        slicing_plane.set_shade_in_3d(True)
        slicing_plane.replace(Line(axes_copy.c2p(0, 0), axes_copy.c2p(10, 100)), stretch=True)
        ...
        graph.add_updater(lambda g: g.become(get_time_slice(t_tracker.get_value())))
        self.orient_mobject_for_3d(final_rod)
        final_rod.shift(10 * UP)
        kw = {'run_time': 10, 'rate_func': linear}
        self.rod.save_state()
        self.play(ApplyMethod(t_tracker.set_value, 10, **kw), Transform(self.rod, final_rod, **kw), ApplyMethod(slicing_plane.shift, 10 * UP, **kw))
        self.wait()
        self.set_variables_as_attrs(t_axis, input_plane, surface, graph_slices, slicing_plane, t_tracker)

    def get_graph(self, time=0):
        graph = self.axes.get_graph(lambda x: self.temp_func(x, time), x_min=self.graph_x_min, x_max=self.graph_x_max, step_size=self.step_size)
        graph.time = time
        graph.color_using_background_image('VerticalTempGradient')
        return graph
    ...
    def get_graph_time_change_animation(self, graph, new_time, **kwargs):
        old_time = graph.time
        graph.time = new_time
        config = {'run_time': abs(new_time - old_time), 'rate_func': linear}
        config.update(kwargs)
        return UpdateFromAlphaFunc(graph, lambda g, a: g.become(self.get_graph(interpolate(old_time, new_time, a))), **config)
...
class ContrastXChangesToTChanges(TalkThrough1DHeatGraph):
    ...
    def emphasize_dimensions_of_input_space(self):
        ...
        self.play(Write(words), self.camera.phi_tracker.set_value, 60 * DEGREES, self.camera.theta_tracker.set_value, -90 * DEGREES, run_time=1)
    ...
    def reset_time_to_zero(self):
        self.play(self.t_tracker.set_value, 0, VFadeOut(self.slicing_plane), Restore(self.rod))

    def show_changes_with_x(self):
        alpha_tracker = ValueTracker(0)
        line = always_redraw(lambda: self.get_tangent_line(self.graph, alpha_tracker.get_value()))
        self.stop_ambient_camera_rotation()
        self.play(ShowCreation(line), FadeOut(self.input_plane_words), self.camera.phi_tracker.set_value, 80 * DEGREES, self.camera.theta_tracker.set_value, -90 * DEGREES)
        self.play(alpha_tracker.set_value, 0.425, run_time=5, rate_func=bezier([0, 0, 1, 1]))
        ...
        self.play(ShowCreation(dx_line), FadeIn(dx, LEFT))
        self.wait()
        self.play(ShowCreation(dT_line), FadeIn(dT, IN))
        self.wait()
        self.play(*map(FadeOut, [line, dx_line, dT_line, dx, dT]))

    def show_changes_with_t(self):
        slices = self.graph_slices
        slice_alpha = 0.075
        graph = VMobject()
        graph.set_points_smoothly([gs.point_from_proportion(slice_alpha) for gs in slices])
        graph.color_using_background_image('VerticalTempGradient')
        graph.set_shade_in_3d(True)
        alpha_tracker = ValueTracker(0)
        line = always_redraw(lambda: self.get_tangent_line(graph, alpha_tracker.get_value()))
        plane = Square()
        ...
        plane.shift(RIGHT)
        self.play(self.camera.theta_tracker.set_value, -20 * DEGREES, self.camera.frame_center.shift, 4 * LEFT)
        self.play(ShowCreation(graph.copy(), remover=True), FadeIn(plane, 6 * DOWN, run_time=2), VFadeIn(line), ApplyMethod(alpha_tracker.set_value, 1, run_time=8))
        self.add(graph)
        self.begin_ambient_camera_rotation(-0.02)
        self.camera.frame_center.add_updater(lambda m, dt: m.shift(0.05 * dt * RIGHT))
        self.play(FadeOut(line), FadeOut(plane))
        self.wait(30)
        self.t_graph = graph
...
class TransitionToTempVsTime(ContrastXChangesToTChanges):
    ...
    def construct(self):
        ...
        for mob in self.get_mobjects():
            mob.clear_updaters()
        self.stop_ambient_camera_rotation()
        self.move_camera(phi=90 * DEGREES, theta=0 * DEGREES, added_anims=[Rotate(y_axis, 90 * DEGREES, axis=OUT, about_point=y_axis.n2p(0)), FadeOut(VGroup(self.graph_slices, self.surface, self.slicing_plane, self.rod, self.graph, self.x_numbers, self.x_axis_label, self.t_graph)), self.camera.frame_center.move_to, 5 * LEFT])
        self.play(VGroup(x_axis, self.input_plane).stretch, 0, 0, {'about_point': y_axis.n2p(0)})
        self.play(t_axis.time_label.scale, 1 / 1.5, t_axis.time_label.next_to, t_axis, IN, MED_LARGE_BUFF, t_axis.numbers.shift, 0.7 * IN)
        self.wait()
...
class ShowDelTermsAsTinyNudges(TransitionToTempVsTime):
    ...
    def show_del_x(self):
        ...
        line_group = self.get_line_group(self.graph, x_tracker, dx_tracker, corner_index=0)
        ...
        for sym in syms:
            sym.add_updater(lambda m: m.next_to(m.line, m.direction, SMALL_BUFF))
            sym.rect.move_to(sym)
        self.move_camera(phi=80 * DEGREES, theta=-90 * DEGREES, added_anims=[self.camera.frame_center.move_to, ORIGIN])
        for sym in reversed(syms):
            self.play(FadeIn(sym, -sym.direction), ShowCreation(sym.line.copy(), remover=True))
            self.add(sym.line)
        ...
        self.play(FadeOut(syms), FadeOut(line_group))

    def show_del_t(self):
        t_tracker = ValueTracker(1)
        dt_tracker = ValueTracker(1)
        line_group = self.get_line_group(self.t_graph, t_tracker, dt_tracker, corner_index=1)
        ...
        del_t.add_updater(lambda m: m.set_height(min(0.5, m.line.get_length())))
        del_T.add_updater(lambda m: m.set_depth(min(0.5, m.line.get_length())))
        for sym in syms:
            sym.add_updater(lambda m: m.next_to(m.line, m.direction, SMALL_BUFF))
            sym.rect.move_to(sym)
        self.move_camera(phi=80 * DEGREES, theta=-10 * DEGREES, added_anims=[self.camera.frame_center.move_to, 5 * LEFT])
        for sym in reversed(syms):
            self.play(FadeIn(sym, -sym.direction), ShowCreation(sym.line.copy(), remover=True))
            self.add(sym.line)
        self.play(ShowCreation(tan_line))
        ...
        self.wait()
        self.add(line_group)
        self.play(dt_tracker.set_value, 0.01, run_time=5)
        self.play(FadeOut(syms), FadeOut(line_group))
    ...
    def get_line_group(self, graph, input_tracker, nudge_tracker, corner_index):
        ...
        def get_graph_point(x):
            return graph.point_from_proportion(inverse_interpolate(self.graph_x_min, self.graph_x_max, x))

        def get_corner(p1, p2):
            result = np.array(p1)
            result[corner_index] = p2[corner_index]
            return result
        line_group = VGroup(Line(color=WHITE), Line(color=RED), Line(color=WHITE, stroke_width=2))
...
class ShowCurvatureToRateOfChangeIntuition(ShowEvolvingTempGraphWithArrows):
    CONFIG = {'freq_amplitude_pairs': [(1, 0.7), (2, 1), (3, 0.5), (4, 0.3), (5, 0.3), (7, 0.2)], 'arrow_xs': [0.7, 3.8, 4.6, 5.4, 6.2, 9.3], 'arrow_scale_factor': 0.2, 'max_magnitude': 1.0, 'wait_time': 20}

    def let_play(self):
        arrows = self.arrows
        curves = VGroup(*[self.get_mini_curve(inverse_interpolate(self.graph_x_min, self.graph_x_max, x)) for x in self.arrow_xs])
        ...
        self.remove(arrows)
        self.play(ShowCreation(curves), LaggedStartMap(FadeIn, curve_words), self.y_label.set_fill, {'opacity': 0})
        self.wait()
    ...
    def get_mini_curve(self, alpha, d_alpha=0.02):
        result = VMobject()
        result.pointwise_become_partial(self.graph, alpha - d_alpha, alpha + d_alpha)
        return result

class DiscreteSetup(ShowEvolvingTempGraphWithArrows):
    CONFIG = {'step_size': 1, 'rod_piece_size_ratio': 1 / 3, 'dashed_line_stroke_opacity': 1.0, 'dot_radius': DEFAULT_DOT_RADIUS, 'freq_amplitude_pairs': [(1, 0.5), (2, 1), (3, 0.5), (4, 0.3), (5, 0.3), (7, 0.2), (21, 0.1)]}
    ...
    def discretize(self):
        ...
        self.remove(graph)
        self.play(ReplacementTransform(piecewise_graph, dots), Write(word, run_time=1))
        self.add(v_lines, word)
        self.play(x_axis.fade, 0.8, TransformFromCopy(x_axis.tick_marks[1:], rod_pieces), LaggedStartMap(ShowCreation, v_lines))
    ...
    def let_time_pass(self):
        ...
        self.add_clock()
        self.time_label.next_to(self.clock, DOWN)
        self.time_label.add_updater(lambda m: m.set_value(t_tracker.get_value()))
        dots.add_updater(lambda d: d.become(self.get_dots(t_tracker.get_value())))
        run_time = 5
        self.play(ClockPassesTime(self.clock, run_time=run_time, hours_passed=run_time))
        t_tracker.clear_updaters()
        t_tracker.set_value(run_time)
        self.wait()
        self.play(t_tracker.set_value, 0, FadeOut(self.clock), FadeOut(self.time_label))
    ...
    def show_nieghbor_rule(self):
        ...
        arrows = VGroup(Arrow(bottom, p1.get_center(), **kw), Arrow(bottom, p3.get_center(), **kw))
        arrows.set_color(WHITE)
        dot = Dot()
        dot.set_fill(GREY, opacity=0.2)
        dot.replace(p2)
        dot.scale(3)
        self.play(dot.scale, 0, dot.set_opacity, 0, FadeIn(point_label, DOWN))
        self.play(FadeIn(neighbors_label, DOWN), *map(GrowArrow, arrows))
        self.wait()
        self.play(d1.set_y, 3, d3.set_y, 3)

        def get_v():
            return 0.25 * np.sum([d1.get_y(), -2 * d2.get_y(), +d3.get_y()])
        v_vect_fader = ValueTracker(0)
        v_vect = always_redraw(lambda: Vector(get_v() * UP, color=temperature_to_color(get_v(), -2, 2)).shift(d2.get_center()).set_opacity(v_vect_fader.get_value()))
        d2.add_updater(lambda d, dt: d.shift(get_v() * dt * UP))
        self.add(v_vect)
        self.play(v_vect_fader.set_value, 1)
        self.wait(3)
        self.play(d1.set_y, 0, d3.set_y, 0)
        self.wait(4)
    ...
    def focus_on_three_points(self):
        ...
        x_labels, T_labels = [VGroup(*[OldTex('{}_{}'.format(s, i)) for i in [1, 2, 3]]).scale(0.8) for s in ('x', 'T')]
        ...
        self.play(ShowCreationThenFadeAround(pieces), FadeOut(self.dots[:self.index]), FadeOut(self.v_lines[:self.index]), FadeOut(self.rod_pieces[:self.index]), FadeOut(self.dots[self.index + 3:]), FadeOut(self.v_lines[self.index + 3:]), FadeOut(self.rod_pieces[self.index + 3:]))
        self.play(LaggedStartMap(FadeInFrom, x_labels, lambda m: (m, LEFT), lag_ratio=0.3, run_time=2))
        self.play(d3.set_y, 1, d2.set_y, 0.25, d1.set_y, 0)
        self.wait()
        self.play(LaggedStart(*[TransformFromCopy(xl, Tl) for xl, Tl in zip(x_labels, T_labels)], lag_ratio=0.3, run_time=2))
        self.wait()
        h_lines = VGroup(*map(self.get_h_line, dots))
        hl1, hl2, hl3 = h_lines
        average_pointer = ArrowTip(start_angle=0, length=0.2)
        average_pointer.set_color(YELLOW)
        average_pointer.stretch(0.25, 1)
        average_pointer.add_updater(lambda m: m.move_to(0.5 * (hl1.get_start() + hl3.get_start()), RIGHT))
        average_arrows = always_redraw(lambda: VGroup(*[Arrow(hl.get_start(), average_pointer.get_right(), color=WHITE, buff=0.0) for hl in [hl1, hl3]]))
        average_label = OldTex('{T_1', '+', 'T_3', '\\over', '2}')
        average_label.scale(0.5)
        average_label.add_updater(lambda m: m.next_to(average_pointer, LEFT, SMALL_BUFF))
        ...
        average_words.add_updater(lambda m: m.next_to(average_rect, UP, SMALL_BUFF))
        ...
        self.play(GrowFromPoint(hl1, hl1.get_end()), GrowFromPoint(hl3, hl3.get_end()), TransformFromCopy(T1, mini_T1), TransformFromCopy(T3, mini_T3), FadeOut(y_axis.numbers), y_axis.set_stroke, {'width': 1})
        self.play(FadeIn(average_pointer), Restore(mini_T1), Restore(mini_T3), FadeIn(average_label[1]), FadeIn(average_label[3:]), *map(GrowArrow, average_arrows))
        self.add(average_arrows, average_label)
        self.play(ShowCreation(average_rect), FadeIn(average_words))
        ...
        diff_value.add_updater(lambda m: m.set_value(y_axis.p2n(average_pointer.get_right()) - y_axis.p2n(d2.get_center())))
        diff_value.next_to(brace, DOWN)
        self.play(ReplacementTransform(average_label.deepcopy(), formula[1:1 + len(average_label)]), TransformFromCopy(T2, formula[-2]), FadeIn(formula[-3]), FadeIn(formula[-1]), FadeIn(formula[0]), GrowFromCenter(brace), FadeIn(diff_value))
        self.wait()
        self.play(FadeIn(self.v_vect))
        d2.add_updater(d2.movement_updater)
        self.wait(5)
        self.play(d3.set_y, 3, d1.set_y, 2.5, d2.set_y, -2)
        self.wait(5)
        self.play(d3.set_y, 1, d1.set_y, -1)
        self.wait(8)
        lhs = OldTex('{dT_2', '\\over', 'dt}', '=', '\\alpha')
        ...
        self.wait()
        self.play(FadeOut(brace), FadeOut(diff_value))
        self.lhs = lhs
        self.rhs = formula
    ...
    def show_difference_formula(self):
        ...
        new_rhs = OldTex('=', '{\\alpha', '\\over', '2}', '\\left(', '(', 'T_3', '-', 'T_2', ')', '-', '(', 'T_2', '-', 'T_1', ')', '\\right)')
        big_parens = VGroup(new_rhs.get_part_by_tex('\\left('), new_rhs.get_part_by_tex('\\right)'))
        ...
        def p2p_anim(mob1, mob2, tex, index=0):
            return TransformFromCopy(mob1.get_parts_by_tex(tex)[index], mob2.get_parts_by_tex(tex)[index])
        self.play(p2p_anim(lhs, new_rhs, '='), p2p_anim(rhs, new_rhs, '\\left('), p2p_anim(rhs, new_rhs, '\\right)'), p2p_anim(lhs, new_rhs, '\\alpha'), p2p_anim(rhs, new_rhs, '\\over'), p2p_anim(rhs, new_rhs, '2'))
        self.play(p2p_anim(rhs, new_rhs, 'T_3'), p2p_anim(rhs, new_rhs, '-'), p2p_anim(rhs, new_rhs, 'T_2'), FadeIn(new_rhs.get_parts_by_tex('(')[1]), FadeIn(new_rhs.get_parts_by_tex(')')[0]))
        self.play(p2p_anim(rhs, new_rhs, 'T_2', -1), p2p_anim(rhs, new_rhs, '-', -1), p2p_anim(rhs, new_rhs, 'T_1'), FadeIn(new_rhs.get_parts_by_tex('-')[1]), FadeIn(new_rhs.get_parts_by_tex('(')[2]), FadeIn(new_rhs.get_parts_by_tex(')')[1]))
        self.wait()
        self.rhs2 = new_rhs
        ...
        minus = OldTex('-')
        minus.move_to(Line(delta_T1.get_right(), delta_T2.get_left()))
        braces = VGroup(brace1, brace2)
        deltas = VGroup(delta_T1, delta_T2)
        kw = {'direction': LEFT, 'buff': SMALL_BUFF, 'min_num_quads': 2}
        lil_brace1 = always_redraw(lambda: Brace(Line(d1.get_left(), d2.get_left()), **kw))
        lil_brace2 = always_redraw(lambda: Brace(Line(d2.get_left(), d3.get_left()), **kw))
        ...
        for brace, delta in zip(lil_braces, lil_deltas):
            delta.brace = brace
            delta.add_updater(lambda d: d.next_to(d.brace, LEFT, SMALL_BUFF))
        ...
        double_difference_words = OldTexText('Difference of differences')
        double_difference_words.next_to(double_difference_brace, DOWN)
        self.play(GrowFromCenter(brace1), GrowFromCenter(lil_brace1), FadeIn(delta_T1), FadeIn(lil_delta_T1))
        self.wait()
        self.play(GrowFromCenter(brace2), GrowFromCenter(lil_brace2), FadeIn(delta_T2), FadeIn(lil_delta_T2))
        self.wait()
        self.play(Write(minus), GrowFromCenter(double_difference_brace), Write(double_difference_words))
        self.wait()
        self.braces = braces
        self.deltas = deltas
    ...
    def gut_check_new_interpretation(self):
        ...
        self.wait()
        self.play(d2.shift, MED_SMALL_BUFF * UP, rate_func=there_and_back)
        self.wait()
        self.play(d3.set_y, 3, d1.set_y, -0.5)
        self.wait(5)
        self.play(d3.set_y, 1.5, d1.set_y, -2)
        self.wait(5)
    ...
    def write_second_difference(self):
        ...
        second_difference_word = OldTexText("``Second difference''")
        second_difference_word.next_to(delta_delta, DOWN)
        self.play(FadeOut(dd_word, UP), FadeIn(delta_delta, UP))
        self.wait()
    ...
    def emphasize_final_expression(self):
        ...
        ao2 = old_ao2.copy()
        new_lhs = lhs[:-1]
        full_rhs = VGroup(lhs[-1], lhs[-2].copy(), rhs, rhs2, self.braces, self.deltas, self.delta_minus, self.double_difference_brace, old_dd, self.second_difference_word)
        new_rhs = VGroup(ao2, dd)
        new_rhs.arrange(RIGHT, buff=SMALL_BUFF)
        new_rhs.next_to(new_lhs, RIGHT)
        self.play(full_rhs.to_edge, DOWN, {'buff': LARGE_BUFF})
        self.play(TransformFromCopy(old_ao2, ao2), TransformFromCopy(old_dd, dd))
        self.play(ShowCreationThenFadeAround(VGroup(new_lhs, new_rhs)))
        self.wait()

    def get_sample_inputs(self):
        return np.arange(self.graph_x_min, self.graph_x_max + self.step_size, self.step_size)

    def get_points(self, time=0):
        return [self.axes.c2p(x, self.temp_func(x, t=time)) for x in self.get_sample_inputs()]

    def get_dots(self, time=0):
        points = self.get_points(time)
        dots = VGroup(*[Dot(point, radius=self.dot_radius) for point in points])
    ...
    def get_dot_dashed_line(self, dot, index, color=False):
        ...
        def get_line():
            ...
            result = DashedLine(p0, p1, stroke_width=2, color=WHITE, stroke_opacity=self.dashed_line_stroke_opacity)
            if color:
...
class ShowFinitelyManyX(DiscreteSetup):
    ...
    def construct(self):
        ...
        x_labels = VGroup(*[OldTex('x_{}'.format(i)).next_to(p, DOWN) for i, p in enumerate(points)])
...
class DiscreteGraphStillImageTenth(DiscreteGraphStillImage1):
    CONFIG = {'step_size': 0.1, 'dashed_line_stroke_opacity': 0.25, 'dot_radius': 0.04}

class DiscreteGraphStillImageHundredth(DiscreteGraphStillImage1):
    CONFIG = {'step_size': 0.01, 'dashed_line_stroke_opacity': 0.1, 'dot_radius': 0.01}

class TransitionToContinuousCase(DiscreteSetup):
    CONFIG = {'step_size': 0.1, 'tangent_line_length': 3, 'wait_time': 30}
    ...
    def add_graph(self):
        self.setup_graph()
        self.play(ShowCreation(self.graph, run_time=3))
        self.wait()

    def show_temperature_difference(self):
        x_tracker = ValueTracker(2)
        dx_tracker = ValueTracker(1)
        line_group = self.get_line_group(x_tracker, dx_tracker)
        ...
        self.play(ShowCreation(VGroup(dx_line, dT_line)), FadeIn(delta_T, LEFT))
        self.play(GrowFromCenter(brace), FadeInFromDown(fixed_distance))
        self.wait()
        self.play(FadeOut(delta_T, UP), FadeIn(dT_sym, DOWN), FadeOut(brace, UP), FadeOut(fixed_distance, UP), FadeIn(dx_sym, DOWN))
        self.add(line_group)
        self.play(dx_tracker.set_value, 0.01, run_time=5)
        self.wait()
        self.play(dx_tracker.set_value, 0.3)
        to_zero = OldTex('\\rightarrow 0')
        to_zero.match_height(dT_sym)
        to_zero.next_to(dT_sym, buff=SMALL_BUFF)
        ratio = OldTex('{\\partial T', '\\over', '\\partial x}')
        ratio[0].match_style(dT_sym)
        ratio.to_edge(UP)
        self.play(ShowCreationThenFadeAround(dT_sym, surrounding_rectangle_config={'buff': 0.05, 'stroke_width': 1}))
        self.play(GrowFromPoint(to_zero, dT_sym.get_right()))
        self.wait()
        self.play(TransformFromCopy(dT_sym, ratio.get_part_by_tex('\\partial T')), TransformFromCopy(dx_sym, ratio.get_part_by_tex('\\partial x')), Write(ratio.get_part_by_tex('\\over')))
        self.play(ShowCreation(tan_line.copy().set_stroke(width=2), remover=True), FadeOut(to_zero))
        tan_line.set_stroke(width=2)
        self.wait()
        x0 = x_tracker.get_value()
        dx = dx_tracker.get_value()
        v_line, lv_line, rv_line = v_lines = VGroup(*[self.get_v_line(x) for x in [x0, x0 - dx, x0 + dx]])
        v_lines[1:].set_color(BLUE)
        self.play(ShowCreation(v_line))
        self.play(TransformFromCopy(v_line, lv_line), TransformFromCopy(v_line, rv_line))
        self.wait()
        self.play(FadeOut(v_lines[1:]), ApplyMethod(dx_tracker.set_value, 0.01, run_time=2))
    ...
    def show_second_derivative(self):
        ...
        deriv = self.deriv
        v_line = self.v_line
        deriv_of_deriv = OldTex('{\\partial', '\\left(', '{\\partial T', '\\over', '\\partial x}', '\\right)', '\\over', '\\partial x}')
        ...
        self.play(ReplacementTransform(deriv, inner_deriv), Write(VGroup(*filter(lambda m: m not in inner_deriv, deriv_of_deriv))))
        v_line.add_updater(lambda m: m.become(self.get_v_line(x_tracker.get_value())))
        for change in [-0.1, 0.1]:
            self.play(x_tracker.increment_value, change, run_time=3)
        second_deriv = OldTex('{\\partial^2 T', '\\over', '\\partial x^2}')
        ...
        self.play(Write(eq))
        self.play(TransformFromCopy(deriv_of_deriv.get_parts_by_tex('\\partial')[:2], second_deriv.get_parts_by_tex('\\partial^2 T')))
        self.play(Write(second_deriv.get_part_by_tex('\\over')), TransformFromCopy(deriv_of_deriv.get_parts_by_tex('\\partial x'), second_deriv.get_parts_by_tex('\\partial x')))
        self.wait()
    ...
    def show_curvature_examples(self):
        ...
        self.wait()
        self.play(x_tracker.set_value, 3.8, run_time=4)
        self.wait()
        x_tracker.set_value(6.2)
        self.wait()
        self.play(x_tracker.set_value, 6.4, run_time=4)
        self.wait()
        dx = 0.2
        neighbor_lines = always_redraw(lambda: VGroup(*[self.get_v_line(x_tracker.get_value() + u * dx, line_class=Line) for u in [-1, 1]]))
        ...
        self.play(x_tracker.set_value, 5, run_time=5, rate_func=lambda t: smooth(t, 3))
        v_line.clear_updaters()
        self.play(FadeOut(v_line), FadeOut(neighbor_lines))
        self.wait()
    ...
    def show_time_changes(self):
        ...
        self.add(time_label)
        self.add_arrows()
        self.play(ClockPassesTime(clock, run_time=self.wait_time, hours_passed=self.wait_time))

    def get_v_line(self, x, line_class=DashedLine, stroke_width=2):
        axes = self.axes
        graph = self.graph
        line = line_class(axes.c2p(x, 0), graph.point_from_proportion(inverse_interpolate(self.graph_x_min, self.graph_x_max, x)), stroke_width=stroke_width)
        return line

    def get_line_group(self, x_tracker, dx_tracker, dx_tex='\\partial x', dT_tex='\\partial T', max_sym_width=0.5):
        graph = self.graph
        get_x = x_tracker.get_value
        ...
        def update_group(group):
            ...
            x = get_x()
            dx = get_dx()
            p0, p2 = [graph.point_from_proportion(inverse_interpolate(self.graph_x_min, self.graph_x_max, x)) for x in [x, x + dx]]
            p1 = np.array([p2[0], *p0[1:]])
            ...
            dxs.next_to(dxl, -dTl.get_vector(), SMALL_BUFF)
            dTs.next_to(dTl, dxl.get_vector(), SMALL_BUFF)
        group.add_updater(update_group)
        return group

class ShowManyVLines(TransitionToContinuousCase):
    CONFIG = {'wait_time': 20, 'max_denom': 10, 'x_step': 0.025}
    ...
    def add_v_lines(self):
        axes = self.axes
        v_lines = always_redraw(lambda: VGroup(*[self.get_v_line(x, line_class=Line, stroke_width=0.5) for x in np.arange(0, 10, self.x_step)]))
        ...
        x_eq.add_updater(lambda m: m[1].set_value(axes.x_axis.p2n(x_pointer.get_top())))
        x_eq.add_updater(lambda m: m.next_to(x_pointer, DOWN, SMALL_BUFF, submobject_to_align=x_eq[0]))
        self.add(x_pointer, x_eq)
        self.play(Write(group, remover=True, lag_ratio=self.x_step / 2, run_time=6), ApplyMethod(x_pointer.next_to, axes.c2p(10, 0), DOWN, {'buff': 0}, rate_func=linear, run_time=5))
        self.add(v_lines)
        x_eq.clear_updaters()
        self.play(FadeOut(x_eq), FadeOut(x_pointer))

class ShowNewtonsLawGraph(Scene):
    CONFIG = {'k': 0.2, 'initial_water_temp': 80, 'room_temp': 20, 'delta_T_color': YELLOW}
    ...
    def setup_axes(self):
        axes = Axes(x_min=0, x_max=10, y_min=0, y_max=100, y_axis_config={'unit_size': 0.06, 'tick_frequency': 10}, center_point=5 * LEFT + 2.5 * DOWN)
        x_axis = axes.x_axis
        y_axis = axes.y_axis
    ...
    def show_temperatures(self):
        ...
        water_words = OldTexText('Initial water\\\\temperature')
        ...
        self.play(FadeIn(water_dot, RIGHT), GrowArrow(water_arrow), Write(water_words), run_time=1)
    ...
    def show_equation(self):
        ...
        equation = OldTex('{d ({\\Delta T}) \\over dt} = -k \\cdot {\\Delta T}', tex_to_color_map={'{\\Delta T}': self.delta_T_color, '-k': WHITE, '=': WHITE})
        ...
        words = OldTexText('Rate of change', 'is proportional to', 'itself')
        words.scale(0.7)
