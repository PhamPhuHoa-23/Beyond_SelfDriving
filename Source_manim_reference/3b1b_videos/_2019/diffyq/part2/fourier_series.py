...
class FourierCirclesScene(Scene):
    CONFIG = {'n_vectors': 10, 'big_radius': 2, 'colors': [BLUE_D, BLUE_C, BLUE_E, GREY_BROWN], 'circle_style': {'stroke_width': 2}, 'vector_config': {'buff': 0, 'max_tip_length_to_length_ratio': 0.35, 'fill_opacity': 0.75}, 'circle_config': {'stroke_width': 1, 'stroke_opacity': 0.75}, 'base_frequency': 1, 'slow_factor': 0.25, 'center_point': ORIGIN, 'parametric_function_step_size': 0.01, 'drawn_path_color': YELLOW, 'drawn_path_stroke_width': 2}
    ...
    def get_vector_sum_path(self, vectors, color=YELLOW):
        ...
        path = ParametricCurve(lambda t: center + reduce(op.add, [complex_to_R3(coef * np.exp(TAU * 1j * freq * t)) for coef, freq in zip(coefs, freqs)]), t_min=0, t_max=1, color=color, step_size=self.parametric_function_step_size)
        return path
    ...
    def add_path_fader(self, path, fade_rate=0.2):
        ...
        def update_path(path_, dt):
            ...
            path_.set_stroke(width=stroke_width * fade_factors, opacity=stroke_opacity * fade_factors)
            return path_
    ...
    def get_y_component_wave(self, vectors, left_x=1, color=PINK, n_copies=2, right_shift_rate=5):
        ...
        wave = ParametricCurve(lambda t: op.add(right_shift_rate * t * LEFT, path.function(t)[1] * UP), t_min=path.t_min, t_max=path.t_max, color=color)
...
class FourierSeriesIntroBackground4(FourierCirclesScene):
    CONFIG = {'n_vectors': 4, 'center_point': 4 * LEFT, 'run_time': 30, 'big_radius': 1.5}
...
class FourierOfPiSymbol(FourierCirclesScene):
    CONFIG = {'n_vectors': 101, 'center_point': ORIGIN, 'slow_factor': 0.1, 'n_cycles': 1, 'tex': '\\pi', 'start_drawn': False, 'max_circle_stroke_width': 1}
...
class FourierOfTexPaths(FourierOfPiSymbol):
    CONFIG = {'n_vectors': 100, 'name_color': WHITE, 'animated_name': 'Abc', 'conjoined': False, 'time_per_symbol': 5, 'slow_factor': 1 / 5, 'parametric_function_step_size': 0.001, 'max_circle_stroke_width': 1.0, 'vector_config': {'buff': 0, 'fill_opacity': 0.75, 'thickness': 0.02, 'max_tip_length_to_length_ratio': 0.5, 'max_width_to_length_ratio': 0.05}}
        ...
        for path in name.family_members_with_points():
            ...
            for subpath in subpaths:
                ...
                if vectors is None:
                    ...
                    vectors.scale(0)
                    circles.scale(0)
                self.play(Transform(vectors, static_vectors, remover=True), Transform(circles, static_circles, remover=True), frame.set_max_width, 1.25 * name.get_width(), frame.move_to, path)
                self.add(drawn_path, new_vectors, new_circles)
                self.vector_clock.set_value(0)
                self.play(ShowCreation(drawn_path), rate_func=linear, run_time=self.time_per_symbol)
...
class FourierOfTrebleClef(FourierOfPiSymbol):
    CONFIG = {'n_vectors': 101, 'run_time': 10, 'start_drawn': True, 'file_name': 'TrebleClef', 'height': 7.5}
...
class FourierOfIP(FourierOfTrebleClef):
    CONFIG = {'file_name': 'IP_logo2', 'height': 6, 'n_vectors': 100}
...
class FourierOfN(FourierOfTrebleClef):
    CONFIG = {'height': 6, 'n_vectors': 1000}

    def get_shape(self):
        return OldTex('N')

class FourierNailAndGear(FourierOfTrebleClef):
    CONFIG = {'height': 6, 'n_vectors': 200, 'run_time': 100, 'slow_factor': 0.01, 'parametric_function_step_size': 0.0001, 'arrow_config': {'tip_length': 0.1, 'stroke_width': 2}}
...
class FourierBatman(FourierOfTrebleClef):
    CONFIG = {'height': 4, 'n_vectors': 100, 'run_time': 10, 'arrow_config': {'tip_length': 0.1, 'stroke_width': 2}}
...
class FourierNDQ(FourierOfTrebleClef):
    CONFIG = {'height': 4, 'n_vectors': 1000, 'run_time': 10, 'arrow_config': {'tip_length': 0.1, 'stroke_width': 2}}
...
class FourierGoogleG(FourierOfTrebleClef):
    CONFIG = {'n_vectors': 10, 'height': 5, 'g_colors': ['#4285F4', '#DB4437', '#F4B400', '#0F9D58']}
...
class ExplainCircleAnimations(FourierCirclesScene):
    CONFIG = {'n_vectors': 100, 'center_point': 2 * DOWN, 'n_top_circles': 9, 'path_height': 3}
    ...
    def show_frequencies(self):
        ...
        ld, rd = [OldTex('\\dots') for x in range(2)]
        ...
        self.play(Write(freq_word), LaggedStartMap(ShowCreationThenFadeAround, freq_numbers))
        self.wait(2)
    ...
    def show_examples_for_frequencies(self):
        ...
        c1, c2, c3 = [list(filter(lambda c: c.freq == k, top_circles))[0] for k in (1, 2, 3)]
        neg_circles = VGroup(*filter(lambda c: c.freq < 0, top_circles))
        for c in [c1, c2, c3, *neg_circles]:
            c.rect = SurroundingRectangle(c)
        self.play(ShowCreation(c2.rect), WiggleOutThenIn(c2.number))
        self.wait(2)
        self.play(ReplacementTransform(c2.rect, c1.rect))
        self.play(FadeOut(c1.rect))
        self.wait()
        self.play(ShowCreation(c3.rect), WiggleOutThenIn(c3.number))
        self.play(FadeOut(c3.rect))
        self.wait(2)
        self.play(LaggedStart(*[ShowCreationThenFadeOut(c.rect) for c in neg_circles]))
        self.wait(3)
    ...
    def show_as_vectors(self):
        ...
        self.play(FadeIn(top_vectors), top_circles.set_opacity, 0)
        self.wait(3)
    ...
    def show_vector_sum(self):
        ...
        plane = self.plane = NumberPlane(x_min=-3, x_max=3, y_min=-2, y_max=2, axis_config={'stroke_color': GREY_B})
        plane.set_stroke(width=1)
        plane.fade(0.5)
        plane.move_to(self.center_point)
        self.play(FadeOut(self.drawn_path), FadeOut(self.circles), self.slow_factor_tracker.set_value, 0.05)
        ...
        for tracker in trackers:
            ...
            self.play(UpdateFromAlphaFunc(tracker, lambda t, a: t.move_to(interpolate(start_point, tracker.new_location_func(), a)), run_time=2))
        ...
        self.add(self.top_circles, self.freq_numbers, self.path)
        self.add_circles()
        for tc in self.top_circles:
            for c in self.circles:
                if c.freq == tc.freq:
                    tc.rotate(angle_of_vector(c.get_start() - c.get_center()) - angle_of_vector(tc.get_start() - tc.get_center()))
        self.wait(10)
    ...
    def tweak_starting_vectors(self):
        ...
        self.wait()
        self.play(ReplacementTransform(top_circles, new_top_circles), ReplacementTransform(circles, new_circles), FadeOut(path), run_time=3)
        new_drawn_path = self.get_drawn_path(new_circles, stroke_width=4)
