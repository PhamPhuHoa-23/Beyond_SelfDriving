...
class IntroduceIMO(Scene):
    CONFIG = {'num_countries': 130, 'use_real_images': True, 'include_labels': False, 'camera_config': {'background_color': GREY_E}, 'random_seed': 6, 'year': 2019, 'n_flag_rows': 10, 'reorganize_students': True}
    ...
    def add_title(self):
        title = OldTexText('International ', 'Mathematical ', 'Olympiad')
        title.scale(1.25)
    ...
    def show_flags(self):
        ...
        self.play(LaggedStartMap(FadeInFromDown, random_flags, run_time=2, lag_ratio=0.03))
    ...
    def show_students(self):
        ...
        student_groups.target.arrange_in_grid(n_rows=self.n_flag_rows, buff=SMALL_BUFF)
        student_groups.target.match_height(flags)
        student_groups.target.match_y(flags)
        student_groups.target.to_edge(RIGHT, buff=0.25)
        self.play(LaggedStart(*[ReplacementTransform(student.preimage, student) for student in all_students], run_time=2, lag_ratio=0.2))
        self.wait()
        if self.reorganize_students:
            self.play(MoveToTarget(student_groups), flags.space_out_submobjects, 0.75, flags.to_edge, LEFT, MED_SMALL_BUFF)
            self.wait()
    ...
    def move_title(self):
        ...
        self.play(MoveToTarget(title), MoveToTarget(logo))
        self.wait()
    ...
    def isolate_usa(self):
        ...
        us_flag.label.next_to(us_flag, DOWN, buff=0.2 * us_flag.get_height())
        ...
        self.remove(us_flag)
        self.play(LaggedStart(*[FadeOut(flag, DOWN) for flag in random_flags], lag_ratio=0.05, run_time=1.5), MoveToTarget(us_flag), student_groups[1:].fade, 0.9)
        self.wait()
    ...
    def get_flags(self):
        ...
        for country, count in countries_with_counts:
            ...
            alt_names = [('united states of america', 'united states'), ("people's republic of china", 'china'), ('macau', 'macao'), ('syria', 'syrian arab republic'), ('north macedonia', 'macedonia, the former yugoslav republic of'), ('tanzania', 'united republic of tanzania'), ('vietnam', 'viet nam'), ('ivory coast', "cote d'ivoire")]
        ...
        images.arrange_in_grid(n_rows=n_rows, buff=1.25)
...
class ShowTinyTao(IntroduceIMO):
    CONFIG = {'reorganize_students': False}
    ...
    def construct(self):
        ...
        self.play(image.set_height, 5, image.to_corner, DR, {'buff': MED_SMALL_BUFF})
        self.wait()
...
class ShowTest(Scene):
    ...
    def introduce_test(self):
        ...
        time_labels = VGroup(OldTexText('Day 1', ': 4.5 hours'), OldTexText('Day 2', ': 4.5 hours'))
        ...
        self.play(LaggedStart(FadeIn(test[0], 2 * RIGHT), FadeIn(test[1], 2 * LEFT), lag_ratio=0.3))
        self.wait()
        self.play(MoveToTarget(test, lag_ratio=0.2), FadeIn(day_labels, UP, lag_ratio=0.2))
        self.wait()
        self.play(*map(Restore, day_labels), FadeIn(hour_labels, LEFT))
        self.wait()
        self.play(FadeOut(test[1]), FadeOut(time_labels[1]), LaggedStartMap(ShowCreation, problem_rects), run_time=1)
        self.play(LaggedStart(*[FadeIn(word, LEFT) for word in proof_words]), LaggedStart(*[GrowArrow(arrow) for arrow in proof_arrows]))
        self.wait()
        self.play(FadeIn(scores))
        self.play(LaggedStart(*[ChangeDecimalToValue(score[0], 7) for score in scores], lag_ratio=0.2, rate_func=rush_into))
        self.wait()
        self.scores = scores
    ...
    def get_test(self):
        group = Group(ImageMobject('imo_2011_p1'), ImageMobject('imo_2011_p2'))
        group.set_height(6)
...
class USProcessAlt(IntroduceIMO):
    ...
    def show_tests(self):
        tests = VGroup(self.get_test(['American ', 'Mathematics ', 'Contest'], n_questions=25, time_string='75 minutes', hours=1.25, n_students=250000), self.get_test(['American ', 'Invitational ', 'Math ', 'Exam'], n_questions=15, time_string='3 hours', hours=3, n_students=12000), self.get_test(['U', 'S', 'A ', 'Math ', 'Olympiad'], n_questions=6, time_string='$2 \\times 4.5$ hours', hours=4.5, n_students=500), self.get_test(['Mathematical ', 'Olympiad ', 'Program'], n_questions=None, time_string='3 weeks', hours=None, n_students=60))
        ...
        for test in tests:
            ...
            self.play(Write(test.name), *next_anims, run_time=1)
            self.wait()
            ...
            next_anims = [MoveToTarget(test), GrowArrow(last_arrow)]
        ...
        self.revert_to_original_skipping_status()
        self.play(LaggedStartMap(FadeInFrom, tests, lambda m: (m, LEFT)), LaggedStartMap(GrowArrow, arrows[:-1]), lag_ratio=0.4)
        self.wait()
        self.tests = tests
    ...
    def show_imo(self):
        ...
        for student, color in zip(students, colors):
            ...
            student.fade(1)
        self.play(FadeInFromDown(group), LaggedStartMap(Restore, students, run_time=3, lag_ratio=0.3))
    ...
    def animate_name_abbreviation(self, test):
        ...
        self.play(FadeIn(test.rect), MoveToTarget(name), FadeIn(test.ns_label))
        test.remove(name)
        test.add(short_name)
        self.remove(name)
        self.add(short_name)
    ...
    def get_test(self, name_parts, n_questions, time_string, hours, n_students):
        ...
        if hours:
            sector = Sector(start_angle=TAU / 4, angle=-TAU * (hours / 12), outer_radius=clock.get_width() / 2, arc_center=clock.get_center())
        ...
        result = VGroup(name, nq_label, t_label, ns_label)
        result.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        rect = SurroundingRectangle(result, buff=MED_SMALL_BUFF)
        rect.set_width(result[1:].get_width() + MED_LARGE_BUFF, about_edge=LEFT, stretch=True)
...
class Describe2011IMO(IntroduceIMO):
    CONFIG = {'year': 2011, 'use_real_images': True, 'n_flag_rows': 10, 'student_data': [[1, 'Lisa Sauermann', 'de', [7, 7, 7, 7, 7, 7]], [2, 'Jeck Lim', 'sg', [7, 5, 7, 7, 7, 7]], [3, 'Lin Chen', 'cn', [7, 3, 7, 7, 7, 7]], [4, 'Jun Jie Joseph Kuan', 'sg', [7, 7, 7, 7, 7, 1]], [4, 'David Yang', 'us', [7, 7, 7, 7, 7, 1]], [6, 'Jie Jun Ang', 'sg', [7, 7, 7, 7, 7, 0]], [6, 'Kensuke Yoshida', 'jp', [7, 6, 7, 7, 7, 1]], [6, 'Raul Sarmiento', 'pe', [7, 7, 7, 7, 7, 0]], [6, 'Nipun Pitimanaaree', 'th', [7, 7, 7, 7, 7, 0]]]}
    ...
    def add_flags_and_students(self):
        ...
        student_groups.arrange_in_grid(n_rows=self.n_flag_rows, buff=SMALL_BUFF)
        ...
        all_students = VGroup(*it.chain(*[[student for student in group if student.get_fill_opacity() > 0] for group in student_groups]))
        student_counter = VGroup(Integer(0), OldTexText('Participants'))
        ...
        self.play(ShowIncreasingSubsets(all_students), run_time=3)
        self.wait()
        self.add(flag_counter)
        self.play(ShowIncreasingSubsets(flags), run_time=3)
        self.wait()
    ...
    def comment_on_primality(self):
        ...
        numbers = VGroup(self.title[0], self.student_counter[0], self.flag_counter[0])
        ...
        self.play(FadeIn(full_rect), randy.change, 'sassy', VFadeIn(randy))
        self.play(ShowCreation(lines), randy.change, 'pondering')
        self.play(Blink(randy))
        self.play(randy.change, 'thinking', LaggedStart(*[FadeIn(word, UP) for word in words], run_time=3, lag_ratio=0.5))
        self.play(Blink(randy))
        self.play(FadeOut(randy), FadeOut(words))
    ...
    def show_top_three_scorers(self):
        ...
        top_row = grid.rows[1]
        self.play(LaggedStartMap(FadeOutAndShiftDown, students_to_fade), LaggedStartMap(FadeOutAndShiftDown, flags_to_fade), ChangeDecimalToValue(student_counter[0], 1), FadeOut(flag_counter), run_time=2)
        student_counter[1][0][-1].fade(1)
        self.play(Write(top_row[0]), ReplacementTransform(student, top_row[1][1]), flag.replace, top_row[1][0])
        self.remove(flag)
        self.add(top_row[1])
        self.play(LaggedStartMap(FadeIn, title_row[2:]), LaggedStartMap(FadeIn, top_row[2:]))
        self.wait()
        self.play(LaggedStart(*[FadeIn(row, UP) for row in grid.rows[2:4]]), LaggedStart(*[ShowCreation(line) for line in grid.h_lines[:2]]), lag_ratio=0.5)
        self.wait()
        self.play(ShowCreationThenFadeAround(Group(title_row[3], grid.rows[3][3])))
        self.wait()
        student_counter.clear_updaters()
        self.play(FadeOut(self.title, UP), FadeOut(student_counter, UP), grid.rows[:4].shift, 3 * UP, grid.h_lines[:3].shift, 3 * UP)
        remaining_rows = grid.rows[4:]
        remaining_lines = grid.h_lines[3:]
        Group(remaining_rows, remaining_lines).shift(3 * UP)
        self.play(LaggedStartMap(FadeInFrom, remaining_rows, lambda m: (m, UP)), LaggedStartMap(ShowCreation, remaining_lines), lag_ratio=0.3, run_time=2)
        self.wait()
    ...
    def get_score_grid(self):
        ...
        students = Group(*[Group(flag.next_to(student, LEFT, buff=0.2), student) for flag, student in zip(flags, students)])
        ...
        titles = VGroup(*[VectorizedPoint(), VectorizedPoint(), *[OldTexText('P{}'.format(i)) for i in range(1, 7)]])
        titles.arrange(RIGHT, buff=MED_LARGE_BUFF)
        titles[2:].shift(students.get_width() * RIGHT)
        rows = Group(titles, *[Group(rank, student, *score_row) for rank, flag, student, score_row in zip(ranks, flags, students, score_rows)])
        rows.arrange(DOWN)
        rows.to_edge(UP)
        for row in rows:
...
class AskWhatsOnTest(ShowTest, MovingCameraScene):
    ...
    def ask_about_questions(self):
        ...
        self.play(FadeOut(scores), FadeOut(proof_words), MoveToTarget(arrows), Write(question))
        self.wait()
        self.play(FadeIn(research, DOWN), question.shift, 2 * UP)
        self.wait()
        ...
        self.play(FadeOut(question), FadeIn(randy), FadeInFromDown(clock))
        self.play(randy.change, 'pondering', ClockPassesTime(clock, run_time=5, hours_passed=5))
        self.play(ClockPassesTime(clock, run_time=2, hours_passed=2), VFadeOut(clock), Blink(randy), VFadeOut(randy), LaggedStartMap(FadeOut, VGroup(research, *arrows, *self.problem_rects, self.time_labels[0])))
        ...
        self.wait()
        self.play(FadeIn(big_rect), MoveToTarget(frame, run_time=6))
        self.wait()
...
class ReadQuestions(Scene):
    ...
    def construct(self):
        ...
        clumps = VGroup(*[lines[i:j] for i, j in zip([0] + partial_sums, partial_sums)])
        faders = []
        for clump in clumps:
            rects = VGroup()
            for line in clump:
                ...
                rects.add(rect)
            self.play(ShowCreation(clump, run_time=2), FadeIn(rects), *faders)
            self.wait()
            faders = [FadeOut(clump), FadeOut(rects)]
        self.play(*faders)
        self.wait()

class WindmillScene(Scene):
    CONFIG = {'dot_config': {'fill_color': GREY_B, 'radius': 0.05, 'background_stroke_width': 2, 'background_stroke_color': BLACK}, 'windmill_style': {'stroke_color': RED, 'stroke_width': 2, 'background_stroke_width': 3, 'background_stroke_color': BLACK}, 'windmill_length': 2 * FRAME_WIDTH, 'windmill_rotation_speed': 0.25, 'hit_sound': 'pen_click.wav', 'leave_shadows': False}

    def get_random_point_set(self, n_points=11, width=6, height=6):
        return np.array([[-width / 2 + np.random.random() * width, -height / 2 + np.random.random() * height, 0] for n in range(n_points)])
    ...
    def update_counter(self, counter):
        ...
        counter.will_be_pivot = dist < 1e-06
...
class IntroduceWindmill(WindmillScene):
    CONFIG = {'final_run_time': 60, 'windmill_rotation_speed': 0.5}
    ...
    def add_points(self):
        ...
        dots.set_height(3)
        braces = VGroup(Brace(dots, LEFT), Brace(dots, RIGHT))
        ...
        self.play(FadeIn(S_eq), FadeIn(braces[0], RIGHT), FadeIn(braces[1], LEFT))
        self.play(LaggedStartMap(FadeInFromLarge, dots))
        self.wait()
        self.play(S.next_to, dots, LEFT, {'buff': 2, 'aligned_edge': UP}, FadeOut(braces), FadeOut(eq))
        self.S_label = S
        self.dots = dots
    ...
    def exclude_colinear(self):
        ...
        self.add(line, dots)
        self.play(ShowCreation(line), FadeIn(words, LEFT), dots[-1].set_color, RED)
        self.wait()
        self.play(FadeOut(line), FadeOut(words))
        self.play(FadeOut(dots[-1], 3 * RIGHT, path_arc=-PI / 4, rate_func=running_start))
        dots.remove(dots[-1])
        self.wait()
    ...
    def add_line(self):
        ...
        l_label.scale(1.5)
        p_label = OldTex('P')
        l_label.next_to(p0 + 2 * normalize(windmill.get_vector()), RIGHT)
        ...
        self.play(GrowFromCenter(windmill), FadeIn(l_label, DL))
        self.wait()
        self.play(TransformFromCopy(pivot_dot, p_label), GrowFromCenter(pivot_dot), dots.set_color, WHITE)
        self.wait()
        self.play(*map(ShowCreation, arcs))
        self.wait()
        next_pivot, angle = self.next_pivot_and_angle(windmill)
        self.play(*[Rotate(mob, -0.99 * angle, about_point=p0, rate_func=linear) for mob in [windmill, arcs, l_label]], VFadeOut(l_label))
        self.add_sound(self.hit_sound)
    ...
    def switch_pivots(self):
        ...
        self.play(FadeIn(q_label, LEFT), FadeOut(p_label), FadeOut(arcs))
        self.wait()
    ...
    def continue_and_count(self):
        ...
        l_label.scale(1.5)
        l_label.match_color(windmill)
        l_label.next_to(windmill.get_center() + -3 * normalize(windmill.get_vector()), DR, buff=SMALL_BUFF)
        self.play(FadeIn(p_label, UL))
        self.play(FadeIn(l_label, LEFT))
        self.wait()
        self.add(windmill.copy().fade(0.75), pivot_dot.copy().fade(0.75))
...
class ContrastToOtherOlympiadProblems(AskWhatsOnTest):
    ...
    def zoom_to_other_questions(self):
        ...
        self.add(test)
        self.play(FadeIn(big_rect), MoveToTarget(frame, run_time=3))
        self.wait()
        for i in [2, 0, 3, 5]:
...
class WindmillExample30Points(WindmillScene):
    CONFIG = {'n_points': 30, 'random_seed': 0, 'run_time': 60, 'counter_config': {'counter_height': 0.15, 'buff': 0.1}}
...
class WindmillExample15Points(WindmillExample30Points):
    CONFIG = {'n_points': 15, 'run_time': 60, 'random_seed': 2, 'counter_config': {'counter_height': 0.25, 'buff': 0.1}}
...
class SpiritOfIMO(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, light_bulbs, run_time=1), LaggedStartMap(LaggedStartMap, lights, lambda l: (VFadeInThenOut, l), run_time=3), randy.change, 'thinking')
        self.wait()
        self.pi_creature_thinks("Oh, I've\\\\seen this...", target_mode='surprised')
        self.wait(3)

    def get_light(self, point):
        radii = np.arange(0, 5, 0.1)
        result = VGroup(*[Annulus(inner_radius=r1, outer_radius=r2, arc_center=point, fill_opacity=1 / (r1 + 1) ** 2, fill_color=YELLOW) for r1, r2 in zip(radii[1:], radii[2:])])
        return result
...
class HarderThanExpected(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(FadeInFromDown(title), ShowCreation(line), self.teacher.change, 'raise_right_hand', self.change_students('pondering', 'confused', 'sassy'))
        self.wait()
        self.play(FadeIn(words, UP), self.change_students(*3 * ['horrified']))
        self.wait(3)
...
class TraditionalDifficulty(ContrastToOtherOlympiadProblems):
    ...
    def construct(self):
        ...
        p_words = VGroup(OldTexText('Doable', color=GREEN), OldTexText('Challenging', color=YELLOW), OldTexText('Brutal', color=RED))
        p_words.add(*p_words.copy())
        for rect, word, label in zip(rects, p_words, p_labels):
            word.next_to(rect, UP)
            label.match_color(word)
        self.add(test[0])
        self.play(FadeIn(harder_words), GrowArrow(arrow), LaggedStart(*[FadeIn(p, UP) for p in p_labels[:3]]), LaggedStartMap(ShowCreation, rects[:3]))
        self.wait()
        self.play(FadeIn(test[1]), FadeIn(p_labels[3:]), FadeIn(rects[3:]), FadeOut(harder_words), FadeOut(arrow))
        self.wait()
        self.add(big_rect, p_labels[0], p_labels[3])
        self.play(FadeIn(big_rect), FadeOut(rects), FadeOut(p_labels[1:3]), FadeOut(p_labels[4:]), FadeInFromDown(p_words[0::3]))
        self.wait()
        self.play(Transform(big_rect, big_rects[1]), FadeOut(p_labels[0::3]), FadeIn(p_labels[1::3]), FadeOut(p_words[0::3], DOWN), FadeIn(p_words[1::3], UP))
        self.wait()
        self.play(Transform(big_rect, big_rects[2]), FadeOut(p_labels[1::3]), FadeIn(p_labels[2::3]), FadeOut(p_words[1::3], DOWN), FadeIn(p_words[2::3], UP))
        self.wait()

class PerfectScoreData(Describe2011IMO):
    CONFIG = {'n_students': 563, 'n_perfect_scores_per_problem': [345, 22, 51, 267, 170, 6], 'full_bar_width': 7}
    ...
    def show_total_number_of_students(self):
        ...
        self.play(self.get_bar_growth_anim(bar), run_time=2)
        self.wait()
    ...
    def add_subtitle(self):
        ...
        self.play(FadeInFromDown(subtitle), LaggedStartMap(FadeInFromDown, problems))
        self.problems = problems

    def show_data(self):
        problems = self.problems
        bars = VGroup(*[self.get_bar(n, p.get_right() + SMALL_BUFF * RIGHT) for n, p in zip(self.n_perfect_scores_per_problem, problems)])
        counters = VGroup(*map(self.get_bar_counter, bars))
        self.play(VFadeIn(counters), *[self.get_bar_growth_anim(bar) for bar in bars])
    ...
    def analyze_data(self):
        problems = VGroup(*[VGroup(p, pb, pc) for p, pb, pc in zip(self.problems, self.problem_bars, self.problem_counters)])
        ...
        self.wait()
        self.play(ReplacementTransform(rect, rects[5]), ReplacementTransform(rects[4], rects[5]), ReplacementTransform(rects[2], rects[5]))
        self.wait()
    ...
    def get_bar_growth_anim(self, bar):
        ...
        return Restore(bar, suspend_mobject_updating=False, run_time=2)
...
class SixOnSix(Describe2011IMO):
    CONFIG = {'student_data': [[1, 'Lisa Sauermann', 'de', [7, 7, 7, 7, 7, 7]], [2, 'Jeck Lim', 'sg', [7, 5, 7, 7, 7, 7]], [3, 'Lin Chen', 'cn', [7, 3, 7, 7, 7, 7]], [14, 'Mina Dalirrooyfard', 'ir', [7, 0, 2, 7, 7, 7]], [202, 'Georgios Kalantzis', 'gr', [7, 0, 1, 1, 2, 7]], [202, 'Chi Hong Chow', 'hk', [7, 0, 3, 1, 0, 7]]]}
    ...
    def construct(self):
        ...
        six_rect, two_rect = [SurroundingRectangle(VGroup(grid.rows[0][index], grid.rows[-1][index])) for index in [7, 3]]
        self.play(Write(title), LaggedStart(*[FadeIn(row, UP) for row in grid.rows]), LaggedStart(*[ShowCreation(line) for line in grid.h_lines]))
...
class TryOutSimplestExamples(WindmillScene):
    CONFIG = {'windmill_rotation_speed': TAU / 8}
    ...
    def two_points(self):
        ...
        self.play(ShowCreation(windmill), LaggedStartMap(FadeInFromLarge, dots, scale_factor=10, run_time=1, lag_ratio=0.4), GrowFromCenter(pivot_dot))
    ...
    def add_fourth_point(self):
        ...
        self.play(dot.next_to, words, DOWN, FadeIn(words, RIGHT))
    ...
    def move_starting_line(self):
        ...
        self.play(windmill.move_to, dots[-1], FadeOut(self.never_hit_words))
...
class FearedCase(WindmillScene):
    CONFIG = {'n_points': 25, 'windmill_rotation_speed': TAU / 16}
...
class WhereItStartsItEnds(WindmillScene):
    CONFIG = {'n_points': 11, 'windmill_rotation_speed': TAU / 8, 'random_seed': 1, 'points_shift_val': 2 * LEFT}
    ...
    def show_stays_in_middle(self):
        ...
        self.add(dots)
        self.play(ShowCreation(windmill), GrowFromCenter(pivot_dot), FadeIn(start_words, LEFT))
        self.wait()
    ...
    def ask_about_proof(self):
        ...
        proof_words.next_to(self.end_words.get_left(), DL, buff=2)
        proof_words.shift(RIGHT)
        proof_words.scale(sf)
        proof_arrow = Arrow(proof_words.get_top(), self.end_words.get_corner(DL), buff=SMALL_BUFF)
        ...
        self.play(Write(proof_words), GrowArrow(proof_arrow), run_time=1)
        self.wait()
        self.play(FadeOut(proof_arrow), FadeOut(proof_words), LaggedStartMap(ShowCreation, middle_rects), Write(middle_words))
        self.wait()
...
class AltWhereItStartsItEnds(WhereItStartsItEnds):
    CONFIG = {'n_points': 9, 'random_seed': 3}

class FormalizeMiddle(WhereItStartsItEnds):
    CONFIG = {'random_seed': 2, 'points_shift_val': 3 * LEFT}
    ...
    def problem_solving_tip(self):
        mid_words = VGroup(self.start_words, self.end_words)
        ...
        pst.add(underline)
        pst.to_corner(UR)
        steps = VGroup(OldTexText('Vague idea'), OldTexText('Put numbers to it'), OldTexText('Ask about those numbers'))
        steps.scale(sf)
        ...
        self.wait()
        self.play(mid_words.scale, 0.75, mid_words.set_opacity, 0.25, mid_words.to_corner, DL, FadeInFromDown(steps[0]))
        self.wait()
        for arrow, step in zip(arrows, steps[1:]):
            self.play(FadeIn(step, UP), GrowArrow(arrow))
            self.wait()
    ...
    def define_colors(self):
        ...
        self.play(Restore(rects), FadeIn(counters))
        for counter, dot_set, label_set in zip(counters, dot_sets, label_sets):
            self.play(ShowIncreasingSubsets(dot_set), ShowIncreasingSubsets(label_set), ChangingDecimal(counter, lambda a: len(dot_set)), rate_func=linear)
            self.wait()
        self.wait()
        ...
        self.wait()
        self.play(counters.space_out_submobjects, 0.8, counters.next_to, mid_words, DOWN, LARGE_BUFF, FadeOut(label_sets))
        eq = OldTex('=')
        eq.scale(2)
        eq.move_to(counters)
        self.play(FadeIn(eq))
    ...
    def mention_odd_case(self):
        ...
        self.wait()
        self.play(ShowCreationThenFadeAround(dots[k]), self.pivot_dot.set_color, WHITE)
        self.play(FadeIn(example, UP))
        self.play(ShowIncreasingSubsets(dot_rects), ChangingDecimal(example[1], lambda a: len(dot_rects)), rate_func=linear)
        self.wait()
        self.remove(dot_rects)
        self.play(ShowCreationThenFadeOut(dot_rects[:k]), ShowCreationThenFadeOut(SurroundingRectangle(counters[0], color=BLUE)))
        self.play(ShowCreationThenFadeOut(dot_rects[-k:]), ShowCreationThenFadeOut(SurroundingRectangle(counters[1], color=GREY_BROWN)))
        self.wait()
        self.play(FadeOut(words), FadeOut(example))
...
class SecondColoringExample(WindmillScene):
    ...
    def construct(self):
        ...
        counts = VGroup(OldTexText('\\# Blues = 4'), OldTexText('\\# Browns = 4'))
        ...
        arrows = VGroup(*[Arrow(const_words.get_left(), count.get_right(), buff=SMALL_BUFF, max_tip_length_to_length_ratio=0.15, max_stroke_width_to_length_ratio=3) for count in counts])
...
class TalkThroughPivotChange(WindmillScene):
    CONFIG = {'windmill_rotation_speed': 0.2}
    ...
    def setup_windmill(self):
        ...
        self.add(self.rects, self.windmill, self.dots, self.pivot_dot)
    ...
    def ask_about_pivot_change(self):
        ...
        self.play(Rotate(windmill, -0.9 * angle, run_time=3, rate_func=linear), Write(words, run_time=1), ShowCreation(arrow))
        self.wait()
        self.question = words
    ...
    def show_above_and_below(self):
        ...
        self.play(ShowCreationThenFadeOut(top_half), FadeIn(top_words, -vect))
        self.add(low_half, tips)
        self.play(ShowCreationThenFadeOut(low_half), FadeIn(low_words, vect))
        self.wait()
        windmill.add(tips)
    ...
    def change_pivot(self):
        ...
        self.play(FadeOut(self.above_below_words), ReplacementTransform(self.question, new_pivot_word), ReplacementTransform(arrow, blue_rect))
        self.wait()
        anims, time = self.rotate_to_next_pivot(windmill)
        self.play(*anims, Rotate(windmill, angle=-windmill.rot_speed, rate_func=linear))
        self.wait()
        self.play(TransformFromCopy(new_pivot_word, old_pivot_word), blue_rect.move_to, old_pivot)
        self.wait(2)
        brown_rect = SurroundingRectangle(dots[1])
        brown_rect.set_color(GREY_BROWN)
        self.play(TransformFromCopy(blue_rect, brown_rect))
        self.play(blue_rect.move_to, windmill.pivot, blue_rect.set_color, GREY_BROWN, old_pivot_word.move_to, new_pivot_word, FadeOut(new_pivot_word, DL))
        self.let_windmill_run(windmill, 1)
        self.wait()
        self.play(FadeOut(old_pivot_word), FadeOut(blue_rect), FadeOut(brown_rect))
...
class Rotate180Argument(WindmillScene):
    CONFIG = {'n_points': 21, 'random_seed': 3}
    ...
    def setup_windmill(self):
        ...
        self.p_label = p_label
        self.add(self.rects, self.windmill, self.dots, self.pivot_dot, self.p_label)
    ...
    def add_total_rotation_label(self):
        ...
        rect = SurroundingRectangle(VGroup(title, circle), buff=MED_LARGE_BUFF)
    ...
    def rotate_180(self):
        ...
        self.let_windmill_run(windmill, PI / windmill.rot_speed)
        self.wait()
...
class Rotate180ArgumentFast(Rotate180Argument):
    CONFIG = {'windmill_rotation_speed': 0.5}

class EvenCase(Rotate180Argument):
    CONFIG = {'n_points': 10, 'dot_config': {'radius': 0.075}}
    ...
    def ask_about_even_number(self):
        ...
        self.play(ShowIncreasingSubsets(dot_rects), ChangingDecimal(counter, lambda a: len(dot_rects)), rate_func=linear)
        ...
        self.play(FadeIn(blues_group, UP), ShowCreation(blue_rects))
        self.play(FadeIn(browns_group, UP), ShowCreation(brown_rects))
        self.wait()
        ...
        self.play(FadeIn(pivot_words, LEFT), ShowCreation(arrow))
        self.play(ShowCreation(pivot_rect), ChangeDecimalToValue(browns_counter, len(brown_rects) + 1), FadeOut(pivot_dot))
        self.wait()
        self.play(FadeOut(dot_rects), FadeOut(pivot_words), FadeOut(arrow))
        self.wait()
        blues_counter.add_updater(lambda c: c.set_value(len(list(filter(lambda d: d.get_fill_color() == Color(BLUE), dots)))))
        browns_counter.add_updater(lambda c: c.set_value(len(list(filter(lambda d: d.get_fill_color() == Color(GREY_BROWN), dots)))))
        self.windmill = windmill
        self.dots = dots
        self.points = points
        self.question = question
        self.counter_group = VGroup(counter_group, blues_group, browns_group)
    ...
    def add_total_rotation_label(self):
        ...
        self.play(FadeOut(self.question), FadeIn(self.rotation_label), self.counter_group.to_edge, UP)
...
class TwoTakeaways(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        items = VGroup(*[OldTexText('1) Social'), OldTexText('2) Mathematical')])
        items.scale(1.5)
        items.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        items.next_to(line, DOWN, buff=MED_LARGE_BUFF)
        self.play(ShowCreation(line), GrowFromPoint(title, self.hold_up_spot), self.teacher.change, 'raise_right_hand')
        self.play_all_student_changes('pondering')
        self.wait()
        for item in items:
        ...
        self.wait()
        for i, j in [(0, 1), (1, 0)]:
            self.play(items[i].become, items[i].big, items[j].become, items[j].small)
            self.wait()

class EasyToFoolYourself(PiCreatureScene):
    CONFIG = {'default_pi_creature_kwargs': {'color': GREY_BROWN}}
    ...
    def construct(self):
        ...
        self.add(morty)
        self.play(ShowCreation(bubble), morty.change, 'pondering')
        self.play(bubble[3].set_fill, GREEN_SCREEN, 0.5)
        self.wait()
        self.play(morty.change, 'thinking')
        self.play(FadeIn(fool_word, LEFT), ShowCreation(fool_arrow))
        self.wait()
        ...
        self.play(FadeInFromLarge(words), morty.change, 'guilty')
        self.wait()
...
class FailureToEmpathize(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.pi_creature_thinks(randy, equation, target_mode='confused', look_at=equation)
        randy_group = VGroup(randy, randy.bubble, randy.bubble.content)
        self.wait()
        self.play(DrawBorderThenFill(big_bubble), morty.change, 'confused', randy_group.scale, 0.5, randy_group.move_to, big_bubble.get_bubble_center(), randy_group.shift, 0.5 * DOWN + RIGHT)
        self.wait()
        ...
        self.play(morty_group.scale, 2, {'about_point': ap}, morty_group.fade, 1, randy_group.scale, 2, {'about_point': ap}, run_time=2)
        self.wait()
...
class DifficultyEstimateVsReality(Scene):
    ...
    def construct(self):
        axes = Axes(x_min=-1, x_max=10, x_axis_config={'include_tip': False}, y_min=-1, y_max=5)
        ...
        for a, e in zip(actual, estimated):
            bars = VGroup(Rectangle(width=width, height=e), Rectangle(width=width, height=a))
        ...
        data_labels = VGroup(OldTexText('Estimated average'), OldTexText('Actual average'))
        ...
        self.play(LaggedStartMap(FadeInFrom, x_labels, lambda m: (m, UP), run_time=2), LaggedStartMap(Restore, estimated_bars, run_time=3), FadeIn(data_labels[0]))
        self.wait()
        self.play(LaggedStartMap(Restore, actual_bars, run_time=3), FadeIn(data_labels[1]))
        self.wait()
...
class KeepInMindWhenTeaching(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.wait(2)
        self.play(PiCreatureSays(self.students[0], 'We know', target_mode='hooray'), self.students[1].change, 'happy', self.students[2].change, 'happy')
        self.wait(2)

class VastSpaceOfConsiderations(Scene):

    def construct(self):
        considerations = VGroup(*[OldTexText(phrase) for phrase in ["Define ``outer'' points", 'Convex hulls', 'Linear equations', "Sort points by when they're hit", 'Sort points by some kind of angle?', 'How does this permute the $n \\choose 2$ lines through pairs?', 'Some points are hit more than others, can we quantify this?']])
...
class WhatStaysConstantWrapper(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeOut(title1, UP), FadeInFromDown(title2))
        self.wait()

class CountHoles(Scene):

    def construct(self):
        labels = VGroup(OldTexText('Genus ', '0'), OldTexText('Genus ', '1'), OldTexText('Genus ', '2'))
        labels.scale(2)
        ...
        self.play(LaggedStartMap(FadeInFromDown, labels, lag_ratio=0.5))
        self.wait()
        self.play(labels[1].shift, 4 * RIGHT, FadeOut(labels[0::2]))
        self.play(FadeIn(equation, RIGHT), GrowArrow(arrow))
...
class LorenzTransform(Scene):
    ...
    def construct(self):
        ...
        self.add(equation)
        beta = 0.4
        self.play(grid.apply_matrix, np.array([[1, beta], [beta, 1]]) / (1 - beta ** 2), run_time=2)
        self.wait()
...
class OnceACleverDiscovery(Scene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(vect), ShowCreation(rect))
        self.play(FadeInFromDown(words))
        self.wait()
...
class TerryTaoQuote(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeInFromDown(image), Write(name))
        self.wait()
...
class WindmillFairyTale(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(LaggedStart(FadeIn(paths[1], RIGHT), FadeIn(paths[2], RIGHT), lag_ratio=0.2, run_time=3))
...
class SolveAProblemOneDay(SpiritOfIMO, PiCreatureScene):
    ...
    def construct(self):
        ...
        you.next_to(arrow)
        self.play(ShowCreation(bubble), randy.change, 'pondering')
        self.play(FadeIn(you, LEFT), GrowArrow(arrow))
        self.wait(2)
        self.play(FadeInFromDown(light_bulb), randy.change, 'hooray')
        self.play(LaggedStartMap(VFadeInThenOut, light, run_time=2), randy.change, 'thinking', light)
        self.wait(2)
...
class Thumbnail(WindmillScene):
    ...
    def construct(self):
        ...
        points[10] += LEFT
        points[6] += 3 * RIGHT
        windmill = self.get_windmill(points, points[1], angle=45 * DEGREES)
        dots = self.get_dots(points)
        pivot_dot = self.get_pivot_dot(windmill)
        pivot_dot.scale(2)
        pivot_dot.set_color(WHITE)
        ...
        arcs = VGroup(*[Arc(start_angle=wa + a, angle=90 * DEGREES, radius=1.5, stroke_width=10).add_tip(tip_length=0.7) for a in [0, PI]])
        arcs.move_to(windmill.pivot)
        arcs.set_color([GREY_B, WHITE])
        polygon1 = Polygon((FRAME_HEIGHT * UP + FRAME_WIDTH * LEFT) / 2, (FRAME_HEIGHT * UP + FRAME_HEIGHT * RIGHT) / 2, (FRAME_HEIGHT * DOWN + FRAME_HEIGHT * LEFT) / 2, (FRAME_HEIGHT * DOWN + FRAME_WIDTH * LEFT) / 2)
        polygon1.set_color([BLUE, GREY_E])
        polygon1.set_fill(opacity=0.5)
        polygon2 = Polygon((FRAME_HEIGHT * UP + FRAME_WIDTH * RIGHT) / 2, (FRAME_HEIGHT * UP + FRAME_HEIGHT * RIGHT) / 2, (FRAME_HEIGHT * DOWN + FRAME_HEIGHT * LEFT) / 2, (FRAME_HEIGHT * DOWN + FRAME_WIDTH * RIGHT) / 2)
