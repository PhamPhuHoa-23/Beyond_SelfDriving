...
class LlamaLearningSketchFour(InteractiveScene):
    ...
    def construct(self):
        ...
        def create_looping_lag_animation(objects, num_loops=2, individual_time=1.5, lag_ratio=0.2, start_opacity=0.0, end_opacity=1.0, fade_out_time=1.0):
        ...
        time_tracker, total_time, objects = create_looping_lag_animation(list(forward_pass) + list(backward_pass[::-1]), num_loops=5, individual_time=1.0, lag_ratio=0.5, start_opacity=0.0, end_opacity=1.0, fade_out_time=7.0)
        self.frame.reorient(-89, 69, 0, (1.19, -0.11, -0.7), 8.95)
        self.wait()
        self.play(time_tracker.animate.set_value(total_time), self.camera.frame.animate.reorient(0, 0, 0, (5.2, -1.28, 0.0), 12.35), run_time=total_time, rate_func=linear)

        def update_time_tracker(mob, alpha):
            time_tracker.set_value(alpha * total_time)
        self.play(AnimationGroup(UpdateFromFunc(time_tracker, update_time_tracker, rate_func=linear), self.camera.frame.animate.reorient(0, 0, 0, (5.2, -1.28, 0.0), 12.35), rate_func=lambda t: rate_func_with_early_end(t, complete_at=0.7)), run_time=total_time)
        for obj in objects:
            obj.clear_updaters()
        self.wait()
        self.wait()
