...
class AttentionPattern(VMobject):
    ...
    def __init__(self, matrix, square_size=0.3, min_opacity=0.0, max_opacity=1.0, stroke_width=1.0, viz_scaling_factor=2.5, stroke_color=PRIMARY_COLOR, colormap=custom_cmap_tans, **kwargs):
...
def get_mlp(w1, w2, neuron_fills=None, grads_1=None, grads_2=None, line_weight=1.0, line_opacity=0.5, neuron_stroke_width=1.0, neuron_stroke_color=PRIMARY_COLOR, line_stroke_color='#948979', connection_display_thresh=0.4):
    ...
    for i, in_neuron in enumerate(input_layer):
        for j, hidden_neuron in enumerate(hidden_layer):
            if np.abs(w1_scaled[i, j]) < THRESHOLDS[0]:
                continue
            if abs(i - j) > 6:
                continue
    ...
    for i, hidden_neuron in enumerate(hidden_layer):
        for j, out_neuron in enumerate(output_layer):
            if np.abs(w2_scaled[i, j]) < THRESHOLDS[1]:
                continue
            if abs(i - j) > 6:
                continue
    ...
    if grads_1 is not None:
        ...
        for i, in_neuron in enumerate(input_layer):
            for j, hidden_neuron in enumerate(hidden_layer):
                if np.abs(grads_1_scaled[i, j]) < THRESHOLDS[2]:
                    continue
                if abs(i - j) > 6:
                    continue
                start_point, end_point = get_edge_points(in_neuron, hidden_neuron, NEURON_RADIUS)
    ...
    if grads_2 is not None:
        ...
        for i, hidden_neuron in enumerate(hidden_layer):
            for j, out_neuron in enumerate(output_layer):
                if np.abs(grads_2_scaled[i, j]) < THRESHOLDS[3]:
                    continue
                if abs(i - j) > 6:
                    continue
                start_point, end_point = get_edge_points(hidden_neuron, out_neuron, NEURON_RADIUS)
...
def get_mlp_connections_left(attention_connections_left, mlp_out, connection_points_left, attention_connections_left_grad=None):
    ...
    for i, mlp_out_neuron in enumerate(mlp_out):
        for j, attention_neuron in enumerate(connection_points_left):
            if np.abs(attention_connections_left_scaled[i, j]) < THRESHOLDS[4]:
                continue
            if abs(i / 4 - j) > 3:
                continue
    ...
    if attention_connections_left_grad is not None:
        ...
        attention_connections_left_grad_scaled = attention_connections_left_grad_abs / np.percentile(attention_connections_left_grad_abs, PERCENTILES[5])
        for i, mlp_out_neuron in enumerate(mlp_out):
            for j, attention_neuron in enumerate(connection_points_left):
                if np.abs(attention_connections_left_grad_scaled[i, j]) < THRESHOLDS[5]:
                    continue
                if abs(i / 4 - j) > 3:
                    continue
                start_point, end_point = get_edge_points(mlp_out_neuron, attention_neuron, 0.06)
...
def get_mlp_connections_right(attention_connections_right, mlp_in, connection_points_right, attention_connections_right_grad=None):
    ...
    for i, attention_neuron in enumerate(connection_points_right):
        for j, mlp_in_neuron in enumerate(mlp_in):
            if np.abs(attention_connections_right_scaled[i, j]) < THRESHOLDS[6]:
                continue
            if abs(j / 4 - i) > 3:
                continue
    ...
    if attention_connections_right_grad is not None:
        ...
        for i, attention_neuron in enumerate(connection_points_right):
            for j, mlp_in_neuron in enumerate(mlp_in):
                if np.abs(attention_connections_right_grad_scaled[i, j]) < THRESHOLDS[7]:
                    continue
                if abs(j / 4 - i) > 3:
                    continue
                start_point, end_point = get_edge_points(mlp_in_neuron, attention_neuron, 0.06)
    ...
    def construct(self):
        ...
        for i, n1 in enumerate(mlps[-1][4]):
            for j, n2 in enumerate(output_layer[0]):
                if np.abs(unembed_scaled[i, j]) < THRESHOLDS[8]:
                    continue
                if abs(j - i) > 8:
                    continue
                start_point, end_point = get_edge_points(n1, n2, 0.06)
        ...
        for i, n1 in enumerate(mlps[-1][4]):
            for j, n2 in enumerate(output_layer[0]):
                if np.abs(unembed_scaled_grad[i, j]) < THRESHOLDS[9]:
                    continue
                if abs(j - i) > 8:
                    continue
                start_point, end_point = get_edge_points(n1, n2, 0.06)
        ...
        for i in range(16):
            ...
            if i % 2 == 1:
                rect.set_color(CHILL_GREEN)
            else:
                rect.set_color(CHILL_BLUE)
            rect.set_opacity(0.25)
            rect.move_to([8.01 - rect_spacing * i, 0, 0])
            if i == 0:
                eq = Tex('\\frac{\\partial L}{\\partial h_{16}}')
            else:
                eq = Tex('\\frac{\\partial h_{' + str(17 - i) + '}}{\\partial h_{' + str(16 - i) + '}}')
            eq.scale(0.5)
