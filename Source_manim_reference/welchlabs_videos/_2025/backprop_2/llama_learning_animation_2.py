    ...
    if grads_1 is not None:
        ...
        for i, in_neuron in enumerate(input_layer):
            for j, hidden_neuron in enumerate(hidden_layer):
                ...
                line_grad.set_stroke(opacity=np.clip(grads_1_scaled[i, j], 0, 1), width=2.0 * grads_1_scaled[i, j])
...
class LlamaLearningSketchOne(InteractiveScene):
    ...
    def construct(self):
        ...
        mlp.move_to([-4, 0, 0])
        neuron_fills = [snapshot['blocks.' + str(layer_num + 1) + '.hook_resid_mid'], snapshot['blocks.' + str(layer_num + 1) + '.mlp.hook_post'], snapshot['blocks.' + str(layer_num + 1) + '.hook_mlp_out']]
