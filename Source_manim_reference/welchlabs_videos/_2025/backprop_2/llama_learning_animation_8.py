...
class LlamaLearningSketchThree(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.wait()
        time_tracker, actual_total_time, objects = create_lag_animation(list(forward_pass) + list(backward_pass[::-1]), individual_time=1.0, lag_ratio=0.5, start_opacity=0.0, end_opacity=1.0, fade_out_time=0.7)
        forward_pass_copy = forward_pass.copy()
        backward_pass_copy = backward_pass.copy()
        time_tracker_2, actual_total_time_2, objects_2 = create_lag_animation(list(forward_pass) + list(backward_pass[::-1]), individual_time=1.0, lag_ratio=0.5, start_opacity=0.0, end_opacity=1.0, fade_out_time=0.7)
        self.wait()
        self.play(Succession(time_tracker.animate.set_value(actual_total_time), time_tracker_2.animate.set_value(actual_total_time), run_time=2 * actual_total_time))
        self.wait()
        self.wait()
        self.wait()
        self.wait()
