...
class LlamaLearningSketchOne(InteractiveScene):
    ...
    def construct(self):
        ...
        for layer_count, layer_num in enumerate([0, 1, 2, 13, 14, 15]):
            ...
            attns.append(attn)
            connections_left, connections_left_grads = get_mlp_connections_left(attention_connections_left=attention_connections_left, mlp_out=mlp[4], connection_points_left=attn[2], attention_connections_left_grad=attention_connections_left_grad)
            if len(mlps) > 1:
                connections_right, connections_right_grads = get_mlp_connections_right(attention_connections_right=attention_connections_right, mlp_in=mlp[2], connection_points_right=attns[-2][3], attention_connections_right_grad=attention_connections_right_grad)
