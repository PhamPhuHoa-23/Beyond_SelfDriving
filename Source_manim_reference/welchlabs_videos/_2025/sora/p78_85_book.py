...
class guidance_book_1(InteractiveScene):
    ...
    def construct(self):
        ...
        for i, d in enumerate(dots):
            if labels_array[i] == 0:
                d.set_color('#5C4E9A').set_opacity(1.0)
                self.wait(0.1)
        for i, d in enumerate(dots):
            if labels_array[i] == 1:
                d.set_color('#FAA726').set_opacity(1.0)
                self.wait(0.1)
        for i, d in enumerate(dots):
            if labels_array[i] == 2:
                d.set_color('#00AEEF').set_opacity(1.0)
                self.wait(0.1)
        self.add(dots)
        self.wait()
        self.embed()
...
class guidance_book_3(InteractiveScene):
    ...
    def construct(self):
        ...
        traced_path = CustomTracedPath(dot_to_move.get_center, stroke_color='#00AEEF', stroke_width=3.5, opacity_range=(0.6, 0.6), fade_length=15)
    ...
    def construct(self):
        ...
        for class_index in range(xt_history.shape[0]):
            for path_index in range(num_dots_per_class):
                ...
                traced_path_2 = CustomTracedPath(dot_to_move_2.get_center, stroke_color=colors_by_class[class_index], stroke_width=1.7, opacity_range=(0.75, 0.75), fade_length=32)
                all_traced_paths.add(traced_path_2)
        self.add(all_traced_paths)
        dots.set_opacity(0.15)
        self.add(all_dots_to_move)
        self.wait()
        self.frame.reorient(0, 0, 0, (0.0, 0.0, 0.0), 9.0)
    ...
    def construct(self):
        ...
        for class_index in range(xt_history.shape[0]):
            for path_index in range(num_dots_per_class):
                ...
                dot_to_move_2.set_color(colors_by_class[class_index])
                all_dots_to_move.add(dot_to_move_2)
                traced_path_2 = CustomTracedPath(dot_to_move_2.get_center, stroke_color=colors_by_class[class_index], stroke_width=1.7, opacity_range=(0.75, 0.75), fade_length=12)
                all_traced_paths.add(traced_path_2)
        self.add(all_traced_paths)
        dots.set_opacity(0.15)
        cat_dots = VGroup()
        ...
        def vector_function_heatmap(coords_array):
            ...
            for i, coord in enumerate(coords_array):
                ...
                result[i] = vector
            return -result
        vector_field = TrackerControlledVectorField(time_tracker=time_tracker, func=vector_function_heatmap, coordinate_system=extended_axes, density=5.0, stroke_width=3, max_radius=5.5, min_opacity=0.5, max_opacity=0.5, tip_width_ratio=4, tip_len_to_width=0.007, max_vect_len_to_step_size=0.7, color='#00AEEF')
        ...
        def vector_function_heatmap_u(coords_array):
            ...
            for i, coord in enumerate(coords_array):
                ...
                result[i] = vector
            return -result
        vector_field_u = TrackerControlledVectorField(time_tracker=time_tracker, func=vector_function_heatmap_u, coordinate_system=extended_axes, density=5.0, stroke_width=3, max_radius=5.5, min_opacity=0.5, max_opacity=0.5, tip_width_ratio=4, tip_len_to_width=0.007, max_vect_len_to_step_size=0.7, color=CHILL_BROWN)
        path_index = 70
        guidance_index = 0
        ...
        traced_path_3 = CustomTracedPath(dot_to_move_3.get_center, stroke_color=BLUE, stroke_width=3.0, opacity_range=(1.0, 1.0), fade_length=64)
        ...
        self.frame.reorient(0, 0, 0, (0.23, 2.08, 0.0), 5.4)
        self.wait()
        for k in range(xt_history.shape[1]):
    ...
    def construct(self):
        ...
        def vector_function_heatmap(coords_array):
            ...
            for i, coord in enumerate(coords_array):
                ...
                result[i] = vector
            return -result
        vector_field = TrackerControlledVectorField(time_tracker=time_tracker, func=vector_function_heatmap, coordinate_system=extended_axes, density=4.0, stroke_width=2, max_radius=5.5, min_opacity=0.75, max_opacity=0.75, tip_width_ratio=5, tip_len_to_width=0.01, max_vect_len_to_step_size=0.7, color='#00AEEF')
        ...
        def vector_function_heatmap_u(coords_array):
            ...
            for i, coord in enumerate(coords_array):
                ...
                result[i] = vector
            return -result
        vector_field_u = TrackerControlledVectorField(time_tracker=time_tracker, func=vector_function_heatmap_u, coordinate_system=extended_axes, density=4.0, stroke_width=2, max_radius=5.5, min_opacity=0.75, max_opacity=0.75, tip_width_ratio=5, tip_len_to_width=0.01, max_vect_len_to_step_size=0.7, color=CHILL_BROWN)
        path_index = 70
        guidance_index = 0
        dot_to_move_3 = Dot(axes.c2p(*[xt_history[guidance_index, 0, path_index, 0], xt_history[guidance_index, 0, path_index, 1], 0]), radius=0.07)
        dot_to_move_3.set_color(BLUE)
        ...
        self.add(vector_field)
        self.frame.reorient(0, 0, 0, (0.05, 0.06, 0.0), 7.05)
        self.wait()
    ...
    def construct(self):
        ...
        def vector_function_heatmap_g(coords_array):
            ...
            for i, coord in enumerate(coords_array):
                ...
                result[i] = vector
            return -result
        vector_field_g = TrackerControlledVectorField(time_tracker=time_tracker, func=vector_function_heatmap_g, coordinate_system=extended_axes, density=4.0, stroke_width=2, max_radius=5.5, min_opacity=0.75, max_opacity=0.75, tip_width_ratio=5, tip_len_to_width=0.01, max_vect_len_to_step_size=0.7, color=GREEN)
        ...
        traced_path_4 = CustomTracedPath(dot_to_move_4.get_center, stroke_color=GREEN, stroke_width=3.0, opacity_range=(1.0, 1.0), fade_length=64)
        ...
        self.wait()
        for k in range(xt_history.shape[1]):
            self.play(time_tracker.animate.set_value(8.0 * (k / 256.0)), dot_to_move_4.animate.move_to(axes.c2p(*[xt_history[guidance_index, k, path_index, 0], xt_history[guidance_index, k, path_index, 1]])), rate_func=linear, run_time=0.01)
        self.wait()
    ...
    def construct(self):
        ...
        for i, d in enumerate(dots):
            if labels_array[i] == 0:
                person_dots.add(d)
        self.wait()
        self.play(FadeOut(dot_to_move_4), FadeOut(dot_to_move_3), FadeOut(traced_path_4), FadeOut(traced_path_3), self.frame.animate.reorient(0, 0, 0, (0.06, -0.02, 0.0), 7.52), run_time=1.0)
    ...
    def construct(self):
        ...
        def vector_function_parent(coords_array, heatmap_array, class_index):
            ...
            for i, coord in enumerate(coords_array):
                ...
                result[i] = vector
            return -result
        vector_field_cats_g = TrackerControlledVectorField(time_tracker=time_tracker, func=partial(vector_function_parent, heatmap_array=heatmaps_2, class_index=2), coordinate_system=extended_axes, density=4.0, stroke_width=2, max_radius=5.5, min_opacity=0.7, max_opacity=0.7, tip_width_ratio=4, tip_len_to_width=0.01, max_vect_len_to_step_size=0.7, color=GREEN)
        vector_field_cats_u = TrackerControlledVectorField(time_tracker=time_tracker, func=partial(vector_function_parent, heatmap_array=heatmaps_u_2, class_index=2), coordinate_system=extended_axes, density=4.0, stroke_width=2, max_radius=5.5, min_opacity=0.7, max_opacity=0.7, tip_width_ratio=4, tip_len_to_width=0.01, max_vect_len_to_step_size=0.7, color=CHILL_BROWN)
        vector_field_cats_c = TrackerControlledVectorField(time_tracker=time_tracker, func=partial(vector_function_parent, heatmap_array=heatmaps_c_2, class_index=2), coordinate_system=extended_axes, density=4.0, stroke_width=2, max_radius=5.5, min_opacity=0.7, max_opacity=0.7, tip_width_ratio=4, tip_len_to_width=0.01, max_vect_len_to_step_size=0.7, color='#00AEEF')
        ...
        for class_index in range(xt_history_2.shape[0]):
            for path_index in range(num_dots_per_class):
                ...
                traced_path = CustomTracedPath(dot_to_move.get_center, stroke_color=colors_by_class[class_index], stroke_width=1.8, opacity_range=(0.75, 0.75), fade_length=128)
        ...
        for k in range(xt_history_2.shape[1]):
            ...
            for class_index in range(xt_history_2.shape[0]):
                for j in range(num_dots_per_class):
                    animations.append(all_dots_to_move[path_index].animate.move_to(axes.c2p(*[xt_history_2[class_index, k, j, 0], xt_history_2[class_index, k, j, 1]])))
                    path_index += 1
            self.play(*animations, rate_func=linear, run_time=0.1)
        self.wait()
        self.remove(all_traced_paths)
