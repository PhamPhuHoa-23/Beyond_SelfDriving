...
class follow_trajectory_2c(InteractiveScene):
    ...
    def construct(self):
        ...
        for k in range(64):
            segment1 = Line(axes.c2p(*[xt_history[k, path_index, 0], xt_history[k, path_index, 1]]), axes.c2p(*[history_pre_noise[k, path_index, 0], history_pre_noise[k, path_index, 1]]), stroke_width=5.0, stroke_color='#00AEEF')
            segment2 = Line(axes.c2p(*[history_pre_noise[k, path_index, 0], history_pre_noise[k, path_index, 1]]), axes.c2p(*[xt_history[k + 1, path_index, 0], xt_history[k + 1, path_index, 1]]), stroke_width=5.0, stroke_color=BLACK)
        ...
        for path_index in range(num_dots):
            ...
            for k in range(64):
                ...
                segment2 = Line(axes.c2p(*[history_pre_noise[k, path_index, 0], history_pre_noise[k, path_index, 1]]), axes.c2p(*[xt_history[k + 1, path_index, 0], xt_history[k + 1, path_index, 1]]), stroke_width=3.0, stroke_color=colors[path_index])
        ...
        for path_index in range(num_dots):
            ...
            dot_to_move.set_color(colors[path_index])
            all_dots_to_move.add(dot_to_move)
            traced_path = CustomTracedPath(dot_to_move.get_center, stroke_color=colors[path_index], stroke_width=3.0, opacity_range=(0.4, 0.4), fade_length=24)
    ...
    def construct(self):
        ...
        def vector_function_with_tracker(coords_array):
            ...
            try:
                ...
                return np.zeros((len(coords_array), 2))
        vector_field = TrackerControlledVectorField(time_tracker=time_tracker, func=vector_function_with_tracker, coordinate_system=extended_axes, density=5.0, stroke_width=1.5, max_radius=6.0, min_opacity=0.6, max_opacity=0.6, tip_width_ratio=4, tip_len_to_width=0.01, max_vect_len_to_step_size=0.7, color=CHILL_BROWN)
        self.add(axes)
        self.add(dots)
