...
class p44_47(InteractiveScene):
    ...
    def construct(self):
        ...
        loader = DataLoader(dataset, batch_size=batch_size)
        batch = next(iter(loader)).numpy()
        axes = Axes(x_range=[-1.2, 1.2, 0.5], y_range=[-1.2, 1.2, 0.5], height=7, width=7, axis_config={'color': CHILL_BROWN, 'stroke_width': 3, 'include_tip': True, 'include_ticks': False, 'tick_size': 0.06, 'tip_config': {'color': CHILL_BROWN, 'length': 0.15, 'width': 0.15}})
        dots = VGroup()
        for point in batch:
            screen_point = axes.c2p(point[0], point[1])
        ...
        i = 75
        dot_to_move = dots[i].copy()
        traced_path = CustomTracedPath(dot_to_move.get_center, stroke_color=CHILL_BROWN, stroke_width=4, opacity_range=(0.4, 0.4), fade_length=90)
        traced_path.set_fill(opacity=0)
        np.random.seed(485)
        random_walk = 0.07 * np.random.randn(100, 2)
        random_walk[0] = np.array([0.2, 0.12])
...
class p40_44v2_black(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.wait()
        traced_path = CustomTracedPath(dot_to_move.get_center, stroke_color=BLACK, stroke_width=3, opacity_range=(0.35, 0.35), fade_length=90)
        traced_path.set_fill(opacity=0)
...
class p40_44v2b(InteractiveScene):
    ...
    def construct(self):
        ...
        for idx, d in enumerate(dots):
            tp = CustomTracedPath(d.get_center, stroke_color=CHILL_BROWN, stroke_width=2.0, opacity_range=(0.3, 0.3), fade_length=90)
            traced_path.set_fill(opacity=0)
            traced_paths.add(tp)
        self.add(traced_paths)
        remaining_indices = np.concatenate((np.arange(75), np.arange(76, len(batch))))
