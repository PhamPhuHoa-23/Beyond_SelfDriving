...
def create_split_line(neuron1, neuron2, all_neurons, neuron_radius, exclude_neurons=None):
    ...
    for neuron in all_neurons:
        if neuron in exclude_neurons:
            continue
        intersections = line_circle_intersection(full_start, full_end, neuron.get_center(), neuron_radius)
    ...
    for i in range(INPUT_NEURONS):
        if i == w1.shape[0] // 2:
            ...
            if neuron_fills is None:
                ...
                neuron.set_fill(color=get_neuron_color(neuron_fills[0][neuron_count], vmax=np.abs(neuron_fills[0]).max()), opacity=1.0)
    ...
    for i in range(num_output_neurons):
        if i == num_output_neurons // 2:
            ...
            if not empty:
                ...
                text_color = get_neuron_color(np.clip(snapshot['topk.probs'][neuron_count], 0.1, 1.0), vmax=np.max(snapshot['topk.probs']))
...
class P18(InteractiveScene):
    ...
    def construct(self):
            ...
            if layer_idx < len(neuron_layers) - 1:
                ...
                for neuron1 in current_layer:
                    for neuron2 in next_layer:
                        line_segments = create_split_line(neuron1, neuron2, all_neurons, neuron_radius, exclude_neurons={neuron1, neuron2})
        ...
        for iteration in range(3):
            ...
            indices_to_disable = np.random.choice(total_neurons, size=num_to_disable, replace=False)
