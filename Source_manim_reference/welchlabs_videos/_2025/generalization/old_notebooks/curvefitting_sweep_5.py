...
def generate_data(config):
    ...
    return {'x_train': x_train, 'y_train': y_train, 'x_test': x_test, 'y_test': y_test, 'x_train_tensor': x_train_tensor, 'y_train_tensor': y_train_tensor, 'x_test_tensor': x_test_tensor, 'y_test_tensor': y_test_tensor, 'all_x': all_x, 'all_y': all_y, 'n_train': n_train, 'n_test': n_test, 'dimension': d}
        ...
        for h in config.hidden_units:
            ...
            for lr in lrs_to_try:
                for seed in config.random_seeds:
                    ...
                    record = {'experiment_id': experiment_id, 'hidden_units': h, 'learning_rate': lr, 'torch_random_seed': seed, 'train_loss': result['final_train_loss'], 'test_loss': result['final_test_loss'], 'noise_level': config.noise_level, 'data_dimension': config.data_dimension, 'n_train': data['n_train'], 'n_test': data['n_test'], 'n_params': h * config.data_dimension + h + h * 1 + 1}
...
def create_visualizations(df, data, exp_dir, config):
    ...
    if d == 1:
        ...
        ax1.scatter(data['x_train'].flatten(), data['y_train'], c='cyan', s=50, label=f'Train (n={data['n_train']})', edgecolors='k')
        ax1.scatter(data['x_test'].flatten(), data['y_test'], c='magenta', s=50, label=f'Test (n={data['n_test']})', edgecolors='k')
        ax1.set_xlabel('x')
        ax1.set_ylabel('y')
    elif d == 2:
        ax1.scatter(data['x_train'][:, 0], data['x_train'][:, 1], c=data['y_train'], s=100, cmap='viridis', edgecolors='cyan', linewidths=2, label=f'Train (n={data['n_train']})')
        ax1.scatter(data['x_test'][:, 0], data['x_test'][:, 1], c=data['y_test'], s=100, cmap='viridis', marker='s', edgecolors='magenta', linewidths=2, label=f'Test (n={data['n_test']})')
        ax1.set_xlabel('x₁')
        ax1.set_ylabel('x₂')
    ...
    for idx, h in enumerate(selected_widths):
        ...
        if d == 1:
            ...
            ax.scatter(data['x_train'].flatten(), data['y_train'], c='cyan', s=30, alpha=0.5, edgecolors='k', linewidths=0.5)
            ...
            for _, row in df_h.iterrows():
                ...
                if curve_file.exists():
                    ...
                    ax.plot(curve_data['x_values'].flatten(), curve_data['predictions'], alpha=0.4, linewidth=1.5)
            ax.set_xlabel('x')
            ax.set_ylabel('y')
        elif d == 2:
            df_h = df[df['hidden_units'] == h]
            if len(df_h) > 0:
                ...
                if curve_file.exists():
                    ...
                    ax.scatter(data['x_train'][:, 0], data['x_train'][:, 1], c='cyan', s=30, edgecolors='k', linewidths=0.5)
                    plt.colorbar(im, ax=ax)
            ax.set_xlabel('x₁')
            ax.set_ylabel('x₂')
        else:
            df_h = df[df['hidden_units'] == h]
            ax.scatter(data['x_train'][:, 0], data['x_train'][:, 1], c=data['y_train'], s=30, cmap='viridis', alpha=0.5, edgecolors='k', linewidths=0.5)
            ax.set_xlabel('x₁')
            ax.set_ylabel('x₂')
            ax.text(0.5, 0.95, f'{d}D data (projection)', transform=ax.transAxes, ha='center', va='top', fontsize=8)
        mean_test_loss = df_h['test_loss'].mean()
        ax.set_title(f'Width={h}, Params={df_h.iloc[0]['n_params']}\nTest Loss={mean_test_loss:.4f}', fontsize=9)
