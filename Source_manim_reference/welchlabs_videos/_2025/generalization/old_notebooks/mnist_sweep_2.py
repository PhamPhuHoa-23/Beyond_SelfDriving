...
class MNISTDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, hf_dataset, transform=None, noise_percent=0.0, random_seed=42):
        ...
        self.corrupted_indices = [i for i, (orig, noisy) in enumerate(zip(self.original_labels, self.noisy_labels)) if orig != noisy]
        if noise_percent > 0:
    ...
    def get_clean_accuracy(self, predictions):
        clean_indices = [i for i in range(len(self.original_labels)) if i not in self.corrupted_indices]
...
def prepare_data(config):
    ...
    train_dataset = MNISTDataset(train_subset, transform=transform, noise_percent=config.label_noise_percent, random_seed=config.random_seed)
    test_dataset = MNISTDataset(dataset['test'], transform=transform, noise_percent=0.0)
...
def train_model_wrapper(args):
    ...
    transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081,))])
    train_dataset = MNISTDataset(train_data, transform=transform, noise_percent=config.label_noise_percent, random_seed=config.random_seed)
    test_dataset = MNISTDataset(test_data, transform=transform, noise_percent=0.0)
    ...
    for epoch in range(1, config.num_epochs + 1):
        ...
        result = {'Hidden_Units': hidden_units, 'Parameters': num_params, 'Epoch': epoch, 'Learning_Rate': config.learning_rate, 'Batch_Size': config.batch_size, 'Optimizer': config.optimizer_name, 'Train_Samples': config.num_train_samples, 'Label_Noise_Percent': config.label_noise_percent, 'Train_Loss': train_loss, 'Train_Accuracy': train_acc, 'Test_Loss': test_loss, 'Test_Accuracy': test_acc}
...
def run_parallel_experiments(config):
    ...
    config_dict = {'num_train_samples': config.num_train_samples, 'batch_size': config.batch_size, 'num_epochs': config.num_epochs, 'learning_rate': config.learning_rate, 'optimizer_name': config.optimizer_name, 'label_noise_percent': config.label_noise_percent, 'random_seed': config.random_seed, 'results_file': config.results_file, 'checkpoint_freq': config.checkpoint_freq}
    initial_df = pd.DataFrame(columns=['Hidden_Units', 'Parameters', 'Epoch', 'Learning_Rate', 'Batch_Size', 'Optimizer', 'Train_Samples', 'Label_Noise_Percent', 'Train_Loss', 'Train_Accuracy', 'Test_Loss', 'Test_Accuracy'])
