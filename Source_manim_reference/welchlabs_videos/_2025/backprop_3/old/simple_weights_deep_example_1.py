...
class simple_weights_deep_example_1(InteractiveScene):
    ...
    def construct(self):
        ...
        with torch.no_grad():
            model.model[0].weight = nn.Parameter(torch.tensor([[-3.0, 3.0], [3.0, 3.0]]))
            model.model[0].bias = nn.Parameter(torch.tensor([0.0, 0.0]))
            model.model[2].weight = nn.Parameter(torch.tensor([[-1.0, -3.0], [-3.0, -1.0]]))
            model.model[2].bias = nn.Parameter(torch.tensor([4.0, 4.0]))
            model.model[4].weight = nn.Parameter(torch.tensor([[1.0, 3.0], [3.0, 1.0]]))
