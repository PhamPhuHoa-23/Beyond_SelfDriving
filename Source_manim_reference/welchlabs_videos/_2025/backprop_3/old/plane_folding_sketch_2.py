...
def get_activation_regions(w1, b1, extent=1):
    ...
    for neuron1_active, neuron2_active in product([False, True], repeat=2):
        region = {'pattern': (neuron1_active, neuron2_active), 'constraints': []}
        if neuron1_active:
...
def find_region_vertices(constraints, extent=1):
    boundary_constraints = [(1, 0, extent, '<='), (-1, 0, extent, '<='), (0, 1, extent, '<='), (0, -1, extent, '<=')]
    ...
    for vertex in vertices:
        ...
        for existing in unique_vertices:
            if np.linalg.norm(np.array(vertex) - np.array(existing)) < 1e-08:
                is_duplicate = True
                break
        if not is_duplicate:
    ...
    for region in regions:
        ...
        if len(vertices) >= 3:
            for i in range(len(vertices)):
                ...
                line = Line(start=[start_vertex[0], start_vertex[1], 0], end=[end_vertex[0], end_vertex[1], 0], color=WHITE, stroke_width=2)
        ...
        for region in regions:
            ...
            if joint_points and len(joint_points) >= 2:
                ...
                if clipped_points and len(clipped_points) >= 2:
                    line = Line(start=[clipped_points[0][0], clipped_points[0][1], 0], end=[clipped_points[1][0], clipped_points[1][1], 0], color=RED, stroke_width=3)
                    boundary_lines.append(line)
...
def clip_line_to_region(line_points, region, extent):
    ...
    for t in np.linspace(0, 1, 200):
        ...
        for constraint in region['constraints']:
            w1, w2, bias, op = constraint
            value = w1 * x + w2 * y + bias
            if op == '<=' and value > 1e-08:
                valid = False
                break
            elif op == '>=' and value < -1e-08:
                valid = False
                break
...
def create_region_polygons(w1, b1, w2=None, b2=None, layer=1, neuron_idx=0, extent=1):
    ...
    for i, region in enumerate(regions):
        ...
        if len(vertices) >= 3:
            ...
            polygons.append({'polygon': polygon, 'region': region, 'vertices': vertices})
    return polygons
...
def create_complete_polytope_outlines(w1, b1, w2=None, b2=None, layer=1, neuron_idx=0, extent=1):
    ...
    for i, region in enumerate(regions):
        ...
        if len(vertices) >= 3:
            ...
            polygon_outline.set_fill(opacity=0)
