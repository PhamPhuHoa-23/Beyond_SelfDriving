...
class P34_38(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(all_svgs[7])
        self.wait()
        self.play(self.frame.animate.reorient(0, 0, 0, (-1.23, 0.07, 0.0), 5.51), FadeOut(all_svgs[5:15]), FadeOut(all_svgs[0][15:]), FadeOut(all_svgs[0][5:14]), FadeOut(all_svgs[4][25:]), FadeOut(all_svgs[3]), run_time=5)
        embedding_acts = np.load(data_dir / 'hook_resid_pre.npy')
        axis_1 = Axes(x_range=[0, 1.0, 1], y_range=[-1.0, 1.0, 1], width=2 * 2.4, height=2 * 0.8, axis_config={'color': CHILL_BROWN, 'include_ticks': False, 'include_numbers': False, 'include_tip': True, 'stroke_width': 2.5, 'tip_config': {'width': 0.02, 'length': 0.02}})
        axis_1.move_to([0, 0.58, 0])
        arrow_1 = Arrow(start=LEFT, end=RIGHT, buff=0.05, stroke_width=1.0, max_tip_length_to_length_ratio=0.3).scale(0.2)
        ...
        neuron_max = np.max(np.abs(embedding_acts[:, 0, 0, neuron_idx] - neuron_average)) * 1.0
        for j in range(p):
            x = j / p
        ...
        wave_label_1 = Tex('A_1 \\cos \\big( \\frac{8 \\pi }{113} x + \\phi_1 \\big)')
        ...
        self.play(ReplacementTransform(curve_1, curve_2), FadeOut(wave_label_1), run_time=5)
        self.wait()
        axes = VGroup()
        for i in range(8):
            a = Axes(x_range=[0, 1.0, 1], y_range=[-1.0, 1.0, 1], width=2.4, height=0.56, axis_config={'color': CHILL_BROWN, 'include_ticks': False, 'include_numbers': False, 'include_tip': True, 'stroke_width': 1.8, 'tip_config': {'width': 0.02, 'length': 0.02}})
            axes.add(a)
        axes.arrange(DOWN, buff=0.1)
        axes.move_to([-1.0, 0, 0])
        all_pts = VGroup()
        ...
        self.play(ReplacementTransform(axis_1, axes[0]), ReplacementTransform(pts_1, all_pts[0]), run_time=3)
        ...
        for j in range(p):
            ...
            pts_probe_1.add(pt)
        self.wait()
        self.play(*[ReplacementTransform(all_pts[i], pts_probe_1) for i in range(8)], FadeOut(all_svgs[17]), FadeOut(axes[1:]), run_time=4)
        self.wait()
        ...
        wave_label_4 = Tex('\\cos \\big(\\tfrac{8\\pi}{113}x\\big)')
        ...
        self.play(self.frame.animate.reorient(0, 0, 0, (0.0, 0.0, 0.0), 8.0), probe_1_group.animate.scale(2.0).move_to([-4, 3, 0]), wave_label_4.animate.scale(1.5).move_to([-0.85, 3.6, 0]), FadeIn(all_svgs[5:15]), FadeIn(all_svgs[0][15:]), FadeIn(all_svgs[0][5:14]), FadeIn(all_svgs[4][25:]), FadeIn(all_svgs[3]), run_time=5)
        self.add(all_svgs[:15], all_svgs[16])
        self.remove(all_svgs[7])
        self.add(all_svgs[7])
        ...
        wave_label_2b = Tex('\\sin \\big(\\tfrac{8\\pi}{113}x\\big)')
...
class P33_34(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(all_svgs[15])
        self.wait()
        self.play(self.frame.animate.reorient(0, 0, 0, (6.47, 1.97, 0.0), 3.63), FadeOut(all_pts_2), FadeOut(axes_2), run_time=5)
        self.wait()
        svg_files = list(sorted(svg_dir.glob('*fft_plots*')))
        fft_svgs = Group()
        for svg_file in svg_files[1:]:
            svg_image = SVGMobject(str(svg_file))
            fft_svgs.add(svg_image[1:])
        fft_svgs.scale(1.8)
        ...
        wave_label_2 = Tex('A_2 \\cos \\big( \\frac{8 \\pi }{113} x + \\phi_2 \\big)')
        wave_label_2.set_color(YELLOW)
        wave_label_2.scale(0.28)
        wave_label_2.move_to([4.8, 2.4, 0])
        wave_label_3 = Tex('A_3 \\cos \\big( \\frac{6 \\pi }{113} x + \\phi_3 \\big)')
        ...
        self.add(wave_label_1, wave_label_2, wave_label_3)
        self.wait()
        self.play(self.frame.animate.reorient(0, 0, 0, (0, 0, 0), 8.0), FadeOut(wave_label_1), FadeOut(wave_label_2), FadeOut(wave_label_3), FadeOut(axes), FadeOut(all_svgs[15]), FadeOut(all_pts), FadeOut(fft_svgs[0]), FadeOut(fft_svgs[1]), FadeOut(curves), FadeOut(fft_svgs[2]), FadeIn(p28_fade_group), run_time=6)
