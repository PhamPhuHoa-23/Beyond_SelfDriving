...
def evaluate_on_test_set(inference_engine: LLM, test_dataset: Dataset, tokenizer: AutoTokenizer, eos_token: str, eval_sampling_params: SamplingParams, reward_func: Callable[[str, Dict[str, Any]], Tuple[float, Dict[str, float]]]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    generations = inference_engine.generate(prompts=[{'prompt_token_ids': ids} for ids in test_dataset['input_ids']], sampling_params=eval_sampling_params)
...
def dump_episodes(episodes: Dict[str, Any], episodes_stats: Dict[str, Any], exp_dir: Path, tokenizer: AutoTokenizer, iteration: int, is_eval: bool=False, do_save: bool=True) -> Union['wandb.Table', None]:
