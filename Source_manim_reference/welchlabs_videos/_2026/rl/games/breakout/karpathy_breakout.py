...
if resume and os.path.exists(checkpoint_file):
    ...
    model = {'W1': np.random.randn(H, D) / np.sqrt(D), 'W2': np.random.randn(n_actions, H) / np.sqrt(H)}
...
def save_checkpoint(is_best=False):
    data = {'model': model, 'rmsprop_cache': rmsprop_cache, 'running_reward': running_reward, 'episode_number': episode_number, 'best_reward': best_reward}
...
try:
    while True:
        ...
        if done:
            ...
            discounted_epr /= discounted_epr.std() + 1e-08
