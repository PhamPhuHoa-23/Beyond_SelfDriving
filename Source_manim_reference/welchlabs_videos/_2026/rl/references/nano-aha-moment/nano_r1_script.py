...
import wandb
from utils import clean_up_checkpoints, close_to_zero, compute_token_log_probs, dump_episodes, evaluate_on_test_set, find_last_checkpoint, fix_oov_logits_processor, initialize_training_process_group, load_model_into_vllm, prepare_model_inputs
...
arg_parser.add_argument('--learning_rate', type=float, default=1e-06, help='Learning rate for training')
...
def preprocess_example(example: Dict[str, Any], tokenizer: AutoTokenizer, SYSTEM_MESSAGE: str, PROMPT_TEMPLATE: str):
...
def equation_reward_func(completion: str, nums: List[int], target: int) -> float:
    try:
        ...
        if abs(float(result) - float(target)) < 1e-05:
            return 1.0
        else:
            return 0.0
    except Exception:
...
def compute_reward(completion: str, sample: Dict[str, Any], EOS_TOKEN: str) -> Tuple[float, Dict[str, float]]:
    ...
    metrics = {'format_reward': format_reward, 'equation_reward': equation_reward}
    return (reward, metrics)

def create_training_episodes(*, samples: List[Dict[str, Any]]=None, all_generations: List[List[int]]=None, all_finish_reasons: List[str]=None, tokenizer: AutoTokenizer=None, EOS_TOKEN: str=None, GENERATIONS_PER_SAMPLE: int=None) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    episodes = {'all_query_token_ids': all_query_token_ids, 'all_response_token_ids': all_responses_token_ids, 'all_advantages': all_advantages}
    return (episodes, stats)

def create_vineppo_training_episodes(*, inference_engine: LLM=None, samples: List[Dict[str, Any]]=None, all_generations: List[List[int]]=None, all_finish_reasons: List[str]=None, tokenizer: AutoTokenizer=None, EOS_TOKEN_ID: int=None, EOS_TOKEN: str=None, GENERATIONS_PER_SAMPLE: int=None, MAX_RESPONSE_TOKENS: int=None, VINEPPO_K: int=None, TEMPERATURE: float=None, TOP_P: float=None, TOP_K: int=None) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    def estimate_values_by_mc_rollouts(episodes_raw: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        def get_mc_queries(query_token_ids: List[int], response_prefixes_token_ids: List[List[int]]) -> Tuple[List[List[int]], List[int]]:
            ...
            for token_ids in response_prefixes_token_ids:
                ...
                max_tokens.append(MAX_RESPONSE_TOKENS - len(token_ids))
        ...
        def update_state_values(old_states: List[int], old_value_estimates: List[float], new_states: List[int], new_value_estimates: List[float]) -> Tuple[List[int], List[float]]:
            values = {}
        ...
        mc_outputs: List[RequestOutput] = inference_engine.generate(prompt_token_ids=flatten_mc_queries, sampling_params=[SamplingParams(n=VINEPPO_K, temperature=TEMPERATURE, top_p=TOP_P, top_k=TOP_K, max_tokens=max_tokens, detokenize=False, stop_token_ids=[EOS_TOKEN_ID]) for max_tokens in flatten_mc_queries_max_tokens])
        ...
        for i, eps in enumerate(episodes_raw):
            ...
            mc_value_estimates = get_value_estimates(sample=eps['sample'], response_prefixes=eps['mc_response_prefixes_token_ids'], mcs_token_ids=mc_token_ids)
            eps['states'], eps['value_estimates'] = update_state_values(old_states=eps['states'], old_value_estimates=eps['value_estimates'], new_states=eps['new_states'], new_value_estimates=mc_value_estimates)
        for i, eps in enumerate(episodes_raw):
            eps.pop('new_states')
    ...
    for i in range(len(all_samples)):
        eps = {'query_token_ids': all_query_token_ids[i], 'response_token_ids': all_responses_token_ids[i], 'sample': all_samples[i], 'reward': all_rewards[i], 'states': [len(all_responses_token_ids[i])], 'value_estimates': [all_rewards[i]], 'new_states': split_response(all_responses_token_ids[i])}
...
def compute_pg_loss(policy_model: Union[DeepSpeedEngine, PreTrainedModel], batch: Dict[str, torch.Tensor], total_response_len: torch.Tensor, TEMPERATURE: float, KL_COEFFICIENT: float) -> Tuple[torch.Tensor, Dict[str, float]]:
    ...
    model_inputs = {'input_ids': input_ids, 'attention_mask': attention_mask, 'labels': labels, 'labels_mask': labels_mask}
    ...
    metrics = {'policy_loss': policy_loss.sum().item() / total_response_len.item(), 'kl_penalty': kl_penalty.sum().item() / total_response_len.item(), 'entropy': entropy.item() / total_response_len.item(), 'zero_advantages_ratio': zero_advantages.item() / total_response_len.item()}
    return (loss, metrics)
...
def main(rank: int):
    ...
    deepspeed_config = {'bf16': {'enabled': True}, 'zero_optimization': {'stage': 2, 'overlap_comm': False}, 'train_batch_size': EPISODES_PER_ITERATION, 'train_micro_batch_size_per_gpu': PER_DEVICE_BATCH_SIZE, 'gradient_accumulation_steps': EPISODES_PER_ITERATION_PER_RANK // PER_DEVICE_BATCH_SIZE, 'gradient_clipping': 1.0, 'optimizer': {'type': 'AdamW', 'params': {'lr': args.learning_rate, 'betas': (0.9, 0.999), 'eps': 1e-08, 'weight_decay': 0.0, 'torch_adam': True, 'fused': True}}}
    ref_deepspeed_config = {'bf16': {'enabled': True}, 'train_batch_size': EPISODES_PER_ITERATION, 'train_micro_batch_size_per_gpu': PER_DEVICE_BATCH_SIZE, 'gradient_accumulation_steps': EPISODES_PER_ITERATION_PER_RANK // PER_DEVICE_BATCH_SIZE}
    ...
    if dist.get_rank() != 0:
        dist.barrier(device_ids=[torch.cuda.current_device()])
    dataset = dataset.map(preprocess_example, num_proc=6, fn_kwargs={'tokenizer': tokenizer, 'SYSTEM_MESSAGE': SYSTEM_MESSAGE, 'PROMPT_TEMPLATE': PROMPT_TEMPLATE}, desc='Preprocessing dataset')
    if dist.get_rank() == 0:
        dist.barrier(device_ids=[torch.cuda.current_device()])
    dist.barrier(device_ids=[torch.cuda.current_device()])
    ...
    policy_model = AutoModelForCausalLM.from_pretrained(MODEL_NAME, attn_implementation='flash_attention_2', torch_dtype=torch.bfloat16, device_map=torch.cuda.current_device())
    reference_model = AutoModelForCausalLM.from_pretrained(MODEL_NAME, attn_implementation='flash_attention_2', torch_dtype=torch.bfloat16, device_map=torch.cuda.current_device())
    policy_model.gradient_checkpointing_enable(gradient_checkpointing_kwargs={'use_reentrant': False})
    see_memory_usage('Before initializing DeepSpeed engines', force=dist.get_rank() == 0)
    policy_model, *_ = deepspeed.initialize(model=policy_model, config=deepspeed_config, model_parameters=policy_model.parameters())
    reference_model, *_ = deepspeed.initialize(model=reference_model, config=ref_deepspeed_config)
    ...
    inference_engine = LLM(model=MODEL_NAME, skip_tokenizer_init=False, gpu_memory_utilization=0.3, enable_prefix_caching=True, swap_space=4, scheduling_policy='fcfs', dtype=torch.bfloat16, max_model_len=MAX_RESPONSE_TOKENS + 1024, enable_sleep_mode=True, device=f'cuda:{torch.cuda.current_device()}', tensor_parallel_size=1)
    ...
    if dist.get_rank() == 0:
        wandb.init(project='nano-aha-moment', name=RUN_NAME, resume='allow', config={'model_name': MODEL_NAME, 'learning_rate': LEARNING_RATE, 'num_iterations': NUM_ITERATIONS, 'episodes_per_iteration': EPISODES_PER_ITERATION, 'rollouts_per_episode': GENERATIONS_PER_SAMPLE, 'kl_coefficient': KL_COEFFICIENT, 'temperature': TEMPERATURE, 'algorithm': args.algorithm, 'vineppo_k': VINEPPO_K})
    ...
    for iteration in trange(begin_iter, NUM_ITERATIONS):
        ...
        metrics = {}
        eval_stats = None
        if iteration % 25 == 0 and iteration > 0 and (dist.get_rank() == 0):
            logger.info('Evaluating on eval set...')
            eval_episodes, eval_stats = evaluate_on_test_set(inference_engine=inference_engine, test_dataset=test_dataset, tokenizer=tokenizer, eos_token=EOS_TOKEN, eval_sampling_params=SamplingParams(temperature=0.3, max_tokens=MAX_RESPONSE_TOKENS, n=1, detokenize=False, stop_token_ids=[EOS_TOKEN_ID]), reward_func=lambda completion, sample: compute_reward(completion, sample, EOS_TOKEN))
        ...
        gen_time = time.time()
        outputs = inference_engine.generate(prompt_token_ids=samples['input_ids'], sampling_params=SamplingParams(n=GENERATIONS_PER_SAMPLE, temperature=TEMPERATURE, top_p=TOP_P, top_k=TOP_K, max_tokens=MAX_RESPONSE_TOKENS, detokenize=False, stop_token_ids=[EOS_TOKEN_ID], logits_processors=logits_processors))
        ...
        if args.algorithm == 'grpo':
            episodes, episodes_stats = create_training_episodes(samples=samples, all_generations=all_generations, all_finish_reasons=all_finish_reasons, tokenizer=tokenizer, EOS_TOKEN=EOS_TOKEN, GENERATIONS_PER_SAMPLE=GENERATIONS_PER_SAMPLE)
        elif args.algorithm == 'vineppo':
            episodes, episodes_stats = create_vineppo_training_episodes(inference_engine=inference_engine, samples=samples, all_generations=all_generations, all_finish_reasons=all_finish_reasons, tokenizer=tokenizer, EOS_TOKEN_ID=EOS_TOKEN_ID, EOS_TOKEN=EOS_TOKEN, GENERATIONS_PER_SAMPLE=GENERATIONS_PER_SAMPLE, MAX_RESPONSE_TOKENS=MAX_RESPONSE_TOKENS, VINEPPO_K=VINEPPO_K, TEMPERATURE=TEMPERATURE, TOP_P=TOP_P, TOP_K=TOP_K)
        else:
        ...
        episode_table = dump_episodes(episodes=episodes, episodes_stats=episodes_stats, exp_dir=EXP_DIR, tokenizer=tokenizer, iteration=iteration, do_save=iteration % 10 == 0 or iteration == 0)
        model_inputs = prepare_model_inputs(query_token_ids=episodes['all_query_token_ids'], response_token_ids=episodes['all_response_token_ids'], advantages=episodes['all_advantages'], device=curr_cuda_device)
        logger.info('Moving reference model to GPU')
        reference_model.module.to(curr_cuda_device)
        reference_model.eval()
        with torch.no_grad():
            ref_log_probs = []
            for i in trange(0, EPISODES_PER_ITERATION_PER_RANK, PER_DEVICE_BATCH_SIZE, desc='Computing reference logprobs', disable=dist.get_rank() != 0):
                batch = {k: v[i:i + PER_DEVICE_BATCH_SIZE] for k, v in model_inputs.items()}
                ref_log_probs.append(compute_token_log_probs(model=reference_model, inputs=batch, temperature=TEMPERATURE))
        ...
        if dist.get_rank() == 0:
            ...
            logs = {'iteration': iteration, f'episodes/iter_{iteration:06d}': episode_table, **{f'train/{k}': v for k, v in train_metrics.items()}}
            if eval_stats is not None:
                logs.update({f'eval/{k}': np.mean(v) for k, v in eval_stats.items()})
            wandb.log(logs)
            selected_keys = ['train/kl_penalty', 'train/rewards', 'train/reward_metrics/format_reward', 'train/reward_metrics/equation_reward', 'train/response_lengths', 'eval/rewards', 'eval/reward_metrics/format_reward', 'eval/reward_metrics/equation_reward']
