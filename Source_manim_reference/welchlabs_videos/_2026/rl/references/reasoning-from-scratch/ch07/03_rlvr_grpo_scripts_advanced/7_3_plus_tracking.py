...
def compute_grpo_loss(model, tokenizer, example, device, num_rollouts=4, max_new_tokens=512, temperature=0.8, top_p=0.9):
    ...
    loss = pg_loss
    return {'loss': loss.item(), 'pg_loss': pg_loss.item(), 'rewards': roll_rewards, 'entropies': roll_entropies, 'advantages': advantages.detach().cpu().tolist(), 'samples': samples, 'loss_tensor': loss}
...
def append_step_metrics(step_idx, total_steps, loss, reward_avg, tokens_per_sec, avg_response_len, adv_avg, adv_std, entropy_avg, eval_acc=None):
    METRICS_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    with METRICS_LOG_PATH.open('a', encoding='utf-8') as f:
        f.write(f'[Step {step_idx}/{total_steps}] loss={loss:.2f} reward_avg={reward_avg:.3f} tokens_per_sec={tokens_per_sec:.1f} avg_response_len={avg_response_len:.1f} adv_avg={adv_avg:.2f} adv_std={adv_std:.2f} entropy_avg={entropy_avg:.2f}\n')
    CSV_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    if not CSV_LOG_PATH.exists():
        CSV_LOG_PATH.write_text('step,total_steps,loss,reward_avg,tokens_per_sec,avg_response_len,adv_avg,adv_std,entropy_avg,eval_acc\n', encoding='utf-8')
    with CSV_LOG_PATH.open('a', encoding='utf-8') as f:
        eval_acc_str = '' if eval_acc is None else f'{eval_acc:.6f}'
        f.write(f'{step_idx},{total_steps},{loss:.6f},{reward_avg:.6f},{tokens_per_sec:.6f},{avg_response_len:.6f},{adv_avg:.6f},{adv_std:.6f},{entropy_avg:.6f},{eval_acc_str}\n')
...
def train_rlvr_grpo(model, tokenizer, math_data, math500_eval_data, device, steps=None, num_rollouts=9, max_new_tokens=512, temperature=0.8, top_p=0.9, lr=1e-05, checkpoint_every=50, checkpoint_dir=CHECKPOINT_DIR, eval_max_items=0):
    if steps is None:
        steps = len(math_data)
        ...
        for step in range(steps):
            ...
            if checkpoint_every and current_step % checkpoint_every == 0:
                ...
                if eval_max_items and math500_eval_data:
                    ...
                    if was_training:
                        model.train()
            append_step_metrics(current_step, steps, stats['loss'], reward_avg, tokens_per_sec, avg_response_len, adv_avg=adv_avg, adv_std=adv_std, entropy_avg=entropy_avg, eval_acc=eval_acc)
            print(f'[Step {current_step}/{steps}] loss={stats['loss']:.2f} reward_avg={reward_avg:.3f} tok/sec={tokens_per_sec:.1f} avg_resp_len={avg_response_len:.1f} adv_avg={adv_avg:.2f} adv_std={adv_std:.2f} entropy_avg={entropy_avg:.2f}')
...
if __name__ == '__main__':
    ...
    trained = train_rlvr_grpo(model=model, tokenizer=tokenizer, math_data=math_data, math500_eval_data=load_math500_test(), device=device, steps=args.steps, num_rollouts=args.num_rollouts, max_new_tokens=args.max_new_tokens, temperature=args.temperature, top_p=args.top_p, eval_max_items=args.eval_on_checkpoint)
