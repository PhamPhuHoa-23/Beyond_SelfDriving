...
def main():
    ...
    parser.add_argument('--csv', required=True, help='Metrics CSV file to plot.')
    parser.add_argument('--save_plot', help='Optional output image path.')
    parser.add_argument('--moving_average', type=int, default=0, help='Moving average window (default: 0 disables).')
    args = parser.parse_args()
    steps, losses, rewards, avg_lens, eval_steps, eval_accs = load_metrics(Path(args.csv))
    ...
    if ma_window > 1:
        ax_loss.plot(steps, losses_ma, linewidth=1.8, linestyle='--', label='Moving average')
