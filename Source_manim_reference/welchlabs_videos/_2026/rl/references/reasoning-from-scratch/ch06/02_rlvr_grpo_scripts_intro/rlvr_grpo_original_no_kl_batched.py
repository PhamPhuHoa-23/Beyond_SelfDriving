...
def train_rlvr_grpo(model, tokenizer, math_data, device, steps=None, num_rollouts=9, batch_size=None, max_new_tokens=512, temperature=0.8, top_p=0.9, lr=1e-05, checkpoint_every=50, checkpoint_dir=CHECKPOINT_DIR):
    if steps is None:
        steps = len(math_data)
...
if __name__ == '__main__':
    ...
    trained = train_rlvr_grpo(model=model, tokenizer=tokenizer, math_data=math_data, device=device, steps=args.steps, num_rollouts=args.num_rollouts, batch_size=args.batch_size, max_new_tokens=args.max_new_tokens, temperature=args.temperature, top_p=args.top_p)
