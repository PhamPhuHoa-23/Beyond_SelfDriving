...
from reasoning_from_scratch.ch03 import render_prompt, extract_final_candidate, grade_answer, load_model_and_tokenizer, load_tokenizer_only
...
@torch.no_grad()
def sample_responses_batched(model, tokenizer, prompt, device, batch_size, max_new_tokens=512, temperature=0.8, top_p=0.9):
...
def compute_grpo_loss(model, tokenizer, example, device, num_rollouts=4, batch_size=None, max_new_tokens=512, temperature=0.8, top_p=0.9):
    ...
    while remaining > 0:
        ...
        batch = sample_responses_batched(model=model, tokenizer=tokenizer, prompt=prompt, device=device, batch_size=current_batch, max_new_tokens=max_new_tokens, temperature=temperature, top_p=top_p)
...
def append_step_metrics(step_idx, total_steps, loss, reward_avg, tokens_per_sec, avg_response_len, eval_acc=None):
    METRICS_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    with METRICS_LOG_PATH.open('a', encoding='utf-8') as f:
        f.write(f'[Step {step_idx}/{total_steps}] loss={loss:.4f} reward_avg={reward_avg:.3f} tokens_per_sec={tokens_per_sec:.1f} avg_response_len={avg_response_len:.1f}\n')
    CSV_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    if not CSV_LOG_PATH.exists():
        CSV_LOG_PATH.write_text('step,total_steps,loss,reward_avg,tokens_per_sec,avg_response_len,eval_acc\n', encoding='utf-8')
    with CSV_LOG_PATH.open('a', encoding='utf-8') as f:
        eval_acc_str = '' if eval_acc is None else f'{eval_acc:.6f}'
        f.write(f'{step_idx},{total_steps},{loss:.6f},{reward_avg:.6f},{tokens_per_sec:.6f},{avg_response_len:.6f},{eval_acc_str}\n')
...
def train_rlvr_grpo(model, tokenizer, math_data, device, steps=None, num_rollouts=9, batch_size=None, max_new_tokens=512, temperature=0.8, top_p=0.9, lr=1e-05, checkpoint_every=50, checkpoint_dir=CHECKPOINT_DIR, is_main=True):
    if steps is None:
        steps = len(math_data)
    ...
    try:
        for step in range(steps):
            ...
            stats = compute_grpo_loss(model=model, tokenizer=tokenizer, example=example, device=device, num_rollouts=num_rollouts, batch_size=batch_size, max_new_tokens=max_new_tokens, temperature=temperature, top_p=top_p)
            optimizer.zero_grad()
            ...
            if is_main:
                append_step_metrics(current_step, steps, stats['loss'], reward_avg, tokens_per_sec, avg_response_len)
                if current_step % 10 == 0:
                    append_sample_logs(current_step, stats['samples'])
            if checkpoint_every and current_step % checkpoint_every == 0:
                ckpt_path = save_checkpoint(model=model, checkpoint_dir=checkpoint_dir, step=current_step, is_main=is_main)
                if is_main and ckpt_path is not None:
                    print(f'Saved checkpoint to {ckpt_path}')
            if is_main:
                print(f'[Step {current_step}/{steps}] loss={stats['loss']:.4f} reward_avg={reward_avg:.3f} tok/sec={tokens_per_sec:.1f} avg_resp_len={avg_response_len:.1f}')
    except KeyboardInterrupt:
        ckpt_path = save_checkpoint(model=model, checkpoint_dir=checkpoint_dir, step=max(1, current_step), suffix='interrupt', is_main=is_main)
        if is_main and ckpt_path is not None:
...
def main_worker(rank, world_size, args):
    ...
    try:
        ...
        trained = train_rlvr_grpo(model=model, tokenizer=tokenizer, math_data=math_data, device=device, steps=total_steps, num_rollouts=args.num_rollouts, batch_size=args.batch_size, max_new_tokens=args.max_new_tokens, temperature=args.temperature, top_p=args.top_p, is_main=is_main)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=None, help='Number of rollouts to generate in a batch (default: num_rollouts).')
    ...
    parser.add_argument('--num_gpus', type=int, default=1, help='Number of GPUs to use for FSDP (default: 1).')
