...
def append_eval_metrics(step_idx, acc, correct, total):
    METRICS_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    with METRICS_LOG_PATH.open('a', encoding='utf-8') as f:
        f.write(f'[Eval step {step_idx}] math500_acc={acc:.4f} ({correct}/{total})\n')
        ...
        for step in range(steps):
            ...
            if checkpoint_every and current_step % checkpoint_every == 0:
                ...
                if eval_max_items and math500_eval_data:
                    ...
                    if was_training:
                        model.train()
            append_step_metrics(current_step, steps, stats['loss'], reward_avg, tokens_per_sec, avg_response_len, eval_acc=eval_acc)
