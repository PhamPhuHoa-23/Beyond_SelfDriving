...
def parse_args():
    p = argparse.ArgumentParser(description='Check code-cell line lengths in specific .ipynb files.')
    p.add_argument('--max-len', type=int, default=76, help='Maximum allowed characters per line (default: 76)')
    p.add_argument('notebooks', nargs='+', help='Paths to .ipynb files to check.')
    return p.parse_args()
        ...
        for ci, cell in enumerate(nb.cells, start=1):
            ...
            for li, line in enumerate(lines, start=1):
                ...
                if length > args.max_len:
                    print(f'::error file={nb_path}::Line length {length} exceeds {args.max_len} in code cell {ci}, line {li}')
