...
def prepare_math500_pairs(math_items):
    ...
    for ex in math_items:
        ...
        if not isinstance(ref, str):
            ref = str(ref)
        pairs.append({'instruction': problem.strip(), 'reference_answer': ref.strip()})
    return pairs

def query_model(prompt, model='gpt-oss:20b', url='http://localhost:11434/api/chat', max_new_tokens=2048):
    data = {'model': model, 'messages': [{'role': 'user', 'content': prompt}], 'options': {'seed': 123, 'temperature': 0, 'num_ctx': 2048, 'num_predict': int(max_new_tokens)}}
...
def rubric_prompt(instruction, reference_answer, model_answer):
    rubric = 'You are a fair judge assistant. You will be given an instruction, a reference answer, and a candidate answer to evaluate, according to the following rubric:\n\n1: The response fails to address the instruction, providing irrelevant, incorrect, or excessively verbose content.\n2: The response partially addresses the instruction but contains major errors, omissions, or irrelevant details.\n3: The response addresses the instruction to some degree but is incomplete, partially correct, or unclear in places.\n4: The response mostly adheres to the instruction, with only minor errors, omissions, or lack of clarity.\n5: The response fully adheres to the instruction, providing a clear, accurate, and relevant answer in a concise and efficient manner.\n\nNow here is the instruction, the reference answer, and the response.\n'
    prompt = f'{rubric}\nInstruction:\n{instruction}\n\nReference Answer:\n{reference_answer}\n\nAnswer:\n{model_answer}\n\nEvaluation: '
    return prompt

def parse_score(judge_text, default=3):
    m = re.search('([1-5])(?:\\D|$)', judge_text)
    return int(m.group(1)) if m else int(default)

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--device', type=str, default='auto', help="Device e.g., 'cpu', 'cuda', 'cuda:0', 'mps'.")
    parser.add_argument('--which_model', type=str, default='base', choices=['base', 'reasoning'], help="Candidate variant to use. Defaults to 'base'.")
    parser.add_argument('--dataset_size', type=int, default=10, help='Number of MATH-500 examples to evaluate. Default: 10')
    parser.add_argument('--max_new_tokens', type=int, default=2048, help='Max new tokens for candidate generation. Default: 2048')
    parser.add_argument('--url', type=str, default='http://localhost:11434/api/chat', help="Ollama chat endpoint for the judge. Default: 'http://localhost:11434/api/chat'")
    parser.add_argument('--judge_model', type=str, default='gpt-oss:20b', help="Judge model name (Ollama). Used only for scoring. Default: 'gpt-oss:20b'")
...
def generate_with_qwen3(model, tokenizer, prompt, device, max_new_tokens):
    ...
    new_token_ids = []
    with torch.no_grad():
        for tok in generate_text_basic_stream_cache(model=model, token_ids=input_ids, max_new_tokens=max_new_tokens, eos_token_id=tokenizer.eos_token_id):
...
if __name__ == '__main__':
    ...
    for idx, ex in enumerate(pairs, start=1):
        ...
        answer = generate_with_qwen3(candidate_model, tokenizer, instruction, device, max_new_tokens).strip()
        judge_in = rubric_prompt(instruction, reference, answer)
        judge_out = query_model(judge_in, model=args.judge_model, url=args.url, max_new_tokens=256).strip()
        score = parse_score(judge_out)
        score_counts[score] += 1
        results.append({'idx': idx, 'instruction': instruction, 'reference_answer': reference, 'model_answer': answer, 'judge_text': judge_out, 'score': score, 'candidate_model': f'qwen3-{which_model}', 'judge_model': args.judge_model})
