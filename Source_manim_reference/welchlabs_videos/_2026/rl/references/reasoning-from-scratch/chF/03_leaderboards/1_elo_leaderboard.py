...
def elo_ratings(vote_pairs, k_factor=32, initial_rating=1000):
    ...
    for winner, loser in vote_pairs:
        expected = 1.0 / (1.0 + 10 ** ((ratings[loser] - ratings[winner]) / 400.0))
        ratings[winner] += k_factor * (1 - expected)
        ratings[loser] += k_factor * (0 - (1 - expected))
    return ratings

def main():
    parser = argparse.ArgumentParser(description='Compute Elo leaderboard.')
    parser.add_argument('--path', type=str, help='Path to votes JSON')
    parser.add_argument('--k', type=int, default=32, help='Elo k-factor')
    parser.add_argument('--init', type=int, default=1000, help='Initial rating')
