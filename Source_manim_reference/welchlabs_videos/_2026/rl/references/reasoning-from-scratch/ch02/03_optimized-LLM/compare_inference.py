...
parser.add_argument('--cache', action='store_true', help='Use KV cache during generation (default: False).')
parser.add_argument('--compile', action='store_true', help='Compile PyTorch model (default: False).')
parser.add_argument('--reasoning', action='store_true', help='Use reasoning model variant.')
parser.add_argument('--optimized', action='store_true', help='Use reasoning model variant.')
...
for iteration in range(1, 4):
    ...
    if is_streaming_generate:
        generated_ids = []
        for token in generate_text(model=model, token_ids=input_token_ids_tensor, max_new_tokens=max_new_tokens, eos_token_id=tokenizer.eos_token_id):
            generated_ids.append(token.squeeze(0).item())
        output_token_ids_tensor = torch.tensor(generated_ids, device=device)
    else:
        output_token_ids_tensor = generate_text(model=model, token_ids=input_token_ids_tensor, max_new_tokens=max_new_tokens, eos_token_id=tokenizer.eos_token_id)
    end_time = time.time()
