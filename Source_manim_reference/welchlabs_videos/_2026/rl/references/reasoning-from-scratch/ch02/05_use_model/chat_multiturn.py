...
parser.add_argument('--max_new_tokens', type=int, default=2048, help='Maximum number of new tokens to generate in each turn (default: 2048).')
...
tokenizer = Qwen3Tokenizer(tokenizer_file_path=tokenizer_path, apply_chat_template=False)
...
def run_generate(user_text):
    ...
    input_token_ids_tensor = trim_input_tensor(input_ids_tensor=input_token_ids_tensor, context_len=model.cfg['context_length'], max_new_tokens=args.max_new_tokens)
    start_time = time.time()
    all_token_ids = []
    print('[Model]\n', end='', flush=True)
    for tok in generate_text_basic_stream_cache(model=model, token_ids=input_token_ids_tensor, max_new_tokens=args.max_new_tokens):
