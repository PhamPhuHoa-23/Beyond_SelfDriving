from reasoning_from_scratch.qwen3 import compute_rope_params, apply_rope, QWEN_CONFIG_06_B, RMSNorm, Qwen3Model, Qwen3Tokenizer, load_hf_weights_into_qwen
...
def test_rope():
    ...
    head_dim = 16
    rope_theta = 1000000
    cos, sin = compute_rope_params(head_dim=head_dim, theta_base=rope_theta, context_length=context_len)
...
def test_rmsnorm_equivalence():
    ...
    out1 = rms_norm(x)
    out2 = ref_norm(x)
    torch.testing.assert_close(out1, out2, atol=1e-05, rtol=1e-05)
    ...
    for apply_chat_template in (True, False):
        for s in ('-Base', ''):
            ...
            for states in ((True, True), (False, False)):
                tokenizer = Qwen3Tokenizer(tokenizer_file_path=new_name, apply_chat_template=apply_chat_template, add_generation_prompt=states[0], add_thinking=states[1])
                input_token_ids = tokenizer.encode(prompt)
                if apply_chat_template:
                    input_token_ids_ref = tokenizer_ref.apply_chat_template(messages, tokenize=True, add_generation_prompt=states[0], enable_thinking=states[1], return_dict=False)
                else:
...
def test_model(ModelClass, qwen3_weights_path, generate_fn):
    ...
    model.eval()
    tokenizer = Qwen3Tokenizer(tokenizer_file_path=qwen3_weights_path / 'tokenizer-base.json', add_generation_prompt=False, add_thinking=False)
    prompt = 'Give me a short introduction to large language models.'
    input_token_ids = tokenizer.encode(prompt)
    input_token_ids = torch.tensor([input_token_ids])
    out = generate_fn(model=model, token_ids=input_token_ids.clone(), max_new_tokens=5)
