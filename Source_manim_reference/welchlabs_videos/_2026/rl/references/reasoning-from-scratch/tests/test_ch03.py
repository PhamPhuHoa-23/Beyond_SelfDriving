...
def test_generate_text_stream_concat(monkeypatch):
    ...
    tok = DummyTokenizer()
    out = ch03.generate_text_stream_concat(model=None, tokenizer=tok, prompt='ignored', device='cpu', max_new_tokens=3, verbose=False)
    assert out == 'ABA'

def test_get_last_boxed():
    cases = [('foo \\boxed{42}', '42'), ('\\boxed{a} bla \\boxed{b+c}', 'b+c'), ('noise \\boxed   {  x^2 } end', '  x^2 '), ('\\boxed{outer {inner} ok}', 'outer {inner} ok'), ('nothing here', None), ('\\boxed  not_brace', None), ('\\boxed{unbalanced', None)]
...
def test_extract_final_candidate():
    cases = [('Steps...\\n\\boxed{3/4}\\nDone.', '3/4'), ('Reasoning...\\n\\boxed{\\frac{3}{4}}', '\\frac{3}{4}'), ('Compute...\\n\\boxed{\\sqrt{2}}', '\\sqrt{2}'), ('Tuple case...\\n\\boxed{(1,2)}', '(1,2)'), ('Earlier box \\boxed{1/3}, later \\boxed{2/3}', '2/3'), ('Noisy \\boxed   {  x^2 } trailing text', 'x^2'), ('Nested braces \\boxed{outer {inner} ok}', 'outer {inner} ok'), ('In math mode: $ \\boxed{ \\dfrac{14}{3} } $', '\\dfrac{14}{3}'), ('Some steps...\nFinal Answer: 14/3', '14/3'), ('All done. 1 \nFinal 2 answer: ', '2')]
...
def test_normalize():
    cases = [('  3/4  ', '3/4'), ('\n\t  (1, 2)  \t', '(1, 2)'), ('$2/3$', '2/3'), ('\\( 2/3 \\)', '2/3'), ('\\left(1,\\,2\\right)', '(1,2)'), ('\\frac{3}{4}', '(3)/(4)'), ('\\dfrac{14}{3}', '(14)/(3)'), ('(3)/(4)', '(3)/(4)'), ('c. 3', '3'), ('b: 2', '2'), ('\\sqrt{2}', 'sqrt(2)'), ('{x}', 'x'), ('{ (1, 2) }', '(1, 2)')]
...
def test_sympy_parser():
    x, y = sp.symbols('x y')
    success_cases = [('2+3', sp.Integer(5)), ('14/3', sp.Rational(14, 3)), ('sqrt(8)/2', sp.sqrt(8) / 2), ('2*x + 3*x', 5 * x), ('2x + 3x', 5 * x), ('3(1+2)', sp.Integer(9)), ('x(1+y)', x * (1 + y)), ('pi*x', sp.pi * x)]
...
def test_split_into_parts():
    cases = [('(a, b)', ['a', 'b']), ('[x,y,z]', ['x', 'y', 'z']), ('( 1 ,  2 )', ['1', '2']), ('hello', ['hello']), ('(a)', ['(a)']), ('(a, )', ['(a, )']), ('', [])]
    for raw, expected in cases:
        got = ch03.split_into_parts(raw)
        assert got == expected, f'{raw!r} -> {got!r}, expected {expected!r}'

def test_grade_answer():
    cases = [('3/4', '\\frac{3}{4}', True), ('(3)/(4)', '3/4', True), ('\\frac{\\sqrt{8}}{2}', 'sqrt(2)', True), ('\\( \\frac{1}{2} + \\frac{1}{6} \\)', '2/3', True), ('(1, 2)', '(1,2)', True), ('(2, 1)', '(1, 2)', False), ('(1, 2, 3)', '(1, 2)', False), ('0.5', '1/2', True), ('0.3333333333', '1/3', False), ('1,234/2', '617', True), ('\\text{2/3}', '2/3', True), ('50%', '1/2', False), ('2\\cdot 3/4', '3/2', True), ('90^\\circ', '90', True), ('\\left(\\frac{3}{4}\\right)', '3/4', True), ('(1,2,3)', '(1,2,3)', True), ('(1,2,3)', '(1,2)', False), ('', '1/2', False), (None, '1/2', False), ('1/2', None, False)]
...
tests = [('check_1', '3/4', '\\frac{3}{4}', True), ('check_2', '(3)/(4)', '3/4', True), ('check_3', '\\frac{\\sqrt{8}}{2}', 'sqrt(2)', True), ('check_4', '\\( \\frac{1}{2} + \\frac{1}{6} \\)', '2/3', True), ('check_5', '(1, 2)', '(1,2)', True), ('check_6', '(2, 1)', '(1, 2)', False), ('check_7', '(1, 2, 3)', '(1, 2)', False), ('check_8', '0.5', '1/2', True), ('check_9', '0.3333333333', '1/3', False), ('check_10', '1,234/2', '617', True), ('check_11', '\\text{2/3}', '2/3', True), ('check_12', '50%', '1/2', False), ('check_13', '2\\cdot 3/4', '3/2', True), ('check_14', '90^\\circ', '90', True), ('check_15', '\\left(\\frac{3}{4}\\right)', '3/4', True)]
...
def test_run_demos_table_pass_fail_labels(capsys):
    ...
    for name, *_ in tests:
        ...
        assert status == expected_label, f'{name}: expected {expected_label} but saw {status}\n{out}'

def test_render_prompt():
    cases = ['Compute 1/2 + 1/6.', 'Find x.\nGiven y.', 'Simplify $\\frac{1}{2}$.']
    for p in cases:
        got = ch03.render_prompt(p)
        expected = f'You are a helpful math assistant.\nAnswer the question and write the final result on a new line as:\n\\boxed{{ANSWER}}\n\nQuestion:\n{p}\n\nAnswer:'
...
def test_evaluate_math500_stream(tmp_path, monkeypatch, qwen3_weights_path):
    outputs = iter(['Reasoning...\n\\boxed{A}', 'Some steps...\nTherefore, \\boxed{X}'])
    monkeypatch.setattr(ch03, 'generate_text_stream_concat', lambda *a, **k: next(outputs))
    math_data = [{'problem': 'Compute #1', 'answer': 'A'}, {'problem': 'Compute #2', 'answer': 'B'}]
    tokenizer = ch03.load_tokenizer_only(which_model='base', local_dir=qwen3_weights_path)
    out_path = tmp_path / 'math500-test.jsonl'
    num_correct, num_examples, acc = ch03.evaluate_math500_stream(model=None, tokenizer=tokenizer, device='cpu', math_data=math_data, out_path=out_path, max_new_tokens=8, verbose=False)
    assert (num_correct, num_examples) == (1, 2)
    assert abs(acc - 0.5) < 1e-09
