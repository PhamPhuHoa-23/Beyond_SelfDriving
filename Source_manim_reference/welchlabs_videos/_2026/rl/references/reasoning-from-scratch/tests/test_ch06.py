...
class DummySampleModel:
    ...
    def __call__(self, token_ids, cache=None):
        ...
        vocab_size = 4
        logits = torch.full((batch_size, seq_len, vocab_size), -1000000000.0, dtype=torch.float32)
        if seq_len > 1:
            logits[..., 2] = 0.0
        else:
            logits[..., 0] = 0.0
...
def test_sample_response_generates_until_eos():
    ...
    token_ids, prompt_len, text = ch06.sample_response(model=model, tokenizer=tokenizer, prompt='ignored', device='cpu', max_new_tokens=5, temperature=1.0, top_p=0.0)
...
def test_compute_grpo_loss_restores_training_and_returns_stats(monkeypatch):
    samples = iter([(torch.tensor([1, 2, 3]), 2, 'sample-1'), (torch.tensor([1, 2, 4]), 2, 'sample-2')])
    rewards = iter([1.0, 0.0])
    logps = iter([torch.tensor(0.4), torch.tensor(0.2)])
    monkeypatch.setattr(ch06, 'sample_response', lambda **kwargs: next(samples))
    monkeypatch.setattr(ch06, 'reward_rlvr', lambda *args, **kwargs: next(rewards))
    monkeypatch.setattr(ch06, 'sequence_logprob', lambda *args, **kwargs: next(logps))
    model = DummyTrainModel()
    example = {'problem': 'Q', 'answer': 'A'}
    result = ch06.compute_grpo_loss(model=model, tokenizer=DummyTokenizer(), example=example, device='cpu', num_rollouts=2, max_new_tokens=4, temperature=0.5, top_p=0.9)
    expected_rewards = torch.tensor([1.0, 0.0])
    advantages = (expected_rewards - expected_rewards.mean()) / (expected_rewards.std() + 0.0001)
    expected_logps = torch.stack([torch.tensor(0.4), torch.tensor(0.2)])
    ...
    assert torch.allclose(torch.tensor(result['pg_loss']), expected_pg_loss)
...
def test_save_checkpoint_writes_file(tmp_path):
    ...
    path = ch06.save_checkpoint(model=DummyModel(), checkpoint_dir=tmp_path, step=7, suffix='test')
    assert path.exists()
