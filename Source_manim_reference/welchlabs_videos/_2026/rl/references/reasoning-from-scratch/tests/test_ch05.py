...
def test_heuristic_score_rewards_boxed_answers_more_than_numbers():
    ...
    assert math.isclose(boxed_score, expected_boxed, rel_tol=1e-06)
    assert math.isclose(number_score, expected_number, rel_tol=1e-06)
...
def test_heuristic_score_adds_fulltext_bonus_when_no_number():
    ...
    score = ch05.heuristic_score(response, brevity_bonus=100.0, fulltext_bonus=0.3)
...
def test_avg_logprob_answer_uses_answer_token_logprobs():
    tokenizer = DummyTokenizer(mapping={'prompt': [1, 2], 'answer': [3, 4]})
    base_logits = [0.0, 0.1, 0.2, 0.3, 0.4]
    model = DummyLogitModel(base_logits)
    expected_logprobs = torch.log_softmax(model.base_logits, dim=-1)
    expected = torch.mean(expected_logprobs[[3, 4]]).item()
    out = ch05.avg_logprob_answer(model=model, tokenizer=tokenizer, prompt='prompt', answer='answer', device='cpu')
    assert math.isclose(out, expected, rel_tol=1e-06)
...
def test_prompt_builders_embed_question_and_context():
    ...
    refine_prompt = ch05.make_refine_prompt(raw_prompt, draft, critique_text)
...
def test_self_refinement_loop_accepts_improving_revisions(monkeypatch):
    responses = iter(['initial draft', 'first critique', 'draft with more detail', 'second critique', 'bad'])
    prompts_seen = []

    def fake_generate_text_stream_concat_flex(**kwargs):
        prompts_seen.append(kwargs.get('prompt'))
        return next(responses)
    monkeypatch.setattr(ch05, 'generate_text_stream_concat_flex', fake_generate_text_stream_concat_flex)

    def score_fn(answer, prompt):
        return len(answer)
    result = ch05.self_refinement_loop(model=None, tokenizer=DummyTokenizer(), raw_prompt='Compute something', device='cpu', iterations=2, score_fn=score_fn, prompt_renderer=lambda x: f'Q: {x}', temperature=0.3, top_p=0.8)
