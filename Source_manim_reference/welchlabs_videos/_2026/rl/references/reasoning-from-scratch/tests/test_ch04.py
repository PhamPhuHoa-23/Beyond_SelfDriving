...
def test_generate_text_stream_concat_flex_uses_custom_generator():
    ...
    out = ch04.generate_text_stream_concat_flex(model=None, tokenizer=tok, prompt='ignored', device='cpu', max_new_tokens=2, generate_func=fake_generate_func, temperature=0.5)
    assert out == 'XY'
...
def test_top_p_filter_truncates_and_renormalizes():
    ...
    assert torch.allclose(filtered, torch.tensor([[0.5555556, 0.4444444, 0.0]]))
...
def test_generate_text_temp_stream_cache_stops_on_eos():
    ...
    out = list(ch04.generate_text_temp_stream_cache(model, token_ids=token_ids, max_new_tokens=5, eos_token_id=3, temperature=0.0))
    assert out == []
...
def test_self_consistency_vote_majority(monkeypatch):
    ...
    result = ch04.self_consistency_vote(model=None, tokenizer=DummyTokenizer(), prompt='unused', device='cpu', num_samples=3, temperature=0.7, top_p=0.9, max_new_tokens=5, show_progress=False, show_long_answer=False, seed=123)
