...
class P60_64(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ShowCreation(all_svgs[4]), run_time=4)
        ...
        self.play(Write(all_svgs[5][1:]), FadeIn(all_svgs[9]))
        self.wait()
        self.play(self.frame.animate.reorient(0, 0, 0, (0.5, 0.01, 0.0), 7.24), Write(all_svgs[0][2:]), FadeOut(all_svgs[9]), run_time=4)
        self.wait()
        self.play(Write(all_svgs[1]), Write(all_svgs[2]), run_time=3)
        self.remove(all_svgs[0][2:])
        self.add(all_svgs[0][2:])
        img_1.scale(0.5)
        img_1.next_to(all_svgs[0], DOWN, buff=0.1)
        self.wait()
        self.play(FadeIn(all_svgs[3]), FadeIn(img_1), FadeIn(all_svgs[0][:2]), run_time=3)
        rect = all_svgs[6][11]
        target = rect.copy()
        rect.stretch(0.001, dim=1, about_edge=DOWN)
        self.wait()
        self.play(Transform(rect, target), Write(all_svgs[6][:11]), Write(all_svgs[6][12:]), run_time=2.5)
        rect = all_svgs[7][0]
        target = rect.copy()
        rect.stretch(0.001, dim=1, about_edge=DOWN)
        self.wait()
        self.play(Transform(rect, target), Write(all_svgs[7][1:]), Write(all_svgs[8]), run_time=2.5)
        self.wait(20)
        self.embed()