...
class P75_80(InteractiveScene):
    ...
    def construct(self):
        ...
        all_svgs[0].shift([0, -0.2, 0])
        self.wait()
        self.play(Write(all_svgs[0]), run_time=4)
        self.wait()
        self.play(Write(all_svgs[5][::-1]), Write(all_svgs[4][:11]), Write(all_svgs[4][11:22]), run_time=4)
        self.wait()
        self.play(Write(all_svgs[1]), Write(all_svgs[18]), Write(all_svgs[4][22:29]), Write(all_svgs[4][29:36]), run_time=4)
        self.add(all_svgs[3][:4])
        self.wait()
        self.play(Write(all_svgs[17]), Write(all_svgs[3][6:]), Write(all_svgs[19]), Write(all_svgs[4][36:45]), run_time=5)
        self.wait()
        self.play(Write(all_svgs[4][45:]), Write(all_svgs[3][4:6]), run_time=3)
        imgs = Group()
        for i in range(NUM_FRAME_TO_RENDER):
            imgs.add(ImageMobject(str(img_dir + '/overhead_ball_3' + str(i).zfill(3) + '.jpg')))
        embedding_1 = Tex('\\begin{bmatrix} 0.21, \\ -0.11, \\ \\dots, \\ 0.32 \\end{bmatrix}')
        embedding_1.set_color(BLUE)
        embedding_1.scale(0.65)
        embedding_1.move_to([-2.68, 1.35, 0])
        embedding_2 = Tex('\\begin{bmatrix} 0.22, \\ -0.13, \\ \\dots, \\ 0.31 \\end{bmatrix}')
        embedding_2.set_color(BLUE)
        embedding_2.scale(0.65)
        embedding_2.move_to([2.6, 1.6, 0])
        embedding_3 = Tex('\\begin{bmatrix} 0.21, \\ -0.10, \\ \\dots, \\ 0.35 \\end{bmatrix}')
        ...
        group_to_remove = Group(all_svgs[5], all_svgs[4][:22], all_svgs[4][45:], all_svgs[3], all_svgs[19], all_svgs[3][6:10])
        self.wait()
        self.play(FadeOut(group_to_remove), self.frame.animate.reorient(0, 0, 0, (0.0, -0.17, 0.0), 7.17), all_svgs[0].animate.move_to([0, 2.85, 0]), all_svgs[1].animate.scale(0.8).move_to([-2.68, -0.2, 0]), all_svgs[17].animate.scale(0.8).move_to([-0.14, 1.35, 0]), all_svgs[18].animate.scale(0.8).move_to([2.72, -0.2, 0]), all_svgs[4][22:29].animate.scale(0.9).move_to([-4.19, -0.2, 0.0]), all_svgs[4][36:45].animate.scale(0.9).move_to([-0.14, 2.2, 0]), all_svgs[4][29:36].animate.scale(0.9).move_to([4.19, -0.2, 0.0]), run_time=2)
        all_svgs[3].scale(0.9)
        all_svgs[3][:2].set_color(CHILL_BROWN).move_to([-2.68, -1.0, 0])
        all_svgs[3][2:4].set_color(CHILL_BROWN).move_to([2.72, -1.0, 0])
        all_svgs[3][10:12].set_color(CHILL_BROWN).move_to([2.72, 0.55, 0])
        (all_svgs[6][1].move_to([-2.68, 0.69, 0]),)
        (all_svgs[6][3].move_to([0.7, 1.5, 0]),)
        (all_svgs[6][0].move_to([-0.85, 1.34, 0]),)
        all_svgs[8][14:].move_to([-5.1, 1.33, 0])
        all_svgs[8][5:14].move_to([2.8, -3.26, 0])
        all_svgs[8][:5].move_to([-5.0, -2.52, 0])
        (all_svgs[9].move_to([5.0, 1.3, 0]),)
        self.wait()
        self.play(FadeIn(input_video_and_borders), FadeIn(border_next_frame), FadeIn(imgs[5]), FadeIn(all_svgs[3][:2]), FadeIn(all_svgs[3][2:4]), run_time=3)
        self.add(all_svgs[8][:5], all_svgs[8][5:14])
        self.wait()
        self.play(Write(embedding_1), Write(embedding_3), Write(all_svgs[3][10:12]), Write(all_svgs[6][1]), FadeIn(all_svgs[8][14:]), run_time=3)
        self.wait()
        self.play(Write(embedding_2), Write(all_svgs[6][0]), Write(all_svgs[6][3]), run_time=4)
        self.add(all_svgs[9])
        ...
        base_e3 = np.array([0.21, -0.1, 0.35])
        ...
        def make_emb_tex(vals, color, position, scale=0.65):
            s = '\\begin{bmatrix} ' + f'{vals[0]:.2f}' + ', \\ ' + f'{vals[1]:.2f}' + ', \\ \\dots, \\ ' + f'{vals[2]:.2f}' + ' \\end{bmatrix}'
            e = Tex(s)
            e.set_color(color)
            e.scale(scale)
            e.move_to(position)
            return e
        current_stack = [imgs[i] for i in range(5)]
        current_next = imgs[5]
        step_run_time = 0.1
