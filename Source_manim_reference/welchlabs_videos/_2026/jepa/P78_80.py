...
class P78_80(InteractiveScene):
    ...
    def construct(self):
        ...
        self.add(all_svgs[0], all_svgs[1], all_svgs[2], all_svgs[3], all_svgs[4], all_svgs[5])
        self.wait()
        ...
        self.play(Write(all_svgs[8][1:]), all_svgs[6][0].animate.move_to([-0.233, 0.95, 0]), run_time=3)
        self.play(self.frame.animate.reorient(0, 0, 0, (-0.19, 0.27, 0.0), 7.43), FadeOut(all_svgs[1]), run_time=2)
        self.add(imgs[0])
        self.wait(0.2)
        self.add(imgs[1])
        self.wait(0.2)
        ...
        for i in range(4, NUM_FRAMES):
            ...
            self.wait(0.1)
        self.play(FadeOut(imgs[0]), FadeOut(imgs[1]), FadeOut(imgs_left[i - 1]), FadeOut(imgs_right[i]), FadeOut(all_svgs[7]), run_time=2)
        all_svgs[10].move_to([-0.15, -2.9, 0])
        self.play(FadeIn(imgs_right[i]), Write(all_svgs[10][5:]), FadeIn(imgs[0]), FadeIn(imgs[1]), FadeIn(imgs[2]), Write(all_svgs[10][:5]), run_time=2)
        embedding_1 = Tex('\\begin{bmatrix} 0.22, \\ -0.13, \\ \\dots, \\ 0.31 \\end{bmatrix}')
        embedding_1.set_color(BLUE)
        embedding_1.scale(0.5)
        embedding_1.move_to([2.3, 2.0, 0])
        embedding_1_og = embedding_1.copy()
        embedding_2 = Tex('\\begin{bmatrix} 0.05, \\ -0.02, \\ \\dots, \\ 0.50 \\end{bmatrix}')
        embedding_2.set_color(YELLOW)
        embedding_2.scale(0.5)
        embedding_2.move_to([2.3, 1.4, 0])
        self.wait()
        self.play(FadeOut(all_svgs[5][1:]))
        self.play(Write(embedding_2), FadeIn(all_svgs[11][:13]), run_time=2)
        self.wait()
        all_svgs[11][13:].set_color(BLUE)
        self.play(Write(embedding_1), FadeIn(all_svgs[11][13:]), Write(all_svgs[9]), run_time=2)
