...
class P28_33c(InteractiveScene):
    ...
    def construct(self):
        ...
        self.frame.reorient(0, 0, 0, (-0.0, -0.0, 0.0), 3.5)
        self.wait()
        for i in range(90):
            if i > 0:
                self.remove(imgs[i - 1])
            self.add(imgs[i])
            self.wait(0.05)
        input_video = Group()
        input_borders = Group()
        ...
        self.wait()
        self.play(self.frame.animate.reorient(0, 0, 0, (4.78, -0.39, 0.0), 10.52), Transform(input_video_and_borders, target), run_time=5)
        self.wait()
        ...
        self.play(ReplacementTransform(t1, t3[:3]), ReplacementTransform(t2, t3[3:]), run_time=3)
        ...
        self.play(ReplacementTransform(t3, t5[:-2]), ReplacementTransform(t4, t5[-2:]), run_time=3)
        ...
        self.play(ReplacementTransform(t5, t7[:-6]), ReplacementTransform(t6, t7[-6:]), run_time=3)
        ...
        self.play(ReplacementTransform(t7, t9[:-2]), ReplacementTransform(t8, t9[-2:]), run_time=3)
        ...
        top_frame = input_video_and_borders[0][5]
        prev_frame = input_video_and_borders[0][4]
        ...
        target_110.scale(1.0 / 0.95)
        target_110.rotate(72 * DEGREES, [0, 1, 0]).rotate(35 * DEGREES, [1, 0, 0])
        target_110[0].set_opacity(0.5)
        target_110[1].set_stroke(width=2, opacity=0.8)
        target_110.move_to(target_center)
        self.wait()
        self.play(Transform(group_110, target_110), all_svgs[0].animate.shift([0.35, 0, 0]), self.frame.animate.reorient(0, 0, 0, (5.06, -0.56, 0.0), 10.73), run_time=3)
        ...
        self.wait()
        self.add(group_120)
        top_frame = input_video_and_borders[0][6]
        prev_frame = input_video_and_borders[0][5]
        stack_offset = top_frame.get_center() - prev_frame.get_center()
        target_center = top_frame.get_center() + stack_offset
        target_120 = group_120.copy()
        target_120.scale(1.0 / 0.95)
        target_120.rotate(72 * DEGREES, [0, 1, 0]).rotate(35 * DEGREES, [1, 0, 0])
        target_120[0].set_opacity(0.5)
        target_120[1].set_stroke(width=2, opacity=0.8)
        target_120.move_to(target_center)
        self.wait()
        self.play(Transform(group_120, target_120), all_svgs[0].animate.shift([0.45, 0, 0]), self.frame.animate.reorient(0, 0, 0, (5.22, -0.73, 0.0), 10.94), run_time=3)
...
class P28_33a(InteractiveScene):
    ...
    def construct(self):
        ...
        self.frame.reorient(0, 86, 0, (0.18, 0.31, 0.15), 8.0)
        self.wait()
