...
class Intro(InteractiveScene):
    ...
    def construct(self):
        ...
        all_svgs[0][1].scale(0.8)
        all_svgs[0][1].next_to(all_svgs[4][-1], RIGHT, buff=0.45)
        all_svgs[0][1].shift([0, -0.06, 0])
        all_svgs[0][0].scale(0.8)
        all_svgs[0][0].next_to(all_svgs[4][-2], LEFT, buff=0.5)
        self.wait()
        self.play(Write(all_svgs[4][0]), Write(all_svgs[4][-2:]), Write(all_svgs[2]), run_time=3)
        self.wait()
        self.add(all_svgs[0][1])
        self.wait()
        self.add(all_svgs[0][0])
        self.wait()
        self.remove(all_svgs[2])
        self.play(ReplacementTransform(all_svgs[0][0], all_svgs[3][0]), FadeIn(all_svgs[4][1:-2]), Write(all_svgs[5][:-5]), self.frame.animate.reorient(0, 0, 0, (-0.32, 1.35, 0.0), 6.2), run_time=4)
        self.wait()
        self.play(ReplacementTransform(all_svgs[0][1], all_svgs[3][1]), Write(all_svgs[5][-5:]), run_time=3)
        self.wait()
        self.play(Write(all_svgs[7]), self.frame.animate.reorient(0, 0, 0, (-0.24, -0.02, 0.0), 6.87), run_time=4)
        cat_img.scale(0.52)
        cat_img.move_to([-3.4, -1.3, 0])
        self.play(FadeIn(cat_img), FadeIn(all_svgs[6][0]))
        self.play(Write(all_svgs[8]), FadeIn(all_svgs[6][1]), run_time=2)
        self.wait()
        self.remove(all_svgs[3], all_svgs[4], all_svgs[5], all_svgs[7], cat_img, all_svgs[8])
        self.play(Write(all_svgs[9]), self.frame.animate.reorient(0, 0, 0, (-0.07, 0.23, 0.0), 7.27), ReplacementTransform(all_svgs[6][0], all_svgs[10][0]), ReplacementTransform(all_svgs[6][1], all_svgs[10][1]), run_time=4)
        self.wait()
