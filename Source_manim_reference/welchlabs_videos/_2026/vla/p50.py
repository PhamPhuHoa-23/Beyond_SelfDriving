...
class P50(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(*[img.animate.match_height(targets[i]).move_to(targets[i].get_center()) for i, img in enumerate(expand_images)], *[sl.animate.move_to(target_label_positions[i].get_center()) for i, sl in enumerate(step_labels)], frame.animate.set_width(target_frame_width).move_to(targets.get_center()), run_time=3)
        self.wait()
        self.embed()