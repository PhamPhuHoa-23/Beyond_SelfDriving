...
class P51(InteractiveScene):
    ...
    def construct(self):
        ...
        col_rect.move_to([colorbar_10.get_center()[0], colorbar_10.get_top()[1] - row_h * (ROW_IDX + 0.5), 0])
        line_width = 5.5
        ...
        shoulder_line = VMobject().set_points_as_corners(np.column_stack([x_pts, y_pts, np.zeros(len(col_vals))]))
...
class P51v2(InteractiveScene):
    ...
    def construct(self):
        ...
        shoulder_line = VMobject().set_points_as_corners(np.column_stack([x_pts, y_pts, np.zeros(len(col_vals))])).set_color('#FFC107').set_stroke(width=2)
        ...
        self.add(pins)
        pin_anims = LaggedStart(*[pin.animate.move_to([x_pts[i], y_pts[i], 0]).set_height(0.06) for i, pin in enumerate(pins)], lag_ratio=0.04)
        self.play(LaggedStart(pin_anims, ShowCreation(shoulder_line), lag_ratio=0.6), run_time=5.5)
        self.wait(0.5)
