...
class P1_6c(InteractiveScene):
    ...
    def construct(self):
        ...
        manual_box = RoundedRectangle(width=all_svgs[0][0].get_width(), height=all_svgs[0][0].get_height(), corner_radius=0.025, stroke_color=CHILL_BROWN, stroke_width=4, fill_opacity=0)
        manual_box.move_to(all_svgs[0][0].get_center())
        self.frame.reorient(0, 0, 0, (-3.11, 1.82, 0.0), 3.99)
        self.wait()
        self.play(FadeIn(coke_start_img), ShowCreation(manual_box), all_svgs[0][1:].animate.set_opacity(0.5), run_time=6)
        self.wait()
        self.play(self.frame.animate.reorient(0, 0, 0, (-3.14, 1.79, 0.0), 3.6), run_time=6)
        self.wait()
        self.play(Write(all_svgs[1]), Write(all_svgs[2]), self.frame.animate.reorient(0, 0, 0, (-1.36, 1.84, 0.0), 5.51), run_time=5)
        self.wait()
        self.remove(coke_start_img)
        self.wait()
        self.add(coke_end_img)
        self.remove(manual_box)
        self.add(manual_box)
        ...
        self.wait()
        self.play(Write(all_svgs[3]), self.frame.animate.reorient(0, 0, 0, (-1.24, 0.78, 0.0), 5.74), run_time=6)
        self.play(FadeIn(quote_group[1:]), run_time=2)
        manual_box_2 = RoundedRectangle(width=all_svgs[-1][0].get_width(), height=all_svgs[-1][0].get_height(), corner_radius=0.025, stroke_color=CHILL_BROWN, stroke_width=4, fill_opacity=0)
        ...
        self.wait()
        self.play(self.frame.animate.reorient(0, 0, 0, (3.29, -1.6, 0.0), 4.23), ShowCreation(manual_box_2), ReplacementTransform(all_svgs[4][:-1].copy(), all_svgs[6][77:89]), all_svgs[7].animate.set_opacity(0.5), Write(all_svgs[6][:77]), Write(all_svgs[6][89:]), run_time=6)
        self.play(FadeIn(keyring_start_img), run_time=3)
        self.remove(manual_box_2)
        ...
        self.wait()
        self.play(self.frame.animate.reorient(0, 0, 0, (0.24, 0.03, 0.0), 7.63), run_time=8)
        self.wait(20)
        self.embed()