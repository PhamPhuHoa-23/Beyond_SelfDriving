...
if not os.path.exists('/root/openpi'):
    subprocess.run(['git', 'clone', '--recurse-submodules', 'https://github.com/Physical-Intelligence/openpi.git', '/root/openpi'], check=True)
subprocess.run([UV, 'pip', 'install', '--system', '-e', '/root/openpi'], check=True)
subprocess.run([UV, 'pip', 'install', '--system', '--reinstall-package', 'typing_extensions', '--reinstall-package', 'transformers', 'matplotlib', 'datasets', 'diffusers', 'accelerate', 'safetensors', 'sentencepiece', 'typing_extensions>=4.12', 'pytest', 'chex', 'transformers==4.53.2'], check=True)
subprocess.run([UV, 'pip', 'install', '--system', 'jax[cuda12]==0.5.3', 'jaxlib==0.5.3', 'numpy<2.0.0'], check=True)
subprocess.run([UV, 'pip', 'install', '--system', '-e', '/root/openpi'], check=True)
...
_tc.disable = _orig_compiler_disable
...
def frame_to_obs(frame):
    ...
    return {'images': {'cam_high': to_chw(frame['observation.images.cam_high']), 'cam_left_wrist': to_chw(frame['observation.images.cam_left_wrist']), 'cam_right_wrist': to_chw(frame['observation.images.cam_right_wrist'])}, 'state': np.array(frame['observation.state'], dtype=np.float32), 'prompt': 'uncap the pen'}
...
for seq_idx in range(NUM_CAT_SEQUENCES):
    ...
    result = pipe(prompt=PROMPT, negative_prompt=NEGATIVE_PROMPT, num_inference_steps=100, guidance_scale=7.0, generator=torch.Generator(device='cuda').manual_seed(seed), callback_on_step_end=capture_latents)
    ...
    for i, latents in enumerate(intermediate_latents):
        with torch.no_grad():
            ...
            img = Image.fromarray((decoded[0].permute(1, 2, 0).cpu().float().numpy() * 255).astype('uint8'))
