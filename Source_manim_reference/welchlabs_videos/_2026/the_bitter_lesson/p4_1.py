...
class Node(VGroup):
    ...
    def __init__(self, phoneme, node_id, x, y, buff=0.2, scale_factor=SCALE_FACTOR, **kwargs):
        ...
        self.box = RoundedRectangle(width=self.text.get_width() + 2 * buff, height=self.text.get_height() + 2 * buff, corner_radius=0.15, color=CHILL_BROWN)
...
class Connection(VGroup):
    ...
    def __init__(self, node_from, node_to, **kwargs):
        ...
        self.arrow = Arrow(node_from.get_center(), node_to.get_center(), buff=0.5, thickness=0.5, fill_color=CHILL_BROWN)
...
class P4(Scene):
    ...
    def construct(self):
        ...
        self.add(progress)
        self.play(progress.animate.set_value(1), run_time=total_duration, rate_func=bezier_rate)
    ...
    def construct(self):
        ...
        for layer_idx, layer in enumerate(network.layers):
            ...
            if arrows_to_show and shown_mobjects:
                ...
                self.play(*[GrowArrow(arrow) for arrow in arrows_to_show], self.camera.frame.animate.set_width(needed_width).move_to(group.get_center()))
        self.wait()
...
class RenderNetworkV2(Scene):
    ...
    def construct(self):
        ...
        for node_data in data['nodes']:
            ...
            box = RoundedRectangle(width=text.get_width() + 0.4, height=text.get_height() + 0.4, corner_radius=0.15, color=CHILL_BROWN)
        ...
        for edge_data in data['edges']:
            ...
            if source_id in nodes and target_id in nodes:
                arrow = Arrow(nodes[source_id].get_center(), nodes[target_id].get_center(), buff=0.5, thickness=0.5, fill_color=CHILL_BROWN)
