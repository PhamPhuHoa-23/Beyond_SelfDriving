...
class Node(VMobject):
    ...
    def __init__(self, node_radius=0.1, node_stroke_color=CHILL_BROWN, node_stroke_width=3, value=0.5, label='A', font_size=24, **kwargs):
    ...
    def build(self):
        self.clear()
        circle = Circle(radius=self.node_radius, stroke_color=self.node_stroke_color, stroke_width=self.node_stroke_width, fill_color=WHITE, fill_opacity=1.0)
...
class Arrow(VMobject):
    ...
    def __init__(self, start_obj, end_obj, stroke_width=2, value=0.5, weight=1.0, stroke_color=CHILL_BROWN, dashed=False, buff=0.0, **kwargs):
        super().__init__(**kwargs)
    ...
    def build(self):
        ...
        line = ManimArrow(start=start_point, end=end_point, stroke_color=self.stroke_color, stroke_width=self.stroke_width, stroke_opacity=self.stroke_opacity, fill_color=self.stroke_color, fill_opacity=self.stroke_opacity, thickness=1.5, buff=0)
        if self.dashed:
...
class Layer(VMobject):
    ...
    def __init__(self, values=None, labels=None, max_display=16, node_radius=0.1, node_spacing=0.3, node_stroke_color=CHILL_BROWN, node_stroke_width=6, colormap=plt.get_cmap('viridis'), position=ORIGIN, **kwargs):
        super().__init__(**kwargs)
        ...
        self.ellipsis_size = self.node_radius * 0.1
        self.ellipsis_spacing = self.node_radius * 0.5
        self.nodes = VGroup()
    ...
    def _create_simple_layer(self):
        ...
        for i, value in enumerate(self.values):
            ...
            node = Node(node_radius=self.node_radius, node_stroke_color=self.node_stroke_color, node_stroke_width=self.node_stroke_width, value=value, label=self.labels[i]).move_to(UP * y)
            self.nodes.add(node)

    def _create_truncated_layer(self):
        visible_per_side = self.max_display // 2
        ...
        for i in range(visible_per_side):
            ...
            node = Node(node_radius=self.node_radius, node_stroke_color=self.node_stroke_color, node_stroke_width=self.node_stroke_width, value=self.values[i], label=self.labels[i]).move_to(UP * y)
            self.nodes.add(node)
        ...
        for i in range(visible_per_side):
            ...
            node = Node(node_radius=self.node_radius, node_stroke_color=self.node_stroke_color, node_stroke_width=self.node_stroke_width, value=self.values[idx], label=self.labels[idx]).move_to(UP * y)
            self.nodes.add(node)
...
class Graph(VMobject):
    ...
    def __init__(self, weight_matrices, layer_labels=None, layer_spacing=2.5, max_display=10, node_radius=0.1, node_spacing=0.3, node_stroke_color=CHILL_BROWN, node_stroke_width=6, arrow_stroke_width=2, **kwargs):
        super().__init__(**kwargs)
    ...
    def build(self):
        ...
        for i, size in enumerate(layer_sizes):
            ...
            layer = Layer(values=values, labels=labels, max_display=self.max_display, node_radius=self.node_radius, node_spacing=self.node_spacing, node_stroke_color=self.node_stroke_color, node_stroke_width=self.node_stroke_width, colormap=plt.get_cmap('viridis'))
            ...
            for r in range(matrix.shape[0]):
                for c in range(matrix.shape[1]):
                    ...
                    if node_start is None or node_end is None:
                        continue
                    weight = matrix[r, c]
                    arrow = Arrow(node_start, node_end, stroke_width=self.arrow_stroke_width, value=np.clip(abs(weight), 0, 1), weight=weight, stroke_color=CHILL_BROWN)
...
class P10(Scene):
    ...
    def construct(self):
        ...
        graph = Graph(weight_matrices=[W1, W2, W3, W4, W5, W6, W7, W8], layer_labels=phoneme_labels, layer_spacing=1.2, max_display=6, node_radius=0.25, node_spacing=0.9, node_stroke_color=BLACK, node_stroke_width=3.0, arrow_stroke_width=0)
        self.add(graph)
        self.remove(graph[1])
        self.add(graph[1])
        self.embed()
...
class P10v2(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(*node_anims, *ellipsis_anims, lag_ratio=0.1), run_time=1.5)
        self.wait(0.5)
        for layer_idx in range(len(graph.layers) - 1):
            ...
            self.play(LaggedStart(*arrow_anims, lag_ratio=0.05), LaggedStart(*next_node_anims, *next_ellipsis_anims, lag_ratio=0.1), run_time=1.5)
        self.embed()
...
class P10v3(Scene):
    ...
    def construct(self):
        ...
        ellipsis_anims = [FadeIn(dot) for dot in first_layer.ellipsis_dots]
        self.wait()
        self.play(LaggedStart(*node_anims, *ellipsis_anims, lag_ratio=0.1), run_time=3.0)
        self.wait(0.5)
        all_anims_with_pos = []
        for arrow in graph.arrows:
        ...
        self.wait()
        self.play(LaggedStart(*sorted_anims, lag_ratio=0.02), run_time=20)
        self.wait(20)
        self.embed()
