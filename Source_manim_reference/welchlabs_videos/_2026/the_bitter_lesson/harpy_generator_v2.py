...
sentences_old = ['tell us about nixon', 'give me the headlines', 'tell me about china', 'give us the stories', 'tell me about the stock market', 'tell us about recent events', 'tell me about russia', 'show me the weather', 'show us the news stories', 'give me the current events', 'show us the top stories', "give us today's stories", 'give us news articles', 'tell me all about china', 'tell us about china', 'tell us all about china', 'tell me about nixon', 'tell me all about nixon', 'tell us all about nixon', 'give me the news']
    ...
    with open(filename, 'w') as f:
        ...
        for node in nodes:
            ...
            if phone in ('<s>', '</s>'):
                ...
                shape = 'circle'
            f.write(f'  {node.id} [label="{phone}", shape={shape}, fillcolor="{color}"];\n')
        f.write('\n')
        for node in nodes:
            for child in node.edges.values():
                f.write(f'  {node.id} -> {child.id};\n')
...
def export_dag_to_json(nodes, filename='phoneme_dag.json'):
    ...
    nodes_data = []
    for node in nodes:
        nodes_data.append({'id': node.id, 'phoneme': node.phoneme, 'connects_from': incoming_connections[node.id]})
    with open(filename, 'w') as f:
