...
START = '<START>'
END = '<END>'
PHONEME_CLASSES = {'vowel': {'AA', 'AE', 'AH', 'AO', 'AW', 'AY', 'EH', 'ER', 'EY', 'IH', 'IY', 'OW', 'OY', 'UH', 'UW'}, 'stop': {'P', 'B', 'T', 'D', 'K', 'G'}, 'fricative': {'F', 'V', 'TH', 'DH', 'S', 'Z', 'SH', 'ZH', 'HH'}, 'nasal': {'M', 'N', 'NG'}, 'liquid': {'L', 'R'}, 'glide': {'W', 'Y'}, 'silence': {'<START>', '<END>'}}
PHONEME_COLORS = {'vowel': 'lightskyblue', 'stop': 'lightcoral', 'fricative': 'khaki', 'nasal': 'lightgreen', 'liquid': 'plum', 'glide': 'orange', 'silence': 'gray90', 'unknown': 'white'}
...
def export_dag_to_json(nodes, positions, filename='phone_dag.json'):
    ...
    for node in nodes:
        ...
        for connection_id in curr_connections:
            ...
            relative_coordinates[connection_id] = {'rel_x': rel_x, 'rel_y': rel_y}
        nodes_data.append({'id': node.id, 'phoneme': node.phoneme, 'connects_from': incoming_connections[node.id], 'relative_coordinates': relative_coordinates})
    ...
    for node in nodes:
        ...
        json_nodes.append({'id': node.id, 'phoneme': phone, 'class': cls, 'color': PHONEME_COLORS.get(cls, 'white'), 'shape': 'box' if phone in ('<START>', '<END>') else 'circle', 'x': float(positions[node.id][0]), 'y': float(positions[node.id][1])})
    for node in nodes:
        for child in node.edges.values():
            json_edges.append({'source': node.id, 'target': child.id})
    graph = {'nodes': json_nodes, 'edges': json_edges}
    with open(filename, 'w') as f:
        json.dump(graph, f, indent=2)

def organic_layout(nodes, depths, x_spacing=200, base_y_spacing=35, attraction=0.25, jitter_y=30, jitter_x=20, min_spacing=12, repel_strength=0.5, iterations=4, seed=42):
    random.seed(seed)
...
def organic_layout_with_loops(nodes, depths, x_spacing=200, base_y_spacing=35, attraction=0.25, jitter_y=30, jitter_x=20, min_spacing=12, repel_strength=0.5, iterations=4, seed=42):
    random.seed(seed)
    layers = defaultdict(list)
    for n in nodes:
        layers[depths[n.id]].append(n.id)
...
def main():
    ...
    phoneme_sentences_brown_sample_n = [sentence_to_phonemes(s, cmu_dict=cmu) for s in full_sentences_brown_sample_n]
    root, nodes = build_phoneme_dag(phoneme_sentences_brown_sample_n)
    depths = compute_depths(root)
    positions_organic = organic_layout(nodes=nodes, depths=depths, x_spacing=200, base_y_spacing=35, attraction=0.25, jitter_y=75, jitter_x=75, min_spacing=12, repel_strength=0.5, iterations=4, seed=42)
    positions_organic_with_loops = organic_layout_with_loops(nodes=nodes, depths=depths, x_spacing=200, base_y_spacing=35, attraction=0.25, jitter_y=75, jitter_x=75, min_spacing=12, repel_strength=0.5, iterations=4, seed=42)
