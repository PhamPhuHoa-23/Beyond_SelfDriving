...
class P4a(Scene):
    ...
    def construct(self):
        with open(json_dir / 'phone_dag_v2.json', 'r') as f:
            data = json.load(f)
        nodes_to_render = -1
        all_nodes_list = copy.deepcopy(small_graph_nodes[:-1])
        ...
        for start_idx, end_idx in all_edges:
            if start_idx not in node_mobjects:
                continue
            if end_idx not in node_mobjects:
                continue
        ...
        self.add(all_nodes)
        self.wait()
        self.play(self.frame.animate.reorient(0, 0, 0, (-0.11, 0.17, 0.0), 14.76), run_time=30)
        self.wait()
