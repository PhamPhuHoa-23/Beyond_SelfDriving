...
class IntegrationProblems(InteractiveScene):
    ...
    def construct(self):
        eq1 = Tex('\\int_0^{2\\pi} \\sin(x) \\cos(x) \\tan(x) \\cot(x) \\sec(x) \\csc(x) \\, dx')
        eq2 = Tex('\\int \\sqrt[3]{x \\sqrt[4]{x \\sqrt[5]{x \\sqrt[6]{x \\sqrt{\\cdots}}}}} \\, dx')
        eq3 = Tex('\\int_0^1 \\left( \\sum_{k=1}^{\\infty} (-1)^k x^{2k} \\right) dx')
