...
def send_terminus_command(command, clear=True, center=True, enter=True):
...
class ManimRunScene(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        sublime.set_timeout(lambda: send_terminus_command(command, enter=enter), timeout)
        if enter:
            sublime.set_timeout(lambda: threading.Thread(target=self.focus_sublime).start(), 1000)
        else:
...
class CommentFold(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        for region in regions:
                ...
                if line.lstrip().startswith('#'):
                    ...
                    if len(line) - len(line.lstrip()) == indent_level and (not last_line_was_comment):
                        if last_comment_line_end:
                            regions_to_fold.append(sublime.Region(last_comment_line_end, last_full_line_end))
            ...
            if last_comment_line_end:
                regions_to_fold.append(sublime.Region(last_comment_line_end, region.end()))
