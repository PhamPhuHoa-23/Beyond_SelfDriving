...
class CountingScene(Scene):
    ...
    def count_mobjects(self, mobjects, mode='highlight', color='red', display_numbers=True, num_offset=DEFAULT_COUNT_NUM_OFFSET, run_time=DEFAULT_COUNT_RUN_TIME):
        if len(mobjects) > 50:
            raise Exception("I don't know if you should be counting                              too many mobjects...")
        if len(mobjects) == 0:
    ...
    def count_regions(self, regions, mode='one_at_a_time', num_offset=DEFAULT_COUNT_NUM_OFFSET, run_time=DEFAULT_COUNT_RUN_TIME, **unused_kwargsn):
...
class GeneralizedPascalsTriangle(VMobject):
    ...
    def init_points(self):
        ...
        for n, k in self.coords:
            ...
            num_mob = self.submob_class(n, k)
            scale_factor = min(1, self.portion_to_fill * self.cell_height / num_mob.get_height(), self.portion_to_fill * self.cell_width / num_mob.get_width())
    ...
    def generate_n_choose_k_mobs(self):
        ...
        for n, k in self.coords:
            nck_mob = OldTex('{%d \\choose %d}' % (n, k))
            scale_factor = min(1, self.portion_to_fill * self.cell_height / nck_mob.get_height(), self.portion_to_fill * self.cell_width / nck_mob.get_width())
