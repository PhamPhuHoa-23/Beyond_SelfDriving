...
class CountingScene(Scene):
    CONFIG = {'digit_place_colors': [YELLOW, MAROON_B, RED, GREEN, BLUE, PURPLE_D], 'counting_dot_starting_position': (FRAME_X_RADIUS - 1) * RIGHT + (FRAME_Y_RADIUS - 1) * UP, 'count_dot_starting_radius': 0.5, 'dot_configuration_height': 2, 'ones_configuration_location': UP + 2 * RIGHT, 'num_scale_factor': 2, 'num_start_location': 2 * DOWN}
    ...
    def get_dot_template(self, place):
        dots = VGroup(*[Dot(point, radius=0.25, fill_opacity=0, stroke_width=2, stroke_color=WHITE) for point in self.get_template_configuration(place)])
    ...
    def increment(self, run_time_per_anim=1):
        moving_dot = Dot(self.counting_dot_starting_position, radius=self.count_dot_starting_radius, color=self.digit_place_colors[0])
        ...
        while continue_rolling_over:
            ...
            if len(self.curr_configurations[place].split()) == self.get_place_max(place):
                ...
                circle = Circle(radius=radius, stroke_width=0, fill_color=self.digit_place_colors[place], fill_opacity=0.5)
    ...
    def get_digit_increment_animations(self):
        ...
        if is_next_digit:
            ...
            arrow = Arrow(new_number_mob[place].get_top(), self.dot_templates[place].get_bottom(), color=self.digit_place_colors[place])
