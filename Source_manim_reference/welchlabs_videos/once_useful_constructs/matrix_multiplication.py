...
class NumericalMatrixMultiplication(Scene):
    ...
    def construct(self):
        left_string_matrix, right_string_matrix = [np.array(matrix).astype('string') for matrix in (self.left_matrix, self.right_matrix)]
    ...
    def get_result_matrix(self, left, right):
        ...
        for a in range(m):
            for b in range(n):
                ...
                parts = [prefix + template % (left[a][c], right[c][b]) for c in range(k) for prefix in ['' if c == 0 else '+']]
                mob_matrix[a][b] = OldTex(parts, next_to_buff=0.1)
        return Matrix(mob_matrix)

    def add_lines(self, left, right):
        line_kwargs = {'color': BLUE, 'stroke_width': 2}
        ...
        for a in range(m):
            for b in range(n):
                ...
                for c in range(k):
                    ...
                    self.play(Transform(start_parts, result_entry.copy().set_color(YELLOW), path_arc=-np.pi / 2, lag_ratio=0), *lagging_anims)
