...
class ComplexTransformationScene(Scene):
    CONFIG = {'plane_config': {}, 'background_fade_factor': 0.5, 'use_multicolored_plane': False, 'vert_start_color': BLUE, 'vert_end_color': BLUE, 'horiz_start_color': BLUE, 'horiz_end_color': BLUE, 'num_anchors_to_add_per_line': 50, 'post_transformation_stroke_width': None, 'default_apply_complex_function_kwargs': {'run_time': 5}, 'background_label_scale_val': 0.5, 'include_coordinate_labels': True}
    ...
    def paint_plane(self, plane):
        for lines in (planes, plane.secondary_lines):
            lines.set_color_by_gradient(self.vert_start_color, self.vert_end_color, self.horiz_start_color, self.horiz_end_color)
