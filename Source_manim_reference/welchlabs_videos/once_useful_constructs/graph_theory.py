...
class CubeGraph(Graph):
    ...
    def construct(self):
        ...
        self.edges = [(0, 1), (0, 2), (3, 1), (3, 2), (4, 5), (4, 6), (7, 5), (7, 6), (0, 4), (1, 5), (2, 6), (3, 7)]
        self.region_cycles = [[0, 2, 3, 1], [4, 0, 1, 5], [4, 6, 2, 0], [6, 7, 3, 2], [7, 5, 1, 3], [4, 6, 7, 5]]

class SampleGraph(Graph):

    def construct(self):
        self.vertices = [(0, 0, 0), (2, 0, 0), (1, 2, 0), (3, 2, 0), (-1, 2, 0), (-2, -2, 0), (2, -2, 0), (4, -1, 0), (6, 2, 0)]
        self.edges = [(0, 1), (1, 2), (1, 3), (3, 2), (2, 4), (4, 0), (2, 0), (4, 5), (0, 5), (1, 5), (5, 6), (6, 7), (7, 1), (7, 8), (8, 3)]
        self.region_cycles = [(0, 1, 2), (1, 3, 2), (2, 4, 0), (4, 5, 0), (0, 5, 1), (1, 5, 6, 7), (1, 7, 8, 3), (4, 5, 6, 7, 8, 3, 2)]

class OctohedronGraph(Graph):

    def construct(self):
        self.vertices = [(r * np.cos(angle), r * np.sin(angle) - 1, 0) for r, s in [(1, 0), (3, 3)] for angle in np.pi / 6 * np.array([s, 4 + s, 8 + s])]
        self.edges = [(0, 1), (1, 2), (2, 0), (5, 0), (0, 3), (3, 5), (3, 1), (3, 4), (1, 4), (4, 2), (4, 5), (5, 2)]
        self.region_cycles = [(0, 1, 2), (0, 5, 3), (3, 1, 0), (3, 4, 1), (1, 4, 2), (2, 4, 5), (5, 0, 2), (3, 4, 5)]
...
class CompleteGraph(Graph):
    ...
    def construct(self):
        self.vertices = [(self.radius * np.cos(theta), self.radius * np.sin(theta), 0) for x in range(self.num_vertices) for theta in [2 * np.pi * x / self.num_vertices]]
...
class DiscreteGraphScene(Scene):
    args_list = [(CubeGraph(),), (SampleGraph(),), (OctohedronGraph(),)]
    ...
    def replace_vertices_with(self, mobject):
        ...
        self.play(*[CounterclockwiseTransform(vertex, mobject.copy().shift(vertex.get_center())) for vertex in self.vertices] + [ApplyMethod(edge.scale, (edge.get_length() - diameter) / edge.get_length()) for edge in self.edges])

    def annotate_edges(self, mobject, fade_in=True, **kwargs):
        angles = list(map(np.arctan, list(map(Line.get_slope, self.edges))))
        self.edge_annotations = [mobject.copy().rotate(angle).move_to(edge.get_center()) for angle, edge in zip(angles, self.edges)]
        if fade_in:
    ...
    def trace_cycle(self, cycle=None, color='yellow', run_time=2.0):
        ...
        next(next_in_cycle)
        self.traced_cycle = Mobject(*[Line(self._points[i], self._points[j]).set_color(color) for i, j in zip(cycle, next_in_cycle)])
        ...
        while len(to_check) > 0:
            ...
            for pair in pairs:
                if pair[0] == curr and pair[1] not in spanned_vertices:
                    ...
                    to_check.add(pair[1])
        self.spanning_tree = Mobject(*[Line(self._points[pair[0]], self._points[pair[1]]).set_color(color) for pair in self.spanning_tree_index_pairs])
    ...
    def generate_treeified_spanning_tree(self):
        ...
        self.treeified_spanning_tree = Mobject(*[Line(new_points[i], new_points[j]).set_color(color) for i, j in self.spanning_tree_index_pairs])
