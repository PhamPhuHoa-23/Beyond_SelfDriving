...
class VectorScene(Scene):
    ...
    def coords_to_vector(self, vector, coords_start=2 * RIGHT + 2 * UP, clean_up=True):
        ...
        self.play(ApplyFunction(lambda y: self.position_y_coordinate(y, y_line, vector), y_coord), FadeOut(array.get_brackets()))
...
class LinearTransformationScene(VectorScene):
    ...
    foreground_plane_kwargs: dict = dict(x_max=FRAME_WIDTH / 2, x_min=-FRAME_WIDTH / 2, y_max=FRAME_WIDTH / 2, y_min=-FRAME_WIDTH / 2, faded_line_ratio=0)
    background_plane_kwargs: dict = dict(color=GREY, axis_config=dict(color=GREY), background_line_style=dict(stroke_color=GREY, stroke_width=1))
    ...
    def get_unit_square(self, color=YELLOW, opacity=0.3, stroke_width=3):
        square = self.square = Rectangle(color=color, width=self.plane.get_x_unit_size(), height=self.plane.get_y_unit_size(), stroke_color=color, stroke_width=stroke_width, fill_color=color, fill_opacity=opacity)
    ...
    def add_transformable_label(self, vector, label, transformation_name='L', new_label=None, **kwargs):
        ...
        if new_label:
            ...
            label_mob.target_text = '%s(%s)' % (transformation_name, label_mob.get_tex())
    ...
    def apply_function(self, function, added_anims=[], **kwargs):
        if 'run_time' not in kwargs:
            kwargs['run_time'] = 3
        anims = [ApplyPointwiseFunction(function, t_mob) for t_mob in self.transformable_mobjects] + [self.get_vector_movement(function), self.get_transformable_label_movement(), self.get_moving_mobject_movement(function)] + [Animation(f_mob) for f_mob in self.foreground_mobjects] + added_anims
