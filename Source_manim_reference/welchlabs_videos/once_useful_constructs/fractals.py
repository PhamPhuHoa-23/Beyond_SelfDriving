...
def fractalification_iteration(vmobject, dimension=1.05, num_inserted_anchors_range=list(range(1, 4))):
    ...
    if num_points > 0:
        ...
        for p1, p2 in zip(original_anchors, original_anchors[1:]):
            ...
            inserted_points = [point + u * offset_len * offset_unit_vect for u, point in zip(it.cycle([-1, 1]), inserted_points)]
            new_anchors += [p1] + inserted_points
        new_anchors.append(original_anchors[-1])
        vmobject.set_points_as_corners(new_anchors)
    vmobject.set_submobjects([fractalification_iteration(submob, dimension, num_inserted_anchors_range) for submob in vmobject.submobjects])
    return vmobject
...
class PiCreatureFractal(VMobject):
    ...
    start_mode = 'hooray'
    height = 6
    colors = [BLUE_D, BLUE_B, MAROON_B, MAROON_D, GREY, YELLOW, RED, GREY_BROWN, RED, RED_E]
    random_seed = 0
    stroke_width = 0
...
class FractalCurve(VMobject):
    ...
    num_submobjects = 20
    monochromatic = False
    order_to_stroke_width_map = {3: 3, 4: 2, 5: 1}
...
class HilbertCurve(SelfSimilarSpaceFillingCurve):
    offsets = [LEFT + DOWN, LEFT + UP, RIGHT + UP, RIGHT + DOWN]
    offset_to_rotation_axis = {str(LEFT + DOWN): RIGHT + UP, str(RIGHT + DOWN): RIGHT + DOWN}

class HilbertCurve3D(SelfSimilarSpaceFillingCurve):
    offsets = [RIGHT + DOWN + IN, LEFT + DOWN + IN, LEFT + DOWN + OUT, RIGHT + DOWN + OUT, RIGHT + UP + OUT, LEFT + UP + OUT, LEFT + UP + IN, RIGHT + UP + IN]
    offset_to_rotation_axis_and_angle = {str(RIGHT + DOWN + IN): (LEFT + UP + OUT, 2 * np.pi / 3), str(LEFT + DOWN + IN): (RIGHT + DOWN + IN, 2 * np.pi / 3), str(LEFT + DOWN + OUT): (RIGHT + DOWN + IN, 2 * np.pi / 3), str(RIGHT + DOWN + OUT): (UP, np.pi), str(RIGHT + UP + OUT): (UP, np.pi), str(LEFT + UP + OUT): (LEFT + DOWN + OUT, 2 * np.pi / 3), str(LEFT + UP + IN): (LEFT + DOWN + OUT, 2 * np.pi / 3), str(RIGHT + UP + IN): (RIGHT + UP + IN, 2 * np.pi / 3)}

    def transform(self, points, offset):
        copy = np.array(points)
        copy = rotate(copy, axis=self.offset_to_rotation_axis_and_angle[str(offset)][0], angle=self.offset_to_rotation_axis_and_angle[str(offset)][1])
        copy /= (self.scale_factor,)
        copy += offset * self.radius * self.radius_scale_factor
        return copy
...
class PeanoCurve(SelfSimilarSpaceFillingCurve):
    colors = [PURPLE, TEAL]
    offsets = [LEFT + DOWN, LEFT, LEFT + UP, UP, ORIGIN, DOWN, RIGHT + DOWN, RIGHT, RIGHT + UP]
    offset_to_rotation_axis = {str(LEFT): UP, str(UP): RIGHT, str(ORIGIN): LEFT + UP, str(DOWN): RIGHT, str(RIGHT): UP}
    scale_factor = 3
    radius_scale_factor = 2.0 / 3

class TriangleFillingCurve(SelfSimilarSpaceFillingCurve):
    colors = [MAROON, YELLOW]
    offsets = [LEFT / 4.0 + DOWN / 6.0, ORIGIN, RIGHT / 4.0 + DOWN / 6.0, UP / 3.0]
    offset_to_rotation_axis = {str(ORIGIN): RIGHT, str(UP / 3.0): UP}
    scale_factor = 2
...
class HexagonFillingCurve(SelfSimilarSpaceFillingCurve):
    start_color = WHITE
    end_color = BLUE_D
    axis_offset_pairs = [(None, 1.5 * DOWN + 0.5 * np.sqrt(3) * LEFT), (UP + np.sqrt(3) * RIGHT, 1.5 * DOWN + 0.5 * np.sqrt(3) * RIGHT), (np.sqrt(3) * UP + RIGHT, ORIGIN), ((UP, RIGHT), np.sqrt(3) * LEFT), (None, 1.5 * UP + 0.5 * np.sqrt(3) * LEFT), (None, 1.5 * UP + 0.5 * np.sqrt(3) * RIGHT), (RIGHT, np.sqrt(3) * RIGHT)]
    scale_factor = 3
...
class KochSnowFlake(LindenmayerCurve):
    ...
    start_step = RIGHT
    angle = np.pi / 3
    order_to_stroke_width_map = {3: 3, 5: 2, 6: 1}
