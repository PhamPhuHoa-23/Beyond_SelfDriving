...
def _cap_icon(symbol, label, color):
    ...
    sym = Text(symbol, font_size=32, color=color).move_to(
        box.get_center() + UP * 0.2)
    lbl = Text(label, font_size=14, color=COL_WHITE).move_to(
        box.get_center() + DOWN * 0.38)
...
class P01S02GenAIBoom(Scene):
    ...
    def construct(self):
        ...
        fm_box = RoundedRectangle(
            corner_radius=0.25, width=5.0, height=1.0,
            fill_color=COL_BLUE, fill_opacity=1,
            stroke_color=COL_WHITE, stroke_width=2,
        ).center()
        fm_lbl = Text("Foundation Models", font_size=28,
        ...
        task_header = Text("Downstream Tasks", font_size=16,
