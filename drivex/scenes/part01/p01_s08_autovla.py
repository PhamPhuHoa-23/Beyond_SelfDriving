...
class P01S08AutoVLA(Scene):
    ...
    def construct(self):
        ...
        diamond = Square(side_length=1.1, fill_color=COL_DEEP_BLUE,
                         ...
                         stroke_width=2).rotate(PI / 4).center().shift(UP * 1.3)
        ...
        divider = Line(LEFT * 5.5, RIGHT * 5.5,
                       ...
                       stroke_opacity=0.3).shift(DOWN * 1.4)
        ...
        ).shift(LEFT * 3.0 + DOWN * 2.5)
        sft_lbl = Text("Stage 1: SFT", font_size=16,
                       color=COL_WHITE, weight=BOLD)
        sft_sub = Text("Teaches both fast & slow modes",
                       font_size=12, color=COL_LIGHT_BLUE)
        ...
        ).shift(RIGHT * 2.0 + DOWN * 2.5)
        rft_lbl = Text("Stage 2: RFT (GRPO)", font_size=16,
                       color=COL_WHITE, weight=BOLD)
        rft_sub = Text("Align with verified rewards",
                       font_size=12, color="#A78BFA")
        VGroup(rft_lbl, rft_sub).arrange(DOWN, buff=0.08).move_to(rft_box)

        self.play(FadeIn(sft_box), Write(sft_lbl),
                  FadeIn(sft_sub), run_time=0.4)
        self.play(GrowArrow(arr_train), run_time=0.3)
        self.play(FadeIn(rft_box), Write(rft_lbl),
                  FadeIn(rft_sub), run_time=0.4)
        ...
        res_lbl1 = Text("10.6% ↑ Planning", font_size=16,
        ...
        res_lbl2 = Text("66.8% ↓ Runtime", font_size=16,
                        color=COL_GREEN, weight=BOLD)
