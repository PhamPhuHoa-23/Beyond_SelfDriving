...
class P04S05MultiTaskConflict(Scene):
    ...
    def construct(self):
        ...
        s_frame = simple_box("Single-frame\n1 agent · 1 task",
                             COL_BLUE, LEFT * 3.5 + UP * 0.8)
        m_frame = simple_box(
            "Multi-frame + Multi-agent\n+ Multi-task", COL_RED,  RIGHT * 2.0 + UP * 0.8)
        ...
        complex_lbl = Text("Many dimensions at once",
                           font_size=14, color=COL_RED, slant=ITALIC)
        ...
        # Conflict pulse
        self.play(model_circ.animate.set_stroke(
            color=COL_RED, width=5), run_time=0.25)
        self.play(model_circ.animate.set_stroke(
            color=COL_BLUE, width=2), run_time=0.25)
