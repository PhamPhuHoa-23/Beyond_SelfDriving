...
class P04S09EfficiencySummary(Scene):
    ...
    def construct(self):
        ...
        card_row = VGroup(*card_objs).arrange(RIGHT, buff=0.45).move_to(DOWN * 0.3)
        con_arrows = []
        ...
        # Pulse all cards
        self.play(*[card[0].animate.set_stroke(width=4) for card in card_objs], run_time=0.3)
        self.play(*[card[0].animate.set_stroke(width=2) for card in card_objs], run_time=0.3)
        self.wait(1.5)
