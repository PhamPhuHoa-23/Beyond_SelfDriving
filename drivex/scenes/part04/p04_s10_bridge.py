...
def _small_human(color=WHITE):
    ...
    head = Circle(radius=0.12, fill_color=color,
...
class P04S10Bridge(Scene):
    ...
    def construct(self):
        ...
        checks = [
            Text("✓  Data Efficiency",       font_size=22,
                 color=INT8_GREEN, weight=BOLD),
            Text("✓  Training Efficiency",   font_size=22,
                 color=INT8_GREEN, weight=BOLD),
            Text("✓  Inference Efficiency",  font_size=22,
                 color=INT8_GREEN, weight=BOLD),
        ]
        check_group = VGroup(*checks).arrange(DOWN,
                                              buff=0.45, aligned_edge=LEFT)
        check_group.move_to(LEFT * 2.5 + UP * 0.4)

        for ck in check_group:
            self.play(Write(ck), run_time=0.35)

        infra_lbl = Text("Cars + Infrastructure",
                         font_size=20, color=COL_LIGHT_BLUE)
        infra_lbl.next_to(check_group[-1], DOWN,
                          buff=0.5).align_to(check_group[0], LEFT)
        self.play(Write(infra_lbl), run_time=0.3)
        self.wait(0.5)

        # ── Section B: expanding agent types ──────────────────────
        sec_title = Text("But who else is on the road?",
        ...
        wc_group = VGroup(wc_circ, wc_seat, wc_lbl).move_to(
            RIGHT * 1.5 + DOWN * 1.0)
        ...
        sc_w2 = Circle(radius=0.1, fill_color=COL_GRAY_FILL, fill_opacity=1,
                       stroke_color=COL_WHITE, stroke_width=1)
        sc_w1.next_to(sc_body, DOWN, buff=0.0).align_to(
            sc_body, LEFT).shift(RIGHT * 0.05)
        sc_w2.next_to(sc_body, DOWN, buff=0.0).align_to(
            sc_body, RIGHT).shift(LEFT * 0.05)
        sc_lbl = Text("Scooter", font_size=12, color=COL_WHITE)
        sc_lbl.next_to(sc_body, DOWN, buff=0.18)
        sc_group = VGroup(sc_body, sc_w1, sc_w2, sc_lbl).move_to(
            RIGHT * 3.2 + DOWN * 1.4)
        ...
        positions = [UL * 0.8, UR * 0.8, DL * 0.5,
                     DR * 0.5, UP * 1.5, DOWN * 1.8 + LEFT * 0.5]
        humans = VGroup(*[_small_human(COL_WHITE).scale(0.65).move_to(p)
                          for p in positions])
        self.play(LaggedStart(*[FadeIn(h)
                  for h in humans], lag_ratio=0.1), run_time=0.7)
        ...
        self.play(FadeIn(VGroup(sub_lbl, teaser),
                  shift=UP * 0.2), run_time=0.4)
        self.wait(2.0)
        self.play(FadeOut(Group(*self.mobjects)), run_time=0.4)
