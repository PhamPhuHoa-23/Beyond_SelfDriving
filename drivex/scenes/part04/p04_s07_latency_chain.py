...
class P04S07LatencyChain(Scene):
    ...
    def construct(self):
        ...
        self.play(
            Transform(fp32_mul.copy().scale(
                [1, 0.01, 1]).align_to(ax_line2, DOWN), fp32_mul),
            run_time=0.4, rate_func=smooth
        )
        self.play(FadeIn(fp32_mul), FadeIn(ml_t), run_time=0.1)
        self.play(
            Transform(fp32_add.copy().scale(
                [1, 0.01, 1]).align_to(ax_line2, DOWN), fp32_add),
        ...
        sram_bar.align_to(dram_bar, LEFT).move_to(
            dram_bar.get_center() + DOWN * 0.6)

        # Align left
        dram_bar.align_to(RIGHT * (-0.3), LEFT)
        sram_bar.align_to(RIGHT * (-0.3), LEFT)
        dram_bar.move_to(
            np.array([-0.3 + dram_bar.width / 2, bar_baseline[1] + 0.9, 0]))
        sram_bar.move_to(
            np.array([-0.3 + sram_bar.width / 2, bar_baseline[1] + 0.3, 0]))

        dram_lbl = Text("DRAM: 640 pJ / access", font_size=12, color=COL_RED)
        sram_lbl = Text("SRAM: 5 pJ / access",
                        font_size=12, color=COL_INT8_GREEN)
        diff_lbl = Text("128× more expensive", font_size=13,
