...
class P03S03SmartIntersection(Scene):
    ...
    def construct(self):
        ...
        # CAVs
        cav1 = Dot(radius=0.18, color=COL_BLUE).move_to(LEFT * 1.5 + DOWN * 0.2)
        ...
        live_lbl   = Text("LIVE INTERSECTION", font_size=12, color=COL_WHITE, weight=BOLD)
        live_badge.to_corner(UR, buff=0.3)
        live_lbl.move_to(live_badge.get_center())

        map_group = VGroup(roads, lane_marks, nw_node, se_node, lbl_nw, lbl_se, cav1, cav2)

        self.play(FadeIn(map_group), run_time=0.6)
        self.play(FadeIn(VGroup(live_badge, live_lbl)), FadeIn(loc_lbl), run_time=0.3)
        self.wait(0.8)

        # ═══ SECTION B: Sensor Table ══════════════════════════════
        self.play(map_group.animate.scale(0.65).to_edge(LEFT, buff=0.4), run_time=0.4)
        ...
        def make_col(header_txt, color, items):
            ...
            rows = VGroup(*[Text(item, font_size=13, color=COL_WHITE) for item in items])
        ...
        table   = VGroup(col_inf, col_cav).arrange(RIGHT, buff=1.0, aligned_edge=UP)
        ...
        def sensor_block(icon_txt, fail_label, fail_color, x):
            ...
            s_name = Text(fail_label, font_size=13, color=fail_color, slant=ITALIC)
            grp    = VGroup(s_icon, s_name).arrange(DOWN, buff=0.15)
            grp.move_to([x, 0.5, 0])
            return grp

        cam  = sensor_block("📷", "Fails in backlight / darkness", COL_RED,  -4.0)
        lid  = sensor_block("🔊", "Degrades in heavy rain",         COL_RED,  0.0)
        gps  = sensor_block("📡", "Lost accuracy between buildings",COL_RED,  4.0)
        ...
        pi_bubble  = PIBubble(pi,  "Tại sao nhiều sensor vậy?\nKhông phải overkill à?", position=UR)
        ...
        tagline = Text("Redundancy = Reliability", font_size=22, color=COL_GOLD, weight=BOLD)
