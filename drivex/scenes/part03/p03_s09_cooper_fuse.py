...
class P03S09CooperFuse(Scene):
    ...
    def construct(self):
        ...
        heading = Text("CooperFuse — Late Fusion", font_size=26, color=COL_GOLD, weight=BOLD)
        heading.to_edge(UP, buff=0.4)
        self.play(FadeIn(heading), run_time=0.3)

        divider = Line(UP * 2.5, DOWN * 2.5, color=COL_WHITE, stroke_opacity=0.3)
        self.play(Create(divider), run_time=0.2)

        # ═══════ LEFT: NMS ══════════════════════════════════════════
        nms_title = Text("NMS (baseline)", font_size=15, color=COL_RED, weight=BOLD)
        ...
        self.play(nms_bbox2.animate.set_stroke(opacity=0.25),
                  conf2.animate.set_opacity(0.25), run_time=0.3)   # NMS suppresses it
        self.play(FadeIn(cross_note), run_time=0.3)

        # ═══════ RIGHT: CooperFuse ════════════════════════════════
        cf_title = Text("CooperFuse", font_size=15, color=COL_GREEN, weight=BOLD)
        ...
        paper_txt   = Text("CooperFuse  ·  IV 2024", font_size=12, color=COL_WHITE)
