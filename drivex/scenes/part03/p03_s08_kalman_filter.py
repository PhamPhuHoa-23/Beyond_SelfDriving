    ...
    def construct(self):
        ...
        inputs = [
            ("GNSS",          "5 Hz",   "loses fix indoors /\nbetween buildings",
             COL_RED,          UP * 1.8),
            ("IMU / Wheel",   "100 Hz", "drifts over time",
             COL_BLUE,         ORIGIN),
            ("LiDAR map-match", "1 Hz",  "heavy compute",
             COL_INFRA_ORANGE, DOWN * 1.8),
        ]
        ...
        for name, freq, limit, color, y_pos in inputs:
            ...
            limit_txt = Text(limit, font_size=10,
                             color=COL_WHITE, slant=ITALIC)
            limit_txt.next_to(freq_badge, DOWN, buff=0.08)

            path = Line(freq_badge.get_right(), dst_pos,
        ...
        out_lbl = Text("100 Hz\nLane-Level Output",
                       font_size=13, color=COL_GREEN, weight=BOLD)
        ...
        self.play(MoveAlongPath(out_dot, out_arr),
