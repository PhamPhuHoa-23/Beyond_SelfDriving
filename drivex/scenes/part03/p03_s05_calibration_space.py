...
class P03S05CalibrationSpace(Scene):
    ...
    def construct(self):
        ...
        heading = Text("Space Calibration", font_size=28,
        ...
        for name, color, pos in frames_info:
            ax_x = Arrow(ORIGIN, RIGHT * 0.5, buff=0, color=color,
                         stroke_width=2, tip_length=0.12)
            ax_y = Arrow(ORIGIN, UP * 0.5, buff=0, color=color,
                         stroke_width=2, tip_length=0.12)
        ...
        misalign_title = Text("Without calibration",
                              font_size=16, color=COL_RED)
        misalign_title.move_to(DOWN * 0.1)
        self.play(FadeIn(misalign_title), run_time=0.3)

        # True object (green)
        obj_true = Circle(radius=0.25, fill_color=COL_GREEN,
        ...
        obj_ghost = Circle(radius=0.25, fill_color=COL_RED,
        ...
        badge = RoundedRectangle(width=3.8, height=0.55, corner_radius=0.12,
                                 ...
                                 stroke_width=1.5)
        badge_txt = Text("PJLab SensorsCalibration  (open-source)",
