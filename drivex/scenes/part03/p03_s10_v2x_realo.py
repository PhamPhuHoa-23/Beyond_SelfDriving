...
class P03S10V2XReaLO(Scene):
    ...
    def construct(self):
        ...
        bev_lbl = Text("BEV Feature\n8×8 grid",
                       font_size=12, color=COL_SENSOR_CYAN)
        ...
        self.play(Create(comp_arr), FadeIn(comp_txt),
                  FadeIn(size_txt), run_time=0.4)
        self.wait(0.4)

        # ═══ SECTION B: Balance Scale ════════════════════════════
        pivot = Dot(radius=0.07, color=COL_WHITE).move_to(
            RIGHT * 3.5 + DOWN * 0.3)
        ...
        self.play(FadeIn(VGroup(pivot, beam, pan_l,
                  pan_r, lbl_acc, lbl_lat)), run_time=0.4)
        ...
        wp = Dot(radius=0.09, color=COL_GOLD).move_to(
            pivot.get_center() + RIGHT * 0.1)
