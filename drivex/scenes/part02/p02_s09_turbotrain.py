...
class P02S09TurboTrain(Scene):
    ...
    def construct(self):
        ...
        y_lbl = Text("EPA  (Prediction)", font_size=13,
                     color=COL_WHITE).rotate(PI / 2)
        ...
        import random
        random.seed(42)
        orange_pts = [(random.uniform(0.05, 0.35),
                       random.uniform(0.05, 0.35)) for _ in range(8)]
        orange_dots = VGroup(*[
            Dot(axes.c2p(x, y), radius=0.08, color=COL_GOLD, fill_opacity=0.85)
            for x, y in orange_pts
        ])
        orange_dots.set_color(
            COL_GOLD).set_color_by_gradient(COL_GOLD, COL_RED)
        # override to orange
        for d in orange_dots:
            d.set_color("#E88C28")

        # blue dots — manual 4-stage (mid-range)
        blue_pts = [(0.45, 0.42), (0.50, 0.47), (0.55, 0.52),
                    (0.58, 0.50), (0.52, 0.55)]
        blue_dots = VGroup(*[
        ...
        leg_gold = VGroup(
            Star(n=5, outer_radius=0.12, inner_radius=0.05,
                 color=COL_GOLD, fill_opacity=1),
            Text("TurboTrain (~45 ep)", font_size=13,
                 color=COL_GOLD, weight=BOLD)
        ).arrange(RIGHT, buff=0.12)
        legend = VGroup(leg_orange, leg_blue, leg_gold).arrange(
        ...
        pt_header = Text("TurboTrain", font_size=26,
                         color=COL_GOLD, weight=BOLD)
        ...
        s2_lbl = Text("Stage 2: Balance", font_size=16,
                      color=COL_GREEN, weight=BOLD)
        s2_sub = Text("Hybrid gradient strategy",
                      font_size=13, color=COL_WHITE)
        VGroup(s2_lbl, s2_sub).arrange(
            DOWN, buff=0.1).move_to(s2_box.get_center())
        ...
        sub_b = Text("Conflict-suppressing steps",
                     font_size=12, color=COL_GREEN)
        sub_labels = VGroup(sub_a, sub_b).arrange(
            DOWN, buff=0.1, aligned_edge=LEFT)
        sub_labels.next_to(axes, DOWN, buff=0.3)

        # Single pair of gradient arrows (simplified)
        ...
        self.play(FadeIn(sub_labels), run_time=0.3)
        ...
        result_box = RoundedRectangle(width=4.0, height=1.2, corner_radius=0.12,
                                      fill_color="#1B4332", fill_opacity=1,
                                      stroke_color=COL_GREEN, stroke_width=2)
        result_box.next_to(s2_box, DOWN, buff=0.6)
        r1 = Text("45 ep  vs.  120 ep  (manual)",
                  font_size=16, color=COL_GREEN, weight=BOLD)
