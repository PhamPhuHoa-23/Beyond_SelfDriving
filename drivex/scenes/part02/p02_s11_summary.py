...
class P02S11Summary(Scene):
    ...
    def _pipeline_icon(self):
        ...
        b1 = Rectangle(width=0.5, height=0.35, fill_color="#1E3A5F", fill_opacity=1, stroke_width=0)
        b2 = Rectangle(width=0.5, height=0.35, fill_color="#1B4332", fill_opacity=1, stroke_width=0)
    ...
    def construct(self):
        ...
        header = Text("Three Problems — Three Solutions", font_size=26, color=COL_GOLD, weight=BOLD)
        header.to_edge(UP, buff=0.4)
        self.play(FadeIn(header), run_time=0.4)

        icons = [self._agent_icon(), self._pipeline_icon(), self._riskmap_icon()]
        card_titles = ["V2XPnP",    "TurboTrain",                           "RiskMap"]
        card_bodies  = [
        ...
        connecting_arrows = VGroup(
            Arrow(cards[0].get_right(), cards[1].get_left(), color=COL_BLUE, buff=0.05),
            Arrow(cards[1].get_right(), cards[2].get_left(), color=COL_BLUE, buff=0.05),
        )
