...
class P02S07V2XPnP(Scene):
    ...
    # ─────────────────────── sub-scene A ───────────────────────────
    def _sub_a(self):
        header = Text("Q1: What to Transmit?", font_size=24,
    ...
    # ─────────────────────── sub-scene B ───────────────────────────
    def _sub_b(self):
        header = Text("Q2: When to Transmit?", font_size=24,
                      color=COL_GOLD, weight=BOLD)
        header.to_edge(UP, buff=0.4)
        self.play(FadeIn(header), run_time=0.4)
        ...
        win_lbl = Text("Only chance to communicate",
                       font_size=14, color=COL_GREEN)
    ...
    # ─────────────────────── sub-scene C ───────────────────────────
    def _sub_c(self):
        header = Text("Q3: How to Fuse?", font_size=24,
                      color=COL_GOLD, weight=BOLD)
        header.to_edge(UP, buff=0.4)
        self.play(FadeIn(header), run_time=0.4)
        ...
        self.play(GrowArrow(arr_to_ta), FadeIn(
            VGroup(ta_box, ta_lbl)), run_time=0.5)

        # Per-agent temporal feature
        feat_box = Rectangle(width=1.8, height=1.0,
                             fill_color="#1E3A5F", fill_opacity=1,
                             stroke_color=COL_BLUE, stroke_width=1.5)
        feat_lbl = Text("Temporal\nfeature", font_size=13,
        ...
        arr_ta_feat = Arrow(ta_box.get_right(), feat_box.get_left(),
                            color=COL_BLUE, buff=0.1)
        self.play(GrowArrow(arr_ta_feat), FadeIn(
            VGroup(feat_box, feat_lbl)), run_time=0.5)
        ...
        self.play(FadeIn(VGroup(feat_box2, lbl2)),
                  GrowArrow(arr2), run_time=0.5)

        # Spatial Attention — moved right for clear separation
        sa_box = RoundedRectangle(width=2.8, height=1.5, corner_radius=0.15,
                                  fill_color="#1B4332", fill_opacity=1,
                                  stroke_color=COL_GREEN, stroke_width=2)
        sa_lbl = Text("Spatial\nAttention", font_size=16, color=COL_WHITE)
        sa_box.move_to(RIGHT * 3.2 + y_center + UP * 0.2)
        sa_lbl.move_to(sa_box.get_center())
        arr_feat_sa = Arrow(feat_box.get_right(), sa_box.get_left(),
                            color=COL_GREEN, buff=0.1)
        self.play(GrowArrow(arr_feat_sa), FadeIn(
            VGroup(sa_box, sa_lbl)), run_time=0.5)

        # Output representation to the right of sa_box
        out_box = RoundedRectangle(width=3.0, height=1.0, corner_radius=0.15,
                                   fill_color=COL_GOLD, fill_opacity=0.95, stroke_width=0)
        out_lbl = Text("Multi-Agent Spatio-\nTemporal Representation",
                       font_size=12, color=BG_DARK, weight=BOLD)
        out_box.move_to(RIGHT * 5.6 + y_center + UP * 0.2)
        out_lbl.move_to(out_box.get_center())
        arr_sa_out = Arrow(sa_box.get_right(),
                           out_box.get_left(), color=COL_GOLD, buff=0.1)
        self.play(GrowArrow(arr_sa_out), FadeIn(
            VGroup(out_box, out_lbl)), run_time=0.5)

        # Fork to detection + prediction (vertically below out_box)
        det_box = RoundedRectangle(width=2.0, height=0.7, corner_radius=0.1,
                                   fill_color="#1E3A5F", fill_opacity=1,
                                   stroke_color=COL_BLUE, stroke_width=1.5)
        pred_box = RoundedRectangle(width=2.0, height=0.7, corner_radius=0.1,
                                    fill_color="#1B4332", fill_opacity=1,
                                    stroke_color=COL_GREEN, stroke_width=1.5)
        det_lbl = Text("Detection",  font_size=13, color=COL_WHITE)
        pred_lbl = Text("Prediction", font_size=13, color=COL_WHITE)
        det_box.move_to(RIGHT * 4.5 + y_center + DOWN * 1.5)
        pred_box.move_to(RIGHT * 6.5 + y_center + DOWN * 1.5)
        det_lbl.move_to(det_box.get_center())
        pred_lbl.move_to(pred_box.get_center())

        arr_det = Arrow(out_box.get_bottom(), det_box.get_top(),
                        color=COL_WHITE, buff=0.08)
        arr_pred = Arrow(out_box.get_bottom(),
                         pred_box.get_top(), color=COL_WHITE, buff=0.08)
        self.play(GrowArrow(arr_det),  FadeIn(
            VGroup(det_box, det_lbl)),  run_time=0.4)
        self.play(GrowArrow(arr_pred), FadeIn(
            VGroup(pred_box, pred_lbl)), run_time=0.4)
        self.wait(1.5)
