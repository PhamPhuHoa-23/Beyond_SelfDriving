...
class P02S02Background(Scene):
    ...
    def construct(self):
        ...
        stat_lbl = Text("deaths per year (road accidents)",
                        font_size=20, color=COL_WHITE)
        VGroup(stat_num, stat_lbl).arrange(
            DOWN, buff=0.2).next_to(crowd, DOWN, buff=0.5)
        ...
        lbl_94 = Text("94% — Human Error", font_size=22,
        ...
        waymo_big = Text("80% reduction", font_size=32,
                         color=COL_GREEN, weight=BOLD)
        waymo_sub = Text(
            "injury accidents vs. human drivers\n(Waymo, 2024)", font_size=14, color=COL_WHITE)
        ...
        lbl_a = Text("Amazon Sidewalk Robot",
                     font_size=14, color=COL_LIGHT_BLUE)
        ...
        arrow_ab = Arrow(robot_a.get_right(),
                         robot_b.get_left(), color=COL_BLUE, buff=0.1)
        arrow_bc = Arrow(robot_b.get_right(),
                         robot_c.get_left(), color=COL_BLUE, buff=0.1)
        ...
        self.play(FadeIn(VGroup(robot_a, lbl_a, robot_b,
                  lbl_b, robot_c, lbl_c)), run_time=0.6)
