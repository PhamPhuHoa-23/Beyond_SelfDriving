...
class P02S08Dataset(Scene):
    ...
    def construct(self):
        ...
        # road lines
        roads = VGroup(
            Line(LEFT * 2.5 + DOWN * 0.8, RIGHT * 0 + DOWN * 0.8,    color=COL_WHITE, stroke_opacity=0.4),
            Line(LEFT * 1.0 + UP * 1.2,   LEFT * 1.0 + DOWN * 2.8,   color=COL_WHITE, stroke_opacity=0.4),
        )
        ...
        i1 = Square(side_length=0.3, fill_color=COL_GOLD, fill_opacity=1, stroke_width=0).move_to(LEFT * 2.8 + UP * 0.1)
        i2 = Square(side_length=0.3, fill_color=COL_GOLD, fill_opacity=1, stroke_width=0).move_to(LEFT * 4.2 + DOWN * 1.8)
        ...
        # V2I links
        link_v2i1 = DashedLine(v1.get_center(), i1.get_center(), color=COL_GREEN,  dash_length=0.1)
        link_v2i2 = DashedLine(v2.get_center(), i2.get_center(), color=COL_GREEN,  dash_length=0.1)
        # I2I link
        link_i2i  = DashedLine(i1.get_center(), i2.get_center(), color=COL_GOLD,   dash_length=0.1)
        ...
        stats_title = Text("Dataset Stats", font_size=16, color=COL_LIGHT_BLUE, weight=BOLD)
        stats_title.next_to(stats_box, UP, buff=0).align_to(stats_box, UP).shift(DOWN * 0.35)
        ...
        self.play(AnimationGroup(*[FadeIn(s) for s in stat_items], lag_ratio=0.2), run_time=0.8)
