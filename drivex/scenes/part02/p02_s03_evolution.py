...
class P02S03Evolution(Scene):
    ...
    def construct(self):
        ...
        # ── header ────────────────────────────────────────────────
        header = Text("Single-Agent Evolution", font_size=28,
        ...
        for i, (name, venue, desc) in enumerate(self.MILESTONES):
            ...
            label_name = Text(name,  font_size=14,
            ...
            lbl_grp = VGroup(label_name, label_venue,
                             label_desc).arrange(DOWN, buff=0.07)
        ...
        self.play(AnimationGroup(*[FadeIn(n)
                  for n in nodes], lag_ratio=0.2), run_time=1.4)
        ...
        tick_l = Line([brace_modular_x_left,  y0 - 0.85, 0],
                      [brace_modular_x_left,  y0 - 1.0, 0], color=COL_RED, stroke_width=3)
        tick_r = Line([brace_modular_x_right, y0 - 0.85, 0],
                      [brace_modular_x_right, y0 - 1.0, 0], color=COL_RED, stroke_width=3)
        brace_mod_lbl = Text("Modular Era", font_size=14, color=COL_RED)
        brace_mod_lbl.move_to(
            [(brace_modular_x_left + brace_modular_x_right) / 2, y0 - 1.3, 0])
        ...
        tick_el = Line([brace_e2e_x_left,  y0 - 0.85, 0],
                       [brace_e2e_x_left,  y0 - 1.0, 0], color=COL_GREEN, stroke_width=3)
        tick_er = Line([brace_e2e_x_right, y0 - 0.85, 0],
                       [brace_e2e_x_right, y0 - 1.0, 0], color=COL_GREEN, stroke_width=3)
        brace_e2e_lbl = Text("End-to-End Era", font_size=14, color=COL_GREEN)
        brace_e2e_lbl.move_to(
            [(brace_e2e_x_left + brace_e2e_x_right) / 2, y0 - 1.3, 0])
        ...
        adv_title = Text("E2E Advantages", font_size=16,
        ...
        adv_box.move_to(ORIGIN + DOWN * 0.4)
        adv_content.move_to(adv_box.get_center())
        adv_full = VGroup(adv_box, adv_content)

        # Fade timeline before showing adv box
        self.play(
            FadeOut(VGroup(nodes, spine, modular_group, e2e_group)), run_time=0.3)
