...
class P02S10RiskMap(Scene):
    ...
    def construct(self):
        ...
        header = Text("Three Planning Paradigms", font_size=24, color=COL_GOLD, weight=BOLD)
        ...
        rm_y = -1.6
        b_sensors = _box("Multi-Agent\nSensors", "#1E3A5F", COL_BLUE,   -4.2, rm_y)
        b_riskmap = _box("Risk Map\n(spatiotemporal)", "#3D1A00", COL_GOLD, -1.2, rm_y)
        b_mpc     = _box("Learning-based\nMPC", "#1B4332", COL_GREEN,    1.8, rm_y)
        b_traj    = _box("Safe Trajectory", COL_GREEN, COL_GREEN,        4.5, rm_y)

        arr_sr = Arrow(b_sensors.get_right(), b_riskmap.get_left(), color=COL_BLUE,  buff=0.05)
        arr_rm = Arrow(b_riskmap.get_right(), b_mpc.get_left(),     color=COL_GOLD,  buff=0.05)
        arr_mt = Arrow(b_mpc.get_right(),     b_traj.get_left(),    color=COL_GREEN, buff=0.05)
        ...
        # Annotations
        ann1 = Text("\"Where dangerous?\"", font_size=12, color=COL_LIGHT_BLUE, slant=ITALIC)
        ann2 = Text("\"What path?\"",        font_size=12, color=COL_GREEN,      slant=ITALIC)
        ...
        self.play(GrowArrow(arr_mt), FadeIn(b_traj), FadeIn(badge_c), run_time=0.4)
