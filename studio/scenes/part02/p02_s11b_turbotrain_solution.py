...
def _surface_point(x: float, y: float, lift: float = 0.05) -> np.ndarray:
    return np.array([x, y, _loss(x, y) + lift])
...
class P02S11BTurboTrainSolution(Studio3DScene):
    PART_NUM = 2
    SCENE_TITLE = "TurboTrain Solution"
    ...
    def construct(self):
        ...
        # Audit reference: Welch/3B1B backprop loss landscapes with ParametricSurface.
        ...
        bad_xy = [
            (-2.0, 1.25), (-1.42, 0.45), (-1.78, -0.25), (-1.05, 0.15),
            (-0.78, -0.7), (-0.28, -0.18),
        ]
